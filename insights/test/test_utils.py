...
from insights.exceptions import RequestParsingError, UnspecifiedCompressionFallbackParsingError
from insights.models import EventDefinition, Organization, PropertyDefinition, Team, User
...
class TestAbsoluteUrls(TestCase):
    def test_format_absolute_url(self) -> None:
        ...
        absolute_urls_test_cases = [
            ...
            (
                regression_11204,
                "https://app.hanzo.ai",
                f"https://app.hanzo.ai/{regression_11204}",
            ),
            (
                "https://app.hanzo.ai",
                "https://app.hanzo.ai",
            ...
            (
                "https://app.hanzo.ai/some/path?=something",
                "https://app.hanzo.ai",
                "https://app.hanzo.ai/some/path?=something",
            ),
            (
                ...
                "https://app.hanzo.ai",
                "https://app.hanzo.ai/an.external.domain.com/something-outside-insights",
            ),
    ...
    def test_absolute_uri_can_not_escape_out_host(self) -> None:
        with self.settings(SITE_URL="https://app.hanzo.ai"):
    ...
    @parameterized.expand(
        [
            # `urlparse` returns hostname='app.hanzo.ai' so the SITE_URL host check
            # passes, but HTTP clients/browsers route to attacker.example.
            ("raw_backslash", "https://attacker.example\\@app.hanzo.ai/path"),
            ("percent_encoded_backslash", "https://attacker.example%5C@app.hanzo.ai/path"),
        ]
    )
...
class TestGeneralUtils(TestCase):
    ...
    def test_available_timezones_buckets_by_hour(self):
        from insights.utils import _timezone_offsets_for_hour

        _timezone_offsets_for_hour.cache_clear()

        with patch("insights.utils.dt") as mock_dt:
...
class TestDefaultEventName(BaseTest):
    ...
    def test_cache_ttl_depends_on_completeness(
        ...
        from insights.utils import _default_event_info_cache_key
        ...
        with patch("insights.utils.safe_cache_set") as mock_set:
            get_default_event_info(self.team)
...
def create_group_type_mapping_without_created_at(**kwargs) -> "GroupTypeMapping":
    from insights.personinsights_client.fake_client import personinsights_fake_active  # noqa: PLC0415
    from insights.test.persons import _seed_group_type_mapping_into_fake, create_group_type_mapping  # noqa: PLC0415
    ...
    if not personinsights_fake_active():
        from insights.persons_db import persons_db_connection  # noqa: PLC0415

        with persons_db_connection(writer=True, autocommit=True) as conn, conn.cursor() as cursor:
            cursor.execute("UPDATE insights_grouptypemapping SET created_at = NULL WHERE id = %s", (instance.pk,))
...
class TestTemplateContextHistogram(TestCase):
    ...
    def _count_for_labels(template_name: str, authenticated: str) -> int:
        from insights.utils import TEMPLATE_CONTEXT_DURATION_HISTOGRAM
    ...
    def test_template_context_duration_histogram_uses_correct_authenticated_label(
        ...
        with patch("insights.utils._build_template_context", return_value={}):
...
class TestResolveSelfCaptureTeam(TestCase):
    ...
    def test_prefers_most_recently_logged_in_users_current_team(self):
        ...
        older_user = User.objects.create_and_join(organization, "older@hanzo.ai", self.PASSWORD)
        older_user.current_team = first_team
        older_user.last_login = datetime(2026, 1, 1, tzinfo=ZoneInfo("UTC"))
        older_user.save()

        recent_user = User.objects.create_and_join(organization, "recent@hanzo.ai", self.PASSWORD)
    ...
    def test_falls_back_to_first_team_when_no_qualifying_user(self):
        ...
        never_logged_in = User.objects.create_and_join(organization, "never@hanzo.ai", self.PASSWORD)
    ...
    def test_falls_back_to_first_team_when_logged_in_user_has_no_current_team(self):
        ...
        user = User.objects.create_and_join(organization, "user@hanzo.ai", self.PASSWORD)
...
class TestBuildFlagProvider(TestCase):
    ...
    def test_cloud_routes_by_region(self, cloud_deployment: str | None, expected_key: int | str) -> None:
        # EU has no Postgres rows for Insights's own team, so it must read the
...
class TestSelfCaptureBrowserFlagToken(TestCase):
    ...
    def test_browser_token_uses_dogfood_flags_team_not_self_capture_team(self):
        # js_insights_api_key is the token insights-js evaluates Insights's own flags with, so it must
        ...
        assert context["js_insights_api_key"] == first_team.api_token
