...
describe('traceloop middleware', () => {
    ...
    describe('process', () => {
        ...
        it('promotes insights_-prefixed association properties as custom properties', () => {
            const event = createEvent('$ai_generation', {
                'llm.request.type': 'chat',
                'traceloop.association.properties.insights_env': 'prod',
            })
            traceloop.process(event, () => mapOtelAttributes(event))

            expect(event.properties!['env']).toBe('prod')
            expect(event.properties!['traceloop.association.properties.insights_env']).toBeUndefined()
        })
