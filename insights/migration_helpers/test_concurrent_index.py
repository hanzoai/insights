...
def _apply(op):
    ...
    try:
        op.database_forwards("insights", schema_editor, from_state=None, to_state=None)
    finally:
...
def test_safe_add_index_tracks_state_without_separate_database_and_state():
    ...
    op.state_forwards("insights", new_state)

    assert new_state.models["insights", MODEL_NAME_LOWER].get_index_by_name("state_tracking_idx") == index
...
def test_safe_remove_index_concurrently_removes_index(temp_model):
    ...
    add.state_forwards("insights", state)
