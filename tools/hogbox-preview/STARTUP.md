...
- `django.setup()` is **~6s warm** (exec into a running container) but the first
  ...
  first-touch faulting of the python runtime + site-packages + insights code.
...
- `start-docker` is 75s because containerd cold-reads its content store on start
  — the baked 8.7 GB insights image + 6 dep images.
...
- **Web: `NGINX_UNIT_APP_PROCESSES=1` + `NGINX_UNIT_PRELOAD_CONFIG=true` → first
  ...
  **4** Unit workers and, when `PRELOAD_CONFIG` is unset, **double-loads** Django
  ...
  (hogland `scripts/insights-preview-setup.sh`).
- **Tighter poll intervals** (`run_long`/`wait_http_ok` 10s → 3s): each completed
  ...
  steps adds up.
- **Don't bother combining migrate + migrate_datastore** — measured ~260s
...
- restore **resumes a serving Insights** — no containerd cold-start, no cold
...
**Validated 2026-06-15** (alias `insights-preview-golden-warm`, `snap-e82258df6119`,
...
cold. The recipe is now the default in hogland `scripts/insights-preview-setup.sh`
...
- **Don't hardcode container names** (`insights-db-1`) — exec via the compose
...
CLI (Tailscale identity, no token), drives every phase over SSH-as-`script`, and
