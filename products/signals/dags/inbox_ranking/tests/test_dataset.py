...
@pytest.mark.parametrize(
    ...
    [
        ("US", "", True),
        ("US", "insights-inbox-ranking", False),
        (None, "", False),
    ],
)
...
def test_utc_bound_carries_an_explicit_offset():
    # InsightsQL parses bare datetime strings in the querying team's timezone (US/Pacific for the
...
def test_alias_spellings_never_overwrite_the_canonical_rows_aggregates(alias_first):
    # Datastore groups on the raw client-supplied id, so a forged uppercase or unhyphenated
