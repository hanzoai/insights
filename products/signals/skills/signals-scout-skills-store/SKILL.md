...
description: >
  Skill-hygiene scout for the team's Insights skills store, read entirely via the MCP skill tools.
...
The team's Insights skills store holds the shared agent skills their coding and analytics agents load on demand — a badly-authored skill silently degrades every agent run that loads it.
...
| No secrets             | no credential shapes in any textual field — description, compatibility, metadata, body, or bundled files — `phx_` / `phs_` (Insights personal / project-secret keys) / `sk-` / `ghp_` / `AKIA…` / `-----BEGIN … PRIVATE KEY` / hardcoded bearer tokens. |
...
- **Canonical seeded scout skills** — rows with `category: "scout"` that the team hasn't edited are Insights-shipped content; flagging them to the team is noise.
