...
from products.web_analytics.backend.insightsql_queries.stats_table_pre_aggregated import (
    WEB_ANALYTICS_STATS_TABLE_PRE_AGGREGATED_SUPPORTED_BREAKDOWNS,
    StatsTablePreAggregatedQueryBuilder,
)
from products.web_analytics.backend.insightsql_queries.test.web_preaggregated_test_base import (
...
class TestWebStatsPreAggregated(WebAnalyticsPreAggregatedTestBase):
    ...
    def test_query_with_supported_properties(self):
        ...
        runner = WebStatsTableQueryRunner(team=self.team, query=query)
        runner.modifiers = InsightsQLQueryModifiers(useWebAnalyticsPreAggregatedTables=True)
    ...
    def test_query_includes_order_by(self):
        ...
        insightsql_query = runner.to_query()

        assert insightsql_query.order_by
        assert len(insightsql_query.order_by) > 0
