...
class TestWorkflowEndpointMapping(BaseTest):
    ...
    def test_workflow_health_maps_and_nulls_empty_window(self) -> None:
        ...
        rows = [
            ...
            ("Insights", "insights", "Deploy", 2, 0, 0, 0, None, float("nan"), float("nan"), None, 0, 0, "", 0, 0, 0),
        ]
        ...
        bucket_rows = [("Insights", "insights", "CI", datetime.now(tz=UTC) - timedelta(days=1), 10, 8, 7, 1)]
        # Third response: the previous-window success rate (the Δ baseline); Deploy had no prior runs.
        prev_rows = [("Insights", "insights", "CI", 0.95)]
        ...
        assert items[0].repo.owner == "Insights" and items[0].repo.name == "insights"
...
class TestWorkflowEndpointsWarehouse(_EndpointsWarehouseMixin, BaseTest):
    ...
    def test_workflow_runner_costs_breaks_down_by_tier(self) -> None:
        ...
        costs = api.get_workflow_runner_costs(team=self.team, repo="Insights/insights", workflow_name="CI")
    ...
    def test_workflow_run_detail_by_id(self) -> None:
        ...
        assert run.repo.owner == "Insights" and run.repo.name == "insights"
    ...
    def test_workflow_run_list_scoped_to_workflow(self) -> None:
        ...
        ci_runs = api.list_workflow_runs(team=self.team, repo="Insights/insights", workflow_name="CI")
        ...
        wide = api.list_workflow_runs(team=self.team, repo="Insights/insights", workflow_name="CI", date_from="-90d")
        assert [r.id for r in wide] == [8002, 8001, 8004]

        # A repo with no such workflow yields an empty list (not an error).
        assert api.list_workflow_runs(team=self.team, repo="Insights/insights", workflow_name="Nope") == []
    ...
    def test_workflow_run_activity_projects_and_windows(self) -> None:
        ...
        activity = api.get_workflow_run_activity(team=self.team, repo="Insights/insights", workflow_name="CI")
        ...
        wide = api.get_workflow_run_activity(
            team=self.team, repo="Insights/insights", workflow_name="CI", date_from="-90d"
        )
        ...
        guard = api.get_workflow_run_activity(team=self.team, repo="Insights/insights", workflow_name="Guard")
        ...
        sparse = api.get_workflow_run_activity(team=self.team, repo="Insights/insights", workflow_name="Sparse")
    ...
    def test_workflow_detail_branch_filter(self) -> None:
        ...
        repo, workflow = "Insights/insights", "CI"
    ...
    def test_workflow_run_detail_handles_null_timestamps(self) -> None:
        ...
        runs = api.list_pr_runs(team=self.team, pr_number=63, repo="Insights/insights")
