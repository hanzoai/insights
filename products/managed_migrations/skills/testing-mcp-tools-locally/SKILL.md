...
If `insightscli wait` fails on `migrate-persons-db` or `migrate-behavioral-cohorts`,
...
```bash
insightscli migrations:run
```

Datastore migration failures are fine — batch imports only need Postgres.

## 3. Verify DB connectivity from the Django shell

```bash
insightscli dev:shell-plus -y -- -c "
...
Use `insightscli dev:shell-plus` to create `BatchImport` records in various states.
...
The wrangler worker (`pnpm run dev`, port 8787) proxies `/mcp` to **production** `mcp.us.hanzo.ai` unless `MCP_HONO_URL` is set,
...
The `insights-db-1` Docker container may have stale port mappings
