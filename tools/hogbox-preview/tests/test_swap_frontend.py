...
Self-contained: no network, no live box. Two seams are covered:
  ...
  - ``InsightsPreviewStack.swap_frontend_only`` — the stack half: attach to the
...
class _RecordingBackend:
    ...
    def web_url(self) -> str:
        return "https://pen-deadbeef.boxes.hogland.prod-us.insights.dev"
...
class SwapFrontendOnlyTest(unittest.TestCase):
    ...
    def test_swap_attaches_reuses_key_and_serves_pr_frontend(self):
        ...
        stack = InsightsPreviewStack(backend, frontend_dist_tar=self._dist_tar())
    ...
    def test_swap_requires_a_dist(self):
        backend = _RecordingBackend()
        stack = InsightsPreviewStack(backend)  # no frontend_dist_tar
