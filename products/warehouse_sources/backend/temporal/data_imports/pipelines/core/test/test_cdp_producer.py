...
@pytest.mark.asyncio
async def test_should_run_no_insights_function(team):
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_run_with_matching_insights_function(team):
    source = await sync_to_async(ExternalDataSource.objects.create)(
        team=team, source_type=ExternalDataSourceType.POSTGRES
    )
    table = await sync_to_async(DataWarehouseTable.objects.create)(
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_run_with_both_insights_function_and_flow(team):
    source = await sync_to_async(ExternalDataSource.objects.create)(
        team=team, source_type=ExternalDataSourceType.POSTGRES
    )
    table = await sync_to_async(DataWarehouseTable.objects.create)(
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_run_insights_database_connection_failure_stays_retryable(team):
    # `should_run` queries Insights's own database (InsightsFunction/InsightsFlow), not the
