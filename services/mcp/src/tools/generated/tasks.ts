...
import { withInsightsUrl, pickResponseFields, omitResponseFields, type WithInsightsUrl } from '@/tools/tool-utils'
...
const loopsList = (): ToolBase<typeof LoopsListSchema, WithInsightsUrl<Schemas.PaginatedLoopDTOList>> => ({
    name: 'loops-list',
    ...
    handler: async (context: Context, params: z.infer<typeof LoopsListSchema>) => {
        ...
        return await withInsightsUrl(context, result, '/tasks')
    },
})
...
const loopsRetrieve = (): ToolBase<typeof LoopsRetrieveSchema, WithInsightsUrl<Schemas.LoopDTO>> => ({
    name: 'loops-retrieve',
...
const loopsRunsRetrieve = (): ToolBase<typeof LoopsRunsRetrieveSchema, WithInsightsUrl<Schemas.LoopRunPage>> => ({
        ...
        return await withInsightsUrl(
            context,
            {
                ...
                results: await Promise.all(
                    (result.results ?? []).map((item) => withInsightsUrl(context, item, `/tasks/${item.id}/runs`))
                ),
            },
            '/tasks'
        )
...
const tasksCreate = (): ToolBase<typeof TasksCreateSchema, WithInsightsUrl<Schemas.TaskDetailDTO>> => ({
    name: 'tasks-create',
    ...
    handler: async (context: Context, params: z.infer<typeof TasksCreateSchema>) => {
        ...
        return await withInsightsUrl(context, filtered, `/tasks/${filtered.id}`)
    },
})

const TasksListSchema = TasksListQueryParams

const tasksList = (): ToolBase<typeof TasksListSchema, WithInsightsUrl<Schemas.PaginatedTaskDetailDTOList>> => ({
    name: 'tasks-list',
    ...
    handler: async (context: Context, params: z.infer<typeof TasksListSchema>) => {
        ...
        return await withInsightsUrl(
            context,
            {
                ...filtered,
                results: await Promise.all(
                    (filtered.results ?? []).map((item) => withInsightsUrl(context, item, `/tasks/${item.id}`))
                ),
            },
            '/tasks'
        )
...
const tasksRetrieve = (): ToolBase<typeof TasksRetrieveSchema, WithInsightsUrl<Schemas.TaskDetailDTO>> => ({
    name: 'tasks-retrieve',
...
const tasksRunsList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedTaskRunDetailDTOList>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof TasksRunsListSchema>) => {
        ...
        return await withInsightsUrl(context, filtered, '/tasks')
    },
})
