//! Minimal ScriptVM bytecode runner. Emits the `{"result", "error"}` JSON shape that the Python
//! (`common/scriptvm/python/execute.py`) and Node runners also emit, so output can be diffed across runtimes.
