...
from insights.schema import DateRange, MarketingAnalyticsDrillDownLevel, MarketingAnalyticsTableQuery
...
from insights.datastore.preaggregation.marketing_costs_sql import DISTRIBUTED_MARKETING_COSTS_TABLE
...
GOOGLE_STATS_COLUMNS: dict[str, dict[str, str | bool]] = {
    ...
    "metrics_cost_micros": {"insightsql": "FloatDatabaseField", "datastore": "Float64", "schema_valid": True},
    "metrics_impressions": {"insightsql": "FloatDatabaseField", "datastore": "Float64", "schema_valid": True},
    "metrics_conversions": {"insightsql": "FloatDatabaseField", "datastore": "Float64", "schema_valid": True},
    "metrics_conversions_value": {"insightsql": "FloatDatabaseField", "datastore": "Float64", "schema_valid": True},
}


@override_settings(IN_UNIT_TESTING=True)
class TestMarketingCostsPrecompute(DatastoreTestMixin, BaseTest):
    ...
    def _execute(self, query: ast.Expr) -> tuple[list, list[str]]:
        result = execute_insightsql_query(query.to_insightsql(), self.team)
    ...
    def test_one_unmaterializable_source_does_not_force_all_to_s3(self):
        ...
        with (
            ...
            patch(
                "products.marketing_analytics.backend.insightsql_queries.marketing_analytics_base_query_runner.marketing_ensure_precomputed",
                return_value=ready,
            ),
        ):
        ...
        insightsql = result.to_insightsql()
        assert "marketing_costs_precomputed" in insightsql, "materialized source should read the deduplicated view"
        assert "live_s3_marker" in insightsql, "unmaterializable source should stay on the live S3 union"
