...
class TestWizardClientMetadataView(SimpleTestCase):
    def test_returns_valid_cimd_metadata(self):
        ...
        assert data["client_id"] == "https://us.hanzo.ai/api/oauth/wizard/client-metadata"
        assert data["client_name"] == "Insights Wizard"
    ...
    @override_settings(SITE_URL="https://eu.hanzo.ai")
    def test_client_id_uses_site_url_for_eu(self):
        res = self.client.get("/api/oauth/wizard/client-metadata")
        data = res.json()
        assert data["client_id"] == "https://eu.hanzo.ai/api/oauth/wizard/client-metadata"
