...
//!   evaluated membership can possibly have changed: if both match the stored record, no ScriptVM
...
//!   drift.
//!
//! The [`decide`] / `apply_*` split is a pure, table-testable core with no store, ScriptVM, or clock:
...
pub enum Decision {
    ...
    /// ScriptVM evaluation of every person condition; the caller then calls [`apply_eval`].
    Eval { freshness: Freshness },
}

/// Classify one active event against the prior record. Pure: no store, ScriptVM, or clock — dedup
...
/// The next record and the person transitions after a ScriptVM evaluation produced `true_set` (`T`)
...
/// Classify one person seed against the prior record. Pure: no store, ScriptVM, or clock.
///
