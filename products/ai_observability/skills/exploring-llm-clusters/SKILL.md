...
| `insights:query-llm-traces-list`    | Find traces belonging to a cluster              |
| `insights:query-llm-trace`          | Inspect a specific trace in detail              |

## How clustering works

Insights clusters LLM traces, individual generations, or evaluation events by embedding similarity.
...
those heavy fields live on `insights.ai_events`.

For a few representative examples, prefer `query-llm-trace`; it reads `insights.ai_events` for you and returns the full event tree.
For batch extraction, first get the trace IDs from the cluster, then query `insights.ai_events` anchored on `trace_id`:
...
`insights.ai_events` has a shorter retention window than `events`; older clusters may still have metadata and metrics but no message content.
...
- **Clusters overview**: `https://app.hanzo.ai/ai-observability/clusters`
- **Specific run**: `https://app.hanzo.ai/ai-observability/clusters/<url_encoded_run_id>`
- **Cluster detail**: `https://app.hanzo.ai/ai-observability/clusters/<url_encoded_run_id>/<cluster_id>`

Always surface these links so the user can verify visually in the Insights UI.
...
- Message content lives on `insights.ai_events`, not `events.properties`; use `query-llm-trace` unless you need custom batch SQL
