"""Tests for person deletion via personinsights RPC.
...
from insights.models.person.util import delete_persons_from_postgres, get_person_by_distinct_id, get_person_by_uuid
...
class TestDeletePersonsFromPostgresRouting(SimpleTestCase):
    ...
    def test_personinsights_batches_over_1000(self):
        mock_persons = []
        for i in range(1500):
            p = MagicMock()
            p.uuid = f"uuid-{i}"
...
class TestDeletePersonsForTeamsRouting(SimpleTestCase):
    @patch("insights.models.team.util._raw_delete_batch")
    def test_routes_to_personinsights(self, mock_raw_delete_batch):
        with fake_personinsights_client():
...
# ── RPC behavior tests (personinsights fake with real test data) ─────────
...
class TestDeletePersonsForTeamsRPC(BaseTest):
    def test_personinsights_path_calls_batch_rpc_per_team(self):
    ...
    def test_personinsights_batch_rpc_loops_until_done(self):
        p1 = create_person(team=self.team, distinct_ids=["a"])
        p2 = create_person(team=self.team, distinct_ids=["b"])
