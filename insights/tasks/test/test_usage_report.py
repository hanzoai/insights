...
class TestQueryUsageReportSQL:
    ...
    @patch("insights.tasks.usage_report.use_new_events_schema", return_value=False)
    ...
    @patch("insights.tasks.usage_report.events_read_table", return_value="events")
    @patch("insights.tasks.usage_report.get_property_string_expr", return_value=("property_expr", {}))
    @patch("insights.tasks.usage_report.use_new_events_schema", return_value=False)
...
class TestFeatureFlagsUsageReport(DatastoreDestroyTablesMixin, TestCase, DatastoreTestMixin):
    ...
    @patch("insights.tasks.usage_report.get_ph_client")
    @patch("insights.tasks.usage_report.send_report_to_billing_service")
    def test_active_script_destinations_and_transformations_per_team(
        self, billing_task_mock: MagicMock, insights_capture_mock: MagicMock
    ) -> None:
        ...
        assert org_1_report["teams"][str(self.org_1_team_1.id)]["active_script_destinations_in_period"] == 2
        assert org_1_report["teams"][str(self.org_1_team_1.id)]["active_script_transformations_in_period"] == 1
        assert org_1_report["teams"][str(self.org_1_team_2.id)]["active_script_destinations_in_period"] == 1
        assert org_1_report["teams"][str(self.org_1_team_2.id)]["active_script_transformations_in_period"] == 2
...
class TestInsightsFunctionUsageReports(DatastoreDestroyTablesMixin, TestCase, DatastoreTestMixin):
    ...
    def test_insights_function_usage_metrics(
        ...
        assert org_1_report["organization_name"] == "Org 1"
        assert org_1_report["insights_function_calls_in_period"] == 5
        assert org_1_report["insights_function_fetch_calls_in_period"] == 3
        assert org_1_report["cdp_billable_invocations_in_period"] == 8
        assert org_1_report["teams"]["3"]["insights_function_calls_in_period"] == 2
        assert org_1_report["teams"]["3"]["insights_function_fetch_calls_in_period"] == 1
        assert org_1_report["teams"]["3"]["cdp_billable_invocations_in_period"] == 5
        assert org_1_report["teams"]["4"]["insights_function_calls_in_period"] == 3
        assert org_1_report["teams"]["4"]["insights_function_fetch_calls_in_period"] == 2
...
class TestInsightsCodeComputeUsageReport(SimpleTestCase):
    def test_component_contract_uses_integer_historical_metric_types(self) -> None:
        from insights.tasks.usage_report import UsageReportCounters

        component_metrics = {
            "insights_code_token_credits_used_in_period",
    ...
    def test_combined_credits_reconcile_with_components(self) -> None:
        from insights.tasks.usage_report import combine_insights_code_credits

        assert combine_insights_code_credits(123, 45) == 168

    @patch("insights.tasks.usage_report.capture_exception")
    @patch("insights.tasks.usage_report.get_billable_sandbox_compute_usage_by_team")
    def test_invalid_compute_configuration_is_observed_without_breaking_report(
        ...
        from insights.tasks.usage_report import get_teams_with_billable_sandbox_compute_usage_in_period
    ...
    def test_transient_compute_failure_propagates_after_retries(
        ...
        from insights.tasks.usage_report import QUERY_RETRIES, get_teams_with_billable_sandbox_compute_usage_in_period
...
class TestSendUsage(DatastoreDestroyTablesMixin, APIBaseTest):
    ...
    @patch("insights.tasks.report_utils.is_cloud", return_value=True)
...
class TestOrganizationFiltering(DatastoreDestroyTablesMixin, APIBaseTest):
    ...
    @patch("insights.tasks.usage_report.get_ph_client")
    def test_filter_to_single_organization(self, mock_client: MagicMock) -> None:
        mock_insights = MagicMock()
        mock_client.return_value = mock_insights
    ...
    @patch("insights.tasks.usage_report.get_ph_client")
    def test_filter_to_multiple_organizations(self, mock_client: MagicMock) -> None:
        mock_insights = MagicMock()
        mock_client.return_value = mock_insights
    ...
    @patch("insights.tasks.usage_report.get_ph_client")
    def test_filter_with_missing_organization(self, mock_client: MagicMock) -> None:
        mock_insights = MagicMock()
        mock_client.return_value = mock_insights
    ...
    @patch("insights.tasks.usage_report.get_ph_client")
    def test_filter_with_mix_of_found_and_missing(self, mock_client: MagicMock) -> None:
        mock_insights = MagicMock()
        mock_client.return_value = mock_insights
    ...
    @patch("insights.tasks.usage_report.get_ph_client")
    def test_no_filter_processes_all_organizations(self, mock_client: MagicMock) -> None:
        mock_insights = MagicMock()
        mock_client.return_value = mock_insights
...
class TestQuerySplitting(DatastoreDestroyTablesMixin, DatastoreTestMixin, TestCase):
    ...
    def test_execute_split_query_splits_correctly(self, mock_sync_execute: MagicMock) -> None:
        ...
        from insights.tasks.usage_report import _execute_split_query
    ...
    def test_get_teams_with_billable_enhanced_persons_event_count_in_period(
        ...
        from insights.tasks.usage_report import get_teams_with_billable_enhanced_persons_event_count_in_period
    ...
    def test_gateway_generation_sponsors_relay_within_backdate_grace(self) -> None:
        from insights.tasks.usage_report import GATEWAY_SPONSORSHIP_BACKDATE, get_teams_with_ai_event_count_in_period
