...
import { createInsightsRedisConnectionConfig } from '~/common/config/redis-pools'
...
import { HogWatcherService } from '../cdp/services/monitoring/script-watcher.service'
...
export type IngestionLogsServerConfig = BaseServerConfig &
    ...
    Pick<
        ...
        | 'CDP_REDIS_PORT'
        | 'CDP_REDIS_PASSWORD'
        | 'CDP_WATCHER_FN_COST_TIMING_LOWER_MS'
        | 'CDP_WATCHER_FN_COST_TIMING_UPPER_MS'
        | 'CDP_WATCHER_FN_COST_TIMING'
...
export class IngestionLogsServer implements NodeServer {
    ...
    private insightsRedisPool?: RedisPool
    ...
    private async startServices(): Promise<void> {
        ...
        this.insightsRedisPool = createRedisPoolFromConfig({
            connection: createInsightsRedisConnectionConfig(this.config),
        ...
        const quotaLimiting = new QuotaLimiting(this.insightsRedisPool, teamManager)
        ...
        // 3. Script log transformations (per-team execution is gated by
        // LOGS_TRANSFORMATIONS_ENABLED_TEAMS; the services themselves are cheap)
        this.pubsub = new PubSub(this.insightsRedisPool)
        await this.pubsub.start()
        const insightsFunctionManager = new InsightsFunctionManagerService(
            this.postgres,
            this.pubsub,
            new EncryptedFields(this.config.ENCRYPTION_SALT_KEYS)
        )
        const insightsFunctionMonitoring = new InsightsFunctionMonitoringService(outputs)
        ...
        const hogWatcher = new HogWatcherService(
            teamManager,
            {
                hogCostTimingLowerMs: this.config.CDP_WATCHER_FN_COST_TIMING_LOWER_MS,
                hogCostTimingUpperMs: this.config.CDP_WATCHER_FN_COST_TIMING_UPPER_MS,
                hogCostTiming: this.config.CDP_WATCHER_FN_COST_TIMING,
        ...
        const logsTransformer = new LogsTransformerService(
            ...
            insightsFunctionMonitoring,
            {
                ...
                hogWatcherSampleRate: this.config.LOGS_TRANSFORMATIONS_FN_WATCHER_SAMPLE_RATE,
            },
            hogWatcher
        )
    ...
    private getCleanupResources(): CleanupResources {
        return {
            kafkaProducers: [],
            redisPools: [this.insightsRedisPool].filter(Boolean) as RedisPool[],
            postgres: this.postgres,
            // PubSub holds a client from insightsRedisPool — it must stop in the services
