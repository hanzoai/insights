...
from insightscli_commands.devenv.cli import dev_regenerate_mprocs
from insightscli_commands.devenv.generator import (
    _READY_MARKER,
...
from insightscli_commands.devenv.registry import ProcessRegistry, create_mprocs_registry
from insightscli_commands.devenv.resolver import Capability, Intent, IntentMap, IntentResolver, load_intent_map
from insightscli_commands.devenv.wizard import _parse_exclude_input
...
class TestSandboxWrapper:
    ...
    def test_wraps_by_default(self, monkeypatch: Any) -> None:
        monkeypatch.delenv("INSIGHTS_DEV_SANDBOX", raising=False)
    ...
    def test_disabled_leaves_command_unwrapped(self, monkeypatch: Any) -> None:
        monkeypatch.setenv("INSIGHTS_DEV_SANDBOX", "0")
        result = self._wrap({"shell": "./bin/start-backend", "sandbox": True})
    ...
    def test_temporal_worker_excluded_by_default(self, monkeypatch: Any) -> None:
        monkeypatch.delenv("INSIGHTS_DEV_SANDBOX", raising=False)
        monkeypatch.delenv("INSIGHTS_DEV_SANDBOX_EXCLUDE", raising=False)
    ...
    def test_excluded_proc_left_unwrapped(self, monkeypatch: Any) -> None:
        # INSIGHTS_DEV_SANDBOX_EXCLUDE is additive on top of the default excludes.
        monkeypatch.delenv("INSIGHTS_DEV_SANDBOX", raising=False)
        monkeypatch.setenv("INSIGHTS_DEV_SANDBOX_EXCLUDE", "other-proc")
        generator = MprocsGenerator(MockRegistry({}))
...
class TestDockerComposeProjectName:
    ...
    def test_defaults_to_insights_when_unset(self, monkeypatch: Any) -> None:
        monkeypatch.delenv("COMPOSE_PROJECT_NAME", raising=False)

        cmd = build_docker_compose_command([])

        assert "-p insights " in cmd

    def test_defaults_to_insights_when_empty(self, monkeypatch: Any) -> None:
    ...
    @parameterized.expand(
        [
            ("plain name", "insights-worktree", "insights-worktree"),
            ("value with a space", "insights dev", "insights dev"),
            ("value with shell metacharacters", "insights; rm -rf /", "insights; rm -rf /"),
        ]
    )
...
class TestRegenerateMprocsCheckFlag:
    ...
    def test_fails_without_writing_when_stale(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("insightscli_commands.devenv.cli.TRACKED_MPROCS_FILES", patched_targets)
...
class TestDockerComposeShellBehavior:
    ...
    (and, for e2e, the personinsights step) succeeds, and must never invoke docker at
    all when INSIGHTS_SANDBOX=1 (e2e never sandboxes, so it has no such case).
    ...
    def test_generated_shell_delegates_to_the_shared_docker_compose_helper(self) -> None:
        # The insightscli-generated shell shares its up/logs/sandbox-guard logic with the
    ...
    @staticmethod
    def _run_shell(
        shell: str, up_exit: int, personinsights_exit: int = 0, *, sandbox: bool
    ) -> subprocess.CompletedProcess[str]:
        with tempfile.TemporaryDirectory() as tmp:
            fake_docker = Path(tmp) / "docker"
            fake_docker.write_text(
                f'#!/bin/sh\ncase "$*" in *personinsights*) exit {personinsights_exit};; *" up "*) exit {up_exit};; esac\nexit 0\n'
            )
            ...
            if sandbox:
                env["INSIGHTS_SANDBOX"] = "1"
            else:
                env.pop("INSIGHTS_SANDBOX", None)
    ...
    def test_ready_marker_tracks_up_result_unless_sandboxed(
        self, source: str, sandbox: bool, up_exit: int, personinsights_exit: int, expect_ready: bool
    ) -> None:
        # Sandbox cases pass up_exit=1 and still expect ready, proving docker's `up`
        # call is never reached once INSIGHTS_SANDBOX=1 short-circuits the shell. The
        # e2e personinsights_exit=1 case proves a failed personinsights step (its own `&&` in
        # the chain) blocks ready too, not just a failed main `up`.
        result = self._run_shell(self._shell(source), up_exit, personinsights_exit, sandbox=sandbox)
...
class TestInfoProcess:
    ...
    @parameterized.expand(
        [
            ...
            ("explicit_enabled_macos", {"INSIGHTS_DEV_SANDBOX": "1"}, "darwin", True, r"\033[32mon"),
            ("disabled_explicit", {"INSIGHTS_DEV_SANDBOX": "0"}, "darwin", True, "INSIGHTS_DEV_SANDBOX=0"),
    ...
    def test_info_process_shows_sandbox_status(
        ...
        with (
            ...
            if "INSIGHTS_DEV_SANDBOX" not in env:
                os.environ.pop("INSIGHTS_DEV_SANDBOX", None)
    ...
    def test_info_process_reads_news_at_runtime(self) -> None:
        ...
        assert "raw.githubusercontent.com/insights/insights/master/devenv/news.txt" in shell
    ...
    def test_info_process_includes_commands(self) -> None:
        ...
        assert "insightscli dev:setup" in shell
        assert "insightscli dev:explain" in shell
    ...
    def test_info_process_includes_path_and_secrets(self) -> None:
        ...
        assert "_INSIGHTS_OP_RESOLVED" in shell
...
class TestPersonhogEnvInjection:
    ...
    def _make_fixtures(self, *, with_personinsights: bool):
        ...
        registry._processes["backend"] = {"shell": "./bin/start-backend", "capability": ""}
        registry._processes["nodejs"] = {"shell": "./bin/insights-node", "capability": "event_ingestion"}
        return intent_map, registry

    @parameterized.expand(
        [
            (True, ["personinsights"], True),
            (False, ["feature_flags"], False),
        ]
    )
    def test_personinsights_env_injection(self, with_personinsights: bool, intents: list[str], should_inject: bool) -> None:
        intent_map, registry = self._make_fixtures(with_personinsights=with_personinsights)
        resolver = IntentResolver(intent_map, registry)
        resolved = resolver.resolve(intents)
        config = MprocsGenerator(registry).generate(resolved)
        ...
        for proc_name in ["backend", "nodejs"]:
            ...
            if should_inject:
                assert "PERSONFN_ADDR" in shell, f"PERSONFN_ADDR should be in {proc_name} shell"
            else:
                assert "PERSONFN_ADDR" not in shell, f"PERSONFN_ADDR should not be in {proc_name} shell"
