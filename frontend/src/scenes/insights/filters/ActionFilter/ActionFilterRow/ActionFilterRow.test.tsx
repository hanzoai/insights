...
// InsightsQLEditor uses Monaco which requires Web Workers and cannot run in jsdom
jest.mock('lib/components/InsightsQLEditor/InsightsQLEditor', () => ({
    InsightsQLEditor: () => <div data-attr="mock-insightsql-editor" />,
}))
...
describe('ActionFilterRow', () => {
    ...
    beforeEach(() => {
        ...
        useMocks({
            get: {
                ...
                '/v1/projects/:team/actions/': { results: filtersJson.actions },
                '/v1/environments/:team/groups_types/': MOCK_GROUP_TYPES,
                '/v1/projects/:team/warehouse_tables/': { results: [] },
                '/v1/projects/:team/warehouse_saved_queries/': { results: [] },
                '/v1/projects/:team/warehouse_view_links/': { results: [] },
            },
        })
                    ...
                    tables: {
                        events_table: {
                            ...
                            fields: {
                                ...
                                customer_name: {
                                    name: 'customer_name',
                                    insightsql_value: 'customer_name',
                                    type: 'string',
                                    schema_valid: true,
                                },
                            },
    ...
    describe('math selection', () => {
        ...
        it('shows InsightsQL expression button when InsightsQL math is active', () => {
            const { logic } = setup()
            renderRow(logic, {
                mathAvailability: MathAvailability.All,
                filter: {
                    ...
                    math_insightsql: 'sum(price)',
                },
            })
            expect(screen.getByTestId('math-insightsql-select-0')).toBeInTheDocument()
            expect(screen.getByText('sum(price)')).toBeInTheDocument()
        })

        it('defaults InsightsQL math expression to person_id for stickiness insights', async () => {
            ...
            expect(setFilters).toHaveBeenCalledWith(
                expect.objectContaining({
                    events: expect.arrayContaining([expect.objectContaining({ math_insightsql: 'person_id' })]),
                })
            )
        })
        ...
        it('selecting property math sets math_property and clears math_insightsql', async () => {
            const { logic, setFilters } = setup()
            renderRow(logic, {
                mathAvailability: MathAvailability.All,
                filter: {
            ...
            await waitFor(() => {
                ...
                // math_insightsql should not be present on the updated filter
                const updatedEvent = call.events.find((e: any) => e.math_property === '$session_duration')
                expect(updatedEvent.math_insightsql).toBeUndefined()
            })
        })
    ...
    describe('event selection onChange handler', () => {
        ...
        it.each([
            ...
            async ({ tab, searchText, resultValue, expectedEventId, expectedPropertyKey, expectedOperator }) => {
                useMocks({
                    get: {
                        '/v1/environments/:team/events/values/': [{ name: resultValue }, { name: 'other-value' }],
                    },
                })
