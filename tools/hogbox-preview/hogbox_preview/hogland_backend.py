"""Hogland preview layer, driven by the ``insights-hogland`` SDK.
...
surface, keyless, with one dependency (``uv run --with insights-hogland``).
...
class HoglandBackend(PreviewBackend):
    def __init__(
        ...
        *,
        host: str | None = None,
        snapshot: str = "alias:insights-preview-golden",
        cpus: float = 8,
        memory_mib: int = 16384,
        disk_gib: int = 100,
        disk_class: str = "mirrored",
        web_port: int = 8000,
        name: str = "insights-preview",
        kind: str = "preview",
