...
export interface handsFreeLogicValues {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface handsFreeLogicActions {
    cancelSpeaking: () => {
        value: true
    }
    ...
    exitHandsFree: (reason?: string) => {
        reason: string
    }
    interruptSpeaking: () => {
        value: true
    }
    setConnection: (connection: HandsFreeConnection) => {
        connection: HandsFreeConnection
    }
    ...
    setStatus: (status: HandsFreeStatus) => {
        status: HandsFreeStatus
    }
    speakAssistantResponse: (summary: AssistantSummary) => {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface handsFreeLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        isActive: (status: HandsFreeStatus) => boolean
        canUseHandsFree: (sdkAvailable: boolean | null) => boolean
    }
}

export type handsFreeLogicType = MakeLogicType<
    handsFreeLogicValues,
    handsFreeLogicActions,
    HandsFreeLogicProps,
    handsFreeLogicMeta
>
...
export const handsFreeLogic = kea<handsFreeLogicType>([
    ...
    selectors({
        isActive: [(s) => [s.status], (status: HandsFreeStatus) => status !== 'off'],
        ...
        canUseHandsFree: [(s) => [s.sdkAvailable], (sdkAvailable: boolean | null) => sdkAvailable !== false],
    }),
    ...
    listeners(({ actions, values, cache, props }) => ({
        ...
        enterHandsFree: async () => {
            ...
            // Per-session analytics counters, captured at exit so a single Insights query can
            ...
            try {
                ...
                toast.error('Hands-free could not load. Please try again.')
            ...
            if (!Scribe || typeof Scribe.connect !== 'function') {
                ...
                // see it in Insights and bail without confusing the user.
                insights.captureException(new Error('handsFree: Scribe namespace missing from @elevenlabs/client'))
            ...
            const establishConnection = async (isReconnect: boolean): Promise<boolean> => {
                ...
                try {
                    ...
                    if (!isReconnect) {
                        ...
                        toast.error('Hands-free could not start. Please try again.')
                    }
                ...
                try {
                    ...
                    if (isPermissionError) {
                        ...
                        toast.error('Microphone access was denied. Hands-free disabled.')
                        ...
                        toast.error('Hands-free failed to start. See console for details.')
            ...
            const onSessionStarted = (): void => {
                ...
                if (wasStarting) {
                    insights.capture('max hands-free entered')
                }
                if (wasReconnecting) {
                    insights.capture('max hands-free reconnected', {
            ...
            const onPartial = (payload: any): void => {
                ...
                if (values.status === 'speaking') {
                    ...
                    if (reason) {
                        insights.capture('max hands-free partial suppressed', {
                            reason,
            ...
            const onClose = (): void => {
                ...
                insights.capture('max hands-free reconnect attempted', { attempt: attempts })
                cache.connection = undefined
                void establishConnection(true)
            }
            const onError = (payload: any): void => {
                insights.captureException(new Error(`scribe error: ${JSON.stringify(payload ?? {})}`))
        ...
        exitHandsFree: ({ reason }) => {
            insights.capture('max hands-free exited', {
                reason,
        ...
        commitTranscript: ({ text }) => {
            ...
            insights.capture('max hands-free user spoke', { transcript_chars: text.length })
        ...
        speakAssistantResponse: async ({ summary }) => {
            ...
            try {
                ...
                insights.capture('max hands-free assistant spoke', {
        ...
        interruptSpeaking: () => {
            ...
            insights.capture('max hands-free tts interrupted')
        },
    })),

    afterMount(({ actions, cache }) => {
        // Probe the SDK once on mount so the mic button can decide whether to render itself.
        // We capture failures to Insights instead of surfacing dev-facing errors to end users.
        ...
        if (!insights.isFeatureEnabled(FEATURE_FLAGS.MAX_HANDS_FREE)) {
            actions.setSdkAvailable(false)
            return
        }
...
// reason to Insights so we can measure misfire rate before tuning the heuristic.
