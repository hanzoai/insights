...
impl ChangelogRecovery {
    pub fn new(cfg: RecoveryConfig) -> Result<Self, String> {
        ...
        let group = format!("personinsights-leader-recovery-{pod}", pod = cfg.pod_name);
    ...
    async fn checkout(&self) -> Result<PooledConsumer<'_>, String> {
        ...
        histogram!("personinsights_leader_recovery_pool_wait_ms")
        ...
        loop {
            ...
            match attempt {
                ...
                Err(FetchError::Transient(message)) => {
                    ...
                    counter!("personinsights_leader_recovery_retries_total").increment(1);
                    tracing::debug!(
                        partition,
                        offset,
                        error = %message,
