...
class TestExperimentMeanMetric(ExperimentQueryRunnerBaseTest):
    ...
    @freeze_time("2024-01-01T12:00:00Z")
    @snapshot_datastore_queries
    def test_count_unique_property_values_via_insightsql(self, name, use_precomputation):
        self._setup_precomputation_test(use_precomputation)
        ...
        metric = ExperimentMeanMetric(
            source=EventsNode(
                ...
                math_insightsql="uniqExact(properties.category)",
            ),
        )
    ...
    @freeze_time("2024-01-01T12:00:00Z")
    @snapshot_datastore_queries
    def test_insightsql_count_metric_with_winsorization(self, name, use_precomputation):
        self._setup_precomputation_test(use_precomputation)
        ...
        # INSIGHTSQL count() with winsorization - this should not fail with
        ...
        metric = ExperimentMeanMetric(
            source=EventsNode(
                event="purchase",
                math=ExperimentMetricMathType.INSIGHTSQL,
                math_insightsql="count(properties.category)",
            ),
            lower_bound_percentile=0.1,
            upper_bound_percentile=0.9,
        )
