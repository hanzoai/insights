...
from products.review.backend.temporal.activities import _resolve_acting_user
...
class TestResolveActingUser(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            # No Insights org user for the author (or no author) → None, so the parent skips the review.
    ...
    def test_resolve_stamps_the_acting_user_onto_the_report(self) -> None:
        ...
        report = ReviewReport.objects.for_team(self.team.id).create(
            ...
            pr_number=7,
            pr_url="https://github.com/Insights/insights/pull/7",
            head_branch="feat",
            base_branch="main",
        )
