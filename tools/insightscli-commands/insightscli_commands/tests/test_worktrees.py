...
import click
from insightscli_commands.worktrees import (
    Worktree,
    _belongs_to_repo,
    _collect_deps_items,
    _compute_git_state,
...
class TestParseCutoff:
    ...
    def _frozen_clock(self, monkeypatch):
        monkeypatch.setattr("insightscli_commands.worktrees.time.time", lambda: _NOW)
...
class TestRegisteredWorktrees:
    def test_parses_branch_detached_and_locked(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("insightscli_commands.worktrees.subprocess.check_output", lambda *a, **k: porcelain)
    ...
    def test_git_error_returns_none(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("insightscli_commands.worktrees.subprocess.check_output", _boom)
...
class TestComputeGitState:
    def test_genuine_detached_head(self, tmp_path, monkeypatch) -> None:
        monkeypatch.setattr(
            "insightscli_commands.worktrees._git",
    ...
    def test_status_timeout_fails_closed(self, tmp_path, monkeypatch) -> None:
        ...
        gitdir.mkdir()
        (tmp_path / ".git").write_text(f"gitdir: {gitdir}\n")
        monkeypatch.setattr("insightscli_commands.worktrees._git", lambda cwd, args, timeout=30.0: None)
        wt = _make_worktree(tmp_path)
        _compute_git_state(wt, has_remotes=True)
        assert wt.state_unknown and wt.unsafe
...
class TestDuSizes:
    def test_parses_per_path_and_skips_garbage(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "insightscli_commands.worktrees.subprocess.run",
