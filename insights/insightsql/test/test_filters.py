...
class TestFilters(BaseTest):
    ...
    def test_replace_filters_relative_date_from_snaps_to_start_of_day(self, date_from: str, expected: str):
        ...
        with freeze_time("2020-02-15T13:37:42Z"):
            select = replace_filters(
                ...
                InsightsQLFilters(dateRange=DateRange(date_from=date_from)),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    def test_replace_filters_date_from_respects_week_start_day(self):
        ...
        with freeze_time("2020-02-15T13:37:42Z"):  # a Saturday
            select = replace_filters(
                self._parse_select("SELECT event FROM events WHERE timestamp >= {filters.dateRange.from}"),
                InsightsQLFilters(dateRange=DateRange(date_from="wStart")),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    def test_replace_filters_sub_day_date_from_stays_rolling(self):
        ...
        with freeze_time("2020-02-15T13:37:42Z"):
            select = replace_filters(
                self._parse_select("SELECT event FROM events where {filters}"),
                InsightsQLFilters(dateRange=DateRange(date_from="-1h")),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    def test_replace_filters_date_to_resolution(self, date_to: Optional[str], expected: str):
        ...
        with freeze_time("2020-02-15T13:37:42Z"):
            select = replace_filters(
                ...
                InsightsQLFilters(dateRange=DateRange(date_from="-7d", date_to=date_to)),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    def test_replace_filters_open_ended_date_to_uses_team_timezone(self):
        ...
        with freeze_time("2020-02-15T03:00:00Z"):  # still Feb 14 in New York
            select = replace_filters(
                self._parse_select("SELECT event FROM events WHERE timestamp <= {filters.dateRange.to}"),
                InsightsQLFilters(dateRange=DateRange(date_from="-7d")),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    def test_replace_filters_date_to_with_explicit_date(self, date_to: Optional[str], expected: str):
        with freeze_time("2020-02-15T13:37:42Z"):
            select = replace_filters(
                self._parse_select("SELECT event FROM events WHERE timestamp <= {filters.dateRange.to}"),
                InsightsQLFilters(dateRange=DateRange(date_from="2020-02-01", date_to=date_to, explicitDate=True)),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    @parameterized.expand(
        [
            ("no_date_range", InsightsQLFilters(properties=[])),
            ("empty_date_range", InsightsQLFilters(dateRange=DateRange())),
            ("rolling_date_from", InsightsQLFilters(dateRange=DateRange(date_from="-1h"))),
            ("all_time_date_from", InsightsQLFilters(dateRange=DateRange(date_from="all"))),
        ]
    )
    def test_replace_filters_date_to_placeholder_skipped_when_open_ended(self, _name: str, filters: InsightsQLFilters):
        select = replace_filters(
            self._parse_select("SELECT event FROM events WHERE timestamp <= {filters.dateRange.to}"),
            filters,
            self.team,
    ...
    def test_replace_filters_all_time_stays_unbounded(self):
        ...
        with freeze_time("2020-02-15T13:37:42Z"):
            select = replace_filters(
                self._parse_select("SELECT event FROM events where {filters}"),
                InsightsQLFilters(dateRange=DateRange(date_from="all")),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
    ...
    def test_replace_filters_this_month_preset(self):
        ...
        with freeze_time("2020-02-15T13:37:42Z"):
            select = replace_filters(
                self._parse_select("SELECT event FROM events where {filters}"),
                InsightsQLFilters(dateRange=DateRange(date_from="mStart")),
                self.team,
            )
        self.assertEqual(
            self._print_ast(select),
