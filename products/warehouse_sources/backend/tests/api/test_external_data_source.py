...
from insights.models.integration import ERROR_TOKEN_REFRESH_FAILED, Integration, OauthIntegration
...
class TestExternalDataSource(APIBaseTest):
    ...
    @parameterized.expand(
        [
            # The MCP tool injects `mcp`; a wizard or Insights Desktop user agent upgrades it.
            ("insights/wizard/1.0.0", ExternalDataSource.CreatedVia.MCP, ExternalDataSource.CreatedVia.WIZARD),
            ("insights/code 1.2.3", ExternalDataSource.CreatedVia.MCP, ExternalDataSource.CreatedVia.SELF_DRIVING),
            # The wizard's self-driving program marks its UA distinctly → self_driving, not plain wizard.
            (
                "insights/wizard; version: 1.0.0; program: self-driving",
                ExternalDataSource.CreatedVia.MCP,
            ...
            (
                "insights/mcp-server; version: 1.0.0; for insights/code",
                ExternalDataSource.CreatedVia.MCP,
                ExternalDataSource.CreatedVia.SELF_DRIVING,
            ),
            # Plain MCP clients keep the machine-injected value.
            (
                "insights/mcp-server; version: 1.0.0",
                ExternalDataSource.CreatedVia.MCP,
                ExternalDataSource.CreatedVia.MCP,
            ),
            # Explicit non-MCP values are never rewritten, even with an upgrading user agent.
            ("insights/wizard/1.0.0", ExternalDataSource.CreatedVia.WEB, ExternalDataSource.CreatedVia.WEB),
            ("insights/wizard/1.0.0", ExternalDataSource.CreatedVia.API, ExternalDataSource.CreatedVia.API),
            ("insights/code 1.2.3", ExternalDataSource.CreatedVia.WEB, ExternalDataSource.CreatedVia.WEB),
            ("insights/code 1.2.3", ExternalDataSource.CreatedVia.API, ExternalDataSource.CreatedVia.API),
        ]
    )
    @patch(
        "products.warehouse_sources.backend.temporal.data_imports.sources.stripe.source.StripeSource.validate_credentials",
    ...
    @parameterized.expand(
        [
            ...
            # UA in X-Insights-Mcp-User-Agent. The self-driving marker lives there, not on User-Agent.
            (
    ...
    def test_create_external_data_source_self_driving_via_proxied_mcp_user_agent(
        ...
        response = self.client.post(
            ...
            headers={
                "user-agent": "insights/mcp-server; version: 1.0.0; for insights/wizard",
                "x-insights-mcp-user-agent": mcp_user_agent,
            },
        )
    ...
    @parameterized.expand(
        [
            ...
            # self-label as wizard- or Insights Desktop-created.
    ...
    def test_patch_external_data_source_preserves_cdc_config_when_schema_cleared(self, _mock_validate):
        source = ExternalDataSource.objects.create(
            ...
            job_inputs={
                ...
                "schema": "public",
                "cdc_enabled": True,
                "cdc_management_mode": "insights",
                "cdc_slot_name": "insights_slot",
                "cdc_publication_name": "insights_pub",
        ...
        assert source.job_inputs["cdc_management_mode"] == "insights"
        assert source.job_inputs["cdc_slot_name"] == "insights_slot"
        assert source.job_inputs["cdc_publication_name"] == "insights_pub"
    ...
    def test_bulk_update_schemas(self, _mock_workflow_exists):
        ...
        response = self.client.patch(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.id}/bulk_update_schemas",
            data={
                "schemas": [
    ...
    def test_source_prefix_validation(
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/source_prefix/",
    ...
    def test_can_recreate_no_prefix_source_after_deletion(self, _mock_validate):
        ...
        delete_response = self.client.delete(f"/v1/environments/{self.team.pk}/external_data_sources/{source_id}")
    ...
    def test_list_external_data_source_query_count_does_not_scale_with_sources(self):
        ...
        self._create_external_data_source()
        self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/")
    ...
    def test_list_omits_table_columns_but_retrieve_includes_them(self):
        ...
        table = DataWarehouseTable.objects.create(
            ...
            columns={"id": {"datastore": "Int32", "insightsql": "integer", "valid": True}},
        )
        ExternalDataSchema.objects.create(
            team_id=self.team.pk,
            source_id=source.pk,
        ...
        # InsightsQL field serialization and returns an empty column list.
        list_payload = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/").json()
        ...
        retrieve_payload = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}").json()
    ...
    def test_list_external_data_source_search(self, _name, term, expected_prefixes):
        ...
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/?search={term}")
    ...
    def test_connections_returns_lightweight_direct_connection_options(self):
        ...
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/connections/")
    ...
    def test_connections_lists_synced_sources_only_when_direct_query_enabled(self):
        ...
        self.assertEqual(payload[0]["supports_insightsql"], True)

    def test_direct_connection_options_lists_every_direct_capable_source_type(self):
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/direct_connection_options/")
        ...
        self.assertTrue({"Postgres", "MySQL", "Snowflake", "Redshift", "Datastore"}.issubset(source_types))
        self.assertNotIn("Stripe", source_types)

        datastore = next(option for option in payload if option["source_type"] == "Datastore")
        self.assertEqual(datastore["label"], "Datastore")
        self.assertIsNotNone(datastore["icon_path"])
        self.assertTrue(datastore["icon_path"].endswith("datastore.png"))
    ...
    def test_system_managed_source_schema_rejects_update(self):
        ...
        response = self.client.patch(
            f"/v1/environments/{self.team.pk}/external_data_schemas/{schema.pk}/",
    ...
    def test_refresh_schemas_idempotent_no_duplicates(self, mock_get_source):
        ...
        self.client.post(f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/refresh_schemas/")
    ...
    def test_reload_direct_external_data_source_refreshes_schemas(self, mock_trigger, mock_get_source):
        ...
        mock_get_source.return_value.get_schemas.return_value = [
            SourceSchema(
                ...
                foreign_keys=[("user_id", "insights_user", "id")],
            ),
        ]
        source = ExternalDataSource.objects.create(
            team_id=self.team.pk,
        ...
        self.assertEqual(
            ...
            [{"column": "user_id", "target_table": "insights_user", "target_column": "id"}],
        )
    ...
    def test_create_direct_postgres_preserves_numeric_as_decimal(self, mock_get_source):
        ...
        self.assertEqual(table.columns["amount"]["datastore"], "Decimal")
        self.assertEqual(table.columns["amount"]["insightsql"], "numeric")
    ...
    def test_create_direct_postgres_rejects_the_managed_warehouse_name(self):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json() == {"message": "This source name is reserved by Insights."}
    ...
    def test_refresh_schemas_renames_legacy_direct_query_rows(self, mock_get_source):
        ...
        legacy_table = DataWarehouseTable.objects.create(
            ...
            url_pattern=DIRECT_POSTGRES_URL_PATTERN,
            external_data_source=source,
            columns={"id": {"insightsql": "IntegerDatabaseField", "datastore": "Int64"}},
        )
    ...
    def test_create_direct_unsupported_source_type_is_rejected(self):
        ...
        self.assertEqual(
            response.json(),
            {
                "message": "Direct query mode is currently supported only for Postgres, MySQL, Snowflake, Redshift, and Datastore sources."
            },
        )
    ...
    def test_internal_postgres(
        ...
        with override_settings(CLOUD_DEPLOYMENT="US"):
            ...
            self.assertEqual(
                response.json(),
                {
                    "message": "This host points to an internal or private IP address, which Insights can't reach. "
    ...
    def test_blocks_internal_host(self, host, _patch_schemas):
        database_schema_url = f"/v1/environments/{self.team.pk}/external_data_sources/database_schema/"
        ...
        create_url = f"/v1/environments/{self.team.pk}/external_data_sources/"
        create_data = {
            "source_type": "Postgres",
            "created_via": "web",
            "payload": {
    ...
    def test_source_jobs_schema_filter(self, _name, query_string, expected_count):
        ...
        response = self.client.get(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/jobs{query_string}",
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_setup_custom_oauth2_source_stores_pointer_not_secrets(
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/setup/",
            data={
                "source_type": "Custom",
    ...
    def test_update_blocks_internal_host_with_credentials(self):
        ...
        assert response.json()["detail"] == (
            "This host points to an internal or private IP address, which Insights can't reach. "
            "Use a host that's reachable from the public internet. "
            "If your database isn't publicly reachable, connect through an SSH tunnel."
        )
    ...
    def test_update_direct_postgres_schema_filter_preserves_selected_table_for_same_physical_schema(
        ...
        table = DataWarehouseTable.objects.create(
            ...
            columns={"id": {"datastore": "Int32", "insightsql": "integer", "valid": True}},
            options={
                "direct_postgres_schema": "insights",
        ...
        source_join = DataWarehouseJoin.objects.create(
            team=self.team,
            source_table_name="insights.events",
            source_table_key="id",
        ...
        joining_join = DataWarehouseJoin.objects.create(
            ...
            source_table_key="id",
            joining_table_name="insights.events",
            joining_table_key="id",
            field_name="insights_events",
        )
        ...
        assert source.job_inputs["schema"] == "insights"
        ...
        assert table.options["direct_postgres_schema"] == "insights"
    ...
    def test_get_wizard_sources_filtered_by_source_type(self, _name, query, expected_keys):
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/wizard{query}")
        assert response.status_code == 200
    ...
    def test_get_wizard_sources_unknown_source_type_returns_400(self):
        response = self.client.get(
            f"/v1/environments/{self.team.pk}/external_data_sources/wizard?source_type=NotARealSource"
        )
        assert response.status_code == 400
    ...
    def test_unresolved_secret_ref_rejected(self, _name, suffix, body):
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{suffix}",
            data=body,
            format="json",
        )
        assert response.status_code == 400
...
class TestCreateWebhook(APIBaseTest):
    ...
    def test_create_webhook_success(self, mock_create_webhook):
        ...
        self._create_insights_function_template()
        source = self._create_stripe_source()
        schema = self._create_webhook_schema(source, STRIPE_CUSTOMER_RESOURCE_NAME)

        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/create_webhook/"
        )
        ...
        insights_function = InsightsFunction.objects.get(team=self.team, type="warehouse_source_webhook")
        assert insights_function.template_id == "template-warehouse-source-stripe"
        assert insights_function.inputs is not None
        schema_mapping = insights_function.inputs["schema_mapping"]["value"]
        ...
        assert insights_function.inputs["source_id"]["value"] == str(source.pk)
    ...
    def test_create_webhook_external_creation_fails(self, mock_create_webhook):
        ...
        assert InsightsFunction.objects.filter(team=self.team, type="warehouse_source_webhook").exists()
    ...
    def test_create_webhook_url_uses_cloud_deployment(self, mock_create_webhook):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["webhook_url"].startswith("https://webhooks.us.hanzo.ai")

    @patch("products.warehouse_sources.backend.temporal.data_imports.sources.stripe.source.StripeSource.create_webhook")
    @override_settings(CLOUD_DEPLOYMENT=None, SITE_URL="https://my.insights.instance")
    def test_create_webhook_url_uses_site_url_for_self_hosted(self, mock_create_webhook):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["webhook_url"].startswith("https://my.insights.instance")
    ...
    def test_create_webhook_merges_schemas_on_update(self, mock_create_webhook):
        ...
        # First call: creates InsightsFunction with Charge schema
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/create_webhook/"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_create_webhook_template_not_in_db(self):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "sync_insights_function_templates" in response.json()["message"]
    ...
    def test_create_webhook_saves_extra_inputs(self, mock_create_webhook):
        ...
        assert insights_function.encrypted_inputs is not None
        assert insights_function.encrypted_inputs["signing_secret"]["value"] == "whsec_test123"
    ...
    def test_update_webhook_inputs(self, mock_create_webhook, mock_inputs_updated):
        ...
        # First create the webhook to set up the InsightsFunction
        self.client.post(f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/create_webhook/")

        # Now update the inputs manually
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/update_webhook_inputs/",
        ...
        insights_function = InsightsFunction.objects.get(team=self.team, type="warehouse_source_webhook")
        assert insights_function.encrypted_inputs is not None
        assert insights_function.encrypted_inputs["signing_secret"]["value"] == "whsec_manual123"
        ...
        assert call_args.args[1].endswith(f"/public/webhooks/dwh/{insights_function.id}")
    ...
    def test_update_webhook_inputs_requires_job_inputs(self, mock_inputs_updated):
        ...
        # we should bail out with a 400 before saving anything to the InsightsFunction.
    ...
    def test_update_webhook_inputs_no_insights_function(self, mock_inputs_updated):
        source = self._create_stripe_source()
        self._create_webhook_schema(source, STRIPE_CUSTOMER_RESOURCE_NAME)
    ...
    def test_update_webhook_inputs_partial_update_preserves_other_required_fields(self, mock_create_webhook):
        ...
        # preserving the existing value on the InsightsFunction.
        ...
        insights_function = InsightsFunction.objects.get(team=self.team, type="warehouse_source_webhook")
        assert insights_function.encrypted_inputs is not None
        assert insights_function.encrypted_inputs["signing_secret"]["value"] == "whsec_rotated"
    ...
    def test_update_webhook_inputs_rotates_secret_without_webhook_schemas(
        ...
        # stored on the InsightsFunction inputs, not per-schema. A source can legitimately
        ...
        # webhook script function authenticating deliveries.
        from products.cdp.backend.models.insights_functions.insights_function import InsightsFunction
...
class TestWebhookInfo(APIBaseTest):
    ...
    def _create_insights_function(self, source: ExternalDataSource, enabled: bool = True):
        from products.cdp.backend.models.insights_functions.insights_function import InsightsFunction

        return InsightsFunction.objects.create(
            team=self.team,
            name="Stripe warehouse source webhook",
            type="warehouse_source_webhook",
            script="// test",
    ...
    def test_webhook_info_non_webhook_source(self):
        ...
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/webhook_info/")

        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["supports_webhooks"] is False

    def test_webhook_info_no_insights_function(self):
        source = self._create_stripe_source()
    ...
    def test_webhook_info_with_insights_function(self, mock_get_info):
        ...
        mock_get_info.return_value = ExternalWebhookInfo(
            exists=True,
            url="https://webhooks.us.hanzo.ai/public/webhooks/dwh/test-id",
            status="enabled",
            enabled_events=["charge.created", "charge.updated"],
            description="Insights data warehouse webhook",
        )

        source = self._create_stripe_source()
        insights_function = self._create_insights_function(source)
        ...
        assert data["insights_function"]["id"] == str(insights_function.id)
        assert data["insights_function"]["enabled"] is True
    ...
    def test_webhook_info_reports_missing_events(self, mock_get_info, mock_desired):
        ...
        source = self._create_stripe_source()
        self._create_insights_function(source)
    ...
    def test_webhook_info_source_returns_none(self):
        ...
        with patch(
            ...
            response = self.client.get(
                f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/webhook_info/"
            )
    ...
    def test_webhook_info_masks_set_secret_input(self, _mock_info):
        ...
        source = self._create_stripe_source()
        insights_function = self._create_insights_function(source)
        insights_function.inputs_schema = [
            *insights_function.inputs_schema,
            {"type": "string", "key": "signing_secret", "label": "Signing secret", "required": True, "secret": True},
        ]
        insights_function.encrypted_inputs = {"signing_secret": {"value": "whsec_existing"}}
        insights_function.save()
        # Sanity check the model routed the secret correctly.
        insights_function.refresh_from_db()
        assert (
            InsightsFunction.objects.get(pk=insights_function.pk).encrypted_inputs["signing_secret"]["value"]
            == "whsec_existing"
        )
...
class TestDeleteWebhook(APIBaseTest):
    ...
    def test_delete_webhook_success(self, mock_delete_webhook):
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/delete_webhook/"
        )
        ...
        insights_function.refresh_from_db()
        assert insights_function.deleted is True
        assert insights_function.enabled is False
    ...
    @patch("products.warehouse_sources.backend.temporal.data_imports.sources.stripe.source.StripeSource.delete_webhook")
    def test_delete_webhook_external_fails_still_deletes_insights_function(self, mock_delete_webhook):
        from products.warehouse_sources.backend.temporal.data_imports.sources.common.base import WebhookDeletionResult
    ...
    def test_delete_webhook_no_insights_function(self):
        source = self._create_stripe_source()
    ...
    def test_delete_webhook_no_job_inputs_still_cleans_up_insights_function(self):
        source = self._create_stripe_source(job_inputs={})
        insights_function = self._create_insights_function(source)
...
class TestDestroySourceCleansUpWebhook(APIBaseTest):
    @patch("products.warehouse_sources.backend.temporal.data_imports.sources.stripe.source.StripeSource.delete_webhook")
    def test_destroy_source_deletes_webhook_and_insights_function(self, mock_delete_webhook):
        from products.cdp.backend.models.insights_functions.insights_function import InsightsFunction
        from products.warehouse_sources.backend.temporal.data_imports.sources.common.base import WebhookDeletionResult
...
def _make_postgres_source(
    ...
    *,
    ...
    management_mode: str = "insights",
    slot_name: str = "insights_slot",
    pub_name: str = "insights_pub",
...
class TestCheckCDCPrerequisitesForSource(APIBaseTest):
    def test_rejects_source_type_without_cdc_support(self) -> None:
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/check_cdc_prerequisites_for_source/",
            data={"cdc_management_mode": "insights"},
            format="json",
        )
        assert response.status_code == 400
...
class TestCheckCDCPrerequisitesWizard(APIBaseTest):
    ...
    def _post(self, **overrides):
        return self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/check_cdc_prerequisites/",
...
class TestEnableCDC(APIBaseTest):
    ...
    def test_enable_cdc_rejects_source_type_without_cdc_support(self, _flag) -> None:
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/enable_cdc/",
            data={"cdc_management_mode": "insights"},
            format="json",
        )
        assert response.status_code == 400
    ...
    def test_enable_cdc_insights_managed_success(
        self,
        mock_ensure_cleanup,
        ...
        def setup_cdc_slot(_adapter, source_model, payload):
            ...
            job_inputs.update(
                {
                    "cdc_enabled": True,
                    "cdc_management_mode": payload.get("cdc_management_mode", "insights"),
                    "cdc_slot_name": payload.get("cdc_slot_name") or f"insights_{source_model.id.hex[:12]}",
                    "cdc_publication_name": (
                        payload.get("cdc_publication_name") or f"insights_pub_{source_model.id.hex[:12]}"
                    ),
        ...
        assert ji["cdc_management_mode"] == "insights"
            ...
            job_inputs.update(
                {
                    "cdc_enabled": True,
                    "cdc_management_mode": "self_managed",
                    "cdc_slot_name": f"insights_{source_model.id.hex[:12]}",
                    "cdc_publication_name": payload.get("cdc_publication_name") or "insights_pub",
    ...
    def test_enable_cdc_insights_rolls_back_partial_slot_on_failure(
        self,
        mock_cdc_pg_connection,
...
class TestDisableCDC(APIBaseTest):
    def test_disable_cdc_rejects_source_type_without_cdc_support(self) -> None:
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/disable_cdc/",
        )
        assert response.status_code == 400
        assert "CDC is not supported" in response.json()["message"]
...
class TestRepairCDC(APIBaseTest):
    def _repair(self, source: ExternalDataSource):
        return self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/repair_cdc/",
        )
...
class TestUpdateCDCSettings(APIBaseTest):
    def test_update_cdc_settings_rejects_source_type_without_cdc_support(self) -> None:
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/update_cdc_settings/",
...
class TestCDCJobInputsExposure(APIBaseTest):
    def test_retrieve_exposes_cdc_fields_but_not_password(self) -> None:
        ...
        assert job_inputs["cdc_management_mode"] == "insights"
        assert job_inputs["cdc_slot_name"] == "insights_slot"
        assert job_inputs["cdc_publication_name"] == "insights_pub"
...
class TestCDCStatus(APIBaseTest):
    def test_rejects_source_type_without_cdc_support(self) -> None:
        ...
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/cdc_status/")
        assert response.status_code == 400
        assert "CDC is not supported" in response.json()["message"]
    ...
    def test_returns_live_status_when_enabled(self, mock_get_status, _mock_paused) -> None:
        ...
        assert body["management_mode"] == "insights"
        assert body["slot_name"] == "insights_slot"
        assert body["publication_name"] == "insights_pub"
...
class TestResumeCDC(APIBaseTest):
    def _resume(self, source: ExternalDataSource):
        return self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{source.pk}/resume_cdc/",
        )
...
class TestExternalDataSourceConnectLink(APIBaseTest):
    def _connect_link(self, source_type: str):
        return self.client.get(
            f"/v1/environments/{self.team.pk}/external_data_sources/connect_link?source_type={source_type}"
        )
    ...
    def test_connect_link_missing_source_type(self):
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/connect_link")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestExternalDataSourceSetup(APIBaseTest):
    # Stripe enables revenue analytics, whose post-create view sync builds the InsightsQL Database — patched
    ...
    def test_setup_auto_registers_webhook_and_switches_capable_tables(
        ...
        insights_function = InsightsFunction.objects.get(team=self.team, type="warehouse_source_webhook", deleted=False)
        assert insights_function.enabled is True
        assert insights_function.inputs is not None
        assert insights_function.inputs["source_id"]["value"] == data["id"]
    ...
    def test_setup_falls_back_to_polling_when_webhook_registration_fails(
        ...
        assert not InsightsFunction.objects.filter(
            team=self.team, type="warehouse_source_webhook", deleted=False
        ).exists()
    ...
    def test_setup_keeps_polling_defaults_when_webhook_template_missing(
        ...
        data = response.json()
        assert data["webhook"]["success"] is False
        assert "sync_insights_function_templates" in data["webhook"]["error"]
    ...
    def test_consuming_another_team_members_credential_returns_400(self, _name, endpoint):
        teammate = self._create_user("teammate@hanzo.ai")
        credential = self._store_stripe_credential(created_by=teammate)
        response = self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/{endpoint}",
            data={"source_type": "Stripe", "payload": {"credential_id": str(credential.pk)}},
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "not found" in response.json()["message"]
...
class TestExternalDataSourceStoreCredentials(APIBaseTest):
    def _store(self, source_type: str = "Stripe", payload: dict | None = None):
        return self.client.post(
            f"/v1/environments/{self.team.pk}/external_data_sources/store_credentials/",
            data={
                "source_type": source_type,
                "payload": payload
    ...
    def test_stored_credentials_list_returns_metadata_only(self, _mock_validate):
        ...
        list_response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/stored_credentials/")
    ...
    def test_stored_credentials_list_filters_by_source_type_and_hides_expired_other_teams_and_other_users(self):
        ...
        _create(self.team, "Stripe", created_by=self._create_user("teammate@hanzo.ai"))

        response = self.client.get(
            f"/v1/environments/{self.team.pk}/external_data_sources/stored_credentials/?source_type=Stripe"
        )
        assert response.status_code == status.HTTP_200_OK
        results = response.json()
    ...
    def test_stored_credentials_list_orders_newest_first(self):
        ...
        response = self.client.get(f"/v1/environments/{self.team.pk}/external_data_sources/stored_credentials/")
        assert response.status_code == status.HTTP_200_OK
...
class TestOAuthAccountsEndpoint(APIBaseTest):
    ...
    def _url(self, source_type: str, integration_id: int) -> str:
        return (
            f"/v1/environments/{self.team.pk}/external_data_sources/oauth_accounts/"
    ...
    def test_missing_params_returns_400(self):
        response = self.client.get(
            f"/v1/environments/{self.team.pk}/external_data_sources/oauth_accounts/?source_type=BingAds"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST, response.content
    ...
    def test_pinterest_ads_maps_accounts_with_permission_badges(self):
        ...
        listed = [
            {"id": "549770029420", "name": "Insights Inc", "permissions": ["OWNER"]},
    ...
    def test_tiktok_ads_maps_advertisers_to_accounts(self):
        integration = self._tiktok_integration()
        advertisers = [
            {"advertiser_id": "7554133187111469074", "advertiser_name": "Insights0925"},
            {"advertiser_id": "7554135782433308688", "advertiser_name": "Insights Inc"},
        ]
    ...
    def test_snapchat_maps_accounts_grouped_by_organization(self):
        ...
        listed = [
            ({"id": "acc-1", "name": "Insights Self Service", "status": "PENDING"}, "Insights"),
            ({"id": "acc-2", "name": "Insights", "status": "ACTIVE"}, "Insights"),
        ]
        with (
        ...
        assert response.json()["accounts"] == [
            {
                "value": "acc-1",
                "display_name": "Insights Self Service",
                "is_primary": False,
                "badges": ["Pending"],
                "group": "Insights",
                "secondary_text": None,
    ...
    def test_github_lists_repositories_with_search(self):
        ...
        with patch(gh_path) as mock_gh:
            mock_gh.return_value.list_cached_repositories.return_value = ([{"full_name": "Insights/insights"}], False)
            response = self.client.get(self._url("Github", integration.id) + "&search=insights")

        assert response.status_code == status.HTTP_200_OK, response.content
        assert [a["value"] for a in response.json()["accounts"]] == ["Insights/insights"]
        mock_gh.return_value.list_cached_repositories.assert_called_once_with(search="insights", limit=100, offset=0)
...
class TestExternalDataSourcePreviewAndCustomPayload(APIBaseTest):
    def _url(self, action: str) -> str:
        return f"/v1/environments/{self.team.pk}/external_data_sources/{action}/"
...
class TestGithubMultiRepoPatch(APIBaseTest):
    ...
    def test_patch_repo_change_reconciles_webhooks_and_prunes_mapping(self, _mock_validate, mock_ensure, mock_delete):
        ...
        insights_function.refresh_from_db()
        assert insights_function.inputs is not None
        mapping = insights_function.inputs["schema_mapping"]["value"]
