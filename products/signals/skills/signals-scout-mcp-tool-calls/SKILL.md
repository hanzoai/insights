...
description: >
  Signals scout for Insights MCP tool calls. Watches $mcp_tool_call telemetry for tools that
...
compatibility: >
  ...
  insights:exploring-mcp-tool-quality and insights:querying-insights-data.
allowed_tools:
  - emit_report
  - edit_report
metadata:
...
MCP tool calls land on the `$mcp_tool_call` event, emitted by both Insights's own hono server **and** external customer servers instrumented with the SDK. Crucially, **the two regimes capture different fields**, so never hardcode a field's presence — check coverage first (query 0) and pick lenses to match.
...
- **`$mcp_error_type` existing ≠ failures being classified.** Even on Insights's own hono data, most `$mcp_is_error` failures are _tool-result_ errors (the handler returned `{isError:true}`) that never get a class — `error_type` stays `'None'`. On Insights's own project only ~4% of failures carry a real class. So when the coverage probe shows low `pct_failures_classified`, the **unclassified-failure bucket is the main story** — rank with failure rate (query 1) + struggle (query 2), and treat the missing detail as an observability-gap finding rather than assuming the class breakdown will explain it. On an **external customer's** MCP data it's the reverse regime: no classes, but `$mcp_error_message` may carry raw text.

The full SQL cookbook is in [`references/queries.md`](references/queries.md) — read it rather than reinventing the queries. Also read `insights:exploring-mcp-tool-quality` and `insights:querying-insights-data` (both baked into the sandbox; `models-mcp` is the schema source of truth) when you go deep.

## Quick close-out: is MCP even in use?

If `$mcp_tool_call` is absent from the profile's `top_events` (or a 7-day `count()` is ~0), this project isn't using the Insights MCP. Write one scratchpad entry and stop:
...
- **Measurement** — the exact InsightsQL to re-run (the cookbook query you used, inlined with the category/tool filters baked in — never a reference to "query 1"), plus the fastest local proxy: reproducing the cited failing calls against a dev MCP server, or the affected handler's unit tests.
...
- **Errors during a known Insights incident** — an `api_5xx` surge across _many_ tools at once is an upstream outage, not a per-tool bug; check timing before attributing it to one tool.
...
Deep-dive skills baked into the sandbox: `insights:exploring-mcp-tool-quality`, `insights:exploring-mcp-tool-usage`, `insights:querying-insights-data`.
