...
import { IconChevronDown, IconCopy, IconLogomark, IconSparkles } from '@hanzo/icons'
...
export function buildInsightsCodeDeepLink(prompt: string, repository?: string): string {
    const repoParam = repository ? `&repo=${encodeURIComponent(repository)}` : ''
    return `insights-code://new?prompt=${encodeURIComponent(prompt)}${repoParam}`
}
...
const AGENTS: AgentDef[] = [
    ...
    },
    {
        ...
        verb: 'Open',
        open: (prompt, { repository }) => window.open(buildInsightsCodeDeepLink(prompt, repository), '_blank'),
    },
    {
        key: 'claude-code',
        name: 'Claude Code',
