...
// `bin/start --detach` returns — useful when `insightscli wait` is run immediately
// after.
...
func printDockerTeardownHint() {
	...
	fmt.Println("Full teardown: insightscli docker:services:down (also wipe volumes: insightscli docker:services:remove)")
}
