...
# multinode stack (see tools/infra-scripts/datastore-multinode/) into one HCL
...
#   DATASTORE_USER / DATASTORE_PASSWORD  credentials (default: default / empty).
set -euo pipefail

HCL=insights/datastore/hcl
EXCLUDE="$HCL/exclude.hcl"
ENV="${VERIFY_LIVE_ENV:-local-multi}"
CH_USER="${DATASTORE_USER:-default}"
CH_PASSWORD="${DATASTORE_PASSWORD:-}"
...
ROLES=(
  "data      localhost 9000 insights"
  "ops       localhost 9300 insights"
  "logs      localhost 9500 insights"
  "ai_events localhost 9100 insights"
  "aux       localhost 9200 insights"
  "sessions  localhost 9400 insights"
)

# Same pin as bin/hclexp; override for a single run via $HCLEXP_IMAGE.
HCLEXP_IMAGE="${HCLEXP_IMAGE:-$(cat "$HCL/bin/image.txt")}"

# hclexp that can reach Datastore on the host's published ports. Prefer a native
