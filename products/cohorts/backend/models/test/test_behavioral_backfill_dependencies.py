...
class TestBehavioralBackfillDependencies(BaseTest):
    def setUp(self) -> None:
        ...
        feature_patch = mock.patch(
            "products.cohorts.backend.models.dependencies.hanzo_insights.feature_enabled", return_value=False
        )
    ...
    def test_flag_off_still_nulls_readiness_without_enqueue(self) -> None:
        ...
        Cohort.objects.filter(id=cohort.id).update(last_backfill_events_at=timezone.now())
        cohort.refresh_from_db()
        with mock.patch("insights.tasks.calculate_cohort.trigger_cohort_events_backfill_task.apply_async") as enqueue:
            cohort.filters = self._filters(30)
            cohort.save()
    ...
    def test_person_only_edit_preserves_events_readiness(self) -> None:
        ...
        with (
            ...
            mock.patch("insights.tasks.calculate_cohort.trigger_cohort_backfill_task.apply_async") as person_enqueue,
            mock.patch(
                "insights.tasks.calculate_cohort.trigger_cohort_events_backfill_task.apply_async"
            ) as event_enqueue,
        ):
    ...
    def test_two_edits_share_the_events_debounce_key(self) -> None:
        ...
        with (
            mock.patch("products.cohorts.backend.models.dependencies.get_redis_client", return_value=redis),
            mock.patch("insights.tasks.calculate_cohort.trigger_cohort_events_backfill_task.apply_async") as enqueue,
        ):
