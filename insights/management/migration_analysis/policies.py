        ...
        for field_name, field in op.fields:
            ...
            if field_type in ["AutoField", "BigAutoField"]:
                return [
                    ...
                    "Use `from insights.models.utils import UUIDModel` and inherit from UUIDModel."
                ]
...
class AtomicFalsePolicy(MigrationPolicy):
    ...
    CONCURRENT_OP_TYPES = {
        ...
        # Insights helpers (see insights/migration_helpers/concurrent_index.py)
        ...
        if not is_atomic and not has_concurrent:
            ...
            if label not in self._acknowledged_migrations():
                violations.append(
                    ...
                    "insights/management/migration_analysis/atomic_false_acknowledged_migrations.txt "
                    "to accept the risk."
                )
...
class ConcurrentIndexIdempotencyPolicy(MigrationPolicy):
    ...
    # The Insights helpers in insights/migration_helpers/concurrent_index.py
    ...
    GUIDANCE = (
        "Use insights.migration_helpers.SafeAddIndexConcurrently (or\n"
        ...
        "\n"
        "See https://github.com/Insights/insights/blob/master/docs/published/handbook/engineering/safe-django-migrations.md#adding-indexes"
    )
...
class HotTableAlterPolicy(MigrationPolicy):
    ...
    Rationale:
    - insights_team, insights_user, insights_organization, and insights_project are
    ...
    - Most new team fields should not be on Team at all: a Team extension
      model (insights/models/team/README.md) only creates a new table, which
      takes no lock on insights_team.
    ...
    FIELD_LEVEL_OPS = {
        ...
        # AddForeignKeyNotValid(model_name=<hot table>) emits ALTER TABLE insights_*
    ...
    _ALTER_HOT_TABLE = re.compile(
        ...
        r'"?(insights_team|insights_user|insights_organization|insights_project)"?\b',
        re.IGNORECASE,
    )
    ...
    def _hot_table_target(self, op, app_label: str) -> str | None:
        ...
        # The hot models all live in the insights app; same-named models in
        ...
        if app_label == "insights":
            ...
            if model_name and model_name.lower() in self.HOT_MODELS:
                return f"insights_{model_name.lower()}"
    ...
    def _resolve_fk_target_table(self, target) -> str | None:
        """Resolve a FK target to a hot insights_* table name, or None.

        `field.remote_field.model` is a string label like "insights.team" in
        migration state. settings.AUTH_USER_MODEL (the swappable user FK) desugars
        to that same "insights.user" string at serialization time, so both the
        explicit and swappable forms land here as strings.
        """
        if target == settings.AUTH_USER_MODEL:
            target = "insights.user"
        if not isinstance(target, str) or "." not in target:
            return None
        app, _, model_name = target.rpartition(".")
        if app.lower() == "insights" and model_name.lower() in self.HOT_MODELS:
            return f"insights_{model_name.lower()}"
        return None
    ...
    def _violation(self, op, table: str, label: str) -> str:
        return (
            ...
            "extension model (see insights/models/team/README.md), which only creates a new table.\n"
            f'If this change genuinely must alter {table}, add "{label}" to '
            "insights/management/migration_analysis/hot_table_acknowledged_migrations.txt to accept the "
            "risk, and coordinate the deploy with #team-infrastructure for a low-traffic window.\n"
            "See https://github.com/Insights/insights/blob/master/docs/published/handbook/engineering/safe-django-migrations.md#altering-hot-tables"
        )
    ...
    def _fk_violation(self, op, table: str, label: str) -> str:
        return (
            ...
            "as a DB constraint via insights.migration_helpers.AddForeignKeyNotValid in a later migration and "
            ...
            "insights/management/migration_analysis/hot_table_acknowledged_migrations.txt to accept the "
            "risk, and coordinate the deploy with #team-infrastructure for a low-traffic window.\n"
            "See https://github.com/Insights/insights/blob/master/docs/published/handbook/engineering/safe-django-migrations.md#foreign-keys-to-hot-tables"
        )
...
# hand-roll it - it's to ship a drop-in helper in insights/migration_helpers that bakes
