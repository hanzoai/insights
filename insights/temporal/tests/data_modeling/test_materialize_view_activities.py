...
from insights.insightsql.resolver import ResolverFactory

from insights.sync import database_sync_to_async
from insights.temporal.data_modeling.activities import (
...
from insights.temporal.data_modeling.activities.materialize_view import (
    LOGGER,
...
async def _make_job(ateam, saved_query, status, *, engine=DataModelingJobEngine.DATASTORE, error=None):
...
class TestFailMaterializationActivity:
    ...
    async def test_suspends_node_after_consecutive_failures(
        ...
        from insights.temporal.data_modeling.activities.utils import is_node_suspended
        ...
        await database_sync_to_async(anode.refresh_from_db)()
        assert is_node_suspended(anode, DataModelingJobEngine.DATASTORE) is True
    ...
    async def test_timeout_does_not_pause_schedule_with_fewer_than_5_previous_jobs(
        ...
        with unittest.mock.patch(
            "insights.temporal.data_modeling.activities.fail_materialization.pause_saved_query_schedule"
        ) as mock_pause:
            await activity_environment.run(fail_materialization_activity, inputs)
...
class TestShouldPauseScheduleForTimeout:
    async def test_returns_false_when_fewer_than_5_previous_jobs(self, ateam, asaved_query):
        from insights.temporal.data_modeling.activities.fail_materialization import should_pause_schedule_for_timeout
    ...
    async def test_streak_ignores_jobs_from_other_engines(self, ateam, asaved_query):
        ...
        # a more recent duckgres failure must not break the datastore timeout streak
        jobs.append(
            await _make_job(
...
class TestNodeSuspension:
    async def test_suspends_for_engine_after_consecutive_failures(self, ateam, anode, asaved_query, adag):
        from insights.temporal.data_modeling.activities.utils import (
        ...
        suspended = await maybe_suspend_node_for_engine(
            ...
            engine=DataModelingJobEngine.DATASTORE,
            reason="boom",
            job_id=str(job.id),
        )
    ...
    async def test_does_not_suspend_when_latest_run_succeeded(self, ateam, anode, asaved_query, adag):
        from insights.temporal.data_modeling.activities.utils import is_node_suspended, maybe_suspend_node_for_engine
        ...
        await database_sync_to_async(anode.refresh_from_db)()
        assert is_node_suspended(anode, DataModelingJobEngine.DATASTORE) is False
    ...
    async def test_does_not_restamp_when_already_suspended(self, ateam, anode, asaved_query, adag):
        from insights.temporal.data_modeling.activities.utils import maybe_suspend_node_for_engine
    ...
    async def test_engine_suspension_is_independent(self, ateam, anode, asaved_query, adag):
        ...
        assert await maybe_suspend_node_for_engine(engine=DataModelingJobEngine.DATASTORE, **kwargs) is False
    ...
    async def test_clear_suspension_only_affects_one_engine(self, ateam, anode, adag):
        ...
        mark_node_suspended(anode, engine=DataModelingJobEngine.DATASTORE, reason="x", job_id="j1")
...
class TestSucceedMaterializationActivity:
    ...
    async def test_clears_datastore_suspension_on_success(self, activity_environment, ateam, anode, ajob, adag):
        from insights.temporal.data_modeling.activities.utils import is_node_suspended, mark_node_suspended

        mark_node_suspended(anode, engine=DataModelingJobEngine.DATASTORE, reason="x", job_id="old")
...
class TestPrepareQueryableTableActivity:
    async def test_creates_warehouse_table_from_saved_query(self, activity_environment, ateam, asaved_query, ajob):
        ...
        with (
            unittest.mock.patch(
                "insights.temporal.data_modeling.activities.prepare_queryable_table.prepare_s3_files_for_querying"
            ) as mock_prepare,
            unittest.mock.patch(
                "insights.temporal.data_modeling.activities.prepare_queryable_table.create_table_from_saved_query"
...
class TestMaterializeViewActivity:
    ...
    async def test_materializes_view_to_delta_table(
        ...
        with (
            ...
            unittest.mock.patch(
                "insights.temporal.data_modeling.activities.materialize_view.insightsql_table", mock_insightsql_table
            ),
        ):
            inputs = MaterializeViewInputs(
                team_id=ateam.pk,
    ...
    async def test_preserves_column_casing_for_non_nullable_columns_across_batches(
        ...
        # regression: Datastore emits NON-nullable columns for expressions, constants,
    ...
    async def test_zero_row_materialization_writes_empty_parquet(
        ...
        with (
            ...
            # Datastore rejects a parquet containing a 0-row row group, so the file must be metadata-only
            s3 = get_s3_client()
...
class TestHogqlTableEmptyResults:
    async def test_zero_row_query_uses_the_initial_stream_schema(self, ateam):
        ...
        with unittest.mock.patch(
            "insights.temporal.data_modeling.activities.materialize_view.get_datastore_client", fake_get_client
        ):
            batches = [batch async for batch in insightsql_table("SELECT 1", ateam, LOGGER.bind())]
...
class TestHogqlTableResolutionDeadline:
    async def test_slow_describe_does_not_exhaust_the_resolution_deadline(self, ateam):
        ...
        with (
            ...
            unittest.mock.patch(
                "insights.temporal.data_modeling.activities.materialize_view.bounded_resolver_factory_for_view",
                short_deadline_factory,
            ),
        ):
            batches = [batch async for batch in insightsql_table("SELECT now() AS ts", ateam, LOGGER.bind())]
