...
(and any other agent on the team's Insights MCP) can call the `signals-scout-*`
...
the standard Insights MCP plumbing.
...
# Insights's `SessionAuthentication` (not DRF's) calls `enforce_two_factor()`.
...
from insights.permissions import AccessControlPermission, APIScopePermission, get_authenticator_scopes
...
def _caller_carries_scout_internal_scope(request: Request) -> bool:
    ...
    runs per-section table scans plus the Datastore top-events aggregation and writes a
...
def _resolve_emission_report_links(
    ...
    One Datastore round-trip for the whole set: resolve every finding's `source_id` to the
    ...
    persist (child-environment requests would otherwise find none). CH/InsightsQL failures degrade to "no
    ...
    Datastore soft-delete and Postgres status can drift, and `SignalReportViewSet` hides both from its
...
class SignalScoutRunViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @validated_request(
        ...
        description=(
            ...
            "walk back past the limit. Pure Postgres, no Datastore round-trip. Capped at "
    ...
    @validated_request(
        ...
        description=(
            ...
            "Datastore round-trip rather than one query per run, which is what made the findings page "
...
class SignalProjectProfileViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @action(
        ...
        # "no scopes declared" branch in `insights/permissions.py:490` and rejects
    ...
    def current(self, request: Request, *args, **kwargs) -> Response:
        ...
        # (per-section table scans + the Datastore top-events aggregation) and write a row,
...
class SignalScoutMembersViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    all rejected (`insights/permissions.py`), and only a harness sandbox OAuth token — which carries
...
class SignalScoutConfigViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @action(
        ...
        # "no scopes declared" rejection branch in `insights/permissions.py`. Write scope:
