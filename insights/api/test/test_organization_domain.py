...
class TestOrganizationDomainsAPI(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("last_verified_domain", [], "hanzo.ai", status.HTTP_400_BAD_REQUEST),
            ("own_domain_with_another_remaining", [("other.com", True)], "hanzo.ai", status.HTTP_400_BAD_REQUEST),
    ...
    def test_cannot_delete_domain_that_would_lock_out_the_admin_under_enforcement(
        ...
        self.organization.save()
        domains = {
            "hanzo.ai": OrganizationDomain.objects.create(
                organization=self.organization, domain="hanzo.ai", verified_at=timezone.now()
            )
        }
