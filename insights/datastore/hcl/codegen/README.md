...
Turns a declarative-HCL change into a Datastore migration the existing
`infi.datastore_orm` runner executes. The HCL is the source of truth for _what_
...
```bash
python insights/datastore/hcl/codegen/gen_migration.py --name add_foo_column --auto
#   --auto            write the next insights/datastore/migrations/NNNN_<name>.py and bump max_migration.txt
...
Then: review the output, save it as the next `insights/datastore/migrations/NNNN_<name>.py`,
