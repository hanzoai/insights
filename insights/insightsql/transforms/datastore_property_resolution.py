"""Datastore pass: read each property from a faster precomputed column when one exists.
...
column. Datastore can use a skip index on a plain column, but not on a value buried inside a JSON extract or an
...
Datastore only: it runs after lowering (`prepare_ast_for_printing`) and emits Datastore-specific nodes, so it must
...
from insights.insightsql.database.models import DatabaseField, MapStringDatabaseField
...
from insights.insightsql.printer.base import resolve_field_type
from insights.insightsql.printer.datastore import AI_BLOOM_FILTER_PROPERTIES, COLUMNS_WITH_HACKY_OPTIMIZED_NULL_HANDLING
...
from insights.datastore.events_json import (
...
from insights.schema_enums import MaterializationMode, PropertyGroupsMode
...
class MaterializedPropertySource:
    ...
    column: str
    is_nullable: bool
    # The column's physical Datastore type (e.g. "Nullable(Float64)"); None means the string default. Typed columns
...
def resolve_materialized_property_source(
    field_type: ast.FieldType, property_name: str, context: InsightsQLContext
    ...
    # The materialized-column registry is keyed by the Datastore table name, which isn't always the InsightsQL name
    # (RawPersonsTable is "raw_persons" in InsightsQL but "person" in Datastore). Use the Datastore name, or person
    # properties would miss their materialized column and fall back to a slower JSON read.
    table_name = table_type.table.to_printed_datastore(context)
...
def resolve_json_subcolumn_source(
    field_type: ast.FieldType, table_name: str, field_name: str, property_name: str, context: InsightsQLContext
) -> MaterializedPropertySource | None:
    if property_name in restricted_property_keys_for_table_type(field_type.table_type, context):
        return None
...
def _synthetic_column_field(field_type: ast.FieldType, column_name: str, *, is_nullable: bool) -> ast.Field | None:
    """A typed `Field` for a physical Datastore column that the InsightsQL schema doesn't know about.

    Materialized / dmat / property-group columns exist on the table but aren't in the InsightsQL schema, so a plain `Field`
...
def _materialized_head_expr(
    ...
    *,
    ...
    # type-check in Datastore — so it is read bare as well.
...
def _map_value_read(blob: ast.Expr, key: str) -> ast.Expr:
    """`has(map, key) ? map[key] : null` for a physical Datastore Map column — a missing key reads NULL, not ''."""
    return ast.Call(
        name="if",
        args=[
...
def _substitute_value_read(node: ast.PropertyAccess, context: InsightsQLContext) -> ast.Expr | None:
    ...
    if source is None:
        ...
        # fallback would print JSONExtract, which Datastore rejects on a Map. Plain JSON blobs (events.properties)
...
# (replaceRegexpAll(nullIf(nullIf(...)))). Datastore can't use the column's skip index through that wrapping, so it
...
# inline so the rows are unchanged. The bare column is index-eligible, so Datastore can skip granules.
...
def _column_constant_type(source: MaterializedPropertySource) -> ast.ConstantType:
    """The backing column's physical Datastore type as a ConstantType; an untyped column is the string default."""
...
# Comparison compatibilities the rewrites accept: provably comparable as-is, or via a cast Datastore does cheaply.
...
def _record_range_rewrite(context: InsightsQLContext, result: str) -> None:
...
def _record_property_usage(context: InsightsQLContext, kind: str | None) -> None:
...
class DatastorePropertyResolver(CloningVisitor):
    ...
    def _property_table_in_scope(self, field_type: ast.FieldType) -> bool:
        ...
        if not self._tables_in_scope:
            # A bare fragment (within_non_insightsql_query) has no SELECT scope, and no transform that could move a read
    ...
    def visit_call(self, node: ast.Call) -> ast.Expr:
        ...
        if (
            node.name in INSIGHTSQL_COMPARISON_MAPPING
            and len(node.args) == 2
            ...
            return self.visit_compare_operation(
                ast.CompareOperation(left=node.args[0], right=node.args[1], op=INSIGHTSQL_COMPARISON_MAPPING[node.name])
            )
    ...
    def _optimize_json_has_on_events_json(self, node: ast.Call) -> ast.Expr | None:
        ...
        if not isinstance(field_type, ast.FieldType):
            return None
        table_type = _unwrap_to_table_type(field_type)
        if table_type is None or table_type.table.to_printed_datastore(self.context) not in (
            "events",
            DISTRIBUTED_EVENTS_JSON_TABLE,
        ):
            return None
        ...
        source = resolve_json_subcolumn_source(
            field_type, table_type.table.to_printed_datastore(self.context), field.name, first_key, self.context
        )
        if source is None:
            return None
    ...
    def _optimize_group_in_with_values(self, values: list[ast.Expr], prop: _OptimizableProperty) -> ast.Expr | None:
        ...
        # Datastore's transform_null_in setting makes `in(map[key], ...)` skip the keys index; the has() guard restores it.
    ...
    def _optimize_typed_materialized_range(self, node: ast.CompareOperation, op_name: str) -> ast.Expr | None:
        ...
        side stays bare. Only nullable typed columns rewrite: a non-nullable typed column stores the Datastore type
    ...
    def _optimize_materialized_in(self, node: ast.CompareOperation) -> ast.Expr | None:
        ...
        if prop.source.is_nullable:
            if node.op == ast.CompareOperationOp.In:
                # Datastore's transform_null_in makes in() hard to index; flip to has([...], col) (safe: NULL already excluded).
    ...
    def _optimize_materialized_lower_in(self, node: ast.CompareOperation) -> ast.Expr | None:
        if node.op not in (ast.CompareOperationOp.In, ast.CompareOperationOp.NotIn):
            return None
        # InsightsQL `lower` is case-insensitive on the Call name.
...
def datastore_property_resolution(node: _T_AST, context: InsightsQLContext) -> _T_AST:
    """Rewrite every backed lowered `PropertyAccess` (and comparison over it) to read its backing column.

    Datastore only. Expects a resolved, swapped, lowered AST, and runs right after `lower_property_access` in the
    Datastore print pipeline (see `prepare_ast_for_printing`).
    """
    return cast(_T_AST, DatastorePropertyResolver(context).visit(node))
