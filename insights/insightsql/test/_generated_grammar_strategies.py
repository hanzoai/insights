...
def select_strategy(depth: int = _DEFAULT_DEPTH) -> st.SearchStrategy[str]:
    ...
    def gen(draw: Any) -> str:
        ...
        if group_idx == 2:
            parts.append(draw(insightsqlxTagElement_strategy(_dec(depth))))
        if _include_optional(draw):
            parts.append(";")
        parts.append("")
        return " ".join(p for p in parts if p)
...
@functools.cache
def insightsqlxChildElement_strategy(depth: int = _DEFAULT_DEPTH) -> st.SearchStrategy[str]:
    @st.composite
    def gen(draw: Any) -> str:
        parts: list[str] = []
        alt_idx = draw(st.integers(min_value=0, max_value=2))
        ...
        if alt_idx == 1:
            parts = []
            parts.append(draw(insightsqlxText_strategy(_dec(depth))))
            return " ".join(p for p in parts if p)
        if alt_idx == 2:
            parts = []
            parts.append("{")
...
@functools.cache
def insightsqlxText_strategy(depth: int = _DEFAULT_DEPTH) -> st.SearchStrategy[str]:
    @st.composite
    def gen(draw: Any) -> str:
        parts: list[str] = []
        parts.append(draw(insightsqlx_text_token))
        return " ".join(p for p in parts if p)
...
@functools.cache
def insightsqlxTagElement_strategy(depth: int = _DEFAULT_DEPTH) -> st.SearchStrategy[str]:
    @st.composite
    def gen(draw: Any) -> str:
        parts: list[str] = []
        alt_idx = draw(st.integers(min_value=0, max_value=1))
        if alt_idx == 0:
            ...
            parts.append(draw(identifier_strategy(_dec(depth))))
            for _ in range(draw(st.integers(min_value=0, max_value=_MAX_REPEAT))):
                parts.append(draw(insightsqlxTagAttribute_strategy(_dec(depth))))
            parts.append("/>")
            return " ".join(p for p in parts if p)
        if alt_idx == 1:
            parts = []
            ...
            parts.append(">")
            for _ in range(draw(st.integers(min_value=0, max_value=_MAX_REPEAT))):
                parts.append(draw(insightsqlxChildElement_strategy(_dec(depth))))
            parts.append("</")
            parts.append(draw(identifier_strategy(_dec(depth))))
            parts.append(">")
            return " ".join(p for p in parts if p)
...
@functools.cache
def insightsqlxTagAttribute_strategy(depth: int = _DEFAULT_DEPTH) -> st.SearchStrategy[str]:
    @st.composite
    def gen(draw: Any) -> str:
        parts: list[str] = []
        alt_idx = draw(st.integers(min_value=0, max_value=2))
