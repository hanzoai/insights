...
export const LOCAL_DEV_INTERNAL_API_SECRET = 'insights123'
...
export type CommonConfig = BaseServerConfig & {
    ...
    PERSONFN_GROUPS_ROLLOUT_TEAM_IDS: string
    PERSONFN_PERSONS_ROLLOUT_PERCENTAGE: number
    PERSONFN_PERSONS_ROLLOUT_TEAM_IDS: string
    PERSONFN_TLS: boolean
    PERSONFN_TIMEOUT_MS: number
    PERSONFN_READ_MAX_BYTES: number
    PERSONFN_WRITE_MAX_BYTES: number
    PERSONFN_PING_INTERVAL_MS: number
    PERSONFN_PING_TIMEOUT_MS: number
    PERSONFN_PING_IDLE_CONNECTION: boolean
    PERSONFN_IDLE_CONNECTION_TIMEOUT_MS: number
    PERSONFN_STATE_MONITOR_POLL_INTERVAL_MS: number
    ...
    // Execute transformations on the Rust ScriptVM instead of the Node VM. Invocations the Rust VM
    // can't run (unsupported host functions, addon not built) fall back to the Node VM.
    CDP_FN_RUST_VM_EXECUTION_ENABLED: boolean

    /** Per-function wall-clock budget for an event transformation, enforced by the ScriptVM. */
    TRANSFORMATIONS_FN_TIMEOUT_MS: number
...
export function getDefaultCommonConfig(): CommonConfig {
    return {
        ...
        PERSONFN_PERSONS_ROLLOUT_PERCENTAGE: 0,
        PERSONFN_PERSONS_ROLLOUT_TEAM_IDS: '',
        PERSONFN_TLS: false,
        PERSONFN_TIMEOUT_MS: 3000,
        PERSONFN_READ_MAX_BYTES: 128 * 1024 * 1024,
        PERSONFN_WRITE_MAX_BYTES: 4 * 1024 * 1024,
        PERSONFN_PING_INTERVAL_MS: 30_000,
        PERSONFN_PING_TIMEOUT_MS: 5_000,
        PERSONFN_PING_IDLE_CONNECTION: true,
        PERSONFN_IDLE_CONNECTION_TIMEOUT_MS: 15 * 60 * 1000,
        PERSONFN_STATE_MONITOR_POLL_INTERVAL_MS: 5_000,
        ...
        INTERNAL_API_BASE_URL: isProdEnv()
            ? 'http://insights-web-django.insights.svc.cluster.local:8000'
            : 'http://localhost:8000',
        ...
        TRANSFORMATIONS_FN_TIMEOUT_MS: 300,
