...
import { getInsightsFunctionDeliveryType, redactSecretInsightsFunctionInputs } from './script-function-utils'
...
describe('redactSecretInsightsFunctionInputs', () => {
    it('redacts entry-marked secrets and leaves plain inputs untouched', () => {
        const redacted = redactSecretInsightsFunctionInputs(
            {
...
describe('getInsightsFunctionDeliveryType', () => {
    it.each([
        ...
        expect(getInsightsFunctionDeliveryType({ id })).toBe(expected)
    })
})
