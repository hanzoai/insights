...
class PostgresCDCAdapter:
    ...
    def validate_prerequisites(
        ...
        management_mode: Literal["insights", "self_managed"],
        tables: list[str],
        schema: str,
        slot_name: str | None,
    ...
    def recreate_slot(self, source: ExternalDataSource, tables: list[str]) -> dict[str, Any]:
        ...
        publication, recreating the publication first when Insights owns it and it's gone.
    ...
    def setup_resources(
        ...
        """Create the logical replication slot (and the publication, for Insights-managed
        ...
        """
        management_mode: Literal["insights", "self_managed"] = (
            "self_managed" if payload.get("cdc_management_mode") == "self_managed" else "insights"
        )
        default_slot_name = f"insights_{source.id.hex[:12]}"
        slot_name = payload.get("cdc_slot_name") or default_slot_name
        default_pub_name = "insights_pub" if management_mode == "self_managed" else f"insights_pub_{source.id.hex[:12]}"
        pub_name = payload.get("cdc_publication_name") or default_pub_name
        uses_default_insights_resources = (
            management_mode == "insights" and slot_name == default_slot_name and pub_name == default_pub_name
        )
        ...
        if management_mode == "insights":
            ...
            try:
                with cdc_pg_connection(source) as conn:
                    ...
                    # Insights state from this source. Custom-name conflicts still belong to the user/DBA.
                    if uses_default_insights_resources and slot_present:
                    ...
                    if uses_default_insights_resources and publication_present:
        ...
        # self_managed: the publication is customer-owned; Insights only creates the slot.
        try:
            with cdc_pg_connection(source) as conn:
    ...
    def cleanup_resources(self, source: ExternalDataSource) -> None:
        """Drop the Insights-managed replication slot (and publication, for Insights-managed
    ...
    def _alter_publication_membership(self, source: ExternalDataSource, schema: str, table: str, add: bool) -> None:
        cdc_config = self.parse_cdc_config(source)
        # Insights only manages the publication in insights-managed mode.
        if cdc_config.management_mode != "insights" or not cdc_config.publication_name:
            return
        try:
            with cdc_pg_connection(source) as conn:
                if add:
