...
from insights.settings import WAREHOUSE_SOURCES_DATABASE_URL
from insights.temporal.common.activity_context import current_workflow_id, current_workflow_run_id
...
SLOT_INVALIDATION_RECOVERY_MESSAGE = (
    ...
    "exceeded), so changes since the last successful sync could not be read. Insights recreated the "
...
def cleanup_orphan_slots_activity() -> None:
    ...
    1. For deleted/inactive Insights-managed sources → drop slot + publication
    2. For active sources → check WAL lag:
       - Warning threshold: log warning, update source status
       - Critical threshold (Insights-managed, safety net on): drop slot, mark error
    ...
    with HeartbeaterSync(logger=log):
        for source in sources:
            ...
            # 1. Deleted sources — drop the Temporal schedule (always; Insights-side)
            #    and Insights-managed slot/publication (only when we own them).
            if source.deleted:
                try:
                    from products.data_warehouse.backend.facade.api import delete_cdc_extraction_schedule
            ...
            if lag_mb >= critical_threshold_mb:
                ...
                if cdc_config.management_mode == "insights" and cdc_config.auto_drop_slot:
                    ...
                    try:
                        mark_cdc_broken(
                            ...
                            f"This slot is self-managed, so Insights did not drop it — reduce load or WAL "
