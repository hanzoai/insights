...
Extracted from ``ee/scriptai/eval/ci/conftest.py`` so both CI evals and sandboxed
...
MASTER_USER_EMAIL = "eval-master-seed@insights.test"
...
def create_isolated_demo_data(
    ...
    *,
    ...
    Datastore data via server-side SQL — much faster than re-simulating
    ...
    email = f"eval-{label}-{today.isoformat()}@insights.test"
...
def copy_demo_data_to_new_team(
    ...
    *,
    ...
    email = f"eval-{label}-{suffix}@insights.test"
    ...
    with django_db_blocker.unblock():
        ...
        with persons_db_connection(writer=True) as conn, conn.cursor() as cursor:
            cursor.execute("DELETE FROM insights_grouptypemapping WHERE project_id = %s", [team.project_id])
            cursor.execute(
                ...
                "FROM insights_grouptypemapping WHERE project_id = %s",
                [team.id, team.project_id, master_team.project_id],
            )

        MatrixManager._sync_postgres_with_datastore_data(master_team_id, team.id)
