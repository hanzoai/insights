...
describe('SlackChannelPicker', () => {
    ...
    it('does not search for the composite key when the input is focused with an existing value', async () => {
        ...
        // Focus the picker. InputSelect._onFocus auto-fills the input with the selected
    ...
    // The saved channel needs to be in slackChannels for InputSelect's options to contain a
    ...
    it('does not reload the bulk list on blur when no search has clobbered the cache', async () => {
        // InputSelect calls setInputValue('') on blur (InputSelect.tsx:_onBlur), which
        ...
        // clicks somewhere else. Clicking outside triggers Dropdown's onClickOutside, which
    ...
    it('still searches when the user actually types a different value', async () => {
        ...
        // to InputSelect's auto-fill of an existing selection.
        const { container } = render(
            <Provider>
                <SlackChannelPicker integration={INTEGRATION} onChange={jest.fn()} />
            </Provider>
