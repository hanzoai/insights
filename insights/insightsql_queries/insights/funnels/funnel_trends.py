...
from insights.schema import BreakdownAttributionType, BreakdownType
...
from insights.insightsql_queries.insights.funnels.funnel import FunnelUDF, FunnelUDFMixin
...
from insights.insightsql_queries.insights.utils.utils import get_start_of_interval_insightsql, get_start_of_interval_insightsql_str
from insights.insightsql_queries.utils.query_date_range import QueryDateRange
from insights.insightsql_queries.utils.timestamp_utils import format_label_date, get_earliest_timestamp_unfiltered
from insights.interval_specs import get_interval_func
from insights.queries.util import correct_result_for_sampling
from insights.utils import DATERANGE_MAP, relative_date_parse
...
class FunnelTrendsUDF(FunnelUDFMixin, FunnelBase):
        ...
        inner_select = cast(
            ...
            SELECT
                arraySort(t -> t.1, groupArray(tuple(
                    toFloat(timestamp),
                    _toUInt64(toDateTime({get_start_of_interval_insightsql_str(self.context.interval.value, team=self.context.team, source="timestamp")})),
                    uuid,
                    {prop_selector},
                    arrayFilter((x) -> x != 0, [{steps}{exclusions}])
                ))) as events_array,
    ...
    def get_query(self) -> ast.SelectQuery:
        ...
        s.settings = InsightsQLQuerySettings(join_algorithm=JOIN_ALGOS)
        return s
        ...
        fill_select: list[ast.Expr] = [
            ast.Alias(
                ...
                expr=ast.ArithmeticOperation(
                    left=get_start_of_interval_insightsql(interval.value, team=team, source=date_from_as_insightsql),
                ...
                ast.ArithmeticOperation(
                    left=ast.Call(
                        ...
                        args=[
                            ...
                            get_start_of_interval_insightsql(interval.value, team=team, source=date_from_as_insightsql),
                            get_start_of_interval_insightsql(interval.value, team=team, source=date_to_as_insightsql),
                        ],
                    ),
    ...
    def get_step_counts_without_aggregation_query(
        ...
        select: list[ast.Expr] = [
            ...
            ast.Alias(alias="entrance_period_start", expr=get_start_of_interval_insightsql(interval.value, team=team)),
