...
**Audience:** Insights engineers shipping a type that does not exist yet.
...
- [ ] Run **`insightscli build:openapi`** after §1 and **commit** `products/dashboards/frontend/generated/*` (CI `check-openapi-types` — [config-and-codegen.md § Codegen & CI](config-and-codegen.md#codegen--ci)). Regenerates (do not edit by hand):
...
- [ ] If `build:openapi-schema` warns on `widget_type` enum collision: `python manage.py find_enum_collisions` → add `{YourWidgetTypeEnum: ["your_widget_type"]}` to `ENUM_NAME_OVERRIDES` in `insights/settings/web.py` (see [config-and-codegen.md § Codegen & CI](config-and-codegen.md#codegen--ci))
...
- [ ] **Adoption CTA on the "no entities yet" empty state** — when the product has nothing to show yet (no surveys, no experiments), render a primary `Button` (`targetBlank`) to the product's create flow and fire `insights.capture('dashboard widget create <product> clicked', { widget_type, tile_id })` on click. Measures adoption driven _from_ the widget, distinct from the platform `dashboard widget added` event. See [§ Product-adoption tracking](#product-adoption-tracking).
...
- [ ] `EditModal` + `edit*WidgetModalLogic.ts` — Zod validate → `Field` errors; disable save while `saving` / invalid
...
<Button
  ...
  targetBlank
  onClick={() =>
    insights.capture('dashboard widget create survey clicked', {
...
<Link
  ...
  target="_blank"
  onClick={() =>
    insights.capture('dashboard widget open survey clicked', {
      widget_type: 'survey_results',
      tile_id: tileId,
      survey_id: survey.id,
    })
