...
@patch("products.data_warehouse.backend.presentation.views.managed_warehouse.update_team")
@patch("insights.ducklake.common.get_earliest_event_date_for_team")
def test_sync_task_skips_datastore_when_date_already_cached(
    mock_get_earliest: MagicMock, mock_update: MagicMock
) -> None:
    # Idempotent re-runs (dispatch retries, re-onboards) must not re-query Datastore or
...
@parameterized.expand(
    [
        ...
        ("no_cp_row", []),
        # An unreachable control plane must not raise or query Datastore: the sensor
