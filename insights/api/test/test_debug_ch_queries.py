...
from insights.api.debug_ch_queries import _cache_table_stats
from insights.datastore.preaggregation.experiment_exposures_sql import SHARDED_EXPERIMENT_EXPOSURES_TABLE
from insights.datastore.preaggregation.experiment_metric_events_sql import SHARDED_EXPERIMENT_METRIC_EVENTS_TABLE
...
from insights.settings.data_stores import DATASTORE_AUX_CLUSTER, DATASTORE_CLUSTER
...
class TestDebugCHQuery(APIBaseTest):
    ...
    @patch("insights.api.debug_ch_queries.sync_execute")
    ...
    def test_filtered_queries_denied_when_requester_has_no_current_team(self):
        ...
        new_user = User.objects.create_user(
            email="staff-no-team@hanzo.ai", password="testpass123", first_name="Staff", is_staff=True
        )
    ...
    @patch("insights.api.debug_ch_queries.sync_execute", return_value=[])
...
class TestCacheTableStats(SimpleTestCase):
    def test_reads_each_table_from_its_own_cluster(self):
        ...
        def fake_sync_execute(_query, params):
            parts_by_cluster = {
                DATASTORE_CLUSTER: [(SHARDED_EXPERIMENT_EXPOSURES_TABLE(), "20260801", 10, 100, 1)],
                DATASTORE_AUX_CLUSTER: [(SHARDED_EXPERIMENT_METRIC_EVENTS_TABLE(), "20260802", 20, 200, 2)],
            }
            return [row for row in parts_by_cluster[params["cluster"]] if row[0] in params["tables"]]

        with patch("insights.api.debug_ch_queries.sync_execute", side_effect=fake_sync_execute):
    ...
    def test_unreachable_cluster_degrades_instead_of_raising(self):
        ...
        def fake_sync_execute(_query, params):
            if params["cluster"] == DATASTORE_AUX_CLUSTER:
