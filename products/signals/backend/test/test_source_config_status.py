...
class TestSignalSourceConfigStatus(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("qualified_row", [("insights/insights.issues", RUNNING)], "running"),
            ("repo_name_with_dots", [("insights/some.repo.issues", COMPLETED)], "completed"),
            (
                "failing_repo_outranks_completed_sibling",
                [("insights/a.issues", COMPLETED), ("insights/b.issues", FAILED)],
                "failed",
            ),
            (
                "running_outranks_everything",
                [("insights/a.issues", FAILED), ("insights/b.issues", RUNNING)],
                "running",
            ),
            ("only_other_endpoints", [("insights/insights.pull_requests", RUNNING)], None),
            ("no_rows", [], None),
        ]
    )
    ...
    def test_ignores_deleted_sources(self) -> None:
        ...
        ExternalDataSchema.objects.create(team=self.team, source=source, name="insights/insights.issues", status=RUNNING)
