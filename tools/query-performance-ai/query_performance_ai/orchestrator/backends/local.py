"""Local-Datastore-backed execution.

Talks to the dev Datastore via the bare `datastore_driver.Client`. We
deliberately do NOT go through `insights.datastore.client.execute.sync_execute`:
...
no app init) so we pick up the same `DATASTORE_HOST`/`PASSWORD`/etc. that
the rest of the repo reads — including non-trivial defaults like
`DATASTORE_SECURE = not TEST and not DEBUG` that env-var-only would miss.
"""
...
_LOCAL_DEV_DB_PROMPT = (
    "## Coordinator routing — LOCAL DATASTORE MODE\n"
    "\n"
    "The coordinator is forwarding your SQL to a developer-local Datastore instance "
...
class LocalDatastoreBackend(ExecutionBackend):
    def __init__(self, *, team_id: int = 1) -> None:
        self._team_id = team_id

    @property
    def name(self) -> str:
        return f"local-datastore[team={self._team_id}]"
    ...
    def run(self, sql: str, *, timeout_s: int) -> ExecutionResult:
        # Lazy import — both datastore_driver and django.conf are in the
        ...
        # DJANGO_SETTINGS_MODULE so the lookup resolves to insights.settings.
        from django.conf import settings as dj  # noqa: PLC0415

        from datastore_driver import Client  # noqa: PLC0415
        from datastore_driver.errors import Error as DatastoreError  # noqa: PLC0415

        client = Client(
            host=dj.DATASTORE_HOST,
            user=dj.DATASTORE_USER,
            password=dj.DATASTORE_PASSWORD,
            database=dj.DATASTORE_DATABASE,
            secure=dj.DATASTORE_SECURE,
            ca_certs=getattr(dj, "DATASTORE_CA", None),
            verify=getattr(dj, "DATASTORE_VERIFY", True),
            connect_timeout=10,
        ...
        try:
            ...
            client.disconnect()
            raise BackendError(f"datastore exec failed: {type(e).__name__}: {str(e)[:1000]}") from e
