...
from insights.llm.gateway_client import resolve_ai_gateway_config
from insights.temporal.ai_observability.eval_reports.output_types import get_outcome_definition
from insights.temporal.ai_observability.eval_reports.report_agent.prompts import build_eval_report_system_prompt
...
from insights.temporal.ai_observability.eval_reports.report_agent.state import EvalReportAgentState
...
from insights.temporal.ai_observability.eval_reports.targets import GENERATION_TARGET
...
def _compute_metrics(
    ...
    """Compute report metrics directly via InsightsQL (independent of agent state).

    Returns None when a transient Datastore failure exhausts the query helper's
...
def run_eval_report_agent(
    ...
    Datastore (not from the agent), so downstream consumers can trust the
    numbers without parsing prose.
    """
    from insights.temporal.ai_observability.eval_reports.constants import (
    ...
    from insights.temporal.ai_observability.eval_reports.metrics import increment_errors, increment_report_generated

    # The agent's query tools would fail under the same sustained Datastore load,
    ...
    if hanzo_insights.default_client and resolve_ai_gateway_config() is None:
        callbacks.append(
            CallbackHandler(
                hanzo_insights.default_client,
