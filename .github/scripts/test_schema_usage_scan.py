"""Tests for the insights.schema AST dependency scanner.
...
def test_direct_symbol_import(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "a.py", "from insights.schema import LogsQuery, InsightsQLFilters\n")
    result = scan(str(tmp_path))
    assert result == {"InsightsQLFilters": ["logs"], "LogsQuery": ["logs"]}


def test_direct_import_uses_original_name_not_alias(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "a.py", "from insights.schema import LogsQuery as LQ\nq = LQ()\n")
    result = scan(str(tmp_path))
    assert result == {"LogsQuery": ["logs"]}


def test_schema_enums_symbol_import(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "a.py", "from insights.schema_enums import ProductKey\n")
    result = scan(str(tmp_path))
    assert result == {"ProductKey": ["logs"]}


def test_import_insights_schema_enums_dotted(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "a.py", "import insights.schema_enums\nk = insights.schema_enums.ProductKey\n")
    result = scan(str(tmp_path))
    assert result == {"ProductKey": ["logs"]}


def test_star_import_is_wildcard(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "a.py", "from insights.schema import *\n")
    assert scan(str(tmp_path)) == {"*": ["logs"]}


def test_from_insights_import_schema_attribute_refs(tmp_path: Path) -> None:
    write_py(
        tmp_path,
        "product_analytics",
        "b.py",
...
def test_from_insights_import_schema_aliased(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "b.py", "from insights import schema as sch\nq = sch.TrendsQuery()\n")
    assert scan(str(tmp_path)) == {"TrendsQuery": ["logs"]}


def test_import_insights_schema_dotted(tmp_path: Path) -> None:
    write_py(tmp_path, "alerts", "c.py", 'import insights.schema\ns = insights.schema.AlertState["FIRING"]\n')
    assert scan(str(tmp_path)) == {"AlertState": ["alerts"]}


def test_import_insights_schema_aliased(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "c.py", "import insights.schema as ps\nx: ps.InsightsQLQuery = ps.InsightsQLQuery()\n")
    assert scan(str(tmp_path)) == {"InsightsQLQuery": ["logs"]}


def test_import_insights_package_dotted(tmp_path: Path) -> None:
    write_py(tmp_path, "alerts", "c.py", "import insights\nx = insights.schema.AlertState\n")
    assert scan(str(tmp_path)) == {"AlertState": ["alerts"]}


def test_nested_attribute_records_type_not_member(tmp_path: Path) -> None:
    write_py(tmp_path, "logs", "b.py", "from insights import schema\nv = schema.AlertState.FIRING\n")
    assert scan(str(tmp_path)) == {"AlertState": ["logs"]}


def test_dynamic_use_of_module_is_wildcard(tmp_path: Path) -> None:
    write_py(tmp_path, "mystery", "d.py", "from insights import schema\nregister(schema)\n")
...
def test_comments_and_strings_do_not_match(tmp_path: Path) -> None:
    write_py(
        ...
        "a.py",
        '''
        # from insights import schema  -> schema.LogsQuery
        """Docstring mentioning schema.TrendsQuery should be ignored."""
        x = "from insights.schema import EventsQuery"
        ''',
    )
...
def test_syntax_error_is_wildcard(tmp_path: Path) -> None:
    write_py(tmp_path, "broken", "a.py", "from insights import schema\ndef oops(:\n")
...
def test_unioned_across_products_and_sorted(tmp_path: Path) -> None:
    write_py(tmp_path, "surveys", "a.py", "from insights.schema import LogsQuery\n")
    write_py(tmp_path, "logs", "b.py", "from insights import schema\nx = schema.LogsQuery\n")
