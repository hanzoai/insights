...
from insights.exceptions import Conflict, EnterpriseFeatureException, PaidFeatureException
from insights.helpers.verified_domain_enforcement import VERIFIED_DOMAIN_REQUIRED_ERROR, is_enforcement_disable_request
from insights.models import Organization, OrganizationDomain, OrganizationMembership, Project, Team, User
from insights.rbac.user_access_control import AccessControlLevel, UserAccessControl, ordered_access_levels
...
from insights.utils import get_can_create_org
...
class PremiumFeaturePermission(BasePermission):
    ...
    so Insights support can debug paid features for non-paying orgs. Plain staff sessions
...
class APIScopePermission(ScopeBasePermission):
    ...
    def check_team_and_org_permissions(self, request, view) -> None:
        ...
        if isinstance(request.successful_authenticator, OAuthAccessTokenAuthentication):
            ...
            # ID-JAG access tokens are bound to the specific Insights Organization
...
def insights_feature_flag_value(
    flag: str,
    distinct_id: str,
    *,
    ...
    """Server-side check of a Insights-internal gating flag with org/project group context.

    Matches in-app flag evaluation: insights-js often has project (team) context; server-only org
    ...
    can't drift from InsightsFeatureFlagPermission.
    """
...
def insights_feature_flag_enabled(
    flag: str,
    distinct_id: str,
    *,
    organization_id: str | uuid.UUID,
    ...
    def has_permission(self, request, view) -> bool:
        ...
        for required_flag, actions in config.items():
            if "*" in actions or view.action in actions:
                ...
                enabled = insights_feature_flag_enabled(
                    required_flag,
...
def is_mcp_built_in_agent_oauth_request(request: Request) -> bool:
    ...
    Insights's built-in agents (carries the server-only `mcp_builtin_agent` scope)."""
    authenticator = request.successful_authenticator
