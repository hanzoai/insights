...
from products.tasks.backend.logic.services.mts_example.schemas import CursedItemCandidates

_PREAMBLE = """You are an agent surfacing cursed code — the worst-named identifiers and the ugliest stale comments — inside the Insights/insights repository.
...
1. **The codebase** — the full Insights/insights repository is cloned on disk. Use file search, grep, code reading, `git log`, and `git blame`.
2. **Insights MCP** — query Insights product data via tools such as `execute-sql`, `list-event-definitions`, `list-property-definitions`, `list-feature-flags`, `insights-get-all`, `experiment-list`, `dashboards-get-all`. Use these to check whether the cursedness has also leaked into user-facing telemetry."""
