        ...
        [
            ...
            (
                "attachments_fallback",
                {
                    ...
                    "blocks": [],
                    "attachments": [
                        {"fallback": "Insights alert: signups dropped 30% week over week"},
                    ],
                },
                "Insights alert: signups dropped 30% week over week",
            ),
            (
...
class TestCollectThreadMessages:
    ...
    def test_alert_blocks_only_message_is_included(self):
        # Insights alert: substantive content lives only in blocks, no text field.
        self._set_thread(
            [
                {
                    "bot_id": "B_ALERTS",
                    "bot_profile": {"name": "Insights"},
                    "text": "",
                    "blocks": [
                        {
                            "type": "header",
    ...
    def test_skips_our_own_bot_reply_messages(self):
        ...
        self._set_thread(
            [
                {"user": "U_ANDY", "text": "@Insights please look at this", "ts": "1.000"},
    ...
    def test_keeps_thread_root_even_when_bot_id_matches_our_own(self):
        # Regression: in workspaces where the alerting Slack app and the `@Insights` code
    ...
    def test_bot_without_profile_falls_back_to_username_field(self):
        self._set_thread([{"bot_id": "B_HOOK", "username": "Insights Webhook", "text": "ping", "ts": "2.000"}])

        result = collect_thread_messages(self.slack, self.integration, "C001", "1.234", our_bot_id=None)

        assert result == [{"user": "Insights Webhook", "user_id": "", "text": "ping", "ts": "2.000"}]
    ...
    def test_includes_ts_for_initiator_disambiguation(self):
        self._set_thread(
            [
                ...
                {"user": "U_ANDY", "text": "@Insights fix this", "ts": "2.000"},
            ]
        )
        self.mock_get_user_info.return_value = {"user": {"profile": {"display_name": "andy"}}}
    ...
    def test_preserves_user_mention_replacement(self):
        ...
        self.mock_get_user_info.return_value = {"user": {"profile": {"display_name": "insights-code"}}}

        result = collect_thread_messages(self.slack, self.integration, "C001", "1.234", our_bot_id=None)

        assert result[0]["text"] == "hey <@UBOT|insights-code> can you help"
    ...
    def test_includes_thread_root_when_our_bot_id_is_none(self):
        ...
        self._set_thread(
            [
                {"bot_id": "B_ALERT", "bot_profile": {"name": "Insights"}, "text": "alert"},
        ...
        assert [m["user"] for m in result] == ["Insights", "andy"]
