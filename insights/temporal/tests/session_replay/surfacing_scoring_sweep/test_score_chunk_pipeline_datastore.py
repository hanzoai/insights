"""End-to-end check that fetch → validate → predict works against real Datastore.
...
from insights.session_recordings.sql.session_replay_feature_sql import TRUNCATE_SESSION_REPLAY_FEATURES_TABLE_SQL
from insights.temporal.session_replay.surfacing_scoring_sweep.activities import _fetch_features_dataframe
from insights.temporal.session_replay.surfacing_scoring_sweep.features import ID_COLUMNS, validate_features
from insights.temporal.session_replay.surfacing_scoring_sweep.scorer import get_feature_names, predict
from insights.temporal.session_replay.surfacing_scoring_sweep.types import ChunkSpec
from insights.temporal.tests.session_replay.surfacing_scoring_sweep.ch_insert_helpers import insert_session_replay_event


@pytest.mark.usefixtures("surfacing_booster_path")
class TestScoreChunkPipelineDatastore(DatastoreTestMixin, BaseTest):
    """One realistic session, full pipeline, synthetic booster."""

    def setUp(self) -> None:
        super().setUp()
        # Both target tables aren't in insights/conftest.py's truncate list —
