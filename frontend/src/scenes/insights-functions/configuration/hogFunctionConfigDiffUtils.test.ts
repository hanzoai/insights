...
describe('buildInsightsFunctionConfigDiff', () => {
    it('classifies each proposed field as added / removed / changed and drops unchanged ones', () => {
        ...
        const proposed = {
            ...
            script: 'return event', // unchanged → dropped
        ...
        const byField = Object.fromEntries(
            buildInsightsFunctionConfigDiff(current, proposed).map((diff) => [diff.field, diff.status])
        )
    ...
    it('ignores server-only input noise so an unchanged input value is not a spurious change', () => {
        ...
        expect(buildInsightsFunctionConfigDiff(current, proposed)).toEqual([])
    })
