...
class SnowflakeSource(SQLSource[SnowflakeSourceConfig]):
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            # rejects the connection because Insights's egress IP isn't permitted. Retrying can never
            # succeed until their admin allowlists our IPs, so stop retrying and surface what to do.
            "is not allowed to access Snowflake": "Snowflake rejected the connection because a network policy (IP allowlist) on your account does not permit Insights's IP address. Ask your Snowflake administrator to add Insights's egress IP addresses to the network policy allowlist, then resync.",
            ...
            # (a network policy/firewall/proxy blocking Insights, or the role's access to the data
            ...
            "HTTP 403: Forbidden": "Snowflake refused the request with an HTTP 403 (forbidden). This usually means a network policy or firewall on your account is blocking Insights's access, or your role's access to the data was revoked. Check your Snowflake network access rules and role grants, then resync.",
        }
