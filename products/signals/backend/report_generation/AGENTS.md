...
- `select_repo.py`
  ...
  - If the team has N repos: spawns a sandbox agent that uses `gh` CLI to explore candidates and pick the best match. Uses `Insights/.github` as a small dummy repo for the sandbox clone, since the agent only needs `gh` CLI access (not the repo itself).
...
- `python manage.py select_repo --repos Insights/insights Insights/insights-js Insights/insights-python`
