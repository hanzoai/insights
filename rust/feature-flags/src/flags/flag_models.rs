...
/// `_get_feature_flags_for_service()` in insights/models/feature_flag/flags_cache.py.
/// Field changes must follow the expand-and-contract pattern — see contract tests in
/// insights/models/feature_flag/test/test_flags_cache.py.
...
/// MinimalFeatureFlagSerializer (insights/api/feature_flag.py). Field changes must
...
pub struct FeatureFlagRow {
    ...
    #[serde(default)]
    pub bucketing_identifier: Option<String>,
    /// Populated by the from_pg fallback query via a correlated EXISTS over insights_experiment.
    #[serde(default)]
    pub has_experiment: bool,
}
