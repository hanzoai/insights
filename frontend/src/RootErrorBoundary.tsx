...
/**
 * Report a boot failure straight to the capture API. insights-js lives inside the App chunk —
...
function reportBootFailure(error: unknown): void {
    try {
        ...
        try {
            ...
            distinctId = JSON.parse(window.localStorage.getItem(`ph_${apiKey}_insights`) || '{}').distinct_id
        } catch {
 ...
 * (insights-js, the full ErrorBoundary, all UI) can fail to load or crash on boot — this
...
export class RootErrorBoundary extends React.Component<{ children: React.ReactNode }, RootErrorBoundaryState> {
    ...
    override componentDidCatch(error: unknown, errorInfo: React.ErrorInfo): void {
        console.error('[Insights] App failed to start:', error, errorInfo.componentStack)
                ...
                <div className="Preloader" role="alert">
                    <div>
                        {isChunkLoadError(this.state.error)
                            ...
                            : 'Insights crashed while starting. '}
