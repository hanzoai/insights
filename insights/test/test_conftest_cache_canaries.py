...
from insights.datastore.client.connection import get_client_from_pool, get_pool
from insights.models import Team
from insights.test import base as test_base
...
def test_datastore_checkout_counter_is_wired():
    # Assumption: get_client_from_pool keeps its @patchable hook and ChPool.get_client
    # keeps its test-time wrapper - insights/test/base.py counts client checkouts
    # through both to decide whether Datastore can have changed since the last reset.
    # If either hook gets unwired, the counter goes stale and
    # DatastoreDestroyTablesMixin silently stops resetting Datastore between tests.
    before = test_base._datastore_pool_checkouts
    with get_client_from_pool():
        pass
    assert test_base._datastore_pool_checkouts > before
