...
import 'products/workflows/frontend/Workflows/customflows/registry/triggers'

import { workflowLogic } from '../../workflowLogic'
import { CustomFlowEventFilters } from '../filters/CustomFlowFilters'
...
import { CustomFlowFunctionConfiguration } from './components/CustomFlowFunctionConfiguration'

type TriggerAction = Extract<CustomFlowAction, { type: 'trigger' }>
...
function StepTriggerConfigurationWebhook({
    ...
    action: Extract<CustomFlowAction, { type: 'trigger' }>
    config: Extract<CustomFlowAction['config'], { type: 'webhook' }>
}): JSX.Element {
    const { setWorkflowActionConfig } = useActions(workflowLogic)
    ...
    return (
        <div className="w-full">
            ...
            />
            <CustomFlowFunctionConfiguration
...
function StepTriggerConfigurationSchedule({
    ...
    action: Extract<CustomFlowAction, { type: 'trigger' }>
    config: Extract<CustomFlowAction['config'], { type: 'schedule' }>
}): JSX.Element {
    const { setWorkflowActionConfig } = useActions(workflowLogic)
    const { actionValidationErrorsById } = useValues(workflowLogic)
    const validationResult = actionValidationErrorsById[action.id]
...
function StepTriggerConfigurationBatch({
    ...
    action: Extract<CustomFlowAction, { type: 'trigger' }>
    config: Extract<CustomFlowAction['config'], { type: 'batch' }>
}): JSX.Element {
...
function StepTriggerConfigurationTrackingPixel({
    ...
    action: Extract<CustomFlowAction, { type: 'trigger' }>
    config: Extract<CustomFlowAction['config'], { type: 'tracking_pixel' }>
}): JSX.Element {
    const { setWorkflowActionConfig } = useActions(workflowLogic)
    const { workflow, actionValidationErrorsById } = useValues(workflowLogic)
    const validationResult = actionValidationErrorsById[action.id]
                        ...
                        content: (
                            <>
                                ...
                                <div>
                                    ...
                                    properties below, or you can hard code the values. This will not create a Insights
