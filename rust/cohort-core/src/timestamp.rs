...
//!
//! [`datastore_timestamp_to_millis`] accepts the same two wire shapes as
//! [`scriptvm::globals`](crate::scriptvm) but is **stricter**: it returns epoch millis and yields [`None`]
...
/// zone-less Datastore form is interpreted as UTC.
pub fn datastore_timestamp_to_millis(raw: &str) -> Option<i64> {
...
mod tests {
    ...
    #[test]
    fn datastore_form_parses_to_golden_millis() {
        assert_eq!(
            datastore_timestamp_to_millis("2026-05-26 12:34:56.789000"),
            Some(GOLDEN_MS),
        );
    }

    #[test]
    fn iso_form_agrees_with_datastore_form() {
        assert_eq!(
            datastore_timestamp_to_millis("2026-05-26T12:34:56.789Z"),
            Some(GOLDEN_MS),
        );
    }
    ...
    fn rfc3339_offset_is_normalized_to_utc() {
        let with_offset = datastore_timestamp_to_millis("2026-05-26T12:34:56.789+02:00").unwrap();
    ...
    fn sub_millisecond_digits_truncate() {
        assert_eq!(
            datastore_timestamp_to_millis("2026-01-01 00:00:00.000999"),
            datastore_timestamp_to_millis("2026-01-01 00:00:00.000000"),
        );
    }
    ...
    fn unparseable_values_are_none() {
        for bad in ["", "not a date", "2026-13-99 99:99:99", "1716800000"] {
            assert_eq!(
                datastore_timestamp_to_millis(bad),
                None,
