...
use personinsights_common::grpc::{current_client_name, current_method_name};
...
impl PersonLookup for PostgresStorage {
    ...
    async fn get_persons_by_distinct_ids_in_team(
        ...
        // with the team predicate parked on `insights_person`. The latter lets
            ...
            stream::iter(chunks.into_iter().map(|chunk| {
                ...
                async move {
                    ...
                    let rows = sqlx::query!(
                        ...
                        INNER JOIN insights_persondistinctid d
                            ON d.team_id = $1 AND d.distinct_id = batch.distinct_id
                        INNER JOIN insights_person p
    ...
    async fn delete_persons(&self, team_id: i64, uuids: &[Uuid]) -> StorageResult<i64> {
        ...
        let person_ids: Vec<i64> = sqlx::query_scalar!(
            r#"
            SELECT id::bigint as "id!" FROM insights_person
            WHERE team_id = $1 AND uuid = ANY($2)
            "#,
            team_id as i32,
            uuids
    ...
    async fn delete_personless_distinct_ids_batch_for_team(
        ...
        let result = sqlx::query!(
            r#"
            DELETE FROM insights_personlessdistinctid
            WHERE id IN (
                SELECT id FROM insights_personlessdistinctid
                WHERE team_id = $1
                LIMIT $2
    ...
    async fn split_person(
        ...
        let new_uuids: Vec<Uuid> = dids
            .iter()
            .map(|did| personinsights_common::persons::person_uuid(team_id, did))
            .collect();
        ...
        if !uuids_to_insert.is_empty() {
            let inserted = sqlx::query!(
                r#"
                INSERT INTO insights_person (uuid, team_id, properties, created_at, version, is_identified)
        ...
        if !uuids_to_update.is_empty() {
            sqlx::query!(
                r#"
                UPDATE insights_person SET version = $3
                WHERE team_id = $1 AND uuid = ANY($2)
                "#,
                team_id as i32,
                &uuids_to_update,
        ...
        let update_result = sqlx::query!(
            r#"
            UPDATE insights_persondistinctid AS pdi
    ...
    async fn set_person_distinct_id_version_floor(
        ...
        let row = sqlx::query_as!(
            ...
            r#"
            WITH target AS (
                SELECT person_id FROM insights_persondistinctid
...
async fn delete_persons_by_ids_chunk(
    ...
    // Cohort memberships have no FK to insights_person (the constraint was dropped
