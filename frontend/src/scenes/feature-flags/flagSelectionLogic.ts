    ...
    listeners(({ actions, values }) => ({
        ...
        bulkCopyFlags: async () => {
            ...
            try {
                ...
                toast.error(`Failed to resolve selected flags: ${errorMessageFrom(error)}`)
                actions.bulkCopyFlagsFinished(null)
                return
            }
            if (keys.length === 0) {
                toast.error('None of the selected flags could be resolved — they may have been deleted.')
                actions.bulkCopyFlagsFinished(null)
                return
            }
            if (keys.length > BULK_COPY_MAX_FLAGS) {
                toast.error(`Bulk copy supports up to ${BULK_COPY_MAX_FLAGS} flags at once.`)
                actions.bulkCopyFlagsFinished(null)
                return
            }
