...
# key prunes by date; `insights.ai_events` is keyed `(team_id, trace_id,
...
class AIObservabilityOfflineEvaluationsViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    def experiment_items(self, request: Request, **kwargs) -> Response:
        ...
        # {placeholder}` trips `ScriptVMException: Global variable not found:
        ...
        with tags_context(product=Product.LLM_ANALYTICS, feature=Feature.QUERY, team_id=self.team_id):
            try:
                preflight_result = execute_insightsql_query(
