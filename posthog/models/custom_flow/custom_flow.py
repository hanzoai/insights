...
from posthog.plugins.plugin_server_api import reload_custom_flows_on_workers
...
class CustomFlow(UUIDTModel):
    """
    Stores the version, layout and other meta information for each CustomFlow
    """
    ...
    def __str__(self):
        return f"CustomFlow {self.id}/{self.version}: {self.name}"


@receiver(post_save, sender=CustomFlow)
def custom_flow_saved(sender, instance: CustomFlow, created, **kwargs):
    reload_custom_flows_on_workers(team_id=instance.team_id, custom_flow_ids=[str(instance.id)])
...
def action_saved_for_custom_flows(sender, instance: Action, created, **kwargs):
    ...
    refresh_affected_custom_flows.delay(action_id=instance.id)


@receiver(post_save, sender=Team)
def team_saved_for_custom_flows(sender, instance: Team, created, **kwargs):
    from posthog.tasks.custom_flows import refresh_affected_custom_flows

    refresh_affected_custom_flows.delay(team_id=instance.id)
