...
### Using insightscli (recommended)

If you enable the `llm_analytics` intent via `insightscli dev:setup`, the sentiment
...
If you aren't using insightscli, install the sentiment dependency group:
...
if it's missing from `ONNX_CACHE_DIR` (default `/tmp/insights-sentiment-onnx-cache`).
...
```bash
pytest insights/temporal/ai_observability/sentiment/ -x -q
```
