...
const LEADER_ADMISSION: WarningSource = WarningSource {
    service: "personinsights-leader",
    path: "admission",
    pipeline_step: "personinsights_admission",
};
    ...
    pub fn emit(&self, warning: &SizeViolationWarning) {
            ...
            .check(&warning.team_id.to_string(), warning_type)
        {
            ...
            ThrottleDecision::Throttled => {
                counter!(
                    "personinsights_leader_ingestion_warnings_suppressed_total",
                    "outcome" => "throttled"
                )
                .increment(1);
                return;
        ...
        match self.producer.send_result(record) {
            Ok(_future) => {
                counter!("personinsights_leader_ingestion_warnings_emitted_total").increment(1);
            }
            Err((e, _)) => {
