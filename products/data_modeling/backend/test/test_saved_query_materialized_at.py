...
class TestSavedQueryMaterializedAt(BaseTest):
    @parameterized.expand(
        [
            ...
            ("completed_job_beats_frozen_saved_query", 3 * 24 * 60, [("Completed", "datastore", 5)], 5),
            (
                # a slow scheduled run created before a quick manual one can still finish last
                "freshest_completion_wins_over_newest_created",
                3 * 24 * 60,
                [("Completed", "datastore", 10, 60), ("Completed", "datastore", 35, 30)],
                10,
            ),
            (
                "newest_failed_job_ignored",
                3 * 24 * 60,
                [("Completed", "datastore", 120), ("Failed", "datastore", 5)],
                120,
            ),
            (
                "duckgres_shadow_job_ignored",
                3 * 24 * 60,
                [("Completed", "datastore", 120), ("Completed", "duckgres", 5)],
                120,
            ),
            ("running_job_ignored", 30, [("Running", "datastore", 1)], 30),
            ("no_jobs_falls_back_to_saved_query", 30, [], 30),
            ("saved_query_newer_than_jobs_wins", 5, [("Completed", "datastore", 120)], 5),
            ("never_materialized_returns_none", None, [], None),
        ]
    )
    def test_returns_latest_successful_datastore_materialization(
