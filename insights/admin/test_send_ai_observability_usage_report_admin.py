...
class TestSendAIObservabilityUsageReportAdmin(BaseTest):
    def setUp(self):
        ...
        reverse(
            "admin:index"
        )  # warm insights.urls import outside freeze_time: the first reverse() lazily imports pydantic v1, which breaks under a frozen datetime.date

    def _post(self, report_date: str):
        http_request = self.factory.post(
            "/admin/insights/organization/send-ai-observability-usage-report/", {"report_date": report_date}
        )
        http_request.user = self.user
        _attach_messages(http_request)
        with (
            patch("insights.admin.admins.organization_admin.reverse", side_effect=_fake_reverse),
            patch("insights.admin.admins.organization_admin.call_command") as mock_call_command,
        ):
    ...
    def test_get_renders_form_without_dispatching(self):
        http_request = self.factory.get("/admin/insights/organization/send-ai-observability-usage-report/")
        http_request.user = self.user
        _attach_messages(http_request)
        with (
            patch("insights.admin.admins.organization_admin.reverse", side_effect=_fake_reverse),
