...
# by the real Datastore query while the post-delivery one survives.
...
class _WindowPipelineHelpers:
    ...
    def _spec(self, insightsql: str) -> EnrichedPromptSpec:
        return EnrichedPromptSpec(
            cleaned_prompt="how many events",
            context_blob="ctx",
            plan=QueryPlan(
                overall_intent="count events",
                steps=[QueryPlanStep(description="Event counts", insightsql=insightsql)],
            ),
        )
...
class TestAIReportPipelineIntegration(_WindowPipelineHelpers, DatastoreTestMixin, NonAtomicBaseTest):
    """Exercises the real plan -> execute -> synthesize wiring: only the two LLM boundaries (planner,
    synthesis) are mocked; the planned InsightsQL runs against the test Datastore for real."""
    ...
    def setUp(self) -> None:
        ...
        # when it flows through InsightsQL (which anchors a bare datetime to the project timezone).
    ...
    async def test_real_insightsql_flows_into_synthesis_and_invalid_insightsql_degrades(
        self, mock_bep: MagicMock, mock_chat: MagicMock
    ) -> None:
        # --- happy path: planned InsightsQL (with the real window-filter literals) runs for real ---
        ...
        # fix LLM (also MaxChatOpenAI) returns a non-InsightsQLFix, so the step can't recover and degrades
        ...
        # The degraded step's generated InsightsQL + error type are captured for persistence/debugging.
        ...
        # "first-ever per user" is valid, executable InsightsQL — if it ever stops parsing, this fails
...
class TestAIReportWindowAnchor(_WindowPipelineHelpers, DatastoreTestMixin, NonAtomicBaseTest):
    """The gap-free "since last send" anchor, proven end-to-end through real Datastore: with a prior
    ...
    def setUp(self) -> None:
        ...
        # this also proves the anchor instant survives the tz round-trip through InsightsQL.
        self.team.timezone = "Australia/Sydney"
        self.team.save()
        _create_person(team=self.team, distinct_ids=["u1"])
