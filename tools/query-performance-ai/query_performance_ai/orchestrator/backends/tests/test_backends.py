...
from query_performance_ai.orchestrator.backends.local import LocalDatastoreBackend
...
def _completed(stdout: str = "", stderr: str = "", returncode: int = 0) -> Any:
    return subprocess.CompletedProcess(args=["insightscli"], returncode=returncode, stdout=stdout, stderr=stderr)
...
def test_metabase_backend_raises_backend_error_on_insightscli_failure() -> None:
    backend = MetabaseBackend(region="us", database_id=7, team_id=2)
...
def test_metabase_backend_raises_backend_error_on_timeout() -> None:
    backend = MetabaseBackend(region="us", database_id=7, team_id=2)
    with mock.patch.object(subprocess, "run", side_effect=subprocess.TimeoutExpired(cmd=["insightscli"], timeout=5)):
...
def test_tag_with_log_comment_escapes_single_quotes_in_json() -> None:
    # Datastore string literals double-up single quotes; the JSON value must
...
def test_query_log_build_lookup_sql_interpolates_table_and_run_id() -> None:
    sql = _query_log.build_lookup_sql("0123456789abcdef", table_expr="clusterAllReplicas(insights, system, query_log)")
    assert sql is not None
    assert "clusterAllReplicas(insights, system, query_log)" in sql
...
# ----------------------------------------------------------------------
# LocalDatastoreBackend `_fetch_query_log_metrics` tests.
#
# We don't end-to-end test `LocalDatastoreBackend.run()` because it pulls in
# Django settings + a live `datastore_driver.Client` connection. The
...
def test_local_fetch_query_log_metrics_returns_tuple_when_row_present() -> None:
    backend = LocalDatastoreBackend(team_id=1)
...
def test_local_fetch_query_log_metrics_swallows_flush_logs_failure() -> None:
    ...
    fake_client = mock.MagicMock()
    fake_client.execute.side_effect = [
        DatastoreError("not permitted"),  # FLUSH LOGS
