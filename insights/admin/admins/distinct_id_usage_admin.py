...
def _escaped_table() -> str:
    db = escape_datastore_identifier(settings.DATASTORE_DATABASE)
    tbl = escape_datastore_identifier(TABLE_BASE_NAME)
    return f"{db}.{tbl}"
            ...
            if distinct_id:
                for r in restrictions:
                    if not r.distinct_ids or distinct_id in r.distinct_ids:
                        edit_url = reverse("admin:insights_eventingestionrestrictionconfig_change", args=[r.pk])
                ...
                edit_url = reverse("admin:insights_eventingestionrestrictionconfig_change", args=[first.pk])
                return {
