...
function logBlockedRequest(
    error: Error,
    { url, templateId, teamId, insightsFunctionId }: CdpFetchAttribution & { url: string; templateId: string }
): void {
...
// owning script function and team. Optional because a few callers (push notifications, script
// transformations) hand `cdpTrackedFetch` to the Script VM without an invocation in scope.
...
export async function cdpTrackedFetch({
    ...
    const doFetch = () =>
        fetchAttribution.run({ teamId, insightsFunctionId, templateId }, () =>
    ...
    if (fetchError && isConnectionLevelError(fetchError)) {
        ...
        if (fetchError && isBlockedRequestError(fetchError)) {
            logBlockedRequest(fetchError, { url, templateId, teamId, insightsFunctionId })
        }
...
export class HogExecutorService {
    ...
    private routeToQueue(
        ...
        return createInvocationResult<CyclotronJobInvocationInsightsFunction>(
            invocation,
            {
                queue: targetQueue as CyclotronJobInvocationInsightsFunction['queue'],
                queueParameters: invocation.queueParameters,
    ...
    async executeFetch(
        ...
        const integrationInputs = await this.hogInputsService.loadIntegrationInputs(invocation.insightsFunction)
        ...
        if (isInsightsIngestUrl(params.url)) {
            try {
                ...
                if (team && isSelfReferentialIngestFetch({ url: params.url, body: params.body, team })) {
                    ...
                    const functionId = invocation.insightsFunction.id
                    ...
                    if (depth >= SELF_LOOP_MAX_DEPTH) {
                        ...
                        addLog(
                            'error',
                            `Refusing to fetch a Insights ingestion endpoint using this project's own API key - this destination's event-forwarding loop has already repeated ${SELF_LOOP_MAX_DEPTH} times. To capture an event back into this project use the 'insightsCapture' helper, or to enrich incoming events use a transformation.`
                        )
        ...
        if (params.aws_sigv4) {
            const resolved = resolveAwsSigV4Credentials(params.aws_sigv4, invocation.insightsFunction)
            if (!resolved.ok) {
        ...
        if (!fetchResponse || (fetchResponse?.status && fetchResponse.status >= 400)) {
            const nonFailureSchemaEntry = invocation.insightsFunction.inputs_schema?.find(
                (s) => s.type === 'non_failure_status_codes'
            )
            const nonFailureConfig = nonFailureSchemaEntry
                ? (invocation.insightsFunction.inputs?.[nonFailureSchemaEntry.key]?.value as
                      | Array<number | string>
                      | null
                      | undefined)
                : undefined
