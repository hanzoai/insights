...
class TestCreateWebhook:
    ...
    def test_registers_all_document_events_with_generated_auth_header(self, mock_session: MagicMock) -> None:
        ...
        result = create_webhook("token-123", "production", "https://webhooks.hanzo.ai/dwh/abc")
        ...
        assert body["url"] == "https://webhooks.hanzo.ai/dwh/abc"
        assert body["type"] == ""
        header = body["headers"][0]
        assert header["name"] == "Authorization"
        assert header["value"].startswith("Bearer ")
        # The generated header value is handed back so the Script template can verify deliveries.
