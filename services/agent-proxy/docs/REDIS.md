...
So every active SSE stream consumes one connection on a resource that almost every Insights service hits. Two failure modes follow, and neither is local to agent-proxy:
...
- **It is observable.** `agent_proxy_sse_open_streams` (gauge) tracks active streams, which is an upper bound on dedicated read connections. `insights_tasks_task_run_stream_connections_opened_total` / `_closed_total` give the same view from counters.
...
1. **Dedicated Redis for the task-run stream plane.** This removes the blast-radius concern entirely by isolating both the connection count and the idle-poll load off the shared instance, and it lets the simple per-stream model stay. Insights already isolates hot workloads this way: `FLAGS_REDIS_URL`, `AI_GATEWAY_REDIS_URL`, `SESSION_RECORDING_REDIS_URL`, `QUERY_CACHE_REDIS_CLUSTER_URL`. Add an equivalent (for example `TASKS_STREAM_REDIS_URL`) and point it at its own cluster.
