...
# Every query module runs InsightsQL through this method; patch it to test row mapping without a
...
TEST_BUCKET = "test_storage_bucket-insights.products.engineering_analytics.logic"
...
def _ago_with_duration(days: int, duration_seconds: int) -> tuple[str, str]:
    # Seed dates relative to real time: InsightsQL now() runs server-side and ignores
...
class _WarehouseMixin(DatastoreTestMixin, BaseTest):
...
class _EndpointsWarehouseMixin(_WarehouseMixin):
    ...
    real time (InsightsQL now() is server-side). Skips when object storage is
    unreachable."""
