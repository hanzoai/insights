...
describe('AI event subpipeline integration', () => {
    it('exercises all steps: script transform → normalize → AI enrich → person → prepare → emit', async () => {
        ...
        // Script transform step: property was added
        ...
        expect(properties.$group_0).toBe('insights')
    })
    ...
    it('script transform dropping event short-circuits the pipeline', async () => {
        const event = createAiEvent()
