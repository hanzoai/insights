...
const FULL_ENTRY = {
    id: 'abc',
    user: { id: 7, first_name: 'Ada', last_name: 'Lovelace', email: 'ada@hanzo.ai' },
    activity: 'updated',
    scope: 'FeatureFlag',
    item_id: '42',
    detail: {
...
function mockContext(): Context {
    return {
        ...
        api: {
            getProjectBaseUrl: () => 'https://us.hanzo.ai/project/1',
...
describe('advanced-activity-logs-list selectable fields', () => {
    ...
    it('returns the full allowlist (including detail.changes) when `fields` is omitted', async () => {
        ...
        expect(entry.user.email).toBe('ada@hanzo.ai')
