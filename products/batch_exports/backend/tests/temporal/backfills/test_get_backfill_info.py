...
from insights.temporal.common.datastore import DatastoreQueryTimeoutError
from insights.temporal.tests.utils.events import generate_test_events_in_datastore, insert_sessions_in_datastore
...
from products.batch_exports.backend.tests.temporal.utils.persons import (
    ...
    insert_person_values_in_datastore,
)
...
class TestGetBackfillInfoForEvents:
    ...
    async def truncate_events_table(self, datastore_client):
    ...
    async def test_query_metadata(self, mock_datastore_client, run_get_backfill_info, create_batch_export, ateam):
        """Test that the query is executed as expected. This test uses a mocked Datastore client to
        ...
        mock_datastore_client.expect_query_count(1)
        mock_datastore_client.expect_all_calls_have_query_id()
    ...
    async def test_query_caps_max_execution_time(
        self, mock_datastore_client, run_get_backfill_info, create_batch_export
    ):
        batch_export = await create_batch_export()
        await run_get_backfill_info(
            batch_export,
        ...
        captured = mock_datastore_client.calls[0]
    ...
    async def test_returns_no_estimate_on_timeout(
        ...
        mock_datastore_client.mock_client.read_query_as_jsonl = AsyncMock(
            side_effect=DatastoreQueryTimeoutError("Code: 159. TIMEOUT_EXCEEDED")
        )
...
class TestGetBackfillInfoForPersons:
    @pytest.fixture(autouse=True)
    async def truncate_persons_tables(self, datastore_client):
    ...
    @pytest.fixture
    def generate_persons(self, datastore_client, ateam):
        async def _generate(
    ...
    @pytest.fixture
    def generate_person_distinct_ids(self, datastore_client, ateam):
        async def _generate(
            ...
            await insert_person_distinct_id2_values_in_datastore(client=datastore_client, persons=pdi_values)
            return pdi_values
    ...
    async def test_count_includes_distinct_ids_from_changed_persons(
        self, ateam, datastore_client, generate_person_distinct_ids, create_batch_export, run_get_backfill_info
    ):
        ...
        await insert_person_values_in_datastore(
            client=datastore_client,
            persons=[
                {
                    "id": str(person_id),
    ...
    async def test_count_deduplicates_by_latest_version(
        self, ateam, datastore_client, create_batch_export, run_get_backfill_info
    ):
    ...
    async def test_query_metadata_when_no_data(
        self, mock_datastore_client, run_get_backfill_info, create_batch_export, ateam
    ):
    ...
    async def test_executes_two_queries_when_data_exists(
        ...
        mock_datastore_client.read_query_as_jsonl_responses = [
        ...
        mock_datastore_client.expect_query_count(2)
        mock_datastore_client.expect_unique_query_ids()
        mock_datastore_client.expect_properties_in_log_comment({"query_type": "backfill_estimate"})
    ...
    async def test_query_caps_max_execution_time(
        ...
        for call in mock_datastore_client.calls:
...
class TestGetBackfillInfoForSessions:
    @pytest.fixture(autouse=True)
    async def truncate_tables(self, datastore_client):
        await truncate_events(datastore_client)
    ...
    @pytest.fixture
    def generate_sessions(self, datastore_client, ateam):
        async def _generate(
            start_time: dt.datetime,
            end_time: dt.datetime | None = None,
            count: int = 10,
    ...
    async def test_query_metadata(self, mock_datastore_client, run_get_backfill_info, create_batch_export, ateam):
        """Test that the query is executed as expected. This test uses a mocked Datastore client to verify the query metadata."""
        batch_export = await create_batch_export()
        await run_get_backfill_info(
            batch_export,
            start_at=dt.datetime(2021, 1, 1, tzinfo=dt.UTC),
    ...
    async def test_query_caps_max_execution_time(
        ...
        captured = mock_datastore_client.calls[0]
        # The sessions path uses InsightsQL placeholders (`{name}`); the printed
