...
describe('script-log-exec', () => {
    describe('buildLogRecordGlobals', () => {
        ...
        // logs product (metric rules, Datastore sink) derives from these buffers.
        it.each([
            [
    ...
    describe('resolveLogTransformationInputs', () => {
        it('reports the VM time spent on script-templated inputs so callers can charge budgets', async () => {
        ...
        it('converts complex template results to plain JS values', async () => {
            // Script objects come back as Maps; consumers of inputs (globals, encoders)
    ...
    describe('executeLogTransformation', () => {
        ...
        ])('fails open without mutating when the result is %s', async (_desc, script) => {
            const record = createRecord()
            const originalBody = record.body
            const outcome = await run(script, record)
        ...
        it('redacts sensitive values written into attribute keys, not just values', async () => {
            // Script can assign a dynamic key, so redacting only values leaves the same
