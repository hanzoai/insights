...
The identity service of the personinsights cluster. It owns the identity graph — distinct id resolution and get-or-create orchestration — per the personinsights-identity saga RFC.
...
- Shared person primitives (row type, storage errors, uuidv5 scheme) live in `personinsights-common::persons`

## Running locally

```bash
bin/start-rust-service personinsights-identity   # gRPC :50055, metrics :9108
```

Tests need the persons database (`insights_persons`) with `rust/persons_migrations` applied:

```bash
cargo test -p personinsights-identity
```
