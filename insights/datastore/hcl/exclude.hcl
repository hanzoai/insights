...
# ("events_main") and the db-qualified form ("insights.events_main").
#
...
exclude {
  patterns = [
    # --- transient (Datastore atomic CREATE-OR-REPLACE / EXCHANGE) ---
    "_tmp_replace_*",
    ...
    #     migrate_datastore path, so the gate ignores them on BOTH sides until
