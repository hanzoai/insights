...
3. Interval/Period Correctness (for Trends, Retention, InsightsQL):
    ...
    - InsightsQL: If the question implies aggregation over time periods (e.g. "average session duration by day of week"), check for appropriate time functions like `dayOfWeek` or `toStartOfWeek`.
...
- InsightsQL: `output.query` is an AssistantInsightsQLQuery object. The actual SQL string is in `output.query.query`. (InsightsQL is a flavor of standard SQL.)
