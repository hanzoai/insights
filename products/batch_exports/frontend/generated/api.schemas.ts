...
export interface BatchExportRequestApi {
    ...
    /** Optional InsightsQL SELECT defining a custom model schema. Only recommended in advanced use cases. */
    insightsql_query?: string
    /** Optional list of property filters to restrict which events are exported. Each filter is a serialized InsightsQL property filter object with a 'type' of one of: 'event', 'insightsql', 'person' (e.g. {"key": "$browser", "operator": "exact", "type": "event", "value": ["Firefox"]}). */
    filters?: unknown
    /**
...
export type FileDownloadInsightsQLRequestApiModel =
 ...
 */
export interface FileDownloadInsightsQLRequestApi {
    file: FileDownloadDestinationFileConfigApi
    model: FileDownloadInsightsQLRequestApiModel
    /** InsightsQL SELECT query whose results are exported. Placeholders are not currently supported, and every column in the SELECT clause must be a field or have an alias. The query runs as of thetime the export starts; events ingested moments before may not be included yet. */
    insightsql_query: string
}
...
export type CreateFileDownloadRequestApi =
    ...
    | FileDownloadInsightsQLRequestApi
 ...
 * * `insightsql` - insightsql
 */
export type FileDownloadInsightsQLRequestModelEnumApi =
