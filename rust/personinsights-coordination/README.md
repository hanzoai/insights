...
Etcd-based coordination primitives for dynamically assigning Kafka partitions to PersonInsights writer pods and orchestrating zero-downtime handoffs when pods join or leave.

## Why this crate exists

PersonInsights processes person data sharded across Kafka partitions. Each partition must be owned by exactly one writer pod at a time (single-writer invariant). When pods scale up, scale down, or crash, partitions need to move between pods without losing inflight requests.
...
| --- | --- | --- |
| **Coordinator** | `personinsights-coordination` | Leader-elected singleton. Watches pods, computes partition assignments, orchestrates handoffs. |
| **Writer pod** | `personinsights-leader` | Owns partitions, processes person data. Registers with etcd, responds to handoff events (warm cache, release partition). |
| **Router** | `personinsights-router` | Routes requests to the correct writer pod. Maintains a local routing table from etcd watches. Executes traffic cutover during handoffs. |
