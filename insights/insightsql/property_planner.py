...
from insights.insightsql.database.schema.events import EventsPersonSubTable, EventsTable
...
from insights.insightsql.errors import (
...
from insights.datastore.materialized_column_types import MATERIALIZATION_VALID_TABLES, MaterializedColumn
from insights.datastore.property_groups import property_groups
from insights.schema_enums import PropertyGroupsMode
...
def plan_property_comparison(node: ast.CompareOperation, context: InsightsQLContext) -> PropertyComparisonPlan | None:
...
def plan_property_access(expr: ast.Expr, context: InsightsQLContext) -> PropertyAccessPlan | None:
...
def is_property_type_restricted(property_type: ast.PropertyType, context: InsightsQLContext) -> bool:
...
def get_restricted_keys_for_table_type(table_type: ast.Type, context: InsightsQLContext) -> set[str]:
...
def _extract_property_access(expr: ast.Expr, context: InsightsQLContext) -> tuple[ast.PropertyType, ast.ConstantType] | None:
...
def _extract_property_access_from_boolean_conversion(
    expr: ast.Expr, context: InsightsQLContext
...
def _plan_property_source(
    property_type: ast.PropertyType, property_name: str, context: InsightsQLContext
) -> PropertySourcePlan:
...
def _json_source_plan(
    ...
    restricted: bool = False,
    context: InsightsQLContext | None = None,
) -> PropertySourcePlan:
...
def _json_source_physical_type(
    ...
    context: InsightsQLContext | None,
) -> ast.ConstantType:
...
def _materialized_table_info(field_type: ast.FieldType, context: InsightsQLContext) -> tuple[str, str] | None:
    ...
    table_name = "person" if isinstance(resolved_table, RawPersonsTable) else resolved_table.to_printed_insightsql()
...
def get_dmat_column(context: InsightsQLContext, table_name: str, field_name: str, property_name: str) -> str | None:
...
def metadata_constant_type(property_type: ast.PropertyType, context: InsightsQLContext) -> ast.ConstantType | None:
...
def _semantic_type_for_property_type(property_type: ast.PropertyType, context: InsightsQLContext) -> ast.ConstantType:
...
def _property_info_for_property_type(
    property_type: ast.PropertyType, context: InsightsQLContext
...
def _property_scope(property_type: ast.PropertyType, context: InsightsQLContext) -> PropertyScope:
    ...
    except (InsightsQLNotImplementedError, QueryError, ResolutionError):
        return PropertyScope.UNKNOWN
...
def _group_property_name(property_type: ast.PropertyType, context: InsightsQLContext, property_name: str) -> str | None:
...
def _constant_type_from_expr(expr: ast.Expr, context: InsightsQLContext) -> ast.ConstantType:
