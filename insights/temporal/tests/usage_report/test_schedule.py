...
from insights.temporal.schedule import create_finalize_usage_reports_schedule, create_run_usage_reports_schedule
...
async def test_create_usage_reports_schedules_do_not_raise(
    ...
    with (
        patch("insights.temporal.schedule.a_schedule_exists", new=AsyncMock(return_value=False)),
        patch("insights.temporal.schedule.a_create_schedule", new=AsyncMock()) as create_mock,
        patch("insights.temporal.schedule.a_update_schedule", new=AsyncMock()) as update_mock,
    ):
    ...
    with (
        patch("insights.temporal.schedule.a_schedule_exists", new=AsyncMock(return_value=True)),
        patch("insights.temporal.schedule.a_create_schedule", new=AsyncMock()) as create_mock,
        patch("insights.temporal.schedule.a_update_schedule", new=AsyncMock()) as update_mock,
    ):
        await create_schedule_fn(client)
...
async def test_usage_reports_schedule_input_is_json_serializable(
    ...
    with (
        patch("insights.temporal.schedule.a_schedule_exists", new=AsyncMock(return_value=False)),
        patch("insights.temporal.schedule.a_create_schedule", new=fake_create_schedule),
    ):
