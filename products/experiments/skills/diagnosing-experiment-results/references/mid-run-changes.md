...
Insights blocks adding/removing variants on running experiments. If the user managed to do it
...
Insights's retention metric for experiments requires the **start event to occur after the user's
first exposure**. This
...
_build_start_event_timestamp_expr in insights/insightsql_queries/experiments/experiment_query_builder.py. The CTE INNER JOINs on start_events, so users with
