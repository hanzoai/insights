...
class S3BatchExportWorkflow(InsightsWorkflow):
    """A Temporal Workflow to export Datastore data into S3 or any S3-compatible bucket.
...
async def insert_into_s3_activity_from_stage(inputs: S3InsertInputs) -> S3BatchExportResult:
    ...
    compression, etc, which Datastore's S3 functions may not support.
    """
    bind_contextvars(
        team_id=inputs.team_id,
        destination="S3",
    ...
    async with Heartbeater():
        ...
        if inputs.integration_id is not None:
            ...
            if isinstance(integration, AwsS3RoleBasedIntegration):
                ...
                external_id = f"insights-{team.organization_id}"
                ...
                credentials = await get_credentials_using_user_aws_role(
                    ...
                    external_id,
                    session_name=f"Insights-batch-exports-{inputs.batch_export_id}",
