...
// The `agent-cli` use case grants the full MCP / `insights-cli api` scope set
...
const NEXT_COMMAND_AGENT_CLI: &str = "insights-cli api tools";
const NEXT_COMMAND_ERROR_TRACKING: &str = "insights-cli sourcemap --help";
const NEXT_COMMAND_SCHEMA: &str = "insights-cli exp schema pull";
const NEXT_COMMAND_ENDPOINTS: &str = "insights-cli exp endpoints list";
const NEXT_COMMAND_DEFAULT: &str = "insights-cli --help";
...
pub fn login_with_use_cases(host_override: Option<String>, use_cases: Vec<&str>) -> Result<()> {
    let host = if let Some(override_host) = host_override {
        ...
        match selection {
            "US" => "https://us.hanzo.ai".to_string(),
            "EU" => "https://eu.hanzo.ai".to_string(),
            "Manual" => {
                return manual_login();
            }
            _ => unreachable!(),
...
fn manual_login() -> Result<(), Error> {
    ...
    let host = Text::new("Enter the Insights host URL")
        .with_default("https://us.hanzo.ai")
        .with_validator(host_validator)
        .prompt()?;

    let env_id = Text::new("Enter your project ID (the number in your Insights homepage URL)")
        .with_validator(env_id_validator)
        .prompt()?;

    let token = Text::new("Enter your personal API token")
        .with_validator(token_validator)
        .with_help_message("See hanzo.ai/docs/api#private-endpoint-authentication. It will need to have the 'error tracking write' scope. To enable tooling, select the 'Agent CLI' preset.")
        .prompt()?;
...
mod tests {
    ...
    fn default_login_requests_the_agent_cli_use_case() {
        // The agent-cli use case is what grants the CLI the full `insights-cli api`
