...
from insights.dags.common import JobOwners, dagster_tags, settings_with_log_comment
from insights.ducklake import team_state
from insights.ducklake.client import make_duckgres_conninfo
from insights.ducklake.common import (
...
def _resolve_table_names(team_id: int) -> tuple[str, str]:
    ...
    org-scoped duckling don't merge into the shared `insights.events` / `insights.persons`.
...
def _connect_duckgres(target: DucklingTarget) -> psycopg.Connection[Any]:
    ...
    # next_day) window off the UTC day the Datastore export wrote and strand/over-delete
...
# monster object. Datastore PARTITION BY with a {_partition_id} placeholder in the
...
# dominant driver of file size and the only one Datastore estimates cheaply from
...
# per-table OCC.
#
# MAX_S3_FILE_FANOUT is bounded by WRITER MEMORY, not file count: Datastore's
...
# is automatically limited to keep that baseline within 32 GiB; Datastore's 100 GiB
# max_memory_usage remains the hard query limit.
# N may exceed Datastore's max_partitions_per_insert_block (default 100) safely —
...
EVENTS_TABLE_DDL = """
CREATE TABLE IF NOT EXISTS {catalog}.insights.{table} (
    uuid VARCHAR,
    event VARCHAR,
    properties VARCHAR,
...
def ensure_events_table_exists(
    ...
    if table_exists(conn, alias, "insights", table):
...
def validate_duckling_schema(
    ...
    with conn.cursor() as cur:
        cur.execute(f"DESCRIBE {alias}.insights.{target.events_table}")
...
def validate_duckling_persons_schema(
    ...
    with conn.cursor() as cur:
        cur.execute(f"DESCRIBE {alias}.insights.{target.persons_table}")
        ducklake_columns = {row[0] for row in cur.fetchall()}
...
def _compute_fanout(row_count: int, target_rows_per_file: int, max_fanout: int) -> int:
    ...
    Pure arithmetic — no Datastore I/O, so no retry decorator (the count that feeds it
...
def _estimate_export_row_count(client: Client, count_sql: str, settings: dict[str, Any]) -> int:
    ...
    Retried like the export itself — it's the only other Datastore call on the path, and
...
def delete_events_partition_data(
    ...
    delete_sql = f"""
    DELETE FROM {alias}.insights.{target.events_table}
    WHERE team_id = %s
      AND timestamp >= %s
      AND timestamp < %s
    """
...
def delete_persons_partition_data(
    ...
    if partition_date is None:
        delete_sql = f"""
        DELETE FROM {alias}.insights.{target.persons_table}
        WHERE team_id = %s
        """
...
def export_events_to_duckling_s3(
    ...
    The day is fanned out across a volume-sized number of Parquet files via Datastore
    ...
    # {_partition_id} is substituted by Datastore per PARTITION BY bucket, so one
    ...
    s3_url = get_s3_url_for_datastore(target.bucket, target.bucket_region, partition_path)
    ...
    # target also derives an automatic fan-out limit. Datastore's 100 GiB ceiling covers
...
def _glob_run_files(conn: psycopg.Connection[Any], s3_glob: str) -> list[str]:
    ...
    A zero-event team-day is normal in a historical backfill: Datastore writes no
...
def _assert_live_spec_matches(
    ...
    if actual != expected:
        raise RuntimeError(
            f"ducklake_file_partition_value fix-up: live catalog spec for insights.{table_name} "
            ...
            with catalog_conn, catalog_conn.cursor() as cur:
                ...
                if len(rows) != 1:
                    raise RuntimeError(
                        ...
                        f"for insights.{table_name}, got {len(rows)}"
                    )
...
def register_files_with_duckling(
    ...
    try:
        ...
        for s3_path in files:
            ...
            conn.execute(
                psql.SQL("CALL ducklake_add_data_files({}, {}, {}, schema => 'insights', allow_missing => true)").format(
                    psql.Literal(alias),
        ...
        if session is not None:
            ...
            if config.delete_tables:
                ...
                try:
                    session.run(
                        "drop events table",
                        lambda c: c.execute(f"DROP TABLE IF EXISTS {DUCKLAKE_ALIAS}.insights.{target.events_table}"),
                    )
                except Exception:
        ...
        if session is not None:
            ...
            if config.delete_tables:
                ...
                try:
                    session.run(
                        "drop persons table",
                        lambda c: c.execute(f"DROP TABLE IF EXISTS {DUCKLAKE_ALIAS}.insights.{target.persons_table}"),
                    )
                except Exception:
...
# Datastore only ever sees a few concurrent exports. These knobs govern how the sensor
# ENQUEUES into that limit; they never widen Datastore load.
#
...
# Cap on per-team earliest-event Datastore lookups per tick. This is the only expensive
...
@sensor(
    ...
    minimum_interval_seconds=60,  # Cheap now (no per-month Datastore query) — tick often to keep the queue fed
    job_name="duckling_events_backfill_job",
...
def duckling_events_full_backfill_sensor(
    ...
    limit throttles to a handful of concurrent runs to protect Datastore):

      * ``earliest_event_date`` is resolved from Datastore ONCE per team and cached on the
        team's duckgres control-plane row (bounded to EVENTS_BACKFILL_MAX_EARLIEST_LOOKUPS_PER_TICK
        lookups per tick), so the hot path issues no Datastore queries and an org's whole
    ...
    #    only expensive op (one Datastore query/team), bounded per tick and cached forever.
