...
def validate_cdc_prerequisites(
    ...
    if management_mode == "insights":
        ...
        # Self-managed: the DBA creates the publication out-of-band, Insights creates
...
def _check_replication_role(conn: psycopg.Connection) -> list[str]:
    """Check if current user can create logical replication slots (Insights-managed mode).
