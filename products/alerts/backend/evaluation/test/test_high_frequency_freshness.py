...
from products.alerts.backend.evaluation.insightsql import InsightsQLExtractor
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, False, IF_STALE),  # no time axis (funnels/insightsql), normal cadence → cache
...
# Only trends/detector escalate on hourly buckets (real time axis); funnels/insightsql have none, so for
...
def _insightsql_forward(mode):
    alert = MagicMock()
    alert.condition = {"type": AlertConditionType.ABSOLUTE_VALUE}
    alert.config = {"type": "InsightsQLAlertConfig", "evaluation": "last_row"}
    InsightsQLExtractor().extract(alert, MagicMock(), MagicMock(), mode)
...
EXTRACTOR_FORWARDING_CASES = [
    ...
    pytest.param(
        "products.alerts.backend.evaluation.insightsql.calculate_for_query_based_insight",
