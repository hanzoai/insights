        ...
        bulkDeleteResponse: [
            null as InsightBulkDeleteResponseApi | null,
            {
                bulkDeleteInsights: async ({ ids }: { ids: number[] }) => {
                    return (await api.create(`v1/environments/${values.currentTeamId}/insights/bulk_delete/`, {
                        ids,
        ...
        bulkRestoreResponse: [
            null as InsightBulkRestoreResponseApi | null,
            {
                bulkRestoreInsights: async ({ ids }: { ids: number[] }) => {
                    return (await api.create(`v1/environments/${values.currentTeamId}/insights/bulk_restore/`, {
                        ids,
    ...
    listeners(({ actions, asyncActions, values, selectors }) => ({
        ...
        bulkDeleteInsightsSuccess: ({ bulkDeleteResponse }) => {
            ...
            if (deletedIds.length > 0) {
                ...
                toast.info(`Deleted ${deletedIds.length} ${noun}.${skippedSuffix}`, {
                ...
                toast.warning(`No insights deleted. ${skipped.length} skipped due to permissions.`)
            }
            actions.loadInsights()
        },
        bulkDeleteInsightsFailure: () => {
        ...
        bulkRestoreInsightsSuccess: ({ bulkRestoreResponse }) => {
            ...
            if (restored.length > 0) {
                ...
                toast.success(`Restored ${restored.length} ${noun}`)
            }
            actions.loadInsights()
        },
        bulkRestoreInsightsFailure: () => {
            toast.error('Failed to restore insights')
        },
        loadDraftQuery: () => {
