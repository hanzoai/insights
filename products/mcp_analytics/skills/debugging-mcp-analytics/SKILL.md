...
description: >
  Debug, support, and build Insights MCP Analytics — product analytics for MCP
  servers (the `@hanzo/mcp` and `insights.mcp` SDKs plus the mcp_analytics
...
**Product analytics for MCP servers.** A team ships an MCP server; the `@hanzo/mcp` SDK
wraps it in one line; every tool call, agent **intent**, and failure lands in Insights as a
`$mcp_*` event you can query, chart, alert on, and cluster — plus a dedicated dashboard. The
MCP-layer sibling of `@hanzo/ai`.
...
Insights dogfoods it — its own MCP server instruments itself, and that data drives the
dashboard. Public tracking: mega-issue **Insights/insights#64016**, which is the live source
...
| --------------------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Product / dashboard**           | `Insights/insights` (this repo) | `products/mcp_analytics/` — Django/DRF + InsightsQL query runners + Temporal, Kea frontend, the `query-mcp-*` tool registry, and the analysis skills            |
| **Self-instrumented server**      | `Insights/insights` (this repo) | `services/mcp/` — Insights's own MCP server (Hono); the dogfood event producer. Also hosts the _generated_ `query-mcp-*` handlers                           |
| **Shared query reference**        | `Insights/insights` (this repo) | `products/insights_ai/skills/querying-insights-data/references/models-mcp.md`                                                                                |
| **TypeScript SDK** `@hanzo/mcp` | `Insights/insights-js`          | `packages/mcp/` — the library customers install. Vocabulary source of truth: `src/extensions/constants.ts`. Internals: `docs/ARCHITECTURE.md`              |
| **Python SDK** `insights.mcp`      | `Insights/insights-python`      | `insights/mcp/` — mirrors `insights.ai`. Ships inside `insights` (`pip install insights`); `mcp`/`fastmcp` are lazily-imported peer deps, **no `[mcp]` extra** |
| **Docs**                          | `Insights/hanzo.ai`         | `contents/docs/mcp-analytics/` (incl. `surfaces/`), plus `src/hooks/productData/mcp_analytics.tsx` and the `mcp_analytics` entry in `src/data/tools.ts`    |
| **Install codemod**               | `Insights/context-mill`        | `context/skills/mcp-analytics/{config.yaml,description.md}`                                                                                                |
| **Wizard CLI**                    | `Insights/wizard`              | `bin.ts`, `src/commands/mcp-analytics.ts`, `src/lib/programs/mcp-analytics/`                                                                               |
| **Wizard test harness**           | `Insights/wizard-workbench`    | `apps/mcp-analytics/` fixtures                                                                                                                             |

**Don't conflate:**

- `Insights/mcp-analytics` is the **archived prototype** of this SDK — stuck at `0.0.9` with an
  old `track(server, {...})` API. It published under the same `@hanzo/mcp` name, so grepping
  that name can land you there. npm `@hanzo/mcp` now resolves to `Insights/insights-js`.
...
- `wizard mcp add` installs the Insights **MCP server** into a coding agent. That is NOT
...
1. **Always resolve the effective tool name through `EFFECTIVE_TOOL_SQL`.** The expression
   lives once, in `products/mcp_analytics/backend/insightsql_queries/base.py`:
   ...
   registered in `insights/taxonomy/taxonomy.py` and coalesced defensively here, but **nothing
...
4. **`harness` is derived, and its logic exists in three places that must move in lockstep:**
   ...
   `products/insights_ai/skills/querying-insights-data/references/models-mcp.md`.
5. **Check which SDK version the dogfood server is on before trusting dogfood data.**
   ...
   identity, payload redaction) can be absent from Insights's own data even when documented as
   current. A query filtering on `$lib = 'insights-node-mcp'` silently excludes all dogfood
   traffic if that pin predates SDK 0.7.0. Note too that `services/mcp` uses the
   **custom-dispatcher** (`InsightsMCP`) path rather than `instrument()`, so behaviour living
...
All data lives on the shared Datastore **`events`** table — there is **no dedicated table**.
...
`Insights/insights-js`, exported as `InsightsMCPAnalyticsEvent` / `InsightsMCPAnalyticsProperty`
(import them for typesafe queries). Insights-side descriptions — including the server-stamped
and exec-mode properties the SDK does not define — live in `insights/taxonomy/taxonomy.py`.
...
Full property tables — split by provenance (SDK-emitted vs stamped by Insights's own server vs
...
- `exploring-mcp-tool-usage` — router: takes a broad "how is my MCP doing"
...
**Typed tools** exist for most questions and are preferable to raw SQL: `insights:query-mcp-tool-stats`,
`-daily-stats`, `-failures`, `-failure-occurrences`, `-descriptions`, `-neighbors`,
`-sample-intents`, `-top-users`, and `insights:query-mcp-harness-breakdown`, plus session tools
(`insights:mcp-analytics-sessions-list` / `-tool-calls` / `-generate-intent`) and the intent-cluster
...
1. **Instrument** -> the server emits `$mcp_*` events via the SDK.
   ...
   protocol stream — wire a `logger`); a disabled or misconfigured insights-node client.
   For `services/mcp` there is a **single emission path**: `src/hono/analytics.ts` +
   `src/hono/tool-executor.ts` -> `getInsightsClient()` (`src/lib/insights/client.ts`) ->
   `InsightsMCP`, consumed through the dependency alias `@hanzo/mcp-analytics` (the alias
...
2. **Ingest** -> events land in Datastore `events`. _Breaks:_ ordinary ingestion and quota
   problems; `$session_id` not materialized, which breaks session grouping.
3. **Session list** -> `backend/logic.py::list_mcp_sessions` runs InsightsQL over a **7-day
...
4. **Charts and tool quality** -> typed `AnalyticsQueryRunner` subclasses in
   `backend/insightsql_queries/` (`base.py`, `dashboard_series.py`, `harness_breakdown.py`,
   `tool_quality_tables.py`, `tool_tables.py`), dispatched via the generic `/query/` endpoint
   and enumerated in `backend/facade/queries.py`, with schemas in `insights/schema.py`.
   Gate: `insightsql_queries/base.py::validate_mcp_analytics_access` — the feature flag **plus**
...
5. **Intent generation** (on demand, per session) -> collect `$mcp_intent` values -> an LLM
   summary of at most two sentences -> Postgres `insights_mcp_session`. A second,
...
6. **Intent clustering** -> embed (cached in `MCPIntentEmbeddingCache`) -> agglomerative
   ...
   (`insights/temporal/mcp_analytics/intent_clustering/schedule.py`, behind the
   ...
   first `$mcp_intent` straight from Datastore and only _overrides_ it with the stored LLM
...
8. **Frontend** -> Kea scene `MCPAnalyticsScene.tsx`, with tabs enumerated by
   ...
   - **Dashboard**: quill composable `Metric` tiles and `@hanzo/quill-primitives`, plus
   ...
   - **Notifications**: first-party destinations for MCP events and recurring AI reports
     (`frontend/notifications/`), thin wiring over the generic script-function destination and
...
| ----------------------------------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SDK behaviour, events, options, instrumentation | `Insights/insights-js`                     | Work in `packages/mcp`. Run its unit tests, build, and lint. **Add a changeset.** Ships to npm; then bump the alias in `services/mcp/package.json` to pick it up.                                                                                                                                                                                                                        |
| Dashboard, queries, clustering, API             | this repo                                | A new chart means a query runner in `backend/insightsql_queries/` behind `validate_mcp_analytics_access` — never a SQL template. Obey Hard rules 1-3.                                                                                                                                                                                                                                         |
| A new `query-mcp-*` agent tool                  | this repo (two places)                   | 1) an entry in `products/mcp_analytics/mcp/tools.yaml` with `schema_ref`, `scopes`, `description`, `feature_flag`; 2) the matching `<Name>Query` schema and `<Name>QueryRunner` in `backend/insightsql_queries/`; 3) regenerate the tool handlers from `services/mcp` (see its `package.json` scripts). The generic `createQueryWrapper` handles the tool shape — no hand-written TypeScript. |
| Insights's own dogfood events                    | this repo                                | `services/mcp/src/hono/analytics.ts` + `tool-executor.ts`; client in `src/lib/insights/client.ts`.                                                                                                                                                                                                                                                                                        |
| Docs                                            | `Insights/hanzo.ai`                    | Keep the event and property tables in `contents/docs/mcp-analytics/events.mdx` synced with **both** the TypeScript `constants.ts` and the Python `insights/mcp/constants.py`.                                                                                                                                                                                                             |
| The install codemod or the wizard command       | `Insights/context-mill`, `Insights/wizard` | See [references/wizard-and-onboarding.md](references/wizard-and-onboarding.md) — in particular the rule about which changes need a wizard release and which do not.                                                                                                                                                                                                                      |

**Rule of thumb:** a change to _what gets captured, or how servers are instrumented_ belongs
in the SDKs. _How data is shown, aggregated, or clustered_ belongs in this product. _Insights's
