...
def _sample_loop(interval_seconds: float) -> None:
    log = structlog.get_logger("insights.web_memory_sampler")
...
def start_web_memory_sampler() -> None:
    ...
    `insights_web_worker_rss_mb` Prometheus gauge and a `worker_memory` log line — so the
    gradual climb toward the cgroup limit that precedes an OOM kill is visible in both
    metrics and Insights logs rather than only on infra dashboards.

    MUST be called post-fork, from inside each worker (see insights/wsgi.py). Nginx Unit
