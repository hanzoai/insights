...
export class RetentionInsight extends ChartInsightBase {
    ...
    async selectPeriod(period: 'days' | 'weeks' | 'months'): Promise<void> {
        const periodButton = this.conditionPanel.locator('button.Select', { hasText: /^(days|weeks|months)$/ })
    ...
    async toggleCumulative(): Promise<void> {
        ...
        const toggle = this.optionsPanel.locator('.SegmentedButton li', { hasText: 'on or after' })
    ...
    private get customBracketsBox(): Locator {
        return this.customBracketsCheckbox.locator('.Checkbox__box')
    }
    ...
    private get bracketInputs(): Locator {
        return this.conditionPanel.locator('.flex.items-center.gap-2:has(.w-24) .Input input')
    }
