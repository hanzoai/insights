...
class TestQueryNewRecords:
    def test_continuous_sync_uses_partition_field_and_placeholders(self):
        ...
        with patch(f"{FETCHER_MODULE_PATH}.execute_insightsql_query", return_value=mock_result):
    ...
    def test_reraises_on_query_error(self):
        ...
        # Re-raising lets the activity's retry policy handle transient InsightsQL/Datastore failures.
        config = _make_config()

        with (
            patch(f"{FETCHER_MODULE_PATH}.execute_insightsql_query", side_effect=Exception("query failed")),
...
class TestCheckActionability:
    ...
    async def test_assumes_actionable_after_retries_exhausted(self):
        ...
        with patch(f"{PIPELINE_MODULE_PATH}.hanzo_insights"):
    ...
    async def test_passes_team_attribution_headers(self):
        ...
        assert headers["x-insights-property-ai_stage"] == "actionability"
        assert headers["x-insights-property-source_product"] == output.source_product
        assert headers["x-insights-property-source_type"] == output.source_type
        # ai_product and $ai_billable are owned by the gateway product config, not headers
        assert "x-insights-property-ai_product" not in headers
    ...
    async def test_gateway_mode_labels_ride_on_properties_blob(self):
        ...
        # The Go gateway reads labels only from X-Insights-Properties; the per-key headers are gone.
        ...
        assert "x-insights-property-ai_stage" not in headers
        assert json.loads(headers["X-Insights-Properties"]) == {
...
class TestSummarizeDescription:
    ...
    async def test_passes_team_attribution_headers(self):
        ...
        headers = call_kwargs["extra_headers"]
        assert headers["x-insights-property-ai_stage"] == "summarization"
        # ai_product and $ai_billable are owned by the gateway product config, not headers
        assert "x-insights-property-ai_product" not in headers
        assert "x-insights-property-$ai_billable" not in headers
...
class TestEmitSignals:
    ...
    async def test_continues_on_individual_emit_failure(self):
        ...
        with (
            ...
            patch(f"{PIPELINE_MODULE_PATH}.activity"),
            patch(f"{PIPELINE_MODULE_PATH}.hanzo_insights.capture") as capture,
        ):
...
class TestEmitActivityTableNameResolution:
    # Regression: InsightsQL exposes warehouse tables under keys built by
    ...
    @pytest.mark.parametrize(
        "prefix,storage_name,expected_insightsql_name",
        [
    ...
    @pytest.mark.asyncio
    async def test_passes_insightsql_resolvable_table_name_to_fetcher(
        self, prefix: str | None, storage_name: str, expected_insightsql_name: str
    ):
        ...
        assert captured_context["table_name"] == expected_insightsql_name
