...
class InsightsDatabaseConnectionError(Exception):
    """Raised when loading sync metadata from Insights's own database fails to connect.

    This is a Insights-side infrastructure blip (e.g. a transient DNS failure resolving our
...
class ForeignServerUnreachableError(Exception):
    ...
    `InsightsDatabaseConnectionError`: its message intentionally avoids those substrings so a transient
