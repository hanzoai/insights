...
from insights.temporal.common.worker import ALL_INTERCEPTOR_CLASSES
from insights.temporal.session_replay.surfacing_scoring_sweep.activities import (
    list_chunks_activity,
    score_chunk_activity,
)
from insights.temporal.session_replay.surfacing_scoring_sweep.constants import SCORE_CHUNK_ACTIVITY_TIMEOUT
...
class TestRecordTickSummary:
    ...
    def test_emits_counter(
        ...
        with patch(
            "insights.temporal.session_replay.surfacing_scoring_sweep.metrics.get_metric_meter",
...
class TestSurfacingScoringActivityInterceptor:
    async def _run_interceptor(
        ...
        *,
        ...
        with (
            patch(
                "insights.temporal.session_replay.surfacing_scoring_sweep.metrics.activity.info",
                return_value=MagicMock(activity_type=activity_type),
            ),
            patch(
                "insights.temporal.session_replay.surfacing_scoring_sweep.metrics.ExecutionTimeRecorder",
            ) as mock_recorder,
        ):
