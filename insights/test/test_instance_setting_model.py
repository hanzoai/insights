...
from insights.models.instance_setting import (
    InstanceSetting,
...
def test_model_creation(db):
    ...
    assert instance.key == "constance:insights:MATERIALIZED_COLUMNS_ENABLED"
...
def test_value_property_handles_bare_strings(db):
    ...
    instance = InstanceSetting.objects.create(
        key="constance:insights:TEST_BARE_HEX", raw_value="1ee937c5cba4da2dbdce84824cfa0e93"
    )
    assert instance.value == "1ee937c5cba4da2dbdce84824cfa0e93"

    # String with dots
    instance2 = InstanceSetting.objects.create(
        key="constance:insights:TEST_BARE_DOTTED", raw_value="910200304849.9220719972545"
    )
    assert instance2.value == "910200304849.9220719972545"

    # Empty string
    instance3 = InstanceSetting.objects.create(key="constance:insights:TEST_BARE_EMPTY", raw_value="")
    assert instance3.value == ""

    # Valid JSON values still work as before
    instance4 = InstanceSetting.objects.create(key="constance:insights:TEST_VALID_JSON_STR", raw_value='"hello"')
    assert instance4.value == "hello"

    instance5 = InstanceSetting.objects.create(key="constance:insights:TEST_VALID_JSON_BOOL", raw_value="true")
    assert instance5.value is True

    instance6 = InstanceSetting.objects.create(key="constance:insights:TEST_VALID_JSON_INT", raw_value="42")
...
def test_admin_save_model_wraps_bare_strings(db):
    ...
    from insights.admin.admins.instance_setting_admin import InstanceSettingAdmin

    site = AdminSite()
    admin = InstanceSettingAdmin(InstanceSetting, site)
    factory = RequestFactory()
    request = factory.post("/admin/insights/instancesetting/add/")

    # Bare string should be wrapped
    obj = InstanceSetting(key="constance:insights:TEST_ADMIN_BARE", raw_value="abc123")
    ...
    obj2 = InstanceSetting(key="constance:insights:TEST_ADMIN_VALID_STR", raw_value='"already_valid"')
    ...
    obj3 = InstanceSetting(key="constance:insights:TEST_ADMIN_VALID_BOOL", raw_value="true")
    ...
    obj4 = InstanceSetting(key="constance:insights:TEST_ADMIN_VALID_NUM", raw_value="42")
...
def test_db_change_propagates_after_ttl(db, monkeypatch):
    ...
    InstanceSetting.objects.create(key="constance:insights:EMAIL_HOST", raw_value='"smtp.changed.test"')
    assert get_instance_setting("EMAIL_HOST") == initial

    monkeypatch.setattr("insights.models.instance_setting._INSTANCE_SETTING_CACHE_TTL_SECONDS", 0)
