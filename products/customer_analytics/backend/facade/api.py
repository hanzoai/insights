...
from insights.models import Integration, OrganizationMembership, Tag
from insights.models.activity_logging.activity_log import AuditableScope, Detail, Trigger, changes_between, log_activity
from insights.models.group.util import get_group_by_key
from insights.models.group_type_mapping import get_group_types_for_project
from insights.models.tag import tagify
...
# The "Update account property" workflow action (Script template) stores the custom property values it
# sets keyed by definition id under its ``properties`` input — the link we resolve into references.
logger = structlog.get_logger(__name__)

logger = structlog.get_logger(__name__)

_ACCOUNT_PROPERTY_TEMPLATE_ID = "template-insights-update-account-property"
...
def count_accounts_for_audience(team: Team, filters: "AccountAudienceFilters") -> int:
    from products.customer_analytics.backend.insightsql_queries import (  # noqa: PLC0415 — keeps InsightsQL off the import path
        account_audience,
    )
...
class CanonicalCustomPropertyReadOnlyError(Exception):
    """Raised when an update would change a field Insights owns on a canonical custom property —
...
def _set_tags(
    ...
    Mirrors ``insights.api.tagged_item.set_tags_on_object`` + ``cleanup_orphan_tags`` but
...
def _log_activity_swallowing(
    *,
    ...
    """Replicates ``insights.api.utils.log_activity_from_viewset`` — including its blanket
...
def _assert_canonical_fields_unchanged(definition: CustomPropertyDefinition, fields: dict[str, Any]) -> None:
    """Refuse a rename or a type change on a canonical property — Insights owns both.
    ...
    for attr in ("name", "display_type"):
        if attr in fields and fields[attr] != getattr(definition, attr):
            raise CanonicalCustomPropertyReadOnlyError(
                f"'{definition.name}' is set by Insights, so its {attr.replace('_', ' ')} can't be changed."
            )
...
def list_accounts_for_view(
    *,
    ...
    # accounts list InsightsQL runner's allRolesUnassigned.
...
def _to_user_basic_info(user: "User | None") -> contracts.UserBasicInfo | None:
    ...
    return contracts.UserBasicInfo(
        ...
        mascot_config=user.mascot_config,
...
def list_account_notes_for_view(
    *,
    ...
    if assigned_to_ids:
        # "Assigned to" means any active relationship, matching the accounts list InsightsQL
