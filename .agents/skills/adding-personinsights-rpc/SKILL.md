---
name: adding-personinsights-rpc
description: >
  Guide for adding a new RPC to personinsights-replica and personinsights-router.
  ...
  and index compatibility validation. Use when adding a new gRPC endpoint to personinsights,
  migrating a Django ORM query to personinsights, or extending the personinsights service API.
---

# Adding a personinsights RPC
...
| `insights_grouptypemapping`           | NonPersonData | All ops: replica                                             |
| `insights_cohortpeople`               | NonPersonData | All ops: replica                                             |
| `insights_featureflaghashkeyoverride` | NonPersonData | All ops: replica                                             |
| `insights_personoverride`             | PersonData    | Reads/writes follow person routing                           |
| `insights_personlessdistinctid`       | PersonData    | Same as person                                               |

If the table is not listed above, **stop** — this data should not go through personinsights.
...
All proto files live in `proto/personinsights/`.
See [references/proto-conventions.md](references/proto-conventions.md) for message conventions and a worked example.

### Where to add what

1. **Message types** → `proto/personinsights/types/v1/<domain>.proto` (person.proto, group.proto, cohort.proto, feature_flag.proto, or common.proto)
2. **Service RPC** → `proto/personinsights/service/v1/service.proto` (the public API clients call)
3. **Replica RPC** → `proto/personinsights/replica/v1/replica.proto` (the internal API the router delegates to)
4. **Leader RPC** → `proto/personinsights/leader/v1/leader.proto` (only if this is a person-data write routed to leader)
...
that residual is accepted today and closes with operation identity (see `personinsights-leader`'s README).
...
- `insights/personinsights_client/proto/__init__.py` — add re-exports for new request/response message types
- `insights/personinsights_client/client.py` — add a wrapper method matching the pattern of existing methods
- `insights/personinsights_client/fake_client.py` — implement the method for test use
...
### 3a. Storage layer (personinsights-replica)

1. **Add a trait method** in `rust/personinsights-replica/src/storage/traits/<domain>.rs`
2. **Implement the query** in `rust/personinsights-replica/src/storage/postgres/<domain>.rs`
...
3. **Add storage tests** in `rust/personinsights-replica/tests/storage_tests.rs`

### 3b. Service layer (personinsights-replica)

1. **Add the RPC handler** in `rust/personinsights-replica/src/service/mod.rs`
...
2. **Add service tests** in `rust/personinsights-replica/tests/service_tests.rs`

### 3c. Router wiring (personinsights-router)

1. **Add the method** to `rust/personinsights-router/src/router/mod.rs`
...
2. **Add the service impl** to `rust/personinsights-router/src/service/mod.rs`
   - Invoke the `route_request!` macro (defined at the top of this file) to delegate to the router
3. **Add to the backend trait** in `rust/personinsights-router/src/backend/mod.rs` and implement in `replica.rs`
4. **Add router tests** in `rust/personinsights-router/tests/`
...
cargo build -p personinsights-replica
cargo build -p personinsights-router
cargo test -p personinsights-replica
cargo test -p personinsights-router
```
