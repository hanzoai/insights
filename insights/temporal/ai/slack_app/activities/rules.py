...
def handle_insights_code_rules_command_activity(
    ...
    if not command:
        return InsightsCodeRulesCommandResult(status="not_a_command")
    ...
    if command.action == "add" and not command.repository:
        return InsightsCodeRulesCommandResult(status="needs_picker", pending_rule_text=command.rule_text)
    ...
    return InsightsCodeRulesCommandResult(status="handled")


@activity.defn
@close_db_connections
def create_insights_code_routing_rule_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs,
    channel: str,
    thread_ts: str,
    user_id: int,
    ...
    if not matched_repo:
        logger.warning(
            "insights_code_rules_add_repo_no_longer_connected",
            repo=repository,
...
@activity.defn
@close_db_connections
def handle_insights_code_slack_mention_command_activity(
    ...
    if not channel or not slack_user_id:
        return InsightsCodeSlackMentionCommandResult(status="done")
    ...
    if result.integration is None:
        ...
        if not result.candidates:
            text = (
                "You don't have access to any Insights project connected to this Slack workspace. "
    ...
    if command.action == "add" and not command.repository:
        return InsightsCodeSlackMentionCommandResult(
            status="needs_picker",
