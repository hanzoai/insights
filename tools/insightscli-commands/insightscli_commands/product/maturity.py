...
Run from repo root:
    bin/insightscli product:maturity          # single product detail
    bin/insightscli product:maturity --all    # ranked report of all products
"""
...
def _count_tach_depends_on(block: str) -> tuple[int, list[str]]:
    ...
    Baseline dependencies (insights, ee) are expected and don't count.
    Cross-product dependencies are the coupling signal.
    """
    baseline = {"insights", "ee"}
    deps: list[str] = []
    in_depends = False
...
def score_models(name: str, backend_dir: Path, assigned_model_counts: dict[str, int]) -> DimensionScore:
    """Are models in products/ or still in insights/models/?

    100 = all models in products/backend/
    0   = all models still in insights/models/ or ee/models/
    """
    ...
    if still_to_move > 0:
        next_steps.append(
            ...
            f"insights/models/ (or ee/models/) and need to be relocated into "
...
def score_facade(backend_dir: Path) -> DimensionScore:
    ...
    if views_path is not None:
        ...
        next_steps.append(
            ...
            "facade has no leverage if the views still live in insights/api/ or ee/api/."
        )
...
def _build_cross_import_maps() -> tuple[dict[str, list[str]], dict[str, list[str]]] | None:
    ...
    for line in result.stdout.strip().split("\n"):
        ...
        # Which product owns the importing file? Empty when it lives in core (ee/, insights/).
        ...
        in_admin = "/insights/admin/" in file_path or rel_path.startswith("insights/admin/")
        if (
            source != imported
            and not is_facade
...
def score_boundaries(
    ...
    if outbound is None:
        ...
        next_steps.append(
            ...
            "`insightscli product:maturity` for an accurate outbound count."
        )
    elif not outbound:
        score += 15
    ...
    if inbound is None:
        ...
        next_steps.append(
            ...
            "`insightscli product:maturity` for an accurate inbound count."
        )
    elif not inbound:
        score += 60
...
def score_codegen(product_dir: Path) -> DimensionScore:
    ...
    manual api.* calls. Having generated/api.ts is free (insightscli build:openapi
    ...
    if score < 100 and manual > 0:
        ...
        next_steps.append(
            ...
            "loose. Fix the backend, run `insightscli build:openapi`, then migrate the call."
        )
        ...
        next_steps.append(
            ...
            "`insightscli build:openapi` and migrate to the generated functions in "
