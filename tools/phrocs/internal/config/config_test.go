...
func TestLoadInsightsConfig(t *testing.T) {
	path := writeYAML(t, `
procs:
  svc:
    shell: echo hi
_insights:
  ...
  exclude_units:
    - celery
`)
	cfg, err := LoadInsightsConfig(path)
	if err != nil {
		t.Fatal(err)
	}
	if cfg == nil {
		t.Fatal("expected non-nil InsightsConfig")
	}
...
func TestLoadInsightsConfig_absent(t *testing.T) {
	path := writeYAML(t, "procs:\n  svc:\n    shell: echo hi\n")
	cfg, err := LoadInsightsConfig(path)
	if err != nil {
		t.Fatal(err)
	...
	if cfg != nil {
		t.Errorf("expected nil when _insights absent, got %+v", cfg)
	}
}
