...
import {
    ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT,
...
// `insightsqlQuery.source` is typed as the full DataTableNode source union; this logic
...
describe('accountsLogic', () => {
    ...
    it('withholds the list and metrics queries until relationship definitions settle', async () => {
        ...
        expect(logic.values.accountsQuerySource).toEqual(logic.values.insightsqlQuery.source)
    ...
    it('keeps the overview tile metrics off the list query so it loads independently', () => {
        const source = logic.values.insightsqlQuery.source as AccountsQuery
    ...
    describe('assignedTo filter and "my accounts" shortcut', () => {
        ...
        it('starts disabled and adds nothing to the query', () => {
            ...
            expect(logic.values.assignedToFilter).toEqual([])
            expect((logic.values.insightsqlQuery.source as AccountsQuery).assignedToUserIds).toBeUndefined()
        })
        ...
        it('the "My accounts" checkbox resolves to the current user id', () => {
            ...
            expect((logic.values.insightsqlQuery.source as AccountsQuery).assignedToUserIds).toEqual([CURRENT_USER_ID])
        })
        ...
        it('the Assigned to picker accepts explicit ids', () => {
            ...
            expect((logic.values.insightsqlQuery.source as AccountsQuery).assignedToUserIds).toEqual([7, 9])
        })
        ...
        it('restores the assigned-to filter from the view hash, independent of the viewer', async () => {
            ...
            expect(logic.values.assignedToFilter).toEqual([7])
            expect(logic.values.assignedToCurrentUser).toBe(false)
            expect((logic.values.insightsqlQuery.source as AccountsQuery).assignedToUserIds).toEqual([7])
        })
    ...
    describe('sortOrder', () => {
        it('starts unset and produces no orderBy on the AccountsQuery', () => {
            ...
            expect(orderByOf(logic.values.insightsqlQuery.source)).toBeUndefined()
        })
        ...
        it('toggleSort on a fresh column starts ascending', () => {
            ...
            expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual(['notebook_count'])
        })
        ...
        it('toggleSort cycles asc -> desc -> null on repeated clicks', () => {
            ...
            expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual(['notebook_count DESC'])
            logic.actions.toggleSort('notebook_count')
            expect(logic.values.sortOrder).toBeNull()
            expect(orderByOf(logic.values.insightsqlQuery.source)).toBeUndefined()
        })
        ...
        it('toggleSort on a different column resets to ascending', () => {
            ...
            expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual(['csm'])
        })

        it('arbitrary column sorts by its alias directly', () => {
            logic.actions.toggleSort('name')
            expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual(['name'])
            logic.actions.toggleSort('name')
            expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual(['name DESC'])
        })
        ...
        describe('custom property columns', () => {
            ...
            it.each(['number', 'currency', 'percent'] as const)(
                ...
                (displayType) => {
                    ...
                    expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual([floatExpr])
                    logic.actions.toggleSort(alias) // desc
                    expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual([`${floatExpr} DESC`])
                }
            )
            ...
            it('sorts a non-numeric custom property lexically by its alias', () => {
                ...
                logic.actions.toggleSort(alias)
                expect(orderByOf(logic.values.insightsqlQuery.source)).toEqual([alias])
            })
        })
    })
    ...
    describe('deep link (path route)', () => {
        ...
        it('filters the list to the account, expands it, and opens the requested tab', async () => {
            ...
            expect(filterExpressionOf(logic.values.insightsqlQuery.source)).toContain(`toString(id) = '${ACCOUNT_ID}'`)
