...
class EvalDatabase:
    ...
    create the base Django test DB once, then drive Insights's own eval DB setup
    (persons DB, Datastore) and leave it committed for the whole run. Sandboxed
    ...
    def setup(self) -> None:
        ...
        # handled by Insights's eval setup below, after the default test DB name is
    ...
    def teardown(self) -> None:
        ...
        if self._eval_setup is not None:
            # Run the generator's post-``yield`` cleanup (Datastore reset / drop).
            try:
                next(self._eval_setup)
