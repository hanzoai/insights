...
description: >
  Recreate open Insights/code pull requests as Insights/insights PRs against the imported
  products/desktop/ tree, preserving commit authorship. Use when asked to port, remake,
  migrate or recreate Insights/code PRs onto the monorepo after the desktop import, whether
  ...
  and opening the monorepo PR with original attribution. Trigger terms: Insights/code,
  code repo PRs, desktop migration, port my PRs, remake PRs onto the monorepo.
---

# Porting Insights/code PRs to the monorepo

Insights/code's `main` is frozen after the desktop import.
The tree lives at `products/desktop/` as a verbatim copy of the source at a pinned SHA, so open PRs on Insights/code can no longer land there and are remade as monorepo PRs instead.
...
Path mapping: `<path>` in Insights/code becomes `products/desktop/<path>`.
...
```sh
gh pr list --repo Insights/code --author <login> --state open \
...
- Never port a PR that was merged into Insights/code. Merged-after-pin changes arrive through the resync protocol in MIGRATION.md, and porting them too would duplicate the diff.
...
```sh
PATCH=$(mktemp)
gh pr diff <N> --repo Insights/code --patch > "$PATCH"
git fetch https://github.com/Insights/code.git refs/pull/<N>/head
```
...
- Body: use `.github/pull_request_template.md`. Carry the original description over, rewriting bare `#N` references as `Insights/code#N` so they do not resolve to unrelated monorepo PRs. State "Ports Insights/code#N" so the pair is linked.
