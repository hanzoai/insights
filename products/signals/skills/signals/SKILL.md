...
name: signals
description: >
  How to query the document_embeddings table for raw signal data using InsightsQL. Use when you need to
...
Signals are automated observations that Insights generates by monitoring a customer's product data across multiple sources — error tracking, web analytics, experiments, session replay, and more. Each signal is a short natural-language description of something noteworthy (e.g. "Error rate spiked 3× on /checkout").
...
Signals and their embeddings are stored in the `document_embeddings` Datastore table, queryable via InsightsQL through the `insights:execute-sql` MCP tool. They may provide a useful way to semantically query for recent things that happened in the user's product.
...
The InsightsQL table alias is `document_embeddings`. InsightsQL automatically constrains queries to the current team — you never need to filter on `team_id`. Key columns for signals:
...
The `model_name` filter is especially critical — the InsightsQL engine uses it to route to the correct underlying Datastore table. If the `WHERE model_name = ...` equality filter is missing or uses an unknown model, the query will fail with an "Invalid model name" error (you cannot use `IN` or other expressions here).
...
**Note:** InsightsQL supports `metadata.field_name` dot access on the raw `metadata` JSON column, but this type information is lost when the column passes through aggregate functions like `argMax()`. You MUST extract individual metadata fields inside the inner dedup subquery — do NOT pass the whole `metadata` blob through `argMax` and dot into it in the outer query, as this will fail with a type error.

InsightsQL's JSON dot access always extracts values as `Nullable(String)`, regardless of the underlying JSON type. This means `metadata.deleted` is the string `'true'`/`'false'`/`null`, not a Bool. Use `deleted != 'true'` — do NOT use `NOT deleted`.
...
`embedText()` is a InsightsQL function that converts a text string into an embedding vector at query compile time. It calls the embedding API and inlines the resulting vector as a constant before executing the query. This means you can do semantic search in a single query without any external embedding step.
...
The `metadata` column is a JSON string. InsightsQL supports `metadata.field_name` dot access **only on the raw table column**. After aggregation (e.g. `argMax`), the JSON type is lost and dot access will fail. Always extract the fields you need inside the dedup subquery.
...
6. **Queries should not end with a semicolon.** InsightsQL does not use them.
...
8. **Extract metadata fields inside the dedup subquery.** InsightsQL's `metadata.field` dot access only works on the raw table column. After `argMax()` aggregation, the JSON type is lost and dot access will fail with a type error. Always use `argMax(metadata.field_name, inserted_at) as field_name` in the inner query.
9. **All JSON dot-access values are `Nullable(String)`.** InsightsQL extracts every JSON field as a String, even booleans and numbers. For `metadata.deleted`, use `deleted != 'true'` — do NOT use `NOT deleted`.
10. **Don't alias `argMax(timestamp, inserted_at)` as `timestamp` if the same inner query also filters on the raw `timestamp` column.** InsightsQL resolves the alias name first, causing an "aggregate in WHERE" error. Either use a distinct alias like `signal_ts`, or move the time filter to the outer query.
