...
from insights.schema_enums import ProductItemCategory, ProductKey

# This module loads at django.setup() via the file-system product list; insights.schema
# (the pydantic models) is runtime-imported only when products.json is actually loaded.
if TYPE_CHECKING:
    from insights.schema import ProductItem, ProductsData
...
class Products:
    ...
    def _load_data(self) -> None:
        ...
        from insights.schema import ProductsData  # noqa: PLC0415

        if self._file_path is None or not self._file_path.exists():
            raise FileNotFoundError(
                f"products.json not found at {self._file_path}. Generate it by running: insightscli build:products"
            )
