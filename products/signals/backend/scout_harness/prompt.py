...
Your job: explore this Insights project, decide what is worth surfacing, and emit findings via `emit_signal` so the existing Signals pipeline can group, research, and route them to the inbox. You are *one* of several scouts running on this project, so be selective. Aim for fewer, better signals.
"""
...
3. **Investigate.** Use the Insights MCP read tools to gather evidence, discovering what's available at run time. Your skill body tells you *what* to look at."""
...
- **Identify a reviewer two ways, and never guess a handle.** `github_login` is a bare lowercase login (`{github_login: "octocat"}`, no `@`, no display name). `user_uuid` (`{user_uuid: "..."}`) is for when your evidence already names a Insights user (an account owner, an entity's creator), and the server resolves it to their linked GitHub login for you. The inbox routes by matching the login exactly, so a guessed, mis-cased, or display-name handle reaches no one: when you only know the owner as a Insights member, pass their `user_uuid`.
...
_SELF_IMPROVEMENT_TAIL = """- Routing: a problem with the tools, the harness, or these shared instructions still goes to `agent-feedback`, which reaches the Insights team rather than yours. An `improve:` entry is only for changes to your own skill body.
...
- **At most one submission per run, near close-out, mentioned in your summary.** This is a side report to the Insights team, never a way to end your turn or skip work: finish the run (emit / remember / summary) exactly as you would otherwise.
