...
Commands:
    insightscli migrations:status      - Show migration diff between DB and code
    insightscli migrations:down        - Roll back orphaned migrations
    insightscli migrations:up          - Apply pending migrations
    insightscli migrations:sync        - Smart sync: down + up in one step
    insightscli migrations:cache-sync  - Populate cache from currently applied migrations

Migration File Caching:
    When migrations are applied, their files are cached to ~/.cache/insights-migrations/.
...
def _get_subprocess_env() -> dict[str, str]:
    """Get environment for subprocess calls that need insightscli module access."""
    env = dict(os.environ)
    env["DJANGO_SETTINGS_MODULE"] = "insights.settings"
    # Ensure common/ is in PYTHONPATH so insightscli module is importable
...
# Matches lines like "Applying insights.0952_add_billable_action..."
...
def _show_manual_rollback_instructions(uncached: list[MigrationInfo], command_name: str) -> None:
    ...
    click.echo("After manual rollback, run 'insightscli migrations:sync' again.\n")
    click.echo("Or use --force to skip schema rollback (just removes DB records):")
    click.secho(f"  insightscli {command_name} --force\n", fg="yellow")
    raise SystemExit(1)
...
def _get_database_url() -> str:
    ...
    user = os.environ.get("PGUSER", "insights")
    password = os.environ.get("PGPASSWORD", "insights")
    database = os.environ.get("PGDATABASE", "insights")
...
def _compute_migration_diff() -> MigrationDiff:
    ...
    Only tracks apps that we manage (insights, ee, rbac, products/*).
...
def migrations_status() -> None:
    ...
    if not diff.orphaned and not diff.pending:
        ...
        click.echo("Run 'insightscli migrations:sync' to fix, or use 'down'/'up' individually.")
...
def migrations_cache_sync() -> None:
    ...
    Run this once after setting up insightscli, or after pulling new migrations.
    """
