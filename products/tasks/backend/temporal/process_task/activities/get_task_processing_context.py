...
from insights.temporal.common.utils import asyncify, close_db_connections
...
def _is_rtk_enabled(
    *,
    ...
    otherwise the per-run state override (the user's toggle in Insights Desktop settings)
...
def get_task_processing_context(input: GetTaskProcessingContextInput) -> TaskProcessingContext:
    ...
    pr_loop_enabled = (
        ...
        or hanzo_insights.feature_enabled(
            "tasks-pr-loop",
            distinct_id=distinct_id,
            groups={"organization": organization_id},
            group_properties={"organization": {"id": organization_id}},
