# ruff: noqa: T201
"""Shared helpers for the InsightsQL parser-parity diagnostic scripts.
...
Importing this module pulls in the InsightsQL parser and AST layer only, which
...
from insightsql_parser import parse_string_literal_text as _cpp_parse_string_literal_text
from insightsql_parser_rs import parse_string_literal_text as _rust_parse_string_literal_text

from insights.insightsql.errors import BaseInsightsQLError, ParsingError
from insights.insightsql.parser import (
    InsightsQLParserShadowMismatch,
    parse_expr,
    parse_program,
    parse_select,
...
# covers the Script imperative-statement layer (let / if / while / for /
...
def _node_fields(node: Any) -> list[tuple[str, Any]]:
    ...
    InsightsQL AST nodes are `__slots__` dataclasses, so `dataclasses.fields()`
...
def asts_agree(oracle: Any, candidate: Any) -> bool:
    ...
    comparison in `insights/insightsql/parser.py` (dataclass `==`), then falls back to
...
def _iter_child_nodes(value: Any) -> Iterator[Any]:
    ...
    InsightsQL AST) but stopping at scalars. A dataclass *type* (as opposed to an
...
def _abort_on_shadow_mismatch(backend: str, exc: InsightsQLParserShadowMismatch) -> NoReturn:
    """Abort the whole diagnostic when a backend raises InsightsQLParserShadowMismatch.
    ...
    raise SystemExit(
        f"\nFATAL: backend {backend!r} raised InsightsQLParserShadowMismatch during the grind.\n"
...
def _safe_parse(query: str, rule: str, backend: str) -> tuple[str, Any, str | None]:
    ...
    - `("reject", None, signature)` — `BaseInsightsQLError`; a legitimate
      "not valid InsightsQL". `signature` is the normalised error message.
    ...
    except InsightsQLParserShadowMismatch as e:
...
def _probe_backend(rule: str, backend: str) -> str | None:
    ...
    swallows `BaseInsightsQLError` (legitimate rejections) AND would let an
...
def shrink_to_shape(
    ...
    # Imported lazily: shrinkray is the optional `insightsql-parser-parity` group,
    ...
    from insights.insightsql.scripts import _shrink  # noqa: PLC0415
...
def insightscli_bin(repo_root: Path) -> str:
    """Absolute path to `bin/insightscli` — `$PATH` has it under flox for
    interactive shells but agent / CI shells often don't."""
    cand = repo_root / "bin" / "insightscli"
    if not cand.is_file():
        raise RuntimeError(f"bin/insightscli not found at {cand} — is the repo root correct?")
    return str(cand)
...
def discover_metabase_db(
    ...
    *,
    ...
    """Return a Metabase database id for the given `engine` (`datastore`
    ...
    Raises if `insightscli metabase:databases` fails — usually a stale cookie
    (`insightscli metabase:login` first)."""
    out = subprocess.check_output(
        [insightscli_bin(repo_root), "metabase:databases", "--region", region, "--format", "json"],
        cwd=repo_root,
        text=True,
...
def _fetch_metabase_page(region: str, database_id: int, sql: str, tmp_path: Path, repo_root: Path) -> dict:
    ...
    subprocess.run(
        ...
        check=True,
        cwd=repo_root,
        timeout=360,  # process-level backstop above insightscli's own `--timeout 300`
    )
...
def download_corpus(
    ...
    try:
        while len(all_rows) < sql_limit:
            ...
            print(f"  downloading rows {offset:,}–{offset + page_limit:,} via `insightscli metabase:query` …")
...
def corpus_try_parse(query: str, rule: str, backend: str) -> tuple[str, Any, str | None]:
    ...
    `reject` (detail = the raw `BaseInsightsQLError` message), or `crash`
...
def print_corpus_summary(result: ParityResult, *, oracle: str, candidate: str) -> None:
    ...
    _dump("Crash buckets (non-InsightsQL exceptions)", result.crash_buckets, 30)
