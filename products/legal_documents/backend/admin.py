...
class LegalDocumentAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request: HttpRequest, extra_context: dict[str, Any] | None = None) -> HttpResponse:
        if not (is_cloud() or is_dev_mode()):
            messages.warning(
                request,
                "Legal documents are only generated on Insights Cloud. On self-hosted "
    ...
    def organization_link(self, document: LegalDocument) -> SafeString:
        url = reverse("admin:insights_organization_change", args=[document.organization_id])
...
# OrganizationAdmin pulls it in via get_inlines() — see insights.admin.inline_registry.
