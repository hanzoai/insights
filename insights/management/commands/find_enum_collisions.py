...
Run this when `insightscli build:openapi-schema` fails with messages like
"enum naming encountered a non-optimally resolvable collision" / "Format5eaEnum" — it
prints a suggested ENUM_NAME_OVERRIDES entry for insights/settings/web.py. The suggestion
...
See also:
    insights/settings/web.py — ENUM_NAME_OVERRIDES (where the fix goes)
...
# Collision logic lives in insights.openapi.enum_collisions so CI
# (test_widget_openapi_enums.py) and this command share one implementation.
from insights.openapi.enum_collisions import collect_enum_hashes, find_unresolved_enum_collisions
