...
from insights.cache_utils import cache_for
...
from insights.helpers.encrypted_fields import FERNET_TOKEN_PREFIX, EncryptedJSONField
from insights.models.github_integration_base import GitHubIntegrationBase, GitHubIntegrationError
from insights.models.instance_setting import get_instance_settings
...
from insights.scopes import get_oauth_scopes_supported
...
# Consecutive-failure backoff for the every-minute refresh sweep (insights/tasks/integrations.py).
...
class Integration(models.Model):
    ...
    def display_name(self) -> str:
        ...
        if self.kind in OauthIntegration.supported_kinds:
            region = self.config.get("region") if self.kind == "insights" else None
...
# StrEnum declared in insights/schema.py (generated from the SlackIntegrationScope enum in
# frontend/src/types.ts via `insightscli build:schema`), so widening it on either side stays in sync.
#
...
def _build_insights_slack_scope() -> str:
...
# Insights connect. Unlike every other OAuth kind — which points at a fixed third-party provider —
# the `insights` kind points at *another Insights project*, in a region chosen by the user at connect
...
def _insights_connect_target(region: str | None) -> tuple[str, str, str]:
    ...
    if normalized not in targets:
        raise NotImplementedError(f"Insights connect OAuth not supported for region {region!r}")
    base_url, client_id, client_secret = targets[normalized]
    if not base_url or not client_id or not client_secret:
        raise NotImplementedError(f"Insights connect app not configured for region {normalized}")
    return base_url.rstrip("/"), client_id, client_secret


def insights_connect_base_url(region: str | None) -> str:
    """Public base URL of a remote target cell (e.g. https://eu.hanzo.ai), for callers that
    need to reach its API with a `insights` integration token. Raises for unknown/unconfigured regions."""
    base_url, _, _ = _insights_connect_target(region)
    return base_url
...
class OauthIntegration:
    ...
    def oauth_config_for_kind(cls, kind: str, region: str | None = None) -> OauthConfig:
        # `region` only applies to the `insights` remote kind, whose endpoints depend on the
    ...
    def _build_oauth_config(cls, kind: str, region: str | None = None) -> OauthConfig:
        if kind == "insights":
            base_url, client_id, client_secret = _insights_connect_target(region)
            return OauthConfig(
    ...
    def integration_from_oauth_response(
        ...
        if kind == "insights":
            ...
            # that same cell. Missing/invalid region fails closed via _insights_connect_target.
        ...
        if oauth_config.pkce:
            ...
            if not code_verifier:
                ...
                # insights is a first-party flow we control both ends of — a missing verifier is never
        ...
        # Persist the target region and namespace the dedup key by it, so the same Insights account
        ...
        if kind == "insights" and integration_id:
    ...
    def revoke_token(self) -> None:
        ...
        Salesforce's revoke semantics), so the provider no longer considers Insights authorized
        after a disconnect. Callers treat this as best-effort — the local deletion proceeds
        regardless.
        """
        region = self.integration.config.get("region") if self.integration.kind == "insights" else None
        ...
        if self.integration.kind == "resend":
            # Resend registers Insights as a confidential client (token_endpoint_auth_method=
...
def sign_slack_request(body: bytes, signing_secret: str) -> SlackRequestSignature:
    ...
    Used by both prod (Insights→Insights cross-region calls that reuse the Slack signing scheme)
...
class LinearIntegration:
    ...
    def create_issue(self, attachment_url: str, config: dict[str, str]) -> dict[str, str]:
        ...
        self.query(
            link_attachment_query,
            variables={"issueId": linear_issue_id, "title": "Insights issue", "url": attachment_url},
        )
...
class StripeIntegration:
    ...
    def write_insights_secrets(self, team_id: int, created_by: "User") -> None:
        """Write Insights OAuth tokens to Stripe's Secret Store so the Stripe App can call Insights APIs."""

        oauth_app = self._get_insights_oauth_app()
        if not oauth_app:
            logger.warning("Insights OAuth app not found, cannot write secrets to Stripe")
            return
        ...
        secrets = {
            "insights_region": region.lower(),
            "insights_access_token": access_token_value,
            "insights_refresh_token": refresh_token_value,
            "insights_project_id": str(team_id),
            "insights_oauth_client_id": oauth_app.client_id,
        }
    ...
    def clear_insights_secrets(self) -> None:
        """Best-effort clear of Insights secrets from Stripe and revoke local OAuth tokens."""
        stripe_user_id = self.integration.integration_id
        if not stripe_user_id:
            raise ValueError("Missing stripe_user_id on integration")
        ...
        client = self._stripe_client()
        if client is None:
            self._destroy_insights_oauth_tokens()
            return
    ...
    def _destroy_insights_oauth_tokens(self) -> None:
...
MISSING_CERT_PATH = "/tmp/insights/batch-exports/MISSING.crt"
...
def cleanup_ses_identity_on_integration_delete(sender: Any, instance: Integration, **kwargs: Any) -> None:
    ...
    from insights.tasks.integrations import (
