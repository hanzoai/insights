...
import { customFunctionTemplateListLogic } from 'scenes/custom-functions/list/customFunctionTemplateListLogic'
...
import { CreateActionType, customFlowEditorLogic } from '../customFlowEditorLogic'
...
function CustomFlowEditorToolbarNode({
    action,
    ...
    const { setNodeToBeAdded } = useActions(customFlowEditorLogic)
    ...
    const step = useCustomFlowStep(action as CustomFlowAction)
...
const customFilterFunction = (template: CustomFunctionTemplateType): boolean => {
...
function CustomFunctionTemplatesChooser(): JSX.Element {
    const logic = customFunctionTemplateListLogic({
        type: 'destination',
        customFilterFunction,
    })

    const { loading, filteredTemplates, filters } = useValues(logic)
    const { loadCustomFunctionTemplates, setFilters } = useActions(logic)

    const [popoverOpen, setPopoverOpen] = useState(false)

    useEffect(() => {
        loadCustomFunctionTemplates()
    }, [loadCustomFunctionTemplates])
                    ...
                    <div className="flex flex-col w-100 h-120 flex-1 overflow-hidden gap-1">
                        ...
                        {loading ? (
                            ...
                            <ul className="overflow-y-auto flex-1">
                                {filteredTemplates.map((template: CustomFunctionTemplateType) => (
                                    <li key={template.type}>
                                        <CustomFlowEditorToolbarNode
                                            action={{
                                                type: 'function',
                                                name: template.name,
                                                description:
                                        ...
                                        >
                                            <div className="py-1 flex items-center gap-1 flex-1">
                                                ...
                                                {template.status && <CustomFunctionStatusTag status={template.status} />}
                                            </div>
                                        </CustomFlowEditorToolbarNode>
                                    </li>
                                ))}
...
export function CustomFlowEditorPanelBuild(): JSX.Element {
    ...
    return (
        <div className="flex overflow-y-auto flex-col gap-px p-2" data-attr="workflow-add-action">
            ...
            {ACTION_NODES_TO_SHOW.map((node, index) => (
                <CustomFlowEditorToolbarNode key={`${node.type}-${index}`} action={node} />
            ))}
            <CustomFunctionTemplatesChooser />

            <span className="flex gap-2 text-sm font-semibold mt-2 items-center">
                Delays <LemonDivider className="flex-1" />
            </span>
            {DELAY_NODES_TO_SHOW.map((action, index) => (
                <CustomFlowEditorToolbarNode key={`${action.type}-${index}`} action={action} />
            ))}
