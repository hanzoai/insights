...
const EventsTab = ({ client, projectId }: Props): JSX.Element => {
    ...
    if (!client || !projectId) {
        return (
            <Banner
                ...
                description="A Insights project_id isn't stored yet — reconnect to load events."
            />
        )
    }
    if (error) {
        ...
        return (
            <>
                ...
                <Banner
                    type="default"
                    title="No events yet"
                    description="Start sending events to Insights to see them here."
                />
            </>
        )
    }
    ...
    const items: DataTableItem[] = events.map((e) => {
        const url = `${insightsBase}/activity/explore-events?q=${buildEventsQuery(e.event, timeframe.value)}`
        linkMap[url] = e.event
        return {
            id: `event:${e.event}`,
            event: url,
        ...
        <>
            ...
            <Box css={{ width: 'fill', stack: 'y', rowGap: 'medium' }}>
                ...
                <Box css={{ paddingX: 'medium' }}>
                    <ExternalLink href={`${insightsBase}/activity`}>View in Insights</ExternalLink>
                </Box>
            </Box>
        </>
    )
