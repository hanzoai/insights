...
from llm_gateway.callbacks.insights import (
    _MAX_CAPTURE_SIZE,
    InsightsCallback,
    _normalize_trace_id,
...
class TestInsightsCallback:
    ...
    @pytest.fixture
    def mock_insights_client(self):
        mock_client = MagicMock()
        with patch("llm_gateway.callbacks.insights.Insights", return_value=mock_client) as mock_cls:
    ...
    def mock_event_loop(self):
        ...
        with patch("llm_gateway.callbacks.insights.asyncio") as mock_asyncio:
    ...
    async def test_on_success_captures_event(
        ...
        mock_insights_client: tuple,
    ) -> None:
        mock_cls, mock_client = mock_insights_client
        kwargs = {
        ...
        with (
            ...
            assert call_kwargs["groups"] == {"instance": "https://us.hanzo.ai", "project": 456}
    ...
    async def test_ai_lane_capture_off_builds_standard_lane_client(
        ...
        mock_cls, _ = mock_insights_client
        callback = InsightsCallback(api_key="test-key", host="https://test.hanzo.ai", ai_lane_capture=False)
    ...
    async def test_effort_is_gateway_owned(
        ...
        _, mock_client = mock_insights_client
        kwargs = {"standard_logging_object": standard_logging_object, "litellm_params": {}}
        caller_props = {"$ai_effort": caller_effort} if caller_effort is not None else {}

        with (
            patch("llm_gateway.callbacks.insights.get_auth_user", return_value=auth_user),
            patch("llm_gateway.callbacks.insights.get_product", return_value="insights_code"),
            patch("llm_gateway.callbacks.insights.get_effort", return_value=effort),
            patch("llm_gateway.callbacks.insights.get_insights_properties", return_value=caller_props),
        ):
    ...
    async def test_on_success_header_team_id_overrides_auth_user_team(
        ...
        """A caller-supplied x-insights-property-team_id wins over the key owner's team.
        ...
        with (
            patch("llm_gateway.callbacks.insights.get_auth_user", return_value=auth_user),
            patch("llm_gateway.callbacks.insights.get_product", return_value="signals"),
            # headers arrive as strings — this is the realistic x-insights-property-team_id path
            patch("llm_gateway.callbacks.insights.get_insights_properties", return_value={"team_id": "999"}),
        ):
            await callback._on_success(kwargs, None, 0.0, 1.0, end_user_id=None)
    ...
    async def test_on_success_invalid_header_team_id_falls_back_to_auth_team(
        ...
        with (
            patch("llm_gateway.callbacks.insights.get_auth_user", return_value=auth_user),
            patch("llm_gateway.callbacks.insights.get_product", return_value="signals"),
            patch("llm_gateway.callbacks.insights.get_insights_properties", return_value={"team_id": "not-a-number"}),
        ):
            await callback._on_success(kwargs, None, 0.0, 1.0, end_user_id=None)
    ...
    async def test_on_success_headers_cannot_override_ai_product_or_billable(
        ...
        with (
            ...
            patch("llm_gateway.callbacks.insights.get_product", return_value="insights_code"),
            patch(
                "llm_gateway.callbacks.insights.get_insights_properties",
            ...
            assert props["ai_product"] == "insights_code"
            # config-derived billable flag wins (insights_code is billable) — a caller
    ...
    async def test_on_success_uses_uuid_when_no_auth_user(
        ...
        with (
            ...
            assert call_kwargs["groups"] == {"instance": "https://us.hanzo.ai"}
    ...
    async def test_oauth_uses_auth_user_distinct_id_not_end_user_id(
        self, callback: InsightsCallback, method_name: str, mock_insights_client: tuple
    ) -> None:
        _, mock_client = mock_insights_client
    ...
    async def test_on_failure_captures_error_event(
        self, callback: InsightsCallback, auth_user: AuthenticatedUser, mock_insights_client: tuple
    ) -> None:
        _, mock_client = mock_insights_client
        kwargs = {
            "standard_logging_object": {
    ...
    async def test_on_success_emits_cache_tokens_when_present(
        ...
        with (
            patch("llm_gateway.callbacks.insights.get_auth_user", return_value=auth_user),
            patch("llm_gateway.callbacks.insights.get_product", return_value="slack_app"),
        ):
            await callback._on_success(kwargs, None, 0.0, 1.0, end_user_id=None)
    ...
    async def test_on_success_emits_cost_breakdown(
        ...
        props = mock_client.capture.call_args.kwargs["properties"]
        # Each LiteLLM cost_breakdown component maps 1:1 to its own Insights
    ...
    async def test_on_success_emits_reasoning_tokens_when_present(
        ...
        with (
            patch("llm_gateway.callbacks.insights.get_auth_user", return_value=auth_user),
            patch("llm_gateway.callbacks.insights.get_product", return_value="slack_app_routing"),
        ):
            await callback._on_success(kwargs, None, 0.0, 1.0, end_user_id=None)
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("product", ["wizard", "insights_code", "llm_gateway"])
    ...
    async def test_on_failure_includes_ai_product(
        self, callback: InsightsCallback, auth_user: AuthenticatedUser, product: str, mock_insights_client: tuple
    ) -> None:
        _, mock_client = mock_insights_client
        kwargs = {
            "standard_logging_object": {
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("product", ["slack_app", "slack_app_routing", "insights_code"])
    ...
    async def test_on_success_uses_configured_region_url_in_groups(
        ...
        callback = InsightsCallback(
            api_key="eu-key",
            host="https://eu.i.hanzo.ai",
            region_url="https://eu.hanzo.ai",
        )
        kwargs = {"standard_logging_object": standard_logging_object, "litellm_params": {}}
        ...
        call_kwargs = mock_client.capture.call_args.kwargs
        assert call_kwargs["groups"] == {"instance": "https://eu.hanzo.ai", "project": 456}
        ...
        assert call_kwargs["properties"]["$group_1"] == "https://eu.hanzo.ai"
    ...
    async def test_on_success_mirrors_to_secondary_destination_when_configured(
        ...
        with patch("llm_gateway.callbacks.insights.Insights", return_value=mock_client) as mock_cls:
            callback = InsightsCallback(
                ...
                secondary_host="https://us.i.hanzo.ai",
            )
            kwargs = {"standard_logging_object": standard_logging_object, "litellm_params": {}}
            ...
            # One Insights client constructed per destination, each captured once.
            primary_call, secondary_call = mock_cls.call_args_list
            assert primary_call.args == ("eu-key",)
            assert primary_call.kwargs["host"] == "https://eu.i.hanzo.ai"
            assert secondary_call.args == ("us-key",)
            assert secondary_call.kwargs["host"] == "https://us.i.hanzo.ai"
            ...
            # report filter ($group_1 = 'https://us.hanzo.ai') excludes the
            ...
            for capture_call in mock_client.capture.call_args_list:
                assert capture_call.kwargs["groups"] == {
                    "instance": "https://eu.hanzo.ai",
                    "project": 456,
                }
                assert capture_call.kwargs["properties"]["$group_1"] == "https://eu.hanzo.ai"
    ...
    async def test_on_success_prefers_traceparent_trace_id(
        self, callback, standard_logging_object, mock_insights_client
    ):
        kwargs = {
            "standard_logging_object": standard_logging_object,
        ...
        with (
            patch(
                "llm_gateway.callbacks.insights.get_traceparent_trace_id",
                return_value="0af76519-16cd-43dd-8448-eb211c80319c",
            ),
            patch("llm_gateway.callbacks.insights._capture_ai_event") as mock_capture,
        ):
            await callback._on_success(kwargs, None, 0.0, 1.0, end_user_id=None)
    ...
    @pytest.mark.asyncio
    async def test_on_failure_prefers_traceparent_trace_id(self, callback, mock_insights_client):
        kwargs = {
            "standard_logging_object": {
                "model": "claude-3-opus",
                "custom_llm_provider": "anthropic",
...
class TestTruncateForCapture:
    ...
    async def test_on_success_truncates_oversized_content(self) -> None:
        ...
        with (
            patch("llm_gateway.callbacks.insights.get_auth_user", return_value=auth_user),
            patch("llm_gateway.callbacks.insights.get_product", return_value="wizard"),
            patch("llm_gateway.callbacks.insights.Insights", return_value=mock_client),
            patch("llm_gateway.callbacks.insights.asyncio") as mock_asyncio,
        ):
            mock_asyncio.get_running_loop.return_value = mock_loop
            await callback._on_success(kwargs, None, 0.0, 1.0, end_user_id=None)
