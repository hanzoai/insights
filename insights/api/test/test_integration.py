...
from insights.api.github_callback.state import store_unified_authorize_state
from insights.api.github_callback.team_services import (
...
from insights.rate_limit import GitHubRepositoryRefreshThrottle
...
class TestSlackIntegration:
    ...
    @patch("insights.models.integration.WebClient")
...
class TestEmailIntegration:
    ...
    @patch("insights.models.integration.SESProvider")
    ...
    def test_email_verify_returns_ses_result(self, mock_ses_provider_class):
        ...
        mock_client.verify_email_domain.assert_called_once_with(
            "hanzo.ai", mail_from_subdomain="feedback", team_id=self.team.id
        )
...
class TestDatabricksIntegration:
    ...
    def setup_integration(self, db):
        ...
        self.user = User.objects.create_and_join(
            self.organization, "test@hanzo.ai", "test", level=OrganizationMembership.Level.ADMIN
        )
...
class TestAwsS3Integration:
    ...
    @patch("insights.models.integration.AwsS3Integration.validate_credentials")
    def test_create_rejects_invalid_credentials(self, mock_validate, client: HttpClient):
        from insights.models.integration import S3CredentialIntegrationError
...
class TestAwsS3RoleBasedIntegration:
    ...
    def test_create_rejects_duplicate_role_in_different_org(self, client: HttpClient):
        ...
        another_user = User.objects.create_and_join(
            another_org, "test2@hanzo.ai", "test", level=OrganizationMembership.Level.ADMIN
        )
...
class TestIntegrationAPIKeyAccess:
    ...
    @patch(
        "insights.models.integration.get_instance_settings",
        return_value={
    ...
    @patch("insights.models.integration.GitHubIntegration.list_cached_repositories")
    ...
    def test_github_repos_passes_search_before_pagination(self, mock_list_repos, client: HttpClient):
        repos = [{"id": 2, "name": "insights-js", "full_name": "org/insights-js"}]
        mock_list_repos.return_value = (repos, False)
        ...
        assert data["has_more"] is False
        mock_list_repos.assert_called_once_with(search="insights", limit=1, offset=1)
    ...
    @patch("insights.models.integration.GitHubIntegration.list_teams")
    ...
    @patch("insights.api.integration._ensure_oauth_token_valid")
    @patch("insights.api.integration.JiraIntegration.list_projects")
    ...
    @patch("insights.api.integration._ensure_oauth_token_valid")
    @patch("insights.api.integration.LinearIntegration.list_teams")
    ...
    @patch("insights.models.integration.GitHubIntegration.sync_repository_cache")
    ...
    @patch("insights.api.integration.SlackIntegration")
...
class TestGithubAccountTypeHelper:
    ...
    def test_github_account_type(self, _name, owner_type, expected):
        from insights.api.integration import _github_account_type
...
class TestGitHubIntegrationStateValidation:
    ...
    @patch("insights.models.integration.GitHubIntegration.integration_from_installation_id")
    ...
    @patch("insights.models.github_integration_base.GitHubIntegrationBase.verify_user_installation_access")
    @patch("insights.models.integration.GitHubIntegration.github_user_from_code")
    @patch("insights.models.integration.GitHubIntegration.integration_from_installation_id")
    @patch("insights.models.user_integration.user_github_integration_from_installation")
    def test_create_github_integration_with_valid_state_succeeds(
        ...
        from insights.models.integration import GitHubUserAuthorization
    ...
    def test_create_github_integration_cross_user_state_rejected(self, mock_from_install, client: HttpClient):
        other_user = User.objects.create_and_join(
            self.organization, "attacker@hanzo.ai", "test", level=OrganizationMembership.Level.ADMIN
        )
...
class TestGitHubTeamIntegrationComplete:
    ...
    @override_settings(GITHUB_APP_CLIENT_ID="client_id", SITE_URL="https://us.hanzo.ai")
    ...
    def test_forged_state_cannot_complete_team_install(self, client: HttpClient):
        member = User.objects.create_and_join(
            self.organization, "member@hanzo.ai", "test", level=OrganizationMembership.Level.MEMBER
        )
    ...
    @patch("insights.models.integration.GitHubIntegration.verify_user_installation_access", return_value=True)
    @patch("insights.models.integration.GitHubIntegration.client_request")
    @patch("insights.models.integration.GitHubIntegration.github_user_from_code")
    ...
    @patch("insights.api.github_callback.team_services.build_team_github_oauth_authorize_url")
    ...
    def test_authorize_link_existing_requires_personal_github_for_non_admin(self):
        ...
        member = User.objects.create_and_join(
            self.organization, "member-linker@hanzo.ai", "test", level=OrganizationMembership.Level.MEMBER
        )
    ...
    def _org_member_with_access_control(self) -> User:
        ...
        return User.objects.create_and_join(
            self.organization, "outsider@hanzo.ai", "test", level=OrganizationMembership.Level.MEMBER
        )
...
class TestStripeIntegration:
    ...
    @patch("insights.api.integration.StripeIntegration")
    def test_destroy_calls_clear_insights_secrets(self, MockStripeIntegration, client: HttpClient):
        ...
        mock_instance.clear_insights_secrets.assert_called_once()
    ...
    def test_destroy_still_deletes_when_clear_secrets_fails(self, MockStripeIntegration, client: HttpClient):
        integration = self._create_stripe_integration()
        mock_instance = MagicMock()
        mock_instance.clear_insights_secrets.side_effect = Exception("Stripe API error")
        MockStripeIntegration.return_value = mock_instance
    ...
    @patch("insights.api.integration.StripeIntegration")
    @patch("insights.api.integration.OauthIntegration.integration_from_oauth_response")
    def test_create_calls_write_insights_secrets(
        ...
        mock_instance.write_insights_secrets.assert_called_once_with(self.team.pk, self.user)
    ...
    def test_create_succeeds_when_write_secrets_fails(
        ...
        mock_oauth_response.return_value = created_integration
        mock_instance = MagicMock()
        mock_instance.write_insights_secrets.side_effect = Exception("Stripe API error")
        MockStripeIntegration.return_value = mock_instance
    ...
    @patch("insights.api.integration.StripeIntegration")
    @patch("insights.api.integration.OauthIntegration.integration_from_oauth_response")
    def test_insights_initiated_oauth_with_state_still_works(
        self, mock_oauth_response, MockStripeIntegration, stripe_settings, client: HttpClient
    ):
        created_integration = self._create_stripe_integration()
        mock_oauth_response.return_value = created_integration
    ...
    @patch("insights.api.integration.StripeIntegration")
    @patch("insights.api.integration.OauthIntegration.integration_from_oauth_response")
    def test_insights_initiated_oauth_ignores_marketplace_conflict_guard(
        self, mock_oauth_response, MockStripeIntegration, stripe_settings, client: HttpClient
    ):
...
class TestStripeIntegrationOAuthTokens:
    ...
    def test_destroy_oauth_tokens_deletes_tokens(self, mock_settings):
        ...
        stripe_int._destroy_insights_oauth_tokens()
    ...
    def test_write_insights_secrets_uses_account_scope(self, mock_settings, MockStripeClient):
        ...
        stripe_int = StripeIntegration(integration)
        stripe_int.write_insights_secrets(self.team.pk, self.user)
        ...
        assert secret_payloads["insights_project_id"] == str(self.team.pk)
        assert secret_payloads["insights_oauth_client_id"] == self.oauth_app.client_id
    ...
    def test_clear_insights_secrets_uses_account_scope(self, mock_settings, MockStripeClient):
        ...
        stripe_int = StripeIntegration(integration)
        stripe_int.clear_insights_secrets()
    ...
    @parameterized.expand(
        [
            ("write", "write_insights_secrets"),
            ("clear", "clear_insights_secrets"),
        ]
    )
    @patch("stripe.StripeClient")
    @patch("insights.models.integration.settings")
    def test_stripe_client_uses_live_secret(self, _name, method_name, mock_settings, MockStripeClient):
        ...
        stripe_int = StripeIntegration(integration)
        if method_name == "write_insights_secrets":
            stripe_int.write_insights_secrets(self.team.pk, self.user)
        else:
            stripe_int.clear_insights_secrets()
    ...
    @patch("insights.models.integration.capture_exception")
    @patch("stripe.StripeClient")
    @patch("insights.models.integration.settings")
...
class TestGitHubBranches:
    ...
    @patch("insights.models.integration.GitHubIntegration.list_cached_branches")
    ...
    @patch("insights.models.integration.GitHubIntegration.get_default_branch", return_value="main")
...
class TestSlackInsightsCodeKindDeprecated:
    @pytest.fixture(autouse=True)
    def setup_environment(self, db):
        self.organization = Organization.objects.create(name="Test Org")
        self.team = Team.objects.create(organization=self.organization, name="Test Team")
    ...
    def test_create_slack_insights_code_integration_rejected(self, client: HttpClient):
        client.force_login(self.user)
        ...
        assert not Integration.objects.filter(team=self.team, kind="slack-insights-code").exists()

    def test_validate_kind_rejects_slack_insights_code(self):
        serializer = IntegrationSerializer(data={"kind": "slack-insights-code", "config": {}})
...
class TestGitHubIntegrationUninstall:
    ...
    @patch("insights.api.integration.GitHubIntegration.uninstall_app_installation")
    ...
    @patch(
        "insights.api.integration.GitHubIntegration.uninstall_app_installation",
    ...
    @patch("insights.api.integration.GitHubIntegration.uninstall_app_installation")
    @patch("insights.api.integration.count_in_progress_runs_for_github_integration")
...
class TestIntegrationDeletionWorkflowGuard:
    ...
    def setup_integration(self, db):
        ...
        self.integration = Integration.objects.create(
            team=self.team,
            kind="email",
            config={"email": "noreply@hanzo.ai", "name": "Test", "domain": "hanzo.ai", "verified": True},
            created_by=self.user,
        )
    ...
    def _create_flow(self, status: str = "active", actions: list | None = None) -> InsightsFlow:
    ...
    def test_destroy_allowed_when_workflow_not_active(self, flow_status: str, client: HttpClient):
        ...
        with patch("insights.api.integration.EmailIntegration"):
            response = self._delete(client)
    ...
    def test_destroy_blocked_when_function_action_references_integration(
        ...
        if reference_kind == "bare_id_via_schema":
            InsightsFunctionTemplate.objects.create(
...
class TestIntegrationDeletionInsightsFunctionGuard:
    @pytest.fixture(autouse=True)
    def setup_integration(self, db):
        self.organization = Organization.objects.create(name="Test Org")
        self.team = Team.objects.create(organization=self.organization, name="Test Team")
...
class TestPushIdentityVerificationAPI(APIBaseTest):
    ...
    @patch("insights.models.integration.GoogleRequest")
    @patch("insights.models.integration.service_account.Credentials.from_service_account_info")
...
class TestInsightsConnectAuthorize:
    @pytest.fixture(autouse=True)
    def setup_environment(self, db):
        self.organization = Organization.objects.create(name="Test Org")
        self.team = Team.objects.create(organization=self.organization, name="Test Team")
    ...
    def test_valid_request_redirects_to_target_region(self, client: HttpClient):
        ...
        assert response.status_code == status.HTTP_302_FOUND
        assert response.headers["Location"].startswith("https://eu.hanzo.ai/oauth/authorize?")
...
class TestInsightsConnectionListScoping:
    @pytest.fixture(autouse=True)
    def setup_environment(self, db, settings):
        ...
        self.owner = User.objects.create_and_join(
            self.organization, "owner@hanzo.ai", "test", level=OrganizationMembership.Level.ADMIN
        )
        self.other = User.objects.create_and_join(
            self.organization, "other@hanzo.ai", "test", level=OrganizationMembership.Level.ADMIN
        )
        self.connection = Integration.objects.create(
            team=self.team, kind="insights", integration_id="EU:owner", created_by=self.owner, config={"region": "EU"}
        )
    ...
    def test_other_member_cannot_see_another_users_connection(self, client: HttpClient):
        # A `insights` connection is only usable by its creator and carries their personal target
