...
from insights.api.oauth.mcp_resource_scopes import (
    _tool_required_scopes,
    build_oauth_mcp_consent_context,
    is_trusted_insights_mcp_resource,
    mcp_advertised_scopes,
)


class TestMcpResourceScopes(SimpleTestCase):
    def test_is_trusted_insights_mcp_resource_production(self):
        for host in (
            "mcp.hanzo.ai",
            "mcp.us.hanzo.ai",
            "mcp.eu.hanzo.ai",
            ...
            self.assertTrue(is_trusted_insights_mcp_resource(f"https://{host}/mcp"))
            self.assertFalse(is_trusted_insights_mcp_resource(f"http://{host}/mcp"))
        self.assertFalse(is_trusted_insights_mcp_resource("https://evil.hanzo.ai/mcp"))
        # A userinfo-smuggled host must resolve to the real hostname, not the allowlisted prefix.
        self.assertFalse(is_trusted_insights_mcp_resource("https://mcp.hanzo.ai@evil.com/mcp"))

    @override_settings(DEBUG=True)
    def test_is_trusted_insights_mcp_resource_local_dev(self):
        self.assertTrue(is_trusted_insights_mcp_resource("http://localhost:8787/mcp"))
        self.assertTrue(is_trusted_insights_mcp_resource("http://127.0.0.1:8787/mcp"))

    @override_settings(DEBUG=False)
    def test_local_mcp_host_blocked_outside_dev(self):
        self.assertFalse(is_trusted_insights_mcp_resource("http://localhost:8787/mcp"))

    @patch(
        "insights.api.oauth.mcp_resource_scopes.get_oauth_scopes_supported",
        return_value=["openid", "profile", "notebook:read", "notebook:write", "webhook:write"],
    )
    @patch(
        "insights.api.oauth.mcp_resource_scopes._tool_required_scopes",
    ...
    def test_tool_required_scopes_merges_by_tool_name_with_generated_winning(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch(
                "insights.api.oauth.mcp_resource_scopes._TOOL_DEFINITIONS_PATHS",
    ...
    def test_build_oauth_mcp_consent_context_success(self):
        context = build_oauth_mcp_consent_context("https://mcp.hanzo.ai/mcp")
