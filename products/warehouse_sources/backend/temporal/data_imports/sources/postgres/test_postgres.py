...
class TestPostgresSourceMetadataConnectionErrors:
    def test_insights_database_connection_failure_stays_retryable(self):
        # `source_for_pipeline` first reads sync metadata from Insights's own database. A transient
        ...
        # re-raised as InsightsDatabaseConnectionError to avoid being misclassified as non-retryable.
        source = PostgresSource()
        config = MagicMock()
        inputs = MagicMock()
        ...
        with (
            ...
            with pytest.raises(InsightsDatabaseConnectionError) as exc_info:
                source.source_for_pipeline(config, inputs)

        non_retryable = source.get_non_retryable_errors()
        error_msg = str(exc_info.value)
        is_non_retryable = any(pattern in error_msg for pattern in non_retryable.keys())
        assert not is_non_retryable, f"A Insights-side DB connection failure must stay retryable: {error_msg}"
...
class TestPostgresSourceNonRetryableErrors:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            'connection failed: connection to server at "10.0.0.1", port 5432 failed: FATAL:  The password that was provided for the role insights_readonly is wrong.',
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "permission denied for view insights_areas",
            "permission denied for materialized view insights_notifications",
            # Temporal-wrapped message (what the workflow-level check sees) — carries the class name.
...
class TestPostgresSourceForPipelineSchemaResolution:
    ...
    def test_s3_folder_name_drives_response_name_so_delta_writes_to_legacy_path(self, source):
        ...
        # points at `.../example_table/` and InsightsQL reads from an empty location.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                'connection failed: connection to server at "66.33.22.254", port 41667 failed: '
                "received invalid response to SSL negotiation: I",
                "Insights reached the host and port you configured, but the server didn't respond like a "
        ...
        [
            ...
            (
                ...
                psycopg.errors.SqlclientUnableToEstablishSqlconnection(
                    'could not connect to server "insights_fdw_payment"\n'
                    'DETAIL:  connection to server at "10.0.0.1", port 5432 failed: '
                    'FATAL:  too many connections for role "insights_fdw_reader"'
                ),
                psycopg.OperationalError,
            ),
