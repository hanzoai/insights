...
Use this skill when the user wants to connect an external data source to Insights's data warehouse for the first time.
...
- The user isn't sure what source types Insights supports
...
- **The host must be reachable from Insights's network.** `localhost`, `127.0.0.1`, and private/RFC-1918 hosts
  (`10.x`, `192.168.x`, `172.16–31.x`) are rejected — Insights runs the connection from its own infrastructure, not the
  user's machine. Serverless/managed Postgres (Neon, Supabase, RDS behind strict rules) often also needs Insights's
...
- **Never pass an unresolved secret reference.** If a credential field is still a `{"secretRef": ...}` object, Insights
...
need the user to authorize in the Insights UI rather than pasting credentials — explain this and direct them to the
...
The source's `prefix` is prepended to table names in InsightsQL. Tables end up as `{prefix}_{table_name}`.
...
- Use lowercase, underscore-separated. The prefix becomes part of every InsightsQL query the user writes.
...
events, and Insights needs to know how to verify them. This is a second call after source creation, not part of the
...
1. Call `external-data-sources-create-webhook-create({id})`. Insights:
   - creates the InsightsFunction that will receive webhook POSTs,
   - builds a schema_mapping from external event types to Insights schema ids,
...
2. If `success: false` with a permissions error like "API key doesn't have permission to create webhooks":
   - The InsightsFunction is still created, just disabled.
   ...
   - Call `external-data-sources-update-webhook-inputs-create({id}, {inputs: {signing_secret: "whsec_..."}})` to
     store it. The InsightsFunction picks it up and verifies incoming payloads.
...
- Explain: every enabled schema enters `Running`, then moves to `Completed` when the first sync finishes. First
  ...
  slow.
- Tell them how to query: `SELECT * FROM {prefix}_{table_name} LIMIT 10` in InsightsQL.
...
- **Prefix is load-bearing.** It's part of every InsightsQL query the user will ever write against these tables. Pick
