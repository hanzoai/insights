...
from insights.datastore.kafka_engine import CONSUMER_GROUP_EVENTS_JSON_NATIVE_JSON, KAFKA_COLUMNS_WITH_PARTITION
...
from insights.models.flag_evaluations.sql import FLAG_EVALUATIONS_KAFKA_COLUMNS, FLAG_EVALUATIONS_MV_SQL


@pytest.mark.parametrize("query", CREATE_TABLE_QUERIES, ids=get_table_name)
def test_create_table_query(query, snapshot, settings):
    settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA = False
...
def test_create_table_query_replicated_and_storage(query, snapshot, settings):
    settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA = False
    settings.DATASTORE_ENABLE_STORAGE_POLICY = True
...
def test_create_kafka_table_with_different_kafka_host(query, snapshot):
    # Historical name; the override that used to drive `KAFKA_HOSTS_FOR_DATASTORE`
...
def test_events_json_table_uses_dedicated_kafka_consumer_group(settings):
    ...
    assert f"FROM {settings.DATASTORE_DATABASE}.{KAFKA_EVENTS_NATIVE_JSON_TABLE}" in mv_query
