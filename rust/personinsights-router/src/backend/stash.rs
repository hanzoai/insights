...
pub struct StashedRequest {
    ...
    /// contract in `personinsights-leader`'s README — and is counted so
...
impl StashTable {
    ...
    pub async fn enqueue_or_forward(
        ...
        if queue.messages >= stash.max_messages {
            metrics::counter!(
                "personinsights_router_stash_rejected_total",
        ...
        metrics::counter!("personinsights_router_stash_enqueued_total").increment(1);
        // Occupancy gauges count a parked entry until its outcome
        // resolves, so in-attempt entries still occupy capacity.
        metrics::gauge!("personinsights_router_stash_queued_messages").increment(1.0);
        metrics::gauge!("personinsights_router_stash_queued_bytes").increment(request_size as f64);
...
impl DrainSession {
    ...
    pub async fn complete(&self, key: StashKey, size: usize) {
        ...
        metrics::gauge!("personinsights_router_stash_queued_messages").decrement(1.0);
        metrics::gauge!("personinsights_router_stash_queued_bytes").decrement(size as f64);
    }
...
mod tests {
    ...
    /// `personinsights_router_stash_dropped_total` counter.
    #[tokio::test]
