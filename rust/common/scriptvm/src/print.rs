//! Canonical Script value formatting, matching the reference VMs' `printHogValue` /
//! `printHogStringOutput` (see `common/scriptvm/typescript/src/stl/print.ts` and
//! `common/scriptvm/python/stl/print.py`). This is what `print(...)` emits and is the
...
/// datastore-style control characters plus the quote and backslash.
...
fn format_literal(
    ...
    match lit {
        ...
        HogLiteral::Object(obj) => {
            // Script temporals are duck-typed objects; the reference prints them as DateTime(...)/Date(...).
            if let Some(temporal) = format_temporal(heap, obj)? {
                return Ok(temporal);
            }
            // Script errors are duck-typed too; the reference prints them as `Type('message'[, payload])`.
            if let Some(err) = format_error(heap, obj, marked, depth)? {
                return Ok(err);
            }
            // InsightsQL AST nodes (`{__hx_ast: …}`) render back to SQL, wrapped as `sql(…)`.
            if obj.contains_key("__hx_ast") {
                return Ok(format!("sql({})", InsightsQLPrinter::new(heap).print_node(obj)?));
            }
...
// Render the Script temporal duck-types: `{__hogDateTime__: true, dt, zone}` -> `DateTime(dt, 'zone')`
...
// Render the Script error duck-type `{__hogError__: true, type, message, payload?}` as
...
// ── InsightsQL AST → SQL printer ──────────────────────────────────────────────────────────────────
// Port of the reference `InsightsQLPrinter` (common/scriptvm/typescript/src/stl/print.ts). InsightsQL queries
...
impl<'a> InsightsQLPrinter<'a> {
    ...
    fn dispatch(&mut self, obj: &IndexMap<String, HogValue>) -> Result<String, crate::VmError> {
        let node_type = self.str_field(obj, "__hx_ast")?.ok_or_else(|| {
            crate::VmError::NativeCallFailed("InsightsQL node missing __hx_ast".to_string())
        })?;
        match node_type.as_str() {
            ...
            other => Err(crate::VmError::NativeCallFailed(format!(
                "Unknown InsightsQL AST node type: {other}"
            ))),
        }
    }
