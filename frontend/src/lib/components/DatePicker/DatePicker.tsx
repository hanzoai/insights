...
import { IconCalendar, IconX } from '@hanzo/icons'
 ...
 * the internal rendering swaps from UI to Quill in one place without touching callers.
 *
 * The swap is gated by the `QUILL_DATE_PICKER` feature flag so UI and Quill run side
 * by side and roll back without a revert. The Quill panel does not yet cover the whole
 * feature surface, so `quillCanRender` falls back to UI whenever a request needs a
 ...
 * maps them onto whichever button backs the current renderer (Button today, Quill
...
export interface DatePickerProps {
    ...
    months?: number
    /** Latest selectable date. Bounds the Quill panel; when omitted Quill caps at today, so future-date callers must pass this. The UI fallback has no equivalent and stays unbounded above. */
    maxDate?: dayjs.Dayjs
...
function DatePickerLemon({
    ...
    const buttonProps: NonNullable<CalendarSelectInputProps['buttonProps']> = {
        fullWidth,
        size,
        className,
        disabledReason,
    ...
    // Only forward `type` when the caller set one — CalendarSelectInput defaults the trigger to
