...
export interface SessionFilterConfig {
     ...
     * `@hanzo/replay/session-blocked`). A secondary pipeline (e.g. the ML mirror) must pass its own
...
export class SessionFilter {
    ...
    constructor(config: SessionFilterConfig) {
        this.keyPrefix = config.keyNamespace
            ? `@hanzo/replay/${config.keyNamespace}/session-blocked`
            : '@hanzo/replay/session-blocked'
