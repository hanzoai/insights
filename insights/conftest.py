...
except ImportError:  # fail-open: runs without tools/insightscli-commands on pythonpath
...
def create_datastore_tables():
    ...
    if mergetree_queries:
        run_datastore_statement_in_parallel(mergetree_queries)

    distributed_queries = list(map(build_query, missing(CREATE_DISTRIBUTED_TABLE_QUERIES)))
    if distributed_queries:
        run_datastore_statement_in_parallel(distributed_queries)
    ...
    # TRUNCATE-based resets go through reset_datastore_tables, which reloads unconditionally.
    for table_name, query_fn in SEED_DATA_TABLES:
        count = sync_execute(f"SELECT count() FROM {table_name}")[0][0]
        if not count:
            run_datastore_statement_in_parallel([build_query(query_fn)])
...
def reset_datastore_tables():
    ...
    from insights.models.channel_type.sql import TRUNCATE_CHANNEL_DEFINITION_TABLE_SQL
    ...
    from insights.models.exchange_rate.sql import TRUNCATE_EXCHANGE_RATE_TABLE_SQL
    from insights.models.group.sql import TRUNCATE_GROUPS_TABLE_SQL
    from insights.models.performance.sql import TRUNCATE_PERFORMANCE_EVENTS_TABLE_SQL
    ...
    if settings.IN_EVAL_TESTING:
        ...
        # Using `ON CLUSTER` takes x20 more time to drop the tables: https://github.com/Datastore/Datastore/issues/15473.
        TABLES_TO_CREATE_DROP += [f"DROP TABLE {table[0]}" for table in kafka_tables]

    # Skip truncating tables Datastore reports as empty: each truncate costs a keeper
    ...
    empty_table_truncates = {
        ...
        for form in (
            ...
            f"TRUNCATE TABLE IF EXISTS `{settings.DATASTORE_DATABASE}`.`{name}`",
        )
    }
...
def _django_db_setup(django_db_keepdb, django_db_blocker):
    ...
    # Point the off-ORM persons_db util (insights/persons_db.py) at the test persons DB. It reads
    ...
    if django_db_keepdb:
        ...
        skip_ch_reset = os.environ.get("SKIP_DATASTORE_RESET", "0").lower() in {"1", "true", "yes"}
...
class _JUnitTimingsPlugin:
    ...
    _PROPERTY_SETUP = "insights.setup_seconds"
    _PROPERTY_COLLECTION = "insights.collection_seconds"
    _PROPERTY_RERUNS = "insights.reruns"
...
def pytest_configure(config):
    ...
    if not config.pluginmanager.hasplugin("insights-junit-timings"):
        config.pluginmanager.register(_JUnitTimingsPlugin(), "insights-junit-timings")
