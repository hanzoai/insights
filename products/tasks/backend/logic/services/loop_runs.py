...
# added on top of whatever insights_mcp_scopes the loop already carries rather than escalating
...
def _augment_scopes_for_context(scopes: InsightsMcpScopes, *, outputs: dict) -> InsightsMcpScopes:
    """Widen a run's Insights MCP scopes by exactly what its context maintenance needs.
...
def _create_loop_task_and_run(loop: Loop, trigger: LoopTrigger | None, trigger_context: str) -> tuple[Task, TaskRun]:
    ...
    insights_mcp_scopes = _augment_scopes_for_context(_resolve_insights_mcp_scopes(loop.connectors), outputs=outputs)
