...
from insights.temporal.mcp_analytics.intent_clustering.constants import COORDINATOR_SCHEDULE_ID
from insights.temporal.mcp_analytics.intent_clustering.schedule import (
    FEATURE_FLAG_KEY,
...
def schedule_helpers():
    ...
    with (
        patch(
            "insights.temporal.mcp_analytics.intent_clustering.schedule.a_create_schedule", new_callable=AsyncMock
        ) as create,
        patch(
            "insights.temporal.mcp_analytics.intent_clustering.schedule.a_update_schedule", new_callable=AsyncMock
        ) as update,
        patch(
            "insights.temporal.mcp_analytics.intent_clustering.schedule.a_delete_schedule", new_callable=AsyncMock
        ) as delete,
        patch(
            "insights.temporal.mcp_analytics.intent_clustering.schedule.a_schedule_exists", new_callable=AsyncMock
        ) as exists,
    ):
...
class TestCreateIntentClusteringCoordinatorSchedule:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # A transient Insights API outage must not silently disable a
    ...
    async def test_create_intent_clustering_coordinator_schedule(
        ...
        if flag_enabled is None:
            feature_enabled_patch = patch(
                "insights.temporal.mcp_analytics.intent_clustering.schedule.hanzo_insights.feature_enabled",
                side_effect=RuntimeError("Insights FF API down"),
            )
        else:
            feature_enabled_patch = patch(
                "insights.temporal.mcp_analytics.intent_clustering.schedule.hanzo_insights.feature_enabled",
...
class TestFeatureFlagKeyIsStable:
    def test_flag_key_matches_documented_value(self) -> None:
        # The feature flag must be created in Insights with this exact key.
