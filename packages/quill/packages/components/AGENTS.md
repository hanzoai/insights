...
Quick-reference for AI agents using `@hanzo/quill-components` — composed components built on quill primitives. For primitive-level guidance (choosing a component, spacing, the `render` prop), read `../primitives/AGENTS.md` first.
...
- `Metric` — composable stat tile (`Card` + `Badge` pill + `Sparkline`); marries primitives with `@hanzo/quill-charts`. Import from the `@hanzo/quill-components/metric` subpath (not the main barrel)

## DataTable

```tsx
import { type ColumnDef } from '@tanstack/react-table'
import { DataTable } from '@hanzo/quill-components'
...
```tsx
import { CUSTOM_RANGE, DateTimePicker, quickRanges } from '@hanzo/quill-components'
;<DateTimePicker
...
```tsx
import { DatePicker } from '@hanzo/quill-components'
;<DatePicker
  value={date}
...
- `value`/`onApply` are a single `Date`, not `{ start, end, range }`. Use this for the single-date Insights callers (currently `CalendarSelect`); reach for `DateTimePicker` only when you need a start→end range.
...
A composable stat tile: a headline number, a `Badge` change pill, and an optional `Sparkline`. `Metric` is **content, not a surface** — wrap it in `<Card flush>` for the border. It's the one component here that depends on `@hanzo/quill-charts` (for `Sparkline` + the headless metric math), which pulls d3 — so it lives behind its own `@hanzo/quill-components/metric` entry point, **not** the main barrel (and not the `@hanzo/quill` umbrella). That keeps charts/d3 out of the always-eager app-shell graph: only code that imports the metric subpath pays for it. The `MetricCard` in `@hanzo/quill-charts` is the older, self-contained (prop-driven, primitives-free) tile; use `Metric` when you want to compose the layout or lean on quill's `Card`/`Badge`.
...
import {
  ...
  MetricSubtitle,
  MetricSparkline,
} from '@hanzo/quill-components/metric'
import { Card } from '@hanzo/quill-primitives'
import { useChartTheme } from '@hanzo/quill-charts'
