...
# import re-opened the path — defer it (function-local / TYPE_CHECKING / lazy facade) instead
...
FORBIDDEN_AT_SETUP = [
    ...
    "insights.session_recordings.session_recording_api",  # pulls the session_summary workflow
    ...
    "user_agents",  # UA parser — only the request-time UA-summary path needs it (deferred in insights.utils)
    ...
    "aiohttp",  # async HTTP — pulled by slack async client / temporal datastore, both deferred
    "s3fs",  # S3 filesystem — only warehouse delete/import paths need it
    "pandas",  # only query/export paths need it — reached via datastore_connect's import-time probe
    ...
    "anthropic",  # Anthropic SDK — same import path as openai (insights.llm.gateway_client)
    ...
    "insights.api.services.query",  # API query service — viewset-request-time only
]
...
import importlib
importlib.import_module("insights.urls")
after = labels()
...
# model_activity_signal is Insights's custom audit-log signal, not a django.db.models one. Its
...
def _capture_setup_import_costs() -> dict[str, int]:
    ...
    for line in result.stderr.splitlines():
        ...
        if root in _FIRST_PARTY_ROOTS:
            # judged per module: aggregating insights.* would be one permanent offender
...
import os
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "insights.settings")
import insights.wsgi  # noqa: F401 — the real web entry; builds the app and resolves the URLconf
import sys
assert "insights.api.rest_router" in sys.modules, "wsgi import did not build the API router"
...
"""
    ...
    assert result.returncode == 0, f"insights.wsgi import failed:\n{result.stderr[-2000:]}"
