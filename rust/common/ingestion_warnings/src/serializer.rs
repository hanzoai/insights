...
//! | Timestamp | RFC 3339 envelope fields | Datastore-format `timestamp` field |
//!
...
impl Warning {
    ...
    /// `timestamp` uses the Datastore format the v2 table materializes
    /// columns from.
...
mod tests {
    ...
    fn envelope_properties_carry_the_structured_type_source_and_details() {
        ...
        assert_eq!(details["lib"], "insights-js");
    ...
    const TEST_ROW_SOURCE: WarningSource = WarningSource {
        service: "personinsights-test",
        path: "test_path",
        pipeline_step: "personinsights_test_step",
    };
    ...
    fn rows_carry_registry_types_and_classifications() {
        for warning in WarningType::ALL {
            ...
            assert_eq!(payload["source"], "personinsights-test");
            let details: Value =
            ...
            assert_eq!(details["pipelineStep"], "personinsights_test_step");
