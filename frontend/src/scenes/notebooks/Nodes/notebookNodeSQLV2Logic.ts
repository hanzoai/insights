...
export type SqlV2RunRef = {
    node_id: string
    // 'insightsql' is a SQL node's query definition; 'local' is a dataframe a Python cell bound in
    // the kernel namespace. The backend routes a SQL run to Datastore or the sandbox's DuckDB
    // based on which kinds the query actually references.
    kind: 'insightsql' | 'local'
}
...
// The backend resolves each referenced insightsql node to its last-run query — the frontend only
// supplies the wiring.
export function collectSqlV2Refs(doc: JSONContent | null | undefined, selfNodeId: string): Record<string, SqlV2RunRef> {
    ...
    for (const node of collectSqlV2Nodes(doc)) {
        ...
        if (node.nodeId && node.nodeId !== selfNodeId && node.returnVariable) {
            refs[node.returnVariable] = { node_id: node.nodeId, kind: 'insightsql' }
        }
    }
...
// The full capped row set a direct (Datastore, no-sandbox) run returns with its result,
...
export interface RunQueryOptions {
    ...
    outputName?: string
    // SQL cells only: the data source to run against instead of Insights, and whether to send the
    // code to it verbatim. Absent means Insights's own Datastore.
...
export const notebookNodeSQLV2Logic = kea<notebookNodeSQLV2LogicType>([
    ...
    actions({
        ...
        // inlines referenced insightsql refs as CTEs (Journey 3) — or runs locally in DuckDB when
        // it references a local frame (Journey 5); a python node materializes the insightsql refs
    ...
    listeners(({ props, actions, cache, values }) => {
        ...
        const loadCurrentPage = async (): Promise<void> => {
            ...
            if (values.isBusy && values.activeOperation?.id !== pageOperation.id) {
                toast.info('Another operation is running in this notebook — wait for it to finish.')
                actions.resetPaging()
                return
            }
            ...
            try {
                ...
                if (error?.status === 409) {
                    toast.info('This result was replaced by a newer run — showing the latest first page.')
                } else {
                    toast.error(error?.detail || error?.message || 'Failed to fetch page')
                }
        ...
        return {
            ...
            runQuery: async ({ code, refs, opts }) => {
                ...
                if (isKernelLane) {
                    ...
                    if (!kernelKnownRunning) {
                        ...
                        toast.info('Starting a compute sandbox. The cell will run once it’s ready.')
                    }
                }
                ...
                try {
                    const { run_id } = await api.notebooks.sqlV2Run(props.notebookShortId, {
                        ...
                        // Omitted entirely for a Insights cell — the backend's defaults say the same
            ...
            interruptRun: async () => {
                ...
                try {
                    ...
                    actions.setIsInterrupting(false)
                    toast.error(error?.detail || error?.message || 'Failed to stop the run')
                }
            },
            stopPolling: () => {
