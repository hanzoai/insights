...
function buildSkillColumns(
    ...
    return [
            ...
            ? ([
                  {
                      ...
                      render: function renderOrigin(_, skill) {
                          ...
                          return (
                              <Tag type={canonical ? 'completion' : 'default'}>
                                  {canonical ? 'Canonical' : 'Custom'}
                              </Tag>
                          )
                      },
                  },
              ] as TableColumns<LLMSkillListApi>)
            : []),
        {
            title: 'Description',
        ...
        atColumn('created_at', 'Latest version created') as TableColumn<
            LLMSkillListApi,
            ...
            render: function renderMore(_, skill) {
                return (
                    <More
                        overlay={
                            <>
                                <Button to={skillUrl(skill.name)} data-attr="llma-skill-dropdown-view" fullWidth>
                                    View
                                </Button>
                                    ...
                                    <Button
                                        onClick={() => {
                                            Dialog.openForm({
                                                ...
                                                content: (
                                                    <Field name="newName" label="New skill name">
                                                        <Input
...
function SkillLeafTable({
    ...
    columns: TableColumns<LLMSkillListApi>
}): JSX.Element {
    return (
        <Table
            columns={columns}
    ...
    return (
        ...
        >
            <div className="flex flex-col gap-4">
                ...
                <section className="flex flex-col gap-2">
                    <h4 className="m-0 font-semibold">Any agent (Insights MCP)</h4>
                    <p className="m-0 text-sm text-secondary">
                        If the Insights MCP is connected, an agent can list and load these skills directly via its{' '}
                        <code>skill-*</code> tools — no marketplace needed. Just ask:
                    </p>
                    <CodeSnippet language={Language.Text} thing="agent prompt">
                        List my Insights skills, then load the one that fits this task.
                    </CodeSnippet>
                </section>
            </div>
        </Modal>
...
export function LLMSkillsScene(): JSX.Element {
    ...
    // nested Table inside the grouped tree reconciles on each parent re-render.
