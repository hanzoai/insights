...
class CustomFunctionMappingTemplateSerializer(serializers.Serializer):
...
class CustomFunctionTemplateSerializer(serializers.ModelSerializer):
    mapping_templates = CustomFunctionMappingTemplateSerializer(many=True, required=False, allow_null=True)
    id = serializers.CharField(source="template_id")

    class Meta:
        model = CustomFunctionTemplate
        fields = [
            "id",
            "name",
            "description",
...
class PublicCustomFunctionTemplateViewSet(
    mixins.ListModelMixin,
    mixins.RetrieveModelMixin,
    viewsets.GenericViewSet,
):
    permission_classes = [permissions.AllowAny]
    serializer_class = CustomFunctionTemplateSerializer
    queryset = CustomFunctionTemplate.objects.all()
    ...
    def filter_queryset(self, queryset: QuerySet) -> QuerySet:
        ...
        if self.request.path.startswith("/api/public_custom_function_templates"):
    ...
    def list(self, request: Request, *args, **kwargs):
        ...
        template_usage = (
            CustomFunction.objects.filter(deleted=False, enabled=True, template_id__in=template_ids)
            .values("template_id")
