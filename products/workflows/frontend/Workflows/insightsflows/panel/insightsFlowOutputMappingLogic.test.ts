...
import { insightsFlowOutputMappingLogic, normalizeOutputVariable } from './insightsFlowOutputMappingLogic'
...
describe('insightsFlowOutputMappingLogic', () => {
    describe('normalizeOutputVariable', () => {
        it.each([
            ...
            expect(normalizeOutputVariable(raw as InsightsFlowAction['output_variable'])).toEqual(expected)
        })
    })

    describe('logic', () => {
        let logic: ReturnType<typeof insightsFlowOutputMappingLogic.build>
        let editorLogic: ReturnType<typeof insightsFlowEditorLogic.build>
        ...
        beforeEach(() => {
            ...
            editorLogic = insightsFlowEditorLogic({ id: WORKFLOW_ID })
            editorLogic.mount()

            logic = insightsFlowOutputMappingLogic({ id: WORKFLOW_ID })
            logic.mount()
        })
