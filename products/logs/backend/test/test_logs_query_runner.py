...
class TestAttributeFilters(APIBaseTest):
    ...
    def _to_datastore_sql(self, query: LogsQuery) -> str:
        runner = LogsQueryRunner(query=query, team=self.team)
        executor = InsightsQLQueryExecutor(
            query_type="LogsQuery",
            query=runner.to_query(),
        ...
        executor.generate_datastore_sql()
        assert executor.datastore_prepared_ast is not None
        return executor.datastore_prepared_ast.to_insightsql()
    ...
    def test_nested_or_group_ors_across_attribute_keys(self):
        ...
        query_str = self._to_datastore_sql(query)

        self.assertIn("insightsDistinctId", query_str)
...
class TestLogsPersonIdFilter(DatastoreTestMixin, APIBaseTest):
    ...
    # Tests share one Datastore team; each uses unique distinct-id values for isolation.
    ...
    def _log_row(
        self, distinct_id_value: str, attribute_key: str = "insightsDistinctId", *, resource: bool = False
    ) -> dict:
        row: dict = {
            "uuid": str(uuid4()),
    ...
    def test_person_id_expands_to_all_distinct_ids(self):
        ...
        self.assertEqual(
            sorted(r["attributes"]["insightsDistinctId"] for r in results),
    ...
    def test_person_id_matches_builtin_convention_keys(self):
        ...
        # only the default `insightsDistinctId` is configured; `distinct_id` is convention-only.
        ...
        self._insert_logs(
            [
                ...
                self._log_row("person-id-test-conv", attribute_key="insightsDistinctId"),
            ]
        )
        ...
        self.assertEqual(
            ...
            {"distinct_id", "insightsDistinctId"},
        )
    ...
    def test_person_id_filter_targets_string_attribute_map_for_numeric_ids(self):
        ...
        self.assertIn("insightsDistinctId__str", query_str)
        self.assertNotIn("insightsDistinctId__float", query_str)
