...
class TestDataWarehouseSavedQueryColumnAnnotation(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("known_column", {"status": {"datastore": "String"}}, "status", 201),
            ("unknown_column_when_known", {"status": {"datastore": "String"}}, "typo", 400),
            ("unknown_column_when_columns_empty", {}, "any_draft_column", 201),
            ("view_level_always_allowed", {"status": {"datastore": "String"}}, "", 201),
        ]
    )
    ...
    def test_cannot_annotate_view_user_is_denied(self):
        ...
        view = DataWarehouseSavedQuery.objects.create(
            ...
            name="secret_metrics",
            query={"kind": "InsightsQLQuery", "query": "select 1"},
            columns={"revenue": {"datastore": "Int64"}},
        )
        ...
        member = self._create_user("member@hanzo.ai")
