...
from insights.insightsql.database.postgres_utils import add_postgres_foreign_key_lazy_joins
from insights.insightsql.database.s3_table import S3Table
from insights.insightsql.database.schema.ai_events import AiEventsTable
from insights.insightsql.database.schema.app_metrics2 import AppMetrics2Table
from insights.insightsql.database.schema.channel_type import create_initial_channel_type, create_initial_domain_type
from insights.insightsql.database.schema.cohort_membership import CohortMembershipTable
from insights.insightsql.database.schema.cohort_people import CohortPeople, RawCohortPeople
from insights.insightsql.database.schema.document_embeddings import (
    INSIGHTSQL_MODEL_TABLES,
    DocumentEmbeddingsTable,
    RawDocumentEmbeddingsTable,
)
from insights.insightsql.database.schema.duckdb_table_functions import GenerateSeriesTable, RangeTable
from insights.insightsql.database.schema.error_tracking_fingerprint_issue_state import (
    ErrorTrackingFingerprintIssueStateTable,
    RawErrorTrackingFingerprintIssueStateTable,
)
from insights.insightsql.database.schema.error_tracking_issue_fingerprint_overrides import (
...
from insights.insightsql.database.schema.exchange_rate import ExchangeRateTable
from insights.insightsql.database.schema.experiment_exposures_preaggregated import ExperimentExposuresPreaggregatedTable
from insights.insightsql.database.schema.experiment_metric_events_preaggregated import (
...
from insights.insightsql.database.schema.numbers import NumbersTable
from insights.insightsql.database.schema.person_distinct_id_overrides import (
    PersonDistinctIdOverridesTable,
    RawPersonDistinctIdOverridesTable,
)
from insights.insightsql.database.schema.person_distinct_ids import PersonDistinctIdsTable, RawPersonDistinctIdsTable
from insights.insightsql.database.schema.persons import PersonsTable, RawPersonsTable
from insights.insightsql.database.schema.persons_revenue_analytics import PersonsRevenueAnalyticsTable
from insights.insightsql.database.schema.pg_embeddings import PgEmbeddingsTable
from insights.insightsql.database.schema.preaggregation_results import PreaggregationResultsTable
...
from insights.insightsql.database.schema.session_replay_events import RawSessionReplayEventsTable, SessionReplayEventsTable
...
from insights.insightsql.database.schema.sessions_v2 import RawSessionsTableV2, SessionsTableV2
from insights.insightsql.database.schema.sessions_v3 import RawSessionsTableV3, SessionsTableV3
...
from insights.insightsql.database.schema.web_analytics_preaggregated import (
...
from insights.insightsql.database.utils import get_join_field_chain, qualify_join_key_expr
from insights.insightsql.database.warehouse_join_resolvers import data_warehouse_resolver_params
from insights.insightsql.errors import QueryError, ResolutionError, TableAccessDeniedError
...
from insights.schema_enums import DatabaseSerializedFieldType, PersonsOnEventsMode, SessionTableVersion
...
# module — and the InsightsQL resolver/printer that import it — load without django.setup(). The
# type-only references live in TYPE_CHECKING; insights.schema (pydantic) is likewise runtime-imported
...
@dataclasses.dataclass
class InsightsQLDatabaseSources:
    ...
    is_insightsql_warehouse_access_control_enabled: bool
...
# READ BEFORE EDITING:
# --------------------
# Do NOT add any new table to this, add them to the `insights` table node.
...
ROOT_TABLES__DO_NOT_ADD_ANY_MORE: dict[str, TableNode] = {
    ...
    **{name: TableNode(name=name, table=table) for name, table in INSIGHTSQL_MODEL_TABLES.items()},
...
# under insights.insightsql.* except the Workload enum and product-owned facade schema modules
...
_CATALOG_PICKLE_MODULE_PREFIXES = ("insights.insightsql.",)
_CATALOG_PICKLE_MODULES = frozenset(
    {
        "insights.datastore.workload",
        "products.customer_analytics.backend.facade.insightsql",
    }
)
...
def build_database_root_node(*, include_insights_tables: bool = True) -> TableNode:
    # Double-checked locking so concurrent first-callers don't each rebuild + pickle the catalog.
    blob = _DATABASE_ROOT_NODE_BLOBS.get(include_insights_tables)
    if blob is None:
        with _DATABASE_ROOT_NODE_BLOBS_LOCK:
            ...
            if blob is None:
                ...
                blob = pickle.dumps(
                    _construct_database_root_node(include_insights_tables=include_insights_tables),
                    protocol=pickle.HIGHEST_PROTOCOL,
                )
                blob = pickletools.optimize(blob)  # drop unused memo opcodes: ~10% smaller, faster load
                _DATABASE_ROOT_NODE_BLOBS[include_insights_tables] = blob
...
def _construct_database_root_node(*, include_insights_tables: bool) -> TableNode:
    ...
    if include_insights_tables:
        ...
        children = {
            **root_tables,
            ...
            "system": SystemTables(),
            # Deduplicated read interface over insights.marketing_costs_preaggregated. Registered at root
...
def _unentitled_system_tables(team: Team) -> set[str]:
    ...
    from insights.cloud_utils import is_cloud  # noqa: PLC0415
...
def _compute_system_table_access_decision(
    ...
    # Lazy imports keep the Django ORM off this module's import path.
    from insights.models.organization import OrganizationMembership  # noqa: PLC0415
    from insights.rbac.user_access_control import NO_ACCESS_LEVEL, UserAccessControl  # noqa: PLC0415
    from insights.shared_link_user import SharedLinkUser  # noqa: PLC0415
...
class Database(BaseModel):
    ...
    # keyed by InsightsQL DataWarehouseTable.table_id (str(Django table UUID)).
    ...
    def __init__(
        ...
        include_insights_tables: bool = True,
    ):
        super().__init__(tables=build_database_root_node(include_insights_tables=include_insights_tables))
        try:
    ...
    def is_table_access_denied(self, table_name: str | list[str]) -> bool:
        """True if access control denied this table when the InsightsQL database was built.
    ...
    def _suggest_table_names(self, name: str, *, limit: int = 3) -> list[str]:
        ...
        try:
            candidates = set(self.get_insights_table_names())
    ...
    def get_insights_table_names(self, include_hidden: bool = False) -> list[str]:
        if include_hidden:
            root_keys = set(ROOT_TABLES__DO_NOT_ADD_ANY_MORE.keys())
            insights_node = self.tables.children.get("insights")
            if insights_node and insights_node.children:
                insights_only_keys = {f"insights.{k}" for k in insights_node.children.keys() if k not in root_keys}
            else:
                insights_only_keys = set()
            return sorted(root_keys | insights_only_keys)
    ...
    def apply_schema_scope(self) -> None:
        ...
        built_in_global_table_names = set(self.get_insights_table_names(include_hidden=True))
        ...
        # built-in Insights tables when a direct source reuses names like `events` or `persons`.
    ...
    def serialize(
        ...
        include_hidden_insights_tables: bool = False,
        ...
        insights_table_names = (
            []
            if self._is_direct_query()
            else self.get_insights_table_names(include_hidden=include_hidden_insights_tables)
        )
        ...
        if self._is_direct_query():
            ...
            from insights.insightsql.direct_sql.capability import is_direct_capable  # noqa: PLC0415
        ...
        for view_name in views:
            ...
            if isinstance(view, RevenueAnalyticsBaseView):
                tables[view_name] = DatabaseSchemaManagedViewTable(
                    ...
                    query=InsightsQLQuery(query=view.query),
                )
            ...
            if saved_query and saved_query.origin == DataWarehouseSavedQuery.Origin.ENDPOINT:
                tables[view_name] = DatabaseSchemaEndpointTable(
                    ...
                    name=view_name,
                    query=InsightsQLQuery(query=saved_query.query["query"]),  # type: ignore[index]
                    row_count=row_count,
    ...
    @staticmethod
    @tracer.start_as_current_span("create_insightsql_database")  # Legacy name to keep backwards compatibility
    def create_for(
        ...
        *,
        ...
        timings: InsightsQLTimings | None = None,
    ...
    def _fetch_sources(
        ...
        *,
        ...
        # Lazy imports keep the Django ORM off this module's import path.
        from insights.models.group_type_mapping import get_group_types_for_project  # noqa: PLC0415
        from insights.models.team.team import Team  # noqa: PLC0415
        from insights.shared_link_user import SharedLinkUser  # noqa: PLC0415
        ...
        is_insightsql_warehouse_access_control_enabled = feature_enabled_or_false(
            "insightsql-warehouse-access-control",
            str(team.uuid),
        ...
        return InsightsQLDatabaseSources(
            team=team,
            user=user,
            connection_id=connection_id,
            modifiers=modifiers,
            is_managed_viewset_enabled=is_managed_viewset_enabled,
            is_insightsql_warehouse_access_control_enabled=is_insightsql_warehouse_access_control_enabled,
    ...
    def _build_from_sources(
        sources: InsightsQLDatabaseSources,
        timings: InsightsQLTimings | None = None,
        build_postgres_foreign_keys: bool = True,
    ) -> Database:
        """Construct the InsightsQL Database purely from already-fetched sources. Performs no I/O: every
        ...
        from products.data_modeling.backend.facade.models import DataWarehouseSavedQuery  # noqa: PLC0415
        from products.data_warehouse.backend.facade.insightsql import get_warehouse_sync_warnings  # noqa: PLC0415
        from products.warehouse_sources.backend.facade.models import (  # noqa: PLC0415
            DataWarehouseTable,
            ExternalDataSource,
        )
        ...
        with timings.measure("database", emit_span=True):
            database = Database(
                ...
                include_insights_tables=sources.connection_id is None,
            )
        ...
        with timings.measure("data_warehouse_saved_query", emit_span=True):
            for saved_query in sources.saved_queries:
                with timings.measure(f"saved_query_{saved_query.name}"):
                    if (
                        sources.is_insightsql_warehouse_access_control_enabled
                    ...
                    views.add_child(
                        TableNode.create_nested_for_chain(
                            ...
                            table=saved_query.insightsql_definition(modifiers),
                        ),
                        ...
                        with timings.measure(f"endpoint_saved_query_{endpoint_saved_query.name}"):
                            ...
                            views.add_child(
                                TableNode(
                                    ...
                                    table=endpoint_saved_query.insightsql_definition(modifiers),
                                ),
                                table_conflict_mode="ignore",
                            )
                except Exception as e:
            ...
            with timings.measure("build_tables", emit_span=True):
                ...
                for table in sources.warehouse_tables:
                    ...
                    with timings.measure(f"table_{table.name}"):
                        s3_table = table.insightsql_definition(modifiers)
        ...
        if build_postgres_foreign_keys:
            with timings.measure("warehouse_foreign_keys", emit_span=True):
                for insightsql_table, warehouse_table_model in warehouse_tables_to_process:
                    add_postgres_foreign_key_lazy_joins(
                        insightsql_table=insightsql_table,
                        ...
                        if isinstance(person_field, ast.FieldTraverser):
                            ...
                            for chain in person_field.chain:
                                if isinstance(table_or_field, ast.LazyJoin):
                                    table_or_field = table_or_field.resolve_table(
                                        InsightsQLContext(team_id=team_id, database=database)
                                    )
                            ...
                            if isinstance(table_or_field, ast.VirtualTable):
                                ...
                                if source_table_key_node is not None:
                                    override_source_table_key = source_table_key_node.to_insightsql()
...
def _setup_group_key_fields(database: Database, group_types: list[dict[str, Any]]) -> None:
    ...
    for group_index in range(5):
        ...
        if group_mapping and group_mapping["created_at"]:
            ...
            # Must stay a datetime constant: a naive string literal would be parsed by Datastore in the
...
def _use_virtual_fields(database: Database, modifiers: InsightsQLQueryModifiers, timings: InsightsQLTimings) -> None:
    events_table = database.get_table("events")
    ...
    with timings.measure("traffic_type_virtual_fields"):
        from insights.insightsql.database.schema.traffic_type import (
...
INSIGHTSQL_CHARACTERS_TO_BE_WRAPPED = ["@", "-", "!", "$", "+"]
...
def _settled_catalog_certifications(
    ...
    """
    from insights.schema import DatabaseSchemaTableCertification  # noqa: PLC0415

    from insights.insightsql.database.schema.information_schema import _can_read_catalog  # noqa: PLC0415
...
def serialize_fields(
    ...
    from insights.schema import DatabaseSchemaField  # noqa: PLC0415
    ...
    for field_key, field in field_input.items():
        ...
        if any(n in field_key for n in INSIGHTSQL_CHARACTERS_TO_BE_WRAPPED):
            insightsql_value = f"`{field_key}`"
        else:
            insightsql_value = str(field_key)
        ...
        if field_key == "team_id" and table_type == "insights":
            ...
            if isinstance(field, IntegerDatabaseField):
                field_output.append(
                    DatabaseSchemaField(
                        name=field_key,
                        insightsql_value=insightsql_value,
                ...
                field_expr = resolve_types_from_table(field.expr, table_chain, context, "insightsql")
            ...
            field_output.append(
                DatabaseSchemaField(
                    ...
                    type=type,
                    schema_valid=schema_valid,
                    table=field.resolve_table(context).to_printed_insightsql(),
            ...
            field_output.append(
                DatabaseSchemaField(
                    ...
                    schema_valid=schema_valid,
                    table=field.to_printed_insightsql(),
                    fields=list(field.fields.keys()),
                )
            )
