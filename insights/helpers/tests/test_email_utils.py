...
from insights.helpers.email_utils import (
...
class TestESPSuppressionCheck(SimpleTestCase):
    ...
    def test_cache_hit_returns_cached_value_without_api_call(self, mock_cache):
        ...
        with patch("insights.helpers.email_utils.requests.get") as mock_get:
...
class TestESPSuppressionAnalytics(SimpleTestCase):
    ...
    @override_settings(CUSTOMER_IO_API_KEY="test-app-api-key")
    @patch("insights.helpers.email_utils.hanzo_insights.capture")
    @patch("insights.helpers.email_utils.cache")
    @patch("insights.helpers.email_utils.requests.get")
