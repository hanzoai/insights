...
`NON_BREAKDOWN_DISPLAY_TYPES`), so segmentation and tail drilldown route through InsightsQL.
...
**InsightsQL** — see [shared-patterns.md](./shared-patterns.md#insightsql-quantile-template).
...
- **Cross-tab.** If two dimensions both look elevated (e.g. host + lib_version),
  `GROUP BY` both in InsightsQL to see whether one is causal or they're correlated.

## 4. Tail actor drilldown

`insights:query-trends-actors` can't select by percentile. Use InsightsQL with a quantile
filter:
...
`insights:query-session-recordings-list` for UI-shaped shifts.
...
Run `insights:query-lifecycle` on the same event. A distribution shift often reflects a
