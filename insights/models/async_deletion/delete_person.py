...
from insights.datastore.client.connection import DatastoreUser, Workload, get_datastore_creds
from insights.datastore.cluster import ON_CLUSTER_CLAUSE
from insights.settings.data_stores import DATASTORE_CLUSTER, DATASTORE_DATABASE
    ...
    while True:
        statuses = [
            ...
            for (status,) in sync_execute(
                f"SELECT status FROM clusterAllReplicas('{DATASTORE_CLUSTER}', system.dictionaries) "
                "WHERE database = %(database)s AND name = %(name)s",
                {"database": DATASTORE_DATABASE, "name": name},
...
def remove_deleted_person_data():
    """Hard-delete persons whose current (latest-version) state is deleted from Datastore.
    ...
    """
    dict_reader_creds = get_datastore_creds(DatastoreUser.DICT_READER)
    ...
    qualified = f"{DATASTORE_DATABASE}.{name}"
