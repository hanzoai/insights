...
def _view(team: Team, name: str = "revenue_view") -> DataWarehouseSavedQuery:
    return DataWarehouseSavedQuery.objects.create(team=team, name=name, query={"kind": "InsightsQLQuery"})


class TestTableCertificationModel(BaseTest):
    def test_requires_exactly_one_target(self) -> None:
        table = _table(self.team)
        view = DataWarehouseSavedQuery.objects.create(team=self.team, name="v", query={"kind": "InsightsQLQuery"})
...
class TestSerializedSchemaCertification(APIBaseTest):
    def setUp(self) -> None:
        ...
        self.view = DataWarehouseSavedQuery.objects.create(
            team=self.team,
            name="revenue_view",
            query={"kind": "InsightsQLQuery", "query": "select event from events limit 1"},
            columns={"event": "String"},
        )

    def _serialized_tables(self) -> dict:
        database = Database.create_for(team=self.team, user=self.user)
        context = InsightsQLContext(team=self.team, team_id=self.team.pk, database=database)
