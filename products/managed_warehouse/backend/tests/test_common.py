...
class TestResetDucklakeCatalog:
    ...
    @patch("products.managed_warehouse.backend.common.is_dev_mode", return_value=True)
    @patch("products.managed_warehouse.backend.common.psycopg")
...
class TestInitializeDucklake:
    @patch.dict("os.environ", {"INSIGHTS_ALLOW_DUCKLAKE_CATALOG_RESET": "1"}, clear=True)
    @patch("products.managed_warehouse.backend.common.is_dev_mode", return_value=True)
    @patch("products.managed_warehouse.backend.common.reset_ducklake_catalog")
    @patch("products.managed_warehouse.backend.common.run_smoke_check")
    @patch("products.managed_warehouse.backend.common.ensure_ducklake_catalog")
    @patch("products.managed_warehouse.backend.common.duckdb")
    def test_auto_resets_on_version_mismatch_dev_mode(
        ...
        with patch("products.managed_warehouse.backend.common.attach_catalog", side_effect=attach_side_effect):
    ...
    def test_raises_in_production_mode(
        ...
        with patch("products.managed_warehouse.backend.common.attach_catalog", side_effect=version_exc):
    ...
    def test_raises_for_non_version_error_dev_mode(
        ...
        with patch("products.managed_warehouse.backend.common.attach_catalog", side_effect=unrelated_exc):
...
class TestValidateDuckgresIdentifier:
    ...
    def test_accepts_safe_identifiers(self, ident):
        from products.managed_warehouse.backend.common import validate_duckgres_identifier
