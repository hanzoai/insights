...
No Insights, Braintrust, or OpenAI dependency — this module is pure data
...
class ParsedLog:
    ...
    the insights claude/codex adapters actually populate).
...
class AcpLogParser:
    ...
    def __init__(self, initial_prompt: str = ""):
        ...
        self._pending_gen_usage: dict[str, int] = {}  # Latest _insights/usage_update, consumed on flush
    ...
    def _handle_entry(self, entry: dict) -> None:
        ...
        if method == "_insights/usage_update":
    ...
    def _handle_result(self, entry_result: dict) -> bool:
        ...
        if not self._result.model:
            # The insights adapters don't fill result.models; the model lives in the
    ...
    def _on_tool_call(self, update: dict, ts: str) -> None:
        ...
        # (claude → _meta.claudeCode, codex → _meta.insights); the title is only a
        # display string (e.g. "insights/exec"), so it's the last resort.
        ...
        tool_name = next(
            (
                ...
                for adapter_meta in (meta.get("claudeCode"), meta.get("insights"))
    ...
    def _flush_generation(self, token_usage: dict[str, int] | None = None) -> None:
        ...
        # latest _insights/usage_update, which codex emits once per model call.
