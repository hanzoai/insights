...
from products.web_analytics.backend.insightsql_queries.web_goals_lazy_precompute import can_use_lazy_precompute


@override_settings(IN_UNIT_TESTING=True)
class TestWebGoalsLazyPrecompute(DatastoreTestMixin, APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    def _enable_lazy(self):
        ...
        # `web-analytics-precompute-toggle` org flag via hanzo_insights.
        return patch(
    ...
    # ----------------------------------------------------------------------
    # Eligibility — no Datastore needed for these.
    # ----------------------------------------------------------------------
    ...
    def test_rejected_for_sessions_v2_uuid_mode(self):
        self._create_action("Pageview")
        with self._enable_lazy():
            modifiers = InsightsQLQueryModifiers(sessionsV2JoinMode=SessionsV2JoinMode.UUID)
    ...
    def test_max_actions_constant_matches_live_runner_slice(self):
        ...
        from products.web_analytics.backend.insightsql_queries.web_goals_lazy_precompute import MAX_ACTIONS
    ...
    def test_stale_served_enqueues_background_revalidation(self):
        ...
        from products.web_analytics.backend.insightsql_queries.web_goals_lazy_precompute import execute_lazy_precomputed_read
        ...
        with (
            ...
            self._enable_lazy(),
            patch(
                "products.web_analytics.backend.insightsql_queries.web_goals_lazy_precompute.ensure_web_goals_precomputed",
