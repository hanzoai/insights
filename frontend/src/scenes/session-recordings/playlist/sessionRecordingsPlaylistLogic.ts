...
export const getDefaultFilters = (
    ...
    const defaults: RecordingUniversalFilters = {
        ...
        order:
            preferredSort?.order ??
            (!hasSpecificIntent &&
            (insights.getFeatureFlag(FEATURE_FLAGS.REPLAY_PLAYLIST_SURFACING_SCORE) ||
                insights.getFeatureFlag(FEATURE_FLAGS.REPLAY_PLAYLIST_RELEVANCE_SORT_EXPERIMENT) === 'test')
                ? 'surfacing_score'
...
export interface sessionRecordingsPlaylistLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        ...
        allowInsightsQLFilters: (featureFlags: FeatureFlagsSet) => boolean
    }
}
...
export const sessionRecordingsPlaylistLogic = kea<sessionRecordingsPlaylistLogicType>([
        ...
        handleDeleteSelectedRecordings: async ({ shortId }: { shortId?: string }) => {
            ...
            try {
                ...
                if (actualCount < deleteCount) {
                    ...
                    toast.success(`${actualCount} recording${actualCount > 1 ? 's' : ''} deleted!`)
                }
            } catch (e) {
    ...
    selectors({
        ...
        allowInsightsQLFilters: [
            (s) => [s.featureFlags],
            (featureFlags: import('lib/logic/featureFlagLogic').FeatureFlagsSet): boolean =>
                !!featureFlags[FEATURE_FLAGS.REPLAY_INSIGHTSQL_FILTERS],
        ],
    }),
