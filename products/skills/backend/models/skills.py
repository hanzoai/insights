...
from insights.models.scoping.manager import TeamScopedManager
...
# generic create flows are how custom scouts and review-script skills are authored — without the stamp
# they'd never surface beside their canonical siblings. Values mirror SCOUT_SKILL_CATEGORY /
# REVIEW_FN_SKILL_CATEGORY (products can't import each other, so they're duplicated here exactly
...
class LLMSkillOwner(UUIDModel):
    ...
    # db_constraint=False on the FKs to hot tables (insights_team, insights_user): a real FK
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, db_constraint=False, related_name="+")
    # Logical skill identity, not a version FK — see the class docstring.
    skill_name = models.CharField(max_length=64)
    user = models.ForeignKey("insights.User", on_delete=models.CASCADE, db_constraint=False, related_name="+")
    created_at = models.DateTimeField(default=timezone.now)
