...
_FLAG_PATH = "products.replay_vision.backend.feature_flag.hanzo_insights.feature_enabled"
_GENERATE_EMBEDDING_PATH = "products.replay_vision.backend.max_tools.async_generate_embedding"
_EXECUTE_INSIGHTSQL_PATH = "products.replay_vision.backend.max_tools.execute_insightsql_query"
...
class TestSearchReplayVisionObservationsTool(BaseTest):
    ...
    def _ch_stub(ranked: list[tuple[ReplayObservation, float]]):
        """Simulate the Datastore ranking, honoring the structured metadata filters (verdict/tags/score) the
    ...
    async def test_ranks_and_formats_matching_observations(self):
        ...
        # Datastore returns ids ordered by ascending cosine distance (nearest first).
        insightsql_results = MagicMock(results=[(str(obs_near.id), 0.1), (str(obs_far.id), 0.4)])
        ...
        with (
            ...
            patch(_EXECUTE_INSIGHTSQL_PATH, return_value=insightsql_results),
        ):
    ...
    async def test_resolves_scanner_id_from_context(self):
        ...
        with (
            ...
            patch(_EXECUTE_INSIGHTSQL_PATH, return_value=MagicMock(results=[(str(obs.id), 0.1)])),
        ):
    ...
    async def test_returns_empty_when_no_matches(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            patch(_EXECUTE_INSIGHTSQL_PATH, return_value=MagicMock(results=[])),
        ):
    ...
    async def test_searches_across_all_readable_scanners_when_no_scanner_given(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            patch(_EXECUTE_INSIGHTSQL_PATH, return_value=MagicMock(results=[(str(obs_a.id), 0.1), (str(obs_b.id), 0.2)])),
        ):
    ...
    async def test_verdict_filter_keeps_only_matching_results(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            # Both would rank highly; filter-first restricts the Datastore ranking to the YES result only.
            patch(_EXECUTE_INSIGHTSQL_PATH, side_effect=self._ch_stub([(obs_no, 0.1), (obs_yes, 0.2)])),
        ):
            content, artifact = await self._tool()._arun_impl(
    ...
    async def test_verdict_filter_is_case_insensitive(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            patch(_EXECUTE_INSIGHTSQL_PATH, side_effect=self._ch_stub([(obs_yes, 0.1)])),
        ):
    ...
    async def test_tag_filter_keeps_only_matching_results(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            patch(_EXECUTE_INSIGHTSQL_PATH, side_effect=self._ch_stub([(obs_completed, 0.1), (obs_abandoned, 0.2)])),
        ):
            content, artifact = await self._tool()._arun_impl(
    ...
    async def test_tag_filter_matches_normalized_form(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            patch(_EXECUTE_INSIGHTSQL_PATH, side_effect=self._ch_stub([(obs, 0.1)])),
        ):
            content, artifact = await self._tool()._arun_impl(
    ...
    async def test_score_range_filter_keeps_only_matching_results(self):
        ...
        with (
            patch(_FLAG_PATH, return_value=True),
            patch(_GENERATE_EMBEDDING_PATH, new_callable=AsyncMock, return_value=MagicMock(embedding=[0.1])),
            patch(_EXECUTE_INSIGHTSQL_PATH, side_effect=self._ch_stub([(obs_five, 0.1), (obs_zero, 0.2)])),
        ):
...
class TestObservationFiltersTagClause:
    """Pure-logic clause construction — no DB/Datastore, so it runs without the full test stack."""
