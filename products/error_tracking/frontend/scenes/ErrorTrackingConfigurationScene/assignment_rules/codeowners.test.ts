...
describe('codeowners helpers', () => {
    describe('parseCodeowners', () => {
        it('parses patterns and owners, skipping comments and blank lines', () => {
            const text = [
                '# Comment line',
                '',
                'products/error_tracking/** @hanzo/error-tracking',
                'frontend/   @hanzo/frontend  @alice',
                '   # indented comment',
                'no-owners-here',
            ].join('\n')

            expect(parseCodeowners(text)).toEqual([
                { pattern: 'products/error_tracking/**', owners: ['@hanzo/error-tracking'] },
                { pattern: 'frontend/', owners: ['@hanzo/frontend', '@alice'] },
            ])
        })
    ...
    describe('bestAssigneeMatch', () => {
        ...
        it('matches roles by longest common substring', () => {
            expect(bestAssigneeMatch('@hanzo/error-tracking', roles, members)).toMatchObject({
                type: 'role',
    ...
    describe('findCodeownersErrors', () => {
        it('flags lines with no owner and invalid owner tokens, with 1-based line numbers', () => {
            const text = [
                ...
                'products/error_tracking/** @hanzo/error-tracking',
                'frontend/**',
                'docs/* not-a-handle dev@hanzo.ai',
            ].join('\n')
