...
from insights.redis import TEST_clear_clients
...
class TestTaskRunEventIngest(TestCase):
    ...
    def test_streaming_ingest_writes_ordered_events_with_explicit_completion(self) -> None:
        ...
        with patch.object(TaskRun, "heartbeat_workflow") as heartbeat_workflow:
            status, body = self._call_ingest(
                ...
                [
                    ...
                    {
                        "seq": 2,
                        "event": {"type": "notification", "notification": {"method": "_insights/task_complete"}},
                    },
        ...
        self.assertEqual(self._read_notification_methods(), ["session/update", "_insights/task_complete"])
    ...
    def test_rtk_savings_capture_uses_authenticated_ids_and_idempotent_event_uuid(self) -> None:
        ...
        capture_rtk_savings.assert_called_once_with(
            self.team.id,
            str(uuid5(NAMESPACE_URL, f"insights-task-rtk-savings:{self.task_run.id}:1")),
            {
    ...
    def test_rtk_savings_capture_failure_can_be_retried(self) -> None:
        ...
        self.assertEqual(self._read_notification_methods(), ["_insights/rtk_savings"])
    ...
    def test_turn_complete_ingest_notifies_interactive_run(self) -> None:
        self.task.created_by = User.objects.create_user("ingest-push@hanzo.ai", None, "Ingest")
                ...
                [
                    {
                        ...
                        "event": {
                            "type": "notification",
                            "notification": {"method": "_insights/turn_complete"},
                        },
                    }
                ],
            )
    ...
    def test_duplicate_terminal_sequence_does_not_complete_without_completion_line(self) -> None:
        token = self._create_token()
        terminal_event = {"type": "notification", "notification": {"method": "_insights/task_complete"}}
            ...
            json.dumps(
                {
                    ...
                    "event": {
                        "type": "notification",
                        "notification": {"method": "_insights/console", "params": {"message": "hello ☃"}},
                    },
                },
                ensure_ascii=False,
            )
                ...
                [
                    {
                        ...
                        "event": {
                            "type": "notification",
                            "notification": {"method": "_insights/console", "params": {"message": "starting"}},
                        },
                    }
                ],
                1,
                ...
                [
                    {
                        ...
                        "event": {
                            "type": "notification",
                            "notification": {
                                "method": "_insights/turn_complete",
                    ...
                    },
                    {
                        ...
                        "event": {
                            "type": "notification",
                            "notification": {"method": "_insights/console", "params": {"message": "tail"}},
                        },
                    },
                ],
                2,
