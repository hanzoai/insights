...
from insights.management.commands import backfill_workflows_slack_integration as backfill
...
def _slack_action(action_id: str, slack_workspace_value: int | None) -> dict:
    ...
    if slack_workspace_value is not None:
        inputs["slack_workspace"] = {"order": 0, "value": slack_workspace_value, "templating": "script"}
    return {
        "id": action_id,
        "name": "Slack",
        "type": "function",
...
class TestBackfillWorkflowsSlackIntegrationCommand(BaseTest):
    ...
    def setUp(self):
        ...
        with patch("products.workflows.backend.models.hog_flow.hog_flow.reload_hog_flows_on_workers"):
            ...
            self.untouched_flow = InsightsFlow.objects.create(
                team=self.team,
            ...
            self.other_team_flow = InsightsFlow.objects.create(
