...
from insights.test.base import APIBaseTest, _create_event, flush_persons_and_events
...
from insights.tasks import exporter
...
class TestExports(APIBaseTest):
    ...
    def test_dataset_exports_are_visible_to_their_creator_in_the_export_list(self) -> None:
        ...
        other_user = User.objects.create_and_join(self.organization, "dataset-export-peer@hanzo.ai", "password")
    ...
    @parameterized.expand(
        [
            (
                ...
                timedelta(seconds=INSIGHTSQL_INCREASED_MAX_EXECUTION_TIME + 31),
                False,
            ),
    ...
    def test_teammate_can_access_session_recording_png_export_by_other_user(self, _name, url_template) -> None:
        ...
        owner = self.user
        teammate = User.objects.create_and_join(self.organization, "session-png-peer@hanzo.ai", "password")
    ...
    def test_cannot_access_export_after_losing_resource_access(self, _name, url_template) -> None:
        other_user = User.objects.create_and_join(self.organization, "rbac-test@hanzo.ai", "password")
    ...
    def test_cannot_access_session_recording_export_after_losing_access(self, _name, url_template) -> None:
        ...
        other_user = User.objects.create_and_join(self.organization, "rbac-recording@hanzo.ai", "password")
    ...
    def test_teammate_can_access_system_session_video_export(self, _name, url_template) -> None:
        ...
        owner = self.user
        teammate = User.objects.create_and_join(self.organization, "signal-session-viewer@hanzo.ai", "password")
    ...
    def test_teammate_blocked_from_system_video_export_when_no_session_access(self, _name, url_template) -> None:
        ...
        owner = self.user
        teammate = User.objects.create_and_join(self.organization, "signal-session-blocked@hanzo.ai", "password")
    ...
    def test_cannot_access_session_video_export_when_session_recording_row_missing_and_resource_denied(
        ...
        owner = self.user
        teammate = User.objects.create_and_join(self.organization, "ghost-session-blocked@hanzo.ai", "password")
    ...
    def test_list_exports_with_session_recording_only_shows_own_matching_exports(self) -> None:
        ...
        owner = self.user
        teammate = User.objects.create_and_join(self.organization, "signal-session-lister@hanzo.ai", "password")
    ...
    def test_list_exports_without_session_filter_hides_others_system_exports(self) -> None:
        owner = self.user
        teammate = User.objects.create_and_join(self.organization, "solo-exports@hanzo.ai", "password")
...
class TestExportHeatmapSSRFValidation(APIBaseTest):
    ...
    def test_accepts_valid_heatmap_url(self, _name: str, url: str) -> None:
        ...
        with (
            patch("insights.security.url_validation.resolve_host_ips") as mock_resolve,
            patch("products.exports.backend.api.exports.ExportedAssetSerializer._start_export_workflow"),
        ):
...
class TestExportAssetCounters(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            (DatastoreAtCapacity(), FAILURE_TYPE_SYSTEM, 0.0, 1.0, False),
        ],
...
class TestExportsResourceAccessControl(APIBaseTest):
    ...
    def setUp(self) -> None:
        ...
        self.organization.available_product_features = [{"key": "access_control", "name": "Access control"}]
        self.organization.save()
        self.member = User.objects.create_and_join(self.organization, "export-member@hanzo.ai", "password")
