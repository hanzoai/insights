...
from insights.schema import InsightsQLQuery, InsightsQLVariable

from insights.ducklake import cp_teams
from insights.ducklake.client import _SEARCH_PATH_SCHEMAS, compile_insightsql_to_ducklake_sql, execute_ducklake_query
...
def _cp_no_rows():
    ...
    with mock.patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=[]):
        yield
    cp_teams.clear_cache()
...
class TestCompileInsightsQLToDuckLakeSQL:
    def test_substitutes_variable_placeholders(self):
        ...
        query = InsightsQLQuery(
            ...
            variables={
                str(variable.id): InsightsQLVariable(variableId=str(variable.id), code_name="event_name", value="purchase")
            },
        )

        postgres_sql, values, _insightsql_pretty = compile_insightsql_to_ducklake_sql(team.pk, query)
...
class TestDuckLakeModelRedirect:
    def test_materialized_model_resolves_to_ducklake_table_not_s3(self):
        ...
        DataWarehouseSavedQuery.objects.create(
            ...
            columns={"org_id": {"datastore": "String", "insightsql": "StringDatabaseField"}},
            table=source_table,
            is_materialized=True,
            status=DataWarehouseSavedQuery.Status.COMPLETED,
        )

        query = InsightsQLQuery(query="SELECT org_id FROM vitally_org")
        postgres_sql, _values, _insightsql = compile_insightsql_to_ducklake_sql(team.pk, query)

        # The duckgres path must read the DuckLake-materialized model, not the
        # Datastore s3() table function, which DuckDB cannot execute.
    ...
    def test_source_table_resolves_to_ducklake_table_not_s3(self):
        from insights.ducklake.common import duckgres_data_imports_schema, duckgres_data_imports_table_name
        from insights.models import Organization, Team
        ...
        query = InsightsQLQuery(query="SELECT id FROM myprefix_stripe_customers")
        postgres_sql, _values, _insightsql = compile_insightsql_to_ducklake_sql(team.pk, query)
...
class TestDuckgresShadowCompilation:
    @mock.patch("insights.ducklake.client.compile_insightsql_to_ducklake_sql")
    def test_materialization_compile_bypasses_warehouse_access_control(self, mock_compile):
        from insights.temporal.data_modeling.activities.materialize_view_duckgres import _compile_insightsql_to_postgres_sql

        mock_compile.return_value = ("SELECT * FROM source", {}, "SELECT * FROM source")

        _compile_insightsql_to_postgres_sql("SELECT * FROM source", 1)
...
class TestExecuteDuckLakeQuery:
    def test_rejects_both_sql_and_query(self):
        with pytest.raises(ValueError, match="not both"):
            execute_ducklake_query(1, sql="SELECT 1", query=InsightsQLQuery(query="SELECT 1"))
    ...
    @mock.patch("insights.ducklake.client.psycopg")
    @mock.patch("insights.ducklake.client.is_dev_mode", return_value=True)
    def test_sql_path_executes_directly(self, _mock_dev_mode, mock_psycopg):
        ...
        assert result.insightsql is None
    ...
    def test_query_path_compiles_and_executes(self, mock_compile, _mock_dev_mode, mock_psycopg):
        ...
        query = InsightsQLQuery(query="SELECT count() FROM events")
        ...
        assert result.insightsql == "SELECT count() FROM events"
    ...
    @mock.patch("insights.ducklake.client.psycopg")
    @mock.patch("insights.ducklake.client.get_duckgres_config_for_org")
    @mock.patch("insights.ducklake.client.is_dev_mode", return_value=False)
    def test_production_path_resolves_org(self, _mock_dev_mode, mock_config_for_org, mock_psycopg):
        ...
        with mock.patch("insights.ducklake.common._get_org_id_for_team", return_value="org-456"):
