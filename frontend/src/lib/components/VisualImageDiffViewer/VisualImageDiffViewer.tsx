...
import { SegmentedButton, Slider, Switch, Tag, type TagType } from '@hanzo/elements'
...
const RESULT_TAG_TYPES: Record<VisualDiffResult, TagType> = {
    changed: 'warning',
    new: 'primary',
    removed: 'danger',
    unchanged: 'success',
            ...
            <div className="border-b bg-gradient-to-r from-bg-light via-bg-light to-bg-light/70 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                        <Tag type={RESULT_TAG_TYPES[result]}>{RESULT_LABELS[result]}</Tag>
                        {diffLabel && <Tag type="muted">{diffLabel}</Tag>}
                        {hasOverlayBoxes && (
                            <Switch
                        ...
                        {isSmallImage && (
                            <Tag type="highlight" className="font-bold">
                ...
                {supportsComparison && mode !== 'sideBySide' && mode !== 'diff' && (
                    <div className="mt-3 flex flex-wrap items-center gap-4 rounded-lg border bg-surface-primary px-3 py-2">
                        {mode === 'split' && hasBothImages && (
                            <Switch checked={flicker} onChange={setFlicker} size="small" label="Flicker" />
                        )}
                        {mode === 'blend' && (
