...
_KNOWN_FAILED_ERROR_TYPES: set[str] = {
    "ServerException",  # datastore_driver
    "NetworkError",  # datastore_driver
...
# subquery. The earlier double-reference let Datastore's analyzer inline the CTE twice and
...
# `<= PATHS_TOP_K` keeps the union of daily top-Ks. (InsightsQL parses `LIMIT n BY` but the
...
#
# The win over the plain JOIN template: instead of InsightsQL's implicit events→sessions
...
# GROUP BY, and InsightsQL globalizes the events↔sessions join automatically, so the
...
# InsightsQL read template. `{...}` placeholders are substituted via `parse_select`
...
# `execute_insightsql_query`, so `time_window_start` (stored UTC) is compared
# directly against the UTC bounds we pass in via `{cur_start}` etc. without
# InsightsQL coercing them to the team's local timezone.
#
...
# shadowing the underlying column name. With the same name, InsightsQL resolves
...
# form, and Datastore rejects the query with "not under aggregate function
...
#     when no entry sessions contributed; Datastore `ORDER BY ... NULLS LAST`
#     only catches NULL)
...
FROM (
    ...
    FROM insights.web_stats_paths_preaggregated
    WHERE and(
        team_id = {team_id},
        job_id IN {job_ids},
...
def execute_read_query(
    *,
    ...
    """Read the precomputed PATHS rows via InsightsQL.

    Returns the raw `response.results` (list of tuples) so the caller can
    materialize without depending on InsightsQL's response type. Sort, pagination,
    ...
    modifiers = runner.modifiers.model_copy() if runner.modifiers else InsightsQLQueryModifiers()
