...
``ee/scriptai/eval/`` so they run under the default pytest configuration — no
...
def _error(ts: str, message: str) -> str:
    return json.dumps(_notification(ts=ts, method="_insights/error", params={"message": message}))
