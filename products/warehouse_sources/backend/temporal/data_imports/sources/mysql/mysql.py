"""MySQL driver for Insights's data-warehouse import pipeline.
...
thin Insights-layer wrapper that just holds an instance and validates
credentials.
...
from insights.exceptions_capture import capture_exception  # noqa: F401
...
# bastion refusing Insights's IPs, or a proxy that resets the stream. sshtunnel doesn't wrap it
...
def get_connection_metadata(conn: pymysql.Connection, *, database: str) -> dict[str, Any]:
    """Connection metadata persisted on a direct-query source for the InsightsQL executor."""
    ...
    # The InsightsQL direct-query executor only branches postgres-vs-mysql on `engine`, so
