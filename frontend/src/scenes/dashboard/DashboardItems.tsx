...
import { DashboardWidgetItem } from '@hanzo/products-dashboards/frontend/components/DashboardWidgetItem/DashboardWidgetItem'
import { getDashboardWidgetFetchDisplayError } from '@hanzo/products-dashboards/frontend/widgets/constants'
...
export function DashboardItems({ showCreateAnomalyAlertButton }: DashboardItemsProps = {}): JSX.Element {
    ...
    const getInsertMenuItems = useCallback(
        (targetX: number, targetY: number, targetW?: number): MenuItem[] =>
            dashboard
                ? getAddTileMenuItems({
        ...
        () =>
            canEnterEditModeFromEdge
                ? (e: React.MouseEvent) => {
                      ...
                      // Don't trigger when clicking obvious interactive controls or readonly rich text (TipTap/Markdown).
                      if (
                          target.closest(
                              'input,textarea,button,select,a,p,h4,[contenteditable="true"],[role="textbox"],.ProseMirror,.Markdown'
                          )
                      ) {
                          return
                      }
