...
from insights.models.scoping import get_current_team_id, team_scope
...
def get_live_query_progress(recalc: ExperimentMetricsRecalculation) -> dict | None:
    """Cumulative Datastore progress for an in-flight run: in-flight queries from system.processes plus
    ...
    deterministic client_query_id `experiment_metric_recalc_{recalc_id}_{metric_uuid}`, which Datastore
    ...
    `estimated_rows_total` is Datastore's own total_rows_approx for in-flight queries (revised upward
    ...
    # system.processes and system.query_log are cluster-global tables with no Datastore-side tenant scoping:
    ...
    # the Django clock that stamped started_at and the Datastore clock behind event_time.
    ...
    try:
        with tags_context(product=Product.EXPERIMENTS, feature=Feature.CACHE_WARMUP, team_id=recalc.team_id):
            rows = sync_execute(
                ...
                """,
                {"cluster": DATASTORE_CLUSTER, "prefix": prefix, "since": int(since.timestamp())},
...
def build_job_payload(
    ...
    *,
    ...
    the poll (GET) path sets it so an in-flight run carries live Datastore progress, while the POST path does not
...
def request_recalculation(experiment: Experiment, user: User, trigger: str = "manual") -> dict:
    ...
    with team_scope(experiment.team_id, canonical=True), transaction.atomic():
        ...
        # executing would otherwise keep burning worker slots and Datastore quota alongside its replacement.
        stale_ids = list(
