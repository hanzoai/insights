...
from insights.temporal.session_replay.delete_recordings.object_storage import (
...
def test_store_session_id_chunks(session_ids, chunk_size, expected_chunks, expected_last_chunk_size):
    ...
    with patch("insights.temporal.session_replay.delete_recordings.object_storage.object_storage") as mock_os:
...
async def test_load_session_id_chunk():
    ...
    with patch("insights.temporal.session_replay.delete_recordings.object_storage._s3_client") as mock_ctx:
...
async def test_delete_session_id_chunks():
    ...
    with (
        patch("insights.temporal.session_replay.delete_recordings.object_storage._s3_client") as mock_ctx,
        patch("insights.temporal.session_replay.delete_recordings.object_storage.settings") as mock_settings,
    ):
