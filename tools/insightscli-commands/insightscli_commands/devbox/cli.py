...
Provides insightscli devbox:* commands for managing Coder-based remote dev environments.
"""
...
import click
from insightscli.manifest import get_manifest
...
_TAILNET_POLICY_URL = "https://github.com/Insights/insights-cloud-infra/blob/main/tailnet-policy.hujson"
_TAILNET_ACCESS_PREREQ = (
    "Devbox access needs your email in `group:engineering` in "
    "insights-cloud-infra/tailnet-policy.hujson.\n"
...
def _print_connection_info(name: str) -> None:
    ...
    for label, command in commands:
        click.echo(f"  {label:<8} insightscli {command}{suffix}")

    if not mutagen.sync_list(label_selector=mutagen.workspace_label_selector(name)):
        click.echo()
        click.echo(f"  Tip: run `insightscli devbox:sync{suffix}` to mirror your local checkout to this devbox.")
...
def _get_workspace_or_fail(name: str, workspaces: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    if workspace is not None:
        return workspace
    _fail("No devbox found. Run 'insightscli devbox:start' to create one.")
    raise SystemExit(1)  # unreachable; helps ty see the function doesn't fall through
...
def _render_sync_status(workspace_name: str) -> str:
    ...
    happen to share the label (shouldn't happen via insightscli, but possible via
...
def _read_legacy_keychain_token() -> str | None:
    ...
    Used during migration only. Older insightscli versions stashed the token under
...
def _resolve_local_identity_agent_for_coder() -> str | None:
    ...
    ``Host *`` / ``Host *.insights.dev`` / specific blocks all flow through.
    """
...
def maybe_configure_git_signing(
    ...
    click.echo()
    click.echo("Restart any running devbox (`insightscli devbox:restart`) to pick up the new gitconfig.")
...
def maybe_configure_region(configure_region: bool | None) -> None:
    ...
    click.echo("  This is saved locally and used as the default for `insightscli devbox:start`.")
    if existing_region:
...
def maybe_configure_dotfiles(configure_dotfiles: bool | None) -> None:
    ...
    if existing_uri:
        ...
        click.echo("  To remove dotfiles entirely, run `insightscli devbox:config:rm dotfiles`.")
    click.echo()
...
def _doctor_footer() -> None:
    ...
    click.echo()
    click.echo("  Fix setup:    insightscli devbox:setup")
...
def devbox_help() -> None:
    """Show the available `insightscli devbox:*` commands."""
    ...
    for name, help_text in commands:
        click.echo(f"  insightscli {name:<20} {help_text}")
    click.echo()
    click.echo("Run `insightscli <command> --help` for command-specific options.")
...
def maybe_configure_claude_secret(configure_claude: bool | None, *, known_secret_names: set[str] | None = None) -> None:
    ...
    if legacy_token and not secret_exists and configure_claude is not True:
        ...
        click.echo("  insightscli now stores the Claude OAuth token as a Coder user secret so that")
        ...
        if click.confirm("Migrate the existing Keychain entry now?", default=True):
            upsert_user_secret(
                ...
                description="Claude Code OAuth token (managed by insightscli)",
            )
...
def _confirm_run_setup() -> bool:
    ...
    if not sys.stdin.isatty():
        return True
    click.echo()
    click.echo("insightscli devbox:setup will check or configure:")
...
def devbox_setup(
    ...
    ensure_tailscale_connected("rerun `insightscli devbox:setup`.")
    ensure_tailscale_routes_accepted()
    ensure_coder_reachable()
    ...
    if not explicit and not _confirm_run_setup():
        click.echo("Aborted. Re-run `insightscli devbox:setup` when ready.")
        return
...
def devbox_list() -> None:
    ...
    if not workspaces:
        click.echo("No devboxes found. Run 'insightscli devbox:start' to create one.")
    else:
...
def _hint_if_positional_looks_like_username(
    ...
    if workspace and not users and not list_sharing:
        raise click.UsageError(
            f"Pass the target user with --user (e.g. `insightscli {command} --user {workspace}`).\n"
            "The positional argument selects one of YOUR workspaces. Run 'insightscli devbox:users' to find usernames."
        )
...
def devbox_share(
    ...
    if not users:
        raise click.UsageError("Specify at least one --user. Run 'insightscli devbox:users' to find usernames.")
...
def devbox_unshare(workspace: str | None, users: tuple[str, ...]) -> None:
    ...
    if not users:
        raise click.UsageError("Specify at least one --user. Run 'insightscli devbox:share --list' to see current access.")
...
def _maybe_hint_region_mismatch(name: str) -> None:
    ...
    if box_region != saved:
        click.echo(
            ...
            f"Create a {saved} devbox with `insightscli devbox:start --region {saved}`."
        )
...
@click.option(
    ...
    default=None,
    help=(
        "Bring the Insights app up in the background on every workspace start. "
...
def devbox_clone(workspace: str | None, new_label: str, yes: bool, verbose: bool) -> None:
    ...
    if status != "running":
        ...
        _fail(
            f"The source devbox must be running to clone it (status: {status}). Start it: `insightscli devbox:start{suffix}`."
        )
...
def devbox_stop(workspace: str | None, verbose: bool) -> None:
    ...
    click.echo("Stopped. Disk preserved. Run 'insightscli devbox:start' to resume.")
...
def devbox_remote_exec(workspace_name: str | None, command: tuple[str, ...]) -> None:
    ...
    interactive shell, so agents and scripts can drive a devbox remotely:
        ...
        insightscli devbox:exec -- gh auth status
        insightscli devbox:exec -n api -- bash -lc 'cd ~/insights && git status'

    Use `--` to separate insightscli's flags from the command's own flags.
    """
    ensure_runtime_ready()
    name, _ = resolve_workspace_name(workspace_name)
    if not coder_ssh_alias_configured(name):
        _fail(
            "SSH access for devboxes isn't configured. Run `insightscli devbox:setup` (it runs `coder config-ssh`), then retry."
        )
...
def devbox_task(prompt: str | None, task_name: str | None, quiet: bool, template: str) -> None:
    ...
    if server_supports_user_secrets() and not has_claude_oauth_secret():
        ...
        click.echo(
            "  Run `insightscli devbox:setup --configure-claude` (or `insightscli devbox:secret:set "
...
def devbox_secret_list() -> None:
    ...
    if not secrets:
        click.echo("No user secrets configured. Run `insightscli devbox:secret:set NAME` to add one.")
        return
...
def devbox_config_show() -> None:
    ...
    ensure_runtime_ready()
    if not _print_setup_status(_collect_setup_status()):
        click.echo("Nothing configured yet. Run `insightscli devbox:setup`.")
...
def devbox_config_rm(keys: tuple[str, ...], reset_all: bool) -> None:
    ...
    if any(fired):
        click.echo()
        click.echo("Restart any running devbox (`insightscli devbox:restart`) to pick up the resets.")
...
def devbox_status(workspace: str | None) -> None:
    ...
    ws = get_workspace(name, workspaces)
    if ws is None:
        click.echo("No devbox found. Run 'insightscli devbox:start' to create one.")
        return
    ...
    if ws.get("outdated"):
        ...
        click.echo("           Run `insightscli devbox:update` to apply it.")
...
@click.command(name="devbox:forward", help="Forward Insights UI to localhost")
@workspace_argument
@click.option("--port", default=8010, type=int, help="Local port to forward to")
def devbox_forward(workspace: str | None, port: int) -> None:
    """Forward the Insights UI port to localhost."""
    ensure_runtime_ready()
    name, _ = resolve_workspace_name(workspace)
    if not _local_port_is_available(port):
        _fail(
            f"Local port {port} is already in use.\n"
            f"Stop the process using that port or rerun with `insightscli devbox:forward --port {port + 1}`."
        )

    click.echo(f"Forwarding {name}:8010 -> localhost:{port}")
    click.echo(f"Insights UI at http://localhost:{port}")
...
def devbox_cleanup_disk(workspace: str | None, prune_docker: bool, prune_cargo: bool) -> None:
    ...
    if workspace:
        ...
        remote_cmd = ["insightscli", "devbox:cleanup:disk"]
        if prune_docker:
    ...
    tips = []
    if not prune_cargo:
        tips.append("  insightscli devbox:cleanup:disk --cargo  (rm ~/.cargo/target, forces recompile)")
    if not prune_docker:
        tips.append("  insightscli devbox:cleanup:disk --docker  (prune stopped containers)")
    if tips:
        click.echo()
