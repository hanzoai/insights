...
description: >
  Investigate the quality of Insights MCP tool calls — error rates, latency,
...
Any MCP server instrumented with Insights's MCP analytics SDK emits a
`$mcp_tool_call` event on the shared `events` table every time an agent invokes a
tool. There is **no dedicated Datastore table** — every field lives as a
...
**For a single tool, prefer the typed tools** — `insights:query-mcp-tool-stats` (calls,
errors, p50/p95, users, sessions, intents), `insights:query-mcp-tool-failures` (top error
messages by harness), and `insights:query-mcp-tool-daily-stats` (day-by-day trend). Each
...
**InsightsQL via `insights:execute-sql` is the path for cross-tool questions** — the
"which tool errors most" ranking below has no typed tool, so rank with SQL, then
drill into the worst tool with `insights:query-mcp-tool-stats` and
`insights:query-mcp-tool-failures`. The full
property schema and the canonical query recipes live in the shared MCP data
reference:
[`products/insights_ai/skills/querying-insights-data/references/models-mcp.md`](../../../insights_ai/skills/querying-insights-data/references/models-mcp.md).
...
`insights:query-mcp-tool-failures` with the `toolName` — it's the typed equivalent of the
...
`error_type`/`error_status` to `insights:query-mcp-tool-failure-occurrences`
...
- **Dashboard**: `https://app.hanzo.ai/project/<project_id>/mcp-analytics/dashboard`
- **Tool quality**: `https://app.hanzo.ai/project/<project_id>/mcp-analytics/tool-quality`
...
- Harness bucketing is resolved **server-side** by
  ...
  and `insights:query-mcp-harness-breakdown` runs it. If your hand-written SQL
