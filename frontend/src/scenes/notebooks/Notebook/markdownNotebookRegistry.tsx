...
import '../Nodes/NotebookNodeInsightsQL'
...
import { CreateInsightsWidgetNodeOptions, NotebookNodeAttributes, NotebookNodeType } from '../types'
...
export const MARKDOWN_TAG_TO_NOTEBOOK_NODE_TYPE: Partial<Record<string, NotebookNodeType>> = {
    ...
    InsightsQLSQL: NotebookNodeType.InsightsQLSQL,
...
export const MARKDOWN_NODE_DEFINITIONS: {
    ...
    { tagName: 'InsightsQLSQL', category: 'SQL', label: 'SQL (InsightsQL)' },
    ...
    { tagName: 'FeatureFlag', category: 'Insights', label: 'Feature flag' },
    { tagName: 'Survey', category: 'Insights' },
    ...
    { tagName: 'Backlink', category: 'Insights' },
    { tagName: 'ReplayTimestamp', category: 'Insights' },
    { tagName: 'PersonFeed', category: 'Data' },
    { tagName: 'PersonProperties', category: 'Data' },
    { tagName: 'GroupProperties', category: 'Data' },
    { tagName: 'TaskCreate', category: 'Insights' },
    { tagName: 'LLMTrace', category: 'Insights' },
    { tagName: 'Issues', category: 'Insights' },
    { tagName: 'UsageMetrics', category: 'Insights' },
    { tagName: 'ZendeskTickets', category: 'Insights' },
    { tagName: 'RelatedGroups', category: 'Insights' },
    { tagName: 'CustomerJourney', category: 'Insights' },
    { tagName: 'SupportTickets', category: 'Insights' },
    { tagName: 'EarlyAccessFeature', category: 'Insights', label: 'Early access feature' },
    {
...
export function getHiddenInsertCommandKeysForFeatureFlags(featureFlags: FeatureFlagsSet): string[] {
    // The built-in SQL command inserts a legacy `<Query>` InsightsQL cell, which SQLV2 replaces: it runs
...
export function getMarkdownNotebookNodeTitle(
    ...
    options: CreateInsightsWidgetNodeOptions<any> | null,
    fallback: string
): string | null {
...
export function RealNotebookNodeAttributeEdit({
    ...
    options: CreateInsightsWidgetNodeOptions<any>
}): JSX.Element {
...
export function ImageEdit({ node, updateProps }: NotebookComponentRenderProps): JSX.Element {
    ...
    const { setFilesToUpload, filesToUpload, uploading } = useUploadFiles({
        onUpload: (url, fileName) => {
            ...
            insights.capture('notebook image uploaded', { name: fileName })
        },
        onError: (detail) => {
            insights.capture('notebook image upload failed', { error: detail })
    ...
    return (
        <div className="MarkdownNotebook__component-form" ref={formRef}>
            ...
            />
            <Input value={alt} onChange={(value) => updateProps({ alt: value })} placeholder="Alt text" />
        </div>
    )
}
...
export function getDefaultProps(options: CreateInsightsWidgetNodeOptions<any> | null): NotebookComponentProps {
    return (
