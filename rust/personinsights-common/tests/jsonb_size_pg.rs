...
use personinsights_common::properties::jsonb_column_size;
use serde_json::{json, Value};
use sqlx::postgres::PgPoolOptions;
use sqlx::Row;

const DB_URL: &str = "postgres://insights:insights@localhost:5432/insights_persons";
...
async fn jsonb_column_size_matches_postgres() {
    let pool = PgPoolOptions::new()
        ...
        .connect(DB_URL)
        .await
        .expect("local insights_persons Postgres must be running (docker compose)");
