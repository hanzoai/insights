...
class ScannerResultSerializer(serializers.Serializer):
    ...
    signals_count = serializers.IntegerField(
        min_value=0,
        help_text="Number of Insights Signals emitted from this observation.",
    )
...
class CreateTaskFromObservationResponseSerializer(serializers.Serializer):
    """The Insights Task created from an observation."""

    task_id = serializers.UUIDField(
        help_text="ID of the Insights Task holding this observation's finding, created now (201) or by an earlier call (200).",
    )
...
class ReplayObservationViewSet(
    ...
    @extend_schema(
        ...
        description=(
            "Create a Insights Task from this observation's finding so it can be triaged and fixed. "
    ...
    def create_task(self, request: Request, **kwargs: Any) -> Response:
        ...
        if not has_tasks_access(user):
            raise PermissionDenied("Creating a task requires access to Insights Desktop.")
