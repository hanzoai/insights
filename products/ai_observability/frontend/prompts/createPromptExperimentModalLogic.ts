...
export interface createPromptExperimentModalLogicValues {
    canAddSlot: boolean
    canSubmit: boolean
    disabledVersionsByIndex: Set<number>[]
    isModalOpen: boolean
    isSubmitting: boolean
    promptName: string | null
    promptVersions: LLMPromptVersionSummary[]
    selectedTemplates: TemplatesEnumApi[]
    selectedVersions: number[]
    templates: ExperimentsPromptTemplatesRetrieve200Item[]
    templatesLoading: boolean
    versionSlots: VersionSlots
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface createPromptExperimentModalLogicActions {
    addVersionSlot: () => {
        value: true
    }
    closeModal: () => {
    ...
    removeVersionSlot: (index: number) => {
        index: number
    }
    setSelectedTemplates: (templates: TemplatesEnumApi[]) => {
    ...
    submitCreateFailure: (error: string) => {
        error: string
    }
    submitCreateSuccess: (experimentId: number) => {
        experimentId: number
    }
    toggleTemplate: (template: TemplatesEnumApi) => {
        template: TemplatesEnumApi
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface createPromptExperimentModalLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        selectedVersions: (versionSlots: VersionSlots) => number[]
        canSubmit: (
            selectedVersions: number[],
            selectedTemplates: TemplatesEnumApi[],
            isSubmitting: boolean,
        ...
        disabledVersionsByIndex: (versionSlots: VersionSlots) => Set<number>[]
        canAddSlot: (versionSlots: VersionSlots) => boolean
    }
}

export type createPromptExperimentModalLogicType = MakeLogicType<
    createPromptExperimentModalLogicValues,
    createPromptExperimentModalLogicActions,
    Record<string, any>,
    createPromptExperimentModalLogicMeta
>
...
export const createPromptExperimentModalLogic = kea<createPromptExperimentModalLogicType>([
    ...
    listeners(({ values, actions }) => ({
        openModal: () => {
            insights.capture('llma prompt experiment modal opened', {
        ...
        submitCreateSuccess: ({ experimentId }) => {
            insights.capture('llma prompt experiment created', {
        ...
        submitCreateFailure: ({ error }) => {
            insights.capture('llma prompt experiment creation failed', {
                prompt_name: values.promptName,
