...
from insights.temporal.ai_observability.eval_reports.emit_signal import (
...
class ScheduleAllEvalReportsWorkflow(InsightsWorkflow):
...
class CheckCountTriggeredReportsWorkflow(InsightsWorkflow):
    ...
    async def run(self, inputs: CheckCountTriggeredReportsWorkflowInputs) -> None:
        ...
        # Batched path: one check activity per team-group, each sharing one Datastore
...
async def _check_count_triggered_eval_report_candidates_batched(report_id_groups: list[list[str]]) -> list[str]:
    ...
    # a Datastore failure is contained to that group. The window keeps at most
...
class GenerateAndDeliverEvalReportWorkflow(InsightsWorkflow):
