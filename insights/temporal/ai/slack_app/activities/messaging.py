...
from insights.temporal.ai.slack_app.helpers import safe_react, swap_reaction
...
def post_insights_code_no_repos_activity(
    ...
    slack.client.chat_postMessage(
        ...
        text=(
            ...
            "Please make sure a GitHub integration is set up in your Insights project."
        ),
    )
...
@activity.defn
@close_db_connections
def post_insights_code_repo_picker_activity(
    ...
    if user_id is None:
        logger.warning(
            "insights_code_picker_legacy_call_skipped",
...
@activity.defn
@close_db_connections
def block_insights_code_task_if_no_personal_github_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs,
    channel: str,
    thread_ts: str,
    user_id: int,
    ...
    `create_insights_code_task_for_repo_activity`. Returns False to let the task proceed.

    The team-level GitHub App can still author commits, but PRs would land under the
    Insights app identity instead of the user's. Rather than degrading silently, hold
...
@activity.defn
@close_db_connections
def resolve_insights_code_authorship_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs,
    channel: str,
    thread_ts: str,
    slack_user_id: str,
    ...
    if is_slack_app_bot_prs_enabled(team) and team_has_github:
        if has_personal_github:
            ...
            text = (
                "You have no personal integration setup yet. The PR will be authored by the Insights bot.\n"
        ...
        slack.client.chat_postMessage(
                    ...
                    "elements": [
                        {
                            ...
                            "text": {"type": "plain_text", "text": "Continue as Insights", "emoji": True},
                            "value": json.dumps(
                                {
                                    "workflow_id": workflow_id,
            ...
            metadata={"event_type": "insights_code_authorship", "event_payload": {"workflow_id": workflow_id}},
        )
        logger.info(
...
@activity.defn
@close_db_connections
def post_insights_code_authorship_timeout_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str
) -> None:
    ...
    slack.client.chat_postMessage(
        channel=channel,
        thread_ts=thread_ts,
        text="I didn't hear back, so I haven't started the task. Mention Insights again to retry.",
    )


@activity.defn
@close_db_connections
def post_insights_code_picker_timeout_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str
) -> None:
...
@activity.defn
@close_db_connections
def post_insights_code_internal_error_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str
) -> None:
    from products.slack_app.backend.api import _clear_pending_repo_picker
