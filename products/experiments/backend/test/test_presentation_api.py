...
class TestExperimentCRUD(_HoistFlagConfigClientMixin, APILicensedTest):
    ...
    def test_list_query_defers_heavy_metric_columns(self) -> None:
        ...
        # be SELECTed from insights_experiment. metrics/metrics_secondary are the exception: the
        ...
        experiment_selects = [
            query["sql"] for query in ctx.captured_queries if 'FROM "insights_experiment"' in query["sql"]
        ]
        self.assertTrue(experiment_selects, "expected at least one SELECT against insights_experiment")
        ...
        for sql in experiment_selects:
            for column in LIST_DEFERRED_FIELDS:
                ...
                self.assertNotIn(f'"insights_experiment"."{column}"', sql)
    ...
    @patch("products.experiments.backend.experiment_service.hanzo_insights.feature_enabled", return_value=False)
    ...
    @patch("products.experiments.backend.experiment_service.report_user_action")
    @patch("products.experiments.backend.experiment_service.hanzo_insights.feature_enabled", return_value=True)
...
class TestExperimentAuxiliaryEndpoints(_HoistFlagConfigClientMixin, DatastoreTestMixin, APILicensedTest):
                ...
                "person2-not-in-prop": [
                    {
                        ...
                        "timestamp": datetime.now() - timedelta(days=2),
                        "properties": {
                            "$current_url": "https://hanzo.ai/feedback/1234"
    ...
    def test_experiment_activity_logging_shows_correct_user_for_updates(self):
        ...
        second_user = User.objects.create_user(
            email="second@hanzo.ai", password="testpass123", first_name="Second", last_name="User"
        )
...
class TestExperimentApiExposureCriteriaParity(unittest.TestCase):
    ...
    def test_api_schema_exposes_every_runtime_field(self) -> None:
        from insights.schema import ExperimentApiExposureCriteria, ExperimentExposureCriteria
        ...
        self.assertFalse(
            ...
            "type in frontend/src/queries/schema/schema-general.ts and rerun insightscli build:schema.",
        )
