...
use personinsights_common::properties::{
...
impl PersonHogLeaderService {
    ...
    fn validate_partition(
        ...
        if partition != expected {
            counter!("personinsights_leader_partition_mismatch_total").increment(1);
    ...
    fn record_cache_hit() {
        counter!(
            "personinsights_leader_person_loads_total",
    ...
    async fn recover_or_load(
        ...
        histogram!("personinsights_leader_person_load_duration_ms", "source" => "changelog")
    ...
    async fn load_from_pg(
        ...
        histogram!("personinsights_leader_person_load_duration_ms", "source" => "pg")
            .record(started.elapsed().as_secs_f64() * 1000.0);
        match result {
            Ok(Some(person)) => {
                counter!(
            ...
            Err(e) => {
                ...
                .increment(1);
                counter!("personinsights_leader_pg_fallback_errors_total").increment(1);
                tracing::error!(
                    team_id = key.team_id,
                    person_id = key.person_id,
                    error = %e,
...
impl PersonHogLeader for PersonHogLeaderService {
    ...
    async fn update_person_properties(
        ...
        if input_stats.nul_strings > 0 {
            counter!("personinsights_leader_properties_nul_sanitized_total")
                .increment(input_stats.nul_strings);
        }
        if input_stats.clamped_numbers > 0 {
            counter!("personinsights_leader_properties_numbers_clamped_total")
        ...
        let _guard = mutex.lock().await;
        histogram!("personinsights_leader_person_lock_wait_ms")
        ...
        if !self.dirty_index.can_admit(&cache_key) {
            counter!("personinsights_leader_writes_shed_total", "reason" => "dirty_index_full")
                .increment(1);
        ...
        if !updates.has_changes {
            counter!("personinsights_leader_updates_total", "outcome" => "no_change").increment(1);
            ...
            if existing_size >= self.size_limits.threshold {
                match trim_properties_to_fit_size(&person.properties, self.size_limits.trim_target)
                {
                    TrimResult::Trimmed(trimmed) => {
                        counter!("personinsights_leader_properties_trimmed_total").increment(1);
                    ...
                    TrimResult::CannotFit => {
                        counter!(
                            "personinsights_leader_updates_total",
                ...
                counter!("personinsights_leader_updates_total", "outcome" => "rejected_oversized")
                    .increment(1);
                self.warnings.emit(&SizeViolationWarning {
                    team_id: cache_key.team_id,
                    person_uuid: person.uuid.clone(),
        ...
        if let Err(reason) = assert_writeable(&updated_person) {
            counter!("personinsights_leader_unwriteable_state_total").increment(1);
            tracing::error!(
                team_id = cache_key.team_id,
        ...
        let offset = if let Some(fenced) = &self.fenced {
            match fenced.produce(partition, &proto).await {
                ...
                Err(e @ FencedProduceError::FencedUncertain(_)) => {
                    ...
                    counter!(
                        "personinsights_leader_indeterminate_outcomes_total",
                        "fenced" => "true"
                    )
                    .increment(1);
                    tracing::error!(
        ...
        // left to say.
        emitted.resolved();
        counter!("personinsights_leader_updates_total", "outcome" => "updated").increment(1);
...
mod tests {
    ...
    async fn make_test_service() -> PersonHogLeaderService {
        ...
        PersonHogLeaderService::new(
            ...
            producer.clone(),
            "personinsights_updates".to_string(),
            None,
            Arc::new(DashMap::new()),
            ...
            Arc::new(
                ChangelogRecovery::new(RecoveryConfig {
                    kafka,
                    topic: "personinsights_updates".to_string(),
            ...
            WarningsProducer::new(producer, "datastore_ingestion_warnings".to_string()),
            None,
            None,
