...
from insights.insightsql_queries.insights.funnels.funnel_correlation_query_runner import (
    EventContingencyTable,
    EventStats,
    FunnelCorrelationQueryRunner,
)
from insights.insightsql_queries.insights.funnels.test.test_funnel_correlation_actors import get_actors
...
class TestDatastoreFunnelCorrelation(DatastoreTestMixin, APIBaseTest):
    ...
    def test_funnel_correlation_with_events_and_insightsql_aggregation(self):
        # When the funnel aggregates by a custom InsightsQL expression, the correlation
    ...
    def test_funnel_correlation_insightsql_aggregation_rejects_injection(self):
        # funnelAggregateByInsightsQL is user-controlled — a value that isn't a single
        # bounded expression (e.g. an attempt to inject an extra JOIN) must be
        # rejected by the parser, not interpolated into the query.
        query = FunnelsQuery(
            series=[EventsNode(event="user signed up"), EventsNode(event="paid")],
            dateRange=DateRange(date_from="2020-01-01", date_to="2020-01-14"),
            funnelsFilter=FunnelsFilter(funnelAggregateByInsightsQL="properties.session_id JOIN persons ON 1=1"),
        )
        with self.assertRaises(Exception):
            self._get_events_for_query(query)

    def test_funnel_correlation_with_properties_and_insightsql_aggregation(self):
        # Property correlation joins funnel_actors onto the persons table on actor_id. When
        # the funnel aggregates by a InsightsQL expression, actor_id holds that value (a string)
