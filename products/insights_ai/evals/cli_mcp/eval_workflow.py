"""Eval cases for the ``insights:exec`` cli workflow conventions.
...
Every case exercises the ``insights:exec`` tool, which the MCP server now
registers by default.

To run a single eval:
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_typo_recovery"
"""
...
from products.insights_ai.evals.cli_mcp.scorers import (
    CalledTargetTool,
    DrilledIntoSchema,
    InfoBeforeCall,
