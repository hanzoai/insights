...
- To update a prior deliverable, GET the returned artifact id or POST new `content`, `content_base64`, or source artifact fields to `$INSIGHTS_API_URL/api/projects/$INSIGHTS_PROJECT_ID/tasks/$INSIGHTS_TASK_ID/runs/$INSIGHTS_TASK_RUN_ID/living_artifacts/<artifact_id>/edit/`.
...
- For Slack deliverables, create a living artifact before claiming delivery. POST to `$INSIGHTS_API_URL/api/projects/$INSIGHTS_PROJECT_ID/tasks/$INSIGHTS_TASK_ID/runs/$INSIGHTS_TASK_RUN_ID/living_artifacts/` with `$INSIGHTS_PERSONAL_API_KEY` using adapter `slack_message`. To update a prior deliverable, GET the returned artifact id or POST new `content` to `$INSIGHTS_API_URL/api/projects/$INSIGHTS_PROJECT_ID/tasks/$INSIGHTS_TASK_ID/runs/$INSIGHTS_TASK_RUN_ID/living_artifacts/<artifact_id>/edit/`.
- Run artifacts that are not your uploaded outputs (plans, context, tree snapshots, checkpoints, user uploads) are internal: never deliver them to Slack or mention them in your reply.
