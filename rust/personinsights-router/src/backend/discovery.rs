    ...
    async fn sync_active(
        ...
        for addr in desired.difference(active_addrs) {
            ...
            if self.tx.send(Change::Insert(*addr, endpoint)).await.is_ok() {
                metrics::counter!("personinsights_router_discovery_updates_total", "action" => "insert")
                    .increment(1);
            }
        }
        ...
        for addr in active_addrs.difference(desired) {
            if self.tx.send(Change::Remove(*addr)).await.is_ok() {
                metrics::counter!("personinsights_router_discovery_updates_total", "action" => "remove")
                    .increment(1);
            }
        }
        ...
        *active_addrs = desired.clone();
        ...
        metrics::gauge!("personinsights_router_discovery_endpoints_active").set(count as f64);
