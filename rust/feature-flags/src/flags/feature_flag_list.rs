    ...
    pub async fn from_pg(
        ...
        let query = r#"
            SELECT f.id,
                  ...
                  EXISTS (
                      SELECT 1 FROM insights_experiment e
                      WHERE e.feature_flag_id = f.id AND e.deleted = false
                  ) AS has_experiment
              FROM insights_featureflag AS f
              JOIN insights_team AS t ON (f.team_id = t.id)
              LEFT JOIN insights_featureflagevaluationcontext AS ec ON (f.id = ec.feature_flag_id)
              LEFT JOIN insights_evaluationcontext AS ctx ON (ec.evaluation_context_id = ctx.id)
            WHERE t.id = $1
              AND f.deleted = false
...
mod tests {
    ...
    ///   - insights/api/feature_flag.py (MinimalFeatureFlagSerializer)
