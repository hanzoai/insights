...
const endpointCreate = (): ToolBase<typeof EndpointCreateSchema, WithInsightsUrl<Schemas.EndpointResponse>> => ({
...
const endpointGet = (): ToolBase<typeof EndpointGetSchema, WithInsightsUrl<Schemas.EndpointVersionResponse>> => ({
    name: 'endpoint-get',
...
const endpointLogs = (): ToolBase<typeof EndpointLogsSchema, unknown> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof EndpointLogsSchema>) => {
        ...
        return await withInsightsUrl(context, result, '/endpoints')
    },
})
...
const endpointMaterializationStatus = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.EndpointMaterialization>
> => ({
...
const endpointMaterializationSuggestion = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.EndpointMaterializationSuggestion>
> => ({
...
const EndpointRunSchema = EndpointsRunCreateParams.omit({ project_id: true })
    ...
    .extend({
        variables: EndpointsRunCreateBody.shape['variables'].describe(
            'Key-value pairs to parameterize the query. For InsightsQL endpoints, keys match variable code_name (e.g. {"event_name": "$pageview"}). For insight endpoints with breakdowns, use the breakdown property name as key.'
        ),
    })

const endpointRun = (): ToolBase<typeof EndpointRunSchema, WithInsightsUrl<Schemas.EndpointRunResponse>> => ({
    name: 'endpoint-run',
...
const endpointUpdate = (): ToolBase<typeof EndpointUpdateSchema, WithInsightsUrl<Schemas.EndpointResponse>> => ({
...
const endpointVersions = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedEndpointVersionResponseList>
> => ({
        ...
        return await withInsightsUrl(
            context,
            {
                ...
                results: await Promise.all(
                    (result.results ?? []).map((item) => withInsightsUrl(context, item, `/endpoints/${item.name}`))
                ),
            },
            '/endpoints'
        )
...
const endpointsGetAll = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedEndpointResponseList>
> => ({
