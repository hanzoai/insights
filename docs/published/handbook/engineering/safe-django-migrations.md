...
For `Team`, most new fields shouldn't be on the table at all. Domain-specific fields belong on a **Team extension model** (see `insights/models/team/README.md`) — that's a `CREATE TABLE`, which takes no lock on `insights_team` whatsoever.
...
2. Add `<app_label>.<migration_name>` to `insights/management/migration_analysis/hot_table_acknowledged_migrations.txt` — this is the explicit "I accept the risk" act, and it's visible in review.
3. Coordinate the deploy with #team-infrastructure for a low-traffic window.

### Foreign Keys to Hot Tables

**Problem:** The hazard above also fires from the _referencing_ side, and from **any** app — a plain `CreateModel` or `AddField` in a product app with a `ForeignKey(to="insights.team", ...)` (or `to=settings.AUTH_USER_MODEL`, which resolves to `insights_user`). Creating the FK constraint takes a `SHARE ROW EXCLUSIVE` lock on the _referenced parent_ table, even though the child table is brand new. That lock conflicts with the `ROW EXCLUSIVE` lock every `INSERT`/`UPDATE`/`DELETE` on the parent holds, so under write traffic the lock request queues behind in-flight writes, `lock_timeout` cancels it, and each `bin/migrate` retry repeats the stall. A `CreateModel` with an FK to `insights_team` has blocked a deploy this way.
...
from insights.migration_helpers import ValidateForeignKey
...
from insights.migration_helpers import CreateIndexConcurrently
...
- **Prefer `SafeAddIndexConcurrently` / `SafeRemoveIndexConcurrently` from `insights.migration_helpers`** — they take a `model_name` + Index, track Django state themselves (no `SeparateDatabaseAndState`), disable both timeouts, and recover from invalid leftover indexes
...
Add CHECK constraints in two phases — add without validation, then validate separately — using the `insights.migration_helpers` helpers.
...
from insights.migration_helpers import AddConstraintNotValid
...
from insights.migration_helpers import ValidateConstraint
