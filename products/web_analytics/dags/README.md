...
Each shard runs as its own subprocess under the multiprocess executor. InsightsQL
compilation is CPU-bound Python, so parallelism comes from shard processes, not
threads; threads inside a shard overlap the IO-bound parts (Datastore reads
...
| `WEB_ANALYTICS_WARMING_SHARD_THREADS`         | 6       | Worker threads inside each shard (clamp 1-64). Total Datastore-side concurrency ≈ shards × this.                                                                  |
...
- Prometheus: `insights_web_analytics_warming_queries_total{outcome=...}`,
