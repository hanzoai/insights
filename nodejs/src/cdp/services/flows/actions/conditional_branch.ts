...
import { findContinueAction, findNextAction, isEvaluableCondition } from '../flow-utils'
...
export class ConditionalBranchHandler implements ActionHandler {
    async execute({
        ...
        Extract<FlowAction, { type: 'conditional_branch' | 'wait_until_condition' }>
    >): Promise<ActionHandlerResult> {
        ...
        if (action.type === 'wait_until_condition' && invocation.state?.currentAction?.eventMatched === true) {
            ...
            return {
                nextAction: findNextAction(invocation.flow, action.id, 0),
...
export async function checkConditions(
    ...
    action: Extract<FlowAction, { type: 'conditional_branch' }>
): Promise<{
    ...
    for (const [index, condition] of action.config.conditions.entries()) {
        ...
        if (filterResults.match) {
            return {
                nextAction: findNextAction(invocation.flow, action.id, index),
            }
        }
    }
