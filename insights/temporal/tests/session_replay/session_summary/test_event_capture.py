...
from insights.temporal.session_replay.session_summary.activities.video_based.a7c_store_video_session_summary import (
    store_video_session_summary_activity,
)
from insights.temporal.session_replay.session_summary.types.video import (
...
def test_capture_session_summary_ready_emits_internal_project_event(
    ...
    mock_capture_internal = mocker.patch(
        "insights.temporal.session_replay.session_summary.event_capture.capture_internal", return_value=result
    )

    from insights.temporal.session_replay.session_summary.event_capture import capture_session_summary_ready
...
def test_capture_session_summary_ready_swallow_capture_errors(
    ...
    mocker.patch("insights.temporal.session_replay.session_summary.event_capture.capture_internal", return_value=result)
    logger = mocker.patch("insights.temporal.session_replay.session_summary.event_capture.logger")
...
async def test_store_video_session_summary_activity_emits_summary_ready_event(
    ...
    mocker.patch(
        "insights.temporal.session_replay.session_summary.activities.video_based.a7c_store_video_session_summary.get_redis_state_client",
        return_value=(None, "input-key", None),
    )
    mocker.patch(
        "insights.temporal.session_replay.session_summary.activities.video_based.a7c_store_video_session_summary.get_data_class_from_redis",
        return_value=llm_input,
    )
    mocker.patch(
        "insights.temporal.session_replay.session_summary.activities.video_based.a7c_store_video_session_summary._convert_video_segments_to_session_summary",
        return_value=mock_session_summary_serializer.data,
    )

    capture_session_summary_ready = mocker.patch(
        "insights.temporal.session_replay.session_summary.activities.video_based.a7c_store_video_session_summary.capture_session_summary_ready"
    )
...
async def test_store_video_session_summary_activity_does_not_emit_existing_summary_ready_event(
    ...
    logger = mocker.patch(
        "insights.temporal.session_replay.session_summary.activities.video_based.a7c_store_video_session_summary.logger"
    )
