...
from insights.temporal.proxy_service.common import SendProxyCreatedEmailInputs, activity_send_proxy_created_email
...
class TestSendProxyCreatedEmail:
    def test_sends_email_when_record_and_user_exist(self):
        ...
        with (
            ...
            patch("insights.email.EmailMessage") as MockEmailMessage,
        ):
    ...
    def test_does_not_send_when_email_unavailable(self):
        ...
        with patch("insights.email.is_email_available", return_value=False):
            with patch("insights.email.EmailMessage") as MockEmailMessage:
                activity_send_proxy_created_email(inputs)
                MockEmailMessage.assert_not_called()
    ...
    def test_swallows_exceptions_without_raising(self):
        ...
        with patch("insights.email.is_email_available", side_effect=Exception("boom")):
            # Should not raise
            activity_send_proxy_created_email(inputs)
