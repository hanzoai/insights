...
from insights.temporal.ai_observability.evaluation_clustering.models import (
    SamplerActivityInputs,
    SamplerWorkflowInputs,
    SamplerWorkflowResult,
)
from insights.temporal.ai_observability.evaluation_clustering.sampling import sample_and_embed_for_job_activity
...
class AIObservabilityEvaluationSamplerWorkflow(InsightsWorkflow):
    """Hourly per-(team, eval ClusteringJob) sampler.

    Computes the sample window deterministically from workflow time and dispatches
    a single activity that runs the InsightsQL query and enqueues embeddings.
    """
...
class AIObservabilityEvaluationClusteringWorkflow(InsightsWorkflow):
    ...
    async def run(self, inputs: SamplerWorkflowInputs) -> SamplerWorkflowResult:
        ...
        from insights.temporal.ai_observability.evaluation_clustering.activities import (
            METADATA_LOOKBACK,
