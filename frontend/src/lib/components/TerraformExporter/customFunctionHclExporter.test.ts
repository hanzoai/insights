import {
    generateCustomFunctionHCL,
    stripFiltersServerFields,
    stripInputsServerFields,
    stripMappingsServerFields,
} from 'lib/components/TerraformExporter/customFunctionHclExporter'

import { CyclotronJobFiltersType, CyclotronJobInputType, CustomFunctionMappingType, CustomFunctionType } from '~/types'

const createTestCustomFunction = (props: Record<string, unknown>): Partial<CustomFunctionType> =>
    props as Partial<CustomFunctionType>

describe('customFunctionHclExporter test', () => {
    describe('sanitizes correctly', () => {
        const testCases = [
            ...
            { input: '', expected: 'custom_function_new' },
        ]

        it.each(testCases)('converts "$input" to "$expected"', ({ input, expected }) => {
            const customFunction = createTestCustomFunction({
                name: input,
                type: 'internal_destination',
            })

            const hcl = generateCustomFunctionHCL(customFunction).hcl

            expect(hcl).toContain(`resource "posthog_custom_function" "${expected}"`)
        })
    })
    ...
    describe('generates valid hcl', () => {
        it('generates valid HCL for a complete custom function', () => {
            ...
            const result = generateCustomFunctionHCL(customFunction)
            const hcl = result.hcl

            expect(hcl).toContain('resource "posthog_custom_function" "my_test_function"')
    ...
    "properties": [
      {
        ...
        it('includes import block for saved custom functions by default', () => {
            ...
            const result = generateCustomFunctionHCL(customFunction, { projectId: 1 })
            const hcl = result.hcl

            expect(hcl).toContain('import {')
            expect(hcl).toContain('to = posthog_custom_function.saved_function')
