...
Lifecycle data the snapshots can't hold (PR state transitions, reviews/approvals, deploys, DORA) needs immutable timestamped events (GitHub webhooks → Insights events, PR as group type). That is the only thing the deferred events destination is for. See README → "The data boundary".
