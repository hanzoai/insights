...
# Consumed by `insights/temporal/common/worker.py` to override Prometheus default
...
LOGS_ALERTING_LATENCY_HISTOGRAM_METRICS = (
    ...
    "logs_alerting_datastore_duration_ms",
...
def record_datastore_duration(duration_ms: int) -> None:
    _record_histogram(
        "logs_alerting_datastore_duration_ms",
        "Datastore query wall time for a single alert evaluation",
        duration_ms,
    )
...
def record_cohort_size(size: int) -> None:
    ...
    hist = meter.create_histogram(
        ...
        description="Number of alerts in a cohort sharing one batched Datastore query and one bulk Postgres save",
        unit="alerts",
    )
    hist.record(size)
...
# TODO: Extract ExecutionTimeRecorder to insights/temporal/common/ — copied from
# insights/temporal/ai_observability/metrics.py to avoid cross-product import.
