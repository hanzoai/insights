...
- For Datastore identifiers, use `escape_datastore_identifier()` from `insights/insightsql/escape_sql.py`
...
For typed objects, use a schema-validated (de)serializer that constructs only declared types — Pydantic `model.model_dump_json()` / `Model.model_validate_json(...)` is the house default.
...
# insights/storage/auth_token_cache_verifier.py -> _JsonOnlyUnpickler
...
# insights/insightsql/database/database.py -> _CatalogUnpickler
...
1. `insightsql-injection-taint` - Flags user data (`self.query.*`, `self.$obj.$field`, etc.) interpolated into f-strings passed to parse functions (HIGH confidence)
...
- If user data is interpolated into an f-string InsightsQL template → wrap with `ast.Constant()` or `ast.Field(chain=[...])` in placeholders
