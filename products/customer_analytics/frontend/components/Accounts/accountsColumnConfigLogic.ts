...
export const ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT: string[] = [
...
// `table.to_printed_insightsql()` which, for system PostgresTables, returns the
...
// Custom property definition ids are UUIDs, which aren't valid InsightsQL identifiers (hyphens).
...
export function buildAccountColumnGroups(
    ...
    if (accountsTable) {
        for (const field of Object.values(accountsTable.fields)) {
            ...
            directOptions.push({
                name: field.name,
                expression: field.insightsql_value || field.name,
                type: field.type,
            })
        }
    }
    ...
    reducers({
        selectColumns: [
            [...ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT],
            {
                ...
                resetColumns: () => [...ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT],
