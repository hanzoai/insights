...
def emit_observation_signal_activity(inputs: EmitObservationSignalInputs) -> int:
    """Emit the observation's side-mission findings as Insights Signals; fails soft, returns the emitted count."""
    try:
        observation = (
        ...
        # `ScannerLlmInputs`) rather than re-querying Datastore here. Enrichment is best-effort: a Redis
