...
replica. The DAG tests (``insights/dags/tests/test_data_deletion_requests.py``) cover the delete flow only over an
unmaterialized property, so the materialized-column case is tested here.

This file checks two things:

1. ``compile_insightsql_predicate`` over ``properties.$browser = 'Chrome'`` with ``$browser`` materialized produces an
...
from insights.models.property import TableColumn
...
class TestWithinNonHogqlDelete(DatastoreTestMixin, APIBaseTest):
    ...
    def _materialized_column_name(self, table: str, prop: str, table_column: str = "properties") -> str:
        from ee.datastore.materialized_columns.columns import get_materialized_columns  # noqa: PLC0415
    ...
    def test_compiled_predicate_is_unqualified_and_mutation_safe_with_materialized_column(self) -> None:
        ...
        with materialized("events", "$browser", is_nullable=False):
            ...
            assert params == {"insightsql_val_0": "Chrome"}, params

    def test_within_non_insightsql_predicate_stays_unqualified_with_materialized_column(self) -> None:
        # within_non_insightsql_query queries run through lowering + the physical pass like any other: the synthetic
