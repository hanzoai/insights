...
function CadencePicker({ accountId }: { accountId: string }): JSX.Element {
    ...
    return (
        <Select<SlackSummaryCadenceEnumApi | null>
            size="small"
    ...
    return (
        <div className="border rounded bg-surface-primary">
            <div
                ...
                onClick={onToggle}
            >
                <Tag type="default">{summary.cadence}</Tag>
            ...
            {expanded && (
                <Markdown lowKeyHeadings disableImages disableDocsRedirect className="text-sm px-4 pt-3 pb-4">
                    {summary.content}
                </Markdown>
            )}
        </div>
