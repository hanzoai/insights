 ...
 * SDK grid for Logs (which uses OpenTelemetry, not the Insights JS wizard).
 */
export const OnboardingInstallStep: OnboardingStepComponentType<OnboardingInstallStepProps> = ({
    ...
    // Both gates required: the flag controls enrollment (targeted in Insights),
    ...
    useEffect(() => {
        ...
        insights.capture('mobile install handoff link opened')
    ...
    // Logs: bare SDK grid — OpenTelemetry, not the Insights JS wizard.
    return (
        <OnboardingStep
            title={installTitle}
