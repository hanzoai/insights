...
describe.each([
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, team, kafkaProducer, token }) => {
            ...
            await waitForExpect(async () => {
                const events = await fetchEvents(datastore, team.id)
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, infra, team, kafkaProducer, token }) => {
            ...
            // Every creation's Datastore message made it to the groups table.
            await waitForExpect(async () => {
                const chGroups = (await datastore.fetchDatastoreGroups()).filter(
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, team, token }) => {
            ...
            const personIdentifier = 'test@hanzo.ai'
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, team, kafkaProducer, token }) => {
            ...
            await waitForExpect(async () => {
                const ingestionWarnings = await fetchIngestionWarnings(datastore, team.id)
    ...
    //         const personsDatastore = await fetchPersons(team.id)
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, team, kafkaProducer, token }) => {
            ...
            await waitForExpect(async () => {
                const warnings = await fetchIngestionWarnings(datastore, team.id)
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, infra, team, kafkaProducer, token }) => {
            ...
            await waitForExpect(async () => {
                ...
                const allEvents = await fetchEvents(datastore, team.id)
                const eventsByPersonId = new Map<string, string[]>()
    ...
    testWithTeamIngester(
        ...
        async ({ ingester, team, kafkaProducer, token }) => {
            ...
            await waitForExpect(async () => {
                const events = await fetchEvents(datastore, team.id)
                expect(events.length).toBe(1)
                // elements_chain is parsed back into element objects by parseRawDatastoreEvent
