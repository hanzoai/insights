...
from insights.clickhouse.client.execute_async import QueryNotFoundError, enqueue_process_query_task, get_query_status
from insights.clickhouse.client.limit import (
...
from insights.clickhouse.query_tagging import get_query_tag_value, tag_queries
from insights.event_usage import groups
from insights.exceptions_capture import capture_exception
from insights.insightsql_queries.query_cache import count_query_cache_hit
from insights.insightsql_queries.query_cache_base import QueryCacheManagerBase
from insights.insightsql_queries.query_cache_factory import get_query_cache_manager
...
def get_query_runner(
    ...
    if kind == "InsightsQLQuery" or kind == "InsightsQLASTQuery":
        from .insightsql_query_runner import InsightsQLQueryRunner

        return InsightsQLQueryRunner(
            query=cast(InsightsQLQuery | InsightsQLASTQuery | dict[str, Any], query),
            team=team,
            timings=timings,
            limit_context=limit_context,
            modifiers=modifiers,
    ...
    if kind == "RevenueAnalyticsGrossRevenueQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_analytics_gross_revenue_query_runner import (
    ...
    if kind == "RevenueAnalyticsMetricsQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_analytics_metrics_query_runner import (
    ...
    if kind == "RevenueAnalyticsMRRQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_analytics_mrr_query_runner import (
    ...
    if kind == "RevenueAnalyticsOverviewQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_analytics_overview_query_runner import (
    ...
    if kind == "RevenueAnalyticsTopCustomersQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_analytics_top_customers_query_runner import (
    ...
    if kind == "RevenueExampleEventsQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_example_events_query_runner import (
    ...
    if kind == "RevenueExampleDataWarehouseTablesQuery":
        from products.revenue_analytics.backend.insightsql_queries.revenue_example_data_warehouse_tables_query_runner import (
    ...
    if kind == "ErrorTrackingSimilarIssuesQuery":
        from products.error_tracking.backend.insightsql_queries.error_tracking_similar_issues_query_runner import (
...
class QueryRunner(ABC, Generic[Q, R, CR]):
    ...
    def get_api_queries_concurrency_limit(self):
        ...
        """
        if not settings.API_QUERIES_ENABLED:
            return None
    ...
    def apply_dashboard_filters(self, dashboard_filter: DashboardFilter):
        ...
        # Notable exception: `InsightsQLQuery`, which has `properties` and `dateRange` within `InsightsQLFilters`
...
class QueryRunnerWithInsightsQLContext(AnalyticsQueryRunner[AR]):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.insightsql_context = InsightsQLContext(team_id=self.team.pk, database=self.database)
