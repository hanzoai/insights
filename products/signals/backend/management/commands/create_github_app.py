...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="App name (must be globally unique on GitHub). Defaults to 'Insights Signals Dev <random>'.",
        )
        parser.add_argument(
            "--base-url",
            type=str,
            default=DEFAULT_BASE_URL,
            help=f"Insights base URL for the homepage/callback/setup URLs (default {DEFAULT_BASE_URL}).",
        )
        parser.add_argument(
            "--port",
            type=int,
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        name: str = options["name"] or f"Insights Signals Dev {secrets.token_hex(3)}"
    ...
    def _save_and_report(self, data: dict[str, Any], *, env_path: Path, base_url: str, verify: bool) -> None:
        ...
        self.stdout.write(f"     (or via Insights: {base_url.rstrip('/')}/project/1/integrations/github)")
...
def _build_manifest(*, name: str, base_url: str, redirect_url: str) -> dict[str, Any]:
    ...
    return {
        "name": name,
        "url": f"{base}/",
        "description": "Insights local dev GitHub App (Signals / background agents).",
        "public": False,
