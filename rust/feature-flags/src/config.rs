...
pub struct Config {
    ...
    // Personal-API-key validation backend for /api/feature_flag/local_evaluation.
    // "postgres" (default, upstream behavior) validates against the Insights
    // personalapikey table. "iam" validates against Hanzo IAM via RFC 7662 token
    // introspection at {hanzo_iam_url}/v1/iam/oauth/introspect. Project (secret)
    // tokens are unaffected — they stay Redis-cached team lookups either way.
    #[envconfig(default = "postgres")]
    pub personal_api_key_backend: String,

    // Base URL of Hanzo IAM (e.g. https://iam.hanzo.ai). Required when
    // personal_api_key_backend = "iam".
    #[envconfig(default = "")]
