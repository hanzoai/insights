...
def push_hypercache_stats_metrics(
    ...
    gauges = [
        ("insights_hypercache_coverage_percent", "Percentage of teams with cached data", coverage_percent),
        ("insights_hypercache_entries_total", "Total number of entries in the HyperCache", entries_total),
        (
            "insights_hypercache_expiry_tracked_total",
            "Number of entries tracked in the expiry sorted set",
            expiry_tracked_total,
        ),
        ("insights_hypercache_size_bytes", "Estimated total cache size in bytes", size_bytes),
    ]
...
class HyperCacheManagementConfig:
    ...
    # make `SELECT *` raise UndefinedColumn (the replica lags on insights_team DDL).
    ...
    # to veto caching an emptied group_type_mapping over populated data when personinsights
