...
// Helpers below build the prompts that the "Ask Insights AI" entry points send to the AI side panel.
// Insights AI receives a snapshot of the issue(s) embedded in the prompt — it does not re-run the checks.
...
/** Preset questions surfaced as a dropdown next to the overview-level "Ask Insights AI" button. */
...
// active health issues, and listing every one would waste — or exceed — Insights AI's token budget. The
...
/** Build the prompt for asking Insights AI about a single health issue. */
export const buildHealthIssuePrompt = (issue: HealthIssue): string => {
    const category = categoryLabel(issue.kind)
    const lines = [
        'I need help with a health issue in my Insights project.',
        '',
...
/** Build the prompt for asking Insights AI about every active issue on the Health overview. */
export const buildHealthOverviewPrompt = (
    ...
    if (issues.length === 0) {
        return [
            question,
            '',
            'The Health overview for my Insights project currently shows no active health issues. ' +
    ...
    const lines = [
        question,
        '',
        `Here ${issuesClause} on my Insights Health overview (${summarizeSeverityCounts(issues)})${
