...
from insights.insightsql.errors import QueryError, ResolutionError, SyntaxError


def lookup_field_by_name(
    scope: ast.SelectQueryType | ast.SelectSetQueryType, name: str, context: InsightsQLContext
) -> Optional[ast.Type]:
...
def _table_source_name(table: ast.TableOrSelectType, context: InsightsQLContext, *, alias: str | None = None) -> str:
    if alias:
        return alias
    ...
    if isinstance(table, ast.TableType | ast.LazyTableType):
        try:
            return table.resolve_database_table(context).to_printed_insightsql()
        except Exception:
            return "source"
...
def _names_on_table_type(table_type: ast.Type, context: InsightsQLContext) -> set[str]:
...
def collect_available_field_names(
    scope: ast.SelectQueryType | ast.SelectSetQueryType, context: InsightsQLContext
) -> set[str]:
...
def ast_to_query_node(expr: ast.Expr | ast.InsightsQLXTag):
    ...
    elif isinstance(expr, ast.InsightsQLXTag):
        # Deferred: insights.schema stays off django.setup(); this module loads there via
        # insightsql.ast, which the warehouse/data-modeling models import.
        from insights import schema  # noqa: PLC0415
...
def expand_insightsqlx_query(node: ast.InsightsQLXTag, team_id: Optional[int]):
