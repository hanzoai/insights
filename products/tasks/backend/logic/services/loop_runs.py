...
from insights.temporal.oauth import InsightsMcpScopes, resolve_scopes
...
def render_loop_run_message(loop_instructions: str, execution_context: str) -> str:
    # Insights Code strips this established wrapper from user-message bubbles while still sending
...
# file_system scopes only, added on top of whatever insights_mcp_scopes the loop already carries,
...
def render_context_target_block(context_target: dict | None) -> str:
    ...
    prompt (the run's Task.channel does it). The agent reaches these through the Insights MCP tools
...
def _augment_scopes_for_context(scopes: InsightsMcpScopes, *, needs_write: bool) -> InsightsMcpScopes:
    """Add file_system write to a run's Insights MCP scopes when it maintains context.md / a canvas.
...
def _seed_skill_bundle_artifacts(task_run: TaskRun) -> None:
    ...
    from insights.storage import object_storage  # noqa: PLC0415 — keep storage deps off the fire path import
...
def _create_loop_task_and_run(loop: Loop, trigger: LoopTrigger | None, trigger_context: str) -> tuple[Task, TaskRun]:
    ...
    insights_mcp_scopes = _augment_scopes_for_context(
        _resolve_insights_mcp_scopes(loop.connectors), needs_write=needs_file_system_write
    )
...
def _resolve_insights_mcp_scopes(connectors: dict) -> InsightsMcpScopes:
    scopes = (connectors or {}).get("insights_mcp_scopes", "read_only")
