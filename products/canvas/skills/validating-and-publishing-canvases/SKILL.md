...
A canvas's source lives in Insights, versioned per publish. Publishing is guarded: every edit is
...
- `capabilities.insights.insights` — every insight short id the canvas passes to `ph.loadInsight`.
- `capabilities.insights.captureEvents` — every event name it passes to `ph.capture`.
- `capabilities.insights.inlineQueries: true` — when it calls `ph.query` at all.
...
- `error` diagnostics block publishing — fix all of them. Common ones: `import_not_allowed`
  (bare imports are limited to react, react-dom, @hanzo/quill, recharts, lucide-react, and dayjs),
