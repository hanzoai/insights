...
from products.warehouse_sources.backend.temporal.data_imports.sources.datastore_cloud.settings import (
    ...
    DatastoreCloudEndpointConfig,
)
from products.warehouse_sources.backend.temporal.data_imports.sources.common.http import make_tracked_session
from products.warehouse_sources.backend.temporal.data_imports.sources.common.resumable import ResumableSourceManager
from products.warehouse_sources.backend.temporal.data_imports.sources.common.typings import SourceResponse

DATASTORE_CLOUD_BASE_URL = "https://api.datastore.cloud"
# Floor for a full-refresh usage_cost backfill when the organization's createdAt is missing —
# Datastore Cloud launched in late 2022, so no billing data can predate this.
DEFAULT_USAGE_COST_START = date(2022, 1, 1)


class DatastoreCloudRetryableError(Exception):
...
def _build_url(path: str, params: dict[str, Any] | None = None) -> str:
    url = f"{DATASTORE_CLOUD_BASE_URL}{path}"
    if not params:
        return url
...
def _fetch(session: requests.Session, url: str, logger: FilteringBoundLogger) -> dict:
    ...
    if response.status_code == 429 or response.status_code >= 500:
        ...
        raise DatastoreCloudRetryableError(
            f"Datastore Cloud API error (retryable): status={response.status_code}, url={url}",
            retry_after=retry_after,
        )

    if not response.ok:
        logger.error(f"Datastore Cloud API error: status={response.status_code}, body={response.text}, url={url}")
...
def _iter_entity_rows(
    ...
    config: DatastoreCloudEndpointConfig,
...
def _iter_usage_cost_rows(
    ...
    for org in organizations:
        ...
        if resume is not None and resume.organization_id == org["id"] and resume.from_date:
            ...
            logger.debug(f"Datastore Cloud: resuming usage_cost from {start.isoformat()}")
        ...
        while True:
            ...
            resumable_source_manager.save_state(
                DatastoreCloudResumeConfig(organization_id=org["id"], from_date=next_start.isoformat())
            )
            start = next_start
...
def get_rows(
    ...
    config = DATASTORE_CLOUD_ENDPOINTS[endpoint]
