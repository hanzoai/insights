...
class HookdeckSource(ResumableSource[HookdeckSourceConfig, HookdeckResumeConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Enter your Hookdeck project API key to pull your webhook delivery history into the Insights Data warehouse.

You can find the key in Hookdeck under **Settings** → **Project** → **Secrets**.

API keys are scoped to a single Hookdeck project, so connect one Insights source per project. Hookdeck deletes events, requests and attempts once your plan's retention window passes, so the first sync only reaches as far back as that window.
""",
            iconPath="/static/services/hookdeck.png",
            docsUrl="https://hanzo.ai/docs/cdp/sources/hookdeck",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
