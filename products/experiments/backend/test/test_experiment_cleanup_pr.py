...
class TestExperimentCleanupPr(APIBaseTest):
    ...
    @patch("products.experiments.backend.experiment_service.hanzo_insights.feature_enabled")
    ...
    def test_cleanup_pr_fires_only_when_flag_on_and_opted_in(
        ...
        mock_resolve_github.return_value = SimpleNamespace(
            list_all_cached_repositories=lambda max_repos: [{"full_name": "insights/insights"}]
        )
        mock_feature_enabled.return_value = flag_enabled
        task_id = uuid4()
        mock_create_task.return_value = SimpleNamespace(task_id=task_id)
        experiment = self._running_experiment(repository="insights/insights")
        ...
        if expect_task_created:
            ...
            self.assertEqual(kwargs["repository"], "insights/insights")
