...
@pytest.mark.asyncio
async def test_should_produce_table_no_insights_function(team):
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_produce_table_with_matching_insights_function(team):
    source = await sync_to_async(ExternalDataSource.objects.create)(
        team=team, source_type=ExternalDataSourceType.POSTGRES
    )
    table = await sync_to_async(DataWarehouseTable.objects.create)(
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_not_produce_table_with_disabled_matching_insights_function(team):
    source = await sync_to_async(ExternalDataSource.objects.create)(
        team=team, source_type=ExternalDataSourceType.POSTGRES
    )
    table = await sync_to_async(DataWarehouseTable.objects.create)(
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_not_produce_table_with_deleted_matching_insights_function(team):
    source = await sync_to_async(ExternalDataSource.objects.create)(
        team=team, source_type=ExternalDataSourceType.POSTGRES
    )
    table = await sync_to_async(DataWarehouseTable.objects.create)(
...
async def test_should_produce_table_with_matching_hog_flow(team):
    ...
    await sync_to_async(InsightsFlow.objects.create)(
        team=team,
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_produce_table_with_both_insights_function_and_flow(team):
    source = await sync_to_async(ExternalDataSource.objects.create)(
        team=team, source_type=ExternalDataSourceType.POSTGRES
    )
    table = await sync_to_async(DataWarehouseTable.objects.create)(
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_should_produce_table_insights_database_connection_failure_stays_retryable(team):
    # `should_produce_table` queries Insights's own database (InsightsFunction/InsightsFlow), not the
    ...
    # source host would, so it must be re-raised as InsightsInternalDatabaseError to avoid being
    ...
    with patch(
        "products.warehouse_sources.backend.temporal.data_imports.pipelines.core.cdp_producer.InsightsFunction.objects"
    ) as mock_insights_function_objects:
        mock_insights_function_objects.filter.side_effect = DjangoOperationalError("[Errno -2] Name or service not known")
        with pytest.raises(InsightsInternalDatabaseError) as exc_info:
