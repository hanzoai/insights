...
function createMockContext(state: MockState): Context {
    ...
    return {
        api: {
            request,
            getProjectBaseUrl: (projectId: string) => `https://us.hanzo.ai/project/${projectId}`,
        } as any,
...
describe('notebook cell tools', () => {
    ...
    it('add sql cell inserts the tag, runs with sibling refs, and writes the result back', async () => {
        ...
        expect(state.runBodies[0]).toMatchObject({
            ...
            node_type: 'insightsql',
            output_name: 'sql_df',
            refs: { events_df: { node_id: 'up', kind: 'insightsql' } },
        })
    ...
    // The legacy SQL cell: a Query node rendering InsightsQL results without a run, a dataframe name, or
    // run history. Reachable only through the component escape hatch, so it is blocked there too.
    it.each([
        ['a SQL chart', { kind: 'DataVisualizationNode', source: { kind: 'InsightsQLQuery', query: 'select 1' } }],
        ['a SQL result table', { kind: 'DataTableNode', source: { kind: 'InsightsQLQuery', query: 'select 1' } }],
