...
Then drill into the top traces with `insights:query-llm-trace` to see which spans
...
1. `insights:read-data-schema` with `kind: "event_properties"` and
   `event_name: "$ai_generation"` to find custom keys
2. `insights:read-data-schema` with `kind: "event_property_values"` to spot-check
