...
DATASTORE_SUPPORTED_JOIN_ALGORITHMS = [
    "default",
    "hash",
    "parallel_hash",
    "direct",
...
def sync_execute(
    ...
    *,
    ...
    settings (Optional[Dict]): Custom Datastore settings for this query.
    ...
    Raises:
    DatastoreError: Custom wrapped Datastore error generated in case of query execution failure.
    """
    if not workload:
    ...
    if workload == Workload.DEFAULT:
        workload = get_default_datastore_workload_type()

    trace.get_current_span().set_attribute("datastore.final_workload", workload.value)
    ...
    if ch_user == DatastoreUser.DEFAULT:
        ...
        elif tags.kind == "request" and tags_id.startswith("/v1/") and "capture" not in tags_id:
    ...
    try:
        ...
        exception_type = datastore_error_type(e)
        ...
        err = wrap_datastore_query_error(e)
        raise err from e
    finally:
...
def _prepare_query(
    ...
    if isinstance(args, list | tuple | types.GeneratorType):
        # If we get one of these it means we have an insert, let the datastore
        # client handle substitution here.
        rendered_sql = query
        prepared_args = args
    elif not args:
        # If `args` is not truthy then make prepared_args `None`, which the
        # datastore client uses to signal no substitution is desired. Expected
...
def _annotate_tagged_query(query, workload: Workload) -> tuple[str, QueryTags]:
    """
    Adds in a /* */ so we can look in datastores `system.query_log`
...
@contextmanager
def datastore_query_counter(query_counter):
