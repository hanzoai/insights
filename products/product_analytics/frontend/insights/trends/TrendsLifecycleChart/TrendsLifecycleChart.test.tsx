...
import { dragSelection, setupJsdom, setupSyncRaf } from '@hanzo/quill-charts/testing'
...
describe('TrendsLifecycleChart', () => {
    ...
    it.each([
        ...
        await screen.findByTestId('trend-lifecycle-graph')
        if (expectedLabels) {
            const legend = await screen.findByTestId('script-chart-timeseries-bar-legend')
            const labels = Array.from(legend.children).map((el) => el.textContent?.trim())
            expect(labels).toEqual(expectedLabels)
        } else {
            expect(screen.queryByTestId('script-chart-timeseries-bar-legend')).not.toBeInTheDocument()
        }
    })
