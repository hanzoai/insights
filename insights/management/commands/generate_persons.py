...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # The connection commits when the block exits, before the Datastore sync reads them back.
        persons_created = 0
    ...
    def _generate_events_for_persons(self, team: Team, person_count: int):
        ...
        with persons_db_connection(writer=False) as conn, conn.cursor() as cursor:
            cursor.execute(
                "SELECT distinct_id FROM insights_persondistinctid WHERE team_id = %s ORDER BY random() LIMIT %s",
