...
class NotebookSQLV2RefSerializer(serializers.Serializer):
    ...
    kind = serializers.ChoiceField(
        choices=["insightsql", "local"],
        required=False,
        default="insightsql",
        help_text=(
            "What the name resolves to: 'insightsql' is a SQL node's query definition (resolved to its "
            "last-run InsightsQL); 'local' is a dataframe a Python node bound in the kernel namespace."
        ),
    )
...
class NotebookSQLV2RunRequestSerializer(serializers.Serializer):
    ...
    node_type = serializers.ChoiceField(
        choices=["insightsql", "python"],
        required=False,
        default="insightsql",
        help_text=(
            "Execution kind. 'insightsql' is a SQL node — pushed to Datastore, or rerouted to the sandbox's "
    ...
    code = serializers.CharField(
        help_text="The node's source — SQL for a insightsql node, Python for a python node. Must not be blank.",
    )
    ...
    refs = serializers.DictField(
        ...
        default=dict,
        help_text=(
            "Available upstream nodes, keyed by dataframe name. A SQL node inlines referenced insightsql "
...
class NotebookSQLV2PageRequestSerializer(serializers.Serializer):
    offset = serializers.IntegerField(
        ...
        default=0,
        min_value=0,
        help_text="Number of rows to skip; pages re-query Datastore with LIMIT/OFFSET.",
    )
...
class NotebookSQLV2DataPlaneRequestSerializer(serializers.Serializer):
    query = serializers.CharField(help_text="InsightsQL SELECT to execute against the notebook team's data.")
    limit = serializers.IntegerField(
        required=False,
        default=50,
        min_value=1,
        # Must admit the kernel executor's full-frame materialize cap (_MATERIALIZE_ROW_CAP);
        # the InsightsQL printer clamps explicit LIMITs to MAX_SELECT_RETURNED_ROWS regardless.
        max_value=2_000_000,
        help_text="Maximum number of rows to return (applied as an outer LIMIT, clamped by InsightsQL's row ceiling).",
    )
    offset = serializers.IntegerField(
        required=False,
        default=0,
...
class NotebookSQLV2EnvelopeSerializer(serializers.Serializer):
    ...
    frames = NotebookSQLV2FrameSerializer(
        ...
        help_text=(
            ...
            "Only kernel runs (python/duckdb) report these; a insightsql run never enters the kernel."
        ),
    )
    ...
    types = serializers.ListField(
        child=serializers.ListField(child=serializers.CharField(), help_text="A [column name, Datastore type] pair."),
        required=False,
        default=list,
        help_text="Datastore type per column, as [name, type] pairs; used by the visualization tab.",
    )
    row_count = serializers.IntegerField(required=False, default=0, help_text="Number of rows in the result.")
    has_more = serializers.BooleanField(
        required=False,
        default=False,
        help_text="Whether Datastore has more rows beyond first_page (detected by fetching limit+1).",
    )
    ...
    timings = LenientTimingsField(
        ...
        help_text=(
            ...
            "datastore_s (pickup to completion). Feeds the node-run metrics."
        ),
    )
...
class NotebookSQLV2RunStatusResponseSerializer(serializers.Serializer):
    ...
    rows = serializers.ListField(
        ...
        required=False,
        help_text=(
            "SQL (insightsql) runs only: the full capped row set for client-side paging, present while the "
