## Creating Datastore schema changes

**Important:** Datastore schema changes should be created as a separate PR from application code changes. This allows for:
...
Some tables are meant to run only on a couple of nodes, like some Kafka tables, to prevent the whole cluster to run too many insertions in Datastore.

And, in some other cases, we want to only create one table (like a unique Kafka consumer) in one node.

Because of the above, take the following advice into consideration when manipulating schemas for Datastore.
...
We have extra nodes with a sole purpose of ingesting the data from Kafka topics into Datastore tables. These nodes don't contain any data perse, only Kafka tables and Distributed ones, along with the materialized views that connect them.

Use these node roles exclusively when you need to ingest data from Kafka into Datastore.

When you want to pull data from Kafka into Datastore, you should:
...
2. Create a writable table only on ingestion nodes. It should be a Distributed table with your data table.
   1. If your data table is non-sharded, you should point it to one shard: `Distributed(..., cluster=settings.DATASTORE_SINGLE_SHARD_CLUSTER)`, without using any sharding key.
...
We are introducing changes to our Datastore topology frequently, introducing new types of nodes.
...
We have extra nodes with a sole purpose of ingesting the data from Kafka topics into Datastore tables. The way to do that is to:

1. Create your data table in Datastore main cluster.
2. Create a writable table only on ingestion nodes: `node_roles=[NodeRole.INGESTION_SMALL]`. It should be Distributed table with your data table. If your data table is non-sharded, you should point it to one shard: `Distributed(..., cluster=settings.DATASTORE_SINGLE_SHARD_CLUSTER)`.
