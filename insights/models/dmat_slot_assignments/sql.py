"""Datastore schema for the dmat slot-assignments dictionary.
...
Pattern mirrors `insights/models/web_preaggregated/team_selection.py` —
ReplacingMergeTree backing table + DATASTORE-source dictionary, created
...
SELECT
    ...
    property_name
FROM
    `{DATASTORE_DATABASE}`.`{DMAT_SLOT_ASSIGNMENTS_TABLE_NAME}`
FINAL
""".replace("\n", " ").strip()
