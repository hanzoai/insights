...
Score writeback piggybacks on the existing `KAFKA_DATASTORE_SESSION_REPLAY_EVENTS`
topic + `session_replay_events_mv` — same pattern as the AI-summary writeback
in `insights.temporal.session_replay.session_summary.activities.video_based.
...
from insights.temporal.session_replay.surfacing_scoring_sweep import sql as surfacing_scoring_sweep_sql
...
from insights.temporal.session_replay.surfacing_scoring_sweep.features import (
    ID_COLUMNS,
...
from insights.temporal.session_replay.surfacing_scoring_sweep.metrics import record_backlog_estimate
from insights.temporal.session_replay.surfacing_scoring_sweep.scorer import get_feature_names, predict
from insights.temporal.session_replay.surfacing_scoring_sweep.types import (
    ChunkResult,
    ChunkSpec,
    ListChunksResult,
...
def _build_partial_row(
    ...
    * `distinct_id` MUST be the session's real distinct_id — it's the
    ...
    partial_ts = format_datastore_timestamp(ts)
    return {
...
def _publish_scores(df: pd.DataFrame, scores: np.ndarray) -> int:
    ...
    producer = get_producer(topic=KAFKA_DATASTORE_SESSION_REPLAY_EVENTS)
...
async def score_chunk_activity(spec: ChunkSpec) -> ChunkResult:
    ...
    # Sessions pulled from Datastore for this chunk, before any out-of-contract drop.
    fetched = len(df)
