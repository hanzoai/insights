...
from insights.datastore.query_tagging import Product, QueryTags, query_tags, reset_query_tags, tag_queries
...
class TestRateLimit(BaseTest):
    ...
    def test_block_counter_attributes_product_via_fallback(self, _name, query_tags, expected_product):
        ...
        with patch("insights.datastore.client.limit.CONCURRENT_QUERY_LIMIT_EXCEEDED_COUNTER") as mock_counter:
            with self.assertRaises(ConcurrencyLimitExceeded):
...
class TestOrgConcurrencyLimit(BaseTest):
    ...
    @patch("insights.datastore.client.limit.TEST", False)
    def test_no_org_limit_returns_none(self):
        """Test that get_org_app_concurrency_limit returns None when no org limit is found"""
        from insights.datastore.client.limit import get_org_app_concurrency_limit

        # Mock Redis to return None (cache miss)
        with patch("insights.datastore.client.limit.redis.get_client") as mock_redis:
    ...
    def test_org_limit_with_cache_miss_then_hit(self):
        ...
        with patch("insights.datastore.client.limit.redis.get_client") as mock_redis2:
...
class TestMaterializedEndpointsRateLimiter(BaseTest):
    ...
    def test_rate_limiter_applies_for_materialized_endpoints(self):
        ...
        from insights.datastore.client.limit import get_materialized_endpoints_rate_limiter
...
class TestDashboardQueriesRateLimiter(BaseTest):
    ...
    def test_rate_limiter_skips_for_celery_tasks(self):
        ...
        from insights.datastore.client.limit import get_app_dashboard_queries_rate_limiter

        rate_limiter = get_app_dashboard_queries_rate_limiter()

        mock_task = Mock(spec=Task)
        with patch("insights.datastore.client.limit.current_task", mock_task):
    ...
    def test_rate_limiter_skips_for_temporal_workflows(self):
        ...
        with patch("insights.datastore.client.limit.current_task", None):
