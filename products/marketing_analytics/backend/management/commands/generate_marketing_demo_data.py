...
from insights.settings import OBJECT_STORAGE_ACCESS_KEY_ID, OBJECT_STORAGE_SECRET_ACCESS_KEY
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if options["skip_events"]:
            ...
            generator.generate()
            self.stdout.write(self.style.SUCCESS(f"Wrote {generator.result.events_written} events to Datastore."))
        ...
        self.stdout.write(
            "Kafka delivery is async: give Datastore a minute before expecting all events in the dashboard."
        )
