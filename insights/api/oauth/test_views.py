...
from insights.api.oauth import OAuthAuthorizationSerializer
from insights.api.oauth.views import OAuthValidator
from insights.models.oauth import (
    OAuthAccessToken,
    OAuthApplication,
...
from insights.settings.utils import generate_rsa_private_key_pem
...
class TestOAuthAPI(APIBaseTest):
    ...
    @patch("insights.api.oauth.views.render_template")
    @patch("insights.api.oauth.mcp_resource_scopes.mcp_advertised_scopes")
    def test_authorize_injects_mcp_scopes_when_resource_omits_scope(self, mock_scopes, mock_render):
        ...
        auth_url = f"{self.base_authorization_url}&resource=https%3A%2F%2Fmcp.hanzo.ai%2Fmcp"
    ...
    def test_authorize_with_prompt_none_openid_is_rejected_not_500(self):
        # Insights does not support OIDC silent authentication: `prompt=none` is always rejected
    ...
    def test_validate_silent_authorization_always_false(self):
        # Insights never authorizes silently; prompt=none must fall back to interactive consent.
    ...
    @parameterized.expand(
        [
            ("us", "https://us.hanzo.ai/api/oauth/wizard/client-metadata"),
            ("eu", "https://eu.hanzo.ai/api/oauth/wizard/client-metadata"),
        ]
    )
    ...
    def test_refresh_succeeds_when_only_scoped_teams_is_set(self):
        ...
        checks (insights/permissions.py, team_access_cache.py, Rust flag service) already
    ...
    def test_application_isolation_different_users(self):
        from insights.models import Organization, OrganizationMembership, User
    ...
    def test_public_client_scoped_access(self):
        ...
        access_token = token_response.json()["access_token"]
        from insights.models.oauth import OAuthAccessToken
    ...
    def test_authorize_wildcard_narrowing_captures_event(self):
        self._set_ceiling("experiment:read")
        with patch("insights.api.oauth.views.render_template", return_value=HttpResponse("")):
            with patch("insights.api.oauth.views.hanzo_insights.capture") as mock_capture:
    ...
    def test_authorize_get_passes_wildcard_read_scopes_to_consent_page(self):
        with patch("insights.api.oauth.views.render_template", return_value=HttpResponse("")) as mock_render:
            response = self.client.get(f"{self.base_authorization_url}&scope=*")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_authorize_wildcard_accepted_when_app_ceiling_empty(self):
        # Existing clients (the Insights Desktop CLI today) still send scope=*
    ...
    def test_error_redirect_with_custom_scheme_does_not_500(self, scheme):
        custom_scheme_app = OAuthApplication.objects.create(
            ...
            client_type=OAuthApplication.CLIENT_PUBLIC,
            authorization_grant_type=OAuthApplication.GRANT_AUTHORIZATION_CODE,
            redirect_uris=f"{scheme}://insights/callback",
            user=self.user,
            hash_client_secret=True,
            algorithm="RS256",
        )
        ...
        url = (
            ...
            f"&redirect_uri={scheme}://insights/callback"
        ...
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
        self.assertTrue(response["Location"].startswith(f"{scheme}://insights/callback"))
        self.assertIn("error=invalid_scope", response["Location"])

    @freeze_time("2025-01-01 00:00:00")
    @override_settings(CLOUD_DEPLOYMENT="US", SITE_URL="https://us.hanzo.ai")
    def test_token_response_includes_region_for_us_cloud(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        self.assertEqual(data["insights_region"], "us")
        self.assertEqual(data["insights_base_url"], "https://us.hanzo.ai")

    @freeze_time("2025-01-01 00:00:00")
    @override_settings(CLOUD_DEPLOYMENT="EU", SITE_URL="https://eu.hanzo.ai")
    def test_token_response_includes_region_for_eu_cloud(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        self.assertEqual(data["insights_region"], "eu")
        self.assertEqual(data["insights_base_url"], "https://eu.hanzo.ai")
    ...
    def test_token_response_excludes_region_for_self_hosted(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        self.assertNotIn("insights_region", data)
        self.assertNotIn("insights_base_url", data)
...
class TestOAuthAuthorizationServerMetadata(APIBaseTest):
    ...
    def test_metadata_excludes_internal_scope_objects(self):
        ...
        scopes = metadata["scopes_supported"]
        for scope in scopes:
            assert not scope.startswith("datastore_test_cluster_perf:"), (
    ...
    def test_metadata_includes_region_for_cloud(self):
        response = self.client.get("/.well-known/oauth-authorization-server")
        metadata = response.json()
        self.assertEqual(metadata["insights_region"], "us")

    @override_settings(CLOUD_DEPLOYMENT=None)
    def test_metadata_excludes_region_for_self_hosted(self):
        response = self.client.get("/.well-known/oauth-authorization-server")
        metadata = response.json()
        self.assertNotIn("insights_region", metadata)
...
class TestOIDCInactiveKeysSetting(SimpleTestCase):
    ...
    Reloading insights.settings.web only rebinds names in that module; django.conf.settings
    is populated from the insights.settings package and is unaffected, so this stays isolated.
    """
    ...
    def test_inactive_keys_built_from_env_vars(self, _name, env_1, env_2, expected):
        ...
        web_settings = importlib.import_module("insights.settings.web")
        try:
