...
from insights.helpers.tiktoken_encoding import LLM_TOKEN_COUNT_PROXY_MODEL, get_tiktoken_encoding_for_model
...
@patch("products.error_tracking.backend.temporal.lifecycle.issue_created.activities.hanzo_insights.capture_exception")
...
@patch("products.error_tracking.backend.temporal.lifecycle.event_properties.execute_insightsql_query")
@patch("products.error_tracking.backend.temporal.lifecycle.event_properties.get_client")
def test_fetches_full_event_properties_from_valkey_without_datastore(
    get_client: MagicMock, execute_insightsql_query: MagicMock
) -> None:
    inputs = _inputs("fingerprint")
...
@patch("products.error_tracking.backend.temporal.lifecycle.event_properties.execute_insightsql_query")
@patch("products.error_tracking.backend.temporal.lifecycle.event_properties.get_client")
def test_falls_back_to_datastore_when_valkey_payload_expired(
    get_client: MagicMock, execute_insightsql_query: MagicMock
) -> None:
    inputs = _inputs("fingerprint")
    properties = {"$exception_list": [{"type": "TypeError", "value": "expired"}]}
    get_client.return_value.get.return_value = None
    execute_insightsql_query.return_value = SimpleNamespace(results=[[json.dumps(properties)]])
