...
describe('alertFormLogic', () => {
    ...
    beforeEach(() => {
        ...
        errorToastSpy = jest.spyOn(toast, 'error').mockImplementation(jest.fn())
        successToastSpy = jest.spyOn(toast, 'success').mockImplementation(jest.fn())
        captureExceptionSpy = jest.spyOn(insights, 'captureException').mockImplementation(jest.fn())
        captureSpy = jest.spyOn(insights, 'capture').mockImplementation(jest.fn())
    ...
    describe('insightsql column prefill', () => {
        ...
        it('does not prefill when editing an existing alert (would dirty the form / clobber a saved column)', async () => {
            const logic = alertFormLogic({
                alert: makeSavedAlert({ config: { type: 'InsightsQLAlertConfig' } as any }),
                insightId: 42,
                onEditSuccess: jest.fn(),
                insightVizDataLogicProps: insightLogicProps,
                insightInterval: 'day',
    ...
    describe('insightsql column options', () => {
        function mountWithData(insightData: Record<string, any>): ReturnType<typeof alertFormLogic.build> {
            ...
            logic.mount()
            logic.actions.setAlertFormValue('config', { type: 'InsightsQLAlertConfig', evaluation: 'last_row' })
            insightDataLogic(insightLogicProps).actions.setInsightData(insightData)
            return logic
        }
        ...
        it('value options are the numeric columns only', () => {
            ...
            expect(logic.values.insightsqlValueColumnOptions).toEqual([{ label: 'count', value: 'count' }])
        })
        ...
        it('value options fall back to every column when nothing is numeric (avoids a dead-end picker)', () => {
            ...
            expect(logic.values.insightsqlValueColumnOptions).toEqual([
                { label: 'a', value: 'a' },
                { label: 'b', value: 'b' },
            ])
        })
        ...
        it('label options exclude the evaluated column', async () => {
            ...
            await expectLogic(logic).toFinishAllListeners()
            expect((logic.values.alertForm.config as InsightsQLAlertConfig).column).toEqual('count')
            expect(logic.values.insightsqlLabelColumnOptions).toEqual([{ label: 'day', value: 'day' }])
        })
    })

    describe('deriveInsightsQLAlertPreview', () => {
        const INSIGHTSQL_CONFIG = { type: 'InsightsQLAlertConfig' } as const
        const ANY_ROW_CONFIG = { type: 'InsightsQLAlertConfig', evaluation: 'any_row' } as const
        ...
        it.each([
            ['no result loaded', null, INSIGHTSQL_CONFIG, null, null],
            ['result not an array', { result: 'oops' }, INSIGHTSQL_CONFIG, null, null],
            ['empty result', { result: [] }, INSIGHTSQL_CONFIG, null, { status: 'no-rows' }],
            ['rows are not arrays', { result: [{ a: 1 }] }, INSIGHTSQL_CONFIG, null, { status: 'bad-shape' }],
            [
            ...
            [
                ...
                { result: [[1, 2]], columns: ['a', 'b'] },
                { type: 'InsightsQLAlertConfig', column: 'b' },
                null,
            ...
            [
                ...
                { result: [[1]], columns: ['a'] },
                { type: 'InsightsQLAlertConfig', column: 'gone' },
                null,
            ...
            [
                ...
                { type: 'InsightsQLAlertConfig', evaluation: 'any_row', column: 'count' },
                { upper: -1 },
                ok({
                    mode: 'any_row',
                    columnName: 'count',
            ...
            [
                ...
                {
                    result: Array.from({ length: INSIGHTSQL_ANY_ROW_MAX_ROWS + 1 }, (_, i) => [i]),
                    columns: ['count'],
                },
                { type: 'InsightsQLAlertConfig', evaluation: 'any_row' },
                null,
                { status: 'too-many-rows', rowCount: INSIGHTSQL_ANY_ROW_MAX_ROWS + 1 },
            ],
            [
                'last-row at the truncation cap warns (tail may be truncated)',
                {
                    result: Array.from({ length: INSIGHTSQL_LAST_ROW_MAX_ROWS }, (_, i) => [i]),
                    columns: ['count'],
                },
                { type: 'InsightsQLAlertConfig', evaluation: 'last_row' },
                null,
                { status: 'last-row-truncated', rowCount: INSIGHTSQL_LAST_ROW_MAX_ROWS },
            ],
            [
                'missing explicit label column',
                { result: [['US', 1]], columns: ['country', 'count'] },
                { type: 'InsightsQLAlertConfig', evaluation: 'any_row', label_column: 'gone' },
                null,
            ...
            expect(
                deriveInsightsQLAlertPreview(insightData as Record<string, any> | null, config as any, bounds as any)
            ).toEqual(expected)
        })

        it('first_row is immune to the last_row truncation cap (reads the head)', () => {
            const result = Array.from({ length: INSIGHTSQL_LAST_ROW_MAX_ROWS }, (_, i) => [i])
            const preview = deriveInsightsQLAlertPreview(
                { result, columns: ['count'] },
                { type: 'InsightsQLAlertConfig', evaluation: 'first_row' } as any,
                null
            )
    ...
    describe('deriveFunnelAlertPreview', () => {
        ...
        it.each([
            ['not a funnel config', { result: steps(100, 40) }, { type: 'InsightsQLAlertConfig' }, undefined, null],
    ...
    describe('ongoing-interval gating', () => {
        ...
        it.each([
            ...
            ['SQL config', { type: 'InsightsQLAlertConfig', evaluation: 'last_row' }, false],
            ['null', null, false],
        ...
        it.each([
            ...
            ['SQL (never shown)', { type: 'InsightsQLAlertConfig', evaluation: 'last_row' }, false, false, false, true],
