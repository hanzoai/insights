...
from products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common import test_account_filter_expr
...
def evaluate_cumulative_pageviews(ctx: EvalContext) -> int:
    ...
    for team in _project_environment_teams(ctx.team):
        ...
        response = execute_insightsql_query(query=query, team=team, query_type="web_achievements_pageviews")
        if response.results:
...
def evaluate_conversions(ctx: EvalContext) -> int:
    ...
    for team in _project_environment_teams(ctx.team):
        ...
        response = execute_insightsql_query(query=query, team=team, query_type="web_achievements_conversions")
        if response.results:
