...
from insights.models.repo_routing_rule import RepoRoutingRule
...
def _create_user_github_integration(
    user: User, *, integration_id: str = "gh-1", name: str = "insights"
) -> UserIntegration:
...
class TestGetFullRepoNames:
    ...
    def test_single_integration_single_page(self, mock_github_class):
        ...
        mock_github.list_all_cached_repositories.return_value = [
            _repo_dict("insights", "insights", 1),
            _repo_dict("insights", "insights-js", 2),
            _repo_dict("insights", "plugin-server", 3),
        ]
        mock_github_class.return_value = mock_github

        result = _get_full_repo_names(self.slack_integration, user_id=self.user.id)
        assert result == ["insights/plugin-server", "insights/insights", "insights/insights-js"]
    ...
    def test_results_are_sorted(self, mock_github_class):
        ...
        mock_github = MagicMock()
        mock_github.list_all_cached_repositories.return_value = [
            _repo_dict("insights", "zebra", 1),
            _repo_dict("insights", "alpha", 2),
            _repo_dict("insights", "middle", 3),
        ]
        mock_github_class.return_value = mock_github

        result = _get_full_repo_names(self.slack_integration, user_id=self.user.id)
        assert result == ["insights/alpha", "insights/middle", "insights/zebra"]
...
class TestGetFullRepoNamesBotPRs:
    ...
    @patch("products.slack_app.backend.api.hanzo_insights.feature_enabled", return_value=False)
    def test_flag_off_excludes_team_repos(self, _mock_feature_enabled, mock_user_github_class, mock_team_github_class):
        ...
        assert result == ["insights/insights", "insights/insights-js"]
        mock_team_github_class.assert_not_called()

    @patch("products.slack_app.backend.api.hanzo_insights.feature_enabled", return_value=True)
    ...
    def test_flag_on_unions_user_and_team_repos(
        ...
        mock_user_github = MagicMock()
        mock_user_github.list_all_cached_repositories.return_value = [
            _repo_dict("insights", "insights-js", 1),
            _repo_dict("insights", "shared", 2),
        ]
        mock_user_github_class.return_value = mock_user_github

        mock_team_github = MagicMock()
        mock_team_github.list_all_cached_repositories.return_value = [
            _repo_dict("insights", "insights", 3),
            _repo_dict("insights", "shared", 2),
        ]
        ...
        assert result == ["insights/insights", "insights/insights-js", "insights/shared"]
    ...
    def test_flag_on_no_personal_install_returns_team_repos(
        ...
        mock_team_github.list_all_cached_repositories.return_value = [
            _repo_dict("insights", "insights", 1),
            _repo_dict("insights", "plugin-server", 2),
        ]
        mock_team_github_class.return_value = mock_team_github

        result = _get_full_repo_names(self.slack_integration, user_id=self.user.id)

        assert result == ["insights/plugin-server", "insights/insights"]
...
class TestGetFullRepoNamesCache:
    ...
    def test_cache_miss_populates_cache(self, mock_github_class):
        _create_user_github_integration(self.user)
        mock_github = MagicMock()
        mock_github.list_all_cached_repositories.return_value = [_repo_dict("insights", "repo-a")]
        mock_github_class.return_value = mock_github

        result = _get_full_repo_names(self.slack_integration, user_id=self.user.id)

        assert result == ["insights/repo-a"]
        assert cache.get(_user_repo_list_cache_key(self.user.id)) == ["insights/repo-a"]
    ...
    def test_invalidation_forces_refetch(self, mock_github_class):
        ...
        mock_github.list_all_cached_repositories.return_value = [
            _repo_dict("insights", "repo-a"),
            _repo_dict("insights", "repo-b", 2),
        ]
        result = _get_full_repo_names(self.slack_integration, user_id=self.user.id)
        assert result == ["insights/repo-a", "insights/repo-b"]
...
class TestPostRepoPickerPrewarm:
    ...
    def test_prewarm_calls_get_full_repo_names(self, mock_slack_cls, mock_github_class):
        ...
        _post_repo_picker_message(
            ...
            action_id="insights_code_repo_select",
        )
...
class TestExtractExplicitRepo:
    @parameterized.expand(
        [
            ("simple", "fix insights/insights-js please", "insights/insights-js"),
            ("no_match", "hello world", None),
            ("case_insensitive", "check Insights/Insights", "insights/insights"),
            ("url_false_positive", "see https://github.com/insights/insights/issues/1", None),
            ("backticks", "please fix `insights/insights-js`", "insights/insights-js"),
            (
                "slack_link_label",
                "use <https://github.com/insights/insights-js|insights/insights-js>",
                "insights/insights-js",
            ),
            ("multiple_first_wins", "check insights/insights-js then insights/insights", "insights/insights-js"),
            ("with_bot_mention", "<@U123> fix insights/insights-js", "insights/insights-js"),
        ]
    )
    def test_extract_explicit_repo(self, _name, text, expected):
        repos = ["insights/insights", "insights/insights-js", "insights/plugin-server"]
...
class TestParseRulesCommand:
    @parameterized.expand(
        [
            ...
            (
                "bot_mention_add",
                '<@U123BOT> rules add "backend bugs" insights/insights',
                RulesCommand(action="add", rule_text="backend bugs", repository="insights/insights"),
            ),
            (
            ...
            (
                ...
                "default repo set insights/insights",
            ...
            (
                ...
                "<@U123BOT> default repo set insights/insights",
                RulesCommand(action="deprecated_default_repo"),
            ),
        ]
    )
    ...
    @parameterized.expand(
        [
            ...
            ("random_text", "fix the bug in insights-js"),
...
class TestHandleRulesCommandActivity:
    ...
    def _make_inputs(self, text: str):
        from insights.temporal.ai.slack_app import InsightsCodeSlackMentionWorkflowInputs

        return InsightsCodeSlackMentionWorkflowInputs(
    ...
    def test_list_empty_rules(self, mock_slack_cls):
        ...
        from insights.temporal.ai.slack_app import handle_insights_code_rules_command_activity

        result = handle_insights_code_rules_command_activity(
    ...
    @patch("insights.models.integration.SlackIntegration")
    def test_help_uses_insights_commands(self, mock_slack_cls):
        mock_slack = MagicMock()
        mock_slack_cls.return_value = mock_slack
        ...
        assert result.status == "handled"
        msg = mock_slack.client.chat_postMessage.call_args
        assert "@Insights <task description>" in msg.kwargs["text"]
        assert "@Insights Desktop" not in msg.kwargs["text"]
    ...
    def test_list_shows_rules(self, mock_slack_cls):
        ...
        RepoRoutingRule.objects.create(
            team=self.team, rule_text="JS SDK bugs", repository="insights/insights-js", priority=0
        )
        RepoRoutingRule.objects.create(
            team=self.team, rule_text="Backend issues", repository="insights/insights", priority=1
        )
    ...
    @patch(
        "products.slack_app.backend.api._get_full_repo_names", return_value=["insights/insights", "insights/insights-js"]
    )
    @patch("insights.models.integration.SlackIntegration")
    def test_add_with_repo_creates_rule(self, mock_slack_cls, _mock_repos):
        ...
        result = handle_insights_code_rules_command_activity(
            self._make_inputs('<@U123> rules add "JS SDK bugs" insights/insights-js'),
            self.channel,
            self.thread_ts,
            self.slack_user_id,
            self.user.id,
        ...
        assert rule.repository == "insights/insights-js"
        msg = mock_slack.client.chat_postMessage.call_args
    ...
    @patch("products.slack_app.backend.api._get_full_repo_names", return_value=["insights/insights"])
    @patch("insights.models.integration.SlackIntegration")
    def test_add_rejects_disconnected_repo(self, mock_slack_cls, _mock_repos):
        ...
        result = handle_insights_code_rules_command_activity(
            self._make_inputs('<@U123> rules add "JS bugs" insights/nonexistent'),
            self.channel,
            self.thread_ts,
            self.slack_user_id,
            self.user.id,
    ...
    def test_non_command_returns_not_a_command(self):
        ...
        result = handle_insights_code_rules_command_activity(
            self._make_inputs("<@U123> fix the bug in insights-js"),
            self.channel,
            self.thread_ts,
            self.slack_user_id,
            self.user.id,
