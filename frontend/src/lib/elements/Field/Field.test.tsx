import '@testing-library/jest-dom'

import { cleanup, render, screen } from '@testing-library/react'
import { MakeLogicType, kea, path } from 'kea'
import { Form, forms } from 'kea-forms'
import type { DeepPartial, DeepPartialMap, FieldName, ValidationErrorType } from 'kea-forms'

import { Input } from 'lib/elements/Input/Input'

import { initKeaTests } from '~/test/init'

import { Field } from './Field'

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface fieldTestLogicValues {
    isMyFormSubmitting: boolean
    isMyFormValid: boolean
    myForm: {
        email: string
        referral_source: string
    }
    myFormAllErrors: Record<string, any>
    myFormChanged: boolean
    myFormErrors: DeepPartialMap<
        {
            email: string
            referral_source: string
        },
        ValidationErrorType
    >
    myFormHasErrors: boolean
    myFormManualErrors: Record<string, any>
    myFormTouched: boolean
    myFormTouches: Record<string, boolean>
    myFormValidationErrors: DeepPartialMap<
        {
            email: string
            referral_source: string
        },
        ValidationErrorType
    >
    showMyFormErrors: boolean
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface fieldTestLogicActions {
    resetMyForm: (values?: { email: string; referral_source: string }) => {
        values?: {
            email: string
            referral_source: string
        }
    }
    setMyFormManualErrors: (errors: Record<string, any>) => {
        errors: Record<string, any>
    }
    setMyFormValue: (
        key: FieldName,
        value: any
    ) => {
        name: FieldName
        value: any
    }
    setMyFormValues: (
        values: DeepPartial<{
            email: string
            referral_source: string
        }>
    ) => {
        values: DeepPartial<{
            email: string
            referral_source: string
        }>
    }
    submitMyForm: () => {
        value: boolean
    }
    submitMyFormFailure: (
        error: Error,
        errors: Record<string, any>
    ) => {
        error: Error
        errors: Record<string, any>
    }
    submitMyFormRequest: (myForm: { email: string; referral_source: string }) => {
        myForm: {
            email: string
            referral_source: string
        }
    }
    submitMyFormSuccess: (myForm: { email: string; referral_source: string }) => {
        myForm: {
            email: string
            referral_source: string
        }
    }
    touchMyFormField: (key: string) => {
        key: string
    }
}

export type fieldTestLogicType = MakeLogicType<fieldTestLogicValues, fieldTestLogicActions>

const fieldTestLogic = kea<fieldTestLogicType>([
    path(['lib', 'elements', 'Field', 'test']),
    forms(() => ({
        myForm: {
            defaults: { referral_source: '', email: '' } as { referral_source: string; email: string },
            errors: () => ({}),
            submit: () => {},
        },
    })),
])

describe('Field', () => {
    beforeEach(() => {
        initKeaTests()
        fieldTestLogic.mount()
    })

    afterEach(() => {
        cleanup()
    })

    it('Pure field wires the explicit htmlFor onto the label', () => {
        render(
            <Field.Pure label="Email" htmlFor="email-input">
                <input id="email-input" />
            </Field.Pure>
        )

        // getByLabelText only resolves when the label is correctly associated with the input,
        // so it's both the assertion and the proof of correct wiring.
        expect(screen.getByLabelText('Email')).toBe(document.getElementById('email-input'))
    })

    it.each([
        {
            desc: 'function-as-child render prop (the signup dead-click pattern)',
            label: 'Where did you hear about us?',
            field: (
                <Field name="referral_source" label="Where did you hear about us?">
                    {({ value, onChange }) => (
                        <Input value={value ?? ''} onChange={(val: string) => onChange(val)} />
                    )}
                </Field>
            ),
        },
        {
            desc: 'React-element child (no render prop)',
            label: 'Email address',
            field: (
                <Field name="email" label="Email address">
                    <Input type="email" />
                </Field>
            ),
        },
    ])('clicking the label focuses the wrapped input — $desc', ({ label, field }) => {
        render(
            <Form logic={fieldTestLogic} formKey="myForm">
                {field}
            </Form>
        )

        // getByLabelText only finds the input when label[for] matches input[id].
        // If Field regressed to emitting htmlFor={undefined}, this throws.
        const input = screen.getByLabelText(label) as HTMLInputElement
        expect(input).toBeInstanceOf(HTMLInputElement)
        expect(input.id).toBeTruthy()
        expect(screen.getByText(label).closest('label')?.getAttribute('for')).toBe(input.id)
    })
})
