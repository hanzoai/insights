...
Reuse the user's existing setup by default. Do not start, restart, or replace a dev stack just because you are running QA. First check whether Insights is already reachable at `BASE_URL`.
...
When `.agents/skills/run-insights/SKILL.md` is present in the repo, read it and use its current readiness checks, phrocs process guidance, and `setup_test`/login recipe as the source of truth. This skill only adds the QA-specific constraints: reuse the user's stack when it is already usable, ask before starting or restarting Insights, and stop only the stack the agent started.
...
If Insights is not reachable, check user memory/settings and local preferences first, then repo guidance and nearby docs such as `AGENTS.md` for the preferred way to start the stack. Then ask the user how they want to proceed before starting Insights. If the folder and command are obvious, you may propose that specific startup path, including whether it runs interactively or in the background, but present it as an inference to confirm. If the folder, command, `BASE_URL`, or startup approach is not obvious, ask how and where the user wants the stack run, or whether to use a different `BASE_URL`. Do not mention team-specific env vars unless the user already brought them up.
...
After startup, repeat the app reachability checks from `run-insights` or, if that skill is unavailable, the minimal checks above. Also query available process checks:
...
Fallback to repo-local logs under `.insights/.generated/logs/` only when phrocs MCP is unavailable.
...
```bash
LOGIN_USERNAME="${LOGIN_USERNAME:-test@hanzo.ai}"
...
2. If using `setup_test`, run the in-page setup and login fetches from `run-insights`, then navigate to the route under the returned `team_id`.
