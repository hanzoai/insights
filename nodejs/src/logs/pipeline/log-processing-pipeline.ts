...
/**
 * Per-message drop accounting produced by `filter` stages (sampling drop rules, rate limits, script
 ...
 * - `filter` removes records and reports what it dropped (sampling drop rules, rate limits, script
...
export async function runPipelineStages(
    ...
    for (const stage of stages) {
        ...
        // script transforms, so a message emptied by the transform is attributed to it, not sampling.
