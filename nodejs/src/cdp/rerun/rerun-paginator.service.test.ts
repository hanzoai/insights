...
describe('RerunPaginatorService integration', () => {
    ...
    beforeAll(() => {
        ...
        chClient = Datastore.createClient()
    })
     ...
     * Kafka produce + MV ingestion, so by the time this returns Datastore has
    ...
    const seedRows = async (
        ...
        // Seed visibility rides the shared Kafka -> Datastore pipe, whose consumer may still be
    ...
    beforeAll(async () => {
        ...
        // Ensure all topics exist (idempotently, without deleting) so the Datastore Kafka engine
    ...
    beforeEach(async () => {
        ...
        // Real script function manager so rehydrate can resolve the function from postgres.
        insightsFunctionManager = new InsightsFunctionManagerService(hub.postgres, hub.pubSub, hub.encryptedFields)
        insightsFunctionManager['onInsightsFunctionsReloaded'](team.id, [insightsFunction.id])
    ...
    describe('by-ids mode', () => {
        it('rehydrates and re-enqueues only the requested invocations, ignoring others', async () => {
            ...
            for (const [invocation] of runningCalls) {
                expect((invocation as CyclotronJobInvocationInsightsFunction).state.rerunAttempts).toBe(1)
            }
        ...
        it('skips invocation ids that have no matching row in datastore', async () => {
    ...
    describe('max_attempts filter (poison-pill autodrain path)', () => {
        // Regression guard for the aggregate-inside-aggregate Datastore error: with
        ...
        // aggregate) → Datastore rejects every page. No other test sets max_attempts
    ...
    describe('rehydration', () => {
        it('skips rows whose script function has been deleted between original run and rerun', async () => {
            const orphanedFn = await _insertInsightsFunction(hub.postgres, team.id, {
                type: 'destination',
                script: 'print("orphan")',
                bytecode: [],
            })
            // Seed under the orphaned function...
            const prevFn = insightsFunction
            insightsFunction = orphanedFn
            await seedRows([{ invocation_id: 'inv-orphan', status: 'failed', error: new Error('5xx') }])
            insightsFunction = prevFn

            // ...then ask the paginator to rerun it. The paginator's script function
            // manager (real) lazy-loads the orphaned function (still exists in PG)
            // — we explicitly stub `getInsightsFunction` to return null to simulate the
            // "deleted between original run and rerun" scenario.
            const realGetInsightsFunction = insightsFunctionManager.getInsightsFunction.bind(insightsFunctionManager)
            jest.spyOn(insightsFunctionManager, 'getInsightsFunction').mockImplementation(async (id) => {
                if (id === orphanedFn.id) {
                    return null
                }
                return realGetInsightsFunction(id)
            })
    ...
    describe('error handling', () => {
        it('captures a Datastore query error on progress.last_error without marking done', async () => {
            // Point the paginator at a deliberately broken Datastore client to
            // exercise the catch path.
            const brokenChClient = {
                query: jest.fn().mockRejectedValue(new Error('datastore boom')),
    ...
    // Silence unused-import noise for InsightsFunctionInvocationGlobals which is
    // referenced indirectly via the rehydration assertion.
    void (undefined as unknown as InsightsFunctionInvocationGlobals)
})
