...
from insights.schema import AssistantFunnelsQuery, AssistantRetentionQuery, AssistantTrendsQuery

from products.insights_ai.eval_harness.log_parser import INFO_SYNTHETIC_PREFIX, LogParser, ToolCall
from products.insights_ai.eval_harness.scorers import (
...
# Insights MCP tools that persist saved-insight state. The sandbox is disposable
...
def extract_last_query_funnel_input(output: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    legitimately answered via InsightsQL (``execute-sql``); that's covered by the
...
8. **Aggregation**: ``aggregation_group_type_index`` must match when the expected sets a non-null value (group-level funnels — e.g. "aggregated by unique accounts"). "Aggregated by session" → ``funnelAggregateByInsightsQL: "properties.$session_id"``. "Aggregated by user" → no ``aggregation_group_type_index`` and no ``funnelAggregateByInsightsQL`` (the default user-level aggregation).
