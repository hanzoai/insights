...
Lives next to the Slack workflow it exercises (`insights_code_slack_mention.py`) rather
than in `insights/management/commands/` because (a) it's co-located with the thing it
...
- Dev stack up (sandbox manager + temporal + LLM creds): `./bin/start` / `insightscli start`

# Usage

    # Full eval (~3-6 min — most cost is the agent runs)
    python insights/temporal/ai/eval_slack_repo_selection.py --team-id 1 --user-id 1

    # Iterate fast on cascade-only changes (no LLM at all)
    python insights/temporal/ai/eval_slack_repo_selection.py --team-id 1 --user-id 1 --skip-llm

    # Iterate on Haiku changes without paying for agent runs
    python insights/temporal/ai/eval_slack_repo_selection.py --team-id 1 --user-id 1 --skip-agent

    # Single case
    python insights/temporal/ai/eval_slack_repo_selection.py --team-id 1 --user-id 1 --case vague_code_bug
    ...
    python insights/temporal/ai/eval_slack_repo_selection.py --list-cases
...
# Must run as `python insights/temporal/ai/eval_slack_repo_selection.py` (not `python -m ...`):
# `python -m` would import `insights/temporal/ai/__init__.py` first, which loads workflows that
...
CASES: list[Case] = [
    ...
    Case(
        ...
        text_template="@Insights can you look at {first_repo} and fix the readme typo",
        thread_messages=[{"user": "tester", "text": "@Insights can you look at {first_repo} and fix the readme typo"}],
    ...
    Case(
        ...
        text_template="@Insights how do I update the credit card on our subscription?",
        thread_messages=[{"user": "tester", "text": "@Insights how do I update the credit card on our subscription?"}],
    ...
    Case(
        ...
        text_template="@Insights the dashboard tile filters are not persisting across refreshes",
        thread_messages=[
            {"user": "tester", "text": "@Insights the dashboard tile filters are not persisting across refreshes"}
        ],
        expected_stage="haiku",
        expected_outcome="no_repo",
    ),
    Case(
        ...
        text_template="@Insights the docs site loads really slowly on mobile",
        thread_messages=[
            {"user": "tester", "text": "@Insights the docs site loads really slowly on mobile"},
    ...
    Case(
        ...
        description="App/SDK crash with stack trace — agent should route to the relevant SDK repo (e.g. insights-ios).",
        text_template="@Insights the iOS SDK is crashing on app launch after upgrade to 3.19",
        thread_messages=[
            {"user": "tester", "text": "@Insights the iOS SDK is crashing on app launch after upgrade to 3.19"},
            {"user": "other", "text": "stack trace shows InsightsReplay.start() failing"},
        ],
        expected_stage="agent",
        expected_outcome="found",
    ),
    Case(
        name="insights_product_hang",
        description="Complaint about a Insights product page hanging — looks like 'broken page' but it's the SaaS, not the team's code.",
        text_template="@Insights the trends page hangs forever when I add 10+ series",
        thread_messages=[
            {"user": "tester", "text": "@Insights the trends page hangs forever when I add 10+ series"},
    ...
    Case(
        ...
        description="Insights SDK behaving oddly on the team's own site — the issue is in their code, not in Insights.",
        text_template="@Insights autocapture isn't picking up clicks on our checkout button, other buttons work",
        thread_messages=[
            {
                "user": "tester",
                "text": "@Insights autocapture isn't picking up clicks on our checkout button, other buttons work",
            },
    ...
    Case(
        ...
        description="'Wrong data in Insights' — almost always a tracking bug in the team's own code, not Insights.",
        text_template="@Insights we're not seeing any signup_completed events even though we tested signups today",
        thread_messages=[
            {
                "user": "tester",
                "text": "@Insights we're not seeing any signup_completed events even though we tested signups today",
            },
    ...
    Case(
        ...
        text_template="@Insights there's a bug in how we render user signup, can you fix it",
        thread_messages=[
            {"user": "tester", "text": "@Insights there's a bug in how we render user signup, can you fix it"}
        ],
        expected_stage="agent",
        expected_outcome="found",
    ),
    ...
    Case(
        ...
        text_template="@Insights add a Cancel button to the signup form in the .tsx component",
        thread_messages=[
            {"user": "tester", "text": "@Insights add a Cancel button to the signup form in the .tsx component"}
        ],
        expected_stage="agent",
        expected_outcome="found",
    ),
    Case(
        ...
        text_template="@Insights please refactor the user permission check into a single helper",
        thread_messages=[
            {"user": "tester", "text": "@Insights please refactor the user permission check into a single helper"}
        ],
        expected_stage="agent",
        expected_outcome="found",
    ),
...
class Command:
    ...
    def _print_picker_previews(self) -> None:
        ...
        for label, reason in [
            ("rejected", "Agent returned an invalid repository: insights/foo"),
            (
                "unavailable",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="python insights/temporal/ai/eval_slack_repo_selection.py",
