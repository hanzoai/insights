...
def bind_tables_to_ducklake(database: Any, team_id: int) -> None:
    """Bind a built InsightsQL database's tables to their duckgres-materialized counterparts.

    On the warehouse InsightsQL database, both materialized data-modeling models and
    imported source tables resolve to the Datastore S3 table function (``s3(...)``),
    ...
    Datastore path never calls this, so its table resolution is unchanged.
    """
...
def _bind_materialized_models(database: Any, team_id: int) -> None:
    ...
    for saved_query in materialized:
        try:
            ...
            logger.debug("Model %s not in InsightsQL database; skipping DuckLake bind", saved_query.name)
            continue
        existing = node.table
        if existing is None:
            continue
...
def _bind_source_tables(database: Any, team_id: int) -> None:
    ...
    for table in tables:
        ...
        try:
            ...
            logger.debug("Source table %s not in InsightsQL database; skipping DuckLake bind", table.name)
            continue
        existing = node.table
        if existing is None:
            continue
