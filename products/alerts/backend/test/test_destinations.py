...
class TestSoftDeleteAlertDestinations(APIBaseTest):
    ...
    def test_deletes_alert_destination_with_matching_alert_id(self) -> None:
        destination = self._make_insights_function(template_id="template-slack", alert_id="alert-1")

        soft_delete_alert_destinations(
            team_id=self.team.id,
            alert_id="alert-1",
            allowed_event_ids=ALLOWED_EVENT_IDS,
            insights_function_ids=[destination.id],
        )
    ...
    def test_reports_invalid_ids_and_does_not_delete_any_destinations(self) -> None:
        destination = self._make_insights_function(template_id="template-slack", alert_id="alert-1")
        other = self._make_insights_function(template_id="template-webhook", alert_id="alert-1", event_id="$unrelated_event")
        ...
        with self.assertRaises(ValidationError) as error:
            soft_delete_alert_destinations(
                ...
                alert_id="alert-1",
                allowed_event_ids=ALLOWED_EVENT_IDS,
                insights_function_ids=[destination.id, other.id],
            )

        assert isinstance(error.exception.detail, dict)
        insights_function_id_errors = error.exception.detail["insights_function_ids"]
        assert isinstance(insights_function_id_errors, list)
        assert str(insights_function_id_errors[0]) == (
            f"These InsightsFunctions do not belong to this alert: {other.id}. Refresh the alert and try again."
        )
        for insights_function in (destination, other):
            insights_function.refresh_from_db()
            assert insights_function.deleted is False
            assert insights_function.enabled is True

    def test_deletes_all_destinations_for_alert_only(self) -> None:
        slack_destination = self._make_insights_function(template_id="template-slack", alert_id="alert-1")
        webhook_destination = self._make_insights_function(template_id="template-webhook", alert_id="alert-1")
        other_alert_destination = self._make_insights_function(template_id="template-slack", alert_id="alert-2")
        non_destination = self._make_insights_function(template_id="template-webhook-custom", alert_id="alert-1")
        unrelated_event_destination = self._make_insights_function(
    ...
    def test_reload_happens_after_destination_delete_commits(self, reload_insights_functions_on_workers) -> None:
        destination = self._make_insights_function(template_id="template-slack", alert_id="alert-1")
        ...
        with self.captureOnCommitCallbacks(execute=True):
            ...
            reload_insights_functions_on_workers.assert_not_called()

        reload_insights_functions_on_workers.assert_called_once_with(
            team_id=self.team.id, insights_function_ids=[str(destination.id)]
        )

    @patch("products.alerts.backend.destinations.reload_insights_functions_on_workers", side_effect=RuntimeError("boom"))
    def test_reload_failure_does_not_fail_committed_delete(self, _reload_insights_functions_on_workers) -> None:
        destination = self._make_insights_function(template_id="template-slack", alert_id="alert-1")
