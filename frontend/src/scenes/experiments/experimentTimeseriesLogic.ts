            ...
            null as ExperimentMetricTimeseries | null,
            {
                loadTimeseries: async ({ metric }: { metric: ExperimentMetric }) => {
                    ...
                    const response = await api.get(
                        `v1/projects/@current/experiments/${props.experiment.id}/timeseries_results/?metric_uuid=${metric.uuid}&fingerprint=${metric.fingerprint}`
                    )
                    return response
                },
                ...
                recalculateTimeseries: async ({ metric }: { metric: ExperimentMetric }) => {
                    ...
                    try {
                        const response = await api.createResponse(
                            `v1/projects/@current/experiments/${props.experiment.id}/recalculate_timeseries/`,
                            {
                                metric: metric,
