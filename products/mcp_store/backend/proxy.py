...
def _gateway_decision(policy_context: PolicyContext, tool: MCPServerInstallationTool) -> tuple[str, bool]:
    ...
    auto. needs_approval blocks at the proxy — approval happens in Insights, not
...
def proxy_mcp_request(
    ...
    *,
    ...
    # Forward the full `x-insights-*` namespace so the upstream server can
    # identify the consumer, mode, and version that the caller declared, plus
    # any custom Insights-namespace headers the caller wants to pass through.
    # Without this, the Insights MCP cannot resolve single-exec mode for
    # insights-code-installed Insights MCPs (the consumer header never reaches
    # the resolver) and the `exec` tool comes back as "Tool exec not found".
    # Non-Insights upstreams ignore unknown headers in that namespace.
    for header_name in request.headers:
        if header_name.lower().startswith("x-insights-"):
