...
from insights.schema import ActorsQuery, InsightActorsQuery, TrendsQuery
...
from insights.insightsql_queries.utils.recordings_helper import RecordingsHelper
from insights.models import Team
from insights.models.person.util import _batched_get_distinct_ids_for_persons, _batched_get_persons_by_uuids
...
# Test account filters authored against events (event/element/insightsql/session types) have no
...
class PersonStrategy(ActorStrategy):
    ...
    # Default 101 matches the canonical Datastore person query (groupArray(101)): consumers read
    ...
    def get_actors(
        ...
        result = personinsights_call(
            "get_actors",
            lambda: self._get_actors_via_personinsights(actor_ids, sort_by_created_at_descending, limit_per_person),
        )
    ...
    def _get_actors_via_personinsights(
        self,
        actor_ids,
        ...
        with personinsights_caller_tag("persons/insightsql-actors"):
