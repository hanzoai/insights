...
import type { insightsFlowOutputMappingLogicType } from './insightsFlowOutputMappingLogicType'
import { createExampleEvent, insightsFlowEditorTestLogic } from './testing/insightsFlowEditorTestLogic'

export type OutputMapping = { key: string; result_path: string; spread?: boolean | null }

export function normalizeOutputVariable(raw: InsightsFlowAction['output_variable']): OutputMapping[] {
    if (!raw) {
        return []
    }
...
export const insightsFlowOutputMappingLogic = kea<insightsFlowOutputMappingLogicType>([
    path((key) => [
        'products',
        'workflows',
        'frontend',
    ...
    connect((props: WorkflowLogicProps) => ({
        values: [
            workflowLogic(props),
            ['workflow', 'insightsFunctionTemplatesById'],
            insightsFlowEditorLogic,
            ['selectedNode'],
            insightsFlowEditorTestLogic(props),
    ...
    listeners(({ actions, values, props }) => {
        ...
        return {
            ...
            runOutputTest: async () => {
                const { selectedNode, workflow, insightsFunctionTemplatesById } = values
                if (!selectedNode) {
                    return
                }
                ...
                try {
                    ...
                    const config = sanitizeWorkflow(JSON.parse(JSON.stringify(workflow)), insightsFunctionTemplatesById)
                    ...
                    const result: InsightsFlowTestResult = await api.insightsFlows.createTestInvocation(workflow.id, {
