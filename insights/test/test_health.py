...
from insights.health import is_kafka_connected, logger
...
def test_readyz_accepts_role_streaming_and_stays_ready_without_postgres(client: Client):
    ...
    with simulate_datastore_cannot_connect(), simulate_celery_cannot_connect():
        resp = get_readyz(client=client, role="streaming")
...
def simulate_prestop_marker():
    ...
    with patch("insights.health.os.path.exists", side_effect=mock_exists):
        yield
...
def test_readyz_skips_prestop_check_when_setting_is_empty(client: Client):
    with patch("insights.health.settings.PRESTOP_MARKER_FILE", ""):
        with simulate_prestop_marker():
            resp = get_readyz(client)
...
def test_is_kafka_connected_short_circuits_under_debug_or_test(debug, test):
    ...
    with patch("insights.health.settings.DEBUG", debug), patch("insights.health.settings.TEST", test):
...
def test_is_kafka_connected_returns_false_when_probe_fails(break_producer):
    ...
    with (
        patch("insights.health.settings.DEBUG", False),
        patch("insights.health.settings.TEST", False),
        patch("insights.health.get_producer") as get_producer_mock,
    ):
