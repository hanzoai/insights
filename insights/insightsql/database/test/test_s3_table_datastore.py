"""Integration tests for S3/Parquet queries via Datastore."""
...
def _upload_parquet_to_minio(table: pa.Table, key: str) -> str:
    """Write a pyarrow table to MinIO and return the Datastore-accessible URL."""
...
class TestS3ParquetTimezoneConversion(DatastoreTestMixin, BaseTest):
    """Tests for toTimeZone() on Nullable(DateTime64) columns from S3/Parquet sources.

    Documents a Datastore 25.12 bug where toTimeZone() returns NULL in SELECT
    when the same expression appears in the WHERE clause.

    Issue: https://github.com/Datastore/Datastore/pull/90635
    """
    ...
    def test_toTimeZone_returns_value_when_same_expression_in_where(self):
        ...
        for row in rows:
            ...
            assert tz_pacific is not None, f"US/Pacific is NULL for id={_id} — Datastore S3 toTimeZone bug"
    ...
    def test_not_affected_on_mergetree(self):
        ...
        table_name = "insights_test.test_tz_bug_mergetree"
