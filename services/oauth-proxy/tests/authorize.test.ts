...
describe('handleAuthorize', () => {
    it('shows region picker when no _region param', async () => {
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/authorize/?client_id=abc&redirect_uri=http://localhost:3000/callback&response_type=code'
        )
        ...
        expect(html).toContain('Authenticate with Insights')
    ...
    it('redirects to US authorize with translated client_id and proxy callback when _region=us', async () => {
        ...
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/authorize/?client_id=us_real_id&redirect_uri=http://localhost:3000/callback&response_type=code&_region=us'
        )
        const response = await handleAuthorize(request, mockKV)

        expect(response.status).toBe(302)
        const location = new URL(response.headers.get('location')!)
        expect(location.origin).toBe('https://us.hanzo.ai')
        expect(location.pathname).toBe('/oauth/authorize/')
        expect(location.searchParams.get('client_id')).toBe('us_real_id')
        expect(location.searchParams.get('redirect_uri')).toBe('https://oauth.hanzo.ai/oauth/callback/')
    ...
    it('redirects to EU authorize with translated client_id and proxy callback when _region=eu', async () => {
        ...
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/authorize/?client_id=us_real_id&redirect_uri=http://localhost:3000/callback&response_type=code&_region=eu'
        )
        const response = await handleAuthorize(request, mockKV)

        expect(response.status).toBe(302)
        const location = new URL(response.headers.get('location')!)
        expect(location.origin).toBe('https://eu.hanzo.ai')
        expect(location.pathname).toBe('/oauth/authorize/')
    ...
    it('rejects unregistered redirect_uri to prevent open redirects', async () => {
        ...
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/authorize/?client_id=us_id&redirect_uri=https://attacker.com/steal&response_type=code&state=abc&_region=eu'
        )
        const response = await handleAuthorize(request, mockKV)
    ...
    it('stores region selection and callback redirect_uri keyed by state and client_id', async () => {
        ...
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/authorize/?client_id=us_id&redirect_uri=http://localhost:3000/callback&response_type=code&state=abc123&_region=eu'
        )
    ...
    it('writes bounded-length KV keys even for very large state values', async () => {
        ...
        const request = new Request(
            `https://oauth.hanzo.ai/oauth/authorize/?client_id=us_id&redirect_uri=http://localhost:3000/callback&response_type=code&state=${longState}&_region=us`
        )
        const response = await handleAuthorize(request, mockKV)
    ...
    it('passes redirect_uri through without interception for legacy clients (no stored redirect_uris)', async () => {
        ...
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/authorize/?client_id=us_id&redirect_uri=http://localhost:3000/callback&response_type=code&_region=eu'
        )
        const response = await handleAuthorize(request, mockKV)
    ...
    it('sets security and cache headers on region picker page', async () => {
        const request = new Request('https://oauth.hanzo.ai/oauth/authorize/?client_id=abc&response_type=code')
        const response = await handleAuthorize(request, mockKV)
