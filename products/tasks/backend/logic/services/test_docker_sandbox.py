...
class TestSandboxProviderGuard:
    ...
    def test_modal_evals_provider_uses_dedicated_app(self, mock_settings):
        ...
        assert getattr(sandbox_class, "DEFAULT_APP_NAME", None) == "insights-sandbox-evals"
...
class TestDockerSandboxUnit:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("https://app.hanzo.ai", "https://app.hanzo.ai"),
    ...
    def test_get_local_insights_code_root(self, tmp_path, monkeypatch):
    ...
    def test_create_transforms_callback_url_env_vars(self, mock_exists, mock_run):
        ...
        with patch.object(DockerSandbox, "_get_image", return_value="insights-sandbox-streamlit"):
            DockerSandbox.create(config)
    ...
    def test_create_raises_clear_error_when_image_missing(self, mock_run):
        ...
        assert exc.value.context["image"] == "insights-sandbox-base"
    ...
    def test_clone_repository_branch_flag(self):
        ...
        with patch.object(sandbox, "is_running", return_value=True):
            with patch.object(sandbox, "execute") as mock_execute:
                sandbox.clone_repository("Insights/insights", github_token="test-token", branch=branch)
                command = mock_execute.call_args[0][0]
    ...
    def test_parse_repo_mount_map_valid(self, tmp_path):
        with patch.dict(os.environ, {"SANDBOX_REPO_MOUNT_MAP": f"Insights/insights:{tmp_path}"}):
            result = parse_sandbox_repo_mount_map()
            assert result == {"insights/insights": str(tmp_path)}
    ...
    def test_clone_repository_skipped_when_mounted(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SANDBOX_REPO_MOUNT_MAP": f"Insights/insights:{tmp_path}"}):
            with patch.object(sandbox, "is_running", return_value=True):
                with patch.object(sandbox, "execute") as mock_execute:
                    result = sandbox.clone_repository("Insights/insights", github_token="tok")
                    assert result.exit_code == 0
    ...
    def test_clone_repository_proceeds_when_not_mounted(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch.object(sandbox, "is_running", return_value=True):
                with patch.object(sandbox, "execute") as mock_execute:
                    sandbox.clone_repository("Insights/insights", github_token="tok")
    ...
    def test_create_adds_volume_mounts(self, mock_run, tmp_path):
        ...
        assert f"-v {tmp_path}:/tmp/workspace/repos/insights/insights" in args_str
