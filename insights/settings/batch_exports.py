...
DATASTORE_OFFLINE_5MIN_CLUSTER_HOST: str | None = os.getenv("DATASTORE_OFFLINE_5MIN_CLUSTER_HOST", None)
# Used in internal stage to cap Arrow record batch size
BATCH_EXPORTS_DATASTORE_MAX_INSERT_BLOCK_SIZE_BYTES: int = get_from_env(
    "BATCH_EXPORTS_DATASTORE_MAX_INSERT_BLOCK_SIZE_BYTES", 64 * 1024 * 1024, type_cast=int
)
...
BATCH_EXPORT_INTERNAL_STAGING_BUCKET: str = os.getenv("BATCH_EXPORT_INTERNAL_STAGING_BUCKET", "insights")
# The number of partitions controls how many files Datastore writes to concurrently. Used as the fallback
# when we have no size estimate to drive a dynamic partition count (e.g. the first ever run of an export).
BATCH_EXPORT_DATASTORE_S3_PARTITIONS: int = get_from_env("BATCH_EXPORT_DATASTORE_S3_PARTITIONS", 10, type_cast=int)
# Kill switch for dynamic partition sizing: when disabled, every run uses the static
# BATCH_EXPORT_DATASTORE_S3_PARTITIONS value.
...
BATCH_EXPORT_DATASTORE_S3_TARGET_ROWS_PER_PARTITION: int = get_from_env(
    "BATCH_EXPORT_DATASTORE_S3_TARGET_ROWS_PER_PARTITION", 250_000, type_cast=int
)
BATCH_EXPORT_DATASTORE_S3_MIN_PARTITIONS: int = get_from_env(
    "BATCH_EXPORT_DATASTORE_S3_MIN_PARTITIONS", 1, type_cast=int
)
BATCH_EXPORT_DATASTORE_S3_MAX_PARTITIONS: int = get_from_env(
    "BATCH_EXPORT_DATASTORE_S3_MAX_PARTITIONS", 50, type_cast=int
)
