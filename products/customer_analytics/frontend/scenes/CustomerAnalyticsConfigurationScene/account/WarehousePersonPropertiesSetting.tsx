...
import { IconInfo, IconPencil, IconPlus, IconRefresh, IconTrash } from '@hanzo/icons'
import { Button, Table, TableColumns, Tooltip } from '@hanzo/elements'
...
const TAG_TYPE_BY_SYNC_LEVEL: Record<SourceSyncStatusLevel, TagType> = {
    synced: 'success',
    error: 'danger',
    disabled: 'muted',
    pending: 'default',
...
const TAG_TYPE_BY_RUN_STATUS: Record<string, TagType> = {
    completed: 'success',
    running: 'primary',
    failed: 'danger',
}
...
function ProfilePropertyRuns({ sourceId, labels }: { sourceId: string; labels: ProfileLabels }): JSX.Element {
    ...
    const columns: TableColumns<CustomPropertySyncRunApi> = [
        {
            title: 'Status',
            render: (_, run) => (
                <Tooltip title={run.error ?? undefined}>
                    <Tag type={TAG_TYPE_BY_RUN_STATUS[run.status] ?? 'default'}>{run.status}</Tag>
                </Tooltip>
            ),
        },
...
function WarehouseProfilePropertiesSetting({ targetType }: { targetType: 'person' | 'group' }): JSX.Element {
    ...
    const columns: TableColumns<CustomPropertyDefinitionApi> = [
        {
            title: 'Name',
            dataIndex: 'name',
                ...
                return (
                    <Tooltip title={tooltipTitle}>
                        <span className="flex items-center gap-2">
                            <Tag type={TAG_TYPE_BY_SYNC_LEVEL[status.level]}>{status.label}</Tag>
