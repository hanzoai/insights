...
from insights.api.embedding_worker import DocumentKey, async_get_recently_seen_documents, emit_embedding_request
...
from products.signals.backend.temporal.datastore import execute_insightsql_query_with_retry
...
# wait_for_signal_in_datastore_activity relies on exact key equality with the emit sites.
SIGNAL_DOCUMENT_PRODUCT = "signals"
SIGNAL_DOCUMENT_TYPE = "signal"
SIGNAL_DOCUMENT_RENDERING = "plain"

# For this long, Datastore is polled only when the recently-seen store confirms the
# emission (or on the final attempt) — the wait is store-exclusive to keep Datastore
# load off the polling path. After it, Datastore also polls on the fallback cadence
# below, because the store is best-effort (writes never block ingestion, and the
# in-memory backend is per-pod), so a negative answer can't gate Datastore forever.
CH_CONFIRM_GRACE_PERIOD_SECONDS = 300

# Fallback cadence once the grace period has elapsed without store confirmation: the
# store keeps polling every attempt, Datastore only every Nth — 3x fewer CH queries.
CH_CONFIRM_EVERY_N_ATTEMPTS = 3


def _ensure_tz_aware(value: Union[datetime, str]) -> datetime:
    """Coerce a Datastore timestamp (usually a datetime, occasionally a string) to a tz-aware datetime."""
...
def _deduped_signals_subquery(
    *, include_embedding: bool = False, extra_where: str | None = None, candidate_document_filter: str | None = None
    ...
    if extra_where:
        ...
        # `metadata` as an `argMax(...)` alias. InsightsQL resolves the name in WHERE to that
...
def _signals_for_report_query(*, include_deleted: bool = False, limit: int | None = None) -> str:
    """Build a InsightsQL query that fetches signal rows for a single report.
...
def _parse_signal_row(row: tuple) -> SignalData:
    """Turn a Datastore document embedding row into a SignalData."""
...
def soft_delete_report_signals(report_id: str, team_id: int, team: Team) -> None:
    """
    Soft-delete all Datastore signals for a report by re-emitting them with metadata.deleted=True.
...
async def fetch_signal_type_examples_activity(input: FetchSignalTypeExamplesInput) -> FetchSignalTypeExamplesOutput:
    ...
    try:
        ...
        result = await execute_insightsql_query_with_retry(
...
# ---------------------------------------------------------------------------
# wait_for_signal_in_datastore_activity
# ---------------------------------------------------------------------------


@dataclass
class WaitForDatastoreSignal:
    signal_id: str
    timestamp: datetime
    inserted_at: datetime | None = None


class WaitForDatastoreMode(StrEnum):
    """How much the wait trusts the recently-seen store vs Datastore.

    OPTIMISTIC: return as soon as the store confirms the emission, without querying
    Datastore at all — cheapest and fastest, but blind to the Kafka-to-Datastore
    insert gap. Datastore still polls on the post-grace fallback cadence when the
    store never confirms.

    CH_CONFIRMED: a store confirmation triggers an immediate Datastore confirm, and
    Datastore stays authoritative — the store only decides when to start querying.
...
@dataclass
class WaitForDatastoreInput:
    team_id: int
    signals: list[WaitForDatastoreSignal]
    max_wait_time_seconds: int = 3600
    mode: WaitForDatastoreMode = WaitForDatastoreMode.CH_CONFIRMED


async def _all_signals_recently_seen(team_id: int, signals: list[WaitForDatastoreSignal]) -> bool:
    """Check the embedding worker's recently-seen store for every signal's emission.

    True only when each document is present and, when its current Datastore inserted_at
    is known, the worker emitted it after that version. False on any miss, stale record,
    or store error.

    A True result is not proof of Datastore visibility: "seen" means committed to the
    output Kafka topic. WaitForDatastoreMode decides how much the caller trusts it.
    """
    documents = [
        DocumentKey(
            product=SIGNAL_DOCUMENT_PRODUCT,
    ...
    try:
        ...
        logger.warning(
            "Recently-seen lookup failed; scheduled Datastore fallback remains active",
            team_id=team_id,
            exc_info=True,
        )
        return False
...
@scoped_temporal()
@close_db_connections
async def wait_for_signal_in_datastore_activity(input: WaitForDatastoreInput) -> None:
    """Wait until all emitted signals are processed, or give up after max_wait_time_seconds.

    Two-tier poll, tuned by input.mode (see WaitForDatastoreMode). Every attempt checks
    the embedding worker's recently-seen store (a cheap key-value lookup): a store
    confirmation ends the wait outright in OPTIMISTIC mode, or triggers the Datastore
    ...
    Datastore also polls on every CH_CONFIRM_EVERY_N_ATTEMPTS-th attempt (a third of
    ...
    which precedes the Datastore insert — modes trade that gap against Datastore
    load.

    The Datastore query filters on inserted_at >= (now - 30 minutes) to avoid matching
    ...
    # Widen the timestamp range to account for precision loss (Python microseconds vs Datastore DateTime64(3) milliseconds)
    ...
    for attempt in range(max_attempts):
        ...
        if not store_confirmed:
            ...
            if store_confirmed:
                ...
                if input.mode == WaitForDatastoreMode.OPTIMISTIC:
        ...
        if ch_confirm_due:
            ...
            result = await execute_insightsql_query_with_retry(
                query_type="SignalsWaitForDatastore",
                query=query,
                team=team,
            ...
            if result.results and result.results[0][0] >= expected_count:
                logger.debug(
                    f"All {expected_count} signal(s) found in Datastore after {attempt + 1} attempt(s)",
                    signal_ids=signal_ids,
                    team_id=input.team_id,
                )
                metrics.increment_ch_wait_completion(input.mode.value, "datastore")
                return
    ...
    logger.warning(
        f"Not all signals found in Datastore after {input.max_wait_time_seconds}s, proceeding anyway",
        signal_ids=signal_ids,
        team_id=input.team_id,
    )
...
def fetch_signals_for_report_sync(team: Team, report_id: str) -> list[dict]:
    """Fetch all signals for a report from Datastore, including full metadata. Synchronous."""
...
def fetch_report_ids_for_source_ids(team: Team, source_ids: list[str]) -> dict[str, str]:
    ...
    signal whose Datastore metadata carries both its deterministic `source_id`
