...
use personinsights_coordination::error::Result as CoordResult;
use personinsights_coordination::routing_table::StashHandler;
...
///    `personinsights_router_stash_replayed_total` and covered by the
///    redelivery contract in `personinsights-leader`'s README. A
...
async fn forward_one(
    ...
    if req.possibly_applied {
        metrics::counter!("personinsights_router_stash_replayed_total").increment(1);
    }
...
async fn forward_key_run(
    ...
    while let Some(mut entry) = entries.next() {
        ...
        let Some(disposition) = forwarded else {
            ...
            metrics::counter!(
                "personinsights_router_forward_retries_total",
        ...
        match disposition {
            Disposition::Reply {
                ...
                metrics::histogram!("personinsights_router_stash_wait_duration_ms")
                    .record(entry.item.enqueued_at.elapsed().as_secs_f64() * 1000.0);
                if entry.item.reply.send(response).is_err() {
                    metrics::counter!(
                        "personinsights_router_stash_dropped_total",
                        "reason" => "receiver_gone"
                    )
                    .increment(1);
                }
                metrics::counter!(
                    "personinsights_router_stash_drained_total",
                    "outcome" => label
                )
                .increment(1);
...
impl StashHandler for RouterStashHandler {
    ...
    async fn drain_stash(
        ...
        metrics::histogram!("personinsights_router_stash_drain_batch_size")
            .record(completed_total as f64);
        let drain_ms = drain_start.elapsed().as_secs_f64() * 1000.0;
        metrics::histogram!("personinsights_router_stash_drain_duration_ms").record(drain_ms);
        tracing::info!(
            partition,
            new_owner,
