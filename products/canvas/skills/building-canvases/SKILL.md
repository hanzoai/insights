...
description: >
  Create or edit a Insights canvas — a sandboxed browser application (data board, document, form,
  small tool, graphics experiment) stored in Insights and rendered by the desktop/web app. Use when
...
A canvas is a client-side browser application that runs in a sandboxed iframe inside Insights.
Its source lives in Insights — not in a repository — and you read and write it through the
...
- **React + Quill** — Insights data products, dashboards, forms, application-like state, and anything
  that should look native to Insights. Load the `building-react-quill-canvases` skill.
...
2. Edit the project files. For any Insights data the canvas shows, follow the `querying-canvas-data`
   skill (saved insights loaded via the `ph` SDK — never fetch or your own Insights client), and
   **declare every `ph` call in `project.capabilities`** (insight short ids in
   `capabilities.insights.insights`, captured events in `captureEvents`, `inlineQueries: true` for
