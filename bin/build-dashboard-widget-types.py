...
invoked from ``insightscli build:*``, reading backend SSOT and writing frontend JSON.
...
presets and modal form field picks. Step 2 of ``insightscli build:widget-types`` runs
``generate-widget-config-zod.mjs`` (Orval ``generateReusableSchemas`` on the
catalog OpenAPI slice).

Run via insightscli: `insightscli build:widget-types` (also runs as part of `build:openapi`).
"""
# ruff: noqa: T201
...
def main() -> int:
    ...
    if missing_overrides:
        print(
            ...
            + ". See insights/settings/web.py and python manage.py find_enum_collisions.",
            file=sys.stderr,
        )
        return 1
