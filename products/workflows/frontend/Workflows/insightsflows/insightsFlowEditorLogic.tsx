...
import type { insightsFlowEditorLogicType } from './insightsFlowEditorLogicType'
...
export const insightsFlowEditorLogic = kea<insightsFlowEditorLogicType>([
    ...
    actions({
        ...
        resetFlowFromInsightsFlow: (insightsFlow: InsightsFlow) => ({ insightsFlow }),
    ...
    selectors({
        nodesById: [
            (s) => [s.nodes],
            (nodes): Record<string, InsightsFlowActionNode> => {
                return nodes.reduce(
                    (acc, node) => {
                        acc[node.id] = node
                        return acc
    ...
    listeners(({ values, actions }) => {
        ...
        return {
            ...
            resetFlowFromInsightsFlow: ({ insightsFlow }) => {
                try {
                    const edges: InsightsFlowActionEdge[] = insightsFlow.edges.map((edge) => {
                        const isOnlyEdgeForNode = insightsFlow.edges.filter((e) => e.from === edge.from).length === 1
                        const edgeSourceAction = insightsFlow.actions.find((action) => action.id === edge.from)
                    ...
                    const nodes: InsightsFlowActionNode[] = insightsFlow.actions.map((action: InsightsFlowAction) => {
            ...
            onNodesDelete: ({ deleted }) => {
                ...
                const updatedEdges = values.workflow.edges
                    .map((insightsFlowEdge) => {
                        if (deletedNodeIds.includes(insightsFlowEdge.to)) {
                            // Find the deleted node
                            const deletedNode = deleted.find((node) => node.id === insightsFlowEdge.to)
                            if (deletedNode) {
                        ...
                        return insightsFlowEdge
                    })
                    .filter(
                        (insightsFlowEdge) =>
                            !deletedNodeIds.includes(insightsFlowEdge.from) && !deletedNodeIds.includes(insightsFlowEdge.to)
                    )
    ...
    subscriptions(({ actions }) => ({
        workflow: (insightsFlow?: InsightsFlow) => {
            if (insightsFlow) {
                actions.resetFlowFromInsightsFlow(insightsFlow)
            }
        },
    })),
