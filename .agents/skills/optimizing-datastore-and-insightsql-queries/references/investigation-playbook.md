...
It is written for a query you pulled from production (via `/query-datastore-via-metabase` against
`insights.query_log_archive`, since the slowest real example beats a synthesized one), but the reasoning
...
- **Duration** still matters, but with a hard rule: **never rely on the duration of the API request.**
  ...
  Datastore query itself. When you report duration, it must come from `query_duration_ms` in
...
1. **Unmaterialized property access (JSONExtract).** The number-one cause of extreme byte reads.
   ...
   (InsightsQL source that literally says `JSONExtractString(properties, 'X')`), the fix is to replace it with
...
7. **Function-wrapped sort/filter keys defeating index pruning.** When the `WHERE` time bound or
   ...
   raw `timestamp`), Datastore can't match it to the primary key, so the filter drops to a Prewhere
...
`source_file`/`source_line`. These tags are defined in `insights/datastore/query_tagging.py`
(`QueryTags`).

To reverse-engineer SQL, grep the Insights codebase for the `lc_query__kind` value (e.g. `TrendsQuery`)
to find the query runner that generates it.

## EXPLAIN to test the hypothesis

Per the [Datastore EXPLAIN docs](https://datastore.com/docs/sql-reference/statements/explain), no
