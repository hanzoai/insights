...
import { parseExecCall, parseExecCommand } from '../insightsExecDisplay'
...
export interface QueryResultExtraction {
    content: VisualizationArtifactContent
    /** The MCP server's `_insightsUrl` enrichment — the "open as insight" CTA target. */
    url: string | null
}

/**
 * Maps a query-wrapper tool's output (`{query, results, _insightsUrl}`) onto visualization
...
export function extractQueryResult(message: ToolCallMessage): QueryResultExtraction | null {
    ...
    return {
        ...
        url: asString(output?._insightsUrl) ?? null,
    }
}

/** Dashboard create/update output — the REST payload (`id`, `name`) plus the MCP server's `_insightsUrl` enrichment. */
...
export function extractDashboard(message: ToolCallMessage): DashboardExtraction | null {
    ...
    return {
        ...
        url: asString(output._insightsUrl) ?? asString(output.url),
    }
}
