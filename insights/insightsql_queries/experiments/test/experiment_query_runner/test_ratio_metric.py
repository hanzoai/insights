...
class TestExperimentRatioMetric(ExperimentQueryRunnerBaseTest):
    ...
    def test_ratio_metric_with_insightsql_math_type(self):
        ...
        metric = ExperimentRatioMetric(
            numerator=EventsNode(
                event="purchase", math=ExperimentMetricMathType.INSIGHTSQL, math_insightsql="sum(toFloat(properties.amount))"
            ),
            denominator=EventsNode(
                event="purchase", math=ExperimentMetricMathType.INSIGHTSQL, math_insightsql="uniqExact(distinct_id)"
            ),
        )
    ...
    def test_ratio_metric_with_parametric_aggregation(self):
        ...
        metric = ExperimentRatioMetric(
            numerator=EventsNode(
                ...
                math_insightsql="quantile(0.95)(properties.amount)",
            ),
            denominator=EventsNode(
                event="purchase",
                math=ExperimentMetricMathType.INSIGHTSQL,
                math_insightsql="uniqExact(distinct_id)",  # Count unique users
            ),
        )
