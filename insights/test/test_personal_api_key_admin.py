...
class TestPersonalAPIKeyAdmin(BaseTest):
    @patch("insights.admin.admins.personal_api_key_admin.send_personal_api_key_exposed")
    def test_roll_rolls_key_and_notifies(self, mock_exposed: MagicMock):
        ...
        response = self.client.post(
            f"/admin/insights/personalapikey/{key.pk}/roll/", data={"_roll_url": "https://github.com/some/leak"}
        )

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["Location"], f"/admin/insights/personalapikey/{key.pk}/change/")
