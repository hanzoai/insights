...
``Insights/insights:master`` arrives carrying the downstream forks' open "sync from upstream"
PRs (``Mu-L/insights-1#1379``, ``em3ndez/insights#3``, ...). Taking the first entry unfiltered
...
row hasn't landed in the snapshot yet. Measured over 30 days of ``Insights/insights`` default-branch
...
``splitByChar`` — Datastore rejects an Array nested inside a Nullable); the outer
...
nothing registers it as a global InsightsQL view.
"""
...
def _merged_pr_index(pull_requests_table: str) -> str:
    ...
    only for PRs that landed together, and one SHA in all of ``Insights/insights`` carries three.
    """
    return f"""
        SELECT
            merge_commit_sha,
...
def build_query(table_name: str, *, pull_requests_table: str | None = None, started_floor: bool = False) -> str:
    ...
    # aliases parseDateTimeBestEffort(run_started_at) AS run_started_at, and Datastore alias resolution
    ...
    if pull_requests_table:
        ...
        # An unmatched LEFT JOIN fills the Int with 0, not NULL (InsightsQL doesn't set `join_use_nulls`),
