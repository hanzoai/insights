...
class TestCheckProductAccess:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # review: shared-key auth, models pinned to the review pipeline's constants —
            # the opus-5 outcome judge and the experiment's Codex arm must stay allowed, and
            # anything off the pin list is rejected.
            ("review", "personal_api_key", None, "claude-opus-5", True, None),
            ("review", "personal_api_key", None, "gpt-5.6-sol", True, None),
            ("review", "personal_api_key", None, "claude-3-opus", False, "not allowed"),
            # unknown product
    ...
    def test_slack_app_rejects_deepseek_despite_shared_allowlist(self):
        allowed, error = check_product_access(
            "slack_app", "oauth_access_token", INSIGHTS_CODE_US_APP_ID, "deepseek-ai/deepseek-v4-flash-0731"
        )
