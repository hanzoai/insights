...
// DMAT uses a single string-only column pool; InsightsQL casts to the property's
...
const STATE_TAG_TYPE: Record<MaterializedColumnSlotState, TagType> = {
...
const STATE_TOOLTIP: Record<MaterializedColumnSlotState, string> = {
    ...
    [MaterializedColumnSlotState.READY]: 'Active — InsightsQL reads from the materialized column.',
        ...
        render: function Render(_, slot: MaterializedColumnSlot): JSX.Element {
            ...
            return (
                <Tooltip title={STATE_TOOLTIP[state] ?? ''}>
                    <Tag type={STATE_TAG_TYPE[state] ?? 'default'} className="uppercase">
                        {slot.state}
                    </Tag>
                </Tooltip>
            )
            ...
            return (
                <div className="flex gap-2">
                    {slot.state === MaterializedColumnSlotState.ERROR && (
                        <Button type="secondary" size="small" onClick={() => retrySlot(slot.id)}>
                            Retry
                        </Button>
                    )}
                    <Button
