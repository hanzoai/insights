...
export interface NotebookSQLV2RefApi {
    ...
    node_id: string
    /** What the name resolves to: 'insightsql' is a SQL node's query definition (resolved to its last-run InsightsQL); 'local' is a dataframe a Python node bound in the kernel namespace.
     *
     * * `insightsql` - insightsql
     * * `local` - local */
...
export interface NotebookSQLV2RunRequestApi {
    ...
    node_id: string
    /** Execution kind. 'insightsql' is a SQL node — pushed to Datastore, or rerouted to the sandbox's DuckDB when it references a local frame; 'python' runs the code in the sandbox kernel, materializing referenced upstream nodes as pandas frames first.
     *
     * * `insightsql` - insightsql
     * * `python` - python */
    node_type?: NotebookSQLV2NodeTypeEnumApi
    /** The node's source — SQL for a insightsql node, Python for a python node. Must not be blank. */
    code: string
...
export interface NotebookSQLV2EnvelopeApi {
    ...
    status: string
    /** DuckDB objects a SQL node can SELECT from as of this run, for the schema browser. Only kernel runs (python/duckdb) report these; a insightsql run never enters the kernel. */
    frames?: NotebookSQLV2FrameApi[]
    ...
    /** Result column names. */
    columns?: string[]
    /** Datastore type per column, as [name, type] pairs; used by the visualization tab. */
    types?: string[][]
    /** Number of rows in the result. */
    row_count?: number
    /** Whether Datastore has more rows beyond first_page (detected by fetching limit+1). */
    has_more?: boolean
...
export interface NotebookSQLV2RunStatusResponseApi {
     ...
     */
    error?: string | null
    /** SQL (insightsql) runs only: the full capped row set for client-side paging, present while the query manager's transient result is alive (~20 minutes). Absent afterwards and for kernel (python/duckdb) runs, which keep only the envelope's first_page preview. */
    rows?: unknown[][]
}
