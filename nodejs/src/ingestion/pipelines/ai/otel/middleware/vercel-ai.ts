...
function promoteInsightsContext(props: Record<string, unknown>): void {
    for (const prefix of AI_CONTEXT_PREFIXES) {
        promoteInsightsCustomMetadata(props, prefix)
    }
}
...
function process(event: PluginEvent, next: () => void): void {
    ...
    const insightsDistinctId = getAiContextValue(props, 'insights_distinct_id')
    if (typeof insightsDistinctId === 'string' && insightsDistinctId) {
        if (props['insights_distinct_id'] === undefined) {
            props['insights_distinct_id'] = insightsDistinctId
        }
        event.distinct_id = insightsDistinctId
    }
