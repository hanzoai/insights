...
// tool resolves entity links from, so agents never hand-build (and mis-slug) Insights URLs.
//
...
function buildSandboxScope() {
    ...
    // guards (`isInsightsQLQuery(query)`). Callable and indexable, always returning undefined, so an
...
if (process.argv.includes('--check')) {
    ...
    if (committed !== formatted) {
        process.stderr.write('app-url-manifest.json is stale. Run `pnpm --filter=@hanzo/frontend build:app-urls`.\n')
        process.exit(1)
    }
