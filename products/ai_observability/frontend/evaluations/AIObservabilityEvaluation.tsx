...
import { AccessControlLevel, AccessControlResourceType, ChartDisplayType, InsightsQLMathType } from '~/types'
...
import { EVALUATION_PASSED_INSIGHTSQL, EVALUATION_SUMMARY_MAX_RUNS } from './constants'
import {
...
export function AIObservabilityEvaluation(): JSX.Element {
                          ...
                          series: [
                              {
                                  ...
                                  math_insightsql: `if(countIf(properties.$ai_evaluation_result IS NOT NULL) > 0, countIf(${EVALUATION_PASSED_INSIGHTSQL}) / countIf(properties.$ai_evaluation_result IS NOT NULL) * 100, 0)`,
                                  properties: [
                                      {
                                  ...
                                  ? [
                                        {
                                            ...
                                            math: InsightsQLMathType.InsightsQL as const,
                                            math_insightsql: `if(count() > 0, countIf(properties.$ai_evaluation_result IS NULL) / count() * 100, 0)`,
                                            properties: [
                                                {
                                                    key: '$ai_evaluation_id',
                                                    value: evaluation.id,
    ...
    const handleSave = (): void => {
        if (basicFieldsValid && percentageUnset) {
            focusTriggers()
            toast.warning('Set a sampling percentage between 0.1% and 100% for every condition set before saving.')
            return
        }

        if (!formValid) {
            if (!hasConditions) {
                toast.error('Add at least one condition set before saving.')
            } else if (percentageOutOfRange) {
                toast.error('Sampling percentage must be between 0.1% and 100%.')
            } else {
                toast.error('Some required fields are missing. Please review the configuration.')
            }
            focusTriggers()
            return
        }
        ...
        if (isReportableEvaluation && reportLogic.isMounted() && reportLogic.values.configError) {
            toast.error(reportLogic.values.configError)
            return
        }
                                                        ...
                                                        <>
                                                            {settlingStrategyEnabled && (
                                                                <Field name="settle_strategy" label="Evaluate when">
                                                                    <Select<EvaluationSettleStrategy>
