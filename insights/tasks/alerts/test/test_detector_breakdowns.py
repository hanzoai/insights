...
class TestSimulateDetectorBreakdowns:
    ...
    @patch("products.alerts.backend.evaluation.detector.upgrade_query")
    @patch("products.alerts.backend.evaluation.insightsql.calculate_for_query_based_insight")
    def test_simulates_a_insightsql_insight_through_the_registry(
        ...
        insight = MagicMock(spec=Insight)
        insight.query = {"kind": "InsightsQLQuery", "query": "SELECT value FROM events"}
        team = MagicMock()
