...
import { isInsightsCodeConsumer } from '@/lib/client-detection'
import { ExecCommandError, findRecoverableApiError, InsightsApiError, ToolInputValidationError } from '@/lib/errors'
...
export interface ExecInnerCallProperties {
    ...
    /**
     * HTTP status when the failure was a typed Insights API error — value-free,
...
export interface ExecToolOptions {
     ...
     * Insights Desktop consumer: structuredContent suppressed toward the model, app data
...
const DEPRECATED_TOOL_REDIRECTS: Record<string, (allTools: Tool<ZodObjectAny>[]) => string> = {
    ...
    'read-data-warehouse-schema': () =>
        'Tool "read-data-warehouse-schema" was removed in favor of SQL-based schema discovery. Use "execute-sql" against `system.information_schema.*` (`tables`, `columns`, `relationships`, `data_types`) — it scales to large catalogs and supports filtering/search (e.g. `WHERE description ILIKE \'%...%\'`). Consult the `querying-insights-data` skill for patterns.',
    'entity-search': (allTools) => {
        const base =
            'Tool "entity-search" was removed. Use "execute-sql" to search Insights data via InsightsQL. Consult the `querying-insights-data` skill for system-table patterns (system.insights, system.dashboards, system.cohorts, ...).'
    ...
    'query-generate-insightsql-from-question': () =>
        'Tool "query-generate-insightsql-from-question" was removed. Write the InsightsQL yourself and run it via "execute-sql". Consult the `querying-insights-data` skill for InsightsQL patterns.',
    'query-run': (allTools) => {
        const queryTools = allTools
            .filter((t) => t.name.startsWith('query-'))
            .map((t) => `- ${t.name}: ${t.description.split('\n')[0]}`)
            .join('\n')
        return `Tool "query-run" was removed. Pick the typed query tool that matches your intent, or use "execute-sql" for arbitrary InsightsQL. Available query-* tools:\n${queryTools}`
    },
}
    ...
    if (!tool) {
        ...
        if (scopeGatedTool) {
            throw new ExecCommandError(
                `Tool "${name}" exists, but this MCP connection is missing the required scope(s): ${scopeGatedTool.missingScopes.join(', ')}. Reconnect or reauthorize the Insights MCP connection and approve these scopes. Logging in to Insights in a browser does not update MCP permissions.`,
                'missing_scope'
            )
        }
        throw new ExecCommandError(
...
export function createExecTool(
    ...
    return {
        name: 'exec',
        title: 'Insights analytics, dashboards, insights, feature flags & more',
            ...
            switch (verb) {
                ...
                case 'call': {
                    ...
                    if (!context) {
                        ...
                        throw new Error('Cannot call Insights tools without an API context')
                    }
                    ...
                    try {
                        ...
                        // InsightsValidationError is the API's 400 validation_error body.
                        ...
                        trackInnerCall?.(tool.name, {
                            ...
                            ...(apiError
                                ? { error_status: apiError instanceof InsightsApiError ? apiError.status : 400 }
                                : {}),
                            input,
                        })
                        throw err
                    ...
                    // Insights Desktop (the UI-apps host), emit a full `CallToolResult` payload
                    ...
                    // single-exec callers (direct Claude Code, cline, Slack- and insights_ai-launched
                    // runs, etc.) don't render UI apps — they should see plain text.
                    const isInlineUiAppHost = isInsightsCodeConsumer(mcpConsumer) || options.isInlineExecUiHost === true
                    if (tool._meta?.ui?.resourceUri && isInlineUiAppHost) {
                        ...
                        const payload = markExecPayload(
                            buildToolResultPayload({
                                ...
                                // Inline-exec UI-app hosts (Insights Desktop, Claude Code, Cowork)
                    ...
                    if (useJson) {
                        ...
                        // `results`/`_insightsUrl` payload would otherwise duplicate the table
