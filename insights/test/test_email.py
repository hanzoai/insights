...
from insights.models import Organization, Person, Team, User
from insights.models.instance_setting import override_instance_config
from insights.models.messaging import MessagingRecord, get_email_hash, get_email_hashes
...
class TestEmail(BaseTest):
    ...
    def test_smtp_send_increments_sent_metric(self) -> None:
        # The charts alert reads insights_email_send_total{outcome,transport}; this locks the
        # name + label contract so a removed/mislabelled increment can't silently blind it.
        labels = {"outcome": "sent", "transport": "smtp"}
        before = REGISTRY.get_sample_value("insights_email_send_total", labels) or 0.0

        with override_instance_config("EMAIL_HOST", "localhost"):
            message = EmailMessage(
                campaign_key="metric_test_campaign", subject="Subject", template_name="async_migration_error"
            )
            message.add_recipient("metric-test@hanzo.ai")
            message.send(send_async=False)

        self.assertEqual(len(mail.outbox), 1)
        after = REGISTRY.get_sample_value("insights_email_send_total", labels) or 0.0
    ...
    @parameterized.expand(
        [
            ...
            ("transient_sender_421", smtplib.SMTPSenderRefused(421, b"service unavailable", "from@hanzo.ai"), True),
            (
                "transient_recipients_greylist",
                smtplib.SMTPRecipientsRefused({"x@hanzo.ai": (450, b"greylisted")}),
                True,
            ),
            ...
            (
                ...
                smtplib.SMTPRecipientsRefused({"x@hanzo.ai": (550, b"no such user")}),
                False,
            ),
        ]
    )
    def test_smtp_error_retry_classification(self, name, exc, should_reraise) -> None:
        ...
        before = REGISTRY.get_sample_value("insights_email_send_total", failed_labels) or 0.0
        ...
        with (
            ...
            kwargs: dict[str, Any] = {
                "to": [{"raw_email": f"{name}@hanzo.ai", "recipient": f"{name}@hanzo.ai"}],
        ...
        after = REGISTRY.get_sample_value("insights_email_send_total", failed_labels) or 0.0
        self.assertEqual(after - before, 1.0)
    ...
    def test_smtp_transient_failure_preserves_already_sent_recipients(self) -> None:
        ...
        with (
            ...
            kwargs: dict[str, Any] = {
                "to": [
                    {"raw_email": "first@hanzo.ai", "recipient": "first@hanzo.ai"},
                    {"raw_email": "second@hanzo.ai", "recipient": "second@hanzo.ai"},
                ],
        ...
        first = MessagingRecord.objects.filter(
            email_hash__in=get_email_hashes("first@hanzo.ai"), campaign_key="batch_transient"
        ).first()
        ...
        second = MessagingRecord.objects.filter(
            email_hash__in=get_email_hashes("second@hanzo.ai"), campaign_key="batch_transient"
        ).first()
    ...
    def test_smtp_connection_built_with_bounded_timeout(self) -> None:
        ...
        with (
            ...
            patch("insights.email.import_string", return_value=mock_backend_cls),
            self.settings(EMAIL_BACKEND="some.smtp.Backend"),
        ):
            _send_via_smtp(
                to=[{"raw_email": "t@hanzo.ai", "recipient": "t@hanzo.ai"}],
    ...
    @patch("insights.email.requests.post")
    def test_send_via_http_failed_metric_counts_undelivered_recipients(self, mock_post) -> None:
        ...
        with override_instance_config("EMAIL_HOST", "localhost"), self.settings(CUSTOMER_IO_API_KEY="test-key"):
            _send_via_http(
                to=[
                    {"raw_email": "a@hanzo.ai"},
                    {"raw_email": "b@hanzo.ai"},
    ...
    def test_sanitize_email_properties_preserves_trusted_url_keys(self) -> None:
        # Clean Insights-built URLs survive sanitization unchanged: html.escape is a
        # no-op on URL-shape characters (`:` `/` `.`), and trusted URL keys skip the
        # defang step so the link stays clickable.
        section: dict[str, str] = {"team_url": "https://app.hanzo.ai/project/3", "team_name": "Acme.com"}
        properties: dict[str, Any] = {
            "href": "http://localhost:8010/replay/test#panel=discussion",
            "url": "https://app.hanzo.ai/project/1/insights",
            "site_url": "https://app.hanzo.ai",
            "link": "/reset/uuid/token",
            "next_url": "https://app.hanzo.ai/dashboard",
            "dashboard_url": "https://app.hanzo.ai/dashboard/2",
            "error_tracking_url": "https://app.hanzo.ai/error_tracking",
    ...
    def test_sanitize_email_properties_html_escapes_trusted_url_keys(self) -> None:
        ...
        properties = {
            "href": 'https://app.hanzo.ai/x"><script>alert(1)</script>',
            "verify_link": "https://app.hanzo.ai/x?a=1&b=2",
        }
        ...
        self.assertEqual(
            sanitized["href"],
            "https://app.hanzo.ai/x&quot;&gt;&lt;script&gt;alert(1)&lt;/script&gt;",
        )
        # `&` becomes `&amp;` — correct HTML attribute encoding for query strings.
        self.assertEqual(sanitized["verify_link"], "https://app.hanzo.ai/x?a=1&amp;b=2")
    ...
    def test_all_http_templates_are_registered_in_customer_io_map(self) -> None:
        ...
        # production code under insights/ needs "X" in CUSTOMER_IO_TEMPLATE_ID_MAP.
        ...
        insights_root = Path(insights_pkg.__file__).parent
        sources = sorted(
            p
            for p in insights_root.rglob("*.py")
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(template_name, ast.Constant) and isinstance(template_name.value, str):
                    if template_name.value not in CUSTOMER_IO_TEMPLATE_ID_MAP:
                        missing.setdefault(template_name.value, str(source_path.relative_to(insights_root.parent)))

        self.assertEqual(
            missing,
            {},
            "These template_name values use use_http=True in production code but are missing "
            "from CUSTOMER_IO_TEMPLATE_ID_MAP in insights/email.py. Add a map entry pointing to "
...
class TestMessagingHashSalt(BaseTest):
    ...
    def test_dedup_matches_record_written_under_fallback_salt(self) -> None:
        ...
        with self.settings(MESSAGING_HASH_SALT="old_salt", MESSAGING_HASH_SALT_FALLBACKS=[]):
            MessagingRecord.objects.get_or_create(
                raw_email="rotate@hanzo.ai", campaign_key="c", defaults={"sent_at": timezone.now()}
            )

        # After rotation, with the old salt listed as a fallback, the existing record is
        # found and reused — no duplicate row, no re-send.
        with self.settings(MESSAGING_HASH_SALT="new_salt", MESSAGING_HASH_SALT_FALLBACKS=["old_salt"]):
            record, created = MessagingRecord.objects.get_or_create(raw_email="rotate@hanzo.ai", campaign_key="c")
    ...
    def test_dedup_misses_after_rotation_without_fallback(self) -> None:
        ...
        with self.settings(MESSAGING_HASH_SALT="new_salt", MESSAGING_HASH_SALT_FALLBACKS=[]):
            _, created = MessagingRecord.objects.get_or_create(raw_email="rotate@hanzo.ai", campaign_key="c")
        ...
        with self.settings(MESSAGING_HASH_SALT="new_salt", MESSAGING_HASH_SALT_FALLBACKS=["old_salt"]):
            self.assertTrue(
                ...
                MessagingRecord.objects.filter(  # type: ignore[misc]
                    raw_email="rotate@hanzo.ai", campaign_key="c", sent_at__isnull=False
                ).exists()
            )
...
async def test_aget_or_create_reuses_record_written_under_fallback_salt() -> None:
    with override_settings(MESSAGING_HASH_SALT="old_salt", MESSAGING_HASH_SALT_FALLBACKS=[]):
        original, created = await MessagingRecord.objects.aget_or_create(
            raw_email="async@hanzo.ai", campaign_key="c", defaults={"sent_at": timezone.now()}
        )
        assert created
    ...
    with override_settings(MESSAGING_HASH_SALT="new_salt", MESSAGING_HASH_SALT_FALLBACKS=["old_salt"]):
        found, created = await MessagingRecord.objects.aget_or_create(raw_email="async@hanzo.ai", campaign_key="c")
        assert not created
...
async def test_aget_or_create_writes_under_primary_salt_not_a_fallback() -> None:
    with override_settings(MESSAGING_HASH_SALT="primary_salt", MESSAGING_HASH_SALT_FALLBACKS=["other_salt"]):
        record, created = await MessagingRecord.objects.aget_or_create(raw_email="async@hanzo.ai", campaign_key="c")
        assert created
        assert record.email_hash == hashlib.sha256(b"primary_salt_async@hanzo.ai").hexdigest()
...
async def test_aget_or_create_without_fallback_creates_duplicate_after_rotation() -> None:
    ...
    with override_settings(MESSAGING_HASH_SALT="new_salt", MESSAGING_HASH_SALT_FALLBACKS=[]):
        _, created = await MessagingRecord.objects.aget_or_create(raw_email="async@hanzo.ai", campaign_key="c")
        assert created
