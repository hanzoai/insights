...
from insights.schema import ReleaseStatus, SourceFieldInputConfig, SourceFieldInputConfigType
...
class TestLemonSqueezySource:
    ...
    def test_get_source_config(self):
        ...
        assert config.docsUrl == "https://hanzo.ai/docs/cdp/sources/lemon-squeezy"
    ...
    def test_create_webhook_delegates(self, mock_create):
        self.source.create_webhook(self.config, "https://us.hanzo.ai/webhooks/abc", self.team_id)
        mock_create.assert_called_once_with("test-api-key", "https://us.hanzo.ai/webhooks/abc")

    @mock.patch(f"{API_CLIENT_PATCH}.delete_webhook")
    def test_delete_webhook_delegates(self, mock_delete):
        self.source.delete_webhook(self.config, "https://us.hanzo.ai/webhooks/abc", self.team_id)
        mock_delete.assert_called_once_with("test-api-key", "https://us.hanzo.ai/webhooks/abc")

    @mock.patch(f"{API_CLIENT_PATCH}.sync_webhook_events")
    def test_sync_webhook_events_passes_desired_events(self, mock_sync):
        self.source.sync_webhook_events(self.config, "https://us.hanzo.ai/webhooks/abc", self.team_id, ["orders"])
        mock_sync.assert_called_once_with(
            "test-api-key", "https://us.hanzo.ai/webhooks/abc", sorted(SCHEMA_TO_WEBHOOK_EVENTS["orders"])
        )
