...
# Each team's compute activity runs a heavy embedding scan against Datastore, so keep the
...
COMPUTE_ACTIVITY_TIMEOUT = timedelta(seconds=300)  # Two 120s Datastore query caps plus clustering
...
AGGREGATES_HEARTBEAT_TIMEOUT = timedelta(seconds=60)  # 1 minute - external calls to Datastore
...
# When Datastore reports it is at capacity (too many simultaneous queries), an immediate retry
...
DATASTORE_AT_CAPACITY_BACKOFF_MIN = timedelta(seconds=20)
DATASTORE_AT_CAPACITY_BACKOFF_MAX = timedelta(seconds=60)
