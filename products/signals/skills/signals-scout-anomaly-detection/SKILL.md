...
compatibility: >
  Runs as the Insights Signals scout in a Claude sandbox with read-only analytics scopes
...
- `alert-simulate` — primary scorer: run Insights's anomaly detectors on a saved insight's series (no alert required); returns per-point scores + triggered dates.
