...
1. A Celery worker executes the InsightsQL and holds the full result as Python tuples.
...
- **Datastore resource-hold.** A `SELECT` counts as running until the client drains the last block.
...
### Datastore writes S3 directly (`INSERT INTO FUNCTION s3(...)`)
...
| Query-log exports (Dagster ×2) — `insights/dags/export_query_logs_to_s3.py:98`                                           | No: internal ops, daily schedule                                                                                                                   | `s3_truncate_on_insert=1` for idempotent re-runs; per-host fan-out                                                                                                                                                                                     |
| Duckling backfill — `insights/dags/events_backfill_to_duckling.py:1420`                                                  | No: operator-run migration tooling                                                                                                                 | Dynamic fan-out sizing (target rows/file), bounded CH writer memory via Parquet row-group settings                                                                                                                                                     |
...
| Web analytics Native export — `insights/models/web_preaggregated/sql.py:835`                                             | No (write side appears dormant)                                                                                                                    | Native format preserves AggregateFunction states across the S3 round-trip                                                                                                                                                                              |

### Python/Temporal worker streams CH results and uploads

| Flow                                                                                                        | User-facing?                                                                                          | What it teaches us                                                                                                                                                                                                                                 |
| ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Data-modeling materialization** — `insights/temporal/data_modeling/activities/materialize_view.py:346-598` | Yes: user creates a saved query / materialized view (Data warehouse), runs it manually or on schedule | InsightsQL → `astream_query_as_arrow` in bounded ~100MB batches → Delta/Parquet on S3 via delta-rs; **CH reads the result back** through `s3()`/`s3Cluster()`/`deltaLake()` table functions — proof that CH→S3→query round-trips are production reality |
...
| Usage reports v2 — `insights/temporal/usage_report/activities.py:105`                                        | No: automated billing                                                                                 | Staging many small CH results as S3 objects with explicit best-effort cleanup                                                                                                                                                                      |

### Relevant negative

There is **no generic S3-backed query-result cache** — the query cache (`insights/query_cache/`) is Redis-only.
...
- **Egress.** The sandbox already makes outbound HTTPS calls to the Insights API; fetching a presigned S3 URL adds
...
## Resource governance — not hammering Datastore
...
- **Dedicated Datastore user as the backstop.** The repo already splits traffic across CH users
  (`DatastoreUser.APP` / `API`), each governed by a server-side settings profile. A materialization user gets
...
global 10 / per-team 2): the activity prints the InsightsQL through the guarded executor under
...
Datastore and atomically overwrites the same key, which is exactly why a re-run after new events lands always
...
Stream integrity: Datastore sends `200 OK` before execution finishes, so a mid-stream failure can't change
...
Datastore error from `system.query_log` — a confirmed query-side exception is terminal (no doomed
...
- **Format: one plain Arrow IPC stream object per frame — not Delta, not Parquet.** Delta earns its machinery
  when Datastore re-reads a versioned, overwritten _table_ (data modeling's case). A frame key is a slot
...
**Phase 2 — let Datastore do the writing (only if the data says so).**
Replace the worker-streamed upload with `INSERT INTO FUNCTION s3(...'ArrowStream')` (batch-exports recipe):
the worker's job shrinks to issuing one statement; zero result bytes transit Insights Python.
...
- **Security path.** Phase 1 keeps the whole InsightsQL runner path (team scoping, property access controls applied
  ...
  but over its own known tables, not arbitrary user InsightsQL. That seam deserves its own review, not a ride-along.
...
- **Load placement.** Worker streaming actually puts _less_ work on Datastore — CH only executes and streams
...
- Whether phase 1 should also carry SQLV2 (insightsql-node) first pages, or stay materialization-only
