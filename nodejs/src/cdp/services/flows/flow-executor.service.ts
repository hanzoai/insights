...
export function createFlowInvocation(
    globals: InsightsFunctionInvocationGlobals,
    flow: Flow,
    filterGlobals: InsightsFunctionFilterGlobals
): CyclotronJobInvocationFlow {
    // Build default variables from flow, then merge in any provided in globals.variables
    ...
    return {
        ...
        state: {
            ...
            // that actually sent (see InsightsFunctionHandler); until then this is the best answer.
...
export class FlowExecutorService {
    private readonly actionHandlers: Record<FlowAction['type'], ActionHandler>
    private readonly duplicateObserver: FlowDuplicateObserverService | null
    ...
    constructor(
        ...
        duplicateObserver?: FlowDuplicateObserverService
    ) {
        this.flowFunctionsService = flowFunctionsService
    ...
    async getSensitiveValues(flow: Flow): Promise<string[]> {
        return this.flowFunctionsService.getSensitiveValues(flow)
    }

    async buildFlowInvocations(
        flows: Flow[],
        triggerGlobals: InsightsFunctionInvocationGlobals
    ): Promise<{
        invocations: CyclotronJobInvocationFlow[]
        metrics: MinimalAppMetric[]
        logs: LogEntry[]
    }> {
        const metrics: MinimalAppMetric[] = []
        const logs: LogEntry[] = []
        const invocations: CyclotronJobInvocationFlow[] = []
        ...
        // FlowInvocationPipeline). Flows that reach this loop are assumed to be source-compatible
    ...
    async execute(
        ...
        let result: CyclotronJobInvocationResult<CyclotronJobInvocationFlow> | null = null
        const metrics: MinimalAppMetric[] = []
        ...
        while (!result || !result.finished) {
            const nextInvocation: CyclotronJobInvocationFlow = result?.invocation ?? invocation
            ...
            if (this.shouldEndFlowExecution(result, logs)) {
                break
            }
        }
    ...
    private shouldEndFlowExecution(
        ...
        if (result.error) {
            ...
            const lastExecutedAction = result.invocation.flow.actions.find((a) => a.id === lastExecutedActionId)
    ...
    private async shouldExitEarly(
    ...
    ): Promise<CyclotronJobInvocationResult<CyclotronJobInvocationFlow> | null> {
        let earlyExitResult: CyclotronJobInvocationResult<CyclotronJobInvocationFlow> | null = null

        const { flow, person } = invocation
        ...
        if (flow.trigger.type === 'event' && flow.trigger.filters && person) {
            const filterResult = await filterFunctionInstrumented({
                fn: flow,
                filters: flow.trigger.filters,
                filterGlobals: invocation.filterGlobals,
            })
            triggerMatch = filterResult.match
        }
        if (flow.conversion?.filters?.length && person) {
            if (flow.conversion.bytecode?.length) {
                const filterResult = await filterFunctionInstrumented({
                    fn: flow,
                    filters: {
                        bytecode: flow.conversion.bytecode || [],
                        properties: flow.conversion.filters || [],
                    },
                    filterGlobals: invocation.filterGlobals,
                })
                ...
                logger.error(
                    'FlowExecutorService: Conversion filters are set but no bytecode is provided. This means we cannot evaluate the conversion filters to determine if we should exit the flow.',
                    { flowId: flow.id }
                )
            }
        }
        ...
        if (propertyConversionMatched && !invocation.state.conversionCounted) {
            ...
            conversionMetric = {
                ...
                app_source_id: invocation.parentRunId ?? flow.id,
                instance_id: flow.id,
                metric_kind: 'other',
        ...
        if (shouldExit) {
            earlyExitResult = createInvocationResult<CyclotronJobInvocationFlow>(invocation)
            earlyExitResult.finished = true
            earlyExitResult.logs.push({
                level: 'info',
                timestamp: DateTime.now(),
                message: `Workflow exited early due to exit condition: ${flow.exit_condition} (${exitReason})`,
            })
    ...
    public async executeCurrentAction(
        ...
        try {
            ...
            logger.debug('🦔', `[FlowActionRunner] Running action ${currentAction.type}`, {
                flowId: invocation.flow.id,
            ...
            if (this.isLiveEditWorkflowChange(err, invocation)) {
                ...
                result.metrics.push({
                    team_id: invocation.flow.team_id,
                    app_source_id: invocation.parentRunId ?? invocation.flow.id,
            ...
            logger.error(
                '🦔',
                `[FlowExecutor] Error executing script flow ${invocation.flow.id} - ${invocation.flow.name}. Event: '${invocation.state.event?.url}'`,
                err
            )
        }
    ...
    private maybeRedirectDeletedAction(
        ...
        const redirectId = deletedActionId ? invocation.flow.action_redirects?.[deletedActionId] : undefined
        ...
        const target = invocation.flow.actions.find((action) => action.id === redirectId)
        if (!target) {
            counterRedirectTargetMissing.inc()
            logger.warn('[FlowExecutor] Redirect target missing from live graph', {
                flowId: invocation.flow.id,
                deletedActionId,
                redirectId,
            })
    ...
    private isLiveEditWorkflowChange(err: unknown, invocation: CyclotronJobInvocationFlow): boolean {
        ...
        const updatedAt = invocation.flow.updated_at ? new Date(invocation.flow.updated_at).getTime() : null
    ...
    ): CyclotronJobInvocationResult<CyclotronJobInvocationFlow> {
        result.finished = false
    ...
    private trackActionMetric(
        ...
        result.metrics.push({
            team_id: result.invocation.flow.team_id,
            app_source_id: result.invocation.parentRunId ?? result.invocation.flow.id,
    ...
    private logExecutionTriggerInfo(invocation: CyclotronJobInvocationFlow): MinimalLogEntry {
        ...
        const isWebhookTriggered = ['webhook', 'manual', 'schedule'].includes(invocation.flow.trigger.type)
        const isBatchWorkflow = invocation.flow.trigger.type === 'batch'
    ...
    private logExecutionErrorInfo(
        ...
        const currentAction = currentActionId ? invocation.flow.actions.find((a) => a.id === currentActionId) : null
