"""Tests for the undelete-repair version writes via personinsights RPC.

Covers the two personinsights RPC writes in the repair flow:
- _update_distinct_id_in_postgres → SetPersonDistinctIdVersionFloor
- _set_person_version_floor (used by _reset_person_in_datastore) → SetPersonVersionFloor
"""
...
from insights.models.person.deletion import _set_person_version_floor, _update_distinct_id_in_postgres
...
class TestUpdateDistinctIdInPostgresRPC(SimpleTestCase):
    def test_personinsights_path_returns_converted_person(self):
    ...
    def test_personinsights_returns_none_when_distinct_id_absent(self):
...
class TestSetPersonVersionFloorRPC(SimpleTestCase):
    def test_personinsights_path_calls_rpc_with_floor(self):
        with fake_personinsights_client() as fake:
