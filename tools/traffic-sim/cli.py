#!/usr/bin/env python3
# ruff: noqa: T201
"""Drive a real browser at a URL and verify Insights instrumentation.
...
Three scenarios:
  ...
  check-loading   Inspect how the Insights snippet is loaded across pages.
...
() => {
    ...
    for (const s of insightsScripts) {
        ...
        const m = text.match(/insights\.init\s*\(\s*['"]([^'"]+)['"]\s*,\s*(\{[\s\S]*?\})\s*\)/);
        if (m) {
            try {
    ...
    const arrScript = insightsScripts.find(s => s.src && s.src.includes('array'));
    const defined = typeof window.insights !== 'undefined';
    const runtimeState = defined && window.insights
        ? {
            defined: true,
            loaded: !!window.insights.__loaded,
            distinct_id: window.insights.get_distinct_id ? window.insights.get_distinct_id() : null,
            config_api_host: window.insights.config ? window.insights.config.api_host : null,
        }
        : {defined: false, loaded: false};
    let loadMethod = 'none';
    if (runtimeState.loaded && hasInsightsInitId) loadMethod = 'head_snippet';
...
class VisitResult:
    ...
    insights_requests: list[AnalyticsRequest] = field(default_factory=list)
    console_lines: list[ConsoleLine] = field(default_factory=list)
    error: str | None = None


class AnalyticsCapture:
    """Captures Insights network requests and console output from a page."""

    def __init__(self, insights_domains: tuple[str, ...], verbose: bool = False):
        self.insights_domains = insights_domains
        self.verbose = verbose
        self.insights_requests: list[AnalyticsRequest] = []
    ...
    def _is_insights(self, url: str) -> bool:
        # Match on parsed hostname rather than substring so URLs that merely
        # contain a Insights domain in their path (e.g. an open-redirect) are
        # not misclassified as Insights ingestion traffic.
        try:
            host = (urlparse(url).hostname or "").lower()
        except ValueError:
            return False
        return any(host == d or host.endswith("." + d) for d in self.insights_domains)

    def _on_request(self, request) -> None:
        if not self._is_insights(request.url):
            return
        ...
        if self.verbose:
            ...
            print(f"    [Insights] {request.method} {_short_url(request.url)} events=[{tags}]")

    def _on_response(self, response) -> None:
        if not self._is_insights(response.url):
            return
        req = self._pending.pop(id(response.request), None)
        if req is None:
            return
        req.status = response.status
        self.insights_requests.append(req)
        if self.verbose:
            marker = "✓" if response.status in (200, 204) else "✗"
            print(f"    [Insights] {marker} {response.status} {_short_url(response.url)}")

    def _on_console(self, msg) -> None:
        text = msg.text
        is_insights = "insights" in text.lower() or "[Insights]" in text
        line = ConsoleLine(
            timestamp=datetime.now(UTC).isoformat(),
            type=msg.type,
            text=text,
        ...
        if self.verbose and is_insights:
...
def _redact_api_key(value: str | None) -> str:
    # Insights project tokens are public by design, but CodeQL flags raw-token
...
def extract_insights_events_from_console(lines: list[ConsoleLine]) -> list[str]:
    pattern = re.compile(r'\[Insights\.js\] send "(\$?\w+)"')
    return [m.group(1) for line in lines if line.is_insights for m in pattern.finditer(line.text)]


def add_tracking_params(url: str, run_id: str, scenario: str) -> str:
    parsed = urlparse(url)
    params = parse_qs(parsed.query)
    params["__insights_debug"] = ["true"]
...
def _build_browserstack_cdp_url() -> str:
    ...
    caps = {
        ...
        "os": "Windows",
        "os_version": "11",
        "name": "Insights instrumentation check",
...
async def _report_cloud(page: Page, capture: AnalyticsCapture) -> None:
    events = extract_insights_events_from_console(capture.console_lines)
    counts = Counter(events)
    pageviews = counts.get("$pageview", 0)
    breakdown = ", ".join(f"{k}: {v}" for k, v in sorted(counts.items())) or "none"
    await _annotate_browserstack(page, f"Insights events — {breakdown}")
    passed = pageviews > 0
        ...
        try:
            for i in range(visits):
                ...
                result = await _do_visit(
                    ...
                    insights_domains=insights_domains,
                )
...
async def _do_visit(
    *,
    ...
    try:
        ...
        capture = AnalyticsCapture(insights_domains=insights_domains, verbose=verbose)
        capture.attach(page)
        ...
        result.insights_requests = capture.insights_requests
...
def _print_visit_summary(result: VisitResult) -> None:
    ...
    network_events = [e for r in result.insights_requests for e in r.events]
    events = network_events or extract_insights_events_from_console(result.console_lines)
    counts = Counter(events)
    breakdown = ", ".join(f"{k}: {v}" for k, v in sorted(counts.items())) or "none"
    success = sum(1 for r in result.insights_requests if r.status in (200, 204))
    print(f"  Page load: {load}")
    print(f"  Insights requests: {len(result.insights_requests)} ({success} OK)")
    print(f"  Insights events: {breakdown}")
    if result.error:
...
def _summary_dict(results: list[VisitResult]) -> dict:
    return {
        "insights_requests": sum(len(r.insights_requests) for r in results),
        "insights_requests_ok": sum(sum(1 for req in r.insights_requests if req.status in (200, 204)) for r in results),
...
def resolve_insights_domains(host: str) -> tuple[str, ...]:
    """Build the set of hostnames we treat as Insights network traffic.

    Always includes the cloud defaults (hanzo.ai / i.hanzo.ai) so events
    sent to the Insights cloud are caught even when --insights-host points at
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="traffic-sim",
        description="Drive a real browser at a URL and verify Insights instrumentation.",
...
""",
    ...
    cl = sub.add_parser("check-loading", help="Inspect how the Insights snippet is loaded.")
    ...
    for s in (nu, ru, cl):
        ...
        s.add_argument("--verbose", action="store_true", help="Print every Insights request and console line.")
...
async def main_async(argv: list[str] | None = None) -> int:
    ...
    insights_domains = resolve_insights_domains(args.insights_host)

    print("Insights Traffic Simulation")
    print(f"Scenario: {args.scenario}")
    print(f"Run ID: {run_id}")
    print(f"URLs: {len(urls)}")
    print(f"Insights host: {args.insights_host}")
    ...
    print(f"Insights requests: {summary['insights_requests']} ({summary['insights_requests_ok']} OK)")
