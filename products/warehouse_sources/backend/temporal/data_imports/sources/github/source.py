...
from insights.models.integration import GitHubIntegration, GitHubIntegrationError
...
If automatic creation failed, your token needs webhook permissions — the **admin:repo_hook** scope on a classic token, or **Repository webhooks: read and write** on a fine-grained token. Add it and reconnect, or set the webhook up manually using the steps above.""",
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            # The GitHub App isn't configured on this Insights instance, so an OAuth source can't mint
            # the App JWT to refresh its installation token. Deterministic — retrying never resolves it.
            "GITHUB_APP_CLIENT_ID is not configured": "The GitHub App is not configured on this Insights instance. Please contact support.",
            "GITHUB_APP_PRIVATE_KEY is not configured": "The GitHub App is not configured on this Insights instance. Please contact support.",
    ...
    def webhook_mapping_key(self, schema_name: str) -> str:
        ...
        # `owner/repo.event` so two repos' rows for the same event don't collide. The script template
    ...
    def create_webhook(
        ...
        # hook's config.secret, and return it via extra_inputs so it lands on the script function for
        # signature verification. (Contrast Stripe, which generates and returns its own secret.)
        # One secret is shared by every repo's hook — the source has a single script function with a
        ...
        # script function anyway, so over-subscribing is harmless while enabling a table later is free.
        events = self.get_desired_webhook_events(config, list(GITHUB_WEBHOOK_RESOURCE_MAP.keys())) or []
    ...
    def sync_webhook_events(
        ...
        # (over-subscribing is harmless, unmapped events no-op in the script function) and auto-heals
