...
import { Banner, Table, Tag } from '@hanzo/elements'
...
import {
    ...
    InsightsQLAlertPreviewRow,
} from 'products/alerts/frontend/logic/insightsqlAlertPreview'
...
export function InsightsQLAlertPreviewRowsTable({
    preview,
}: {
    preview: Extract<InsightsQLAlertPreview, { status: 'ok' }>
}): JSX.Element | null {
                ...
                columns={[
                    {
                        ...
                        render: (_, row: InsightsQLAlertPreviewRow) => row.label,
                    },
                    {
                        title: preview.columnName ?? 'Value',
                        render: (_, row: InsightsQLAlertPreviewRow) =>
                    ...
                    ...(showStatus
                        ? [
                              {
                                  title: '',
                                  render: (_: unknown, row: InsightsQLAlertPreviewRow, index: number) =>
                                      index === evaluatedIndex || isAnyRow ? (
                                          row.breaching ? (
                                              <Tag type="danger">Would fire</Tag>
                                          ) : (
                                              <Tag type="success">Would not fire</Tag>
                                          )
                                      ) : null,
                              },
                          ]
...
function getInsightsQLPreviewBannerCopy(preview: Exclude<InsightsQLAlertPreview, { status: 'ok' }>): ReactNode {
    switch (preview.status) {
        ...
        case 'too-many-rows':
            return (
                <>
                    Any-row alerts evaluate at most {INSIGHTSQL_ANY_ROW_MAX_ROWS} rows, but the query returns{' '}
...
export function InsightsQLAlertPreviewBanner({
    preview,
    conditionType,
}: {
    preview: InsightsQLAlertPreview | null
    ...
    if (preview.status !== 'ok') {
        return (
            <Banner type={preview.status === 'no-rows' ? 'info' : 'warning'}>
                {getInsightsQLPreviewBannerCopy(preview)}
            </Banner>
        )
    }
    ...
    if (preview.mode === 'any_row') {
        ...
        return (
            <InsightsQLPreviewStatus wouldFire={breaching !== null ? breaching > 0 : null}>
                {breaching !== null ? (
                    <>
    ...
    return (
        <InsightsQLPreviewStatus wouldFire={wouldFire}>
