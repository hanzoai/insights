...
through the same Datastore DESCRIBE that computes stored endpoint columns). The caller is
...
from insights.llm.gateway_client import Product, get_llm_client
...
def build_system_prompt(conditions_source: str) -> str:
    return "\n".join(
        [
            "You are an expert in InsightsQL, Insights's SQL dialect (closely following Datastore SQL). "
            "Your job is to rewrite an endpoint's SQL query so Insights can materialize it, WITHOUT "
...
def _validate_suggestion(
    ...
    *,
    ...
    come from the same Datastore DESCRIBE machinery, which also proves the rewrite compiles.
    """
...
def suggest_materialization_fix(
    *,
    ...
    the query is a InsightsQLQuery that currently fails the materialization checks — the view enforces
    both.
    """
