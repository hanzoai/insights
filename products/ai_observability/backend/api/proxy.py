...
from insights.api.streaming import SyncIterableToAsync
...
class LLMProxyViewSet(viewsets.ViewSet):
    ...
    def _handle_completion_request(self, request: Request) -> HttpResponseBase | Response:
        ...
        try:
            ...
            # Enforce playground model allowlist when using Insights-funded keys
            if provider_key is None and model not in PLAYGROUND_MODEL_IDS:
                return Response(
                    {
                        "error": f"Model '{model}' is not available on the Insights-funded playground. Please add your own API key to use this model."
                    },
                    status=403,
                )
    ...
    def models(self, request):
        ...
        Otherwise, returns only playground-eligible models (Insights pays for these).
        """
        ...
        # Default: return only playground-eligible models (Insights pays for these)
