...
export function signAwsRequest({
    ...
    // Insights jest pattern) flow through to the signature timestamp.
 ...
 * InsightsFunction's inputs.
 *
 * `secret: true` InsightsFunction inputs land in `encrypted_inputs` after Django's
...
export function resolveAwsSigV4Credentials(
    ...
    insightsFunction: Pick<InsightsFunctionType, 'inputs' | 'encrypted_inputs'>
): ResolvedAwsSigV4Credentials {
    const lookup = (key: string): unknown =>
        insightsFunction.encrypted_inputs?.[key]?.value ?? insightsFunction.inputs?.[key]?.value
    ...
    if (missing.length > 0) {
        return {
            ok: false,
            error: `AWS SigV4 signing failed: input(s) ${missing.join(', ')} not found on script function or not a string. Refusing to send an unsigned request to AWS.`,
        }
    }
