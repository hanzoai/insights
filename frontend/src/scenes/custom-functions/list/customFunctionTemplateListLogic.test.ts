...
import { customFunctionTemplateListLogic } from './customFunctionTemplateListLogic'

describe('customFunctionTemplateListLogic - configuration structure', () => {
    beforeEach(() => {
        initKeaTests()
    })
    ...
    it('should wrap filters in configuration object, not spread at top level', () => {
        ...
        const logic = customFunctionTemplateListLogic.build({
            type: 'destination',
            getConfigurationOverrides,
        })
        logic.mount()

        const template: CustomFunctionTemplateWithSubTemplateType = {
            id: 'template-slack',
            name: 'Slack',
            type: 'destination',
            status: 'stable',
