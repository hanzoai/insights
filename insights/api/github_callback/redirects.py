...
class _AppDeepLinkRedirect(HttpResponseRedirect):
    """Redirect that also permits the mobile app's custom ``insights://`` scheme."""

    allowed_schemes = [*HttpResponseRedirect.allowed_schemes, "insights"]
...
def redirect_from_finish_result(result: FinishResult) -> HttpResponseRedirect:
    ...
    if result.redirect_kind == "personal_finish":
        app_base_urls: dict[str, str] = {
            "insights_mobile": MOBILE_GITHUB_CALLBACK_URL,
            "insights_code": ACCOUNT_CONNECTED_GITHUB_INTEGRATION_PATH,
