...
from insights.session.middleware import SessionRiskMiddleware
from insights.session.models import Session
from insights.session.risk import Context, RiskFlags, RiskTier, _baseline_for_session, evaluate_session_risk
...
class TestEvaluateSessionRisk(BaseTest):
    ...
    @patch("insights.session.risk.current_request_context", return_value=IMPOSSIBLE_TRAVEL_CTX)
    @patch("insights.session.risk.hanzo_insights.capture")
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(False, False, False))
    ...
    @patch("insights.session.risk.current_request_context", return_value=HIGH_RISK_CTX)
    @patch("insights.session.risk.hanzo_insights.capture")
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, False, False))
    ...
    @patch("insights.session.risk.current_request_context", return_value=UA_CHANGE_CTX)
    @patch("insights.session.risk.hanzo_insights.capture")
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, False))
    ...
    @patch("insights.session.risk.current_request_context", return_value=HIGH_RISK_CTX)
    @patch("insights.session.risk.hanzo_insights.capture")
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, False, True))
    ...
    @patch("insights.session.risk.current_request_context", return_value=IMPOSSIBLE_TRAVEL_CTX)
    @patch("insights.session.risk.hanzo_insights.capture")
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, True))
    ...
    @patch("insights.session.risk.current_request_context", return_value=HIGH_RISK_CTX)
    @patch("insights.session.risk.hanzo_insights.capture", side_effect=RuntimeError("telemetry down"))
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, False, True))
    ...
    def test_persistent_anomaly_emits_once_per_cooldown(self, _name, cooldown, expected_calls):
        ...
        with (
            ...
            patch("insights.session.risk.current_request_context", return_value=UA_CHANGE_CTX),
            patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, False)),
            patch("insights.session.risk.hanzo_insights.capture") as mock_capture,
        ):
            evaluate_session_risk(request)
            evaluate_session_risk(request)
    ...
    def test_new_signature_re_emits_within_cooldown(self, _flags, mock_capture):
        ...
        request = self._authed_request_with_baseline(self._make_user())
        with patch("insights.session.risk.current_request_context", return_value=UA_CHANGE_CTX):
            evaluate_session_risk(request)
        with patch("insights.session.risk.current_request_context", return_value=HIGH_RISK_CTX):
            evaluate_session_risk(request)
    ...
    def test_step_up_applied_when_enabled_after_report_only(self, mock_capture, _ctx):
        ...
        request = self._authed_request_with_baseline(self._make_user())
        with patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, False, False)):
            evaluate_session_risk(request)  # report-only: emits telemetry, sets no step-up
        self.assertNotIn("step_up_required", request.session)
        with patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, False)):
    ...
    @patch("insights.session.risk.current_request_context", return_value=NEARBY_CTX)
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, True))
    ...
    @patch("insights.session.risk.current_request_context", return_value=STABLE_CTX)
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, True))
    ...
    @patch("insights.session.risk.current_request_context", return_value=NO_GEO_CTX)
    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(True, True, True))
...
class TestSessionRiskMiddleware(BaseTest):
    ...
    @patch("insights.session.middleware.evaluate_session_risk", return_value=RiskTier.HIGH)
    ...
    @patch("insights.session.middleware.evaluate_session_risk", return_value=RiskTier.NONE)
    ...
    def test_unauthenticated_request_skips_evaluation(self):
        ...
        with patch("insights.session.middleware.evaluate_session_risk") as mock_evaluate:
            response = SessionRiskMiddleware(lambda _req: HttpResponse("ok"))(request)

        mock_evaluate.assert_not_called()
        self.assertEqual(response.status_code, 200)

    @patch("insights.session.risk.risk_flags", return_value=RiskFlags(detection=True, step_up=True, session_end=False))
    @patch(
        "insights.session.risk.get_geoip_location",
