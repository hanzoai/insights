...
import { CustomFlowTestResult } from '../../steps/types'
import type { customFlowEditorNotificationTestLogicType } from './customFlowEditorNotificationTestLogicType'
import { createExampleEvent, createGlobalsFromResponse } from './customFlowEditorTestLogic'
import type { CustomFlowTestInvocation } from './customFlowEditorTestLogic'

// Time range constants for event search
const STANDARD_SEARCH_RANGE = `-7d`

export const customFlowEditorNotificationTestLogic = kea<customFlowEditorNotificationTestLogicType>([
    path((key) => [
        'products',
        'workflows',
        'frontend',
        ...
        'panel',
        'testing',
        'customFlowEditorNotificationTestLogic',
        key,
    ]),
    props({} as WorkflowLogicProps),
    key((props) => `${props.id}`),
    connect((props: WorkflowLogicProps) => ({
        values: [workflowLogic(props), ['workflow', 'workflowSanitized'], customFlowEditorLogic, ['selectedNodeId']],
        actions: [customFlowEditorLogic, ['setSelectedNodeId']],
    })),
    actions({
        ...
        setTestResult: (testResult: CustomFlowTestResult | null) => ({ testResult }),
    ...
    reducers({
        ...
        testResult: [
            null as CustomFlowTestResult | null,
            {
    ...
    forms(({ actions, values }) => ({
        testInvocation: {
            ...
            } as CustomFlowTestInvocation,
            errors: (data: CustomFlowTestInvocation) => {
            ...
            submit: async (testInvocation: CustomFlowTestInvocation) => {
                try {
                    ...
                    const apiResponse = await api.customFlows.createTestInvocation(values.workflow.id, {
                    ...
                    const result: CustomFlowTestResult = {
                        ...apiResponse,
