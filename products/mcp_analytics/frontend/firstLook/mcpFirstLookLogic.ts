import { MakeLogicType, actions, connect, kea, listeners, path, reducers, selectors } from 'kea'

import insights from 'lib/insights-typed'
import { organizationLogic } from 'scenes/organizationLogic'
import { teamLogic } from 'scenes/teamLogic'
import { userLogic } from 'scenes/userLogic'

import { sidePanelStateLogic } from '~/layout/navigation-3000/sidepanel/sidePanelStateLogic'
import { ProductKey } from '~/queries/schema/schema-general'
import { SidePanelTab } from '~/types'

import type { OrganizationType, UserType } from '../../../../frontend/src/types'
import type { TeamPublicType, TeamType } from '../../../../frontend/src/types'
import { mcpAnalyticsOnboardingLogic } from '../mcpAnalyticsOnboardingLogic'
import type { HarnessRow, KPIData, ToolRow } from '../mcpDashboardOverviewLogic'
import { mcpDashboardOverviewLogic } from '../mcpDashboardOverviewLogic'
import { buildChips, buildEditorPrompt, buildHeadline, buildMaxPrompt, type FirstLookChip } from './firstLookCopy'

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface mcpFirstLookLogicValues {
    isOnboarded: boolean // mcpAnalyticsOnboardingLogic
    harnessRows: HarnessRow[] // mcpDashboardOverviewLogic
    kpis: KPIData // mcpDashboardOverviewLogic
    toolRows: ToolRow[] // mcpDashboardOverviewLogic
    currentOrganization: OrganizationType | null // organizationLogic
    currentTeam: TeamPublicType | TeamType | null // teamLogic
    user: UserType | null // userLogic
    chips: FirstLookChip[]
    dominantClient: string | null
    editorExpanded: boolean
    editorPrompt: {
        label: string
        prompt: string
    }
    eventProperties: Record<string, unknown>
    headline: string
    maxPrompt: string
    shouldShow: boolean
    topTool: ToolRow | null
    worstErrorTool: ToolRow | null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface mcpFirstLookLogicActions {
    updateHasSeenProductIntroFor: (
        productKey: ProductKey,
        value?: boolean | undefined
    ) => {
        productKey: ProductKey
        value: boolean
    } // userLogic
    askMax: () => {
        value: true
    }
    dismiss: () => {
        value: true
    }
    dismissAndAskMax: () => {
        value: true
    }
    toggleEditor: () => {
        value: true
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface mcpFirstLookLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        shouldShow: (
            isOnboarded: boolean,
            user: UserType | null,
            currentTeam: TeamPublicType | TeamType | null,
            kpis: KPIData,
            toolRows: ToolRow[]
        ) => boolean
        topTool: (toolRows: ToolRow[]) => ToolRow | null
        worstErrorTool: (toolRows: ToolRow[]) => ToolRow | null
        dominantClient: (harnessRows: HarnessRow[]) => string | null
        headline: (
            currentOrganization: OrganizationType | null,
            currentTeam: TeamPublicType | TeamType | null,
            topTool: ToolRow | null,
            dominantClient: string | null,
            kpis: KPIData
        ) => string
        maxPrompt: (worstErrorTool: ToolRow | null, topTool: ToolRow | null) => string
        editorPrompt: (dominantClient: string | null) => {
            label: string
            prompt: string
        }
        chips: (
            topTool: ToolRow | null,
            worstErrorTool: ToolRow | null,
            kpis: KPIData,
            dominantClient: string | null
        ) => FirstLookChip[]
        eventProperties: (
            dominantClient: string | null,
            topTool: ToolRow | null,
            worstErrorTool: ToolRow | null
        ) => Record<string, unknown>
    }
}

export type mcpFirstLookLogicType = MakeLogicType<
    mcpFirstLookLogicValues,
    mcpFirstLookLogicActions,
    Record<string, any>,
    mcpFirstLookLogicMeta
>

export const mcpFirstLookLogic = kea<mcpFirstLookLogicType>([
    path(['products', 'mcp_analytics', 'frontend', 'firstLook', 'mcpFirstLookLogic']),
    connect(() => ({
        values: [
            mcpDashboardOverviewLogic,
            ['kpis', 'toolRows', 'harnessRows'],
            mcpAnalyticsOnboardingLogic,
            ['isOnboarded'],
            userLogic,
            ['user'],
            teamLogic,
            ['currentTeam'],
            organizationLogic,
            ['currentOrganization'],
        ],
        actions: [userLogic, ['updateHasSeenProductIntroFor']],
    })),
    actions({
        dismiss: true,
        askMax: true,
        dismissAndAskMax: true,
        toggleEditor: true,
    }),
    reducers({
        // Secondary path; collapsed by default to keep the hero tight.
        editorExpanded: [false, { toggleEditor: (state) => !state }],
    }),
    selectors({
        // Once per user, never on demo, and only after data loads (avoids an empty-flash).
        shouldShow: [
            (s) => [s.isOnboarded, s.user, s.currentTeam, s.kpis, s.toolRows],
            (
                isOnboarded: boolean,
                user: null | import('~/types').UserType,
                currentTeam: null | import('~/types').TeamPublicType | import('~/types').TeamType,
                kpis: KPIData,
                toolRows: ToolRow[]
            ): boolean => {
                const hasData = toolRows.length > 0 || kpis.toolCalls.value > 0
                return (
                    Boolean(isOnboarded) &&
                    hasData &&
                    !user?.has_seen_product_intro_for?.[ProductKey.MCP_ANALYTICS] &&
                    !currentTeam?.is_demo
                )
            },
        ],
        topTool: [(s) => [s.toolRows], (toolRows: ToolRow[]): ToolRow | null => toolRows[0] ?? null],
        worstErrorTool: [
            (s) => [s.toolRows],
            (toolRows: ToolRow[]): ToolRow | null =>
                toolRows.length ? [...toolRows].sort((a, b) => b.error_rate_pct - a.error_rate_pct)[0] : null,
        ],
        dominantClient: [
            (s) => [s.harnessRows],
            (harnessRows: HarnessRow[]): string | null => harnessRows[0]?.category ?? null,
        ],
        headline: [
            (s) => [s.currentOrganization, s.currentTeam, s.topTool, s.dominantClient, s.kpis],
            (
                currentOrganization: null | import('~/types').OrganizationType,
                currentTeam: null | import('~/types').TeamPublicType | import('~/types').TeamType,
                topTool: ToolRow | null,
                dominantClient: string | null,
                kpis: KPIData
            ): string =>
                buildHeadline({
                    company: currentOrganization?.name,
                    project: currentTeam?.name,
                    topTool,
                    client: dominantClient,
                    kpis,
                }),
        ],
        maxPrompt: [
            (s) => [s.worstErrorTool, s.topTool],
            (worstErrorTool: ToolRow | null, topTool: ToolRow | null): string =>
                buildMaxPrompt({ worstErrorTool, topTool }),
        ],
        editorPrompt: [
            (s) => [s.dominantClient],
            (dominantClient: string | null): { label: string; prompt: string } =>
                buildEditorPrompt({ client: dominantClient }),
        ],
        chips: [
            (s) => [s.topTool, s.worstErrorTool, s.kpis, s.dominantClient],
            (
                topTool: ToolRow | null,
                worstErrorTool: ToolRow | null,
                kpis: KPIData,
                dominantClient: string | null
            ): FirstLookChip[] => buildChips({ topTool, worstErrorTool, kpis, client: dominantClient }),
        ],
        eventProperties: [
            (s) => [s.dominantClient, s.topTool, s.worstErrorTool],
            (
                dominantClient: string | null,
                topTool: ToolRow | null,
                worstErrorTool: ToolRow | null
            ): Record<string, unknown> => ({
                client: dominantClient,
                busiest_tool: topTool?.tool ?? null,
                flakiest_tool: worstErrorTool?.tool ?? null,
            }),
        ],
    }),
    listeners(({ values, actions }) => ({
        dismiss: () => {
            insights.captureRaw('mcp analytics first look dismissed', values.eventProperties)
            actions.updateHasSeenProductIntroFor(ProductKey.MCP_ANALYTICS)
        },
        askMax: () => {
            insights.captureRaw('mcp analytics first look ask ai clicked', values.eventProperties)
            // Prefill, don't auto-submit: the prompt embeds tool names from `$mcp_tool_call`
            // events, which anyone with the project token can set — auto-running it would let a
            // seeded tool name reach Insights AI as an instruction. The user reviews, then sends.
            sidePanelStateLogic.findMounted()?.actions.openSidePanel(SidePanelTab.Max, values.maxPrompt)
        },
        dismissAndAskMax: () => {
            actions.askMax()
            actions.dismiss()
        },
        toggleEditor: () => {
            // Capture the expand only, not the collapse.
            if (values.editorExpanded) {
                insights.captureRaw('mcp analytics first look editor expanded', values.eventProperties)
            }
        },
    })),
])
