...
# JSON-string extraction calls that are equivalent to a InsightsQL string property access.
...
def _matched_property_access(node: ast.AST, context: InsightsQLContext):
...
def has_rewritable_json_extract(node: ast.AST, context: InsightsQLContext) -> bool:
    finder = _Finder(context)
    finder.visit(node)
    return finder.found


def rewrite_json_extract_to_property(node: ast.AST, context: InsightsQLContext):
    """
