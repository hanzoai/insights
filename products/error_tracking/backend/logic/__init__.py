...
# Operators insights-js implements in propertyComparisons. Anything outside this set is undefined when
...
def create_external_reference(
    *,
    ...
    if issue is None:
        raise ErrorTrackingExternalReferenceValidationError("Issue does not belong to this project.")

    integration = Integration.objects.filter(id=integration_id, team_id=team_id).first()
    if integration is None:
        raise ErrorTrackingExternalReferenceValidationError("Integration does not belong to this project.")
...
# The InsightsQL compiler is a heavy import, so it loads lazily inside these helpers
...
def compile_filter_bytecode(team_id: int, filters: dict) -> list[Any]:
    from insights.schema import PropertyGroupFilterValue  # noqa: PLC0415

    from insights.insightsql import ast  # noqa: PLC0415
    from insights.insightsql.compiler.bytecode import create_bytecode  # noqa: PLC0415
    from insights.insightsql.property import property_to_expr  # noqa: PLC0415
