...
_START = "products.review.backend.api.reviews.start_review_pr_workflow"
_ACCESS = "products.review.backend.api.reviews.GitHubIntegration.first_for_team_repository"
_META = "products.review.backend.api.reviews._fetch_pr_metadata"
...
class TestReviewUiTriggerApi(APIBaseTest):
    ...
    def test_trigger_starts_a_publishing_workflow_acting_as_the_requester(self, mock_start, mock_access, _mock_meta):
        ...
        with override_settings(REVIEWFN_TEAM_ID=self.team.id):
            resp = self._trigger("https://github.com/Insights/hanzo.ai/pull/123/files")
        ...
        mock_access.assert_called_once_with(self.team.id, "Insights/hanzo.ai")
        mock_start.assert_called_once_with(
            pr_url="https://github.com/Insights/hanzo.ai/pull/123",
    ...
    def test_non_pr_github_url_rejected(self, mock_start):
        with override_settings(REVIEWFN_TEAM_ID=self.team.id):
            resp = self._trigger("https://github.com/Insights/insights/issues/1")
    ...
    def test_unreviewable_pr_rejected_synchronously(self, _name, meta_or_error, message, mock_start, _mock_access):
        ...
        with (
            override_settings(REVIEWFN_TEAM_ID=self.team.id),
            patch(_META, side_effect=side_effect, return_value=return_value),
        ):
            resp = self._trigger("https://github.com/Insights/hanzo.ai/pull/7")
    ...
    def test_already_published_head_answers_honestly(
        ...
        with override_settings(REVIEWFN_TEAM_ID=self.team.id):
            resp = self._trigger("https://github.com/Insights/hanzo.ai/pull/123")
    ...
    def test_github_rate_limit_maps_to_the_shared_429(self, _name, rate_limited_call, mock_start):
        ...
        with (
            ...
            resp = self._trigger("https://github.com/Insights/hanzo.ai/pull/9")
