...
from insights.schema import InsightsQLQuery, PropertyOperator, RecordingPropertyFilter, RecordingsQuery
...
from insights.temporal.session_replay.count_playlist_items import convert_filters_to_recordings_query
from insights.temporal.session_replay.summarization_sweep.constants import DEFAULT_SAMPLE_RATE, SAMPLE_RATE_PRECISION
...
if not settings.DEBUG:
    _BASELINE_HAVING_PREDICATES.append(
        ...
        RecordingPropertyFilter(
            ...
            value=0,  # bool is represented as 0/1 in Datastore
        ),
    )
...
def fetch_recent_session_ids(
    ...
    *,
    ...
    max_execution_time_seconds: int = INSIGHTSQL_INCREASED_MAX_EXECUTION_TIME,
) -> list[str]:
...
def filter_session_ids_with_events(
    ...
    insightsql_settings = InsightsQLGlobalSettings(max_execution_time=max_execution_time_seconds)
    with tags_context(product=Product.SESSION_SUMMARY, feature=Feature.ENRICHMENT):
        result = InsightsQLQueryRunner(team=team, query=query, settings=insightsql_settings).calculate()
