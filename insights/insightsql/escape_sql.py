...
# The InsightsQL/Datastore parsers only accept a doubled backtick inside a quoted identifier, not a backslash-escaped one.
...
def _quote_postgres_wire_identifier(v: str, extra_reserved_keywords: set[str] | None) -> str:
    # Reject ``%`` for parity with the InsightsQL and Datastore escape paths. psycopg
...
def escape_datastore_identifier(identifier: str) -> str:
    if "%" in identifier:
        raise QueryError(f'The InsightsQL identifier "{identifier}" is not permitted as it contains the "%" character')
    return quote_datastore_identifier(identifier)


def quote_datastore_identifier(identifier: str) -> str:
...
class SQLValueEscaper:
    ...
    def visit_datetime(self, value: datetime):
        ...
        if self._dialect == "insightsql":
            return f"toDateTime({self.visit(datetime_string)})"  # no timezone for insightsql
