...
export function StepConditionalBranchConfiguration({
    ...
    node: Node<Extract<CustomFlowAction, { type: 'conditional_branch' }>>
}): JSX.Element {
    const action = node.data
    const { conditions } = action.config

    const { edgesByActionId, selectedNodeCanBeDeleted } = useValues(customFlowEditorLogic)
    const { setWorkflowAction, setWorkflowActionEdges } = useActions(customFlowEditorLogic)

    const nodeEdges = edgesByActionId[action.id] ?? []

    const setConditions = (
        conditions: Extract<CustomFlowAction, { type: 'conditional_branch' }>['config']['conditions']
    ): void => {
    ...
    const [branchEdges, nonBranchEdges] = useMemo(() => {
        const branchEdges: CustomFlow['edges'] = []
        const nonBranchEdges: CustomFlow['edges'] = []
