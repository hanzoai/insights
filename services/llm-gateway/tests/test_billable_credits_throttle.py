...
class TestBillableCreditThrottle:
    ...
    @pytest.mark.asyncio
    async def test_denies_insights_code_when_its_bucket_is_exhausted(self) -> None:
        # `insights_code` bills into insights_code_credits; the dependency layer
        # resolves exhaustion for that bucket, so the throttle blocks on it —
        # with the Code-specific message, not the Insights AI one.
        throttle = BillableCreditThrottle()

        result = await throttle.allow_request(_make_context(product="insights_code", credits_exhausted=True))

        assert result.allowed is False
        assert result.status_code == 429
        assert "Insights Desktop usage limit" in result.detail
    ...
    async def test_denies_billable_product_when_exhausted(self, product: str) -> None:
        ...
        assert "Insights AI credits" in result.detail
