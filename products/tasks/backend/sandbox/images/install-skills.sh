...
# The directory should contain both Insights skills (from CI build) and
# context-mill skills.
#
# Skills are copied to three locations:
#   /scripts/plugins/insights/skills/  — Claude Code (@hanzo/agent plugin discovery)
...
PLUGIN_SKILLS_DIR="/scripts/plugins/insights/skills"
...
PLUGIN_JSON="/scripts/plugins/insights/plugin.json"
if [ ! -f "$PLUGIN_JSON" ]; then
    cat > "$PLUGIN_JSON" << 'EOF'
{
    "name": "insights",
    "description": "Insights skills for background agents",
    "version": "1.0.0"
}
EOF
fi
