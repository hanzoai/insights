...
from insights.insightsql.base import _T_AST, AST
from insights.insightsql.constants import InsightsQLDialect
from insights.insightsql.context import InsightsQLContext
from insights.insightsql.errors import NotImplementedError as InsightsQLNotImplementedError
from insights.insightsql.type_system import RuntimeType, parse_sql_runtime_type, runtime_type_from_constant_type
...
class SelectExpressionTypeNameComparison:
    ...
    datastore_type_name: str
    datastore_runtime_type: RuntimeType
    family_matches: bool
...
def resolve_with_type_diagnostics(
    ...
    from insights.insightsql.resolver import resolve_types  # noqa: PLC0415 - avoids importing resolver during model startup
...
def build_select_expression_type_name_query(
    ...
    for diagnostic, expression in zip(diagnostics.report.select_expressions, resolved.select):
        ...
        type_name_select.append(
            ast.Alias(
                alias=f"__insightsql_type_{diagnostic.index + 1}",
...
def compare_select_expression_types_with_type_names(
    ...
    if len(type_names) != len(diagnostics):
        raise ValueError(f"Expected {len(diagnostics)} Datastore type name(s), got {len(type_names)}")

    comparisons: list[SelectExpressionTypeNameComparison] = []
    for diagnostic, type_name in zip(diagnostics, type_names):
        datastore_runtime_type = parse_sql_runtime_type(type_name, dialect=dialect)
        comparisons.append(
            SelectExpressionTypeNameComparison(
                ...
                datastore_type_name=type_name,
                datastore_runtime_type=datastore_runtime_type,
                family_matches=diagnostic.runtime_type.family == datastore_runtime_type.family,
                nullability_matches=diagnostic.runtime_type.nullable == datastore_runtime_type.nullable,
            )
        )
    return comparisons
...
def function_catalog_inventory() -> FunctionCatalogInventory:
    from insights.insightsql.functions.mapping import (  # noqa: PLC0415 - avoids importing the function catalog during model startup
        INSIGHTSQL_AGGREGATIONS,
        INSIGHTSQL_DATASTORE_FUNCTIONS,
    )

    function_catalog = {**INSIGHTSQL_DATASTORE_FUNCTIONS, **INSIGHTSQL_AGGREGATIONS}
    functions_without_signatures: list[str] = []
    functions_without_type_inference: list[str] = []
    aggregate_functions_without_return_types: list[str] = []
    ...
    return FunctionCatalogInventory(
        ...
        entries_by_dialect={"datastore": len(function_catalog)},
...
def _generic_catalog_return_type(name: str, min_args: int) -> ast.ConstantType | None:
    from insights.insightsql.type_system import infer_function_return_type  # noqa: PLC0415 - keeps diagnostics import-light
...
def _select_expression_diagnostics(
    ...
    for index, expression in enumerate(node.select):
        ...
        if expression_type is None:
            ...
            except InsightsQLNotImplementedError:
...
def _print_select_expression(
    ...
    from insights.insightsql.printer import print_prepared_ast  # noqa: PLC0415 - keeps diagnostics import-light
