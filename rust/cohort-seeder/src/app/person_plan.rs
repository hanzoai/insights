...
//! Datastore boundary stream never runs inside the liveness deadline. Depends on `datastore`,
//! `store`, `domain`, and its `app` siblings.
//!
//! Planning is claimed cluster-wide (a per-run advisory lock) before Datastore is touched, so R
