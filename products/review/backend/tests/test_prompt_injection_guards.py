...
from products.review.backend.reviewer.artefact_content import ReviewIssueFinding, ValidationVerdict
from products.review.backend.reviewer.models.github_meta import PRMetadata
from products.review.backend.reviewer.models.issues_review import Issue, IssuePriority, LineRange
from products.review.backend.reviewer.models.split_pr_into_chunks import Chunk, FileInfo
from products.review.backend.reviewer.outcomes.judge import (
...
from products.review.backend.reviewer.tools.issues_review import build_review_prompt
from products.review.backend.reviewer.tools.prompt_helpers import load_template_and_schema
from products.review.backend.reviewer.tools.select_perspectives import generate_selection_prompt
from products.review.backend.reviewer.tools.split_pr_into_chunks import generate_chunking_prompt
...
def _issues_review_prompt() -> str:
    return build_review_prompt(
        skill_name="review-script-perspective-logic-correctness",
        skill_version=2,
        chunk=_chunk(),
...
def _selection_prompt() -> str:
    return generate_selection_prompt(
        _pr_metadata(), [_chunk()], [], [_Perspective("review-script-perspective-logic-correctness", "logic")]
    )
...
def _validation_prompt() -> str:
    return build_validation_prompt(
        issue=_issue(),
        chunk=_chunk(),
        skill_name="review-script-validation-general",
        skill_version=1,
        pr_metadata=_pr_metadata(),
        pr_files=[],
    )
