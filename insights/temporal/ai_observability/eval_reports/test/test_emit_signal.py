...
class TestEmitEvalReportSignalActivity:
    ...
    async def test_skips_when_gate_fails(self, ateam, setup_fn):
        ...
        with (
            patch(
                "insights.temporal.ai_observability.eval_reports.emit_signal.summarize_report_for_signal"
            ) as mock_summarize,
    ...
    async def test_emits_signal_when_gates_pass(self, ateam):
        ...
        with (
            ...
            patch(
                "insights.temporal.ai_observability.eval_reports.emit_signal.EvaluationReportRun.objects.values_list"
            ) as mock_values_list,
            patch("products.signals.backend.facade.api.emit_signal", new_callable=AsyncMock) as mock_emit,
        ):
