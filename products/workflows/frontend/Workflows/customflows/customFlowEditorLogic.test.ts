...
import { computeMoveEdges, customFlowEditorLogic } from './customFlowEditorLogic'
import { CustomFlow, CustomFlowAction } from './types'

type Edge = CustomFlow['edges'][0]
...
describe('customFlowEditorLogic', () => {
    let logic: ReturnType<typeof customFlowEditorLogic.build>

    beforeEach(() => {
        initKeaTests()
        logic = customFlowEditorLogic()
        logic.mount()
    })

    describe('conditional branch naming', () => {
        const createMockCustomFlow = (conditionNames?: (string | undefined)[]): CustomFlow => ({
            id: 'test-flow',
            team_id: 1,
            version: 1,
            name: 'Test Flow',
        ...
        it('should use default labels when condition names are not provided', () => {
            const mockFlow = createMockCustomFlow()
            logic.actions.resetFlowFromCustomFlow(mockFlow)
        ...
        it('should use custom names when provided for conditional branches', () => {
            const mockFlow = createMockCustomFlow(['User is premium', 'User is in trial'])
            logic.actions.resetFlowFromCustomFlow(mockFlow)
        ...
        it('should fall back to default labels for conditions without names', () => {
            const mockFlow = createMockCustomFlow(['User is premium', undefined])
            logic.actions.resetFlowFromCustomFlow(mockFlow)
        ...
        it('should update edge labels when conditions are modified', () => {
            ...
            const updatedAction: CustomFlowAction = {
                id: 'branch',
                name: 'Conditional Branch',
                description: '',
                type: 'conditional_branch',
            ...
            logic.actions.resetFlowFromCustomFlow(updatedFlow)
        ...
        it('should use custom names for wait_until_condition when provided', () => {
            const mockFlow: CustomFlow = {
                ...createMockCustomFlow(),
                actions: [
                    {
                        id: 'trigger',
                        name: 'Trigger',
