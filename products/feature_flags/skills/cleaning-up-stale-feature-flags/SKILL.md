...
description: 'Identify and clean up stale feature flags in a Insights project. Use when the user wants to find unused, fully rolled out, or abandoned feature flags, review them for safety, and then disable or delete them. Covers staleness detection, dependency checking, and safe removal workflows.'
---
...
A feature flag is considered stale when it's no longer doing useful work. Insights tracks this with two signals:
...
Call `insights:feature-flag-get-all` with `active: "STALE"`. This returns all stale flags in a single request — Insights handles the staleness detection server-side using the criteria described above.
...
The `insights:feature-flag-get-definition` tool returns an `experiment_set` field. If non-empty, the flag is used by an experiment — check the experiment status before touching it.
...
```text
Search for: isFeatureEnabled, useFeatureFlag, getFeatureFlag, insights.isFeatureEnabled, insights.getFeatureFlag
...
- **Disable** (`active: false`) via `insights:update-feature-flag`: The flag stops being evaluated but the configuration is preserved. If something was missed in the code cleanup, re-enabling is instant. Recommended as the default.
- **Delete** via `insights:delete-feature-flag`: A soft-delete — the flag is marked as deleted but not physically removed. Keeps the flag list clean, but re-enabling requires recreating the flag. Better for flags the user is confident they'll never need again.
...
Agent steps:
- Call insights:feature-flag-get-all with active: "STALE" to get all stale flags in one request
- For each stale flag, call insights:feature-flag-get-definition to check experiment_set and dependencies
- Present findings:
...
- Generate and present cleanup prompt:
   ...
   getFeatureFlag, insights.isFeatureEnabled, insights.getFeatureFlag, etc.
...
- Disable each flag using insights:update-feature-flag (active: false)
- Confirm: "Both flags are now disabled in Insights."
```
...
- **Code cleanup is the real win.** Removing the flag from Insights is the easy part. The value comes from removing the dead code paths.

## Related tools

- `insights:feature-flag-get-all`: List and search feature flags (supports `active: "STALE"` filter)
- `insights:feature-flag-get-definition`: Get full flag details including experiment associations
- `insights:feature-flags-status-retrieve`: Get the status and reason for a single flag
- `insights:update-feature-flag`: Disable a flag by setting `active: false`
- `insights:delete-feature-flag`: Soft-delete a flag
