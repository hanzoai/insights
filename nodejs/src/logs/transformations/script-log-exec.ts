 ...
 * attribute is stored as `"error"` (with quotes), a number as `123`. The Datastore
 ...
 * plain strings are JSON-encoded so the Datastore sink's `JSONExtractString`
...
function redactSensitiveStrings(
    ...
    if (value && typeof value === 'object') {
        ...
        for (const [key, item] of Object.entries(value)) {
            // Keys need redacting as much as values do: Script can assign a dynamic key
 ...
 * inputs, so resolution happens in declared order; script-templated inputs execute their
...
export function resolveLogTransformationInputs(
    fn: Pick<InsightsFunctionType, 'inputs' | 'encrypted_inputs'>,
    ...
    for (const [key, input] of entries) {
        if (input?.bytecode && (input.templating ?? 'script') === 'script') {
            const {
                execResult,
                error,
                durationMs: execMs,
