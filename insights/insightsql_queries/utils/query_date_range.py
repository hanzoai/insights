...
from insights.schema import DateRange, InsightsQLFilters, IntervalType

from insights.insightsql.parser import ast

from insights.interval_specs import ORDERED_INTERVALS, PERIOD_MAP, IntervalLiteral, get_trunc_func, interval_spec
from insights.models.team import Team, WeekStartDay
from insights.utils import DEFAULT_DATE_FROM_DAYS, relative_date_parse, relative_date_parse_with_delta_mapping
...
class QueryDateRange:
    ...
    def get_earliest_timestamp(self) -> datetime:
        ...
        from insights.insightsql_queries.utils.timestamp_utils import get_earliest_timestamp_unfiltered  # noqa: PLC0415
    ...
    def day_of_week_filter_expr(self, timestamp_field: ast.Expr) -> Optional[ast.Expr]:
        ...
        No explicit timezone argument: the InsightsQL property-type transform wraps DateTime fields
    ...
    def number_interval_periods_insightsql(self) -> ast.Expr:
    ...
    def interval_period_string_as_insightsql_constant(self) -> ast.Expr:
        return ast.Constant(value=self.interval_name)

    def interval_count_as_insightsql_constant(self) -> ast.Expr:
    ...
    def date_to_start_of_interval_insightsql(self, date: ast.Expr) -> ast.Call:
    ...
    def date_from_to_start_of_interval_insightsql(self) -> ast.Call:
        return self.date_to_start_of_interval_insightsql(self.date_from_as_insightsql())

    def date_from_with_adjusted_start_of_interval_insightsql(self) -> ast.Call:
        ...
        return self.date_from_to_start_of_interval_insightsql()

    def date_to_to_start_of_interval_insightsql(self) -> ast.Call:
        return self.date_to_start_of_interval_insightsql(self.date_to_as_insightsql())

    def date_to_with_extra_interval_insightsql(self) -> ast.Call:
        return ast.Call(
            name="plus",
            args=[self.date_to_start_of_interval_insightsql(self.date_to_as_insightsql()), self.one_interval_period()],
        )

    def to_placeholders(self) -> dict[str, ast.Expr]:
        return {
            "interval": self.interval_period_string_as_insightsql_constant(),
            "interval_count": self.interval_count_as_insightsql_constant(),
            "one_interval_period": self.one_interval_period(),
            "number_interval_period": self.number_interval_periods_insightsql(),
            "date_from": self.date_from_as_insightsql(),
            "date_to": self.date_to_as_insightsql(),
            "date_from_start_of_interval": self.date_from_to_start_of_interval_insightsql(),
            "date_to_start_of_interval": self.date_to_to_start_of_interval_insightsql(),
            "date_from_with_adjusted_start_of_interval": (
                self.date_from_with_adjusted_start_of_interval_insightsql()
                if self.use_start_of_interval()
                else self.date_from_as_insightsql()
            ),
        }

    def to_insightsql_filters(self) -> InsightsQLFilters:
        """InsightsQLFilters carrying this range's bounds as absolute datetimes. The {filters} resolver
        (insights.insightsql.filters.ReplaceFilters) snaps day-level relative bounds like "-7d" to calendar
...
class QueryDateRangeWithIntervals(QueryDateRange):
    ...
    def get_start_of_interval_insightsql(self, *, source: ast.Expr | None = None) -> ast.Expr:
