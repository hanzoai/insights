...
description: >
  Audits services/llm-gateway against Insights/ai-gateway and updates services/llm-gateway/PARITY.md from current implementation evidence. Use when either gateway changes auth, attribution, billing, endpoints, providers, models, routing, or metadata; when reviewing a Python gateway change; or when asked to refresh, verify, or report gateway parity. This skill updates the parity record but does not migrate callers.
---
...
- Fetch `origin/master` and record its SHA as the Insights baseline.
...
- Read the current `Insights/ai-gateway` main SHA with `gh api repos/Insights/ai-gateway/commits/main` and record it as the Go baseline.
...
- Otherwise, inspect an authenticated checkout of `Insights/ai-gateway` at `main`.
...
- `insights/llm/gateway_client.py` and real call sites for required contracts
...
Do not treat Python's unbilled flag as an automatic blocker. An internal workload can move to Go with a Insights-owned team credential when debiting that wallet is the intended way to attribute Insights spend. It is blocked only when it must preserve customer-specific billing policy or must debit no wallet.
...
- Keep detailed Go design in `Insights/ai-gateway`.
