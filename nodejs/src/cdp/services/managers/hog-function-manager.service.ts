...
export class InsightsFunctionManagerService {
    ...
    constructor(
        ...
        this.pubSub.on<{ teamId: Team['id']; insightsFunctionIds: InsightsFunctionType['id'][] }>(
            'reload-insights-functions',
            ({ teamId, insightsFunctionIds }) => {
                logger.debug('⚡', '[PubSub] Reloading script functions!', { teamId, insightsFunctionIds })
    ...
    public async getInsightsFunctionsForTeams(
        ...
        /** Optional way to pre-filter script functions before returning them */
    ...
    private async fetchTeamInsightsFunctions(teamIds: string[]): Promise<Record<string, InsightsFunctionTeamInfo[]>> {
        logger.debug('[InsightsFunctionManager]', 'Fetching team script functions', { teamIds })
        const response = await this.postgres.query<Pick<InsightsFunctionType, 'id' | 'team_id' | 'type'>>(
            PostgresUse.COMMON_READ,
            `SELECT id, team_id, type FROM insights_hogfunction WHERE enabled = TRUE AND deleted = FALSE AND team_id = ANY($1)`,
            [teamIds],
    ...
    private async fetchInsightsFunctions(ids: string[]): Promise<Record<string, InsightsFunctionType | undefined>> {
        logger.debug('[InsightsFunctionManager]', 'Fetching script functions', { ids })
