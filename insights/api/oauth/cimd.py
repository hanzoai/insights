...
from insights.rate_limit import IPThrottle
from insights.scopes import filter_to_unprivileged_scopes
from insights.security.pinned_requests import PinnedIPAdapter, select_pinned_ip
from insights.security.url_validation import is_url_allowed, validate_url_and_pin_ips
...
class ComInsightsNamespace(TypedDict, total=False):
    verification_token: str
    scopes: list[str]


# Functional form required: "com.insights" is not a valid Python identifier.
...
def fetch_client_json_document(
    ...
    *,
    ...
    user_agent: str = "Insights-CIMD/1.0",
...
def _resolve_verification_token(metadata: CIMDMetadataDocument) -> CIMDVerificationToken | None:
    ...
    `com.insights.verification_token` and falling back to the legacy top-level
    `insights_verification_token`. Falls back to the top-level token when the nested
    ...
    com_insights = metadata.get("com.insights")
    if isinstance(com_insights, dict):
        nested_raw = com_insights.get("verification_token")
        if nested_raw and isinstance(nested_raw, str):
            token = find_cimd_verification_token(nested_raw)
            if token is not None:
                return token

    raw = metadata.get("insights_verification_token")
...
def _resolve_scopes(metadata: CIMDMetadataDocument) -> list[str] | None:
    """Resolve the allow-listed `com.insights.scopes` for an app, or None when the field
...
def _resolve_optional_scopes(metadata: CIMDMetadataDocument) -> list[str] | None:
    """Resolve `com.insights.optional_scopes` — the declinable subset a partner offers on top
    ...
    raw_optional: object = com_insights.get("optional_scopes")
...
def fetch_and_upsert_cimd_application(url: str, capture_ph_event=hanzo_insights.capture) -> OAuthApplication | None:
    """
            ...
            capture_ph_event(
                ...
                properties={
                    ...
                    "had_verification_token_attempt": bool(
                        metadata.get("insights_verification_token")
                        or (isinstance(ns := metadata.get("com.insights"), dict) and ns.get("verification_token"))
                    ),
                },
            )
            return new_app
...
# presented a valid `insights_verification_token`) get a higher default since abuse is
# traceable to a real Insights organization.
...
def _cimd_provisioning_defaults_for(app: OAuthApplication) -> "ProvisioningConfig":
    ...
    that vouched for itself by publishing a document is not the same as one Insights decided to
    ...
    # Verified partners (those who presented a valid `insights_verification_token`) get a higher
    # account-request limit, since abuse is traceable to a real Insights organization. An admin
