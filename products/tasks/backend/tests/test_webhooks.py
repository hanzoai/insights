...
class TestGitHubPRWebhook(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.task_run = TaskRun.objects.create(
            ...
            state={"verified_pr_urls": ["https://github.com/insights/insights/pull/123"]},
    ...
    def test_pr_merged_for_other_pr_on_same_branch_does_not_record_pr_merged(self, mock_capture, mock_get_secret):
        ...
        self.assertEqual(
            run.output,
            {
                "pr_url": "https://github.com/insights/insights/pull/10",
                "pr_urls": [
                    "https://github.com/insights/insights/pull/10",
                    "https://github.com/insights/insights/pull/11",
                ],
            },
        )
    ...
    def test_pr_opened_does_not_overwrite_existing_pr_url(self, mock_capture, mock_get_secret):
        ...
        self.assertEqual(
            run.output["pr_urls"],
            [existing, "https://github.com/insights/insights/pull/901"],
        )
...
class TestGitHubPRWebhookResolvesSignalReports(TestCase):
    ...
    def setUp(self):
        ...
        self.task_run = TaskRun.objects.create(
            ...
            team=self.team,
            status=TaskRun.Status.COMPLETED,
            state={"verified_pr_urls": ["https://github.com/insights/insights/pull/42"]},
...
class TestFindTaskRun(TestCase):
    ...
    def test_finds_multi_repository_run_from_snapshot(self):
        self.task.repositories = ["insights/insights", "insights/code"]
        self.task.save(update_fields=["repositories"])
        task_run = self.task.create_run(branch="feature/my-branch")
        task_run.output = {"pr_urls": ["https://github.com/insights/code/pull/123"]}
        task_run.state["verified_pr_urls"] = ["https://github.com/insights/code/pull/123"]
        task_run.save(update_fields=["output", "state"])

        self.assertEqual(
            find_task_run(
                pr_url="https://github.com/insights/code/pull/123",
    ...
    def test_caller_reported_pr_url_is_not_trusted_for_lookup(self):
        ...
        self.assertIsNone(find_task_run(pr_url=pr_url, repository="insights/insights"))
