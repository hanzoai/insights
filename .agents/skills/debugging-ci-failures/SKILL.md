...
description: >
  Debugs failing GitHub Actions CI runs for Insights PRs, commits, and branches,
  ...
  build failure. Start with the `insightscli ci:insights` digest (aggregated cross-run
  CI intelligence), then guides read-only inspection, failure classification,
  smallest local reproduction with insightscli, and safe reporting without rerunning CI
  or posting to GitHub.
---

# Debugging Insights CI failures
...
Always start with the `insightscli ci:insights` digest. It is the institutional,
...
`insightscli ci:insights` is the institutional CI-intelligence backend. It aggregates
...
```bash
insightscli ci:insights                                # digest for the current repo + branch
insightscli ci:insights search "<error or test name>"  # match a specific failure
insightscli ci:insights view <id>                       # one insight + its remediation actions
insightscli ci:insights plan <id>                       # print the recommended fix plan (does not apply it)
```
...
`insightscli ci:insights` prints a setup hint if the backend isn't installed or
...
- Most Insights jobs run on `depot-ubuntu-latest` or `depot-ubuntu-latest-16`.
...
- Insights CI frequently parallelizes the same test class across N shards
