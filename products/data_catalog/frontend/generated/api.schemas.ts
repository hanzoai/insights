...
export interface DataCatalogMetricApi {
    ...
    /**
     * Query kind of the definition (InsightsQLQuery, TrendsQuery, ...), or null for a stub.
     * @nullable
     */
...
export interface DataCatalogMetricRunRequestApi {
    /** Override the start of the query window (e.g. '-7d'). Rejected for InsightsQLQuery metrics, whose window is fixed in SQL. */
    date_from?: string
    /** Override the end of the query window. */
    date_to?: string
    /** Override the bucket interval. Rejected for InsightsQLQuery metrics.
     *
     * * `second` - second
     * * `minute` - minute
     * * `hour` - hour
...
export interface DataCatalogMetricRunApi {
    ...
    results: unknown
    /**
     * The compiled InsightsQL, when available.
     * @nullable
     */
...
export interface DataCatalogRelationshipProposalApi {
    ...
    /**
     * InsightsQL key expression on the source table (casts allowed).
     * @maxLength 400
     */
    ...
    /**
     * InsightsQL key expression on the joining table (casts allowed).
     * @maxLength 400
     */
