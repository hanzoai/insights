...
//!
//! The reference VM (`@hanzo/scriptvm`) is a suspendable coroutine: when a program calls a registered
...
//! state. CDP script functions (destinations) depend on this. This module snapshots the live
//! [`crate::vm::ScriptVM`] to a [`VmSnapshot`] whose JSON is **byte-compatible with the reference's
