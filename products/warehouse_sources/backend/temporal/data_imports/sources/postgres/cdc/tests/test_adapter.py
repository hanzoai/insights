...
class TestSetupResourcesPreflight:
    ...
    def test_insights_refuses_when_slot_already_exists(
        ...
        fields, error = PostgresCDCAdapter().setup_resources(
            _source(), {"cdc_management_mode": "insights", "cdc_slot_name": "existing_slot"}
        )
        assert fields == {}
    ...
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_insights_refuses_when_publication_already_exists(
        self, _conn, _slot_exists, _pub_exists, mock_create_slot, mock_create_publication, mock_drop
    ) -> None:
        fields, error = PostgresCDCAdapter().setup_resources(
            _source(), {"cdc_management_mode": "insights", "cdc_publication_name": "existing_pub"}
        )
        assert fields == {}
        assert error is not None and "already exists" in error
        mock_create_slot.assert_not_called()
    ...
    @patch(f"{_ADAPTER}.slot_exists", return_value=True)
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_insights_adopts_existing_default_slot_and_publication(
        self, _conn, _slot_exists, _pub_exists, mock_create_publication, mock_create_slot
    ) -> None:
        source = _source()
        fields, error = PostgresCDCAdapter().setup_resources(source, {"cdc_management_mode": "insights"})

        assert error is None
        assert fields == {
            "cdc_management_mode": "insights",
            "cdc_slot_name": "insights_019ef4e83bfd",
            "cdc_publication_name": "insights_pub_019ef4e83bfd",
        }
        mock_create_slot.assert_not_called()
        mock_create_publication.assert_not_called()
    ...
    @patch(f"{_ADAPTER}.slot_exists", return_value=True)
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_insights_recreates_missing_default_publication_when_adopting_slot(
        self, _conn, _slot_exists, _pub_exists, mock_create_publication, mock_create_slot
    ) -> None:
        source = _source()
        fields, error = PostgresCDCAdapter().setup_resources(source, {"cdc_management_mode": "insights"})
        ...
        assert error is None
        assert fields["cdc_slot_name"] == "insights_019ef4e83bfd"
        assert fields["cdc_publication_name"] == "insights_pub_019ef4e83bfd"
        mock_create_publication.assert_called_once()
        assert mock_create_publication.call_args.args[1] == "insights_pub_019ef4e83bfd"
        mock_create_slot.assert_not_called()
    ...
    @patch(f"{_ADAPTER}.slot_exists", return_value=False)
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_insights_adopts_existing_default_publication_and_creates_missing_slot(
        self, _conn, _slot_exists, _pub_exists, mock_create_publication, mock_create_slot
    ) -> None:
        source = _source()
        fields, error = PostgresCDCAdapter().setup_resources(source, {"cdc_management_mode": "insights"})
        ...
        assert mock_create_slot.call_args.args[1] == "insights_019ef4e83bfd"
        mock_create_publication.assert_not_called()
    ...
    @patch(f"{_ADAPTER}.slot_exists", return_value=False)
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_insights_rolls_back_only_after_verifying_absence(
        ...
        fields, error = PostgresCDCAdapter().setup_resources(
            _source(),
            {"cdc_management_mode": "insights", "cdc_slot_name": "s", "cdc_publication_name": "p"},
        )
        assert fields == {}
...
class TestRecreateSlot:
    ...
    def test_drops_and_recreates_slot_against_existing_publication(
        ...
        assert mock_drop.call_args.args[1] == "insights_slot"
        mock_create.assert_called_once()
        assert mock_create.call_args.args[1] == "insights_slot"
    ...
    def test_recreates_publication_qualifying_each_table_by_its_own_schema(
        ...
        assert mock_create_slot_and_pub.call_args.args[1:3] == ("insights_slot", "insights_pub")
...
class TestAlterPublicationMembership:
    ...
    @patch(f"{_ADAPTER}.add_table_to_publication")
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_add_table_for_insights_managed(self, _conn, mock_add) -> None:
        source = _source(cdc_enabled=True, cdc_management_mode="insights", cdc_publication_name="pub")
        PostgresCDCAdapter().add_table(source, "public", "orders")
    ...
    @patch(f"{_ADAPTER}.cdc_pg_connection", new_callable=_fake_conn)
    def test_remove_table_for_insights_managed(self, _conn, mock_remove) -> None:
        source = _source(cdc_enabled=True, cdc_management_mode="insights", cdc_publication_name="pub")
    ...
    def test_add_table_noop_without_publication(self, _conn, mock_add) -> None:
        source = _source(cdc_enabled=True, cdc_management_mode="insights")
        PostgresCDCAdapter().add_table(source, "public", "orders")
        mock_add.assert_not_called()
