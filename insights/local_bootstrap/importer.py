...
from insights.local_bootstrap.source import iter_table_rows
...
from insights.persons_seed import PERSON_DISTINCT_ID_TABLE
...
def _write_persons_to_datastore(team: Team, persons: dict[str, _PersonAccumulator]) -> None:
    now = datetime.now(UTC).replace(tzinfo=None)
...
def import_persons(
    ...
    _write_persons_to_datastore(team, live)
