...
from insights.api.oauth.cimd import CIMDFetchError
from insights.api.oauth.client_assertion import (
...
from insights.models.oauth import OAuthApplication, TokenEndpointAuthMethod
...
AUDIENCE = "https://us.hanzo.ai/api/agentic/oauth/token"
KID = "partner-key-1"
...
class TestClientAssertion(BaseTest):
    ...
    def _verify(self, assertion: str) -> None:
        with patch("insights.api.oauth.client_assertion.fetch_client_json_document", return_value=(self.jwks, None)):
    ...
    def test_jwks_is_cached_across_verifications(self):
        with patch(
            "insights.api.oauth.client_assertion.fetch_client_json_document", return_value=(self.jwks, None)
        ) as fetch:
    ...
    def test_jwks_fetch_failure_is_negatively_cached(self):
        with patch(
            "insights.api.oauth.client_assertion.fetch_client_json_document",
    ...
    def test_jwks_with_too_many_keys_is_rejected(self):
        ...
        with patch("insights.api.oauth.client_assertion.fetch_client_json_document", return_value=(many, None)):
            with self.assertRaises(ClientAssertionError):
                load_jwks(JWKS_URI)
