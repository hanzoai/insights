...
export function getInsightsFunctionEventKind(hf: InsightsFunctionType): LogsAlertEventKind | null {
...
// Filter used to list every InsightsFunction tied to a given alert, regardless of which
// event kind it handles. Deliberately omits the `events` array: the backend
// create endpoint fans out into one InsightsFunction per event kind, and JSONB `@>`
// matching would require a InsightsFunction's `filters.events` to contain every event
// we list — which no single InsightsFunction does post-fan-out. The `alert_id`
// property alone uniquely identifies all InsightsFunctions belonging to the alert.
...
export function resolveGroupLabel(group: LogsAlertDestinationGroup, slackChannels: SlackChannelType[]): string {
    if (group.type === LOGS_ALERT_NOTIFICATION_TYPE_SLACK) {
        const hf = group.insightsFunctions[0]
...
export function groupLogsAlertDestinations(
    ...
    for (const hf of insightsFunctions) {
        ...
        if (existing) {
            existing.insightsFunctions.push(hf)
            existing.enabled = existing.enabled && hf.enabled
        } else {
            groups.set(key, { key, type, label, insightsFunctions: [hf], enabled: hf.enabled })
        }
    }
