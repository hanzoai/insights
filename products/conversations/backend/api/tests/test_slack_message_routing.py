...
class TestSlackMessageRouting(BaseTest):
    ...
    @patch(f"{MODULE}.resolve_insights_user_for_slack")
    @patch(f"{MODULE}.create_or_update_slack_ticket")
...
class TestSlackLastCustomerMessage(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("insights teammate", {}, USE_TEAMMATE_EMAIL),
...
class TestSlackNudge(BaseTest):
    ...
    @patch(f"{MODULE}.hanzo_insights.feature_enabled", return_value=True)
    ...
    def test_llm_gates_nudge_when_ai_processing_approved(
        ...
        assert create_kwargs["extra_headers"] == {
            "x-insights-property-feature": "slack_nudge_classifier",
            "x-insights-property-$ai_span_name": "slack_nudge_classifier",
            "x-insights-property-slack_channel_id": "C_OTHER",
            "x-insights-property-slack_thread_ts": "1700000000.000100",
        }
    ...
    @override_settings(TEST=False, LLM_GATEWAY_URL="http://gateway.local", LLM_GATEWAY_API_KEY="test-key")
    @patch(f"{MODULE}.hanzo_insights.feature_enabled")
    @patch("insights.llm.gateway_client.get_llm_client")
    @patch(f"{MODULE}.get_slack_client")
    @patch(f"{MODULE}.create_or_update_slack_ticket")
...
class TestSlackMemberAlerts(BaseTest):
    ...
    @patch(f"{MODULE}.get_bot_user_id", return_value="U_OWN_BOT")
    @patch(f"{MODULE}.get_slack_client")
    def test_bot_join_fires_insights_event(
    ...
    @patch(f"{MODULE}.get_bot_user_id", return_value="U_OWN_BOT")
    @patch(f"{MODULE}.get_slack_client")
    def test_non_bot_join_does_not_fire_insights_event(self, _mock_get_client, _mock_bot_id, mock_report):
        handle_member_joined_channel({"user": "U123", "channel": "C_SUPPORT"}, self.team, "T123")
