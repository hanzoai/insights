...
export function FunnelTrendsTable(): JSX.Element | null {
    ...
    const periodColumns = Array.from(
        { length: periodCount },
        (_, index): TableColumn<FunnelTrendSeries, keyof FunnelTrendSeries | undefined> => {
    ...
    const columns: TableColumns<FunnelTrendSeries> = [
        {
            title: 'Series',
            key: 'series',
            render: (_, row) => (
                <span className="font-medium inline-flex items-center gap-2">
                    {seriesLabel(row)}
                    {row.compare_label && <Tag>{capitalizeFirstLetter(row.compare_label)}</Tag>}
                </span>
            ),
        },
        ...periodColumns,
