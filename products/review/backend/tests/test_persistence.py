...
from products.review.backend.reviewer.constants import DEFAULT_REVIEW_ARM, ReviewArm
from products.review.backend.reviewer.models.github_meta import PRComment, PRMetadata
...
class TestUpsertReviewReport(BaseTest):
    ...
    def test_review_arm_is_drawn_once_and_sticky_across_turns(self) -> None:
        ...
        with patch("products.review.backend.reviewer.persistence.draw_review_arm", return_value=sol):
        ...
        with patch(
            "products.review.backend.reviewer.persistence.draw_review_arm", return_value=DEFAULT_REVIEW_ARM
        ) as redraw:
            assert (
...
class TestPersistResults(BaseTest):
    ...
    def test_load_run_issues_round_trips_persisted_findings_by_id(self) -> None:
        ...
        b = _issue(
            ...
            suggestion="fix B",
            priority=IssuePriority.SHOULD_FIX,
            source_perspective="review-script-blind-spots-general",
        )
        report_id = upsert_review_report(team_id=self.team.id, repository="o/r", pr_url="u", pr_metadata=_pr_metadata())
