...
from datastore_driver.errors import ErrorCodes
from dagster import (
...
from insights.redis import get_client as get_redis_client

# This is the number of days to backfill in one SQL operation
MAX_PARTITIONS_PER_RUN = 1

# Number of sub-chunks to split a chunk into when retrying after a Datastore OOM error.
...
def _get_experimental_chunking(
    ...
    letting Datastore skip granules during the SELECT.
    ...
    - distinct_id_chunks: splits by cityHash64(distinct_id) range — uses contiguous
      ranges instead of modulo so Datastore can skip granules via the primary index
...
def _is_oom_error(exc: Exception) -> bool:
    """Check if an exception is a Datastore MEMORY_LIMIT_EXCEEDED error."""
    error_str = str(exc)
...
def _is_too_many_parts_error(exc: Exception) -> bool:
    """Check if an exception is a Datastore TOO_MANY_PARTS error."""
    error_str = str(exc)
...
def _is_too_many_simultaneous_queries_error(exc: Exception) -> bool:
    """Check if an exception is a Datastore TOO_MANY_SIMULTANEOUS_QUERIES error."""
    error_str = str(exc)
    return (
...
def _execute_with_retries(
    ...
    """Run the INSERT, retrying on transient Datastore overload errors until the shared budget is spent.
...
def _progress_key(asset_name: str, partition_key: str) -> str:
    return f"insights:sessions_backfill:progress:{asset_name}:{partition_key}"
...
def _do_experimental_backfill(
    ...
    # and is a standalone node not in the main Datastore cluster
