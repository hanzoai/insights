...
# topic) with `vision_action_id` + `slack_text`. Per-action `internal_destination` InsightsFunctions filter
...
_WEBHOOK_HEADERS = {"Content-Type": "application/json", "X-Insights-Webhook-Version": "1"}


def _managed_destinations(action: VisionAction, team: Team) -> QuerySet[InsightsFunction]:
    """The internal_destination InsightsFunctions this action owns.
...
def provision_delivery(action: VisionAction, *, request: Request, team: Team) -> None:
    """Reconcile this action's `internal_destination` InsightsFunctions to its `delivery_config`.
    ...
    # InsightsFunctionSerializer.create() reads context["request"].user, so provisioning stays in the viewset.
    context = {"request": request, "team_id": team.id, "get_team": lambda: team, "is_create": True}
    for target in action.delivery_config:
        serializer = InsightsFunctionSerializer(data=_destination_payload(action, target), context=context)
