from abc import ABC
from typing import TYPE_CHECKING, Optional

if TYPE_CHECKING:
    from .ast import Expr

# Base


class BaseInsightsQLError(Exception, ABC):
    message: str
    start: Optional[int]
    end: Optional[int]

    def __init__(
        self,
        message: str,
        *,
        start: Optional[int] = None,
        end: Optional[int] = None,
        node: Optional["Expr"] = None,
    ):
        super().__init__(message)
        if node is not None and node.start is not None and node.end is not None:
            self.start = node.start
            self.end = node.end
        else:
            self.start = start
            self.end = end


# Exposed vs. internal


class ExposedInsightsQLError(BaseInsightsQLError):
    """An exception that can be exposed to the user."""

    pass


class InternalInsightsQLError(BaseInsightsQLError):
    """An internal exception in the InsightsQL engine."""

    pass


# Specific exceptions


class SyntaxError(ExposedInsightsQLError):
    """The input does not conform to InsightsQL syntax."""

    pass


class QueryError(ExposedInsightsQLError):
    """The query is invalid, though correct syntactically."""

    pass


class TableAccessDeniedError(QueryError):
    """The user has no access to the table (raised by Database.get_table)."""

    # Surfaces as the error code on API responses (see insights/api/query.py), so clients can tell a
    # denial from any other query error without matching on the message.
    code_name = "table_access_denied"

    table_name: str

    def __init__(self, table_name: str):
        super().__init__(f"You don't have access to table `{table_name}`.")
        self.table_name = table_name


class NotImplementedError(InternalInsightsQLError):
    """This feature isn't implemented in InsightsQL (yet)."""

    pass


class ParsingError(InternalInsightsQLError):
    """Parsing failed."""

    pass


class ImpossibleASTError(InternalInsightsQLError):
    """Parsing or resolution resulted in an impossible AST."""

    pass


class ResolutionError(InternalInsightsQLError):
    """Resolution of a table/field/expression failed."""

    pass
