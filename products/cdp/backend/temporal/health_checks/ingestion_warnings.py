...
from insights.models.ingestion_warnings.sql_v2 import DISTRIBUTED_TABLE_NAME
...
# Ingestion warnings are written to Datastore during event ingestion (not detected here).
...
class IngestionWarningsCheck(HealthCheck):
    ...
    remediation = Remediation(
        ...
        agent="""
            ...
            Then fix it in the user's codebase at the `insights.capture` (or autocapture) call sites that
    ...
    def render_signal(cls, issue: HealthIssue) -> SignalContent | None:
        ...
        return SignalContent(
            description=(
                f"Insights raised the ingestion warning `{warning_type}` {count_clause} for this project in the last "
