...
description: >
  Diagnoses the health of a project's Insights SDK integrations — which SDKs are out of date
  and how to fix them. Use when a user asks about Insights SDK versions, outdated SDKs, upgrade
...
Outdated Insights SDKs surface through the project's generic **health issues** — the same
...
| `insights:execute-sql`           | Run the query from `remediation.agent` to see which versions still send events.              |
| `insights:docs-search`           | Look up an SDK's changelog / upgrade guide, as `remediation.agent` directs.                  |

## Trust boundary (read this first)

Each issue mixes **Insights-authored guidance** with **project- and event-supplied data**:
...
```json
insights:health-issues-summary
{}
```
...
- `link` — relative path (e.g. `/health/sdk-health`). Combine with the user's Insights host
  (e.g. `us.hanzo.ai`) for a clickable link.
- `remediation.human` — how the user fixes it in the Insights UI. Relay this verbatim when
...
- `remediation.agent` — **the instruction you act on.** For `sdk_outdated` it tells you to
  ...
  codebase: bump the Insights SDK dependency in the relevant manifest (`package.json`,
...
- For **mobile SDKs** (`insights-ios`, `insights-android`, `insights-flutter`,
  `insights-react-native`) the rule flips — the SDK ships in the app binary and users control
...
**https://hanzo.ai/docs/sdk-doctor/keeping-sdks-current**
...
> [Keeping SDKs current](https://hanzo.ai/docs/sdk-doctor/keeping-sdks-current) — it walks
...
- No `sdk_outdated` issues means the SDKs are healthy — there's nothing to fix. Say so plainly
  ...
  events either, suggest checking that `insights-js` or another SDK is actually wired up.)
