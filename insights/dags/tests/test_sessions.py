...
def _patch_experimental_backfill_deps(redis_get_return=None):
    """Patch external dependencies so _do_experimental_backfill can run without Datastore."""
    mock_client = MagicMock()
    mock_redis = MagicMock()
    mock_redis.get.return_value = redis_get_return

    with (
        patch("insights.dags.sessions.get_kwargs_for_client", return_value={}),
        patch("insights.dags.sessions.get_http_client") as mock_get_http_client,
        patch("insights.dags.sessions.sync_execute") as mock_sync_execute,
        patch("insights.dags.sessions.wait_for_parts_to_merge"),
        patch("insights.dags.sessions.get_git_commit_short", return_value="abc123"),
        patch("insights.dags.sessions.metabase_debug_query_url", return_value=None),
        patch("insights.dags.sessions.get_redis_client", return_value=mock_redis),
    ):
...
class TestTooManyPartsRetry:
    ...
    def test_retries_on_too_many_parts_then_succeeds(self):
        ...
        with (
            ...
            patch("insights.dags.sessions.wait_for_parts_to_merge") as mock_wait,
        ):
