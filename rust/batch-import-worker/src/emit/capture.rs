...
use metrics::counter;
use insights_rs::{Client, Error as InsightsError, Event};
...
/// Maps a insights-rs capture error to a bounded `&'static str` failure reason for
...
/// `_` arm is required because `insights_rs::Error` is `#[non_exhaustive]`.
fn failure_reason(err: &InsightsError) -> &'static str {
    match err {
        InsightsError::BillingLimitExceeded(_) => "quota", // 402
        InsightsError::BadRequest(_) => "bad_request",     // 400 / 413 (malformed / oversize)
        InsightsError::ServerError { .. } => "server_error", // 5xx
        InsightsError::RateLimit => "rate_limited",        // 429
        InsightsError::Unauthorized => "unauthorized",     // 401
        InsightsError::Connection(_) => "transport",       // network / unexpected status
        InsightsError::Serialization(_) => "serialization", // local encode failure
        _ => "other",
    }
}
...
mod tests {
    ...
    fn failure_reason_maps_every_variant() {
        ...
        let cases: &[(InsightsError, &str)] = &[
            (InsightsError::BillingLimitExceeded("x".into()), "quota"),
            (InsightsError::BadRequest("x".into()), "bad_request"),
            (
                InsightsError::ServerError {
                    status: 503,
                    message: "x".into(),
                },
                "server_error",
            ),
            (InsightsError::RateLimit, "rate_limited"),
            (InsightsError::Unauthorized, "unauthorized"),
            (InsightsError::Connection("x".into()), "transport"),
            (InsightsError::Serialization("x".into()), "serialization"),
            // A variant capture_batch_immediate never returns still maps safely via the
            // non_exhaustive catch-all rather than panicking.
            (InsightsError::NotInitialized, "other"),
        ];
