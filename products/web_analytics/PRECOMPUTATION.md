...
Insights web analytics has two parallel precomputation systems. They target the same problem (avoid scanning raw events on every dashboard load) but use different mechanisms and apply to different query shapes.
...
DAG-warmed Datastore tables (`web_pre_aggregated_stats`, `web_pre_aggregated_bounces`) that store hourly-rollup data computed in the background. Gated per-team by the `useWebAnalyticsPreAggregatedTables` modifier plus the `SETTINGS_WEB_ANALYTICS_PRE_AGGREGATED_TABLES` feature flag.
...
- **Coverage** (today): overview, goals, vitals path breakdown, and three stats-table families — paths (`WebStatsBreakdown.PAGE`/`INITIAL_PAGE` + `includeBounceRate`), frustration metrics, and simple breakdowns. See the `*_lazy_precompute.py` modules in `products/web_analytics/backend/insightsql_queries/`
...
- The `web-analytics-precompute-toggle` Insights feature flag is off for the team's organization
...
The read is a single `sync_execute` call (not InsightsQL — see "Why bypass InsightsQL" below). It computes the 5 metric pairs (current + previous period) via `*MergeIf` aggregates filtered by the team-tz date range converted to UTC. Settings:
...
- `optimize_skip_unused_shards=1` — `job_id IN (...)` + sharding-by-`sipHash64(job_id)` lets Datastore prune to the right shards.
...
#### Why bypass InsightsQL

InsightsQL `InsightsQLGlobalSettings` is `extra="forbid"`, so we couldn't get arbitrary CH settings (originally `select_sequential_consistency=1`, since dropped) through `execute_insightsql_query`. The read query shape is stable enough that `sync_execute` with parameterized values is appropriate. The team_id WHERE clause is enforced manually (the InsightsQL printer's auto-injected `team_id_guard_for_table` is not applied for sync_execute).

If we later move back to InsightsQL (after the consistency story is settled), the InsightsQL table registration at `insights/insightsql/database/schema/web_overview_preaggregated.py` is still present and ready.
...
- **Adoption / latency**: see [`evaluating-web-analytics-performance`](https://github.com/Insights/insights/blob/master/.agents/skills/) or query `system.query_log` directly.
...
5. **InsightsQL `top_level_settings`** on `WebOverviewPreaggregatedTable` is currently unused (read bypasses InsightsQL); kept as a hook if we re-route through InsightsQL later.

## Adding lazy computation to another web analytics query family

Reference implementation: `products/web_analytics/backend/insightsql_queries/web_overview_lazy_precompute.py`.

Roughly:

1. **Schema** — new CH table under `insights/datastore/preaggregation/`. ReplacingMergeTree with `(team_id, job_id, time_window_start)` ORDER BY, sharded by `sipHash64(job_id)`, partitioned by `toYYYYMMDD(expires_at)`. Register in `insights/datastore/schema.py` test fixtures.
2. **Migration** — `insights/datastore/migrations/0XXX_<name>.py`. Sharded + distributed CREATE on `NodeRole.DATA`.
3. **InsightsQL table** — `insights/insightsql/database/schema/<name>.py`. Register in `insights/insightsql/database/database.py`.
...
5. **Insert query** — InsightsQL template using `{time_window_min}`/`{time_window_max}` placeholders. Add a session-boundary pad if the query has session-level joins.
...
Four columns vs. a metric discriminator: ARRAY JOIN would fan one event into four rows, but the new Datastore analyzer rejects bare `events.properties` references inside the ARRAY JOIN source array (the source array is resolved before the FROM alias scope). Four columns let the INSERT stay a single `FROM events GROUP BY (hour, path)`, no fan-out, and each metric tab reads exactly one column.
...
FROM (
    ...
    FROM insights.web_vitals_paths_preaggregated
...
2. **Adding a metric** (e.g. TTFB) is a schema change — add the column to `web_vitals_paths_preaggregated`, the InsightsQL table registration, the INSERT template, and the `_METRIC_STATE_COLUMN` map.
3. **Bands are computed in Datastore from the runtime thresholds**, not stored — so a threshold change is free on the read side.
...
- **Audience**: teams belonging to organizations rolled out on the `web-analytics-precompute-toggle` feature flag — the same flag the runtime lazy read path checks. The job parses the flag's `Match organizations against id equals <uuid>` group conditions and resolves them to teams via `Team.objects.filter(organization_id__in=...)`. The flag lives on Insights's internal dogfooding project; self-hosted instances are gated out via `is_cloud()` so a same-keyed flag on someone else's team-2 doesn't trigger anything.
- **Audience cap**: 200 teams. A typo in the flag config fails-loudly (op returns with `skipped=N` and zero warmed) rather than silently overloading Datastore.
...
- `products/web_analytics/backend/insightsql_queries/web_overview.py` — runner
- `products/web_analytics/backend/insightsql_queries/web_overview_lazy_precompute.py` — overview lazy path
- `products/web_analytics/backend/insightsql_queries/web_overview_pre_aggregated.py` — overview v2 path
- `products/web_analytics/backend/insightsql_queries/stats_table.py` — stats table runner
- `products/web_analytics/backend/insightsql_queries/web_stats_paths_lazy_precompute.py` — PATHS lazy path
- `products/web_analytics/backend/insightsql_queries/web_lazy_precompute_common.py` — shared eligibility gate + helpers
- `insights/datastore/preaggregation/web_overview_preaggregated_sql.py` — overview schema
- `insights/datastore/preaggregation/web_stats_paths_preaggregated_sql.py` — PATHS schema
- `products/web_analytics/backend/insightsql_queries/web_vitals_path_breakdown.py` — vitals runner
- `products/web_analytics/backend/insightsql_queries/web_vitals_paths_lazy_precompute.py` — vitals lazy path
- `insights/datastore/preaggregation/web_vitals_paths_preaggregated_sql.py` — vitals schema
