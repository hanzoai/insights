...
class TestCustomImageRefresh(APIBaseTest):
    def _create_image(self, *, base_image_reference: str | None) -> SandboxCustomImage:
        ...
        return SandboxCustomImage.objects.for_team(self.team.id).create(
            ...
            modal_image_name=f"insights-sandbox-custom-{image_count}:latest",
    ...
    @patch(
        ...
        return_value="ghcr.io/insights/insights-sandbox-vm@sha256:current",
    )
    def test_refresh_dispatches_oldest_stale_image_with_batch_limit(self, _mock_reference, mock_execute) -> None:
        ...
        newer_stale = self._create_image(base_image_reference="ghcr.io/insights/insights-sandbox-vm@sha256:old")
        current = self._create_image(base_image_reference="ghcr.io/insights/insights-sandbox-vm@sha256:current")
        ...
        already_attempted = self._create_image(base_image_reference="ghcr.io/insights/insights-sandbox-vm@sha256:old")
        already_attempted.base_image_refresh_reference = "ghcr.io/insights/insights-sandbox-vm@sha256:current"
    ...
    def test_refresh_restores_ready_status_when_dispatch_fails(self, _mock_reference, _mock_execute) -> None:
        stale = self._create_image(base_image_reference="ghcr.io/insights/insights-sandbox-vm@sha256:old")
    ...
    def test_refresh_build_failure_remains_ready_and_can_retry(self) -> None:
        ...
        stale.base_image_refresh_reference = "ghcr.io/insights/insights-sandbox-vm@sha256:current"
