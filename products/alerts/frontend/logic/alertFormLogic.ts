...
import { containsInsightsQLQuery, isFunnelsQuery, isInsightVizNode, isMetricsQuery } from '~/queries/utils'
...
export type InsightAlertKind = 'trends' | 'insightsql' | 'funnels' | 'metrics'
...
export function insightAlertKindForQuery(query?: Record<string, any> | null): InsightAlertKind {
    if (containsInsightsQLQuery(query)) {
        return 'insightsql'
    }
...
const defaultConfigForInsight = (kind: AlertFormLogicProps['insightAlertKind']): AlertConfig => {
    if (kind === 'insightsql') {
        ...
        return { type: 'InsightsQLAlertConfig', evaluation: 'last_row' }
    }
    if (kind === 'funnels') {
...
export interface alertFormLogicValues {
    ...
    insightsqlAlertPreview: InsightsQLAlertPreview | null
    insightsqlConfigPrefill: Partial<Pick<InsightsQLAlertConfig, 'column' | 'label_column'>> | null
    insightsqlLabelColumnOptions: {
        label: string
        value: string
    }[]
    insightsqlNumericColumns: string[] | null
    insightsqlResultColumns: string[] | null
    insightsqlSuggestedColumn: string | null
    insightsqlValueColumnOptions: {
        label: string
        value: string
    }[]
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface alertFormLogicActions {
    clearSimulation: () => {
        value: true
    }
    ...
    resetAlertForm: (values?: AlertFormType) => {
    ...
    submitAlertFormRequest: (alertForm: AlertFormType) => {
        alertForm: AlertFormType
    }
    submitAlertFormSuccess: (alertForm: AlertFormType) => {
        alertForm: AlertFormType
    }
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface alertFormLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        ...
        insightsqlConfigPrefill: (
            ...
            insightsqlSuggestedColumn: string | null,
            arg: AlertConfig
        ) => Partial<Pick<InsightsQLAlertConfig, 'column' | 'label_column'>> | null
        insightsqlValueColumnOptions: (
            insightsqlResultColumns: string[] | null,
            insightsqlNumericColumns: string[] | null
        ) => {
...
export type alertFormLogicType = MakeLogicType<
    alertFormLogicValues,
    alertFormLogicActions,
    AlertFormLogicProps,
    alertFormLogicMeta
>
...
export const alertFormLogic = kea<alertFormLogicType>([
        ...
        alertForm: {
            ...
            submit: async (alert) => {
                ...
                if (entitlementCheck.blocked) {
                    toast.error(entitlementCheck.message)
                ...
                const flushPendingNotifications = async (savedAlertId: string): Promise<void> => {
                    if (notifLogic.values.pendingNotifications.length > 0) {
                        await notifLogic.asyncActions.createPendingInsightsFunctions(savedAlertId, alert.name)
                    }
                }
                ...
                try {
                    ...
                    toast.error(`Error saving alert: ${formatSaveError(error)}`)
                    throw error
                }

                if (isNewAlert) {
                    insights.capture('alert creation completed', {
                ...
                try {
                    ...
                    insights.captureException(postSaveError)
                }
    ...
    selectors(({ props }) => ({
        ...
        insightsqlAlertPreview: [
            ...
            // don't re-derive it (the per-row map can cover up to INSIGHTSQL_ANY_ROW_MAX_ROWS rows).
            (s) => [
                s.insightData,
        ...
        insightsqlResultColumns: [
            (s) => [s.insightData],
            (insightData: Record<string, any>): string[] | null =>
                props.insightAlertKind === 'insightsql' && Array.isArray(insightData?.columns)
        ...
        insightsqlNumericColumns: [
            (s) => [s.insightData, s.insightsqlResultColumns],
            (insightData: Record<string, any>, resultColumns: string[] | null): string[] | null =>
        ...
        insightsqlSuggestedColumn: [
            (s) => [s.insightsqlResultColumns, s.insightsqlNumericColumns],
            (resultColumns: string[] | null, numericColumns: string[] | null): string | null =>
        ...
        insightsqlConfigPrefill: [
            (s) => [
                s.insightsqlResultColumns,
                s.insightsqlSuggestedColumn,
                (state, logicProps) => s.alertForm(state, logicProps)?.config,
            ],
            (
                ...
                if (!isInsightsQLAlertConfig(config)) {
                    return null
                }
                const patch: Partial<Pick<InsightsQLAlertConfig, 'column' | 'label_column'>> = {}
        ...
        insightsqlLabelColumnOptions: [
            (s) => [s.insightsqlResultColumns, (state, logicProps) => s.alertForm(state, logicProps)?.config],
            (
                resultColumns: string[] | null,
                config: AlertConfig | null | undefined
            ): { label: string; value: string }[] => {
                const evaluated = isInsightsQLAlertConfig(config) ? (config.column ?? null) : null
        ...
        return {
            ...
            submitAlertForm: () => {
                ...
                if (validationErrors.length > 0) {
                    ...
                    toast.error(`Couldn't save alert: ${message}`)
                }
            },
            ...
            simulateAlertSuccess: ({ simulationResult }) => {
                ...
                if (simulationResult) {
                    ...
                    insights.capture('alert simulation run', {
                        success: true,
    ...
    subscriptions(({ props, values, actions }) =>
        ...
        props.insightAlertKind !== 'insightsql' || props.alert
            ? {}
            : {
