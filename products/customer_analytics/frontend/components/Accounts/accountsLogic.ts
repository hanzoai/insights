...
import {
    ACCOUNTS_INSIGHTSQL_DATA_NODE_KEY,
...
export const accountsLogic = kea<accountsLogicType>([
    ...
    connect(() => ({
        values: [
            ...
            customerAnalyticsSceneLogic,
            ['mineOnly'],
            dataNodeLogic({ key: ACCOUNTS_INSIGHTSQL_DATA_NODE_KEY } as DataNodeLogicProps),
    ...
    reducers({
        ...
        // stale InsightsQL cell until the async refetch lands.
        ...
        // Tags written from the list, keyed by account id — masks the stale InsightsQL
    ...
    selectors({
        ...
        insightsqlQuery: [
            ...
            (
                ...
                // InsightsQL ORDER BY resolves SELECT aliases by name, so the visible column
        ...
        accountsQuerySource: [
            (s) => [s.insightsqlQuery, s.relationshipDefinitionsLoaded],
            (insightsqlQuery: DataTableNode, relationshipDefinitionsLoaded: boolean): AccountsQuery | null =>
                relationshipDefinitionsLoaded ? (insightsqlQuery.source as AccountsQuery) : null,
        ],
    ...
    listeners(({ actions, values, cache }) => ({
        setSearchInput: async ({ query }, breakpoint) => {
            ...
            insights.capture(AccountsEvents.Searched, {
        ...
        reportFilterChange: ({ filterType }) => {
            ...
            insights.capture(AccountsEvents.FilterChanged, properties)
        },
        ...
        toggleSort: ({ column }) => {
            ...
            insights.capture(AccountsEvents.Sorted, {
                column,
        ...
        refresh: () => {
            insights.capture(AccountsEvents.Refreshed, {
                has_search: !!values.searchQuery.trim(),
                active_filter_count: values.activeFilterCount,
                sort_column: values.sortOrder?.column ?? null,
            })
            dataNodeLogic.findMounted({ key: ACCOUNTS_INSIGHTSQL_DATA_NODE_KEY })?.actions.loadData('force_async')
        ...
        updateAccountRole: async ({ accountId, column, user }) => {
            ...
            try {
                ...
                insights.capture(AccountsEvents.RoleAssigned, {
                ...
                insights.captureException(error as Error, { scope: 'accountsLogic.updateAccountRole' })
                toast.error(`Failed to update ${definition.name}`)
            } finally {
        ...
        updateAccountTags: async ({ accountId, tags }, breakpoint) => {
            ...
            try {
                ...
                insights.capture(AccountsEvents.TagsUpdated, { tag_count: tags.length })
                ...
                insights.captureException(error as Error, { scope: 'accountsLogic.updateAccountTags' })
        ...
        openAccount: ({ accountId, externalId, name, tab }) => {
            const dataNode = dataNodeLogic.findMounted({ key: ACCOUNTS_INSIGHTSQL_DATA_NODE_KEY })
    ...
    afterMount(({ actions }) => {
        insights.capture(AccountsEvents.ListViewed)
    ...
    urlToAction(({ actions, values }) => {
        ...
        const openAccountByPath = (accountId: string | undefined, rawTab?: string): void => {
            // Guard the path param before it's interpolated into the InsightsQL id filter.
