...
from insights.utils import absolute_uri, generate_cache_key, generate_short_id
...
class Insight(RootTeamMixin, FileSystemSyncMixin, models.Model):
    ...
    def get_analytics_query_kinds(self) -> dict[str, str]:
        ...
        (`query_source_kind`, e.g. "TrendsQuery"/"InsightsQLQuery") for analytics, read straight off the stored
        query. Covers every query (InsightsQL/table included). Reliable because the backend has the full query —
    ...
    def get_analytics_query_metadata(self) -> dict[str, Any]:
        ...
        `sanitizeQuery`. Gated to insight-style queries (like the frontend), so InsightsQL/SQL/table queries
    ...
    def dashboard_filters(
        ...
        if (dashboard is not None or dashboard_filters_override is not None) and not self.query:
            ...
            if isinstance(dashboard_properties, list) or (
                ...
                from insights.insightsql_queries.apply_dashboard_filters import flatten_property_leaves  # noqa: PLC0415
    ...
    def alertable_query_kind(self) -> "NodeKind | None":
        ...
        from insights.schema import NodeKind  # noqa: PLC0415
...
class InsightViewed(models.Model):
    ...
    class Meta:
        ...
        db_table = "insights_insightviewed"


@timed("generate_insight_cache_key")
def generate_insight_filters_hash(insight: Insight, dashboard: Optional["Dashboard"]) -> str:
    # Deferred: the legacy filters layer imports the InsightsQL/schema universe, and this model
    # loads at django.setup() in every process.
    from insights.models.filters.utils import get_filter  # noqa: PLC0415
