...
6. Docker only: the `insights-sandbox-base` image freshness check runs (rebuilding on a new `@hanzo/agent` version or Dockerfile change). Modal only: ngrok tunnels come up, exposing the callback services publicly.
