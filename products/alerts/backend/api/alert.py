...
from insights.api.documentation import extend_schema_field
...
from insights.rate_limit import AlertTestDeliveryThrottle
...
from insights.tasks.alerts.detector import MAX_DETECTOR_BREAKDOWN_VALUES
from insights.tasks.alerts.schedule_restriction import validate_and_normalize_schedule_restriction
...
class AlertConfigUnion(RootModel):
    ...
    root: Annotated[
        TrendsAlertConfig | InsightsQLAlertConfig | FunnelsAlertConfig | MetricsAlertConfig,
...
class AlertSerializer(SearchMatchTypeSerializerMixin, serializers.ModelSerializer):
    ...
    config = AlertConfigField(
        ...
        help_text=(
            ...
            "interval). InsightsQLAlertConfig (SQL insights): column (which result column to evaluate, defaults to "
...
class AlertViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def test_delivery(self, request, *args, **kwargs):
        ...
        if destination_count and trigger_alert_insights_functions(
            alert,
            {
