...
def test_trusted_reactor_predicate_gates_untrusted_and_author(
    ...
    is_trusted = _trusted_reactor_predicate("Insights/insights", author="prauthor")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param({"login": "insights-bot", "type": "User"}, True, id="machine-user"),
        pytest.param({"login": "INSIGHTS-BOT", "type": "User"}, True, id="machine-user-case-insensitive"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "* fix: address review\n\x1e\n",
            CommitProvenance(
                commit_count=1, agent_commit_count=1, generated_by=("Insights Desktop",), task_ids=("task-1",)
            ),
            id="squash-style-message-with-embedded-trailers",
        ),
        pytest.param(
            "chore: retry task\n\ngenerated-by: insights code\ntask-id: task-9\n\x1e\n",
            CommitProvenance(
                commit_count=1, agent_commit_count=1, generated_by=("insights code",), task_ids=("task-9",)
            ),
...
def test_fetch_threads_excludes_stamphogs_own_inline_comments(
    ...
    comments, _ = github._fetch_threads_and_reactions("Insights/insights", 1, author="alice")
