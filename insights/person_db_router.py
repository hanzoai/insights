...
# A persons-DB model resolved through the Django ORM. personinsights serves person/group/cohort data,
...
PERSONS_ORM_ACCESS_COUNTER = Counter(
    "insights_persons_orm_access_total",
...
class PersonsDBORMBlockedError(RuntimeError):
    ...
    personinsights is the sole source of truth for person/group/cohort data. The test
    suite activates this block (via the personinsights fake) so that any code path that
    still reaches for the persons DB through the ORM fails loudly instead of
    silently reading the main database. Use the personinsights helpers
    (``insights/models/person/util.py``, ``insights/models/group_type_mapping.py``)
    for production reads and ``insights/test/persons.py`` for test data.
    """
...
def allow_persons_orm():
    ...
    test's personinsights fake is active. Saves and restores the previous block state so
    nesting is safe.
    """
...
# personinsights is the source of truth for these; the ORM must not query them.
PERSONS_DB_MODELS = {
    "person",
    "persondistinctid",
...
class PersonDBRouter:
    ...
    The persons data lives behind the personinsights service, not the Django ORM, so
    ...
    # historically in the `insights` app; it moved to `feature_flags` when the FF
    # models were extracted, and CohortPeople moved to `cohorts`, but both keep
    # their persons-DB tables.
    PERSONS_APP_LABELS = {"insights", "feature_flags", "cohorts"}
    ...
    def _raise_if_blocked(model) -> None:
        if persons_orm_blocked():
            raise PersonsDBORMBlockedError(
                ...
                f"personinsights is the sole source of truth — read via the helpers in "
                f"insights/models/person/util.py or insights/models/group_type_mapping.py, "
                f"and create test data via insights/test/persons.py."
            )
