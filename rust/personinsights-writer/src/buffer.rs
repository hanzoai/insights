...
impl PersonBuffer {
    ...
    pub fn insert(&mut self, person: Person, partition: i32, offset: i64) {
        ...
        match self.entries.entry(key) {
            Entry::Occupied(mut e) => {
                e.insert(person);
                counter!("personinsights_writer_messages_deduped_total").increment(1);
            }
            Entry::Vacant(e) => {
                e.insert(person);
            }
        ...
        gauge!("personinsights_writer_buffer_size").set(self.entries.len() as f64);
    }
    ...
    pub fn drain(&mut self) -> (Vec<Person>, HashMap<i32, i64>) {
        ...
        gauge!("personinsights_writer_buffer_size").set(0.0);
        (persons, offsets)
    }
}
