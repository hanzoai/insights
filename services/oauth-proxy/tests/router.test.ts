...
describe('router', () => {
    it.each([
        ...
        const request = new Request(`https://oauth.hanzo.ai${path}`)
    ...
    it.each([
        ...
        const request = new Request(
            `https://oauth.hanzo.ai/oauth/authorize/?client_id=abc&response_type=code&${param}=first&${param}=second&_region=us`
        )
    ...
    it('returns 404 for unknown paths', async () => {
        const request = new Request('https://oauth.hanzo.ai/unknown')
        const response = await worker.fetch(request, mockEnv)
        expect(response.status).toBe(404)
    })

    it('returns the landing page at /', async () => {
        const request = new Request('https://oauth.hanzo.ai/')
        const response = await worker.fetch(request, mockEnv)
        const text = await response.text()
        expect(text).toContain('Insights OAuth Proxy')
    })
    ...
    it('catches async handler rejections and returns 500 instead of crashing', async () => {
        ...
        const request = new Request('https://oauth.hanzo.ai/oauth/authorize/?client_id=abc')
