...
from insights.llm.semantic_enrichment import extract_json_object
...
def classify_task_needs_repo(
    ...
    False if it does not (analytics, data queries, Insights config).
    ...
    prompt = (
        ...
        "querying data, Insights configuration, general knowledge questions, planning, or "
        "investigating product behavior in a Insights workspace using MCP/tools).\n\n"
        ...
        "recordings, traces, or Slack integrations inside Insights, unless the user explicitly "
        ...
        "needs_repo. But complaints about Insights itself as a product (its dashboards hanging, "
        "product pages loading slowly, UI bugs in Insights screens) are SaaS product issues, not "
        "the team's code → no_repo. Important exception: 'wrong data', 'missing events', or "
        "'numbers look off' in Insights usually means the team's tracking code is broken (wrong "
...
@activity.defn
def classify_insights_code_task_needs_repo_activity(
    event_text: str,
    thread_messages: list[dict[str, str]],
) -> bool:
...
def classify_message_is_agent_directed(
    ...
    """Classify whether a Slack thread reply is addressing the running Insights
    ...
    ``@Insights`` to recover. Cheap pre-LLM heuristics still drop trivial
    ...
    prompt = (
        "You are routing replies in a Slack thread where the Insights Slack App is "
        ...
        "  - Direct address ('@Insights', 'agent, please…', 'bot, …').\n"
...
def classify_untagged_followup_activity(
    ...
    try:
        ...
        logger.info(
            "insights_code_thread_message_mapping_gone",
            integration_id=inputs.integration_id,
            channel=channel,
            thread_ts=thread_ts,
        )
    ...
    try:
        ...
        logger.exception(
            "insights_code_thread_message_history_fetch_failed",
            channel=channel,
            thread_ts=thread_ts,
        )
        thread_history = []
    ...
    logger.info(
        "insights_code_thread_message_classified_chitchat",
        channel=channel,
        thread_ts=thread_ts,
...
def classify_slack_app_model_override(
    ...
    """
    prompt = (
        "You are routing a Slack message addressed to the Insights agent. Decide whether "
