 ...
 * - HogTransformerServiceConfig: CDP keys needed by the script transformer running in-process
...
export class IngestionGeneralServer implements NodeServer {
    ...
    private async startServices(): Promise<void> {
        ...
        const personRepository = buildPersonRepository(
            ...
            this.config.PERSONFN_PERSONS_ROLLOUT_PERCENTAGE,
            this.config.PERSONFN_PERSONS_ROLLOUT_TEAM_IDS,
            clientLabel
        )
        ...
        const groupRepository = buildGroupRepository(
            ...
            this.config.PERSONFN_GROUPS_ROLLOUT_PERCENTAGE,
            this.config.PERSONFN_GROUPS_ROLLOUT_TEAM_IDS,
            clientLabel
        )
        ...
        const startAi = (override?: { topic: string; groupId: string }) => {
            serviceLoaders.push(async () => {
                const consumerConfig = override
                    ...
                    : this.config
                // The AI lane can't construct the cdp-owned script transformer itself (boundary),
                ...
                // everything else (incl. its personinsights client), taking only config + parent scope.
        ...
        if (isCombinedMode) {
            ...
            const consumersOptions = [
                { topic: KAFKA_EVENTS_PLUGIN_INGESTION, group_id: 'datastore-ingestion' },
                { topic: KAFKA_EVENTS_PLUGIN_INGESTION_HISTORICAL, group_id: 'datastore-ingestion-historical' },
