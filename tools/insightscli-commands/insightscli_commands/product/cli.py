...
@click.command(
    ...
    help="Validate product.yaml owners against Insights/insights collaborator teams. "
...
def cmd_isolate_move(name: str, views: tuple[str, ...], dry_run: bool) -> None:
    ...
    if dry_run:
        ...
        click.echo("\nNext: review `git status`, then run tach check + lint-imports + insightscli product:lint")
