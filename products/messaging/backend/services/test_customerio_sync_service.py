...
class TestCustomerIOSyncService(BaseTest):
    def setUp(self):
        ...
        # A Insights-only category — should NOT be synced
        self.cat_insights = MessageCategory.objects.create(team=self.team, key="internal_updates", name="Internal")
    ...
    @patch("products.messaging.backend.services.customerio_sync_service.CustomerIOTrackClient")
    def test_does_not_sync_insights_only_categories(self, mock_client_class):
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client

        prefs = {str(self.cat_insights.id): PreferenceStatus.OPTED_OUT.value}
    ...
    def test_syncs_global_unsubscribe_without_customerio_categories(self, mock_client_class):
        ...
        # Team has only Insights-native categories, no customerio_* ones
