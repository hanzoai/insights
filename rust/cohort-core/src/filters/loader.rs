...
/// `insights_team` for the team timezone the bucket variants use for calendar-day computation.
pub const REALTIME_COHORTS_SQL: &str = "SELECT c.id, c.team_id, c.filters, \
            c.behavioral_filters_shape_hash, t.timezone \
     FROM insights_cohort c \
     JOIN insights_team t ON t.id = c.team_id \
...
pub struct CohortRow {
    ...
    /// `insights_team.timezone` — a non-null IANA zone name (default `"UTC"`).
    pub timezone: String,
}
...
/// Resolve a team's `insights_team.timezone`, falling back to UTC for an unrecognized zone. Counts
