...
import type { DashboardWidgetRunResultApi } from '@hanzo/products-dashboards/frontend/generated/api.schemas'
import { isWidgetConfigValidationError, updateDashboardWidgetTile } from '@hanzo/products-dashboards/frontend/utils'
import {
    DASHBOARD_WIDGET_CATALOG,
    getDashboardWidgetCatalogEntry,
} from '@hanzo/products-dashboards/frontend/widget_types/catalog'
import { DASHBOARD_WIDGET_FETCH_ERROR_MESSAGE } from '@hanzo/products-dashboards/frontend/widgets/constants'
import {
    applyIssueMetadataToWidgetListResult,
    type WidgetIssueMetadataContext,
    type WidgetIssueMetadataDelta,
} from '@hanzo/products-dashboards/frontend/widgets/error_tracking/applyWidgetIssueMetadataChange'
            ...
            null as DashboardType<QueryBasedInsightModel> | null,
            {
                ...
                copyToDashboard: async ({ tile, fromDashboard, toDashboard, toDashboardName }) => {
                    ...
                    if (tile.button_tile && !tile.insight && !tile.text && !tile.widget) {
                        toast.error('Copying button tiles to another dashboard is not supported')
                        return values.dashboard
                    }
                    ...
                    try {
                        ...
                        toast.error(`${copyErrorPrefix[widgetType]} to dashboard: ${String(e)}`)
                    }
            ...
            null,
            {
                updateWidgetTile: async ({
                    ...
                    try {
                        ...
                        toast.error(e instanceof ApiError ? (e.detail ?? e.message) : 'Could not update widget')
                        throw e
                    }
                },
            },
    ...
    listeners(({ actions, values, cache, props, sharedListeners }) => ({
        ...
        updateTileColor: async ({ tileId, color }) => {
            ...
            try {
                ...
                toast.error('Failed to update tile color')
            }
        },
        ...
        applyPendingInsertion: async () => {
            ...
            try {
                ...
                toast.error('Could not position the new tile: ' + String(e))
            }
        },
                    ...
                    button: {
                        ...
                        action: async () => {
                            try {
                                ...
                                toast.error('Could not restore tile: ' + String(e))
                            }
                        },
                    },
                }
        ...
        addWidgetTiles: async ({ dashboardId, widgets }) => {
            ...
            try {
                ...
                if (count > 1) {
                    toast.success(`Added ${count} widgets`)
                }
            } catch (e) {
                toast.error(e instanceof ApiError ? (e.detail ?? e.message) : 'Could not add widgets')
                throw e
            } finally {
        ...
        setDashboardMode: async ({ mode, source }) => {
            ...
            if (
                ...
                toast.info('Now editing the dashboard – press E or click Save to persist changes')
