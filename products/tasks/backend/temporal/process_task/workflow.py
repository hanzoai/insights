...
class ProcessTaskInput:
    ...
    insights_mcp_scopes: InsightsMcpScopes = "read_only"
...
class ProcessTaskWorkflow(InsightsWorkflow):
    def __init__(self) -> None:
        ...
        self._insights_mcp_scopes: InsightsMcpScopes = "read_only"
    ...
    def parse_inputs(inputs: list[str]) -> ProcessTaskInput:
        ...
        return ProcessTaskInput(
            ...
            insights_mcp_scopes=loaded.get("insights_mcp_scopes", "read_only"),
    ...
    async def run(self, input: ProcessTaskInput) -> ProcessTaskOutput:
        ...
        try:
            ...
            self._insights_mcp_scopes = input.insights_mcp_scopes
    ...
    async def _provision_and_start_agent(self, input: ProcessTaskInput, run_id: str) -> tuple[str, str, str | None]:
        ...
        # Run the Insights setup wizard before the agent, when this is a cloud wizard run.
        # The wizard integrates Insights and dirties the working tree; the agent then commits
        # those changes, opens the PR, and keeps it green (it never implements Insights itself).
        await self._run_wizard_if_configured(sandbox_output)

        # Start agent-server for direct connection from Insights Desktop
    ...
    def _build_resumed_input(self, input: ProcessTaskInput, sandbox_id: str) -> ProcessTaskInput:
        return ProcessTaskInput(
            ...
            insights_mcp_scopes=self._insights_mcp_scopes,
            prewarmed=False,
    ...
    async def _run_wizard_if_configured(self, sandbox_output: GetSandboxForRepositoryOutput) -> None:
        ...
        await self._emit_progress("wizard", "in_progress", "Running Insights setup wizard", "setup")
        await workflow.execute_activity(
            run_wizard,
            RunWizardInput(
                context=self.context,
        ...
        await self._emit_progress("wizard", "completed", "Ran Insights setup wizard", "setup")
