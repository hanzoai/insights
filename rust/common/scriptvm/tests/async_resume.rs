...
//!
//! These lock in the contract the CDP script-function path depends on: a program that calls a
...
use scriptvm::{execute_resumable, resume, ExecutionContext, Program, Resumable, VmSnapshot};
use serde_json::{json, Value};

// Opcodes used below (see common/scriptvm/python/operation.py).
...
fn resumes_a_node_produced_state() {
    // A real VMState captured from the Node reference VM (@hanzo/scriptvm) for the open-upvalue
