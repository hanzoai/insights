...
# Roots collected by the Django Core/CorePOE segments. insights/temporal is excluded — it is
# the Temporal segment, which always runs alongside the product's own temporal job.
_SCAN_ROOTS = ("insights", "ee", "products/product_analytics")
...
def test_core_facade_coupled_sources_are_covered_by_contract_check():
    root = _repo_root()
    excluded = root / "insights" / "temporal"  # the Temporal segment; always runs
