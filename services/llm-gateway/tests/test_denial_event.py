...
from llm_gateway.rate_limiting.denial_event import DENIAL_EVENT_NAME, InsightsDenialCapturer
...
def _context(user: AuthenticatedUser | None = None, end_user_id: str | None = None) -> ThrottleContext:
    return ThrottleContext(user=user or _user(), product="insights_code", end_user_id=end_user_id)
...
class TestInsightsDenialCapturer:
    ...
    def test_event_payload(
        self,
        capturer: InsightsDenialCapturer,
        ...
        assert props["product"] == "insights_code"
    ...
    @pytest.mark.asyncio
    async def test_call_dispatches_to_executor_inside_event_loop(self, capturer: InsightsDenialCapturer) -> None:
    ...
    def test_capture_sync_swallows_insights_failures(self, capturer: InsightsDenialCapturer) -> None:
        broken_client = MagicMock()
        broken_client.capture.side_effect = RuntimeError("network down")
        with patch("llm_gateway.rate_limiting.denial_event.Insights", return_value=broken_client):
            # Must not raise.
            capturer._capture_sync(distinct_id="d", event=DENIAL_EVENT_NAME, properties={})
        broken_client.shutdown.assert_called_once()

    def test_capture_sync_swallows_insights_constructor_failures(self, capturer: InsightsDenialCapturer) -> None:
        with patch("llm_gateway.rate_limiting.denial_event.Insights", side_effect=RuntimeError("bad host")):
            # Must not raise.
            capturer._capture_sync(distinct_id="d", event=DENIAL_EVENT_NAME, properties={})
