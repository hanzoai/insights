...
def _insightsql_table_tree(node: TableNode) -> dict[str, Any]:
    table = node.table
    return {
        "fields": sorted(table.fields) if isinstance(table, Table) else None,
        "children": {name: _insightsql_table_tree(child) for name, child in node.children.items()},
    }
...
class TestSessionExperimentContext(DatastoreTestMixin, APIBaseTest):
    ...
    def test_tags_its_scans_as_experiments(self) -> None:
        ...
        tagged_products = []
        original = insightsql_query_module.sync_execute

        def _capturing_sync_execute(*args: Any, **kwargs: Any) -> Any:
            tagged_products.append(get_query_tags().product)
            return original(*args, **kwargs)

        with patch.object(insightsql_query_module, "sync_execute", side_effect=_capturing_sync_execute):
    ...
    def test_excludes_private_experiments(self) -> None:
        ...
        other_user = self._create_user("other-experimenter@hanzo.ai")
        self._create_recording()
        self._create_experiment()
    ...
    def test_uncached_request_shares_one_readonly_insightsql_database(self) -> None:
        # Building the InsightsQL virtual database costs seconds on teams with a large warehouse
        ...
        def _capturing_create_for(*args: Any, **kwargs: Any) -> Database:
            ...
            built.append((database, _insightsql_table_tree(database.tables)))
            return database
        ...
        assert _insightsql_table_tree(database.tables) == tree_before_scans
    ...
    def test_batch_never_caches_sessions_a_single_request_would_enrich_more(self) -> None:
        ...
        # lookup's dict order — which follows Datastore row order. Pin it so the first
    ...
    def test_batch_builds_insightsql_database_once(self) -> None:
        # Two sessions on different days run as two day-chunks; the expensive InsightsQL database
    ...
    def test_batch_throttles_session_authenticated_users(self, _enabled: Any) -> None:
        # The Datastore* throttle pair only limits personal-API-key traffic; this endpoint's
