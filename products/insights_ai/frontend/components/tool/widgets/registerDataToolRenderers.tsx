...
// Product-specific tool-call renderers for the Insights AI agent run surface. These render Insights
...
// `requiresInsightsOrigin` on every entry: these render first-party Insights cards (some with an Open
// button built from a payload `_insightsUrl`), so they must only match a call that came through the
// trusted single-exec Insights server — not a user-installed MCP tool whose bare name collides with one
// of these keys. See `ToolRegistryEntry.requiresInsightsOrigin`.
function register(
    keys: string[],
    displayName: string,
    icon: JSX.Element,
    ...
    registerToolRenderers(
        keys.map((key): ToolRegistryEntry => ({ key, displayName, icon, Renderer, requiresInsightsOrigin: true }))
    )
}
