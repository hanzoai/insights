...
export interface ProductEmptyStateWizard {
    /** Subcommand for `npx -y @hanzo/wizard@latest <slug>` */
    slug: string
...
export interface ProductEmptyStateConfig {
    ...
    /** A `pngMascot()` illustration slot, rendered above the product name */
 ...
 * Both fields live in the scene's lazy chunk, so heavy assets (mascot PNGs,
