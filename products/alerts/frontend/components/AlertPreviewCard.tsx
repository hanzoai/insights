...
import { isFunnelsAlertConfig, isInsightsQLAlertConfig, isTrendsAlertConfig } from 'products/alerts/frontend/types'
...
import { InsightsQLAlertPreviewBanner } from './InsightsQLAlertPreview'
...
export function AlertPreviewCard({
    ...
    } else if (isInsightsQLAlertConfig(config) && insightsqlPreview) {
        body = <InsightsQLAlertPreviewBanner preview={insightsqlPreview} conditionType={alertForm.condition?.type} />
    }
                ...
                <div className="flex items-center gap-2">
                    {useLogScale ? (
                        <Tooltip title="A log scale keeps thresholds with very different values visually distinct.">
                            <Tag type="default" className="m-0">
                                Log scale
                            </Tag>
                        </Tooltip>
                    ) : null}
