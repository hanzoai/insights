...
`@hanzo/quill-blocks` is the product-level patterns layer of quill (tokens > primitives > components > blocks). It is currently an empty placeholder — `src/index.ts` exports nothing so downstream `export *` re-exports keep working while the layer is unimplemented.
