...
from products.insights_ai.backend.context_wrapper import (
    ALLOWED_TYPES,
    MAX_ATTACHED_ITEMS,
    MAX_TEXT_LENGTH,
    AttachedContext,
...
from products.insights_ai.backend.models.assistant import Thread
from products.insights_ai.backend.run_state import InsightsAIRunState
from products.insights_ai.backend.services.system_prompt.service import PromptService
from products.insights_ai.backend.wire_types import UnknownFrame, is_user_message_params, parse_log_entry
...
@contextmanager
def lock_conversation_for_followup(conversation_id: str, team_id: int) -> Iterator[Thread]:
    ...
    produces duplicate Runs. This helper takes a row-level `SELECT FOR UPDATE` on the `Thread`
    ...
    with transaction.atomic():
        ...
        conversation = Thread.objects.select_for_update().get(id=conversation_id, team_id=team_id)
        logger.debug(
...
class SandboxSession(BaseSandboxService):
    ...
    def __init__(self, conversation: "Thread", user: User) -> None:
        super().__init__(team=conversation.team, user=user)
        self.conversation: Thread = conversation
    ...
    def _warm(
        ...
        # Thread row lock released. The facade provisions under its own Task-row lock and
        # dispatches the workflow on commit; it is idempotent (an existing non-terminal Run is returned
        # as-is). `systemPrompt` is the one Insights AI-specific Run-state key the generic warmer can't know.
        try:
    ...
    def _handle_first_message(
        ...
        *,
        ...
        # Seed the Insights AI per-Run state keys. `attached_context` keeps the full,
        ...
        with lock_conversation_for_followup(str(self.conversation.id), self.team.pk) as locked:
            ...
            if convert_to_acp:
                locked.agent_runtime = Thread.AgentRuntime.SANDBOX
        ...
        if convert_to_acp:
            self.conversation.agent_runtime = Thread.AgentRuntime.SANDBOX

        # Start the run after the commit. `insights_mcp_scopes="full"` mirrors the legacy
        ...
        try:
            ...
            if convert_to_acp:
                self.conversation.agent_runtime = Thread.AgentRuntime.LANGGRAPH
    ...
    def _handle_in_progress_followup(
        ...
        *,
        ...
        try:
            ...
            # without leaving a duplicated `_insights/user_message` entry behind.
            logger.warning(
    ...
    def _handle_terminal_resume(
        ...
        *,
        ...
        The successor create runs under a row lock on the Thread: two
    ...
    def _collect_seen_entity_refs(self, run: "TaskRun") -> list[tuple[str, str | int]]:
        ...
        every prior `_insights/user_message` log entry's `_meta.attached_context`
    ...
    def _log_user_message(self, run: "TaskRun", wrapped: str, attached_context: list[AttachedContext]) -> None:
        """Append a `_insights/user_message` entry to the Run's ACP log.
