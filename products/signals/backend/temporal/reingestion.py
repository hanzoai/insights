...
async def soft_delete_report_signals_activity(input: SoftDeleteReportSignalsInput) -> None:
    """Soft-delete all Datastore signals for a report by re-emitting with metadata.deleted=True."""
...
async def process_team_signals_batch_activity(input: ProcessTeamSignalsBatchInput) -> ProcessTeamSignalsBatchOutput:
    ...
    await wait_for_signal_in_datastore_activity(
        WaitForDatastoreInput(
            team_id=input.team_id,
            signals=[
                WaitForDatastoreSignal(
            ...
            # confirm in Datastore before the next batch re-reads these rows.
            mode=WaitForDatastoreMode.CH_CONFIRMED,
        )
    )
...
class SignalReportReingestionWorkflow:
    ...
    async def run(self, inputs: SignalReportReingestionWorkflowInputs) -> None:
        with hanzo_insights.new_context(capture_exceptions=False):
            hanzo_insights.tag("team_id", inputs.team_id)
            hanzo_insights.tag("report_id", inputs.report_id)
    ...
    async def _run_impl(self, inputs: SignalReportReingestionWorkflowInputs) -> None:
        ...
        # 2. Soft-delete all signals in Datastore
        await workflow.execute_activity(
        ...
        # 2b. Wait for all soft-deleted signals to land in Datastore before re-ingesting
        await workflow.execute_activity(
            wait_for_signal_in_datastore_activity,
            WaitForDatastoreInput(
