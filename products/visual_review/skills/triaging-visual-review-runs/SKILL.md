...
description: >
  Inspects Insights Visual Review (VR) runs that gate PR merges with screenshot regression checks.
...
Visual Review is Insights's screenshot-regression product: CI captures storybook + playwright screenshots,
...
snapshot is approved or tolerated in the [VR UI](https://us.hanzo.ai/project/2/visual_review).
...
- A failing `visual-review` GitHub check or a PR comment from the `insights-bot` mentioning visual review.

When the user asks for the rendered diff image itself, the [VR web UI](https://us.hanzo.ai/project/2/visual_review)
...
| `insights:visual-review-runs-snapshot-history-list` | A single story's last N runs across master/PRs — the flake check.                                                                                                                                                                        |
| `insights:visual-review-runs-counts-retrieve`       | Aggregate counts for queue triage (how many runs in `needs_review`, etc.).                                                                                                                                                               |
| `insights:visual-review-runs-tolerated-hashes-list` | Hashes the team has explicitly accepted as "known flake / acceptable variation".                                                                                                                                                         |
| `insights:visual-review-repos-list`                 | Repos (one per GitHub repo) — usually only one matters; useful for filtering.                                                                                                                                                            |
| `insights:visual-review-repos-retrieve`             | Repo metadata: baseline file paths, PR-comment configuration.                                                                                                                                                                            |
...
1. `insights:visual-review-runs-list { pr_number: <n>, limit: 5 }` — sort by `created_at` desc, take the latest non-stale one.
2. If the run has `summary.changed > 0` or `summary.unresolved > 0`, drill in:
   `insights:visual-review-runs-snapshots-list { id: <run_id> }` and report the `changed` snapshots.

Report back: PR number, run UUID, `review_state`, summary counts, and the `_insightsUrl` deep link so the
...
1. **Scope check** — `git diff master...HEAD --stat` (or against the PR's base branch) → list of touched paths.
   Cross-reference with `insights:visual-review-runs-snapshots-list { id }` filtered to `result: changed` → story identifiers.
...
`insights:visual-review-runs-snapshot-history-list { id: <snapshot_id> }` → returns prior outcomes for the same story.
...
1. `insights:visual-review-runs-counts-retrieve` → total queue size.
2. `insights:visual-review-runs-list { review_state: needs_review, limit: 50 }` (paginate if needed).
...
include the `_insightsUrl` deep link to the run — humans need to see the rendered images to make the call,
