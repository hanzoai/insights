---
name: extending-personinsights-test-harness
description: >
  When and how to add scenarios, chaos events, and invariants to the personinsights
  e2e test harness (rust/personinsights-test-harness). Use after fixing a bug or
  regression in the personinsights leader path (leader, router, writer, replica,
  ...
  terms: personinsights gate, chaos scenario, test harness, leader path regression,
  handoff bug, eviction, writer lag, acked write.
---

# Extending the personinsights test harness

The harness (`rust/personinsights-test-harness/`, see its README for usage) spawns a
real personinsights stack and asserts one invariant three ways: **every acked write
...
Add one whenever you fix a personinsights bug whose _trigger_ the harness can
...
6. Add the scenario to the `personinsights-gate` CI job in
...
```bash
cargo build -p personinsights-replica -p personinsights-router -p personinsights-leader -p personinsights-writer
personinsights-test-harness gate <scenario flags> --bin-dir <fix-worktree>/rust/target/debug
```
