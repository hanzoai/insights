...
class TestTeamLogsConfig(BaseTest):
    def test_default_attribute_key(self):
        ...
        # Matches the JS SDK / docs convention so logs from insights-js are linked to
        # their person without any team configuration.
        assert config.logs_distinct_id_attribute_key == "insightsDistinctId"
        assert config.logs_distinct_id_attribute_keys == ["insightsDistinctId"]

    def test_custom_attribute_keys_persist(self):
        config = get_or_create_team_extension(self.team, TeamLogsConfig)
        config.logs_distinct_id_attribute_keys = ["user.id", "insightsDistinctId"]
        config.save()

        refetched = get_or_create_team_extension(self.team, TeamLogsConfig)
        assert refetched.logs_distinct_id_attribute_keys == ["user.id", "insightsDistinctId"]
    ...
    def test_migration_backfill_carries_customized_single_key_into_array(self):
        ...
        customized.logs_distinct_id_attribute_keys = ["insightsDistinctId"]
        customized.save()
