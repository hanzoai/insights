...
locals {
  export_insight_regions = {
    us = {
      ...
      unknown_result_sql = "https://metabase.prod-us.insights.dev/question/1838-analyse-unknown-exportedasset-exceptions"
    }
    eu = {
      table_name         = "eu_insights_exportedasset"
      dashboard_ids      = [insights_dashboard.team_analytics_platform_key_metrics.id]
      unknown_result_sql = "https://metabase.prod-eu.insights.dev/question/565-analyse-unknown-exportedasset-exceptions"
    }
  }
...
import {
  to = insights_insight.dashboard_created_unique_users_by_event_s_source
  id = "2/7067184"
}

resource "insights_insight" "dashboard_created_unique_users_by_event_s_source" {
...
import {
  to = insights_insight.alert_failures_by_exception_type_aggregated
  id = "2/6984945"
}

resource "insights_insight" "subscription_delivery_success_rate" {
...
resource "insights_insight" "alert_failures_by_exception_type_aggregated" {
  ...
  tags = ["managed-by:terraform"]
  dashboard_ids = [1142938, 821321, insights_dashboard.team_analytics_platform_key_metrics.id, 1294240]
}
