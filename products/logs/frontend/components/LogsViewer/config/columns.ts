...
export interface LogsColumnConfig {
     ...
     * `resource_attributes.<key>`, `body.<json.path>`) or a scalar InsightsQL expression, sent
...
function escapeInsightsQLString(value: string): string {
...
export function attributeLookupExpression(key: string): string {
    const escaped = escapeInsightsQLString(key)
