...
func (c *InsightsKafkaConsumer) Consume(ctx context.Context) {
...
func (c *InsightsKafkaConsumer) runParsing(ctx context.Context) {
	for {
		value, ok := <-c.incoming
		if !ok {
			return
...
func parse(geolocator geo.GeoLocator, classifier *bot.Classifier, kafkaMessage []byte) InsightsEvent {
