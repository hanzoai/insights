...
Given an MCP session, fetch its per-tool-call ``$mcp_intent``s from Datastore
...
# get a definitive answer and we don't re-query Datastore on the next request.
...
def summarize_intents(intents: list[str], team: Team) -> str:
    ...
    client = OpenAI(insights_client=hanzo_insights.default_client, base_url=settings.OPENAI_BASE_URL)
    try:
        response = client.chat.completions.create(  # type: ignore
            ...
            insights_properties={"ai_product": "mcp_analytics", "ai_feature": "session-intent-generation"},
        )
...
def summarize_project_intents(intents: list[tuple[str, str]], team: Team) -> IntentThemesSchema:
    ...
    try:
        response = client.models.generate_content(
            ...
            config=config,
            insights_distinct_id=f"team/{team.id}/mcp-analytics-intent-digest",
            insights_properties={"ai_product": "mcp_analytics", "ai_feature": "activity-intent-digest"},
            insights_groups={"project": str(team.id)},
        )
