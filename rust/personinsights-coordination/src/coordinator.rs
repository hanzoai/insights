...
impl Coordinator {
    ...
    async fn try_lead(&self, cancel: CancellationToken) -> Result<bool> {
        ...
        gauge!("personinsights_coordination_is_coordinator").set(1.0);
        counter!("personinsights_coordination_elections_won_total").increment(1);
    ...
    async fn handle_pod_change_static(
        ...
        for disposition in &replaced_dispositions {
            counter!(
                "personinsights_coordination_handoffs_replaced_total",
        ...
        counter!("personinsights_coordination_handoffs_created_total", "kind" => "move")
            .increment(moves as u64);
        counter!("personinsights_coordination_handoffs_created_total", "kind" => "fresh")
    ...
    async fn log_cancellation(
        ...
        counter!(
            "personinsights_coordination_handoffs_cancelled_total",
            "reason" => reason,
        )
        .increment(1);
        for router in &missing_ackers {
            counter!(
                "personinsights_coordination_freeze_ack_missing_total",
                "router" => router.clone(),
            )
            .increment(1);
        }
...
fn record_phase_advance(handoff: &HandoffState, to: HandoffPhase) {
    ...
    counter!(
        "personinsights_coordination_handoff_transitions_total",
    ...
    if let Some(reached_ms) = reached_ms {
        histogram!(
            "personinsights_coordination_handoff_phase_reached_ms",
    ...
    if handoff.phase_entered_at_ms > 0 {
        histogram!(
            "personinsights_coordination_handoff_phase_duration_ms",
...
fn record_cluster_gauges(handoffs: &[HandoffState], pods: &[RegisteredPod], routers: usize) {
    ...
    for phase in [
        ...
        let max_age_secs = handoffs
            ...
            .max(0);
        gauge!(
            "personinsights_coordination_handoff_phase_age_seconds",
            "phase" => phase_label(phase),
        )
        .set(max_age_secs as f64);
        let count = handoffs.iter().filter(|h| h.phase == phase).count();
        gauge!("personinsights_coordination_handoffs_in_flight", "phase" => phase_label(phase))
            .set(count as f64);
    }
    for (status, label) in [
        ...
        gauge!("personinsights_coordination_pods_registered", "status" => label).set(count as f64);
    }
    gauge!("personinsights_coordination_routers_registered").set(routers as f64);
}
...
fn reset_coordinator_gauges() {
    gauge!("personinsights_coordination_is_coordinator").set(0.0);
