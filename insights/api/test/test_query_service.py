...
from insights.exceptions import DatabaseSchemaUnavailable
...
class TestQueryService(APIBaseTest):
    ...
    def test_data_visualization_node_surfaces_insightsql_resolution_error_without_value_error_context(
        self, mock_get_query_runner_or_none: MagicMock
    ):
        query = DataVisualizationNode(source=InsightsQLQuery(query="SELECT properties FROM events JOIN persons ON 1 = 1"))
        runner = MagicMock()
    ...
    @patch("insights.api.services.query.DataWarehouseJoin.objects.filter")
    @patch("insights.api.services.query.resolve_database_for_connection")
            ...
            "selected_table": DatabaseSchemaDataWarehouseTable(
                fields={
                    "selected_join": DatabaseSchemaField(
                        name="selected_join",
                        insightsql_value="selected_join",
    ...
    @patch("insights.api.services.query.get_insightsql_autocomplete")
    @patch("insights.api.services.query.resolve_database_for_connection")
    def test_insightsql_autocomplete_without_connection_hides_direct_tables(
        self,
        mock_resolve_database_for_connection: MagicMock,
        mock_get_insightsql_autocomplete: MagicMock,
    ):
        database = Database()
        ...
        def _mock_autocomplete(*args, **kwargs):
            ...
            return InsightsQLAutocompleteResponse(suggestions=[], incomplete_list=False)

        mock_get_insightsql_autocomplete.side_effect = _mock_autocomplete
        ...
        self.assertEqual(response, InsightsQLAutocompleteResponse(suggestions=[], incomplete_list=False))

    @patch("insights.api.services.query.get_insightsql_autocomplete")
    @patch("insights.api.services.query.resolve_database_for_connection")
    def test_insightsql_autocomplete_with_direct_connection_hides_insights_tables(
        self,
        mock_resolve_database_for_connection: MagicMock,
        mock_get_insightsql_autocomplete: MagicMock,
    ):
        ...
        database.tables.add_child(
            TableNode(
                ...
                table=PostgresTable(name="insights_dashboard", fields={}, postgres_table_name="insights_dashboard"),
            )
        )
        database._connection_id = str(source.id)
        database._warehouse_table_names = ["insights_dashboard"]
        database.apply_schema_scope()
        ...
        def _mock_autocomplete(*args, **kwargs):
            database_arg = kwargs["database_arg"]
            self.assertIsNotNone(database_arg)
            self.assertTrue(database_arg.has_table("insights_dashboard"))
        ...
        self.assertEqual(mock_get_insightsql_autocomplete.call_args.kwargs["user"], None)

    @patch("insights.api.services.query.get_insightsql_autocomplete")
    @patch("insights.api.services.query.resolve_database_for_connection")
    def test_insightsql_autocomplete_with_direct_connection_filters_other_source_tables(
        self,
        mock_resolve_database_for_connection: MagicMock,
        mock_get_insightsql_autocomplete: MagicMock,
    ):
    ...
    @patch("insights.api.services.query.get_insightsql_autocomplete")
    @patch("insights.api.services.query.resolve_database_for_connection")
    def test_insightsql_autocomplete_with_direct_connection_exposes_selected_tables_only(
        self,
        mock_resolve_database_for_connection: MagicMock,
        mock_get_insightsql_autocomplete: MagicMock,
    ):
    ...
    @patch("insights.api.services.query.get_insightsql_autocomplete")
    @patch("insights.api.services.query.resolve_database_for_connection")
    def test_insightsql_autocomplete_passes_user_context(
        self,
        mock_resolve_database_for_connection: MagicMock,
        mock_get_insightsql_autocomplete: MagicMock,
    ):
        ...
        database = Database()
        mock_resolve_database_for_connection.return_value = (source, database)
        mock_get_insightsql_autocomplete.return_value = InsightsQLAutocompleteResponse(suggestions=[], incomplete_list=False)
        ...
        self.assertEqual(mock_get_insightsql_autocomplete.call_args.kwargs["user"], self.user)
    ...
    def test_query_service_rejects_soft_deleted_connection_ids(self, _label: str, query_cls):
        query: InsightsQLAutocomplete | DatabaseSchemaQuery
        source = ExternalDataSource.objects.create(
            source_id="selected-upstream-source",
            connection_id="selected-connection",
            destination_id="destination-1",
        ...
        if query_cls is InsightsQLAutocomplete:
    ...
    def test_database_schema_query_without_connection_excludes_direct_sources(
        ...
        mock_database.serialize.return_value = {
            ...
            "events": DatabaseSchemaInsightsTable(fields={}, id="events", name="events"),
        }
        mock_resolve_database_for_connection.return_value = (None, mock_database)
        mock_join_queryset = MagicMock()
        mock_filtered_join_queryset = MagicMock()
    ...
    def test_database_schema_query_without_connection_preserves_insights_tables_with_direct_name_collisions(self):
        ...
        self.assertIsInstance(response.tables["events"], DatabaseSchemaInsightsTable)
        self.assertIsInstance(response.tables["persons"], DatabaseSchemaInsightsTable)
