...
class TestPathTraversalValidation:
    ...
    def test_valid_inputs_pass_validation(self, app: str, name: str) -> None:
        ...
        from common.migration_utils import validate_migration_path_components
    ...
    @parameterized.expand(
        [
            ...
            ("../etc", "0001_passwd", "app"),
            ("insights/../../../etc", "0001_test", "app"),
            ("..", "0001_test", "app"),
            ("insights/../../bad", "0001_test", "app"),
            ("./insights", "0001_test", "app"),
            # Path traversal via migration name
            ("insights", "../../../etc/passwd", "migration"),
            ("insights", "0001_../../bad", "migration"),
            ("insights", "0001_test/../../../etc", "migration"),
            ("insights", "./0001_test", "migration"),
        ]
    )
    ...
    @parameterized.expand(
        [
            ...
            ("insights;rm -rf /", "0001_test", "app"),
            ("insights|whoami", "0001_test", "app"),
            ("insights`id`", "0001_test", "app"),
            ("insights$(whoami)", "0001_test", "app"),
            ("insights&echo", "0001_test", "app"),
            # Shell injection via migration name
            ("insights", "0001_test;DROP TABLE", "migration"),
            ("insights", "0001_test|cat /etc/passwd", "migration"),
            ("insights", "0001_`id`", "migration"),
            ("insights", "0001_$(whoami)", "migration"),
        ]
    )
    ...
    @parameterized.expand(
        [
            ...
            ("insights", "", "Empty migration name"),
            ("insights", "0001", "Missing description after number"),
            ("insights", "001_test", "Wrong digit count (3)"),
            ("insights", "00001_test", "Wrong digit count (5)"),
            ("insights", "test_0001", "Number not at start"),
            ("insights", "0001-test", "Hyphen instead of underscore"),
            ("insights", "0001_test.py", "Includes file extension"),
            ("insights", "0001 test", "Space in name"),
            ("insights", "0001_Test-Name", "Hyphen in description"),
        ]
    )
...
class TestCachePathSecurity:
    ...
    def test_get_cache_path_validates_inputs(self) -> None:
        ...
        from common.migration_utils import get_cache_path

        # Valid input should return a path
        path = get_cache_path("insights", "0001_initial")
...
class TestSharedMigrationUtils:
    ...
    def test_validate_raises_for_invalid_migration(self) -> None:
        ...
        with pytest.raises(ValueError, match="Invalid migration name"):
            validate_migration_path_components("insights", "bad_name")

    def test_is_valid_returns_bool(self) -> None:
        """is_valid_migration_path should return bool without raising."""
        from common.migration_utils import is_valid_migration_path

        assert is_valid_migration_path("insights", "0001_initial") is True
        assert is_valid_migration_path("../etc", "0001_passwd") is False
        assert is_valid_migration_path("insights", "bad") is False
    ...
    def test_get_cached_migration_returns_none_for_invalid(self) -> None:
        ...
        from common.migration_utils import get_cached_migration

        # Invalid inputs should return None (not raise)
        assert get_cached_migration("../etc", "0001_passwd") is None
        assert get_cached_migration("insights", "bad") is None
    ...
    def test_cache_migration_file_returns_false_for_invalid(self) -> None:
        ...
        assert cache_migration_file("insights", "bad", Path("/fake")) is False
