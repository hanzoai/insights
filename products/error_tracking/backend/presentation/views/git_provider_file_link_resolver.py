...
from insights.egress.github.transport import GitHubEgressBudgetExhausted, github_request
...
def search_github_file(
    ...
    rate-limit gauges are recorded; the public Insights-token path leaves it None (no installation).
...
class GitProviderFileLinksViewSet(TeamAndOrgViewSetMixin, viewsets.ViewSet):
    ...
    def resolve_github(self, request, **kwargs):
        ...
        # Try with Insights's token first (public repos). Skip while the circuit is open — a dead PAT
