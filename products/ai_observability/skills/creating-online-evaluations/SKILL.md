...
description: >
  Author continuously-running online evaluations in Insights AI observability, grounded in real failure
  ...
  eval type (script / llm_judge / sentiment), configuring a provider, model, and usable provider key for an
...
For the mechanics of _writing and iterating_ an evaluator (Script source vs LLM-judge prompt, dry-running,
...
they'll see when it fails. No Script snippets, property filters, or `script`/`llm_judge` internals here. Number
...
Reach for `script` first, escalate to `llm_judge` if there is no deterministic way to check for what we want to check.
...
evaluation. `$ai_session_id` is not `$session_id`: the second is Insights's product-analytics
...
New Script source should use the globals shared by all targets:
...
evaluators. Session evaluations do not carry them: session Script source only receives `target` and
...
Script and sentiment evaluations skip this step.

```json
insights:llma-evaluation-config-get        // check active_provider_key for an unpinned judge
insights:llma-provider-key-list            // find an ok-state key to pin
insights:llma-evaluation-judge-models      // { "provider": "openai" } → valid models
```
...
Create with `enabled: false` so nothing fires until the scope is verified. Minimal `script` example:

Evaluations may be created at the top level or in one directory. If the user names a directory, call
`insights:llma-evaluation-directory-list` and pass its UUID as `directory_id`. Create a directory only when
...
`llma-evaluation-test-script` (script) or `llma-evaluation-run` against one generation (llm_judge).
...
only `rollout_percentage` (0–100). Property filters use the standard Insights shape
...
Build links with `insights:generate-app-url` — never hand-write the host or the `/project/<id>/` prefix.
...
`https://us.hanzo.ai/project/<id>/ai-evals/evaluations/<evaluation_id>`). Surface the link after
...
- **`script` first.** No provider key, no AI approval, deterministic. Reach for `llm_judge` only when the
...
- **`bytecode` is server-written** for script evals — never pass it; send only `evaluation_config.source`.
