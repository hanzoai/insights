...
class TestGetTeamBatchesActivity(DatastoreTestMixin, APIBaseTest):
...
class TestRefreshRecommendationsBatchActivity(APIBaseTest):
    ...
    def test_skips_deleted_teams_without_failing_batch(self, _alerts, _long, _source, _rate_limits):
        # Datastore retains events for teams later deleted from Postgres; the batch must
...
class TestLongRunningIssuesComputeBatch(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
    ...
    def _create_issue_with_events(
        ...
        sync_issues_to_datastore(issue_ids=[issue.id], team_id=team.id)
