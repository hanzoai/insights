...
from insights.models.utils import convert_legacy_metrics
...
class ExperimentAdmin(admin.ModelAdmin):
    ...
    def team_link(self, experiment: Experiment):
        return format_html(
            '<a href="{}">{}</a>',
            reverse("admin:insights_team_change", args=[experiment.team.pk]),
    ...
    def migrated_links(self, experiment: Experiment):
        if experiment.stats_config and "migrated_from" in experiment.stats_config:
            return format_html(
                ...
                reverse("admin:insights_experiment_change", args=[experiment.stats_config["migrated_from"]]),
        ...
        if experiment.stats_config and "migrated_to" in experiment.stats_config:
            return format_html(
                ...
                reverse("admin:insights_experiment_change", args=[experiment.stats_config["migrated_to"]]),
                experiment.stats_config["migrated_to"],
            )
        return ""

    change_form_template = "admin/insights/experiment/change_form.html"
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        if obj is None:
            ...
            return redirect("admin:insights_experiment_changelist")
            ...
            shared_metrics_status.append(
                {
                    ...
                    "is_legacy": is_legacy,
                    "migrated": migrated,
                    "migrate_url": reverse("admin:insights_experimentsavedmetric_change", args=[metric.id]),
                }
            )
        ...
        try:
            with transaction.atomic():
                ...
                if original.stats_config and original.stats_config.get("migrated_to"):
                    ...
                    return redirect("admin:insights_experiment_change", original.stats_config["migrated_to"])
            ...
            return redirect("admin:insights_experiment_change", new_experiment.pk)
        except Experiment.DoesNotExist:
            messages.error(request, "Experiment not found")
            return redirect("admin:insights_experiment_changelist")
        except Exception as e:
            messages.error(request, f"Error migrating experiment: {e}")
            return redirect("admin:insights_experiment_change", object_id)
