...
from insights.models.cohort.cohort import CohortType
...
class TestCohort(TestExportMixin, DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    @patch("insights.models.cohort.util.sync_execute", side_effect=sync_execute)
...
"""
    ...
    def test_cohort_list_with_search(self):
        ...
        response = self.client.get(f"/v1/projects/{self.team.id}/cohorts?search=cohort1").json()
        self.assertEqual(len(response["results"]), 1)
        self.assertEqual(response["results"][0]["name"], "cohort1")

        response = self.client.get(f"/v1/projects/{self.team.id}/cohorts?search=nomatch").json()
    ...
    def test_creating_with_query_and_fields(self, patch_capture):
        ...
        query_get_response = self.client.get(
            f"/v1/projects/{self.team.id}/cohorts/{query_post_response.json()['id']}/"
        )
    ...
    @patch("django.db.transaction.on_commit", side_effect=lambda func: func())
    @patch("insights.models.cohort.dependencies._on_cohort_changed")
    ...
    def test_cohort_last_error_message_from_calculation_history(self):
        ...
        from insights.models.cohort.util import CohortErrorCode
