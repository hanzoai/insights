//! InsightsQLX tag-element parsing — JSX-like `<Tag attr='v' >children</Tag>`
//! and self-closing `<Tag />` syntax (grammar rules `insightsqlxTagElement` /
//! `insightsqlxTagAttribute` / `insightsqlxChildElement` / `insightsqlxText`).
//!
//! Three contexts admit a tag:
//!   - top of a SELECT (`select: insightsqlxTagElement`)
//!   - FROM table position (`tableExpr: insightsqlxTagElement`)
//!   - column position (`columnExpr: insightsqlxTagElement`)
//!
//! All three route to `parse_insightsqlx_tag_element` after detecting
...
//!
//! cpp's emit:
//!   - `<Tag a='v' />`   → InsightsQLXTag(kind="Tag", attributes=[Attr(a, Constant("v"))])
//!   - `<Tag a={e} />`   → InsightsQLXTag(kind="Tag", attributes=[Attr(a, e)])
//!   - `<Tag a />`       → InsightsQLXTag(kind="Tag", attributes=[Attr(a, Constant(true))])
//!   - `<Tag>x</Tag>`    → InsightsQLXTag(..., attributes=[Attr("children", [Constant("x")])])
//!   - `<Tag></Tag>`     → InsightsQLXTag(..., attributes=[])  (no children attr when empty)
//!
//! Ported verbatim from `parser_backtrack/src/parse/insightsqlx.rs`; the
...
impl<'a, E: Emitter + Clone> Parser<'a, E> {
    /// True when peek/peek_next look like the start of a InsightsQLX tag —
    ...
    pub(crate) fn peek_starts_insightsqlx_tag(&self) -> bool {
    ...
    /// Like `peek_starts_insightsqlx_tag`, but one token further ahead: is the
    ...
    /// the token after it.
    pub(crate) fn peek_next_starts_insightsqlx_tag(&self) -> bool {
    ...
    /// cpp's `isOpeningTag()` lexer predicate (`InsightsQLLexer.cpp.g4`). It decides
    /// whether a `<` enters cpp's dedicated InsightsQLX tag lexer mode ("tight": tag
    ...
    /// then `[A-Za-z0-9_]` / `>` / `/`.
    fn insightsqlx_tag_is_tight(&self, lt_end: usize) -> bool {
        ...
        match bytes.get(i) {
            ...
            Some(&c) if c.is_ascii_whitespace() => {
                let j = self.insightsqlx_skip_ws_and_comments(i);
    ...
    fn insightsqlx_skip_ws_and_comments(&self, mut i: usize) -> usize {
        let bytes = self.src.as_bytes();
        loop {
    ...
    /// Parse a InsightsQLX tag element starting at `<`.
    ///
    ...
    /// "Opening and closing InsightsQLX tags must match" error otherwise)
    ...
    /// tags): cpp's `INSIGHTSQLX_TEXT` lexer mode admits any byte except
    /// `<` / `{` inside the body, but rust's mode-less lexer rejects
    /// punctuation like `&` / `!` / `@` when pre-loading peek1 across
    /// a `>` / `/>` / `</…>` boundary. `parse_insightsqlx_children`
    /// byte-walks the body directly and re-seeks the lexer via
    /// `consume_insightsqlx_text`, so peek1's transient invalid state is
    /// recoverable.
    pub(crate) fn parse_insightsqlx_tag_element(&mut self) -> Result<E::Value, ParseError> {
        self.insightsqlx_text_lookahead_depth += 1;
        // cpp pushes INSIGHTSQLX_TAG_OPEN at `<`; those tag modes have no
        ...
        let was_in_tag = self.lexer.in_insightsqlx_tag();
        self.lexer.set_in_insightsqlx_tag(true);
        let result = self.parse_insightsqlx_tag_element_inner();
        self.lexer.set_in_insightsqlx_tag(was_in_tag);
    ...
    fn reseek_peek_window(&mut self, pos: usize) {
        let in_tag = self.lexer.in_insightsqlx_tag();
        self.lexer = Lexer::with_pos(self.src, pos);
        self.lexer.set_in_insightsqlx_tag(in_tag);
    ...
    fn parse_insightsqlx_tag_element_inner(&mut self) -> Result<E::Value, ParseError> {
        ...
        let tight = self.insightsqlx_tag_is_tight(self.last_consumed_end);
        ...
        // INSIGHTSQLX_TAG_CLOSE mode (bare only — see the `false` below), so a quoted
        // opening can only ever pair with a self-closing `/>`, never `</"x">`.
        let kind = self.parse_insightsqlx_identifier("tag name", !tight)?;
        let mut attributes: Vec<E::Value> = Vec::new();
        loop {
            match self.peek() {
                TokenKind::SlashGt => {
                    self.bump()?;
                    return Ok(self.wrap_pos(self.emit.insightsqlx_tag(&kind, attributes), tag_start));
                }
                TokenKind::Gt => {
                    self.bump()?;
                    let children = self.parse_insightsqlx_children(tight)?;
                    self.expect(TokenKind::LtSlash, "</")?;
                    let close = self.parse_insightsqlx_identifier("closing tag name", false)?;
                    if close != kind {
                        return Err(self.err(format!(
                            "Opening and closing InsightsQLX tags must match. Got {} and {}",
                            kind, close
                        )));
                    }
                    ...
                    if !children.is_empty() {
                        if attributes.iter().any(|a| {
                            ...
                            return Err(self.err(
                                "Can't have a InsightsQLX tag with both children and a 'children' attribute",
                            ));
                        }
                        let kids = self.emit.list_value(children);
                        attributes.push(self.emit.insightsqlx_attribute("children", kids));
                    }
                    return Ok(self.wrap_pos(self.emit.insightsqlx_tag(&kind, attributes), tag_start));
                }
                TokenKind::Ident | TokenKind::QuotedIdent | TokenKind::Keyword(_) => {
                    attributes.push(self.parse_insightsqlx_attribute(tight)?);
                }
                TokenKind::Eof => {
                    return Err(self.err("unexpected end of input inside InsightsQLX tag"));
                }
                _ => {
                    return Err(self.err(format!(
                        "expected attribute name, `>`, or `/>` inside InsightsQLX tag, got {:?}",
                        self.peek()
                    )));
                }
            }
    ...
    /// Parse one of:
    ///   - `name = 'string'` → InsightsQLXAttribute(name, Constant(string))
    ///   - `name = { expr }` → InsightsQLXAttribute(name, expr)
    ///   - `name`            → InsightsQLXAttribute(name, Constant(true))
    fn parse_insightsqlx_attribute(&mut self, tight: bool) -> Result<E::Value, ParseError> {
        // cpp positions the InsightsQLXAttribute from the name start to the value end
        ...
        let name = self.parse_insightsqlx_identifier("attribute name", true)?;
        let name_end = self.last_consumed_end;
        // No `=` → bare attribute, value is Constant(true).
        if self.peek() != TokenKind::EqDouble {
            let attr = self
                .emit
                .insightsqlx_attribute(&name, self.emit.constant(self.emit.bool(true)));
        ...
        let value = match self.peek() {
            ...
            TokenKind::LBrace => {
                ...
                // other mode.
                self.lexer.set_in_insightsqlx_tag(false);
                self.reseek_peek_window(self.last_consumed_end);
        ...
        let attr = self.emit.insightsqlx_attribute(&name, value);
    ...
    fn parse_insightsqlx_children(&mut self, tight: bool) -> Result<Vec<E::Value>, ParseError> {
        let mut children: Vec<E::Value> = Vec::new();
        loop {
            // A tight tag's `>` opens cpp's INSIGHTSQLX_TEXT lexer mode, which captures
            // child text (incl. whitespace). A loose tag's `>` is a plain GT in the
            // default mode — there is no INSIGHTSQLX_TEXT token, so child text isn't
            ...
            if tight {
                // `consume_insightsqlx_text` scans from `last_consumed_end`, so that is
            ...
            match self.peek() {
                ...
                TokenKind::Lt => {
                    ...
                    let nested = self.parse_insightsqlx_tag_element()?;
                ...
                TokenKind::LBrace => {
                    ...
                    // the loop's `consume_insightsqlx_text` byte-walks from
                ...
                TokenKind::Eof => {
                    return Err(self.err("unexpected end of input inside InsightsQLX tag children"));
                }
                _ => {
                    // Tight: unreachable — `consume_insightsqlx_text` advances past
                    ...
                    return Err(self.err(format!(
                        "unexpected token in InsightsQLX tag children: {:?}",
                        self.peek()
                    )));
                }
            }
    ...
    fn consume_insightsqlx_text(&mut self) -> Result<String, ParseError> {
        ...
        // `insightsqlx_text_lookahead_depth > 0` the parser may carry a
    ...
    /// `INSIGHTSQLX_TAG_OPEN` / `INSIGHTSQLX_TAG_CLOSE` lexer modes
    /// (`InsightsQLLexer.common.g4:315 + 326`) admit
    ...
    fn parse_insightsqlx_identifier(
        &mut self,
        what: &str,
        allow_quoted: bool,
    ) -> Result<String, ParseError> {
        let head = self.bump()?;
        // A tag/attr name is an `identifier` (grammar `insightsqlxTagElement` /
        // `insightsqlxTagAttribute`), so a keyword head is only valid when it is a
        // grammar-`keyword`-rule member. The Script-statement keywords (fn/let/…),
