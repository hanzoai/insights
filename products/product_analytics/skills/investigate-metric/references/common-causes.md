...
Suggest pausing or reverting; offer `insights:annotation-create` if no annotation exists.
...
- `insights:read-data-schema` (`kind: "events"`) for recently renamed / deprecated events.

## Cohort / lifecycle shift

Same product, different mix of users. New-user influx pulls engagement metrics down.

- `insights:query-lifecycle` — change in new / returning / resurrecting / dormant mix.
- `insights:query-retention` comparing affected-period cohorts to prior.
...
- Cross-check with `insights:query-error-tracking-issues-list`.

## Rate limit / upstream outage

A consumer hit a quota or an upstream dependency degraded.

- `insights:query-trends` on API error events.
- `insights:query-logs` for an error surge in the window.
