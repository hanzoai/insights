...
class TestCohortUtils(BaseTest):
    ...
    def test_print_cohort_insightsql_query_raises_error_on_mixed_id_types_in_union(self):
        ...
        with patch("insights.insightsql_queries.query_runner.get_query_runner", mock_get_query_runner):
    ...
    def test_print_cohort_insightsql_query_drops_order_by_on_stripped_alias(self):
        ...
        ORDER BY, so InsightsQL resolution raises `QueryError: Unable to resolve field: <alias>`.
        """
        cohort = Cohort.objects.create(
            team=self.team,
    ...
    def test_print_cohort_insightsql_query_keeps_order_by_and_limit_for_bounded_query(self):
    ...
    def test_print_cohort_insightsql_query_source_without_id_column_uses_table_fallback(self, _name, source_sql, expected):
        ...
        # print_cohort_insightsql_query could apply its events→person.id / persons→id fallback.
        cohort = Cohort.objects.create(
            team=self.team,
            name="Test Source Without ID Column",
            query={
                "kind": "ActorsQuery",
                "source": {"kind": "InsightsQLQuery", "query": source_sql},
            },
        )
    ...
    def test_print_cohort_insightsql_query_source_without_id_column_still_raises_when_unresolvable(self):
        ...
        cohort = Cohort.objects.create(
            ...
            query={
                "kind": "ActorsQuery",
                "source": {"kind": "InsightsQLQuery", "query": "SELECT count() AS cnt FROM sessions"},
            },
        )
    ...
    def test_print_cohort_insightsql_query_resolves_positional_references(
        ...
        # first pointing out of bounds, so Datastore rejects the query and the cohort never fills.
        cohort = Cohort.objects.create(
            team=self.team,
            name="Test Positional Cohort",
            query={"kind": "InsightsQLQuery", "query": query},
        )

        context = InsightsQLContext(team_id=self.team.id, enable_select_queries=True)

        sql = print_cohort_insightsql_query(cohort, context, team=self.team).lower()
    ...
    def test_print_cohort_insightsql_query_leaves_positional_references_on_wildcard_select(self):
        # `*` is a single node in the SELECT list until the resolver fans it out into one node per
        # column, and Datastore numbers positional arguments against that expanded list. Guessing
...
class TestRecalculationErrorRecovery(BaseTest):
    def test_original_error_surfaces_when_recovery_history_save_fails(self):
        ...
        with (
            ...
            patch(
                "products.cohorts.backend.models.util._recalculate_cohortpeople_for_team_insightsql",
