...
from insights.models.github_integration_base import INSTALLATION_UNAVAILABLE_SINCE_CONFIG_KEY
from insights.models.integration import ERROR_TOKEN_REFRESH_FAILED, Integration
from insights.models.scoping.root_mixin import TeamScopedRootMixin
from insights.models.team.team import Team
from insights.models.user import User
from insights.models.utils import DeletedMetaFields, UUIDModel
...
class Channel(TeamScopedRootMixin):
    """A shared feed of tasks (rendered as "#<name>" in Insights Desktop). Every task is
    ...
    # db_constraint=False on the team/user FKs: insights_team and insights_user are written on
    ...
    created_by = models.ForeignKey(
        "insights.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="+", db_constraint=False
    )
    ...
    class Meta:
        db_table = "insights_task_channel"
        constraints = [
...
class Task(DeletedMetaFields, models.Model):
    ...
    )  # Format is organization/repo, for example insights/insights-js
    repositories = ArrayField(
        models.CharField(max_length=255),
        default=list,
        db_default=[],
    ...
    archived = models.BooleanField(
        default=False,
        help_text=(
            "If true, the task is hidden from default list responses. Used by Insights Desktop clients "
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["channel", "-created_at"], name="insights_task_channel_feed_idx"),
            models.Index(fields=["loop"], name="insights_task_loop_idx"),
        ]
    ...
    def capture_event(
        ...
        # hanzo_insights.capture silently drops events in workers (see insights.ph_client).
        try:
            distinct_id = (
            ...
            (capture_fn or hanzo_insights.capture)(
    ...
    def create_and_run(
        *,
        ...
        repository: str | None = None,  # Format: "organization/repository", e.g. "insights/insights-js"
        channel: Channel | None = None,
        create_pr: bool = True,
        mode: str = "background",
        slack_thread_context: Optional["SlackThreadContext"] = None,
        ...
        insights_mcp_scopes: InsightsMcpScopes = "full",
        branch: str | None = None,
        signal_report_id: str | None = None,
        sandbox_environment_id: str | None = None,
        internal: bool = False,
        ...
        if start_workflow:
            ...
            run_extra_state["pending_dispatch"] = {
                "create_pr": create_pr,
                "insights_mcp_scopes": insights_mcp_scopes,
                "user_id": user_id,
...
class TaskSession(TeamScopedRootMixin, UUIDModel):
    ...
    class Meta:
        db_table = "insights_task_session"
        indexes = [
...
class TaskThreadMessage(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_thread_message"
        indexes = [
...
class TaskThreadMessageMention(TeamScopedRootMixin):
    ...
    mentioned_user = models.ForeignKey("insights.User", on_delete=models.CASCADE, related_name="+", db_constraint=False)
    created_at = models.DateTimeField(default=django_timezone.now)

    class Meta:
        db_table = "insights_task_thread_message_mention"
        constraints = [
...
class TaskActivity(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_activity"
...
class ChannelFeedMessage(TeamScopedRootMixin):
    ...
    task cards as a "Insights agent" system row (e.g. "Adam created this context").
    ...
    class Meta:
        db_table = "insights_task_channel_feed_message"
...
class ChannelInstructions(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_channel_instructions"
        constraints = [
            models.UniqueConstraint(
...
class ChannelContextGeneration(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_channel_context_generation"
...
class ChannelStar(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_channel_star"
        constraints = [
...
class TaskAutomation(models.Model):
    ...
    class Meta:
        db_table = "insights_task_automation"
...
class LoopTrigger(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_loop_trigger"
        indexes = [
...
class LoopFire(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_task_loop_fire"
        constraints = [
...
class TaskRun(models.Model):
    ...
    # Local url-based MCP servers imported from the creating client (Insights Desktop),
    ...
    def append_log(self, entries: list[dict], *, ttl_days: int | None = DEFAULT_LOG_TTL_DAYS):
        ...
        self._mirror_logs_to_insights_logs(entries)
    ...
    def _mirror_logs_to_insights_logs(self, entries: list[dict]) -> None:
        """Mirror persisted entries into the Insights Logs product via stdout (dogfooding).
        ...
        try:
            ...
            logger.warning(
                "task_run.mirror_logs_to_insights_logs_failed",
                task_run_id=str(self.id),
                error=str(e),
            )
    ...
    def emit_progress_event(
        ...
        Consumed by the desktop client as `_insights/progress`. Events sharing a
...
class TaskArtifact(TeamScopedRootMixin, UUIDModel):
    ...
    class Meta:
        db_table = "insights_task_artifact"
        indexes = [
...
class SandboxSession(TeamScopedRootMixin, UUIDModel):
    ...
    (``user_attributed_at`` NULL, on Insights's dime) until a user claims the run with
    ...
    class Meta:
        db_table = "insights_task_sandbox_session"
        indexes = [
...
class SandboxCustomImage(TeamScopedRootMixin):
    ...
    class Meta:
        db_table = "insights_sandbox_custom_image"
        indexes = [
    ...
    def modal_publish_name(self) -> str:
        ...
        return f"insights-sandbox-custom-{self.team_id}-{self.id.hex}:latest"


class CodeInvite(UUIDModel):
    """Invite codes for Insights Desktop access."""
    ...
    created_by = models.ForeignKey(
        "insights.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="created_code_invites"
    )
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        db_table = "insights_code_invite"
...
class CodeInviteRedemption(UUIDModel):
    """Tracks each redemption of a Insights Desktop invite."""

    invite_code = models.ForeignKey(CodeInvite, on_delete=models.CASCADE, related_name="redemptions")
    user = models.ForeignKey("insights.User", on_delete=models.CASCADE)
    organization = models.ForeignKey("insights.Organization", on_delete=models.SET_NULL, null=True, blank=True)
    redeemed_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        db_table = "insights_code_invite_redemption"
...
class TaskPresence(TeamScopedRootMixin):
    ...
    Created/refreshed by the desktop and mobile Insights Desktop clients while a
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, related_name="+")
    task = models.ForeignKey(Task, on_delete=models.CASCADE, related_name="+")
    user = models.ForeignKey("insights.User", on_delete=models.CASCADE, related_name="+")
    ...
    class Meta:
        db_table = "insights_task_presence"
        constraints = [
            models.UniqueConstraint(
