...
from products.data_warehouse.backend.datastore_helpers import (
    get_datastore_source_location,
    reconcile_datastore_schemas,
    reproject_direct_datastore_table,
)
...
class _DatastoreEngine(DirectQueryEngine):
    engine = "datastore"

    def source_table_location(self, *, schema_name, source_schema, default_schema, default_catalog=None):
        # Datastore has no catalog level; database occupies the "schema" slot, catalog stays None.
        database, table = get_datastore_source_location(
            schema_name=schema_name,
            schema_metadata=_location_metadata(source_schema),
    ...
    def columns_to_dwh_columns(self, source_columns):
        return datastore_columns_to_dwh_columns(source_columns)

    def upsert_table(
        self, existing_table, *, schema_name, source, columns, source_catalog, source_schema, source_table_name
    ):
        # source_schema carries the Datastore database; there is no catalog.
    ...
    def reproject_table(self, schema_row, *, source, enabled_columns):
        return reproject_direct_datastore_table(schema_row, source=source, enabled_columns=enabled_columns)

    def hide_table(self, table):
        hide_direct_datastore_table(table)

    def reconcile_schemas(self, *, source, source_schemas, team_id):
        return reconcile_datastore_schemas(source=source, source_schemas=source_schemas, team_id=team_id)

    # No refresh_name_substitutions override — like MySQL/Snowflake/Redshift, Datastore has no
...
_ENGINES: dict[str, DirectQueryEngine] = {
    ...
    for engine in (_PostgresEngine(), _MySQLEngine(), _SnowflakeEngine(), _RedshiftEngine(), _DatastoreEngine())
}
