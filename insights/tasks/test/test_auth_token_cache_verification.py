...
from insights.storage.auth_token_cache_verifier import AuthTokenVerificationResult
from insights.tasks.auth_token_cache_verification import LOCK_KEY, verify_and_fix_auth_token_cache_task
...
class TestVerifyAndFixAuthTokenCacheTask(PushGatewayTaskTestMixin, TestCase):
    @patch("insights.storage.auth_token_cache_verifier.verify_and_fix_auth_token_cache")
    @patch("insights.redis.get_client")
    ...
    @patch("insights.tasks.auth_token_cache_verification.capture_exception")
    @patch("insights.storage.auth_token_cache_verifier.verify_and_fix_auth_token_cache")
    @patch("insights.redis.get_client")
    ...
    def test_pushgateway_metrics_recorded(self, mock_get_client: MagicMock, mock_verify: MagicMock) -> None:
        ...
        success = self.registry.get_sample_value("insights_celery_verify_and_fix_auth_token_cache_task_success")
        duration = self.registry.get_sample_value(
            "insights_celery_verify_and_fix_auth_token_cache_task_duration_seconds"
        )
        assert success == 1
    ...
    @patch("insights.storage.auth_token_cache_verifier.verify_and_fix_auth_token_cache")
    @patch("insights.redis.get_client")
    @patch("insights.tasks.auth_token_cache_verification.django_cache")
