...
def get_survey_response(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    Process getSurveyResponse() function call and return InsightsQL AST.
...
def unique_survey_submissions_filter(node: ast.Call, args: list[ast.Expr], team_id: int | None) -> ast.Expr:
    """
    Process uniqueSurveySubmissionsFilter() function call and return InsightsQL AST.
    ...
    return parse_expr(insightsql, placeholders={"survey_id": ast.Constant(value=survey_id)}, start=None)
