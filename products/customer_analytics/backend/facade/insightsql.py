"""Customer analytics' federated InsightsQL system tables (`system.accounts`,
...
Owned here rather than in core so the coupling between core's InsightsQL schema and this
...
account_tagged_items: _AccountScopedPostgresTable = _AccountScopedPostgresTable(
    ...
    description="Internal federated junction table (PostgreSQL `insights_taggeditem`) of tag-to-account links; not for direct querying — use `system.accounts.tags`.",
    fields={
...
account_resource_notebooks: _AccountScopedPostgresTable = _AccountScopedPostgresTable(
    ...
    postgres_table_name="insights_resourcenotebook",
    description="Internal federated junction table (PostgreSQL `insights_resourcenotebook`) of notebook-to-account links; not for direct querying — use `system.accounts.notebooks`.",
    fields={
...
# The isNull guard keeps non-boolean rows falling through the coalesce (Datastore if()
...
def _account_custom_properties_select(fields_accessed: dict[str, list[str | int]]) -> ast.SelectQuery:
    ...
    for name, chain in fields_accessed.items():
        ...
        if len(chain) >= 2 and chain[0] == "values":
            ...
            select.append(
                # nosemgrep: insightsql-fstring-audit - only interpolates the module-level _COALESCED_VALUE constant (no user input)
                parse_expr(
...
def account_tags_join(join_to_add: LazyJoinToAdd, context: InsightsQLContext, node: ast.SelectQuery) -> ast.JoinExpr:
...
def account_notebooks_join(join_to_add: LazyJoinToAdd, context: InsightsQLContext, node: ast.SelectQuery) -> ast.JoinExpr:
    if not join_to_add.fields_accessed:
...
account_relationship_definitions: PostgresTable = PostgresTable(
    ...
    description="Customer analytics account relationship definitions: team-defined relationship types between Insights users and accounts (CSM, Account executive, ...), one row per definition. Per-account assignments live in system.account_relationships and via the system.accounts.relationships lazy join.",
    fields={
        ...
        "created_by_id": IntegerDatabaseField(
            name="created_by_id", nullable=True, description="Insights user who created the definition."
        ),
...
account_relationships: PostgresTable = PostgresTable(
    ...
    fields={
        ...
        "user_id": IntegerDatabaseField(name="user_id", nullable=True, description="Assigned Insights user id."),
        "created_by_id": IntegerDatabaseField(
            name="created_by_id", nullable=True, description="Insights user who made the assignment."
        ),
