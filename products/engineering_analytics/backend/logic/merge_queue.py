...
months of ``Insights/insights``: 51,052 gate-shaped runs, all with ``actor`` = ``trunk-io[bot]``, none
...
# keeps the pattern backslash-free, so it survives embedding in a InsightsQL string literal unescaped.
...
def looks_like_merge_queue_branch_expr(branch_column: str) -> str:
    """InsightsQL predicate on the branch **shape alone** — no identity corroboration.
...
def source_pr_string_expr(branch_column: str, *, queue_actor_column: str) -> str:
    """InsightsQL: the PR number a corroborated gate branch is landing, as a string; '' otherwise.
...
def source_pr_number_expr(branch_column: str, *, queue_actor_column: str) -> str:
    """InsightsQL: the PR number a corroborated gate branch is landing, 0 otherwise.
...
def merge_queue_branch_expr(branch_column: str, *, queue_actor_column: str) -> str:
    """InsightsQL predicate: is this a merge-queue gate branch, pushed by the queue?
