...
/**
 * * `web` - web
 * * `insights-ios` - insights-ios
 * * `insights-android` - insights-android
 * * `insights-java` - insights-java
 * * `insights-server` - insights-server
 * * `insights-node` - insights-node
 * * `insights-python` - insights-python
 * * `insights-php` - insights-php
 * * `insights-ruby` - insights-ruby
 * * `insights-go` - insights-go
 * * `insights-flutter` - insights-flutter
 * * `insights-react-native` - insights-react-native
 * * `insights-kmp` - insights-kmp
 * * `insights-dotnet` - insights-dotnet
 * * `insights-elixir` - insights-elixir
 */
export type LibEnumApi = (typeof LibEnumApi)[keyof typeof LibEnumApi]

export const LibEnumApi = {
    Web: 'web',
    InsightsIos: 'insights-ios',
    InsightsAndroid: 'insights-android',
    InsightsJava: 'insights-java',
    InsightsServer: 'insights-server',
    InsightsNode: 'insights-node',
    InsightsPython: 'insights-python',
    InsightsPhp: 'insights-php',
    InsightsRuby: 'insights-ruby',
    InsightsGo: 'insights-go',
    InsightsFlutter: 'insights-flutter',
    InsightsReactNative: 'insights-react-native',
    InsightsKmp: 'insights-kmp',
    InsightsDotnet: 'insights-dotnet',
    InsightsElixir: 'insights-elixir',
} as const
...
export interface SdkReleaseAssessmentApi {
    ...
    sql_query: string
    /** Relative URL path (starting with /project/{id}/) for the Activity > Explore page pre-filtered to events captured with this lib and lib_version over the last 7 days. Combine with the user's Insights host (e.g. us.hanzo.ai) for a clickable link. */
...
export interface SdkAssessmentApi {
    /** SDK identifier, e.g. 'web', 'insights-python', 'insights-node', 'insights-ios'.
     *
     * * `web` - web
     * * `insights-ios` - insights-ios
     * * `insights-android` - insights-android
...
export interface SdkHealthReportApi {
    ...
    /** Number of distinct Insights SDKs the project is actively using. */
...
export type SdkHealthReportRetrieveParams = {
    /**
     * When true, bypasses the Redis cache and re-queries Datastore for SDK usage. Use sparingly — data is refreshed every 12 hours by a background job.
     */
