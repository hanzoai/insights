...
from insights.temporal.ai.anomaly_investigation.report import salvage_report
from insights.temporal.ai.anomaly_investigation.runner import (
...
def test_build_callbacks_tags_ai_product_for_llm_analytics(alert_id, expected_properties) -> None:
    ...
    with (
        patch("insights.temporal.ai.anomaly_investigation.runner.hanzo_insights") as mock_module,
        patch("insights.temporal.ai.anomaly_investigation.runner.CallbackHandler") as mock_handler,
    ):
...
def test_build_callbacks_skips_when_default_client_missing() -> None:
    ...
    with patch("insights.temporal.ai.anomaly_investigation.runner.hanzo_insights") as mock_module:
