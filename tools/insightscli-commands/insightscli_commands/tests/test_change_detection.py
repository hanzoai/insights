...
class TestChangedFiles:
    @patch("insightscli_commands.change_detection.subprocess.run")
    ...
    def test_paths_with_spaces_stay_unquoted(self, mock_run: MagicMock) -> None:
        mock_run.side_effect = [
            _completed(stdout="insights/migrations/0067_table copy.py\0"),
            _completed(stdout=" M docs/a b.md\0"),
        ]
        assert changed_files() == ["docs/a b.md", "insights/migrations/0067_table copy.py"]
...
class TestMatchesGlobs:
    @pytest.mark.parametrize(
        "path,globs,expected",
        [
            ("insights/api/insight.py", ["insights/api/*"], True),
            ("insights/api/dashboards/dashboard.py", ["insights/api/*"], True),
            ("insights/models/team.py", ["insights/api/*"], False),
            ("tools/a/b.py", ["*.py"], True),
            ("notes.md", ["*.py"], False),
            ("insights/migrations/0001_x.py", ["*/migrations/*.py"], True),
