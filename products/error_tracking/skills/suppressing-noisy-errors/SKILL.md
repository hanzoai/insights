...
description: >
  Create Insights error tracking suppression rules to drop high-volume,
...
| `insights:error-tracking-suppression-rules-list`   | Check existing suppression rules                                                                     |
| `insights:error-tracking-suppression-rules-create` | Create the suppression rule                                                                          |
| `insights:error-tracking-issues-partial-update`    | Hide past data via issue status without dropping events at ingestion                                 |
...
Insights's property filters special-case them — each filter matches against
...
the ScriptVM `Operation::Regex`, which is **case-sensitive**. Use the `(?i)`
...
filters directly — no InsightsQL translation needed:
...
fall back to the InsightsQL aggregate at the end of this step.
...
`filterGroup` is **flat AND only**. Drop into InsightsQL when:
...
The InsightsQL shape mirrors what the suppression rule bytecode compiles to.
The materialized property column is nullable, so the `coalesce(..., '[]')`
wrapper is required — without it Datastore rejects the query with
...
3. Whether it overlaps any existing suppression rules
   (`insights:error-tracking-suppression-rules-list` first)

Wait for explicit confirmation. Then create:

```json
insights:error-tracking-suppression-rules-create
{
  "filters": {
    "type": "AND",
    "values": [
