...
| `python/` | `deltalite-python`: thin pyo3 binding, built by maturin, imported as `deltalite`. Workspace member shaped like `rust/insightsql/parser` (cdylib+rlib, `extension-module`, abi3-py312) — no Rust tests, see below. |
...
Linux**. `insightsql/parser` sidesteps this by having no Rust tests; deltalite needs
...
pyo3 0.28 — so `rust/insightsql/parser` was migrated from pyo3 0.22 to 0.28 to share
...
Mirrors `insightsql-parser-rs` (see `.github/workflows/build-insightsql-parser-rs.yml`):
...
- When a release is cut, a `build-deltalite` workflow (to be cloned from
  `build-insightsql-parser-rs.yml` at rollout time: version-bump detection on
  ...
  pinning `deltalite==<version>` in `pyproject.toml`) publishes it; the insights
