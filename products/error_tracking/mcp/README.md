...
`error_tracking_alerts.yaml` is a subset file (not synced by `scaffold-yaml --sync-all` beyond validation) that wraps the generic `insights_functions_*` operations as error-tracking alert tools.
