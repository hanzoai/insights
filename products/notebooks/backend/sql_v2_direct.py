...
def _wrap_page_query(query: str, limit: int, offset: int) -> str:
    ...
    FROM; InsightsQL accepts the alias and ignores it. The inner query is either validated InsightsQL
    ...
    return f"select * from ({query}\n) as insights_notebook_page limit {int(limit)} offset {int(offset)}"
...
def apply_raw_page_bounds(query: str, limit: int, offset: int) -> str:
    """Bound a raw (engine-dialect) query, which the InsightsQL parser can't read.
...
def enqueue_direct_run(team: "Team", user: "User | None", run: NotebookNodeRun) -> None:
    ...
    query_json: dict[str, Any] = {"kind": "InsightsQLQuery", "query": bounded}
