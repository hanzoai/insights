...
The actual deletion timestamp lives in the per-flag activity log, reachable only via `insights:feature-flags-activity-retrieve` (one call per flag id). There is no bulk activity endpoint.
...
For each candidate id, call `insights:feature-flags-activity-retrieve` with `limit: 5, page: 1`. **Issue all calls in one message so they run concurrently** — sequential calls are dramatically slower.
...
- `insights:execute-sql`: Used in step 2 to enumerate soft-deleted candidates against `system.feature_flags`
- `insights:feature-flags-activity-retrieve`: Used in step 3 to find the actual deletion event for each candidate
- `insights:feature-flag-get-definition`: Useful if the user then wants to inspect what the deleted flag looked like
