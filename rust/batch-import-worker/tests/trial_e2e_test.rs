...
const TEST_BUCKET: &str = "insights";
const TEAM_ID: i32 = 1;
...
impl TrialRun {
    /// Set up config, context, source dir, and a leased `insights_batchimport`
    ...
    async fn create(record_limit: u64) -> Option<Self> {
        ...
        sqlx::query(
            r#"
            INSERT INTO insights_batchimport
    ...
    async fn db_status_and_state(&self) -> (String, Value) {
        let row: (String, Value) =
            sqlx::query_as("SELECT status, state FROM insights_batchimport WHERE id = $1")
                .bind(self.job_id)
    ...
    async fn cleanup(&self) {
        sqlx::query("DELETE FROM insights_batchimport WHERE id = $1")
            .bind(self.job_id)
