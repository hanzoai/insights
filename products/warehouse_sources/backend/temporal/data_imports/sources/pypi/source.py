...
class PyPISource(SimpleSource[PyPISourceConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Pull metadata for Python packages from the [PyPI](https://pypi.org) JSON API into the Insights Data warehouse.
...
Each sync fetches the current metadata for every configured package. PyPI has no server-side "changed since" filter, so all tables sync as a full refresh.""",
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/pypi",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
