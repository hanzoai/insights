...
//! `personinsights_traffic_chaos_actions_total` with a scenario/target/mode
...
//! holds pod get/list/delete in exactly the personinsights namespaces, so no
...
pub async fn run(cfg: ChaosConfig, shutdown: Arc<AtomicBool>) {
    let client = match kube::Client::try_default().await {
        ...
        Err(e) => {
            ...
            counter!("personinsights_traffic_chaos_skipped_total", "reason" => "kube_client")
                .increment(1);
            ...
            gauge!("personinsights_traffic_chaos_enabled").set(0.0);
            return;
        }
    };
    ...
    let store = match &cfg.etcd {
        Some((endpoints, prefix)) => match connect_store(endpoints, prefix).await {
            ...
            Err(e) => {
                ...
                counter!("personinsights_traffic_chaos_skipped_total", "reason" => "etcd_unreachable")
                    .increment(1);
                None
            }
        },
        ...
        Scenario::EtcdBounce => {
            ...
            let ready = match list_ready(client, target).await {
                ...
                Err(e) => {
                    ...
                    counter!("personinsights_traffic_chaos_skipped_total", "reason" => "list_failed")
                        .increment(1);
                    return;
                }
            };
            if ready.len() < 3 {
                counter!("personinsights_traffic_chaos_skipped_total", "reason" => "etcd_quorum_guard")
                    .increment(1);
                return;
            }
        ...
        Scenario::CoordinatorDoubleTap => {
            ...
            match successor {
                ...
                None => {
                    ...
                    counter!("personinsights_traffic_chaos_skipped_total", "reason" => "no_successor")
                        .increment(1);
                }
            }
        }
...
async fn kill_coordinator(
    ...
    let Some(holder) = resolve_coordinator(store).await else {
        ...
        counter!("personinsights_traffic_chaos_skipped_total", "reason" => "no_coordinator")
            .increment(1);
        return None;
    };
    let router = cfg.targets.iter().find(|t| t.kind == TargetKind::Router)?;
    ...
    if ready.is_empty() {
        ...
        counter!("personinsights_traffic_chaos_skipped_total", "reason" => "no_targets").increment(1);
        return None;
    }
    let alive = ready.iter().filter(|n| !killed.contains(*n)).count();
    if alive < 2 {
        tracing::info!(alive, "min-alive guard: skipping coordinator kill");
        counter!("personinsights_traffic_chaos_skipped_total", "reason" => "min_alive").increment(1);
        return None;
    }
...
async fn kill_named(
    ...
    match pods.delete(pod_name, &params).await {
        Ok(_) => {
            ...
            counter!(
                "personinsights_traffic_chaos_actions_total",
                "scenario" => scenario.label(),
                "target" => target.kind.label(),
                "mode" => mode.label(),
            )
            ...
            let now = SystemTime::now()
                ...
                .unwrap_or_default()
                .as_secs_f64();
            gauge!("personinsights_traffic_chaos_last_action_timestamp_seconds").set(now);
            true
        }
        Err(e) => {
            tracing::warn!(error = %e, pod = pod_name, "chaos delete failed");
            counter!("personinsights_traffic_chaos_skipped_total", "reason" => "delete_failed")
                .increment(1);
            false
        }
    }
