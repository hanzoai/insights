...
description: >
  ...
  Insights, model them in InsightsQL (mirroring query-retention / query-stickiness / query-lifecycle); in dbt,
...
**Insights:** InsightsQL recipes mirroring the built-in insights, so the model reuses the same logic in SQL and
downstream views:
[`references/insights/retention_matrix.sql`](references/insights/retention_matrix.sql),
[`stickiness.sql`](references/insights/stickiness.sql),
[`lifecycle.sql`](references/insights/lifecycle.sql). For quick interactive analysis prefer the native
...
`querying-insights-data` (interactive analysis + InsightsQL), `modeling-activation-metrics` (uses retention lift),
