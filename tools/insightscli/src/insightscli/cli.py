"""insightscli - Developer CLI framework with YAML-based command definitions.

All commands are defined in insightscli.yaml and auto-discovered.
...
from insightscli.validate import auto_update_manifest, find_missing_manifest_entries, find_orphan_manifest_entries

_DEFAULT_HELP = "Developer CLI framework with YAML-based command definitions."

# Sentinel inherited by subprocesses so nested insightscli invocations (composite
# steps, prechecks, process managers re-running insightscli) can be told apart in
# telemetry. is_nested means "descends from a insightscli process tree" -- a human
...
class CategorizedGroup(click.Group):
    ...
    def invoke(self, ctx: click.Context) -> Any:
        ctx.meta["insightscli.start_time"] = _time.monotonic()
        ctx.meta["insightscli.has_extra_argv"] = len(sys.argv) > 2
        exit_code = 0
        try:
...
def cli(ctx: click.Context) -> None:
    """insightscli - YAML-driven developer CLI."""
    ...
    # Fire early so long-running commands (e.g. insightscli start) are counted even
    ...
    ctx.meta["insightscli.telemetry_active"] = _telemetry_active(ctx.invoked_subcommand)
    if ctx.meta["insightscli.telemetry_active"]:
        try:
            telemetry.track(
...
def telemetry_on() -> None:
    ...
    click.echo("Telemetry enabled. Thank you for helping improve insightscli!")
...
# `insightscli run`). Manifest commands opt in via `needs_secrets: true`.
_BUILTIN_COMMANDS_NEEDING_SECRETS = frozenset({"run"})

# False until `main()` flips it (the only path the `insightscli` script and
# `python -m insightscli` both take). The secret-wrap re-exec is gated on it; see
...
def _apply_env_config(invoked_subcommand: str | None = None) -> None:
    """Apply ``config.env`` from insightscli.yaml: load files, optionally re-exec via wrap.
    ...
    try:
        ...
        click.echo(f"⚠️  Invalid config.env in insightscli.yaml: {e}", err=True)
        return
...
def _maybe_reexec_via_wrap(secrets: dict[str, Any], env_files: list[Path]) -> None:
    """Re-exec insightscli under the configured wrap command if the secrets file warrants it.
    ...
    Only ever re-execs when insightscli owns the process (``_is_process_entrypoint``).
    ...
    os.execvp(resolved_wrap[0], [*resolved_wrap, sys.executable, "-m", "insightscli", *sys.argv[1:]])
...
def run_with_env(command: tuple[str, ...]) -> None:
    """Run a command with the env loaded from ``config.env`` in insightscli.yaml.
    ...
    Examples:
        insightscli run ./manage.py shell
        insightscli run pytest insights/api/
    """
    os.execvp(command[0], list(command))


def _register_script_commands() -> None:
    """Dynamically register commands from insightscli.yaml.

    Supports four eagerly registered entry types:
    1. ``steps:`` — compose multiple insightscli commands in sequence.
    2. ``cmd:`` — execute a direct shell command.
    3. ``insightscli:`` — wrap another insightscli command with extra args.
    ...
    for _category, scripts in manifest.data.items():
        ...
        for cli_name, config in scripts.items():
            ...
            cmd = config.get("cmd")
            insightscli = config.get("insightscli")
...
def _load_boot_modules() -> None:
    ...
    post-command hooks against ``insightscli.hooks``. They run eagerly so the hooks
    ...
    import failure aborts insightscli, so these are validated implicitly by every
...
@functools.cache
def _insightscli_version() -> str | None:
    try:
        return importlib.metadata.version("insightscli")
...
def _env_properties(command: str | None = None) -> dict[str, Any]:
    ...
    props: dict[str, Any] = {
        ...
        "insightscli_version": _insightscli_version(),
...
def _should_track(command: str | None) -> bool:
    ...
    Requires a real subcommand (so bare ``insightscli`` / ``--help`` don't fire a
...
def _fire_telemetry(ctx: click.Context, exit_code: int) -> None:
    ...
    if not ctx.meta.get("insightscli.telemetry_active", False):
        return
    try:
        start_time: float = ctx.meta.get("insightscli.start_time", 0.0)
        ...
        props: dict[str, Any] = {
            ...
            "has_extra_argv": ctx.meta.get("insightscli.has_extra_argv", False),
            **_env_properties(command),
        }
        # Merge devenv-specific properties (set by dev:generate)
        devenv = ctx.meta.get("insightscli.devenv")
        if devenv:
            props.update(devenv)
