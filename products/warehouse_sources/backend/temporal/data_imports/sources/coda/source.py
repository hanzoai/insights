...
class CodaSource(SimpleSource[CodaSourceConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            label="Coda",
            caption="""Enter your Coda API token to pull your docs, tables, and rows into the Insights Data warehouse.
...
You can generate an API token in [Coda account settings](https://coda.io/account). The token only sees docs its creator can access. Rows are synced from every table of every doc — note Coda's doc-listing rate limit makes large workspaces slow to sync.""",
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/coda",
    ...
    def validate_credentials(
        self, config: CodaSourceConfig, team_id: int, schema_name: Optional[str] = None, api_version: str | None = None
    ) -> tuple[bool, str | None]:
