...
This script:
  1. Finds every dagster source file (insights/dags/**, products/*/dags/**)
  2. Extracts imports via AST
  3. Resolves each internal (insights / ee / products / common) import to a
     concrete file path
...
INTERNAL_PREFIXES = ("insights.", "ee.", "products.", "common.")


def find_dag_files() -> list[Path]:
    files = list((REPO_ROOT / "insights" / "dags").rglob("*.py"))
...
def resolve_module(module: str) -> Path | None:
    ...
    Walks up the hierarchy so `from insights.utils import foo` resolves to
    insights/utils.py (the attribute `foo` isn't a submodule). Namespace
    packages (directories with no __init__.py, e.g. insights/insightsql/) resolve
