...
import { wrapWithInsightsContext } from '../../utils/insightsContextBlock'
...
const LAST_REPOSITORY_CONFIG_STORAGE_KEY = 'insights_ai.tasks.lastRepositoryConfig'
...
export const taskTrackerSceneLogic = kea<taskTrackerSceneLogicType>([
    path(['products', 'insights_ai', 'frontend', 'scenes', 'TaskTracker', 'taskTrackerSceneLogic']),
    ...
    listeners(({ actions, values, cache, props }) => ({
        ...
        submitNewTask: async () => {
            ...
            try {
                const taskData: TaskWriteApi = {
                    title: '',
                    description,
                    origin_product: OriginProductEnumApi.InsightsAi,
                    // Insights AI can run without a repo; null means the task is not scoped to any repository.
                ...
                const runResponse = await tasksRunCreate(projectId, newTask.id, {
                    ...
                    pending_user_message: wrapWithInsightsContext(description, seededContext),
                })
        ...
        openExistingTask: ({ task }) => {
            ...
            // Never-ran task (rare for this panel's insights_ai origin) — fall back to the full detail page.
