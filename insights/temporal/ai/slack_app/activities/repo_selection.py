...
def cascade_insights_code_repository_activity(
    ...
    if user_id is None:
        logger.warning(
            "insights_code_cascade_legacy_call",
            integration_id=inputs.integration_id,
            slack_team_id=inputs.slack_team_id,
        )
        return InsightsCodeRepoCascadeOutcome(mode="no_repo", repository=None, reason="legacy_no_user_id")
    ...
    if not all_repos:
        ...
        if team_has_github and not is_slack_app_bot_prs_enabled(integration.team):
            return InsightsCodeRepoCascadeOutcome(mode="needs_user_github", repository=None, reason="no_user_repos")
        return InsightsCodeRepoCascadeOutcome(mode="no_repo", repository=None, reason="no_repos")

    if len(all_repos) == 1:
        return InsightsCodeRepoCascadeOutcome(mode="auto", repository=all_repos[0], reason="single_repo")

    explicit_repo = _extract_explicit_repo(event_text, all_repos)
    if explicit_repo:
        return InsightsCodeRepoCascadeOutcome(mode="auto", repository=explicit_repo, reason="explicit_mention")

    return InsightsCodeRepoCascadeOutcome(mode="agent_needed", repository=None, reason="needs_agent")


@activity.defn
@close_db_connections
async def discover_insights_code_repository_via_agent_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs,
    channel: str,
    event: dict[str, Any],
    ...
    if user_message_ts:
        try:
            ...
            logger.warning("insights_code_search_reaction_failed", channel=channel)
    ...
    try:
        ...
        logger.warning(
            "insights_code_repo_selection_rejected",
            channel=channel,
        ...
        logger.warning(
            "insights_code_repo_selection_unavailable",
            channel=channel,
            reason=exc.reason,
        )
        return SlackRepoSelectionOutcome(
        ...
        logger.exception(
            "insights_code_repo_selection_failed",
            channel=channel,
            error=str(exc),
        )
        return SlackRepoSelectionOutcome(
