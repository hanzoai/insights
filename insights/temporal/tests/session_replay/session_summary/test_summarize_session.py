...
from insights.temporal.session_replay.session_summary.activities.event_based.fetch_session_data import (
...
from insights.temporal.session_replay.session_summary.types.inputs import SingleSessionProgress
...
class TestExecuteSummarizeSessionVideoStream:
    ...
    def cap_mocks(self) -> Iterator[SimpleNamespace]:
        ...
        with (
            patch(
                "insights.temporal.session_replay.session_summary.workflow._workflow_is_running",
                workflow_is_running,
            ),
            patch(
                "insights.temporal.session_replay.session_summary.workflow.atomic_check_and_consume",
                atomic,
            ),
            patch(
                "insights.temporal.session_replay.session_summary.workflow.refund",
                refund,
            ),
    ...
    async def test_fast_path_returns_cached_summary_without_starting_workflow(
        ...
        with (
            ...
            patch("insights.temporal.session_replay.session_summary.workflow._prepare_execution") as mock_prepare,
            patch(
                "insights.temporal.session_replay.session_summary.workflow._start_video_summary_workflow"
            ) as mock_start,
            patch("insights.temporal.session_replay.session_summary.workflow.async_connect") as mock_connect,
        ):
    ...
    async def test_polling_loop_yields_progress_then_final_summary(
        ...
        with (
            ...
            patch(
                "insights.temporal.session_replay.session_summary.workflow._prepare_execution",
            ...
            patch(
                "insights.temporal.session_replay.session_summary.workflow.async_connect",
                AsyncMock(return_value=MagicMock()),
            ),
            patch(
                "insights.temporal.session_replay.session_summary.workflow.asyncio.sleep",
                AsyncMock(),
            ),
        ):
            events = await self._collect(
    ...
    async def test_rendering_video_phase_queries_rasterizer_child_workflow(
        ...
        with (
            ...
            patch(
                "insights.temporal.session_replay.session_summary.workflow._get_rasterizer_frame_progress",
    ...
    async def test_length_precheck_fails_open_when_metadata_read_errors(
        ...
        cap_mocks.get_metadata.side_effect = Exception("datastore unavailable")
        completed_summary = MagicMock()
        completed_summary.id = "completed-summary-id"
        completed_summary.summary = mock_enriched_llm_json_response
        handle = self._make_handle([(WorkflowExecutionStatus.COMPLETED, None)])
    ...
    async def test_quota_blocked_emits_error_and_skips_workflow_start(
        ...
        with (
            ...
            patch("insights.temporal.session_replay.session_summary.workflow.hanzo_insights.capture") as mock_capture,
        ):
            events = await self._collect(
                execute_summarize_session_video_stream(
                    session_id=mock_session_id,
...
class TestVideoStreamCapEndToEnd:
    ...
    def _stub_metadata(self) -> Iterator[None]:
        # In-range metadata so the up-front length precheck is a no-op (avoids a real Datastore read).
        with patch.object(
    ...
    def _isolate_redis_key(self, mock_team: MagicMock) -> Iterator[None]:
        ...
        from insights.session_recordings.ai_summary_cap import _redis_key
    ...
    async def test_cap_blocks_after_quota_exhausted_under_load(
        ...
        from insights.session_recordings.ai_summary_cap import current_usage
        ...
        with (
            ...
            patch(
                "insights.session_recordings.ai_summary_cap.get_cap_for_team",
                return_value=cap,
            ),
            patch.object(
                SingleSessionSummary.objects,
...
class TestWorkflowIsRunning:
    ...
    async def test_returns_true_when_status_is_running(self) -> None:
        from insights.temporal.session_replay.session_summary.workflow import _workflow_is_running
...
class TestExecuteSummarizeSession:
    ...
    async def test_returns_existing_summary_without_starting_workflow(
        ...
        with (
            ...
            patch(
                "insights.temporal.session_replay.session_summary.workflow._execute_single_session_summary_workflow"
            ) as mock_execute,
        ):
