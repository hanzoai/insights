...
def _parse_point_time(value: str) -> dt.datetime:
    """Bucket times come back from InsightsQL rendered in the *project* timezone,
...
def _discover_candidate_keys(
    ...
    response = execute_insightsql_query(query_type="MetricQuery", query=query, team=team, workload=Workload.LOGS)
    keys: list[str] = []
