...
The observation's `_insightsUrl` is its recording; append `?t=<seconds>` (`timestamp_ms` / 1000, rounded down) to seek there.

```text
https://us.hanzo.ai/project/<project_id>/replay/<session_id>?t=1420
```
...
- **Make it trackable.** When a finding is corroborated across several sessions (not one low-confidence
  ...
  evidence, and add an `annotation` if it marks a regression. There is **no MCP tool to open a Insights
