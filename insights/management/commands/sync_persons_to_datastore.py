"""Reconcile a team's person, distinct-id, and group rows from Postgres to Datastore.

LOCAL/DEV USE ONLY for now: this loads the whole team's rows into memory (no batching), so it
is not safe to run against large production teams. It exists to seed local Datastore from the
...
from insights.models.person.util import _delete_ch_distinct_id, create_person, create_person_distinct_id
...
def run_person_sync(team_id: int, live_run: bool, deletes: bool):
    ...
    with persons_db_connection(writer=False) as conn, conn.cursor() as cursor:
        cursor.execute(
            "SELECT uuid, version, properties, is_identified, created_at FROM insights_person WHERE team_id = %s",
            [team_id],
        )
        persons = [
            {
...
def run_distinct_id_sync(team_id: int, live_run: bool, deletes: bool):
    ...
    with persons_db_connection(writer=False) as conn, conn.cursor() as cursor:
        cursor.execute(
            ...
            "FROM insights_persondistinctid pdi JOIN insights_person p ON p.id = pdi.person_id "
            "WHERE pdi.team_id = %s",
            [team_id],
        )
        person_distinct_ids = [
...
def run_group_sync(team_id: int, live_run: bool):
    ...
    with persons_db_connection(writer=False) as conn, conn.cursor() as cursor:
        cursor.execute(
            "SELECT group_type_index, group_key, group_properties, created_at FROM insights_group WHERE team_id = %s",
            [team_id],
        )
        pg_groups = [
            {
