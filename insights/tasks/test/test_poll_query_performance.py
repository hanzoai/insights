...
import insights.tasks.tasks
from insights.redis import get_client
from insights.tasks.poll_query_performance import poll_query_performance, query_manager_from_initial_query_id
from insights.tasks.tasks import Polling
...
class TestPollQueryPerformance(SimpleTestCase):
    ...
    @patch("insights.tasks.poll_query_performance.QueryStatusManager")
    @patch("insights.tasks.poll_query_performance.sync_execute")
...
class TestPollQueryPerformanceTask(SimpleTestCase):
    @patch("insights.tasks.tasks.logger.error")
    def test_poll_query_performance_does_not_run_if_last_update_does_not_match(
        ...
        insights.tasks.tasks.poll_query_performance("DIFFERENT TIME")
        mock_logger_error.assert_called_once_with("Poll query performance task terminating: another poller is running")

    # Mock the inner Datastore-querying function: these tests exercise the task wrapper's
    # timing/scheduling logic, not the query. Without this the wrapper runs a real query against a
    # Datastore test database that this SimpleTestCase never sets up, so the call raises, gets
    ...
    @patch("insights.tasks.poll_query_performance.poll_query_performance")
    @patch("insights.tasks.tasks.logger.error")
    @patch("insights.tasks.tasks.poll_query_performance.apply_async")
    def test_poll_query_performance_runs_and_restarts_itself_with_delay(
        ...
        insights.tasks.tasks.poll_query_performance(key)
    ...
    @patch("insights.tasks.poll_query_performance.poll_query_performance")
    @patch("insights.tasks.tasks.logger.error")
    @patch("insights.tasks.tasks.poll_query_performance.delay")
    ...
    @patch("insights.tasks.poll_query_performance.poll_query_performance", MagicMock())
    ...
    def test_start_poll_query_performance_does_nothing_for_14_seconds(
        ...
        for _ in range(3):
            insights.tasks.tasks.start_poll_query_performance()
