    ...
    const traceIndicator =
        value.targetType === 'trace_id' && value.targetId ? (
            <Tooltip title={<div className="font-mono text-xs">{value.targetId}</div>}>
                <Link to={urls.aiObservabilityTrace(value.targetId)} className="text-muted">
    ...
    if (status !== 'ok') {
        return (
            <MetricCellWrapper
                ...
                children={
                    <span>
                        <Tag type={getStatusType(status)}>{getStatusLabel(status)}</Tag>
                    </span>
                }
            />
        )
    ...
    if (value.resultType === 'binary') {
        ...
        return (
            <MetricCellWrapper
                ...
                children={
                    <span>
                        <Tag type={booleanTagType}>{booleanLabel}</Tag>
                    </span>
                }
            />
        )
        ...
        return (
            <div className="flex flex-col">
                <Link
                    to={
                        item.datasetItemId
                            ? urls.aiObservabilityDataset(item.datasetId, { item: item.datasetItemId })
                            : urls.aiObservabilityDataset(item.datasetId)
                    }
...
): TableColumns<OfflineExperiment> {
    return [
        {
            title: 'Experiment',
            key: 'experimentId',
...
function itemTableColumns(metricColumns: OfflineMetricColumn[]): TableColumns<OfflineExperimentItem> {
    const baseColumns: TableColumns<OfflineExperimentItem> = [
        {
            title: 'Item',
            key: 'itemId',
    ...
    const dynamicMetricColumns: TableColumns<OfflineExperimentItem> = metricColumns.map((metricColumn) => ({
        title: (
    ...
    const lastSeenColumn: TableColumns<OfflineExperimentItem> = [
        {
            title: 'Last seen',
            key: 'lastSeenAt',
