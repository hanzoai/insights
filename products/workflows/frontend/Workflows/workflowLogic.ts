...
export const workflowLogic = kea<workflowLogicType>([
                ...
                loadWorkflow: async () => {
                    if (!props.id || props.id === 'new') {
                        if (props.editTemplateId) {
                            ...
                            const templateWorkflow = await api.hogFlowTemplates.getInsightsFlowTemplate(
    ...
    selectors({
        ...
        actionValidationErrorsById: [
            (s) => [
                s.workflow,
                s.insightsFunctionTemplatesById,
                s.insightsFunctionTemplatesByIdLoading,
                s.scheduleStartsAt,
            ],
            (
                workflow: InsightsFlow,
                ...
                if (hasScheduleChanges) {
                    try {
                        if (pendingSchedule === null && existingScheduleId) {
                            ...
                            await api.hogFlows.updateInsightsFlowSchedule(
                                workflowId,
                                existingScheduleId,
                                pendingSchedule
                            )
