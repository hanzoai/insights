...
description: Write Streamlit app source code that runs well in a Insights sandbox — the insights_apps.query() bridge for reading Insights data, the packages baked into the sandbox image, caching and session state across Streamlit reruns, layout and chart patterns, and single-file app.py structure. Use when authoring or debugging the Python source of a Insights Streamlit app, when a query inside an app fails, or when asked to "write a streamlit app that shows Insights data".
---

# Writing Streamlit apps for the Insights sandbox
...
df = insights_apps.query("SELECT event, count() FROM events GROUP BY event LIMIT 10")
```

- Takes a InsightsQL string, returns a **pandas DataFrame**.
...
- `import insights` does NOT exist in the sandbox — the module is `insights_apps`, deliberately distinct from the insights-python SDK's name.
- The bridge is pre-authenticated to the app's project; user code never sees a token, and there is nothing to configure.
- Queries run with server-side caps (30 s execution, 256 MB memory) that don't scale with sandbox sizing — so bound time ranges, `LIMIT` results, and aggregate in InsightsQL rather than pulling raw events into pandas.
...
1. **Cache every bridge call** — uncached, one slider drag re-fires every query:
   ...
   def run_query(insightsql: str) -> pd.DataFrame:
       return insights_apps.query(insightsql)
   ```
...
Widgets drive parameters naturally, but **never interpolate a free-text widget value into InsightsQL**.
...
- Your code runs as an unprivileged user; there's no insights SDK, no way to pass your own environment variables or secrets to the app, and no expectation of general network egress. Don't read from `os.environ` — anything there belongs to the sandbox runtime, not your app. Design around `insights_apps.query()` as the data source.
