...
class TestEndpointOpenAPISpec(DatastoreTestMixin, APIBaseTest):
    ...
    def test_openapi_spec_marks_insightsql_variables_without_default_as_required(self):
    ...
    def test_openapi_spec_keeps_request_body_optional_when_all_variables_have_defaults(self):
        """When every InsightsQL variable has a default value, nothing is required."""
        from products.product_analytics.backend.models.insight_variable import InsightVariable
...
class TestEndpointOpenAPIRequiredVariables(DatastoreTestMixin, APIBaseTest):
    ...
    def test_insightsql_inline_does_not_mark_variables_required(self):
        """Inline InsightsQL substitutes defaults/NULLs for missing variables — that's a coherent
        contract. The OpenAPI spec must not mark inline InsightsQL variables required, or generated
        ...
        create_endpoint_with_version(
            name="insightsql_inline_no_required",
            team=self.team,
            query={
                "kind": "InsightsQLQuery",
        ...
        schema = self._get("insightsql_inline_no_required")
        self.assertIn("event_name", schema["properties"])
        self.assertNotIn("required", schema)

    def test_insightsql_materialized_marks_all_variables_required(self):
        """Materialized InsightsQL rejects ANY missing variable at run time, defaults included, so the
    ...
    def test_deprecation_warning_mentions_marking_breakdown_optional(self):
        """The X-Insights-Warn deprecation message must mention the optional opt-in (not just
        ...
        warn = response.headers.get("X-Insights-Warn", "")
