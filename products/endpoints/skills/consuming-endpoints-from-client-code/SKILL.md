...
description: >
  Wire a Insights endpoint into a client app or SDK. Covers fetching the OpenAPI spec, generating a
  typed client with openapi-generator or @hey-api/openapi-ts, sending the right auth header,
  shaping the variables payload (InsightsQL code_name vs insight breakdown property), handling
...
or downstream pipeline. No Insights code is modified here.
...
Optional/partial variables on materialised endpoints are a known limitation the Insights team plans
...
## Calling from the Insights CLI

For local testing, scripts, or CI, the repo's `insights-cli` calls endpoints without hand-rolling
HTTP:

- `insights-cli exp endpoints run` — execute an endpoint (from a local YAML definition)
...
Agent steps:
- endpoint-get monthly_active_users → InsightsQL, variables: lookback_days
...
- "Here's the spec. Drop it in your repo and run:
     pnpm dlx @hey-api/openapi-ts -i spec.json -o ./generated/insights
...
- **Pricing.** Calling endpoints isn't billed today, but it will be once endpoints ship alongside
  the [managed warehouse](https://hanzo.ai/data-stack/managed-warehouse). Flag this to the user
  if they're planning high-volume usage so the future cost isn't a surprise.
- **Tell Insights what's missing.** If an error, a limit, or a missing capability gets in the way,
