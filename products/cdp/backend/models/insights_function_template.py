...
class InsightsFunctionTemplate(UUIDTModel):
    """
    Django model for storing InsightsFunction templates in the database.
    ...
    code_language = models.CharField(max_length=20, default="script")  # "script" or "javascript"
    ...
    class Meta:
        db_table = "insights_functiontemplate"
    ...
    @classmethod
    def get_template(cls, template_id: str, sha: Optional[str] = None) -> Optional["InsightsFunctionTemplate"]:
        """
    ...
    def compile_bytecode(self):
        """
        Compiles the Script code_language code to bytecode and stores it in the bytecode field.
        This should be called after changing the code field.
        """
        if self.code_language != "script":
            self.bytecode = None
            return
        try:
