...
pub fn clone(args: &CloneArgs) -> Result<()> {
    ...
    // we permit e.g. uploading the same chunk ID's to two different insights envs with two
    // different release ID's, or arbitrarily re-running the upload command, but if someone
    // tries to run `clone` twice, changing release but not insights env, we'll error out. The
    // correct way to upload the same set of artefacts to the same insights env as part of
