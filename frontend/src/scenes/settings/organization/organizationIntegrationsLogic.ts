import { MakeLogicType, actions, afterMount, kea, listeners, path, selectors } from 'kea'
import { loaders } from 'kea-loaders'

import { Dialog, toast } from '@hanzo/elements'

import api from 'lib/api'
import { ICONS } from 'lib/integrations/utils'

import { IntegrationKind, IntegrationType } from '~/types'

import type { UserBasicType } from '../../../types'

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface organizationIntegrationsLogicValues {
    getOrganizationIntegrationsByKind: (
        kinds: (
            | 'apns'
            | 'aws-s3'
            | 'azure-blob'
            | 'bing-ads'
            | 'clickup'
            | 'customerio-app'
            | 'customerio-track'
            | 'customerio-webhook'
            | 'databricks'
            | 'email'
            | 'firebase'
            | 'github'
            | 'gitlab'
            | 'google-ads'
            | 'google-analytics'
            | 'google-cloud-service-account'
            | 'google-cloud-storage'
            | 'google-pubsub'
            | 'google-search-console'
            | 'google-sheets'
            | 'hubspot'
            | 'intercom'
            | 'jira'
            | 'linear'
            | 'linkedin-ads'
            | 'meta-ads'
            | 'pardot'
            | 'pinterest-ads'
            | 'postgresql'
            | 'reddit-ads'
            | 's3-compatible'
            | 'salesforce'
            | 'slack'
            | 'snapchat'
            | 'snowflake'
            | 'stripe'
            | 'tiktok-ads'
            | 'twilio'
            | 'vercel'
        )[]
    ) => IntegrationType[]
    organizationIntegrations: IntegrationType[] | null
    organizationIntegrationsLoading: boolean
    vercelIntegrations: IntegrationType[] | undefined
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface organizationIntegrationsLogicActions {
    deleteOrganizationIntegration: (id: IntegrationType['id']) => {
        id: number
    }
    loadOrganizationIntegrations: () => any
    loadOrganizationIntegrationsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadOrganizationIntegrationsSuccess: (
        organizationIntegrations: {
            config: any
            created_at: string
            created_by?: UserBasicType | null | undefined
            display_name: string
            errors?: string | undefined
            icon_url: any
            id: number
            kind:
                | 'apns'
                | 'aws-s3'
                | 'azure-blob'
                | 'bing-ads'
                | 'clickup'
                | 'customerio-app'
                | 'customerio-track'
                | 'customerio-webhook'
                | 'databricks'
                | 'email'
                | 'firebase'
                | 'github'
                | 'gitlab'
                | 'google-ads'
                | 'google-analytics'
                | 'google-cloud-service-account'
                | 'google-cloud-storage'
                | 'google-pubsub'
                | 'google-search-console'
                | 'google-sheets'
                | 'hubspot'
                | 'intercom'
                | 'jira'
                | 'linear'
                | 'linkedin-ads'
                | 'meta-ads'
                | 'pardot'
                | 'pinterest-ads'
                | 'postgresql'
                | 'reddit-ads'
                | 's3-compatible'
                | 'salesforce'
                | 'slack'
                | 'snapchat'
                | 'snowflake'
                | 'stripe'
                | 'tiktok-ads'
                | 'twilio'
                | 'vercel'
        }[],
        payload?: any
    ) => {
        organizationIntegrations: {
            config: any
            created_at: string
            created_by?: UserBasicType | null | undefined
            display_name: string
            errors?: string | undefined
            icon_url: any
            id: number
            kind:
                | 'apns'
                | 'aws-s3'
                | 'azure-blob'
                | 'bing-ads'
                | 'clickup'
                | 'customerio-app'
                | 'customerio-track'
                | 'customerio-webhook'
                | 'databricks'
                | 'email'
                | 'firebase'
                | 'github'
                | 'gitlab'
                | 'google-ads'
                | 'google-analytics'
                | 'google-cloud-service-account'
                | 'google-cloud-storage'
                | 'google-pubsub'
                | 'google-search-console'
                | 'google-sheets'
                | 'hubspot'
                | 'intercom'
                | 'jira'
                | 'linear'
                | 'linkedin-ads'
                | 'meta-ads'
                | 'pardot'
                | 'pinterest-ads'
                | 'postgresql'
                | 'reddit-ads'
                | 's3-compatible'
                | 'salesforce'
                | 'slack'
                | 'snapchat'
                | 'snowflake'
                | 'stripe'
                | 'tiktok-ads'
                | 'twilio'
                | 'vercel'
        }[]
        payload?: any
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface organizationIntegrationsLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        vercelIntegrations: (organizationIntegrations: IntegrationType[] | null) => IntegrationType[] | undefined
        getOrganizationIntegrationsByKind: (
            organizationIntegrations: IntegrationType[] | null
        ) => (
            kinds: (
                | 'apns'
                | 'aws-s3'
                | 'azure-blob'
                | 'bing-ads'
                | 'clickup'
                | 'customerio-app'
                | 'customerio-track'
                | 'customerio-webhook'
                | 'databricks'
                | 'email'
                | 'firebase'
                | 'github'
                | 'gitlab'
                | 'google-ads'
                | 'google-analytics'
                | 'google-cloud-service-account'
                | 'google-cloud-storage'
                | 'google-pubsub'
                | 'google-search-console'
                | 'google-sheets'
                | 'hubspot'
                | 'intercom'
                | 'jira'
                | 'linear'
                | 'linkedin-ads'
                | 'meta-ads'
                | 'pardot'
                | 'pinterest-ads'
                | 'postgresql'
                | 'reddit-ads'
                | 's3-compatible'
                | 'salesforce'
                | 'slack'
                | 'snapchat'
                | 'snowflake'
                | 'stripe'
                | 'tiktok-ads'
                | 'twilio'
                | 'vercel'
            )[]
        ) => IntegrationType[]
    }
}

export type organizationIntegrationsLogicType = MakeLogicType<
    organizationIntegrationsLogicValues,
    organizationIntegrationsLogicActions,
    Record<string, any>,
    organizationIntegrationsLogicMeta
>

export const organizationIntegrationsLogic = kea<organizationIntegrationsLogicType>([
    path(['scenes', 'settings', 'organization', 'organizationIntegrationsLogic']),

    actions({
        deleteOrganizationIntegration: (id: IntegrationType['id']) => ({ id }),
    }),

    loaders(() => ({
        organizationIntegrations: [
            null as IntegrationType[] | null,
            {
                loadOrganizationIntegrations: async () => {
                    const res = await api.organizationIntegrations.list()

                    return res.results.map((integration) => {
                        return {
                            ...integration,
                            icon_url: ICONS[integration.kind],
                        }
                    })
                },
            },
        ],
    })),

    selectors({
        vercelIntegrations: [
            (s) => [s.organizationIntegrations],
            (organizationIntegrations: IntegrationType[] | null) => {
                return organizationIntegrations?.filter((x) => x.kind === 'vercel')
            },
        ],
        getOrganizationIntegrationsByKind: [
            (s) => [s.organizationIntegrations],
            (organizationIntegrations: IntegrationType[] | null) => {
                return (kinds: IntegrationKind[]) =>
                    organizationIntegrations?.filter((i) => kinds.includes(i.kind)) || []
            },
        ],
    }),

    listeners(({ actions, values }) => ({
        deleteOrganizationIntegration: ({ id }) => {
            const integration = values.organizationIntegrations?.find((x) => x.id === id)
            if (!integration) {
                return
            }

            Dialog.open({
                title: `Disconnect ${integration.kind} integration?`,
                description:
                    'This will remove the integration from your organization. Insights resources configured to use this integration will stop working.',
                primaryButton: {
                    children: 'Yes, disconnect',
                    status: 'danger',
                    onClick: async () => {
                        try {
                            await api.organizationIntegrations.delete(id)
                            toast.success('Integration disconnected.')
                            actions.loadOrganizationIntegrations()
                        } catch (error: any) {
                            toast.error(error?.detail || 'Failed to disconnect integration. Please try again.')
                        }
                    },
                },
                secondaryButton: {
                    children: 'Cancel',
                },
            })
        },
    })),

    afterMount(({ actions }) => {
        actions.loadOrganizationIntegrations()
    }),
])
