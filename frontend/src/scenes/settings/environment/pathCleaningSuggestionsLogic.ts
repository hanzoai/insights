...
export const pathCleaningSuggestionsLogic = kea<pathCleaningSuggestionsLogicType>([
    ...
    listeners(({ values, actions }) => ({
        ...
        loadPreviewFailure: () => {
            ...
            toast.error('Could not load the preview. Please try again.')
        },
        applySuggestion: async ({ id }) => {
            ...
            try {
                ...
                toast.success(`Applied ${result.applied} path cleaning rule${result.applied === 1 ? '' : 's'}`)
                ...
                toast.error('Could not apply the path cleaning suggestions. Please try again.')
            } finally {
                actions.applySuggestionFinished()
            }
        },
