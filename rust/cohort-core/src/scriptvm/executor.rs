//! Run compiled cohort-filter bytecode through `scriptvm::sync_execute`, coercing the result to the
...
use dashmap::DashSet;
use scriptvm::{sync_execute, ExecutionContext, Program, VmError};
use metrics::counter;
...
/// Unknown-native function names already logged once. Bounded by the InsightsQL native surface
...
/// ScriptVM stack ceiling for cohort evaluation. A `person.properties.X IN (...)` leaf compiles to a
...
/// Python/TS/Datastore reference; opt-in, so other `scriptvm` consumers like cymbal keep strict ones.
...
mod tests {
    ...
    fn numeric_gt_person_cohort_coerces_the_string_threshold() {
        ...
        // Coercing comparisons promote both sides to numeric, matching Datastore/TS behaviour.
        let mut bc = vec![json!("_H"), json!(1)];
        bc.extend_from_slice(&[json!(OP_STRING), json!("10")]);
        bc.extend_from_slice(&push_person_property("bc_num"));
        bc.push(json!(OP_GT));
