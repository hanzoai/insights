import { DiffEvidenceCard } from 'products/insights_ai/frontend/api/tools'

import type { InsightsFunctionFieldDiff } from './insightsFunctionConfigDiffUtils'

// A pseudo-path per field so the shared diff viewer picks the right syntax highlighting — JSON
// fields pretty-print as JSON; script source has no registered grammar and falls back to plain text.
function fieldPath(diff: InsightsFunctionFieldDiff): string {
 ...
 * by `buildInsightsFunctionConfigDiff`.
 */
export function InsightsFunctionConfigDiff({ diffs }: { diffs: InsightsFunctionFieldDiff[] }): JSX.Element {
    return (
