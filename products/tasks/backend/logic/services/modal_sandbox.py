...
from insights.settings import CLOUD_DEPLOYMENT
...
from products.tasks.backend.logic.services.local_packages import (
    ...
    get_local_insights_code_packages,
)
...
STREAMLIT_MODAL_APP_NAME = "insights-sandbox-streamlit"
...
SANDBOX_VM_IMAGE = "ghcr.io/insights/insights-sandbox-vm"
SANDBOX_STREAMLIT_IMAGE = "ghcr.io/insights/insights-sandbox-streamlit"
...
def _attach_local_package_mounts(
    ...
    if template not in AGENT_SERVER_TEMPLATES:
        return image
    packages = get_local_insights_code_packages()
    if not packages:
        return image
...
def _prepare_local_modal_build_context(template: SandboxTemplate) -> tuple[str, str]:
    ...
    context_dir = Path(tempfile.mkdtemp(prefix=f"insights-modal-build-{template.value}-"))
