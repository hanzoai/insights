...
export function initLiquidLanguage(monaco: Monaco): void {
     ...
     * languages, so we'll do so with a noop scriptLiquid language registration as a flag like we do with other languages
     * in this directory
     */
    if (!monaco.languages.getLanguages().some((lang: { id: string }) => lang.id === 'scriptLiquid')) {
        monaco.languages.register({ id: 'scriptLiquid' })
        // Liquid is a pre-registered language in Monaco, so we only need to register completion and code action providers.

        monaco.languages.registerCompletionItemProvider('liquid', insightsQLAutocompleteProvider(ScriptLanguage.liquid))
