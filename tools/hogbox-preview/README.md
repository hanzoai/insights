...
Per-PR Insights preview environments, the layer-agnostic way.

Spins Insights up inside a disposable box and serves it at a **stable URL** that
survives the box being recreated. Today the box is a
[hogland](https://github.com/Insights/hogland) hogbox (Firecracker microVM,
...
- **Stack** (`stack.py`) — _what runs in the box_: the Insights docker-compose
...
backend = HoglandBackend(host="https://hogland.mascot-kitefin.ts.net", name="preview-pr-123")
url = InsightsPreviewStack(backend, branch="pull/123/head").bring_up()
print(url)  # https://pen-….boxes.hogland.prod-us.insights.dev/  (stable across rebuilds)
```

## How it actually works

1. **Restore a warm golden** (`alias:insights-preview-golden`) — a Firecracker
   snapshot of Insights already running on the `ghcr.io/insights/insights:master`
   image, pg + Datastore migrated, demo-data seeded. A warm restore resumes a
   live, serving Insights in ~30s (no cold boot, no rebuild). Sizing must match
   the golden (8 vCPU / 16 GiB / 100 GiB).
2. **Mount the PR's backend** (`insights`, `ee`, `products`) over the image's
...
Driven entirely by the **`insights-hogland` Python SDK** over hogplane's HTTP API
...
Access is **tailnet-only** (Insights VPN) — internal reviewers, no public URL.
...
per-PR. The golden is built on the **hogland** side by `scripts/insights-preview-setup.sh`
...
(stack running) so restores resume a serving Insights. It bakes `JS_URL=""` +
