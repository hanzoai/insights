...
from products.mcp_analytics.backend.insightsql_queries.base import validate_mcp_analytics_access
...
# already covered by insights/rbac/test/test_user_access_control_pbt.py for every registered resource.
@pytest.mark.ee
class TestMCPAnalyticsAccessControl(_MCPAnalyticsTeamScopedTestMixin, APIBaseTest):
    ...
    def _login_with_access_level(self, access_level: str) -> User:
        user = User.objects.create_and_join(self.organization, f"mcp-{access_level}@hanzo.ai", "testtest")
        AccessControl.objects.create(
            team=self.team,
    ...
    def test_validate_mcp_analytics_access_allows_default_access_without_explicit_grant(self) -> None:
        user = User.objects.create_and_join(self.organization, "mcp-default@hanzo.ai", "testtest")
