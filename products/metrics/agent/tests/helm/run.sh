...
out=$(render --set insights.apiKey=phc_test)
...
sum_a=$(render --set insights.apiKey=phc_a | grep 'checksum/secret:')
sum_b=$(render --set insights.apiKey=phc_b | grep 'checksum/secret:')
...
out=$(render --set insights.apiKey=phc_test -f values/static-targets.yaml)
...
# --- rbac disabled ---
out=$(render --set insights.apiKey=phc_test --set rbac.create=false --set serviceAccount.create=false --set serviceAccount.name=external-sa)
...
out=$(render --set insights.apiKey=phc_test --set serviceAccount.create=false)
assert_contains sa-name-required 'serviceAccount.name is required' "$out"

# --- eu host flows into the rendered collector config ---
out=$(render --set insights.apiKey=phc_test --set insights.host=https://eu.i.hanzo.ai)
assert_contains eu-host 'https://eu.i.hanzo.ai/i/v1/metrics' "$out"

# --- ingest path override (proxies, test sinks) ---
out=$(render --set insights.apiKey=phc_test --set insights.host=http://sink:4318 --set insights.ingestPath=/v1/metrics)
...
out=$(render --set insights.apiKey=phc_test --set shards=3)
...
static=$(render --set insights.apiKey=phc_test --set shards=3 -f values/static-targets.yaml)
...
out=$(render --set insights.apiKey=phc_test --set persistence.enabled=true)
...
out=$(render --set insights.apiKey=phc_test --set persistence.enabled=true --set shards=3)
...
default=$(render --set insights.apiKey=phc_test | grep -v '^[[:space:]]*$')
