...
class GoogleSheetsSource(SimpleSource[GoogleSheetsSourceConfig]):
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            "Spreadsheet access denied": "Import failed: Insights does not have access to this spreadsheet. Please share it with our service account as described at https://hanzo.ai/docs/cdp/sources/google-sheets",
    ...
    def validate_credentials(
        ...
        try:
            ...
            return (
                False,
                "Insights does not have access to this spreadsheet. Share it with our service account "
                f"({settings.GOOGLE_SHEETS_SERVICE_ACCOUNT_CLIENT_EMAIL}) as a Viewer, then try again. "
                "See https://hanzo.ai/docs/cdp/sources/google-sheets for more.",
            )
            ...
            return (
                ...
                "with our service account as described at https://hanzo.ai/docs/cdp/sources/google-sheets",
            )
        except (google_auth_exceptions.RefreshError, google_auth_exceptions.TransportError) as e:
            # These come from fetching Insights's own service-account OAuth token (the user only shares
            ...
            if getattr(e, "retryable", False):
                return (
                    False,
                    "Insights couldn't verify access to your Google Sheet right now because Google returned a "
                    "temporary error. Please try again in a moment.",
                )
            return (
                False,
                "Insights couldn't authenticate with Google to verify access to your Google Sheet. This looks "
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            label="Google Sheets",
            caption="Ensure you have granted Insights access to your Google Sheet as instructed in the [documentation](https://hanzo.ai/docs/cdp/sources/google-sheets). The first row of each sheet must contain unique column headers, since Insights reads it as the column names when syncing.",
