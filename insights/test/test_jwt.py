...
from insights.jwt import InsightsJwtAudience, decode_jwt, encode_jwt, signing_key_fingerprint

PRIMARY = "primary-signing-key"
ROTATED = "rotated-new-signing-key"
UNKNOWN = "never-configured-signing-key"
AUD = InsightsJwtAudience.EXPORTED_ASSET


def _raw_token(
    key: str,
    *,
    audience: InsightsJwtAudience = AUD,
    kid: str | None = None,
...
class TestJwt(SimpleTestCase):
    ...
    @parameterized.expand(
        [
            ...
            (
                "wrong audience",
                lambda: encode_jwt({"id": 7}, timedelta(minutes=5), InsightsJwtAudience.UNSUBSCRIBE),
    ...
    @parameterized.expand(
        [
            ...
            (
                "wrong audience",
                lambda: _raw_token(PRIMARY, audience=InsightsJwtAudience.UNSUBSCRIBE),
                jwt.InvalidAudienceError,
            ),
        ]
    )
