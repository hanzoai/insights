...
the Insights UI — `archived` was used by a short-lived "Archive" button (Sep 2024 to Mar 2025)
...
Usage:
    ...
    # Live run — updates Postgres and re-syncs to Datastore
...
from products.error_tracking.backend.models import ErrorTrackingIssue, sync_issues_to_datastore
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--live-run",
            action="store_true",
            help="Actually update rows and re-sync to Datastore (default is dry-run).",
        )
        parser.add_argument(
            "--team-id",
            type=int,
    ...
    def _backfill_team(self, *, team_id: int, statuses: tuple[str, ...]) -> int:
        with transaction.atomic():
            ...
            # sync_issues_to_datastore maps deprecated statuses to resolved.
            sync_issues_to_datastore(issue_ids=updated_issue_ids, team_id=team_id)
