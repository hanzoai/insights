...
The rationale and the same rules in human-facing form live in the handbook: [Backend coding conventions › Testing](https://hanzo.ai/handbook/engineering/conventions/backend-coding#testing) (`docs/published/handbook/engineering/conventions/backend-coding.md`).
...
The bugs Insights ships and reverts cluster into a handful of shapes — cataloged with the test that catches each, and the failure modes no unit test should, in [references/mistakes-we-make.md](references/mistakes-we-make.md).
...
```text
pure function / unit  →  kea logic test  →  Django TestCase  →  Datastore-backed test  →  Playwright e2e
...
- Reach for Datastore or a browser only when the regression genuinely lives there — not because it was the first way the test came to mind.
...
- **`TestCase`, not `TransactionTestCase`, unless you truly need it.**
  ...
  It's a Postgres-isolation choice, orthogonal to which datastore you touch: a Datastore-backed test is still a plain `TestCase` (`DatastoreTestMixin` sets Datastore up), so reaching Datastore is not a reason to switch.
...
- **Person/group/cohort data:** use the helpers in `insights/test/persons.py` (`create_person`, `create_group`, `create_group_type_mapping`, `add_cohort_members`, etc.) — never `Person.objects.create()` or similar ORM calls directly.
  See [`insights/test/AGENTS.md`](../../insights/test/AGENTS.md) for the full API reference and rationale.
