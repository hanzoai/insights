...
To run:
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_basic"
...
from products.insights_ai.evals.cli_mcp.scorers import CalledTargetTool, SurfacedGeneratedAppUrl
...
async def eval_bugfix(ctx: EvalContext) -> None:
    cases = [
        ...
        SandboxedEvalCase(
            ...
            prompt=(
                "Using the Insights MCP tools available to you, query how many $pageview events "
                "were captured in the last 7 days for the current project. Run a InsightsQL query "
                "via the Insights MCP server and report the total count as a single integer in "
                "your final reply."
            ),
        ),
    ]
...
async def eval_app_link_generation(ctx: EvalContext) -> None:
    ...
    tool-attached ``_insightsUrl`` on a raw query result, so the agent must call ``generate-app-url``
    ...
    cases: list[SandboxedEvalCase] = [
        ...
        # Forcing the UUID via SQL keeps the agent off persons-retrieve's `_insightsUrl` shortcut,
        ...
        SandboxedEvalCase(
            ...
            prompt=(
                ...
                "clickable link to open that person's profile page in Insights."
            ),
            expected={
        ...
        # Event link: another raw-SQL entity with no `_insightsUrl` tool, where ids must be passed
        # as params rather than transcribed into the path.
        SandboxedEvalCase(
            name="event_link",
            prompt=(
                "Using SQL, find one recent `$pageview` event and read its uuid and timestamp. "
                "Then give me a clickable link to view that specific event in Insights."
            ),
            expected={
                "called_target_tool": {"tool": "generate-app-url"},
                "surfaced_generated_app_url": {},
