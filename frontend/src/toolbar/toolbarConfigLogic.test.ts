...
describe('toolbar toolbarConfigLogic', () => {
    ...
    it('normalizes uiHost to not end with a slash', () => {
        const logic = toolbarConfigLogic.build({
            insights: { config: { ui_host: 'https://selfhosted.example.com/' } } as any,
        } as any)
        logic.mount()
    ...
    describe('uiHost resolution', () => {
        it('prefers explicit uiHost prop over everything else', () => {
            const logic = toolbarConfigLogic.build({
                ...
                insights: { requestRouter: { uiHost: 'https://should-not-be-used.com' }, config: {} } as any,
            } as any)
            logic.mount()
            expect(logic.values.uiHost).toBe('https://us.hanzo.ai')
        })

        it.each([
            ['https://us.hanzo.ai', 'https://us.hanzo.ai'],
            ['https://eu.hanzo.ai', 'https://eu.hanzo.ai'],
        ])('uses requestRouter.uiHost when no explicit uiHost prop', (requestRouterUiHost, expected) => {
            const logic = toolbarConfigLogic.build({
                apiURL: 'http://should-not-be-used',
                insights: { requestRouter: { uiHost: requestRouterUiHost }, config: {} } as any,
            } as any)
            logic.mount()
        ...
        it('uses requestRouter.uiHost even when apiURL is a reverse proxy', () => {
            const logic = toolbarConfigLogic.build({
                ...
                insights: { requestRouter: { uiHost: 'https://us.hanzo.ai' }, config: {} } as any,
            } as any)
            logic.mount()
            expect(logic.values.uiHost).toBe('https://us.hanzo.ai')
        })
        ...
        it('falls back to ui_host config when no requestRouter', () => {
            const logic = toolbarConfigLogic.build({
                insights: {
                    config: { ui_host: 'https://my-insights.example.com' },
                } as any,
            } as any)
            logic.mount()
            expect(logic.values.uiHost).toBe('https://my-insights.example.com')
        })

        it('falls back to apiURL when no requestRouter and no ui_host', () => {
            const logic = toolbarConfigLogic.build({
                apiURL: 'https://selfhosted.example.com',
                insights: { config: {} } as any,
            } as any)
            logic.mount()
            expect(logic.values.uiHost).toBe('https://selfhosted.example.com')
        })
        ...
        it('accepts valid https uiHost', () => {
            const logic = toolbarConfigLogic.build({
                uiHost: 'https://app.hanzo.ai',
                apiURL: 'https://fallback.example.com',
            } as any)
            logic.mount()
            expect(logic.values.uiHost).toBe('https://app.hanzo.ai')
        })
    ...
    describe('authenticate confirm modal gating', () => {
        it.each([
            ['https://us.hanzo.ai', true],
            ['https://eu.hanzo.ai', true],
            ['https://app.hanzo.ai', true], // legacy canonical
            ['https://selfhosted.example.com', false],
            ['http://us.hanzo.ai', false], // http scheme is not trusted
            ['https://us.hanzo.ai.evil.com', false],
        ...
        it('skips the confirm modal and authenticates directly for Insights Cloud hosts', async () => {
            mockTokenExchangeSuccess()
            const logic = toolbarConfigLogic.build({ uiHost: 'https://us.hanzo.ai' } as any)
            logic.mount()
        ...
        it('opens the confirm modal for non-Insights hosts', async () => {
            mockTokenExchangeSuccess()
    ...
    describe('reachability check gating', () => {
        it('skips the HEAD check entirely for trusted Insights Cloud hosts', () => {
            const logic = toolbarConfigLogic.build({ uiHost: 'https://us.hanzo.ai' } as any)
            logic.mount()
    ...
    describe('canonicalizeUiHost', () => {
        it.each([
            ...
            ['https://us.hanzo.ai', 'https://us.hanzo.ai'],
            ['https://us.hanzo.ai/', 'https://us.hanzo.ai'],
            ['HTTPS://US.INSIGHTS.COM', 'https://us.hanzo.ai'], // lowercased
            ['https://us.hanzo.ai:443', 'https://us.hanzo.ai'], // default port stripped
            ['https://us.hanzo.ai/some/path?q=1#hash', 'https://us.hanzo.ai'], // origin only
            ...
            ['blob:https://us.hanzo.ai/abc', null],
            ['https://us.hanzo.ai@evil.com', null], // userinfo rejected
            ['https://user:pass@us.hanzo.ai', null], // userinfo rejected
            ['not a url', null],
    ...
    describe('OAuth localStorage restoration', () => {
        ...
        it('restores legacy tokens (no stored uiHost) on trusted Insights Cloud hosts', () => {
        ...
        it('discards tokens whose stored fields are not strings', () => {
            localStorage.setItem(
                OAUTH_LOCALSTORAGE_KEY,
                JSON.stringify({ accessToken: 123, refreshToken: {}, clientId: null, uiHost: 'https://us.hanzo.ai' })
            )
            const logic = toolbarConfigLogic.build({ uiHost: 'https://us.hanzo.ai' } as any)
            logic.mount()
    ...
    describe('toolbar re-launch and token persistence', () => {
        ...
        it('restores tokens from OAuth localStorage when insights-js re-launches without tokens', () => {
            // insights-js overwrites _insightsToolbarParams on re-launch, losing tokens.
            // OAuth tokens are persisted separately in _insightsToolbarOAuth.
            localStorage.setItem(
                OAUTH_LOCALSTORAGE_KEY,
                JSON.stringify({
                    accessToken: 'stored-access',
        ...
        it('code exchange takes priority over stored tokens', async () => {
            ...
            window.history.pushState({}, '', '/#__insights_toolbar=code:fresh,client_id:new-client')
            mockTokenExchangeSuccess()
        ...
        it('falls back to stored tokens when code exchange fails due to expired PKCE', async () => {
            ...
            window.history.pushState({}, '', '/#__insights_toolbar=code:stale,client_id:some-client')
            mockTokenExchangeSuccess()
        ...
        it('logout clears both localStorage keys', () => {
            ...
            expect(localStorage.getItem(OAUTH_LOCALSTORAGE_KEY)).toBeNull()
            expect(localStorage.getItem('_insightsToolbarParams')).toBeNull()
            expect(localStorage.getItem(PKCE_STORAGE_KEY)).toBeNull()
        })
    ...
    describe('authorization code extraction and hash cleanup', () => {
        ...
        it.each([
            ['toolbar-only hash is fully removed', '#__insights_toolbar=code:abc,client_id:xyz', '/'],
            [
                'preserves original fragment before toolbar param',
                '#section1&__insights_toolbar=code:abc,client_id:xyz',
                '/#section1',
            ],
            [
                'preserves multi-part original fragment',
                '#/dashboard&tab=1&__insights_toolbar=code:abc,client_id:xyz',
                '/#/dashboard&tab=1',
            ],
            ['handles percent-encoded delimiters', '#__insights_toolbar=code%3Aabc%2Cclient_id%3Axyz', '/'],
            [
                'restores ? when toolbar param was the first hash query on a SPA route',
                '#/login?__insights_toolbar=code:abc,client_id:xyz&foo=bar',
                '/#/login?foo=bar',
            ],
            [
                'preserves trailing hash query when toolbar param was last',
                '#/login?foo=bar&__insights_toolbar=code:abc,client_id:xyz',
                '/#/login?foo=bar',
            ],
        ...
        it('uses uiHost-derived URLs for token exchange', async () => {
            ...
            const logic = toolbarConfigLogic.build({
                insights: { config: { ui_host: 'https://us.hanzo.ai' } } as any,
            } as any)
            logic.mount()
            ...
            expect(tokenExchangeCall[0]).toBe('https://us.hanzo.ai/oauth/token/')
            const body = new URLSearchParams(tokenExchangeCall[1].body)
            expect(body.get('redirect_uri')).toBe('https://us.hanzo.ai/toolbar_oauth/callback')
        })
    ...
    describe('readToolbarAuthHash', () => {
        ...
        it.each([
            ...
            ['__insights= without _toolbar', '/#__insights=%7B%22action%22%3A%22ph_authorize%22%7D', null],
            ['missing code field', '/#__insights_toolbar=client_id:xyz', null],
            ['missing client_id field', '/#__insights_toolbar=code:abc', null],
            ['empty code value', '/#__insights_toolbar=code:,client_id:xyz', null],
            ['similar-looking param that is not toolbar', '/#__insights_toolbar_v2=code:abc,client_id:xyz', null],
        ...
        it.each([
            ['standard params', '/#__insights_toolbar=code:abc,client_id:xyz', { code: 'abc', clientId: 'xyz' }],
            [
                'base64url-safe code with dashes and underscores',
                '/#__insights_toolbar=code:a2gGQw68uN8fzaKelhTZZY-cuSDnP7H_x,client_id:QUKsGDyHrQqrbBwdMYaL2rmp2rPDfHJICOM5EZzY',
                {
            ...
            [
                ...
                '/#/login&__insights_toolbar=code:abc,client_id:xyz',
                { code: 'abc', clientId: 'xyz' },
            ],
            [
                'preceded by multiple hash params',
                '/#section1&tab=2&__insights_toolbar=code:abc,client_id:xyz',
                { code: 'abc', clientId: 'xyz' },
            ],
            [
                'coexists with __insights= param',
                '/#__insights=%7B%7D&__insights_toolbar=code:abc,client_id:xyz',
                { code: 'abc', clientId: 'xyz' },
            ],
            [
                'percent-encoded delimiters',
                '/#__insights_toolbar=code%3Aabc%2Cclient_id%3Axyz',
                { code: 'abc', clientId: 'xyz' },
            ],
    ...
    describe('cleanToolbarAuthHash', () => {
        ...
        it.each([
            ['toolbar-only hash', '/#__insights_toolbar=code:abc,client_id:xyz', '/'],
            [
                'preserves fragment before toolbar param',
                '/#section1&__insights_toolbar=code:abc,client_id:xyz',
                '/#section1',
            ],
            ['hash-based SPA route (Angular-style)', '/#/login&__insights_toolbar=code:abc,client_id:xyz', '/#/login'],
            [
                'hash-based SPA route with nested path',
                '/#/dashboard/settings&__insights_toolbar=code:abc,client_id:xyz',
                '/#/dashboard/settings',
            ],
            [
            ...
            [
                ...
                '/#section1&__insights_toolbar=code:abc,client_id:xyz&other=value',
                '/#section1&other=value',
            ],
            [
                'duplicate toolbar params from re-authentication',
                '/#__insights_toolbar=code:old,client_id:old&__insights_toolbar=code:new,client_id:new',
                '/',
            ],
            [
                'duplicate toolbar with SPA route preserved',
                '/#/app&__insights_toolbar=code:old,client_id:old&__insights_toolbar=code:new,client_id:new',
                '/#/app',
            ],
            [
                'preserves __insights= param (owned by insights-js)',
                '/#__insights=%7B%7D&__insights_toolbar=code:abc,client_id:xyz',
                '/#__insights={}',
            ],
            ['percent-encoded toolbar param', '/#__insights_toolbar=code%3Aabc%2Cclient_id%3Axyz', '/'],
            ['page has query string and hash', '/#__insights_toolbar=code:abc,client_id:xyz', '/'],
        ...
        it.each([
            ...
            ['__insights= without toolbar', '/#__insights=%7B%7D'],
            ['empty hash', '/#'],
        ...
        it('preserves page query string when cleaning hash', () => {
            // Simulate: https://example.com/page?q=search#__insights_toolbar=code:abc,client_id:xyz
            window.history.pushState({}, '', '/page?q=search#__insights_toolbar=code:abc,client_id:xyz')
            cleanToolbarAuthHash()
    ...
    describe('apiHost validation', () => {
        it('prefers insights.config.api_host over apiURL', () => {
            const logic = toolbarConfigLogic.build({
                apiURL: 'https://should-not-be-used.example.com',
                insights: { config: { api_host: 'https://us.i.hanzo.ai' } } as any,
            } as any)
            logic.mount()
            expect(logic.values.apiHost).toBe('https://us.i.hanzo.ai')
        })

        it('falls back to apiURL when no insights config', () => {
        ...
        it('preserves the URL path for reverse-proxy deployments', () => {
            // Self-hosted customers routing Insights through a reverse proxy at a
        ...
        it('resolves a relative reverse-proxy api_host against the current origin', () => {
            // insights-js commonly points api_host at a relative reverse-proxy path
            ...
            const logic = toolbarConfigLogic.build({
                insights: { config: { api_host: '/ingest' } } as any,
            } as any)
            logic.mount()
            expect(logic.values.apiHost).toBe(`${window.location.origin}/ingest`)
            expect(logic.values.apiHostResolution.source).toBe('insights_api_host')
        })

        it.each([
            ['apiURL', { apiURL: '' }],
            ['api_host', { apiURL: '', insights: { config: { api_host: '' } } as any }],
            ...
            const logic = toolbarConfigLogic.build({
                ...
                ...(field === 'apiURL'
                    ? { apiURL: malicious }
                    : { insights: { config: { api_host: malicious } } as any }),
            })
            logic.mount()
        ...
        it.each(['data:text/html,<script>alert(1)</script>', 'vbscript:msgbox'])(
            ...
            (maliciousHost) => {
                const logic = toolbarConfigLogic.build({
                    ...
                    insights: { config: { api_host: maliciousHost } } as any,
                } as any)
                logic.mount()
                expect(logic.values.apiHost).toBe(window.location.origin)
            }
        ...
        it('rejects apiHost with userinfo on either branch (visual spoof protection)', () => {
            const logic = toolbarConfigLogic.build({
                apiURL: 'https://us.hanzo.ai@evil.com',
                insights: { config: { api_host: 'https://app.hanzo.ai@attacker.com' } } as any,
            } as any)
            logic.mount()
            expect(logic.values.apiHost).toBe(window.location.origin)
            expect(logic.values.apiHostResolution.source).toBe('fallback_rejected')
    ...
    describe('canonicalizeApiHost', () => {
        it.each([
            ...
            ['https://proxy.example.com/ingest/', 'https://proxy.example.com/ingest'],
            ['https://us.i.hanzo.ai', 'https://us.i.hanzo.ai'],
            ['https://us.i.hanzo.ai/', 'https://us.i.hanzo.ai'],
