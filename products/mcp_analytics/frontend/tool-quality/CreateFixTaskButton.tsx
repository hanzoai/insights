...
function openCreateFixTaskForm(context: MCPErrorContext, githubIntegrations: IntegrationType[]): void {
    ...
    if (!defaultIntegration) {
        toast.error('Connect the GitHub integration (Settings → Integrations) before creating a fix task')
        return
    }
    ...
    Dialog.openForm({
        ...
        content: (
            <div className="flex flex-col gap-y-4">
                ...
                <Field name="title" label="Title">
                    <Input data-attr="mcp-fix-task-title" placeholder="Task title" size="small" />
                </Field>
                <Field name="description" label="Description">
                    <TextArea data-attr="mcp-fix-task-description" placeholder="Start typing..." rows={10} />
                </Field>
            </div>
        ),
        errors: {
        ...
        onSubmit: async ({ title, description, repositories }) => {
            ...
            if (!repoName || typeof repoName !== 'string') {
                toast.error('You must choose a repository')
            ...
            if (!repoName.includes('/') && !owner) {
                ...
                toast.error('Could not determine the repository owner from the GitHub integration')
            ...
            try {
                ...
                toast.success('Fix task created and agent run started')
            } catch {
                toast.warning('Task created, but the agent run could not be started')
            }
        },
    })
}
