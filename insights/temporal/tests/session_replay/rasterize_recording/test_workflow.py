...
from insights.temporal.session_replay.rasterize_recording.workflow import RasterizeRecordingWorkflow
...
async def _register_search_attributes(env: WorkflowEnvironment) -> None:
    ...
    await env.client.operator_service.add_search_attributes(
        AddSearchAttributesRequest(
            namespace="default",
            search_attributes={
                "InsightsTeamId": IndexedValueType.INDEXED_VALUE_TYPE_INT,
                "InsightsSessionRecordingId": IndexedValueType.INDEXED_VALUE_TYPE_KEYWORD,
            },
        )
    )
...
def _counter_value(product: str, task_queue: str) -> float:
    ...
    return (
        REGISTRY.get_sample_value(
            "insights_rasterization_completed_total",
