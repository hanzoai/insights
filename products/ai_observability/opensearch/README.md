...
Local Docker OpenSearch cluster + checked-in `llm-traces-v0_1` index template. Stage 0 of the LLMA reverse-index POC; the (forthcoming) `rust/opensearch-indexer` writes to `localhost:9201`. Prod uses managed AWS OpenSearch Service via Terraform in `insights-cloud-infra`.
