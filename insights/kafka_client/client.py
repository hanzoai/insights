...
points for producing are in `insights.kafka_client.routing`:
...
KAFKA_PRODUCER_MESSAGES_COUNTER = Counter(
    "insights_kafka_producer_messages_total",
...
class DatastoreProducer:
    """Writes a row to Datastore via Kafka in production, or directly via SQL in tests.
