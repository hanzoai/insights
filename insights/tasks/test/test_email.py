...
from insights.models.messaging import MessagingRecord, get_email_hashes
...
from insights.tasks.test.utils_email_tests import mock_email_messages
...
class TestEmail(APIBaseTest, DatastoreTestMixin):
    ...
    def test_send_invite_delegation_uses_dedicated_template_and_subject(self, MockEmailMessage: MagicMock) -> None:
        ...
        invite = OrganizationInvite.objects.create(
            organization=org,
            created_by=user,
            target_email="delegate@hanzo.ai",
        ...
        assert "setting up Insights" in subject
    ...
    def test_send_delegation_invite_falls_back_when_organization_name_is_a_url(
        ...
        assert len(mocked_email_messages) == 1
        subject = MockEmailMessage.call_args.kwargs["subject"]
        assert subject == "Admin asked you to finish setting up Insights for their organization"
        assert MockEmailMessage.call_args.kwargs["template_name"] == "delegation_invite"
    ...
    def test_send_invite_falls_back_when_organization_name_is_a_url(self, MockEmailMessage: MagicMock) -> None:
        ...
        assert mocked_email_messages[0].send.call_count == 1
        subject = MockEmailMessage.call_args.kwargs["subject"]
        assert subject == "Admin invited you to join their organization on Insights"
        html = mocked_email_messages[0].html_body
        assert "their organization" in html
        assert "https://acme.example.com" not in html
    ...
    def test_send_member_join_falls_back_when_organization_name_is_a_url(self, MockEmailMessage: MagicMock) -> None:
        ...
        org, _admin = create_org_team_and_user("2022-01-02 00:00:00", "admin@hanzo.ai")
        org.name = "https://acme.example.com"
        org.save()
    ...
    def test_send_member_join_falls_back_when_invitee_first_name_is_a_url(self, MockEmailMessage: MagicMock) -> None:
        ...
        assert len(mocked_email_messages) == 1
        subject = MockEmailMessage.call_args.kwargs["subject"]
        assert subject == "A new teammate joined you on Insights"
        html = mocked_email_messages[0].html_body
    ...
    def test_send_email_change_emails_falls_back_when_user_name_is_a_url(self, MockEmailMessage: MagicMock) -> None:
        ...
        send_email_change_emails(
            ...
            old_address="old@hanzo.ai",
            new_address="new@hanzo.ai",
        )
    ...
    def test_send_member_join_skips_users_who_disabled_org_notifications(self, MockEmailMessage: MagicMock) -> None:
        ...
        org, admin_user = create_org_team_and_user("2022-01-02 00:00:00", "admin@hanzo.ai")
    ...
    def test_send_wizard_pr_ready_email_uses_customer_io_context(
        ...
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        org, user = create_org_team_and_user("2022-01-02 00:00:00", "wizard@hanzo.ai")
        team = user.team
        ...
        assert call_kwargs["template_context"] == {
            ...
            "branch_name": "insights-code/install-insights",
            "task_id": str(task.id),
            "run_id": str(run.id),
            "site_url": settings.SITE_URL,
            "team_name": team.name,
            "utm_tags": "utm_source=insights&utm_medium=email&utm_campaign=wizard_pr_ready",
        }
    ...
    def test_send_wizard_pr_ready_email_skips_when_creator_cannot_access_project(
        ...
        _org, user = create_org_team_and_user("2022-01-02 00:00:00", "wizard-former-member@hanzo.ai")
        team = user.team
        assert team is not None
        Task = apps.get_model("tasks", "Task")
        TaskRun = apps.get_model("tasks", "TaskRun")
    ...
    def test_send_wizard_pr_ready_email_skips_when_delivery_already_recorded(self, MockEmailMessage: MagicMock) -> None:
        _org, user = create_org_team_and_user("2022-01-02 00:00:00", "wizard-delivered@hanzo.ai")
        team = user.team
        assert team is not None
        Task = apps.get_model("tasks", "Task")
        TaskRun = apps.get_model("tasks", "TaskRun")
                ...
                [
                    {
                        ...
                        "source_id": "abc",
                        "source_prefix": "",
                        "source_url": "https://app.hanzo.ai/project/1/data-management/sources/managed-abc/syncs",
                        "error": "Invalid API key",
                        "paused": True,
                        "url": "https://app.hanzo.ai/project/1/data-management/sources/managed-abc/syncs?schema=Invoice",
                    },
                    {
                        "schema_name": "Charge",
                        "source_type": "Stripe",
                        ...
                        "paused": False,
                        "url": "https://app.hanzo.ai/project/1/data-management/sources/managed-abc/syncs?schema=Charge",
                    },
                ],
            )
    ...
    def test_send_external_data_failure_digest_respects_rollout_flag(
        ...
        with (
            ...
            patch("insights.tasks.email.hanzo_insights.feature_enabled", return_value=flag_enabled) as mock_flag,
        ):
    ...
    def test_should_send_pipeline_error_notification_per_pipeline_opt_out(self, MockEmailMessage: MagicMock) -> None:
        self.user.partial_notification_settings = {
            ...
            "pipeline_notifications_disabled": {
                "insights_function:abc-123": True,
    ...
    def test_get_members_to_notify_for_pipeline_error_per_pipeline_opt_out(self, MockEmailMessage: MagicMock) -> None:
        ...
        pipeline_id = "insights_function:test-function-id"
        ...
        memberships = get_members_to_notify_for_pipeline_error(
            cast(Team, self.user.team), failure_rate=1.0, pipeline_id="insights_function:other-id"
        )
        assert len(memberships) == 2
    ...
    def test_send_insights_function_disabled_per_pipeline_opt_out(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        ...
        self.user.partial_notification_settings = {
            "plugin_disabled": True,
            "pipeline_notifications_disabled": {f"insights_function:{insights_function.id}": True},
        }
        self.user.save()

        send_insights_function_disabled(str(insights_function.id))
    ...
    def test_send_insights_functions_digest_email(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
            ...
            "functions": [
                {
                    ...
                    "failed": 5,
                    "failure_rate": 5.0,
                    "url": "http://localhost:8000/project/1/pipeline/destinations/test-script-function-1",
                },
                {
                    "id": "test-script-function-2",
                    ...
                    "failed": 50,
                    "failure_rate": 20.0,
                    "url": "http://localhost:8000/project/1/pipeline/destinations/test-script-function-2",
                },
            ],
        }

        send_insights_functions_digest_email(digest_data)
        ...
        digest_data = {
            ...
            "functions": [
                ...
                },
                {
                    ...
                    "failed": 3500,
                    "failure_rate": 4.5,
                    "url": "http://localhost:8000/project/1/pipeline/destinations/test-script-function-3",
                },
                {
                    "id": "test-script-function-4",
                    ...
                    "failed": 150000,
                    "failure_rate": 7.0,
                    "url": "http://localhost:8000/project/1/pipeline/destinations/test-script-function-4",
                },
                {
                    "id": "test-script-function-5",
                    ...
                    "failed": 12000,
                    "failure_rate": 2.3,
                    "url": "http://localhost:8000/project/1/pipeline/destinations/test-script-function-5",
                },
            ],
        }
    ...
    def test_send_insights_functions_digest_email_comma_formatting(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
    ...
    def test_send_insights_functions_daily_digest(self, MockEmailMessage: MagicMock) -> None:
        ...
        from insights.models.activity_logging.activity_log import ActivityLog, Detail
        ...
        ActivityLog.objects.create(
            ...
            item_id=str(insights_function.id),
            detail=Detail(name=insights_function.name, type="destination"),
        ...
        create_app_metric2(
            ...
            app_source_id=str(insights_function.id),
    ...
    def test_send_insights_functions_digest_email_with_test_email_override(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        ...
        send_insights_functions_digest_email(digest_data, test_email_override="invalid@example.com")
    ...
    def test_send_insights_functions_daily_digest_no_eligible_functions(self, MockEmailMessage: MagicMock) -> None:
        ...
        # Test 1: No InsightsFunctions created - should not send email
        send_insights_functions_daily_digest()
        assert len(mocked_email_messages) == 0

        # Test 2: InsightsFunction with no failures - should not send email
        insights_function = InsightsFunction.objects.create(
            team=self.team,
        ...
        # Test 3: Disabled InsightsFunction with failures - should not send email
        InsightsFunction.objects.all().delete()  # Clear previous functions
        disabled_function = InsightsFunction.objects.create(
            team=self.team,
        ...
        # Test 4: Deleted InsightsFunction with failures - should not send email
        InsightsFunction.objects.all().delete()  # Clear previous functions
        deleted_function = InsightsFunction.objects.create(
            team=self.team,
    ...
    @patch("insights.tasks.email.get_client")
    @patch("insights.tasks.email.check_and_cache_login_device")
    ...
    def test_send_insights_ai_access_request(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)

        org, _owner = create_org_team_and_user("2022-01-02 00:00:00", "ai-owner@hanzo.ai")
        team = org.teams.first()
        assert team is not None
        member = User.objects.create_and_join(
            organization=org,
            email="ai-member@hanzo.ai",
            password=None,
            level=OrganizationMembership.Level.MEMBER,
        )

        send_insights_ai_access_request(organization_id=str(org.id), requesting_user_id=member.id)

        assert len(mocked_email_messages) == 1
        message = mocked_email_messages[0]
        assert message.send.call_count == 1
        assert message.template_name == "insights_ai_access_requested"
        # The owner who can enable Insights AI is notified, not the requesting member.
        recipient_emails = {dest["raw_email"] for dest in message.to}
        assert recipient_emails == {"ai-owner@hanzo.ai"}
        assert message.properties["organization_name"] == org.name
        assert (
            message.properties["insights_ai_url"]
    ...
    def test_send_insights_ai_access_request_no_admins_is_noop(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)

        org, owner = create_org_team_and_user("2022-01-02 00:00:00", "solo-member@hanzo.ai")
        # Drop the only admin to a member so there's nobody who could enable Insights AI.
        membership = OrganizationMembership.objects.get(organization=org, user=owner)
        membership.level = OrganizationMembership.Level.MEMBER
        membership.save()

        send_insights_ai_access_request(organization_id=str(org.id), requesting_user_id=owner.id)
    ...
    def test_send_matview_failure_digest_shows_datastore_error_not_newer_shadow(
        self, MockEmailMessage: MagicMock
    ) -> None:
        from products.data_modeling.backend.facade.models import (
            DataModelingJob,
        ...
        assert len(mocked_email_messages) == 1
        assert "datastore boom" in mocked_email_messages[0].html_body
    ...
    def test_send_matview_failure_digest_kitchen_sink_snapshot(self, MockEmailMessage: MagicMock) -> None:
        ...
        failed_cases = [
            ...
            (
                ...
                "InsightsQL: unknown property `utm_soruce` on table `events` — did you mean `utm_source`?",
            ),
        ]
