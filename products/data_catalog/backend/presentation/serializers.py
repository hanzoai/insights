...
from insights.schema_enums import IntervalType
...
class MetricDefinitionField(serializers.JSONField):
    """A machine-readable query (InsightsQLQuery, TrendsQuery, event node, ...). Typed as a free object."""
...
class MetricRunResponseSerializer(serializers.Serializer):
    ...
    compiled_query = serializers.CharField(allow_null=True, help_text="The compiled InsightsQL, when available.")
    query_status = _FreeJSONField(allow_null=True, help_text="Async query status, when the run is not blocking.")
    insights_url = serializers.CharField(
...
class MetricRunRequestSerializer(serializers.Serializer):
    ...
    date_from = serializers.CharField(
        required=False,
        help_text="Override the start of the query window (e.g. '-7d'). Rejected for InsightsQLQuery metrics, whose window is fixed in SQL.",
    )
    ...
    interval = serializers.ChoiceField(
        ...
        required=False,
        help_text="Override the bucket interval. Rejected for InsightsQLQuery metrics.",
    )
...
class MetricSerializer(serializers.ModelSerializer):
    ...
    definition_kind = serializers.CharField(
        read_only=True,
        allow_null=True,
        help_text="Query kind of the definition (InsightsQLQuery, TrendsQuery, ...), or null for a stub.",
    )
...
class RelationshipProposalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "source_table_key": {"help_text": "InsightsQL key expression on the source table (casts allowed)."},
            "joining_table_name": {"help_text": "Name of the table being joined in."},
            "joining_table_key": {"help_text": "InsightsQL key expression on the joining table (casts allowed)."},
