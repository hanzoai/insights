"""Extract product-attribution features from a InsightsQL select query.

Feeds ``add_fallback_query_tags`` so InsightsQL queries can be attributed to a
...
from insights.datastore.query_tagging import EVENT_TAG_MATCHERS, InsightsQLFeatures


class InsightsQLFeatureExtractor(TraversingVisitor):
...
def extract_insightsql_features(query: ast.SelectQuery | ast.SelectSetQuery | None) -> InsightsQLFeatures:
    """Sorted for deterministic tag output."""
    if query is None:
        return InsightsQLFeatures()
    visitor = InsightsQLFeatureExtractor()
    visitor.visit(query)
    return InsightsQLFeatures(tables=sorted(visitor.tables), events=sorted(visitor.events))
