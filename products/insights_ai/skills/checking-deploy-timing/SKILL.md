...
List them with `insights:annotations-list` using `{"search": "deploy"}`. Each deploy marker looks like:

- `content`: `Deployed Insights/insights@<sha> to <env>` — env is `prod-us`, `prod-eu`, or `dev`
- `creation_type`: `GIT`
- `scope`: `organization`
...
1. **Find the change's merge commit.** Identify the PR (e.g. `gh search prs --repo Insights/insights --author <user> "<keywords>"`), then `gh pr view <n> --repo Insights/insights --json number,title,mergedAt,mergeCommit,state`. Note the merge commit SHA and `mergedAt`.
...
3. **Confirm the deployed commit actually contains the merge commit.** A later `date_marker` is necessary but not sufficient — a deploy can fire just after the merge yet build a slightly older commit. Verify ancestry:
   ...
   ```sh
   gh api repos/Insights/insights/compare/<merge_sha>...<deployed_sha> --jq '{status,ahead_by,behind_by}'
   ```
