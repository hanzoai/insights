...
from insights.models.team.production_event_activation import (
...
def _mock_capture():
    ...
    with patch("insights.models.team.production_event_activation.ph_scoped_capture") as mock_csm:
...
def _seed_mobile_events(team_id: int, user_count: int, is_emulator: Any = False) -> None:
    for i in range(user_count):
        _seed_event(
            team_id,
            properties={"$lib": "insights-ios", "$is_emulator": is_emulator},
            distinct_id=f"mobile-user-{i}",
        )


def _seed_server_events(team_id: int, user_count: int, lib: str = "insights-python") -> None:
    for i in range(user_count):
...
class TestTeamsMeetingCriterion(DatastoreTestMixin, BaseTest):
    ...
    def test_event_without_host_signal_does_not_qualify(self) -> None:
        _seed_event(self.team.id, properties={"$lib": "insights-python"})
        self.assertEqual(_teams_meeting_criterion([self.team.id]), {})
    ...
    def test_mobile_qualifies_without_device_id_or_emulator_flag(self) -> None:
        ...
        for i in range(MOBILE_LIB_USERS_THRESHOLD):
            _seed_event(self.team.id, properties={"$lib": "insights-ios"}, distinct_id=f"mobile-user-{i}")
        signal = _teams_meeting_criterion([self.team.id])[self.team.id]
        self.assertEqual(signal.kind, "mobile_lib_users")
    ...
    def test_mobile_counts_distinct_users(self) -> None:
        ...
        for i in range(MOBILE_LIB_USERS_THRESHOLD + 5):
            _seed_event(
                self.team.id,
                properties={"$lib": "insights-ios", "$is_emulator": False},
...
class TestEvaluateAndMarkTeamBatch(DatastoreTestMixin, BaseTest):
