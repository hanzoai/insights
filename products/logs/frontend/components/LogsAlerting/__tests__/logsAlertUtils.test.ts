import { FilterLogicalOperator, InsightsFunctionType, PropertyFilterType, PropertyOperator } from '~/types'
...
describe('logsAlertUtils', () => {
    describe('buildLogsAlertFilterConfig', () => {
        it('filters by alert_id property only so it matches every per-event InsightsFunction', () => {
    ...
    describe('groupLogsAlertDestinations', () => {
        const slackHf = (id: string, channel: string, enabled = true): InsightsFunctionType =>
            ({
                id,
                name: `slack-${id}`,
                enabled,
        ...
        const webhookHf = (id: string, url: string, enabled = true): InsightsFunctionType =>
            ({
                id,
                name: `webhook-${id}`,
                enabled,
        ...
        const teamsHf = (id: string, url: string, enabled = true): InsightsFunctionType =>
            ({
                id,
                name: `teams-${id}`,
                enabled,
        ...
        it('collapses multiple InsightsFunctions for the same slack channel into one group', () => {
            ...
            expect(groups[0].insightsFunctions).toHaveLength(2)
        })

        it('collapses multiple InsightsFunctions for the same webhook url into one group', () => {
        ...
        it('collapses multiple InsightsFunctions for the same teams url into one group', () => {
        ...
        it('marks a group as disabled when any InsightsFunction in it is disabled (AND semantics)', () => {
        ...
        it('produces an isolated per-InsightsFunction group for unrecognised inputs', () => {
            const unknown = {
                id: 'hf-x',
