...
from insights.test.base import APIBaseTest, FuzzyInt, override_settings
...
from insights.middleware import per_request_logging_context_middleware
...
class TestInsightsTokenCookieMiddleware(APIBaseTest):
    ...
    def test_logged_in_client(self):
        ...
        self.assertEqual(ph_project_token_cookie["domain"], "hanzo.ai")
        ...
        self.assertEqual(ph_project_name_cookie["domain"], "hanzo.ai")
        ...
        self.assertEqual(ph_instance_cookie["domain"], "hanzo.ai")
        ...
        self.assertEqual(ph_last_login_method_cookie["domain"], "hanzo.ai")
...
class TestAutoLogoutImpersonateMiddleware(APIBaseTest):
    ...
    @parameterized.expand(
        [
            # A safe `next` lets staff return straight to the Insights app.
            ("safe_next", "?next=/", "/"),
    ...
    def test_loginas_logout_with_next_returns_to_app_after_expiry(self):
        ...
        bounce so staff still land back in the Insights app."""
        now = datetime.now()
        with freeze_time(now):
            self.login_as_other_user()
...
class TestImpersonationReadOnlyMiddleware(APIBaseTest):
    ...
    def test_reimpersonating_same_user_read_write_clears_read_only(self):
        self.login_as_other_user_read_only()
        assert self.client.get("/v1/users/@me/").json()["is_impersonated_read_only"] is True
        ...
        self.login_as_other_user()
        assert self.client.get("/v1/users/@me/").json()["is_impersonated_read_only"] is False
    ...
    def test_rejected_reimpersonation_preserves_read_only_and_reason(self):
        ...
        user = self.client.get("/v1/users/@me/").json()
    ...
    @parameterized.expand(
        [
            ...
            ("query_kind", "query/InsightsQLQuery/", {"query": {"kind": "InsightsQLQuery", "query": "select 1"}}),
    ...
    def test_read_only_impersonation_allows_allowlisted_post(self, _name, path_suffix, body):
        ...
        response = self.client.post(
            f"/v1/projects/{self.team.id}/{path_suffix}",
            data=body,
...
class TestAdminImpersonationMiddleware(APIBaseTest):
    ...
    def test_admin_index_accessible_during_impersonation(self):
        ...
        assert b"other-user@hanzo.ai" in response.content
    ...
    @patch("insights.views.get_client")
    ...
    @patch("insights.views.get_client")
    @patch("insights.views.log_activity")
    def test_admin_action_attributes_activity_to_staff_user_during_impersonation(
        ...
        activity logging. The PR's middleware-ordering comment in `insights/settings/web.py`
...
class TestImpersonationBlockedPathsMiddleware(APIBaseTest):
    ...
    def test_impersonation_blocks_post_to_personal_api_keys_without_trailing_slash(self):
        ...
        (trailing_slash = r"/?"), so /v1/personal_api_keys reaches the same viewset as
        /v1/personal_api_keys/. The middleware must block both forms — a startswith
        check against "/v1/personal_api_keys/" alone is bypassed by the slashless variant."""
        self.login_as_other_user()
    ...
    def test_impersonation_blocks_patch_to_users_api_without_trailing_slash(self):
        """Same bypass class as the personal_api_keys slashless test: /v1/users/@me
        (no trailing slash) must be blocked just like /v1/users/@me/."""
        self.login_as_other_user()
...
class TestUpgradeImpersonation(APIBaseTest):
    ...
    def test_start_exposes_reason_on_user_api(self):
        self.login_as_read_only()
        assert self.client.get("/v1/users/@me/").json()["is_impersonated_reason"] == "Initial read-only impersonation"
...
class TestActiveOrganizationMiddleware(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("/v1/users/@me/", status.HTTP_200_OK, None),
        ]
    )
...
class TestActivityLoggingMiddleware(APIBaseTest):
    def setUp(self):
        ...
        from insights.middleware import ActivityLoggingMiddleware
...
class TestSocialAuthExceptionMiddleware(APIBaseTest):
    ...
    def setUp(self):
        ...
        from insights.middleware import SocialAuthExceptionMiddleware
...
def test_oauth_coop_middleware(path, query_string, expected_coop):
    ...
    from insights.middleware import OAuthCoopMiddleware
...
@parameterized.expand(
    [
        ("insights/mcp-server v1", "mcp"),
        ("Mozilla/5.0", "web"),
        ("insights/code", "insights_code"),
    ]
)
def test_chqueries_middleware_tags_source(user_agent, expected_source):
    ...
    from insights.middleware import CHQueries
    ...
    factory = RequestFactory()
    request = factory.get("/v1/projects/@current/query/", HTTP_USER_AGENT=user_agent)
...
@parameterized.expand(
    [
        ("api_path_emits", "/v1/projects/@current/query/", True),
        ("api_capture_excluded", "/v1/projects/@current/capture/", False),
...
def test_chqueries_middleware_api_latency_histogram_scope(name, path, should_emit):
    ...
    def sample_count() -> float:
        return REGISTRY.get_sample_value("insights_api_requests_latency_seconds_count", labels=labels) or 0.0
...
@parameterized.expand(
    [
        ("source_mcp", "insights/mcp-server v1", None, "mcp", ""),
...
def test_chqueries_middleware_api_latency_labels(name, user_agent, access_method, expected_source, expected_access):
    ...
    request = RequestFactory().get("/v1/projects/@current/query/", HTTP_USER_AGENT=user_agent)
    request.user = MagicMock(pk=1, is_authenticated=False)
    request.session = MagicMock(session_key="abc123")
...
def test_chqueries_middleware_api_latency_status_class(name, status_code, expected_class):
    ...
    request = RequestFactory().get("/v1/projects/@current/query/")
    request.user = MagicMock(pk=1, is_authenticated=False)
    request.session = MagicMock(session_key="abc123")
...
def test_query_time_counting_middleware_emits_durations_in_milliseconds() -> None:
    from insights.middleware import QueryTimeCountingMiddleware
...
@parameterized.expand(
    [
        ("/v1/projects/@current/insights/", True),
        ("/v1/projects/@current/dashboards/", True),
        ("/v1/projects/@current/property_definitions/", True),
        ("/", True),
        ("/insights/abc123", True),
        ("/dashboard/42", True),
        ("/project/2/insights", True),
        ("/v1/projects/@current/feature_flags/", False),
    ]
)
...
class TestPerRequestLoggingContextMiddlewareMcpHeaders(APIBaseTest):
    ...
    def test_binds_mcp_ids_when_present(self, _name, headers, expected_session, expected_conversation):
        with patch("insights.middleware.trace") as mock_trace:
            span = MagicMock()
