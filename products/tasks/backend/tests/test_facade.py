...
class TestFacadeReadsAndMappers(TestCase):
    ...
    def test_get_task_run_maps_all_fields(self):
        ...
        self.assertEqual(dto.pr_url, "https://github.com/insights/insights/pull/1")
    ...
    def test_warm_task_run_returns_contract(self, _mock_quota, mock_workflow):
        task = self._make_task(origin_product=Task.OriginProduct.INSIGHTS_AI)
    ...
    @parameterized.expand(
        [
            ("ready", SandboxCustomImage.Status.READY, "insights-sandbox-custom-1-abc:latest", True),
    ...
    def test_run_task_resume_carries_self_driving_head_branch(self):
        ...
        previous_run = TaskRun.objects.create(
            ...
            team=self.team,
            status=TaskRun.Status.COMPLETED,
            state={"self_driving_head_branch": "insights-self-driving/fix-abc123"},
        )
        ...
        assert result is not None and result.error is None
        new_run = task.runs.exclude(id=previous_run.id).get()
        self.assertEqual(new_run.state.get("self_driving_head_branch"), "insights-self-driving/fix-abc123")
    ...
    def test_create_and_run_persists_dispatch_params_for_reconcile(self, _mock_workflow):
        ...
        self.assertEqual(run.state["pending_dispatch"]["insights_mcp_scopes"], "full")
    ...
    def test_create_wizard_cloud_run_seeds_pending_user_message(self, _mock_workflow):
        ...
        self.assertRegex(head_branch, r"^insights/instrumentation-[0-9a-f]{6}$")
