...
- **exploring-llm-evaluations** — how to manage and investigate AI observability
  evaluations (`script`, `llm_judge`, and `sentiment`), run them on specific generations,
...
```bash
insightscli init:skill -- --product ai_observability --name my-new-skill
```
...
```bash
insightscli sync:skill -- --name exploring-llm-traces
```
