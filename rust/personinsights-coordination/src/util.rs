...
pub(crate) fn note_run_failure(
    ...
    counter!(
        "personinsights_coordination_run_restarts_total",
        "component" => component
    )
    .increment(1);
    tracing::warn!(
...
pub async fn run_lease_keepalive(
    ...
    'stream: loop {
        ...
        let (mut keeper, mut stream) = loop {
            ...
            counter!(
                "personinsights_coordination_keepalive_retries_total",
                "component" => component
            )
            .increment(1);
            tracing::warn!(
        ...
        loop {
            ...
            match outcome {
                Ok(true) => {
                    ...
                    histogram!(
                        "personinsights_coordination_lease_renewal_interval_ms",
                        "component" => component
                    )
...
pub fn record_phase_watch_delivery(
    ...
    metrics::histogram!(
        "personinsights_coordination_phase_watch_delivery_ms",
        "observer" => observer
    )
    .record(lag as f64);
}
...
pub fn record_ack_to_advance(phase: &'static str, ack_stamps_ms: impl Iterator<Item = i64>) {
    ...
    metrics::histogram!(
        "personinsights_coordination_ack_to_advance_ms",
        "phase" => phase
    )
    .record(lag as f64);
}
...
pub fn preregister_coordinator_metrics() {
    for kind in ["fresh", "move"] {
        metrics::counter!("personinsights_coordination_handoffs_created_total", "kind" => kind)
            .increment(0);
    }
    metrics::counter!("personinsights_coordination_elections_won_total").increment(0);
    metrics::counter!("personinsights_coordination_partition_releases_total").increment(0);
}
...
pub fn preregister_router_coordination_metrics() {
    for outcome in ["revoked", "revoke_failed"] {
        metrics::counter!(
            "personinsights_coordination_router_deregistered_total",
            "outcome" => outcome
        )
        .increment(0);
    }
