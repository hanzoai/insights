...
from insights.egress.github.transport import (
    GitHubEgressBudgetExhausted,
    GitHubRateLimitError,
    raise_if_github_rate_limited,
)
from insights.egress.limiter.policies import Priority
from insights.models.github_integration_base import GITHUB_BRANCH_CACHE_TTL_SECONDS, GITHUB_REPOSITORY_CACHE_TTL_SECONDS
...
class TestLinearIntegrationModel(BaseTest):
    def create_integration(self) -> Integration:
        return Integration.objects.create(
            team=self.team,
            kind="linear",
            config={"data": {"viewer": {"organization": {"urlKey": "insights"}}}},
    ...
    def test_create_issue_passes_user_fields_as_graphql_variables(self):
        ...
        with patch.object(
            ...
            attachment_url = f'https://us.hanzo.ai/project/{self.team.id}/error_tracking/issue-id" }} mutation {{'
        ...
        assert attachment_variables["title"] == "Insights issue"
...
class TestOauthIntegrationModel(BaseTest):
    ...
    @patch("insights.models.integration.requests.post")
    ...
    @patch("insights.models.integration.requests.post")
    @patch("insights.models.integration.requests.get")
    ...
    @patch("insights.models.integration.reload_integrations_on_workers")
    @patch("insights.models.integration.requests.post")
    ...
    def test_refresh_falls_back_to_previous_credentials(
        ...
        with (
            ...
            patch("insights.models.integration.reload_integrations_on_workers"),
            patch(
                "insights.models.integration.requests.post",
...
class TestInsightsConnectIntegration(BaseTest):
    ...
    @parameterized.expand(
        [
            ("US", "https://us.hanzo.ai/oauth/authorize", "us-client-id"),
            ("EU", "https://eu.hanzo.ai/oauth/authorize", "eu-client-id"),
    ...
    def test_authorize_url_targets_selected_region(self, region, expected_base, expected_client_id):
        with self.settings(**self.connect_settings):
            url = OauthIntegration.authorize_url(
                "insights", token="tok", next="/projects/test", region=region, scopes=["task:read", "task:write"]
            )
            base, _, query = url.partition("?")
            params = {k: v[0] for k, v in parse_qs(query).items()}
            ...
            assert params["redirect_uri"].endswith("/integrations/insights/callback")
            # insights uses PKCE
    ...
    def test_authorize_url_lowercases_region_input(self):
        with self.settings(**self.connect_settings):
            url = OauthIntegration.authorize_url("insights", token="tok", region="eu", scopes=["task:read"])
            assert url.startswith("https://eu.hanzo.ai/oauth/authorize")
    ...
    def test_authorize_url_unknown_region_raises(self):
        with self.settings(**self.connect_settings):
            with pytest.raises(NotImplementedError):
                OauthIntegration.authorize_url("insights", token="tok", region="ASIA", scopes=["task:read"])
    ...
    def test_authorize_url_unconfigured_region_raises(self):
        ...
        with self.settings(**unconfigured):
            with pytest.raises(NotImplementedError):
                OauthIntegration.authorize_url("insights", token="tok", region="EU", scopes=["task:read"])
    ...
    def test_integration_from_oauth_response_persists_region_and_namespaces_id(self, mock_post, mock_get):
        with self.settings(**self.connect_settings):
            ...
            mock_get.return_value.json.return_value = {"sub": "user-uuid-123", "email": "person@hanzo.ai"}

            # insights is a PKCE flow; the authorize step caches a verifier keyed on the state token.
            cache.set("oauth_pkce_verifier/tok", "the-verifier")
            state = urlencode({"next": "/", "token": "tok", "region": "EU"})
            integration = OauthIntegration.integration_from_oauth_response(
                "insights", self.team.id, self.user, {"code": "auth-code", "state": state}
            )

            # Token exchange must hit the region carried in state.
            assert mock_post.call_args[0][0] == "https://eu.hanzo.ai/oauth/token"
            assert integration.kind == "insights"
            ...
            assert integration.config["email"] == "person@hanzo.ai"
    ...
    def test_integration_from_oauth_response_fails_closed_without_pkce_verifier(self, mock_post):
        # No cached verifier (as if the authorize step was skipped / replayed). A first-party insights
...
class TestGoogleCloudIntegrationModel(BaseTest):
    mock_keyfile = {
        ...
        "client_email": "script-pubsub-test@insights-301601.iam.gserviceaccount.com",
...
class TestGitHubIntegrationModel(BaseTest):
    ...
    @patch("insights.models.github_integration_base.GitHubIntegrationBase.client_request")
    ...
    def test_get_diff_compares_branch_tips(self):
        ...
        with patch.object(github, "api_request", return_value=mock_response) as mock_get:
            result = github.get_diff("Insights/insights", target_branch="feature/foo", base_branch="master")
            assert result == {
                "success": True,
                "diff": "diff --git a b",
                "truncated": False,
    ...
    def test_get_open_pr_base_for_head_returns_none_when_no_open_pr(self):
        ...
        with patch.object(github, "_installation_authenticated_get", return_value=mock_response):
            assert github.get_open_pr_base_for_head("Insights/insights", "master") is None
    ...
    def test_get_open_pr_base_for_head_returns_none_on_error(self):
        ...
        with patch.object(github, "_installation_authenticated_get", return_value=None):
            assert github.get_open_pr_base_for_head("Insights/insights", "insights-code/fix") is None
    ...
    def test_get_pull_request_comments_merges_conversation_and_review_sorted(self):
                ...
                response.json.return_value = [
                    {
                        ...
                        "html_url": "https://github.com/Insights/insights/pull/1#issuecomment-2",
                    }
                ]
                ...
                response.json.return_value = [
                    {
                        ...
                        "html_url": "https://github.com/Insights/insights/pull/1#discussion_r1",
        ...
        with patch.object(github, "_installation_authenticated_get", side_effect=fake_get):
            result = github.get_pull_request_comments("Insights/insights", 1)
        ...
        assert [(c["author"], c["comment_type"], c["path"]) for c in result["comments"]] == [
            ("alice", "review", "insights/models.py"),
    ...
    def test_get_pull_request_checks_merges_check_runs_and_statuses(self):
        ...
        with (
            ...
            result = github.get_pull_request_checks("Insights/insights", 1)
    ...
    def test_get_pull_request_checks_follows_pagination(self):
        ...
        def fake_get(url, **kwargs):
            ...
            if "/check-runs" not in url:
                ...
                response.links = {
                    "next": {"url": "https://api.github.com/repos/Insights/insights/commits/abc123f/check-runs?page=2"}
                }
            return response
    ...
    def test_get_pull_request_checks_fails_when_later_page_is_unavailable(self, failing_source):
        ...
        def fake_get(url, **kwargs):
            ...
            if source == failing_source:
                ...
                response.links = {
                    "next": {"url": f"https://api.github.com/repos/Insights/insights/commits/abc123f/{next_path}?page=2"}
                }
            return response
    ...
    def test_get_diff_truncates_oversized_diff(self):
        from insights.models.integration import _MAX_DIFF_CHARS
    ...
    def test_get_diff_rejects_unsafe_values(self, _name, overrides):
        integration = self.create_integration(sensitive_config={"access_token": "ACCESS_TOKEN"})
        github = GitHubIntegration(integration)
        kwargs: dict = {"repository": "Insights/insights", "target_branch": "feature/foo", "base_branch": "master"}
    ...
    def test_get_diff_allows_nested_branch_names(self):
        ...
        with patch.object(github, "api_request", return_value=mock_response) as mock_get:
            result = github.get_diff(
                "Insights/insights", target_branch="feature/nested/branch", base_branch="release/v1.2"
            )
        assert result["success"] is True
        mock_get.assert_called_once()
    ...
    def test_first_for_team_repository_allows_owner_repo(self):
        ...
        with patch.object(GitHubIntegration, "installation_can_access_repository", return_value=True) as mock_access:
            result = GitHubIntegration.first_for_team_repository(self.team.id, "Insights/insights")
        assert result is not None
        mock_access.assert_called_once_with("Insights/insights")

    @parameterized.expand(
        [
            ("owner_repo", "Insights/insights", "https://api.github.com/repos/Insights/insights/pulls/123"),
            ("bare_repo", "insights", "https://api.github.com/repos/Insights/insights/pulls/123"),
        ]
    )
    def test_close_pull_request_patches_state_closed(self, _name, repository, expected_url):
        ...
        integration = self.create_integration(
            config={"account": {"name": "Insights"}}, sensitive_config={"access_token": "ACCESS_TOKEN"}
        )
        github = GitHubIntegration(integration)
        mock_response = MagicMock(status_code=200)
    ...
    def test_close_pull_request_maps_upstream_error(self):
        ...
        with patch.object(github, "_installation_authenticated_patch", return_value=mock_response):
            result = github.close_pull_request("Insights/insights", 123)
    ...
    def test_close_pull_request_from_url_parses_and_closes(self):
        ...
        with patch.object(github, "_installation_authenticated_patch", return_value=mock_response) as mock_patch:
            result = github.close_pull_request_from_url("https://github.com/Insights/insights/pull/42")
        assert result["success"] is True
        assert mock_patch.call_args.args[0] == "https://api.github.com/repos/Insights/insights/pulls/42"
    ...
    def test_close_pull_request_from_url_rejects_non_pr_url(self):
        ...
        with patch.object(github, "_installation_authenticated_patch") as mock_patch:
            result = github.close_pull_request_from_url("https://github.com/Insights/insights/issues/42")
        assert result["success"] is False
    ...
    def test_comment_on_pull_request_posts_to_issues_endpoint(self):
        ...
        with patch.object(github, "_installation_authenticated_post", return_value=mock_response) as mock_post:
            result = github.comment_on_pull_request("Insights/insights", 123, "hello")
        assert result == {"success": True}
        # PR comments go through the issues endpoint, not /pulls.
        assert mock_post.call_args.args[0] == "https://api.github.com/repos/Insights/insights/issues/123/comments"
    ...
    def test_comment_on_pull_request_from_url_parses_and_posts(self):
        ...
        mock_response = MagicMock(status_code=201)
        with patch.object(github, "_installation_authenticated_post", return_value=mock_response) as mock_post:
            result = github.comment_on_pull_request_from_url("https://github.com/Insights/insights/pull/42", "note")
        assert result["success"] is True
        assert mock_post.call_args.args[0] == "https://api.github.com/repos/Insights/insights/issues/42/comments"
    ...
    @patch("insights.egress.transport.transport.requests.request")
    def test_github_api_request_metrics_include_integration_and_rate_limit_headers(
        ...
        integration = self.create_integration(
            {"installation_id": f"INSTALL-{_name}", "account": {"name": "Insights"}},
            {"access_token": "ACCESS_TOKEN"},
        )
        response = MagicMock()
    ...
    def test_github_api_request_metrics_include_request_exceptions(self, mock_get):
        integration = self.create_integration(
            {"installation_id": "INSTALL", "account": {"name": "Insights"}},
            {"access_token": "ACCESS_TOKEN"},
        )
    ...
    @patch("insights.egress.transport.transport.requests.request")
    @patch("insights.models.integration.GitHubIntegration.access_token_expired", return_value=False)
    def test_list_repositories_retries_transient_non_json_response(self, _mock_expired, mock_get):
        ...
        success.json.return_value = {
            "repositories": [
                ...
                {"id": 2, "name": "insights-js", "full_name": "Insights/insights-js"},
            ]
        }
    ...
    def test_list_all_repositories_raises_when_later_page_fails(self, _mock_expired, mock_get):
        ...
        first_page.json.return_value = {
            "repositories": [{"id": i, "name": f"repo-{i}", "full_name": f"Insights/repo-{i}"} for i in range(100)]
        }
    ...
    @patch("insights.models.integration.GitHubIntegration.list_repositories")
    def test_list_all_repositories_fetches_all_pages(self, mock_list):
        ...
        first_page = [{"id": i, "name": f"repo-{i}", "full_name": f"Insights/repo-{i}"} for i in range(100)]
        second_page = [{"id": i, "name": f"repo-{i}", "full_name": f"Insights/repo-{i}"} for i in range(100, 130)]
    ...
    @patch("insights.models.integration.GitHubIntegration.list_all_repositories")
    def test_list_cached_repositories_uses_cached_data_when_fresh(self, mock_list_all):
        ...
        assert repos == [{"id": 2, "name": "insights-js", "full_name": "Insights/insights-js"}]
        assert has_more is False
    ...
    def test_list_cached_repositories_surfaces_optional_fields_when_present(self, mock_list_all):
        cached_repositories = [
            ...
            {"id": 2, "name": "legacy", "full_name": "Insights/legacy"},
        ]
        integration = self.create_integration(
            {"installation_id": "INSTALL", "account": {"name": "Insights"}},
            {"access_token": "ACCESS_TOKEN"},
        ...
        assert repos[1] == {"id": 2, "name": "legacy", "full_name": "Insights/legacy"}
        mock_list_all.assert_not_called()
    ...
    def test_list_cached_repositories_populates_cache_on_miss(self, mock_list_all):
        ...
        integration.refresh_from_db()
        assert repos == [{"id": 1, "name": "insights", "full_name": "Insights/insights"}]
    ...
    def test_invalidate_github_repository_caches_for_installation_clears_team_and_personal_rows(self):
        from insights.models.user_integration import UserIntegration

        team_integration = self.create_integration(
            {"installation_id": "12345", "account": {"name": "Insights"}},
            {"access_token": "ACCESS_TOKEN"},
        )
    ...
    def test_list_cached_repositories_pages_with_full_cached_snapshot(self, mock_list_all):
        fetched_repositories = [{"id": i, "name": f"repo-{i}", "full_name": f"Insights/repo-{i}"} for i in range(650)]
        integration = self.create_integration(
            {"installation_id": "INSTALL", "account": {"name": "Insights"}},
            {"access_token": "ACCESS_TOKEN"},
        )
    ...
    @parameterized.expand(
        [
            ...
            ("casefold_matches_owner_prefix", "INSIGHTS", 10, 0, [1, 2, 3, 4], False),
            ("pagination_applies_after_filter", "insights", 1, 1, [2], True),
        ]
    )
    @patch("insights.models.integration.GitHubIntegration.list_all_repositories")
    def test_list_cached_repositories_filters_search_before_pagination(
        ...
        fetched_repositories = [
            {"id": 1, "name": "insights", "full_name": "Insights/insights"},
            {"id": 2, "name": "insights-js", "full_name": "Insights/insights-js"},
            {"id": 3, "name": "code", "full_name": "Insights/code"},
            {"id": 4, "name": "insights-python", "full_name": "Insights/insights-python"},
        ]
        integration = self.create_integration(
            {"installation_id": "INSTALL", "account": {"name": "Insights"}},
            {"access_token": "ACCESS_TOKEN"},
    ...
    @patch("insights.models.integration.GitHubIntegration.list_branches")
    @patch("insights.models.integration.GitHubIntegration.get_default_branch")
...
class TestGitHubIntegrationGhApiGet(BaseTest):
    def _create_integration(self) -> Integration:
        ...
        return Integration.objects.create(
            ...
            config={"installation_id": "INSTALL", "account": {"name": "Insights"}},
            sensitive_config={"access_token": "ACCESS_TOKEN"},
        )
    ...
    def test_returns_parsed_json_body(self, _mock_expired, mock_get):
        ...
        body = GitHubIntegration(integration)._gh_api_get("/repos/Insights/insights", endpoint="/repos/{owner}/{repo}")
        assert body == {"default_branch": "main"}

    @patch("insights.egress.transport.transport.requests.request")
    @patch("insights.egress.github.transport.consume_github_installation_sync", return_value=False)
    @patch("insights.models.integration.GitHubIntegration.access_token_expired", return_value=False)
    def test_batch_instance_is_shed_when_budget_denied(self, _mock_expired, _mock_consume, mock_request):
        ...
        with pytest.raises(GitHubEgressBudgetExhausted):
            github.api_request("GET", "/repos/Insights/insights", endpoint="/repos/{owner}/{repo}")
    ...
    def test_critical_default_proceeds_when_budget_denied(self, _mock_expired, _mock_consume, mock_request):
        ...
        response = GitHubIntegration(integration).api_request(
            "GET", "/repos/Insights/insights", endpoint="/repos/{owner}/{repo}"
        )
    ...
    def test_raises_rate_limit_error_on_secondary_limit(self, _mock_expired, mock_get):
        ...
        with pytest.raises(GitHubRateLimitError) as excinfo:
            GitHubIntegration(integration)._gh_api_get("/repos/Insights/insights", endpoint="/repos/{owner}/{repo}")
    ...
    @patch("insights.egress.transport.transport.requests.request")
    @patch("insights.models.integration.GitHubIntegration.refresh_access_token")
    @patch("insights.models.integration.GitHubIntegration.access_token_expired", return_value=False)
...
class TestGitHubIntegrationGraphQL(BaseTest):
    ...
    def test_retries_transient_server_error_and_returns_data(self, _mock_expired, mock_request):
        ...
        ok = self._graphql_response({"data": {"repository": {"name": "insights"}}})
        ...
        assert data == {"repository": {"name": "insights"}}
    ...
    def test_returns_partial_data_with_field_errors(self, _mock_expired, mock_request):
        ...
        mock_request.return_value = self._graphql_response(
            {"data": {"repository": {"name": "insights"}}, "errors": [{"message": "field-level error"}]}
        )
...
class TestDatabricksIntegrationModel(BaseTest):
    @patch("insights.models.integration.is_url_allowed", return_value=(True, None))
    ...
    @patch("insights.models.integration.is_url_allowed", return_value=(False, "Could not resolve host"))
...
class TestGoogleAdsIntegrationModel(BaseTest):
    ...
    @patch("insights.models.integration.requests.request")
...
class TestPinterestAdsIntegrationDisplayName(BaseTest):
    @parameterized.expand(
        [
            (
                "business",
                {"id": "1", "username": "13x6ppss87fecv1q790xh1orhyp9th", "business_name": "Insights Inc"},
                "Insights Inc",
            ),
            ("personal", {"id": "1", "username": "javierinsights", "business_name": ""}, "javierinsights"),
            # Older connections predate business_name being stored.
            ("legacy", {"id": "1", "username": "javierinsights"}, "javierinsights"),
        ]
    )
...
class TestRedditAdsIntegrationDisplayName(BaseTest):
    @parameterized.expand(
        [
            (
                "username",
                {"reddit_user_id": "t2_1tqubocxl4", "data.reddit_username": "javierinsights"},
                "javierinsights",
            ),
...
class TestSnowflakeIntegrationModel(BaseTest):
    ...
    @parameterized.expand(
        [
            ("missing_name", "", "myorg-myaccount", "insights_svc"),
            ("missing_account", "prod-snowflake", "", "insights_svc"),
...
class TestEmailIntegrationCrossTenantStaleVerification(BaseTest):
    ...
    def test_destroy_email_integration_deletes_ses_identity(self, mock_create_email_domain, mock_delete_identity):
        from insights.api.integration import IntegrationViewSet
...
class TestGitLabIntegrationSSRFProtection:
    ...
    @patch("insights.models.integration.requests.get")
    @patch("insights.models.integration.is_url_allowed")
    def test_get_uses_allow_redirects_false(self, mock_is_url_allowed, mock_get):
        ...
        from insights.models.integration import GitLabIntegration
    ...
    def test_get_validates_url_before_request(self, mock_is_url_allowed, mock_get):
        ...
        from insights.models.integration import GitLabIntegration, GitLabIntegrationError
