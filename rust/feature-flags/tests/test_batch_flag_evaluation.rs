...
async fn test_version_mismatch_returns_conflict_with_actual_version() -> Result<()> {
    ...
    sqlx::query("UPDATE insights_featureflag SET version = 3 WHERE id = $1")
        .bind(flag.id)
        .execute(&mut *conn)
        .await?;
...
async fn test_datetime_filter_uses_team_timezone_not_utc() -> Result<()> {
    ...
    // same as live `/flags` evaluation and InsightsQL/Datastore cohort membership. The handler
...
async fn test_person_with_no_distinct_ids_is_skipped() -> Result<()> {
    ...
    // A person with no insights_persondistinctid rows at all.
...
async fn test_distinct_id_choice_is_deterministic_min_id() -> Result<()> {
    // Evaluation uses the distinct_id with the lowest insights_persondistinctid.id.
    ...
    let person_id = context
        ...
        .await?;
    // Second distinct_id gets a strictly higher insights_persondistinctid.id.
    let mut conn = context.get_persons_connection().await?;
    sqlx::query(
...
async fn test_static_and_dynamic_cohort_conditions() -> Result<()> {
    ...
    // with live-evaluation semantics: static cohorts resolve via insights_cohortpeople,
