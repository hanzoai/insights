...
1. **Temporal worker up** — the dev-stack (phrocs) process `temporal-worker` must be `running`
   (phrocs MCP `get_process_status`, or the `insightscli` TUI). It hot-reloads via nodemon on any
...
```bash
flox activate -- bash -c "DJANGO_SETTINGS_MODULE=insights.settings python manage.py \
...
```bash
docker exec insights-temporal-1 tctl --address temporal:7233 --ns default workflow show \
...
5. **Parallel perspective review** — `review_chunks` runs **three independent specialist perspectives
   ...
   **pull** — the prompt instructs the sandbox agent to `skill-get(review-script-perspective-…, version=N)` over
...
8. **Validate** — the `ValidateIssuesWorkflow` child groups the survivors by chunk and fans out **one warm
   ...
   **Resume-aware:** `load_run_validations` splits the chunk's issues into already-judged / pending, so a retry
   ...
   agent to `skill-get` the team's `review-script-validation-criteria` skill (version pinned by
...
`insights/insights` _sets_ the knobs and the `@hanzo/agent` package _applies_ them. A pinned run is three values —
...
here + `reasoning-effort.ts` / the gateway model list in `@hanzo/agent`) or startup validation rejects it on one side.
...
- `issue_validation/prompt.jinja` — validate one issue against the live codebase; "DO NOT implement fixes,
  ...
  `skill-get(review-script-validation-criteria, version=N)` over MCP and apply that team-owned bar for the keep/drop
  ...
  `products/review/skills/review-script-validation-criteria/SKILL.md`. → `IssueValidation`.

A separate **blind-spots** skill (`skills/review-script-blind-spots-general/SKILL.md`) drives the per-chunk sweep;
...
**Data model** (`products/review/backend/models.py`). Both models are **fail-closed team-scoped** (CLAUDE.md
...
- **Run a review:** `python manage.py run_review --pr-url <github_pr_url> --team-id <id> --user-id <id>`
  ...
  - **Default local-testing PR:** an **origin-branch** PR — its head branch must live on `Insights/insights`,
...
- **Lint:** `ruff check products/review/ --fix && ruff format products/review/`
...
- **GitHub auth** — the team's **GitHub App installation token**, resolved server-side per activity via
  ...
  needs one. All calls route through `insights.egress.github.transport.github_request` (via
...
- **Prod label trigger** (settings, `insights/settings/access.py`) — `REVIEWFN_TRIGGER_TOKEN` (shared secret),
...
(`review-script-reviews-{trigger,list,get}`, defined in `products/review/mcp/tools.yaml` and gated on the
