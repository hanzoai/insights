jest.mock('@hanzo/mcp-ui', () => ({ DescriptionList: () => null }), {
    virtual: true,
})
jest.mock('@hanzo/quill', () => ({ Button: () => null, Card: () => null, CardContent: () => null }), {
    virtual: true,
})
...
describe('LoopReviewView', () => {
    ...
    describe('describeRunBehavior', () => {
        it.each<[string, Pick<LoopReviewData, 'triggers' | 'enabled' | 'overlap_policy'>, string]>([
            ...
            [
                'github trigger without events',
                {
                    triggers: [
                        {
                            type: 'github',
                            config: { repository: 'insights/code' },
                        },
                    ],
                },
                'GitHub (insights/code) · skips overlapping runs',
            ],
            [
                ...
                'GitHub (insights/code: issues, pull_request) · skips overlapping runs',
            ],
            [
                ...
                'GitHub (insights/code: issues opened) · skips overlapping runs',
            ],
            [
            ...
            [
                ...
                'GitHub (insights/code: push on main) · skips overlapping runs',
            ],
            [
                ...
                'GitHub (insights/code: issues labeled bug) · skips overlapping runs',
            ],
            [
                ...
                'GitHub (insights/insights: pull_request review_requested where requested_team.slug is team-security or team-infra and pull_request.draft is false) · skips overlapping runs',
            ],
    ...
    describe('describeInsightsAccess', () => {
        it.each<[string, LoopReviewConnectors | undefined, string]>([
            ['missing connectors', undefined, 'Read-only'],
            ['read_only scope', { insights_mcp_scopes: 'read_only' }, 'Read-only'],
            ['full scope', { insights_mcp_scopes: 'full' }, 'Full (read-write)'],
        ])('%s', (_label, connectors, expected) => {
            expect(describeInsightsAccess(connectors)).toBe(expected)
        })
    })
})
