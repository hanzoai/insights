...
def _handle_help(
    ...
    *,
    ...
    # context the workflow needs, so omit it when the user discovered help via ``/insights``.
    lines = ["*Available commands:*\n"]
    if command_prefix == "@Insights":
    ...
    lines.extend(
        [
            ...
            f"`{command_prefix} project` — Show which Insights project your mentions route to in this workspace",
            f"`{command_prefix} project <id>` — Set the Insights project your mentions route to in this workspace",
        ]
    )
    ...
    *,
    ...
    if not result.candidates:
        slack.client.chat_postEphemeral(
            ...
            user=slack_user_id,
            thread_ts=thread_ts,
            text=("I couldn't find your Insights account in any organization connected to this Slack workspace."),
        )
        return
    ...
    slack.client.chat_postEphemeral(
        ...
        thread_ts=thread_ts,
        text=(
            "You haven't set a default project for this Slack workspace yet. Available Insights "
...
def dispatch_rules_command(
    ...
    *before* calling this dispatcher.
    ...
    error strings — ``@Insights`` for mentions, ``/insights`` for the slash command
