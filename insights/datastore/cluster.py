...
logger = dagster.get_dagster_logger("datastore")


def ON_CLUSTER_CLAUSE(on_cluster=True):
    return f"ON CLUSTER '{DATASTORE_CLUSTER}'" if on_cluster else ""
...
class DatastoreCluster:
    def __init__(
        ...
        cluster_hosts = self.__get_cluster_hosts(bootstrap_client, cluster or settings.DATASTORE_CLUSTER, retry_policy)
        ...
        for row in cluster_hosts:
            ...
            # service DNS name (DATASTORE_HOST).  Override the host from
            ...
            if not settings.CLOUD_DEPLOYMENT and host_name in ("localhost", "127.0.0.1"):
                host_name = settings.DATASTORE_HOST
    ...
    def __get_cluster_hosts(self, client: Client, cluster: str, retry_policy: RetryPolicy | None = None):
        ...
        # because the service DNS name may not resolve as local on the Datastore node.
                        ...
                        arrayZip(
                            arrayMap(
                                ...
                                arraySlice(  -- drop "ALTER TABLE" preamble line
                                    splitByChar('\n', formatQuery($__sql$ALTER TABLE {settings.DATASTORE_DATABASE}.{self.table} {", ".join(command_list)}$__sql$)),
                                    2
                                )
                            ) as commands,
