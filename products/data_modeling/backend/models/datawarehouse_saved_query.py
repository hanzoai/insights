...
from insights.models.utils import CreatedMetaFields, DeletedMetaFields, UpdatedMetaFields, UUIDTModel
from insights.schema_enums import DataWarehouseSavedQueryOrigin
...
class DataWarehouseSavedQuery(CreatedMetaFields, UUIDTModel, UpdatedMetaFields, DeletedMetaFields):
    ...
    def set_columns(self, columns: dict[str, Any]) -> None:
        ...
        Datastore output order), and both the jsonb payload and the ordered names are set here so
    ...
    def get_columns(self, user: Optional["User"] = None) -> dict[str, dict[str, Any]]:
        ...
        # process_query_dict requires a valid QuerySchemaRoot, so wrap as InsightsQLQuery.
        if "kind" not in query and "query" in query:
            query = {"kind": "InsightsQLQuery", **query}
    ...
    def get_s3_tables(self, database=None):
        ...
        context = InsightsQLContext(
            ...
            # Internal saved-query resolution (no user); bypass warehouse InsightsQL access control.
        ...
        resolved_node = resolve_types(node, context, dialect="datastore")
    ...
    def insightsql_definition(
        ...
        for column, type in reconstruct_ordered_columns(columns, self.column_order):
            ...
            if isinstance(type, str):
                ...
                fields[column] = LEGACY_DATASTORE_INSIGHTSQL_MAPPING[insightsql_type_str](name=column)
            elif isinstance(type, dict):
                fields[column] = STR_TO_INSIGHTSQL_MAPPING[type["insightsql"]](name=column)
            else:
                raise Exception(f"Unknown column type: {type}")  # Never reached
