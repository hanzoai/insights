...
jest.mock('lib/insights-typed', () => ({
    __esModule: true,
    default: { __loaded: true, capture: jest.fn() },
}))

const mockInsights = insights as unknown as { __loaded: boolean; capture: jest.Mock }
...
describe('useKeyboardHotkeys', () => {
    ...
    it('runs the action and captures keybind triggered on a match', () => {
        ...
        expect(mockInsights.capture).toHaveBeenCalledWith('keybind triggered', {
            keybind: 'k',
            mechanism: 'hotkey',
        })
    })
    ...
    it('does not capture when insights is uninitialized, as in the toolbar bundle on customer sites', () => {
        mockInsights.__loaded = false
        const action = jest.fn()
