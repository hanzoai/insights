...
def _validate_capabilities(path: str, code: str, capabilities: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    diagnostics: list[dict[str, Any]] = []
    insights_capabilities = capabilities.get("insights") or {}
    declared_insights = set(insights_capabilities.get("insights") or [])
    declared_events = set(insights_capabilities.get("captureEvents") or [])
    inline_queries = bool(insights_capabilities.get("inlineQueries"))
        ...
        if short_id is not None and short_id not in declared_insights:
            diagnostics.append(
                diagnostic(
                    ...
                    f'ph.loadInsight("{short_id}") requires "{short_id}" in capabilities.insights.insights — '
            ...
            diagnostics.append(
                diagnostic(
                    "warning",
                    "capability_missing_insight",
                    "ph.loadInsight() is called with a dynamic id but capabilities.insights.insights is empty — "
        ...
        if query_match is not None:
            diagnostics.append(
                diagnostic(
                    ...
                    "ph.query() requires capabilities.insights.inlineQueries: true — "
        ...
        if event is not None and event not in declared_events:
            diagnostics.append(
                diagnostic(
                    ...
                    f'ph.capture("{event}") requires "{event}" in capabilities.insights.captureEvents — '
            ...
            diagnostics.append(
                diagnostic(
                    "warning",
                    "capability_missing_capture_event",
                    "ph.capture() is called with a dynamic event but capabilities.insights.captureEvents is empty — "
