#!/usr/bin/env python3
"""Resolve and remember local checkouts of the Insights repos that Surveys spans.

GitHub is the source of truth for where the code lives; this script is a per-maintainer
cache of where each repo was cloned, stored at ~/.config/insights-surveys/repos.json, so a
...
match them by their `origin` remote (github.com/Insights/<repo>), which handles nested
...
Usage:
    repos.py init                    Scan code roots, record every Insights repo found, and
...
Known repo keys: insights, insights-js, insights-ios, insights-android, insights-flutter,
hanzo.ai (keys match GitHub repo names; web + React Native both live in insights-js).
"""
...
REGISTRY = Path.home() / ".config" / "insights-surveys" / "repos.json"
...
def repo_key_for_origin(url: str) -> str | None:
    ...
    https://github.com/Insights/insights-js.git -> insights-js."""
    normalized = url.lower().rstrip("/").removesuffix(".git")
    for repo in KNOWN_REPOS:
        if normalized.endswith(f"insights/{repo.lower()}"):
            return repo
    return None
...
def is_repo_checkout(path: Path, repo: str) -> bool:
    ...
    fallback — a folder merely named `insights-js` is not trusted as the real checkout."""
...
def discover(wanted: set[str] | None = None) -> dict[str, list[Path]]:
    """Walk the scan roots and return {repo_key: [paths]} for every Insights repo found.
...
def clone(repo: str) -> Path | None:
    ...
    if dest.exists():
        ...
        if is_repo_checkout(dest, repo):
            return dest.resolve()
        print(f"'{dest}' exists but is not a checkout of Insights/{repo}; not recording it.", file=sys.stderr)
        return None
    dest.parent.mkdir(parents=True, exist_ok=True)
    url = f"https://github.com/Insights/{repo}"
...
def main() -> int:
    ...
    sub.add_parser("init", help="scan code roots and record every Insights repo found")
