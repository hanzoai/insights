...
CREATE_METRICS_TIME_TO_SEE = (
    lambda: f"""
CREATE TABLE metrics_time_to_see_data ON CLUSTER '{DATASTORE_CLUSTER}' (
...
DROP_METRICS_TIME_TO_SEE_TABLE = lambda: f"DROP TABLE metrics_time_to_see_data ON CLUSTER '{DATASTORE_CLUSTER}' SYNC"

CREATE_KAFKA_METRICS_TIME_TO_SEE = (
    lambda: f"""
CREATE TABLE kafka_metrics_time_to_see_data ON CLUSTER '{DATASTORE_CLUSTER}' (
    `team_events_last_month` UInt64,
    `query_id` String,
    `team_id` UInt64,
    `user_id` UInt64,
...
DROP_KAFKA_METRICS_TIME_TO_SEE = (
    lambda: f"DROP TABLE kafka_metrics_time_to_see_data ON CLUSTER '{DATASTORE_CLUSTER}' SYNC"
)

CREATE_METRICS_TIME_TO_SEE_MV = (
    lambda: f"""
CREATE MATERIALIZED VIEW metrics_time_to_see_data_mv ON CLUSTER '{DATASTORE_CLUSTER}'
TO {DATASTORE_DATABASE}.metrics_time_to_see_data
AS SELECT
...
_offset,
_partition
FROM {DATASTORE_DATABASE}.kafka_metrics_time_to_see_data
"""
)
DROP_METRICS_TIME_TO_SEE_MV = lambda: f"DROP TABLE metrics_time_to_see_data_mv ON CLUSTER '{DATASTORE_CLUSTER}' SYNC"
...
CREATE_METRICS_QUERY_LOG = (
    lambda: f"""
CREATE TABLE metrics_query_log ON CLUSTER '{DATASTORE_CLUSTER}'
(
    `host` String,
    `timestamp` DateTime,
...
CREATE_METRICS_QUERY_LOG_MV = (
    lambda: f"""
CREATE MATERIALIZED VIEW metrics_query_log_mv ON CLUSTER '{DATASTORE_CLUSTER}'
...
DROP_METRICS_QUERY_LOG = lambda: f"DROP TABLE metrics_query_log ON CLUSTER '{DATASTORE_CLUSTER}' SYNC"
DROP_METRICS_QUERY_LOG_MV = lambda: f"DROP TABLE metrics_query_log_mv ON CLUSTER '{DATASTORE_CLUSTER}' SYNC"
