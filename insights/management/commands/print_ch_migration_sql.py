...
"""Render the SQL operations of one or more Datastore migrations.

Unlike ``migrate_datastore --plan --print-sql``, this command does not connect
to Datastore. It imports the requested migration modules, walks their
...
class Command(BaseCommand):
    help = (
        "Print rendered SQL for the given Datastore migration(s) under the current "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "migrations",
            nargs="+",
            help="Migration names or paths (e.g. 0247_foo or insights/datastore/migrations/0247_foo.py).",
        )
        parser.add_argument(
            "--format",
