...
/// addresses (e.g. insights-rs). Resolution model: when `instruction_addr` is
...
pub(crate) mod test_support {
    ...
    pub(crate) fn zip_fixture(elf: &[u8], source: Option<(&str, &str)>) -> Vec<u8> {
        ...
        insights_symbol_data::write_symbol_data(insights_symbol_data::ElfDebugInfo {
...
mod test {
    ...
    /// binaries uploaded via `insights-cli dsym upload` work without re-upload.
    #[sqlx::test(migrations = "./tests/test_migrations")]
    async fn test_native_resolves_apple_dsym_container(db: sqlx::PgPool) {
        ...
        let wrapped = insights_symbol_data::write_symbol_data(insights_symbol_data::AppleDsym {
    ...
    /// disambiguators (`[abc123]`). insights-rs normalizes its client-resolved
