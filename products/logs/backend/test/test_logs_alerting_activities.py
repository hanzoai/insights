...
from insights.test.base import APIBaseTest, DatastoreTestMixin, NonAtomicBaseTest
...
from insights.errors import QueryErrorCategory
...
class TestRunCohortQueryFallbackEndToEnd(DatastoreTestMixin, APIBaseTest):
    ...
    @patch("products.logs.backend.temporal.activities.increment_cohort_query_fallback")
    @patch("products.logs.backend.temporal.activities._run_batched_query")
    def test_fallback_runs_per_alert_queries_against_real_datastore(self, mock_batched, _mock_counter, mock_classify):
...
class TestEvaluateSingleAlert(APIBaseTest):
    def setUp(self):
        ...
        for target in (
            ...
            "products.logs.backend.temporal.activities.record_datastore_duration",
            "products.logs.backend.temporal.activities.record_cohort_save_duration",
            "products.logs.backend.temporal.activities.record_cohort_event_insert_duration",
            "products.logs.backend.temporal.activities.record_cohort_update_duration",
    ...
    def test_datastore_failure_creates_error_event_row(self, _mock_classify, mock_produce, mock_query_cls):
...
class TestEvaluateSingleAlertEndToEnd(DatastoreTestMixin, APIBaseTest):
    """End-to-end coverage of `_evaluate_single_alert` against real Datastore.
    ...
    @patch("products.alerts.backend.destinations.produce_internal_event")
    def test_breach_against_real_datastore_fires_alert(self, _mock_produce):
...
class TestEvaluateCohortBatchActivity(NonAtomicBaseTest):
    ...
    @patch("insights.slo.context.emit_slo_completed")
    @patch("insights.slo.context.emit_slo_started")
