# InsightsQL Type System TODO

This is a planning document for making InsightsQL's expression types strong enough to support optimizer work.
...
This document is about InsightsQL expression and SQL runtime types.
It is not about the Insights TypeScript/Python query schema type generation pipeline described in `docs/published/handbook/engineering/type-system.md`.

InsightsQL has multiple print targets.
Datastore is the highest-value optimization target today, but the type system should account for `datastore`, `postgres`, and `duckdb` output from the start.
...
- Added `insights/insightsql/type_system.py`, a structured runtime type model that keeps the existing resolver-facing `ConstantType` API while adding dialect-aware runtime metadata.
- Added adapters between current `ConstantType` objects, `DatabaseField` objects, and structured SQL runtime types.
- Added a structured Datastore type parser for wrappers and parametric types such as `Nullable`, `LowCardinality`, `Array`, `Tuple`, `Map`, `Decimal`, `DateTime64`, integer widths, float widths, and aggregate states.
...
- Added `insights/insightsql/type_diagnostics.py` with a typed-AST diagnostic helper and a function-catalog inventory helper that distinguishes missing legacy signatures from missing type inference.
- Added `insights/insightsql/transforms/type_aware_simplification.py` with an opt-in internal simplifier for conservative redundant casts and nullability wrappers.
- Added `InsightsQLContext.enable_type_aware_cast_simplification`, which keeps the simplifier disabled by default while letting internal callers exercise it.
- Added the `typeAwareCastSimplification` query modifier, which makes the simplifier reachable in production per team via `team.modifiers` without a deploy, still off by default, with `insightsql_type_simplification_total{dialect, kind}` counting removals during a pilot.
...
- Added focused tests in `insights/insightsql/test/test_type_system.py` for runtime type parsing, database-field adapters, algebra, resolver inference, set-query unification, diagnostics, and catalog inventory.
- Added `docs/internal/insightsql-type-system-now-possible.md`, which documents the new capabilities and the next optimizer hooks.
- Added `insights/insightsql/property_planner.py`, which combines semantic property types, physical property sources, source index metadata, restricted-property materialization rules, and comparison compatibility into optimizer-ready property comparison plans.
- Wired the Datastore materialized-column range rewrite through that property comparison plan, so the existing string minmax-friendly rewrite only uses direct physical-source comparisons when the planner says the source type matches the property semantics.
- Materialized-column introspection now carries the Datastore physical type reported by `system.columns`, so property planning can distinguish string-backed property materialization from typed physical sources.
...
- Added Datastore planner integration tests proving minmax skip-index usage for typed numeric and datetime materialized property comparisons.
...
- Added a `toTypeName(...)` companion-query builder and comparison helper so inferred select-expression families/nullability can be checked against Datastore type-name output.
...
- Expanded generic function inference for date/time part and formatting helpers, readable string helpers, Datastore bitmap helpers, vector distance/norm helpers, more map helpers, and explicit date-add/subtract helper families.
...
- Fixed Datastore overload selection to prefer transformed first-argument types over stale call metadata, so typed DateTime property expressions do not re-enter best-effort parsing.
...
- Full Datastore parity for every function signature and aggregate combinator.
- Full higher-order array parity beyond common lambda-first functions, especially strict lambda arity/return validation and less-used Datastore variants.
...
- Query-corpus unknown-type baselines and broader Datastore integration tests for inferred result types, nullability, timezone-sensitive behavior, and planner/index wins beyond the typed materialized-property cases already covered.
...
InsightsQL already has type annotations on AST nodes, but they are mostly good enough for field resolution, printing, nullability checks, and a few local transformations.
...
- `insights/insightsql/resolver.py` assigns `CallType` only after matching `func_meta.signatures`.
  Missing signatures fall back to `UnknownType`, and the stricter error path is commented out until the mappings are complete.
- `insights/insightsql/printer/test/test_printer.py` has tests showing missing function type inference makes comparisons need `ifNull(...)` wrapping unless the user writes `assumeNotNull(...)`.
- `insights/insightsql/transforms/property_types.py` manually attaches a `DateTimeType` return type to a generated `toDateTime(...)` call so an outer `toDateTime(...)` can avoid reparsing an already-datetime expression.
- `insights/insightsql/printer/datastore.py` has to look through aliases because a transform can rewrite an inner expression while the alias's declared type is stale.
- `insights/insightsql/test/test_property_skip_indexes.py` documents cases where typed property conversion makes a query execute correctly, but wrapping a materialized column in `toFloat(...)` or `toDateTime(...)` hides that column from minmax skip indexes.
...
All existing InsightsQL and SQL that works today should keep working by default.
...
- [x] Add compatibility tests that compile representative typed InsightsQL query shapes before optimizer work depends on them.
...
- `insights/insightsql/base.py`
- `insights/insightsql/ast.py`
- `insights/insightsql/database/models.py`
...
The structured runtime model now carries Datastore precision, scale, timezone, signedness, low cardinality, enum values, named tuple fields, aggregate states, nested nullability, and most first-slice parametric constructors.
The legacy `ConstantType` compatibility model now carries map key/value types, tuple field names, and aggregate-state wrappers, but it still omits most Datastore-specific precision details for optimizer decisions.
...
The resolver lives in `insights/insightsql/resolver.py`.
...
- Rewrites selected Insights-specific functions into lower-level AST.
- Assigns `type` metadata onto returned AST nodes.
- Handles dialect-specific behavior for Datastore and the Postgres-family targets (`postgres` and `duckdb`).
...
- Tests in `insights/insightsql/test/test_resolver.py` cover subquery type propagation, arithmetic, booleans, comparisons, selected function types, date truncation return types, and nullability overrides for `assumeNotNull`, `nullIf`, and `toNullable`.

The function boundary behavior has improved, but catalog coverage is still the main limitation.
For a Datastore function call, the resolver:

1. Visits all arguments.
2. Resolves each argument to a `ConstantType`.
3. Looks up the function in `INSIGHTSQL_DATASTORE_FUNCTIONS`.
...
The old signature matcher in `insights/insightsql/functions/core.py` is still class-based.
...
- `insights/insightsql/functions/mapping.py`
- `insights/insightsql/functions/aggregations.py`
- `insights/insightsql/functions/datastore/*.py`
- `insights/insightsql/functions/insights.py`
- `insights/insightsql/functions/udfs.py`

`InsightsQLFunctionMeta` currently records arity, aggregate status, Datastore name, overloads, suffix args, timezone behavior, placeholder rendering, parametric first-arg behavior, and optional signatures.

A rough source-level inventory on 2026-06-02 found:

- 881 `InsightsQLFunctionMeta(...)` constructor occurrences under `insights/insightsql/functions`.
...
- Some Insights extension functions have signatures.
...
- Specialized Datastore families that are not common optimizer inputs yet.
...
The skip-index tests in `insights/insightsql/test/test_property_skip_indexes.py` show why this matters.
A materialized property column can be directly usable by Datastore indexes.
...
Typed physical numeric and datetime materialized columns now have integration tests proving that direct range comparisons stay visible to Datastore minmax skip indexes.
...
- InsightsQL can print to Datastore, Postgres, DuckDB, and InsightsQL itself, so expression typing should not assume a single SQL backend.
- `BasePrinter._is_nullable()` uses type metadata to decide when comparisons need `ifNull(...)` wrappers.
- `DatastorePrinter` chooses function overloads based on the first argument type for selected functions.
...
- Data modeling uses Datastore result metadata and warehouse mappings to store saved query column types.
...
- [x] Represent Datastore primitive families with enough detail for the first optimizer work:
...
- [x] Represent a common cross-dialect type core plus dialect tags for Datastore, Postgres, and DuckDB.
...
- [x] Make type conversion from Datastore type strings structured instead of lossy string cleanup.
- [x] Add basic structured type adapters for Postgres and DuckDB metadata where InsightsQL prints or introspects those backends.
...
- A typed field can round-trip from database schema or Datastore `DESCRIBE` metadata into the type model without losing major type constructors.
- A typed field can carry enough dialect metadata to print correctly to Datastore, Postgres, and DuckDB.
...
- [x] Add numeric promotion rules matching common Datastore paths well enough for first optimizer decisions.
- [ ] Add string/date/datetime coercion rules for Insights-supported syntax by dialect.
...
- [x] Add comparison compatibility checks that can distinguish:
  ...
  - [x] incompatible
  - [x] unknown
- [ ] Keep Datastore-specific rules separate from Postgres/DuckDB rules.
...
- [ ] Support dialect-specific function signatures, names, and return types where Datastore, Postgres, and DuckDB diverge.
...
- [ ] Cover property and JSON functions:
  ...
  - [ ] Insights property extraction wrappers if any are introduced
...
- [ ] Cover Postgres and DuckDB functions that InsightsQL passes through or rewrites, especially casts, date/time functions, string functions, comparisons, and aggregations.
- [ ] Decide how UDFs should be typed:
  - [ ] typed manually
  - [ ] introspected from Datastore
...
- Cast typing remains backwards compatible with currently accepted Datastore, Postgres, and DuckDB casts.
...
Property type information is central to the optimization story because many InsightsQL queries are property-heavy.
...
- [x] Use the property comparison planner to guard the existing Datastore materialized string-column range rewrite.
...
- Typed materialized property comparisons can use skip indexes where Datastore can support them.
...
- [ ] Add a query-corpus job that compiles representative InsightsQL queries and reports unknown-type rates.
- [ ] Add a way to compare inferred types with Datastore result metadata.
...
- [x] Add resolver tests for:
  ...
  - [x] set queries
  - [x] typed properties
- [ ] Add dialect compatibility tests for Datastore, Postgres, and DuckDB print targets.
...
- [ ] Add Datastore integration tests for:
...
- At least one integration test proves an optimizer win against Datastore's planner, not just a string diff.
...
- [x] Add parser/adapter for Datastore type strings.
- [x] Add basic adapters for Postgres and DuckDB type metadata used by InsightsQL print targets.
...
- [ ] Type core Insights extension functions.
...
It should not become the default behavior for user-authored InsightsQL unless there is a separate compatibility review.
...
- [ ] How exact do we need to be compared with each backend?
      Exactness helps optimization but may require frequent updates as Datastore, Postgres, and DuckDB behavior changes.
- [ ] How should dialect-specific types be modeled?
      InsightsQL targets Datastore most heavily, but the resolver and printers also support Postgres and DuckDB syntax paths.
...
- [ ] Can Datastore function metadata be generated or validated from upstream docs/system tables, or should Insights curate only the subset it exposes?
...
- Do not stop existing InsightsQL/SQL from compiling by default.
- Do not enable strict mode for user-authored queries as part of the initial type-system work.
- Do not type every Datastore function before getting value from high-use functions.
...
- Do not make optimizer changes that depend on untrusted type facts without Datastore integration tests.
...
- [x] Add cross-dialect adapters for Datastore, Postgres, and DuckDB type metadata.
...
- `UnknownType` rate across representative InsightsQL queries goes down.
- Percentage of function calls with precise return types goes up.
- Query compilation overhead remains acceptable.
- Existing representative InsightsQL queries continue compiling and printing by default.
- Redundant casts and nullability wrappers decrease in emitted Datastore SQL.
- Type-aware rewrites are valid for the active print target, including Postgres and DuckDB where applicable.
- Typed property comparisons use skip indexes in cases where Datastore supports them.
- Fewer production bugs require local workarounds for stale aliases or lost function return types.
- Adding a new InsightsQL function requires declaring type behavior or explicitly documenting why it is unknown.

## Immediate Next Step

Phase 0's inventory and diagnostic hook now exists in `insights/insightsql/type_diagnostics.py`, the guarded simplifier exists in `insights/insightsql/transforms/type_aware_simplification.py`, property comparison planning metadata exists in `insights/insightsql/property_planner.py`, the existing materialized string-column range rewrite now consumes that planner, physically typed materialized property columns can now use direct numeric/datetime range comparisons, and Datastore planner tests now prove those shapes use minmax skip indexes.
