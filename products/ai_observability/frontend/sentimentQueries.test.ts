...
describe('sentimentQueries', () => {
    ...
    it('reads stored generation sentiment from ai_events first', async () => {
        jest.spyOn(mockApi, 'queryInsightsQL').mockResolvedValueOnce({
            results: [
                [
                    'trace-1',
                    'generation-uuid',
        ...
        const sentimentQuery = mockApi.queryInsightsQL.mock.calls[0][0]
        expect(sentimentQuery).toContain('FROM insights.ai_events AS ai_events')
