...
export const aiFirstHomepageLogic = kea<aiFirstHomepageLogicType>([
    ...
    listeners(({ actions, values }) => ({
        ...
        submitQuery: async ({ mode }, breakpoint) => {
            ...
            // The homepage chat only drives the legacy runtime, so on the new Insights AI surface a prompt
        ...
        startHandsFreeChat: () => {
            insights.capture('homepage hands-free started')
    ...
    actionToUrl(({ values }) => ({
        submitQuery: ({ mode: submittedMode }) => {
            ...
            // On the new Insights AI surface the homepage never owns an AI route: a submit with a prompt
    ...
    urlToAction(({ actions, values }) => ({
        [urls.projectHomepage()]: (_, searchParams, hashParams) => {
            // A `#panel=max:<prompt>` link (e.g. app.hanzo.ai/#panel=max:...) means "open Insights
            // AI prefilled". Here the homepage already IS the full-scene Insights AI, so consume the
