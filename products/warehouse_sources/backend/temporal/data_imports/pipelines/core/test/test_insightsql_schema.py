...
from products.warehouse_sources.backend.temporal.data_imports.pipelines.core.insightsql_schema import InsightsQLSchema
from products.warehouse_sources.backend.temporal.data_imports.pipelines.pipeline_sync import merge_columns


class TestInsightsQLSchemaJsonDetection:
...
class TestMergeColumnsJsonPreservation:
    @parameterized.expand(
        [
            (
                ...
                {"col": {"datastore": "String", "insightsql": "StringJSONDatabaseField"}},
                {"col": "StringDatabaseField"},
                "StringJSONDatabaseField",
            ),
            (
                "keeps_string_when_no_prior_json",
                {"col": {"datastore": "String", "insightsql": "StringDatabaseField"}},
                {"col": "StringDatabaseField"},
                "StringDatabaseField",
            ),
            (
    ...
    def test_merge_columns_json_type_handling(self, _name, existing_columns, table_schema_dict, expected_insightsql):
        db_columns = {"col": "String"}

        result = merge_columns(db_columns, table_schema_dict, existing_columns)

        assert result["col"]["insightsql"] == expected_insightsql
        assert result["col"]["datastore"] == "String"


class TestMergeColumnsRaceCondition:
    def test_merge_columns_preserves_existing_when_db_columns_incomplete(self):
        existing_columns = {
            "size_range": {"datastore": "String", "insightsql": "StringDatabaseField"},
        ...
        assert result["size_range"] == {"datastore": "String", "insightsql": "StringDatabaseField"}
    ...
    def test_merge_columns_updates_existing_column_types(self):
        ...
        assert result["user_id"] == {"datastore": "Int64", "insightsql": "IntegerDatabaseField"}

    def test_merge_columns_skips_column_when_insightsql_type_missing_from_schema(self):
        existing_columns = {
            "user_id": {"datastore": "String", "insightsql": "StringDatabaseField"},
        }
    ...
    def test_merge_columns_keeps_binary_column_registered_by_insightsql_schema(self):
        # A binary Arrow column (e.g. a BigQuery BYTES field) is read back by Datastore's
        # deltaLake() reader as String, so InsightsQLSchema must register a type for it or this
        ...
        result = merge_columns(db_columns, schema.to_insightsql_types(), {})

        assert result["key"] == {"datastore": "String", "insightsql": "StringDatabaseField"}
