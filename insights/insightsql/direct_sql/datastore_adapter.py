...
from insights.insightsql.direct_query_metrics import DIRECT_QUERY_ROW_CAP_EXCEEDED_TOTAL, observe_direct_query
from insights.insightsql.direct_sql.adapter import DirectQueryRequest, DirectQueryResult
from insights.insightsql.direct_sql.capability import is_direct_capable
...
if TYPE_CHECKING:
    ...
    from products.warehouse_sources.backend.temporal.data_imports.sources.datastore.source import DatastoreSource
    from products.warehouse_sources.backend.temporal.data_imports.sources.generated_configs.datastore import (
        DatastoreSourceConfig,
    )

DIRECT_DATASTORE_DEFAULT_STATEMENT_TIMEOUT_SECONDS = 600
# Hard backstop against loading an unbounded result set into memory — guards a raw passthrough
# `SELECT * FROM huge_table` with no LIMIT (the shared client runs with query_limit=0). InsightsQL-
# authored queries already carry a LIMIT from the printer.
DIRECT_DATASTORE_MAX_ROWS = 1_000_000
DIRECT_DATASTORE_ROW_CAP_ERROR = (
    f"Datastore query returned more than {DIRECT_DATASTORE_MAX_ROWS:,} rows. Add a LIMIT clause."
)
DIRECT_DATASTORE_TIMEOUT_ERROR = "Datastore query exceeded the execution time limit."
# Bounds the rows Datastore packs into one response block (the Datastore default). A block is
...
DIRECT_DATASTORE_MAX_BLOCK_SIZE = 65_536
RAW_DATASTORE_READ_ONLY_ERROR = "Raw Datastore queries must be read-only SELECT statements."
RAW_DATASTORE_SETTINGS_ERROR = "Raw Datastore queries may not set query-level SETTINGS."
RAW_DATASTORE_BLOCKED_FUNCTION_ERROR = "This Datastore table function is not allowed in direct queries."
...
# InsightsQL-authored queries never emit these; only the raw passthrough path can, so block them by
...
_RAW_DATASTORE_BLOCKED_TABLE_FUNCTIONS = frozenset(
    {
        "URL",
        "URLCLUSTER",
        "S3",
...
def datastore_error_to_message(error: Exception) -> str:
    message = str(error).strip()
    if not message:
        return "Datastore query failed."
    return message.splitlines()[0]


def _normalize_function_name(name: str) -> str:
    """Strip Datastore identifier quoting so a quoted callee matches the blocklist by its bare name."""
    name = name.strip()
...
def _has_settings_clause(statement: TokenList) -> bool:
    """Whether the statement carries a Datastore ``SETTINGS <name> = ...`` clause.
...
def ensure_read_only_raw_datastore_statement(sql: str) -> str:
    """Enforce read-only for raw Datastore SQL.

    Datastore has no read-only transaction switch we can set on the connection, so read-only is
    ...
    A first-keyword check is not enough: Datastore accepts ``WITH 1 AS x INSERT INTO t SELECT x``,
    ...
    (SSRF), read local files, or federate in other databases. InsightsQL-authored queries only ever emit
    ...
    A query-level ``SETTINGS`` clause is rejected as well: Datastore lets it override connection
    settings, so a raw query could otherwise reset ``max_execution_time`` or inflate
    ``max_block_size`` to defeat the timeout and per-block memory bounds. InsightsQL-authored queries
    ...
    if len(statements) != 1 or statements[0].get_type() != "SELECT":
        raise ExposedInsightsQLError(RAW_DATASTORE_READ_ONLY_ERROR)
    ...
    for function_name in _iter_function_names(statement):
        if function_name.upper() in _RAW_DATASTORE_BLOCKED_TABLE_FUNCTIONS:
            raise ExposedInsightsQLError(RAW_DATASTORE_BLOCKED_FUNCTION_ERROR)
    if _has_settings_clause(statement):
        raise ExposedInsightsQLError(RAW_DATASTORE_SETTINGS_ERROR)
    return sql


def _fetch_capped_datastore_rows(
    client: "DatastoreClient", sql: str, parameters: dict[str, Any] | None, deadline_seconds: float
    ...
    closing the client, which drops the connection so Datastore cancels the query) enforces the
    ...
    with client.query_row_block_stream(sql, parameters=parameters) as stream:
        ...
        for block in stream:
            if perf_counter() - started > deadline_seconds:
                raise ExposedInsightsQLError(DIRECT_DATASTORE_TIMEOUT_ERROR)
            rows.extend(block)
            if len(rows) > DIRECT_DATASTORE_MAX_ROWS:
                DIRECT_QUERY_ROW_CAP_EXCEEDED_TOTAL.labels(dialect="datastore").inc()
                raise ExposedInsightsQLError(DIRECT_DATASTORE_ROW_CAP_ERROR)
    return rows, column_names, column_types


class DatastoreAdapter:
    engine = "datastore"
    dialect: InsightsQLDialect | None = "datastore"

    def validate_source_config(
        self, source: "ExternalDataSource", team: "Team"
    ) -> tuple["DatastoreSource", "DatastoreSourceConfig"]:
        ...
        if not (is_direct_capable(source) and source.direct_engine == self.engine):
            raise ExposedInsightsQLError("Invalid direct Datastore connection.")

        source_type = (
            ExternalDataSourceType.DATASTORECLOUD
            if source.source_type == ExternalDataSourceType.DATASTORECLOUD
            else ExternalDataSourceType.DATASTORE
        )
        datastore_source = cast("DatastoreSource", SourceRegistry.get_source(source_type))
        config = datastore_source.parse_config(source.job_inputs or {})

        is_ssh_valid, ssh_valid_errors = datastore_source.ssh_tunnel_is_valid(config, team.pk)
        if not is_ssh_valid:
            raise ExposedInsightsQLError(ssh_valid_errors or "Invalid SSH tunnel configuration.")

        valid_host, host_errors = datastore_source.is_database_host_valid(
            config.host, team.pk, using_ssh_tunnel=config.ssh_tunnel.enabled if config.ssh_tunnel else False
        )
        if not valid_host:
            raise ExposedInsightsQLError(host_errors or "Invalid Datastore host.")

        return datastore_source, config
    ...
    def execute(self, request: DirectQueryRequest) -> DirectQueryResult:
        ...
        statement_timeout_seconds = max(
            settings.max_execution_time or DIRECT_DATASTORE_DEFAULT_STATEMENT_TIMEOUT_SECONDS, 1
        )
        ...
        try:
            with request.timings.measure("datastore_execute"), observe_direct_query("datastore"):
                # The SSH tunnel (if any) is opened for the life of the query; the native driver
                # connects to the external Datastore directly, never through Insights's cluster.
                with datastore_source.direct_query_client(
                    config,
                    request.team.pk,
                    ...
                    settings={
                        ...
                        "max_block_size": DIRECT_DATASTORE_MAX_BLOCK_SIZE,
                    },
                ) as client:
                    rows, column_names, column_types = _fetch_capped_datastore_rows(
                        client, request.sql, request.values or None, statement_timeout_seconds
                    )
        except (DatastoreError, OSError, ExposedInsightsQLError) as error:
            span.set_attribute("error_type", error.__class__.__name__)
            if request.debug:
                return DirectQueryResult(
                    results=[], types=[], print_columns=[], error=datastore_error_to_message(error)
                )
            raise ExposedInsightsQLError(datastore_error_to_message(error)) from error
