...
from insights.temporal.ai_observability.eval_reports.constants import COUNT_TRIGGER_QUERY_WIDTH
from insights.temporal.ai_observability.eval_reports.output_types import get_outcome_definition
from insights.temporal.ai_observability.eval_reports.targets import (
    GENERATION_TARGET,
...
async def fetch_due_eval_reports_activity(
    ...
    from insights.temporal.ai_observability.eval_reports.metrics import record_coordinator_reports_found
...
async def fetch_count_triggered_eval_report_candidates_activity(
    ...
    from insights.temporal.ai_observability.eval_reports.metrics import record_coordinator_check_count
...
async def check_count_triggered_eval_reports_activity(
    ...
    """Check a batch of count-triggered reports, sharing one Datastore query per team."""
...
def _fetch_count_triggered_eval_report_candidate_groups() -> list[list[str]]:
    ...
    COUNT_TRIGGER_QUERY_WIDTH wide, so one check activity runs exactly one Datastore
...
def _check_count_triggered_eval_reports_batch(
    ...
    """Check a group of count-triggered reports, sharing one Datastore count query per team.
    ...
    A Datastore failure propagates and fails the whole activity, which normally spans just
...
def _count_eval_results_for_report(report: "EvaluationReport", since: dt.datetime) -> int:
    ...
    # Pass the datetime directly to ast.Constant. InsightsQL's printer serializes it
    ...
    # nosemgrep: insightsql-fstring-audit (the predicate comes from fixed internal output-type definitions)
    query = parse_select(
        f"""
    ...
    with tags_context(product=Product.LLM_ANALYTICS, feature=Feature.ENRICHMENT, team_id=report.team_id):
        result = execute_insightsql_query(query=query, team=report.team, workload=Workload.OFFLINE)
...
def _count_eval_results_for_reports(
    ...
    """Count `$ai_evaluation` events for many reports in a single Datastore query.
    ...
    # `since` stays a datetime so InsightsQL prints toDateTime64(..., 6, <team_tz>) — a bare string
    ...
    select_columns: list[ast.Expr] = [
        # nosemgrep: insightsql-fstring-audit (the predicates come from fixed internal definitions)
        parse_expr(
    ...
    with tags_context(product=Product.LLM_ANALYTICS, feature=Feature.ENRICHMENT, team_id=team.pk):
        result = execute_insightsql_query(query=query, team=team, workload=Workload.OFFLINE)
...
def _find_nth_eval_timestamp(
    ...
    # Pass `before` as a datetime so InsightsQL serializes it as toDateTime64(..., 6, <team_tz>)
...
async def run_eval_report_agent_activity(
    ...
    async with Heartbeater():
        ...
        @database_sync_to_async(thread_sensitive=False)
        def run_agent():
            from insights.temporal.ai_observability.eval_reports.report_agent import run_eval_report_agent
...
async def store_report_run_activity(
    ...
    def store() -> str:
        ...
        from insights.temporal.ai_observability.eval_reports.report_agent.schema import (  # noqa: PLC0415 -- keeps report agent dependencies off the activity import path
        ...
        # Emit $ai_evaluation_report event to Datastore
...
async def deliver_report_activity(
    ...
    async with Heartbeater():
        ...
        @database_sync_to_async(thread_sensitive=False)
        def deliver():
            from insights.temporal.ai_observability.eval_reports.delivery import deliver_report
