...
interface InsightsConnectProps {
    constants: AppConstants
    mode: 'live' | 'test'
}

const InsightsConnect = ({ constants, mode }: InsightsConnectProps): JSX.Element => {
    const [connectionState, setConnectionState] = useState<ConnectionState>({
        status: 'loading',
    })

    const checkCredentials = useCallback((): void => {
        logger.debug('InsightsConnect: checking credentials...')
        loadCredentials(stripe)
            .then((creds) => {
                if (creds) {
                    logger.info('InsightsConnect: connected, region:', creds.region)
                    setConnectionState({ status: 'connected', region: creds.region })
                } else {
                    logger.info('InsightsConnect: not connected')
                    setConnectionState({ status: 'disconnected' })
                }
            })
            .catch((error) => {
                logger.info('InsightsConnect: error checking credentials:', error)
                setConnectionState({ status: 'disconnected' })
            })
    }, [])
        ...
        return (
            <Box css={{ stack: 'y', rowGap: 'medium' }}>
                ...
                <Banner
                    ...
                    description={
                        <Inline>
                            This app connects your Stripe data to Insights's Data warehouse for analysis.{' '}
                            <Link href="https://hanzo.ai/docs/cdp/sources/stripe" target="_blank">
                                Learn more
                            </Link>
                        </Inline>
                    }
    ...
    return (
        ...
        >
            <Banner
                type="default"
                title="Dev mode"
                description="Paste values from `python manage.py generate_stripe_app_tokens --team-id 1` to connect to Insights in development."
            />
