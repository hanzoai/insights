...
class TestInsightsCodeEventHandler(SimpleTestCase):
    def setUp(self):
        self.client = APIClient()
        self.signing_secret = "insights-code-test-secret"
    ...
    @patch("products.slack_app.backend.api.route_insights_code_event_to_relevant_region")
    @patch("products.slack_app.backend.api.SlackIntegration.slack_config")
    ...
    def test_link_shared_logs_unfurl_context(self, mock_config, mock_route):
        ...
        payload = {
            ...
            "event": {
                ...
                "user": "U123",
                "links": [
                    {"url": "https://us.hanzo.ai/project/2/insights/abc123"},
                    {"url": "https://us.hanzo.ai/project/2/dashboard/456"},
                    {"url": "https://example.com/project/2/insights/not-insights"},
                ],
            },
        }
...
class TestRouteInsightsCodeEventToRelevantRegion(TestCase):
    def setUp(self):
        ...
        self.user.save()
        self.insights_code_integration = Integration.objects.create(
            team=self.team,
            kind="slack",
            integration_id="T12345",
            config={"scope": ",".join(sorted(REQUIRED_SLACK_SCOPES))},
            sensitive_config={"access_token": "xoxb-insights-code-test"},
        )
    ...
    def _seed_slack_user_cache(self, slack_user_id: str, email: str | None) -> SlackUserProfileCache:
        ...
        return SlackUserProfileCache.objects.create(
            integration=self.insights_code_integration,
    ...
    def test_local_match_starts_temporal_workflow(self, mock_sync_connect, mock_asyncio_run):
        request = self.factory.post("/slack/event-callback/", HTTP_HOST="us.hanzo.ai")

        from products.slack_app.backend.api import ROUTE_HANDLED_LOCALLY, route_insights_code_event_to_relevant_region

        result = route_insights_code_event_to_relevant_region(request, self.event, "T12345")
    ...
    def test_app_uninstalled_clears_workspace_profile_cache(
        ...
        headers = {"x-insights-region-proxied": "1"} if proxied else {}
        request = self.factory.post("/slack/event-callback/", HTTP_HOST="us.hanzo.ai", headers=headers)
        result = route_insights_code_event_to_relevant_region(request, {"type": "app_uninstalled"}, "T12345")

        assert result == ROUTE_HANDLED_LOCALLY
        assert not SlackUserProfileCache.objects.filter(integration=self.insights_code_integration).exists()
        assert SlackUserProfileCache.objects.filter(integration=other_integration).exists()
        assert Integration.objects.filter(id=self.insights_code_integration.id).exists()
    ...
    @parameterized.expand(
        [
            (
                ...
                True,
                "<@U_BOT> use insights/insights-js",
                ["insights/insights", "insights/insights-js"],
                False,
                True,
            ),
            (
                ...
                True,
                "<@U_BOT> use insights/unknown-repo",
                ["insights/insights", "insights/insights-js"],
                True,
                False,
            ),
    ...
    def test_explicit_repo_followup_handling(
        ...
        # The route runs _missing_insights_code_slack_scopes against a real SlackIntegration
        ...
        from products.slack_app.backend.api import (
            ...
            route_insights_code_event_to_relevant_region,
        )

        if has_pending_picker:
            _set_pending_repo_picker(
                integration_id=self.insights_code_integration.id,
                channel="C001",
                thread_ts="1234.5678",
                slack_user_id="U123",
                user_id=self.user.id,
                workflow_id="insights-code-mention-T12345:pending",
                context_token="ctx-1",
        ...
        if expect_picker_resolution:
            mock_sync_connect.return_value.get_workflow_handle.assert_called_once_with(
                "insights-code-mention-T12345:pending"
            )
        ...
        if expect_new_workflow:
            if has_pending_picker:
                ...
                assert pending_picker["workflow_id"] == "insights-code-mention-T12345:pending"
            else:
    ...
    @patch("products.slack_app.backend.api.hanzo_insights.capture")
    @patch("products.slack_app.backend.api.asyncio.run")
    @patch("products.slack_app.backend.api.sync_connect")
    @override_settings(DEBUG=False, CLOUD_DEPLOYMENT="US")
    def test_app_mention_ignored_does_not_start_workflow(
        ...
        result = route_insights_code_event_to_relevant_region(request, ignored_event, "T12345")
    ...
    def test_app_mention_from_unknown_user_posts_in_thread_failure_reply(
        ...
        # A Slack user whose email doesn't map to any Insights ``User`` in any connected
        ...
        # user can spot a wrong-email mismatch against their Insights account.
        ...
        # The mention is still reported to analytics with ``insights_user_identified=False``
        ...
        assert captured["insights code slack mention received"]["insights_user_identified"] is False
    ...
    def test_unknown_user_in_unapproved_ext_shared_channel_replies_ephemerally(
        ...
        request = self.factory.post("/slack/event-callback/", HTTP_HOST="us.hanzo.ai")
        result = route_insights_code_event_to_relevant_region(request, self.event, "T12345", is_ext_shared_channel=True)
    ...
    def test_app_mention_filters_candidates_to_user_accessible_only(self, mock_sync_connect, mock_asyncio_run):
        # When the workspace spans two orgs and the resolved Insights user only
        ...
        other_integration = Integration.objects.create(
            ...
            kind="slack",
            integration_id="T12345",
            config=self.insights_code_integration.config,
            sensitive_config={"access_token": "xoxb-other"},
        )

        from products.slack_app.backend.api import route_insights_code_event_to_relevant_region

        request = self.factory.post("/slack/event-callback/", HTTP_HOST="us.hanzo.ai")
        route_insights_code_event_to_relevant_region(request, self.event, "T12345")
        ...
        assert workflow_inputs.integration_id == self.insights_code_integration.id
    ...
    def test_app_mention_passes_resolved_user_id_into_workflow_inputs(self, mock_sync_connect, mock_asyncio_run):
        # The routing layer must propagate the resolved Insights user id into the
    ...
    def test_command_workflow_receives_resolved_user_id(self, mock_sync_connect, mock_asyncio_run):
        ...
        route_insights_code_event_to_relevant_region(request, event, "T12345")
    ...
    def test_command_text_routes_to_command_workflow(self, mock_sync_connect, mock_asyncio_run):
        ...
        from insights.temporal.ai.slack_app.insights_code_slack_mention_command import (
        ...
        assert kicked_off == InsightsCodeSlackMentionCommandWorkflow.run
        mock_asyncio_run.assert_called_once()
    ...
    def test_command_workflow_receives_all_workspace_candidates(self, mock_sync_connect, mock_asyncio_run):
        ...
        assert set(workflow_inputs.integration_ids) == {
            self.insights_code_integration.id,
    ...
    @patch("products.slack_app.backend.api.handle_insights_link_unfurl")
    @override_settings(DEBUG=False, CLOUD_DEPLOYMENT="US")
    def test_link_shared_routes_to_unfurl(self, mock_unfurl):
        ...
        result = route_insights_code_event_to_relevant_region(request, link_shared_event, "T12345")

        assert result == ROUTE_HANDLED_LOCALLY
        mock_unfurl.assert_called_once()
        passed_integration = mock_unfurl.call_args[0][1]
        assert passed_integration.id == self.insights_code_integration.id
    ...
    def test_link_shared_works_with_only_notifications_integration(self, mock_unfurl):
        ...
        self.insights_code_integration.delete()
    ...
    def test_us_no_local_proxies_to_eu(self, mock_proxy):
        ...
        from products.slack_app.backend.api import ROUTE_PROXIED, route_insights_code_event_to_relevant_region

        result = route_insights_code_event_to_relevant_region(request, self.event, "T_UNKNOWN")

        assert result == ROUTE_PROXIED
        mock_proxy.assert_called_once()
        assert mock_proxy.call_args.args[1] == "eu.hanzo.ai"
    ...
    def test_proxy_failure_returns_proxy_failed(self, mock_proxy):
        ...
        from products.slack_app.backend.api import ROUTE_PROXY_FAILED, route_insights_code_event_to_relevant_region
    ...
    def test_no_local_with_loop_header_returns_no_integration(self, mock_sync_connect, mock_asyncio_run, mock_proxy):
        request = self.factory.post(
            "/slack/event-callback/",
            HTTP_HOST="us.hanzo.ai",
            headers={"x-insights-region-proxied": "1"},
        )

        from products.slack_app.backend.api import ROUTE_NO_INTEGRATION, route_insights_code_event_to_relevant_region
    ...
    def test_eu_local_match_with_us_lookup_true_proxies_to_us(
        ...
        mock_proxy.assert_called_once()
        assert mock_proxy.call_args.args[1] == "us.hanzo.ai"
        mock_sync_connect.assert_not_called()
        mock_asyncio_run.assert_not_called()
    ...
    def test_hosted_dev_does_not_cross_region_proxy(self, mock_sync_connect, mock_asyncio_run, mock_proxy, mock_lookup):
        # The hosted dev environment (app.dev.insights.dev) runs as a single region and must not
        # probe or proxy to us.hanzo.ai — it has no row in that workspace and the upstream
        # responds 403 to every such hit (regression for slack_app_region_proxy_non_success).
        request = self.factory.post("/slack/event-callback/", HTTP_HOST="app.dev.insights.dev")
    ...
    def test_app_mention_with_missing_scopes_posts_reauth_and_skips_workflow(
        ...
        if scope_value is None:
            self.insights_code_integration.config = {}
        else:
            self.insights_code_integration.config["scope"] = scope_value
        self.insights_code_integration.save(update_fields=["config"])
...
class TestChannelApprovalGate(TestCase):
    ...
    def setUp(self):
        ...
        # Routing now resolves the Slack user to a Insights user before any
...
class TestAssistantEvents(TestCase):
    ...
    def _route(self, event: dict):
        ...
        with override_settings(DEBUG=False):
            return route_insights_code_event_to_relevant_region(request, event, "T12345")
