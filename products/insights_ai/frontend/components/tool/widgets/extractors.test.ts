...
function toolMessage(
    ...
    return {
        ...
        rawToolName: 'mcp__insights__exec',
        rawInput: {},
        innerInput,
        rawOutput,
        content: [],
...
describe('mcp tool adapter extractors', () => {
    ...
    describe('extractDashboard', () => {
        it('reads id and the _insightsUrl enrichment from the REST payload', () => {
            const dashboard = extractDashboard(
                toolMessage({ id: 42, name: 'KPIs', _insightsUrl: 'https://us.hanzo.ai/project/1/dashboard/42' })
            )
            expect(dashboard).toEqual({
                id: 42,
                name: 'KPIs',
                url: 'https://us.hanzo.ai/project/1/dashboard/42',
            })
        })
    ...
    describe('extractRecordingFilters', () => {
        it('maps the query-wrapper output back to universal filters', () => {
            const filters = extractRecordingFilters(
                toolMessage({
                    query: {
                        ...
                        properties: [{ type: 'person', key: 'email', operator: 'icontains', value: 'insights' }],
                    },
                    results: [],
                    _insightsUrl: 'https://us.hanzo.ai/project/1/replay',
                })
            )
            ...
            expect(filters?.filter_group.values).toEqual([
                {
                    type: 'AND',
                    values: [{ type: 'person', key: 'email', operator: 'icontains', value: 'insights' }],
                },
            ])
        })
            ...
            (kind) => {
                const result = extractQueryResult(
                    toolMessage({
                        query: { kind, series: [] },
                        results: [],
                        _insightsUrl: 'https://us.hanzo.ai/insights/new',
                    })
                )
                expect(result?.content.query).toEqual({ kind, series: [] })
                expect(result?.url).toBe('https://us.hanzo.ai/insights/new')
            }
        )
