# ruff: noqa: T201, E402
"""InsightsQL log-corpus parser-parity diagnostic.

Pulls the last 7 days of InsightsQL queries run against production
Datastore by any team that has opted into AI data processing
...
1. **Auto-discover the Datastore database id** via
   `insightscli metabase:databases`, preferring an `OFFLINE` shard so the
...
2. **Download a redacted dump** via `insightscli metabase:query` to the
   gitignored `insights/insightsql/scripts/.local/insightsql_log_corpus.json`.
   The embedded SQL applies a chain of `replaceRegexpAll` redaction
   passes (emails, UUIDs, IPv4/6, Insights API tokens, plus
...
DEFAULT_DUMP = REPO_ROOT / "insights" / "insightsql" / "scripts" / ".local" / "insightsql_log_corpus.json"
...
_LOG_COMMENT_TAG = '{"source": "insightsql_log_corpus_diagnostic"}'

# Redaction passes over the InsightsQL text, applied in list order (each
...
_REDACTION_PASSES: list[tuple[str, str]] = [
    ...
    # UUID — case-insensitive (raw user InsightsQL needn't be lowercase).
    (
...
def _sql_str_literal(s: str) -> str:
    """Escape a string for a Datastore single-quoted literal — double
...
def _build_corpus_sql(limit: int, offset: int = 0) -> str:
    ...
    `ORDER BY` carries `insightsql` as a tiebreaker so pagination is a stable
    total order (occurrence count alone has ties)."""
    return f"""
SELECT
    {_build_redaction_expr()} AS insightsql,
...
WHERE event_time > now() - INTERVAL 7 DAY
  ...
  -- the corpus beyond Insights's own team 2 to any consenting team's
  -- real InsightsQL, while keeping queries from non-consenting teams out.
  AND JSONExtractString(log_comment, 'ai_data_processing_approved') = 'true'
  AND JSONExtractString(log_comment, 'query', 'kind') = 'InsightsQLQuery'
  AND length(JSONExtractString(log_comment, 'query', 'query')) > 0
GROUP BY insightsql
ORDER BY n_occurrences DESC, insightsql
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Datastore Metabase DB id. Default: auto-discover, preferring OFFLINE shards.",
    )
    p.add_argument(
        "--prefer-online",
        action="store_true",
    ...
    print(f"=== InsightsQL log-corpus diagnostic: oracle={args.oracle} candidate={args.candidate} ===")
    print()
    ...
    if args.skip_download:
        ...
        if args.database_id is None:
            print("Auto-discovering Datastore DB id …")
            try:
        ...
        try:
            ...
            print("ERROR: `insightscli metabase:query` timed out — narrow the scan with --limit, or retry")
            return 1

    # 2. Load.
    rows = load_corpus_rows(args.input, text_col="insightsql", count_col="n_occurrences")
    ...
    if failures:
        ...
        write_failures(out_path, failures, REPO_ROOT, title="insightsql_log_corpus_failures.sql")
        print()
