...
def client_from_pool():
    with patch("insights.datastore.client.execute.get_client_from_pool") as mock:
        client = MagicMock()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Product.LLM_ANALYTICS, "celery", "insights.tasks.usage_report", DatastoreUser.DEFAULT, DatastoreUser.DEFAULT),
        # InsightsQL's materialized-column lookups name their own user, and must keep it rather than
        # spending a slot of the concurrency budget sized for real queries.
        (Product.LLM_ANALYTICS, "temporal", "llma-eval-reports", DatastoreUser.INSIGHTSQL, DatastoreUser.INSIGHTSQL),
        (Product.LLM_ANALYTICS, "temporal", "llma-eval-reports", DatastoreUser.META, DatastoreUser.META),
    ],
)
...
def test_llm_analytics_queries_take_a_concurrency_slot(client_from_pool, llm_analytics_slots, product, expected_slots):
    # Asserted here rather than at the AI observability call sites because those reach Datastore
