...
class TestSelectRepositoryForMessage:
    ...
    async def test_explicit_mention_short_circuits(self):
        resolve, list_repos = _patch_candidates(MagicMock(), ["insights/insights", "insights/insights-js"])
        with resolve, list_repos:
            assert await _run("please fix insights/insights-js") == "insights/insights-js"
...
class TestCascadeSelectRepository:
    @pytest.mark.parametrize("single_repo_wins,expected", [(True, "insights/insights"), (False, None)])
    def test_lone_repo_only_taken_when_opted_in(self, single_repo_wins, expected):
        ...
        resolve, list_repos = _patch_candidates(MagicMock(), ["insights/insights"])
        with resolve, list_repos:
    ...
    def test_unsynced_cache_is_read_without_a_live_sync(self, team):
        ...
        Integration.objects.create(
            ...
            sensitive_config={},
            repository_cache=[{"full_name": "Insights/Insights", "name": "Insights", "id": 1}],
        )
