...
from insights.temporal.session_replay.summarization_sweep.activities import find_sessions_for_team_activity
from insights.temporal.session_replay.summarization_sweep.types import FindSessionsInput
...
async def test_find_sessions_no_recent_sessions(activity_environment, team):
    ...
    with patch(
        "insights.temporal.session_replay.summarization_sweep.activities.fetch_recent_session_ids",
        return_value=[],
    ):
        result = await activity_environment.run(
            find_sessions_for_team_activity,
...
async def test_find_sessions_filters_summarized(activity_environment, organization, team):
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "sweep-user@hanzo.ai", "pw", "Sweep")
    try:
        raw_ids = ["s1", "s2", "s3"]
        ...
        with (
            ...
            patch(
                "insights.temporal.session_replay.summarization_sweep.activities.filter_session_ids_with_events",
                return_value={"s2"},
            ),
        ):
            result = await activity_environment.run(
...
async def test_find_sessions_dispatches_all_unsummarized_candidates(activity_environment, organization, team):
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "sweep-user2@hanzo.ai", "pw", "Sweep")
    try:
...
async def test_find_sessions_passes_sample_rate_to_fetch(activity_environment, organization, team):
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "sample@hanzo.ai", "pw", "Sample")
...
async def test_find_sessions_handles_config_disabled_between_check_and_ch_query(activity_environment, team):
    ...
    # _load_team_user_and_sessions hits Datastore. Returns an empty FindSessionsResult.
    await sync_to_async(enable_signal_source)(team, enabled=False)
    ...
    with patch(
        "insights.temporal.session_replay.summarization_sweep.activities._is_team_summarization_allowed",
        return_value=True,
    ):
        result = await activity_environment.run(
            find_sessions_for_team_activity,
...
async def test_find_sessions_prefers_created_by_user(activity_environment, organization, team):
    ...
    first_user = await sync_to_async(User.objects.create_and_join)(organization, "first@hanzo.ai", "pw", "First")
    second_user = await sync_to_async(User.objects.create_and_join)(organization, "second@hanzo.ai", "pw", "Second")
    try:
...
async def test_find_sessions_falls_back_when_created_by_is_null(activity_environment, organization, team):
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "fallback@hanzo.ai", "pw", "Fallback")
    try:
        with (
            patch(
                "insights.temporal.session_replay.summarization_sweep.activities.fetch_recent_session_ids",
...
async def test_list_enabled_teams_filters_ai_consent(activity_environment, organization):
    from insights.temporal.session_replay.summarization_sweep.activities import list_enabled_teams_activity
...
async def test_find_sessions_skips_recordings_with_too_many_failures(activity_environment, organization, team):
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "skip@hanzo.ai", "pw", "Skip")
    try:
        ...
        with (
            ...
            patch(
                "insights.temporal.session_replay.summarization_sweep.activities._stuck_session_ids",
...
async def test_find_sessions_drops_sessions_without_events(
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "events@hanzo.ai", "pw", "Events")
    try:
...
async def test_find_sessions_skips_events_filter_when_all_summarized(activity_environment, organization, team):
    ...
    await sync_to_async(enable_signal_source)(team)
    user = await sync_to_async(User.objects.create_and_join)(organization, "skipfilter@hanzo.ai", "pw", "Skip")
    try:
        candidate_ids = ["s1", "s2"]
        with (
            patch(
...
async def test_filter_session_ids_with_events_returns_empty_for_empty_input():
    from insights.temporal.session_replay.summarization_sweep.session_candidates import filter_session_ids_with_events
...
async def test_stuck_session_ids_returns_empty_for_empty_input():
    from insights.temporal.session_replay.summarization_sweep.activities import _stuck_session_ids
...
async def test_stuck_session_ids_thresholds_failures():
    ...
    from insights.temporal.session_replay.summarization_sweep.activities import _stuck_session_ids
    from insights.temporal.session_replay.summarization_sweep.constants import STUCK_RASTERIZE_THRESHOLD
    ...
    with patch(
        "insights.temporal.session_replay.summarization_sweep.activities.get_async_client",
