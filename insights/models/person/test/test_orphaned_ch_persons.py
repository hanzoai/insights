...
from insights.models.person.deletion import find_orphaned_ch_persons, tombstone_orphaned_ch_persons
...
class TestOrphanedCHPersonRepair(DatastoreTestMixin, BaseTest):
    def _seed_ch_only_person(self, uuid: str, distinct_ids: list[str], version: int = 5) -> None:
        # Live in Datastore, never seeded into the persons DB (fake) — an orphan.
        create_person_in_ch(
            uuid=uuid,
