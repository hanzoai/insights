...
from insights.models.property import GroupTypeIndex, Property, PropertyGroup, PropertyValidationError
from insights.models.team.team import Team
from insights.queries.base import relative_date_parse_for_feature_flag_matching
...
# Datastore codes for "this literal can't be parsed as the column's type": 6 CANNOT_PARSE_TEXT,
...
# insights/errors.py but not user_safe, so they wrap to InternalCHQueryError, not Exposed.
_VALUE_PARSE_CH_ERROR_CODES = frozenset({6, 72})


@contextmanager
def unevaluable_filters_as_validation_errors() -> Iterator[None]:
    # Sizing runs caller-supplied condition filters through InsightsQL and Datastore. Shapes those
    ...
    # already names the offending property), and the Datastore cannot-parse-value codes above.
    # Caller-shaped ValueError is converted separately in the parse phase
    # (replace_proxy_properties), so a bare ValueError from InsightsQL internals or team config
...
def _build_person_query(team: Team, filter: Filter, return_count: bool = True, cursor: Optional[str] = None):
    """Build InsightsQL AST query to count or select distinct persons matching filters."""
...
def _build_group_query(
    ...
    """Build InsightsQL AST query to count or select distinct groups matching filters."""
    from insights.insightsql import ast
    from insights.insightsql.property import property_to_expr
