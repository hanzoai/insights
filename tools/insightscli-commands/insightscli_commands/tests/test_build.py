"""Tests for the insightscli build command."""
...
from insightscli_commands.build import TRIGGERS, _match_commands
...
class TestTriggerMatching:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("insights/schema_migrations/0001_foo.py", "build:schema"),
            ("insights/api/serializers.py", "build:openapi"),
            ("insights/api/dashboard/views.py", "build:openapi"),
            ("insights/scopes.py", "build:openapi"),
            ...
            ("insights/insightsql/grammar/InsightsQLParser.g4", "build:grammar"),
            ("insights/taxonomy/taxonomy.py", "build:taxonomy-json"),
            ("products/surveys/frontend/src/Survey.tsx", "build:products"),
            ("products/insights_ai/skills/foo/SKILL.md", "build:skills"),
    ...
    def test_multiple_files_trigger_multiple_commands(self) -> None:
        changed = {
            ...
            "insights/api/serializers.py",
            "insights/insightsql/grammar/InsightsQLParser.g4",
        }
...
class TestBuildCommand:
    ...
    @patch("insightscli_commands.build.changed_files")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("frontend/src/queries/schema/index.ts", "build:schema"),
            ("insights/taxonomy/foo.py", "build:taxonomy-json"),
        ],
    )
    @patch("insightscli_commands.build.subprocess.run")
    @patch("insightscli_commands.build.changed_files")
