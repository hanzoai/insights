...
from insights.temporal.ai_observability.eval_reports.workflow import (
...
async def test_generate_workflow_forwards_sentiment_output_type() -> None:
    ...
    with (
        patch(
            "insights.temporal.ai_observability.eval_reports.workflow.temporalio.workflow.execute_activity",
            new=fake_execute_activity,
        ),
        patch(
            "insights.temporal.ai_observability.eval_reports.workflow.temporalio.workflow.patched",
            return_value=False,
        ),
    ):
...
async def test_generate_workflow_does_not_emit_signal_without_metrics() -> None:
    ...
    with (
        ...
        patch(
            "insights.temporal.ai_observability.eval_reports.workflow.temporalio.workflow.start_child_workflow"
...
async def test_count_triggered_report_check_continues_after_activity_failure() -> None:
    ...
    async def fake_execute_activity(_activity, inputs, **_kwargs):
        ...
        if inputs.report_id == "failed":
            raise RuntimeError("datastore at capacity")
    ...
    with (
        ...
        patch("insights.temporal.ai_observability.eval_reports.workflow.record_coordinator_reports_found") as record,
        patch("insights.temporal.ai_observability.eval_reports.workflow.temporalio.workflow.logger") as logger,
    ):
...
async def test_batched_count_check_caps_concurrent_group_activities() -> None:
    ...
    # flight; losing it would fire every team's query at once — the exact Datastore
    ...
    with (
        ...
        patch("insights.temporal.ai_observability.eval_reports.workflow.COUNT_TRIGGER_MAX_CONCURRENT_CHECKS", 2),
        patch("insights.temporal.ai_observability.eval_reports.workflow.record_coordinator_reports_found"),
        patch("insights.temporal.ai_observability.eval_reports.workflow.temporalio.workflow.logger"),
    ):
