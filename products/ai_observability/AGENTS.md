...
(and to the query runners in `insights/insightsql_queries/ai/`).
...
- `insights/insightsql_queries/ai/trace_query_runner.py` (single trace read)
- `insights/insightsql_queries/ai/traces_query_runner.py` (list of traces)
...
`insights/insightsql_queries/ai/test/`. The type checker will not catch drift
here.
...
`ScopeBasePermission` (in `insights/permissions.py`) only resolves
