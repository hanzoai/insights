        ...
        fuse: [
            ...
            (
                ...
                return createFuse(haystack, {
                    keys: ['name', 'insightsName', 'recentLabel'],
                    ignoreLocation: true,
                })
            },
        ],
    ...
    listeners(({ values, actions, props, cache }) => ({
                ...
                if (cache.lastEmptyResultDedupeKey !== dedupeKey) {
                    ...
                    insights.capture('taxonomic filter empty result', {
                        surface: legacyTaxonomicSurface(
                            insights.getFeatureFlag(FEATURE_FLAGS.TAXONOMIC_FILTER_CATEGORY_DROPDOWN)
                        ),
        ...
        remoteItemsFetchFailedForQuery: ({ searchQuery }) => {
            ...
            if (cache.lastFetchFailedDedupeKey !== dedupeKey) {
                ...
                insights.capture('taxonomic filter fetch failed', {
                    surface: legacyTaxonomicSurface(
                        insights.getFeatureFlag(FEATURE_FLAGS.TAXONOMIC_FILTER_CATEGORY_DROPDOWN)
                    ),
                    groupType: props.listGroupType,
