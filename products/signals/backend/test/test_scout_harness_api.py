...
def _authenticate_as_scout(test: APIBaseTest, *, scopes: InsightsMcpScopes = "signals_scout") -> None:
...
class TestScoutHarnessEmissionReportsAPI(APIBaseTest):
    ...
    def test_deleted_report_is_treated_as_no_link(self) -> None:
        # Datastore soft-delete and Postgres status can drift, so the reverse lookup can resolve a
    ...
    def test_datastore_failure_degrades_to_null_links(self) -> None:
        # A transient CH/InsightsQL failure shouldn't 500 the whole page — each finding still
        # comes back, just with `report: null` instead of a resolved link.
        run = _make_run(self.team, emitted_finding_ids=["f-a"])
        _make_emission(self.team, run, finding_id="f-a")
        with patch(_FETCH_REPORT_IDS, side_effect=Exception("Datastore timeout")):
            response = self.client.get(self._url(str(run.id)))
        assert response.status_code == status.HTTP_200_OK
        assert response.json()[0]["report"] is None
    ...
    def test_report_lookup_query_compiles_without_alias_collision(self) -> None:
        ...
        # which exposes `metadata` as an `argMax(...)` alias. InsightsQL resolved the `metadata`
        ...
        # Run the real query (no mock) against an empty Datastore: it must compile and return
    ...
    def test_empty_run_returns_empty_and_skips_datastore(self) -> None:
        run = _make_run(self.team)
...
class TestScoutHarnessNotesAPI(APIBaseTest):
    ...
    def test_writes_require_skill_editor_rbac(self) -> None:
        ...
        from insights.rbac.user_access_control import AccessControlLevel, UserAccessControl
...
class TestAgentHarnessProjectProfileAPI(APIBaseTest):
    ...
    The build path (per-section table scans + a Datastore aggregation, plus a row write)
...
class TestScoutHarnessConfigAPI(APIBaseTest):
    ...
    def test_list_excludes_withheld_config(self) -> None:
        ...
        payload_path = "products.signals.backend.scout_harness.team_limits.hanzo_insights.get_feature_flag_payload"
        with patch(
            payload_path,
    ...
    def test_re_enabling_an_inactivity_paused_scout_emits_the_revert_event(self) -> None:
        ...
        with patch("products.signals.backend.scout_harness.serializers.hanzo_insights.capture") as capture:
            response = self.client.patch(self._detail_url(str(config.id)), data={"enabled": True}, format="json")
...
_METADATA_PAYLOAD_PATH = "products.signals.backend.scout_harness.team_limits.hanzo_insights.get_feature_flag_payload"
...
class TestScoutHarnessMembersAPI(APIBaseTest):
    ...
    def test_lists_project_members_with_resolved_github_login(self) -> None:
        ...
        User.objects.create_and_join(self.organization, "second@hanzo.ai", None, first_name="Sec")
        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_200_OK
        ...
        assert by_email["second@hanzo.ai"]["github_login"] is None

    @parameterized.expand(
        [
            # a single token matches an email / one name part …
            ("single_token", "alice", {"alice@hanzo.ai"}),
            ...
            ("full_display_name", "jane doe", {"jane@hanzo.ai"}),
        ]
    )
    def test_search_narrows_the_roster(self, _name: str, query: str, expected_emails: set[str]) -> None:
        ...
        User.objects.create_and_join(self.organization, "jane@hanzo.ai", None, first_name="Jane", last_name="Doe")
    ...
    def test_non_scout_auth_cannot_list_members(self, _name: str, scopes: InsightsMcpScopes | None) -> None:
