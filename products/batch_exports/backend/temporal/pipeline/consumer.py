...
class Consumer:
    ...
    def __init__(self, model: str = "events"):
        ...
        # Total rows expected for this run (from the staged Datastore count). When set, the
    ...
    def _maybe_log_progress(self) -> None:
        ...
        staged for this run (from Datastore), so it reflects how far through the export
...
async def run_consumer_from_stage(
    ...
    Arguments:
        ...
        transformer: The transformer used to convert record batches into their desired
            export format.
        records_total: Total rows staged for this run (from Datastore). When provided, the
