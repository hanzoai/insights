...
class NoPageleaveEventsCheck(HealthCheck):
    ...
    remediation = Remediation(
        human="""
            ...
            on a recent insights-js with pageview autocapture enabled, which emits $pageleave automatically
        ...
        agent="""
            ...
            in the user's codebase: locate the `insights.init` call and ensure pageview autocapture is
            enabled; if pageviews are captured manually (`capture_pageview: false`), add a matching
            `insights.capture` of `$pageleave` on route changes / unload. Use `docs-search` for the
