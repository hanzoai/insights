 ...
 * at `products/insights_ai/backend/wire_types.py`; keep that slice in sync when it grows.
 */
...
// --- `_insights/*` notification params (adapter-owned; keys are camelCase as on the wire) ---

export interface InsightsConsoleParams {
    sessionId?: string
    level?: string
    message?: string
}

export interface InsightsProgressParams {
    sessionId?: string
    step?: string
    status?: string
    label?: string
...
export interface InsightsSandboxOutputParams {
    sessionId?: string
    stdout?: string
    stderr?: string
    exitCode?: number
...
export interface InsightsUserMessageParams {
    content?: string | unknown[]
}

export interface InsightsUsageTokens {
    inputTokens?: number
    outputTokens?: number
...
export interface InsightsUsageBreakdown {
    systemPrompt?: number
    tools?: number
    rules?: number
    skills?: number
...
/**
 * The `_insights/usage_update` ext-notification. Permissive on purpose: the Codex adapter emits two
 ...
 */
export interface InsightsUsageUpdateParams {
    sessionId?: string
    used?: InsightsUsageTokens
    cost?: number | { amount?: number; currency?: string } | null
    breakdown?: InsightsUsageBreakdown
}

/**
 * The numeric `used`/`size` aggregate that drives the context-usage percentage ring. It does NOT
 * arrive on the `_insights/usage_update` ext-notification — it is a `session/update`-framed update
 ...
 * `session/update` (not a `_insights/*` ext-notification). Carries the full prompt text in a single
...
export interface InsightsStatusParams {
    sessionId?: string
    status?: string
    isComplete?: boolean
}

export interface InsightsCompactBoundaryParams {
    sessionId?: string
    trigger?: string
    preTokens?: number
    contextSize?: number
...
export interface InsightsTaskNotificationParams {
    sessionId?: string
    taskId?: string
    status?: string
    summary?: string
...
export interface InsightsErrorParams {
    message?: string
    classification?: string
}

export interface InsightsSdkSessionParams {
    taskRunId?: string
    sessionId?: string
    adapter?: string
}

export interface InsightsResourcesUsedParams {
    sessionId?: string
    products?: { id?: string; label?: string }[]
}

/** Permission lifecycle persisted to the run log — pending approvals are re-derived from these on bootstrap. */
export interface InsightsPermissionRequestParams {
    requestId?: string
    toolCallId?: string
    options?: PermissionOption[]
    toolCall?: Record<string, unknown>
...
export interface InsightsPermissionResolvedParams {
    requestId?: string
    toolCallId?: string
    optionId?: string
}

export interface InsightsRunStartedParams {
    sessionId?: string
    runId?: string
    taskId?: string
    agentVersion?: string
...
export interface InsightsTurnCompleteParams {
    sessionId?: string
    stopReason?: string
}

export interface InsightsNotificationParamsByMethod {
    '_insights/console': InsightsConsoleParams
    '_insights/progress': InsightsProgressParams
    '_insights/sandbox_output': InsightsSandboxOutputParams
    '_insights/user_message': InsightsUserMessageParams
    '_insights/usage_update': InsightsUsageUpdateParams
    '_insights/status': InsightsStatusParams
    '_insights/compact_boundary': InsightsCompactBoundaryParams
    '_insights/task_notification': InsightsTaskNotificationParams
    '_insights/error': InsightsErrorParams
    '_insights/sdk_session': InsightsSdkSessionParams
    '_insights/resources_used': InsightsResourcesUsedParams
    '_insights/permission_request': InsightsPermissionRequestParams
    '_insights/permission_resolved': InsightsPermissionResolvedParams
    '_insights/run_started': InsightsRunStartedParams
    '_insights/turn_complete': InsightsTurnCompleteParams
}

/**
 * Narrows a notification to a known `_insights/*` method and its params shape. Discriminant-only:
 * the params contents stay trusted per the module contract.
 */
export function isInsightsNotification<M extends keyof InsightsNotificationParamsByMethod>(
    notification: AcpNotification,
    method: M
): notification is AcpNotification & { method: M; params?: InsightsNotificationParamsByMethod[M] } {
    return notification.method === method
}

/** "Has field" checks rather than mutually exclusive — the Claude combined frame carries both. */
export function hasUsageUsed(params: InsightsUsageUpdateParams | undefined): boolean {
    return !!params && params.used != null
}

export function hasUsageBreakdown(params: InsightsUsageUpdateParams | undefined): boolean {
