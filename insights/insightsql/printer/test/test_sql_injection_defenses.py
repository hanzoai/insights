"""SQL-injection-defense regression suite for the InsightsQL printer.

Trust model: every AST string field that reaches printer output is either escape-wrapped (via `_print_identifier`, `_print_escaped_string`, or `context.add_value` parameterization) or interpolated verbatim against an allowlist enforced at both construction (`__post_init__`) and print time. Script-only nodes compile to ScriptVM bytecode and never reach a SQL printer; out of scope here.
...
def _print(
    self_: BaseTest, query: ast.SelectQuery | ast.SelectSetQuery, *, dialect: InsightsQLDialect = "datastore"
) -> tuple[str, InsightsQLContext]:
    """Resolve + print, return (sql, context). Inspect `context.values` for parameterized string content."""
    context = InsightsQLContext(team_id=self_.team.pk, enable_select_queries=True)
...
def _select_parse_resolve_mutate_print(
    ...
    context = InsightsQLContext(team_id=self_.team.pk, enable_select_queries=True)
    from insights.insightsql.printer.utils import prepare_ast_for_printing, print_prepared_ast

    prepared = prepare_ast_for_printing(parsed, context=context, dialect="datastore")
...
class TestEscapePreservationOnParserOutput(BaseTest):
    """Group 4: escape-wrapped paths preserve attacker-controlled content as escaped literals — `Constant.value` (str) lands in `context.values` (CH driver binds it), `Field.chain` identifiers backtick-wrap with embedded-backtick escape via `escape_datastore_identifier`."""
    ...
    def test_string_constant_with_control_chars_does_not_leak_into_sql(self):
        # Whatever subset of `\n` / `\t` / `\0` survives the InsightsQL parser must end up in `context.values`, not embedded in the emitted SQL where a raw newline / tab could terminate the statement.
    ...
    def test_identifier_with_percent_rejected(self):
        ...
        with self.assertRaises((QueryError, ExposedInsightsQLError)):
...
class TestParserPathProducesOnlyAllowlistedVerbatimValues(BaseTest):
    ...
    def test_left_outer_semi_join_rejected_by_all_paths(self):
        ...
        for backend in ("cpp-json", "rust-json", "rust-py"):
            with self.assertRaises((ValueError, ExposedInsightsQLError)):
