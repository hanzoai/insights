...
class TestNotebookAnalytics(BaseTest):
    ...
    def test_classify_request_source_api_key_captures_mcp_client_identity(self):
        # The MCP server forwards mcp_consumer (insights-code -> Insights Desktop), which is how
        # Insights Desktop is split from a customer's own MCP client.
        source, extra = classify_request_source(_fake_request(_FakeKeyAuth(), mcp_consumer="insights-code"))
        self.assertEqual(source, NotebookCreationSource.MCP)
        self.assertEqual(extra, {"api_key_type": "_FakeKeyAuth", "mcp_consumer": "insights-code"})
