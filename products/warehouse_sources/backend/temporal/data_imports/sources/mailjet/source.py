...
class MailJetSource(
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Enter your Mailjet API key and secret key to pull your Mailjet data into the Insights Data warehouse.
...
""",
            ...
            webhookSetupCaption=(
                "Insights registers one Mailjet event callback URL per event type using the API key above, "
                "covering sent, open, click, bounce, blocked, spam and unsub.\n\n"
                "Mailjet does not sign its deliveries, so Insights embeds generated basic-auth credentials in "
                "the callback URL it registers. Mailjet sends those back as an Authorization header on every "
                "delivery, and Insights rejects anything that doesn't match.\n\n"
