...
these typed contracts. The product runs its curated InsightsQL privately behind them;
...
# The product's rollout flag: gates the API surface (InsightsFeatureFlagPermission) and the CI-signals sweep.
...
class ExpectedWarehouseView:
    ...
    internals (avoids a dependency cycle). ``query`` is the InsightsQL SELECT body; ``fields`` maps column
    name -> a ``FieldOrTable`` instance, from which data_modeling derives the stored
    ``{"insightsql": <field class>, "datastore": <type>, "valid": True}`` metadata via its shared
...
class TeamCIHealthItem:
    ...
    # Owning team slug (CODEOWNERS handle minus '@Insights/'), or 'unowned' for unstamped spans.
    owner_team: str
