...
| `insights:read-data-schema`      | Discover custom event or person properties before grouping by them |
| `insights:generate-app-url`      | Build region- and project-qualified links back to the UI           |
...
1. Call `insights:read-data-schema` with `kind: "event_properties"` and
   `event_name: "$ai_generation"`.
2. For promising fields, call `insights:read-data-schema` with
...
Use `insights:generate-app-url` for links. Do not hardcode the host because the
