...
const InsightsQLMathType = z.literal('insightsql')
...
const AssistantPathCleaningFilter = z.object({
    alias: z
        .string()
        .describe(
            'A human-readable alias that replaces matched path patterns in the visualization. For example, `/user/:id/profile` to replace `/user/123/profile`. Uses Datastore `replaceRegexpAll` replacement syntax — use `\\\\1` for capture group back-references.'
        ),
    regex: z
        .string()
        .describe(
            'A Datastore regex pattern to match against path values. Matched paths will be replaced with the alias. For example, `\\/user\\/\\d+\\/profile` to match any user profile URL.'
        ),
})
...
const AssistantPathsFilter = z.object({
    ...
    includeEventTypes: z
        .array(PathType)
        .describe(
            'Which event types to include in the path analysis. Available values: `$pageview` - web page views. Path values are page URLs (from `$current_url`), with trailing slashes stripped. `$screen` - mobile screen views. Path values are screen names (from `$screen_name`). `custom_event` - custom events (any event not starting with `$`). Path values are event names. `insightsql` - custom InsightsQL expression defined in `pathsInsightsQLExpression`. Path values come from evaluating the expression. You can combine multiple types. If not specified, all events are included without type filtering.'
        )
        .optional(),
    localPathCleaningFilters: z
        .array(AssistantPathCleaningFilter)
        .describe(
            'Datastore regex-based rules to clean and normalize path values at the query level. Each rule applies `replaceRegexpAll(path, regex, alias)` in sequence. Useful for removing dynamic IDs or parameters from URLs.'
        )
        .default([])
        .optional(),
    ...
    pathsInsightsQLExpression: z
        .string()
        .describe(
            'A InsightsQL expression to use as the path item. Required when `insightsql` is included in `includeEventTypes`. For example, `properties.$current_url` to use the current URL as the path item.'
        )
        .optional(),
    startPoint: z
        .string()
...
export const GENERATED_TOOLS: Record<string, ReturnType<typeof createQueryWrapper<ZodObjectAny>>> = {
    'query-trends': createQueryWrapper({
        ...
        kind: 'TrendsQuery',
        uiResourceUri: 'ui://insights/query-results.html',
    ...
    'query-trends-actors': createQueryWrapper({
        ...
        uiResourceUri: 'ui://insights/insight-actors.html',
        outputFormat: 'optimized',
    }),
