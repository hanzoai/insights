...
class TestDmatIntegration(BaseTest):
    """Test that InsightsQL queries use dmat columns when available."""
...
class TestDmatExtractionConsistency(DatastoreTestMixin, APIBaseTest):
    ...
    def test_dmat_string_extraction_matches_json_for_all_property_types(self):
        ...
        as InsightsQL's property type wrappers (toFloat, toBool, toDateTime).
        """
        # Build test data
        ...
        result_json = execute_insightsql_query(
        ...
        result_dmat = execute_insightsql_query(
            f"SELECT {select_fields} FROM events WHERE event = 'dmat_event'",
            team=self.team,
        )

        assert result_dmat.datastore is not None
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA:
            assert "events_json" in result_dmat.datastore, result_dmat.datastore
            assert "dmat_" not in result_dmat.datastore, (
                f"New events schema should use JSON subcolumns, not dmat columns. SQL: {result_dmat.datastore}"
            )
        else:
            assert "dmat_" in result_dmat.datastore, f"Should use dmat columns. SQL: {result_dmat.datastore}"
