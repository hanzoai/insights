...
class TestMCPAnalyticsPresentation(_MCPAnalyticsTeamScopedTestMixin, APIBaseTest):
    ...
    def test_intent_clusters_recompute_starts_workflow_and_returns_computing(self) -> None:
        ...
        with (
            patch("insights.temporal.common.client.async_connect", new=AsyncMock(return_value=mock_client)),
...
class TestMCPSessionIntentEndpoint(_MCPAnalyticsTeamScopedTestMixin, APIBaseTest):
    ...
    def test_generates_and_persists_when_empty(self) -> None:
        ...
        # Mock the two primitives so the endpoint path runs without Datastore or a real LLM call.
        with (
            patch("hanzo_insights.feature_enabled", return_value=True),
...
class TestMCPSessionToolCallsEndpoint(_MCPAnalyticsTeamScopedTestMixin, DatastoreTestMixin, APIBaseTest):
