...
GROUP_TYPES_FETCH_FAILURES = Counter(
    "insights_group_types_fetch_failures",
...
GROUP_TYPES_REPLICA_EMPTY_DISCREPANCIES = Counter(
    "insights_group_types_replica_empty_discrepancies",
...
def _record_group_types_fetch_failure(*, operation: str, log_event: str, exc: BaseException, **log_fields: Any) -> None:
    ...
    """
    GROUP_TYPES_FETCH_FAILURES.labels(operation=operation, source="personinsights", error_type="db_error").inc()
...
def get_group_types_for_project(project_id: int, *, caller_tag: str | None = None) -> list[dict[str, Any]]:
    """Fetch group types from cache, falling back to personinsights, then stale cache, then empty list."""
...
def get_group_types_for_team(team_id: int, *, caller_tag: str | None = None) -> list[dict[str, Any]]:
    """Fetch group types for a team via personinsights."""

    def _fetch() -> list[dict[str, Any]]:
        # require_personinsights_client() must run inside personinsights_call so a missing client
        # (RuntimeError) is wrapped as DatabaseError and recovered like any fetch failure.
        return _fetch_group_types_for_team_via_personinsights(require_personinsights_client(), team_id)
...
def _fetch_group_types_for_projects_via_personinsights(
    ...
    for i in range(0, len(project_ids), settings.PERSONFN_BATCH_SIZE):
        resp = client.get_group_type_mappings_by_project_ids(
            GetGroupTypeMappingsByProjectIdsRequest(
                project_ids=project_ids[i : i + settings.PERSONFN_BATCH_SIZE],
...
def get_group_types_for_projects(
    ...
    """Batch fetch group types for multiple projects via personinsights, falling back to
    ...
    Raises GroupTypesUnavailable if personinsights is unavailable and any requested
...
def count_group_type_mappings_per_team(*, caller_tag: str | None = None) -> list[dict[str, int]]:
    """Count group type mappings per team via personinsights."""
    from insights.personinsights_client.proto import CountGroupTypeMappingsRequest
    ...
    def _fetch() -> list[dict[str, int]]:
        ...
        return [
            ...
            for c in require_personinsights_client().count_group_type_mappings(CountGroupTypeMappingsRequest()).counts
        ]
...
def get_group_type_mapping_instance(
    ...
    *,
    ...
    """Fetch a single GroupTypeMapping by (project_id, group_type_index) via personinsights.
    ...
    # Cache may be stale — bust it and retry once via personinsights.
...
def update_group_type_mapping_fields(
    ...
    *,
    ...
    """Update specific fields on a GroupTypeMapping via personinsights.
