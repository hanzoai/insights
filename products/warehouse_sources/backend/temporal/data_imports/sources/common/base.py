...
class _BaseSource(ABC, Generic[ConfigType]):
    ...
    # `True` for sources whose InsightsQL tables use a Insights-managed canonical schema
    ...
    has_managed_insightsql_schema: bool = False
    ...
    # from `SourceConfig.docsUrl` (the hanzo.ai docs page for the source).
    ...
    def get_documented_tables(self) -> list[dict[str, Any]]:
        """Credential-free table catalog for public documentation (hanzo.ai).
...
class WebhookSource(_BaseSource[ConfigType], Generic[ConfigType]):
    ...
    def webhook_inputs_updated(
        ...
        """Called when webhook inputs have been set on the underlying script function.
        ``api_version`` follows the `create_webhook` contract.
    ...
    def webhook_resource_map(self) -> dict[str, str]:
        """The schema mapping to use to be stored on the InsightsFunction for matching incoming webhooks with tables.
    ...
    def webhook_template_inputs(self, config: ConfigType) -> dict[str, Any]:
        """Extra static `inputs` to persist on the webhook InsightsFunction beyond `schema_mapping` and
