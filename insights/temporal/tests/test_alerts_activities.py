...
from insights.slo.types import SloOperation, SloOutcome
from insights.tasks.alerts.utils import AlertEvaluationResult
from insights.temporal.alerts.activities import cleanup_alert_checks, evaluate_alert, notify_alert, prepare_alert
from insights.temporal.alerts.types import (
...
async def alert_with_user(ateam, aorganization):
    ...
    def _create() -> tuple[AlertConfiguration, User]:
        user = User.objects.create_and_join(
            organization=aorganization, email=f"alerts-{uuid.uuid4().hex[:6]}@hanzo.ai", password=None
        )
...
class TestEvaluateAlert:
    ...
    async def test_evaluate_auto_disables_and_skips_error_tracking_on_extraction_error(self, alert_with_user) -> None:
        ...
        with (
            ...
            patch("insights.temporal.alerts.activities.capture_exception") as mock_capture,
            patch("insights.tasks.alerts.utils.send_notifications_for_disabled") as mock_notify,
        ):
            env = ActivityEnvironment()
    ...
    async def test_evaluate_reraises_ch_transient_error(self, alert) -> None:
        ...
        # Capacity errors (codes 202/439) surface as DatastoreAtCapacity, so that's what we simulate.
        with patch(
            "insights.temporal.alerts.activities.check_alert_for_insight",
            side_effect=DatastoreAtCapacity(),
        ):
            env = ActivityEnvironment()
...
class TestNotifyAlert:
    async def test_noop_when_not_firing(self, alert_with_user) -> None:
        ...
        with (
            patch("insights.tasks.alerts.utils.send_notifications_for_breaches") as mock_breaches,
            patch("insights.tasks.alerts.utils.send_notifications_for_errors") as mock_errors,
        ):
            env = ActivityEnvironment()
            await env.run(
                notify_alert,
    ...
    async def test_sends_breach_notifications_when_firing(self, alert_with_user) -> None:
        ...
        with (
            patch("insights.slo.events.hanzo_insights") as mock_slo_analytics,
            patch("products.alerts.backend.delivery_slo.get_instance_region", return_value="US"),
            patch(
                "insights.tasks.alerts.utils.send_notifications_for_breaches",
                return_value=["alice@hanzo.ai"],
            ) as mock_breaches,
            patch("insights.tasks.alerts.utils.send_notifications_for_errors") as mock_errors,
        ):
            env = ActivityEnvironment()
        ...
        assert refreshed.targets_notified == {"users": ["alice@hanzo.ai"]}
    ...
    async def test_firing_dispatches_realtime_notification(self, alert_with_user) -> None:
        ...
        with (
            ...
            patch("insights.temporal.alerts.activities.create_notification") as mock_create_notification,
        ):
            env = ActivityEnvironment()
            await env.run(
                notify_alert,
    ...
    async def test_realtime_failure_does_not_break_email_path(self, alert_with_user) -> None:
        ...
        with (
            ...
            patch(
                "insights.temporal.alerts.activities.create_notification",
