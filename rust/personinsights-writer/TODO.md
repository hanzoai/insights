# personinsights-writer follow-ups
...
Add input validation/sanitization in personinsights-leader before producing to
`personinsights_updates`:
...
1. Switch `PG_TARGET_TABLE` from `personinsights_person_tmp` to `insights_person`
2. Drop `personinsights_person_tmp` table
3. Remove `personinsights_person_tmp` from the table allowlist
