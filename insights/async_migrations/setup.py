...
ASYNC_MIGRATIONS_MODULE_PATH = "insights.async_migrations.migrations"
ASYNC_MIGRATIONS_EXAMPLE_MODULE_PATH = "insights.async_migrations.examples"
...
def setup_async_migrations(ignore_insights_version: bool = False):
    ...
    3. Check if all migrations necessary for this Insights version have completed (else don't start)
    ...
    for migration_name, migration in ALL_ASYNC_MIGRATIONS.items():
        ...
        if (
            ...
            and (FROZEN_POSTFN_VERSION > Version(migration.insights_max_version))
            and migration.is_required()
        ):
            raise ImproperlyConfigured(
                f"Migration {migration_name} is required for Insights versions above {FROZEN_POSTFN_VERSION}."
            )
...
def kickstart_migration_if_possible(migration_name: str, applied_migrations: set):
    ...
    from insights.async_migrations.runner import run_next_migration
