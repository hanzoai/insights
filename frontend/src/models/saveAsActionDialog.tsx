...
export function openSaveAsActionDialog({ suggestedName, step, createInFolder }: OpenSaveAsActionDialogPayload): void {
    ...
    Dialog.openForm({
        ...
        content: (
            <Field name="actionName" label="Action name">
                <Input data-attr="save-as-action-name" placeholder="e.g., Clicked signup button" autoFocus />
            </Field>
        ),
        onSubmit: async ({ actionName }) => {
            try {
                ...
                insights.captureException(error, { action: 'save-as-action' })
                toast.error(getServerErrorMessage(error) ?? 'Failed to create action. Please try again.')
            }
        },
    })
}
