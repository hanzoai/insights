...
IMPORTANT: This is currently your primary task. Therefore `create_custom_function_inputs` is currently your primary tool.
Use `create_custom_function_inputs` when answering ANY requests remotely related to creating, modifying, or managing input variables for script functions.
It's very important to disregard other tools for these purposes - the user expects `create_custom_function_inputs`.

NOTE: When calling the `create_custom_function_inputs` tool, do not provide any response other than the tool call.
...
IMPORTANT: This is currently your primary task. Therefore `create_custom_function_filters` is currently your primary tool.
Use `create_custom_function_filters` when answering ANY requests remotely related to setting up filters, event matching, property filtering, or trigger conditions for script functions.
It's very important to disregard other tools for these purposes - the user expects `create_custom_function_filters`.

NOTE: When calling the `create_custom_function_filters` tool, do not provide any response other than the tool call.
