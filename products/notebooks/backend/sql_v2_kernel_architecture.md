...
2. make HTTP requests to the Insights backend to fetch Datastore data (the data plane),
...
| `POST /page`           | sync  | one result page — bounded, no callback. A insightsql run re-queries the data plane with the run’s code + LIMIT/OFFSET; a kernel run (python/duckdb) sends `result_id` and slices its `/data/results/<result_id>.arrow` frame in the server process                             |
...
{
  "run_id": "…",
  "node": { "type": "insightsql | duckdb | python", "code": "…", "output_name": "df3" },
  "inputs": [
    { "name": "df1", "kind": "insightsql", "query": "<CTE-resolved InsightsQL>", "query_hash": "sha256…" },
    { "name": "df2", "kind": "local" }
  ],
  "callback": { "url": "…", "token": "…" },
...
The backend resolves upstream InsightsQL references into `inputs` before dispatch (it owns the notebook document); the sandbox never parses InsightsQL. `kind: local` asserts the frame already exists in the kernel (made by an earlier Python/DuckDB node); the server fails the run cleanly if it doesn't.
...
- **Execution**: the endpoint validates the InsightsQL, then enqueues it on the **async query manager** — the same Celery-backed path insights and the SQL editor use — so no web worker ever waits on Datastore. The kernel polls the status route; the Celery query worker runs the InsightsQL layer (access controls apply) against Datastore — or DuckLake for warehouse sources (Journey 6) — and the completed result returns as Arrow on the poll. Large materializations that outgrow the manager's Redis-resident results will need a streaming variant of this endpoint (direct execution, record batches into the response) — that's a later, additive change.
...
1. **Display page for a pure-InsightsQL node** — the server requests the query with `LIMIT/OFFSET` for the capped page, builds the envelope from the small Arrow payload, and **never involves the kernel**. Paging/sorting a displayed InsightsQL node is a fresh CH query every time (accepted trade-off from the walkthrough).
...
1. **Registers inputs.** For each `insightsql` input: `pyarrow.ipc.open_file(path)` (memory-mapped, zero-copy) and `duckdb.register(name, arrow_object)` — the frame is queryable in DuckDB without ever fully loading into RAM. For a **Python** node it additionally binds `name = <arrow>.to_pandas()` into the user namespace — this is the one step that materializes in RAM, matching the accepted trade-off ("pandas may need to materialize all in memory"); heavy reduction should happen in InsightsQL/DuckDB first.
...
- `node.type == insightsql` and **all** inputs are `insightsql` → the **direct lane** (`sql_v2_direct.py`): the backend enqueues the inlined query on the async query manager and the run never reaches the sandbox at all — no kernel required, results land on the run row via the result poll. This amends walkthrough decision 4 ("the sandbox drives execution"): the backend routes, and the sandbox drives only kernel-lane runs. The server's own insightsql handling (capped fetch, result cache, `/page` re-query) remains only for runs dispatched before the direct lane and is slated for removal.
- anything else (Python node, DuckDB node, or a InsightsQL node would go here if we ever allow InsightsQL over local frames — we don't; that's what DuckDB syntax is for) → materialize `insightsql` inputs to frame files, run in the kernel. Dispatch provisions the kernel itself when none is running (the run is the user's ask for compute; the panel is presentation, not a prerequisite).

### External connections

A SQL cell can target a direct-query data source (the SQL editor's connection selector, rendered in the notebook's database tree) instead of Insights's Datastore.
The cell persists `connectionId` / `sendRawQuery`, the run request carries them, and `NotebookNodeRun` stores them — they reach the query runner as the `InsightsQLQuery` fields of the same name, so the engine choice is entirely the runner's concern.

Consequences the lanes have to respect:

- A connection run always takes the **direct lane**. The sandbox only reaches Insights data, so a connection cell that reads a Python frame is rejected at dispatch rather than rerouted to DuckDB.
...
- **Raw mode carries no references at all.** The InsightsQL parser can't read the engine's dialect, so there is nothing to inline and nothing to bound in place: `apply_raw_page_bounds` wraps the query in an aliased derived table (Postgres and MySQL reject an unaliased one) and the engine sees the rest verbatim.
...
3. **Backend data-plane endpoint** (InsightsQL → Arrow stream) + server-side fetch. Pure-InsightsQL display nodes return real Datastore data — Journey 1 is real end to end, Journey 2 paging via re-query.
