...
export function execHogImmediate(
    ...
    try {
        execResult = exec(bytecode, {
            ...
            external: {
                regex: {
                    match: (regex, str) => createTrackedRE2(regex, undefined, 'script-exec:regex.match').test(str),
                    extract: (regex, str) => {
                        const match = createTrackedRE2(regex, undefined, 'script-exec:regex.extract').exec(str)
                        if (!match) {
                            return ''
                        }
