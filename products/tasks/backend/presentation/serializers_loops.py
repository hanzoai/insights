...
class LoopRepositoryEntrySerializer(serializers.Serializer):
    ...
    full_name = serializers.CharField(
        max_length=255, help_text="Repository in `organization/repo` format, e.g. `insights/insights`."
    )
...
class LoopWriteSerializer(serializers.Serializer):
    ...
    model = serializers.CharField(
        ...
        help_text=(
            ...
            "Leave blank to let Insights pick a sensible default at run time."
        ),
    )
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if runtime_adapter is not None and reasoning_effort is not None:
            # A blank model means "Insights picks at run time", so the effort is
