...
// (insights/api/organization_invite.py). Client-side limit mirrors the server cap so users
                ...
                <form onSubmit={onDelegateSubmit} className="flex flex-col gap-2" data-attr="onboarding-exit-modal">
                    ...
                    <div className="flex items-start justify-between gap-2 mt-1">
                        <p className="text-secondary text-xs m-0">
                            They'll be added as an admin so they can finish setting up Insights.
                        </p>
                        <p
                            className={
                ...
                <div className="hidden md:flex flex-col gap-2">
                    ...
                    <div className="rounded-lg border border-primary bg-surface-primary overflow-hidden">
                        <div className="flex items-center gap-2 px-3 py-2 border-b border-primary bg-surface-secondary">
                            <MailScript className="w-8 h-6 object-contain shrink-0" />
                            <span className="text-xs font-semibold">Insights invitation</span>
                        </div>
                        <div className="p-3 flex flex-col gap-2">
                            <p className="m-0 text-xs text-muted">To: {recipientDisplay}</p>
                            {/* Match the real email's H1 (see insights/templates/email/delegation_invite.html)
                                so the live preview reflects what the recipient will actually see. */}
                            <p className="m-0 text-sm font-semibold">
                                {senderName} asked you to finish setting up Insights for {orgName}
                            </p>
                            {hasMessage ? (
