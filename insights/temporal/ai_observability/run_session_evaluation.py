"""Session-level evaluation: fetch, payload caps, Script globals and judge transcript.
...
from insights.schema import DateRange, LLMTrace, SessionQuery

from insights.insightsql import ast
from insights.insightsql.constants import MAX_SELECT_TRACES_LIMIT_EXPORT
...
from insights.insightsql_queries.ai.session_query_runner import SessionQueryRunner
...
from insights.temporal.ai_observability.run_trace_evaluation import TRACE_EVENTS_LOOKBACK
...
_SESSION_SKIP_REASONING = {
    ...
    "session_too_long_to_judge": (
        "This session's transcript is too long to grade in one piece. A Script evaluation has no transcript limit, "
...
# forces Datastore to decompress the payload columns, and a runaway session id should be rejected
...
def build_session_hog_globals(
    ...
    trace-only `events` / `trace` globals exist for Script source saved before those two landed,
...
def _session_io_preview(traces: list[LLMTrace]) -> tuple[str, str]:
    """First input of the session's opening trace and last output of its closing one. The Script body
