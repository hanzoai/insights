...
from insights.datastore.client.connection import (
    ...
    get_pool,
    make_ch_pool,
    set_default_datastore_workload_type,
)
...
def test_connection_pool_creation_without_offline_cluster(settings):
    settings.DATASTORE_OFFLINE_CLUSTER_HOST = None
...
def test_connection_pool_creation_with_offline_cluster(settings):
    settings.DATASTORE_OFFLINE_CLUSTER_HOST = "ch-offline.example.com"
...
def test_connection_pool_creation_with_team_id(settings):
    settings.DATASTORE_PER_TEAM_SETTINGS = {}
    ...
    settings.DATASTORE_PER_TEAM_SETTINGS = {"2": {"host": "clicky", "user": "default"}}
    ...
    assert online_pool.connection_args["host"] == settings.DATASTORE_HOST
...
def reset_state():
    ...
    make_ch_pool.cache_clear()
    set_default_datastore_workload_type(Workload.ONLINE)
