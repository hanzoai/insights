...
from insights.insightsql.ast import AST
...
from insights.insightsql.database.models import StructDatabaseField
...
from insights.insightsql.printer.postgres_functions import (
...
# Prevents SQL injection via backtick-quoted identifiers in InsightsQL.
_SAFE_FUNCTION_NAME_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")


class PostgresPrinter(BasePrinter):
    DIALECT_NAME: ClassVar[InsightsQLDialect] = "postgres"
    ...
    def _apply_window_function_rewrites(
        ...
        # Postgres's native lag/lead already has the semantics we want; skip the Datastore-style rewrite.
        return identifier
    ...
    def _visit_set_operand(self, node: ast.SelectQuery | ast.SelectSetQuery) -> str:
        # Datastore accepts a bare per-branch LIMIT/ORDER BY inside a set operation; the
    ...
    def visit_call(self, node: ast.Call):
        ...
        if func_name == "count" and not args and not node.distinct:
            # Datastore's zero-arg count() is spelled count(*) everywhere else.
            args_str = "*"
        if node.distinct:
    ...
    def _get_function_renames(self) -> dict[str, str]:
        """Lowercased InsightsQL-name → target-name map for simple function renames. Overridable by subclasses."""
        return POSTGRES_FUNCTION_RENAMES_LOWER

    def _get_function_handlers(self) -> dict[str, Callable[[list[str]], str]]:
        """Lowercased InsightsQL-name → handler-callable map for functions that need custom rendering."""
    ...
    def visit_constant(self, node: ast.Constant):
        ...
        # through the InsightsQL string escape path would produce Datastore-style ``\'`` escape
    ...
    def _print_table_sql(self, table) -> str:
        ...
        if hasattr(table, "to_printed_postgres"):
            return table.to_printed_postgres(self.context)
        return table.to_printed_datastore(self.context)
    ...
    def _json_property_args(self, chain):
        ...
        # safely. Inlining them through ``_print_escaped_string`` would emit Datastore-style
    ...
    def visit_arithmetic_operation(self, node):
        if node.op == ast.ArithmeticOperationOp.Add:
            ...
            # `%(insightsql_val_0)s`) and errors on the incomplete placeholder. So modulo renders
