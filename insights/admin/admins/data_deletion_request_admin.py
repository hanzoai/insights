...
DATASTORE_TEAM_GROUP = "Datastore Team"
...
# Dagster Cloud deployment slug per Insights cloud environment. The admin runs on web pods, which
...
def dagster_run_url(run_id: str) -> str | None:
    ...
    if deployment:
        return f"https://insights.dagster.cloud/{deployment}/runs/{run_id}"
    return None
...
class DataDeletionRequestForm(forms.ModelForm):
    ...
    insightsql_predicate = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"rows": 4, "style": "font-family: monospace; width: 100%;"}),
        help_text="Optional InsightsQL boolean expression (validated against the events table). "
...
class DataDeletionRequestAdmin(admin.ModelAdmin):
    ...
    ordering = ("-created_at",)
    change_form_template = "admin/insights/datadeletionrequest/change_form.html"
    ...
    fieldsets = (
        ...
        (
            "Datastore stats",
            {
                ...
                "description": "Populated by executing a Datastore query. Not editable.",
            },
        ),
        (
            "Audit trail",
    ...
    def duplicate_requests(self, request, queryset):
        ...
        for original in queryset:
            original_url = request.build_absolute_uri(
                reverse("admin:insights_datadeletionrequest_change", args=[original.pk])
            )
    ...
    def rendered_count_query(self, obj: DataDeletionRequest) -> str:
        """Show the fully-substituted Datastore COUNT query operators can copy/paste."""
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.request_type == RequestType.PERSON_REMOVAL and (
            obj.events or obj.delete_all_events or obj.insightsql_predicate
        ):
            obj.events = []
            obj.delete_all_events = False
            obj.insightsql_predicate = ""
            messages.info(
                request,
                "Event filters cleared — person removal requests do not use events/insightsql_predicate.",
            )
        if obj.request_type != RequestType.PERSON_REMOVAL and (
            obj.person_uuids
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        if obj:
            ...
            extra_context["submit_url"] = reverse("admin:insights_datadeletionrequest_submit", args=[obj.pk])
            extra_context["can_approve"] = (
                obj.status == RequestStatus.PENDING and request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists()
            )
            extra_context["approve_url"] = reverse("admin:insights_datadeletionrequest_approve", args=[obj.pk])
            ...
            extra_context["revert_to_draft_url"] = reverse(
                "admin:insights_datadeletionrequest_revert_to_draft", args=[obj.pk]
            )
            extra_context["can_retry"] = (
                obj.status == RequestStatus.FAILED and request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists()
            )
            extra_context["retry_url"] = reverse("admin:insights_datadeletionrequest_retry", args=[obj.pk])
            extra_context["can_verify"] = request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists()
            extra_context["verify_url"] = reverse("admin:insights_datadeletionrequest_verify", args=[obj.pk])

            # Datastore stats are calculated from this page (works for any status).
            extra_context["fetch_stats_url"] = reverse("admin:insights_datadeletionrequest_fetch_stats", args=[obj.pk])
            extra_context["preview_stats_url"] = reverse(
                "admin:insights_datadeletionrequest_preview_stats", args=[obj.pk]
            )
            extra_context["is_datastore_team"] = request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists()
    ...
    def submit_view(self, request, object_id):
        obj = self.get_object(request, object_id)
        if not obj:
            return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_changelist"))

        if obj.status != RequestStatus.DRAFT:
            messages.error(request, "Only draft requests can be submitted.")
            return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_change", args=[obj.pk]))
        ...
        return TemplateResponse(request, "admin/insights/datadeletionrequest/submit.html", context)
    ...
    def _submit_for_approval(
        ...
        if requires_approval:
            messages.success(request, "Request submitted and is now pending Datastore Team approval.")
        else:
            messages.success(
                request,
    ...
    def fetch_stats_view(self, request, object_id):
        ...
        if obj.request_type == RequestType.PERSON_REMOVAL:
            # No Datastore query yet for person_removal — just count selectors.
        ...
        try:
            ...
            self.log_change(request, obj, "Fetched Datastore stats.")
    ...
    def preview_stats_view(self, request, object_id):
        """Datastore-Team-only ephemeral stats run.

        Runs the same Datastore queries as ``fetch_stats_view`` but does **not**
        ...
        if not request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists():
            messages.error(request, "Only Datastore Team members can preview stats.")
            return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_change", args=[obj.pk]))
    ...
    def approve_view(self, request, object_id):
        ...
        if not request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists():
            messages.error(request, "Only Datastore Team members can approve deletion requests.")
            return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_change", args=[obj.pk]))
        ...
        if request.method == "POST":
            ...
            if execution_mode not in ExecutionMode.values:
                ...
                return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_approve", args=[obj.pk]))
        ...
        return TemplateResponse(request, "admin/insights/datadeletionrequest/approve.html", context)
    ...
    def retry_view(self, request, object_id):
        ...
        if not request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists():
            messages.error(request, "Only Datastore Team members can retry deletion requests.")
            return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_change", args=[obj.pk]))
    ...
    def verify_view(self, request, object_id):
        ...
        if not request.user.groups.filter(name=DATASTORE_TEAM_GROUP).exists():
            messages.error(request, "Only Datastore Team members can verify deletion requests.")
            return HttpResponseRedirect(reverse("admin:insights_datadeletionrequest_change", args=[obj.pk]))
        ...
        if remaining:
            messages.warning(
                request,
                f"{remaining} matching row(s) still present in Datastore. "
