...
import { InsightsQLQueryResponse, NodeKind } from '~/queries/schema/schema-general'
...
// A warehouse table column as offered in the pickers: its name, InsightsQL type (shown as a tag), and
        ...
        warehouseTables: [
            [] as DataWarehouseTable[],
            {
                loadWarehouseTables: async (
                    ...
                    // Skip column serialization (expensive per-table InsightsQL work) — the picker only needs
        ...
        selectedTableColumns: [
            [] as WarehouseColumn[],
            {
                loadSelectedTableColumns: async ({
                    ...
                    try {
                        const tableName = table.insightsql_name || table.name
    ...
    listeners(({ actions, values, cache }) => ({
        ...
        submitCustomPropertyFormFailure: ({ error }) => {
            if ((error as { sourceStep?: boolean })?.sourceStep) {
                insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.submit' })
                // The definition was saved — refresh the table and keep the modal open for a retry.
                actions.loadDefinitions()
                toast.error('Property saved, but the sync configuration failed. Fix it and save again.')
                return
            }
            ...
            insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.submit' })
            toast.error('Failed to save custom property')
        },
        deleteDefinitionSuccess: () => {
        ...
        deleteDefinitionFailure: ({ errorObject }) => {
            ...
            insights.captureException(errorObject, { scope: 'customPropertyDefinitionsLogic.delete' })
            toast.error('Failed to delete custom property')
        },
        loadDefinitionsFailure: ({ error }) => {
            insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.load' })
        ...
        createWorkflowForPropertySuccess: ({ newWorkflowUrl }) => {
            if (newWorkflowUrl && window.open(newWorkflowUrl, '_blank')) {
                toast.success('Workflow editor opened in a new tab — save the workflow there, then refresh')
            } else {
                toast.error('Could not open a new tab — check your popup blocker')
            }
        },
        createWorkflowForPropertyFailure: ({ errorObject }) => {
            ...
            insights.captureException(errorObject, { scope: 'customPropertyDefinitionsLogic.createWorkflow' })
            toast.error('Failed to create workflow')
        },
        loadSavedQueriesFailure: ({ error }) => {
            insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.loadSavedQueries' })
            toast.error('Failed to load data warehouse views')
        },
        loadWarehouseTablesFailure: ({ error }) => {
            insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.loadWarehouseTables' })
            toast.error('Failed to load data warehouse tables')
        },
        triggerSync: async ({ sourceId }) => {
            ...
            try {
                ...
                toast.success('Sync triggered — it may take a few minutes to run')
                actions.loadDefinitions()
                actions.pollRunsStatus({ sourceId })
            } catch (error) {
                insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.triggerSync' })
                toast.error('Could not trigger a sync for this property')
            } finally {
        ...
        triggerBackfill: async ({ sourceId }) => {
            ...
            try {
                ...
                insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.triggerBackfill' })
                toast.error('Could not start a backfill for this property')
            } finally {
                actions.removeTriggeringSource({ sourceId })
            }
        },
        loadRuns: async ({ sourceId }) => {
            try {
                ...
                insights.captureException(error, { scope: 'customPropertyDefinitionsLogic.loadRuns' })
