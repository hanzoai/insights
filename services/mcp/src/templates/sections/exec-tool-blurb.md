...
Insights: dashboards, insights, funnels, SQL, experiments, surveys, replay, error tracking, flags.
...
insights:exec({ "command": "search <regex>" })
insights:exec({ "command": "tools" })            # fallback: list all

# 2. Inspect once if the schema is missing
insights:exec({ "command": "info <tool_name>" })

# 3. Drill into complex fields — REQUIRED for any field with a `hint`
insights:exec({ "command": "schema <tool_name> <field_path>" })

# 4. Call; reuse the schema
insights:exec({ "command": "call <tool_name> <json_input>" })
insights:exec({ "command": "call --json <tool_name> <json_input>" })
```
