...
const EVENTS_TOPIC = 'datastore_events_json_test'
const AI_EVENTS_TOPIC = 'datastore_ai_events_json_test'
...
describe('AiIngestionPipeline', () => {
    ...
    beforeEach(() => {
        ...
        config = {
            outputs: new IngestionOutputs({
                ...
                [INGESTION_WARNINGS_OUTPUT]: single(INGESTION_WARNINGS_OUTPUT, 'datastore_ingestion_warnings_test'),
                [OVERFLOW_OUTPUT]: single(OVERFLOW_OUTPUT, 'events_plugin_ingestion_overflow_test'),
                [APP_METRICS_OUTPUT]: single(APP_METRICS_OUTPUT, 'datastore_app_metrics2_test'),
            }),
    ...
    it('prefetches script transformation states for the batch teams before transforming', async () => {
        ...
        // Without this prefetch the transformer can't see Script watcher's disabled state,
