...
pub fn debounce_key(pak_id: &str) -> String {
    format!("insights:pak_last_used:{pak_id}")
}
...
pub async fn update_pak_last_used_at(
    ...
    if let Err(e) = sqlx::query(
        "UPDATE insights_personalapikey SET last_used_at = NOW() \
...
mod tests {
    ...
    async fn test_skips_update_when_last_used_at_is_recent() {
        ...
        let mut conn = ctx.get_non_persons_connection().await.unwrap();
        sqlx::query(
            "UPDATE insights_personalapikey SET last_used_at = NOW() - INTERVAL '30 minutes' WHERE id = $1",
        )
        .bind(&pak_id)
        .execute(&mut *conn)
        .await
        ...
        let mut conn = ctx.get_non_persons_connection().await.unwrap();
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM insights_personalapikey \
    ...
    async fn assert_record_pak(
        ...
        let count: (i64,) = sqlx::query_as(if expect_last_used_set {
            ...
            "SELECT COUNT(*) FROM insights_personalapikey WHERE id = $1 AND last_used_at IS NULL"
        })
        .bind(&pak_id)
        .fetch_one(&mut *conn)
        .await
