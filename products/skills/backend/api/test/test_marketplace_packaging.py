...
class TestFrontmatter:
    ...
    def test_version_lives_under_metadata_not_top_level(self):
        ...
        assert parsed["metadata"]["author"] == "insights"
...
class TestMarketplaceTree:
    def _tree(self, skills=None):
        return build_marketplace_tree(
            plugin_name="insights-skill-store",
            plugin_description="Team skills",
            plugin_version="1.0.42",
            owner_name="Insights",
            marketplace_name="insights-skill-store-marketplace",
    ...
    def test_marketplace_json_lists_the_plugin_with_version(self):
        ...
        assert manifest["name"] == "insights-skill-store-marketplace"
        assert manifest["owner"]["name"] == "Insights"
        plugin = manifest["plugins"][0]
        assert plugin["name"] == "insights-skill-store"
        assert plugin["source"] == "./plugins/insights-skill-store"
        assert plugin["version"] == "1.0.42"

    def test_plugin_json_present_and_versioned(self):
        plugin = json.loads(self._tree()["plugins/insights-skill-store/.claude-plugin/plugin.json"])
        assert plugin["name"] == "insights-skill-store"
        assert plugin["version"] == "1.0.42"

    def test_skill_files_nested_under_plugin_skill_dir(self):
        tree = self._tree()
        assert "plugins/insights-skill-store/skills/make-fractals/SKILL.md" in tree
        assert "plugins/insights-skill-store/skills/make-fractals/scripts/mandelbrot.py" in tree
    ...
    def test_empty_team_still_produces_a_valid_marketplace(self):
        ...
        assert manifest["plugins"][0]["name"] == "insights-skill-store"
...
class TestZipImport:
    ...
    def test_platform_version_dropped_user_metadata_kept(self):
        parsed = parse_skill_zip(build_skill_zip(_skill(version=9, metadata={"author": "insights"})))
        assert "version" not in parsed.metadata
        assert parsed.metadata.get("author") == "insights"
...
class TestGitTreeSafety:
    def _synth(self, files: dict[str, str]):
        return synthesize_repo(files, author="Insights", message="m")
...
class TestSynthesizedPackfile:
    def test_repo_carries_a_packfile(self):
        repo = synthesize_repo({"SKILL.md": "a", "scripts/x.sh": "b"}, author="Insights", message="m")
