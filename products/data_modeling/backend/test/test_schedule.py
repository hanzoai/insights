...
class TestGetV2ScheduledDagIds:
    ...
    def test_full_sweep_scopes_by_schedule_type_server_side(self):
        ...
        # InsightsScheduleType tag instead of paginating the whole namespace.
        assert captured["kwargs"]["query"] == 'InsightsScheduleType = "data-modeling-execute-dag"'
        assert result == {"dag-on-v2"}

    def test_scopes_listing_by_insights_dag_id_when_candidates_given(self):
        captured: dict = {}
        listings = [
            self._listing("dag-on-v2", "data-modeling-execute-dag"),
            self._listing("sq-on-v1", "data-modeling-run"),
        ...
        # Server-side filtering on the InsightsDagId search attribute (allowed, unlike WorkflowType)
        # keeps us from paginating the whole namespace.
        assert captured["kwargs"]["query"] == "InsightsDagId IN ('dag-on-v2')"
        assert result == {"dag-on-v2"}
