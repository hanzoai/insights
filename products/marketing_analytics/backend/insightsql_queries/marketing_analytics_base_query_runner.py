...
from insights.insightsql.database.schema.channel_type import ChannelTypeExprs, create_channel_type_expr
...
from products.marketing_analytics.backend.insightsql_queries.marketing_lazy_precompute import (
...
def _session_start_day(expr: ast.Expr) -> ast.Expr:
    ...
    `toStartOfDay` is on the allowlist in insights/insightsql/helpers/timestamp_visitor.py — `toDate` is
...
class MarketingAnalyticsBaseQueryRunner(AnalyticsQueryRunner[ResponseType], ABC, Generic[ResponseType]):
    ...
    @cached_property
    def _shared_insightsql_database(self) -> Database:
        """Built once and reused across every MarketingSourceFactory AND passed into
        `execute_insightsql_query` (via `_shared_insightsql_context`), so the request pays Database.create_for
        ...
        # timings instead of a discarded InsightsQLTimings — otherwise this whole build shows as an
        # opaque flat span.
    ...
    @cached_property
    def _shared_insightsql_context(self) -> InsightsQLContext:
        """InsightsQLContext carrying the prebuilt database, passed to execute_insightsql_query so its
        `_generate_insightsql` reuses the database instead of building a second one.

        Carries the runner's team and user because supplying a context stops `execute_insightsql_query` from
        ...
        return InsightsQLContext(
            team=self.team,
            database=self._shared_insightsql_database,
            user=self.user,
        )
        ...
        for index, conversion_goal in enumerate(conversion_goals):
            ...
            if should_create:
                processor = ConversionGoalProcessor(
                    ...
                    config=self.config,
                    user=self.user,
                    # Goals are built in parallel and InsightsQLTimings is not thread safe, so hand each
    ...
    def _get_where_conditions(
        ...
        if include_date_range:
            ...
            # stored as String, and Datastore cannot compare String with Date/DateTime.
    ...
    def _build_complete_query_ast(
        ...
        # the CTE there, but Datastore still scans the events table to materialize it.
    ...
    def to_query(self) -> ast.SelectQuery:
        """Generate the InsightsQL query using the new adapter architecture"""
        with self.timings.measure("marketing_analytics_base_query"):
            ...
            with self.timings.measure("ma_build_database"):
                _ = self._shared_insightsql_database
    ...
    def _build_compare_pivot(
        ...
        Datastore runs a LEFT JOIN sequentially (build the right side, then probe the left); the
        ...
        - For a current row with no previous counterpart, `anyIf(col, _period='previous')` matches
          ...
          produces under Datastore's default `join_use_nulls = 0`, so reproducing it manually
