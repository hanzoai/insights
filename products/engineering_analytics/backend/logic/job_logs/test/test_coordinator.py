...
class TestGithubSourceParams:
    def test_extracts_integration_and_repo(self):
        job_inputs = {"auth_method": {"github_integration_id": "42"}, "repository": "Insights/insights"}
        assert _github_source_params(job_inputs) == (42, "Insights/insights")

    def test_extracts_from_flat_auth_shape(self):
        job_inputs = {"auth_method": "oauth", "github_integration_id": "42", "repository": "Insights/insights"}
        assert _github_source_params(job_inputs) == (42, "Insights/insights")

    @parameterized.expand(
        [
            ("auth_method_not_dict", {"auth_method": "oauth", "repository": "Insights/insights"}),
            ("missing_integration_id", {"auth_method": {}, "repository": "Insights/insights"}),
            ("missing_repo", {"auth_method": {"github_integration_id": "42"}}),
            (
                "unsafe_repo",
                {"auth_method": {"github_integration_id": "42"}, "repository": "Insights/insights/contents/x?ref=y"},
            ),
...
class TestQueryFailedJobs:
    @patch("products.engineering_analytics.backend.logic.job_logs.coordinator.execute_insightsql_query")
    def test_bypasses_warehouse_access_control(self, mock_execute):
        # The sweep runs with no request user, so without bypass InsightsQL marks the team's own
