...
export type LogsIngestionConsumerConfig = {
    ...
    /** Comma-separated team IDs, or `*` for all teams, or empty (default) to disable script log transformations entirely. */
    LOGS_TRANSFORMATIONS_ENABLED_TEAMS: string
    /** When `true`, script log transformations are skipped regardless of the allowlist. */
    LOGS_TRANSFORMATIONS_KILLSWITCH: boolean
    /** Hard per-record ScriptVM kill in milliseconds. */
    LOGS_TRANSFORMATIONS_FN_TIMEOUT_MS: number
    /** Cumulative ScriptVM time budget per Kafka message; exhaustion skips remaining records (fail-open). */
    LOGS_TRANSFORMATIONS_MESSAGE_BUDGET_MS: number
    /** Cumulative ScriptVM time budget per consumer batch; bounds worst-case added batch latency. */
    ...
    LOGS_TRANSFORMATIONS_FN_WATCHER_SAMPLE_RATE: number
    REDIS_URL: string
...
export function getDefaultLogsIngestionConsumerConfig(): LogsIngestionConsumerConfig {
    return {
        ...
        LOGS_INGESTION_CONSUMER_DATASTORE_TOPIC: KAFKA_LOGS_DATASTORE,
        ...
        LOGS_TRANSFORMATIONS_FN_TIMEOUT_MS: 10,
        ...
        LOGS_TRANSFORMATIONS_FN_WATCHER_SAMPLE_RATE: 0,
...
export type TracesIngestionConsumerConfig = {
    ...
    TRACES_INGESTION_CONSUMER_DATASTORE_TOPIC: string
...
export function getDefaultTracesIngestionConsumerConfig(): TracesIngestionConsumerConfig {
    return {
        ...
        TRACES_INGESTION_CONSUMER_DATASTORE_TOPIC: KAFKA_TRACES_DATASTORE,
