"""Slack link unfurling for Insights insight, dashboard, and support ticket URLs (metadata only)."""
...
from insights.rbac.user_access_control import UserAccessControl, access_level_satisfied_for_resource
...
def parse_insights_resource_link(url: str) -> tuple[Literal["insight", "dashboard", "ticket"], str | int] | None:
    """
    Parse a Insights app URL into (resource kind, reference id).
    ...
    Slack-connected project and the resolved Insights user, not to any project id in the URL.
    ...
    if len(parts) >= 2 and parts[0] == "project":
        # Skip project/{anything} — do not use for authorization (see handle_insights_link_unfurl).
        idx = 2
...
def _extract_query_source_kind(query: dict) -> str | None:
    """Return the inner query kind (e.g. TrendsQuery, InsightsQLQuery) for labeling; no DB access."""
    kind = query.get("kind")
...
def _ticket_requester(team: Team, ticket: Ticket) -> str:
    ...
    """
    # Deferred to keep the heavy insights.api.person module off the Django startup import path.
    from insights.api.person import PERSON_DEFAULT_DISPLAY_NAME_PROPERTIES  # noqa: PLC0415
...
def handle_insights_link_unfurl(event: dict, integration: Integration) -> None:
    """
    Unfurl Insights insight, dashboard, and support ticket links with title and description.
    ...
    for link_obj in links:
        ...
        parsed = parse_insights_resource_link(raw_url)
        if not parsed:
            continue
