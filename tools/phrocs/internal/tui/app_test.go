...
import (
	...
	"github.com/insights/insights/phrocs/internal/config"
	"github.com/insights/insights/phrocs/internal/docker"
	"github.com/insights/insights/phrocs/internal/process"
)
...
func TestSetup_handleListUnitsMsgError(t *testing.T) {
	m := setupModel(t)
	m.handleListUnitsMsg(listUnitsMsg{err: fmt.Errorf("insightscli failed")})
	if m.setupError == "" {
...
func TestMascot_enterAndExit(t *testing.T) {
	m := readyModel(t, "backend")
	m = update(m, keypress('h'))
	if !m.mascotMode {
		t.Error("h should enter mascot mode")
	}
	if m.mascotX != 0 {
		t.Errorf("initial X: got %d, want 0", m.mascotX)
	}
	if m.mascotDir != 1 {
		t.Errorf("initial dir: got %d, want 1 (right)", m.mascotDir)
	}
	m = update(m, keypress('h'))
	if m.mascotMode {
		t.Error("second h should exit mascot mode")
	}
}

func TestMascot_exitWithEscape(t *testing.T) {
	m := readyModel(t, "backend")
	m = update(m, keypress('h'))
	if !m.mascotMode {
		t.Fatal("should be in mascot mode")
	}
	m = update(m, specialKey(tea.KeyEscape))
	if m.mascotMode {
		t.Error("esc should exit mascot mode")
	}
}

func TestMascot_jump(t *testing.T) {
	m := readyModel(t, "backend")
	m = update(m, keypress('h'))
	if m.mascotY != 0 {
		t.Fatalf("initial Y: got %d, want 0", m.mascotY)
	}
	m = update(m, specialKey(tea.KeySpace))
	if m.mascotY != 1 {
		t.Errorf("Y after jump: got %d, want 1", m.mascotY)
	}
	if m.mascotVelY != 1 {
		t.Errorf("velY after jump: got %d, want 1", m.mascotVelY)
	}
}

func TestMascot_jumpOnlyFromGround(t *testing.T) {
	m := readyModel(t, "backend")
	m = update(m, keypress('h'))
	// First jump
	m = update(m, specialKey(tea.KeySpace))
	...
	m = update(m, specialKey(tea.KeySpace))
	if m.mascotY != y || m.mascotVelY != vel {
		t.Error("space while airborne should not change Y or velY")
	}
}

func TestMascot_advanceMoves(t *testing.T) {
	m := readyModel(t, "backend")
	m.mascotMode = true
	m.mascotDir = 1
	m.mascotX = 0
	...
	m.advanceMascot()
	if m.mascotX != 1 {
		t.Errorf("X after advance: got %d, want 1", m.mascotX)
	}
	if m.mascotFrame != 1 {
		t.Errorf("frame after advance: got %d, want 1", m.mascotFrame)
	}
}

func TestMascot_bounceAtEdge(t *testing.T) {
	m := readyModel(t, "backend")
	m.mascotMode = true
	spriteW := len(mascotFramesRight[0][0])
	m.mascotX = m.viewport.Width() - spriteW
	m.mascotDir = 1

	m.advanceMascot()
	if m.mascotDir != -1 {
		t.Errorf("dir after hitting right edge: got %d, want -1", m.mascotDir)
	}
}

func TestMascot_bounceAtLeftEdge(t *testing.T) {
	m := readyModel(t, "backend")
	m.mascotMode = true
	m.mascotX = 0
	m.mascotDir = -1
	...
	m.advanceMascot()
	if m.mascotDir != 1 {
		t.Errorf("dir after hitting left edge: got %d, want 1", m.mascotDir)
	}
}

func TestMascot_gravity(t *testing.T) {
	m := readyModel(t, "backend")
	m.mascotMode = true
	m.mascotY = 2
	m.mascotVelY = 0
	...
	m.advanceMascot()
	if m.mascotY != 2 {
		t.Errorf("Y with velY=0 at Y=2: got %d, want 2", m.mascotY)
	}
	// Y>0 and velY=0 means gravity pulls down: velY becomes -1
	if m.mascotVelY != -1 {
		t.Errorf("velY should decrease by 1, got %d", m.mascotVelY)
	}

	m.advanceMascot()
	// Y=2+(-1)=1, velY=-1-1=-2
	if m.mascotY != 1 {
		t.Errorf("Y after gravity: got %d, want 1", m.mascotY)
	}

	m.advanceMascot()
	// Y=1+(-2)=-1 → clamped to 0
	if m.mascotY != 0 {
		t.Errorf("Y should clamp to 0, got %d", m.mascotY)
	}
	if m.mascotVelY != 0 {
		t.Errorf("velY should reset to 0 on landing, got %d", m.mascotVelY)
	}
}
