...
from insights.helpers.slack_scopes import REQUIRED_SLACK_SCOPES
...
from insights.models.user_integration import UserGitHubIntegration, UserIntegration
...
from insights.temporal.ai.slack_app.insights_code_slack_interactivity import (
    InsightsCodeSlackInteractivityInputs,
...
from insights.temporal.ai.slack_app.slack_app_mention import (
...
from products.slack_app.backend.services.slack_user_oauth import (
    build_invite_url,
    find_linked_insights_user,
    post_link_invite_message,
)
from products.slack_app.backend.slack_link_unfurl import handle_insights_link_unfurl, parse_insights_resource_link
...
CHANNEL_ONBOARDING_DOCS_URL = "https://hanzo.ai/docs/slack-app"
...
SLACK_MENTION_DROPPED_EVENT = "insights code slack mention dropped"
...
PICKER_TOKEN_SALT = "insights_code_repo_picker"
PICKER_TOKEN_MAX_AGE_SECONDS = 900

CHANNEL_APPROVAL_BLOCK_ID_PREFIX = "insights_code_channel_approval"
CHANNEL_APPROVAL_ACTION_APPROVE = "insights_code_channel_approve"
CHANNEL_APPROVAL_ACTION_DENY = "insights_code_channel_deny"
...
def _user_repo_list_cache_key(user_id: int) -> str:
    return f"insights_code:user_repo_list:v1:{user_id}"
...
def _pending_repo_picker_cache_key(integration_id: int, channel: str, thread_ts: str, slack_user_id: str) -> str:
    ...
    return f"insights_code:pending_repo_picker:v1:{hashlib.sha256(raw_key.encode('utf-8')).hexdigest()}"
...
class SlackUserContext:
    ...
    # Insights user without ever consulting Slack's email, so no value is
...
@dataclass
class RulesCommand:
    """Parsed `@Insights <command>` mention text.
...
QUOTA_EXHAUSTED_MESSAGE = (
    ...
    "Top up at https://us.hanzo.ai/organization/billing to continue."
)
...
def resolve_slack_user(
    ...
    *,
    ...
    """Resolve a Slack user to a Insights user. Posts an ephemeral error message and returns None on failure (unless post_feedback is False)."""
    ...
    try:
        ...
        # Insights account via the OAuth link flow, we resolve directly without
        ...
        linked_user = find_linked_insights_user(
            ...
            if user_permissions.current_team.effective_membership_level is None:
                ...
                if post_feedback:
                    _post_slack_user_feedback(
                        ...
                        thread_ts,
                        (
                            "Sorry, you don't have access to the Insights project connected to this Slack workspace. "
                            "Please ask an admin of your Insights organization to grant you access."
                        ),
                    )
                return None
        ...
        if (dev_email := local_dev_slack_email()) is not None:
            ...
            # other callers (e.g. `resolve_insights_user_from_event` from the
            ...
            if post_feedback:
                _post_slack_user_feedback(
                    ...
                    (
                        ...
                        "and contact the Insights team if the issue persists."
                    ),
                    prefer_thread_message=True,
                )
            return None
            ...
            if post_feedback:
                ...
                if is_slack_app_oauth_enabled(integration, slack_team_id):
                    invite_url = build_invite_url(
                        slack_user_id=slack_user_id,
                        slack_team_id=slack_team_id,
                        insights_team_id=integration.team_id,
                        channel=channel,
                        thread_ts=thread_ts,
                    )
                    post_link_invite_message(
        ...
        return SlackUserContext(user=insights_user, slack_email=slack_email)
...
# Slack delivers a single webhook URL per app, but the workspace's Insights Integration may live
...
#
# This keeps the slack manifest endpoint swappable between us.hanzo.ai and eu.hanzo.ai
# without any other coordination.
REGION_PROXY_HEADER = "X-Insights-Region-Proxied"
...
def cross_region_routing_enabled() -> bool:
    # Cross-region routing only makes sense between Insights Cloud US and EU — they share the
    ...
    # as a single region; proxying their Slack events to us.hanzo.ai targets a different
...
def slack_workspace_claims_view(request: HttpRequest) -> HttpResponse:
    ...
    Both Cloud regions provision the Insights Desktop Slack signing secret, so a region can HMAC-sign
...
def _post_repo_picker_message(
    *,
    ...
    blocks: list[dict[str, Any]] = [
        {
            "type": "section",
            "block_id": f"insights_code_repo_picker_v2:{integration.id}:{slack_user_id}:{context_token}",
    ...
    if allow_no_repo:
        blocks.append(
            {
                "type": "actions",
                "block_id": f"insights_code_repo_picker_v2:{integration.id}:{slack_user_id}:{context_token}:actions",
                "elements": [
                    {
                        "type": "button",
                        "action_id": "insights_code_repo_none",
    ...
    response = slack.client.chat_postMessage(
        ...
        blocks=blocks,
        metadata={
            "event_type": "insights_code_repo_picker",
...
def _resolve_pending_repo_picker_from_followup(event: dict[str, Any], integration: Integration) -> bool:
    ...
    if not isinstance(mentioning_user_id, int):
        # Without a known Insights user we can't scope the picker to a personal install;
    ...
    try:
        ...
        asyncio.run(handle.signal(InsightsCodeSlackMentionWorkflow.repo_selected, selected_repo))
    except Exception as e:
        logger.warning(
...
def _app_mention_ignore_reason(event: dict[str, Any]) -> str | None:
    ...
    - "bot_author" / "app_authored": see ``_app_authorship_ignore_reason``. Foreign bots
      that quote `<@Insights>` in their text (incident bots, alert relays, our own
...
def _notify_missing_slack_scopes(
    ...
    text = (
        ":warning: Insights can't reply because the Slack integration is missing required "
...
def resolve_insights_user_from_event(
    *,
    slack_user_id: str,
    ...
    """Resolve the acting Slack user to a Insights ``User`` who is a member of
    ...
    # their Slack identity to a Insights account. The cheap indexed lookup
...
def _post_pick_a_project_hint(
    ...
    """Tell the user that this workspace is connected to multiple Insights
    projects, and that they should pick one.

    The selection command differs by surface: in a channel the user mentions the app
    (`@Insights project <id>`), but in a DM there is no app to mention, so they just reply
    with `project <id>`.
    ...
    if not isinstance(slack_user_id, str) or not isinstance(channel, str) or not isinstance(thread_ts, str):
        return False
    pick_command = "`project <id>`" if event.get("channel_type") == "im" else "`@Insights project <id>`"
    text = (
        "This Slack workspace is connected to multiple Insights projects:\n"
...
def _post_user_resolution_failure_reply(
    *,
    ...
    """Tell a Slack user when we can't route their mention to a Insights project.
    ...
    so the user sees which address Insights tried to match.
    ...
    if failure_reason == "user_not_found" and is_slack_app_oauth_enabled(probe, probe.integration_id):
        invite_url = build_invite_url(
            ...
            insights_team_id=probe.team_id,
            channel=channel,
            thread_ts=thread_ts,
        )
        post_link_invite_message(
...
def _start_insights_code_workflow(
    workflow_cls: Any,
    workflow_inputs: Any,
    *,
    id_prefix: str,
...
_ASSISTANT_WELCOME = (
    "Hi! I'm Insights, an AI agent. DM me to investigate issues using your Insights data and "
    "open PRs in your connected repos to fix them!"
)
_ASSISTANT_INSTALL_WELCOME = (
    "Thanks for adding Insights! :tada: I'm an AI agent - DM me here or @mention me in a channel "
    "to investigate issues or open PRs in your connected repos"
)
_ASSISTANT_UNAVAILABLE = (
    "I can only help Insights org members whose project has a connected repo. Make sure your Slack "
    "email matches your Insights account and that a repo is connected, then try again."
)
_ASSISTANT_MEMBER_JOIN_WELCOME = (
    ":wave: Welcome! I'm Insights, an AI agent your team uses. DM me here to investigate issues "
    "using your Insights data or open PRs in your connected repos - you can also @mention me in "
    "any channel."
)
...
def _handle_assistant_dm_message(
    ...
    *,
    ...
    return _start_mention_workflow(agent_event, integration, slack_team_id, event_id, insights_user=insights_user)
...
def _route_assistant_event(
    ...
    *,
    ...
    # against projects the resolved Insights user can actually access (no cross-org metadata leak).
...
def route_insights_code_event_to_relevant_region(
    request: HttpRequest,
    event: dict,
    slack_team_id: str,
    ...
    *,
    ...
    if event_type in ("app_mention", "message"):
        ...
        if resolution.user is None:
            ...
            # ``no_team_access`` means the Slack user *did* map to a Insights user, just
            ...
            attributed_user = _report_slack_mention_received(event, probe, slack_team_id, insights_user=None)
        ...
        # ``@Insights`` — an untagged thread reply can never be a rules command.
        if untagged_followup_mapping is None and parse_rules_command(event.get("text", "")) is not None:
            return _start_command_workflow(event, candidates, slack_team_id, event_id, user_id=insights_user.id)
    ...
    if local_match:
        ...
        if event_type == "link_shared":
            ...
            if (
                ...
                return ROUTE_HANDLED_LOCALLY
            handle_insights_link_unfurl(event, local_match)
        return ROUTE_HANDLED_LOCALLY
...
def _start_command_workflow(
    ...
    *,
    ...
    _start_insights_code_workflow(
        ...
        id_prefix="insights-code-mention-command",
        slack_team_id=slack_team_id,
        event=event,
        event_id=event_id,
    )
...
def _route_member_joined_channel(
    ...
    *,
    ...
    """Welcome the @Insights bot to a new channel exactly once.
    ...
    first @Insights mention there will run the existing approval flow.
    """
...
def _post_channel_onboarding_message(slack: SlackIntegration, integration: Integration, channel_id: str) -> bool:
    ...
    blocks: list[dict[str, Any]] = [
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": (
                    ":wave: Thanks for adding the Insights app to this channel! "
                    "Mention me with `@Insights` to get started – I can answer "
                    "questions about your Insights data, research your codebase, "
                    "and kick off coding tasks backed by real usage data. "
                    "I'll also unfurl Insights links you share here."
                ),
            },
        },
        {
            ...
            "text": {
                ...
                "text": (
                    "*Try one of these:*\n"
                    "• `@Insights what's our weekly active user count this month?`\n"
                    "• `@Insights open a PR that adds a unit test for src/utils.py`"
                ),
            },
        },
        {
    ...
    try:
        slack.client.chat_postMessage(
            channel=channel_id,
            text="Thanks for adding the Insights app – mention me with @Insights to get started.",
            blocks=blocks,
            unfurl_links=False,
            unfurl_media=False,
        )
...
def _channel_is_approved(slack_workspace_id: str, channel_id: str) -> bool:
    """True iff a user in this channel has already granted Insights approval.

    The ``is_ext_shared_channel`` flag that gates whether approval is required
    at all lives on the Slack event envelope — see ``insights_code_event_handler``
...
def _post_channel_approval_prompt(
    ...
    text = (
        ...
        "Insights project — including data about users, customers, and other internal metrics that aren't "
            ...
            "elements": [
                {
                    ...
                    "style": "primary",
                    "text": {"type": "plain_text", "text": "✅ Allow Insights in this channel"},
                    "value": context_token,
                },
                {
                    "type": "button",
    ...
    try:
        slack.client.chat_postEphemeral(
            channel=channel,
            user=slack_user_id,
            text="The Insights Slack app needs approval before answering in this externally-shared channel.",
            blocks=blocks,
        )
    except Exception:
        logger.warning(
...
def _report_slack_mention_received(
    ...
    slack_team_id: str,
    *,
    insights_user: User | None = None,
) -> User | None:
    """Capture a product-analytics event each time the @Insights bot is mentioned.

    Returns the Insights user the event was attributed to (``None`` when the Slack user
    ...
    Slack user is resolved to a Insights ``User`` so the event is attributed to a real person where
    one exists; otherwise it falls back to a stable Slack-derived distinct id.

    When the caller has already resolved the Insights user at routing time it passes ``insights_user``
    ...
    try:
        ...
        if insights_user is None and slack_user_id:
            insights_user = resolve_insights_user_from_event(
                slack_user_id=slack_user_id,
                probe_integration=integration,
                candidate_integrations=[integration],
            )
        # Prefer the resolved Insights user's distinct id so the event attributes to a real person;
        # fall back to a stable Slack-derived id so anonymous-but-valid mentions are still captured.
        identified_distinct_id = insights_user.distinct_id if insights_user else None
        ...
        properties: dict[str, Any] = {
            ...
            "slack_user_id": slack_user_id,
            "insights_user_identified": identified_distinct_id is not None,
        }
        if insights_user is not None and identified_distinct_id is not None:
            properties["$set"] = insights_user.get_analytics_metadata()

        hanzo_insights.capture(
            distinct_id=distinct_id,
            event="insights code slack mention received",
            properties=properties,
            groups=groups(team=integration.team),
            send_feature_flags=True,
        )
        return insights_user
    except Exception:
        logger.warning(
    ...
    **extra: object,
    ...
    try:
        ...
        properties: dict[str, Any] = {
            ...
            "slack_user_id": slack_user_id,
            "insights_user_identified": insights_user is not None,
            **extra,
        }
        ...
        hanzo_insights.capture(
            distinct_id=(insights_user.distinct_id if insights_user else None)
...
def _start_mention_workflow(
    ...
    *,
    ...
    (which is meaningful only when the user actually typed ``@Insights``). It
    ...
    if not untagged_followup:
        assert insights_user is not None, "app_mention path must always resolve a user before dispatch"
        _report_slack_mention_received(event, integration, slack_team_id, insights_user=insights_user)
        if _resolve_pending_repo_picker_from_followup(event, integration):
            return ROUTE_HANDLED_LOCALLY
    workflow_inputs = InsightsCodeSlackMentionWorkflowInputs(
        event=event,
        integration_id=integration.id,
        slack_team_id=slack_team_id,
        slack_event_id=event_id,
        user_id=insights_user.id if insights_user else None,
    ...
    _start_insights_code_workflow(
        ...
        workflow_inputs,
        id_prefix="insights-code-mention",
        slack_team_id=slack_team_id,
        event=event,
        event_id=event_id,
...
def _picker_context_cache_key(context_token: str) -> str:
    ...
    return f"insights_code_repo_picker_ctx:{token_hash}"
...
def _decode_picker_context(context_token: str) -> dict[str, Any] | None:
    ...
    if len(context_token) < 120:
        cached = cache.get(f"insights_code_repo_picker_ctx:{context_token}")
    if isinstance(cached, dict):
        return cached
...
@csrf_exempt
def insights_code_event_handler(request: HttpRequest) -> HttpResponse:
    if request.method != "POST":
        return HttpResponse(status=405)
    ...
    # insights_code_event_handler: unrecognized event type
    return HttpResponse(status=200)


def _is_insights_app_url(url: str) -> bool:
...
def _link_shared_resource_refs(event: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for link in links:
        if not isinstance(link, dict):
            continue
        url = link.get("url")
        if not isinstance(url, str) or not _is_insights_app_url(url):
            continue
        parsed = parse_insights_resource_link(url)
        if parsed:
...
def _extract_context_token(payload: dict) -> str:
    ...
    def token_from_block_id(raw_block_id: str) -> str:
        if not raw_block_id:
            return ""
        if raw_block_id.startswith("insights_code_repo_picker_v2:"):
...
def _extract_picker_hints(payload: dict) -> tuple[int | None, str | None]:
    ...
    if not block_id.startswith("insights_code_repo_picker_v2:"):
        return None, None
...
def _extract_terminate_hints(payload: dict) -> tuple[int | None, str | None]:
    return _extract_action_value_hints(payload, "insights_code_terminate_task")
...
def _handle_repo_picker_options(payload: dict) -> JsonResponse:
    ...
    if action != "insights_code_repo_select":
    ...
    if not isinstance(mentioning_user_id, int):
        # Without a known Insights user we can't scope the options to a personal install;
...
def _handle_repo_picker_submit(payload: dict) -> HttpResponse:
    ...
    actions = payload.get("actions", [])
    action = next((a for a in actions if a.get("action_id") == "insights_code_repo_select"), None)
    ...
    def post_selection_expired() -> None:
        ...
        try:
            ...
            SlackIntegration(integration).client.chat_postMessage(
                channel=channel,
                thread_ts=thread_ts,
                text="Repository selection expired. Please mention Insights again to retry.",
            )
        except Exception:
            logger.warning(
...
def _handle_no_repo_needed_submit(payload: dict) -> HttpResponse:
    ...
    try:
        client = sync_connect()
        handle = client.get_workflow_handle(workflow_id)
        asyncio.run(handle.signal(InsightsCodeSlackMentionWorkflow.no_repo_needed))
...
def _handle_continue_as_bot(payload: dict) -> HttpResponse:
    action = next(
        (a for a in payload.get("actions", []) if a.get("action_id") == "insights_code_continue_as_bot"),
        None,
    )
    if not action:
        return HttpResponse(status=200)
    ...
    try:
        client = sync_connect()
        handle = client.get_workflow_handle(workflow_id)
        asyncio.run(handle.signal(InsightsCodeSlackMentionWorkflow.authorship_confirmed))
    except Exception as e:
    ...
    if integration_id and slack_team_id and channel and message_ts:
        try:
            ...
            text = "Continuing as Insights — the PR will be authored by the Insights bot."
...
def _is_org_member(integration: Integration, clicker_slack_user_id: str) -> User | None:
    """Resolve the clicker to a Insights ``User`` belonging to the integration's
    organization, or ``None`` if no such membership exists.
    """
    return resolve_insights_user_from_event(
...
def _org_label(integration: Integration) -> str:
    try:
        ...
        org_name = ""
    return f"*{org_name}*" if org_name else "this Insights organization"
...
def _reject_non_member_click(
    ...
    _post_channel_approval_outcome(
        ...
        channel_id,
        thread_ts,
        f":warning: Only members of {_org_label(integration)} can manage the Insights Slack app in this channel.",
    )
    _delete_ephemeral_via_response_url(response_url)
...
def _handle_channel_approval_submit(payload: dict) -> HttpResponse:
    ...
    member of the integration's Insights organization.
    ...
    insights_user = _is_org_member(integration, clicker_slack_user_id)
    ...
    _post_channel_approval_outcome(
        ...
        channel_id,
        thread_ts,
        f"✅ <@{clicker_slack_user_id}> enabled the Insights Slack app in this channel.",
    )
    _delete_ephemeral_via_response_url(response_url)
    return HttpResponse(status=200)
...
def _handle_channel_approval_deny(payload: dict) -> HttpResponse:
    ...
    _post_channel_approval_outcome(
        ...
        channel_id,
        thread_ts,
        f"✋ <@{clicker_slack_user_id}> dismissed the Insights Slack app for this channel. Mention me again to retry.",
    )
    _delete_ephemeral_via_response_url(response_url)
    return HttpResponse(status=200)
...
def _handle_signals_dismiss_report(payload: dict) -> HttpResponse:
    ...
    # Only Insights org members may dismiss — a non-member in a shared channel must not suppress reports.
...
def _replace_message_stripping_actions(payload: dict, text: str, *, keep_link_buttons: bool = True) -> None:
    ...
    Insights") are preserved instead of being dropped along with the interactive ones.
    """
    response_url = payload.get("response_url")
    if not response_url:
        return
...
def _post_signals_dismiss_feedback(payload: dict, *, dismissed: bool, slack_user_id: str) -> None:
    ...
    # Insights" link buttons in the same actions block as Dismiss (see the pre-removal
...
def _handle_terminate_task_submit(payload: dict) -> HttpResponse:
    ...
    action = next((a for a in payload.get("actions", []) if a.get("action_id") == "insights_code_terminate_task"), None)
    action_ts = action.get("action_ts") if action else ""
    team_id = payload.get("team", {}).get("id", "")
    user_id = payload.get("user", {}).get("id", "")
    workflow_id = (
        f"insights-code-terminate-task:{team_id}:{user_id}:{action_ts or payload.get('message', {}).get('ts', '')}"
    )
    try:
        client = sync_connect()
        asyncio.run(
            client.start_workflow(
                InsightsCodeSlackTerminateTaskWorkflow.run,
                InsightsCodeSlackInteractivityInputs(payload=payload),
                id=workflow_id,
                task_queue=settings.TASKS_TASK_QUEUE,
                id_conflict_policy=WorkflowIDConflictPolicy.USE_EXISTING,
                id_reuse_policy=WorkflowIDReusePolicy.ALLOW_DUPLICATE,
...
@csrf_exempt
def insights_code_interactivity_handler(request: HttpRequest) -> HttpResponse:
    if request.method != "POST":
        return HttpResponse(status=405)
    ...
    authorship_integration_id, _ = _extract_action_value_hints(payload, "insights_code_continue_as_bot")
    ...
    if payload_type == "block_actions":
        ...
        for action in actions:
            ...
            if action_id == "insights_code_repo_select":
                return _handle_repo_picker_submit(payload)
            if action_id == "insights_code_repo_none":
                return _handle_no_repo_needed_submit(payload)
            if action_id == "insights_code_continue_as_bot":
                return _handle_continue_as_bot(payload)
            if action_id == "insights_code_terminate_task":
