...
This skill captures the Insights-specific conventions on top of the upstream
...
- `pnpm --filter=@hanzo/frontend typegen:watch` — watch mode while writing logics
- `pnpm --filter=@hanzo/frontend typegen:write` — one-shot write
- `pnpm --filter=@hanzo/frontend typegen:check` — CI parity check
...
pnpm --filter=@hanzo/frontend typegen:file frontend/src/scenes/foo/fooLogic.ts
```
