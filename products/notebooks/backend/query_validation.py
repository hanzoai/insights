...
# schema at insights/schema.py — InsightVizNode.source).
...
def _normalize_query(query: Any) -> Any:
    ...
    if kind not in _VALID_TOP_LEVEL_QUERY_KINDS:
        # Other QuerySchema kinds (InsightsQLQuery, EventsQuery, ...) can be used as a top-level
...
def _normalize_insight_viz_node(query: dict[str, Any]) -> dict[str, Any]:
    ...
    if source_kind == "InsightsQLQuery":
        # Same shape of mistake, one level up: agent wrapped a raw InsightsQLQuery in an
        # InsightVizNode. The correct shell for a SQL chart is DataVisualizationNode.
        logger.warning(
            "notebook_query_auto_rewrapped_insightsql_query",
            reason="InsightVizNode wrapping InsightsQLQuery",
        )
...
def _normalize_data_visualization_node(query: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(source, dict):
        raise InvalidNotebookQueryError(
            ...
            "DataVisualizationNode.source must be a InsightsQLQuery."
        )
    ...
    if source_kind != "InsightsQLQuery":
        raise InvalidNotebookQueryError(
            ...
            "is not a valid SQL chart source. DataVisualizationNode.source must be a InsightsQLQuery."
        )
