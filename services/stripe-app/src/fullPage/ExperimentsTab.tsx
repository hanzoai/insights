...
import type { InsightsExperiment } from '../insights/types'
...
const ExperimentsTab = ({ client, projectId }: Props): JSX.Element => {
    const [experiments, setExperiments] = useState<InsightsExperiment[]>([])
    ...
    if (!client || !projectId) {
        return (
            <Banner
                ...
                description="A Insights project_id isn't stored yet — reconnect to load experiments."
            />
        )
    }
    ...
    if (!loading && experiments.length === 0) {
        return <Banner type="default" title="No experiments yet" description="Create one in Insights to see it here." />
    }
    ...
    const visibleExperiments = needle
        ? experiments.filter(
              (e: InsightsExperiment) =>
                  e.name.toLowerCase().includes(needle) || (e.feature_flag_key ?? '').toLowerCase().includes(needle)
          )
        : experiments

    const linkMap: Record<string, string> = {}
    const items: DataTableItem[] = visibleExperiments.map((e: InsightsExperiment) => {
        const url = `${insightsBase}/experiments/${e.id}`
        linkMap[url] = e.name
        return {
            id: `experiment:${e.id}`,
            name: url,
        ...
        <>
            ...
            <Box css={{ width: 'fill', stack: 'y', rowGap: 'medium' }}>
                ...
                <Box css={{ paddingX: 'medium' }}>
                    <ExternalLink href={`${insightsBase}/experiments`}>View in Insights</ExternalLink>
                </Box>
            </Box>
        </>
    )
