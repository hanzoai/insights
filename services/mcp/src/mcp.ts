...
- You are a helpful assistant that can query Insights API.
${SHARED_PROMPT}
`.trim()
...
export class MCP extends McpAgent<Env> {
    server = new McpServer({ name: 'Insights', version: '1.0.0' }, { instructions: INSTRUCTIONS_V1 })
    ...
    async init(): Promise<void> {
        ...
        this.server = new McpServer({ name: 'Insights', version: '1.0.0' }, { instructions })
