...
class TestSessionReplayFeaturesTable(BaseTest):
    def setUp(self):
        ...
        self.context = InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, database=self.database)

    def _print_datastore(self, query: str) -> str:
        out = prepare_and_print_ast(parse_select(query), self.context, dialect="datastore")
        return out[0] if isinstance(out, tuple) else out

    def test_resolves_under_insights_namespace(self):
        table = self.database.get_table(["insights", "session_replay_features"])
        assert isinstance(table, SessionReplayFeaturesTable)
        assert table.to_printed_datastore(self.context) == "session_replay_features"
        assert table.to_printed_insightsql() == "session_replay_features"
    ...
    def test_max_last_timestamp_compiles(self):
        ...
        sql = self._print_datastore(
            "SELECT max(max_last_timestamp) AS last_seen FROM insights.session_replay_features WHERE team_id = 1 LIMIT 1"
        )
        assert "max(toTimeZone(session_replay_features.max_last_timestamp" in sql

    def test_resolvable_in_table_tree_under_insights_namespace(self):
        names = self.database.tables.resolve_all_table_names()
        assert "insights.session_replay_features" in names
