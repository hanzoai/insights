...
- **Direct dispatch by task name, not a signal.** Core (`succeed_materialization_activity`) enqueues
  ...
  InsightsQL out of the data-modeling worker's process, since core imports nothing from here. Trade-off:
...
- **Create-path sync is synchronous, best-effort, and workflow-only.** When the external create
  endpoint is called by a workflow "Create account" step (the `X-Insights-Script-Flow-Id` header),
...
- [ ] Relationship read path in InsightsQL/query runner replaces the JSON `ExpressionField`s (PR 2)
...
- **Cascade deletions don't archive the Slack destination.** The managed InsightsFunction is archived
  ...
  is harmless — the InsightsFunction is team-scoped and dies in the same cascade. Hard-deleting an
...
- **`SLACK_ARCHIVES_ORIGIN` hardcodes Insights's own workspace** — in `backend/constants.py` and
  ...
  Fine while the product is Insights-internal; **must be fixed before GA**. The correct value is
