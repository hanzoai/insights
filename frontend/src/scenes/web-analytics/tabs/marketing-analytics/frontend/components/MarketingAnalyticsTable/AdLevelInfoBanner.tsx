...
export const AdLevelInfoBanner = ({ drillDownLevel, sourcesHierarchyStatus }: AdLevelInfoBannerProps): JSX.Element => {
    ...
    return (
        <Banner type={fixableSources.length > 0 ? 'warning' : 'info'} dismissKey={dismissKey}>
            <div>
            ...
            {platformUnsupportedSources.length > 0 && (
                <div className="mt-2">
                    ...
                    {isAdGroup ? 'ad-group' : 'ad'}-level data through Insights's data warehouse import — coming in a
                    follow-up.
                </div>
            )}
        </Banner>
