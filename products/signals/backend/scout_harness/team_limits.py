...
# Fail-safe allowlist used when the flag payload is missing/invalid — but only on Insights
...
def _fallback_team_ids() -> list[int]:
    """Default allowlist when the flag payload is absent/unreadable — gated to Insights Cloud
...
def _read_flag_payload() -> dict | None:
    ...
    the same snapshot. Mirrors `insights/temporal/ai_observability/team_discovery.py`.
    """
    try:
        payload = hanzo_insights.get_feature_flag_payload(
