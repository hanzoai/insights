...
// A destination that fetches one of Insights's own ingestion endpoints, authenticating
...
// Event property carrying the self-loop depth *per script function*, keyed by function id:
...
// `$insights_function_execution_count` that bounds the `insightsCapture` path.
export const SELF_LOOP_DEPTH_PROPERTY = '$insights_function_self_loop_depth'
...
export const isInsightsIngestUrl = (urlString: string): boolean => {
    try {
        const url = new URL(urlString)
        const host = url.hostname.toLowerCase()
        const isIngestHost = host === 'hanzo.ai' || host.endsWith('.hanzo.ai')
        if (!isIngestHost) {
            return false
        }
...
// True when a fetch targets a Insights ingestion endpoint authenticating with the
...
export const isSelfReferentialIngestFetch = (input: {
    ...
    if (!isInsightsIngestUrl(input.url)) {
        return false
    }
...
// How many times the given script function has already re-fed the pipeline on this chain,
