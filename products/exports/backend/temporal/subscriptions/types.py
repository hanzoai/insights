...
from insights.slo.types import SloConfig
...
def safe_error_message(exc: BaseException) -> typing.Optional[str]:
    ...
    InsightsQL/Datastore error text can echo team-scoped identifiers (query data, internal
    ...
    the InsightsQL repair loop uses when forwarding to the fixer. Everything else returns None so
...
# Per-step query diagnostics (generated InsightsQL + failure type) so a degraded report is debuggable
