...
# Re-exported from the Django-free insights.raw_sessions_v3_ad_ids module so the InsightsQL schema can
# use it without booting Django; kept importable here for existing callers.
from insights.raw_sessions_v3_ad_ids import SESSION_V3_LOWER_TIER_AD_IDS  # noqa: E402
...
def GET_NUM_RAW_SESSIONS_ACTIVE_PARTS(
    ...
    *,
    ...
    Datastore's parts_to_throw_insert is per-partition, so this checks the max
    ...
    parts_table = (
        f"clusterAllReplicas('{settings.DATASTORE_CLUSTER}', system.parts)" if use_cluster else "system.parts"
    )
