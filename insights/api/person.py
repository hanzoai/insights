...
from insights.schema import ActorsQuery, ProductKey

from insights.insightsql.constants import CSV_EXPORT_LIMIT

from insights.api.capture import CaptureInternalError, capture_internal
from insights.api.documentation import PersonPropertiesSerializer
from insights.api.fields import CoercedStringListField
...
from insights.models import Filter, Person, Team, User
...
from insights.models.filters.properties_timeline_filter import PropertiesTimelineFilter
...
from insights.queries.actor_base_query import get_serialized_people
...
class PersonBulkDeleteRequestSerializer(serializers.Serializer):
    ids = serializers.ListField(
        ...
        required=False,
        help_text="A list of Insights person UUIDs to delete (max 1000).",
    )
...
class PersonViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def safely_get_object(self, queryset):
        ...
        with personinsights_caller_tag(f"persons/{self.action.replace('_', '-')}"):
    ...
    def list(self, request: request.Request, *args: Any, **kwargs: Any) -> response.Response:
        ...
        from insights.insightsql import ast  # noqa: PLC0415 — deferred to avoid a circular import at module load
        from insights.insightsql.query import execute_insightsql_query  # noqa: PLC0415

        from insights.insightsql_queries.actors_query_runner import ActorsQueryRunner  # noqa: PLC0415
        from insights.models.person.util import get_person_by_distinct_id  # noqa: PLC0415
        ...
        if filter.distinct_id:
            ...
            if matched is None:
                ...
                # lazy table, so Datastore would still aggregate every person row for the
            ...
            person_properties.append({"type": "insightsql", "key": f"id = toUUID('{matched.uuid}')"})
        ...
        with personinsights_caller_tag("persons/list"):
        ...
        if "include_total" in request.GET:
            ...
            total_count = execute_insightsql_query(count_query, team=team).results[0][0]
    ...
    def values(self, request: request.Request, **kwargs) -> response.Response:
        from insights.insightsql_queries.property_values_query_runner import (
        ...
        from insights.insightsql_queries.query_runner import ExecutionMode, execution_mode_from_refresh
    ...
    def delete_property(self, request: ValidatedRequest, pk=None, **kwargs) -> response.Response:
        ...
        with personinsights_caller_tag("persons/delete-property"):
    ...
    def cohorts(self, request: request.Request, **kwargs) -> response.Response:
        from insights.api.cohort import CohortMinimalSerializer
        ...
        with personinsights_caller_tag("persons/cohorts"):
    ...
    def _message_assets_response(self, request: request.Request, kind: str) -> response.Response:
        ...
        # InsightsFlow.id is a UUID column; Datastore function_id is a plain string, so coerce
    ...
    def batch_by_distinct_ids(self, request: request.Request, *args: Any, **kwargs: Any) -> response.Response:
        ...
        with personinsights_caller_tag("persons/batch-by-distinct-ids"):
    ...
    def batch_by_uuids(self, request: request.Request, *args: Any, **kwargs: Any) -> response.Response:
        ...
        with personinsights_caller_tag("persons/batch-by-uuids"):
