...
from insights.insightsql_queries.ai.utils import HEAVY_PROPERTY_NAMES
...
def execute_hog_eval_bytecode(bytecode: list, globals_dict: dict[str, Any], allows_na: bool) -> dict[str, Any]:
    ...
    was a bug in our code rather than in the user's Script source.
    """
    try:
        ...
        logger.exception("Unexpected error executing Script eval bytecode")
        return {
            "verdict": None,
            "reasoning": "",
    ...
    if result["error"]:
        if result.get("unexpected"):
            ...
            raise ApplicationError(
                f"Script evaluation error ({unit_label}): {result['error']}",
                non_retryable=True,
            )
...
def run_hog_eval(bytecode: list, event_data: dict[str, Any], allows_na: bool = False) -> dict[str, Any]:
    ...
    ScriptVM error — i.e. a bug in our code rather than in the user's Script source.
    """
...
async def execute_hog_eval_activity(evaluation: dict[str, Any], event_data: dict[str, Any]) -> EvaluationActivityResult:
    ...
    if result["error"]:
        ...
        # The user's Script source itself errored (invalid code, execution timeout, or a
