...
describe('shim consumer integration', () => {
    beforeEach(() => {
        // The toolbar runs on third-party pages without an authenticated Insights user, so
        // mascotModeLogic.values.user must be null. We pre-set current_user: null here because
    ...
    describe('mascotModeLogic with shims', () => {
        ...
        it('mounts without error and has shimmed defaults', async () => {
            const { mascotModeLogic } = await import('~/lib/components/MascotMode/mascotModeLogic')
            const logic = mascotModeLogic.build()

            expect(() => logic.mount()).not.toThrow()
            expect(logic.values.mascotMode).toBeNull()
        ...
        it('does not PATCH mascot_config from the Toolbar, keeping the session alive', async () => {
            ...
            // session reset, which used to log the user out of the Toolbar whenever mascot mode
            // moved the mascot around. Swap in our own mock and let afterEach restore the original.
            ...
            // The change is still applied locally so the mascot reflects it this session.
