...
from insights.kafka_client.routing import flush_all_producers
from insights.models.person.deletion import OrphanRepairResult, find_orphaned_ch_persons, tombstone_orphaned_ch_persons
...
class Command(BaseCommand):
    help = (
        "Tombstone Datastore person rows that are live in Datastore but absent from the persons DB "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--all",
            action="store_true",
            help="Scan every live Datastore person for the team (full scan). Mutually exclusive with --person-uuid.",
        )
        parser.add_argument(
            "--dry-run",
...
def run(options) -> None:
    ...
    if uuids:
        ...
        if not_orphaned:
            ...
            # or absent from Datastore entirely. Never tombstoned by this command.
            logger.warning(
                "Some requested UUIDs are not orphaned (live in persons DB or absent from Datastore); skipping them",
                team_id=team_id,
    ...
    if undelivered:
        logger.error(
            "Kafka flush left messages undelivered — tombstones may not have landed in Datastore",
            team_id=team_id,
...
def _log_result(team_id: int, result: OrphanRepairResult) -> None:
    ...
    logger.info(
        f"{verb} orphaned Datastore persons",
        team_id=team_id,
    ...
    if result.reverse_drift_mappings:
        logger.warning(
            "Found distinct_ids tombstoned in Datastore whose person is still live in the persons DB "
