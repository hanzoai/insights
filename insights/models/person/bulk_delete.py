...
from insights.models.activity_logging.activity_log import Detail, LogActivityEntry, bulk_log_activity
...
from insights.temporal.session_replay.delete_recordings.types import DeletionConfig, RecordingsWithPersonInput
...
def resolve_persons_for_deletion(
    ...
    """Materialize Persons matching either uuids or distinct_ids, via personinsights."""
    ...
    def _fetch() -> builtins.list[Person]:
        if uuids:
            return _fetch_persons_by_uuids_via_personinsights(team_id, uuids)
        return _fetch_persons_by_distinct_ids_via_personinsights(team_id, cast(builtins.list[str], distinct_ids))

    return personinsights_call("resolve_persons_for_deletion", _fetch, caller_tag="persons/deletion-resolve")
...
def delete_persons_profile(
    ...
    *,
    ...
    """Run Datastore Kafka tombstones, then a single Postgres batch delete.
...
# Batch persons per workflow (not one each) to bound concurrent Datastore load queries; cap distinct IDs per chunk to stay under Temporal's payload limit.
