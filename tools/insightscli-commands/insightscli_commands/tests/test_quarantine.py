...
from insightscli_commands.quarantine import core
from insightscli_commands.quarantine.cli import quarantine
from insightscli_commands.quarantine.pytest_support import apply_quarantine_markers
...
@pytest.mark.parametrize(
    ...
    [
        # exact nodeid
        ("insights/api/test/test_foo.py::TestFoo::test_bar", "insights/api/test/test_foo.py::TestFoo::test_bar", True),
        # function selector covers parameterized variants
        (
            "insights/api/test/test_foo.py::TestFoo::test_bar",
            "insights/api/test/test_foo.py::TestFoo::test_bar[a-1]",
            True,
        ),
        # class prefix
        ("insights/api/test/test_foo.py::TestFoo", "insights/api/test/test_foo.py::TestFoo::test_other", True),
        # file prefix
        ("insights/api/test/test_foo.py", "insights/api/test/test_foo.py::TestBar::test_x", True),
        # directory prefix, with and without trailing slash
        ("insights/api/test", "insights/api/test/test_foo.py::TestFoo::test_bar", True),
        ("insights/api/test/", "insights/api/test/test_foo.py::TestFoo::test_bar", True),
        # partial names never match
        ("insights/api/test", "insights/api/test_organization.py::test_x", False),
        (
            "insights/api/test/test_foo.py::TestFoo::test_bar",
            "insights/api/test/test_foo.py::TestFoo::test_barbell",
            False,
        ),
        ...
        ("product:batch-exports", "insights/api/test/test_foo.py::test_x", False),
        # unrelated paths
        ("insights/api/test/test_foo.py", "insights/api/test/test_food.py::test_x", False),
...
def test_parse_skips_broken_entry_but_keeps_good_ones(broken: Any) -> None:
    good = {"id": "insights/a.py", "added": "2026-06-10", "expires": "2026-06-20"}
    result = core.parse(json.dumps({"version": 1, "entries": [broken, good]}))
    assert [e.id for e in result.entries] == ["insights/a.py"]
    assert len(result.errors) == 1


def test_parse_warns_on_unknown_field_and_preserves_it_on_render() -> None:
    entry = {"id": "insights/a.py", "added": "2026-06-10", "expires": "2026-06-20", "future_field": "x"}
...
def test_adapter_marks_matching_item_xfail(tmp_path: Path) -> None:
    ...
    item = FakeItem("insights/api/test/test_foo.py::TestFoo::test_bar")
    other = FakeItem("insights/api/test/test_other.py::test_x")
...
def test_adapter_most_specific_selector_wins(tmp_path: Path) -> None:
    entries = [
        raw_entry(id="insights/api/test/test_foo.py", mode="run"),
        raw_entry(id="insights/api/test/test_foo.py::TestFoo::test_bar", mode="skip"),
    ]
    path = write_file(tmp_path / "q.json", entries)
    narrow = FakeItem("insights/api/test/test_foo.py::TestFoo::test_bar")
    broad = FakeItem("insights/api/test/test_foo.py::TestFoo::test_other")
...
def test_add_creates_canonical_file(runner: CliRunner, tmp_path: Path) -> None:
    path = tmp_path / "q.json"
    result = cli(runner, path, "add", "insights/api/test/test_foo.py", "--reason", "flaky", "--owner", "@team-devex")
    assert result.exit_code == 0, result.output
    text = path.read_text()
    assert text.endswith("\n")
    data = json.loads(text)
    entry = data["entries"][0]
    assert entry["id"] == "insights/api/test/test_foo.py"
...
def test_remove_absent_id_succeeds(runner: CliRunner, tmp_path: Path) -> None:
    path = write_file(tmp_path / "q.json", [raw_entry()])
    result = cli(runner, path, "remove", "insights/never/was/quarantined.py")
    assert result.exit_code == 0
