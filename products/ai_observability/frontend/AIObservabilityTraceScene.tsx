...
function QueueTraceNavigationButtons({
    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            {showBackToQueue ? (
                <Button type="secondary" size="xsmall" to={getTraceReviewsBackPath(searchParams)}>
                    Back to queue
                </Button>
            ) : null}
            {!previousTraceId ? (
            ...
            primaryButton: {
                ...
                onClick: async () => {
                    ...
                    try {
                        ...
                        setQueueAssignment(null)
                        toast.success(`Removed from "${queueAssignment.queueName}".`)
                    } catch {
                        toast.error('Failed to remove the trace from its queue.')
                    } finally {
