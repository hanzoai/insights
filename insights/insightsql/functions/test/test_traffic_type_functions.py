...
class TestBotIPDefinitionsDataStructure:
    ...
    @parameterized.expand(
        [
            ...
            # rendering service crawling with a real Android UA (insights#66604).
            "66.249.84.5",
            "74.125.150.10",
            "192.178.10.5",
        ]
...
class TestMacroExpansionGuard(BaseTest):
    ...
    def test_matches_action_with_macro_property_still_resolves(self):
        # matchesAction expands a user-defined action, whose insightsql property filters re-parse
        # arbitrary user InsightsQL. A guarded macro referenced there must still expand — the guard
            ...
            steps_json=[
                {
                    "event": "$pageview",
                    "properties": [
                        {"type": "insightsql", "key": "__preview_getTrafficType(properties.$user_agent) = 'Bot'"}
                    ],
                }
            ],
        )
