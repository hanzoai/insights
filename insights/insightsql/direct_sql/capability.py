...
def direct_supports_insightsql(source: ExternalDataSource) -> bool:
    """Whether InsightsQL compiles for this source's direct engine (False means raw SQL only)."""
    if source.direct_engine is None:
        return False
    # Function-local: the registry is populated by the package root, which pulls the drivers.
    from insights.insightsql.direct_sql.registry import get_adapter  # noqa: PLC0415
