...
use personinsights_common::grpc::{tracked_tcp_incoming, GrpcMetricsLayer};
...
use personinsights_router::backend::discovery::{EndpointConfig, EndpointDiscovery};
use personinsights_router::backend::{
    LeaderBackend, LeaderBackendConfig, ReplicaBackend, ReplicaDnsConfig, StashTable,
};
use personinsights_router::config::{Config, ReplicaDiscoveryMode, RouterMode};
use personinsights_router::proxy::RawProxyService;
use personinsights_router::stash_handler::RouterStashHandler;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Starting personinsights-router service");
    ...
    let mut manager = Manager::builder("personinsights-router")
    ...
    // Create backend connection(s) to personinsights-replica
    ...
    tokio::spawn(async move {
        ...
        let recorder_handle = PrometheusBuilder::new()
            .add_global_label("service", "personinsights-router")
            .set_buckets(BUCKETS)
            .unwrap()
            .set_buckets_for_metric(
                metrics_exporter_prometheus::Matcher::Prefix(
                    "personinsights_router_response_size".into(),
                ),
            ...
            .set_buckets_for_metric(
                metrics_exporter_prometheus::Matcher::Full(
                    "personinsights_coordination_handoff_phase_reached_ms".into(),
                ),
                &[
            ...
            .set_buckets_for_metric(
                metrics_exporter_prometheus::Matcher::Full(
                    "personinsights_coordination_handoff_phase_duration_ms".into(),
                ),
                &[
                    50.0, 250.0, 1000.0, 2000.0, 5000.0, 10000.0, 30000.0, 60000.0, 120000.0,
                    300000.0, 600000.0,
...
fn preregister_metrics() {
    use metrics::counter;
    counter!("personinsights_router_stash_enqueued_total").increment(0);
    counter!("personinsights_router_stash_replayed_total").increment(0);
    counter!("personinsights_router_forward_retries_exhausted_total").increment(0);
    for outcome in ["success", "error", "expired"] {
        counter!("personinsights_router_stash_drained_total", "outcome" => outcome).increment(0);
    }
    for cause in ["max_messages", "max_bytes"] {
        counter!("personinsights_router_stash_rejected_total", "cause" => cause).increment(0);
    }
    counter!("personinsights_router_stash_dropped_total", "reason" => "receiver_gone").increment(0);
    for reason in ["unrouted", "fenced", "transport"] {
        counter!("personinsights_router_forward_retries_total", "path" => "direct", "reason" => reason)
            .increment(0);
    }
    for reason in ["unrouted", "fenced", "transport", "cancelled"] {
        counter!("personinsights_router_forward_retries_total", "path" => "stash", "reason" => reason)
            .increment(0);
    }
    personinsights_coordination::preregister_router_coordination_metrics();
}
