...
class BatchExportDestination(UUIDTModel):
    ...
    class Meta:
        db_table = "insights_batchexportdestination"
...
class BatchExportSource(TeamScopedRootMixin, UUIDTModel):
    """A model for the source of the data that a Insights BatchExport will export.

    This model answers the question: what data are we exporting? For now it holds a
    InsightsQL query whose results are exported, but it's designed to grow into the single
    ...
    class Meta:
        db_table = "insights_batchexportsource"

    # `db_constraint=False`: a real FK constraint to the hot `insights_team` table would
    ...
    insightsql_query = models.TextField(
        null=True,
        blank=True,
        help_text="The InsightsQL query whose results are exported.",
    )
    created_at = models.DateTimeField(
        auto_now_add=True,
...
class BatchExportRun(UUIDTModel):
    """A single run of a Insights batch export defined by its data interval bounds.
    ...
    class Meta:
        db_table = "insights_batchexportrun"
        constraints = [
    ...
    records_failed = models.IntegerField(
        null=True,
        help_text="The number of records that failed downstream processing (e.g. script function execution errors).",
    )
...
class BatchExport(ModelActivityMixin, UUIDTModel):
    """A model for a Insights batch export that runs on a schedule.

    A batch export exports data from Insights to the configured destination on a
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, help_text="The team this belongs to.")
...
class BatchExportBackfill(UUIDTModel):
    class Meta:
        db_table = "insights_batchexportbackfill"
...
class BatchExportFileDownload(ModelActivityMixin, UUIDTModel):
    ...
    class Meta:
        db_table = "insights_batchexportfiledownload"
        indexes = [
...
class BatchExportOnDemand(TeamScopedRootMixin, ModelActivityMixin, UUIDTModel):
    """A model for a Insights batch export triggered on demand.
    ...
    class Meta:
        db_table = "insights_batchexportondemand"
