...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["insights/management/commands/backfill_distinct_id_overrides.py"],
            id="backfill-management-command-not-migration",
        ),
        pytest.param(
            ["insights/dags/backfill_materialized_column.py"],
            id="dagster-backfill-dag-not-migration",
        ),
        pytest.param(
            ["insights/management/commands/migrate_team.py"],
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ["insights/api/authentication.py"],
            "auth",
        ...
        pytest.param(
            ["insights/api/login.py"],
            "auth",
            id="login-endpoint",
        ),
        pytest.param(
            ["insights/models/oauth_config.py"],
            "auth",
            id="oauth-config",
        ),
        pytest.param(
            ["insights/api/auth/session_token.py"],
            "auth",
            id="auth-session-token-path",
        ),
        pytest.param(
            ["insights/crypto/encrypt.py"],
            "crypto_secrets",
            id="encryption-file",
        ),
        pytest.param(
            ["insights/settings/.env.example"],
            "crypto_secrets",
            id="dot-env-file",
        ),
        pytest.param(
            ["insights/api/api_key.py"],
            "crypto_secrets",
            id="api-key-file",
        ),
        pytest.param(
            ["insights/models/secret_key_store.py"],
            "crypto_secrets",
            id="secret-key-file",
        ),
        pytest.param(
            ["insights/migrations/0400_add_column.py"],
            "migrations",
            id="migration-file",
        ),
        pytest.param(
        ...
        pytest.param(
            ["insights/models/two_factor_auth.py"],
            "auth",
            id="two-factor-auth-file",
        ),
        pytest.param(
            ["insights/billing/stripe_webhook.py"],
            "billing",
            id="billing-file",
        ),
        pytest.param(
...
def test_ignored_files_bypass_deny_list() -> None:
    files = [
        "insights/migrations/1117_alter_integration_kind.py",
        "insights/migrations/max_migration.txt",
    ]
    ignored = set(files)
...
def test_ignored_files_does_not_bypass_other_deny_list_files() -> None:
    files = [
        "insights/migrations/1117_alter_integration_kind.py",
        "insights/migrations/1118_add_column.py",
    ]
    ignored = {"insights/migrations/1117_alter_integration_kind.py"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("insights/api/test/__snapshots__/test_api.ambr", True, id="ambr-snapshot"),
        ...
        pytest.param("insights/test/__snapshots__/helper.py", True, id="snapshots-inside-test-dir-exempt"),
        ...
        pytest.param("insights/api/insight.py", False, id="python-code"),
        pytest.param("frontend/src/scenes/insights/Insight.tsx", False, id="frontend-code"),
        pytest.param("insights/settings/web.py", False, id="settings-code"),
        ...
        pytest.param("insights/api/test/test_insight.py", True, id="test-dir-exempt"),
        pytest.param("frontend/src/scenes/insights/Insight.test.tsx", True, id="dot-test-exempt"),
        pytest.param("insights/personinsights_client/test_interceptor.py", True, id="bare-pytest-file-exempt"),
        pytest.param(
        ...
        pytest.param("insights/tasks/protest.py", False, id="test-suffix-substring-counted"),
        pytest.param("insights/latest/models.py", False, id="latest-dir-counted"),
        ...
        pytest.param(
            "insights/temporal/ingestion_acceptance_test/worker.py", False, id="runtime-worker-test-suffix-counted"
        ),
    ],
)
...
def test_substantive_size_counts_only_non_exempt_files() -> None:
    ...
    files = [
        ...
        {"filename": "insights/api/insight.py", "additions": 30, "deletions": 10},
        {"filename": "insights/api/test/test_insight.py", "additions": 50, "deletions": 0},
    ]
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("owners:\n  - team-devex\n", ["@Insights/team-devex"], [], 1, id="plain-slug-prefixed"),
...
def test_ownership_cross_team_and_unowned(tmp_path: Path) -> None:
    ...
    outside = detect_ownership(["insights/models/x.py"], resolvers)
