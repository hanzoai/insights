...
# usable both as in-repo packaged skills (consumed by `insightscli build:skills` for the AI plugin
...
# Mirrors the regex in `products/insights_ai/scripts/build_skills.py` so frontmatter parsing
...
# `products/insights_ai/scripts/build_skills.py` — diverging here means a file format
# `insightscli build:skills` ignores would silently land in the team's `LLMSkillFile` rows
