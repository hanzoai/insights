...
1. Confirm the event actually captures: `insights:read-data-schema` for its shape, a quick `insights:execute-sql` for volume.
   No data yet? Wait until it flows — a scout can't baseline an empty stream.
2. Check the roster (`insights:scout-config-list` descriptions): if the event belongs to a surface a specialist already watches (an error, a survey response, a flag call), a **note** telling that scout about the new event is enough.
...
1. Find the specialist in `insights:scout-config-list` and confirm it's `enabled` with `emit: true` — and that the project can emit at all (`emit_eligibility.can_emit` on `insights:scout-project-profile-get`; when false, writes are silently dropped fleet-wide; a 404 just means no fresh cached profile, so fall back to `insights:inbox-source-configs-list` and treat eligibility as unknown).
...
## "Watch something outside Insights" (vendor status page, docs, a competitor)

Scouts run in a sandbox that defaults to a trusted-domain allowlist (Insights, GitHub, package registries).
...
5. Right signal, wrong volume? Slow it down: `insights:scout-config-update` with a larger `run_interval_minutes` — and if the config has a `run_cron_schedule`, clear or update it too, since a cron schedule takes precedence over the interval.
...
1. Confirm the project is still enrolled (`insights:scout-metadata-get` — config rows outlive enrollment, so a full-looking roster can belong to a drained fleet), that the fleet is actually on (`insights:scout-config-list`: `enabled`, `emit` — dry-run scouts write nothing), and that runs execute (`exploring-scouts` health check).
2. Read a few run summaries: a scout narrating "surface at baseline" is working; if it keeps saying "no data for X", the watched surface may not capture data (`insights:scout-project-profile-get` shows what's in use).
...
4. Check the inbox default view isn't hiding output: suppressed reports (judged not-actionable) don't surface — `insights:inbox-reports-list` with `status: "suppressed"` plus the `scout: "<skill_name>"` filter shows whether _this_ scout is finding things that get filtered (without the scout filter you'd be reading the whole project's suppressed reports).
...
1. Create it with **both** `enabled: false` and `emit: false` in the nested config at `insights:scout-create-prepare` time.
...
2. Spend one `insights:scout-run-now` (it works on a disabled scout), then read the run via `exploring-scouts` to see what it would have written.
