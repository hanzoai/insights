...
def _build_insert_query(actions: Sequence) -> str:
    """Synthesize the InsightsQL INSERT template for the given action set.
    ...
    # tuple types unify cleanly across the array. Datastore requires
    ...
    # `countIf(...)` (UInt64) would force Datastore into type-unification
    ...
    # live path. (InsightsQL exposes `toInt`, which compiles to Datastore's
    # `toInt64`; `toInt64` itself is not a valid InsightsQL identifier.)
...
FROM insights.web_goals_preaggregated
...
def execute_read_query(
    *,
    ...
    """Run the precompute-read InsightsQL. Returns raw `response.results`."""
...
def execute_lazy_precomputed_read(runner: "WebGoalsQueryRunner") -> Optional[dict]:
    ...
    (caller falls through to the live InsightsQL path).
