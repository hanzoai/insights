 ...
 *                     node_modules/@hanzo/quill/dist, so it scans our
...
const tokensSource = resolve(packageRoot, 'node_modules/@hanzo/quill-tokens/dist/tailwind-lib.css')
const colorsSource = resolve(packageRoot, 'node_modules/@hanzo/quill-tokens/dist/color-system.css')
const tokensScopedSource = resolve(packageRoot, 'node_modules/@hanzo/quill-tokens/dist/tailwind-lib.scoped.css')
const colorsScopedSource = resolve(packageRoot, 'node_modules/@hanzo/quill-tokens/dist/color-system.scoped.css')
/*
 ...
 */
const primitivesSource = resolve(packageRoot, 'node_modules/@hanzo/quill-primitives/dist/quill-primitives.css')

for (const path of [tokensSource, colorsSource, tokensScopedSource, colorsScopedSource, primitivesSource]) {
    if (!existsSync(path)) {
        throw new Error(
            `Cannot build quill CSS: ${path} missing. Run @hanzo/quill-tokens and ` +
                `@hanzo/quill-primitives builds first ` +
                `(\`pnpm --filter '@hanzo/quill...' build\`).`
        )
    }
}
...
if (themeWithDefaults === theme) {
    throw new Error(
        'build-css: failed to inject Tailwind defaults into tailwind-lib.css — ' +
            ...
            '`@hanzo/quill-tokens build` and update the regex if the format changed.'
    )
}
...
// `@hanzo/quill/primitives.css` to register the `.quill-*` component rules.
...
writeFileSync(
    ...
    [
        '/* @hanzo/quill — scoped design tokens.',
