...
# Calibrated on observed week-over-week p95 movement in Insights/insights: stable high-volume
...
def detect_all(curated: CuratedGitHubSource) -> list[CISignalFinding]:
    ...
    if len(errors) == len(detectors):
        # Every detector failing (Datastore outage, schema drift) must fail the activity so it
