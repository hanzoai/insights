...
from insights.insightsql.escape_sql import escape_insightsql_string
...
def escape_insightsql_like_pattern(value: str) -> str:
    pattern = value.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
    return escape_insightsql_string(f"%{pattern}%")
...
def add_release_filter(filters: list[dict[str, object]], release: str) -> None:
    escaped_release = escape_insightsql_string(release)
...
def build_fingerprint_where(fingerprints: list[str]) -> list[str]:
    if not fingerprints:
        return ["1 = 0"]
    escaped_fingerprints = ", ".join(escape_insightsql_string(fingerprint) for fingerprint in fingerprints)
...
def add_event_search_where(where: list[str], search_query: str | None) -> list[str]:
    if search_query:
        search = escape_insightsql_like_pattern(search_query)
