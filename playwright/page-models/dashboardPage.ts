...
export class DashboardPage {
    ...
    async createFromTemplate(): Promise<DashboardPage> {
        ...
        // New dashboard modal uses DialogPrimitive, not Modal (see NewDashboardModal.tsx).
    ...
    async addInsightToNewDashboard(insightName?: string): Promise<void> {
        ...
        const row = insightName
            ? this.page.locator('.Modal .Table tbody tr').filter({ hasText: insightName }).first()
            : this.page.locator('.Modal .Table tbody tr').first()
        await row.click()
    ...
    async addTextCard(text: string): Promise<void> {
        ...
        // Text card edit UI uses DialogPrimitive, not Modal (see TextCardModal.tsx).
    ...
    async duplicate(): Promise<void> {
        ...
        const modal = this.page.locator('.Modal').filter({ hasText: 'Duplicate dashboard' })
        await expect(modal).toBeVisible()
    ...
    async deleteDashboard(): Promise<void> {
        ...
        const modal = this.page.locator('.Modal').filter({ hasText: 'Delete dashboard' })
        await expect(modal).toBeVisible()
