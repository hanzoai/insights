...
from insights.insightsql.ast import AST, Constant, StringType
...
from insights.insightsql.database.schema.events import EVENTS_TABLE_TYPES
...
from insights.insightsql.escape_sql import (
    ...
    quote_datastore_identifier,
    safe_identifier,
)
from insights.insightsql.functions import ADD_OR_NULL_DATETIME_FUNCTIONS, FIRST_ARG_DATETIME_FUNCTIONS
from insights.insightsql.functions.embed_text import resolve_embed_text
from insights.insightsql.functions.udfs import JSON_DROP_KEYS_DATASTORE_NAME
from insights.insightsql.helpers.timestamp_visitor import parse_zoned_datetime_string
from insights.insightsql.printer.base import BasePrinter, get_channel_definition_dict, resolve_field_type
from insights.insightsql.printer.insightsql import InsightsQLPrinter
from insights.insightsql.restricted_properties import restricted_property_keys_for_table_type
from insights.insightsql.type_system import parse_sql_runtime_type
from insights.insightsql.visitor import GetFieldsTraverser, clone_expr

from insights.datastore.events_json import EVENTS_PROPERTIES_JSON_SUBCOLUMNS, PERSON_PROPERTIES_JSON_SUBCOLUMNS
from insights.exchange_rate_constants import EXCHANGE_RATE_DECIMAL_PRECISION, EXCHANGE_RATE_DICTIONARY_NAME
...
def team_id_guard_for_table(table_type: ast.TableOrSelectType, context: InsightsQLContext) -> ast.Expr:
    """Add a mandatory "and(team_id, ...)" filter around the expression."""
    if not context.team_id:
        raise InternalInsightsQLError("context.team_id not found")
...
# wrapping — so the index stays usable. Canonical set; Datastore property resolution imports it to make the same call.
...
class DatastorePrinter(BasePrinter):
    DIALECT_NAME: ClassVar[InsightsQLDialect] = "datastore"

    def visit_cte(self, node: ast.CTE):
        if node.materialized is False:
            raise ImpossibleASTError("Datastore does not support NOT MATERIALIZED CTEs")
    ...
    def _limit_percent_constant_value(self, limit: ast.Expr) -> float:
        if not isinstance(limit, ast.Constant) or not isinstance(limit.value, (int, float)):
            raise QueryError("LIMIT percent with expressions is not supported in datastore dialect")
    ...
    def _validate_within_group_for_aggregation(self, node: ast.Call, func_meta) -> None:
        if node.within_group is not None:
            raise QueryError(f"Aggregation '{node.name}' with WITHIN GROUP is not supported in Datastore dialect")
    ...
    def _render_function_call(self, node: ast.Call, func_meta) -> str:
        ...
        return f"{relevant_datastore_name}{params_part}{args_part}{filter_part}"
    ...
    def visit_select_query(self, node: ast.SelectQuery):
        if not self.context.enable_select_queries:
            raise InternalInsightsQLError("Full SELECT queries are disabled if context.enable_select_queries is False")
        if not self.context.team_id:
            raise InternalInsightsQLError("Full SELECT queries are disabled if context.team_id is not set")

        return super().visit_select_query(node)

    def visit_join_expr(self, node: ast.JoinExpr):
        if node.type is None:
            raise InternalInsightsQLError("Printing queries with a FROM clause is not permitted before type resolution")

        return super().visit_join_expr(node)

    def visit_values_query(self, node: ast.ValuesQuery):
        raise QueryError("VALUES clause is not supported in Datastore dialect")
    ...
    def visit_interpolate_expr(self, node: ast.InterpolateExpr):
        # Datastore requires backtick-quoted column references in INTERPOLATE clauses
        printed_expr = self.visit(node.expr)
        quoted_expr = escape_datastore_identifier(printed_expr)
    ...
    def visit_field(self, node: ast.Field):
        if node.type is None:
            field = ".".join([self._print_insightsql_identifier_or_index(identifier) for identifier in node.chain])
    ...
    def _maybe_apply_json_drop_keys(self, type: ast.FieldType, field_sql: str) -> str:
        ...
        # but Datastore resolves it back to the original column. Comparing ``type.name`` here
        ...
        return f"{JSON_DROP_KEYS_DATASTORE_NAME}({keys_placeholder})({field_sql})"
            ...
            if isinstance(field.type, ast.FieldType):
                ...
                while True:
                    if isinstance(table_type, ast.TableType):
                        table_name = table_type.table.to_printed_insightsql()
                        if table_name in (
                            "events",
                            "raw_sessions",
                            "raw_sessions_v3",
    ...
    def visit_compare_operation(self, node: ast.CompareOperation):
        ...
        # property-based skip-index rewrite (materialized column, property group) now runs in Datastore property
        # resolution, which emits the optimized form as AST before printing (see datastore_property_resolution.py).
        ...
        # :HACK: until the new type system is out: https://github.com/Insights/insights/pull/17267
        ...
        # Datastore can't parse datetime strings ending in 'Z' or an offset, so print those as datetime literals.
        ...
        if node.op == ast.CompareOperationOp.Eq:
            ...
            # With transform_null_in=1, Datastore rewrites notIn() to notNullIn().
            ...
            raise InternalInsightsQLError("Cohort operations should have been resolved before printing")
        else:
    ...
    def visit_arithmetic_operation(self, node: ast.ArithmeticOperation):
        # Datastore's plain divide() derives a decimal result scale of (dividend_scale - divisor_scale).
        ...
        # path in _render_postinsights_function_call which uses divideDecimal for the same reason.
    ...
    def visit_call(self, node: ast.Call):
        ...
        # Property-group call optimizations (isNull/isNotNull/JSONHas over a property-group key) now run in Datastore
        # property resolution, which rewrites them to the keys-index `has(group, key)` form before printing.
        # The type-name argument reaches Datastore's type parser verbatim, so bound it to
    ...
    def visit_insightsqlx_tag(self, node: ast.InsightsQLXTag):
        raise QueryError("Printing InsightsQLX tags is only supported in InsightsQL queries")

    def visit_insightsqlx_attribute(self, node: ast.InsightsQLXAttribute):
        raise QueryError("Printing InsightsQLX tags is only supported in InsightsQL queries")
    ...
    def _ensure_team_id_where_clause(
        ...
        if isinstance(table_type.table, DirectSQLTable):
            ...
            # Datastore is both our own dialect and a direct engine, so refuse to print a direct table into a
            ...
            if not self.context.is_direct_query:
                raise QueryError(
                    f'Table "{table_type.table.to_printed_insightsql()}" can only be queried through its direct connection.'
                )
            return None
    ...
    def _ensure_access_control_where_clause(
        ...
        from insights.insightsql.printer.access_control import build_access_control_guard
    ...
    def _print_table_ref(self, table_type: ast.TableType | ast.LazyTableType, node: ast.JoinExpr) -> str:
        ...
        if hasattr(table, "to_printed_datastore_table_ref"):
            use_logical_alias = not isinstance(node.type, (ast.TableAliasType, ast.ColumnAliasedTableType))
            sql = table.to_printed_datastore_table_ref(self.context, use_logical_alias=use_logical_alias)
        else:
            sql = table.to_printed_datastore(self.context)
        ...
        for column in columns:
            ...
            if isinstance(column, ast.Call) and not dropped_hidden_alias:
                with self.context.timings.measure("printer"):
                    column_alias = safe_identifier(InsightsQLPrinter(context=self.context).visit(column))
                # Datastore rejects duplicate aliases for different expressions in the
    ...
    def visit_property_type(self, type: ast.PropertyType) -> str:
        ...
        # Struct columns (e.g. Parquet structs from the data warehouse) are backed by a Datastore
        # Tuple, not a JSON string. Emit chained tupleElement() calls instead of JSONExtractRaw(),
        # which Datastore rejects on Tuple arguments. Closes #58480.
    ...
    def visit_json_subcolumn_access(self, node: ast.JsonSubcolumnAccess) -> str:
        ...
        if any("%" in key for key in node.keys):
            if node.access_type == "sub_object":
                for key in node.keys:
                    subcolumn = f"^{quote_datastore_identifier(key)}"
        ...
        for index, key in enumerate(node.keys):
            ...
            expr = f"{expr}{separator}{escape_datastore_identifier(key)}"
        return expr
