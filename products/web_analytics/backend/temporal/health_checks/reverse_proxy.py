...
class ReverseProxyCheck(HealthCheck):
    ...
    remediation = Remediation(
        human="""
            ...
            https://example.com/ingest) to Insights, then point your SDK's `api_host` at that proxy URL and
            redeploy. Insights supports several options (managed reverse proxy, Cloudflare, Vercel, AWS
            CloudFront, nginx, etc.).
        """,
        agent="""
            Confirm traffic isn't proxied with `execute-sql` (check recent events'
            `properties.$lib_custom_api_host`). Then either stand up a Insights-managed reverse proxy with
            ...
            `api_host` (and `ui_host`) in the `insights.init` call to the proxied URL and redeploy. Use
    ...
    def render_signal(cls, issue: HealthIssue) -> SignalContent | None:
        ...
        return SignalContent(
            description=(
                "Insights isn't receiving events through a reverse proxy for this project. "
