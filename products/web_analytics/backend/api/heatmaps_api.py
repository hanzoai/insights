...
from insights.schema import DateRange, InsightsQLFilters, InsightsQLQueryResponse, ProductKey

from insights.insightsql import ast
from insights.insightsql.ast import Constant
from insights.insightsql.base import Expr
from insights.insightsql.constants import MAX_SELECT_HEATMAPS_LIMIT, LimitContext
...
from insights.auth import ExportRendererAuthentication
...
from insights.permissions import AccessControlPermission, is_service_auth
...
from insights.rbac.user_access_control import AccessControlLevel, UserAccessControlSerializerMixin
...
def anchor_url_pattern(value: str) -> str:
    """Anchor (and loosen unescaped `*`) a raw url_pattern the same way the heatmaps Datastore
...
class HeatmapAggregateQueryScopingPermission(AccessControlPermission):
    ...
    `list`/`events` have no bindable object of their own — they're arbitrary Datastore
...
class HeatmapViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    def _compute_fold_summary(self, exprs: List[ast.Expr]) -> dict[str, Any]:  # noqa: UP006
        ...
        context = InsightsQLContext(team_id=self.team.pk, limit_top_select=False)
        result = execute_insightsql_query(query=stmt, team=self.team, limit_context=LimitContext.HEATMAPS, context=context)
    ...
    def _build_test_accounts_filter(self, date_from: date, date_to: date | None) -> ast.CompareOperation:
        ...
        # Pass the same bound as an explicit next-day-midnight datetime, which InsightsQLFilters uses verbatim with a
    ...
    def _return_heatmap_coordinates_response(
        query_response: InsightsQLQueryResponse, fold: dict[str, Any], has_more: bool
    ) -> response.Response:
        data = [
            {
...
class HeatmapPreflightResponseSerializer(serializers.Serializer):
    ...
    http_status = serializers.IntegerField(
        ...
        "rather than at Insights. Null when the page could not be reached at all.",
    )
...
class SavedHeatmapViewSet(
    ...
    @validated_request(
        ...
        "both cases explainable. The fetch comes from Insights's own network rather than from the screenshot "
