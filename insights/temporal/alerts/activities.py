...
from insights.schema import AlertState
...
from insights.tasks.alerts.investigation_notifications import run_investigation_notification_safety_net
from insights.tasks.alerts.metrics_investigation import run_metrics_alert_investigation, should_investigate_metrics_alert
from insights.tasks.alerts.schedule_restriction import is_utc_datetime_blocked, next_unblocked_utc
from insights.tasks.alerts.utils import (
...
from insights.temporal.alerts.investigation import claim_investigation_slot, should_trigger_investigation
...
async def evaluate_alert(inputs: EvaluateAlertActivityInputs) -> EvaluateAlertResult:
    """Run the insight Datastore query, apply the state machine, persist an AlertCheck row."""
    ...
    def _evaluate() -> EvaluateAlertResult:
        ...
        # (real_time vs every_15_minutes vs ...) and query shape (trends vs InsightsQL) without a join.
        tag_queries(
        ...
        # Datastore queries and must never hold the row lock; a failure is
