...
function copyFile(from: string, to: string): void {
    try {
        ...
        // Silently fail if file doesn't exist (matches copy-insights-js script behavior)
...
function copyInsightsJsFiles(): void {
    const nodeModulesInsightsJs = resolve('.', 'node_modules/insights-js/dist')
    ...
    // Copy specific files (matching frontend/bin/copy-insights-js)
    const filesToCopy = [
        'array.js',
        'array.js.map',
        'array.full.js',
    ...
    console.info('✅ Copied insights-js files to dist/')
}

export function insightsJsPlugin(): Plugin {
    return {
        name: 'insights-js-copy',
        configureServer() {
            // Copy insights-js files when dev server starts
            copyInsightsJsFiles()
        },
        buildStart() {
