...
class TestMetricQueryRunner(DatastoreTestMixin, APIBaseTest):
...
class TestMetricsQueryAPI(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestAttributeField(DatastoreTestMixin, APIBaseTest):
    """End-to-end tests for the `attribute_field` helper.

    The helper builds an AST node; correctness depends on what Datastore
    actually returns, so we execute a real query against `insights.metrics`
    ...
    def _select_attribute(self, expr: ast.Expr, metric_name: str) -> str | None:
        query = parse_select(
            """
                SELECT {expr} AS value FROM insights.metrics
...
class TestRunMetricQueryFacade(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestMetricFilters(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestGroupBy(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestRateIncrease(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestHistogramQuantileRunner(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestMultiClauseAndFormulas(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
...
class TestMetricTypeIsolation(DatastoreTestMixin, APIBaseTest):
...
class TestNonFiniteAggregates(DatastoreTestMixin, APIBaseTest):
    """Datastore float aggregates can overflow to inf (two 1e308 samples in
