...
from insights.schema import ExperimentDataWarehouseNode, ExperimentFunnelMetric, MultipleVariantHandling, StepOrderValue

from insights.insightsql import ast
from insights.insightsql.parser import parse_expr, parse_select

from products.experiments.backend.insightsql_queries import MULTIPLE_VARIANT_KEY
from products.experiments.backend.insightsql_queries.base_query_utils import (
...
if TYPE_CHECKING:
    from products.experiments.backend.insightsql_queries.experiment_query_builder import ExperimentQueryBuilder
...
class FunnelQueryBuilder:
    ...
    def build_funnel_query_legacy(self) -> ast.SelectQuery:
        ...
        placeholders: dict[str, ast.Expr | ast.SelectQuery] = {
            ...
            "date_from": self._b.date_range_query.date_from_as_insightsql(),
            "date_to": self._b.date_range_query.date_to_as_insightsql(),
        }
        if self._b.metric_events_preaggregation_job_ids:
            ...
            placeholders["metric_events_date_from"] = self._b.date_range_query.date_from_as_insightsql()
            placeholders["metric_events_date_to"] = self._b.date_range_query.date_to_as_insightsql()
    ...
    def get_funnel_metric_events_query_for_precomputation(self) -> tuple[str, dict[str, ast.Expr]]:
        ...
        placeholders: dict[str, ast.Expr] = {
            ...
            **step_filter_placeholders,
            ...
            "experiment_date_from": self._b.date_range_query.date_from_as_insightsql(),
            "experiment_date_to": self._b.date_range_query.date_to_as_insightsql(),
    ...
    def build_funnel_steps_filter(self) -> ast.Expr:
        ...
        if conversion_window_seconds > 0:
            date_to = parse_expr(
                ...
                placeholders={
                    "to_date": self._b.date_range_query.date_to_as_insightsql(),
                    "conversion_window_seconds": ast.Constant(value=conversion_window_seconds),
                },
            )
        else:
        ...
        date_from = self._b._extend_date_from_for_funnel_cuped(self._b.date_range_query.date_from_as_insightsql())
    ...
    def build_funnel_events_subquery_for_union(
        ...
        if conversion_window_seconds > 0:
            date_to_expr = ast.Call(
                name="plus",
                args=[
                    self._b.date_range_query.date_to_as_insightsql(),
                    ast.Call(
            ...
            date_to_expr = self._b.date_range_query.date_to_as_insightsql()
        ...
        time_range_filter = ast.And(
            exprs=[
                ast.CompareOperation(
                    ...
                    right=self._b.date_range_query.date_from_as_insightsql(),
                ),
                ast.CompareOperation(
    ...
    def build_dw_step_predicate(
        ...
        date_from_expr = self._b.date_range_query.date_from_as_insightsql()
        date_to_expr = self._b.date_range_query.date_to_as_insightsql()
