...
class TestMCPProxyEndpoint(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    def setUp(self):
        super().setUp()
    ...
    @patch("products.mcp_store.backend.proxy.httpx.Client")
    def test_proxy_forwards_insights_namespace_headers(self, mock_client_cls):
        # Required for Insights MCP installs through the Store: without
        # `x-insights-mcp-consumer` reaching the upstream, single-exec mode
        # never resolves and `exec` comes back as "Tool exec not found".
        # The full `x-insights-*` namespace forwards so callers can also pass
        ...
        response = self.client.post(
            ...
            format="json",
            headers={
                "x-insights-mcp-consumer": "insights-code",
                "x-insights-mcp-mode": "cli",
                "x-insights-mcp-version": "2",
                "x-insights-project-id": "42",
                "x-insights-read-only": "true",
                "x-insights-custom-future-header": "anything",
                "x-not-insights-namespace": "should-not-be-forwarded",
            },
        )
        ...
        assert forwarded["x-insights-mcp-consumer"] == "insights-code"
        assert forwarded["x-insights-mcp-mode"] == "cli"
        assert forwarded["x-insights-mcp-version"] == "2"
        assert forwarded["x-insights-project-id"] == "42"
        assert forwarded["x-insights-read-only"] == "true"
        assert forwarded["x-insights-custom-future-header"] == "anything"
        assert "x-not-insights-namespace" not in forwarded
...
class TestMCPProxyToolApproval(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
...
class TestMCPProxyAccessControl(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_proxy_scopes_to_authenticated_user_not_installation_owner(self, mock_client_cls):
        ...
        other_user = User.objects.create_and_join(self.organization, "colleague@hanzo.ai", "password")
