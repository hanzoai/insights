 ...
 * in insights_healthissue); this page only renders the result. Copy, actions, and docs links are
...
const WEB_HEALTH_CHECKS: WebHealthCheckConfig[] = [
    {
        ...
        failingDescription: 'Complete the Insights installation to start seeing events in your dashboard.',
...
export interface webAnalyticsHealthLogicValues {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface webAnalyticsHealthLogicActions {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface webAnalyticsHealthLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        activeIssuesByKind: (healthIssues: HealthIssuesResponse | null) => Record<string, HealthIssue>
        allChecks: (
            activeIssuesByKind: Record<string, HealthIssue>,
            healthIssuesLoading: boolean,
            healthIssues: HealthIssuesResponse | null
...
export type webAnalyticsHealthLogicType = MakeLogicType<
    webAnalyticsHealthLogicValues,
    webAnalyticsHealthLogicActions,
    Record<string, any>,
    webAnalyticsHealthLogicMeta
>
        ...
        refreshHealthChecks: async ({ isManual }, breakpoint) => {
            ...
            try {
                ...
                if ((response?.scheduled_kinds ?? []).length === 0) {
                    if (isManual) {
                        toast.info('No health checks are registered for this project.')
                    }
                    return
                }

                if (isManual) {
                    toast.success('Refreshing health checks...', { autoClose: 2000 })
                }
