 ...
 *
 * Addressed by id and read from the logic rather than passed down, so that the callback `Markdown`
 * turns into its anchor component depends on where the charts go and not on what they contain. A
 * refresh that appends a new version of one chart would otherwise hand `Markdown` a new component
...
export function ReportChart({ chartId }: { chartId: string }): JSX.Element | null {
    ...
    return (
        <Card hoverEffect={false} className="flex flex-col gap-2 p-3" data-attr="report-chart">
