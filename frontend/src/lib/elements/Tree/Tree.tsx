...
export type TreeSelectMode = 'default' | 'multi' | 'folder-only' | 'all'
export type TreeSize = 'default' | 'narrow'
...
type TreeBaseProps = Omit<HTMLAttributes<HTMLDivElement>, 'onDragEnd'> & {
...
export type TreeNodeProps = TreeBaseProps & {
    /** The ID of the item. */
    selectedId?: string
    /** Handle a click on the item. */
    handleClick: (
    ...
    depth?: number
    /** Tell <Tree> to disable keyboard input */
                            ...
                            <ContextMenuTrigger asChild>
                                <Link
                                    ...
                                    buttonProps={{
                                        ...
                                        className: cn(
                                            'group/tree-button gap-[5px]',
                                            'relative z-1 focus-visible:bg-fill-button-tertiary-hover motion-safe:transition-[padding] duration-50 h-[var(--tree-button-height)] [&_.icon-shortcut]:size-3',
                                            {
                                                ...
                                                'group-hover/tree-button-group:bg-fill-button-tertiary-hover cursor-pointer':
                                                    !isEmptyFolder,
                                ...
                                >
                                    {size === 'default' && (
                                        <span
                                            ...
                                            className="h-[var(--tree-button-height)] bg-transparent pointer-events-none flex-shrink-0 transition-[width] duration-50 -ml-1.5"
                                            // eslint-disable-next-line react/forbid-dom-props
                                            style={{
                    ...
                    if (enableDragAndDrop && isItemDraggable?.(item) && item.id) {
                        button = (
                            <TreeNodeDraggable
                                id={item.id}
                                enableDragging
                                className="h-[var(--tree-button-height)]"
                            >
                                {button}
                            </TreeNodeDraggable>
                        )
                            ...
                            <AccordionPrimitive.Item value={item.id} className="flex flex-col w-full gap-y-px">
                                <AccordionPrimitive.Trigger className="flex items-center gap-2 w-full h-8" asChild>
                                    <ButtonGroupPrimitive
                                        fullWidth
                                        className="group/tree-button-group relative h-[var(--tree-button-height)] bg-transparent"
                                    >
                                        <TreeNodeDisplayCheckbox
                                            item={item}
                                                    ...
                                                    <DropdownMenuTrigger asChild>
                                                        {itemSideActionButton?.(item) ?? (
                                                            <ButtonPrimitive
                                                                iconOnly
                                                                isSideActionRight
                                                                className="z-2 opacity-0 group-hover/tree-button-group:opacity-100 data-[state=open]:opacity-100 transition-opacity"
                                                            >
                    ...
                    style={
                        {
                            ...
                            // for tree element
                            '--tree-button-height': 'var(--button-height-base)',
                            '--tree-button-icon-offset-top': '5px',
                        } as CSSProperties
                    }
                >
                    {mode === 'table' && (
