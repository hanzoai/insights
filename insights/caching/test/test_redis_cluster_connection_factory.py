...
from insights.caching.redis_cluster_connection_factory import (
    QUERY_CACHE_ALIAS,
...
class TestRedisClusterConnectionFactory(TestCase):
    ...
    @patch("insights.caching.redis_cluster_connection_factory.RedisCluster.from_url")
    ...
    @patch("insights.caching.redis_cluster_connection_factory.os.getpid")
    @patch("insights.caching.redis_cluster_connection_factory.RedisCluster.from_url")
...
class TestPrewarmQueryCacheCluster(TestCase):
    def test_no_op_when_no_cluster_configured(self) -> None:
        with patch("insights.caching.redis_cluster_connection_factory.settings") as mock_settings:
            mock_settings.QUERY_CACHE_REDIS_CLUSTER_URL = None
            with patch("insights.caching.redis_cluster_connection_factory.caches") as mock_caches:
                prewarm_query_cache_cluster()
    ...
    def test_swallows_and_logs_connection_errors(self) -> None:
        with patch("insights.caching.redis_cluster_connection_factory.settings") as mock_settings:
            ...
            with patch("insights.caching.redis_cluster_connection_factory.caches") as mock_caches:
                ...
                with patch("insights.caching.redis_cluster_connection_factory.logger") as mock_logger:
                    prewarm_query_cache_cluster()
...
class TestPrewarmInBackground(TestCase):
    ...
    def test_thread_never_raises_when_prewarm_fails(self) -> None:
        with patch("insights.caching.redis_cluster_connection_factory.settings") as mock_settings:
            ...
            with patch("insights.caching.redis_cluster_connection_factory.caches") as mock_caches:
                mock_caches[QUERY_CACHE_ALIAS].get.side_effect = ConnectionError("cluster down")
                with patch("insights.caching.redis_cluster_connection_factory.logger"):
                    thread = prewarm_query_cache_cluster_in_background()
                    thread.join(timeout=5)
                    assert not thread.is_alive()
