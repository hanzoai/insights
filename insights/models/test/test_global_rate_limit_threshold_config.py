...
from insights.models.global_rate_limit_threshold_config import (
    CUSTOM_THRESHOLDS_REDIS_KEY,
    MAX_DISTINCT_ID_CHARS,
    GlobalRateLimitThresholdConfig,
)
from insights.redis import get_client
from insights.settings import PLUGINS_RELOAD_REDIS_URL
...
class TestGlobalRateLimitThresholdConfig(BaseTest):
    ...
    @parameterized.expand(
        [
            ("token_only", "pk-abc", "", "pk-abc"),
            ("token_and_distinct_id", "pk-abc", "noisy_user", "pk-abc:noisy_user"),
        ]
    )
    ...
    def test_resolved_key_truncates_long_distinct_id(self):
        ...
        config = GlobalRateLimitThresholdConfig(token="pk-abc", distinct_id=long_distinct_id, threshold=10)
        self.assertEqual(config.resolved_key, f"pk-abc:{'d' * MAX_DISTINCT_ID_CHARS}")

    def test_post_save_writes_blob(self):
        with self.captureOnCommitCallbacks(execute=True):
            GlobalRateLimitThresholdConfig.objects.create(token="pk-abc", threshold=1000)
            GlobalRateLimitThresholdConfig.objects.create(token="pk-abc", distinct_id="noisy_user", threshold=50)

        self.assertEqual(self._blob(), {"pk-abc": 1000, "pk-abc:noisy_user": 50})

    def test_update_threshold_updates_blob(self):
        with self.captureOnCommitCallbacks(execute=True):
            config = GlobalRateLimitThresholdConfig.objects.create(token="pk-abc", threshold=1000)
        self.assertEqual(self._blob(), {"pk-abc": 1000})

        with self.captureOnCommitCallbacks(execute=True):
            config.threshold = 25
            config.save()
        self.assertEqual(self._blob(), {"pk-abc": 25})

    def test_delete_regenerates_blob(self):
        with self.captureOnCommitCallbacks(execute=True):
            config = GlobalRateLimitThresholdConfig.objects.create(token="pk-abc", threshold=1000)
            GlobalRateLimitThresholdConfig.objects.create(token="pk-xyz", threshold=2000)

        with self.captureOnCommitCallbacks(execute=True):
            config.delete()
        self.assertEqual(self._blob(), {"pk-xyz": 2000})
