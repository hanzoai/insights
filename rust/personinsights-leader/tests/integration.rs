...
use personinsights_coordination::pod::HandoffHandler;
use personinsights_coordination::strategy::StickyBalancedStrategy;
...
use personinsights_proto::personinsights::leader::v1::person_hog_leader_server::PersonHogLeader;
...
async fn pg_fallback_loads_person_on_cache_miss() {
    ...
    let (person_id,): (i64,) = sqlx::query_as(
        "INSERT INTO insights_person (created_at, properties, is_identified, uuid, version, team_id)
...
async fn pg_fallback_reads_numerics_the_leaders_parser_rejects() {
    ...
    let person = personinsights_leader::pg::load_person_from_pg(&pool, "insights_person", &key)
        .await
...
async fn oversize_updates_are_rejected_and_oversized_rows_remediated() {
    ...
    mock_cluster
        .create_topic("datastore_ingestion_warnings", 1, 1)
        .unwrap();
    ...
    let service = PersonHogLeaderService::new(
        ...
        WarningsProducer::with_throttle(
            kafka_producer,
            "datastore_ingestion_warnings".to_string(),
    ...
    while warnings.len() < 2 && Instant::now() < deadline {
        if let Some(Ok(message)) = consumer.poll(Duration::from_millis(200)) {
            ...
            assert_eq!(payload["source"], "personinsights-leader");
            // The pipeline's canonical shape: a Datastore-format
            ...
            assert!(
                ...
                "timestamp must be Datastore-format, got {timestamp}"
            );
            ...
            assert_eq!(details["pipelineStep"], "personinsights_admission");
            assert!(
...
async fn a_fence_with_an_unknown_outcome_keeps_both_its_version_and_its_cache_entry() {
    for (staged, expected) in [
        (
            personinsights_leader::fencing::FencedProduceError::FencedUncertain("staged".to_string()),
            tonic::Code::FailedPrecondition,
        ),
        (
            personinsights_leader::fencing::FencedProduceError::Indeterminate("staged".to_string()),
            tonic::Code::Unknown,
        ),
    ] {
...
async fn an_unknown_outcome_keeps_its_version(
    staged: personinsights_leader::fencing::FencedProduceError,
    expected: tonic::Code,
) {
    ...
    assert!(
        matches!(
            ...
            personinsights_leader::cache::CacheLookup::Found(_)
        ),
