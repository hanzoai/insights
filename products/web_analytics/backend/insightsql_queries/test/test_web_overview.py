...
from insights.models import Element
...
from products.web_analytics.backend.insightsql_queries.test.first_pageview_attribution_test_base import (
...
from products.web_analytics.backend.insightsql_queries.web_overview_pre_aggregated import (
    WebOverviewPreAggregatedQueryBuilder,
)


@snapshot_datastore_queries
class TestWebOverviewQueryRunner(FirstPageviewAttributionTestMixin, DatastoreTestMixin, APIBaseTest):
    ...
    @patch("products.web_analytics.backend.insightsql_queries.web_overview.execute_insightsql_query")
    def test_empty_insightsql_results_do_not_crash(self, _name: str, with_conversion_goal: bool, mock_execute: MagicMock):
    ...
    @patch("insights.insightsql.query.sync_execute", wraps=sync_execute)
    ...
    def test_can_use_preaggregated_tables_with_supported_properties(self):
        query = WebOverviewQuery(
            dateRange=DateRange(date_from="2023-11-01", date_to="2023-11-30"),
            properties=[{"key": "$host", "value": "app.hanzo.ai"}],
        )
        runner = WebOverviewQueryRunner(team=self.team, query=query)
        pre_agg_builder = WebOverviewPreAggregatedQueryBuilder(runner)
        self.assertTrue(
    ...
    def test_web_overview_with_channel_type_filter_execution(self):
        query = WebOverviewQuery(
            dateRange=DateRange(date_from="2023-11-01", date_to="2023-11-30"),
            properties=[SessionPropertyFilter(key="$channel_type", value="Direct", operator="exact", type="session")],
            modifiers=InsightsQLQueryModifiers(useWebAnalyticsPreAggregatedTables=True),
        )
        runner = WebOverviewQueryRunner(team=self.team, query=query)
    ...
    def test_preaggregated_queries_use_utc_filtering(self):
        ...
        runner = WebOverviewQueryRunner(team=self.team, query=query)
        pre_agg_builder = WebOverviewPreAggregatedQueryBuilder(runner)
        insightsql_query = pre_agg_builder.get_query()

        context_with_tz = InsightsQLContext(
            team_id=self.team.pk,
    ...
    def test_convertToProjectTimezone_affects_date_range_calculation(self):
        ...
        modifiers_with_tz = InsightsQLQueryModifiers(convertToProjectTimezone=True)
        ...
        modifiers_utc = InsightsQLQueryModifiers(convertToProjectTimezone=False)
    ...
    def test_compare_to_date_range_respects_convertToProjectTimezone(self):
        ...
        modifiers_tz = InsightsQLQueryModifiers(convertToProjectTimezone=True)
    ...
    def test_is_recent_relative_date_range(self, _name, hours_delta, explicit_date_to, expected_is_recent):
        ...
        runner.modifiers = InsightsQLQueryModifiers(useWebAnalyticsPreAggregatedTables=True, convertToProjectTimezone=False)
...
class TestWebOverviewNoJoinFastPath(DatastoreTestMixin, APIBaseTest):
    QUERY_TIMESTAMP = "2025-01-29"
...
class TestWebOverviewSessionIdSetFastPath(DatastoreTestMixin, APIBaseTest):
    QUERY_TIMESTAMP = "2025-01-29"
    snapshot: Any
    ...
    def test_session_id_set_feature_flag_gating(
        ...
        with (
            ...
            patch(
                "products.web_analytics.backend.insightsql_queries.first_pageview_flag.hanzo_insights.feature_enabled",
                **flag_mock_kwargs,
            ),
        ):
            runner = self._make_runner(
    ...
    @parameterized.expand(
        [
            ...
            (
                "person_test_filter",
                [{"key": "email", "type": "person", "operator": "not_icontains", "value": "@hanzo.ai"}],
                True,
            ),
    ...
    def test_session_id_set_pushes_id_filter_below_session_aggregation(self):
        with freeze_time(self.QUERY_TIMESTAMP):
            with override_settings(WEB_ANALYTICS_SESSION_ID_SET_TEAM_IDS=[self.team.pk]):
                ...
                context = InsightsQLContext(
                    team_id=self.team.pk,
                    enable_select_queries=True,
                    modifiers=InsightsQLQueryModifiers(sessionIdPushdown=True),
                )
                sql, _ = prepare_and_print_ast(runner.to_query(), context=context, dialect="datastore")
        ...
        # predicate makes Datastore execute the identical GLOBAL IN id-subquery
    ...
    def test_session_id_set_executes_with_pushdown_modifier_and_tag(self):
        ...
        self._create_pageviews()
        calls: list[tuple[Optional[str], Optional[InsightsQLQueryModifiers]]] = []

        def record_calls(*args, **kwargs):
            calls.append((kwargs.get("query_type"), kwargs.get("modifiers")))
            return execute_insightsql_query(*args, **kwargs)

        with freeze_time(self.QUERY_TIMESTAMP):
            with override_settings(WEB_ANALYTICS_SESSION_ID_SET_TEAM_IDS=[self.team.pk]):
                with patch(
                    "products.web_analytics.backend.insightsql_queries.web_overview.execute_insightsql_query",
    ...
    def test_session_id_set_falls_back_to_join_when_preflight_query_fails(self):
        ...
        def fail_preflight(*args, **kwargs):
            if kwargs.get("query_type") == "web_overview_session_id_set_preflight":
                raise Exception("datastore unavailable")
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            with override_settings(WEB_ANALYTICS_SESSION_ID_SET_TEAM_IDS=[self.team.pk]):
                ...
                with (
                    patch(
                        "products.web_analytics.backend.insightsql_queries.web_analytics_query_runner.execute_insightsql_query",
    ...
    def test_session_id_set_falls_back_to_join_when_filter_is_unselective(self):
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            with override_settings(WEB_ANALYTICS_SESSION_ID_SET_TEAM_IDS=[self.team.pk]):
                with patch(
                    "products.web_analytics.backend.insightsql_queries.web_analytics_query_runner.SESSION_ID_SET_MAX_MATCHING_SESSIONS",
                    0,
                ):
                    # The preflight executes from the shared base module; the main
...
class TestWebOverviewNoJoinRolloutPercent(DatastoreTestMixin, APIBaseTest):
    ...
    def test_no_join_executions_emit_their_own_query_type_tag(self):
        from insights.insightsql import query as insightsql_query_module

        with override_settings(WEB_ANALYTICS_NO_JOIN_TEAM_IDS=[self.team.pk], WEB_ANALYTICS_NO_JOIN_ROLLOUT_PERCENT=0):
            with patch(
                "products.web_analytics.backend.insightsql_queries.web_overview.execute_insightsql_query",
                wraps=insightsql_query_module.execute_insightsql_query,
            ) as spy:
