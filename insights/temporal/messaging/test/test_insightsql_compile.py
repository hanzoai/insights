...
class TestCompileHogqlForStreaming:
    ...
    async def test_bypasses_property_restrictions(self):
        ...
        with patch(
            ...
            sql, params = await compile_insightsql_for_streaming(node, team_id=team.id)

        assert "FORMAT JSONEachRow" in sql

    @pytest.mark.asyncio
    async def test_property_key_is_parameterized(self):
        """Property keys must reach Datastore as parameters, not as unquoted SQL fragments.

        When the ``persons`` lazy table is used, InsightsQL generates a subquery that references the
        property via ``JSONExtractRaw(person.properties, %(insightsql_val_N)s, ...)`` with the key as
        a parameter, and also as a backtick-quoted column alias. The backtick quoting makes the
        key inert as a SQL identifier — Datastore treats it as a column name, not executable code.
        ...
        # inside backtick-quoted identifiers, which Datastore treats as column names.
