...
gather queries (the Datastore + ORM functions in
`insights.tasks.usage_report`) are mocked — by seeding canned per-spec
...
from insights.tasks.usage_report import InstanceMetadata
from insights.temporal.usage_report import storage
from insights.temporal.usage_report.activities import aggregate_and_chunk_org_reports
from insights.temporal.usage_report.queries import QUERIES, QUERY_INDEX
from insights.temporal.usage_report.storage import queries_key, write_json
from insights.temporal.usage_report.types import AggregateInputs, RunQueryToS3Result, WorkflowContext
...
async def test_aggregate_writes_chunks_and_manifest(minio_workflow_ctx: WorkflowContext, activity_environment) -> None:
    ...
    with patch(
        "insights.temporal.usage_report.activities.get_instance_metadata",
...
async def test_aggregate_chunks_into_batches_of_ten_thousand(
    ...
    from insights.temporal.usage_report import activities as module
