...
from insights.exceptions import Conflict, QuotaLimitExceeded
...
ROUTING = "products.insights_ai.backend.message_routing"
...
class TestOpenSandboxMessage(APIBaseTest):
    def setUp(self):
        super().setUp()
        self.conversation = Thread.objects.create(
            user=self.user,
            team=self.team,
            agent_runtime=Thread.AgentRuntime.SANDBOX,
        )
    ...
    def test_first_message_creates_run_and_persists_task(self):
        ...
        assert "<insights_context>" in run.state["pending_user_message"]
        ...
        assert wf_kwargs["insights_mcp_scopes"] == "full"

    def test_first_message_threads_routed_repository(self):
        task, _ = self._stub_task()
        car, workflow, sysprompt = self._patches(task)
        with car as m_car, workflow, sysprompt:
            self._service().open({"content": "fix the SDK", "trace_id": "t"}, repository="insights/insights-js")

        _, kwargs = m_car.call_args
        assert kwargs["repository"] == "insights/insights-js"
    ...
    def test_terminal_followup_creates_new_run_with_resume(self):
        ...
        assert m_workflow.call_args.kwargs["insights_mcp_scopes"] == "full"
...
class TestLockConversationForFollowup(APIBaseTest):
    def test_lock_acquires_select_for_update_on_conversation(self):
        conversation = Thread.objects.create(user=self.user, team=self.team)

        with patch(f"{ROUTING}.Thread.objects") as mock_objects:
...
class TestAwaitUserMessageStoredKey:
    ...
    def test_await_user_message_is_stored_under_its_literal_key(self):
        ...
        dumped = InsightsAIRunState(await_user_message=True).model_dump(by_alias=True, exclude_unset=True)
...
class TestSandboxFirstMessageConversion(APIBaseTest):
    ...
    def setUp(self):
        ...
        self.conversation = Thread.objects.create(
            ...
            agent_runtime=Thread.AgentRuntime.LANGGRAPH,
            status=Thread.Status.IDLE,
        )

    def _block(self) -> str:
        return "<insights_context>This session was resumed from the legacy implementation.\nUser: hi</insights_context>"
    ...
    def test_first_message_conversion_flips_runtime_and_links_task(self):
        ...
        self.conversation.refresh_from_db()
        assert self.conversation.agent_runtime == Thread.AgentRuntime.SANDBOX
        assert self.conversation.task_id is not None
    ...
    def test_first_message_conversion_idempotent_under_lock(self):
        ...
        Thread.objects.filter(id=self.conversation.id).update(task=other_task)
    ...
    def test_first_message_conversion_reverts_on_workflow_start_failure(self):
        ...
        self.conversation.refresh_from_db()
        assert self.conversation.agent_runtime == Thread.AgentRuntime.LANGGRAPH
        assert self.conversation.task_id is None
