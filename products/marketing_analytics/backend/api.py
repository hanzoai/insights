...
from products.marketing_analytics.backend.insightsql_queries.constants import CONVERSION_GOAL_KIND_CHOICES
...
class ConversionGoalSummarySerializer(serializers.Serializer):
    ...
    kind = serializers.ChoiceField(
        ...
        help_text="Goal type: EventsNode (Insights event), ActionsNode (Insights action), or DataWarehouseNode (external table)",
    )
...
class MarketingAnalyticsViewSet(TeamAndOrgViewSetMixin, GenericViewSet):
    ...
    def test_mapping(self, request: Request, *args, **kwargs) -> Response:
        ...
        try:
            ...
            result = execute_insightsql_query(query, self.team, user=cast(User, request.user))
