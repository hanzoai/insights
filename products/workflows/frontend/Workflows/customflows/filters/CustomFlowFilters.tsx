...
export type CustomFlowFiltersProps = {
    filtersKey: string
    filters: CustomFlowAction['filters']
    setFilters: (filters: CustomFlowAction['filters']) => void
    typeKey?: string
    buttonCopy?: string
}
 ...
 */
export function CustomFlowEventFilters({ filters, setFilters, typeKey, buttonCopy }: CustomFlowFiltersProps): JSX.Element {
    ...
    return (
        <ActionFilter
            ...
            setFilters={(filters: FilterType): void => {
                ...
                setFilters(filters as CustomFlowAction['filters'])
            }}
...
export function CustomFlowPropertyFilters({ filtersKey, filters, setFilters }: CustomFlowFiltersProps): JSX.Element {
    const sampleGlobals = useSampleGlobals()
    return (
        <PropertyFilters
            propertyFilters={filters?.properties}
            onChange={(properties: FilterType['properties']): void => {
                setFilters({ ...filters, properties: properties ?? [] } as CustomFlowAction['filters'])
            }}
            pageKey={`CustomFlowPropertyFilters.${filtersKey}`}
            taxonomicGroupTypes={[
                TaxonomicFilterGroupType.WorkflowVariables,
                TaxonomicFilterGroupType.EventProperties,
                TaxonomicFilterGroupType.EventFeatureFlags,
