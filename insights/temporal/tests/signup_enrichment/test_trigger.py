...
from insights.temporal.signup_enrichment.trigger import dispatch_signup_enrichment, start_signup_enrichment_workflow
from insights.temporal.signup_enrichment.workflow import SignupEnrichmentInputs
...
def _run_dispatch_pool_inline():
    with patch("insights.temporal.signup_enrichment.trigger._dispatch_executor", _InlineExecutor()):
        yield
...
def _dispatch_mocks(region="US"):
    ...
    return (
        patch("insights.temporal.signup_enrichment.trigger.transaction.on_commit", side_effect=lambda fn: fn()),
        patch("insights.temporal.signup_enrichment.trigger.sync_connect"),
        patch("insights.temporal.signup_enrichment.trigger.asyncio.run"),
        patch("insights.temporal.signup_enrichment.trigger.get_instance_region", return_value=region),
        patch("insights.temporal.signup_enrichment.trigger.record_signup_work_email"),
    )
...
def test_work_email_write_failure_does_not_block_dispatch():
    ...
    with on_commit, connect as connect_mock, run, region, record as record_mock:
        ...
        with patch("insights.temporal.signup_enrichment.trigger.capture_exception") as capture_mock:
            start_signup_enrichment_workflow(organization_id="org-1", distinct_id="d1", email="founder@stripe.com")
    connect_mock.assert_called_once()
...
def test_dispatch_signup_enrichment_swallows_only_already_started(error, propagates):
    ...
    with patch("insights.temporal.signup_enrichment.trigger._start_workflow", side_effect=error):
        if propagates:
...
def test_dispatch_dropped_when_backlog_full():
    ...
    full.acquire()
    on_commit, connect, run, region, record = _dispatch_mocks()
    with patch("insights.temporal.signup_enrichment.trigger._dispatch_slots", full):
        with on_commit, connect as connect_mock, run, region, record:
            start_signup_enrichment_workflow(organization_id="org-1", distinct_id="d1", email="founder@stripe.com")
    connect_mock.assert_not_called()
...
def test_dispatch_slot_released_after_run():
    ...
    on_commit, connect, run, region, record = _dispatch_mocks()
    with patch("insights.temporal.signup_enrichment.trigger._dispatch_slots", single):
        with on_commit, connect as connect_mock, run, region, record:
            start_signup_enrichment_workflow(organization_id="org-1", distinct_id="d1", email="founder@stripe.com")
...
def test_signup_geoip_country_is_threaded_into_workflow_inputs():
    ...
    with (
        ...
        record,
        patch(
            "insights.temporal.signup_enrichment.trigger.get_geoip_properties",
