...
To link a user to a specific recording, build the URL as `{insights_base_url}/replay/{id}` using the `id` returned in each result row.
...
- Deep-link to a specific recording as `{insights_base_url}/replay/{id}`, never `/replay/home?sessionRecordingId={id}`.
