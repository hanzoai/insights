...
class TestUpsertDashboardTool(BaseTest):
    ...
    async def _create_insight(
        ...
        return await Insight.objects.acreate(
            ...
            if isinstance(query, InsightsQLQuery | AssistantInsightsQLQuery)
    ...
    @parameterized.expand(
        [
            ...
            ("insightsql", InsightsQLQuery(query="SELECT 1")),
        ]
    )
    async def test_dashboard_accepts_different_insight_types(
        self, _name: str, query: TrendsQuery | FunnelsQuery | RetentionQuery | LifecycleQuery | InsightsQLQuery
    ):
