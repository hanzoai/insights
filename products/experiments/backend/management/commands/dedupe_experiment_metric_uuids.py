...
WITH metrics_unnested AS (
    ...
    FROM insights_experiment e
    JOIN insights_experimenttosavedmetric link ON link.experiment_id = e.id
    JOIN insights_experimentsavedmetric sm ON sm.id = link.saved_metric_id
