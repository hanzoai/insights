...
from posthog.models.custom_flow.custom_flow_template import CustomFlowTemplate
...
class TestCustomFlowTemplateAPI(APIBaseTest):
    def setUp(self):
        super().setUp()
        ...
        self.template_with_defaults = CustomFunctionTemplate.objects.create(
    ...
    def _create_custom_flow_data(self, include_metadata=False, custom_inputs=None):
        ...
        return custom_flow_data

    def test_template_creation(self):
        """Test that templates are created with correct function actions"""
        custom_flow_data = self._create_custom_flow_data(custom_inputs={"url": {"value": "https://custom.example.com"}})

        response = self.client.post(f"/api/projects/{self.team.id}/custom_flow_templates", custom_flow_data)
        assert response.status_code == 201, response.json()

        template = CustomFlowTemplate.objects.get(pk=response.json()["id"])
    ...
    def test_template_creation_sets_team_and_created_by(self):
        ...
        custom_flow_data = self._create_custom_flow_data()
    ...
    def test_template_image_url_field(self):
        ...
        custom_flow_data = self._create_custom_flow_data()
        custom_flow_data["image_url"] = "https://example.com/image.png"
    ...
    def test_template_tags_field(self):
        ...
        custom_flow_data["tags"] = ["ingestion", "batch"]
        response = self.client.post(f"/api/projects/{self.team.id}/custom_flow_templates", custom_flow_data)
        assert response.status_code == 201, response.json()
        ...
        update_data = self._create_custom_flow_data()
        update_data["tags"] = ["updated-tag"]
        response = self.client.patch(f"/api/projects/{self.team.id}/custom_flow_templates/{template_id}", update_data)
    ...
    def test_template_filtering_returns_global_and_team_templates(self):
        ...
        team_template_data = self._create_custom_flow_data()
        team_template_data["scope"] = "team"
        response = self.client.post(f"/api/projects/{self.team.id}/custom_flow_templates", team_template_data)
        ...
        other_team_template = CustomFlowTemplate.objects.create(
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_flow_templates")
        assert response.status_code == 200
    ...
    def test_template_filtering_global_templates_visible_to_all_teams(self):
        ...
        response = self.client.get(f"/api/projects/{other_team.id}/custom_flow_templates")
        assert response.status_code == 200
    ...
    def test_cannot_read_other_team_template(self):
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_flow_templates/{other_team_template.id}")
    ...
    def test_cannot_update_other_team_template(self):
        ...
        response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_flow_templates/{other_team_template.id}", update_data
        )
        assert response.status_code == 404
    ...
    def test_cannot_delete_other_team_template(self):
        ...
        response = self.client.delete(f"/api/projects/{self.team.id}/custom_flow_templates/{other_team_template.id}")
        assert response.status_code == 404

        assert CustomFlowTemplate.objects.filter(id=other_team_template.id).exists()
    ...
    def test_can_read_update_delete_own_team_template(self):
        ...
        template_data = self._create_custom_flow_data()
        response = self.client.post(f"/api/projects/{self.team.id}/custom_flow_templates", template_data)
        assert response.status_code == 201
        template_id = response.json()["id"]

        response = self.client.get(f"/api/projects/{self.team.id}/custom_flow_templates/{template_id}")
        assert response.status_code == 200
        ...
        response = self.client.delete(f"/api/projects/{self.team.id}/custom_flow_templates/{template_id}")
        assert response.status_code == 204

        assert not CustomFlowTemplate.objects.filter(id=template_id).exists()
    ...
    def test_updating_team_template_to_global_is_blocked(self):
        ...
        template = CustomFlowTemplate.objects.get(id=template_id)
        assert template.scope == "team"
    ...
    def test_public_list_flow_templates(self):
        ...
        self.client.logout()
        response = self.client.get("/api/public_custom_flow_templates/")
    ...
    def test_org_scoped_template_visible_to_other_teams_in_same_org(self):
        ...
        org_template = CustomFlowTemplate.objects.create(
            name="Org Template",
            team=self.team,
            scope="organization",
            trigger={"type": "event"},
    ...
    def test_org_scoped_template_not_visible_to_other_orgs(self):
        ...
        CustomFlowTemplate.objects.create(
            name="Org Template",
            team=self.team,
            scope="organization",
            trigger={"type": "event"},
        ...
        response = self.client.get(f"/api/projects/{other_org_team.id}/custom_flow_templates")
        assert response.status_code == 200
    ...
    def test_cannot_update_org_scoped_template_from_other_org(self):
        ...
        custom_flow_data = self._create_custom_flow_data()
        custom_flow_data["scope"] = "organization"
        response = self.client.post(f"/api/projects/{self.team.id}/custom_flow_templates", custom_flow_data)
        assert response.status_code == 201
        template_id = response.json()["id"]
        ...
        response = self.client.patch(
            f"/api/projects/{other_org_team.id}/custom_flow_templates/{template_id}",
            update_data,
        )
        assert response.status_code == 404

        template = CustomFlowTemplate.objects.get(pk=template_id)
    ...
    def test_can_update_org_scoped_template_from_same_org(self):
        ...
        response = self.client.patch(
            f"/api/projects/{other_team.id}/custom_flow_templates/{template_id}",
            update_data,
        )
        assert response.status_code == 200
