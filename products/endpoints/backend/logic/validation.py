...
it reaches the CRUD service: name format, data freshness buckets, InsightsQL
...
from insights.schema import EndpointRequest, InsightsQLQuery, InsightsQLVariable
...
def validate_insightsql_query(query: InsightsQLQuery, team: Team, user: User) -> None:
    """Validate that a InsightsQL query parses, its variables are valid, and the author can
...
def _validate_query_access(
    ...
    try:
        ...
        prepare_ast_for_printing(node=resolvable_ast, context=context, dialect="datastore")
    except ExposedInsightsQLError as err:
        # Surfaces "You don't have access to table `X`." (QueryError) and other resolver errors.
        raise ValidationError({"query": f"Invalid InsightsQL query: {err}"}) from err
...
def sync_insightsql_query_variables(query: InsightsQLQuery, team: Team) -> None:
    ...
    synced_variables: dict[str, InsightsQLVariable] = {}
    ...
    if missing_code_names:
        ...
        for variable in missing_variables:
            if not variable.code_name:
                continue
            synced_variables[str(variable.id)] = InsightsQLVariable(
...
def validate_variable_placeholders(node: ast.AST, variables: Optional[dict[str, InsightsQLVariable]], team: Team) -> None:
            ...
            raise ValidationError(
                {
                    ...
                    "Make sure the variables exist in https://app.hanzo.ai/data-management/variables."
                }
            )
...
def validate_endpoint_request(data: EndpointRequest, team: Team, user: User, strict: bool = True) -> None:
    ...
    if query and isinstance(query, InsightsQLQuery) and query.query:
        sync_insightsql_query_variables(query, team)
        validate_insightsql_query(query, team, user)
...
def validate_update_request(
    ...
    if data.query and isinstance(data.query, InsightsQLQuery) and data.query.query:
        sync_insightsql_query_variables(data.query, team)
        validate_insightsql_query(data.query, team, user)
