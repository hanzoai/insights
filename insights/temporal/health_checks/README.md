...
A framework for running scheduled health checks across Insights teams. Subclass `HealthCheck`, implement a `detect` method, and the framework handles team batching, parallel execution, issue persistence, auto-resolution, and rollout control — all orchestrated by Temporal.

Issues are written to the `insights_healthissue` table and automatically resolved when a team passes a subsequent check run.
...
from insights.temporal.health_checks.framework import HealthCheck
...
```python
from insights.temporal.health_checks.detectors import HealthExecutionPolicy
...
`execute_datastore_health_team_query()` is a safe wrapper for running Datastore queries in health checks. It enforces a `%(team_ids)s` placeholder, applies conservative default settings, and handles parameter merging.
...
rows = execute_datastore_health_team_query(
    ...
    settings={"max_execution_time": 60},  # override Datastore settings
)
```
...
## Datastore kill switch

Health checks are automatically skipped when the Datastore kill switch is active (LIGHT or FULL).
