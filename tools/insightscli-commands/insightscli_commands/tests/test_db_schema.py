...
from insightscli_commands import db_schema
...
def test_restore_schema_if_fresh_skips_without_download(
    ...
    assert db_schema.restore_schema_if_fresh(target_db="insights", mode=mode) is False
...
def test_restore_schema_if_fresh_recreates_empty_db(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert db_schema.restore_schema_if_fresh(target_db="insights", mode="auto") is True
    assert restore_calls == [{"target_db": "insights", "recreate": True, "ensure_defaults": True}]
...
def test_restore_schema_if_fresh_skips_when_db_populated_after_download(
    ...
    assert db_schema.restore_schema_if_fresh(target_db="insights", mode="auto") is False
...
def test_run_psql_with_gzip_input_uses_transactional_error_stopping(
    ...
    db_schema._run_psql_with_gzip_input(schema_path, "insights")
...
def test_restore_schema_dump_recreate_drops_and_creates(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    db_schema.restore_schema_dump(target_db="test_insights", recreate=True, schema_path=schema_path)

    assert any("DROP DATABASE IF EXISTS test_insights;" in command for call in commands for command in call)
    assert any("CREATE DATABASE test_insights;" in command for call in commands for command in call)
    assert restored == [(schema_path, "test_insights")]
    assert defaults == ["test_insights"]
...
def test_restore_schema_dump_recreate_cleans_up_after_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    terminate_sql = (
        ...
        "WHERE datname = 'test_insights' AND pid <> pg_backend_pid();"
    )
    assert admin_sql == [
        terminate_sql,
        "DROP DATABASE IF EXISTS test_insights;",
        "CREATE DATABASE test_insights;",
        terminate_sql,
        "DROP DATABASE IF EXISTS test_insights;",
        "CREATE DATABASE test_insights;",
    ]
...
def test_restore_schema_dump_without_recreate_does_not_drop(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    db_schema.restore_schema_dump(target_db="insights", recreate=False, schema_path=schema_path)
...
def test_restore_schema_fresh_recreates_target_db(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TARGET_DB", "insights_e2e_test")
    ...
    assert result.exit_code == 0
    assert calls == [{"target_db": "insights_e2e_test", "recreate": True, "ensure_defaults": True}]
...
def test_converted_commands_accept_forwarded_yes_flag(command: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    """insightscli composite commands forward --yes to children; converted click
