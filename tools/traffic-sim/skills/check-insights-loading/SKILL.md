---
name: check-insights-loading
description: >
  Inspect how the Insights JavaScript SDK is loaded across a list of URLs.
...
# Check Insights loading

For each URL, navigates a real browser to the page, waits for Insights to
initialize, and reports:

- Whether `window.insights` is defined and `__loaded`.
...
Call the `check_insights_loading` MCP tool with:

- `urls` (required) — list of URLs to inspect.
- `insights_host` (default `https://us.i.hanzo.ai`).
...
- **`load_method: array_js_only`** with no init config — `array.js` was
  loaded but `insights.init()` was never called. Common with manual installs
