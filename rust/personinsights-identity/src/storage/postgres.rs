...
use personinsights_common::persons::person_uuid;
...
impl IdentityStorage for PostgresIdentityStorage {
    ...
    async fn create_person_stubs(&self, stubs: &[PersonStub]) -> StorageResult<Vec<StubOutcome>> {
        ...
        // id already used personless needs its row at version 1 so Datastore
        ...
        if !personless_dids.is_empty() {
            let rows = sqlx::query!(
                r#"
                INSERT INTO insights_personlessdistinctid (distinct_id, is_merged, created_at, team_id)
                SELECT d, true, now(), t FROM unnest($1::int[], $2::text[]) AS u(t, d)
                ON CONFLICT (team_id, distinct_id) DO UPDATE SET is_merged = true
                    WHERE insights_personlessdistinctid.is_merged = false
        ...
        for (i, stub) in stubs.iter().enumerate() {
            ...
            if *created {
                sqlx::query!(
                    "DELETE FROM insights_persondistinctid WHERE team_id = $1 AND person_id = $2",
                    stub.team_id as i32,
                    person.id
                )
                .execute(&mut *tx)
                .await?;
                sqlx::query!(
                    "DELETE FROM insights_person WHERE team_id = $1 AND id = $2",
                    stub.team_id as i32,
                    person.id
                )
                .execute(&mut *tx)
            ...
            let existing = sqlx::query_scalar!(
                "SELECT person_id FROM insights_persondistinctid WHERE team_id = $1 AND distinct_id = $2",
                stub.team_id as i32,
                &stub.distinct_id
            )
        ...
        if !delete_dids.is_empty() {
            sqlx::query!(
                r#"
                DELETE FROM insights_personlessdistinctid pl
        ...
        if !unmark_dids.is_empty() {
            sqlx::query!(
                r#"
                UPDATE insights_personlessdistinctid pl
                SET is_merged = false
