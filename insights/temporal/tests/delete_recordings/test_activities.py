...
from insights.temporal.delete_recordings.activities import (
...
from insights.temporal.delete_recordings.types import (
...
async def test_load_recordings_with_query_single_page():
    ...
    with (
        patch("insights.temporal.delete_recordings.activities.Team.objects") as mock_team_objects,
        patch("insights.temporal.delete_recordings.activities.database_sync_to_async") as mock_sync_to_async,
        patch("insights.temporal.delete_recordings.activities.SessionRecordingListFromQuery") as mock_query_class,
    ):
...
async def test_load_recordings_with_person():
    ...
    with patch("insights.temporal.delete_recordings.activities.get_client") as mock_get_client:
...
async def test_load_recording_blocks():
    ...
    with (
        patch("insights.temporal.delete_recordings.activities.get_client") as mock_get_client,
        patch("insights.temporal.delete_recordings.activities.build_block_list") as mock_build_block_list,
        patch("insights.temporal.delete_recordings.activities.get_block_loaded_counter") as mock_get_counter,
    ):
        mock_get_client.return_value = mock_client
...
async def test_delete_recording_blocks():
    ...
    try:
        ...
        with (
            patch(
                "insights.temporal.delete_recordings.activities.session_recording_v2_object_storage.async_client"
            ) as mock_client,
            patch("insights.temporal.delete_recordings.activities.mkstemp") as mock_mkstemp,
            patch("insights.temporal.delete_recordings.activities.os.remove") as mock_remove,
            patch("insights.temporal.delete_recordings.activities.get_block_deleted_counter") as mock_get_deleted,
            patch("insights.temporal.delete_recordings.activities.get_block_deleted_error_counter") as mock_get_error,
        ):
...
async def test_schedule_recording_metadata_deletion():
    ...
    with patch("insights.temporal.delete_recordings.activities.redis.from_url", return_value=mock_redis):
...
async def test_perform_recording_metadata_deletion_with_sessions():
    ...
    with (
        patch("insights.temporal.delete_recordings.activities.redis.from_url", return_value=mock_redis),
        patch("insights.temporal.delete_recordings.activities.get_client", return_value=mock_client),
        patch("insights.temporal.delete_recordings.activities.SessionRecording.objects", mock_session_recording_qs),
        patch(
            "insights.temporal.delete_recordings.activities.SessionRecordingViewed.objects",
...
async def test_delete_recording_lts_data_recording_not_found():
    ...
    with patch("insights.temporal.delete_recordings.activities.SessionRecording.objects", mock_qs):
...
async def test_delete_recording_lts_data_deletes_file():
    ...
    with (
        patch("insights.temporal.delete_recordings.activities.SessionRecording.objects", mock_qs),
        patch(
            "insights.temporal.delete_recordings.activities.session_recording_v2_object_storage.async_client"
        ) as mock_client,
    ):
...
async def test_delete_recording_blocks_upload_error():
    ...
    with (
        ...
        patch("insights.temporal.delete_recordings.activities.get_block_deleted_counter") as mock_get_deleted,
        patch("insights.temporal.delete_recordings.activities.get_block_deleted_error_counter") as mock_get_error,
        patch("insights.temporal.delete_recordings.activities.overwrite_block"),
        patch("insights.temporal.delete_recordings.activities.Path") as mock_path,
    ):
        mock_client.return_value = mock_storage
