...
def _request(body: bytes, *, event: str = "pull_request", delivery_id: str = "delivery-1", signature: str | None):
    ...
    return factory.post("/stamp/webhook/", data=body, content_type="application/json", **headers)
...
def test_valid_pull_request_event_enqueues_and_returns_202(mock_delay):
    ...
    response = stamp_github_webhook(request)
...
@override_settings(STAMPFN_GITHUB_APP_WEBHOOK_SECRET="")
