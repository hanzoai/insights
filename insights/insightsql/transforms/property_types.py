...
from insights.insightsql.database.models import BooleanDatabaseField, DateTimeDatabaseField, StringJSONDatabaseField
from insights.insightsql.database.s3_table import S3Table
from insights.insightsql.database.schema.events import (
    EVENTS_TABLE_TYPES,
    EventsGroupSubTable,
...
from insights.insightsql.property_metadata import load_property_metadata
from insights.insightsql.property_planner import PropertySourceKind, plan_property_access
from insights.insightsql.restricted_properties import restricted_property_keys_for_table_type
from insights.insightsql.type_system import normalized_runtime_type, parse_sql_runtime_type
...
def build_property_swapper(node: ast.AST, context: InsightsQLContext) -> None:
...
class PropertySwapper(CloningVisitor):
    ...
    # Datastore string-parsing conversions (toFloat64OrZero, toInt64OrZero,
    ...
    # Datastore array-membership functions whose first argument must be an array. Users write these
    ...
    def _try_rewrite_json_extract_to_mat_column(self, node: ast.Call) -> ast.Expr | None:
        ...
        When users write raw JSONExtractString(properties, '$foo') in InsightsQL, Datastore otherwise decompresses the full
        ...
        Complex typed JSONExtract(...) values are left alone because Datastore's native JSON subcolumn access exposes
        ...
        table_name = table_type.resolve_database_table(self.context).to_printed_insightsql()
    ...
    def _json_extract_subcolumn_expr(
        ...
        # Keep Datastore's JSONExtract implementation as the source of truth for coercion and
    ...
    def _move_timezone_from_field_to_constant(self, node: ast.CompareOperation) -> ast.CompareOperation | None:
        ...
        Datastore DateTime values are epoch seconds internally, and toTimeZone()
        ...
        ensures Datastore interprets it in the correct timezone.
    ...
    def _ensure_constant_has_timezone(expr: ast.Expr, tz: str) -> ast.Expr:
        ...
        if isinstance(inner, ast.Constant):
            if isinstance(inner.value, datetime) and inner.value.tzinfo is not None:
                return expr
            # Datastore's toDateTime64 can't parse 'Z'/offset strings, so parse them in Python instead.
    ...
    def _convert_string_property_to_type(
        ...
        # toFloatOrDefault). Those Datastore functions require a String argument, so
    ...
    def _add_notice(self, node: ast.Field, message: str):
        ...
        self.context.add_notice(
            start=max(node.start, node.end - len(escape_insightsql_identifier(node.chain[-1]))),
            end=node.end,
            message=message,
        )
