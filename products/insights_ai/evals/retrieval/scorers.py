...
* ``SkillLoaded`` — did the agent load the named skill (e.g.
  ``querying-insights-data``) before producing its answer?
* ``LookupIdInOutput`` — does the agent's final assistant message contain
  the seeded lookup insight's ID, proving it actually queried Insights
...
from products.insights_ai.eval_harness.log_parser import INFO_SYNTHETIC_PREFIX, LogParser
...
class InformationSchemaBeforeSql(Scorer):
    ...
    * a **graded** call hits a ``system.*`` entity table (``system.insights``,
    ...
    came via tools-mode dispatch or via ``insights:exec`` unwrapping; the
