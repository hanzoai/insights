...
describe('handleCallback', () => {
    it('redirects to original redirect_uri with code and state', async () => {
        ...
        const request = new Request('https://oauth.hanzo.ai/oauth/callback/?code=auth_code_abc&state=test_state_123')
    ...
    it('returns 400 when state parameter is missing', async () => {
        const request = new Request('https://oauth.hanzo.ai/oauth/callback/?code=auth_code_abc')
        const response = await handleCallback(request, mockKV)
    ...
    it('returns 400 when state is expired or unknown', async () => {
        ...
        const request = new Request('https://oauth.hanzo.ai/oauth/callback/?code=auth_code_abc&state=expired_state')
        const response = await handleCallback(request, mockKV)
    ...
    it('forwards error params to client redirect_uri', async () => {
        ...
        const request = new Request(
            'https://oauth.hanzo.ai/oauth/callback/?error=access_denied&error_description=User+denied+access&state=err_state'
        )
        const response = await handleCallback(request, mockKV)
