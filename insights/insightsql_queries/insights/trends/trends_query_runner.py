...
from insights.caching.insights_api import (
...
from insights.insightsql_queries.insights.trends.series_with_extras import SeriesWithExtras
from insights.insightsql_queries.insights.trends.trend_validation_rules import (
...
from insights.insightsql_queries.insights.trends.trends_query_builder import TrendsQueryBuilder
...
from insights.insightsql_queries.insights.utils.utils import get_response_insightsql
from insights.insightsql_queries.query_runner import AnalyticsQueryRunner, resolve_series_custom_name
from insights.insightsql_queries.utils.formula_ast import FormulaAST
...
from insights.insightsql_queries.utils.timestamp_utils import format_label_date, get_earliest_timestamp_from_series
from insights.insightsql_queries.validation.rules import DisallowUnsupportedDataWarehouseSettings, RequireAtLeastOneSeries
from insights.insightsql_queries.validation.validation import QueryValidationRule
...
from insights.utils import multisort

from products.actions.backend.models.action import Action
from products.cohorts.backend.models.cohort import Cohort
from products.event_definitions.backend.models.property_definition import PropertyDefinition
from products.warehouse_sources.backend.facade.insightsql import get_view_or_table_by_name
...
class TrendsQueryRunner(AnalyticsQueryRunner[TrendsQueryResponse]):
    ...
    def __init__(
        ...
        from insights.insightsql_queries.insights.utils.utils import convert_active_user_math_based_on_interval
    ...
    def __post_init__(self):
        self.update_insightsql_modifiers()
    ...
    def to_actors_query_options(self) -> InsightActorsQueryOptionsResponse:
        ...
        if has_breakdown_filter(self.query.breakdownFilter):
            ...
            if self.query.breakdownFilter.breakdown_type == "cohort":
                ...
                results = execute_insightsql_query(
    ...
    def _calculate(self):
        ...
        response_insightsql = get_response_insightsql(queries, team=self.team, timings=self.timings, modifiers=self.modifiers)
        ...
        return TrendsQueryResponse(
            ...
            hasMore=has_more,
            timings=timings,
            insightsql=response_insightsql,
            modifiers=self.modifiers,
    ...
    def build_series_response(
        self, response: InsightsQLQueryResponse, series: SeriesWithExtras, series_count: int
    ) -> list[dict[str, Any]]:
        def get_value(name: str, val: Any):
            if name not in ["date", "total", "breakdown_value"]:
                raise Exception("Column not found in insightsql results")
            if response.columns is None:
                raise Exception("No columns returned from insightsql results")
            ...
            if series.aggregate_values:
                series_object = {
                    ...
                    "action": {  # TODO: Populate missing props in `action`
                        ...
                        "math_insightsql": series.series.math_insightsql,
    ...
    def update_insightsql_modifiers(self) -> None:
        if (
    ...
    def _is_breakdown_filter_field_boolean(self):
        ...
        if isinstance(self.query.series[0], DataWarehouseNode) and breakdown_type == "data_warehouse":
            ...
            # A DataWarehouseNode may target a native InsightsQL table (e.g. a preaggregated table) that has
            ...
            field_type = self._data_warehouse_column_datastore_type(columns[breakdown_key])
    ...
    def _data_warehouse_column_datastore_type(self, column: object) -> str | None:
        if isinstance(column, str):
            return column

        if isinstance(column, dict):
            datastore_type = column.get("datastore")
            if isinstance(datastore_type, str):
    ...
    def _is_breakdown_field_boolean(
        ...
        if breakdown_type == "insightsql" or breakdown_type == "cohort" or breakdown_type == "session":
            return False
