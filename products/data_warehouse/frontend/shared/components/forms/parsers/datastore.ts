...
const SCHEME_DEFAULTS: Record<string, SchemeDefaults> = {
    ...
    'datastores:': { secure: 'true', port: 9440, family: 'native' },
    'http:': { secure: 'false', port: 8123, family: 'http' },
    'datastore:': { secure: 'false', port: 9000, family: 'native' },
}
...
export function parseDatastoreConnectionString(str: string): ParseResult {
    let result: URL
    try {
        result = new URL(str)
    } catch {
    ...
    // the HTTP family (8123 ↔ 8443), and datastore ↔ datastores stays native (9000 ↔ 9440).
