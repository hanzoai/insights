...
The Live tab is **not** a InsightsQL playground — its data comes from a livestream backed by
short InsightsQL backfills. When the user wants to query "right now" data with InsightsQL, point
...
- Setting the host filter narrows: the SSE stream, the InsightsQL backfill queries (so the
...
- `$http_log` events come from server-side log capture, not from `insights-js`. If a
...
- The 30-minute window is a sliding aggregation over an in-memory buffer in the
  browser — refreshing the page replays the backfill InsightsQL, not the SSE stream. Do
