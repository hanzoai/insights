...
from llm_gateway.cli.reset_insights_code_usage import _non_empty_user_id, reset_usage
...
class TestResetUsage:
    async def test_resets_every_user_regardless_of_plan(self, redis: fakeredis.FakeRedis) -> None:
        ...
        keys = [
            "ratelimit:cost:user:user_cost_burst:insights_code:100",
            "ratelimit:cost:user:user_cost_sustained:insights_code:100:period:0",
            "ratelimit:cost:user:user_cost_burst:insights_code:200",
            "ratelimit:cost:user:user_cost_sustained:insights_code:200:period:0",
            "ratelimit:cost:user:user_cost_burst:insights_code:300:tm2",
            "ratelimit:cost:user:user_cost_sustained:insights_code:300:tm2:period:1",
        ]
        for k in keys:
    ...
    async def test_dry_run_counts_without_deleting(self, redis: fakeredis.FakeRedis, user_id: str | None) -> None:
        await redis.set("ratelimit:cost:user:user_cost_burst:insights_code:100", "1.0")
        await redis.set("ratelimit:cost:user:user_cost_sustained:insights_code:100:period:0", "1.0")

        deleted = await reset_usage(redis, dry_run=True, user_id=user_id)

        assert deleted == 2
        assert await redis.get("ratelimit:cost:user:user_cost_burst:insights_code:100") is not None
        assert await redis.get("ratelimit:cost:user:user_cost_sustained:insights_code:100:period:0") is not None
    ...
    async def test_user_id_resets_only_that_user(self, redis: fakeredis.FakeRedis) -> None:
        target_keys = [
            "ratelimit:cost:user:user_cost_burst:insights_code:100",
            "ratelimit:cost:user:user_cost_sustained:insights_code:100:period:0",
            "ratelimit:cost:user:user_cost_burst:insights_code:100:tm2",
            "ratelimit:cost:user:user_cost_sustained:insights_code:100:tm2:period:1",
        ]
        ...
        other_keys = [
            "ratelimit:cost:user:user_cost_burst:insights_code:1",
            "ratelimit:cost:user:user_cost_sustained:insights_code:1:period:0",
            "ratelimit:cost:user:user_cost_burst:insights_code:1000",
            "ratelimit:cost:user:user_cost_sustained:insights_code:1000:period:0",
            "ratelimit:cost:user:user_cost_burst:insights_code:200",
        ]
    ...
    async def test_user_id_glob_metachars_do_not_expand_match(self, redis: fakeredis.FakeRedis) -> None:
        ...
        survivors = [
            "ratelimit:cost:user:user_cost_burst:insights_code:100",
            "ratelimit:cost:user:user_cost_burst:insights_code:1000",
            "ratelimit:cost:user:user_cost_burst:insights_code:10abc",
            "ratelimit:cost:user:user_cost_sustained:insights_code:100:period:0",
        ]
        for k in survivors:
            await redis.set(k, "1.0")
