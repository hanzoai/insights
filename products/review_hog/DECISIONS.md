...
repository, branch, …)` builds `CustomPromptSandboxContext` inline. Each fan-out child bounds its sandbox-turn
    ...
    (`start_temporal_worker.py` + `tach.toml` `insights → products.review_hog` + the `container-images-cd.yml`
...
"lens_result"` → `PERSPECTIVE_RESULT = "perspective_result"` (a **Django migration** generated via
  ...
  - **Move the three perspectives into DB-synced LLMA skills (the full Signals-scout pattern, "v2").** On disk,
    `products/review_hog/skills/review-script-perspective-{logic-correctness,contracts-security,performance-reliability}/SKILL.md`
...
- **Validator as a team-customizable skill — step 13 (✅ built; as-built + original design here).** _As built:_ the
  ...
  `products/review_hog/skills/review-script-validation-criteria/SKILL.md` mirrored into per-team `LLMSkill` rows by
...
- **New:** `products/review_hog/backend/api/trigger.py` + `backend/routes.py`; `insights/settings/access.py` (3 settings);
