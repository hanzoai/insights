...
/// alike — `insights_person` is deliberately not reachable from here.
///
...
/// swaps in the personinsights-identity GetOrCreatePersonsByDistinctIds RPC
/// instead.
...
mod tests {
    ...
    fn validate_table_name_gates_what_reaches_sql_interpolation() {
        for ok in ["insights_person", "personinsights_person_tmp", "t1"] {
            assert!(validate_table_name(ok).is_ok(), "{ok} should be valid");
        }
        for bad in [
            "",
            "insights_person; DROP TABLE insights_person",
            "table-name",
            "table name",
            "table\"quoted",
        ] {
