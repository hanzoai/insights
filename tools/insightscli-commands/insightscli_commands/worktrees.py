"""Clean up unused agent worktrees (Claude Code, Codex, Insights Desktop).

These tools each create throwaway git worktrees that accumulate over time. A
single Insights worktree carries multi-GB `node_modules`, Python venvs, and Rust
...
The discovery roots and dependency patterns are Insights-specific, but the age
...
# Claude Code and Codex nest worktrees directly under the repo; Insights Desktop
# keeps them in the home directory as <id>/insights. Returned as (source, root).
def _worktree_roots(repo_root: Path) -> list[tuple[str, Path]]:
    return [
        ("claude", repo_root / ".claude" / "worktrees"),
        ("codex", repo_root / ".codex" / "worktrees"),
        ("insights-code", Path.home() / ".insights-code" / "worktrees"),
    ]
...
@click.command(
    ...
    help="Clear out unused Claude Code / Codex / Insights Desktop worktrees by age",
)
@click.option(
    "--before",
    default=None,
...
@click.option(
    ...
    "sources",
    multiple=True,
    type=click.Choice(["claude", "codex", "insights-code"], case_sensitive=False),
...
@click.option(
    ...
    "Lets you reuse this on repos that don't ship insightscli.",
)
@click.option(
    "--include-dirty",
    is_flag=True,
...
def worktrees_clean(
    ...
    if before is None or mode is None:
        raise click.UsageError(
            ...
            "Examples:\n"
            "  insightscli worktrees:clean --before 7d --mode deps     # free deps in worktrees idle >7 days\n"
            "  insightscli worktrees:clean --before 2026-06-01 --mode full\n"
            "  insightscli worktrees:clean --before all --mode full --dry-run\n"
            "  insightscli worktrees:clean --before 30d --mode full --repo ~/Projects/other-repo"
        )
    ...
    for source, root in _worktree_roots(repo_root):
        ...
        for path in _discover_worktrees(root):
            ...
            if resolved in protected:
                continue
            # Insights Desktop worktrees live outside the repo and the home root is
...
def _discover_worktrees(root: Path, max_depth: int = 2) -> list[Path]:
    ...
    Depth 1 covers Claude/Codex (root/<name>); depth 2 covers Insights Desktop
    (root/<id>/insights). Found worktrees are not descended into.
    """
...
def _belongs_to_repo(resolved_path: Path, repo_root: Path, repo_common: Path | None) -> bool:
    ...
    worktrees (Insights Desktop's home root is shared across repos), confirm via the
...
def _free_by_device(anchors: Sequence[Path]) -> dict[int, int]:
    ...
    volumes (e.g. the repo vs the home-dir insights-code root) independent.
    """
...
def _cleanup_empty_parent(path: Path) -> None:
    """Remove a now-empty parent (e.g. Insights Desktop's <id>/ after <id>/insights)."""
