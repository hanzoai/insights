...
from insights.insightsql_queries.ai.utils import HEAVY_COLUMN_NAMES, HEAVY_COLUMN_TO_PROPERTY
...
def _setup_team():
    ...
    User.objects.create_and_join(org, "test-evaluations@hanzo.ai", "testpassword123")
    return team
...
class TestEvaluationConfigsApi(APIBaseTest):
    ...
    def test_switching_existing_evaluation_to_llm_judge_requires_model_configuration(
        ...
        eval_obj.refresh_from_db()
        self.assertEqual(eval_obj.evaluation_type, "script")

    @parameterized.expand(
        [
            ("script", "script", "boolean", {"source": "return true"}),
...
class TestTestHogEndpoint(APIBaseTest):
    ...
    def _mock_insightsql_response(self, count=1):
        from insights.insightsql.query import InsightsQLQueryResponse
    ...
    def test_test_hog_loads_ai_input_and_output(self, mock_query, mock_report_user_action):
        ...
        self.assertEqual(query.select_from.table.chain, ["insights", "ai_events"])
