...
class TestGenerateSuggestionsForTeam(BaseTest):
    def test_skips_inactive_team(self) -> None:
        ...
        mock_count.assert_not_called()  # gate short-circuits before any further Datastore work
    ...
    def test_generates_and_validates(self) -> None:
        ...
        # (the examples, and the model's free-text reason which can quote them) must never be
        # stored in them.
        self.assertNotIn("examples", payload["rules"][0])
        self.assertNotIn("reason", payload["rules"][0])

    def test_error_is_captured_not_raised(self) -> None:
        self.team.path_cleaning_filters = []
        self.team.save()
        with patch.object(service, "count_distinct_pathnames", side_effect=RuntimeError("datastore down")):
            result = generate_suggestions_for_team(self.team, visited_within_days=None)

        self.assertEqual(result.status, "error")
        self.assertIn("datastore down", result.error or "")

    def test_visit_gate_failure_is_captured_not_raised(self) -> None:
        # The gate query runs inside the per-team guard: a Datastore failure there must
        # produce an error result, not propagate out of a cohort sweep.
        self.team.path_cleaning_filters = []
        self.team.save()
        with patch.object(service, "has_recent_pageviews", side_effect=RuntimeError("datastore down")):
            result = generate_suggestions_for_team(self.team, visited_within_days=30)
