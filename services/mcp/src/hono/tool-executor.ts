...
import { EXECUTE_SQL_TOOL_NAME } from '@/tools/insightsAiTools/executeSql'
...
export class ToolExecutor {
    ...
    private injectContext(tools: ListToolsResult['tools']): ListToolsResult['tools'] {
        try {
            return getInsightsClient().prepareToolList(tools)
        } catch {
            return tools
        }
    }
    ...
    private extractIntent(
        ...
        try {
            const prepared = getInsightsClient().prepareToolCall(toolName, rawArgs)
            return {
    ...
    private resolveExecTool(
        ...
        const trackInnerCall: ExecInnerCallTracker = (toolName, properties) => {
            ...
            // dispatcher. The Insights event is intentionally NOT emitted here: the wrapper
...
interface ToolErrorClassification {
    ...
    /** Upstream HTTP status, when the failure came from a Insights API error. */
    status?: number
...
function resolveToolErrorClassification(error: unknown): ToolErrorClassification {
    ...
    if (findInsightsPermissionError(error)) {
    ...
    if (apiError instanceof InsightsValidationError) {
        return { errorType: 'validation' }
    }
    if (apiError instanceof InsightsApiError && apiError.status === 429) {
        return { errorType: 'rate_limited', status: apiError.status }
    }
    if (apiError instanceof InsightsApiError && apiError.status >= 500) {
        return { errorType: 'api_5xx', status: apiError.status }
    }
    if (apiError instanceof InsightsApiError) {
 ...
 * fragments) and `InsightsApiError`'s default message embeds the upstream
...
function resolveSafeErrorMessage(error: unknown): string | undefined {
    ...
    if (apiError instanceof InsightsValidationError) {
        ...
        // offending InsightsQL/filter expression (`/query/` resolver errors quote the bad name),
