...
jest.mock('lib/elements/Select', () => ({
    Select: ({ onChange, options, value }: any): JSX.Element => {
...
describe('Modifiers', () => {
    ...
    it('renders a connection selector for InsightsQL queries', async () => {
        ...
        render(
            <Provider>
                <Modifiers
                    setQuery={setQuery}
                    query={{ kind: NodeKind.InsightsQLQuery, query: 'SELECT 1', modifiers: {} }}
                    response={null}
                />
            </Provider>
        )
    ...
    it('does not render a connection selector for non-InsightsQL queries', () => {
        const setQuery = jest.fn()
