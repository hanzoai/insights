...
from insights.insightsql.database.models import BooleanDatabaseField, DateTimeDatabaseField
...
def build_property_swapper(node: ast.AST, context: InsightsQLContext) -> None:
    ...
    from insights.models.materialized_column_slots import MaterializedColumnSlot, MaterializedColumnSlotState
    ...
    def visit_field(self, node: ast.Field):
        ...
        if isinstance(type, ast.PropertyType) and type.field_type.name == "person_properties" and len(type.chain) == 1:
            ...
            if isinstance(type.field_type.table_type, ast.BaseTableType):
                table = type.field_type.table_type.resolve_database_table(self.context).to_printed_insightsql()
                if table == "events":
                    if property_name in self.person_properties:
                        return self._convert_string_property_to_type(node, "person", property_name)
