...
from insights.models import ProxyRecord
...
from insights.temporal.proxy_service.common import (
    CaptureEventInputs,
...
from insights.temporal.proxy_service.proto import CertificateState_READY, StatusRequest
...
async def check_proxy_is_live(inputs: CheckActivityInput) -> CheckActivityOutput:
    ...
    try:
        ...
        # point us at internal targets (Datastore's HTTP interface, cloud metadata, management
        # APIs) - an SSRF. A working proxy answers /i/v0/e/ with a 2xx directly. Same protection
        # as the on-demand diagnostics probe in insights/api/proxy_record_diagnostics.py
        # (_check_live_event).
...
class MonitorManagedProxyWorkflow(InsightsWorkflow):
