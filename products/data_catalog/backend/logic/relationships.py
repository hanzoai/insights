...
from insights.insightsql.database.lazy_join_tags import DATA_WAREHOUSE, DATA_WAREHOUSE_EXPERIMENTS
...
def _canonicalize_join_key(key: str) -> str:
    try:
        return parse_expr(key).to_insightsql()
    except InsightsQLSyntaxError as error:
        raise ValidationError({"keys": f"Invalid InsightsQL join key: {error}"}) from error
...
def _probe_join(
    ...
    try:
        ...
        execute_insightsql_query(
            query=validation_query, team=proposal.team, context=InsightsQLContext(database=database, user=user)
        )
