...
export function initInsights(): void {
    if (typeof window !== 'undefined') {
        const demoApiToken = process.env.NEXT_PUBLIC_POSTFN_KEY
        if (!demoApiToken) {
            console.warn(
                'NEXT_PUBLIC_POSTFN_KEY is not set, skipping Insights initialization.\n' +
                    'Run "npm run fetch-key" to automatically fetch the key from the database.'
            )
            return
        }
        const localApiHost = process.env.NEXT_PUBLIC_POSTFN_HOST || 'http://localhost:8010'
