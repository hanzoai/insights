"""`insightscli devbox:sync` -- one-way-safe local->remote file sync for Coder devboxes.

Lets engineers edit code locally while running the full Insights dev stack
...
_REMOTE_REPO_PATH = "/home/coder/insights"


def _detect_local_insights_checkout() -> Path:
    """Find the local Insights checkout root by walking up from cwd.

    A checkout is identified by a co-located ``insightscli.yaml`` and ``.git``
    directory at the same level. Bails with a hint if we can't find one --
    running ``insightscli devbox:sync`` from anywhere outside the repo is almost
    certainly a mistake.
    """
    cwd = Path.cwd().resolve()
    for candidate in (cwd, *cwd.parents):
        if (candidate / "insightscli.yaml").is_file() and (candidate / ".git").exists():
            return candidate
    _fail(
        "Could not find a Insights checkout above the current directory.\n"
        "Run `insightscli devbox:sync` from inside a `insights` clone."
    )
...
def _ensure_ssh_config_for_workspace(workspace: str) -> None:
    ...
    if result.returncode != 0:
        _fail(
            f"SSH config for `{host}` is missing. Run `insightscli devbox:setup` to install "
...
@click.command(name="devbox:sync", help="Mirror your local Insights checkout to a devbox")
@workspace_argument
...
def cmd_sync(
    ...
    if mutagen.sync_list(label_selector=label):
        click.echo(
            f"Sync already running for {name}. Use `insightscli devbox:sync{_workspace_arg_suffix(name)} --status` to inspect."
        )
        return
    ...
    local_path = _detect_local_insights_checkout()
    ...
    click.echo(f"Inspect: insightscli devbox:sync{_workspace_arg_suffix(name)} --status")
...
def _ensure_workspace_running(name: str, workspaces: list[dict[str, Any]] | None) -> None:
    ...
    if status != "running":
        _fail(f"Devbox '{name}' is {status}, not running. Run `insightscli devbox:start` first.")
