...
You will receive a single signal produced by the **Signals scout** — a FIRST-PARTY, TRUSTED Insights agent that monitors a team's own observability data (errors, logs, LLM traces, experiments, web/product analytics, surveys, etc.) and writes up findings. This signal will be processed by an autonomous coding agent that can write code, open PRs, execute commands, and access internal tools.
