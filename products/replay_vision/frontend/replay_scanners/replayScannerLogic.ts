        ...
        scanner: {
            ...
            submit: async (scanner: ReplayScanner) => {
                ...
                try {
                    ...
                    toast.error(`Failed to save scanner${error.detail ? `: ${error.detail}` : ''}`)
                    throw error
                }
            },
        },
            ...
            null as { cohort_id: number } | null,
            {
                saveAffectedCohort: async ({ tag }) => {
                    ...
                    try {
                        ...
                        toast.error(`Failed to create cohort${error?.detail ? `: ${error.detail}` : ''}`)
                        return values.affectedCohort
                    }
                },
            },
            ...
            [] as TagSuggestionApi[],
            {
                loadTagSuggestions: async () => {
                    ...
                    try {
                        ...
                        toast.error(`Couldn't generate suggestions${error?.detail ? `: ${error.detail}` : ''}`)
                        return []
                    }
                },
            },
        ...
        return {
            loadScanner: async () => {
                ...
                try {
                    ...
                    toast.error(`Failed to load scanner${error.detail ? `: ${error.detail}` : ''}`)
            ...
            toggleEnabled: async () => {
                ...
                try {
                    ...
                    toast.error(`Failed to ${verb} scanner${error.detail ? `: ${error.detail}` : ''}`)
                    actions.toggleEnabledFailure()
                }
            },
            ...
            triggerOnDemandObservation: async ({ sessionId, silent }) => {
                ...
                try {
                    ...
                    toast.error(`Failed to scan session${error.detail ? `: ${error.detail}` : ''}`)
                    actions.triggerOnDemandObservationFailure()
                }
            },
            ...
            copyAllObservations: async (_, breakpoint) => {
                try {
                    ...
                    if (texts.length === 0) {
                        toast.info('No results to copy for the current filters.')
                        return
                    }
