Executes InsightsQL — Insights's variant of SQL that supports most of Datastore SQL. "InsightsQL" and "SQL" are used interchangeably.
...
- **Searching or listing existing Insights entities** — insights, dashboards, cohorts, feature flags, experiments, surveys. No `query-*` tool covers these; query the `system.*` tables.
...
### Large LLM trace fields live on the `insights.ai_events` table, not `events.properties`

For LLM events (`$ai_generation`, `$ai_trace`, `$ai_span`, etc.) the heavy keys are **not stored on `events.properties`** — they live as native columns on a dedicated Datastore table. Like `insights.trace_spans` / `insights.metrics`, reference it as `insights.ai_events` (a bare `FROM ai_events` errors with "Unknown table"):
...
`insights.ai_events` is `ORDER BY (team_id, trace_id, timestamp)`, so **anchor on `trace_id`, never scan by `timestamp`**. Rows are dropped after the retention period (30 days by default), so older traces have no content. Nothing restricts which heavy columns an event can carry, but the typical shape is: `$ai_generation` carries `input` / `output_choices` / `tools` (embeddings carry `input`); `$ai_span` and `$ai_trace` carry `input_state` / `output_state`.

- **Single trace** — `SELECT input, output_choices FROM insights.ai_events WHERE trace_id = '<id>' ORDER BY timestamp`.
- **Batch / analytics** — filter the timestamp-indexed `events` table first to get the trace IDs, then fetch heavy content from `insights.ai_events` anchored on `trace_id`:
...
The `query-llm-trace` / `query-llm-traces-list` tools read `insights.ai_events` for you; prefer them when a single trace's content is all you need.

### Observability data-plane tables: `logs`, `insights.trace_spans`, `insights.metrics`

Insights ingests OpenTelemetry signals into three Datastore-backed tables that are queryable via InsightsQL. **Note the namespacing asymmetry** — `logs` is registered at the root, while `trace_spans` and `metrics` live under the `insights.` namespace and must be referenced as such (e.g. `FROM insights.trace_spans`):

- `logs` — log entries. Common fields: `body` (also exposed as `message`), `severity_text`, `severity_number`, `service_name`, `attributes`, `resource_attributes`, `trace_id`, `span_id`, `timestamp`. Prefer `insights:query-logs` for filtered list queries; reach for SQL for aggregations across services or joins with `insights.trace_spans` / `insights.metrics` by `trace_id`.
- `insights.trace_spans` — OpenTelemetry spans. Common fields: `trace_id`, `span_id`, `parent_span_id`, `is_root_span`, `name`, `service_name`, `kind` (0-5), `status_code` (0 Unset, 1 OK, 2 Error), `duration_nano`, `timestamp`, `end_time`, `attributes`, `resource_attributes`. Prefer `insights:query-apm-spans` / `insights:apm-trace-get` for span listing and full-trace fetches; reach for SQL for joins with `logs` / `insights.metrics` by `trace_id`, exemplar lookups, or aggregations the typed tools don't expose.
- `insights.metrics` — OpenTelemetry metric points. Common fields: `metric_name`, `metric_type` (counter/gauge/histogram), `value`, `count`, `histogram_bounds`, `histogram_counts`, `unit`, `aggregation_temporality` (`delta` or `cumulative`), `is_monotonic`, `service_name`, `trace_id`, `span_id`, `attributes`, `resource_attributes`, `timestamp`. No typed `query-metrics` tool — use SQL. A projection pre-aggregates by `(team_id, time_bucket, toStartOfMinute(timestamp), service_name, metric_name, metric_type, resource_fingerprint)` with `count/sum/min/max(value)`, so per-minute aggregations grouped by those keys are very cheap. **Important for counters:** check `aggregation_temporality` before aggregating — `SUM(value)` over a window is only correct for `delta`. For `cumulative` counters, the value is a running total, so use `argMax(value, timestamp)` (or `max(value) - min(value)` for a rate) to avoid double-counting. Always filter or split by `aggregation_temporality` when both regimes can appear for a given `metric_name`.

All three share `team_id`, `time_bucket`, `service_name`, `resource_fingerprint`, and where applicable `trace_id`. `trace_id` on `insights.metrics` is the OpenTelemetry exemplar pattern — a metric anomaly can be drilled into via a sample `trace_id` to pull the full trace and correlated logs. **Note:** exemplar extraction is not yet wired up in the ingestion pipeline (the `_exemplars` argument is unused in `rust/capture-logs/src/metric_record.rs`), so `insights.metrics.trace_id` is always empty today. The pattern below describes the intended capability; the "works today" alternative anchors on `insights.trace_spans` instead.

**`trace_id` format:** Both `logs` and `insights.trace_spans` store `trace_id` as a 24-character base64-encoded 16-byte value (e.g. hex `21EDB3A025A9ECD32ADF3E5D7548A4F4` encodes to `Ie2zoCWp7NMq3z5ddUik9A==`). The MCP API layer converts to hex via `hex(tryBase64Decode(trace_id))` for display. Raw InsightsQL queries see the base64 form. Unset = `'AAAAAAAAAAAAAAAAAAAAAA=='`. `insights.metrics.trace_id` is a plain string, empty when unset (once exemplars land it will match the base64 format of the other two tables). Joins are direct equality on `trace_id`.

User InsightsQL queries against `logs`, `insights.trace_spans`, and `insights.metrics` are capped at 50 GB read per query to prevent unbounded scans.
...
<reasoning>
- Anchoring on `insights.trace_spans` works today because spans always carry a populated `trace_id`. Once `insights.metrics` exemplars land, the CTE can be swapped for `argMax(trace_id, value) FROM insights.metrics WHERE … AND trace_id != ''` to drill from a metric spike instead.
- `trace_id` is stored as base64 in both `logs` and `insights.trace_spans`, so direct equality works — no decoding needed.
- `UNION ALL` with a `source` discriminator avoids three separate calls and keeps the timeline interleaved.
- Note `logs` is root-level while `insights.trace_spans` and `insights.metrics` require the namespace prefix.
