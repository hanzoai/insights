...
use personinsights_coordination::strategy::{
...
async fn graceful_drain_transfers_partitions(
    ...
    wait_for_condition(WAIT_TIMEOUT, POLL_INTERVAL, || {
        ...
        async move {
            ...
            !pods.iter().any(|p| {
                p.pod_name == "writer-0"
                    && p.status == personinsights_coordination::types::PodStatus::Ready
            })
        }
    })
    .await;
...
async fn a_dead_new_owner_handoff_is_reaffirmed_to_the_current_owner() {
    use personinsights_coordination::types::{HandoffPhase, HandoffState};
...
async fn stale_handoff_address_never_overwrites_a_fresher_registration() {
    ...
    store
        .put_handoff(&personinsights_coordination::types::HandoffState {
            partition: 0,
            old_owner: None,
            new_owner: "writer-0".to_string(),
            new_owner_address: Some("10.0.0.1:50053".to_string()),
            phase: personinsights_coordination::types::HandoffPhase::Complete,
            started_at: 0,
...
async fn pod_re_registration_refreshes_router_addresses_without_a_handoff() {
    ...
    store
        .register_pod(
            &personinsights_coordination::types::RegisteredPod {
                pod_name: "writer-0".to_string(),
                generation: String::new(),
                status: personinsights_coordination::types::PodStatus::Ready,
                registered_at: now,
                last_heartbeat: now,
                controller: None,
...
async fn reconcile_advances_warming_with_pre_staged_warmed_ack() {
    use personinsights_coordination::types::{HandoffPhase, HandoffState, PodWarmedAck};
...
async fn draining_old_owner_blocks_phase_advance() {
    use personinsights_coordination::types::{
...
async fn draining_old_owner_does_not_trigger_cleanup() {
    use personinsights_coordination::types::{HandoffPhase, HandoffState, PodStatus, RegisteredPod};
...
async fn freezing_blocks_until_routers_ack_before_draining() {
    ...
    let router_lease = store.grant_lease(60).await.unwrap();
    let slow_router = personinsights_coordination::types::RegisteredRouter {
    ...
    store
        .put_freeze_ack(&personinsights_coordination::types::RouterFreezeAck {
            router_name: "slow-router".to_string(),
            partition: 1,
            acked_at: 0,
            acked_at_ms: 0,
