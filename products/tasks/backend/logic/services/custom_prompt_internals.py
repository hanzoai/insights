...
# the Insights side can surface the real cause instead of Temporal's opaque
# "Activity task failed" wrapper.
AGENT_ERROR_METHOD = "_insights/error"

# Observability side-channels the relay interleaves into the turn log asynchronously
# (agentsh network-audit dumps and sandbox credential refreshes ride on `_insights/console`,
# sandbox stdout/stderr on `_insights/sandbox_output`, setup steps on `_insights/progress`).
...
TRANSIENT_SIDE_CHANNEL_METHODS = frozenset(
    {
        "_insights/console",
        "_insights/progress",
        "_insights/sandbox_output",
    }
)

# `_insights/progress` is normally an informational setup step (status in_progress/completed), but the
...
class CustomPromptSandboxContext:
    ...
    insights_mcp_scopes: InsightsMcpScopes | None = None
    model: str | None = None
...
async def create_task_and_trigger(
    ...
    insights_mcp_scopes: InsightsMcpScopes = (
        context.insights_mcp_scopes if context.insights_mcp_scopes is not None else "full"
    )
...
def _extract_agent_error(log_content: str | None, skip_lines: int = 0) -> AgentError | None:
    ...
    Returns the last `_insights/error` entry carrying a non-empty message (with the
...
def _is_failed_progress(notification: dict) -> bool:
    """True for a `_insights/progress` notification carrying the failed/cancelled status the workflow's
    exception and cancel handlers emit before writing the terminal TaskRun status."""
    if notification.get("method") != "_insights/progress":
        return False
    params = notification.get("params")
...
def _ended_on_pending_finalization(full_log: str | None) -> bool:
    ...
    Any other trailing turn-relevant notification (an end_turn/result, a `_insights/error`, a mid-turn
    ...
    timeout and being marked failed. The one exception is a failed/cancelled `_insights/progress`
