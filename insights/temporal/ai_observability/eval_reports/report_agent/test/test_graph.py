...
from insights.temporal.ai_observability.eval_reports.report_agent import graph
from insights.temporal.ai_observability.eval_reports.report_agent.graph import (
...
class TestComputeMetrics(SimpleTestCase):
    ...
    def test_transient_query_failure_returns_no_metrics(self, mock_fetch):
        mock_fetch.side_effect = DatastoreAtCapacity()
...
class TestRunEvalReportAgentRouting(SimpleTestCase):
    ...
    @patch.object(graph, "hanzo_insights")
...
class TestRunEvalReportAgentMetricsUnavailable(SimpleTestCase):
    ...
    def test_metrics_unavailable_skips_agent_and_returns_fallback(
        ...
        with (
            patch("insights.temporal.ai_observability.eval_reports.metrics.increment_errors") as mock_increment_errors,
            patch(
                "insights.temporal.ai_observability.eval_reports.metrics.increment_report_generated"
