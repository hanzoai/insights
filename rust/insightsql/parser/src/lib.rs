//! Hand-rolled Rust InsightsQL parser — ALL(*) variant.
//!
//! Forked from `insightsql_parser_rs` at the ~99.88% cpp-parity baseline.
...
//! envelope expected by [`insights/insightsql/json_ast.py`] so the Python side
//! can raise `ExposedInsightsQLError` / `SyntaxError` from it.
    ...
    F: FnOnce(emit_py::PyEmitter<'py>) -> Result<emit_py::PyAst, error::ParseError>,
{
    ...
    let result = match outcome {
        ...
        Err(panic) => {
            ...
            // Genuine (non-PyErr) panic: wrap as a `NotImplementedError` envelope so production callers catching the `ExposedInsightsQLError` family intercept it, instead of PyO3 surfacing a raw `PanicException`.
            let msg = panic
                .downcast_ref::<&'static str>()
                .copied()
                .map(str::to_string)
...
/// Raise the matching `insights.insightsql.errors` exception for `err`, importing only the errors module (not the AST/enum-laden `Converter`).
...
#[pymodule]
fn insightsql_parser_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    #[cfg(feature = "coverage")]
    {
        ...
        // `hasattr(insightsql_parser_rs, "cov_snapshot")` and gracefully skips the
