...
export function buildElementStatsParams(
    ...
    // generated schema, hence the widening cast - see the stats action in insights/api/element.py
    return {
...
export type recordingClickmapLogicType = MakeLogicType<
    recordingClickmapLogicValues,
    recordingClickmapLogicActions,
    RecordingClickmapLogicProps,
    recordingClickmapLogicMeta
>
...
export const recordingClickmapLogic = kea<recordingClickmapLogicType>([
    ...
    selectors({
        ...
        tooltipSuppressed: [
            ...
            (hoveredBoxKey: string | null, selectedBoxKey: string | null): boolean =>
                hoveredBoxKey !== null || selectedBoxKey !== null,
        ],
    }),
    ...
    listeners(({ actions, values, props }) => ({
        setClickmapEnabled: ({ enabled }) => {
            insights.capture('in-app heatmap clickmap toggled', { enabled })
            if (enabled) {
        ...
        recomputeClickmap: async (_, breakpoint) => {
            ...
            insights.capture('in-app heatmap clickmap rendered', {
