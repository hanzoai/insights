...
# Datastore node and the MSK pipeline is the only consumer for several of
...
operations = (
    ...
    if settings.CLOUD_DEPLOYMENT not in ("US", "EU", "DEV")
        ...
        run_sql_with_exceptions(
            "DROP TABLE IF EXISTS insights_document_embeddings_mv",
            node_roles=[NodeRole.INGESTION_SMALL],
        ),
        run_sql_with_exceptions(
            "DROP TABLE IF EXISTS kafka_insights_document_embeddings",
            node_roles=[NodeRole.INGESTION_SMALL],
        ),
