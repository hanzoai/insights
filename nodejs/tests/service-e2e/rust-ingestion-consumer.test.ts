...
const POSTGRES_URL = 'postgres://insights:insights@localhost:5432'
...
// so we never delete the shared topics that Datastore's Kafka consumers subscribe to.
...
describe('Rust ingestion consumer with Node ingestion API workers', () => {
    ...
    beforeEach(async () => {
        ...
        await waitForDatastoreKafkaConsumer(datastore)
    ...
    test('Rust consumer ingests Kafka events through multiple real Node ingestion API workers', async () => {
        ...
        try {
            ...
            await waitForEvents(datastore, initialEvents)
            await assertKafkaIngestionState(groupId, initialEvents)

            const ingestedEvents = await fetchEvents(datastore, TEST_TEAM_ID)
    ...
    test('Rust consumer retries a transient Node ingestion API failure without duplicate output', async () => {
        ...
        try {
            ...
            await waitForEvents(datastore, events)
    ...
    test('Rust consumer commits a mixed batch after Node DLQs an invalid event', async () => {
        ...
        try {
            ...
            await waitForEvents(datastore, validEvents)
    ...
    test('Rust consumer handles healthy, failed, and recovered Node ingestion API workers across batches', async () => {
        ...
        try {
            ...
            await waitForEvents(datastore, allEvents)
...
function configureParentTestEnv(): void {
    ...
    process.env.DATABASE_URL = `${POSTGRES_URL}/test_insights`
    ...
    process.env.DATASTORE_HOST = 'localhost'
    process.env.DATASTORE_DATABASE = 'insights_test'
...
function testDependencyEnv(): NodeJS.ProcessEnv {
    return {
        DATABASE_URL: `${POSTGRES_URL}/test_insights`,
...
function testTopicEnv(): NodeJS.ProcessEnv {
    return {
        ...
        DATASTORE_JSON_EVENTS_KAFKA_TOPIC: KAFKA_EVENTS_JSON,
        DATASTORE_AI_EVENTS_KAFKA_TOPIC: 'datastore_ai_events_json_test',
        DATASTORE_HEATMAPS_KAFKA_TOPIC: 'datastore_heatmap_events_test',
        INGESTION_OUTPUT_EVENTS_TOPIC: KAFKA_EVENTS_JSON,
        INGESTION_OUTPUT_AI_EVENTS_TOPIC: 'datastore_ai_events_json_test',
        INGESTION_OUTPUT_HEATMAPS_TOPIC: 'datastore_heatmap_events_test',
        INGESTION_OUTPUT_INGESTION_WARNINGS_TOPIC: 'datastore_ingestion_warnings_test',
        ...
        INGESTION_OUTPUT_GROUPS_TOPIC: 'datastore_groups_test',
        INGESTION_OUTPUT_PERSONS_TOPIC: 'datastore_person_test',
...
async function waitForEvents(datastore: Datastore, events: TestEvent[]): Promise<void> {
    await waitForExpect(async () => {
        const ingestedEvents = await fetchEvents(datastore, TEST_TEAM_ID)
