...
def _channel_blocks(integration: Integration, slack: SlackIntegration, *, done: bool) -> list[dict]:
    ...
    if done:
        blocks.append(_done("Posting to #insights-inbox"))
        return blocks
    ...
    name = channel_name_from_target(configured or "") if channel_exists else "#insights-inbox"
    if channel_exists and has_scope:
        ...
        blocks.append(
            {"type": "actions", "elements": [_button("Create #insights-inbox", INBOX_CREATE_ACTION_ID, value)]}
        )
    else:
        blocks.append(
            {
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": f"Create a #insights-inbox channel, then set it in your <{_public_url('/inbox')}|inbox>.",
                    }
                ],
            }
        )
...
def _ai_approval_blocks(integration: Integration, *, is_admin: bool) -> list[dict]:
    ...
    blocks: list[dict] = [
        _section(
            ...
            "with Insights won't cover these features."
        )
    ]
    if not is_admin:
        blocks.append(
    ...
    blocks.append(
        {
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": "🎉 Once you're set up, I'll start watching your product and send your first report to #insights-inbox as soon as I spot something.",
                }
            ],
        }
    )
    return "Set up Insights - self-driving for your product", blocks
...
def _resolve_onboarding_user(slack: SlackIntegration, integration: Integration, slack_user_id: str) -> int | None:
    """Resolve the Slack user to a Insights user id (org-member email match), or None."""
...
def mark_channel_joined(
    ...
    swapped = _replace_actions_with_context(
        ...
        ":white_check_mark: You're in your #insights-inbox channel",
    )
    _chat_update_blocks(integration, channel, message_ts, "You're in your #insights-inbox channel", swapped)
    capture_slack_event(
