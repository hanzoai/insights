...
def _insightsql_config() -> dict[str, Any]:
    return {"type": "InsightsQLAlertConfig", "evaluation": "last_row"}


def _insightsql_query() -> dict[str, Any]:
    return {"kind": "InsightsQLQuery", "query": "SELECT count() FROM events"}
...
class TestValidateAlertConfig:
    @parameterized.expand(
        [
            ...
            (
                "insightsql_config_with_trends_query_rejected",
                _base_query(),
                _base_condition(),
                _insightsql_config(),
                _base_threshold(),
                "daily",
                "SQL alert config requires a InsightsQLQuery insight",
            ),
            (
                "insightsql_absolute_condition_with_percentage_threshold_rejected",
                _insightsql_query(),
                _base_condition("absolute_value"),
                _insightsql_config(),
    ...
    @parameterized.expand(
        [
            ("insightsql", _insightsql_query(), _insightsql_config()),
    ...
    def test_any_row_insightsql_alert_rejects_relative_conditions(self) -> None:
    ...
    def test_invalid_insightsql_config_rejected(self) -> None:
        with pytest.raises(ValueError, match="invalid InsightsQLAlertConfig"):
            validate_alert_config(
                _insightsql_query(),
                _base_condition(),
                {"type": "InsightsQLAlertConfig", "evaluation": "sideways"},
                _base_threshold(),
                "daily",
            )

    def test_insightsql_config_without_evaluation_rejected(self) -> None:
    ...
    def test_first_row_insightsql_alert_accepts_relative_conditions(self) -> None:
        # Unlike any_row, first_row is a time axis (newest first), so relative is valid.
        validate_alert_config(
            _insightsql_query(),
            _base_condition("relative_increase"),
            {"type": "InsightsQLAlertConfig", "evaluation": "first_row"},
            _base_threshold(type="percentage"),
            "daily",
        )

    def test_detector_config_accepted_for_insightsql_insight(self) -> None:
        # SQL/InsightsQL insights support anomaly detection (last/first-row series), so a detector_config
    ...
    def test_detector_config_rejected_for_any_row_insightsql_alert(self) -> None:
