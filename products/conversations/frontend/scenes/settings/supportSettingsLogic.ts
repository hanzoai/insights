...
export interface supportSettingsLogicValues {
    ...
    emailVerifyingConfigId: string | null
    githubConnected: boolean
    githubIntegrations: {
        id: number
        name: string
    }[]
    githubIntegrationsLoading: boolean
    githubRepos: {
        full_name: string
        name: string
    }[]
    ...
    slackBotIconUrl: string | null
    slackBotIconUrlValue: string | null
    ...
    slackTicketEmoji: string
    slackTicketEmojiValue: string | null
    teamsChannelId: string | null
    teamsChannelName: string | null
    teamsChannelPairLoading: string | null
    teamsChannelPairs: {
        channel_id: string
    ...
    >
    teamsChannelsLoading: boolean
    teamsConnected: boolean
    teamsInstallStatus: 'error' | 'idle' | 'installed' | 'installing' | 'needs_org_catalog'
    teamsInstallingForTeamId: string | null
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface supportSettingsLogicActions {
    ...
    connectEmailDone: (config: EmailConfigStatus | null) => {
        config: EmailConfigStatus | null
    }
    connectGithub: (integrationId: number) => {
        integrationId: number
    }
    connectSlack: (nextPath: string) => {
        nextPath: string
    }
    connectTeams: (nextPath: string) => {
        nextPath: string
    }
    disconnectEmail: (configId: string) => {
        configId: string
    }
    disconnectEmailDone: (configId: string) => {
        configId: string
    }
    disconnectGithub: () => {
        value: true
    ...
    loadSlackChannelsWithTokenFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadSlackChannelsWithTokenSuccess: (
        slackChannels: SlackChannelType[],
        payload?: {
            value: true
        }
    ...
    loadTeamsChannelsForTeam: (teamId: string) => {
        teamId: string
    }
    loadTeamsChannelsForTeamFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadTeamsChannelsForTeamSuccess: (
        teamsChannels: {
            id: string
            membership_type?: string | null
            name: string
    ...
    loadTeamsTeamsWithTokenFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadTeamsTeamsWithTokenSuccess: (
        teamsTeams: {
            id: string
            name: string
        }[],
    ...
    sendTestEmailDone: (configId: string) => {
        configId: string
    }
    ...
    setAiSuggestionsLoading: (loading: boolean) => {
        loading: boolean
    }
    setConversationsEnabledLoading: (loading: boolean) => {
        loading: boolean
    }
    setDefaultEmail: (configId: string) => {
        configId: string
    }
    setDefaultEmailDone: (configId: string) => {
        configId: string
    }
    ...
    setNewEmailFromEmail: (value: string) => {
        value: string
    }
    setNewEmailFromName: (value: string) => {
        value: string
    }
    ...
    setSlackBotDisplayNameValue: (value: string | null) => {
        value: string | null
    }
    setSlackBotIconUrlValue: (value: string | null) => {
        value: string | null
    }
    setSlackChannels: (channelIds: string[]) => {
        channelIds: string[]
    }
    setSlackNotifyOnJoin: (enabled: boolean) => {
        enabled: boolean
    }
    setSlackNotifyOnLeave: (enabled: boolean) => {
        enabled: boolean
    }
    setSlackNudgeEnabled: (enabled: boolean) => {
        enabled: boolean
    }
    setSlackTicketEmojiValue: (value: string | null) => {
        value: string | null
    }
    ...
    verifyEmailDomain: (configId: string) => {
        configId: string
    }
    verifyEmailDomainDone: (
        configId: string,
        verified: boolean,
        dnsRecords: Record<string, any> | null
    ) => {
        configId: string
        dnsRecords: Record<string, any> | null
        verified: boolean
...
export interface supportSettingsLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        slackNotifyOnLeave: (currentTeam: TeamPublicType | TeamType | null) => boolean
        ...
        teamsChannelId: (currentTeam: TeamPublicType | TeamType | null) => string | null
        teamsChannelName: (currentTeam: TeamPublicType | TeamType | null) => string | null
        teamsChannelPairs: (currentTeam: TeamPublicType | TeamType | null) => {
            channel_id: string
            channel_name?: string | null
            membership_type?: string | null
            team_id: string
        ...
        aiAllChannels: (featureFlags: FeatureFlagsSet) => TicketChannel[]
...
export type supportSettingsLogicType = MakeLogicType<
    supportSettingsLogicValues,
    supportSettingsLogicActions,
    Record<string, any>,
    supportSettingsLogicMeta
>
            ...
            [] as { id: string; name: string; membership_type?: string | null }[],
            {
                loadTeamsChannelsForTeam: async ({ teamId }: { teamId: string }) => {
                    try {
                        ...
                        toast.error('Failed to load Teams channels')
    ...
    listeners(({ values, actions }) => ({
        ...
        saveSlackTicketEmoji: () => {
            ...
            if (emoji !== null) {
                ...
                toast.success('Ticket emoji saved')
            }
        },
        saveSlackBotSettings: () => {
            ...
            if (iconUrl && !iconUrl.startsWith('https://')) {
                toast.error('Icon URL must start with https://')
                return
            }
        ...
        connectEmail: async () => {
            ...
            if (!newEmailFromEmail || !newEmailFromName) {
                toast.error('Please enter both an email address and display name')
            ...
            try {
                ...
                toast.success('Email address connected')
            } catch {
        ...
        disconnectEmail: async ({ configId }) => {
            ...
            toast.success('Email address disconnected')
        },
        setDefaultEmail: async ({ configId }) => {
            ...
            toast.success('Default email address updated')
        },
        verifyEmailDomain: async ({ configId }) => {
            try {
                ...
                if (response.domain_verified) {
                    toast.success('Domain verified successfully! Outbound email is now active.')
                } else {
                    toast.warning('Domain not yet verified. Please check your DNS records and try again.')
                }
            } catch {
        ...
        sendTestEmail: async ({ configId }) => {
            try {
                ...
                toast.success(`Test email sent to ${response.sent_to}`)
            } catch {
        ...
        addTeamsChannelPair: async ({ teamId, channelId }) => {
            try {
                ...
                if (detail === 'max_channels_exceeded') {
                    toast.error('Maximum number of Teams channels reached')
                } else {
                    toast.error('Failed to add Teams channel')
                }
        ...
        removeTeamsChannelPair: async ({ channelId }) => {
            try {
                ...
                toast.error('Failed to remove Teams channel')
                actions.setTeamsChannelPairLoading(null)
                return
            }
            actions.loadCurrentTeam()
