...
from insights.auth import PersonalAPIKeyAuthentication, ProjectSecretAPIKeyAuthentication, TeamSecretTokenAuthentication
...
from insights.permissions import AccessControlPermission, InsightsFeatureFlagPermission
...
class TestAccessControlPermission(BaseTest):
    ...
    def test_has_permission_with_team_secret_token_authentication(self):
        ...
        from insights.auth import TeamSecretTokenAuthentication
...
class TestTeamSecretTokenPermission(BaseTest):
    ...
    def setUp(self):
        super().setUp()
        from insights.permissions import TeamSecretTokenPermission
...
class TestProjectSecretAPIKeyAPIScopePermission(SimpleTestCase):
    ...
    def _make_psak_request(self, scopes=("endpoint:read",), team_id=1):
        from insights.auth import ProjectSecretAPIKeyAuthentication
...
class TestTeamMemberAccessPermission(BaseTest):
    ...
    def setUp(self):
        super().setUp()
        from insights.permissions import TeamMemberAccessPermission
...
class TestOAuthAccessTokenAPIScopePermission(BaseTest):
    ...
    def test_forbids_wildcard_scope_for_internal_viewset(self):
        ...
        self.access_token.scope = "*"
        self.access_token.save()
        response = self._do_request("/v1/query_performance_proxy/execute-test/", method="POST")
        self.assertEqual(response.status_code, 403)
        self.assertIn("datastore_test_cluster_perf:read", response.json()["detail"])

    def test_allows_explicit_scope_for_internal_viewset(self):
        self.access_token.scope = "datastore_test_cluster_perf:read"
        self.access_token.save()
        response = self._do_request(
            "/v1/query_performance_proxy/execute-test/", method="POST", data={"sql": "SELECT 1"}
        )
    ...
    def test_forbids_wildcard_scope_for_internal_required_scope_on_public_viewset(self):
        ...
        self.access_token.save()
        response = self._do_request(
            f"/v1/projects/{self.team.id}/signals/scout/scratchpad/forget/",
            method="POST",
            data={"key": "noop"},
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_denies_derived_scope_for_write(self):
        ...
        response = self._do_request(f"/v1/projects/{self.team.id}/feature_flags/", method="POST")
        self.assertEqual(response.status_code, 403)
...
class TestOAuthAccessTokenWithOrganizationScoping(BaseTest):
    ...
    def test_denies_access_to_non_scoped_org_and_team(self):
        ...
        response = self._do_request(f"/v1/organizations/{other_org.id}/")
...
class TestOAuthAccessTokenWithTeamScoping(BaseTest):
    ...
    def test_denies_access_to_non_scoped_team(self):
        ...
        response = self._do_request(f"/v1/projects/{other_team.id}/feature_flags/")
        self.assertEqual(response.status_code, 403)
...
class TestOAuthAccessTokenWithBothTeamAndOrgScoping(BaseTest):
    ...
    def test_denies_access_to_other_team_in_scoped_org(self):
        ...
        response = self._do_request(f"/v1/projects/{self.team2.id}/feature_flags/")
    ...
    def test_denies_access_to_team_in_non_scoped_org(self):
        ...
        response = self._do_request(f"/v1/projects/{self.other_org_team.id}/feature_flags/")
        self.assertEqual(response.status_code, 403)
...
class TestOAuthAccessTokenExpiration(BaseTest):
    ...
    def _do_request(self, token=None):
        ...
        return self.client.get(
            f"/v1/projects/{self.team.id}/feature_flags/", headers={"authorization": f"Bearer {token}"}
        )
...
class TestOAuthAccessTokenUserMembership(BaseTest):
    ...
    def test_team_scoped_token_fails_when_user_not_in_team_org(self):
        ...
        response = self.client.get(
            f"/v1/projects/{other_team.id}/feature_flags/",
            headers={"authorization": f"Bearer {other_team_token.token}"},
        )
        self.assertEqual(response.status_code, 403)  # Forbidden - user not in org


class TestInsightsFeatureFlagPermission(BaseTest):
    def setUp(self):
        super().setUp()
        self.permission = InsightsFeatureFlagPermission()
        self.factory = APIRequestFactory()
    ...
    def _create_mock_view(self, flag="test-flag", action="list"):
        view = Mock()
        view.insights_feature_flag = flag
        view.action = action
    ...
    def test_delegates_to_hanzo_insights_by_default(self, mock_ff):
        request = self._create_mock_request()
    ...
    @patch("insights.permissions._FORCE_ENABLED_FLAGS", frozenset({"my-flag"}))
    @patch("hanzo_insights.feature_enabled")
    def test_force_enabled_bypasses_hanzo_insights(self, mock_ff):
        request = self._create_mock_request()
        view = self._create_mock_view(flag="my-flag")
    ...
    @patch("insights.permissions._FORCE_ENABLED_FLAGS", frozenset({"flag-a", "flag-b", "flag-c"}))
    @patch("hanzo_insights.feature_enabled")
    ...
    @patch("insights.permissions._FORCE_ENABLED_FLAGS", frozenset({"other-flag"}))
    @patch("hanzo_insights.feature_enabled", return_value=False)
