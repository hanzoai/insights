...
from insights.datastore.client.execute import KillSwitchLevel
...
class _DigestNotificationTestBase(APIBaseTest):
    def setUp(self):
        ...
        self.feature_enabled_patcher = patch(f"{ACTIVITIES}.hanzo_insights.feature_enabled", return_value=True)
        self.mock_feature_enabled = self.feature_enabled_patcher.start()

        self.get_flag_patcher = patch(f"{ACTIVITIES}.hanzo_insights.get_feature_flag", return_value="test")
...
class TestRunWaDigestNotificationBatch(_DigestNotificationTestBase):
    ...
    def test_isolates_per_org_failures(self):
        def fake_build_and_send(org_id, flag_key, dry_run):
            if org_id == "2":
                raise RuntimeError("datastore exploded for org 2")
