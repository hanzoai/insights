...
import { CyclotronJobInvocationGlobals, InsightsFunctionConfigurationContextId } from '~/types'
...
/**
 * Per-context overrides for the "load sample globals" flow in the script function test panel.
 * Contexts without an entry load the last event matching the configured filters.
 */
export const SAMPLE_GLOBALS_CONTEXTS: Partial<Record<InsightsFunctionConfigurationContextId, SampleGlobalsLoader>> = {
    'error-tracking': async (exampleGlobals) => {
        ...
        const properties: Record<string, any> = {
            ...
            description: 'Insights test alert',
            status: issue.status,
