...
class TestAssistantContextManager(BaseTest):
    ...
    @patch("ee.hogai.context.insight.context.execute_and_format_query")
    async def test_build_and_execute_insight_insightsql_query(self, mock_execute):
        mock_execute.return_value = "Query results: 42 events"

        insight = MaxInsightContext(
            id="101",
            name="Custom Query",
            description="InsightsQL analysis",
        ...
        self.assertIn("Description: InsightsQL analysis", result)
        self.assertIn("InsightsQLQuery", result)
