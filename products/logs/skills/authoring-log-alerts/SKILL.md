...
description: >
  Author useful, low-noise log alerts on services in a Insights project. Use when the user asks to set up
...
- Tuning an alert that already exists — that's a different job (use `insights:logs-alerts-events-list` to inspect
  fire/resolve cadence and `insights:logs-alerts-partial-update` to adjust).
- Investigating an active incident — pull rows with `insights:query-logs`, don't author an alert mid-incident.
...
Call `insights:logs-services` for the last 24h with no filters. The response is capped at 25 services and includes a
...
`insights:logs-attributes-list` (try `attribute_type: log`) and `insights:logs-attribute-values-list` to find a discriminator —
...
Call `insights:logs-count-ranges` with the candidate's filters, `dateRange: { date_from: "-7d" }`, and
...
Call `insights:logs-alerts-simulate-create` with these settings and `date_from: "-7d"`. The response gives you `fire_count`
and `resolve_count`.
...
1. Call `insights:logs-alerts-create` with the validated config. Use a name like `<service> error rate (auto)` so the
   user can see at a glance which alerts came from this skill.
2. Call `insights:logs-alerts-destinations-create` to wire it to a notification target. **An alert with no destination
...
The `filters` field on `insights:logs-alerts-create` takes a subset of `LogsViewerFilters` and **must contain at
least one of**:
...
The same shape goes into `insights:logs-alerts-simulate-create`'s `filters` field. Match the simulate filters to the alert filters
...
- One `insights:logs-services` call at the start, not per-candidate.
- One `insights:logs-count-ranges` call per candidate at `targetBuckets: 24`. Don't go above 30 during authoring.
- ≤ 3 `insights:logs-alerts-simulate-create` calls per candidate.
- Zero `insights:query-logs` calls during the authoring loop.
