...
# Cap on how many distinct utm_source values we pull from Datastore, ordered
...
class AttributionHealthResponse:
    ...
    # Distinct utm_source values among the top `INSIGHTSQL_GROUP_LIMIT` by event count.
    # When `utm_source_catalogue_truncated` is true this is a subtotal (capped at
    # INSIGHTSQL_GROUP_LIMIT), not the true distinct count.
    total_distinct_utm_sources: int = 0
    # True when the Datastore aggregation hit INSIGHTSQL_GROUP_LIMIT distinct
...
async def get_attribution_health(
    ...
    *,
    ...
    return AttributionHealthResponse(
        ...
        utm_source_catalogue_truncated=len(rows) >= INSIGHTSQL_GROUP_LIMIT,
    )
...
def _fetch_utm_groups(team: Team, *, lookback_days: int) -> list[_UtmRow]:
    """InsightsQL aggregation of utm_source counts and latest timestamp within the window.
    ...
    with tags_context(product=Product.MARKETING_ANALYTICS, feature=Feature.HEALTH_CHECK, team_id=team.pk):
        result = execute_insightsql_query(
            ...
            placeholders={
                ...
                "limit": ast.Constant(value=INSIGHTSQL_GROUP_LIMIT),
            },
        )
    rows: list[_UtmRow] = []
