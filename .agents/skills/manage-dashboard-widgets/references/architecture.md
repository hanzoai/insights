...
Dashboards already have **insight tiles** for InsightsQL/query visualizations — comparisons, formulas, breakdowns, subscriptions, and insight-linked alerts. A chart widget duplicates that stack in a smaller tile with worse ergonomics.
...
- Runtime: `getDashboardWidgetDefinition` → Insights `captureException` on miss (deploy skew)
...
Easy-to-miss paths when adding a type: `backend/widget_layouts.py` (batch-add placement), `insights/models/resource_transfer/visitors/dashboard_widget.py` (cross-project copy), `insights/api/test/test_sharing.py` (shared payload).
