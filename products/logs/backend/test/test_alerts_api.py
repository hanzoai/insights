...
from insights.cdp.templates.microsoft_teams.template_microsoft_teams import template as template_microsoft_teams
...
class TestLogsAlertAPI(APIBaseTest):
    ...
    def test_last_error_message_returns_most_recent_errored_check(self):
        ...
        LogsAlertEvent.objects.create(
            ...
            state_before="not_firing",
            state_after="errored",
            error_message="Latest Datastore timeout",
        )
        LogsAlertEvent.objects.create(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["last_error_message"] == "Latest Datastore timeout"
    ...
    def _sync_destination_templates(self) -> None:
        # Destination creation goes through the full InsightsFunctionSerializer pipeline,
        # which looks up a InsightsFunctionTemplate by template_id.
        sync_template_to_db(template_slack)
        sync_template_to_db(template_microsoft_teams)
        InsightsFunctionTemplate.objects.get_or_create(
    ...
    @patch("products.logs.backend.presentation.views.alerts_api.report_user_action")
    def test_create_slack_destination_creates_one_insights_function_per_event_kind(self, mock_report):
        ...
        assert response.status_code == status.HTTP_201_CREATED, response.json()
        ids = response.json()["insights_function_ids"]
        assert len(ids) == 4  # firing + resolved + broken + errored

        insights_functions = InsightsFunction.objects.filter(id__in=ids).order_by("name")
        event_ids = sorted([(hf.filters or {})["events"][0]["id"] for hf in insights_functions])
        assert event_ids == [
    ...
    @patch("products.alerts.backend.destinations.reload_insights_functions_on_workers")
    @patch("products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers")
    def test_create_webhook_destination_creates_one_insights_function_per_event_kind(
        self, signal_reload_insights_functions, alert_reload_insights_functions
    ):
        self._sync_destination_templates()
        created = self._create_via_api()
        with self.captureOnCommitCallbacks(execute=True):
            ...
            assert signal_reload_insights_functions.call_count == 4
            alert_reload_insights_functions.assert_not_called()

        assert response.status_code == status.HTTP_201_CREATED, response.json()
        ids = response.json()["insights_function_ids"]
        assert len(ids) == 4  # firing + resolved + broken + errored
        alert_reload_insights_functions.assert_called_once_with(team_id=self.team.id, insights_function_ids=sorted(ids))

        insights_functions = InsightsFunction.objects.filter(id__in=ids)
        for hf in insights_functions:
    ...
    def test_create_teams_destination_creates_one_insights_function_per_event_kind(self):
        self._sync_destination_templates()
        created = self._create_via_api()
    ...
    @patch("products.alerts.backend.destinations.reload_insights_functions_on_workers")
    def test_delete_destination_removes_insights_functions(self, reload_insights_functions):
        self._sync_destination_templates()
        created = self._create_via_api()
        ...
        ids = create_response.json()["insights_function_ids"]

        with self.captureOnCommitCallbacks(execute=True):
            delete_response = self.client.post(
                self._destinations_delete_url(created["id"]),
                {"insights_function_ids": ids},
                format="json",
            )
            reload_insights_functions.assert_not_called()

        assert delete_response.status_code == status.HTTP_204_NO_CONTENT
        assert InsightsFunction.objects.filter(id__in=ids, deleted=False).count() == 0
        assert InsightsFunction.objects.filter(id__in=ids, enabled=True).count() == 0
        reload_insights_functions.assert_called_once_with(team_id=self.team.id, insights_function_ids=sorted(ids))

    def test_delete_alert_soft_deletes_destination_insights_functions(self):
        self._sync_destination_templates()
        created = self._create_via_api()
        create_response = self.client.post(
            self._destinations_url(created["id"]),
    ...
    def test_delete_destination_rejects_foreign_insights_function_ids(self):
        self._sync_destination_templates()
        ...
        response = self.client.post(
            ...
            {"insights_function_ids": [a_ids[0], b_ids[0]]},
            format="json",
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json()["attr"] == "insights_function_ids"
    ...
    def test_delete_destination_rejects_more_ids_than_one_destination_group(self):
        ...
        response = self.client.post(
            self._destinations_delete_url(created["id"]),
            {"insights_function_ids": [str(uuid4()) for _ in range(len(LOGS_ALERT_EVENT_IDS) + 1)]},
            format="json",
        )
...
class TestSimulateEvaluatorParity(DatastoreTestMixin, APIBaseTest):
...
class TestSimulateEvaluatorLifecycleParity(DatastoreTestMixin, APIBaseTest):
...
class TestLogsAlertAPIPersonalAPIKeyScopes(APIBaseTest):
    ...
    def test_delete_destination_allowed_with_logs_write_scope(self):
        ...
        response = self.client.post(
            url,
            {"insights_function_ids": [str(uuid4())]},
            format="json",
            **self._auth(key),
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND, response.json()
