...
export const heatmapLogic = kea<heatmapLogicType>([
    ...
    listeners(({ actions, values, props }) => ({
                ...
                if (item.type === 'screenshot') {
                    ...
                    if (item.status === 'completed' && item.has_content) {
                        actions.setScreenshotUrl(
                            `/v1/environments/${window.INSIGHTS_APP_CONTEXT?.current_team?.id}/heatmap_screenshots/${item.id}/content/?width=${desiredWidth}`
                        )
        ...
        setWindowWidthOverride: async ({ widthOverride }) => {
            ...
            actions.setScreenshotError(null)
            actions.setScreenshotUrl(
                `/v1/environments/${window.INSIGHTS_APP_CONTEXT?.current_team?.id}/heatmap_screenshots/${values.heatmapId}/content/?width=${w}`
            )
        },
            ...
            while (attempts < maxAttempts) {
                ...
                try {
                    ...
                    if (contentResponse.success) {
                        ...
                        actions.setScreenshotUrl(
                            `/v1/environments/${window.INSIGHTS_APP_CONTEXT?.current_team?.id}/heatmap_screenshots/${id}/content/?width=${w}`
                        )
                        actions.loadHeatmap()
                        ...
                        if (screenshot.status === 'completed' && screenshot.has_content) {
                            const w = width ?? DEFAULT_HEATMAP_WIDTH
                            actions.setScreenshotUrl(
                                `/v1/environments/${window.INSIGHTS_APP_CONTEXT?.current_team?.id}/heatmap_screenshots/${screenshot.id}/content/?width=${w}`
                            )
                            actions.loadHeatmap()
                            actions.setGeneratingScreenshot(false)
                            break
        ...
        createHeatmap: async () => {
            ...
            try {
                ...
                toast.error(error.detail || 'Failed to create heatmap')
            } finally {
                actions.setLoading(false)
            }
        },
        updateHeatmap: async () => {
            ...
            try {
                ...
                toast.error(error.detail || 'Failed to update heatmap')
            } finally {
                actions.setLoading(false)
            }
        },
