...
// Render Select as a native <select> so each option's disabled state is assertable from the DOM
// without driving the (portal-rendered) popover. We only override Select; the other Lemon components stay real.
jest.mock('@hanzo/elements', () => ({
    ...jest.requireActual('@hanzo/elements'),
    Select: ({ options, value, onChange, placeholder }: any): JSX.Element => (
        <select
