...
def source_uses_delta_write_column_selection(source_type: str) -> bool:
    ...
    - managed-schema sources (Stripe/Paddle/Zendesk) — column selection is disabled for them
      (their canonical InsightsQL schema needs the full physical column set), so the pipeline must
    ...
    try:
        ...
        return False
    return not source.supports_column_selection and not source.has_managed_insightsql_schema
