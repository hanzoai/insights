...
from insights.datastore.table_engines import Buffer, Distributed, MergeTreeEngine, ReplicationScheme
...
def QUERY_LOG_ARCHIVE_CROSS_CLUSTER_DISTRIBUTED_TABLE_SQL(table_name=QUERY_LOG_ARCHIVE_WRITABLE_DISTRIBUTED_TABLE):
    ...
    return QUERY_LOG_ARCHIVE_NEW_TABLE_SQL(
        table_name=table_name,
        engine=Distributed(data_table="query_log_archive", cluster=settings.DATASTORE_SINGLE_SHARD_CLUSTER),
...
#
# log_comment is stored as a native Datastore JSON column holding a curated
...
# filter, insightsql_features) so they never reach the archive. The `query` and
...
def DISTRIBUTED_QUERY_LOG_ARCHIVE_OPS_TABLE_SQL(cluster=None):
    ...
    return QUERY_LOG_ARCHIVE_OPS_TABLE_SQL(
        ...
        engine=Distributed(
            data_table=SHARDED_QUERY_LOG_ARCHIVE_TABLE,
            cluster=cluster or settings.DATASTORE_OPS_CLUSTER,
        ),
        include_table_clauses=False,
    )
...
# (team, user, query-attribution) tuple. The data warehouse Datastore source
# (products/warehouse_sources/backend/temporal/data_imports/sources/datastore) syncs it into an internal
# project incrementally on `day`, so query-cost analysis can live in Insights
# instead of Metabase.
#
