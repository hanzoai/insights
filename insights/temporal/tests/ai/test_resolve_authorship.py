...
class TestResolveInsightsCodeAuthorship(TestCase):
    def setUp(self):
        cache.clear()
    ...
    def _call(self, repository: str = "insights/target-repo") -> str:
        return resolve_insights_code_authorship_activity(
            _make_inputs(self.integration.id), "C123", "1234.5678", "U_ALICE", self.user.id, "wf-123", repository
        )

    @patch("products.slack_app.backend.feature_flags.hanzo_insights.feature_enabled", return_value=True)
    @patch("insights.temporal.ai.slack_app.activities.messaging.SlackIntegration")
    def test_personal_github_with_repo_access_proceeds_and_posts_nothing(self, mock_slack_cls, _mock_flag):
        self._add_personal_github(repos=["insights/target-repo"])
    ...
    @parameterized.expand(
        [
            ("no_repo_access_flag_on", ["insights/other-repo"], None, True, "awaiting_confirmation"),
            ("no_repo_access_flag_off", ["insights/other-repo"], None, False, "blocked"),
            ("expired_refresh_token_flag_on", ["insights/target-repo"], 1, True, "awaiting_confirmation"),
        ]
    )
    @patch("insights.temporal.ai.slack_app.activities.messaging.SlackIntegration")
    def test_unusable_personal_github_never_proceeds(
        ...
        with patch("products.slack_app.backend.feature_flags.hanzo_insights.feature_enabled", return_value=flag_on):
            assert self._call() == expected_status

        kwargs = mock_slack.client.chat_postMessage.call_args.kwargs
        assert "can't author PRs in `insights/target-repo`" in kwargs["text"]

    @patch("products.slack_app.backend.feature_flags.hanzo_insights.feature_enabled", return_value=False)
    @patch("insights.temporal.ai.slack_app.activities.messaging.SlackIntegration")
