...
async def test_alias_products_ramp_through_canonical_fraction(product: str) -> None:
    # These requests must follow insights_code's per-product ramp end to end.
    handle = AsyncMock(return_value={"ok": True})
    settings = _settings(glm_modal_product_traffic_fractions={"insights_code": 1.0})
...
async def test_forwarded_flag_header_cannot_force_modal() -> None:
    # x-insights-flag-* headers come from any authenticated caller — they must not override the
    # server-side flag/fraction, or a client could pin itself to a backend operators turned off.
    handle = AsyncMock(return_value={"ok": True})
    set_request_context(RequestContext(request_id="test", insights_flags={"tasks-glm-modal-inference": "true"}))
