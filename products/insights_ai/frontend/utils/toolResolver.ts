...
/**
 * Resolves the registry key for a tool call. The single-exec `insights` MCP server exposes one
...
export function resolveToolKey(
    ...
    if (isInsightsExecTool && typeof input.command === 'string') {
        ...
        if (!verb) {
            return { resolvedKey: '__insights_exec_unknown__' }
        }

        if (verb !== 'call') {
            return { resolvedKey: `__insights_exec_${verb}__` }
        }
 ...
 * `renderPermissionPreview` needs to preview what an approval will do. For a Insights exec call this is
