...
A Rust implementation of the Script virtual machine, at behavioral parity with the Node reference VM
(`@hanzo/scriptvm`, `common/scriptvm/typescript`). Parity is enforced by `tests/parity.rs` (the full
...
- **`execute_resumable` / `resume`** — the suspendable-coroutine model for CDP script functions
...
[#67272](https://github.com/Insights/insights/issues/67272), surfaced in review of
[#66631](https://github.com/Insights/insights/pull/66631):
...
3. **Cumulative timeout not preserved across resume.** `drive_resumable` restarts the step counter
   ...
   resumes.
4. **Header-inclusive ip for module chunks.** A snapshot taken while suspended inside an STL script
