...
from insights.test.persons import add_cohort_members, create_person
...
class TestRemoveUserByUuid(BaseTest):
    ...
    @patch("products.cohorts.backend.models.util.remove_person_from_static_cohort")
    @patch("products.cohorts.backend.models.util.get_static_cohort_size", return_value=0)
    def test_personinsights_resolves_person_for_removal(self, mock_get_size, mock_remove_ch):
        person = create_person(team=self.team, distinct_ids=["d1"])
        cohort = self._create_static_cohort()
...
class TestCheckCohortMembership(BaseTest):
    ...
    def test_isolates_cross_team_cohort(self):
        ...
        of which path (personinsights vs ORM) runs. Tenant scoping happens in the
...
class TestDeleteCohortMembersBulkMaxIterations(BaseTest):
    def test_stops_after_max_iterations(self):
        ...
        from insights.personinsights_client.proto import DeleteCohortMembersBulkResponse
        ...
        with (
            ...
            patch("insights.personinsights_client.client.get_personinsights_client", return_value=mock_client),
        ):
            total = cohort_util._delete_cohort_members_bulk_via_personinsights([1], batch_size=100)
...
class TestInsertUsersListWithBatchingPerson(BaseTest):
    ...
    def test_insert_capacity_error_surfaces_only_with_raise_on_error(self, raise_on_error, mock_insert_ch):
        # A transient Datastore capacity error during static-cohort population must propagate when
