...
All events are sent via `toolbarInsightsJS`, the toolbar's internal Insights instance,
unless noted otherwise. Events go to Insights's internal project (prod) or `localhost:8000` (dev).
...
Fired after the CORS reachability check to the Insights app.
...
## Heatmap sampling (page's Insights instance)

These events use the **page's** `insights.capture()`, not `toolbarInsightsJS`.
They are sent to the customer's project, not Insights's internal project.
