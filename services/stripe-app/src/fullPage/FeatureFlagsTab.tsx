...
const FeatureFlagsTab = ({ client, projectId }: Props): JSX.Element => {
    const [flags, setFlags] = useState<InsightsFeatureFlag[]>([])
    ...
    if (!client || !projectId) {
        return (
            <Banner
                ...
                description="A Insights project_id isn't stored yet — reconnect to load feature flags."
            />
        )
    }
    ...
    if (!loading && flags.length === 0) {
        return (
            <Banner type="default" title="No feature flags yet" description="Create one in Insights to see it here." />
        )
    }

    const insightsBase = `${client.baseUrl}/project/${projectId}`
    const needle = filter.trim().toLowerCase()
    const visibleFlags = needle
        ? flags.filter(
              (f: InsightsFeatureFlag) =>
                  f.key.toLowerCase().includes(needle) || (f.name ?? '').toLowerCase().includes(needle)
          )
        : flags

    const linkMap: Record<string, string> = {}
    const items: DataTableItem[] = visibleFlags.map((f: InsightsFeatureFlag) => {
        const url = `${insightsBase}/feature_flags/${f.id}`
        linkMap[url] = f.key
        return {
            id: `flag:${f.id}`,
            key: url,
        ...
        <>
            ...
            <Box css={{ width: 'fill', stack: 'y', rowGap: 'medium' }}>
                ...
                <Box css={{ paddingX: 'medium' }}>
                    <ExternalLink href={`${insightsBase}/feature_flags`}>View in Insights</ExternalLink>
                </Box>
            </Box>
        </>
    )
...
function formatGroups(groups: InsightsFlagGroup[]): string {
    if (groups.length === 0) {
        return '—'
    }
    return groups
        .map((g: InsightsFlagGroup, i: number) => {
...
function formatVariants(variants: InsightsFlagVariant[]): string {
    if (variants.length === 0) {
        return '—'
    }
    return variants.map((v: InsightsFlagVariant) => `${v.key} · ${v.rollout_percentage}%`).join('\n')
}
