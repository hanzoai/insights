 ...
 *   - `core`     — a built-in Insights definition (`isCoreFilter`). Badge
 *                  reads "By Insights" with the logomark.
 *   - `verified` — a custom definition a team explicitly marked verified
 *                  (`item.verified`). Badge reads "Verified".
 */
import { IconBadge, IconLogomark } from '@hanzo/icons'
import { Badge, cn, Tooltip, TooltipContent, TooltipTrigger } from '@hanzo/quill'
...
export function getVerification(entry: MenuFilterEntry): Verification | null {
    // Core Insights definitions ($pageview, $browser, …) are inherently
    // verified — surfaced as "By Insights" rather than a team flag.
