...
description: >
  Guides exploration of $autocapture events captured by insights-js to understand user interactions,
  ...
  using insights-js autocapture.
---

# Exploring autocapture events

if users opt in then insights-js automatically captures clicks, form submissions, and page changes as `$autocapture` events.
Each event records the clicked DOM element and its ancestors in the `elements_chain` column.

`$autocapture` is intentionally excluded from the `insights:read-data-schema` taxonomy
...
Every `$autocapture` event from insights-js ships with a fixed set of properties.
...
- **Project settings** — the team can set `autocapture_opt_out` in Insights project settings
- **SDK config** — the insights-js `init()` call can pass `autocapture: false`
...
When the user wants a funnel, trend, or other insight, the filter shape is different from InsightsQL.
...
Decision rule: prefer an action (`ActionsNode` referencing an existing action — see Step 8) when the interaction will be referenced more than once; inline `type: "element"` / `type: "event"` filters when it's a one-off insight; raw InsightsQL (Step 7) when joining across events or doing custom aggregations.

### 7. Use in ad-hoc queries

The discovered selector can be used directly in InsightsQL without creating an action.
...
For recurring analysis, prefer creating an action (next step) or using `insights:query-trends` / `insights:query-funnel` with the action.

### 8. Create an action

Actions are the durable version of ad-hoc selector queries.
Once the criteria uniquely identify the interaction, create an action using `insights:action-create`.
...
- This workflow only applies to insights-js — other SDKs do not capture elements
