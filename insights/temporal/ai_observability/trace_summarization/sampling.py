...
# Filter `type` values that resolve to cohort membership lookups in InsightsQL.
...
def _missing_cohort_ids(team: Team, event_filters: list[dict[str, Any]]) -> list[int]:
    ...
    Mirrors the lookup the InsightsQL cohort resolver does (`team__project_id`,
    ...
    def _sample_items(
        ...
        if analysis_level == "generation":
            ...
            generations_query = parse_select(
                ...
                GROUP BY trace_id
                -- argMaxIf returns the zero UUID (not NULL) with no matches; toUUIDOrZero isn't in the InsightsQL allowlist.
