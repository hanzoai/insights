...
class ExposedCHQueryError(InternalCHQueryError):
    """User-safe Datastore query error. Subclasses have user_safe=True in ErrorCodeMeta,
...
def wrap_datastore_query_error(err: Exception) -> Exception:
    ...
    if name in ("TOO_MANY_SIMULTANEOUS_QUERIES", "CANNOT_SCHEDULE_TASK"):
        ...
        memory_error = DatastoreQueryMemoryLimitExceeded()
        # Match the known per-query phrasings ("Memory limit (for query) exceeded" before
        # Datastore 26, "Query memory limit exceeded" since). Anything else - "(total)",
        ...
        # Datastore surfaces this as a raw STD_EXCEPTION (code 1001), so translate it
...
def classify_query_error(e: Exception) -> QueryErrorCategory:
    ...
    if isinstance(e, ServerException):
        return look_up_datastore_error_code_meta(e).get_category()

    if isinstance(e, (DatastoreAtCapacity, ConcurrencyLimitExceeded)):
...
class CHQueryErrorS3FileChangedDuringRead(ExposedCHQueryError):
    """A file backing a warehouse table was overwritten or deleted while Datastore was reading it."""
...
#
# From https://github.com/Datastore/Datastore/blob/v25.8.12.129-lts/src/Common/ErrorCodes.cpp#L17-L650
#
...
DATASTORE_ERROR_CODE_LOOKUP: dict[int, ErrorCodeMeta] = {
    ...
    241: ErrorCodeMeta(
        # Code 241 short-circuits to DatastoreQueryMemoryLimitExceeded in wrap_datastore_query_error,
...
# Transient Datastore infrastructure errors that are safe to retry.
# This can be used in things like celery `autoretry_for` to increase resiliency.
# Capacity errors (codes 202/439) are wrapped as DatastoreAtCapacity by wrap_datastore_query_error.
CH_TRANSIENT_ERRORS = (
    CHQueryErrorS3Error,
