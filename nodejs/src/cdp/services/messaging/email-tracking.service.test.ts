...
describe('EmailTrackingService', () => {
    ...
    describe('api', () => {
        ...
        describe('SES webhook log entries', () => {
            ...
            it('records the metric but writes no log entry when the bounce resolves to a insights_function', async () => {
                const res = await postBounce({ functionId: insightsFunction.id })
                expect(res.status).toBe(200)
    ...
    describe('resolveEmailEngagementDistinctId', () => {
        const buildInvocation = (
            globals: Partial<NonNullable<CyclotronJobInvocationInsightsFunction['state']['globals']>>
