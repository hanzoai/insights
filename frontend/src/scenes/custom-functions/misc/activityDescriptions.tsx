...
const nameOrLinkToCustomFunction = (id?: string | null, name?: string | null): string | JSX.Element => {
    const displayName = name?.trim() ? name : 'Untitled custom function'
    return id ? <Link to={urls.customFunction(id)}>{displayName}</Link> : displayName
}
...
export function customFunctionActivityDescriber(logItem: ActivityLogItem, asNotification?: boolean): HumanizedChange {
    if (logItem.scope != 'CustomFunction') {
        console.error('CustomFunction describer received a non-CustomFunction activity')
        return { description: null }
    }

    const rawType = logItem?.detail.type as CustomFunctionTypeType | undefined
    const objectNoun = rawType ? humanizeCustomFunctionType(rawType) : 'custom function'
        ...
        return {
            description: (
                <>
                    ...
                    {nameOrLinkToCustomFunction(logItem?.item_id, logItem?.detail.name)}
                </>
            ),
        }
    }
    ...
    if (logItem.activity == 'restored') {
        ...
        const functionName = nameOrLinkToCustomFunction(logItem?.item_id, logItem?.detail.name)
    ...
    return defaultDescriber(logItem, asNotification, nameOrLinkToCustomFunction(logItem?.item_id, logItem?.detail.name))
}
