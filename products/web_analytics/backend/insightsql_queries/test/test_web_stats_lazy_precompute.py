...
from products.web_analytics.backend.insightsql_queries.web_analytics_query_runner import SESSION_ID_SET_FEATURE_FLAG_KEY
from products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common import ORG_FEATURE_FLAG_KEY
from products.web_analytics.backend.insightsql_queries.web_stats_lazy_precompute import _breakdown_having_expr
...
class TestWebStatsLazyPrecompute(DatastoreTestMixin, APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
        ...
        # precompute rows are "born expired" relative to the real Datastore server
    ...
    def _enable_lazy(self):
        ...
        # the default `hanzo_insights.feature_enabled` returns False (no API key in
        # tests), which models a flag-disabled org. The patch target is the shared
        # `hanzo_insights` module, so an unscoped True would enable other rollouts too.
        return patch(
    ...
    def test_views_desc_orderby_lazy_matches_raw(self):
        # Forwarding the user's orderBy through to Datastore: sorting by views
    ...
    def test_stale_served_enqueues_background_revalidation(self):
        ...
        from products.web_analytics.backend.insightsql_queries.web_stats_lazy_precompute import execute_lazy_precomputed_read

        with (
            tags_context(),
            self._enable_lazy(),
            patch(
                "products.web_analytics.backend.insightsql_queries.web_stats_lazy_precompute.ensure_web_stats_precomputed",
