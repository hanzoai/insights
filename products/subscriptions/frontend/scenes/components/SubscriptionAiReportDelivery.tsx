...
import { Collapse, Tag, Tooltip } from '@hanzo/elements'
 ...
 * explain it (a message is present, i.e. a resolution/exposed InsightsQL error); a generic/internal exception
...
function queryStatusTag(d: AIReportQueryDiagnosticApi): JSX.Element {
    return <Tag type={d.ok === false ? 'danger' : 'success'}>{queryStatusLabel(d)}</Tag>
}
...
function recipientStatusTag(status: DeliveryRecipientStatus): JSX.Element {
    ...
    return <Tag type="warning">Partial</Tag>
}
...
function insightItemTag(item: DeliveryInsightItem): JSX.Element {
    ...
    if (item.pending) {
        return <Tag type="default">Did not run</Tag>
    }
    return <Tag type="success">Computed</Tag>
}
            ...
            {report ? (
                <div className="flex flex-col gap-1">
                    ...
                    <div className="max-h-96 overflow-auto rounded border bg-bg-light p-3">
                        ...
                        <Markdown disableImages>{report}</Markdown>
                    </div>
                </div>
            ) : null}
            {recipients.length > 0 ? (
