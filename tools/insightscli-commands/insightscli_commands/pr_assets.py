"""Shared client for the public Insights/pr-assets evidence repo.
...
from insightscli_commands.github_auth import github_headers

REPO: Final = "Insights/pr-assets"
PUBLIC_WARNING: Final = (
    "⚠  PUBLIC + PERMANENT upload to Insights/pr-assets.\n"
...
def _denied_message(path: Path) -> str:
    ...
    return (
        f"upload of {path.name} was denied. Writing to {REPO} needs write access to a public Insights repo: "
        "confirm your token is a Insights org account with the `repo` scope "
