...
description: >
  Investigates a single Insights error tracking issue end-to-end. Use when
...
| `insights:query-logs`                        | OTEL log entries around the error timestamp for server-side issues                          |
| `insights:query-session-recordings-list`     | Linked replays (delegate ranking to `finding-replay-for-issue`)                             |
| `insights:read-data-schema`                  | Confirm property keys before filtering on them                                              |
...
```json
insights:query-error-tracking-issue
{
  "issueId": "<issue_id>",
...
asking "did upgrading Insights cause this?".
...
JSON-encoded column in Datastore, so `arrayJoin(properties.$active_feature_flags)`
directly will fail. `JSONExtract` is the working pattern, but you must coerce the
argument to a non-nullable String first: Datastore refuses to return a nested
...
The `left(event, 1) != '$'` clause drops Insights autocapture / system events
...
propagates `$session_id` from the client request (Insights's own backend does
...
- For a "what version introduced this?" breakdown, prefer `$app_version` (the
  ...
  `$lib_version` for this question — it's the Insights SDK library version, not
