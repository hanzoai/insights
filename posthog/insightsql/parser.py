...
from posthog.insightsql.ast import SelectSetNode
from posthog.insightsql.base import AST
from posthog.insightsql.constants import RESERVED_KEYWORDS, InsightsQLParserBackend
from posthog.insightsql.errors import BaseInsightsQLError, NotImplementedError, SyntaxError
from posthog.insightsql.grammar.InsightsQLLexer import InsightsQLLexer
from posthog.insightsql.grammar.InsightsQLParser import InsightsQLParser
from posthog.insightsql.json_ast import deserialize_ast
from posthog.insightsql.parse_string import parse_string_literal_ctx, parse_string_literal_text, parse_string_text_ctx
from posthog.insightsql.placeholders import replace_placeholders
...
RULE_TO_PARSE_FUNCTION: dict[
    ...
    "python": {
        "expr": safe_lambda(
            lambda string, start: InsightsQLParseTreeConverter(start=start).visit(get_parser(string).expr())
        ),
        "order_expr": safe_lambda(lambda string: InsightsQLParseTreeConverter().visit(get_parser(string).orderExpr())),
        "select": safe_lambda(lambda string: InsightsQLParseTreeConverter().visit(get_parser(string).select())),
        "full_template_string": safe_lambda(
            lambda string: InsightsQLParseTreeConverter().visit(get_parser(string).fullTemplateString())
        ),
        "program": safe_lambda(lambda string: InsightsQLParseTreeConverter().visit(get_parser(string).program())),
    },
    "cpp-json": {
...
def parse_string_template(
    ...
    *,
    backend: InsightsQLParserBackend = DEFAULT_BACKEND,
) -> ast.Call:
...
def get_parser(query: str) -> InsightsQLParser:
    input_stream = InputStream(data=query)
    lexer = InsightsQLLexer(input_stream)
    stream = CommonTokenStream(lexer)
    parser = InsightsQLParser(stream)
    parser.removeErrorListeners()
    parser.addErrorListener(InsightsQLErrorListener(query))
    return parser


class InsightsQLErrorListener(ErrorListener):
    query: str
...
class InsightsQLParseTreeConverter(ParseTreeVisitor):
    ...
    def visitSelect(self, ctx: InsightsQLParser.SelectContext):
        return self.visit(ctx.selectSetStmt() or ctx.selectStmt() or ctx.insightsqlxTagElement())
    ...
    def visitLimitAndOffsetClauseClause(self, ctx: InsightsQLParser.LimitAndOffsetClauseContext):
    ...
    def visitColumnExprTagElement(self, ctx: InsightsQLParser.ColumnExprTagElementContext):
        return self.visit(ctx.insightsqlxTagElement())
    ...
    def visitInsightsqlxChildElement(self, ctx: InsightsQLParser.InsightsqlxChildElementContext):
        if ctx.insightsqlxTagElement():
            return self.visit(ctx.insightsqlxTagElement())
        if ctx.insightsqlxText():
            return self.visit(ctx.insightsqlxText())
        return self.visit(ctx.columnExpr())

    def visitInsightsqlxText(self, ctx: InsightsQLParser.InsightsqlxTextContext):
        return ast.Constant(value=ctx.INSIGHTSQLX_TEXT_TEXT().getText())
    ...
    def visitInsightsqlxTagElementClosed(self, ctx: InsightsQLParser.InsightsqlxTagElementClosedContext):
        ...
        attributes = [self.visit(a) for a in ctx.insightsqlxTagAttribute()] if ctx.insightsqlxTagAttribute() else []
        return ast.InsightsQLXTag(kind=kind, attributes=attributes)
    ...
    def visitInsightsqlxTagElementNested(self, ctx: InsightsQLParser.InsightsqlxTagElementNestedContext):
        ...
        if opening != closing:
            raise SyntaxError(f"Opening and closing InsightsQLX tags must match. Got {opening} and {closing}")

        attributes = [self.visit(a) for a in ctx.insightsqlxTagAttribute()] if ctx.insightsqlxTagAttribute() else []

        # ── collect child nodes, discarding pure-indentation whitespace ──
        kept_children = []
        for element in ctx.insightsqlxChildElement():
        ...
        if kept_children:
            if any(a.name == "children" for a in attributes):
                raise SyntaxError("Can't have a InsightsQLX tag with both children and a 'children' attribute")
            attributes.append(ast.InsightsQLXAttribute(name="children", value=kept_children))

        return ast.InsightsQLXTag(kind=opening, attributes=attributes)

    def visitInsightsqlxTagAttribute(self, ctx: InsightsQLParser.InsightsqlxTagAttributeContext):
        name = self.visit(ctx.identifier())
        if ctx.columnExpr():
            return ast.InsightsQLXAttribute(name=name, value=self.visit(ctx.columnExpr()))
        elif ctx.string():
            return ast.InsightsQLXAttribute(name=name, value=self.visit(ctx.string()))
        else:
            return ast.InsightsQLXAttribute(name=name, value=ast.Constant(value=True))
