...
token, user token, PAT, or Insights's shared token) and the ``installation_id`` as ``scope`` when known.

This module lives in ``egress`` (not the model layer) and imports nothing from ``insights.models`` — the
...
from insights.egress.github.limiter import classify_github_resource, consume_github_installation_sync
from insights.egress.github.observability import record_github_api_exception, record_github_api_response
from insights.egress.limiter.policies import Priority
from insights.egress.transport.transport import EgressBudgetExhausted, EgressClient

# The GitHub REST API version we pin every request to. Lives here (not integration.py) so the egress
# layer stays free of any insights.models import; integration.py imports it back from here.
...
def github_request(
    ...
    **kwargs: Any,
    ...
    ``None`` for identity-blind callers (raw PATs, Insights's public token), which record volume only.
