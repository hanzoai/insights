"""Auto-detecting test runner for insightscli.
...
from insightscli_commands.change_detection import changed_files

_PYTHON_ROOTS = ("insights/", "ee/", "products/", "common/", "dags/", "tools/", "services/")
...
def _is_test_file(path: str, rs_cfg_test: set[str] | None = None) -> bool:
    ...
    if path.endswith(".py"):
        return name.startswith("test_") or (name.startswith("eval_") and path.startswith("ee/scriptai/eval/"))
...
def _detect_jest_test(file_only: str, file_path: str, node_id: str | None = None) -> TestRunConfig:
    ...
    Tests under products/ are collected by the root @hanzo/frontend jest config (its
...
def detect_test_type(file_path: str) -> TestRunConfig:
    ...
    if file_only.startswith("ee/scriptai/eval/") and ext == ".py":
        return TestRunConfig(
            test_type="python-eval",
            command=["pytest", "-c", "ee/scriptai/eval/pytest.ini", "-s", file_path],
    ...
    for test_type, entries in groups.items():
        if test_type in ("python", "python-eval"):
            ...
            for f, cfg in entries:
                pkg = cfg.command[1]  # e.g. "--filter=@hanzo/frontend"
...
def _run_watch(file_path: str, extra_args: list[str]) -> None:
    ...
    if config.test_type in ("python", "python-eval"):
        ...
        watch_dirs = ["./insights", "./common/scriptvm/python", "./ee", "./dags", "./products"]
...
@click.command(
    ...
    help=(
        ...
        "Examples:\n\n"
        "\b\n"
        "  insightscli test insights/api/test/test_user.py                          # single Python file\n"
        "  insightscli test insights/api/test/test_user.py::TestUser::test_create   # single Python test\n"
        "  insightscli test insights/api/test/test_user.py --watch                  # re-run on changes\n"
        "  insightscli test frontend/src/lib/utils.test.ts                         # single Jest file\n"
        '  insightscli test "frontend/src/lib/utils.test.ts::my test name"         # Jest by name pattern\n'
        "  insightscli test playwright/e2e/sql-editor.spec.ts                      # Playwright spec\n"
        "  insightscli test cli/src/utils/throttler.rs                             # single Rust module\n"
        "  insightscli test cli/src/utils/throttler.rs::test_create                # single Rust test\n"
        "  insightscli test --changed                                              # tests for branch changes\n"
        "  insightscli test products/alerts/                                       # all tests in a product"
    ),
