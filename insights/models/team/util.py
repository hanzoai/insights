...
from insights.models.async_migration import is_async_migration_complete
...
# Batch size for the personinsights batch-delete RPCs on the largest team-scoped tables
...
# start_to_close (HEAVY_ACTIVITY_TIMEOUT, 2h in insights/temporal/delete_teams/workflows.py)
...
# (which would let a retry start while the previous DELETE is still running in personinsights).
...
def delete_bulky_postgres_data(team_ids: list[int]):
    ...
    # Delete Person + PersonDistinctId via personinsights RPC (handles both tables).
...
def _delete_hash_key_overrides_for_teams(team_ids: list[int]) -> None:
    """Delete FeatureFlagHashKeyOverride rows for the given teams via personinsights.
    ...
    from insights.personinsights_client.proto import DeleteHashKeyOverridesByTeamsRequest
...
def _delete_personless_distinct_ids_for_teams(team_ids: list[int]) -> None:
    """Delete insights_personlessdistinctid rows for teams via personinsights RPC."""
    ...
    for team_id in team_ids:
        personinsights_call(
            ...
            partial(_delete_personless_distinct_ids_for_team_via_personinsights, team_id),
        )


def _delete_personless_distinct_ids_for_team_via_personinsights(team_id: int) -> None:
    from insights.personinsights_client.client import require_personinsights_client
    from insights.personinsights_client.proto import DeletePersonlessDistinctIdsBatchForTeamRequest
...
def _delete_persons_for_teams(team_ids: list[int]) -> None:
    """Delete Person + PersonDistinctId rows for teams via personinsights RPC.
    ...
    for team_id in team_ids:
        personinsights_call(
            ...
            partial(_delete_persons_for_team_via_personinsights, team_id),
        )


def _delete_persons_for_team_via_personinsights(team_id: int) -> None:
    from insights.personinsights_client.client import require_personinsights_client
    from insights.personinsights_client.proto import DeletePersonsBatchForTeamRequest
...
def _delete_groups_for_teams(team_ids: list[int]) -> None:
    from insights.personinsights_client.client import personinsights_call, require_personinsights_client
    from insights.personinsights_client.proto import DeleteGroupsBatchForTeamRequest
    ...
    for team_id in team_ids:
        ...
        personinsights_call("delete_groups_for_team", _fn)


def _delete_group_type_mappings_for_teams(team_ids: list[int]) -> None:
    from insights.personinsights_client.client import personinsights_call, require_personinsights_client
    from insights.personinsights_client.proto import DeleteGroupTypeMappingsBatchForTeamRequest
    ...
    for team_id in team_ids:
        ...
        personinsights_call("delete_group_type_mappings_for_team", _fn)


def _delete_cohort_members_for_teams(team_ids: list[int], cohort_ids: list[int]) -> None:
    """Delete CohortPeople rows for teams via personinsights RPC.
...
def delete_data_modeling_schedules(team_ids: list[int]) -> None:
    ...
    from insights.temporal.common.schedule import delete_schedule
