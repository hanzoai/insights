 ...
 *   - the GroupNamesPrefix datastore fast path (still goes through generic
...
export function useGroupList(input: UseGroupListInput): UseGroupListResult {
    ...
    const fuse = useMemo(() => {
        ...
        const haystack = filteredLocalItems.map((item) => {
            ...
            const insightsName = getCoreFilterDefinition(name, group.type)?.label
            const recentLabel =
                hasRecentContext(item) && item._recentContext.propertyFilter
                    ? formatPropertyLabel(item._recentContext.propertyFilter, {})
                    : undefined
            return { name, insightsName, recentLabel, item }
        })
        return createFuse(haystack, { keys: ['name', 'insightsName', 'recentLabel'], ignoreLocation: true })
    ...
    const remoteFuse = useMemo(() => {
        ...
        const haystack = remoteItemsRaw.results.map((item) => {
            const name = group.getName?.(item) ?? ('name' in item ? (item as { name?: string }).name : '') ?? ''
            const insightsName = getCoreFilterDefinition(name, group.type)?.label
            return { name, insightsName, item }
        })
        return createFuse(haystack, { keys: ['name', 'insightsName'], ignoreLocation: true })
