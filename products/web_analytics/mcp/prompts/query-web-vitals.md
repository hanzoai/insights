...
Requires the project to capture the `$web_vitals` event (`capture_performance` in insights-js). If results come back empty, check capture with `read-data-schema` before concluding pages are fine.
