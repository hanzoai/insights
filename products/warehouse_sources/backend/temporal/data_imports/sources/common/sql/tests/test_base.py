...
from insights.schema import SourceConfig
...
class TestReconcileSourceSchemaMetadata(BaseTest):
    """The driver-agnostic reconcile helper that `SQLSource` and `DatastoreSource` share."""

    def _source(self) -> ExternalDataSource:
        return ExternalDataSource.objects.create(team=self.team, source_type=ExternalDataSourceType.DATASTORE)
