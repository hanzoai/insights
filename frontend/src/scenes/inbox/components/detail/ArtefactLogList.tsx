...
import { Task } from 'products/insights_ai/frontend/types/taskTypes'
...
function ActivityBodyCard({ children }: { children: React.ReactNode }): JSX.Element {
    return (
        <Card hoverEffect={false} className="w-full p-2 shadow-none">
            {children}
        </Card>
    )
}
...
function NoteBody({ note, author }: { note: string; author?: string }): JSX.Element {
    return (
        <div className="w-full">
            <Markdown className="text-xs text-secondary leading-normal" disableImages>
                {note}
            </Markdown>
...
function ContentChangeBody({
    ...
    const renderText = (text: string, muted: boolean): JSX.Element => {
        ...
        return markdown ? (
            <Markdown className={`text-xs leading-normal ${color}`} disableImages>
                {text}
            </Markdown>
        ) : (
...
const CODE_REVIEW_OUTCOME: Record<NonNullable<CodeReviewContent['outcome']>, { label: string; type: TagType }> = {
...
function renderArtefactSummary(artefact: SignalReportArtefact): JSX.Element | null {
    ...
    switch (artefact.type) {
        case 'priority_judgment': {
            ...
            return priority ? (
                <Tag size="small" type={PRIORITY_TAG_TYPE[priority] ?? 'muted'}>
                    {priority}
                </Tag>
            ) : null
        }
        ...
        case 'safety_judgment': {
            ...
            return (
                <Tag size="small" type={choice ? 'success' : 'danger'}>
