...
from insights.schema import InsightsQLAlertConfig
...
from products.alerts.backend.evaluation.insightsql import (
    LAST_ROW_MAX_ROWS,
    InsightsQLDetectorExtractor,
    extract_insightsql_detector_series,
)
...
def _alert(rows_config: dict | None = None, detector_config: dict | None = ZSCORE) -> MagicMock:
    alert = MagicMock()
    alert.config = {"type": "InsightsQLAlertConfig", "evaluation": "last_row", **(rows_config or {})}
...
def _extract(values, *, columns=None, rows_config=None, detector_config=ZSCORE):
    ...
    with patch(CALC_PATH) as calc:
        ...
        return InsightsQLDetectorExtractor().extract(
...
def test_last_row_truncation_guard_rejects_a_capped_result():
    # last_row scores the tail; a result at InsightsQL's hard cap may be truncated, so the detector path
...
def test_extract_insightsql_detector_series_is_alert_less():
    # The simulation reuses the alert-less builder directly (no AlertConfiguration).
    config = InsightsQLAlertConfig(type="InsightsQLAlertConfig", evaluation="last_row")
    with patch(CALC_PATH) as calc:
