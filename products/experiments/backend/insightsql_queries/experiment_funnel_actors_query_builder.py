...
from insights.schema import ActionsNode, ExperimentEventExposureConfig, ExperimentFunnelMetric, MultipleVariantHandling
...
from products.experiments.backend.insightsql_queries.base_query_utils import funnel_steps_to_filter
...
class ExperimentFunnelActorsQueryBuilder:
    ...
    def _build_entity_metrics_cte(self) -> ast.SelectQuery:
        ...
        if self.metric.conversion_window is not None and self.metric.conversion_window_unit is not None:
            from products.experiments.backend.insightsql_queries.base_query_utils import conversion_window_to_seconds
        ...
        if self.include_recordings:
            ...
            from products.experiments.backend.insightsql_queries.base_query_utils import funnel_evaluation_expr
        ...
        # covered by semgrep exception (see .semgrep/rules/security/insightsql-no-fstring.yaml line 114)
        query = cast(
            ast.SelectQuery,
            parse_select(
                f"""
