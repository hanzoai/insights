...
- ``logic.strategies``: InsightsQL vs insight query behavior
...
from insights.api.mixins import PydanticModelMixin, ValidatedRequest, validated_request
...
class EndpointViewSet(
    ...
    @validated_request(
        ...
        description=(
            ...
            "materialized. Only applicable to SQL (InsightsQL) endpoints that currently fail the "
    ...
    def materialization_suggestion(self, request: ValidatedRequest, name=None, *args, **kwargs) -> Response:
        ...
        if (version.query or {}).get("kind") != "InsightsQLQuery":
            return Response(
                status=status.HTTP_400_BAD_REQUEST,
