...
describe('PostgresPersonRepository', () => {
    ...
    describe('tombstoned rows', () => {
        async function tombstonePerson(person: InternalPerson): Promise<void> {
            await postgres.query(
                PostgresUse.PERSONS_WRITE,
                'UPDATE insights_person SET is_deleted = true, version = version + 1 WHERE team_id = $1 AND id = $2',
        ...
        async function tombstoneDistinctId(teamId: number, distinctId: string): Promise<void> {
            await postgres.query(
                PostgresUse.PERSONS_WRITE,
                'UPDATE insights_persondistinctid SET is_deleted = true, version = COALESCE(version, 0) + 1 WHERE team_id = $1 AND distinct_id = $2',
                [teamId, distinctId],
                'tombstoneDistinctId'
            )
        }
    ...
    describe('person properties size violation handling', () => {
        ...
        describe('new metrics and error handling', () => {
            it('should increment personPropertiesSizeViolationCounter with correct labels', async () => {
                ...
                const metrics = require('~/common/persons/metrics')
    ...
    describe('JSON field size metrics', () => {
        ...
        beforeEach(() => {
            ...
            const metricsModule = require('~/common/persons/metrics')
