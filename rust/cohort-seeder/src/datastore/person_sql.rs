//! Datastore person-scan SQL: the byte-frozen renderers for the boundary and range scans over the
...
//! version)`) with `team_id` equality-fixed. Range predicates are defined in Datastore's UUID
//! order. Every column reference is table-qualified because the output alias is itself named `id`:
//! Datastore binds a bare `id` in `WHERE`, `GROUP BY`, or `ORDER BY` to that alias rather than to
...
mod tests {
    ...
    /// SQL the renderer emits, including SQL no Datastore can execute. This one states the property
