...
- **Config.** Each scout has one `SignalScoutConfig` per `(project, skill_name)` carrying `run_interval_minutes` (default 1440), `enabled`, `emit`, `network_access` (`trusted` default, `full` for scouts that read arbitrary external sites), and a `last_run_at` stamp.
  ...
  Prepare a fresh per-team scout and its config together with `insights:scout-create-prepare`; the nested `config` object sets its schedule, emit posture, and destinations before it can run.
  ...
  The lower-level `insights:scout-config-create` remains available when a skill already exists without a config.
...
All of these via `insights:scout-config-update` (get the `id` from `-config-list`), or set at creation time in the nested `config` object passed to `insights:scout-create-prepare`.
...
insights:skill-list {"search": "signals-scout"}

# Read a canonical scout to use as a template
insights:skill-get {"skill_name": "signals-scout-error-tracking"}

# New scout from scratch: prepare the complete definition and config.
insights:scout-create-prepare {"name": "signals-scout-<scope>", "description": "...", "body": "...", "config": {"run_interval_minutes": 120}}

# Show the returned message and wait for the user to type `confirm`, then execute.
insights:scout-create-execute {"confirmation_hash": "<returned-hash>", "confirmation": "confirm"}

# Adapt an existing per-team scout — use the SMALLEST primitive (find/replace, not full-body)
insights:skill-get {"skill_name": "signals-scout-<scope>"}          # get current version first
insights:skill-update {"skill_name": "signals-scout-<scope>", "base_version": N, "edits": [{"old": "...", "new": "..."}]}

# Duplicate a canonical scout into a new per-team scout you then edit (keeps the canonical intact)
insights:skill-duplicate {"skill_name": "signals-scout-general", "new_name": "signals-scout-<scope>"}

# Bundle a reference file onto a per-team scout
insights:skill-file-create {"skill_name": "signals-scout-<scope>", "path": "references/cookbook.md", "content": "...", "content_type": "text/markdown", "base_version": N}
```
...
## Path B — canonical (in-repo, for Insights contributors)
...
insightscli lint:skills           # validate frontmatter / syntax / binaries — fast, no Django
insightscli build:skills          # render + package into dist/skills.zip
insightscli sync:skill -- --name signals-scout-<scope>   # build + sync to .agents/skills/ for local agent testing
insightscli unsync:skill -- --name signals-scout-<scope>
```
...
`insights:scout-run-now {"id": <config_id>}` dispatches one run of the scout immediately, regardless of its schedule (get the `id` from `-config-list`) — the **initial real run**, the scout executing end-to-end in the harness.
...
2. Create the scout and its config together via `insights:scout-create-prepare` → `-execute` (the default `emit=true` goes in the nested `config`), leaving `run_interval_minutes` at a sustainable value — no short-interval trick needed.
   Then spend one `-run-now` to watch the whole scout execute end-to-end, and inspect once it finishes:
   - `insights:inbox-reports-list` — the reports it actually wrote.
   - `insights:scout-runs-list` — run summaries.
   - `insights:scout-runs-retrieve` — the full reasoning for one run.
   - `insights:scout-scratchpad-search` — the durable memory it wrote.
...
Repo contributors additionally get `insightscli sync:skill` to run the scout against the local harness for a tighter loop before merging.
