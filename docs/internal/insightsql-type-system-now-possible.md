...
This is important because InsightsQL is used in user-authored queries, generated product queries, internal query runners, and multiple SQL print targets.
The type-system work improves metadata first.
It does not turn InsightsQL into a strict SQL validator.

## Structured Runtime Types

The new runtime type model lives in `insights/insightsql/type_system.py`.
...
## Structured Datastore Type Parsing

Datastore type strings can now be parsed structurally instead of being reduced to broad buckets.

Examples:

```python
parse_datastore_type("UInt64")
# RuntimeType(family="integer", signed=False, bits=64, nullable=False)

parse_datastore_type("Nullable(DateTime64(3, 'UTC'))")
# RuntimeType(family="datetime", precision=3, timezone="UTC", nullable=True)

parse_datastore_type("Array(Tuple(id UInt64, ts DateTime64(3, 'UTC')))")
# RuntimeType(
#     family="array",
...
- comparing inferred InsightsQL expression types with Datastore result metadata
...
That function first tries generic inference for high-value functions, then falls back to legacy `InsightsQLFunctionMeta.signatures`.
...
This is not full Datastore function parity.
...
`parse_datastore_type("Map(String, Nullable(Float64))")` produces a structured runtime map type, and `constant_type_from_runtime_type(...)` can bridge that into `MapType(key_type=StringType, value_type=FloatType(nullable=True))`.
...
`insights/insightsql/type_diagnostics.py` adds several developer-facing entry points.
...
That gives tests and diagnostics a direct way to ask Datastore what runtime type it sees for the same expression shape.
...
The comparison is intentionally family/nullability based rather than exact-width based because the current resolver compatibility layer can infer `Integer` while Datastore reports a narrower literal type such as `UInt8`.
...
- materialized-column introspection now carries the physical Datastore type reported by `system.columns`
...
- Datastore integration tests now prove minmax skip-index use for typed numeric and datetime materialized property comparisons, not only emitted-SQL string shape
...
- common non-null string and URL helper calls such as `base64Encode('test')` and `protocol('https://hanzo.ai')` now stay non-null through resolution, so emitted comparisons no longer need manual `assumeNotNull(...)` to avoid nullable boolean wrappers
...
Remaining higher-order gaps include broader aggregate-name coverage, aggregate combinator return typing, less-used Datastore variants, and strict validation of lambda arity and predicate return types.

Property-definition metadata is now part of property comparison planning.
`insights/insightsql/property_planner.py` combines semantic property-definition types, physical source metadata, materialized-column index metadata, restricted-property access control, and comparison compatibility.
The Datastore materialized range rewrite now consumes that plan before using direct physical-source comparisons.
...
If Datastore introspection reports a typed physical materialized source, such as `Nullable(Float64)` or `Nullable(DateTime64(6, 'UTC'))`, the planner can now prove that the physical column preserves semantic ordering.
...
The APIs needed for safe rewrites now exist, and the first guarded simplifier is available behind the `typeAwareCastSimplification` query modifier or the `InsightsQLContext.enable_type_aware_cast_simplification` flag.
...
Moving conversions across comparisons or simplifying generated property wrappers should still be done in separate guarded changes with emitted-SQL tests and Datastore integration tests where planner behavior matters.
...
When adding a Datastore function, choose the narrowest useful type behavior:
...
`insights/insightsql/transforms/type_aware_simplification.py` adds the first optimizer consumer.

It runs only when the `typeAwareCastSimplification` query modifier is enabled, or when the `InsightsQLContext.enable_type_aware_cast_simplification` flag is true.
...
While the modifier is enabled, every removal or fold increments `insightsql_type_simplification_total{dialect, kind}`.
That counter is deliberately unsampled, so a single-team pilot stays visible despite the 1% sampling of the other `insightsql_*` type metrics.
...
This gives internal callers a safe way to compare emitted SQL before and after simplification without changing user-authored InsightsQL behavior.

## Suggested Next Work

`insights/insightsql/property_planner.py` now adds property-planning metadata for materialized property comparisons.
...
The Datastore materialized range rewrite now uses that plan as its guard.
...
The integration tests now prove that `Nullable(Float64)` and `Nullable(DateTime64(6, 'UTC'))` materialized property comparisons are visible to Datastore's minmax skip indexes.
