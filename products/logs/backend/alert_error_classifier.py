...
Thin adapter over `insights.errors.classify_query_error` — maps the existing
...
from insights.errors import ExposedCHQueryError, QueryErrorCategory, classify_query_error
...
_UNKNOWN = ClassifiedAlertError(
    code="unknown",
    user_message="Alert check failed unexpectedly. Insights has been notified.",
)
...
def classify(exc: Exception) -> ClassifiedAlertError:
    ...
    if category == QueryErrorCategory.RATE_LIMITED:
        return ClassifiedAlertError(
            code="server_busy",
            user_message="Insights is temporarily busy. The alert check will retry automatically.",
        )
    ...
    if category == QueryErrorCategory.USER_ERROR:
        ...
        # ExposedInsightsQLError carries an explicit user-safe message. Many Datastore error
        # codes carry USER_ERROR category but ship as raw InternalCHQueryError (via
        # wrap_datastore_query_error) with the full DB::Exception text intact — fall
        # those through to the generic unknown message rather than leaking internals.
        if isinstance(exc, ExposedCHQueryError | ExposedInsightsQLError):
