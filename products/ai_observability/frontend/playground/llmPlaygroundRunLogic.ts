...
export const llmPlaygroundRunLogic = kea<llmPlaygroundRunLogicType>([
    ...
    listeners(({ actions, values }) => ({
        abortRun: () => {
            insights.capture('llma playground prompt aborted')
            currentAbortController?.abort()
        },
        setRateLimited: ({ retryAfterSeconds }) => {
            insights.capture('llma playground rate limited', { retry_after_seconds: retryAfterSeconds })
        },
        setSubscriptionRequired: ({ required }) => {
            if (required) {
                insights.capture('llma playground subscription required')
            }
        },
        ...
        submitPrompt: async (_: unknown, breakpoint: () => void) => {
            ...
            if (values.runAccessDeniedReason) {
                toast.error(values.runAccessDeniedReason)
            ...
            if (runnablePrompts.length === 0) {
                toast.error('Please add some messages before running prompts')
                actions.finishSubmitPrompt()
                return
            }

            insights.capture('llma playground prompt submitted', {
            ...
            try {
                const runs = runnablePrompts.map(async ({ prompt, index, messagesToSend }) => {
                    ...
                    insights.capture('llma playground prompt completed', {
                        model: prompt.model,
                        provider: selectedModelProvider,
