...
# Mirrors `AI_EVENT_NAMES` in insights/insightsql_queries/ai/ai_table_resolver.py
# and the Node.js list in nodejs/src/ingestion/ai/process-ai-event.ts —
# kept as a tuple here so InsightsQL placeholders can build a deterministic
...
# Retry policies
# Do not make Datastore capacity errors non-retryable here. Each run samples a disjoint window
