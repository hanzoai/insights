...
## Quick-probe InsightsQL gotchas
...
The sandbox bakes four upstream Insights skills — they hold the exact, maintained query
recipes so this scout doesn't have to. Read the matching one **when you go deep on a
lens**, not on every run:

- `insights:exploring-llm-costs` — cost over time, breakdowns by any dimension, cache /
  token economics, the 5-step cost-regression playbook.
- `insights:exploring-llm-traces` — finding traces by filter, the event hierarchy, drilling
  a single trace/session, parsing scripts. (Message content lives on the `insights.ai_events`
  table, not `events.properties`.)
- `insights:exploring-llm-evaluations` — the real eval-results path: AI pass/fail/N/A
  pattern summaries and raw `$ai_evaluation` SQL, plus config inspection and Script dry-runs.
- `insights:exploring-llm-clusters` — per-cluster cost/latency/error metrics, comparing
  clusters across runs.

`insights:querying-insights-data` is also baked — read it before writing non-trivial InsightsQL.

---

## Cost — read `insights:exploring-llm-costs`
...
## Latency — read `insights:exploring-llm-traces`
...
- **Discipline.** Never band latency in aggregate — o3 / preview models are structurally
  ...
  during a known upstream incident is not a Insights bug (but may be worth a `watch:`).

## Errors — read `insights:exploring-llm-traces`
...
## Volume — read `insights:exploring-llm-traces` / `execute-sql`
...
## Eval performance — read `insights:exploring-llm-evaluations`
...
## Eval / enrichment config health — read `insights:exploring-llm-evaluations`

The configuration surface, not the telemetry — evals, **taggers**, and **scorers** are all
LLM/Script jobs that can silently break.
...
- **Signal.** A config that's meant to be running but isn't doing its job: an eval / tagger
  ...
  **disabled/revoked provider key**, or a Script evaluator hitting the 5s execution limit.
...
## Clusters — read `insights:exploring-llm-clusters`
...
## Tool usage — read `insights:exploring-llm-traces`
