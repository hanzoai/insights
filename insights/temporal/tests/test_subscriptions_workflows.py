...
from insights.errors import CHQueryErrorS3Error
...
async def test_mixed_permanent_and_transient_failures_are_retryable(team, user) -> None:
    subscription = await sync_to_async(create_subscription)(team=team, target_value="a@hanzo.ai,b@hanzo.ai")
    inputs = DeliverSubscriptionInputs(
        subscription_id=subscription.id,
        exported_asset_ids=[],
        total_insight_count=0,
...
async def test_non_retryable_error_details_are_bounded(team, user) -> None:
    subscription = await sync_to_async(create_subscription)(
        team=team, target_value=",".join(f"user{i}@hanzo.ai" for i in range(120))
    )
    inputs = DeliverSubscriptionInputs(
        subscription_id=subscription.id,
        exported_asset_ids=[],
...
@patch("insights.temporal.exports.activities.exporter")
...
@pytest.mark.parametrize(
    ...
    [
        ("test_existing@hanzo.ai", ["test_new@hanzo.ai"]),
        ("test_existing@hanzo.ai,test_new@hanzo.ai", ["test_existing@hanzo.ai", "test_new@hanzo.ai"]),
    ],
)
@pytest.mark.asyncio
...
async def test_deliver_subscription_handles_slack_api_errors(team, user, slack_error_code, expect_auto_disable):
    ...
    if expect_auto_disable:
        ...
        assert result.recipient_results[0].error == {
            "message": "Insights can no longer post to this Slack channel",
...
async def test_deliver_subscription_legacy_false_sends_all_recipients(
    ...
    # Factory default target_value is "test1@hanzo.ai,test2@hanzo.ai" (2 recipients)
    subscription = await sync_to_async(create_subscription)(team=team, insight=insight, created_by=user)
    ...
    await env.run(
        ...
        DeliverSubscriptionInputs(
            ...
            exported_asset_ids=[asset.id],
            total_insight_count=1,
            previous_value="test1@hanzo.ai",
            is_new_subscription_target=False,
        ),
    )

    assert mock_send_email.call_count == 2  # "test1@hanzo.ai" and "test2@hanzo.ai"
...
async def test_deliver_subscription_workflow_end_to_end(
    ...
    # Both started and completed events flow through insights.slo.events
    started_calls = [
        c
...
async def test_new_subscription_sends_invite_email(
    ...
    subscription = await sync_to_async(create_subscription)(
        ...
        insight=insight,
        created_by=user,
        target_value="new_user@hanzo.ai",
    )
    ...
    assert call_args[0][0] == "new_user@hanzo.ai"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda: QueryError("Invalid InsightsQL query"), 999, 1, SloOutcome.SUCCESS),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            lambda: RuntimeError("Datastore connection timeout"),
            SloOutcome.FAILURE,
        ...
        pytest.param(
            lambda: QueryError("Invalid InsightsQL query"),
            SloOutcome.SUCCESS,
            None,
            None,
...
async def test_workflow_survives_large_insight_snapshot(
    ...
    # A raw InsightsQL query with `LIMIT 50000` over 7 narrow columns produces ~4.4 MB
...
@sync_to_async
def _create_ai_subscription(team, user, *, target_type="email", target_value="ai@hanzo.ai") -> Subscription:
