...
``InsightsPreviewStack`` knows how to bring Insights up inside any
...
Recipe (mount-over-image — the default, ~minutes per PR):
  - Run the ready-made published image (``ghcr.io/insights/insights:master``) and
    bind-mount the PR's backend source (``insights``/``ee``/``products``) over the
  ...
  - DB coherence: the restored golden's DB was migrated + seeded against the same
    ...
    (``migrate`` + ``migrate_datastore``). Reseeding is skipped — the golden is
  ...
  - ``build`` ESCAPE HATCH: pass ``image=None`` to build ``web`` from the
    ...
    to pin nothing). ``reset_db`` wipes pg+datastore so they migrate fresh
...
# scripts/insights-preview-setup.sh): both write docker-compose.preview.yml, and
...
# (missing PERSONFN_ADDR -> "personinsights client not configured" 500s); see
...
_CSRF_TRUSTED_ORIGINS = ",".join(f"https://*.boxes.hogland.{env}.insights.dev" for env in ("dev", "prod-us", "prod-eu"))
# The seeded demo login. These are the defaults of insights's
# `manage.py generate_demo_data` (--email / --password, see
# insights/management/commands/generate_demo_data.py), which generate_demo_data()
# runs unchanged. Not importable here — the tool runs in an SDK-only env with no
# insights Django on the path — so they're duplicated with that source named. If
# the seed defaults ever change, change these too.
_DEMO_EMAIL = "test@hanzo.ai"
_DEMO_PASSWORD = "12345678"

# Insights's prod settings refuse to boot on the default SECRET_KEY, so the
...
# PUBLIC URLs (pen-<id>.boxes.hogland.<env>.insights.dev) — the old
...
class InsightsPreviewStack:
    ...
    IMAGE = "ghcr.io/insights/insights:master"
    REPO_DIR = "/home/script/insights"
    COMPOSE = "docker-compose.dev-full.yml"
    OVERRIDE = "docker-compose.preview.yml"
    # Dependency services (published images, pulled by `up`).
    # personinsights-replica/router are defined in the OVERRIDE (not dev-full.yml) —
    ...
    # PERSONFN_ADDR pointing at a service that was never started. On the warm
    ...
    MOUNTS = [("insights", "/code/insights"), ("ee", "/code/ee"), ("products", "/code/products")]
    ...
    def __init__(
        ...
        *,
        ...
        # reset_db wipes pg + datastore before migrating, so the DB is migrated
    ...
    def bring_up(self) -> str:
        """Provision the box and bring Insights up; return the public URL."""
    ...
    def checkout_branch(self, branch: str) -> None:
        # The box's exec API runs as root while the repo is script-owned, so git
    ...
    def write_override(self) -> None:
        ...
        # scripts/insights-preview-setup.sh (the golden bake script). Both write
        ...
        # PERSONFN_ADDR + the personinsights services on 2026-07-06 (master's Django
        ...
        if self.image and self.mount:
            ...
            # .:/app/insights stays and these are appended.
        ...
        lines += [
            ...
            "      - DEBUG=0",
            # Random per-preview key (see self.secret_key) — Insights's prod
            ...
            # master's Django hard-requires the personinsights service for group-type
            # lookups (require_personinsights_client() raises "personinsights client not
            ...
            "      - PERSONFN_ADDR=personinsights-router:50052",
        ]
        # Mirror the bake script's personinsights service definitions (hogland
        # scripts/insights-preview-setup.sh): dev-full.yml carries NO personinsights
        # services (dev runs them via insightscli), so define them here the way HOBBY
        # does — extend the base definitions and point the replica at the MAIN
        # insights DB (persons live there for single-node deployments). The
        ...
        personinsights_tag = self.image.rsplit(":", 1)[-1] if self.image else "master"
        lines += [
            "  personinsights-replica:",
            "    extends:",
            "      file: docker-compose.base.yml",
            "      service: personinsights-replica",
            f"    image: ghcr.io/insights/insights/personinsights-replica:{personinsights_tag}",
            "    environment:",
            "      PRIMARY_DATABASE_URL: postgres://insights:insights@db:5432/insights",
            "  personinsights-router:",
            "    extends:",
            "      file: docker-compose.base.yml",
            "      service: personinsights-router",
            f"    image: ghcr.io/insights/insights/personinsights-router:{personinsights_tag}",
        ]
    ...
    def reset_database(self) -> None:
        ...
        # name; Datastore is volume-less, so recreating its container is enough.
        cmd = (
            f"cd {self.repo_dir} && "
            f"docker compose -f {self.COMPOSE} -f {self.OVERRIDE} stop db datastore web ; "
            f"docker compose -f {self.COMPOSE} -f {self.OVERRIDE} rm -f db datastore web ; "
            "docker volume rm insights_postgres-15-data || true"
        )
    ...
    def migrate(self) -> None:
        # Insights needs both: Postgres (schema) and Datastore (events DB +
        # tables). Datastore must be migrated before demo-data generation,
        # which writes events to it.
        timing.stage("migrate start (postgres + datastore)")
        self.backend.run_long(
        ...
        self.backend.run_long(
            self._compose("run --rm -T web python manage.py migrate_datastore"),
    ...
    def generate_demo_data(self) -> None:
        ...
        # test@hanzo.ai / 12345678 login so the preview opens populated.
...
"""
        ...
        logs = self.backend.exec("docker logs --tail 40 insights-web-1 2>&1", timeout=60)
        raise RuntimeError(
            "deep health (authed api) failed — the app is up but not usable:\n"
            f"{r.stdout.strip()}\n{r.stderr.strip()}\n"
            f"--- docker logs --tail 40 insights-web-1 ---\n{logs.stdout.strip()}"
        )
