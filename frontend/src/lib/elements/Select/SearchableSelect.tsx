...
function getSearchableLeaves<T>(options: SelectOptions<T>): SelectOption<T>[] {
...
function highlightActiveOption<T>(
    ...
    const mark = (item: SelectOption<T> | SelectSection<T>): typeof item => {
        if ('options' in item) {
            return { ...item, options: item.options.map(mark) } as typeof item
        }
        return item === activeOption ? { ...item, className: clsx(item.className, 'Button--active') } : item
    }
    return options.map(mark) as SelectOptions<T>
}
...
export function SearchableSelect<T extends string | number | boolean | null>({
    ...
    const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>): void => {
        if (e.key === 'ArrowDown') {
            // Stop propagation so an enclosing form (e.g. FormDialog) doesn't also act on the key.
            e.preventDefault()
            e.stopPropagation()
            ...
            // Keep Enter scoped to the dropdown so it never submits an enclosing form (e.g. FormDialog),
    ...
    const optionsWithSearch = useMemo(() => {
        ...
        const emptyMenuItem: SelectOption<T> = {
        ...
        return [searchMenuItem, ...listOptions] as SelectOptions<T>
