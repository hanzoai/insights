...
def _end_gc_boot_window() -> None:
    ...
    # objects for a 5-way shard of insights/api/test) costs ~0.5s per invocation but
...
def _cache_url_resolution() -> None:
    # URL resolution walks Insights's very large nested router linearly on every request
...
def _activate_personinsights_fake(request):
    """Force all person/group reads through the personinsights fake for every test.

    The fake is seeded explicitly by the test helpers in insights.test.persons
    ...
    if request.node.get_closest_marker("persons_db_direct"):
        yield
        return
    from insights.personinsights_client.fake_client import activate_personinsights_fake  # noqa: PLC0415, I001 — lazy import avoids connecting signals before Django is ready

    with activate_personinsights_fake():
        yield
...
def _clean_persons_db_for_direct_tests(request):
    ...
    These tests seed the persons DB through off-Django psycopg (insights.test.persons), which
...
def _query_cache_raw_redis_uses_fakeredis(monkeypatch):
    ...
    from insights import redis  # noqa: PLC0415
    from insights.query_cache import size_tracker  # noqa: PLC0415
