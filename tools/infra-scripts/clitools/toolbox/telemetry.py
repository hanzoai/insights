...
Sends a single event per invocation via the Insights Python SDK (``insights``, see
https://hanzo.ai/docs/libraries/python). The SDK is a declared dependency of
...
DEFAULT_HOST = "https://us.i.hanzo.ai"

EVENT_NAME = "toolbox cli used"

# The free-text reason is also submitted as a Insights API-type survey response so
# it lands in the Surveys UI (response browsing + AI summary). Create an API-type
# survey with one open-text question in Insights, then set SURVEY_ID (or the env var
...
# https://hanzo.ai/docs/surveys/implementing-custom-surveys
...
def _capture_survey_response(client, distinct_id: str, answer: str) -> None:
    """Submit the free-text reason as a Insights API-type survey response.
...
def capture_invocation(distinct_id: str, properties: dict) -> None:
    ...
    try:
        ...
        from insights import (
            Insights,  # type: ignore[import-not-found]  # noqa: PLC0415 — optional dep, kept off the import path
        )

        resolved_distinct_id = distinct_id or "unknown"
        client = Insights(project_api_key=api_key, host=host)
        client.capture(
            event=EVENT_NAME,
        ...
        print("📊 Telemetry skipped: insights SDK not installed (pip install -r requirements.txt).")  # noqa: T201
