...
The single source of truth for SDK outdatedness detection. Consumed by:
  ...
  - the SDK Health report endpoint (insights/api/sdk_health.py), which the SDK Health UI and the
...
def _build_sql_query(sdk_type: str, version: Optional[str]) -> str:
    ...
    # psycopg.sql.SQL or similar would produce the wrong string for Datastore/InsightsQL, and
    # `insights:execute-sql` parameterizes on its end anyway. The MCP agent receives the
...
def _build_activity_page_url(project_id: Optional[int], sdk_type: str, version: Optional[str]) -> str:
    ...
    can combine it with the user's known Insights host (e.g. us.hanzo.ai).
...
LEGACY_JAVA_MIGRATION_REASON = (
    "The com.insights.java:insights SDK has been archived. Migrate to com.insights:insights-server."
)
