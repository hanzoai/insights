...
# Limits concurrent offline-cluster Datastore queries across the messaging workflow family
...
# Datastore TOO_MANY_SIMULTANEOUS_QUERIES rejection. The cap is per worker process; aggregate =
...
messaging_datastore_query_semaphore = asyncio.Semaphore(max(1, settings.MESSAGING_DATASTORE_MAX_CONCURRENT_QUERIES))


@asynccontextmanager
async def get_messaging_client(team_id: int) -> AsyncIterator[DatastoreClient]:
    """Open a Datastore client while holding a messaging query slot.
    ...
    """
    async with messaging_datastore_query_semaphore, get_client(team_id=team_id) as client:
        yield client
