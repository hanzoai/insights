#!/bin/sh
# Renders the OpenTelemetry Collector config for the Insights metrics agent,
...
CONFIG_DIR="${CONFIG_DIR:-/etc/insights}"
...
awk -v snippet="$SNIPPET" -v debug="$DEBUG_ENABLED" -v persist="$PERSIST_ENABLED" '
    ...
    $0 == "#__FILE_STORAGE__" {
        if (persist == "1") {
            print "    file_storage:"
            print "        directory: \x27${env:QUEUE_DIR:-/var/lib/insights-agent}\x27"
