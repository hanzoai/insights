...
- `SQLSource` subclasses (Postgres, MySQL, BigQuery, Snowflake, MongoDB, Supabase, Redshift, MSSQL, Datastore, Neon, Convex) and file sources (Google Sheets, Custom) introspect user schemas and legitimately return nothing. Exclude them from the audit entirely; there is no fixed endpoint set to be missing.
...
FROM (
    SELECT source_type, team_id FROM postgres_insights_externaldatasource WHERE deleted = false
    UNION ALL
    SELECT source_type, team_id FROM eu_postgres_insights_externaldatasource WHERE deleted = false
)
...
- `documenting-warehouse-sources` — the public hanzo.ai docs for a source, which render from `get_documented_tables()`, the same call this audit uses.
