...
from insights.temporal.messaging.backfill_precalculated_person_properties_coordinator_workflow import (
...
class TestGetPersonIdRangesPageActivity:
    """Tests for get_person_id_ranges_page_activity's InsightsQL AST construction."""
    ...
    async def test_where_is_none_with_no_filters(self):
        ...
        with (
            patch(
                "insights.temporal.messaging.backfill_precalculated_person_properties_coordinator_workflow.compile_insightsql_for_streaming",
...
class TestPageActivityRowConsumption:
    ...
    async def test_rows_batched_into_ranges(self):
        ...
        module = "insights.temporal.messaging.backfill_precalculated_person_properties_coordinator_workflow"
        with (
            patch(f"{module}.compile_insightsql_for_streaming", side_effect=lambda node, *, team_id: ("SELECT 1", {})),
            patch(
                "insights.temporal.messaging.datastore_concurrency.get_client",
                return_value=_AsyncClientContextManager(mock_client),
            ),
