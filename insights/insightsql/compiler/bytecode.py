...
def create_bytecode(
    ...
    bytecode: list[Any] = []
    if args is None:
        bytecode.append(INSIGHTSQL_BYTECODE_IDENTIFIER)
        bytecode.append(INSIGHTSQL_BYTECODE_VERSION)
...
class BytecodeCompiler(Visitor):
    ...
    def __init__(
        ...
        self.context = context or InsightsQLContext(team_id=None)
    ...
    def visit(self, node: ast.AST | None):
        ...
        return self._visit_script_ast(node)
    ...
    def visit_call(self, node: ast.Call):
        ...
        # InsightsQL functions can have two sets of parameters: asd(args) or asd(params)(args)
    ...
    def visit_insightsqlx_tag(self, node: ast.InsightsQLXTag):
        ...
        if tag_is_callable:
            ...
            for attribute in node.attributes:
                response.extend(self._visit_insightsqlx_value(attribute.name))
                response.extend(self._visit_insightsqlx_value(attribute.value))
            response.append(Operation.DICT)
            ...
            response.extend(self._visit_insightsqlx_value("__hx_tag"))
            response.extend(self._visit_insightsqlx_value(node.kind))
    ...
    def _visit_script_ast(self, node: ast.AST | None):
        if node is None:
            return [Operation.NULL]
        if isinstance(node, ast.InsightsQLXTag):
            ...
            if tag_is_callable:
                return self.visit_insightsqlx_tag(node)
        response = []
        # We consider any object with the element "__hx_ast" to be a InsightsQLX AST node
        ...
        for field in dataclasses.fields(node):
            ...
            response.extend(self._visit_insightsqlx_value(value))
            fields += 1
        response.append(Operation.DICT)
        response.append(fields)
        return response

    def _visit_insightsqlx_value(self, value: Any) -> list[Any]:
        if isinstance(value, AST):
            return self.visit(value)
        if isinstance(value, list):
            elems = []
            for v in value:
                elems.extend(self._visit_insightsqlx_value(v))
            return [*elems, Operation.ARRAY, len(value)]
        if isinstance(value, dict):
            elems = []
            for k, v in value.items():
                elems.extend(self._visit_insightsqlx_value(k))
                elems.extend(self._visit_insightsqlx_value(v))
        ...
        if isinstance(value, ast.AST):
            if isinstance(value, ast.Placeholder):
                ...
                try:
                    ...
                    self.mode = prev_mode
                return response
            return self._visit_script_ast(value)
    ...
    def visit_select_query(self, node: ast.SelectQuery):
        ...
        self.mode = "ast"
        try:
            response = self._visit_script_ast(node)
        finally:
            self.mode = prev_mode
        return response
...
def execute_script(
    ...
    bytecode = create_bytecode(
        ...
        context=InsightsQLContext(team_id=team.id if team else None),
    ).bytecode
