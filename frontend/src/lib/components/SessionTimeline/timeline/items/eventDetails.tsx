...
import { escapeInsightsQLString, parseRecordIfJSONString } from './parsing'
...
function buildEventDetailsWhere(item: TimelineItem, sessionId?: string): string[] {
    const where: string[] = [`equals(uuid, '${escapeInsightsQLString(item.id)}')`]

    if (sessionId && shouldFilterBySessionId(item, sessionId)) {
        where.push(`equals($session_id, '${escapeInsightsQLString(sessionId)}')`)
    }
