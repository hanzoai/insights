...
description: 'Guide for exposing Insights product endpoints as MCP tools. Use when creating new or updating API endpoints, adding MCP tool definitions, scaffolding YAML configs, or writing serializers with good descriptions. Covers the full pipeline from Django serializer to generated TypeScript tool handler.'
---
...
With the server name "insights" (7 chars) plus a separator, tool names must stay at
or below 52 characters to fit within Cursor's 60-char combined limit.

### CI enforcement

- `pnpm --filter=@hanzo/mcp lint-tool-names` — validates length and pattern for YAML and JSON definitions
...
tools:
  your-tool-name: # kebab-case
    ...
    annotations:
      ...
      idempotent: true
    # Optional:
    mcp_version: 1 # 2 for create/update/delete ops, 1 for read/list if available via InsightsQL
    title: List things
    description: >
    ...
    feature_flag: my-flag-key # gate this tool behind a Insights feature flag
...
Add `feature_flag` to any tool (standard or query wrapper) to gate its exposure on a Insights feature flag evaluated at MCP init time for the current user.
...
## InsightsQL system tables

Every list/get endpoint should have a corresponding InsightsQL system table
in [`insights/insightsql/database/schema/system.py`](../../../insights/insightsql/database/schema/system.py).
...
(`models-<domain>.md`) in [`products/insights_ai/skills/querying-insights-data/references/`](../../../products/insights_ai/skills/querying-insights-data/references/)
and register it in [`products/insights_ai/skills/querying-insights-data/SKILL.md`](../../../products/insights_ai/skills/querying-insights-data/SKILL.md) under **Data Schema**.

## Two MCP versions

- **v1 (legacy)**: all CRUD tools exposed, for clients without skill support.
- **v2 (SQL-first)**: read/list tools replaced by InsightsQL, create/update/delete tools kept. For coding agents.
