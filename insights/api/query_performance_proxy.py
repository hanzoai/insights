"""OAuth-gated Datastore passthrough for query-performance autoresearch.

Stub: route + M2M auth only. The execution layer that forwards SQL to the
``autoresearch`` Datastore user will be added in a follow-up PR.
"""
...
class ExecuteTestClusterRequestSerializer(serializers.Serializer):
    sql = serializers.CharField(max_length=MAX_SQL_LENGTH, help_text="Datastore SQL to run against the test cluster.")
...
class ExecuteTestClusterResponseSerializer(serializers.Serializer):
    ...
    query_id = serializers.CharField(allow_null=True, help_text="Datastore query_id for this execution.")
...
_ACTION_SCOPES: dict[str, list[str]] = {
    "execute_test": ["datastore_test_cluster_perf:read"],
}
...
class QueryPerformanceProxyViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        description=(
            "Stub for the autoresearch Datastore passthrough. Auth + scope are wired up; "
