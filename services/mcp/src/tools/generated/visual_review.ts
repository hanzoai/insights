...
const visualReviewReposList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedRepoList>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof VisualReviewReposListSchema>) => {
        ...
        return await withInsightsUrl(context, result, '/visual_review')
    },
})
...
const visualReviewRunsList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedRunList>
> => ({
            ...
            context,
            {
                ...
                results: await Promise.all(
                    (result.results ?? []).map((item) =>
                        withInsightsUrl(context, item, `/visual_review/runs/${item.id}`)
                    )
                ),
            },
            '/visual_review'
...
const visualReviewRunsRetrieve = (): ToolBase<typeof VisualReviewRunsRetrieveSchema, WithInsightsUrl<Schemas.Run>> => ({
...
const visualReviewRunsSnapshotsList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedSnapshotList>
> =>
