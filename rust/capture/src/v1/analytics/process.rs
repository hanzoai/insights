...
/// strip or stamp it. `RawValue` and Datastore accept JSON the typed parse rejects,
...
mod tests {
    ...
    async fn td_limits_emit_a_warning_naming_the_hot_key(
        ...
        // Attribution comes from the request's Insights-Sdk-Info header.
        assert_eq!(w.extra_details["lib"], serde_json::json!("insights-rs"));
    ...
    async fn warnings_single_drop_includes_event_identifiers_and_sdk_info() {
        ...
        // sdk_info "insights-rs/1.0.0" from test_context splits into lib fields.
        assert_eq!(
            emitted[0].extra_details.get("lib"),
            Some(&serde_json::json!("insights-rs"))
        );
        assert_eq!(
