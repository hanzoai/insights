...
export class EmailService {
    ...
    public async executeSendEmail(
        ...
        try {
            ...
            // of whether the invocation came from a workflow action or an email destination script
            ...
            // (workflow action or email destination script function) resolves it the same way.
    ...
    // 1. Message-level: `tracking_enabled` on the email step's config (via `insightsFunction.metadata`,
    //    same as `message_category_type`). False is a hard off regardless of consent. Absent means
    //    tracked, so email destination script functions (which have no such config) keep tracking.
    ...
    private async resolveTrackingEnabled(
        ...
        if (invocation.insightsFunction?.metadata?.tracking_enabled === false) {
            return false
        }
        if (invocation.insightsFunction?.metadata?.message_category_type === 'transactional') {
            return true
        }
    ...
    private async sendEmailWithSES(
        ...
        if (this.sesConfig.sesTenantAttributionEnabled) {
            ...
            // insights/management/commands/migrate_ses_tenants.py). Deliberately NOT gated on
