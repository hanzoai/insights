...
def _detect_actor_type(
    ...
    # Sessions via funnel InsightsQL aggregation
    if hasattr(inner_source, "funnelsFilter") and inner_source.funnelsFilter:
        insightsql_agg = getattr(inner_source.funnelsFilter, "funnelAggregateByInsightsQL", None)
        if insightsql_agg and "session_id" in str(insightsql_agg):
            return "sessions"
...
def _summarize_events_query(query: EventsQuery) -> str:
    ...
    if query.where:
        lines.append(f"InsightsQL filters: {', '.join(query.where)}")
    ...
    for attempt in range(_MAX_PARSE_ATTEMPTS):
        try:
            content, _, _ = hit_openai(
                ...
                insights_properties=billable_ai_properties(team.id, "insight-ai-metadata-generation"),
                insights_groups=groups(),
