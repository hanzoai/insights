...
from insights.models.event.deletion import events_data_tables_via_sync_execute, events_read_tables_via_sync_execute
from insights.models.event.util import create_event
from insights.models.person.util import create_person, create_person_distinct_id, get_person_by_distinct_id
...
from insights.persons_seed import insert_seed_distinct_id, insert_seed_person, update_seed_person
...
NEW_SDK_SOURCE = "insights_mcp_analytics"
MCP_SERVER_NAME = "insights-mcp"
...
TOOL_DESCRIPTIONS = {
    "query_run": "Run a InsightsQL query against the project's events and return rows.",
...
IDENTIFIED_PERSONAS: list[dict[str, str]] = [
    {
        "distinct_id": "alice@mascot.dev",
        "email": "alice@mascot.dev",
        "name": "Alice Mascot",
        "role": "Product engineer",
    },
    {"distinct_id": "ben@mascot.dev", "email": "ben@mascot.dev", "name": "Ben Mascot", "role": "Data scientist"},
    {
        "distinct_id": "carol@mascot.dev",
        "email": "carol@mascot.dev",
        "name": "Carol Mascot",
        "role": "Product manager",
    },
    {
        "distinct_id": "dan@mascot.dev",
        "email": "dan@mascot.dev",
        "name": "Dan Mascot",
...
EXCEPTION_MESSAGES: list[str] = [
    ...
    "ConnectionError: Datastore connection reset by peer",
...
class Command(BaseCommand):
    help = "Seed MCP analytics events into Datastore for local testing."
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        def ensure_person(
            ...
            with personinsights_caller_tag("mcp-analytics/seed-sessions"):
        ...
        for session_idx in range(session_count):
            # $session_id is the canonical session key — the @hanzo/mcp SDK emits only
            # this (no $mcp_session_id), so these fixtures mirror a plain SDK-instrumented
            # server. uuid7 matches the Insights session-id convention.
