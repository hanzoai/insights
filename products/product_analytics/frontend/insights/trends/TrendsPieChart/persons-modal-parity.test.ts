...
/** Verifies that the script-charts TrendsPieChart click handler builds the same `InsightActorsQuery`
 ...
 *  `index`, while the script-charts path passes a one-slice dataset assembled from the source
...
describe('persons modal parity between ActionsPie and TrendsPieChart', () => {
    ...
    // Build the input the script-charts TrendsPieChart passes to datasetToActorsQuery — single slice.
