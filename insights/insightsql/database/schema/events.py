...
def events_table_datastore_table_ref(context) -> str:
...
class EventsPersonSubTable(VirtualTable):
    ...
    def to_printed_datastore_table_ref(self, context, use_logical_alias=True):
        if context.uses_new_events_schema() and use_logical_alias:
            return f"{events_table_datastore_table_ref(context)} AS events"
        return events_table_datastore_table_ref(context)
...
class EventsTable(Table):
    ...
    fields: dict[str, FieldOrTable] = {
        ...
        "created_at": DateTimeDatabaseField(
            name="created_at",
            nullable=False,
            description="When the event was ingested by Insights (server timestamp); differs from `timestamp`.",
        ),
