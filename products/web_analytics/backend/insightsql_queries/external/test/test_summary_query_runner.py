...
from insights.schema import DateRange, WebAnalyticsExternalSummaryQuery
...
from products.web_analytics.backend.insightsql_queries.external.summary_query_runner import (
    QueryResult,
...
class TestWebAnalyticsExternalSummaryQueryRunner(APIBaseTest):
    ...
    @patch("products.web_analytics.backend.insightsql_queries.external.summary_query_runner.chdb")
    ...
    @patch("products.web_analytics.backend.insightsql_queries.external.summary_query_runner.build_function_call")
    ...
    def test_build_correct_paths_and_structures_for_s3_chdb_queries_in_production_mode(self):
        with patch("insights.insightsql.database.schema.web_analytics_s3.DEBUG", False):
            runner = WebAnalyticsExternalSummaryQueryRunner(query=self.query, team=self.team)
    ...
    def test_build_correct_paths_and_structures_for_s3_chdb_queries_in_debug_mode(self):
        with patch("insights.insightsql.database.schema.web_analytics_s3.DEBUG", True):
            runner = WebAnalyticsExternalSummaryQueryRunner(query=self.query, team=self.team)
    ...
    def test_credential_helper_function_behavior(self):
        ...
        with patch("insights.insightsql.database.schema.web_analytics_s3.DEBUG", True):
            with patch("insights.insightsql.database.schema.web_analytics_s3.OBJECT_STORAGE_ACCESS_KEY_ID", "debug_key"):
                with patch(
                    "insights.insightsql.database.schema.web_analytics_s3.OBJECT_STORAGE_SECRET_ACCESS_KEY", "debug_secret"
                ):
                    creds = _get_s3_credentials()
