...
from insights.kafka_client.routing import KafkaClusterProfile, async_producer_scope
from insights.kafka_client.topics import KAFKA_DWH_CDP_RAW_TABLE
from insights.sync import database_sync_to_async_pool

from products.cdp.backend.models.insights_functions import InsightsFunction
...
class CDPProducer:
    ...
    async def should_produce_table(self) -> bool:
        ...
        @database_sync_to_async_pool
        def _check() -> bool:
            self.logger.debug(f"Checking if table {dot_notated_table_name} is used in any script functions or workflows")
            self.logger.debug(f"Using table_name = {dot_notated_table_name}, source = data-warehouse-table")

            try:
                has_matching_insights_function = (
                ...
                if has_matching_insights_function:
                    return True

                # Also gate on active workflows (InsightsFlows) triggered by this table - without this the
                # producer never emits to Kafka for a team whose only consumer is a warehouse-triggered workflow.
                return InsightsFlow.objects.filter(
                    team_id=self.team_id,
                ...
                # This queries Insights's own database, not the source being synced. A transient
                ...
                raise InsightsInternalDatabaseError(
                    "Failed to check script function/workflow triggers in Insights's database"
                ) from e
    ...
    async def produce_to_kafka_from_s3(self) -> None:
        ...
        # workflows (InsightsFlows) against trigger.table_name without an extra lookup.
        dot_notated_table_name = await self.get_dot_notated_table_name()
