...
import { insertInsightsFunction } from '../../_tests/fixtures'
import { InsightsFunctionManagerService } from './script-function-manager.service'

describe('InsightsFunctionManager', () => {
    ...
    beforeEach(async () => {
        ...
        manager = new InsightsFunctionManagerService(hub.postgres, hub.pubSub, hub.encryptedFields)
        ...
        insightsFunctions.push(
            await insertInsightsFunction(hub.postgres, teamId1, {
        ...
        insightsFunctions.push(
            await insertInsightsFunction(hub.postgres, teamId1, {
                name: 'Test Script Function team 1 - transformation',
                type: 'transformation',
                inputs_schema: [],
                inputs: {},
            })
        ...
        insightsFunctions.push(
            await insertInsightsFunction(hub.postgres, teamId2, {
                name: 'Test Script Function team 2',
                inputs_schema: [
                    {
                        type: 'string',
                        key: 'input_1',
    ...
    it('returns the script functions', async () => {
        let items = await manager.getInsightsFunctionsForTeam(teamId1, ['destination'])

        expect(items[0].id).toEqual(insightsFunctions[0].id)
        ...
        await hub.postgres.query(
            ...
            `UPDATE insights_hogfunction SET name='Test Script Function team 1 updated', updated_at = NOW() WHERE id = $1`,
            [insightsFunctions[0].id],
            'testKey'
        )

        // This is normally dispatched by django
        manager['onInsightsFunctionsReloaded'](teamId1, [insightsFunctions[0].id])

        items = await manager.getInsightsFunctionsForTeam(teamId1, ['destination'])

        expect(items).toMatchObject([
            {
                id: insightsFunctions[0].id,
                name: 'Test Script Function team 1 updated',
            },
        ])
    })

    describe('filters script functions by type', () => {
        it('for just transformations', async () => {
            expect((await manager.getInsightsFunctionsForTeam(teamId1, ['transformation'])).length).toEqual(1)
            expect((await manager.getInsightsFunctionsForTeam(teamId1, ['transformation']))[0].type).toEqual(
                'transformation'
            )
        })

        it('for just destinations', async () => {
            expect((await manager.getInsightsFunctionsForTeam(teamId1, ['destination'])).length).toEqual(1)
            expect((await manager.getInsightsFunctionsForTeam(teamId1, ['destination']))[0].type).toEqual('destination')
        })
    ...
    describe('getInsightsFunctionIdsForTeams', () => {
        it('returns function IDs filtered by type', async () => {
            const result = await manager.getInsightsFunctionIdsForTeams(
                [teamId1, teamId2],
                ['destination', 'transformation']
            )

            expect(result[teamId1]).toHaveLength(2)
            expect(result[teamId1]).toContain(insightsFunctions[0].id) // destination function
            expect(result[teamId1]).toContain(insightsFunctions[1].id) // transformation function

            expect(result[teamId2]).toHaveLength(1)
            expect(result[teamId2]).toContain(insightsFunctions[2].id) // destination function
        })

        it('returns empty arrays for teams with no matching functions', async () => {
            const nonExistentTeamId = teamId2 + 1
            const result = await manager.getInsightsFunctionIdsForTeams([nonExistentTeamId], ['transformation'])

            expect(result[nonExistentTeamId]).toEqual([])
        })

        it('filters by specific type correctly', async () => {
            const result = await manager.getInsightsFunctionIdsForTeams([teamId1], ['transformation'])

            expect(result[teamId1]).toHaveLength(1)
            expect(result[teamId1]).toContain(insightsFunctions[1].id) // only the transformation function
            expect(result[teamId1]).not.toContain(insightsFunctions[0].id) // not the destination function
        })
        ...
        it('handles disabled functions', async () => {
            ...
            await hub.postgres.query(
                PostgresUse.COMMON_WRITE,
                `UPDATE insights_hogfunction SET enabled=false, updated_at = NOW() WHERE id = $1`,
                [insightsFunctions[0].id],
                'testKey'
            )
            ...
            const result = await manager.getInsightsFunctionIdsForTeams([teamId1], ['destination'])
    ...
    it('does not return deleted functions when fetched by id', async () => {
        ...
        expect(await manager.getInsightsFunction(insightsFunctions[0].id)).toEqual(null)
    })
})
...
describe('Hogfunction Manager - Execution Order', () => {
    ...
    beforeEach(async () => {
        ...
        insightsFunctions.push(
            await insertInsightsFunction(hub.postgres, teamId, {
                name: 'fn1',
                execution_order: 1,
                type: 'transformation',
            })
    ...
    it('maintains correct execution order after individual reloads', async () => {
        // Initial order check
        let teamFunctions = await manager.getInsightsFunctionsForTeam(teamId, ['transformation'])
        ...
        await hub.postgres.query(
            PostgresUse.COMMON_WRITE,
            `UPDATE insights_hogfunction SET execution_order = 3, updated_at = NOW() WHERE id = $1`,
        ...
        await hub.postgres.query(
            PostgresUse.COMMON_WRITE,
            `UPDATE insights_hogfunction SET execution_order = 2, updated_at = NOW() WHERE id = $1`,
            [insightsFunctions[2].id],
            'testKey'
        )

        manager['onInsightsFunctionsReloaded'](teamId, [insightsFunctions[2].id, insightsFunctions[1].id])
        ...
        await hub.postgres.query(
            PostgresUse.COMMON_WRITE,
            `UPDATE insights_hogfunction SET execution_order = 1, updated_at = NOW() WHERE id = $1`,
            [insightsFunctions[2].id],
            'testKey'
        )
    ...
    it('should handle null/undefined execution orders and created_at ordering', async () => {
        ...
        manager['onInsightsFunctionsReloaded'](teamId2, [insightsFunctions[2].id, insightsFunctions[1].id])
        const teamFunctions = await manager.getInsightsFunctionsForTeam(teamId2, ['transformation'])
