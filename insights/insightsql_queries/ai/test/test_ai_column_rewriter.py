...
from insights.insightsql_queries.ai.ai_column_rewriter import (
    _BOOLEAN_COLUMNS,
    _EVENTS_TABLE_ALIAS,
    _NUMERIC_COLUMNS,
...
class TestAiColumnToPropertyRewriter:
    ...
    def test_numeric_column_usable_in_sum(self):
        ...
        query = parse_select("SELECT sumIf(input_tokens, event = '$ai_generation') FROM insights.ai_events AS ai_events")
    ...
    def test_scope_only_rewrites_in_ai_events_query(self):
        query = parse_select("SELECT trace_id FROM insights.ai_events AS ai_events")
        result = rewrite_query_for_events_table(query)
        assert isinstance(result, ast.SelectQuery)
    ...
    def test_subquery_inner_rewritten_outer_preserved(self):
        query = parse_select(
            "SELECT trace_id FROM (SELECT trace_id AS trace_id FROM insights.ai_events AS ai_events GROUP BY trace_id)"
        )
        result = rewrite_query_for_events_table(query)
        assert isinstance(result, ast.SelectQuery)
    ...
    def test_subquery_inner_bare_field_gets_alias_for_outer_reference(self):
        ...
        query = parse_select("SELECT trace_id FROM (SELECT trace_id FROM insights.ai_events AS ai_events)")
        result = rewrite_query_for_events_table(query)
        assert isinstance(result, ast.SelectQuery)
        assert isinstance(result.select[0], ast.Field)
        assert result.select[0].chain == ["trace_id"]
    ...
    def test_subquery_result_alias_named_events_is_preserved(self):
        query = parse_select(
            "SELECT events FROM (SELECT groupArray(event) AS events FROM insights.ai_events AS ai_events)"
        )
    ...
    def test_union_all_both_branches_rewritten(self):
        query = parse_select(
            """
            SELECT trace_id AS trace_id FROM insights.ai_events AS ai_events
            UNION ALL
            SELECT trace_id AS trace_id FROM insights.ai_events AS ai_events
            """
        )
    ...
    def test_column_to_property_mapping_is_inverse(self):
        from insights.insightsql_queries.ai.ai_property_rewriter import AI_PROPERTY_TO_COLUMN
    ...
    def test_custom_alias_fields_rewritten(self):
        query = parse_select("SELECT e.trace_id, e.latency, e.is_error, e.timestamp FROM insights.ai_events AS e")
        result = rewrite_query_for_events_table(query)
        assert isinstance(result, ast.SelectQuery)
