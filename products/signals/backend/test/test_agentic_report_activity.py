...
def _build_research_output() -> ReportResearchOutput:
    ...
    return ReportResearchOutput(
        ...
        new_artefacts=[
            ...
            SignalFinding(
                signal_id="sig-2",
                relevant_code_paths=["insights/api/event.py"],
    ...
    with patch("products.signals.backend.temporal.agentic.report.Heartbeater"):
        return await run_agentic_report_activity(
            RunAgenticReportInput(
                ...
                repo_selection=RepoSelectionResult(repository="insights/insights", reason="test"),
            )
        )
...
async def test_select_repository_activity_returns_repo(monkeypatch, ateam):
    ...
    async def fake_select_repo(*args, **kwargs):
        return RepoSelectionResult(repository="insights/insights", reason="Single repository connected: insights/insights")
    ...
    assert result.repository == "insights/insights"
...
async def test_select_repository_activity_reuses_previous_selection(monkeypatch, ateam):
    previous = RepoSelectionResult(repository="insights/insights", reason="Previously selected")
...
async def test_select_repository_activity_does_not_raise_with_only_user_integration(monkeypatch, ateam):
    # Insights Desktop installs land in `UserIntegration`, never on `Integration`. Before the cascade
    ...
    user = await sync_to_async(User.objects.create)(email=f"insights-code-{random.randint(1, 99999)}@example.com")
...
async def test_run_agentic_report_activity_persists_artefacts(monkeypatch, ateam):
    ...
    with patch("products.signals.backend.temporal.agentic.report.Heartbeater"):
        result = await run_agentic_report_activity(
            RunAgenticReportInput(
                ...
                repo_selection=RepoSelectionResult(
                    repository="insights/insights", reason="Single repository connected: insights/insights"
                ),
            )
        )
        ...
        assert repo_selection_content == {
            ...
            "reason": "Single repository connected: insights/insights",
            "task_id": None,
        }
