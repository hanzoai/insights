...
from insights.egress.transport.transport import EgressBudgetExhausted
from insights.exceptions_capture import ambient_exception_properties
from insights.temporal.common.errors import NonReportableError
from insights.temporal.common.interceptor import ALL_TASK_QUEUES
from insights.temporal.common.logger import get_write_only_logger
from insights.temporal.common.shutdown import WorkerShuttingDownError
...
def _tag_team_id_on_current_span(input: ExecuteActivityInput | ExecuteWorkflowInput) -> None:
    ...
    team_id is named consistently across Insights's Temporal input dataclasses, so a reflective
    ...
    async def execute_workflow(self, input: ExecuteWorkflowInput) -> Any:
        ...
        try:
            ...
            try:
                ...
                capture_kwargs = {
                    "properties": {
                        # See the matching comment in _InsightsClientActivityInboundInterceptor:
...
class InsightsClientInterceptor(Interceptor):
    """Insights Interceptor: reports workflow & activity exceptions to Insights and tags the
