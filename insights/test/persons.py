...
than calling ORM methods directly.  personinsights is the sole source of truth, so
these helpers seed the active personinsights fake (and, for persons, Datastore) —
...
test's personinsights fake is active, so a direct `Person.objects.create(...)` in a
...
from insights.models import signals
...
from insights.persons_db import persons_db_connection
from insights.persons_seed import (
...
def _get_active_fake():
    from insights.personinsights_client.fake_client import _active_fake  # noqa: PLC0415
...
def _seed_distinct_id_into_fake(team_id: int, person_id: int, distinct_id: str, version: int = 0) -> None:
    ...
    from insights.personinsights_client.proto.generated.personinsights.types.v1 import person_pb2  # noqa: PLC0415
...
# ── Datastore sync (replaces the post_save/post_delete signal mirror) ─


def _ch_sync_person(person: Person, distinct_ids: list[str]) -> None:
    """Mirror a person + its distinct ids into Datastore, matching the old
    post_save signal behavior that fired on ORM creation.  Like that signal it is
    a @mutable_receiver, so mute_selected_signals() suppresses the Datastore write."""
    if signals.is_muted:
        return
    _ch_create_person(
        team_id=person.team_id,
...
def create_people_bulk(specs: list[dict[str, Any]]) -> list[Person]:
    """Create many persons at once: one Datastore insert per table instead of two per person.

    Each spec is a create_person() kwargs dict (team/team_id, distinct_ids, uuid, ...). Row values
    mirror what create_person writes via insights.models.person.util.create_person /
    create_person_distinct_id — only batched. Note: distinct-id rows use Python-side
    (freezegun-frozen) time for _timestamp rather than Datastore server now(); harmless because
    pdi2 dedup uses version, not _timestamp. The persons-DB-layer path (fake off) falls back to
    per-person create_person.
    """
    import json  # noqa: PLC0415

    from insights.datastore.client import sync_execute  # noqa: PLC0415
    from insights.models.person.sql import BULK_INSERT_PERSON_DISTINCT_ID2, INSERT_PERSON_BULK_SQL  # noqa: PLC0415
...
def create_person(*, team: Team | None = None, distinct_ids: list[str] | None = None, **kwargs: Any) -> Person:
    ...
    Consumer tests (the personinsights fake is active): seed Datastore + the fake,
    ...
    persons DB directly, sees them — matching the pre-personinsights behavior.
    """
...
def delete_person(person: Person) -> None:
    """Soft-delete a person in Datastore and unseed the personinsights fake.

    Mirrors insights.models.person.util.delete_person: writes CH tombstones with
...
def update_person(person: Person) -> None:
    """Re-sync a mutated person into Datastore and the personinsights fake."""
    _ch_create_person(
        team_id=person.team_id,
        properties=person.properties or {},
        uuid=str(person.uuid),
...
def add_distinct_id(*, person: Person, distinct_id: str, version: int = 0) -> PersonDistinctId:
    """Add a distinct ID to a person in Datastore + the personinsights fake (or the persons DB when fake-off)."""
    if not signals.is_muted:
...
def stage_person_for_bulk_create(*args: Any, **kwargs: Any) -> Person:
    ...
    all staged persons into Datastore + the personinsights fake.
...
def flush_persons_to_db_and_datastore() -> dict[str, Person]:
    """Bulk-insert all staged persons into Datastore + the personinsights fake.

    Returns the person_mapping (distinct_id → Person) for event flushing.
    Called by flush_persons_and_events() in base.py.
    """
    from insights.models.person.util import bulk_create_persons  # noqa: PLC0415
...
def create_group(*, team: Team | None = None, group_type_index: int, group_key: str, **kwargs: Any) -> Group:
    ...
    Fake active: seed the personinsights fake only (no persons DB write).  Fake off
...
def update_group(group: Group) -> None:
    """Re-seed a mutated group into the personinsights fake."""
    _seed_group_into_fake(group)
...
def update_group_type_mapping(mapping: GroupTypeMapping) -> None:
    """Re-seed a mutated group type mapping into the personinsights fake."""
    _seed_group_type_mapping_into_fake(mapping)
