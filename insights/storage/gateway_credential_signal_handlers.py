...
old hash. Wired from InsightsConfig.ready(). All handlers no-op without AI_GATEWAY_REDIS_URL,
...
from insights.models.utils import SHA256_HASH_PREFIX
...
from insights.storage.hypercache_manager import HYPERCACHE_SIGNAL_UPDATE_COUNTER

# The gateway-credential task functions are imported lazily inside the handlers
# below: this module is wired at django.setup(), and importing insights.tasks.*
# eagerly loads every task module (celery autoimport), dragging insights.schema /
# insights.insightsql onto the startup path of every process. See django-startup-time.
...
def _on_credential_save(
    ...
    def enqueue() -> None:
        from insights.tasks.gateway_credential import update_gateway_credential_cache_task  # noqa: PLC0415
...
def _reproject_user_sync_then_async(user_id: int) -> None:
    ...
    def _invalidate() -> None:
        from insights.tasks.gateway_credential import reproject_user_gateway_credentials_task  # noqa: PLC0415
...
def _reproject_team_on_change(sender: type[Team], instance: Team, created: bool, **kwargs: Any) -> None:
    ...
    from insights.tasks.gateway_credential import reproject_team_gateway_credentials_task  # noqa: PLC0415
