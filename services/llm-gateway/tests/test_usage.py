...
class TestUsageEndpoint:
    ...
    def test_returns_pro_limits_by_default(self, authenticated_usage_client: TestClient) -> None:
        ...
        assert data["product"] == "insights_code"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("insights-code-200-20260301", True),
    ...
    def test_returns_pro_limits_with_pro_plan(self, authenticated_usage_client: TestClient) -> None:
        app = authenticated_usage_client.app
        app.state.plan_resolver.get_plan = AsyncMock(
            return_value=PlanInfo(plan_key="insights-code-200-20260301", seat_created_at=None)
        )
    ...
    def test_returns_401_without_auth(self, client: TestClient) -> None:
        response = client.get("/v1/usage/insights_code")
    ...
    def test_credits_reflect_products_own_bucket(self, authenticated_usage_client: TestClient) -> None:
        # insights_code's usage reports the insights_code_credits bucket (under the
        ...
        assert data["is_rate_limited"] is True
        assert resolver_mock.call_args.args[0] == "insights_code_credits"

    @pytest.mark.parametrize("plan_key", ["insights-code-200-20260301", "insights-code-free-20260301", None])
    def test_exhausted_bucket_reported_for_every_caller(
        self, authenticated_usage_client: TestClient, plan_key: str | None
    ) -> None:
        # Mirrors BillableCreditThrottle: every insights_code generation counts into
    ...
    def test_ai_credits_carries_org_spend_numbers(self, authenticated_usage_client: TestClient) -> None:
        """Insights Desktop renders "used $X of $Y" (titlebar, plans page) off these
    ...
    def test_invalidate_plan_cache_401_without_auth(self, client: TestClient) -> None:
        response = client.post("/v1/usage/insights_code/invalidate-plan-cache")
        assert response.status_code == 401
