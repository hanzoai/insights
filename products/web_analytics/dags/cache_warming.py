...
from insights.datastore.query_tagging import Feature, reset_query_tags, tag_queries
...
from insights.insightsql_queries.query_runner import ExecutionMode, get_query_runner_or_none
...
from products.web_analytics.backend.insightsql_queries.web_goals_lazy_precompute import (
    can_use_lazy_precompute as can_use_goals_lazy_precompute,
)
from products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common import (
    BACKGROUND_WARMING_TRIGGERS,
    MAX_PRECOMPUTE_DAYS,
    SHAPE_CAP_KEY_IGNORED_QUERY_FIELDS,
)
from products.web_analytics.backend.insightsql_queries.web_overview_lazy_precompute import (
    can_use_lazy_precompute as can_use_overview_lazy_precompute,
)
from products.web_analytics.backend.insightsql_queries.web_stats_frustration_lazy_precompute import (
    can_use_lazy_precompute as can_use_frustration_lazy_precompute,
)
from products.web_analytics.backend.insightsql_queries.web_stats_lazy_precompute import (
    can_use_lazy_precompute as can_use_stats_lazy_precompute,
)
from products.web_analytics.backend.insightsql_queries.web_stats_paths_lazy_precompute import (
    can_use_lazy_precompute as can_use_paths_lazy_precompute,
)
from products.web_analytics.backend.insightsql_queries.web_vitals_paths_lazy_precompute import (
...
# the Datastore kill switch's overload cap still clamps it during an overload.
...
def queries_to_keep_fresh(
    ...
    # insights/models/query_metrics/sql.py was never migrated, the table does not
    ...
    # _SELECTION_MAX_BYTES_TO_READ — a finite value, not 0, so the Datastore
    ...
    # datastore_driver %-formats the query when params are passed.
    # nosemgrep: datastore-fstring-param-audit (interpolations are module-level constants from hardcoded tuples, not user input; everything dynamic is parameterized)
...
# Datastore pool. Overridable via WEB_ANALYTICS_WARMING_SHARD_THREADS without
...
# overridable live via WEB_ANALYTICS_WARMING_SHARDS. Total Datastore-side
...
# A Datastore node dying mid-pass can leave every worker thread blocked in a
...
# bounded by Datastore-side execution timeouts at minutes, so zero completions
...
# The warmer shares its per-user Datastore query budget with every other
...
def split_warmable_queries_op(context: dagster.OpExecutionContext, config: WarmQueriesConfig, queries: list[dict]):
    ...
    multiprocess executor, with its own GIL. InsightsQL compilation is CPU-bound
...
@dagster.job(
    ...
    tags={
        ...
        # sharded pass runs one subprocess per shard, each compiling InsightsQL on its
