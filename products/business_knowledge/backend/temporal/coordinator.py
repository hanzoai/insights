...
def _reemit_one_document(doc: logic.DocumentToEmbed) -> int:
    ...
    vectors alive past the 3-month Datastore TTL.
...
async def refresh_aging_embeddings_activity() -> dict[str, Any]:
    ...
    3-month Datastore TTL, so long-lived knowledge never silently loses its
...
def _present_chunk_ids_in_datastore(team_id: int, chunk_ids: list[UUID]) -> set[str]:
    """Return the subset of ``chunk_ids`` that already have a vector row in the
    shared Datastore embeddings table for this team + model. InsightsQL auto-scopes
...
async def reconcile_embeddings_activity() -> dict[str, Any]:
    """
    Re-verify that already-emitted SAFE docs actually landed in Datastore.
    ...
    # One Datastore query per team, not per doc: InsightsQL auto-scopes to a single
    ...
    for team_id, team_docs in docs_by_team.items():
        ...
        present = await database_sync_to_async(_present_chunk_ids_in_datastore, thread_sensitive=False)(
            team_id, chunk_ids
        )
...
class BusinessKnowledgeIngestSourceWorkflow(InsightsWorkflow):
    @workflow.run
...
class BusinessKnowledgeRefreshSourceWorkflow(InsightsWorkflow):
...
class BusinessKnowledgeRefreshCoordinatorWorkflow(InsightsWorkflow):
    @workflow.run
    async def run(self) -> dict[str, Any]:
        ...
        # Reconcile first so any "emitted but never landed in Datastore" docs are
