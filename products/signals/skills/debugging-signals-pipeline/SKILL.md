...
description: >
  ...
  diagnosing common failures (stale Datastore embeddings, agentsh network
...
```json
{
  "type": "notification",
  "timestamp": "...",
  "notification": { "jsonrpc": "2.0", "method": "_insights/console", "params": { "level": "debug", "message": "..." } }
}
```
...
- **`_insights/progress`** events — show which setup step the sandbox reached
- **`_insights/console`** debug messages — show sandbox provisioning, cloning, agent startup
...
The container is named `task-sandbox-<task-id>-<random>` and uses the `insights-sandbox-base` image.
...
Usually caused by stale embeddings in Datastore after a `cleanup_signals` that failed to delete them.

**Root cause:** `DATASTORE_DATABASE` not set in `.env`. The cleanup command uses `sync_execute`
which connects to the `DATASTORE_DATABASE` (defaults to `default`), but the embedding tables
live in the `insights` database.

**Fix:** Add `DATASTORE_DATABASE=insights` to `.env` and restart workers.

**Manual cleanup of stale embeddings:**

```bash
curl -s 'http://localhost:8123/' --data-binary \
  "ALTER TABLE insights.sharded_insights_document_embeddings_text_embedding_3_small_1536 DELETE WHERE product = 'signals' AND team_id = 1 SETTINGS mutations_sync = 1"
```
...
```bash
curl -s 'http://localhost:8123/' --data-binary \
  "SELECT count() FROM insights.sharded_insights_document_embeddings_text_embedding_3_small_1536 WHERE team_id = 1 AND product = 'signals'"
```
...
### Datastore embedding tables "not found" during cleanup

The tables exist in the `insights` database but `sync_execute` queries the `default` database.

```bash
# Verify tables exist
curl -s 'http://localhost:8123/' --data-binary "SHOW TABLES FROM insights LIKE '%embed%'"

# Check current DATASTORE_DATABASE setting
grep DATASTORE_DATABASE .env
```
