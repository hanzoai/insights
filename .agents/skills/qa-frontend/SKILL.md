...
name: qa-frontend
description: >
  Internal Insights developer frontend/browser QA skill. Use only when a Insights
  developer explicitly asks to run frontend QA, browser-test a PR, verify a UI
  flow against the local Insights stack, use qa-frontend, or QA current frontend
...
Run the code, not just the diff. This is a repo-local skill for Insights developers working on Insights itself. It executes a bounded frontend QA loop against a local Insights stack and operates in one of two modes:
...
3. Require a reachable local stack and working browser MCP/tooling session. If browser MCP tools are missing, load `references/browser-mcp-patterns.md` and ask before configuring anything. Reuse the developer's current Insights setup by default; always ask before starting Insights.
...
10. PR mode only: after approval, upload selected evidence with `insightscli pr:upload-image`, verify PR comment connectivity, and post one final PR comment for every completed run, including clean runs. Push only after explicit approval.
...
Load `references/stack-and-login.md` now. Do not checkout, edit, upload, comment, or push until it confirms the local Insights stack is reachable enough for the planned QA target. It owns `BASE_URL`, `STACK_STARTED_BY_AGENT`, repo-local `run-insights` delegation, phrocs checks, approval rules for startup, and login/setup workspace handling.
...
Do not autonomously edit auth, permissions, SQL/InsightsQL construction, migrations, workflow files, or skill files. Route those findings to comment-only.
