...
Timeouts and retry policies mirror ``insights/temporal/ai_observability/trace_clustering/constants.py``
— the workloads have the same shape (CPU-bound compute → external embedding
worker → Datastore-backed aggregates → Datastore write). Diverging numbers
...
JOURNEYS_ACTIVITY_TIMEOUT = timedelta(seconds=60)  # InsightsQL journey aggregation
...
# Journeys activity — Datastore-backed, may hit query limits. Retry once
