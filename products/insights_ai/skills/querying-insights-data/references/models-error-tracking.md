...
- Internal storage pointers and content hashes are intentionally omitted from InsightsQL.
- Use `insights:error-tracking-symbol-sets-list` with `status = 'valid'` or `status = 'invalid'` to check upload availability.
- Use `insights:error-tracking-symbol-sets-list` with an exact `ref` to resolve a reference to an ID, then `insights:error-tracking-symbol-sets-retrieve` or `insights:error-tracking-symbol-sets-download-retrieve` by ID. Download URLs expire after one hour; use them immediately and do not echo them back unless the user explicitly asks.
