...
class BigQuerySource(SQLSource[BigQuerySourceConfig]):
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            # Writing query results into the `__insights_import_...` temp tables Insights creates
            ...
            "bigquery.tables.update": "BigQuery denied write access to a temporary table Insights creates in your dataset. Insights copies query results into temporary tables before reading them, so read access alone isn't enough. Please grant your service account write access (for example the BigQuery Data Editor role) on the dataset where these temporary tables are created — your main dataset, or the temporary dataset if you configured one — then reconnect the source.",
            # Creating the `__insights_import_...` temp tables (the `WRITE_TRUNCATE` destination in
            ...
            "bigquery.tables.create": "BigQuery denied permission to create a temporary table Insights needs in your dataset. Insights copies query results into temporary tables before reading them, so read access alone isn't enough. Please grant your service account permission to create tables (for example the BigQuery Data Editor role) on the dataset where these temporary tables are created — your main dataset, or the temporary dataset if you configured one — then reconnect the source.",
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption=(
                "Enter your BigQuery credentials to automatically pull your BigQuery data into the Insights Data "
                "warehouse. To send Insights data to BigQuery instead, set up a batch export."
            ),
