...
query-time computation:
  ...
  1. Resolve a normalized token from the strongest available signal (the
     ...
     then the clientInfo.name — `$mcp_client_name` as reported by the insights-node
     MCP analytics SDK, or `mcp_session_client_name` as reported by Insights's hosted
...
copies must move in lockstep: the query in the `querying-insights-data` skill's
...
# the strip keeps the pattern case-flag-free; the doubled backslashes survive InsightsQL
...
def harness_label_or_token_sql(token_col: str = "h", display_col: str = "client_display") -> str:
    ...
    ("insights-attribution-service", "openclaw-bundle-mcp"), which is strictly more
...
def _label_multi_if(token_col: str, fallback_sql: str) -> str:
    ...
    return f"""multiIf(
        ...
        -- Insights's own CLI (`services/mcp/src/cli/context.ts` hard-codes this name).
        ...
        {token_col} = 'insights-cli', 'Insights CLI',
        {fallback_sql}
    )"""
