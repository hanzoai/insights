# Insights AI agent-run surface — consumer guide
...
```ts
import { ReadonlyRunSurface } from 'products/insights_ai/frontend/api/readableRun'
import { isTerminalRunStatus } from 'products/insights_ai/frontend/api/logics'
```
...
import { Composer, QueuedMessageList } from 'products/insights_ai/frontend/api/primitives'
import { runInteractionLogic } from 'products/insights_ai/frontend/api/logics'
...
```tsx
import { runStreamLogic } from 'products/insights_ai/frontend/api/logics'
import { RunSurface } from 'products/insights_ai/frontend/api/runSurface'
...
```tsx
import { Thread } from 'products/insights_ai/frontend/api/primitives'
...
```tsx
import { registerToolRenderers, type ToolRegistryEntry } from 'products/insights_ai/frontend/api/tools'
...
This is the generic per-product mechanism. The surface itself is its first consumer: the Insights product
...
```ts
import { runStreamLogic, isTerminalRunStatus } from 'products/insights_ai/frontend/api/logics'
...
`type: 'instructions'` items render into `<insights_trusted_context>` (our own injected guidance — never put
user/ingested data there), everything else into `<insights_untrusted_context>` behind hardening prose. Items
...
```tsx
import { AttachedContextBar, Composer } from 'products/insights_ai/frontend/api/primitives'
;<Composer.Header>
  <AttachedContextBar />
</Composer.Header>
```
...
```ts
import { useToolStreamListener } from 'products/insights_ai/frontend/api/logics'
...
Note: for exec-wrapped Insights tools the resolved name can be unknown at `phase: 'started'` (the command
