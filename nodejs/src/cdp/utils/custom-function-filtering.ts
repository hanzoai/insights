...
const HREF_REGEX = createTrackedRE2(/(?::|")href="(.*?)"/, undefined, 'script-filtering:href')
const TEXT_REGEX = createTrackedRE2(/(?::|")text="(.*?)"/g, undefined, 'script-filtering:text')
const ID_REGEX = createTrackedRE2(/(?::|")attr_id="(.*?)"/g, undefined, 'script-filtering:id')
const ELEMENT_REGEX = createTrackedRE2(
    /(?:^|;)(a|button|form|input|select|textarea|label)(?:\.|$|:)/g,
    undefined,
    'script-filtering:element'
)

const customFunctionFilterDuration = new Histogram({
    name: 'cdp_custom_function_filter_duration_ms',
...
const customFunctionFilterOutcomes = new Counter({
    name: 'cdp_custom_function_filter_outcome',
    help: 'Count of filter outcomes',
    labelNames: ['result', 'result_type'],
})

const customFunctionPreFilterCounter = new Counter({
    name: 'cdp_custom_function_prefilter_result',
...
export function convertClickhouseRawEventToFilterGlobals(event: RawClickHouseEvent): CustomFunctionFilterGlobals {
    ...
    // Handle person
    let person: CustomFunctionFilterGlobals['person'] = null
    let pdi: CustomFunctionFilterGlobals['pdi'] = null
    ...
    const response: CustomFunctionFilterGlobals = {
        event: event.event,
        uuid: event.uuid,
    ...
    groupProperties.forEach((groupPropsString, index) => {
        if (groupPropsString) {
            ...
            const groupKey = `group_${index}` as keyof CustomFunctionFilterGlobals
    ...
    for (let i = 0; i < 5; i++) {
        const groupIdKey = `$group_${i}` as keyof CustomFunctionFilterGlobals
...
export function convertToCustomFunctionFilterGlobal(
    globals: Pick<CustomFunctionInvocationGlobals, 'event' | 'person' | 'groups' | 'variables'>
): CustomFunctionFilterGlobals {
...
function preFilterResult(filters: CustomFunctionType['filters'], filterGlobals: CustomFunctionFilterGlobals): boolean {
...
/**
 * Shared utility to check if an event matches the filters of a CustomFunction.
...
export async function filterFunctionInstrumented(options: {
    fn: CustomFunctionType | CustomFlow
    ...
    const type = 'type' in fn ? fn.type : 'customflow'
    const fnKind = 'type' in fn ? 'CustomFunction' : 'CustomFlow'
    const logs: LogEntry[] = []
    ...
    try {
        ...
        if (filters && Object.keys(filters).length === 1 && 'bytecode' in filters) {
            customFunctionPreFilterCounter.inc({ result: 'bytecode_execution_skipped__no_filters' })
            result.match = true
            return result
        }
        ...
        if (filters?.events?.length && !filters?.actions?.length) {
            ...
            if (preFilterMatch === false) {
                customFunctionPreFilterCounter.inc({ result: 'bytecode_execution_skipped__pre_filtered_out' })
                result.match = false
                metrics.push({
                    team_id: fn.team_id,
                    app_source_id: fn.id,
        ...
        if (execScriptOutcome) {
            customFunctionFilterDuration.observe({ type }, execScriptOutcome.durationMs)
        }

        if (execScriptOutcome.durationMs > HOG_FILTERING_TIMEOUT_MS) {
        ...
        customFunctionFilterOutcomes.inc({
        ...
        logs.push({
            team_id: fn.team_id,
            log_source: fnKind === 'CustomFunction' ? 'custom_function' : 'custom_flow',
            log_source_id: fn.id,
