...
from insights.insightsql.printer.mysql_functions import (
...
# CAST target mapping: InsightsQL/Datastore/Postgres-flavored type names → MySQL CAST types.
...
class MySQLPrinter(PostgresPrinter):
    """Prints a InsightsQL AST as MySQL 8 SQL.
    ...
    DIALECT_NAME: ClassVar[InsightsQLDialect] = "mysql"
    ...
    def _print_table(self, table) -> str:
        ...
        if hasattr(table, "to_printed_mysql"):
            return table.to_printed_mysql(self.context)
        # Same fallback as the Postgres printer: non-direct tables print their Datastore
    ...
    def _get_compare_op(self, op: ast.CompareOperationOp, left: str, right: str) -> str:
        ...
        # pin case sensitivity to match Datastore semantics regardless of collation.
