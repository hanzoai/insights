...
const NoMetricsPrompt = ({ className }: { className?: string }): JSX.Element | null => {
    ...
    useEffect(() => {
        insights.capture('metrics setup prompt viewed')
    }, [])
    ...
    const onDocsLinkClick = (docsType: string): void => {
        insights.capture('metrics onboarding docs clicked', { docs_type: docsType })
        addProductIntent({
    ...
    const helmSnippet = [
        'helm install insights-metrics-agent \\',
        '  oci://ghcr.io/insights/charts/insights-metrics-agent \\',
        `  --set insights.apiKey=${apiKey} \\`,
        `  --set insights.host=${apiHostOrigin()}`,
        '',
    ...
    return (
        <ProductIntroduction
            ...
            description="Insights metrics works with any OpenTelemetry-compatible client. You don't need any Insights-specific packages – just use standard OpenTelemetry libraries to send metrics via OTLP."
            isEmpty={true}
            ...
            actionElementOverride={
                <div className="flex flex-col items-start gap-4">
                    <p className="text-sm text-secondary m-0">
                        Read our{' '}
                        <Link to="https://hanzo.ai/docs/metrics" onClick={() => onDocsLinkClick('Metrics')}>
                            metrics docs
                        </Link>{' '}
                        or learn more about{' '}
                        <Link
                    ...
                    <div className="flex flex-col gap-1 w-full max-w-160">
                        ...
                        <p className="text-sm text-secondary m-0">
                            Already exposing Prometheus metrics? Deploy the Insights metrics agent in your infra and it
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Button
                            type="secondary"
                            size="small"
                            to="https://hanzo.ai/docs/metrics"
