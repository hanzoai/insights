"""Tests for Insights-specific insightscli commands and hooks."""
...
from insightscli_commands.telemetry_props import (
    _AGENT_ENV_MARKERS,
    _DEVBOX_ENV_MARKERS,
    _detect_agent,
    _detect_environment,
...
class TestProcessManagerInference:
    @pytest.mark.parametrize(
        ...
        [
            (None, ["insightscli", "start"], "phrocs"),
            (None, ["insightscli", "start", "--mprocs"], "mprocs"),
            ("/usr/local/bin/phrocs", ["insightscli", "start", "--mprocs"], "phrocs"),
        ],
...
class TestDetectAgent:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"POSTFN_CODE_VERSION": "1.2.3"}, "insights-code"),
            ({"POSTFN_CODE_VERSION": "1.2.3", "CLAUDECODE": "1"}, "insights-code"),
...
class TestRepoCommitProperties:
    ...
    def test_returns_empty_when_git_fails(self) -> None:
        with patch("insightscli_commands.telemetry_props.subprocess.run") as mock_run:
            mock_run.return_value.returncode = 128
            assert _repo_commit_properties() == {}


class TestIsInsightsDev:
    @pytest.fixture()
    def _config_dir(self, tmp_path, monkeypatch):
        config_file = tmp_path / "insightscli_telemetry.json"
        monkeypatch.setattr("insightscli.telemetry.get_config_path", lambda: config_file)
        return config_file
    ...
    def test_fresh_cache_skips_api(self, _config_dir):
        self._write_config(_config_dir, is_insights_org_member=True, org_check_timestamp=time.time())
        assert _is_insights_dev() is True

    @patch("insightscli_commands.telemetry_props._check_github_org_membership", return_value=True)
    def test_cache_miss_calls_gh_and_persists(self, _mock_gh, _config_dir):
        assert _is_insights_dev() is True
        assert json.loads(_config_dir.read_text())["is_insights_org_member"] is True

    @patch("insightscli_commands.telemetry_props._check_email_domain", return_value=True)
    @patch("insightscli_commands.telemetry_props._check_github_org_membership", return_value=None)
