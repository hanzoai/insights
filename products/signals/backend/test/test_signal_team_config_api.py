...
class TestSignalTeamConfigAPI(APIBaseTest):
    ...
    def test_post_config_lazily_creates_when_no_config_exists(self):
        ...
        response = self.client.post(
            self._url(),
            data={"default_slack_notification_channel": "C123|#insights-signals"},
            format="json",
        )
        data = response.json()
        assert response.status_code == status.HTTP_200_OK, data
        assert data["default_slack_notification_channel"] == "C123|#insights-signals"
        config = SignalTeamConfig.objects.get(team=self.team)
        assert config.default_slack_notification_channel == "C123|#insights-signals"

    @parameterized.expand(
        [
            ("set", None, "C123|#insights-signals", "C123|#insights-signals"),
            ("clear", "C123|#insights-signals", None, None),
        ]
    )
    ...
    def test_partial_update_preserves_default_autostart_priority(self):
        ...
        assert self.config.default_slack_notification_channel == "C123|#insights-signals"
