...
// InsightsConfig represents the _insights section embedded in generated mprocs configs.
type InsightsConfig struct {
	Intents      []string `yaml:"intents"`
	ExcludeUnits []string `yaml:"exclude_units"`
}

// LoadInsightsConfig reads a generated mprocs.yaml and extracts the _insights section.
// Returns nil (no error) if the section is absent.
func LoadInsightsConfig(configPath string) (*InsightsConfig, error) {
	data, err := os.ReadFile(configPath)
	if err != nil {
		return nil, err
	}
	var wrapper struct {
		Insights *InsightsConfig `yaml:"_insights"`
	}
	if err := yaml.Unmarshal(data, &wrapper); err != nil {
		return nil, err
	}
	return wrapper.Insights, nil
}
