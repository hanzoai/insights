...
class TestReapOrphanedDuckgresJobs(BaseTest):
    ...
    def test_reaps_only_stale_running_duckgres_jobs(self):
        ...
        datastore = self._make_job(
            engine=DataModelingJobEngine.DATASTORE, status=DataModelingJobStatus.RUNNING, age_minutes=8 * 60
        )
        ...
        # a possibly-retrying duckgres job, the datastore path, and terminal jobs are untouched
        assert within_retry_window.status == DataModelingJobStatus.RUNNING
        assert datastore.status == DataModelingJobStatus.RUNNING
