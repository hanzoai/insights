...
export function coerceTemplateValueForDisplay(value: unknown, templating: 'script' | 'liquid' | false): string {
    ...
    if (templating === 'script' && (typeof value === 'boolean' || typeof value === 'number')) {
        // A single-expression script template evaluates to the raw value, so the runtime type is
...
// Keyed by the full CyclotronJobInputSchemaType['type'] union — the schema editor's Select
...
function JsonConfigField(props: {
    ...
    const templatingKind = props.input.templating ?? 'script'
...
function CyclotronJobTemplateInput(props: {
    ...
    const templating = props.input.templating ?? 'script'
...
function SearchableChoiceCombobox({
    ...
    // Mirrors quill's `InputInsidePopup` combobox story: Button as the trigger,
