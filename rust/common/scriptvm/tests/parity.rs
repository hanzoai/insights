//! Correctness parity harness — the engine of the "ScriptVM in Rust" loop.
//!
//! For every program in the shared reference corpus (`common/scriptvm/__tests__/*.script`) it:
...
//! `HOGVM_CORPUS_DIR=/abs/path/to/common/scriptvm/__tests__`; otherwise it walks up from the
//! manifest dir looking for `common/scriptvm/__tests__`.
...
fn corpus_dir() -> Option<PathBuf> {
    ...
    // Walk up from the crate manifest dir looking for common/scriptvm/__tests__
    let mut cur = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    loop {
        let candidate = cur.join("common/scriptvm/__tests__");
        if candidate.is_dir() {
...
fn parity_report() {
    let Some(corpus) = corpus_dir() else {
        panic!("Could not locate common/scriptvm/__tests__ — set HOGVM_CORPUS_DIR");
    };
    ...
    let mut programs: Vec<String> = std::fs::read_dir(&corpus)
        ...
        .filter(|n| n.ends_with(".script"))
        .map(|n| n.trim_end_matches(".script").to_string())
        .collect();
    programs.sort();
    ...
    println!("\n================ ScriptVM Rust↔Node parity report ================");
    println!(
