"""Resolves a team's quota state via the Insights API quota_limits endpoint.
...
values in the product registry — e.g. ``ai_credits``, ``insights_code_credits``).
...
# (the billing product config for AI credits and `insights_code_usage`).
...
class QuotaResolver:
    ...
    async def _fetch(self, resource_key: str, team_id: int, auth_header: str) -> tuple[QuotaResourceStatus, int]:
        ...
        if not settings.insights_api_base_url:
            return QuotaResourceStatus(limited=False), _FAIL_OPEN_CACHE_TTL_SECONDS

        url = f"{settings.insights_api_base_url.rstrip('/')}/api/projects/{team_id}/quota_limits/"
        try:
