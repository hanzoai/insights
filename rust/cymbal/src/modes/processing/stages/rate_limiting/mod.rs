...
impl Stage for RateLimitingStage {
    ...
    async fn process(self, batch: Batch<Self::Input>) -> StageResult<Self> {
        ...
        let settings = self
            .ctx
            .team_manager
            .get_rate_limit_settings(&self.ctx.insights_pool, team_ids)
            .await;
    ...
    async fn apply_bypass(
        ...
        for team_id in eligible_teams {
            match self
                .ctx
                .team_manager
                .get_bypass_rules(&self.ctx.insights_pool, team_id)
                .await
            {
        ...
        for entry in to_disable {
            if let Err(e) = entry
                .rule
                .disable(&self.ctx.insights_pool, entry.error, entry.props_json)
                .await
            {
...
/// disabling) so it can be unit-tested without a database or the ScriptVM. Returns the bypassed event
            ...
            match match_fn(rule, &props_json) {
                ...
                Err(VmError::OutOfResource(resource)) if resource == "steps" => {
                    warn!(
                        ...
                        "bypass rule exceeded ScriptVM step budget for this event, skipping"
                    );
                    continue;
                }
