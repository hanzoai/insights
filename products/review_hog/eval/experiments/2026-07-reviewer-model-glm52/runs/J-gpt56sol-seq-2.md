...
- **Validator:** - **Checked:** Read `find_task_run` (`products/tasks/backend/webhooks.py:29-98`), `find_signal_implementation_run` (`products/tasks/backend/facade/api.py:484-517`), the carve-out call site (`products/stamp/backend/tasks/tasks.py:191`), and the run-provenance invariants in `products/review_hog/backend/receivers.py`.
...
### [✅ VALID] must_fix · bug — products/review_hog/backend/migrations/0019_reviewusersettings_stamp_review_inbox_prs.py:7-9
...
- **Found:** `ReviewUserSettings` is in the `review` app (`products/review_hog/backend/apps.py:6`), which is NOT listed in `products/db_routing.yaml` (only `stamp`, `visual_review`, `warehouse_sources_queue` are). So `ProductDBRouter.db_for_read`/`db_for_write` return `None` for it — no reader/writer split.
- **Found:** `ReplicaRouter.db_for_read` (`insights/dbrouter.py:22-31`) sends reads to `"replica"` only when the model name is in `READ_REPLICA_OPT_IN`, else `"default"`. `READ_REPLICA_OPT_IN` is env-driven and empty by default (`insights/settings/data_stores.py:271-272`), and nothing opts `ReviewUserSettings` in. `load()` (`products/review_hog/backend/models.py:363-367`) issues a plain `.for_team(...).filter(...).first()` with no `.using()`, so it already reads from `default` (the writer/primary) — a strongly-consistent read.
