...
description: >
  Author the React + Quill implementation of a Insights canvas: the single-component contract, the
  allowed imports, Quill (Insights's design system) component and composition rules, theme-aware
  ...
  canvas that should look native to Insights.
---
...
Import only from: `react`, `react-dom`, `react-dom/client`, `@hanzo/quill`, `recharts`,
`lucide-react`, `dayjs`. Anything else — including dynamic `import()`, `require()`, `fetch()`,
`<script>` tags, or remote code — fails validation. Use `@hanzo/quill` for UI, `recharts` for
charts, `lucide-react` for icons, `dayjs` for dates.

## Quill component rules

A Insights data board must be built entirely from `@hanzo/quill` components — never a native
...
- The canvas follows the user's Insights theme; a `.dark` class on the document root flips at runtime.
