...
class TestOrganizationInvitesAPI(APIBaseTest):
    ...
    def test_member_cannot_list_invites_for_higher_levels(self):
        ...
        member_invite = OrganizationInvite.objects.create(
            organization=self.organization,
            target_email="peer@hanzo.ai",
            level=OrganizationMembership.Level.MEMBER,
        )
        admin_invite = OrganizationInvite.objects.create(
            organization=self.organization,
            target_email="future-admin@hanzo.ai",
    ...
    def test_admin_can_list_invites_for_all_levels(self):
        ...
        admin_invite = OrganizationInvite.objects.create(
            organization=self.organization,
            target_email="another-admin@hanzo.ai",
            level=OrganizationMembership.Level.ADMIN,
        )
    ...
    @patch("insights.api.organization_invite.send_invite")
    def test_invite_api_key_scope_enforcement(self, scopes, http_method, expected_status, error_scope, _):
        ...
        if http_method == "post":
            response = self.client.post(
                url,
                {"target_email": "scope-test-invite@hanzo.ai"},
    ...
    def test_invite_delete_api_key_scope_enforcement(self, _):
        invite = OrganizationInvite.objects.create(target_email="todelete@hanzo.ai", organization=self.organization)
        self.organization_membership.level = OrganizationMembership.Level.ADMIN
        self.organization_membership.save()
...
class TestOnboardingSkipAPI(APIBaseTest):
    ...
    def test_user_patch_cannot_set_onboarding_skipped_reason(self):
        ...
        # be a way around those guarantees.
    ...
    def test_partial_index_exists_and_is_valid(self):
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE c.relname = 'insights_user_onboarding_delegated_to_invite_id_idx'
                """
            )
            row = cursor.fetchone()
...
@patch("insights.permissions.TimeSensitiveActionPermission.has_permission", return_value=True)
...
class TestOrganizationInviteRateLimits(APIBaseTest):
    ...
    def _payload(self, count: int, seed: str = "burst") -> list[dict]:
        return [{"target_email": f"test+{seed}_{i}@hanzo.ai"} for i in range(count)]

    @patch("insights.rate_limit.OrganizationInviteBurstThrottle.rate", new="3/hour")
    ...
    @patch("insights.rate_limit.OrganizationInviteBurstThrottle.rate", new="10/hour")
    ...
    @patch("insights.rate_limit.OrganizationInviteBurstThrottle.rate", new="5/hour")
    ...
    @patch("insights.rate_limit.OrganizationInviteBurstThrottle.rate", new="2/hour")
    ...
    @patch("insights.rate_limit.OrganizationInviteBurstThrottle.rate", new="1000/hour")
    @patch("insights.rate_limit.OrganizationInviteSustainedThrottle.rate", new="3/day")
