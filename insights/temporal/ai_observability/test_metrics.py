...
class TestExecutionTimeRecorder:
    def test_records_completed_status_on_success(self):
        ...
        with patch("insights.temporal.common.metrics.get_metric_meter", return_value=mock_meter) as mock_get:
    ...
    def test_forwards_duration_and_terminal_attributes_to_recorder(self):
        ...
        with (
            patch("insights.temporal.common.metrics.get_metric_meter", return_value=mock_meter),
            patch("insights.temporal.common.metrics.time.perf_counter", side_effect=[1.0, 1.25]),
        ):
