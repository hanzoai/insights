...
- [ ] **4. `ROOT_FONT_SIZE: 14 → 16` is a silent breaking change**
      ...
      verifying: - Does `apps/storybook` set `html { font-size: 14px }`? - Does `apps/web` (the main Insights frontend) rely on the old values? - Do any stories visually regress in light + dark?
...
- [x] **7. Pre-compile Tailwind in the library build** ⭐ DONE
      A new `@hanzo/quill` aggregate package at
...
- [x] **9. Collapse to one published package: `@hanzo/quill`** ✅
...
'@hanzo/quill'`. The old `packages/quill/package.json` umbrella
      was renamed from `@hanzo/quill` to `@hanzo/quill-workspace`
      (private) so the name is free for the aggregate. The
      `@hanzo/quill-primitives`, `@hanzo/quill-components`, and
      `@hanzo/quill-blocks` packages are now `private: true` workspace
      members that ship nowhere. `@hanzo/quill-tokens` stays
      ...
      publish only `@hanzo/quill-tokens` and `@hanzo/quill`.

- [x] **10. Drop `shadcn` as a peer dependency** ✅
      Removed from `@hanzo/quill-primitives` peer dependencies and
      declared as a `devDependency` of the `@hanzo/quill` aggregate
...
- [x] **11. Drop `tw-animate-css` as a peer dependency** ✅
      ...
      `@hanzo/quill-primitives` peer dependencies into the aggregate's
...
- [x] **12. Stop shipping `src/` in the tarball** ✅
      `files` on `@hanzo/quill-primitives`, `@hanzo/quill-components`,
      and `@hanzo/quill-blocks` dropped from `["src", "dist"]` to
      `["dist"]`. Those packages are now also `private: true` so they
      are never published anyway; the only package that goes to the
      registry is `@hanzo/quill`, which was configured with
...
- [x] **13. Declare `sideEffects` in every published package** ✅
      `@hanzo/quill` declares `sideEffects: ["*.css"]` so bundlers
      preserve the compiled stylesheet import. The internal
      `@hanzo/quill-primitives`, `@hanzo/quill-components`, and
      `@hanzo/quill-blocks` packages declare `sideEffects: false`
      (pure JS re-exports, no CSS side effects).

- [x] **14. Add `"./package.json": "./package.json"` to exports** ✅
      Added to `@hanzo/quill`, `@hanzo/quill-primitives`,
      `@hanzo/quill-components`, and `@hanzo/quill-blocks`.
...
- [x] **16. Add an `engines` field** ✅
      Added `"engines": { "node": ">=20" }` to the `@hanzo/quill`
...
- [ ] **17. The README has five different ways to import CSS**
      ...
      (`@import 'tailwindcss';` followed by `@import '@hanzo/quill';`).
...
- [ ] **18. Contrast tests at the token level**
      ...
      pairs and asserts WCAG AA. Lives in `@hanzo/quill-tokens`. Ten
...
- [ ] **21. Pack-and-install smoke test**
      Run `pnpm --filter @hanzo/quill pack`, install the resulting
...
3. **#21** (pack-and-install smoke test) — protects the new
   `@hanzo/quill` aggregate against future regressions.
