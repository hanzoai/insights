...
use personinsights_common::properties::rewrite_out_of_range_numbers;
...
pub async fn load_person_from_pg(
    ...
    histogram!("personinsights_leader_pg_fallback_duration_ms")
        .record(start.elapsed().as_secs_f64() * 1000.0);

    let Some(row) = row else {
        counter!("personinsights_leader_pg_fallback_total", "outcome" => "not_found").increment(1);
        return Ok(None);
    };
    ...
    let properties = match serde_json::from_str(properties_text) {
        ...
        Err(strict_err) => {
            match serde_json::from_str(&rewrite_out_of_range_numbers(properties_text)) {
                Ok(value) => {
                    counter!("personinsights_leader_pg_properties_rewritten_total").increment(1);
                    tracing::warn!(
                        team_id,
                        person_id = id,
    ...
    counter!("personinsights_leader_pg_fallback_total", "outcome" => "found").increment(1);
