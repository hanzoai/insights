...
description: >
  ...
  queue. To *use* an existing catalog to answer a business-number question, see querying-insights-data
...
by checking for a canonical metric before deriving one — see the `querying-insights-data` skill.
...
1. **Certify the sources.** Survey the most-queried warehouse tables/views. For the ones the team
   clearly relies on, `insights:data-catalog-certification-propose` them (the tool's default
...
2. **Discover joins with evidence.** For plausible table pairs, sample both sides with
   `insights:execute-sql` to measure the match rate of a candidate key (e.g. `count(DISTINCT a.key)`
   present in `b.key`). Only `insights:data-catalog-relationship-propose` a join backed by a real match
...
3. **Seed metrics from insights.** Mine the project's most-used insights (query `system.insights`),
   and for the load-bearing ones create metrics from them with `insights:data-catalog-metric-create`
...
3. **On the human's instruction**, promote with the confirmed-action tools:
   `insights:data-catalog-metric-approve`, `insights:data-catalog-certification-certify` / `-deprecate`,
   `insights:data-catalog-relationship-accept` / `-reject` (pass the `id` from the queue). A row proposed
...
4. **Handle drift.** A metric with `is_drifted = true` has diverged from its source insight (or the
   ...
   - re-snapshotting the insight's current query with `insights:data-catalog-metrics-refresh-from-insight-create`
   ...
   The `refresh` parameter on `insights:data-catalog-metric-run` is a query-cache mode, not a drift fix —
