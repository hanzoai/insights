...
pub struct KafkaMetricRow {
    ...
    /// Datastore (which never recomputes it). Links a sample to its series at read
...
///
/// Computed once here and carried to Datastore as `series_fingerprint`, so storage
...
/// Datastore hash; any deterministic algorithm works. SipHash-1-3 with a fixed key is
/// stable across builds. Returned as i64 (the u64 bit pattern) to fit Avro's `long`;
/// Datastore reinterprets it back to UInt64.
///
