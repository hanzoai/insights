...
class SimilarFingerprint(BaseModel):
    fingerprint: str
    timestamp: datetime
    # Distance to the average embedding of the target issue's fingerprints
    distance: float
...
class IssueWithSimilarFingerprints(BaseModel):
    ...
    @property
    def first_fingerprint(self) -> SimilarFingerprint | None:
        if len(self.fingerprints) > 0:
            return min(self.fingerprints, key=lambda f: f.timestamp)
        return None

    @property
    def closest_fingerprint(self) -> SimilarFingerprint | None:
        if len(self.fingerprints) > 0:
...
class ErrorTrackingSimilarIssuesQueryRunner(AnalyticsQueryRunner[ErrorTrackingQueryResponse]):
    ...
    def _calculate(self):
        res: tuple[list[SimilarFingerprint], InsightsQLQueryResponse] = self.get_similar_fingerprints()
    ...
    def enrich_issues(self, similar_issues: list[IssueWithSimilarFingerprints]):
        ...
        with self.timings.measure("error_tracking_first_event_fetching"):
            ...
            results: InsightsQLQueryResponse = execute_insightsql_query(query, team=self.team)
            for row in results.results:
                if row[0] in issues_by_fingerprint:
                    issues_by_fingerprint[row[0]].set_library(row[1])

    def get_similar_fingerprints(
        self,
    ) -> tuple[list[SimilarFingerprint], InsightsQLQueryResponse]:
        with self.timings.measure("error_tracking_similar_issues_insightsql_execute"):
    ...
    def to_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        ...
        result = execute_insightsql_query(avg_embedding_query, team=self.team)
    ...
    def query_template(self):
        return """
        SELECT fingerprint, timestamp, distance
        FROM (
            SELECT
