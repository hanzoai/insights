# Insights numbers don't match the user's SQL / raw count
...
- D11 — Metric definition traps (empty event filter, InsightsQL count(boolean))

## D1 — Scope mismatch checklist (the eight sources) [HIGH]

When the user reports "Insights says X, my SQL says Y", walk this checklist:
...
8. **Winsorization (outlier clamping) on mean metrics.** Mean metrics support a percentile-clamp
   ...
   insights/insightsql_queries/experiments/experiment_query_builder.py -->
...
### Canonical scope-reproducing InsightsQL skeleton
...
**Implication:** comparing Insights's funnel conversion rate to a SQL query that counts intermediate
...
exposed users. Insights instead returns the **mean of per-user totals** — for each exposed user, sum
their revenue events, then average across users in the variant.

**Worked example:** user A spent $50, user B spent $10. Insights reports `($50 + $10) / 2 = $30`, not
...
- **Mean / ratio metrics.** Events count when
  ...
  insights/insightsql_queries/experiments/experiment_query_builder.py (mean/ratio branch).
...
- **Funnel metrics.** The conversion window is enforced _between consecutive funnel steps_ by the
  `aggregate_funnel_array` Datastore UDF — not as a single window from first exposure. Each new
...
- Funnel reconciliation: compute step-to-step gaps, not a single window from first exposure. A user
  ...
  Insights's numbers will look larger than yours.
...
the cache. If pre-aggregation is enabled and a precomputation insert fails, Insights falls back to a
...
- A **dynamic cohort** for "currently paid" users, and target the experiment to that cohort via
  release conditions.
- A **InsightsQL expression** in the metric filter that joins person properties at query time, accepting
...
## D11 — Metric definition traps (empty event filter, InsightsQL count(boolean)) [HIGH]
...
**`count(boolean_expression)` counts non-null, not true.** A InsightsQL `math_insightsql` of the form
...
**Verify directly.** Inspect the rendered `datastore_sql` field from `experiment-results-get` —
...
**Validation signals from Insights.** A `validation_failures` entry of `"baseline-mean-is-zero"` on
...
- For InsightsQL math: pick `countIf(...)` or `sum(toInt(...))` over `count(...)` of a boolean.
