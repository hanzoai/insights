...
type ManagementMode = 'insights' | 'self_managed'
...
function getCdcConfig(source: ExternalDataSource): {
    ...
    return {
        ...
        management_mode: (ji.cdc_management_mode === 'self_managed' ? 'self_managed' : 'insights') as ManagementMode,
...
// Self-managed CDC setup SQL the customer's DBA runs before Insights creates the slot.
...
function buildSelfManagedCdcSql(source: ExternalDataSource, publicationName: string): string {
    ...
    const pubName = publicationName.trim() || 'insights_pub'
    ...
    return `-- 1. Grants for the Insights user
...
--    tables you intend to switch to CDC on the Schemas tab. Insights creates and manages
    ...
    return (
        <div className="mt-6 rounded border p-4">
            <div className="flex items-center gap-2 mb-1">
                ...
                <Tag type="completion">Alpha</Tag>
                {cdc.enabled && <Tag type="success">Enabled</Tag>}
            </div>
        ...
        confirmThen({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    toast.success('CDC settings updated')
                    loadSource()
                } catch (e: any) {
                    toast.error(e?.message ?? "Couldn't update CDC settings")
                } finally {
                    setBusy(false)
                }
            },
        ...
        confirmThen({
            ...
            description: (
                <div className="space-y-2">
                    <p className="m-0">
                        Insights will recreate the replication resources on your database and fully re-sync every CDC
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    toast.success('CDC repaired — schemas are re-syncing')
                    loadSource()
                    loadCdcStatus()
                } catch (e: any) {
                    toast.error(e?.message ?? "Couldn't repair CDC")
                } finally {
                    setBusy(false)
                }
            },
        ...
        confirmThen({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    toast.success('CDC resumed')
                    loadSource()
                    loadCdcStatus()
                } catch (e: any) {
                    toast.error(e?.message ?? "Couldn't resume CDC")
                } finally {
                    setBusy(false)
                }
            },
        ...
        confirmThen({
            ...
            description: (
                <div className="space-y-2">
                    ...
                    <ul className="list-disc ml-5 m-0 text-sm">
                        <li>
                            Drop the replication slot{cdc.management_mode === 'insights' && ' and publication'} on your
                            source database.
                        </li>
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    toast.success('CDC disabled')
                    loadSource()
                } catch (e: any) {
                    toast.error(e?.message ?? "Couldn't disable CDC")
                } finally {
                    setBusy(false)
                }
            },
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <div>{cdc.management_mode === 'insights' ? 'Insights-managed' : 'Self-managed'}</div>
                </div>
                <div>
            ...
            <div>
                <div className="flex items-center justify-between mb-2">
                    ...
                    <Button size="xsmall" type="secondary" onClick={loadCdcStatus} loading={statusLoading}>
                        Refresh
                    </Button>
                </div>
                {statusLoading && !status ? (
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                            <div className="text-secondary text-xs">Replication slot</div>
                            <Tag type={status.slot_exists ? 'success' : 'danger'}>
                                {status.slot_exists ? 'Active' : 'Missing'}
                            </Tag>
                        </div>
                        <div>
                            <div className="text-secondary text-xs">Publication</div>
                            <Tag type={status.publication_exists ? 'success' : 'danger'}>
                        ...
                        <div className="space-y-2">
                            <p className="m-0">
                                {status.slot_exists === false
                                    ...
                                    : 'The publication is missing on your database — recreate it (self-managed) or repair CDC (Insights-managed).'}
                            </p>
                            {(status.slot_exists === false || cdc.management_mode === 'insights') && (
                                <AccessControlAction
                                    resourceType={AccessControlResourceType.ExternalDataSource}
                                    minAccessLevel={AccessControlLevel.Editor}
                                    userAccessLevel={source.user_access_level}
                                >
                                    <Button type="secondary" size="small" onClick={onRepair} loading={busy}>
                                        Repair CDC
                                    </Button>
                                </AccessControlAction>
                            )}
                    ...
                    <Banner type="warning" className="mt-2">
                        <div className="space-y-2">
                            ...
                            <AccessControlAction
                                ...
                                userAccessLevel={source.user_access_level}
                            >
                                <Button type="secondary" size="small" onClick={onResume} loading={busy}>
                                    Resume CDC
                                </Button>
                            </AccessControlAction>
                        </div>
            ...
            <div>
                <Field.Pure label="Automatic slot protection" htmlFor="cdc-auto-drop-slot">
                    <Switch id="cdc-auto-drop-slot" checked={autoDrop} onChange={setAutoDrop} />
                </Field.Pure>
                <p className="text-xs text-secondary mt-1 mb-0">
                    When enabled, Insights drops the replication slot if WAL lag exceeds the critical threshold —
                    preventing disk exhaustion on your database.
                </p>
            </div>

            {autoDrop && cdc.management_mode === 'insights' && (
            ...
            {validationError && <Banner type="error">{validationError}</Banner>}
            ...
            <div className="flex justify-between items-center">
                <AccessControlAction
                    ...
                    userAccessLevel={source.user_access_level}
                >
                    <Button type="secondary" status="danger" onClick={onDisable} loading={busy}>
                        Disable CDC
                    </Button>
                </AccessControlAction>
                <AccessControlAction
...
function DisabledControls({ source }: { source: ExternalDataSource }): JSX.Element {
    ...
    const [mode, setMode] = useState<ManagementMode>('insights')
    ...
    const doEnable = async (): Promise<void> => {
        ...
        try {
            ...
            if (Array.isArray(errs) && errs.length > 0) {
                ...
                toast.error(e?.message ?? "Couldn't enable CDC")
            }
        } finally {
            setEnabling(false)
        }
    ...
    const onCheckPrereqs = async (): Promise<void> => {
        ...
        try {
            ...
            toast.error(e?.message ?? "Couldn't check prerequisites")
        } finally {
            setChecking(false)
        }
    }
    ...
    const onEnable = (): void => {
        ...
        // Self-managed: the publication must exist before Insights can create the slot. Walk the
        // user through the setup SQL first, then enable (which verifies it server-side).
        if (mode === 'self_managed') {
            if (!publicationName.trim()) {
                toast.error('Enter a publication name first.')
                return
            }
            setModalErrors(null)
        ...
        // Insights-managed: Insights creates the slot + publication itself, no SQL to run.
        confirmThen({
            title: 'Enable CDC',
            description: (
                <div className="space-y-2">
    ...
    return (
        <>
            <div className="space-y-4">
                <Field.Pure label="Slot management">
                    <Radio
                        value={mode}
                        onChange={(v) => {
                            setMode(v)
                            ...
                            },
                            {
                                ...
                                label: (
                                    <div>
                                        ...
                                        <div className="text-xs text-secondary">
                                            You (or your DBA) create the publication once as the table owner. Insights
                ...
                {mode === 'self_managed' && (
                    <Field.Pure label="Publication name">
                        <Input
                            ...
                            placeholder="insights_pub"
                        />
                    </Field.Pure>
                )}

                <div>
                    <Field.Pure label="Automatic slot protection" htmlFor="cdc-auto-drop-slot-new">
                        <Switch id="cdc-auto-drop-slot-new" checked={autoDrop} onChange={setAutoDrop} />
                    </Field.Pure>
                    <p className="text-xs text-secondary mt-1 mb-0">
                        Insights will drop the slot if WAL lag exceeds the critical threshold.
                    </p>
                </div>
                ...
                {thresholdsInvalid && (
                    <Banner type="error">Warning threshold must be less than critical threshold.</Banner>
                )}

                <div className="flex flex-col gap-2">
                    <Button type="secondary" onClick={onCheckPrereqs} loading={checking}>
                        Check database prerequisites
                    </Button>
                    {prereqResult && (
                        <Banner type={prereqResult.valid ? 'success' : 'error'}>
                            {prereqResult.valid ? (
                            ...
                            disabledReason={
                                thresholdsInvalid
                                    ...
                                    : // Self-managed verifies in the modal, so a failed check must not block it.
                                      mode === 'insights' && prereqResult && !prereqResult.valid
            ...
            <Modal
                ...
                description="Self-managed CDC needs the publication to exist before Insights connects — Insights creates and manages the replication slot itself. Run the SQL below as the table owner, then enable CDC."
                width={720}
                footer={
                    <>
                        <Button
