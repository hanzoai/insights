...
Non-goals: NO pipeline changes, NO eval runs, NO harness (Insights Desktop repo) changes, NO builds of #4/#8/#10. Zero prod risk by construction.
...
Part 1 recomputes the archived 07-03 eval arms from local Datastore `$ai_generation` events.
...
Instrument: `products/review/eval/scripts/dump_result.py` (currently sums `$ai_input_tokens`/`$ai_output_tokens` with no cache
split, lines ~50-61; OUT_DIR-overridable; queries local Datastore over `$ai_generation`).
...
- flox env; local Datastore reachable and holding `$ai_generation` for the archived eval windows (check FIRST — see urgency note).
...
- No dev stack needed beyond Datastore; no sandboxes except arm 5 (one throwaway task or a shell into an existing sandbox image).
...
5. recommendation for the next rounds, for the user to greenlight: Round 1 default per CANDIDATES.md = #4 skill-body splice +
   ...
   (2026-07-06): experiments run on a locally patched Insights Desktop checkout (`/Users/woutut/Documents/Code/code`);
