...
// `textColor` mirrors the Tag text color for each `type` (see Tag.scss; for these outlined
...
const ACTIONABILITY_STYLE: Record<
    ...
    { type: TagType; label: string; tooltip: string; textColor: string }
> = {
...
export function SignalReportActionabilityBadge({
    ...
    // Tag to Base UI as the trigger. When a rationale exists the help icon overlays the (rounded)
    ...
    return (
        <Tooltip title={hasWhy ? explanation : style.tooltip}>
            <span className="relative inline-flex cursor-help">
                <Tag size="small" type={style.type} className="select-none">
                    {style.label}
                </Tag>
                {hasWhy && (
                    <IconQuestion
