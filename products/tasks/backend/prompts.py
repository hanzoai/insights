...
WIZARD_HEAD_BRANCH_PREFIX = "insights/instrumentation-"
...
Insights's setup wizard has already run in this repository and integrated Insights. The working tree
contains its uncommitted changes: modified source files, an updated package manifest, installed
dependencies, a `insights-setup-report.md` summary. It'll also possibly contain a
`.insights-events.json` plan and some skills definitions under a harness skills folder such as
...
You are NOT here to integrate Insights or write any product/instrumentation code. Your job is to
...
- Do not add, remove, edit, or "improve" the Insights setup the wizard produced.
- Do not create new Insights events, dashboards, or insights.
...
- Whenever you mention the pull request in any output, summary, or comment, always hyperlink it to
  ...
  `Opened [#42123](https://github.com/org/repo/pull/42123) with the Insights integration.`
...
1. Create a branch named exactly `{WIZARD_HEAD_BRANCH_PLACEHOLDER}`. This name was
   pre-generated for this run and Insights uses it to link the pull request back to the
...
3. Commit the wizard's changes in that style; the message should resemble the concept of
   "Add Insights to codebase". For example, in a repo using conventional commits:

   ```text
   feat: add Insights analytics integration
   ```

4. Do NOT commit `insights-setup-report.md` or `.insights-events.json` - they are local reference
...
git show --stat HEAD | grep -E 'insights-setup-report|insights-events|wizard-output' && echo "FAIL: reference files committed" || echo "OK: no reference files"
...
2. If you can automatically configure the required Insights environment variables in a file that
...
4. You should know the project API token from the Insights MCP server context. If you don't have
...
Write the PR description FROM the contents of `insights-setup-report.md`:
...
- List EVERY Insights insight and dashboard the wizard created, with their names and links from the
  report. Use a table for the list of insights and dashboards.
- Add a short "How to verify" section explaining how to verify Insights is working.
- Add a section explaining the environment variables situation, based on the Step 3 outcome:
  - If you configured them automatically: explain that Insights will work automatically as soon as
...
This PR adds Insights analytics to the app using the `insights-js` SDK (installed via npm).

- `src/lib/insights.ts` - new Insights client initialization
- `src/App.tsx` - wraps the app in the Insights provider
- `package.json` / `package-lock.json` - adds `insights-js@1.x`

## Insights and dashboards created

| Type      | Name                    | Link                                     |
| --------- | ----------------------- | ---------------------------------------- |
| Dashboard | Product overview        | https://app.hanzo.ai/dashboard/123     |
| Insight   | Signup conversion       | https://app.hanzo.ai/insights/abc123   |
| Insight   | Checkout funnel         | https://app.hanzo.ai/insights/def456   |

## How to verify

1. Deploy this branch (or run it locally after setting the env vars from the section below).
2. Open the app and click around.
3. In Insights, open [Activity](https://app.hanzo.ai/activity/explore) - you should see
   `$pageview` events arriving within a minute.

## Environment variables

Environment variables were configured automatically in `.env.production`, so Insights will start
...
```markdown
This PR adds Insights to the codebase. The setup wizard made some changes to various files to
integrate analytics. Some insights and dashboards were also created in Insights. Let me know if
you have any questions!
```

What makes it bad: no file list, "various files" and "some insights" instead of naming every one
from `insights-setup-report.md`, no table, no links, no "How to verify" section, and it says
...
```markdown
## Environment variables (action needed before Insights works)
...
they let the app know things (like the Insights token below) without hardcoding them in the code.
Insights will not receive any data until these two are set:
