...
class ReviewReport(UUIDModel, TeamScopedRootMixin):
    ...
    # db_constraint=False keeps the migration lock-free on hot insights_team.
    ...
    # recent reviews"; db_constraint=False keeps the migration lock-free on hot insights_user.
...
class ReviewReportArtefact(UUIDModel, TeamScopedRootMixin):
    ...
    # migration lock-free on hot insights_user.
...
class ReviewSkillConfig(UUIDModel, TeamScopedRootMixin):
    ...
    `review-script-validation-*` (the validator) and `review-script-blind-spots-*` (the blind-spot check) —
    ...
    # db_constraint=False keeps the migration lock-free on hot insights_team / insights_user.
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, related_name="+", db_constraint=False)
