...
Datastore over the physical `raw_sessions` table and materialized event columns;
the new path is InsightsQL over `events`+`sessions` driven by `ensure_precomputed`.
