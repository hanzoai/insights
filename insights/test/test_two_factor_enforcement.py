...
from insights.helpers.two_factor_session import (
...
class TestSessionAuthenticationTwoFactor(TestCase):
    def setUp(self):
        ...
        domain_enforcement_patcher = patch("insights.auth.enforce_verified_domain")
    ...
    @patch("insights.helpers.two_factor_session.default_device")
    @patch("insights.helpers.two_factor_session.is_impersonated_session")
    ...
    @patch("insights.helpers.two_factor_session.has_passkeys")
    @patch("insights.helpers.two_factor_session.default_device")
    @patch("insights.helpers.two_factor_session.is_impersonated_session")
    ...
    @patch("insights.helpers.two_factor_session.default_device")
    @patch("insights.helpers.two_factor_session.is_impersonated_session")
    @patch("insights.helpers.two_factor_session.is_two_factor_verified_in_session")
    ...
    @patch("insights.helpers.two_factor_session.is_domain_sso_enforced", return_value=True)
    @patch("insights.helpers.two_factor_session.is_two_factor_enforcement_in_effect", return_value=True)
...
class TestAPIAuthenticationTwoFactorBypass(TestCase):
    ...
    def test_session_authentication_enforces_two_factor(self):
        ...
        with (
            patch("insights.helpers.two_factor_session.is_impersonated_session", return_value=False),
            patch("insights.helpers.two_factor_session.default_device", return_value=None),
            patch("insights.helpers.two_factor_session.has_passkeys", return_value=False),
    ...
    def test_sso_authentication_backend_bypasses_two_factor(self):
        ...
        with (
            ...
            patch("insights.helpers.two_factor_session.default_device", return_value=None),
            patch("insights.helpers.two_factor_session.has_passkeys", return_value=False),
            patch("insights.helpers.two_factor_session.is_sso_authentication_backend", return_value=True),
            patch.object(auth, "enforce_csrf"),
        ):
            result = auth.authenticate(request)
            self.assertEqual(result, (user, None))
    ...
    def test_authentication_allows_when_user_has_passkeys_but_no_totp(
        ...
        with (
            patch("insights.helpers.two_factor_session.is_two_factor_verified_in_session", return_value=True),
            patch.object(auth, "enforce_csrf"),
        ):
            result = auth.authenticate(request)
            self.assertEqual(result, (user, None))
    ...
    def test_authentication_requires_verification_when_user_has_passkeys_but_not_verified(
        ...
        with (
            patch("insights.helpers.two_factor_session.is_two_factor_verified_in_session", return_value=False),
            patch.object(auth, "enforce_csrf"),
        ):
            with self.assertRaises(PermissionDenied) as cm:
                auth.authenticate(request)
...
class TestUserTwoFactorSessionIntegration(TestCase):
    ...
    @patch("insights.rate_limit.TwoFactorThrottle.rate", new="3/minute")
