...
import { DateMappingOption, InsightsFunctionTypeType, PersonType } from '~/types'
...
const tagTypeForStatus = (status: RunStatus): TagProps['type'] => {
    switch (status) {
        case 'succeeded':
            return 'success'
        case 'failed':
...
async function countRerunMatches(
    ...
    const statusClause = params.status?.length
        ? insightsql.raw(`AND status IN (${params.status.map(escapeInsightsQLString).join(',')})`)
        : insightsql.raw('')
...
export interface HogInvocationsProps extends HogInvocationsLogicProps {
     ...
     * matching the standalone Logs tab. Defaults to the script-function renderer.
     */
    ...
    compact?: boolean
    /**
     * The script function's type, for `functionKind === 'insights_function'`. Re-run is only offered
     * for types a cyclotron worker executes (see `RERUNNABLE_FN_FUNCTION_TYPES`); for others
     * the re-run controls are disabled, matching the API's 400. Workflows (script flows) leave
     * this undefined.
     */
    insightsFunctionType?: InsightsFunctionTypeType
}
...
export function HogInvocations({
    ...
    // Script functions whose type a cyclotron worker can't execute can't be re-run — the API
    // rejects it with a 400, so disable the controls here rather than letting a click fail.
    // Script flows always execute on cyclotron, so they leave `insightsFunctionType` undefined.
    const rerunUnsupportedReason: string | undefined =
        functionKind === 'insights_function' && !isRerunnableInsightsFunctionType(insightsFunctionType)
    ...
    const columns: TableColumns<HogInvocationRow> = [
        ...
        },
        {
            ...
            render: (_, row) => (
                <div className="flex items-center gap-1">
                    <Tag type={tagTypeForStatus(row.status)}>{row.status.toUpperCase()}</Tag>
        ...
        },
        {
            ...
            render: (_, row) => {
                if (isRerunWrapperKind(row.function_kind)) {
                    return <Tag type="primary">RERUN</Tag>
                }
        ...
        <div className="flex-1 deprecated-space-y-2 flex flex-col min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 justify-between">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {compact ? null : (
                        <>
                            <Select<'all' | 'invocations' | 'rerun_jobs'>
                                size="small"
            ...
            {selectedCount > 0 ? (
                <div className="flex items-center justify-between border rounded p-2 bg-bg-light">
                    ...
                    <div className="flex items-center gap-2">
                        <Button size="small" type="tertiary" onClick={() => clearSelected()}>
                            Clear
                        </Button>
                        <Button
                            size="small"
            ...
            <Table
                ...
                // `hover:!` modifiers beat Table's default
                ...
                onRow={(record) => ({
                    onClick: (e) => {
                        ...
                        if (
                            target &&
                            target.closest(
                                'a, button, input, label, .Checkbox, [role="button"], [data-attr="expand-row"]'
                            )
                        ) {
                            return
                        }
            ...
            {runs.length > 0 && hasMore ? (
                <div className="flex justify-center py-2">
                    <Button size="small" type="secondary" loading={runsLoading} onClick={() => loadMore(null)}>
                        Load more
                    </Button>
                </div>
            ) : null}
...
function RunDetail({
    ...
    const logsLogicProps: LogsViewerLogicProps = {
        ...
        sourceId: record.parent_run_id || insightsFunctionId,
        ...
        <div className="p-3 deprecated-space-y-3 bg-surface-secondary min-w-0">
            ...
            <div className="border rounded bg-surface-primary p-3">
                <LogsViewer
                    ...
                    defaultAscending
                    renderMessage={renderLogMessage ?? ((message) => renderInsightsFunctionMessage(message))}
                />
            </div>
        </div>
    )
...
function PersonFilterPicker({ id, functionKind }: HogInvocationsLogicProps): JSX.Element {
    ...
    return (
        ...
        >
            <Button type="secondary" size="small" icon={<IconPerson />} onClick={() => setOpen((prev) => !prev)}>
                Filter by person
            </Button>
        </Popover>
    )
