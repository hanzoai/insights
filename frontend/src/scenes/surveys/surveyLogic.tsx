...
export interface surveyLogicValues {
    ...
    reusableSurveyNotifications: InsightsFunctionType[]
...
export interface surveyLogicActions {
    ...
    deleteSurveyNotification: (notification: InsightsFunctionType) => {
...
export interface surveyLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        ...
        answerFilterInsightsQLExpression: (survey: NewSurvey | Survey, answerFilters: EventPropertyFilter[]) => string
        dataTableQuery: (
            survey: NewSurvey | Survey,
            propertyFilters: AnyPropertyFilter[],
            answerFilterInsightsQLExpression: string,
...
export const surveyLogic = kea<surveyLogicType>([
    ...
    actions({
        ...
        deleteSurveyNotification: (notification: InsightsFunctionType) => ({ notification }),
    ...
    loaders(({ props, actions, values }) => ({
        ...
        surveyBaseStats: {
            loadSurveyBaseStats: async (): Promise<SurveyBaseStatsResult> => {
                ...
                const response = await api.queryInsightsQL(query, SURVEY_QUERY_TAGS.baseStats, {
                    queryParams: {
                        filters: {
        ...
        surveyDismissedAndSentCount: {
            loadSurveyDismissedAndSentCount: async (): Promise<DismissedAndSentCountResult> => {
                ...
                const response = await api.queryInsightsQL(query, SURVEY_QUERY_TAGS.dismissedAndSent, {
                    queryParams: {
                        filters: {
        ...
        consolidatedSurveyResults: {
            loadConsolidatedSurveyResults: async (): Promise<ConsolidatedSurveyResults> => {
                ...
                const queryFilters: SurveyQueryFilters = {
                    ...
                    answerFilterInsightsQLExpression: values.answerFilterInsightsQLExpression,
                ...
                const [aggregateResponse, openEndedResponse] = await Promise.all([
                    aggregateQuery
                        ? api
                              .queryInsightsQL(
                                  aggregateQuery as InsightsQLQueryString,
                    ...
                    openEndedResult
                        ? api
                              .queryInsightsQL(
                                  openEndedResult.query as InsightsQLQueryString,
        ...
        reusableSurveyNotifications: [
            [] as InsightsFunctionType[],
            {
                loadReusableSurveyNotifications: async (): Promise<InsightsFunctionType[]> => {
                    if (props.id === NEW_SURVEY.id) {
                        return []
                    }
        ...
        return {
            ...
            loadConsolidatedSurveyResultsSuccess: async ({ consolidatedSurveyResults }) => {
                ...
                try {
                    ...
                    for (let i = 0; i < allIds.length; i += BATCH_SIZE) {
                        ...
                        const response = await api.create(`v1/environments/${teamId}/persons/batch_by_distinct_ids/`, {
                            distinct_ids: batch,
                        })
            ...
            generateTranslationDrafts: async ({ language, overwrite }) => {
                if (values.survey.id === NEW_SURVEY.id) {
                    toast.error('Save the survey before generating translations')
                    return
                }

                const teamId = teamLogic.values.currentTeamId
                if (!teamId) {
                    toast.error('Select a project before generating translations')
                    return
                }
                ...
                try {
                    ...
                    toast.success('Generated translation drafts')
                } catch (error) {
            ...
            deleteSurveyNotification: async ({ notification }) => {
                ...
                await deleteWithUndo({
                    endpoint: `projects/${projectLogic.values.currentProjectId}/insights_functions`,
        ...
        survey: {
            ...
            submit: (surveyPayload) => {
                if (values.hasCycle) {
                    ...
                    toast.error('Your survey contains an endless cycle. Please revisit your branching rules.')
                    return
                }
