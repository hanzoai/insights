...
def _run_job(cluster: DatastoreCluster, **config_overrides: Any) -> tuple[dagster.ExecuteInProcessResult, UUID]:
...
def test_identity_matching_job(cluster: DatastoreCluster) -> None:
...
def test_logreg_skips_without_labels(cluster: DatastoreCluster) -> None:
    cluster.any_host(_insert_fixture_events).result()
...
def test_eval_labels_survive_edge_truncation(cluster: DatastoreCluster) -> None:
    cluster.any_host(_insert_fixture_events).result()
