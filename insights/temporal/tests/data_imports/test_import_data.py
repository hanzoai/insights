...
from insights.tasks.test.test_usage_report import freeze_time
...
async def test_job_inputs_with_whitespace(activity_environment, team, **kwargs):
    ...
    with (
        mock.patch("insights.temporal.data_imports.sources.postgres.source.postgres_source") as mock_postgres_source,
        mock.patch("insights.temporal.data_imports.workflow_activities.import_data_sync._run"),
    ):
...
def test_report_heartbeat_timeout_first_attempt(team):
    ...
    with mock.patch(
        "insights.temporal.data_imports.pipelines.common.extract.activity.info", return_value=mock_info
...
def test_report_heartbeat_timeout_heartbeat_not_within_timeout(team):
    ...
    with freeze_time("2024-01-01 12:00:00"):
        ...
        with (
            mock.patch(
                "insights.temporal.data_imports.pipelines.common.extract.activity.info",
                return_value=mock_info,
            ) as mock_activity_info,
            mock.patch(
                "insights.temporal.data_imports.pipelines.common.extract.posthoganalytics.capture"
