# Quill — Insights's unified design system

A React component library powering Insights's unified UI surfaces (web, MCP, electron), built on Base UI and shadcn primitives. **Quill requires Tailwind v4 on the consumer side** — the library ships component source and theme metadata, and your app's Tailwind compiles the utilities into your own CSS bundle.
...
`@hanzo/quill` is self-contained for styling purposes — tokens are bundled into `dist/tokens.css`, so consumers don't also need `@hanzo/quill-tokens` unless they want the typed JS exports.
...
@import '@hanzo/quill/tokens.css'; /* design tokens (CSS vars + @theme) */
@import '@hanzo/quill/base.css'; /* one border-color reset rule */
@import '@hanzo/quill/tailwind.css'; /* @source directive pointing at quill's dist */
```
...
- `tailwind.css` — a single `@source "./**/*.js"` directive. When your Tailwind v4 processes the import, it resolves that glob **relative to the file's on-disk location inside `node_modules/@hanzo/quill/dist`**, so it scans quill's compiled library JS for class-name strings and generates the matching utilities in your own `utilities` layer. No pre-compiled stylesheet, no cascade-layer fight with your own Tailwind output.
...
@import '@hanzo/quill/tokens.css';
@import '@hanzo/quill/base.css';
@import '@hanzo/quill/tailwind.css';
```
...
1. **It ignores `@source` directives nested inside `@import`ed CSS files.** Add an explicit `@source` pointing at quill's dist next to the imports:
   ...
   ```css
   @source "../../../../node_modules/@hanzo/quill/dist/**/*.js";
   ```

   Adjust the relative path so it resolves from your CSS entry to `node_modules/@hanzo/quill/dist`. Yes, it's fragile — if you can, migrate off `@config` and delete this line.
...
```tsx
import { ThemeProvider } from '@hanzo/quill'
...
```tsx
import { Button, Card, CardHeader, CardTitle, CardContent } from '@hanzo/quill'
...
```tsx
import { buttonVariants } from '@hanzo/quill'
...
```bash
pnpm add @hanzo/quill-tokens
```

```ts
import { semanticColors, spacing, spacingPx, shadow } from '@hanzo/quill-tokens'
...
Quill follows the **shadcn-style build model**: the library ships component source and theme metadata, and the consumer's Tailwind does all the utility compilation. There is no pre-compiled utility stylesheet inside `@hanzo/quill`.

```text
@hanzo/quill-tokens          →  Raw design token values (TS, CSS)
@hanzo/quill dist/index.js   →  Compiled library code — class strings preserved as literals
@hanzo/quill dist/tokens.css →  :root / .dark vars + Tailwind v4 @theme inline block
@hanzo/quill dist/tailwind.css → @source "./**/*.js" — points Tailwind's scanner at the dist
...
Quill lives inside the Insights monorepo. Clone, install dependencies at the repo root, and run:
...
1. `@hanzo/quill-tokens build` — generates the initial token CSS so storybook has something to import at start-up.
2. `@hanzo/quill-storybook storybook` — the dev server.
...
- **`quillTokensWatcher` plugin** watches `packages/tokens/src/*.ts`. Editing a token source file (colors, spacing, typography) reruns `tsx src/build.ts`, regenerates `@hanzo/quill-tokens/dist/*.css`, and triggers a full page reload so the new custom property values are live.
...
packages/quill/
├── packages/
│   ├── tokens/          @hanzo/quill-tokens        (published)
│   ├── primitives/      @hanzo/quill-primitives    (private, bundled into @hanzo/quill)
│   ├── components/      @hanzo/quill-components    (private, bundled into @hanzo/quill)
│   ├── blocks/          @hanzo/quill-blocks        (private, bundled into @hanzo/quill)
│   └── quill/           @hanzo/quill               (published — the aggregate consumers install)
└── apps/
    └── storybook/       @hanzo/quill-storybook     (private, dev tool)
```
