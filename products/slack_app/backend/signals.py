...
def invalidate_repo_list_on_user_github_change(sender: Any, instance: UserIntegration, **kwargs) -> None:
    if instance.kind != UserIntegration.IntegrationKind.GITHUB:
        return
    # Deferred: api.py imports the temporal.ai workflows at module scope.
...
def onboard_slack_inbox_on_install(sender: Any, instance: Integration, created: bool, **kwargs) -> None:
    """Fresh Slack install -> enqueue the #insights-inbox onboarding Temporal workflow on commit (the
...
def _start_inbox_onboarding_workflow(integration_id: int) -> None:
    ...
    from insights.temporal.ai.slack_app.insights_slack_inbox_onboarding import (  # noqa: PLC0415
        InsightsSlackInboxOnboardingWorkflow,
    )
    from insights.temporal.ai.slack_app.types import InsightsSlackInboxOnboardingInputs  # noqa: PLC0415
    from insights.temporal.common.client import sync_connect  # noqa: PLC0415
    ...
    try:
        ...
        asyncio.run(
            client.start_workflow(
                InsightsSlackInboxOnboardingWorkflow.run,
                InsightsSlackInboxOnboardingInputs(integration_id=integration_id),
                id=f"insights-slack-inbox-onboarding-{integration_id}",
