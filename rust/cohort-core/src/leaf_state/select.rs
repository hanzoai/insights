...
/// oracle's `if prop.explicit_datetime` branch in `insightsql_cohort_query.py`). The oracle resolves each
...
///   - `%Y-%m-%d %H:%M:%S%.f` (the space-separated Datastore form),
...
fn absolute_datetime_to_day(raw: &str) -> Option<DayIdx> {
    ...
    // Naive datetime, T-separated (the date picker) or space-separated (Datastore) — date part only.
...
mod tests {
    ...
    fn absolute_explicit_datetime_yields_an_explicit_window() {
        // The space-separated Datastore form and the date picker's T-separated naive form must both
    ...
    fn absolute_datetime_to_day_is_the_literal_date_for_every_accepted_shape() {
        // Bare, T-separated naive (the date picker), space-separated Datastore, and RFC3339-with-offset
