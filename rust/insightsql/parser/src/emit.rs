//! The `Emitter` trait abstracts AST node construction so the parser can emit either `serde_json::Value` (via `JsonEmitter`, used by `parse_*_json` entry points + the future WASM build) or Python `insights.insightsql.ast` dataclass instances directly during parse (via `PyEmitter` in `emit_py.rs`, used by `parse_*_py`).
//!
//! Field names and shapes mirror the dataclasses at [`insights/insightsql/ast.py`]. The Python deserialiser at [`insights/insightsql/json_ast.py`] also builds these dataclasses from the JSON shape, so the two backends produce `__eq__`-equivalent objects.
...
pub trait Emitter {
    ...
    /// Raw list value (JSON array / Python list). Distinct from the `Array(...)` AST node — used for fields whose value is a plain list, e.g. `InsightsQLXAttribute.children`.
    ...
    /// `{positional_ref: index}` — the `$N` form inside Script blocks. Mirrors cpp's `PositionalRef` node.
    ...
    /// `InsightsQLXTag(kind, attributes)`.
    fn insightsqlx_tag(&self, kind: &str, attributes: Vec<Self::Value>) -> Self::Value;
    /// `InsightsQLXAttribute(name, value)`.
    fn insightsqlx_attribute(&self, name: &str, value: Self::Value) -> Self::Value;
...
impl Emitter for JsonEmitter {
    ...
    fn insightsqlx_tag(&self, kind: &str, attributes: Vec<Value>) -> Value {
        json!({"node": "InsightsQLXTag", "kind": kind, "attributes": attributes})
    }
    fn insightsqlx_attribute(&self, name: &str, value: Value) -> Value {
        json!({"node": "InsightsQLXAttribute", "name": name, "value": value})
    }
    fn window_function(
        &self,
        name: &str,
