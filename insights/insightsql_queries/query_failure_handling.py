...
from insights.query_cache.failures import BUDGET_EXTENDED, BUDGET_INTERACTIVE, Budget, FailureKind, QueryFailureRecord
...
FAILURE_KIND_EXCEPTIONS: dict[FailureKind, type[APIException]] = {
    ...
    "too_slow": DatastoreEstimatedQueryExecutionTimeTooLong,
...
def classify_failure(error: Exception) -> Optional[FailureKind]:
    ...
    if isinstance(error, DatastoreQueryTimeOut):
        return "timeout"
    if isinstance(error, DatastoreEstimatedQueryExecutionTimeTooLong):
        return "too_slow"
    if isinstance(error, DatastoreQuerySizeExceeded):
        return "query_size"
    return None


def budget_for_limit_context(limit_context: Optional[LimitContext]) -> Budget:
    """Interactive requests get 60s of Datastore execution time while async workers and other
