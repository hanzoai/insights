...
def _check_host_safe(host: str, team_id: int) -> None:
    ...
    Insights may sit next to an internal TeamCity server over http, so http stays allowed.
    """
    parsed = urlparse(normalize_host(host))
    if is_cloud() and parsed.scheme != "https":
        raise ValueError(f"Invalid TeamCity server URL: {host!r} must use https:// on Insights Cloud")
