"""Delivering a subscription report to a Slack channel.

Building the message and sending it are separate: `build_slack_message` is pure,
so what a channel will see can be asserted without a Slack client, and
`deliver_slack_message_data` is the one send path — the AI report path builds a
different message but posts it through the same retries and partial-failure
accounting.
"""
...
from insights.helpers.slack_subscription_explore import build_explore_hint
...
# Slack rejects a section block over 3000 characters, which would fail the whole message.
...
def _block_for_asset(asset: ExportedAsset, resource_url: str) -> dict:
    if has_asset_failed(asset):
        ...
        error_text = (
            f"*{failure.insight_name}*\n"
            f"There was an error generating your asset: {failure.message}\n"
...
def build_slack_message(
    ...
    """The blocks a channel will see. Pure — reads the subscription, sends nothing."""
    ...
    if is_new_subscription:
        title = f"This channel has been subscribed to {display_name} on Insights! 🎉"
        title += (
    ...
    if change_summary:
        ...
        if len(summary_text) > _MAX_SECTION_CHARS:
            summary_text = summary_text[: _MAX_SECTION_CHARS - 3] + "..."
    ...
    thread_messages = [
        {"blocks": [_block_for_asset(asset, resource_url=resource_info.url)]} for asset in other_assets
    ]
...
async def send_slack_subscription_report(
    integration: Integration,
    subscription: Subscription,
    assets: list[ExportedAsset],
    total_asset_count: int,
    ...
    # `build_slack_message` reads lazily-loaded ORM relations (e.g. `integration.team.organization`),
    # which Django forbids on the event loop. Build it in a thread before the async Slack send.
    message_data = await database_sync_to_async(build_slack_message, thread_sensitive=False)(
        subscription,
        assets,
        total_asset_count,
        is_new_subscription,
