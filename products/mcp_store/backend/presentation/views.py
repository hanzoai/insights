...
from insights.cloud_utils import is_dev_mode
...
from insights.permissions import DenyMCPBuiltInAgentOAuth
...
def _create_oauth_state(
    ...
    token: str,
    install_source: str,
    insights_code_callback_url: str = "",
    pkce_verifier: str = "",
...
def _is_valid_insights_code_callback_url(url: str) -> bool:
    """Validate that a Insights Desktop callback URL is safe to redirect to (prevents open redirect)."""
    parsed = urlparse(url)
    if parsed.scheme in ("array", "insights-code"):
        return True
...
def _oauth_authorize_response(authorize_url: str, install_source: str) -> HttpResponse:
    if install_source == "insights-code":
...
class MCPServerTemplateSerializer(serializers.ModelSerializer):
    ...
    # Kept until Insights Desktop stops reading it; drop together with the model column.
...
class MCPServerViewSet(TeamAndOrgViewSetMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    Templates are seeded by Insights operators and carry shared, encrypted
...
class InstallCustomSerializer(serializers.Serializer):
    ...
    install_source = serializers.ChoiceField(choices=["insights", "insights-code"], required=False, default="insights")
    insights_code_callback_url = serializers.CharField(required=False, allow_blank=True, default="")
    ...
    def validate_insights_code_callback_url(self, value: str) -> str:
        if value and not _is_valid_insights_code_callback_url(value):
...
class MCPServerInstallationViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def _validate_gateway_options(self, data: dict) -> None:
        ...
        if not agent_ids.issubset(built_in_agent_ids):
            raise serializers.ValidationError({"agent_ids": "Select only the Insights agents listed for this project."})
    ...
    def _apply_gateway_options(self, installation: MCPServerInstallation, data: dict) -> None:
        ...
        if agent_ids != set(accounts_by_id):
            raise serializers.ValidationError(
                {"agent_ids": "One or more selected Insights agents are no longer available. Refresh and try again."}
            )
    ...
    def install_template(self, request: Request, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        install_source = data.get("install_source", "insights")
        insights_code_callback_url = data.get("insights_code_callback_url", "")
    ...
    def authorize(self, request: Request, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        install_source = query.get("install_source", "insights")
        insights_code_callback_url = query.get("insights_code_callback_url", "")
        web_return_path = query.get("return_path", "")

        if insights_code_callback_url and not _is_valid_insights_code_callback_url(insights_code_callback_url):
...
class MCPOAuthRedirectViewSet(viewsets.ViewSet):
    ...
    redirects to the originating client (Insights web or Insights Desktop).
    """
    ...
    def list(self, request: Request, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        insights_code_callback_url = oauth_state.insights_code_callback_url
    ...
    def _consume_oauth_state(self, request: Request, state_token: str) -> MCPOAuthState | None:
        ...
        if dev_mode:
            ownership_filter = Q(install_source="insights-code", created_by__isnull=False)
            if is_authenticated:
        ...
        with transaction.atomic():
            ...
            if (
                dev_mode
                and oauth_state.install_source == "insights-code"
                and oauth_state.created_by_id != request_user_id
            ):
                # Code's dev callback crosses into the system browser, which
                # does not share the API client's local Insights session.
                logger.info("OAuth redirect: allowing Insights Code callback without user binding in dev mode")
    ...
    def _build_oauth_redirect(
        ...
        *,
        ...
        if install_source == "insights-code" and insights_code_callback_url:
            params = {"status": "error", "error": error} if error else {"status": "success"}
            separator = "&" if "?" in insights_code_callback_url else "?"
            redirect_url = f"{insights_code_callback_url}{separator}{urlencode(params)}"
        else:
