...
export interface MaxWebAnalyticsNudgeLogicProps {
    ...
    // Domain classified by Insights AI (from the conversation's first question); authoritative when present.
...
export const maxWebAnalyticsNudgeLogic = kea<maxWebAnalyticsNudgeLogicType>([
    ...
    listeners(({ props, values, actions }) => ({
        nudgeClicked: () => {
            insights.capture('insights ai web analytics nudge clicked', values.reportProperties)
            ...
            router.actions.push(urls.webAnalytics(), { utm_source: 'insights-ai-nudge' })
        },
        nudgeDismissed: () => {
            insights.capture('insights ai web analytics nudge dismissed', values.reportProperties)
        ...
        evaluateReporting: () => {
            if (
                ...
                insights.capture('insights ai web analytics nudge eligible', values.reportProperties)
            }
            if (values.shouldShowNudge && values.shownForMessageId === null) {
                actions.markNudgeShown(props.messageId)
                insights.capture('insights ai web analytics nudge shown', values.reportProperties)
            }
        },
    })),
