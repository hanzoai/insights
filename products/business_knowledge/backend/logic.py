...
class _SemanticCandidate:
    """A chunk_id + cosineDistance returned from Datastore vector search."""
...
def _semantic_chunk_candidates(
    ...
    *,
    ...
    """
    from insights.insightsql import ast  # noqa: PLC0415 — keeps InsightsQL compiler off the import path
    from insights.insightsql.query import execute_insightsql_query  # noqa: PLC0415

    from insights.datastore.query_tagging import Feature, Product, tag_queries  # noqa: PLC0415
    ...
    # so cosineDistance runs once per row (Datastore can't reuse a SELECT alias
...
# actual produce-to-Kafka and Datastore presence check live in the coordinator
# activity so this module stays free of Kafka / Datastore dependencies.
# ---------------------------------------------------------------------------
...
class DocumentToEmbed:
    ...
    # a re-emit of the same chunk_id collapses onto one Datastore sort key /
...
class EmittedDocument:
    """An already-emitted SAFE document, for Datastore-presence reconciliation."""
...
def _embeddable_documents_qs() -> QuerySet[KnowledgeDocument]:
    ...
    reconciliation finds no vectors in Datastore and clears the stamp, putting
...
def list_documents_pending_embedding(*, limit: int = PENDING_EMBEDDING_SCAN_CAP) -> list[DocumentToEmbed]:
    ...
    collapses onto one Datastore sort key. Old docs (``created_at`` older than
...
def list_documents_for_embedding_reconciliation(
    *,
    ...
    be in Datastore, so the coordinator can re-verify they actually landed.

    ``embeddings_emitted_at`` only means "produced to Kafka", not "present in
    Datastore": a transient produce failure that did NOT raise, or a worker
...
def list_documents_for_embedding_refresh(
    *,
    ...
    aging toward the 3-month Datastore TTL, so their chunks can be re-emitted
...
def clear_document_embeddings_emitted(*, team_id: int, document_id: UUID) -> None:
    ...
    Used by reconciliation when a doc's vectors never landed in Datastore.
    """
