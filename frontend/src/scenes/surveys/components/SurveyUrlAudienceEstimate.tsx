...
import type { InsightsQLQueryString } from '~/queries/utils'
...
export function getUrlAudienceEstimateParams(
    ...
    if (matchType === SurveyMatchType.Regex) {
        // The estimate runs in Datastore, which evaluates RE2 — JS-only syntax (e.g. lookaheads)
    ...
    useEffect(() => {
        ...
        const timeout = window.setTimeout(async () => {
            try {
                ...
                queryInFlight = true
                const response = await api.queryInsightsQL<[[number]]>(query, URL_AUDIENCE_ESTIMATE_TAGS, {
                    clientQueryId,
        ...
        return () => {
            ...
            if (queryInFlight) {
                // Aborting only drops the connection — also cancel the Datastore query server-side
