...
class TestModelPath(BaseTest):
    ...
    def test_update_paths_for_insights_namespaced_table_query(self):
        """`insights.*`-namespaced tables (e.g. ai_events) must resolve as valid model-path parents on
        the UPDATE path too. `update_paths_from_query` keys off `get_insights_table_names()`, which omits
        namespaced tables — so re-saving/re-materializing a view over `insights.ai_events` raised
        UnknownParentError until the hidden table names were unioned in. (The create path already
        resolves them via `get_table()`.)"""
        query = "SELECT trace_id FROM insights.ai_events"
        saved_query = DataWarehouseSavedQuery.objects.create(
            team=self.team,
            name="my_model",
            query={"query": query},
        ...
        # `insights.ai_events` is stored as two ltree labels (LabelTreeField splits on "."), exactly
        ...
        self.assertEqual(len(paths), 1)
        self.assertIn(["insights", "ai_events", saved_query.id.hex], paths)
...
class TestBoundedResolver(BaseTest):
    ...
    def test_soft_mode_cycle_still_raises(self):
        ...
        context = InsightsQLContext(team_id=self.team.pk, team=self.team, enable_select_queries=True)
        context.database = Database.create_for(team_id=context.team_id, modifiers=context.modifiers, team=context.team)
        resolver = BoundedResolver(context=context, dialect="insightsql", initial_view_name="a", enforce_bounds=False)
...
class TestResolverFactoryInjection(BaseTest):
    ...
    def test_prepare_ast_for_printing_uses_injected_bounded_resolver(self):
        ...
        with pytest.raises(ResolutionDepthExceededError):
            prepare_ast_for_printing(query_node, context=context, dialect="datastore", resolver_factory=factory)
    ...
    def test_prepare_ast_for_printing_default_resolver_is_unbounded(self):
        ...
        prepare_ast_for_printing(query_node, context=context, dialect="datastore")
    ...
    def test_shared_deadline_anchor_spans_multiple_resolvers(self):
        ...
        r1 = factory(context, "insightsql", None)
        r2 = factory(context, "insightsql", None)
