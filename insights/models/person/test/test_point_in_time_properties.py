...
class TestPointInTimeProperties(SimpleTestCase):
    ...
    @patch("insights.models.person.point_in_time_properties.sync_execute")
    ...
    @patch("insights.models.person.point_in_time_properties.sync_execute")
    def test_build_person_properties_at_time_datastore_error(self, mock_sync_execute):
        mock_sync_execute.side_effect = Exception("Datastore connection failed")
        ...
        self.assertIn("Failed to query Datastore events", str(cm.exception))
...
class TestGetPersonAndDistinctIdsForIdentifierPersonhog(SimpleTestCase):
    ...
    def test_distinct_ids_from_personinsights_used_directly(self):
        with fake_personinsights_client() as fake:
            fake.add_person(
                team_id=1,
                person_id=42,
