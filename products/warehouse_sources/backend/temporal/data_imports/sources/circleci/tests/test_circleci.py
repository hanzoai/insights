...
class TestBuildUrl:
    ...
    def test_drops_none_values_and_encodes(self):
        url = _build_url("/pipeline", {"org-slug": "gh/insights", "page-token": None})
        assert url == "https://circleci.com/api/v2/pipeline?org-slug=gh%2Finsights"
...
class TestValidateCredentials:
    ...
    def test_status_mapping(self, me_status, pipeline_status, expected, mock_session):
        ...
        is_valid, error = validate_credentials("token", "gh/insights")
...
class TestPipelinesRows:
    ...
    def test_paginates_via_next_page_token(self, mock_session):
        ...
        batches = list(get_rows("token", "gh/insights", "pipelines", mock.MagicMock(), manager))

        assert [item["id"] for batch in batches for item in batch] == ["p1", "p2", "p3"]
        urls = _requested_urls(mock_session)
        assert parse_qs(urlparse(urls[0]).query) == {"org-slug": ["gh/insights"]}
        assert parse_qs(urlparse(urls[1]).query) == {"org-slug": ["gh/insights"], "page-token": ["token-2"]}
    ...
    def test_resumes_from_saved_state(self, mock_session):
        ...
        list(get_rows("token", "gh/insights", "pipelines", mock.MagicMock(), manager))
    ...
    def test_state_saved_after_yield_not_before(self, mock_session):
        ...
        manager = _make_manager()
        rows = get_rows("token", "gh/insights", "pipelines", mock.MagicMock(), manager)
    ...
    def test_page_cap_stops_pagination_and_logs(self, mock_session):
        ...
        manager = _make_manager()
        batches = list(get_rows("token", "gh/insights", "pipelines", logger, manager))
    ...
    def test_unknown_endpoint_raises(self):
        with pytest.raises(ValueError, match="Unknown CircleCI endpoint"):
            list(get_rows("token", "gh/insights", "nope", mock.MagicMock(), _make_manager()))
...
class TestRetryBehavior:
    ...
    def test_persistent_5xx_raises_after_max_retries(self, mock_session):
        ...
        with pytest.raises(CircleCIRetryableError):
            list(get_rows("token", "gh/insights", "pipelines", mock.MagicMock(), _make_manager()))
...
class TestWorkflowsFanOut:
    ...
    def test_workflows_fetched_per_pipeline(self, mock_session):
        ...
        batches = list(get_rows("token", "gh/insights", "workflows", mock.MagicMock(), _make_manager()))
    ...
    def test_child_page_cap_logs_and_moves_on(self, mock_session):
        ...
        batches = list(get_rows("token", "gh/insights", "workflows", logger, _make_manager()))
    ...
    def test_saves_pipeline_scan_state_after_fanout_page(self, mock_session):
        ...
        manager = _make_manager()
        list(get_rows("token", "gh/insights", "workflows", mock.MagicMock(), manager))
...
class TestJobsFanOut:
    ...
    def test_job_rows_carry_parent_identifiers(self, mock_session):
        _route_session(
            mock_session,
            {
                "/api/v2/pipeline": _page([{"id": "p1", "project_slug": "gh/insights/insights"}], None),
                "/api/v2/pipeline/p1/workflow": _page(
                    [{"id": "w1", "pipeline_id": "p1", "created_at": "2026-01-01T00:00:00Z"}], None
                ),
                "/api/v2/workflow/w1/job": _page(
                    [
                        {"id": "j1", "job_number": 42, "project_slug": "gh/insights/insights", "status": "success"},
                        {"id": "j2", "job_number": None, "project_slug": "gh/insights/insights", "status": "blocked"},
                    ],
                    None,
                ),
            },
        ...
        batches = list(get_rows("token", "gh/insights", "jobs", mock.MagicMock(), _make_manager()))
        rows = [row for batch in batches for row in batch]
        ...
        for row in rows:
            ...
            assert row["project_slug"] == "gh/insights/insights"
    ...
    def test_job_page_cap_logs_workflow_identifier(self, mock_session):
        ...
        batches = list(get_rows("token", "gh/insights", "jobs", logger, _make_manager()))
        ...
        _route_session(
            mock_session,
            {
                "/api/v2/pipeline": _page(
                    [
                        {"id": "p1", "project_slug": "gh/insights/insights"},
                        {"id": "p2", "project_slug": "gh/insights/insights"},
                        {"id": "p3", "project_slug": "gh/insights/hanzo.ai"},
                        {"id": "p4"},
                    ],
                    None,
                ),
                "/api/v2/project/gh/insights/insights": {"id": "proj-1", "slug": "gh/insights/insights"},
                "/api/v2/project/gh/insights/hanzo.ai": {"id": "proj-2", "slug": "gh/insights/hanzo.ai"},
            },
        )

        batches = list(get_rows("token", "gh/insights", "projects", mock.MagicMock(), _make_manager()))
        rows = [row for batch in batches for row in batch]
...
class TestCircleCISourceResponse:
    ...
    def test_response_metadata_per_endpoint(self, endpoint):
        ...
        response = circleci_source("token", "gh/insights", endpoint, mock.MagicMock(), _make_manager())
