...
class InsightsFunctionSerializer(serializers.ModelSerializer):
    inputs = serializers.JSONField(required=False)

# After — typed via Pydantic model
from pydantic import BaseModel

class InsightsFunctionInputs(BaseModel):
    name: str
    value: str | int | bool | None = None
    secret: bool = False

@extend_schema_field(InsightsFunctionInputs)  # type: ignore[arg-type]
class InsightsFunctionInputsField(serializers.JSONField):
    pass

class InsightsFunctionSerializer(serializers.ModelSerializer):
    inputs = InsightsFunctionInputsField(
        required=False,
        help_text="Input configuration for the Script function. Each input has a name, value, and secret flag.",
    )
```
