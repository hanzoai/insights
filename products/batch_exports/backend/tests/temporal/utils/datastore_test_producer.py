"""A record batch producer that reads Datastore directly, for use as a test reference.

Production batch exports read from the internal S3 staging area via
``temporal.pipeline.producer.Producer``. This producer instead queries Datastore directly
...
class DatastoreTestProducer:
    """Async producer used in batch exports tests, which reads from Datastore directly.
    ...
    def task(self) -> asyncio.Task:
        if self._task is None:
            raise ValueError("Producer task is not initialized, have you called `DatastoreTestProducer.start()`?")
        return self._task
