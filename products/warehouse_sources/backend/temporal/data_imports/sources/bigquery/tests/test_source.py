...
def test_bigquery_get_columns_filters_existing_destination_tables():
    """`get_columns` strips `__insights_import_*` tables before returning."""
    ...
    fake_row_skip = mock.MagicMock(
        table_name="__insights_import_0000_0000", column_name="c", data_type="STRING", is_nullable="NO"
    )
...
def test_bigquery_build_pipeline_resolves_dataset_routing(
    ...
    expected_table_id = (
        f"project-id.{expected_destination_dataset_id}.__insights_import_schema_id_job_id_"
    ...
    assert mock_delete_all.call_args.kwargs["table_prefix"] == "__insights_import_schema_id"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'table_id must be a fully-qualified ID in standard SQL format, e.g., "project.dataset.table_id", '
        "got immortal-407108.immortal-407108.analytics_529249625.__insights_import_abc_def_123",
    ],
)
...
def test_bigquery_get_columns_qualifies_information_schema_with_dataset_project():
    ...
    config = _make_config(
        project_id="service-account-project",
        dataset_id="insights_export",
        dataset_project=BigQueryDatasetProjectConfig(dataset_project_id="dataset-project", enabled=True),
    )
    BigQueryImplementation().get_columns(fake_client, config, names=None)

    sql = fake_client.query.call_args.args[0]
    assert "`dataset-project.insights_export.INFORMATION_SCHEMA.COLUMNS`" in sql
...
def test_bigquery_discovery_qualifies_information_schema_with_dataset_project(method_name):
    ...
    sql = fake_client.query.call_args.args[0]
    assert "`dataset-project.insights_export`.INFORMATION_SCHEMA" in sql
    assert fake_client.query.call_args.kwargs["project"] == "dataset-project"
...
def test_bigquery_build_pipeline_trims_whitespace_in_destination_table():
    ...
    config = _make_config(project_id=" 524098457564", dataset_id=" bigquery_aloalo ")
    expected_table_id = (
        f"524098457564.bigquery_aloalo.__insights_import_schema_id_job_id_"
        f"{str(parser.parse('2025-01-01T12:00:00.000Z').timestamp()).replace('.', '')}"
    )
...
@pytest.mark.parametrize(
    ...
    [
        # Overwriting a Insights temp table — denied with bigquery.tables.update on the table.
        (
            str(
                Forbidden(
                    "Access Denied: Table prj:ds.__insights_import_abc_123: Permission bigquery.tables.update "
                    "denied on table prj:ds.__insights_import_abc_123 (or it may not exist)."
                )
            ),
            "bigquery.tables.update",
            "write access",
        ),
        # Creating a Insights temp table — denied with bigquery.tables.create on the dataset.
        (
            str(
                Forbidden(
