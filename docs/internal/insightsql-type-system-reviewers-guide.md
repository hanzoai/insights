# InsightsQL Type System Reviewers Guide

This guide is for reviewers of the InsightsQL type-system PR on branch `the-real-type-system`.
...
- `docs/internal/insightsql-type-system-now-possible.md`
- `docs/internal/insightsql-type-system-todo.md`

## Review Goal

The main thing to verify is that this PR improves InsightsQL type metadata and selected optimizer inputs without making InsightsQL a strict validator by default.
...
The PR is intentionally centered on InsightsQL expression typing and the optimizations that directly depend on that typing.
The relevant product problem is that InsightsQL frequently loses type facts at function, property, cast, array, tuple, map, aggregate, and materialized-column boundaries.
...
`insights/insightsql/type_system.py` adds a structured `RuntimeType` model and conversion helpers between runtime types, existing `ast.ConstantType` objects, database fields, and SQL type strings.
...
`insights/insightsql/resolver.py` now assigns better output types to:
...
`insights/insightsql/type_diagnostics.py` adds tools to resolve a query and report:

- unknown type occurrences
- optimizer blockers grouped by source/detail
- top-level select-expression inferred types
- a companion `toTypeName(...)` query for comparing inferred types with Datastore result metadata
...
`insights/insightsql/transforms/type_aware_simplification.py` adds an internal simplifier for conservative rewrites.
It is disabled by default and only runs when the `typeAwareCastSimplification` query modifier is enabled, or when the `InsightsQLContext.enable_type_aware_cast_simplification` flag is true.
...
`insights/insightsql/property_planner.py` centralizes property access and comparison planning.
It combines:
...
- physical materialized-column type from Datastore metadata
- index availability
...
Materialized-column introspection now carries the Datastore physical column type from `system.columns`.
...
The Datastore printer now uses the property planner for materialized range comparisons.
...
Some emitted Datastore SQL no longer has defensive `ifNull(...)` wrappers when the resolver now knows an expression is non-nullable.
...
- `products/revenue_analytics/backend/insightsql_queries/test/__snapshots__/test_revenue_analytics_gross_revenue_query_runner.ambr`: 16 entries to 2 entries.
- `products/revenue_analytics/backend/insightsql_queries/test/__snapshots__/test_revenue_analytics_metrics_query_runner.ambr`: 15 entries to 1 entry.
- `products/revenue_analytics/backend/insightsql_queries/test/__snapshots__/test_revenue_analytics_overview_query_runner.ambr`: 12 entries to 1 entry.
- `products/revenue_analytics/backend/insightsql_queries/test/__snapshots__/test_revenue_analytics_top_customers_query_runner.ambr`: 10 entries to 1 entry.
...
The branch adds `insights/insightsql/test/test_type_system.py` and expands coverage in:

- `insights/insightsql/transforms/test/test_property_types.py`
- `insights/insightsql/printer/test/test_printer.py`
- `insights/insightsql/test/test_property_skip_indexes.py`
- `ee/datastore/materialized_columns/test/test_columns.py`
...
- Compare inferred select-expression types against Datastore `toTypeName(...)` results.
...
It does not yet enable strict InsightsQL typing by default.
It does not roll out typed materialized columns broadly.
It does not claim full Datastore function parity.

## Review Strategy

Start with the tests and docs, then review from the compatibility boundary inward.

1. Read `insights/insightsql/test/test_type_system.py` to understand the intended type behavior.
2. Read `insights/insightsql/property_planner.py` and the property-planner tests before reviewing printer range rewrites.
3. Review `insights/insightsql/type_system.py` for algebra, parser, and generic inference correctness.
4. Review `insights/insightsql/resolver.py` for where inferred types are attached to AST nodes.
5. Review `insights/insightsql/printer/datastore.py` for behavior-changing SQL output.
6. Review materialized-column type introspection and cache-version changes in `ee/datastore/materialized_columns/columns.py`.
...
Some type-driven decisions are only correct under the Datastore settings Insights pins for every InsightsQL query via `InsightsQLGlobalSettings` (`insights/insightsql/constants.py`), most notably `transform_null_in=1`.
...
It does not implement strict arity validation for every Datastore variant.
...
The runtime type parser has Datastore, Postgres, and DuckDB adapters, but Datastore is the main optimization target.
Review cast and try-cast behavior carefully for Postgres-family targets.
Datastore-only printer behavior should not leak into Postgres or DuckDB printing.

### Opt-In Simplification

The simplifier is intentionally behind the `typeAwareCastSimplification` modifier and the `InsightsQLContext.enable_type_aware_cast_simplification` flag, both off by default.
...
```bash
insightscli test insights/insightsql/test/test_type_system.py
```

Run property planning and JSONExtract rewrite coverage:

```bash
insightscli test insights/insightsql/transforms/test/test_property_types.py
```

Run printer coverage for nullability and materialized-column SQL:

```bash
insightscli test insights/insightsql/printer/test/test_printer.py
```

Run Datastore skip-index integration tests when Datastore is available:

```bash
insightscli test insights/insightsql/test/test_property_skip_indexes.py
```

Run materialized-column DDL/type coverage:

```bash
insightscli test ee/datastore/materialized_columns/test/test_columns.py
```

Regenerate or verify the revenue analytics snapshots before merge:

```bash
insightscli test products/revenue_analytics/backend/insightsql_queries/test/test_revenue_analytics_gross_revenue_query_runner.py
insightscli test products/revenue_analytics/backend/insightsql_queries/test/test_revenue_analytics_metrics_query_runner.py
insightscli test products/revenue_analytics/backend/insightsql_queries/test/test_revenue_analytics_overview_query_runner.py
insightscli test products/revenue_analytics/backend/insightsql_queries/test/test_revenue_analytics_top_customers_query_runner.py
insightscli test products/revenue_analytics/backend/views/test/test_mrr_views.py
```

For a narrower pass, prioritize:

```bash
insightscli test insights/insightsql/test/test_type_system.py::TestInsightsQLTypeSystem::test_type_aware_simplification_is_opt_in
insightscli test insights/insightsql/test/test_type_system.py::TestInsightsQLTypeSystem::test_type_aware_simplification_keeps_unsafe_casts
insightscli test insights/insightsql/transforms/test/test_property_types.py::TestPropertyTypes::test_property_comparison_planner_blocks_numeric_minmax_until_source_type_matches
insightscli test insights/insightsql/transforms/test/test_property_types.py::TestPropertyTypes::test_property_comparison_planner_allows_numeric_minmax_when_source_type_matches
insightscli test insights/insightsql/test/test_property_skip_indexes.py::TestEventPropertySkipIndexes::test_typed_numeric_mat_col_uses_minmax_index
insightscli test insights/insightsql/test/test_property_skip_indexes.py::TestEventPropertySkipIndexes::test_typed_datetime_mat_col_uses_minmax_index
```

## Good Reviewer Questions

- Does this inferred type match what Datastore actually returns for representative non-literal inputs?
...
- strict InsightsQL validation
- full Datastore function-signature parity
...
- broad query-corpus validation against Datastore `toTypeName(...)`
