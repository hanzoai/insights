...
from insights.datastore.preaggregation.marketing_touchpoints_sql import TRUNCATE_MARKETING_TOUCHPOINTS_TABLE_SQL
from insights.datastore.query_tagging import Feature, get_query_tag_value, reset_query_tags, tag_queries
...
def _normalize_job_uuids(insightsql: str) -> str:
    ...
    return _JOB_UUID_RE.sub(_repl, insightsql)
...
class TestConversionGoalsAggregator(DatastoreTestMixin, BaseTest):
    ...
    def test_unified_cte_sql_snapshot(self):
        ...
        cte = aggregator.generate_unified_cte(self.date_range, additional_conditions_getter)
        response = execute_insightsql_query(query=cte.expr, team=self.team)

        assert _normalize_job_uuids(pretty_print_in_tests(response.insightsql, self.team.pk)) == self.snapshot
    ...
    def test_campaign_name_mapping_integration_with_events(self):
        ...
        sql_string = pretty_print_in_tests(response.insightsql, self.team.pk)
    ...
    def test_touchpoints_precompute_materialized_once_across_goals(self):
        ...
        with patch(
            "products.marketing_analytics.backend.insightsql_queries.conversion_goal_processor.marketing_ensure_precomputed",
