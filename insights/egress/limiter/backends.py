...
``limits`` builds its storage from our shared sync ``insights.redis`` client; the async ``acquire``
...
from insights.egress.limiter.policies import Priority, RateLimit, RatePolicy
