"""insightscli - Unified developer CLI for Insights monorepo.
...
import click
from insightscli.core.command_types import BinScriptCommand, CompositeCommand, DirectCommand, InsightscliCommand
from insightscli.core.manifest import REPO_ROOT, load_manifest
...
class CategorizedGroup(click.Group):
    ...
    def format_commands(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        from insightscli.core.manifest import (
        ...
        for cmd_name, cmd in self.commands.items():
            ...
            insightscli_config = getattr(cmd, "insightscli_config", {})
            if insightscli_config.get("hidden", False):
                continue
...
def _auto_update_manifest() -> None:
    ...
    from insightscli.core.validate import auto_update_manifest
...
def cli(ctx: click.Context) -> None:
    """insightscli - Developer CLI for Insights."""
...
def meta_check() -> None:
    ...
    from insightscli.core.validate import find_missing_manifest_entries
...
def _register_script_commands() -> None:
    ...
    2. steps: Compose multiple insightscli commands in sequence
    ...
    for _category, scripts in manifest.items():
        ...
        for cli_name, config in scripts.items():
            ...
            if not (bin_script or steps or cmd or insightscli):
                continue
            ...
            # Handle insightscli wrapper commands (insightscli field)
            if insightscli:
                command = InsightscliCommand(cli_name, config)
                command.register(cli)
                continue
...
try:
    import insightscli.commands  # noqa: F401
except ImportError:
