"""Central Insights model admin classes.

Importing this package triggers every ``@admin.register(Model)`` decorator in
``insights/admin/admins/*.py``. ``register_all_admin()`` does ``import
insights.admin.admins`` for that side effect; nothing else relies on the
package re-exporting names. Consumers that want a specific admin class
should import it directly from its submodule, e.g.
``from insights.admin.admins.user_admin import UserAdmin``.
