...
> fail-fast + instant retry) belongs in insights/code, not this experiment.
>
...
> both. E2E verified: gateway probe → luna gen with cost in local Datastore. Cleanup: 15 orphan
...
> 3. Diagnose Codex MCP flakiness (9/21 sessions in C1 had no `insights/exec`) — likely a connect race
...
3. `flox activate -- bash -c "SANDBOX_PROVIDER=MODAL_DOCKER DJANGO_SETTINGS_MODULE=insights.settings \
python manage.py run_review --pr-url https://github.com/Insights/insights/pull/72680 --team-id 1 --user-id 1"`
4. Dump: `LABEL=<label> RUN_SECONDS=<s> RUN_START_EPOCH=<epoch> \
OUT_DIR=products/review/eval/experiments/2026-07-reviewer-model-glm52/runs \
python manage.py shell -c "exec(open('products/review/eval/scripts/dump_result.py').read())"`
