"""InsightsQL `information_schema`: a self-describing, queryable view of the database catalog.

Nested under the `system` namespace, these virtual tables let agents (and humans) discover what
tables, columns, data types, relationships, and descriptions are available without leaving InsightsQL:
...
from insights.insightsql.errors import BaseInsightsQLError
...
# --- value coercion --------------------------------------------------------------------------- #
# Every cell in the constant-row array is emitted as a String so Datastore infers a single,
...
def _capture_unexpected(message: str, error: Exception) -> None:
    ...
    tracking — expected `BaseInsightsQLError`s are handled by the caller and never sent here."""
    # Deferred: keeps the (heavier) capture dependency off this schema module's import path.
    from insights.exceptions_capture import capture_exception  # noqa: PLC0415
...
def _classify_table(name: str, table: Table, warehouse: set[str], views: set[str]) -> tuple[str, str]:
    ...
    if name in views or isinstance(table, SavedQuery):
        return "view", "views"
    return "insights", "public"


def _visible_table_names(database: "Database") -> list[str]:
    # The `insights.*` namespace holds two kinds of tables: copies of the top-level tables
    # (`insights.events`, `insights.persons`, …) and data-plane tables that live *only* there
    # (`insights.ai_events`, `insights.trace_spans`, `insights.metrics`, the web pre-aggregated
    # tables, …). Hide the copies to keep the catalog clean, but keep the unique ones — they're
    # queryable by their `insights.`-qualified name (a bare `FROM ai_events` errors), so leaving
    ...
    return [n for n in names if not (n.startswith("insights.") and n[len("insights.") :] in root_names)]
...
def _warehouse_metadata(team_id: Optional[int]) -> _WarehouseMetadata:
    ...
    `create_insightsql_database` path. Mirrors how `serialize_database` sources counts so the catalog and
    ...
    # Inline imports: keeps the products dependency off the insightsql import path (avoids an import
    # cycle, since products import insightsql) and off every non-information_schema query.
    from insights.models.scoping import team_scope  # noqa: PLC0415
...
def _bound_table_names(expr: Optional[ast.Expr], column: str) -> Optional[set[str]]:
    ...
    Datastore to apply the real predicate. The invariant is one-directional: the returned set must
...
# Datastore column types for the external data table, keyed by the same kinds as `_constant_rows_select`.
_KIND_TO_DATASTORE: dict[str, str] = {
    _STRING: "String",
    _NULLABLE_STRING: "Nullable(String)",
    _INTEGER: "Int64",
    _NULLABLE_INTEGER: "Nullable(Int64)",
    _NULLABLE_FLOAT: "Nullable(Float64)",
    # InsightsQL's BooleanDatabaseField maps to Datastore UInt8; Python bool serializes to it directly.
    _BOOLEAN: "UInt8",
}
...
class _ExternalDataTable(DANGEROUS_NoTeamIdCheckTable):
    """A query-scoped Datastore external data table, referenced by a bare name.

    Its rows travel out-of-band via `sync_execute(external_tables=...)`, so the printed query text
    stays small no matter how large the catalog is. `to_printed_datastore` emits the registered name
    verbatim; Datastore resolves it against the external data sent alongside the request. The
...
def _rows_select(
    ...
    if ext_name not in context.external_tables:
        context.external_tables[ext_name] = {
            "name": ext_name,
            "structure": [(name, _KIND_TO_DATASTORE[kind]) for name, kind in columns],
...
class _Introspection:
    ...
    def __init__(
        self, database: "Database", context: "InsightsQLContext", allowed_tables: Optional[frozenset[str]] = None
    ) -> None:
    ...
    def _table_scope(self, table_name: str) -> Optional["ast.SelectQueryType"]:
        if table_name not in self._table_scope_cache:
            ...
            from insights.insightsql.resolver import resolve_table_scope  # noqa: PLC0415

            scope: Optional[ast.SelectQueryType] = None
            try:
                scope = resolve_table_scope(table_name.replace("`", "").split("."), self.context, "insightsql")
            except BaseInsightsQLError:
    ...
    def _expression_data_type(self, table_name: str, field: ExpressionField) -> str:
        """Type an expression column by the value it evaluates to, like the InsightsQL autocomplete does;
        fall back to the generic "Expression" if it can't be resolved."""
        scope = self._table_scope(table_name)
        if scope is None:
            return "Expression"
        # Deferred: see `_table_scope`.
        from insights.insightsql.resolver import resolve_types  # noqa: PLC0415
        from insights.insightsql.visitor import clone_expr  # noqa: PLC0415

        try:
            # Clone so resolution never mutates the shared schema field's expression.
            resolved = resolve_types(clone_expr(field.expr, clear_locations=True), self.context, "insightsql", [scope])
...
def _introspection(
    context: "InsightsQLContext", allowed_tables: Optional[frozenset[str]] = None
...
_RELATIONSHIPS_DESCRIPTION = (
    ...
    "Use it to discover how to join from one table to another in InsightsQL."
)
...
def _can_read_catalog(context: "InsightsQLContext") -> bool:
...
def _catalog_metrics(context: "InsightsQLContext", allowed: Optional[frozenset[str]]) -> list[list[Any]]:
...
def _catalog_certifications(context: "InsightsQLContext", allowed: Optional[frozenset[str]]) -> dict[tuple[str, str], str]:
...
def _catalog_table_visible(context: "InsightsQLContext", table_name: str) -> bool:
    database = context.database
...
def _catalog_accepted_relationships(
    context: "InsightsQLContext", allowed: Optional[frozenset[str]]
...
def _catalog_relationship_proposals(context: "InsightsQLContext", allowed: Optional[frozenset[str]]) -> list[list[Any]]:
...
def _catalog_certification_rows(context: "InsightsQLContext", allowed: Optional[frozenset[str]]) -> list[list[Any]]:
...
class InformationSchemaTablesTable(LazyTable):
    ...
    fields: dict[str, FieldOrTable] = {
        "table_catalog": _string_field(
            "table_catalog",
            nullable=False,
            description="Table name (same as table_name); Insights does not use a separate catalog identifier.",
        ),
        ...
        "table_type": _string_field(
            "table_type",
            nullable=False,
            description="Origin of the table: 'insights', 'system', 'information_schema', 'data_warehouse', or 'view'.",
        ),
    ...
    def lazy_select(self, table_to_add: LazyTableToAdd, context: "InsightsQLContext", node: Any) -> ast.SelectQuery:
...
class InformationSchemaColumnsTable(LazyTable):
    ...
    fields: dict[str, FieldOrTable] = {
        ...
        "data_type": _string_field(
            "data_type",
            nullable=False,
            description="InsightsQL data type, e.g. String, Integer, DateTime, JSON; see information_schema.data_types.",
        ),
...
class InformationSchemaDataTypesTable(LazyTable):
    description: str = (
        "Reference list of the InsightsQL data types reported in information_schema.columns, with a short "
...
class InformationSchemaMetricsTable(LazyTable):
    ...
    fields: dict[str, FieldOrTable] = {
        ...
        "definition_kind": _string_field(
            "definition_kind",
            nullable=True,
            description="Query kind: InsightsQLQuery, TrendsQuery, FunnelsQuery, or an event node.",
        ),
