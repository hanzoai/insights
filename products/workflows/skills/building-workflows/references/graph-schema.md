...
- `hash` — InsightsQL template defining the dedup key. `"{person.id}"` = once per person.
...
- Templating uses **single-curly** `{person.x}` / `{event.x}` inside the value string. Liquid-style `{{ ... }}` is rejected on script-templated fields ("Placeholders are not allowed in this context") — the only fields that accept Liquid are ones whose input schema declares `templating: liquid` (the email input on `function_email` does; most others don't).
...
- **Templating differs per input.** `body` is **liquid** — interpolate with `{{ person.x }}` / `{{ event.x }}` (double braces) and set `"templating": "liquid"`. `title` and the other string inputs are **script** — use `{event.x}` / `{person.x}` (single braces). The wrong brace style leaves the expression as a literal.
