...
class TestFetchScreenshotBytes(APIBaseTest):
    ...
    def test_returns_bytes_for_matching_screenshot(self):
        ...
        result = _fetch_screenshot_bytes(self.team, "https://hanzo.ai/pricing")
    ...
    @parameterized.expand(
        [
            ...
            ("url_mismatch", {"url": "https://hanzo.ai/other"}, True),
    ...
    def test_excludes_unusable_screenshots(self, _name, overrides, with_content):
        ...
        assert _fetch_screenshot_bytes(self.team, "https://hanzo.ai/pricing") is None
...
class TestGroundHeatmapHotspots(APIBaseTest):
    @sync_to_async
    def _seed_screenshot(self, url: str = "https://hanzo.ai/pricing") -> None:
    ...
    async def test_returns_none_when_no_cached_screenshot(self):
        result = await ground_heatmap_hotspots(
            self.team, self.user, page_url="https://hanzo.ai/pricing", heatmap_data=_heatmap_data()
        )
