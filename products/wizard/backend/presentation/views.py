...
def _wizard_sync_killswitch_enabled(distinct_id: str) -> bool:
    ...
    # Flag definitions are served via HyperCache (insights/apps.py). Fail-open:
...
class WizardSessionViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @extend_schema(
        description=(
            ...
            "This should only be called by the Insights Wizard. "
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description="Filter to a single workflow (e.g. 'insights-integration').",
            ),
            OpenApiParameter(
                name="skill_id",
                required=False,
