...
class TestSlackThreadHandler(SimpleTestCase):
    ...
    def test_progress_message_has_no_terminate_button(self, mock_get_client, _mock_find_progress):
        ...
        handler.post_or_update_progress(
            "In progress...", task_url="https://us.hanzo.ai/project/1/tasks/abc?runId=xyz"
        )
        ...
        assert actions[0]["url"] == "https://us.hanzo.ai/project/1/tasks/abc?runId=xyz"
    ...
    def test_update_reaction_removes_eyes_then_adds_new(self, mock_get_client):
        ...
        handler = SlackThreadHandler(context)
        handler.update_reaction("mascot")

        remove_calls = mock_client.reactions_remove.call_args_list
        assert len(remove_calls) == 1
        assert remove_calls[0].kwargs["name"] == "eyes"
        mock_client.reactions_add.assert_called_once_with(channel="C001", timestamp="1234.5678", name="mascot")
    ...
    def test_post_pr_opened_posts_buttons(self, mock_get_client):
        ...
        handler.post_pr_opened("https://github.com/org/repo/pull/1", "https://hanzo.ai/task/1")
        ...
        assert actions[1]["text"]["text"] == "Open in Insights"
...
class TestSlackThreadHandlerWithoutTaskUrl(SimpleTestCase):
    """A ``task_url=None`` payload signals the recipient does not have Insights Desktop access.

    Each renderer must drop the Insights button (or the entire actions block when
