...
-- Full CREATE schema for the prod-us/ops node. Apply to a fresh Datastore to build it.

CREATE TABLE insights.events_main (
  uuid UUID,
  event String,
  properties String,
...
) ENGINE = Distributed('insights', 'insights', 'sharded_events', sipHash64(distinct_id));
CREATE TABLE insights.events_recent (
  uuid UUID,
  event String,
  properties String,
  timestamp DateTime64(6, 'UTC'),
  ...
  _offset UInt64,
  inserted_at DateTime64(6, 'UTC')
) ENGINE = Distributed('batch_exports', 'insights', 'sharded_events_recent', sipHash64(distinct_id));
CREATE TABLE insights.events_team_daily_stats (
  analysis_date Date,
  team_id Int64,
  event String,
  event_count UInt64,
...
) ENGINE = ReplicatedMergeTree('/datastore/ops/tables/{shard}/insights.events_team_daily_stats', '{replica}') ORDER BY (analysis_date, team_id, event) SETTINGS index_granularity = 8192;
CREATE TABLE insights.metrics_exemplars (
  team_id UInt64,
  timestamp DateTime64(3, 'UTC'),
  id UInt64,
  value Float64,
  labels_json String
) ENGINE = ReplicatedMergeTree('/datastore/ops/tables/{shard}/insights.metrics_exemplars', '{replica}') ORDER BY (team_id, id, timestamp) PARTITION BY toYYYYMMDD(timestamp) SETTINGS index_granularity = 1024;
CREATE TABLE insights.metrics_histograms (
  team_id UInt64,
  ...
  histogram String,
  version UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/ops/tables/{shard}/insights.metrics_histograms', '{replica}', version) ORDER BY (team_id, id, timestamp) PARTITION BY toYYYYMMDD(timestamp) SETTINGS index_granularity = 1024;
CREATE TABLE insights.metrics_label_index (
  team_id UInt64,
  metric_name LowCardinality(String),
  label_name LowCardinality(String),
  label_value String,
  id UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/ops/tables/{shard}/insights.metrics_label_index', '{replica}') ORDER BY (team_id, metric_name, label_name, label_value, id) SETTINGS deduplicate_merge_projection_mode = 'rebuild', index_granularity = 1024;
CREATE TABLE insights.metrics_metadata (
  team_id UInt64,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/ops/tables/{shard}/insights.metrics_metadata', '{replica}', updated_at) ORDER BY (team_id, metric_family_name) SETTINGS index_granularity = 1024;
CREATE TABLE insights.metrics_samples (
  team_id UInt64,
  metric_name LowCardinality(String),
  timestamp DateTime64(3, 'UTC'),
  id UInt64,
  value Float64
) ENGINE = ReplicatedMergeTree('/datastore/ops/tables/{shard}/insights.metrics_samples', '{replica}') ORDER BY (team_id, metric_name, id, timestamp) PARTITION BY toYYYYMMDD(timestamp) SETTINGS index_granularity = 1024;
CREATE TABLE insights.metrics_series (
  team_id UInt64,
  id UInt64,
  metric_name LowCardinality(String),
  labels_json String,
...
) ENGINE = ReplicatedMergeTree('/datastore/ops/tables/{shard}/insights.metrics_series', '{replica}') ORDER BY (team_id, metric_name, id) SETTINGS index_granularity = 1024;
CREATE TABLE insights.sharded_query_log_archive (
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/ops/{shard}/insights.tophog_new', '{replica}') ORDER BY (pipeline, lane, metric, timestamp, key) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(timestamp) + toIntervalDay(30) SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE MATERIALIZED VIEW insights.metrics_label_index_from_series_mv TO insights.metrics_label_index (team_id UInt64, metric_name LowCardinality(String), label_name LowCardinality(String), label_value String, id UInt64) AS SELECT
  ...
  id
FROM
  insights.metrics_series ARRAY JOIN JSONExtractKeysAndValues(labels_json, 'String') AS label_pair;
...
CREATE TABLE insights.query_log_archive_buffer (
  hostname LowCardinality(String),
  user LowCardinality(String),
  query_id String,
  initial_query_id String,
  ...
  log_comment JSON(max_dynamic_paths=256, access_method LowCardinality(String), alert_config_id String, api_key_label String, api_key_mask String, batch_export_id String, chargeable Bool, client_query_id String, cohort_id Int64, `dagster.job_name` String, `dagster.run_id` String, `dagster.tags.owner` String, dashboard_id Int64, experiment_feature_flag_key String, experiment_id Int64, feature LowCardinality(String), id String, insight_id Int64, is_impersonated Bool, kind LowCardinality(String), name String, org_id String, person_on_events_mode LowCardinality(String), product LowCardinality(String), query_type LowCardinality(String), request_name String, route_id String, service_name String, session_id String, table_id String, team_id Int64, `temporal.activity_id` String, `temporal.activity_type` String, `temporal.attempt` Int64, `temporal.workflow_id` String, `temporal.workflow_namespace` String, `temporal.workflow_run_id` String, `temporal.workflow_type` String, user_id Int64, warehouse_query Bool, workflow LowCardinality(String), workload LowCardinality(String), SKIP cache_key, SKIP filter, SKIP insightsql_features, SKIP http_referer, SKIP http_request_id, SKIP http_user_agent, SKIP query_settings, SKIP timings, SKIP user_email),
  ProfileEvents Map(String, UInt64)
) ENGINE = Buffer('insights', 'sharded_query_log_archive', 16, 10, 60, 10000, 1000000, 10000000, 100000000);
