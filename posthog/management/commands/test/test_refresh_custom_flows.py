...
from posthog.management.commands.refresh_custom_flows import remove_event_filters_from_conditionals
from posthog.models import Team
from posthog.models.custom_flow.custom_flow import CustomFlow


class TestRefreshCustomFlows(BaseTest):
    def setUp(self):
        super().setUp()
        ...
        # Create CustomFlows for testing with proper trigger actions
        with patch("posthog.models.custom_flow.custom_flow.reload_custom_flows_on_workers"):
            trigger_config_1 = {
                "type": "event",
                "filters": {
            ...
            self.custom_flow1 = CustomFlow.objects.create(
                team=self.team,
                name="Test Flow 1",
                status=CustomFlow.State.ACTIVE,
            ...
            self.custom_flow2 = CustomFlow.objects.create(
                team=self.team,
                name="Test Flow 2",
                status=CustomFlow.State.DRAFT,
            ...
            self.custom_flow3 = CustomFlow.objects.create(
                team=self.team2,
                name="Test Flow 3",
                status=CustomFlow.State.ACTIVE,
            ...
            self.archived_flow = CustomFlow.objects.create(
                team=self.team,
                name="Archived Flow",
                status=CustomFlow.State.ARCHIVED,
    ...
    def test_refresh_all_custom_flows(self, mock_reload):
        """Test refreshing all CustomFlows across all teams."""

        out = StringIO()
        call_command("refresh_custom_flows", stdout=out)

        # Should have refreshed all 4 flows (including archived)
        assert mock_reload.call_count == 4

        output = out.getvalue()
        self.assertIn("Found 4 CustomFlows to process", output)
    ...
    def test_refresh_by_team_id(self, mock_reload):
        """Test refreshing CustomFlows for a specific team."""

        out = StringIO()
        call_command("refresh_custom_flows", team_id=self.team.id, stdout=out)

        # Should have refreshed flows from team1 (custom_flow1, custom_flow2, archived_flow)
        assert mock_reload.call_count == 3

        output = out.getvalue()
        self.assertIn(f"Processing CustomFlows for team: {self.team.id}", output)
        self.assertIn("Found 3 CustomFlows to process", output)
        self.assertIn("Updated: 3", output)

    @patch("posthog.models.custom_flow.custom_flow.reload_custom_flows_on_workers")
    def test_refresh_by_custom_flow_id(self, mock_reload):
        """Test refreshing a specific CustomFlow by ID."""

        out = StringIO()
        call_command("refresh_custom_flows", custom_flow_id=str(self.custom_flow1.id), stdout=out)

        # Should have refreshed only the specific flow
        assert mock_reload.call_count == 1

        output = out.getvalue()
        self.assertIn(f"Processing single CustomFlow: {self.custom_flow1.id}", output)
        self.assertIn("Found 1 CustomFlows to process", output)
    ...
    def test_nonexistent_team_id(self, mock_reload):
        ...
        out = StringIO()
        call_command("refresh_custom_flows", team_id=99999, stdout=out)

        assert mock_reload.call_count == 0

        output = out.getvalue()
        self.assertIn("Found 0 CustomFlows to process", output)
        self.assertIn("No CustomFlows found matching criteria", output)

    @patch("posthog.models.custom_flow.custom_flow.reload_custom_flows_on_workers")
    def test_nonexistent_custom_flow_id(self, mock_reload):
        """Test handling of nonexistent CustomFlow ID."""

        out = StringIO()
        # Use a valid UUID format that doesn't exist
        nonexistent_uuid = "00000000-0000-0000-0000-000000000000"
        call_command("refresh_custom_flows", custom_flow_id=nonexistent_uuid, stdout=out)
    ...
    def test_page_size_option(self, mock_reload):
        ...
        call_command("refresh_custom_flows", page_size=2, stdout=out)
    ...
    def test_bytecode_regeneration_on_conditional_branch(self, mock_reload):
        ...
        # Create a CustomFlow with conditional branch that has filters but no bytecode
        actions = [
            {
                "id": "trigger_node",
                "name": "Trigger",
        ...
        test_flow = CustomFlow.objects.create(
            team=self.team,
        ...
        out = StringIO()
        call_command("refresh_custom_flows", custom_flow_id=str(test_flow.id), stdout=out)
