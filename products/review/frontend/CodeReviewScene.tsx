...
function prettifySkillName(skillName: string): string {
    const cleaned = skillName
        .replace(/^review-script-(perspective|blind-spots|validation)-/, '')
        .replace(/[-_]/g, ' ')
        .trim()
...
function ProofCard(): JSX.Element | null {
    ...
    return (
        <Card hoverEffect={false} className="mt-2 flex flex-col gap-3 p-5">
            ...
            <div className="flex flex-wrap items-stretch gap-2.5">
                {PIPELINE_PHASES.map((phase, i) => (
                    <div key={phase.name} className="flex min-w-52 flex-1 items-center gap-2.5">
                        ...
                        <Card hoverEffect={false} className="flex h-full flex-1 flex-col gap-3 p-4">
...
function perspectiveLabel(skillName: string): string {
    if (skillName.startsWith('review-script-blind-spots-')) {
        return 'Blind spots'
    }
...
function RunningReviewRow({ review }: { review: ReviewRecentReviewApi }): JSX.Element {
    ...
    return (
        <div className="flex items-center gap-3 px-4 py-3">
            ...
            <Button size="small" type="secondary" to={review.github_url} targetBlank sideIcon={<IconExternal />}>
        ...
        <div className="flex flex-col">
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                        ...
                        {review.in_progress && (
                            <Tag type="warning" size="small" className="inline-flex items-center gap-1">
            ...
            {expanded && (
                <div className="flex flex-col gap-2 border-t border-primary bg-fill-highlight-50 py-3 pl-13 pr-4">
                    ...
                    <div>
                        <Button size="small" type="secondary" onClick={() => openReviewDetail(review)}>
                            View findings
                        </Button>
                    </div>
                </div>
                ...
                {recentReviews === null || emptyAwaitingReload ? (
                    [0, 1, 2].map((i) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-3">
                            <span className="flex w-6 shrink-0 justify-center">
                                <Skeleton.Circle className="size-2" />
                            </span>
                        ...
                        {(moreReviewsAvailable || recentReviews.length > REVIEWS_PAGE_SIZE) && (
                            <div className="flex justify-center gap-2 px-4 py-1.5">
                                ...
                                {recentReviews.length > REVIEWS_PAGE_SIZE && (
                                    <Button size="small" type="tertiary" onClick={showFewerReviews}>
                                        Show fewer
                                    </Button>
                                )}
                            </div>
        ...
        <section className="flex flex-col gap-4">
            ...
            >
                <Input
                    ...
                    placeholder="https://github.com/Insights/hanzo.ai/pull/1234"
                    value={triggerPrUrl}
    ...
    return (
        <div className="flex flex-col gap-2 py-4">
            <div className="flex flex-wrap items-center gap-2">
                <Tag type={dismissed ? 'muted' : priority.type} size="small">
                    {priority.label}
                </Tag>
                ...
                panels={[
                    {
                        ...
                        header: 'Description',
                        content: (
                            <Markdown className="text-sm text-secondary" disableImages>
                                {finding.body}
                            </Markdown>
                        ),
                    },
                    ...
                    <div key={chunk.chunk_id} className="flex flex-col gap-1.5 py-3">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                            {chunk.perspectives.map((name) => (
                                <Tag key={name} type="muted" size="small">
                                    {perspectiveLabel(name)}
                                </Tag>
                            ))}
                            {chunk.skipped.map((name) => (
                                <Tag key={name} type="muted" size="small" className="line-through opacity-60">
                                    {perspectiveLabel(name)}
                                </Tag>
                            ))}
                        </div>
...
function ReviewDetailDrawer(): JSX.Element {
    ...
    return (
        <Drawer
            ...
            footer={
                review ? (
                    <Button type="secondary" to={review.github_url} targetBlank icon={<IconExternal />}>
                ...
                {perspectiveScoreboard && (
                    <div className="flex flex-wrap items-center gap-1.5 text-xs text-secondary">
                        ...
                        {perspectiveScoreboard.map(({ skillName, count }) => (
                            <Tag key={skillName} type="muted" size="small">
                                {perspectiveLabel(skillName)} {count}
                            </Tag>
                        ))}
                    </div>
                )}
                <Tabs<ReviewDrawerTab>
                        ...
                        },
                        {
                            ...
                            content: reviewDetail ? (
                                reviewDetail.report_markdown ? (
                                    <Markdown className="text-sm" disableImages>
                                        {reviewDetail.report_markdown}
                                    </Markdown>
                                ) : (
                                    <div className="text-sm text-secondary">
        ...
        <section className="flex flex-col gap-4 border-t border-primary pt-8">
            ...
            <Card hoverEffect={false} className="flex flex-col p-5">
                <Slider
                    ...
                    value={activeIndex}
                    onChange={(value) => {
                        // Slider fires per mousemove while dragging — only PATCH on a stop change.
            ...
            <div className="flex items-start justify-between gap-4">
                <div className="flex min-w-0 flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                        ...
                        <Tag type={skill.on ? 'warning' : 'muted'} size="small">
...
function EffectivenessCard({ kind }: { kind: 'perspectives' | 'blind_spots' }): JSX.Element | null {
    ...
    const items = perspectiveStats.perspectives.filter(
        (p) => p.skill_name.startsWith('review-script-blind-spots-') === (kind === 'blind_spots')
    )
    if (!items.length) {
        return null
    }
    ...
    return (
        <Card hoverEffect={false} className="flex flex-col gap-3 p-4">
                ...
                <section className="flex flex-col gap-8 border-t border-primary pt-8">
                    <div className="flex flex-col gap-1.5">
                        ...
                        <p className="m-0 max-w-160 text-sm text-secondary">
                            Everything below is a regular skill, stored in your Insights skills store like anything else
