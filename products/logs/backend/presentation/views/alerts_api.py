...
from insights.schema import LogsAlertFilters
...
class LogsAlertDeleteDestinationSerializer(serializers.Serializer):
    insights_function_ids = serializers.ListField(
        child=serializers.UUIDField(),
        min_length=1,
        max_length=len(LOGS_ALERT_EVENT_IDS),
        help_text="InsightsFunction IDs to delete as one atomic destination group.",
    )


class LogsAlertDestinationResponseSerializer(serializers.Serializer):
    insights_function_ids = serializers.ListField(child=serializers.UUIDField())
...
def _fill_empty_buckets(
    ...
    Datastore GROUP BY only returns buckets that have data. The state machine
    ...
    # Datastore returns naive datetimes; normalize to UTC for both lookup and output
    existing = {
...
class LogsAlertViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    lookup_field = "id"
    insights_feature_flag = "logs-alerting"
    ...
    @extend_schema(
        ...
        description="Create a notification destination for this alert. One InsightsFunction is created per alert event kind (firing, resolved, ...) atomically.",
    )
    ...
    def create_destination(self, request: Request, *args: object, **kwargs: object) -> Response:
        ...
        with transaction.atomic():
            ...
            insights_functions = create_alert_destination_insights_functions(configs, request=self.request)
        ...
        response = LogsAlertDestinationResponseSerializer({"insights_function_ids": [hf.id for hf in insights_functions]})
        return Response(response.data, status=201)

    @extend_schema(
        request=LogsAlertDeleteDestinationSerializer,
        responses={204: None},
        description="Delete a notification destination by deleting its InsightsFunction group atomically.",
    )
    ...
    def delete_destination(self, request: Request, *args: object, **kwargs: object) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        insights_function_ids = serializer.validated_data["insights_function_ids"]
        ...
        with transaction.atomic():
            ...
            soft_delete_alert_destinations(
                ...
                insights_function_ids=insights_function_ids,
            )

        report_user_action(
            request.user,
            "logs alert destination deleted",
            {"alert_id": str(alert.id), "count": len(insights_function_ids)},
        )
