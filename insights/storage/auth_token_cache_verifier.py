...
Scans all `insights:auth_token:*` keys in Redis, deserializes the cached
...
from insights.metrics import TOMBSTONE_COUNTER
...
from insights.storage.team_access_cache import TOKEN_CACHE_PREFIX
...
AUTH_TOKEN_VERIFY_ERRORS_COUNTER = Counter(
    "insights_auth_token_verify_errors_total",
...
def _deserialize_cache_value(raw_value: bytes | str) -> dict | None:
    """Deserialize a Redis cache value that may be pickle-wrapped JSON or raw JSON.

    The Rust service writes auth token cache entries as pickle-wrapped JSON strings
    (via RedisValueFormat::Pickle). Entries may also be optionally zstd-compressed
    when ``redis_compression_enabled`` is set. This function tries pickle first
    (the production hot path), then falls back to raw JSON and zstd+pickle.
    """
    raw_bytes = raw_value.encode("utf-8") if isinstance(raw_value, str) else raw_value

    # Try pickle first — the production hot path (Rust writes with RedisValueFormat::Pickle)
    result = _unpickle_json(raw_bytes)
    if result is not None:
        return result

    # Try raw JSON (Python-written entries and tests)
    try:
    ...
    try:
        result = _unpickle_json(zstd.ZstdDecompressor().decompress(raw_bytes))
        if result is not None:
            return result
    except Exception:
        pass
...
class _JsonOnlyUnpickler(pickle.Unpickler):
    """Unpickler that refuses to construct any class or callable.

    The auth-token cache only ever holds ``pickle(json_string)`` or ``pickle(dict)``
    (Django's cache and the Rust service both wrap a JSON payload). JSON-shaped values —
    str, dict, list, int, float, bool, None — deserialize through dedicated pickle opcodes
    that never call ``find_class``. Every code-execution path (GLOBAL/REDUCE/NEWOBJ) routes
    a class or callable through ``find_class``, so denying it blocks arbitrary-code execution
    while still loading exactly the data we expect from this internal cache.
    """

    def find_class(self, module: str, name: str) -> Any:
        # nosemgrep: python.lang.security.deserialization.pickle.avoid-pickle (deny-all guard: rejects every class/callable, never deserializes one)
        raise pickle.UnpicklingError(f"refusing to unpickle disallowed global {module}.{name}")


def _unpickle_json(raw_bytes: bytes) -> dict | None:
    """Unpickle bytes and parse the inner JSON string into a dict.

    Uses ``_JsonOnlyUnpickler`` so a hostile payload written into Redis can't execute
    code — the cache only ever holds a JSON string or a plain dict.
    """
    try:
        unpickled = _JsonOnlyUnpickler(io.BytesIO(raw_bytes)).load()
        if isinstance(unpickled, str):
            data = json.loads(unpickled)
            if isinstance(data, dict):
                return data
...
def _verify_batch(
    ...
    for key, raw_value in zip(keys, values):
        ...
        data = _deserialize_cache_value(raw_value)
        if data is None:
