#!/usr/bin/env python3
"""Regenerate the InsightsQL grammar PBT strategies module.

Reads ``insights/insightsql/grammar/InsightsQLParser.g4`` and
``insights/insightsql/grammar/InsightsQLLexer.common.g4`` and emits
``insights/insightsql/test/_generated_grammar_strategies.py``.

The output is a Python module of Hypothesis strategies — one per
parser rule — that produce syntactically valid InsightsQL strings. It's
consumed by ``test_parser_grammar_pbt.py`` to drive differential
parsing against the Python and C++ InsightsQL parsers.

Variable-content tokens (``IDENTIFIER``, ``STRING_LITERAL``, etc.)
are hand-written once in
``insights/insightsql/test/_grammar_token_strategies.py`` and referenced by
...
Modes:
    ...
    python -m insights.insightsql.scripts.build_grammar_strategies
        Regenerate and write the file.

    python -m insights.insightsql.scripts.build_grammar_strategies --check
...
Pipeline:
    ...
    4. Drop rules and alternatives that reference excluded regions —
       InsightsQLX, Script programs, template strings (apply_exclusions)
...
# ---------------------------------------------------------------------------
#
# InsightsQLX and Script-program productions are explicitly out of scope (per
...
#
# InsightsQLX tags, template strings, the lexer mode-stack tokens, and Script kv-pairs
# are all enabled. Escape-hatch strategies for their variable-content tokens
# (INSIGHTSQLX_TEXT_TEXT, QUOTE_SINGLE_TEMPLATE*, STRING_TEXT*, ESCAPE_CHAR_COMMON,
...
# / INSIGHTSQLX_TEXT_TEXT all permit whitespace within their alphabets.
EXCLUDED_RULES: frozenset[str] = frozenset()
# NB: the Script program rules (`program`, `declaration`, `statement`,
...
# are intentionally NOT excluded — the grammar PBT drives Script-program
...
_ESCAPE_HATCH_NAMES: dict[str, str] = {
    ...
    # InsightsQLX / template-string variable-content tokens. These belong to lexer
    ...
    "INSIGHTSQLX_TEXT_TEXT": "insightsqlx_text_token",
...
"""Auto-generated grammar strategies for the InsightsQL parser PBT.

DO NOT EDIT. Regenerate via::

    python -m insights.insightsql.scripts.build_grammar_strategies

The generator reads ``insights/insightsql/grammar/InsightsQLParser.g4`` and
``insights/insightsql/grammar/InsightsQLLexer.common.g4`` and emits this file.
...
from insights.insightsql.test._grammar_token_strategies import (
...
PARSER_GRAMMAR = REPO_ROOT / "insights" / "insightsql" / "grammar" / "InsightsQLParser.g4"
LEXER_GRAMMAR = REPO_ROOT / "insights" / "insightsql" / "grammar" / "InsightsQLLexer.common.g4"
OUTPUT_PATH = REPO_ROOT / "insights" / "insightsql" / "test" / "_generated_grammar_strategies.py"


def _ruff_format(source: str) -> str:
    # Pipe `source` through `ruff format -` so the codegen output exactly matches what `bin/insightscli format:python`
    ...
    if args.check:
        ...
        if existing != source:
            print(
                ...
                f"Run `python -m insights.insightsql.scripts.build_grammar_strategies` to regenerate."
            )
            return 1
