...
from insights.admin.inlines.organization_member_for_related_inline import OrganizationMemberForRelatedInline
from insights.admin.inlines.team_experiments_config_inline import TeamExperimentsConfigInline
from insights.admin.inlines.team_marketing_analytics_config_inline import TeamMarketingAnalyticsConfigInline
from insights.admin.inlines.user_product_list_inline import UserProductListInline
from insights.helpers.impersonation import is_impersonated
from insights.llm.gateway_internal_client import AIGatewayInternalError, AIGatewayNotConfigured, add_credit, get_wallet
from insights.models import Team
from insights.models.activity_logging.activity_log import ActivityContextBase, ActivityLog, Detail, log_activity
...
from insights.models.team.team import DEPRECATED_ATTRS
...
from insights.personinsights_client.proto import (
...
class TeamAdmin(admin.ModelAdmin):
    ...
    def group_type_mappings_display(self, team: Team):
        ...
        if client is None:
            return format_html("<em>personinsights client not configured</em>")
        try:
            ...
            return format_html(
                ...
                "Failed to fetch group type mappings from personinsights</p>",
            )
        mappings = []
            ...
            mappings.append(
                {
                    **m,
                    ...
                    "edit_url": reverse(
                        "admin:insights_team_edit_group_type_mapping",
        ...
        return mark_safe(
            render_to_string(
                "admin/insights/team/group_type_mappings_display.html",
                {"mappings": mappings, "team": team},
                request=getattr(self, "_current_request", None),
            )
        )

    def _get_personinsights_client_or_error(self, request, redirect_url):
        """Return (client, None) or (None, redirect_response) if personinsights is unavailable."""
        client = get_personinsights_client()
        if client is None:
            messages.error(request, "personinsights client is not configured — cannot manage group type mappings.")
            return None, redirect(redirect_url)
        return client, None

    def _fetch_group_type_mapping_via_personinsights(self, client, project_id, group_type_index):
        """Fetch a single mapping from personinsights by project_id + group_type_index. Returns dict or None."""
    ...
    def edit_group_type_mapping_view(self, request, object_id, group_type_index):
        ...
        team_url = reverse("admin:insights_team_change", args=[object_id])

        client, err_response = self._get_personinsights_client_or_error(request, team_url)
        if err_response is not None:
            return err_response

        try:
            mapping_dict = self._fetch_group_type_mapping_via_personinsights(client, team.project_id, group_type_index)
        except Exception as exc:
            logger.warning(
            ...
            messages.warning(request, "Failed to fetch group type mappings from personinsights")
        ...
        if request.method == "GET":
            ...
            return render(request, "admin/insights/team/group_type_mapping_edit.html", context)
        ...
        if default_columns_raw:
            try:
                ...
                return redirect(
                    reverse("admin:insights_team_edit_group_type_mapping", args=[object_id, group_type_index])
                )
        ...
        try:
            ...
            messages.error(request, f"Failed to update via personinsights: {exc}")
            return redirect(reverse("admin:insights_team_edit_group_type_mapping", args=[object_id, group_type_index]))
    ...
    def internal_properties(self, team: Team):
        ...
        from insights.rate_limit import team_is_allowed_to_bypass_throttle
    ...
    def api_token_display(self, team: Team):
        if not team.pk:
            return "-"
        set_url = reverse("admin:insights_team_set_api_token", args=[team.pk])
        return format_html(
    ...
    def delete_recordings(self, team: Team):
        if not team.pk:
            return "-"
        delete_url = reverse("admin:insights_team_delete_recordings", args=[team.pk])
        return format_html(
    ...
    def _refresh_ai_gateway_policy_cache(self, team: Team) -> None:
        ...
        from insights.storage.team_llm_gateway_policy_cache import update_team_llm_gateway_policy_cache
    ...
    def suspend_email_sending_view(self, request, object_id):
        ...
        suspend_url = reverse("admin:insights_team_suspend_email_sending", args=[object_id])
        team_url = reverse("admin:insights_team_change", args=[object_id])
        ...
        if request.method == "GET":
            ...
            return render(request, "admin/insights/team/suspend_email_sending_form.html", context)
    ...
    def email_sending_suspension_actions(self, team: Team):
        ...
        return mark_safe(
            render_to_string(
                "admin/insights/team/email_sending_suspension_actions.html",
                {
                    "team": team,
                    "suspend_url": reverse("admin:insights_team_suspend_email_sending", args=[team.pk]),
                    "unsuspend_url": reverse("admin:insights_team_unsuspend_email_sending", args=[team.pk]),
                    "team_name_escaped": escapejs(team.name),
    ...
    def add_ai_gateway_credit_view(self, request, object_id):
        ...
        credit_url = reverse("admin:insights_team_add_ai_gateway_credit", args=[object_id])
        ...
        if request.method == "GET":
            ...
            return render(request, "admin/insights/team/add_ai_gateway_credit_form.html", context)
    ...
    def ai_gateway_actions(self, team: Team):
        ...
        return mark_safe(
            render_to_string(
                "admin/insights/team/ai_gateway_actions.html",
                {
                    "team": team,
                    "enable_url": reverse("admin:insights_team_enable_ai_gateway", args=[team.pk]),
                    "revoke_url": reverse("admin:insights_team_revoke_ai_gateway", args=[team.pk]),
                    "clear_revoke_url": reverse("admin:insights_team_clear_ai_gateway_revoke", args=[team.pk]),
                    "team_name_escaped": escapejs(team.name),
    ...
    def ai_gateway_wallet(self, team: Team):
        ...
        return mark_safe(
            render_to_string(
                "admin/insights/team/ai_gateway_wallet_actions.html",
                {
                    "wallet_url": reverse("admin:insights_team_ai_gateway_wallet", args=[team.pk]),
                    "add_credit_url": reverse("admin:insights_team_add_ai_gateway_credit", args=[team.pk]),
                },
            )
        )
    ...
    def ai_gateway_wallet_view(self, request, object_id):
        ...
        return render(request, "admin/insights/team/ai_gateway_wallet_page.html", context)

    @admin.display(description="Policy cache blob (what the gateway sees)")
    def policy_cache_blob(self, team: Team):
        if not team.pk:
            return "-"
        from insights.storage.team_llm_gateway_policy_cache import get_team_llm_gateway_policy_from_redis
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                ...
                name="insights_team_edit_group_type_mapping",
            ),
            path(
    ...
    def set_api_token_view(self, request, object_id):
        ...
        if request.method == "GET":
            ...
            return render(request, "admin/insights/team/set_api_token_form.html", context)

        new_token = request.POST.get("new_token", "").strip()
        if not new_token:
            messages.error(request, "New API token is required")
            return redirect(reverse("admin:insights_team_set_api_token", args=[object_id]))
    ...
    def _get_delete_workflows(self, team_id: int) -> list[dict]:
        ...
        try:
            ...
            # Use the InsightsTeamId search attribute (indexed) instead of WorkflowId range scan.
            ...
            query = f"InsightsTeamId = {team_id} AND ({type_clauses})"
    ...
    def delete_recordings_workflows_fragment(self, request, object_id):
        ...
        return render(request, "admin/insights/team/_delete_recordings_workflows.html", context)
    ...
    def delete_recordings_view(self, request, object_id):
        ...
        if request.method == "GET":
            ...
            return render(request, "admin/insights/team/delete_recordings.html", context)
        ...
        if not reason:
            messages.error(request, "Reason is required")
            return redirect(reverse("admin:insights_team_delete_recordings", args=[object_id]))
    ...
    def deletion_certificate_view(self, request, object_id, workflow_id):
        ...
        return render(request, "admin/insights/team/deletion_certificate.html", context)
