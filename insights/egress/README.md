...
General-purpose controls for the calls Insights makes _out_ to third-party APIs.
...
- **`limiter/`** — shared, Redis-backed budgets so every worker process draws from one limit and Insights stays inside an external API's rate limit, with priority lanes so bulk traffic can't starve critical traffic.
...
This is _outbound_ egress — what Insights sends.
It is unrelated to `insights.rate_limit`, which throttles _inbound_ DRF requests from clients.
...
from insights.egress.github.limiter import consume_github_installation_sync
...
Icon bytes are never stored server-side (logo.dev licenses that separately), so steady-state traffic is deduped only by browser caching (`insights/cdp/services/icons.py` sets `Cache-Control`) and tracks unique (user, icon) first views per day — raise the settings if that outgrows the defaults.
...
It's **token-agnostic** (installation token, user token, PAT, or Insights's shared token) and stateless.
...
It is **never** a Insights DB row id (`Integration.id`).
Several Insights integration rows can point at the same installation (multiple projects, one org), and GitHub gives that installation one shared budget: key a gauge by the row and one real budget splits into N flip-flopping series; key by the installation and you get one true series.
...
The important one is a warehouse source authenticated with a **personal access token (PAT)**: a customer's own token on the customer's own budget, disconnected from any Insights installation (error tracking's public-repo path is the other).
...
Keep the identity in the external API's id space, keep the subpackage free of `insights.models` imports, and remember the limiter is non-blocking — the caller owns the back-off.
