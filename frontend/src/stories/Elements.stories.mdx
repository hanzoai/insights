...
Elements is a suite of building blocks powering Hanzo Insights frontend. These components are abstract in nature so they
...
Elements has grown as a replacement for [Ant Design](https://ant.design/), from which we've started transitioning away
...
- `Button` – instead go for `Button`
- `Select` – instead go for `Select`
- `Input`, `Input.Password`, `Input.Search` – instead go for `Input`
- `Input.TextArea` – instead go for `TextArea`
- `Table` – instead go for `Table`
- `Checkbox` – instead go for `Checkbox`
- `Spin` – instead go for `Spinner`
- `Alert` – instead go for `Banner`
- `Typography` – just use HTML elements
- `Modal` – instead go for `Modal`
- `Divider` – instead go for `Divider`
- `Row` – instead go for `Row` (for standard rows of content) or roll your own `display: flex` as needed (for general layout)
