### personinsights-router context

### Requirements

- defines the API contract for all external personinsights clients to consume
- provides a stateless/dependency-less routing path to personinsights-replica pods
- participates in the handoff protocol to ensure requests are correctly and efficiently routed to personinsights-leader pods
...
- consistently/correctly/efficiently route requests to personinsights-leader pods
- personinsights-leader client installed on service to consume personinsights-leader API
- define API contract that allows clients to consume strongly consistent/write to person state (consume personinsights-leader BE capabilities)

### Implementation Details

#### personinsights-replica routing
...
always routes to personinsights-replica regardless of consistency level or operation type.
...
- `EVENTUAL` or unset → routes to personinsights-replica
- `STRONG` → returns `UNIMPLEMENTED` (requires personinsights-leader)
- Writes → returns `UNIMPLEMENTED` (requires personinsights-leader)

#### personinsights-leader routing

Person writes and strong reads route to the partition's owning leader pod. The
router holds a per-partition routing table fed by `personinsights-coordination`'s
...
- **Direct path** (`forward_or_stash`): a bounced request is held in its handler
  ...
  counted by `personinsights_router_forward_retries_total` under the reason that
  caused it. After a few consecutive bounces it fails with a definitive
  `UNAVAILABLE`, counted by `personinsights_router_forward_retries_exhausted_total`
  ...
  replay, covered by the redelivery contract in `personinsights-leader`'s README.
...
(see `personinsights-coordination`'s README for the full protocol). Routers
...
3. **Bounce and retry**: a classified attempt that concludes no outcome
   ...
   `personinsights_router_forward_retries_total` as a stash-path retry under
   ...
   replay, counted by `personinsights_router_stash_replayed_total` and
   covered by the redelivery contract in `personinsights-leader`'s README
