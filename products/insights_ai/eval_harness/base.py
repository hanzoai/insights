...
def _log_conversation_spans(hooks: CaseHooks, parsed: ParsedLog) -> None:
    ...
    Uses the same Anthropic-format messages as Insights trace capture.
    """
...
class _BaseEvalRun:
    ...
    def __init__(
        ...
        self.insights_client = ctx.insights_client
        ...
        # Wrap scorers with tracing if Insights client is available
        self.scorer_traces: dict[tuple[str, str], str] = {}
        if self.insights_client:
    ...
    async def _finalize(self, result: ExperimentResult) -> None:
        """Append scores to local summaries, emit Insights evaluation/trace-root
        ...
        # Emit evaluation events and trace roots to Insights (after scoring)
        if self.insights_client and result.results:
            try:
                emit_evaluation_events(
                    self.insights_client, self.experiment_id, self.experiment_name, result.results, self.scorer_traces
                )
                ...
                self.insights_client.flush()
                await self.ctx.reporter.record_insights_evaluations_url(self.experiment_name, self.experiment_id)
            except Exception:
...
class _SandboxedEvalRun(_BaseEvalRun):
    ...
    async def _run_sandbox_window(
        ...
        async with self._sandbox_slots:
            # Team cloning and some case seeders both write to Datastore. Bound
            ...
            # nor the Datastore copy can consume it.
    ...
    async def _post_process(
        ...
        # Parse the log once, use for both Braintrust spans and Insights trace capture
        last_message = ""
