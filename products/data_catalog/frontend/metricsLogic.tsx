import { MakeLogicType, actions, afterMount, kea, listeners, path, reducers, selectors } from 'kea'
import { loaders } from 'kea-loaders'
import { router } from 'kea-router'

import api, { ApiConfig, ApiError } from 'lib/api'
import { toast } from 'lib/elements/Toast/Toast'
import { createFuse } from 'lib/utils/fuseSearch'
import { urls } from 'scenes/urls'

import {
    dataCatalogMetricsApproveCreate,
    dataCatalogMetricsCreate,
    dataCatalogMetricsDestroy,
    dataCatalogMetricsList,
    dataCatalogMetricsRefreshFromInsightCreate,
} from './generated/api'
import type { DataCatalogMetricApi } from './generated/api.schemas'

export type MetricStatusFilter = 'all' | 'proposed' | 'approved'

export interface MetricsFilters {
    search: string
    status: MetricStatusFilter
}

export const DEFAULT_METRICS_FILTERS: MetricsFilters = {
    search: '',
    status: 'all',
}

export type NewMetricDefinitionType = 'markdown' | 'insight' | 'sql'

export interface NewMetricForm {
    name: string
    display_name: string
    description: string
    unit: string
    definitionType: NewMetricDefinitionType
    markdown: string
    sourceInsightShortId: string
}

export const EMPTY_NEW_METRIC_FORM: NewMetricForm = {
    name: '',
    display_name: '',
    description: '',
    unit: '',
    definitionType: 'sql',
    markdown: '',
    sourceInsightShortId: '',
}

export interface SavedInsightOption {
    short_id: string
    label: string
}

export interface MetricFromInsightRequest {
    name: string
    display_name: string
    description: string
    source_insight_short_id: string
}

const MARKDOWN_DEFINITION_KIND = 'MarkdownDefinition'

function projectId(): string {
    return String(ApiConfig.getCurrentTeamId())
}

function buildMarkdownDefinition(markdown: string): Record<string, unknown> {
    return { kind: MARKDOWN_DEFINITION_KIND, markdown }
}

function apiErrorDetail(error: unknown): string | null {
    return error instanceof ApiError ? error.detail : null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface metricsLogicValues {
    actionsInFlight: Record<string, boolean>
    allMetrics: DataCatalogMetricApi[]
    allMetricsLoading: boolean
    filters: MetricsFilters
    insightSearch: string
    isCreatingMetric: boolean
    metricFromInsightModalOpen: boolean
    metrics: DataCatalogMetricApi[]
    newMetricForm: NewMetricForm
    newMetricModalOpen: boolean
    proposedCount: number
    savedInsights: SavedInsightOption[]
    savedInsightsLoading: boolean
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface metricsLogicActions {
    approveMetric: (name: string) => {
        name: string
    }
    closeMetricFromInsightModal: () => {
        value: true
    }
    closeNewMetricModal: () => {
        value: true
    }
    createMetric: () => {
        value: true
    }
    createMetricFromInsight: (request: MetricFromInsightRequest) => {
        request: MetricFromInsightRequest
    }
    deleteMetric: (name: string) => {
        name: string
    }
    loadMetrics: () => any
    loadMetricsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadMetricsSuccess: (
        allMetrics: DataCatalogMetricApi[],
        payload?: any
    ) => {
        allMetrics: DataCatalogMetricApi[]
        payload?: any
    }
    loadSavedInsights: () => {
        value: true
    }
    loadSavedInsightsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadSavedInsightsSuccess: (
        savedInsights: {
            label: string
            short_id: string
        }[],
        payload?: {
            value: true
        }
    ) => {
        savedInsights: {
            label: string
            short_id: string
        }[]
        payload?: {
            value: true
        }
    }
    openMetricFromInsightModal: () => {
        value: true
    }
    openNewMetricModal: () => {
        value: true
    }
    openSqlEditorForNewMetric: () => {
        value: true
    }
    refreshMetricFromInsight: (name: string) => {
        name: string
    }
    setActionInFlight: (
        name: string,
        inFlight: boolean
    ) => {
        inFlight: boolean
        name: string
    }
    setCreatingMetric: (creating: boolean) => {
        creating: boolean
    }
    setFilters: (filters: Partial<MetricsFilters>) => {
        filters: Partial<MetricsFilters>
    }
    setInsightSearch: (search: string) => {
        search: string
    }
    setNewMetricForm: (form: Partial<NewMetricForm>) => {
        form: Partial<NewMetricForm>
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface metricsLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        metrics: (allMetrics: DataCatalogMetricApi[], filters: MetricsFilters) => DataCatalogMetricApi[]
        proposedCount: (allMetrics: DataCatalogMetricApi[]) => number
    }
}

export type metricsLogicType = MakeLogicType<
    metricsLogicValues,
    metricsLogicActions,
    Record<string, any>,
    metricsLogicMeta
>

export const metricsLogic = kea<metricsLogicType>([
    path(['products', 'data_catalog', 'frontend', 'metricsLogic']),
    actions({
        setFilters: (filters: Partial<MetricsFilters>) => ({ filters }),
        openNewMetricModal: true,
        openSqlEditorForNewMetric: true,
        closeNewMetricModal: true,
        setNewMetricForm: (form: Partial<NewMetricForm>) => ({ form }),
        setInsightSearch: (search: string) => ({ search }),
        loadSavedInsights: true,
        createMetric: true,
        setCreatingMetric: (creating: boolean) => ({ creating }),
        approveMetric: (name: string) => ({ name }),
        refreshMetricFromInsight: (name: string) => ({ name }),
        deleteMetric: (name: string) => ({ name }),
        setActionInFlight: (name: string, inFlight: boolean) => ({ name, inFlight }),
        openMetricFromInsightModal: true,
        closeMetricFromInsightModal: true,
        createMetricFromInsight: (request: MetricFromInsightRequest) => ({ request }),
    }),
    loaders(({ values }) => ({
        allMetrics: [
            [] as DataCatalogMetricApi[],
            {
                loadMetrics: async () => {
                    const response = await dataCatalogMetricsList(projectId())
                    return response.results || []
                },
            },
        ],
        savedInsights: [
            [] as SavedInsightOption[],
            {
                // Search server-side so insights beyond the first page are still selectable.
                loadSavedInsights: async (_, breakpoint) => {
                    await breakpoint(300)
                    const response = await api.insights.list({
                        saved: true,
                        basic: true,
                        limit: 100,
                        search: values.insightSearch || undefined,
                    })
                    breakpoint()
                    return (response.results || [])
                        .filter((insight) => !!insight.short_id)
                        .map((insight) => ({
                            short_id: insight.short_id as string,
                            label: insight.name || insight.derived_name || (insight.short_id as string),
                        }))
                },
            },
        ],
    })),
    reducers({
        filters: [
            DEFAULT_METRICS_FILTERS as MetricsFilters,
            {
                setFilters: (state, { filters }) => ({ ...state, ...filters }),
            },
        ],
        newMetricModalOpen: [
            false,
            {
                openNewMetricModal: () => true,
                closeNewMetricModal: () => false,
            },
        ],
        newMetricForm: [
            EMPTY_NEW_METRIC_FORM as NewMetricForm,
            {
                setNewMetricForm: (state, { form }) => ({ ...state, ...form }),
                openNewMetricModal: () => EMPTY_NEW_METRIC_FORM,
                closeNewMetricModal: () => EMPTY_NEW_METRIC_FORM,
            },
        ],
        isCreatingMetric: [
            false,
            {
                setCreatingMetric: (_, { creating }) => creating,
            },
        ],
        metricFromInsightModalOpen: [
            false,
            {
                openMetricFromInsightModal: () => true,
                closeMetricFromInsightModal: () => false,
            },
        ],
        actionsInFlight: [
            {} as Record<string, boolean>,
            {
                setActionInFlight: (state, { name, inFlight }) => ({ ...state, [name]: inFlight }),
            },
        ],
        insightSearch: [
            '',
            {
                setInsightSearch: (_, { search }) => search,
                openNewMetricModal: () => '',
                closeNewMetricModal: () => '',
            },
        ],
    }),
    selectors({
        metrics: [
            (s) => [s.allMetrics, s.filters],
            (allMetrics: DataCatalogMetricApi[], filters: MetricsFilters) => {
                let results = allMetrics
                if (filters.status !== 'all') {
                    results = results.filter((metric) => metric.status === filters.status)
                }
                if (filters.search) {
                    const fuse = createFuse<DataCatalogMetricApi>(results, {
                        keys: ['name', 'display_name', 'description'],
                        threshold: 0.3,
                    })
                    results = fuse.search(filters.search).map((result) => result.item)
                }
                return results
            },
        ],
        proposedCount: [
            (s) => [s.allMetrics],
            (allMetrics: DataCatalogMetricApi[]) => allMetrics.filter((metric) => metric.status === 'proposed').length,
        ],
    }),
    listeners(({ values, actions }) => ({
        openNewMetricModal: () => actions.loadSavedInsights(),
        setInsightSearch: () => actions.loadSavedInsights(),
        openSqlEditorForNewMetric: () => {
            actions.closeNewMetricModal()
            router.actions.push(urls.sqlEditor({ source: 'metric' }))
        },
        createMetric: async () => {
            if (values.isCreatingMetric) {
                return
            }
            const form = values.newMetricForm
            if (form.definitionType === 'sql') {
                // SQL metrics are defined from the SQL editor, so the else branch below only handles markdown.
                toast.error('Create SQL metrics from the SQL editor.')
                return
            }
            actions.setCreatingMetric(true)
            try {
                const created = await dataCatalogMetricsCreate(projectId(), {
                    name: form.name,
                    display_name: form.display_name || undefined,
                    description: form.description,
                    unit: form.unit || undefined,
                    ...(form.definitionType === 'insight'
                        ? { source_insight_short_id: form.sourceInsightShortId }
                        : { definition: buildMarkdownDefinition(form.markdown) }),
                    created_source: 'user',
                })
                actions.loadMetricsSuccess([created, ...values.allMetrics])
                actions.closeNewMetricModal()
                toast.success('Metric created')
            } catch (error) {
                toast.error(
                    apiErrorDetail(error) || 'Could not create the metric. Check the fields and try again.'
                )
            } finally {
                actions.setCreatingMetric(false)
            }
        },
        createMetricFromInsight: async ({ request }) => {
            if (values.isCreatingMetric) {
                return
            }
            actions.setCreatingMetric(true)
            try {
                const created = await dataCatalogMetricsCreate(projectId(), {
                    name: request.name,
                    display_name: request.display_name || undefined,
                    description: request.description,
                    source_insight_short_id: request.source_insight_short_id,
                })
                actions.loadMetricsSuccess([created, ...values.allMetrics])
                actions.closeMetricFromInsightModal()
                toast.success('Metric created from insight', {
                    button: {
                        label: 'View metric',
                        action: () => router.actions.push(urls.dataCatalogMetric(created.name)),
                    },
                })
            } catch (error) {
                toast.error(apiErrorDetail(error) || 'Could not create the metric from this insight. Try again.')
            } finally {
                actions.setCreatingMetric(false)
            }
        },
        approveMetric: async ({ name }) => {
            if (values.actionsInFlight[name]) {
                return
            }
            actions.setActionInFlight(name, true)
            try {
                const approved = await dataCatalogMetricsApproveCreate(projectId(), name)
                actions.loadMetricsSuccess(
                    values.allMetrics.map((metric) => (metric.name === name ? approved : metric))
                )
            } catch (error) {
                if (error instanceof ApiError && error.status === 409) {
                    toast.error(
                        'This metric has drifted from its source insight and cannot be approved. Refresh it first.',
                        {
                            button: {
                                label: 'Refresh from insight',
                                action: () => actions.refreshMetricFromInsight(name),
                            },
                        }
                    )
                } else {
                    toast.error(apiErrorDetail(error) || 'Could not approve the metric. Try again.')
                }
            } finally {
                actions.setActionInFlight(name, false)
            }
        },
        refreshMetricFromInsight: async ({ name }) => {
            if (values.actionsInFlight[name]) {
                return
            }
            actions.setActionInFlight(name, true)
            try {
                const refreshed = await dataCatalogMetricsRefreshFromInsightCreate(projectId(), name)
                actions.loadMetricsSuccess(
                    values.allMetrics.map((metric) => (metric.name === name ? refreshed : metric))
                )
                toast.success('Metric refreshed from its source insight')
            } catch (error) {
                toast.error(
                    apiErrorDetail(error) || 'Could not refresh the metric from its source insight. Try again.'
                )
            } finally {
                actions.setActionInFlight(name, false)
            }
        },
        deleteMetric: async ({ name }) => {
            if (values.actionsInFlight[name]) {
                return
            }
            actions.setActionInFlight(name, true)
            try {
                await dataCatalogMetricsDestroy(projectId(), name)
                actions.loadMetricsSuccess(values.allMetrics.filter((metric) => metric.name !== name))
                toast.success('Metric deleted')
            } catch (error) {
                toast.error(apiErrorDetail(error) || 'Could not delete the metric. Try again.')
            } finally {
                actions.setActionInFlight(name, false)
            }
        },
    })),
    afterMount(({ actions }) => {
        actions.loadMetrics()
    }),
])
