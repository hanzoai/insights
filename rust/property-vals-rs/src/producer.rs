...
pub struct AggregatedProducer {
    ...
    // by Datastore and must stay `None`.
...
mod tests {
    ...
    fn partition_key_is_stable_and_tuple_specific() {
        let a = partition_key(&outgoing("https://hanzo.ai/a"));
        let b = partition_key(&outgoing("https://hanzo.ai/a"));
        let c = partition_key(&outgoing("https://hanzo.ai/b"));
        assert_eq!(a, b);
        assert_ne!(a, c);
        assert_eq!(a.len(), 16);
    }
