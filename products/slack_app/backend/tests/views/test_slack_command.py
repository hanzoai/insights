...
``/insights`` surface to the command workflow.
"""
...
SIGNING_SECRET = "insights-code-test-secret"
...
class TestSlashCommandDispatch(_SlashCommandTestBase):
    ...
    @parameterized.expand(
        [
            ...
            # A bare ``/insights`` (empty text) is treated as ``/insights help``.
    ...
    def test_known_subcommand_starts_command_workflow(self, _name: str, text: str, expected_event_text: str) -> None:
        ...
        # match the entry point so the strings tell users to type ``/insights ...``.
        assert self.mock_start.call_args.kwargs["command_prefix"] == "/insights"
