...
def _validate_datastore_team_query(sql: str) -> None:
    if "%(team_ids)s" not in sql:
        raise ValueError("Health Datastore queries must include a %(team_ids)s placeholder")


def execute_datastore_health_team_query(
    sql: str,
    *,
    team_ids: list[int],
    ...
    _validate_datastore_team_query(sql)
    ...
    logger.info("running health datastore query", team_count=len(team_ids))
