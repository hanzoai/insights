...
from insights.test.base import DatastoreTestMixin, snapshot_datastore_queries
...
class TestQueryStatusManager(SimpleTestCase):
    ...
    def test_store_datastore_query_progress(self):
        ...
        self.manager._store_datastore_query_progress_dict(query_status)
        self.assertEqual(self.manager._get_datastore_query_progress_dict(), query_status)
    ...
    def test_update_datastore_query_progresses(self):
        self.manager.store_query_status(self.query_status)
        ...
        self.manager._store_datastore_query_progress_dict(query_progress_dict)
        self.manager.update_datastore_query_progresses(
            [
                {
                    **ZERO_PROGRESS,
                    "bytes_read": 10,
        ...
        self.query_status.query_progress = DatastoreQueryProgress(**{**ZERO_PROGRESS, "bytes_read": 31})
...
class DatastoreClientTestCase(TestCase, DatastoreTestMixin):
    ...
    def test_async_query_client_is_lazy(self, execute_process_query_mock):
        ...
        # Assert that we only called datastore once
    ...
    def test_async_query_client_is_lazy_but_not_too_lazy(self, execute_process_query_mock):
        ...
        # Assert that we called datastore twice
    ...
    @patch("insights.datastore.client.async_task_chain.execute_task_chain")
    @patch("django.db.transaction.on_commit")
    def test_context_manager_exit(self, on_commit_mock, execute_task_chain_mock):
        mock_chain: list[Any] = []
        with patch("insights.datastore.client.async_task_chain.get_task_chain", return_value=mock_chain):
