...
export class StateManager {
    ...
    private async _fetchApiKey(): Promise<NonNullable<State['apiKey']>> {
        ...
        if (sanitizedClientName) {
            ...
            // `x-insights-mcp-oauth-client-name` instead of waiting for the next cache hit.
    ...
    private _isRecoverableNotFound(error: unknown): boolean {
        return error instanceof InsightsApiError && error.status === 404
    }
     ...
     * `organization` and `project` Insights groups. Reuses the cached user/org/project
