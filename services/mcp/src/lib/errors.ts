...
function formatMissingProjectContextMessage(organizationId: string | undefined): string {
    ...
    return (
        'No Insights project is selected for this MCP session, and a default could not be derived from your API key.' +
        orgScopeLine +
        '\n\n' +
        ...
        '3. (For MCP client maintainers) Pin a project at session start by sending the `x-insights-project-id` header on the initialize request.' +
        '\n\n' +
...
function formatMissingOrganizationContextMessage(): string {
    return (
        'No Insights organization is selected for this MCP session, and a default could not be derived from your API key.' +
        '\n\n' +
        'To pick one (in order of preference):\n' +
        '1. Call `organizations-get` to list organizations you can access, then `switch-organization` with the chosen organization id.\n' +
        '2. (For MCP client maintainers) Pin an organization at session start by sending the `x-insights-organization-id` header on the initialize request.'
    )
}

export interface InsightsValidationErrorOptions {
    detail: string
...
/**
 * Thrown when the Insights API rejects a request with a `validation_error`
 ...
 * example, the InsightsQL metadata response attached to a failed /query/ call.
 */
export class InsightsValidationError extends Error {
    ...
    constructor(options: InsightsValidationErrorOptions) {
...
export interface InsightsApiErrorOptions {
    status: number
    statusText: string
    body: string
    url: string
...
/**
 * Thrown when the Insights API rejects a request with a non-success status that
 * isn't already handled by a more specific subclass (InsightsPermissionError,
 * InsightsValidationError). Carries the status code so callers can distinguish
...
export class InsightsApiError extends Error {
    ...
    constructor(options: InsightsApiErrorOptions) {
...
function buildDefaultApiErrorMessage(options: InsightsApiErrorOptions): string {
    return `Request failed:\nURL: ${options.method} ${options.url}\nStatus Code: ${options.status} (${options.statusText})\nError Message: ${options.body}`
}

export interface InsightsRateLimitErrorOptions {
    body: string
    url: string
    method: string
...
/**
 * Thrown when the Insights API responds with HTTP 429. Never retried inside the
 ...
 */
export class InsightsRateLimitError extends InsightsApiError {
    public readonly retryAfterSeconds: number | null

    constructor(options: InsightsRateLimitErrorOptions) {
        ...
        super({
            ...
            message: `Insights API rate limit exceeded (429) on ${options.method} ${options.url}.${retryHint}`,
        })
...
export interface InsightsPermissionErrorOptions {
    detail: string
...
/**
 * Thrown when the Insights API rejects a request with HTTP 403 `permission_denied`.
...
export class InsightsPermissionError extends Error {
    ...
    constructor(options: InsightsPermissionErrorOptions) {
        const message = options.missingScope
            ? `Missing Insights API scope: '${options.missingScope}'`
            : `Insights API permission denied: ${options.detail}`
        super(message)
...
const PERSONAL_API_KEY_DOCS_URL = 'https://hanzo.ai/docs/api#how-to-authenticate-with-the-insights-api'

export function formatPermissionErrorMessage(error: InsightsPermissionError): string {
    const callTarget = error.method && error.url ? `${error.method} ${error.url}` : 'this MCP request'
    if (error.missingScope) {
        return [
            `Missing Insights API scope: '${error.missingScope}'`,
            '',
            `Your Personal API key is missing the '${error.missingScope}' scope, which is required to call ${callTarget}.`,
            '',
            `To fix: edit the Personal API key in Insights (User settings → Personal API keys) and add the '${error.missingScope}' scope. Alternatively, select the "MCP Server" scope preset which includes every scope the MCP needs.`,
            '',
    ...
    return [
        `Insights API permission denied: ${error.detail}`,
        '',
 ...
 */
export function buildInsufficientScopeChallenge(error: InsightsPermissionError): string {
...
// Literal message shape produced by `new InsightsPermissionError({...})`. We
...
// `findInsightsPermissionError` for the full reasoning.
const MISSING_SCOPE_MESSAGE_PATTERN = /Missing Insights API scope: ['"]([^'"]+)['"]/

/**
 * Walk `Error.cause` chains to find a wrapped InsightsPermissionError.
 ...
 * InsightsPermissionError prototype, and any custom own-properties have all
 ...
 *
 * The literal `Missing Insights API scope: 'X'` shape produced by the
 * `InsightsPermissionError` constructor is the one piece of information that
 ...
 */
export function findInsightsPermissionError(error: unknown): InsightsPermissionError | undefined {
    let current: unknown = error
    const seen = new Set<unknown>()
    while (current && !seen.has(current)) {
        if (current instanceof InsightsPermissionError) {
            return current
        }
        seen.add(current)
    ...
    if (error instanceof Error && typeof error.message === 'string') {
        ...
        if (match) {
            ...
            return new InsightsPermissionError({
...
/**
 * Walks `Error.cause` chains to find a wrapped InsightsApiError or
 * InsightsValidationError. Tool-level wrappers (e.g. `throw new Error("Failed
 * to X: ...", { cause })`) hide the underlying typed error, so callers must
 * unwrap before classifying the failure.
 */
export function findRecoverableApiError(error: unknown): InsightsApiError | InsightsValidationError | undefined {
    let current: unknown = error
    const seen = new Set<unknown>()
    while (current && !seen.has(current)) {
        if (current instanceof InsightsApiError || current instanceof InsightsValidationError) {
            return current
        }
        seen.add(current)
        current = current instanceof Error ? (current as Error & { cause?: unknown }).cause : undefined
...
export function handleToolError(error: any, tool?: string, distinctId?: string, sessionUuid?: string): CallToolResult {
    ...
    // Recoverable: 4xx responses from the Insights API (and validation errors)
    ...
    if (recoverableApiError) {
        const isFourXx =
            recoverableApiError instanceof InsightsValidationError ||
    ...
    const permissionError = findInsightsPermissionError(error)
    if (permissionError) {
        const properties: Record<string, any> = {
            ...
            $exception_fingerprint: `insights-permission-error:${toolName}:${permissionError.missingScope ?? 'unknown'}`,
        }
        ...
        try {
            getInsightsClient().captureException(permissionError, distinctId, properties)
        } catch {
    ...
    const recoveryHint =
        recoverableApiError instanceof InsightsApiError
