...
One exception to the Django-plus-React shape: `products/desktop/` is the Insights desktop app (Electron, plus mobile and web hosts), imported from the Insights/code repo. It is a nested standalone pnpm workspace with its own lockfile, Node version and Biome toolchain, deliberately excluded from the root pnpm workspace, with its own `desktop-*` CI. Its `AGENTS.md` covers the architecture and its `MIGRATION.md` covers the resync contract with Insights/code. Day to day, drive it through `insightscli desktop:*` commands, or `cd products/desktop` and use pnpm directly.
