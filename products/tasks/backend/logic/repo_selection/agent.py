...
from insights.models.github_integration_base import INSTALLATION_UNAVAILABLE_SINCE_CONFIG_KEY, GitHubIntegrationBase
from insights.models.integration import ERROR_TOKEN_REFRESH_FAILED, GitHubIntegration, Integration
from insights.models.integration_repository_cache import GitHubRepositoryFullCache
...
REPO_SELECTION_DUMMY_REPOSITORY = "Insights/.github"
...
def _first_user_github_integration(user_ids: Iterable[int]) -> UserIntegration | None:
    ...
    merely belongs to (a ``insights/*`` org the user happens to work in must not outrank their own
...
in `system.integration_repository_cache`. Use the Insights `execute-sql` tool to query it.

Schema:

- `full_name` (text, lowercased): repo identifier, e.g. `insights/insights`.
...
- **Concrete identifiers** (file paths, file extensions, function/class names, error messages,
  ...
  WHERE full_name IN ('insights/insights', 'insights/insights-js')
...
- **Domain-level signals** (customer support frustration, vague feature requests, "the product
  ...
  FROM system.integration_repository_cache
  WHERE full_name IN ('insights/insights', 'insights/hanzo.ai')
...
Request: "Session replay is crashing on mobile after upgrading Insights Android 4.2.0 → 4.3.0 and
Insights iOS 3.18.0 → 3.19.0."
Candidates include `insights/insights-android` and `insights/insights-ios`.
...
FROM system.integration_repository_cache
ARRAY JOIN splitByString('\\n', tree_paths) AS path
WHERE full_name IN ('insights/insights-android', 'insights/insights-ios')
...
Request: "I'm running self-hosted Insights FOSS edition and need MP4 export for session replays.
Is this in the open-source build?"
Candidates include `insights/insights` and `insights/insights-foss`.
...
FROM system.integration_repository_cache
ARRAY JOIN splitByString('\\n', tree_paths) AS path
WHERE full_name IN ('insights/insights', 'insights/insights-foss')
  AND path ILIKE '%mp4%'
```
...
Request: "The marketing homepage at hanzo.ai loads slowly on mobile."
Candidates include `insights/hanzo.ai`, `insights/insights-js`, `insights/insights-android`.

Only `hanzo.ai` is the marketing site; the others are SDKs. Pick `insights/hanzo.ai`
directly. `reason`: "Only candidate that owns the marketing site domain; `insights-js` and
`insights-android` are SDKs and don't host the homepage."
...
async def select_repository(
    ...
    *,
    ...
    sandbox_context = CustomPromptSandboxContext(
        ...
        # Read-only Insights scopes so the agent can call `execute-sql` against `system.integration_repository_cache`.
