...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
@patch("insights.temporal.data_imports.pipelines.pipeline.cdp_producer.aget_s3_client")
...
@pytest.mark.asyncio
@patch("insights.temporal.data_imports.pipelines.pipeline.cdp_producer.aget_s3_client")
@patch("insights.temporal.data_imports.pipelines.pipeline.cdp_producer.capture_exception")
...
async def test_write_chunk_for_cdp_producer(team):
    ...
    with patch("insights.temporal.data_imports.pipelines.pipeline.cdp_producer.write_table") as mock_write_table:
...
async def test_serialize_json_fallback_to_standard_json(team):
    ...
    with patch("insights.temporal.data_imports.pipelines.pipeline.cdp_producer.orjson.dumps") as mock_orjson:
