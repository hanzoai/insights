...
const SLACK_LINK_ERROR_MESSAGES: Record<string, string> = {
    ...
    workspace_not_found: 'The Slack workspace is no longer connected to Insights.',
    ...
    org_mismatch: "You aren't a member of the Insights organization connected to this Slack workspace.",
    session_mismatch:
        'This Slack link was started in a different Insights session. Please start the link again from settings.',
    not_configured: 'Slack is not configured for this Insights instance.',
}
 ...
 *
 * The install flow leaves hanzo.ai for github.com and comes back, which drops the query
...
export const personalIntegrationsLogic = kea<personalIntegrationsLogicType>([
                ...
                connectSlack: async (payload: { workspace?: LinkableSlackWorkspace } = {}) => {
                    try {
                        const body = payload.workspace
                            ? {
                                  team_id: payload.workspace.insights_team_id,
                        ...
                        const message =
                            error instanceof Error
                                ? (error as any).detail ||
                                  ...
                                  error.message
                                : undefined
                        toast.error(message || 'Could not start Slack linking.')
                        return false
                    }
                },
            },
    ...
    listeners(({ actions, values }) => ({
        ...
        disconnectSlack: async ({ slackUserId }) => {
            try {
                ...
                toast.success('Unlinked your Slack account')
                ...
                toast.error('Could not unlink your Slack account.')
            }
        },
        connectGitHub: async () => {
            try {
                const connectFrom = readConnectFromStorage()
                const body: { connect_from?: 'insights_code'; team_id?: number } = {}
                if (connectFrom === 'insights_code') {
                    body.connect_from = 'insights_code'
                }
                ...
                toast.error(message || 'Could not start GitHub installation.')
            }
        },
        disconnectGitHub: async ({ installationId }) => {
            try {
                await usersIntegrationsGithubDestroy('@me', installationId)
                toast.success('Disconnected GitHub installation')
                actions.loadIntegrations()
                actions.loadProjectIntegrations()
            } catch {
                toast.error('Could not disconnect GitHub installation.')
            }
        },
    })),
    ...
    events(({ actions }) => ({
        afterMount: () => {
            ...
            // "Return to Insights Desktop" CTA.
            ...
            if (params.has('github_link_success')) {
                ...
                writeConnectFromStorage(null)
                toast.error(describeGithubLinkError(params.get('github_link_error')))
            }
            ...
            if (params.has('slack_link_success')) {
                ...
                toast.error(SLACK_LINK_ERROR_MESSAGES[reason] ?? SLACK_LINK_ERROR_FALLBACK)
            }
        },
    })),
])
