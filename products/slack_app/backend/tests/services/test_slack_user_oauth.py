...
class TestFindLinkedInsightsUser:
    def test_returns_linked_user_when_scoped_org_matches(self, org_team_user, link_user):
        ...
        found = find_linked_insights_user(
    ...
    def test_returns_none_when_team_id_does_not_match(self, org_team_user, link_user):
        ...
        link_user(user)
        assert (
            find_linked_insights_user(slack_user_id=SLACK_USER_ID, slack_team_id="T-OTHER", candidate_org_ids={org.id})
            is None
        )
    ...
    def test_empty_inputs_return_none(self):
        ...
        assert (
            find_linked_insights_user(slack_user_id="", slack_team_id=SLACK_TEAM_ID, candidate_org_ids={sentinel_org_id})
            is None
        )
        assert (
            find_linked_insights_user(slack_user_id=SLACK_USER_ID, slack_team_id="", candidate_org_ids={sentinel_org_id})
            is None
        )
        assert (
            find_linked_insights_user(slack_user_id=SLACK_USER_ID, slack_team_id=SLACK_TEAM_ID, candidate_org_ids=set())
            is None
        )

    def test_multiple_linked_users_returns_most_recent_accessible(self, org_team_user, link_user):
        org, _, first_user = org_team_user
        # Two Insights accounts in the same org, both linked to the same Slack identity.
...
class TestUserSlackIntegrationFromIdentity:
    ...
    row), even though the function itself is defined under ``insights.models``.
    """
...
class TestInviteToken:
    ...
    def test_settings_initiated_invite_omits_slack_user_id(self):
        original = InviteToken(slack_team_id=SLACK_TEAM_ID, insights_team_id=42)
        decoded = InviteToken.decode(original.encode())
    ...
    def test_rejects_tampered_token(self):
        token = InviteToken(slack_team_id=SLACK_TEAM_ID, insights_team_id=42).encode()
    ...
    def test_rejects_callback_state_signed_with_other_salt(self):
        ...
        callback_state = CallbackState(slack_team_id="T1", insights_team_id=1, insights_user_id=99).encode()
...
class TestCallbackState:
    def test_round_trips(self):
        original = CallbackState(slack_team_id="T1", insights_team_id=1, insights_user_id=99, slack_user_id="U1")
    ...
    def test_rejects_invite_token(self):
        ...
        invite = InviteToken(slack_team_id="T1", insights_team_id=1).encode()
