...
from insights.insightsql.insightsqlx import convert_dict_to_hx, convert_tag_to_hx, convert_to_hx


class TestInsightsQLX(BaseTest):
    def test_convert_tag_to_hx(self):
        tag = ast.InsightsQLXTag(kind="Sparkline", attributes=[])
        self.assertEqual(
            convert_tag_to_hx(tag), ast.Tuple(exprs=[ast.Constant(value="__hx_tag"), ast.Constant(value="Sparkline")])
        )

        tag = ast.InsightsQLXTag(
            kind="Sparkline", attributes=[ast.InsightsQLXAttribute(name="color", value=ast.Constant(value="red"))]
        )
        self.assertEqual(
    ...
    def test_convert_to_hx(self):
        ...
        self.assertEqual(
            convert_to_hx(ast.InsightsQLXTag(kind="Sparkline", attributes=[])),
