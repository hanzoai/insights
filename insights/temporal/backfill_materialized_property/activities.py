...
from insights.datastore.cluster import AlterTableMutationRunner, get_cluster
from insights.datastore.kafka_engine import json_extract_trim_quotes
from insights.datastore.materialized_columns import DMAT_STRING_COLUMN_NAME_PREFIX
...
from insights.models.dmat_slot_assignments.sql import (
...
from insights.models.event.sql import DMAT_STRING_COLUMN_COUNT
from insights.models.materialized_column_slots import MaterializedColumnSlotState
from insights.temporal.common.heartbeat_sync import HeartbeaterSync
...
def assign_pending_columns(inputs: AssignPendingColumnsInputs) -> AssignPendingColumnsResult:
    ...
    with transaction.atomic():
        ...
        # per-type filter (all dmat columns are `Nullable(String)`, InsightsQL casts at read).
        ...
        if stranded:
            ...
            # Surface to Sentry — the workflow doesn't fail (InsightsQL falls back to JSON for
...
def _build_dict_backed_update_command(
    ...
    the SQL is constant-size regardless of team count, and Datastore can prune sharded_events
    ...
    # Fully-qualified identifiers — Datastore normalizes bare references when storing
    ...
    qualified_dict = f"{settings.DATASTORE_DATABASE}.{DMAT_SLOT_ASSIGNMENTS_DICTIONARY_NAME}"
    qualified_assignments_table = f"{settings.DATASTORE_DATABASE}.dmat_slot_assignments"
