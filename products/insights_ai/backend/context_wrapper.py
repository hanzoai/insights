...
class ContextService:
    ...
    async def abuild_resumed_legacy_context(
        ...
        """Render a converted conversation's legacy history into a one-time `<insights_context>` block.
        ...
        if not transcript:
            return None
        return f"<insights_context>{RESUMED_CONTEXT_PREFIX}\n{transcript}</insights_context>"
    ...
    def _render_legacy_transcript(self, messages: Sequence[Any]) -> str:
        ...
        """
        from insights.schema import (  # noqa: PLC0415 — large schema module
            AssistantMessage,
