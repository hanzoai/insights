...
SELECT
    ...
    -- The actual rendered Datastore SQL that ran. This is what we hand to the
    -- campaign — the metabase / local-CH backends both speak Datastore SQL,
    -- not InsightsQL.
    query AS datastore_query,
    -- The InsightsQL JSON envelope kept for context only (some campaigns may want
    -- to reason about the higher-level query intent).
    JSONExtractString(log_comment, 'query') AS insightsql_query,
    query_duration_ms,
    read_bytes,
...
# Same idea as the column filter, but for Datastore dictionaries (referenced via
...
def _ch_string_array(items: list[str]) -> str:
    """Render ``items`` as a Datastore string-literal array.
    ...
    Datastore parses double-quoted tokens as identifier references — not
...
class SlowQuery:
    query_id: str
    team_id: int
    # Rendered Datastore SQL exactly as it ran on the cluster — what we hand
    # to the campaign as `CAMPAIGN_SQL`.
    datastore_query: str
    # The InsightsQL JSON envelope (from log_comment.query). Useful as context but
    # not directly executable on the test cluster — it references InsightsQL
...
def fetch_slow_queries(
    *,
    ...
    """Run the slow-queries SQL via `insightscli metabase:query` and parse the rows.

    We shell out to `insightscli` rather than re-implementing cookie handling
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"insightscli metabase:query failed (exit {result.returncode}):\n"
...
def fetch_available_columns(
    *,
    ...
    ``insights.<table>`` for each of ``tables``.
    ...
    sql = f"SELECT DISTINCT name FROM system.columns WHERE database = 'insights' AND table IN ({table_list})"
    cmd = [
        "insightscli",
        "metabase:query",
        "--region",
...
def fetch_available_dictionaries(*, region: str, database_id: int, timeout_s: float = 60.0) -> list[str]:
    """Return ``database.name`` for each Datastore dictionary on the test cluster.
    ...
    query that uses one.
    """
    sql = "SELECT database, name FROM system.dictionaries WHERE database = 'insights'"
    cmd = [
        "insightscli",
        "metabase:query",
        "--region",
    ...
    for row in body["data"]["rows"]:
        ...
        out.append(
            SlowQuery(
                ...
                datastore_query=str(record["datastore_query"]),
                insightsql_query=str(record["insightsql_query"]),
