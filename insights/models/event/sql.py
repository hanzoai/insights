...
# Django-free insights.datastore.events_json module so the InsightsQL engine can use them without
...
def _json_column_type(max_dynamic_types: int, max_dynamic_paths: int, subcolumns: dict[str, str]) -> str:
    explicit_paths = ", ".join(
        f"{escape_datastore_identifier(name)} {column_type}" for name, column_type in subcolumns.items()
    )
...
def json_property_presence_expr(column: str, prop: str) -> str:
    ...
    Mirrors the InsightsQL resolver's JSONHas lowering: a typed subcolumn is present when non-null; a
    dynamic path is present when its scalar read is non-null or its sub-object read is non-empty.
    Datastore's JSONHas() cannot be used directly on a JSON-typed column — it does not see typed
    ...
    parts = prop.split(".")
    column_sql = escape_datastore_identifier(column)
    path_sql = ".".join(escape_datastore_identifier(part) for part in parts)
    ...
    if parts[0] in subcolumns:
        head = f"{column_sql}.{escape_datastore_identifier(parts[0])}"
        head_document = head if subcolumns[parts[0]] in ("String", "Nullable(String)") else f"toJSONString({head})"
        tail = ", ".join(escape_datastore_string(part) for part in parts[1:])
...
def _json_subcolumn(column: str, path: str) -> str:
    return f"{escape_datastore_identifier(column)}.{escape_datastore_identifier(path)}"
...
""".format(
        ...
        on_cluster_clause=f"ON CLUSTER '{settings.DATASTORE_CLUSTER}'" if on_cluster else "",
...
--       https://github.com/Datastore/Datastore/issues/7322
--
