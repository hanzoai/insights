...
const UsageMetricDataWarehouseFiltersSchema = z
    .object({
        ...
        timestamp_field: z
            .string()
            .describe(
                'Timestamp column or InsightsQL expression on the row (e.g. "created" or "toDateTime(created_at)"). Use `execute-sql` (`SELECT * FROM <table> LIMIT 1`) to inspect available columns.'
            ),
        key_field: z
            .string()
            .describe(
...
export const UsageMetricFiltersSchema = z
    ...
    .describe(
        'Filter definition. Pick exactly one branch: `data_warehouse` (set `source: "data_warehouse"` plus `table_name`/`timestamp_field`/`key_field`) or `events` (InsightsFunction filter shape with an `events` array).'
    )
...
export const FeedbackSubmitSchema = z
    .object({
        ...
        feedback_type: z
            ...
            .describe(
                'What this feedback is about. "product" = any Insights product or feature (insights, session replay, feature flags, the data warehouse, web analytics, error tracking, etc.). "mcp" = this MCP server itself — a tool, its input schema, response format, an error, or these instructions. "docs" = Insights documentation. "scout" = a canonical Insights scout skill\'s content — reserved for scheduled scout runs reporting an improvement opportunity in their own Insights-authored skill (set `scout_skill_name`, `scout_skill_version`, and `scout_category`). "other" = anything that doesn\'t fit the above.'
            ),
        sentiment: z
        ...
        product_area: z
            ...
            .optional()
            .describe(
                'The Insights product or area this is about, in free text (e.g. "session replay", "insights", "data warehouse", "feature flags", "docs"). Most useful for product feedback; for MCP feedback the tool name belongs in `details`/`friction_points` instead.'
            ),
        category: z
            .enum([
                'tool_correctness',
...
export const InsightQueryInputSchema = z.preprocess(
    ...
    z.object({
        ...
        variables_override: z
            ...
            .optional()
            .describe(
                'Object (or pre-encoded JSON string) to override the insight\'s InsightsQL variables for this run only (not persisted). Format: {"<variable_id>": {"code_name": "<code_name>", "variableId": "<variable_id>", "value": <new_value>}}. Each entry must include `code_name` — partial entries are silently dropped. The simplest workflow is to call `insight-get` first, copy the matching entry from the response\'s query variables, and mutate `value`. Top-level keys replace; nested values are not deep-merged. Ignored when accessed via a sharing token.'
            ),
        filters_override: z
            .union([z.string(), z.record(z.string(), z.unknown())])
            .optional()
...
export const ExecuteSQLSchema = z.object({
    ...
    connectionId: z
        ...
        .optional()
        .describe(
            'Optional id of an external data source (e.g. a Postgres, DuckDB, or MySQL direct-query connection). When set, runs the query against that source instead of the Datastore catalog. Use external-data-sources-list to discover available connection ids.'
        ),
})
...
export const WorkflowGraphPatchSchema = z.object({
    id: z.string().describe('The workflow (InsightsFlow) id to edit.'),
    operations: z
...
export const WorkflowActionEmailPatchSchema = z
    .object({
        id: z.string().describe('The workflow (InsightsFlow) id.'),
