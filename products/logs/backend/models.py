...
# Default log attribute key whose value matches a Insights person's distinct_id. Mirrors
# the convention documented at https://hanzo.ai/docs/logs/link-session-replay: the
# insights-js / insights-react-native SDKs auto-attach `insightsDistinctId` to every log
...
DEFAULT_LOGS_DISTINCT_ID_ATTRIBUTE_KEY = "insightsDistinctId"
...
# Default log attribute keys whose values hold the Insights session ID. `insightsSessionId`
# is the key the insights-js / insights-react-native SDKs auto-attach to every log they
# emit (see https://hanzo.ai/docs/logs/link-session-replay). Ordered: detection checks
...
DEFAULT_LOGS_SESSION_ID_ATTRIBUTE_KEYS = ["insightsSessionId"]
...
class TeamLogsConfig(models.Model):
    ...
    team = models.OneToOneField("insights.Team", on_delete=models.CASCADE, primary_key=True)
    ...
    # Log attribute keys whose values match a Insights person's distinct_id — a log links
    ...
    logs_distinct_id_attribute_keys = ArrayField(
        ...
        db_default=Value("{insightsDistinctId}"),
    )
    ...
    logs_session_id_attribute_keys = ArrayField(
        ...
        db_default=Value("{insightsSessionId}"),
    )
...
class LogsAlertEvent(UUIDModel):
    ...
    class Kind(models.TextChoices):
        # Worker-produced row from evaluating the Datastore check query. Only CHECK rows
...
class LogsMetricRule(ModelActivityMixin, TeamScopedRootMixin, CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    # db_constraint=False on the team/user FKs: insights_team and insights_user are hot tables,
