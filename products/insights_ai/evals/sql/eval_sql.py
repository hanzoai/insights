...
Prompts are rephrased from the CI dataset to explicitly request InsightsQL.
...
``Write a InsightsQL query that…`` framing, several cases would route to a
...
To run:
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_sql"
"""
...
from products.insights_ai.eval_harness.scorers import LastToolCallNot, NoToolCall
from products.insights_ai.evals.product_analytics.scorers import INSIGHT_WRITE_TOOLS
from products.insights_ai.evals.retrieval.scorers import SkillLoaded
from products.insights_ai.evals.sql.scorers import AnswerQueryRan, SQLResultMessageAlignment, SQLSchemaAlignment
...
""",
    ...
    await SandboxedPublicEval(
        ...
        scorers=[
            ...
            SkillLoaded("querying-insights-data", name="querying_insights_data_skill_loaded"),
            SQLSchemaAlignment(),
