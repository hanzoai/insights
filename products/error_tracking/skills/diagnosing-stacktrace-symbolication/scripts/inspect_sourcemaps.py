#!/usr/bin/env python3
"""Summarize JavaScript source maps and emitted JS chunks for Insights symbolication debugging.
...
Operates on plain `.js` / `.map` files only. To inspect a Insights symbol-data container downloaded
from the API, extract it first with `insights-cli symbol-sets extract <file> -o <dir>` (or
`npx @hanzo/cli ...` / `bunx @hanzo/cli ...`) and point this script at the resulting directory.
"""
        ...
        return [
            {
                **base,
                "error": (
                    "Insights symbol-data container — extract first with "
                    "`insights-cli symbol-sets extract <file> -o <dir>` "
                    "(or `npx @hanzo/cli ...` / `bunx @hanzo/cli ...`), "
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=Path,
        help="JS/map files, directories, or globs. Extract symbol-data containers with insights-cli first.",
    )
