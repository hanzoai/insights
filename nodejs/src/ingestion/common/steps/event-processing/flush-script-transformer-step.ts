...
/**
 * afterBatch step that drains the script transformer's accumulated invocation
 ...
 */
export function createFlushScriptTransformerStep<T>(
    scriptTransformer: Pick<ScriptTransformer, 'processInvocationResults'>
): ProcessingStep<T, T> {
    return function flushScriptTransformerStep(input: T): Promise<PipelineResult<T>> {
        const stopTimer = backgroundTaskScriptTransformerDuration.startTimer()
        const drain = scriptTransformer.processInvocationResults().finally(stopTimer)
