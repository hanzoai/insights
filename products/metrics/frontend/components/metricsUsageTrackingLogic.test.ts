...
const captures = (event: string): any[][] =>
    (insights.capture as jest.Mock).mock.calls.filter(([name]) => name === event)

const allCapturedProperties = (): string => JSON.stringify((insights.capture as jest.Mock).mock.calls)
