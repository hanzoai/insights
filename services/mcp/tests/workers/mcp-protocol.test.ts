...
describe('MCP HTTP entry point (Cloudflare Workers)', () => {
    describe('OAuth Protected Resource Metadata (RFC 9728)', () => {
        it('returns metadata advertising scopes_supported for /mcp', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/.well-known/oauth-protected-resource/mcp')

            expect(response.status).toBe(200)
            expect(response.headers.get('content-type')).toContain('application/json')
            const body = (await response.json()) as Record<string, unknown>
            expect(body.resource).toBe('https://mcp.hanzo.ai/mcp')
        ...
        it('returns metadata for /sse with the same shape', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/.well-known/oauth-protected-resource/sse')

            expect(response.status).toBe(200)
            const body = (await response.json()) as Record<string, unknown>
            expect(body.resource).toBe('https://mcp.hanzo.ai/sse')
        })

        it('allows CORS from Insights app origins for consent-page metadata fetch', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/.well-known/oauth-protected-resource/mcp', {
                headers: { Origin: 'https://us.hanzo.ai' },
            })

            expect(response.status).toBe(200)
            expect(response.headers.get('Access-Control-Allow-Origin')).toBe('https://us.hanzo.ai')
            expect(response.headers.get('Vary')).toBe('Origin')
        })

        it('answers OPTIONS preflight for Insights app origins', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/.well-known/oauth-protected-resource/mcp', {
                method: 'OPTIONS',
                headers: {
                    Origin: 'https://us.hanzo.ai',
    ...
    describe('Authentication challenges', () => {
        it('returns 401 with WWW-Authenticate Bearer + resource_metadata when no token', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/mcp', { method: 'POST' })
        ...
        it('returns 401 for malformed bearer tokens (wrong prefix)', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/mcp', {
                method: 'POST',
                headers: { Authorization: 'Bearer not_a_insights_token' },
            })
        ...
        it('passes the gate for ID-JAG access tokens (typ: at+jwt)', async () => {
            ...
            // downstream against the Insights API (`IDJagAccessTokenAuthentication`).
            // Here we assert the response is NOT the gate's "Invalid token" 401 —
            // anything else (init failure against a missing Insights API, etc.) is
    ...
    describe('Authorization server redirects', () => {
        it('redirects /.well-known/oauth-authorization-server to the auth server', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/.well-known/oauth-authorization-server', {
                redirect: 'manual',
            })
        ...
        it('rewrites /register to /oauth/register with a 307 (preserves POST)', async () => {
            const response = await SELF.fetch('https://mcp.hanzo.ai/register', {
                method: 'POST',
                redirect: 'manual',
            })
