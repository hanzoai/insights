...
//         depend on the affected insights.schema types.
...
function packageToProduct(pkg) {
    return pkg.replace('@hanzo/products-', '')
}
...
// Schema contract: tools/insightscli-commands/insightscli_commands/quarantine/core.py.
...
//
// Only products.* modules become nodes or edges. insights/ee (and the
...
// is fixing).
//
// Deliberately never traverses through insights/ee (moduleGraph has already
...
// Core: insights/ + ee/ minus temporal, dags, scriptvm
// Core POE: subset of Core (ignores insightsql, insightsql_queries) — same pool, fewer tests
// Temporal: insights/temporal + products/batch_exports/backend/tests/temporal + products/tasks/backend/temporal
const DJANGO_SEGMENTS = {
    Core: {
        include: ['insights/', 'ee/'],
        exclude: ['insights/temporal/', 'insights/dags/', 'common/scriptvm/'],
    },
    CorePOE: {
...
function buildMatrix(products, durations) {
    ...
    for (const product of products) {
        ...
        if (raw > PRODUCT_TARGET_WALL_SECONDS) {
            ...
            // optimal_chunks (Insights pytest-split fork) makes the same contiguous,
            ...
            matrix.push({
                group: product,
                filters: `--filter=@hanzo/products-${product}`,
                pytest_args: '',
            })
        } else {
