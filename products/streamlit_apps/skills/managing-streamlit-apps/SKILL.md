...
description: Create, deploy, and operate Streamlit apps in Insights via the streamlit-apps MCP tools — create an app, set its source, start and stop its sandbox, poll status, list versions, delete, and share the app with humans via its Insights URL. Use when asked to "create a streamlit app", "deploy a data app", "ship a dashboard app", "restart/stop my app", "why is my app not running", or "give me a link to the app".
---

# Managing Streamlit apps

A Streamlit app is a Python data app running in an isolated sandbox inside Insights.
...
2. `streamlit-apps-set-source` with the complete `app.py` source as one string.
   ...
   Write the source per the `writing-streamlit-apps` skill — in particular use `insights_apps.query()` for Insights data, never `import insights`.
...
5. Share the app: the `_insightsUrl` returned by create/get/start (`/streamlit-apps/{short_id}`) is the app's page in Insights.
   That link is what you give humans.
   Viewers need to be logged into Insights with access to the project (the app renders in an authenticated iframe; there is no public URL).
...
Raise `memory_gb` only when the app itself holds large DataFrames in memory — the 256 MB InsightsQL bridge query cap is server-side and does NOT change with sandbox sizing, so bigger sandboxes don't fix failing queries.
