...
/** Narrowed Hub type for ScriptRateLimiterService */
export type ScriptRateLimiterServiceHub = Pick<
    Hub,
...
export class ScriptRateLimiterService {
    constructor(
        private hub: ScriptRateLimiterServiceHub,
    ...
    public async rateLimitMany(idCosts: [string, number][]): Promise<[string, ScriptRateLimit][]> {
        const res = await this.redis.usePipeline({ name: 'script-rate-limiter', failOpen: true }, (pipeline) => {
