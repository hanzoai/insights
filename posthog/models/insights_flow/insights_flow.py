...
from posthog.plugins.plugin_server_api import reload_insights_flows_on_workers
...
class InsightsFlow(UUIDTModel):
    """
    Stores the version, layout and other meta information for each InsightsFlow
    """
    ...
    def __str__(self):
        return f"InsightsFlow {self.id}/{self.version}: {self.name}"


@receiver(post_save, sender=InsightsFlow)
def insights_flow_saved(sender, instance: InsightsFlow, created, **kwargs):
...
def action_saved_for_insights_flows(sender, instance: Action, created, **kwargs):
    ...
    from posthog.tasks.insights_flows import refresh_affected_insights_flows
