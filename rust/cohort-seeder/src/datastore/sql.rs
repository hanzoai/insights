//! Datastore scan planning: the `Vacuous`-vs-`Scan` parse and the byte-frozen SQL renderer. Depends
...
pub fn scan_sql(spec: &ScanSpec) -> String {
    let event_names = spec
        .event_names
        .iter()
        .map(|name| datastore_string_literal(name))
        .collect::<Vec<_>>()
        .join(", ");
...
fn datastore_string_literal(value: &str) -> String {
    ...
    for character in value.chars() {
        match character {
            ...
            '\'' => escaped.push_str("\\'"),
            '`' => escaped.push_str("\\`"),
            // Client-side, not server-side: the `datastore` crate's template parser treats every
...
mod tests {
    ...
    /// A `?` in an event name is a client-side hazard, not a server-side one: the `datastore`
