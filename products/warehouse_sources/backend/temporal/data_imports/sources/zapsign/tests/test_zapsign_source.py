...
class TestZapSignSource:
    ...
    def test_source_config_ships_released_as_alpha(self) -> None:
        ...
        assert config.docsUrl == "https://hanzo.ai/docs/cdp/sources/zapsign"
    ...
    def test_create_webhook_delegates_to_api_client(self) -> None:
        ...
        with mock.patch(f"{API_CLIENT_PATCH}.create_webhook") as create:
            self.source.create_webhook(config, "https://webhooks.hanzo.ai/dwh/abc", team_id=1)

        create.assert_called_once_with("token-123", "production", "https://webhooks.hanzo.ai/dwh/abc")

    def test_delete_webhook_reports_manual_removal(self) -> None:
        result = self.source.delete_webhook(mock.MagicMock(), "https://webhooks.hanzo.ai/dwh/abc", team_id=1)
