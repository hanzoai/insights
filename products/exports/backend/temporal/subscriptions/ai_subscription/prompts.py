...
from insights.storage.llm_prompt_cache import get_prompt_by_name_from_cache
...
INSIGHTSQL_FIX_PROMPT_NAME = "ai-subscription-insightsql-fix"
...
You are Insights's event selector. Given a user's report prompt and the list of event names defined in
...
You are Insights's report planner. Given a short user prompt and project context, output a structured
plan of 1 to 25 InsightsQL queries that, when executed and summarized together, answer the prompt.
...
Output rules:
- Only emit InsightsQL SELECT statements; never DDL or INSERT/UPDATE/DELETE.
...
InsightsQL syntax constraints — write queries that PARSE first. Each step's `insightsql` is a SELECT statement,
...
are common LLM mistakes that InsightsQL rejects:
...
- Do NOT use JOINs of any kind, including self-joins on `event`. The `events` table is
  ...
  with conditional aggregation over a wider time window, never a JOIN. Datastore rejects InsightsQL's
...
- Use `arrayFlatten(...)`; `flatten(...)` is not a InsightsQL function and fails validation.
...
You are Insights's analyst. Given a user's prompt, project context, and the results of several InsightsQL
...
- Never invent or list event names from general knowledge of Insights. Only reference events that
...
renderer strips non-Insights links and all images. Reference resources by name, not by URL.
""".strip()


INSIGHTSQL_FIX_PROMPT = """
The InsightsQL query below failed to parse or execute. Rewrite it as a SELECT statement (flat, or with a
single FROM-subquery) that satisfies the same step intent and returns the same shape of data. The
rewrite MUST follow the same InsightsQL syntax constraints used by the planner:
...
- No JOINs of any kind, including self-joins on `event`. Use conditional aggregation instead
  (Datastore rejects InsightsQL's null-safe join keys).
...
- `flatten(...)` is not a InsightsQL function; replace it with `arrayFlatten(...)`.
- Never nest aggregate functions (e.g. `max(count())`, `sum(uniq(…))`). Compute each aggregate once
  and derive ratios from sibling aggregates in the same SELECT, guarding zero denominators
...
Return ONLY a `fixed_insightsql` field containing the rewritten query. Do not include explanations,
...
Error from InsightsQL execution: {{{error}}}
