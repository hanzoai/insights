...
7. Creates the `insights-cli/vX.Y.Z` GitHub release, refreshes `insights-cli-latest` for stable releases, and publishes the npm package

Do not run `sampo publish`; cargo-dist owns publishing for `insights-cli`.

If you need to cut a release by hand, merge a CLI changeset to `master` and let `Release CLI` run from there.
Do not push `insights-cli/vX.Y.Z` tags manually; cargo-dist tag-push releases are disabled.
If cargo-dist fails after `Release CLI` commits the release bump, rerun the failed jobs from the same workflow run.

The release workflow also builds `services/mcp` into `cli/lib/insights-api-cli.mjs` before cargo-dist packages artifacts.
This keeps `insights-cli api` aligned with the generated MCP tool catalog at release time.
...
The smoke test verifies that the archive includes `lib/insights-api-cli.mjs`, that `insights-cli api --agent-help` works from the unpacked artifact, and that the generated shell installer produces a working `insights-cli api` install without relying on a Insights repo checkout.
...
It does not replace any `insights-cli` already on your `PATH`; to manually test the built artifact, run `target/distrib/insights-cli-$(rustc -vV | sed -n 's/^host: //p')/insights-cli api --agent-help`.
