...
function createCohortFromLinks(links: IdentityMatchingLinkApi[], modelPreference: ModelPreference): void {
    ...
    Dialog.openForm({
        ...
        content: (
            <Field name="name">
                <Input placeholder="Cohort name" autoFocus />
            </Field>
        ),
        ...
        onSubmit: async ({ name }: Record<string, any>) => {
            try {
                ...
                toast.success(`Created cohort "${name}" with ${ids.length} persons`)
            } catch {
...
function KpiBar({ links, loading }: { links: IdentityMatchingLinkApi[]; loading: boolean }): JSX.Element {
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <Tag type="success">{stats.high.toLocaleString()} high</Tag>
            <span className="text-tertiary">{highPct}%</span>
            <span className="text-border">·</span>
            <Tag type="warning">{stats.medium.toLocaleString()} medium</Tag>
            <span className="text-border">·</span>
            <Tag>{stats.low.toLocaleString()} low</Tag>
            <span className="text-border">·</span>
            <Tag type="completion">{stats.paidTouches.toLocaleString()} paid touches</Tag>
...
export function IdentityMatchingScene(): JSX.Element {
    ...
    const columns: TableColumns<IdentityMatchingLinkApi> = [
        {
        ...
        },
        {
            ...
            render: (_, link) => <Tag>{link.model_version}</Tag>,
            width: 110,
        },
        {
            title: (
                ...
                return (
                    <div className="flex flex-col gap-1 min-w-28">
                        ...
                        <div className="flex items-center gap-1.5">
                            <Tag type={TIER_TAG_TYPE[link.tier]}>{link.tier}</Tag>
                                ...
                                <div key={cat} className="flex items-center gap-1">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {catSignals.map((signal, i) => (
                                            <Tag key={`${cat}-${i}`} type={CATEGORY_TAG_TYPE[cat]}>
                                                {signal.label}
                                            </Tag>
                                        ))}
                                    </div>
    ...
    const tabs: Tab<string>[] = [
        {
            ...
            label: (
                <span className="flex items-center gap-1.5">
                    Links
                    {linksCount > 0 && <Tag className="ml-1">{linksCount.toLocaleString()}</Tag>}
                </span>
            ),
            content: (
