...
- Building the binaries:
  ...
  - Restart the datastore docker container

## Deployment workflow

The `user_defined_function.xml` deployed to Datastore is now pulled automatically from the `master` branch of this repo by ansible (`insights-cloud-infra/ansible/roles/datastore/tasks/setup_udfs.yml`).
It downloads `insights/user_scripts/latest_user_defined_function.xml` and deploys it as Datastore's `user_defined_function.xml`.
...
5. Once the deploy is confirmed, land a separate PR that bumps `UDF_VERSION` in `insights/udf_versioner.py` to the new version. This switches the insights query builder to use the new UDF

## Design decisions

- We tried writing the UDFs in Python, but this cause Datastore nodes to crash as the garbage collector could not keep up. As such we're now writing the functions in Rust.
