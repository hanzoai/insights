...
class TestValidateCDCPrerequisites:
    def test_all_checks_pass_insights_managed(self):
        conn = _mock_conn([_PG_15, _WAL_LOGICAL, _HAS_PK, _HAS_REPL_ROLE, _MAX_SLOTS_10, _SLOT_COUNT_2])
        errors = validate_cdc_prerequisites(conn=conn, management_mode="insights", tables=["users"], schema="public")
        assert errors == []

    def test_all_checks_pass_self_managed(self):
        # Self-managed now also checks replication role + slot capacity (Insights creates the slot).
    ...
    def test_pg_version_too_old(self):
        ...
        errors = validate_cdc_prerequisites(conn=conn, management_mode="insights", tables=["users"])
    ...
    def test_table_missing_primary_key(self):
        ...
        errors = validate_cdc_prerequisites(conn=conn, management_mode="insights", tables=["orders"])
    ...
    def test_self_managed_no_slot_name_is_ok(self):
        # Insights creates the slot — slot_name omission is fine in self-managed.
        conn = _mock_conn([_PG_15, _WAL_LOGICAL, _HAS_PK, _HAS_REPL_ROLE, _MAX_SLOTS_10, _SLOT_COUNT_2, _PUB_EXISTS])
        errors = validate_cdc_prerequisites(
            conn=conn,
            management_mode="self_managed",
    ...
    def test_self_managed_replication_role_required(self):
        # Self-managed also needs REPLICATION (Insights creates the slot itself).
    ...
    def test_empty_tables_list(self):
        ...
        errors = validate_cdc_prerequisites(conn=conn, management_mode="insights", tables=[])
        assert errors == []
