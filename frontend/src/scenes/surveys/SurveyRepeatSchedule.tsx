...
function SurveyAutoCloseHelper({
    ...
    return (
        <Banner type={hasCloseDatePassed ? 'warning' : 'info'} className="text-xs">
            {!autoCloseDate ? (
                <span>
                            ...
                            description: showSurveyRepeatSchedule ? (
                                <div className="flex flex-col gap-1">
                                    <div className="flex flex-row gap-2 items-center text-secondary">
                                        ...
                                        <Field name="iteration_frequency_days">
                                            {({ onChange, value }) => {
                                                return (
                                                    <Input
                                                        type="number"
        ...
        <div className="mt-4">
            {repeatsOnEveryEvent ? (
                <span className="font-medium">
                    ...
                    <Snack>{survey.conditions?.events?.values.map((v) => v.name).join(', ')}</Snack>&nbsp;
