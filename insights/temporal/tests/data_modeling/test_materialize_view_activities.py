...
from insights.temporal.data_modeling.activities.materialize_view import InvalidNodeTypeException
...
class TestFailMaterializationActivity:
    ...
    async def test_sends_failure_notification_email(self, activity_environment, ateam, anode, ajob, asaved_query):
        ...
        with unittest.mock.patch("insights.tasks.email.send_saved_query_materialization_failure") as mock_send_email:
            await activity_environment.run(fail_materialization_activity, inputs)
...
class TestMaterializeViewActivity:
    ...
    async def test_materializes_view_to_delta_table(
        ...
        with (
            ...
            unittest.mock.patch(
                "insights.temporal.data_modeling.activities.materialize_view.get_query_row_count",
                return_value=3,
            ),
        ):
            inputs = MaterializeViewInputs(
