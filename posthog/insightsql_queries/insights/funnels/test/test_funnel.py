...
from posthog.insightsql_queries.insights.funnels.test.breakdown_cases import (
    assert_funnel_results_equal,
    funnel_breakdown_group_test_factory,
    funnel_breakdown_test_factory,
)
from posthog.insightsql_queries.insights.funnels.test.conversion_time_cases import funnel_conversion_time_test_factory
...
class TestFOSSFunnelUDF(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_insightsql_aggregation(self):
        ...
        # distinct_id
        results = (
            self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_insightsql": "distinct_id"})
            .calculate()
            .results
        )
        self.assertEqual(results[0]["count"], 2)
        ...
        # person_id
        results = (
            self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_insightsql": "person_id"}).calculate().results
        )
        self.assertEqual(results[0]["count"], 2)
        self.assertEqual(results[1]["count"], 1)
        self.assertEqual(results[2]["count"], 1)
        ...
        result = (
            self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_insightsql": "person.properties.common_prop"})
            .calculate()
            .results
        )
        self.assertEqual(result[0]["count"], 1)
