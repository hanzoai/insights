...
Gated behind the ``mcp-analytics-clustering-schedule`` Insights feature flag.
...
from insights.temporal.mcp_analytics.intent_clustering.constants import (
...
from insights.temporal.mcp_analytics.intent_clustering.coordinator import IntentClusteringCoordinatorInputs
...
def _schedule_enabled() -> tuple[bool, bool]:
    ...
    * ``(False, False)`` — FF check raised. **Preserve current state.** Do
      ...
      delete an existing one (a transient Insights API outage shouldn't tear
      down a properly-enabled production schedule).
    """
    try:
        enabled = hanzo_insights.feature_enabled(FEATURE_FLAG_KEY, FEATURE_FLAG_DISTINCT_ID)
...
async def create_intent_clustering_coordinator_schedule(client: Client) -> None:
    ...
    deploy. If the FF check itself fails (transient Insights API outage),
    the current schedule state is preserved — we don't tear down a valid
    production schedule on a blip.
    """
    # hanzo_insights.feature_enabled() is sync and can block on a network call,
    ...
    if not enabled:
        if not check_succeeded:
            ...
            # transient Insights outages must not silently disable production.
            logger.info(
