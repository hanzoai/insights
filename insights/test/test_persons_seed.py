...
def _cleanup(conn: psycopg.Connection[Any]) -> None:
    with conn.cursor() as cursor:
        cursor.execute("DELETE FROM insights_persondistinctid WHERE team_id = %s", (SEED_TEST_TEAM_ID,))
    ...
    def test_insert_distinct_id_links_to_person(self):
        with persons_db_connection(writer=True) as conn:
            try:
                ...
                with conn.cursor() as cursor:
                    cursor.execute(
                        "SELECT person_id FROM insights_persondistinctid WHERE team_id = %s AND distinct_id = %s",
