...
class SlackSource(ResumableSource[SlackSourceConfig, SlackResumeConfig], WebhookSource[SlackSourceConfig], OAuthMixin):
    ...
    def webhook_resource_map(self) -> dict[str, str]:
        ...
        # fallback in get_or_create_webhook_insights_function that defaults to using
    ...
    def _resolve_access_token(self, config: SlackSourceConfig, team_id: int) -> tuple[str, str | None, str]:
        ...
        - Otherwise the source predates bring-your-own and still points at Insights's shared app
    ...
    def delete_webhook(
        ...
        # toggle it off manually in the app settings. Returning success lets the InsightsFunction
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Sync Slack channels, users, and messages into Insights by connecting your own Slack app.

You create a Slack app in your workspace and paste its bot token here. Using your own app keeps message syncing within Slack's API terms — [read the docs](https://hanzo.ai/docs/cdp/sources/slack) for the full walkthrough.
...
{
    "display_information": {
        "name": "Insights data warehouse",
        "description": "Sync Slack channels, users, and messages to Insights data warehouse"
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
Invite the bot to any channel whose messages you want to sync (`/invite @Insights data warehouse`). After the source is created, follow the webhook steps to start receiving messages.""",
            iconPath="/static/services/slack.png",
            docsUrl="https://hanzo.ai/docs/cdp/sources/slack",
            ...
            webhookSetupCaption="""Slack delivers new messages to Insights through your Slack app's Event Subscriptions.
