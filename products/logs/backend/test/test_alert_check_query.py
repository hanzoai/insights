...
class TestAlertCheckQuery(DatastoreTestMixin, APIBaseTest):
    ...
    def test_bucketed_count_matches_python_histogram_across_random_inputs(self):
        # Stress test the actual Datastore bucketing: seed N logs at random
    ...
    def test_timeout_propagates(self):
        ...
        query = self._make_query(alert)
        with patch(
            "products.logs.backend.alert_check_query.execute_insightsql_query",
            side_effect=Exception("Datastore timeout"),
        ):
            with self.assertRaisesRegex(Exception, "Datastore timeout"):
                query.execute()


class TestEvaluatorWindowAccuracy(DatastoreTestMixin, APIBaseTest):
...
class TestExecuteRollingChecks(DatastoreTestMixin, APIBaseTest):
...
class TestExecuteRollingChecksBatched(DatastoreTestMixin, APIBaseTest):
...
class TestBatchedAlertCheckQuery(DatastoreTestMixin, APIBaseTest):
...
class TestFetchLiveLogsCheckpoint(APIBaseTest):
    @patch("products.logs.backend.alert_check_query.execute_insightsql_query")
    ...
    def test_attaches_utc_to_naive_datetime(self, mock_execute):
        # Datastore returns tz-naive datetimes for DateTime64 columns in some code paths.
