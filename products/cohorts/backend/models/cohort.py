...
from insights.models.file_system.constants import DEFAULT_SURFACE
...
from insights.models.person.util import get_person_by_uuid, get_person_ids_and_uuids_by_uuids
from insights.models.property import Property, PropertyGroup
from insights.models.utils import RootTeamManager, RootTeamMixin, sane_repr
...
class Cohort(FileSystemSyncMixin, RootTeamMixin, models.Model):
    ...
    people = models.ManyToManyField("insights.Person", through="CohortPeople")  # type: models.ManyToManyField
    version = models.IntegerField(blank=True, null=True)
    pending_version = models.IntegerField(blank=True, null=True)
    count = models.IntegerField(blank=True, null=True)

    created_by = models.ForeignKey("insights.User", on_delete=models.SET_NULL, blank=True, null=True)
    ...
    def is_flag_compatible(self) -> bool:
        ...
        - Cohorts with neither recognized filter type (empty filters, cohort-reference-only, etc.)
          are not flag-compatible, even if stale timestamps are set, because InsightsQLRealtimeCohortQuery
    ...
    def insert_users_by_list(
        ...
        *,
        ...
        def create_uuid_batch(batch_index: int, batch_size: int) -> list[str]:
            from insights.models.person.util import get_person_uuids_by_distinct_ids

            start_idx = batch_index * batch_size
            end_idx = start_idx + batch_size
            with personinsights_caller_tag("cohorts/uuid-batch"):
        ...
        return self._insert_users_list_with_batching(
            batch_iterator, insert_in_datastore=True, team_id=team_id, raise_on_error=raise_on_error
        )
    ...
    def insert_users_list_by_id_uuid_pairs_skip_validation(
        ...
        *,
        ...
        the same set) and must not pay for a second personinsights lookup. Semantics otherwise match
        ...
        team-scoped personinsights read for this same ``team_id`` (e.g.
        ...
        return self._insert_users_list_with_batching(
            batch_iterator,
            insert_in_datastore=True,
            team_id=team_id,
            raise_on_error=raise_on_error,
            insert_batch=lambda batch: self._insert_resolved_batch(batch, insert_in_datastore=True, team_id=team_id),
        )
        ...
        *,
        ...
        Args:
            ...
            email_property_key: Accepted for backwards compatibility but ignored — all lookups
                                use the Datastore pmat_email materialized column.
        """
        if team_id is None:
            team_id = self.team_id
        ...
        *,
        ...
        Args:
            ...
            insert_batch: Override for the per-batch write. Defaults to resolving each batch of
                UUIDs via personinsights and inserting (``_insert_batch_via_personinsights``, which honors
                ``insert_in_datastore``); callers whose batches are not plain UUID lists supply
        ...
        def _resolve_and_insert_batch(batch: list[Any]) -> None:
            self._insert_batch_via_personinsights(batch, insert_in_datastore, team_id=team_id)
    ...
    def _insert_batch_via_personinsights(
        self,
        batch: list[str],
        insert_in_datastore: bool,
        *,
        team_id: int,
    ) -> None:
        ...
        """
        with personinsights_caller_tag("cohorts/static-insert"):
            id_uuid_pairs = get_person_ids_and_uuids_by_uuids(team_id, batch)
        self._insert_resolved_batch(id_uuid_pairs, insert_in_datastore=insert_in_datastore, team_id=team_id)
    ...
    def _insert_resolved_batch(
        ...
        *,
        ...
        Datastore inserts (if requested) exclude persons already in the cohort
    ...
    def remove_user_by_uuid(self, user_uuid: str, *, team_id: int) -> bool:
        ...
        try:
            ...
            with personinsights_caller_tag("cohorts/static-remove"):
                person = get_person_by_uuid(team_id, str(user_uuid), distinct_id_limit=0)
            if person is None:
                raise Person.DoesNotExist

            # Check if person is in the cohort via personinsights.
    ...
    def to_dict(self) -> dict:
        from insights.models.activity_logging.activity_log import common_field_exclusions, field_exclusions
    ...
    def enqueue_calculation(self, *, initiating_user=None) -> None:
        ...
        def trigger_calculation():
            from insights.tasks.calculate_cohort import increment_version_and_enqueue_calculate_cohort
...
def get_or_create_internal_test_users_cohort(
    ...
    """
    from insights.utils import GenericEmails
...
class CohortPeople(models.Model):
    ...
    person = models.ForeignKey("insights.Person", on_delete=models.DO_NOTHING, db_constraint=False)
    version = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        ...
        db_table = "insights_cohortpeople"
