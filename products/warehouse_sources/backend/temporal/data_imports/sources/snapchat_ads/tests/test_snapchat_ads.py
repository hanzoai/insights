...
class TestListAdAccounts:
            ...
            "organizations": [
                {
                    ...
                    "organization": {
                        "name": "Insights",
                        "ad_accounts": [
                            {"id": "acc-1", "name": "Insights Self Service", "status": "PENDING"},
                            {"id": "acc-2", "name": "Insights", "status": "ACTIVE"},
                        ],
                    },
                },
    ...
    def test_follows_next_link_until_exhausted(self) -> None:
        ...
        session = self._session_returning(
            self._organizations_page("Insights", "acc-1", next_link=next_link),
    ...
    def test_page_cap_raises_instead_of_returning_partial(self) -> None:
        ...
        response.json.return_value = self._organizations_page(
            "Insights", "acc-1", next_link="https://adsapi.snapchat.com/v1/me/organizations?cursor=loop"
        )
        session = MagicMock()
        session.get.return_value = response
