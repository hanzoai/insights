...
from insights.constants import LOGS_RETENTION_FEATURES_BY_DAYS, AvailableFeature
...
from insights.jwt import InsightsJwtAudience, encode_jwt, signing_key_fingerprint
...
from insights.models.data_color_theme import DataColorTheme
...
from insights.models.group_type_mapping import cached_group_types_for_team
from insights.models.organization import Organization, OrganizationMembership
from insights.models.product_intent.product_intent import (
...
from insights.models.team.event_retention import should_enforce_events_retention
...
from insights.models.team.util import actions_that_require_current_team
...
from insights.scopes import APIScopeObjectOrNotSupported
from insights.session_recordings.data_retention import (
...
from insights.user_permissions import UserPermissions, UserPermissionsSerializerMixin
...
class TeamLogsConfigSerializer(serializers.ModelSerializer):
    ...
    logs_distinct_id_attribute_keys = serializers.ListField(
        ...
        help_text=(
            ...
            "['insightsDistinctId'] — the convention documented at "
            "https://hanzo.ai/docs/logs/link-session-replay and the key the "
            "insights-js / insights-react-native SDKs auto-attach. Add keys only if your "
    ...
    logs_session_id_attribute_keys = serializers.ListField(
        ...
        max_length=10,
        help_text=(
            "Ordered list of log attribute keys whose values hold the Insights session ID. "
            "Detection checks keys in order; the first key with a value wins. Defaults to "
            "['insightsSessionId'] — the key the insights-js / insights-react-native SDKs "
...
def validate_secret_token_generation(team: Team, user: User) -> None:
    ...
    if team.conversations_enabled:
        ...
        return
    if hanzo_insights.feature_enabled(
...
# The filters a conversion goal can carry, rather than every filter InsightsQL knows about. The goal
...
class TeamWorkflowsConfigSerializer(serializers.ModelSerializer, UserAccessControlSerializerMixin):
    capture_workflows_engagement_events = serializers.BooleanField(
        ...
        help_text=(
            ...
            "spam reports, unsubscribes) is captured as standard Insights events ($workflows_email_*) "
...
def get_or_mint_live_events_token(team: Team, user_id: int | None) -> str:
    ...
    token = encode_jwt(claims, timedelta(days=7), InsightsJwtAudience.LIVESTREAM)
...
class TeamViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, viewsets.ModelViewSet):
    ...
    def perform_destroy(self, team: Team):
        ...
        # Datastore, email) to the durable Temporal workflow.
        from insights.temporal.delete_teams.dispatch import start_delete_project_data_workflow
