...
from insights.rate_limit import TeamsEventWebhookThrottle
...
def _claims_match_activity(claims: dict, activity: dict) -> bool:
    ...
    attributes messages to a different Insights-connected tenant, or steer the
...
def teams_event_handler(request: HttpRequest) -> HttpResponse:
    ...
    if activity_type == "message":
        ...
        # that haven't completed Insights OAuth yet (this is exactly the path
    ...
    # cross-check still applies, but we don't need a Insights-side team match —
