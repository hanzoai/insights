...
describe('CDP Consumer loop', () => {
    ...
    describe('e2e fetch call', () => {
        ...
        // (insights/cdp/templates/aws_kinesis/test_template_aws_kinesis.py) asserted a
        // similar shape but ran against the Python Script VM, whose `jsonStringify`
        // formats objects WITH spaces (`{"k": "v"}`). The Node Script VM — what CDP
        ...
        // regression in Script body construction, executor wiring, credential lookup,
        ...
        it('matches the production outgoing request shape for the kinesis template fixture', async () => {
            ...
            try {
                ...
                // Same Script body the production Kinesis template produces, parameterised
                ...
                // Body bytes the Node Script VM emits for this input (no spaces between
    ...
    describe('e2e data warehouse destination', () => {
        ...
        let kafkaProducer: KafkaProducerWrapper = undefined as unknown as KafkaProducerWrapper
        let team: Team
        let fnDataWarehouseDestination: InsightsFunctionType
        let mockProducerObserver: KafkaProducerObserver

        const TABLE_NAME = 'postgres.orders'

        const insertInsightsFunction = async (insightsFunction: Partial<InsightsFunctionType>): Promise<InsightsFunctionType> => {
            return await _insertInsightsFunction(hub.postgres, team.id, insightsFunction)
        }
