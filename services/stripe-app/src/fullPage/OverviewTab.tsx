...
interface State {
    ...
    featureFlags: InsightsFeatureFlag[] | null
...
async function loadJourneyFunnels(client: InsightsClient, projectId: string): Promise<JourneyFunnel[]> {
...
const OverviewTab = ({ client, projectId }: Props): JSX.Element => {
    ...
    const activeFlags: FlagRow[] = featureFlags
        ? featureFlags.slice(0, 5).map((f: InsightsFeatureFlag) => ({
              key: f.key,
    ...
    const seeInInsights = (path: string): JSX.Element | null =>
        insightsBase ? (
            <Box css={{ paddingTop: 'small' }}>
                <ExternalLink href={`${insightsBase}${path}`}>View in Insights</ExternalLink>
            </Box>
        ) : null
    ...
    const eventTrendsFooter = seeInInsights('/activity')
        ...
        : !journeyFunnels
                ...
                <OverviewPageModule key="empty" title="Customer journeys">
                    <PromoBanner hero>
                        ...
                        <PromoBannerText>
                            ...
                            individual customer inside Insights.
                        </PromoBannerText>
                        {insightsBase && (
                            <PromoBannerPrimaryLink href={`${insightsBase}/customer_analytics/journeys`}>
          ...
          : [
                <OverviewPageModule
                    ...
                    subtitle="Define the key steps your users take — from sign-up to purchase — and see where they drop off. View each journey for an individual customer inside Insights."
                />,
                    ...
                    ...(insightsBase
                        ? [
                              <Box key={`link-${funnel.name}`} css={{ paddingTop: 'xsmall' }}>
                                  <ExternalLink href={`${insightsBase}/customer_analytics/journeys/${funnel.id}/edit`}>
                                      View in Insights
                                  </ExternalLink>
                              </Box>,
                          ]
                ...
                ...(insightsBase && journeyFunnels.length <= 2
                    ? [
                          <OverviewPageModule key="journeys-cta" title="">
                              <PromoBanner>
                                  ...
                                  <PromoBannerLink href={`${insightsBase}/customer_analytics/journeys`}>
