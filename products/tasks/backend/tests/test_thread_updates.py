...
_FLAG_TARGET = "products.tasks.backend.facade.api.hanzo_insights.feature_enabled"
...
class TestAgentThreadUpdates(TestCase):
    ...
    def test_pr_created_posts_authorless_artifact_message(self, _flag) -> None:
        post_pr_created_thread_update(self.task_run, "https://github.com/insights/insights/pull/123")
        ...
        self.assertEqual(messages[0].payload, {"pr_url": "https://github.com/insights/insights/pull/123"})
        self.assertEqual(
            messages[0].content,
            "[insights/insights#123](https://github.com/insights/insights/pull/123) has been opened",
        )

    @parameterized.expand(
        [
            ("non_github", "https://example.com/pr/9"),
            ("github_in_path", "https://evil.example/github.com/insights/insights/pull/123"),
        ]
    )
    @patch(_FLAG_TARGET, return_value=True)
    ...
    def test_pr_created_posts_separate_messages_for_distinct_prs(self, _flag) -> None:
        post_pr_created_thread_update(self.task_run, "https://github.com/insights/insights/pull/1")
        post_pr_created_thread_update(self.task_run, "https://github.com/insights/insights/pull/2")
    ...
    def test_pr_created_skips(self, _name, flag_on, has_creator, flag_mock) -> None:
        ...
        post_pr_created_thread_update(run, "https://github.com/insights/insights/pull/123")
    ...
    def test_pr_created_posts_when_run_patch_records_pr_url(self, _flag) -> None:
        ...
        with team_scope(self.team.id):
            update_task_run(
                ...
                self.task.id,
                self.team.id,
                validated_data={"output": {"pr_url": "https://github.com/insights/insights/pull/321"}},
            )

        messages = self._messages(self.task)
        self.assertEqual([message.event for message in messages], ["pr_created"])
        self.assertEqual(messages[0].payload, {"pr_url": "https://github.com/insights/insights/pull/321"})
    ...
    def test_pr_created_posts_when_set_output_records_pr_url(self, _flag) -> None:
        with team_scope(self.team.id):
            set_task_run_output(
                ...
                self.task.id,
                self.team.id,
                output={"pr_url": "https://github.com/insights/insights/pull/654"},
            )
    ...
    @parameterized.expand(
        [
            (
                "with_link",
                "Signups overview",
                "https://us.hanzo.ai/code/canvas/c/d",
                "[Signups overview](https://us.hanzo.ai/code/canvas/c/d) has been created",
            ),
            (
                "name_sanitized_for_link_token",
                "[Q3] KPIs",
                "https://us.hanzo.ai/code/canvas/c/d",
                "[Q3  KPIs](https://us.hanzo.ai/code/canvas/c/d) has been created",
            ),
