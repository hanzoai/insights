...
How to read Insights experiment results without falling into common interpretation pitfalls.
...
- C11 — External calculator disagrees with Insights
...
In Bayesian: Insights applies a minimum-sample-size guard before analysis proceeds — a low
...
- Frequentist: Insights uses α=0.05 by default → a single metric has ~5% chance of false-positive
...
Insights's calculations account for this; the visible ratio is not a bug.
...
4. **Is the implementation correct?** A large multiple-fold gap between equal-sized variants is
   **extremely unlikely** to be random — instrumentation is the more likely cause. A specific
   ...
   significant under the i.i.d. assumption. _Detect:_ read the generated `datastore_sql` from
...
Insights **does not** apply multiple-comparisons correction:
...
Insights defaults to Bayesian. Common misreads:
...
- **Non-informative priors.** Insights uses non-informative priors (mean 0, large variance). Early swings
...
- **Legacy methodology (pre-2025 experiments).** Experiments created before January 2025 may use
  ...
  expected, see Insights's
  [legacy-methodology docs page](https://hanzo.ai/docs/experiments/legacy-methodology).

## C6 — Frequentist interpretation traps [HIGH]

Insights has Frequentist support (rolled out June 2025). Set in `stats_config`. Quick rules:

- 95% CI **doesn't cross 0** → significant vs control. CI **crosses 0** → not significant.
- Insights uses **Welch's t-test** as the default — it handles unequal variance between groups, unlike
...
`insights:analyzing-experiment-session-replays` skill — it surfaces variant-level replay patterns and
...
Insights can mark a metric as significant and send a notification well before the experiment has
...
## C11 — External calculator disagrees with Insights [MEDIUM]

A common case: conversion counts from the experiment page get pasted into an online A/B
calculator, which returns a different verdict ("not significant" vs Insights's "significant", or
vice versa).

Two questions to ask before debugging stats:

1. **Which methodology?** Insights is **Bayesian by default**. Most online calculators are
...
2. **Are the inputs actually the same?** The numbers on the experiment page are post-scope —
   ...
   the user typed in raw event counts they grabbed from SQL, the calculator and Insights are
