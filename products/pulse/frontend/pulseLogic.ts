    ...
    listeners(({ actions, values, cache }) => ({
        ...
        generateBriefFailure: ({ errorObject }) => {
            if (errorObject instanceof ApiError && errorObject.status === 409) {
                toast.info(BRIEF_ALREADY_GENERATING_MESSAGE)
            }
        },
        startPolling: () => {
            ...
            try {
                ...
                results.forEach((result, index) => {
                    ...
                    if (rounds >= MAX_CONSECUTIVE_POLL_FAILURES) {
                        failures.delete(briefId)
                        insights.capture('pulse brief poll gave up', { brief_id: briefId })
        ...
        configSaved: ({ config, created }) => {
            insights.capture(created ? 'pulse config created' : 'pulse config updated', {
                config_id: config.id,
                name: config.name,
            })
            toast.success(created ? 'Brief config created' : 'Brief config updated')
            if (created) {
        ...
        submitConfigFormFailure: ({ error }) => {
            ...
            if (error instanceof ApiError) {
                ...
                toast.error(error.detail || 'Saving the brief config failed')
            }
        },
        deleteConfig: async ({ configId }) => {
            try {
                ...
                toast.error('Deleting the brief config failed')
                return
            }
            ...
            insights.capture('pulse config deleted', { config_id: configId })
            toast.success('Brief config deleted')
        },
    })),
