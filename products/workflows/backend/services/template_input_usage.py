"""Find workflows (InsightsFlows) that reference a given function-template input by its keys.
...
def get_hog_flows_referencing_template_input_keys(
    ...
    """
    references: dict[str, list[InsightsFlowReference]] = {}
    # Project to only the columns the scanner reads — the rest (trigger, edges, draft, …) are unused.
    for flow in InsightsFlow.objects.filter(team_id=team_id).only("id", "name", "status", "actions"):
        ref: InsightsFlowReference | None = None
        for key in _referenced_keys(flow.actions or [], template_id, input_key, only_value_key=only_value_key):
            if ref is None:
                ref = InsightsFlowReference(id=str(flow.id), name=flow.name or str(flow.id), status=flow.status)
