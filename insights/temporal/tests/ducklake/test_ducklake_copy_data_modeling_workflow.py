...
import insights.ducklake.verification.config as verification_config
from insights.ducklake.verification import DuckLakeCopyVerificationParameter, DuckLakeCopyVerificationQuery
from insights.temporal.ducklake import ducklake_copy_data_modeling_workflow as ducklake_module
from insights.temporal.ducklake.ducklake_copy_data_modeling_workflow import (
...
from insights.temporal.ducklake.types import DataModelingDuckLakeCopyInputs, DuckLakeCopyModelInput
...
async def test_prepare_data_modeling_ducklake_metadata_activity_returns_models(
    ...
    assert model_metadata.schema_name == f"insights_data_modeling_team_{ateam.pk}"
...
def test_copy_data_modeling_model_to_ducklake_activity_via_duckgres(monkeypatch):
    ...
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.HeartbeaterSync",
        MagicMock(return_value=mock_heartbeater),
    )

    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.is_dev_mode",
        MagicMock(return_value=False),
    )
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow._get_org_id_for_team",
        MagicMock(return_value="org-123"),
    )
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.get_duckgres_server_for_organization",
        MagicMock(return_value=_create_mock_server()),
    )
    mock_stage = MagicMock()
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.stage_delta_table",
        mock_stage,
    )
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.connect_to_duckgres",
        MagicMock(return_value=mock_conn),
    )
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.setup_duckgres_session",
        MagicMock(),
    )
    ...
    metadata = DuckLakeCopyModelMetadata(
        ...
        schema_name="insights_data_modeling_team_1",
        table_name="model_a",
        verification_queries=[],
        staging_uri="s3://test-bucket/__insights_staging/table",
    )
    ...
    assert "s3://test-bucket/__insights_staging/table" in str(table_call)
...
def test_verify_ducklake_copy_activity_runs_queries(monkeypatch):
    ...
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.get_duckgres_server_by_team_org",
        MagicMock(return_value=_create_mock_server()),
    )
    monkeypatch.setattr(
        "insights.temporal.ducklake.ducklake_copy_data_modeling_workflow.configure_connection",
        MagicMock(),
    )
...
def test_verify_ducklake_copy_activity_raises_when_no_catalog(monkeypatch):
    ...
    metadata = DuckLakeCopyModelMetadata(
        ...
        normalized_name="ducklake_model",
        source_table_uri="s3://source/table",
        schema_name="insights_data_modeling_team_42",
        table_name="model_a",
