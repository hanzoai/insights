"""Command classes for insightscli CLI."""
...
from insightscli.hooks import precheck_handlers
from insightscli.manifest import REPO_ROOT, get_manifest, get_services_for_command


def _find_insightscli_executable() -> list[str]:
    """Resolve the insightscli executable for recursive invocations.

    Resolution order:
    1. ``bin/insightscli`` under the repo root (Insights monorepo convention).
    2. ``insightscli`` on PATH (installed console script, e.g. via pip/uv).
    3. ``sys.executable -m insightscli`` (same interpreter, works anywhere the
       package is importable).
    """
    bin_insightscli = REPO_ROOT / "bin" / "insightscli"
    if bin_insightscli.exists():
        return [str(bin_insightscli)]

    on_path = shutil.which("insightscli")
    if on_path:
        return [on_path]

    return [sys.executable, "-m", "insightscli"]
...
class Command:
    ...
    def register(self, cli_group: click.Group) -> Any:
        ...
        cmd.insightscli_config = self.config  # type: ignore[attr-defined]
        return cmd
...
def _run_prechecks(prechecks: list[dict[str, Any]], yes: bool = False) -> bool:
    ...
    Handlers are registered via insightscli.hooks.register_precheck().
    """
...
def execute_command_config(
    ...
    """Execute a command config (recursive for insightscli/else).

    Handles all step/command formats:
    - String: insightscli command name (shorthand for insightscli: <name>)
    - Dict with insightscli: insightscli command, can have prompt/else modifiers
    - Dict with cmd: inline shell command
    - Dict with steps: composite (each step follows same rules)
    """
    insightscli_exe = _find_insightscli_executable()

    def _run_insightscli(name: str) -> None:
        click.echo(f"✨ Executing: {name}")
        cmd_args = [*insightscli_exe, name]
        if confirmed:
            cmd_args.append("--yes")
        _run(cmd_args, env=env)

    if isinstance(config, str):
        # String = insightscli command name
        _run_insightscli(config)

    elif isinstance(config, dict):
        if "insightscli" in config:
            insightscli_cmd = config["insightscli"]
            ...
            if prompt:
                ...
                if prompt is True:
                    ...
                    should_run = confirmed or _prompt_user(prompt, name=insightscli_cmd)
                else:
                    # String prompt: always ask - this is a user choice, not just confirmation
                    should_run = _prompt_user(prompt, name=insightscli_cmd)

                if should_run:
                    _run_insightscli(insightscli_cmd)
                elif "else" in config:
                ...
                # No prompt, just run the insightscli command
                _run_insightscli(insightscli_cmd)
...
class CompositeCommand(Command):
    """Command that runs multiple insightscli commands in sequence."""
    ...
    def get_underlying_command(self) -> str:
        ...
        for step in steps:
            if isinstance(step, str):
                step_strs.append(f"insightscli {step}")
            elif isinstance(step, dict):
                if "insightscli" in step:
                    step_strs.append(f"insightscli {step['insightscli']}")
                else:
...
class HogliCommand(Command):
    ...
    def get_underlying_command(self) -> str:
        """Return the wrapped insightscli command."""
        return f"insightscli {self.config.get('insightscli', '')}"
    ...
    def execute(self, *args: str, confirmed: bool = False) -> None:
        """Execute the wrapped insightscli command."""
