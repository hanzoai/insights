...
export const selfReadOnlyModeLogic = kea<selfReadOnlyModeLogicType>([
    ...
    listeners(({ actions, cache }) => ({
        escalate: ({ durationSeconds }) => {
            ...
            insights.capture?.('read_only_escalated', { duration_seconds: durationSeconds })
        },
        endEscalation: () => {
            cache.disposables.dispose('escalationTimer')
            toast.info('Back to read-only mode.', { toastId: 'read-only-resumed' })
            insights.capture?.('read_only_ended')
        },
        ...
        notifyBlocked: ({ method }) => {
            insights.capture?.('read_only_write_blocked', { method })
        },
    })),
    ...
    afterMount(({ actions }) => {
        ...
        insights.set_config({ before_send: dropReadOnlyExceptions })

        // The user-facing toast for blocked writes is shown by the standard
        // `e instanceof ApiError → toast.error(e.detail)` pattern that
    ...
    beforeUnmount(() => {
        ...
        // Releasing ownership of `before_send` — if Insights adds another filter
        // here in the future, it should compose rather than be clobbered.
        insights.set_config({ before_send: undefined })
    }),
])
