...
// Cap per field in characters. Bounds network payload and Datastore
...
export interface traceMessagesLazyLoaderLogicValues {
    getTraceMessages: (traceId: string) => TraceMessages | null | undefined
    loadingTraceIds: Set<string>
    messagesByTraceId: Record<string, TraceMessages | null>
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface traceMessagesLazyLoaderLogicActions {
    ensureTraceMessagesLoaded: (requests: TraceLazyLoadRequest[]) => {
        requests: TraceLazyLoadRequest[]
    }
    loadTraceMessagesBatchFailure: (requestedTraceIds: string[]) => {
        requestedTraceIds: string[]
    }
    loadTraceMessagesBatchSuccess: (
        results: Record<string, TraceMessages>,
        requestedTraceIds: string[]
    ) => {
        requestedTraceIds: string[]
        results: Record<string, TraceMessages>
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface traceMessagesLazyLoaderLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        getTraceMessages: (
            messagesByTraceId: Record<string, TraceMessages | null>
        ) => (traceId: string) => TraceMessages | null | undefined
    }
}

export type traceMessagesLazyLoaderLogicType = MakeLogicType<
    traceMessagesLazyLoaderLogicValues,
    traceMessagesLazyLoaderLogicActions,
    Record<string, any>,
    traceMessagesLazyLoaderLogicMeta
>

export const traceMessagesLazyLoaderLogic = kea<traceMessagesLazyLoaderLogicType>([
    path(['products', 'ai_observability', 'frontend', 'traceMessagesLazyLoaderLogic']),
                ...
                await Promise.allSettled(
                    chunks.map(async (batch) => {
                        try {
                            ...
                            // Stays narrow when rows are clustered in time, so Datastore
                            ...
                            // escapeInsightsQLString returns the value already wrapped in single quotes.
                            const idList = safe.map((s) => escapeInsightsQLString(s.id)).join(',')
...
// Parse Datastore-truncated JSON: strict parse, then partial-JSON recovery,
