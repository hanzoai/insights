...
Datastore sample query without touching the workflow wiring.
...
# 2 = Insights Cloud US — the project this repo dogfoods against.
GUARANTEED_TEAM_IDS: list[int] = [2]

# Activity envelope — generous timeout in case we add the Datastore sample
...
async def get_team_ids_for_mcp_analytics(inputs: TeamDiscoveryInput) -> list[int]:
    ...
    union with a Datastore-sampled set of teams that have ``$mcp_tool_call``
