...
// engineering_analytics flaky_tests endpoint for candidates, then one InsightsQL read
...
// and the relay.
//
//   GHA cron ──> flaky_tests endpoint + one InsightsQL query ──> Slack
//
...
const GITHUB_REPOSITORY = process.env.GITHUB_REPOSITORY || 'Insights/insights'
...
// `values` bind through InsightsQL's {placeholder} syntax, escaped server-side — never
    ...
    return withRetry(() =>
        request(
            `${HOST}/api/projects/${PROJECT_ID}/query/`,
            {
                ...
                body: JSON.stringify({ query: { kind: 'InsightsQLQuery', query, values } }),
            },
            'query'
        )
    )
...
function resolveOwners(items) {
    ...
    if (allPaths.length > 0) {
        try {
            const out = execFileSync('python3', ['-m', 'insights_owners'], {
                encoding: 'utf8',
                input: allPaths.join('\n'),
    ...
    const blocks = [
        ...
        },
        {
            ...
            elements: [
                {
                    type: 'mrkdwn',
                    text: 'Fix: `/fixing-flaky-tests` · Park 14 days: `insightscli test:quarantine add <test id>`',
                },
            ],
        },
    ]
