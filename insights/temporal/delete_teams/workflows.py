...
from insights.temporal.delete_teams.activities import (
    ...
    enqueue_datastore_deletion_activity,
...
class DeleteTeamsDataWorkflow(InsightsWorkflow):
    ...
    async def run(self, inputs: DeleteTeamsDataWorkflowInputs) -> None:
        ...
        # The bulky children are gone, so the Team row delete is cheap, then hand off to Datastore.
        await temporalio.workflow.execute_activity(
            delete_team_records_activity,
            team_inputs,
            start_to_close_timeout=LIGHT_ACTIVITY_TIMEOUT,
...
class DeleteProjectDataWorkflow(InsightsWorkflow):
...
class DeleteOrganizationWorkflow(InsightsWorkflow):
