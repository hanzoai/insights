            ...
            [] as SignalReportApi[],
            {
                loadLinkedReports: async (): Promise<SignalReportApi[]> => {
                    ...
                    try {
                        ...
                        // Supplementary context: a signals or Datastore hiccup must not break the ticket.
            ...
            (messages: CommentType[], ticket: Ticket | null): ChatMessage[] =>
                messages.map((message) => {
                    ...
                    if (message.created_by) {
                        ...
                        displayName = 'Insights Assistant'
                    } else {
    ...
    listeners(({ actions, values, props, cache }) => ({
        ...
        sendMessage: async ({ content, richContent, isPrivate, onSuccess, statusAfterSend }) => {
            ...
            try {
                ...
                toast.success(isPrivate ? 'Private note added' : 'Reply added')
                actions.setMessageSending(false)
                onSuccess?.()
                if (!isPrivate) {
                ...
                toast.error(values.editingMessageId ? 'Failed to update note' : 'Failed to send message')
                actions.setMessageSending(false)
            }
        },
