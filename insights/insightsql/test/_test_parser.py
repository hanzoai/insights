...
import pytest
from insights.test.base import BaseTest, MemoryLeakTestMixin, no_memory_leak_check
...
from insights.insightsql.errors import BaseInsightsQLError, ExposedInsightsQLError, SyntaxError
from insights.insightsql.parser import parse_expr, parse_order_expr, parse_program, parse_select, parse_string_template
from insights.insightsql.test.utils import pretty_dataclasses
...
def parser_test_factory(backend: InsightsQLParserBackend):
    ...
    class TestParser(*base_classes):  # type: ignore
        ...
        ) -> ast.SelectQuery | ast.SelectSetQuery | ast.InsightsQLXTag:
            return cast(
                ast.SelectQuery | ast.SelectSetQuery | ast.InsightsQLXTag,
        ...
        @parameterized.expand(
            [
                ...
                # Leading zeros are no-ops, never octal — "017" → 17, "09" → 9 — matching Datastore/Postgres.
                ...
                # `infinity` spelling (INF token matches it too) — accepted, matching Datastore.
        ...
        def test_binary_literals(self, _name: str, expr: str, expected: int):
            # `0b<binary-digits>` is a real lexer token; Datastore parses binary literals natively.
            self.assertEqual(self._expr(expr), ast.Constant(value=expected))
        ...
        def test_postgres_octal_literals_rejected(self, _name: str, query: str):
            # `0o<digits>` lexes as OCTAL_PREFIX_LITERAL and visitNumberLiteral throws — matches Datastore / pre-pg16 PG.
            with self.assertRaises((ExposedInsightsQLError, SyntaxError)):
                self._select(query)
        ...
        def test_oversized_binary_literals_rejected(self, _name: str, query: str):
            # Datastore caps binary literals at 64 bits — magnitude must fit UInt64 (positive) or Int64 (negative).
            with self.assertRaises((ExposedInsightsQLError, SyntaxError)):
                self._select(query)
        ...
        def test_unexpected_character_rejected(self, _name: str, program: str, code_point: str):
            ...
            with self.assertRaises((ExposedInsightsQLError, SyntaxError)) as caught:
        ...
        def test_clause_keywords_reused_as_identifiers_throughout_query(self):
            # Every word in this query is a InsightsQL clause/operator keyword
        ...
        def test_table_function_arg_rejects_bare_select_subquery(self):
            ...
            for src in (
                ...
                with self.assertRaises((ExposedInsightsQLError, SyntaxError), msg=f"{backend}: {src!r}"):
        ...
        def test_select_placeholders(self):
            self.assertEqual(
                self._select("select 1 where 1 == {insightsql_val_1}"),
                ast.SelectQuery(
                    select=[ast.Constant(value=1)],
                    where=ast.CompareOperation(
                        op=ast.CompareOperationOp.Eq,
                        left=ast.Constant(value=1),
                        right=ast.Placeholder(expr=ast.Field(chain=["insightsql_val_1"])),
                    ),
                ),
            )
            self.assertEqual(
                self._select(
                    "select 1 where 1 == {insightsql_val_1}",
                    {"insightsql_val_1": ast.Constant(value="bar")},
                ),
                ast.SelectQuery(
                    select=[ast.Constant(value=1)],
                    where=ast.CompareOperation(
        ...
        def test_placeholder_expressions(self):
            actual = self._select("select 1 where 1 == {1 ? insightsql_val_1 : insightsql_val_2}")
                    ...
                    where=ast.CompareOperation(
                        ...
                        right=ast.Placeholder(
                            expr=ast.Call(
                                ...
                                args=[
                                    ast.Constant(value=1),
                                    ast.Field(chain=["insightsql_val_1"]),
                                    ast.Field(chain=["insightsql_val_2"]),
                                ],
                            )
                        ),
                    ),
        ...
        def test_statement_level_sample_rejected(self, _name: str, query: str):
            # `selectStmt`-level `(USING? sampleClause)?` is DuckDB's `USING SAMPLE`, which InsightsQL has no AST home for; every backend rejects rather than silently dropping it.
            with self.assertRaises((ExposedInsightsQLError, SyntaxError)):
                self._select(query)
        ...
        def test_visit_insightsqlx_tag_nested(self):
            # Basic case
            node = self._select(
                "select event from <OuterQuery><InsightsQLQuery query='select event from events' /></OuterQuery>"
            )
            assert isinstance(node, ast.SelectQuery)
            assert isinstance(node.select_from, ast.JoinExpr)
            table_node = node.select_from.table
            ...
            table_node = node.select_from.table
            assert isinstance(table_node, ast.InsightsQLXTag)
            assert table_node == ast.InsightsQLXTag(kind="OuterQuery", attributes=[])

            # With attribute
            node = self._select(
                "select event from <OuterQuery q='b'><InsightsQLQuery query='select event from events' /></OuterQuery>"
            )
            assert isinstance(node, ast.SelectQuery)
            assert isinstance(node.select_from, ast.JoinExpr)
            table_node = node.select_from.table
            ...
            assert table_node == ast.InsightsQLXTag(
                kind="OuterQuery",
                attributes=[
                    ast.InsightsQLXAttribute(name="q", value=ast.Constant(value="b")),
                    ast.InsightsQLXAttribute(
                        name="children",
                        value=[
                            ast.InsightsQLXTag(
            ...
            with self.assertRaises(ExposedInsightsQLError) as e:
                self._select(
                    "select event from <OuterQuery children='b'><InsightsQLQuery query='select event from events' /></OuterQuery>"
                )
            assert str(e.exception) == "Can't have a InsightsQLX tag with both children and a 'children' attribute"

        def test_visit_insightsqlx_tag_alias(self):
            node = self._select("select event from <InsightsQLQuery query='select event from events' /> as a")
            assert isinstance(node, ast.SelectQuery)
            assert isinstance(node.select_from, ast.JoinExpr)
            table_node = node.select_from.table
            ...
            node2 = self._select("select event from <InsightsQLQuery query='select event from events' /> a")
            assert node2 == node

        def test_visit_insightsqlx_tag_source(self):
            query = """
                select id, email from (
                    <ActorsQuery
                        select={['id', 'properties.email as email']}
                        source={
                            <InsightsQLQuery query='select distinct person_id from events' />
                        }
                    />
                )
            """
        ...
        def test_visit_insightsqlx_tag_column_source(self):
            query = """
                select <a href='https://google.com'>{event}</a> from events
            """
            node = self._select(query)
            assert isinstance(node, ast.SelectQuery) and cast(ast.InsightsQLXTag, node.select[0]) == ast.InsightsQLXTag(
                kind="a",
                attributes=[
                    ast.InsightsQLXAttribute(name="href", value=Constant(value="https://google.com")),
                    ast.InsightsQLXAttribute(name="children", value=[ast.Field(chain=["event"])]),
                ],
            )

        def test_visit_insightsqlx_multiple_children(self):
            query = """
                    ...
                    ast.InsightsQLXAttribute(
                        ...
                        value=[
                            ...
                            ast.InsightsQLXTag(
                                kind="b",
                                attributes=[
                                    ast.InsightsQLXAttribute(name="children", value=[ast.Constant(value="Bold!")]),
                                ],
                            ),
                        ],
                    ),
        ...
        def test_visit_insightsqlx_text_only_child(self):
            ...
            assert isinstance(node, ast.SelectQuery)
            tag = cast(ast.InsightsQLXTag, node.select[0])
            self.assertEqual(
                tag,
                ast.InsightsQLXTag(
                    kind="span",
        ...
        def test_visit_insightsqlx_text_and_expr_children(self):
        ...
        def test_visit_insightsqlx_nested_tags(self) -> None:
        ...
        # 2. <em />
        def test_visit_insightsqlx_self_closing(self) -> None:
        ...
        def test_return_insightsqlx_tag_value(self):
            ...
            self.assertEqual(stmt.expr, ast.InsightsQLXTag(kind="Sparkline", attributes=[]))
            nested = cast(ast.ReturnStatement, cast(ast.Program, self._program("return <a>x</a>")).declarations[0])
            self.assertEqual(
                nested.expr,
                ast.InsightsQLXTag(
                    kind="a",
                    attributes=[ast.InsightsQLXAttribute(name="children", value=[ast.Constant(value="x")])],
                ),
            )
        ...
        def test_visit_insightsqlx_expr_text_and_tag_children(self) -> None:
        ...
        def test_visit_insightsqlx_mixed_nested_attributes(self) -> None:
            node = self._select(
                "select <strong><a href='https://google.com'>Hello <em>{event}</em></a>{'a'}</strong> from events"
            )
            assert isinstance(node, ast.SelectQuery)
            outer = cast(ast.InsightsQLXTag, node.select[0])

            expected = ast.InsightsQLXTag(
                kind="strong",
                attributes=[
                    ast.InsightsQLXAttribute(
                        name="children",
        ...
        # Regression tests: “<” operator vs INSIGHTSQLX-tag opener
        ...
        def test_program_bare_throw_rejected(self):
            # `throwStmt` requires an expression — Script has no bare-throw / implicit re-throw.
            with self.assertRaises((ExposedInsightsQLError, SyntaxError)):
                self._program("throw")
            with self.assertRaises((ExposedInsightsQLError, SyntaxError)):
                self._program("throw;")
        ...
        def test_statement_keywords_rejected_as_expressions(self):
            # Script statement keywords (`fn`, `let`, `while`, …) aren't in the `keyword` rule, unlike `if` / `for` / `return`.
            for kw in ("fn", "fun", "let", "while", "throw", "try", "catch", "finally"):
                with self.assertRaises(ExposedInsightsQLError, msg=f"{backend}: {kw!r} should reject"):
        ...
        def test_reserved_keyword_alias_rejected(self):
            ...
            for src in (
                ...
                with self.assertRaises(ExposedInsightsQLError, msg=f"{backend}: {src!r}"):
                    parse_select(src, backend=backend)
        ...
        def test_settings_and_top_clause_error_class(self):
            # SETTINGS/TOP parse but the visitor rejects them — `NotImplementedError` is `InternalInsightsQLError` and is rewritten to `ExposedInsightsQLError` at the parser boundary.
            for src in ("SELECT 1 SETTINGS x = 1", "SELECT TOP 5 x FROM t"):
                with self.assertRaises(ExposedInsightsQLError) as cpp_cm:
                    parse_select(src, backend="cpp-json")
                with self.assertRaises(ExposedInsightsQLError) as backend_cm:
                    parse_select(src, backend=backend)
        ...
        def test_hex_e_is_hex_digit_not_exponent_marker(self):
            # `0x1e+4` is `0x1e` (=30) + `4`, not a hex-float — only `p`/`P` marks the exponent (strict C99, matches Datastore).
        ...
        def test_hex_float_no_integer_part_rejected(self):
            ...
            with self.assertRaises(ExposedInsightsQLError, msg=f"{backend}: '0x.8p3'"):
        ...
        def test_between_binds_tighter_than_and_or(self):
            ...
            # BETWEEN also binds tighter than prefix NOT now (matching Datastore
        ...
        def test_between_bound_rejects_lambda_and_named_argument(self):
            ...
            for query in (
                ...
                with self.assertRaises(BaseInsightsQLError, msg=f"{backend}: {query!r}"):
        ...
        def test_join_op_modifier_arity_validation(self):
            ...
            for src in invalid:
                with self.assertRaises((BaseInsightsQLError, SyntaxError), msg=f"{backend}: {src!r}"):
                    parse_select(src, backend=backend)
        ...
        def test_pivot_in_list_must_be_non_empty(self):
            ...
            with self.assertRaises((BaseInsightsQLError, SyntaxError)):
        ...
        def test_interpolate_no_trailing_comma(self):
            ...
            with self.assertRaises((BaseInsightsQLError, SyntaxError), msg=backend):
                parse_select(
        ...
        def test_reserved_keywords_rejected_as_identifiers(self):
            # `identifier` excludes NULL_SQL/INF/NAN_SQL/EXCEPT/INTERSECT and Script-statement keywords (FN/FUN/LET/WHILE/THROW/TRY/CATCH/FINALLY).
        ...
        def test_insightsqlx_tag_in_from_paren_decorations(self):
        ...
        def test_insightsqlx_tag_text_accepts_arbitrary_non_brace_non_lt(self):
            # `INSIGHTSQLX_TEXT_TEXT: ~[<{]+` — any byte except `<` and `{` is valid tag-body text.
            cases = (
                "<a>foo&bar</a>",
                "<a>foo!</a>",
                "<a>foo@bar</a>",
        ...
        def test_insightsqlx_tag_identifier_allows_hyphens(self):
            # `INSIGHTSQLX_TAG_OPEN`/`_CLOSE` modes admit `[a-zA-Z_][a-zA-Z0-9_-]*` for tag and attribute names — hyphens are part of the ident.
            cases = (
                "<a-b />",
                "<a-b-c />",
        ...
        def test_insightsqlx_tight_vs_loose_tags(self):
        ...
        def test_bare_zero_x_prefix_lexes_as_zero_plus_ident(self):
            ...
            for src in cases:
                with self.assertRaises((BaseInsightsQLError, SyntaxError), msg=src):
                    parse_select(src, backend="cpp-json")
                with self.assertRaises((BaseInsightsQLError, SyntaxError), msg=src):
                    parse_select(src, backend=backend)
        ...
        def test_insightsqlx_comments_skipped_between_attributes(self):
            # `INSIGHTSQLX_TAG_OPEN`/`_CLOSE` modes skip block + line comments and route unknown bytes to UNEXPECTED_CHARACTER.
            cases = (
                "<a /*c*/ b={1}/>",
                "<a /* c */b={1}/>",
                "<a -- comment\n b={1}/>",
        ...
        def test_enum_cast_rejected_as_unsupported(self):
            ...
            for src in (
                ...
                with self.assertRaises(ExposedInsightsQLError, msg=src) as cpp_cm:
                    parse_expr(src, backend="cpp-json")
                with self.assertRaises(ExposedInsightsQLError, msg=src) as rust_cm:
                    parse_expr(src, backend=backend)
        ...
        def test_interval_combined_string_validates_count_and_unit(self):
            ...
            cases = (
                ...
                # Datastore stores intervals as Int64, so both parsers convert the count with `std::stoll` (i64); a value past Int64 max is rejected as too large. Both parsers emit this exact message (no leaked stdlib `stoll` text), so assert it in full.
                (
            ...
            # Counts past int32 (`2147483648`) up to Int64 max (`9223372036854775807`) must parse — Datastore stores intervals as Int64, so `std::stoll` accepts the whole range. This guards the boundary against the out_of_range reject case above.
            for src in (
                "INTERVAL 'a' || 'b' hour",
                "INTERVAL 'x' day",
        ...
        def test_join_constraint_rejected_on_lead_table_and_cross_join(self):
            ...
            for src in (
                ...
                # Statement-level `USING SAMPLE` (the peel loop must not treat the `USING` as a join constraint): it's DuckDB's statement-level sample, which InsightsQL doesn't implement, so it's rejected rather than silently dropped.
        ...
        def test_columns_exclude_replace_reject_reserved_keywords(self):
            # `columnsExcludeItem` and `columnsReplaceItem` use the strict `identifier` rule (excludes NULL/INF/NAN/EXCEPT/INTERSECT + Script-stmt keywords).
            for src in (
        ...
        def test_invalid_interval_in_block_body_rejected(self):
            ...
            # `interval`-as-Field. Inside a Script `{ … }` block body the fall-back would
            ...
            with self.assertRaises(BaseInsightsQLError):
        ...
        def test_date_timestamp_literal_in_block_body_rejected(self):
            ...
            # identifier and strand the string; otherwise inside a Script `{ … }` block body
        ...
        def test_arrow_lambda_block_body_span_with_trailing_postfix(self):
            # An arrow lambda with a Script BLOCK body (`x -> { … }`) ends at `}`, so a
        ...
        def test_not_and_modulo_accept_insightsqlx_tag_operand(self):
            # `<` begins a InsightsQLX tag as well as the less-than operator. In bounded
        ...
        def test_insightsqlx_tag_name_rejects_non_identifier_keywords(self):
            # A InsightsQLX tag/attr name is a grammar `identifier`, so a keyword head is
            # valid only when the grammar's `keyword` rule admits it. The Script-statement
        ...
        def test_insightsqlx_attribute_and_text_child_positions_match(self):
            # cpp positions each InsightsQLXAttribute (name start -> value end, or name end
        ...
        def test_lambda_keyword_is_a_plain_alias_after_as(self):
            ...
            # needs parentheses (`(1 as lambda) + 2`). This matches Datastore/SQL.
        ...
        def test_leading_comma_from_implicit_alias_dangling_clause(self):
            ...
            for query in (
                ...
                with self.assertRaises(BaseInsightsQLError, msg=query):
                    parse_select(query, backend=backend)
        ...
        def test_window_filter_body_suppresses_visitor_rejections(self):
            ...
            for query in strict:
                with self.assertRaises((BaseInsightsQLError, SyntaxError), msg=query):
                    parse_expr(query, backend=backend)
        ...
        def test_nested_string_interval_dispatches_on_trailing_unit_count(self):
            ...
            with self.assertRaises((BaseInsightsQLError, SyntaxError), msg="interval interval 'bm ' day"):
        ...
        def test_cast_type_enum_template_string_key_rejected(self):
            ...
            for query in (
                ...
                with self.assertRaises(BaseInsightsQLError, msg=f"{backend}: {query}"):
                    parse_expr(query, backend=backend)
