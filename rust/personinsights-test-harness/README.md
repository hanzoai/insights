...
Load, consistency, and e2e correctness harness for the personinsights leader path.
Revived from the original personinsights-cannon draft (#55581) and extended with stack orchestration, Postgres seeding, and an acked-write journal so it can gate CI, not just generate load.
...
The docker-compose dev dependencies must be running: Postgres (`insights_persons`), Kafka, and etcd (`COMPOSE_PROFILES=etcd`).
For `gate` without `--external-router-url`, build the service binaries first:

```bash
cargo build -p personinsights-replica -p personinsights-router -p personinsights-leader -p personinsights-writer -p personinsights-test-harness
```
...
```bash
target/debug/personinsights-test-harness gate --duration 10s --persons 100 --concurrency 10

# More leaders/partitions
target/debug/personinsights-test-harness gate --leaders 3 --partitions 8 --duration 30s

# Against an already-running stack instead of spawning one. The dev stack's
# writer targets personinsights_person_tmp, so tell the verifier to read it there.
target/debug/personinsights-test-harness gate --external-router-url http://127.0.0.1:50054 \
  --pg-target-table personinsights_person_tmp
```

The spawned stack is isolated from the dev stack: its own port range (24xxx, kept below the ephemeral range so outbound connections cannot steal a listen port), its own etcd prefix (`/personinsights-test-harness/`), and a per-run changelog topic (`personinsights_test_harness_<run_id>`, deleted on teardown).
...
`--create-via-identity` swaps SQL seeding for the personinsights-identity get-or-create path: the spawned stack also runs `personinsights-identity`, persons are created through `GetOrCreatePersonsByDistinctIds` (stub insert on the Postgres primary, then initial `$set` properties through the router to the owning leader), and the update traffic then targets the created persons.
...
target/debug/personinsights-test-harness gate --create-via-identity \
...
target/debug/personinsights-test-harness gate --leaders 3 --duration 15s --kill-after 5s
...
target/debug/personinsights-test-harness gate --leaders 3 --duration 20s \
  --leader-lease-ttl 5 --kill-after 4s --kill-fast false

# Graceful shutdown: SIGTERM, drain, partitions hand off while traffic flows
target/debug/personinsights-test-harness gate --leaders 3 --duration 15s --shutdown-after 5s

# Scale out mid-traffic, then crash the busiest leader
target/debug/personinsights-test-harness gate --leaders 2 --duration 20s --scale-up-after 5s --kill-after 12s

# Zombie: SIGSTOP + lease revoke so ownership moves, then SIGCONT the old
# owner — it wakes believing it still owns its partitions
target/debug/personinsights-test-harness gate --leaders 3 --duration 20s --zombie-after 4s --zombie-duration 8s
...
target/debug/personinsights-test-harness gate --leaders 3 --duration 15s --restart-after 5s

# Writer crash-restart (at-least-once redelivery under the version guard)
# and writer pause/resume (controlled lag injection)
target/debug/personinsights-test-harness gate --duration 15s --writer-crash-after 5s
target/debug/personinsights-test-harness gate --duration 15s --writer-pause-after 3s --writer-pause-duration 8s
...
target/debug/personinsights-test-harness gate --leaders 3 --routers 3 --duration 20s \
...
```bash
target/debug/personinsights-test-harness gate --leaders 3 --partitions 8 --persons 50 \
...
target/debug/personinsights-test-harness gate --routers 3 --leaders 3 --duration 15s \
...
target/debug/personinsights-test-harness gate --routers 3 --leaders 3 --duration 18s \
...
Build this only when `personinsights_leader_recovery_pool_wait_ms` shows sustained queuing.
...
target/debug/personinsights-test-harness seed --team-id 900001 --count 100

# Remove everything the harness wrote for a team
target/debug/personinsights-test-harness cleanup --team-id 900001
```
...
```bash
target/debug/personinsights-test-harness blast \
...
```bash
target/debug/personinsights-test-harness consistency \
...
```text
=== personinsights-test-harness gate results ===
...
Set `RUST_LOG=personinsights_test_harness=debug` for per-request logging.
