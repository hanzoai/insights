...
| Config fields (filters, limits, sort)  | `widget_specs/configs.py` (Pydantic SSOT), `WidgetSpec.form_fields`, `Edit*WidgetModal.tsx`, `*WidgetConfigValidation.ts` (API errors)    | `generated/widget-configs.zod.ts`, `insightscli build:openapi` (OpenAPI/Zod derive from `WIDGET_SPECS`), registry **`parseConfigApiError`**, MCP catalog `config_schema` snapshots |
...
3. **Touch every layer** for non-breaking adds/removals:
   ...
   - Codegen: `insightscli build:openapi` — OpenAPI + `widget-configs.zod.ts` + MCP schemas regenerate from Pydantic (new `widget_type` needs a `*WidgetConfig` Pydantic model + `form_fields` on `WidgetSpec`)
   - Stories/fixtures: component stories, edit modal stories, preview, `widgetOverviewStoryFixtures.ts`
4. **Relative date ranges** — update `backend/constants.py` (`WIDGET_DATE_FROM_VALUES_ORDERED`) and Pydantic `WidgetDateFrom` in `widget_specs/common.py`, then `insightscli build:openapi` (regenerates `widget-date-from-options.json`; `widgetConfigShared.ts` re-exports `WIDGET_DATE_RANGE_SELECT_OPTIONS` from that JSON).
...
- Full-width fields (name, description, test-account filter): put `WIDGET_SETTINGS_FIELD_FULL_WIDTH_CLASS` (`sm:col-span-2`) on the **grid child** — `Field.Pure` or a wrapper `<div>` — **not** on the inner `Input` / `TextArea`.
- Half-width fields (date range, sort, limit): omit `col-span-2`; use `fullWidth` on `Select` where needed.
...
Do not remove backend validation for a type that still has rows in `insights_dashboardwidget`.
