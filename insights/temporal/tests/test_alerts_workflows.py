...
from insights.slo.types import SloArea, SloConfig, SloOperation, SloOutcome
from insights.tasks.alerts.utils import AlertEvaluationResult
from insights.temporal.alerts.activities import evaluate_alert, notify_alert, prepare_alert
from insights.temporal.alerts.schedule import create_schedule_due_alert_checks_schedule
from insights.temporal.alerts.types import AlertInfo, CheckAlertWorkflowInputs, SkipReason
from insights.temporal.alerts.workflows import CheckAlertWorkflow, ScheduleDueAlertChecksWorkflow
...
async def test_schedule_due_alert_checks_adds_shared_slo_context() -> None:
    ...
    with (
        patch(
            "insights.temporal.alerts.workflows.temporalio.workflow.execute_activity",
            new=AsyncMock(return_value=[alert]),
        ),
        patch(
            "insights.temporal.alerts.workflows.temporalio.workflow.execute_child_workflow", new=AsyncMock()
        ) as execute_child,
    ):
...
def test_schedule_is_registered_in_init_schedules():
    from insights.temporal.schedule import schedules
...
async def alert_with_subscriber(ateam, aorganization):
    ...
    def _create() -> AlertConfiguration:
        user = User.objects.create_and_join(
            ...
            email=f"alerts-wf-{uuid.uuid4().hex[:6]}@hanzo.ai",
            password=None,
        )
...
@patch("insights.slo.events.hanzo_insights")
@pytest.mark.asyncio
...
async def test_check_alert_workflow_firing_drives_full_chain_with_slo(
    ...
    recipients = ["alerts-wf-test@hanzo.ai"]

    with (
        patch("insights.temporal.alerts.activities.check_alert_for_insight", return_value=evaluation_result),
        patch(
            "insights.tasks.alerts.utils.send_notifications_for_breaches", return_value=recipients
...
async def test_check_alert_workflow_skip_short_circuits_before_evaluate(
    ...
    with (
        patch("insights.temporal.alerts.activities.check_alert_for_insight") as mock_ch_query,
        patch("insights.tasks.alerts.utils.send_notifications_for_breaches") as mock_send_breaches,
    ):
        await _run_check_alert_workflow(
...
async def test_check_alert_workflow_records_errored_check_on_permanent_evaluation_failure(
    ...
    with (
        patch(
            "insights.temporal.alerts.activities.check_alert_for_insight",
            side_effect=PermanentError("insight query broken"),
        ),
        patch(
            "insights.tasks.alerts.utils.send_notifications_for_errors",
            return_value=["alerts-wf-test@hanzo.ai"],
        ) as mock_send_errors,
    ):
        await _run_check_alert_workflow(
            alert_id=str(alert_with_subscriber.id),
