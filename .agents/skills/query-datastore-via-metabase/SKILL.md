---
name: query-datastore-via-metabase
description: >
  Run Datastore `system.query_log` analysis via the internal Metabase API.
  Use when investigating slow queries, materialization candidates, per-team
  query performance, Datastore cost or memory issues, or any system.query_log
  question. Covers prod-us and prod-eu, SSO-gated cookie auth via `insightscli`,
  and ready-to-run query patterns.
---

# Querying Datastore via Metabase

Insights's production Datastore clusters are reachable for ad-hoc analysis through
...
This skill is for `system.query_log` analysis from inside the insights repo.
...
```bash
insightscli metabase:databases --region us
insightscli metabase:databases --region eu
```

Regional layout (names may vary; re-check with `metabase:databases`):

- **US** exposes one Datastore database (used for `query_log` and data reads).
- **EU** exposes two Datastore databases — a **query tier** (use for
...
Use `insightscli` to get a valid cookie. It opens the system browser for SSO,
captures cookies from the user's logged-in browser profile, and caches them
at `~/.config/insights/metabase/cookie-{region}` (mode `0600`).
...
# is cheap.
insightscli metabase:login --region us
insightscli metabase:login --region eu
```

**Prompt the user to run `insightscli metabase:login` themselves** — the harness
blocks Keychain access from agent shells, so the user has to authenticate
interactively.

### Agents: use `metabase:query`

`insightscli metabase:query` reads the cached cookie internally and only emits
...
1. Discover the current Datastore DB ID: `insightscli metabase:databases --region <region>`.
2. Pass that ID into `insightscli metabase:query`. Pipe SQL via stdin or `--file`.

```bash
# 1. Find the Datastore database ID for your region
insightscli metabase:databases --region us
# e.g. output row:  42  Datastore  datastore

# 2. Run the query. The cookie is read internally; nothing leaks to stdout.
insightscli metabase:query --region us --database-id 42 --save /tmp/out.tsv <<'SQL'
SELECT
...
`clusterAllReplicas(insights, system, query_log)` is the standard table reference —
...
```text
# US
https://metabase.prod-us.insights.dev/question/795-look-up-query-by-query-id?query_id=<ID>&include_query_start=No&event_date=<YYYY-MM-DD>

# EU (same card ID may differ — find it in EU Metabase if 795 doesn't resolve)
https://metabase.prod-eu.insights.dev/question/795-look-up-query-by-query-id?query_id=<ID>&include_query_start=No&event_date=<YYYY-MM-DD>
```
...
- **Cookie scope.** Each region has its own cookie cache. Run `insightscli metabase:login --region <region>` for every region you need; `--region` is required.
