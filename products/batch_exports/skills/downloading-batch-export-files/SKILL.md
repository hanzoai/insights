...
description: >
  Export Insights events, persons, or sessions on demand and download the resulting files. Use when the user asks to
  download/export raw Insights data, create a one-off file export, fetch a Parquet or JSONLines export, or use the
...
Use this skill when a user wants a one-off downloadable export of Insights data.
...
Call `insights:file-download-batch-exports-create` with the selected shape.
...
Call `insights:file-download-batch-exports-retrieve` with the returned `id`.
...
If required by the user, a running export can be cancelled by calling `insights:file-download-batch-exports-cancel-create` with the returned `id`.
...
Use the same Insights authentication context as other API calls.
...
```text
insights-events-<run_id>-<part>.jsonl.gz
insights-persons-<run_id>-<part>.parquet
```
