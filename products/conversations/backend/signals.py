...
from insights.models.signals import secret_api_token_rotated
...
def update_ticket_on_message(sender, instance: Comment, created: bool, **kwargs):
    ...
    def do_update():
        ...
        try:
            ...
            # Internal analytics (Insights tracking its own usage)
            ...
            if ticket.message_count == 1 and not is_team_message:
                try:
                    ...
                    if conversations_settings.get("notification_recipients"):
                        # insights.tasks.__init__ eagerly imports every task module; this signal
                        # module is wired at django.setup(), so import the task lazily.
                        from insights.tasks.email import send_new_ticket_notification  # noqa: PLC0415
