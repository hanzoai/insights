...
_STRING_COLUMN = {"insightsql": "StringDatabaseField", "datastore": "Nullable(String)", "schema_valid": True}
...
def _require_incident_warehouse_paths(team: Team, user: User) -> None:
    ...
    for name, query in _INCIDENT_WAREHOUSE_QUERIES.items():
        try:
            response = execute_insightsql_query(query, team=team, user=user)
...
def _accept_seeded_relationship(
    *,
    ...
    with patch("products.data_catalog.backend.logic.relationships.execute_insightsql_query"):
