...
function SaveDiscardActions({
    ...
    const handleSave = (): void => {
        if (queryInput) {
            updateView({
                id: savedQuery.id,
                query: { kind: NodeKind.InsightsQLQuery, query: queryInput },
                types: [],
            })
        }
    }
    ...
    return (
        <>
            <Button type="secondary" onClick={() => setQueryInput(queryString)} size="small">
                Discard
            </Button>
            <AccessControlAction
            ...
            >
                <Button type="primary" onClick={handleSave} loading={updatingDataWarehouseSavedQuery} size="small">
                    Save
                </Button>
            </AccessControlAction>
        </>
