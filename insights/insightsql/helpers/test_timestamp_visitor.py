...
def _make_insightsql_context() -> InsightsQLContext:
    return InsightsQLContext(team_id=1)
...
class TestTimestampVisitorSelectSetQuery(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            (
                "IsSimpleTimestampFieldExpressionVisitor",
                lambda: IsSimpleTimestampFieldExpressionVisitor(_make_insightsql_context(), None),
            ),
    ...
    def test_select_set_query_does_not_raise_not_implemented(self) -> None:
        node = _make_select_set_query()
        visitors = [
            IsSimpleTimestampFieldExpressionVisitor(_make_insightsql_context(), None),
    ...
    @parameterized.expand(
        [
            (
                "is_simple_timestamp_field_expression",
                lambda n: is_simple_timestamp_field_expression(n, _make_insightsql_context()),
            ),
...
class TestTimestampVisitorTypeCast(unittest.TestCase):
    ...
    def test_cast_preserves_simple_timestamp_field_recognition(
        ...
        self.assertEqual(is_simple_timestamp_field_expression(node, _make_insightsql_context()), expected)
...
class TestParseZonedDatetimeString(unittest.TestCase):
    @parameterized.expand(
        [
            ...
            # No timezone -> left for Datastore to interpret in the field timezone.
