...
@stdlib_dataclass(frozen=True)
class UserBasicInfo:
    """Mirror of ``insights.api.shared.UserBasicSerializer`` output, field-for-field.
    ...
    generated ``UserBasic`` OpenAPI component identical. ``mascot_config`` stays the
    raw stored value; ``UserBasicSerializer.get_mascot_config`` shapes it at render
