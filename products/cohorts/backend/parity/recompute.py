...
``event_filters`` (property matching is ScriptVM bytecode, not reproducible in SQL), and whole-day
...
Pure — no Datastore, no Kafka, no Django. The Datastore member-set / segmentation reads and the
...
class OracleLeaf:
    """A recompute-reproducible behavioral leaf: one member-set Datastore query per leaf key."""
...
def _parse_behavioral_leaf(node: Mapping[str, Any], ctx: _ScreenCtx) -> Optional[_TreeLeaf]:
    ...
    if pinned is None:
        ctx.fail("parse_error")
        return None
    # Property matching is ScriptVM bytecode; the SQL member-set query cannot reproduce it.
