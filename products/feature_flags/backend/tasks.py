...
from insights.api.capture import capture_batch_internal
...
def migrate_feature_enrollment_on_key_change(team_id: int, old_key: str, flag_id: int) -> None:
    ...
    while True:
        # Property access (rather than JSONExtractString) lets the InsightsQL printer use
...
def refresh_expiring_flag_definitions_cache_entries(self: PushGatewayTask) -> None:
    ...
    from insights.storage.cache_expiry_manager import refresh_expiring_caches

    successful_gauge = Gauge(
        "insights_flag_definitions_cache_refresh_successful_count",
        "Number of flag definitions caches successfully refreshed",
        registry=self.metrics_registry,
    )
    failed_gauge = Gauge(
        "insights_flag_definitions_cache_refresh_failed_count",
...
def cleanup_stale_flag_definitions_expiry_tracking_task(self: PushGatewayTask) -> None:
    ...
    from insights.storage.cache_expiry_manager import cleanup_stale_expiry_tracking

    entries_cleaned_gauge = Gauge(
        "insights_cleanup_stale_flag_definitions_expiry_entries_cleaned",
...
def feature_flags_local_eval_canary_task(self: PushGatewayTask) -> None:
    ...
    Metrics:
    - insights_feature_flags_local_eval_canary_group_mapping_present (gauge, 1/0)
    - insights_feature_flags_local_eval_canary_failure_total (counter)
    """
    if settings.FEATURE_FLAGS_CANARY_TEAM_ID is None:
        return

    lock_key = "insights:feature_flags_local_eval_canary:lock"
