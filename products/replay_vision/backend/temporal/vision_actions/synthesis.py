...
# Matches how insight AI summaries synthesize: Insights AI through the LLM gateway
...
# Markdown links in the report body (e.g. the alert header's scanner link). Only Insights-hosted links
...
def _synthesize(inputs: SynthesizeGroupSummaryInputs) -> SynthesizeGroupSummaryResult:
    ...
    # links) so the Insights URL survives on non-hanzo.ai hosts.
...
def _run_synthesis(team: Team, action: VisionAction, lines: list[str]) -> str:
    ...
    # Insights AI, matching insight AI summaries: the Insights-instrumented OpenAI client pointed at
    ...
    client = OpenAI(insights_client=hanzo_insights.setup(), base_url=settings.OPENAI_BASE_URL, max_retries=3)
    ...
    response = client.chat.completions.create(  # type: ignore[call-overload]
        ...
        insights_groups={**groups(team=team), "project": str(team.id)},
    )
...
def _linkify_summary_header(markdown: str, scanner_name: str, run_url: str) -> str:
    ...
    cited observation. Added AFTER `strip_external_links_markdown` so the Insights URL isn't defanged on
    instances whose SITE_URL isn't a hanzo.ai host (self-hosted, dev). If the strip pass rewrote the
...
def _markdown_to_slack(markdown: str, *, team_id: int, observation_ids: list[str]) -> str:
    ...
    `[N]` links to each observation, and (Insights-only) Markdown links become `<url|label>`. Truncates
    long reports."""
