...
All queries run against `insights.query_log_archive` via `insightscli metabase:query --region <us|eu>
--database-id <id>` (discover the id with `insightscli metabase:databases`). Save wide results with
...
```sql
AND normalized_query_hash != normalizedQueryHash('SELECT hostname(), query_id FROM clusterAllReplicas(insights, system.processes) WHERE query_id LIKE ? SETTINGS max_execution_time = 2')
```
...
Do not wrap the slow predicate in a CTE that projects only some columns: Datastore's analyzer prunes
...
[`optimizing-datastore-and-insightsql-queries`](../../optimizing-datastore-and-insightsql-queries/references/investigation-playbook.md)
...
```sql
SELECT * FROM insights.query_log_archive
...
BASE = {"us": "https://metabase.prod-us.insights.dev", "eu": "https://metabase.prod-eu.insights.dev"}

def _ch_literal(value: str) -> str:
    # query_id derives from the caller-supplied client_query_id, so it can contain a single quote.
    # Escape for a Datastore single-quoted literal by doubling quotes. Treat this as required, not
...
def query_link(query_id, event_date, region="us", database_id=43):
    # database_id is a Datastore connection on the cluster (US 43). IDs can change when Metabase's
    # metadata is rebuilt: rediscover with `insightscli metabase:databases` and any Datastore id works,
    # since query_log_archive is a Distributed table. EU needs its own id.
    sql = (f"  SELECT lc_query__query FROM insights.query_log_archive\n"
...
for the executed Datastore SQL) if you want more detail in the linked view. Example output:

```text
https://metabase.prod-us.insights.dev/question#eyJkYXRhc2V0X3F1ZXJ5Ijp7InR5cGUiOiJuYXRpdmUiLCJ...
```
