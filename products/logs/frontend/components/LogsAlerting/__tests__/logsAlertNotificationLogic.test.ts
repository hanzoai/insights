...
jest.mock('lib/api', () => {
    ...
    // on mount, which resolves via the MSW floor. Only insightsFunctions is stubbed.
    const actual = jest.requireActual('lib/api')
    return {
        ...actual,
        __esModule: true,
        default: { ...actual.default, insightsFunctions: { list: jest.fn() } },
    }
})
...
describe('logsAlertNotificationLogic', () => {
    beforeEach(() => {
        initKeaTests()
        jest.clearAllMocks()
        ;(mockApi.insightsFunctions.list as jest.Mock).mockResolvedValue({ results: [] })
    })
    ...
    describe('loadExistingInsightsFunctions', () => {
        it('returns empty array when no alertId', async () => {
            const logic = logsAlertNotificationLogic({ alertId: undefined })
            logic.mount()

            await expectLogic(logic, () => {
                logic.actions.loadExistingInsightsFunctions()
            }).toFinishAllListeners()

            expect(logic.values.existingInsightsFunctions).toEqual([])
            expect(mockApi.insightsFunctions.list).not.toHaveBeenCalled()

            logic.unmount()
        })

        it('loads script functions filtered by alert id only (not by event)', async () => {
            ;(mockApi.insightsFunctions.list as jest.Mock).mockResolvedValue({
                results: [MOCK_FN_FUNCTION],
            })
            ...
            expect(mockApi.insightsFunctions.list).toHaveBeenCalledWith({
                types: ['internal_destination'],
                filter_groups: [buildLogsAlertFilterConfig('alert-1')],
                full: true,
            })
            // The filter must not include events — otherwise per-event InsightsFunctions
            ...
            expect(logic.values.existingInsightsFunctions).toEqual([MOCK_FN_FUNCTION])

            logic.unmount()
        })
    })

    describe('createPendingInsightsFunctions', () => {
        it('skips when no pending notifications', async () => {
            const logic = logsAlertNotificationLogic({ alertId: undefined })
            logic.mount()

            await expectLogic(logic, () => {
                logic.actions.createPendingInsightsFunctions('alert-1')
            }).toFinishAllListeners()
        ...
        it('sends one bundle-create call per pending notification (backend fans out per event)', async () => {
            mockCreate.mockResolvedValue({ insights_function_ids: ['hf-1', 'hf-2'] } as any)
            ...
            expect(toast.success).toHaveBeenCalledWith('2 notification destination(s) created.')
            expect(logic.values.pendingNotifications).toHaveLength(0)
        ...
        it('retains only the failed notifications so the user can retry them', async () => {
            mockCreate
                .mockResolvedValueOnce({ insights_function_ids: ['hf-ok'] } as any)
            ...
            expect(toast.error).toHaveBeenCalledWith(expect.stringContaining('1 notification(s) failed to create'))
            expect(logic.values.pendingNotifications).toHaveLength(1)
    ...
    describe('deleteExistingDestination', () => {
        it('sends the whole group of InsightsFunction ids in a single atomic delete call', async () => {
            ;(mockApi.insightsFunctions.list as jest.Mock).mockResolvedValue({
                results: [MOCK_FN_FUNCTION],
            })
            ...
            await expectLogic(logic).toFinishAllListeners()
            expect(logic.values.existingInsightsFunctions).toHaveLength(1)

            const group: LogsAlertDestinationGroup = {
                key: 'slack:C123',
                type: 'slack',
                label: 'Slack #alerts',
                insightsFunctions: [MOCK_FN_FUNCTION, { ...MOCK_FN_FUNCTION, id: 'hf-2' }],
                enabled: true,
            }
            ...
            expect(mockDelete).toHaveBeenCalledWith(expect.any(String), 'alert-1', {
                insights_function_ids: ['hf-1', 'hf-2'],
            })
            expect(toast.success).toHaveBeenCalledWith('Removed Slack #alerts')
            expect(mockApi.insightsFunctions.list).toHaveBeenCalledTimes(2)
        ...
        it('reloads from the server on delete failure so the list reflects actual state', async () => {
            ...
            const group: LogsAlertDestinationGroup = {
                ...
                type: 'slack',
                label: 'Slack #alerts',
                insightsFunctions: [MOCK_FN_FUNCTION],
                enabled: true,
            }
            ...
            expect(toast.error).toHaveBeenCalledWith(expect.stringContaining('Failed to remove Slack #alerts'))
