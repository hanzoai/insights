...
export function SlackNotConfiguredBanner({
    type = 'info',
    className,
}: Partial<Pick<BannerProps, 'type' | 'className'>>): JSX.Element {
            ...
            <div className="flex flex-col gap-2">
                ...
                {featureFlags[FEATURE_FLAGS.SLACK_APP_ASSISTANT] && (
                    <span className="text-sm text-secondary">
                        Adding Insights creates a public #insights-inbox channel in your Slack workspace, where Insights
                        posts what it finds.
                    </span>
                )}
            </div>
...
export function SlackChannelPicker({ onChange, value, integration, disabled }: SlackChannelPickerProps): JSX.Element {
    ...
    // Gates the empty-val recovery reload: InputSelect's setInputValue('') on blur and
    ...
    // InputSelect can't find an option matching the saved value's key and falls back to
                ...
                onInputChange={(val) => {
                    if (val) {
                        ...
                        if (SLACK_CHANNEL_ID_PATTERN.test(idCandidate)) {
                            ...
                            // InputSelect auto-fills the input with the selected option's key on
                            // focus (see InputSelect._onFocus). Don't treat that auto-fill as a
