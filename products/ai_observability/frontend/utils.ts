...
import {
    EVALUATION_NOT_SKIPPED_INSIGHTSQL,
    EVALUATION_PASSED_INSIGHTSQL,
...
/**
 * Epoch ms when the operation behind an event began. Insights AI SDKs capture an
...
export function normalizeEvaluationType(value: unknown): EvaluationType | undefined {
    if (value === 'llm_judge' || value === 'script' || value === 'sentiment') {
        return value
    }
    return undefined
}
...
export async function queryEvaluationRuns(params: {
    ...
    const query = insightsql`
        ...
        WHERE
            ...
            ${lookbackDays ? insightsql.raw(`AND timestamp >= now() - INTERVAL ${Math.floor(lookbackDays)} DAY`) : insightsql.raw('')}
...
export async function queryEvaluationRunsStats(params: {
    ...
    const query = insightsql`
        SELECT
            count() as total,
            countIf(properties.$ai_evaluation_result IS NOT NULL AND ${insightsql.raw(EVALUATION_NOT_SKIPPED_INSIGHTSQL)}) as applicable,
            countIf(${insightsql.raw(EVALUATION_PASSED_INSIGHTSQL)} AND ${insightsql.raw(EVALUATION_NOT_SKIPPED_INSIGHTSQL)}) as passed
        FROM events
        WHERE
            event = '$ai_evaluation'
            AND ${insightsql.raw(`properties.${propertyName}`)} = ${propertyValue}
