...
```bash
insightscli build            # auto-detects what changed and rebuilds
insightscli build:openapi    # or run this pipeline explicitly
```
...
For detailed guidance on serializer patterns, field typing, and annotations that improve OpenAPI generation, see the [`improving-drf-endpoints` skill](https://github.com/Insights/insights/blob/master/.agents/skills/improving-drf-endpoints/SKILL.md).
...
```bash
insightscli build            # auto-detects what changed and rebuilds
insightscli build:schema     # or run this pipeline explicitly
```
