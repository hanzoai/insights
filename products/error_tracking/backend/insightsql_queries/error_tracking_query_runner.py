...
from insights.schema import CachedErrorTrackingQueryResponse, ErrorTrackingQuery, ErrorTrackingQueryResponse

from insights.insightsql import ast
from insights.insightsql.constants import LimitContext
from insights.insightsql.context import InsightsQLContext
from insights.insightsql.database.schema.error_tracking_fingerprint_issue_state import PENDING_UPDATES_INSIGHTSQL_CONTEXT_KEY
...
class ErrorTrackingQueryRunner(AnalyticsQueryRunner[ErrorTrackingQueryResponse]):
    ...
    def _insightsql_context(self) -> InsightsQLContext:
        ctx = InsightsQLContext(team_id=self.team.pk, team=self.team, user=self.user, enable_select_queries=True)
        raw = (self.query.pendingFingerprintIssueStateUpdates or [])[: self.MAX_PENDING_FINGERPRINT_ISSUE_STATE_UPDATES]
        if raw:
            ctx.data_to_ingest[PENDING_UPDATES_INSIGHTSQL_CONTEXT_KEY] = [row.model_dump(mode="json") for row in raw]
        return ctx

    def _calculate(self):
        with self.timings.measure("error_tracking_query_insightsql_execute"):
            query_result = self.paginator.execute_insightsql_query(
                ...
                filters=self._builder.insightsql_filters(),
                user=self.user,
                context=self._insightsql_context(),
            )
    ...
    def _attach_events(self, columns: list[str], results: list) -> tuple[list[str], list]:
        ...
        if uuids:
            with self.timings.measure("error_tracking_query_event_fetch"):
                event_result = execute_insightsql_query(
                    query=parse_select(
                        # The explicit LIMIT matters: without one, execute_insightsql_query applies
