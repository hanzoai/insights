"""Resolves a user's Insights Desktop plan via the Insights API seats endpoint.

Calls ``GET /api/seats/me/?product_key=insights_code`` on the Insights API,
...
class PlanResolver:
    ...
    async def _fetch_plan(self, auth_header: str) -> tuple[str | None, str | None, BillingPeriod | None, bool]:
        """Call the Insights API seats endpoint to get the user's plan.
