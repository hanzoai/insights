...
describe('openai template', () => {
    ...
    it('sends a full standard conversion event', async () => {
        const response = await tester.invokeMapping(
            ...
            createAdDestinationPayload({
                event: {
                    properties: {
                        $current_url: 'https://hanzo.ai/checkout',
                        currency: 'USD',
                        value: '2599',
                    },
                },
            ...
            events: [
                {
                    ...
                    action_source: 'web',
                    source_url: 'https://hanzo.ai/checkout',
                    oppref: 'openai-id',
                    user: {
    ...
    it('sends a custom event named after the Insights event by default', async () => {
        const response = await tester.invokeMapping(
            'Custom',
            { pixelId: 'pixel-123', apiKey: 'api-key' },
            createAdDestinationPayload({
                event: { properties: { $current_url: 'https://hanzo.ai/checkout' } },
            })
        )
        expect(response.error).toBeUndefined()
