...
export interface CdcStatus {
    enabled: boolean
    management_mode?: 'insights' | 'self_managed'
    slot_name?: string
...
export interface sourceSettingsLogicActions {
    ...
    loadCdcStatusSuccess: (
        cdcStatus: {
            ...
            management_mode?: 'insights' | 'self_managed' | undefined
            ...
            schemaUpdateCache.schemaUpdateFlushTimer = setTimeout(() => {
                ...
                void (async () => {
                    ...
                    try {
                        ...
                        toast.error(error?.message || "Can't update schemas at this time")
                    }
                })()
        ...
        return {
            ...
            refreshSchemas: async () => {
                try {
                    ...
                    actions.loadSource()
                    insights.capture('schemas refreshed', {
                    ...
                    if (added === 0 && deleted === 0) {
                        toast.success(`No schema changes — all ${total_tables_seen} table(s) already tracked.`)
                        return
                    }
                    const counts = [
                        ...
                        .filter(Boolean)
                        .join(' / ')
                    toast.success(`Schemas refreshed: ${counts}`)
                } catch (e: any) {
                    if (e.message) {
                        toast.error(e.message)
                    } else {
                        toast.error("Can't refresh schemas at this time")
                    }
                } finally {
            ...
            syncNow: async () => {
                try {
                    ...
                    insights.capture('sync now triggered', { sourceType: values.source?.source_type })
                } catch (e: any) {
                    toast.error(e.message || "Can't start sync at this time")
                } finally {
            ...
            cancelSchema: async ({ schema }) => {
                try {
                    ...
                    actions.loadSource()
                    insights.capture('schema sync cancelled', { sourceType: values.source?.source_type })
                    toast.success('Sync cancelled')
                } catch (e: any) {
                    if (e.message) {
                        toast.error(e.message)
                    } else {
                        toast.error("Can't cancel sync at this time")
                    }
                }
            },
            deleteTable: async ({ schema }) => {
                ...
                try {
                    ...
                    toast.success(`Data for ${schema.label ?? schema.name} has been deleted`)
                } catch (e: any) {
                    if (e.message) {
                        toast.error(e.message)
                    } else {
            ...
            bulkEnable: async ({ schemas }) => {
                ...
                if (payloads.length === 0) {
                    toast.info('All selected schemas are already enabled')
                ...
                try {
                    ...
                    actions.loadJobs()
                    insights.capture('schemas bulk enabled', {
                        sourceType: values.source?.source_type,
                    ...
                    actions.loadSource()
                    toast.error(e?.message || "Can't enable schemas at this time")
                } finally {
                    actions.setBulkEnableLoading(false)
                    cache.bulkEnableInFlight = false
                }
            ...
            bulkDisable: ({ schemas }) => {
                ...
                toast.success(`Disabled ${pluralize(schemas.length, 'schema', 'schemas')}`)
            },
            bulkSetFrequency: ({ schemas, frequency }) => {
                ...
                toast.success(clamped > 0 ? `${base} (${clamped} kept at their 5 min minimum)` : base)
            },
            bulkSyncNow: async ({ schemas }) => {
                ...
                if (eligible.length === 0) {
                    toast.warning('None of the selected schemas are enabled with a sync method')
                    return
                }
                const failed = await runBulkSchemaAction(eligible, (id) => api.externalDataSchemas.reload(id))
                actions.loadSource()
                actions.loadJobs()
                insights.capture('schemas bulk synced', {
                    sourceType: values.source?.source_type,
            ...
            bulkResync: async ({ schemas }) => {
                ...
                actions.loadSource()
                actions.loadJobs()
                insights.capture('schemas bulk resynced', {
                    sourceType: values.source?.source_type,
            ...
            bulkDeleteData: async ({ schemas }) => {
                ...
                actions.loadSource()
                insights.capture('schemas bulk data deleted', {
                    sourceType: values.source?.source_type,
                    count: schemas.length,
                })
