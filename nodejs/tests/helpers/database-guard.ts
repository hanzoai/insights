...
// test databases (test_insights, insights_test) pass while names that merely
// embed the substring (latest, insights_latest) are still refused.
const TEST_DATABASE_NAME_PATTERN = /(^|_)test(_|$)/i

/**
 * Throws unless `databaseName` looks like a test database — i.e. it contains
 * "test" as an underscore- or boundary-delimited token (test_insights,
 * insights_test). Names that merely embed the substring (latest, mytest) are
 * refused.
 *
 ...
 * DATABASE_URL pointing at the dev `insights` database leaking in from a shell
...
export function assertTestDatabaseName(databaseName: string, source: string): void {
    ...
    throw new Error(
        `🛑 Refusing to run a destructive test helper against database "${databaseName}" (${source}).\n\n` +
            ...
            `data from it.\n\n` +
            `The nodejs tests expect dedicated test databases (test_insights, test_persons,\n` +
            `test_behavioral_cohorts on Postgres; insights_test on Datastore). This error\n` +
            `usually means DATABASE_URL, PERSONS_DATABASE_URL, DATASTORE_DATABASE or similar\n` +
            `leaked in from your shell or IDE. Unset them, or create the test databases with\n` +
            `\`pnpm --filter=@hanzo/nodejs setup:test\`.`
    )
}
