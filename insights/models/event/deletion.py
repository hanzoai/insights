...
from insights.models.event.sql import DISTRIBUTED_EVENTS_JSON_TABLE, EVENTS_DATA_TABLE, EVENTS_JSON_DATA_TABLE
...
def _table_exists(client: Client, table: str) -> bool:
    [[count]] = client.execute(
        ...
        {"database": settings.DATASTORE_DATABASE, "name": table},
    )
    return bool(count)
...
def cluster_has_events_json_table(cluster: DatastoreCluster) -> bool:
...
def events_data_tables(cluster: DatastoreCluster) -> list[str]:
...
def events_data_tables_via_sync_execute() -> list[str]:
    """Like ``events_data_tables``, for callers that talk to Datastore through ``sync_execute``."""
    tables = [EVENTS_DATA_TABLE()]
