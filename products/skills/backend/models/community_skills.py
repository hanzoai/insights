...
class CommunitySkill(UUIDModel):
    ...
    The source of truth for skill content is the Insights/community-skills GitHub repo;
...
class CommunitySkillVote(UUIDModel):
    ...
    # db_constraint=False: insights_user is a hot table — a real FK constraint takes a SHARE ROW
    # EXCLUSIVE lock on it at create time. Cascade is still enforced by the ORM collector.
    user = models.ForeignKey("insights.User", on_delete=models.CASCADE, db_constraint=False)
    created_at = models.DateTimeField(default=timezone.now)
