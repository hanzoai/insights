...
class TestSignalTaskFollowupMessage(SimpleTestCase):
    ...
    def test_capability_gates_versioned_signal_and_preserves_sender_fields(
        ...
        with (
            patch("products.tasks.backend.feature_flags.hanzo_insights.feature_enabled") as feature_enabled,
...
class TestExecuteTaskProcessingWorkflow(TestCase):
    ...
    def test_marks_run_failed_when_temporal_start_fails(self, executor: str) -> None:
        ...
        with (
            ...
            patch("products.tasks.backend.temporal.client.hanzo_insights.feature_enabled", return_value=False),
    ...
    def test_captures_run_feature_flags_before_starting_workflow(self, executor: str) -> None:
        ...
        with (
            patch(connect_target, connect_mock),
            patch("products.tasks.backend.temporal.client.hanzo_insights.feature_enabled", return_value=True) as flag,
        ):
            self._execute_workflow(executor, run, self.user.id)
        ...
        # Patching the shared hanzo_insights module attribute covers both evaluation
    ...
    def test_captures_sandbox_event_ingest_flag_before_resuming_workflow(self) -> None:
        ...
        with (
            patch("products.tasks.backend.temporal.client.sync_connect", return_value=client),
            patch("products.tasks.backend.temporal.client.hanzo_insights.feature_enabled", return_value=True),
        ):
    ...
    def test_captures_sandbox_event_ingest_flag_without_clobbering_concurrent_state(self, executor: str) -> None:
        ...
        with (
            patch(connect_target, connect_mock),
            patch(
                "products.tasks.backend.temporal.client.hanzo_insights.feature_enabled",
...
class TestRedispatchOrphanedTaskRun(TestCase):
    ...
    def _orphaned_loop_run(self, storage_path: str | None = None) -> TaskRun:
        ...
        run = self._orphaned_run(pending_dispatch={"create_pr": False, "insights_mcp_scopes": "read_only"})
        run.state = {
            **run.state,
            "skill_bundle_seeds": [
                {
    ...
    @patch("insights.storage.object_storage.copy", side_effect=RuntimeError("s3 down"))
    ...
    def test_recovers_orphaned_run_with_persisted_dispatch_params(self) -> None:
        run = self._orphaned_run(pending_dispatch={"create_pr": False, "insights_mcp_scopes": "full"})
        start_workflow = AsyncMock()
    ...
    def test_falls_back_to_run_source_scopes_when_dispatch_params_absent(
        ...
        self.assertEqual(outcome, "recovered")
        _, workflow_input = start_workflow.call_args.args
        self.assertEqual(workflow_input.insights_mcp_scopes, expected_scopes)
    ...
    def test_falls_back_to_scout_scopes_for_signals_scout_run(self) -> None:
        ...
        self.assertEqual(outcome, "recovered")
        _, workflow_input = start_workflow.call_args.args
        self.assertEqual(workflow_input.insights_mcp_scopes, "signals_scout_reports")
