...
from insights.security.llm_prompt_sanitization import GENERIC_VALUE_MAX_LEN, sanitize_user_text
...
# windows can pin a Datastore node on a high-volume team, and this surface isn't interactive.
...
MARKETING_CONTEXT_PROMPT = dedent("""
    ...
    - Marketing analytics 'Reported conversions' = numbers PULLED from connected ad platforms via their APIs (Google Ads metrics_conversions, Meta CAPI, etc.). Insights does not compute these.
    - Marketing analytics 'Conversion goals' = Insights events/actions configured by the team to be matched with UTM-tagged sessions for attribution.
...
class MarketingListDataSourcesTool(MaxTool):
    ...
    description: str = dedent("""
        ...
        This is the platform → DW side only. For Insights-events-side health (UTM matching), call `marketing_diagnose_setup`.
    """).strip()
    context_prompt_template: str = MARKETING_CONTEXT_PROMPT
...
def _format_utm_mapping_suggestions_for_llm(response) -> str:
    ...
    if response.current_mappings:
        ...
        lines.append("## Already-active mappings (canonical aliases hardcoded by Insights)")
