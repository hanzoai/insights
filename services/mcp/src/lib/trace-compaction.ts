 ...
 * Insights UI still have the complete, untruncated trace. It walks the result
 ...
 * trace in Insights for the full data.
 */
...
function truncateString(value: string, budget: number): string {
    ...
    return `${value.slice(0, limit)}… [truncated ${dropped} of ${value.length} chars — open the trace in Insights for the full value]`
}
 ...
 * may not be fully tree-reconstructable — hence the pointer back to Insights.
 */
export function compactTrace(trace: unknown, budget: number = MAX_TRACE_CHARS): unknown {
    ...
    if (omitted > 0) {
        assignKey(base, '_truncated', {
            ...
            note: 'Open the trace in Insights for the complete, untruncated data, or narrow the query to the events you need.',
        })
    }
    return base
}
