...
from insights.api.documentation import (
...
from insights.models.event.event import Selector
...
class ActionReferenceSerializer(serializers.Serializer):
    type = serializers.CharField(help_text="Resource type: insight, experiment, cohort, or insights_function")
...
def find_action_references(action_id: int, team: Team) -> list[dict[str, Any]]:
    ...
    # nosemgrep: python.django.security.audit.query-set-extra.avoid-query-set-extra (parameterized via params)
    insights_functions = (
        InsightsFunction.objects.filter(team_id=team.pk, deleted=False)
        .select_related("created_by")
        .extra(
    ...
    for hf in insights_functions[:remaining]:
        refs.append(
            {
...
def count_action_references_bulk(action_ids: list[int], team: Team) -> dict[int, int]:
    ...
    hf_table = InsightsFunction._meta.db_table
