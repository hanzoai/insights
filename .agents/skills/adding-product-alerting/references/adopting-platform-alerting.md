...
- Does the product need InsightsFunction delivery, email, or both?
...
- Keep event IDs and properties stable because InsightsFunctions filter and render from them.
...
- Validate, build, create, and delete destination InsightsFunctions through `products.alerts.backend.facade.api`.
...
For InsightsFunction destinations:
...
Producer acknowledgement does not confirm downstream InsightsFunction execution or final destination delivery. Logs is the reference for reevaluating on the next cadence after an internal event is not acknowledged.
...
Use `AlertWizard` when the product creates InsightsFunction-backed alerts from trigger and destination templates.

- Configure a unique `logicKey`.
- Supply supported sub-template IDs, triggers, and destinations.
- Add product-specific configuration outside the shared wizard when it is not a reusable InsightsFunction input.
- Keep the traditional InsightsFunction editor as the advanced fallback when the product already supports it.
