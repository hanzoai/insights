...
from insights.egress.github.limiter import remember_observed_core_limit
...
def _build_app_jwt() -> str:
    ...
    signs with (see ``insights.models.github_integration_base``).
    """
...
# Insights App's flow: exchange the post-install OAuth `code` for the *user's* own access token, then
...
def exchange_oauth_code_for_user_token(code: str) -> str | None:
    ...
    # (identical to how the shared Insights App exchanges its code) rather than the REST egress transport.
...
class StamphogGitHubClient:
    ...
    def add_pr_reaction(self, repo: str, number: int, content: str = "eyes") -> int | None:
        ...
        "review in flight" signal ``get_pr_reactions``/``STAMPFN_TRUSTED_REACTOR_BOTS`` reads off
    ...
    def list_own_active_approvals(self, repo: str, number: int) -> list[dict]:
        ...
        doing nothing — so an unconfigured ``STAMPFN_GITHUB_APP_SLUG`` yields an empty list, never a guess.
