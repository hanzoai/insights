"""Verify property_to_expr emits SQL that uses Datastore skip indexes.
...
from insights.insightsql.observability import InsightsQLTypeObservability
...
def _find_all_skip_indexes(plan_json: Any) -> set[str]:
    """Recursively walk a Datastore EXPLAIN PLAN JSON, return Skip-type index names."""
    out: set[str] = set()
...
def _extract_persons_where_optimization_subquery(sql: str) -> str | None:
    ...
    The InsightsQL ``persons`` lazy table wraps reads in an argMax + ``IN (SELECT where_optimization.id ... WHERE <predicate>)`` dedup pattern. Datastore precomputes that IN-subquery as a separate ``CreatingSets`` pipeline whose index usage doesn't surface in the outer ``EXPLAIN PLAN`` — so to check predicate-level skip indexes we EXPLAIN the subquery body directly.
...
def _run_explain_and_get_skip_indexes(query: str, values: dict[str, Any]) -> set[str]:
    ...
    if inner is not None:
        query = inner
    # Apply the runtime Datastore settings (``transform_null_in`` etc.) — skip index selection diverges from real queries without them.
    settings = {
...
class _PropertySkipIndexTestBase(DatastoreTestMixin, APIBaseTest):
    ...
    def _seed(self) -> None:
        """Seed rows so Datastore doesn't compile the read to NullSource — EXPLAIN PLAN won't show index info if the planner already concluded the scan reads nothing."""
        if self.SCOPE == "event":
    ...
    def _filter_to_sql(
        ...
        query, _ = prepare_and_print_ast(select_query, context, "datastore")
    ...
    def _assert_indexes(
        ...
        *,
        ...
        # Execute so ``@snapshot_datastore_queries`` captures this SELECT (EXPLAIN below doesn't match the SELECT/WITH-prefix filter). Result discarded.
        ...
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA and self.SCOPE != "person":
        ...
        if not expected_used_set.issubset(used):
            ...
            raise AssertionError(
                f"Expected skip indexes {sorted(missing)} to be used, but Datastore picked {sorted(used)}.\n"
                f"Filter: {property_filter}\nSQL: {query}"
            )
        if expected_not_used_set & used:
            unwanted = expected_not_used_set & used
            raise AssertionError(
                f"Did not expect skip indexes {sorted(unwanted)} to be used, but Datastore picked them.\n"
                f"Filter: {property_filter}\nSQL: {query}"
            )
...
class TestEventPropertySkipIndexes(_PropertySkipIndexTestBase):
    ...
    @parameterized.expand(
        [
            ...
            # is_set / is_not_set (``col IS NOT NULL`` / ``col IS NULL``) — whether minmax is considered is Datastore-version-dependent (CH 26.3 yes, 25.12 no), so we don't assert it.
        ]
    )
    ...
    # Mat columns are ``Nullable(String)`` / ``String``; the rewrite doesn't coerce — Datastore decides whether to accept the comparison.
    ...
    @parameterized.expand(
        [
            # Non-string Python constants — printer emits the constant raw (``less(col, 5)`` or ``less(col, toDateTime64('2024-01-15 ...'))``) and Datastore refuses ``String < UInt8 / Float64 / DateTime64`` at execution. (Same behavior as the existing equality / IN rewrites — none of them gate on constant type. For numeric/datetime compare, declare ``property_type`` on the PropertyDefinition; see ``test_mat_col_lt_typed_*``.)
            ("int", 5),
            ("float", 5.5),
    ...
    def test_mat_col_nullable_minmax_lt_non_string_constant_against_string_column_errors(
        ...
        with self.assertRaises(Exception) as ctx:
            sync_execute(query, values)
        # Datastore: ``No supertype for types String, UInt8`` or ``No operation less between String and DateTime64``.
        message = str(ctx.exception).lower()
        assert "supertype" in message or "no operation" in message, (
            f"Expected a type-mismatch error from Datastore, got: {ctx.exception}"
        )
    ...
    @parameterized.expand(
        [
            ...
            # Range rewrite emits ``less(col, '5') AND notEquals(col, '') AND notEquals(col, 'null')``; Datastore considers each AND-ed clause against every applicable index — minmax fires on ``less``, bloom filter fires on the sentinel ``notEquals`` clauses (granules with no ``''`` / ``'null'`` rows trivially satisfy them).
            ("lt", PropertyOperator.LT, "5", True),
        ]
    )
    ...
    def test_observability_records_property_usage_and_range_rewrite(self) -> None:
        ...
        stats = InsightsQLTypeObservability(dialect="datastore", source="unknown")
        with patch("insights.insightsql.printer.utils.create_insightsql_type_observability", return_value=stats):
...
class TestPersonOnEventsPropertySkipIndexes(_PropertySkipIndexTestBase):
    ...
    @parameterized.expand(
        [
            ...
            ("in_multi", PropertyOperator.IN_, ["2", "5"], True),
            ("icontains_with_isnotnull", PropertyOperator.ICONTAINS, "5", True),
            # is_set omitted — minmax usage for ``col IS NOT NULL`` is Datastore-version-dependent (CH 26.3 yes, 25.12 no).
        ]
    )
    def test_mat_col_nullable_minmax(
        self, _name: str, operator: PropertyOperator, value: Any, should_use: bool
