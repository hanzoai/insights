...
from insights.constants import TREND_FILTER_TYPE_ACTIONS, TREND_FILTER_TYPE_EVENTS, PropertyOperatorType
...
class TestProperty(BaseTest):
    ...
    def _property_to_expr(
        self,
        property: Union[PropertyGroup, Property, InsightsQLPropertyFilter, dict, list],
    ...
    def test_property_to_expr_insightsql(self):
        self.assertEqual(self._property_to_expr({"type": "insightsql", "key": "1"}), ast.Constant(value=1))
        self.assertEqual(
            self._property_to_expr(Property(type="insightsql", key="1")),
            ast.Constant(value=1),
        )
        self.assertEqual(
            self._property_to_expr(InsightsQLPropertyFilter(type="insightsql", key="1")),
            ast.Constant(value=1),
        )
    ...
    def test_person_distinct_id_property_resolves_under_all_poe_modes(self, _name, mode):
        ...
        context = InsightsQLContext(
            ...
            modifiers=create_default_modifiers_for_team(self.team, InsightsQLQueryModifiers(personsOnEventsMode=mode)),
        )
        sql, _ = prepare_and_print_ast(query_ast, context=context, dialect="datastore")
    ...
    def test_property_to_expr_group_key_numeric_value(self):
        ...
        # would crash Datastore with NO_COMMON_TYPE, so it is coerced to a string.
        self.assertEqual(
            self._property_to_expr(
    ...
    def test_person_metadata_fields_match_taxonomy(self):
        ...
        from insights.insightsql.property import PERSON_METADATA_FIELDS
        ...
        self.assertEqual(
            ...
            taxonomy_keys,
            "PERSON_METADATA_FIELDS in insights/insightsql/property.py must match the "
            "person_metadata group in core-filter-definitions-by-group.json. "
    ...
    def test_person_metadata_fields_match_rust(self):
        ...
        self.assertEqual(
            PERSON_METADATA_FIELDS,
            rust_fields,
            "PERSON_METADATA_FIELDS in insights/insightsql/property.py must match the Rust slice in "
    ...
    def test_property_to_expr_invalid_regex_raises_query_error(self, _name: str, bad_regex: str):
        ...
        # whole query in Datastore with CANNOT_COMPILE_REGEXP.
    ...
    def test_property_to_expr_semver_edge_cases(self):
        ...
        # reach sortableSemver and trigger Datastore's array NULL ordering — the
    ...
    def test_flag_dependency_combined_with_person_property(self):
        ...
        result = self._property_to_expr(
            PropertyGroup(
                ...
                values=[
                    Property(**flag_prop),
                    Property(type="person", key="email", value="script@hanzo.ai", operator="exact"),
                ],
            ),
            scope="person",
        )
...
class TestPropertyIsSetIsNotSetWithData(APIBaseTest):
    ...
    def test_is_set_operator(self, _name: str, is_materialized: bool):
        ...
        result = execute_insightsql_query(team=self.team, query=query_ast)
...
class TestPropertyDateOperatorsWithData(APIBaseTest):
    ...
    Datastore SQL is syntactically valid but rejected at runtime. In particular,
