...
payload = insights.get_feature_flag_payload(flag_key, distinct_id, send_feature_flag_events=True)
if not payload:
...
client = OpenAI(api_key=os.environ["OPENAI_API_KEY"], insights_client=insights)
...
const result = await insights.getFeatureFlagResult(flagKey, distinctId)
...
// 3. Call the LLM — the wrapper auto-emits $ai_generation
const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY!, insights })
...
export function PromptExperimentAgentPromptSnippet({ flagKey }: PromptSnippetProps): JSX.Element {
    return (
        <CodeSnippet language={Language.Text} wrap>
            {`Wire up a Insights prompt experiment in this project.
...
3. Find where the project makes LLM calls (route handler, service module, agent loop,
   ...
   a) Evaluate the feature flag "${flagKey}" for the current user, and make sure the
      ...
      insights.get_feature_flag_payload(..., send_feature_flag_events=True) in Python or
      insights.getFeatureFlagResult(...) in Node — without this event the experiment
   ...
   c) Use Insights's prompt management API to fetch the prompt by name + version, then
      compile it. Pass the compiled prompt as the system message.
   d) Wrap the LLM client with Insights's AI tracing wrapper (e.g. @hanzo/ai for
      Node, insights.ai for Python) so $ai_generation events are auto-emitted with
      cost, latency, and tokens.
   e) Pass insights_distinct_id / insightsDistinctId and attach event properties:
...
4. Do NOT also call insights.capture('$ai_generation', ...) — the wrapper already emits
...
Reference docs
- AI observability overview:     https://hanzo.ai/docs/ai-observability
- Prompt management:             https://hanzo.ai/docs/prompt-management
- Feature flag payloads:         https://hanzo.ai/docs/feature-flags/payloads
- Experiments:                   https://hanzo.ai/docs/experiments
- Insights AI wrappers (OpenAI, Anthropic, Gemini): https://hanzo.ai/docs/ai-engineering
`}
        </CodeSnippet>
    )
}
