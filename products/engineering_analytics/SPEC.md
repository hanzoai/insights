...
The product surfaces PR + CI data through **named, typed endpoints** that run curated InsightsQL privately over the warehouse (`github_*` tables), Logs, and Traces. Two first-class surfaces consume the same endpoints: the in-app UI and **MCP tools**. Nothing is registered as a global InsightsQL view, so the product stays isolated and off the per-query catalog hot path; core imports only the viewset, exactly like `visual_review`.
...
The goal is Signals for Insights Desktop (README → "The goal"): Signal detection is defined once in `logic/` over the read layer, shared by the surfaces and the Signal emitter. Emission rides the curated builders; it does not wait on lifecycle events.
...
graph TB
    ...
    subgraph "Curated read layer (domain rules defined ONCE)"
        Sys["curated query builders: build_query()<br/>embedded as subqueries, run via execute_insightsql_query<br/>NOT registered as global views"]
    end
...
- **Never register anything in `Database.create_for`.** Run the builders privately via `execute_insightsql_query`; a global view puts the product on every team's per-query hot path.
- **One endpoint set for every consumer.** A capability is a named typed endpoint returning `facade/contracts.py` types; the UI and MCP tools consume that same endpoint (no client-side InsightsQL, no UI-only read paths), and its `mcp/tools.yaml` entry is set in the same PR.
...
- Cost is defined once, in `logic/cost.py`, rendered to InsightsQL; a Datastore-backed parity test asserts the view equals the Python model. The endpoint cost queries read the same rendered SELECT (via `_curated.job_cost_source()`), so there is no second cost path to drift.
...
- **Warehouse columns are strings + Nullable JSON; the builders parse and `ifNull`-guard.** Timestamps parse via `parseDateTimeBestEffort`; Nullable columns unwrap before any array function (Datastore rejects an Array inside a Nullable). `source_schema.py` mirrors the real landed types so seed and tests exercise the real path; violating this 500'd every endpoint on real data while idealized fixtures stayed green.
- **The warehouse views are managed data, not code registration.** "No global InsightsQL views" locks out `Database.create_for` (core importing the product, every team's per-query hot path). A per-team `DataWarehouseSavedQuery`, synced only for qualifying teams, reopens nothing: it exists so cost and CI history are queryable by insights, subscriptions, and `execute-sql`.
...
- **InsightsQL only for analytics data.** No raw Datastore.
- **No product Postgres DB.** Analytics data lives in the warehouse / Datastore; any product-config model goes on the main DB as a team-scoped model (`TeamScopedRootMixin`), never a separate DB.
...
Lifecycle data the snapshots can't hold and `github_issue_events` doesn't carry (reviews/approvals, deploys, DORA) needs immutable timestamped events (GitHub webhooks → Insights events, PR as group type). That is the only thing the deferred events destination is for. See README → "The data boundary".
