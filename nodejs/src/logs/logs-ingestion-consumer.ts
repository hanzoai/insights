...
export interface LogsIngestionConsumerDeps {
    ...
    /** When set, enabled teams may run head sampling before Datastore Kafka produce. */
    ...
    /** When set, enabled teams run script log transformations after the built-in processing. */
...
/**
 * Wraps a script log transform (mutates the surviving records in place, may drop) as the last pipeline
...
export class LogsIngestionConsumer {
    ...
    /**
     * Builds the script log transformation hook for a message, or undefined when the team
    ...
    > {
        ...
        // limits first, script transformations last. An empty list (with no metric visitor) leaves the
