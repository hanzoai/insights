...
// `Metric` is intentionally NOT re-exported here: it pulls `@hanzo/quill-charts` (d3), and this
// barrel is inlined into the always-eager `@hanzo/quill` bundle. Import it from its own entry —
// `@hanzo/quill-components/metric` — so charts only loads where a metric tile is actually used.
