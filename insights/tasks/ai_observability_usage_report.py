...
from insights.utils import get_instance_region, get_previous_day
...
# Datastore query settings for AI observability queries
...
CELERY_TASK_ID = "insights.tasks.llm_analytics_usage_report.send_llm_analytics_usage_reports"
...
@shared_task(
    name="insights.tasks.llm_analytics_usage_report.capture_llm_analytics_report",
    max_retries=3,
    bind=True,
    **AI_OBSERVABILITY_USAGE_REPORT_TASK_KWARGS,
)
def capture_ai_observability_report(
    ...
    *,
    ...
    try:
        ...
        log(
            "[AIO Usage Error] AI observability usage report sent to Insights for organization failed",
            organization_id=organization_id,
            error=str(err),
            event_source="ai_observability_usage_report",
            exc_info=True,
...
@shared_task(
    name="insights.tasks.llm_analytics_usage_report.send_llm_analytics_usage_reports",
    max_retries=3,
    bind=True,
    **AI_OBSERVABILITY_USAGE_REPORT_TASK_KWARGS,
)
def send_ai_observability_usage_reports(
    ...
    if are_usage_reports_disabled:
        hanzo_insights.capture_exception(Exception(f"AI observability usage reports are disabled for {at}"))
        return
