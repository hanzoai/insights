...
use personinsights_coordination::types::HandoffPhase;
...
const ETCD_PREFIX: &str = "/personinsights-test-harness/";
...
pub struct StackConfig {
    ...
    /// Also spawn the personinsights-identity service, pointed at the traffic
...
/// A locally-spawned personinsights stack: replica, writer, N leaders, and M
...
pub struct Stack {
    ...
    /// Set when the stack spawned a personinsights-identity service.
...
impl Stack {
    pub async fn up(config: StackConfig) -> Result<Self> {
        ...
        let topic = format!("personinsights_test_harness_{run_id}");
        ...
        if config.spawn_identity {
            binaries.push("personinsights-identity");
        }
        for bin in &binaries {
            ...
            if !path.exists() {
                bail!(
                    "{} not found — build the stack first:\n  cargo build -p personinsights-replica \
                     -p personinsights-router -p personinsights-leader -p personinsights-writer -p personinsights-identity",
                    path.display()
                );
            }
        }
        ...
        infra.push(ServiceProcess::spawn(
            "replica",
            &config.bin_dir.join("personinsights-replica"),
            &[
        ...
        infra.push(ServiceProcess::spawn(
            "writer",
            &config.bin_dir.join("personinsights-writer"),
            &[
                ...
                (
                    ...
                    "personinsights-test-harness-writer".to_string(),
                ),
        ...
        for i in 0..config.routers {
            ...
            let proc = ServiceProcess::spawn(
                &name,
                &config.bin_dir.join("personinsights-router"),
                &[
        ...
        let identity_url = if config.spawn_identity {
            infra.push(ServiceProcess::spawn(
                "identity",
                &config.bin_dir.join("personinsights-identity"),
                &[
    ...
    pub fn spawn_leader(&mut self) -> Result<String> {
        ...
        let pod_name = format!("personinsights-leader-{index}");
        ...
        let proc = ServiceProcess::spawn(
            &format!("leader-{index}"),
            &self.config.bin_dir.join("personinsights-leader"),
            &[
