...
TEST_BUCKET = "test_storage_bucket-insights.products.engineering_analytics.ci_views"
GITHUB_SOURCE_PREFIX = "myprefix"

_BASE = "2026-01-01 10:00:00"
_LATER = "2026-01-01 10:10:00"
_REPO = "Insights/insights"
...
def _run_row(
    ...
    return {
        ...
        "pull_requests": json.dumps(
            [pr_association_entry(number, base_repo="Mu-L/insights-1") for number in fork_pr_numbers]
...
class TestCIJobHistoryView(DatastoreTestMixin, BaseTest):
    ...
    def test_exposes_fields_and_derives_commit_attribution(self) -> None:
        ...
        assert (value("job-a", "repo_owner"), value("job-a", "repo_name")) == ("Insights", "insights")
        ...
        # here). Datastore fills the unmatched run side with type defaults, so attribution is empty
...
class TestCIFailuresView(DatastoreTestMixin, BaseTest):
    ...
    def _log(self, body: str, attributes: dict[str, str], minute: int = 0) -> dict[str, Any]:
        ...
        # `attributes` map (see logs34 mapApply), so a InsightsQL `attributes['run_id']` reads "run_id__str".
        return {
    ...
    def test_fingerprints_pytest_failures_and_skips_other_lines(self) -> None:
        ...
        self._insert_logs(
            [
                ...
                self._log(
                    "FAILED insights/api/test_foo.py::TestFoo::test_bar - AssertionError: got 42 at 0xdeadbeefcafe1234",
                    attrs,
                    minute=0,
                ),
                # A failure with no " - detail" — its signature is empty, so error_signature is NULL.
                self._log("FAILED insights/api/test_baz.py::test_qux", attrs, minute=1),
                # Not a pytest FAILED line — must be filtered out by the WHERE clause.
                self._log("PASSED insights/api/test_ok.py::test_ok", attrs, minute=2),
        ...
        assert set(by_test) == {
            "insights/api/test_foo.py::TestFoo::test_bar",
            "insights/api/test_baz.py::test_qux",
        ...
        detailed = by_test["insights/api/test_foo.py::TestFoo::test_bar"]
