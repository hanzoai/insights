...
from insights.tasks.ai_observability_usage_report import (
...
class TestAIObservabilityUsageReport(APIBaseTest, DatastoreTestMixin, DatastoreDestroyTablesMixin):
    ...
    def test_get_all_ai_metrics(self) -> None:
        ...
        self._create_ai_events(
            ...
            "$ai_evaluation",
            3,
            properties={"$ai_evaluation_key_type": "insights", "$ai_evaluation_runtime": "llm_judge"},
        )
        self._create_ai_events(
            self.team, distinct_id, "$ai_evaluation", 2, properties={"$ai_evaluation_runtime": "script"}
        )
        self._create_ai_events(
    ...
    def test_get_all_ai_dimension_breakdowns(self) -> None:
        ...
        assert breakdowns.library_breakdown.get("insights-python") == 13  # 10 + 3
    ...
    def test_full_ai_observability_report(self) -> None:
        ...
        self._create_ai_events(
            self.team, distinct_id_1, "$ai_evaluation", 1, properties={"$ai_evaluation_runtime": "script"}
        )
        self._create_ai_events(
        ...
        assert org_1_report["library_breakdown"] == {"insights-python": 3}
    ...
    @patch("insights.tasks.ai_observability_usage_report.capture_ai_observability_report")
    @patch("insights.tasks.ai_observability_usage_report.get_ph_client")
    ...
    def test_dimension_breakdown_aggregation_across_teams(self) -> None:
        ...
        assert org_report["library_breakdown"]["insights-python"] == 18  # 10 + 5 + 3
    ...
    def test_dimension_breakdowns_filter_empty_values(self) -> None:
        ...
        assert breakdowns.library_breakdown.get("insights-python") == 7  # 5 + 2
    ...
    @patch("insights.tasks.ai_observability_usage_report.capture_exception")
    @patch("insights.tasks.ai_observability_usage_report.get_llm_prompt_fetched_counts", side_effect=Exception("boom"))
    ...
    @patch("insights.tasks.ai_observability_usage_report.capture_ai_observability_report")
    @patch("insights.tasks.ai_observability_usage_report._get_all_ai_observability_reports")
...
class TestAIObservabilityUsageReportTaskWiring(SimpleTestCase):
    ...
    @patch("insights.tasks.ai_observability_usage_report.logger")
    @patch("insights.tasks.ai_observability_usage_report.get_teams_with_ai_events", side_effect=Exception("CH down"))
    @patch("hanzo_insights.feature_enabled", return_value=False)
    ...
    @patch("insights.tasks.ai_observability_usage_report.logger")
    @patch("insights.tasks.ai_observability_usage_report.capture_event", side_effect=Exception("capture down"))
    @patch("insights.tasks.ai_observability_usage_report.get_ph_client")
