...
2. **On flag key rename**, `_update_super_groups_for_key_change()` in `insights/api/feature_flag.py` rewrites the enrollment key to match the new flag key
3. **Migration 0748** (`insights/migrations/0748_update_featureflag_super_groups.py`) was specifically written to fix historical cases where the enrollment key drifted out of sync — it forced `$feature_enrollment/{flag.key}` for every active flag
...
- [x] Update `_update_super_groups_for_key_change` in `insights/api/feature_flag.py` — added comment noting `feature_enrollment` is key-independent
...
- [ ] Remove `("super_groups", "rollout_percentage")` from `ROLLOUT_PERCENTAGE_PATHS` in `insights/approvals/actions/feature_flags.py`
  - Feature enrollment doesn't have a rollout percentage concept — no replacement path needed
- [ ] Remove `_update_super_groups_for_key_change()` from `insights/api/feature_flag.py`
...
- [ ] Update test assertions that check `filters["super_groups"]`:
  ...
  - `insights/api/test/test_feature_flag.py` (`test_updating_feature_flag_key_updates_super_groups` — remove or rewrite)
  - `insights/approvals/tests/test_update_feature_flag_action.py`
- [ ] Update frontend — see [Frontend changes detail](#frontend-changes-detail) below
- [ ] Opportunistic cleanup: strip `super_groups` from `filters` on next flag save (`update()` in `insights/api/feature_flag.py`)
  - Same pattern as `holdout_groups` cleanup (line 1364)
- [ ] Run `insightscli build:openapi` to regenerate TypeScript types
...
- [ ] Write a Django data migration to remove `super_groups` from `filters` JSON on all flags
  ...
  - Consider doing this as a raw SQL migration for efficiency: `UPDATE insights_featureflag SET filters = filters - 'super_groups' WHERE filters ? 'super_groups'`
...
| --------------------------------- | ------------------------------------------------------------------------ | ----- |
| Django serializer schema          | `insights/api/feature_flag.py`                                            | 1, 5  |
...
| Approvals change tracking         | `insights/approvals/actions/feature_flags.py`                             | 5     |
...
| Data migration (backfill)         | `insights/migrations/XXXX_backfill_feature_enrollment.py`                 | 3     |
| Data migration (cleanup)          | `insights/migrations/XXXX_strip_super_groups.py`                          | 6     |
...
- The Python flag matching code in `insights/models/feature_flag/flag_matching.py` is legacy but still actively used for database-backed evaluation. Unlike holdouts, super_groups ARE evaluated in this code path — Phase 4 must update it.
