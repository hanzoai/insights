...
def _product_taxonomy(team: Team) -> _ProductTaxonomy:
    ...
    try:
        events = list(
            ...
            .exclude(name__startswith="$")  # Insights internal events ($pageview etc.) aren't product categories
...
_SYSTEM_PROMPT = """
You help a Insights user build the tag vocabulary for a session-replay classifier. The classifier
...
def _generate(*, user_content: str, team_id: int, distinct_id: str) -> _LlmSuggestions:
    ...
    try:
        response = client.models.generate_content(
            ...
            insights_properties={"ai_product": "replay_vision", "feature": "suggest_classifier_tags"},
            insights_groups={"project": str(team_id)},
        )
    except Exception as e:
