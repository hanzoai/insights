...
_BOT_MACHINE_USERS = {"insights-bot"}
...
def is_bot_author(user: dict) -> bool:
    ...
    machine users like insights-bot are type "User", so match them by login.
...
def check_team_membership(author: str, team_slug: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ["gh", "api", f"orgs/Insights/teams/{team_slug}/memberships/{author}"],
            capture_output=True,
            text=True,
            timeout=10,
        )
