...
export function getTraceReviewTagItems({
    ...
    tagTypeOverride?: React.ComponentProps<typeof Tag>['type']
}): TraceReviewTagItem[] {
    const scores = getTraceReviewScores(review)
...
function renderTraceReviewTags({
    ...
    const tagNodes = items.map((item) => (
        <Tag key={item.key} size={size} type={item.type} className={className} onClick={onClick}>
...
export function TraceReviewStatusTag({
    ...
    if (!review) {
        return (
            <Tag size={size} type={tagTypeOverride ?? 'muted'} className={className} onClick={onClick}>
                Not reviewed
            </Tag>
        )
    }
