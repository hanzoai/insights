...
def prepare_pi_runtime() -> None:
    ...
    plugin_dir = Path.home() / ".pi/packages/pi-datastore-autoresearch"
    if not plugin_dir.is_dir():
        log("installing local pi-datastore-autoresearch plugin")
        run(["pi", "install", str(PI_PLUGIN_DIR)])
    else:
        log("pi-datastore-autoresearch already installed")
