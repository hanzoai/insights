...
interface TextAreaPropsBase extends Pick<
...
export type TextAreaProps = TextAreaWithEnterProps | TextAreaWithCmdEnterProps
    ...
    return (
        <div className={cn('flex flex-col rounded', !hideFocus && 'input-like', className)}>
            <TextareaAutosize
                minRows={minRows}
                ref={textRef}
                className={cn('TextArea w-full', hasFooter ? 'rounded-t' : 'rounded', className)}
                onKeyDown={(e) => {
                    if (stopPropagation) {
                        e.stopPropagation()
                    }
