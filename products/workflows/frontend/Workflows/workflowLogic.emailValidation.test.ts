...
const makeEmailAction = (fromValue: any): Extract<InsightsFlowAction, { type: 'function_email' }> => ({
    id: EMAIL_NODE_ID,
    type: 'function_email',
    name: 'Send email',
    description: '',
...
const makeWorkflow = (fromValue: any): InsightsFlow => ({
    id: WORKFLOW_ID,
    ...
    status: 'draft',
    team_id: 1,
    trigger: { type: 'event', filters: {} } as InsightsFlow['trigger'],
    created_at: '2026-05-01T00:00:00.000Z',
    updated_at: '2026-05-01T00:00:00.000Z',
})

// Hangs the templates fetch so `insightsFunctionTemplatesByIdLoading` stays true and the
...
describe('workflowLogic email step "from" validation', () => {
    ...
    it('keeps the email-block error after templates load (function-action branch must not clobber it)', async () => {
        ...
        logic = workflowLogic({ id: WORKFLOW_ID })
        logic.mount()
        await expectLogic(logic).toDispatchActions(['loadWorkflowSuccess', 'loadInsightsFunctionTemplatesByIdSuccess'])
