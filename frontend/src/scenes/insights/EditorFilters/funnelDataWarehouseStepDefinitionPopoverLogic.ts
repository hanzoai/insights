import { MakeLogicType, actions, afterMount, connect, kea, key, listeners, path, props, reducers, selectors } from 'kea'
import insights from 'insights-js'

import { definitionPopoverLogic } from 'lib/components/DefinitionPopover/definitionPopoverLogic'
import type { TablePreviewExpressionColumn } from 'lib/components/TablePreview/types'
import { getDataWarehouseItemWithFieldDefaults } from 'lib/components/TaxonomicFilter/dataWarehouseItemUtils'
import type {
    DataWarehousePopoverField,
    TaxonomicFilterGroup,
    TaxonomicFilterValue,
} from 'lib/components/TaxonomicFilter/types'
import { funnelDataLogic } from 'scenes/funnels/funnelDataLogic'

import type { DatabaseSerializedFieldType } from '~/queries/schema/schema-general'
import { InsightLogicProps } from '~/types'

import { DataWarehouseTableForInsight } from 'products/data_warehouse/frontend/types'

import type { TaxonomicDefinitionTypes } from '../../../lib/components/TaxonomicFilter/types'
import type {
    DatabaseSchemaField,
    DatabaseSchemaSchema,
    DatabaseSchemaSource,
    FunnelsQuery,
} from '../../../queries/schema/schema-general'
import type { DatabaseSchemaTableCertification } from '../../../queries/schema/schema-general'

export type FunnelFieldKey = 'id_field' | 'timestamp_field' | 'aggregation_target_field'

export const EDITABLE_FIELD_ORDER: FunnelFieldKey[] = ['aggregation_target_field', 'timestamp_field', 'id_field']
const ALLOWED_COLUMN_TYPES_BY_FIELD_KEY: Record<FunnelFieldKey, DatabaseSerializedFieldType[]> = {
    aggregation_target_field: ['string'],
    timestamp_field: ['datetime', 'date', 'string'],
    id_field: ['string', 'integer', 'decimal', 'float'],
}
const HIDDEN_FIELD_TYPES: DatabaseSerializedFieldType[] = ['lazy_table', 'virtual_table', 'view', 'materialized_view']
const LINKED_TABLE_TYPES: DatabaseSerializedFieldType[] = ['lazy_table', 'virtual_table']

export interface FunnelDataWarehouseStepDefinitionPopoverLogicProps {
    table: DataWarehouseTableForInsight
    group: TaxonomicFilterGroup
    dataWarehousePopoverFields: DataWarehousePopoverField[]
    selectedItemMeta?: Record<string, any> | null
    onSelectItem: (group: TaxonomicFilterGroup, value: TaxonomicFilterValue | null, item: any) => void
    insightProps: InsightLogicProps
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface funnelDataWarehouseStepDefinitionPopoverLogicValues {
    definition: Partial<TaxonomicDefinitionTypes> // definitionPopoverLogic
    localDefinition: Partial<TaxonomicDefinitionTypes> // definitionPopoverLogic
    querySource: FunnelsQuery | null // funnelDataLogic
    activeField: DataWarehousePopoverField | undefined
    activeFieldIsInsightsQL: boolean
    activeFieldKey: FunnelFieldKey
    activeFieldKeyOptions: {
        label: string
        value: FunnelFieldKey
    }[]
    activeFieldOptions: (
        | {
              label: string
              value: string
          }
        | {
              label: string
              type: DatabaseSerializedFieldType
              value: string
          }
    )[]
    activeFieldValue: string | undefined
    columnOptions: {
        label: string
        type: DatabaseSerializedFieldType
        value: string
    }[]
    dataWarehousePopoverFields: DataWarehousePopoverField[]
    isAggregatingByGroup: boolean
    isAggregatingByInsightsQL: boolean
    linkedTables: string[]
    previewExpressionColumns: TablePreviewExpressionColumn[]
    previewSelectedKey: string | undefined
    previewTable: {
        aggregation_target_field?: string | undefined
        certification?: DatabaseSchemaTableCertification | undefined
        distinct_id_field?: string | undefined
        fields: {
            [k: string]: DatabaseSchemaField
        }
        format?: string | undefined
        id: string
        id_field?: string | undefined
        name: string
        row_count?: number | undefined
        schema?: DatabaseSchemaSchema | undefined
        search_aliases?: string[] | undefined
        source?: DatabaseSchemaSource | undefined
        timestamp_field?: string | undefined
        type: 'data_warehouse'
        url_pattern?: string | undefined
    }
    scopedLocalDefinition: Partial<TaxonomicDefinitionTypes>
    selectedItemMeta: any
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface funnelDataWarehouseStepDefinitionPopoverLogicActions {
    setLocalDefinition: (item: Partial<TaxonomicDefinitionTypes>) => {
        item: Partial<TaxonomicDefinitionTypes>
    } // definitionPopoverLogic
    selectTable: () => {
        value: true
    }
    setActiveFieldKey: (activeFieldKey: FunnelFieldKey) => {
        activeFieldKey: FunnelFieldKey
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface funnelDataWarehouseStepDefinitionPopoverLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        dataWarehousePopoverFields: (
            dataWarehousePopoverFields: DataWarehousePopoverField[]
        ) => DataWarehousePopoverField[]
        selectedItemMeta: (arg: any) => any
        columnOptions: (table: DataWarehouseTableForInsight) => {
            label: string
            type: DatabaseSerializedFieldType
            value: string
        }[]
        linkedTables: (table: DataWarehouseTableForInsight) => string[]
        activeFieldKeyOptions: (dataWarehousePopoverFields: DataWarehousePopoverField[]) => {
            label: string
            value: FunnelFieldKey
        }[]
        activeField: (
            dataWarehousePopoverFields: DataWarehousePopoverField[],
            activeFieldKey: FunnelFieldKey
        ) => DataWarehousePopoverField | undefined
        scopedLocalDefinition: (
            definition: Partial<TaxonomicDefinitionTypes>,
            localDefinition: Partial<TaxonomicDefinitionTypes>,
            table: DataWarehouseTableForInsight,
            selectedItemMeta: any
        ) => Partial<TaxonomicDefinitionTypes>
        activeFieldValue: (
            scopedLocalDefinition: Partial<TaxonomicDefinitionTypes>,
            activeFieldKey: FunnelFieldKey
        ) => string | undefined
        previewTable: (table: DataWarehouseTableForInsight) => {
            aggregation_target_field?: string | undefined
            certification?: DatabaseSchemaTableCertification | undefined
            distinct_id_field?: string | undefined
            fields: {
                [k: string]: DatabaseSchemaField
            }
            format?: string | undefined
            id: string
            id_field?: string | undefined
            name: string
            row_count?: number | undefined
            schema?: DatabaseSchemaSchema | undefined
            search_aliases?: string[] | undefined
            source?: DatabaseSchemaSource | undefined
            timestamp_field?: string | undefined
            type: 'data_warehouse'
            url_pattern?: string | undefined
        }
        previewExpressionColumns: (
            table: DataWarehouseTableForInsight,
            dataWarehousePopoverFields: DataWarehousePopoverField[],
            scopedLocalDefinition: Partial<TaxonomicDefinitionTypes>
        ) => TablePreviewExpressionColumn[]
        previewSelectedKey: (
            previewExpressionColumns: TablePreviewExpressionColumn[],
            activeFieldKey: FunnelFieldKey,
            activeFieldValue: string | undefined
        ) => string | undefined
        activeFieldOptions: (
            columnOptions: {
                label: string
                type: DatabaseSerializedFieldType
                value: string
            }[],
            activeField: DataWarehousePopoverField | undefined,
            activeFieldKey: FunnelFieldKey
        ) => (
            | {
                  label: string
                  value: string
              }
            | {
                  label: string
                  type: DatabaseSerializedFieldType
                  value: string
              }
        )[]
        activeFieldIsInsightsQL: (activeFieldValue: string | undefined, table: DataWarehouseTableForInsight) => boolean
        isAggregatingByGroup: (querySource: FunnelsQuery | null) => boolean
        isAggregatingByInsightsQL: (querySource: FunnelsQuery | null, isAggregatingByGroup: boolean) => boolean
    }
}

export type funnelDataWarehouseStepDefinitionPopoverLogicType = MakeLogicType<
    funnelDataWarehouseStepDefinitionPopoverLogicValues,
    funnelDataWarehouseStepDefinitionPopoverLogicActions,
    FunnelDataWarehouseStepDefinitionPopoverLogicProps,
    funnelDataWarehouseStepDefinitionPopoverLogicMeta
>

export const funnelDataWarehouseStepDefinitionPopoverLogic = kea<funnelDataWarehouseStepDefinitionPopoverLogicType>([
    props({} as FunnelDataWarehouseStepDefinitionPopoverLogicProps),
    key((props) => props.table.name),
    path((key) => ['scenes', 'insights', 'EditorFilters', 'funnelDataWarehouseStepDefinitionPopoverLogic', key]),
    connect((props: FunnelDataWarehouseStepDefinitionPopoverLogicProps) => ({
        values: [
            definitionPopoverLogic,
            ['definition', 'localDefinition'],
            funnelDataLogic(props.insightProps),
            ['querySource'],
        ],
        actions: [definitionPopoverLogic, ['setLocalDefinition']],
    })),
    actions(() => ({
        setActiveFieldKey: (activeFieldKey: FunnelFieldKey) => ({
            activeFieldKey,
        }),
        selectTable: true,
    })),
    selectors({
        dataWarehousePopoverFields: [
            (_, props) => [props.dataWarehousePopoverFields],
            (fields: DataWarehousePopoverField[]) => fields,
        ],
        selectedItemMeta: [() => [(_, props) => props.selectedItemMeta], (selectedItemMeta) => selectedItemMeta],
    }),
    reducers({
        activeFieldKey: [
            'aggregation_target_field' as FunnelFieldKey,
            {
                setActiveFieldKey: (_, { activeFieldKey }) => activeFieldKey,
            },
        ],
    }),
    selectors({
        columnOptions: [
            (_, p) => [p.table],
            // `table.fields` may be missing when the table is reconstructed
            // from a committed filter (new menu rebuild path passes
            // `{ name, id }` without re-attaching the full schema). Empty
            // array prevents the `Object.values(undefined)` crash; the
            // user reopens via the table picker to get full options.
            (table: DataWarehouseTableForInsight) =>
                table.fields
                    ? Object.values(table.fields).map((column) => ({
                          label: `${column.name} (${column.type})`,
                          value: column.name,
                          type: column.type,
                      }))
                    : [],
        ],
        linkedTables: [
            (_, p) => [p.table],
            (table: DataWarehouseTableForInsight) =>
                table.fields
                    ? Object.values(table.fields)
                          .filter((field) => LINKED_TABLE_TYPES.includes(field.type))
                          .map((field) => field.name)
                    : [],
        ],
        activeFieldKeyOptions: [
            (s) => [s.dataWarehousePopoverFields],
            (dataWarehousePopoverFields: DataWarehousePopoverField[]) =>
                EDITABLE_FIELD_ORDER.map((key) => ({
                    value: key,
                    label: dataWarehousePopoverFields.find((f) => f.key === key)?.label ?? key,
                })),
        ],
        activeField: [
            (s) => [s.dataWarehousePopoverFields, s.activeFieldKey],
            (dataWarehousePopoverFields: DataWarehousePopoverField[], activeFieldKey: FunnelFieldKey) =>
                dataWarehousePopoverFields.find((f) => f.key === activeFieldKey),
        ],
        scopedLocalDefinition: [
            (s, p) => [s.definition, s.localDefinition, p.table, s.selectedItemMeta],
            (
                definition: Partial<import('lib/components/TaxonomicFilter/types').TaxonomicDefinitionTypes>,
                localDefinition: Partial<import('lib/components/TaxonomicFilter/types').TaxonomicDefinitionTypes>,
                table: DataWarehouseTableForInsight,
                selectedItemMeta
            ) =>
                definition.name === table.name
                    ? localDefinition
                    : getDataWarehouseItemWithFieldDefaults(table, selectedItemMeta),
        ],
        activeFieldValue: [
            (s) => [s.scopedLocalDefinition, s.activeFieldKey],
            (
                scopedLocalDefinition: Partial<import('lib/components/TaxonomicFilter/types').TaxonomicDefinitionTypes>,
                activeFieldKey: FunnelFieldKey
            ) => (scopedLocalDefinition as Partial<Record<FunnelFieldKey, string | undefined>>)[activeFieldKey],
        ],
        previewTable: [
            (_, p) => [p.table],
            (table: DataWarehouseTableForInsight) => ({
                ...table,
                fields: table.fields
                    ? Object.fromEntries(
                          Object.entries(table.fields).filter(([_, field]) => !HIDDEN_FIELD_TYPES.includes(field.type))
                      )
                    : {},
            }),
        ],
        previewExpressionColumns: [
            (s, p) => [p.table, s.dataWarehousePopoverFields, s.scopedLocalDefinition],
            (
                table: DataWarehouseTableForInsight,
                dataWarehousePopoverFields: DataWarehousePopoverField[],
                scopedLocalDefinition: Partial<import('lib/components/TaxonomicFilter/types').TaxonomicDefinitionTypes>
            ): TablePreviewExpressionColumn[] => {
                const tableFieldNames = new Set(
                    table.fields ? Object.values(table.fields).map((field) => field.name) : []
                )
                const usedKeys = new Set(tableFieldNames)
                return EDITABLE_FIELD_ORDER.flatMap((fieldKey) => {
                    const configuredValue = (
                        scopedLocalDefinition as Partial<Record<FunnelFieldKey, string | undefined>>
                    )[fieldKey]
                    if (typeof configuredValue !== 'string') {
                        return []
                    }

                    const expression = configuredValue.trim()
                    if (!expression || tableFieldNames.has(expression)) {
                        return []
                    }

                    const label = dataWarehousePopoverFields.find((field) => field.key === fieldKey)?.label ?? fieldKey
                    const keyBase = `__${fieldKey}_insightsql_expression`
                    let key = keyBase
                    let suffix = 2

                    while (usedKeys.has(key)) {
                        key = `${keyBase}_${suffix}`
                        suffix += 1
                    }

                    usedKeys.add(key)

                    return [
                        {
                            key,
                            expression,
                            label: `${label} (SQL expression)`,
                            type: 'SQL expression',
                        },
                    ]
                })
            },
        ],
        previewSelectedKey: [
            (s) => [s.previewExpressionColumns, s.activeFieldKey, s.activeFieldValue],
            (
                previewExpressionColumns: TablePreviewExpressionColumn[],
                activeFieldKey: FunnelFieldKey,
                activeFieldValue: string | undefined
            ) =>
                previewExpressionColumns.find((column) => column.key.startsWith(`__${activeFieldKey}_insightsql_expression`))
                    ?.key ?? activeFieldValue,
        ],
        activeFieldOptions: [
            (s) => [s.columnOptions, s.activeField, s.activeFieldKey],
            (
                columnOptions: {
                    label: string
                    type: DatabaseSerializedFieldType
                    value: string
                }[],
                activeField: DataWarehousePopoverField | undefined,
                activeFieldKey: FunnelFieldKey
            ) => {
                const filteredColumnOptions = columnOptions.filter((column) =>
                    ALLOWED_COLUMN_TYPES_BY_FIELD_KEY[activeFieldKey].includes(column.type)
                )

                return activeField
                    ? [
                          ...filteredColumnOptions,
                          ...(activeField.allowInsightsQL ? [{ label: 'SQL Expression', value: '' }] : []),
                      ]
                    : filteredColumnOptions
            },
        ],
        activeFieldIsInsightsQL: [
            (s, p) => [s.activeFieldValue, p.table],
            (activeFieldValue: string | undefined, table: DataWarehouseTableForInsight) =>
                !table.fields || !Object.values(table.fields).some((field) => field.name === activeFieldValue),
        ],
        isAggregatingByGroup: [
            (s) => [s.querySource],
            (querySource: null | import('~/queries/schema/schema-general').FunnelsQuery) =>
                querySource?.aggregation_group_type_index != null,
        ],
        isAggregatingByInsightsQL: [
            (s) => [s.querySource, s.isAggregatingByGroup],
            (
                querySource: null | import('~/queries/schema/schema-general').FunnelsQuery,
                isAggregatingByGroup: boolean
            ) => Boolean(querySource?.funnelsFilter?.funnelAggregateByInsightsQL) && !isAggregatingByGroup,
        ],
    }),
    listeners(({ values, props }) => ({
        selectTable: () => {
            props.onSelectItem(props.group, props.table.name, values.scopedLocalDefinition)
            insights.capture('funnel data warehouse step selected')
        },
    })),
    afterMount(() => {
        insights.capture('funnel data warehouse step popover viewed')
    }),
])
