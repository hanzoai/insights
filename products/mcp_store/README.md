...
This is unrelated to `products/*/mcp/tools.yaml`, which exposes Insights's own endpoints as MCP tools.
...
Settings supplies its own navigation shell so the gateway workflows fit the main Insights application without importing the Insights Code layout.
...
1. Register an OAuth app in the vendor's developer console with redirect URI `https://us.hanzo.ai/api/mcp_store/oauth_redirect/` (repeat for EU with the EU host).
