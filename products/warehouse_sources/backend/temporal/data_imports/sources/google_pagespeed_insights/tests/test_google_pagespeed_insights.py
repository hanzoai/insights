...
class TestParseUrls:
    @pytest.mark.parametrize(
        "raw, expected",
        [
            ("https://hanzo.ai", ["https://hanzo.ai"]),
            ("  https://hanzo.ai  ", ["https://hanzo.ai"]),
            ("https://hanzo.ai\nhttps://hanzo.ai/docs", ["https://hanzo.ai", "https://hanzo.ai/docs"]),
    ...
    @pytest.mark.parametrize(
        "raw",
        [None, "", "   \n  ", "hanzo.ai", "ftp://hanzo.ai", "not a url", "://missing-scheme"],
    )
...
class TestBuildUrl:
    def test_includes_url_strategy_key_and_repeated_categories(self):
        url = _build_url("https://hanzo.ai", "MOBILE", "secret-key")
        ...
        assert "url=https%3A%2F%2Fhanzo.ai" in url
...
class TestNormalizeRow:
    def test_injects_requested_url_strategy_and_timestamp(self):
        ...
        row = _normalize_row(PAGESPEED_ENDPOINTS["pagespeed_desktop"], response, "https://hanzo.ai")

        assert row is not None
        # The requested URL (not the echoed final `id`) is injected so the primary key stays stable.
        assert row["requested_url"] == "https://hanzo.ai"
        ...
        assert row["id"] == "https://hanzo.ai/"

    def test_mobile_endpoint_stamps_mobile_strategy(self):
        response = {"analysisUTCTimestamp": "2024-01-15T12:34:56Z"}
        row = _normalize_row(PAGESPEED_ENDPOINTS["pagespeed_mobile"], response, "https://hanzo.ai")
    ...
    def test_unparseable_timestamp_returns_none(self):
        ...
        assert _normalize_row(PAGESPEED_ENDPOINTS["pagespeed_desktop"], response, "https://hanzo.ai") is None
    ...
    def test_missing_timestamp_field_raises(self):
        ...
        with pytest.raises(KeyError):
            _normalize_row(PAGESPEED_ENDPOINTS["pagespeed_desktop"], {"lighthouseResult": {}}, "https://hanzo.ai")
...
class TestFetch:
    def test_success_returns_json(self):
        ...
        body = _fetch_once(session, "k", "DESKTOP", "https://hanzo.ai", structlog.get_logger())
    ...
    def test_retryable_statuses_raise_retryable(self, status):
        ...
        with pytest.raises(PageSpeedRetryableError):
            _fetch_once(session, "k", "DESKTOP", "https://hanzo.ai", structlog.get_logger())
    ...
    def test_body_read_errors_are_retried(self, exc):
        ...
        with mock.patch.object(_fetch.retry, "sleep"):  # type: ignore[attr-defined]
            body = _fetch(session, "k", "DESKTOP", "https://hanzo.ai", structlog.get_logger())
...
class TestValidateCredentials:
    ...
    def test_status_mapping(self, status, expected_valid):
        with mock.patch(f"{MODULE}.make_tracked_session") as mock_session:
            ...
            is_valid, message = validate_credentials("test-key", "https://hanzo.ai")
    ...
    def test_server_error_shows_transient_copy(self):
        with mock.patch(f"{MODULE}.make_tracked_session") as mock_session:
            ...
            _, message = validate_credentials("test-key", "https://hanzo.ai")
    ...
    def test_probes_first_url(self):
        with mock.patch(f"{MODULE}.make_tracked_session") as mock_session:
            ...
            validate_credentials("test-key", "https://hanzo.ai\nhttps://example.com")

            called_url = mock_session.return_value.get.call_args[0][0]

        assert "url=https%3A%2F%2Fhanzo.ai" in called_url
...
class TestSource:
    ...
    def test_source_response_shape(self, endpoint):
        response = google_pagespeed_insights_source("test-key", endpoint, "https://hanzo.ai", structlog.get_logger())
