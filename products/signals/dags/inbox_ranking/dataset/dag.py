...
COMMON_ASSET_KWARGS: dict[str, Any] = {
    ...
    "tags": owner_tags,
    # Transient Datastore/S3 blips shouldn't fail the daily partition outright.
...
def _tag_dagster_queries(context: dagster.AssetExecutionContext) -> None:
    """Stamp product + feature + dagster run tags into the thread's query tags so every Datastore
    query this asset issues (sync_execute and InsightsQL alike) is attributable in system.query_log.
...
def inbox_report_labels(context: dagster.AssetExecutionContext) -> None:
    ...
    for stream_name, sql, _columns in LABEL_STREAMS:
        stream_rows[stream_name] = insightsql_rows(
            sql,
            team=team,
