...
def _listing_client(schedules_by_dag):
    # query-aware fake: schedule listings are per-DAG (InsightsDagId search attribute), and the
...
class TestConsolidateDags(BaseTest):
    def _query(self, name: str, sql: str, interval: timedelta | None = None) -> DataWarehouseSavedQuery:
        return DataWarehouseSavedQuery.objects.create(
            name=name, team=self.team, query={"query": sql, "kind": "InsightsQLQuery"}, sync_frequency_interval=interval
        )
    ...
    def test_end_to_end_shared_dropped_unique_moved_with_rebuilt_edges(self):
        ...
        source = DAG.objects.create(team=self.team, name="insights_team")
    ...
    def test_source_dag_interval_carries_as_declared_target(self):
        ...
        target.save()
        source = DAG.objects.create(team=self.team, name="insights_team", sync_frequency_interval=H6)
    ...
    def test_adopted_node_unblocks_its_dependents_moves(self):
        ...
        # resolves in InsightsQL but fails Node.DoesNotExist and is adopted edge-less → its dependent
    ...
    def test_target_selection(self, _name, kind):
        ...
        other = DAG.objects.create(team=self.team, name="insights_team")
        self._node(other, self._query("a", "SELECT 1"))

        if kind == "explicit":
            output = self._run("--target-dag-id", str(other.id))
            self.assertIn(f"Merge target: insights_team ({other.id})", output)
