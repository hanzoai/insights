...
class TestAppRuntimeStartApp(BaseTest):
    ...
    def test_bridge_token_is_minted_for_the_active_version_author(self, mock_mint, mock_get_sandbox_class, _mock_wait):
        """The bridge runs the app's InsightsQL as this user, so it must be the author of
        ...
        other_user = User.objects.create_and_join(self.organization, "editor@hanzo.ai", "pw")
...
class TestBuildSandboxConfig(BaseTest):
    ...
    def test_config_locks_outbound_egress_to_insights_hosts(self):
        from products.streamlit_apps.backend.logic.app_runtime import _build_sandbox_config
        ...
        config = _build_sandbox_config(app, version)
        assert config.outbound_domain_allowlist == ["us.hanzo.ai"]
    ...
    def test_config_includes_otel_env_vars(self):
        ...
        assert f"insights.team_id={app.team_id}" in attrs
        assert f"insights.app_id={app.id}" in attrs
...
class TestGetOtelLogsConfig(BaseTest):
    ...
    def test_endpoint_and_token(self, _name, is_cloud, region, expected_endpoint, expected_token):
        ...
        # `from insights.cloud_utils import is_cloud` copies the reference into
