...
- **Core**: `products/web_analytics/backend/path_cleaning_suggestions/service.py`
  - `sample_pathnames` / `count_distinct_pathnames` — top `$pathname` by views via InsightsQL.
  ...
  - `validate_and_annotate_rules` — compiles each regex with **re2** (the engine Datastore
...
- **API** (`products/web_analytics/backend/api/web_analytics_path_cleaning_suggestions.py`):
  `POST /v1/projects/:id/web_analytics_path_cleaning_suggestions/generate/` produces and stores a
  ...
  (`GET /v1/projects/:id/health_issues/?kind=path_cleaning_suggestions&status=active&dismissed=false`,
...
- **Insights AI (Max)**: generate/apply are exposed as MCP tools in
