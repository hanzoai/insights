...
from products.experiments.backend.insightsql_queries.base_query_utils import event_or_action_to_filter
...
@dataclass(frozen=True)
class SharedInsightsQLDatabase:
    """A InsightsQL virtual database built once per request and shared across that request's
    ...
    session-context scans run against it concurrently from a thread pool. InsightsQL mutates a
    ...
    insights/insightsql/database/schema/information_schema.py), and direct-connection queries make
    ...
    def fresh_context(self, team: Team, user: User) -> InsightsQLContext:
        """A per-query context carrying the shared database, so `execute_insightsql_query` reuses
        ...
        return InsightsQLContext(team_id=team.pk, user=user, database=self.database)
...
def _node_condition(node: MetricSourceNode, team: Team) -> ast.Expr:
    ...
    resolve (a cohort filter whose cohort doesn't exist here, a filter InsightsQL can't compile)
...
def scan_sessions_for_metric_events(
    ...
    *,
    ...
    window_end: datetime,
    shared_insightsql: SharedInsightsQLDatabase | None = None,
) -> MetricScanResult:
    ...
    metric, which dominated the endpoint's Datastore time in production once a session
    ...
    unbounded hit list by piling metrics onto one source. `user` threads through to InsightsQL for
    property-level access control — metric source nodes can carry property filters.

    `shared_insightsql` optionally carries a prebuilt virtual database (session_context builds
    ...
    # Condition asts are deep-copied per use site (three aggregates + the WHERE): the InsightsQL
    ...
    # execute_insightsql_query treats a passed context as fully caller-owned, so only build one when
    # there is a shared database to carry; otherwise let the executor construct its default.
    extra_kwargs: dict[str, InsightsQLContext | InsightsQLQueryModifiers] = {}
    if shared_insightsql is not None:
        extra_kwargs["context"] = shared_insightsql.fresh_context(team, user)
        extra_kwargs["modifiers"] = shared_insightsql.modifiers
    response = execute_insightsql_query(query, team=team, user=user, **extra_kwargs)
