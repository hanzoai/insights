...
export async function loadTraceSummaries(
    ...
    const response = await api.queryInsightsQL(
        insightsql`
            SELECT
                ${insightsql.raw(idExpression)} as item_id,
            ...
            WHERE event = ${eventName}
                ...
                AND ${insightsql.raw(idExpression)} IN ${missingItemIds}
            GROUP BY item_id
            LIMIT 10000
        `,
...
async function loadEvaluationSummaries(
    ...
    for (const row of response.results || []) {
        ...
        summaries[evalId] = {
            ...
            // the verdict separately as a Tag and don't need it duplicated
            // in the title.
            title: name,
            flowDiagram: '',
...
export async function loadEvaluationItemAttributes(
    ...
    // Datastore partition pruning intact.
    const response = await api.queryInsightsQL(
        insightsql`
            SELECT
                toString(uuid) as eval_id,
