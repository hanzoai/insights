...
type Result = (WithInsightsUrl<Schemas.FeatureFlag> & { found: true }) | FeatureFlagLookupMiss
...
const featureFlagGetDefinitionByKey = (): ToolBase<typeof schema, Result> => ({
    ...
    handler: async (context: Context, params: Params) => {
        ...
        const flagWithUrl = await withInsightsUrl(context, flag, `/feature_flags/${flag.id}`)
