...
const meta: Meta = {
    ...
    parameters: {
        docs: {
            description: {
                component:
                    'Popover-fronted TaxonomicFilter — dropdown menu for shortcuts, combobox with chips + preview pane for browsing, and dedicated panels for DataWarehouse config and InsightsQL expression editing. Gated in production by the `taxonomic-filter-menu-rebuild` flag.',
            },
        },
    },
    tags: ['autodocs'],
...
export const SeriesPlusDataWarehouse: Story = {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Mirrors the ActionFilterRow series picker — Data warehouse tables and InsightsQL expression appear as separate dropdown-menu entries (with chevrons) so they bypass the chip row and route directly to their dedicated panels.',
            },
        },
    },
}

export const InsightsQLOnly: Story = {
    render: () => (
        <Container
            taxonomicGroupTypes={[TaxonomicFilterGroupType.InsightsQLExpression]}
    ...
    parameters: {
        docs: {
            description: {
                story: 'Dropdown only shows the InsightsQL entry — clicking it opens the lazy-loaded Monaco editor. ⌘+Enter saves; Esc returns to the menu (and falls through to suggestions / find widgets first).',
            },
        },
    },
}
...
export const PreSelectedInsightsQL: Story = {
    render: () => (
        <Container
            taxonomicGroupTypes={[TaxonomicFilterGroupType.Events, TaxonomicFilterGroupType.InsightsQLExpression]}
            initialSelected={
                {
                    item: { name: "properties.$browser = 'Chrome'" },
                    group: { type: TaxonomicFilterGroupType.InsightsQLExpression },
    ...
    parameters: {
        docs: {
            description: {
                story: 'Pre-existing InsightsQL expression — re-opening lands directly on the Monaco editor pre-filled with the saved expression for editing.',
            },
        },
    },
}
...
// the DatabaseTablePreview / Tabs / Select / InsightsQL fallback chrome.
...
const COMPLEX_DWH_TABLE = {
    ...
    fields: {
        ...
        // Linked tables to demo the linked-tables hint in InsightsQL fallback.
