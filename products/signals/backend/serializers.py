...
class SignalSourceConfigSerializer(serializers.ModelSerializer):
    ...
    def _get_session_analysis_status(self, team_id: int) -> str | None:
        ...
        query = f'InsightsTeamId = {team_id} AND WorkflowType = "summarize-session" AND ExecutionStatus = "Running"'
        ...
        extra_kwargs = {
            ...
            "default_slack_notification_channel": {
                "help_text": (
                    ...
                    "`channel_id|#channel-name` shape Insights uses elsewhere (only the channel id is required). "
...
class SignalUserAutonomyConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            "slack_notification_channel": {
                "help_text": (
                    "Slack channel target in the same `channel_id|#channel-name` shape Insights uses elsewhere "
...
class ReportChartSerializer(serializers.Serializer):
    ...
    query = ChartQueryField(
        help_text=(
            ...
            "chart), `DataVisualizationNode` (a SQL series — a `InsightsQLQuery` source plus a `display`), "
...
class SignalReportSerializer(serializers.ModelSerializer):
    ...
    source_products = serializers.SerializerMethodField(
        help_text="Distinct source products contributing signals to this report (from Datastore).",
    )
    scout_name = serializers.SerializerMethodField(
        help_text="skill_name slug of the scout that authored this report, when scout-authored (from Datastore); null otherwise.",
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            "billing_exempt_reason": {
                "help_text": (
                    "Non-null when this report is system-marked never-billable (Insights-system origin, "
...
class SignalNodeSerializer(serializers.Serializer):
    signal_id = serializers.CharField(help_text="Datastore document id of the signal.")
...
class SignalReportArtefactSerializer(serializers.ModelSerializer):
    ...
    def get_content(self, obj: SignalReportArtefact) -> dict | list:
        ...
        # Enrich suggested_reviewers with fresh Insights user info at read time
...
class SuggestedReviewerEntryWriteSerializer(serializers.Serializer):
    ...
    user_uuid = serializers.UUIDField(
        required=False,
        help_text=(
            "Insights user UUID. Must be an org member on this team with a linked GitHub identity. "
