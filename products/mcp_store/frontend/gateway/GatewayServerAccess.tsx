                ...
                <div className="border rounded divide-y">
                    {server.agents.map((agent) => (
                        <div key={agent.service_account_id} className="flex items-center gap-3 p-2">
                            ...
                            <Tag type={agent.status === 'active' ? 'success' : 'muted'} size="small">
                                {agent.status === 'active'
...
function ConnectionStatus({
    ...
    if (accessRevoked) {
        return <Tag type="muted">Access revoked</Tag>
    }
    if (needsReauth) {
        return <Tag type="danger">Needs reauth</Tag>
    }
    if (pendingOauth) {
        return <Tag type="warning">Finishing setup</Tag>
    }
    return <Tag type="success">{lastUsedAt ? `Used ${dayjs(lastUsedAt).fromNow()}` : 'Connected'}</Tag>
}
                    ...
                    <>
                        ...
                        {teamToolPoliciesLoading ? (
                            ...
                            <div className="flex items-center justify-between gap-3 rounded border border-dashed p-4 text-sm text-secondary">
                                ...
                                <Button type="secondary" size="small" onClick={() => loadTeamToolPolicies()}>
                                    Retry
                                </Button>
                            </div>
