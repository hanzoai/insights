...
The three observability tables — `insights.metrics`, `insights.trace_spans`, `logs` — share `trace_id` as a join key. The schema is wired so `insights.metrics` can carry an exemplar `trace_id` on every point when the SDK attached one (OpenTelemetry exemplar pattern).

**Namespacing:** `logs` is registered at the InsightsQL root level; `insights.trace_spans` and `insights.metrics` live under the `insights.` namespace and must be referenced with the prefix. Bare names fail.
...
> ⚠️ **Status (as of PR [#50936](https://github.com/Insights/insights/pull/50936)):** exemplar extraction is not yet wired up in `rust/capture-logs/src/metric_record.rs` — the `_exemplars` argument is prefixed with underscore (unused). Every metric row has `trace_id = ''` today. The example query below describes the intended pattern but returns empty until exemplars are populated. The "Works today" alternative further down uses `insights.trace_spans` directly as the starting point and works against current data.
...
- **`argMax(trace_id, value)` is cheap** because the per-minute projection on `insights.metrics` pre-aggregates by `(service_name, metric_name, ...)`. Constrain the time window tightly (15 minutes is plenty for a spike).
...
- **`status_code = 2` is Error** in `insights.trace_spans` (OTel semantics). Use this column to flag error spans inline in the result.
- If you need to drill into the span tree visually, take the resulting `trace_id` and call `insights:apm-trace-get` to get the full waterfall.
...
`sample_error_trace` is then a candidate for `insights:apm-trace-get` or a `logs` lookup by `trace_id`.
