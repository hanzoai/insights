...
impl EventListener for CacheEventMetrics {
    ...
    fn on_leave(&self, reason: Event, _key: &Self::Key, _value: &Self::Value) {
        ...
        counter!("personinsights_leader_cache_entries_left_total", "reason" => reason).increment(1);
    }
}
...
impl PersonCache {
    ...
    pub fn get(&self, key: &PersonCacheKey) -> Option<Arc<CachedPerson>> {
        match self.inner.get(key) {
            Some(entry) => {
                counter!("personinsights_leader_cache_hits_total").increment(1);
                Some(entry.value().clone())
            }
            None => {
                counter!("personinsights_leader_cache_misses_total").increment(1);
                None
            }
        }
    }
