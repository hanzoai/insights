                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-alt mb-0">
                                {selectedAccessMethod === 'direct'
                                    ? `Query selected ${selectedConnector.label ?? selectedConnector.name} tables live from Insights. Tables stay in the source database and are not synced into the data warehouse.`
                                    : `Sync data from ${selectedConnector.label ?? selectedConnector.name} into the Insights data warehouse.`}
                            </p>
                        </div>
                    </div>
                )}
...
function CDCSelfManagedSetupDialog(): JSX.Element | null {
    ...
    const pubName = (payload.cdc_publication_name as string) || 'insights_pub'
    ...
    const sql = `-- 1. Grants for the Insights user
...
--    Run this as the table owner (or a superuser). Insights will create and manage
...
-- ALTER PUBLICATION "${pubName}" ADD TABLE "${schema}"."new_table";`
    ...
    return (
        <Modal
            ...
            description={`Self-managed CDC needs the publication to exist before Insights connects — Insights will create and manage the replication slot itself. Run the SQL below (covering the ${cdcTableNames.length} table${cdcTableNames.length === 1 ? '' : 's'} you selected for CDC) as the table owner, then click Verify & create.`}
            width={720}
            footer={
                <>
                    <Button
        ...
        >
            <div className="space-y-3">
                <div className="flex justify-end">
                    <Button size="small" type="secondary" icon={<IconCopy />} onClick={() => void handleCopy()}>
                        Copy SQL
                    </Button>
                </div>
...
function UnknownWizardStepFallback({
    ...
    useEffect(() => {
        insights.captureException(new Error(`Data warehouse source wizard reached an unexpected step: ${currentStep}`))
        // oxlint-disable-next-line react-hooks/exhaustive-deps
    }, [])

    return (
        <Banner type="error" action={{ children: 'Start over', onClick: onRestart }}>
