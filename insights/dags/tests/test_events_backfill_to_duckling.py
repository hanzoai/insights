...
class TestResolveDucklingTarget:
    def _resolve(
        ...
        with (
            ...
            patch("insights.dags.events_backfill_to_duckling.get_duckgres_server_for_organization", return_value=server),
            patch(
...
class TestFixupPartitionValuesForAddedFiles:
    ...
    def _seed_catalog(self, conn: psycopg.Connection[Any]) -> None:
        with conn.cursor() as cur:
            # A dropped-and-recreated 'insights' schema: only the live row may resolve.
            cur.executemany(
                "INSERT INTO public.ducklake_schema VALUES (%s, %s, %s, %s, %s, %s, %s)",
                [
                    (2, uuid.uuid4(), 1, 5, "insights", "insights/", True),
                    (1, uuid.uuid4(), 5, None, "insights", "insights/", True),
                ],
            )
            cur.execute(
    ...
    def _run_fixup(self, file_paths: list[str]) -> None:
        ...
        with patch(
            "insights.dags.events_backfill_to_duckling._open_catalog_conn",
