...
(`insights/api/github_callback/`) handles the install/OAuth callbacks that create
...
1. Start the stack via `bin/start` / `insightscli start`.
...
A GitHub App installs **once per org**, so a second Insights project in the same
...
.flox/cache/venv/bin/python manage.py shell -c \
  "from insights.models.integration import Integration; print(Integration.objects.filter(kind='github').values_list('team_id', flat=True))"
```
