# This module is responsible for adding tags/metadata to outgoing datastore queries in a thread-safe manner
import os
import sys
import uuid
import types
...
from insights.schema_enums import NodeKind, ProductKey
...
class InsightsQLFeatures(BaseModel):
    """Tables and event filters extracted from a InsightsQL AST — feeds product
    attribution in ``add_fallback_query_tags`` for ``kind=InsightsQLQuery``."""
...
class QueryTags(BaseModel):
    ...
    # and by config type (TrendsAlertConfig vs InsightsQLAlertConfig) without joining to Postgres.
    ...
    datastore_exception_type: Optional[str] = None
    ...
    # True when the query embeds a user-supplied InsightsQL string; used to split user vs platform errors in system.query_log.
    contains_user_insightsql: Optional[bool] = None

    insightsql_features: Optional[InsightsQLFeatures] = None
...
def __get_constant_tags() -> dict[str, str]:
    ...
    from insights.settings import CONTAINER_HOSTNAME, OTEL_SERVICE_NAME, TEST
...
# on every request and every Datastore query. Keep that invariant when adding new tag helpers.
...
def tag_queries(**kwargs) -> None:
    """
    The purpose of tag_queries is to pass additional context for Datastore executed queries. The tags
    are serialized into Datastore' system.query_log.log_comment column.
...
def tag_contains_user_insightsql() -> None:
    """Mark the current query as embedding a user-supplied InsightsQL string; used to separate user vs platform errors in system.query_log.
    ...
    """
    if get_query_tag_value("contains_user_insightsql"):
        return
    tag_queries(contains_user_insightsql=True)
...
# Union of every event the fallback can match — exposed so InsightsQLFeatureExtractor can use it as
...
def add_fallback_query_tags(tags: QueryTags) -> None:
    """Order: scene → kind → query structure → insightsql features (InsightsQLQuery only) → mcp source. Never overrides set values."""
...
_SOURCE_SKIP_PREFIXES: tuple[str, ...] = (
    os.path.join(_PROJECT_ROOT, "insights", "datastore", "client") + os.sep,
    _THIS_FILE,
    os.path.join(_PROJECT_ROOT, "insights", "insightsql", "query.py"),
    os.path.join(_PROJECT_ROOT, "insights", "insightsql_queries", "insights", "paginators.py"),
    os.path.join(_PROJECT_ROOT, "insights", "queries", "insight.py"),
    os.path.join(_PROJECT_ROOT, "insights", "utils.py"),
)
