...
async def test_capture_signal_dropped_activity_emits_event(ateam):
    with patch(f"{PIPELINE_MODULE_PATH}.hanzo_insights.capture") as capture:
...
async def test_capture_failure_is_swallowed(ateam):
    with (
        patch(f"{PIPELINE_MODULE_PATH}.hanzo_insights.capture", side_effect=RuntimeError("capture down")),
        patch(f"{PIPELINE_MODULE_PATH}.hanzo_insights.capture_exception") as capture_exception,
    ):
        await capture_signal_dropped_activity(
            CaptureSignalDroppedInput(
                team_id=ateam.id,
