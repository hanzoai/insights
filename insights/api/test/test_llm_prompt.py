...
from insights.api.llm_prompt import LLMPromptViewSet
from insights.api.llm_prompt_serializers import (
...
from insights.api.services.llm_prompt import MAX_PROMPT_VERSION
from insights.models.activity_logging.activity_log import ActivityLog
from insights.rate_limit import BurstRateThrottle, LLMPromptPublishBurstRateThrottle, SustainedRateThrottle
...
class TestLLMPromptAPI(APIBaseTest):
    ...
    def test_archive_endpoint_batches_version_cache_invalidation_when_many_versions_exist(self):
        ...
        with (
            patch("insights.api.services.llm_prompt.transaction.on_commit", side_effect=lambda callback: callback()),
            patch("insights.api.services.llm_prompt.invalidate_prompt_latest_cache") as mock_invalidate_latest,
    ...
    @override_settings(TEST=False)
    @patch("insights.api.llm_prompt.capture_internal")
...
class TestLLMPromptConfigAPI(APIBaseTest):
    ...
    def test_fetch_returns_null_config_for_cache_entries_written_before_config_existed(self):
        from insights.storage.llm_prompt_cache import llm_prompts_hypercache
        from insights.storage.llm_prompt_cache_keys import prompt_latest_cache_key
...
class TestLLMPromptLabelsAPI(APIBaseTest):
    ...
    def test_set_label_limit_blocks_creates_but_not_moves(self):
        ...
        with patch("insights.api.services.llm_prompt.MAX_PROMPT_LABELS", 1):
            assert self._set_label("my-prompt", "production", 1).status_code == status.HTTP_201_CREATED
