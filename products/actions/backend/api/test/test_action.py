...
class TestActionApi(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_listing_actions_filters_by_created_by(self) -> None:
        other = User.objects.create_and_join(self.organization, "other@hanzo.ai", "")
    ...
    def test_references_returns_insights_function_using_action(self):
        action = Action.objects.create(team=self.team, name="test action", steps_json=[{"event": "$pageview"}])
        hf = InsightsFunction.objects.create(
            team=self.team,
            name="My destination",
        ...
        data = response.json()
        assert len(data) == 1
        assert data[0]["type"] == "insights_function"
    ...
    def test_references_does_not_return_deleted_insights_functions(self):
        action = Action.objects.create(team=self.team, name="test action", steps_json=[{"event": "$pageview"}])
