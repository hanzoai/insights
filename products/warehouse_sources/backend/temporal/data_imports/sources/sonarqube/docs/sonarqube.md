...
<!--
This is the user-facing source doc. It must be moved to the hanzo.ai repo at
contents/docs/cdp/sources/sonarqube.md (it lives here only because this PR has no hanzo.ai
checkout). Once there, run `python manage.py audit_source_docs --docs-dir <hanzo.ai>/contents/docs/cdp/sources`.
-->
...
Sync code-quality data from your self-hosted [SonarQube Server](https://www.sonarsource.com/products/sonarqube/) into the Insights data warehouse — projects, metric definitions, coding rules, issues, and users — so you can trend technical debt, reliability, and issue counts across your projects over time.
...
- **Fewer issues than expected on a large project.** SonarQube caps issue search at 10,000 results per query window. Insights automatically pages past this by re-windowing on the issue creation date, so all issues are synced; if you still see gaps, check that the token can browse the affected projects.
