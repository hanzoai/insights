...
description: 'Author, save, and edit email templates in the Insights workflows library — compose email design JSON with Liquid personalization and create and round-trip-edit templates over MCP. Use when asked to design, build, update, or fix an email template for workflows, broadcasts, or campaigns.'
---

# Designing email templates

Use this skill when creating or editing email templates for Insights workflows — broadcast campaigns and `function_email` workflow actions send the rendered template.

## How authoring works

You author the **design JSON** (`content.email.design`) and save it with `workflows-create-email-template`. The server renders the sent email from your design with the same renderer Insights's visual editor uses, so the template opens as editable blocks for humans and sends exactly what the design describes. Schema and a working example in [references/unlayer-design-json.md](references/unlayer-design-json.md).

When talking to the user, call it the template's **design** — the design document format is an internal implementation detail. Always share the template's `_insightsUrl` edit link in your reply after creating or updating, so the user can open it in Insights directly.
...
- The tool result returns an edit link into the Insights library.
...
`content` is replaced as a whole on update, never merged — and humans may have edited the design in Insights's visual editor since you last saw it:
...
- Reference a template from a workflow's `function_email` action, or start a broadcast from it in the Insights UI.
