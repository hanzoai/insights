...
-- modelled after datastore-logs.sql following the same patterns
...
create or replace TABLE metrics AS metrics1 ENGINE = Distributed('insights', 'default', 'metrics1');

-- Raw metrics as a TSDB series/samples split (NOT the fat one-row-per-event
-- shape logs/traces use). Mirrors insights/datastore/metrics/metric_events.py
...
create or replace TABLE metric_series AS metric_series1 ENGINE = Distributed('insights', 'default', 'metric_series1');
...
create or replace TABLE metric_samples AS metric_samples1 ENGINE = Distributed('insights', 'default', 'metric_samples1');
...
-- Kafka engine table reading AVRO from datastore_metrics topic
...
ENGINE = Kafka('kafka:9092', 'datastore_metrics', 'datastore-metrics-avro', 'Avro')
SETTINGS
...
-- Two MVs on the existing kafka_metrics_avro: every datastore_metrics point fans
...
-- NOT recompute it. Datastore never computes the identity (no cityHash64 over the
...
-- Read aliases in the `insights` database.
-- The product connection uses DATASTORE_DATABASE=insights and resolves the
...
-- through docker datastore-client). Without these aliases the product reads
-- empty same-named tables in `insights` while data accumulates in `default`.
DROP TABLE IF EXISTS insights.metrics1_to_metric_attributes;
DROP TABLE IF EXISTS insights.metrics1_to_resource_attributes;
DROP TABLE IF EXISTS insights.metrics1;
CREATE OR REPLACE TABLE insights.metrics AS default.metrics1 ENGINE = Distributed('insights', 'default', 'metrics1');
DROP TABLE IF EXISTS insights.metric_attributes;
CREATE TABLE insights.metric_attributes AS default.metric_attributes ENGINE = Distributed('insights', 'default', 'metric_attributes');
DROP TABLE IF EXISTS insights.metrics_kafka_metrics;
CREATE TABLE insights.metrics_kafka_metrics AS default.metrics_kafka_metrics ENGINE = Distributed('insights', 'default', 'metrics_kafka_metrics');
DROP TABLE IF EXISTS insights.metric_series;
CREATE OR REPLACE TABLE insights.metric_series AS default.metric_series1 ENGINE = Distributed('insights', 'default', 'metric_series1');
DROP TABLE IF EXISTS insights.metric_samples;
CREATE OR REPLACE TABLE insights.metric_samples AS default.metric_samples1 ENGINE = Distributed('insights', 'default', 'metric_samples1');

select 'datastore metrics tables initialised successfully!';
