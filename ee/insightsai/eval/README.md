...
2. Run all evals with:
   ...
   ```bash
   pytest ee/insightsai/eval/ci
   ```

   The key bit is specifying the `ee/insightsai/eval/ci` directory – that activates our eval-specific config, `ee/insightsai/eval/pytest.ini`!

   As always with pytest, you can also run a specific file, e.g. `pytest ee/insightsai/eval/ci/eval_root.py`. Apply the `--eval sql` argument to only run evals for test cases that contain `sql`.
...
Additionally, you need an evaluation module in `ee/insightsai/eval/offline/*` that contains an evaluation test case with defined scorers. The test suite may contain multiple test cases, and they are separately reported. For example, if we wanted to evaluate SQL, we could implement the following evaluation module:
...
from ee.insightsai.eval.base import MaxPrivateEval
from ee.insightsai.eval.offline.conftest import EvaluationContext, capture_score, get_eval_context
from ee.insightsai.eval.schema import DatasetInput
from ee.insightsai.eval.scorers.sql import SQLSemanticsCorrectness, SQLSyntaxCorrectness
from ee.insightsai.chat_agent import AssistantGraph
from ee.insightsai.utils.types import AssistantState
...
ops:
  ...
  spawn_evaluation_container:
    config:
      evaluation_module: ee/insightsai/eval/offline/eval_sql.py # Evaluation module
