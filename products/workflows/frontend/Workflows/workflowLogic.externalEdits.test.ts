...
const makeWorkflow = (overrides: Partial<InsightsFlow> = {}): InsightsFlow => ({
    id: WORKFLOW_ID,
...
describe('workflowLogic external edits', () => {
    ...
    it('adopts the latest server baseline (and keeps edits) when the user chooses Keep mine', async () => {
        ...
        useMocks({
            get: {
                '/v1/environments/:team_id/script_flows/:id/': () => [200, makeWorkflow({ updated_at: NEWER })],
                '/v1/projects/:team_id/insights_function_templates/': { results: [], count: 0 },
            },
        })
    ...
    it('shows the banner when a save is rejected as stale (409 backstop)', async () => {
        ...
        useMocks({
            get: {
                '/v1/environments/:team_id/script_flows/:id/': () => [200, makeWorkflow()],
                '/v1/projects/:team_id/insights_function_templates/': { results: [], count: 0 },
            },
            patch: {
                '/v1/environments/:team_id/script_flows/:id/': () => [409, { detail: 'stale_update' }],
            },
        })
        logic.actions.setAutoSaveEnabled(false)
