...
# InsightsQL property accessors (backticked because the keys are literal property names, not
# placeholder values).
...
def fetch_response_rows(
    *,
    ...
    paginator = InsightsQLHasMorePaginator(limit=limit, offset=offset)
    ...
    # Dynamically add one column per question using the InsightsQL getSurveyResponse helper —
    ...
    if question_id:
        ...
        # coalesce-then-trim defends against NULL semantics in InsightsQL — without it the
