...
2. ``early_significance_notification`` (C9 — significance reached early) —
   user reports Insights flagging significance at low sample / short runtime
...
To run:
    ...
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_interpretation_traps"
"""
...
from products.insights_ai.evals.experiments.scorers import AdvisesAgainstShipping, CitesDiagnosticGroup
...
async def eval_interpretation_traps(ctx: EvalContext) -> None:
    cases: list[SandboxedEvalCase] = [
        ...
        SandboxedEvalCase(
            # Inline-evidence pattern. The shape: Insights flagged significance early, the user
            ...
            prompt=(
                f"Insights just notified me that my experiment '{ROLLOUT_EXPERIMENT_NAME}' has "
