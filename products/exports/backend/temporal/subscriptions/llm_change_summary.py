...
from insights.llm_prompt import normalize_prompt_to_string
from insights.security.llm_prompt_sanitization import (
...
SUBSCRIPTION_PROMPT_SOURCE = Counter(
    "insights_subscription_prompt_source_total",
...
CHANGE_SYSTEM_PROMPT = """You are a data analyst assistant for Insights. You summarize changes in analytics data between subscription deliveries.
...
INITIAL_SYSTEM_PROMPT = """You are a data analyst assistant for Insights. You summarize the current state of analytics data for a subscription delivery.
...
def _get_openai_client() -> OpenAI:
    ...
    return OpenAI(insights_client=hanzo_insights.setup(), base_url=settings.OPENAI_BASE_URL, max_retries=3)
...
def generate_change_summary(
    ...
    insights_properties: dict[str, object] = {"ai_product": "subscriptions"}
    if delivery_id:
        insights_properties["delivery_id"] = delivery_id

    extra_capture_kwargs: dict[str, object] = {}
    if team is not None:
        insights_properties["$ai_billable"] = True
        insights_properties["team_id"] = team.id
        extra_capture_kwargs["insights_groups"] = {"project": str(team.id), **groups()}
