...
class TestFireLoopCreatesRun(LoopRunsTestCase):
    def test_successful_fire_creates_an_internal_task_with_the_full_config_snapshot(self):
        ...
        loop = self.create_loop(
            ...
            connectors={"insights_mcp_scopes": "full"},
    ...
    def test_fire_dispatches_the_workflow_with_derived_create_pr_and_mcp_scopes(
        ...
        mock_dispatch.assert_called_once_with(
            ...
            insights_mcp_scopes=expected_scopes,
        )

    @parameterized.expand(
        [
            ("report_only_read_only_default", {}, {}, False, "read_only"),
            ("create_prs_and_full_scopes_opt_in", {"create_prs": True}, {"insights_mcp_scopes": "full"}, True, "full"),
        ]
    )
    def test_fire_persists_pending_dispatch_for_the_orphan_reconciler(
        ...
        self.assertEqual(pending_dispatch["insights_mcp_scopes"], expected_scopes)
...
class TestFireLoopSeedsSkillBundles(LoopRunsTestCase):
    ...
    def test_fire_without_bundles_does_not_touch_storage(self):
        ...
        with patch("insights.storage.object_storage.copy") as mock_copy:
            result, mock_dispatch = self.fire_with_post_commit(loop, trigger)
    ...
    @patch("insights.storage.object_storage.delete_objects")
...
class TestFireLoopContextTarget(LoopRunsTestCase):
    ...
    def fire_and_capture(self, loop: Loop, trigger: LoopTrigger, fire_key: str = "fire-ctx"):
        ...
        insights_mcp_scopes are observable. Returns (result, dispatched_scopes | None)."""
        with patch(f"{LOOP_RUNS_MODULE}._execute_task_processing_workflow_for_loop") as mock_dispatch:
            with self.captureOnCommitCallbacks(execute=True):
                result = fire_loop(loop, trigger, fire_key, "ctx")
        scopes = mock_dispatch.call_args.kwargs["insights_mcp_scopes"] if mock_dispatch.call_args else None
    ...
    def test_context_write_outputs_grant_file_system_write_without_widening_to_full(self, _name, outputs):
        ...
        loop = self.create_loop(
            connectors={"insights_mcp_scopes": "read_only"}, context_target=self.context_target(**outputs)
        )
        trigger = self.create_trigger(loop)
    ...
    def test_feed_only_attachment_keeps_read_only_scope_and_omits_publish_block(self):
        ...
        loop = self.create_loop(
            connectors={"insights_mcp_scopes": "read_only"}, context_target=self.context_target(post_to_feed=True)
        )
        trigger = self.create_trigger(loop)
