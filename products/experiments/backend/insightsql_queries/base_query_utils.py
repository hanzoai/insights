...
from insights.insightsql_queries.insights.trends.aggregation_operations import ALLOWED_SESSION_MATH_PROPERTIES
from insights.insightsql_queries.utils.query_date_range import QueryDateRange
from insights.models.team.team import Team

from products.actions.backend.models.action import Action
from products.experiments.backend.insightsql_queries.insightsql_aggregation_utils import (
...
def get_source_value_expr(source: Union[EventsNode, ActionsNode, ExperimentDataWarehouseNode]) -> ast.Expr:
    ...
    if isinstance(source, EventsNode) or isinstance(source, ActionsNode):
        ...
        if hasattr(source, "math") and is_continuous(source.math):
            ...
            hasattr(source, "math")
            and source.math == ExperimentMetricMathType.INSIGHTSQL
            and getattr(source, "math_insightsql", None) is not None
        ):
            # Extract the inner expression from the InsightsQL expression
            math_insightsql = source.math_insightsql
            if math_insightsql:
                tag_contains_user_insightsql()
                _, inner_expr, _, _ = extract_aggregation_and_inner_expr(math_insightsql)
                return inner_expr
...
def data_warehouse_node_to_filter(team: Team, node: ExperimentDataWarehouseNode) -> ast.Expr:
    ...
    # Use property_to_expr to convert properties to InsightsQL expressions
        ...
        if math_type in [
            ...
            if math_insightsql is not None:
                ...
                if aggregation_function:
                    ...
                    # incompatible with Float64 in Datastore greatest/least functions used
