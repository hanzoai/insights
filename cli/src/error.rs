...
use anyhow::Error;
use insights_rs::CaptureExceptionOptions;
...
fn capture_exception(metadata: ErrorTelemetryMetadata) {
    ...
    if let Err(err) = insights_rs::capture_exception_with(&SanitizedCliError, options) {
...
mod tests {
    ...
    fn proxy_launch_errors_produce_structured_telemetry() {
        let error = anyhow::Error::new(ApiProxyError::MaterializeFailed {
            ...
            path: "/home/user/.insights/api-cli".to_string(),
        ...
        assert!(metadata.fingerprint().starts_with("insights-cli:"));
        assert!(metadata
            .fingerprint()
