...
export const webhookTabLogic = kea<webhookTabLogicType>([
    ...
    selectors({
        ...
        internalStateLabel: [
            ...
            (
                ...
                const state = webhookInfo?.insights_function?.status?.state
                switch (state) {
                    case 1:
    ...
    listeners(({ actions, props, values }) => ({
        loadWebhookInfoSuccess: ({ webhookInfo }) => {
            ...
            // (InsightsFunctionSerializer convention). Unwrap non-secret values and use them as
            ...
            // user actually edits a field. Mirrors the InsightsFunctions configuration logic
            // pattern (`loadInsightsFunctionSuccess: () => actions.resetForm()`).
            ...
            try {
                ...
                if (result.success) {
                    if ((result.pending_inputs?.length ?? 0) === 0) {
                        ...
                        toast.info('Webhook created — enter the remaining details below to finish setup')
                    }
                }
            } catch (e: any) {
        ...
        submitWebhookFields: async () => {
            ...
            try {
                ...
                toast.success('Webhook inputs saved')
        ...
        deleteWebhook: async () => {
            try {
                ...
                if (result.success) {
                    toast.success(
                        result.external_deleted
                            ? 'Webhook deleted from source and Insights'
                            : 'Webhook deleted from Insights'
                    )
                    if (result.error) {
                ...
                toast.error(e.data?.message ?? e.message ?? 'Failed to delete webhook')
            }
