...
resolved `KafkaProfileSettings`; `insights.kafka_client.routing` does exactly that.
"""
...
# bakes the host list into Datastore Kafka-engine DDL at table creation time —
# the six historical `insights/datastore/migrations/*` files plus
# `insights/models/person_overrides/sql.py`. Those sites render `ENGINE = Kafka(...)`
# and the produced DDL is stored in Datastore, so swapping them out now would
...
# Example: "datastore_precalculated_person_properties=warpstream_calculated_events"
