...
export interface MCPSessionApi {
    ...
    /** Number of distinct Insights distinct_ids that produced events in the session. */
...
export interface MCPToolCallApi {
    /** Datastore uuid of the $mcp_tool_call event. */
...
export type McpAnalyticsSessionsListParams = {
    /**
     * Start of the window to aggregate sessions over. Insights date string — relative (e.g. '-7d', '-24h') or an absolute ISO timestamp. Defaults to '-7d'.
     */
    date_from?: string
    /**
     * End of the window. Insights date string or absolute ISO timestamp. Defaults to now.
     */
    date_to?: string
    /**
