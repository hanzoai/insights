...
- **Validator:** - **Checked:** The PR diff for all files the finding names — `products/stamp/backend/facade/api.py`, `facade/inbox_hooks.py`, `backend/tasks/tasks.py` (`_review_skip_reason`), and `products/review_hog/backend/receivers.py` — plus the migration/model/engine/test files, via `gh pr diff 75215`.
