...
def receiver_setup_logging(loglevel, logfile, format, colorize, **kwargs) -> None:
    ...
    from insights.settings import logs
...
def on_worker_start(**kwargs) -> None:
    ...
    from insights.otel_instrumentation import (
...
def on_worker_process_shutdown(**kwargs) -> None:
    ...
    # Flush the hanzo_insights SDK's final metrics window: `client.metrics`
    ...
    import hanzo_insights  # noqa: PLC0415 — keep the SDK off the celery import path
        ...
        if metrics is not None:
            ...
            def _flush() -> None:
                try:
                    ...
                    logger.warning("hanzo_insights_metrics_flush_failed", exc_info=True)

            flush_thread = threading.Thread(target=_flush, name="hanzo_insights-metrics-flush", daemon=True)
            flush_thread.start()
            flush_thread.join(timeout=_ANALYTICS_METRICS_FLUSH_TIMEOUT_SECONDS)
            if flush_thread.is_alive():
                logger.warning("hanzo_insights_metrics_flush_timed_out")
    except Exception:
        logger.warning("hanzo_insights_metrics_flush_failed", exc_info=True)
