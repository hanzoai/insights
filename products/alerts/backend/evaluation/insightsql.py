...
from insights.schema import AlertCondition, AlertConditionType, InsightsQLAlertConfig, InsightsQLAlertEvaluation

from insights.insightsql.constants import MAX_SELECT_RETURNED_ROWS

from insights.api.services.query import ExecutionMode
from insights.caching.calculate_results import calculate_for_query_based_insight
from insights.event_usage import EventSource
from insights.tasks.alerts.detector import _compute_min_samples_for_detector
...
_INSIGHTSQL_SUBJECT = "The SQL insight value"
...
# frontend preview as ``INSIGHTSQL_ANY_ROW_MAX_ROWS`` (frontend/src/lib/components/Alerts/alertFormLogic.ts);
# keep the two in sync.
ANY_ROW_MAX_ROWS = 50
# last_row reads the tail, so a result truncated at InsightsQL's hard cap can't be trusted (the real
...
_DEFAULT_INSIGHTSQL_CONFIG = {"type": "InsightsQLAlertConfig", "evaluation": "last_row"}


def insightsql_config_or_default(raw: dict | None) -> InsightsQLAlertConfig:
    """Validate a stored SQL alert config, defaulting an absent one to last-row evaluation."""
    return InsightsQLAlertConfig.model_validate(raw or _DEFAULT_INSIGHTSQL_CONFIG)
...
def _check_row_caps(rows: list, evaluation: InsightsQLAlertEvaluation) -> None:
    """Fail loud on a result the evaluation mode can't trust. last_row reads the tail, which InsightsQL's
    ...
    """
    if evaluation == InsightsQLAlertEvaluation.ANY_ROW and len(rows) > ANY_ROW_MAX_ROWS:
    ...
    if evaluation == InsightsQLAlertEvaluation.LAST_ROW and len(rows) >= LAST_ROW_MAX_ROWS:
        raise AlertExtractionError(
...
class InsightsQLExtractor:
    """Normalize a InsightsQL/SQL-backed insight into ``ComparableSeries``.
    ...
    rules in TypeScript (``deriveInsightsQLAlertPreview`` in
    ...
    at evaluation time — but if you change any of these rules, update the mirror to match:
      ...
      6. any-row cap: ``ANY_ROW_MAX_ROWS`` (mirrored as ``INSIGHTSQL_ANY_ROW_MAX_ROWS``)
    ...
    def extract(
        ...
        config = insightsql_config_or_default(alert.config)
        ...
        if evaluation == InsightsQLAlertEvaluation.ANY_ROW:
            ...
            return ExtractionResult(
                series=series, is_breakdown=True, subject=_INSIGHTSQL_SUBJECT, framed=False, aggregate_breaches=True
            )

        # last_row / first_row: read the (previous, current) pair from the tail or head respectively.
        values = _anchor_values(rows, value_index, from_head=evaluation == InsightsQLAlertEvaluation.FIRST_ROW)
        ...
        label_index = _resolve_label_column_index(config.label_column, column_names, value_index, rows)
        anchor_row = rows[0] if evaluation == InsightsQLAlertEvaluation.FIRST_ROW else rows[-1]
...
def extract_insightsql_detector_series(
    insight: Insight,
    team: Any,
    config: InsightsQLAlertConfig,
    detector_config: dict[str, Any],
    *,
    execution_mode: ExecutionMode,
    user: Any = None,
) -> ExtractionResult:
    """Build the full ordered value series an anomaly detector scores from a SQL/InsightsQL insight.
    ...
    """
    if config.evaluation == InsightsQLAlertEvaluation.ANY_ROW:
        raise AlertExtractionError(
    ...
    if len(rows) == 0:
        return ExtractionResult(
            series=[], is_breakdown=False, subject=_INSIGHTSQL_SUBJECT, framed=False, empty_query_result=True
        )
    # last_row scores the tail as the current value, so the same truncation guard the threshold path
    # enforces applies here — a tail truncated at InsightsQL's hard cap would score the wrong "current" row.
    ...
    from_head = config.evaluation == InsightsQLAlertEvaluation.FIRST_ROW
    ...
    if len(values) < min_samples:
        return ExtractionResult(series=[], is_breakdown=False, subject=_INSIGHTSQL_SUBJECT, framed=False)
    ...
    points = [SeriesPoint(date=None, value=v) for v in values]
    single = ComparableSeries(label=series_label, points=points, current_index=len(points) - 1)
    return ExtractionResult(series=[single], is_breakdown=False, subject=_INSIGHTSQL_SUBJECT, framed=False)


class InsightsQLDetectorExtractor:
    """Detector-path extractor for SQL/InsightsQL insights — thin alert adapter over
    ``extract_insightsql_detector_series`` (which the read-only simulation also uses), mirroring how
    ...
    def extract(
        ...
        if not detector_config:
            raise ValueError("InsightsQLDetectorExtractor requires detector_config — dispatcher invariant violated")
        config = insightsql_config_or_default(alert.config)
        return extract_insightsql_detector_series(
    ...
    def simulate(self, insight: Insight, query: object, ctx: SimulationContext) -> tuple[ExtractionResult, str | None]:
        ...
        result = extract_insightsql_detector_series(
            insight,
            ctx.team,
            insightsql_config_or_default(ctx.config),
            ctx.detector_config,
...
def _numeric_cell(row: Any, index: int, *, position: str) -> float:
    ...
    if raw is None:
        return 0.0
    # Datastore Decimal columns surface as decimal.Decimal; accept them alongside int/float.
