...
describe('Sandbox approval input area', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks()
        window.localStorage.removeItem('insights-ai.lastPlanApprovalMode')
    ...
    describe('PermissionInput', () => {
        it('renders the one-voice card: headline sentence, tool identity, option rows', () => {
            ...
            expect(screen.getByText('insights - insight-create (MCP)')).toBeInTheDocument()
        ...
        it('renders the unwrapped Insights exec payload like Insights Desktop', () => {
            render(
                <PermissionInput
                    streamKey="conv-1"
                    request={makeRequest({
            ...
            expect(screen.getByText('insights - execute-sql (MCP)')).toBeInTheDocument()
            expect(
                ...
                <PermissionInput
                    ...
                    request={makeRequest({
                        rawToolCall: {
                            ...rawToolCall,
                            contentBlocks: [
                                { type: 'diff', path: 'source.script', oldText: 'let a := 1', newText: 'let a := 2' },
                            ],
                        },
                    })}
                />
        ...
        it('pre-selects the remembered last-approved mode over Auto', () => {
            window.localStorage.setItem('insights-ai.lastPlanApprovalMode', 'bypassPermissions')
            render(<PermissionInput streamKey="conv-1" request={makePlanRequest()} />)
        ...
        it('falls back to Auto when the remembered mode is retired', () => {
            window.localStorage.setItem('insights-ai.lastPlanApprovalMode', 'acceptEdits')
            render(<PermissionInput streamKey="conv-1" request={makePlanRequest()} />)
