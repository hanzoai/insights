...
from products.error_tracking.backend.insightsql_queries.error_tracking_query_runner_utils import search_tokenizer
...
class TestErrorTrackingQueryRunner(DatastoreTestMixin, NonAtomicBaseTestKeepIdentities):
    ...
    def override_fingerprint(self, fingerprint, issue_id, version=1):
        ...
        # reflect the new fingerprint -> issue mapping in the denormalized Datastore table
    ...
    def test_fingerprint_grouping_key_uses_stable_json_expression(self):
        ...
        self.assertIn("JSONExtractString(e.properties, '$exception_fingerprint')", response["insightsql"])
        self.assertNotIn("mat_$exception_fingerprint", response["insightsql"])
    ...
    def test_search_query_with_multiple_search_items(self):
        self.create_events_and_issue(
            ...
            additional_properties={
                ...
                "$exception_sources": "['insights/datastore/client/execute.py']",
            },
        )
        ...
        results = self._calculate(
            filterTestAccounts=True, searchQuery="databasenotfoundX datastore/client/execute.py", withAggregations=True
        )["results"]
    ...
    @freeze_time("2022-01-10T12:11:00")
    @snapshot_datastore_queries
    def test_insightsql_filters(self):
        results = self._calculate(
    ...
    def test_status(self):
        ...
        sync_issues_to_datastore(issue_ids=[self.issue_id_one], team_id=self.team.pk)
    ...
    def test_nested_filter_group_routes_issue_filters_to_issue_fields(self):
        ...
        user_filter_insightsql = user_filter_expr.to_insightsql()
        self.assertIn("e.issue_name", user_filter_insightsql)
        self.assertNotIn("properties.name", user_filter_insightsql)
