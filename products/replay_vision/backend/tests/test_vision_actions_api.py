...
class _VisionActionAPITestCase(APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
        # Creating an action provisions an internal_destination InsightsFunction, which resolves the
...
class TestVisionActionViewSet(_VisionActionAPITestCase):
    ...
    def test_promotion_blocked_when_current_digest_reads_a_restricted_scanner(self) -> None:
        ...
        # way around that check, and the restricted digest must stay put.
        hidden = self._create_scanner(name="restricted")
    ...
    def test_actions_flag_off_hides_endpoint(self) -> None:
        ...
        with patch("products.replay_vision.backend.feature_flag.hanzo_insights.feature_enabled", side_effect=_flags):
    ...
    def test_too_many_delivery_targets_rejected(self) -> None:
        # Each target provisions an enabled InsightsFunction that POSTs on every run, so an over-cap list
