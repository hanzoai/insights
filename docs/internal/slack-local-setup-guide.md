...
How to point a real Slack workspace at your local Insights so you can test the **Insights Desktop**
coding agent (`@Insights <task>` → sandbox run) end to end. The same tunnel + workspace works for
...
> Prerequisite: the Insights Desktop stack must already work locally — GitHub App, temporal
...
- Everything Slack needs — the event webhook, the interactivity callback, the OAuth
  redirect, and the Insights UI — is served by **Caddy on port 8010** (the dev proxy that
  `insightscli start` runs). Port 8000 is Django direct and does **not** serve the frontend.
...
- The OAuth `redirect_uri` is built **server-side** from `SITE_URL`, not from the browser
  address bar (`OauthIntegration.redirect_uri()` in `insights/models/integration.py`), and it
  force-upgrades the scheme to https — `SITE_URL.replace('http://', 'https://')`. `SITE_URL`
  defaults to `http://localhost:8010` (`insights/settings/__init__.py:69`), so at that default
...
tunnels:
  app:
    proto: http
    addr: 8010
    domain: <you>-insights.ngrok.dev # a domain you've reserved in the ngrok dashboard
...
```bash
curl -sS https://<you>-insights.ngrok.dev/_preflight | jq .
```
...
> reach Insights over the public URL and work fine through Caddy, so you do **not** need a
...
1. Create a throwaway Slack workspace (e.g. `insights-slack-dev-<you>`). You need admin to
   install apps.
...
```yaml
display_information:
  name: insights-slack-dev
features:
  bot_user:
    display_name: insights-slack-dev
    always_online: true
  app_home:
...
oauth_config:
  redirect_urls:
    - https://<you>-insights.ngrok.dev/integrations/slack/callback
    - https://<you>-insights.ngrok.dev/complete/slack-link/
  scopes:
    bot:
      - app_mentions:read
      - channels:read
...
settings:
  event_subscriptions:
    request_url: https://<you>-insights.ngrok.dev/slack/event-callback
    bot_events:
      - app_mention
      - app_home_opened
  interactivity:
    is_enabled: true
    request_url: https://<you>-insights.ngrok.dev/slack/interactivity-callback
...
Insights Desktop reuses the regular Slack notifications app, so set the standard
...
seeded from the env on boot (`insights/settings/dynamic_settings.py`):
...
```bash
curl -sS https://<you>-insights.ngrok.dev/_preflight | jq '.slack_service'
...
```bash
export SITE_URL=https://<you>-insights.ngrok.dev   # then (re)start the stack from this shell
```
...
connected, `@Insights` events reach the agent unconditionally.

## Step 5 — connect the integrations

**Slack.** Insights Desktop piggybacks on the regular Slack notifications install — there's no
separate "Insights Desktop Slack" install anymore. Go to **Settings → Project → Integrations**,
find Slack, click **Connect to Slack**, and authorize in your dev workspace. The Insights Desktop
agent reads the same `Integration` row that the notifications product writes. Verify:

```bash
python manage.py shell -c "from insights.models.integration import Integration; \
...
## Step 6 — Slack user → Insights user

`resolve_slack_user` matches your Slack profile email to a Insights org member. Locally this is
handled for you: a `DEBUG` branch (`products/slack_app/backend/api.py:417-419`) forces the email
to `test@hanzo.ai`, which is the default local dev user — so resolution works out of the box.
...
```text
/invite @insights-slack-dev
@insights-slack-dev add a comment to the README explaining how to run tests
```
...
- **Temporal Web** (`http://localhost:8233`, filter workflow type
  `insights-code-slack-mention-processing`) — shows which activity the run reached. The happy path
  ends at `create_insights_code_task_for_repo_activity`.
- Going deeper, the `django` / `temporal-django-worker` logs carry `insights_code_*` structlog
