...
class TestAttachmentsService(SimpleTestCase):
    def test_build_content_renders_files_as_links(self) -> None:
        files = [{"url": "https://app.hanzo.ai/uploaded_media/a", "name": "invoice.pdf"}]
        content, rich_content = build_content_with_images("hello", None, [], files)

        assert "[invoice.pdf](https://app.hanzo.ai/uploaded_media/a)" in content
    ...
    def test_build_content_renders_images_and_files_together(self) -> None:
        images = [{"url": "https://app.hanzo.ai/uploaded_media/img", "name": "shot.png"}]
        files = [{"url": "https://app.hanzo.ai/uploaded_media/doc", "name": "doc.pdf"}]
        content, rich_content = build_content_with_images("", None, images, files)

        assert "![shot.png](https://app.hanzo.ai/uploaded_media/img)" in content
        assert "[doc.pdf](https://app.hanzo.ai/uploaded_media/doc)" in content
        assert rich_content is not None
