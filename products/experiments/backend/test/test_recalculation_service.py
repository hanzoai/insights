...
class TestLiveQueryProgress(BaseTest):
    ...
    def test_returns_none_without_querying_datastore_when_not_in_progress(self, status: str):
    ...
    def test_returns_none_without_querying_datastore_when_team_context_mismatches(self):
    ...
    def test_returns_none_when_datastore_read_raises(self):
        # The live read is best-effort on the poll's hot path: a Datastore hiccup must degrade to None, never
...
@pytest.mark.django_db(transaction=True)
class TestLiveQueryProgressFinishedQueries(BaseTest, DatastoreTestMixin):
