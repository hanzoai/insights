...
impl ExceptionEvent<Finalized> {
    pub fn into_datastore_properties(self) -> Value {
        let ExceptionEvent {
            exception_list,
            debug_images,
            props,
...
impl TryFrom<DatastoreEvent> for ExceptionEvent<Parsed> {
    type Error = EventError;
...
impl TryFrom<Result<DatastoreEvent, EventError>> for ExceptionEvent<Parsed> {
    type Error = EventError;

    fn try_from(event: Result<DatastoreEvent, EventError>) -> Result<Self, Self::Error> {
        event?.try_into()
    }
}
