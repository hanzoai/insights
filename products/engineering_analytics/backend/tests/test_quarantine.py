...
# The exact bytes core.render (and therefore `insightscli test:quarantine add`) writes for one
...
def _entry(**overrides: Any) -> dict[str, Any]:
    raw: dict[str, Any] = {
        "id": "insights/api/test/test_foo.py::TestFoo::test_bar",
        "runner": "pytest",
        "reason": "flaky",
        "owner": "@Insights/team-foo",
        "issue": "https://github.com/Insights/insights/issues/1",
...
def _github_mock(**overrides: Any) -> mock.Mock:
    github = mock.Mock()
    github.organization.return_value = "Insights"
    github.get_default_branch.return_value = "master"
    github.get_file_contents.return_value = None
    github.create_issue.return_value = {"number": 4242, "repository": "insights"}
    ...
    github.create_pull_request.return_value = {
        "success": True,
        "pr_url": "https://github.com/Insights/insights/pull/99",
    }
...
def _request(**overrides: Any) -> contracts.QuarantineRequest:
    fields: dict[str, Any] = {
        ...
        "selector": "insights/api/test/test_foo.py::TestFoo::test_bar",
...
class TestQuarantineParse(TestCase):
    def test_parses_valid_file_and_applies_defaults(self) -> None:
        full = _entry()
        minimal = {"id": "insights/api/test", "added": "2026-06-01", "expires": "2026-06-09"}
        ...
        assert by_id[full["id"]].issue == "https://github.com/Insights/insights/issues/1"
        assert by_id[full["id"]].days_until_expiry == 8
        defaulted = by_id["insights/api/test"]
    ...
    def test_malformed_entry_dropped_while_good_ones_kept(self, _name: str, bad: Any, message: str) -> None:
        good = _entry(id="insights/api/test/test_ok.py")
        entries, errors, warnings = parse_quarantine_text(_text(bad, good), _TODAY)

        assert [entry.id for entry in entries] == ["insights/api/test/test_ok.py"]
        assert len(errors) == 1 and message in errors[0]
        assert warnings == []
    ...
    @parameterized.expand(
        [
            ...
            ("insights/api/test/test_foo.py::TestFoo::test_bar", contracts.QuarantineSelectorKind.TEST),
            ("insights/api/test/test_foo.py::TestFoo", contracts.QuarantineSelectorKind.TEST),
            ("insights/api/test/test_foo.py", contracts.QuarantineSelectorKind.FILE),
            ("frontend/src/scenes/dashboard/dashboardLogic.test.ts", contracts.QuarantineSelectorKind.FILE),
            ("insights/api/test", contracts.QuarantineSelectorKind.DIRECTORY),
        ]
    )
...
class TestQuarantineBuild(BaseTest):
    ...
    def test_fetches_and_parses_remote_file(self) -> None:
        with mock.patch(_REQUESTS_GET, return_value=_response(200, _text(_entry()))) as get:
            result = build_quarantine(team=self.team, repo="Insights/insights")

        assert result.available is True
        assert result.repo == contracts.RepoRef(provider="github", owner="Insights", name="insights")
        assert result.source_url == "https://github.com/Insights/insights/blob/HEAD/.test_quarantine.json"
        ...
        get.assert_called_once_with(
            "https://raw.githubusercontent.com/Insights/insights/HEAD/.test_quarantine.json", timeout=3, stream=True
        )
    ...
    def test_caches_fetched_text(self) -> None:
        with mock.patch(_REQUESTS_GET, return_value=_response(200, _text(_entry()))) as get:
            build_quarantine(team=self.team, repo="Insights/insights")
            result = build_quarantine(team=self.team, repo="Insights/insights")
    ...
    def test_oversized_streamed_body_rejected_and_not_cached(self) -> None:
        ...
        with mock.patch(_MAX_BYTES, 16), mock.patch(_REQUESTS_GET, return_value=_response(200, _text(_entry()))) as get:
            first = build_quarantine(team=self.team, repo="Insights/insights")
            second = build_quarantine(team=self.team, repo="Insights/insights")
    ...
    def test_resolves_most_active_repo_from_workflow_runs(self) -> None:
        source = _curated_source([("Insights", "hanzo.ai")])
        with (
        ...
        assert result.repo == contracts.RepoRef(provider="github", owner="Insights", name="hanzo.ai")
...
class TestQuarantineAPI(APIBaseTest):
    ...
    def test_quarantine_serializes(self) -> None:
        with mock.patch(_REQUESTS_GET, return_value=_response(200, _text(_entry()))):
            response = self.client.get(self._url(), {"repo": "Insights/insights"})

        assert response.status_code == status.HTTP_200_OK
        body = response.json()
        assert body["available"] is True
        assert body["repo"] == {"provider": "github", "owner": "Insights", "name": "insights"}
        assert body["source_url"] == "https://github.com/Insights/insights/blob/HEAD/.test_quarantine.json"
        entry = body["entries"][0]
        assert entry["id"] == "insights/api/test/test_foo.py::TestFoo::test_bar"
    ...
    def test_quarantine_available_false_when_repo_has_no_file(self) -> None:
        with mock.patch(_REQUESTS_GET, return_value=_response(404)):
            response = self.client.get(self._url(), {"repo": "Insights/no-quarantine"})
    ...
    def test_repo_param_passes_through_to_facade(self) -> None:
        ...
        with mock.patch(f"{_VIEWS}.get_quarantine", return_value=empty) as get:
            response = self.client.get(self._url(), {"repo": "Insights/hanzo.ai"})

        assert response.status_code == status.HTTP_200_OK
        assert get.call_args.kwargs["repo"] == "Insights/hanzo.ai"
...
class TestQuarantineRequest(BaseTest):
    def _install(self, github: mock.Mock, *, has_integration: bool = True, connected: bool = True) -> mock.Mock:
        ...
        sources = [SimpleNamespace(repo="Insights/insights")] if connected else []
    ...
    def test_quarantine_opens_issue_then_pr_and_writes_canonical_entry(self) -> None:
        ...
        assert result.issue_url == "https://github.com/Insights/insights/issues/4242"
        assert result.pr_url == "https://github.com/Insights/insights/pull/99"
        ...
        assert _request().selector in issue_config["title"] and issue_config["repository"] == "insights"
        ...
        assert entry["issue"] == "https://github.com/Insights/insights/issues/4242"
    ...
    def test_extend_reuses_existing_issue_and_files_no_new_one(self) -> None:
        existing = _text(_entry(expires="2026-06-15", issue="https://github.com/Insights/insights/issues/7"))
        ...
        result = request_quarantine(
            ...
            request=_request(
                ...
                selector=_entry()["id"],
                issue="https://github.com/Insights/insights/issues/7",
        ...
        assert entry["issue"] == "https://github.com/Insights/insights/issues/7"
    ...
    def test_explicit_repo_outside_the_team_is_rejected_before_any_write(self) -> None:
        ...
        with self.assertRaises(contracts.QuarantineWriteError):
            request_quarantine(team=self.team, request=_request(repo="Insights/not-ours"))
        github.create_branch.assert_not_called()
        github.create_issue.assert_not_called()
...
class TestQuarantineRequestAPI(APIBaseTest):
    ...
    def test_returns_201_with_pr_and_issue_links(self) -> None:
        result = contracts.QuarantineRequestResult(
            pr_url="https://github.com/Insights/insights/pull/99",
            issue_url="https://github.com/Insights/insights/issues/4242",
    ...
    def test_write_error_becomes_400_with_detail(self) -> None:
        with mock.patch(
            ...
            side_effect=contracts.QuarantineWriteError("The connected GitHub App is installed on 'X', not 'Insights'."),
        ):
            response = self.client.post(self._url(), self._body(), format="json")

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "not 'Insights'" in response.json()["detail"]
