...
from insights.tasks.tasks import find_flags_with_enriched_analytics as find_flags_with_enriched_analytics_task
...
class TestFeatureFlagAnalytics(BaseTest, QueryMatchingTest):
    ...
    def test_increment_request_count_remote_config_uses_own_bucket(self):
        ...
        with freeze_time("2022-05-07 12:23:07"):
            ...
            self.assertEqual(
                client.hgetall(f"insights:remote_config_requests:{team_id}"),
                {b"165192618": b"6"},
            )
...
class TestSdkBreakdown(BaseTest):
    ...
    def test_get_team_request_library_key_local_evaluation(self):
        self.assertEqual(
            get_team_request_library_key(123, FlagRequestType.LOCAL_EVALUATION, "insights-python"),
            "insights:local_evaluation_requests:sdk:123:insights-python",
        )
    ...
    def test_capture_local_evaluation_usage_includes_sdk_breakdown(self):
        ...
        with (
            ...
            client.hincrby(f"insights:local_evaluation_requests:sdk:{team_id}:insights-python", time_bucket_1, 50)
            ...
            client.hincrby(f"insights:local_evaluation_requests:sdk:{team_id}:insights-python", time_bucket_2, 1)
            ...
            mock_capture.capture.assert_called_once_with(
                ...
                properties={
                    ...
                    "max_time": 1651926180,
                    "token": "token",
                    "sdk_breakdown": {"insights-python": 50, "insights-node": 30},
                },
            )
...
class TestFindFlagsWithEnrichedAnalyticsTask(BaseTest):
    ...
    def test_logs_and_reraises_on_failure(self, mock_find_flags: MagicMock) -> None:
        ...
        with patch("insights.tasks.tasks.capture_exception") as mock_capture, self.assertRaises(Exception):
            find_flags_with_enriched_analytics_task()

        mock_capture.assert_called_once()


class TestCrossProjectEvaluations(DatastoreTestMixin, APIBaseTest):
    ...
    def test_returns_none_when_datastore_fails(self):
        with patch(
...
class TestCachedCrossProjectEvaluations(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
        cache.clear()
