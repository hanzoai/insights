...
from ee.hogai.chat_agent.sql.mixins import InsightsQLOutputParserMixin
...
class ExecuteSQLMCPTool(InsightsQLOutputParserMixin, MCPTool[ExecuteSQLMCPToolArgs]):
    """
    MCP version of ExecuteSQLTool.

    Executes InsightsQL queries without LangChain context or artifact creation.
    """
    ...
    async def execute(self, args: ExecuteSQLMCPToolArgs) -> str:
        try:
            validated_query = await self._validate_insightsql_query(args.query)
