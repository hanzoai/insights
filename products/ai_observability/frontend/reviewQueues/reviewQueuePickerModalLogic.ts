...
export interface reviewQueuePickerModalLogicValues {
    ...
    selectedQueueValue: string[] | null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface reviewQueuePickerModalLogicActions {
    loadQueues: () => {
        value: true
    }
    loadQueuesFailure: (
    ...
    setSelectedQueueKey: (queueKey: string | null) => {
        queueKey: string | null
    }
    submit: (queueKey?: string | null) => {
        queueKey: string | null
    }
    submitFailure: () => {
        value: true
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface reviewQueuePickerModalLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        selectedQueueValue: (selectedQueueKey: string | null) => string[] | null
        selectedQueue: (queues: PaginatedReviewQueueListApi, selectedQueueKey: string | null) => ReviewQueueApi | null
        isCreatingQueue: (selectedQueueKey: string | null, selectedQueue: ReviewQueueApi | null) => boolean
    }
}

export type reviewQueuePickerModalLogicType = MakeLogicType<
    reviewQueuePickerModalLogicValues,
    reviewQueuePickerModalLogicActions,
    ReviewQueuePickerModalProps,
    reviewQueuePickerModalLogicMeta
>

export const reviewQueuePickerModalLogic = kea<reviewQueuePickerModalLogicType>([
    path(['products', 'ai_observability', 'frontend', 'reviewQueues', 'reviewQueuePickerModalLogic']),
    ...
    listeners(({ actions, values, props }) => ({
        ...
        submit: async ({ queueKey }) => {
            ...
            if (!selectedQueueKey) {
                ...
                toast.error('Select a queue or type a new queue name.')
                return
            }
            ...
            try {
                ...
                if (props.queueItemId) {
                    ...
                    toast.success(`Added to "${queueName}".`)
                }

                actions.submitSuccess(queueId, createdQueue)
            } catch (error) {
                actions.submitFailure()
                toast.error(getApiErrorDetail(error) || 'Failed to update the trace queue.')
            }
        },
