...
class TestConversionGoalProcessorRefactor(BaseTest):
    ...
    Snapshot tests pin the actual InsightsQL SQL of _generate_funnel_query so changes in the
    ...
    def _print_sql(self, node: ast.SelectQuery) -> str:
        context = InsightsQLContext(team=self.team, enable_select_queries=True)
        sql, _ = prepare_and_print_ast(node, context=context, dialect="insightsql")
    ...
    def test_precompute_skipped_when_materialized_property_restricted(
        ...
        # InsightsQL applies to events.properties. When one is restricted for the user, eligibility must fail
        ...
        target = (
            "products.marketing_analytics.backend.insightsql_queries.conversion_goal_processor.get_restricted_property_names"
        )
    ...
    def test_tracked_fields_match_touchpoints_table_schema(self):
        ...
        from products.marketing_analytics.backend.insightsql_queries.conversion_goal_processor import TRACKED_FIELDS
