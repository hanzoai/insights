...
class TestStripProperties:
    @parameterized.expand(
        [
            ...
            (
                "insightsql session.properties reference is stripped",
                InsightsQLPropertyFilter(key="session.properties.$channel_type = 'Direct'"),
            ),
        ]
    )
    ...
    def test_strip_preserves_truly_unexpected_properties(self) -> None:
        unexpected = InsightsQLPropertyFilter(key="some_unrelated_insightsql_expression = 1")
    ...
    def test_strip_removes_person_event_group_cohort_but_keeps_unknown(self) -> None:
        ...
        unexpected = InsightsQLPropertyFilter(key="unrelated = 1")
    ...
    def test_is_session_property_matches_type_and_insightsql(self) -> None:
        assert is_session_property(
            SessionPropertyFilter(key="$entry_pathname", operator=PropertyOperator.EXACT, value="/")
        )
        assert is_session_property(InsightsQLPropertyFilter(key="session.properties.$channel_type = 'Direct'"))
