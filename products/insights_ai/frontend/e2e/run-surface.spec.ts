...
// A persisted `_insights/error` frame — the synthetic backend/agent error the run log carries; `foldLogToThread`
// folds it into an inline error card titled "Agent error".
function insightsErrorFrame(message: string): AcpFrame {
    return { type: 'notification', notification: { method: '_insights/error', params: { message } } }
}
...
function makeTask(status: string): Record<string, unknown> {
    return {
        ...
        created_at: '2026-07-01T00:00:00Z',
        updated_at: '2026-07-01T00:00:00Z',
        created_by: { id: 1, uuid: 'user-1', distinct_id: 'user-1', first_name: 'Test', email: 'test@hanzo.ai' },
    }
}
...
async function openRunDeepLink(page: Page, teamId: string): Promise<void> {
    ...
    // The fresh workspace has no tasks flag; force insights-js to re-fetch so the mocked flag lands and the
    // scene's `FEATURE_FLAGS.TASKS` gate opens.
    await page.evaluate(() => {
        const ph = (window as unknown as { insights?: { reloadFeatureFlags?: () => void } }).insights
...
test.describe('Task run surface', () => {
    ...
    test('terminal replay surfaces the agent error inline', async ({ page }) => {
        // Regression: a terminal run's persisted `_insights/error` frame must fold into the inline "Agent error"
        ...
        await routeTasksApi(page, {
            ...
            logs: {
                ...
                body: toJsonl([
                    ...
                    insightsErrorFrame('The agent hit an unexpected error'),
                ]),
            },
            stream: { mode: 'hang' },
        })
