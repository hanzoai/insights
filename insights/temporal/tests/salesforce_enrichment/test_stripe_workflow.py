...
from insights.temporal.salesforce_enrichment.stripe_workflow import (
...
WORKFLOW_MODULE = "insights.temporal.salesforce_enrichment.stripe_workflow"
...
class TestEnrichStripePageActivity(SimpleTestCase):
    ...
    async def test_happy_path(
        ...
        mock_sf.query_all.return_value = {
            "records": [
                {"Id": "001ABC", "Insights_Org_ID__c": "org-1"},
                {"Id": "001DEF", "Insights_Org_ID__c": "org-2"},
            ]
        }
    ...
    async def test_skips_rows_with_no_matching_sfdc_account(
        ...
        mock_sf = MagicMock()
        mock_sf.query_all.return_value = {"records": [{"Id": "001ABC", "Insights_Org_ID__c": "org-1"}]}
        mock_sf_client.return_value = mock_sf
    ...
    async def test_soql_lookup_chunks_large_pages(
        ...
        mock_sf = MagicMock()
        mock_sf.query_all.return_value = {
            "records": [{"Id": f"001{i:04d}", "Insights_Org_ID__c": f"org-{i}"} for i in range(250)]
        }
        mock_sf_client.return_value = mock_sf
...
class TestWorkflowRun(SimpleTestCase):
    ...
    async def test_force_full_refresh_skips_read_but_commits(self, mock_workflow):
        ...
        assert commit_inputs.insights_organization_id == "org-9"
        assert result["committed_watermark_ts"] == "2026-04-12T00:00:00+00:00"
    ...
    async def test_failure_after_successful_page_freezes_watermark(self, mock_workflow):
        ...
        assert commit_inputs.insights_organization_id == "org-last-good"
