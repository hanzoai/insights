...
test.describe('Before Onboarding', () => {
    ...
    test('Navigate to a settings page even when a product has not been set up', async ({ page }) => {
        ...
        await expect(page.locator('.scene-tab-row .scene-tab-title')).toContainText('Settings - Profile')

        await page.goto('/settings/organization')
        await expect(page.locator('.scene-tab-row .scene-tab-title')).toContainText('Settings - General')
    })
})
