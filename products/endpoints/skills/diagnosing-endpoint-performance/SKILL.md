...
description: >
  Diagnose why a Insights endpoint is slow or expensive and propose a concrete fix — bump the cache
...
`endpoint-materialization-suggestion`. Insights rewrites the query into a semantically equivalent
...
- **InsightsQL with `*` / non-deterministic functions** → narrow the columns selected, replace
...
Agent steps:
- endpoint-get monthly_active_users → InsightsQL endpoint, data_freshness_seconds=900,
...
- **Tell Insights what's missing.** If the diagnosis runs into a product limitation (an eligibility
