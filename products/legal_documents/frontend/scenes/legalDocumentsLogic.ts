...
export interface legalDocumentsLogicValues {
    ...
    isLegalDocumentValid: boolean
    isOnEnterpriseStandardTrial: boolean
    isOnQualifyingAddonTrial: boolean
    legalDocument: LegalDocumentFormValues
    legalDocumentAllErrors: Record<string, any>
    legalDocumentChanged: boolean
    legalDocumentErrors: DeepPartialMap<LegalDocumentFormValues, ValidationErrorType>
    legalDocumentHasErrors: boolean
    legalDocumentManualErrors: Record<string, any>
    legalDocumentTouched: boolean
    legalDocumentTouches: Record<string, boolean>
    legalDocumentValidationErrors: DeepPartialMap<LegalDocumentFormValues, ValidationErrorType>
    legalDocuments: LegalDocument[]
    legalDocumentsLoading: boolean
    showLegalDocumentErrors: boolean
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface legalDocumentsLogicActions {
    ...
    loadLegalDocuments: () => any
    loadLegalDocumentsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    resetLegalDocument: (values?: LegalDocumentFormValues) => {
        values?: LegalDocumentFormValues
    }
    setDeletingId: (id: string | null) => {
        id: string | null
    }
    setDocumentType: (documentType: LegalDocumentType) => {
        documentType: LegalDocumentType
    }
    setDpaMode: (dpaMode: DPAMode) => {
        dpaMode: DPAMode
    }
    setLegalDocumentManualErrors: (errors: Record<string, any>) => {
    ...
    setLegalDocumentValues: (values: DeepPartial<LegalDocumentFormValues>) => {
        values: DeepPartial<LegalDocumentFormValues>
    }
    submitLegalDocument: () => {
        value: boolean
    }
    submitLegalDocumentFailure: (
        error: Error,
        errors: Record<string, any>
    ) => {
        error: Error
    ...
    submitLegalDocumentRequest: (legalDocument: LegalDocumentFormValues) => {
        legalDocument: LegalDocumentFormValues
    }
    submitLegalDocumentSuccess: (legalDocument: LegalDocumentFormValues) => {
        legalDocument: LegalDocumentFormValues
    }
    touchLegalDocumentField: (key: string) => {
        key: string
    }
}
...
export type legalDocumentsLogicType = MakeLogicType<
    legalDocumentsLogicValues,
    legalDocumentsLogicActions,
    Record<string, any>,
    legalDocumentsLogicMeta
>
    ...
    listeners(({ values, actions }) => ({
        ...
        deleteLegalDocument: async ({ id, documentType }) => {
            ...
            try {
                ...
                actions.loadLegalDocuments()
                toast.success(`${documentType} deleted. You can now generate a new ${documentType}.`)
            } catch (error: any) {
                ...
                toast.error(
                    error?.detail ||
                        `Could not delete the ${documentType}. Please try again, or contact Insights support.`
                )
            } finally {
