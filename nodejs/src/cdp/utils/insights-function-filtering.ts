...
const insightsFunctionFilterDuration = new Histogram({
    name: 'cdp_insights_function_filter_duration_ms',
...
const insightsFunctionFilterOutcomes = new Counter({
    name: 'cdp_insights_function_filter_outcome',
    help: 'Count of filter outcomes',
    labelNames: ['result', 'result_type'],
})

const insightsFunctionPreFilterCounter = new Counter({
    name: 'cdp_insights_function_prefilter_result',
...
export function convertClickhouseRawEventToFilterGlobals(event: RawClickHouseEvent): InsightsFunctionFilterGlobals {
    ...
    let pdi: InsightsFunctionFilterGlobals['pdi'] = null
    ...
    const response: InsightsFunctionFilterGlobals = {
        event: event.event,
        uuid: event.uuid,
    ...
    groupProperties.forEach((groupPropsString, index) => {
        if (groupPropsString) {
            ...
            const groupKey = `group_${index}` as keyof InsightsFunctionFilterGlobals
    ...
    for (let i = 0; i < 5; i++) {
        const groupIdKey = `$group_${i}` as keyof InsightsFunctionFilterGlobals
...
export function convertToInsightsFunctionFilterGlobal(
    globals: Pick<InsightsFunctionInvocationGlobals, 'event' | 'person' | 'groups' | 'variables'>
): InsightsFunctionFilterGlobals {
...
export async function filterFunctionInstrumented(options: {
    ...
    try {
        ...
        if (filters && Object.keys(filters).length === 1 && 'bytecode' in filters) {
            insightsFunctionPreFilterCounter.inc({ result: 'bytecode_execution_skipped__no_filters' })
            result.match = true
            return result
        }
        ...
        if (filters?.events?.length && !filters?.actions?.length) {
            ...
            if (preFilterMatch === false) {
                insightsFunctionPreFilterCounter.inc({ result: 'bytecode_execution_skipped__pre_filtered_out' })
                result.match = false
                metrics.push({
                    team_id: fn.team_id,
                    app_source_id: fn.id,
        ...
        const execFnOutcome = await execFn(filters.bytecode, { globals: filterGlobals })
        ...
        if (execFnOutcome.durationMs > HOG_FILTERING_TIMEOUT_MS) {
            logger.error('🦔', `[${fnKind}] Filter took longer than expected`, {
                ...
                teamId: fn.team_id,
                duration: execFnOutcome.durationMs,
                eventId: filterGlobals.uuid,
            })
        }

        execResult = execFnOutcome.execResult
        ...
        insightsFunctionFilterOutcomes.inc({
            result: JSON.stringify(execFnOutcome.execResult.result),
            result_type: typeof execFnOutcome.execResult.result,
        })

        result.match = typeof execFnOutcome.execResult.result === 'boolean' && execFnOutcome.execResult.result
        ...
        logs.push({
            team_id: fn.team_id,
            log_source: fnKind === 'InsightsFunction' ? 'insights_function' : 'insights_flow',
            log_source_id: fn.id,
