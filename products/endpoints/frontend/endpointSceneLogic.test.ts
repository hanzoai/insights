...
describe('endpointSceneLogic', () => {
    ...
    describe('materialization suggestion', () => {
        const insightsqlEndpoint = {
            ...endpoint,
            query: { kind: 'InsightsQLQuery', query: 'SELECT count() FROM events WHERE 1 = 1 OR 0 = 1' },
        }
        const suggestion = {
        ...
        it('applies the suggestion to the latest-version editor even when the cached editor tab is stale', async () => {
            logic.actions.loadEndpointSuccess(insightsqlEndpoint)
            await expectLogic(logic).toFinishAllListeners()
