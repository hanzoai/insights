...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "insights/insightsql/test/test_resolver.py",
            "insights.insightsql.test.test_resolver.TestResolver",
            "test_x",
            "insights/insightsql/test/test_resolver.py::TestResolver::test_x",
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("junit-results-insightscli", ("insightscli", "insightscli", None)),
    ],
)
...
def test_collect_jest_shard_marks_tolerated_failures_as_quarantined(tmp_path: Path) -> None:
    ...
    (artifact_dir / "insights-jest-quarantine-123.jsonl").write_text(
...
def test_load_jest_quarantine_signals_rejects_oversized_artifacts(
    ...
    (tmp_path / "insights-jest-quarantine-123.jsonl").write_text('{"test_id":"too-large"}\n')
...
# ---------- rerun classification (insights.reruns testcase property) ----------


@pytest.mark.parametrize(
    "testcase_xml,expected",
    [
        # pytest 8's junitxml drops rerun attempts entirely; the insights-junit-timings
        # plugin records them as a testcase property — the only rerun signal we get.
        (
            '<testcase name="t"><properties><property name="insights.reruns" value="2"/></properties></testcase>',
            ("rerun_passed", 3),
        ),
        # A rerun count must not mask a test that exhausted its retries and failed.
        (
            '<testcase name="t"><properties><property name="insights.reruns" value="2"/></properties>'
            '<failure message="x"/></testcase>',
            ("failed", 3),
        ),
        # Malformed value must never crash the exporter.
        (
            '<testcase name="t"><properties><property name="insights.reruns" value="garbage"/></properties></testcase>',
            ("passed", 1),
        ),
...
# ---------- setup_seconds (insights-junit-timings plugin) ----------
...
def test_parse_shard_shifts_tests_past_setup_seconds(tmp_path: Path) -> None:
    _write_shard_xml(
        ...
        timestamp="2026-05-04T10:00:00",
        time="10.0",
        properties={"insights.setup_seconds": "3.5", "insights.collection_seconds": "0.4"},
        body="""\
...
def test_parse_shard_handles_malformed_or_out_of_range_setup_seconds(
    ...
    _write_shard_xml(
        ...
        timestamp="2026-05-04T10:00:00",
        time="5.0",
        properties={"insights.setup_seconds": raw_value},
        body='<testcase classname="pkg.t.T" name="test_a" time="1.0"/>',
    )
...
def test_emit_shard_span_emits_setup_span_when_setup_seconds_positive(monkeypatch: pytest.MonkeyPatch) -> None:
    """When the insights-junit-timings plugin reported setup_seconds, a sibling `setup`
...
def test_workflow_resource_attributes_includes_query_and_drilldown_fields(
    ...
    monkeypatch.setenv("GITHUB_REPOSITORY", "Insights/insights")
    ...
    assert attrs["ci.repository"] == "Insights/insights"
    assert attrs["ci.run_url"] == "https://github.com/Insights/insights/actions/runs/25218527467"
