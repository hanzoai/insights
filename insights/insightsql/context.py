...
if TYPE_CHECKING:
    from insights.schema import DataWarehouseSyncWarning, InsightsQLNotice, InsightsQLQueryModifiers
    ...
    from insights.datastore.client.execute import DatastoreExternalTable
...
def _default_modifiers() -> "InsightsQLQueryModifiers":
    # Deferred: insights.schema (the pydantic models) stays off django.setup(); this module
...
class InsightsQLContext:
    ...
    # SECURITY-SENSITIVE: bypass for InsightsQL access control on warehouse tables.
    ...
    # Set when the query executes against an external direct-SQL connection instead of Insights's own cluster
    is_direct_query: bool = False
    # If set, will save string constants to this dict. Inlines strings into the query if None.
    values: dict = field(default_factory=dict)
    # Query-scoped Datastore external data tables accumulated during printing (keyed by table name).
    ...
    external_tables: dict[str, "DatastoreExternalTable"] = field(default_factory=dict, compare=False, repr=False)
    ...
    # Per-query data that query runners want to ingest into the InsightsQL resolution (e.g. pending updates
    ...
    modifiers: "InsightsQLQueryModifiers" = field(default_factory=_default_modifiers)
    ...
    # Optional per-query InsightsQL type-system observability accumulator.
    type_observability: Optional["InsightsQLTypeObservability"] = None
    ...
    # insights/insightsql/database/schema/information_schema.py). A dict keyed by the pushed-down table
    ...
    # Cohort-gated events data retention: when set, the Datastore printer floors every events-table scan to
    ...
    # InsightsQLQueryModifier, so a query can't disable enforcement.
    ...
    def uses_new_events_schema(self) -> bool:
        if self.use_new_events_schema is None:
            # Deferred: keeps insights.models off this module's import path (see _default_modifiers).
            from insights.models.event.new_events_schema import use_new_events_schema  # noqa: PLC0415
    ...
    def add_notice(
        ...
        if not any(n.start == start and n.end == end and n.message == message and n.fix == fix for n in self.notices):
            from insights.schema import InsightsQLNotice  # noqa: PLC0415
