...
// The livestream service matches against the raw event payload only, so person, session and cohort
// filters can never be evaluated there. Operators mirror the set compiled in livestream/events/filter.go.
export const LIVE_STREAM_OPERATORS: PropertyOperator[] = [
...
const LIVE_STREAM_OPERATOR_SET = new Set<PropertyOperator>(LIVE_STREAM_OPERATORS)

export const isLiveStreamFilter = (filter: AnyPropertyFilter): filter is EventPropertyFilter =>
    filter.type === PropertyFilterType.Event && !!filter.operator && LIVE_STREAM_OPERATOR_SET.has(filter.operator)

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface webAnalyticsFilterLogicValues {
    rawAuthorizedUrls: string[] // authorizedUrlListLogic
    ...
    liveStreamFilters: WebAnalyticsPropertyFilter[]
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface webAnalyticsFilterLogicActions {
    clearFilters: () => {
        value: true
    }
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface webAnalyticsFilterLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        selectedHost: (validatedDomainFilter: string | null) => string | null
        ...
        liveStreamFilters: (liveFilters: WebAnalyticsPropertyFilter[]) => WebAnalyticsPropertyFilter[]
    }
}

export type webAnalyticsFilterLogicType = MakeLogicType<
    webAnalyticsFilterLogicValues,
    webAnalyticsFilterLogicActions,
    Record<string, any>,
    webAnalyticsFilterLogicMeta
>
...
export const webAnalyticsFilterLogic = kea<webAnalyticsFilterLogicType>([
    ...
    selectors({
        hasHostFilter: [
            (s) => [s.rawWebAnalyticsFilters],
            (filters: WebAnalyticsPropertyFilters) => filters.some((f) => f.key === '$host'),
        ],
        authorizedDomains: [
        ...
        // Everything the live dashboard's InsightsQL backfill can evaluate, which is every filter type.
        liveFilters: [
            ...
            (
                ...
                const filters: WebAnalyticsPropertyFilter[] = [...rawWebAnalyticsFilters]
                if (host) {
        ...
        // The narrower subset the SSE stream can apply on its own.
        liveStreamFilters: [
            (s) => [s.liveFilters],
            (liveFilters: WebAnalyticsPropertyFilter[]): WebAnalyticsPropertyFilter[] =>
                liveFilters.filter(isLiveStreamFilter),
        ],
    }),
    listeners(({ values }) => ({
