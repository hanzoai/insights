...
import { escapeInsightsQLString, escapePropertyAsInsightsQLIdentifier } from '~/queries/utils'
...
// at all. Same fix as insights/insightsql/property.py wrapping NOT_REGEX in ifNull(..., 1).
...
export function customPropertyFilterToExpression(
    ...
    // The key round-trips through saved views and the URL hash before landing in raw InsightsQL,
    // so only ever interpolate a validated UUID (the shared escaper is defense in depth).
    if (!filter.key || !isUUIDLike(filter.key)) {
        return null
    }
    const column = `accounts.custom_properties.values.${escapePropertyAsInsightsQLIdentifier(filter.key)}`
    ...
    switch (operator) {
        ...
        case PropertyOperator.IsNot: {
            ...
            const literals =
                propertyType === PropertyType.Boolean
                    ...
                    : values.map((value) => escapeInsightsQLString(String(value)))
        ...
        case PropertyOperator.IsDateAfter:
            // parseDateTimeBestEffort is the InsightsQL name (it maps to the Datastore
            // ...OrNull variant); the OrNull-suffixed name isn't registered in InsightsQL.
            return `parseDateTimeBestEffort(${column}) ${
                DATE_OPERATOR_SYMBOLS[operator]
            } parseDateTimeBestEffort(${escapeInsightsQLString(String(values[0]))})`
        default:
            return null
    }
}

/** One InsightsQL predicate per compilable filter; filters for deleted definitions or with
