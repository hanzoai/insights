...
from insights.api.routing import DefaultRouterPlusPlus, RouterRegistry, TeamAndOrgViewSetMixin
...
from insights.models.scoping import get_current_team_id
...
@override_settings(ROOT_URLCONF=__name__)  # Use `urlpatterns` from this file and not from `insights.urls`
...
# Mirrors the discovery loop in insights/api/__init__.py so the tests exercise the same
...
def test_discovery_covers_every_product_with_a_routes_module():
    ...
    # would silently drop it. parents[3] of .../insights/api/test/test_routing.py is the repo root.
...
def test_project_secret_api_key_authentication_routes_include_api_scope_permission():
    from insights.api import router
