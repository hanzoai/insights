...
function PromptDiff({
    ...
    return (
        <>
            ...
            />
            <Modal isOpen={isExpanded} onClose={() => setIsExpanded(false)} title="Recommendation" fullScreen>
                <SuggestionDiffPanes
                    original={original}
                    modified={modified}
                    onChange={onChange}
...
const TAG_OP_TAG_TYPE: Record<ScannerConfigChange['op'], TagType> = {
    add: 'success',
    remove: 'danger',
    rename: 'default',
    set: 'default',
    ...
    return (
        <div className="flex flex-wrap gap-1">
            {changes.map((change, index) => (
                <Tag key={index} type={TAG_OP_TAG_TYPE[change.op]}>
...
function FieldValueEditor({
    ...
    if (kind === 'prompt') {
        if (!basePrompt) {
            return <TextArea value={String(value ?? '')} onChange={onChange} minRows={6} />
        }
        return (
            <PromptDiff
                original={basePrompt}
    ...
    return <TextArea value={String(value ?? '')} onChange={onChange} minRows={2} />
}
