...
from insights.egress.vapi import vapi_request
...
class _RateLimitMetricsMixin(SimpleRateThrottle):
    ...
    def allow_request(self, request: Request, view: Any) -> bool:
        from insights.rate_limit import RATE_LIMIT_EXCEEDED_COUNTER, get_route_from_path
...
def start_call(request: Request, access_token: str) -> Response:
    ...
    * a non-personalised (shared) topic share — every visitor is a new anonymous respondent who
    ...
    if not settings.VAPI_PUBLIC_KEY or not settings.VAPI_ASSISTANT_ID:
        ...
        return Response(
            {"error": "Vapi is not configured on this Insights instance."},
    ...
    if is_shared_interviewee_context(ic.interviewee_identifier):
        ...
        metadata: dict[str, str] = {
            ...
            # session_id isn't persisted in the DB — it rides on the lifecycle Insights event (as
...
def vapi_webhook(request: Request) -> Response:
    ...
    if not settings.VAPI_WEBHOOK_SECRET:
        ...
        return Response(
            {"error": "Vapi webhook secret is not configured on this Insights instance."},
            status=status.HTTP_503_SERVICE_UNAVAILABLE,
        )
...
def _capture_user_interview_event(
    ...
    *,
    ...
    """Fire a Insights event for a user-interview lifecycle moment (conversation started/ended).
    ...
    until we ack. Set `$insert_id` to `<event>:<call_id>` so Insights dedupes the second
