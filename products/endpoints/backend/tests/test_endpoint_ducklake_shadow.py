...
from insights.ducklake.client import DuckLakeQueryResult
...
INSIGHTSQL_QUERY = {"kind": "InsightsQLQuery", "query": "SELECT count() as cnt FROM events"}
...
class TestShadowDispatch(APIBaseTest):
    ...
    def test_dispatches_shadow_for_inline_execution(self, _mock_should, mock_task):
        endpoint = create_endpoint_with_version(
            name="shadow-inline", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user
        )
        ...
        assert kwargs["datastore_cached"] is False
        assert kwargs["datastore_row_count"] == 1
        assert isinstance(kwargs["datastore_ms"], float)
    ...
    def test_no_dispatch_on_cache_hit(self, _mock_should, mock_task):
        create_endpoint_with_version(name="shadow-cached", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user)
        cached = Response(
    ...
    def test_dispatch_propagates_pagination(self, _mock_should, mock_task):
        create_endpoint_with_version(name="shadow-paged", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user)
    ...
    def test_no_dispatch_when_flag_off(self, _mock_should, mock_task):
        create_endpoint_with_version(name="shadow-off", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user)
...
class TestShadowComparison(APIBaseTest):
    ...
    def test_emits_comparison_event_with_both_timings(self):
        ...
        with mock.patch.object(
            ...
            return_value=DuckLakeQueryResult(
                columns=["cnt"], types=["20"], results=[[1]], sql="", insightsql=None, connect_ms=8.0, query_ms=4.0
            ),
        ) as mock_execute:
            ducklake_shadow.run_ducklake_shadow_comparison(
                ...
                datastore_cached=False,
                datastore_ms=12.5,
                datastore_row_count=1,
                limit=None,
                offset=None,
            )
    ...
    def test_no_duckgres_server_is_noop_in_prod(self):
        endpoint = create_endpoint_with_version(
            name="cmp-noserver", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user
        )
        version = endpoint.get_version()
        captured = self._capture_events()
    ...
    def test_dev_mode_shadows_without_provisioned_server(self):
        ...
        with (
            ...
            mock.patch.object(
                ducklake_shadow,
                "execute_ducklake_query",
                return_value=DuckLakeQueryResult(columns=["cnt"], types=["20"], results=[[1]], sql="", insightsql=None),
            ),
        ):
            ducklake_shadow.run_ducklake_shadow_comparison(
                team_id=self.team.pk,
...
class TestShouldShadowDucklake(APIBaseTest):
    ...
    def test_does_not_shadow_during_test_suite(self):
        ...
        # must not fire during tests: it builds a userless InsightsQL database and runs eagerly under Celery,
        # which trips access-control assertions on unrelated runs (e.g. PSAK system-table gating).
        endpoint = create_endpoint_with_version(
            name="should-shadow-test", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user
        )
    ...
    def test_shadows_in_local_dev(self, _mock_dev):
        endpoint = create_endpoint_with_version(
            name="should-shadow-dev", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user
        )
        with self.settings(TEST=False):
            assert self._service()._should_shadow_ducklake(endpoint, endpoint.get_version()) is True

    @mock.patch("products.endpoints.backend.logic.execution.is_dev_mode", return_value=True)
    def test_no_shadow_for_non_insightsql_query(self, _mock_dev):
        endpoint = create_endpoint_with_version(
    ...
    @mock.patch("products.endpoints.backend.logic.execution.hanzo_insights.feature_enabled")
    def test_shadows_in_prod_when_flag_enabled(self, mock_flag, _mock_dev):
        endpoint = create_endpoint_with_version(
            name="should-shadow-flag", team=self.team, query=INSIGHTSQL_QUERY, created_by=self.user
        )
        version = endpoint.get_version()
        with self.settings(TEST=False):
