...
from insights.datastore.table_engines import Distributed
...
# person joins back to the main cluster. Without these, InsightsQL queries against
# ai_events that touch person.* fail with UNKNOWN_TABLE because the real
# person tables only live on NodeRole.DATA.
#
# Column DDL is imported from insights.models.person.sql so the shim stays
...
def PERSON_DISTINCT_ID2_AI_EVENTS_SHIM_SQL() -> str:
    return PERSON_DISTINCT_ID2_TABLE_BASE_SQL.format(
        ...
        engine=Distributed(data_table=PERSON_DISTINCT_ID2_TABLE, cluster=settings.DATASTORE_CLUSTER),
        extra_fields="",
    )
...
def PERSON_AI_EVENTS_SHIM_SQL() -> str:
    return PERSONS_TABLE_BASE_SQL.format(
        ...
        on_cluster_clause=ON_CLUSTER_CLAUSE(False),
        engine=Distributed(data_table=PERSONS_TABLE, cluster=settings.DATASTORE_CLUSTER),
        extra_fields="",
    )
