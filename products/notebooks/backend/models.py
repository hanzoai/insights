...
class NotebookNodeRun(TeamScopedRootMixin, UUIDModel):
    ...
    class NodeType(models.TextChoices):
        INSIGHTSQL = "insightsql", "insightsql"
        PYTHON = "python", "python"
        DUCKDB = "duckdb", "duckdb"

    # db_constraint=False: creating a real FK to the hot insights_team table locks it on deploy.
    ...
    # db_constraint=False: a real FK to the hot insights_user table locks it on deploy.
    ...
    user = models.ForeignKey(
        "insights.User", on_delete=models.DO_NOTHING, null=True, blank=True, db_constraint=False, db_index=False
    )
    node_id = models.CharField(max_length=128)
    # How the run executed: insightsql pushed to Datastore (pages re-query by `code`); python and
    # duckdb ran in the sandbox kernel (pages slice the on-sandbox result frame by `result_id`).
    node_type = models.CharField(choices=NodeType, default=NodeType.INSIGHTSQL, max_length=20)
    # The node's code at run time — paging must re-query what produced the result,
    # not whatever the editor holds now.
    code = models.TextField(blank=True, default="")
    # The direct-query data source this run executed against (null = Insights's own Datastore).
    ...
    # Whether `code` went to the connection verbatim instead of being compiled from InsightsQL. Set,
    # it means `code` is the engine's own dialect and the InsightsQL parser can't read it.
    ...
    class Meta:
        db_table = "insights_notebooknoderun"
        indexes = [
