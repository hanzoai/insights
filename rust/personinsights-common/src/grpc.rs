...
impl ClientInFlightGuard {
    pub fn new(backend: &'static str) -> Self {
        ...
        gauge!("personinsights_router_client_requests_in_flight", "backend" => backend, "client" => client.clone())
            .increment(1.0);
        Self { backend, client }
    }
}
...
impl Drop for ClientInFlightGuard {
    fn drop(&mut self) {
        gauge!("personinsights_router_client_requests_in_flight", "backend" => self.backend, "client" => self.client.clone())
            .decrement(1.0);
    }
}
...
mod tests {
    ...
    fn sanitize_allows_valid_tag_chars() {
        ...
        assert_eq!(
            sanitize_header_tag("insights-django-web"),
            "insights-django-web"
        );
