...
mod tests {
    ...
    /// Record `n` Decide requests for teams `1..=n` with the InsightsJs library.
    ...
    fn record_n_decide_with_library(agg: &Arc<BillingAggregator>, n: i32) {
        for team_id in 1..=n {
            agg.record(team_id, FlagRequestType::Decide, Some(Library::InsightsJs));
        }
    }
    ...
    #[case::same_key_aggregates(
        &[(1, Some(Library::InsightsJs)), (1, Some(Library::InsightsJs)), (1, Some(Library::InsightsJs))],
        &[((1, Some(Library::InsightsJs)), 3)],
    )]
    // Same team but distinct libraries (including no library) split into separate keys.
    #[case::distinct_libraries_split(
        &[(1, Some(Library::InsightsJs)), (1, Some(Library::InsightsNode)), (1, None)],
        &[
            ((1, Some(Library::InsightsJs)), 1),
            ((1, Some(Library::InsightsNode)), 1),
            ((1, None), 1),
        ],
    )]
    ...
    async fn test_flush_once_writes_pipelined_hincrby() {
        ...
        agg.record(42, FlagRequestType::Decide, Some(Library::InsightsJs));
        agg.record(42, FlagRequestType::Decide, Some(Library::InsightsJs));
        ...
        let expected_team_42_sdk = format!(
            "{}:{bucket}",
            get_team_request_library_key(42, FlagRequestType::Decide, Library::InsightsJs)
        );
    ...
    async fn test_flush_once_writes_flag_definitions_keys() {
        ...
        let calls = hincrby_calls(&redis);
        let expected_team_42_def = format!("insights:local_evaluation_requests:42:{bucket}");
        let expected_sdk_42_def =
            format!("insights:local_evaluation_requests:sdk:42:insights-js:{bucket}");
        let expected_team_7_decide = format!("insights:decide_requests:7:{bucket}");
        assert_eq!(calls.len(), 3);
        ...
        agg.seed_pending(old_buckets.iter().zip(counts.iter()).map(|(&b, &c)| {
            (
                AggregationKey {
                    team_id: 1,
                    request_type: FlagRequestType::Decide,
                    library: Some(Library::InsightsJs),
                    bucket: b,
                },
                c,
            )
