...
class TestCustomFlowAPI(APIBaseTest):
    def setUp(self):
        super().setUp()
    ...
    def _create_custom_flow_with_action(self, action_config: dict):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
        ...
        return custom_flow, action

    def test_custom_flow_function_trigger_check(self):
        custom_flow = {
            "name": "Test Flow",
            "actions": [],
        }

        response = self.client.post(f"/api/projects/{self.team.id}/custom_flows", custom_flow)
    ...
    def test_custom_flow_function_trigger_copied_from_action(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_function_validation(self):
        custom_flow, action = self._create_custom_flow_with_action(
            {
                "template_id": "missing",
                "inputs": {},
            }
        )
        custom_flow["status"] = "active"
    ...
    def test_custom_flow_bytecode_compilation(self):
        custom_flow, action = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
        ...
        assert response.status_code == 201, response.json()
        custom_flow = CustomFlow.objects.get(pk=response.json()["id"])

        assert custom_flow.trigger["filters"].get("bytecode") == ["_H", 1, 32, "$pageview", 32, "event", 1, 1, 11]

        assert custom_flow.actions[1]["filters"].get("bytecode") == ["_H", 1, 32, "custom_event", 32, "event", 1, 1, 11]

        assert custom_flow.actions[1]["config"]["inputs"] == {
            "url": {"order": 0, "value": "https://example.com", "bytecode": ["_H", 1, 32, "https://example.com"]}
        }

    def test_custom_flow_conditional_branch_filters_bytecode(self):
        conditional_action = {
            "id": "cond_1",
            "name": "cond_1",
            "type": "function",
    ...
    def test_custom_flow_single_condition_field(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_condition_and_conditions_mutually_exclusive(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_enable_disable(self):
        custom_flow, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example.com"}},
            }
        ...
        response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_flows/{response.json()['id']}", {"status": "active"}
        )
        assert response.status_code == 200, response.json()
        assert response.json()["status"] == "active"
        response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_flows/{response.json()['id']}", {"status": "draft"}
        )
        assert response.status_code == 200, response.json()
        assert response.json()["status"] == "draft"

    def test_custom_flow_conditional_event_filter_rejected(self):
        conditional_action = {
            "id": "cond_1",
            "name": "cond_1",
            "type": "function",
    ...
    def test_custom_flow_batch_trigger_valid_filters(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_batch_trigger_without_filters(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_batch_trigger_filters_not_dict(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_batch_trigger_properties_not_array(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_user_blast_radius_requires_filters(self):
        with patch("posthog.api.custom_flow.get_user_blast_radius") as mock_get_user_blast_radius:
            response = self.client.post(f"/api/projects/{self.team.id}/custom_flows/user_blast_radius", {})

        assert response.status_code == 400, response.json()
        assert "Missing filters" in response.json().get("detail", "")
        mock_get_user_blast_radius.assert_not_called()

    def test_custom_flow_user_blast_radius_returns_counts(self):
        with patch("posthog.api.custom_flow.get_user_blast_radius") as mock_get_user_blast_radius:
            mock_get_user_blast_radius.return_value = (4, 10)

            response = self.client.post(
                f"/api/projects/{self.team.id}/custom_flows/user_blast_radius",
                {"filters": {"properties": []}},
            )
    ...
    def test_billable_action_types_computed_correctly(self):
        ...
        flow = CustomFlow.objects.get(pk=response.json()["id"])
    ...
    def test_billable_action_types_recomputed_on_update(self):
        """Test that billable_action_types is recomputed when CustomFlow is updated"""
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
        ...
        custom_flow = {"name": "Test Update Billable Types", "actions": initial_actions}
        response = self.client.post(f"/api/projects/{self.team.id}/custom_flows", custom_flow)
        assert response.status_code == 201, response.json()
        ...
        update_response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_flows/{flow_id}", {"actions": updated_actions}
        )
        ...
        override_response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_flows/{flow_id}",
            {
        ...
        flow = CustomFlow.objects.get(pk=flow_id)
        assert flow.billable_action_types == ["function"]
    ...
    def test_post_custom_flow_batch_jobs_endpoint_creates_job(self, mock_create_invocation):
        custom_flow, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example.com"}},
        ...
        create_response = self.client.post(f"/api/projects/{self.team.id}/custom_flows", custom_flow)
        ...
        response = self.client.post(f"/api/projects/{self.team.id}/custom_flows/{flow_id}/batch_jobs", batch_job_data)

        assert response.status_code == 200, response.json()
        assert response.json()["custom_flow"] == flow_id
        assert response.json()["variables"] == batch_job_data["variables"]
        assert response.json()["status"] == "queued"
        mock_create_invocation.assert_called_once()

    def test_post_custom_flow_batch_jobs_endpoint_nonexistent_flow(self):
        batch_job_data = {"variables": [{"key": "first_name", "value": "Test"}]}

        response = self.client.post(f"/api/projects/{self.team.id}/custom_flows/99999/batch_jobs", batch_job_data)
    ...
    def test_get_custom_flow_batch_jobs_only_returns_jobs_for_flow(self, mock_create_invocation):
        custom_flow, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example.com"}},
        ...
        # Create another flow
        custom_flow_2, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example2.com"}},
            }
        )
        create_response_2 = self.client.post(f"/api/projects/{self.team.id}/custom_flows", custom_flow_2)
        ...
        job_response_1 = self.client.post(
            f"/api/projects/{self.team.id}/custom_flows/{flow_id}/batch_jobs", batch_job_data_1
        )
        assert job_response_1.status_code == 200, job_response_1.json()

        job_response_2 = self.client.post(
            f"/api/projects/{self.team.id}/custom_flows/{flow_id_2}/batch_jobs", batch_job_data_2
        )
        assert job_response_2.status_code == 200, job_response_2.json()

        # Fetch jobs for the first flow
        get_response = self.client.get(f"/api/projects/{self.team.id}/custom_flows/{flow_id}/batch_jobs")
        assert get_response.status_code == 200, get_response.json()
        jobs = get_response.json()
        assert len(jobs) == 1
        assert jobs[0]["id"] == job_response_1.json()["id"]

    def test_custom_flow_filter_test_accounts_compiles_bytecode(self):
        ...
        response_without = self.client.post(f"/api/projects/{self.team.id}/custom_flows", custom_flow_without)
        ...
        response_with = self.client.post(f"/api/projects/{self.team.id}/custom_flows", custom_flow_with)
    ...
    def test_custom_flow_draft_compiles_bytecode_for_complete_actions(self):
        custom_flow, action = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example.com"}},
    ...
    def test_custom_flow_draft_to_active_compiles_bytecode(self):
        custom_flow, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example.com"}},
    ...
    def test_custom_flow_draft_partial_inputs_skips_input_bytecode(self):
        custom_flow, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {},
    ...
    def _get_custom_flow_activity(self, flow_id: Optional[str] = None) -> list:
        params: dict = {"scope": "CustomFlow", "page": 1, "limit": 20}
        if flow_id:
    ...
    def test_create_custom_flow_logs_activity(self):
        custom_flow, _ = self._create_custom_flow_with_action(
            {
                "template_id": "template-webhook",
                "inputs": {"url": {"value": "https://example.com"}},
    ...
    def test_update_custom_flow_logs_activity(self):
        ...
        update_response = self.client.patch(f"/api/projects/{self.team.id}/custom_flows/{flow_id}", {"name": new_name})
    ...
    def test_custom_flow_draft_allows_incomplete_actions(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_active_rejects_incomplete_actions(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_draft_invalid_can_be_archived(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
    ...
    def test_custom_flow_draft_to_active_rejects_incomplete(self):
        trigger_action = {
            "id": "trigger_node",
            "name": "trigger_1",
            "type": "trigger",
