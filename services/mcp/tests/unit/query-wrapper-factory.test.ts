...
describe('createQueryWrapper _meta', () => {
    ...
    it('sets both uiResourceUri and outputFormat in _meta', () => {
        const factory = createQueryWrapper({
            ...
            schema,
            kind: 'TestQuery',
            uiResourceUri: 'ui://insights/test.html',
            outputFormat: 'json',
        })
...
describe('createQueryWrapper _insightsUrl', () => {
    ...
    it.each(['TrendsQuery', 'FunnelsQuery', 'RetentionQuery', 'StickinessQuery', 'PathsQuery', 'LifecycleQuery'])(
        ...
        async (kind) => {
            ...
            const hash = result._insightsUrl.split('#q=')[1]
    ...
    it.each([
        ...
        expect(result._insightsUrl).toContain('DataTableNode')
    })
    ...
    it('uses hash param not query param', async () => {
        ...
        expect(result._insightsUrl).toContain('/insights/new#q=')
        expect(result._insightsUrl).not.toContain('/insights/new?q=')
    })
    ...
    it('uses urlPrefix directly without InsightVizNode wrapping', async () => {
        ...
        expect(result._insightsUrl).toBe('http://localhost:8010/project/1/error_tracking')
        expect(result._insightsUrl).not.toContain('InsightVizNode')
    })
})
...
describe('createQueryWrapper trace compaction', () => {
    ...
    it('leaves non-trace query results untouched', async () => {
        const tool = createQueryWrapper({ name: 'test', schema, kind: 'InsightsQLQuery' })()

        const result = (await tool.handler(contextWithResults([oversizedTrace]), { kind: 'InsightsQLQuery' })) as any
...
describe('createQueryWrapper warnings', () => {
    ...
    it('omits the key when there are no warnings', async () => {
        ...
        const result = (await tool.handler(contextWithRunQuery({ results: [] }), { kind: 'InsightsQLQuery' })) as any
