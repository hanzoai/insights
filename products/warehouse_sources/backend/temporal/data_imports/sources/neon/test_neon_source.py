...
from insights.schema import ReleaseStatus, SourceFieldInputConfig
...
def test_cdc_prerequisites_reject_pooled_host_without_connecting(host):
    ...
    with mock.patch.object(PostgresSource, "check_cdc_prerequisites") as super_check:
        errors = NeonSource().check_cdc_prerequisites(config, management_mode="insights", tables=["users"])
