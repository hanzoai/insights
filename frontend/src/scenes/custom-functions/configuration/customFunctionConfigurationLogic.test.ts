...
import { CustomFunctionTemplateType, CustomFunctionType } from '~/types'
...
const mockApi = api.customFunctions as jest.Mocked<typeof api.customFunctions>

const HOG_TEMPLATE: CustomFunctionTemplateType = {
    free: false,
    status: 'beta',
    id: 'template-webhook',
    type: 'destination',
...
const CUSTOM_FUNCTION: CustomFunctionType = {
    ...HOG_TEMPLATE,
...
describe('customFunctionConfigurationLogic', () => {
    let logic: ReturnType<typeof customFunctionConfigurationLogic.build>
    ...
    describe('template', () => {
        beforeEach(() => {
            ...
            mockApi.create.mockReturnValue(Promise.resolve(CUSTOM_FUNCTION))
            mockApi.update.mockReturnValue(Promise.resolve(CUSTOM_FUNCTION))
        ...
        it('saves if form valid', async () => {
            ...
            await expectLogic(logic, () => {
                logic.actions.submitConfiguration()
            }).toDispatchActions(['upsertCustomFunction', 'submitConfigurationSuccess'])
        })
    })
})
