...
from insights.temporal.session_replay.summarization_sweep.reconciler import ReconcileSummarizationSchedulesWorkflow
from insights.temporal.session_replay.summarization_sweep.types import (
...
def _make_listing(schedule_id: str, workflow_type: str = WORKFLOW_NAME, fingerprint: str | None = None) -> Any:
    ...
    if fingerprint is not None:
        key = MagicMock()
        key.name = "InsightsScheduleFingerprint"
        pair = MagicMock()
        pair.key = key
...
async def test_list_summarization_schedule_team_ids_queries_by_schedule_type(activity_environment):
    ...
    with patch(
        "insights.temporal.common.client.async_connect",
    ...
    assert call_kwargs["query"] == f'InsightsScheduleType = "{SCHEDULE_TYPE}"'


@pytest.mark.asyncio
async def test_upsert_team_schedule_activity_delegates(activity_environment):
    with patch(
        "insights.temporal.session_replay.summarization_sweep.schedule.a_upsert_team_schedule",
        AsyncMock(),
    ) as mock_upsert:
...
def test_summarize_team_workflow_parse_inputs():
    ...
    from insights.temporal.session_replay.summarization_sweep.types import SummarizeTeamSessionsInputs
    from insights.temporal.session_replay.summarization_sweep.workflow import SummarizeTeamSessionsWorkflow
