---
name: writing-skills
description: 'Guide for writing Insights agent skills — job-to-be-done templates that teach agents how to use MCP tools to achieve a goal. Use when adding new product functionality that agents should know how to work with, creating a new skill, or updating existing skills in products/*/skills/.'
---

# Writing skills for Insights agents
...
Distribution is automatic after merge — CI publishes to [Insights/skills](https://github.com/Insights/skills).
...
- **Name**: lowercase kebab-case, prefer gerund form (`analyzing-llm-traces`, not `llm-analytics`). Never prefix with `insights-*`.
...
by [`products/insights_ai/scripts/build_skills.py`](../../../products/insights_ai/scripts/build_skills.py).
...
- `render_insightsql_example({"kind": "TrendsQuery", ...})` — renders a query spec to InsightsQL SQL
- `insightsql_functions()` — returns all available InsightsQL function names

## Good example: `querying-insights-data`
...
- See [`products/insights_ai/skills/querying-insights-data/SKILL.md`](../../../products/insights_ai/skills/querying-insights-data/SKILL.md)
