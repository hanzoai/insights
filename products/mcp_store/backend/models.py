...
InstallSource = Literal["insights", "twig", "insights-code"]
INSTALL_SOURCE_CHOICES = [("insights", "insights"), ("twig", "twig"), ("insights-code", "insights-code")]
...
class MCPServerTemplate(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    """A curated, pre-registered MCP server. Insights operators register a real
...
class MCPServerInstallation(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    user = models.ForeignKey("insights.User", on_delete=models.CASCADE, related_name="mcp_server_installations")
...
class MCPOAuthState(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    install_source = models.CharField(max_length=20, choices=INSTALL_SOURCE_CHOICES, default="insights")
    insights_code_callback_url = models.TextField(blank=True, default="", db_column="twig_callback_url")
...
class TeamMCPGatewayConfig(TeamScopedRootMixin, UUIDModel):
    ...
    team = models.OneToOneField(
        "insights.Team", on_delete=models.CASCADE, related_name="mcp_gateway_config", db_constraint=False
    )
...
class MCPGatewayServer(TeamScopedRootMixin, UUIDModel):
    ...
    team = models.ForeignKey(
        "insights.Team", on_delete=models.CASCADE, related_name="gateway_servers", db_constraint=False
    )
    name = models.CharField(max_length=200)
...
class MCPOrgRule(TeamScopedRootMixin, UUIDModel):
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, related_name="org_rules", db_constraint=False)
    name = models.CharField(max_length=200)
    description = models.TextField(blank=True, default="")
...
class MCPServiceAccount(TeamScopedRootMixin, UUIDModel):
    """A fixed Insights agent identity with independent MCP access policies."""

    team = models.ForeignKey(
        "insights.Team", on_delete=models.CASCADE, related_name="mcp_service_accounts", db_constraint=False
    )
    name = models.CharField(max_length=200)
    description = models.TextField(blank=True, default="")
...
class MCPToolPolicy(TeamScopedRootMixin, UUIDModel):
    ...
    scope_user = models.ForeignKey(
        "insights.User", on_delete=models.CASCADE, null=True, blank=True, related_name="+", db_constraint=False
    )
