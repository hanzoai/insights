...
pnpm --filter=@hanzo/mcp run generate:ui-apps
pnpm --filter=@hanzo/mcp run build
```
...
import { DescriptionList } from '@hanzo/mcp-ui'
import { Card, CardHeader, CardTitle, CardContent } from '@hanzo/quill'
...
import { DataTable, type DataTableColumn, ListDetailView } from '@hanzo/mcp-ui'
...
type Result = WithInsightsUrl<MyEntityData>
...
export default (): ToolBase<typeof schema, Result> =>
  withUiApp('my-entity', {
    ...
    handler: async (context, params) => {
      ...
      return await withInsightsUrl(context, data, `/my-entities/${data.id}`)
    },
  })
```
...
If you change YAML `ui_apps` sections, run `pnpm --filter=@hanzo/mcp run generate:ui-apps`
