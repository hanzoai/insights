# Declarative Datastore schema (HCL)

Source of truth for satellite Datastore clusters, managed declaratively with
[`hclexp`](../../../../python-datastore-schema) instead of hand-written migrations.
...
hcl/
  ...
  sql/<env>-<role>.sql     # generated build-from-scratch CREATE schema per node (apply to a fresh Datastore)
...
`insights/datastore/migrations/` still produce the schema the HCL declares. That gap is how old
...
(`tools/infra-scripts/datastore-multinode/`, workflow `ci-datastore-multinode-migrations.yml`)
**after** `manage.py migrate_datastore`:
...
```bash
DUMP=$(bash insights/datastore/hcl/dump-live.sh)   # step 1 -> prints the dump dir
bash insights/datastore/hcl/check-live.sh "$DUMP"  # step 2
```
...
Per-node `{shard}` / `{replica}` stay as Datastore macros, so replicas collapse to one definition.
...
skipped. `system.*` remotes are always resolvable. The `insights` data cluster is `local`-only here
(prod goldens live in insights-cloud-infra), so `check.sh` passes it via `-cluster` flags — composed
...
build it yourself with `go build -o hclexp ./cmd/hclexp` in `../../../../python-datastore-schema`.
...
1. **Edit the right layer** for what you're changing:
   ...
   - a column list shared by a sharded table and its Distributed siblings → an `abstract` table the
     ...
     compresses with ZSTD, and `insights/datastore/migrations/AGENTS.md` has the rule for the rare
...
3. **Generate the migration** — `--auto` writes the next numbered migration and bumps `max_migration.txt`:
   ...
   Review the generated `insights/datastore/migrations/NNNN_<slug>.py`: add `settings.CLOUD_DEPLOYMENT`
...
`sql/local-ops.sql` to a local Datastore to create the OPS schema. (It is faithful to the HCL, so it
