"""Convert between Insights rich content/markdown and Slack payload formats."""
...
def slack_blocks_to_rich_content(blocks: list[JSON] | None, user_names: dict[str, str] | None = None) -> JSON | None:
    """Parse Slack rich_text blocks into Insights SupportEditor-compatible JSON."""
    if not blocks:
        return None
...
def rich_content_to_markdown(rich_content: JSON | None, include_images: bool = True) -> str:
    """Serialize Insights rich content JSON to markdown text."""
    if not rich_content:
        return ""
...
def rich_content_to_slack_blocks(rich_content: JSON | None, include_images: bool = True) -> list[JSON] | None:
    """Serialize Insights rich content JSON into Slack rich_text blocks."""
    if not rich_content or rich_content.get("type") != "doc":
        return None
...
def rich_content_to_html(rich_content: JSON | None) -> str:
    """Serialize Insights rich content JSON to email-safe HTML."""
    if not rich_content or rich_content.get("type") != "doc":
        return ""
