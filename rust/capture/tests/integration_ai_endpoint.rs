...
async fn test_ai_event_with_blobs_published_with_s3_placeholders() {
    ...
    // URL format: s3://test-bucket/llma/<team_hash>/<uuid>?range=...
    // Partitioned by the validated team id (StaticTeamResolver → TEST_TEAM_ID),
    // hashed (first 16 chars of SHA-256) by the blob store's path sanitizer.
    let team_hash = "6b86b273ff34fce1"; // SHA-256 of "1" (TEST_TEAM_ID)
    let expected_prefix = format!("s3://{TEST_BLOB_BUCKET}/{TEST_BLOB_PREFIX}{team_hash}/");
    assert!(
...
async fn send_signed_ai_request(
    ...
    client
        ...
        .header("Insights-Ai-Gateway-Signature", signature)
        .header("Insights-Ai-Gateway-Signed-At", signed_at)
        .header("Insights-Ai-Gateway-Request-Id", request_id)
        .body(body)
        .send()
        .await
}
    ...
    let form = Form::new().part(
        ...
        Part::bytes(
            serde_json::to_vec(&json!({
                ...
                "event": "$pageview",
                "distinct_id": "user-1",
                "properties": {"$ai_model": "gpt-4", "$lib": "insights-python", "$lib_version": "3.1.0"}
            }))
            .unwrap(),
        )
        .mime_str("application/json")
