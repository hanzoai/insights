...
//! Depends on `store`, `datastore`, `kafka`, `domain`, and its `app` siblings (`prepare`,
...
use crate::datastore::person_scanner::PersonScanner;
use crate::datastore::scanner::ChunkScanner;
...
/// One planning scan at a time: it streams a team's whole live person-id set from Datastore.
...
/// boundary scan is a full-table aggregation on shared Datastore, so a failure must not be
