...
def register_all_admin():
    ...
    # Central Insights admins live in `insights/admin/admins/<x>_admin.py` —
    ...
    import insights.admin.admins  # noqa: F401
    ...
    from insights.admin.admins.oauth_admin import OAuthApplicationAdmin
...
# :KRUDGE: OAuth models live in the `insights` app, so by default they appear
# under "Insights" in the admin sidebar alongside dozens of unrelated models.
...
def install_admin_app_list_overrides():
    ...
    def get_app_list(request, app_label=None):
        ...
        # so we have to source its models from the `insights` app and rebuild the
        # group ourselves — otherwise visiting /admin/oauth/ would 404.
        if app_label == "oauth":
            insights_app_list = original_get_app_list(request, app_label="insights")
            oauth_models = _extract_oauth_models(insights_app_list)
