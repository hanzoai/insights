...
class WebAnalyticsPathCleaningSuggestionViewSet(TeamAndOrgViewSetMixin, viewsets.ViewSet):
    ...
    def get_throttles(self):
        if self.action in {"generate", "preview"}:
            # generation runs a Datastore path sample plus an LLM call, and preview runs a
            # fresh Datastore sample per request — cap both for session-authenticated users
    ...
    def generate(self, request: Request, **kwargs: Any) -> Response:
        # Dogfooding gate on the one verb that spends money (Datastore sampling + an LLM call).
        ...
        if not distinct_id or not hanzo_insights.feature_enabled(
            FEATURE_FLAG_KEY,
            distinct_id,
