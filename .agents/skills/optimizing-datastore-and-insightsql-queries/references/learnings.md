...
>
> No raw person / group / distinct_id UUIDs, no custom property names or values, no team or org names, no row samples, no precise customer-specific operational scale (exact row counts, durations tied to a specific team). Use placeholders (`<bound_uuid>`, `<custom_property>`, `<team_id>`) or describe the shape (`a 1M-person slice picked by sort offset`, `tens of millions of persons`). Insights's own team 2 is fine to name as the canonical Test Cluster target; redact other team IDs. Insights-standard properties prefixed with `$` (`$browser`, `$os`, `$current_url`) are safe; customer-defined properties are not.
...
\* L was cancelled at ~11s all three runs by Metabase aborting the ~100MB result download (`QUERY_WAS_CANCELLED`), not by Datastore — the scan and memory numbers before cancellation are the signal.
...
**Context.** `insights/temporal/messaging/backfill_precalculated_person_properties_workflow.py` builds a raw Datastore query that does `SELECT id, JSONExtract(properties, '<key>', 'String'), ... FROM person FINAL WHERE team_id = ... AND id BETWEEN ... AND is_deleted = 0 ORDER BY id FORMAT JSONEachRow`. We flagged the `FINAL` as a smell.
...
## 2026-06-24: Datastore prunes unused `argMax(content)`; the real win is bounding the dedup, not dropping columns
...
**The proposed smell was partly wrong.** The hypothesis was "it `argMax`'s the big `content` string for every document, then throws it away." Measured against local synthetic single-team data (incompressible 3KB `content`), `read_bytes` for the original was **identical** to a variant that never selects content — Datastore's analyzer drops the `argMax(content)`/`argMax(timestamp)` because the outer query never references those aliases. Dead-column elimination already handled it; "drop the unused content argMax" buys ~0 at the Datastore layer.
