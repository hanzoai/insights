...
- ``InsightsQLEndpointStrategy``: raw InsightsQL queries with ``{variables.x}`` placeholders.
...
from insights.schema import DashboardFilter, EndpointRunRequest, InsightsQLVariable, PropertyOperator
...
class PlaceholderPreservingPrinter(InsightsQLPrinter):
    """InsightsQL printer that preserves {placeholder} syntax instead of raising on unresolved placeholders."""
...
def clean_breakdown_sentinels(result: dict, clean_empty_string: bool) -> bool:
    ...
    Handles both list-of-lists (materialized/InsightsQL) and list-of-dicts (inline insight).
    ...
    applied to arbitrary InsightsQL results where '' is a legitimate value.
    ...
    if columns:
        # InsightsQL/materialized path: results are list[tuple] or list[list]
...
FILTERS_OVERRIDE_DEPRECATION_HEADERS = {
    "X-Insights-Warn": (
        ...
        "https://hanzo.ai/docs/api/endpoints"
    )
}
...
class EndpointQueryStrategy(abc.ABC):
    ...
    def apply_pagination(self, query: dict, limit: int, offset: int) -> tuple[dict, EndpointPagination]:
        ...
        raise ValidationError(
            {"limit": f"Limit/offset parameters are only supported for InsightsQLQuery, not {self.query_kind}"}
        )


class InsightsQLEndpointStrategy(EndpointQueryStrategy):
    """Endpoints backed by a raw InsightsQL query with {variables.x} placeholders."""
    ...
    def _parse_original_query(self) -> tuple[list | None, int | None, bool]:
        """Parse the original InsightsQL query and extract SELECT columns, LIMIT, and GROUP BY presence.
        ...
        try:
            ...
            logger.debug("Failed to parse original InsightsQL query", exc_info=True)
    ...
    def transform_materialized_response(self, response_data: dict, saved_query: "DataWarehouseSavedQuery") -> None:
        """InsightsQL materialized rows are flat and returned as-is — no re-shaping needed."""
        return None

    def clean_response_sentinels(self, response_data: dict) -> None:
        """Clean sentinel literals only — plain '' is a legitimate InsightsQL value and stays."""
    ...
    def _parse_variables(self, query: dict, variables: dict[str, str]) -> list[InsightsQLVariable] | None:
    ...
    def apply_pagination(self, query: dict, limit: int, offset: int) -> tuple[dict, EndpointPagination]:
        """Apply pagination to a InsightsQL query.

        Parses the InsightsQL AST, applies LIMIT/OFFSET, and reprints using a
        ...
        ctx = InsightsQLContext(enable_select_queries=True, limit_top_select=False)
        query = query.copy()
...
class InsightEndpointStrategy(EndpointQueryStrategy):
    ...
    def _variables_to_filters(
        ...
        for prop_name, prop_type in breakdown_infos or []:
            ...
            try:
                if prop_type == "insightsql":
                    # InsightsQLPropertyFilter.key is a full InsightsQL predicate and doesn't accept
                    # `operator` (InsightsQLPropertyFilter has extra="forbid"), so build a
                    ...
                    properties.append({"key": predicate_expr.to_insightsql(), "type": "insightsql"})
                    continue
                if value == "":
...
def strategy_for(endpoint: Endpoint, version: EndpointVersion, team: Team) -> EndpointQueryStrategy:
    ...
    if version.query.get("kind") == "InsightsQLQuery":
        return InsightsQLEndpointStrategy(endpoint, version, team)
