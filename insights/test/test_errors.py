...
from insights.errors import (
    ...
    QueryErrorCategory,
    look_up_datastore_error_code_meta,
    wrap_datastore_query_error,
)


class TestWrapDatastoreQueryError:
    @parameterized.expand(
        [
            # Ambiguous-identifier query errors are the user's malformed InsightsQL, not a server fault,
    ...
    def test_ambiguous_identifier_codes_wrap_as_exposed_error(self, code: int, name: str) -> None:
        ...
        wrapped = wrap_datastore_query_error(err)

        assert isinstance(wrapped, ExposedCHQueryError)
        assert look_up_datastore_error_code_meta(err).get_category() == QueryErrorCategory.USER_ERROR
