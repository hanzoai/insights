    ...
    listeners(({ actions }) => ({
        setAutostartPriority: async ({ priority }) => {
            ...
            try {
                ...
                success = false
                toast.error(error?.detail ?? error?.message ?? 'Failed to update auto-start threshold')
            } finally {
        ...
        updateSlackNotifications: async ({ updates }) => {
            ...
            try {
                ...
                success = false
                toast.error(error?.detail ?? error?.message ?? 'Failed to update Slack notification setting')
            } finally {
