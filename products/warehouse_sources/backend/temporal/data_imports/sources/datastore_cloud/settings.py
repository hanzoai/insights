...
# Recent usage cost records are restated until Datastore locks them (the API marks them
...
@dataclass
class DatastoreCloudEndpointConfig:
    name: str
...
DATASTORE_CLOUD_ENDPOINTS: dict[str, DatastoreCloudEndpointConfig] = {
    "organizations": DatastoreCloudEndpointConfig(
        name="organizations",
        path="/v1/organizations",
        primary_keys=["id"],
        description="The organization the API key is scoped to (the API returns exactly one).",
    ),
    "services": DatastoreCloudEndpointConfig(
        name="services",
        path="/v1/organizations/{organization_id}/services",
        primary_keys=["organizationId", "id"],
        description="All Datastore Cloud services in the organization, including state, tier, region, and scaling configuration.",
    ),
    "usage_cost": DatastoreCloudEndpointConfig(
        name="usage_cost",
        ...
        description="Daily per-entity usage cost records in Datastore Credits (CHCs), broken down by storage, compute, backup, and data transfer.",
    ),
    "api_keys": DatastoreCloudEndpointConfig(
        name="api_keys",
    ...
    "members": DatastoreCloudEndpointConfig(
        name="members",
    ...
    "activities": DatastoreCloudEndpointConfig(
        name="activities",
    ...
    "backups": DatastoreCloudEndpointConfig(
        name="backups",
...
ENDPOINTS = tuple(DATASTORE_CLOUD_ENDPOINTS.keys())

INCREMENTAL_FIELDS: dict[str, list[IncrementalField]] = {
    name: config.incremental_fields for name, config in DATASTORE_CLOUD_ENDPOINTS.items()
}
