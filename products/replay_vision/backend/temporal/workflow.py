...
# Generous because fetch's deterministic errors are non_retryable; this budget only covers transient infra (e.g. Datastore at capacity).
...
# Provider-facing activities whose Temporal timeout means "provider slow", not a Insights bug.
...
def _activity_timeout_kind(e: BaseException) -> str | None:
    ...
    # Every other activity here talks to our own dependencies (Datastore, Postgres, Redis, object storage). A
...
class ApplyScannerWorkflow(InsightsWorkflow):
