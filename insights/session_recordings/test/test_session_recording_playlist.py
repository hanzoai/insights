...
from insights.models import Organization, PersonalAPIKey, SessionRecording, SessionRecordingPlaylistItem, Team
...
from insights.session_recordings.models.session_recording_playlist import (
...
from insights.session_recordings.session_recording_playlist_api import (
...
from insights.session_recordings.synthetic_playlists import ExpiringPlaylistSource
...
class TestSessionRecordingPlaylist(APIBaseTest, QueryMatchingTest):
    ...
    def test_list_does_not_issue_redundant_db_count(self) -> None:
        ...
        playlist_count_queries = [
            ...
            and "insights_sessionrecordingplaylist" in q["sql"].lower()
            and "insights_sessionrecordingplaylistitem" not in q["sql"].lower()
        ]
    ...
    def test_does_not_count_empty_object_as_filters(self) -> None:
        ...
        a regression test for https://github.com/Insights/insights/issues/35820
        """
        create_response = self._create_playlist({"type": "collection"})
        assert "short_id" in create_response.json(), create_response.json()
        short_id = create_response.json()["short_id"]
    ...
    def test_filters_playlist_by_type(self):
        ...
        # warehouse InsightsQL Database and emits a DataWarehouseSavedQuery lookup) stays
...
class TestPrecomputeRecordingsCounts(APIBaseTest):
    ...
    def test_redis_mget_failure_degrades_without_raising(self) -> None:
        ...
        with patch("insights.session_recordings.session_recording_playlist_api.get_client") as mock_get_client:
