...
from insights.temporal.health_checks.detectors import DEFAULT_EXECUTION_POLICY, HealthExecutionPolicy
from insights.temporal.health_checks.models import DEFAULT_ACTIVE_SINCE_DAYS, HealthCheckResult
from insights.temporal.health_checks.registry import _DETECT_FNS, HEALTH_CHECKS, ensure_registry_loaded
...
class AlertContent:
    ...
    `$health_check_issue_resolved` event properties, where InsightsFunction
    templates pick them up as `event.properties.title`, etc.
    """

    title: str
    summary: str
    # Relative path inside the Insights app (e.g. "/health/sdk-health").
    # InsightsFunction templates concatenate this onto {project.url}.
    link: str
...
class HealthCheck:
    ...
    #   - human: how to fix it in the Insights UI (no need to repeat the
    ...
    def render_alert(cls, issue: HealthIssue) -> AlertContent:
        """Build the alert content surfaced to InsightsFunction destinations.
