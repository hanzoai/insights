...
// The toolbar loader: a tiny classic script served as /static/toolbar.js. insights-js (every
...
//   the versioned, major-alias and compatibility prefixes on the insights-js CDN.
...
// build-time public path (set for insights-js versioned CDN bundles) is the fallback.
...
function loadApp(): Promise<ToolbarModule> {
    if (!appPromise) {
        appPromise = importWithRetry(resolveAppUrl(__POSTFN_TOOLBAR_APP_ENTRY__), 2, 300)
            ...
            .then((module) => {
                controllerDelegate = module.insightsToolbarController
                return module
            })
            .catch((error) => {
...
// The app module exposes the real window.insightsToolbarController when it evaluates, but
...
let controllerDelegate: ToolbarModule['insightsToolbarController'] | null = null
...
win['insightsToolbarController'] = win['insightsToolbarController'] || controllerStub

win['ph_load_toolbar'] = async function (toolbarParams: ToolbarParams, insights?: Insights): Promise<void> {
    ...
    win.__insights_toolbar_load_start = performance.now()
    let module: ToolbarModule
    try {
        module = await loadApp()
    } catch (error) {
        // Graceful degradation is "no toolbar", never a broken host page.
        console.warn('[Insights Toolbar] Failed to load the toolbar bundle', error)
        return
    }
    await module.loadToolbar(toolbarParams, insights)
}
