...
const makeFlow = (overrides: Partial<Flow> & { id: string; waitUntil?: boolean }): Flow => {
...
class MatcherUnderTest extends CdpHogflowSubscriptionMatcherConsumer {
    ...
    constructor() {
        ...
        // Stub flowManager
        ;(this as any).flowManager = {
            getFlowsForTeams: jest.fn().mockResolvedValue({}),
            getFlowsForTeam: jest.fn().mockResolvedValue([]),
        }
    ...
    public setFlows(map: Record<string, Flow>): void {
        const byTeam: Record<number, Flow[]> = {}
        for (const flow of Object.values(map)) {
            byTeam[flow.team_id] = byTeam[flow.team_id] ?? []
            byTeam[flow.team_id].push(flow)
        }
        ;(this as any).flowManager.getFlowsForTeams.mockResolvedValue(byTeam)
        ;(this as any).flowManager.getFlowsForTeam.mockImplementation((teamId: number) =>
...
describe('CdpHogflowSubscriptionMatcherConsumer', () => {
    ...
    describe('wakeMatchingWorkflows', () => {
        ...
        it('correlates team_id with distinct_id so a cross-team pairing is never queried or woken', async () => {
            ...
            matcher.setFlows({
                'flow-1': makeFlow({ id: 'flow-1', team_id: 1 }),
                'flow-2': makeFlow({ id: 'flow-2', team_id: 2 }),
            })
        ...
        it('scopes the lookup to qualifying flows, excluding non-wait flows on the same team', async () => {
            matcher.setFlows({
                'flow-1': makeFlow({ id: 'flow-1' }),
                'flow-2': makeFlow({ id: 'flow-2', waitUntil: false }),
            })
            await matcher.runWake([makeGlobals({})])
            const lookup = matcher.calls.find((c) => c.sql.includes('SELECT id, team_id, function_id'))!
            expect(lookup).not.toBeUndefined()
        ...
        it('skips cyclotron entirely when no team in the batch has a wait_until_condition or conversion goal', async () => {
            // Default: no flows configured → team has nothing the matcher could wake.
        ...
        it('logs the hogflow id alongside the action id when wait-step bytecode evaluation throws', async () => {
            ...
            try {
                ...
                expect(errorCall![2]).toMatchObject({ flowId: 'flow-1', actionId: 'wait_node' })
                expect(captureSpy).toHaveBeenCalledWith(expect.any(Error), {
                    extra: { flowId: 'flow-1', actionId: 'wait_node' },
                })
            } finally {
        ...
        it('does not wake when the wait step has events without bytecode (fail-closed)', async () => {
            // FlowSerializer compiles bytecode at save time; a row that reaches the
        ...
        it('wakes only matching subset when multiple candidates returned', async () => {
            ...
            const noMatchFlow = makeFlow({ id: 'flow-no-match' })
    ...
    describe('_parseKafkaBatch', () => {
        ...
        beforeEach(() => {
            ...
            matcher.setFlows({ 'flow-1': makeFlow({ id: 'flow-1', team_id: 1 }) })
        })
    ...
    describe('processMoveBatch', () => {
        ...
        it('skips a job parked on a delay step and scopes the lock to wait actions only', async () => {
            ...
            const flow = makeFlow({ id: 'flow-1', team_id: 1 })
            flow.actions.push({
                id: 'delay_node',
                name: 'Delay',
                type: 'delay',
