...
from products.error_tracking.backend.facade import sync_issues_to_datastore
...
class HedgeboxMatrix(Matrix):
    ...
    def _set_up_file_engagement_experiment(self, team: "Team", user: "User", flag: FeatureFlag) -> None:
        ...
        # Skipped in tests: one Datastore query per experiment day is too slow for CI seeding.
        if settings.TEST:
            return
        ...
        try:
            ...
            # so without this bound the backfill runs one Datastore query per day between the
    ...
    def _set_up_error_tracking_demo_data(self, team: "Team") -> None:
        ...
        people = [
            person for person in cast(list[HedgeboxPerson], self.people) if person.past_events and person.in_insights_id
        ]
        if not people:
            return
