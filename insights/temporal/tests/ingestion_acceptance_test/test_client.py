...
from insights.temporal.ingestion_acceptance_test.client import CapturedEvent, Person, InsightsClient
...
@pytest.fixture
def mock_insights_sdk() -> MagicMock:
    return MagicMock()


@pytest.fixture
def client(config: Config, mock_insights_sdk: MagicMock) -> Generator[InsightsClient]:
    client = InsightsClient(config, mock_insights_sdk)
    yield client
    client.shutdown()


class TestCaptureEvent:
    def test_calls_sdk_with_correct_arguments(self, client: InsightsClient, mock_insights_sdk: MagicMock) -> None:
        ...
        mock_insights_sdk.capture.assert_called_once_with(
            distinct_id="user_123",
            event="test_event",
    ...
    def test_uses_empty_dict_when_no_properties(self, client: InsightsClient, mock_insights_sdk: MagicMock) -> None:
        client.capture_event(event_name="test_event", distinct_id="user_123")

        call_kwargs = mock_insights_sdk.capture.call_args[1]
        assert call_kwargs["properties"] == {"$ignore_sent_at": True}


class TestFetchEventByUuid:
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_sends_correct_datastore_query(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_returns_captured_event_when_found(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
        mock_sync_execute.return_value = [
            (
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_returns_none_when_not_found(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
        mock_sync_execute.return_value = []
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_parses_json_string_properties(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
        mock_sync_execute.return_value = [
            (
                uuid.UUID("00000000-0000-0000-0000-000000000123"),
                "test_event",
...
class TestFetchPersonByDistinctId:
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_returns_person_when_found(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
        mock_sync_execute.return_value = [
            (
    ...
    def test_filters_out_deleted_persons_and_distinct_ids(
        self, mock_sync_execute: MagicMock, client: InsightsClient
    ) -> None:
        ...
        can be returned before Datastore merges parts.
        """
        mock_sync_execute.return_value = []
...
class TestFetchEventsByPersonId:
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_resolves_via_person_distinct_id2_final(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
        mock_sync_execute.return_value = []
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_returns_none_when_not_all_uuids_found(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
        mock_sync_execute.return_value = [
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_returns_events_when_all_uuids_found(self, mock_sync_execute: MagicMock, client: InsightsClient) -> None:
...
class TestPollUntilFoundRetries:
    """_poll_until_found must retry on transient Datastore errors instead of crashing."""

    @pytest.mark.parametrize(
        "exception",
        [
            pytest.param(DatastoreAtCapacity(), id="TOO_MANY_SIMULTANEOUS_QUERIES"),
            pytest.param(DatastoreQueryTimeOut(), id="TIMEOUT_EXCEEDED"),
            pytest.param(DatastoreQueryMemoryLimitExceeded(), id="MEMORY_LIMIT_EXCEEDED"),
        ],
    )
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_retries_transient_datastore_errors(
        self, mock_sync_execute: MagicMock, exception: Exception, client: InsightsClient
    ) -> None:
        event_row = (
    ...
    @patch("insights.temporal.ingestion_acceptance_test.client.sync_execute")
    def test_raises_on_non_retryable_datastore_error(
        self, mock_sync_execute: MagicMock, client: InsightsClient
    ) -> None:
