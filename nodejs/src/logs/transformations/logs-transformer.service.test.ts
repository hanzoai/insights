...
const createFunction = async (script: string, overrides: Partial<InsightsFunctionType> = {}): Promise<InsightsFunctionType> => {
    functionCounter++
    return {
...
describe('LogsTransformerService', () => {
    ...
    let manager: jest.Mocked<Pick<InsightsFunctionManagerService, 'getInsightsFunctionsForTeams' | 'getInsightsFunctionIdsForTeams'>>
    let monitoring: jest.Mocked<Pick<InsightsFunctionMonitoringService, 'queueAppMetric' | 'queueLogs' | 'flush'>>
    let config: LogsTransformerConfig

    const setFunctions = (functions: InsightsFunctionType[]) => {
        manager.getInsightsFunctionsForTeams.mockResolvedValue({ [TEAM_ID]: functions })
        manager.getInsightsFunctionIdsForTeams.mockResolvedValue({ [TEAM_ID]: functions.map((f) => f.id) })
    }
    ...
    beforeEach(() => {
        functionCounter = 0
        manager = {
            getInsightsFunctionsForTeams: jest.fn(),
            getInsightsFunctionIdsForTeams: jest.fn(),
        } as any
        monitoring = {
    ...
    it('teamHasTransformations uses the cheap id lookup', async () => {
        manager.getInsightsFunctionIdsForTeams.mockResolvedValue({ [TEAM_ID]: ['some-id'] })
        await expect(service.teamHasTransformations(TEAM_ID)).resolves.toBe(true)

        manager.getInsightsFunctionIdsForTeams.mockResolvedValue({ [TEAM_ID]: [] })
        await expect(service.teamHasTransformations(TEAM_ID)).resolves.toBe(false)
        expect(manager.getInsightsFunctionsForTeams).not.toHaveBeenCalled()
    })
    ...
    describe('HogWatcher integration', () => {
        ...
        it('reports one aggregated observation per function when sampled', async () => {
            ...
            expect(observations[0].insightsFunction.id).toEqual(fn.id)
    ...
    describe('fail-open on infrastructure errors', () => {
        it('passes records through untouched when the function fetch throws', async () => {
            manager.getInsightsFunctionsForTeams.mockRejectedValue(new Error('pg down'))
        ...
        it('reports no transformations when the id fetch throws', async () => {
            manager.getInsightsFunctionIdsForTeams.mockRejectedValue(new Error('pg down'))
