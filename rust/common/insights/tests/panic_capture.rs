...
async fn init_enables_panic_capture() {
    ...
    common_insights::init("panic-test", Some("test-api-key"), &insights.base_url())
        .await
        .expect("insights init");

    common_insights::capture_exception(Arc::new(TestError), [("request_id", json!("req-1"))]);
