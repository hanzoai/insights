...
from posthog.insightsql.database.models import TableNode
from posthog.insightsql.database.s3_table import build_function_call
from posthog.insightsql.database.test.tables import create_aapl_stock_s3_table
...
class TestS3Table(BaseTest):
    ...
    def test_s3_table_select_and_non_s3_join_first(self):
        with override_settings(
            ...
            self._init_database()
            with mock.patch("posthog.insightsql.resolver.USE_GLOBAL_JOINS", True):
                insightsql = self._select(
                    query="SELECT aapl_stock.High, aapl_stock.Low FROM aapl_stock JOIN events ON aapl_stock.High = events.event LIMIT 10",
                    dialect="insightsql",
                )
