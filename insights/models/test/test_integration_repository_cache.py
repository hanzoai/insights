...
from insights.models.integration import GitHubIntegration, GitHubIntegrationError, Integration
from insights.models.integration_repository_cache import (
...
class TestGitHubRepositoryFullCache(BaseTest):
    ...
    def _default_gh_api_responses(self, *, sha: str = "SHA1", readme: str = "# Hello", truncated: bool = False) -> dict:
        return {
            "/repos/insights/insights": {
                "default_branch": "main",
                "description": "Insights open-source",
                "topics": ["analytics", "observability"],
                "archived": False,
                "fork": False,
                "language": "Python",
            },
            "/repos/insights/insights/branches/main": {"commit": {"sha": sha}},
            f"/repos/insights/insights/readme?ref={sha}": self._readme_payload(readme),
            f"/repos/insights/insights/git/trees/{sha}?recursive=1": {
                "tree": [
    ...
    def _patch_gh_api_get(self, responses: dict):
        ...
        return patch(
            "insights.models.integration.GitHubIntegration._gh_api_get", autospec=True, side_effect=_side_effect
        )
    ...
    def test_sync_full_cache_entry_populates_all_fields(self):
        ...
        with self._patch_gh_api_get(responses):
            entry = async_to_sync(self._cache_for(integration).sync_full_cache_entry_async)("insights/insights")

        assert entry.full_name == "insights/insights"
        assert entry.team_id == self.team.id
        assert entry.description == "Insights open-source"
    ...
    def test_sync_full_cache_entry_returns_cached_within_ttl_without_api_calls(self):
        ...
        with patch("insights.models.integration.GitHubIntegration._gh_api_get", autospec=True) as mock_gh_api_get:
            entry = async_to_sync(self._cache_for(integration).sync_full_cache_entry_async)("insights/insights")
    ...
    def test_sync_full_cache_entry_skips_readme_refetch_when_sha_unchanged(self):
        ...
        responses.pop("/repos/insights/insights/readme?ref=SHA1")
        responses.pop("/repos/insights/insights/git/trees/SHA1?recursive=1")

        with self._patch_gh_api_get(responses):
            entry = async_to_sync(self._cache_for(integration).sync_full_cache_entry_async)("insights/insights")

        assert entry.description == "Insights open-source"  # metadata updated
        assert entry.primary_language == "Python"
    ...
    def test_sync_full_cache_entry_handles_readme_fetch_failure_gracefully(self):
        ...
        def _side_effect(self_, path, *, endpoint=None, timeout=10):
            ...
            if path.startswith("/repos/insights/insights/readme"):
                raise GitHubIntegrationError("readme missing", status_code=404)
            return responses[path]

        with patch(
            "insights.models.integration.GitHubIntegration._gh_api_get",
            autospec=True,
            ...
            cached = async_to_sync(self._cache_for(integration).sync_full_cache_entry_async)("insights/insights")
    ...
    def test_sync_full_cache_entry_propagates_rate_limited_readme(self):
        ...
        with patch(
            ...
            with pytest.raises(GitHubRateLimitError) as excinfo:
                async_to_sync(self._cache_for(integration).sync_full_cache_entry_async)("insights/insights")
    ...
    def test_sync_full_cache_entry_url_encodes_branch_name_with_slash(self):
        ...
        responses = {
            ...
            "/repos/insights/insights/branches/release%2F1.0": {"commit": {"sha": "SHA1"}},
            "/repos/insights/insights/readme?ref=SHA1": self._readme_payload("# Hello"),
            "/repos/insights/insights/git/trees/SHA1?recursive=1": {
    ...
    def test_sync_full_cache_entry_raises_on_missing_branch_sha(self):
        integration = self._create_integration()
        responses = self._default_gh_api_responses()
        responses["/repos/insights/insights/branches/main"] = {"commit": {}}  # missing sha
    ...
    def test_sync_full_cache_entry_rejects_bare_name(self):
        ...
        with pytest.raises(ValueError, match="'owner/repo' format"):
            async_to_sync(self._cache_for(integration).sync_full_cache_entry_async)("insights")
    ...
    def test_sync_full_cache_runs_with_bounded_concurrency(self):
        ...
        repos = [{"full_name": f"insights/repo-{i}"} for i in range(20)]
    ...
    def test_sync_full_cache_returns_exceptions_inline(self):
        ...
        async def fake_entry_async(full_name, *, path_log=None):
            if full_name == "insights/broken":
                raise GitHubIntegrationError("boom")
            return MagicMock(full_name=full_name)

        async def fake_list():
            return [{"full_name": "insights/ok"}, {"full_name": "insights/broken"}]
    ...
    def test_sync_full_cache_evicts_orphans_not_in_light_cache(self):
        integration = self._create_integration()
        # Seed: one repo still in the light list ("insights/insights") and one orphan ("insights/gone").
        IntegrationRepositoryCacheEntry.objects.create(
            integration=integration,
            team=self.team,
            full_name="insights/insights",
        ...
        async def fake_list():
            return [{"full_name": "insights/insights"}]  # insights/gone removed
        ...
        assert remaining == {"insights/insights"}
    ...
    def test_sync_full_cache_entry_lowercases_mixed_case_input(self):
        ...
        with self._patch_gh_api_get(responses):
            first = async_to_sync(cache_obj.sync_full_cache_entry_async)("Insights/Insights")
            second = async_to_sync(cache_obj.sync_full_cache_entry_async)("insights/insights")

        assert first.full_name == "insights/insights"
    ...
    def test_sync_full_cache_lowercases_orphan_eviction_set(self):
        ...
        async def fake_list():
            # GitHub returns "Insights/insights" — must still match the lowercased seed row.
            return [{"full_name": "Insights/insights"}]
    ...
    def test_sync_full_cache_evicts_cross_source_rows_for_team(self):
        # Team upgrade scenario: Insights Desktop (UserIntegration) hydrated rows for this team, then
        # the team connected a team-level Integration. Cascade now picks the Integration; the old
        # UserIntegration row would otherwise linger and surface as a duplicate to InsightsQL queries.
...
class TestSyncFullCacheLock(BaseTest):
    ...
    # In TEST mode `insights.redis.get_client()` and `get_async_client()` return separate
    ...
    def test_acquires_lock_and_releases_on_success(self):
        ...
        from insights import redis as insights_redis

        async def run_and_check():
            await repo_cache.sync_full_cache()
            redis = insights_redis.get_async_client()
    ...
    def test_follower_waits_then_runs_after_leader_releases(self):
        ...
        from insights import redis as insights_redis
        from insights.models.integration_repository_cache import _acquire_sync_lock
        ...
        with patch("insights.models.integration_repository_cache.SYNC_LOCK_POLL_INTERVAL_SECONDS", 0.05):
    ...
    def test_follower_times_out_when_leader_never_releases(self):
        ...
        with (
            patch("insights.models.integration_repository_cache.SYNC_LOCK_POLL_INTERVAL_SECONDS", 0.05),
            patch("insights.models.integration_repository_cache.SYNC_LOCK_MAX_WAIT_SECONDS", 0.15),
        ):
    ...
    def test_heartbeat_cancels_body_when_lease_is_lost(self):
        ...
        with patch("insights.models.integration_repository_cache.SYNC_LOCK_HEARTBEAT_INTERVAL_SECONDS", 0.05):
