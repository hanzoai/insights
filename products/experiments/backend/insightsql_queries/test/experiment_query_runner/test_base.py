...
from products.experiments.backend.insightsql_queries.test.experiment_query_runner.utils import (
...
class TestExperimentQueryRunner(ExperimentQueryRunnerBaseTest):
    def test_deleted_feature_flag_tombstone_uses_original_key_for_query_builder(self):
        ...
        with patch(
            "products.experiments.backend.insightsql_queries.experiment_query_runner.ExperimentQueryBuilder"
        ) as builder_class:
    ...
    def test_query_runner_with_insightsql_aggregation_expressions(self):
        ...
        metric_sum = ExperimentMeanMetric(
            source=EventsNode(
                ...
                math_insightsql="sum(toFloat(properties.revenue) - toFloat(properties.cost))",
            ),
        )
    ...
    def test_query_runner_with_insightsql_aggregation_end_to_end(self):
        ...
        metric_avg = ExperimentMeanMetric(
            source=EventsNode(
                event="purchase", math=ExperimentMetricMathType.INSIGHTSQL, math_insightsql="avg(properties.amount)"
            ),
        )
    ...
    def test_query_runner_with_insightsql_fallback_to_sum(self):
        ...
        metric_simple = ExperimentMeanMetric(
            source=EventsNode(
                event="purchase",
                math=ExperimentMetricMathType.INSIGHTSQL,
                math_insightsql="properties.price",  # No aggregation function, should default to sum
            ),
        )
    ...
    def test_mean_metric_with_parametric_aggregation(self):
        ...
        metric = ExperimentMeanMetric(
            source=EventsNode(
                ...
                math=ExperimentMetricMathType.INSIGHTSQL,
                math_insightsql="quantile(0.90)(properties.margin)",
            )
        )
