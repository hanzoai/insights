...
/** Pull the InsightsQL text out of an insight `query` field activity change (a DataVisualizationNode). */
...
export interface insightHistoryLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        activity: (activityPage: InsightActivityPage) => ActivityLogItem[]
        historyComplete: (activityPage: InsightActivityPage) => boolean
        versions: (activity: ActivityLogItem[]) => InsightQueryVersion[]
    }
}
...
export const insightHistoryLogic = kea<insightHistoryLogicType>([
    ...
    selectors({
        activity: [(s) => [s.activityPage], (activityPage: InsightActivityPage) => activityPage.items],
        historyComplete: [(s) => [s.activityPage], (activityPage: InsightActivityPage) => activityPage.complete],
        // Newest first, only edits that changed the SQL — each one is a "version"
        versions: [
            (s) => [s.activity],
            (activity: ActivityLogItem[]): InsightQueryVersion[] =>
                activity
                    .map((logItem): InsightQueryVersion | null => {
                        ...
                        return {
                            ...
                            authorName: logItem.is_system ? 'Insights' : fullName(logItem.user),
