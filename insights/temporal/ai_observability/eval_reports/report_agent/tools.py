...
All query tools hit Datastore live via InsightsQL since the dataset is too large/variable
...
from insights.temporal.ai_observability.eval_reports.output_types import (
...
from insights.temporal.ai_observability.trace_summarization.constants import (
    MAX_TRACE_EVENTS_LIMIT,
    MAX_TRACE_PROPERTIES_SIZE,
)
from insights.temporal.ai_observability.trace_summarization.fetch_and_format import _fetch_and_format_trace
...
def _ch_ts(iso_str: str) -> datetime:
    ...
    Returns a datetime (not a string) so InsightsQL's printer serializes it as
    ...
    string here caused Datastore to coerce via the team's timezone, silently
...
def _is_retriable_ch_error(error: Exception) -> bool:
    return isinstance(error, RETRIABLE_CH_ERRORS) or (
        isinstance(error, DatastoreQueryMemoryLimitExceeded) and not error.is_per_query_limit
    )
...
def _execute_ch_query_with_retry(
    ...
    *,
    ...
    """Run a Datastore-backed callable, retrying transient errors with exponential backoff + jitter.
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(
                "llma_eval_reports_datastore_retry",
                error=str(error),
...
def _execute_insightsql(team_id: int, query_str: str, placeholders: dict | None = None) -> list[list]:
    """Execute a InsightsQL query against `events` and return results.
    ...
    with tags_context(product=Product.LLM_ANALYTICS, feature=Feature.ENRICHMENT, team_id=team_id):
        result = _execute_ch_query_with_retry(
            lambda: execute_insightsql_query(
...
def _execute_insightsql_via_ai_events(team: "Team", query_str: str, placeholders: dict | None = None) -> list[list]:
    """Execute a InsightsQL query written against `insights.ai_events` with the events-table fallback.
    ...
    should keep using `_execute_insightsql` against `events`.
    """
    from insights.insightsql.parser import parse_select

    from insights.datastore.query_tagging import Feature, Product, tags_context
    from insights.insightsql_queries.ai.ai_table_resolver import query_ai_events
...
def list_all_eval_results(
    ...
    count_rows = _execute_insightsql(
        team_id,
        f"""
        SELECT count() as cnt
        FROM events
...
def sample_generation_details(
    ...
    # anything that isn't a canonical UUID before passing into InsightsQL.
    ...
    from insights.insightsql_queries.ai.trace_id_resolver import resolve_trace_ids_for_generation_uuids
    from insights.models import Team
    ...
    rows = _execute_insightsql_via_ai_events(
        team,
        """
        SELECT
            ...
            input_state,
            output_state
        FROM insights.ai_events AS ai_events
...
def get_generation_detail(
    ...
    gen_rows = _execute_insightsql_via_ai_events(
        team,
        """
        SELECT
            toString(uuid) as generation_id,
...
def get_generation_text_repr(
    ...
    Insights UI summarization panel. Use this when raw JSON from
