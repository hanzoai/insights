...
from insights.api.file_system.deletion import undo_delete
...
class TestFileSystemAPI(APIBaseTest):
    ...
    def test_search_insights_function_types(self):
        """
        Ensure the search functionality is working on the 'path' field.
        """
        FileSystem.objects.create(
            team=self.team, path="Analytics/Report 1", type="insights_function/source", created_by=self.user
        )
        FileSystem.objects.create(
            team=self.team, path="Analytics/Report 2", type="insights_function/destination", created_by=self.user
        )
        FileSystem.objects.create(team=self.team, path="Random/Other File", type="misc", created_by=self.user)
    ...
    def test_assure_parent_folders(self):
        ...
        from insights.api.file_system.file_system import FileSystemViewSet
    ...
    def test_meta_sync_create_or_update_file(self):
        ...
        from insights.models.file_system.file_system import FileSystem, create_or_update_file
...
class TestFileSystemAPIAdvancedPermissions(APIBaseTest):
    ...
    def test_destroy_requires_editor_access_to_the_backing_object(self, target, mock_flag):
        ...
        response = self.client.delete(f"/v1/projects/{self.team.id}/file_system/{delete_id}/")
    ...
    def test_none_access_hides_entries_whose_ref_is_a_short_id(self, mock_flag):
        ...
        list_response = self.client.get(f"/v1/projects/{self.team.id}/file_system/")
        ...
        delete_response = self.client.delete(f"/v1/projects/{self.team.id}/file_system/{entry.id}/")
    ...
    def test_destroy_requires_editor_access_in_the_objects_own_environment(self, mock_flag):
        ...
        response = self.client.delete(f"/v1/projects/{self.team.id}/file_system/{entry.id}/")
    ...
    def test_destroy_does_not_collapse_access_levels_across_teams_sharing_a_ref(self, mock_flag):
        ...
        response = self.client.delete(f"/v1/projects/{self.team.id}/file_system/{folder.id}/")
    ...
    def test_destroy_does_not_translate_a_short_id_through_another_teams_object(self, mock_flag):
        ...
        response = self.client.delete(f"/v1/projects/{self.team.id}/file_system/{victim_entry.id}/")
    ...
    @parameterized.expand([("integer_pk", "dashboard"), ("uuid_pk", "insights_function/destination")])
    ...
    def test_list_queries_do_not_scale_with_environment_count(self, mock_flag):
        ...
        with CaptureQueriesContext(connection) as one_environment:
            self.client.get(f"/v1/projects/{self.team.id}/file_system/")
    ...
    def test_list_includes_users_array(self):
        ...
        second_user = User.objects.create_and_join(self.organization, "second@hanzo.ai", "testpass")
...
class TestFileSystemProjectScoping(APIBaseTest):
    """
    - Any *non-insights_function* item belonging to **any team in the same project**
      must be visible & editable.
    - Items whose type starts with "insights_function/" are **team-scoped** and
    ...
    def setUp(self):
        ...
        self.script_t1 = FileSystem.objects.create(
        ...
        self.script_t3 = FileSystem.objects.create(
    ...
    def test_list_scopes_correctly(self):
        ...
        # Non-insights_function items from *both* teams in the project
        ...
        # insights_function only from the *current* team
        self.assertIn("Functions/Script-T1", paths)
        self.assertNotIn("Functions/Script-T2", paths)
        self.assertNotIn("Functions/Script-T3", paths)
    ...
    def test_retrieve_insights_function_from_other_team_is_forbidden(self):
        url = f"/v1/projects/{self.team.id}/file_system/{self.script_t2.id}/"
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)
...
@pytest.mark.django_db
class TestMoveRepairsLeftoverInsightsFunctions(APIBaseTest):
    """
    When a folder is moved we copy / delete only the rows that belong to the
    *current* team. `insights_function/*` rows owned by *other* teams stay where they
    ...
    def test_move_recreates_folder_for_leftover_items(self):
        ...
        self.script_t2.refresh_from_db()
        self.assertEqual(self.script_t2.path, "Shared/Script-func.js")
        ...
        self.assertTrue(folder_t2_qs.exists(), "Left-behind insights_function lost its parent folder")
...
@pytest.mark.django_db
class TestDestroyRepairsLeftoverInsightsFunctions(APIBaseTest):
    def setUp(self):
        super().setUp()
    ...
    def test_destroy_folder_repairs_for_leftover_items(self):
        ...
        self.assertTrue(
            FileSystem.objects.filter(id=self.script_t2.id).exists(),
    ...
    def test_delete_and_restore_activity_logs_for_supported_file_types(self):
        cases: list[RestoreTestCase] = [
            ...
            {
                "file_type": f"insights_function/{InsightsFunctionType.DESTINATION}",
                "scope": "InsightsFunction",
                "factory": self._prepare_insights_function_case,
                "supports_restore": True,
    ...
    def _prepare_insights_function_case(self):
        ...
        fs_entry = self._ensure_file_system_entry(
            file_type=file_type, ref=str(insights_function.id), fallback_name=str(insights_function.id)
        )
        return {
            "fs_entry": fs_entry,
            "item_id": str(insights_function.id),
            "ref": str(insights_function.id),
            "path": fs_entry.path,
        }
