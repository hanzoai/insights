...
Resolver, linter, and formatter for Insights's distributed `owners.yaml` ownership model.
...
```bash
uvx --from "git+https://github.com/Insights/insights#subdirectory=tools/owners" owners lint
```

Pin to a commit for CI so the resolver semantics can't shift under you: append `@<sha>` to the URL (`...insights@<sha>#subdirectory=tools/owners`).
