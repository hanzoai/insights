    ...
    F: FnOnce() -> Result<Value, UnhandledError>,
{
    ...
    // lookup, `props()` serialization, or ScriptVM rule evaluation below.
        ...
        match rule.try_match(&props) {
            ...
            Err(VmError::OutOfResource(resource)) if resource == "steps" => {
                tracing::warn!(
                    ...
                    "grouping rule exceeded ScriptVM step budget for this event, skipping"
                );
                continue;
            }
...
mod test {
    ...
    async fn test_grouping_rules(db: PgPool) {
        ...
        let matched =
            evaluate_grouping_rules(&ctx.insights_pool, test_team_id, &ctx.team_manager, || {
                Ok(props.clone())
            })
            .await
            .unwrap();
