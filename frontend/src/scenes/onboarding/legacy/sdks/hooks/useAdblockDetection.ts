...
/**
 * Detects whether Insights requests are being blocked (e.g. by an adblocker).
 *
 * Two checks are performed:
 *  1. `window.insights?.__loaded` – if insights-js failed to initialize the
 *     property will be falsy.
 *  2. A lightweight fetch to the Insights ingestion `/decide/` endpoint – a
...
export function useAdblockDetection(delayMs: number = 20_000): AdblockDetectionResult {
    ...
    useEffect(() => {
        ...
        const detect = async (): Promise<AdblockDetectionResult> => {
            // Check 1 – insights-js loaded flag
            ...
            try {
                await fetch('https://us.i.hanzo.ai/decide/?v=3', {
                    method: 'POST',
                    mode: 'no-cors',
                    body: JSON.stringify({}),
                })
            ...
            detect()
                .then((r) => {
                    if (!cancelled) {
                        ...
                        setResult(r)
                        insights.capture('onboarding adblock detection completed', { status: r })
                    }
                })
                .catch(() => {
                    if (!cancelled) {
                        detectionDone.current = true
                        setResult('blocked')
                        insights.capture('onboarding adblock detection completed', { status: 'blocked' })
                    }
                })
        }, delayMs)
