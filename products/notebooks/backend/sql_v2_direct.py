"""The SQLV2 direct lane: pure-InsightsQL runs executed with no sandbox.

A SQL node whose refs are all insightsql (node_type "insightsql" after `resolve_sql_node_run`)
...
from insights.datastore.client.execute_async import (
    QueryNotFoundError,
    cancel_query,
...
# insightsql run, whose callback should land well within this window).
...
def _wrap_insightsql_page_query(query: str, limit: int, offset: int) -> str:
    ...
    The inner query is validated InsightsQL and the wrapper is re-parsed as InsightsQL downstream, so
...
def apply_page_bounds(query: str, limit: int, offset: int) -> str:
    """Bound a InsightsQL query to `limit` rows at `offset`, preserving Datastore limit pushdown.

    A query with no LIMIT of its own gets one appended to its own outermost SELECT, so
    Datastore can push it into an aggregated view like `persons`: an unbounded
    ...
    Everything else falls back to `_wrap_insightsql_page_query`: a query with its own LIMIT already
    ...
    # subquery), so normalize it away for both lanes. InsightsQL is single-statement, so this only
    ...
    try:
        ...
        return _wrap_insightsql_page_query(query, limit, offset)
...
def enqueue_direct_run(team: "Team", user: "User | None", run: NotebookNodeRun) -> None:
    """Enqueue a direct (insightsql) run on the async query manager.

    The same engine the data plane rides for sandbox fetches — user-threaded InsightsQL
    ...
    with tags_context(product=Product.NOTEBOOKS, feature=Feature.QUERY, team_id=team.id):
        enqueue_process_query_task(
            ...
            query_json={"kind": "InsightsQLQuery", "query": bounded},
...
def cancel_direct_run(run: NotebookNodeRun) -> None:
    """Stop a direct (insightsql) run's query: revoke it if still queued, else KILL it on Datastore.
...
def _query_status_timings(status: Any) -> dict[str, float]:
    ...
    `queued_s` is enqueue -> Celery pickup (slot/queue wait); `datastore_s` is pickup ->
    completion — InsightsQL compile plus the Datastore execution, the closest server-side
    ...
    if pickup_time and end_time:
        timings["datastore_s"] = round(max((end_time - pickup_time).total_seconds(), 0.0), 3)
    return timings


def sync_direct_run(run: NotebookNodeRun) -> list[list[Any]] | None:
    """Advance a direct (insightsql) run from its async query status and return its transient rows.
    ...
    Returns None when no rows are available — including for kernel-executed insightsql runs,
