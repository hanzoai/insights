...
- **Stream logic**: `maxThreadLogic.tsx` (LangGraph stream loop) → `runStreamLogic` (SSE; in `products/insights_ai/frontend/logics/`)
- **Activity renderer**: `components/Activity/LangGraphActivity.tsx` → `RunActivity` (in `products/insights_ai/frontend/components/`)
...
- **Tool widgets**: `messages/` (LangGraph presenters) → the surface's `toolRegistry`. The Insights product renderers (insight/dashboard/recordings/error-tracking/notebook/query) now live in `products/insights_ai/frontend/components/tool/widgets` and self-register there; Max is a consumer.
...
**`products/insights_ai/frontend/AGENTS.md`**, where that code now lives. Read it before touching the surface.
...
## 4. The sandbox surface lives in `products/insights_ai/frontend`

The conversation-agnostic sandbox run surface (stream logic, thread/tool/permission/composer components,
policy, wire types) **no longer lives under `scenes/max`** — it's the composable Insights AI agent-run
library at `products/insights_ai/frontend`, consumed here through its tiered `api/<module>` facade
(`import { ... } from 'products/insights_ai/frontend/api/readableRun'` / `api/runSurface` / `api/primitives` /
...
path (including its `messages/*` presenters). The **Insights product tool renderers** (insight, dashboard,
recordings, error-tracking, notebook, query) now live in `products/insights_ai/frontend/components/tool/widgets`
...
The first three now live in `products/insights_ai/frontend` (the shared surface) — make the change there, not in `scenes/max`:

- **A new thread-item type** → `products/insights_ai/frontend/types/streamTypes.ts` (the `ThreadItem` union), handle it in `foldLogToThread`, and add a memoized leaf renderer wired into `ThreadView`/`ThreadRow`.
- **A new permission affordance / auto-approval rule** → `products/insights_ai/frontend/policy/toolPolicy.ts`, then surface it through `PermissionInput`.
- **New stream telemetry** → a guarded `insights.capture` in the relevant `runStreamLogic` listener (fire-once, suppressed on replay).
- **A new product tool renderer** (renders a Insights entity) → `products/insights_ai/frontend/components/tool/widgets/*` + register it in `widgets/registerDataToolRenderers` (the surface self-registers; every consumer, including Max's sandbox path, picks it up).
- **New context the agent should see** → register it from the owning product via the surface's context seam: `useAttachedContext` / `AttachedContextProvider` / `attachedContextLogic.registerContext` (see `products/insights_ai/frontend/AGENTS.md` §3). Not `MaxUIContext`, and no new fields on the legacy stores. Scenes now register their on-screen context natively into that seam, and `maxThreadLogic`'s sandbox send merges the seam's items into `attached_context` — that bridge lives here and is deleted with `scenes/max`.
