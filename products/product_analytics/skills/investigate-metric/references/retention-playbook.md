...
cohort for the affected period (`insights:cohorts-create` / `-list`), then run
`insights:query-trends` on the activity event filtered to that cohort.

## 3. Split the dropout

Run `insights:query-lifecycle` scoped to the affected cohort to separate "never retained"
