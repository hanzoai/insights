...
from insights.scopes import API_SCOPE_OBJECTS, INTERNAL_API_SCOPE_OBJECTS, APIScopeObject
...
# We need to restrict this for InsightsQL access control which uses `NOT IN (...)`
...
class UserAccessControl:
    ...
    def _cached_access_controls(self) -> list[_AccessControl]:
        ...
        if not EE_AVAILABLE or not self._team:
            return []
        # Annotate with team.organization_id only — avoids fetching the full ~150-column insights_team row.
        return list(
    ...
    @property
    def user(self) -> User:
        """The principal this access control was built for. Callers that run InsightsQL on a user's behalf
    ...
    def blocked_resource_ids_by_scope(self) -> dict[APIScopeObject, set[str]]:
        ...
        Consumed by InsightsQL object-level access control (schema filtering / printer guard) and by
