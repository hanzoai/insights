...
pub struct EmittedVersions {
    ...
    /// greater-than, and Datastore collapses person rows by the highest
...
impl EmittedVersions {
    ...
    fn raise(&self, partition: u32, key: PersonCacheKey, version: i64) {
            ...
            && !self.floors.contains_key(&(partition, key.clone()))
        {
            ...
            counter!("personinsights_leader_unresolved_versions_spilled_total").increment(1);
            return;
        }
        ...
        counter!("personinsights_leader_unresolved_versions_total").increment(1);
        gauge!("personinsights_leader_unresolved_versions").set(self.floors.len() as f64);
    }
