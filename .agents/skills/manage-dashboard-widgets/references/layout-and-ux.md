...
3. Run `insightscli test frontend/src/scenes/dashboard/tileLayouts.test.ts`.
...
- Title: **Add widget**; description: **Bring context from your different Insights products into one dashboard.**
...
- Populated dashboard: `DashboardHeaderActions` — `Menu` item `tag: 'new'`
- Empty dashboard: `EmptyDashboardComponent` — inline `<Tag type="success" size="small">NEW</Tag>` on the Widget menu item
