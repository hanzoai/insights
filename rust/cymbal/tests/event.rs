...
async fn insert_grouping_rule(db: &PgPool) -> Uuid {
    ...
    sqlx::query(
        r#"
            INSERT INTO insights_errortrackinggroupingrule
...
async fn existing_issue_under_older_version_wins(db: PgPool) {
    ...
    sqlx::query("UPDATE insights_errortrackingissuefingerprintv2 SET fingerprint = $1 WHERE fingerprint = $2")
        .bind(&v1_value)
        .bind(&v2_value)
        .execute(&harness.db)
        .await
...
async fn legacy_order_issue_wins_via_legacy_version(db: PgPool) {
    ...
    input.properties["$lib"] = json!("insights-go");
    ...
    let canonical_rows: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM insights_errortrackingissuefingerprintv2 WHERE fingerprint = $1",
    )
    .bind(&canonical_value)
