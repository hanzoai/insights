...
# Simple 1:1 function name renames (Datastore name → MySQL name)
MYSQL_FUNCTION_RENAMES: dict[str, str] = {
    ...
    # Datastore formatDateTime and MySQL DATE_FORMAT share the %-based format
...
def _handle_to_day_of_week(args: list[str]) -> str:
    # Datastore toDayOfWeek defaults to ISO numbering (Monday = 1); MySQL WEEKDAY is Monday = 0.
...
def _handle_match(args: list[str]) -> str:
    # The 'c' flag forces case-sensitive matching, mirroring Datastore's match().
...
def _handle_position(args: list[str]) -> str:
    # Datastore position(haystack, needle[, start]) → MySQL LOCATE(needle, haystack[, start])
    if len(args) == 3:
...
# Complex handlers: Datastore function name → callable(list[rendered_arg_strings]) → SQL string
#
...
# Any Datastore function NOT in handlers, renames, or this set raises a compile-time error.
