...
import type { InsightsFunctionTemplateType, UserBasicType } from '../../../../../frontend/src/types'
...
import { getInsightsFlowStep } from './steps/InsightsFlowSteps'
import { CyclotronInputType, StepViewNodeHandle } from './steps/types'
import type { DropzoneNode, InsightsFlow, InsightsFlowAction, InsightsFlowActionEdge, InsightsFlowActionNode } from './types'
import type { InsightsFlowEdge } from './types'

const getEdgeId = (edge: InsightsFlow['edges'][number]): string =>
    `${edge.from}->${edge.to} ${edge.type} ${edge.index ?? ''}`.trim()

/**
 * Helper to get branch label with custom name fallback
 */
const getBranchLabel = (action: InsightsFlowAction | undefined, edge: InsightsFlow['edges'][0]): string => {
    if (!action) {
    ...
    switch (action.type) {
        case 'wait_until_condition': {
            const waitAction = action as Extract<InsightsFlowAction, { type: 'wait_until_condition' }>
            const customName = waitAction.config.condition?.name
            return customName || 'If condition matches'
        }
        case 'random_cohort_branch': {
            const cohortAction = action as Extract<InsightsFlowAction, { type: 'random_cohort_branch' }>
            const cohort = cohortAction.config.cohorts?.[edge.index || 0]
            return cohort?.name || `If cohort #${(edge.index || 0) + 1} matches`
        }
        case 'conditional_branch': {
            const branchAction = action as Extract<InsightsFlowAction, { type: 'conditional_branch' }>
...
export function computeMoveEdges(
    ...
    movingNodeId: string,
    targetEdge: InsightsFlow['edges'][0],
    isBranchJoinDropzone: boolean
): InsightsFlow['edges'] | null {
...
export type CreateActionType = Pick<InsightsFlowAction, 'type' | 'config' | 'name' | 'description'> & {
    branchEdges?: number
    output_variable?: InsightsFlowAction['output_variable']
...
export interface flowEditorLogicValues {
    categories: MessageCategory[]; // optOutCategoriesLogic
    categoriesLoading: boolean; // optOutCategoriesLogic
    edgesByActionId: Record<string, InsightsFlowEdge[]>; // workflowLogic
    insightsFunctionTemplatesById: Record<string, InsightsFunctionTemplateType>; // workflowLogic
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface flowEditorLogicActions {
    loadCategories: () => {
        value: true;
    }; // optOutCategoriesLogic
    loadWorkflowSuccess: (originalWorkflow: InsightsFlow | {
        ...
        actions: ({
            ...
            config: {
                inputs: Record<string, {
                    ...
                    templating?: "liquid" | "script" | undefined;
                    value: any;
                }>;
                mappings?: {
                    ...
                    inputs_schema?: {
                        ...
                        type: "boolean" | "choice" | "customer_analytics_account_properties" | "customer_analytics_account_relationships" | "dictionary" | "email" | "insights_assignee" | "insights_business_hours" | "insights_ticket_tags" | "integration" | "integration_field" | "integration_multi" | "json" | "native_email" | "non_failure_status_codes" | "number" | "string";
                    }[] | undefined;
                    name: string;
                }[] | undefined;
                template_id: string;
    ...
    setWorkflowAction: (actionId: string, action: InsightsFlowAction) => {
        action: InsightsFlowAction;
        actionId: string;
    }; // workflowLogic
    setWorkflowActionEdges: (actionId: string, edges: {
        from: string;
        index?: number | undefined;
        to: string;
        type: "branch" | "continue";
    ...
    loadActionMetricsById: (params: Pick<AppMetricsTotalsRequest, "appSource" | "appSourceId" | "dateFrom" | "dateTo">, timezone: string) => {
        params: Pick<AppMetricsTotalsRequest, "appSource" | "appSourceId" | "dateFrom" | "dateTo">;
        timezone: string;
    };
    loadActionMetricsByIdFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    loadActionMetricsByIdSuccess: (actionMetricsById: Record<string, InsightsFlowEditorActionMetrics>, payload?: {
        params: Pick<AppMetricsTotalsRequest, "appSource" | "appSourceId" | "dateFrom" | "dateTo">;
        timezone: string;
    }) => {
    ...
    resetFlowFromInsightsFlow: (flow: InsightsFlow) => {
        flow: InsightsFlow;
    };
    setAnimatingEdgePair: (from: string, to: string) => {
        from: string;
        to: string;
    };
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface flowEditorLogicMeta {
    key: string;
    __keaTypeGenInternalSelectorTypes: {
        ...
        selectedNodeCanBeDeleted: (selectedNode: InsightsFlowActionNode | null, nodes: InsightsFlowActionNode[], edges: InsightsFlowActionEdge[]) => boolean;
        selectedNodeCanBeCopiedOrMoved: (selectedNode: InsightsFlowActionNode | null, selectedNodeCanBeDeleted: boolean) => boolean;
    };
}

export type flowEditorLogicType = MakeLogicType<flowEditorLogicValues, flowEditorLogicActions, WorkflowLogicProps, flowEditorLogicMeta>;

export const flowEditorLogic = kea<flowEditorLogicType>([
    props({} as WorkflowLogicProps),
    path((key) => ['scenes', 'insightsflows', 'flowEditorLogic', key]),
    key((props) => `script-flow-editor-${props.id}`),
    connect(() => ({
        values: [
            workflowLogic,
            ['workflow', 'edgesByActionId', 'insightsFunctionTemplatesById'],
            optOutCategoriesLogic(),
    ...
    actions({
        onEdgesChange: (edges: EdgeChange<InsightsFlowActionEdge>[]) => ({ edges }),
        onNodesChange: (nodes: NodeChange<InsightsFlowActionNode>[]) => ({ nodes }),
        onNodesDelete: (deleted: InsightsFlowActionNode[]) => ({ deleted }),
        setNodes: (nodes: InsightsFlowActionNode[]) => ({ nodes }),
        setDropzoneNodes: (dropzoneNodes: DropzoneNode[]) => ({
            dropzoneNodes,
        }),
        showDropzones: true,
        hideDropzones: true,
        setNodesRaw: (nodes: InsightsFlowActionNode[]) => ({ nodes }),
        setEdges: (edges: InsightsFlowActionEdge[]) => ({ edges }),
        setSelectedNodeId: (selectedNodeId: string | null) => ({ selectedNodeId }),
        resetFlowFromInsightsFlow: (flow: InsightsFlow) => ({ flow }),
        setReactFlowInstance: (reactFlowInstance: ReactFlowInstance<Node, Edge>) => ({
            reactFlowInstance,
        }),
        ...
        setNodeToBeAdded: (nodeToBeAdded: CreateActionType | InsightsFlowActionNode | null) => ({ nodeToBeAdded }),
        setHighlightedDropzoneNodeId: (highlightedDropzoneNodeId: string | null) => ({ highlightedDropzoneNodeId }),
        setMode: (mode: InsightsFlowEditorMode) => ({ mode }),
        setAnimatingEdgePair: (from: string, to: string) => ({ from, to }),
        clearAnimatingEdgePair: true,
        startCopyingNode: (node: InsightsFlowActionNode) => ({ node }),
        stopCopyingNode: true,
        copyNodeToHighlightedDropzone: true,
        startMovingNode: (node: InsightsFlowActionNode) => ({ node }),
        stopMovingNode: true,
    ...
    reducers(() => ({
        mode: [
            'build' as InsightsFlowEditorMode,
            {
                setMode: (_, { mode }) => mode,
            },
        ],
        ...
        nodeToBeAdded: [
            null as CreateActionType | InsightsFlowActionNode | null,
            {
    ...
    selectors({
        nodesById: [
            (s) => [s.nodes],
            (nodes: InsightsFlowActionNode[]): Record<string, InsightsFlowActionNode> => {
                return nodes.reduce(
                    (acc, node) => {
                        acc[node.id] = node
                        return acc
                    },
                    {} as Record<string, InsightsFlowActionNode>
                )
            },
        ],
        selectedNode: [
            (s) => [s.nodes, s.selectedNodeId],
            (nodes: InsightsFlowActionNode[], selectedNodeId: string | null) => {
                return nodes.find((node) => node.id === selectedNodeId) ?? null
            },
        ],
        selectedNodeCanBeDeleted: [
            (s) => [s.selectedNode, s.nodes, s.edges],
            (selectedNode: InsightsFlowActionNode | null, nodes: InsightsFlowActionNode[], edges: InsightsFlowActionEdge[]) => {
                if (!selectedNode) {
                    return false
                }
        ...
        selectedNodeCanBeCopiedOrMoved: [
            ...
            (selectedNode: InsightsFlowActionNode | null, selectedNodeCanBeDeleted: boolean) => {
    ...
    loaders(() => ({
        actionMetricsById: [
            null as Record<string, InsightsFlowEditorActionMetrics> | null,
            {
                loadActionMetricsById: async ({ params, timezone }, breakpoint) => {
                    ...
                    const res: Record<string, InsightsFlowEditorActionMetrics> = {}
    ...
    listeners(({ values, actions }) => {
        ...
        return {
            ...
            resetFlowFromInsightsFlow: ({ flow }) => {
                try {
                    const edges: InsightsFlowActionEdge[] = flow.edges.map((edge) => {
                        const isOnlyEdgeForNode = flow.edges.filter((e) => e.from === edge.from).length === 1
                        const edgeSourceAction = flow.actions.find((action) => action.id === edge.from)
                    ...
                    const nodes: InsightsFlowActionNode[] = flow.actions.map((action: InsightsFlowAction) => {
                        const step = getInsightsFlowStep(action, values.insightsFunctionTemplatesById)
                        ...
                        const migratedAction: InsightsFlowAction =
                            !step && action.type.startsWith('function_')
                                ? ({ ...action, type: 'function' } as InsightsFlowAction)
                                : action
                    ...
                    console.error('Error resetting flow from script flow', error)
            ...
            onNodesDelete: ({ deleted }) => {
                ...
                const updatedEdges = values.workflow.edges
                    .map((flowEdge) => {
                        if (deletedNodeIds.includes(flowEdge.to)) {
                            // Find the deleted node
                            const deletedNode = deleted.find((node) => node.id === flowEdge.to)
                            if (deletedNode) {
                    ...
                    .filter(
                        (flowEdge) =>
                            !deletedNodeIds.includes(flowEdge.from) && !deletedNodeIds.includes(flowEdge.to)
                    )
            ...
            onDrop: ({ event }) => {
                ...
                if (values.nodeToBeAdded && dropzoneNode) {
                    ...
                    // Check if nodeToBeAdded is a InsightsFlowActionNode (has 'data' property) or CreateActionType
                    const isInsightsFlowActionNode = 'data' in values.nodeToBeAdded
                    const partialNewAction = isInsightsFlowActionNode
                        ? (values.nodeToBeAdded as InsightsFlowActionNode).data
                        : (values.nodeToBeAdded as CreateActionType)

                    let config = partialNewAction.config
                    if (!isInsightsFlowActionNode) {
                    ...
                    if ('template_id' in config && config.template_id) {
                        const template = values.insightsFunctionTemplatesById[config.template_id]
                        if (template) {
                    ...
                    const newAction = {
                        id: isInsightsFlowActionNode
                            ? (values.nodeToBeAdded as InsightsFlowActionNode).id
                        ...
                        created_at: Date.now(),
                        updated_at: Date.now(),
                        ...(!isInsightsFlowActionNode && (partialNewAction as CreateActionType).output_variable
                            ? { output_variable: (partialNewAction as CreateActionType).output_variable }
                            : {}),
                    } as InsightsFlowAction

                    const step = getInsightsFlowStep(newAction, values.insightsFunctionTemplatesById)

                    const branchEdges = isInsightsFlowActionNode
                        ? 0
                    ...
                    const newEdges: InsightsFlow['edges'] = [...values.workflow.edges]
            ...
            copyNodeToHighlightedDropzone: () => {
                ...
                if (!dropzoneNode || !values.nodeToBeAdded) {
                    toast.error("Couldn't copy this step there. Try dropping it on a highlighted spot.")
            ...
            moveNodeToHighlightedDropzone: () => {
                ...
                const dropzoneNode = values.dropzoneNodes.find((x) => x.id === values.highlightedDropzoneNodeId)
                if (!dropzoneNode) {
                    toast.error("Couldn't move this step there. Try dropping it on a highlighted spot.")
                    actions.stopMovingNode()
                    return
                }

                const targetInsightsFlowEdge = dropzoneNode.data.edge.data?.edge
                if (!targetInsightsFlowEdge) {
                    toast.error("Couldn't move this step there. Try a different spot.")
                    actions.stopMovingNode()
                    return
                }
    ...
    subscriptions(({ actions }) => ({
        workflow: (flow?: InsightsFlow, oldInsightsFlow?: InsightsFlow) => {
            ...
            if (flow && !objectsEqual(flow, oldInsightsFlow)) {
                actions.resetFlowFromInsightsFlow(flow)
            }
        },
    })),
