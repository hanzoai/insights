...
```bash
pip install insightscli
```

## Quick Start

Create `insightscli.yaml` in your repo root:
...
```bash
insightscli dev:start
insightscli --help        # Shows all commands grouped by category
insightscli dev:start -h  # Help for specific command
```
...
Chain multiple insightscli commands:
...
For complex logic, define plain Click commands and reference them from `insightscli.yaml` with `click: module.path:attribute`.

Click command modules are lazy-loaded. Top-level `insightscli --help` uses the manifest description without importing Python command modules; `insightscli <command> --help` and command execution import the target on demand.

The Click command name must match the manifest key — drift surfaces as a `ClickException` on resolution. The framework follows Click's recommendation that lazy loading be paired with a test that runs `--help` on each subcommand; Insights's test suite parametrizes over every `click:` entry in `insightscli.yaml` to do exactly that.

Mark commands hidden via `hidden: true` in `insightscli.yaml`. Don't use `@click.command(hidden=True)` — the manifest is the single source of truth.
...
# insightscli.yaml
config:
  commands_dir: tools/insightscli_commands

db:
  db:migrate:
    click: insightscli_commands.db:db_migrate
    description: Run database migrations
```

```python
# tools/insightscli_commands/db.py
import click
...
`commands_dir` is optional and explicit: insightscli only uses it when configured. It must be a relative path to an existing directory. insightscli puts that directory's parent on `sys.path`, so the directory should be an importable package or module tree. It must not be named `insightscli`, since that shadows the installed framework.
...
├── insightscli.yaml
└── tools/
    └── insightscli-commands/
        ├── pyproject.toml
        └── insightscli_commands/    # underscored: this is the import name
            ├── __init__.py
            ├── build.py
            ├── db.py
            └── deploy.py
...
# insightscli.yaml
config:
  commands_dir: tools/insightscli-commands/insightscli_commands

build:
  build:
    click: insightscli_commands.build:build
    description: Run build pipelines
```

The dashed outer dir + underscored inner package follows PEP 8 (dashed project name, underscored import name). This is the layout Insights itself uses.
...
config:
  commands_dir: tools/insightscli_commands
  boot_modules:
    - insightscli_commands.boot
```

### Prechecks

Run validation before a command executes, keyed by `type:` in a `prechecks:` entry in `insightscli.yaml`. Return `False` to abort, `True`/`None` to continue.

```python
from insightscli.hooks import register_precheck
...
```python
from insightscli.hooks import register_telemetry_properties
...
```python
from insightscli.hooks import register_post_command_hook
...
insightscli can load dotenv files into every command's environment, the same way
[just](https://just.systems/man/en/settings.html),
[mise](https://mise.jdx.dev/environments/), and
[task](https://taskfile.dev/docs/guide) do. Declare them in `insightscli.yaml`:
...
env. insightscli supports this generically — no provider-specific knowledge in core:
...
pre-commit hooks pulling on that. The built-in `insightscli run <cmd>` always
...
2. If the wrap binary is missing or the marker isn't present, insightscli loads the
...
For commands without `needs_secrets: true`, insightscli skips the wrap entirely
...
process exec on every insightscli invocation, which we never want.
...
wrap-and-exec convention. insightscli itself stays ignorant of which tool you use.
...
- `insightscli quickstart` - Getting started guide
- `insightscli meta:check` - Validate manifest against bin scripts (for CI)
- `insightscli meta:concepts` - Show infrastructure concepts (if defined)
...
Releases are published to PyPI via `.github/workflows/publish-insightscli.yml`,
triggered by pushing a `insightscli-v*` tag from `master`.

1. Bump `version` in `tools/insightscli/pyproject.toml` and merge to `master`.
2. From `master`, tag and push:

   ```bash
   git tag insightscli-v0.1.1
   git push origin insightscli-v0.1.1
   ```
...
```bash
gh workflow run publish-insightscli.yml --ref insightscli-v0.1.1
```

The publish job is guarded by `if: startsWith(github.ref, 'refs/tags/insightscli-v')`,
