...
impl PersonScanner {
    ...
    /// Stream the run's live person ids in Datastore order, keeping the first id of every new
...
pub enum PersonScanError {
    #[error("building Datastore person scan cursor")]
    Query(#[source] datastore::error::Error),
    #[error("streaming Datastore person scan cursor")]
