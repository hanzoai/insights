...
def to_script_datetime(timestamp: int | float | dict, zone: Optional[str] = None):
    if isinstance(timestamp, dict) and is_script_date(timestamp):
...
def now(zone: Optional[str] = None):
    return to_script_datetime(datetime.datetime.now().timestamp(), zone)


def toUnixTimestamp(date, timezone: Optional[str] = None):
    if isinstance(date, dict) and is_script_datetime(date):
        return date["dt"]
    if isinstance(date, dict) and is_script_date(date):
...
def fromUnixTimestamp(timestamp: int | float):
    return to_script_datetime(timestamp)
...
def toTimeZone(date: dict, timezone: str):
    if not is_script_datetime(date):
...
def formatDateTime(input: dict, format: str, zone: Optional[str] = None) -> str:
    if not is_script_datetime(input):
        raise ValueError("Expected a DateTime")
    format_string = ""
    acc = ""
    i = 0
