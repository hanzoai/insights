...
from insights.tasks.tasks import capture_task_run_state_metrics
...
class TestCaptureTaskRunStateMetrics(TestCase):
    ...
    def _run_with_registry(self) -> CollectorRegistry:
        ...
        with patch("insights.tasks.tasks.pushed_metrics_registry", return_value=context):
    ...
    def test_emits_runs_created_1h_by_origin_and_environment(self) -> None:
        Task = apps.get_model("tasks", "Task")
        TaskRun = apps.get_model("tasks", "TaskRun")
        # Two Slack runs created just now, one old run (outside the 1h window), one Insights-code run.
        self._make_task_run(origin=Task.OriginProduct.SLACK, status=TaskRun.Status.QUEUED)
    ...
    @parameterized.expand(
        [
            ("missing_table", ProgrammingError('relation "insights_task_run" does not exist')),
    ...
    def test_transient_db_errors_are_suppressed_without_reporting(self, _name: str, error: Exception) -> None:
        with (
            ...
            patch("insights.tasks.tasks.capture_exception") as capture_mock,
        ):
