...
route through `_execute_insightsql_via_ai_events` which wraps the resolver. The
other query sites in `tools.py` deliberately stay on `events` directly via
`_execute_insightsql` (see the module docstring on `_execute_insightsql` for the
rationale).
"""
...
from insights.temporal.ai_observability.eval_reports.report_agent.schema import EvalReportContent
...
_RESOLVE_PATH = "insights.insightsql_queries.ai.trace_id_resolver.resolve_trace_ids_for_generation_uuids"
...
class TestSampleGenerationDetailsRoutesThroughResolver(BaseTest):
    @patch("insights.insightsql_queries.ai.ai_table_resolver.query_ai_events")
    ...
    def test_routes_through_resolver_against_ai_events(self, _mock_lookup, mock_resolver):
        """The heavy-prop tool must hit the resolver, not raw `execute_insightsql_query`."""
        ...
        # nosemgrep: insightsql-no-string-table-chain
        assert from_chain == ["insights", "ai_events"]
...
class TestGetGenerationDetailRoutesThroughResolver(BaseTest):
    ...
    def test_main_query_uses_resolver_eval_query_uses_events(self, _mock_lookup, mock_events, mock_resolver):
        ...
        assert mock_resolver.call_count == 1
        # Non-heavy eval-rows query went through plain execute_insightsql_query (events).
    ...
    def test_invalid_generation_id_returns_error_without_querying(self):
        with patch("insights.insightsql_queries.ai.ai_table_resolver.query_ai_events") as mock_resolver:
...
class TestGetGenerationTextReprRoutesThroughResolver(BaseTest):
    ...
    @patch(_RESOLVE_PATH, return_value={_VALID_GEN_ID: "trace-1"})
    def test_routes_through_resolver_against_ai_events(self, _mock_lookup, mock_resolver):
        """Heavy fetch goes through the resolver and selects from insights.ai_events."""
        from datetime import datetime
