...
from insights.datastore.routing import ORG_PROJECT_COLUMN, ORG_PROJECT_TABLE, PROJECT_SQL
...
# WHICH SIGNAL. The rename was also a MERGE: `event.fact` holds every signal
# ingest accepts — act, clip, error, log, span — discriminated by `signal`, so the
...
#
# Defined in `insights.datastore.routing` and imported above, because the LOG
# plane needs the identical expression and is read through the InsightsQL schema,
# which is imported before the Django app registry is ready. One definition, two
# planes: see that module for why the org and the fork's `team` meet there.
