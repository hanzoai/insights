...
class MailgunSource(
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            label="Mailgun",
            caption=f"""Enter your Mailgun private API key to pull your Mailgun data into the Insights Data warehouse.
...
Note: Mailgun only retains events for a limited period (1 day on free plans, up to 30 days on paid plans), so the initial events sync is bounded by your plan's retention.""",
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/mailgun",
            ...
            webhookSetupCaption=(
                "Insights registers a webhook on every sending domain of your account, for every "
            ...
            webhookFields=cast(
                ...
                [
                    SourceFieldInputConfig(
                        ...
                        placeholder="",
                        caption=(
                            f"From [Settings > API security]({API_SECURITY_URL}). Insights uses it to verify "
