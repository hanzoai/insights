---
description: Insights coding conventions for frontend and backend development
---

# Insights Coding Conventions

When writing code, follow these Insights coding conventions.
...
- Use `@frozen` from `insights.dataclasses` (applies `frozen=True`, `kw_only=True`, `slots=True`; every flag overridable, e.g. `@frozen(slots=False)` for `functools.cached_property`)
...
- A bare `@dataclass` without an explicit `frozen=` choice fails the `insights/test/test_dataclass_defaults.py` ratchet and is flagged by the `prefer-frozen-dataclasses` semgrep rule
