...
class TestPlanResolver:
    ...
    async def test_fetches_from_api_without_billing_period(self, resolver_with_redis: PlanResolver) -> None:
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {"plan_key": "insights-code-200-20260301", "created_at": None}
        mock_resp.raise_for_status = MagicMock()
        resolver_with_redis._http.get = AsyncMock(return_value=mock_resp)  # type: ignore[method-assign]  # ty: ignore[invalid-assignment]
...
class TestResolvePlanInfoCredentialForwarding:
    ...
    def _make_request(self, headers: dict[str, str]) -> MagicMock:
        ...
        resolver.get_plan = AsyncMock(
            return_value=PlanInfo(plan_key="insights-code-pro-200-20260301", seat_created_at=None)
        )
