...
from insights.models.utils import UUIDModel, UUIDTModel
...
class ErrorTrackingIssueAssignment(UUIDTModel):
    ...
    team = models.ForeignKey("insights.Team", null=True, on_delete=models.CASCADE, db_index=False)
    ...
    role = models.ForeignKey("insights.Role", null=True, on_delete=models.CASCADE, db_constraint=False)
    created_at = models.DateTimeField(auto_now_add=True)
...
def _sync_error_tracking_issue_changes_on_commit(
    *, team_id: int, issue_ids: list[UUID], overrides: list[ErrorTrackingIssueFingerprintV2]
    ...
    def sync_issues() -> None:
        sync_issues_to_datastore(issue_ids=issue_ids, team_id=team_id)
...
class ErrorTrackingBypassRule(UUIDTModel):
    ...
    # db_constraint=False keeps the create lock-free on insights_team (a hot table); team scoping
...
DEPRECATED_DATASTORE_STATUSES = frozenset(
    {ErrorTrackingIssue.Status.ARCHIVED, ErrorTrackingIssue.Status.PENDING_RELEASE}
)


def _datastore_status(issue_status: str) -> str:
    if issue_status in DEPRECATED_DATASTORE_STATUSES:
        return ErrorTrackingIssue.Status.RESOLVED
    return issue_status


def sync_issues_to_datastore(*, issue_ids: list, team_id: int) -> None:
    if not issue_ids:
        return
    ...
    for fp in fingerprints:
        ...
        first_seen = format_datastore_timestamp(first_seen_raw) if first_seen_raw else None
        producer.produce(
            ...
            data={
                ...
                "issue_status": _datastore_status(issue.status),
...
class ErrorTrackingSettings(models.Model):
    ...
    class Meta:
        db_table = "insights_errortrackingsettings"
...
class ErrorTrackingSpikeEvent(UUIDModel):
    ...
    class Meta:
        db_table = "insights_errortrackingspikeevent"
        indexes = [
...
class ErrorTrackingRecommendation(UUIDTModel):
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, related_name="error_tracking_recommendations")
    ...
    class Meta:
        db_table = "insights_errortrackingrecommendation"
        constraints = [
