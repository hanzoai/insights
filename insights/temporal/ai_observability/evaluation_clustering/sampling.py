...
def _compose_evaluation_text(
    ...
    for short Script-runtime reasoning like ``"OK"`` or ``"Total tokens 17250 exceeds 4000"``.
...
def _sample_and_embed_sync(inputs: SamplerActivityInputs) -> SamplerActivityResult:
    ...
    # Translate the user-supplied property filters into a InsightsQL expression.
...
async def sample_and_embed_for_job_activity(inputs: SamplerActivityInputs) -> SamplerActivityResult:
    ...
    doesn't trip on cyclic references inside InsightsQL AST nodes (query objects hold
    back-references to their parents) or Datastore driver exception payloads
