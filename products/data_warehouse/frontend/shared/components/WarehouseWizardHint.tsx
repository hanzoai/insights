 ...
 * pushes the `npx @hanzo/wizard warehouse` CLI, which auto-detects and connects a user's
...
export function WarehouseWizardHint({
    ...
    const command = `npx -y @hanzo/wizard@latest warehouse`

    const handleDismiss = (): void => {
        localStorage.setItem(DISMISSED_KEY, '1')
        setDismissed(true)
        insights.capture('warehouse wizard hint dismissed')
    }
        ...
        >
            ...
            <div className="pt-1">
                <CommandBlock
                    ...
                    onCopy={() => insights.capture('warehouse wizard hint command copied')}
                />
            </div>
        </div>
    )
