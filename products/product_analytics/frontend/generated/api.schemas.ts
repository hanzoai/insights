...
export interface ColumnConfigurationApi {
    ...
    filters?: unknown
    /**
     * Ordered list of InsightsQL expressions describing the table sort. Null preserves the current sort on apply (legacy rows); an empty list explicitly means no sort.
     * @nullable
     */
...
export interface InsightsQLQueryModifiersApi {
    ...
    /** InsightsQL parser backend; absent → `rust_py_with_cpp_shadow` (rust-py is primary, cpp runs as a sampled shadow). `*_shadow` modes return the primary result and sample-compare against the other parser, reporting divergences without failing the request. The `rust_py_*` modes drive the same hand-rolled Rust parser as `rust_*` but build `insights.insightsql.ast` dataclass instances directly via PyO3, skipping the JSON round-trip. */
...
export interface TrendsQueryResponseApi {
    ...
    /** Warnings about data warehouse sources referenced by the query whose latest sync failed, is paused, hit a billing limit, or is otherwise stale. Results may not reflect current source data. Accumulated across every InsightsQL execution that contributes to this response — so insights backed by warehouse tables (Trends, Funnels, etc.) receive the same warnings as raw InsightsQL queries. Also carries access control warnings when a system-table query filters out objects the user can't access. */
...
export interface WebStatsTableQueryApi {
    ...
    /** Opt this specific query into the web stats table precompute path. Requires the `web-analytics-precompute-toggle` Insights feature flag to be on for the team's organization for the gate to pass. * */
...
export interface WebOverviewQueryApi {
    ...
    tags?: QueryLogTagsApi | null
    useSessionsTable?: boolean | null
    /** Opt this specific query into the web_overview_query precompute path. Requires the `web-analytics-precompute-toggle` Insights feature flag to be on for the team's organization for the gate to pass. * */
    useWebAnalyticsPrecompute?: boolean | null
    /** version of the node, used for schema migrations */
    version?: number | null
}
 ...
 */
export type InsightsQLQueryApiValues = { [key: string]: unknown } | null

/**
 * Variables to be substituted into the query
 */
export type InsightsQLQueryApiVariables = { [key: string]: InsightsQLVariableApi } | null
...
export interface InsightsQLQueryApi {
    ...
    /** Run the selected connection query directly without translating it through InsightsQL first */
...
export interface GroupsQueryApi {
    ...
    offset?: number | null
    orderBy?: string[] | null
    properties?: (GroupPropertyFilterApi | InsightsQLPropertyFilterApi)[] | null
...
export interface WebGoalsQueryApi {
    ...
    tags?: QueryLogTagsApi | null
    useSessionsTable?: boolean | null
    /** Opt this specific query into the web_goals_query precompute path. Requires the `web-analytics-precompute-toggle` Insights feature flag to be on for the team's organization for the gate to pass. * */
    useWebAnalyticsPrecompute?: boolean | null
    /** version of the node, used for schema migrations */
    version?: number | null
}
...
export interface WebVitalsPathBreakdownQueryApi {
    ...
    thresholds: number[]
    useSessionsTable?: boolean | null
    /** Opt this specific query into the web vitals path breakdown precompute path. Requires the `web-analytics-precompute-toggle` Insights feature flag to be on for the team's organization for the gate to pass. * */
    useWebAnalyticsPrecompute?: boolean | null
    /** version of the node, used for schema migrations */
    version?: number | null
}
...
export interface AccountsQueryApi {
    ...
    /** Optional InsightsQL boolean expression AND-ed into the WHERE clause. Used by the overview tile click-to-filter affordance. */
...
export interface DataTableNodeApi {
    ...
    showInsightsQLEditor?: boolean | null
...
export interface InsightApi {
    ...
    /** @nullable */
    readonly insightsql: string | null
    /** @nullable */
...
export interface ActivityLogEntryApi {
    ...
    /** Whether the acting user was being impersonated by Insights staff. */
    readonly was_impersonated: boolean
    /**
     * API client that triggered the activity, from the x-insights-client request header (e.g. 'mcp'). Null for requests that did not send the header.
     * @nullable
     */
...
export type ElementsStatsRetrieveParams = {
    ...
    offset?: number
    /**
     * JSON-encoded list of property filters to apply to the underlying events, e.g. [{"key": "$current_url", "value": "https://example.com/page"}] or [{"key": "email", "value": "@hanzo.ai", "operator": "icontains", "type": "person"}]. Supports event, person, cohort, element, and InsightsQL property filter types.
     */
    properties?: string
    /**
...
export type InsightsListParams = {
    ...
    /**
     * Restrict to a single insight type. `JSON` matches non-wrapper query insights; `SQL` matches InsightsQL queries.
     */
...
export type InsightsRetrieveParams = {
    ...
    /**
     * Object (or pre-encoded JSON string) to override the insight's InsightsQL variables for this request only (not persisted). Format: {"<variable_id>": {"code_name": "<code_name>", "variableId": "<variable_id>", "value": <new_value>}}. Each entry must include `code_name` — partial entries are silently dropped. The simplest workflow is to call `insight-get` first, copy the matching entry from the response, and mutate `value`. Top-level keys replace; nested values are not deep-merged. Ignored when accessed via a sharing token.
     */
