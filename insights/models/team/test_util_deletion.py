...
from insights.models.team.util import (
...
class TestDeleteGroupsForTeams(SimpleTestCase):
    @patch(_CLIENT_PATCH)
    def test_deletes_via_personinsights_per_team(self, mock_get_client):
...
class TestDeleteHashKeyOverridesForTeams(SimpleTestCase):
    @patch(_CLIENT_PATCH)
    def test_deletes_via_personinsights(self, mock_get_client):
        mock_client = MagicMock()
        resp = MagicMock()
        resp.deleted_count = 0
