...
Reference doc for the `products/signals/backend/custom_agent/` package: a thin abstraction that lets Insights engineers ship one-off Signals-style agents that produce final READY reports in the Code Inbox without writing Temporal, sandbox, repo-selection, or persistence code.
...
- Runs against a Insights team (with or without a target GitHub repository).
...
`base.py` has zero `temporalio` / `insights.temporal` imports. The agent is a
...
Free-form selection (`select_repository_for_prompt`) reuses the existing repo cache (`system.integration_repository_cache`) and the `Insights/.github` dummy clone for the selection sandbox. It uses `MultiTurnSession.start()` (no validation retry — if the model returns malformed JSON, the activity fails and produces no report).
...
- Always uses `insights_mcp_scopes="read_only"`.
...
Any future fix to the autostart hacks (assignment-by-self-opt-in, GitHub-login → Insights-user resolution, `interaction_origin="signal_report"` magic string, non-transactional task creation) lands in that one module and benefits both code paths.
...
- **GitHub integration required end-to-end.** `resolve_user_id_for_team` raises when the team has no GitHub integration, including for `NO_REPO` runs. Intentional: custom agents are a Insights-eng tool right now and every relevant team has a GitHub integration.
...
- Whether to expose `insights_mcp_scopes` and sandbox env choice on the public API. Currently locked to `read_only` + `SIGNALS_REPORT_RESEARCH`.
