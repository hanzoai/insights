...
class AsyncMigrationsViewset(viewsets.ModelViewSet):
    ...
    def trigger(self, request, **kwargs):
        ...
        if not is_insights_version_compatible(
            ...
            return response.Response(
                {
                    "success": False,
                    "error": f"Can't run migration. Minimum Insights version: {migration_instance.insights_min_version}. Maximum Insights version: {migration_instance.insights_max_version}",
                },
                status=400,
            )
