...
from insights.schema import ProductKey, SessionPropertyFilter, WebStatsBreakdown
...
def is_web_analytics_query(query: Any) -> bool:
    """Read from the query's own tags rather than the ambient Datastore query
...
def resolve_first_pageview_filters_modifier(query: Any, team: "Team", current: Optional[bool]) -> Optional[bool]:
    ...
    `insightsql_modifiers`: entry-attributed and first-pageview runs of the same query
