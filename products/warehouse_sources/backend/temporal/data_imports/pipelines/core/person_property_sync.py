...
3. diffs against the source's last-sent snapshot (S3) so unchanged values are skipped even on a
   full refresh,
4. drops distinct_ids that don't resolve to an existing person (personinsights),
...
S3/personinsights/Kafka helpers are the boundaries and are mocked in the orchestration test.
"""
...
from insights.kafka_client.routing import producer_scope
from insights.kafka_client.topics import KAFKA_WAREHOUSE_PERSON_PROPERTY_UPDATES
...
# --- S3 / Kafka / personinsights boundaries (mocked in tests) ---------------------------------
