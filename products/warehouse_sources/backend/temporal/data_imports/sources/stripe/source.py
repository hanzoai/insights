...
from insights.models.integration import OauthIntegration
...
class StripeSource(
    ...
    has_managed_insightsql_schema = True  # canonical Stripe schema in external_table_definitions
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption=f"Connect your Stripe account to automatically sync your Stripe data into Insights. You can choose between OAuth (recommended) or legacy RAK Stripe keys. If you choose the latter, you will need your [Stripe account ID]({STRIPE_ACCOUNT_URL}), and create a [restricted API key]({STRIPE_API_KEYS_URL})",
...
If automatic creation failed due to a permissions error and you're using a restricted API key (not OAuth), your key needs **Write** access on **Webhook endpoints**. You can update this in your [Stripe API keys settings](https://dashboard.stripe.com/apikeys).""",
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            # include Insights's egress IPs. This is a customer-side key configuration that retrying
            # can never satisfy, so stop retrying. Match the stable phrase, not the appended IP.
            "does not allow requests from your IP address": "Your Stripe API key restricts requests by IP address and is blocking Insights. Remove the IP restriction on your restricted key in Stripe (or allowlist Insights's IP addresses), then try again.",
