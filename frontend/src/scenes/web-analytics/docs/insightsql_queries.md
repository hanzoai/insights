...
- [API Schema (Swagger UI)](https://insights.hanzo.ai/v1/schema/swagger-ui)
...
```bash
curl -X POST https://insights.hanzo.ai/v1/projects/:project_id/query \
...
### 3. Using Raw InsightsQL Queries from Snapshots

The examples above use high-level query types like `WebOverviewQuery` and `WebStatsTableQuery`. You can also use the **raw InsightsQL/SQL** queries directly from the snapshot files. This gives you full control and allows for customization.

#### Finding InsightsQL Queries in Snapshots
...
3. Copy the InsightsQL query between the triple quotes `'''`

#### Using InsightsQLQuery with the API

Use the `InsightsQLQuery` kind instead of web analytics-specific kinds:

```bash
curl -s -X POST https://insights.hanzo.ai/v1/projects/:project_id/query \
  -H "Authorization: Bearer PERSONAL_API_KEY" \
  -H "Content-Type: application/json" \
  -d @- <<'EOF'
{
...
The SQL editor uses `InsightsQLQuery` under the hood, so what works there will work in the API.
...
```bash
curl -X POST https://insights.hanzo.ai/v1/projects/123/query \
  -H "Authorization: Bearer PERSONAL_API_KEY" \
  -H "Content-Type: application/json" \
  -d @- <<'EOF'
{
...
Common modifications you can make to the InsightsQL queries:
...
- Use `equals()` function instead of `=` operator in InsightsQL
