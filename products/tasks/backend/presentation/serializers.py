...
from insights.security.url_validation import is_url_allowed, resolve_url_hosts_ips
...
# report (e.g. Insights Desktop inbox), recorded as a signals `task_run` work-log entry. This is a
...
class TaskWriteSerializer(serializers.Serializer):
    ...
    origin_product = serializers.ChoiceField(
        ...
        required=False,
        help_text=(
            "Insights product or surface that created this task (e.g. error_tracking, slack, user_created). "
    ...
    repository = serializers.CharField(
        ...
        allow_blank=True,
        allow_null=True,
        help_text="Target GitHub repository in `organization/repo` format (e.g. `insights/insights-js`).",
    )
    ...
    def validate_signal_report(self, value):
        if value and value.team_id != self.context["team"].id:
            raise serializers.ValidationError("Signal report must belong to the same project")
        return value
...
class TaskRunLivingArtifactChartRequestSerializer(serializers.Serializer):
    ...
    query = InsightQueryJSONField(
        ...
        help_text=(
            ...
            "SQL queries (DataVisualizationNode, InsightsQLQuery) are not supported yet. "
...
class TaskRunLivingArtifactChartResponseSerializer(serializers.Serializer):
    ...
    url = serializers.URLField(
        allow_null=True,
        required=False,
        help_text="Link to explore this chart interactively in Insights.",
    )
...
RESERVED_IMPORTED_MCP_SERVER_NAMES = {"insights"}
...
class ImportedMcpServersFieldMixin(serializers.Serializer):
    ...
    imported_mcp_servers = ImportedMcpServerSerializer(
        ...
        write_only=True,
        help_text=(
            "Local url-based MCP servers from the creating client (Insights Desktop) to make "
...
class RelayedMcpServersFieldMixin(serializers.Serializer):
    ...
    relayed_mcp_servers = RelayedMcpServerSerializer(
        ...
        write_only=True,
        help_text=(
            "Names of desktop-only MCP servers the creating client (Insights Desktop) relays into the "
...
class TaskRunCreateRequestSerializer(ImportedMcpServersFieldMixin, RelayedMcpServersFieldMixin, serializers.Serializer):
    ...
    github_user_token = serializers.CharField(
        ...
        write_only=True,
        help_text=(
            "Optional GitHub user token from Insights Desktop for user-authored cloud pull requests. "
...
class TaskRunBootstrapCreateRequestSerializer(
    ...
    github_user_token = serializers.CharField(
        ...
        allow_blank=False,
        write_only=True,
        help_text="Ephemeral GitHub user token from Insights Desktop for user-authored cloud pull requests.",
    )
    initial_permission_mode = serializers.ChoiceField(
        choices=ALL_INITIAL_PERMISSION_MODE_CHOICES,
        required=False,
        default=None,
        help_text=(
            "Initial permission mode for the agent session. Claude runtimes accept Insights permission "
...
class WarmTaskRequestSerializer(serializers.Serializer):
    ...
    repository = serializers.CharField(
        max_length=255,
        help_text="Target GitHub repository to clone, in `organization/repo` format (e.g. `insights/insights`).",
    )
        ...
        if value:
            for key in value:
                ...
                if tasks_facade.is_reserved_sandbox_env_var_key(key):
                    raise serializers.ValidationError(
                        f"Environment variable key {key!r} is reserved and managed by Insights; it cannot be set."
                    )
        return value
...
class SlackThreadContextQuerySerializer(serializers.Serializer):
    ...
    url = serializers.URLField(
        help_text=(
            ...
            "https://insights.slack.com/archives/C…/p1779956938619299). Replies inside the "
...
class SlackThreadContextTaskSerializer(serializers.Serializer):
    ...
    url = serializers.CharField(help_text="Absolute URL to the task detail page in the Insights app.")
...
class SlackThreadContextRunSerializer(serializers.Serializer):
    ...
    mention_workflow_id = serializers.CharField(
        ...
        help_text=(
            ...
            "(`insights-code-mention-<workspace>:<event_id_or_channel:ts>`). Null for runs "
...
class SlackThreadContextResponseSerializer(serializers.Serializer):
    ...
    task = SlackThreadContextTaskSerializer(
        allow_null=True,
        help_text="Linked Insights Task. Null when no mapping was found for the thread.",
    )
