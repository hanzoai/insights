...
- its OTLP twin (the Insights Metrics product),
...
sandbox-side run). Direct (insightsql) runs report them from the async query manager's status
(``sql_v2_direct._query_status_timings``): ``queued_s`` (enqueue -> Celery pickup) and
``datastore_s`` (pickup -> completion, i.e. InsightsQL compile + Datastore execution).
"""
...
from insights.event_usage import report_user_or_team_action
...
NODE_RUN_SECONDS = Histogram(
    "insights_notebooks_node_run_seconds",
...
NODE_RUN_PHASE_SECONDS = Histogram(
    "insights_notebooks_node_run_phase_seconds",
