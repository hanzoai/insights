...
SSE_STREAM_DURATION_HISTOGRAM = Histogram(
    "insights_sse_stream_duration_seconds",
...
SSE_REJECTED_OVER_CAP_COUNTER = Counter(
    "insights_sse_rejected_over_cap_total",
...
def _release_request_connections() -> None:
    ...
    corrupts it. Insights never streams from inside ``transaction.atomic()``, so in
...
def streaming_response(
    ...
    *,
    ...
    in-stream DB work through ``insights.sync.database_sync_to_async`` so it
...
def sse_streaming_response(
    ...
    *,
    ...
    Insights runs with ``CONN_MAX_AGE = 0``, so any connection still open when the
