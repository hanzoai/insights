        ...
        "values": [
            {
                ...
                "event_filters": [
                    {"type": "insightsql", "key": "person_id IN (SELECT string_prop FROM extended_properties)"}
                ],
            }
        ],
    }
...
class TestCohortRecalculationWarehouseAccessControl(BaseTest):
    ...
    def test_background_recalculation_bypasses_warehouse_access_control(self):
        ...
        with pytest.raises(TableAccessDeniedError):
            insightsql_cohort_subquery_sql(self.cohort, team=self.team)

        # Recalculation always opts into the bypass, so the same cohort compiles.
        sql, _ = insightsql_cohort_subquery_sql(self.cohort, team=self.team, bypass_warehouse_access_control=True)
    ...
    def test_denied_member_cannot_save_query_cohort_over_warehouse_table(self):
        ...
        response = self.client.post(
            f"/api/projects/{self.team.id}/cohorts/",
            {
                "name": "query cohort",
                "is_static": True,
                "query": {"kind": "InsightsQLQuery", "query": "SELECT string_prop FROM extended_properties"},
            },
        )
