...
class BillComSource(ResumableSource[BillComSourceConfig, BillComResumeConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Connect BILL to pull your accounts payable and receivable data into the Insights Data warehouse.

Create a developer key in your [BILL developer account](https://developer.bill.com/docs/bill-keys-tokens), then enter the email and password you sign in with, your organization ID, and that developer key. Insights uses them to start an API session for each sync.

BILL Spend & Expense data is not included — it uses a separate API token.""",
            iconPath="/static/services/bill_com.png",
            docsUrl="https://hanzo.ai/docs/cdp/sources/bill-com",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
