...
from posthog.plugins.plugin_server_api import get_insights_function_templates
...
class Command(BaseCommand):
    help = "Sync InsightsFunction templates from in-memory and node.js to database"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Starting InsightsFunction template sync...")
        ...
        try:
            existing_templates = InsightsFunctionTemplate.objects.values_list("template_id", flat=True).distinct()
            ...
            if candidates_for_deletion:
                templates_to_delete = InsightsFunctionTemplate.objects.filter(template_id__in=candidates_for_deletion)
