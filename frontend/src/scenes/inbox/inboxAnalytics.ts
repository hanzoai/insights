 ...
 * one Insights project. Every event carries `inbox_client` so funnels and breakdowns can split
...
function captureInboxEvent(event: InboxEvent, properties: Record<string, unknown>): void {
    insights.capture(event, { inbox_client: INBOX_CLIENT, ...properties })
}
