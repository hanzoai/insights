...
pnpm --filter=@hanzo/mcp run scaffold-yaml -- --product your_product \
...
# 3. Add a InsightsQL system table in insights/insightsql/database/schema/system.py
#    and a model reference in products/insights_ai/skills/querying-insights-data/references/
...
`x-insights-mcp-version: <version_number>` header.
...
v2 instructs the agent to read data through a unified InsightsQL interface
...
giving it richer context about Insights's data model.

Primarily oriented toward coding agents (Insights Desktop, Insights AI, Claude Code).

## SQL-first MCP: InsightsQL system tables

Every list/get endpoint exposed as an MCP tool must have a corresponding InsightsQL system table.
This lets agents query Insights data via SQL in addition to (or instead of) the REST API tools.

System tables are defined in [`insights/insightsql/database/schema/system.py`](https://github.com/Insights/insights/blob/master/insights/insightsql/database/schema/system.py) as `PostgresTable` instances.
...
also add a model reference file to [`products/insights_ai/skills/querying-insights-data/references/`](https://github.com/Insights/insights/tree/master/products/insights_ai/skills/querying-insights-data/references).
...
See [`models-flags-experiments.md`](https://github.com/Insights/insights/blob/master/products/insights_ai/skills/querying-insights-data/references/models-flags-experiments.md) for a good example.
Register your new reference in [`products/insights_ai/skills/querying-insights-data/SKILL.md`](https://github.com/Insights/insights/blob/master/products/insights_ai/skills/querying-insights-data/SKILL.md) under **Data Schema**.
...
1. **Scaffold** a starter YAML with all operations disabled.
   ...
   ViewSets elsewhere (e.g. `insights/api/`, `ee/`) need
...
2. **Configure** the YAML – enable tools, add scopes, annotations, and descriptions.
   Each YAML file has a top-level structure validated by Zod ([`scripts/yaml-config-schema.ts`](https://github.com/Insights/insights/blob/master/services/mcp/scripts/yaml-config-schema.ts)):
...
`feature_flags`. They should match the product folder name.
   ...
   **Tool name length limit:** tool names must be **52 characters or fewer**.
   ...
   With the server name "insights" (7 chars) plus a separator, 52 characters is the safe zone.
   CI runs `pnpm --filter=@hanzo/mcp lint-tool-names` to enforce both length and pattern.
   ...
   tools:
     domain-action: # e.g. feature-flags-list, experiments-create
       ...
       annotations:
         ...
         idempotent: true
       # --- optional: ---
       mcp_version: 2 # 2 for create/update/delete operations or not available through SQL for retrieval, 1 for read/list if available via InsightsQL
...
```sh
pnpm --filter=@hanzo/mcp run scaffold-yaml -- --sync-all
```
...
See [`services/mcp/definitions/README.md`](https://github.com/Insights/insights/blob/master/services/mcp/definitions/README.md) for the full YAML schema reference (note: YAML definitions themselves now live in product folders)
and [`services/mcp/scripts/yaml-config-schema.ts`](https://github.com/Insights/insights/blob/master/services/mcp/scripts/yaml-config-schema.ts) for the Zod validation source.
...
see the [`improving-drf-endpoints` skill](https://github.com/Insights/insights/blob/master/.agents/skills/improving-drf-endpoints/SKILL.md).
...
- `ListField` and `JSONField` need explicit types —
  ...
  (see `insights/api/alert.py` for the pattern).
...
## InsightsQL query schemas (WIP)

[`frontend/src/queries/schema/schema-assistant-queries.ts`](https://github.com/Insights/insights/blob/master/frontend/src/queries/schema/schema-assistant-queries.ts) defines structured query types
...
that help agents generate correct InsightsQL.
The cleaner and better-described these schemas are,
the better agents perform at query generation.

This is a work in progress –
the goal is to make it easier to generate InsightsQL queries from typed schemas
...
- **`querying-insights-data`** – InsightsQL query patterns, system model schemas, and available functions.
  Extend this skill to explain how agents should use your InsightsQL-exposed tables and queries.
  See [`products/insights_ai/skills/querying-insights-data/SKILL.md`](https://github.com/Insights/insights/blob/master/products/insights_ai/skills/querying-insights-data/SKILL.md).
- **`improving-drf-endpoints`** – Audit checklist and patterns for DRF serializers and viewsets.
  ...
  See [`.agents/skills/improving-drf-endpoints/SKILL.md`](https://github.com/Insights/insights/blob/master/.agents/skills/improving-drf-endpoints/SKILL.md).
