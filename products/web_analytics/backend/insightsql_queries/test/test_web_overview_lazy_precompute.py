...
class TestWebOverviewLazyPrecompute(DatastoreTestMixin, APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    def _enable_lazy(self):
        ...
        # team. Outside this context manager the default `hanzo_insights.feature_enabled`
    ...
    def test_compare_period_falls_back_when_previous_not_ready(self):
        ...
        with (
            self._enable_lazy(),
            patch(
                "products.web_analytics.backend.insightsql_queries.web_overview_lazy_precompute.ensure_web_overview_precomputed",
    ...
    def test_recomputation_picks_up_late_events_changing_bounce_and_duration(self):
        ...
        # The stale precomputed row stays in Datastore with the old job_id;
...
@override_settings(IN_UNIT_TESTING=True)
class TestWebOverviewSessionIdSetInsert(DatastoreTestMixin, APIBaseTest):
    ...
    def test_insert_template_chooser(self, _name, properties, cohort_test_filter, allowlisted, expected):
        from products.web_analytics.backend.insightsql_queries import web_overview_lazy_precompute as mod
