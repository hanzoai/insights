...
class TestColumnToExpr(SimpleTestCase):
    ...
    def test_bare_source_name_is_not_shorthand(self):
        # "attributes" with no path falls through to InsightsQL as a plain field
        assert clear_locations(column_to_expr("attributes")) == ast.Field(chain=["attributes"])

    def test_unrecognized_prefix_parses_as_insightsql(self):
        # a top-level field with dots but no recognized source is InsightsQL field access, not shorthand
    ...
    @parameterized.expand(
        [
            ...
            # Window functions evaluate over a frame of rows, forcing Datastore to process every
