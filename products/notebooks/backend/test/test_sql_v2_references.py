...
def insightsql_ref(code: str | None, node_id: str = "node-df1", run_id: str = "run-1") -> SQLV2Ref:
    # A never-run node (code None) has no run to key on, mirroring how the view builds refs.
    return SQLV2Ref(kind="insightsql", node_id=node_id, run_id=None if code is None else run_id, last_run_code=code)
...
class TestResolvePythonNodeInputs(SimpleTestCase):
    def test_only_referenced_frames_are_materialized(self):
        ...
        inputs = resolve_python_node_inputs(
            "df1.head()", {"df1": insightsql_ref("select id from events"), "df2": insightsql_ref("select 1", node_id="node-df2")}
        )
        self.assertEqual(len(inputs), 1)
        self.assertEqual(inputs[0]["name"], "df1")
        self.assertEqual(inputs[0]["kind"], "insightsql")
        self.assertEqual(inputs[0]["query"], "select id from events")

    def test_unused_refs_are_ignored(self):
        self.assertEqual(
            resolve_python_node_inputs("import pandas as pd\npd.DataFrame()", {"df1": insightsql_ref("select 1")}), []
        )
    ...
    def test_reassigned_ref_is_still_materialized(self):
        ...
        inputs = resolve_python_node_inputs(
            "df1.columns = ['a']\ndf1 = df1.assign(x=1)", {"df1": insightsql_ref("select 1")}
        )
    ...
    def test_frame_is_keyed_on_the_upstream_run_id(self):
        ...
        inputs = resolve_python_node_inputs("df1.head()", {"df1": insightsql_ref("select 1", node_id="n1", run_id="r2")})
    ...
    def test_referencing_a_never_run_node_raises(self):
        with self.assertRaises(SQLV2ReferenceError):
            resolve_python_node_inputs("df1.head()", {"df1": insightsql_ref(None)})
    ...
    def test_used_local_upstream_becomes_a_local_input(self):
        ...
        inputs = resolve_python_node_inputs(
            "new_events.describe()", {"new_events": LOCAL, "df1": insightsql_ref("select 1")}
        )
        self.assertEqual(inputs, [{"name": "new_events", "kind": "local"}])


class TestResolveSQLNodeRun(SimpleTestCase):
    def test_all_insightsql_refs_push_to_datastore(self):
        node_type, run_code, inputs = resolve_sql_node_run(
            "select * from df1", {"df1": insightsql_ref("select id from events")}
        )
        self.assertEqual(node_type, "insightsql")
    ...
    def test_unreferenced_local_frame_does_not_reroute(self):
        ...
        node_type, run_code, inputs = resolve_sql_node_run(
            "select * from df1", {"df1": insightsql_ref("select id from events"), "new_events": LOCAL}
        )
        self.assertEqual(node_type, "insightsql")
        self.assertIn("WITH df1 AS", run_code)
        self.assertEqual(inputs, [])

    def test_referenced_local_frame_reroutes_to_duckdb_and_materializes_insightsql_refs(self):
        ...
        node_type, run_code, inputs = resolve_sql_node_run(
            code, {"df2": insightsql_ref("select id from persons"), "new_events": LOCAL}
        )
        self.assertEqual(node_type, "duckdb")
        self.assertEqual(run_code, code)  # DuckDB gets the SQL as written, not a CTE rewrite
        self.assertEqual([(spec["name"], spec["kind"]) for spec in inputs], [("df2", "insightsql"), ("new_events", "local")])
    ...
    def test_duckdb_run_referencing_a_never_run_insightsql_node_raises(self):
        with self.assertRaises(SQLV2ReferenceError):
            resolve_sql_node_run(
                "select * from df2 join new_events on true", {"df2": insightsql_ref(None), "new_events": LOCAL}
            )

    def test_unparseable_insightsql_naming_a_local_frame_still_routes_to_duckdb(self):
        # DuckDB-only syntax (PIVOT isn't InsightsQL; QUALIFY actually parses) must still run
...
class TestResolveSQLV2References(SimpleTestCase):
    ...
    def test_transitive_reference_is_ordered_before_its_user(self):
        # df2 reads df1, so df1's CTE must be printed first or Datastore can't resolve it.
        resolved = resolve_sql_v2_references(
            "select * from df2",
    ...
    def test_refs_nested_below_the_top_level_are_still_inlined(self, _name, code):
        ...
        # CTE — the query then fails in Datastore with an unknown table.
    ...
    def test_cycle_through_a_user_cte_raises(self):
        ...
        # collision to hit, and it must fail here as a 400 rather than as a Datastore error.
        with self.assertRaises(SQLV2ReferenceError):
            resolve_sql_v2_references(
...
class TestResolvedQueryIsValidInsightsQL(BaseTest):
    ...
    # `df1 AS (…)` *after* the CTE reading it, so every substring check passed while Datastore
    ...
    def test_resolved_query_typechecks(self, _name, code):
        ...
        prepare_and_print_ast(parse_select(resolved), context=context, dialect="datastore")
