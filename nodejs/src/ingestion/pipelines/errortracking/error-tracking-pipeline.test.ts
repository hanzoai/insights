...
describe('ErrorTrackingPipeline', () => {
    ...
    const getProducedEvents = (): any[] => {
        return getProducedMessagesForTopic('datastore_events_json_test')
    }
    ...
    beforeEach(() => {
        ...
        pipelineConfig = {
            outputs: new IngestionOutputs({
                events: new SingleIngestionOutput('events', 'datastore_events_json_test', mockKafkaProducer, 'test'),
    ...
    describe('runErrorTrackingPipeline', () => {
        it('processes a single event through the full pipeline and emits to Kafka', async () => {
            ...
            // Verify properties contain GeoIP (from Script transformations) and Cymbal data
        ...
        it('runs Script transformations on events', async () => {
            mockPersonRepository.fetchPerson.mockResolvedValue(undefined)
            mockCymbalClient.processExceptions.mockResolvedValue([createCymbalResponse()])
        ...
        it('preserves GeoIP properties added by Script transformations', async () => {
            mockPersonRepository.fetchPerson.mockResolvedValue(undefined)
            mockCymbalClient.processExceptions.mockResolvedValue([createCymbalResponse()])
    ...
    describe('cookieless processing', () => {
        ...
        it('uses the rewritten distinct_id from the cookieless manager downstream', async () => {
            ...
            const message = createKafkaMessage({ distinctId: '$insights_cookieless' })
            const pipeline = createErrorTrackingPipeline(pipelineConfig)
            await runErrorTrackingPipeline(pipeline, [message])
