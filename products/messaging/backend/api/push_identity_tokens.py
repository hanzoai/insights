...
`distinct_id`, signed with the project's secret API key. Insights re-verifies the signature at
...
1. Asymmetric ES256 (preferred). The customer holds an EC (P-256) private key and registers only the
   public key on the push channel (`config["push_identity_public_keys"]`). Insights verifies with the
...
PUSH_IDENTITY_TOKEN_AUDIENCE = "insights:push_identity"
...
def sign_push_identity_token(
    ...
    authenticated the end user. It is not called by Insights's own ingestion (which only verifies);
...
def sign_push_identity_token_es256(
    ...
    """Reference ES256 signer: what the customer's backend runs with its EC private key. Insights only
