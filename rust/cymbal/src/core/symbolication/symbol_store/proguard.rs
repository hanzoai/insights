...
use bytes::Bytes;
use insights_symbol_data::{read_symbol_data_with_byte_count, ProguardMapping};
...
mod tests {
    ...
    fn fetched_mapping_uses_proguard_cache_for_lookups() {
        ...
        assert_eq!(
            ...
            Some("com.insights.android.sample.MyCustomException3".to_string())
        );
        ...
        assert!(remapped.iter().any(|frame| {
            frame.class() == "com.insights.android.sample.MyCustomException3"
                && frame.method() == "<init>"
        }));
    }
}
