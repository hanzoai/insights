...
Combines migrate, apply_persons_migrations, and migrate_datastore into one
command to avoid three separate Django cold starts (~15s each). Used by the
sandbox entrypoint to speed up boot time.

Migrations run sequentially because Datastore migrations call
get_instance_setting() which queries the Django insights_instancesetting table,
so Datastore must run after Django.
"""
...
class Command(BaseCommand):
    help = "Run all migrations (Django, persons, Datastore) in one process"
    ...
    def handle(self, *args, **options):
        ...
        tasks = [
            ...
            ("Datastore", lambda: call_command("migrate_datastore")),
        ]
