...
use personinsights_proto::personinsights::types::v1::Person;

use personinsights_coordination::authority::AuthorityClock;
...
fn transactional_id(topic: &str, partition: u32) -> String {
    format!("personinsights-changelog-{topic}-p{partition}")
}
...
impl PartitionFence {
    ...
    fn condemn(&self, partition: u32, reason: &'static str) {
        if !self.unusable.swap(true, Ordering::Relaxed) {
            counter!(
                "personinsights_leader_fence_condemned_total",
                "reason" => reason
            )
            .increment(1);
            error!(
...
impl Drop for WindowSlot {
    fn drop(&mut self) {
        if !self.released {
            ...
            counter!("personinsights_leader_fence_slots_abandoned_total").increment(1);
...
impl Drop for FenceGuard {
    fn drop(&mut self) {
        if self.armed {
            counter!("personinsights_leader_fence_abandoned_total").increment(1);
            warn!(
...
impl FencedChangelogProducers {
    ...
    async fn acquire_installed(&self, partition: u32) -> Result<Arc<PartitionFence>, String> {
        ...
        .map_err(|e| {
            counter!("personinsights_leader_fence_init_total", "outcome" => "error").increment(1);
            format!("fence init: {e}")
        })?;
        counter!("personinsights_leader_fence_init_total", "outcome" => "ok").increment(1);
        histogram!("personinsights_leader_fence_init_ms").record(start.elapsed().as_secs_f64() * 1000.0);
    ...
    pub async fn settle(&self, partition: u32) {
        let Some(fence) = self.installed(partition) else {
            ...
            // denominator.
            counter!("personinsights_leader_fence_settle_total", "outcome" => "absent").increment(1);
            return;
        };
        ...
        counter!("personinsights_leader_fence_settle_total", "outcome" => outcome).increment(1);
        if outcome != "settled" {
            warn!(
                partition,
                outcome,
    ...
    pub async fn produce(
        ...
        let fence = self
            ...
            .map(|f| Arc::clone(&f))
            .ok_or_else(|| {
                counter!("personinsights_leader_kafka_produce_errors_total").increment(1);
        ...
        histogram!("personinsights_leader_fence_window_wait_ms", "partition" => partition.to_string())
        ...
        let offset = match delivery {
            Ok(fut) => match fut.await {
                Ok(Ok((_, offset))) => {
                    histogram!("personinsights_leader_fence_send_ms")
        ...
        histogram!("personinsights_leader_fence_ack_wait_ms", "partition" => partition.to_string())
            .record(ack_wait_start.elapsed().as_secs_f64() * 1000.0);
        match outcome {
            Ok(Ok(())) => {
                counter!("personinsights_leader_kafka_produces_total").increment(1);
                ...
                histogram!("personinsights_leader_kafka_produce_duration_ms")
    ...
    fn classify(
        ...
        if is_fenced(&e) || producer_fenced(fence.producer.inner()) {
            counter!(
                "personinsights_leader_produce_fenced_total",
    ...
    fn forget_fence(&self, partition: u32, fence: &Arc<PartitionFence>) {
        ...
        if let Some((_, evicted)) = removed {
            ...
            counter!(
                "personinsights_leader_fenced_partition_drops_total",
                "partition" => partition.to_string()
            )
            .increment(1);
        }
...
async fn commit_window_after(
    ...
    histogram!("personinsights_leader_fence_window_writes", "partition" => partition.to_string())
    ...
    let result = spawn_blocking(move || {
        ...
        if poisoned {
            ...
            match abort_window(&producer, timeout) {
                ...
                Err(e) => {
                    counter!("personinsights_leader_fence_abort_failed_total").increment(1);
            ...
            for _ in 0..COMMIT_RETRIES {
                match &attempt {
                    Err(KafkaError::Transaction(e)) if e.is_retriable() => {
                        counter!("personinsights_leader_fence_commit_retries_total").increment(1);
    ...
    let outcome: Result<(), FencedProduceError> = match result {
        Ok(Ok(())) => {
            histogram!("personinsights_leader_fence_commit_ms", "outcome" => "committed")
                .record(commit_start.elapsed().as_secs_f64() * 1000.0);
            Ok(())
        }
        Ok(Err((e, outcome))) => {
            histogram!("personinsights_leader_fence_commit_ms", "outcome" => "failed")
                .record(commit_start.elapsed().as_secs_f64() * 1000.0);
            ...
            if verdict == WindowVerdict::FencedUncertain {
                ...
                counter!(
                    "personinsights_leader_fence_aborts_total",
                    "partition" => partition.to_string()
                )
                .increment(1);
                ...
                counter!(
                    "personinsights_leader_fence_commit_indeterminate_total",
                    "partition" => partition.to_string()
                )
                .increment(1);
                error!(
...
pub fn preregister_fencing_metrics(partitions: u32) {
    for outcome in ["ok", "error"] {
        counter!("personinsights_leader_fence_init_total", "outcome" => outcome).increment(0);
    }
    ...
    for outcome in ["settled", "timeout", "unusable", "absent"] {
        counter!("personinsights_leader_fence_settle_total", "outcome" => outcome).increment(0);
    }
    counter!("personinsights_leader_fence_slots_abandoned_total").increment(0);
    counter!("personinsights_leader_fence_abandoned_total").increment(0);
    counter!("personinsights_leader_fence_abort_failed_total").increment(0);
    ...
    counter!("personinsights_leader_fence_healed_total").increment(0);
    counter!("personinsights_leader_fence_heal_abandoned_total").increment(0);
    for reason in ["abort_failed", "commit_indeterminate", "commit_task_lost"] {
        counter!("personinsights_leader_fence_condemned_total", "reason" => reason).increment(0);
    }
    counter!("personinsights_leader_fence_commit_retries_total").increment(0);
    counter!("personinsights_leader_kafka_produce_errors_total").increment(0);
    for partition in 0..partitions {
        let p = partition.to_string();
        counter!("personinsights_leader_fence_aborts_total", "partition" => p.clone()).increment(0);
        counter!(
            "personinsights_leader_fenced_partition_drops_total",
            "partition" => p.clone()
        )
        .increment(0);
        counter!(
            "personinsights_leader_fence_heal_failures_total",
            "partition" => p.clone()
        )
        .increment(0);
        counter!(
        ...
        .increment(0);
        counter!("personinsights_leader_produce_fenced_total", "partition" => p).increment(0);
    }
...
pub async fn heal_fence(
    ...
    if lost_standing || inflight.is_fenced(partition) {
        ...
        counter!("personinsights_leader_fence_heal_abandoned_total").increment(1);
        warn!(
            partition,
    ...
    guard.keep();
    counter!("personinsights_leader_fence_healed_total").increment(1);
    warn!(
        partition,
