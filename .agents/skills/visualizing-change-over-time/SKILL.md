...
description: >
  Helps Insights engineers and agents pick the right chart when a request is about
  visualizing change over time, before/after comparisons, or period-over-period
  movement while building UI with @hanzo/quill-charts (dashboards, reports, the
...
[`@hanzo/quill-charts`](../../../packages/quill/packages/charts/AGENTS.md).
...
```tsx
import { SlopeChart } from '@hanzo/quill-charts'
import type { Series } from '@hanzo/quill-charts'
import type { SlopeSeriesMeta } from '@hanzo/quill-charts'
...
- **`SlopeChart`** — the `@hanzo/quill-charts` component. Use it when building UI
...
- **`ChartDisplayType.SlopeGraph`** — a first-class insight display (value
  ...
  (`insights/insightsql_queries/insights/trends/slope_graph_trends_query_runner.py`). It
  ...
  - Via the **API / `insights:insight-create` MCP tool** it works without the flag —
