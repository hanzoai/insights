...
from insights.insightsql.direct_sql.postgres_adapter import postgres_error_to_message, postgres_oid_to_datastore_type
...
# `SELECT * FROM huge_table` with no LIMIT. InsightsQL-authored queries already carry a LIMIT from
# the printer.
...
def _fetch_capped_redshift_rows(cursor: psycopg.Cursor) -> list:
    ...
    if len(rows) > DIRECT_REDSHIFT_MAX_ROWS:
        ...
        raise ExposedInsightsQLError(DIRECT_REDSHIFT_ROW_CAP_ERROR)
    return list(rows)
...
def ensure_read_only_raw_redshift_statement(sql: str) -> str:
    ...
    Snowflake adapter, since they're callable from a plain SELECT. InsightsQL-authored queries only
    ...
    if len(statements) != 1 or statements[0].get_type() != "SELECT":
        raise ExposedInsightsQLError(RAW_REDSHIFT_READ_ONLY_ERROR)
    for token in statements[0].flatten():
        ...
        if token.ttype in sqlparse_tokens.Name and token.value.upper() in _RAW_REDSHIFT_BLOCKED_FUNCTIONS:
            raise ExposedInsightsQLError(RAW_REDSHIFT_BLOCKED_FUNCTION_ERROR)
    return sql


class RedshiftAdapter:
    engine = "redshift"
    dialect: InsightsQLDialect | None = "redshift"
    ...
    def validate_source_config(
        ...
        if not (is_direct_capable(source) and source.direct_engine == self.engine):
            raise ExposedInsightsQLError("Invalid direct Redshift connection.")
        ...
        if not valid_host:
            raise ExposedInsightsQLError(host_errors or "Invalid Redshift host.")
    ...
    def execute(self, request: DirectQueryRequest) -> DirectQueryResult:
        ...
        except (psycopg.Error, ExposedInsightsQLError) as error:
            span.set_attribute("error_type", error.__class__.__name__)
            if request.debug:
                return DirectQueryResult(results=[], types=[], print_columns=[], error=postgres_error_to_message(error))
            raise ExposedInsightsQLError(postgres_error_to_message(error)) from error

        span.set_attribute("row_count", len(results))
        types: list[tuple[str, str]] = [
            (column.name, postgres_oid_to_datastore_type(getattr(column, "type_code", None))) for column in description
        ]
