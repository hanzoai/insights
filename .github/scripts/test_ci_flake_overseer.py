...
def make_job(
    ...
    *,
    ...
    return Job(
        ...
        html_url="https://github.com/Insights/insights/actions/runs/1/job/123",
...
def make_workflow_run(
    ...
    return WorkflowRun(
        ...
        head_sha="abc123",
        run_attempt=run_attempt,
        html_url="https://github.com/Insights/insights/actions/runs/999",
...
def test_build_decision_events_one_event_per_decision() -> None:
    ...
    events = build_decision_events("Insights/insights", make_workflow_run(), decisions)

    assert [event["event"] for event in events] == [DECISION_EVENT, DECISION_EVENT, DECISION_EVENT]
    assert all(event["distinct_id"] == "Insights/insights" for event in events)
...
def test_report_rerun_outcomes_empty_on_first_attempt() -> None:
    assert report_rerun_outcomes("Insights/insights", make_workflow_run(run_attempt=1)) == []
...
def test_report_rerun_outcomes_attributes_prior_test_failure(
    ...
    events = report_rerun_outcomes("Insights/insights", make_workflow_run(run_attempt=2))
...
def test_build_decision_events_is_master(head_branch: str, event: str, expected_is_master: bool) -> None:
    ...
    events = build_decision_events(
        "Insights/insights", make_workflow_run(head_branch=head_branch, event=event), decisions
    )
...
def test_report_rerun_outcomes_ignores_non_test_jobs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert report_rerun_outcomes("Insights/insights", make_workflow_run(run_attempt=2)) == []
...
def test_capture_events_noop_without_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    capture_events("", "https://us.i.hanzo.ai", [{"event": DECISION_EVENT}])
