"""Eval: agent handles Insights-vs-SQL divergence in two shapes — scope mismatch and anti-routing.
...
1. ``insights_count_below_raw_sql`` — the user's hand-written SQL counts more
   events than Insights's experiment view. The skill lists the canonical
...
To run:
    ...
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_numbers_vs_sql"
"""
...
from products.insights_ai.evals.experiments.scorers import CitesDiagnosticGroup
from products.insights_ai.evals.experiments.seeders import ROLLOUT_EXPERIMENT_NAME, seed_running_experiment


async def eval_numbers_vs_sql(ctx: EvalContext) -> None:
    cases: list[SandboxedEvalCase] = [
        SandboxedEvalCase(
            name="insights_count_below_raw_sql",
            prompt=(
                ...
                "events WHERE event = '$pageview' AND timestamp >= <start_date>`) I get 1000. Insights "
            ...
            expected={
                ...
                "diagnosis_group": (
                    "The agent names AT LEAST ONE of the scope filters that the Insights experiment "
                    ...
                    "date range. The agent must also frame the gap as 'Insights and raw SQL measure "
                    "different populations' — i.e. Insights's number is not wrong, the SQL is "
