...
import { buildMCPAnalyticsGroups, buildMCPContextProperties } from '@/lib/insights/analytics'
...
export async function flushAnalytics(timeoutMs = 2000): Promise<void> {
    try {
        ...
        await getInsightsClient().shutdown(timeoutMs)
    } catch {
            ...
            const capture = (async (): Promise<void> => {
                try {
                    ...
                    getInsightsClient().capture({
                        ...
                        properties: {
                            $ai_product: 'mcp',
                            $mcp_source: 'insights_cli',
                            $mcp_client_name: 'insights-cli',
                            $mcp_consumer: 'insights-cli',
                            $mcp_mode: 'cli',
