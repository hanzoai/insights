"""Insights AI extension of the products/tasks per-Run state bag.
...
from products.insights_ai.backend.context_wrapper import AttachedContext
from products.insights_ai.backend.services.system_prompt.service import ClaudeCodeSystemPrompt
from products.tasks.backend.facade.run_config import RunState


class InsightsAIRunState(RunState):
    """The base ``RunState`` already covers ``pending_user_message`` and
    ``initial_permission_mode``; this adds the Insights AI-only keys. The bag is persisted
