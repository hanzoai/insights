...
land in Insights's own data warehouse bucket, namespaced by team then by a per-upload id, which is
...
object: Insights reads it in place from its own bucket, so there is no import pipeline and no
...
# Datastore read format (`FILE_FORMAT_TO_TABLE_FORMAT`) when the table is created.
FORMAT_CSV = "csv"
FORMAT_JSON = "json"
...
# Maps an uploaded file's format to the `DataWarehouseTable.TableFormat` value Datastore reads it
...
# source, where Insights reads the customer's bucket directly instead of hosting the bytes.
...
def build_file_upload_url_pattern(team_id: int, upload_id: str, filename: str) -> str:
    ...
    This is the form `DataWarehouseTable.get_columns` builds its Datastore s3 table function from.
    The object lives in Insights's own bucket, so the table carries no credential and reads fall back
...
def hosted_upload_s3_path(url_pattern: str) -> str | None:
    ...
    whose file Insights hosts in its own data warehouse bucket, or ``None`` when the table reads from
