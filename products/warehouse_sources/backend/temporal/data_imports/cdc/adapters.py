...
class CDCSourceAdapter(Protocol[CDCConfigT_co]):
    ...
    def is_connection_error(self, exc: BaseException) -> bool:
        ...
        than a bug in Insights's own code. Callers use it to avoid capturing expected
    ...
    def recreate_slot(self, source: ExternalDataSource, tables: list[str]) -> dict[str, Any]:
        ...
        existing capture definition (recreating it when Insights owns it). ``tables`` is
    ...
    def cleanup_resources(self, source: ExternalDataSource) -> None:
        """Drop engine-side CDC resources owned by Insights for the source.
        ...
        has no CDC config or no Insights-owned resources to drop.
        """
        ...
    ...
    def add_table(self, source: ExternalDataSource, schema: str, table: str) -> None:
        ...
        No-op when Insights doesn't own the capture definition (e.g. self-managed)."""
        ...
