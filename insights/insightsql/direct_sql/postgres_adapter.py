...
POSTGRES_OID_TO_DATASTORE_TYPE: dict[int, str] = {
    16: "Bool",
    20: "Int64",
    21: "Int16",
    23: "Int32",
...
def postgres_oid_to_datastore_type(oid: int | None) -> str:
    if oid is None:
        return "String"

    return POSTGRES_OID_TO_DATASTORE_TYPE.get(oid, "String")
...
class PostgresAdapter:
    engine = "postgres"
    dialect: InsightsQLDialect | None = "postgres"
    ...
    def validate_source_config(
        ...
        if not (is_direct_capable(source) and source.direct_engine == self.engine):
            raise ExposedInsightsQLError("Invalid direct Postgres connection.")
        ...
        if not valid_host:
            raise ExposedInsightsQLError(host_errors or "Invalid Postgres host.")
