...
describe('hogInvocationsLogic', () => {
    describe('buildSearchClause', () => {
        ...
        it('escapes ILIKE wildcards in the message arm but not the id arms', () => {
            ...
            // backslash-escaped only for the ILIKE arm (then doubled by escapeInsightsQLString for the SQL
    ...
    describe('isRerunnableInsightsFunctionType', () => {
        ...
        it.each(['destination', 'internal_destination'] as const)('is true for rerunnable type %s', (type) => {
            expect(isRerunnableInsightsFunctionType(type)).toBe(true)
        })

        it.each(['source_webhook', 'transformation', 'site_app', 'site_destination', 'source'] as const)(
            'is false for non-rerunnable type %s',
            (type) => {
                expect(isRerunnableInsightsFunctionType(type)).toBe(false)
            }
        )

        it('is false when the type is unknown', () => {
            expect(isRerunnableInsightsFunctionType(undefined)).toBe(false)
            expect(isRerunnableInsightsFunctionType(null)).toBe(false)
        })
    })
    ...
    describe('parentClauseFor', () => {
        ...
        it('scopes to the batch job id when parentRunId is set', () => {
            ...
            // not just that broadcast's. The id goes in as an escaped InsightsQL string literal.
