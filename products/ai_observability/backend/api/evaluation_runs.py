...
from insights.insightsql_queries.ai.ai_table_resolver import AIEventsExpiredError, AIEventsNotFoundError
from insights.insightsql_queries.ai.utils import HEAVY_COLUMN_NAMES, HEAVY_COLUMN_TO_PROPERTY, merge_heavy_properties
from insights.permissions import AccessControlPermission
from insights.temporal.ai_observability.run_evaluation import RunEvaluationInputs
...
class EvaluationRunRequestSerializer(serializers.Serializer):
    ...
    timestamp = serializers.DateTimeField(
        required=True, help_text="ISO 8601 timestamp of the target event (needed for efficient Datastore lookup)."
    )
...
class EvaluationRunViewSet(TeamAndOrgViewSetMixin, viewsets.ViewSet):
    ...
    def create(self, request: Request, **kwargs) -> Response:
        ...
        # Fetch event data from Datastore using available keys
        where_clauses = [
            "team_id = %(team_id)s",
    ...
    def _fetch_event_for_evaluation(self, where_clauses: list[str], params: dict[str, object]) -> dict:
        ...
        # Tag these direct Datastore reads so the manual-eval-trigger lookup is attributed
