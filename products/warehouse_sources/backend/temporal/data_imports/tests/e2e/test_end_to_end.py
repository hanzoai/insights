...
async def _row_ids(team, table: str) -> list[str]:
    # Explicit LIMIT: InsightsQL defaults to 100 rows, which would silently truncate the identity
    # comparison and let a row-losing rewrite pass on any table bigger than that.
    result = await sync_to_async(execute_insightsql_query)(f"SELECT id FROM {table} ORDER BY id LIMIT 10000", team)
