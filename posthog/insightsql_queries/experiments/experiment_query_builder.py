...
from posthog.insightsql_queries.experiments import MULTIPLE_VARIANT_KEY
from posthog.insightsql_queries.experiments.base_query_utils import (
...
from posthog.insightsql_queries.experiments.breakdown_injector import BreakdownInjector
from posthog.insightsql_queries.experiments.exposure_query_logic import normalize_to_exposure_criteria
from posthog.insightsql_queries.experiments.insightsql_aggregation_utils import (
    aggregation_needs_numeric_input,
    build_aggregation_call,
    extract_aggregation_and_inner_expr,
)
from posthog.insightsql_queries.insights.utils.utils import get_start_of_interval_insightsql
...
class ExperimentQueryBuilder:
    ...
    def _build_metric_predicate(self, source=None, table_alias: str = "events") -> ast.Expr:
        ...
        if isinstance(source, ExperimentDataWarehouseNode):
            ...
            #    confuse InsightsQL field resolution when used as a prefix
        ...
        return parse_expr(
            ...
            placeholders={
                ...
                "date_from": self.date_range_query.date_from_as_insightsql(),
                "date_to": self.date_range_query.date_to_as_insightsql(),
                "conversion_window_seconds": ast.Constant(value=conversion_window_seconds),
    ...
    def _build_value_expr(self, source=None, apply_coalesce: bool = True) -> ast.Expr:
        ...
        # Don't coalesce values for count distinct types (IDs) or INSIGHTSQL (user controls the expression)
        ...
        # We need toFloat to ensure type consistency - base_expr could be String (INSIGHTSQL),
    ...
    def _build_value_aggregation_expr(
        ...
        elif math_type == ExperimentMetricMathType.INSIGHTSQL:
            math_insightsql = getattr(source, "math_insightsql", None)
            if math_insightsql is not None:
                aggregation_function, _, params, distinct = extract_aggregation_and_inner_expr(math_insightsql)
    ...
    def _build_funnel_steps_filter(self) -> ast.Expr:
        ...
        if conversion_window_seconds > 0:
            date_to = parse_expr(
                ...
                placeholders={
                    "to_date": self.date_range_query.date_to_as_insightsql(),
                    "conversion_window_seconds": ast.Constant(value=conversion_window_seconds),
                },
            )
        else:
    ...
    def _get_retention_window_truncation_expr(self, timestamp_expr: ast.Expr) -> ast.Expr:
        ...
        return get_start_of_interval_insightsql(interval=interval_name, team=self.team, source=timestamp_expr)
