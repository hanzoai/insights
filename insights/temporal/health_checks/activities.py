...
from insights.temporal.common.rollout import filter_ids_for_rollout
from insights.temporal.health_checks.models import BatchResult, HealthCheckWorkflowInputs
from insights.temporal.health_checks.observability import push_health_check_metrics
from insights.temporal.health_checks.processing import _process_batch_detection
from insights.temporal.health_checks.registry import ensure_registry_loaded, get_detect_fn, get_product
...
def _get_team_id_batches_sync(inputs: HealthCheckWorkflowInputs) -> list[list[int]]:
    ...
    if kill_switch_level != KillSwitchLevel.OFF:
        logger.info(
            "skipping health check due to datastore kill switch",
            kind=inputs.kind,
