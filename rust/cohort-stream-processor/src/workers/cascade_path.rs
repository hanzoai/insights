...
pub(crate) async fn handle_cascade(
    ...
    let evaluated_at_ms = datastore_millis(last_updated);
...
/// Parse a Datastore `DateTime64(6)` string into epoch millis, `0` on a miss. Feeds only the
/// write-only `Stage2State::last_evaluated_at_ms`, so a miss costs a diagnostic timestamp, never
/// membership.
fn datastore_millis(ts: &str) -> i64 {
