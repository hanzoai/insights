...
import { getInsightsExecDisplay } from './insightsExecDisplay'
...
/**
 * Insights single-exec discovery verbs (`tools` / `search` / `info` / `schema`, plus the `unknown`
 * fallback). `getInsightsExecDisplay` turns the `command` into a friendly label ("List tools",
 ...
 */
const InsightsExecRenderer = memo(function InsightsExecRenderer(props: ToolRendererProps): JSX.Element {
    const { message, icon, turnComplete, turnCancelled } = props
    const display = getInsightsExecDisplay(message.rawInput)
...
/**
 * ToolSearch — Claude's deferred-tool search, rendered like Insights's MCP tool search: a "Search tools"
...
export const BuiltinToolRenderer = memo(function BuiltinToolRenderer(props: ToolRendererProps): JSX.Element {
    if (props.message.resolvedKey.startsWith('__insights_exec_')) {
        return <InsightsExecRenderer {...props} />
    }
