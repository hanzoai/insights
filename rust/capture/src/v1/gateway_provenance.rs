...
pub fn parse_signature(headers: &HeaderMap) -> Option<GatewaySignature> {
    Some(GatewaySignature {
        signature: shared::header_str(headers, INSIGHTS_AI_GATEWAY_SIGNATURE)?,
        signed_at: shared::header_str(headers, INSIGHTS_AI_GATEWAY_SIGNED_AT)?,
        request_id: shared::header_str(headers, INSIGHTS_AI_GATEWAY_REQUEST_ID).unwrap_or_default(),
    })
}
...
pub enum StripOutcome {
    ...
    /// typed parse rejects (out-of-range numbers, over-deep nesting) and Datastore's
