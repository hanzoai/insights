...
# Back off on transient Datastore overload (Code 202); fail fast on deterministic errors that won't recover.
...
class DeleteRecordingsWithSessionIdsWorkflow(InsightsWorkflow):
...
class PurgeDeletedRecordingMetadataWorkflow(InsightsWorkflow):
    """Nightly workflow to purge metadata from Datastore for deleted recordings.

    After recordings are deleted, the metadata remain in Datastore with is_deleted=1.
