...
def test_flag_returns_false_when_team_missing():
    with patch("insights.models.Team") as team_cls:
...
def test_flag_returns_false_when_evaluation_raises():
    ...
    with (
        patch("insights.models.Team") as team_cls,
        patch.object(deltalite_write.hanzo_insights, "feature_enabled", side_effect=RuntimeError("flags down")),
    ):
...
def test_flag_passes_write_key_and_person_properties():
    ...
    with (
        patch("insights.models.Team") as team_cls,
        patch.object(deltalite_write.hanzo_insights, "feature_enabled", return_value=True) as fe,
    ):
        team_cls.objects.only.return_value.get.return_value = fake_team
