...
3. Install dependencies (the Insights SDK, used for anonymous usage telemetry):
...
- the Insights Python SDK (`pip install -r requirements.txt`) — telemetry is skipped gracefully if it's missing
- a Fleet-managed kubeconfig with the standard Insights contexts
...
4. Finds an available Insights toolbox pod or connects to one you've already claimed
