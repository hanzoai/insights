...
It runs *after* logical lowering and *before* the Datastore physical passes (see `prepare_ast_for_printing`), so it
...
from insights.insightsql.constants import LimitContext, get_max_limit_for_context
...
def events_pushdown_enabled(modifiers: "InsightsQLQueryModifiers") -> bool:
...
class EventsSubexprHoister(CloningVisitor):
    ...
    reference to that column. The Datastore physical pass, running afterwards, resolves any `PropertyAccess` /
...
class EventsPredicatePushdownTransform(TraversingVisitor):
    ...
    Runs after logical lowering (so property reads are PropertyAccess) and before the Datastore physical passes,
    ...
    def _apply_pushdown(self, node: ast.SelectQuery) -> str | None:
        ...
        # would survive un-rewritten and point at a column the pre-filtering subquery doesn't project (Datastore:
    ...
    def _from_is_events_table(self, join_expr: ast.JoinExpr) -> bool:
        ...
        resolved type also matches the qualified `FROM insights.events`, which a chain comparison misses."""
    ...
    def _build_subquery(
        ...
        subquery reuses it directly as its FROM — no schema mimicry, no synthetic materialized columns. The Datastore
        ...
        events_field = ast.Field(chain=[base_table_type.table.to_printed_insightsql()], type=base_table_type)
