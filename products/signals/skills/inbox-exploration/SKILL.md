...
description: >
  Explore Insights's Inbox and act on what it surfaces — the place where signal reports cluster into
  actionable issues and trends. Use when the user asks "what's in my inbox?", "what should I look at?",
  "which reports are actionable?", "what's Insights flagged recently?", asks about a specific report by
...
The **Inbox** is where Insights surfaces signal reports — clusters of related observations
...
Inbox is part of [Insights Desktop](https://hanzo.ai/code), Insights's agentic surface for
engineering teams.
...
- "Show me actionable reports" / "What's Insights flagged recently?"
...
- **`signals` skill** — query `document_embeddings` via InsightsQL for raw signal text, semantic
  search across signals, or to inspect every signal that contributed to a report.
- **Insights's product-specific MCP tools** — when a report points at a specific error, log line,
...
A signal report tells you _what_ Insights clustered. The product-specific tools tell you the
...
| `insights:execute-sql` (signals skill) | InsightsQL access to underlying signals (read the `signals` skill first)                                           |
...
`is_suggested_reviewer: true` on a report means **the current Insights user is one of up to
...
3. At read time, those GitHub logins are mapped back to Insights users via each org member's
...
this is to install **Insights Desktop** at <https://hanzo.ai/code>. Example response:

> Your project doesn't have any signal sources configured yet, so the Inbox is empty. Inbox surfaces
> issues and trends that Insights automatically clusters from sources like error tracking, session
> replay, GitHub, Linear, and Zendesk. The fastest way to set this up is to install
> [Insights Desktop](https://hanzo.ai/code) — once it's connected, signals will start flowing in
...
- `is_suggested_reviewer` — whether the current user is a suggested reviewer for this
  ...
  authorship of the relevant code, mapped to Insights users via linked GitHub identity)
- `implementation_pr_url` — if a PR has been opened against this report
- `_insightsUrl` — clickable deep-link to the report; **always include this in your response**
...
3. Use the `signals` skill's Example 2 (fetch all signals for a specific report) — pass the
   report ID as `metadata.report_id` in the InsightsQL query — only for the raw underlying signal text
...
against the report. So reference the report in the PR description (its `_insightsUrl`) and tell the
...
- **Check setup before assuming the inbox is empty.** If `inbox-reports-list` returns `count: 0`,
  ...
  [Insights Desktop](https://hanzo.ai/code) to start receiving signals; sources-but-no-reports
  means signals are flowing but nothing has clustered yet
- **Always surface `_insightsUrl`** so the user can click through to the report
...
- The inbox writes exposed via MCP are `inbox-reports-set-state` (resolve / dismiss / snooze one
  ...
  the product surface, not via MCP. Always also surface the `_insightsUrl` deep-link
...
- For "what kinds of signals exist?" or "what's been happening recently across all sources?",
  ...
  InsightsQL on `document_embeddings` to see them
- Source configs don't have per-record deep-links — they live behind project settings, so
  `inbox-source-configs-retrieve` returns no `_insightsUrl`. Don't confuse them with reports
