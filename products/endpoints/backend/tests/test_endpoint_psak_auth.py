...
from insights.insightsql.database.database import _compute_system_table_access_decision
...
SAMPLE_QUERY = {"kind": "InsightsQLQuery", "query": "SELECT 1"}
_UNSET = object()
...
class TestEndpointViewSetPSAKAuth(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
...
class TestEndpointPSAKRateLimit(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
        self.endpoint = create_endpoint_with_version(
            name="rl_endpoint",
