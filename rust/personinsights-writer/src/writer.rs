...
impl<D: PersonDb + 'static> WriterTask<D> {
    ...
    async fn process_batch(&mut self, batch: FlushBatch) -> ControlFlow<()> {
        ...
        counter!("personinsights_writer_flushes_total").increment(1);
            ...
            match self.store.upsert_batch(to_process).await {
                ...
                BatchOutcome::Partial {
                    ...
                    if !data_failed.is_empty() {
                        counter!("personinsights_writer_batch_fallback_total").increment(1);
                        warn!(
    ...
    fn commit_and_record(
        ...
        if let Err(e) = self.consumer.commit_offsets(offsets) {
            counter!("personinsights_writer_offset_commit_errors_total").increment(1);
            error!(error = %e, "failed to commit offsets");
        }

        counter!("personinsights_writer_offset_commits_total").increment(1);
        ...
        if let Some(ts_ms) = oldest_ts_ms {
            ...
            histogram!("personinsights_writer_e2e_latency_ms").record(latency_ms as f64);
        }
