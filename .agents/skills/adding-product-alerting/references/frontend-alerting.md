...
Use `AlertWizard` from `frontend/src/lib/components/Alerting/AlertWizard/` when the product is creating InsightsFunction-backed alerts from trigger and destination templates.
...
A product can use both paths when it has a simple InsightsFunction creation flow and a richer product-owned alert configuration surface.
...
- Render threshold modes directly with `SegmentedButton` while keeping options and validation in product logic.
...
- How a pending destination becomes InsightsFunction input.
- How saved InsightsFunctions are grouped. Logs groups firing, resolved, and broken functions into one destination row.
...
- Use `Form` and `Field` for persisted fields.
...
2. Run `pnpm --filter=@hanzo/frontend typescript:check` and separate changed-file failures from unrelated repository failures.
