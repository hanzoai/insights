"""Metric execution — run a metric's definition through the standard InsightsQL query runner.
...
params map into the prepared query's dateRange for insight/node kinds and are rejected on InsightsQLQuery
...
from ..facade.enums import INSIGHTSQL_DEFINITION_KIND, MARKDOWN_DEFINITION_KIND, NODE_DEFINITION_KINDS
...
def _apply_date_params(query: dict, date_from: Optional[str], date_to: Optional[str], interval: Optional[str]) -> None:
    if date_from is None and date_to is None and interval is None:
        return
    if query.get("kind") == INSIGHTSQL_DEFINITION_KIND:
...
def _envelope(metric: Metric, payload: dict, team: Team, prepared_query: dict, is_drifted: bool) -> dict:
    return {
        ...
        "compiled_query": payload.get("insightsql"),
        "query_status": payload.get("query_status"),
        "insights_url": _deep_link(team, prepared_query),
        "instructions": None,
    }
...
def _deep_link(team: Team, prepared_query: dict) -> str:
    ...
    if prepared_query.get("kind") == INSIGHTSQL_DEFINITION_KIND:
