...
use personinsights_coordination::coordinator::{Coordinator, CoordinatorConfig};
use personinsights_coordination::error::Result;
use personinsights_coordination::pod::{PodConfig, PodHandle};
use personinsights_coordination::routing_table::{RoutingTable, RoutingTableConfig, StashHandler};
use personinsights_coordination::store::PersonhogStore;
use personinsights_coordination::strategy::AssignmentStrategy;
use rdkafka::mocking::MockCluster;
use rdkafka::producer::{DefaultProducerContext, FutureProducer};

use assignment_coordination::store::{EtcdStore, StoreConfig};
use personinsights_common::partitioning::partition_for_person;
use personinsights_leader::cache::{
...
use personinsights_leader::pg::PgFallback;
use personinsights_leader::recovery::{ChangelogRecovery, RecoveryConfig};
use personinsights_leader::service::{PersonHogLeaderService, PropertySizeLimits};
use personinsights_leader::warming::WarmClientPools;
use personinsights_leader::warnings::WarningsProducer;
use personinsights_proto::personinsights::leader::v1::person_hog_leader_client::PersonHogLeaderClient;
...
pub const PERSONS_DB_URL: &str = "postgres://insights:insights@localhost:5432/insights_persons";
...
pub const CHANGELOG_TOPIC: &str = "personinsights_updates";
...
pub fn test_warming_config(
    pod_name: &str,
    kafka_bootstrap: &str,
) -> personinsights_leader::warming::WarmingConfig {
    ...
    personinsights_leader::warming::WarmingConfig {
        ...
        writer_consumer_group: "personinsights-writer".to_string(),
        lookback_offsets: 0,
        committed_offsets_timeout: Duration::from_secs(5),
        fetch_watermarks_timeout: Duration::from_secs(5),
        recv_timeout: Duration::from_secs(10),
        retry: personinsights_leader::warming::WarmingRetryPolicy {
            max_attempts: 3,
...
pub async fn start_leader_pod(
    ...
    let emitted_versions = Arc::new(personinsights_leader::emitted::EmittedVersions::new(1_000_000));
    ...
    let service = PersonHogLeaderService::new(
        ...
        WarningsProducer::new(kafka_producer, "datastore_ingestion_warnings".to_string()),
        None,
...
pub async fn start_leader_with_pg_fallback(
    ...
    let service = PersonHogLeaderService::new(
        ...
        Some(PgFallback {
            pool,
            table: "insights_person".to_string(),
        }),
        Arc::new(DashMap::new()),
        ...
        None,
        None,
        std::sync::Arc::new(personinsights_leader::emitted::EmittedVersions::new(1_000_000)),
    );
...
/// test.
pub fn fenced_producers_for(topic: &str) -> personinsights_leader::fencing::FencedChangelogProducers {
    let mut kafka = test_kafka_config();
    kafka.kafka_hosts = KAFKA_BOOTSTRAP.to_string();
    personinsights_leader::fencing::FencedChangelogProducers::new(
        personinsights_leader::fencing::FencedProducerConfig {
            kafka,
...
pub fn test_handoff_handler(
    topic: &str,
    fenced: Arc<personinsights_leader::fencing::FencedChangelogProducers>,
) -> personinsights_leader::coordination::LeaderHandoffHandler {
    handoff_handler_with(
        topic,
        fenced,
        Arc::new(personinsights_leader::inflight::InflightTracker::new()),
        live_authority(),
    )
}
...
pub fn test_handoff_handler_with_inflight(
    topic: &str,
    fenced: Arc<personinsights_leader::fencing::FencedChangelogProducers>,
    inflight: Arc<personinsights_leader::inflight::InflightTracker>,
) -> personinsights_leader::coordination::LeaderHandoffHandler {
...
fn handoff_handler_with(
    ...
    personinsights_leader::coordination::LeaderHandoffHandler::new(
        Arc::new(PartitionedCache::new(1 << 20)),
        inflight,
        Arc::new(DirtyIndex::new(1_000_000)),
        warming,
        Arc::new(personinsights_leader::warming::WarmClientPools::new(
            &test_kafka_config(),
            "test",
        ...
        Some(fenced),
        Some(authority),
        Arc::new(personinsights_leader::emitted::EmittedVersions::new(1_000_000)),
    )
}
