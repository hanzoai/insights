...
from insights.models.github_integration_base import GitHubCommitAttribution
...
class TestAreaForPath:
    ...
    def test_single_directory(self):
        assert area_for_path("insights/models.py") == "insights"
    ...
    def test_fallback_chain_walks_up_to_repo_wide(self):
        ...
        assert area_fallback_chain("insights") == ["insights", REPO_WIDE_AREA]
...
class TestRebuildRepositoryActivity:
    def test_builds_area_map_joining_git_history_with_github_attribution(self, team):
        commits = [
            ...
            _commit("c" * 7, "bob@example.com", 30, ["products/signals/backend/tasks.py", "insights/models/user.py"]),
        ...
        activity = get_area_activity(team.id, "acme/app", ["products/signals", "insights/models"])
        ...
        assert [c.login for c in activity["insights/models"]] == ["bobdev"]
