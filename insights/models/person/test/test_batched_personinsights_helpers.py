...
from insights.personinsights_client.proto import ReadOptions
...
class TestBatchedGetPersonsByUuids(SimpleTestCase):
    ...
    def test_returns_all_persons(self, _name, batch_size, n_persons, expected_calls):
        with patch("insights.models.person.util.PERSONFN_BATCH_SIZE", batch_size):
    ...
    @patch("insights.models.person.util.PERSONFN_BATCH_SIZE", 2)
    def test_team_mismatch_metric_across_batches(self):
        ...
        with (
            patch("insights.models.person.util._get_client", return_value=mock_client),
            patch("insights.models.person.util.PERSONFN_TEAM_MISMATCH_TOTAL") as mock_metric,
        ):
    ...
    @patch("insights.models.person.util.TEST", False)
    @patch("insights.models.person.util.PERSONFN_BATCH_SIZE", 2)
    ...
    def test_default_concurrency_stays_sequential(self):
        ...
        # personinsights bulk pools must not silently multiply.
        with fake_personinsights_client() as fake:
            for i in range(6):
                fake.add_person(team_id=1, person_id=i + 1, uuid=f"uuid-{i}", distinct_ids=[f"d{i}"])

            with patch("insights.models.person.util.ThreadPoolExecutor") as mock_executor:
