...
//! `insights_instancesetting`.
//!
//! Django writes settings under the key `constance:insights:<NAME>` with one
...
const CONSTANCE_PREFIX: &str = "constance:insights:";
...
/// Fetch a Constance dynamic setting from `insights_instancesetting`, returning
...
pub async fn fetch_instance_setting_raw_value(
    ...
    let row: Option<(String,)> =
        sqlx::query_as("SELECT raw_value FROM insights_instancesetting WHERE key = $1")
            .bind(full_key)
            .fetch_optional(pool)
            .await
