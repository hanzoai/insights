...
Mirrors the limiter's shape (:mod:`insights.egress.limiter.outbound`): a domain-agnostic mechanism,
...
class EgressMetrics:
    ...
    ``installation_id`` (what GitHub actually meters), never a Insights DB row id. Keeping the identity in
    the external API's namespace is what lets one shared budget map to one series: several Insights
...
class EgressObservability:
    ...
    id, which several Insights integrations can share) — not a Insights DB row id. The counter is always
