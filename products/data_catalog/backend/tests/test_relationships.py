...
from insights.test.base import APIBaseTest, BaseTest, NonAtomicBaseTest
...
from insights.insightsql.database.lazy_join_tags import DATA_WAREHOUSE_EXPERIMENTS
...
from insights.rate_limit import InsightsQLQueryThrottle
...
# events and persons always exist in the InsightsQL database, so propose-time existence checks pass.
_JOIN: dict[str, Any] = {
...
class TestAcceptProposal(BaseTest):
    ...
    def test_accept_creates_one_join_and_is_idempotent(self) -> None:
        ...
        with patch.object(relationships, "execute_insightsql_query"):  # mock the Datastore probe boundary
    ...
    def test_broken_join_probe_rejects(self) -> None:
        proposal = self._propose()
        with patch.object(relationships, "execute_insightsql_query", side_effect=QueryError("no such column")):
            with self.assertRaises(ValidationError):
    ...
    def test_experiments_optimized_probe_uses_experiments_resolver(self) -> None:
        ...
        with patch.object(relationships, "execute_insightsql_query", side_effect=_capture):
            accept_proposal(proposal, self.user)
...
class TestRelationshipAPI(APIBaseTest):
    ...
    def test_accept_uses_insightsql_query_throttle(self) -> None:
        proposal = propose_relationship(team=self.team, user=self.user, **_JOIN)
        with (
            patch.object(InsightsQLQueryThrottle, "allow_request", return_value=False),
            patch.object(InsightsQLQueryThrottle, "wait", return_value=None),
        ):
