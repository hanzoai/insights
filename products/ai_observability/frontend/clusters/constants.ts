...
// Scene names used as Datastore query tags. These match the route registrations in
...
// hex / dashes before interpolating into a InsightsQL `IN` literal so a malformed key
// can't break out of the string. UUIDs already match this character set.
export const SAFE_ID_RE = /^[a-f0-9-]+$/i

// Mirrors `MAX_SELECT_RETURNED_ROWS` in `insights/insightsql/constants.py`. EventsQuery rows above
