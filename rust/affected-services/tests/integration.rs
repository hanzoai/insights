...
fn every_deployable_binary_has_an_image_entry() {
    let non_deployable: HashSet<&str> = [
        ...
        "debug_rule",
        "stl_dump",
        "run", // scriptvm dev/diff CLI, not a service
        "hermes",
        "personinsights-stateright",   // model-checker explorer CLI, not a service
        "personinsights-test-harness", // e2e load/correctness harness CLI, not a service
...
fn proto_change_maps_to_proto_crates() {
    let (graph, images) = load_test_fixtures();
    let result =
        compute_affected(&["proto/personinsights.proto".into()], None, &graph, &images).unwrap();
    assert!(!result.rebuild_all);
    assert!(result.directly_changed.contains(&"personinsights-proto".into()));
    assert!(result
        .directly_changed
...
fn old_graph_proto_propagation() {
    let (graph, images) = load_test_fixtures();
    let result = compute_affected(
        &["proto/personinsights.proto".into()],
        Some(&graph),
        &graph,
        &images,
    )
