...
3. **Auto-shrinker (`--shrink-failures`).** For each ast_mismatch
   ...
   handful. Needs the optional `insightsql-parser-parity` dependency group
...
6. **Crash bucketing.** A query that makes a backend raise a non-InsightsQL
...
Typical usage:
    ...
    PYTHONPATH=. python insights/insightsql/scripts/pbt_diagnostic.py --n 5000
    ...
    # first (rust: `uv pip install -e rust/insightsql/parser`; cpp: rebuild
    # `common/insightsql_parser` and copy the built .so into `.flox/cache/venv`).
    # `--write-divergences` streams each finding as it's found, so a long run
    # that gets killed still leaves its divergences on disk:
    PYTHONPATH=. .flox/cache/venv/bin/python insights/insightsql/scripts/pbt_diagnostic.py \\
...
# Eager so a missing `insightsql-parser-parity` dep group fails at script-load,
# not mid-grind where shrink_to_shape would lose every finding so far.
from insights.insightsql.scripts import _shrink  # noqa: F401
...
from insights.insightsql.test.test_parser_grammar_pbt import (
    _PBT_SETTINGS,
...
def main() -> int:
    ...
    # A candidate that raises a non-InsightsQL exception (RecursionError, a
    ...
    if crash_buckets:
        ...
        print()
        print(f"=== candidate_crash categories ({total} total — non-InsightsQL exceptions) ===")
