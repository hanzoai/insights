...
description: >
  ...
  Postgres, Datastore downstream health, or any ingestion worker question.
  Covers prod-us and prod-eu environments.
---

# Monitoring the ingestion pipeline with Grafana MCP

The ingestion pipeline (`nodejs/`) is Insights's Node.js event processing layer.
...
and produces enriched events to Datastore-bound Kafka topics.

A single codebase is deployed as **many K8s Deployments** via the `insights-app`
...
and Datastore `type` label values are **identical** across prod-us and prod-eu.
...
- Both envs use a **dedicated ingestion MSK** cluster separate from the events cluster —
  ...
  (prod-us: c21; prod-eu: `insights-prod-eu-ingestion-2026-05-04`).
...
The prod-eu primary Redis is `insights-prod-redis-encripted` (sic — the typo is in the actual cluster name).
...
1. **MSK ingestion cluster** (consume side) — `KAFKA_CONSUMER_METADATA_BROKER_LIST`.
   ...
   Both envs have a dedicated cluster (prod-us: c21, 12 brokers; prod-eu: `insights-prod-eu-ingestion-2026-05-04`).
   KMinion: `kminion-msk-ingestion`.

2. **WarpStream ingestion VC** (output side) — `KAFKA_WARPSTREAM_PRODUCER_METADATA_BROKER_LIST`.
   Ingestion workers produce ALL Datastore-bound output here (`datastore_events_json`,
   `datastore_person`, `datastore_groups`, `datastore_heatmap_events`, `datastore_ai_events_json`, etc.).
...
- **MSK (events/analytics)** — the original events cluster. Still carries some legacy topics and
  Datastore consumer groups during migration. prod-us: `insights-prod-us-events-2026-03-08`
  (12 brokers, `kafka.m7g.8xlarge`); prod-eu: `insights-prod-eu-events-2025-10-16` (15 brokers).
...
prod-eu: `insights-cloud-prod-eu-central-1` and `insights-cloud-persons-prod-eu-central-1`.

### Datastore topology

Datastore is the **ultimate downstream consumer** of events the ingestion pipeline produces.
...
**Cluster roles** (discovered via `type` label on `DatastoreMetrics_*`):
...
**Two consumption paths from Kafka to Datastore:**

Datastore now reads primarily from the **WarpStream ingestion VC** (where ingestion workers
produce their output), not directly from MSK.

1. **Datastore Kafka Engine** — native CH feature. Metrics prefixed `DatastoreProfileEvents_Kafka*`
   (e.g., `KafkaMessagesPolled`, `KafkaRowsRead`, `KafkaRowsRejected`, `KafkaCommitFailures`).
   Consumer groups: `datastore_events_json` (prod-us), `group1` / `group1_recent` (prod-eu).
...
- `kminion_kafka_consumer_group_topic_lag{app_kubernetes_io_instance="kminion-warpstream-ingestion", group_id=~"datastore_events_json|group1|group1_recent", topic_name="datastore_events_json"}` — lag between ingestion output and CH consumption on the WarpStream path (the primary path)
- `kminion_kafka_consumer_group_topic_lag_seconds` with same group filter — same in seconds
- `DatastoreProfileEvents_KafkaRowsRejected` — rows CH couldn't parse/insert
- `DatastoreProfileEvents_FailedInsertQuery` — insert failures (schema issues, too many parts, etc.)
- `DatastoreAsyncMetrics_MaxPartCountForPartition` — rising part count = merge pressure
- `DatastoreMetrics_ReadonlyReplica` — replicas that fell behind and went read-only
- `DatastoreAsyncMetrics_ReplicasMaxAbsoluteDelay` — max replication delay
- `insights_celery_datastore_table_parts_count` / `_table_row_count` — Django-side CH health monitors
...
Repeat with other prefixes: `consumed_batch_*`, `person_*`, `personinsights_*`,
`overflow_redirect_*`, `DatastoreMetrics_*`, `kafka_connect_*`, etc.
...
3. `query_loki_logs` — e.g. `{app=~"ingestion-.*"} |= "error"` or `{namespace="datastore"} |= "Exception"`
...
1. `search_dashboards` — query `"ingestion"` or `"datastore"` or a specific deployment name
...
- Infrastructure: CloudWatch datasource `P034F075C744B399F` for ElastiCache
  ...
  Cluster IDs: `ingestion-prod-redis`, `insights-solo` (prod-us primary),
...
- CloudWatch RDS: datasource `P034F075C744B399F` with cluster IDs
  `insights-cloud-prod-us-east-1` / `insights-cloud-persons-prod-us-east-1`

### Datastore

- Cluster health: `list_prometheus_metric_names` with regex `"DatastoreMetrics_.*"` or `"DatastoreProfileEvents_.*"`
  Scope with `type` label for cluster role (e.g. `type="events"`)
- Kafka engine health: regex `"DatastoreProfileEvents_Kafka.*"`
...
- Consumer lag (the bridge): `kminion_kafka_consumer_group_topic_lag` with
  `group_id=~"datastore_events_json|group1|group1_recent|connect-events-ducklake.*"` and `topic_name="datastore_events_json"`.
...
- Logs: `{namespace="datastore"} |= "Exception"` or `{namespace="kafka-connect"}`
- Dashboards: `vm-datastore-cluster-overview`, `datastore-ingestion-overview-20260615`,
...
merge failures, properties size. Metrics: `person_*`, `group_*`, `personinsights_*`.

**Outputs** — Kafka production to Datastore-bound topics. Message size, latency, errors.
...
**Datastore downstream health** — CH cluster QPS, memory, disk, merge pressure,
...
to the query layer. Metrics: `DatastoreMetrics_*`, `DatastoreProfileEvents_*`,
`DatastoreAsyncMetrics_*` scoped by `type`; `kafka_connect_*` scoped by `namespace`.

**K8s resources** — `container_*` and `kube_*` for CPU, memory, restarts, HPA state.
Scope: `namespace=~"ingestion-.*"`, `pod=~"ingestion-.*"` (or `namespace="datastore"`,
