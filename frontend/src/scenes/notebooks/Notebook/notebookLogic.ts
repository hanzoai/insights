...
export const notebookLogic = kea<notebookLogicType>([
            ...
            null as NotebookType | null,
            {
                ...
                saveNotebook: async ({ notebook }) => {
                    ...
                    try {
                        ...
                        refreshTreeItem('notebook', String(values.notebook.short_id))
                        insights.capture('notebook saved', {
                        ...
                        if (error.status === 409 && error.data?.updates) {
                            ...
                            insights.capture('notebook markdown save conflict retried', {
                                short_id: values.notebook.short_id,
                            ...
                            if (serverMarkdown === null) {
                                ...
                                insights.capture('notebook markdown full reload', {
                                    short_id: values.notebook.short_id,
                        ...
                        insights.capture('notebook save failed', {
                            short_id: values.notebook.short_id,
                            save_path: 'markdown_realtime',
                            is_markdown: true,
                            status: error.status,
    ...
    listeners(({ values, actions, cache }) => ({
            ...
            cache.disposables.add(
                () => {
                    ...
                    const onMessage = (msg: EventSourceMessage): void => {
                        ...
                        if (msg.event === 'presence' && msg.data) {
                            try {
                                ...
                                insights.captureException(e as Error, { action: 'notebook presence stream parse' })
                            }
                            return
                        }
                        ...
                        if (msg.event === 'update' && msg.data) {
                            try {
                                ...
                                insights.captureException(e as Error, { action: 'notebook markdown stream parse' })
                            }
                        }
                    ...
                    const onError = (error: any): void => {
                        ...
                        insights.captureException(error instanceof Error ? error : new Error(message), {
        ...
        reportMarkdownMergeConflicts: ({ conflicts }) => {
            ...
            insights.capture('notebook markdown merge conflict', {
                short_id: values.notebook?.short_id,
            ...
            toast.warning(
                    ...
                    : `Your edits and a collaborator's edits to ${conflicts.length} blocks couldn't be merged — your versions are showing.`,
                {
                    ...
                    button: {
                        label: 'Review',
                        action: () => {
                            insights.capture('notebook markdown merge conflict reviewed', {
                                short_id: values.notebook?.short_id,
                                conflict_count: conflicts.length,
                            })
        ...
        loadNotebookSuccess: ({ notebook }) => {
            ...
            if (!cache.hasCapturedOpen) {
                ...
                if (openedEvent) {
                    ...
                    insights.capture('notebook opened', openedEvent)
                }
            }
        },
        loadNotebookFailure: () => {
