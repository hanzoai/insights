...
_MONGO_UNREACHABLE_MESSAGE = (
    "Could not reach your MongoDB cluster. Check that the cluster is running and that Insights's "
...
_MONGO_ATLAS_SQL_MESSAGE = (
    ...
    "(its host ends in .query.mongodb.net), which Insights can't import from — those endpoints "
...
_MONGO_NOT_AUTHORIZED_MESSAGE = (
    "Insights connected to MongoDB, but this user isn't authorized to list collections on the "
...
_MONGO_NO_COLLECTIONS_MESSAGE = (
    "Insights connected to MongoDB but found no collections in the selected database. Check that "
...
class MongoDBSource(SimpleSource[MongoDBSourceConfig], ValidateDatabaseHostMixin):
    ...
    def get_retryable_errors(self) -> set[str]:
        ...
        # resolver Insights's worker queries, unrelated to the user's cluster hostname — so Temporal
    ...
    def validate_credentials(
        ...
        try:
            ...
            # message for and classify as non-retryable — never a Insights bug — so don't report
