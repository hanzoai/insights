...
_INSIGHTSQL_A = {"kind": "InsightsQLQuery", "query": "select count() from events"}
_INSIGHTSQL_B = {"kind": "InsightsQLQuery", "query": "select count() from persons"}
...
class TestMetricUpsert(BaseTest):
    ...
    def test_refine_leaves_unspecified_fields_untouched(self) -> None:
        self._upsert(
            ...
            ai_model="claude",
            definition={"kind": "InsightsQLQuery", "query": "select count() from events"},
        )
        refined = self._upsert("mrr", description="v2")
        assert refined.description == "v2"
        assert refined.definition is not None
        assert refined.definition["kind"] == "InsightsQLQuery"
...
class TestValidateMetricDefinition(BaseTest):
    ...
    def test_valid_insightsql_extracts_referenced_tables(self, _name: str, query: str, expected: list[str]) -> None:
        _, tables = validate_metric_definition({"kind": "InsightsQLQuery", "query": query}, self.team, self.user)
        assert tables == expected

    def test_insight_viz_node_is_unwrapped(self) -> None:
        canonical, _ = validate_metric_definition(
            {"kind": "InsightVizNode", "source": {"kind": "InsightsQLQuery", "query": "select 1"}},
            self.team,
            self.user,
        )
        assert canonical["kind"] == "InsightsQLQuery"
    ...
    @parameterized.expand(
        [
            ...
            ("raw_connection", {"kind": "InsightsQLQuery", "query": "select 1", "connectionId": "abc"}),
            ("raw_query", {"kind": "InsightsQLQuery", "query": "select 1", "sendRawQuery": True}),
            ("bad_sql", {"kind": "InsightsQLQuery", "query": "not valid sql !!!"}),
...
class TestCreateFromInsight(BaseTest):
    def _insight(self, query: dict | None = None) -> Insight:
        return Insight.objects.create(team=self.team, created_by=self.user, query=query or _INSIGHTSQL_A)
    ...
    def test_snapshots_query_and_is_not_drifted(self) -> None:
        ...
        assert metric.definition["kind"] == "InsightsQLQuery"
    ...
    def test_reordered_insight_query_is_not_drift(self) -> None:
        insight = self._insight(query={"kind": "InsightsQLQuery", "query": "select 1"})
        metric = upsert_metric(
            team=self.team, user=self.user, name="mrr", description="d", source_insight_short_id=insight.short_id
        )
        # Same query, keys reordered: canonicalization (sort_keys + upgrade) must not read as drift.
        Insight.objects.filter(pk=insight.pk).update(query={"query": "select 1", "kind": "InsightsQLQuery"})
        assert compute_drift([metric])[metric.id] is False
...
class TestApproveMetric(BaseTest):
    ...
    def test_approve_sets_status_and_approver(self) -> None:
        metric = upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL_A)
    ...
    def test_approve_blocked_while_drifted(self) -> None:
        ...
        Insight.objects.filter(pk=insight.pk).update(query=_INSIGHTSQL_B)
    ...
    def test_approve_checks_drift_on_the_current_row_not_the_loaded_instance(self) -> None:
        ...
        insight_b = self._insight(query=_INSIGHTSQL_B)
        update_metric(metric, team=self.team, user=self.user, source_insight_short_id=insight_b.short_id)
        Insight.objects.filter(pk=insight_b.pk).update(query={"kind": "InsightsQLQuery", "query": "select 1"})
...
class TestUpdateResetsApproval(BaseTest):
    def test_editing_definition_resets_approval(self) -> None:
        metric = upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL_A)
        approve_metric(metric, self.user)
        updated = update_metric(metric, team=self.team, user=self.user, definition=_INSIGHTSQL_B)
    ...
    def test_refine_definition_resets_approval(self) -> None:
        metric = upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL_A)
        approve_metric(metric, self.user)
        refined = upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL_B)
    ...
    def test_stale_update_after_approve_still_resets_approval(self) -> None:
        ...
        update_metric(stale, team=self.team, user=self.user, definition=_INSIGHTSQL_B)
...
class TestUpdateInsightLink(BaseTest):
    ...
    def test_definition_edit_unlinks_source_insight(self, path: str) -> None:
        ...
        if path == "update":
            edited = update_metric(metric, team=self.team, user=self.user, definition=_INSIGHTSQL_B)
        else:
            edited = upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL_B)
