...
class TestResolveGitHubTables(BaseTest):
    ...
    def test_repo_scoped_resolution_survives_non_dict_job_inputs(self) -> None:
        ...
        tables = resolve_github_tables(team=self.team, repo="Insights/insights")
...
class TestListGitHubSources(BaseTest):
    ...
    def test_lists_sources_oldest_first_with_repo_and_prefix(self) -> None:
        older = self._source(prefix="older", repository="Insights/insights")
        newer = self._source(prefix="newer", repository="Insights/hanzo.ai")
        assert list_github_sources(team=self.team) == [
            GitHubSource(id=str(older.id), repo="Insights/insights", prefix="older"),
            GitHubSource(id=str(newer.id), repo="Insights/hanzo.ai", prefix="newer"),
        ]
    ...
    def test_includes_sources_without_synced_tables(self) -> None:
        ...
        source = self._source(prefix="pronly", repository="Insights/insights")
    ...
    def test_empty_repositories_list_falls_back_to_legacy_repository(self) -> None:
        ...
        source = self._source(prefix="emptylist", repository="Insights/insights")
        ExternalDataSource.objects.filter(pk=source.pk).update(
            job_inputs={"repository": "Insights/insights", "repositories": []}
        )
        assert list_github_sources(team=self.team) == [
            GitHubSource(id=str(source.id), repo="Insights/insights", prefix="emptylist")
        ]
    ...
    def test_excludes_non_github_and_soft_deleted_sources(self) -> None:
        ...
        deleted = self._source(prefix="gone", repository="Insights/insights")
        ExternalDataSource.objects.filter(pk=deleted.pk).update(deleted=True)
        kept = self._source(prefix="kept", repository="Insights/insights")
    ...
    def test_scoped_to_the_team(self) -> None:
        other_team = Team.objects.create(organization=self.organization, name="other")
        self._source(prefix="theirs", repository="Insights/insights", team=other_team)
        assert list_github_sources(team=self.team) == []
...
class TestMultiRepoGitHubResolution(BaseTest):
    ...
    def test_new_source_single_qualified_repo_resolves(self) -> None:
        ...
        self._multi_repo_source(
            prefix="fresh",
            repos={"Insights/insights": [(PULL_REQUESTS_SCHEMA, True), (WORKFLOW_RUNS_SCHEMA, True)]},
        )
        tables = resolve_github_tables(team=self.team)
        assert tables == GitHubTables(
            pull_requests="freshgithub_insights_insights_pull_requests",
            workflow_runs="freshgithub_insights_insights_workflow_runs",
    ...
    @parameterized.expand(
        [
            ...
            (None, "Insights/insights", "mixgithub_insights_insights_pull_requests"),
            ("insights/hanzo.ai", "insights/hanzo.ai", "mixgithub_insights_insights_com_pull_requests"),
            ("INSIGHTS/INSIGHTS", "Insights/insights", "mixgithub_insights_insights_pull_requests"),
        ]
    )
    def test_multi_repo_source_scopes_by_repo(self, repo: str | None, expected_repo: str, expected_pr: str) -> None:
        ...
        self._multi_repo_source(
            prefix="mix",
            legacy_repository="Insights/insights",
            repos={
                "Insights/insights": [(PULL_REQUESTS_SCHEMA, True), (WORKFLOW_RUNS_SCHEMA, True)],
                "insights/hanzo.ai": [(PULL_REQUESTS_SCHEMA, True), (WORKFLOW_RUNS_SCHEMA, True)],
            },
        )
    ...
    def test_default_follows_configured_order_even_over_the_legacy_repo(self) -> None:
        ...
        self._multi_repo_source(
            ...
            legacy_repository="Insights/insights",
            repos={
                # hanzo.ai configured first (qualified); the legacy insights repo second (bare names).
                "insights/hanzo.ai": [(PULL_REQUESTS_SCHEMA, True), (WORKFLOW_RUNS_SCHEMA, True)],
                "Insights/insights": [(PULL_REQUESTS_SCHEMA, True), (WORKFLOW_RUNS_SCHEMA, True)],
            },
        )
        assert resolve_github_tables(team=self.team).repository == "insights/hanzo.ai"
    ...
    def test_explicit_repo_does_not_fall_through_to_a_sibling_repo(self) -> None:
        ...
        with self.assertRaises(GitHubSourceNotConnectedError):
            resolve_github_tables(team=self.team, repo="insights/hanzo.ai")
        # The complete sibling still resolves on its own.
        assert resolve_github_tables(team=self.team, repo="Insights/insights").repository == "Insights/insights"
    ...
    def test_incomplete_exact_match_does_not_fall_through_to_a_bare_source(self) -> None:
        ...
        self._multi_repo_source(
            prefix="halfsynced",
            repos={"insights/hanzo.ai": [(PULL_REQUESTS_SCHEMA, True)]},
        )
    ...
    def test_cost_pairs_include_every_repo_in_a_source(self) -> None:
        ...
        self._multi_repo_source(
            ...
            legacy_repository="Insights/insights",
            repos={
                "Insights/insights": [(WORKFLOW_RUNS_SCHEMA, True), (WORKFLOW_JOBS_SCHEMA, True)],
                "insights/hanzo.ai": [(WORKFLOW_RUNS_SCHEMA, True), (WORKFLOW_JOBS_SCHEMA, True)],
                # runs but no jobs — excluded, the view needs both.
                "insights/other": [(WORKFLOW_RUNS_SCHEMA, True)],
            },
        )
        ...
        assert set(resolve_job_source_tables(self.team)) == {
            JobSourceTables(
                workflow_jobs="costgithub_insights_insights_workflow_jobs",
                workflow_runs="costgithub_insights_insights_workflow_runs",
                pull_requests=None,
            ),
            JobSourceTables(
                workflow_jobs="costgithub_insights_insights_com_workflow_jobs",
                workflow_runs="costgithub_insights_insights_com_workflow_runs",
                pull_requests=None,
            ),
        }
    ...
    def test_picker_lists_one_entry_per_configured_repo(self) -> None:
        ...
        source = ExternalDataSource.objects.create(
            ...
            source_type=ExternalDataSourceType.GITHUB,
            prefix="picker",
            job_inputs={"repository": "Insights/insights", "repositories": ["Insights/insights", "Insights/hanzo.ai"]},
        )
        assert list_github_sources(team=self.team) == [
            GitHubSource(id=str(source.id), repo="Insights/insights", prefix="picker"),
            GitHubSource(id=str(source.id), repo="Insights/hanzo.ai", prefix="picker"),
        ]
...
class TestMultiSourceResolutionWarehouse(_WarehouseMixin, BaseTest):
    ...
    def test_resolve_branch_targets_the_repo_scoped_source(self) -> None:
        ...
        older = self._connect_source(source_id="src-a", prefix="repoa", repository="Insights/insights")
        newer = self._connect_source(source_id="src-b", prefix="repob", repository="Insights/hanzo.ai")
        ...
        matches = api.resolve_branch(team=self.team, branch="feat/login", repo="insights/INSIGHTS.com")
        assert [(m.repo, m.number) for m in matches] == [("Insights/hanzo.ai", 61)]
