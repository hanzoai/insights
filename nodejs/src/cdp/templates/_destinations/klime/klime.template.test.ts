...
describe('klime template', () => {
    ...
    it('sends a track event with correct shape', async () => {
        ...
        expect(batchEvent).toMatchObject({
            ...
            context: { library: { name: 'insights-cdp', version: '1.0.0' } },
        })
    })
    ...
    it('sends custom property mapping as properties on track events', async () => {
        const response = await tester.invoke({
            ...defaultInputs,
            properties: { plan: 'enterprise', source: 'insights' },
        })

        expect(response.error).toBeUndefined()
        expect(parseBatchEvent(response).properties).toEqual({ plan: 'enterprise', source: 'insights' })
    })
