...
description: >
  Investigates distributed application performance using Insights APM (OpenTelemetry span) data via MCP.
  ...
  Uses insights:query-apm-spans, insights:apm-trace-get, insights:apm-spans-sparkline,
  insights:apm-services-list, insights:apm-attributes-list, and insights:apm-attribute-values-list.
---

# Exploring APM traces (OpenTelemetry spans)

Insights captures distributed traces from OpenTelemetry. Each trace is a tree of spans representing a request's path through services.

**Disambiguation:** This skill is for **APM / OpenTelemetry traces**. Do not confuse with **AI observability traces** (agent/model `$ai_*` events) or **logs** (`insights:query-logs`, `insights:logs-*`).
...
| `insights:apm-spans-tree`               | Call-tree aggregates per `(parent, child)` edge   |
| `insights:apm-spans-count`              | Scalar span count — cheap filter pre-flight       |
| `insights:apm-spans-sparkline`          | Span counts over time (zero-filled time series)   |
| `insights:apm-spans-duration-histogram` | Trace counts per log-scale duration bucket        |
| `insights:apm-attribute-breakdown`      | Span counts grouped by one attribute's value      |
| `insights:apm-services-list`            | List distinct service names                       |
| `insights:apm-attributes-list`          | List span or resource attribute keys              |
| `insights:apm-attribute-values-list`    | List values for a specific attribute key          |
...
```json
insights:apm-trace-get
{
  "trace_id": "<hex_trace_id>"
}
```

The response is `{ results: [span, span, …], _insightsUrl: "…" }` — a flat list of every span in the trace.
...
- The `_insightsUrl` — a deep link to this trace in the tracing UI; **always include this in your response** so the user can click through
...
`apm-trace-get` returns a `_insightsUrl` deep link that opens the trace in the tracing UI — **always surface this to the user** so they can verify in the Insights UI.

`query-apm-spans` does not return `_insightsUrl`.
To link a trace found via the query tool, feed its `trace_id` to `apm-trace-get` and surface the `_insightsUrl` from that response.
Never hand-construct Insights URLs.

## Finding traces

Use `insights:query-apm-spans` to search and filter spans. Note this returns spans, not a tree — pass `query.traceId` or grab a `trace_id` from the results and feed it to `apm-trace-get` for the tree.
...
- Always include the `_insightsUrl` from `apm-trace-get` in your response so the user can click through to the trace.
