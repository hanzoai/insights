...
type Action = Extract<CustomFlowAction, { type: 'wait_until_time_window' }>
...
export class WaitUntilTimeWindowHandler implements ActionHandler {
    execute({
        invocation,
        action,
    }: ActionHandlerOptions<Extract<CustomFlowAction, { type: 'wait_until_time_window' }>>): ActionHandlerResult {
...
export const getWaitUntilTime = (
    action: Extract<CustomFlowAction, { type: 'wait_until_time_window' }>,
