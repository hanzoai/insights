...
Consistency caveat (documented, accepted): the signal rows go Kafka -> MV -> Datastore and are NOT
atomic with the Postgres report write. A read immediately after `create_scout_report` may see
`signal_count=N` in Postgres but zero rows in Datastore for ~1s. Callers (and the emit-and-inspect
...
class ScoutReportSignal:
    ...
    `edit_report` can address it for soft-delete). `document_id` is the Datastore row id; left
...
class PersistedScoutReport:
    ...
    signal_count: int
    total_weight: float
    # The Datastore `document_id`s written for the backing signals, in input order. Lets a caller
