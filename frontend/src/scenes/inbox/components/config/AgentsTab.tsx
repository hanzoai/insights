...
function BackLink({ onClick }: { onClick: () => void }): JSX.Element {
    return (
        <Button type="tertiary" size="small" icon={<IconArrowLeft />} onClick={onClick} className="-ml-2 w-fit">
            Back
        </Button>
    )
}
 ...
 * Insights Desktop Agents view. Composes Connections, the agent roster,
        ...
        <div className="flex flex-col">
            <div className="mx-auto max-w-4xl w-full px-6 py-6 flex flex-col gap-8">
                ...
                {featureFlags[FEATURE_FLAGS.INBOX_SLACK_NOTIFICATIONS] && (
                    <Subsection
                        title="Slack"
                        description="Post reports to channels and ping suggested reviewers. Invite Insights with /invite @Insights in each channel you use."
                    >
