...
def resolve_insights_user_for_teams(email: str | None, team: Team) -> User | None:
    """Match a Teams user's email to a Insights user within the team's organization."""
    if not email:
        return None
    membership = (
...
def _build_help_card() -> dict[str, Any]:
    ...
    not Insights operators, so an "Open Insights" link would 404 for them."""
    return {
...
def create_or_update_teams_ticket(
    *,
    ...
    insights_user = resolve_insights_user_for_teams(user_info.get("email"), team)
    ...
    if graph_post_context:
        ...
        post_teams_channel_message_via_graph(
            ...
            channel_id=channel_id,
            html=f'\U0001f3ab Ticket #{ticket.ticket_number} created. <a href="{ticket_url}">View in Insights</a>',
