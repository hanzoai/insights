...
from insights.temporal.backfill_group_type_created_at.activities import (
...
# Integration tests below exercise the real activities against Postgres + Datastore.
...
def _read_created_at(project_id: int, index: int) -> datetime | None:
    with persons_db_connection(writer=True) as conn, conn.cursor() as cursor:
        cursor.execute(
            "SELECT created_at FROM insights_grouptypemapping WHERE project_id = %s AND group_type_index = %s",
            (project_id, index),
        )
        row = cursor.fetchone()
...
class TestApplyGroupTypeCreatedAtBackfillIntegration:
    ...
    async def test_invalidates_group_types_cache(self):
        ...
        with patch(
            "insights.temporal.backfill_group_type_created_at.activities.invalidate_group_types_cache"
        ) as mock_invalidate:
            await self.activity_environment.run(
                apply_group_type_created_at_backfill,
                ApplyBackfillInput(
