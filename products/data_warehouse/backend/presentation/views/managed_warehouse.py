...
identifier is the Insights `organization_id` and the Data ops feature flag is evaluated
...
from insights.security.outbound_proxy import internal_requests
...
def _block_if_pending_deletion(organization_id: UUID | str) -> Response | None:
    ...
    accessible.
    """
    # Keep insights.models off this adapter's import path.
    from insights.models.organization import Organization  # noqa: PLC0415
...
def _validate_schema_name(name: str | None) -> str | None:
    ...
    from insights.ducklake.common import validate_schema_name  # noqa: PLC0415
...
def team_backfill_state(team_id: int) -> dict:
    ...
    # Keep ducklake.common (and its duckdb dependency) off the API import path.
    from insights.ducklake.common import get_team_backfill_state  # noqa: PLC0415
...
def _persist_duckgres_server(organization_id: UUID | str, database_name: str | None, body: dict) -> None:
    ...
    """
    # Keep ducklake.common (and its duckdb dependency) off the API import path.
    from insights.ducklake.common import default_bucket_region, upsert_duckgres_server_for_org  # noqa: PLC0415
...
def list_all_teams() -> Response:
    ...
    Backend-only: feeds the cp-mode sensor enumeration in insights.ducklake.cp_teams,
...
def _invalidate_team_state_cache(organization_id: UUID | str) -> None:
    from insights.ducklake import cp_teams  # noqa: PLC0415
...
def onboard_team(
    ...
    if existing is not None:
        ...
        # Pin the legacy table names the duckling DAG writes today (insights.events_<suffix>,
        # insights_data_imports_<suffix>): the suffix for a newly onboarded team IS its schema
...
def _org_has_warehouse(organization_id: UUID | str) -> bool:
    ...
    from insights.ducklake.models import DuckgresServer  # noqa: PLC0415
...
def _legacy_table_fields(schema_name: str) -> dict:
    ...
    (`insights.events_<schema>`, `insights_data_imports_<schema>`), so a new row pins those
    ...
    return {
        ...
        "schema_data_imports_name": f"insights_data_imports_{schema_name}",
    }
...
def block_team_deletion(team_id: int, organization_id: UUID | str) -> str | None:
    ...
    # API import path.
    from insights.ducklake import team_state  # noqa: PLC0415
    from insights.ducklake.models import DuckgresServer  # noqa: PLC0415
...
def _reconcile_bucket_from_status(organization_id: UUID | str, body: dict) -> None:
    ...
    if not bucket:
        ...
        return
    # Keep ducklake.common (and its duckdb dependency) off the API import path.
    from insights.ducklake.common import default_bucket_region  # noqa: PLC0415
