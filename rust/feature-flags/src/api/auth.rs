...
pub const TOKEN_CACHE_PREFIX: &str = "insights:auth_token:";
...
///
/// The unified loader tries Team.secret_api_token first, then insights_projectsecretapikey.
/// Both share the same cache key space (insights:auth_token:{hash}), so a single loader
...
async fn load_token_from_pg(
    ...
    match Team::from_pg_by_secret_token(pg_reader.clone(), plaintext_token).await {
        ...
        Err(FlagError::RowNotFound) => { /* token not in insights_team, try PSAK below */ }
        Err(e) => return Err(e),
    }

    // Try ProjectSecretAPIKey (JOIN with insights_team to get api_token for HyperCache lookups)
    let mut conn =
    ...
    let query = r#"
        ...
        FROM insights_projectsecretapikey k
        JOIN insights_team t ON t.id = k.team_id
    ...
    let query = r#"
        SELECT
            ...
            ARRAY(
                ...
                FROM insights_organizationmembership om
...
/// Ported from Insights's `hash_key_value` function in `insights/models/utils.py`
