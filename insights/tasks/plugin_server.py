...
from insights.tasks.email import get_members_to_notify_for_pipeline_error, send_fatal_plugin_error
...
# Called from plugin-server/../script-watcher.service.ts
@shared_task(ignore_result=True, queue=queue)
def insights_function_state_transition(insights_function_id: str, state: int) -> None:
    ...
    #     "script function state changed",
    #     {
    #         "insights_function_id": insights_function_id,
    #         "insights_function_url": f"{settings.SITE_URL}/project/{insights_function.team.id}/pipeline/destinations/script-{insights_function_id}",
    #         "state": state,
    #     },
    # )

    # # TRICKY: It seems like without this call the events don't get flushed, possibly due to celery worker threads exiting...
    # logger.info("insights_function_state_transition: Flushing hanzo_insights")
