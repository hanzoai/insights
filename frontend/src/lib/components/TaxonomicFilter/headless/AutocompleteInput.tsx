 ...
 * before commit (DWH columns, InsightsQL editor). `details` = read-only
 ...
 * also includes render-driven groups like `InsightsQLExpression` and
...
export function Root({
    ...
    const visibleGroups = useMemo(
        ...
        // InsightsQL surfaced as menu shortcuts, not chips).
        () => {
    ...
    const indexed = useMemo<IndexedItem[]>(() => {
        ...
        for (const group of targetGroups) {
            ...
            // Render-driven groups (e.g. `InsightsQLExpression` with InlineInsightsQLEditor)
 ...
 *
 * Sub-views (DataWarehouse field config, InsightsQL editor, etc.) render inside
...
function DefaultRow({ entry }: DefaultRowProps): JSX.Element {
    ...
    // Rows whose group has a `<ConfigureView>` registered (DWH, InsightsQL) get
...
export interface TaxonomicAutocompleteMenuTriggerProps {
     ...
     * from popover chips/list. Default: DataWarehouse + InsightsQLExpression.
     */
 ...
 *   - Per-shortcut group → DWH opens popover at DWH category, InsightsQL opens
 *     the editor sub-page
 *
                    ...
                    <DropdownMenuItem
                        onClick={() => {
                            ...
                            const synthetic: IndexedItem = {
                                item: { name: insightsqlGroup.name } as TaxonomicDefinitionTypes,
                                group: insightsqlGroup,
                                name: insightsqlGroup.name,
                            }
                            ctx.openConfigureFor(synthetic)
                            ctx.setOpen(true)
                        }}
                        data-attr="taxonomic-menu-shortcut-insightsql"
                    >
                        {insightsqlGroup.name}
                    </DropdownMenuItem>
                )}
            </DropdownMenuContent>
        </DropdownMenu>
