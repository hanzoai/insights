...
from products.conversations.backend.slack import (
    ...
    resolve_insights_user_for_slack,
)
...
class TestResolveInsightsUserForSlack(BaseTest):
    def test_returns_user_when_email_matches_org_member(self):
        result = resolve_insights_user_for_slack(self.user.email, self.team)
        assert result is not None
        assert result.id == self.user.id

    def test_returns_none_for_non_member_email(self):
        result = resolve_insights_user_for_slack("stranger@example.com", self.team)
        assert result is None

    def test_returns_none_for_empty_email(self):
        assert resolve_insights_user_for_slack(None, self.team) is None
        assert resolve_insights_user_for_slack("", self.team) is None

    def test_scoped_to_team_organization(self):
        other_org = Organization.objects.create(name="Other Org")
        other_team = Team.objects.create(organization=other_org)
        result = resolve_insights_user_for_slack(self.user.email, other_team)
        assert result is None
...
class TestSlackEchoPreventionSignal(BaseTest):
    ...
    @patch("products.conversations.backend.tasks.post_reply_to_slack.delay")
    def test_insights_ui_team_message_does_echo_to_slack(self, mock_delay, _on_commit):
        Comment.objects.create(
            team=self.team,
            scope="conversations_ticket",
            item_id=str(self.slack_ticket.id),
            content="Reply from Insights UI",
            created_by=self.user,
