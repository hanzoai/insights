...
- **A chart renders data, it does not run code.** ScriptVM `bytecode`, a nested `HogQuery`, `sendRawQuery`, and a nested `SuggestedQuestionsQuery` are each refused wherever they sit in the node. A warehouse query is fine through InsightsQL — keep `connectionId`, drop `sendRawQuery`.
