...
function WorkflowRunMetrics(props: WorkflowLogicProps): JSX.Element {
    const logicKey = `script-flow-metrics-${props.id}`
    const { searchParams } = useValues(router)
    const { workflow, insightsFunctionTemplatesById } = useValues(workflowLogic)
    ...
    const workflowStepOptions: SelectOptions<string> = useMemo(
        () => [
            {
        ...
        [workflow.actions, insightsFunctionTemplatesById]
    )
...
function BatchJobMetricsHeader({ job }: { job: InsightsFlowBatchJob }): JSX.Element {
    return (
...
function BatchJobMetrics({ job }: { job: InsightsFlowBatchJob }): JSX.Element {
    const logicKey = `script-flow-metrics-batch-${job.id}`
    ...
    if (!jobs.length) {
        return (
            <div className="flex flex-col bg-surface-primary rounded px-4 py-8 items-center text-center mx-auto">
                <MascotGreek width="100" height="100" className="mb-4" />
