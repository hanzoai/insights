...
export function buildSemanticColors(): Record<string, ColorTuple> {
    return {
        ...
        // Matched to Insights app's --color-bg-primary so quill panels blend
        // with the surrounding app: light hsl(75 14% 95%), dark hsl(240 8% 8%),
        // converted to OKLCH. Dark equals `muted` — Insights uses one token for
        ...
        // Matched to Insights app's --color-bg-surface-secondary (subdued cards,
        ...
        // Insights app's --color-bg-surface-tertiary: light hsl(77 13% 89%),
...
function assertThemeDerivedSyncedWithColors(colors: Record<string, ColorTuple>): void {
    ...
    for (const [key, [light, dark]] of Object.entries(colors)) {
        ...
        if (refsThemeVar && !THEME_DERIVED_TOKENS.has(key)) {
            throw new Error(
                `[@hanzo/quill-tokens] Token "${key}" references a theme variable ` +
