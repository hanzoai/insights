...
import { DatastoreConfig, getDefaultDatastoreConfig } from '../common/datastore-config'
import {
    CdpProducerName,
    WAREHOUSE_PRODUCER,
...
// CdpConfig intersects DatastoreConfig so any consumer reading
// `this.config.DATASTORE_HOST` etc. gets typed, defaulted values — fixes the
// case where `CdpRerunWorkerConsumer` silently fell back to `default` DB.
export type CdpConfig = DatastoreConfig & {
    CDP_WATCHER_COST_ERROR: number
    CDP_WATCHER_FN_COST_TIMING: number
    CDP_WATCHER_FN_COST_TIMING_LOWER_MS: number
    CDP_WATCHER_FN_COST_TIMING_UPPER_MS: number
    ...
    CDP_FN_FILTERS_TELEMETRY_TEAMS: string
...
export function getDefaultCdpConfig(): CdpConfig {
    return {
        ...
        CDP_WATCHER_FN_COST_TIMING: 100,
        CDP_WATCHER_FN_COST_TIMING_LOWER_MS: 50,
        CDP_WATCHER_FN_COST_TIMING_UPPER_MS: 550,
        ...
        CDP_FN_FILTERS_TELEMETRY_TEAMS: '',
        DISABLE_OPENTELEMETRY_TRACING: false,
        CDP_CYCLOTRON_JOB_QUEUE_CONSUMER_KIND: 'script',
        ...
        CDP_PREFILTERED_EVENTS_TOPIC: KAFKA_CDP_DATASTORE_PREFILTERED_EVENTS,
        CDP_PREFILTERED_EVENTS_PRODUCER: WARPSTREAM_CALCULATED_EVENTS_PRODUCER,
        CDP_PRECALCULATED_PERSON_PROPERTIES_TOPIC: KAFKA_CDP_DATASTORE_PRECALCULATED_PERSON_PROPERTIES,
        ...
        CYCLOTRON_NODE_DATABASE_URL: isTestEnv()
            ? 'postgres://insights:insights@localhost:5432/test_cyclotron_node'
            : isDevEnv()
              ? 'postgres://insights:insights@localhost:5432/cyclotron_node'
              : undefined,
        ...
        SES_TRACKED_CONFIGURATION_SET: 'insights-messaging',
