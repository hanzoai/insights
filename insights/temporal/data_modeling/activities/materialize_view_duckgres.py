...
from insights.ducklake.common import duckgres_data_modeling_schema, get_duckgres_server_for_organization, is_dev_mode
...
def _compile_insightsql_to_postgres_sql(insightsql_query: str, team_id: int) -> tuple[str, dict[str, object]]:
    from insights.schema import InsightsQLQuery

    from insights.ducklake.client import compile_insightsql_to_ducklake_sql

    postgres_sql, values, _ = compile_insightsql_to_ducklake_sql(
        team_id,
        InsightsQLQuery(query=insightsql_query),
        # Userless shadow materialization; mirror Datastore materialization so the
...
async def materialize_view_duckgres_activity(inputs: DuckgresShadowInputs) -> DuckgresShadowResult:
    ...
    This is a fire-and-forget companion to the main Datastore-based materialize_view_activity.
    ...
    insightsql_query = typing.cast(dict, saved_query.query)["query"]
    ...
    try:
        if inputs.dangerously_execute_raw_sql:
            sql = insightsql_query
        else:
            sql, values = await database_sync_to_async_pool(_compile_insightsql_to_postgres_sql)(insightsql_query, team.pk)
        await logger.adebug("Duckgres shadow SQL generated", sql=sql)

        from insights.ducklake.client import execute_ducklake_create_table
