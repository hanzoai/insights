...
impl FeatureFlagStorage for PostgresStorage {
    async fn get_hash_key_override_context(
        ...
        // When personinsights-leader is implemented, person table reads will be served from
        // the leader's cache, and strong consistency will require routing to the leader
        // service instead. Before the personinsights-leader is implemented, we can serve consistent read after writes
        ...
        let rows = if check_person_exists {
            sqlx::query_as!(
                ...
                WHERE p.team_id = $1 AND p.distinct_id = ANY($2) AND p.is_deleted = false
                    AND EXISTS (SELECT 1 FROM insights_person WHERE id = p.person_id AND team_id = p.team_id AND is_deleted = false)
                "#,
                team_id as i32,
                distinct_ids
            )
    ...
    async fn delete_hash_key_overrides_by_teams(
        ...
        let result = sqlx::query!(
            ...
            WHERE id IN (
                SELECT id FROM insights_featureflaghashkeyoverride
                WHERE team_id = ANY($1)
                LIMIT $2
