...
from insights.admin.admins.duckgres_server_admin import DuckgresServerAdmin
from insights.models import DuckgresServer, Organization, Team
...
class TestDuckgresServerAdminProvision(BaseTest):
    ...
    def _grant_admin_permissions(self) -> None:
        permissions = Permission.objects.filter(
            content_type__app_label="insights",
            codename__in=[
                "add_duckgresserver",
    ...
    def test_change_fieldsets_do_not_expose_password_for_existing_server(self) -> None:
        request = self._get("/admin/insights/duckgresserver/")
        server = self._server()
    ...
    def test_provision_post_calls_managed_warehouse_bypassing_flag(self) -> None:
        request = self._post(
            "/admin/insights/duckgresserver/provision/",
            {
    ...
    def test_enable_backfill_post_calls_helper_bypassing_flag(self) -> None:
        server = self._server()
        request = self._post(
            f"/admin/insights/duckgresserver/{server.pk}/enable-backfill/",
    ...
    def test_enable_backfill_invalid_server_returns_404(self) -> None:
        request = self._get("/admin/insights/duckgresserver/999999/enable-backfill/")
    ...
    def test_deprovision_post_calls_helper_bypassing_flag(self) -> None:
        server = self._server()
        request = self._post(f"/admin/insights/duckgresserver/{server.pk}/deprovision/", {})
    ...
    def test_deprovision_invalid_server_returns_404(self) -> None:
        request = self._get("/admin/insights/duckgresserver/999999/deprovision/")
    ...
    def test_deprovision_head_does_not_call_helper(self) -> None:
        server = self._server()
        request = self._head(f"/admin/insights/duckgresserver/{server.pk}/deprovision/")
    ...
    def test_deprovision_failure_returns_to_change_page_and_logs(self) -> None:
        ...
        with (
            ...
            patch("insights.admin.admins.duckgres_server_admin.logger.warning") as mock_warning,
        ):
            response = self.admin.deprovision_view(request, str(server.pk))

        assert response["Location"] == reverse("admin:insights_duckgresserver_change", args=[server.pk])
