 ...
 * Insights Desktop uses). `runStreamLogic` folds the raw wire frames (typed in
 ...
 * the inner tool name for Insights's single-exec MCP server, happens outside this stream state.
 */
export interface ToolInvocation {
    toolCallId: string
    /** ACP-reported MCP server, e.g. 'insights' or '<user-installed>'. */
...
export interface ToolStreamEvent {
    ...
    streamKey: string
    toolCallId: string
    /** Resolved registry key (inner Insights MCP tool, e.g. 'create_dashboard') via `resolveToolCall`. */
    toolName: string
 ...
 * inline `_insights/*` notifications (status / compaction / task milestones) all flow through this
 * list.
 */
export interface ThreadItem {
    ...
    /**
     * For `debug` items — the `_insights/console` level (debug/info/warn/error), or `context` for the
     * copyable rows carrying a send's attached trusted/untrusted context blocks.
     */
    debugLevel?: string
}

/** One Insights product the agent grounded an answer in, accumulated across the whole session. */
 ...
 * arrive on the `_insights/usage_update` ext-notification.
 */
...
export interface PermissionRequestRecord {
    requestId: string
    toolCallId: string
    /** Canonical ACP tool name (`mcp__insights__exec`, or a built-in like `Bash`) — drives the default permission policy. */
    toolName: string
