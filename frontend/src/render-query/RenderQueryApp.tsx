...
declare global {
    interface Window {
        POSTFN_RENDER_QUERY_PAYLOAD?: RenderQueryExternalPayload
    }
}
...
function initializeState(): RenderQueryState {
    return [
        parsePayload(window.POSTFN_RENDER_QUERY_PAYLOAD, 'initial payload'),
        extractFromSearch(),
        extractFromHash(),
...
function extractFromFrameDataset(): ParsedPayload {
    try {
        ...
        console.warn('Insights render query: Unable to read iframe dataset.', error)
        return EMPTY_PAYLOAD
    }
}
