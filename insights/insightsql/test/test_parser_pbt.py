"""Property-based tests for the InsightsQL parser ↔ printer roundtrip.

We generate random AST expression trees, print them to InsightsQL strings, then
...
from insights.insightsql.scripts._diagnostic_common import ast_depth
from insights.insightsql.test._pbt_corpus_db import shared_corpus_database

# These tests are too slow for CI (~8 min). Run manually with:
#   RUN_PBT=1 pytest insights/insightsql/test/test_parser_pbt.py
...
# `insightsql_parser_rs` wheel was built with `--features coverage` (see
# `rust/insightsql/parser/README.md` → "Coverage-instrumented build"). Production
...
_insightsql_parser_rs_module: ModuleType | None
try:
    _insightsql_parser_rs_module = importlib.import_module("insightsql_parser_rs")
except ImportError:
    _insightsql_parser_rs_module = None
_RUST_COV: bool = _insightsql_parser_rs_module is not None and hasattr(_insightsql_parser_rs_module, "cov_snapshot")
# Must match `cov::BITMAP_SIZE` in rust/insightsql/parser/src/cov.rs.
...
# Identifiers that are safe for round-tripping: valid InsightsQL identifiers that
...
# A subset of functions that are known to the InsightsQL printer/parser and accept
...
# Comparison ops that the InsightsQL printer can round-trip.
...
def _print_insightsql(node: ast.Expr) -> str:
    """Print an AST node to a InsightsQL string using a minimal context."""
    return node.to_insightsql()


def _parse_both_backends(insightsql_string: str) -> tuple[ast.Expr | None, ast.Expr | None]:
    ...
    try:
        rust_ast = parse_expr(insightsql_string, backend="rust-py")
    except BaseInsightsQLError:
        pass
    try:
        cpp_ast = parse_expr(insightsql_string, backend="cpp-json")
    except BaseInsightsQLError:
        pass
...
def _roundtrip_check(expr: ast.Expr) -> None:
    ...
    try:
        printed1 = _print_insightsql(expr)
    except BaseInsightsQLError:
        assume(False)
    ...
    printed2 = _print_insightsql(parsed)
...
def _emit_rust_edges_target() -> None:
    ...
    Hypothesis `target("rust_edges", ...)`. No-op when the loaded `insightsql_parser_rs`
    ...
    if not _RUST_COV or _insightsql_parser_rs_module is None:
        return
    bitmap = np.frombuffer(_insightsql_parser_rs_module.cov_snapshot(), dtype=np.uint8)
...
class TestExprRoundTrip:
    """Print → parse → print yields the same InsightsQL string."""
...
class TestParserBackendEquivalence:
    """Both parser backends produce equivalent ASTs for generated InsightsQL strings."""
    ...
    def test_rust_py_and_cpp_backends_agree(self, expr: ast.Expr) -> None:
        target(float(ast_depth(expr)), label="ast_depth")
        try:
            insightsql_string = _print_insightsql(expr)
        except BaseInsightsQLError:
            assume(False)
            return  # type: ignore[unreachable]
        ...
        if _RUST_COV and _insightsql_parser_rs_module is not None:
            _insightsql_parser_rs_module.cov_reset()

        rust_ast, cpp_ast = _parse_both_backends(insightsql_string)
        ...
        if (rust_ast is None) != (cpp_ast is None):
            ...
            raise AssertionError(
                f"Backend disagreement on parsability of: {insightsql_string!r}\n"
        ...
        # Compare by printing both back to InsightsQL — this normalises away
        # any minor structural differences (like start/end positions).
        rust_printed = _print_insightsql(rust_ast)
        cpp_printed = _print_insightsql(cpp_ast)

        if rust_printed != cpp_printed:
            event("outcome", "ast mismatch")
            raise AssertionError(
                f"Backend outputs differ for: {insightsql_string!r}\n"
...
class TestConstantRoundTrip:
    ...
    def test_float_constant_roundtrip(self, f: float) -> None:
        ...
        printed = _print_insightsql(node)
        parsed = parse_expr(printed)
        reprinted = _print_insightsql(parsed)
