...
**authoring companion** (`review-script-authoring`, scouts'-`authoring-scouts` counterpart).
It reads the matching dirs under `products/review/skills/` and
...
from products.review.backend.reviewer.skill_loader import (
    REVIEW_FN_AUTHORING_PREFIX,
    REVIEW_FN_BLIND_SPOTS_PREFIX,
    REVIEW_FN_PERSPECTIVE_PREFIX,
    REVIEW_FN_VALIDATION_PREFIX,
)
from products.skills.backend.models.skills import LLMSkill, LLMSkillFile

logger = logging.getLogger(__name__)

# Canonical review-script-perspective-* skills live on disk under `products/review/skills/`.
...
_SOURCE = "products/review/skills"
...
class CanonicalSkill:
    """A canonical `review-script-perspective-*` skill discovered on disk.
...
def _discover_canonical(prefix: str, skills_dir: Path | None = None) -> tuple[CanonicalSkill, ...]:
    """Walk `products/review/skills/` and return every parsed `<prefix>*` skill.
...
def discover_canonical_perspectives(skills_dir: Path | None = None) -> tuple[CanonicalSkill, ...]:
    """Every parsed `review-script-perspective-*` skill on disk."""
    return _discover_canonical(REVIEW_FN_PERSPECTIVE_PREFIX, skills_dir)


def discover_canonical_validation(skills_dir: Path | None = None) -> tuple[CanonicalSkill, ...]:
    """Every parsed `review-script-validation-*` skill on disk (the single criteria skill today)."""
    return _discover_canonical(REVIEW_FN_VALIDATION_PREFIX, skills_dir)


def discover_canonical_blind_spots(skills_dir: Path | None = None) -> tuple[CanonicalSkill, ...]:
    """Every parsed `review-script-blind-spots-*` skill on disk (the single general sweep today)."""
    return _discover_canonical(REVIEW_FN_BLIND_SPOTS_PREFIX, skills_dir)


def discover_canonical_authoring(skills_dir: Path | None = None) -> tuple[CanonicalSkill, ...]:
    """Every parsed `review-script-authoring*` skill on disk (the single companion guide today)."""
    return _discover_canonical(REVIEW_FN_AUTHORING_PREFIX, skills_dir)
...
def _update_skill_from_canonical(
    ...
    with transaction.atomic():
        ...
        new_metadata["seeded_by"] = REVIEW_FN_SEEDED_BY
...
def _sync_canonicals(
    ...
    for canonical in canonicals:
        ...
        if (live.metadata or {}).get("seeded_by") != REVIEW_FN_SEEDED_BY:
    ...
    if prune:
        ...
        orphan_rows = LLMSkill.objects.filter(
            ...
            metadata__seeded_by=REVIEW_FN_SEEDED_BY,
        ...
        for row in orphan_rows:
            ...
            LLMSkill.objects.filter(
                team=team, name=row.name, deleted=False, metadata__seeded_by=REVIEW_FN_SEEDED_BY
...
def sync_canonical_perspectives(team: Team, *, prune: bool = False) -> SyncResult:
    """Reconcile a team's rows with the canonical `review-script-perspective-*` skills on disk."""
    return _sync_canonicals(
        team,
        canonicals=discover_canonical_perspectives(),
        category=REVIEW_FN_SKILL_CATEGORY,
        prefix=REVIEW_FN_PERSPECTIVE_PREFIX,
        prune=prune,
    )


def sync_canonical_validation(team: Team, *, prune: bool = False) -> SyncResult:
    """Reconcile a team's rows with the canonical `review-script-validation-*` criteria skill on disk."""
    return _sync_canonicals(
        team,
        canonicals=discover_canonical_validation(),
        category=REVIEW_FN_SKILL_CATEGORY,
        prefix=REVIEW_FN_VALIDATION_PREFIX,
        prune=prune,
    )


def sync_canonical_blind_spots(team: Team, *, prune: bool = False) -> SyncResult:
    """Reconcile a team's rows with the canonical `review-script-blind-spots-*` skill on disk."""
    return _sync_canonicals(
        team,
        canonicals=discover_canonical_blind_spots(),
        category=REVIEW_FN_SKILL_CATEGORY,
        prefix=REVIEW_FN_BLIND_SPOTS_PREFIX,
        prune=prune,
    )


def sync_canonical_authoring(team: Team, *, prune: bool = False) -> SyncResult:
    """Reconcile a team's rows with the canonical `review-script-authoring` companion guide on disk."""
    return _sync_canonicals(
        team,
        canonicals=discover_canonical_authoring(),
        category=REVIEW_FN_SKILL_CATEGORY,
        prefix=REVIEW_FN_AUTHORING_PREFIX,
        prune=prune,
    )
