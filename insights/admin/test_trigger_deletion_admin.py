...
from insights.admin.admins.organization_admin import OrganizationAdmin
from insights.admin.admins.project_admin import ProjectAdmin
from insights.admin.authorization import DELETION_AUTHORIZED_GROUP
from insights.models import Organization, Project
...
class TestOrganizationAdminTriggerDeletion(BaseTest):
    ...
    def _call(self, method: str, start_side_effect=None):
        path = f"/admin/insights/organization/{self.organization.pk}/trigger-deletion/"
        http_request = self.factory.post(path) if method == "POST" else self.factory.get(path)
        http_request.user = self.user
        _attach_messages(http_request)
        with (
            patch("insights.admin.admins.organization_admin.reverse", side_effect=_fake_reverse),
            patch(
                "insights.temporal.delete_teams.dispatch.start_delete_organization_workflow",
...
class TestProjectAdminTriggerDeletion(BaseTest):
    ...
    def _call(self, method: str, start_side_effect=None):
        path = f"/admin/insights/project/{self.project.pk}/trigger-deletion/"
        http_request = self.factory.post(path) if method == "POST" else self.factory.get(path)
        http_request.user = self.user
        _attach_messages(http_request)
        with (
            patch("insights.admin.admins.project_admin.reverse", side_effect=_fake_reverse),
            patch(
                "insights.temporal.delete_teams.dispatch.start_delete_project_data_workflow",
                side_effect=start_side_effect,
            ) as mock_start,
        ):
