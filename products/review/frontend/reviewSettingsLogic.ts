...
import { OriginProduct } from 'products/insights_ai/frontend/types/taskTypes'
import {
    reviewBlindSpotsList,
    reviewBlindSpotsPartialUpdate,
    reviewPerspectivesList,
    reviewPerspectivesPartialUpdate,
    reviewReviewsList,
    reviewReviewsPerspectiveStatsRetrieve,
    reviewReviewsRetrieve,
    reviewReviewsTriggerCreate,
    reviewSettingsPartialUpdate,
    reviewSettingsRetrieve,
    reviewValidatorsList,
    reviewValidatorsPartialUpdate,
} from 'products/review/frontend/generated/api'
import type {
...
} from 'products/review/frontend/generated/api.schemas'
...
export interface ViewedSkill {
    title: string
    body: string
    /** The `review-script-*` skill name, for the drawer's link to the skill's editor page. */
    skillName: string
}
...
// `review-script-authoring` team skill (canonical source: products/review/skills/), synced per
...
const SKILL_AUTHOR_TASKS: Record<ReviewSkillKind, { title: string; prompt: string }> = {
    perspective: {
        title: 'Create a Review perspective',
        prompt: `I'd like to create a custom Review review perspective for this Insights project.

Use the review-script-authoring skill from the Insights MCP to guide creating it — follow its review-perspective path.
...
If the review-script-authoring skill is unavailable, fall back to the Insights MCP skill tools directly: list the team's review-script-perspective-* skills and read a canonical one to learn the shape before authoring.`,
    },
    blind_spots: {
        title: 'Create a Review blind-spot check',
        prompt: `I'd like to create a custom Review blind-spot check for this Insights project.

Use the review-script-authoring skill from the Insights MCP to guide creating it — follow its blind-spot-check path.
...
If the review-script-authoring skill is unavailable, fall back to the Insights MCP skill tools directly: read the canonical review-script-blind-spots-general skill to learn the shape before authoring.`,
    },
    validator: {
        title: 'Create Review validation criteria',
        prompt: `I'd like to create custom Review validation criteria for this Insights project.

Use the review-script-authoring skill from the Insights MCP to guide creating it — follow its validation-criteria path.
...
If the review-script-authoring skill is unavailable, fall back to the Insights MCP skill tools directly: read the canonical review-script-validation-criteria skill to learn the shape before authoring.`,
    },
}
...
export interface reviewSettingsLogicValues {
    ...
    reviewsLimit: number
    reviewsScope: ReviewReviewsListScope
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface reviewSettingsLogicActions {
    applyDefaultReviewsScope: (scope: ReviewReviewsListScope) => {
    ...
    setReviewsScope: (scope: ReviewReviewsListScope) => {
        scope: ReviewReviewsListScope
    }
    setSkillSaving: (
        skillName: string,
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface reviewSettingsLogicMeta {
...
export type reviewSettingsLogicType = MakeLogicType<
    reviewSettingsLogicValues,
    reviewSettingsLogicActions,
    Record<string, any>,
    reviewSettingsLogicMeta
>

/**
 * State for the "Code review" scene: the user's Review settings (triggers + urgency
 ...
 */
export const reviewSettingsLogic = kea<reviewSettingsLogicType>([
    path(['products', 'review', 'frontend', 'reviewSettingsLogic']),
    ...
    actions({
        ...
        setReviewsScope: (scope: ReviewReviewsListScope) => ({ scope }),
        ...
        applyDefaultReviewsScope: (scope: ReviewReviewsListScope) => ({ scope }),
    ...
    loaders(({ values }) => ({
        settings: [
            ...
            {
                loadSettings: async () => await reviewSettingsRetrieve(currentProjectId()),
                updateSettings: async (patch: PatchedReviewUserSettingsApi, breakpoint) => {
                    const response = await reviewSettingsPartialUpdate(currentProjectId(), patch)
        ...
        perspectives: [
            ...
            {
                loadPerspectives: async () => await reviewPerspectivesList(currentProjectId()),
            },
        ],
        blindSpots: [
            null as ReviewBlindSpotsConfigApi[] | null,
            {
                loadBlindSpots: async () => await reviewBlindSpotsList(currentProjectId()),
            },
        ],
        validators: [
            null as ReviewValidatorConfigApi[] | null,
            {
                loadValidators: async () => await reviewValidatorsList(currentProjectId()),
            },
        ],
        recentReviewsPage: [
            null as ReviewRecentReviewsPageApi | null,
            {
                ...
                loadRecentReviews: async (_ = null, breakpoint) => {
                    ...
                    const response = await reviewReviewsList(currentProjectId(), { scope, limit })
        ...
        reviewDetail: [
            null as ReviewDetailApi | null,
            {
                loadReviewDetail: async (reviewId: string, breakpoint) => {
                    const response = await reviewReviewsRetrieve(currentProjectId(), reviewId)
        ...
        perspectiveStats: [
            null as ReviewPerspectiveStatsApi | null,
            {
                ...
                loadPerspectiveStats: async (_ = null, breakpoint) => {
                    const response = await reviewReviewsPerspectiveStatsRetrieve(currentProjectId(), {
    ...
    reducers({
        ...
        reviewsScope: [
            ReviewReviewsListScope.Mine as ReviewReviewsListScope,
            { persist: true },
            {
    ...
    listeners(({ actions, values, cache }) => ({
        ...
        loadRecentReviewsSuccess: () => {
            ...
            if (
                ...
                values.reviewsScope === ReviewReviewsListScope.Mine &&
                !values.hasUserChosenReviewsScope
            ) {
                actions.applyDefaultReviewsScope(ReviewReviewsListScope.Everyone)
            }
        },
        startTriggeredReviewWatch: () => {
        ...
        togglePerspective: async ({ skillName, enabled }) => {
            ...
            if (!enabled && enabledCount <= 1) {
                toast.info('Keep at least one perspective enabled')
                return
            }
            actions.patchPerspectiveLocally(skillName, enabled)
            actions.setSkillSaving(skillName, true)
            try {
                await reviewPerspectivesPartialUpdate(currentProjectId(), skillName, { enabled })
            } catch (error: any) {
        ...
        selectBlindSpots: async ({ skillName }) => {
            actions.setSkillSaving(skillName, true)
            try {
                await reviewBlindSpotsPartialUpdate(currentProjectId(), skillName, { active: true })
            } catch (error: any) {
                toast.error(
        ...
        selectValidator: async ({ skillName }) => {
            actions.setSkillSaving(skillName, true)
            try {
                await reviewValidatorsPartialUpdate(currentProjectId(), skillName, { active: true })
            } catch (error: any) {
                toast.error(
        ...
        blockSingleActiveDeactivation: ({ kindLabel }) => {
            toast.info(`One ${kindLabel} always runs — switch by selecting another one`)
        },
        ...
        loadReviewDetailFailure: () => {
            ...
            if (values.reviewDrawerOpen && !values.openedReview) {
                toast.error("That review couldn't be opened. The link may be stale, or the review deleted.")
        ...
        submitTriggerReview: async () => {
            ...
            try {
                const response = await reviewReviewsTriggerCreate(currentProjectId(), { pr_url: prUrl })
                ...
                if (response.status === 'already_reviewed') {
                    ...
                    toast.success('Review started. It will appear under recent reviews as it runs.')
                ...
                toast.error(error?.data?.error || error?.detail || error?.message || 'Failed to start the review')
            } finally {
        ...
        startSkillAuthorTask: async ({ kind }) => {
            ...
            try {
                ...
                toast.error(error?.detail || error?.message || `Failed to start "${title}"`)
            } finally {
        ...
        return {
            setReviewsScope: (): [string, Record<string, any>, Record<string, any>, { replace: boolean }] => [
                router.values.location.pathname,
                {
                    ...
                    reviews_scope:
                        values.reviewsScope === ReviewReviewsListScope.Mine ? undefined : values.reviewsScope,
                },
                router.values.hashParams,
                { replace: true },
            ],
    ...
    urlToAction(({ actions, values }) => ({
        [urls.codeReview()]: (_, searchParams) => {
            ...
            if (
                (parsed === ReviewReviewsListScope.Mine || parsed === ReviewReviewsListScope.Everyone) &&
