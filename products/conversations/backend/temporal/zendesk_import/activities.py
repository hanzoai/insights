...
# Django/InsightsQL/model imports are non-deterministic and trip sandbox restrictions (e.g. the
...
def _import_ticket_batch_sync(input: ImportBatchInput) -> ImportBatchOutput:
    ...
    for idx, (zendesk_ticket, comments) in enumerate(ticket_data):
        ...
        try:
            ...
            # "Anonymous user" when no Insights person matched — same shape as the other import
            ...
            # Zendesk tags are plain strings on the ticket payload; Insights tags are per-team
