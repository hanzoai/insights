...
export const rustVmExecution = new Counter({
    name: 'scriptvm_rust_execution_total',
    help: 'Outcomes of transformation executions where the Rust ScriptVM is the primary executor',
    labelNames: ['outcome'],
})

export const rustVmExecutionDuration = new Histogram({
    name: 'scriptvm_rust_execution_duration_ms',
    help: 'Per-invocation script execution duration on the Rust ScriptVM as the primary executor',
...
export class RustVmExecutor {
    ...
    private fallback(
        ...
        logger.warn('🦀', 'Rust ScriptVM invocation fell back to the node vm', {
            outcome,
            functionId: invocation.functionId,
            teamId: invocation.teamId,
            // Same redaction as script print logs: marshalling errors and panic messages can embed
    ...
    public execute(
        invocation: CyclotronJobInvocationInsightsFunction,
        sensitiveValues: string[]
    ): CyclotronJobInvocationResult<CyclotronJobInvocationInsightsFunction> | null {
        ...
        let rust
        try {
            rust = module_.executeSync(invocation.insightsFunction.bytecode, invocation.state.globals, {
        ...
        result.invocation.state.timings.push({ kind: 'script', duration_ms: durationMs })
