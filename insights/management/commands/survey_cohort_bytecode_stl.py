"""Survey ScriptVM bytecode (STL functions + opcodes) across realtime cohorts, flagging Rust ScriptVM gaps."""
...
# Snapshot of rust/common/scriptvm/src/stl.rs (regenerate via `bin/dump_scriptvmrs_stl`); a snapshot avoids a cross-product import.
...
RUST_FN_STL: frozenset[str] = frozenset(
    {"arrayCount", "arrayExists", "arrayFilter", "arrayMap", "arrayReduce", "sortableSemver"}
)
RUST_SUPPORTED_STL: frozenset[str] = RUST_NATIVE_STL | RUST_FN_STL

# `sortableSemver`'s script body calls these unregistered natives, transitively blocking any SEMVER_* cohort.
...
def iter_instructions(bytecode: list[Any]) -> Iterator[tuple[Operation, list[Any]]]:
    """Disassemble a ScriptVM bytecode program, yielding (opcode, operands) per instruction.
    ...
    if n > 0 and bytecode[0] == INSIGHTSQL_BYTECODE_IDENTIFIER:
        ip = 2
    elif n > 0 and bytecode[0] == INSIGHTSQL_BYTECODE_IDENTIFIER_V0:
        ip = 1
...
class Command(BaseCommand):
    help = "Survey ScriptVM bytecode (STL functions + opcodes) across production realtime cohorts (M0)"
