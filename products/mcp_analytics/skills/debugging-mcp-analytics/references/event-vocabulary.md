...
Insights-side descriptions, including everything the SDK does not define, live in
`insights/taxonomy/taxonomy.py`. **When this file and those disagree, they win** — grep them.

Properties fall into three groups, and the distinction matters: SDK-emitted properties exist
for any instrumented customer server, server-stamped ones exist only for Insights's own
...
Added by Insights's own server (`services/mcp/src/hono/analytics.ts::buildBaseProperties`),
**not** by the SDK. They exist for Insights's dogfood data and will be absent for a customer
...
`insights/taxonomy/taxonomy.py` and coalesced by the read path, but **no producer on master emits
...
- **`$session_id`** — the standard Insights session id, a materialized column. The default
...
Since TS 0.7.0 events stamp the standard `$lib` = `insights-node-mcp` (plus `$lib_version`) via
...
Note that insights-node sets `$lib` at the client level, so `instrument()` relabels **every**
event sent by the client you pass it. Give an MCP server a insights-node client dedicated to its
...
- **0.5.0** — `instrumentMutator(insights)`, a point-free `(server) => server` helper for
...
`insights/mcp/` in `Insights/insights-python`, mirroring the `insights.ai` layout. `instrument()`
...
library rejects unexpected kwargs). `InsightsMCP` covers custom dispatchers.
...
- Scope to `$mcp_source = 'insights_mcp_analytics'` to isolate MCP events.
