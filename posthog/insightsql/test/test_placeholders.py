...
class TestBytecodePlaceholders(BaseTest):
    ...
    def test_sql_expression_placeholder(self):
        ...
        self.assertEqual(to_printed_insightsql(expr, team=self.team), "plus(1, 2)")

    def test_immediate_function_placeholder_with_insightsqlx(self):
        """
        The complex lambda / JSX example supplied in the prompt should parse
        and substitute to:
          * select[0]  -> InsightsQLXTag(kind="strong", …)
          * select[1]  -> Constant(2)
          * where      -> Constant(True)
        """
        ...
        parsed = cast(ast.SelectQuery, parse_select(insightsql))
        ...
        self.assertIsInstance(first, ast.InsightsQLXTag)
        ...
        self.assertIsInstance(elements[2], ast.InsightsQLXTag)
        self.assertEqual(elements[2].kind, "blink")
        self.assertEqual(elements[2].attributes, [ast.InsightsQLXAttribute(name="children", value=["fun"])])
