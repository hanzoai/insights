...
-- Full CREATE schema for the local-multi/logs node. Apply to a fresh Datastore to build it.
...
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'warpstream_logs', kafka_format = 'kafka_format = \'Avro\'', kafka_group_name = 'kafka_group_name = \'datastore-logs-avro-new\'', kafka_num_consumers = 8, kafka_poll_max_batch_size = 1000, kafka_poll_timeout_ms = 3000, kafka_skip_broken_messages = 100, kafka_thread_per_consumer = 1, kafka_topic_list = 'kafka_topic_list = \'datastore_logs\'';
...
CREATE TABLE insights.log_attributes (
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/noshard/insights.log_attributes', '{replica}-{shard}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(original_expiry_time_bucket) SETTINGS deduplicate_merge_projection_mode = 'drop', index_granularity = 8192, storage_policy = 'default';
CREATE TABLE insights.log_attributes2 (
  ...
  INDEX idx_attribute_key_n3 attribute_key TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 1,
  INDEX idx_attribute_value_n3 attribute_value TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 1
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/noshard/insights.log_attributes2', '{replica}-{shard}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(original_expiry_time_bucket) TTL original_expiry_time_bucket SETTINGS deduplicate_merge_projection_mode = 'drop', index_granularity = 8192;
...
CREATE TABLE insights.log_attributes_distributed (
  ...
  original_expiry_time_bucket DateTime DEFAULT now(),
  severity_text LowCardinality(String)
) ENGINE = Distributed('insights_single_shard', 'insights', 'log_attributes3');
CREATE TABLE insights.logs32 (
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/noshard/insights.logs_billing_metrics', '{replica}-{shard}') ORDER BY (team_id, time_bucket, service_name) PARTITION BY toYYYYMM(time_bucket) SETTINGS deduplicate_merge_projection_mode = 'rebuild', index_granularity = 8192;
CREATE TABLE insights.logs_billing_metrics_distributed (
  ...
  bytes_compressed SimpleAggregateFunction(sum, UInt64),
  record_count SimpleAggregateFunction(sum, UInt64)
) ENGINE = Distributed('insights_single_shard', 'insights', 'logs_billing_metrics');
CREATE TABLE insights.logs_distributed (
  ...
  _bytes_compressed UInt64,
  _record_count UInt64
) ENGINE = Distributed('insights_single_shard', 'insights', 'logs34');
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/noshard/insights.logs_kafka_metrics', '{replica}-{shard}') ORDER BY (_topic, _partition) SETTINGS index_granularity = 8192;
CREATE TABLE insights.logs_kafka_metrics_distributed (
  ...
  max_created_at SimpleAggregateFunction(max, DateTime64(9)),
  max_lag SimpleAggregateFunction(max, UInt64)
) ENGINE = Distributed('insights_single_shard', 'insights', 'logs_kafka_metrics');
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.trace_attributes', '{replica}-{shard}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(original_expiry_time_bucket) TTL original_expiry_time_bucket SETTINGS index_granularity = 8192;
CREATE TABLE insights.trace_attributes2 (
  team_id Int32,
  original_expiry_time_bucket DateTime64(0),
  time_bucket DateTime64(0),
  service_name LowCardinality(String),
  ...
  INDEX idx_attribute_key_n3 attribute_key TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 4,
  INDEX idx_attribute_value_n3 attribute_value TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 4
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/noshard/insights.trace_attributes2', '{replica}-{shard}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(original_expiry_time_bucket) TTL original_expiry_time_bucket SETTINGS index_granularity = 8192;
CREATE TABLE insights.trace_attributes_distributed (
  ...
  attribute_type LowCardinality(String),
  attribute_count SimpleAggregateFunction(sum, UInt64)
) ENGINE = Distributed('insights_single_shard', 'insights', 'trace_attributes');
...
CREATE TABLE insights.trace_spans_distributed (
  ...
  _bytes_compressed UInt64,
  _record_count UInt64
) ENGINE = Distributed('insights_single_shard', 'insights', 'trace_spans');
CREATE TABLE insights.trace_spans_kafka_metrics (
  ...
  max_created_at SimpleAggregateFunction(max, DateTime64(9)),
  max_lag SimpleAggregateFunction(max, UInt64)
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.trace_spans_kafka_metrics', '{replica}-{shard}') ORDER BY (_topic, _partition) SETTINGS index_granularity = 8192;
...
FROM
  (
    ...
    FROM insights.logs32
    GROUP BY
      team_id, time_bucket, original_expiry_time_bucket, service_name, resource_fingerprint, attributes
  );
CREATE MATERIALIZED VIEW insights.logs32_to_resource_attributes TO insights.log_attributes (team_id Int32, time_bucket DateTime64(0), original_expiry_time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  time_bucket,
  original_expiry_time_bucket,
  service_name,
...
CREATE MATERIALIZED VIEW insights.trace_span_to_attributes2 TO insights.trace_attributes2 (team_id Int32, original_expiry_time_bucket DateTime64(0), time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  original_expiry_time_bucket,
  time_bucket,
  service_name,
...
CREATE MATERIALIZED VIEW insights.trace_span_to_resource_attributes2 TO insights.trace_attributes2 (team_id Int32, original_expiry_time_bucket DateTime64(0), time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  original_expiry_time_bucket,
  time_bucket,
  service_name,
...
CREATE MATERIALIZED VIEW insights.trace_span_to_span_attributes2 TO insights.trace_attributes2 (team_id Int32, original_expiry_time_bucket DateTime64(0), time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  original_expiry_time_bucket,
  time_bucket,
  service_name,
...
CREATE TABLE insights.logs (
  ...
  _bytes_compressed UInt64 CODEC(DoubleDelta, ZSTD(1)),
  _record_count UInt64 CODEC(DoubleDelta, ZSTD(1))
) ENGINE = Distributed('insights_single_shard', 'insights', 'logs32');
CREATE TABLE insights.metric_samples (
  ...
  span_id String,
  trace_flags Int32
) ENGINE = Distributed('insights_single_shard', 'insights', 'metric_samples1');
CREATE TABLE insights.metric_series (
  ...
  attributes Map(LowCardinality(String), String),
  last_seen DateTime64(6) CODEC(DoubleDelta)
) ENGINE = Distributed('insights_single_shard', 'insights', 'metric_series1');
