...
import { InsightsFlowPropertyFilters } from './InsightsFlowFilters'
...
describe('InsightsFlowPropertyFilters search', () => {
    ...
    function renderFilters(): { setFilters: jest.Mock } {
        ...
        const filters = { properties: [] } as InsightsFlowAction['filters']
        render(
            <Provider>
                <InsightsFlowPropertyFilters filtersKey="search-test" filters={filters} setFilters={setFilters} />
            </Provider>
        )
        return { setFilters }
    }
    ...
    async function openTaxonomicFilter(): Promise<void> {
        fireEvent.click(screen.getByTestId('new-prop-filter-InsightsFlowPropertyFilters.search-test'))
        await waitFor(() => {
