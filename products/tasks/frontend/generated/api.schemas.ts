...
export interface LoopRepositoryEntryApi {
    ...
    github_integration_id: number
    /**
     * Repository in `organization/repo` format, e.g. `insights/insights`.
     * @maxLength 255
     */
    full_name: string
}
 ...
 * * `full` - full
 */
export type InsightsMcpScopesEnumApi = (typeof InsightsMcpScopesEnumApi)[keyof typeof InsightsMcpScopesEnumApi]

export const InsightsMcpScopesEnumApi = {
    ReadOnly: 'read_only',
    Full: 'full',
} as const
...
export interface LoopConnectorsApi {
    ...
    mcp_installation_ids?: string[]
    /** Scope of the Insights MCP access injected into this loop's runs.
     *
     * * `read_only` - read_only
     * * `full` - full */
    insights_mcp_scopes?: InsightsMcpScopesEnumApi
}
...
export interface LoopWriteApi {
    ...
    /** LLM model identifier, validated against `runtime_adapter`'s catalog. Leave blank to let Insights pick a sensible default at run time. */
    model?: string
 ...
 * @nullable
 */
export type TaskUserBasicInfoApiMascotConfig = { [key: string]: unknown } | null
...
export interface TaskUserBasicInfoApi {
    ...
    /** @nullable */
    mascot_config?: TaskUserBasicInfoApiMascotConfig
    /** @nullable */
...
export interface TaskCreateApi {
    ...
    description?: string
    /** Insights product or surface that created this task (e.g. error_tracking, slack, user_created). Origins reserved for server-created agents cannot be set through this API.
     *
     * * `onboarding` - Onboarding
     * * `error_tracking` - Error Tracking
     * * `eval_clusters` - Eval Clusters
    ...
    /**
     * Target GitHub repository in `organization/repo` format (e.g. `insights/insights-js`).
     * @maxLength 255
     * @nullable
     */
    repository?: string | null
...
export interface ClaudeTaskRunCreateSchemaApi {
    /**
     * Local url-based MCP servers from the creating client (Insights Desktop) to make available inside the cloud sandbox. Header values are treated as credentials: stored encrypted and never returned by the API.
     * @nullable
     */
    imported_mcp_servers?: ImportedMcpServerApi[] | null
    /**
     * Names of desktop-only MCP servers the creating client (Insights Desktop) relays into the cloud sandbox over the durable event/command channel. Names only — the server configuration (command, env, URL, headers) never crosses the wire.
     * @nullable
     */
    ...
    /** Optional GitHub user token from Insights Desktop for user-authored cloud pull requests. Prefer linking GitHub from Settings → Linked accounts so the server can manage tokens; this field remains supported for callers that still manage their own tokens. */
...
export interface TaskRunBootstrapCreateRequestApi {
     ...
     */
    fast_mode?: boolean | null
    /** Ephemeral GitHub user token from Insights Desktop for user-authored cloud pull requests. */
    github_user_token?: string
    /** Initial permission mode for the agent session. Claude runtimes accept Insights permission presets like 'plan'. Codex runtimes accept native Codex modes like 'plan', 'auto', and 'read-only'.
     *
     * * `default` - default
     * * `acceptEdits` - acceptEdits
     * * `plan` - plan
...
/**
 * Insight query JSON to render ad hoc, e.g. {"kind": "InsightVizNode", "source": {"kind": "TrendsQuery", ...}}. SQL queries (DataVisualizationNode, InsightsQLQuery) are not supported yet. Provide exactly one of query or insight_id.
 */
...
export interface TaskRunLivingArtifactChartResponseApi {
    ...
    /**
     * Link to explore this chart interactively in Insights.
     * @nullable
     */
    url?: string | null
}
...
/**
 * The Insights Task linked to the Slack thread.
 */
export interface SlackThreadContextTaskApi {
    ...
    created_at: string
    /** Absolute URL to the task detail page in the Insights app. */
    url: string
}
...
export interface SlackThreadContextRunApi {
    ...
    task_processing_workflow_url: string | null
    /**
     * Temporal workflow id of the Slack mention that dispatched this run (`insights-code-mention-<workspace>:<event_id_or_channel:ts>`). Null for runs created before this field was persisted.
     * @nullable
     */
...
export interface SlackThreadContextResponseApi {
    ...
    /** Linked Insights Task. Null when no mapping was found for the thread. */
...
export interface WarmTaskRequestApi {
    /**
     * Target GitHub repository to clone, in `organization/repo` format (e.g. `insights/insights`).
     * @maxLength 255
     */
    repository: string
...
export type TasksSlackThreadContextRetrieveParams = {
    /**
     * Full Slack permalink to any message in the thread (e.g. https://insights.slack.com/archives/C…/p1779956938619299). Replies inside the thread are accepted too — the `thread_ts` query param (when present) takes precedence over the in-path message ts.
     * @minLength 1
     */
    url: string
}
