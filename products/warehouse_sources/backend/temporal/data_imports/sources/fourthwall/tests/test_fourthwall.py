...
class TestWebhookManagement:
    ...
    def test_sync_webhook_events_merges_instead_of_replacing(self, MockSession):
        ...
        session.get.return_value = self._webhook_page(
            [{"id": "wcon_1", "url": "https://us.hanzo.ai/w/1", "allowedTypes": ["THANK_YOU_SENT"]}]
        )

        result = sync_webhook_events("api-user", "api-secret", "v1.0", "https://us.hanzo.ai/w/1", ["ORDER_PLACED"])
    ...
    def test_sync_webhook_events_skips_an_already_correct_webhook(self, MockSession):
        session = self._session(MockSession)
        session.get.return_value = self._webhook_page(
            [{"id": "wcon_1", "url": "https://us.hanzo.ai/w/1", "allowedTypes": ["ORDER_PLACED"]}]
        )

        sync_webhook_events("api-user", "api-secret", "v1.0", "https://us.hanzo.ai/w/1", ["ORDER_PLACED"])
    ...
    def test_webhook_lookup_walks_every_page(self, MockSession):
        ...
        session.get.side_effect = [
            ...
            self._webhook_page(
                [{"id": "wcon_2", "url": "https://us.hanzo.ai/w/1", "allowedTypes": []}], total_pages=2
            ),
        ]
        session.delete.return_value = mock.MagicMock(status_code=200)

        result = delete_webhook("api-user", "api-secret", "v1.0", "https://us.hanzo.ai/w/1")
    ...
    def test_delete_webhook_reports_a_failed_deletion(self, MockSession):
        session = self._session(MockSession)
        session.get.return_value = self._webhook_page(
            [{"id": "wcon_1", "url": "https://us.hanzo.ai/w/1", "allowedTypes": []}]
        )
    ...
    def test_external_webhook_info_matches_on_url(self, MockSession):
        ...
        session.get.return_value = self._webhook_page(
            [
                ...
                {"id": "wcon_2", "url": "https://us.hanzo.ai/w/1", "allowedTypes": ["ORDER_PLACED"]},
            ]
        )

        info = get_external_webhook_info("api-user", "api-secret", "v1.0", "https://us.hanzo.ai/w/1")
    ...
    def test_external_webhook_info_when_absent(self, MockSession):
        ...
        assert get_external_webhook_info("api-user", "api-secret", "v1.0", "https://us.hanzo.ai/w/1").exists is False
