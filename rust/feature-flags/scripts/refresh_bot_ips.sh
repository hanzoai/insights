...
#
# USAGE
#   From the insights repo (or anywhere — the script resolves its own path):
...
if [[ ! -d "$JSON_DIR" ]]; then
    echo -e "${RED}error:${RESET} expected $JSON_DIR to exist; run from a checkout of insights" >&2
    exit 1
fi
