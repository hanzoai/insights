...
def _is_flag_enabled(flag_key: str, user: User, team: "Team | None" = None) -> bool:
    ...
    return bool(hanzo_insights.feature_enabled(flag_key, user.distinct_id, **kwargs))


def has_tasks_access(user: User) -> bool:
    """
    User has access to Insights Desktop if the `tasks` feature flag is enabled for them
