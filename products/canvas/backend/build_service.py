...
CANVAS_BUILD_OUTCOMES = Counter(
    "insights_canvas_build_outcomes_total", "Canvas build terminal outcomes", ["outcome", "code"]
)
CANVAS_BUILD_QUEUE_SECONDS = Histogram(
    "insights_canvas_build_queue_seconds", "Time a canvas build waits before execution"
)
CANVAS_BUILD_DURATION_SECONDS = Histogram(
    "insights_canvas_build_duration_seconds", "End-to-end canvas build latency", ["outcome"]
)
CANVAS_BUILD_ARTIFACT_BYTES = Histogram(
    "insights_canvas_build_artifact_bytes", "Total emitted bytes for successful canvas builds"
)
CANVAS_BUILD_SWEEP_OUTCOMES = Counter(
    "insights_canvas_build_sweep_total", "Stuck canvas builds handled by the sweeper", ["outcome"]
)
CANVAS_BUILD_ACTIVE = Gauge("insights_canvas_builds_active", "Canvas builds currently queued or building")
