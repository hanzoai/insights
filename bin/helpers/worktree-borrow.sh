...
# insightscli / ruff / ty without its own node_modules or venv -- by borrowing the
...
#
# POSIX sh; sourced by the husky hooks and by bin/insightscli (bash). Sourcing this
...
insights_worktree_borrow() {
    _ph_scope="${1:-all}"
    _ph_main="$(insights_main_repo "$(pwd)")"
    [ -n "$_ph_main" ] || return 0
    ...
    if [ "$_ph_scope" = "all" ] && [ -z "$(insights_find_venv "$(pwd)")" ]; then
        _ph_main_venv="$(insights_find_venv "$_ph_main")"
