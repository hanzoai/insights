"""Dagster logger that routes op/asset logs through Insights's structlog pipeline.

Dagster's default `colored_console_logger` writes plain colored text to the
process stdout. The Insights Logs product is fed by the OTel collector scraping
each container's stdout and parsing structlog's JSON lines (see
`insights/settings/logs.py`) — plain text isn't parseable, so Dagster job logs
...
logger that `django.setup()` (run in `insights/dags/__init__.py`) configured
...
@dagster.logger(
    ...
    description=(
        "Routes Dagster logs through Insights's structlog pipeline (JSON to stdout, like Django) "
...
def structlog_console_logger(init_context: dagster.InitLoggerContext) -> logging.Logger:
    ...
    # `insights/settings/logs.py` sets `disable_existing_loggers=True`, which can
