...
from posthog.cdp.templates import INSIGHTS_FUNCTION_TEMPLATES
from posthog.management.commands.sync_insights_function_templates import (
...
class TestSyncInsightsFunctionTemplates:
    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_sync_python_templates(self, mock_get_insights_function_templates):
        """Test that Python templates are synced to the database."""
        # Mock the Node.js API to avoid external dependencies
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = []
        mock_get_insights_function_templates.return_value = mock_response

        # Run the command
        call_command("sync_insights_function_templates")

        # Verify that the Python test template was created successfully
        db_templates = InsightsFunctionTemplate.objects.filter(template_id__in=TEST_INCLUDE_PYTHON_TEMPLATE_IDS)
    ...
    @patch("posthog.management.commands.sync_insights_function_templates.get_insights_function_templates")
    def test_sync_nodejs_templates(self, mock_get_insights_function_templates):
        ...
        for template_id in TEST_INCLUDE_NODEJS_TEMPLATE_IDS:
            node_template = InsightsFunctionTemplate.objects.filter(template_id=template_id).first()
            assert node_template is not None
            assert node_template.type == "transformation"
            assert node_template.name == f"Test {template_id}"

    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_sync_handles_invalid_template(self, mock_get_insights_function_templates):
        ...
        db_template_ids = set(InsightsFunctionTemplate.objects.values_list("template_id", flat=True))
    ...
    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_sync_handles_api_error(self, mock_get_insights_function_templates):
        """Test that the command handles API errors gracefully."""
        # Mock an API error
        mock_get_insights_function_templates.side_effect = Exception("API Error")
    ...
    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_sync_metrics(self, mock_get_insights_function_templates):
        ...
        stdout = StringIO()
        call_command("sync_insights_function_templates", stdout=stdout)
    ...
    def test_template_contents(self, mock_get_insights_function_templates):
        ...
        db_template = InsightsFunctionTemplate.objects.get(template_id=slack_template.id)
    ...
    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_template_version_behavior(self, mock_get_insights_function_templates):
    ...
    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_delete_deleted_coming_soon_templates(self, mock_get_insights_function_templates):
        ...
        initial_count = InsightsFunctionTemplate.objects.count()
        ...
        assert InsightsFunctionTemplate.objects.filter(template_id="coming-soon-old-template").exists()

        # Run sync again - should detect and delete the old template
        call_command("sync_insights_function_templates")

        # Verify the old template was deleted
        assert not InsightsFunctionTemplate.objects.filter(template_id="coming-soon-old-template").exists()

    @patch("posthog.plugins.plugin_server_api.get_insights_function_templates")
    def test_does_not_delete_deleted_templates(self, mock_get_insights_function_templates):
        ...
        # Verify the old template was created
        assert InsightsFunctionTemplate.objects.filter(template_id="old-template").exists()
