...
def derive_slack_app_mention_workflow_id(inputs: InsightsCodeSlackMentionWorkflowInputs) -> str | None:
...
class SlackAppMentionWorkflow(InsightsWorkflow):
    """Per-conversation queue over the per-message mention workflow.

    Standalone ``InsightsCodeSlackMentionWorkflow`` executions race when
    ...
    ``InsightsCodeSlackMentionWorkflow`` — under the same per-message workflow
    ...
    def __init__(self, inputs: SlackAppMentionWorkflowInputs) -> None:
        ...
        self._queue: list[InsightsCodeSlackMentionWorkflowInputs] = []
    ...
    @workflow.signal
    async def new_message(self, message: InsightsCodeSlackMentionWorkflowInputs) -> None:
    ...
    def parse_inputs(inputs: list[str]) -> SlackAppMentionWorkflowInputs:
        ...
        loaded["pending_messages"] = [
            InsightsCodeSlackMentionWorkflowInputs(**message) for message in loaded.get("pending_messages", [])
        ]
        return SlackAppMentionWorkflowInputs(**loaded)

    async def _react(self, message: InsightsCodeSlackMentionWorkflowInputs, reaction_activity: Any) -> None:
    ...
    async def _notify_child_failed(self, message: InsightsCodeSlackMentionWorkflowInputs) -> None:
    ...
    async def _process(self, message: InsightsCodeSlackMentionWorkflowInputs) -> None:
        ...
        try:
            ...
            await workflow.execute_child_workflow(
                InsightsCodeSlackMentionWorkflow.run,
                message,
