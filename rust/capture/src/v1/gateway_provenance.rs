...
pub fn parse_signature(headers: &HeaderMap) -> Option<GatewaySignature> {
    Some(GatewaySignature {
        signature: shared::header_str(headers, POSTFN_AI_GATEWAY_SIGNATURE)?,
        signed_at: shared::header_str(headers, POSTFN_AI_GATEWAY_SIGNED_AT)?,
        request_id: shared::header_str(headers, POSTFN_AI_GATEWAY_REQUEST_ID).unwrap_or_default(),
    })
}
