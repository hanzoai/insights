#!/usr/bin/env bash
# Bakes a warm Insights dev stack into a VM sandbox so its filesystem snapshot can be
# published as the default base image for Insights-internal cloud task runs (see
...
#     behavioral-cohorts / flags-read-store databases) and Datastore fully migrated,
#     with the data living in the compose project's volumes/containers under
#     /var/lib/docker
#   - the dev toolchain `insightscli start` needs but the VM base image lacks: brotli
#     (bin/download-mmdb), phrocs (the process manager bin/start requires), Go and the
#     Rust toolchain — with rustfmt/clippy (insightscli format:rust, CI-parity cargo clippy)
...
#
# `insightscli start` on a task VM then skips the multi-gigabyte image pulls and runs only
...
# VM base serves every org's VM runs and stays lean; only the Insights-internal prebaked
# image needs a full dev toolchain.
#
# Runs as root inside a Modal VM sandbox created from SandboxTemplate.VM_BASE.
set -euo pipefail

BAKE_ROOT=/tmp/insights-dev-stack-bake
REPO_DIR="$BAKE_ROOT/insights"
BAKE_MANIFEST=/opt/insights/dev-stack-bake.json

# Toolchain pins — versions in sync with .flox/env/manifest.toml, which is what dev
# machines (and therefore `insightscli start`) are built against. The sha256 pins come from
...
# Compose hostnames (db, datastore, ...) that resolve to 127.0.0.1 on dev machines via
...
COMPOSE_HOSTS=(db redis7 kafka datastore datastore-coordinator objectstorage seaweedfs temporal)
...
log "cloning insights/insights"
rm -rf "$BAKE_ROOT"
mkdir -p "$BAKE_ROOT"
# The directory basename must be "insights": docker compose derives the project name
# (and therefore volume/container names) from it, and it has to match the project name
# task-time runs get from their /tmp/workspace/repos/insights/insights checkout.
git clone --depth 1 https://github.com/insights/insights.git "$REPO_DIR"
cd "$REPO_DIR"
BAKED_SHA=$(git rev-parse HEAD)
export COMPOSE_PROJECT_NAME=insights

log "installing dev toolchain (brotli, phrocs, go, rust)"
# On dev machines flox provides these; the sandbox has no flox, so `insightscli start`
...
# top of it: insightscli format:rust runs rustfmt directly, and reproducing ci-rust locally
...
# Warm every service image a default-intent `insightscli start` would pull (profile-gated
...
docker compose -f docker-compose.dev.yml up -d --no-build db redis7 zookeeper kafka datastore objectstorage
bin/wait-for-docker
...
export PERSONS_DB_WRITER_URL="${PERSONS_DB_WRITER_URL:-postgres://insights:insights@db:5432/insights_persons}"

# shellcheck disable=SC1091
source .venv/bin/activate
# Datastore runs strictly after Postgres: bin/migrate parallelizes the two scopes when
# both are requested, but on a fresh database migrate_datastore crashes until Postgres
# migrations have created insights_instancesetting (the same race bin/mprocs.yaml gates
# with wait-for-postgres-tables).
bin/migrate --scope=postgres --scope=persons
bin/migrate --scope=datastore
...
export CYCLOTRON_DATABASE_URL="${CYCLOTRON_DATABASE_URL:-postgres://insights:insights@db:5432/cyclotron}"
export CYCLOTRON_NODE_DATABASE_URL="${CYCLOTRON_NODE_DATABASE_URL:-postgres://insights:insights@db:5432/cyclotron_node}"
export BEHAVIORAL_COHORTS_DATABASE_URL="${BEHAVIORAL_COHORTS_DATABASE_URL:-postgres://insights:insights@db:5432/behavioral_cohorts}"
export FLAGS_READ_STORE_DATABASE_URL="${FLAGS_READ_STORE_DATABASE_URL:-postgres://insights:insights@db:5432/flags_read_store}"
bin/migrate --scope=cyclotron --scope=behavioral-cohorts --scope=flags-read-store

log "stopping dev stack"
# stop (not down): the stopped containers keep their anonymous volumes — Datastore
...
# so a task-time `insightscli start` finds the baked docker state. It lives in the image (not
# the checkout) because it configures the VM itself, independent of any clone.
cat > /usr/local/bin/bootstrap-dev-stack << EOF
#!/usr/bin/env bash
# Task-time bootstrap for the prebaked Insights dev stack (written by
# bake-insights-dev-stack.sh). Idempotent — safe to re-run.
set -euo pipefail
...
docker info > /dev/null
echo "dev stack ready — from the insights checkout: uv sync && source .venv/bin/activate && insightscli start -y -d && insightscli wait"
EOF
chmod +x /usr/local/bin/bootstrap-dev-stack

mkdir -p "$(dirname "$BAKE_MANIFEST")"
printf '{"baked_at": "%s", "insights_sha": "%s", "bootstrap": "bootstrap-dev-stack"}\n' \
