...
class PulseModel(TeamScopedRootMixin, CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    # FKs to the hot insights_team / insights_user tables use db_constraint=False so creating the
...
class ResourceLink(PulseModel):
    """Evidence link from an opportunity to a Insights resource it cites.
