...
class TestRouteThreadMessage(TestCase):
    ...
    def setUp(self):
        ...
        # him as a real Insights user with team access so the user-resolution
    ...
    def _route(self, event: dict) -> str:
        ...
        return route_insights_code_event_to_relevant_region(request, event, "T_SLACK")
    ...
    def test_rules_command_text_does_not_trigger_command_workflow(self):
        ...
        rules_text = '@Insights rules add "use the helper" org/repo'
        with (
    ...
    def test_tagged_message_starts_workflow_with_resolved_user_and_untagged_flag(self):
        ...
        resolved Insights user. The classifier and thread-history fetch happen
        inside the workflow."""
        from products.slack_app.backend.api import ROUTE_HANDLED_LOCALLY
        ...
        assert kwargs["insights_user"].id == self.user.id
    ...
    def test_app_mention_and_tagged_message_both_reach_mention_workflow(self):
        ...
        assert first.kwargs["insights_user"].id == self.user.id
        assert second.kwargs["untagged_followup"] is True
        assert second.kwargs["insights_user"].id == self.user.id
