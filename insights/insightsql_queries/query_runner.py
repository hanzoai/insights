...
from insights.insightsql.modifiers import create_default_modifiers_for_user
...
from insights.insightsql.warehouse_warnings import accumulator_scope
...
from insights.insightsql_queries.access_controlled_resources import queried_access_controlled_resources
from insights.insightsql_queries.insights.utils.breakdowns import has_multi_breakdown, has_single_breakdown
from insights.insightsql_queries.insights.utils.entities import has_data_warehouse_node
from insights.insightsql_queries.insights.utils.properties import has_any_property_filters
from insights.insightsql_queries.query_failure_handling import (
    budget_for_limit_context,
    build_failure_exception,
    classify_failure,
)
from insights.insightsql_queries.query_metadata import extract_query_metadata
from insights.insightsql_queries.utils.event_usage import log_event_usage_from_query_metadata
from insights.insightsql_queries.validation.validation import (
    QueryValidationContext,
    QueryValidationRule,
    run_validation_rules,
)
from insights.models import Team, User
from insights.models.team import WeekStartDay
from insights.models.team.event_retention import events_retention_months_for_team
from insights.query_cache import QueryCache, count_query_cache_hit
from insights.query_cache.failures import (
    BUDGET_EXTENDED,
...
from insights.rbac.user_access_control import WAREHOUSE_ACCESS_SCOPES, UserAccessControl, UserAccessControlError
from insights.schema_helpers import to_dict
from insights.scopes import APIScopeObject
from insights.shared_link_user import SharedLinkUser
from insights.slo.context import JsonValue, SloSpec, slo_operation
from insights.slo.types import SloArea, SloOperation, SloOutcome
...
QUERY_EXECUTION_TOTAL = Counter(
    ...
    labelnames=["query_type", "category", "error_type", "contains_user_insightsql"],
)

QUERY_EXECUTION_DURATION = Histogram(
    "insights_query_execution_duration_seconds",
...
def _contains_user_insightsql_label() -> str:
    # Read the tag set by `tag_contains_user_insightsql()` at InsightsQL parse sites; lets
    # observability split user-InsightsQL failures from query-builder failures on the
    # same metric. The tag is the canonical source — see `insights.datastore.query_tagging`.
    return "true" if get_query_tag_value("contains_user_insightsql") else "false"
...
def get_query_runner(
    ...
    if kind == "PathsQuery":
        from products.product_analytics.backend.insightsql_queries.paths.paths_query_runner import PathsQueryRunner
    ...
    if kind == "StickinessQuery":
        from products.product_analytics.backend.insightsql_queries.stickiness.stickiness_query_runner import (
    ...
    if kind == "InsightsQLQuery":
        from .insightsql_query_runner import InsightsQLQueryRunner

        return InsightsQLQueryRunner(
            query=cast(InsightsQLQuery | dict[str, Any], query),
            team=team,
            timings=timings,
            limit_context=limit_context,
            modifiers=modifiers,
    ...
    if kind == "WebStatsTableQuery":
        from products.web_analytics.backend.insightsql_queries.stats_table import WebStatsTableQueryRunner
    ...
    if kind == "WebGoalsQuery":
        from products.web_analytics.backend.insightsql_queries.web_goals import WebGoalsQueryRunner
    ...
    if kind == "WebNotableChangesQuery":
        from products.web_analytics.backend.insightsql_queries.notable_changes import WebNotableChangesQueryRunner
    ...
    if kind == "WebExternalClicksTableQuery":
        from products.web_analytics.backend.insightsql_queries.external_clicks import WebExternalClicksTableQueryRunner
    ...
    if kind == "WebBotsTableQuery":
        from products.web_analytics.backend.insightsql_queries.web_bots import WebBotsTableQueryRunner
    ...
    if kind == "WebVitalsPathBreakdownQuery":
        from products.web_analytics.backend.insightsql_queries.web_vitals_path_breakdown import (
    ...
    if kind == "SessionAttributionExplorerQuery":
        from products.web_analytics.backend.insightsql_queries.session_attribution_explorer_query_runner import (
    ...
    if kind == "ExperimentFunnelsQuery":
        from products.experiments.backend.insightsql_queries.experiment_funnels_query_runner import (
    ...
    if kind == "ExperimentTrendsQuery":
        from products.experiments.backend.insightsql_queries.experiment_trends_query_runner import (
    ...
    if kind == "ExperimentExposureQuery":
        from products.experiments.backend.insightsql_queries.experiment_exposures_query_runner import (
    ...
    if kind == "SuggestedQuestionsQuery":
        from insights.insightsql_queries.ai.suggested_questions_query_runner import SuggestedQuestionsQueryRunner
    ...
    if kind == NodeKind.MARKETING_ANALYTICS_TABLE_QUERY:
        from products.marketing_analytics.backend.insightsql_queries.marketing_analytics_table_query_runner import (
    ...
    if kind == NodeKind.MARKETING_ANALYTICS_ATTRIBUTION_QUERY:
        from products.marketing_analytics.backend.insightsql_queries.attribution_table_query_runner import (
    ...
    if kind == NodeKind.MARKETING_ANALYTICS_ATTRIBUTION_PATHS_QUERY:
        from products.marketing_analytics.backend.insightsql_queries.attribution_paths_query_runner import (
    ...
    if kind == NodeKind.NON_INTEGRATED_CONVERSIONS_TABLE_QUERY:
        from products.marketing_analytics.backend.insightsql_queries.non_integrated_conversions_table_query_runner import (
    ...
    if kind == "RecordingsQuery":
        from insights.session_recordings.queries.recordings_query_runner import RecordingsQueryRunner
    ...
    if kind == "TraceSpansQuery":
        from insights.schema import TraceSpansQuery
    ...
    if kind == "PropertyValuesQuery":
        from insights.insightsql_queries.property_values_query_runner import PropertyValuesQueryRunner
...
class QueryRunner(ABC, Generic[Q, R, CR]):
    ...
    def _on_user_changed(self) -> None:
        ...
        cached InsightsQLContext / Database that was created with a stale user)."""
        pass
    ...
    def run(
        ...
        # (e.g. QueryRunnerWithInsightsQLContext) construct user-dependent state in
        ...
        # Some queries read live, mutable state that must never be served stale — e.g. InsightsQL against
        ...
        # blocking recompute here so every entry point (SQL editor, insights, Insights AI, MCP) sees the
        ...
        with hanzo_insights.new_context(capture_exceptions=False):
            ...
            if tags := getattr(self.query, "tags", None):
                if tags.name:
                    hanzo_insights.tag("query_name", tags.name)
                    tag_queries(name=tags.name)
                if tags.productKey:
                    product_key = tags.productKey
                    hanzo_insights.tag("product_key", product_key)
            ...
            with slo_operation(
                ...
                try:
                    ...
                    if getattr(exc, "served_from_query_failure_cache", False):
                        # Datastore was never touched; the original failure was already
    ...
    def _execute_and_cache_blocking(
        ...
        *,
        ...
        # breaker that covers this run's execution budget forbids touching Datastore.
        if self._query_failure_caching_enabled:
        ...
        # Capture data warehouse sync warnings from every InsightsQL execution that contributes to this
        ...
        with accumulator_scope() as warnings_accumulator:
            ...
            try:
                ...
                QUERY_EXECUTION_TOTAL.labels(
                    ...
                    category="success",
                    error_type="none",
                    contains_user_insightsql=_contains_user_insightsql_label(),
                ).inc()
                ...
                QUERY_EXECUTION_TOTAL.labels(
                    ...
                    error_type=datastore_error_type(e),
                    contains_user_insightsql=_contains_user_insightsql_label(),
                ).inc()
                if survey_query_metric_labels:
                    SURVEY_QUERY_EXECUTION_TOTAL.labels(
            ...
            if warnings_accumulator and "warnings" in CachedResponse.model_fields:
                ...
                # control warnings attached by InsightsQLQueryExecutor — which must survive the replace.
                other_warnings = [
                    w
    ...
    def get_api_queries_concurrency_limit(self):
        ...
        # TODO - remove once no longer needed, as per https://insights.slack.com/archives/C075D3C5HST/p1766275591753869
    ...
    def to_insightsql(self, **kwargs) -> str:
        with self.timings.measure("to_insightsql"):
    ...
    def get_cache_payload(self) -> dict:
        ...
        payload = {
            ...
            "query": query,
            "team_id": self.team.pk,
            "insightsql_modifiers": to_dict(self.modifiers),
            "products_modifiers": {
    ...
    def requires_fresh_calculation(self) -> bool:
        ...
        `InsightsQLQueryRunner` for the `system.information_schema.*` (data-catalog) case."""
        return False
...
class QueryRunnerWithInsightsQLContext(AnalyticsQueryRunner[AR]):
    database: Database
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._build_insightsql_context_for_user(self.user)

    def _build_insightsql_context_for_user(self, user: Optional[User]) -> None:
        self.database = Database.create_for(team=self.team, user=user)
        self.insightsql_context = InsightsQLContext(team_id=self.team.pk, database=self.database, user=user)

    def _on_user_changed(self) -> None:
        if self.insightsql_context.user is self.user:
            return
        self._build_insightsql_context_for_user(self.user)
