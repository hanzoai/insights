...
from insights.models.js_snippet_versioning import (
...
class Command(BaseCommand):
    help = "Manage insights-js version manifest (publish new versions, yank existing ones). Dry-run by default."
    ...
    def _purge_changed(self, before: dict[str, str], after: dict[str, str]) -> None:
        ...
        for pointer in sorted(changed):
            self.stdout.write(f"Purged CDN cache for insights-js-{pointer}")
    ...
    def _handle_publish(self, options: dict) -> None:
        ...
        self.stdout.write(self.style.SUCCESS(f"Published insights-js v{version}"))
    ...
    def _handle_yank(self, options: dict) -> None:
        ...
        self.stdout.write(self.style.SUCCESS(f"Yanked insights-js v{version}"))
