...
# Datastore-backed correctness proof for the bounded exposed-set scan (`_fetch_exposed_person_uuids`),
...
class TestFreezeExposureDatastore(DatastoreTestMixin, APIBaseTest):
    ...
    def test_fetch_exposed_person_uuids_is_not_clamped_by_default_query_limit(self) -> None:
        ...
        # Under the default limit context the InsightsQL printer rewrites any top-level LIMIT to
        ...
        with patch("insights.insightsql.constants.MAX_SELECT_RETURNED_ROWS", 2):
            uuids = self._service()._fetch_exposed_person_uuids(experiment)
