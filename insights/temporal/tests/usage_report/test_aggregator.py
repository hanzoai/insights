...
from insights.tasks.usage_report import InstanceMetadata, OrgReport, UsageReportCounters
...
from insights.temporal.usage_report.types import Manifest, ReportCompleteness, RunQueryToS3Result, WorkflowContext
...
def _patch_query_index(specs: dict) -> Any:
    return patch.dict(
        "insights.temporal.usage_report.queries.QUERY_INDEX",
        specs,
        clear=True,
    )
...
def test_load_all_data_single_output_uses_spec_name_as_key() -> None:
    ...
    with (
        ...
        patch("insights.temporal.usage_report.aggregator.read_json", side_effect=lambda k: json.loads(s3[k])),
    ):
...
def test_iter_chunk_lines_emits_id_and_report() -> None:
    ...
    with patch("insights.temporal.usage_report.aggregator.serialize_full_org_report", side_effect=fake_serialize):
        result = list(
...
def test_build_manifest_returns_typed_manifest() -> None:
    ...
    with (
        patch("insights.temporal.usage_report.aggregator.settings") as mock_settings,
        patch("insights.temporal.usage_report.aggregator.bucket", return_value="insights-billing-usage-reports"),
    ):
    ...
    assert manifest.site_url == "https://us.hanzo.ai"
    assert manifest.bucket == "insights-billing-usage-reports"
