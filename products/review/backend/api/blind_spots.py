...
from products.review.backend.reviewer.lazy_seed import seed_canonicals_tolerantly, sync_canonical_blind_spots
...
class ReviewBlindSpotsConfigSerializer(serializers.Serializer):
    skill_name = serializers.CharField(
        help_text="Name of the `review-script-blind-spots-*` skill this row represents (the sweep's identity)."
    )
...
class ReviewBlindSpotsConfigViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @extend_schema(
        ...
        description=(
            "List the `review-script-blind-spots-*` skills visible to the requesting user — the "
    ...
    def list(self, request: Request, **kwargs) -> Response:
        ...
        active_by_name = dict(
            ...
            .filter(user_id=user_id, skill_name__startswith=REVIEW_FN_BLIND_SPOTS_PREFIX)
        ...
        visible = visible_skill_names(
            team_id, user_id, prefix=REVIEW_FN_BLIND_SPOTS_PREFIX, canonical_names=CANONICAL_BLIND_SPOTS_SKILL_NAMES
        )
    ...
    @extend_schema(
        ...
        description=(
            "Make a `review-script-blind-spots-*` skill the single sweep that runs on the requesting "
    ...
    def partial_update(self, request: Request, skill_name: str, **kwargs) -> Response:
        if not skill_name.startswith(REVIEW_FN_BLIND_SPOTS_PREFIX):
        ...
        with transaction.atomic():
            configs.filter(user_id=user_id, skill_name__startswith=REVIEW_FN_BLIND_SPOTS_PREFIX, enabled=True).exclude(
