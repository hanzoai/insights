...
class KnowledgeDocument(UUIDModel):
    ...
    # team_id is denormalized off source.team_id so every InsightsQL-exposed table
    ...
    # (Kafka -> worker -> Datastore `document_embeddings`). NULL means "not yet
    ...
    # finds the vectors never landed in Datastore. "Emitted" means produced to
    # Kafka, not confirmed-present in Datastore — see logic.emit_pending_embeddings.
    embeddings_emitted_at = models.DateTimeField(null=True, blank=True)

    objects = TeamScopedManager()

    class Meta:
        db_table = "insights_business_knowledge_knowledgedocument"
        indexes = [
