...
const withNotebookInsightsQLTags = (query: DataVisualizationNode): DataVisualizationNode => ({
    ...query,
    source: {
        ...query.source,
        tags: {
...
export const getSqlEditorSourceQuery = (query: QuerySchema): DataVisualizationNode | null => {
    ...
    if (isDataVisualizationNode(convertedQuery) && isInsightsQLQuery(convertedQuery.source)) {
        return withNotebookInsightsQLTags(convertedQuery)
    }

    if (isInsightsQLQuery(query)) {
        return withNotebookInsightsQLTags({
...
function getRunnableInsightsQLSource(source: InsightsQLQuery): Omit<InsightsQLQuery, 'tags'> {
...
export function hasAlreadyRunSqlEditorSourceQuery(
    ...
    return (
        !!lastRunQuery &&
        equal(getRunnableInsightsQLSource(editorSourceQuery.source), getRunnableInsightsQLSource(lastRunQuery.source))
    )
}
