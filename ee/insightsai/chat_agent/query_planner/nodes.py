...
from posthog.insightsql.ai import SCHEMA_MESSAGE
...
class QueryPlannerNode(TaxonomyUpdateDispatcherNodeMixin, AssistantNode):
    ...
    def _construct_messages(self, state: AssistantState) -> ChatPromptTemplate:
        ...
        serialized_database = database.serialize(
            InsightsQLContext(team=self._team, enable_select_queries=True, database=database)
        )
        insightsql_schema_description = "\n\n".join(
            (
            ...
            [
                (
                    ...
                    [
                        ...
                        {
                            "type": "text",
                            "text": SCHEMA_MESSAGE.format(schema_description=insightsql_schema_description),
                        },
