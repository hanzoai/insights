...
from insights.models.person.util import create_person, create_person_distinct_id, get_persons_by_uuids

logger = structlog.get_logger(__name__)

# personinsights clamps uuid lookups at 250 per request; batch to match.
_PERSONFN_UUID_BATCH = 250
...
def _updated_distinct_ids(team_id: int, distinct_id_versions: list[tuple[str, int]]):
    ...
    for distinct_id, version in distinct_id_versions:
        ...
        # The write goes through personinsights (an external RPC that can't join a
        ...
        if person:
            ...
            if person_uuid not in reset_person_uuids:
                ...
                _reset_person_in_datastore(team_id, person)
...
def _update_distinct_id_in_postgres(distinct_id: str, version: int, team_id: int) -> Optional[Person]:
    ...
    return personinsights_call(
        ...
        lambda: _set_distinct_id_version_floor_via_personinsights(team_id, distinct_id, version),
    )


def _set_distinct_id_version_floor_via_personinsights(team_id: int, distinct_id: str, version: int) -> Optional[Person]:
    from insights.personinsights_client.client import require_personinsights_client
    from insights.personinsights_client.converters import proto_person_to_model
    from insights.personinsights_client.proto import SetPersonDistinctIdVersionFloorRequest
...
def _reset_person_in_datastore(team_id: int, person: Person) -> None:
    person_uuid = str(person.uuid)
    ...
    # reads version from Postgres) won't be ignored by Datastore.
...
def _set_person_version_floor(team_id: int, person_id: int, new_version: int) -> None:
    ...
    personinsights_call(
        ...
        lambda: _set_person_version_floor_via_personinsights(team_id, person_id, new_version),
    )


def _set_person_version_floor_via_personinsights(team_id: int, person_id: int, new_version: int) -> None:
    from insights.personinsights_client.client import require_personinsights_client
    from insights.personinsights_client.proto import SetPersonVersionFloorRequest
...
# ── Orphaned Datastore person repair ────────────────────────────────
#
# The inverse of the reset helpers above: a person can be hard-deleted from the
# persons DB (insights_person + insights_persondistinctid) with no matching
# Datastore tombstone. The row then stays visible to every Datastore-backed
# read path (InsightsQL `persons`, the UI Persons page) while every persons-DB write
# path 404s. This produces the missing tombstones so Datastore agrees with the
# persons DB.


@dataclass
class OrphanedPerson:
    """A Datastore person row that is live in CH but absent from the persons DB."""
...
@dataclass
class _Mapping:
    """The current Datastore winner for a distinct_id (ReplacingMergeTree argMax)."""
...
def find_orphaned_ch_persons(team_id: int, uuids: Optional[list[str]] = None) -> list[OrphanedPerson]:
    """Return Datastore person rows that are live in CH but have no persons-DB row.
    ...
    for start in range(0, len(candidate_uuids), _PERSONFN_UUID_BATCH):
        batch = candidate_uuids[start : start + _PERSONFN_UUID_BATCH]
...
def tombstone_orphaned_ch_persons(
    ...
    """Produce Datastore tombstones for orphaned persons and the distinct_id
    ...
    for orphan in orphans:
        # No persons-DB row exists, so derive the tombstone from Datastore. Version
...
def _find_reverse_drift(team_id: int, deleted_winners: list[_Mapping], orphan_uuids: set[str]) -> list[tuple[str, str]]:
    ...
    for start in range(0, len(winner_uuids), _PERSONFN_UUID_BATCH):
        batch = winner_uuids[start : start + _PERSONFN_UUID_BATCH]
