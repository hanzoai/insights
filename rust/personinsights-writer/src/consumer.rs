...
impl ConsumerTask {
    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            if self.buffer.is_full() {
                counter!("personinsights_writer_flushes_by_trigger_total", "trigger" => "backpressure")
                    .increment(1);
            ...
            tokio::select! {
                ...
                _ = self.handle.shutdown_recv() => {
                    ...
                    counter!("personinsights_writer_flushes_by_trigger_total", "trigger" => "shutdown")
                        .increment(1);
                    self.send_flush().await;
                    break;
                }
                ...
                _ = flush_timer.tick() => {
                    if !self.buffer.is_empty() {
                        counter!("personinsights_writer_flushes_by_trigger_total", "trigger" => "timer")
                            .increment(1);
                        self.send_flush().await;
                    }
                }
                ...
                msg = self.consumer.recv() => {
                    match msg {
                        Ok(borrowed_msg) => {
                            ...
                            gauge!(
                                "personinsights_writer_partition_offset",
                            ...
                            if let Some(payload) = borrowed_msg.payload() {
                                match Person::decode(payload) {
                                    Ok(person) => {
                                        counter!("personinsights_writer_messages_consumed_total")
                                            .increment(1);
                                        self.buffer.insert(person, partition, offset);
                                        gauge!("personinsights_writer_buffer_size")
                                            .set(self.buffer.len() as f64);

                                        if self.buffer.len() >= self.flush_buffer_size {
                                            counter!(
                                                "personinsights_writer_flushes_by_trigger_total",
                                                "trigger" => "size"
                                            )
                                            .increment(1);
                                            self.send_flush().await;
                                    ...
                                    Err(e) => {
                                        counter!("personinsights_writer_decode_errors_total")
                                            .increment(1);
                                        warn!(
                        ...
                        Err(e) => {
                            counter!("personinsights_writer_kafka_errors_total").increment(1);
    ...
    async fn send_flush(&mut self) {
        ...
        gauge!("personinsights_writer_buffer_size").set(self.buffer.len() as f64);
        if persons.is_empty() {
            return;
        }
        ...
        histogram!("personinsights_writer_channel_send_duration_ms")
