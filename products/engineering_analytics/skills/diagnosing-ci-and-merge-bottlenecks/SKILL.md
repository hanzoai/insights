...
MCP tools** — you call them, you don't write SQL. Dogfooded on `Insights/insights`; the same tools serve
autonomous agents (e.g. Insights Desktop) reasoning about their own PRs.
...
are directly queryable with InsightsQL, and that skill carries the curated column semantics plus the
