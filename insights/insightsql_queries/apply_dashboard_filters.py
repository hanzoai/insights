...
from insights.schema import DashboardFilter, InsightsQLVariable, NodeKind

from insights.exceptions_capture import capture_exception
from insights.insightsql_queries.query_runner import get_query_runner
from insights.insightsql_queries.utils.dashboard_filter_conflicts import filters_contradict
...
def apply_dashboard_variables_to_dict(query: dict, variables_overrides: dict[str, dict], team: Team) -> dict:
    ...
    if query.get("kind") == NodeKind.FN_QL_QUERY:
        ...
        for variable_id, overriden_insightsql_variable in variables_overrides.items():
            query_variable = query_variables.get(variable_id)
            if query_variable:
                query_variables[variable_id] = {
                    "variableId": variable_id,
                    "code_name": query_variable["code_name"],
                    "value": overriden_insightsql_variable.get("value"),
                }
...
def apply_dashboard_variables(query: Any, variables_overrides: dict[str, dict], team: Team) -> Any:
    ...
    if getattr(query, "kind", None) == NodeKind.FN_QL_QUERY:
        ...
        for variable_id, overriden_insightsql_variable in variables_overrides.items():
            query_variable: InsightsQLVariable = updated_variables.get(variable_id)

            if query_variable:
                updated_variables[variable_id] = InsightsQLVariable(
                    variableId=variable_id,
                    code_name=query_variable.code_name,
                    value=overriden_insightsql_variable.get("value"),
                    isNull=overriden_insightsql_variable.get("isNull"),
                )
