...
from posthog.insightsql_queries.insights.trends.display import TrendsDisplay
from posthog.insightsql_queries.insights.trends.series_with_extras import SeriesWithExtras
from posthog.insightsql_queries.insights.trends.trends_actors_query_builder import TrendsActorsQueryBuilder
from posthog.insightsql_queries.insights.trends.trends_query_builder import TrendsQueryBuilder
from posthog.insightsql_queries.query_runner import AnalyticsQueryRunner
from posthog.insightsql_queries.utils.formula_ast import FormulaAST
...
from posthog.insightsql_queries.utils.timestamp_utils import format_label_date, get_earliest_timestamp_from_series
...
class TrendsQueryRunner(AnalyticsQueryRunner[TrendsQueryResponse]):
    ...
    def __init__(
        ...
        from posthog.insightsql_queries.insights.utils.utils import convert_active_user_math_based_on_interval
    ...
    def _calculate(self):
        ...
        if len(queries) == 0:
            ...
            if len(queries) == 1:
                response_insightsql_query = queries[0]
            else:
                response_insightsql_query = ast.SelectSetQuery.create_from_queries(queries, "UNION ALL")

            with self.timings.measure("printing_insightsql_for_response"):
                response_insightsql = to_printed_insightsql(response_insightsql_query, self.team, self.modifiers)
    ...
    def build_series_response(self, response: InsightsQLQueryResponse, series: SeriesWithExtras, series_count: int):
