...
from products.alerts.backend.evaluation.insightsql import (
    ANY_ROW_MAX_ROWS,
    LAST_ROW_MAX_ROWS,
    InsightsQLExtractor,
...
CALC_PATH = "products.alerts.backend.evaluation.insightsql.calculate_for_query_based_insight"
...
def _alert(condition_type=AlertConditionType.ABSOLUTE_VALUE, config: dict | None = None):
    ...
    alert.config = {"type": "InsightsQLAlertConfig", "evaluation": "last_row", **(config or {})}
    return alert
...
def _extract(rows, *, columns=None, condition_type=AlertConditionType.ABSOLUTE_VALUE, config: dict | None = None):
    with patch(CALC_PATH) as calc:
        ...
        return InsightsQLExtractor().extract(_alert(condition_type, config), MagicMock(), MagicMock(), _IF_STALE)
...
def test_accepts_decimal_columns():
    # Datastore Decimal columns surface as decimal.Decimal — they are valid numeric values.
...
def test_evaluation_uses_saved_variable_values_not_session_overrides():
    ...
    with patch(CALC_PATH) as calc:
        ...
        InsightsQLExtractor().extract(_alert(), MagicMock(), MagicMock(), _IF_STALE)
...
def test_explicit_column_pick():
    ...
    result = _extract(rows, columns=["day", "errors", "total"], config={"type": "InsightsQLAlertConfig", "column": "total"})
    assert [p.value for p in result.series[0].points] == [10.0, 42.0]
...
def test_explicit_column_not_in_result_raises():
    with pytest.raises(AlertExtractionError, match="not in the result columns"):
        _extract([[1]], columns=["count"], config={"type": "InsightsQLAlertConfig", "column": "missing"})


def test_explicit_column_without_metadata_raises():
    with pytest.raises(AlertExtractionError, match="no column metadata"):
        _extract([[1]], config={"type": "InsightsQLAlertConfig", "column": "count"})
...
def test_single_row_labels_evaluated_row_by_label_column(evaluation, expected_label):
    ...
    result = _extract(
        ...
        config={"type": "InsightsQLAlertConfig", "evaluation": evaluation, "label_column": "metric"},
    )
...
def test_single_row_missing_label_column_raises():
    with pytest.raises(AlertExtractionError, match="label column"):
        _extract([["a", 1]], columns=["metric", "value"], config={"type": "InsightsQLAlertConfig", "label_column": "gone"})


def test_any_row_breaches_on_any_value_with_row_label():
    rows = [["US", 0.1], ["DE", 0.4], ["FR", 0.2]]
    result = _extract(
        rows, columns=["country", "error_rate"], config={"type": "InsightsQLAlertConfig", "evaluation": "any_row"}
    )
...
def test_any_row_explicit_label_column():
    ...
    result = _extract(
        ...
        config={"type": "InsightsQLAlertConfig", "evaluation": "any_row", "column": "errors", "label_column": "env"},
    )
    assert [s.label for s in result.series] == ["prod", "dev"]


def test_any_row_label_falls_back_to_row_number():
    result = _extract([[5], [50]], config={"type": "InsightsQLAlertConfig", "evaluation": "any_row"})
...
def test_any_row_missing_label_column_raises():
    with pytest.raises(AlertExtractionError, match="label column"):
        _extract(
            [[1]],
            columns=["count"],
            config={"type": "InsightsQLAlertConfig", "evaluation": "any_row", "label_column": "missing"},
        )
...
def test_any_row_breach_list_is_capped():
    ...
    result = _extract(rows, columns=["name", "value"], config={"type": "InsightsQLAlertConfig", "evaluation": "any_row"})
...
def test_any_row_accepts_exactly_the_row_cap():
    ...
    result = _extract(rows, config={"type": "InsightsQLAlertConfig", "evaluation": "any_row"})
...
def test_any_row_rejects_too_many_rows():
    ...
    with pytest.raises(AlertExtractionError, match=f"at most {ANY_ROW_MAX_ROWS} rows"):
        _extract(rows, config={"type": "InsightsQLAlertConfig", "evaluation": "any_row"})
...
def test_any_row_rejects_relative_conditions():
    with pytest.raises(AlertExtractionError, match="absolute value conditions"):
        _extract(
            ...
            config={"type": "InsightsQLAlertConfig", "evaluation": "any_row"},
        )
