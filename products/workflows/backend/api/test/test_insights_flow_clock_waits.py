...
def wait_on(insightsql: str) -> dict:
    return {
        "id": "wait_1",
        "name": "wait_1",
        "type": "wait_until_condition",
        "config": {
            "condition": {"filters": {"properties": [{"key": insightsql, "type": "insightsql", "value": None}]}},
...
class TestClockBasedWaitRejection(APIBaseTest):
    ...
    def test_rejects_a_wait_that_depends_on_the_clock(self, _name: str, insightsql: str):
        ...
        status, body = self._post(wait_on(insightsql))
    ...
    def test_accepts_a_wait_a_stream_can_wake(self, _name: str, insightsql: str):
...
class TestGrandfatheredClockWaits(APIBaseTest):
    ...
    def _legacy_flow(self, status: str = "active") -> InsightsFlow:
    ...
    def test_a_paused_flow_can_be_resumed(self):
        ...
        response = self.client.patch(f"/v1/projects/{self.team.id}/insights_flows/{flow.id}", {"status": "active"})
    ...
    def test_editing_the_clock_condition_itself_is_still_refused(self):
        ...
        response = self.client.patch(f"/v1/projects/{self.team.id}/insights_flows/{flow.id}", {"actions": edited})
    ...
    def test_adding_a_second_clock_wait_is_still_refused(self):
        ...
        response = self.client.patch(
            f"/v1/projects/{self.team.id}/insights_flows/{flow.id}", {"actions": [*flow.actions, added]}
        )
