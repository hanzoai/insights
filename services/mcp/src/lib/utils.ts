...
export function hash(data: string): string {
    ...
    const salt = crypto.createHash('sha256').update('insights_mcp_salt').digest()
...
export function formatPrompt(template: string, vars: Record<string, string>): string {
    ...
    // e.g. `` `$` `` (common in Insights docs that mention `$pageview`) would splice the
...
// Caller-supplied selection between the tool-based MCP (each Insights tool registered
// individually) and the CLI-based MCP (a single `insights` CLI-like tool that wraps
