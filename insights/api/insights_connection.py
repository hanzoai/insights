"""Forward arbitrary API requests to another Insights project through a `insights` connection.

A `insights` integration (see `insights.models.integration`) holds a user-consented, refreshable OAuth
grant against another Insights project, in another region or your own. This viewset lets the connecting
...
def _connection_access_token(integration: Integration) -> str:
    ...
    if not token:
        raise ValidationError("This Insights connection has no usable access token — reconnect it.")
    return token
...
def _enforce_caller_covers_connection_scopes(request: Request, integration: Integration) -> None:
    ...
    if not granted:
        ...
        raise PermissionDenied(
            "This Insights connection's granted scopes are unknown, so it can only be used with a "
...
def _inflight_slot(integration_id: int) -> Iterator[bool]:
    ...
    key = f"insights_connection_inflight:{integration_id}"
...
class InsightsConnectionForwardSerializer(serializers.Serializer):
...
class InsightsConnectionForwardResponseSerializer(serializers.Serializer):
...
class InsightsConnectionViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    serializer_class = InsightsConnectionForwardSerializer

    def get_throttles(self) -> list[BaseThrottle]:
        if self.action == "forward":
            return [InsightsConnectionForwardThrottle(), *super().get_throttles()]
    ...
    def _get_connection(self, pk: str | None) -> Integration:
        ...
        if integration.created_by_id != getattr(self.request.user, "id", None):
            raise PermissionDenied("You can only use a Insights connection you created.")
        return integration

    @extend_schema(
        request=InsightsConnectionForwardSerializer,
        responses={200: OpenApiResponse(response=InsightsConnectionForwardResponseSerializer)},
        summary="Forward a request through a Insights connection",
        description="Replay an API request against the connected Insights project. The server injects the connection's token; the response is passed through.",
    )
    ...
    def forward(self, request: Request, pk: str | None = None, **kwargs: Any) -> Response:
        ...
        if request.headers.get(CONNECTION_MARKER_HEADER):
            raise ValidationError("A request forwarded through a Insights connection cannot be forwarded again.")

        integration = self._get_connection(pk)
        _enforce_caller_covers_connection_scopes(request, integration)

        serializer = InsightsConnectionForwardSerializer(data=request.data)
        ...
        base = insights_connect_base_url(integration.config.get("region"))
        ...
        try:
            with _inflight_slot(integration.id) as acquired:
                if not acquired:
                    raise Throttled(detail="Too many concurrent requests through this Insights connection.")
            ...
            logger.warning(
                "insights_connection_forward_unreachable",
