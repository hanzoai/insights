...
from insights.workos_radar import (
...
class TestRadarApiCall(TestCase):
    @patch("insights.workos_radar.requests.post")
...
class TestRadarEventLogging(TestCase):
    @patch("insights.workos_radar.hanzo_insights.capture")
...
class TestEvaluateAuthAttempt(TestCase):
    ...
    @patch("insights.workos_radar._log_radar_event")
    @patch("insights.workos_radar._call_radar_api")
    @patch("insights.workos_radar.is_radar_bypass_email", return_value=True)
    @override_settings(WORKOS_RADAR_ENABLED=True, WORKOS_RADAR_API_KEY="test_key")
...
class TestRadarBypassViewSet(TestCase):
    def setUp(self):
        ...
        self.staff_user = User.objects.create_user(
            email="admin@hanzo.ai", password="testpass123!", is_staff=True, first_name="Admin"
        )
        self.non_staff_user = User.objects.create_user(
            email="user@hanzo.ai", password="testpass123!", is_staff=False, first_name="User"
        )
        self.redis_client = get_client()
        self.redis_client.delete(WORKOS_RADAR_BYPASS_REDIS_KEY)
...
class TestChallengeFlow(TestCase):
    ...
    @patch("insights.workos_radar.verify_turnstile_token", return_value=False)
    @patch("insights.workos_radar.validate_and_consume_nonce", return_value=True)
    @patch("insights.workos_radar._log_radar_event")
    @patch("insights.workos_radar._call_radar_api")
    @override_settings(WORKOS_RADAR_ENABLED=True, WORKOS_RADAR_API_KEY="test_key")
    ...
    @patch("insights.workos_radar.verify_turnstile_token")
    @patch("insights.workos_radar.validate_and_consume_nonce", return_value=False)
    @patch("insights.workos_radar._log_radar_event")
    @patch("insights.workos_radar._call_radar_api")
    @override_settings(WORKOS_RADAR_ENABLED=True, WORKOS_RADAR_API_KEY="test_key")
    ...
    def test_challenge_verdict_blocks_with_expired_nonce_despite_valid_token(
        ...
        from insights.turnstile import create_challenge_nonce
