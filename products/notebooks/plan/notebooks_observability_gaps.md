...
Goal: instrument Notebooks so we (Insights, dogfooding) can answer real usage questions
...
4. How often do **Insights Desktop / Max** create notebooks?
...
All direct `insights.capture(...)`; **no** `eventUsageLogic` involvement.
...
| 3    | Max `create_notebook` tool    | `ee/scriptai/tools/create_notebook/helpers.py:105-192` (`aupsert_notebook`) | **none**  | **none**                                  |
...
- Public link: `SharingConfigurationViewSet` enable/disable at `insights/api/sharing.py:463-560`;
...
`Notebook.short_id` → `AgentArtifact.short_id` (`ee/scriptai/tools/create_notebook/helpers.py:124,181`)
→ `AgentArtifact.conversation` FK (`products/insights_ai/backend/models/assistant.py:340`)
...
### ⚠️ Note on "Insights Desktop" vs generic MCP clients (Q4 vs Q5)

Insights Desktop creates notebooks **via the MCP `notebooks-create` tool** — it is itself an MCP
...
- the MCP client name / user-agent (Insights Desktop should send an identifiable client id).

Action: when emitting the server-side `notebook created` event, capture `mcp_client` /
`api_key_type` (or an `agent`-tagged sub-source like `mcp_insights_code`) so Insights Desktop is
distinguishable from third-party MCP usage. Confirm Insights Desktop sends a stable client id;
...
- `mcp_client` / `api_key_type` (when `creation_source = mcp`): identify the calling client so
  **Insights Desktop** (Q4) is separable from a customer's own MCP client (Q5). See the Insights
...
- `notebook sharing enabled` — at `insights/api/sharing.py` enable path. Props: `short_id`,
...
- `notebook viewed via shared link` on the anonymous `SharingViewerPageViewSet`
  (`insights/api/sharing.py:1202`) for external-view counting.
...
- **Server-side transport.** Use `report_user_action` (`insights/event_usage.py:396`) for
  ...
  `ph_scoped_capture` (`insights/ph_client.py:63`) — a plain `hanzo_insights.capture()` in
...
- [x] Emit `insights.capture('notebook opened', …)` in the load path — `loadNotebookSuccess`
...
- [ ] `create_notebook` (`api.py:142`) — add a `creation_source` param (default e.g.
      ...
      (`insights/temporal/ai/anomaly_investigation/workflow.py:149`) then only passes
...
- [ ] `create_group_notebook` (`api.py:192`) — default source `group_auto`. `_create_notebook_for_group`
      (`ee/datastore/views/groups.py:873`) needs **no change**.
...
2. **InsightsQL / SQL** — the `notebooks` system table (`insights/insightsql/database/schema/system.py:1225`,
   maps to `insights_notebook`, `access_scope="notebook"`) exposes `content` **and** `text_content`
   ...
   below).
3. **Max artifact handler** — `NotebookHandler.alist` (`ee/scriptai/artifacts/handlers/notebook.py:47`)
   reads from the **`AgentArtifact` table, not `insights_notebook`** (docstring: "Notebooks are only
...
- [ ] **Do not** capture at the facade `get_notebook` / `aget_notebook` — those are called for
      ...
      (`ee/scriptai/tools/create_notebook/helpers.py:124`), so capturing there would fire constantly
...
from 2e are a **lower bound**. Capturing it would mean inspecting executed InsightsQL for a reference to
...
- [ ] `notebook sharing enabled` — `SharingConfigurationViewSet` enable path
      (`insights/api/sharing.py:463-560`, next to the `log_activity` at `:533`). Props: `short_id`,
...
`SELECT content, text_content FROM notebooks` against the `notebooks` InsightsQL system table
(`insights/insightsql/database/schema/system.py:1225`), which bypasses the REST endpoint entirely.

**Prerequisite (not done here): analyze overall SQL access metrics first.** Before instrumenting,
understand how notebook-table reads sit within general InsightsQL/query usage — volume, who runs them
...
- [ ] (After the analysis) Emit `notebook read` with `read_source: sql` from the query layer when
      an executed InsightsQL query selects from the `notebooks` table. Mark it clearly so REST/MCP vs SQL
