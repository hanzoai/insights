...
describe('utils', () => {
    ...
    describe('extractBearerToken', () => {
        ...
        const req = (opts: { header?: string; url?: string }): Request =>
            new Request(opts.url ?? 'https://mcp.hanzo.ai/', {
    ...
    describe('formatPrompt', () => {
        ...
        it('does not splice the template prefix when a value contains $` (realistic Insights case)', () => {
    ...
    describe('sanitizeHeaderValue', () => {
        it.each([
            ...
            ['strips non-Latin1 chars (emoji)', 'Claude \u{1F916} Desktop', 'Claude  Desktop'],
            ['strips non-Latin1 scripts', 'クライアント', undefined],
            ['keeps Latin-1 high range', 'café', 'café'],
    ...
    describe('omitResponseFields', () => {
        ...
        it('strips compiled InsightsQL artifacts from cohort filter conditions (cohorts-retrieve)', () => {
            const cohort = {
                id: 350280,
                name: 'sibling cohort',
                filters: {
    ...
    describe('withInsightsUrl', () => {
        const context = {
            stateManager: { getProjectId: async () => 42 },
            api: { getProjectBaseUrl: (id: number) => `https://app/project/${id}` },
        } as unknown as Context

        it('adds _insightsUrl as a sibling field on an object result', async () => {
            const result = await withInsightsUrl(context, { id: 7, name: 'x' }, '/inbox/7')
            expect(result).toEqual({ id: 7, name: 'x', _insightsUrl: 'https://app/project/42/inbox/7' })
        })

        // Regression: spreading an array into an object (`{ ...arr }`) corrupts a raw-array
        // list response into `{ 0: …, 1: …, _insightsUrl: … }`. Arrays must be wrapped in
        // `{ results, _insightsUrl }` so they stay iterable for the agent.
        it('wraps a raw array result in { results, _insightsUrl }', async () => {
            const arr = [{ id: 1 }, { id: 2 }]
            const result = await withInsightsUrl(context, arr, '/inbox')
            expect(result).toEqual({ results: [{ id: 1 }, { id: 2 }], _insightsUrl: 'https://app/project/42/inbox' })
        })

        it('does not corrupt the array into numeric-keyed object fields', async () => {
            const result = await withInsightsUrl(context, [{ id: 1 }], '/inbox')
