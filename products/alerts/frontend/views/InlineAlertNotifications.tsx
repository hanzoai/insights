...
function getInsightsFunctionDestination(
    ...
    const inputs = insightsFunction.inputs as SlackDestinationInputs | null | undefined
    ...
    if (insightsFunction.template_id === 'template-discord') {
        const webhookUrl = insightsFunction.inputs?.webhookUrl?.value
        return { type: 'Discord', detail: typeof webhookUrl === 'string' ? webhookUrl : null }
    }
    if (insightsFunction.template_id === 'template-microsoft-teams') {
        const webhookUrl = insightsFunction.inputs?.webhookUrl?.value
        return { type: 'Microsoft Teams', detail: typeof webhookUrl === 'string' ? webhookUrl : null }
    }
    const urlValue = insightsFunction.inputs?.url?.value
    if (urlValue && typeof urlValue === 'string') {
        return { type: 'Webhook', detail: urlValue }
    }
    return { type: insightsFunction.name, detail: null }
}
        ...
        <div>
            <AlertNotificationDestinationEditor
                destinations={{
                    ...
                    existingLoading: existingInsightsFunctionsLoading,
