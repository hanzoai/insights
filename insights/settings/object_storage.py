...
from insights.settings import get_from_env
...
OBJECT_STORAGE_BUCKET = os.getenv("OBJECT_STORAGE_BUCKET", "insights")
...
OBJECT_STORAGE_EXTERNAL_WEB_ANALYTICS_BUCKET = os.getenv("OBJECT_STORAGE_EXTERNAL_WEB_ANALYTICS_BUCKET", "insights")
...
# per-run Parquet objects via Datastore `INSERT INTO FUNCTION s3(...)` and the read API globs
# them back with `s3(...)`, so only the Datastore cluster needs bucket access — the Dagster
...
#
# This must be the endpoint the Datastore *cluster* can reach, which is not always
# OBJECT_STORAGE_ENDPOINT: CI points OBJECT_STORAGE_ENDPOINT at `localhost:19000` for the test
# process, but Datastore runs in docker-compose and reaches object storage by its service name
