...
local_manifest="${CLI_RELEASE_SMOKE_MANIFEST:-/tmp/insights-cli-local-dist-manifest.json}"
global_manifest="${CLI_RELEASE_SMOKE_GLOBAL_MANIFEST:-/tmp/insights-cli-local-dist-global-manifest.json}"
...
test -s "$repo_root/cli/lib/insights-api-cli.mjs"
...
archive="$repo_root/target/distrib/insights-cli-$target.tar.gz"
if [[ ! -f "$archive" ]]; then
...
tar -tzf "$archive" | grep -qx "insights-cli-$target/lib/insights-api-cli.mjs"
...
test -s "$tmpdir/unpack/lib/insights-api-cli.mjs"

echo "Running packaged insights-cli api --agent-help..."
if ! HOME="$tmpdir/home" "$tmpdir/unpack/insights-cli" api --agent-help > "$tmpdir/agent-help.txt" 2> "$tmpdir/agent-help.err"; then
    cat "$tmpdir/agent-help.err" >&2
    exit 1
fi
grep -q "# Insights API guide for agents" "$tmpdir/agent-help.txt"
...
installer="$repo_root/target/distrib/insights-cli-installer.sh"
if [[ ! -s "$installer" ]]; then
...
test -x "$tmpdir/installed/insights-cli"

echo "Running installed insights-cli api --agent-help..."
if ! HOME="$tmpdir/installed-home" "$tmpdir/installed/insights-cli" api --agent-help > "$tmpdir/installed-agent-help.txt" 2> "$tmpdir/installed-agent-help.err"; then
    cat "$tmpdir/installed-agent-help.err" >&2
    exit 1
fi
grep -q "# Insights API guide for agents" "$tmpdir/installed-agent-help.txt"

if [[ ! -s "$tmpdir/installed/lib/insights-api-cli.mjs" ]]; then
    shopt -s nullglob
    materialized_bundles=("$tmpdir/installed-home/.insights/api-cli"/*/insights-api-cli.mjs)
...
echo "Packaged CLI available at: $repo_root/target/distrib/insights-cli-$target/insights-cli"
echo "Note: this smoke test does not replace any insights-cli already on your PATH."
