"""Post-command contextual hints for insightscli CLI.
...
_HINT_DEFS: tuple[_HintDef, ...] = (
    _HintDef("doctor:disk", 7, "Run `insightscli doctor:disk` to check for disk bloat"),
    _HintDef("doctor:zombies", 7, "Run `insightscli doctor:zombies` to find orphaned processes"),
    _HintDef("doctor", 14, "Run `insightscli doctor` for a quick health check"),
)
...
def _get_state_path() -> Path:
    return Path.home() / ".config" / "insights" / "insightscli_hints.json"
