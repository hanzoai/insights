...
import structlog
from datastore_driver.errors import ServerException as DatastoreServerException
...
from insights.insightsql.database.models import DatabaseField, FieldOrTable, StructDatabaseField
from insights.insightsql.database.s3_table import (
    DataWarehouseTable as InsightsQLDataWarehouseTable,
    build_function_call,
)
from insights.insightsql.escape_sql import escape_datastore_identifier, escape_param_datastore
...
from .external_table_definitions import external_tables, get_insightsql_column_name_mapping
...
ExtractErrors = {
    ...
    # tables) use a different vocabulary than Datastore's native S3 errors above.
...
# Internal plumbing columns added during sync, hidden from the InsightsQL catalog (see insightsql_definition)
...
# (each request also pins ~300MB of RSS for the embedded Datastore). Running it in a
# subprocess lets us kill it and degrade to the Datastore-cluster fallback.
CHDB_QUERY_TIMEOUT_SECONDS = 30.0

# Datastore's Hive-style partition inference guesses a type per partition-folder value it
...
# InsightsQLGlobalSettings.use_hive_partitioning disables this for the normal InsightsQL query path; the
# raw Datastore queries below bypass that path and must opt out the same way.
...
def get_insightsql_field_for_column(
    column_name: str,
    column_definition: dict[str, Any] | str,
    datastore_type: str,
    is_nullable: bool,
) -> DatabaseField:
    if isinstance(column_definition, dict) and column_definition.get("insightsql") == "StructDatabaseField":
        ...
        if isinstance(nested_definitions, dict):
            for nested_name, nested_definition in nested_definitions.items():
                ...
                nested_datastore_type = str(nested_definition.get("datastore", "String"))
                nested_is_nullable = False
                if nested_datastore_type.startswith("Nullable("):
                    nested_datastore_type = nested_datastore_type.replace("Nullable(", "")[:-1]
                    nested_is_nullable = True

                child_fields[nested_name] = get_insightsql_field_for_column(
                    nested_name,
                    nested_definition,
                    nested_datastore_type,
                    nested_is_nullable,
                )
    ...
    if isinstance(column_definition, str):
        ...
        return LEGACY_DATASTORE_INSIGHTSQL_MAPPING[insightsql_type_str](name=column_name, nullable=is_nullable)

    return STR_TO_INSIGHTSQL_MAPPING.get(
        str(column_definition.get("insightsql", "UnknownDatabaseField")),
        STR_TO_INSIGHTSQL_MAPPING["UnknownDatabaseField"],
    )(name=column_name, nullable=is_nullable)


def insightsql_fields_and_structure_for_columns(
    columns: dict[str, Any],
    modifiers: Optional["InsightsQLQueryModifiers"] = None,
    column_order: list[str] | None = None,
) -> tuple[dict[str, FieldOrTable], list[str]]:
    """Shared columns → InsightsQL fields mapping for warehouse and direct virtual tables.
    ...
    for column, type in reconstruct_ordered_columns(columns, column_order):
        ...
        # TODO: remove when addressed https://github.com/Datastore/Datastore/issues/37594
        ...
        fields[column] = get_insightsql_field_for_column(column, type, datastore_type, is_nullable)
...
class DataWarehouseTable(CreatedMetaFields, UpdatedMetaFields, UUIDTModel, DeletedMetaFields):
    ...
    def get_user_facing_columns(self) -> list[dict[str, Any]]:
        ...
        `ExternalDataSchema.schema_metadata`. Handles both the dict (`{"datastore": ...}`) and the
        legacy plain-string column shapes.

        Curated sources (Stripe, etc.) rename or wrap some raw columns when exposing them via InsightsQL
        (`created` -> `created_at`, `customer` -> `customer_id`), so `name` is the InsightsQL-visible name
        ...
        `get_insightsql_column_name_mapping(self.table_name_without_prefix())` directly.
        """
        insightsql_by_raw = get_insightsql_column_name_mapping(self.table_name_without_prefix())
        result: list[dict[str, Any]] = []
        for name, definition in (self.columns or {}).items():
            if name in HIDDEN_COLUMNS:
                continue
            if isinstance(definition, dict):
                datastore_type = definition.get("datastore") or definition.get("insightsql") or ""
            else:
                datastore_type = definition or ""
            result.append(
                {
                    "name": insightsql_by_raw.get(name, name),
                    "data_type": clean_type(datastore_type) if datastore_type else "unknown",
                    "is_nullable": "Nullable(" in datastore_type,
                }
            )
        return result
    ...
    def validate_column_type(
        ...
        *,
        ...
        try:
            ...
            # Deferred: insights.schema (the pydantic models) stays off django.setup(),
    ...
    def get_columns(
        ...
        try:
            ...
            quoted_placeholders = {k: escape_param_datastore(v) for k, v in placeholder_context.values.items()}
    ...
    def get_max_value_for_column(self, column: str) -> Any | None:
        try:
            ...
            return result[0][0]
        except DatastoreServerException as err:
    ...
    def _direct_columns_are_complete(self) -> bool:
        ...
        `insightsql_definition`); if they aren't preloaded, returns False (safe: expand) rather than issue
    ...
    def insightsql_definition(
        ...
        from products.data_warehouse.backend.facade.sources import (  # noqa: PLC0415 — breaks an import cycle
            DIRECT_DATASTORE_DATABASE_OPTION,
            DIRECT_DATASTORE_TABLE_OPTION,
        ...
        fields, structure = insightsql_fields_and_structure_for_columns(columns, modifiers, column_order=self.column_order)
        ...
        # Engine-keyed (no is_direct_datastore) to satisfy the source-agnostic guard. The
        ...
        if (
            ...
            and self.external_data_source.direct_engine == "datastore"
        ):
            job_inputs = self.external_data_source.job_inputs or {}
            # Every direct-Datastore table is discovered from the source's single configured
            ...
            if isinstance(configured_database, str) and configured_database.strip():
                datastore_database = configured_database
            else:
                stored_database = self.options.get(DIRECT_DATASTORE_DATABASE_OPTION)
                datastore_database = stored_database if isinstance(stored_database, str) else "default"
            datastore_table_name = (
                self.options.get(DIRECT_DATASTORE_TABLE_OPTION)
                if isinstance(self.options.get(DIRECT_DATASTORE_TABLE_OPTION), str)
                else self.name
            )
            return DirectDatastoreTable(
                name=self.name,
                fields=fields,
                datastore_database=datastore_database,
                datastore_table_name=datastore_table_name,
                external_data_source_id=str(self.external_data_source_id),
                connection_metadata=self.external_data_source.connection_metadata,
        ...
        table_def = InsightsQLDataWarehouseTable(
            name=self.name,
            url=self.url_pattern,
            queryable_folder=self.queryable_folder,
            format=self.format,
        ...
        if self._is_csv_format():
            ...
            table_def.top_level_settings = InsightsQLQuerySettings(
    ...
    def get_datastore_column_type(self, column_name: str) -> Optional[str]:
        columns = self.columns or {}
        datastore_type = columns.get(column_name, None)

        if isinstance(datastore_type, dict) and columns[column_name].get("datastore"):
            datastore_type = columns[column_name].get("datastore")
    ...
    # Datastore error codes from CSV double-quote parse mismatches.
    # Wrong quoting causes Datastore to mis-split fields, producing
    ...
    def _validate_csv_double_quotes_setting(self) -> None:
        ...
        except DatastoreServerException as e:
    ...
    def _safe_expose_ch_error(self, err):
        # Match ExtractErrors against the raw Datastore message: wrap_datastore_query_error may
        # rewrite the message for some codes (e.g. STD_EXCEPTION), which would hide the substrings
        # we key on here.
        raw_message = err.message if isinstance(err, DatastoreServerException) else str(err)
        err = wrap_datastore_query_error(err)

        # Only Datastore ServerException-derived errors carry a `.message`. Everything else —
        # transient connection/read errors (e.g. an EOFError from a dropped Datastore socket) and
        # already-translated APIExceptions like DatastoreAtCapacity — has no `.message`, so re-raise
