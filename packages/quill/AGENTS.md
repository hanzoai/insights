...
Quill is Insights's design system built on React, TypeScript, Base UI, and Tailwind v4.
...
packages/quill/
├── packages/
│   ├── tokens/       @hanzo/quill-tokens      — Design tokens (JS + generated CSS)
│   ├── primitives/   @hanzo/quill-primitives   — Base UI components (this guide)
│   ├── components/   @hanzo/quill-components   — Composed primitives
│   └── blocks/       @hanzo/quill-blocks       — Product-level patterns
└── apps/
...
- Build primitives: `pnpm --filter @hanzo/quill-primitives build`
- Build tokens: `pnpm --filter @hanzo/quill-tokens build`
...
1. `@hanzo/quill-tokens` defines colors as JS objects
