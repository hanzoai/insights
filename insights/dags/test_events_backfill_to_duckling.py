...
class TestResolveDucklingTarget:
    ...
    @patch("insights.dags.events_backfill_to_duckling.get_stored_bucket_config", return_value=None)
    @patch("insights.dags.events_backfill_to_duckling.get_org_id_for_team", return_value="org-1")
    ...
    def test_control_plane_wins_over_stale_stored_server_bucket(self, _mock_org: MagicMock, _mock_tables: MagicMock):
        ...
        server = MagicMock(bucket="insights-duckling-stale-prod-us", region="us-east-1")
        with (
            patch(
                "insights.dags.events_backfill_to_duckling.get_stored_bucket_config",
                return_value=server,
            ),
            patch(
                "products.managed_warehouse.backend.facade.api.get_control_plane_bucket",
    ...
    def test_falls_back_to_stored_server_when_control_plane_unavailable(
        ...
        server = MagicMock(bucket="insights-duckling-org-1-mw-prod-us", region="us-east-1")
        with (
            patch(
                "insights.dags.events_backfill_to_duckling.get_stored_bucket_config",
                return_value=server,
...
class TestResolveTableNames:
    ...
    def test_passes_through_resolved_names(self):
        with patch(
            "insights.dags.events_backfill_to_duckling.resolve_events_persons_tables",
...
class TestFullBackfillSensorEarliestDate:
    ...
    @patch("insights.dags.events_backfill_to_duckling.resolve_team_earliest_event_date")
    @patch("insights.dags.events_backfill_to_duckling.list_enabled_backfill_team_memberships")
    @patch("insights.dags.events_backfill_to_duckling.timezone")
    ...
    def _bf(team_id: int, earliest: date | None = None) -> ManagedWarehouseTeamMembership:
        ...
        return ManagedWarehouseTeamMembership(
            ...
            table_names=ManagedWarehouseTableNames(
                ...
                data_imports_schema=f"insights_data_imports_{schema_name}",
            ),
    ...
    def _run_full_sensor(self, backfills, *, now, get_earliest, existing=None, get_runs=None):
        ...
        with (
            ...
            patch("insights.dags.events_backfill_to_duckling.list_enabled_backfill_team_memberships") as mock_list_rows,
            ...
            patch("insights.dags.events_backfill_to_duckling.resolve_team_earliest_event_date") as mock_ge,
        ):
...
class TestFullBackfillSensorCpEnumeration:
    ...
    def _run_cp_sensor(self, cp_rows, *, now, get_earliest=None):
        ...
        with (
            patch(
                "insights.dags.events_backfill_to_duckling.list_enabled_backfill_team_memberships",
            ...
            patch("insights.dags.events_backfill_to_duckling.stale_running_partitions", return_value=[]),
            patch(
                "insights.dags.events_backfill_to_duckling.resolve_team_earliest_event_date",
                return_value=(
                    get_earliest.date()
                    if isinstance(get_earliest, datetime)
                    else NO_HISTORY_SENTINEL
