...
In case some steps here have fallen out of date, please tell us about it – feel free to [submit a patch](https://github.com/hanzoai/hanzo.ai/blob/main/contents/handbook/engineering/developing-locally.md)!
...
```bash
git clone --filter=blob:none https://github.com/hanzoai/insights && cd insights/
```
...
The Insights repository includes [VS Code launch options for debugging](https://github.com/hanzoai/insights/blob/main/.vscode/launch.json). Simply go to the `Run and Debug` tab in VS Code, select the desired service you want to debug, and run it. Once it starts up, you can set breakpoints and step through code to see exactly what is happening. There are also debug launch options for frontend and backend tests if you're dealing with a tricky test failure.
...
If you're a Insights employee, you can get access to paid features on your local instance to make development easier. [Learn how to do so in our internal billing guide](https://github.com/hanzoai/billing?tab=readme-ov-file#licensing-your-local-instance).
