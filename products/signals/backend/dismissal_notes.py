...
from insights.permissions import get_authenticator_scoped_team_ids
...
def _target_skill_names(team_id: int, report_ids: list[str]) -> dict[str, str]:
    ...
    Datastore, but that read lags emit and can fail, and this runs on the dismissal request path,
