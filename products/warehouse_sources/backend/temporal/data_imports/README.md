...
More info on what partitioning options and the different modes can be found here: [https://github.com/Insights/insights/blob/master/products/warehouse_sources/backend/temporal/data_imports/sources/README.md#partitioning](https://github.com/Insights/insights/blob/master/products/warehouse_sources/backend/temporal/data_imports/sources/README.md#partitioning)
...
Observability: `warehouse_repartition_flagged` / `started` / `completed` / `failed` / `skipped` Insights events (with full team/schema/source/table context, before→after scheme, sizes, durations, and trigger reason) plus `DELTA_REPARTITION_*` Prometheus metrics.
...
For SQL sources (Postgres, Redshift, Datastore) the pipeline auto-sizes how many rows it reads per fetch by sampling the p95 row size and targeting ~150 MB per chunk.
...
To connect to a pod, follow this runbook: [https://runbooks.hanzo.ai/EKS/access](https://runbooks.hanzo.ai/EKS/access)
