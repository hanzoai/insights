...
import { parsePersonFromInsightsQLRow } from 'scenes/persons/person-utils'
...
import type { LivePersonDrillDownSelection } from './livePersonDrillDownDrawerLogic'
...
export interface livePersonDrillDownLogicValues {
    currentSelection: LivePersonDrillDownSelection | null // livePersonDrillDownDrawerLogic
    eventsVersion: number // liveWebAnalyticsMetricsLogic
    geoVersion: number // liveWebAnalyticsMetricsLogic
    slidingWindow: LiveMetricsSlidingWindow // liveWebAnalyticsMetricsLogic
    currentTeam: TeamPublicType | TeamType | null // teamLogic
    anonymousCount: number
    currentDistinctIds: string[]
    displayedDistinctIds: string[]
    identifiedCount: number
    identifiedDistinctIds: string[]
    isTruncated: boolean
    ...
    recordingCountByPersonKey: Record<string, number>
    recordingCountsByDistinctId: Record<string, number>
    recordingCountsByDistinctIdLoading: boolean
    totalCount: number
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface livePersonDrillDownLogicActions {
    loadPersons: ({ distinctIds }: { distinctIds: string[] }) => {
        distinctIds: string[]
    }
    loadPersonsFailure: (
        error: string,
    ...
    loadRecordingCounts: ({ distinctIds }: { distinctIds: string[] }) => {
        distinctIds: string[]
    }
    loadRecordingCountsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface livePersonDrillDownLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        ...
        partitionedDistinctIds: (currentDistinctIds: string[]) => {
            anonymous: string[]
            identified: string[]
        }
        identifiedDistinctIds: (partitionedDistinctIds: { anonymous: string[]; identified: string[] }) => string[]
        anonymousCount: (partitionedDistinctIds: { anonymous: string[]; identified: string[] }) => number
        totalCount: (currentDistinctIds: string[]) => number
        identifiedCount: (identifiedDistinctIds: string[]) => number
        newVisitorCount: (identifiedDistinctIds: string[], displayedDistinctIds: string[]) => number
        isTruncated: (identifiedCount: number) => boolean
...
export type livePersonDrillDownLogicType = MakeLogicType<
    livePersonDrillDownLogicValues,
    livePersonDrillDownLogicActions,
    LivePersonDrillDownLogicProps,
    livePersonDrillDownLogicMeta
>
        ...
        persons: [
            [] as PersonType[],
            {
                loadPersons: async ({ distinctIds }: { distinctIds: string[] }) => {
                    ...
                    return (response.results ?? []).map(parsePersonFromInsightsQLRow)
                },
            },
        ],
