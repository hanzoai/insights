...
paths exercise live. Yardstick = the old Review 10 findings
(`../2026-07-reviewer-topology/fixtures/old_review_report.md`).
...
2. **`reviewer/sandbox/direct_llm.py` → `run_oneshot_review(...)`**: one Messages call through the LLM
   gateway (`get_async_anthropic_gateway_client(product="review")`), adaptive thinking +
...
3. **Branches**: `split_chunks_activity` (additions ≤ gate → one-shot, else sandbox unchanged) and
   ...
   both paths.
4. **Gateway product registration** (permanent parity plumbing): `review` added to
   `insights/llm/gateway_client.py`'s `Product` literal and
...
1. **Mechanics:** run timeline shows NO `sandbox_prompt:chunking` / `sandbox_prompt:dedup` tasks; the
   ...
   `ai_product=review` + `ai_stage` stamps (silent-fallback guard now applies to one-shot calls too).
...
1. Pre-flight (below). 2. `RUN_START_EPOCH=$(date +%s)` then
   `flox activate -- bash -c "SANDBOX_PROVIDER=MODAL_DOCKER DJANGO_SETTINGS_MODULE=insights.settings python manage.py run_review --pr-url https://github.com/Insights/insights/pull/62096 --team-id 1 --user-id 1"`
   (NO `--publish`).
2. Dump: `LABEL=ONESHOT-<n> RUN_SECONDS=<s> RUN_START_EPOCH=<epoch> OUT_DIR=products/review_hog/eval/experiments/2026-07-oneshot-chunking-dedup/runs flox activate -- bash -c "DJANGO_SETTINGS_MODULE=insights.settings python manage.py shell -c \"exec(open('products/review_hog/eval/scripts/dump_result.py').read())\""`
...
6. `flox activate -- bash -c "DJANGO_SETTINGS_MODULE=insights.settings python manage.py reset_review --yes"` (dump BEFORE reset).
...
- **LLM gateway up on :3308 with the `review` product loaded** (uvicorn --reload picks up config
  ...
  ai_product=review, ai_stage stamped (593 in / 20 out).
- **Env gotcha (agent-driven shells only):** the Insights Desktop harness overrides
...
- **`review` registered as a gateway product** (user, 2026-07-03) — permanent parity plumbing.
