...
Insights's Grafana instances are reachable directly over Tailscale at plain HTTP hostnames
...
- **Tailscale** connected, with access to the Insights tailnet (hostnames `grafana-prod-us`, `grafana-prod-eu`, `grafana-dev` must resolve)
...
export PATH="$HOME/dev/insights/insights/tools/infra-scripts/mcp:$PATH"
```
...
```json
{
  "mcpServers": {
    "grafana": {
      "command": "/Users/YOUR_USERNAME/dev/insights/insights/tools/infra-scripts/mcp/mcp-grafana-wrapper.sh",
      "args": []
    }
  }
}
...
The Tailscale hostnames (`grafana-prod-us`, `grafana-prod-eu`, `grafana-dev`) only resolve when Tailscale is connected to the Insights tailnet. Check with:

```bash
tailscale status
```

If you're not connected, run `tailscale up` (or open the Tailscale app) and ensure you're logged into the Insights tailnet.
