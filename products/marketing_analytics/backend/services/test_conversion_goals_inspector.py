...
class TestCountDwGoalSafety:
    ...
    async def test_rejects_invalid_identifiers_without_executing(self, table_name, timestamp_field):
        ...
        with patch(
            "products.marketing_analytics.backend.services.conversion_goals_inspector.execute_insightsql_query",
        ) as p_exec:
    ...
    async def test_casts_the_timestamp_column_before_comparing(self):
        ...
        # several DLT sources do it), and Datastore refuses `String >= DateTime64`.
        ...
        # Both sides, or Datastore still has two types to reconcile.
...
class TestExplainGoalBasicCountDatastore(DatastoreTestMixin, BaseTest):
...
class TestExplainGoalUtmSplitDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestExplainGoalUnmatchedUtmDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestExplainGoalSampleCapDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestListGoalCountsDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestListGoalIntegratedSplitDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestListGoalNoGoalsDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestListActionGoalFiltersDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
