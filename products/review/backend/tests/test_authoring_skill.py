...
from products.review.backend.reviewer.skill_loader import REVIEW_FN_AUTHORING_SKILL_NAME
...
def test_discover_finds_the_authoring_skill() -> None:
    ...
    assert {s.name for s in discover_canonical_authoring()} == {REVIEW_FN_AUTHORING_SKILL_NAME}
...
class TestAuthoringSkillSync(BaseTest):
    def test_run_sync_seeds_the_authoring_skill(self) -> None:
        ...
        row = LLMSkill.objects.get(team=self.team, name=REVIEW_FN_AUTHORING_SKILL_NAME, is_latest=True)
