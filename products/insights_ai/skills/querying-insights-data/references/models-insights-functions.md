...
## Script Function (`system.insights_functions`)

Script functions are programmable event handlers in Insights's CDP (Customer Data Platform). They process events in real time to send data to external destinations, transform events during ingestion, or run site-side apps.
...
- `internal_destination` — Insights internal processing (e.g. triggering workflows)
- `source_webhook` — receives inbound webhooks and converts them to Insights events
...
- `transformation` — modifies events during ingestion before they reach Datastore
...
FROM system.insights_functions
