"""App Home tab + edit modal renderers for the Insights Slack app.
...
from products.slack_app.backend.services.slack_user_oauth import build_invite_url, find_linked_insights_user
...
@dataclass(frozen=True)
class ProjectChoice:
    """One Insights project the user can route their @Insights mentions to."""
...
def render_home_view(
    *,
    ...
    # started via @Insights mentions, so they can see status without
...
def _header_blocks() -> list[dict]:
    return [
        _section_title(
            "Welcome to Insights! 👋",
            "Tune how @Insights mentions get routed and answered from this Slack workspace.",
        ),
    ]
...
def _active_model_blocks(effective: AIPreferences, source: PreferenceSource) -> list[dict]:
    ...
    header = _section_title(
        "🤖 AI model",
        "Which Claude / Codex configuration handles your @Insights mentions.",
    )
...
def _project_section_blocks(state: ProjectState, *, is_admin: bool) -> list[dict]:
    ...
    blocks: list[dict] = [
        _section_title(
            "🧭 Project routing",
            "Which Insights project @Insights mentions land in. Personal picks override the workspace default.",
        ),
    ]
...
def _account_section_blocks(account_state: AccountState) -> list[dict]:
    ...
    if account_state.linked_email:
        return [
            _section_title("🔗 Linked Insights account"),
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                ...
                "elements": [
                    {
                        ...
                        "confirm": {
                            "title": {"type": "plain_text", "text": "Disconnect your Insights account?"},
                            "text": {
                                "type": "mrkdwn",
                                "text": "@Insights will fall back to matching your Slack email against Insights users until you link again.",
                            },
    ...
    blocks: list[dict] = [
        _section_title(
            "🔗 Connect your Insights account",
            "Link your Slack identity to a Insights user so @Insights knows it's you without falling back to email matching.",
        ),
    ]
                ...
                "elements": [
                    {
                        ...
                        "text": {"type": "plain_text", "text": "Connect to Insights", "emoji": True},
                        "style": "primary",
                    }
                ],
            }
...
def _workspace_section_blocks(
    ...
    *,
    ...
    summary = (
        ...
        if has_default
        else "_No workspace default set — falls back to Insights's system default._"
    )
    blocks: list[dict] = [
...
def _tasks_section_blocks(state: TasksState) -> list[dict]:
    ...
    """
    blocks: list[dict] = [_section_title("🦔 Tasks", "Tasks you started by mentioning @Insights.")]
    ...
    if not state.items:
        empty_text = (
            ...
            if state.has_any_tasks
            else "Mention @Insights in any channel to start a task."
        )
...
def _task_item_block(item: TaskItem) -> list[dict]:
    ...
    title_line = f"*<{item.insights_url}|{item.title}>*" if item.insights_url else f"*{item.title}*"
...
def _stats_section_blocks(state: StatsState) -> list[dict]:
    ...
    blocks: list[dict] = [
        _section_title(
            "📊 Workspace activity",
            "What your team shipped with @Insights. Only workspace admins see this.",
        ),
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": (
                        "Pick the runtime and model that should handle Insights Slack requests for you."
                        if scope == "personal"
...
def handle_app_home_view_submission(payload: dict) -> HttpResponse | JsonResponse:
    ...
    integration = _get_slack_integration(slack_team_id)
    if integration is None:
        return _modal_error_response("This Slack workspace is no longer connected to Insights.")
...
# already moved past in the Insights UI.
...
def _resolve_tasks_state(
    ...
    *,
    ...
    """List tasks the calling Slack user started via @Insights mentions.
    ...
    for task_id in task_ids_ordered:
        ...
        all_items.append(
            TaskItem(
                title=t.title,
                insights_url=f"{site_url}/project/{t.team_id}/tasks/{t.id}",
...
def _workspace_integrations(slack_team_id: str) -> list[Integration]:
    """Every Insights project this Slack workspace is connected to."""
    return list(
...
def _accessible_integrations(integration: Integration, slack_user_id: str) -> list[Integration]:
    """The workspace's projects this Slack user can actually reach in Insights.
...
def _resolve_stats_state(
    ...
    *,
    ...
    says nothing about Insights org membership, so the card must never widen what its
...
def _post_ephemeral_admin_only(slack: SlackIntegration, payload: dict) -> None:
    ...
    if not slack_user_id:
        return
    text = "Only Slack workspace admins can change the Insights workspace default."
