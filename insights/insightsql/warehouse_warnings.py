...
Every InsightsQL execution that resolves a warehouse table writes warnings into the active accumulator
...
InsightsQL query, without each runner having to copy them through by hand.
...
Pattern mirrors ``insights.datastore.query_tagging.tags_context`` — same ContextVar lifecycle, same
...
if TYPE_CHECKING:
    from insights.schema import DataWarehouseSyncWarning
