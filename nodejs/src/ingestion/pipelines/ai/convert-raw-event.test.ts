...
describe('convertRawEvent', () => {
    it('maps OTel attributes to Insights properties when source is otel', () => {
    ...
    describe('debug mode', () => {
        ...
        it.each([
            ...
            ['empty string', { $ai_ingestion_source: 'otel', 'insights.ai.debug': '', 'gen_ai.request.model': 'gpt-4' }],
            ['zero', { $ai_ingestion_source: 'otel', 'insights.ai.debug': 0, 'gen_ai.request.model': 'gpt-4' }],
            ['false', { $ai_ingestion_source: 'otel', 'insights.ai.debug': false, 'gen_ai.request.model': 'gpt-4' }],
        ])('does not set debug properties when insights.ai.debug is %s', (_label, properties) => {
            const event = createEvent(properties)
            convertRawEvent(event)
