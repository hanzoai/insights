...
import { castTimestampOrNow, castTimestampToDatastoreFormat } from '~/common/utils/utils'
...
export function serializeEvent(event: ProcessedEvent): RawKafkaEvent {
    return {
        uuid: event.uuid,
        event: safeDatastoreString(event.event),
        properties: JSON.stringify(event.properties ?? {}),
        timestamp: castTimestampOrNow(event.timestamp, TimestampFormat.Datastore),
        team_id: event.team_id,
        project_id: event.project_id,
        distinct_id: safeDatastoreString(event.distinct_id),
        elements_chain: safeDatastoreString(event.elements_chain),
        created_at: castTimestampOrNow(null, TimestampFormat.Datastore),
        captured_at:
            event.captured_at !== null
                ? castTimestampToDatastoreFormat(DateTime.fromJSDate(event.captured_at), TimestampFormat.Datastore)
                : null,
        person_id: event.person_id,
        person_properties: JSON.stringify(event.person_properties ?? {}),
        person_created_at: castTimestampOrNow(event.person_created_at, TimestampFormat.DatastoreSecondPrecision),
