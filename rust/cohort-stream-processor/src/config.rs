...
const POOL_NAME: &str = "insights_cohort";

/// The smallest person-record TTL that stays safely beyond the replay horizon of the merge and
/// `datastore_events_json` topics (~7 days): a record reclaimed while its events can still replay
...
pub struct Config {
    ...
    /// DSN for the main Insights database that owns `insights_cohort`.
    ...
    // `datastore_events_json` = 7d). If any of those `retention.ms` values change, these knobs
    ...
    /// older than `now − this`. Default 9d = `datastore_events_json` 7d + `cohort_stream_events`
    ...
    /// overwrite live-evaluated state (ms). Covers Datastore replication lag plus modest client
    ...
    ///
    /// In production this MUST be well beyond the replay horizon of the merge / `datastore_events_json`
...
mod tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            database_url: "postgres://insights:insights@localhost:5432/insights".to_string(),
