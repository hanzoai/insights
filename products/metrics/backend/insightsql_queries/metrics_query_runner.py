...
-> `run_metric_query` -> schema `MetricsQuerySeries`. The single-clause InsightsQL
...
from insights.permissions import insights_feature_flag_enabled
...
class MetricsQueryRunner(AnalyticsQueryRunner[MetricsQueryResponse]):
    ...
    def validate_query_runner_access(self, user: "User") -> bool:
        ...
        if not insights_feature_flag_enabled(
            METRICS_FEATURE_FLAG,
    ...
    def to_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        raise NotImplementedError(
            "MetricsQuery composes one InsightsQL query per clause via the metrics facade; there is no single statement"
        )
    ...
    def apply_dashboard_filters(self, dashboard_filter: DashboardFilter) -> None:
        # Metric label predicates are not Insights property filters, so only the
