...
class ChannelsAPITestCase(TestCase):
    ...
    @patch("insights.models.integration.GitHubIntegration.list_all_cached_repositories")
    def test_cannot_configure_someone_elses_personal_channel_repositories(self, list_repositories):
        list_repositories.return_value = [{"full_name": "insights/insights"}]
        ...
        sneaky = other_client.patch(
            ...
            {"github_integration": integration.id, "repositories": ["insights/insights"]},
            format="json",
        )
        ...
        self.assertEqual(owned.json()["repositories"], ["insights/insights"])
    ...
    def test_new_tasks_inherit_channel_repositories(self, list_repositories):
        list_repositories.return_value = [
            ...
            {"full_name": "insights/insights-js"},
        ]
        integration = Integration.objects.create(team=self.team, kind="github", integration_id="1", config={})
        channel_id = self.client.post(self._channels_url(), {"name": "growth"}).json()["id"]
        ...
        self.assertEqual(created.status_code, status.HTTP_201_CREATED, created.content)
        self.assertEqual(created.json()["repositories"], ["insights/insights", "insights/insights-js"])
...
class TaskMentionsAPITestCase(ChannelTaskAPITestCase):
    ...
    @patch("products.tasks.backend.push_dispatcher.hanzo_insights.feature_enabled", return_value=True)
