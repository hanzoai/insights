 ...
 * Context: https://github.com/Insights/insights/pull/49891
 *
...
export function captureRapidUrlChangeWarning(
    ...
    const sessionReplayUrl = insights.get_session_replay_url?.({
        withTimestamp: true,
    })

    // Console warning for local debugging
    // eslint-disable-next-line no-console
    console.warn('[Insights] Rapid URL changes detected - possible infinite loop', {
        currentUrl,
        logicPath,
        actionName,
        ...debugInfo,
    ...
    // Capture exception to Insights for monitoring
    ...
    if (containsSerializationBug(currentUrl)) {
        insights.captureException(new Error('URL contains [object Object] - serialization bug'), {
    ...
    insights.capture('kea_router_rapid_url_changes', {
        url: currentUrl,
        logic_path: logicPath,
        action_name: actionName,
        change_count: debugInfo.changeCount,
...
export function trackUrlChange(response: ActionToUrlResponse, logicPath: string, actionName: string): void {
    ...
    try {
        ...
        if (containsSerializationBug(urlString)) {
            // eslint-disable-next-line no-console
            console.error('[Insights] Invalid URL detected - contains [object Object]', {
                url: urlString,
                action: actionName,
                logic: logicPath,
            })
