...
def enrich_table_semantics_sync(team_id: int, schema_id: uuid.UUID) -> dict[str, Any]:
    ...
    # keyed by the raw source field name (`created`, `customer`); re-key them to the InsightsQL-visible name
    ...
    insightsql_name_by_raw = get_insightsql_column_name_mapping(table.table_name_without_prefix())
    canonical = get_canonical_descriptions_for_source(schema.source.source_type).get(schema.name, {})
    canonical_columns = {
        insightsql_name_by_raw.get(raw_name, raw_name): description
...
class EnrichTableSemanticsWorkflow(InsightsWorkflow):
    @staticmethod
