...
export function seatPriceFromPlanKey(planKey: string): number {
    ...
    const match = planKey.match(/insights-code-pro-(\d+)/)
            ...
            [] as SeatData[],
            {
                loadOrgSeats: async (): Promise<SeatData[]> => {
                    try {
                        ...
                        toast.error(seatErrorMessage(e, 'Failed to load organization seats'))
                        return []
                    }
                },
            },
    ...
    listeners(({ actions }) => ({
        adminCancelSeat: async ({ userDistinctId }) => {
            try {
                ...
                toast.error(seatErrorMessage(e, 'Failed to cancel seat'))
            }
        },
    })),
