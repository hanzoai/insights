...
because no hanzo.ai checkout was available when the source was implemented — move it there and
...
products, invoices, the chart of accounts, and more — into Insights so you can join financial data with
...
Insights sends these as the `X-AppSecretToken` and `X-AgreementGrantToken` headers on every request.
...
- **Rate limiting (429)** — e-conomic throttles heavy traffic. Insights retries with backoff
