...
def test_discover_finds_the_blind_spots_skill() -> None:
    ...
    assert {s.name for s in discover_canonical_blind_spots()} == {REVIEW_FN_BLIND_SPOTS_SKILL_NAME}
...
class TestLoadBlindSpotsSkillForRun(BaseTest):
    ...
    def test_cold_user_gets_the_canonical_pinned(self) -> None:
        ...
        assert loaded.skill_name == REVIEW_FN_BLIND_SPOTS_SKILL_NAME
    ...
    def test_resolves_the_selected_custom_skill(self) -> None:
        ...
        configs.filter(user_id=self.user.id, skill_name=REVIEW_FN_BLIND_SPOTS_SKILL_NAME).update(enabled=False)
    ...
    def test_falls_back_to_canonical_when_the_selected_skill_row_is_missing(self) -> None:
        ...
        configs.filter(user_id=self.user.id, skill_name=REVIEW_FN_BLIND_SPOTS_SKILL_NAME).update(enabled=False)
        configs.create(
            team_id=self.team.id, user_id=self.user.id, skill_name=f"{REVIEW_FN_BLIND_SPOTS_PREFIX}ghost", enabled=True
        )
...
class TestColdStartSyncSeedsBlindSpots(BaseTest):
    def test_cold_start_sync_seeds_the_blind_spots_skill(self) -> None:
        ...
        assert LLMSkill.objects.filter(team=self.team, name=REVIEW_FN_BLIND_SPOTS_SKILL_NAME, is_latest=True).exists()
