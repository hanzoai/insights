...
PI_IMAGE_NAME = "insights-sandbox-pi"
STREAMLIT_IMAGE_NAME = "insights-sandbox-streamlit"
SLIM_IMAGE_NAME = "insights-sandbox-slim"

# Stamped on the base image so a later run can tell whether it must rebuild: the sha of
# the Dockerfile that produced it, and the @hanzo/agent version baked into the npm layer.
_DOCKERFILE_SHA_LABEL = "com.insights.sandbox.dockerfile-sha"
_AGENT_VERSION_LABEL = "com.insights.sandbox.agent-version"
...
# Env vars holding a Insights URL the sandbox must reach from inside the container.
...
class DockerSandbox(SandboxBase):
    ...
    def _build_local_image(monorepo_root: str) -> None:
        logger.info("Building insights-sandbox-base-local image with local Insights Desktop packages...")
    ...
    def _ensure_image_exists(template: SandboxTemplate) -> str:
        ...
        local_monorepo_root = DockerSandbox._get_local_insights_code_root()
    ...
    def _get_image(config: SandboxConfig) -> str:
        ...
        if config.snapshot_external_id:
            snapshot_image = f"insights-sandbox-snapshot:{config.snapshot_external_id}"
    ...
    def _verify_image_available(image: str, config: SandboxConfig) -> None:
        ...
        Sandbox images (insights-sandbox-base and friends) are built on the host, never
            ...
            if local_skills_host and os.path.isdir(local_skills_host):
                for entry in sorted(os.listdir(local_skills_host)):
                    ...
                    if not os.path.isdir(host_skill):
                        continue
                    container_skill = f"/scripts/plugins/insights/skills/{entry}"
    ...
    def _build_agent_server_command(
        ...
        exec_permission_flag = (
            f" --insightsExecPermissionRegex {shlex.quote(insights_exec_permission_regex)}"
            if insights_exec_permission_regex
            else ""
        )
    ...
    def start_agent_server(
        ...
        command = self._build_agent_server_command(
            ...
            rtk_enabled=rtk_enabled,
            insights_exec_permission_regex=exec_permission_regex,
        )
...
def _resolve_latest_agent_version() -> str | None:
    """Latest published @hanzo/agent version, or ``None`` if npm is unavailable.
...
def ensure_fresh_base_image(*, force: bool = False) -> None:
    """Rebuild ``insights-sandbox-base`` when it is stale, otherwise reuse it.

    Stale means any of: ``force``; the image is missing; the Dockerfile changed since the
    image was built; or @hanzo/agent published a newer version than the one baked in.
    ...
    if reason is None:
        if latest is None:
            ...
            logger.warning(
                "could not check @hanzo/agent freshness (npm unreachable); reusing existing insights-sandbox-base"
            )
        else:
            logger.info("insights-sandbox-base is up to date (agent %s); reusing existing image", latest)
        return
    ...
    logger.info("Rebuilding insights-sandbox-base: %s", reason)
    DockerSandbox._build_image_if_needed(
        DEFAULT_IMAGE_NAME,
        dockerfile_path,
