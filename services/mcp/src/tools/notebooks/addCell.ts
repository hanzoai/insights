...
export const NotebooksAddCellSchema = z
    .object({
        ...
        cell_type: z
            ...
            .describe(
                "Cell kind: 'sql' (InsightsQL against Insights data) and 'python' run immediately; 'markdown' inserts prose (headings, tables, code/mermaid fences); 'saved_insight' embeds an existing insight; 'component' inserts any other notebook component tag (charts, media, Insights entities)."
            ),
        ...
        props: z
            ...
            .optional()
            .describe(
                'Component cells: the props for the tag, matching what the notebook UI stores for that component. For Query: {"query": {"kind": "InsightVizNode", "source": <TrendsQuery|FunnelsQuery|RetentionQuery|PathsQuery|StickinessQuery|LifecycleQuery>}} for insights, or {"query": {"kind": "DataTableNode", "source": {"kind": "EventsQuery", …}}} for event tables. InsightsQLQuery sources are not accepted here — use cell_type sql, which charts its result too.'
            ),
        dataframe_name: z
            .string()
...
/**
 * A `<Query>` whose source is InsightsQL is the legacy SQL cell. It renders a result table or chart but
 ...
 */
function hasInsightsQLQuerySource(props: Record<string, unknown> | undefined): boolean {
    ...
    return !!source && typeof source === 'object' && (source as { kind?: unknown }).kind === 'InsightsQLQuery'
}
...
export const addCellHandler: ToolBase<typeof NotebooksAddCellSchema, AddCellResult>['handler'] = async (
    ...
    if (params.cell_type === 'component') {
        ...
        if (['SQLV2', 'PythonV2', 'Python', 'DuckSQL', 'InsightsQLSQL'].includes(params.tag_name)) {
            throw new Error(
                `Use cell_type 'sql' or 'python' for executable cells instead of tag_name ${params.tag_name}.`
            )
        }
        if (hasInsightsQLQuerySource(params.props)) {
            throw new Error(
                "Use cell_type 'sql' for SQL instead of a component with a InsightsQLQuery source. A sql cell runs the query, names a dataframe other cells can reference, and charts its result."
            )
        }
    }
    ...
    const run = await runAndWriteBack(
        ...
        params.notebook_id,
        nodeId,
        params.cell_type === 'sql' ? 'insightsql' : 'python',
        params.code!,
        dataframeName,
