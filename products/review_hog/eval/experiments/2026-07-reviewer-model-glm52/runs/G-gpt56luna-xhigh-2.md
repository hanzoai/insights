...
- **chunk 1** (8 files): products/review_hog/backend/models.py, products/review_hog/backend/migrations/0019_reviewusersettings_stamp_review_inbox_prs.py, products/review_hog/backend/api/settings.py, products/review_hog/backend/receivers.py, products/review_hog/frontend/CodeReviewScene.tsx, products/review_hog/frontend/generated/api.schemas.ts, products/review_hog/frontend/generated/api.zod.ts, services/mcp/src/api/generated.ts
...
- **Validator:** - **Checked:** The real PR head (`1341596e`, fetched — the working tree was on base `d24a844e` and lacked the changes) for `products/review_hog/backend/receivers.py`, its tests (`tests/test_inbox_trigger.py`), the stamp webhook consumer (`products/stamp/backend/tasks/tasks.py`), and the inversion hook (`facade/inbox_hooks.py`).
...
- **Validator:** - **Checked:** The real PR head (`1341596e`) `_start_stamp_review` (`products/review_hog/backend/receivers.py`), the facade `queue_inbox_pr_review` (`facade/api.py:128` — a bare `process_inbox_pr_review.delay()`), and the Stamp webhook/task path (`products/stamp/backend/tasks/tasks.py`).
...
### [✅ VALID] must_fix · bug — products/review_hog/backend/migrations/0019_reviewusersettings_stamp_review_inbox_prs.py:1-17
...
- **Found:** `queue_inbox_pr_review` is a real function defined by this PR in `products/stamp/backend/facade/api.py` (`def queue_inbox_pr_review(*, team_id, pr_url, acting_user_id, signal_report_id, task_run_id)`), which delegates to the `process_inbox_pr_review` Celery task. The inbox receiver `_start_stamp_review` in `products/review_hog/backend/receivers.py` imports it (`from products.stamp.backend.facade.api import queue_inbox_pr_review`) and calls `queue_inbox_pr_review(...)`. The README's `queue_inbox_pr_review` reference is therefore accurate and traceable.
- **Found:** The finding conflates two distinct legs. `start_review_pr_workflow` is the Review leg (`_START = "products.review_hog.backend.temporal.client.start_review_pr_workflow"`, used by `_start_review`); the Stamp inbox leg is a separate path that goes through `queue_inbox_pr_review` (`_STAMPFN_QUEUE = "products.stamp.backend.facade.api.queue_inbox_pr_review"`, used by `_start_stamp_review`). They are not substitutes for each other. Tests in `test_inbox_trigger.py` patch each target independently.
...
- **Validator:** - **Checked:** the full reachability chain into `process_inbox_pr_review` — its sole trigger `handle_task_run_saved` → `_start_stamp_review` → `queue_inbox_pr_review` (products/review_hog/backend/receivers.py:98-139), every writer of `output.pr_url` in tasks (products/tasks/backend/webhooks.py:184-212), the repo-config gate inside the task (products/stamp/backend/tasks/tasks.py:1138-1153), and the engine's self-driving flag wiring (products/stamp/backend/temporal/activities.py:237-238,448-451).
