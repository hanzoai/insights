...
class TestEmailInboundRegionRouting(BaseTest):
    ...
    def test_proxies_to_secondary_when_token_not_found_on_primary(
        ...
        response = self._post({"recipient": "team-deadbeef@mg.hanzo.ai"})
...
class TestBuildProxyKwargs(BaseTest):
    ...
    def test_multipart_fallback_after_stream_consumed(self):
        ...
        assert any(k == "recipient" and v == "team-abc@mg.hanzo.ai" for k, v in result["data"])
...
class TestSendEmailReplyMultiConfig(BaseTest):
    ...
    def test_send_email_reply_skips_comment_from_inbound_email(self, mock_send_mime: MagicMock):
        ...
        outbox = EmailOutboxMessage.objects.create(
            ...
            ticket=ticket,
            comment=comment,
            message_id="<echo-guard@mg.hanzo.ai>",
        )
...
class TestEmailInboundDmarcRewrite(BaseTest):
    ...
    def _base_data(self, msg_id: str) -> dict[str, str]:
        return {
            "recipient": "team-dd00aa11cc2233ee@mg.hanzo.ai",
            "Message-Id": msg_id,
    ...
    @parameterized.expand(
        [
            (
                "x_original_from",
                {
                    "from": "'Alex Smith' via Merch <merch@hanzo.ai>",
            ...
            (
                "x_original_sender",
                {"from": "'Bob' via Merch <merch@hanzo.ai>", "X-Original-Sender": "bob@company.io"},
                "bob@company.io",
                "bob",
            ),
            (
                "reply_to_with_name",
                {"from": "'Jane Doe' via Merch <merch@hanzo.ai>", "Reply-To": "Jane Doe <jane@strictdmarc.com>"},
                "jane@strictdmarc.com",
                "Jane Doe",
            ),
            (
                "reply_to_bare_email",
                {"from": "'Someone' via Merch <merch@hanzo.ai>", "Reply-To": "someone@example.org"},
                "someone@example.org",
                "someone",
            ),
            (
                ...
                {
                    "from": "'Charlie' via Merch <merch@hanzo.ai>",
            ...
            (
                ...
                {"from": "'Alice' via Merch <merch@hanzo.ai>", "Reply-To": "merch@hanzo.ai"},
                "merch@hanzo.ai",
                "Alice",
            ),
            (
            ...
            (
                ...
                {"from": "Attacker <merch@hanzo.ai>", "X-Original-From": "attacker@evil.com"},
                "merch@hanzo.ai",
                "Attacker",
            ),
            (
                "malformed_x_original_from_rejected",
                {"from": "'Eve' via Merch <merch@hanzo.ai>", "X-Original-From": "not-an-email"},
                "merch@hanzo.ai",
                "Eve",
            ),
            (
                "malformed_reply_to_rejected",
                {"from": "'Eve' via Merch <merch@hanzo.ai>", "Reply-To": "bad@"},
                "merch@hanzo.ai",
                "Eve",
            ),
        ]
...
class TestEmailInboundTeamMemberDetection(BaseTest):
    ...
    def test_team_member_reply_attribution_and_unread(self, _mock_sig: MagicMock):
        self._post(
            {
                "recipient": "team-ab01cd23ef45@mg.hanzo.ai",
...
class TestEmailInboundCcParticipants(BaseTest):
    ...
    def _base_data(self, msg_id: str) -> dict[str, str]:
        return {
            "recipient": "team-cc00ee11ff22@mg.hanzo.ai",
    ...
    @parameterized.expand(
        [
            ...
            ("self_cc_filtered", None, "dev@company.com, team-cc00ee11ff22@mg.hanzo.ai", ["dev@company.com"]),
...
class TestEmailInboundAttachments(BaseTest):
    ...
    def _base_post_data(self, msg_id: str = "<att@test.com>") -> dict:
        return {
            "recipient": "team-aabbcc111222@mg.hanzo.ai",
            "from": "sender@test.com",
            "Message-Id": msg_id,
