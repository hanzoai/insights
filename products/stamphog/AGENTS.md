...
- `dismiss_stale_approvals` also runs a GitHub-side belt-and-braces sweep after the DB sweep: it
  ...
  do NOTHING when `STAMPFN_GITHUB_APP_SLUG` is unset — a fuzzy "any Bot" match must never dismiss or
...
- Egress is an explicit domain allowlist (`_sandbox_egress_allowlist`). Additions go through
  `STAMPFN_SANDBOX_EXTRA_EGRESS_DOMAINS`, not code edits.
...
- Identification is **server-attested task linkage plus server-attested PR identity** — both
  ...
  auto_start generates the head branch name (`insights-self-driving/<slug>-<hex>`) before the
  ...
  by this instance's Insights GitHub App machine user (`<GITHUB_APP_SLUG>[bot]`) on a repo-native head
  ...
  insights-bot / any foreign App fail it.
