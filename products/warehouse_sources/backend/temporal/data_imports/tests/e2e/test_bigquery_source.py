...
async def test_bigquery_source_full_refresh_table(
    ...
    assert columns[bigquery_table_primary_key]["datastore"] == "Nullable(String)"
    assert columns["incremental"]["datastore"] == "Nullable(Int64)"
    assert columns["value"]["datastore"] == "Nullable(String)"
...
async def test_bigquery_source_incremental_timestamp(
    ...
    assert columns[bigquery_table_primary_key]["datastore"] == "Nullable(String)"
    assert columns["incremental"]["datastore"] == "Nullable(DateTime64(6))"
    assert columns["value"]["datastore"] == "Nullable(String)"
