...
// Imported from the source module rather than the `@hanzo/elements` barrel so that the
// spy below replaces `.error` on the same `toast` singleton that `paymentEntryLogic`
// calls at runtime. `jest.mock('@hanzo/elements', ...)` did not propagate through the
