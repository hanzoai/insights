...
are chartable in Insights — trend a `verdict` breakdown, alert on a rate — with no export
step. The events ARE the record: there is no parallel Postgres copy, so past records are
read back the way any event is (insights, InsightsQL over `events`), and retention follows the
...
# a Insights-generated event kept out of the customer's own custom-event namespace).
    ...
    with transaction.atomic():
        ...
        if not team.organization.is_ai_data_processing_approved:
            raise InvalidStructuredOutputError(
                ...
                "cannot be recorded. An org admin must turn on the 'Enable Insights features that use "
...
def _capture_recorded(*, team: Team, run: SignalScoutRun, recorded_count: int) -> None:
    """Internal (Insights-side) telemetry: one event per accepted call, mirroring the
