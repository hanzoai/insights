...
/// `source_partition` / `source_offset` carry the upstream `datastore_events_json` coordinates so
...
#[cfg(test)]
pub(crate) fn sample_datastore_event(team_id: i32, person_id: Option<&str>) -> DatastoreEvent {
...
mod tests {
    ...
    fn sample_event(team_id: i32, person_id: Option<&str>) -> DatastoreEvent {
        sample_datastore_event(team_id, person_id)
    }
    ...
    #[test]
    fn from_datastore_maps_every_field() {
        let event = sample_event(42, Some("01928aaa-bbbb-cccc-dddd-eeeeeeeeeeee"));
        // from_datastore consumes the event, so capture the uuid first.
        let expected_uuid = event.uuid.to_string();
        let envelope = CohortStreamEvent::from_datastore(
            event,
    ...
    #[test]
    fn from_datastore_preserves_null_optionals() {
        let mut event = sample_event(1, Some("p"));
        event.properties = None;
        event.person_properties = None;
        event.elements_chain = None;

        let envelope = CohortStreamEvent::from_datastore(event, "p".to_string(), 0, 0);
    ...
    fn envelope_partition_key_matches_free_function() {
        ...
        let envelope = CohortStreamEvent::from_datastore(event, "xyz".to_string(), 3, 9);
    ...
    fn serialized_envelope_has_tdd_4_3_keys() {
        ...
        let envelope = CohortStreamEvent::from_datastore(event, "p1".to_string(), 17, 12345);
