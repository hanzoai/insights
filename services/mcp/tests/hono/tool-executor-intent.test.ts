...
// Replace the project's Insights client with a real (but disabled, no-network)
// InsightsMCP so `prepareToolList` / `prepareToolCall` run the actual SDK injection
...
describe('ToolExecutor intent capture', () => {
    ...
    it.each([
        ...
        const captureSpy = vi.spyOn(getInsightsClient(), 'captureToolCall').mockImplementation(() => {})
