...
impl GroupStorage for PostgresStorage {
    ...
    async fn create_group(
        ...
        let row = sqlx::query_as!(
            Group,
            r#"
            INSERT INTO insights_group (team_id, group_type_index, group_key, group_properties, created_at, properties_last_updated_at, properties_last_operation, version)
    ...
    async fn delete_groups_batch_for_team(
        ...
        let result = sqlx::query!(
            r#"
            DELETE FROM insights_group
            WHERE id IN (
                SELECT id FROM insights_group
                WHERE team_id = $1
                LIMIT $2
    ...
    async fn update_group_type_mapping(
        ...
        let row = sqlx::query_as!(
            GroupTypeMapping,
            r#"
            UPDATE insights_grouptypemapping
    ...
    async fn delete_group_type_mapping(
        ...
        let result = sqlx::query!(
            r#"
            DELETE FROM insights_grouptypemapping
            WHERE project_id = $1 AND group_type_index = $2
            "#,
            project_id,
            group_type_index
    ...
    async fn delete_group_type_mappings_batch_for_team(
        ...
        let result = sqlx::query!(
            ...
            DELETE FROM insights_grouptypemapping
            WHERE id IN (
                SELECT id FROM insights_grouptypemapping
                WHERE team_id = $1
                LIMIT $2
                FOR UPDATE SKIP LOCKED
            )
