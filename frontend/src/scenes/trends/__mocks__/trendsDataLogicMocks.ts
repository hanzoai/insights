import { TrendAPIResponse } from '~/types'

export const trendResult: TrendAPIResponse = {
    result: [
        {
            action: {
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: null,
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$pageview',
            count: 538545.0,
            data: [87830.0, 91561.0, 104892.0, 74522.0, 26535.0, 35259.0, 91718.0, 26228.0],
            labels: [
                '28-Feb-2023',
                '1-Mar-2023',
                '2-Mar-2023',
                '3-Mar-2023',
                '4-Mar-2023',
                '5-Mar-2023',
                '6-Mar-2023',
                '7-Mar-2023',
            ],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            persons_urls: [
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-02-28T00:00:00-08:00',
                        date_to: '2023-02-28T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-02-28T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-02-28T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-01T00:00:00-08:00',
                        date_to: '2023-03-01T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-01T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-01T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-02T00:00:00-08:00',
                        date_to: '2023-03-02T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-02T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-02T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-03T00:00:00-08:00',
                        date_to: '2023-03-03T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-03T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-03T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-04T00:00:00-08:00',
                        date_to: '2023-03-04T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-04T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-04T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-05T00:00:00-08:00',
                        date_to: '2023-03-05T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-05T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-05T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-06T00:00:00-08:00',
                        date_to: '2023-03-06T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-06T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-06T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: null,
                        date_from: '2023-03-07T00:00:00-08:00',
                        date_to: '2023-03-07T23:59:59.999999-08:00',
                        entity_order: 0,
                    },
                    url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=2023-03-07T00%3A00%3A00-08%3A00&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&properties=%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24host%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22127.0.0.1%3A3000%22%2C+%22127.0.0.1%3A5000%22%2C+%22localhost%3A5000%22%2C+%22localhost%3A8000%22%2C+%22localhost%3A8001%22%2C+%22localhost%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22distinct_id%22%2C+%22operator%22%3A+%22not_icontains%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%22posthog.com%22%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22capturi%40capturi.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22email%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22person%22%2C+%22value%22%3A+%5B%22fuziontech%40gmail.com%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22realm%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%22demo%22%5D%7D%5D%7D%2C+%7B%22type%22%3A+%22AND%22%2C+%22values%22%3A+%5B%7B%22key%22%3A+%22%24ip%22%2C+%22operator%22%3A+%22is_not%22%2C+%22type%22%3A+%22event%22%2C+%22value%22%3A+%5B%2271.84.26.175%22%5D%7D%5D%7D%5D%7D&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&date_to=2023-03-07T23%3A59%3A59.999999-08%3A00&entity_order=0',
                },
            ],
            filter: {
                breakdown_attribution_type: 'first_touch',
                breakdown_normalize_url: false,
                date_from: '-7d',
                display: 'ActionsLineGraph',
                events: [
                    {
                        id: '$pageview',
                        type: 'events',
                        order: 0,
                        name: '$pageview',
                        custom_name: null,
                        math: null,
                        math_property: null,
                        math_group_type_index: null,
                        properties: {},
                    },
                ],
                insight: 'TRENDS',
                interval: 'day',
                properties: {
                    type: 'AND',
                    values: [
                        {
                            type: 'AND',
                            values: [{ key: 'email', operator: 'not_icontains', type: 'person', value: 'posthog.com' }],
                        },
                        {
                            type: 'AND',
                            values: [
                                {
                                    key: '$host',
                                    operator: 'is_not',
                                    type: 'event',
                                    value: [
                                        '127.0.0.1:3000',
                                        '127.0.0.1:5000',
                                        'localhost:5000',
                                        'localhost:8000',
                                        'localhost:8001',
                                        'localhost',
                                    ],
                                },
                            ],
                        },
                        {
                            type: 'AND',
                            values: [
                                { key: 'distinct_id', operator: 'not_icontains', type: 'event', value: 'posthog.com' },
                            ],
                        },
                        {
                            type: 'AND',
                            values: [
                                { key: 'email', operator: 'is_not', type: 'person', value: ['capturi@capturi.com'] },
                            ],
                        },
                        {
                            type: 'AND',
                            values: [
                                { key: 'email', operator: 'is_not', type: 'person', value: ['fuziontech@gmail.com'] },
                            ],
                        },
                        { type: 'AND', values: [{ key: 'realm', operator: 'is_not', type: 'event', value: ['demo'] }] },
                        {
                            type: 'AND',
                            values: [{ key: '$ip', operator: 'is_not', type: 'event', value: ['71.84.26.175'] }],
                        },
                    ],
                },
                sampling_factor: '',
                smoothing_intervals: 1,
            },
        },
    ],
    type: 'Trends',
    last_refresh: '2023-03-07T13:42:17.746798Z',
    is_cached: true,
    next: null,
} as unknown as TrendAPIResponse

// 1. Add step "Autocapture"
// 2. Add step "Feature Flag Called"
// 3. Select viz type "Pie Chart"
export const trendPieResult: TrendAPIResponse = {
    result: [
        {
            action: {
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: null,
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$pageview',
            count: 0,
            data: [],
            labels: [],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            aggregated_value: 553348,
            filter: {
                breakdown_attribution_type: 'first_touch',
                breakdown_normalize_url: false,
                date_from: '-7d',
                display: 'ActionsPie',
                events: '[{"id": "$pageview", "type": "events", "order": 0, "name": "$pageview", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}, {"id": "$autocapture", "type": "events", "order": 1, "name": "$autocapture", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}, {"id": "$feature_flag_called", "type": "events", "order": 2, "name": "$feature_flag_called", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}]',
                insight: 'TRENDS',
                interval: 'day',
                sampling_factor: '',
                smoothing_intervals: 1,
            },
            persons: {
                filter: { entity_id: '$pageview', entity_type: 'events', entity_math: null, entity_order: 0 },
                url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsPie&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%2C+%7B%22id%22%3A+%22%24autocapture%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+1%2C+%22name%22%3A+%22%24autocapture%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%2C+%7B%22id%22%3A+%22%24feature_flag_called%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+2%2C+%22name%22%3A+%22%24feature_flag_called%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&sampling_factor=&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_order=0',
            },
        },
        {
            action: {
                id: '$autocapture',
                type: 'events',
                order: 1,
                name: '$autocapture',
                custom_name: null,
                math: null,
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$autocapture',
            count: 0,
            data: [],
            labels: [],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            aggregated_value: 874570,
            filter: {
                breakdown_attribution_type: 'first_touch',
                breakdown_normalize_url: false,
                date_from: '-7d',
                display: 'ActionsPie',
                events: '[{"id": "$pageview", "type": "events", "order": 0, "name": "$pageview", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}, {"id": "$autocapture", "type": "events", "order": 1, "name": "$autocapture", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}, {"id": "$feature_flag_called", "type": "events", "order": 2, "name": "$feature_flag_called", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}]',
                insight: 'TRENDS',
                interval: 'day',
                sampling_factor: '',
                smoothing_intervals: 1,
            },
            persons: {
                filter: { entity_id: '$autocapture', entity_type: 'events', entity_math: null, entity_order: 1 },
                url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsPie&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%2C+%7B%22id%22%3A+%22%24autocapture%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+1%2C+%22name%22%3A+%22%24autocapture%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%2C+%7B%22id%22%3A+%22%24feature_flag_called%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+2%2C+%22name%22%3A+%22%24feature_flag_called%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&sampling_factor=&smoothing_intervals=1&entity_id=%24autocapture&entity_type=events&entity_order=1',
            },
        },
        {
            action: {
                id: '$feature_flag_called',
                type: 'events',
                order: 2,
                name: '$feature_flag_called',
                custom_name: null,
                math: null,
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$feature_flag_called',
            count: 0,
            data: [],
            labels: [],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            aggregated_value: 3377681,
            filter: {
                breakdown_attribution_type: 'first_touch',
                breakdown_normalize_url: false,
                date_from: '-7d',
                display: 'ActionsPie',
                events: '[{"id": "$pageview", "type": "events", "order": 0, "name": "$pageview", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}, {"id": "$autocapture", "type": "events", "order": 1, "name": "$autocapture", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}, {"id": "$feature_flag_called", "type": "events", "order": 2, "name": "$feature_flag_called", "custom_name": null, "math": null, "math_property": null, "math_group_type_index": null, "properties": {}}]',
                insight: 'TRENDS',
                interval: 'day',
                sampling_factor: '',
                smoothing_intervals: 1,
            },
            persons: {
                filter: {
                    entity_id: '$feature_flag_called',
                    entity_type: 'events',
                    entity_math: null,
                    entity_order: 2,
                },
                url: 'api/projects/2/persons/trends/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsPie&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%2C+%7B%22id%22%3A+%22%24autocapture%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+1%2C+%22name%22%3A+%22%24autocapture%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%2C+%7B%22id%22%3A+%22%24feature_flag_called%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+2%2C+%22name%22%3A+%22%24feature_flag_called%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+null%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=TRENDS&interval=day&sampling_factor=&smoothing_intervals=1&entity_id=%24feature_flag_called&entity_type=events&entity_order=2',
            },
        },
    ],
    timezone: 'US/Pacific',
    last_refresh: '2023-03-07T15:19:31.730326Z',
    is_cached: false,
    next: null,
} as unknown as TrendAPIResponse

export const breakdownPieResult: TrendAPIResponse = {
    result: [
        {
            data: [],
            days: [],
            count: 0,
            aggregated_value: 27,
            label: '["signup-page-4.0","billing-limit","kafka-inspector","person-on-events-enabled","auto-rollback-feature-flags","onboarding-v2-demo","query_running_time","query-timings","hedgehog-mode","web-analytics","web-analytics-sampling","high-frequency-batch-exports","exception-autocapture","data-warehouse","data-warehouse-postgres-import","ff-dashboard-templates","show-product-intro-existing-products","cs-dashboards","redirect-signups-to-instance","apps-and-exports-ui","insightsql-insights-preview","insightsql-insights-lifecycle","insightsql-insights-paths","insightsql-insights-retention","insightsql-insights-trends","insightsql-insights-stickiness","insightsql-insights-funnels","insightsql-insight-live-compare","insightsql-in-insight-serialization","bi_viz","webhooks-denylist","pipeline-ui","session-recording-sampling","person-feed-canvas","feature-flag-cohort-creation","insight-horizontal-controls","surveys-widgets","invite-team-member-onboarding","year-in-hog","session-replay-export-mobile-data","discussions","ai-session-summary","ai-session-permissions","product-intro-pages","session-replay-doctor","session-replay-similar-recordings","saved-not-pinned","billing-upgrade-language","new-experiments-ui","session-replay-filter-ordering","session-replay-linked-variants","session-replay-error-clustering","audit-logs-access","reverse-proxy-onboarding","session-replay-mobile-onboarding","ip-allowlist-setting","email-verification-ticket-submission","theme"]',
            filter: {
                insight: 'TRENDS',
                filter_test_accounts: true,
                date_to: '2024-05-09T23:59:59.999999Z',
                date_from: '2024-05-02T00:00:00Z',
                entity_type: 'events',
                interval: 'day',
                display: 'ActionsPie',
                breakdown: '$active_feature_flags',
                breakdown_type: 'event',
            },
            action: {
                days: [
                    '2024-05-02T00:00:00Z',
                    '2024-05-03T00:00:00Z',
                    '2024-05-04T00:00:00Z',
                    '2024-05-05T00:00:00Z',
                    '2024-05-06T00:00:00Z',
                    '2024-05-07T00:00:00Z',
                    '2024-05-08T00:00:00Z',
                    '2024-05-09T00:00:00Z',
                ],
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_insightsql: null,
                math_group_type_index: null,
                properties: {},
            },
            breakdown_value:
                '["signup-page-4.0","billing-limit","kafka-inspector","person-on-events-enabled","auto-rollback-feature-flags","onboarding-v2-demo","query_running_time","query-timings","hedgehog-mode","web-analytics","web-analytics-sampling","high-frequency-batch-exports","exception-autocapture","data-warehouse","data-warehouse-postgres-import","ff-dashboard-templates","show-product-intro-existing-products","cs-dashboards","redirect-signups-to-instance","apps-and-exports-ui","insightsql-insights-preview","insightsql-insights-lifecycle","insightsql-insights-paths","insightsql-insights-retention","insightsql-insights-trends","insightsql-insights-stickiness","insightsql-insights-funnels","insightsql-insight-live-compare","insightsql-in-insight-serialization","bi_viz","webhooks-denylist","pipeline-ui","session-recording-sampling","person-feed-canvas","feature-flag-cohort-creation","insight-horizontal-controls","surveys-widgets","invite-team-member-onboarding","year-in-hog","session-replay-export-mobile-data","discussions","ai-session-summary","ai-session-permissions","product-intro-pages","session-replay-doctor","session-replay-similar-recordings","saved-not-pinned","billing-upgrade-language","new-experiments-ui","session-replay-filter-ordering","session-replay-linked-variants","session-replay-error-clustering","audit-logs-access","reverse-proxy-onboarding","session-replay-mobile-onboarding","ip-allowlist-setting","email-verification-ticket-submission","theme"]',
        },
        {
            data: [],
            days: [],
            count: 0,
            aggregated_value: 322,
            label: '$$_posthog_breakdown_other_$$',
            filter: {
                insight: 'TRENDS',
                filter_test_accounts: true,
                date_to: '2024-05-09T23:59:59.999999Z',
                date_from: '2024-05-02T00:00:00Z',
                entity_type: 'events',
                interval: 'day',
                display: 'ActionsPie',
                breakdown: '$active_feature_flags',
                breakdown_type: 'event',
            },
            action: {
                days: [
                    '2024-05-02T00:00:00Z',
                    '2024-05-03T00:00:00Z',
                    '2024-05-04T00:00:00Z',
                    '2024-05-05T00:00:00Z',
                    '2024-05-06T00:00:00Z',
                    '2024-05-07T00:00:00Z',
                    '2024-05-08T00:00:00Z',
                    '2024-05-09T00:00:00Z',
                ],
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_insightsql: null,
                math_group_type_index: null,
                properties: {},
            },
            breakdown_value: '$$_posthog_breakdown_other_$$',
        },
        {
            data: [],
            days: [],
            count: 0,
            aggregated_value: 801,
            label: '["signup-page-4.0","billing-limit","kafka-inspector","person-on-events-enabled","auto-rollback-feature-flags","onboarding-v2-demo","query_running_time","query-timings","hedgehog-mode","web-analytics","web-analytics-sampling","high-frequency-batch-exports","exception-autocapture","data-warehouse","data-warehouse-postgres-import","ff-dashboard-templates","show-product-intro-existing-products","cs-dashboards","redirect-signups-to-instance","apps-and-exports-ui","insightsql-insights-preview","insightsql-insights-lifecycle","insightsql-insights-paths","insightsql-insights-retention","insightsql-insights-trends","insightsql-insights-stickiness","insightsql-insights-funnels","insightsql-in-insight-serialization","bi_viz","webhooks-denylist","pipeline-ui","session-recording-sampling","person-feed-canvas","feature-flag-cohort-creation","insight-horizontal-controls","surveys-widgets","invite-team-member-onboarding","year-in-hog","session-replay-export-mobile-data","discussions","ai-session-summary","ai-session-permissions","product-intro-pages","session-replay-doctor","session-replay-similar-recordings","saved-not-pinned","billing-upgrade-language","new-experiments-ui","session-replay-filter-ordering","session-replay-linked-variants","session-replay-error-clustering","audit-logs-access","reverse-proxy-onboarding","session-replay-mobile-onboarding","ip-allowlist-setting","email-verification-ticket-submission","theme"]',
            filter: {
                insight: 'TRENDS',
                filter_test_accounts: true,
                date_to: '2024-05-09T23:59:59.999999Z',
                date_from: '2024-05-02T00:00:00Z',
                entity_type: 'events',
                interval: 'day',
                display: 'ActionsPie',
                breakdown: '$active_feature_flags',
                breakdown_type: 'event',
            },
            action: {
                days: [
                    '2024-05-02T00:00:00Z',
                    '2024-05-03T00:00:00Z',
                    '2024-05-04T00:00:00Z',
                    '2024-05-05T00:00:00Z',
                    '2024-05-06T00:00:00Z',
                    '2024-05-07T00:00:00Z',
                    '2024-05-08T00:00:00Z',
                    '2024-05-09T00:00:00Z',
                ],
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_insightsql: null,
                math_group_type_index: null,
                properties: {},
            },
            breakdown_value:
                '["signup-page-4.0","billing-limit","kafka-inspector","person-on-events-enabled","auto-rollback-feature-flags","onboarding-v2-demo","query_running_time","query-timings","hedgehog-mode","web-analytics","web-analytics-sampling","high-frequency-batch-exports","exception-autocapture","data-warehouse","data-warehouse-postgres-import","ff-dashboard-templates","show-product-intro-existing-products","cs-dashboards","redirect-signups-to-instance","apps-and-exports-ui","insightsql-insights-preview","insightsql-insights-lifecycle","insightsql-insights-paths","insightsql-insights-retention","insightsql-insights-trends","insightsql-insights-stickiness","insightsql-insights-funnels","insightsql-in-insight-serialization","bi_viz","webhooks-denylist","pipeline-ui","session-recording-sampling","person-feed-canvas","feature-flag-cohort-creation","insight-horizontal-controls","surveys-widgets","invite-team-member-onboarding","year-in-hog","session-replay-export-mobile-data","discussions","ai-session-summary","ai-session-permissions","product-intro-pages","session-replay-doctor","session-replay-similar-recordings","saved-not-pinned","billing-upgrade-language","new-experiments-ui","session-replay-filter-ordering","session-replay-linked-variants","session-replay-error-clustering","audit-logs-access","reverse-proxy-onboarding","session-replay-mobile-onboarding","ip-allowlist-setting","email-verification-ticket-submission","theme"]',
        },
        {
            data: [],
            days: [],
            count: 0,
            aggregated_value: 25567,
            label: '$$_posthog_breakdown_null_$$',
            filter: {
                insight: 'TRENDS',
                filter_test_accounts: true,
                date_to: '2024-05-09T23:59:59.999999Z',
                date_from: '2024-05-02T00:00:00Z',
                entity_type: 'events',
                interval: 'day',
                display: 'ActionsPie',
                breakdown: '$active_feature_flags',
                breakdown_type: 'event',
            },
            action: {
                days: [
                    '2024-05-02T00:00:00Z',
                    '2024-05-03T00:00:00Z',
                    '2024-05-04T00:00:00Z',
                    '2024-05-05T00:00:00Z',
                    '2024-05-06T00:00:00Z',
                    '2024-05-07T00:00:00Z',
                    '2024-05-08T00:00:00Z',
                    '2024-05-09T00:00:00Z',
                ],
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_insightsql: null,
                math_group_type_index: null,
                properties: {},
            },
            breakdown_value: '$$_posthog_breakdown_null_$$',
        },
        {
            data: [],
            days: [],
            count: 0,
            aggregated_value: 2,
            label: '[]',
            filter: {
                insight: 'TRENDS',
                filter_test_accounts: true,
                date_to: '2024-05-09T23:59:59.999999Z',
                date_from: '2024-05-02T00:00:00Z',
                entity_type: 'events',
                interval: 'day',
                display: 'ActionsPie',
                breakdown: '$active_feature_flags',
                breakdown_type: 'event',
            },
            action: {
                days: [
                    '2024-05-02T00:00:00Z',
                    '2024-05-03T00:00:00Z',
                    '2024-05-04T00:00:00Z',
                    '2024-05-05T00:00:00Z',
                    '2024-05-06T00:00:00Z',
                    '2024-05-07T00:00:00Z',
                    '2024-05-08T00:00:00Z',
                    '2024-05-09T00:00:00Z',
                ],
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_insightsql: null,
                math_group_type_index: null,
                properties: {},
            },
            breakdown_value: '[]',
        },
        {
            data: [],
            days: [],
            count: 0,
            aggregated_value: 9,
            label: '["signup-page-4.0","billing-limit","kafka-inspector","person-on-events-enabled","auto-rollback-feature-flags","onboarding-v2-demo","query_running_time","query-timings","hedgehog-mode","web-analytics","web-analytics-sampling","high-frequency-batch-exports","exception-autocapture","data-warehouse","data-warehouse-postgres-import","ff-dashboard-templates","show-product-intro-existing-products","cs-dashboards","redirect-signups-to-instance","apps-and-exports-ui","insightsql-insights-preview","insightsql-insights-lifecycle","insightsql-insights-paths","insightsql-insights-retention","insightsql-insights-trends","insightsql-insights-stickiness","insightsql-insights-funnels","insightsql-insight-live-compare","insightsql-in-insight-serialization","bi_viz","webhooks-denylist","pipeline-ui","session-recording-sampling","person-feed-canvas","feature-flag-cohort-creation","insight-horizontal-controls","surveys-widgets","invite-team-member-onboarding","year-in-hog","session-replay-export-mobile-data","discussions","ai-session-summary","ai-session-permissions","product-intro-pages","session-replay-doctor","session-replay-similar-recordings","saved-not-pinned","billing-upgrade-language","new-experiments-ui","session-replay-filter-ordering","session-replay-linked-variants","session-replay-error-clustering","audit-logs-access","reverse-proxy-onboarding","session-replay-mobile-onboarding","ip-allowlist-setting","email-verification-ticket-submission","theme"]',
            filter: {
                insight: 'TRENDS',
                filter_test_accounts: true,
                date_to: '2024-05-09T23:59:59.999999Z',
                date_from: '2024-05-02T00:00:00Z',
                entity_type: 'events',
                interval: 'day',
                display: 'ActionsPie',
                breakdown: '$active_feature_flags',
                breakdown_type: 'event',
            },
            action: {
                days: [
                    '2024-05-02T00:00:00Z',
                    '2024-05-03T00:00:00Z',
                    '2024-05-04T00:00:00Z',
                    '2024-05-05T00:00:00Z',
                    '2024-05-06T00:00:00Z',
                    '2024-05-07T00:00:00Z',
                    '2024-05-08T00:00:00Z',
                    '2024-05-09T00:00:00Z',
                ],
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_insightsql: null,
                math_group_type_index: null,
                properties: {},
            },
            breakdown_value:
                '["signup-page-4.0","billing-limit","kafka-inspector","person-on-events-enabled","auto-rollback-feature-flags","onboarding-v2-demo","query_running_time","query-timings","hedgehog-mode","web-analytics","web-analytics-sampling","high-frequency-batch-exports","exception-autocapture","data-warehouse","data-warehouse-postgres-import","ff-dashboard-templates","show-product-intro-existing-products","cs-dashboards","redirect-signups-to-instance","apps-and-exports-ui","insightsql-insights-preview","insightsql-insights-lifecycle","insightsql-insights-paths","insightsql-insights-retention","insightsql-insights-trends","insightsql-insights-stickiness","insightsql-insights-funnels","insightsql-insight-live-compare","insightsql-in-insight-serialization","bi_viz","webhooks-denylist","pipeline-ui","session-recording-sampling","person-feed-canvas","feature-flag-cohort-creation","insight-horizontal-controls","surveys-widgets","invite-team-member-onboarding","year-in-hog","session-replay-export-mobile-data","discussions","ai-session-summary","ai-session-permissions","product-intro-pages","session-replay-doctor","session-replay-similar-recordings","saved-not-pinned","billing-upgrade-language","new-experiments-ui","session-replay-filter-ordering","session-replay-linked-variants","session-replay-error-clustering","audit-logs-access","reverse-proxy-onboarding","session-replay-mobile-onboarding","ip-allowlist-setting","email-verification-ticket-submission","theme"]',
        },
    ],
    timezone: 'UTC',
} as unknown as TrendAPIResponse

// 1. Select "Lifecycle"
export const lifecycleResult: TrendAPIResponse = {
    result: [
        {
            action: {
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$pageview - new',
            count: 35346.0,
            data: [2757.0, 3021.0, 15176.0, 4456.0, 2172.0, 3038.0, 3244.0, 1482.0],
            labels: [
                '28-Feb-2023',
                '1-Mar-2023',
                '2-Mar-2023',
                '3-Mar-2023',
                '4-Mar-2023',
                '5-Mar-2023',
                '6-Mar-2023',
                '7-Mar-2023',
            ],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            status: 'new',
            persons_urls: [
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-02-28',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-02-28&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-01',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-01&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-02',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-02&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-03',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-03&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-04',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-04&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-05',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-05&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-06',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-06&entity_order=0&lifecycle_type=new',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-07',
                        entity_order: 0,
                        lifecycle_type: 'new',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-07&entity_order=0&lifecycle_type=new',
                },
            ],
        },
        {
            action: {
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$pageview - dormant',
            count: -50255.0,
            data: [-4906.0, -4421.0, -4609.0, -17127.0, -7016.0, -2762.0, -3740.0, -5674.0],
            labels: [
                '28-Feb-2023',
                '1-Mar-2023',
                '2-Mar-2023',
                '3-Mar-2023',
                '4-Mar-2023',
                '5-Mar-2023',
                '6-Mar-2023',
                '7-Mar-2023',
            ],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            status: 'dormant',
            persons_urls: [
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-02-28',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-02-28&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-01',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-01&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-02',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-02&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-03',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-03&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-04',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-04&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-05',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-05&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-06',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-06&entity_order=0&lifecycle_type=dormant',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-07',
                        entity_order: 0,
                        lifecycle_type: 'dormant',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-07&entity_order=0&lifecycle_type=dormant',
                },
            ],
        },
        {
            action: {
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$pageview - returning',
            count: 9814.0,
            data: [1714.0, 1617.0, 1677.0, 1805.0, 578.0, 457.0, 879.0, 1087.0],
            labels: [
                '28-Feb-2023',
                '1-Mar-2023',
                '2-Mar-2023',
                '3-Mar-2023',
                '4-Mar-2023',
                '5-Mar-2023',
                '6-Mar-2023',
                '7-Mar-2023',
            ],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            status: 'returning',
            persons_urls: [
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-02-28',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-02-28&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-01',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-01&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-02',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-02&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-03',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-03&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-04',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-04&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-05',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-05&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-06',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-06&entity_order=0&lifecycle_type=returning',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-07',
                        entity_order: 0,
                        lifecycle_type: 'returning',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-07&entity_order=0&lifecycle_type=returning',
                },
            ],
        },
        {
            action: {
                id: '$pageview',
                type: 'events',
                order: 0,
                name: '$pageview',
                custom_name: null,
                math: 'total',
                math_property: null,
                math_group_type_index: null,
                properties: {},
            },
            label: '$pageview - resurrecting',
            count: 11612.0,
            data: [1567.0, 1648.0, 2079.0, 1333.0, 469.0, 1124.0, 2638.0, 754.0],
            labels: [
                '28-Feb-2023',
                '1-Mar-2023',
                '2-Mar-2023',
                '3-Mar-2023',
                '4-Mar-2023',
                '5-Mar-2023',
                '6-Mar-2023',
                '7-Mar-2023',
            ],
            days: [
                '2023-02-28',
                '2023-03-01',
                '2023-03-02',
                '2023-03-03',
                '2023-03-04',
                '2023-03-05',
                '2023-03-06',
                '2023-03-07',
            ],
            status: 'resurrecting',
            persons_urls: [
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-02-28',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-02-28&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-01',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-01&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-02',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-02&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-03',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-03&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-04',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-04&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-05',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-05&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-06',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-06&entity_order=0&lifecycle_type=resurrecting',
                },
                {
                    filter: {
                        entity_id: '$pageview',
                        entity_type: 'events',
                        entity_math: 'total',
                        target_date: '2023-03-07',
                        entity_order: 0,
                        lifecycle_type: 'resurrecting',
                    },
                    url: 'api/person/lifecycle/?breakdown_attribution_type=first_touch&breakdown_normalize_url=False&date_from=-7d&display=ActionsLineGraph&events=%5B%7B%22id%22%3A+%22%24pageview%22%2C+%22type%22%3A+%22events%22%2C+%22order%22%3A+0%2C+%22name%22%3A+%22%24pageview%22%2C+%22custom_name%22%3A+null%2C+%22math%22%3A+%22total%22%2C+%22math_property%22%3A+null%2C+%22math_group_type_index%22%3A+null%2C+%22properties%22%3A+%7B%7D%7D%5D&insight=LIFECYCLE&interval=day&sampling_factor=&shown_as=Lifecycle&smoothing_intervals=1&entity_id=%24pageview&entity_type=events&entity_math=total&target_date=2023-03-07&entity_order=0&lifecycle_type=resurrecting',
                },
            ],
        },
    ],
    timezone: 'US/Pacific',
    last_refresh: '2023-03-07T15:44:27.385509Z',
    is_cached: true,
    next: null,
} as unknown as TrendAPIResponse
