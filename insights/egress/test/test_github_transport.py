...
from insights.egress.github.limiter import GitHubRateResource
from insights.egress.github.transport import GitHubClient, github_request
...
class TestGitHubTransport(SimpleTestCase):
    ...
    def test_consume_routes_resource_by_url(self, _name: str, url: str, expected: GitHubRateResource) -> None:
        ...
        client = GitHubClient()
        with patch("insights.egress.github.transport.consume_github_installation_sync", return_value=True) as consume:
    ...
    def test_identity_blind_call_never_touches_the_limiter(self) -> None:
        ...
        with (
            patch("insights.egress.github.transport.consume_github_installation_sync") as consume,
            patch("requests.request", return_value=_response()),
            patch("insights.egress.github.transport.record_github_api_response"),
        ):
