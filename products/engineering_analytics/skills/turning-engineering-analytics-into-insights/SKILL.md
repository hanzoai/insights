...
description: >
  Converts engineering analytics (PR / CI) data into saved Insights insights, dashboards, and subscriptions, and
  explains what data the product reads so it can be queried directly with SQL. The engineering analytics dashboard
  and MCP tools run curated InsightsQL privately over per-team GitHub warehouse tables; this skill teaches discovering
  those tables via engineering-analytics-sources, replicating the curated column semantics in InsightsQL, reading the
...
run curated InsightsQL privately: nothing in the UI or the tool output names the underlying tables,
...
- **Raw warehouse tables** — `<prefix>github_pull_requests`, `<prefix>github_workflow_runs`,
  ...
  `<prefix>github_team_members` (org team membership — the author→team map) — ordinary team-scoped tables you query with InsightsQL.
...
re-rendered into the team's view whenever the code changes. Everything else is just table data, and **pure InsightsQL over
...
## Step 2: write InsightsQL that carries the curated semantics
...
Copy the base subqueries from [references/insightsql-recipes.md](references/insightsql-recipes.md) (they mirror the product's own curated builders) and follow these rules:

- **Timestamps are strings.** Always `parseDateTimeBestEffort(created_at)` etc. before comparing or diffing.
- **Nested JSON is Nullable.** `ifNull(...)`-unwrap before `JSONExtractArrayRaw` / `splitByChar`: Datastore rejects an Array inside a Nullable.
...
- **Bot detection**: `author_handle LIKE '%[bot]' OR author_handle IN ('dependabot', 'github-actions', 'insights-bot', 'renovate')`. Exclude bots and drafts from throughput / merge-time metrics by default.
...
```json
{
  ...
  "query": {
    ...
    "source": { "kind": "InsightsQLQuery", "query": "<tested SQL>" }
  }
}
```
