...
function getInsightsQLEvaluatedText(alertForm: AlertFormType): string {
    if (!isInsightsQLAlertConfig(alertForm.config)) {
        return ''
    }
...
export function AlertIntervalRow({
    ...
    if (!supportsTimeWindow(alertForm.config)) {
        evaluatedWindow = <div>{getInsightsQLEvaluatedText(alertForm)}</div>
    } else {
        const period =
