...
class TestPropertyValuesQueryRunner(DatastoreTestMixin, APIBaseTest):
    ...
    def test_event_property_values_new_schema_uses_json_subcolumn(self):
        ...
        physical_query = execute_insightsql_query(runner.to_query(), team=self.team)

        assert {r.name for r in response.results} == {"Chrome"}
        assert physical_query.datastore is not None
        assert "events_json" in physical_query.datastore
        assert "events.properties.browser" in physical_query.datastore
        assert "events.properties.^browser" in physical_query.datastore
        assert "JSONExtractRaw" not in physical_query.datastore
        assert "toJSONString" in physical_query.datastore
        assert "toString(events.properties)" not in physical_query.datastore
    ...
    def test_execution_mode_recorded_in_slo_started_event(self, _name, execution_mode, expected_value):
        ...
        with patch("insights.insightsql_queries.query_runner.hanzo_insights.capture") as mock_capture:
...
class TestPropertyValuesQueryRunnerAggregatedTable(DatastoreTestMixin, APIBaseTest):
    def _run(self, query: PropertyValuesQuery) -> list[PropertyValueItem]:
        runner = PropertyValuesQueryRunner(team=self.team, query=query)
        return runner.calculate().results
    ...
    def _flag_on(self):
        return patch(
            "insights.insightsql_queries.property_values_query_runner.hanzo_insights.feature_enabled",
    ...
    def test_restricted_property_key_is_not_served(self):
        ...
        with (
            self._flag_on(),
            patch(
                "insights.insightsql_queries.property_values_query_runner.get_restricted_property_names",
