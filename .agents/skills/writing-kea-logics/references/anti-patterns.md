...
Instance identity is handled by `key`, but Insights convention is to also append the
...
```tsx
// don't
<Input value={values.signup.email} onChange={(v) => actions.setSignupValue('email', v)} />
```
...
- Commit [`7ef47b68e75`](https://github.com/Insights/insights/commit/7ef47b68e75) —
