...
"fields": [
    ...
    "type": ["null", "int"],
    "default": null,
    "doc": "Per-row retention in days, stamped by the Node consumer from team retention rules. Null when unset — Datastore then falls back to the batch `retention-days` header. capture-logs always writes null; it does not evaluate team rules."
    }
]
}"#;
