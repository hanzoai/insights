...
End-to-end tests that verify the Insights ingestion pipeline is functioning correctly. These tests capture real events via the Insights SDK and query them back via direct Datastore queries to ensure events land in Datastore as expected.
...
- `query_event_by_uuid(uuid)` - Poll Datastore for an event by UUID
- `query_person_by_distinct_id(distinct_id)` - Poll Datastore for a person
- `query_events_by_person_id(person_id, expected_event_uuids)` - Poll Datastore for events by person
...
├── client.py                # Insights SDK wrapper with retry and direct Datastore queries
