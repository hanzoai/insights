...
# recent N pushes per PR *in Datastore* (rows are ordered newest-first, so the cap keeps the newest),
...
# InsightsQL applies its default 100-row limit and silently truncates the whole result.
...
def query_pr_push_history(
    *, curated: CuratedGitHubSource, pr_numbers: list[int]
    ...
    Datastore to the most recent ``_PUSH_HISTORY_LIMIT`` per PR. Scoped to the visible PR numbers so
