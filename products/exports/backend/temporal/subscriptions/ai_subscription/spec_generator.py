...
from insights.schema import CachedTeamTaxonomyQueryResponse, SubscriptionAIPromptMaxLength, TeamTaxonomyQuery
...
from insights.models import EventDefinition, EventProperty, PropertyDefinition, Team, User
from insights.models.group_type_mapping import get_group_types_for_project
from insights.security.llm_prompt_sanitization import sanitize_user_text
...
# Placeholder tokens the planner writes instead of concrete dates, so frozen InsightsQL stays
...
class ReportWindow:
    ...
    The literals render as project-tz wall clock without an offset: InsightsQL resolves bare datetime
    ...
    def render_window_filter(self, insightsql: str) -> str:
        # str.replace is non-recursive, and the substituted SQL contains no tokens, so nothing re-expands.
        return (
            insightsql.replace(DATE_RANGE_PLACEHOLDER, self.window_filter_sql)
...
def _no_data_event_names(team: Team, limit: int) -> list[str]:
    ...
    # plausible list of dormant events from its general knowledge of Insights event names. The cutoff is a
...
def _group_type_labels(team: Team) -> list[str]:
    # Map each configured group type to its InsightsQL virtual-join path (group_0..group_4) so the planner
...
def _llm_selected_events(
    ...
    insights_properties: dict[str, Union[str, int]] = {"feature": "ai_subscription", "stage": "event_selection"}
...
def _event_property_names(team: Team, events: list[str], per_event_limit: int) -> dict[str, list[str]]:
    ...
    # property names — the top cause of InternalInsightsQLError.
    if not events:
        return {}
...
def build_context_blob(team: Team, window: ReportWindow, relevant_events: Sequence[str] = ()) -> str:
    ...
    # window advances every run) and keeps timezone math out of InsightsQL. The concrete bounds are still
...
def generate_query_plan(
    *,
    ...
    insights_properties: dict[str, Union[str, int]] = {"feature": "ai_subscription", "stage": "plan"}
    if trace_correlation_id is not None:
        insights_properties["subscription_id"] = trace_correlation_id
    llm = MaxChatOpenAI(
        model=DEFAULT_PLANNER_MODEL,
