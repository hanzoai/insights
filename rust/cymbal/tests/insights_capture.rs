...
// One test per binary: common_insights::init configures a process-wide global
// client, so a second init with a different mock server would be ignored.
#[sqlx::test(migrations = "./tests/test_migrations")]
async fn pipeline_failure_is_captured_as_insights_exception(db: PgPool) {
    let insights = MockServer::start_async().await;
    let capture = insights
    ...
    common_insights::init("cymbal-test", Some("test-api-key"), &insights.base_url())
        .await
        .expect("insights init");
