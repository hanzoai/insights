...
from insights.test.base import BaseTest, NonAtomicBaseTest
...
from insights.insightsql.database.models import Table
...
from insights.models import Group, GroupTypeMapping, GroupUsageMetric, Organization, Tag, Team
...
from insights.persons_seed import insert_seed_group, insert_seed_group_type_mapping
...
def _create_data_modeling_job(team: Team, label: str) -> DataModelingJob:
    saved_query = DataWarehouseSavedQuery.objects.create(
        team=team, name=f"query_{label}", query={"kind": "InsightsQLQuery", "query": "SELECT 1"}
    )
...
def _create_data_warehouse_saved_query(team: Team, label: str) -> DataWarehouseSavedQuery:
    return DataWarehouseSavedQuery.objects.create(
        team=team, name=f"view_{label}", query={"kind": "InsightsQLQuery", "query": "SELECT 1"}
    )
...
def _get_or_create_user_for_team(team: Team, label: str):
    ...
    if not user:
        user = User.objects.create(email=f"test_{label}@hanzo.ai")
    return user
...
def _create_insights_function(team: Team, label: str) -> InsightsFunction:
...
def _create_group(team: Team, label: str) -> Group:
    ...
    # it back from there. The personinsights fake stays active for the InsightsQL Database build, so this
...
def _create_integration_repository_cache_entry(team: Team, label: str):
    from insights.models.integration import Integration
    from insights.models.integration_repository_cache import IntegrationRepositoryCacheEntry
...
def _create_session_recording(team: Team, label: str):
    from insights.models import SessionRecording
...
def _create_session_recording_playlist(team: Team, label: str):
    from insights.models import SessionRecordingPlaylist
...
def _create_sandbox_environment(team: Team, label: str):
    ...
    # private=False so the row is queryable via InsightsQL — the privacy predicate
...
class TestSystemTablesTeamIsolation(NonAtomicBaseTest):
    ...
    federated system table reads), while the personinsights fake stays active so the InsightsQL Database
    ...
    def setUp(self):
        ...
        with persons_db_connection(writer=True, autocommit=True) as conn, conn.cursor() as cursor:
            cursor.execute("TRUNCATE TABLE insights_group, insights_grouptypemapping RESTART IDENTITY CASCADE")
    ...
    def test_system_table_returns_only_own_team_data(self, table_name, factory):
        ...
        response = execute_insightsql_query(f"SELECT id FROM system.{table_name}", team=self.team, user=self.user)
...
class TestSystemTablesSandboxEnvironmentPrivacy(BaseTest):
    ...
    def test_generated_sql_includes_private_predicate(self):
        ...
        query, _ = prepare_and_print_ast(
            parse_select("SELECT id FROM system.sandbox_environments"), context, dialect="datastore"
        )
...
class TestSystemTablesSandboxEnvironmentPrivacyIsolation(NonAtomicBaseTest):
    """End-to-end check that private and internal sandbox environments are never returned via InsightsQL,
    ...
    def test_private_environments_excluded(self):
        ...
        response = execute_insightsql_query("SELECT id FROM system.sandbox_environments", team=self.team, user=self.user)
        ids = {str(row[0]) for row in response.results}
...
class TestSystemTablesCanvasDeletedExclusion(BaseTest):
    ...
    def test_generated_sql_includes_deleted_predicate(self):
        db = Database.create_for(team=self.team, user=self.user)
        context = InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, database=db)
        query, _ = prepare_and_print_ast(parse_select("SELECT id FROM system.canvases"), context, dialect="datastore")
...
class TestSystemTablesCanvasDeletedExclusionIsolation(NonAtomicBaseTest):
    """End-to-end check that soft-deleted canvases are never returned via InsightsQL."""
    ...
    def test_deleted_canvases_excluded(self):
        ...
        response = execute_insightsql_query("SELECT id FROM system.canvases", team=self.team, user=self.user)
        ids = {str(row[0]) for row in response.results}
...
class TestSystemTablesTaskInternalExclusion(BaseTest):
    ...
    def test_generated_sql_includes_internal_predicate(self):
        db = Database.create_for(team=self.team, user=self.user)
        context = InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, database=db)
        query, _ = prepare_and_print_ast(parse_select("SELECT id FROM system.tasks"), context, dialect="datastore")
...
class TestSystemTablesTaskInternalExclusionIsolation(NonAtomicBaseTest):
    """End-to-end check that internal tasks are never returned via InsightsQL."""
    ...
    def test_internal_tasks_excluded(self):
        ...
        response = execute_insightsql_query("SELECT id FROM system.tasks", team=self.team, user=self.user)
        ids = {str(row[0]) for row in response.results}
...
class TestSystemAccountsLazyJoins(NonAtomicBaseTest):
    ...
    def test_relationships_lazy_join_multi_holder_returns_all_active(self):
        ...
        other_user = self._create_user("fde2@hanzo.ai")
        self._create_relationship(account, definition, self.user)
