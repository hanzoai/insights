...
use personinsights_proto::personinsights::identity::v1::GetOrCreatePersonEntry;
...
pub async fn run(args: GateArgs) -> Result<()> {
    ...
    // The identity service writes the real insights_person table (its distinct
    // id FKs require it); a stack targeting another table would recover
    // created persons from a table they were never written to.
    if args.create_via_identity && args.pg_target_table != "insights_person" {
        bail!("--create-via-identity requires --pg-target-table insights_person");
    }
...
mod tests {
    ...
    fn gate_args(extra: &[&str]) -> GateArgs {
        let mut argv = vec!["personinsights-test-harness", "gate"];
