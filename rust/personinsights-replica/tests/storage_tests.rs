...
use personinsights_replica::storage::postgres::ConsistencyLevel;
use personinsights_replica::storage::GroupKey;
use rand::Rng;
use rstest::rstest;
use uuid::Uuid;
...
async fn cohort_row_count(pool: &sqlx::PgPool, cohort_id: i64, person_ids: &[i64]) -> i64 {
    sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM insights_cohortpeople WHERE cohort_id = $1 AND person_id = ANY($2)",
    )
    .bind(cohort_id)
    .bind(person_ids)
    .fetch_one(pool)
...
async fn test_delete_persons_clears_cohort_memberships() {
    // insights_cohortpeople has no FK to insights_person (no DB cascade), so the
...
async fn test_delete_persons_batch_for_team_leaves_cohort_memberships() {
    ...
    sqlx::query("DELETE FROM insights_cohortpeople WHERE cohort_id = $1 AND person_id = $2")
        .bind(cohort_id)
        .bind(person.id)
        .execute(&ctx.pool)
        .await
...
async fn test_delete_personless_distinct_ids_batch_for_team_cross_team_isolation() {
    ...
    let other_team_id = ctx.team_id + 1;
    sqlx::query(
        r#"INSERT INTO insights_personlessdistinctid
    ...
    let remaining: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM insights_personlessdistinctid WHERE team_id = $1")
            .bind(other_team_id)
            .fetch_one(&ctx.pool)
            .await
            .unwrap();
    ...
    sqlx::query("DELETE FROM insights_personlessdistinctid WHERE team_id = $1")
        .bind(other_team_id)
        .execute(&ctx.pool)
        .await
        .ok();
...
async fn test_delete_persons_batch_for_team_rolls_back_on_partial_failure() {
    ...
    // Create a table with a RESTRICT FK to insights_person. When
    // delete_persons_batch_for_team reaches step 3 (DELETE FROM insights_person),
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS _test_person_fk_block (
            ...
            FOREIGN KEY (team_id, person_id)
                REFERENCES insights_person(team_id, id) ON DELETE RESTRICT
        )",
    )
    .execute(&ctx.pool)
    .await
...
async fn test_delete_persons_cross_team_isolation() {
    ...
    let still_exists = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM insights_person WHERE team_id = $1 AND uuid = $2",
    )
    .bind(other_team_id as i32)
    .bind(other_uuid)
    .fetch_one(&ctx.pool)
...
async fn test_get_groups_without_properties() {
    ...
    let with_props = ctx
        ...
        .get_groups(
            ctx.team_id,
            &[personinsights_replica::storage::GroupIdentifier {
                group_type_index: 0,
...
async fn test_set_person_distinct_id_version_floor_updates_and_returns_person() {
    ...
    let version: i64 = sqlx::query_scalar(
        "SELECT version FROM insights_persondistinctid WHERE team_id = $1 AND distinct_id = $2",
    )
    .bind(ctx.team_id as i32)
    .bind("repair_did")
    .fetch_one(&ctx.pool)
...
async fn test_set_person_distinct_id_version_floor_does_not_lower() {
    ...
    sqlx::query(
        "UPDATE insights_persondistinctid SET version = 200 WHERE team_id = $1 AND distinct_id = $2",
    )
    .bind(ctx.team_id as i32)
