...
from insights.schema import AlertCalculationInterval, AlertState, ChartDisplayType, NodeKind, TrendsQuery

from insights.slo.context import get_current_slo
from insights.slo.types import SloOperation
from insights.tasks.alerts.schedule_restriction import snap_candidate_utc_to_schedule_restriction
...
# insights/temporal/alerts/activities.py retrieve_due_alerts.
...
def trigger_alert_insights_functions(alert: AlertConfiguration, properties: dict) -> bool:
    """Trigger all InsightsFunctions linked to the alert as notification destinations by producing an internal event."""

    logger.info(
        "Triggering internal event for alert destinations/script functions",
        alert_id=alert.id,
...
def send_notifications_for_breaches(
    ...
    `extra_properties` are merged into the internal-event properties that InsightsFunction
    ...
    if email_targets:
        subject = f"Insights alert {alert.name} is firing for {alert.team.name}"
    ...
    trigger_alert_insights_functions(
        alert=alert,
...
def send_notifications_for_disabled(alert: AlertConfiguration, reason: str, targets: list[str]) -> None:
    ...
    subject = f"Insights alert {alert.name} for {alert.team.name} has been disabled"
