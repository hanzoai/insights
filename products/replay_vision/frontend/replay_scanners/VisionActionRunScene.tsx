...
import { Card, Table, TableColumns, Link, Tooltip } from '@hanzo/elements'
...
function RecordingsIncluded({ observations }: { observations: readonly RunObservationApi[] }): JSX.Element {
    const columns: TableColumns<RunObservationApi> = [
        {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <Table columns={columns} dataSource={[...observations]} rowKey="id" />
        </div>
    )
}
        ...
        <SceneContent>
            ...
            {run.synthesized_markdown ? (
                <Card hoverEffect={false} className="p-4">
                    ...
                    <Markdown className="ScannerSummaryMarkdown text-base" disableImages>
                        {summaryMarkdown}
                    </Markdown>
                </Card>
            ) : run.status === 'running' ? (
                ...
                <Banner type={run.status === 'failed' ? 'error' : 'info'}>
