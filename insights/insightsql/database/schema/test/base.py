...
INVOICES_TEST_BUCKET = f"{TEST_BUCKET_BASE}-insights.revenue_analytics.insights_query_runner.stripe_invoices"
CUSTOMERS_TEST_BUCKET = f"{TEST_BUCKET_BASE}-insights.revenue_analytics.insights_query_runner.stripe_customers"
SUBSCRIPTIONS_TEST_BUCKET = f"{TEST_BUCKET_BASE}-insights.revenue_analytics.insights_query_runner.stripe_subscriptions"
CHARGES_TEST_BUCKET = f"{TEST_BUCKET_BASE}-insights.revenue_analytics.insights_query_runner.stripe_charges"
_TEST_DATA_DIR = (
    Path(__file__).resolve().parents[5] / "products" / "revenue_analytics" / "backend" / "views" / "test" / "data"
)


class RevenueAnalyticsTestBase(DatastoreTestMixin, BaseTest):
...
class RevenueAnalyticsManagedViewsetsTestMixin(RevenueAnalyticsTestBase):
    def setUp(self) -> None:
        super().setUp()
        self.mock_flag = patch("hanzo_insights.feature_enabled", return_value=True)
        ...
        for saved_query in DataWarehouseSavedQuery.objects.filter(
            ...
            for col_name, col_def in (saved_query.columns or {}).items():
                if isinstance(col_def, dict):
                    ch_type = col_def["datastore"]
                    if not ch_type.startswith("Nullable("):
                        col_def = {**col_def, "datastore": f"Nullable({ch_type})"}
