...
import pytest
from insights.test.base import BaseTest, _create_event, flush_persons_and_events
...
class TestPRLifecycleMapping(BaseTest):
    """InsightsQL parsing (parse_select runs for real) plus row mapping and event
    ...
    def test_assembles_ordered_events_and_marks_partial(self) -> None:
        ...
        with mock.patch(_RUN_QUERY, side_effect=[_resp([header]), _resp(runs)]):
            lifecycle = api.get_pr_lifecycle(team=self.team, pr_number=10, repo="Insights/insights")
        ...
        assert lifecycle.pull_request.repo.owner == "Insights" and lifecycle.pull_request.repo.name == "insights"
    ...
    def test_returns_none_when_not_found(self) -> None:
        with mock.patch(_RUN_QUERY, return_value=_resp([])):
            assert api.get_pr_lifecycle(team=self.team, pr_number=999, repo="Insights/insights") is None

    @parameterized.expand(["Insights", "Insights/", "/insights", "/"])
...
class TestPullRequestEndpointMapping(BaseTest):
    ...
    def test_pull_request_list_maps_row(self) -> None:
        ...
        push_rows = [
            ("Insights", "insights", 10, "sha-new", _dt("2026-01-11T10:00:00"), None, 0, 1),
            ("Insights", "insights", 10, "sha-old", _dt("2026-01-10T10:00:00"), 900, 1, 0),
        ]
        ...
        assert item.repo.owner == "Insights" and item.repo.name == "insights"
...
class TestResolveBranchMapping(BaseTest):
    ...
    def test_maps_rows_and_normalizes_empty_fields(self) -> None:
        ...
        rows = [("Insights", "insights", 42, "Fix bug", "merged"), ("Insights", "insights", 7, "", None)]
        ...
        assert [(m.repo, m.number, m.title, m.state) for m in matches] == [
            ("Insights/insights", 42, "Fix bug", "merged"),
            ("Insights/insights", 7, None, None),
        ]
...
class TestPullRequestEndpointsWarehouse(_EndpointsWarehouseMixin, BaseTest):
    ...
    def test_ci_cards_counts(self) -> None:
        ...
        assert cards.repos == 1  # all Insights/insights
    ...
    def test_pr_cost_sums_all_jobs_past_the_default_row_cap(self) -> None:
        # A PR with more jobs than InsightsQL's default 100-row cap: the detail cost must sum every job, not
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=71, repo="Insights/insights")
    ...
    def test_pr_cost_clamps_clock_skewed_negative_durations(self) -> None:
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=72, repo="Insights/insights")
    ...
    def test_pr_runs_span_all_commits(self) -> None:
        ...
        runs = api.list_pr_runs(team=self.team, pr_number=70, repo="Insights/insights")
    ...
    def test_pr_cost_aggregates_billable_jobs_across_runs(self) -> None:
        ...
        empty = api.get_pr_cost(team=self.team, pr_number=60, repo="Insights/insights")
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=60, repo="Insights/insights")
    ...
    def test_pull_request_list_rollup_is_repo_qualified(self) -> None:
        ...
        self._create_table(
            ...
            PULL_REQUESTS_COLUMNS,
            [
                _pr_row(10, "alice", "open", 0, _ago(1), head_sha="sha10", full_name="Insights/insights"),
                _pr_row(10, "bob", "open", 0, _ago(1), head_sha="shaB10", full_name="Insights/hanzo.ai"),
            ],
        )
        self._create_table(
            "github_workflow_runs",
            ...
            [
                ...
                # A second push + re-run on the other repo's PR #10 — must not leak onto insights's #10.
                _run_row(
                    3003,
                    "CI",
                    "shaB10b",
        ...
        assert (by_repo[("Insights", "insights")].pushes, by_repo[("Insights", "insights")].rerun_cycles) == (1, 0)
        assert (by_repo[("Insights", "hanzo.ai")].pushes, by_repo[("Insights", "hanzo.ai")].rerun_cycles) == (2, 1)
...
class TestPRLLMSpendWarehouse(_WarehouseMixin, BaseTest):
    ...
    def test_llm_spend_attributes_by_branch_within_window(self) -> None:
        ...
        self._generation(
            branch=branch, days_ago=10, cost=2.0, input_tokens=200, output_tokens=80, repo="insights/INSIGHTS"
        )
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=80, repo="Insights/insights")
    ...
    def test_llm_spend_none_when_head_is_base(self) -> None:
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=84, repo="Insights/insights")
    ...
    def test_llm_spend_failure_degrades_to_none(self) -> None:
        ...
        with mock.patch(
            ...
            cost = api.get_pr_cost(team=self.team, pr_number=85, repo="Insights/insights")
        assert cost.llm_spend is None
    ...
    def test_llm_spend_none_when_no_generations(self) -> None:
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=81, repo="Insights/insights")
        assert cost.llm_spend is None
    ...
    def test_prefix_credits_head_only_when_first_feature_branch_is_head(
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=82, repo="Insights/insights")
        assert cost.llm_spend is not None
    ...
    def test_carry_forward_follows_latest_stamp_until_a_branch_switch(self) -> None:
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=83, repo="Insights/insights")
        assert cost.llm_spend is not None
        assert cost.llm_spend.generations == 2
        assert cost.llm_spend.cost_usd == pytest.approx(3.0)
    ...
    def test_session_with_only_base_stamps_is_not_eligible(self) -> None:
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=86, repo="Insights/insights")
        assert cost.llm_spend is None
    ...
    def test_newest_snapshot_by_updated_at_drives_attribution(self) -> None:
        ...
        cost = api.get_pr_cost(team=self.team, pr_number=90, repo="Insights/insights")
        assert cost.llm_spend is not None
        assert cost.llm_spend.generations == 1
        assert cost.llm_spend.cost_usd == pytest.approx(3.0)
    ...
    def test_returns_merged_prs_since_cutoff_scoped_to_repo(self) -> None:
        self._create_table(
            ...
            [
                ...
                _pr_row(
                    23, "dave", "closed", 0, _ago(4), merged_at=_ago(3), head_sha="sha23", full_name="Insights/other"
                ),
        ...
        merged = api.list_recently_merged_pull_requests(team=self.team, repository="Insights/insights", since=since)
        ...
        scoped = api.list_recently_merged_pull_requests(
            team=self.team, repository="Insights/insights", since=since, numbers=[20]
        )
        ...
        by_number = api.list_recently_merged_pull_requests(
            team=self.team, repository="Insights/insights", since=since, numbers=[22]
        )
