...
from insights.api.embedding_worker import async_generate_embedding
...
from products.replay_vision.backend.tags import datastore_slugify_sql, slugify_tag
...
_TAGS_FILTER_CLAUSE = (
    f"hasAny(arrayMap(t -> {datastore_slugify_sql('t')}, JSONExtract(metadata, 'tags', 'Array(String)')), {{tags}})"
)


@dataclass(frozen=True)
class _ObservationFilters:
    """Exact-outcome filters, applied inside the Datastore ranking query against the embedding metadata
    ...
    def where_clauses(self, placeholders: dict[str, "ast.Expr"]) -> list[str]:
        """InsightsQL predicates over `metadata`, registering their values into `placeholders`. The metadata key is
...
class SearchReplayVisionObservationsTool(MaxTool):
    ...
    async def _search(
        ...
        # / Datastore pieces stay in `database_sync_to_async` blocks on either side so each thread held is
        # genuinely DB-bound.
    ...
    def _resolve_search_scope(
        ...
        # Clamp into [1, MAX] so a negative/zero/oversized limit can't reach the Datastore LIMIT clause.
    ...
    def _rank_and_format(
        ...
        """Sync Datastore rank + ORM fetch + format — runs after the embedding HTTP call has resolved."""
        empty = (f"No recordings from {scope_label} matched that search yet.", {"result_count": 0})

        # Filter + rank in one Datastore query: the structured outcome filters run against the embedding
    ...
    def _rank_observation_ids(
        ...
        result = execute_insightsql_query(query=insightsql_query, team=self._team, user=self._user, placeholders=placeholders)
