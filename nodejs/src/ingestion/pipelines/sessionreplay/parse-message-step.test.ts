...
describe('createParseMessageStep', () => {
    ...
    function createReplayHeaders(overrides: Partial<SessionReplayHeaders> = {}): SessionReplayHeaders {
        return { org: 'test-org', session_id: 'session-1', distinct_id: 'user-123', ...overrides }
    }
    ...
    it('should set org from the validated session replay headers', async () => {
        const step = createParseMessageStep()
        const payload = createValidSnapshotPayload('session-1')
        const input = createInput(0, 1, payload, undefined, undefined, createReplayHeaders({ org: 'my-org' }))
    ...
    it('should ignore a raw token header entirely', async () => {
        // The snapshot path carries no credential. A raw `token` Kafka header is not a
        // fallback for the org — it is a value with no meaning here, and reading it would
        ...
        expect(result.type).toBe(PipelineResultType.OK)
        if (result.type === PipelineResultType.OK) {
            expect(result.value.parsedMessage.org).toBe('test-org')
        }
    })
