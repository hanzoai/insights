...
from insights.schema import ProductKey, QueryStatus

from insights.insightsql.constants import BREAKDOWN_VALUES_LIMIT
...
from insights.api.documentation import extend_schema, extend_schema_field, extend_schema_serializer
from insights.api.forbid_destroy_model import ForbidDestroyModel
from insights.api.mixins import ValidatedRequest, validated_request
...
from insights.api.sharing_publish_gate import (
...
from insights.caching.insight_result import InsightResult
...
from insights.constants import INSIGHT, AvailableFeature
from insights.errors import ExposedCHQueryError
from insights.event_usage import EventSource, get_event_source, get_request_analytics_properties, report_user_action
from insights.exceptions_capture import capture_exception
from insights.helpers.impersonation import is_impersonated
from insights.helpers.multi_property_breakdown import protect_old_clients_from_multi_property_default
...
from insights.insightsql_queries.apply_dashboard_filters import (
    WRAPPER_NODE_KINDS,
...
from insights.insightsql_queries.query_runner import BLOCKING_EXECUTION_MODES, ExecutionMode, execution_mode_from_refresh
from insights.insightsql_queries.refresh_policy import ComputeSurface, resolve_execution_mode
from insights.kafka_client.topics import KAFKA_METRICS_TIME_TO_SEE_DATA
from insights.models import Filter, User
...
from insights.query_cache import QueryCache
...
from insights.resource_limits import LimitKey, check_count_limit
...
from insights.settings import CAPTURE_TIME_TO_SEE_DATA, SITE_URL
from insights.shared_link_user import SharedLinkUser
from insights.user_permissions import UserPermissionsSerializerMixin
...
DEPRECATED_DASHBOARDS_FIELD_USED_COUNTER = Counter(
    "insights_api_insight_deprecated_dashboards_field_used_total",
...
class _InsightQuerySchema(RootModel):
    ...
    root: schema.InsightVizNode | schema.DataTableNode | schema.DataVisualizationNode | schema.ScriptQuery = PydanticField(
        discriminator="kind"
    )
...
class InsightSerializer(InsightBasicSerializer):
    ...
    def validate_query(self, value: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        if not value:
            return value
        try:
            if value.get("kind") == "InsightsQLQuery":
                return schema.DataVisualizationNode(source=schema.InsightsQLQuery.model_validate(value)).model_dump(
...
class MCPInsightSerializer(InsightSerializer):
    ...
    wrapper before persisting: InsightsQLQuery → DataVisualizationNode, insight queries
    ...
    def validate_query(self, value: dict[str, Any] | None) -> dict[str, Any]:
        # Raw InsightsQL → DataVisualizationNode
        try:
            return schema.DataVisualizationNode(source=schema.InsightsQLQuery.model_validate(value)).model_dump(
                exclude_none=True, mode="json"
            )
...
Background calculation can be tracked using the `query_status` response field.""",
            ...
            OpenApiParameter(
                ...
                description="Restrict to a single insight type. `JSON` matches non-wrapper query insights; `SQL` matches InsightsQL queries.",
            ),
            OpenApiParameter(
                name="date_from",
                type=OpenApiTypes.STR,
...
class InsightViewSet(
    ...
    def list(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        span.set_attribute("insights.team_id", self.team_id)
        span.set_attribute("insights.basic", self._is_basic_request())
        span.set_attribute("insights.saved", str_to_bool(request.query_params.get("saved", "0")))
        span.set_attribute("insights.order", request.query_params.get("order", ""))
        ...
        for key in filters:
            if key == "saved":
                ...
                if str_to_bool(request.GET["hide_feature_flag_insights"]):
                    ...
                    from insights.helpers.dashboard_templates import (
...
When set, the specified dashboard's filters and date range override will be applied.""",
            ...
            make_variables_override_param(subject_label="the insight's InsightsQL", tool_name="insight-get"),
