...
| `insights:query-stickiness`       | Stickiness (active days per user)                |
...
Plus the standard Insights tools the playbooks reference by name (`feature-flag-get-all`,
...
- Saved insight (URL, `short_id`): `insights:insight-get` → `query.kind`. Use
  `insights:insight-query` if you also need the numbers.
...
For `InsightsQLQuery` insights, classify by the SQL's shape: count over time → trend
playbook, multi-step conversion → funnel playbook, cohort return → retention playbook.
Run the SQL through `insights:execute-sql` to get the data, then follow the closest
playbook's steps. See **InsightsQL insights** in shared-patterns.md.
...
- `insights:feature-flag-get-all` → flags with `updated_at` near the anomaly start.
- `insights:experiment-get-all` → `start_date` / `end_date` near the start.
- `insights:annotations-list` → `date_marker` near the start.
...
Use the format below. Offer to save key charts via `insights:insight-create`. If a
cause is found and no annotation marks it, offer `insights:annotation-create`. See
