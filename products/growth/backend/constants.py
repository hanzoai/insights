...
# the cache-miss fallback in insights/api/sdk_health.py:get_team_data, which runs one fresh
# single-team InsightsQL query and repopulates Redis. Steady-state load is unchanged; the trade-off
...
# Identity matching no longer persists tables on the Datastore cluster. Each run of
# products/growth/dags/identity_matching.py writes its datasets as Parquet to a pre-provisioned
# Datastore "scratch" S3 bucket, namespaced per team and run:
#
...
# mediated by Datastore (`INSERT INTO FUNCTION s3` / `FROM s3`); the dataset path-segment
...
# "no run yet" path). `job_id` is `String` because Datastore cannot write `UUID` to Parquet;
# `model_version`/`tier` are plain `String` because `LowCardinality` is a MergeTree-only
# optimisation. On writes the structure also drives Datastore's cast of the projection, exactly
...
# and every s3() call would target the wrong bucket (the app-assets bucket the Datastore role
...
IDENTITY_MATCHING_S3_UNCONFIGURED_MESSAGE = (
    ...
    "the general object-storage bucket, which the Datastore role cannot access. Set it to the "
...
def identity_matching_s3_args(team_id: int, job_id: str, relative_path: str, structure: str) -> str:
    """Build the argument list for a Datastore `s3(...)` call: ``url[, key, secret], 'Parquet', structure``.
