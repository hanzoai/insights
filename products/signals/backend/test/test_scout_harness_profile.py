...
class TestRecentInsightsFunctions(BaseTest):
    def test_total_enabled_counts_and_excludes_deleted(self) -> None:
        InsightsFunction.objects.create(team=self.team, name="a", enabled=True, type="destination", script="")
        InsightsFunction.objects.create(team=self.team, name="b", enabled=False, type="transformation", script="")
        InsightsFunction.objects.create(team=self.team, name="c", enabled=True, type="destination", deleted=True, script="")
        result = _recent_insights_functions(self.team)
        assert result["total_count"] == 2
        assert result["enabled_count"] == 1

    def test_carries_type_and_kind_for_orientation(self) -> None:
        InsightsFunction.objects.create(
            team=self.team, name="dest", enabled=True, type="destination", kind="webhook", script=""
        )
        result = _recent_insights_functions(self.team)
        row = result["recent"][0]
    ...
    def test_team_isolated(self) -> None:
        other = self.organization.teams.create(name="other")
        InsightsFunction.objects.create(team=other, name="x", enabled=True, type="destination", script="")
        result = _recent_insights_functions(self.team)
        assert result["total_count"] == 0


class TestRecentInsightsFlows(BaseTest):
...
class TestTopEvents(BaseTest):
    @patch("products.signals.backend.scout_harness.profile.builders.execute_insightsql_query")
