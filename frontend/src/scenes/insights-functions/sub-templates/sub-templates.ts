...
// $mcp_error_type — see products/mcp_analytics/backend/insightsql_queries/tool_tables.py for the list.
...
const FLAG_ACTOR_NAME = "{event.properties.user.first_name ? event.properties.user.first_name : 'Insights'}"
...
// The message copy lives here once; the Script templates and the in-app preview differ only in how
...
/**
 * The Slack message a notification will post, rendered with sample values in place of the Script
    ...
    'activity-log': [
        {
            ...
            inputs: {
                content: {
                    value: "**{event.properties.user.first_name ? event.properties.user.first_name : 'Insights'}** {event.properties.activity} {event.properties.scope} `{event.properties.item_id}`",
                },
            },
        },
        {
            ...
            inputs: {
                blocks: {
                    value: [
                        {
                            text: {
                                text: "*{event.properties.user.first_name ? event.properties.user.first_name : 'Insights'}* {event.properties.activity} {event.properties.scope} {event.properties.item_id}",
                                type: 'mrkdwn',
                            },
                            type: 'section',
                        },
...
[View issue](${errorTrackingIssueLinkHogTemplate('discord')})`,
    ...
    'health-check-firing': buildHealthAlertSubTemplates('health-check-firing', {
        ...
        discordContent:
            '**🩺 Insights health check**\n\n*{event.properties.title}*\n{event.properties.summary}\n\n[View in Insights]({project.url}{event.properties.link})',
        teamsText:
            '**🩺 Insights health check:** *{event.properties.title}* — {event.properties.summary} (View in [Insights]({project.url}{event.properties.link}))',
        actionButtonText: 'View in Insights',
    ...
    'health-check-resolved': buildHealthAlertSubTemplates('health-check-resolved', {
        ...
        discordContent:
            '**✅ Insights health check resolved**\n\n*{event.properties.title}*\n{event.properties.summary}\n\n[View in Insights]({project.url}{event.properties.link})',
        teamsText:
            '**✅ Resolved:** *{event.properties.title}* — {event.properties.summary} (View in [Insights]({project.url}{event.properties.link}))',
        actionButtonText: 'View in Insights',
