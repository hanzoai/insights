...
from insights.api import streaming
from insights.api.streaming import (
    _instrument_stream,
...
def _open_connections(endpoint: str) -> float:
    return REGISTRY.get_sample_value("insights_open_sse_connections", {"endpoint": endpoint}) or 0.0


def _closed_total(endpoint: str, outcome: str) -> float:
    return (
        REGISTRY.get_sample_value("insights_sse_stream_closed_total", {"endpoint": endpoint, "outcome": outcome}) or 0.0
    )
    ...
    def test_cap_of_zero_rejects_everything(self):
        with override_settings(SSE_MAX_CONCURRENT_STREAMS_PER_PROCESS=0):
            ...
            rejected_count = REGISTRY.get_sample_value(
                "insights_sse_rejected_over_cap_total", {"endpoint": "test_cap_zero"}
            )
