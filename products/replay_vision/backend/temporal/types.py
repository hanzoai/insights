...
class SessionMetadata(BaseModel, frozen=True):
    ...
    # Datastore derives these from `sum(active_milliseconds)/1000`, so they're floats in practice (e.g. 30.5s).
...
class EmitObservationSignalInputs(BaseModel, frozen=True):
    """Input to the side-effect activity that emits the side-mission findings as Insights Signals."""
