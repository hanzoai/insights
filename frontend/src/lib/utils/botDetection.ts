/**
 * Client-side bot detection — mirrors `insights.insightsql_queries.web_analytics.bot_definitions`
 * and the `getBotName` / `isLikelyBot` / `getTrafficCategory`
 * InsightsQL functions. Used by real-time dashboards where events arrive via the
 * livestream SSE feed and therefore cannot be classified server-side.
 *
 * Keep this file in sync with `insights/insightsql_queries/web_analytics/bot_definitions.py`.
 */
