...
from insights.insightsql_queries.ai.ai_column_rewriter import rewrite_expr_for_events_table, rewrite_query_for_events_table
...
from insights.insightsql_queries.ai.utils import merge_heavy_properties
        ...
        if not query_result.results:
            ...
            with (
                ...
                query_result = self.paginator.execute_insightsql_query(
                    ...
                    modifiers=self.modifiers,
                    limit_context=self.limit_context,
                    context=InsightsQLContext(team_id=self.team.pk, user=self.user, use_new_events_schema=events_schema),
                )
        ...
        date_filter = ast.And(
            exprs=[
                ast.CompareOperation(
                    ...
                    right=self._date_range.date_from_as_insightsql(),
                ),
                ast.CompareOperation(
                    op=ast.CompareOperationOp.LtEq,
                    left=ast.Field(chain=["ai_events", "timestamp"]),
                    right=self._date_range.date_to_as_insightsql(),
                ),
            ],
        )
        return (
