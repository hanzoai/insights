...
if [ "${1:-}" != "--skip-build" ]; then
    docker build -t insights-metrics-agent:test "$AGENT_DIR"
fi

kind get clusters 2>/dev/null | grep -qx "$CLUSTER" || kind create cluster --name "$CLUSTER" --wait 120s
kind load docker-image insights-metrics-agent:test --name "$CLUSTER"
...
helm --kube-context "$CTX" install scrape "$AGENT_DIR/chart/insights-metrics-agent" \
    ...
    --set image.repository=insights-metrics-agent \
    --set image.tag=test \
...
STS=scrape-insights-metrics-agent
$K -n agent rollout status "statefulset/$STS" --timeout=180s
check restricted-admission-ready-pods 2 "$($K -n agent get pods -l app.kubernetes.io/name=insights-metrics-agent --field-selector=status.phase=Running --no-headers | wc -l | tr -d ' ')"
