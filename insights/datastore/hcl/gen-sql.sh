...
# Datastore to create the OPS schema.
#
# Run from the repo root. Uses ./insights/datastore/hcl/bin/hclexp.
set -euo pipefail
...
# shellcheck source=insights/datastore/hcl/lib.sh
. "$HCL/lib.sh"
...
EMPTY="$HCL/.empty-schema.$$.hcl"; printf 'database "insights" {\n}\n' > "$EMPTY"
...
for env in $envs; do
  ...
  for role in $roles; do
    ...
    stack="$(manifest_stack "$env" "$role")"
    {
      ...
      echo "-- Full CREATE schema for the $env/$role node. Apply to a fresh Datastore to build it."
      echo
