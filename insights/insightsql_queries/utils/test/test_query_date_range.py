...
from insights.insightsql_queries.utils.query_date_range import QueryDateRange, QueryDateRangeWithIntervals
...
class TestQueryDateRange(APIBaseTest):
    ...
    def test_adjusted_start_of_interval_filters_from_start_of_day(self, _name, interval):
        ...
        ).date_from_with_adjusted_start_of_interval_insightsql()
...
class TestQueryDateRangeWithIntervals(APIBaseTest):
    ...
    def test_get_start_of_interval_insightsql_day_interval(self):
        query = QueryDateRangeWithIntervals(None, 1, self.team, IntervalType.DAY, self.now)
        expected_expr = ast.Call(name="toStartOfDay", args=[ast.Constant(value=query.date_from())])
        self.assertEqual(query.get_start_of_interval_insightsql(), expected_expr)

    def test_get_start_of_interval_insightsql_hour_interval(self):
        query = QueryDateRangeWithIntervals(None, 1, self.team, IntervalType.HOUR, self.now)
    ...
    def test_get_start_of_interval_insightsql_with_source(self):
        source_expr = ast.Constant(value="2021-08-25T00:00:00.000Z")
        query = QueryDateRangeWithIntervals(None, 1, self.team, IntervalType.DAY, self.now)
        expected_expr = ast.Call(name="toStartOfDay", args=[source_expr])
        self.assertEqual(query.get_start_of_interval_insightsql(source=source_expr), expected_expr)
