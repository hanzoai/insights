...
from insights.tasks.tasks import delete_organization_data_and_notify_task, delete_project_data_and_notify_task
...
class TestDeleteProjectDataAndNotifyTask(BaseTest):
    ...
    @patch("insights.tasks.email.send_project_deleted_email")
...
class TestDeleteOrganizationDataAndNotifyTask(BaseTest):
    ...
    @patch("insights.tasks.email.send_organization_deleted_email")
    @patch("insights.email.is_email_available", return_value=True)
    def test_sends_email_when_available(self, mock_email_available: Any, mock_send_email: Any) -> None:
        org = Organization.objects.create(name="Org to delete")
        org.members.add(self.user)
