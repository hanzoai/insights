...
def test_self_driving_quota_gate_enforces_only_when_limited_and_flag_on(limited, flag_on, expected_enforced):
    with (
        patch("products.signals.backend.quota.is_team_signals_quota_limited", return_value=limited),
        patch("products.signals.backend.quota.hanzo_insights.feature_enabled", return_value=flag_on) as flag_mock,
    ):
...
def test_self_driving_quota_gate_fails_open_on_flag_error():
    ...
    with (
        patch("products.signals.backend.quota.is_team_signals_quota_limited", return_value=True),
        patch(
            "products.signals.backend.quota.hanzo_insights.feature_enabled",
