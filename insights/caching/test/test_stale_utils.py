...
from insights.caching.utils import is_stale
...
def test_is_stale(team, date_to, interval, last_refresh, expected):
    with pytest.MonkeyPatch.context() as m:
        m.setattr("insights.caching.utils.stale_cache_invalidation_disabled", lambda t: team.name == "B")
...
def test_is_stale_with_target_age(team, date_to, interval, last_refresh, target_age, expected):
    ...
    with pytest.MonkeyPatch.context() as m:
        m.setattr("insights.caching.utils.stale_cache_invalidation_disabled", lambda t: False)
