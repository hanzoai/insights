...
use personinsights_writer::buffer::PersonBuffer;
use personinsights_writer::consumer::{ConsumerTask, FlushBatch};
use personinsights_writer::kafka::PersonConsumer;
use personinsights_writer::pg::PgStore;
use personinsights_writer::store::{
    BatchOutcome, PersonDb, PersonWriteStore, WriteError, WriteErrorKind,
};
use personinsights_writer::writer::WriterTask;
use prost::Message;
...
async fn writer_upserts_person_to_pg() {
    ...
    let row: (i64, i64, bool) = sqlx::query_as(
        "SELECT id, version, is_identified FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2",
    )
    .bind(team_id)
    .bind(1_i64)
    .fetch_one(&pool)
...
async fn writer_version_guard_skips_stale_updates() {
    ...
    let row: (i64,) =
        sqlx::query_as("SELECT version FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2")
            .bind(team_id)
            .bind(1_i64)
            .fetch_one(&pool)
            .await
...
async fn writer_upsert_batch_multiple_persons() {
    ...
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM personinsights_person_tmp WHERE team_id = $1")
            .bind(team_id)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn consumer_flushes_on_buffer_size_threshold() {
    ...
    loop {
        ...
        if retries > 50 {
            panic!(
                "Expected 6 rows in personinsights_person_tmp for team {}, got {}",
                team_id, count.0
            );
        }
...
async fn consumer_deduplicates_multiple_updates_for_same_person() {
    ...
    loop {
        let result: Option<(i64, i64)> = sqlx::query_as(
            "SELECT version, id FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2",
        )
        .bind(team_id)
        .bind(1_i64)
        .fetch_optional(&pool)
...
async fn writer_processes_batch_from_channel() {
    ...
    loop {
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2",
        )
        .bind(team_id)
        .bind(1_i64)
        .fetch_one(&pool)
        ...
        retries += 1;
        if retries > 50 {
            panic!("Row never appeared in personinsights_person_tmp");
        }
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
...
async fn writer_crashes_after_exhausting_transient_retries() {
    ...
    let store = PersonWriteStore::new(
        mock_db,
        personinsights_writer::store::StoreConfig {
            chunk_size: 10,
...
async fn properties_size_violation_errors_and_writes_nothing() {
    ...
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2")
            .bind(team_id)
            .bind(1_i64)
            .fetch_one(&pool)
            .await
...
async fn e2e_produce_to_kafka_and_verify_pg_write() {
    ...
    loop {
        let result: Option<(i64, i64)> = sqlx::query_as(
            "SELECT id, version FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2",
        )
        .bind(team_id)
        .bind(42_i64)
        .fetch_optional(&pool)
        ...
        retries += 1;
        if retries > 100 {
            panic!("Row never appeared in personinsights_person_tmp for team {team_id}, person 42");
        }
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
