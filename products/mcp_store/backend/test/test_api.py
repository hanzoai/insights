...
from products.mcp_store.backend.presentation.views import _is_valid_insights_code_callback_url
...
class TestIsValidInsightsCodeCallbackUrl(TestCase):
    @parameterized.expand(
        [
            ("array_scheme", "array://callback", True),
            ("twig_scheme", "twig://oauth/callback", False),
            ("insights_code_scheme", "insights-code://oauth/callback", True),
    ...
    def test_callback_url_validation(self, _name, url, expected):
        assert _is_valid_insights_code_callback_url(url) == expected
...
class TestMCPServerTemplateIconKeyNormalization(TestCase):
    @parameterized.expand(
        [
            ...
            ("multi_word", "Insights MCP", "insights_mcp"),
...
class TestMCPServerAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
...
class TestMCPGatewayServerAPI(APIBaseTest):
    ...
    def test_delete_disables_linked_installations_only(self) -> None:
        ...
        other_user = self._create_user("other-delete-member@hanzo.ai")
    ...
    def test_member_metadata_is_visible_only_to_admins(self, _name: str, retrieve: bool) -> None:
        self._make_admin()
        server, own_installation, _tool = self._server_with_personal_tool()
        other_user = self._create_user("other-gateway-member@hanzo.ai")
        other_installation = MCPServerInstallation.objects.create(
            team=self.team,
            user=other_user,
            display_name=server.name,
...
class TestMCPGatewayMemberAPI(APIBaseTest):
    ...
    def test_list_paginates_organization_members(self) -> None:
        self._make_admin()
        self._create_user("alice-gateway-member@hanzo.ai", first_name="Alice")
        self._create_user("zoe-gateway-member@hanzo.ai", first_name="Zoe")
    ...
    def test_retrieve_returns_one_members_connections_and_revocations(self) -> None:
        self._make_admin()
        member = self._create_user("gateway-member-detail@hanzo.ai")
    ...
    def test_retrieve_does_not_expose_members_from_another_organization(self) -> None:
        ...
        other_member = User.objects.create_and_join(
            other_organization,
            "other-organization-gateway-member@hanzo.ai",
            "password",
        )
...
class TestMCPServiceAccountAPI(APIBaseTest):
    ...
    def _active_scout_account(self) -> MCPServiceAccount:
        return next(agent for agent in sync_built_in_agents(self.team) if agent.handle == "insights-scout")
    ...
    def test_list_materializes_only_the_fixed_insights_agent_catalog(self) -> None:
        self._make_admin()
        ...
        assert [agent["handle"] for agent in results] == ["insights-support", "insights-scout"]
    ...
    def test_list_reconciles_legacy_built_in_agent_handles(self) -> None:
        ...
        assert support_account.handle == "insights-support"
    ...
    def test_agent_gateway_rate_limits_are_scoped_per_service_account(
        ...
        accounts = sync_built_in_agents(self.team)
        primary_account = next(account for account in accounts if account.handle == "insights-scout")
        secondary_account = next(account for account in accounts if account.handle == "insights-support")
...
class TestMCPServerInstallationAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_list_installation_icon_fields_from_template(self):
        ...
        # until Insights Desktop stops reading it.
        template = MCPServerTemplate.objects.create(
            name="Insights MCP",
...
class TestInstallCustomAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    @ALLOW_URL
    ...
    def test_install_custom_rejects_invalid_insights_code_callback_url(self):
        response = self.client.post(
            ...
            data={
                ...
                "auth_type": "api_key",
                "install_source": "insights-code",
                "insights_code_callback_url": "https://evil.com/steal",
            },
            format="json",
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_install_custom_accepts_insights_code_callback_url(self, _mock):
        response = self.client.post(
            ...
            data={
                ...
                "auth_type": "api_key",
                "install_source": "insights-code",
                "insights_code_callback_url": "insights-code://oauth/callback",
            },
            format="json",
        )
        assert response.status_code == status.HTTP_201_CREATED
...
class TestOAuthCallback(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def _create_oauth_state(
        ...
        state_token,
        pkce_verifier="",
        install_source="insights",
        insights_code_callback_url="",
        *,
        template=None,
        created_by=None,
    ):
        ...
        return MCPOAuthState.objects.create(
            ...
            insights_code_callback_url=insights_code_callback_url,
    ...
    @patch("products.mcp_store.backend.oauth.is_url_allowed", return_value=(True, None))
    @patch("products.mcp_store.backend.oauth.requests.post")
    def test_oauth_redirect_uses_insights_code_callback_url(self, mock_post, _allow):
        installation = self._create_installation()

        mock_post.return_value.status_code = 200
        mock_post.return_value.json.return_value = {"access_token": "tok", "token_type": "bearer"}

        callback_url = "insights-code://oauth/callback"
        state_token = "test-insights-code-state"
        self._create_oauth_state(
            installation,
            state_token,
            pkce_verifier="test-verifier",
            install_source="insights-code",
            insights_code_callback_url=callback_url,
        )
        ...
        assert location.startswith("insights-code://oauth/callback?")
        assert "status=success" in location

    @patch("products.mcp_store.backend.oauth.is_url_allowed", return_value=(True, None))
    @patch("products.mcp_store.backend.oauth.requests.post")
    def test_oauth_redirect_insights_code_error_includes_error_param(self, mock_post, _allow):
        installation = self._create_installation()

        callback_url = "insights-code://oauth/callback"
        state_token = "test-insights-code-error"
        self._create_oauth_state(
            installation, state_token, install_source="insights-code", insights_code_callback_url=callback_url
        )
    ...
    @patch("products.mcp_store.backend.oauth.is_url_allowed", return_value=(True, None))
    @patch("products.mcp_store.backend.oauth.requests.post")
    def test_callback_rejects_anonymous_insights_consumer_in_dev(self, mock_post, _allow, _is_dev):
        mock_post.return_value.status_code = 200
        mock_post.return_value.json.return_value = {"access_token": "tok", "token_type": "bearer"}

        installation = self._create_installation()
        state_token = "anonymous-insights-dev-callback"
        self._create_oauth_state(installation, state_token, pkce_verifier="v", install_source="insights")
    ...
    def test_callback_rejects_state_for_different_authenticated_user(self, _is_dev):
        ...
        Covers the scenario where the victim IS logged into Insights but as a
    ...
    def test_callback_happy_path_cross_client_same_user(self, mock_post, _allow):
        """insights-code scenario: initiate and callback happen in different HTTP clients.
...
class TestOAuthIssuerSpoofingProtection(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
...
class TestMCPAuthorizeInsightsCodeResponse(APIBaseTest):
...
class TestInstallTemplateAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
...
class TestInstallationToolsAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
...
class TestInstallDispatchesToolSync(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    @ALLOW_URL
...
class TestMCPInstallationScopeAccess(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_list_shows_own_personal_and_all_shared(self) -> None:
        ...
        other = User.objects.create_and_join(self.organization, "other@hanzo.ai", "password")
...
class TestMCPScopeAdminGateWithAccessControlFeature(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def _other_user(self):
        ...
        return User.objects.create_and_join(self.organization, "other@hanzo.ai", "password")
