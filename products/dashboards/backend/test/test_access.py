...
class TestDashboardAccessMetrics:
    ...
    def test_records_dashboard_access(self, access_method: DashboardAccessMethod) -> None:
        ...
        before = REGISTRY.get_sample_value("insights_dashboard_access_total", labels) or 0

        record_dashboard_access(access_method)

        assert REGISTRY.get_sample_value("insights_dashboard_access_total", labels) == before + 1
    ...
    def test_records_dashboard_cache_outcome(
        ...
        before = REGISTRY.get_sample_value("insights_dashboard_cache_outcome_total", labels) or 0

        record_dashboard_cache_outcome(access_method, is_cached=is_cached)

        assert REGISTRY.get_sample_value("insights_dashboard_cache_outcome_total", labels) == before + 1
