...
from insights.egress.github.transport import github_request, raise_if_github_rate_limited
...
from insights.models.integration import invalidate_github_repository_caches_for_installation
from insights.models.utils import UUIDModel

if TYPE_CHECKING:
    from insights.models.integration import GitHubInstallationAccess, GitHubUserAuthorization
...
class UserIntegration(UUIDModel):
    ...
    Cardinality (Slack):
    - One Insights user → many rows, one per Slack workspace they've linked
    ...
    - One Slack identity (workspace × Slack user id) → may have multiple
      Insights rows when the same Slack account is linked to more than one
      Insights account (e.g. personal + work). The inbound resolver picks
      the most-recently-linked accessible row and warns when it sees more
      than one match — see `find_linked_insights_user`.

    The `unique_together = ("user", "kind", "integration_id")` constraint only
    forbids the same Insights user linking the same Slack workspace identity
    twice (which would have to be a re-OAuth that `update_or_create` already
    folds into an in-place refresh). It does NOT forbid two Insights users
