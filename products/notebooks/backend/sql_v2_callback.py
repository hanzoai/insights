...
Mirrors Insights Desktop's agent-proxy callback: a plain function view (no team
scoping, no session), authed by a Bearer token the run endpoint minted. Wired in
insights/urls.py at internal/notebooks/runs/<run_id>/result/.
"""
...
def _store_frame_snapshot(run: NotebookNodeRun, frames: list | None, team_id: int) -> None:
    ...
    `frames` is absent for a insightsql run (it never enters the kernel) and for a kernel whose
    catalog read failed. Both mean "leave the stored snapshot alone" — a insightsql run changes no
