...
class ColumnConfigurationSerializer(serializers.ModelSerializer):
    ...
    order_by = serializers.ListField(
        ...
        allow_null=True,
        allow_empty=True,
        help_text="Ordered list of InsightsQL expressions describing the table sort. Null preserves the current sort on apply (legacy rows); an empty list explicitly means no sort.",
    )
