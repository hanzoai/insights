...
from insights.models import Organization, Person, PropertyDefinition, Team
...
from insights.models.person.sql import PERSON_DISTINCT_ID2_TABLE
from insights.models.person.util import (
...
from insights.test.persons import add_distinct_id, create_person, delete_person
...
class TestPerson(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_person_property_values_refresh(self, _name, param, expected_mode_name):
        from insights.insightsql_queries.property_values_query_runner import PropertyValuesQueryResponse
        ...
        url = "/v1/person/values/?key=country"
        if param:
            url += f"&{param}"

        with mock.patch(
            "insights.insightsql_queries.property_values_query_runner.PropertyValuesQueryRunner.run",
    ...
    def test_filter_person_list(self):
        ...
        response = self.client.get("/v1/person/?distinct_id=inexistent&include_total")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_person(self):
        ...
        with snapshot_postgres_queries_context(
            ...
            custom_query_matcher=lambda query: (
                f"DELETE FROM insights_person WHERE team_id = {self.team.pk} AND id = {person.pk}" in query
            ),
        ):
    ...
    @freeze_time("2021-08-25T22:09:14.252Z")
    @mock.patch("insights.api.person.queue_person_recording_deletion")
    ...
    def test_deletion_status_lists_pending_deletions(self):
        ...
        response = self.client.get(f"/v1/person/deletion_status/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
    ...
    def test_deletion_status_filters_by_status(self):
        ...
        # Filter pending
        response = self.client.get(f"/v1/person/deletion_status/?status=pending")
        data = response.json()
        self.assertEqual(data["count"], 1)
        ...
        # Filter completed
        response = self.client.get(f"/v1/person/deletion_status/?status=completed")
        data = response.json()
        self.assertEqual(data["count"], 1)
    ...
    def test_deletion_status_filters_by_person_uuid(self):
        ...
        response = self.client.get(f"/v1/person/deletion_status/?person_uuid={person1.uuid}")
        data = response.json()
        self.assertEqual(data["count"], 1)
        self.assertEqual(data["results"][0]["person_uuid"], str(person1.uuid))

    @mock.patch("insights.models.person.bulk_delete.delete_person")
    ...
    def test_deletion_status_rejects_invalid_status(self):
        ...
        response = self.client.get(f"/v1/person/deletion_status/?status=invalid")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_deletion_status_rejects_invalid_person_uuid(self):
        """Test that deletion_status returns 400 for non-UUID person_uuid"""
        response = self.client.get(f"/v1/person/deletion_status/?person_uuid=not-a-uuid")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_split_people_partial_moves_only_specified_ids(self) -> None:
        ...
        with fake_personinsights_client() as fake:
            ...
            response = self.client.post(
                "/v1/person/{}/split/".format(person1.pk),
    ...
    def test_new_delete_person_properties(self, _name, unset, expected, mock_capture) -> None:
        ...
        self.client.post(f"/v1/person/{person.uuid}/delete_property", {"$unset": unset}, format="json")
    ...
    def test_delete_property_rejects_invalid_or_forbidden_unset(self, _name, body, non_writable, mock_capture) -> None:
        ...
        with mock.patch(
            "insights.api.person.PersonViewSet._get_non_writable_person_properties",
    ...
    @mock.patch("insights.api.person.capture_internal")
    @mock.patch("insights.api.person.get_person_by_pk_or_uuid")
    def test_delete_property_rejects_person_without_distinct_ids(self, mock_get_person, mock_capture) -> None:
        ...
        response = self.client.post(
            f"/v1/person/{uuid4()}/delete_property",
            {"$unset": "foo"},
            format="json",
        )
    ...
    def test_update_person_property_by_numeric_id(self, mock_capture) -> None:
        ...
        self.client.post(f"/v1/person/{person.id}/update_property", {"key": "foo", "value": "bar"})
    ...
    def test_delete_person_property_by_numeric_id(self, mock_capture) -> None:
        ...
        self.client.post(f"/v1/person/{person.id}/delete_property", {"$unset": "foo"})
    ...
    def test_update_person_property_with_null_value(self, mock_capture) -> None:
        ...
        response = self.client.post(
            f"/v1/person/{person.uuid}/update_property",
            {"key": "foo", "value": None},
            format="json",
        )
    ...
    def test_person_cohorts_via_nested_project_url(self) -> None:
        ...
        response = self.client.get(f"/v1/projects/{self.team.id}/persons/cohorts/?person_id={person.uuid}")
        self.assertEqual(response.status_code, 200, response.json())
        self.assertEqual(len(response.json()["results"]), 1)
    ...
    def test_split_person_datastore(self):
        ...
        # Datastore ends up with the original person plus one new person per
        ...
        self.assertCountEqual(datastore_people, [(uuid,) for uuid in expected_person_by_did.values()])
    ...
    def test_pagination_no_gaps_or_duplicates_when_created_at_is_tied(self):
        ...
        full = [row["id"] for row in self.client.get("/v1/person/?limit=100").json()["results"]]
        ...
        paged: list[str] = []
        url: Optional[str] = "/v1/person/?limit=2"
        while url:
    ...
    def test_reset_person_distinct_id(self):
        ...
        # Verify: personinsights distinct_id version was bumped
        ...
        # Verify: personinsights person version was bumped so future plugin-server updates aren't ignored
    ...
    def test_reset_person_distinct_id_not_found(self, mocked_ch_call):
        ...
        # personinsights: only the non-deleted distinct_id still resolves to its person
...
class TestPersonBatchRestrictedProperties(DatastoreTestMixin, APIBaseTest):
    ...
    def test_batch_endpoint_strips_restricted_person_properties(self, action: str) -> None:
        ...
        response = self.client.post(
            f"/v1/environments/{self.team.id}/persons/{action}/",
            body,
            format="json",
        )
