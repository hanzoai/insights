...
exercise ``_build_insights_code_task_description`` directly so the prompt shape
...
from insights.temporal.ai.slack_app.activities.task_creation import (
    ...
    _build_insights_code_task_description,
...
def test_build_description_includes_delivery_constraints_when_thread_has_only_initiator():
    ...
    out = _build_insights_code_task_description(
        "do something",
...
def test_build_description_falls_back_to_default_prompt_when_initiator_text_is_blank():
    out = _build_insights_code_task_description("   ", [], None, canvas_file_artifacts_enabled=True)
    assert _SLACK_DELIVERY_CONSTRAINTS in out
...
def test_build_description_collapses_role_annotations_when_same_person():
    ...
    assert "Thread started by and tagged the Insights app: <@U_GEORGIY|georgiy>" in out
...
def test_build_description_separates_role_annotations_when_different_people():
    ...
    assert "Tagged the Insights app: <@U_THEO|theo lin>" in out
...
def test_build_description_falls_back_to_plain_name_for_bot_authors():
    # Bot posts (Insights alerts, Grafana, etc.) arrive without a `U…` id. We want
