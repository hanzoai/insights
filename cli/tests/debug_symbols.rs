...
use insights_cli::debug_symbols::{
    dedup_uploads_by_chunk_id, discover, elf_debug_id, package_dsym_bundles, report_problems,
};
use insights_cli::dsym::source_bundle::extract_source_paths_from_dwarf_bytes;
use insights_symbol_data::{read_symbol_data, AppleDsym, ElfDebugInfo};
...
fn classifies_elfs_without_debug_info_or_build_id() {
    ...
    let err = insights_cli::debug_symbols::report_problems(&report, dir.path()).unwrap_err();
    ...
    assert!(insights_cli::debug_symbols::report_problems(&report, dir.path()).is_ok());
}
...
fn flags_compressed_dwarf_macho_with_guidance() {
    ...
    assert!(insights_cli::debug_symbols::report_problems(&report, dir.path()).is_err());
}
