...
All queries run through the Insights MCP `execute-sql` tool in the internal project (US, project 2).
...
FROM postgres.insights_user u WHERE u.email = '{email}'
UNION ALL
SELECT 'eu', u.email, u.current_team_id
FROM eu_postgres_insights_user u WHERE u.email = '{email}'
```

Also useful: `postgres.insights_organizationdomain` (verified domains only — often empty for smaller orgs), `postgres.insights_organization` by name, `all_insights_team`.

EU customer data is not queryable from the US MCP project.
For per-team event data on either region, use the `query-datastore-via-metabase` skill.

## Error tracking cross-reference

Frontend crashes reported by EU customers usually have US occurrences too (Insights staff and US users hit the same code).
