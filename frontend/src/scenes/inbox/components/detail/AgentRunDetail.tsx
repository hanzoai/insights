...
import { TaskRunStatusDot } from 'products/insights_ai/frontend/api/primitives'
import { ReadonlyRunSurface } from 'products/insights_ai/frontend/api/readableRun'
import { isPiTaskRuntime, Task, TaskRunStatus } from 'products/insights_ai/frontend/types/taskTypes'
...
function RunOutputWidget({ report }: { report: SignalReport }): JSX.Element {
    ...
    return (
        <DetailSection icon={<IconDocument />} title="Draft summary">
            {report.summary ? (
                <Markdown className="text-sm text-secondary leading-normal" disableImages>
                    {report.summary}
                </Markdown>
            ) : (
...
/**
 * "Open task": continues the selected task in a new Insights AI chat bound to it — the chat's first
 * message creates a conversation linked to this task and resumes its run interactively. A plain click
 * opens Insights AI in the side panel (staying in the inbox); cmd/ctrl/middle-click follows the href to
...
export function OpenTaskButton({
    ...
    return (
        <Button
            ...
            tooltip="Continue this task in a new Insights AI chat"
        >
            Open task
        </Button>
    )
 ...
 * `ReadonlyRunSurface` — live for an in-progress run, static replay once terminal. A `Select`
 * switches between linked tasks (research / implementation) when there's more than one; "Open task"
 * continues the task in a new Insights AI chat. Mirrors desktop `AgentRunDetail`'s Task-log section.
 */
            ...
            <div className="flex items-center gap-2 flex-wrap mb-4">
                ...
                {isReResearch && (
                    <Tooltip title="A prior research run on this report already completed – this is a re-attempt.">
                        <Tag size="small" type="warning" className="cursor-help select-none">
                            Re-research
                        </Tag>
                    </Tooltip>
                )}
