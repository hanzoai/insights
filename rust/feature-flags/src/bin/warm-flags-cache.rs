...
struct Cli {
    ...
    /// Warm every team in `insights_team`, not just teams that have ever had a flag.
...
struct InfraConfig {
    #[envconfig(
        ...
        default = "postgres://insights:insights@localhost:5432/insights"
    )]
    ...
    #[envconfig(from = "OBJECT_STORAGE_BUCKET", default = "insights")]
...
async fn build_team_plan(cli: &Cli, pg_reader: &PostgresReader) -> TeamPlan {
    ...
    if cli.all_teams {
        tracing::info!("Found {} teams (all teams in insights_team)", total);
    } else {
    ...
    if supplied.is_empty() || !validated.is_empty() {
        ...
        Some(format!(
            "All supplied team IDs were invalid: {supplied:?}. \
             Check the values against insights_team."
        ))
    }
}
...
async fn validate_team_ids(pg_reader: &PostgresReader, supplied: &[TeamId]) -> Vec<TeamId> {
    ...
    let rows: Vec<(i32,)> = sqlx::query_as("SELECT id FROM insights_team WHERE id = ANY($1)")
        .bind(supplied)
        .fetch_all(&mut *conn)
        .await
    ...
    if !missing.is_empty() {
        tracing::warn!(
            missing = ?missing,
            "Some supplied team IDs do not exist in insights_team — skipping"
        );
    }
...
async fn count_teams(pg_reader: &PostgresReader, all_teams: bool) -> usize {
    ...
    let query = if all_teams {
        "SELECT COUNT(*) FROM insights_team"
    } else {
        "SELECT COUNT(*) FROM insights_team t \
         WHERE EXISTS (SELECT 1 FROM insights_featureflag f WHERE f.team_id = t.id)"
    };
...
async fn fetch_team_id_page(
    ...
    let query = if all_teams {
        "SELECT id FROM insights_team WHERE id > $1 ORDER BY id LIMIT $2"
    } else {
        "SELECT t.id FROM insights_team t \
         WHERE t.id > $1 \
         AND EXISTS (SELECT 1 FROM insights_featureflag f WHERE f.team_id = t.id) \
...
mod tests {
    ...
    async fn test_persist_flags_cache_writes_etag_and_skips_del() {
        ...
        let data_key = "insights:1:cache/teams/42/feature_flags/flags.json";
        let etag_key = "insights:1:cache/teams/42/feature_flags/flags.json:etag";
