...
def _validate_resource_graph(manifest: dict[str, Any]) -> dict[str, Optional[list[ResolvedParam]]]:
    ...
    ancestor full-scans on every run — and no shipped fan-out source (Insights
...
def validate_manifest_urls(manifest: dict[str, Any], team_id: int) -> tuple[bool, str | None]:
    ...
    Also enforces ``https://`` on Insights Cloud. Self-hosted instances skip
...
def _check_url(url: str, team_id: int) -> tuple[bool, str | None]:
    ...
    if is_cloud() and urlparse(url).scheme != "https":
        return False, f"URL {url!r} must use https:// on Insights Cloud"
...
class CustomSource(SimpleSource[CustomSourceConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption=(
                ...
                "as Insights's built-in REST sources — see the docs for the field reference."
            ),
            iconPath="/static/insights-icon.svg",
            docsUrl=None,
    ...
    def source_for_pipeline(self, config: CustomSourceConfig, inputs: SourceInputs) -> SourceResponse:
        ...
        # match Insights's other REST sources. An incorrect "asc" assumption on a
