...
2. **flox `[vars]`** (`.flox/env/manifest.toml`) — injected on every `flox activate`, which `direnv` runs automatically via `.envrc` whenever you `cd` into the repo. This is where `DEBUG=1`, `DATASTORE_DATABASE`, and other always-on dev knobs live. These are **not** in any `.env` file, which is why grepping `.env*` for them comes up empty.
...
`DEBUG` is on by default in local dev (flox `[vars]`), and there is a hard guard (`insights/settings/utils.py`) that refuses to boot with `DEBUG` **and** `CLOUD_DEPLOYMENT` in `US`/`EU`/`DEV` — because `DEBUG` relaxes authentication, so it must never coincide with a real deployed-cloud identity.
...
See also [sandboxes-setup-guide.md](sandboxes-setup-guide.md) for the Insights Desktop sandbox providers.
