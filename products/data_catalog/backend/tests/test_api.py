...
from insights.rate_limit import DatastoreBurstRateThrottle, InsightsQLQueryThrottle
...
class TestMetricAPI(APIBaseTest):
    ...
    def test_refine_via_post_preserves_omitted_definition(self) -> None:
        ...
        assert body["definition"]["kind"] == "InsightsQLQuery"
...
class TestMetricLifecycleAPI(APIBaseTest):
    ...
    def test_list_computes_drift_in_a_single_insight_query(self) -> None:
        ...
        insight_queries = [q for q in ctx.captured_queries if "insights_dashboarditem" in q["sql"]]
    ...
    def test_run_rejects_date_params_on_insightsql_metric(self) -> None:
        upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL)
    ...
    def test_run_reports_drift_on_approved_metric(self) -> None:
        ...
        with patch(_PROCESS_QUERY, return_value={"results": [[1]], "insightsql": "SELECT 1"}):
            response = self.client.post(f"{self.url}mrr/run/")
        assert response.status_code == status.HTTP_200_OK, response.json()
        body = response.json()
...
class TestMetricRunThrottles(APIBaseTest):
    ...
    def test_insightsql_metric_uses_insightsql_query_throttle(self) -> None:
        upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL)
        deny, wait = self._denied(InsightsQLQueryThrottle)
        with deny, wait:
            response = self.client.post(f"{self.url}mrr/run/")
        assert response.status_code == status.HTTP_429_TOO_MANY_REQUESTS

    def test_structured_metric_uses_datastore_throttles(self) -> None:
        upsert_metric(
            team=self.team,
            user=self.user,
            name="purchases",
        ...
        deny, wait = self._denied(DatastoreBurstRateThrottle)
        with deny, wait:
    ...
    def test_markdown_metric_keeps_default_throttles(self) -> None:
        ...
        insightsql_deny, insightsql_wait = self._denied(InsightsQLQueryThrottle)
        ch_deny, ch_wait = self._denied(DatastoreBurstRateThrottle)
        with insightsql_deny, insightsql_wait, ch_deny, ch_wait:
            response = self.client.post(f"{self.url}activation/run/")
        assert response.status_code == status.HTTP_200_OK
