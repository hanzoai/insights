...
- **Builtin events without saved insights** — `$pageview`, `$autocapture`, `$identify`, `$set`, `$opt_in`, `$groupidentify`, `$feature_flag_called` are surfaced through Insights's product views (Web Analytics, Feature Flags) without needing a custom insight. Don't recommend creating one.
...
For deeper investigation playbooks, the sandbox image bakes upstream Insights skills: `insights:querying-insights-data` (InsightsQL syntax + system.\* search patterns) and `insights:exploring-autocapture-events` (custom-event vs autocapture distinctions, when each lens applies).
