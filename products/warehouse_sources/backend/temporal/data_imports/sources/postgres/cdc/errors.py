...
# so the connection never leaves Insights. Unlike a refused or reset connection — where the host is
...
# Insights can't route to), so retrying re-hits the same wall. Mirrors the non-retryable treatment
