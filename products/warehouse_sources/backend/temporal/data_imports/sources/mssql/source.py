...
_FIREWALL_BLOCKED_ERROR = (
    "Your SQL Server's firewall is blocking Insights. Add a firewall rule allowing Insights's IP "
...
class MSSQLSource(SQLSource[MSSQLSourceConfig], SSHTunnelMixin, ValidateDatabaseHostMixin):
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            # Azure SQL error 40615 — the server-level firewall rejected Insights's client IP. This
            ...
            # (security group doesn't allow Insights's IPs, the instance is stopped, or the
            # hostname is wrong), not a momentary blip, so retrying the job won't recover it.
            "Adaptive Server is unavailable or does not exist": "Could not reach your SQL Server. Check that the server is running and reachable, and that Insights's IP addresses are allowed through its firewall / security group.",
            ...
            "Login failed for user": "Invalid user or password",
            # SQL Server error 229 — the login Insights connects with was authenticated but lacks
            ...
            # wrong, the SSH key/credentials are rejected, or a firewall blocks Insights's IPs. This
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/microsoft-sql-server",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
