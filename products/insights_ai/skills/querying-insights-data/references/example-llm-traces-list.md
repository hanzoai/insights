...
This content lives only on `insights.ai_events` (not `events`), retained 30 days by default — read it anchored on `trace_id`. See [AI observability events](./models-ai-observability-events.md) for the column mapping and access patterns.
