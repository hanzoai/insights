...
const lines = [
    'What the toolbar ships to customer pages, measured from the esbuild *output* (minified, post-tree-shake). ' +
        ...
        '[check-toolbar-graph](https://github.com/Insights/insights/blob/master/frontend/bin/check-toolbar-graph.mjs).',
    '',
...
lines.push('', '</details>', '')
lines.push(
    '<sub>Posted automatically by [check-toolbar-size](https://github.com/Insights/insights/blob/master/frontend/bin/check-toolbar-size.mjs) · sizes are toolbar output bytes (shipped, post-tree-shake) from the esbuild metafile</sub>'
)
