...
``logs/`` path) — today to walk prior ``_insights/user_message`` notifications for context
dedup on follow-ups. Live SSE frames are consumed exclusively by the frontend, where the full
contract is typed at ``products/insights_ai/frontend/sandbox/types/sandboxWireTypes.ts``; keep that copy in
...
METHOD_USER_MESSAGE = "_insights/user_message"


class UserMessageParams(TypedDict):
    """Params of a ``_insights/user_message`` notification — the user's message as the
