...
and columns that are `null=True` at the DB layer (`InsightsFunction.type`/`kind`,
...
class InsightsFunctionEntry(_Section):
    id: str
    name: str
    type: str | None
    kind: str | None
...
class RecentInsightsFunctions(_Section):
    total_count: int
    enabled_count: int
    recent: list[InsightsFunctionEntry]


class InsightsFlowEntry(_Section):
    id: str
    name: str
    status: str
    updated_at: str | None
...
class RecentInsightsFlows(_Section):
    total_count: int
    active_count: int
    recent: list[InsightsFlowEntry]
...
class Inventory(_Section):
    ...
    stable key order across builds. `top_events` is `None` (not `[]`) when the Datastore
