...
from insightscli.manifest import Manifest
...
class TestManifestCommandEnumeration:
    ...
    def test_get_all_commands_excludes_config_section(self) -> None:
        ...
        test_data = {
            "config": {"commands_dir": "tools/insightscli-commands/insightscli_commands"},
            "core": {
                "dev:setup": None,
...
class TestCommandsDir:
    ...
    def test_commands_dir_is_explicit(self, tmp_path: Path) -> None:
        (tmp_path / "insightscli").mkdir()

        with (
            patch.object(Manifest, "_load", return_value={"config": {}}),
            patch("insightscli.manifest.REPO_ROOT", tmp_path),
            patch("insightscli.manifest.MANIFEST_FILE", tmp_path / "insightscli.yaml"),
        ):
            manifest = Manifest()
            assert manifest.commands_dir is None

    def test_commands_dir_resolves_configured_path(self, tmp_path: Path) -> None:
        commands_dir = tmp_path / "tools" / "insightscli-commands" / "insightscli_commands"
        ...
        with (
            patch.object(
                Manifest,
                "_load",
                return_value={"config": {"commands_dir": "tools/insightscli-commands/insightscli_commands"}},
            ),
            patch("insightscli.manifest.REPO_ROOT", tmp_path),
            patch("insightscli.manifest.MANIFEST_FILE", tmp_path / "insightscli.yaml"),
        ):
...
class TestEnvConfig:
    ...
    def manifest_at(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        def _build(config: dict) -> Manifest:
            monkeypatch.setattr("insightscli.manifest.REPO_ROOT", repo)
            monkeypatch.setattr("insightscli.manifest.MANIFEST_FILE", repo / "insightscli.yaml")
    ...
    def test_unset_returns_empty_and_none(self, manifest_at) -> None:
        """Without `config.env`, both accessors stay quiet — generic insightscli is transparent."""
