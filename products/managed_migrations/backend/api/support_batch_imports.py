...
from insights.permissions import APIScopePermission, IsStaffUser
...
class BatchImportSupportListSerializer(serializers.ModelSerializer):
    """Compact cross-team diagnostics view of a batch import job for Insights support staff.
    ...
    display_status_message = serializers.SerializerMethodField(
        help_text="Customer-facing status message shown in the Insights UI. Embedded URLs are redacted the same way as status_message."
    )
...
@extend_schema_view(
    list=extend_schema(
        description="List batch import (managed migration) jobs across all teams. Insights staff only.",
    ),
    retrieve=extend_schema(
        description="Get one batch import job with its raw worker state and import config. Insights staff only.",
    ),
)
