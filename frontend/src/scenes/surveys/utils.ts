...
export function validateSurveyAppearance(
    ...
    // API surveys are rendered by the customer, so Insights's appearance CSS is not applied.
...
export function getSurveyResponse(question: SurveyQuestion, index: number): string {
    // Delegate to the backend InsightsQL helper so survey response typing stays
 ...
 * Mirrors the backend logic in insights/tasks/update_survey_iteration.py, which computes iteration windows
...
export function buildAggregateQuery(
    ...
    const baseWhere = `event = '${SurveyEventName.SENT}'
        ...
        ${filters.answerFilterInsightsQLExpression}
...
export function captureMaxAISurveyCreationException(error?: string, source?: SURVEY_CREATED_SOURCE): void {
    insights.captureException(error || 'Undefined error when creating MaxAI survey', {
...
export function getExpressionCommentForQuestion(
    ...
    if (question.length > 0) {
        // This is appended after `--` in the generated InsightsQL, and InsightsQL `--` comments are
