...
import { EVALUATION_PASSED_INSIGHTSQL, EVALUATION_SUMMARY_MAX_RUNS } from './evaluations/constants'
...
export async function queryEvaluationRunsStats(params: {
    ...
    const query = insightsql`
        SELECT
            ...
            countIf(${insightsql.raw(EVALUATION_PASSED_INSIGHTSQL)}) as passed
        FROM events
        WHERE
            event = '$ai_evaluation'
            AND ${insightsql.raw(`properties.${propertyName}`)} = ${propertyValue}
