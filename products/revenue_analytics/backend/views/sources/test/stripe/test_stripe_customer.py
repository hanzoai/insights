...
from insights.insightsql.database.schema.test.base import RevenueAnalyticsTestBase
...
class TestCustomerStripeMetadataResolution(RevenueAnalyticsTestBase):
    ...
    def setUp(self):
        ...
        self.team.save()
        self.view_name = f"stripe.insights_test.{CUSTOMER_SCHEMA.source_suffix}"
    ...
    def test_metadata_resolution_from_customer(self):
        """cus_1 has insights_person_distinct_id set directly on the customer object."""
        results = self._query_metadata()

        self.assertEqual("person_cus_1", results["cus_1"]["insights_person_distinct_id"])
        self.assertEqual("customer", results["cus_1"]["insights_person_distinct_id_source"])

    def test_metadata_resolution_from_subscription(self):
        """cus_2 has no distinct_id on the customer, but sub_2 has it in metadata."""
        results = self._query_metadata()

        self.assertEqual("person_cus_2", results["cus_2"]["insights_person_distinct_id"])
        self.assertEqual("subscription::sub_2", results["cus_2"]["insights_person_distinct_id_source"])

    def test_metadata_resolution_from_charge(self):
        """cus_3 has no distinct_id on the customer, but ch_3 has it in metadata."""
        results = self._query_metadata()

        self.assertEqual("person_cus_3", results["cus_3"]["insights_person_distinct_id"])
        self.assertEqual("charge::ch_3", results["cus_3"]["insights_person_distinct_id_source"])

    def test_no_metadata_when_absent_everywhere(self):
        """cus_4 has no distinct_id on customer, subscriptions, or charges."""
        results = self._query_metadata()

        self.assertNotIn("insights_person_distinct_id", results["cus_4"])
        self.assertNotIn("insights_person_distinct_id_source", results["cus_4"])
    ...
    def test_freshest_child_wins_across_types(self):
        ...
        self.assertEqual("person_cus_5_from_charge", results["cus_5"]["insights_person_distinct_id"])
        self.assertEqual("charge::ch_15", results["cus_5"]["insights_person_distinct_id_source"])
