...
// Re-exported so the codegen picks them up and emits matching `StrEnum`s in insights/schema.py.
...
export interface EndpointRequest {
    name?: string
    description?: string
    query?: InsightsQLQuery | EndpointQueryNode
    is_active?: boolean
...
export interface AnalyticsQueryResponseBase {
     ...
     * Warnings about data warehouse sources referenced by the query whose latest sync failed,
     * is paused, hit a billing limit, or is otherwise stale. Results may not reflect current source data.
     * Accumulated across every InsightsQL execution that contributes to this response — so insights backed
     * by warehouse tables (Trends, Funnels, etc.) receive the same warnings as raw InsightsQL queries.
     * Also carries access control warnings when a system-table query filters out objects the user can't access.
     */
    warnings?: (DataWarehouseSyncWarning | AccessControlFilterWarning)[]
...
export interface AccountsQuery extends DataNode<AccountsQueryResponse> {
    ...
    filterExpression?: InsightsQLExpression
    orderBy?: string[]
    limit?: integer
    offset?: integer
}
...
export interface LogsQuery extends DataNode<LogsQueryResponse> {
     ...
     * InsightsQL expression (`upper(level)`, `coalesce(attributes['a'], attributes['b'])`).
...
export interface ExperimentTrendsQuery extends DataNode<ExperimentTrendsQueryResponse> {
    ...
    // https://github.com/Insights/insights/blob/master/insights/insightsql_queries/experiments/experiment_trends_query_runner.py
    exposure_query?: TrendsQuery
    fingerprint?: string
}
...
export interface ExperimentApiEventSource {
     ...
     *  Other options: 'avg', 'min', 'max', 'unique_session', 'dau', 'unique_group', 'insightsql'. */
    math?: ExperimentMetricMathType
    /** Numeric event property to aggregate when math is 'sum', 'avg', 'min', or 'max' (e.g. 'revenue'). */
    math_property?: string
    /** InsightsQL aggregation expression. Required when math is 'insightsql' — without it the metric silently
...
export type InsightsQLAlertEvaluation = 'last_row' | 'first_row' | 'any_row'

/** Alert config for InsightsQL/SQL-backed insights. The query owns its own time window. */
export interface InsightsQLAlertConfig {
    ...
    evaluation: InsightsQLAlertEvaluation
...
// with backend `HIERARCHY_BASE_COLUMNS` (products/marketing_analytics/backend/insightsql_queries/constants.py).
