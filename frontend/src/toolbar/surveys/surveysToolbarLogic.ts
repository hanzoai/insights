...
export interface surveysToolbarLogicValues {
    allSurveys: Survey[]
    allSurveysLoading: boolean
    canProceed: boolean
    canProceedReason: string | null
    ...
    previewSurvey: Survey | null
    quickForm: QuickSurveyForm
    searchTerm: string
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface surveysToolbarLogicActions {
    archiveSurvey: (survey: Survey) => {
        survey: Survey
    }
    cancelQuickCreate: () => {
        value: true
    ...
    resumeSurvey: (survey: Survey) => {
        survey: Survey
    }
    setFormField: (
        field: keyof QuickSurveyForm,
        value: unknown
    ) => {
        field: keyof QuickSurveyForm
        value: unknown
    }
    setHasMoreSurveys: (hasMore: boolean) => {
        hasMore: boolean
    }
    setLifecyclePending: (pending: boolean) => {
        pending: boolean
    }
    ...
    startQuickEdit: (survey: Survey) => {
        survey: Survey
    }
    stopSurvey: (survey: Survey) => {
        survey: Survey
    }
    submitQuickCreate: (launch?: boolean) => {
        launch: boolean
    }
    submitQuickCreateFailure: () => {
        value: true
    }
    submitQuickCreateSuccess: () => {
        value: true
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface surveysToolbarLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        previewSurvey: (quickForm: QuickSurveyForm, isCreating: boolean, editingSurvey: Survey | null) => Survey | null
        canProceed: (quickForm: QuickSurveyForm) => boolean
        canProceedReason: (quickForm: QuickSurveyForm) => string | null
    }
}

export type surveysToolbarLogicType = MakeLogicType<
    surveysToolbarLogicValues,
    surveysToolbarLogicActions,
    Record<string, any>,
    surveysToolbarLogicMeta
>
...
export const surveysToolbarLogic = kea<surveysToolbarLogicType>([
    ...
    selectors({
        previewSurvey: [
            ...
            (form: QuickSurveyForm, isCreating: boolean, editingSurvey: Survey | null): Survey | null => {
                if (!isCreating) {
                    return null
                }
        ...
        canProceedReason: [
            (s) => [s.quickForm],
            (form: QuickSurveyForm): string | null => {
                if (!form.name.trim()) {
    ...
    listeners(({ actions, values }) => ({
        ...
        submitQuickCreate: async ({ launch }) => {
            ...
            toolbarInsightsJS.capture(editingId ? 'toolbar survey edited' : 'toolbar survey created', {
