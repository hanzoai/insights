...
from insights.schema import AttributionMode, BaseMathType, ConversionGoalFilter1
...
from insights.datastore.preaggregation.marketing_conversions_sql import (
...
class TestConversionGoalPrecomputeDedup(DatastoreTestMixin, APIBaseTest):
    ...
    def _execute(self, query: ast.SelectQuery) -> list[tuple]:
        return execute_insightsql_query(query, team=self.team).results or []
    ...
    def _duplicate_under_new_job_id(self, sharded_table: str, copies: int) -> None:
        ...
        original_rows = sync_execute(  # nosemgrep: datastore-fstring-param-audit — test helper; sharded_table is a hardcoded preagg table, column_names come from system.columns, team_id parameterized
        ...
        for _ in range(copies):
            ...
            sync_execute(  # nosemgrep: datastore-fstring-param-audit — test helper; sharded_table + column_names (from system.columns) are not user input
    ...
    def _count_and_unique(self, distributed_table: str, ts_column: str) -> tuple[int, int]:
        rows = sync_execute(  # nosemgrep: datastore-fstring-param-audit — test helper; distributed_table + ts_column are hardcoded preagg identifiers, team_id parameterized
            f"""
