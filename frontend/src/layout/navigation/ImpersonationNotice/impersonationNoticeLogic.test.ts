...
jest.mock('@hanzo/elements', () => {
    const actual = jest.requireActual('@hanzo/elements')
    return {
        ...actual,
        toast: {
            ...actual.toast,
...
describe('impersonationNoticeLogic', () => {
            ...
            it('returns a single region url when the region is known', async () => {
                ...
                await expectLogic(logic).toMatchValues({
                    adminLoginUrls: [
                        {
                            region: Region.EU,
                            url: 'https://eu.hanzo.ai/admin/insights/user/?q=a%2Bb%40example.com',
                        },
                    ],
                })
            })
            ...
            it('falls back to both production regions when the region is unknown', async () => {
                ...
                await expectLogic(logic).toMatchValues({
                    adminLoginUrls: [
                        {
                            region: Region.US,
                            url: 'https://us.hanzo.ai/admin/insights/user/?q=slack%40example.com',
                        },
                        {
                            region: Region.EU,
                            url: 'https://eu.hanzo.ai/admin/insights/user/?q=slack%40example.com',
                        },
                    ],
                })
            })
    ...
    describe('returnToInsights listener', () => {
        it('navigates to the loginas logout endpoint with next pointing back to the app', async () => {
            ...
            try {
                await expectLogic(logic, () => {
                    logic.actions.returnToInsights()
                }).toFinishAllListeners()
    ...
    describe('loadUserSuccess listener', () => {
        it('clears expired session when user is impersonated', async () => {
            ...
            expect(toast.success).toHaveBeenCalledWith('Impersonation session renewed')
        })
