...
export function collectRunRefs(
    cells: CellTagBlock[],
    selfNodeId: string
): Record<string, { node_id: string; kind: 'insightsql' | 'local' }> {
    const refs: Record<string, { node_id: string; kind: 'insightsql' | 'local' }> = {}
    for (const cell of cells) {
        if (cell.tagName !== 'SQLV2' || !cell.nodeId || cell.nodeId === selfNodeId) {
            continue
        }
        if (DATAFRAME_NAME_REGEX.test(cell.returnVariable)) {
            refs[cell.returnVariable] = { node_id: cell.nodeId, kind: 'insightsql' }
        }
    }
    for (const cell of cells) {
