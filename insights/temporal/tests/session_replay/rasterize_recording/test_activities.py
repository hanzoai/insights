...
from insights.temporal.session_replay.rasterize_recording.activities import (
...
MOCK_SETTINGS.OBJECT_STORAGE_BUCKET = "insights"
...
def _patches(asset: MagicMock, head_object_return: dict | None = None):
    ...
    return (
        patch(
            "insights.temporal.session_replay.rasterize_recording.activities.rasterize.ExportedAsset.objects", mock_qs
        ),
        patch("insights.temporal.session_replay.rasterize_recording.activities.rasterize.settings", MOCK_SETTINGS),
        patch("insights.temporal.session_replay.rasterize_recording.activities.rasterize.close_old_connections"),
        patch(
            "insights.temporal.session_replay.rasterize_recording.activities.rasterize.object_storage.head_object",
            head_mock,
        ),
        patch(
            "insights.temporal.session_replay.rasterize_recording.activities.rasterize.transaction.atomic",
...
class TestBuildRasterizationCache:
    ...
    def test_cache_hit_returns_synthesized_output(self):
        ...
        assert result.cached_output.s3_uri == "s3://insights/exports/mp4/team-1/task-50/video.mp4"
...
class TestFinalizeRasterization:
    def _make_result(self, **overrides) -> RasterizationActivityOutput:
        defaults = {
            "s3_uri": "s3://insights/exports/mp4/team-1/task-42/video.mp4",
