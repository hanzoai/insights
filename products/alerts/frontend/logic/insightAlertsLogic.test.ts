...
import { ChartDisplayType, InsightsFunctionType, InsightLogicProps, InsightShortId } from '~/types'
...
describe('insightAlertsLogic', () => {
    ...
    let insightsFunctionsListSpy: jest.SpyInstance
    ...
    afterEach(() => {
        listSpy.mockRestore()
        insightsFunctionsListSpy.mockRestore()
    })
    ...
    it('loads destination counts for all alerts in one request', async () => {
        ...
        mountInsightStack(insightLogicProps)
        insightsFunctionsListSpy.mockResolvedValue({
            count: 3,
            results: [
        ...
        expect(insightsFunctionsListSpy).toHaveBeenCalledTimes(1)
    })
    ...
    it('refreshes destination counts after an alert is saved', async () => {
        ...
        insightsFunctionsListSpy.mockResolvedValue({
            count: 1,
            results: [{ filters: { properties: [{ key: 'alert_id', value: 'alert-a' }] } }] as InsightsFunctionType[],
        })
        const alertsLogic = insightAlertsLogic({ insightId: 42, insightLogicProps, deferInitialAlertsLoad: true })
        alertsLogic.mount()
