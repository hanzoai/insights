...
description: >
  Audit the health of a Insights project's materialized views (saved queries) — find every failed materialization and
...
- Reviewing view health after a InsightsQL or schema change
...
- `transformation` (InsightsFunction) → owned by the CDP / ingestion side
...
Materialized view failures are usually independent of sources — a view failure is a InsightsQL or data issue in the view
...
- monthly_revenue — view failed (syntax error in InsightsQL: 'FORM' instead of 'FROM')
- active_users_30d — view failed (missing table reference)

Both are InsightsQL issues in the view definitions — independent of your sources. Want me to open one?
```
...
End the audit with a clear hand-off — e.g. "Want me to open `monthly_revenue` and fix the InsightsQL?" Never apply fixes
