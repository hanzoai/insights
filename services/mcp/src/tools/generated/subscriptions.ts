...
const subscriptionsDeliveriesList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedSubscriptionDeliveryList>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof SubscriptionsDeliveriesListSchema>) => {
        ...
        return await withInsightsUrl(context, filtered, '/subscriptions')
    },
})
...
const subscriptionsList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedSubscriptionList>
> => ({
        ...
        return await withInsightsUrl(
            context,
            {
                ...
                results: await Promise.all(
                    (filtered.results ?? []).map((item) => withInsightsUrl(context, item, `/subscriptions/${item.id}`))
                ),
            },
            '/subscriptions'
        )
