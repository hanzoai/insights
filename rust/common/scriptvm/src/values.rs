...
pub enum Callable {
    ...
    // Calling it dispatches to the native function by name rather than jumping to script bytecode.
    Stl(String),
}
...
// script has "primitives", which are copied by e.g, "get_local", and "objects", which are passed around by reference. This is distinct from the
...
// Basically, for anything we want to be able to cheaply convert try and convert a script value into, e.g.
...
// For anything where we want to deconstruct the script value, like a String or an Array
...
impl HogValue {
    ...
    pub fn get_nested<'a, 'b: 'a>(
        ...
        match lit {
            ...
            HogLiteral::Array(vals) | HogLiteral::Tuple(vals) => {
                ...
                // Script indices are 1-based; the reference VMs also allow negative indices counting
    ...
    pub fn equals(&self, rhs: &HogValue, heap: &VmHeap) -> Result<HogLiteral, VmError> {
        ...
        // epoch-equality lives behind the opt-in flag in `ScriptVM::eq_op`, not here, so `Eq`/`in`/`has`
...
impl HogLiteral {
    ...
    /// Seconds since the Unix epoch if this literal is a **Script datetime/date** object, else `None`.
    ///
    /// Script represents temporals as marker-keyed objects (mirroring the Python/TS runtimes' dicts):
...
/// behavior. The semantics here match the Python/TS reference VMs (and Datastore for temporals).
///
/// 1. If *both* operands are temporal ([`HogLiteral::as_temporal_seconds`]) they are ordered by
///    epoch seconds to match Datastore — the reference Python/TS ScriptVMs can't and so always return
...
// TODO - script values are actually "truthy", as in, will coerce to boolean
...
impl Num {
    ...
    pub fn compare(&self, other: &Num) -> Ordering {
        match (self, other) {
            ...
            // instead of panicking — reachable from min2/max2/arraySort via Script-produced NaN.
...
mod tests {
    ...
    fn compare_with_nan_does_not_panic() {
        ...
        // reachable from min2/max2/arraySort with a Script-produced NaN, so a panic here is a
