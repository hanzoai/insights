...
from insights.temporal.session_replay.delete_recordings.activities import (
...
async def test_delete_recordings_parses_response(response_json, expected_deleted, expected_failed_count):
    ...
    with (
        patch("insights.temporal.session_replay.delete_recordings.activities.settings") as mock_settings,
        patch(
            "insights.temporal.session_replay.delete_recordings.activities.internal_httpx_async_client"
        ) as mock_client_cls,
    ):
...
async def test_delete_recordings_url_construction():
    ...
    with (
        ...
        await delete_recordings(DeleteRecordingsInput(team_id=456, session_ids=["s1"], deleted_by="test@hanzo.ai"))

    mock_client.post.assert_called_once_with(
        "http://recording-api:8000/api/projects/456/recordings/delete",
        json={"session_ids": ["s1"], "deleted_by": "test@hanzo.ai"},
    )
...
async def test_delete_recordings_sends_auth_header():
    ...
    with (
        ...
        await delete_recordings(DeleteRecordingsInput(team_id=1, session_ids=["s1"], deleted_by="test@hanzo.ai"))
...
async def test_delete_recordings_raises_when_no_recording_api_url():
    with patch("insights.temporal.session_replay.delete_recordings.activities.settings") as mock_settings:
...
async def test_purge_deleted_metadata_parameterizes_grace_period(grace_period_days):
    ...
    with (
        patch("insights.temporal.session_replay.delete_recordings.activities.get_client", return_value=mock_client),
        patch("insights.settings.data_stores.DATASTORE_CLUSTER", "insights"),
    ):
