...
class TestAttributionHealthKnownSourcesDatastore(DatastoreTestMixin, BaseTest):
...
class TestAttributionHealthMisspelledSourceDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestAttributionHealthNoUtmDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestAttributionHealthUtmNormalizationDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
    ...
    @pytest.mark.asyncio
    async def test_insightsql_trims_and_lowercases_utm_source(self) -> None:
        response = await get_attribution_health(self.team, lookback_days=30)
...
class TestAttributionHealthMixedSourcesDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestAttributionHealthCatalogueTruncatedDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False

    def setUp(self) -> None:
        super().setUp()
        for i in range(INSIGHTSQL_GROUP_LIMIT):
...
class TestAttributionHealthSourceTypeFilterDatastore(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
