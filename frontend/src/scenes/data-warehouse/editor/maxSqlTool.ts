import type { DataVisualizationNode, InsightsQLFilters } from '~/queries/schema/schema-general'
...
export function applyExecuteSqlToolOutput({
    ...
    let nextFilters: InsightsQLFilters | null | undefined
    ...
    if (typeof toolOutput === 'string') {
        ...
        if ('filters' in toolOutput) {
            nextFilters = (toolOutput.filters ?? null) as InsightsQLFilters | null
        } else if (sourceOutput && 'filters' in sourceOutput) {
            nextFilters = (sourceOutput.filters ?? null) as InsightsQLFilters | null
        }
