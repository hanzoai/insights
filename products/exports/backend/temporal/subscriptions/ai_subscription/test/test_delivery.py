...
_SUBSCRIPTION_URL = "https://app.hanzo.ai/project/1/subscriptions/2"
...
class TestExternalUrlExfilGuard:
    ...
    def test_email_keeps_insights_links(self) -> None:
        html = render_ai_email_html("Open [the dashboard](https://app.hanzo.ai/insights/abc).")
        assert "app.hanzo.ai/insights/abc" in html

    @pytest.mark.parametrize(
        "raw",
        [
            "https://attacker.example\\@hanzo.ai/exfil",  # literal backslash authority bypass
            "https://attacker.example%5C@hanzo.ai/exfil",  # percent-encoded backslash
            "https://hanzo.ai@attacker.example/exfil",  # plain userinfo confusion
        ],
    )
    def test_email_rejects_authority_confusion_bypass(self, raw: str) -> None:
        # urlparse may read the host as hanzo.ai, but browsers navigate to attacker.example —
    ...
    def test_slack_keeps_insights_link(self) -> None:
        message = _build_message("Open [dashboard](https://app.hanzo.ai/insights/abc)")
        all_text = " ".join(b["text"]["text"] for b in message.blocks if b["type"] == "section")
        assert "app.hanzo.ai/insights/abc" in all_text
    ...
    def test_slack_keeps_bare_insights_url(self) -> None:
        message = _build_message("Open https://app.hanzo.ai/insights/abc now")
        all_text = " ".join(b["text"]["text"] for b in message.blocks if b["type"] == "section")
        assert "https://app.hanzo.ai/insights/abc" in all_text
        assert "`https://app.hanzo.ai" not in all_text  # Insights hosts stay live, not defanged
    ...
    def test_slack_keeps_www_insights_url(self) -> None:
        message = _build_message("Docs at www.hanzo.ai/docs here")
        all_text = " ".join(b["text"]["text"] for b in message.blocks if b["type"] == "section")
        assert "www.hanzo.ai/docs" in all_text
        assert "`www.hanzo.ai" not in all_text
...
class TestAIExploreHint:
    def test_no_hint_without_integration(self) -> None:
        assert not any("@Insights" in t for t in _hint_texts(_ai_message()))

    def test_bot_ready_hint_nudges_mention(self) -> None:
        message = _ai_message(integration=_mock_integration(REQUIRED_SLACK_SCOPES))
        assert any("@Insights" in t and "docs/slack-app" not in t for t in _hint_texts(message))
