...
class BakeDevStackImageWorkflow(InsightsWorkflow):
    """Bakes and republishes the prebaked Insights dev-stack VM image.

    Dispatched nightly (see `bake_dev_stack_image_task` in the tasks facade) so the
    baked migration state stays close to master and task-time `insightscli start` only
    ...
    async def run(self, input: BakeDevStackImageInput) -> str:
        return await workflow.execute_activity(
            ...
            # The activity heartbeats while the sync bake blocks, so a worker crash or
            # redeploy mid-bake is detected here instead of at the 3-hour start_to_close.
