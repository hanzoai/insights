...
from insights.temporal.ai import AI_WORKFLOWS
from insights.temporal.session_replay.session_summary import SESSION_SUMMARY_WORKFLOWS
from insights.temporal.session_replay.session_summary.activities.capture_timing import capture_timing_activity
from insights.temporal.session_replay.session_summary.activities.check_summary_exists import (
    check_summary_exists_activity,
)
from insights.temporal.session_replay.session_summary.activities.event_based.get_llm_single_session_summary import (
...
from insights.temporal.session_replay.session_summary.types.inputs import SingleSessionSummaryInputs
from insights.temporal.session_replay.session_summary_group import SESSION_SUMMARY_GROUP_WORKFLOWS
from insights.temporal.session_replay.session_summary_group.activities.fetch_session_batch_events import (
    fetch_session_batch_events_activity,
)
from insights.temporal.session_replay.session_summary_group.activities.group_patterns import (
    assign_events_to_patterns_activity,
    combine_patterns_from_chunks_activity,
    extract_session_group_patterns_activity,
    split_session_summaries_into_chunks_for_patterns_extraction_activity,
)
from insights.temporal.session_replay.session_summary_group.types import (
    FailedSessionInfo,
    SessionGroupSummaryOfSummariesInputs,
    SessionGroupSummaryPatternsExtractionChunksInputs,
    SessionSummaryStreamUpdate,
)
from insights.temporal.session_replay.session_summary_group.workflow import (
    SessionGroupSummaryInputs,
    SummarizeSessionGroupWorkflow,
    _start_session_group_summary_workflow,
    execute_summarize_session_group,
)
from insights.temporal.tests.session_replay.session_summary.conftest import AsyncRedisTestContext
...
async def test_extract_session_group_patterns_activity_standalone(
    ...
    with (
        ...
        patch("temporalio.activity.info") as mock_activity_info,
        patch(
            "insights.temporal.session_replay.session_summary_group.activities.group_patterns.async_connect"
...
class TestSummarizeSessionGroupWorkflow:
    ...
    def execute_test_environment(
        ...
        with (
            ...
            patch(
                "insights.temporal.session_replay.session_summary_group.activities.fetch_session_batch_events.SessionReplayEvents.get_group_metadata",
                return_value=MockMetadataDict(),
            ),
            patch(
                "insights.temporal.session_replay.session_summary_group.activities.fetch_session_batch_events._get_db_events_per_page",
    ...
    async def test_execute_summarize_session_group(
        ...
        with patch(
            "insights.temporal.session_replay.session_summary_group.workflow._start_session_group_summary_workflow",
    ...
    async def test_start_session_group_summary_workflow_fetches_from_db(
        ...
        with patch(
            "insights.temporal.session_replay.session_summary_group.workflow.async_connect",
            return_value=mock_client,
        ):
...
class TestPatternExtractionChunking:
    ...
    async def test_all_sessions_fit_in_single_chunk(
        ...
        with patch(
            "insights.temporal.session_replay.session_summary_group.activities.group_patterns.estimate_tokens_from_strings"
        ) as mock_estimate:
...
async def test_combine_patterns_from_chunks_activity(
    ...
    with (
        patch(
            "insights.temporal.session_replay.session_summary_group.activities.group_patterns.get_llm_session_group_patterns_combination"
        ) as mock_combine,
        patch("temporalio.activity.info") as mock_activity_info,
    ):
        mock_activity_info.return_value.workflow_id = "test_workflow_id"
...
async def test_run_patterns_extraction_with_chunking_and_redis_keys(
    ...
    with (
        patch(
            "insights.temporal.session_replay.session_summary_group.workflow.split_session_summaries_into_chunks_for_patterns_extraction_activity",
            new_callable=AsyncMock,
        ) as mock_split,
        patch(
            "insights.temporal.session_replay.session_summary_group.workflow.extract_session_group_patterns_activity",
            new_callable=AsyncMock,
        ) as mock_extract,
        patch(
            "insights.temporal.session_replay.session_summary_group.activities.group_patterns.get_llm_session_group_patterns_combination",
...
class TestSessionBatchFetchExpectedSkips:
    ...
    async def test_short_sessions_are_expected_skips(self, ateam: Team, auser: User):
        ...
        from insights.temporal.session_replay.session_summary_group.types import SessionBatchFetchOutput
        ...
        with (
            ...
            patch(
                "insights.session_recordings.queries.session_replay_events.SessionReplayEvents.get_group_metadata",
                return_value=mock_metadata,
            ),
            patch(
                "insights.temporal.session_replay.session_summary_group.activities.fetch_session_batch_events.get_async_client"
            ),
        ):
