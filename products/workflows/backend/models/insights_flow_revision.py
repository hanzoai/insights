...
class InsightsFlowRevision(TeamScopedRootMixin, UUIDTModel):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["insights_flow", "version"], name="unique_hogflow_revision_version"),
        ]
    ...
    insights_flow = models.ForeignKey("workflows.InsightsFlow", on_delete=models.CASCADE, related_name="revisions")
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        # A revision's tenant scope always mirrors its workflow's. A mismatched (team, insights_flow)
        ...
        self.team_id = self.insights_flow.team_id
        super().save(*args, **kwargs)

    def __str__(self) -> str:
        return f"InsightsFlowRevision {self.insights_flow_id} v{self.version}"
