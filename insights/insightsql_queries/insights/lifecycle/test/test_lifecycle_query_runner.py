...
class TestLifecycleQueryRunner(DatastoreTestMixin, APIBaseTest):
    ...
    def test_days_of_week_actors_match_chart(self):
        ...
        monday_new_actors = execute_insightsql_query(
            team=self.team,
        ...
        saturday_new_actors = execute_insightsql_query(
            team=self.team,
            query="SELECT actor_id FROM {actors_query}",
    ...
    def test_format_results_aligns_misaligned_status_series(self):
        # Regression: each lifecycle status is grouped independently in Datastore and groupArray does
    ...
    def test_lifecycle_virtual_person_property_insightsql_filter(self):
        with freeze_time("2020-01-12T12:00:00Z"):
            _create_person(
                team_id=self.team.pk,
                distinct_ids=["p1"],
                ...
                query=LifecycleQuery(
                    ...
                    series=[
                        EventsNode(
                            event="$pageview",
                            properties=[
                                InsightsQLPropertyFilter(key="person.$virt_initial_channel_type = 'Organic Search'")
                            ],
                        )
                    ],
                ),
    ...
    def test_dashboard_breakdown_filter_does_not_update_breakdown_filter(self):
        ...
        with patch("insights.insightsql_queries.query_runner.capture_exception") as mock_capture_exception:
