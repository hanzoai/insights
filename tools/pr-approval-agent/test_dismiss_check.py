"""Tests for the Stamp dismiss-decision logic."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("insights/test_views.py", True),
        ("insights/api/test/foo.py", True),
        ("tests/test_foo.py", True),
        ("foo/__tests__/bar.ts", True),
        ("conftest.py", True),
        ("insights/conftest.py", True),
        ("internal_test.go", True),
        ("insights/fixtures/foo.json", True),
        ...
        ("insights/generated/evil.py", False),
        ("insights/personinsights_client/proto/generated/foo.py", False),
...
def test_decide_no_prior_approval(monkeypatch: pytest.MonkeyPatch, repo: Path) -> None:
    ...
    result = decide("Insights/insights", 1, _head(repo), repo)
...
def test_select_last_bot_approval_recognizes_app_identity() -> None:
    # The Stamp app posts the body-carrying approval as stamp[bot]; it
    # must count as a prior bot approval so dismiss-on-push can find its SHA.
    reviews = [_review("stamp[bot]", "APPROVED", "sha-app", "2026-01-01T00:00:00Z")]
