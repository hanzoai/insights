...
describe('aiObservabilityAIDataLogic', () => {
    ...
    it('passes through when both input and output are already present — no heavy-prop lookup', async () => {
        ...
        expect(mockApi.queryInsightsQL).not.toHaveBeenCalled()
    })

    it('fetches from ai_events and populates heavy props when input is missing', async () => {
        jest.spyOn(mockApi, 'queryInsightsQL').mockResolvedValue({
            results: [
                [
        ...
        expect(mockApi.queryInsightsQL).toHaveBeenCalledTimes(1)
        expect(mockApi.queryInsightsQL.mock.calls[0][0]).toContain('FROM insights.ai_events AS ai_events')
        expect(mockApi.query).not.toHaveBeenCalled()
    })

    it('falls back to events when ai_events has no heavy props row', async () => {
        jest.spyOn(mockApi, 'queryInsightsQL')
        ...
        expect(mockApi.queryInsightsQL).toHaveBeenCalledTimes(2)
        expect(mockApi.queryInsightsQL.mock.calls[0][0]).toContain('FROM insights.ai_events AS ai_events')
        expect(mockApi.queryInsightsQL.mock.calls[1][0]).toContain('FROM events')
    })
    ...
    it('degrades gracefully to the passed-in values when the lookup throws', async () => {
        jest.spyOn(mockApi, 'queryInsightsQL').mockRejectedValue(new Error('network down'))
    ...
    it.each([
        ...
        const querySpy = jest.spyOn(mockApi, 'queryInsightsQL')
