...
class TestActorsQueryRunner(ClickhouseTestMixin, APIBaseTest):
        ...
        runner = self._create_runner(
            ActorsQuery(
                properties=[
                    ...
                    InsightsQLPropertyFilter(key="toInt(properties.index) > 5"),
                ]
            )
        )
        ...
        runner = self._create_runner(
            ActorsQuery(
                fixedProperties=[
                    ...
                    InsightsQLPropertyFilter(key="toInt(properties.index) < 2"),
                ]
            )
        )
    ...
    def test_persons_query_search_snapshot(self):
        ...
        assert pretty_print_in_tests(runner.to_insightsql(), self.team.pk) == self.snapshot
    ...
    def test_source_insightsql_query_poe_on(self):
        self.random_uuid = self._create_random_persons()
        source_query = InsightsQLQuery(query="SELECT distinct person_id FROM events WHERE event='clicky-4'")
        query = ActorsQuery(
            select=["properties.email"],
            orderBy=["properties.email DESC"],
            source=source_query,
    ...
    @patch("posthog.insightsql_queries.insights.paginators.execute_insightsql_query", wraps=execute_insightsql_query)
    def test_funnel_source_with_poe_mode(self, spy_execute_insightsql_query):
        self.team.modifiers = {
        ...
        # Verify that execute_insightsql_query was called with the correct modifiers
        called_modifiers: InsightsQLQueryModifiers = spy_execute_insightsql_query.call_args[1]["modifiers"]
