...
import { CustomFunctionConfiguration } from 'scenes/custom-functions/configuration/CustomFunctionConfiguration'
import {
    CustomFunctionConfigurationLogicProps,
    customFunctionConfigurationLogic,
} from 'scenes/custom-functions/configuration/customFunctionConfigurationLogic'
import { CustomFunctionLogs } from 'scenes/custom-functions/logs/CustomFunctionLogs'
import { CustomFunctionTesting } from 'scenes/custom-functions/testing/CustomFunctionTesting'
...
import type { customFunctionSceneLogicType } from './CustomFunctionSceneType'
import { CustomFunctionBackfills } from './backfills/CustomFunctionBackfills'
import { CustomFunctionIconEditable } from './configuration/CustomFunctionIcon'
import {
    CustomFunctionConfigurationClearChangesButton,
    CustomFunctionConfigurationSaveButton,
} from './configuration/components/CustomFunctionConfigurationButtons'
import { CustomFunctionMetrics } from './metrics/CustomFunctionMetrics'
import { CustomFunctionSkeleton } from './misc/CustomFunctionSkeleton'

const CUSTOM_FUNCTION_SCENE_TABS = ['configuration', 'metrics', 'logs', 'testing', 'backfills', 'history'] as const
export type CustomFunctionSceneTab = (typeof CUSTOM_FUNCTION_SCENE_TABS)[number]

const CustomFunctionSceneMapping: Partial<Record<CustomFunctionTypeType, { scene: Scene; url: () => string }>> = {
...
export const customFunctionSceneLogic = kea<customFunctionSceneLogicType>([
    props({} as CustomFunctionConfigurationLogicProps),
    key(({ id, templateId }: CustomFunctionConfigurationLogicProps) => id ?? templateId ?? 'new'),
    path((key) => ['scenes', 'custom-functions', 'customFunctionSceneLogic', key]),
    ...
    actions({
        setCurrentTab: (tab: CustomFunctionSceneTab) => ({ tab }),
    }),
    reducers(() => ({
        currentTab: [
            'configuration' as CustomFunctionSceneTab,
            {
    ...
    selectors({
        ...
        alertId: [
            (s) => [s.configuration],
            (configuration: CustomFunctionType | null): string | undefined => {
        ...
        isSurveyNotification: [
            (s) => [s.configuration],
            (configuration: CustomFunctionType | null): boolean => {
        ...
        breadcrumbs: [
            ...
            (
                ...
                loading: boolean,
                configuration: CustomFunctionType | null,
                ...
                const sceneMapping = CustomFunctionSceneMapping[type]
                ...
                if (type === 'internal_destination') {
                    ...
                    // This is used to help the CustomFunctionScene render correct breadcrumbs and redirections
    ...
    urlToAction(({ actions, values }) => {
        const reactToTabChange = (_: any, search: Record<string, string>): void => {
            const possibleTab = (search.tab ?? 'configuration') as CustomFunctionSceneTab

            const tab = CUSTOM_FUNCTION_SCENE_TABS.includes(possibleTab) ? possibleTab : 'configuration'
        ...
        return {
            ...
            [urls.customFunction(':id')]: reactToTabChange,
            [urls.errorTrackingAlert(':id')]: reactToTabChange,
        }
    }),
])

export const scene: SceneExport<CustomFunctionConfigurationLogicProps> = {
    component: CustomFunctionScene,
    logic: customFunctionSceneLogic,
...
function CustomFunctionHeader(): JSX.Element {
    const { configuration, logicProps, loading, isLegacyPlugin } = useValues(customFunctionConfigurationLogic)
    const { setConfigurationValue, duplicate, deleteCustomFunction } = useActions(customFunctionConfigurationLogic)
                                ...
                                <More
                                    ...
                                    overlay={
                                        <>
                                            ...
                                            <LemonDivider />
                                            <LemonButton status="danger" fullWidth onClick={() => deleteCustomFunction()}>
                                                Delete
                                            </LemonButton>
                                        </>
                                    }
...
export function CustomFunctionScene(): JSX.Element {
    const { currentTab, loading, loaded, logicProps, type, teamHasCohortFilters, currentProjectId } =
        useValues(customFunctionSceneLogic)
    const { setCurrentTab } = useActions(customFunctionSceneLogic)
    ...
    const tabs: (LemonTab<CustomFunctionSceneTab> | null)[] = [
        {
            label: 'Configuration',
            key: 'configuration',
            content: <CustomFunctionConfiguration id={id} />,
        },
        ...
        type === 'site_app' || type === 'site_destination'
            ...
            : {
                  label: 'Metrics',
                  key: 'metrics',
                  content: <CustomFunctionMetrics id={id} />,
              },
        type === 'site_app' || type === 'site_destination'
            ? null
            : {
        ...
        type === 'destination' && featureFlags[FEATURE_FLAGS.BACKFILL_WORKFLOWS_DESTINATION]
            ? {
                  label: 'Backfills',
                  key: 'backfills',
                  content: <CustomFunctionBackfills id={id} />,
              }
            : null,

        {
            label: 'History',
            key: 'history',
            content: <ActivityLog id={id} scope={ActivityScope.CUSTOM_FUNCTION} />,
        },
    ]
    ...
    return (
        <SceneContent>
            <BindLogic logic={customFunctionConfigurationLogic} props={logicProps}>
                ...
                {templateId ? (
                    <CustomFunctionConfiguration templateId={templateId} subTemplateId={subTemplateId} />
                ) : (
