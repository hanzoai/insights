...
describe('webAnalyticsRecapLogic', () => {
    ...
    beforeEach(() => {
        initKeaTests()
        mockRecap.mockReset()
        jest.spyOn(insights, 'capture').mockReturnValue(null as any)
    ...
    it('does not record a share when the clipboard write fails', async () => {
        ...
        expect(insights.capture).not.toHaveBeenCalledWith('web_analytics_recap_shared', expect.anything())
    })
    ...
    it('records Insights AI CTA usage with recap context', async () => {
        mockRecap.mockResolvedValue(makeRecap())
        logic = webAnalyticsRecapLogic()
        logic.mount()
        await expectLogic(logic).toDispatchActions(['loadRecapSuccess'])
        ...
        await expectLogic(logic, () => {
            logic.actions.recordCtaClicked('ask_insights_ai')
        }).toDispatchActions(['recordCtaClicked', 'recordButtonClicked'])

        expect(insights.capture).toHaveBeenCalledWith(
            'web_analytics_recap_button_clicked',
            expect.objectContaining({
                button: 'ask_insights_ai',
                intent: 'insights_ai_usage',
                destination: 'insights_ai_side_panel',
                period_end: '2025-01-29',
            })
        )
        expect(insights.capture).toHaveBeenCalledWith(
            'web_analytics_recap_cta_clicked',
            expect.objectContaining({ cta: 'ask_insights_ai', persona: 'rising_star' })
        )
    })
