...
export function WelcomeDialog(): JSX.Element | null {
    ...
    return (
        <Modal
            ...
            onClose={() => closeDialog()}
            width={640}
            title={`Welcome to ${organizationName || 'Insights'}`}
            description={inviterLine}
            data-attr="welcome-dialog"
            footer={
                <div className="flex flex-row justify-between items-center w-full">
                    <Button type="tertiary" onClick={() => dismissWelcome()} data-attr="welcome-dismiss">
                        Don't show again
                    </Button>
                    <Button type="primary" onClick={() => closeDialog()} data-attr="welcome-close">
                        Start exploring
                    </Button>
                </div>
            }
            ...
            {welcomeDataLoading && !welcomeData.organization_name ? (
                ...
                <div className="flex flex-col gap-3">
                    {welcomeDataError ? (
                        ...
                        >
                            We couldn't load your team's activity. You can still explore Insights from here.
                        </Banner>
                    ) : (
                        <p className="text-muted text-sm m-0">{introCopy}</p>
                    )}
                    {/* Provisioned/new accounts lead with the background install + a tour of what Insights
