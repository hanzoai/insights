...
from insights.temporal.ai_observability.sentiment.extraction import truncate_to_head_tail
from insights.temporal.ai_observability.sentiment.schema import SentimentResult
...
class TestRunEvaluationWorkflow:
    ...
    async def test_fetch_evaluation_activity(self, setup_data):
        ...
        with patch(
            "insights.temporal.ai_observability.evaluation_workflow_activities.Evaluation.objects.get"
        ) as mock_get:
    ...
    def test_execute_llm_judge_activity(self, setup_data, active_key_config):
        ...
        with patch("insights.temporal.ai_observability.evaluation_llm_judge.Client") as mock_client_class:
    ...
    async def test_emit_evaluation_event_activity(self, setup_data):
        ...
        with patch(
            "insights.temporal.ai_observability.evaluation_workflow_activities.Team.objects.get"
        ) as mock_team_get:
            with patch(
                "insights.temporal.ai_observability.evaluation_workflow_activities.capture_internal"
            ) as mock_capture:
    ...
    def test_execute_llm_judge_activity_parse_error_raises_non_retryable(self):
        ...
        with (
            patch(
                "insights.temporal.ai_observability.model_resolution.EvaluationConfig.objects.get_or_create"
            ) as mock_get_or_create,
            patch("insights.temporal.ai_observability.evaluation_llm_judge.Client") as mock_client_class,
            patch("insights.temporal.ai_observability.evaluation_llm_judge.increment_errors") as mock_increment_errors,
        ):
    ...
    def test_execute_llm_judge_activity_terminal_team_requires_provider_key(self, setup_data):
        ...
        with (
            patch("insights.temporal.ai_observability.evaluation_llm_judge.Client") as mock_client_class,
            patch("insights.temporal.ai_observability.evaluation_llm_judge.increment_user_errors") as mock_user_errors,
            patch("insights.temporal.ai_observability.evaluation_llm_judge.increment_errors") as mock_errors,
        ):
            result = execute_llm_judge_activity(ExecuteLLMJudgeInputs(evaluation=evaluation, event_data=event_data))
...
class TestExecuteHogEvalActivity:
    ...
    async def test_hog_eval_runtime_error_returns_skipped(self, setup_data):
        ...
        # A ScriptVM runtime error in user-authored Script (e.g. unsupported function, unknown global)
...
class TestExecuteSentimentEvalActivity:
    ...
    async def test_sentiment_eval_classifies_user_messages(self, setup_data):
        ...
        with patch(
            "insights.temporal.ai_observability.sentiment.model.classify", return_value=[classification]
        ) as mock_classify:
    ...
    async def test_sentiment_eval_skips_without_user_messages(self, setup_data):
        ...
        with patch("insights.temporal.ai_observability.sentiment.model.classify") as mock_classify:
            result = await execute_sentiment_eval_activity(evaluation, event_data)
    ...
    async def test_sentiment_eval_skips_when_classifier_returns_nothing(self, setup_data):
        ...
        with patch("insights.temporal.ai_observability.sentiment.model.classify", return_value=[]):
            result = await execute_sentiment_eval_activity(evaluation, event_data)
...
class TestSendEvaluationDisabledEmailActivity:
    ...
    async def test_sends_email_with_evaluation_disabled_template(self, setup_data):
        ...
        with (
            patch("insights.email.is_email_available", return_value=True),
            patch("insights.email.EmailMessage") as mock_email_class,
        ):
    ...
    async def test_skips_when_email_not_available(self, setup_data):
        ...
        with (
            patch("insights.email.is_email_available", return_value=False),
            patch("insights.email.EmailMessage") as mock_email_class,
        ):
            await send_evaluation_disabled_email_activity(
                SendEvaluationDisabledEmailInputs(
