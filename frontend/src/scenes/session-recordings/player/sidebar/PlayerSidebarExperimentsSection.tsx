...
import { IconCollapse, IconExpand, IconExternal, IconInfo, IconMinus, IconWarning } from '@hanzo/icons'
...
function VariantTag({ item }: { item: ExperimentSessionContextItemApi }): JSX.Element | null {
    ...
    const type: TagType = item.multiple_variants ? 'warning' : isControlVariant(item) ? 'muted' : 'highlight'
    return (
        <Tooltip title={variantTooltip(item)}>
            <Tag type={type} icon={item.multiple_variants ? <IconWarning /> : undefined} className="shrink-0">
                        ...
                        anySeenExpandable ? (
                            <div className="shrink-0 flex w-[1.625rem] items-center justify-center -me-1.5">
                                {hasMetrics ? (
                                    ...
                                    // it hoverable (Button disables via aria-disabled, not the native
