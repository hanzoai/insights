...
class RecentEmissionsQuerySerializer(serializers.Serializer):
    ...
    scoped to one scout or a time window. Pure Postgres — no Datastore round-trip.
    """
...
class SuggestedReviewerSerializer(serializers.Serializer):
    ...
    user_uuid = serializers.UUIDField(
        required=False,
        help_text=(
            "Insights user UUID (e.g. from `scout-members-list`, or an entity's `created_by`). "
            "Resolved server-side to the member's linked GitHub login — use this when you know the Insights "
...
class RecentDashboardEntrySerializer(serializers.Serializer):
    ...
    last_accessed_at = serializers.CharField(
        allow_null=True,
        help_text="ISO-8601 timestamp of the most recent view in the Insights UI.",
    )
...
class RecentFeatureFlagEntrySerializer(serializers.Serializer):
    ...
    key = serializers.CharField(help_text="Flag key used in code (`insights.isFeatureEnabled('<key>')`).")
...
class RecentInsightsFunctionEntrySerializer(serializers.Serializer):
    """One row in `inventory.recent_insights_functions.recent`."""

    id = serializers.CharField(help_text="Script function UUID.")
    name = serializers.CharField(allow_blank=True, help_text="Script function name.")
...
class RecentInsightsFunctionsSerializer(serializers.Serializer):
    """`inventory.recent_insights_functions` — total + enabled count, plus the 5 most recently modified."""

    total_count = serializers.IntegerField(help_text="Total non-deleted script functions on the team.")
    enabled_count = serializers.IntegerField(help_text="Script functions currently enabled (`enabled=true`).")
    recent = serializers.ListField(
        child=RecentInsightsFunctionEntrySerializer(),
        help_text="The 5 most recently updated script functions.",
    )
...
class RecentInsightsFlowsSerializer(serializers.Serializer):
    ...
    recent = serializers.ListField(
        child=RecentInsightsFlowEntrySerializer(),
        help_text="The 5 most recently updated script flows.",
    )
...
class ProjectProfileInventorySerializer(serializers.Serializer):
    ...
    recent_insights_functions = RecentInsightsFunctionsSerializer(
        help_text=(
            "Script function orientation: total + enabled count, plus the 5 most recently "
    ...
    top_events = serializers.ListField(
        ...
        help_text=(
            ...
            "`null` if the underlying Datastore query failed or timed out (distinct "
...
class SignalScoutConfigSerializer(serializers.ModelSerializer):
    ...
    scout_origin = serializers.SerializerMethodField(
        help_text=(
            "Where this scout came from: `canonical` for a scout Insights ships and maintains "
    ...
    network_access = serializers.ChoiceField(
        ...
        help_text=(
            ...
            "default) restricts runs to the platform's trusted-domain allowlist (Insights, GitHub, "
...
class SignalScoutConfigOptionsSerializer(serializers.Serializer):
    ...
    network_access = serializers.ChoiceField(
        ...
        help_text=(
            ...
            "`trusted`, the platform's trusted-domain allowlist (Insights, GitHub, common package "
...
class ScoutMemberSerializer(serializers.Serializer):
    ...
    user_uuid = serializers.CharField(
        help_text=(
            "The member's stable Insights user UUID — the same id that appears as `created_by.uuid` on "
