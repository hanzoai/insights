...
def test_build_message_blocks_includes_recipient_and_open_in_insights_button() -> None:
    report = SignalReport(
        id="report-uuid",
        team_id=42,
    ...
    assert buttons[0]["text"]["text"] == "Review in Insights"
...
def test_build_message_blocks_includes_repository_in_metadata_line() -> None:
    ...
    assert blocks[1]["text"]["text"] == "*🟠 P2 · Error tracking · Insights/insights*"
...
def test_dispatch_posts_to_team_channel_without_per_user_config(org_and_team):
    ...
    _set_team_channel(team, "CTEAM|#insights-signals")
...
def test_dispatch_includes_repository_from_repo_selection_artefact(org_and_team):
    ...
    SignalReportArtefact.objects.create(
        ...
        content=json.dumps({"repository": "Insights/insights", "reason": "owns the code"}),
    )
    ...
    assert "Insights/insights" in blocks[1]["text"]["text"]
...
def test_build_signal_thread_blocks_renders_header_content_and_github_details() -> None:
    signal = {
        ...
        "extra": {
            "number": 42,
            "labels": ["bug", "export"],
            "html_url": "https://github.com/Insights/insights/issues/42",
        },
    }
    ...
    assert "<https://github.com/Insights/insights/issues/42|View on GitHub>" in detail
...
def test_dispatch_caps_thread_signals_and_posts_overflow_note(org_and_team):
    ...
    assert f"+{extra_count} more signals in Insights" in overflow_call.kwargs["text"]
