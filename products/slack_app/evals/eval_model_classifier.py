...
To run:
    insightscli evals eval_model_classifier
    insightscli evals eval_model_classifier --eval names_model_in_code_change
"""
...
from insights.temporal.ai.slack_app.activities import classifiers
...
from products.insights_ai.eval_harness.one_shot import OneShotPublicEval
...
INSTRUCTION_CASES = [
    BaseEvalCase(
        ...
        prompt="@Insights use opus 5 for this — the retention query is returning empty buckets",
        expected=_asks(model="claude-opus-5"),
    ),
    BaseEvalCase(
        name="instruction_after_the_ask",
        prompt="@Insights can you look into why the billing webhook retries twice? run it on fable please",
        expected=_asks(model="claude-fable-5"),
    ),
    BaseEvalCase(
        name="effort_without_model",
        prompt="@Insights this one's gnarly, give it max effort — the session replay index is corrupt somehow",
        expected=_asks(reasoning_effort="max"),
    ),
    BaseEvalCase(
        name="model_and_effort",
        prompt="@Insights run this on sonnet 5 with low effort, it's a one-line copy change in the nav",
        expected=_asks(model="claude-sonnet-5", reasoning_effort="low"),
    ),
    BaseEvalCase(
        name="colloquial_model_name",
        prompt="@Insights do this one with luna: add a loading state to the cohort picker",
        expected=_asks(model="gpt-5.6-luna"),
    ),
    BaseEvalCase(
        name="instruction_with_model_also_as_subject",
        prompt=(
            "@Insights use fable for this one. The task: our model picker is missing gpt-5.6-sol, add it to the dropdown"
        ),
        expected=_asks(model="claude-fable-5"),
    ),
]
...
SUBJECT_MATTER_CASES = [
    BaseEvalCase(
        ...
        prompt="@Insights add gpt-5.6-luna to the model picker in settings, it's missing from the dropdown",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="question_comparing_models",
        prompt="@Insights why is opus 5 slower than sonnet 5 on our eval suite? dig into the traces",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="model_as_incident_topic",
        prompt="@Insights the fable rollout broke the usage dashboard last night, can you find what changed",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="effort_as_english",
        prompt="@Insights reasoning about this caching bug is hard, can you take a look at the invalidation logic",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="model_in_config_file",
        prompt="@Insights our CLASSIFIER_MODEL constant still says claude-haiku-4-5, update it and the tests",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="model_in_error_message",
        prompt='@Insights users are seeing "model gpt-5.6-sol is not available" on the tasks page, fix the fallback',
        expected=_asks(),
    ),
    BaseEvalCase(
        name="past_tense_report",
        prompt="@Insights I ran this on opus 5 yesterday and it missed the race condition, try again",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="effort_as_product_copy",
        prompt="@Insights the effort selector tooltip says 'max effort' but the value sent is xhigh, fix the label",
        expected=_asks(),
    ),
    BaseEvalCase(
        name="no_model_mentioned",
        prompt="@Insights the checkout funnel drops 40% between steps 2 and 3, can you work out why",
        expected=_asks(),
    ),
]
