...
from insights.models.activity_logging.activity_log import Change, ChangeAction, Detail, log_activity
...
class TestTimestampBasedReconstruction(TestReconstructFlagAtVersion):
    ...
    def test_bulk_delete_without_version_returns_none(self):
        ...
        # Log a bulk delete with empty changes (like insights/api/feature_flag.py:2844)
        log_activity(
            organization_id=self.organization.id,
            team_id=self.team.id,
            user=self.user,
