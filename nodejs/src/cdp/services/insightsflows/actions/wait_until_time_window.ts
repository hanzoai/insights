...
export class WaitUntilTimeWindowHandler implements ActionHandler {
    execute({
        invocation,
        action,
    }: ActionHandlerOptions<Extract<InsightsFlowAction, { type: 'wait_until_time_window' }>>): ActionHandlerResult {
...
export const getWaitUntilTime = (
    action: Extract<InsightsFlowAction, { type: 'wait_until_time_window' }>,
