...
description: 'Copy a feature flag from one Insights project to one or more target projects in the same organization. Use when the user wants to duplicate a flag, promote a flag from staging to production, sync flags across projects, or replicate a flag configuration in a different workspace. Covers cohort remapping, scheduled-change handling, encrypted payloads, and the safe defaults (disabled in target, no scheduled changes).'
---

# Copying feature flags across projects

This skill guides you through duplicating a feature flag from a source project into one or more target projects within the same Insights organization.
...
- If the user gave a flag name (e.g. "the new pricing flag"), call `insights:feature-flag-get-all` in the source project to find the matching flag and read its `key`.
...
Targets must be in the same organization as the source. Call `insights:projects-get` to list available projects and confirm membership before issuing the copy.
...
Call `insights:feature-flag-get-definition` on the source flag and present a concise summary to the user before copying:
...
Call `insights:feature-flags-copy-flags-create` with:
...
- **Editor access on the source.** The user must have at least editor-level access on the source flag's team. The endpoint enforces this via Insights's access control before copying.
...
>
> _(calls `insights:feature-flag-get-definition`)_
>
...
>
> _(calls `insights:feature-flags-copy-flags-create` with `disable_copied_flag: false`, `copy_schedule: false`)_
>
> Done. Created flag id 99887 in project 67890 (`active: true`). No failed targets.

## Available tools

- `insights:feature-flags-copy-flags-create` — performs the copy. Required fields: `feature_flag_key`, `from_project`, `target_project_ids`. Optional: `disable_copied_flag`, `copy_schedule`.
- `insights:feature-flag-get-all` — find a flag by key/name in a given project when the user only gave a friendly name.
- `insights:feature-flag-get-definition` — fetch the full source flag (filters, variants, cohort references, encryption flags) so you can preview before copying.
- `insights:projects-get` — list projects in the active organization, used to resolve and validate target project ids.
