...
10-finding yardstick) is archived at `products/review/eval/experiments/2026-07-reviewer-topology/` —
...
2. **✅ DONE 2026-07-02 — Blind-spot check in prod, always-on** (every review, incl. single-chunk PRs).
   ...
   - **Customizable single-active blind-spots skill**, exactly the validator pattern: third prefix
     `review-script-blind-spots-*` in `ReviewSkillConfig` (canonical: `review-script-blind-spots-general`, on
...
9. **Make the reviewers strict-er — stop spamming the validator with findings it refuses anyway (noted
   2026-07-08).** Live evidence (insights#69066 e2e): 35 post-dedup candidates → 8 valid / **27 dismissed
...
- **Three thin tools over the existing surface** (`products/review/mcp/tools.yaml`):
  `review-script-reviews-trigger` (write), `review-script-reviews-list` (status polling — running turns first with the
  `progress` stage / counters), `review-script-reviews-get` (validated + dismissed findings, published body). All
  three are gated on the `review-script` feature flag — the same flag that gates the Code review menu entry —
...
- **Identity + gates inherited, not widened:** an MCP-triggered review runs under the token's user — the same
  ...
  `REVIEWFN_TEAM_ID` dogfood gate and its synchronous URL / App-access / fork / open-state checks regardless of
  caller.
...
First cross-repo dogfood run (a `Insights/billing` PR via the Stage 5c UI trigger) died in dedup on
...
Live misread on [insights#71025](https://github.com/Insights/insights/pull/71025) (63 additions): the
...
- **Where the code lives:** `products/review/frontend/` (`CodeReviewScene.tsx`,
...
- **Skill lists feed the drawer:** the three config list endpoints (+ PATCH responses) now include the
  skill `body`; "Edit skill ↗" links to `/skills/review-script`.
...
- **Environments 500 (real bug, database agent).** `settings.py::_get_or_create` mixed a canonicalized
  ...
  environments.**
- **Slider PATCH flood (5 agents converged).** `Slider.onChange` fires per mousemove; it was wired
...
- **Load-failure dead-end (5 agents).** A failed initial load left permanent skeletons + an untrue
  "Loading…" with no retry. Added `loadAll` (mount + retry) and an error `Banner` with Retry.
...
`insightscli build:openapi` round-trips with zero generated drift.
...
- **UI (CodeReviewTab), after two user-feedback iterations (fonts too small / published-vs-not
  ...
  validation), and **Review body** (`Markdown`). Finding cards mirror the published PR comment:
  tags + title + file:lines visible, with **Description / Suggested fix / Why we think it's a valid
  issue** (or **Why it was dismissed**) as collapsed `Collapse` sections — the validator's
...
- **Logic:** loader holds the envelope (`recentReviewsPage`); `recentReviews` / `moreReviewsAvailable` are selectors over it.
  ...
  `Button.loading` hard-disables, so it's also the double-click guard.
...
#### ✅ BUILT 2026-07-02 — authoring guide moved to a canonical skill (`review-script-authoring`)
...
is the canonical **`review-script-authoring`** skill at `products/review/skills/review-script-authoring/`.
...
- **The skill** covers all three kinds in one guide: pipeline context, the naming/category contract,
  ...
  matching scouts — instructs the agent to **create the row itself via `insights:skill-create`**
...
- **Seeding:** `REVIEW_FN_AUTHORING_PREFIX`/`_SKILL_NAME` (`skill_loader.py`),
...
team/user/repository; tests updated & green; `ruff check products/review/` clean.
...
> each perspective is a DB-synced `LLMSkill` (`products/review/skills/review-script-perspective-*/SKILL.md`,
...
- **Review stays a top-level peer product** (`products/review/`). It is **not** nested under
...
##### Data model (`products/review/backend/models.py`)
...
- **`ReviewReport(UUIDModel, TeamScopedRootMixin)`** — the living per-PR document:
  - `team` FK → `insights.Team` (CASCADE); `repository` (`owner/repo`); `pr_number`; `pr_url`; `head_branch`;
    `base_branch`.
...
- **`ReviewReportArtefact(UUIDModel, TeamScopedRootMixin)`** — the append-only work log, mirroring
  ...
  - `team` FK; `report` FK → `ReviewReport` (CASCADE, `related_name="artefacts"`); `type` CharField(choices);
    ...
    `insights.User` (SET_NULL, null); `task` FK → `tasks.Task` (SET_NULL, null).
...
Content schemas (`products/review/backend/reviewer/artefact_content.py`, pydantic):
...
2. ✅ **tach:** added the `[[modules]] path = "products.review"` block with
   `depends_on = ["ee", "insights", "products.signals", "products.tasks"]`; `tach check --dependencies --interfaces`
...
3. ✅ **Models:** added `products/review/backend/models.py` (`ReviewReport` + `ReviewReportArtefact` + funnel)
...
10. ✅ **Perspectives as LLMA skills — rename "lens" → "perspective" + DB-synced skill pipeline (before
    ...
    first-class **LLMA skills** at `products/review/skills/review-script-perspective-*/SKILL.md`, synced into
...
repository, branch, …)` builds `CustomPromptSandboxContext` inline. Each fan-out child bounds its sandbox-turn
    ...
    (`start_temporal_worker.py` + `tach.toml` `insights → products.review` + the `container-images-cd.yml`
...
`products/review/eval/RUN_LOG.md` is a committed, agent-maintained log of every end-to-end
...
identical code. The standing sample is **#63625** (`insights-code/fix-stickiness-dw-timestamp-field`,
...
"lens_result"` → `PERSPECTIVE_RESULT = "perspective_result"` (a **Django migration** generated via
  ...
  - **Move the three perspectives into DB-synced LLMA skills (the full Signals-scout pattern, "v2").** On disk,
    `products/review/skills/review-script-perspective-{logic-correctness,contracts-security,performance-reliability}/SKILL.md`
    ...
    add `products.skills` to `products.review`'s `tach` `depends_on` (skills is un-isolated; no facade).
    Delivery flips to **pull**: the issues-review prompt stops splicing the focus text and instead instructs the
    sandbox agent to `skill-get(skill_name="review-script-perspective-…", version=N)` over the Insights MCP (the
...
- **Pre-Temporal cleanup — this was step 12 (✅ done).** Audited the whole reviewer package for clear duplication
  ...
  - **Insights-conformance pass (same window, 7-dimension adversarially-verified audit).** Removed import-time
    ...
    - refs), and **added `products/review/package.json`** (`name` + `backend:test`; no `backend:contract-check`
...
- **Validator as a team-customizable skill — step 13 (✅ built; as-built + original design here).** _As built:_ the
  ...
  `products/review/skills/review-script-validation-criteria/SKILL.md` mirrored into per-team `LLMSkill` rows by
...
- **Everything on Temporal — this is step 15** (the major step after the validator-skill + dedup-cleanup work; design detail here).
  ...
  the **loop** is a deferred follow-up (below). Topology, all on a new `products/review/backend/temporal/`
  package:
  ...
  - **Task queue:** register the orchestrator + child workflows + Review activities on
    **`video-export-task-queue`** (`settings.VIDEO_EXPORT_TASK_QUEUE`) for now — it already hosts Signals' product
    ...
    `products/review/backend/temporal/__init__.py` (`WORKFLOWS` / `ACTIVITIES`) + a facade re-export, append to
  ...
  **Grounded implementation map (researched 2026-06-25 — file:line cites for the rewrite).** A 6-reader sweep of
  ...
  - **Workflow shape:** inherit `InsightsWorkflow` + `@temporalio.workflow.defn(name="…")` with explicit kebab names,
    ...
    class name omits it. Activities are bare `@temporalio.activity.defn async def`. `insights/temporal/common/base.py:5-37`;
  ...
  - **Bounded fan-out (replaces `executor.py`'s module-global `_sandbox_semaphore` — illegal inside a workflow):**
    ...
    `insights/temporal/session_replay/session_summary/workflow.py:431-469`;
    `insights/temporal/experiments/workflows.py:60-88`; `insights/temporal/data_modeling/workflows/execute_dag.py:272-377`.
  ...
  - **Child workflows:** the parent `await`s its three children (they return results it persists — **NOT**
    ...
    `insights/temporal/data_modeling/workflows/execute_dag.py:331`.
  - **Timeouts / retries / heartbeats:** every `execute_activity` sets `start_to_close_timeout` + an explicit
    ...
    `insights/temporal/common/heartbeat.py:17-108`; `insights/temporal/delete_teams/workflows.py:40-58,94-114`.
  ...
  - **Testing:** `WorkflowEnvironment.start_time_skipping()` + `env.client.execute_workflow(..., runner=UnsandboxedWorkflowRunner())`
    ...
    `insights/temporal/tests/test_alerts_workflows.py:127-148`; `products/tasks/backend/temporal/code_workstreams/test_workflow.py:26-53`;
  ...
  - `products/review/backend/temporal/__init__.py` — module-level `WORKFLOWS = [ReviewPRWorkflow,
AnalyzeChunksWorkflow, ReviewPerspectivesWorkflow, ValidateIssuesWorkflow]` + `ACTIVITIES = [...]` (mirror
  ...
  - `.github/workflows/container-images-cd.yml` (~356-363) — add `^products/review/backend/temporal` to the
...
- **Variant A — per-PR singleton loop (decision: the end-state; promote to it when live control is the actual ask).**
  ...
  legs so per-chunk/per-file maps must be re-cast (`insights/temporal/ai_observability/trace_clustering/coordinator.py:122-129`);
...
`get_access_token`, `insights/models/integration.py:2504-2519`, `github_integration_base.py:1433`) — or the
...
`dismiss_check` delta classifier) into `products/review/` as a deterministic `route` activity at the top of each
turn:
...
`review-script-perspective-sql-checks`), the router should also decide **which enabled perspectives make sense for
...
Per the maintainer the **interaction channel is pluggable** (Slack, GitHub comments, Insights Desktop, …) and out of
...
- **Router = copy-and-own** Stamp's pure gate logic into `products/review/`, **invert** deny→enable; don't
  cross-import `tools/`.
...
- **Run `user_id` = resolved server-side** (team-2 `integration.created_by`, or a `REVIEWFN_RUN_USER_ID` setting).
  The sandbox fan-out needs a **real** Insights user; the CI caller never carries or chooses it. (A new dedicated
...
2. ✅ **The endpoint** (`products/review/backend/api/trigger.py` + `routes.py`, mounted unscoped at
...
3. ✅ **Publish for real:** `_post_github_review` and `PRFetcher` now take a passed-in installation token (no env);
   ...
   row is created).
4. ✅ **The Action:** `.github/workflows/review-script.yml` — `on: pull_request_target [labeled]`,
...
- ✅ **Team 2 (prod) has the `insights` GitHub App installed** for `Insights/insights` (maintainer-confirmed 2026-06-26;
  also satisfied locally on team 1 via `insights-local-dev`). **Still confirm before turning publish on in prod:** the
  `pull_requests=write` validation (the duck comment) was on the **separate `insights-local-dev` app** — confirm the
  **prod `insights`** app's installation also grants `pull_requests: write`. If it's read-only, publishing needs a
  permission bump or a dedicated app.
- **`settings.REVIEWFN_TRIGGER_TOKEN`** — generate a value (`openssl rand -hex 32`), put it in **both** the Insights
  deploy config (env `REVIEWFN_TRIGGER_TOKEN`, read in `insights/settings/access.py`) **and** the GitHub Actions secret
  `REVIEWFN_TRIGGER_TOKEN`. Shared password between CI and the app; the only secret the Action carries. Unset =
...
- `insights/models/github_integration_base.py:1433` (`get_access_token`, auto-refresh) ·
  `insights/models/integration.py:2505` (`first_for_team_repository`).
- **New:** `products/review/backend/api/trigger.py` + `backend/routes.py`; `insights/settings/access.py` (3 settings);
...
- **Don't silently drop valid findings that have no diff position — surface them anyway.** `_build_inline_comments`
  ...
  findings, `insights/email.py:276` had no resolvable diff position and was dropped, only `email.py:104` posted
...
> "As built" at the end of this section). Not yet e2e'd against a live non-`insights/insights` PR.

**Motivation.** The label trigger needs `.github/workflows/review-script.yml` in every repo it serves — only
`Insights/insights` has it, so every other `insights/*` repo is out of reach unless we replicate the workflow
...
- **Repo scope = installation access, not a hardcoded org list.**
  ...
  the `insights` org — checked **synchronously at trigger time** so an inaccessible repo 400s immediately
...
- **Accepted / out of scope for v1:** no rate limit beyond the per-PR workflow-id dedupe (internal dogfood
  ...
  unchanged for `Insights/insights`.
...
- Generated clients (`frontend/generated/*`) come from CI's OpenAPI auto-commit
  (`insightscli build:openapi` was unavailable in the authoring environment).
...
>   `insights/test/setup_receivers_baseline.txt` (regenerate with
>   `UPDATE_SETUP_RECEIVERS_BASELINE=1 pytest insights/test/test_startup_import_budget.py -k receivers_match`).
...
edits the canonical-named `LLMSkill` row (e.g. `review-script-perspective-logic-correctness`,
`review-script-validation-criteria`) via the skills API/MCP → `load_perspectives_for_run` /
...
custom perspective (any `review-script-perspective-*` skill, via the skills API/MCP) and enables it via the config
...
**What a perspective is.** Any team `LLMSkill` named `review-script-perspective-*` (the prefix is the **sole** marker,
...
100% copy of a canonical. Skills stay team-level. The validator (`review-script-validation-criteria`) stays a **single
...
`products/review/backend/models.py`:
...
**PR-author → Insights user (label trigger falls back; other triggers don't).** The acting user whose perspectives
...
`insights/models/user.py:438`; already used in prod). **CORRECTION to earlier docs: this reverse-lookup DOES exist**
(a prior research pass wrongly said it didn't). When the author does **not** map to a Insights org user, the **label
...
model was renamed generic for exactly this). Validator rows carry the `review-script-validation-*` prefix; the prefix
...
`review-script-validation-*` skill and selects which one validates their reviews; absent any enabled validation row the
canonical `review-script-validation-criteria` is the default, so there is no min-1 floor (always a fallback).

What shipped:

- **`load_validation_skill_for_run(team_id, acting_user_id)`** (`skill_loader.py`, replacing the old single-arg
  team-latest-by-name resolve) seeds the canonical config, reads the user's single enabled `review-script-validation-*`
...
- **Shared-table hardening (the non-obvious part):** because perspectives and validators now share `ReviewSkillConfig`,
  ...
  `skill_name__startswith=REVIEW_FN_PERSPECTIVE_PREFIX`. Without this, an enabled validator row would be loaded/counted
...
**e2e ✅ (publish, 2026-06-30).** Ran `run_review --publish` against PR Insights/insights#67015 (team 1) with both
...
`"full-access"`.** The mode lives in `@hanzo/agent` `codex-client.ts` (`AUTO_APPROVED_KINDS` / `shouldAutoApprove`).
...
2. **Codex skill-get over MCP** — the review pulls the enabled `review-script-perspective-*` skills via `skill-get`; the
   sandbox installs the Codex skills dir + MCP serves live, so it should work, but confirm under the codex adapter.
3. **End-turn detection under the codex adapter** — `@hanzo/agent` normalizes `stopReason == "end_turn"` across
...
> `insights/insights` _sets_ the knobs, the `@hanzo/agent` package _applies_ them.** Verified end-to-end 2026-07-01.
...
- **Adapter split `src/server/agent-server.ts`:**
  - **Codex** → `codexOptions = {model: config.model ?? DEFAULT_CODEX_MODEL, reasoningEffort: config.reasoningEffort}`
    ...
    `-c model_reasoning_effort="…"` onto the Codex CLI, talking to the Insights gateway
    (`model_provider="insights"`, `wire_api="responses"`). _This is the line that applies our `xhigh` to `gpt-5.5`._
...
gateway model list in `@hanzo/agent`) or startup validation rejects it on one side.
