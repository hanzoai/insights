    ...
    url: z
        ...
        .describe(
            'A path template copied verbatim from the catalog below (e.g. `/persons/{uuid}`). Its ' +
                "`{placeholders}` are filled from `params`. These slugs come from Insights's canonical route " +
...
interface AppUrlResult {
    /** Full, canonical, clickable Insights URL. Surface this verbatim — do not rewrite it. */
    url: string
}
