...
from insights.models.message_assets.sql import INSERT_MESSAGE_ASSET_SQL
...
class TestPersonEmails(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
    ...
    def test_function_name_empty_when_workflow_deleted(self):
        ...
        # No InsightsFlow row → the frontend falls back to function_id for display, so this must be blank
    ...
    def test_excludes_standalone_insights_function_emails(self):
        self._seed("flow-row")
        self._seed("fn-row", function_kind="insights_function")
        rows = self._emails().json()
