...
Post one comment per completed run. Do not edit the PR body. The template uses a compact Insights QA Frontend format so reviewers can scan verdict, coverage, and findings quickly.
...
- ![flow](https://raw.githubusercontent.com/Insights/pr-assets/<sha>/2026/07/<uuid>.webp)
- ![still](https://raw.githubusercontent.com/Insights/pr-assets/<sha>/2026/07/<uuid>.png)
...
Use the `![alt](url)` markdown lines printed by `insightscli pr:upload-image` verbatim - they point at SHA-pinned `raw.githubusercontent.com/Insights/pr-assets` URLs. Do not reconstruct, shorten, or edit them.
...
Never include GitHub tokens or raw upload response bodies. If you copy any `insightscli pr:upload-image` output into the comment, take only the `![alt](url)` markdown lines from stdout.
