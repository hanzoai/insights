...
`run_node` for a Python node:
  1. registers the run's inputs — InsightsQL frames the server already streamed to local
...
A DuckDB node (Journey 5 — SQL over local frames, which can't push to Datastore) shares
...
mmapped InsightsQL inputs; the result binds back into the user namespace under `output_name`
...
class _Registration(NamedTuple):
    ...
    # "input": materialized from an upstream InsightsQL node's Arrow file, re-fetched on demand, so it
...
class KernelSession:
    ...
    def _preview(
        ...
        try:
            ...
            # Frames can carry values ujson can't encode — raw bytes from Datastore-native
