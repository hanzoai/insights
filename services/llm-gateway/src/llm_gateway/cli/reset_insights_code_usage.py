"""Reset Insights Desktop usage counters in Redis. Resets all users by default, or a single user with --user-id."""
...
async def _amain(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="reset-insights-code-usage",
        description="Reset Insights Desktop usage counters in the LLM gateway Redis. Resets every user unless --user-id is given.",
    )
    ...
    print(f"[{mode}] reset insights_code usage ({scope}): {deleted} keys")
    return 0
