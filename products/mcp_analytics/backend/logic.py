...
from insights.utils import generate_cache_key
...
# Short TTL so concurrent dashboard tabs / auto-refreshes share one Datastore
...
# validated structural fragments injected before parsing; {placeholders} are InsightsQL
# value placeholders.
#
...
# mismatch the GROUP BY key and Datastore rejects it. The bare field renders the raw
...
def list_mcp_sessions(
    ...
    One row per $session_id whose session overlaps the selected window, grouped in Datastore and
    ...
    ``date_from`` / ``date_to`` accept Insights date strings (relative like ``-7d`` or absolute
    ...
    Person email/name are resolved from distinct_id via personinsights. ``intent`` is
...
def _query_mcp_sessions(
    ...
    # so resolve here rather than via the InsightsQL {filters} placeholder (which only yields one).
...
def _extract_error_message(raw: Any) -> str | None:
    ...
    try:
        ...
        # InsightsQL's property accessor strips a string value's outer quotes but keeps the
...
def _resolve_persons(team_id: int, distinct_ids: list[str]) -> dict[str, Person]:
    ...
    if not unique_ids:
        return {}
    with personinsights_caller_tag("mcp-analytics/persons"):
