...
# Column names ``print_cohort_insightsql_query`` accepts for actor extraction, in
...
class QueryTargetsActorColumn(Scorer):
    ...
    A InsightsQL query must select one of ``person_id`` / ``actor_id`` / ``id`` /
    ...
    def _run_eval_sync(self, output: dict | None, expected=None, **kwargs) -> Score:
        ...
        return Score(
            name=self._name(),
            score=0.0,
            metadata={"reason": "InsightsQL query exposes no resolvable actor column", "sql": sql[:300]},
        )
