...
import { INSIGHTSQL_COLUMNS_KEY, getDefaultEventsQueryForTeam } from './defaultEventsQuery'
...
describe('getDefaultEventsQueryForTeam', () => {
    ...
    it.each([
        {
            name: 'prepends * for INSIGHTSQL columns that do not include it',
            columns: [INSIGHTSQL_COLUMNS_KEY, 'event', 'timestamp'],
            expectedSelect: ['*', 'event', 'timestamp'],
            expectedOrderBy: ['timestamp DESC'],
        },
        {
            name: 'does not duplicate * when INSIGHTSQL columns already include it',
            columns: [INSIGHTSQL_COLUMNS_KEY, '*', 'event', 'timestamp'],
            expectedSelect: ['*', 'event', 'timestamp'],
            expectedOrderBy: ['timestamp DESC'],
        },
        ...
        },
        {
            ...
            columns: [INSIGHTSQL_COLUMNS_KEY, 'event'],
