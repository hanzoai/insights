...
describe('getRandomCohort', () => {
    let action: Extract<CustomFlowAction, { type: 'random_cohort_branch' }>
    ...
    beforeEach(() => {
        ...
        action = findActionByType(customFlow, 'random_cohort_branch')!
    ...
    it('should select first cohort when random is in first range', () => {
        ...
        expect(result).toEqual(findActionById(invocation.customFlow, 'cohort_a'))
    })
    ...
    it('should select second cohort when random is in second range', () => {
        ...
        const result = getRandomCohort(invocation, action)
        expect(result).toEqual(findActionById(invocation.customFlow, 'cohort_b'))
    })
    ...
    it('should select third cohort when random is in third range', () => {
        ...
        const result = getRandomCohort(invocation, action)
        expect(result).toEqual(findActionById(invocation.customFlow, 'cohort_c'))
    })
    ...
    it('should handle edge cases at boundaries', () => {
        ...
        expect(result2).toEqual(findActionById(invocation.customFlow, 'cohort_b'))
    })
