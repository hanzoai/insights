...
import { DataTableNode, InsightsQLQuery, InsightsQLQueryResponse, NodeKind } from '~/queries/schema/schema-general'
import {
    ActivityScope,
    AnyPropertyFilter,
    Breadcrumb,
            ...
            person: [
                null as PersonType | null,
                {
                    ...
                    loadPersonUUID: async ({ uuid }, breakpoint): Promise<PersonType | null> => {
                        let response: InsightsQLQueryResponse
                        try {
                            ...
                            if (isAbortedRequest(error)) {
                                insights.capture('person_uuid_load_aborted')
                                breakpoint()
        ...
        editProperty: async ({ key, newValue }) => {
            ...
            if (person && person.id) {
                ...
                try {
                    ...
                    toast.success(`Person property ${action}`)
                    ...
                    toast.error(`Failed to update person property`)
                }
            }
        },
        deleteProperty: async ({ key }) => {
            ...
            if (person && person.id) {
                ...
                try {
                    ...
                    toast.success(`Person property deleted`)

                    eventUsageLogic.actions.reportPersonPropertyUpdated('removed', 1, undefined, undefined)
                } catch {
                    actions.setPerson({ ...person })
                    toast.error(`Failed to delete person property`)
                }
            }
        },
