...
_CATEGORY_DEFAULTS: dict[CDCErrorCategory, tuple[str, bool]] = {
    ...
    CDCErrorCategory.CONNECTION_FAILED: (
        "Could not connect to the source database. Insights will keep retrying — if this persists, "
        "check that the database is reachable and accepting connections.",
        True,
    ),
    CDCErrorCategory.HOST_UNREACHABLE: (
        "Insights has no network route to the source database host, so it can't be reached. Check "
        "that the host and port are correct and reachable from the public internet (Insights's IP "
    ...
    CDCErrorCategory.PUBLICATION_MISSING: (
        ...
        "Recreate it (self-managed), or use Repair CDC (Insights-managed) to recreate it and re-sync.",
        False,
    ),
    CDCErrorCategory.SLOT_IN_USE: (
        "The replication slot is currently in use by another connection. Insights will retry shortly.",
        True,
    ),
    CDCErrorCategory.WAL_DECODE_ERROR: (
        "Insights could not decode the change stream from the source database. This usually points to "
    ...
    CDCErrorCategory.UNKNOWN: (
        "Change data capture hit an unexpected error. Insights will retry automatically — if it "
