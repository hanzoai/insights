...
Run via manage.py shell so Django is configured; set OUT_DIR to the experiment's runs/ directory:
    ...
    LABEL=C0-baseline RUN_SECONDS=812 RUN_START_EPOCH=1751... OUT_DIR=products/review/eval/experiments/<exp>/runs \
...
from products.review.backend.reviewer import constants
...
OUT_DIR = os.environ.get("OUT_DIR", "products/review/eval/experiments/2026-07-reviewer-topology/runs")
...
def _spend_rows(start_dt):
    ...
    from insights.datastore.client import sync_execute  # noqa: PLC0415 — optional, only for the spend tally
