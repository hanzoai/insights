...
from insights.insightsql.property_planner import (
...
from insights.insightsql.type_system import ComparisonCompatibility
...
class TestNewEventsSchemaArraySubcolumns(SimpleTestCase):
    def _context(self) -> InsightsQLContext:
        team = Team(id=1, project_id=1)
        context = InsightsQLContext(team_id=team.id, team=team, enable_select_queries=True)
    ...
    def _print_select(self, select: str) -> str:
        ...
        with patch("insights.insightsql.printer.utils.build_property_swapper"):
    ...
    def _plan_where_comparison(self, select: str) -> PropertyComparisonPlan:
        expr = parse_select(select)
        context = self._context()
        resolved = cast(ast.SelectQuery, resolve_types(expr, context, dialect="datastore"))
...
class TestPropertyTypes(BaseTest):
    ...
    def _resolve_select(
        ...
        select: str,
        restricted_properties: set[tuple[str, int]] | None = None,
    ) -> tuple[InsightsQLContext, ast.SelectQuery]:
        ...
        node = cast(ast.SelectQuery, resolve_types(expr, context, dialect="datastore"))
    ...
    def _prepare_select(
        ...
        _, prepared = prepare_and_print_ast(expr, context, "datastore")
        assert isinstance(prepared, ast.SelectQuery)
        return context, prepared

    def test_property_comparison_planner_marks_string_minmax_ready(self) -> None:
        property_name = "$session_id" if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA else "$browser"
    ...
    def test_property_comparison_planner_allows_numeric_minmax_when_source_type_matches(self) -> None:
        ...
        with patch(
            "insights.datastore.materialized_columns.get_enabled_materialized_columns_by_table",
    ...
    def test_property_type_override_beats_detected_type(self):
        ...
        def _print(overrides: dict[str, str] | None) -> str:
            query, _ = prepare_and_print_ast(
                ...
                InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, property_type_overrides=overrides),
                "datastore",
            )
    ...
    def test_resolve_property_types_from_qualified_insights_events(self):
        # Selecting from the qualified `insights.events` form must produce the same property-type
        ...
        qualified = self._print_select("select properties.$screen_width, properties.bool from insights.events")
        # Both root-level `events` and `insights.events` resolve to the same EventsTable and print
    ...
    def test_numeric_property_not_double_cast_inside_string_parser(self, _name: str, expr: str, ch_fn: str):
        ...
        # Float, otherwise Datastore raises ILLEGAL_TYPE_OF_ARGUMENT on a Float64 argument.
...
class TestJSONExtractToMaterializedColumn(DatastoreTestMixin, BaseTest):
    ...
    def test_new_events_schema_nested_jsonextractstring_uses_string_default(self):
        ...
        assert "%(insightsql_val_0)s" in printed, printed
        assert "JSONExtractKeysAndValuesRaw" not in printed, printed
    ...
    def test_scalar_jsonextract_preserves_datastore_coercions(self):
        for tag, value in (
            ("boolean", True),
            ("float", 1.5),
            ("integer", 2),
        ...
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA:
            ...
            assert "events.properties.value" in response.datastore
            assert "JSONExtractKeysAndValuesRaw" not in response.datastore
    ...
    def test_jsonextractraw_typed_array_preserves_missing_default(self):
        ...
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA:
            assert response.datastore is not None
            assert "events.properties.`$active_feature_flags`" in response.datastore
            assert "JSONExtractKeysAndValuesRaw" not in response.datastore
    ...
    def _run_and_collect(
        ...
        insightsql = f"SELECT properties.tag, {extract_expr} FROM events WHERE event = 'pageview' ORDER BY properties.tag"
        response = execute_insightsql_query(insightsql, team=self.team)
        assert response.results is not None
        values = {row[0]: row[1] for row in response.results}
        return values, response.datastore or ""
...
class TestEventsSchemaPropertyParity(DatastoreTestMixin, HypothesisDjangoTestCase, BaseTest):
...
#
# InsightsQL wraps timestamp fields with toTimeZone(timestamp, tz) for timezone
# support (see PropertySwapper.visit_field). Datastore can't derive partition
...
class TestTimezoneIndexPruning(DatastoreTestMixin, BaseTest):
    """
    Verify that timezone-aware date filters allow Datastore to prune
    partitions and use primary key indexes on the events table.
    """

    def setUp(self):
        super().setUp()
        # Create events across multiple months so Datastore produces
    ...
    def _events_table_ref(self) -> str:
        return "events_json" if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA else "events"

    def _assert_primary_key_uses_timestamp_range(self, primary_key: dict) -> None:
        pk_keys = primary_key.get("Keys", [])
        expected_timestamp_key = "timestamp" if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA else "toDate(timestamp)"
        assert any(expected_timestamp_key in key for key in pk_keys), (
            f"Expected PK to use {expected_timestamp_key}, got Keys={pk_keys}"
        )

    def _compile_insightsql(self, insightsql: str, timezone: str = "UTC") -> tuple[str, dict]:
        ...
        context = InsightsQLContext(
            team_id=self.team.pk,
            enable_select_queries=True,
            modifiers=InsightsQLQueryModifiers(pushDownPredicates=False),
        )
        node = parse_select(insightsql)
        datastore_sql, _ = prepare_and_print_ast(node, context=context, dialect="datastore")
        return datastore_sql, context.values
    ...
    def test_toTimeZone_breaks_partition_and_pk_pruning(self, tz):
        ...
        If this test starts failing, Datastore has learned to derive
        ...
        assert partition is not None
        assert partition.get("Condition") == "true", (
            f"tz={tz}: Datastore is now pruning partitions with toTimeZone — "
            f"the workaround may be removable. "
            f"Partition Condition={partition.get('Condition')!r}"
        )

    def test_insightsql_compiled_query_has_partition_pruning(self):
        """The InsightsQL pipeline strips toTimeZone from WHERE comparisons to restore pruning."""
        sql, values = self._compile_insightsql(
    ...
    def test_toTimeZone_stripped_from_where_but_kept_in_select(self):
        ...
        sql, _ = self._compile_insightsql(
    ...
    def _assert_correct_results(self, insightsql: str, timezone: str, expected_count: int):
        self.team.timezone = timezone
        self.team.save()
        response = execute_insightsql_query(insightsql, team=self.team)
        assert response.results is not None
    ...
    def test_dst_boundary_does_not_drop_events(self):
        ...
        insightsql = "SELECT count() FROM events WHERE event = 'dst_test' AND timestamp >= '2024-03-10' AND timestamp < '2024-03-11'"
        self._assert_correct_results(insightsql, timezone="America/New_York", expected_count=2)
    ...
    def test_positive_utc_offset_does_not_drop_events(self):
        ...
        insightsql = "SELECT count() FROM events WHERE event = 'tokyo_test' AND timestamp >= '2024-03-01' AND timestamp < '2024-03-02'"
        self._assert_correct_results(insightsql, timezone="Asia/Tokyo", expected_count=2)
    ...
    def test_utc_returns_correct_results(self):
        ...
        insightsql = "SELECT count() FROM events WHERE event = 'utc_test' AND timestamp >= '2024-03-01' AND timestamp < '2024-03-02'"
        self._assert_correct_results(insightsql, timezone="UTC", expected_count=2)
    ...
    def test_brazil_historical_dst_does_not_drop_events(self):
        ...
        insightsql = "SELECT count() FROM events WHERE event = 'brazil_test' AND timestamp >= '2018-11-15' AND timestamp < '2018-11-16'"
        self._assert_correct_results(insightsql, timezone="America/Sao_Paulo", expected_count=2)
    ...
    def test_half_hour_offset_does_not_drop_events(self):
        ...
        insightsql = "SELECT count() FROM events WHERE event = 'kolkata_test' AND timestamp >= '2024-03-01' AND timestamp < '2024-03-02'"
        self._assert_correct_results(insightsql, timezone="Asia/Kolkata", expected_count=2)
    ...
    def test_lord_howe_half_hour_dst_does_not_drop_events(self):
        ...
        insightsql = "SELECT count() FROM events WHERE event = 'lhi_test' AND timestamp >= '2024-01-16' AND timestamp < '2024-01-17'"
        self._assert_correct_results(insightsql, timezone="Australia/Lord_Howe", expected_count=2)
    ...
    def test_iso8601_z_suffixed_filter_value_does_not_error(self, tz):
        ...
        self._assert_correct_results(insightsql, timezone=tz, expected_count=3)
    ...
    def test_alias_preserved_when_recursing_into_assumeNotNull(self):
        ...
        from insights.insightsql import ast as ast_module
