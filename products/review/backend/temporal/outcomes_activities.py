...
from insights.ph_client import ScopedCapture, ph_scoped_capture
...
from products.review.backend.reviewer.outcomes.classify import classify_team
from products.review.backend.reviewer.outcomes.discovery import team_ids_with_unclassified_published_reports
from products.review.backend.temporal.outcomes_types import ClassifyTeamOutcomesInputs
...
async def _scoped_capture_off_loop() -> AsyncIterator[ScopedCapture]:
    ...
    Leaving the scope calls `Insights.shutdown()`, which flushes with `timeout_seconds=None` and so
