...
class TestUpdateTaskRunStatusActivity:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"wizard_head_branch": "insights/instrumentation-ab12cd"}, None, 1.0),
            ({"wizard_head_branch": "insights/instrumentation-ab12cd"}, {"pr_url": "https://x/pull/1"}, 0.0),
            ({}, None, 0.0),
        ],
    )
    def test_terminal_status_counts_unbound_wizard_runs(
        ...
        before = REGISTRY.get_sample_value("insights_tasks_wizard_run_unbound_total", labels) or 0.0

        input_data = UpdateTaskRunStatusInput(run_id=str(test_task_run.id), status=TaskRun.Status.COMPLETED)
        async_to_sync(activity_environment.run)(update_task_run_status, input_data)

        after = REGISTRY.get_sample_value("insights_tasks_wizard_run_unbound_total", labels) or 0.0
