...
from insights.api.sharing import check_can_edit_sharing_configuration
from insights.api.test.test_sharing import mock_exporter_template
...
class _FeatureFlagEnabledMixin(APIBaseTest):
    """Auto-mock `hanzo_insights.feature_enabled` so flag-gated viewsets accept calls in tests."""
...
class TestVapiWebhook(APIBaseTest):
    ...
    @override_settings(VAPI_WEBHOOK_SECRET="topsecret")
    @patch("products.user_interviews.backend.presentation.webhooks.hanzo_insights.capture")
    ...
    def test_webhook_status_update_duplicate_in_progress_emits_same_insert_id(self, mock_capture):
        ...
        # so Insights dedupes the second delivery at ingest. Both captures fire here (we don't
    ...
    def test_send_invites_uses_explicit_subject_and_reply_to(self):
        ...
        with (
            ...
            self.client.post(
                ...
                data={"subject": "Hey, can we chat?", "reply_to": "research@hanzo.ai"},
                format="json",
            )

        kwargs = mock_message_cls.call_args.kwargs
        self.assertEqual(kwargs["subject"], "Hey, can we chat?")
        self.assertEqual(kwargs["reply_to"], "research@hanzo.ai")
...
class TestInterviewInviteTemplateRendering(unittest.TestCase):
    def _render(self, **context: Any) -> str:
        ctx: dict[str, Any] = {
            ...
            "interview_url": "https://app.hanzo.ai/interview/abc",
