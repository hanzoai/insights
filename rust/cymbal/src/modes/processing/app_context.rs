...
pub struct AppContext {
    ...
    // Dedicated producer for `datastore_app_metrics2`. Points at warpstream-ingestion when
...
impl AppContext {
    pub async fn from_config(config: &ProcessingConfig) -> Result<Self, UnhandledError> {
        let insights_pool = build_pg_pool(
        ...
        AppContext::new(config, s3_client, insights_pool, issue_buckets_redis_client).await
    }
    ...
    pub async fn new(
        ...
        let catalog = build_catalog(&config.resolver, s3_client, insights_pool.clone());
