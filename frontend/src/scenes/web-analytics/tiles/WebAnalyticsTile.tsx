...
const BreakdownValueCell: QueryContextColumnComponent = (props) => {
    ...
    switch (breakdownBy) {
        ...
        case WebStatsBreakdown.InitialReferringDomain:
        case WebStatsBreakdown.FirstPageviewReferringDomain:
            // NULL referrer is canonically "Direct" in Insights (matches the channel-type
            ...
            {(showIntervalTile || preZoomDateFilter) && (
                <div className="flex flex-row items-center justify-end m-2 mr-4">
                    <div className="flex flex-row items-center gap-2">
                        ...
                        {preZoomDateFilter && (
                            <Button type="secondary" size="small" icon={<IconUndo />} onClick={resetZoom}>
                                Reset zoom
                            </Button>
                        )}
                    </div>
...
interface InsightsQLTableTileProps {
    uniqueKey: string
...
const InsightsQLTableTile = (props: InsightsQLTableTileProps): JSX.Element => {
    if (BOT_INSIGHTSQL_TILES.has(props.tileId)) {
        return <BotInsightsQLTableTile {...props} />
    }
    return (
        <div className="border rounded bg-surface-primary flex-1 flex flex-col py-2 px-1">
            {props.headerSlot}
...
export const WebQuery = ({
    ...
    if (query.kind === NodeKind.DataTableNode && query.source.kind === NodeKind.InsightsQLQuery) {
        return (
            <InsightsQLTableTile
                uniqueKey={uniqueKey}
                attachTo={attachTo}
