...
from insights.dags.detach_distinct_id import (
...
# persons DB rather than the personinsights fake.
...
class TestDetachDistinctIdJob:
    ...
    @patch("insights.dags.detach_distinct_id.sync_execute")
    @patch("insights.dags.detach_distinct_id.uuid.uuid4", return_value=UUID(DUMMY_OVERRIDE_UUID))
...
class TestDetachDistinctIdIntegration:
    ...
    def _pdi_exists(cls, pdi_id: int) -> bool:
        with cls._get_persons_conn() as conn, conn.cursor() as cursor:
            cursor.execute("SELECT 1 FROM insights_persondistinctid WHERE id = %s", [pdi_id])
    ...
    def person_with_two_distinct_ids(self, team):
        ...
        with self._get_persons_conn() as conn, conn.cursor() as cursor:
            cursor.execute(
                "INSERT INTO insights_person (team_id, uuid, properties, is_identified, version, created_at, properties_last_updated_at, properties_last_operation) "
            ...
            cursor.execute(
                "INSERT INTO insights_persondistinctid (team_id, person_id, distinct_id, version) "
                "VALUES (%s, %s, %s, %s) RETURNING id",
                [team.id, person_id, "$insights_cookieless", 3],
            )
        ...
        pdi_detach = SimpleNamespace(id=pdi_detach_id, distinct_id="$insights_cookieless", version=3)
