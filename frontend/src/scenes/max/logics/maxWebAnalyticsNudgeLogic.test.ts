...
describe('maxWebAnalyticsNudgeLogic', () => {
    ...
    describe('insights.capture reporting', () => {
        let captureSpy: jest.SpyInstance

        beforeEach(() => {
            captureSpy = jest.spyOn(insights, 'capture').mockImplementation(() => undefined as any)
            setNoRecentVisit()
        })
        ...
        it('fires both "eligible" and "shown" exactly once for variant "test" on mount', () => {
            ...
            expect(captureSpy).toHaveBeenCalledWith('insights ai web analytics nudge eligible', expectedProperties)
            expect(captureSpy).toHaveBeenCalledWith('insights ai web analytics nudge shown', expectedProperties)
            expect(
        ...
        it('fires "eligible" only (no "shown") for variant "control"', () => {
            ...
            expect(captureSpy).toHaveBeenCalledWith(
                'insights ai web analytics nudge eligible',
                expect.objectContaining({ variant: 'control' })
            )
            expect(captureSpy).not.toHaveBeenCalledWith('insights ai web analytics nudge shown', expect.anything())
        })
        ...
        it('fires neither event when not enrolled in the flag', () => {
            ...
            expect(captureSpy).not.toHaveBeenCalledWith('insights ai web analytics nudge eligible', expect.anything())
            expect(captureSpy).not.toHaveBeenCalledWith('insights ai web analytics nudge shown', expect.anything())
        })
    })
    ...
    describe('nudgeClicked and nudgeDismissed actions', () => {
        ...
        it('nudgeClicked fires the clicked event with reportProperties', async () => {
            ...
            expect(captureSpy).toHaveBeenCalledWith('insights ai web analytics nudge clicked', {
                conversation_id: 'conv-1',
                message_id: 'msg-1',
                variant: 'test',
                trigger: 'chart',
        ...
        it('nudgeClicked navigates to web analytics so the chat moves to the side panel', async () => {
            ...
            expect(pushSpy).toHaveBeenCalledWith(urls.webAnalytics(), { utm_source: 'insights-ai-nudge' })
        })
        ...
        it('nudgeDismissed fires the dismissed event and flips shouldShowNudge to false', async () => {
            ...
            expect(captureSpy).toHaveBeenCalledWith('insights ai web analytics nudge dismissed', {
                conversation_id: 'conv-1',
                message_id: 'msg-1',
                variant: 'test',
                trigger: 'chart',
    ...
    describe('once-per-session deduplication', () => {
        ...
        it('does not fire eligible or shown again for a second message, and shouldShowNudge is false on it', async () => {
            ...
            const eligibleCalls1 = captureSpy.mock.calls.filter(
                (c) => c[0] === 'insights ai web analytics nudge eligible'
            ).length
            const shownCalls1 = captureSpy.mock.calls.filter(
                (c) => c[0] === 'insights ai web analytics nudge shown'
            ).length
