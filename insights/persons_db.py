...
that are too bespoke for the personinsights gRPC client connect to the persons database
...
# the Django persons-DB config (insights/settings/data_stores.py), so the fallback
# resolves to the same local database whether or not Django is in the loop.
LOCAL_DEV_PERSONS_DB_URL = (
    f"postgres://{os.getenv('PGUSER', 'insights')}:{os.getenv('PGPASSWORD', 'insights')}"
    f"@{os.getenv('PGHOST', 'db')}:{os.getenv('PGPORT', '5432')}/insights_persons"
)
