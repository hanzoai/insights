...
from posthog.insightsql.compiler.javascript_stl import STL_FUNCTIONS, import_stl_functions
...
class JavaScriptCompiler(Visitor):
    ...
    def _visit_hog_ast(self, node: AST | None) -> str:
        ...
        for field in dataclasses.fields(node):
            ...
            if value is None:
                continue
            fields.append(f"{json.dumps(field.name)}: {self._visit_insightsqlx_value(value)}")
        return "{" + ", ".join(fields) + "}"
    ...
    def visit_call(self, node: ast.Call):
        # InsightsQL functions can come as name(params)(args), or name(args) if no params
    ...
    def visit_lambda(self, node: ast.Lambda):
        ...
        elif isinstance(node.expr, ast.Dict) or isinstance(node.expr, ast.InsightsQLXTag):
    ...
    def visit_insightsqlx_tag(self, node: ast.InsightsQLXTag):
        if any(local for local in self.locals if local.name == node.kind):
            ...
            for attr in node.attributes:
                attrs.append(f'"{attr.name}": {self._visit_insightsqlx_value(attr.value)}')
    ...
    def _visit_insightsqlx_value(self, value: Any) -> str:
        if isinstance(value, AST):
            return self.visit(value)
        if isinstance(value, list):
            elems = ", ".join([self._visit_insightsqlx_value(v) for v in value])
            return f"[{elems}]"
        if isinstance(value, dict):
            items = ", ".join(
                [f"{self._visit_insightsqlx_value(k)}: {self._visit_insightsqlx_value(v)}" for k, v in value.items()]
            )
            return f"{{{items}}}"
