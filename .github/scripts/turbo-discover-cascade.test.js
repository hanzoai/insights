...
test('core is never a node: insights/ee/<root> are excluded as keys and values, and closures never traverse through them', () => {
    const toml = `
[[modules]]
path = "<root>"
depends_on = []
...
`
    const graph = parseTachModules(toml)
    assert.equal(graph.has('insights'), false)
    assert.equal(graph.has('ee'), false)
    assert.equal(graph.has('<root>'), false)
    for (const deps of graph.values()) {
        assert.ok(!deps.includes('insights'))
        assert.ok(!deps.includes('ee'))
    }

    // x and y both only reach each other via insights — not a real edge in the
