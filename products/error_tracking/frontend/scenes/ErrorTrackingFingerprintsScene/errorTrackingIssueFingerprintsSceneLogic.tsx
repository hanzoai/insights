...
export type errorTrackingIssueFingerprintsSceneLogicType = MakeLogicType<
    errorTrackingIssueFingerprintsSceneLogicValues,
    errorTrackingIssueFingerprintsSceneLogicActions,
    ErrorTrackingIssueFingerprintsSceneProps,
    errorTrackingIssueFingerprintsSceneLogicMeta
>
    ...
    listeners(({ actions, props, values }) => ({
        ...
        splitIssueSuccess: ({ newIssueIds }) => {
            if (newIssueIds.length === 0) {
                toast.warning('No fingerprints were unmerged')
                actions.loadIssueFingerprints()
            } else if (newIssueIds.length === 1) {
                toast.success('Fingerprint unmerged successfully', {
                    button: {
                        label: 'View issue',
                ...
                toast.success(`${newIssueIds.length} fingerprints unmerged successfully`)
            }
        },
        mutationFailure: ({ mutationName }) => {
            if (mutationName === 'splitIssues') {
                toast.error('Failed to unmerge fingerprint')
                actions.loadIssueFingerprints()
            }
        },
    })),
