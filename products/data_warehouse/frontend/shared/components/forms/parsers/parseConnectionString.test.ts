import { parseDatastoreConnectionString } from './datastore'
...
describe('parseDatastoreConnectionString', () => {
    it('parses an https URL with secure=true and default port 8443', () => {
        const result = parseDatastoreConnectionString('https://default:pw@play.datastore.com/default')
        expect(result.isValid).toBe(true)
        expect(fieldMap(result.fields)).toEqual({
            host: 'play.datastore.com',
            port: '8443',
            database: 'default',
            user: 'default',
            password: 'pw',
    ...
    it('parses an http URL with secure=false and default port 8123', () => {
        const result = parseDatastoreConnectionString('http://default:pw@localhost/default')
        expect(result.isValid).toBe(true)
        expect(fieldMap(result.fields).secure).toBe('false')
        expect(fieldMap(result.fields).port).toBe('8123')
    })

    it('parses a datastore:// URL with secure=false and default port 9000', () => {
        const result = parseDatastoreConnectionString('datastore://default:pw@localhost/default')
        expect(fieldMap(result.fields).secure).toBe('false')
        expect(fieldMap(result.fields).port).toBe('9000')
    })

    it('parses a datastores:// URL as secure with default port 9440', () => {
        const result = parseDatastoreConnectionString('datastores://default:pw@localhost/default')
        expect(fieldMap(result.fields).secure).toBe('true')
        expect(fieldMap(result.fields).port).toBe('9440')
    })

    it('honors ?secure=true override on a datastore:// URL and switches default to 9440 (native family)', () => {
        const result = parseDatastoreConnectionString('datastore://default:pw@localhost/default?secure=true')
        expect(fieldMap(result.fields).secure).toBe('true')
        expect(fieldMap(result.fields).port).toBe('9440')
    })

    it('honors ?secure=true override on an http:// URL and switches default to 8443 (HTTP family)', () => {
        const result = parseDatastoreConnectionString('http://default:pw@localhost/default?secure=true')
        expect(fieldMap(result.fields).secure).toBe('true')
    ...
    it('honors ?secure=false override on an https:// URL and switches default to 8123 (HTTP family)', () => {
        const result = parseDatastoreConnectionString('https://default:pw@localhost/default?secure=false')
        expect(fieldMap(result.fields).secure).toBe('false')
        expect(fieldMap(result.fields).port).toBe('8123')
    })

    it('respects an explicit port over the scheme default', () => {
        const result = parseDatastoreConnectionString('https://default:pw@play.datastore.com:8123/default')
        expect(fieldMap(result.fields).port).toBe('8123')
    })

    it('rejects unsupported schemes', () => {
        expect(parseDatastoreConnectionString('postgres://default:pw@host/default').isValid).toBe(false)
    })
})
...
describe('parseConnectionStringForSource dispatcher', () => {
    it('returns a parser for every supported source name', () => {
        expect([...SUPPORTS_CONNECTION_STRING].sort()).toEqual(
            ['Datastore', 'MSSQL', 'MySQL', 'Postgres', 'Redshift', 'Snowflake', 'Supabase'].sort()
        )
    })
