...
Any MCP server instrumented with the `@hanzo/mcp` SDK — and Insights's own MCP server — emits a `$mcp_tool_call` event on the shared `events` table every time an agent invokes a tool. There is **no dedicated Datastore table** — all fields live as `$mcp_*` properties on `events`, queried directly with `insights:execute-sql`. This is the data behind the MCP analytics dashboard, tool-quality, and tool-detail screens; every metric on those screens is reproducible as InsightsQL over this event.

Query the canonical `$`-prefixed event name. Servers instrumented with the `@hanzo/mcp` SDK emit only `$mcp_tool_call` / `$mcp_initialize`; Insights's own hosted server additionally dual-emits legacy un-prefixed `mcp_tool_call` / `mcp_initialize` aliases through a transition shim. Match the canonical name only — an `event IN ('mcp_tool_call', '$mcp_tool_call')` would double-count Insights's own server.

**For a single tool, prefer the typed tools.** Each takes a `toolName` plus a `dateRange`, runs the same query runner the tool-detail UI uses, and is gated behind the `mcp-analytics` flag, so results match the UI exactly and you don't re-derive the SQL below. `toolName` is the effective name (resolved server-side — the inner tool of a single-exec wrapper call) for all of them, including `insights:query-mcp-tool-failures`:

| question about one tool                                             | tool                                         |
| ------------------------------------------------------------------- | -------------------------------------------- |
| headline numbers (calls, errors, p50/p95, users, sessions, intents) | `insights:query-mcp-tool-stats`               |
| day-by-day trend                                                    | `insights:query-mcp-tool-daily-stats`         |
| top failure buckets, by harness                                     | `insights:query-mcp-tool-failures`            |
| individual errored calls in one failure bucket                      | `insights:query-mcp-tool-failure-occurrences` |
| top callers (incl. person email/name)                               | `insights:query-mcp-tool-top-users`           |
| tools called before/after it (`neighborDirection: before`/`after`)  | `insights:query-mcp-tool-neighbors`           |
| recent agent intents                                                | `insights:query-mcp-tool-sample-intents`      |
| distinct descriptions seen                                          | `insights:query-mcp-tool-descriptions`        |

And `insights:query-mcp-harness-breakdown` for the cross-tool harness cut (see below).
...
| ------------------------------------------------------- | ------------------------------------------------ |
| list sessions (calls, start/end, tools, client, person) | `insights:mcp-analytics-sessions-list`            |
| one session's calls, chronological                      | `insights:mcp-analytics-sessions-tool-calls`      |
| LLM summary of one session's goal                       | `insights:mcp-analytics-sessions-generate-intent` |

Three things to know before using them:

- **7-day lookback by default.** `insights:mcp-analytics-sessions-tool-calls` and `insights:mcp-analytics-sessions-generate-intent` both scan 7 days back, so an older session returns empty unless you pass its `session_start` as `date_from`. Carry that value forward from the list row.
...
And two tools cover what SQL can't express at all: `insights:mcp-analytics-intent-clusters-retrieve` and `insights:mcp-analytics-intent-clusters-recompute` (embedding-based intent clustering).

**InsightsQL is the path for everything else** — cross-tool rankings (the tool-quality matrix), custom breakdowns, errored-session filtering, effective tool names within a session — query them with `execute-sql`. It is also the fallback when the `mcp-analytics` flag is off: every typed tool above is gated behind it, `execute-sql` is not.

## Key properties

`Source` says where the property comes from: **SDK** — set by `@hanzo/mcp` (`packages/mcp/src/extensions/constants.ts`), present on any instrumented customer server; **server** — stamped only by Insights's own hosted MCP server (`services/mcp`), so it exists only for Insights's dogfood data; **exec** — only present when the server runs in single-exec mode (one `exec` dispatcher tool instead of one tool per name).
...
| `$mcp_error_status`               | server | HTTP status code for an errored call, when the failure came from an HTTP call. Stamped by Insights's own server (`services/mcp/src/hono/tool-executor.ts`) — not part of the SDK's field set, so an externally-instrumented server on the SDK alone won't emit it.                                                                                                                                                                                                                                                                |
...
| `$mcp_response`                   | SDK    | The response the MCP server returned, redacted the same way as `$mcp_parameters`. Stays empty on Insights's hosted server.                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `$mcp_tool_category`              | server | Tool category, when tagged. Stamped from Insights's tool catalog; external servers can declare one per tool.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `$mcp_tool_description`           | SDK    | Tool description as seen by the agent (revisions over time), clipped to 512 chars on capture. Gap warning: the hono migration silently dropped this stamp, so there is a window (roughly Jun-Jul 2026) with no descriptions on Insights's hosted server; `notEmpty(...)` filters are mandatory.                                                                                                                                                                                                                                   |
...
| `$mcp_server_name`                | SDK    | Advertised name of the MCP server that handled the request (e.g. `Insights`).                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
The `coalesce(..., '[]')` is required: the property accessor is Nullable, and `JSONExtract` of a Nullable into `Array(String)` is a Datastore type error.

**Failures with detail.** `$mcp_tool_call` carries `$mcp_is_error` plus a semantic `$mcp_error_type` and, for HTTP failures, `$mcp_error_status` (see the Source column above: `$mcp_error_type` is an SDK field that Insights's own server also stamps itself, while `$mcp_error_status` is stamped only by Insights's server, so a separately-instrumented server on the SDK alone won't emit it). `insights:query-mcp-tool-failures` groups errored tool calls by these two fields and returns each bucket's raw `error_type`/`error_status`; pass those to `insights:query-mcp-tool-failure-occurrences` for individual errored calls with the free-text `$mcp_error_message` (sanitized, truncated to 2048 chars — empty on events captured before Insights's server started emitting it). `$mcp_response` stays empty on Insights's hosted server. Insights's own tool calls also don't emit `$exception` events — those only exist for separately-instrumented MCP servers.
...
**Error rate of one tool** (single-tool headline numbers are `insights:query-mcp-tool-stats`; use this for a custom predicate):
...
**Tool-quality matrix** (error rate + latency percentiles + reach, one row per tool) — this cross-tool ranking has no typed tool; once you've picked a tool, drill into it with `insights:query-mcp-tool-stats`, `insights:query-mcp-tool-failures`, or `insights:query-mcp-tool-daily-stats`:
...
**Daily activity** (success/error split for a time series) — for one tool's daily series prefer `insights:query-mcp-tool-daily-stats`; this all-tools version is the custom cut:
