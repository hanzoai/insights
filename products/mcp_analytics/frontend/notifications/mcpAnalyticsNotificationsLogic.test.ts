...
function makeNotification(id: string, overrides: Partial<InsightsFunctionType> = {}): InsightsFunctionType {
    return {
        id,
...
describe('mcpAnalyticsNotificationsLogic', () => {
    ...
    beforeEach(async () => {
        initKeaTests()
        listSpy = jest.spyOn(api.insightsFunctions, 'list').mockResolvedValue({
            count: 2,
            next: null,
            previous: null,
            results: [],
        })
        jest.spyOn(api.insightsFunctions, 'update').mockResolvedValue(makeNotification('updated'))
        jest.spyOn(toast, 'warning').mockImplementation(() => 'toast-id')
    ...
    it('keeps the fetched rows and warns when the list is truncated', async () => {
        ...
        expect(toast.warning).toHaveBeenCalledWith(expect.stringContaining('first 500'))
    })
    ...
    it('blocks a second toggle and reconciles the first with the server response', async () => {
        ...
        const updateDeferred = createDeferred<InsightsFunctionType>()
        const updateSpy = jest.spyOn(api.insightsFunctions, 'update').mockReturnValue(updateDeferred.promise)
    ...
    it('keeps an in-flight reload visible when a toggle writes its confirmed row', async () => {
        const pendingReload = createDeferred<Awaited<ReturnType<typeof api.insightsFunctions.list>>>()
