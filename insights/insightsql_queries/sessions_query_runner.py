...
class SessionsQueryRunner(AnalyticsQueryRunner[SessionsQueryResponse]):
    ...
    def _build_person_display_name_expr(self) -> str:
        """Build the InsightsQL expression for person_display_name using a subquery join."""
        ...
        with self.timings.measure("build_ast"):
            ...
            with self.timings.measure("filters"):
                ...
                if self.query.personId:
                    with self.timings.measure("person_id"), personinsights_caller_tag("persons/sessions-query"):
                    ...
                    with self.timings.measure("test_account_filters"):
                        for prop in self._get_test_account_filters():
                            ...
                            if prop_type == "session":
                                ...
                                # Unknown types (insightsql, etc.) — route through events subquery as a best-effort fallback
                                test_account_event_filters.append(prop)
    ...
    def _calculate(self) -> SessionsQueryResponse:
        # `SessionsQuery.select`, `where`, `orderBy` are user-supplied InsightsQL strings.
