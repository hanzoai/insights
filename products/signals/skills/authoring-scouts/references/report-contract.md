...
**A chart query must not carry anything executable.** ScriptVM `bytecode` (what conditional formatting compiles to), a nested `ScriptQuery`, and `sendRawQuery` are each refused with a 400 wherever they sit in the node, because a chart renders data rather than running code in the reader's session.
...
A query over a warehouse connection is fine as long as it goes through InsightsQL: keep `connectionId`, drop `sendRawQuery`.
...
- **`user_uuid`** — a **Insights user UUID**.
  The server resolves it to that org member's linked GitHub login for you (and it wins if you also pass a `github_login`).
  Use this whenever your evidence already names a Insights user — an account owner, an entity's `created_by`, a CSM — so you can route to them without ever looking up their handle.
...
If you already hold a Insights user UUID, prefer passing it as `user_uuid` — it's the most reliable.
