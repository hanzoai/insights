...
Follows the established Temporal interceptor pattern (see insights/temporal/ai_observability/metrics.py).
...
registered in ``insights/temporal/common/worker.py`` actually apply.
"""
...
class _ActivityInboundInterceptor(ActivityInboundInterceptor):
    async def execute_activity(self, input: ExecuteActivityInput) -> typing.Any:
        ...
        # retry rate; without it a transient Datastore blip is indistinguishable from a healthy first-try
...
class ExperimentsRecalculationMetricsInterceptor(Interceptor):
    ...
    # Required by `is_task_queue_supported` in `insights/temporal/common/interceptor.py` — without this attribute
    # the interceptor is filtered out of every worker and the metrics never emit. The recalc workflow + activities
    # are registered on this queue in `insights/management/commands/start_temporal_worker.py`.
