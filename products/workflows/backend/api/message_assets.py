...
# names in any outer WHERE — Datastore resolves the bare name to the aggregate
...
class MessageAssetsRequestSerializer(serializers.Serializer):
    parent_run_id = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Only return assets for this batch run (InsightsFlowBatchJob id). Pass an empty string to return only "
    ...
    after = serializers.CharField(
        ...
        "Defaults to -30d (the retention window) — bounds the Datastore partition scan.",
    )
