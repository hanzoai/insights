...
//!
//! It claims day-chunks from PostgreSQL (`cohort_backfill_*`), scans each chunk's Datastore event
//! history, evaluates every event with `cohort-core`'s exact production Stage-1/ScriptVM code — so the
...
//! `domain` (pure, depends only on `cohort-core`) ← {`store` (PostgreSQL), `datastore`, `kafka`}
