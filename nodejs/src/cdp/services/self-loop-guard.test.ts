...
describe('self-loop-guard', () => {
    describe('isInsightsIngestUrl', () => {
        it.each([
            ['https://us.i.hanzo.ai/capture/', true],
            ['https://us.i.hanzo.ai/capture', true],
            ['https://eu.i.hanzo.ai/batch/', true],
            ['https://us.i.hanzo.ai/e/', true],
            ['https://us.i.hanzo.ai/track/', true],
            ['https://us.i.hanzo.ai/i/v0/e/', true],
            ['https://us.i.hanzo.ai/capture/?api_key=abc', true],
            ...
            ['https://hanzo.ai.evil.com/capture', false],
            ['https://nothanzo.ai/capture', false],
            ['not a url', false],
        ])('classifies %s as ingest=%s', (url, expected) => {
            expect(isInsightsIngestUrl(url)).toBe(expected)
        })
    })
    ...
    describe('isSelfReferentialIngestFetch', () => {
        ...
        it.each([
            ...
            { case: 'external (non-Insights) fetch', args: { url: EXTERNAL_URL }, expected: false },
            { case: 'observability logs endpoint (not ingestion)', args: { url: LOGS_URL }, expected: false },
            {
                case: 'workflow step posting to a Insights REST API endpoint (with $lib_token in body)',
