...
class TestInsightsFunctionsBackgroundReloading(TestCase, QueryMatchingTest):
    ...
    def test_insights_functions_reload_on_action_saved(self):
        ...
        assert (
            json.dumps(insights_function_1.filters["bytecode"])
        ...
        assert (
            json.dumps(insights_function_2.filters["bytecode"])
    ...
    def test_insights_functions_reload_on_team_saved(self):
        ...
        # Check that the bytecode is correct
        assert json.dumps(insights_function_1.filters["bytecode"]) == f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 29]'
        assert (
            json.dumps(insights_function_2.filters["bytecode"])
            == f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 32, "$pageview", 32, "event", 1, 1, 11]'
        )

        assert json.dumps(insights_function_3.filters["bytecode"]) == f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 29]'
