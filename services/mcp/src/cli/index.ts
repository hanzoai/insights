#!/usr/bin/env node
import { AnalyticsEvent } from '@/lib/insights/analytics'
...
function usage(): string {
    return `Insights agent CLI

Usage:
  insights-cli api --agent-help
  insights-cli api tools
  insights-cli api search <regex>
  insights-cli api info [--json] <tool>
  insights-cli api schema <tool> [field.path]
  insights-cli api call [--json] [--dry-run] [--confirm] <tool> '<json>'
  insights-cli api skill list [--json]
  insights-cli api skill install [--force] <skill-id>
  insights-cli api agents-md install [--path AGENTS.md]

Destructive tools require --confirm when executed. Use --dry-run before mutations.
Agents: run \`insights-cli api --agent-help\` and load the output into context before anything else.`
}
...
function buildStaticExec(): BuiltStaticExec {
    ...
    const execTool = createExecTool(
        tools,
        undefined,
        'Execute a Insights CLI command',
        COMMAND_REFERENCE,
        'insights-cli',
        undefined,
        [],
...
async function runDryCall(args: string[]): Promise<void> {
    ...
    if (!toolName) {
        throw new Error('Usage: insights-cli api call --dry-run [--json] [--confirm] <tool> <json>')
    }

    const { tools } = buildStaticExec()
    const tool = tools.find((candidate) => candidate.name === toolName)
    if (!tool) {
        throw new Error(`Unknown tool: "${toolName}". Run "insights-cli api search <term>".`)
    }
...
async function runSkillCommand(args: string[]): Promise<void> {
    ...
    switch (subcommand) {
        ...
        case 'install': {
            ...
            if (!skillId) {
                throw new Error('Usage: insights-cli api skill install [--force] <skill-id>')
            }
            printResult(await installSkill(skillId, { force }))
            return
        }
        default:
            throw new Error('Usage: insights-cli api skill list|install')
    }
}
...
async function runAgentsMdCommand(args: string[]): Promise<void> {
    ...
    if (subcommand !== 'install') {
        throw new Error('Usage: insights-cli api agents-md install [--path AGENTS.md]')
    }
...
async function main(): Promise<void> {
    ...
    switch (command) {
        ...
        case 'info': {
            ...
            const toolName = args.shift()
            if (!toolName) {
                throw new Error('Usage: insights-cli api info [--json] <tool>')
            }
        ...
        case 'schema': {
            const toolName = args.shift()
            if (!toolName) {
                throw new Error('Usage: insights-cli api schema <tool> [field.path]')
            }
        ...
        case 'call': {
            ...
            const jsonBody = args.length > 0 ? args.join(' ') : '{}'
            if (!toolName) {
                throw new Error('Usage: insights-cli api call [--json] [--dry-run] [--confirm] <tool> <json>')
            }
