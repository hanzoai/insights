...
from insights.insightsql.database.schema.information_schema import (
    _bound_table_names,
...
class TestInformationSchema(DatastoreTestMixin, APIBaseTest):
    def _context(self, db: Database) -> InsightsQLContext:
        return InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, database=db)
    ...
    def test_select_compiles_to_an_external_table_reference(self):
        # Rows are sent out-of-band as a Datastore external data table, so the printed SQL references
    ...
    def test_tables_lists_builtin_and_system_tables(self):
        ...
        assert rows.get("events") == "insights"
        assert rows.get("persons") == "insights"
        assert rows.get("sessions") == "insights"
    ...
    def test_insights_namespaced_data_plane_tables_are_discoverable(self):
        # The `insights.*` namespace holds data-plane tables (ai_events, trace_spans, metrics, …) that
        # exist ONLY there — a bare `FROM ai_events` errors, and the execute-sql prompt points agents at
        # their `insights.`-qualified names. If the catalog drops them, they become undiscoverable through
        # the schema-discovery workflow and a healthy project looks like it has no such schema. Copies of
        # root tables (insights.events) must stay hidden so the catalog isn't cluttered with duplicates.
        names = {
            row[0]
            for row in execute_insightsql_query(
                "SELECT table_name FROM system.information_schema.tables", team=self.team
            ).results
            or []
        }
        assert {"insights.ai_events", "insights.trace_spans", "insights.metrics"}.issubset(names)
        assert "events" in names
        assert "insights.events" not in names

    def test_insights_namespaced_table_columns_resolve(self):
        # Being listed isn't enough — an agent that discovers `insights.trace_spans` must then be able to
        # inspect its columns. Guards the split between enumerating a dotted name and resolving it.
        columns = {
            row[0]
            for row in execute_insightsql_query(
                "SELECT column_name FROM system.information_schema.columns WHERE table_name = 'insights.trace_spans'",
                team=self.team,
            ).results
            or []
        }
    ...
    def test_expression_columns_resolve_to_their_value_type(self, column_name: str, expected_type: str):
        # Expression columns must report the type they evaluate to (like insightsql autocomplete), not the
    ...
    def test_data_types_is_static_reference(self):
        response = execute_insightsql_query("SELECT type_name FROM system.information_schema.data_types", team=self.team)
    ...
    def _create_warehouse_table(self, name: str = "stripe_charges", column: str = "id") -> DataWarehouseTable:
        ...
        return DataWarehouseTable.objects.create(
            ...
            url_pattern="https://bucket.s3/data/*",
            row_count=42,
            columns={column: {"insightsql": "StringDatabaseField", "datastore": "Nullable(String)", "schema_valid": True}},
        )
    ...
    def _create_saved_query_view(
        ...
        return DataWarehouseSavedQuery.objects.create(
            ...
            columns={
                "order_id": {"insightsql": "StringDatabaseField", "datastore": "String", "schema_valid": True},
                "amount": {"insightsql": "IntegerDatabaseField", "datastore": "Int64", "schema_valid": True},
            },
            table=table,
    ...
    def test_warehouse_column_statistics_are_merged(self):
        ...
        table = DataWarehouseTable.objects.create(
            ...
            columns={
                ...
                "amount": {"insightsql": "StringDatabaseField", "datastore": "Nullable(String)", "schema_valid": True},
            },
        )
        with team_scope(self.team.id, canonical=True):
    ...
    def test_saved_query_view_appears_in_tables(self):
        # A saved-query view must surface in the catalog as a discoverable table so Insights AI can find
        # it. Regression guard for the whole semantic-layer effort: if views stop being enumerated (or are
        # misclassified as insights/data_warehouse), descriptions wired onto them later never reach the AI.
    ...
    def test_saved_query_view_columns_appear_with_types(self):
        # The view's columns (built from the model's `columns` JSONField via `insightsql_definition`) must be
        # enumerated with their InsightsQL types. Guards both the field enumeration and the JSONField→InsightsQL
    ...
    def test_saved_query_view_descriptions_are_merged_from_annotations(self):
        ...
        # surface in the catalog so Insights AI can read them. Guards the metadata loader plus the
    ...
    def test_columns_filter_and_join_against_tables(self):
        ...
        response = execute_insightsql_query(
            ...
            WHERE t.table_type = 'insights' AND c.table_name = 'persons'
