...
async function queryStoredGenerationSentiments(
    ...
    const response = await api.queryInsightsQL<SentimentEvaluationQueryRow[]>(
        insightsql`
            SELECT
                trace_id,
                generation_id,
            ...
            FROM (
                SELECT
                    ${insightsql.raw(source.traceIdExpression)} AS trace_id,
                    ${insightsql.raw(EVALUATION_TARGET_ID_SELECT)} AS generation_id,
                    timestamp,
                ...
                WHERE event = '$ai_evaluation'
                  ...
                  AND ${insightsql.raw(source.traceIdExpression)} IN ${traceIds}
            )
...
async function queryGenerationInputs(
    ...
    const response = await api.queryInsightsQL<GenerationInputQueryRow[]>(
        insightsql`
            SELECT
                uuid,
                trace_id,
