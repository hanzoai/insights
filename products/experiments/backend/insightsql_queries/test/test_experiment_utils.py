...
from products.experiments.backend.insightsql_queries.utils import (
...
class TestGetVariantResult:
    ...
    def test_breakdown_with_insights_null_label(self):
...
class TestGetExperimentQueryDebug:
    ...
    def test_generates_insightsql_and_datastore_sql(self, team):
        """Test that function generates both InsightsQL and Datastore SQL."""
        ...
        insightsql, datastore_sql = get_experiment_query_debug(select_query, team)

        # InsightsQL should be generated
        assert insightsql is not None
        assert len(insightsql) > 0

        # Datastore SQL should be generated
        assert datastore_sql is not None
        assert len(datastore_sql) > 0

        # Basic sanity check - should contain SQL keywords
        assert "SELECT" in datastore_sql
    ...
    def test_masks_sensitive_parameters(self, team):
        ...
        # Mock the executor's generate_datastore_sql to return params with _sensitive suffix
        with patch("products.experiments.backend.insightsql_queries.utils.InsightsQLQueryExecutor") as mock_executor_class:
            mock_executor = MagicMock()
            mock_executor_class.return_value = mock_executor
            mock_executor.insightsql = "SELECT 1"

            # Simulate query with sensitive parameters
            mock_context = InsightsQLContext()
            mock_context.values = {
                "insightsql_val_0": "s3://bucket/path",
                "insightsql_val_1_sensitive": "SECRETKEY123",
                "insightsql_val_2_sensitive": "SECRETTOKEN456",
            }
            mock_executor.generate_datastore_sql.return_value = (
                "SELECT * FROM s3(%(insightsql_val_0)s, %(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s)",
                mock_context,
            )
            ...
            assert "s3://bucket/path" in datastore_sql
            # Verify secrets are hidden
            assert "SECRETKEY123" not in datastore_sql
            assert "SECRETTOKEN456" not in datastore_sql
            assert datastore_sql.count("[HIDDEN]") == 2
...
class TestMetricVariantToStatistic:
    ...
    def test_threshold_proportion_coerces_sum_to_int(self) -> None:
        """Datastore may return total_sum as a float; ProportionStatistic.sum must be an int count."""
        metric = ExperimentMeanMetric(
            source=EventsNode(event="purchase", math=ExperimentMetricMathType.SUM, math_property="amount"),
            threshold=100,
        )
