...
from insights.schema import ExperimentDataWarehouseNode, ExperimentFunnelMetric
...
class FunnelDWValidator:
    ...
    def validate_required_fields(node: ExperimentDataWarehouseNode, step_index: int) -> list[str]:
        ...
        if not node.events_join_key:
            errors.append(
                f"Step {step_index}: events_join_key is required to specify the field in Insights events "
