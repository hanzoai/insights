...
from products.warehouse_sources.backend.temporal.data_imports.sources.datastore_cloud import datastore_cloud
...
class _FakeResumableManager:
    def __init__(self, state: DatastoreCloudResumeConfig | None = None) -> None:
        self._state = state
        self.saved: list[DatastoreCloudResumeConfig] = []

    def can_resume(self) -> bool:
        return self._state is not None

    def load_state(self) -> DatastoreCloudResumeConfig | None:
        return self._state

    def save_state(self, data: DatastoreCloudResumeConfig) -> None:
...
def _collect(
    ...
    **kw: Any,
    ...
    monkeypatch.setattr(datastore_cloud, "_fetch", fake_fetch)
    monkeypatch.setattr(datastore_cloud, "make_tracked_session", lambda **_: MagicMock())
...
class TestUsageCostWindowing:
    ...
    def test_resume_state_used_and_saved_only_after_yield(self, monkeypatch: Any) -> None:
        manager = _FakeResumableManager(DatastoreCloudResumeConfig(organization_id="org-1", from_date="2026-06-20"))
...
class TestEntityRows:
    ...
    def test_org_id_stamped_onto_child_rows(self, endpoint: str, path_suffix: str) -> None:
        ...
        with patch.object(datastore_cloud, "make_tracked_session", return_value=MagicMock()):
            with patch.object(datastore_cloud, "_fetch", side_effect=lambda _session, url, _logger: handler(url)):
                rows: list[dict] = []
...
class TestValidateCredentials:
    ...
    def test_status_mapping(self, _name: str, status: int, expected: bool) -> None:
        ...
        with patch.object(datastore_cloud, "make_tracked_session", return_value=session):
...
class TestFetch:
    ...
    def test_retryable_statuses_exhaust_retry_budget(self, _name: str, status: int) -> None:
        ...
        session = MagicMock()
        session.get.return_value = response
        with patch.object(datastore_cloud._fetch.retry, "sleep", lambda *_: None):  # type: ignore[attr-defined]
            with pytest.raises(datastore_cloud.DatastoreCloudRetryableError):
                datastore_cloud._fetch(session, "https://api.datastore.cloud/v1/organizations", MagicMock())
    ...
    def test_429_honors_retry_after_header(self) -> None:
        ...
        session.get.return_value = response
        with patch.object(datastore_cloud._fetch.retry, "sleep", lambda *_: None):  # type: ignore[attr-defined]
            with pytest.raises(datastore_cloud.DatastoreCloudRetryableError) as exc_info:
                datastore_cloud._fetch(session, "https://api.datastore.cloud/v1/organizations", MagicMock())
...
class TestNonRetryableErrors:
    @parameterized.expand(
        [
            ("unauthorized", "401 Client Error: Unauthorized for url: https://api.datastore.cloud/v1/organizations"),
            (
                "forbidden",
                "403 Client Error: Forbidden for url: https://api.datastore.cloud/v1/organizations/org-1/usageCost?from_date=2026-07-01&to_date=2026-07-15",
            ),
        ]
    )
    def test_credential_errors_are_non_retryable(self, _name: str, observed_error: str) -> None:
        non_retryable = DatastoreCloudSource().get_non_retryable_errors()
        assert any(key in observed_error for key in non_retryable)

    @parameterized.expand(
        [
            ("read_timeout", "HTTPSConnectionPool(host='api.datastore.cloud', port=443): Read timed out."),
            (
                "server_error",
                "500 Server Error: Internal Server Error for url: https://api.datastore.cloud/v1/organizations",
            ),
        ]
    )
