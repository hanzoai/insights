...
from insights.utils import relative_date_parse, str_to_bool
...
class BatchExportRequestSerializer(serializers.Serializer):
    ...
    insightsql_query = serializers.CharField(
        required=False,
        help_text="Optional InsightsQL SELECT defining a custom model schema. Only recommended in advanced use cases.",
    )
    filters = serializers.JSONField(
        ...
        help_text=(
            ...
            f"serialized InsightsQL property filter object with a 'type' of one of: {_SUPPORTED_FILTER_TYPES_DISPLAY} "
...
class BatchExportSerializer(serializers.ModelSerializer):
    ...
    insightsql_query = InsightsQLSelectQueryField(
        required=False,
        help_text="Optional InsightsQL SELECT defining a custom model schema. Only recommended in advanced use cases.",
    )
    ...
    def validate_destination(self, destination_attrs: dict):
        ...
        if destination_type == BatchExportDestination.Destination.HTTP:
            ...
            if url and url not in ("https://us.i.hanzo.ai/batch/", "https://eu.i.hanzo.ai/batch/"):
...
class TooManyConcurrentBackfills(APIException):
    ...
    default_detail = "Too many concurrent batch export backfills for this project."
...
def create_backfill(
    ...
    # via the get_backfill_info activity. This allows the potentially slow Datastore query to run
