...
- **Exemptions** (prospective, `SignalReport.billing_exempt_reason`): Insights-system-origin
...
from insights.models.organization import BillingPeriod
...
# Insights-system scout skills whose reports must never bill — users must not pay for PRs that fix
...
# health-checks pipeline, `insights/temporal/health_checks/signal_emitter.py`). Per-signal
...
def _scout_skill_is_canonical(team_id: int, skill_name: str) -> bool:
    """Whether the team's row for `skill_name` is still the Insights-shipped scout, byte for byte.

    `skill_name` alone doesn't attest Insights-system origin: a team can edit a seeded scout in
...
def system_billing_exempt_reason(team_id: int, report_id: str | uuid.UUID) -> str | None:
    ...
    Pure Postgres (no Datastore), so it is usable under the auto-start row lock: a report is
    exempt-origin when an exempt scout skill's run authored it via `emit_report` — and the
    team's row for that skill is still the canonical Insights-shipped content (a team-edited
    fork keeps the name but is no longer Insights's system, so its reports stay billable).
    """
...
def get_signals_billing_credits_by_team(
    ...
    # Billing-exempt reports (Insights-system origins) never bill; the reason is frozen before the
