...
// Imported from the source module rather than the `@hanzo/elements` barrel so the spy below
// replaces `.error` on the same `toast` singleton the logic calls at runtime (see the same
...
describe('billingProductLogic', () => {
    ...
    describe('unsubscribe survey state', () => {
        ...
        it('deactivates the product, submits the survey, and clears the modal on confirm', async () => {
            ...
            const captureSpy = jest.spyOn(insights, 'capture').mockReturnValue(undefined as any)
            const logic = mountProduct('product_analytics')
...
describe('billingProductLogic — confirm purchase modal', () => {
    ...
    beforeEach(async () => {
        initKeaTests()
        toastErrorSpy = jest.spyOn(toast, 'error').mockImplementation(() => ({ id: 'x' }) as any)
