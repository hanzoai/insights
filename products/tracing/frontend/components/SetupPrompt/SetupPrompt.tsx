...
const NoSpansPrompt = ({ className }: { className?: string }): JSX.Element | null => {
    ...
    useEffect(() => {
        insights.capture('tracing setup prompt viewed')
    }, [])
    ...
    const onDocsLinkClick = (docsType: string): void => {
        insights.capture('tracing onboarding docs clicked', { docs_type: docsType })
        addProductIntent({
    ...
    return (
        <ProductIntroduction
            ...
            description="Insights tracing works with any OpenTelemetry-compatible client. You don't need any Insights-specific packages – just use standard OpenTelemetry libraries to send spans via OTLP."
            isEmpty={true}
            ...
            actionElementOverride={
                <div className="flex flex-col items-start gap-4">
                    <p className="text-sm text-secondary m-0">
                        Read our{' '}
                        <Link to="https://hanzo.ai/docs/tracing" onClick={() => onDocsLinkClick('Tracing')}>
                            tracing docs
                        </Link>
                        , learn more about{' '}
                        <Link
