...
class TestBackfillErrorTrackingIssueLegacyStatuses(DatastoreTestMixin, BaseTest):
    def _run(
        self,
        *,
        live_run: bool = True,
    ...
    def test_live_run_resolves_legacy_statuses_and_syncs_to_datastore(self):
        archived = self._make_issue(team=self.team, status=ErrorTrackingIssue.Status.ARCHIVED, fingerprint="fp_a")
        pending = self._make_issue(team=self.team, status=ErrorTrackingIssue.Status.PENDING_RELEASE, fingerprint="fp_p")
    ...
    def test_datastore_sync_maps_legacy_statuses_to_resolved(self):
        archived = self._make_issue(team=self.team, status=ErrorTrackingIssue.Status.ARCHIVED, fingerprint="fp_a")

        sync_issues_to_datastore(issue_ids=[archived.id], team_id=self.team.pk)
    ...
    def test_live_run_rolls_back_status_update_when_datastore_sync_fails(self):
        issue = self._make_issue(team=self.team, status=ErrorTrackingIssue.Status.ARCHIVED, fingerprint="fp_a")

        with (
            patch(
                "products.error_tracking.backend.management.commands.backfill_error_tracking_issue_legacy_statuses.sync_issues_to_datastore",
