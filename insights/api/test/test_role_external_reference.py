...
class TestRoleExternalReferenceAPI(APILicensedTest):
    ...
    def _create_reference(self, **overrides: str):
        payload = {
            "provider": "github",
            "provider_organization_id": "insights",
    ...
    def test_lookup(self, query_string: str, expected_key: str, expected_value: str) -> None:
        ...
        response = self.client.get(
            f"{self._base_url()}/lookup/?provider=github&provider_organization_id=insights&{query_string}"
        )
    ...
    def test_lookup_requires_identifier(self) -> None:
        response = self.client.get(f"{self._base_url()}/lookup/?provider=github&provider_organization_id=insights")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    @parameterized.expand(
        [
            ({"provider_role_id": "67890"},),
            ({"provider_role_slug": "Frontend-Team", "provider_organization_id": "Insights"},),
        ]
    )
