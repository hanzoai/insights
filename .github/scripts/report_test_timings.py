...
#
# [tool.uv.sources]
# insights-owners = { path = "../../tools/owners" }
# ///
...
from insights_owners import OwnersResolver

logger = logging.getLogger("report_test_timings")

DEFAULT_OTLP_ENDPOINT = "https://us.i.hanzo.ai/i/v1/traces"
Runner = Literal["pytest", "jest"]

DEFAULT_RUNNER: Runner = "pytest"
SERVICE_NAMES: dict[Runner, str] = {"pytest": "ci-backend", "jest": "ci-frontend"}
INSTRUMENTATION_NAME = "insights-ci-test-timings"
INSTRUMENTATION_VERSION = "0.1.0"
JEST_QUARANTINE_SIGNAL_GLOB = "insights-jest-quarantine-*.jsonl"
...
class Shard:
    ...
    # reported by the `insights-junit-timings` pytest plugin. Captures the shared
...
def classify_testcase(testcase: Any) -> tuple[str, int]:
    ...
    conftest's `insights-junit-timings` plugin surfaces the retry count as a
    `insights.reruns` testcase property. `<rerunFailure>`/`<rerunError>`
    ...
    for child in testcase:
        ...
        if tag == "properties":
            for prop in child.findall("property"):
                if prop.get("name") == "insights.reruns":
                    try:
...
def to_pytest_nodeid(classname: str, name: str) -> str:
    """`insights.insightsql.test.test_resolver.TestResolver`, `test_x` → `insights/insightsql/test/test_resolver/TestResolver::test_x`.
...
def parse_testsuite_properties(suite_elem: Any) -> dict[str, str]:
    ...
    exists (e.g., external shards without the `insights-junit-timings` plugin).
    """
...
def parse_shard(
    ...
    for suite_elem in suite_elements:
        ...
        try:
            suite_setup_seconds = max(0.0, float(properties.get("insights.setup_seconds", "0")))
        except ValueError:
