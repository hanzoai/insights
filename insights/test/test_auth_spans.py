...
from insights.auth import JwtAuthentication, PersonalAPIKeyAuthentication, SessionAuthentication
...
def _install_in_memory_tracer(exporter: InMemorySpanExporter) -> trace.Tracer:
    """Wire up an in-memory span exporter and rebind `insights.auth.tracer` to use it.
    ...
    """
    import insights.auth as auth_module
...
class TestAuthSpans(BaseTest):
    ...
    def test_personal_api_key_span_records_source_and_hash_mode_on_match(self):
        ...
        parent = self._spans_named("insights.auth.personal_api_key")
        assert len(parent) == 1
        assert parent[0].attributes["auth.source"] == "header"

        db_lookup = self._spans_named("insights.auth.personal_api_key.db_lookup")
