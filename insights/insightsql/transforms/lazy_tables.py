...
from insights.insightsql.database.models import LazyJoinToAdd, LazyTableToAdd
...
from insights.insightsql.resolver_utils import get_long_table_name
        ...
        for table_name, table_to_add in tables_to_add.items():
            ...
            while join_ptr:
                if join_ptr.table is not None and (
                    ...
                    # A lazy table expands into an aggregating subquery. Datastore rejects a SAMPLE
        ...
        for to_table, join_scope in joins_to_add.items():
            ...
            while join_ptr:
                if join_scope.from_table == join_ptr.alias or (
                    ...
                    # which Datastore resolves left-to-right and so must appear before this join.
