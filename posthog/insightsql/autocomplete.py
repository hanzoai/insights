...
from posthog.insightsql.base import AST, CTE, ConstantType
from posthog.insightsql.context import InsightsQLContext
from posthog.insightsql.database.database import INSIGHTSQL_CHARACTERS_TO_BE_WRAPPED, Database
...
from posthog.insightsql.functions.mapping import ALL_EXPOSED_FUNCTION_NAMES
from posthog.insightsql.parser import parse_expr, parse_program, parse_select, parse_string_template
from posthog.insightsql.resolver import resolve_types, resolve_types_from_table
...
from posthog.insightsql.visitor import TraversingVisitor, clone_expr
...
def convert_field_or_table_to_type_string(
    field_or_table: FieldOrTable, parent_table: str, context: InsightsQLContext
) -> str | None:
    ...
    if isinstance(field_or_table, ast.ExpressionField):
        ...
        try:
            field_expr = resolve_types_from_table(field_or_table.expr, parent_table_chain, context, "insightsql")
...
def get_table(context: InsightsQLContext, join_expr: ast.JoinExpr, ctes: Optional[dict[str, CTE]]) -> None | Table:
    ...
    def resolve_fields_on_table(table: Table | None, table_query: ast.SelectQuery) -> Table | None:
        ...
        try:
            node = cast(ast.SelectQuery, resolve_types(node=table_query, dialect="insightsql", context=context))
            ...
            table_name = table.to_printed_insightsql()
...
def get_tables_aliases(query: ast.SelectQuery, context: InsightsQLContext) -> dict[str, ast.Table]:
...
def resolve_table_field_traversers(table: Table, context: InsightsQLContext) -> Table:
...
def append_table_field_to_response(
    table: Table, suggestions: list[AutocompleteCompletionItem], language: str, context: InsightsQLContext
) -> None:
    keys: list[str] = []
    ...
    for field_name, field_or_table in table_fields:
        ...
        details.append(convert_field_or_table_to_type_string(field_or_table, table.to_printed_insightsql(), context))
                        ...
                        if is_last_part:
                            ...
                            if isinstance(field, StringJSONDatabaseField):
                                ...
                                extend_responses(
                                    ...
                                    details=[
                                        convert_field_or_table_to_type_string(
                                            inner_field, field.to_printed_insightsql(), context
                                        )
                                ...
                                extend_responses(
                                    ...
                                    details=[
                                        convert_field_or_table_to_type_string(
                                            inner_field, field_table.to_printed_insightsql(), context
                                        )
                                        for key, inner_field in fields
                                    ],
                                )
...
def add_globals_to_suggestions(globalVars: dict, response: InsightsQLAutocompleteResponse):
