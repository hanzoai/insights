...
TeamId = str
InsightsFunctionId = str
Body = bytes


class RequestData(typing.NamedTuple):
    team_id: TeamId
    insights_function_id: InsightsFunctionId
    body: Body


async def assert_datastore_records_were_handled(
    ...
    handled_records = [json.loads(request.body)["datastore_event"] for request in handler.data]
    insights_function_ids = {request.insights_function_id for request in handler.data}

    assert insights_function_id in insights_function_ids and len(insights_function_ids) == 1, (
        f"Expected only '{insights_function_id}' but got '{insights_function_ids}'"
    )
