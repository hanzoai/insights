...
from insights.event_usage import groups as build_groups
from insights.models.group.util import get_groups_by_identifiers
...
# $groupidentify events: insights.group(type, key) called without properties associates the group
...
# _resolve_groups_from_analytics); column names can't be InsightsQL placeholders.
...
def _resolve_groups_from_analytics(team: Team, distinct_ids: list[str]) -> dict | None:
    """Resolve ``$groups`` from the customer's recent analytics events in Datastore.
    ...
    # projects without an organization group type before touching Datastore.
    group_type_index = {
    ...
    # Deferred: insightsql.query pulls the whole query-runner layer, and this module loads
    # at django.setup() via the conversations signal wiring.
    from insights.insightsql import ast  # noqa: PLC0415
...
def _org_groups_for_person(ticket: Ticket, team: Team, person: Person, distinct_ids: list[str]) -> dict | None:
    ...
    Each lookup hits a different subsystem (Postgres, Datastore, personinsights), so failures
...
def _resolve_person_org_groups(ticket: Ticket, team: Team) -> tuple[bool, dict | None]:
    ...
    customer can't be tied to a Insights organization.

    The web widget sets ``ticket.distinct_id`` to a real Insights distinct_id,
    ...
    back to looking the person up by ``properties.email`` in Datastore and
    ...
    if ticket.distinct_id:
        ...
        try:
            ...
            with personinsights_caller_tag("conversations/ticket-event-person"):
            ...
            # path instead of losing attribution to a transient personinsights error.
    ...
    if email:
        # person_lookup pulls the InsightsQL query layer; this module loads at django.setup()
