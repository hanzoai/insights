...
teams whose only traffic is insights-js from non-public origins (Electron /
...
# Datastore scan volume and the `insights_team` write churn bounded as the
...
def _teams_meeting_criterion(team_ids: Iterable[int]) -> dict[int, ProductionTrafficSignal]:
    ...
    Single source of truth for "what counts as production traffic." Datastore
    ...
    Evidence is gathered in two passes so the scan stays under Datastore's
    ...
    # attributed to growth in Datastore query analytics (and so it doesn't trip
...
def _earliest_production_host_timestamps(
    ...
    for team_id, host in team_hosts.items():
        ...
        if converted_at is not None:
            # Datastore DateTime is UTC; coerce naive driver values to aware so
            # the Insights client doesn't guess a timezone at capture time.
...
def _mark_teams_ingested_production_event(team_signals: Mapping[int, ProductionTrafficSignal], now: datetime) -> int:
    ...
    # Emit outside the transaction so the Insights client round-trip doesn't
