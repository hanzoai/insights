...
const ALLOWED_TABLES: &[&str] = &["personinsights_person_tmp", "insights_person"];
...
fn push_person<'a>(arrays: &mut PreparedArrays<'a>, p: &'a Person) -> Result<(), WriteError> {
    let unbindable = |field: &str, detail: String| {
        counter!("personinsights_writer_unbindable_field_total", "field" => field.to_string())
            .increment(1);
        WriteError {
            message: format!(
...
async fn run_upsert(
        ...
        .await
    {
        Ok(result) => {
            ...
            counter!("personinsights_writer_rows_upserted_total", "mode" => mode).increment(affected);
            let skipped = chunk_size.saturating_sub(affected);
            if skipped > 0 {
                counter!("personinsights_writer_rows_version_skipped_total", "mode" => mode)
                    .increment(skipped);
            }
            Ok(())
        }
        Err(e) => {
            ...
            counter!("personinsights_writer_upsert_errors_total", "mode" => mode).increment(1);
...
mod tests {
    ...
    async fn table_name_allowlist_accepts_valid() {
        ...
        let _store = PgStore::new(pool.clone(), "personinsights_person_tmp".to_string());
        let _store = PgStore::new(pool, "insights_person".to_string());
    }
    ...
    async fn table_name_allowlist_rejects_invalid() {
        let pool = PgPool::connect_lazy("postgres://localhost/test").unwrap();
        let _store = PgStore::new(pool, "evil_table; DROP TABLE insights_person".to_string());
    }
