...
from insights.temporal.ai_observability.eval_reports.activities import (
...
from insights.temporal.ai_observability.eval_reports.report_agent.schema import EvalReportContent, EvalReportMetrics
from insights.temporal.ai_observability.eval_reports.targets import target_event_predicate
from insights.temporal.ai_observability.eval_reports.types import (
...
async def test_run_agent_activity_loads_target_and_forwards_output_type(
    ...
    with (
        patch(
            "insights.temporal.ai_observability.eval_reports.activities.Heartbeater",
            return_value=noop_heartbeater(),
        ),
        patch(
            "insights.temporal.ai_observability.eval_reports.report_agent.run_eval_report_agent",
            return_value=content,
        ) as run_agent,
        patch(
            "insights.temporal.ai_observability.eval_reports.activities._load_evaluation_target",
...
async def test_store_sentiment_report_emits_generic_metrics_only() -> None:
    ...
    with (
        ...
        patch("insights.models.team.Team.objects.get", return_value=MagicMock()),
        patch("insights.models.event.util.create_event") as create_event,
    ):
...
def test_count_trigger_uses_current_output_type() -> None:
    ...
    with (
        patch("insights.insightsql.parser.parse_select", return_value=MagicMock()) as parse_select,
        patch("insights.insightsql.query.execute_insightsql_query", return_value=Mock(results=[[4]])),
    ):
...
def test_manual_count_window_uses_current_output_type() -> None:
    ...
    with (
        patch("insights.insightsql.parser.parse_select", return_value=MagicMock()) as parse_select,
        patch("insights.insightsql.query.execute_insightsql_query", return_value=Mock(results=[[expected]])),
        patch("insights.models.Team.objects.get", return_value=MagicMock()),
    ):
...
class TestCountTriggeredReportChecks(BaseTest):
    ...
    def test_fetch_candidates_returns_only_deliverable_count_triggered_reports(self):
        ...
        with patch("insights.insightsql.query.execute_insightsql_query") as execute_insightsql_query:
            groups = _fetch_count_triggered_eval_report_candidate_groups()

        self.assertEqual(groups, [[str(count_triggered_report.id)]])
        execute_insightsql_query.assert_not_called()
    ...
    def test_fetch_candidates_groups_by_team_and_chunks_by_width(self):
        ...
        with patch("insights.temporal.ai_observability.eval_reports.activities.COUNT_TRIGGER_QUERY_WIDTH", 2):
            groups = _fetch_count_triggered_eval_report_candidate_groups()
    ...
    def test_check_report_returns_due_when_threshold_is_crossed(self):
        ...
        with patch("insights.insightsql.query.execute_insightsql_query") as execute_insightsql_query:
            execute_insightsql_query.return_value = Mock(results=[[100]])
            result = _check_count_triggered_eval_report_sync(str(report.id), timezone.now())

        self.assertTrue(result.due)
        self.assertIsNone(result.skipped_reason)
        execute_insightsql_query.assert_called_once()

    def test_check_report_skips_cooldown_without_datastore_query(self):
        now = timezone.now()
        report = self._create_report(
    ...
    def test_check_report_skips_daily_cap_without_datastore_query(self):
        now = timezone.now()
    ...
    def test_batch_skips_gated_reports_without_datastore_and_preserves_order(self):
        # Every Postgres-gated report must be resolved without touching Datastore — that's
...
class TestBatchedCountTriggeredQuery(DatastoreTestMixin, BaseTest):
    """Exercises the batched count check against real Datastore events — no query mocking —
    ...
    def test_width_chunking_returns_all_counts(self):
        ...
        with patch("insights.temporal.ai_observability.eval_reports.activities.COUNT_TRIGGER_QUERY_WIDTH", 1):
            results = _check_count_triggered_eval_reports_batch(report_ids, self.NOW)
    ...
    def test_since_boundary_respects_non_utc_team_timezone(self):
        # `since` is passed as an ast.Constant datetime so Datastore compares the same absolute
