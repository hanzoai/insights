...
@pytest.mark.ee
class TestInsightsFlowAccessControl(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
    ...
    def test_create_blocked_without_resource_editor_access(self):
        ...
        self.assertFalse(InsightsFlow.objects.filter(team=self.team, name="new_workflow").exists())
    ...
    def test_create_allowed_with_resource_editor_access(self):
        ...
        self.assertTrue(InsightsFlow.objects.filter(team=self.team, name="editor_create").exists())
    ...
    def test_bulk_delete_only_deletes_editable_workflows(self):
        ...
        archived_ok = self._create_workflow(name="archived_ok", state=InsightsFlow.State.ARCHIVED)
        archived_denied = self._create_workflow(name="archived_denied", state=InsightsFlow.State.ARCHIVED)
        archived_viewer = self._create_workflow(name="archived_viewer", state=InsightsFlow.State.ARCHIVED)
        self._create_access_control(self.editor_user, access_level="editor")
        ...
        self.assertFalse(InsightsFlow.objects.filter(id=archived_ok.id).exists())
        self.assertTrue(InsightsFlow.objects.filter(id=archived_denied.id).exists())
        self.assertTrue(InsightsFlow.objects.filter(id=archived_viewer.id).exists())
