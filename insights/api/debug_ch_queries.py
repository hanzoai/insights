...
from insights.auth import PersonalAPIKeyAuthentication, SessionAuthentication
...
from insights.datastore.preaggregation.experiment_metric_events_sql import (
...
from insights.settings.data_stores import DATASTORE_AUX_CLUSTER, DATASTORE_CLUSTER, DATASTORE_DATABASE
...
def _cache_table_stats() -> list[dict]:
    ...
    tables = {
        SHARDED_EXPERIMENT_EXPOSURES_TABLE(): (DISTRIBUTED_EXPERIMENT_EXPOSURES_TABLE(), DATASTORE_CLUSTER),
        SHARDED_EXPERIMENT_METRIC_EVENTS_TABLE(): (
            DISTRIBUTED_EXPERIMENT_METRIC_EVENTS_TABLE(),
            DATASTORE_AUX_CLUSTER,
        ),
    }
...
def _bucket_axis(hours: int) -> tuple[str, str, list[str], dict[str, int]]:
    ...
    Returns the Datastore bucketing function, the interval name, the bucket keys (ISO strings in
    explicit UTC, matching what the SQL's formatDateTime emits — so no assumption about the
    Datastore server timezone or driver naive/aware behavior is needed), and a key→index lookup.
    """
...
class DebugCHQueries(viewsets.ViewSet):
    ...
    def hourly_stats(self, filter_key: str, filter_value: str, team_id: int):
        ...
        # nosemgrep: datastore-fstring-param-audit - filter_clause from internal _log_comment_filter
    ...
    def queries(
        ...
        if filter_key and filter_value:
            # nosemgrep: datastore-fstring-param-audit - where_clause from internal _log_comment_filter
    ...
    def _fetch_org_arr(self, org_ids: set[str]) -> dict[str, int]:
        ...
        Uses InsightsQL to access data warehouse tables via the Insights internal team.
        ...
        try:
            # nosemgrep: insightsql-fstring-param-audit - org_ids are UUIDs from our own DB
    ...
    def slowest_queries(self, request):
        ...
        # Each row is one Datastore query. A top-level read (surface != 'precompute_build') and the
        ...
        # nosemgrep: datastore-fstring-param-audit - extra_filters is built from hardcoded SQL fragments; user values flow through params
        sql_query = f"""
            WITH per_query AS (
                SELECT
                    query_id,
    ...
    def precompute_overview(self, request):
        ...
        # typed JSON column there, so tags are dot-accessed; ifNull(toString(...), '') preserves the
        # ''-when-missing semantics JSONExtractString gave us on the raw column.
        # nosemgrep: datastore-fstring-param-audit - skip_reason_counts is built from a hardcoded tuple
        reads_sql = f"""
            SELECT
                coalesce(
    ...
    def precompute_timeseries(self, request):
        ...
        # nosemgrep: datastore-fstring-param-audit - bucket_fn is one of two hardcoded function names
        reads_sql = f"""
            SELECT
    ...
    def cache_growth(self, request):
        ...
        written_bytes is Datastore's uncompressed INSERT accounting, so it will read higher than
