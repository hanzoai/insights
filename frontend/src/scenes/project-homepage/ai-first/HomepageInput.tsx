...
import { IconArrowRight, IconClock, IconInfo, IconLock, IconMicrophone, IconPin, IconStar } from '@hanzo/icons'
...
function IdleInput(): JSX.Element {
    ...
    const submitAi = (): void => {
        ...
        insights.capture('homepage query submitted', { mode: 'ai' })
        submitQuery('ai')
    }
                ...
                <div className="flex w-full py-1 px-1 max-h-[300px] items-end gap-1">
                    ...
                    <TextareaPrimitive
                        ...
                        onChange={(e) => {
                            ...
                            if (value === '/' || value === '@') {
                                insights.capture('homepage ai mode entered', { trigger: value })
                                enterAiMode(value)
                                return
                            }
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Tab' && query.trim()) {
                                e.preventDefault()
                                insights.capture('homepage query submitted', { mode: 'search' })
                                submitQuery('search')
                            }
        ...
        return (
            <div className="border border-primary rounded-lg bg-surface-primary p-4 flex flex-col gap-2">
                <p className="font-medium text-pretty m-0">
                    Insights AI needs your approval to potentially process identifying user data with external AI
                    providers.
                </p>
                ...
                {isAdmin ? (
                    ...
                    <Button type="secondary" size="small" disabled sideIcon={<IconLock />}>
...
function IdleGrid(): JSX.Element {
    ...
    const handleItemClick = useCallback((item: HomepageGridItem) => {
        if (item.href) {
            insights.capture('homepage grid item clicked', { kind: item.kind, href: item.href })
