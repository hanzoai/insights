...
false-positive traps (weekly seasonality and low-count noise). Two scorers: Insights's own
...
instead of hand-rolling stats. It runs Insights's production anomaly detectors server-side over
...
behind Insights's shipped anomaly alerts, so lean on it rather than reinvent z-scores in SQL.
...
proven across Insights's own working alert inventories is an `or`-ensemble of `zscore`
...
- **`alert-simulate` only accepts `TrendsQuery` insights.** A SQL-backed saved insight
  (`DataVisualizationNode` wrapping a `InsightsQLQuery` — most revenue, MRR/ARR, and LLM-cost
...
Insights SQL is InsightsQL (Datastore dialect). Its date functions (`toStartOfDay`,
