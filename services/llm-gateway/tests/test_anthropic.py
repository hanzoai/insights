...
from llm_gateway.request_context import (
    extract_insights_flags_from_headers,
    extract_insights_properties_from_headers,
    extract_insights_provider_from_headers,
    extract_insights_use_bedrock_fallback_from_headers,
)
...
class TestExtractInsightsFlagsFromHeaders:
    def test_extracts_x_insights_flag_headers(self) -> None:
        request = MagicMock()
        request.headers = MagicMock()
        request.headers.items.return_value = [
            ("X-INSIGHTS-FLAG-EXPERIMENT-FEATURE-FLAG-KEY", "variant-name"),
            ("X-INSIGHTS-FLAG-ANOTHER-FLAG", "control"),
            ("Content-Type", "application/json"),
        ]
        result = extract_insights_flags_from_headers(request)
        assert result == {
...
class TestExtractInsightsPropertiesFromHeaders:
    def test_extracts_x_insights_property_headers(self) -> None:
        request = MagicMock()
        request.headers = MagicMock()
        request.headers.items.return_value = [
            ("X-INSIGHTS-PROPERTY-VARIANT", "memes"),
            ("X-INSIGHTS-PROPERTY-FOO", "bar"),
            ("Content-Type", "application/json"),
        ]
        result = extract_insights_properties_from_headers(request)
...
class TestExtractInsightsProviderFromHeaders:
    def test_extracts_provider_header(self) -> None:
        request = MagicMock()
        request.headers = Headers({"X-Insights-Provider": "bedrock"})

        assert extract_insights_provider_from_headers(request) == "bedrock"

    def test_extracts_provider_header_case_insensitive(self) -> None:
        request = MagicMock()
        request.headers = Headers({"x-insights-provider": "ANTHROPIC"})

        assert extract_insights_provider_from_headers(request) == "anthropic"

    def test_invalid_provider_header_raises(self) -> None:
        request = MagicMock()
        request.headers = Headers({"X-Insights-Provider": "vertex"})

        with pytest.raises(ValueError, match="Expected one of: anthropic, bedrock"):
            extract_insights_provider_from_headers(request)


class TestExtractInsightsUseBedrockFallbackFromHeaders:
    def test_extracts_true_fallback_header(self) -> None:
        request = MagicMock()
        request.headers = Headers({"X-Insights-Use-Bedrock-Fallback": "true"})

        assert extract_insights_use_bedrock_fallback_from_headers(request) is True

    def test_extracts_false_fallback_header(self) -> None:
        request = MagicMock()
        request.headers = Headers({"X-Insights-Use-Bedrock-Fallback": "FALSE"})

        assert extract_insights_use_bedrock_fallback_from_headers(request) is False

    def test_invalid_fallback_header_raises(self) -> None:
        request = MagicMock()
        request.headers = Headers({"X-Insights-Use-Bedrock-Fallback": "1"})

        with pytest.raises(ValueError, match="Expected: true or false"):
            extract_insights_use_bedrock_fallback_from_headers(request)
