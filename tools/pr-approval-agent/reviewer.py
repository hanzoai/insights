...
from version import STAMPFN_VERSION

# Traced wrapper, bound only with a Insights key and no gateway route (gateway
...
_REVIEWER_SCAFFOLD_TAIL = "\n" + textwrap.dedent(
    ...
    (e.g. "request review from @Insights/team-x"); when the prompt lists who is
    ...
    - "Request review from @Insights/team-x (owns the changed files)."
...
class Reviewer:
    ...
    async def _review(
        ...
        if active_query is None and _POSTFN_AI_AVAILABLE:
            ...
            props = insights_kwargs["insights_properties"]

        # Neither gateway nor a Insights key: plain, untraced SDK query.
        if active_query is None:
            active_query = query

        structured_output = None
        async for message in active_query(prompt=prompt, options=options, **insights_kwargs):
            if self.verbose:
    ...
    def _format_self_driving(self, cl: dict) -> str:
        ...
        return (
            ...
            "member's Inbox report. What the platform attested: the PR is authored by the Insights "
