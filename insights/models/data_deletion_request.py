...
def jsonhas_expr(prop: str, param_prefix: str, column: str = "properties") -> str:
    """Build a Datastore ``JSONHas`` expression for a (possibly nested) property path.
...
def compile_insightsql_predicate(obj, use_new_events_schema: bool = False) -> tuple[str, dict]:
    """Parse and compile ``obj.insightsql_predicate`` into a Datastore SQL fragment.
    ...
    lightweight DELETE: Datastore rewrites it into a mutation whose expression
    ...
    """
    predicate = (getattr(obj, "insightsql_predicate", "") or "").strip()
    if not predicate:
        return "", {}

    # Imported lazily: InsightsQL pulls in the full schema graph, which we don't want
    ...
    from insights.insightsql.insightsql import translate_insightsql
    ...
    try:
        ...
        raise ValidationError({"insightsql_predicate": f"Could not parse InsightsQL: {exc}"}) from exc

    if obj.team_id is None:
        raise ValidationError({"insightsql_predicate": "team_id must be set before validating the predicate."})

    # Subqueries are allowed (e.g. ``person_id IN (SELECT id FROM persons WHERE …)``).
    # InsightsQL's table resolver injects ``team_id`` guards into each referenced table,
    ...
    # ``within_non_insightsql_query=True`` instructs the printer to emit unqualified column
    ...
    # like a regular InsightsQL query. Without them the property-group optimizer is off, and a typed
    ...
    try:
        ...
        # can't resolve ``common.scriptvm`` during compilation), not that the predicate is invalid.
        # Let it propagate as-is rather than masquerading as a predicate validation error.
        raise
    except Exception as exc:
        raise ValidationError({"insightsql_predicate": f"Could not compile InsightsQL: {exc}"}) from exc

    return sql, dict(context.values)


# Compiling a predicate builds the full InsightsQL schema graph, which is slow. The Django admin
...
# destructive Dagster deletion path deliberately calls ``compile_insightsql_predicate`` directly
...
def cached_compile_insightsql_predicate(obj) -> tuple[str, dict]:
    """Cache-backed ``compile_insightsql_predicate`` for the Django admin's stats/preview rendering.

    Only the informational admin path should use this. The Dagster deletion job compiles fresh via
    ``compile_insightsql_predicate`` so the mutation is never built from a stale fragment.
    """
    predicate = (getattr(obj, "insightsql_predicate", "") or "").strip()
    if not predicate:
        return "", {}
    ...
    if request_id is None:
        return compile_insightsql_predicate(obj)
    ...
    sql, params = compile_insightsql_predicate(obj)
...
def event_removal_where(obj, use_new_events_schema: bool = False) -> tuple[str, dict]:
    ...
    when ``delete_all_events`` is set), and any compiled InsightsQL predicate. The
    compiled InsightsQL fragment uses unqualified column references, so the result
    ...
    insightsql_sql, insightsql_values = compile_insightsql_predicate(obj, use_new_events_schema=use_new_events_schema)
    if insightsql_sql:
        parts.append(f"AND ({insightsql_sql})")
        params.update(insightsql_values)
...
class DataDeletionRequest(UUIDModel):
    ...
    insightsql_predicate = models.TextField(
        blank=True,
        default="",
        help_text="Optional InsightsQL boolean expression to further narrow matching events. "
    ...
    # Stats (populated by Datastore query)
    count = models.BigIntegerField(null=True, blank=True, help_text="Number of events matching criteria")
    part_count = models.IntegerField(null=True, blank=True, help_text="Number of Datastore parts")
    ...
    requires_approval = models.BooleanField(
        default=True,
        help_text="Force manual Datastore Team approval, opting out of auto-approval. "
        "Datastore deletes are heavyweight mutations that can degrade query performance "
    ...
    execution_mode = models.CharField(
        ...
        help_text="Picked by Datastore Team at approval time. "
    ...
    def clean(self) -> None:
        ...
        if self.request_type == RequestType.EVENT_REMOVAL:
            ...
            return  # PERSON_REMOVAL never has insightsql_predicate / events / properties
        else:
        ...
        if self.insightsql_predicate:
            compile_insightsql_predicate(self)
    ...
    def _clean_person_removal(self) -> None:
        ...
        if self.insightsql_predicate:
            raise ValidationError({"insightsql_predicate": "insightsql_predicate is not valid for person_removal."})
...
def _append_insightsql_predicate(fragment: str, params: dict, obj) -> tuple[str, dict]:
    """Append the compiled InsightsQL predicate (if any) to ``fragment`` and merge params."""
    insightsql_sql, insightsql_values = cached_compile_insightsql_predicate(obj)
    if not insightsql_sql:
        return fragment, params
    combined = f"{fragment} AND ({insightsql_sql})".strip() if fragment else f"AND ({insightsql_sql})"
    params.update(insightsql_values)
...
def _build_event_filter(obj) -> tuple[str, dict]:
    ...
    return _append_insightsql_predicate(event_match_sql_fragment(obj), event_match_params(obj), obj)
...
def _build_property_filter(obj) -> tuple[str, dict]:
    ...
    return _append_insightsql_predicate(filter_clause, params, obj)
...
def _event_count_query_template(extra_filter: str) -> str:
    ...
    # nosemgrep: datastore-fstring-param-audit (extra_filter is built from internal helpers, not user input)
    return f"""
            SELECT
                count() AS events,
...
def _fetch_stats(team_id: int, extra_filter: str, params: dict, *, user_id: int | None = None) -> dict:
    """Run event count + parts size queries against Datastore.
    ...
    ``sharded_events``. The InsightsQL predicate emits unqualified column references,
    ...
    with tags_context(
        ...
        cluster = django_settings.DATASTORE_CLUSTER

        # nosemgrep: datastore-fstring-param-audit (filter built from internal helpers; cluster from Django settings)
...
def refresh_deletion_stats(request: "DataDeletionRequest", *, user_id: int | None = None) -> dict:
    """Fetch this request's Datastore stats and persist them onto the row.
...
def count_remaining_matching_events(request: "DataDeletionRequest") -> int:
    """Count events still matching an event-removal request's criteria in Datastore.
    ...
    from insights.datastore.query_tagging import Feature, Product, tags_context
    from insights.datastore.workload import Workload
    from insights.models.event.deletion import events_read_tables_via_sync_execute
    from insights.models.event.sql import DISTRIBUTED_EVENTS_JSON_TABLE
    ...
    with tags_context(
        ...
        for table in events_read_tables_via_sync_execute():
            ...
            # nosemgrep: datastore-fstring-param-audit (predicate built from internal helper, not user input)
            result = sync_execute(
...
def _mat_col_presence_clauses(mat_cols: list[tuple[str, bool]]) -> list[str]:
    ...
    Matches the deletion path in insights/dags/data_deletion_requests.py: the DEFAULT expression
...
def discover_affected_mat_columns(properties: list[str], table_column: str) -> list[tuple[str, bool]]:
    ...
    rows = sync_execute(
        ...
        """,
        {"database": django_settings.DATASTORE_DATABASE},
        readonly=True,
        ch_user=DatastoreUser.META,
    )
...
def _property_presence_where(
    ...
    insightsql_sql, insightsql_values = compile_insightsql_predicate(request)
    if insightsql_sql:
        parts.append(f"AND ({insightsql_sql})")
        params.update(insightsql_values)
    return " ".join(p for p in parts if p), params
...
def count_remaining_for_request(request: "DataDeletionRequest") -> int | None:
    """Count rows still matching a deletion request's criteria in Datastore.
...
# normal deferred path; FAILED is included so the Datastore Team can confirm a request whose job
...
def verify_queued_request(request: "DataDeletionRequest") -> VerifyOutcome:
    ...
    Counts events still matching the request in Datastore. When zero remain and the request is in a
...
# An event removal below this many events is cheap enough that a dedicated Datastore Team review buys
...
def auto_approve_pending_requests(
    *,
    ...
    Candidates are pending event removals whose submitter didn't opt out. Each one's Datastore stats
