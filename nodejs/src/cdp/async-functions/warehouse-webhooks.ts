...
registerAsyncFunction('produceToWarehouseWebhooks', {
    execute: (args, context, result) => {
        ...
        if (!payload || typeof payload !== 'object') {
            throw new Error('[InsightsFunction] - produceToWarehouseWebhooks requires an object payload')
        }
        ...
        // Use explicit schema_id from Script code args, or fall back to inputs for backward compat
        const schemaId = explicitSchemaId ?? context.invocation.insightsFunction.inputs?.schema_id?.value
        if (!schemaId || typeof schemaId !== 'string') {
            throw new Error(
                '[InsightsFunction] - produceToWarehouseWebhooks requires a schema_id (either as second argument or in script function inputs)'
            )
        }
