"""Turning off a subscription that can never deliver again, and saying why.

A subscription whose target is permanently broken — a disconnected Slack, a
channel the bot was thrown out of, a prompt that no longer has an author — would
otherwise re-fire and fail on every schedule. One reason vocabulary serves the
three readers of that decision: the analytics event keys on `key`, the recipient
sees `description`, and someone trying to switch it back on gets `user_message`.
"""
...
SLACK_PERMISSION_REVOKED_DISABLE_REASON = DisableReason(
    ...
    description="Insights can no longer post to this Slack channel",
    user_message="Cannot re-enable {target_type} subscription: Insights can't post to this Slack channel. Reconnect Slack or re-add the bot to the channel, then try again.",
)
...
def _notification_creator(subscription: Subscription) -> User | None:
...
def disable_invalid_subscription(subscription: Subscription, reason: DisableReason) -> None:
    ...
    try:
        create_disabled_notification(subscription, reason)
    ...
    creator = _notification_creator(subscription)
    if creator and creator.email:
        try:
            send_disabled_email(subscription, reason, [creator.email])
        except Exception as e:
            # Disabling is the durable side effect; email is best-effort. If the email
            # fails (SMTP outage, ImproperlyConfigured on self-hosted, provider 5xx)
...
def create_disabled_notification(subscription: Subscription, reason: DisableReason) -> None:
    creator = _notification_creator(subscription)
    if creator is None:
        return
    ...
    create_notification(
        NotificationData(
            ...
            body=(
                f"Insights automatically disabled this subscription because {reason.description.lower()}. "
...
def send_disabled_email(subscription: Subscription, reason: DisableReason, targets: list[str]) -> None:
    logger.info(
    ...
    subject = (
        f'Insights subscription "{subscription.title}" has been automatically disabled'
        if subscription.title
        else "Your Insights subscription has been automatically disabled"
    )
