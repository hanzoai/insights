...
from insights.schema import RecordingOrder, RecordingOrderDirection

from insights.api.test.dashboards import DashboardAPI
from insights.datastore.client.execute import UntaggedQueryError
...
from insights.session_recordings.models.session_recording_playlist_item import SessionRecordingPlaylistItem
...
class TestWidgetRegistry(APIBaseTest):
    ...
    @parameterized.expand(
        [
            (
                "missing_key",
                {"properties": [{"type": "person", "operator": "icontains", "value": "@hanzo.ai"}]},
            ),
            (
                "unknown_type",
...
class TestDashboardRunWidgets(APIBaseTest):
    ...
    def test_activity_events_widget_applies_date_range_and_property_filters(self, mock_runner_cls: MagicMock) -> None:
        ...
        self.assertEqual(query.properties[0].value, "@hanzo.ai")
    ...
    @patch("insights.session_recordings.session_recording_api.ListingBurstRateThrottle.wait", return_value=30)
    @patch("insights.session_recordings.session_recording_api.list_recordings_from_query")
    ...
    def test_session_replay_widget_tags_queries_in_debug_mode(self, mock_list_recordings: MagicMock) -> None:
        ...
        def assert_tagged(*_args: object, **_kwargs: object) -> tuple[list[object], bool, None, None]:
            ...
            if tags.product != Product.REPLAY or tags.feature != Feature.QUERY:
                raise UntaggedQueryError("session replay widget must tag Datastore queries")
    ...
    @patch("products.error_tracking.backend.facade.queries.ErrorTrackingQueryRunner.calculate")
    @patch("insights.slo.events.hanzo_insights.capture")
