...
from insights.insightsql.constants import (
    ...
    InsightsQLParserBackend,
    InsightsQLQuerySettings,
    LimitContext,
)
...
class TestPrinter(BaseTest):
    ...
    def test_fields_and_properties(self):
        ...
        with override_settings(PERSON_ON_EVENTS_V2_OVERRIDE=False):
            ...
            self.assertEqual(
                ...
                "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(person_props, %(insightsql_val_0)s), ''), 'null'), '^\"|\"$', '')",
            )
            context = InsightsQLContext(team_id=self.team.pk)
            self.assertEqual(
                self._expr("person.properties.bla", context),
    ...
    def test_insightsql_properties(self):
        self.assertEqual(
            self._expr("event", InsightsQLContext(team_id=self.team.pk), "insightsql"),
            "event",
        )
        self.assertEqual(
            self._expr("person", InsightsQLContext(team_id=self.team.pk), "insightsql"),
            "person",
        )
        self.assertEqual(
            self._expr(
        ...
        self.assertEqual(
            self._expr("properties.$browser", InsightsQLContext(team_id=self.team.pk), "insightsql"),
            "properties.$browser",
        )
        self.assertEqual(
            self._expr(
    ...
    def _test_property_group_comparison(
        ...
        def build_context(property_groups_mode: PropertyGroupsMode) -> InsightsQLContext:
        ...
        if expected_skip_indexes_used is not None or expected_skip_indexes_not_used is not None:
            ...
            # Include InsightsQLGlobalSettings() so that when we check indexes, we see what skip indexes would be used with realistic settings.
            ...
            [[raw_explain_result]] = sync_execute(
                ...
                settings={
                    ...
                    for k, v in InsightsQLGlobalSettings().model_dump().items()
                    if v is not None
                },
            )
    ...
    def test_property_groups_optimized_basic_equality_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "equals(events.properties_group_custom[%(insightsql_val_0)s], %(insightsql_val_1)s) AS eq",
            {"insightsql_val_0": "key", "insightsql_val_1": "value"},
    ...
    def test_property_groups_optimized_boolean_equality_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "equals(events.properties_group_custom[%(insightsql_val_0)s], 'true')",
            {"insightsql_val_0": "is_boolean"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )

        self._test_property_group_comparison(
            "properties.is_boolean = false",
            "equals(events.properties_group_custom[%(insightsql_val_0)s], 'false')",
            {"insightsql_val_0": "is_boolean"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )
    ...
    def test_property_groups_optimized_null_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "has(events.properties_group_custom, %(insightsql_val_0)s) AS p",
            {"insightsql_val_0": "key"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf"},
        )
        self._test_property_group_comparison(
        ...
        self._test_property_group_comparison(
            ...
            "not(has(events.properties_group_custom, %(insightsql_val_0)s)) AS p",
            {"insightsql_val_0": "key"},
        )
        self._test_property_group_comparison(
            "properties.key = null as p",
    ...
    def test_property_groups_optimized_has(self) -> None:
        self._test_property_group_comparison(
            ...
            "has(events.properties_group_custom, %(insightsql_val_0)s) AS j",
            {"insightsql_val_0": "key"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf"},
        )
    ...
    def test_property_groups_optimized_in_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "equals(events.properties_group_custom[%(insightsql_val_0)s], %(insightsql_val_1)s)",
            {"insightsql_val_0": "key", "insightsql_val_1": "a"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )
        self._test_property_group_comparison(
            "properties.key IN ['a']",
    ...
    def test_event_property_groups_optimized_in_query_results(self):
        ...
        def assert_result_is_equal(expr: str, labels):
            ...
            enabled_context = InsightsQLContext(
                team_id=self.team.pk,
                enable_select_queries=True,
                modifiers=InsightsQLQueryModifiers(
                    materializationMode=MaterializationMode.AUTO,
            ...
            optimized_context = InsightsQLContext(
                team_id=self.team.pk,
                enable_select_queries=True,
                modifiers=InsightsQLQueryModifiers(
                    materializationMode=MaterializationMode.AUTO,
    ...
    def test_functions(self):
        context = InsightsQLContext(team_id=self.team.pk)  # inline values

        self.assertEqual(self._expr("abs(1)"), "abs(1)")
        self.assertEqual(self._expr("max2(1,2)"), "max2(1, 2)")
        self.assertEqual(self._expr("toInt('1')", context), "accurateCastOrNull(%(insightsql_val_0)s, %(insightsql_val_1)s)")
        self.assertEqual(self._expr("toFloat('1.3')", context), "accurateCastOrNull(%(insightsql_val_2)s, %(insightsql_val_3)s)")
        self.assertEqual(
            self._expr("toUUID('470f9b15-ff43-402a-af9f-2ed7c526a6cf')", context),
            "accurateCastOrNull(%(insightsql_val_4)s, %(insightsql_val_5)s)",
        )
        self.assertEqual(
    ...
    def test_expr_syntax_errors(self):
        ...
        self._assert_expr_error("b.a(bla)", "You can only call simple functions in InsightsQL, not expressions")
    ...
    def test_boolean_and_optimization(self):
        self.assertEqual(
            ...
            "and(equals(events.team_id, 1), equals(events.event, %(insightsql_val_0)s))",
        )
        self.assertEqual(
    ...
    def test_boolean_or_optimization(self):
        self.assertEqual(
            ...
            "or(equals(events.team_id, 1), equals(events.event, %(insightsql_val_0)s))",
        )
        self.assertEqual(
    ...
    def test_logic(self):
        self.assertEqual(
            ...
            "or(events.event, toTimeZone(events.timestamp, %(insightsql_val_0)s))",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "or(events.event, toTimeZone(events.timestamp, %(insightsql_val_0)s), count())",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "or(events.event, not(toTimeZone(events.timestamp, %(insightsql_val_0)s)))",
        )

    def test_comparisons(self):
        context = InsightsQLContext(team_id=self.team.pk)
        self.assertEqual(self._expr("event == 'E'", context), "equals(events.event, %(insightsql_val_0)s)")
        self.assertEqual(
            self._expr("event != 'E'", context),
            "notEquals(events.event, %(insightsql_val_1)s)",
        )
        self.assertEqual(self._expr("event > 'E'", context), "greater(events.event, %(insightsql_val_2)s)")
        self.assertEqual(
            self._expr("event >= 'E'", context),
            "greaterOrEquals(events.event, %(insightsql_val_3)s)",
        )
        self.assertEqual(self._expr("event < 'E'", context), "less(events.event, %(insightsql_val_4)s)")
        self.assertEqual(
            self._expr("event <= 'E'", context),
            "lessOrEquals(events.event, %(insightsql_val_5)s)",
        )
        self.assertEqual(self._expr("event like 'E'", context), "like(events.event, %(insightsql_val_6)s)")
        self.assertEqual(
            self._expr("event not like 'E'", context),
            "notLike(events.event, %(insightsql_val_7)s)",
        )
        self.assertEqual(
            self._expr("event ilike 'E'", context),
            "ilike(events.event, %(insightsql_val_8)s)",
        )
        self.assertEqual(
            self._expr("event not ilike 'E'", context),
            "notILike(events.event, %(insightsql_val_9)s)",
        )
        self.assertEqual(self._expr("event in 'E'", context), "in(events.event, %(insightsql_val_10)s)")
        self.assertEqual(
            self._expr("event not in 'E'", context),
            "notIn(events.event, %(insightsql_val_11)s)",
        )
        self.assertEqual(self._expr("event ~ 'E'", context), "match(events.event, %(insightsql_val_12)s)")
        self.assertEqual(self._expr("event =~ 'E'", context), "match(events.event, %(insightsql_val_13)s)")
        self.assertEqual(
            self._expr("event !~ 'E'", context),
            "not(match(events.event, %(insightsql_val_14)s))",
        )
        self.assertEqual(
            self._expr("event ~* 'E'", context),
            "match(events.event, concat('(?i)', %(insightsql_val_15)s))",
        )
        self.assertEqual(
            self._expr("event =~* 'E'", context),
            "match(events.event, concat('(?i)', %(insightsql_val_16)s))",
        )
        self.assertEqual(
            self._expr("event !~* 'E'", context),
            "not(match(events.event, concat('(?i)', %(insightsql_val_17)s)))",
        )
    ...
    def test_values(self):
        context = InsightsQLContext(team_id=self.team.pk)
        self.assertEqual(self._expr("event == 'E'", context), "equals(events.event, %(insightsql_val_0)s)")
        self.assertEqual(context.values, {"insightsql_val_0": "E"})
        self.assertEqual(
            self._expr("coalesce(4.2, 5, 'lol', 'hoo')", context),
            "coalesce(4.2, 5, %(insightsql_val_1)s, %(insightsql_val_2)s)",
        )
        self.assertEqual(
            context.values,
            {"insightsql_val_0": "E", "insightsql_val_1": "lol", "insightsql_val_2": "hoo"},
        )
    ...
    def test_print_timezone(self):
        ...
        self.assertEqual(
            context.values,
            {
                "insightsql_val_0": "UTC",
                "insightsql_val_1": "UTC",
                "insightsql_val_2": "UTC",
                "insightsql_val_3": "2020-02-02",
                "insightsql_val_4": "UTC",
                "insightsql_val_5": "2020-02-02 12:25",
                "insightsql_val_6": "UTC",
            },
        )
    ...
    def test_print_timezone_custom(self):
        ...
        self.assertEqual(
            context.values,
            {
                "insightsql_val_0": "Europe/Brussels",
                "insightsql_val_1": "Europe/Brussels",
                "insightsql_val_2": "Europe/Brussels",
                "insightsql_val_3": "2020-02-02",
    ...
    def test_nullish_concat(self):
        self.assertEqual(
            ...
            f"concat('', %(insightsql_val_0)s, toString(3), toString(4), '')",
        )

    def test_concat_pipes(self):
        self.assertEqual(
            self._expr("'a' || 'b' || 3 || timestamp"),
            f"concat(%(insightsql_val_0)s, %(insightsql_val_1)s, toString(3), ifNull(toString(toTimeZone(events.timestamp, %(insightsql_val_2)s)), ''))",
        )
    ...
    def test_to_start_of_week_gets_mode(self):
        ...
        default_week_context = InsightsQLContext(team_id=self.team.pk, database=Database(None, None))
        sunday_week_context = InsightsQLContext(team_id=self.team.pk, database=Database(None, WeekStartDay.SUNDAY))
        monday_week_context = InsightsQLContext(team_id=self.team.pk, database=Database(None, WeekStartDay.MONDAY))

        self.assertEqual(
            self._expr("toStartOfWeek(timestamp)", default_week_context),  # Sunday is the default
            f"toStartOfWeek(toTimeZone(events.timestamp, %(insightsql_val_0)s), 0)",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            f"toStartOfWeek(toTimeZone(events.timestamp, %(insightsql_val_0)s), 3)",
        )

    def test_functions_expecting_datetime_arg(self):
        self.assertEqual(
            self._expr("tumble(toDateTime('2023-06-12'), toIntervalDay('1')) as t"),
            f"tumble(assumeNotNull(toDateTime(toDateTime(%(insightsql_val_0)s, %(insightsql_val_1)s))), toIntervalDay(%(insightsql_val_2)s)) AS t",
        )
        self.assertEqual(
            self._expr("tumble(now(), toIntervalDay('1')) as t"),
            f"tumble(toDateTime(now64(6, %(insightsql_val_0)s), 'UTC'), toIntervalDay(%(insightsql_val_1)s)) AS t",
        )
        self.assertEqual(
            self._expr("tumble(parseDateTime('2021-01-04+23:00:00', '%Y-%m-%d+%H:%i:%s'), toIntervalDay('1')) as t"),
            f"tumble(assumeNotNull(toDateTime(parseDateTimeOrNull(%(insightsql_val_0)s, %(insightsql_val_1)s, %(insightsql_val_2)s))), toIntervalDay(%(insightsql_val_3)s)) AS t",
        )
        self.assertEqual(
            self._expr("tumble(parseDateTimeBestEffort('23/10/2020 12:12:57'), toIntervalDay('1')) as t"),
            f"tumble(assumeNotNull(toDateTime(parseDateTime64BestEffort(%(insightsql_val_0)s, 6, %(insightsql_val_1)s))), toIntervalDay(%(insightsql_val_2)s)) AS t",
        )
        self.assertEqual(
    ...
    def test_field_nullable_like(self):
        ...
        assert generated_sql_statements1 == self._with_active_events_table(
            ...
            # null like 'a',
            "ifNull(like(NULL, %(insightsql_val_1)s), 0) AS d, "
            # 'a' like event,
            "ifNull(like(%(insightsql_val_2)s, events.nullable_field), 0) AS e, "
            # 'a' like null
            "isNull(%(insightsql_val_3)s) AS f "
            f"FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}"
        )
    ...
    def test_field_nullable_not_like(self):
        ...
        assert generated_sql_statements1 == self._with_active_events_table(
            ...
            # null like 'a',
            "ifNull(notLike(NULL, %(insightsql_val_1)s), 1) AS d, "
            # 'a' like event,
            "ifNull(notLike(%(insightsql_val_2)s, events.nullable_field), 1) AS e, "
            # 'a' like null
            "isNotNull(%(insightsql_val_3)s) AS f "
            f"FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}"
        )
    ...
    def test_print_query_level_settings(self):
        ...
        query.settings = InsightsQLQuerySettings(optimize_aggregation_in_order=True)
        printed, _ = prepare_and_print_ast(
            query,
            InsightsQLContext(team_id=self.team.pk, enable_select_queries=True),
            "datastore",
    ...
    def test_table_top_level_settings_conflict_between_tables(self):
        db = Database()
        db.get_table("preaggregation_results").top_level_settings = InsightsQLQuerySettings(load_balancing="round_robin")
    ...
    def test_table_top_level_settings_conflict_with_query_settings(self):
        ...
        assert isinstance(query, ast.SelectQuery)
        query.settings = InsightsQLQuerySettings(load_balancing="round_robin")
        with self.assertRaises(QueryError) as cm:
            prepare_and_print_ast(
                query,
                InsightsQLContext(team_id=self.team.pk, enable_select_queries=True),
    ...
    def test_table_top_level_settings_same_value_in_query_settings(self):
        query = parse_select("SELECT job_id FROM preaggregation_results")
        assert isinstance(query, ast.SelectQuery)
        query.settings = InsightsQLQuerySettings(load_balancing="in_order")
        printed, _ = prepare_and_print_ast(
            query,
            InsightsQLContext(team_id=self.team.pk, enable_select_queries=True),
            "datastore",
    ...
    def test_override_timezone(self):
        ...
        self.assertEqual(
            context.values,
            {
                ...
                "insightsql_val_1": "UTC",
                "insightsql_val_2": "UTC",
                "insightsql_val_3": "US/Pacific",
                "insightsql_val_4": "UTC",
                "insightsql_val_5": "US/Pacific",
            },
        )
    ...
    def test_print_insightsql_aggregation_function_uses_insightsql_function_names(self):
        query = parse_expr("avgArray([1, 2, 3])")
        printed, _ = prepare_and_print_ast(query, InsightsQLContext(team_id=self.team.pk), dialect="insightsql")
    ...
    def test_print_percentage_call_alias(self):
        ...
        assert (
            printed
            == "SELECT concat(%(insightsql_val_0)s, %(insightsql_val_1)s, %(insightsql_val_2)s) AS `concat('', 'word', '')` LIMIT 1"
        )

    def test_print_insightsql_output_format(self):
        printed = self._print(
            "select 1 limit 1",
    ...
    def test_print_insightsql_in_cohort(self):
    ...
    def test_print_insightsql_not_in_cohort(self):
        Cohort.objects.create(team=self.team, name="some fake cohort", created_by=self.user)
        query = parse_select(
    ...
    def test_s3_tables_global_join_with_cte(self, name, using_global_joins, optimize_projections):
        with mock.patch("insights.insightsql.resolver.USE_GLOBAL_JOINS", using_global_joins):
    ...
    def test_final_keyword_not_supported(self):
        ...
        self.assertEqual("The FINAL keyword is not supported in InsightsQL as it causes slow queries", str(e.exception))
    ...
    @parameterized.expand(
        [
            ...
            # Constant cast
            ("const_int", "'123'::int", "toInt64(%(insightsql_val_0)s)"),
...
class TestMaterializedColumnOptimization(DatastoreTestMixin, APIBaseTest):
    ...
    def test_materialized_column_optimized_equality_comparison_non_nullable(self) -> None:
        ...
        with materialized("events", "test_prop", is_nullable=False) as mat_col:
            self._test_materialized_column_comparison(
                ...
                f"equals(events.{mat_col.name}, %(insightsql_val_0)s)",
            ...
            self._test_materialized_column_comparison(
                "properties.test_prop != 'some_value'",
                f"notEquals(events.{mat_col.name}, %(insightsql_val_0)s)",
                {"insightsql_val_0": "some_value"},
            )
            self._test_materialized_column_comparison(
                "'some_value' != properties.test_prop",
    ...
    def test_materialized_column_optimized_equality_comparison_nullable(self) -> None:
        ...
        with materialized("events", "test_prop", is_nullable=True) as mat_col:
            ...
            self._test_materialized_column_comparison(
                "properties.test_prop != 'some_value'",
                f"ifNull(notEquals(events.{mat_col.name}, %(insightsql_val_0)s), 1)",
                {"insightsql_val_0": "some_value"},
            )
            self._test_materialized_column_comparison(
                "'some_value' != properties.test_prop",
    ...
    def test_materialized_column_equality_not_optimized_for_empty_string(self) -> None:
        with materialized("events", "test_prop", is_nullable=False) as mat_col:
            self._test_materialized_column_comparison(
                ...
                f"ifNull(equals(nullIf(nullIf(events.{mat_col.name}, ''), 'null'), %(insightsql_val_0)s), 0)",
            )
    ...
    def test_materialized_column_ilike_with_tostring_not_optimized_for_numeric_property(self) -> None:
        ...
        with materialized("events", "test_numeric_prop", is_nullable=False) as mat_col:
            ...
            self._test_materialized_column_comparison(
                ...
                f"ifNull(ilike(accurateCastOrNull(nullIf(nullIf(events.{mat_col.name}, ''), 'null'), %(insightsql_val_0)s), %(insightsql_val_1)s), 0)",
                {"insightsql_val_1": "%123%"},
            )

            # With toString() wrapper: same behavior, optimization not applied
            self._test_materialized_column_comparison(
                "ilike(toString(properties.test_numeric_prop), '%123%')",
                f"ifNull(ilike(toString(accurateCastOrNull(nullIf(nullIf(events.{mat_col.name}, ''), 'null'), %(insightsql_val_0)s)), %(insightsql_val_1)s), 0)",
                {"insightsql_val_1": "%123%"},
            )
    ...
    def test_materialized_column_like_bails_out_for_sentinel_pattern_on_non_nullable(self) -> None:
        ...
        with materialized("events", "test_prop", is_nullable=False) as mat_col:
            self._test_materialized_column_comparison(
                "properties.test_prop like '%null%'",
                f"ifNull(like(nullIf(nullIf(events.{mat_col.name}, ''), 'null'), %(insightsql_val_0)s), 0)",
                {"insightsql_val_0": "%null%"},
            )
    ...
    def test_materialized_column_in_uses_raw_column_for_non_nullable(self) -> None:
        with materialized("events", "test_prop", is_nullable=False) as mat_col:
            self._test_materialized_column_comparison(
                ...
                f"has([%(insightsql_val_0)s, %(insightsql_val_1)s], events.{mat_col.name})",
    ...
    def test_materialized_column_not_in_uses_raw_column_for_non_nullable(self) -> None:
        with materialized("events", "test_prop", is_nullable=False) as mat_col:
            self._test_materialized_column_comparison(
                ...
                f"notIn(events.{mat_col.name}, tuple(%(insightsql_val_0)s, %(insightsql_val_1)s))",
                {"insightsql_val_0": "value1", "insightsql_val_1": "value2"},
            )
    ...
    def test_materialized_column_in_bails_out_for_sentinel_value_on_non_nullable(self) -> None:
        ...
        with materialized("events", "test_prop", is_nullable=False) as mat_col:
            self._test_materialized_column_comparison(
                ...
                f"in(nullIf(nullIf(events.{mat_col.name}, ''), 'null'), tuple(%(insightsql_val_0)s, %(insightsql_val_1)s))",
                {"insightsql_val_0": "null", "insightsql_val_1": "value2"},
            )
            self._test_materialized_column_comparison(
                "properties.test_prop in ('', 'value2')",
                f"in(nullIf(nullIf(events.{mat_col.name}, ''), 'null'), tuple(%(insightsql_val_0)s, %(insightsql_val_1)s))",
                {"insightsql_val_0": "", "insightsql_val_1": "value2"},
            )
    ...
    def test_materialized_column_not_in_nullable(self) -> None:
        with materialized("events", "test_prop", is_nullable=True) as mat_col:
            self._test_materialized_column_comparison(
                "properties.test_prop not in ('value1', 'value2')",
                f"ifNull(notIn(events.{mat_col.name}, tuple(%(insightsql_val_0)s, %(insightsql_val_1)s)), 1)",
                {"insightsql_val_0": "value1", "insightsql_val_1": "value2"},
            )
...
class TestPostgresPrinter(BaseTest):
    ...
    def _select(
        ...
        context: Optional[InsightsQLContext] = None,
        placeholders: Optional[dict[str, ast.Expr]] = None,
        dialect: InsightsQLDialect = "postgres",
    ) -> str:
        return prepare_and_print_ast(
    ...
    def test_insightsqlx_row_literals_render_without_tuple_function(self):
        hx_tag = convert_tag_to_hx(ast.InsightsQLXTag(kind="div", attributes=[]))
        context = InsightsQLContext(team_id=self.team.pk, enable_select_queries=True)
...
class TestDuckDBPrinter(SimpleTestCase):
    ...
    def _expr(
        ...
        node = parse_expr(query, backend=backend) if isinstance(query, str) else query
        context = context or InsightsQLContext(team_id=self.team_id, enable_select_queries=True)
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                "date_trunc(%(insightsql_val_0)s, events.timestamp)",
            ),
    ...
    @parameterized.expand(
        [
            (
                ...
                "arg_max(events.event, events.timestamp) FILTER (WHERE (events.event = %(insightsql_val_0)s))",
            ),
            (
                "argMinIf_uses_filter",
                "argMinIf(event, timestamp, event = 'x')",
                "arg_min(events.event, events.timestamp) FILTER (WHERE (events.event = %(insightsql_val_0)s))",
            ),
            (
                "dateAdd_builds_interval",
                "dateAdd('day', 2, timestamp)",
                "date_add(events.timestamp, CAST((CAST(2 AS VARCHAR) || ' ' || CAST(%(insightsql_val_0)s AS VARCHAR)) AS INTERVAL))",
            ),
            (
            ...
            (
                ...
                "CAST(date_add(CAST(%(insightsql_val_1)s AS DATE), CAST((CAST(2 AS VARCHAR) || ' ' || CAST(%(insightsql_val_0)s AS VARCHAR)) AS INTERVAL)) AS DATE)",
            ),
            (
                "dateTrunc_preserves_date_type",
                "dateTrunc('month', toDate('2026-08-04'))",
                "CAST(date_trunc(%(insightsql_val_0)s, CAST(%(insightsql_val_1)s AS DATE)) AS DATE)",
            ),
            ...
            (
                ...
                "list(DISTINCT events.event) FILTER (WHERE (events.event = %(insightsql_val_0)s))",
            ),
            (
            ...
            ("like_uses_operator", "like(event, 'x%')", "(events.event LIKE %(insightsql_val_0)s)"),
    ...
    def test_identifier_no_truncation(self):
        ...
        printer = DuckDBPrinter(context=InsightsQLContext(team_id=self.team_id))
    ...
    def test_percent_in_identifier_rejected_postgres_family(self):
        ...
        ctx = InsightsQLContext(team_id=self.team_id)
    ...
    def test_dollar_prefixed_property_renders_as_jsonpath_member(self):
        ...
        context = InsightsQLContext(team_id=self.team_id, enable_select_queries=True)
