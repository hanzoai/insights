...
/// The `cohort_backfill_*` DDL (plus a schema-local `insights_cohort` projection), pinned to Django
...
/// Insert a row into the schema-local `insights_cohort` projection for `load_current_behavioral_hashes`.
pub async fn insert_cohort(
    ...
    sqlx::query(
        "INSERT INTO insights_cohort (id, team_id, behavioral_filters_shape_hash, deleted) \
         VALUES ($1, $2, $3, $4)",
    )
    .bind(id)
    .bind(team_id)
