...
from insights.helpers.slack_scopes import bot_is_ready
from insights.models.integration import Integration

BOT_SETUP_DOCS_URL = "https://hanzo.ai/docs/slack-app"


def build_explore_hint(integration: Integration | None, *, utm_tags: str, ai_enabled: bool) -> dict[str, Any] | None:
    """Slack context block nudging the channel to @Insights this report (or to set the bot up).
    ...
    if bot_is_ready(integration):
        text = "💬 Reply in this thread and mention *@Insights* with a question to dig into this report."
    else:
        text = f"💬 <{BOT_SETUP_DOCS_URL}?{utm_tags}|Set up the @Insights bot> to ask follow-up questions about your reports here."
