...
from insights_owners import fmt as fmt_module
from insights_owners.cli import _consolidation_suggestions, _live_scope, _reserved_location_error
from insights_owners.fmt import CanonicalPlacer, CanonicalPlan
from insights_owners.matcher import path_matches_pattern
from insights_owners.resolver import OwnersResolver
from insights_owners.schema import is_simple_owners_file, parse_owners_file
...
def resolver_repo(tmp_path: Path) -> Path:
    ...
    _write(
        tmp_path,
        "insights/owners.yaml",
        "version: 1\nowners: [team-a]\n"
        "rules:\n  - match: '/vendor/**'\n    owners: null\n  - match: legacy.py\n    owners: [team-legacy]\n",
    )
    _write(tmp_path, "insights/sub/owners.yaml", "version: 1\nowners: [team-b]\n")
    _write(tmp_path, "insights/noinherit/owners.yaml", "version: 1\ninherit: false\nowners: [team-c]\n")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("README.md", None, True),
        ("insights/x.py", ["team-a"], False),
        ("insights/legacy.py", ["team-legacy"], False),
        ("insights/vendor/lib.py", None, True),
        ("insights/sub/y.py", ["team-b"], False),
        ("insights/sub/legacy.py", ["team-b"], False),
        ("insights/noinherit/z.py", ["team-c"], False),
...
def test_resolver_no_contribution_is_unowned_not_exempt(tmp_path: Path) -> None:
    _write(tmp_path, "insights/owners.yaml", "version: 1\nowners: [team-a]\n")
    resolver = OwnersResolver(repo_root=tmp_path)
    r = resolver.resolve("other/file.py")
    assert r.owners is None
    assert r.unowned_by_design is False
    assert resolver.unowned(["other/file.py", "insights/x.py"]) == ["other/file.py"]


@pytest.mark.parametrize(
    "path,slack",
    [
        ("insights/x.py", "#team-a"),
        ("insights/sub/y.py", "#team-b"),
        ("insights/noinherit/z.py", "#team-c"),
