"""Parse and serialize insights and dag configuration files.

A insights config file (insights.toml / insights.yaml / insights.yml) defines the root of a modeling
...
@dataclass
class ProjectConfig:
    """Parsed representation of a insights config file."""
...
def parse_project_config(content: str, format: ConfigFormat | None = None) -> ProjectConfig:
    """Parse a insights config file (TOML or YAML) into a ProjectConfig.
    ...
    try:
        ...
        raise ValueError(f"Invalid insights config: {e}") from e

    unknown_sections = set(data.keys()) - _VALID_PROJECT_SECTIONS
    if unknown_sections:
        raise ValueError(f"Unknown sections in insights config: {', '.join(sorted(unknown_sections))}")

    # [project] section is required
    project = data.get("project")
    if not project or not isinstance(project, dict):
        raise ValueError("insights config must have a [project] section")

    name = project.get("name", "")
    if not name:
        raise ValueError("insights config [project] must have a name")
    ...
    if version not in SUPPORTED_VERSIONS:
        raise ValueError(f"Unsupported insights config version {version}, expected one of: {SUPPORTED_VERSIONS}")
    ...
    if not environments:
        raise ValueError("insights config must define at least one environment")
