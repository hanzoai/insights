...
from posthog.insightsql_queries.legacy_compatibility.filter_to_query import (
    filter_to_query,
    ...
    "values": [
        {
            ...
            "values": [
                ...
                {
                    "key": "$feature/insightsql-insights",
                    "value": ["true"],
                    "operator": "exact",
                    "type": "event",
                },
                ...
                {"type": "insightsql", "key": "properties.name", "value": None},
            ],
        },
        {"type": "OR", "values": [{}]},
    ],
...
class TestFilterToQuery(BaseTest):
        ...
        filter: dict[str, Any] = {
            "events": [
                ...
                },  # unique group
                {
                    "id": "$pageview",
                    "math": "insightsql",
                    "math_insightsql": "avg(toInt(properties.$session_id)) + 1000",
                },  # insightsql
            ]
        }
        ...
        self.assertEqual(
            ...
            [
                ...
                EventsNode(
                    event="$pageview",
                    name="$pageview",
                    properties=[InsightsQLPropertyFilter(key="dateDiff('minute', timestamp, now()) < 30")],
                ),
                EventsNode(
                    event="$pageview",
                    name="$pageview",
    ...
    def test_funnels_filter(self):
        filter: dict[str, Any] = {
            ...
            "funnel_aggregate_by_insightsql": "person_id",
            "exclusions": [
                {
                    "id": "$pageview",
                    "type": "events",
        ...
        self.assertEqual(
            ...
            FunnelsFilter(
                ...
                binCount=15,
                funnelAggregateByInsightsQL="person_id",
    ...
    def test_paths_filter(self):
        ...
        self.assertEqual(
            query.pathsFilter,
            PathsFilter(
                includeEventTypes=[PathType.FIELD_PAGEVIEW, PathType.INSIGHTSQL],
                pathsInsightsQLExpression="event",
