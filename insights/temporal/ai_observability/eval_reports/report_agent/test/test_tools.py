...
from datastore_driver.errors import NetworkError, SocketTimeoutError
from parameterized import parameterized

from insights.insightsql import ast

from insights.exceptions import DatastoreAtCapacity, DatastoreQueryMemoryLimitExceeded, DatastoreQueryTimeOut
from insights.temporal.ai_observability.eval_reports.report_agent.schema import (
    MAX_REPORT_SECTIONS,
    Citation,
    EvalReportContent,
    ReportSection,
)
from insights.temporal.ai_observability.eval_reports.report_agent.tools import (
    _SESSION_TRACES_SQL,
    _UUID_RE,
    _ch_ts,
...
def _per_query_memory_limit_error() -> DatastoreQueryMemoryLimitExceeded:
...
class TestChTs(SimpleTestCase):
    ...
    def test_converts_iso_to_utc_datetime(self, _name, iso_input, expected):
        ...
        # Returning a datetime (not string) so InsightsQL serializes with correct TZ alignment.
...
class TestSummaryMetrics(SimpleTestCase):
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_boolean_keeps_pass_rate_separate_from_outcome_distribution(self, mock_execute_insightsql):
        mock_execute_insightsql.side_effect = [
            [[80, 18, 2, 100]],
            [[7, 2, 1, 10]],
        ]
        state = {
        ...
        for call in mock_execute_insightsql.call_args_list:
            self.assertIn("properties.$ai_target_type = 'trace_id'", call.args[1])

    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_boolean_current_period_preserves_zero_pass_rate_when_no_results_are_applicable(self, mock_execute_insightsql):
        mock_execute_insightsql.side_effect = [
            [[0, 0, 4, 4]],
            [[0, 0, 3, 3]],
        ]
    ...
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_sentiment_uses_label_predicates_and_returns_distribution(self, mock_execute_insightsql):
        mock_execute_insightsql.side_effect = [
            [[2, 1, 1, 4]],
            [[1, 1, 0, 2]],
        ]
        ...
        current_query = mock_execute_insightsql.call_args_list[0].args[1]
...
class TestTargetAwareEvalResults(SimpleTestCase):
    ...
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_sample_uses_new_target_id_with_legacy_fallback_and_target_specific_key(self, mock_execute_insightsql):
        mock_execute_insightsql.side_effect = [
        ...
        generation_query = mock_execute_insightsql.call_args_list[0].args[1]
        trace_query = mock_execute_insightsql.call_args_list[1].args[1]
    ...
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_list_uses_unified_target_id_for_trace_results(self, mock_execute_insightsql):
        mock_execute_insightsql.side_effect = [
            [[1]],
        ...
        query = mock_execute_insightsql.call_args_list[1].args[1]
        self.assertIn("properties.$ai_target_id", query)
        self.assertIn("properties.$ai_target_event_id", query)

    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_sentiment_list_omits_classifier_reasoning(self, mock_execute_insightsql):
        mock_execute_insightsql.side_effect = [
            [[1]],
    ...
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_sentiment_sample_orders_by_score_and_omits_reasoning(self, mock_execute_insightsql):
        mock_execute_insightsql.return_value = [[_VALID_GEN_ID, "negative", "identical classifier reasoning", None, 0.91]]
        ...
        self.assertIn("ORDER BY score DESC, timestamp DESC", mock_execute_insightsql.call_args.args[1])
...
class TestTraceDetailTools(SimpleTestCase):
    ...
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._fetch_and_format_trace")
    def test_sample_accepts_evaluated_opaque_trace_ids_and_bounds_each_rendering(
        self, mock_fetch: MagicMock, mock_execute_insightsql: MagicMock
    ) -> None:
        mock_fetch.return_value = MagicMock(text_repr="x" * 5_000, event_count=4)
        trace_id = " customer/trace:alpha 42 "
        mock_execute_insightsql.return_value = [[trace_id, True, "matched", True, None]]
        state = self._state()
        ...
        mock_execute_insightsql.assert_called_once()

    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools.logger")
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._fetch_and_format_trace")
    ...
    def test_deep_dive_rejects_trace_not_returned_by_evaluation_query(
        ...
        state = self._state()
        mock_execute_insightsql.return_value = [["allowed-trace", True, "matched", True, None]]
        _sample_eval_results_fn(state=state)
        mock_execute_insightsql.reset_mock()
        ...
        mock_execute_insightsql.assert_not_called()
...
class TestSessionTracesQueryFallback(SimpleTestCase):
    def test_placeholder_names_survive_the_events_table_rewrite(self) -> None:
        ...
        from insights.insightsql_queries.ai.ai_column_rewriter import rewrite_query_for_events_table
...
class TestSessionDetailTools(SimpleTestCase):
    ...
    def _allowlisted_state(
        self, mock_execute_insightsql: MagicMock, session_id: str = _VALID_SESSION_ID
    ) -> _ReportToolState:
        state = self._state()
        mock_execute_insightsql.return_value = [[session_id, True, "matched", True, None]]
        _sample_eval_results_fn(state=state)
        mock_execute_insightsql.reset_mock()
        return state
    ...
    def test_evaluation_results_fill_the_session_allowlist_not_the_trace_one(
        self, mock_execute_insightsql: MagicMock
    ) -> None:
        state = self._allowlisted_state(mock_execute_insightsql, "customer/session:42")

        self.assertEqual(state["session_id_allowlist"], ["customer/session:42"])
        self.assertEqual(state["trace_id_allowlist"], [])

    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql_via_ai_events")
    ...
    @patch("insights.models.Team.objects.get")
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql_via_ai_events")
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_detail_maps_traces_and_unlocks_them_for_the_trace_tools(
        self, mock_execute_insightsql: MagicMock, mock_ai_events: MagicMock, _mock_get_team: MagicMock
    ) -> None:
        state = self._allowlisted_state(mock_execute_insightsql)
...
class TestUuidRegex(SimpleTestCase):
    ...
    def test_rejects_uppercase(self):
        # Our pattern is strict lowercase — matches the format Insights emits.
...
class TestAddCitation(SimpleTestCase):
    ...
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools._execute_insightsql")
    def test_trace_target_allows_evaluated_trace_with_empty_generation_id(self, mock_execute_insightsql: MagicMock) -> None:
        state = _trace_report_tool_state()
        mock_execute_insightsql.return_value = [["customer/trace:42", True, "matched", True, None]]
        _sample_eval_results_fn(state=state)
        mock_execute_insightsql.reset_mock()
        result = _add_citation_fn(
            state=state,
...
class TestExecuteChQueryWithRetry(SimpleTestCase):
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools.time.sleep")
    @patch(
        "insights.temporal.ai_observability.eval_reports.report_agent.tools.random.uniform",
    ...
    def test_reraises_after_exhausting_retries(self, mock_sleep: MagicMock) -> None:
        ...
        with self.assertRaises(DatastoreAtCapacity):
    ...
    @parameterized.expand(
        [
            ...
            ("cluster_memory_pressure", DatastoreQueryMemoryLimitExceeded()),
        ]
    )
    @patch("insights.temporal.ai_observability.eval_reports.report_agent.tools.time.sleep")
    ...
    @parameterized.expand(
        [
            ("query_timeout", DatastoreQueryTimeOut()),
