...
from insights.management.migration_analysis.analyzer import RiskAnalyzer
from insights.management.migration_analysis.models import RiskLevel
from insights.management.migration_analysis.policies import (
    AtomicFalsePolicy,
    ConcurrentIndexIdempotencyPolicy,
    HotTableAlterPolicy,
)
from insights.management.migration_analysis.utils import _model_name_for_table
from insights.migration_helpers import (
...
class TestRunSQLOperations:
    ...
    def test_run_sql_alter_table_drop_column_quoted_identifiers(self):
        ...
        op = create_mock_operation(
            migrations.RunSQL,
            sql='ALTER TABLE "insights_experiment" DROP COLUMN IF EXISTS "exposure_preaggregation_enabled";',
        )
...
class TestDropTableValidation:
    ...
    def test_drop_table_with_immediate_predecessor_state_removal(self):
        ...
        mock_migration.dependencies = [("insights", "0877_delete_named_query_from_state")]
        ...
        parent_migration.app_label = "insights"
        ...
        mock_loader.disk_migrations = {
            ("insights", "0877_delete_named_query_from_state"): parent_migration,
            ("insights", "0878_drop_named_query"): mock_migration,
        }
    ...
    def test_drop_table_with_gap_between_state_removal_and_drop(self):
        ...
        mock_migration.dependencies = [("insights", "0877_unrelated")]
        ...
        state_removal_migration.app_label = "insights"
        ...
        intermediate1.app_label = "insights"
        intermediate1.name = "0876_unrelated"
        intermediate1.operations = []
        intermediate1.dependencies = [("insights", "0875_delete_old_model_from_state")]

        intermediate2 = MagicMock()
        intermediate2.app_label = "insights"
        intermediate2.name = "0877_unrelated"
        intermediate2.operations = []
        intermediate2.dependencies = [("insights", "0876_unrelated")]

        # Create mock loader with full chain
        mock_loader = MagicMock()
        mock_loader.disk_migrations = {
            ("insights", "0875_delete_old_model_from_state"): state_removal_migration,
            ("insights", "0876_unrelated"): intermediate1,
            ("insights", "0877_unrelated"): intermediate2,
            ("insights", "0878_drop_old_model"): mock_migration,
        }
    ...
    def test_drop_table_without_prior_state_removal(self):
        ...
        mock_migration.dependencies = [("insights", "0877_other")]
        ...
        mock_loader = MagicMock()
        mock_loader.disk_migrations = {
            ("insights", "0877_other"): parent_migration,
            ("insights", "0878_drop_something"): mock_migration,
        }
    ...
    def test_drop_table_wrong_model_removed_from_state(self):
        ...
        mock_migration.dependencies = [("insights", "0877_delete_modela")]
        ...
        mock_loader = MagicMock()
        mock_loader.disk_migrations = {
            ("insights", "0877_delete_modela"): parent_migration,
            ("insights", "0878_drop_modelb"): mock_migration,
        }
    ...
    def test_drop_table_without_if_exists_even_if_staged(self):
        ...
        mock_migration.dependencies = [("insights", "0877_delete_model_from_state")]
        ...
        mock_loader = MagicMock()
        mock_loader.disk_migrations = {
            ("insights", "0877_delete_model_from_state"): parent_migration,
            ("insights", "0878_drop_model"): mock_migration,
        }
...
class TestCreateModelOperations:
    ...
    def test_create_model_with_autofield(self):
        ...
        mock_migration = MagicMock()
        mock_migration.app_label = "insights"  # Use Insights app to trigger policy
        mock_migration.name = "0001_test"
        mock_migration.operations = [
            create_mock_operation(
                migrations.CreateModel,
...
class TestCombinationRisks:
    ...
    def test_separate_database_and_state_with_alter_table_no_false_positive(self):
        ...
        db_op = create_mock_operation(
            migrations.RunSQL,
            sql='ALTER TABLE "insights_hogfunction" ADD COLUMN "batch_export_id" uuid NULL;',
        )
        ...
        migration_risk = self.analyzer.analyze_migration(
            mock_migration, "insights/migrations/0948_function_batch_export.py"
        )
...
class TestAtomicFalsePolicy:
    ...
    def test_insights_helpers_recognized_as_concurrent(self, op_name):
        """The Insights helpers must register as concurrent ops in
...
class TestConcurrentIndexIdempotencyPolicy:
    ...
    def _analyze(self, operations, atomic=False, app_label="insights"):
        mock_migration = MagicMock()
    ...
    def test_insights_helper_ops_pass_through(self, op_name):
        """The Insights migration helpers encode the idempotency guarantee internally
...
class TestHotTableAlterPolicy:
    ...
    traffic on insights_team in recurring waves (one per bin/migrate retry).
    """

    def setup_method(self):
        self.analyzer = RiskAnalyzer()

    def _analyze(self, operations, app_label="insights", name="0001_test"):
        mock_migration = MagicMock()
        mock_migration.atomic = True
        mock_migration.app_label = app_label
    ...
    def test_violation_names_the_acknowledgment_entry(self):
        ...
        assert any('"insights.1234_team_new_field"' in v for v in risk.policy_violations)

    def test_runsql_alter_on_hot_table_blocked(self):
        op = create_mock_operation(
            migrations.RunSQL, sql='ALTER TABLE "insights_team" ADD COLUMN "foo" timestamptz NULL;'
        )
        risk = self._analyze([op])
        assert any("ACCESS EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    @parameterized.expand(
        [
            ("if_exists", "ALTER TABLE IF EXISTS insights_team ADD COLUMN foo int NULL;"),
            ("if_exists_quoted", 'ALTER TABLE IF EXISTS "insights_team" ADD COLUMN foo int NULL;'),
            ("if_exists_only", "ALTER TABLE IF EXISTS ONLY insights_team ADD COLUMN foo int NULL;"),
            ("schema_qualified", "ALTER TABLE public.insights_team ADD COLUMN foo int NULL;"),
            ("schema_qualified_quoted", 'ALTER TABLE "public"."insights_team" ADD COLUMN foo int NULL;'),
            ("if_exists_schema_qualified", "ALTER TABLE IF EXISTS public.insights_team ADD COLUMN foo int NULL;"),
        ]
    )
    ...
    def test_runsql_validate_constraint_on_hot_table_not_flagged(self):
        op = create_mock_operation(migrations.RunSQL, sql='ALTER TABLE "insights_team" VALIDATE CONSTRAINT "some_fk";')
        risk = self._analyze([op])
        assert not any("ACCESS EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    def test_runsql_hot_table_only_in_comment_not_flagged(self):
        op = create_mock_operation(
            migrations.RunSQL,
            sql='-- cleanup after ALTER TABLE "insights_team"\nALTER TABLE "insights_dashboard" ADD COLUMN "foo" int NULL;',
        )
        risk = self._analyze([op])
        assert not any("ACCESS EXCLUSIVE" in v for v in risk.policy_violations)

    def test_runsql_alter_on_regular_table_not_flagged(self):
        op = create_mock_operation(migrations.RunSQL, sql='ALTER TABLE "insights_dashboard" ADD COLUMN "foo" int NULL;')
        risk = self._analyze([op])
        assert not any("ACCESS EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    def test_concurrent_index_on_hot_model_not_flagged(self, op_type):
        ...
        op.__class__.__name__ = op_type
        op.model_name = "team"
        op.sql = 'CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx" ON "insights_team" (c)'
        op.reverse_sql = 'DROP INDEX CONCURRENTLY IF EXISTS "idx"'
        risk = self._analyze([op])
        assert not any("ACCESS EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    def test_create_model_with_fk_to_hot_table_blocked(self):
        """The warehouse_sources.0034 case: a product CreateModel with a FK to insights.team."""
        op = create_mock_operation(
            migrations.CreateModel,
    ...
    def test_add_field_fk_to_hot_table_blocked(self):
        op = create_mock_operation(
            ...
            name="team",
            field=models.ForeignKey("insights.team", on_delete=models.CASCADE),
        )
        risk = self._analyze_product([op])
        assert any("SHARE ROW EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    def test_add_field_m2m_to_hot_table_blocked(self):
        ...
        op = create_mock_operation(
            ...
            model_name="datawarehousetable",
            name="teams",
            field=models.ManyToManyField("insights.team"),
        )
        risk = self._analyze_product([op])
        assert any("SHARE ROW EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    def test_add_field_m2m_to_hot_table_db_constraint_false_not_flagged(self):
        ...
        op = create_mock_operation(
            ...
            model_name="datawarehousetable",
            name="teams",
            field=models.ManyToManyField("insights.team", db_constraint=False),
        )
        risk = self._analyze_product([op])
        assert not any("SHARE ROW EXCLUSIVE" in v for v in risk.policy_violations)
    ...
    def test_add_field_m2m_with_explicit_through_not_flagged(self):
        ...
        op = create_mock_operation(
            ...
            model_name="datawarehousetable",
            name="teams",
            field=models.ManyToManyField("insights.team", through="some_product.TableTeam"),
        )
        risk = self._analyze_product([op])
        assert not any("SHARE ROW EXCLUSIVE" in v for v in risk.policy_violations)

    def test_create_model_with_swappable_user_fk_blocked(self):
        """settings.AUTH_USER_MODEL desugars to insights.user, a hot table."""
        op = create_mock_operation(
            migrations.CreateModel,
            name="WarehouseColumnAnnotation",
            fields=[
    ...
    def test_add_foreign_key_not_valid_on_hot_child_blocked(self):
        """The helper run against a hot child still ALTERs insights_team itself - gate it."""
    ...
    def test_add_foreign_key_not_valid_pointing_at_hot_parent_not_flagged(self):
        ...
        op = AddForeignKeyNotValid(
            model_name="mymodel", name="mymodel_team_fk", column="team_id", to_table="insights_team"
        )
        risk = self._analyze([op])
        assert not any("ACCESS EXCLUSIVE" in v for v in risk.policy_violations)
