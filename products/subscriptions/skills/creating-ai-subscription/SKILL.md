...
description: >
  Create a recurring AI-generated Insights report — schedule a free-text prompt to
...
A **subscription** delivers a Insights report to email or Slack on a recurring
...
- **`prompt`** — a recurring **AI-generated** report from a free-text prompt: an LLM
  plans and runs InsightsQL over the project's data and synthesizes a fresh markdown report
...
| `insights:subscriptions-test-delivery-create` | Send an immediate test delivery to its target(s)    |
| `insights:subscriptions-delete`               | Soft-delete it (stops all future deliveries)        |
| `insights:integrations-list`                  | Find a Slack `integration_id` (filter `kind=slack`) |
| `insights:integrations-channels-retrieve`     | List a Slack integration's channels (id + name)     |

## What you need before calling

The endpoint enforces three create-time gates and will return 400 if any fails:

1. **Insights Cloud, or `DEBUG=true`** — self-hosted production deployments are not
   eligible (the LLM call routes through a Insights-managed key).
...
3. **Prompt subscriptions enabled** for the organization — a Insights-managed rollout
   flag. If it's off, the org has not been granted access yet; tell the user to
   reach out to Insights to enable it (there is no self-serve toggle).
...
`subscription:write`: a prompt subscription runs LLM-generated InsightsQL over the project's
...
1. `insights:integrations-list` filtered by `kind=slack` → pick the Slack
   integration's `id`.
2. `insights:integrations-channels-retrieve` with that `id` → pick a channel; it
...
you sent. The first scheduled tick will run the planner → InsightsQL → synthesis
