...
describe('operationalMetrics', () => {
    // insights-js is mocked app-wide in jest.setup.ts, including insights.metrics.
    const metrics = insights.metrics as unknown as { count: jest.Mock; histogram: jest.Mock }

    beforeEach(() => {
        jest.clearAllMocks()
        ;(insights as { metrics?: unknown }).metrics = metrics
    })
    ...
    it('is a no-op when the SDK build has no metrics extension', () => {
        ;(insights as { metrics?: unknown }).metrics = undefined
