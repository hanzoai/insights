...
function makeRecord(
    ...
    const rawToolCall: ToolInvocation = {
        toolCallId: 'tc',
        rawServerName: overrides.rawServerName ?? 'insights',
    ...
    return {
        requestId: 'r',
        toolCallId: 'tc',
        toolName: overrides.toolName ?? 'mcp__insights__exec',
...
describe('toolPolicy', () => {
    describe('isInsightsExecTool', () => {
        it.each([
            ['mcp__insights__exec', true],
            ['mcp__insights_us__exec', true],
            ['mcp__insights__query', false],
            ['Bash', false],
        ])('%s → %s', (name, expected) => {
            expect(isInsightsExecTool(name)).toEqual(expected)
        })
    })

    describe('isInsightsDestructiveSubTool', () => {
        // Cases ported from Twig insights-exec-gate.test.ts.
        it.each([
            'experiment-update',
            'feature-flag-delete',
            'notebooks-destroy',
        ...
        ])('%s is destructive', (sub) => expect(isInsightsDestructiveSubTool(sub)).toBe(true))
        it.each([
            'experiment-get',
            'feature-flag-list',
            'experiment-create',
        ...
        ])('%s is not destructive', (sub) => expect(isInsightsDestructiveSubTool(sub)).toBe(false))
    })
