...
from insights.models.utils import RootTeamMixin, UUIDModel, UUIDTModel
...
class Survey(FileSystemSyncMixin, RootTeamMixin, UUIDTModel):
    ...
    is_publicly_shareable = deprecate_field(
        models.BooleanField(
            null=True,
            blank=True,
            help_text="Allow this survey to be accessed via public URL (https://app.hanzo.ai/surveys/[survey_id]) without authentication",
        ),
    )
...
def survey_changed(sender, instance: "Survey", **kwargs):
    from insights.tasks.surveys import update_team_surveys_cache
...
class SurveyResponseArchive(UUIDModel):
    ...
    class Meta:
        db_table = "insights_surveyresponsearchive"
        constraints = [
