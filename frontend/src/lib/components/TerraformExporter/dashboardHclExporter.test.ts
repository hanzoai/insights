...
describe('dashboardHclExporter test', () => {
    ...
    describe('includes dashboard layout resource', () => {
        it('generates layout resource when tiles are present', () => {
            ...
            expect(result.hcl).toContain(`resource "insights_dashboard" "with_tiles" {
  name = "With Tiles"
  tags = ["managed-by:terraform"]
}`)
            expect(result.hcl).toContain(`resource "insights_insight" "my_insight" {
  name = "My Insight"
  tags = ["managed-by:terraform"]
}`)
            expect(result.hcl).toContain(`resource "insights_dashboard_layout" "with_tiles" {
  dashboard_id = insights_dashboard.with_tiles.id
  tiles = [
    {
    ...
    },
    {
      ...
      color = "blue"
        ...
        it('does not generate layout resource when no tiles exist', () => {
            ...
            expect(result.hcl).not.toContain('insights_dashboard_layout')
        })
    })
})
