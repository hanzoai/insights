...
class TestCoerceBool:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("", None),
            ("maybe", None),
            (1, None),  # not in our contract; Datastore returns True/False or strings
        ],
    )
...
class TestFetchEvaluationEmbeddings:
    def test_filters_by_doc_type_and_job_suffix(self, mock_team):
        with patch("insights.temporal.ai_observability.evaluation_clustering.data.execute_insightsql_query") as mock_execute:
...
class TestFetchEvaluationMetadata:
    ...
    def test_populates_all_fields_when_generation_present(self, mock_team):
        ...
        eval_row = [
            ...
            "cfg-accuracy",
            "Accuracy",
            "true",  # Datastore bool properties come back as strings
