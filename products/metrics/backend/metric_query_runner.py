...
isn't going through InsightsQL `DataNode` caching, schema-gen or the data-viz
pipeline yet.
"""
...
from insights.insightsql.database.schema.metrics import INSIGHTSQL_MAX_BYTES_TO_READ_FOR_METRICS_USER_QUERIES
...
# the range on the Datastore cluster shared with the live logs/traces
...
_QUERY_SETTINGS = InsightsQLGlobalSettings(
    max_bytes_to_read=INSIGHTSQL_MAX_BYTES_TO_READ_FOR_METRICS_USER_QUERIES,
...
def attribute_field(name: str, *, scope: AttributeScope = "auto") -> ast.Expr:
    """Build the InsightsQL AST node that accesses a metric attribute by name.
    ...
    - ``"auto"`` (default) — try resource first, fall back to attribute if
      empty. Map lookups in Datastore return ``''`` for missing keys, not
    ...
    # arrayElement, not subscript: InsightsQL prints `field[...]` on a
    # StringJSONDatabaseField as JSONExtractRaw, which is illegal on the
    # physical Map columns. arrayElement passes through and is Datastore's
...
def _aggregation_expr(name: str) -> ast.Expr:
    """Build the InsightsQL AST for the supported aggregations.

    Kept as AST nodes (rather than string interpolation) so the
    `insightsql-no-fstring` semgrep rule doesn't have to special-case this
...
def _finite_or_none(value: float | None) -> float | None:
    """Datastore float aggregates can overflow to inf/-inf (or produce NaN);
...
def _filter_condition(filter: MetricFilter) -> ast.Expr:
    """One label predicate as a InsightsQL boolean expression.
    ...
    if filter.op in (FilterOp.REGEX, FilterOp.NOT_REGEX):
        # Pre-validate so a bad pattern is a 400, not a Datastore
...
class MetricQueryRunner:
    ...
    def run(self) -> list[dict[str, Any]]:
        ...
        response = execute_insightsql_query(
            ...
            query=query,
            team=self.team,
            workload=Workload.LOGS,  # metrics share the logs Datastore workload pool for now
    ...
    def _run_histogram_quantile(self) -> list[dict[str, Any]]:
        """Datastore sums the per-le distributions (temporality-aware,
    ...
    def _raise_on_truncation(self, results: list[Any]) -> None:
        """A full page means Datastore hit the row LIMIT and dropped the
    ...
    def _build_simple_query(self) -> ast.SelectQuery:
        ...
        # (see insights/insightsql/database/database.py).
        query = parse_select(
            """
                SELECT
