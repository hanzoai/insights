...
const cohortsCreate = (): ToolBase<typeof CohortsCreateSchema, WithInsightsUrl<Schemas.Cohort>> =>
    withUiApp('cohort', {
        name: 'cohorts-create',
...
const cohortsList = (): ToolBase<typeof CohortsListSchema, WithInsightsUrl<Schemas.PaginatedCohortList>> =>
            ...
            return await withInsightsUrl(
                context,
                {
                    ...
                    results: await Promise.all(
                        (filtered.results ?? []).map((item) => withInsightsUrl(context, item, `/cohorts/${item.id}`))
                    ),
                },
                '/cohorts'
            )
...
const cohortsPartialUpdate = (): ToolBase<typeof CohortsPartialUpdateSchema, WithInsightsUrl<Schemas.Cohort>> =>
    withUiApp('cohort', {
...
const cohortsRetrieve = (): ToolBase<typeof CohortsRetrieveSchema, WithInsightsUrl<Schemas.Cohort>> =>
    withUiApp('cohort', {
        ...
        handler: async (context: Context, params: z.infer<typeof CohortsRetrieveSchema>) => {
            ...
            return await withInsightsUrl(context, filtered, `/cohorts/${filtered.id}`)
        },
    })
