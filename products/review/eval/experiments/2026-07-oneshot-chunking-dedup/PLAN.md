...
1. Pre-flight (below). 2. `RUN_START_EPOCH=$(date +%s)` then
   ...
   (NO `--publish`).
2. Dump: `LABEL=ONESHOT-<n> RUN_SECONDS=<s> RUN_START_EPOCH=<epoch> OUT_DIR=products/review/eval/experiments/2026-07-oneshot-chunking-dedup/runs flox activate -- bash -c "DJANGO_SETTINGS_MODULE=insights.settings python manage.py shell -c \"exec(open('products/review/eval/scripts/dump_result.py').read())\""`
