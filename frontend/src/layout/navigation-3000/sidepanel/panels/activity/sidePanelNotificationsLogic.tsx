            ...
            startSSE: () => {
                ...
                cache.disposables.add(
                    () => {
                        ...
                        insights.capture('livestream_sse_startsse_called', {
                            reason,
                        ...
                        if (!values.realTimeNotificationsEnabled) {
                            insights.capture('livestream_sse_startsse_skipped', { reason: 'flag_disabled' })
                            return () => {}
                        }

                        const token = values.currentTeam?.live_events_token
                        if (!token) {
                            insights.capture('livestream_sse_startsse_skipped', { reason: 'no_token' })
                            return () => {}
                        }

                        const host = liveEventsHostOrigin()
                        if (!host) {
                            insights.capture('livestream_sse_startsse_skipped', { reason: 'no_host' })
                            return () => {}
                        }
                        ...
                        insights.capture('livestream_sse_connecting', { url, reason })
                        ...
                        void retryWithBackoff(
                                    ...
                                    },
                                    {
                                        ...
                                        onFirstMessage: () => {
                                            if (!cache.firstMessageLogged) {
                                                ...
                                                insights.capture('livestream_sse_first_message', { url })
                                            }
                                        },
                                        onError: (error) => {
                                            insights.capture('livestream_sse_error', {
                                                url,
                            ...
                            // TEMPORARY: livestream SSE lifecycle tracking.
                            insights.capture('livestream_sse_max_errors', {
                                url,
                            ...
                            cache.disposables.add(
                                () => {
                                    const onFocus = (): void => {
                                        insights.capture('livestream_sse_refocus_reconnect', { url })
                        ...
                        return () => {
                            ...
                            cache.nextStopReason = null
                            insights.capture('livestream_sse_stopped', {
                                reason: stopReason,
        ...
        legacyNotifications: [
            ...
            (importantChanges: ChangesResponse | null): HumanizedActivityLogItem[] => {
                try {
                    ...
                    const flagPayload = insights.getFeatureFlagResult('changelog-notification', {
                        send_event: false,
                    })?.payload
