...
const NODE_DB_URL = 'postgres://insights:insights@localhost:5432/test_cyclotron_node'
 ...
 *
 * Everything is real: real Kafka, real Datastore Kafka MV, real cyclotron-v2
 ...
 */
describe('CDP script invocation rerun e2e', () => {
    jest.setTimeout(60_000)

    let hub: Hub
    let kafkaProducer: KafkaProducerWrapper
    let team: Team
    let fnFetch: InsightsFunctionType
    ...
    beforeEach(async () => {
        ...
        // Ensure all topics exist (idempotently, without deleting) so the Datastore
            ...
            fetchPersonsByDistinctIds: jest.fn().mockResolvedValue([
                {
                    ...
                    team_id: team.id,
                    properties: { email: 'rerun-e2e@hanzo.ai' },
    ...
    it('produces a succeeded lifecycle row, then reruns the invocation via the wrapper-job pipeline', async () => {
        ...
        // Wait for the terminal 'succeeded' row to flow Kafka -> MV -> Datastore.
        ...
        expect(originalRows[0].function_kind).toBe('insights_function')
        ...
        const rerunJobId = await rerunManager.enqueue(team.id, 'insights_function', fnFetch.id, {
            filter: {
        ...
        // ── 6. The script function's fetch was called twice — once original, once rerun ─
    ...
    it('reruns successfully when person rehydration misses and the input reads person.properties', async () => {
        ...
        // distinct_id has no row in insights_persondistinctid (cookieless
        ...
        const windowStart = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()
        const windowEnd = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString()
        const rerunJobId = await rerunManager.enqueue(team.id, 'insights_function', personPropsFn.id, {
            filter: {
                window_start: windowStart,
                window_end: windowEnd,
                status: ['succeeded'],
    ...
    it('reruns a multi-action script flow by resuming, not restarting, so completed actions do not re-fire', async () => {
        // The script-flow counterpart to the insights_function reruns above, and the core
        // guard for #70792: a insights_function rerun re-executes the whole function
        // (fetch fires again, by design), but a script FLOW rerun must RESUME from
        ...
        // from the beforeEach insights_function that also matches this event.
        ...
        // The flow's function action runs a template, not inline script — insert the same
        ...
        // ── 4. Rerun invocation flows back through the real script-flow worker ─────────
