...
impl S3SourceConfig {
    ...
    async fn assume_role_credentials(
        ...
        let mut builder = aws_config::sts::AssumeRoleProvider::builder(role_arn)
            .session_name("insights-batch-import")
        ...
        let Some(intermediate_arn) = managed_migrations_role_arn else {
            return Err(Error::from(UserError::new(
                "The Insights import service is missing its cross-account role configuration, \
                 so it cannot use IAM role authentication. This is not a problem with your S3 \
                 or IAM setup. Please contact Insights support.",
            )));
        };
        ...
        let intermediate = aws_config::sts::AssumeRoleProvider::builder(intermediate_arn)
            ...
            .build()
            .await;
        // An unassumable intermediate role is a Insights deployment problem, so surface the
        ...
        intermediate.provide_credentials().await.map_err(|e| {
            Error::from(e)
                ...
                .context(UserError::new(
                    "Insights's import service could not authenticate with its own cross-account \
                     role, so it cannot use IAM role authentication. This is not a problem with \
                     your S3 or IAM setup. Please contact Insights support.",
                ))
        })?;
        ...
        provider.provide_credentials().await.user_error(format!(
            "Insights could not assume IAM role {role_arn}. Verify the role exists, its trust \
             policy allows Insights's import role, and the External ID matches the one shown in \
             the migration setup."
        ))?;
...
mod tests {
    ...
    fn test_s3_source_config_role_auth_deserializes() {
        let json = r#"{
            ...
            "prefix": "p",
            "region": "us-east-1",
            "role_arn": "arn:aws:iam::123456789012:role/insights-import",
            "external_id": "insights-us-some-team-uuid"
        }"#;
        let config: S3SourceConfig = serde_json::from_str(json).unwrap();
        assert_eq!(
            config.role_arn.as_deref(),
            Some("arn:aws:iam::123456789012:role/insights-import")
        );
        assert_eq!(
            config.external_id.as_deref(),
            Some("insights-us-some-team-uuid")
        );
    ...
    async fn test_build_s3_config_rejects_role_auth_without_managed_migrations_role() {
        ...
        let err = config
            ...
            .unwrap_err()
            .to_string();
        assert!(err.contains("contact Insights support"), "got: {err}");
    }
