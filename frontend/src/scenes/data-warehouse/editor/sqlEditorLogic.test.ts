...
describe('sqlEditorLogic', () => {
            ...
            post: {
                ...
                '/api/environments/:team_id/warehouse_saved_queries/': () => [
                    200,
                    {
                        ...
                        query: { kind: NodeKind.InsightsQLQuery, query: 'SELECT 1' },
                        is_materialized: false,
                        latest_history_id: null,
                        sync_frequency: null,
                        status: null,
    ...
    it('keeps configured filters when the filters placeholder is removed from the query text', () => {
        ...
        expect((logic.values.sourceQuery.source as InsightsQLQuery).filters).toEqual({
            filterTestAccounts: true,
        })
    })
    ...
    it('restores filters from the URL hash', async () => {
        const filters: InsightsQLFilters = {
            dateRange: {
                date_from: '-7d',
                date_to: null,
            },
    ...
    describe('open_insight URL parameter', () => {
            ...
            await expectLogic(logic)
                ...
                .toMatchValues({
                    ...
                    sourceQuery: partial({
                        ...
                        source: partial({
                            kind: NodeKind.InsightsQLQuery,
                            query: (MOCK_DATA_TABLE_INSIGHT_QUERY.source as InsightsQLQuery).query,
                        }),
                    }),
                })
        ...
        it('applies filters from the URL hash on top of the opened insight so view-mode filter edits can be saved', async () => {
            ...
            const filters: InsightsQLFilters = { dateRange: { date_from: '-30d', date_to: null } }
            ...
            await expectLogic(logic)
                ...
                .toMatchValues({
                    ...
                    sourceQuery: partial({
                        source: partial({
                            query: (MOCK_INSIGHT_QUERY.source as InsightsQLQuery).query,
            ...
            useMocks({
                get: {
                    '/api/projects/:team_id/data_catalog/metrics/my_metric/': [
                        ...
                        {
                            name: 'my_metric',
                            definition: { kind: NodeKind.InsightsQLQuery, query: 'SELECT count() FROM events' },
                        },
                    ],
                },
            })
    ...
    describe('Update view', () => {
        it('advances the saved baseline after updating so reverting to the original query re-enables Update view', async () => {
            ...
            logic.actions.updateViewSuccess({
                id: MOCK_VIEW.id,
                query: { kind: NodeKind.InsightsQLQuery, query: 'SELECT 2' },
                types: [],
            })
    ...
    describe('BI editor hash parameters', () => {
        ...
        it('offers every sidebar table while excluding hidden Insights tables', () => {
            const biLogic = biEditorLogic({ tabId: TAB_ID })
            biLogic.mount()

            databaseLogic.actions.loadDatabaseSuccess({
                tables: {
                    persons: { id: 'persons', name: 'persons', type: 'insights', fields: {} },
                    hidden_table: { id: 'hidden_table', name: 'hidden_table', type: 'insights', fields: {} },
                    events: { id: 'events', name: 'events', type: 'insights', fields: {} },
                    sessions: { id: 'sessions', name: 'sessions', type: 'insights', fields: {} },
                    groups: { id: 'groups', name: 'groups', type: 'insights', fields: {} },
    ...
    describe('source URL parameter', () => {
        ...
        it('ignores send raw query from hash for Insights warehouse', async () => {
            logic = sqlEditorLogic({
                tabId: TAB_ID,
                monaco: createMockMonaco(),
                editor: createMockEditor(),
        ...
        it('defaults to raw SQL mode for the managed warehouse connection', async () => {
            ...
            expect(logic.values.selectedConnectionSupportsInsightsQL).toEqual(true)
        ...
        it('forces raw SQL mode when the selected connection does not support InsightsQL', async () => {
            ...
            expect(logic.values.selectedConnectionSupportsInsightsQL).toEqual(false)
            expect(logic.values.sourceQuery.source.sendRawQuery).toEqual(true)
            expect(logic.values.sendRawQueryEnabled).toEqual(true)
            expect(String(router.values.hashParams.raw)).toEqual('1')
        })
        ...
        it("doesn't enable send raw query for Insights warehouse", async () => {
            logic = sqlEditorLogic({
                tabId: TAB_ID,
                monaco: createMockMonaco(),
                editor: createMockEditor(),
        ...
        it("doesn't pass sendRawQuery when running against Insights warehouse", async () => {
            const performQuerySpy = jest
                .spyOn(queryRunner, 'performQuery')
                .mockResolvedValue({ results: [], columns: [], types: [] } as never)
