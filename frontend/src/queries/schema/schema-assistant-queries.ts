...
export interface AssistantInsightsQLPropertyFilter {
    /**
...
> {
    ...
    /**
     * Custom InsightsQL expression for aggregation. Use when the predefined `math` types are not sufficient.
     * When set, `math` must be set to `insightsql`.
     *
     * Examples:
...
export interface AssistantTrendsGroupNode {
    ...
    /** Custom InsightsQL aggregation. When set, `math` must be `insightsql`. */
    math_insightsql?: string
}
...
export interface AssistantPathCleaningFilter {
     ...
     * Uses Datastore `replaceRegexpAll` replacement syntax — use `\\1` for capture group back-references.
     */
    alias: string
    /**
     * A Datastore regex pattern to match against path values. Matched paths will be replaced with the alias.
...
export interface AssistantPathsFilter {
     ...
     * `insightsql` - custom InsightsQL expression defined in `pathsInsightsQLExpression`. Path values come from evaluating the expression.
     * You can combine multiple types. If not specified, all events are included without type filtering.
     */
    includeEventTypes?: PathType[]
    /**
     * A InsightsQL expression to use as the path item. Required when `insightsql` is included in `includeEventTypes`.
     * For example, `properties.$current_url` to use the current URL as the path item.
     */
    pathsInsightsQLExpression?: string
    /**
    ...
    edgeLimit?: integer
    /**
     * Datastore regex-based rules to clean and normalize path values at the query level.
...
export interface AssistantDataVisualizationNode {
    ...
    /** InsightsQL query object that produces the rows to visualize. */
    source: Record<string, any>
    /**
