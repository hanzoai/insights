...
// This matches Button's height when its size small
...
interface VirtualizedOptionRowProps<T = string> {
    visibleOptions: InputSelectOption<T>[]
    ...
    _onFocus: () => void
    getInputLabel: (option: InputSelectOption<T>) => React.ReactNode
    getOptionIcon: (option: InputSelectOption<T>, isSelected: boolean) => JSX.Element | null | undefined
}
...
export type InputSelectAction = SideAction & Pick<ButtonPropsBase, 'children'>
...
export function InputSelect<T = string>({
    placeholder,
    title,
    options = [],
    value,
    ...
    const optionMaps = useMemo(() => {
        const valueToOption = new Map<T, InputSelectOption<T>>()
        const keyToOption = new Map<string, InputSelectOption<T>>()
    ...
    const allOptionsMap: Map<string, InputSelectOption<T>> = useMemo(() => {
        ...
        const allOptionsMap = new Map<string, InputSelectOption<T>>()
    ...
    const visibleOptions = useMemo(() => {
        ...
        let relevantOptions: InputSelectOption<T>[]
        ...
        return (
            <PopoverReferenceContext.Provider value={null}>
                ...
                {isClearButtonVisible && (
                    <div
                        className={clsx(
                            'grow flex flex-col items-end InputSelect__edit-button-wrapper',
                            size && `InputSelect__edit-button-wrapper--${size}`
                        )}
                    >
                        <Button
                            icon={<IconX />}
    ...
    const getInputLabel = (option: InputSelectOption<T>): React.ReactNode => {
    ...
    const getOptionIcon = (
        ...
        if (mode === 'multiple') {
            return <Checkbox checked={false} className="pointer-events-none" />
        }
    ...
    return (
                        ...
                        <>
                            {range(5).map((x) => (
                                ...
                                <div key={x} className="flex gap-2 items-center h-[33px] px-2">
                                    <Skeleton.Circle className="size-[18px]" />
        ...
        >
            <Input
                ...
                className={clsx(
                    '!h-auto leading-7 max-w-full w-full', // leading-7 means line height aligned with Snack height
...
function ValueSnacks<T = string>({
    ...
    const content = values.map((value) => {
        const option: InputSelectOption<T> = options.find((option) => option.key === value) ?? {
            key: value,
            label: value,
            labelComponent: null,
        }
