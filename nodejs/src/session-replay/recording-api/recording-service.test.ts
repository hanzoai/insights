...
describe('RecordingService', () => {
    ...
    let mockDatastore: jest.Mocked<DatastoreClient>
    ...
    beforeEach(() => {
        ...
        mockDatastore = {
            query: jest.fn(),
        } as unknown as jest.Mocked<DatastoreClient>
    ...
    describe('parseBlockUrl', () => {
        it.each([
            [
                's3://insights/session_recordings/30d/1000-abc123?range=bytes=0-5000',
        ...
        it.each([
            ['s3://insights/key?no-range', 'missing range'],
    ...
    describe('listBlocks', () => {
        function mockDatastoreResult(rows: any[]): void {
            mockDatastore.query.mockResolvedValue({
                json: jest.fn().mockResolvedValue(rows),
            } as any)
        }

        it('returns parsed blocks from Datastore result', async () => {
            mockDatastoreResult([
                {
                    start_time: '2024-01-01 00:00:00.000000',
                    block_first_timestamps: ['2024-01-01 00:00:00.000000', '2024-01-01 00:01:00.000000'],
        ...
        it('returns empty array when Datastore returns no rows', async () => {
            mockDatastoreResult([])
        ...
        it('throws when Datastore client is not initialized', async () => {
            ...
            await expect(serviceWithoutCH.listBlocks('sess-1', 1)).rejects.toThrow('Datastore client not initialized')
        })

        it('propagates Datastore query errors', async () => {
            mockDatastore.query.mockRejectedValue(new Error('Connection refused'))
    ...
    describe('deleteRecordings', () => {
        ...
        it('inserts activity log entries for newly deleted sessions', async () => {
            ...
            expect(mockPostgres.query).toHaveBeenCalledWith(
                expect.anything(),
                expect.stringContaining('insights_activitylog'),
