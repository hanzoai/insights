...
/// Public because the stateright model (`personinsights-stateright`) drives
...
impl PodHandle {
    ...
    async fn self_fence_locally(&self, drain_bound: Duration) -> Result<()> {
        ...
        gauge!("personinsights_coordination_partitions_held")
        ...
        match desired {
            DesiredState::Serving => {
                if !self.warmed_partitions.lock().await.contains_key(&partition) {
                    ...
                    histogram!("personinsights_coordination_partition_warm_ms", "trigger" => "restart")
            ...
            DesiredState::Drained { ack } => {
                ...
                if newly_fencing {
                    ...
                    histogram!("personinsights_coordination_partition_drain_ms")
                        .record(start.elapsed().as_secs_f64() * 1000.0);
                }
                if ack {
            ...
            DesiredState::Acquiring => {
                ...
                if !valid {
                    ...
                    let start = Instant::now();
                    self.handler.warm_partition(partition).await?;
                    histogram!("personinsights_coordination_partition_warm_ms", "trigger" => "handoff")
                        .record(start.elapsed().as_secs_f64() * 1000.0);
            ...
            DesiredState::Released => {
                ...
                if was_warmed || was_fenced {
                    ...
                    self.warmed_partitions.lock().await.remove(&partition);
                    self.fenced_partitions.lock().await.remove(&partition);
                    counter!("personinsights_coordination_partition_releases_total").increment(1);
            ...
            tokio::select! {
                ...
                _ = reconcile_tick.tick() => {
                    ...
                    if let Some(e) = tick_err {
                        ...
                        counter!(
                            "personinsights_coordination_reconcile_failures_total",
                            "component" => "pod"
                        )
                        .increment(1);
                        tracing::warn!(
...
async fn watch_own_registration(
    ...
    loop {
        ...
        for event in response.events() {
            ...
            if deleted_us {
                counter!("personinsights_coordination_registration_deleted_total").increment(1);
                tracing::error!(
                    pod = %pod_name,
