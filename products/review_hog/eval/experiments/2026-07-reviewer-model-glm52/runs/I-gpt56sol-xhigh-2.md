...
- **Validator:** - **Checked:** The sole production caller of `queue_inbox_pr_review` (`_start_stamp_review` in products/review_hog/backend/receivers.py) and the receiver `handle_task_run_saved` that invokes it; grepped the diff for every reference to confirm no other call path exists.
...
- **Validator:** - **Checked:** The PR-head `receivers.py` gating (`_resolve_assigned_reviewer` 159-207; both toggles checked on the one resolved reviewer at 114-139; `resolve_stamp_acting_reviewer` 144-156) and the PR's own tests in `products/review_hog/backend/tests/test_inbox_trigger.py`.
