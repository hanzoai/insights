...
use cohort_stream_processor::stage1::{
    datastore_timestamp_to_millis, AppliedOffsets, LeafTransition, Stage1State, StateVariant,
...
/// [`behavioral_leaf`], so both fan out from one ScriptVM eval but key to distinct leaf state.
...
fn c1_same_hash_two_windows_get_independent_state_and_deadlines() {
    ...
    assert_eq!(
        ...
        1,
        "one unique conditionHash → one ScriptVM eval that fans out",
    );
    for lsk in lsks {
    ...
    let event_ms = datastore_timestamp_to_millis(BASE_TS).unwrap();
    ...
    let later_ms = datastore_timestamp_to_millis(later_ts).unwrap();
...
fn mixed_present_corrupt_absent_read_set_stays_aligned() {
    ...
    let newer_ms = datastore_timestamp_to_millis(newer_ts).unwrap();
...
fn single_eviction_deadline_is_team_local_midnight_for_both_edge_persons() {
    ...
    let alice_ms = datastore_timestamp_to_millis(alice_ts).unwrap();
    let bob_ms = datastore_timestamp_to_millis(bob_ts).unwrap();
...
fn out_of_order_person_events_keep_the_latest_by_event_time() {
    ...
    let newest_ms = datastore_timestamp_to_millis(newest_ts).unwrap();
...
async fn worker_produces_changes_and_advances_offset() {
    ...
    assert_eq!(
        ...
        Some(Stage2State {
            in_cohort: true,
            last_evaluated_at_ms: datastore_timestamp_to_millis(BASE_TS).unwrap(),
        }),
...
fn daily_multiple_stores_eviction_deadline_at_oldest_bucket_day_boundary() {
    ...
    let event_day = day_idx_in_tz(datastore_timestamp_to_millis(ts).unwrap(), UTC);
    assert_eq!(
        window_start_day,
...
fn daily_multiple_buckets_by_team_timezone_day_not_utc_day() {
    ...
    let event_ms = datastore_timestamp_to_millis(ts).unwrap();
...
async fn daily_multiple_single_leaf_cohort_emits_entered_then_left_to_the_sink() {
    ...
    assert_eq!(
        ...
        Some(Stage2State {
            in_cohort: false,
            last_evaluated_at_ms: datastore_timestamp_to_millis("2026-05-28 10:00:00.000000")
                .unwrap(),
        }),
...
fn day_of(ts: &str) -> i32 {
    day_idx_in_tz(datastore_timestamp_to_millis(ts).unwrap(), UTC)
}
