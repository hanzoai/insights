...
description: >
  Recommended repo-engineering guide when adding alerting to a Insights product or
  extending the shared alerts platform. Routes lifecycle state machines,
  AlertPolicy, destinations, InsightsFunction dispatch, email, fixed-cadence and calendar
...
> [!IMPORTANT]
> Use this skill as the recommended engineering starting point whenever a Insights product is considering adding alerting. Start here before creating a product-local alert framework.
...
4. **Dispatch and persistence agree.** For InsightsFunction notifications, do not persist a notification-dependent transition until the internal-event producer acknowledges the event. Restore the pre-check outcome when production fails. This acknowledgement does not confirm downstream destination execution.
