...
//! process that computes cohort membership: ScriptVM configuration, timezone calendar-day bucketing,
//! `condition_hash` / leaf-state-key derivation, the filter catalog, and the metric surface.
//!
//! ## Load-bearing contracts
//!
//! 1. **Parity is frozen against the Node reference.** The ScriptVM execution config (stack/step
//!    ceilings, coercing comparisons), the tz calendar-day bucketing, and `condition_hash` handling
//!    reproduce the TS/Python/Datastore oracle exactly. Changing any of them silently diverges
...
pub use timestamp::datastore_timestamp_to_millis;
