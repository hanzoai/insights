...
from insights.insightsql.database.lazy_join_tags import DATA_WAREHOUSE
...
class ViewLinkSerializer(serializers.ModelSerializer, ViewLinkValidationMixin):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "source_table_key": {"help_text": "Column or InsightsQL expression on the source table used as the join key."},
            "joining_table_name": {"help_text": "Name of the table or view being joined onto the source table."},
            "joining_table_key": {"help_text": "Column or InsightsQL expression on the joining table used as the join key."},
            "field_name": {
...
class ViewLinkValidationSerializer(serializers.Serializer, ViewLinkValidationMixin):
    ...
    joining_table_key = serializers.CharField(
        max_length=255, required=True, help_text="Column or InsightsQL expression on the joining table used as the join key."
    )
    ...
    source_table_key = serializers.CharField(
        max_length=255, required=True, help_text="Column or InsightsQL expression on the source table used as the join key."
    )
...
class ViewLinkValidationResponseSerializer(serializers.Serializer):
    ...
    insightsql = serializers.CharField(allow_null=True, help_text="The InsightsQL statement used to validate the join.")
...
class ViewLinkValidationErrorSerializer(serializers.Serializer):
    ...
    insightsql = serializers.CharField(allow_null=True, help_text="The InsightsQL statement that failed to validate.")
...
def _join_match_stats(
    ...
    try:
        ...
        stats_response = execute_insightsql_query(
            query=stats_query,
            team=team,
            context=InsightsQLContext(database=database, user=user),
            settings=InsightsQLGlobalSettings(max_execution_time=MATCH_RATE_MAX_EXECUTION_TIME),
        )
