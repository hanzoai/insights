...
from insights.api.capture import (
...
from insights.settings.ingestion import CAPTURE_INTERNAL_URL, CAPTURE_V1_INTERNAL_ENDPOINT
...
class TestBuildV1Headers(SimpleTestCase):
    def test_all_required_headers_present(self) -> None:
        ...
        assert len(headers["Insights-Request-Id"]) == 36
        assert "T" in headers["Insights-Request-Timestamp"]

    def test_attempt_increments(self) -> None:
        h1 = _build_v1_headers("tok", attempt=1)
        h2 = _build_v1_headers("tok", attempt=3)
        assert h1["Insights-Attempt"] == "1"
        assert h2["Insights-Attempt"] == "3"

    def test_request_id_unique_per_call(self) -> None:
        h1 = _build_v1_headers("tok", attempt=1)
        h2 = _build_v1_headers("tok", attempt=1)
        assert h1["Insights-Request-Id"] != h2["Insights-Request-Id"]
...
class TestCaptureBatchInternal(SimpleTestCase):
    @patch("insights.api.capture.internal_requests_session")
    ...
    @patch("insights.api.capture.time.sleep")
    @patch("insights.api.capture.internal_requests_session")
    def test_auto_resubmit_retry_events(self, mock_session_fn: MagicMock, mock_sleep: MagicMock) -> None:
        ...
        assert second_headers["Insights-Attempt"] == "2"
        assert second_headers["Insights-Request-Id"] != spy.calls[0]["headers"]["Insights-Request-Id"]
    ...
    def test_headers_on_resubmit(self, mock_session_fn: MagicMock) -> None:
        ...
        with patch("insights.api.capture.time.sleep"):
            capture_batch_internal(events=events, token="tok", event_source="rh")

        h1 = spy.calls[0]["headers"]
        h2 = spy.calls[1]["headers"]
        assert h1["Insights-Attempt"] == "1"
        assert h2["Insights-Attempt"] == "2"
        assert h1["Insights-Request-Id"] != h2["Insights-Request-Id"]
        for h in (h1, h2):
            assert "Authorization" in h
            assert "Insights-Sdk-Info" in h
            assert "Insights-Request-Timestamp" in h
...
class TestBatchChunking(SimpleTestCase):
    @patch("insights.api.capture.CAPTURE_INTERNAL_BATCH_CHUNK_SIZE", 200)
    @patch("insights.api.capture.CAPTURE_INTERNAL_MAX_WORKERS", 8)
    @patch("insights.api.capture.internal_requests_session")
