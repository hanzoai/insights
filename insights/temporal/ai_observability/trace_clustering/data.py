...
from insights.temporal.ai_observability.trace_summarization.constants import EMBEDDING_DOCUMENT_ID_JOB_DELIMITER

from products.ai_observability.backend.summarization.models import SummarizationMode

logger = structlog.get_logger(__name__)

# Datastore max_execution_time for clustering queries (seconds).
# Default InsightsQL timeout is 60s which is too tight for the legacy unfiltered
...
def fetch_item_metrics(
    ...
    if is_generation:
        ...
        # Use ast.Field placeholder so InsightsQL resolves materialized columns for $ai_trace_id
        # (JSONExtractString would bypass materialized column optimization)
        item_ids_tuple = ast.Tuple(exprs=[ast.Constant(value=iid) for iid in item_ids])
        query = parse_select(
            """
