...
from products.cdp.backend.api.insights_function import (
    MAX_FN_CODE_SIZE_BYTES,
...
class TestInsightsFunctionAPIWithoutAvailableFeature(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_create_insights_function_works_for_free_template(self):
        response = self._create_slack_function()
        assert response.status_code == status.HTTP_201_CREATED, response.json()
        assert response.json()["created_by"]["id"] == self.user.id
        assert response.json()["script"] == template_slack.code
...
class TestInsightsFunctionAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_create_from_hidden_template_is_blocked(self, _name, is_staff):
        ...
        assert not InsightsFunction.objects.filter(template_id="template-hidden-dest").exists()
    ...
    def test_enabling_function_from_hidden_template_is_blocked(self, _name, initial_enabled, patch, expected):
        self._create_hidden_template()
        fn = InsightsFunction.objects.create(
            team=self.team,
    ...
    def test_creates_with_template_id(self, *args):
        ...
        assert response.json()["script"] == "fetch(inputs.url);"
    ...
    def test_secret_inputs_not_returned(self, *args):
        ...
        obj = InsightsFunction.objects.get(id=res.json()["id"])
        assert obj.inputs == {}
    ...
    def test_loads_status_when_enabled_and_available(self, *args):
        with patch("insights.plugins.plugin_server_api.internal_requests.get") as mock_get:
    ...
    def test_patches_status_on_enabled_update(self, *args):
        ...
        with patch("insights.plugins.plugin_server_api.internal_requests.get") as mock_get:
            with patch("insights.plugins.plugin_server_api.internal_requests.patch") as mock_patch:
    ...
    def test_list_with_filter_groups_filter(self, *args):
        ...
        insights_function_id_2 = response.json()["id"]
        assert response.status_code == status.HTTP_201_CREATED, response.json()
    ...
    def test_warehouse_source_webhook_excluded(self, *args):
        ...
        webhook_func = InsightsFunction.objects.create(
            team=self.team,
            name="Webhook Source",
    ...
    def test_create_typescript_destination_with_inputs(self, mock_transpile_fn):
        payload = {
            ...
            "script": "export function onLoad() { console.log(inputs.message); }",
            "type": "site_destination",
            "inputs_schema": [
    ...
    def test_list_insights_functions_ordered_by_execution_order_and_updated_at(self):
    ...
    def test_rerun_rejected_for_non_rerunnable_type(self, insights_function_type):
        ...
        with patch("products.cdp.backend.api.insights_function.rerun_hog_invocations") as mock_rerun:
            ...
            assert response.status_code == status.HTTP_400_BAD_REQUEST, response.json()
            assert insights_function_type in response.json()["detail"]
    ...
    def test_list_filter_by_search_handles_null_name_with_description_match(self):
        named_match = InsightsFunction.objects.create(
            team=self.team, name="Marketing webhook", type="destination", script="return event"
        )
        unnamed_match = InsightsFunction.objects.create(
            team=self.team,
            name=None,
    ...
    def test_list_filter_by_search_surfaces_match_type_on_full_serializer(self):
        matching = InsightsFunction.objects.create(
            team=self.team, name="alerts+ops@example.com", type="destination", script="return event"
        )
    ...
    def test_limits_transformation_functions_per_team(self):
        ...
        enabled_transformation = InsightsFunction.objects.filter(
        ...
        disabled_transformation = InsightsFunction.objects.filter(
    ...
    def test_insights_function_template_fk_set_on_create(self):
        """
        When creating a InsightsFunction with a template_id, the insights_function_template FK should be set to the latest template.
        When creating without a template_id, the FK should be null.
        """

        # Create a template in the DB
        _template = InsightsFunctionTemplate.objects.create(
    ...
    def test_insights_function_template_fk_validation_error_on_missing_template(self):
        """
        Creating a InsightsFunction with a template_id that does not exist in the DB should raise a validation error and not create the object.
        """
        ...
        assert InsightsFunction.objects.count() == initial_count, "No InsightsFunction should be created on error"
    ...
    def test_list_with_custom_limit_parameter(self):
        ...
        This verifies the frontend fix that adds limit=300 to InsightsFunction list requests.
        """
    ...
    @patch("products.cdp.backend.api.insights_function.hanzo_insights.feature_enabled", return_value=False)
...
class TestLogTransformationAPI(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    def setUp(self):
        super().setUp()
        patcher = patch("hanzo_insights.feature_enabled", return_value=True)
    ...
    @parameterized.expand(
        [
            ...
            ("insightsCapture", "insightsCapture({})\nreturn record"),
