...
def _sender_authenticated(request: HttpRequest, sender_email: str) -> bool:
    ...
    We require SPF pass + envelope-to-From domain alignment:
      - SPF pass means the sending IP is authorized by the envelope sender's
        domain DNS (X-Mailgun-Spf). An attacker can't pass SPF for hanzo.ai
        without controlling hanzo.ai's DNS records.
      - Domain alignment means the envelope sender (MAIL FROM) domain matches
        ...
        evil.com while forging From: teammate@hanzo.ai.
    ...
    An attacker signing with evil.com's key but forging From: teammate@hanzo.ai
...
def _resolve_team_member(email: str, team: Team) -> User | None:
    """Match a sender email to a Insights user within the team's organization."""
    if not email:
        return None
    membership = (
...
def email_inbound_handler(request: HttpRequest) -> HttpResponse:
    ...
    insights_user = _resolve_team_member(sender_email, team) if sender_authenticated else None
