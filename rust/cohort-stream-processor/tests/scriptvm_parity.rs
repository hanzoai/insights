//! ScriptVM regression goldens for the supported cohort bytecode surface. Each fixture's
//! `expected_result` is a committed oracle snapshot from the Python
//! `common/scriptvm/python/execute.py::execute_bytecode` runtime. The test asserts the Rust executor
...
//! `../common/scriptvm/__tests__/cohort_bytecode` is also loaded when present.
//!
//! Provenance note: the temporal fixtures (`toDateTime`/`toDate` ordering) are a Rust-vs-Datastore
//! oracle, not a Python one — the reference Python/TS VMs return `false` for Script temporal ordering,
//! so `expected_result` reflects the Datastore/instant-ordering answer the Rust VM deliberately
//! matches; see `rust/common/scriptvm/tests/datetime.rs`.

use std::fs;
use std::path::{Path, PathBuf};

use cohort_stream_processor::scriptvm::{evaluate_detailed, EvalOutcome};
use serde_json::Value;

/// ScriptVM `RETURN`. Fixtures store compiled bytecode without it; append it so the parity oracle runs
...
fn in_crate_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/scriptvm_parity")
}

fn all_fixtures() -> Vec<(PathBuf, Value)> {
    let mut fixtures = fixtures_in(&in_crate_dir());
    let corpus =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../common/scriptvm/__tests__/cohort_bytecode");
