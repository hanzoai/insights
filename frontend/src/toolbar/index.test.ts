...
// Keep the real toolbar insights-js instance (rendering depends on it) but spy on
// the exception capture helper so we can assert what does and doesn't get reported.
jest.mock('~/toolbar/toolbarInsightsJS', () => ({
    ...jest.requireActual('~/toolbar/toolbarInsightsJS'),
...
describe('Toolbar flag loading', () => {
    ...
    it.each([
        ...
        await import('./index')
        const { captureToolbarException } = await import('~/toolbar/toolbarInsightsJS')
