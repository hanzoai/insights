...
def ensure_waitlist_survey_for_feature(instance: EarlyAccessFeature) -> Optional[Survey]:
    ...
    survey, and record its id (+ first question id) on the feature's payload so insights-js
    consumers (hanzo.ai/roadmap and the in-app Feature Previews) know where to send the
