...
class HogTaggerConfigSerializer(serializers.Serializer):
    ...
    tags = TagDefinitionSerializer(
        ...
        required=False,
        default=list,
        help_text="Optional tag whitelist. Leave empty to allow any tag returned by the Script code.",
    )
...
class TaggerModelConfigurationWriteSerializer(serializers.Serializer):
    ...
    provider_key_id = serializers.UUIDField(
        ...
        help_text=(
            ...
            "ID returned by Insights, or omit/null when no provider key should be pinned."
        ),
    )
...
class TaggerBaseWriteSerializer(serializers.ModelSerializer):
    ...
    tagger_config = TaggerConfigField(
        help_text=(
            ...
            "For tagger_type 'script': {source, tags?}."
        )
    )
    ...
    def create(self, validated_data: dict[str, Any]) -> Tagger:
        ...
        # land together — otherwise a Script bytecode compile error on the tagger save
...
class TestHogTaggerRequestSerializer(serializers.Serializer):
    source = serializers.CharField(
        required=True,
        min_length=1,
        help_text="Script source code to test. Return a tag name string, a list of tag name strings, or null.",
...
class TestHogTaggerResultItemSerializer(serializers.Serializer):
    ...
    tags = serializers.ListField(child=serializers.CharField(), help_text="Tag names returned by the Script code.")
    reasoning = serializers.CharField(allow_blank=True, help_text="Text written to stdout by the Script code.")
...
class TaggerViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def test_hog(self, request: Request, **kwargs) -> Response:
        ...
        response = execute_insightsql_query(query=query, team=team, limit_context=None)
