...
def run_utm_audit(
    ...
    *,
    ...
    with UTM parameters in Insights to identify campaigns that are spending
...
# The audit is sync (it reads team config through the ORM and runs InsightsQL inline), but the
...
def get_utm_campaign_catalogue(
    ...
    with tags_context(product=Product.MARKETING_ANALYTICS, feature=Feature.HEALTH_CHECK, team_id=team.pk):
        result = execute_insightsql_query(
            ...
            user=user,
            placeholders={
                "date_from": date_range.date_from_as_insightsql(),
                "date_to": date_range.date_to_as_insightsql(),
            },
        )
