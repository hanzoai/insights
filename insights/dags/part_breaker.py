"""
Automated Datastore Part Breaker

Finds oversized parts across sharded Datastore tables and breaks them into smaller
...
from insights.dags.common import JobOwners, skip_if_already_running
from insights.dags.common.resources import PartBreakerDatastoreClusterResource
...
PART_BREAKER_RESOURCE_DEFS = {
    "cluster": PartBreakerDatastoreClusterResource(),
}
...
def _get_ssh_client(hostname: str) -> paramiko.SSHClient:
    ...
    Uses the SSH key from DATASTORE_SSH_PRIVATE_KEY env var (or falls back to
    ...
    if ssh_key:
        ...
        if pkey is None:
            raise dagster.Failure(
                description="Could not parse DATASTORE_SSH_PRIVATE_KEY as any supported key type (Ed25519, RSA, ECDSA)"
            )
        ...
        raise dagster.Failure(
            description="No SSH key configured. Set DATASTORE_SSH_PRIVATE_KEY (raw key) or "
...
def _ssh_exec(ssh: paramiko.SSHClient, cmd: str, sudo: bool = True, timeout: int = 3600) -> str:
    ...
    on datastore-owned files require sudo (enabled by default).
    """
    if sudo:
        cmd = f"sudo {cmd}"
...
def _get_database() -> str:
    """Return the Datastore database name from settings."""
    return settings.DATASTORE_DATABASE
...
def _check_disk_space(client: Client, required_bytes: int, multiplier: float) -> tuple[bool, int]:
    ...
    Datastore can write new parts to any disk in the storage policy, so we check
...
def _get_mutation_ids_for_part(client: Client, source_table: str, partition_id: str, part_name: str) -> set[str]:
    ...
    A mutation includes our part iff both:
      ...
      - that entry's block number is strictly greater than our part's min_block
        (Datastore: "Only parts that contain blocks with numbers less than this
...
def _get_disk_paths(client: Client) -> dict[str, str]:
    ...
    if not rows:
        raise dagster.Failure(description="Could not determine Datastore disk paths from system.disks")
...
def _get_table_store_path(client: Client, table: str, disk_path: str) -> str:
    ...
    Datastore stores table data under store/{prefix}/{uuid}/ rather than
...
def _run_insert_select(client: Client, source_table: str, target_table: str) -> str:
    ...
    part (the oversized one). Datastore will write it as multiple smaller parts.
...
def break_part(
    ...
    if config.dry_run:
        ...
        def _dry_run(client: Client) -> None:
            ...
            # Replication status
            cluster_name = settings.DATASTORE_CLUSTER
    ...
    def _process(client: Client) -> BreakResult:
        ...
        try:
            ...
            # Datastore uses store/{prefix}/{uuid}/ layout — the data/{db}/{table}/ paths
            ...
            _ssh_exec(ssh, f"chown -R datastore:datastore {staging_src_detached}{part.part_name}/")
            ...
            # Datastore handles block renumbering and replication internally,
            ...
            # (CH's tryLoadPartsToAttach — see Datastore/Datastore PR 97040).
...
def break_oversized_parts():
    """Find and break oversized parts across sharded Datastore tables.
