...
// Recursion guard for walking nested JSON/Script containers (input deserialization, output
...
pub struct ScriptVM<'a> {
    ...
    // The base program is None, but calling into e.g. script standard library functions involves changing the "module"
...
impl<'a> ScriptVM<'a> {
    ...
    pub fn step(&mut self) -> Result<StepOutcome, VmError> {
        ...
        match op {
            Operation::GetGlobal => {
                ...
                // function reference (referred to in other impls as a "closure") onto the stack - either a native one, or a script one
            ...
            Operation::Dict => {
                ...
                // mirrors the semantics of e.g. JavaScript or Python, which is what script is based around.
            ...
            Operation::Throw => {
                ...
                // as far as I can tell, a "script error" is just a HogValue::Object with some specific properties, so
                // I'll just check those exist. script is mostly duck-typed, based on the existing impls
            ...
            Operation::Closure => {
                ...
                // Only script-local callables are wrapped by the CLOSURE opcode; native (STL) callables
            ...
            Operation::CallLocal => {
                ...
                // For pure-script function calls (calls to "local" callables), we just push a null onto the stack for each missing argument,
    ...
    /// realtime-cohort evaluator) do two Script temporals compare by epoch seconds to match Datastore
    ...
    // Construct a script value from a Json object. If the json object would be heap allocated
    ...
    //
    // This is a function on the VM, rather than being standalone, because script values don't really
    ...
    fn json_to_hog_impl(&mut self, current: &JsonValue, depth: usize) -> Result<HogValue, VmError> {
        if depth > MAX_JSON_SERDE_DEPTH {
            return Err(VmError::OutOfResource(
                "json->script deserialization depth".to_string(),
            ));
        };
    ...
    fn hog_to_json_impl(&self, value: &HogValue, depth: usize) -> Result<JsonValue, VmError> {
        if depth > MAX_JSON_SERDE_DEPTH {
            return Err(VmError::OutOfResource(
                "script->json serialization depth".to_string(),
            ));
        };
...
pub fn sync_execute(context: &ExecutionContext, print_debug: bool) -> Result<JsonValue, VmFailure> {
    let fail = |e, vm: Option<&ScriptVM>, step: usize| VmFailure {
        error: e,
    ...
    let mut vm = ScriptVM::new(context).map_err(|e| fail(e, None, 0))?;
...
pub fn execute_resumable(context: &ExecutionContext) -> Result<Resumable, VmFailure> {
    let vm = ScriptVM::new(context).map_err(|e| failure(e, None, 0))?;
...
pub fn resume(
    ...
    let mut vm = ScriptVM::restore(context, state).map_err(|e| failure(e, None, 0))?;
    let value = vm
...
fn drive_resumable(mut vm: ScriptVM, context: &ExecutionContext) -> Result<Resumable, VmFailure> {
    let mut i = 0;
    while i < context.max_steps {
...
fn failure(error: VmError, vm: Option<&ScriptVM>, step: usize) -> VmFailure {
    VmFailure {
        error,
        ip: vm.map_or(0, |vm| vm.ip),
        stack: vm.map_or(Vec::new(), |vm| vm.stack.clone()),
