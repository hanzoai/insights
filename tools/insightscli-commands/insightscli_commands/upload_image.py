"""insightscli pr:upload-image: upload screenshots to the public Insights/pr-assets repo.
...
line for a PR description with one command. Running through insightscli means the usage is
tracked by insightscli's built-in command telemetry.
...
Insights org repo is available. The target repo is public by design: GitHub renders PR
...
from insightscli_commands import pr_assets
from insightscli_commands.github_auth import github_token
...
def upload_image(files: tuple[Path, ...], alt: str | None, yes: bool) -> None:
    """Upload screenshot(s) to the public Insights/pr-assets repo and print embeddable markdown.
    ...
    \b
        insightscli pr:upload-image screenshot.png
        insightscli pr:upload-image --alt "dashboard after fix" result.png
        insightscli pr:upload-image before.png after.png
