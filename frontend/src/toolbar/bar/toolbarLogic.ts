...
import type { MascotModeInterface } from '../../lib/components/MascotMode/types'
...
export interface toolbarLogicValues {
    ...
    mascotMode: MascotModeInterface | null // mascotModeLogic
    ...
    >
    getMascotActor: () => MascotActor | null
    mascotModeAvailable: boolean
...
export interface toolbarLogicActions {
    ...
    setMascotModeEnabled: (mascotModeEnabled: boolean) => {
        mascotModeEnabled: boolean
    }
...
export interface toolbarLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        getMascotActor: (mascotMode: MascotModeInterface | null) => () => MascotActor | null
        ...
        mascotModeAvailable: (cspBlocksNewFunction: boolean) => boolean
    }
}
...
export const toolbarLogic = kea<toolbarLogicType>([
    ...
    selectors({
        ...
        getMascotActor: [
            (s) => [s.mascotMode],
            (
                mascotMode: null | import('lib/components/MascotMode/types').MascotModeInterface
            ): (() => MascotActor | null) => {
                return () => {
    ...
    listeners(({ actions, values }) => ({
        ...
        setVisibleMenu: ({ visibleMenu }, _, __, previousState) => {
            ...
            if (visibleMenu !== 'none') {
                toolbarInsightsJS.capture('toolbar menu opened', { menu: visibleMenu, previous_menu: previousMenu })
            } else if (previousMenu !== 'none') {
                toolbarInsightsJS.capture('toolbar menu closed', { menu: previousMenu })
            }
        ...
        syncWithMascot: () => {
            ...
            if (values.isDragging) {
                ...
                player.setPosition({
                    x: values.position.x,
                    y: values.position.y + HEDGEFN_OFFSET,
                })
        ...
        if (isInIframe) {
            cache.disposables.add(() => {
                const iframeEventListener = (e: MessageEvent): void => {
                    // Only the Insights app at uiHost should be driving these actions.
