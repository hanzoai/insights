...
from insights.temporal.ai_observability.trace_summarization.models import SummarizeAndSaveInput, TextReprExpiredError
from insights.temporal.ai_observability.trace_summarization.summarize_and_save import summarize_and_save_activity
...
@patch(
    "insights.temporal.ai_observability.trace_summarization.summarize_and_save.Heartbeater",
    _noop_heartbeater,
)
class TestSummarizeAndSaveActivity:
    ...
    async def test_redis_key_missing_raises_text_repr_expired(self, mock_team):
        with (
            patch("insights.temporal.ai_observability.trace_summarization.summarize_and_save.get_async_client"),
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.load_text_repr"
            ) as mock_load,
        ):
    ...
    async def test_trace_summary_success(self, mock_team):
        ...
        with (
            ...
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.delete_text_repr"
            ) as mock_delete,
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.summarize"
            ) as mock_summarize,
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.create_event"
            ) as mock_create_event,
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.LLMTracesSummarizerEmbedder"
            ...
            # and document_id is scoped per (item, job) so overlapping jobs don't collapse on the RMT key
            ...
            assert embed_kwargs["document_id"] == f"{input_data.trace_id}::{input_data.job_id}"
    ...
    async def test_generation_summary_success(self, mock_team):
        ...
        with (
            ...
            patch("insights.temporal.ai_observability.trace_summarization.summarize_and_save.delete_text_repr"),
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.summarize"
            ) as mock_summarize,
            patch(
            ...
            # rendering stays the low-cardinality mode enum; ids live in metadata; document_id is
            # scoped per (generation, job) so overlapping jobs don't collapse on the RMT key
            embed_kwargs = mock_embedder_class.return_value.embed_document.call_args.kwargs
            assert embed_kwargs["rendering"] == input_data.mode
            assert embed_kwargs["metadata"] == {
                "batch_run_id": input_data.batch_run_id,
            ...
            assert embed_kwargs["document_id"] == f"{generation_id}::{input_data.job_id}"
    ...
    async def test_embedding_failure_captured_not_fatal(self, mock_team):
        ...
        with (
            ...
            patch("insights.temporal.ai_observability.trace_summarization.summarize_and_save.create_event"),
            patch(
                "insights.temporal.ai_observability.trace_summarization.summarize_and_save.LLMTracesSummarizerEmbedder"
            ) as mock_embedder_class,
        ):
