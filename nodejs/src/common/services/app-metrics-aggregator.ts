...
/**
 * One v2 app metric row, matching the Datastore `app_metrics2` schema.
...
export class AppMetricsAggregator {
    ...
    async flush(): Promise<void> {
        ...
        // No partition key — rows are re-aggregated by Datastore, ordering is
        // irrelevant, and round-robin distributes load evenly across partitions.
        const messages = drained.map((m) => ({
            value: Buffer.from(
                // safeDatastoreString strips lone Unicode surrogates that Datastore rejects —
                // identity fields are Insights-generated UUIDs today, but cheap insurance if
