...
from insights.temporal.ai.slack_app.activities.task_creation import _build_terminal_recovery_prompt
from insights.temporal.ai.slack_app.helpers import safe_react

from products.slack_app.backend.api import SlackUserContext
from products.slack_app.backend.models import SlackThreadTaskMapping


def _make_inputs(integration_id: int, slack_team_id: str = "T_SLACK") -> InsightsCodeSlackMentionWorkflowInputs:
...
class TestCreateInsightsCodeTaskForRepoActivity(TestCase):
    def setUp(self):
        self.Task = apps.get_model("tasks", "Task")
        self.TaskRun = apps.get_model("tasks", "TaskRun")
        self.org = Organization.objects.create(name="TestOrg")
    ...
    def test_no_repo_task_starts_with_pr_creation_enabled(self, mock_slack_cls, mock_execute_workflow):
        ...
        assert task.latest_run.state["slack_mention_workflow_id"] == "insights-code-mention-T_SLACK:C123:1234.5678"
        ...
        assert call_kwargs["insights_mcp_scopes"] == "full"
    ...
    def test_initial_task_uploads_slack_attachment_to_pending_prompt(
        ...
        with (
            patch("insights.temporal.ai.slack_app.attachments._download_slack_file", return_value=b"log bytes"),
            patch("insights.storage.object_storage.write") as mock_write,
    ...
    def test_persists_explicit_event_id_in_workflow_id(self, mock_slack_cls, mock_execute_workflow):
        ...
        task = self.Task.objects.get(team=self.team)
        assert task.latest_run.state["slack_mention_workflow_id"] == "insights-code-mention-T_SLACK:Ev01234567"
    ...
    # `insights/temporal/tests/ai/slack_app/activities/test_task_creation.py`. Tests
...
class TestForwardInsightsCodeFollowupActivity(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(email="alice@test.com")
        self.integration = Integration.objects.create(team=self.team, kind="slack", integration_id="T_SLACK", config={})
        # See note in TestCreateInsightsCodeTaskForRepoActivity.setUp: force the
    ...
    def test_terminal_run_resumes_same_task(self, mock_slack_cls, mock_execute_workflow):
        ...
        assert new_run.state.get("slack_mention_workflow_id") == "insights-code-mention-T_SLACK:C123:1234.5678"
    ...
    def test_terminal_run_resumes_with_slack_attachment(self, mock_slack_cls, mock_execute_workflow) -> None:
        ...
        with (
            patch("insights.temporal.ai.slack_app.attachments._download_slack_file", return_value=b"resume data"),
    ...
    def test_cross_user_followup_authorized_prefixes_actor_name(self, mock_slack_cls, mock_resolve, mock_signal):
        # A second user in the same Insights org and team should be allowed to chip in
        # on the thread; their message is queued under their own Insights identity
    ...
    def test_cross_user_followup_falls_back_to_email_when_no_full_name(self, mock_slack_cls, mock_resolve, mock_signal):
        ...
        inputs = _make_inputs(self.integration.id)
        forward_insights_code_followup_activity(inputs, "C123", "1234.5678", "U_BOB", "<@BOT> ping", "1234.5679")
    ...
    def test_cross_user_followup_unmapped_user_delegates_feedback_to_resolver(self, mock_slack_cls, mock_resolve):
        # A second Slack user who can't be resolved to a Insights member of this team
    ...
    def test_successful_forwarding(self, mock_slack_cls, mock_signal):
        ...
        # is not swapped to :mascot: until the task genuinely completes.
        mock_slack_instance.client.reactions_add.assert_called_once_with(
            channel="C123", timestamp="1234.5679", name="eyes"
        )
    ...
    def test_attachment_only_followup_uploads_and_forwards_to_sandbox(self) -> None:
        ...
        with (
            patch("insights.models.integration.SlackIntegration") as mock_slack_cls,
            ...
            result = forward_insights_code_followup_activity(inputs, "C123", "1234.5678", "U_ALICE", "", "1234.5679")
            # Temporal retries re-run the whole activity body; the re-upload must
            # upsert the same manifest entry, not append a duplicate.
            forward_insights_code_followup_activity(inputs, "C123", "1234.5678", "U_ALICE", "", "1234.5679")
...
class TestEventLevelDedupe(TestCase):
    ...
    def test_same_event_id_produces_same_workflow_id(self):
        ...
        wf_id_1 = f"insights-code-mention-{slack_team_id}:{event_id_or_fallback}"
        wf_id_2 = f"insights-code-mention-{slack_team_id}:{event_id_or_fallback}"
        assert wf_id_1 == wf_id_2

    def test_different_event_ids_produce_different_workflow_ids(self):
        slack_team_id = "T_SLACK"
        wf_id_1 = f"insights-code-mention-{slack_team_id}:Ev111"
        wf_id_2 = f"insights-code-mention-{slack_team_id}:Ev222"
        assert wf_id_1 != wf_id_2
    ...
    def test_fallback_uses_channel_and_ts(self):
        ...
        wf_id = f"insights-code-mention-{slack_team_id}:{event_id_or_fallback}"
        assert wf_id == "insights-code-mention-T_SLACK:C123:1234.5678"
