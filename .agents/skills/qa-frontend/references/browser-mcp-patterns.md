...
- 500s from invocations / script flow paths when the `insights-node` (CDP) or `temporal-worker` process is down
...
Then navigate to the target route (a navigation reloads kea state). `<html>` carries a server-rendered `data-boot-theme` attribute (check `document.documentElement.dataset.bootTheme` for the boot state), but a runtime toggle does not update it - verify the toggle took effect by reading the computed background color: the kea state drives CSS variables directly, and Insights sets no runtime `dark` class or `data-theme`/`data-color-mode` attribute.
...
- **Datastore** for events, person properties, session recordings, LLM spans, etc. Prefer the existing factory utilities under `insights/test/` and `insights/datastore/`; only drop to raw `INSERT INTO ... VALUES (...)` when no factory covers the shape you need.
...
insights.featureFlags.overrideFeatureFlags({ flags: { 'my-flag-key': true } })

// Set a multivariate flag to a specific variant
insights.featureFlags.overrideFeatureFlags({ flags: { 'my-flag-key': 'variant-name' } })

// Clear all overrides
insights.featureFlags.overrideFeatureFlags(false)
```
...
Prefer the Insights workspace's existing browser tooling for screenshots: capture frames through the active browser MCP/tooling or the repo's existing `@playwright/test` dependency. Do not add screenshot, image, or video packages to `package.json`.
