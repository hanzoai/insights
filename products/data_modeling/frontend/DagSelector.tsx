...
export function openCreateDagDialog({
    ...
    Dialog.openForm({
        ...
        content: (
            <>
                <Field name="dagName" label="Name">
                    <Input placeholder="Enter a DAG name" autoFocus />
                </Field>
                <Field name="dagDescription" label="Description" className="mt-2">
                    <Input placeholder="Optional description" />
                </Field>
                <Field name="syncFrequency" label="Sync frequency" className="mt-2">
                    {({ value, onChange }) => (
                        <Select value={value} onChange={onChange} options={SYNC_FREQUENCY_OPTIONS} fullWidth />
                    )}
                </Field>
            </>
        ),
