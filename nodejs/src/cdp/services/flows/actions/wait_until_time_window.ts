...
type Action = Extract<FlowAction, { type: 'wait_until_time_window' }>
...
export class WaitUntilTimeWindowHandler implements ActionHandler {
    execute({
        invocation,
        action,
    }: ActionHandlerOptions<Extract<FlowAction, { type: 'wait_until_time_window' }>>): ActionHandlerResult {
...
export const getWaitUntilTime = (
    action: Extract<FlowAction, { type: 'wait_until_time_window' }>,
