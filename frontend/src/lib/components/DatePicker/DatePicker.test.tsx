...
jest.mock('@hanzo/quill', () => {
    const react = require('react')
    return {
        ...jest.requireActual('@hanzo/quill'),
        DatePicker: ({
            onApply,
            onCancel,
            onIncludeTimeChange,
...
describe('DatePicker', () => {
    ...
    it.each<[string, Partial<DatePickerProps>, string]>([
        // No explicit type must keep CalendarSelectInput's `secondary` trigger default rather than
        // clobbering it with `type: undefined` (which would fall back to Button's tertiary default).
        ['no type keeps the secondary default', {}, 'Button--secondary'],
        ['type primary maps through', { type: 'primary' }, 'Button--primary'],
        ['type tertiary maps through', { type: 'tertiary' }, 'Button--tertiary'],
        ['size small maps through', { size: 'small' }, 'Button--small'],
        ['custom className forwards through', { className: 'bg-bg-light' }, 'bg-bg-light'],
    ])('maps trigger %s onto the UI button', (_name, props, expectedClass) => {
    ...
    describe('when the QUILL_DATE_PICKER flag is enabled', () => {
        ...
        it.each<[string, Partial<DatePickerProps>]>([
            ...
            ['months', { months: 2 }],
        ])('falls back to UI when %s is requested', (_name, props) => {
            const { container } = renderDatePicker(dayjs('2023-01-15'), props)
