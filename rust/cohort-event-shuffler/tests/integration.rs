...
const INPUT_TOPIC: &str = "datastore_events_json";
...
fn datastore_event(uuid: Uuid, spec: &InputSpec) -> DatastoreEvent {
    DatastoreEvent {
        uuid,
...
async fn shuffler_forwards_only_gated_events_with_stable_partitioning() {
    ...
    for (index, spec) in inputs.iter().enumerate() {
        ...
        let payload = serde_json::to_string(&datastore_event(uuid, spec)).unwrap();
