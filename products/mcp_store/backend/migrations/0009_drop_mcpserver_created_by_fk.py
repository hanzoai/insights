...
class Migration(migrations.Migration):
    """Drop the orphan ``mcp_store_mcpserver.created_by_id`` FK to ``insights_user``.
    ...
    a foreign key from ``created_by_id`` to ``insights_user`` (inherited from
    ``CreatedMetaFields``), which breaks test teardown: Postgres refuses to
    ``TRUNCATE insights_user`` without ``CASCADE`` while any other table
