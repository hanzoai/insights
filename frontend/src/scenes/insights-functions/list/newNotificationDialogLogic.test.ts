...
describe('newNotificationDialogLogic', () => {
    ...
    beforeEach(() => {
        ...
        initKeaTests()
        jest.spyOn(api.insightsFunctions, 'getTemplate').mockResolvedValue({
            id: 'template-slack',
            code: 'print(1)',
            inputs_schema: [],
            icon_url: null,
        } as any)
        createSpy = jest.spyOn(api.insightsFunctions, 'create').mockResolvedValue({ id: 'created' } as any)
    ...
    it('creates the MCP failure alert deduped per failing tool', async () => {
        ...
        // property. ScriptMaskerService skips masking when the hash evaluates falsy, so the expression
