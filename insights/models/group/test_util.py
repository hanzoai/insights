...
from insights.models.group.util import (
    create_group,
    get_group_by_key,
...
# `require_personinsights_client` resolves the client via `get_personinsights_client`, so patching that
# controls whether a client is available; `personinsights_call` runs for real (its metrics and
...
_CONVERTER_PATCH = "insights.personinsights_client.converters.proto_group_to_model"
...
class TestCreateGroup(SimpleTestCase):
    ...
    @patch("insights.models.group.util.raw_create_group_ch")
    @patch(_CONVERTER_PATCH)
    @patch(_CLIENT_PATCH)
    def test_builds_request_converts_response_and_writes_datastore(self, mock_get_client, mock_convert, mock_ch):
    ...
    @patch("insights.models.group.util.raw_create_group_ch")
    @patch(_CLIENT_PATCH)
    def test_datastore_write_happens_even_when_personinsights_fails(self, mock_get_client, mock_ch):
        mock_client = MagicMock()
