...
// already certified the matches, so re-applying the filter would force Datastore to re-read
...
async function runTwoPhase(intent: EventsQuery): Promise<EventsQueryResponse> {
    ...
    const tupleList = phaseOneResults.map(([u, t]) => `('${u}', ${formatDatastoreUtcDateTime64(t)})`).join(', ')
...
function formatDatastoreUtcDateTime64(timestamp: string): string {
