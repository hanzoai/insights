...
from insights.schema import ReleaseStatus, SourceFieldOauthAccountSelectConfig, SourceFieldOauthConfig
...
class TestBingAdsSource:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Insights's own app secret is invalid/expired — internal config, not customer-actionable.
            (
                "AADSTS7000215",
    ...
    def test_aadsts7000215_is_internal_config_not_customer_actionable(self):
        # AADSTS7000215 means Insights's own app secret is invalid/expired. The message also contains
        ...
        # keeping the misleading toast off an error only a Insights config change can fix.
        non_retryable_errors = self.source.get_non_retryable_errors()
        keys = list(non_retryable_errors.keys())
