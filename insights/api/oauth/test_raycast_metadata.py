...
from insights.api.oauth.cimd import fetch_and_upsert_cimd_application, get_application_by_client_id
from insights.api.oauth.raycast_metadata import RAYCAST_SCOPES
from insights.scopes import UNPRIVILEGED_SCOPES
...
@override_settings(SITE_URL="https://us.hanzo.ai")
class TestRaycastClientMetadataView(SimpleTestCase):
    def test_returns_valid_cimd_metadata(self):
        ...
        data = res.json()
        assert data["client_name"] == "Raycast extension for Insights"
        assert data["redirect_uris"] == [
            "https://raycast.com/redirect?packageName=Extension",
            "https://raycast.com/redirect?packageName=insights",
        ]
        assert data["grant_types"] == ["authorization_code"]
        assert data["response_types"] == ["code"]
        assert data["token_endpoint_auth_method"] == "none"
        assert data["com.insights"]["scopes"] == RAYCAST_SCOPES

    @parameterized.expand(
        [
            ("https://us.hanzo.ai", "https://us.hanzo.ai/api/oauth/raycast/client-metadata"),
            ("https://eu.hanzo.ai", "https://eu.hanzo.ai/api/oauth/raycast/client-metadata"),
        ]
    )
...
class TestRaycastClientMetadataRegistration(APIBaseTest):
    ...
    def test_document_registers_through_cimd(self, mock_get, _url_mock):
        ...
        assert app.name == "Raycast extension for Insights"
