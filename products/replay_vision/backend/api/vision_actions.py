...
# Each delivery target provisions one enabled InsightsFunction that POSTs to its destination on every run,
...
class VisionActionSerializer(serializers.ModelSerializer):
    ...
    def validate_delivery_config(self, value: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        # Cap the list: each target provisions an enabled InsightsFunction that POSTs on every run, so an
    ...
    def _demote_existing_digest(self, scanner: ReplayScanner) -> None:
        ...
        # _validate_scanner_access; authorize the same way here so promoting can't be a second path to
