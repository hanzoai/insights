...
UPSERT_ACCOUNT_TOOL_DESCRIPTION = dedent("""
    ...
    An account represents a customer organization. Its `external_id` must be the id Insights uses to
    ...
    any definition the team has created), keyed by definition name: the value is the Insights user id,
    ...
    # Examples
    - "Add an account for Acme Corp whose Insights group key is acme-123": action=create, name="Acme Corp", external_id="acme-123"
...
class CreateAccountAction(BaseModel):
    ...
    external_id: str | None = Field(
        default=None,
        description=(
            "The id Insights uses to identify this organization in group analytics (its group key). "
    ...
    relationships: dict[str, int | None] | None = Field(
        ...
        description=(
            ...
            "Insights user id to assign, or null to end the current assignment."
        ),
    )
...
class UpdateAccountAction(BaseModel):
    ...
    external_id: str | None = Field(
        default=None,
        description="New external id — the organization's group key in Insights, not a CRM/billing id.",
    )
    ...
    relationships: dict[str, int | None] | None = Field(
        ...
        description=(
            "Relationship assignments keyed by definition name (e.g. 'CSM'); each value is the "
            "Insights user id to assign, or null to end the current assignment. Only the named "
