...
use personinsights_common::grpc::{code_as_str, NON_STATUS};
...
pub fn record_write_ok(lane: &'static str, elapsed: Duration) {
    counter!(
        "personinsights_traffic_writes_total",
        "lane" => lane,
        "outcome" => "ok",
        "reason" => "ok",
    )
    .increment(1);
    histogram!("personinsights_traffic_write_duration_ms", "lane" => lane).record(millis(elapsed));
}
...
pub fn record_read_ok(lane: &'static str, elapsed: Duration) {
    counter!(
        "personinsights_traffic_reads_total",
        "lane" => lane,
        "outcome" => "ok",
        "reason" => "ok",
    )
    .increment(1);
    histogram!("personinsights_traffic_read_duration_ms", "lane" => lane).record(millis(elapsed));
}
...
pub fn record_violations(epoch: u64, violations: &[ConsistencyViolation]) {
    for violation in violations {
        ...
        counter!("personinsights_traffic_violations_total", "kind" => kind).increment(1);
        tracing::error!(
            epoch,
            kind,
