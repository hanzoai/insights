...
from insights.models.github_integration_base import GitHubIntegrationError
...
def _post_commit_status(
    ...
    run is posted to a separate "Insights Visual Review / {run_type} (partial)"
    context rather than the gating "Insights Visual Review / {run_type}" one.
    ...
    context = f"Insights Visual Review / {run.run_type}"
...
def _post_review_prompt_comment(run: Run, repo: Repo) -> None:
    ...
    comment_body = (
        ...
        f"[Review and approve in Insights Visual Review]({run_url})\n\n"
...
def _run_url(run: Run, repo: Repo) -> str:
    """Link to the run page in Insights."""
...
def _build_snapshot_image_tables(run: Run, repo: Repo) -> str:
    ...
    anything beyond links back to Insights. Returns "" when no image could be
    ...
    if shown < total:
        sections.append(f"…and {total - shown} more — [view all in Insights]({_run_url(run, repo)}).")
...
def _build_approval_comment_body(run: Run, repo: Repo, approver: _Approver | None, add_images: bool = False) -> str:
    ...
    sections = [
        ...
        f"[View this run in Insights]({_run_url(run, repo)})",
    ]
    if summary:
