...
class TestInsightsAIRunState(unittest.TestCase):
    def test_validate_accepts_wire_alias(self) -> None:
        state = InsightsAIRunState.model_validate({"systemPrompt": SYSTEM_PROMPT})
        assert state.system_prompt == SYSTEM_PROMPT

    def test_dump_emits_wire_alias(self) -> None:
        state = InsightsAIRunState(system_prompt=SYSTEM_PROMPT)
    ...
    def test_parent_extra_allow_config_survives_subclassing(self) -> None:
        state = InsightsAIRunState.model_validate({"systemPrompt": SYSTEM_PROMPT, "sandbox_quirk": True})
    ...
    def test_base_run_state_fields_still_parse(self) -> None:
        state = InsightsAIRunState.model_validate(
