...
from insights.ph_client import PH_EU_API_KEY, PH_EU_HOST, PH_US_API_KEY, PH_US_HOST
...
def _get_sandbox_callback_url() -> str:
    """URL that sandboxes use to call back to Insights (OAuth introspect, bridge queries).
...
def _get_otel_logs_config(callback_url: str) -> tuple[str, str]:
    ...
    Mirrors insights.ph_client.get_regional_ph_client. On cloud, logs ship to
...
def _build_sandbox_config(app: StreamlitApp, version: StreamlitAppVersion) -> SandboxConfig:
    ...
    otel_resource_attrs = ",".join(
        [
            ...
            f"insights.team_id={app.team_id}",
            f"insights.app_id={app.id}",
            f"insights.app_short_id={app.short_id}",
        ]
    )
...
class AppRuntimeService:
    ...
    def start_app(self, app: StreamlitApp, zip_content: bytes | None = None) -> StreamlitAppSandbox:
        ...
        # The bridge runs the app's InsightsQL as the author of the code that is about to
