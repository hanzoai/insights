from insights.test.base import BaseTest, DatastoreTestMixin, cleanup_materialized_columns, materialized
...
class TestRestrictPropertiesInInsightsQL(BaseTest):
    """Integration tests for property-level access control in InsightsQL query compilation."""
    ...
    def _compile_select_with_values(self, query: str, user=None) -> tuple[str, dict]:
        ...
        sql, _ = prepare_and_print_ast(node, context=context, dialect="datastore")
    ...
    def test_person_property_access_is_consistent_across_poe_modes(
        ...
        context = InsightsQLContext(
            ...
            user=self.user,
            enable_select_queries=True,
            modifiers=InsightsQLQueryModifiers(personsOnEventsMode=persons_on_events_mode),
        )
...
class TestRestrictedPropertyWithMaterializedColumn(DatastoreTestMixin, BaseTest):
    ...
    def _compile_select(self, query: str) -> str:
        ...
        sql, _ = prepare_and_print_ast(parse_select(query), context=context, dialect="datastore")
        return sql
