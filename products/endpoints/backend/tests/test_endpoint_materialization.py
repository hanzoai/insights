...
from insights.schema import RetentionQuery
...
from products.endpoints.backend.materialization_transforms import build_endpoint_insightsql
...
class TestEndpointMaterialization(DatastoreTestMixin, APIBaseTest):
    ...
    def test_materialization_status_endpoint(self):
        ...
        response = self.client.get(f"/v1/environments/{self.team.id}/endpoints/{endpoint.name}/materialization_status/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_data = response.json()
    ...
    def test_materialization_replaces_breakdown_sentinels_in_insightsql(self):
        ...
        insightsql_text = saved_query.query.get("query", "")
        self.assertNotIn(BREAKDOWN_NULL_STRING_LABEL, insightsql_text)
        self.assertNotIn(BREAKDOWN_OTHER_STRING_LABEL, insightsql_text)
    ...
    def test_materialized_relative_date_range_advances_on_refresh(self):
        ...
        with freeze_time("2026-04-30T12:00:00Z"):
            ...
            saved_query.refresh_from_db()
            insightsql = saved_query.query["query"]
            self.assertIn("2026-04-30", insightsql)
            self.assertNotIn("2026-04-20", insightsql)
    ...
    def test_optional_breakdown_materialized_allows_missing_variable(self):
        ...
        the resulting InsightsQL should NOT include a $browser WHERE filter (aggregate across all values)."""
        endpoint = self._create_materialized_trends_endpoint(
    ...
    def test_build_endpoint_insightsql_performs_no_db_writes(self):
        _create_event(team=self.team, event="$pageview", distinct_id="u1")
        flush_persons_and_events()
        ...
        with CaptureQueriesContext(connection) as ctx:
            build_endpoint_insightsql(insight_query, self.team)
        ...
        self.assertEqual(writes, [], f"build_endpoint_insightsql wrote to Postgres: {writes}")
    ...
    def test_deactivating_one_version_keeps_current_version_materialization(self):
        ...
        response = self.client.patch(
            f"/v1/environments/{self.team.id}/endpoints/{endpoint.name}/",
            {"query": {"kind": "InsightsQLQuery", "query": "SELECT event FROM events LIMIT 5"}},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.json())
    ...
    def test_enable_materialization_does_not_hijack_user_saved_query(self):
        ...
        user_query = {"kind": "InsightsQLQuery", "query": "SELECT 42 AS answer"}
