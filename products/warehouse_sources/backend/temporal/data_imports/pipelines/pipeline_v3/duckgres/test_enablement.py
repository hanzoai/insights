...
def _patch_all_rows(rows):
    return patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=rows)


@pytest.mark.django_db
@patch.object(enablement, "is_dev_mode", return_value=False)
@patch.object(enablement.hanzo_insights, "feature_enabled")
...
def test_is_duckgres_sink_team_member_reads_the_control_plane() -> None:
    ...
    with patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=[_cp_row(member)]):
        assert is_duckgres_sink_team_member(member.id) is True
        assert is_duckgres_sink_team_member(non_member.id) is False

    cp_teams.clear_cache()
    with patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=None):
        with pytest.raises(RuntimeError):
