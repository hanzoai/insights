...
Owns the entire error-tracking surface for a per-case team:
  ...
  * Wipes any pre-existing ``ErrorTrackingIssue`` rows so the Hedgebox
    ...
    sandboxed-eval Datastore) can't leave stale, empty issues lying
  ...
  * Writes matching ``error_tracking_fingerprint_issue_state`` rows directly to
    Datastore so V3 queries don't depend on the async Kafka/backfill path.
  * Creates matching Datastore person rows for the synthetic distinct IDs so
    person joins and test-account filters can't drop seeded events.
  * Writes the matching ``$exception`` events directly to Datastore so
    concurrent setup hooks don't depend on the process-global ``settings.TEST``
    value used by ``DatastoreProducer``. That makes the seed self-contained —
...
from insights.models.event.sql import INSERT_EVENT_SQL
from insights.models.event.util import format_datastore_timestamp
from insights.models.person.sql import INSERT_PERSON_DISTINCT_ID2, INSERT_PERSON_SQL
from insights.models.utils import uuid7
from insights.session_recordings.queries.test.session_replay_sql import INSERT_SINGLE_SESSION_REPLAY
...
_ZERO_DATASTORE_TIMESTAMP = "1970-01-01 00:00:00.000000"
    ...
    *,
    ...
    sync_execute(
        INSERT_EVENT_SQL(),
        {
            ...
            "person_id": _person_id_for(distinct_id),
            "person_properties": "{}",
            "person_created_at": _ZERO_DATASTORE_TIMESTAMP,
            "group0_properties": "{}",
            "group1_properties": "{}",
            "group2_properties": "{}",
            "group3_properties": "{}",
            "group4_properties": "{}",
            "group0_created_at": _ZERO_DATASTORE_TIMESTAMP,
            "group1_created_at": _ZERO_DATASTORE_TIMESTAMP,
            "group2_created_at": _ZERO_DATASTORE_TIMESTAMP,
            "group3_created_at": _ZERO_DATASTORE_TIMESTAMP,
            "group4_created_at": _ZERO_DATASTORE_TIMESTAMP,
            "person_mode": "full",
        },
    )
    ...
    *,
    ...
    sync_execute(
        INSERT_ERROR_TRACKING_FINGERPRINT_ISSUE_STATE,
        {
            ...
            "first_seen": format_datastore_timestamp(first_seen) if first_seen else None,
            "is_deleted": 0,
