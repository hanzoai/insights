...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://insights:insights@localhost:5432/test_insights")
...
class TestDB:
    ...
    def create_person(self, distinct_ids: list[str], properties: dict[str, Any]) -> int:
        ...
        cur.execute(
            """
            INSERT INTO insights_person (team_id, uuid, properties, created_at,
    ...
    def create_group(
        ...
        for conn in [self.main_conn, self.persons_conn]:
            ...
            cur.execute(
                """
                INSERT INTO insights_grouptypemapping (team_id, project_id, group_type, group_type_index)
                VALUES (%s, %s, %s, %s)
            ...
            cur.execute(
                """
                INSERT INTO insights_group (team_id, group_key, group_type_index,
    ...
    def set_cohort_type(self, cohort_id: int, cohort_type: str | None) -> None:
        ...
        cur.execute(
            "UPDATE insights_cohort SET cohort_type = %s WHERE id = %s",
            (cohort_type, cohort_id),
        )

    def cleanup(self) -> None:
        # insights_cohortpeople has no team_id — delete via cohort_id from the main DB
        persons_cur = self.persons_conn.cursor()
        main_cur = self.main_conn.cursor()

        main_cur.execute("SELECT id FROM insights_cohort WHERE team_id = %s", (self.team_id,))
        cohort_ids = [row[0] for row in main_cur.fetchall()]
        if cohort_ids:
            persons_cur.execute(
                "DELETE FROM insights_cohortpeople WHERE cohort_id = ANY(%s)",
                (cohort_ids,),
            )

        for table in ["insights_persondistinctid", "insights_person"]:
            persons_cur.execute(f"DELETE FROM {table} WHERE team_id = %s", (self.team_id,))  # noqa: S608

        for table in ["insights_group", "insights_grouptypemapping"]:
