...
class TestKernelSessionRunNode(SimpleTestCase):
    ...
    def test_insightsql_input_is_bound_as_a_pandas_frame(self):
        ...
        envelope = self._run("df1[df1.id > 1]", inputs=[{"name": "df1", "kind": "insightsql", "path": path}])
        self.assertEqual(envelope["status"], "ok")
    ...
    def test_binary_column_degrades_the_preview_instead_of_failing_the_run(self):
        # Datastore's native Arrow output emits UUID/FixedString columns as fixed-size
        ...
        path = os.path.join(self._dir.name, "frames", "df1.arrow")
        envelope = self._run("df1", inputs=[{"name": "df1", "kind": "insightsql", "path": path}])
        self.assertEqual(envelope["status"], "ok")
        self.assertEqual(envelope["row_count"], 2)
    ...
    def test_duckdb_node_joins_a_insightsql_frame_with_a_local_frame(self):
        # Journey 5 step 4: the local frame forces the join into DuckDB, over the mmapped
        # InsightsQL input and the pandas frame a Python node left in the namespace.
        ...
        envelope = self._run_duckdb(
            ...
            inputs=[{"name": "df2", "kind": "insightsql", "path": path}, {"name": "new_events", "kind": "local"}],
            output_name="joined",
        )
        self.assertEqual(envelope["status"], "ok")
