...
function captionHelp(caption?: string): JSX.Element | undefined {
    return caption ? <Markdown className="text-xs">{caption}</Markdown> : undefined
}
...
function AccountTextField({
    ...
    return (
        <Field name={fieldName} label={fieldLabel} help={captionHelp(caption)}>
            {({ value, onChange }) => (
                <Input
...
function accountOptionLabel(
    ...
    return (
        <div className="flex items-center gap-2">
            <span>{labelText}</span>
            {isPrimary && <Tag type="primary">Primary</Tag>}
            {badges.map((badge) => (
                <Tag type="muted" key={badge}>
                    {badge}
                </Tag>
            ))}
...
function MultiAccountFieldWithOptions({
    ...
    const options = useMemo<InputSelectOption[]>(() => {
