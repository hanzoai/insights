...
from insights.models.group_type_mapping import (
    ...
    _fetch_group_types_for_projects_via_personinsights,
...
from insights.utils import get_safe_cache, safe_cache_delete, safe_cache_set
...
_CLIENT_PATCH = "insights.personinsights_client.client.get_personinsights_client"
...
class TestGetGroupTypesForProject(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
    @patch("insights.models.group_type_mapping._fetch_group_types_via_personinsights")
    def test_personinsights_success_returns_data_and_caches(
        self,
        mock_fetch_personinsights,
        mock_objects,
    ):
        mock_fetch_personinsights.return_value = PERSONFN_SUCCESS_DATA
    ...
    @patch("insights.models.group_type_mapping._fetch_group_types_via_personinsights")
    def test_personinsights_failure_without_stale_returns_empty(
        self,
        mock_fetch_personinsights,
    ):
        mock_fetch_personinsights.side_effect = RuntimeError("grpc timeout")
    ...
    def test_cache_hit_skips_both_paths(self, mock_objects):
        from insights.utils import safe_cache_set
    ...
    @patch("insights.models.group_type_mapping._fetch_group_types_via_personinsights")
    def test_personinsights_failure_returns_stale_cache(
        self,
        mock_fetch_personinsights,
    ):
        from insights.utils import safe_cache_set
    ...
    def test_unconfigured_client_falls_back_instead_of_raising(self):
        # Worker without PERSONFN_ADDR: require_personinsights_client() raises RuntimeError.
...
class TestGetGroupTypesForTeam(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
    @patch("insights.models.group_type_mapping._fetch_group_types_for_team_via_personinsights")
    def test_personinsights_success_returns_data_without_orm(
        self,
        mock_fetch_personinsights,
        mock_objects,
    ):
        ...
        assert result == PERSONFN_SUCCESS_DATA
        mock_objects.filter.assert_not_called()

    @patch("insights.models.group_type_mapping._fetch_group_types_for_team_via_personinsights")
    def test_personinsights_failure_returns_empty(
        self,
        mock_fetch_personinsights,
    ):
        mock_fetch_personinsights.side_effect = RuntimeError("grpc timeout")
...
class TestGetGroupTypesForProjects(SimpleTestCase):
    ...
    @override_settings(PERSONFN_BATCH_SIZE=2)
    def test_fetch_via_personinsights_chunks_project_ids(self):
        ...
        result = _fetch_group_types_for_projects_via_personinsights(mock_client, [1, 2, 3, 4, 5])
    ...
    @patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
    @patch("insights.models.group_type_mapping._fetch_group_types_for_projects_via_personinsights")
    def test_personinsights_success_returns_grouped_data_without_orm(
        self,
        mock_fetch_personinsights,
        mock_objects,
    ):
        personinsights_result = {
            1: [{"group_type": "organization", "group_type_index": 0}],
            2: [{"group_type": "company", "group_type_index": 0}],
            3: [],
        }
        mock_fetch_personinsights.return_value = personinsights_result

        result = get_group_types_for_projects(self.project_ids)

        assert result == personinsights_result
        mock_objects.filter.assert_not_called()

    @patch("insights.models.group_type_mapping._fetch_group_types_for_projects_via_personinsights")
    def test_personinsights_failure_without_stale_raises_unavailable(
        self,
        mock_fetch_personinsights,
    ):
    ...
    @patch("insights.models.group_type_mapping._fetch_group_types_for_projects_via_personinsights")
    def test_empty_project_ids_returns_empty_dict_via_personinsights(
        self,
        mock_fetch_personinsights,
    ):
        mock_fetch_personinsights.return_value = {}
    ...
    @patch("insights.models.group_type_mapping._fetch_group_types_for_projects_via_personinsights")
    def test_personinsights_success_backfills_missing_project_ids_with_empty_lists(
        self,
        mock_fetch_personinsights,
    ):
        mock_fetch_personinsights.return_value = {
...
class TestGetGroupTypesForProjectsReplicaReconfirm(SimpleTestCase):
    ...
    def _set_stale(self, pid: int) -> None:
        safe_cache_set(f"{GROUP_TYPES_STALE_CACHE_KEY_PREFIX}{pid}", PERSONFN_SUCCESS_DATA, 3600)
    ...
    def _patch_fetch(self, *, replica: dict, primary: dict):
        ...
        return patch(
            "insights.models.group_type_mapping._fetch_group_types_for_projects_via_personinsights",
            side_effect=fetch,
        )
    ...
    def test_replica_empty_with_stale_but_primary_empty_stays_empty(self):
        ...
        # group types on a later personinsights outage, and the confirmed-empty marker is set so
    ...
    def test_primary_reconfirm_failure_serves_stale_not_empty(self):
        ...
        assert result[self.populated_pid] == PERSONFN_SUCCESS_DATA
        assert result[self.empty_pid] == []
...
class TestGetGroupTypesForProjectCacheBehavior(SimpleTestCase):
    ...
    def test_empty_list_result_is_cached(
        ...
        assert result1 == []
        assert result2 == []
        assert mock_fetch_personinsights.call_count == 1

    @patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
    @patch("insights.models.group_type_mapping._fetch_group_types_via_personinsights")
    def test_personinsights_success_populates_stale_cache(
        self,
        mock_fetch_personinsights,
        mock_objects,
    ):
        from insights.models.group_type_mapping import GROUP_TYPES_STALE_CACHE_KEY_PREFIX
        ...
        assert stale == PERSONFN_SUCCESS_DATA
    ...
    def test_empty_success_does_not_clobber_populated_stale(
        ...
        safe_cache_set(stale_key, PERSONFN_SUCCESS_DATA, 3600)
        ...
        assert result == []
        assert get_safe_cache(stale_key) == PERSONFN_SUCCESS_DATA
...
class TestGetGroupTypesForTeamEdgeCases(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
    @patch("insights.models.group_type_mapping._fetch_group_types_for_team_via_personinsights")
    def test_personinsights_returns_empty_list(
        self,
        mock_fetch_personinsights,
        mock_objects,
    ):
...
class TestCountGroupTypeMappingsPerTeam(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
    def test_personinsights_success_returns_converted_counts(
        self,
        mock_objects,
    ):
...
class TestUpdateGroupTypeMappingFields(SimpleTestCase):
    ...
    def _mock_objects_filter(self):
        ...
        patcher = patch("insights.models.group_type_mapping.GroupTypeMapping.objects")
        mock_objects = patcher.start()
        self.addCleanup(patcher.stop)
        return mock_objects

    @patch(_CLIENT_PATCH)
    def test_personinsights_success_does_not_call_orm(self, mock_get_client):
        mock_client = MagicMock()
...
class TestDeleteGroupTypeMapping(SimpleTestCase):
    ...
    @patch(_CLIENT_PATCH)
    def test_personinsights_success_does_not_call_orm_delete(self, mock_get_client):
        mock_client = MagicMock()
...
class TestClearDashboardFromGroupTypeMapping(SimpleTestCase):
    @patch("insights.models.group_type_mapping.invalidate_group_types_cache")
    @patch(_CLIENT_PATCH)
    def test_personinsights_success_reads_then_updates(self, mock_get_client, mock_invalidate):
    ...
    @patch("insights.models.group_type_mapping.invalidate_group_types_cache")
    @patch(_CLIENT_PATCH)
    def test_personinsights_group_type_index_zero_still_clears(self, mock_get_client, mock_invalidate):
    ...
    @patch("insights.models.group_type_mapping.invalidate_group_types_cache")
    @patch(_CLIENT_PATCH)
    def test_personinsights_no_matching_mapping_skips_update(self, mock_get_client, mock_invalidate):
        mock_resp = MagicMock()
...
class TestTerminalFetchFailureMetric(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.GROUP_TYPES_FETCH_FAILURES")
    @patch("insights.models.group_type_mapping._fetch_group_types_via_personinsights", side_effect=RuntimeError("grpc fail"))
    def test_single_project_failure_increments_fetch_failures(self, _mock_fetch, mock_fetch_failures):
        ...
        mock_fetch_failures.labels.assert_called_once_with(
            operation="get_group_types_for_project", source="personinsights", error_type="db_error"
        )

    @patch("insights.models.group_type_mapping.GROUP_TYPES_FETCH_FAILURES")
    @patch(
        "insights.models.group_type_mapping._fetch_group_types_for_team_via_personinsights",
    ...
    def test_team_failure_increments_fetch_failures(self, _mock_fetch, mock_fetch_failures):
        ...
        assert result == []
        mock_fetch_failures.labels.assert_called_once_with(
            operation="get_group_types_for_team", source="personinsights", error_type="db_error"
        )
    ...
    def test_projects_failure_increments_fetch_failures(self, _mock_fetch, mock_fetch_failures):
        ...
        mock_fetch_failures.labels.assert_called_once_with(
            operation="get_group_types_for_projects", source="personinsights", error_type="db_error"
        )
...
class TestRecordGroupTypesFetchFailureThrottle(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.GROUP_TYPES_FETCH_FAILURES")
    @patch("insights.models.group_type_mapping.logger")
    @patch("insights.utils.capture_exception")
...
class TestProjectHasGroupTypesAuthoritatively(SimpleTestCase):
    ...
    _DIRECT_PATCH = "insights.models.group_type_mapping._fetch_group_types_for_project_direct"
    _SAMPLE_ROW = {
        "group_type": "organization",
        "group_type_index": 0,
        "name_singular": None,
...
# Missing client (PERSONFN_ADDR unset) raises RuntimeError; read paths must recover
...
class TestGetGroupTypeMappingInstance(SimpleTestCase):
    ...
    @patch("insights.models.group_type_mapping.get_group_types_for_project")
    def test_returns_matching_instance(self, mock_get):
        mock_get.return_value = PERSONFN_SUCCESS_DATA
    ...
    def test_cache_bust_retry_finds_mapping(self, mock_get, mock_invalidate):
        ...
        mock_get.side_effect = [PERSONFN_SUCCESS_DATA, fresh_data]
    ...
    @patch("insights.models.group_type_mapping._dict_to_group_type_mapping_model")
    @patch("insights.models.group_type_mapping.get_group_types_for_project")
    def test_passes_team_to_model_builder(self, mock_get, mock_builder):
        ...
        mock_builder.assert_called_once_with(PERSONFN_SUCCESS_DATA[0], project_id=self.project_id, team=team)

    @patch("insights.models.group_type_mapping._fetch_group_types_for_project_direct")
    def test_consistency_strong_bypasses_cache(self, mock_fetch):
        mock_fetch.return_value = PERSONFN_SUCCESS_DATA
        mock_builder_result = MagicMock(spec=GroupTypeMapping)

        with patch(
            "insights.models.group_type_mapping._dict_to_group_type_mapping_model", return_value=mock_builder_result
        ):
