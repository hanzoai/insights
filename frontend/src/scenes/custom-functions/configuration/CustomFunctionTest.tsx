...
import { customFunctionTestLogic } from './customFunctionTestLogic'

export function CustomFunctionTestPlaceholder({
    title,
    description,
}: {
...
export const CustomFunctionTestEditor = ({
    value,
    onChange,
    readOnly = false,
}: {
...
export function CustomFunctionTest(): JSX.Element {
    const { logicProps, canLoadSampleGlobals, customFunction, template } = useValues(customFunctionConfigurationLogic)
    const {
        ...
        sortedTestsResult,
        jsonError,
    } = useValues(customFunctionTestLogic(logicProps))
    const {
    ...
    } = useActions(customFunctionTestLogic(logicProps))

    const testResultsRef = useRef<HTMLDivElement>(null)
    const inactive = !expanded
    const canMockFetchRequests =
        template?.id?.startsWith('template-') || customFunction?.template?.id?.startsWith('template-')

    return (
        <Form logic={customFunctionTestLogic} props={logicProps} formKey="testInvocation" enableFormOnSubmit>
            <div
                ref={testResultsRef}
                className={clsx(
                        ...
                        <>
                            {testResult ? (
                                ...
                                <>
                                    ...
                                    <LemonButton
                                        type="primary"
                                        data-attr="test-custom-function"
