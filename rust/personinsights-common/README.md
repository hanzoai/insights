...
Context for the implementation details and current state of each service lives in the README.md in the root of the respective service's folder, e.g more context on personinsights-replica lives at insights/rust/personinsights-replica/README.md

## Personhog Cluster

The personinsights cluster is composed of the following pieces:

- personinsights-router
- personinsights-replica
- personinsights-leader cluster
- personinsights-coordinator + metdata store
...
- responsible for routing requests to the correct personinsights pod
- serves as a single entry point into all things person/personinsights
- no state or dependencies are needed for the router to route a request to a personinsights-replica pod
- stateful/protocol enabled routing to decide which personinsights-leader pod should serve an incoming request
...
- isolates all the simple data access patterns from the stateful personinsights-leader BE
- gRPC service
- creates a request path that is cheaper and easier to scale/operate for clients that don't need strong consistent reads/writes to the cached tables on personinsights-leader BEs
