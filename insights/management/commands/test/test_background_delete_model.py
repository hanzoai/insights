...
from insights.management.commands.background_delete_model import Command
...
class TestBackgroundDeleteModel(BaseTest):
    ...
    def test_valid_model_with_team_id_field(self, mock_input, mock_task):
        ...
        self.command.handle("insights.Tag", team_id=self.team.id)

        # Verify task was called
        mock_task.delay.assert_called_once_with(
            model_name="insights.Tag", team_id=self.team.id, batch_size=10000, records_to_delete=2
        )
    ...
    def test_dry_run_does_not_start_task(self, mock_task):
        ...
        self.command.handle("insights.Tag", team_id=self.team.id, dry_run=True)
    ...
    def test_batch_size_limit(self):
        ...
        with patch("insights.management.commands.background_delete_model.background_delete_model_task") as mock_task:
            mock_task.delay.return_value = MagicMock(id="test-task-id")
            ...
            with patch("builtins.input", return_value="DELETE 2 RECORDS"):
                self.command.handle("insights.Tag", team_id=self.team.id, batch_size=100000)

            # Verify task was called with the limited batch size
            mock_task.delay.assert_called_once_with(
                model_name="insights.Tag", team_id=self.team.id, batch_size=50000, records_to_delete=2
            )
    ...
    def test_synchronous_execution(self, mock_input, mock_task):
        ...
        self.command.handle("insights.Tag", team_id=self.team.id, synchronous=True)
    ...
    def test_max_delete_size_limit(self):
        ...
        with patch("django.apps.apps.get_model") as mock_get_model:
            ...
            with patch("builtins.input", return_value="DELETE 5,000,000 RECORDS"):
                with patch(
                    "insights.management.commands.background_delete_model.background_delete_model_task"
                ) as mock_task:
                    mock_task.delay.return_value = MagicMock(id="test-task-id")
                    self.command.handle("test.TestModel", team_id=1)
