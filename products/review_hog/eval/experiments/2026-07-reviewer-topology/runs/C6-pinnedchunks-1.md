...
- **Report id:** `019f20d2-b4d5-7d4b-8c08-8cec90dbff69` · **PR:** https://github.com/Insights/insights/pull/62096
...
- `EXPERIMENT_PINNED_CHUNKS` = {1: ['ee/scriptai/tools/actions/core.py'], 2: ['ee/scriptai/tools/actions/tool.py', 'ee/scriptai/tools/actions/**init**.py', 'ee/scriptai/tools/**init**.py', 'ee/scriptai/chat_agent/toolkit.py'], 3: ['frontend/src/scenes/max/max-constants.tsx', 'frontend/src/queries/schema/schema-assistant-messages.ts', 'frontend/src/queries/schema.json', 'insights/schema_enums.py']}
...
### [❌ dismissed] must_fix · security — ee/scriptai/tools/actions/tool.py:69-73
...
### [✅ VALID] should_fix (validator→consider) · performance — ee/scriptai/tools/actions/core.py:143-150
...
- **Problem:** `list_actions` loads full `Action` rows via `list(qs[start : start + capped_limit])` (up to MAX_LIST_LIMIT=100 rows), but the compact list rendering in `_format_action` only reads `id`, `name`, `description`, `steps` (steps_json) and `bytecode_error`. The `bytecode` JSONField — the compiled InsightsQL bytecode, which can be several KB per action — is pulled from Postgres and JSON-deserialized into Python for every row on every call, then discarded. This tool is explicitly designed for repeated, paginated calls by an LLM agent over projects that (per the tool's own description) 'can have thousands of actions', so the wasted I/O and deserialization compounds across pages and calls.
