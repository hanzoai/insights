"""Click entrypoint for ``insightscli test:quarantine``.

Wired into the insightscli CLI via the ``click:`` manifest entry in ``insightscli.yaml``.
Schema contract and selector grammar: ``insightscli_commands.quarantine.core``.

    insightscli test:quarantine add <id> --reason ... --owner ... [--issue ...] [--days 14] [--mode run|skip]
    insightscli test:quarantine list [--json]
    insightscli test:quarantine remove <id>
    insightscli test:quarantine check [--grace-days 7]
"""
...
# avoid installing the full dev environment): python -m insightscli_commands.quarantine.cli
