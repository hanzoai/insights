...
# Schema name -> the `schema_mapping` key the Script template routes incoming events by.
SCHEMA_TO_WEBHOOK_RESOURCE: dict[str, str] = {WEBHOOK_TABLE_NAME: WEBHOOK_TABLE_NAME}

# Mailjet event payloads carry no event identifier, so the Script template derives one by hashing
