...
export const insightsQLAutocompleteProvider = (type: ScriptLanguage): languages.CompletionItemProvider => ({
    triggerCharacters: [' ', ',', '.', '{'],
    provideCompletionItems: async (model, position) => {
        ...
        const connectionId =
            logic.isMounted() && logic.props.sourceQuery?.kind === NodeKind.InsightsQLQuery
        ...
        let response: NonNullable<InsightsQLAutocomplete['response']>
        try {
            response = await performQuery<InsightsQLAutocomplete>(query)
        } catch {
            return emptyCompletionList()
        }
