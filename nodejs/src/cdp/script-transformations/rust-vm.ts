...
// prefix (rust/common/scriptvm/node/src/ext_fns.rs `unsupported()`); the scriptvm crate reports calls to
...
export function loadHogvmNodeModule(options: { mmdbPath: string }): HogvmNodeModule | null {
    ...
    try {
        ...
        cachedModule = null
        logger.warn('🦀', 'Rust ScriptVM native module unavailable', { error: String(error) })
    }
    return cachedModule
}
