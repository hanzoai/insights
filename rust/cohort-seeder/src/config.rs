...
const POOL_NAME: &str = "insights_cohort_seeder";
...
pub struct Config {
    ...
    #[envconfig(default = "")]
    pub datastore_offline_cluster_host: String,

    #[envconfig(default = "localhost:8123")]
    pub datastore_host: String,

    #[envconfig(default = "default")]
    pub datastore_user: String,

    #[envconfig(default = "")]
    pub datastore_password: String,

    #[envconfig(default = "default")]
    pub datastore_database: String,

    #[envconfig(default = "false")]
    pub datastore_secure: bool,

    /// Validate the server certificate against the public root CAs. Defaults on so an unconfigured
    /// deployment fails closed; the chart sets it to `false` for internal Datastore.
    #[envconfig(default = "true")]
    pub datastore_verify: bool,

    /// PEM CA bundle to validate the Datastore server certificate against, as Django's
    /// `DATASTORE_CA`. Naming a CA is an explicit request to authenticate the server, so it wins
    /// over `datastore_verify` rather than being downgraded by an inherited `DATASTORE_VERIFY=false`.
    #[envconfig(default = "")]
    pub datastore_ca: String,
    ...
    /// settings validation refuses a value the Datastore execution-time budget cannot cover.
...
mod tests {
    ...
    #[test]
    fn datastore_tls_defaults_are_fail_closed() {
        let config = default_config();
        assert!(config.datastore_verify);
        assert!(config.datastore_ca.is_empty());
    }
