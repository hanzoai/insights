"""InsightsQL INSERT templates for the fixed-dimension web precompute tables.

These are the precomputation-framework successors to the v2 pre-aggregation
`WEB_STATS_INSERT_SQL` / `WEB_BOUNCES_INSERT_SQL` (`insights/models/web_preaggregated/sql.py`).
Where v2 was hand-written Datastore SQL run by a staging-table + partition-swap
Dagster ETL, these are InsightsQL templates fed to `ensure_precomputed`, which manages
...
Parity with v2 is intentional, with two deliberate deviations:
  - `region_name` is taken from the event's `$geoip_subdivision_1_name` (like the
    other geoip dimensions) because the InsightsQL `sessions` table exposes no initial
...
from insights.models.web_preaggregated.sql import is_eu_cluster
...
from products.web_analytics.backend.insightsql_queries.web_analytics_lazy_precompute import SESSION_FORWARD_PAD_MINUTES
...
def _mat_metadata_placeholders() -> dict[str, ast.Expr]:
    ...
    if not is_eu_cluster():
        # Typed NULLs without a CAST (InsightsQL rejects `CAST(NULL AS Nullable(...))`):
