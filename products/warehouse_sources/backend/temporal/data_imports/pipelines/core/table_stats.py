...
# Any single table whose in-memory Arrow payload crosses this is logged and captured as a Insights
...
def record_table_stats(
    *,
    ...
    outlier log and the Insights event only.
    """
    ...
    if payload_bytes is not None and payload_bytes >= OUTLIER_TABLE_BYTES:
        ...
        # Long-lived Temporal worker, so hanzo_insights' background flush thread stays alive
