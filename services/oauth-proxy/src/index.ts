/**
 * oauth.hanzo.ai — Cross-Region OAuth Proxy
 *
 * A Cloudflare Worker that sits in front of both US and EU Insights instances,
 * providing a single OAuth endpoint that handles region routing transparently.
 *
 * Used by the Insights MCP server, Insights Desktop, and any future OAuth integration
...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        try {
            ...
            if (normalized === '') {
                return new Response('Insights OAuth Proxy - https://hanzo.ai/docs/api/oauth', {
