...
class TestTimingRescheduleFlag(SimpleTestCase):
    ...
    def test_flag_value_passthrough(self, _name, enabled):
        with patch("hanzo_insights.feature_enabled", return_value=enabled):
            assert use_workflows_timing_reschedule(self._team()) is enabled

    def test_flag_check_failure_defaults_off(self):
        with patch("hanzo_insights.feature_enabled", side_effect=Exception("flag service down")):
...
class TestRescheduleInsightsFlowTimingTask(BaseTest):
    def _create_flow(self, status: str = "active") -> InsightsFlow:
        return InsightsFlow.objects.create(name="Test Flow", team=self.team, status=status)
