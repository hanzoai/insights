...
import { escapePropertyAsInsightsQLIdentifier, setLatestVersionsOnQuery } from '~/queries/utils'
import {
    BaseMathType,
    ChartDisplayType,
...
import { customFunctionConfigurationLogic, sanitizeConfiguration } from '../configuration/customFunctionConfigurationLogic'
import type { customFunctionTestingLogicType } from './customFunctionTestingLogicType'

export interface CustomFunctionTestingLogicProps {
    id: string
}
...
export const customFunctionTestingLogic = kea<customFunctionTestingLogicType>([
    path((key) => ['scenes', 'pipeline', 'customFunctionTestingLogic', key]),
    props({} as CustomFunctionTestingLogicProps),
    key(({ id }: CustomFunctionTestingLogicProps) => id),
    connect(() => ({
        values: [
        ...
        retries: [
            [] as CyclotronJobTestInvocationResultWithEventId[],
            {
                retryInvocation: async ({
                    ...
                    try {
                        res = await api.customFunctions.createTestInvocation(props.id ?? 'new', {
                            globals,
