...
def _capture_mode_usage(
    ...
    *,
    ...
    # ph_scoped_capture (not hanzo_insights.capture) — events from Celery tasks are otherwise
    # silently lost; no-ops off Insights Cloud. Telemetry must never fail the task, so swallow errors.
    ...
    for w in pending:
        ...
        if page_status is not None and not 200 <= page_status < 300:
            raise PageHttpStatusError(
                ...
                f"is a picture of that response. This comes from the site's host or CDN, not from Insights.",
