...
export function parseInsightsQLX(value: any): any {
    ...
    if (value[0] === '__hx_tag') {
        ...
        for (let i = start; i < value.length; i += 2) {
            const key = parseInsightsQLX(value[i])
            object[key] = parseInsightsQLX(value[i + 1])
        }
        return object
    }
    return value.map((v) => parseInsightsQLX(v))
}

export function renderInsightsQLX(value: any): JSX.Element {
    const object = parseInsightsQLX(value)
    ...
    if (typeof object === 'object') {
        ...
        if (!tag) {
            ...
            return (
                <LightErrorBoundary>
                    <Link to={href} target={target ?? undefined}>
                        {value ? renderInsightsQLX(value) : href}
                    </Link>
                </LightErrorBoundary>
            )
            ...
            const value = children ?? source
            const renderedChildren = value ? renderInsightsQLX(value) : ''

            return (
                <LightErrorBoundary>
                    <span className={`insightsqlx-${tag}`}>
