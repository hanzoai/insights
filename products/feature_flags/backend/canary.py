...
def run_local_eval_canary(registry: CollectorRegistry | None) -> None:
    ...
    Metrics:
    - ``insights_feature_flags_local_eval_canary_group_mapping_present``: 1 when the
      mapping is non-empty, else 0.
    - ``insights_feature_flags_local_eval_canary_failure_total``: incremented on an
    ...
    present_gauge = Gauge(
        "insights_feature_flags_local_eval_canary_group_mapping_present",
        "Whether the canary team's local-eval group_type_mapping is non-empty (1) or empty/errored (0)",
        registry=registry,
    )
    failure_counter = Counter(
        "insights_feature_flags_local_eval_canary_failure",
