 ...
 *   - Each panel (Combobox / DwhConfig / InsightsQL) is a pure component; it
...
type MenuOption = 'new' | 'recent' | 'pinned' | 'dwh' | 'insightsql'
 ...
 * the user lands on something editable (e.g. the InsightsQL editor pre-filled with
...
export function resolveSelectedOpenState(selected: MenuFilterEntry | null): MenuFilterState {
    ...
    if (selected.group.type === TaxonomicFilterGroupType.InsightsQLExpression) {
        return { kind: 'insightsql-edit' }
    }
...
export function TaxonomicFilterMenu({
    ...
    useEffect(() => {
        ...
        if (previous === 'closed' && next !== 'closed') {
            ...
            insights.capture('taxonomic filter menu opened', {
                openedTo: next,
            ...
            insights.capture('taxonomic filter menu drilled', {
                fromState: previous,
                toState: next,
            })
            ...
            insights.capture('taxonomic filter menu closed', {
                dwellMs,
    ...
    // DWH / InsightsQL rather than just a generic menu→panel `drilled` event.
    const selectMenuOption = useCallback((option: MenuOption, action: () => void): void => {
        insights.capture('taxonomic filter menu option clicked', { option })
        action()
    }, [])
    ...
    const handleCommit = useCallback<CommitFn>(
        (entry, extra, selection) => {
            ...
            insights.capture('taxonomic filter menu item selected', {
            ...
            // here, while a config-form or InsightsQL commit does. `groupType` is the
            ...
            // surface. `selection` is absent for non-row commits (DWH form, InsightsQL).
    ...
    // so Insights's stock z-order (`--z-popover` > `--z-modal`) keeps
    ...
    // table list.
    const popoverOpen = state.kind === 'combobox' || state.kind === 'dwh-pick' || state.kind === 'insightsql-edit'
    ...
    // trigger yields to an invisible spacer). `insightsql-edit` is intentionally
    ...
    // Filter-icon menu anchor, styled to match the scene (Button). Lives
        ...
        <DropdownMenuTrigger
            render={
                <Button
                    ...
                    // Stop the click bubbling to the Input wrapper, whose
        ...
        >
            ...
            >
                ...
                >
                    ...
                    {state.kind === 'insightsql-edit' && (
                        <MenuFilterInsightsQLEditor
                            initialExpression={
                                selected?.group.type === TaxonomicFilterGroupType.InsightsQLExpression
                                    ? selected.name
                                    : undefined
                            }
                            onCommit={handleCommit}
