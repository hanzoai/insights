...
from insights.rate_limit import DatastoreBurstRateThrottle, DatastoreSustainedRateThrottle, InsightsQLQueryThrottle

from ..facade import api
from ..facade.enums import INSIGHTSQL_DEFINITION_KIND, INSIGHT_DEFINITION_KINDS, NODE_DEFINITION_KINDS
...
# Kinds that execute a Datastore query through the trends/funnels pipeline (node kinds run as a
...
class MetricViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_throttles(self) -> list[BaseThrottle]:
        ...
        # markdown and definition-less metrics never reach Datastore and keep the API defaults.
        if getattr(self, "action", None) == "run":
            ...
            if kind == INSIGHTSQL_DEFINITION_KIND:
...
class RelationshipProposalViewSet(
    ...
    @action(
        ...
        throttle_classes=[InsightsQLQueryThrottle],
        request=None,
    ...
    def accept(self, request: Request, **kwargs) -> Response:
        ...
        # explicitly: query (the Datastore acceptance probe) and warehouse_view (the join it creates).
        if not self.user_access_control.check_access_level_for_resource("query", "viewer"):
