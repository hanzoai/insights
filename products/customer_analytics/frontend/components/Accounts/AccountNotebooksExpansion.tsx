...
import { IconCloud, IconGraph, IconPencil, IconPeople, IconPiggyBank, IconReceipt } from '@hanzo/icons'
import {
    Button,
    Input,
    Label,
        ...
        <div className="flex flex-col gap-2">
            ...
            {activeRelationships.map((relationship) => (
                <div key={relationship.id} className="flex flex-col gap-1">
                    <Label>{relationship.definition.name}</Label>
            ...
            {accountLoading ? (
                ...
                links.map((link) => (
                    <Button
                        ...
                        onClick={() =>
                            insights.capture(AccountsEvents.LinkClicked, {
                                link_key: link.key,
...
export function AccountNotebooksExpansion({
    ...
    // Tabs only renders the active tab, so without this the data refetches on every tab switch.
    ...
    const columns: TableColumns<AccountNotebookApi> = [
        {
            title: 'Note',
            key: 'title',
            render: (_, notebook) => {
                ...
                return (
                    <div className="flex flex-col gap-1 py-1 max-w-2xl">
                        <Link
                            ...
                            onClick={(event) => {
                                insights.capture(AccountsEvents.NoteClicked, {
