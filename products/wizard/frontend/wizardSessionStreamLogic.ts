 ...
 * Usage:
 *
 *   // Listen to any skill under the insights-integration workflow:
 *   const { latestSession } = useValues(
 *       wizardSessionStreamLogic({ workflowId: 'insights-integration' })
 *   )
 *
 *   // Or pin to one skill:
 *   const { latestSession } = useValues(
 *       wizardSessionStreamLogic({ workflowId: 'insights-integration', skillId: 'nextjs' })
 *   )
 */
export const wizardSessionStreamLogic = kea<wizardSessionStreamLogicType>([
    ...
    listeners(({ values, actions, props, cache }) => ({
        ...
        sessionUpdated: () => {
            ...
            insights.capture('wizard sync transport ready', {
        ...
        connectionErrored: ({ error }) => {
            ...
            insights.capture('wizard sync transport error', {
                transport: cache.syncMode ?? 'sse',
                workflow_id: props.workflowId,
                error,
            })
        },
        // Flags can resolve after a cold-cache connect (insights-js loads them async), and ops can flip
