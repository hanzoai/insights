...
out the curated ``SELECT`` subqueries and the CI rollup CTE, and runs the assembled InsightsQL.
The resolved table names live inside it, so the query layer never threads or re-derives them.
The product reads its data privately this way — nothing is registered as a global InsightsQL view,
...
class CuratedGitHubSource:
    ...
    modules then ask the returned instance for the curated subqueries and run InsightsQL through it.
    """
    ...
    def job_cost_source(self) -> str | None:
        ...
        / ``estimated_cost_usd`` are rendered from ``logic.cost`` in Datastore, so every endpoint cost
    ...
    def run(
        ...
        *,
        ...
        """Parse + execute a curated InsightsQL query for this team.

        Mirrors the two paths the data warehouse team intends for ``insightsql-warehouse-access-control``
        (#61686). Request-driven reads (the common case — the views thread the requesting user through)
        forward that user so InsightsQL honors the per-table warehouse ACL: access is enforced twice over —
        ...
        ``workload`` routes the read to a non-default Datastore cluster (e.g. ``Workload.LOGS`` for the
        ...
        with tags_context(product=Product.ENGINEERING_ANALYTICS, feature=Feature.QUERY, team_id=self._team.pk):
            return execute_insightsql_query(
                ...
                # The logs table lives on a separate Datastore cluster (Workload.LOGS); warehouse
                ...
                # lets InsightsQL honor the per-table warehouse ACL.
...
def opt_float(value: float | None) -> float | None:
    """Datastore aggregate → optional float: quantile/avg over an empty set returns NaN, nullIf None."""
