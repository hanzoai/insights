...
def has_recent_pageviews(team: Team, *, days: int) -> bool:
    ...
    web analytics". `LIMIT 1` lets Datastore early-terminate instead of counting the full window."""
    query = f"""
        SELECT 1
        FROM events
        ...
        LIMIT 1
    """
    response = execute_insightsql_query(query=query, team=team, query_type="web_path_cleaning_recent_pageviews")
...
def sample_pathnames(team: Team, *, days: int, limit: int) -> list[tuple[str, int]]:
    ...
    query = f"""
        ...
        LIMIT {int(limit)}
    """
    response = execute_insightsql_query(query=query, team=team, query_type="web_path_cleaning_sample")
...
def count_distinct_pathnames(team: Team, *, days: int) -> int:
    query = f"""
        ...
        WHERE event = '$pageview'
          ...
          AND properties.$pathname != ''
    """
    response = execute_insightsql_query(query=query, team=team, query_type="web_path_cleaning_distinct")
...
def validate_and_annotate_rules(
    ...
    """Compile each suggested regex with re2 (the same engine Datastore `replaceRegexpAll` uses) and
...
def generate_suggestions_for_team(
    ...
    *,
    ...
    try:
        # The whole suggestion pipeline rides the health-check framework, so its Datastore
        ...
        with tags_context(
            ...
            # The activity gate runs a Datastore query, so it lives inside the guard with the
...
def preview_rules_on_team(
    ...
    *,
    ...
    """Apply the suggested rules (in order, output feeding the next — exactly how Datastore
