...
A single eager `import` chain reaching the AI core, the batch-export Temporal framework, embedded Datastore (`chdb`), `scipy`, or the Stripe SDK adds hundreds of ms each.
...
The DRF route aggregator (the router build plus ~200 viewset imports) used to run at _package import_ of `insights.api`.
It now lives in `insights/api/rest_router.py`, and `insights/api/__init__.py` is a thin PEP 562 `__getattr__` shim that imports the aggregator only when a router object (`router`, `projects_router`, …) is first accessed — i.e. when the URLconf resolves, on the first web request.
Real submodules (`insights.api.monitoring`, `.file_system`, …) resolve directly without building the aggregator, so a plain `django.setup()` stays cheap.
...
The entrypoints that own a setup (`manage.py`, `insights/wsgi.py`, `insights/asgi.py`) wrap it in `gc.disable()` → boot → `gc.freeze()` → `gc.enable()`.
...
`insights.schema` (the generated pydantic data model, ~2s to import) and the InsightsQL/query-runner layer used to load during `django.setup()` because dozens of model files and `ready()` chains imported them at module scope.
...
the enums (~270 classes) live in `insights.schema_enums`, a separate generated module that imports in ~20ms — `bin/split-schema-enums.py` extracts them as a post-generation step of `insightscli build:schema`, and `insights.schema` re-exports every name so existing imports keep working;
everything else got one of the standard treatments — enum-only imports repointed to `insights.schema_enums`, annotation-only model uses moved under `TYPE_CHECKING` with quoted annotations, method-body uses imported at call time.
The default when writing a model file or any other setup-path module: take enums from `insights.schema_enums`; if you need an actual pydantic model, import it inside the method that uses it.
The same applies to the celery task graph: `insights/tasks/__init__.py` eagerly imports every task module so autodiscovery registers them, so any module-level `from insights.tasks...` import on a setup path drags all of them in — import tasks at the call site, and take `CeleryQueue` from `insights.celery_queues` (import-light, made for decorator-eval consumers).

## The regression guard

`insights/test/test_startup_import_budget.py` boots a bare `django.setup()` in a clean subprocess and asserts three things, one per mechanism:

1. No `FORBIDDEN_AT_SETUP` heavy module (the lazy router aggregator, the generated `insights.schema`, the query-runner layer, the AI core, `chdb`, `scipy`, …) is in `sys.modules`.
...
Removing an entry to make the test pass puts back the import the guard exists to keep off the startup path.
...
**Adding a heavy dependency** (a vendor SDK, a Temporal/AI/Datastore path, anything pulling pandas/pyarrow/scipy).
...
New routes go in `insights/api/rest_router.py` (or a product's `register_routes`), never back in the `__init__.py` shim.
...
Long-lived branches that move the aggregator into `rest_router.py` keep colliding with master, which still edits the old eager `insights/api/__init__.py` (product migrations move viewset modules out of `insights/api` into `products/`).
...
The eager router imported a large chain when `insights/urls.py` loaded, _before_ urls.py reached its own product imports.
...
The real example: `slack_app.backend.api` imports workflow classes from `insights_code_slack_mention`, which imported a helper straight back from `slack_app.backend.api` at module scope (placed late with `# noqa: E402` — a tell that someone already fought the ordering).
...
Do not assume: the decisive test is `django.setup()` then `import_module("insights.urls")` in a clean subprocess, run on a detached `origin/master` worktree _and_ the branch.
...
Generating `insights.schema` against a `defer_build` base took ~400ms of core-schema construction off every setup, and looked safe — validation, `model_dump`, `model_json_schema`, and `TypeAdapter` all build on demand in round-trip tests.
...
The eviction hit this with `from insights.insightsql.modifiers import InsightsQLQueryModifiers` in test files: fine for years, ImportError the day modifiers stopped binding the name.
Before unbinding a name, grep for every import form — including `from package import module` and relative `from ...schema import` spellings, which a `^from insights\.schema import` regex misses — and repoint consumers to the defining module.
...
Fix the test, not the deferral: patch the name where it is _read_ — the defining module (`@patch("insights.insightsql.query.execute_insightsql_query")`), which the call-time import resolves at call time, after the patch is in place.
