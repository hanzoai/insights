...
-- same version as the Datastore tombstone, and re-creation of a previously
-- deleted key becomes a revival upsert (version bumped above the tombstone)
-- instead of an insert restarting at version 0 that loses to the Datastore
-- tombstone forever.
--
...
-- The writer's validation target mirrors insights_person (minus is_user_id),
