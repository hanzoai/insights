...
LEGACY_TEMPLATE_ID = "plugin-insights-plugin-geoip"
NEW_TEMPLATE_ID = "template-geoip"
BATCH_SIZE = 1000


class Command(BaseCommand):
    help = "Migrate legacy plugin-insights-plugin-geoip transformations in place to template-geoip"
    ...
    def handle(self, *args, **options):
        ...
        template = InsightsFunctionTemplate.get_template(NEW_TEMPLATE_ID)
        if not template:
        ...
        for batch_start in range(0, total_found, BATCH_SIZE):
            ...
            for insights_function in InsightsFunction.objects.filter(id__in=batch_ids).select_related("team").order_by("id"):
                if dry_run:
                    migrated_count += 1
                    self.stdout.write(
                        f"  Would migrate id={insights_function.id} team={insights_function.team_id} "
                        f"enabled={insights_function.enabled} name={insights_function.name!r}"
                    )
                    continue

                # One broken row must not abort the whole run
                try:
                    insights_function.template_id = NEW_TEMPLATE_ID
                    insights_function.script = template.code
                    insights_function.bytecode = bytecode
                    insights_function.insights_function_template = template
                    insights_function.icon_url = template.icon_url
                    ...
                    failed.append((str(insights_function.id), insights_function.team_id, str(e)))
