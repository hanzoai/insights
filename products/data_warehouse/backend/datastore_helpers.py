"""Datastore source helpers shared between warehouse and direct-query modes.

Direct-query-only utilities (DataWarehouseTable upserts, the `direct://datastore` url_pattern, the
option keys that encode source location on a direct table) live in `direct_datastore.py`. Datastore
...
from products.data_warehouse.backend.direct_datastore import (
    DIRECT_DATASTORE_DATABASE_OPTION,
    DIRECT_DATASTORE_TABLE_OPTION,
    hide_direct_datastore_table,
    upsert_direct_datastore_table,
)
from products.warehouse_sources.backend.facade.models import (
    ExternalDataSource,
    datastore_column_to_dwh_column,
    datastore_columns_to_dwh_columns,
)
...
type DatastoreDwhColumns = dict[str, dict[str, Any]]
# (database, table). Datastore has no separate catalog level.
type DatastoreSourceLocation = tuple[str, str]

# Datastore's implicit default database when a source config leaves it unset.
_DEFAULT_DATASTORE_DATABASE = "default"


def datastore_schema_metadata_to_dwh_columns(schema_metadata: dict[str, Any] | None) -> DatastoreDwhColumns:
    resolved: DatastoreDwhColumns = {}
    ...
    for column in columns:
        ...
        datastore_type = column.get("data_type")
        nullable = bool(column.get("is_nullable"))
        if not isinstance(column_name, str) or not isinstance(datastore_type, str):
            continue
        resolved[column_name] = datastore_column_to_dwh_column(column_name, datastore_type, nullable)
    return resolved


def get_datastore_source_location(
    *,
    schema_name: str,
    schema_metadata: dict[str, Any] | None = None,
    default_database: str | None = None,
) -> DatastoreSourceLocation:
    """Resolve `(source_database, source_table_name)` for a Datastore row.
    ...
    return normalized_default or _DEFAULT_DATASTORE_DATABASE, schema_name


def get_datastore_source_location_for_schema_model(
    *,
    schema_name: str,
    ...
    if schema_metadata is not None:
        return get_datastore_source_location(
            schema_name=schema_name, schema_metadata=schema_metadata, default_database=default_database
        )

    table_source_database = (
        table_options.get(DIRECT_DATASTORE_DATABASE_OPTION) if isinstance(table_options, dict) else None
    )
    table_source_table_name = (
        table_options.get(DIRECT_DATASTORE_TABLE_OPTION) if isinstance(table_options, dict) else None
    )
...
def reconcile_datastore_schemas(
    *,
    source: ExternalDataSource,
    source_schemas: list[SourceSchema],
    team_id: int,
) -> list[str]:
    """Persist `schema_metadata` on every Datastore row + (direct mode only) upsert its live-query
    ...
    schema_models_by_location: dict[DatastoreSourceLocation, ExternalDataSchema] = {}
    for schema_model in schema_models.values():
        location = get_datastore_source_location_for_schema_model(
    ...
    for source_schema in source_schemas:
        ...
        if matched is None:
            location = get_datastore_source_location(
        ...
        resolved_database, resolved_table = get_datastore_source_location(
            schema_name=source_schema.name,
            schema_metadata={
                "source_schema": source_schema.source_schema,
                "source_table_name": source_schema.source_table_name,
        ...
        if removed_columns:
            log.info(
                "datastore.reconcile_schemas.pruned_enabled_columns",
        ...
        if not matched.should_sync:
            hide_direct_datastore_table(matched.table)
            continue
        ...
        table_model = upsert_direct_datastore_table(
            matched.table,
            schema_name=source_schema.name,
            source=source,
            columns=datastore_columns_to_dwh_columns(projected_columns),
    ...
    for s in stale:
        hide_direct_datastore_table(s.table)
        if not s.deleted:
            s.soft_delete()
...
def reproject_direct_datastore_table(
    schema_row: Any,
    *,
    source: ExternalDataSource,
    ...
    source_database, source_table_name = get_datastore_source_location(
        schema_name=schema_row.name,
        schema_metadata=schema_row.schema_metadata,
        default_database=(source.job_inputs or {}).get("database"),
    )
    return upsert_direct_datastore_table(
        schema_row.table,
        schema_name=schema_row.name,
        source=source,
        columns=filter_dwh_columns_by_enabled_columns(
            datastore_schema_metadata_to_dwh_columns(schema_row.schema_metadata),
            enabled_columns,
            schema_row.primary_key_columns,
            schema_row.incremental_field,
            # Direct-Datastore columns are keyed by raw, case-sensitive source names.
            normalize=False,
        ),
