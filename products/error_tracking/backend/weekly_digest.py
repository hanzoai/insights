...
from insights.utils import compact_number

logger = structlog.get_logger(__name__)

DIGEST_WEBHOOK_TIMEOUT_SECONDS = 10

# Above InsightsQL's 60s default: digest scans on high-volume teams (14-day windows, plus a persons
# join when test-account filters include person properties) legitimately run past 60s. Matches
# the error_tracking Datastore profile's execution ceiling — keep the two in sync.
DIGEST_MAX_EXECUTION_TIME_SECONDS = 120

# Keep in sync with SOURCE_MAPS_DOCS_URL in sourceMapsFixWizardLogic.ts
SOURCE_MAPS_DOCS_URL = "https://hanzo.ai/docs/error-tracking/upload-source-maps"
...
def query_daily_rows(team: Team) -> list:
    """Per-day counts over 14 days. Explicit LIMIT: InsightsQL appends LIMIT 100 to unlimited selects.
    ...
    response = execute_insightsql_query(
        ...
        """,
        team=team,
        filters=InsightsQLFilters(filterTestAccounts=True),
        # Weekly batch job: Celery pinned this to the offline cluster process-wide, Temporal does not.
        workload=Workload.OFFLINE,
        ch_user=DatastoreUser.ERROR_TRACKING,
        settings=InsightsQLGlobalSettings(max_execution_time=DIGEST_MAX_EXECUTION_TIME_SECONDS),
    )
...
def get_exception_summary_for_team(team: Team, daily_rows: list | None = None) -> dict:
    ...
    # Datastore returns team-timezone dates, so bucket against the team-local today, not UTC
...
# notification type in insights.tasks.email._DIGEST_PROJECT_SETTING_KEYS, which is module-private.
...
def auto_select_project_for_user(
    ...
    from insights.tasks.email_utils import auto_select_digest_project
...
def get_crash_free_sessions(team: Team) -> dict:
    ...
    # insights.tasks.__init__ eagerly imports every task module (celery autoimport);
    # import the helper at call time so this module doesn't pull the task graph.
    from insights.tasks.email_utils import compute_week_over_week_change  # noqa: PLC0415

    tag_queries(product=ProductKey.ERROR_TRACKING, team_id=team.pk, name="weekly_digest:crash_free_sessions")

    # A Datastore failure propagates (task fails and retries) rather than being swallowed into an
...
def _query_issue_rows(team: Team) -> list:
    ...
    with issue cardinality (Datastore caps query text at 1 MiB).
    """
    from insights.insightsql.constants import InsightsQLGlobalSettings
    from insights.insightsql.query import execute_insightsql_query
...
def _source_maps_wizard_command() -> str:
    ...
    return "npx -y @hanzo/wizard@latest upload-source-maps"
...
def build_team_digest_data(team: Team, daily_rows: list | None = None) -> dict[str, Any] | None:
    ...
    # Two bounded Datastore passes + crash-free: three queries per team instead of five.
    if daily_rows is None:
        daily_rows = query_daily_rows(team)
...
# Insights project. Protected by WORKFLOWS_WEBHOOK_SECRET, so the URL is not a secret.
DIGEST_WORKFLOW_WEBHOOK_URL = "https://webhooks.us.hanzo.ai/public/webhooks/019f2754-aeff-0000-6a0d-5d3933a94b08"
