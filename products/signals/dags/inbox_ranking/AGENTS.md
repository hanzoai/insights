...
- Register new jobs/schedules in `insights/dags/locations/signals.py`, keep the EU gate (`is_inbox_ranking_registered()`), and remember the ci-dagster paths filter must cover every module a dag file imports (`uv run .github/scripts/check-dagster-paths.py`).
...
- Cross-team Datastore reads stay on the offline workload with explicit guards; team-2 label queries keep their event + timestamp bounds aligned with the events sort key.

## Context that lives outside the repo

The cross-session design record (decision history, label catalog, schema mock, backlog) is the `inbox-ranking` skill in the Insights skills store — load it with `/phs inbox-ranking` before making design-level changes here.

## Verify loop

`pytest products/signals/dags/inbox_ranking/tests/`, then confirm the location resolves (import `insights.dags.locations.signals` under Django) and run repo-wide mypy.
