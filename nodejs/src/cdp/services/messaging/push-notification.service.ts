...
import type { CyclotronJobInvocationInsightsFunction, CyclotronJobInvocationResult, IntegrationType } from '../../types'
...
const APNS_JWT_CACHE_PREFIX = '@hanzo/apns-provider-jwt/'
...
//
// The key and the shape are fixed by the mobile SDKs: both read the single `insights` entry out of the
...
// sibling keys.
const PUSH_CORRELATION_KEY = 'insights'
...
function pushCorrelationData(invocation: CyclotronJobInvocationInsightsFunction): Record<string, string> {
...
function pushSendError(platform: PushPlatform, err: NormalizedPushError, retryAfterMs?: number): PushSendError {
    // Append the raw provider code so the failure surfaced to the script template stays debuggable, while
...
export class PushNotificationService {
    ...
    async executeSendPushNotification(
        ...
        const result = createInvocationResult<CyclotronJobInvocationInsightsFunction>(invocation, {}, { finished: true })
        ...
        // Fan out to every selected channel here rather than looping sendPushNotification in the script
        // template — a script function only runs one async call per invocation, so a per-channel loop in
        // script would only ever deliver to the first channel. Each channel is isolated: one failing must
        ...
        if (!success && retriableErrors.length > 0 && tries < this.fetchUtils.maxRetries) {
            ...
            // routes on an undefined queue type and resumes the script VM instead of re-running this send,
        ...
        // Surface the error to the script template too, otherwise its failure message renders blank.
    ...
    private async executeFcm(
        ...
        const templateId = result.invocation.insightsFunction.template_id ?? 'unknown'
