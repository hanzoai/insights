...
class TestRecalculationActivities(BaseTest):
    ...
    def test_mark_started_returns_none_when_force_failed_after_query_to_pinned(self):
        ...
        # string: that would pass the workflow's isinstance(str) check and let calc activities run Datastore
...
class TestCalculateActivity(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("cluster_at_capacity", DatastoreAtCapacity()),
        ]
    )
    def test_backpressure_bounce_defers_with_retry_delay_and_no_capture(self, name: str, exc: Exception):
        # A query bouncing off the per-org Datastore limiter or the cluster's at-capacity guard is
    ...
    @parameterized.expand(
        [
            ("out_of_memory", DatastoreQueryMemoryLimitExceeded(), "out_of_memory"),
    ...
    def test_permanent_error_fails_non_retryable_and_persists_on_first_attempt(
        ...
        # retry would burn a full Datastore query without changing the outcome.
    ...
    def test_query_to_is_passed_as_as_of_to_runner(self):
        # The run's shared query_to MUST be threaded into the Datastore query bounds via the runner's
    ...
    def test_query_id_is_persisted_on_result_row(self):
        # The deterministic client_query_id is stamped into Datastore's query_id and stored on the row so a
    ...
    def test_skips_query_when_completed_result_already_exists_for_this_fingerprint(self):
        ...
        # the activity returns success without re-running the Datastore query. The fingerprint is the
...
def _record_captures():
    """Patch the global hanzo_insights client so the analytics tests can assert the exact event
    ...
    with patch(
        "products.experiments.backend.temporal.recalculation_logic.hanzo_insights.capture",
        _fake_capture,
    ):
        yield captured
...
class TestRecalculationAnalytics(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("wrapped_oom", DatastoreQueryMemoryLimitExceeded(), "out_of_memory", True),
            ("wrapped_timeout", DatastoreQueryTimeOut(), "timeout", False),
            ("wrapped_at_capacity", DatastoreAtCapacity(), "rate_limited", True),
