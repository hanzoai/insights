...
export interface ExternalDataSourceSerializersApi {
    ...
    /** How this source was created. Defaults to `api` on create when omitted. `web` for the in-app UI, `api` for direct API callers, `mcp` for agent/MCP tool calls, `wizard` for the setup wizard and `self_driving` for the Insights Desktop app (both derived server-side from the caller's user agent). Ignored on update.
     *
     * * `web` - web
     * * `api` - api
     * * `mcp` - mcp
...
export interface ExternalDataSourceCreateApi {
    ...
    /** Where the request came from: `web` for the in-app UI, `api` for direct API callers, `mcp` for agent/MCP tool calls. `wizard` and `self_driving` cannot be set directly — they are derived server-side for wizard- and Insights Desktop-driven MCP calls. Defaults to `api`.
     *
     * * `web` - web
     * * `api` - api
     * * `mcp` - mcp */
...
export interface SourceConnectLinkApi {
    ...
    /** Full URL to share with the user. It opens the source's connection form in Insights — credentials never pass through the agent or the chat. */
    connect_url: string
...
export interface ExternalDataSourceConnectionOptionApi {
     ...
     * * `direct` - direct */
    readonly access_method: AccessMethodEnumApi
    /** Whether InsightsQL queries compile for this connection. When false, only raw SQL (sendRawQuery) works. */
    readonly supports_insightsql: boolean
    /**
...
export interface DirectConnectionSourceOptionApi {
    /** The source type to start a direct-connection setup for (e.g. 'Postgres', 'Datastore').
     *
     * * `Ashby` - Ashby
     * * `Supabase` - Supabase
     * * `CustomerIO` - CustomerIO
...
export interface SourceSetupWebhookApi {
    ...
    success: boolean
    /**
     * The Insights endpoint the external service delivers events to.
     * @nullable
     */
