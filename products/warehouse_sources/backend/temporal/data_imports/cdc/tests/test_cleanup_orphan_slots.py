...
def _cdc_job_inputs(*, enabled=True, management="insights", auto_drop_slot=True, slot="insights_slot", pub="insights_pub"):
    return {
        "cdc_enabled": enabled,
...
def test_deleted_insights_managed_drops_resources_and_schedule(team):
    _create_source(team, deleted=True, job_inputs=_cdc_job_inputs(management="insights"))
    adapter = _mock_adapter()
...
def test_critical_lag_insights_auto_drop_marks_broken_and_pauses(team):
