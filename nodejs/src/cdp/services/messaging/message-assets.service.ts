...
import { CyclotronJobInvocationInsightsFunction, CyclotronJobInvocationResult, MessageAssetRow } from '../../types'
...
const counterMessageAssetsCaptured = new Counter({
    ...
    help: 'Sent-email assets produced to the message_assets Kafka topic (Datastore-backed).',
    labelNames: ['kind'],
})
            ...
            await Promise.all(
                rows.map((row) =>
                    this.outputs.produce(MESSAGE_ASSETS_OUTPUT, {
                        ...
                        value: Buffer.from(safeDatastoreString(JSON.stringify(row))),
                    })
                )
            )
