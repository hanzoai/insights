...
class TestErrorTrackingQueryAPI(DatastoreTestMixin, APIBaseTest):
    ...
    def create_issue(self, issue_id: str | None = None, fingerprint: str | None = None) -> ErrorTrackingIssue:
        ...
        # the query always uses the denormalized Datastore table, so mirror the issue state there
    ...
    def test_rejects_insightsql_property_filters(self) -> None:
        ...
        assert "InsightsQL property filters are not supported here" in str(response.json())
    ...
    def test_issues_list_filters_by_assignee(self) -> None:
        ...
        with freeze_time(now() + timedelta(seconds=1)):
            sync_issues_to_datastore(issue_ids=[self.issue_id], team_id=self.team.pk)
        flush_persons_and_events()
    ...
    def test_issues_list_tags_datastore_queries(self) -> None:
    ...
    def test_issue_detail_tags_datastore_queries(self) -> None:
        self.create_issue()
        observed_tags: list[tuple[object, object]] = []
    ...
    def test_issue_events_tags_datastore_queries(self) -> None:
        self.create_issue()
        observed_tags: list[tuple[object, object]] = []
