...
from insights.settings import (
    DATASTORE_CLUSTER,
    DATASTORE_DATABASE,
    DATASTORE_KAFKA_WARPSTREAM_INGESTION_NAMED_COLLECTION,
)
...
def KAFKA_LOG_ENTRIES_WS_TABLE_SQL():
    ...
    """
        ...
        engine=kafka_engine(
            ...
            named_collection=DATASTORE_KAFKA_WARPSTREAM_INGESTION_NAMED_COLLECTION,
        ),
        extra_fields="",
    )
...
def LOG_ENTRIES_DISTRIBUTED_TABLE_SQL():
    return (LOG_ENTRIES_TABLE_BASE_SQL).format(
        ...
        on_cluster_clause=ON_CLUSTER_CLAUSE(False),
        extra_fields=KAFKA_COLUMNS,
        engine=Distributed(data_table=LOG_ENTRIES_SHARDED_TABLE, cluster=DATASTORE_CLUSTER, sharding_key="rand()"),
    )
