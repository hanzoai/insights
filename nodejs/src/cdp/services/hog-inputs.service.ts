...
export class HogInputsService {
    ...
    public async buildInputs(
        ...
        const inputs: InsightsFunctionType['inputs'] = {
            // Include the inputs from the script function
            ...
            ...(await this.loadIntegrationInputs(insightsFunction, newGlobals)),
        }

        const _formatInput = async (input: CyclotronInputType, key: string): Promise<any> => {
            const templating = input.templating ?? 'script'
        ...
        for (const schema of insightsFunction.inputs_schema ?? []) {
    ...
    private resolvePushSubscriptionInputs(
        ...
        newGlobals: InsightsFunctionInvocationGlobalsWithInputs
    ...
    public async loadIntegrationInputs(
        ...
        const allSchemas = [
            ...(insightsFunction.inputs_schema ?? []),
            ...(insightsFunction.mappings?.flatMap((m) => m.inputs_schema ?? []) ?? []),
        ]
        const allInputs: Record<string, CyclotronInputType | null | undefined> = {
            ...insightsFunction.inputs,
        }
        for (const mapping of insightsFunction.mappings ?? []) {
        ...
        if (newGlobals) {
            const pushSubscriptionInputs = this.resolvePushSubscriptionInputs(insightsFunction, returnInputs, newGlobals)
