...
const SEEN_PREFIX = '@hanzo/replay/session-seen'
const BLOCK_PREFIX = '@hanzo/replay/session-blocked'
...
describe('session-replay-pipeline rate limiter failure modes', () => {
    ...
    function buildPipeline() {
        ...
        return createSessionReplayPipeline({
            ...
            teamService: {
                getTeamByOrg: jest.fn().mockResolvedValue(team),
    ...
    function createMessage(sessionId: string, offset: number): Message {
        ...
        const headers = { org: 'test-org', session_id: sessionId, distinct_id: 'user-123' }
        return {
            partition: 0,
            offset,
            topic: 'test-topic',
