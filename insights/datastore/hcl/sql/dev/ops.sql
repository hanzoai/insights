...
-- Full CREATE schema for the dev/ops node. Apply to a fresh Datastore to build it.
...
CREATE TABLE insights.prom_metrics_data (
  id UUID,
  timestamp DateTime64(3),
  value Float64
) ENGINE = ReplicatedMergeTree('/datastore/ops/tables/{shard}/insights.prom_metrics/data', '{replica}') ORDER BY (id, timestamp) SETTINGS index_granularity = 8192;
CREATE TABLE insights.prom_metrics_metrics (
  metric_family_name String,
  type String,
  unit String,
  help String
) ENGINE = ReplicatedReplacingMergeTree('/datastore/ops/tables/{shard}/insights.prom_metrics/metrics', '{replica}') ORDER BY (metric_family_name) SETTINGS index_granularity = 8192;
CREATE TABLE insights.prom_metrics_tags (
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/ops/tables/{shard}/insights.prom_metrics/tags', '{replica}') PRIMARY KEY (metric_name) ORDER BY (metric_name, id) SETTINGS index_granularity = 8192;
...
CREATE TABLE insights.writable_prom_metrics_data (
  id UUID,
  timestamp DateTime64(3),
  value Float64
) ENGINE = Buffer('insights', 'prom_metrics_data', 1, 2, 5, 1, 1000000, 1, 100000000);
CREATE TABLE insights.writable_prom_metrics_metrics (
  metric_family_name String,
  type String,
  unit String,
  help String
) ENGINE = Buffer('insights', 'prom_metrics_metrics', 1, 2, 5, 1, 1000000, 1, 100000000);
CREATE TABLE insights.writable_prom_metrics_tags (
  id UUID DEFAULT reinterpretAsUUID(sipHash128(metric_name, all_tags)),
  metric_name LowCardinality(String),
  tags Map(LowCardinality(String), String),
  all_tags Map(String, String),
  min_time SimpleAggregateFunction(min, Nullable(DateTime64(3))),
  max_time SimpleAggregateFunction(max, Nullable(DateTime64(3)))
) ENGINE = Buffer('insights', 'prom_metrics_tags', 1, 2, 5, 1, 1000000, 1, 100000000);
...
CREATE VIEW insights.custom_metrics_test AS SELECT
  ...
  'gauge' AS type;
CREATE TABLE insights.prom_metrics (
  id UUID DEFAULT reinterpretAsUUID(sipHash128(metric_name, all_tags)),
  timestamp DateTime64(3),
  value Float64,
  metric_name LowCardinality(String),
  ...
  unit String,
  help String
) ENGINE = TimeSeries SAMPLES insights.prom_metrics_data TAGS insights.prom_metrics_tags METRICS insights.prom_metrics_metrics;
...
CREATE TABLE insights.writable_prom_metrics (
  id UUID DEFAULT reinterpretAsUUID(sipHash128(metric_name, all_tags)),
  timestamp DateTime64(3),
  value Float64,
  metric_name LowCardinality(String),
  ...
  unit String,
  help String
) ENGINE = TimeSeries SAMPLES insights.writable_prom_metrics_data TAGS insights.writable_prom_metrics_tags METRICS insights.writable_prom_metrics_metrics;
