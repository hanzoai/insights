...
description: >
  Analyze Insights insights, dashboards, or teams beyond the current project by
  querying the prod Postgres replicas synced into the dogfood data warehouse
  (US project 2, "Insights App + Website"). Use when asked to analyze insights
...
so cross-team entity metadata **is** queryable with `insights:execute-sql`.
Do not stop at `system.insights` when the question spans teams.

This skill is deliberately repo-local (`.agents/skills/`): it documents Insights's internal dogfood setup,
...
- Underscore aliases (e.g. `postgres_insights_dashboarditem`) point at the same synced data.
- These are replicas of the Django tables in this repo (`insights_dashboarditem` backs the `Insight` model), so rows span every team; `team_id` is the scoping column.
...
1. Confirm columns before projecting — synced schemas drift with the Django models:
   ...
   WHERE table_name = 'postgres.insights_dashboarditem'
   ```

2. Query with `insights:execute-sql`, filtering or grouping by `team_id`. Example — most active teams by insights created in the last 30 days:

   ```sql
   SELECT team_id, count() AS insights_created
   FROM postgres.insights_dashboarditem
   ...
   Join `postgres.insights_team` on `id = team_id` for team names only when the output stays on an internal surface (see below).
...
- For cross-team **event/analytics** data (not entity metadata), see the `query-datastore-via-metabase` skill instead.
