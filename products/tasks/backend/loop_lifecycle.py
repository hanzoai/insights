...
Wired from the ``pre_save`` signal on ``User`` in ``insights/models/user.py`` and the GitHub
``installation`` webhook in ``insights/api/github_callback/installation_events.py``.
"""
