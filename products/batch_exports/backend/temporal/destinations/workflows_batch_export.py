...
INSIGHTS_FUNCTION_API_PATH = "/v1/projects/{team_id}/insights_functions/{insights_function_id}/batch_export_invocations"
...
class InsightsFunctionErrorThresholdExceeded(Exception):
    """Raised when too many Script Function executions fail with status=error."""
    ...
    def __init__(self, failed_count: int, total_count: int, latest_error: str | None = None):
        ...
        message = f"Script Function error rate above threshold: {failed_count}/{total_count} executions failed."
        if latest_error:
...
class WorkflowsConsumer(Consumer):
    """Consumer that posts each record as a Script Function invocation to the CDP API.
    ...
    A 2xx response with a `{"status": "error", ...}` body is treated as a Script Function
    ...
    `insights_function_error_threshold_pct` — once at least
    `insights_function_error_threshold_min_records` records have been handled —
    the consumer aborts the run by raising a non-retryable
    `InsightsFunctionErrorThresholdExceeded` exception.
    """

    def __init__(
        self,
        url: str,
        insights_function_id: str,
        team_id: int,
        ...
        insights_function_error_threshold_pct: float = 0.5,
        insights_function_error_threshold_min_records: int = 100,
    ):
        ...
        self.insights_function_error_threshold_pct = insights_function_error_threshold_pct
        self.insights_function_error_threshold_min_records = insights_function_error_threshold_min_records
        self.records_handled_count = 0
        self.latest_insights_function_error: str | None = None
            ...
            async with self.session.post(
                ...
                try:
                    ...
                    if response_body.get("status") == "error":
                        ...
                        self.logger.warning("Script Function execution failed", errors=errors)
                        self.records_failed_count += 1
                        if errors:
                            self.latest_insights_function_error = errors[-1]

                        if (
                            self.records_handled_count >= self.insights_function_error_threshold_min_records
                            and self.records_failed_count / self.records_handled_count
                            >= self.insights_function_error_threshold_pct
                        ):
                            raise InsightsFunctionErrorThresholdExceeded(
                                failed_count=self.records_failed_count,
                                total_count=self.records_handled_count,
                                latest_error=self.latest_insights_function_error,
                            )
    ...
    async with Heartbeater():
        ...
        async with aiohttp.ClientSession(
            ...
            consumer = WorkflowsConsumer(
                inputs.url,
                insights_function_id=inputs.insights_function_id,
                team_id=inputs.batch_export.team_id,
                session=session,
            ...
            # exist). If you are adding a TaskGroup inside this TaskGroup revisit this!
            ...
            except* InsightsFunctionErrorThresholdExceeded:
                ...
                exc = InsightsFunctionErrorThresholdExceeded(
                    failed_count=consumer.records_failed_count,
                    total_count=consumer.records_handled_count,
                    latest_error=consumer.latest_insights_function_error,
                )
