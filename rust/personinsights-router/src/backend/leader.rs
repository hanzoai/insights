...
use personinsights_common::grpc::current_client_name;
...
const LEADER_PREFIX: &str = "/personinsights.leader.v1.PersonHogLeader/";
...
impl LeaderBackend {
    ...
    /// `personinsights_router_forward_retries_total` under the reason that
    /// caused it. After `MAX_CONSECUTIVE_BOUNCES` the request fails with
    /// a definitive `UNAVAILABLE`, counted by
    /// `personinsights_router_forward_retries_exhausted_total`, so a router
    ...
    /// `personinsights-leader`'s README.
                ...
                .await
            {
                ...
                ForwardDecision::Bounced(reason) => {
                    ...
                    if consecutive_bounces >= MAX_CONSECUTIVE_BOUNCES {
                        counter!("personinsights_router_forward_retries_exhausted_total").increment(1);
                        return (
                            grpc_error_response(
                                Code::Unavailable,
