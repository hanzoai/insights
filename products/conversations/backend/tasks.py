...
from insights.models.comment import Comment as CommentModel
...
def is_duplicate_teams_event(activity_id: str) -> bool:
    ...
    """
    key = f"{SUPPORTFN_TEAMS_EVENT_IDEMPOTENCY_KEY_PREFIX}{activity_id}"
    return not cache.add(key, True, timeout=SUPPORTFN_EVENT_IDEMPOTENCY_TTL_SECONDS)
...
def _is_duplicate_github_event(delivery_id: str) -> bool:
    key = f"{SUPPORTFN_GITHUB_EVENT_IDEMPOTENCY_KEY_PREFIX}{delivery_id}"
    return cache.get(key) is not None


def _mark_github_event_processed(delivery_id: str) -> None:
    key = f"{SUPPORTFN_GITHUB_EVENT_IDEMPOTENCY_KEY_PREFIX}{delivery_id}"
    cache.set(key, True, timeout=SUPPORTFN_EVENT_IDEMPOTENCY_TTL_SECONDS)
...
def _get_or_create_github_ticket(team: Team, repo: str, issue_number: int, payload: dict[str, Any]) -> Ticket:
    ...
    insights_con_github_issue_uniq to guarantee exactly one ticket per issue.
    """
