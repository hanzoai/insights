...
from insights.tasks.calculate_cohort import trigger_cohort_events_backfill_task
...
class TestPersonBackfillRuns(BaseTest):
    def setUp(self) -> None:
        super().setUp()
        feature_patch = mock.patch(
            "products.cohorts.backend.models.dependencies.hanzo_insights.feature_enabled",
            return_value=False,
        )
        feature_patch.start()
        self.addCleanup(feature_patch.stop)
