...
export const BillingExemptReasonEnumApi = {
    InsightsHealthCheck: 'insights_health_check',
    InsightsOnboarding: 'insights_onboarding',
    InsightsSystem: 'insights_system',
} as const
...
export interface SignalReportApi {
    ...
    /** Distinct source products contributing signals to this report (from Datastore). */
    readonly source_products: readonly string[]
    /**
     * skill_name slug of the scout that authored this report, when scout-authored (from Datastore); null otherwise.
     * @nullable
     */
    ...
    /** Non-null when this report is system-marked never-billable (Insights-system origin, e.g. a health-check scout finding) — its implementation PRs are free and cannot be refunded because nothing was charged.
     *
     * * `insights_health_check` - Insights health check
     * * `insights_onboarding` - Insights onboarding
     * * `insights_system` - Insights system */
...
export interface SignalNodeApi {
    /** Datastore document id of the signal. */
    signal_id: string
...
export interface SignalScoutConfigOptionsApi {
    ...
    /** What the scout's sandbox can reach over the network while it runs. Defaults to `trusted`, the platform's trusted-domain allowlist (Insights, GitHub, common package registries). Set `full` to let this scout reach any site, for skills that read external sources such as documentation or papers.
     *
     * * `trusted` - Trusted domains only
     * * `full` - Full */
...
export interface SignalScoutConfigApi {
    ...
    /** What the scout's sandbox can reach over the network while it runs. `trusted` (the default) restricts runs to the platform's trusted-domain allowlist (Insights, GitHub, common package registries). `full` lets the scout reach any site, for skills that read external sources such as documentation or papers.
     *
     * * `trusted` - Trusted domains only
     * * `full` - Full */
...
export interface ScoutMemberApi {
    /** The member's stable Insights user UUID — the same id that appears as `created_by.uuid` on entities they own. A durable handle for this person across runs. */
    user_uuid: string
...
export interface RecentDashboardEntryApi {
    ...
    name: string
    /**
     * ISO-8601 timestamp of the most recent view in the Insights UI.
     * @nullable
     */
...
export interface RecentFeatureFlagEntryApi {
    /** Feature flag ID. */
    id: number
    /** Flag key used in code (`insights.isFeatureEnabled('<key>')`). */
    key: string
...
/**
 * One row in `inventory.recent_insights_functions.recent`.
 */
export interface RecentInsightsFunctionEntryApi {
...
/**
 * `inventory.recent_insights_functions` — total + enabled count, plus the 5 most recently modified.
 */
export interface RecentInsightsFunctionsApi {
    /** Total non-deleted script functions on the team. */
    total_count: number
    /** Script functions currently enabled (`enabled=true`). */
    enabled_count: number
    /** The 5 most recently updated script functions. */
    recent: RecentInsightsFunctionEntryApi[]
}
...
export interface RecentInsightsFlowsApi {
    ...
    total_count: number
    /** Script flows that are not archived. */
    active_count: number
    /** The 5 most recently updated script flows. */
    recent: RecentInsightsFlowEntryApi[]
}
...
export interface SuggestedReviewerApi {
     ...
     */
    github_login?: string
    /** Insights user UUID (e.g. from `scout-members-list`, or an entity's `created_by`). Resolved server-side to the member's linked GitHub login — use this when you know the Insights user but not their GitHub handle. Must be a concrete UUID; the `@me` alias is not valid here. */
    user_uuid?: string
    /**
 ...
 * reports lookup, one Datastore round-trip — per emitted run) into a single request.
 */
...
export interface SignalUserAutonomyConfigApi {
    ...
    /**
     * Slack channel target in the same `channel_id|#channel-name` shape Insights uses elsewhere (only the channel id is required). Null disables Slack notifications.
     * @maxLength 255
     * @nullable
     */
...
export type SignalsReportsListParams = {
    ...
    status?: string
    /**
     * Comma-separated list of Insights user UUIDs. Reports are kept if their suggested reviewers include any of the given users.
     */
