...
import { createFlowInvocationContext, insertFlow } from '../_tests/fixtures-flows'
import {
...
describe('CdpCyclotronWorkerFlow', () => {
    let processor: CdpCyclotronWorkerFlow
    let hub: Hub
    let team: Team
    let team2: Team
    let flows: Flow[]

    const createSerializedFlowInvocation = (
        flow: Flow,
    ...
    beforeEach(async () => {
        ...
        processor = new CdpCyclotronWorkerFlow(
            hub,
    ...
    describe('loadFlows', () => {
        ...
        beforeEach(() => {
            const invocation1 = createSerializedFlowInvocation(flows[0], {
                event: {
            ...
            const invocation2 = createSerializedFlowInvocation(flows[0], {
                event: {
            ...
            const invocation3 = createSerializedFlowInvocation(flows[1], {
                event: {
            ...
            const invocation4 = createSerializedFlowInvocation(flows[1], {
                event: {
        ...
        it('should load script flows and their persons and globals', async () => {
            const results = (await processor.processInvocations(
                invocations
            )) as CyclotronJobInvocationResult<CyclotronJobInvocationFlow>[]

            const toMinimalCompare = (
                result: CyclotronJobInvocationResult<CyclotronJobInvocationFlow>
            ): Record<string, unknown> => {
                return {
                    flowName: result.invocation.flow.name,
                ...
                [
                  {
                    ...
                    "flowName": "Test Script Flow team 2",
        ...
        it('should resolve person by personId when distinct_id is empty (batch invocations)', async () => {
            ...
            const invocation = createSerializedFlowInvocation(flows[0], {
                event: {
                    distinct_id: '',
        ...
        it('should skip invocations when workflow is disabled after being queued', async () => {
            ...
            const invocation1 = createSerializedFlowInvocation(flow, {
                event: {
            ...
            const invocation2 = createSerializedFlowInvocation(flow, {
                event: {
