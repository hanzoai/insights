...
from insights.temporal.common.utils import retry_on_db_connection_drop
...
def merge_columns(
    ...
    results during a sync (e.g., transient S3/Datastore introspection failures).
    """
    ...
    for column_name, db_column_type in db_columns.items():
        ...
        existing_insightsql_type = existing_column.get("insightsql") if isinstance(existing_column, dict) else None
        if existing_insightsql_type == "StringJSONDatabaseField" and insightsql_type == "StringDatabaseField":
            insightsql_type = "StringJSONDatabaseField"
    ...
    for column_name, column_meta in existing_columns.items():
        ...
        if isinstance(column_meta, dict):
            ...
            columns[column_name] = {
                "datastore": column_meta,
                "insightsql": table_schema_dict.get(column_name, "StringDatabaseField"),
            }
...
async def validate_schema_and_update_table(
    ...
    def _validate_and_update():
        ...
        # The InsightsQL table name derives from the raw schema name (only lower-cased); the S3 folder is
        ...
        try:
            ...
            # Resolve (or create) the target table and introspect its Datastore schema BEFORE
            # opening a transaction. get_count()/get_columns() hit Datastore and retry with
            ...
            if table_created:
                ...
                # get_count() above can retry against a degraded Datastore cluster for minutes, long
            ...
            db_columns = {key: str(column.get("datastore", "")) for key, column in raw_db_columns.items()}
            ...
            def _persist_columns() -> None:
                with transaction.atomic():
                    ...
                    # creep back into InsightsQL via the Delta schema (which still contains them historically).
            ...
            # get_columns() above retries against Datastore the same way and can also block for
...
async def register_cdc_companion_table(
    ...
    def _register():
        ...
        try:
            # Resolve (or create) the companion table and introspect its Datastore schema BEFORE
            # opening a transaction — get_count()/get_columns() hit Datastore and can take minutes
