...
[fix(dashboards): tolerate legacy keys in persisted dashboard filters](https://github.com/Insights/insights/pull/78235).
...
[fix(data-warehouse): recognize Neon's pooler rejection of libpq options](https://github.com/Insights/insights/pull/77899). Problem and Changes only.
...
On the CDC path that connect is the first thing `cdc_extract_activity` does, and the stream reader always sends `options` (`statement_timeout=1800000 -c idle_in_transaction_session_timeout=0`). So every extraction run against a Neon pooled endpoint fails at `stream_reader.connect()`, forever. It classifies as a generic retryable `connection_failed`, so the user sees "Could not connect to the source database. Insights will keep retrying — if this persists, check that the database is reachable and accepting connections" for a database that is reachable and perfectly healthy. There is nothing they can do to make that message go away, because it isn't describing their problem.
...
[fix(flags): drop custom codecs from flag_evaluations columns](https://github.com/Insights/insights/pull/77910). Problem and Changes only.
...
`insights/models/flag_evaluations/sql.py` is the shared column template other DDL reads from, so the per-column `CODEC(...)` clauses are stripped there too. The HCL golden/SQL mirrors and the `test_schema.ambr` snapshot are regenerated to match.
```
...
- `insights/models/flag_evaluations/sql.py` is the shared column template, so the `CODEC(...)` clauses are stripped there too.
