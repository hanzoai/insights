...
def _make_mcp_config(name: str = "insights", token: str = "tok") -> McpServerConfig:
...
class TestRefreshSandboxMcp:
    ...
    def test_refresh_keeps_imported_mcp_servers(
        ...
        task_run.imported_mcp_servers = [
            ...
            # collides with the Insights MCP config: existing servers win
            {"type": "http", "name": "insights", "url": "https://shadow.example.com/mcp", "headers": []},
        ]

        _refresh(task_run, auth_token="jwt")

        mcp_servers = mock_send_refresh.call_args.args[1]
        assert [server["name"] for server in mcp_servers] == ["insights", "grafana"]
...
class TestSendFollowupActivityRefreshOrdering:
    ...
    def test_refresh_called_before_user_message(self, _patches):
        ...
        send_followup_to_sandbox(SendFollowupToSandboxInput(run_id="run-1", message="hi", insights_mcp_scopes="full"))
