...
use insights_rs::{ErrorTrackingOptionsBuilder, Event};
...
/// Captures the `insights cli command run` usage event without requiring an
...
pub fn capture_command_run_without_context() -> JoinHandle<()> {
    std::thread::spawn(|| {
        ...
        insights_rs::capture(Event::new_anon("insights cli command run"));
...
pub fn init_insights_telemetry() {
    ...
    match insights_rs::init_global(ph_config) {
        Ok(()) => {}
        Err(insights_rs::Error::AlreadyInitialized) => {
            debug!("Insights client already initialized");
        }
        Err(err) => {
            debug!("Insights client unavailable: {err:?}");
        }
    }
}
...
pub fn init_context(
    ...
    let config = InvocationConfig {
        ...
        host: host.unwrap_or(token.host.unwrap_or("https://us.i.hanzo.ai".into())),
...
impl InvocationContext {
    ...
    pub fn capture_command_invoked(&self, command: &str) {
        ...
        self.capture_event("insights cli command run", Vec::new());
    }
