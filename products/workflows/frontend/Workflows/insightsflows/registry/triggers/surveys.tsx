...
export function isSurveyTriggerConfig(config: Extract<InsightsFlowAction, { type: 'trigger' }>['config']): boolean {
...
export function getSelectedSurveyId(config: InsightsFlowAction['config']): string | null | 'any' {
...
function StepTriggerConfigurationSurvey({ node }: { node: any }): JSX.Element {
    ...
    return (
        <>
            <Field.Pure label="Select a survey">
                <Select
            ...
            {(() => {
                if (selectedSurveyId === 'any' && !surveysLoading && allSurveys.length === 0) {
                    return (
                        <Banner type="warning" className="w-full">
                            <p>
            ...
            <Field.Pure label="Only trigger for specific answers">
