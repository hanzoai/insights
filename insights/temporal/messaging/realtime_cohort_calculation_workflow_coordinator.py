...
from insights.temporal.messaging.constants import get_child_workflow_id
...
class RealtimeCohortCalculationCoordinatorWorkflowInputs:
    ...
    def __post_init__(self):
        ...
        if self.team_ids is None:
            from insights.settings.schedules import REALTIME_COHORT_CALCULATION_TEAMS

            self.team_ids = REALTIME_COHORT_CALCULATION_TEAMS.copy()

        if self.global_percentage is None:
            from insights.settings.schedules import REALTIME_COHORT_CALCULATION_GLOBAL_PERCENTAGE
