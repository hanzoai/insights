...
# Simple 1:1 name swaps (args unchanged). Datastore/InsightsQL name → Snowflake name.
...
def _handle_match(args: list[str]) -> str:
    # Datastore match() is a partial (search) match; Snowflake REGEXP_LIKE is
...
def _handle_log(args: list[str]) -> str:
    # InsightsQL log() is natural log; Snowflake LOG requires a base, so use LN.
    return f"LN({args[0]})"
...
def _handle_count(args: list[str]) -> str:
    # InsightsQL's argless count() means "count all rows", but Snowflake rejects a bare COUNT() —
