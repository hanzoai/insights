...
from insights.management.migration_analysis.utils import VolatileFunctionDetector, check_drop_properly_staged

# Base URL for migration safety documentation
SAFE_MIGRATIONS_DOCS_URL = (
    "https://github.com/Insights/insights/blob/master/docs/published/handbook/engineering/safe-django-migrations.md"
)
...
class AddConstraintAnalyzer(OperationAnalyzer):
    ...
    def analyze(self, op) -> OperationRisk:
        ...
        return OperationRisk(
            ...
            details={"model": model_name},
            guidance=f"""Add constraints in 2 phases without locking, using the Insights helpers:
...
2. ValidateConstraint in a separate migration (scans table with non-blocking lock)
    ...
    from insights.migration_helpers import AddConstraintNotValid, ValidateConstraint
...
class _SafeConcurrentIndexAnalyzer(OperationAnalyzer):
    """Base for the Insights concurrent-index helpers.
    ...
    default_score = 1
    safe_reason = "Insights concurrent-index helper: idempotent (timeout disabling + invalid-leftover recovery)"
