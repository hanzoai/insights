...
import { AnyPropertyFilter, CyclotronJobFiltersType, CustomFunctionConfigurationContextId } from '~/types'
 ...
 */
export const getProductEventFilterOptions = (contextId: CustomFunctionConfigurationContextId): FilterOption[] => {
    switch (contextId) {
        case 'error-tracking':
            return [
                {
...
export const getProductEventPropertyFilterOptions = (contextId: CustomFunctionConfigurationContextId): string[] => {
    switch (contextId) {
        case 'activity-log':
            return [
                'id',
...
export function CustomFunctionFiltersInternal(): JSX.Element {
    const { contextId } = useValues(customFunctionConfigurationLogic)
                    ...
                    <>
                        ...
                        {taxonomicGroupTypes.length > 0 ? (
                            <PropertyFilters
                                ...
                                pageKey={`custom-function-internal-property-filters-${contextId}`}
                                buttonSize="small"
                                disablePopover
                            />
                        ) : null}
