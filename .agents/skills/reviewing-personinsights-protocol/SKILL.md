---
name: reviewing-personinsights-protocol
description: >
  The full review process for personinsights coordination-protocol changes —
  ...
  personinsights protocol changeset, when asked for an exhaustive or careful
  review of personinsights code, and after any reviewer finds a gap the author
...
# Reviewing personinsights protocol changes
...
of layers a personinsights change must clear before it ships.
...
2. **Independent adversarial pass.** Spawn a cold-context agent on the
   ...
   ([references/personinsights-invariants.md](references/personinsights-invariants.md))
...
A personinsights protocol change is not reviewed by reasoning alone. Check
each layer, in order of cost:

1. **Decision-logic coupling (stateright).** `personinsights-stateright`
...
2. **Protocol integration tests** (`personinsights-coordination/tests/`,
   ...
   lines.
3. **The e2e harness gates** (`personinsights-test-harness gate`): run the CI
...
5. **Observability and the residual ledger.** New failure modes need
   ...
   response, dashboard panels. `rust/personinsights-coordination/README.md`
...
- `/adding-personinsights-rpc` — the data-plane counterpart (RPCs, storage,
  routing); this skill owns the coordination plane.
- `rust/personinsights-coordination/README.md` — the protocol's own design and
