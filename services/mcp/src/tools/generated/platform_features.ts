...
const advancedActivityLogsList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedActivityLogList>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof AdvancedActivityLogsListSchema>) => {
        ...
        const result = await context.api.request<Schemas.PaginatedActivityLogList>({
            ...
            query: {
                ...
                insightsql_filter: params.insightsql_filter,
        ...
        return await withInsightsUrl(context, filtered, '/activity')
    },
})
...
const changeRequestsList = (): ToolBase<
    ...
    WithInformationalResponse<WithInsightsUrl<Schemas.PaginatedChangeRequestList>>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof ChangeRequestsListSchema>) => {
        ...
        return withInformationalResponse(
            await withInsightsUrl(context, filtered, '/'),
            'change-request-content',
...
const OrgMemberGetGithubLoginSchema = MembersGithubLoginRetrieveParams.omit({ organization_id: true }).extend({
    user__uuid: MembersGithubLoginRetrieveParams.shape['user__uuid'].describe(
        'The Insights user UUID of the organization member, as returned by org-members-list. Pass "@me" for the current user.'
    ),
})
...
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof OrganizationEnforce2faSchema>) => {
        ...
        return await prepareConfirmedAction(context, {
            ...
            messageTemplate:
                "About to set organization-wide two-factor-authentication enforcement to {enforce_2fa}. This immediately affects every member of the organization — when enabled, all members must set up 2FA before they can continue using Insights. Reply 'confirm' to proceed.\n",
            codec: __runtime.codec,
            stash: __runtime.stash,
        })
    },
...
const organizationsList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedOrganizationList>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof OrganizationsListSchema>) => {
        ...
        return await withInsightsUrl(context, filtered, '/')
    },
})
