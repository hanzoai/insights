...
# Pagination shape mirrors session_summary's fetcher; without it InsightsQL applies LimitContext.QUERY's default of 100.
...
# `properties.*` is the InsightsQL prefix for JSON properties; `uuid` is surfaced to the LLM as the `event_uuid` citation handle.
_EXTRA_FIELDS = [
    "uuid",
    "elements_chain_ids",
...
async def fetch_session_events_activity(inputs: FetchSessionEventsInputs) -> None:
    ...
    # Persist the session identity so downstream steps read it off the row instead of re-querying Datastore.
...
def _fetch_payload(team_id: int, session_id: str) -> ScannerLlmInputs | None:
    ...
    metadata = events_obj.get_metadata(session_id=session_id, team=team, ch_user=DatastoreUser.REPLAY_VISION)
    if metadata is None:
        raise IneligibleSessionError(
