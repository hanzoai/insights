...
from insights.models import Organization, ProxyRecord
from insights.temporal.proxy_service.monitor import PROXY_LIVE_CHECK_TIMEOUT_S, CheckActivityInput, check_proxy_is_live
...
class TestCheckProxyIsLive(TestCase):
    ...
    @patch("insights.temporal.proxy_service.monitor.socket.create_connection")
    @patch("insights.temporal.proxy_service.monitor.ssl.create_default_context")
    @patch("insights.temporal.proxy_service.monitor.requests.post")
    @patch("insights.temporal.proxy_service.monitor.get_record")
