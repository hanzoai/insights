"""
Real-Datastore integration tests for point-in-time person properties.
...
green. Each test now seeds events into Datastore and exercises the real query.
"""
...
from insights.models.person.point_in_time_properties import build_person_properties_at_time


class TestPointInTimePropertiesDatastore(DatastoreTestMixin, BaseTest):
    """Hits the real Datastore query — guards against SQL typos and the
    column-order / ORDER-BY-through-UNION-ALL pitfalls."""

    def test_chronological_set_resolution(self):
        distinct_id = "user-datastore-chrono"
    ...
    def test_set_once_first_write_wins(self):
        distinct_id = "user-datastore-set-once"
        upper_bound = datetime(2024, 6, 1, 12, 0, 0, tzinfo=UTC)
    ...
    def test_set_then_set_once_interleaving(self):
        ...
        distinct_id = "user-datastore-interleave"
        upper_bound = datetime(2024, 6, 1, 12, 0, 0, tzinfo=UTC)
    ...
    def test_only_non_property_events_returns_not_existed(self):
        ...
        distinct_id = "user-datastore-pageview-only"
        upper_bound = datetime(2024, 6, 1, 12, 0, 0, tzinfo=UTC)
    ...
    def test_row_limit_truncates_oldest_first(self):
        ...
        distinct_id = "user-datastore-rowlimit"
        upper_bound = datetime(2024, 6, 1, 12, 0, 0, tzinfo=UTC)
        row_limit = 5
    ...
    def test_happy_path_with_default_row_limit(self):
        ...
        distinct_id = "user-datastore-default-limit"
        upper_bound = datetime(2024, 6, 1, 12, 0, 0, tzinfo=UTC)
