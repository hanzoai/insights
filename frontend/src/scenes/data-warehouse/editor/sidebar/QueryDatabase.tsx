...
import { escapeDottedInsightsQLIdentifier, escapePropertyAsInsightsQLIdentifier } from '~/queries/utils'
...
export function getColumnInsertText(record: Record<string, any> | undefined): string | null {
    ...
    return escapeDottedInsightsQLIdentifier(record.columnName)
}
...
export const QueryDatabase = ({
    ...
    const warehouseAccessControlEnabled =
        !!featureFlags[FEATURE_FLAGS.INSIGHTSQL_WAREHOUSE_ACCESS_CONTROL] && !isEmbeddedMode
        ...
        <Tree
            ...
            itemSideAction={(item) => {
                ...
                if (item.record?.type === 'table') {
                    // Per-object access control only exists for actual warehouse tables, not insights/system tables
                        ...
                        <DropdownMenuGroup>
                            ...
                            <DropdownMenuItem asChild>
                                <Link
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation()
                                        insights.capture('sql-editor-add-source-clicked', {
