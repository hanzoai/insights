...
/// Datastore collapses toward the new incarnation, and reset properties.
...
async fn insert_or_revive_persons(
    ...
    let inserted = sqlx::query!(
        ...
        ON CONFLICT (team_id, uuid) DO UPDATE SET
            is_deleted = false,
            version = COALESCE(insights_person.version, 0) + 1,
            properties = '{}'::jsonb,
            ...
            WHERE insights_person.is_deleted = true
...
async fn insert_distinct_id_mappings(
    ...
    let rows = sqlx::query!(
        ...
        ON CONFLICT (team_id, distinct_id) DO UPDATE SET
            ...
            version = COALESCE(insights_persondistinctid.version, 0) + 1,
            is_deleted = false
            WHERE insights_persondistinctid.is_deleted = true
...
async fn resolve_stub_outcomes(
    ...
    for (i, stub) in stubs.iter().enumerate() {
        ...
        let existing = sqlx::query_scalar!(
            "SELECT person_id FROM insights_persondistinctid WHERE team_id = $1 AND distinct_id = $2 AND is_deleted = false",
            stub.team_id as i32,
            &stub.distinct_id
        )
...
async fn undo_created_person(
    ...
    sqlx::query!(
        "DELETE FROM insights_persondistinctid WHERE team_id = $1 AND person_id = $2 AND is_deleted = false",
        team_id as i32,
        resolved.person.id
    )
    .execute(&mut **tx)
