...
from insights.dags.common.resources import OpsDatastoreClusterResource

SOURCE_TABLE = "insights.query_log_archive"
...
# Datastore folds leaf read_rows/read_bytes into the initial row via progress packets, but
...
class QueryLogArchiveExportConfig(dagster.Config):
    max_threads: int = pydantic.Field(
        default=12,
        ge=1,
        description="Datastore max_threads for the export scan. Must be ≥ 1 (0 means auto/all-cores in Datastore and will re-enable the OOM risk).",
    )
...
SELECT
    ...
    normalizeQuery(lc_query__query) AS insightsql_shape,
    leaf_count,
...
@dagster.job(
    ...
    resource_defs={
        "cluster": OpsDatastoreClusterResource(max_execution_time=2 * 60 * 60, max_memory_usage=20 * ONE_GB)
    },
    tags={
