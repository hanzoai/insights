...
class TestFetchTraceForEvaluation:
    ...
    def test_skips_when_no_events_found(self, setup_data):
        ...
        with patch("insights.temporal.ai_observability.run_trace_evaluation._count_trace_events", return_value=0):
    ...
    def test_skips_oversized_traces_without_fetching(self, setup_data):
        ...
        with patch(
            "insights.temporal.ai_observability.run_trace_evaluation._count_trace_events",
            return_value=MAX_TRACE_EVAL_EVENTS + 1,
        ):
            with patch("insights.temporal.ai_observability.run_trace_evaluation.TraceQueryRunner") as mock_runner:
                outcome = fetch_trace_for_evaluation(team.id, "trace-123", FROZEN_NOW)
    ...
    def test_returns_trace_when_found(self, setup_data):
        ...
        with patch("insights.temporal.ai_observability.run_trace_evaluation._count_trace_events", return_value=1):
            with patch("insights.temporal.ai_observability.run_trace_evaluation.TraceQueryRunner") as mock_runner:
...
class TestEmitTraceEvaluationEventActivity:
    ...
    async def test_emits_event_targeting_trace(self, setup_data):
        ...
        with patch("insights.temporal.ai_observability.run_trace_evaluation.Team.objects.get", return_value=team):
            with patch("insights.temporal.ai_observability.run_trace_evaluation.capture_internal") as mock_capture:
    ...
    async def test_emits_event_billing_limit(self, setup_data, status_code: int, should_raise: bool):
        ...
        with patch("insights.temporal.ai_observability.run_trace_evaluation.Team.objects.get", return_value=team):
            with patch(
                "insights.temporal.ai_observability.run_trace_evaluation.capture_internal", return_value=capture_result
            ):
                if should_raise:
...
class TestEmitSessionEvaluationEvent:
    ...
    def test_target_linkage_per_target(
        ...
        with (
            patch("insights.temporal.ai_observability.run_trace_evaluation.capture_internal", side_effect=_capture),
            patch("insights.temporal.ai_observability.run_trace_evaluation.Team.objects.get", return_value=MagicMock()),
        ):
