...
# `Authorization` header on every POST. Insights generates the password, registers the URL with it
# and stores the expected header on the webhook function.
WEBHOOK_BASIC_AUTH_USERNAME = "insights"
...
def _webhook_table_transformer(table: pa.Table) -> pa.Table:
    ...
    for row in table.to_pylist():
        ...
        if isinstance(event_id, str) and event_id:
            ...
            # Shouldn't happen — the Script template always sets it — but dropping rows silently
...
def create_webhook(api_key: str, secret_key: str, webhook_url: str) -> WebhookCreationResult:
    ...
    function via `extra_inputs` so the Script template can reject anything else.
    """
