...
import {
    personinsightsConnectionEstablishmentSeconds,
    personinsightsConnectionState,
    personinsightsConnectionStateTransitionsTotal,
    personinsightsStreamAcquisitionSeconds,
    personinsightsStreamsInFlight,
} from './metrics'
...
export class SessionStateMonitor implements SessionManager {
    ...
    private readonly acquisitionMetric: ReturnType<typeof personinsightsStreamAcquisitionSeconds.labels>
    private readonly streamsMetric: ReturnType<typeof personinsightsStreamsInFlight.labels>
    private readonly establishmentMetric: ReturnType<typeof personinsightsConnectionEstablishmentSeconds.labels>

    constructor(inner: Http2SessionManager, clientName: string, pollIntervalMs: number = 5_000) {
        this.inner = inner
        this.clientName = clientName
        this.acquisitionMetric = personinsightsStreamAcquisitionSeconds.labels({ client: clientName })
        this.streamsMetric = personinsightsStreamsInFlight.labels({ client: clientName })
        this.establishmentMetric = personinsightsConnectionEstablishmentSeconds.labels({ client: clientName })
    ...
    private setStateGauge(activeState: SessionState): void {
        for (const s of ALL_STATES) {
            personinsightsConnectionState.labels({ state: s, client: this.clientName }).set(s === activeState ? 1 : 0)
        }
    }
}
