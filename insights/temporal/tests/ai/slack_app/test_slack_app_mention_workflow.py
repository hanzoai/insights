...
from insights.temporal.ai.slack_app import derive_mention_workflow_id, slack_app_mention
...
def _fake_activities(rec: _Recorder) -> list:
    @activity.defn(name="enforce_insights_code_billing_quota_activity")
    async def quota(
        inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str, slack_user_id: str
    ) -> bool:
        return False
    ...
    @activity.defn(name="forward_insights_code_followup_activity")
    async def forward(
        inputs: InsightsCodeSlackMentionWorkflowInputs,
        channel: str,
        thread_ts: str,
    ...
    @activity.defn(name="collect_insights_code_thread_messages_activity")
    async def collect(
        inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str
    ) -> list[dict[str, str]]:
        return [{"user": "U1", "text": inputs.event["text"]}]

    @activity.defn(name="cascade_insights_code_repository_activity")
    async def cascade(
        inputs: InsightsCodeSlackMentionWorkflowInputs, event_text: str, user_id: int | None = None
    ) -> InsightsCodeRepoCascadeOutcome:
        mode = rec.cascade_modes.get(inputs.event["ts"], "auto")
        repository = "org/auto-repo" if mode == "auto" else None
        return InsightsCodeRepoCascadeOutcome(mode=mode, repository=repository, reason="test")

    @activity.defn(name="classify_insights_code_task_needs_repo_activity")
    async def needs_repo(event_text: str, thread_messages: list[dict[str, str]]) -> bool:
        return True

    @activity.defn(name="discover_insights_code_repository_via_agent_activity")
    async def discover(
        inputs: InsightsCodeSlackMentionWorkflowInputs,
        channel: str,
        event: dict[str, Any],
    ...
    @activity.defn(name="post_insights_code_repo_picker_activity")
    ...
    @activity.defn(name="resolve_insights_code_authorship_activity")
    ...
    @activity.defn(name="block_insights_code_task_if_no_personal_github_activity")
    ...
    @activity.defn(name="create_insights_code_task_for_repo_activity")
    ...
    @activity.defn(name="post_insights_code_picker_timeout_activity")
    async def picker_timeout(inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str) -> None:
        return None

    @activity.defn(name="post_insights_code_authorship_timeout_activity")
    async def authorship_timeout(inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str) -> None:
        return None

    @activity.defn(name="post_insights_code_internal_error_activity")
    async def internal_error(inputs: InsightsCodeSlackMentionWorkflowInputs, channel: str, thread_ts: str) -> None:
        rec.internal_errors.append(inputs.event["ts"])

    @activity.defn(name="resolve_insights_code_slack_user_activity")
...
async def _signal_with_start(env, task_queue: str, workflow_id: str, message: InsightsCodeSlackMentionWorkflowInputs):
...
class _Harness:
    ...
    async def start_worker(self) -> None:
        ...
        self._worker_cm = Worker(
            ...
            workflows=[SlackAppMentionWorkflow, InsightsCodeSlackMentionWorkflow],
