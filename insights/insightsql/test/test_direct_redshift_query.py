...
from insights.insightsql.direct_sql.redshift_adapter import RedshiftAdapter, ensure_read_only_raw_redshift_statement
...
class TestDirectRedshiftQuery(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            # Redshift CONCAT is two-arg and NULL-propagating; InsightsQL concat is variadic, NULL→''.
            ...
            # Redshift avg(int) truncates to an integer; InsightsQL avg is float.
            ("avg_cast_to_float", "SELECT avg(id) FROM orders", "avg(CAST("),
            # Redshift `/` on integers truncates; InsightsQL division is float.
    ...
    def test_raw_query_result_row_cap(self):
        ...
        with patch("insights.insightsql.direct_sql.redshift_adapter.DIRECT_REDSHIFT_MAX_ROWS", 3):
            with patch(
                "insights.insightsql.direct_sql.redshift_adapter.RedshiftAdapter.validate_source_config",
    ...
    def test_validate_source_config_gates_on_capability(
        ...
        if expect_valid:
            ...
            with self.assertRaisesRegex(ExposedInsightsQLError, "Invalid direct Redshift connection."):
