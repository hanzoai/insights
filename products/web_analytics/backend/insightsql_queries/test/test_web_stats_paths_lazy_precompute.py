...
class TestWebStatsPathsLazyPrecompute(DatastoreTestMixin, APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    def _enable_lazy(self):
        return patch(
            "products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common.hanzo_insights.feature_enabled",
            return_value=True,
        )
    ...
    def test_failed_lazy_read_clears_stale_tag(self):
        from insights.datastore.query_tagging import get_query_tag_value, reset_query_tags, tag_queries

        from products.web_analytics.backend.insightsql_queries import stats_table as stats_table_mod
    ...
    @unittest.skip(
        "CI-only flake since #59075 (passes 10/10 locally on the CI Datastore image) — "
    ...
    @unittest.skip(
        "CI-only flake since #59075 (passes locally on the CI Datastore image) — "
    ...
    def test_uuid_session_mode_falls_through(self):
        ...
        query.modifiers = InsightsQLQueryModifiers(sessionsV2JoinMode=SessionsV2JoinMode.UUID)
        with self._enable_lazy():
            self._run(query)
        assert PreaggregationJob.objects.filter(team_id=self.team.pk).count() == 0
    ...
    def test_falls_back_when_current_period_not_ready(self):
        ...
        with (
            self._enable_lazy(),
            patch(
                "products.web_analytics.backend.insightsql_queries.web_stats_paths_lazy_precompute.ensure_web_stats_paths_precomputed",
    ...
    def test_ensure_wait_budget_by_trigger(self, _name: str, trigger: str | None) -> None:
        ...
        from insights.datastore.query_tagging import reset_query_tags, tags_context

        from products.web_analytics.backend.insightsql_queries import web_stats_paths_lazy_precompute as mod
    ...
    def test_read_scan_is_pruned_to_requested_windows(self, _name: str, compare: bool) -> None:
        ...
        with patch.object(mod, "execute_insightsql_query", side_effect=fake_execute):
...
@override_settings(IN_UNIT_TESTING=True)
class TestWebStatsPathsSessionIdSetInsert(DatastoreTestMixin, APIBaseTest):
    ...
    def test_filtered_insert_matches_join_insert(self):
        ...
        from products.web_analytics.backend.insightsql_queries import web_stats_paths_lazy_precompute as mod
        from products.web_analytics.backend.insightsql_queries.web_analytics_lazy_precompute import (
        ...
        with override_settings(
            ...
            def run_template(template, pushdown: bool):
                ...
                assert isinstance(inner, ast.SelectQuery)
                sql, _ = prepare_and_print_ast(inner, context=context, dialect="datastore")
                merged = f"""
