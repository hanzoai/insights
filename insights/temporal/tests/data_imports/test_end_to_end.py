...
from insights.insightsql_queries.insights.funnels.funnel import FunnelUDF
...
from insights.temporal.common.shutdown import ShutdownMonitor, WorkerShuttingDownError
from insights.temporal.data_imports.cdp_producer_job import CDPProducerJobWorkflow
from insights.temporal.data_imports.external_data_job import ExternalDataJobWorkflow
from insights.temporal.data_imports.pipelines.pipeline.cdp_producer import CDPProducer
from insights.temporal.data_imports.pipelines.pipeline.consts import PARTITION_KEY
from insights.temporal.data_imports.pipelines.pipeline.delta_table_helper import DeltaTableHelper
from insights.temporal.data_imports.pipelines.pipeline.pipeline import PipelineNonDLT
...
from insights.temporal.data_imports.row_tracking import get_rows
from insights.temporal.data_imports.settings import ACTIVITIES
...
from insights.temporal.data_imports.sources.stripe.custom import InvoiceListWithAllLines
from insights.temporal.data_imports.workflow_activities.calculate_table_size import (
    CalculateTableSizeActivityInputs,
    calculate_table_size_activity,
)
from insights.temporal.data_imports.workflow_activities.sync_new_schemas import ExternalDataSourceType
from insights.temporal.ducklake import ACTIVITIES as DUCKLAKE_ACTIVITIES
from insights.temporal.ducklake.ducklake_copy_data_imports_workflow import DuckLakeCopyDataImportsWorkflow
...
def mock_stripe_client(
    ...
    with mock.patch("insights.temporal.data_imports.sources.stripe.stripe.StripeClient") as MockStripeClient:
...
async def _replay_v3_consumer(team_id: int, schema_id, job_id: str | None = None):
    ...
    with (
        ...
        mock.patch(
            "insights.temporal.data_imports.pipelines.pipeline_v3.load.idempotency.is_batch_already_processed",
            return_value=False,
        ),
    ):
...
async def _execute_run(workflow_id: str, inputs: ExternalDataWorkflowInputs, mock_data_response):
    ...
    with (
        ...
        if _current_pipeline_mode == "v3":
            stack.enter_context(
                mock.patch(
                    "insights.temporal.data_imports.workflow_activities.import_data_sync._is_pipeline_v3_enabled",
            ...
            stack.enter_context(
                mock.patch(
                    "insights.temporal.data_imports.pipelines.pipeline_v3.kafka.producer.get_warpstream_kafka_producer",
...
async def test_missing_source(team):
    ...
    with (
        ...
        mock.patch(
            "insights.temporal.data_imports.workflow_activities.create_job_model.delete_external_data_schedule"
...
async def test_delta_no_merging_on_first_sync(team, postgres_config, postgres_connection, pipeline_mode):
    ...
    with (
        mock.patch("insights.temporal.data_imports.sources.postgres.postgres.DEFAULT_CHUNK_SIZE", 1),
        mock.patch("insights.temporal.data_imports.sources.postgres.postgres._get_table_chunk_size") as mock_chunk_size,
        ...
        mock.patch(
            "insights.temporal.data_imports.pipelines.pipeline_v3.load.processor.run_post_load_operations",
            new_callable=AsyncMock,
...
async def test_partition_folders_with_existing_table(team, postgres_config, postgres_connection, minio_client):
    ...
    with (
        mock.patch(
            "insights.temporal.data_imports.pipelines.pipeline.pipeline.setup_partitioning", mock_setup_partitioning
        ),
        mock.patch(
            "insights.temporal.data_imports.pipelines.pipeline_v3.load.processor._apply_partitioning",
...
async def test_row_tracking_incrementing(team, postgres_config, postgres_connection):
    ...
    with (
        mock.patch("insights.temporal.data_imports.pipelines.common.extract.decrement_rows") as mock_decrement_rows,
        mock.patch("insights.temporal.data_imports.external_data_job.finish_row_tracking") as mock_finish_row_tracking,
    ):
        _, inputs = await _run(
            team=team,
...
async def test_postgres_duplicate_primary_key(team, postgres_config, postgres_connection):
    ...
    with (
        ...
        mock.patch("insights.temporal.data_imports.external_data_job.update_should_sync") as mock_update_should_sync,
    ):
        await _run(
            team=team,
...
async def test_worker_shutdown_desc_sort_order(team):
    ...
    with (
        ...
        mock.patch(
            "insights.temporal.data_imports.external_data_job.trigger_schedule_buffer_one"
        ) as mock_trigger_schedule_buffer_one,
        mock.patch("insights.temporal.data_imports.pipelines.pipeline.batcher.DEFAULT_CHUNK_SIZE", 1),
        mock.patch("insights.temporal.data_imports.sources.vitally.vitally.get_messages", mock_get_messages),
    ):
        _, inputs = await _run(
            team=team,
...
async def test_billing_limits_too_many_rows(team, postgres_config, postgres_connection):
    ...
    with (
        ...
        mock.patch("insights.cloud_utils.is_instance_licensed_cached", None),
    ):
...
async def test_pipeline_mb_chunk_size(team, zendesk_brands, pipeline_mode):
    ...
    with (
        mock.patch("insights.temporal.data_imports.pipelines.pipeline.batcher.DEFAULT_CHUNK_SIZE_BYTES", 1),
        mock.patch(
            "insights.temporal.data_imports.pipelines.pipeline.batcher.DEFAULT_CHUNK_SIZE", 5000
...
async def test_timestamped_query_folder(team, stripe_balance_transaction, mock_stripe_client, minio_client):
    ...
    with freeze_time(datetime_5), mock.patch("insights.temporal.data_imports.util.S3_DELETE_TIME_BUFFER", 1):
        await _execute_run(workflow_id, inputs, stripe_balance_transaction["data"])
        await _replay_v3_consumer(team_id=team.pk, schema_id=inputs.external_data_schema_id)
...
async def test_non_retryable_error_short_circuiting(team, stripe_customer, mock_stripe_client):
    with mock.patch("insights.temporal.data_imports.sources.stripe.stripe.get_rows") as mock_get_rows:
...
async def test_cdp_producer_push_to_s3(team, stripe_customer, mock_stripe_client, minio_client):
    ...
    with mock.patch(
        "insights.temporal.data_imports.external_data_job.start_child_workflow"
...
async def test_cdp_producer_push_to_kafka(team, stripe_customer, mock_stripe_client, minio_client, pipeline_mode):
    ...
    with (
        ...
        mock.patch(
            "insights.temporal.data_imports.pipelines.pipeline.pipeline.time.time_ns", return_value=1768828644858352000
        ),
    ):
        _, inputs = await _run(
            team=team,
