...
//! the `lifecycle::Manager`. Depends on `app`, `datastore`, `kafka`, `config`, and `observability` —
...
use cohort_seeder::datastore::client::build_client;
use cohort_seeder::datastore::person_scanner::PersonScanner;
use cohort_seeder::datastore::scanner::ChunkScanner;
...
async fn async_main(config: Config) -> Result<()> {
    ...
    let datastore_client = build_client(&config).context("building Datastore client")?;
    let scanner = ChunkScanner::new(datastore_client.clone());
    ...
    // Shares the built Datastore client with the behavioral scanner.
    let person = settings.person().map(|person_settings| PersonComponents {
        scanner: PersonScanner::new(datastore_client),
