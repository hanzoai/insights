...
const LOCAL_DISMISSED_KEY_PREFIX = 'insights_welcome_dismissed:'
...
const SESSION_LOOKED_AROUND_KEY = 'insights_welcome_looked_around'
...
export const welcomeDialogLogic = kea<welcomeDialogLogicType>([
            ...
            EMPTY_PAYLOAD,
            {
                loadWelcomeData: async () => {
                    try {
                        ...
                        // Surface in Insights so fleet-level error rate is observable; console.warn
                        // alone is invisible at scale.
                        insights.capture('welcome_screen_load_failed', { status })
    ...
    listeners(({ actions, values }) => ({
        loadWelcomeDataSuccess: ({ welcomeData }) => {
            ...
            actions.markShown()
            insights.capture('welcome_screen_shown', {
        ...
        dismissWelcome: () => {
            ...
            insights.capture('welcome_screen_dismissed', {
        ...
        trackCardClick: ({ card, targetHref }) => {
            ...
            rememberLookedAround(values.user?.organization?.id)
            insights.capture('welcome_screen_card_clicked', {
                card,
