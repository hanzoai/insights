...
describe('heatmapCreationLogic', () => {
    ...
    it('discards a readiness result after the effective data URL changes', async () => {
        ...
        const queryMock = jest.mocked(api.queryInsightsQL)
        const logic = heatmapCreationLogic
        logic.mount()
        await expectLogic(logic).toFinishAllListeners()
        queryMock.mockClear()
        queryMock.mockImplementationOnce(() => {
            ...
            return firstCheck as ReturnType<typeof api.queryInsightsQL>
        })
    ...
    it('clears an in-flight readiness check when the URL becomes invalid', async () => {
        ...
        logic.mount()
        await expectLogic(logic).toFinishAllListeners()
        jest.mocked(api.queryInsightsQL).mockImplementationOnce(() => {
            markCheckStarted?.()
            return pendingCheck as ReturnType<typeof api.queryInsightsQL>
        })
