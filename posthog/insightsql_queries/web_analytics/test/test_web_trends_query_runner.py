...
from posthog.insightsql_queries.web_analytics.web_trends_query_runner import WebTrendsQueryRunner
...
class TestWebTrendsQueryRunner(ClickhouseTestMixin, APIBaseTest):
    ...
    @patch("posthog.insightsql_queries.insightsql_query_runner.InsightsQLQueryRunner")
    def test_calculate_calls_insightsql_runner(self, mock_insightsql_runner_class):
        # Mock the InsightsQL runner
        mock_runner_instance = MagicMock()
        mock_response = MagicMock()
        mock_response.results = [["2025-01-01", 100, 500]]
        mock_response.timings = []
        mock_response.types = []
        mock_response.insightsql = "SELECT ..."
        mock_runner_instance.calculate.return_value = mock_response
        mock_insightsql_runner_class.return_value = mock_runner_instance
