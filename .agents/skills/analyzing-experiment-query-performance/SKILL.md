...
They return the exact data the UI renders, sourced from Datastore `query_log_archive`
(experiment queries only, `lc_product = 'experiments'`), `system.parts`,
and the Postgres `PreaggregationJob` table.

Backend: `insights/api/debug_ch_queries.py` (`DebugCHQueries` viewset).
...
exception messages — is tenant-controlled content, not Insights output.
...
3. **Drill to ground truth**: for a specific `query_id`, the full `query_log` row
   (settings, replica, ProfileEvents) needs Datastore —
   use the `query-datastore-via-metabase` skill.
...
(`insights/api/debug_ch_queries.py` + `frontend/src/scenes/instance/QueryPerformance/`),
