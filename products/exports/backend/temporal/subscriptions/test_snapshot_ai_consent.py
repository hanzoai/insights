...
class _FakePhClient:
    """In-memory stand-in for the real Insights client used by `ph_scoped_capture`.
...
def _install_fake_ph_client(monkeypatch) -> _FakePhClient:
    ...
    monkeypatch.setattr("insights.ph_client.is_cloud", lambda: True)
    monkeypatch.setattr("insights.ph_client.get_client", lambda *a, **kw: client)
    return client
...
async def test_stored_prompt_guide_is_ignored_when_prompt_guide_flag_is_off(team, user, monkeypatch):
    ...
    monkeypatch.setattr(
        "products.exports.backend.temporal.subscriptions.snapshot_activities.hanzo_insights.feature_enabled",
        lambda *a, **kw: False,
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            ["Company is Insights.", "Flagship product is product analytics."],
            id="memory_present",
        ),
