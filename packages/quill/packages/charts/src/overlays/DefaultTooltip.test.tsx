...
describe('DefaultTooltip', () => {
    ...
    describe('row presentation props', () => {
        ...
        it('renders the header label by default', () => {
            renderTooltip({})
            expect(document.querySelector('[data-attr="script-chart-tooltip-label"]')?.textContent).toBe('When')
        })

        it('hides the header label when showHeader is false', () => {
            renderTooltip({ showHeader: false })
            expect(document.querySelector('[data-attr="script-chart-tooltip-label"]')).toBeNull()
        })
        ...
        it('onRowClick fires with the clicked row entry', () => {
            ...
            const rows = document.querySelectorAll<HTMLElement>('[data-attr="script-chart-tooltip-row"]')
        ...
        it('onRowClick does not fire when the click completes a text selection', () => {
            ...
            fireEvent.click(document.querySelectorAll<HTMLElement>('[data-attr="script-chart-tooltip-row"]')[1])
        ...
        it('marks the row whose yPixel is closest to the cursor with data-closest', () => {
            ...
            expect(closest[0].querySelector('[data-attr="script-chart-tooltip-series"]')?.textContent).toBe('Beta')
        })
    })
})
