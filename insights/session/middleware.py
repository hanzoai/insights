...
from insights.session.activity import sync_current_session_metadata
from insights.session.risk import RiskTier, evaluate_session_risk


class UserAuthSessionActivityMiddleware:
    """Refreshes display metadata on the current login session's row (see insights.session.activity)."""
...
class SessionRiskMiddleware:
    ...
    def __call__(self, request: HttpRequest):
        ...
        # pollute tests that assert hanzo_insights.feature_enabled call counts — when disabled.
