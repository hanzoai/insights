...
async def test_persist_ai_report_writes_markdown_query_diagnostics_and_prompt(team, user) -> None:
    ...
    await _persist_ai_report(
        ...
        AiReportResult(
            ...
            diagnostics=(
                QueryStepDiagnostic(description="adoption", insightsql="SELECT count()", ok=True, error_type=None),
                QueryStepDiagnostic(
                    description="reliability", insightsql="SELECT bad", ok=False, error_type="ResolutionError"
                ),
            ),
        ),
...
async def test_persist_ai_report_strips_null_bytes(team, user) -> None:
    ...
    await _persist_ai_report(
        ...
        AiReportResult(
            ...
            window_end_utc=_WINDOW_END_UTC,
            diagnostics=(
                QueryStepDiagnostic(description="adop\x00tion", insightsql="SELECT co\x00unt()", ok=True, error_type=None),
            ),
        ),
    ...
    assert snapshot[AI_REPORT_DIAGNOSTICS_KEY][0]["insightsql"] == "SELECT count()"
...
class TestReportDiagnosticCounts:
    ...
    def test_distinct_error_types_are_sorted_and_deduped(self):
        result = AiReportResult(
            ...
            window_end_utc=_WINDOW_END_UTC,
            diagnostics=(
                QueryStepDiagnostic(description="a", insightsql="x", ok=False, error_type="ResolutionError"),
                QueryStepDiagnostic(description="b", insightsql="y", ok=False, error_type="ExposedInsightsQLError"),
                QueryStepDiagnostic(description="c", insightsql="z", ok=False, error_type="ResolutionError"),
            ),
        )
        assert _report_diagnostic_counts(result) == (3, 3, ["ExposedInsightsQLError", "ResolutionError"])
...
async def test_snapshot_diagnostic_counts_reads_persisted_failure_shape(team, user) -> None:
    ...
    await _persist_ai_report(
        ...
        AiReportResult(
            ...
            window_end_utc=_WINDOW_END_UTC,
            diagnostics=(
                QueryStepDiagnostic(description="ok step", insightsql="SELECT 1", ok=True, error_type=None),
                QueryStepDiagnostic(description="bad step", insightsql="SELECT bad", ok=False, error_type="ResolutionError"),
            ),
        ),
        prompt=None,
    )
