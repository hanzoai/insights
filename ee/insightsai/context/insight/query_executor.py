...
from posthog.insightsql_queries.query_runner import BLOCKING_EXECUTION_MODES, ExecutionMode
...
class AssistantQueryExecutor:
    ...
    Attributes:
        _team: The Insights team context for query execution
    ...
    async def aexecute_query(
        ...
        try:
            # Execute the query using Insights's query processing system
    ...
    async def _compress_results(
        ...
        try:
            ...
            elif isinstance(query, AssistantInsightsQLQuery | InsightsQLQuery):
...
def get_example_prompt(query: AnyPydanticModelQuery | AnyAssistantGeneratedQuery) -> str:
    ...
    if isinstance(query, AssistantInsightsQLQuery | InsightsQLQuery):
...
async def execute_and_format_query(
    ...
    insight_schema = ""
    if not isinstance(query, AssistantInsightsQLQuery | InsightsQLQuery):
        insight_schema = query.model_dump_json(exclude_none=True)

    # Check if SQL results contain truncated values
    has_truncated_values = (
        isinstance(query, AssistantInsightsQLQuery | InsightsQLQuery) and TRUNCATED_MARKER in results and not used_fallback
    )
    ...
    query_result = format_prompt_string(
        ...
        sql_query=True if isinstance(query, AssistantInsightsQLQuery | InsightsQLQuery) else None,
    )
