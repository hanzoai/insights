...
from products.review.backend.reviewer.lazy_seed import sync_canonical_validation
...
_CUSTOM = f"{REVIEW_FN_VALIDATION_PREFIX}strict"
...
class TestReviewValidatorConfigAPI(APIBaseTest):
    ...
    def test_list_shows_canonical_active_and_custom_inactive(self) -> None:
        ...
        assert active_by_name[REVIEW_FN_VALIDATION_SKILL_NAME] is True
    ...
    def test_selecting_a_custom_validator_flips_the_canonical_off(self) -> None:
        ...
        assert configs.get(skill_name=REVIEW_FN_VALIDATION_SKILL_NAME).enabled is False
    ...
    def test_switching_between_custom_validators_keeps_exactly_one_active(self) -> None:
        ...
        c1, c2 = f"{REVIEW_FN_VALIDATION_PREFIX}c1", f"{REVIEW_FN_VALIDATION_PREFIX}c2"
        for name in (c1, c2):
            LLMSkill.objects.create(
                team=self.team,
                name=name,
        ...
        assert configs.filter(skill_name__startswith=REVIEW_FN_VALIDATION_PREFIX, enabled=True).count() == 1
    ...
    def test_selecting_a_validator_leaves_perspective_configs_enabled(self) -> None:
        ...
        perspective = f"{REVIEW_FN_PERSPECTIVE_PREFIX}logic-correctness"
    ...
    def test_list_excludes_perspective_skills(self) -> None:
        ...
        LLMSkill.objects.create(
            team=self.team,
            name=f"{REVIEW_FN_PERSPECTIVE_PREFIX}logic-correctness",
            description="d",
            body="x" * 250,
            version=1,
            is_latest=True,
        ...
        assert names == {REVIEW_FN_VALIDATION_SKILL_NAME}
    ...
    def test_select_rejects_deactivating(self) -> None:
        ...
        res = self.client.patch(f"{self.base}/{REVIEW_FN_VALIDATION_SKILL_NAME}/", {"active": False}, format="json")
    ...
    def test_patch_404_for_unknown_validator(self) -> None:
        res = self.client.patch(
            f"{self.base}/{REVIEW_FN_VALIDATION_PREFIX}does-not-exist/", {"active": True}, format="json"
        )
