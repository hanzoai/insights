...
client mocked — this environment has no Datastore, so the task's static-cohort insert
cannot run here.
...
def _person_uuid(db: TestDB, person_id: int) -> str:
    with db.persons_conn.cursor() as cur:
        cur.execute("SELECT uuid FROM insights_person WHERE id = %s", (person_id,))
        row = cur.fetchone()
...
def test_batch_evaluation_returns_property_matched_persons(db: TestDB, api: DjangoAPI, env: TestEnv):
    from insights.api.services.flags_service import batch_evaluate_flag_for_team
