"""insightscli owners:* commands — resolve, who, unowned, lint, fmt."""
...
def _validate_owners_live(all_owners: set[str]) -> list[str]:
    ...
    # --live reaches back into the insightscli-commands extension for the cached team-slug
    # fetch. It is a dev/CI convenience gated behind the flag, so a standalone uvx
    # install (no insightscli-commands on the path) simply can't run --live; plain lint works.
    from insightscli_commands.product.gh import get_team_slugs  # noqa: PLC0415 — optional org-validation dep, only on --live
    ...
    for handle in sorted(handles):
        ...
        result = subprocess.run(
            ["gh", "api", f"orgs/Insights/members/{handle}"],
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            errors.append(f"not a Insights org member (unassignable): @{handle}")
    return errors
...
def main() -> None:
    ...
    The console-script entry point (``owners <subcommand>``). insightscli registers the
