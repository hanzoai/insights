...
a raw psycopg connection (see :mod:`insights.persons_db`), bypassing the Django ORM so the
...
creation flows through ingestion and the personinsights service, which has no create RPC by
design.
...
PERSON_DISTINCT_ID_TABLE = "insights_persondistinctid"
...
def insert_seed_person(
    ...
    *,
    ...
    mirror it into Datastore); otherwise a fresh ``UUIDT`` is generated. ``created_at``
...
def fetch_recent_persons_with_distinct_id(team_id: int, *, limit: int = 50) -> list[PersonData]:
    ...
    query = (
        ...
        "JOIN LATERAL (SELECT distinct_id FROM insights_persondistinctid "
