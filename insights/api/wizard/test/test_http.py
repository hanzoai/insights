...
from insights.api.wizard.http import SETUP_WIZARD_CACHE_PREFIX, SETUP_WIZARD_CACHE_TIMEOUT
from insights.cloud_utils import get_api_host
from insights.models import Organization, PersonalAPIKey, User
...
class SetupWizardTests(APIBaseTest):
    ...
    @patch("insights.rate_limit.SetupWizardAuthenticationRateThrottle.rate", new="2/day")
...
class SetupWizardCloudRunTests(APIBaseTest):
    ...
    @patch("insights.api.wizard.http.tasks_facade.create_wizard_cloud_run")
    ...
    @patch("insights.api.wizard.http.WIZARD_CLOUD_RUN_DAILY_ATTEMPT_CAP", 2)
    @patch("products.tasks.backend.facade.api.recent_wizard_cloud_run_times")
    @patch("insights.api.wizard.http.tasks_facade.create_wizard_cloud_run")
