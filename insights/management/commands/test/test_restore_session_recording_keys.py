...
def _patch_clients(backup_client: MagicMock, dynamodb_client: MagicMock):
    ...
    class _Combined:
        def __enter__(self):
            self._p1 = patch(
                "insights.management.commands.restore_session_recording_keys.boto3.client",
                side_effect=side_effect,
            )
            self._p2 = patch(
                "insights.management.commands.restore_session_recording_keys.sync_execute",
                ctx.sync_execute,
            )
            self._p1.__enter__()
            self._p2.__enter__()
...
class TestRestoreSessionRecordingKeysCommand:
    ...
    def test_restore_timeout_raises_command_error(self):
        ...
        with _patch_clients(backup, dynamodb):
            with (
                patch("insights.management.commands.restore_session_recording_keys.time.time") as mock_time,
                patch("insights.management.commands.restore_session_recording_keys.time.sleep"),
            ):
    ...
    def test_undelete_datastore_after_single_key_restore(self):
        backup = _make_backup_client(recovery_point_arn=RECOVERY_ARN)
        dynamodb = _make_dynamodb_client(get_item_response={"Item": ITEM_ENCRYPTED})
