...
description: >
  Investigate individual Insights MCP sessions — the sequence of tool calls a
...
have a **typed tool** — reach for those first. Drop to InsightsQL only for the three
...
| `insights:execute-sql`                            | Errored sessions, effective tool names, cross-session cuts |
...
`insights:execute-sql`, which is ungated.
...
- `insights:mcp-analytics-sessions-tool-calls` — `date_from` is an absolute ISO
  timestamp; pass the `session_start` you got from
  `insights:mcp-analytics-sessions-list`.
- `insights:mcp-analytics-sessions-generate-intent` — same `date_from` query
  param, same reason.
...
```json
insights:mcp-analytics-sessions-tool-calls
...
```json
insights:mcp-analytics-sessions-generate-intent
...
Four cases, all via `insights:execute-sql`, which — unlike the typed tools above —
...
- **Sessions list**: `https://app.hanzo.ai/project/<project_id>/mcp-analytics/sessions`
