import { InsightsFunctionInputSchemaType, InsightsFunctionTemplate } from '~/cdp/types'
...
// Insights events with a default mapping to an OpenAI standard event type. The other
// standard types (lead_created, trial_started, ...) have no standard Insights
...
const build_inputs = (defaults: { eventType?: string; actionSource?: string } = {}): InsightsFunctionInputSchemaType[] => {
    return [
        {
            key: 'eventType',
            type: 'choice',
        ...
        },
        {
            ...
            description:
                'The privacy-preserving identifier (oppref) OpenAI appends to ad click-through URLs. Insights does not capture it automatically, so send it as an event or person property yourself. It gives the most accurate attribution, but conversions can also match on the obref cookie, hashed email, or hashed external ID when it is not available.',
        ...
        },
        {
            ...
            description:
                'The value of the __obref first-party cookie set by the OpenAI pixel, sent unhashed. Insights does not read the cookie automatically, so send it as a person property yourself. Only send it for users who have consented to measurement.',
        ...
        },
        {
            ...
            description:
                'The monetary value of the conversion as an integer in ISO 4217 minor units, e.g. 2599 for $25.99. Note this differs from most Insights revenue properties, which are in major units.',
            default: '',
            secret: false,
            required: false,
        },
...
if (inputs.eventType == 'custom') {
    ...
    // so the {event.event} default works for typical Insights event names
