...
Provides insightscli dev:* commands for managing the development environment.
"""
...
def dev_generate(
    ...
    Run 'insightscli start' to start the dev environment.
    """
    try:
    ...
    if saved_config is None:
        ...
        click.echo("Run 'insightscli dev:setup' to configure your environment.")
        click.echo("")
    ...
    click.echo("")
    click.echo("Run 'insightscli dev:setup' to change your environment.")
...
def dev_explain(intents: tuple[str, ...]) -> None:
    ...
    if intents:
        ...
        if saved_config is None:
            ...
            click.echo("")
            click.echo("Usage:")
            click.echo("  insightscli dev:explain error_tracking session_replay")
            click.echo("  insightscli dev:setup  # to create a config")
            return
...
@click.option("--log", "log_to_files", is_flag=True, help="Log process output to /tmp/insights-*.log files")
...
def dev_regenerate_mprocs(check: bool) -> None:
    ...
    if check:
        ...
        click.echo("run `insightscli dev:regenerate-mprocs` to fix", err=True)
        raise SystemExit(1)
...
def docker_services_up(yes: bool) -> None:
    ...
    Run 'insightscli dev:setup' to configure which services to start.
    """
    import subprocess
