...
description: >
  ...
  Covers Insights's workflow-authoring conventions and the reasons behind them: the 500-runs/10s dispatch cap, shallow vs full clone, per-SHA push concurrency, dedicated App-token rate-limit buckets, and fork-safe secrets on a public repo.
  Points to the linters (`bin/insightscli lint:workflows`, actionlint) that enforce the mechanical rules, and to the narrower skills for production deploys, secrets, and Depot runners.
...
Run `bin/insightscli lint:workflows` and `actionlint` before pushing — they gate CI, and they (not this list) are the source of truth for what's enforced.
...
- A reusable-workflow call counts as **one** run.
  Small always-fire PR workflows should be jobs under a single `workflow_call` parent, not their own dispatches (see `pr-updated.yml` / `pr-opened.yml` folded behind their parent — [fold pr housekeeping into one dispatch](https://github.com/Insights/insights/pull/68964)).
...
- Prefer a trigger-level `paths:` filter over dispatch-then-skip: a run that only starts to no-op still spends a dispatch ([gate container workflows on trigger paths](https://github.com/Insights/insights/pull/68975)).
...
- Cancel superseded **PR** runs; never cancel across **master** pushes.
  ...
  Where latest-wins is genuinely right (a cache warmer), say so with `# insightscli-lint: allow-master-cancel -- <reason>`.
...
A job that inspects results without gating anything opts out with `# insightscli-lint: not-a-required-gate — <reason>` above the job key.
...
Measured on a live superseded run ([evidence](https://github.com/Insights/insights/actions/runs/29765284128)): an `always()` worker dispatched and ran to completion _after_ the cancel, while the `!cancelled()` worker never started and reported `cancelled` (not `skipped`), so the gate still fails closed.
...
- The linter rejects `fetch-depth: 0` unless you add `filter: blob:none`, use `sparse-checkout`, or justify it with `# insightscli-lint: allow-full-depth-checkout -- <reason>`.
...
- Secret-injecting builds (BuildKit `--secret`, registry login) must skip forks — gate **both** the `changes` job and any `always()` build job ([block fork PRs from rust image build](https://github.com/Insights/insights/pull/68628)).
...
- [ ] `bin/insightscli lint:workflows` and `actionlint` pass locally.
