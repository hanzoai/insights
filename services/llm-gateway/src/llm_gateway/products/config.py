...
class ProductConfig:
    ...
    # (insights/tasks/usage_report.py) ignores them. A bucket value tags events billable
    ...
    # Insights Desktop OAuth app but are only ever driven by sandbox runs — a user's own Code
    # OAuth token can't carry an internal scope, so this stops it routing around the
    # insights_code free-tier model gate through these products. Personal API keys are
...
# insights_code_model_gate_enabled. The flag exists so products that already shipped accepting plain
...
PRODUCTS: Final[dict[str, ProductConfig]] = {
    ...
    # CI / end-to-end test runs (e.g. insights/code agent e2e tests). Authenticates with a
    ...
    "insights_code": ProductConfig(
        ...
        allowed_models=_INSIGHTS_CODE_AGENT_MODELS | {BASETEN_DEEPSEEK_PUBLIC_MODEL} | BEDROCK_MODELS,
    ...
    # "work completed by Insights" per the pricing RFC, which gets its own (marked-up)
    ...
    # SherlockHog (https://github.com/Insights/SherlockHog) — the internal SRE
    ...
    "conversations": ProductConfig(
        ...
        # Insights" per the pricing RFC — it gets its own pricing later, not insights_code's
        # pass-through bucket.
        credit_bucket=None,
        requires_server_credential=True,
    ),
...
# insights/temporal/oauth.py). Being an internal scope, it can't be obtained through the OAuth
...
def check_free_tier_model_access(
    ...
    if not settings.insights_code_model_gate_enabled:
        return True, None
    if resolve_product_alias(product) != "insights_code":
        return True, None
    ...
    free_models = frozenset(settings.insights_code_free_tier_models)
    ...
    return False, (
        f"Model '{model}' needs a paid Insights plan. Models available on the free tier: {available}. "
        "Add a payment method to your organization to unlock all models."
    )


def filter_to_free_tier_models(model_ids: list[str]) -> list[str]:
    """Subset of model_ids on the insights_code free tier."""
    settings = get_settings()
    free_models = frozenset(settings.insights_code_free_tier_models)
...
def check_product_access(
    ...
    # Internal products that share the Insights Desktop OAuth app are only ever driven by
    # server-minted sandbox tokens; a user's own Desktop OAuth token would otherwise reach them
    # and route around the insights_code free-tier model gate. Require the internal marker that
    ...
    if (
        ...
        and (settings.insights_code_model_gate_enabled or resolved_product in UNCONDITIONAL_SERVER_CREDENTIAL_PRODUCTS)
    ):
