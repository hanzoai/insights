...
use personinsights_coordination::error::{Error as CoordError, Result as CoordResult};
    ...
    for attempt in 1..=max_attempts {
        match f().await {
            ...
            Err(e) if attempt == max_attempts => {
                counter!(
                    "personinsights_leader_warm_retries_exhausted_total",
            ...
            Err(e) => {
                ...
                counter!(
                    "personinsights_leader_warm_retries_total",
                    "stage" => stage.to_string()
                )
                .increment(1);
...
impl ConsumerPool {
    ...
    pub fn checkout(&self) -> CoordResult<StreamConsumer> {
        ...
        counter!(
            "personinsights_leader_warm_clients_created_total",
            "pool" => self.label
        )
        .increment(1);
    ...
    pub fn new(kafka: &KafkaConfig, pod_name: &str, writer_group: &str) -> Self {
        Self {
            ...
            warming: ConsumerPool::new(
                kafka.clone(),
                format!("personinsights-leader-warm-{pod_name}"),
                "warming",
            ),
        }
    }
...
pub async fn warm_from_kafka(
    ...
    loop {
        ...
        if let Some(payload) = msg.payload() {
            ...
            // to `personinsights_updates` today. If one ever appears it would
            ...
            counter!("personinsights_leader_warm_tombstones_skipped_total").increment(1);
            tracing::warn!(
                partition,
                offset,
    ...
    histogram!("personinsights_leader_warm_duration_ms").record(elapsed.as_secs_f64() * 1000.0);
    counter!("personinsights_leader_warmed_messages_total").increment(count);
