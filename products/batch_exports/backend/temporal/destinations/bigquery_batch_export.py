...
from insights.models.integration import GoogleCloudServiceAccountIntegration, Integration
...
class ServiceAccountOwnershipError(Exception):
    def __init__(self, email: str, organization_id: str):
        super().__init__(
            ...
            f"Have you added 'insights:{organization_id}' to your service account's description?"
        )
...
class GoogleCloudCredentialsError(Exception):
    """Raised when we cannot acquire Insights Google Cloud credentials."""

    def __init__(self):
        super().__init__("Failed to acquire Insights Google Cloud credentials")
...
async def get_service_account_description(
    ...
    try:
        ...
        EXTERNAL_LOGGER.exception(
            ...
            "Have you granted 'iam.serviceAccounts.get' to the Insights service account to operate on it?",
...
async def verify_impersonated_service_account_ownership(
    ...
    We do this by checking if 'insights:{organization_id}' is present in the service
    ...
    while attempt < max_attempts:
        ...
        if f"insights:{organization_id}" in description:
            return
