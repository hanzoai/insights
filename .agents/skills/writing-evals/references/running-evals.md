...
Both providers keep the eval database, Django server, LLM gateway, MCP server, Temporal, personinsights, case setup, and scoring on the machine running `insightscli evals`.
...
The personinsights build needs flox's Rust toolchain, `pkg-config`, and OpenSSL.
When a flox shell is not already active, wrap the whole command instead of starting an interactive activation:

```bash
flox activate -- bash -c "insightscli evals --list"
```

Use `insightscli evals` for normal runs.
...
The direct `python -m products.insights_ai.eval_harness.harness` entry point loads only `.env`, so export any other credentials before using it.
...
```bash
insightscli evals '<product>/eval_example::eval_my_thing' --eval '<case-name>'
```
...
The higher limit lets managed development and CI environments prepare cases in parallel while keeping local Datastore memory use conservative.
...
Before a run, the harness checks whether `insights-sandbox-base` is current and rebuilds it when the Dockerfile or published agent version changed.
The first personinsights build and any required image build can take several minutes; later runs reuse their outputs.

Run one case with one live sandbox first:

```bash
insightscli evals '<suite-selector>' \
    --eval '<case-name>' \
    --provider docker \
    --max-sandboxes 1
```
...
Remote sandboxed evals use the dedicated `insights-sandbox-evals` Modal app.
...
Cloudflare Tunnel can expose the same services in other development workflows, but it is not wired into `insightscli evals --provider modal` and does not satisfy the provider's current ngrok preflight.
...
```bash
insightscli evals '<product-or-domain>' --provider modal --max-sandboxes 4
```
...
- Running outside flox, which makes the personinsights Rust build fail on its toolchain or OpenSSL dependencies.
...
Every real invocation mirrors stdout and stderr to `products/insights_ai/eval_harness/logs/harness/<timestamp>_<id>.log`.
