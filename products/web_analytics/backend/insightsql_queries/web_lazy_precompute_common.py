...
from insights.schema import SessionsV2JoinMode
...
# covers warmed query shapes). Duplicate enqueues need no dedup layer: the InsightsQL result
...
# outages. Must stay well under the framework's 48h Datastore expiry buffer (rows must
# still exist).
...
# InsightsQL execution hints applied after the fact, and the rest are
...
#    waits. Recomputing more often buys nothing anyway — the ~6h InsightsQL result
...
def check_common_eligibility(
    *,
    ...
    which can trigger a Datastore min-timestamp lookup for `-all` ranges.
    """
