...
from insights.api.documentation import FeatureFlagFiltersSchemaSerializer
...
class ExperimentSerializer(ExperimentBaseSerializer):
    ...
    def validate_repository(self, value: str | None) -> str | None:
        ...
        # needs the same Insights Desktop access as opening one (mirrors open_cleanup_pr).
        if self.instance is None or value != self.instance.repository:
            request = self.context.get("request")
            if request is None or not has_tasks_access(request.user):
                raise PermissionDenied("Setting a cleanup repository requires access to Insights Desktop.")
        return value
...
# Advertises the writable feature_flag input in the OpenAPI request schema. Insights's drf-spectacular
...
class EndExperimentSerializer(serializers.Serializer):
    ...
    open_cleanup_pr = serializers.BooleanField(
        ...
        help_text=(
            ...
            "from the linked repository. Requires the requesting user to have access to Insights Desktop "
...
class ExperimentFlagCleanupTaskSerializer(serializers.Serializer):
    ...
    can_view_task = serializers.BooleanField(
        help_text=(
            "Whether the requesting user can open the task in Insights Desktop. Cleanup tasks are "
...
class MetricRecalculationResultSerializer(serializers.Serializer):
    ...
    # insights.schema.ExperimentQueryResponse (variants/baseline/credible intervals/etc., depending on metric type).
...
class ExperimentMetricsRecalculationSerializer(serializers.Serializer):
    ...
    estimated_rows_total = serializers.IntegerField(
        ...
        allow_null=True,
        help_text=(
            "Datastore's total_rows_approx across running queries plus the final read_rows of finished ones. "
