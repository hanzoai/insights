...
from products.review.backend.models import ReviewReport, ReviewReportArtefact, ReviewSkillConfig, ReviewUserSettings
...
class TestResetReview(BaseTest):
    ...
    def test_wipes_every_review_model(self) -> None:
        ...
        ReviewSkillConfig.objects.for_team(self.team.id).create(
            team_id=self.team.id, user_id=self.user.id, skill_name="review-script-perspective-x", enabled=True
        )
