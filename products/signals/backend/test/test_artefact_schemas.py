...
class TestArtefactSchemas(SimpleTestCase):
    ...
    def test_commit_defaults_note_to_none(self):
        commit = Commit(repository="Insights/insights", branch="fix/foo", commit_sha="abc123f", message="fix: foo")
    ...
    def test_commit_rejects_blank_strings(self, field):
        kwargs = {"repository": "Insights/insights", "branch": "fix/foo", "commit_sha": "abc123f", "message": "m"}
        kwargs[field] = "   "
        with self.assertRaises(ValidationError):
            Commit(**kwargs)
...
class TestValidateArtefactContent(SimpleTestCase):
    ...
    @parameterized.expand(
        [
            ...
            ("note", {"note": "   "}),
            ("commit", {"repository": "Insights/insights", "branch": "b", "commit_sha": "  ", "message": "m"}),
