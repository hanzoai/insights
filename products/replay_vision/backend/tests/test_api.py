...
from insights.models import Organization, PersonalAPIKey, Team, User
...
class _VisionAPITestCase(APIBaseTest):
    def setUp(self) -> None:
        ...
        self.flag_patcher = patch(
            "products.replay_vision.backend.feature_flag.hanzo_insights.feature_enabled",
            return_value=True,
        )
        self.flag_patcher.start()
        # Scanner saves recompute the volume estimate against Datastore; keep CRUD tests off that path.
...
class TestReplayScannerViewSet(_VisionAPITestCase):
    ...
    def _patch_deny_resource(self, denied: str):
        return patch(
            "insights.rbac.user_access_control.UserAccessControl.check_access_level_for_resource",
...
class TestScannerLifecycleTelemetry(_VisionAPITestCase):
    def test_create_reports_config_choices(self) -> None:
        ...
        with patch("hanzo_insights.capture") as capture:
            resp = self.client.post(
                self.scanners_url,
                data={
...
class TestScannerEstimatePersistence(_VisionAPITestCase):
    ...
    def test_create_succeeds_when_estimate_refresh_fails(self) -> None:
        self.mock_refresh_estimate.side_effect = RuntimeError("datastore down")
        resp = self.client.post(self.scanners_url, data=self._create_payload(), format="json")
        self.assertEqual(resp.status_code, 201, resp.json())
...
class TestObserveAction(_VisionAPITestCase):
    ...
    def test_observe_returns_workflow_id_and_starts_workflow(
        ...
        self.assertTrue(
            any(p.key.name == "InsightsScannerId" and p.value == str(self.scanner.id) for p in scanner_attrs)
        )
...
class TestReplayScannerEstimateAction(DatastoreTestMixin, _VisionAPITestCase):
    @property
    def estimate_url(self) -> str:
        return f"{self.scanners_url}estimate/"

    def _ingest_session(self, *, days_ago: float) -> None:
        # InsightsQL skips non-UUIDv7 `$session_id` values, so the estimate query would return 0 for them.
