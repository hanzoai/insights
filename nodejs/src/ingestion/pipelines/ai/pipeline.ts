...
import { createPrefetchInsightsFunctionsStep } from '~/ingestion/common/steps/event-processing/prefetch-insights-functions-step'
...
>(config: AiIngestionPipelineConfig) {
    ...
    return (
        newCommonIngestionPipeline<TInput, TContext, OverflowOutput>({
            ...
            // Prefetch script functions for the batch's teams so the transformer
            // honors Script watcher's disabled-function state (mirrors analytics).
            .pipeChunk(createPrefetchInsightsFunctionsStep(hogTransformer, cdpHogWatcherSampleRate))
