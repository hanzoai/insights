...
from insights.models import IdentityProviderConfig, LinkedIdentityProviderConfig, Organization, OrganizationDomain
...
class TestIdentityProviderConfig(BaseTest):
    def _create_domain(self, domain: str = "hanzo.ai", **kwargs) -> OrganizationDomain:
    ...
    def test_deleting_domain_keeps_config_linked_to_another_domain(self):
        domain = self._create_domain()
        other_domain = self._create_domain(domain="other.hanzo.ai")
        config = self._create_linked_config(domain, saml_entity_id="entity-id")
    ...
    def test_cross_org_config_link_fails_on_create(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            OrganizationDomain.objects.create(
                organization=self.organization, domain="hanzo.ai", identity_provider_config=other_config
            )
        assert "identity_provider_config" in exc_info.value.message_dict
