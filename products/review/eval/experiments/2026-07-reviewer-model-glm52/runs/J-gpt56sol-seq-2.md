...
- **Report id:** `019fb731-4834-71f4-8863-7010a1f5831d` · **PR:** https://github.com/Insights/insights/pull/75215
...
### [❌ dismissed] should_fix · security — products/stamp/backend/tasks/tasks.py:113-120
...
- **Validator:** - **Checked:** Traced `_parse_pr_url` (PR-head `products/stamp/backend/tasks/tasks.py:116`) and its sole non-test caller `process_inbox_pr_review` (`tasks.py:1110`); traced `pr_url` back through `queue_inbox_pr_review` (`facade/api.py:128`) to review's `handle_task_run_saved`/`_start_stamp_review` (`receivers.py`), where it is `TaskRun.output["pr_url"]`; checked every downstream use of the parsed `(owner/repo, number)`.
- **Found:** The parsed tuple only resolves a `team_id`-scoped `StampRepoConfig` filtered `enabled=True`, non-blank `installation_id`, `connected_by_user_id` set, `repository__iexact=<owner/repo>` (`tasks.py:1139-1148`); the subsequent GitHub fetch uses `repo_config.repository` and `repo_config.installation_id` from the DB (`tasks.py:1160`). The URL's scheme/host/port are never used for any request — so a spoofed `attacker.example/github.com/...` host yields the exact same extracted target as a clean URL and changes nothing downstream.
...
- **Impact:** For all legitimate inputs the leftmost-match regex extracts correctly, so there is no correctness bug; the target is bounded to the team's own enabled+synced Stamp repos; and the proposed anchoring provides no security benefit against the described redirection. This is minor input-hardening/consistency polish (matching `_is_github_pull_request_url_for_repository`), not a reachable security defect — below the keep bar (speculative/wrong-premise).
...
### [✅ VALID] must_fix (validator→should_fix) · bug — products/review/backend/receivers.py:111-135,144-160

**Stamp gate checks only one assignee instead of any opted-in assignee**  
_perspective: review-script-perspective-logic-correctness · directly-related: True_

- **Problem:** Both the initial trigger and webhook resolver select one canonical acting reviewer, then inspect only that user's `stamp_review_inbox_prs` setting. If that reviewer is opted out while another assigned reviewer is opted in, no Stamp review runs. This contradicts the stated requirement that a review run when at least one assigned user has enabled the toggle, and turning off one user's toggle can incorrectly stop later re-reviews despite another assignee remaining opted in.
- **Suggestion:** Resolve the complete ordered set of assigned organization users for the Stamp path, select an opted-in user with `next((user for user in resolved if ReviewUserSettings.load(team_id, user.id).stamp_review_inbox_prs), None)`, and return that user's ID. Preserve the existing single acting-reviewer selection separately for Review, whose configuration is intentionally tied to the task creator or primary assignee. Use the same any-opted-in resolver for both initial dispatch and webhook rechecks.
- **Validator:** - **Checked:** the full receivers.py PR diff — `_resolve_assigned_reviewer`, the initial trigger in `handle_task_run_saved`, the webhook re-check `resolve_stamp_acting_reviewer`, and the `inbox_hooks` registry that stamp's webhook task calls through.
- **Found:** `_resolve_assigned_reviewer` collapses to ONE acting reviewer — `acting = next((user for user in resolved if user.id == task_created_by_id), resolved[0])` (receivers.py ~L204) — and both legs gate on only that user: the trigger does `if pr_url is not None and settings.stamp_review_inbox_prs:` where `settings = ReviewUserSettings.load(team_id, acting_user_id)`, and `resolve_stamp_acting_reviewer` re-derives the same single acting id then checks only its toggle. No path iterates the resolved set for any opted-in user.
- **Found:** for a fully auto-started self-driving PR, `created_by` is the GitHub-integration bot (per the module docstring) and is not in the resolved set, so the acting reviewer is `resolved[0]` — the first suggested reviewer by artefact order. If `resolved[0]` is opted out of stamp while a later assignee is opted in, no stamp review is queued; and on a later push, if `resolved[0]` toggles off, `resolve_stamp_acting_reviewer` returns None and re-reviews stop even though another assignee remains opted in.
- **Found:** the PR body states the gate three times as any-opted-in — "any assigned reviewer opted in" (Case 1), "at least one of the assigned users has `stamp_review_inbox_prs` enabled", "nobody is opted in, so no review runs" (Case 3). The implementation checks exactly one, so it diverges from the feature's own stated central behavior. Architecturally this fits: Review needs the single-acting collapse to pick ONE user's perspectives/urgency config to drive the review, but stamp's toggle is a binary gate with no per-user options, so reusing the single-acting resolver for it silently ignores other assignees' opt-in.
- **Impact:** realistic multi-reviewer reports (differing per-user opt-in, default-off) hit this; stamp reviews are skipped for cases the stated design says should run, and one user toggling off can halt re-reviews despite another remaining opted in. Concrete trigger + concrete consequence, directly in the code this PR adds.
...
- **Validator:** - **Checked:** Read `find_task_run` (`products/tasks/backend/webhooks.py:29-98`), `find_signal_implementation_run` (`products/tasks/backend/facade/api.py:484-517`), the carve-out call site (`products/stamp/backend/tasks/tasks.py:191`), and the run-provenance invariants in `products/review/backend/receivers.py`.
...
- **Validator:** - **Checked:** `_start_stamp_review` in the PR diff, the pre-existing `_start_review` (Review) leg it mirrors, the module-level docstring on re-trigger semantics (receivers.py L59-63), and the `queue_inbox_pr_review` facade behavior.
- **Found:** the catch is `except Exception: logger.exception("review_stamp_inbox_review_queue_failed")` — the failure is logged at exception level, not hidden. Both the docstring ("the broker being down must never surface into the saver") and the identical, already-accepted pattern on the Review `_start_review` leg show this swallow-and-log is a deliberate, documented convention for this file, which runs inside the tasks save path and must never raise into it. This is not a new defect introduced by the change.
- **Found:** the dispatch is a Celery broker publish (`queue_inbox_pr_review` "queues a Celery task, so the only work on this save path is the broker publish"); the only loss scenario is a rare transient broker outage at the exact on-commit moment. Recovery paths already exist: receivers.py L61-63 states repeat TaskRun saves with an unchanged target deliberately re-fire the receiver (re-dispatching the stamp leg while the toggle is on), and PR Case 2 re-reviews via stamp's webhook path on the next push — the finding itself concedes a later save retriggers it.
- **Impact:** the suggested remedy (a durable outbox / persisted pending-dispatch state + a retrying worker) is a substantial reliability subsystem for a best-effort, fail-safe (no wrong approval — only a possibly-delayed review), explicitly experimental feature that already has natural re-trigger paths. That is the 'add an abstraction / future-proof for a case not in scope' shape the criteria drops as overengineering, and it would apply equally to the long-standing Review leg it mirrors. It does not clear the bar of a reliability defect that will realistically bite.

### [✅ VALID] must_fix · security — products/stamp/backend/tasks/tasks.py:1158-1183
...
- **Impact:** A signal run that reports (via prompt-injected/mistaken agent output) a `pr_url` pointing at a fork or human-authored PR in one of the team's enabled Stamp repos gets that PR privileged-reviewed and potentially approved, with no fork/bot/association gate anywhere — an errant GitHub approval that can satisfy required reviews. Reachable, though bounded to the team's own enabled repos and gated on the acting reviewer's toggle.
- **Priority:** Keeping must_fix. Note two overstatements in the suggestion that do NOT lower the core severity: re-verifying that `task_run_id` belongs to `team_id`/carries `signal_report_id` is largely redundant (the in-process review caller already establishes these from the real TaskRun and Celery args aren't externally injectable), and the claim that webhook re-reviews 'treat stored pr_url alone as proof' is inaccurate (the carve-out already does bot+fork+facade-match). The genuinely missing controls are the bot-author and repo-native checks on the receiver leg's fetched PR.

### [❌ dismissed] should_fix · bug — products/stamp/backend/tasks/tasks.py:1185-1201
...
### [❌ dismissed] must_fix · best_practice — products/stamp/backend/tasks/tasks.py:1185-1190
...
### [✅ VALID] must_fix · security — products/stamp/backend/tasks/tasks.py:124-127,161-171
...
- **Suggestion:** Require the expected Insights Code GitHub App bot login and confirm the matched run used bot authorship before granting provenance. Add those identity facts to SignalImplementationRunDTO if necessary, and fail closed when the expected bot identity cannot be established.
- **Validator:** - **Checked:** `_inbox_rereview_carve_out` (products/stamp/backend/tasks/tasks.py:144-215), its author gate `_is_bot_authored` (tasks.py:124-127), the run lookup `find_signal_implementation_run` (products/tasks/backend/facade/api.py:484-516) and its `find_task_run` matcher (products/tasks/backend/webhooks.py:30-99), plus the product's own bot-identity model in logic/github_client.py and products/stamp/CLAUDE.md.
...
- **Impact:** An attacker-controlled bot with repository write access can open a repo-native PR whose head branch reuses a signal-run branch name, pass the 'any Bot' floor and the branch-leg run match, and inherit the draft/bot/review-mode/write-permission bypass — causing Stamp to review and potentially post a standing GitHub approval on a PR that is not genuinely a Insights Code self-driving run. This is a real trust-boundary gap consistent with the product's documented invariant, and the requested fix (require the expected `<slug>[bot]` login, confirm bot authorship on the matched run, fail closed when identity is unestablished) matches the existing `allow_any_bot=False` pattern.

### [✅ VALID] must_fix · bug — products/review/backend/migrations/0019_reviewusersettings_stamp_review_inbox_prs.py:7-9

**Rebase this migration onto the current Review migration leaf**  
_perspective: review-script-perspective-logic-correctness · directly-related: True_
...
- **Suggestion:** Run `python manage.py rebase_migration review` so this becomes the next migration after `0024_reviewreport_published_head_shas` (currently `0025`), and update `max_migration.txt` accordingly.
- **Validator:** - **Checked:** the full `products/review/backend/migrations/` chain on master, the fork point's `dependencies`, `max_migration.txt`, and whether django_linear_migrations is active.
- **Found:** master already has `0019_reviewreport_author_login_and_more` → ... → `0024_reviewreport_published_head_shas`, and `0019_reviewreport_author_login_and_more.py:8` depends on `0018_backfill_urgency_threshold_to_consider`. The PR's new `0019_reviewusersettings_stamp_review_inbox_prs` also depends on `0018`, so `0018` now has two children and the graph has two leaves (`0024` and the new `0019`). `max_migration.txt` currently pins the single leaf `0024_reviewreport_published_head_shas`.
- **Found:** `django_linear_migrations` is installed (`insights/settings/web.py:207`), so besides Django's own "multiple leaf nodes" error on migrate/makemigrations, the linear-migrations consistency check (single leaf must match `max_migration.txt`) fails in CI.
- **Impact:** Concrete trigger — merging this branch onto current master; concrete consequence — `manage.py migrate`/`makemigrations --check` errors with conflicting leaf nodes and CI blocks, so the migration never applies. The suggested `rebase_migration review` (renumber to `0025` after `0024` and update `max_migration.txt`) is the correct remedy. Matches the PR's `dirty` mergeable state.

### [❌ dismissed] must_fix · bug — products/review/backend/receivers.py:114-114,151-155

**Pin toggle gates to the writer database**  
_perspective: review-script-perspective-performance-reliability · directly-related: True_

- **Problem:** Both the initial dispatch and webhook re-review gate call `ReviewUserSettings.load()` through the default read route. A recently changed toggle can therefore be read from a lagging replica. Most critically, a webhook arriving immediately after opt-out may observe the old `true` value and launch a review that can restore an approval the user intended to disable. Stamp's existing `enabled` gates explicitly pin reads to `router.db_for_write(...)` to avoid this failure mode.
- **Suggestion:** Load `ReviewUserSettings` from the writer for these decision gates, for example by adding a writer-pinned settings loader using `.using(router.db_for_write(ReviewUserSettings))`, and use it in both the TaskRun receiver and `resolve_stamp_acting_reviewer`.
- **Validator:** - **Checked:** the two active DB routers (`insights/dbrouter.py`, `insights/product_db_router.py`), the product-DB registry `products/db_routing.yaml`, the replica opt-in setting, and `ReviewUserSettings.load()` itself.
- **Found:** `ReviewUserSettings` is in the `review` app (`products/review/backend/apps.py:6`), which is NOT listed in `products/db_routing.yaml` (only `stamp`, `visual_review`, `warehouse_sources_queue` are). So `ProductDBRouter.db_for_read`/`db_for_write` return `None` for it — no reader/writer split.
- **Found:** `ReplicaRouter.db_for_read` (`insights/dbrouter.py:22-31`) sends reads to `"replica"` only when the model name is in `READ_REPLICA_OPT_IN`, else `"default"`. `READ_REPLICA_OPT_IN` is env-driven and empty by default (`insights/settings/data_stores.py:271-272`), and nothing opts `ReviewUserSettings` in. `load()` (`products/review/backend/models.py:363-367`) issues a plain `.for_team(...).filter(...).first()` with no `.using()`, so it already reads from `default` (the writer/primary) — a strongly-consistent read.
- **Found:** Stamp's `db_for_write` pins exist because Stamp IS a registered product DB whose `db_for_read` returns `stamp_db_reader`, a genuine lagging replica (`insights/product_db_router.py:27-35`, and its own code comments warn about a "lagged reader"). That rationale is specific to product-DB models and does not transfer to a main-DB model.
...
### [✅ VALID] should_fix · performance — products/stamp/backend/tasks/tasks.py:1201-1207
...
- **Validator:** - **Checked:** the dedup at `process_inbox_pr_review` (products/stamp/backend/tasks/tasks.py:1202-1210), what re-fires it (review/backend/receivers.py `handle_task_run_saved` + `_start_stamp_review` → facade `queue_inbox_pr_review`), the stamp workflow-id/reuse policy (products/stamp/backend/temporal/client.py:43-48), and the webhook path's create/dedup (tasks.py:1054-1073) for comparison.
...
- **Found:** Nothing collapses the re-launch at the Temporal layer: the workflow id is `f"stamp-review-{review_run_id}"` (client.py:43), keyed per ReviewRun, so each new row starts a _fresh_ full sandbox+LLM+GitHub review. This is the Review leg's deterministic-workflow-id + USE_EXISTING collapse (receivers.py docstring) that the stamp leg lacks specifically after a FAILED run.
