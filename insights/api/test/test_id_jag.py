...
The strategy is:
  * Generate a throwaway RSA keypair in `setUpClass` and use it to sign a
    synthetic IdP-issued ID-JAG. We patch `insights.api.id_jag._get_jwks_client`
...
from insights.api.id_jag import (
    ACCESS_TOKEN_TYPE,
    ID_JAG_TOKEN_TYPE,
...
from insights.auth import IDJagAccessTokenAuthentication
...
from insights.models.user import User as UserModel
...
_SITE_URL = "https://insights.test"
_AUTH_SERVER_URL = _SITE_URL
_RESOURCE_URL = _SITE_URL
_RESOURCE_CLIENT_ID = "client_abc-at-insights"
...
class TestIdJagTokenEndpoint(APIBaseTest):
    ...
    def setUp(self) -> None:
        ...
        self._jwks_patch = patch("insights.api.id_jag._get_jwks_client", return_value=mock_jwks_client)
    ...
    def test_issue_access_token_helper_rejects_without_billing_feature(self) -> None:
        ...
        from insights.api.id_jag import AccessDeniedError
    ...
    def test_email_claim_is_preferred_over_sub_for_user_lookup(self) -> None:
        ...
        # Insights user identity is stamped separately into `email` so the
    ...
    def test_unknown_scopes_in_id_jag_are_dropped(self) -> None:
        """We never propagate scope strings we don't recognize as Insights scopes —
    ...
    def test_rejects_unverified_domain_in_sub(self) -> None:
        ...
        # `sub` doesn't belong to a verified domain on any Insights org is
    ...
    def test_rejects_request_client_id_mismatch(self) -> None:
        assertion = _make_id_jag(client_id="client_abc-at-insights")
        resp = self._post_token(
            {
                "grant_type": JWT_BEARER_GRANT_TYPE,
                "assertion": assertion,
                "client_id": "client_DIFFERENT-at-insights",
            }
        )
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(resp.json()["error"], "invalid_grant")
    ...
    def test_arbitrary_client_id_is_accepted_when_no_allowlist(self) -> None:
        ...
        assertion = _make_id_jag(client_id="client_anything-at-insights")
        resp = self._post_token({"grant_type": JWT_BEARER_GRANT_TYPE, "assertion": assertion})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        body = resp.json()
        claims = jwt.decode(
        ...
        self.assertEqual(claims["client_id"], "client_anything-at-insights")
    ...
    @override_settings(ID_JAG_ALLOWED_AUDIENCES=["https://oauth.hanzo.ai"])
    def test_accepts_aud_matching_configured_allowed_audience(self) -> None:
        ...
        assertion = _make_id_jag(audience="https://oauth.hanzo.ai")
        resp = self._post_token({"grant_type": JWT_BEARER_GRANT_TYPE, "assertion": assertion})
        self.assertEqual(resp.status_code, status.HTTP_200_OK, resp.content)

    @override_settings(ID_JAG_ALLOWED_RESOURCES=["https://mcp.hanzo.ai"])
    def test_accepts_resource_matching_configured_allowed_resource(self) -> None:
        ...
        assertion = _make_id_jag(resource="https://mcp.hanzo.ai/")
        resp = self._post_token({"grant_type": JWT_BEARER_GRANT_TYPE, "assertion": assertion})
        self.assertEqual(resp.status_code, status.HTTP_200_OK, resp.content)
        claims = jwt.decode(
            resp.json()["access_token"],
            _public_key_for(_AS_PRIVATE_KEY_PEM),
            algorithms=["RS256"],
            audience="https://mcp.hanzo.ai",
            issuer=_AUTH_SERVER_URL,
        )
        self.assertEqual(claims["aud"], "https://mcp.hanzo.ai")
    ...
    def test_custom_jwks_url_is_passed_to_jwks_client(self) -> None:
        ...
        mock_jwks_client = MagicMock()
        mock_jwks_client.get_signing_key_from_jwt.return_value = signing_key
        with patch("insights.api.id_jag._get_jwks_client", return_value=mock_jwks_client) as mock_get:
            assertion = _make_id_jag()
            resp = self._post_token({"grant_type": JWT_BEARER_GRANT_TYPE, "assertion": assertion})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
...
class TestIDJagAccessTokenAuthentication(APIBaseTest):
    ...
    def test_opaque_sub_with_email_claim_authenticates_user(self) -> None:
        ...
        # Insights identity is in the separate `email` claim. The resource-side
