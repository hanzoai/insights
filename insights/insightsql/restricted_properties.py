...
def restricted_property_keys_for_table_type(table_type: ast.Type, context: InsightsQLContext) -> set[str]:
    """Top-level property names restricted by property-level access control for a table, or an empty set.

    Single source of truth shared by the Datastore printer (which JSONDropKeys-wraps the blob) and the property
