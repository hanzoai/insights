...
from insights.schema import DataWarehouseNode, TrendsQuery
...
class TestVariableAnalysis(APIBaseTest):
    ...
    def test_materialization_transform_compiles_warehouse_table_without_user_context(self):
        table = DataWarehouseTable.objects.create(
            team=self.team,
            name="web_vitals_mv",
            columns={"page": {"insightsql": "StringDatabaseField", "datastore": "String", "valid": True}},
        ...
        with mock.patch("insights.insightsql.database.database.feature_enabled_or_false", return_value=True):
            materialized_query = build_endpoint_insightsql(query, self.team, bypass_warehouse_access_control=True)
    ...
    def test_materialization_transform_respects_user_warehouse_access_control(self):
        ...
        table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "StringDatabaseField", "datastore": "String", "valid": True},
                "timestamp": {"insightsql": "DateTimeDatabaseField", "datastore": "DateTime", "valid": True},
            },
            format=DataWarehouseTable.TableFormat.Parquet,
        ...
        with mock.patch("insights.insightsql.database.database.feature_enabled_or_false", return_value=True):
            with pytest.raises(QueryError, match=f"You don't have access to table `{table.name}`"):
                build_endpoint_insightsql(query, self.team, user=self.user)
    ...
    def test_or_and_multi_column_variable_analysis(
        ...
        query = {"kind": "InsightsQLQuery", "query": query_str, "variables": variables}
...
class TestMaterializationEquivalence(DatastoreTestMixin, APIBaseTest):
    ...
    Strategy:
      1. Insert real events into Datastore with varied property values.
    ...
    def _run_insightsql(self, query_str: str) -> list[list]:
        ...
        runner = InsightsQLQueryRunner(team=self.team, query=InsightsQLQuery(query=query_str))
    ...
    def _assert_equivalent(self, original_query: str, variables: dict, variable_values: dict):
        ...
        Args:
            original_query: InsightsQL with {variables.X} placeholders
        ...
        inline_results = self._run_insightsql(inline_query)

        # 2. Transform for materialization
        insightsql_query = {"kind": "InsightsQLQuery", "query": original_query, "variables": variables}
        can_materialize, reason, var_infos = analyze_variables_for_materialization(insightsql_query)
        ...
        runner = InsightsQLQueryRunner(team=self.team, query=InsightsQLQuery(query=transformed["query"]))
        response = runner.calculate()
...
class TestDownstreamCTEClassifier(APIBaseTest):
    ...
    def test_extract_aggregate_name_canonicalizes_count_distinct(self, src, expected):
        from insights.insightsql.parser import parse_expr as _parse_expr
