...
describe('Hono App Routes', () => {
    ...
    describe('GET /', () => {
        it('should redirect to the docs', async () => {
            ...
            expect(res.headers.get('location')).toBe('https://hanzo.ai/docs/model-context-protocol')
        })
    })
    ...
    describe('OAuth Protected Resource Metadata', () => {
        ...
        it('should use oauth.hanzo.ai as authorization server', async () => {
            const { app } = createApp(mockRedis)
            const res = await app.request('/.well-known/oauth-protected-resource/mcp')
            const body = (await res.json()) as Record<string, any>
            expect(body.authorization_servers[0]).toBe('https://oauth.hanzo.ai')
        })
        ...
        it('should allow CORS from Insights app origins for consent-page metadata fetch', async () => {
            const { app } = createApp(mockRedis)
            const res = await app.request('/.well-known/oauth-protected-resource/mcp', {
                headers: { Origin: 'https://us.hanzo.ai' },
            })
            expect(res.status).toBe(200)
            expect(res.headers.get('Access-Control-Allow-Origin')).toBe('https://us.hanzo.ai')
            expect(res.headers.get('Vary')).toBe('Origin')
        })

        it('should answer OPTIONS preflight for Insights app origins', async () => {
            const { app } = createApp(mockRedis)
            const res = await app.request('/.well-known/oauth-protected-resource/mcp', {
                method: 'OPTIONS',
                headers: {
    ...
    describe('hostname-based region detection', () => {
        it('should detect EU from mcp.eu.hanzo.ai via X-Forwarded-Host', async () => {
            const { app } = createApp(mockRedis)
            const res = await app.request('/mcp', {
                method: 'POST',
                headers: { 'X-Forwarded-Host': 'mcp.eu.hanzo.ai' },
            })
            expect(res.status).toBe(401)
            const wwwAuth = res.headers.get('WWW-Authenticate') || ''
            expect(wwwAuth).toContain('mcp.eu.hanzo.ai')
        })

        it('should detect US from mcp.us.hanzo.ai', async () => {
            const { app } = createApp(mockRedis)
            const res = await app.request(
                new Request('http://mcp.us.hanzo.ai/.well-known/oauth-protected-resource/mcp')
            )
            expect(res.status).toBe(200)
        })

        it('should detect EU from legacy mcp-eu.hanzo.ai', async () => {
            const { app } = createApp(mockRedis)
            const res = await app.request('/mcp', {
                method: 'POST',
                headers: { 'X-Forwarded-Host': 'mcp-eu.hanzo.ai' },
    ...
    describe('MCP auth on /mcp', () => {
        ...
        it('should pass auth check for ID-JAG access tokens (typ: at+jwt)', async () => {
            ...
            // the header — the Insights API verifies the signature downstream.
            const headerB64 = btoa('{"typ":"at+jwt","alg":"RS256"}')
                .replace(/=+$/, '')
                .replace(/\+/g, '-')
                .replace(/\//g, '_')
            const payloadB64 = btoa('{"sub":"example.com:user@example.com","aud":"https://insights.test"}')
                .replace(/=+$/, '')
                .replace(/\+/g, '-')
                .replace(/\//g, '_')
    ...
    describe('OAuth redirect routes', () => {
        it('should redirect /.well-known/oauth-authorization-server', async () => {
            ...
            expect(res.headers.get('Location')).toContain('oauth.hanzo.ai')
        })
