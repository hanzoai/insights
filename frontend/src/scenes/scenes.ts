...
const redirectPipeline = (id: string, fallbackUrl: string): string => {
    // Script functions (destinations & transformations)
    if (id.startsWith('script-')) {
        return urls.insightsFunction(id.replace('script-', ''))
    }
...
> = {
    ...
    '/functions/:id/configuration': ({ id }) => urls.insightsFunction(id, 'configuration'),
