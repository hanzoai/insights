...
description: >
  Signals scout for Insights revenue analytics. Watches for upstream failures (Stripe sync
...
Event source configured for a subscription business, but `RevenueAnalyticsConfig.events[].subscriptionProperty` is null. The MRR view will be empty because Insights can't tell which charges belong to the same subscription. The dashboard renders but only gross revenue is meaningful.
...
#### Stripe-customer ↔ Insights-person join broken

Stripe customers should carry `insights_person_distinct_id` metadata so Insights can attach revenue to the person profile. If newly-created customers stop carrying that metadata (post-deploy regression in checkout flow), aggregate views still work but person-level revenue (group analytics, customer journeys) goes dark.

Detect via the `customer` view: count of customers with non-null `insights_person_distinct_id` in last 30d vs the 30d before. Scratchpad-worthy if the team isn't using person-level revenue features; report-worthy if they are (check `popular_insights` for person-breakdown revenue charts).
...
For deeper investigation, the sandbox image bakes `insights:auditing-warehouse-source-health` (catches Stripe-source failures upstream of revenue analytics) and `insights:diagnosing-failed-warehouse-syncs` (recovery actions for a failing sync).
