...
def _activate_personinsights_fake():
    """No-op override of the repo-root autouse fixture for the script tests.

    The repo-root ``conftest.py`` registers an autouse ``_activate_personinsights_fake``
    that imports ``insights`` -> ``django``. The ``.github/scripts`` unit tests run
    ...
    person/group data, so the personinsights fake is unnecessary here. Shadowing the
