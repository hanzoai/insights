...
class TestFlushKafkaBatchAsync:
    ...
    async def test_successful_batch_flush(self):
        ...
        with patch(
            "insights.temporal.messaging.backfill_precalculated_events_workflow.asyncio.to_thread"
        ) as mock_thread:
...
class TestBackfillPrecalculatedEventsActivity:
    ...
    async def test_missing_filters_raises_non_retryable(self):
        ...
        with (
            patch(
                "insights.temporal.messaging.backfill_precalculated_events_workflow.get_event_filters",
                return_value=None,
            ),
            patch(
                "insights.temporal.messaging.backfill_precalculated_events_workflow.asyncio.to_thread",
...
class TestBackfillResolvesMergedPersons:
    async def test_backfill_resolves_person_id_through_overrides(self, team):
        ...
        with (
            ...
            patch(
                "insights.temporal.messaging.backfill_precalculated_events_workflow.get_producer",
