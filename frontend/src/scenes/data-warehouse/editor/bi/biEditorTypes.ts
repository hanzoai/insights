...
export const BI_FIELD_DRAG_MIME_TYPE = 'application/x-insights-bi-field'
...
function fieldExpression(field: BIField): string {
    ...
    const escapedExpression = DOTTED_IDENTIFIER_REGEX.test(expression)
        ? escapeDottedInsightsQLIdentifier(expression)
        : expression
...
function filterExpression(filter: BIFilter): string | null {
    ...
    const value =
        isNumericBIField(filter.field) && Number.isFinite(Number(filter.value))
            ...
            : escapeInsightsQLString(filter.value)
    ...
    switch (filter.operator) {
        ...
        case 'not_equals':
            return `${field} != ${value}`
        case 'contains':
            return `lower(${field}) LIKE lower(${escapeInsightsQLString(`%${filter.value}%`)})`
        case 'greater_than':
            return `${field} > ${value}`
        case 'less_than':
            return `${field} < ${value}`
...
export function buildBIQuery(config: BIConfig): BIQueryBuildResult | null {
    ...
    const valueExpressions =
        configuredValues.length > 0
            ? configuredValues.map(({ value, expression }, index) => {
                  ...
                  return `${expression} AS ${escapePropertyAsInsightsQLIdentifier(alias)}`
              })
            : ['count(*) AS count']
    ...
    const queryParts = [
        ...
        `FROM ${escapePropertyAsInsightsQLIdentifier(config.source.table)}`,
    ]
