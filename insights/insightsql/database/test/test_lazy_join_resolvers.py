...
from insights.insightsql.database.lazy_join_registry import RESOLVERS
from insights.insightsql.database.lazy_join_tags import DATA_WAREHOUSE, DATA_WAREHOUSE_EXPERIMENTS, GROUP_N
from insights.insightsql.database.models import LazyJoin, LazyJoinToAdd, Table, TableNode
...
class TestLazyJoinResolvers(SimpleTestCase):
    ...
    def test_unsupported_join_key_is_an_exposed_query_error(self):
        ...
        with self.assertRaises(QueryError) as ctx:
            resolve_data_warehouse_join(join_to_add, InsightsQLContext(team_id=1), ast.SelectQuery(select=[]))
    ...
    def test_group_n_join_requires_group_index(self):
        ...
        with self.assertRaises(ResolutionError):
            join_with_group_n_table(join_to_add, InsightsQLContext(team_id=1), ast.SelectQuery(select=[]))
    ...
    def test_experiments_join_builds_asof_join_with_timestamp_bounds(self):
        ...
        join_expr = resolve_data_warehouse_experiments_join(join_to_add, InsightsQLContext(team_id=1), node)
...
class TestBuiltDatabaseSerializable(BaseTest):
    ...
    def test_built_database_lazy_joins_are_serializable(self, _name: str, modifiers: InsightsQLQueryModifiers) -> None:
