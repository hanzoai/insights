...
from insights.temporal.proxy_service.common import is_cloudflare_proxy_by_cname

# --- Customer-facing message helpers ---
# All user-visible strings live here so copy can be reviewed in one place. Functions
# return Insights-flavored sentences without leaking vendor terminology (no mentions of
...
def _check_live_event(record: ProxyRecord) -> CheckResult:
    try:
        # Same dummy payload the daily monitor uses (insights/temporal/proxy_service/monitor.py
        # check_proxy_is_live). Insights's capture endpoint accepts unknown api_keys and returns
