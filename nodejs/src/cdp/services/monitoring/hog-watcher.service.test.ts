...
describe('HogWatcher', () => {
    ...
    beforeAll(async () => {
        ...
        process.env.CDP_FN_WATCHER_2_ENABLED = 'true'
        process.env.CDP_FN_WATCHER_2_CAPTURE_ENABLED = 'true'
    })
    ...
    describe('constructor', () => {
        it('should validate the bounds configuration', () => {
            expect(() => {
                ...
                'Lower bound for kind script of 100ms must be lower than upper bound of 100ms. This is a configuration error.'
            )
        })
    })
    ...
    describe('observeResults', () => {
        ...
        describe('onStateChange', () => {
            it('should trigger state change events', async () => {
                ...
                await watcher.observeResults(Array(10).fill(createResult({ duration: 1000, kind: 'script' })))
                expect(await watcher.getPersistedState(insightsFunctionId)).toMatchInlineSnapshot(`
                    {
                      "state": 1,
                      "tokens": 8100,
                ...
                await watcher.clearLock(insightsFunctionId) // For testing the logic
                await watcher.observeResults(Array(100).fill(createResult({ duration: 1000, kind: 'script' })))
            ...
            it('should not automatically transition out of disabled', async () => {
                ...
                advanceTime(1000)
                await watcher.observeResults(Array(1).fill(createResult({ duration: 10, kind: 'script' })))
                expect(await watcher.getPersistedState(insightsFunctionId)).toMatchInlineSnapshot(`
                    {
                      "state": 3,
                      "tokens": 20,
            ...
            it('should not transition to a different state if forcefully set', async () => {
                ...
                `)
                await watcher.observeResults(Array(1000).fill(createResult({ duration: 1, kind: 'script' })))
    ...
    describe('getPersistedStates', () => {
        ...
        it('should cap refill at bucketSize after large time gap', async () => {
            ...
            expect((await watcher.getPersistedState(insightsFunctionId)).tokens).toBeLessThan(10000)
        ...
        it('should handle pool existing without ts in Redis', async () => {
            ...
            await redis.useClient({ name: 'test-setup' }, async (client) => {
                await client.hset(`${BASE_REDIS_KEY}/tokens/${insightsFunctionId}`, 'pool', '5000')
            })
        ...
        it('should handle ts existing without pool in Redis', async () => {
            ...
            await redis.useClient({ name: 'test-setup' }, async (client) => {
                await client.hset(`${BASE_REDIS_KEY}/tokens/${insightsFunctionId}`, 'ts', Math.round(now / 1000))
            })
    ...
    describe('reader fallback', () => {
        ...
        it('should fall back to the writer when the reader throws on getPersistedStates', async () => {
            ...
            await watcherWithFailingReader.observeResults([createResult({ duration: 1000, kind: 'script' })])
        ...
        it('should fall back to the writer when the reader throws on getAllFunctionStates', async () => {
            ...
            expect(failingReader.useClient).toHaveBeenCalledTimes(1)
            expect(Object.keys(states)).toContain(insightsFunctionId)
            expect(loggerWarnSpy).toHaveBeenCalledWith(
                '🔀',
    ...
    describe('observeAggregatedResults', () => {
        ...
        it('charges no cost when the aggregate duration is below the lower bound', async () => {
            await watcher.observeAggregatedResults([{ insightsFunction, totalDurationMs: 50 }])
            const state = await watcher.getPersistedState(insightsFunctionId)
            expect(watcherConfig.bucketSize - state.tokens).toEqual(0)
        })

        it('charges the full cost at the upper bound', async () => {
            await watcher.observeAggregatedResults([{ insightsFunction, totalDurationMs: 550 }])
            const state = await watcher.getPersistedState(insightsFunctionId)
        ...
        it('scales cost with total VM time past the upper bound', async () => {
            ...
            await watcher.observeAggregatedResults([{ insightsFunction, totalDurationMs: 1050 }])
            const state = await watcher.getPersistedState(insightsFunctionId)
        ...
        it('accumulates cost across observations for the same function', async () => {
            await watcher.observeAggregatedResults([
                { insightsFunction, totalDurationMs: 300 },
                { insightsFunction, totalDurationMs: 300 },
            ])
            const state = await watcher.getPersistedState(insightsFunctionId)
        ...
        it('charges each function independently', async () => {
            const other = createInsightsFunction({ id: 'other-fn', team_id: 2 })
            await watcher.observeAggregatedResults([
                { insightsFunction, totalDurationMs: 550 },
                { insightsFunction: other, totalDurationMs: 50 },
            ])
            expect(watcherConfig.bucketSize - (await watcher.getPersistedState(insightsFunctionId)).tokens).toEqual(100)
