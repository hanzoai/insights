...
# `insights.schema` onto the research path.
...
- **Each chart is `chart_id` + `title` + `query`.** `chart_id` is your own slug (lowercase letters, numbers, `_`, `-`); `title` is the heading above it; `query` is a query node — `InsightVizNode` (an ad-hoc product-analytics chart), `DataVisualizationNode` (a `InsightsQLQuery` source, plus `display` and `chartSettings` for a graph rather than a result table), or `SavedInsightNode` (an existing insight by `shortId`). Any other kind is refused. Add a `caption` when there's a specific thing to look at.
...
- **Only attach a query you actually ran this session.** A well-formed node of an allowed kind holding a broken query is stored without complaint and then fails to draw when the reader opens the report, with nothing to tell you. So build each chart from a query you already executed through `mcp__insights__exec` (`call query-run {{...}}`, `call execute-sql {{...}}`, or read the exact node off an existing insight) – never one written from memory.
- **A chart renders data, it does not run code.** ScriptVM `bytecode`, a nested `ScriptQuery`, `sendRawQuery`, and a nested `SuggestedQuestionsQuery` are each refused wherever they sit in the node. A warehouse query is fine through InsightsQL — keep `connectionId`, drop `sendRawQuery`.
...
_RESEARCH_PREAMBLE = """You are a research agent investigating a signal report for the Insights codebase.
...
When naming a Insights product, we use its real name (for example "error tracking", not a third-party equivalent like "Sentry"). We only name an external vendor if the source data explicitly does.
...
1. **The codebase** – the full Insights repository is available on disk. Use file search, grep, and code reading.
2. **Insights analytics data** – one MCP tool, `mcp__insights__exec`, which takes a CLI-style `command` string. Load it in your first tool call with exactly: `ToolSearch("select:mcp__insights__exec")`
then use `call execute-sql {...}`, `call read-data-schema {...}`, `call query-run {...}`, and `info <command>` when you need a command's schema. `execute-sql`, `query-run`, `read-data-schema`, `insights-get-all`, `experiment-get`, `list-errors`, `feature-flag-get-all` and the rest are *commands you pass to* `mcp__insights__exec`, not tool names – there is no `mcp__insights__execute-sql` tool, and searching for one wastes a turn.
...
- **Data:** Run Insights MCP commands through `mcp__insights__exec` (`call execute-sql {...}`, `call query-run {...}`, `call read-data-schema {...}`, etc.) to check real impact – error rates, user counts, conversion metrics. If the signal references a specific insight, experiment, or feature flag, look it up directly.
