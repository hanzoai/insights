...
4. **Envoy backend health** — `envoy_cluster_membership_healthy` vs `envoy_cluster_membership_total`
   for `envoy_cluster_name=~"insights_capture-.*"`. Ratio < 1.0 outside of deploys is a problem.
...
1. **Envoy layer** — `envoy_cluster_upstream_rq_time_bucket` histogram for
   `envoy_cluster_name=~"insights_capture-.*"`.
...
5. **Contour rate limit** (per-IP, Envoy-layer, NOT capture binary)
   ...
   - Labels: `domain="insights"`, `key1`, `key2` for rule identification
...
1. **CloudWatch ElastiCache** — use datasource `P034F075C744B399F`.
   ...
   Cluster IDs:
   - Primary: `insights-solo` (prod-us) or `insights-prod-redis-encripted` (prod-eu; sic — typo in actual name)
...
5. **Both envs have a dedicated ingestion MSK cluster** — capture-analytics and capture-ai
   produce to `msk-ingestion`, not the events cluster (prod-us: c21; prod-eu: `insights-prod-eu-ingestion-*`).
