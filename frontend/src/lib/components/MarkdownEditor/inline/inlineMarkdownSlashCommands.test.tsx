...
const PREFLIGHT_WITH_OBJECT_STORAGE = {
    ...
    demo: false,
    celery: true,
    realm: Realm.SelfHostedDatastore,
    region: null,
...
describe('InlineMarkdownSlashMenu', () => {
    ...
    it('clamps keyboard selection when the Image row disappears without a query change', () => {
        ...
        expect(screen.getByText('Image').closest('button')).toHaveClass('Button--active')
        ...
        expect(screen.queryByText('Image')).not.toBeInTheDocument()
        expect(screen.getByText('Gamma').closest('button')).toHaveClass('Button--active')
    })
