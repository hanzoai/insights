"""Precheck handlers for insightscli command lifecycle.

Precheck handlers run before composite commands execute (see ``steps:`` /
``prechecks:`` entries in insightscli.yaml). They register against
``insightscli.hooks.register_precheck`` at boot, and the heavy work happens lazily
...
def _migrations_precheck(check: dict, yes: bool) -> bool | None:
    ...
    # not at boot.
    from insightscli_commands.migrations import _compute_migration_diff, _get_cached_migration
