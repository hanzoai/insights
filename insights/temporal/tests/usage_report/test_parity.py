...
from insights.temporal.usage_report.activities import aggregate_and_chunk_org_reports
from insights.temporal.usage_report.queries import QUERIES
from insights.temporal.usage_report.storage import queries_key, write_json
from insights.temporal.usage_report.types import AggregateInputs, AggregateResult, RunQueryToS3Result, WorkflowContext
...
def _install_in_memory_object_storage(monkeypatch: pytest.MonkeyPatch) -> dict[str, bytes]:
    ...
    monkeypatch.setattr("insights.storage.object_storage.write", _write)
    monkeypatch.setattr("insights.storage.object_storage.write_stream", _write_stream)
    monkeypatch.setattr("insights.storage.object_storage.read_bytes", _read_bytes)
    return s3
...
def test_end_to_end_parity_celery_task_vs_temporal_activity(
    ...
    # and silence the start/end Insights `capture` calls.
    monkeypatch.setattr("hanzo_insights.feature_enabled", lambda *a, **kw: False)
    monkeypatch.setattr("insights.tasks.usage_report.get_ph_client", lambda *a, **kw: mock.MagicMock())
    # Skip the heavy gather queries — both paths use this same `seeded`
    # dict, just shaped differently for each entry point.
    monkeypatch.setattr(
        "insights.tasks.usage_report._get_all_usage_data_as_team_rows",
        lambda *a, **kw: seeded,
    )
