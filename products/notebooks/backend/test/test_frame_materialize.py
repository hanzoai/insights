...
from insights.temporal.common.datastore import DatastoreMemoryLimitExceededError, DatastoreTooManyRowsOrBytesError
...
class TestFrameMaterializeEnqueue(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            (
                "memory_budget",
                DatastoreMemoryLimitExceededError("MEMORY_LIMIT_EXCEEDED", query="SELECT 1"),
            ...
            (
                "result_size_budget",
                DatastoreTooManyRowsOrBytesError("TOO_MANY_ROWS_OR_BYTES", query="SELECT 1"),
                "too large",
            ),
        ]
    )
    def test_budget_error_is_terminal_with_a_clear_message(self, _name, datastore_error, expected_message):
        # A deterministic Datastore budget failure must be non-retryable and carry a
        ...
        with (
            patch.object(frame_materialize, "_print_datastore_sql", return_value=("SELECT 1", {})),
            patch.object(frame_materialize, "_materialize_slots"),
            patch.object(frame_materialize.DatastoreClient, "post_query", side_effect=datastore_error),
        ):
    ...
    def test_mid_stream_failure_removes_the_corrupt_object_and_surfaces_the_real_error(self):
        # Datastore streams 200 before execution finishes; a mid-stream failure can close
        ...
        with self.settings(OBJECT_STORAGE_ENABLED=True):
            with (
                patch.object(frame_materialize, "_print_datastore_sql", return_value=("SELECT 1", {})),
                patch.object(frame_materialize, "_materialize_slots"),
                patch.object(frame_materialize.DatastoreClient, "post_query") as post_query,
                patch.object(
                    frame_materialize,
    ...
    @parameterized.expand(
        [
            # A storage-side upload failure (or a torn stream) with no Datastore-side
