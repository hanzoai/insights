                ...
                if (usesIamRole) {
                    errors.role_arn = !role_arn
                        ...
                        : !S3_ROLE_ARN_REGEX.test(role_arn)
                          ? 'Enter a valid IAM role ARN, e.g. arn:aws:iam::123456789012:role/insights-import'
                          : null
                }
    ...
    listeners(({ actions, values, cache }) => ({
        ...
        submitManagedMigrationSuccess: async ({ managedMigration }) => {
            if (managedMigration?.is_trial) {
                toast.success('Trial run started — results will appear here when it completes')
            }
        ...
        loadTrialRecordsPageFailure: async ({ errorObject }) => {
            if (errorObject?.status === 410) {
                toast.error('Trial results have expired and are no longer available')
            } else {
        ...
        confirmPromoteTrial: async ({ id }) => {
            try {
                ...
                toast.success('Import started from trial run')
                actions.loadMigrations()
            } catch (error: any) {
                toast.error(error?.data?.error || error?.message || 'Failed to start import from trial')
            } finally {
