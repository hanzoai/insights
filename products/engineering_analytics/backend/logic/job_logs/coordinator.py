...
from insights.models.integration import _is_safe_github_repo_path
...
def _query_failed_jobs(team: Team, prefix: str, cutoff_iso: str) -> list[dict[str, Any]]:
    ...
    with tags_context(product=Product.ENGINEERING_ANALYTICS, feature=Feature.QUERY, team_id=team.pk):
        response = execute_insightsql_query(
            ...
            # Trusted internal sweep with no request user: without this, InsightsQL's access-control build
...
class GithubJobLogsCoordinatorWorkflow(InsightsWorkflow):
    @staticmethod
