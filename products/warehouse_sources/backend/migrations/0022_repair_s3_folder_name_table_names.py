...
`stripe_balance_transaction`), breaking InsightsQL queries on the conventional name. The Delta data is
...
UPDATE insights_datawarehousetable AS t
SET name = lower(coalesce(src.prefix, '') || src.source_type || '_' || replace(s.name, '.', '__')),
    updated_at = now()
FROM insights_externaldataschema AS s
JOIN insights_externaldatasource AS src ON src.id = s.source_id
WHERE s.table_id = t.id
  ...
  AND NOT EXISTS (
      SELECT 1
      FROM insights_datawarehousetable AS t2
