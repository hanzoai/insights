...
convention documented in ``insights/api/github_callback/README.md``: per-user
...
* ``GET /complete/slack-link/start/``
  ...
  Login-gated, so an unauthenticated visitor is bounced through Insights
...
* ``GET /complete/slack-link/``
  ...
  Slack identity
  to the currently-logged-in Insights user. On success we DM the user back in
...
from insights.api.github_callback.redirects import PERSONAL_INTEGRATIONS_SETTINGS_PATH
...
from insights.models.user_integration import user_slack_integration_from_identity
...
def _load_workspace_integration(insights_team_id: int, slack_team_id: str) -> Integration | None:
    return (
        Integration.objects.filter(team_id=insights_team_id, kind="slack", integration_id=slack_team_id)
...
def slack_user_link_authorize(request: HttpRequest) -> HttpResponse:
    ...
    workspace_integration = _load_workspace_integration(invite.insights_team_id, invite.slack_team_id)
    ...
    callback_state = CallbackState(
        ...
        insights_team_id=invite.insights_team_id,
        insights_user_id=request.user.id,
...
def slack_user_link_callback(request: HttpRequest) -> HttpResponse:
    ...
    Insights user who started the flow (``state.insights_user_id``) — without
    ...
    workspace_integration = _load_workspace_integration(state.insights_team_id, state.slack_team_id)
    if workspace_integration is None:
        return _settings_redirect(error="workspace_not_found")
    ...
    insights_user = cast(User, request.user)

    # CSRF guard: bind the link write to the same Insights user who initiated
    ...
    # Insights account — every subsequent @mention from the attacker would then
    ...
    if state.insights_user_id != insights_user.id:
        logger.warning(
            "slack_app_user_link_callback_session_mismatch",
            state_insights_user_id=state.insights_user_id,
            request_insights_user_id=insights_user.id,
        )
        return _settings_redirect(error="session_mismatch")

    # Refuse to write a link row for a Insights user who isn't a member of
    # the workspace's org. The org-scope filter in `find_linked_insights_user`
    ...
    if not OrganizationMembership.objects.filter(
        ...
        logger.warning(
            ...
            insights_user_id=insights_user.id,
            insights_team_id=workspace_integration.team_id,
            organization_id=workspace_integration.team.organization_id,
        )
    ...
    *,
    ...
    try:
        ...
        client.chat_postEphemeral(
            ...
            text="✅ Your Insights account is now linked. Mention me again and I'll route to you correctly.",
        )
    except Exception:
        logger.info(
