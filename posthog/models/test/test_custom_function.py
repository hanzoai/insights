...
class TestCustomFunction(TestCase):
    ...
    def test_custom_function_basic(self):
        item = CustomFunction.objects.create(name="Test", team=self.team, type="destination")
        assert item.name == "Test"
        assert item.hog == ""
        assert not item.enabled

    def test_custom_function_team_no_filters_compilation(self):
        item = CustomFunction.objects.create(name="Test", team=self.team, type="destination")
    ...
    def test_custom_function_filters_compilation(self):
        ...
        item = CustomFunction.objects.create(
            name="Test",
    ...
    def test_custom_function_team_filters_only_compilation(self):
        self.team.test_account_filters = [
            {"key": "$host", "operator": "not_regex", "value": r"^(localhost|127\.0\.0\.1)($|:)", "type": "event"},
        ]
        self.team.save()
...
class TestCustomFunctionsBackgroundReloading(TestCase, QueryMatchingTest):
    def setUp(self):
        super().setUp()
        org, team, user = User.objects.bootstrap("Test org", "ben@posthog.com", None)
        self.team = team
    ...
    def test_custom_functions_reload_on_action_saved(self):
        custom_function_1 = CustomFunction.objects.create(
            name="func 1",
            type="destination",
            team=self.team,
            filters={
        ...
        custom_function_2 = CustomFunction.objects.create(
            name="func 2",
            type="destination",
            team=self.team,
            filters={
        ...
        assert (
            json.dumps(custom_function_1.filters["bytecode"])
        ...
        assert (
            json.dumps(custom_function_2.filters["bytecode"])
        ...
        with self.assertNumQueries(12):
            self.action.save()
        custom_function_1.refresh_from_db()
        custom_function_2.refresh_from_db()
    ...
    def test_custom_functions_reload_on_team_saved(self):
        self.team.test_account_filters = []
        ...
        custom_function_3 = CustomFunction.objects.create(
            name="func 3",
            type="destination",
            team=self.team,
            filters={
        ...
        # Check that the bytecode is correct
        assert json.dumps(custom_function_1.filters["bytecode"]) == f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 29]'
        assert (
            json.dumps(custom_function_2.filters["bytecode"])
            == f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 32, "$pageview", 32, "event", 1, 1, 11]'
        )

        assert json.dumps(custom_function_3.filters["bytecode"]) == f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 29]'
        ...
        custom_function_1.refresh_from_db()
        custom_function_2.refresh_from_db()
        custom_function_3.refresh_from_db()
    ...
    def test_geoip_transformation_created_when_enabled(self, mock_get_templates):
        ...
        transformations = CustomFunction.objects.filter(team=team, type="transformation")
    ...
    def test_custom_function_file_system(self):
        custom_function_3 = CustomFunction.objects.create(
            name="func 3",
            type="destination",
            team=self.team,
        ...
        file = FileSystem.objects.filter(
            team=self.team, type="custom_function/destination", ref=str(custom_function_3.id)
        ).first()
        assert file is not None
        assert file.path == "Unfiled/Destinations/func 3"

        custom_function_3.deleted = True
        custom_function_3.save()
