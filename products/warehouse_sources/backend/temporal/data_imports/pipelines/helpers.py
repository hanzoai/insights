...
def build_table_name(source: ExternalDataSource, schema_name: str):
    # Dots in `schema_name` would parse as `<table>.<column>` in InsightsQL, so any source that ever
...
def resolve_table_and_folder_names(schema_name: str, resolved_s3_folder_name: str | None) -> tuple[str, str]:
    ...
    - `build_table_name` only lower-cases, so the InsightsQL table name must derive from the *raw* schema
