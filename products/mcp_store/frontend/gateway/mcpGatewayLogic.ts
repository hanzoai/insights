    ...
    listeners(({ actions, cache, values }) => ({
        ...
        submitAddServer: async () => {
            ...
            try {
                ...
                toast.success(`${values.addServerForm.name.trim()} added`)
                actions.closeAddServerModal()
                actions.refreshServersAfterConnection()
                actions.loadServiceAccounts()
            } catch (error: unknown) {
                toast.error(errorDetail(error) ?? 'Could not add this server')
            } finally {
        ...
        refreshServerTools: async ({ installationId }) => {
            try {
                ...
                toast.error(errorDetail(error) ?? 'Could not refresh tools')
            } finally {
        ...
        setAllServersEnabled: async ({ enabled }) => {
            try {
                ...
                toast.error(errorDetail(error) ?? 'Could not update all servers')
            } finally {
        ...
        removeServer: async ({ serverId }) => {
            ...
            try {
                ...
                toast.error(errorDetail(error) ?? `Could not remove ${server?.name ?? 'this server'}`)
            } finally {
        ...
        performConnection: async ({ serverId, authType, apiKey, clientId, clientSecret }) => {
            ...
            try {
                ...
                actions.loadServers()
                toast.error(errorDetail(error) ?? `Could not connect to ${server.name}`)
            } finally {
                actions.performConnectionComplete()
            }
        },
        disconnectServer: async ({ serverId, installationId, navigateToServers }) => {
            ...
            try {
                ...
                toast.error(errorDetail(error) ?? `Could not disconnect from ${server?.name ?? 'this server'}`)
            } finally {
        ...
        toggleYourConnectionEnabled: async ({ installationId, enabled }) => {
            try {
                ...
                toast.error(errorDetail(error) ?? 'Could not update this connection')
            } finally {
        ...
        toggleRuleEnabled: async ({ ruleId, enabled }) => {
            ...
            try {
                ...
                toast[enabled ? 'success' : 'info'](`${rule?.name ?? 'Rule'} ${enabled ? 'enabled' : 'disabled'}`)
            } catch (error: unknown) {
                toast.error(errorDetail(error) ?? `Could not update ${rule?.name ?? 'this rule'}`)
            } finally {
        ...
        setAllowCustomServers: async ({ allowed }) => {
            try {
                ...
                actions.loadConfigSuccess(config)
                toast[allowed ? 'success' : 'info'](
                    allowed ? 'Members can add custom servers' : 'Custom servers are now admin-only'
                )
            } catch (error: unknown) {
                toast.error(errorDetail(error) ?? 'Could not update custom server access')
            } finally {
        ...
        setAllowMemberAgentAccess: async ({ allowed }) => {
            try {
                ...
                toast.error(errorDetail(error) ?? 'Could not update member-managed agent access')
            } finally {
        ...
        applyPreset: async ({ audience, preset }) => {
            ...
            try {
                ...
                actions.loadConfigSuccess(config)
                toast.success(`Baseline applied to ${audience}`)
            } catch (error: unknown) {
                toast.error(errorDetail(error) ?? `Could not apply the baseline to ${audience}`)
            } finally {
        ...
        setMemberServerAccess: async ({ userId, serverId, enabled }) => {
            ...
            try {
                ...
                toast.error(errorDetail(error) ?? `Could not update access to ${server?.name ?? 'this server'}`)
            } finally {
