...
from insights.models import OAuthApplication, Team
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Run `insightscli start` to launch the dev environment.")
    ...
    def _check_database(self):
        ...
        try:
            ...
            raise CommandError(f"Cannot connect to the database. Is the dev environment running? (insightscli start)\n{e}")
    ...
    def _build_skills(self):
        ...
        process = subprocess.Popen(
            [sys.executable, "products/insights_ai/scripts/build_skills.py"],
...
def _append_env_vars(env_path: Path, values: dict[str, str]) -> None:
    ...
    Quotes values so they survive `set -o allexport; source .env` in insightscli start.
    """
