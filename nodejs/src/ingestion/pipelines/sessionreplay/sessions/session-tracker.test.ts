...
describe('SessionTracker', () => {
    ...
    describe('hasSeen', () => {
        it('reports new and already-seen sessions from a single MGET', async () => {
            ...
            expect(mockRedisClient.mget).toHaveBeenCalledWith([
                '@hanzo/replay/session-seen:1:new',
                '@hanzo/replay/session-seen:1:old',
            ])
        })
    ...
    describe('markSeen', () => {
        it('sets each session key with a 48h TTL in one pipeline', async () => {
            ...
            expect(mockPipeline.set).toHaveBeenCalledWith('@hanzo/replay/session-seen:1:a', '1', 'EX', TTL_SECONDS)
            expect(mockPipeline.set).toHaveBeenCalledWith('@hanzo/replay/session-seen:2:b', '1', 'EX', TTL_SECONDS)
        ...
        it('namespaces the Redis keys when a key namespace is given, isolating it from the main lane', async () => {
            ...
            expect(mockPipeline.set).toHaveBeenCalledWith(
                '@hanzo/replay/ml-mirror/session-seen:1:a',
                '1',
                'EX',
                TTL_SECONDS
            )
