...
from products.review.backend.reviewer.persistence import load_findings_bundle, load_valid_findings
...
# or the built-in default when the author has no linked Insights user. The default variant is
...
_NO_ISSUES_MEDIA = (
    (
        "https://raw.githubusercontent.com/Insights/pr-assets/"
...
def report_deep_link(team_id: int, report_id: str) -> str:
    """The app URL opening this report's review drawer — the held-back "View them in Insights" target.
...
def render_final_body(
    ...
    *,
    ...
    and links to the report in Insights (`report_url`, auth-gated) — the PR is otherwise the only
