...
`insights/temporal/data_modeling/workflows/{materialize_view,execute_dag}.py`.
...
from insights.temporal.data_modeling.activities.fail_materialization import (
    CONSECUTIVE_TIMEOUTS_TO_PAUSE,
    should_pause_schedule_for_timeout,
)
from insights.temporal.data_modeling.activities.utils import strip_hostname_from_error
from insights.temporal.data_modeling.metrics import get_data_modeling_finished_metric
from insights.temporal.ducklake.ducklake_copy_data_modeling_workflow import DuckLakeCopyDataModelingWorkflow
...
async def materialize_model(
    ...
    insightsql_query_value = saved_query.query.get("query")
    if not isinstance(insightsql_query_value, str):
        raise ValueError(f"Saved query {saved_query.id} has an invalid query payload")

    insightsql_query = insightsql_query_value
...
async def build_dag_from_selectors(selector_paths: SelectorPaths, team_id: int) -> DAG:
    ...
    """
    insights_table_names = await get_insights_table_names(team_id)
    dag = {}
        ...
        for path in paths:
            ...
            for index, label in enumerate(path):
                ...
                if (
                    ...
                    and label not in insights_table_names
...
async def get_insights_table_names(team_id: int) -> list[str]:
    team = await database_sync_to_async(Team.objects.get)(id=team_id)
