...
export function ObservationStatusTag({
    ...
    if (status === 'succeeded') {
        return <Tag type="success">Succeeded</Tag>
    }
    if (status === 'failed') {
    ...
    return <Tag type="default">Pending</Tag>
}
        ...
        return (
            <div className="flex flex-col gap-1">
                <Tooltip title={promptTooltip}>
                    <Tag size="medium" type={tagType} className="self-start">
                        {tagLabel}
                    </Tag>
                </Tooltip>
    ...
    if (scannerType === 'classifier') {
        ...
        const renderFreeform = (): JSX.Element[] =>
            freeformTags.map((tag, index) => (
                <Tag key={`freeform-${index}-${tag}`} size="medium" type="default" icon={<IconSparkles />}>
                    {tag}
                </Tag>
            ))
        if (compact) {
                        ...
                        {empty ? (
                            ...
                            <>
                                {fixedTags.map((tag, index) => (
                                    <Tag key={`fixed-${index}-${tag}`} size="medium" type="option">
                                        {tag}
                                    </Tag>
                                ))}
                                {renderFreeform()}
            ...
            {keywords.length > 0 && (
                <div className="flex flex-wrap gap-1">
                    {keywords.map((keyword) => (
                        <Tag key={keyword} type="option" size="small">
                            {keyword}
                        </Tag>
                    ))}
                </div>
