...
const PROBLEM_TYPE_TAG: Record<SessionProblemSignalExtraApi['problem_type'], { label: string; type: TagType }> = {
...
export function SessionReplaySignalCard({ signal }: SignalCardProps): JSX.Element {
    ...
    return (
        <SignalCardShell
            ...
            rightSlot={
                problemTag ? (
                    <Tag type={problemTag.type} size="small" className="shrink-0">
                        {problemTag.label}
                    </Tag>
                ) : undefined
            }
