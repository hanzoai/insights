...
class Command(BaseCommand):
    help = "Sync CustomFunction templates from in-memory and node.js to database"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Starting CustomFunction template sync...")

        all_templates: list[dict] = []
        current_template_ids = set()

        # Process templates from CUSTOM_FUNCTION_TEMPLATES (Python templates)
        for template_dc in CUSTOM_FUNCTION_TEMPLATES:
        ...
        try:
            existing_templates = CustomFunctionTemplate.objects.values_list("template_id", flat=True).distinct()
            ...
            if candidates_for_deletion:
                templates_to_delete = CustomFunctionTemplate.objects.filter(template_id__in=candidates_for_deletion)
