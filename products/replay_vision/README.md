...
A sub-product of Session Replay. Users configure named **scanners** that Insights applies to completed session recordings; results land as queryable `$recording_observed` events that feed insights, dashboards, and Insights Signals.
...
- `backend/queries/` — Datastore candidate selection (watermark + settle window + eligibility + sampling) and volume estimates.
