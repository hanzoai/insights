...
@pytest.mark.parametrize(
    "bad_url",
    [
        pytest.param("https://gateway.us.hanzo.ai/v1?x=y", id="query"),
        pytest.param("https://gateway.us.hanzo.ai/v1#frag", id="fragment"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("https://gateway.us.hanzo.ai/v1", "https://gateway.us.hanzo.ai", id="strips-v1"),
        pytest.param(
            "https://gateway.us.hanzo.ai/v1/", "https://gateway.us.hanzo.ai", id="strips-v1-trailing-slash"
        ),
    ],
)
...
def test_extra_properties_degrade_to_empty(monkeypatch, raw):
    ...
    if raw is None:
        monkeypatch.delenv("STAMPFN_EXTRA_PROPERTIES", raising=False)
    else:
        monkeypatch.setenv("STAMPFN_EXTRA_PROPERTIES", raw)
