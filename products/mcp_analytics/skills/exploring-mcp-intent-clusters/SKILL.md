...
description: >
  Explore Insights MCP intent clusters — agent goals grouped by semantic
...
```json
insights:mcp-analytics-intent-clusters-retrieve
{}
```
...
```json
insights:mcp-analytics-intent-clusters-recompute
{}
```

Returns immediately with `status: computing` (HTTP 202); the work runs in the
background. Poll `insights:mcp-analytics-intent-clusters-retrieve` until `status`
returns to `idle` (done) or `error`. Don't block waiting — tell the user to
re-ask in a minute.

## Constructing UI links

- **Intent clustering**: `https://app.hanzo.ai/project/<project_id>/mcp-analytics/intent-clustering`
