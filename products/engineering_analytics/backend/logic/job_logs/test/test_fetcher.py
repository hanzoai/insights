...
_URL = "https://api.github.com/repos/Insights/insights/actions/jobs/123/logs"
...
def test_returns_log_text_on_success(requests_mock):
    ...
    assert fetch_job_log("Insights/insights", 123, "tok") == "2026-06-25T09:14:02.0Z line one"
...
def test_returns_none_when_log_expired(requests_mock):
    ...
    assert fetch_job_log("Insights/insights", 123, "tok") is None
...
def test_raises_on_rate_limit(requests_mock):
    ...
    with pytest.raises(GitHubRateLimitError):
        fetch_job_log("Insights/insights", 123, "tok")
...
def test_caps_log_but_keeps_failure_tail(requests_mock):
    ...
    result = fetch_job_log("Insights/insights", 123, "tok", max_bytes=400)
...
@pytest.mark.parametrize(
    "bad_repo",
    [
        "Insights/insights/contents/secret?ref=main",  # extra path + query steers to another endpoint
        "../../other/repo",  # traversal
        "Insights/insights#frag",
        "owner",  # no slash
