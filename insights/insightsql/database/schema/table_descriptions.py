...
A materialized view is a special case: when queried in materialized mode, InsightsQL swaps the view for
...
from insights.insightsql.database.models import FieldOrTable, SavedQuery, Table
...
class TableDescriptions:
    ...
    def load(cls, team_id: Optional[int]) -> "TableDescriptions":
        ...
        # Inline imports: keep the products dependency off the insightsql import path (products import
        # insightsql, so a module-level import would cycle) and off every code path that never resolves
        # descriptions.
