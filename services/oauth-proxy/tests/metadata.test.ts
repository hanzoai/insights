...
const AUTHORITATIVE_METADATA = {
    issuer: 'https://us.hanzo.ai',
    authorization_endpoint: 'https://us.hanzo.ai/oauth/authorize/',
    token_endpoint: 'https://us.hanzo.ai/oauth/token/',
    revocation_endpoint: 'https://us.hanzo.ai/oauth/revoke/',
    introspection_endpoint: 'https://us.hanzo.ai/oauth/introspect/',
    userinfo_endpoint: 'https://us.hanzo.ai/oauth/userinfo/',
    jwks_uri: 'https://us.hanzo.ai/.well-known/jwks.json',
    registration_endpoint: 'https://us.hanzo.ai/oauth/register/',
    ...
    service_documentation: 'https://hanzo.ai/docs/model-context-protocol',
...
describe('handleMetadata', () => {
    ...
    it('returns valid OAuth authorization server metadata with endpoints rewritten', async () => {
        ...
        const request = new Request('https://oauth.hanzo.ai/.well-known/oauth-authorization-server')
        const response = await handleMetadata(request)
        const data = (await response.json()) as Record<string, unknown>

        expect(response.status).toBe(200)
        expect(data.issuer).toBe('https://oauth.hanzo.ai')
        expect(data.authorization_endpoint).toBe('https://oauth.hanzo.ai/oauth/authorize/')
        expect(data.token_endpoint).toBe('https://oauth.hanzo.ai/oauth/token/')
        expect(data.revocation_endpoint).toBe('https://oauth.hanzo.ai/oauth/revoke/')
        expect(data.introspection_endpoint).toBe('https://oauth.hanzo.ai/oauth/introspect/')
        expect(data.userinfo_endpoint).toBe('https://oauth.hanzo.ai/oauth/userinfo/')
        expect(data.jwks_uri).toBe('https://oauth.hanzo.ai/.well-known/jwks.json')
        expect(data.registration_endpoint).toBe('https://oauth.hanzo.ai/oauth/register/')
    })
