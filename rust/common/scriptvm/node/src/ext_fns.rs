...
//! `nodejs/src/cdp/script-transformations/transformation-functions.ts` (plus `print`, which the Node
//! VM surfaces as logs we don't compare).

use std::collections::HashMap;
use std::sync::OnceLock;

use scriptvm::{construct_free_standing, native_func, HogLiteral, NativeFunction, VmError};
...
// (nodejs/src/cdp/script-transformations/rust-vm.ts `isUnsupportedByRustVm`): it makes the executor
...
pub fn transformation_ext_fns() -> HashMap<String, NativeFunction> {
    ...
    EXT_FNS
        .get_or_init(|| {
            ...
            fns.insert(
                "insightsCapture".to_string(),
                native_func(|_vm, _args| {
                    Err(VmError::NativeCallFailed(
                        "insightsCapture is not supported in transformations".to_string(),
                    ))
                }),
            );
