...
import { InsightsQLQueryModifiers, NodeKind } from '~/queries/schema/schema-general'
...
function ConnectionIdModifier<Q extends QueryWithModifiers>({
    ...
    const isInsightsQLQuery = query.kind === NodeKind.InsightsQLQuery

    useEffect(() => {
        if (isInsightsQLQuery && !dataWarehouseSources) {
            loadSources()
        }
    }, [dataWarehouseSources, isInsightsQLQuery, loadSources])
