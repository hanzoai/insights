...
from products.insights_ai.eval_harness.acp_log import parse_log
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"claudeCode": {"toolName": "mcp__insights__execute_sql"}},
            "execute sql",
            "mcp__insights__execute_sql",
            id="claude_runtime_meta",
        ),
        pytest.param(
            {"insights": {"toolName": "mcp__insights__exec", "mcp": {"server": "insights", "tool": "exec"}}},
            "insights/exec",
            "mcp__insights__exec",
            id="codex_runtime_meta",
        ),
        pytest.param(None, "/bin/bash -lc ls", "/bin/bash -lc ls", id="null_meta_falls_back_to_title"),
        pytest.param({"insights": {}}, "insights/exec", "insights/exec", id="empty_adapter_meta_falls_back_to_title"),
    ],
)
...
def test_parse_log_extracts_tool_result_from_content_list(
    ...
    lines = [_tool_call_line(None, "insights/exec"), _tool_call_update_line(status, extra)]
    parsed = parse_log("\n".join(lines))
    tool_results = [
        block
        for message in parsed.messages
    ...
    [
        pytest.param(
            [
                ...
                _tool_call_line({"insights": {"toolName": "mcp__insights__exec"}}, "insights/exec"),
                _tool_call_update_line("completed", {"content": [_text_block("42 rows")]}),
                _line(
                    {
                        "jsonrpc": "2.0",
                        "method": "_insights/usage_update",
                        "params": {
                            "sessionId": "s1",
                            "usage": {
                                "inputTokens": 100,
