...
from insights.admin.inline_registry import register_admin_inline
...
class ProductPushCampaignAdmin(admin.ModelAdmin):
    ...
    # FK to insights.Organization — without this the add view renders a <select>
    ...
    def organization_link(self, campaign: ProductPushCampaign) -> SafeString:
        url = reverse("admin:insights_organization_change", args=[campaign.organization_id])
...
# product. OrganizationAdmin pulls it in via get_inlines() — see insights.admin.inline_registry.
...
class EnrichmentLabelResultAdmin(admin.ModelAdmin):
    ...
    # organization_id__exact avoids the join to insights_organization: construct_search casts the
    ...
    def organization_link(self, result: EnrichmentLabelResult) -> SafeString:
        url = reverse("admin:insights_organization_change", args=[result.organization_id])
