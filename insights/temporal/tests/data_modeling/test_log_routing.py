...
from insights.temporal.common.logger import BACKGROUND_LOGGER_TASKS, configure_logger
...
async def test_failure_logs_do_not_expose_datastore_hostnames(
    ...
    await v2_activity_environment.run(
        ...
        FailMaterializationInputs(
            ...
            dag_id=str(adag.id),
            job_id=str(ajob.id),
            error="Code 241. Memory limit exceeded (from chi-insights-data-0-0.svc.cluster.local:9000)",
            update_node=False,
        ),
    )
