"""Grammar-driven property-based tests for the InsightsQL parser.

Strategies are auto-generated from ``InsightsQLParser.g4`` /
``InsightsQLLexer.common.g4`` by
``insights/insightsql/scripts/build_grammar_strategies.py`` and imported
...
``insightsql_parser_rs`` wheel) and the C++ parser (the reference oracle, a
hand-ported ANTLR4 visitor exposed via the ``insightsql_parser`` wheel). AST
...
from insights.insightsql.parser import parse_expr, parse_select, parse_string_template
from insights.insightsql.scripts._diagnostic_common import ast_depth, ast_kpaths
from insights.insightsql.test._generated_grammar_strategies import (
    expr_strategy,
    fullTemplateString_strategy as full_template_string_strategy,
    select_strategy,
)
from insights.insightsql.test._grammar_token_strategies import _RESERVED_KEYWORDS
...
# malformed-literal shapes. Mirrors the hostile-token seeding in Datastore's
...
def _bracket_pairs(tokens: list[str]) -> list[tuple[int, int]]:
    ...
    never occur inside InsightsQL string / identifier literals (their alphabets
...
# exposed through the `insightsql_parser` wheel).
_BACKEND_A = "rust-py"
_BACKEND_B = "cpp-json"


def _try_parse(query: str, rule: str, backend: str) -> tuple[bool, ast.AST | None]:
    """Return ``(accepted, ast_or_none)``. Only ``BaseInsightsQLError`` counts
