...
- Investigating an active incident — query the issue / its events directly via
  `insights:query-error-tracking-issue` and `insights:query-error-tracking-issue-events` instead of
...
Call `insights:error-tracking-alerts-list`. Filter the response client-side by `filters.events[].id`.
...
Insights's "alerts configured" recommendation only inspects `filters.events` — adding per-issue
...
1. `insights:integrations-list` with `kind=slack` → pick the integration `id` (an integer).
2. `insights:integrations-channels-retrieve` with that id → pick the channel id (e.g. `C0123ABC`). Channel
...
For Linear / GitHub / GitLab: confirm the integration is connected via `insights:integrations-list` first,
then ask the user which project / repository / team to file issues into.

### 4. Create the alert

Call `insights:error-tracking-alerts-create` with:
...
- One `insights:error-tracking-alerts-list` call up front, not per candidate.
