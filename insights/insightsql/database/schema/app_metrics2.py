...
APP_METRICS2_FIELDS: dict[str, FieldOrTable] = {
    ...
    "app_source": StringDatabaseField(
        name="app_source",
        nullable=False,
        description="Kind of app that produced the metric, e.g. 'insights_function' or 'plugin'.",
    ),
...
class AppMetrics2Table(Table):
    description: str = (
        "Aggregated success/failure counts emitted by apps such as script functions and plugins, bucketed by time."
    )
