...
from insights.errors import CH_TRANSIENT_ERRORS, CHQueryErrorTooManyBytes
from insights.exceptions import DatastoreAtCapacity
from insights.tasks.tasks import sync_feature_flag_last_called
...
class TestSyncFeatureFlagLastCalled(BaseTest):
    ...
    def test_skips_flag_keys_with_nul_bytes(self, mock_get_client: MagicMock, mock_sync_execute: MagicMock) -> None:
        """A Datastore-sourced flag key containing a NUL byte must not reach the Postgres query,
...
class TestSyncFeatureFlagLastCalledChunking(BaseTest):
    ...
    def test_chunking_splits_large_window(self, mock_get_client: MagicMock, mock_sync_execute: MagicMock) -> None:
        """A 20-minute window with 5-minute chunks should produce 4 Datastore queries"""
        redis_mock = mock_redis_client()
        checkpoint_key = "insights:feature_flag_last_called_sync:last_timestamp"
    ...
    def test_checkpoint_not_updated_on_failure(self, mock_get_client: MagicMock, mock_sync_execute: MagicMock) -> None:
        ...
        mock_sync_execute.side_effect = [[], Exception("Datastore error")]
    ...
    def test_autoretry_for_membership(self) -> None:
        ...
        # sync_execute wraps capacity errors (code 202) into DatastoreAtCapacity,
        # so the wrapped form must be retryable too
        assert DatastoreAtCapacity in autoretry_for
