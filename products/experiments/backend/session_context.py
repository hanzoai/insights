...
- `$feature/<key>` properties stamped on every captured event by insights-js. These cover the
...
from insights.schema import ExperimentEventExposureConfig, ExperimentExposureCriteria
...
from insights.session_recordings.queries.session_replay_events import SessionReplayEvents, uuidv7_session_lower_bound
from insights.utils import get_safe_cache, safe_cache_set
...
# is a backstop far above any real configuration — without it InsightsQL applies an implicit
...
# many days would fan one throttled HTTP request out into dozens of Datastore queries. The
...
def get_session_experiment_context(
    ...
    a recording doesn't re-run the Datastore scans. "Recording not found" is not cached: the
...
def _compute_session_experiment_contexts(
    ...
    *,
    ...
    # Every scan below goes through InsightsQL, and constructing the virtual database dominates this
    # endpoint's wall time on teams with a large warehouse schema — several seconds per query,
    # paid once per query, while Datastore itself answers in well under a second. Build the
    ...
    # SharedInsightsQLDatabase documents the two query shapes that mutate a database at query time,
    # and test_uncached_request_shares_one_readonly_insightsql_database pins that these scans don't.
    ...
    insightsql_modifiers = create_default_modifiers_for_team(team)
    shared_insightsql = SharedInsightsQLDatabase(
        database=Database.create_for(
            team=team,
            user=user,
            modifiers=insightsql_modifiers,
    ...
    for chunk, candidate_ids in zip(chunks, candidate_ids_by_chunk):
        ...
        if not candidates:
            ...
            continue
        try:
            chunk_results, chunk_capped = _compute_chunk_contexts(team, user, shared_insightsql, resolved, chunk, candidates)
...
def _compute_chunk_contexts(
    ...
    if settings.TEST:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            # current context into each worker so tagged Datastore queries don't fail untagged.
    ...
    if rescued_keys:
        ...
        rescued_stamped = _query_stamped_flag_properties(
            team, user, shared_insightsql, session_ids, rescued_keys, window_start, window_end
        )
    ...
    try:
        ...
        if all_sources:
            scan = scan_sessions_for_metric_events(
                ...
                shared_insightsql=shared_insightsql,
            )
...
def _query_flag_evaluations(
    ...
    response = execute_insightsql_query(
        query, team=team, user=user, context=shared_insightsql.fresh_context(team, user), modifiers=shared_insightsql.modifiers
    )
...
def _query_exposure_event_branches(
    ...
    for experiment_id, resolution, variants in branch_meta:
        ...
        try:
        ...
        except (Cohort.DoesNotExist, BaseInsightsQLError):
            ...
            # property filter InsightsQL can't compile — must not fail the whole surface. Match
        ...
        # The backstop must sit on each branch: InsightsQL stamps an implicit LIMIT 100 on every
    ...
    # LIMIT, which Datastore rejects as a syntax error. The per-branch limits above are
