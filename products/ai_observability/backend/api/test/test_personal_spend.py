...
class TestPersonalSpendEuRedirect(APIBaseTest):
    ...
    def test_eu_redirect_view_preserves_query_string(self) -> None:
        ...
        assert response["Location"].startswith("https://us.hanzo.ai/api/llm_analytics/@me/spend/")
        assert "date_from=-7d" in response["Location"]
        assert "product=insights_code" in response["Location"]
    ...
    def test_eu_redirect_view_strips_unknown_params(self) -> None:
        ...
        assert response["Location"].startswith("https://us.hanzo.ai/")
...
class TestPersonalSpendValidation(APIBaseTest):
    ...
    def test_date_params_default_to_last_30_days(self) -> None:
        ...
        assert summary["product"] == "insights_code"
    ...
    @parameterized.expand(
        [
            ("supported_insights_code", "insights_code", status.HTTP_200_OK),
...
class TestPersonalSpendQueries(DatastoreTestMixin, APIBaseTest):
    """End-to-end tests against Datastore with real $ai_generation events."""
    ...
    def test_summary_reports_cross_product_totals_alongside_scoped(self) -> None:
        self._create_generation(ai_product="insights_code", cost=2.0)
        self._create_generation(ai_product="background_agents", cost=1.0)
        self._create_generation(ai_product="insights_code", cost=0.5, event_name="$ai_embedding")
        flush_persons_and_events()
        ...
        # `scoped_*` is filtered to insights_code only.
        ...
        code_row = _by_product(body["by_product"]["items"], "insights_code")
    ...
    def test_product_filter_scopes_summary_and_breakdowns(self) -> None:
        self._create_generation(ai_product="insights_code", tool="Bash", cost=2.0)
        self._create_generation(ai_product="background_agents", tool="Bash", cost=10.0)
        flush_persons_and_events()

        response = self.client.get(f"{ENDPOINT}?product=insights_code")
        body = response.json()

        # Summary still reports cross-product totals; scoped_* is filtered.
        assert body["summary"]["product"] == "insights_code"
        assert body["summary"]["total_cost_usd"] == 12.0
        assert body["summary"]["scoped_cost_usd"] == 2.0

        # by_tool is scoped to the filter — only the insights_code Bash row.
        ...
        # by_product is always cross-product, regardless of the filter.
        assert {r["product"] for r in body["by_product"]["items"]} == {"insights_code", "background_agents"}
    ...
    def test_date_window_passes_through_to_query_layer(self) -> None:
        with patch("products.ai_observability.backend.api.personal_spend.execute_insightsql_query") as mock_exec:
...
class TestPersonalSpendInternalEndpoint(DatastoreTestMixin, APIBaseTest):
    ...
    def setUp(self) -> None:
        ...
        self.client.logout()
        self.payload = {"email": "someone@example.com", "product": "insights_code"}
    ...
    def test_missing_email_rejected(self) -> None:
        body = _json_body({"product": "insights_code"})
    ...
    def test_signed_request_other_email_sees_nothing(self) -> None:
        ...
        _create_event(
            ...
            team=self.team,
            distinct_id="eu-user",
            properties={"$ai_total_cost_usd": 2.5, "ai_product": "insights_code"},
        )
        flush_persons_and_events()

        body = _json_body({"email": "other@example.com", "product": "insights_code"})
        response = self._post(body, _signed_headers(body))
        assert response.status_code == status.HTTP_200_OK
...
class TestPersonalSpendEUProxy(APIBaseTest):
    ...
    def test_over_cap_bucket_window_rejected_without_upstream_call(self) -> None:
        ...
        with override_settings(PERSONAL_SPEND_CROSS_REGION_SECRET=CROSS_REGION_SECRET):
            with patch("products.ai_observability.backend.api.personal_spend.requests.post") as post:
                response = self._get(
                    {"product": "insights_code", "date_from": "-30d", "bucket_minutes": "60"}, user=self.user
                )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        post.assert_not_called()
    ...
    def test_relays_upstream_success_and_signs_asserted_email(self) -> None:
        ...
        assert sent["product"] == "insights_code"
