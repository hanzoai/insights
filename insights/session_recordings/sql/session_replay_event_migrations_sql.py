...
from insights.session_recordings.sql.session_replay_event_sql import SESSION_REPLAY_EVENTS_DATA_TABLE
...
ADD_CONSOLE_COUNTS_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_CONSOLE_COLUMNS.format(
...
ADD_EVENT_COUNT_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_EVENT_COUNT_COLUMN.format(
        table_name="session_replay_events",
        cluster=settings.DATASTORE_CLUSTER,
    )
)
...
ADD_RETENTION_PERIOD_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_COLUMN.format(
        table_name="session_replay_events",
    )
)

ADD_RETENTION_PERIOD_WRITABLE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_COLUMN.format(
        table_name="writable_session_replay_events",
    )
)
...
ADD_RETENTION_PERIOD_DAYS_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_DAYS_COLUMN.format(
        table_name="session_replay_events",
    )
)

ADD_RETENTION_PERIOD_DAYS_WRITABLE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_DAYS_COLUMN.format(
        table_name="writable_session_replay_events",
    )
)

ADD_RETENTION_PERIOD_DAYS_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_DAYS_COLUMN.format(
        table_name=SESSION_REPLAY_EVENTS_DATA_TABLE(),
    )
)
...
DROP_RETENTION_PERIOD_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_DROP_RETENTION_PERIOD_COLUMN.format(
        table_name="session_replay_events",
    )
)

DROP_RETENTION_PERIOD_WRITABLE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_DROP_RETENTION_PERIOD_COLUMN.format(
        table_name="writable_session_replay_events",
    )
)

DROP_RETENTION_PERIOD_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_DROP_RETENTION_PERIOD_COLUMN.format(
        table_name=SESSION_REPLAY_EVENTS_DATA_TABLE(),
    )
)
...
DROP_RETENTION_PERIOD_DAYS_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_DROP_RETENTION_PERIOD_DAYS.format(
        table_name="session_replay_events",
    )
)

DROP_RETENTION_PERIOD_DAYS_WRITABLE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_DROP_RETENTION_PERIOD_DAYS.format(
        table_name="writable_session_replay_events",
    )
)

DROP_RETENTION_PERIOD_DAYS_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_DROP_RETENTION_PERIOD_DAYS.format(
        table_name=SESSION_REPLAY_EVENTS_DATA_TABLE(),
    )
)

ADD_RETENTION_PERIOD_DAYS_AGGREGATE_TYPE_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_DAYS_AGGREGATE_TYPE.format(
        table_name="session_replay_events",
    )
)

ADD_RETENTION_PERIOD_DAYS_AGGREGATE_TYPE_WRITABLE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_DAYS_AGGREGATE_TYPE.format(
        table_name="writable_session_replay_events",
    )
)

ADD_RETENTION_PERIOD_DAYS_AGGREGATE_TYPE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_RETENTION_PERIOD_DAYS_AGGREGATE_TYPE.format(
        table_name=SESSION_REPLAY_EVENTS_DATA_TABLE(),
    )
)
...
ADD_SURFACING_SCORE_WRITABLE_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_SURFACING_SCORE_COLUMN.format(
        table_name="writable_session_replay_events",
    )
)

ADD_SURFACING_SCORE_DISTRIBUTED_SESSION_REPLAY_EVENTS_TABLE_SQL = lambda: (
    ALTER_SESSION_REPLAY_ADD_SURFACING_SCORE_COLUMN.format(
        table_name="session_replay_events",
    )
)
