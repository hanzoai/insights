...
PYTHON_ONLY_ALLOWLIST: dict[str, str] = {
    "between": "Range comparison, only used in InsightsQL insights queries",
    "not_between": "Range comparison, only used in InsightsQL insights queries",
    "min": "Alias for gte, only used in InsightsQL insights queries",
    "max": "Alias for lte, only used in InsightsQL insights queries",
    "is_cleaned_path_exact": "Path normalization, only used in InsightsQL path analysis",
}
...
def main() -> int:
    ...
    if has_errors:
        ...
        print(f"     - Python/TS: frontend/src/types.ts (then run: insightscli build:schema)")
