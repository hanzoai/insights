...
from insights.models.event.util import get_event_count, get_event_count_for_last_month, get_event_count_month_to_date
from insights.session_recordings.models.system_status_queries import get_recording_status_month_to_date

SLOW_THRESHOLD_MS = 10000
SLOW_AFTER = relativedelta(hours=6)

DATASTORE_FLAMEGRAPH_EXECUTABLE = abspath(join(dirname(__file__), "bin", "datastore-flamegraph"))
...
def system_status() -> Generator[SystemStatusRow, None, None]:
    ...
    yield {
        "key": "datastore_alive",
        "metric": "Datastore database alive",
        "value": alive,
    }
    ...
    yield {
        "key": "datastore_event_count",
        "metric": "Events in Datastore",
        "value": get_event_count(),
    }
    yield {
        "key": "datastore_event_count_last_month",
        "metric": "Events recorded last month",
        "value": get_event_count_for_last_month(),
    }
    yield {
        "key": "datastore_event_count_month_to_date",
    ...
    for index, (total_space, free_space) in enumerate(disk_status):
        metric = "Datastore disk" if len(disk_status) == 1 else f"Datastore disk {index}"
        yield {
            "key": f"datastore_disk_{index}_free_space",
            "metric": f"{metric} free space",
            "value": free_space,
        }
        yield {
            "key": f"datastore_disk_{index}_total_space",
    ...
    yield {
        "key": "datastore_table_sizes",
        "metric": "Datastore table sizes",
        "value": "",
    ...
    yield {
        "key": "datastore_system_metrics",
        "metric": "Datastore system metrics",
        "value": "",
        "subrows": {
...
def get_datastore_running_queries() -> list[dict]:
...
def get_datastore_slow_log() -> list[dict]:
    return query_with_columns(
        f"""
