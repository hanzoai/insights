...
// Single-column scalar aggregations whose metric-type name is also the InsightsQL
...
// Strip a trailing `AS alias` from a InsightsQL fragment — column entries in the
...
export function isNumericColumnType(type: string | undefined): boolean {
    ...
    // Regular columns carry a InsightsQL field type; custom-property columns carry a display type.
...
// from the editor's numeric input, so it is safe to inline into the InsightsQL.
...
export function tileMetricExpression(tile: AccountsOverviewTile): string {
    ...
    switch (metric.type) {
        ...
        default:
            // sum | avg | min | max | median: the metric type is the InsightsQL aggregate fn name.
...
export const accountsOverviewTilesLogic = kea<accountsOverviewTilesLogicType>([
    ...
    listeners(({ actions, values, cache }) => ({
        ...
        hideEditor: () => {
            ...
            if (diff.changed) {
                insights.capture(AccountsEvents.OverviewTilesEdited, {
    ...
    afterMount(({ actions }) => {
        ...
        if (legacyTiles) {
            ...
            insights.capture(AccountsEvents.OverviewTilesLocalStorageRead, { tile_count: legacyTiles.length })
        }
    }),
])
