...
def create_mock_query_executor():
    ...
    async def mock_arun_and_format_query(query_obj):
        ...
        elif isinstance(query_obj, InsightsQLQuery | AssistantInsightsQLQuery):
            return "Mocked InsightsQL query results: Result count = 42", {}
        else:
...
class TestInsightSearchNode(BaseTest):
    ...
    async def test_returns_artifact_with_insightsql_query(self):
        """Test that ArtifactMessage content contains AssistantInsightsQLQuery."""
        query = {"source": {"kind": "InsightsQLQuery", "query": "SELECT 1"}}
        insight = await Insight.objects.acreate(
            team=self.team,
            name="InsightsQL Query",
            description="InsightsQL Query",
            query=query,
            filters={},
            created_by=self.user,
            saved=True,
        ...
        query_obj, _ = await self.node._process_insight_query(insight_dict)
        self.assertIsNotNone(query_obj, f"Query object should not be None for insight. Query: {insight_dict['query']}")
        self.assertIsInstance(query_obj, AssistantInsightsQLQuery)
        ...
        self.assertIsInstance(viz_message.content.query, AssistantInsightsQLQuery)
