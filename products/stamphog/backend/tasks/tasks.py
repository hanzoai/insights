...
def _self_driving_pr_author_login() -> str | None:
    ...
    team's Insights GitHub App and authored by its machine user, ``<slug>[bot]``. Returns ``None``
...
def _is_self_driving_pr(pr: dict[str, Any], repo: str) -> bool:
    ...
    the PR was opened by this instance's Insights GitHub App machine user, and its head is in the
...
def _inbox_rereview_carve_out(
    ...
    empty carve-out and the caller behaves exactly as before, so dependabot, renovate, insights-bot
...
def _is_duplicate_pr_event(delivery_id: str) -> bool:
    return cache.get(f"{STAMPFN_PR_EVENT_IDEMPOTENCY_KEY_PREFIX}{delivery_id}") is not None


def _mark_pr_event_processed(delivery_id: str) -> None:
    cache.set(
        f"{STAMPFN_PR_EVENT_IDEMPOTENCY_KEY_PREFIX}{delivery_id}",
        True,
        timeout=STAMPFN_PR_EVENT_IDEMPOTENCY_TTL_SECONDS,
    )
