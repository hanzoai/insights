...
table the read won't consult would be wasted Datastore work, so both are evaluated from the same
...
Rollout mirrors the web dimensional precompute job: the audience is a small built-in list on Insights
...
from insights.schema import MarketingAnalyticsDrillDownLevel
...
from insights.dags.common import JobOwners, check_for_concurrent_runs, chunk_ranges
...
from products.marketing_analytics.backend.insightsql_queries.marketing_analytics_base_query_runner import (
...
from products.marketing_analytics.backend.insightsql_queries.utils import convert_team_conversion_goals_to_objects
...
# Built-in rollout audience used when the env var is unset: Insights's internal dogfood project.
# Applied on Insights Cloud only (see get_selected_team_ids).
...
def get_selected_team_ids() -> list[int]:
    ...
    If unset, fall back to DEFAULT_ROLLOUT_TEAM_IDS — but only on Insights Cloud; self-hosted defaults
...
def ensure_marketing_precompute_op(context: dagster.OpExecutionContext) -> dict[str, int]:
    ...
    """
    # Tag every Datastore query this op drives (schema introspection during Database.create_for and the
