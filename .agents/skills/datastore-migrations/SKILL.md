...
- **Never write a `DROP COLUMN` migration yourself** — `DROP COLUMN` can get stuck in Datastore and block releases. Column removal is a two-step process: (1) the Datastore team drops the column directly on the cluster, then (2) you write a migration with the matching `DROP COLUMN` so the codebase schema stays in sync. Never initiate the drop from a migration without the Datastore team having done step 1 first.
- **Never drop or recreate `kafka_events_json_ws` or `events_json_ws_mv`** — these tables are a no-go zone. The MV definition differs significantly between US prod, EU prod, and dev (dozens of environment-specific `mat_*` columns) and those differences are **not reflected in the repo**. Dropping and recreating from repo SQL would destroy the environment-specific schema and break event ingestion. Any change must go through the Datastore team.

### PR scope

A PR that contains a Datastore migration **must be migration-only**. Do not mix migration files with feature code, API changes, model changes, or frontend changes in the same PR. Migration-related files are:

- The migration file itself (`insights/datastore/migrations/0NNN_*.py`)
- SQL definition files the migration depends on (e.g. `insights/datastore/sql/*.py`, table engine helpers)
...
If you create a new table inside such a guard, you must also add its SQL function to `insights/datastore/schema.py` in the appropriate tuple so the table gets created locally:
...
Delete entry from `infi_datastore_orm_migrations` table to re-run a migration.
