...
def alter_columns_in_required_tables(_):
    sync_execute(ALTER_COLUMNS_BASE_SQL.format(table="events", cluster=DATASTORE_CLUSTER))
    sync_execute(ALTER_COLUMNS_BASE_SQL.format(table="writable_events", cluster=DATASTORE_CLUSTER))
    sync_execute(ALTER_COLUMNS_BASE_SQL.format(table="sharded_events", cluster=DATASTORE_CLUSTER))
