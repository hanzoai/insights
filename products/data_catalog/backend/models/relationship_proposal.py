...
class RelationshipProposal(TeamScopedRootMixin, CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    Table identity is name-based, mirroring ``DataWarehouseJoin`` itself; keys are InsightsQL expressions
    ...
    source_table_key = models.CharField(max_length=400, help_text="InsightsQL key expression on the source table.")
    joining_table_name = models.CharField(max_length=400, help_text="Name of the table being joined in.")
    joining_table_key = models.CharField(max_length=400, help_text="InsightsQL key expression on the joining table.")
