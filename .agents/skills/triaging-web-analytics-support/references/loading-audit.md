...
1. **Normal pass**: real-user UA, US locale/timezone. Record every request to tracker domains with a timestamp relative to navigation start. After load + ~10s, evaluate in-page state: `window.insights` (`__loaded`, `config.api_host`, `config.token` prefix, `person_profiles`), tag-manager containers and `dataLayer` length, consent platform object and its resolved state, competitor tracker globals and script tags.
...
- **Both major SDKs suppress automation**: insights-js and most competitors detect `navigator.webdriver` and do not send events from headless browsers. Zero capture requests in the audit is expected and says nothing about real users. Assert on script/runtime presence and timing, not on event POSTs.
