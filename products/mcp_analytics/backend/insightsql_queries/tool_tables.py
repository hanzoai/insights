...
from products.mcp_analytics.backend.insightsql_queries.base import (
...
def _tool_call_where(tool: str, date_range: QueryDateRange, *, extra: list[ast.Expr] | None = None) -> ast.Expr:
    ...
    exprs: list[ast.Expr] = [
        ...
        parse_expr("timestamp >= {date_from}", placeholders={"date_from": date_range.date_from_as_insightsql()}),
        parse_expr("timestamp <= {date_to}", placeholders={"date_to": date_range.date_to_as_insightsql()}),
...
# re-capped here in case a non-Insights server emits an unbounded value.
...
class MCPToolNeighborsQueryRunner(AnalyticsQueryRunner[MCPToolNeighborsQueryResponse]):
    ...
    def to_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        ...
        # parse_expr as a variable (not a literal f-string) so insightsql-fstring-audit stays clean.
        neighbor_expr = (
        ...
        cte_where = ast.And(
            exprs=[
                parse_expr("event = {event}", placeholders={"event": ast.Constant(value=MCP_TOOL_CALL_EVENT)}),
                parse_expr(
                    "timestamp >= {date_from}", placeholders={"date_from": self.query_date_range.date_from_as_insightsql()}
                ),
                parse_expr(
                    "timestamp <= {date_to}", placeholders={"date_to": self.query_date_range.date_to_as_insightsql()}
                ),
                parse_expr(
