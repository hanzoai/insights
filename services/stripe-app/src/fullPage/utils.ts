import type { InsightsExperiment, InsightsFeatureFlag, InsightsFlagProperty } from '../insights/types'

export type FlagStatus = 'enabled' | 'disabled' | 'beta'

export type ExperimentStatus = 'running' | 'complete' | 'draft'

export function flagStatusOf(flag: InsightsFeatureFlag): FlagStatus {
    if (!flag.active) {
        return 'disabled'
    }
...
export function experimentStatusOf(experiment: InsightsExperiment): ExperimentStatus {
...
// Render a single PropertyFilter as a human-readable clause, e.g. `email icontains "@hanzo.ai"`.
export function formatProperty(p: InsightsFlagProperty): string {
...
function formatValue(v: InsightsFlagProperty['value']): string {
