...
from insights.insightsql.constants import MAX_BYTES_BEFORE_EXTERNAL_GROUP_BY, InsightsQLGlobalSettings, LimitContext
...
from insights.caching.insights_api import BASE_MINIMUM_INSIGHT_REFRESH_INTERVAL, REDUCED_MINIMUM_INSIGHT_REFRESH_INTERVAL
from insights.datastore.query_tagging import tag_contains_user_insightsql
from insights.constants import INSIGHTSQL, PAGEVIEW_EVENT, SCREEN_EVENT
...
class PathsQueryRunner(AnalyticsQueryRunner[PathsQueryResponse]):
    ...
    def _strip_trailing_slash(url: Optional[str]) -> Optional[str]:
        # Mirrors the `(.)/$` regex applied to event URLs in `construct_event_insightsql`,
    ...
    def construct_event_insightsql(self) -> ast.Expr:
        event_insightsql: ast.Expr = parse_expr("event")

        if self._should_query_event(INSIGHTSQL) and self.query.pathsFilter.pathsInsightsQLExpression:
            tag_contains_user_insightsql()
            event_insightsql = parse_expr(self.query.pathsFilter.pathsInsightsQLExpression)

        if self._should_query_event(PAGEVIEW_EVENT):
            event_insightsql = parse_expr(
                "if(event = {event}, replaceRegexpAll(ifNull(properties.$current_url, ''), '(.)/$', '\\\\1'), {event_insightsql})",
                {"event": ast.Constant(value=PAGEVIEW_EVENT), "event_insightsql": event_insightsql},
            )

        if self._should_query_event(SCREEN_EVENT):
            event_insightsql = parse_expr(
                "if(event = {event}, properties.$screen_name, {event_insightsql})",
                {"event": ast.Constant(value=SCREEN_EVENT), "event_insightsql": event_insightsql},
            )

        return event_insightsql
    ...
    def _funnel_events_join_expr(self, funnelSource: FunnelsQuery) -> ast.Expr:
        ...
        group or a InsightsQL expression (currently ``properties.$session_id``) produce a join
        ...
        # InsightsQL aggregation (currently only properties.$session_id is exposed in the UI,
        # but funnelAggregateByInsightsQL supports arbitrary expressions).
        if funnelsFilter.funnelAggregateByInsightsQL and funnelsFilter.funnelAggregateByInsightsQL != "person_id":
            tag_contains_user_insightsql()
            return parse_expr(funnelsFilter.funnelAggregateByInsightsQL)
    ...
    def paths_events_query(self) -> ast.SelectQuery:
        ...
        event_insightsql = self.construct_event_insightsql()
        event_conditional = parse_expr("ifNull({event_insightsql}, '') AS path_item_ungrouped", {"event_insightsql": event_insightsql})
        ...
        query = ast.SelectQuery(
            ...
            order_by=[
                ...
                # unqualified reference ambiguous to the InsightsQL resolver.
        ...
        return ast.And(
            exprs=[
                ast.CompareOperation(
                    ...
                    right=self.query_date_range.date_from_to_start_of_interval_insightsql(),
                ),
                ast.CompareOperation(
                    op=ast.CompareOperationOp.LtEq,
                    left=field_to_compare,
                    right=self.query_date_range.date_to_as_insightsql(),
                ),
            ]
        )
    ...
    def _calculate(self) -> PathsQueryResponse:
        query = self.to_query()
        # Display-only response InsightsQL (never executed); bypass warehouse ACL so printing doesn't fail closed userless.
        insightsql = to_printed_insightsql(query, self.team, bypass_warehouse_access_control=True)
