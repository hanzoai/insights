...
from ee.hogai.chat_agent.sql.mixins import InsightsQLDatabaseMixin
...
class ReadDataTool(InsightsQLDatabaseMixin, MaxTool):
    ...
    async def _read_data_warehouse_schema(self) -> str:
        ...
        return await self._build_tables_list(database, insightsql_context)

    async def _read_data_warehouse_table_schema(self, table_name: str) -> str:
        database = await self._aget_database()
        insightsql_context = self._get_default_insightsql_context(database)
        return await self._build_table_schema(database, insightsql_context, table_name)

    @database_sync_to_async
    def _build_tables_list(self, database: Database, insightsql_context: InsightsQLContext) -> str:
    ...
    @database_sync_to_async
    def _build_table_schema(self, database: Database, insightsql_context: InsightsQLContext, table_name: str) -> str:
        ...
        serialized = database.serialize(insightsql_context, include_only={table_name})
