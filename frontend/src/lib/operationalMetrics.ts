import type { MetricAttributes } from 'insights-js'
import insights from 'insights-js'

/**
 * Operational metrics into the Insights Metrics product via the SDK's aggregated
 * `insights.metrics` API (samples are batched to one data point per series every 10s).
 *
 ...
 *
 * Both helpers are hard no-ops when the loaded insights-js build doesn't ship the metrics
...
export function metricCount(name: string, value: number = 1, attributes?: MetricAttributes): void {
    try {
        insights.metrics?.count(name, value, attributes ? { attributes } : undefined)
    } catch {
...
export function metricHistogram(name: string, value: number, unit: string, attributes?: MetricAttributes): void {
    try {
        insights.metrics?.histogram(name, value, { unit, ...(attributes ? { attributes } : {}) })
    } catch {
        // never let telemetry break the product
    }
}
