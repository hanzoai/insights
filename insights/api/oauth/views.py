...
from insights.api import id_jag
...
from insights.api.oauth.client_auth import verify_client_secret
from insights.api.oauth.mcp_resource_scopes import build_oauth_mcp_consent_context
from insights.helpers.impersonation import get_original_user_from_session, is_impersonated_session
from insights.middleware import is_read_only_impersonation
from insights.models import OAuthAccessToken, OAuthApplication, Organization, Team, User
...
from insights.utils import absolute_uri, render_template
from insights.views import login_required
...
# Insights apps. Safe to extend because these tokens stay opaque and DB-backed:
...
CLIENT_IDS_WITHOUT_REFRESH_TOKEN: frozenset[str] = frozenset(
    {
        # Insights Wizard CLI (CIMD) — short-lived auth, no persistent session needed.
        "https://us.hanzo.ai/api/oauth/wizard/client-metadata",
        "https://eu.hanzo.ai/api/oauth/wizard/client-metadata",
    }
)
...
def get_region_info() -> dict | None:
    """Return region metadata if running on Insights Cloud US/EU, else None."""
    cloud = getattr(settings, "CLOUD_DEPLOYMENT", None)
    if cloud in ("US", "EU"):
        region = cloud.lower()
        return {"insights_region": region, "insights_base_url": settings.SITE_URL}
    return None
...
class OAuthValidator(OAuth2Validator):
    ...
    def _is_first_party_client(self, request) -> bool:
        """Check if the client is a first-party Insights application."""
        if hasattr(request, "client") and request.client:
    ...
    # Insights deliberately does NOT support OIDC silent authentication (`prompt=none`). Every
    ...
    def validate_scopes(self, client_id, scopes, client, request, *args, **kwargs):
        ...
        `*` is still accepted verbatim under an empty ceiling here (legacy Insights
    ...
    def _get_token_expires_in(self, request) -> int:
        ...
        don't reliably refresh; first-party Insights apps get the same extended TTL.
    ...
    def _get_impersonator_id(self, request, refresh_token: OAuthRefreshToken | None = None):
        ...
        cached = getattr(request, "_insights_impersonator_id", _IMPERSONATOR_CACHE_UNSET)
        ...
        request._insights_impersonator_id = resolved
        return resolved
...
class OAuthTokenView(TokenView):
    ...
    def _handle_jwt_bearer_grant(self, request) -> JsonResponse:
        ...
        `insights.api.id_jag.issue_access_token`."""
        ...
        if response.status_code == 200:
            try:
                ...
                if access_token_value:
                    ...
                    # First-party clients (Insights Desktop) read scoped_teams from /oauth/token
                    ...
                    # TODO(@charlesvien): remove this after a migration period in Insights Desktop.
                    if (
                        not scoped_teams
...
class OAuthProtectedResourceMetadataView(_PublicMetadataView):
    ...
    Insights already points agents at this document via the
    `WWW-Authenticate: Bearer resource_metadata=...` header on 401 responses
    (see insights/exceptions.py). This serves the document it promises, letting
    ...
    def get(self, request, *args, **kwargs):
        ...
        metadata = {
            ...
            "resource": base_url,
            # The same Insights instance is its own authorization server
            "authorization_servers": [base_url],
            "scopes_supported": get_oauth_scopes_supported(),
            "bearer_methods_supported": ["header"],
            "resource_documentation": "https://hanzo.ai/docs/api",
        }
...
class OAuthClientManifestView(_PublicMetadataView):
    ...
    against Insights without a human-driven signup. Served at /auth.md, the
