...
import { Banner, Button, Divider, Input, Tag, Spinner } from '@hanzo/elements'
    ...
    return (
        <SceneContent>
            <SceneTitleSection
                ...
                description="Look up the Insights Task, runs, and Temporal workflows linked to a Slack thread. Restricted to Insights-internal debugging (team 2)."
            ...
            <div className="flex items-end gap-2">
                <Input
                    placeholder="https://insights.slack.com/archives/C…/p1779956938619299"
                    value={url}
                    onChange={setUrl}
                    onPressEnter={() => {
                        if (canSubmit) {
                ...
                {(result || submissionError) && !resultLoading ? (
                    <Button type="secondary" onClick={() => clearResult()}>
                        Clear
                    </Button>
                ) : null}
            </div>
            ...
            {submissionError ? (
                <Banner type="error">
                    {submissionError.status === 403
                        ? 'This endpoint is restricted to Insights-internal debugging (team 2).'
    ...
    return (
        <div className="space-y-4">
            ...
            {task ? (
                ...
                <Banner type="warning">No task is linked to this thread.</Banner>
            )}
    ...
    return (
        <div className="border rounded p-3">
            <div className="flex items-center justify-between mb-2">
                <h4 className="m-0">
                    Run #{index} <Tag size="small">{run.status}</Tag>
                </h4>
    ...
    return (
        <div className="mt-3 pl-3 border-l-2">
            <div className="text-muted font-semibold mb-1">
                Repo research sandbox <Tag size="small">{research.status ?? 'unknown'}</Tag>
            </div>
