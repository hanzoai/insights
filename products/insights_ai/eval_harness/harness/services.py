...
from products.insights_ai.eval_harness.long_lived_subprocess import LongLivedSubprocessManager, SubprocessStartupError
from products.tasks.backend.facade.agents import ENV_LOCAL_SKILLS_HOST_PATH, LocalSkillsCache

from .ports import LLM_GATEWAY_PORT, MCP_PORT, PERSONFN_REPLICA_PORT, PERSONFN_ROUTER_PORT
...
def start_llm_gateway(live_server_url: str) -> Callable[[], None]:
    ...
    db_user = conn.settings_dict.get("USER", "insights")
    db_password = conn.settings_dict.get("PASSWORD", "insights")
...
def _personinsights_target_dir(rust_dir: Path) -> Path:
...
def ensure_personinsights_binaries() -> None:
    """Build the personinsights binaries the harness runs for person/group reads.

    The query API reads persons and groups exclusively through personinsights (there is
    no ORM fallback), so the harness stands up ``personinsights-replica`` +
    ``personinsights-router`` from ``rust/`` against the test persons DB. Building here,
    during preflight, keeps the slow first compile off the async run path.
    """
    if shutil.which("cargo") is None:
        raise PreflightError(
            "`cargo` not found on PATH. The query API reads persons/groups through personinsights, "
    ...
    logger.info(
        "Building personinsights binaries (first build can take several minutes; later builds are incremental no-ops)"
    )
    result = subprocess.run(
        ["cargo", "build", "-p", "personinsights-replica", "-p", "personinsights-router"],
        cwd=rust_dir,
        capture_output=True,
        text=True,
    ...
    if result.returncode != 0:
        ...
        raise PreflightError(f"Failed to build personinsights binaries (cargo exit {result.returncode}):\n{tail}")


def start_personinsights() -> Callable[[], None]:
    """Start personinsights-replica + personinsights-router against the test persons DB.

    Django reads persons/groups only through personinsights (no ORM fallback), so
    ``/query`` scorers read 0 unless the router is up and ``PERSONFN_ADDR`` points
    ...
    if not persons_url:
        raise SubprocessStartupError(
            "PERSONS_DB_WRITER_URL is unset. personinsights reads the seeded persons DB from it; "
            "start_personinsights() must run after EvalDatabase.setup() has provisioned the test persons DB."
        )

    rust_dir = Path(settings.BASE_DIR) / "rust"
    target_dir = _personinsights_target_dir(rust_dir)
    replica_bin = target_dir / "debug" / "personinsights-replica"
    router_bin = target_dir / "debug" / "personinsights-router"
    ...
    logger.info("Starting personinsights-replica on port %d", PERSONFN_REPLICA_PORT)
    _, stop_replica = LONG_LIVED_SUBPROCESSES.start(
        name="personinsights-replica",
        port=PERSONFN_REPLICA_PORT,
        cmd=[str(replica_bin)],
        cwd=rust_dir,
        env={
            **os.environ,
            "PRIMARY_DATABASE_URL": persons_url,
            "GRPC_ADDRESS": f"127.0.0.1:{PERSONFN_REPLICA_PORT}",
            "METRICS_PORT": "0",
            "RUST_LOG": "info",
        },
        log_prefix="personinsights-replica",
    )

    logger.info("Starting personinsights-router on port %d", PERSONFN_ROUTER_PORT)
    try:
        _, stop_router = LONG_LIVED_SUBPROCESSES.start(
            name="personinsights-router",
            port=PERSONFN_ROUTER_PORT,
            cmd=[str(router_bin)],
            cwd=rust_dir,
            env={
                **os.environ,
                "REPLICA_URL": f"http://127.0.0.1:{PERSONFN_REPLICA_PORT}",
                "GRPC_ADDRESS": f"127.0.0.1:{PERSONFN_ROUTER_PORT}",
                "METRICS_PORT": "0",
                "RUST_LOG": "info",
            },
            log_prefix="personinsights-router",
        )
    except Exception:
        stop_replica()
        raise

    logger.info("personinsights ready (router on port %d)", PERSONFN_ROUTER_PORT)
...
def build_local_skills(*, set_bind_mount_env: bool) -> Path:
    ...
    template helpers (e.g. InsightsQL example rendering) guard on that setting. The
