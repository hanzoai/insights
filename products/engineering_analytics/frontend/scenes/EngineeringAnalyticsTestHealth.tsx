        ...
        default:
            return (
                <Tooltip title="Expired beyond the grace period. The quarantine check workflow fails until the entry is removed or re-triaged.">
                    <Tag type="danger">Overdue · blocks CI</Tag>
                </Tooltip>
            )
    }
}
...
function BrokenTestsPanel(): JSX.Element {
    ...
    const columns: TableColumns<BrokenTestRow> = [
        {
            title: 'State',
            key: 'state',
            width: 160,
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {brokenTestsError ? (
                <Banner type="error">Couldn't load broken tests: {brokenTestsError}</Banner>
...
const FLAKY_CLASSIFICATION: Record<FlakyTestClassification, { label: string; type: TagType; tooltip: string }> = {
    confirmed_flake: {
...
function ActiveTestHealthQueue(): JSX.Element {
    ...
    const columns: TableColumns<FlakyTestRow> = [
        {
            title: 'Test',
            key: 'nodeid',
            width: 360,
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {flakyTestsStatus === 'error' ? (
                <Banner type="warning">Couldn't load flaky test data. Try refreshing.</Banner>
            ) : (
                <>
                    <Table
...
function QuarantineRegister(): JSX.Element {
    ...
    if (quarantine && !quarantine.available) {
        return (
            <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-primary p-10 text-center">
                ...
                <Button type="primary" onClick={openNewQuarantine} data-attr="eng-analytics-quarantine-open">
                    Quarantine a test
                </Button>
            </div>
        )
    ...
    const columns: TableColumns<QuarantineEntryRow> = [
        {
            title: 'Selector',
            key: 'id',
            render: (_, row) => {
                    ...
                    <div className="flex max-w-[28rem] flex-col gap-0.5">
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            {row.runner !== 'pytest' && (
                                ...
                                >
                                    <Tag type={isEnforced ? 'option' : 'muted'} size="small">
                                        {row.runner}
                                    </Tag>
                                </Tooltip>
                            )}
        ...
        },
        {
            ...
            render: (_, row) => (
                ...
                >
                    <Button size="small" icon={<IconEllipsis />} aria-label="More actions" />
                </Menu>
            ),
        },
    ]
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                {hasActiveQuarantineFilters && (
                    <Button type="secondary" size="small" onClick={resetQuarantineFilters}>
                        Reset
                    </Button>
                )}
