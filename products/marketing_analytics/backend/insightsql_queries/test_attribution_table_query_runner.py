...
from insights.models.team.team_marketing_analytics_config import MAX_ATTRIBUTION_WINDOW_DAYS
...
class TestMarketingAnalyticsAttributionQueryRunner(DatastoreTestMixin, BaseTest):
    maxDiff = None
    ...
    def test_per_conversion_is_materialized_for_its_two_readers(self):
        # Two CTEs read per_conversion, and Datastore re-evaluates a plain CTE at each reference, which
    ...
    def test_property_denied_to_this_user_is_never_read(self, _name: str, restricted: bool):
        # The runner hands execute_insightsql_query a prebuilt context, which stops it building a user-aware
        ...
        context = runner._shared_insightsql_context
        # execute_insightsql_query flips this on the context it is handed; do the same to print the real query.
        ...
        with patch(
            ...
            prepare_and_print_ast(runner.to_query(), context=context, dialect="datastore")

        # The property only reaches Datastore as a parameter when the query actually extracts it.
