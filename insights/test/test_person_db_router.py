...
from insights.person_db_router import (
    PersonDBRouter,
...
class TestPersonDBRouterGuard:
    ...
    personinsights fake so the block flag is controlled explicitly here, not by the fixture."""
    ...
    def test_records_orm_access_metric(self):
        ...
        before = REGISTRY.get_sample_value("insights_persons_orm_access_total", labels) or 0.0
        self.router.db_for_read(Person)
        after = REGISTRY.get_sample_value("insights_persons_orm_access_total", labels) or 0.0
        assert after == before + 1


@pytest.mark.django_db
class TestPersonsORMBlockedEndToEnd:
    """End-to-end: with the personinsights fake active (the default autouse fixture), a real
