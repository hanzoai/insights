...
from products.review.backend.reviewer.lazy_seed import seed_canonicals_tolerantly, sync_canonical_perspectives
...
class ReviewPerspectiveConfigSerializer(serializers.Serializer):
    skill_name = serializers.CharField(
        help_text="Name of the `review-script-perspective-*` skill this row toggles (the perspective's identity)."
    )
...
class ReviewPerspectiveConfigViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @extend_schema(
        ...
        description=(
            "List the `review-script-perspective-*` skills visible to the requesting user — the "
    ...
    def list(self, request: Request, **kwargs) -> Response:
        ...
        enabled_by_name = dict(
            ...
            .filter(user_id=user_id, skill_name__startswith=REVIEW_FN_PERSPECTIVE_PREFIX)
        ...
        visible = visible_skill_names(
            team_id, user_id, prefix=REVIEW_FN_PERSPECTIVE_PREFIX, canonical_names=CANONICAL_PERSPECTIVE_SKILL_NAMES
        )
    ...
    @extend_schema(
        ...
        description=(
            "Toggle whether a `review-script-perspective-*` skill runs on the requesting user's PR "
    ...
    def partial_update(self, request: Request, skill_name: str, **kwargs) -> Response:
        if not skill_name.startswith(REVIEW_FN_PERSPECTIVE_PREFIX):
        ...
        if not enabled:
            ...
            others_enabled = (
                ReviewSkillConfig.objects.for_team(team_id, canonical=True)
                .filter(user_id=user_id, enabled=True, skill_name__startswith=REVIEW_FN_PERSPECTIVE_PREFIX)
