...
class TestWebStatsFrustrationLazyPrecompute(DatastoreTestMixin, APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    # ----------------------------------------------------------------------
    # Eligibility — these do not need Datastore, they exercise the gate only.
    # ----------------------------------------------------------------------
    ...
    def test_rejected_when_conversion_goal_set(self):
        from insights.schema import ActionConversionGoal
    ...
    def test_stale_served_enqueues_background_revalidation(self):
        ...
        with (
            ...
            self._enable_lazy(),
            patch(
                "products.web_analytics.backend.insightsql_queries.web_stats_frustration_lazy_precompute.ensure_web_stats_frustration_precomputed",
