...
// UI just renders the typed rows, so there is no client-side InsightsQL or classifier here any more.
...
export function inferOwnerFromSelector(selector: string): string {
    ...
    return product ? `@Insights/team-${product}` : ''
}
    ...
    kea<engineeringAnalyticsLogicType>([
        ...
        listeners(({ actions, values }) => ({
            ...
            submitQuarantineFailure: ({ error }) => {
                toast.error(quarantineRequestErrorMessage(error))
            },
        })),
