...
def patch_side_effects():
    """Mock only the external side effects (Kafka, analytics, Datastore). The Postgres state
    machine is the SUT and is exercised against a real DB."""
    with (
        patch(f"{GROUPING_MODULE_PATH}.emit_embedding_request") as emit_mock,
        patch(f"{GROUPING_MODULE_PATH}.hanzo_insights.capture") as capture_mock,
...
# ---------------------------------------------------------------------------
# Billing exemption: Insights-system signal sources
# ---------------------------------------------------------------------------
...
async def test_new_report_billing_exemption_follows_source_product(ateam, source_product, expected_reason):
    """A report formed from a Insights-system signal (health checks) is stamped never-billable at
...
async def test_deleted_report_skips_counter_updates_and_marks_signal_deleted(ateam, patch_side_effects):
    ...
    - the signal is still emitted to Datastore but marked deleted=True in metadata
