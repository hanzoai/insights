...
import {
    ...
    convertDroppedInsightsUrlToMarkdownNode,
...
<Query hideFilters query={{"kind":"DataVisualizationNode","source":{"kind":"InsightsQLQuery","query":"select event from events limit 1"}}} />`)
    })
    ...
    it('serializes a json-string query attr as an expression that parses back to an object', () => {
        ...
        const queryObject = {
            ...
            source: { kind: 'InsightsQLQuery', query: 'select event, count() from events group by event' },
    ...
    it('converts legacy markdown ast alias nodes without losing structure', () => {
        const content: JSONContent = {
            ...
            content: [
                ...
                { type: 'ph-link', attrs: { href: 'https://app.hanzo.ai/cohorts/37958' } },
            ],
        }
        ...
        expect(markdown).toContain('[https://app.hanzo.ai/cohorts/37958](https://app.hanzo.ai/cohorts/37958)')
...
<Recording id="018a8a51-a39d-7b18-897f-94054eec5f61" timestampMs={12000} title="Activation replay" />`)
    ...
    it('preserves SQL pie chart display intent from notebook artifacts', () => {
        ...
        expect(notebookArtifactContentToMarkdown(content)).toEqual(
            '<Query hideFilters query={{"kind":"DataVisualizationNode","source":{"kind":"InsightsQLQuery","query":"select event, count() from events group by event"},"display":"ActionsPie"}} title="Events pie chart" />'
        )
    })
    ...
    it('converts visualization artifacts to notebook query content', () => {
        ...
        expect(notebookArtifactContentToMarkdown(notebookContent)).toEqual(
            '<Query hideFilters query={{"kind":"DataVisualizationNode","source":{"kind":"InsightsQLQuery","query":"select event, count() from events group by event"},"display":"ActionsPie"}} title="Create a pie chart" />'
        )
    })
...
Body`)
    ...
    describe('convertDroppedInsightsUrlToMarkdownNode', () => {
        ...
        it.each<[string, string, { tagName: string; props: Record<string, unknown> } | null]>([
            ...
            const node = convertDroppedInsightsUrlToMarkdownNode(`${window.location.origin}${path}`)
        ...
        it('ignores URLs from other origins', () => {
            expect(convertDroppedInsightsUrlToMarkdownNode('https://example.com/feature_flags/123')).toBeNull()
        })
    })
