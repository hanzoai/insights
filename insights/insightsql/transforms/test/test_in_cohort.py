...
import pytest
from insights.test.base import BaseTest, QueryMatchingTest, _create_event, _create_person, flush_persons_and_events

from django.conf import settings
from django.test import override_settings

from insights.schema import InsightsQLQueryModifiers, InCohortVia, InlineCohortCalculation
...
class TestInCohort(EventsSchemaSnapshotMixin, BaseTest):
    ...
    def test_in_cohort_dynamic(self):
        ...
        response = execute_insightsql_query(
            ...
            self.team,
            modifiers=InsightsQLQueryModifiers(inCohortVia=InCohortVia.LEFTJOIN),
            pretty=False,
        )
    ...
    def test_in_cohort_error(self):
        with self.assertRaises(QueryError) as e:
            execute_insightsql_query(
                ...
                self.team,
                modifiers=InsightsQLQueryModifiers(inCohortVia=InCohortVia.SUBQUERY),
                pretty=False,
            )
    ...
    def test_in_cohort_conjoined_string(self):
        ...
        response = execute_insightsql_query(
            f"SELECT event FROM events WHERE person_id IN COHORT 'my cohort'",
            self.team,
            modifiers=InsightsQLQueryModifiers(inCohortVia=InCohortVia.LEFTJOIN_CONJOINED),
            pretty=False,
        )
        self._assert_response_matches_snapshot(response)
