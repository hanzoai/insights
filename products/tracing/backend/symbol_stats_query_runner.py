...
class TraceSpansSymbolStatsQueryRunner(AnalyticsQueryRunner[TraceSpansSymbolStatsQueryResponse]):
    ...
    def to_query(self) -> ast.SelectQuery:
        ...
        # scan window, so no InsightsQLFilters/{filters} placeholder is needed — one source of truth.
        inner_where = ast.And(
            exprs=[
    ...
    def _calculate(self) -> TraceSpansSymbolStatsQueryResponse:
        ...
        return TraceSpansSymbolStatsQueryResponse(
            results=[self._row_from_datastore(row, symbols_by_start) for row in response.results],
            granularity=granularity,
        )

    def _row_from_datastore(self, row: list, symbols_by_start: dict[int, SourceSymbol]) -> SymbolStatsRow:
