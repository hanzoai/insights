...
export class HogWatcherService {
    ...
    /**
     * Get the persisted states of a list of script functions.
     *
    ...
    public async observeAggregatedResults(
        observations: { insightsFunction: InsightsFunctionType; totalDurationMs: number }[]
    ): Promise<void> {
        const functionCosts: Record<string, FunctionCostEntry> = {}
        const costConfig = this.costsMapping.script
        if (!costConfig) {
            return
        }

        for (const { insightsFunction, totalDurationMs } of observations) {
            const functionCost = functionCosts[insightsFunction.id] ?? {
            ...
            functionCost.cost += Math.round(costConfig.cost * ratio)
            functionCosts[insightsFunction.id] = functionCost
        }
