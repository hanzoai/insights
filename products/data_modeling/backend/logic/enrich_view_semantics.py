...
their types — so NL-to-InsightsQL picks the right view and columns. It reuses the shared enrichment core
(`insights.llm.semantic_enrichment`) that also backs warehouse table enrichment; only the view-flavoured
...
from insights.llm.gateway_client import Product
from insights.llm.semantic_enrichment import (
...
# InsightsQL view definitions are user-authored and can be arbitrarily long; cap what we ship to the model.
...
def _datastore_type(column_meta: Any) -> str:
    """The Datastore type string from a `saved_query.columns` value (dict or legacy bare string)."""
    if isinstance(column_meta, dict):
        return column_meta.get("datastore") or "unknown"
...
def _view_columns(saved_query: DataWarehouseSavedQuery) -> list[dict[str, Any]]:
    ...
    for name, column_meta in (saved_query.columns or {}).items():
        if not name or len(name) > _MAX_COLUMN_NAME_LENGTH:
            continue
        datastore_type = _datastore_type(column_meta)
        is_nullable = datastore_type.startswith("Nullable(")
        data_type = datastore_type[len("Nullable(") : -1] if is_nullable else datastore_type
...
def compute_enrichment_hash(saved_query: DataWarehouseSavedQuery) -> str:
    ...
    column_pairs = sorted(
        (name, _datastore_type(column_meta)) for name, column_meta in (saved_query.columns or {}).items()
    )
...
def _gather_lineage(team: Team, saved_query: DataWarehouseSavedQuery, query_str: str) -> list[dict[str, Any]]:
    ...
    # Everything else (Insights core tables, unresolved names) contributes its name only.
...
def _get_row_sample(saved_query: DataWarehouseSavedQuery) -> list[dict[str, str]]:
    ...
    """
    from insights.insightsql.query import execute_insightsql_query  # noqa: PLC0415 — heavy InsightsQL/query stack

    from insights.datastore.query_tagging import Feature, Product, tags_context  # noqa: PLC0415
    ...
    try:
        with tags_context(product=Product.WAREHOUSE, feature=Feature.DATA_MODELING):
            ...
            response = execute_insightsql_query(query, team=saved_query.team, bypass_warehouse_access_control=True)
    except Exception as e:
        capture_exception(e)
        return []
...
def _start_enrichment_workflow(team_id: int, saved_query_id: str) -> None:
    ...
    from insights.temporal.data_modeling.activities import EnrichViewSemanticsInputs  # noqa: PLC0415
