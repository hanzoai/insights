...
from posthog.insightsql.ast import SelectQuery, SelectSetNode, SelectSetQuery
...
from posthog.insightsql.property import get_property_type
...
def property_to_typed_property(property: Property) -> EventPropertyFilter | InsightsQLPropertyFilter:
    type = get_property_type(property)
    if type == "event":
        return EventPropertyFilter(**property.to_dict())
    if type == "insightsql":
        return InsightsQLPropertyFilter(**property.to_dict())
...
class InsightsQLRealtimeCohortQuery(InsightsQLCohortQuery):
    """
    ...
    This extends InsightsQLCohortQuery and overrides the methods that need to query
    ...
    def _should_combine_person_properties(self) -> bool:
        ...
        Why disabled:
        - Parent class (InsightsQLCohortQuery) combines multiple person properties into a single query
