...
import type { CustomFlowTestResult } from '../steps/types'
import type { CustomFlowAction } from '../types'
import type { customFlowOutputMappingLogicType } from './customFlowOutputMappingLogicType'
import { createExampleEvent, customFlowEditorTestLogic } from './testing/customFlowEditorTestLogic'

export type OutputMapping = { key: string; result_path: string; spread?: boolean | null }

export function normalizeOutputVariable(raw: CustomFlowAction['output_variable']): OutputMapping[] {
    if (!raw) {
        return []
    }
...
export const customFlowOutputMappingLogic = kea<customFlowOutputMappingLogicType>([
    path((key) => [
        'products',
        'workflows',
        'frontend',
    ...
    connect((props: WorkflowLogicProps) => ({
        values: [
            workflowLogic(props),
            ['workflow', 'customFunctionTemplatesById'],
            customFlowEditorLogic,
            ['selectedNode'],
            customFlowEditorTestLogic(props),
    ...
    listeners(({ actions, values, props }) => {
        ...
        return {
            ...
            runOutputTest: async () => {
                const { selectedNode, workflow, customFunctionTemplatesById } = values
                if (!selectedNode) {
                    return
                }
                ...
                try {
                    ...
                    const config = sanitizeWorkflow(JSON.parse(JSON.stringify(workflow)), customFunctionTemplatesById)
                    ...
                    const result: CustomFlowTestResult = await api.customFlows.createTestInvocation(workflow.id, {
