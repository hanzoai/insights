...
**GitHub access.**
Before your first push, set up an [SSH key and commit signing](https://hanzo.ai/handbook/engineering/security#github) using Secretive or 1Password.
Signing is required: an org-wide ruleset rejects unsigned commits in every Insights repository, so an unsigned push fails with `GH013: Commits must have verified signatures`.
