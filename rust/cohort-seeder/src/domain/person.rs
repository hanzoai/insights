...
//! **Datastore's UUID order** — boundaries arrive from a `GROUP BY id … ORDER BY id` stream and are
//! opaque here. Rust never sorts or compares person UUIDs beyond byte equality: a Rust-side
//! reordering would produce overlapping ranges in Datastore semantics, and an ordering check on
...
/// only on the last band). Both endpoints are opaque Datastore-order boundaries.
...
impl PersonRange {
    ...
    /// check — the ordering lives in Datastore.
...
/// evaluator, and mint at most one [`PersonSeed`]. Unit-testable without Datastore or Kafka.
...
mod tests {
    ...
    proptest! {
        ...
        /// Datastore-order values and the tiling must preserve their arrival order.
        #[test]
    ...
    fn evaluate_row_matches_the_live_event_paths_verdict() {
        ...
        use cohort_core::scriptvm::build_person_property_globals;
        ...
        for properties in [r#"{"email":"a@b.com","plan":"paid"}"#, r#"{"email":"x"}"#] {
            ...
            for (hash, bytecode) in build_evaluator(true).conditions.iter() {
                let live = matches!(
                    cohort_core::scriptvm::evaluate_detailed(bytecode, globals.clone()),
                    EvalOutcome::Matched(true)
                );
