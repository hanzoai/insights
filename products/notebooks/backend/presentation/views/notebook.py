...
class NotebookViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def sql_v2_run(self, request: Request, **kwargs):
        ...
        # A SQL node's runs can alternate between insightsql and duckdb (Journey 5 rerouting), so a
        # kind=insightsql ref is only trustworthy when the node's LATEST result really is insightsql —
        # a duckdb run's code is raw, non-self-contained SQL naming kernel frames, and inlining
        # it as a CTE would ship it to Datastore. Treat that node as not-run instead.
