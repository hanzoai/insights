//! Datastore client construction: endpoint precedence, the TLS posture, and the typed
//! `join_algorithm` setting.
//!
//! Depends on `config` for the raw env strings; every value that shapes a Datastore query option is
...
/// The resolved Datastore HTTP endpoint. Precedence is explicit URL > offline cluster host > host;
/// the scheme comes from `secure`, and a bare host gets the canonical port (8443 secure, 8123 plain).
/// Resolved exactly once by [`DatastoreEndpoint::resolve`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DatastoreEndpoint(String);
...
impl DatastoreEndpoint {
    ...
    /// Whether the resolved URL actually carries TLS. `datastore_secure` alone does not answer
    /// this: an explicit `datastore_url` or a scheme-qualified host bypasses it entirely.
...
fn resolve_endpoint(config: &Config) -> String {
    if !config.datastore_url.is_empty() {
        return config.datastore_url.clone();
    }
    let host = if config.datastore_offline_cluster_host.is_empty() {
        &config.datastore_host
    } else {
        &config.datastore_offline_cluster_host
    };
    if host.starts_with("http://") || host.starts_with("https://") {
        return host.clone();
    }
    let scheme = if config.datastore_secure {
        "https"
    } else {
        "http"
    };
    if has_explicit_port(host) {
        ...
        let port = if config.datastore_secure { 8443 } else { 8123 };
...
/// The Datastore `join_algorithm` query setting. Parsed from config so an unknown value is a startup
/// failure rather than a silent pass-through that would degrade join memory behavior. `as_str` emits
/// the exact Datastore token, so the option value is byte-identical to the raw string it parses.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DatastoreJoinAlgorithm {
    Default,
    Auto,
    Hash,
    ParallelHash,
...
impl FromStr for DatastoreJoinAlgorithm {
...
#[error("unknown Datastore join algorithm {0:?}")]
...
pub enum DatastoreClientError {
    ...
    #[error("building the Datastore TLS configuration")]
    Tls(#[from] rustls::Error),
    #[error("reading the Datastore CA bundle at {path}")]
    CaBundleUnreadable {
        path: String,
        #[source]
        source: pem::Error,
    },
    #[error("the Datastore CA bundle at {path} contains no certificates")]
    CaBundleEmpty { path: String },
    #[error("DATASTORE_CA is set but the endpoint {endpoint} is not https, so nothing would be verified")]
    CaBundleWithoutTls { endpoint: String },
}

// Copied from `datastore` 0.13's default HTTP client; recheck on upgrade. The idle timeout must
// stay under Datastore's server-side keep-alive or the pool hands out sockets the server closed.
...
/// anything able to redirect traffic can impersonate Datastore. Set `DATASTORE_CA` to close that.
#[derive(Debug)]
...
fn pinned_ca_tls_config(ca_path: &str) -> Result<ClientConfig, DatastoreClientError> {
    let unreadable = |source| DatastoreClientError::CaBundleUnreadable {
    ...
    if roots.is_empty() {
        return Err(DatastoreClientError::CaBundleEmpty {
            path: ca_path.to_string(),
        });
    }
...
fn client_with_tls_config(tls_config: ClientConfig) -> datastore::Client {
    ...
    datastore::Client::with_http_client(
...
pub fn build_client(config: &Config) -> Result<datastore::Client, DatastoreClientError> {
    let join_algorithm = config
        .seeder_ch_join_algorithm
        .parse::<DatastoreJoinAlgorithm>()?;
    let endpoint = DatastoreEndpoint::resolve(config);
    // Ordered most to least trusted.
    let client = if !config.datastore_ca.is_empty() {
        // A CA over a plaintext endpoint verifies nothing; refuse rather than appear authenticated.
        if !endpoint.is_tls() {
            return Err(DatastoreClientError::CaBundleWithoutTls {
                endpoint: endpoint.as_str().to_string(),
            });
        }
        client_with_tls_config(pinned_ca_tls_config(&config.datastore_ca)?)
    } else if config.datastore_verify {
        datastore::Client::default()
    } else {
        client_with_tls_config(unverified_tls_config()?)
    };
    Ok(client
        .with_url(endpoint.as_str())
        .with_user(&config.datastore_user)
        .with_password(&config.datastore_password)
        .with_database(&config.datastore_database)
        .with_option(
            "max_execution_time",
...
";
    ...
    fn endpoint(config: &Config) -> String {
        DatastoreEndpoint::resolve(config).as_str().to_string()
    }

    #[test]
    fn bare_datastore_hosts_get_the_canonical_port_for_the_scheme() {
        for (secure, expected) in [
            (false, "http://datastore.internal:8123"),
            (true, "https://datastore.internal:8443"),
        ] {
            let mut config = default_config();
            config.datastore_host = "datastore.internal".to_string();
            config.datastore_secure = secure;
            assert_eq!(endpoint(&config), expected);
        }

        let mut config = default_config();
        config.datastore_host = "fallback.internal".to_string();
        config.datastore_offline_cluster_host = "offline.internal".to_string();
        config.datastore_secure = true;
        assert_eq!(endpoint(&config), "https://offline.internal:8443");
    }

    #[test]
    fn explicit_datastore_urls_and_ports_are_preserved() {
        let mut config = default_config();
        config.datastore_url = "https://proxy.example:9440/datastore".to_string();
        assert_eq!(endpoint(&config), "https://proxy.example:9440/datastore");

        config.datastore_url.clear();
        config.datastore_host = "datastore.internal:9000".to_string();
        config.datastore_secure = true;
        assert_eq!(endpoint(&config), "https://datastore.internal:9000");
    }
    ...
    fn join_algorithm_default_config_parses_and_round_trips() {
        assert_eq!(
            default_config()
                ...
                .parse::<DatastoreJoinAlgorithm>()
                .unwrap(),
            DatastoreJoinAlgorithm::GraceHash
        );
        for algorithm in [
            DatastoreJoinAlgorithm::Default,
            DatastoreJoinAlgorithm::Auto,
            DatastoreJoinAlgorithm::Hash,
            DatastoreJoinAlgorithm::ParallelHash,
            DatastoreJoinAlgorithm::GraceHash,
            DatastoreJoinAlgorithm::PartialMerge,
            DatastoreJoinAlgorithm::PreferPartialMerge,
            DatastoreJoinAlgorithm::FullSortingMerge,
            DatastoreJoinAlgorithm::Direct,
        ] {
            assert_eq!(
                algorithm
                    .as_str()
    ...
    fn build_client_rejects_a_join_algorithm_typo_at_startup() {
        ...
        assert!(matches!(
            build_client(&config),
            Err(DatastoreClientError::JoinAlgorithm(UnknownJoinAlgorithm(algorithm)))
    ...
    fn the_verify_off_verifier_accepts_an_untrusted_certificate() {
        ...
        assert!(verifier
            .verify_server_cert(
                ...
                &[],
                &ServerName::try_from("datastore.internal").unwrap(),
                &[],
                UnixTime::now(),
            )
            .is_ok());
    ...
    fn a_pinned_ca_bundle_builds_a_verifying_client() {
        ...
        let mut config = default_config();
        config.datastore_secure = true;
        config.datastore_ca = bundle.to_string_lossy().into_owned();
        assert!(build_client(&config).is_ok());

        // A CA over plaintext would verify nothing, so it must not look like it succeeded.
        config.datastore_secure = false;
    ...
    fn an_unusable_ca_bundle_fails_startup_rather_than_downgrading() {
        ...
        for (ca_path, expected) in [
            (
                ...
                "reading the Datastore CA bundle",
            ),
            (
            ...
            config.datastore_verify = false;
            config.datastore_ca = ca_path.clone();
            let Err(error) = build_client(&config) else {
