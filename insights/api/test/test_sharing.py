...
from insights.api.sharing import (
...
from insights.models import ActivityLog, OrganizationMembership
...
<html>
    ...
    <body>
        ...
        with patch("insights.api.sharing.render_template") as mock_render_template:
...
class TestSharing(APIBaseTest):
    ...
    @patch("insights.api.sharing.asset_for_token")
    ...
    def test_sharing_endpoints_succeed_with_duplicate_active_configs(
        ...
        base_url = f"/v1/projects/{self.team.id}/dashboards/{dashboard.id}/sharing"
        if action == "get":
...
class TestSharingConfigurationSerializerValidation(APIBaseTest):
    ...
    @patch("products.exports.backend.api.exports.ExportedAssetSerializer._start_export_workflow")
    @patch("insights.api.sharing.render_template")
...
class TestSharedAdhocQueryExport(APIBaseTest):
    ...
    @patch("insights.api.sharing.process_query_dict")
    @patch("insights.api.sharing.render_template")
...
class TestSharedCohortInlining(APIBaseTest):
    ...
    def _parse_exported_data(html: str) -> dict:
        ...
        start_marker = '<script id="insights-exported-data" type="application/json">'
...
class TestSharingResourceEditChecks(APIBaseTest):
    ...
    def test_assertion_rejects_a_registry_out_of_sync_with_the_model(self, _name, drop_key, add_key):
        ...
        with patch.dict("insights.api.sharing.SHARING_RESOURCE_EDIT_CHECKS", mutated, clear=True):
            with self.assertRaises(ImproperlyConfigured):
                _assert_every_shareable_resource_is_gated()

    @patch("insights.api.sharing.UserAccessControl")
...
def _warehouse_ac_flag(key: str, *args, **kwargs) -> bool:
    return key == "insightsql-warehouse-access-control"


@patch("hanzo_insights.feature_enabled", new=Mock(side_effect=_warehouse_ac_flag))
class TestSharedLinkWarehouseExecution(APIBaseTest):
    def setUp(self):
        ...
        self.insight = Insight.objects.create(
            ...
            query={
                "kind": "DataTableNode",
                "source": {"kind": "InsightsQLQuery", "query": "SELECT id FROM governed_view"},
            },
            created_by=self.user,
        )
        self.client.logout()
...
class TestSharingPublishGate(APIBaseTest):
    ...
    def _enable_sharing(self, kind: str):
        if kind == "insight":
            url = f"/v1/projects/{self.team.id}/insights/{self.insight.id}/sharing"
        elif kind == "dashboard":
            dashboard = Dashboard.objects.create(team=self.team, created_by=self.user)
            DashboardTile.objects.create(dashboard=dashboard, insight=self.insight)
            url = f"/v1/projects/{self.team.id}/dashboards/{dashboard.id}/sharing"
        else:
            notebook = Notebook.objects.create(
                team=self.team,
                created_by=self.user,
            ...
            url = f"/v1/projects/{self.team.id}/notebooks/{notebook.short_id}/sharing"
    ...
    def test_system_table_denial_blocks_publishing(self):
        ...
        self.insight.query = {
            "kind": "DataTableNode",
            "source": {"kind": "InsightsQLQuery", "query": "SELECT id FROM system.dashboards"},
        }
        self.insight.save()
    ...
    def test_granted_view_over_denied_table_gates_unless_materialized(self, case: str):
        ...
        outer = DataWarehouseSavedQuery.objects.create(
            team=self.team,
            name="safe_view",
            query={"kind": "InsightsQLQuery", "query": "SELECT id FROM restricted_inner"},
            columns={"id": "String"},
        )
        ...
        self.insight.query = {
            "kind": "DataTableNode",
            "source": {"kind": "InsightsQLQuery", "query": "SELECT id FROM safe_view"},
        }
        self.insight.save()
    ...
    def test_unparseable_query_does_not_block_publishing(self):
        ...
        self.insight.query = {
            "kind": "DataTableNode",
            "source": {"kind": "InsightsQLQuery", "query": "SELECT FROM WHERE ((("},
        }
        self.insight.save()
...
class TestSaveTimeAccessBlock(APIBaseTest):
    def setUp(self):
        ...
        self.insight = Insight.objects.create(
            team=self.team,
            query={"kind": "DataTableNode", "source": {"kind": "InsightsQLQuery", "query": "SELECT 1 AS one"}},
            created_by=self.user,
        )
    ...
    def _patch_insight_query(self):
        return self.client.patch(
            f"/v1/projects/{self.team.id}/insights/{self.insight.id}/", {"query": self._DENIED_QUERY}
        )
    ...
    def test_query_update_blocked_when_insight_is_publicly_shared(self, coverage: str):
        ...
        assert self.insight.query == {
            "kind": "DataTableNode",
            "source": {"kind": "InsightsQLQuery", "query": "SELECT 1 AS one"},
        }
    ...
    def test_adding_insight_to_shared_dashboard_blocked(self):
        ...
        response = self.client.patch(
            f"/v1/projects/{self.team.id}/insights/{self.insight.id}/", {"dashboards": [dashboard.id]}
        )
    ...
    @parameterized.expand(
        [
            ("inline_query", {"kind": "InsightsQLQuery", "query": "SELECT id FROM governed_view"}),
            ("incomplete_query", {"kind": "InsightsQLQuery", "query": "SELECT FROM WHERE ((("}),
        ]
    )
