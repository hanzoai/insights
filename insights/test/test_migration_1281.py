...
migration = importlib.import_module("insights.migrations.1281_migrate_managed_warehouse_models")
...
def test_content_type_copy_preserves_grants_across_forward_reverse_and_reapply(model_name: str) -> None:
    migration_apps = (
        MigrationExecutor(connection).loader.project_state([("insights", "1281_migrate_managed_warehouse_models")]).apps
    )
    source_content_type, _ = ContentType.objects.get_or_create(app_label="insights", model=model_name)
