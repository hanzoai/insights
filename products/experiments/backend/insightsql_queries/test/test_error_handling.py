...
from products.experiments.backend.insightsql_queries.error_handling import (
    ERROR_TYPE_TO_CODE,
...
def _record_captures():
    ...
    with patch(
        "products.experiments.backend.insightsql_queries.error_handling.ph_scoped_capture",
...
class TestExperimentErrorHandling(BaseTest):
    ...
    @patch("products.experiments.backend.insightsql_queries.error_handling.capture_exception")
    ...
    @parameterized.expand(
        [
            ("wrapped_timeout", DatastoreQueryTimeOut(), "timeout"),
            ("ch_timeout_code", ServerException("timed out", code=159), "timeout"),
            ("wrapped_oom", DatastoreQueryMemoryLimitExceeded(), "out_of_memory"),
            ...
            ("wrapped_at_capacity", DatastoreAtCapacity(), "rate_limited"),
            ...
            ("exposed_insightsql_error", ExposedInsightsQLError("unknown property"), "validation_error"),
...
class TestTerminalErrorEventEmission(BaseTest):
    ...
    def test_user_facing_validation_errors_still_emit(self):
        # ValidationError/ExposedInsightsQLError pass through the boundary unconverted, but a misconfigured
