...
Serializer fields are the source of truth for Insights's entire type pipeline:
...
15. **ViewSets outside `products/` need `@extend_schema(extensions={"x-product": "<product>"})`** — ViewSets in `products/<name>/backend/` are auto-attributed via module path; ViewSets in `insights/api/` or `ee/` aren't and must declare attribution explicitly via the `x-product` extension. Accepts a plain string (`"product_analytics"`) or `ProductKey.X` enum (kebab values are normalized). Don't use `tags=["<product>"]` to influence codegen routing — `tags` is for Swagger UI display only. Without `x-product`, the MCP scaffold and frontend type generator can't route the endpoint to the right product
16. **`partial_update` `request=` override must be a superset of runtime write fields** — `extend_schema(request=CustomSerializer)` replaces drf-spectacular's inference from `serializer_class`; omitted fields disappear from OpenAPI, frontend types, and MCP tool schemas even when the runtime serializer still accepts them. After changing the override, run `insightscli build:openapi` and verify generated MCP tool schemas still expose every OpenAPI body field
...
Product routes are **auto-discovered** — `insights/api/__init__.py` iterates
...
`PRODUCTS_APPS` (`insights/settings/web.py`). Only core, non-product viewsets still
...
eager (it runs when `insights.api` is first imported, i.e. on the first request) and
...
`RouterRegistry` docstring and the discovery loop in `insights/api/__init__.py` for
the full reasoning.
...
- **JSONField + @extend_schema_field:** `insights/api/alert.py`
...
- **Mixins:** `insights/api/mixins.py` (`@validated_request` source)
- **drf-spectacular config:** `insights/settings/web.py` (`SPECTACULAR_SETTINGS`)
