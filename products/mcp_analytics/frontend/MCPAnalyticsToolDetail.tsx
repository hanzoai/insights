...
import { Button, Divider, Modal, Skeleton, Tooltip } from '@hanzo/elements'
import {
    type ChartTheme,
    type Series,
    type TimeInterval,
...
function DescriptionBlock({
    ...
    if (loading) {
        return <Skeleton className="h-8 w-2/3 mt-2" />
    }
    ...
    return (
        <div className="flex flex-col gap-1 max-w-3xl">
            ...
            <Markdown className="text-sm leading-snug line-clamp-3" lowKeyHeadings>
                {latest.description}
            </Markdown>
            {older.length > 0 ? (
                <Tooltip
                        ...
                        <div className="flex flex-col gap-2 max-w-md">
                            {older.map((d) => (
                                <div key={d.last_seen} className="text-xs">
                                    ...
                                    <Markdown lowKeyHeadings>{d.description}</Markdown>
                                </div>
                            ))}
                        </div>
                    }
