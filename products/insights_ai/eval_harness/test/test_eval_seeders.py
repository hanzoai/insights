...
from insights.schema import DateRange, ErrorTrackingQuery
...
from products.insights_ai.evals.error_tracking.seeders import _EVAL_DISTINCT_IDS, seed_error_tracking_issues
...
class TestErrorTrackingEvalSeeders(DatastoreTestMixin, APIBaseTest):
    ...
    def test_error_tracking_seeded_events_survive_person_filters(self) -> None:
        test_users_cohort = get_or_create_internal_test_users_cohort(
            self.team, initiating_user_email="eval-master-seed@insights.test"
        )
        ...
        seed = seed_error_tracking_issues(
            _EvalSeedContext(team_id=self.team.id, user_id=self.user.id, repository="insights/hedgebox")
        )
