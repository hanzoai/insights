...
It ingests data from Kafka to a proper Datastore shard.
Ingestion nodes are part of the extended Datastore cluster
...
`insights/datastore/schema.py` in the appropriate tuple so the table is created locally:
...
- `cluster`: Override cluster (default: `settings.DATASTORE_CLUSTER`)
  - Use `cluster=settings.DATASTORE_SINGLE_SHARD_CLUSTER` for non-sharded tables to point to single shard
...
> [!CAUTION]
> Never write a `DROP COLUMN` migration on your own. `DROP COLUMN` can get stuck in Datastore
> and block releases. Column removal follows a two-step process:
>
> 1. The Datastore team drops the column directly on the cluster.
...
> operation, **not** on the table — `ALTER TABLE IF EXISTS ...` is a Datastore syntax error.
>
...
> nothing, but Datastore still keeps a declared CODEC in `SHOW CREATE TABLE`, so one there
...
> sharded instance via `patch_column` — see `insights/datastore/hcl/README.md`.
...
> schema and break event ingestion. Any change must go through the Datastore team.

> [!INFO]
> A PR containing a Datastore migration must be migration-only. Do not mix it with feature code,
> API changes, model changes, or frontend changes. Migration-related files are:
>
> - The migration file itself (`insights/datastore/migrations/0NNN_*.py`)
> - SQL definition files the migration depends on (e.g. `insights/datastore/sql/*.py`)
...
2. Create writable distributed table on ingestion nodes with `DATASTORE_SINGLE_SHARD_CLUSTER` for non-sharded tables
...
operations = [
    ...
    run_sql_with_exceptions(
        WRITABLE_TABLE_SQL(),  # Uses cluster=settings.DATASTORE_SINGLE_SHARD_CLUSTER
        node_roles=[NodeRole.INGESTION_SMALL]
    ),
