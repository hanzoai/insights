...
the InsightsQL query against an expected reference query. Mirrors the trends /
...
in ``ee/scriptai/eval/scorers/sql.py`` — same InsightsQL guidance, but graded on
...
``JUDGE_MODEL``) lives in ``products/insights_ai/eval_harness/scorers/``; only the
...
from products.insights_ai.eval_harness.scorers import GRADED_ALIGNMENT_CHOICE_SCORES, JUDGE_MODEL, JudgedScorer
from products.insights_ai.eval_harness.scorers.contract import Score, Scorer
from products.insights_ai.evals.product_analytics.scorers import GRADED_ALIGNMENT_RUBRIC, parser_for, user_prompt
...
You are an expert Datastore SQL auditor judging whether an agent-produced InsightsQL query would equally answer the user's question, compared to a reference "ideal" query.

InsightsQL is an SQL flavor derived from Datastore SQL, with Insights-specific syntax:
...
class SQLSchemaAlignment(JudgedScorer):
    """Graded score: how well does the actual InsightsQL query match the expected one?
...
You are an expert analytics QA reviewer judging whether an agent's final response correctly answers the user's question after running InsightsQL.
