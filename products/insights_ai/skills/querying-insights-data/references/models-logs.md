...
OpenTelemetry log entries. One row per log line. Backed by Datastore `logs_distributed`.

**Prefer the typed tool when it fits:** `insights:query-logs` for filtered list queries (with the encoded discovery → narrow → count → drill-down workflow). Reach for InsightsQL when you need cross-signal joins (with `insights.trace_spans` or `insights.metrics` by `trace_id`) or aggregations the typed tool doesn't expose.

**Namespacing:** `logs` and `log_attributes` are registered at the InsightsQL root level — reference them as bare names. (Asymmetric with `insights.trace_spans` and `insights.metrics`, which require the `insights.` namespace prefix.)
...
- **`trace_id` and `span_id` are base64-encoded bytes**, not hex. The displayed hex form (e.g. `21EDB3A025A9ECD32ADF3E5D7548A4F4`) comes from the API layer via `hex(tryBase64Decode(trace_id))`. Raw InsightsQL queries see the 24-character base64 form (e.g. `21EDB3A025A9ECD32ADF3E5D7548A4F4` becomes `Ie2zoCWp7NMq3z5ddUik9A==`).
...
- Cross-signal joins by `trace_id` work against `insights.trace_spans` (both store base64) and `insights.metrics` _once exemplar extraction is wired up in ingestion_ — see the metrics reference for the current state.
- User InsightsQL queries on `logs` are capped at 50 GB read per query.
...
Prefer `insights:logs-attributes-list` / `insights:logs-attribute-values-list` over querying this table directly — they handle the aggregation correctly.
