"""insightscli lint:migration-deletions — fail when a PR deletes a historical Django migration.
...
# Datastore and async migrations are separate systems, excluded by grandparent dir name.
_NON_DJANGO_PARENTS = {"datastore", "async_migrations"}
...
def is_django_migration(path: str) -> bool:
    """True for a numbered Django migration file, e.g. insights/migrations/0001_initial.py.

    Require the NNNN_ prefix: it excludes __init__.py and, crucially, the standalone data
    scripts that also live in a migrations/ dir (e.g. insights/rbac/migrations/rbac_*.py),
    which are not Django ORM migrations. Also excludes the Datastore/async systems.
    """
