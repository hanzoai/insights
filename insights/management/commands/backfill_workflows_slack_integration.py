...
# primary key (173069). 40 insightsflows still reference the old integration row (54567),
...
class Command(BaseCommand):
    help = (
        ...
        f"in InsightsFlow actions on team {TEAM_ID}."
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Scanning InsightsFlows for team {TEAM_ID}, "
            f"swapping slack_workspace {OLD_INTEGRATION_ID} → {NEW_INTEGRATION_ID}..."
        )

        # Team 2 has on the order of a few hundred insightsflows. Pull them all in one go;
        # no pagination required at this scale, and a single transaction keeps the
        # write atomic for the team.
        flows = list(InsightsFlow.objects.filter(team_id=TEAM_ID).order_by("id"))
        total_count = len(flows)
        self.stdout.write(f"Found {total_count} InsightsFlows on team {TEAM_ID}")

        if total_count == 0:
            return

        updated_flows: list[InsightsFlow] = []
        ...
        for flow in flows:
            try:
                ...
                error_count += 1
                logger.exception(
                    "Error processing InsightsFlow id=%s team_id=%s: %s",
                    flow.id,
                    flow.team_id,
                    e,
                )
        ...
        if updated_flows and not dry_run:
            ...
            with transaction.atomic():
                InsightsFlow.objects.bulk_update(updated_flows, ["actions", "draft"], batch_size=200)
