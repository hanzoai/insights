...
function taxonomicSelectionToColumn(group: TaxonomicFilterGroup, value: string): Omit<LogsColumnConfig, 'id'> | null {
    ...
    if (group.type === TaxonomicFilterGroupType.InsightsQLExpression) {
    ...
    return (
        <div className="flex flex-col gap-2 p-3 border rounded">
            <div className="flex items-end gap-2">
                <Field.Pure label="Column type">
                    <Select<LogsColumnType>
                        size="small"
                ...
                <Field.Pure label="Name" className="flex-1">
                    <Input
                        size="small"
                        placeholder={
    ...
    return (
        <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} {...attributes}>
            <div className="flex items-center justify-start px-2 py-1 my-1 rounded bg-accent-highlight-secondary">
                ...
                {column.type === 'custom' && (
                    <Tooltip title="Edit">
                        <Button size="small" onClick={() => setEditingColumnId(isEditing ? null : column.id)}>
                            <IconPencil data-attr="logs-column-edit-icon" />
                        </Button>
                    </Tooltip>
                )}
                <Tooltip title="Remove">
                    <Button size="small" status="danger" onClick={() => removeDraftColumn(column.id)}>
        ...
        <>
            ...
            <Modal
                ...
                footer={
                    <>
                        <div className="flex-1">
                            <Button type="secondary" onClick={() => setDraft(DEFAULT_LOGS_COLUMNS)}>
                                Reset to defaults
                            </Button>
                        </div>
                        <Button type="secondary" onClick={closeConfigurator}>
                            Close
                        </Button>
                        <Button
                            type="primary"
