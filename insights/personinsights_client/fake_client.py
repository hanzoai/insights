...
def _order_identified_first(
    ...
    """Mirror the server-side ordering of limited distinct_id fetches (personinsights-replica
...
class FakePersonHogClient:
    ...
    # _fetch_persons_by_distinct_ids_via_personinsights) must deduplicate
    # themselves.
...
def personinsights_fake_active() -> bool:
    """Whether a personinsights fake is active for the current test.
...
def activate_personinsights_fake():
    ...
    helpers in insights.test.persons seed the fake explicitly when creating data.
    """
    ...
    original_get_client = personinsights_client_module.get_personinsights_client
    personinsights_client_module.get_personinsights_client = _get_fake_client  # ty: ignore[invalid-assignment]
    try:
        yield fake
    finally:
        personinsights_client_module.get_personinsights_client = original_get_client
        unblock_persons_orm()
