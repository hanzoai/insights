...
class TestBuildPromptMessages:
    ...
    def test_includes_core_memory(self):
        ...
        messages = build_prompt_messages(
            previous,
            current,
            core_memory_text="Company is Insights.\nFlagship product is product analytics.",
        )

        user_content = messages[-1]["content"]
        assert "<core_memory>" in user_content
        assert "</core_memory>" in user_content
        assert "Company is Insights." in user_content
...
class TestGenerateChangeSummary:
    ...
    @patch("insights.event_usage.SITE_URL", "https://us.hanzo.ai")
    @patch("products.exports.backend.temporal.subscriptions.llm_change_summary._get_openai_client")
    def test_generation_is_tagged_billable(self, mock_get_client):
        ...
        properties = call_kwargs["insights_properties"]
        assert properties["$ai_billable"] is True
        assert properties["team_id"] == 42
        assert properties["ai_product"] == "subscriptions"
        assert call_kwargs["insights_groups"] == {"project": "42", "instance": "https://us.hanzo.ai"}
    ...
    def test_marks_generation_billable_to_team_when_team_provided(self, mock_get_client):
        ...
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs["insights_properties"]["$ai_billable"] is True
        assert call_kwargs["insights_properties"]["team_id"] == 42
        assert call_kwargs["insights_properties"]["ai_product"] == "subscriptions"
        assert call_kwargs["insights_properties"]["delivery_id"] == "abc-123"
        assert call_kwargs["insights_groups"] == {"project": "42", "instance": "https://us.hanzo.ai"}
        assert call_kwargs["insights_distinct_id"] == call_kwargs["user"]
    ...
    def test_does_not_mark_billable_when_team_missing(self, mock_get_client):
        ...
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert "$ai_billable" not in call_kwargs["insights_properties"]
        assert "team_id" not in call_kwargs["insights_properties"]
        assert "insights_groups" not in call_kwargs
    ...
    def test_emits_ai_generation_event_with_billing_properties(self, monkeypatch):
        ...
        monkeypatch.setattr("insights.event_usage.SITE_URL", "https://us.hanzo.ai")

        fake_client = MagicMock()
        monkeypatch.setattr("hanzo_insights.default_client", fake_client)
        ...
        assert captured["groups"] == {"project": "42", "instance": "https://us.hanzo.ai"}
