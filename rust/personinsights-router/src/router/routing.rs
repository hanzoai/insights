use personinsights_proto::personinsights::types::v1::{ConsistencyLevel, ReadOptions};
use tonic::Status;
...
pub enum RouteDecision {
    /// Route to personinsights-replica (Postgres replicas, or primary for strong consistency on non-person data)
    Replica,
    /// Route to personinsights-leader (Kafka-backed cache, Phase 2)
    Leader,
}
...
/// # Phase 2 Evolution
///
/// When personinsights-leader is implemented:
/// 1. Add leader_backend to PersonInsightsRouter
...
pub fn route_request(
    ...
    match (category, operation) {
            ...
            match consistency {
                ...
                ConsistencyLevel::Strong => {
                    ...
                    Err(RoutingError::BackendNotAvailable {
                        message: "Strong consistency for person data requires personinsights-leader \
        ...
        (DataCategory::PersonData, OperationType::Write) => {
            // Phase 2: Return RouteDecision::Leader when available
            Err(RoutingError::BackendNotAvailable {
                message: "Person data writes require personinsights-leader (Phase 2). \
...
mod tests {
    ...
    fn test_person_data_strong_returns_error() {
        ...
        assert!(status.message().contains("personinsights-leader"));
    }
