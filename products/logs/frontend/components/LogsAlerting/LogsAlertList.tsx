...
export function LogsAlertList(): JSX.Element {
    ...
    const columns: TableColumns<LogsAlertConfigurationApi> = [
        {
            title: 'Name',
            dataIndex: 'name',
            render: (_, alert) => (
                <Button type="tertiary" size="small" to={urls.logsAlertDetail(alert.id)}>
                    {alert.name}
                </Button>
            ),
        },
        ...
        createdByColumn() as unknown as TableColumn<
            LogsAlertConfigurationApi,
    ...
    if (alerts.length === 0 && !createdByFilter) {
        return (
            <ProductIntroduction
                ...
                docsURL="https://hanzo.ai/docs/logs/alerts"
            />
        )
    }
