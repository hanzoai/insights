...
from posthog.plugins.plugin_server_api import (
    get_custom_function_status,
    patch_custom_function_status,
...
class CustomFunctionState(enum.Enum):
    UNKNOWN = 0
    HEALTHY = 1
    DEGRADED = 2
    DISABLED = 3
...
class CustomFunctionType(models.TextChoices):
...
TYPES_THAT_RELOAD_PLUGIN_SERVER = (
    CustomFunctionType.DESTINATION,
    CustomFunctionType.TRANSFORMATION,
    CustomFunctionType.INTERNAL_DESTINATION,
    CustomFunctionType.SOURCE_WEBHOOK,
    CustomFunctionType.WAREHOUSE_SOURCE_WEBHOOK,
)
TYPES_WITH_TRANSPILED_FILTERS = (CustomFunctionType.SITE_DESTINATION, CustomFunctionType.SITE_APP)
TYPES_WITH_JAVASCRIPT_SOURCE = (CustomFunctionType.SITE_DESTINATION, CustomFunctionType.SITE_APP)


class CustomFunction(FileSystemSyncMixin, UUIDTModel):
    class Meta:
        indexes = [
    ...
    custom_function_template = models.ForeignKey(
        "posthog.CustomFunctionTemplate",
    ...
    @classmethod
    def get_file_system_unfiled(cls, team: "Team") -> QuerySet["CustomFunction"]:
        base_qs = CustomFunction.objects.filter(team=team, deleted=False)
        return cls._filter_unfiled_queryset(base_qs, team, type__startswith="custom_function/", ref_field="id")
    ...
    @property
    def template(self) -> Optional[CustomFunctionTemplate]:
        if self.custom_function_template:
            return self.custom_function_template

        if not self.template_id:
            return None

        return CustomFunctionTemplate.get_template(self.template_id)
    ...
    def status(self) -> dict:
        ...
        try:
            ...
            res = get_custom_function_status(self.team_id, self.id)
    ...
    def set_function_status(self, state: int) -> dict:
        if not self.enabled:
            return self.status
        try:
            res = patch_custom_function_status(self.team_id, self.id, state)
            if res.status_code == 200:
    ...
    def __str__(self):
        return f"CustomFunction {self.id}: {self.name}"


@receiver(post_save, sender=CustomFunction)
def custom_function_saved(sender, instance: CustomFunction, created, **kwargs):
    if instance.type is None or instance.type in TYPES_THAT_RELOAD_PLUGIN_SERVER:
        reload_custom_functions_on_workers(team_id=instance.team_id, custom_function_ids=[str(instance.id)])
...
def action_saved(sender, instance: Action, created, **kwargs):
    ...
    from posthog.tasks.custom_functions import refresh_affected_custom_functions

    refresh_affected_custom_functions.delay(action_id=instance.id)
...
def team_saved(sender, instance: Team, created, **kwargs):
    ...
    refresh_affected_custom_functions.delay(team_id=instance.id)


@mutable_receiver([post_save, post_delete], sender=CustomFunction)
...
def enabled_default_custom_functions_for_new_team(sender, instance: Team, created: bool, **kwargs):
    ...
    from posthog.models.custom_function_template import CustomFunctionTemplate
    from posthog.plugins.plugin_server_api import get_custom_function_templates
    ...
    template = CustomFunctionTemplate.get_template("template-geoip")
    ...
    if not template:
        ...
        try:
            ...
            response = get_custom_function_templates()
    ...
    if template:
        ...
        # Use the serializer to create the CustomFunction
