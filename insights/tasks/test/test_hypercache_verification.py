...
class TestVerifyAndFixFlagsCacheTask(PushGatewayTaskTestMixin, TestCase):
    ...
    @patch("insights.tasks.hypercache_verification.capture_exception")
    @patch("insights.tasks.hypercache_verification._run_verification_for_cache")
    ...
    def test_soft_time_limit_exceeded_winds_down_without_capturing(
        ...
        success = self.registry.get_sample_value("insights_celery_verify_and_fix_flags_cache_task_success")
        assert success == 1
    ...
    def test_pushgateway_metrics_recorded_on_success(self, mock_run_verification: MagicMock) -> None:
        ...
        success = self.registry.get_sample_value("insights_celery_verify_and_fix_flags_cache_task_success")
        duration = self.registry.get_sample_value("insights_celery_verify_and_fix_flags_cache_task_duration_seconds")
        assert success == 1
...
class TestVerifyAndFixTeamMetadataCacheTask(PushGatewayTaskTestMixin, TestCase):
    ...
    def test_pushgateway_metrics_recorded_on_success(self, mock_run_verification: MagicMock) -> None:
        ...
        success = self.registry.get_sample_value("insights_celery_verify_and_fix_team_metadata_cache_task_success")
        duration = self.registry.get_sample_value(
            "insights_celery_verify_and_fix_team_metadata_cache_task_duration_seconds"
        )
        assert success == 1
        assert duration is not None and duration >= 0
...
class TestVerifyAndFixFlagDefinitionsCacheTask(PushGatewayTaskTestMixin, TestCase):
    ...
    def test_soft_time_limit_exceeded_winds_down_without_capturing(
        ...
        mock_capture.assert_not_called()
        success = self.registry.get_sample_value("insights_celery_verify_and_fix_flag_definitions_cache_task_success")
        assert success == 1
    ...
    def test_releases_lock_after_error(self, mock_run_verification: MagicMock) -> None:
        ...
        lock_key = "insights:hypercache_verification:flag_definitions:lock"
    ...
    def test_pushgateway_metrics_recorded_on_success(self, mock_run_verification: MagicMock) -> None:
        ...
        success = self.registry.get_sample_value("insights_celery_verify_and_fix_flag_definitions_cache_task_success")
        duration = self.registry.get_sample_value(
            "insights_celery_verify_and_fix_flag_definitions_cache_task_duration_seconds"
        )
        assert success == 1
        assert duration is not None and duration >= 0
