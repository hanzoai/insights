...
**Application path (canonical platform fix, via PR to `insights/insights`):** in `products/signals/backend/` (the scout runner / Temporal `custom_prompt` activity), fix turn-end detection so a completed turn closes the run promptly and persists its summary. Add a guard that flushes the captured summary on timeout. This is a runner change, not a per-scout `SKILL.md` edit.
