...
and renders the Depot cost model from ``logic.cost`` as Datastore expressions, so
...
Nothing here is registered as a global InsightsQL view; the view is provisioned per-team as a
...
# ``{"insightsql": <field class>, "datastore": <type>, "valid": True}`` metadata via the same
