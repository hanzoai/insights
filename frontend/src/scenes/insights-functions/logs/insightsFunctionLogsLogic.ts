...
import type { insightsFunctionLogsLogicType } from './insightsFunctionLogsLogicType'
...
const loadClickhouseEvents = async (
    ...
    const query = insightsql`
        ...
        FROM events
        WHERE uuid in (${insightsql.raw(eventIds.map((x) => `'${x}'`).join(','))})
...
export const insightsFunctionLogsLogic = kea<insightsFunctionLogsLogicType>([
    path((key) => ['scenes', 'pipeline', 'customfunctions', 'logs', 'insightsFunctionLogsLogic', key]),
