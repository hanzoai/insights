...
describe('customer analytics action registry', () => {
    ...
    it('wires the Get account node to its script function template', () => {
        const node = getCategory().nodes.find((n) => n.name === 'Get account')
        expect(node).toMatchObject({
            type: 'function',
            config: { template_id: 'template-insights-get-account' },
        })
    })
    ...
    it('wires the Tag account node to its script function template', () => {
        const node = getCategory().nodes.find((n) => n.name === 'Tag account')
        expect(node).toMatchObject({
            type: 'function',
            config: { template_id: 'template-insights-tag-account' },
        })
    })

    it('wires the Update account relationships node to its script function template', () => {
        const node = getCategory().nodes.find((n) => n.name === 'Update account relationships')
        expect(node).toMatchObject({
            type: 'function',
            config: { template_id: 'template-insights-update-account-relationships' },
        })
    })
