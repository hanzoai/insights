    ...
    const columnOptions = selectedTableColumns.map((column) => ({
        ...
        labelComponent: (
            <span className="flex items-center gap-2">
                ...
                <Tag type="muted">{column.type}</Tag>
            </span>
        ),
    }))
    ...
    return (
        <>
            {isGroup && !hasExistingSource && (
                <Field name="groupTypeIndex" label="Group type" help="Which group type this property attaches to.">
                    {({ value, onChange }) => (
                        <Select
                            value={value}
                            onChange={onChange}
                    ...
                    {({ value, onChange }) => (
                        <InputSelect
                            ...
                            options={warehouseTables.map((table) => ({
                                key: table.id,
                                label: table.insightsql_name || table.name,
                            }))}
                ...
                <div key={option.id ?? index} className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                        <Field name={['options', index, 'label']}>
                            <Input placeholder="Option label" fullWidth />
                        </Field>
                    </div>
                    <Button
                        icon={<IconTrash />}
...
export function CustomPropertyModal(): JSX.Element {
    ...
    const canonicalReason = 'Insights sets this property automatically, so its name and type are fixed.'
    ...
    // aren't Fields, so this gates the submit button rather than showing a per-field error.
    ...
    return (
        ...
        >
            ...
            >
                <Field name="name" label="Name" help={isCanonical ? canonicalReason : undefined}>
                    <Input placeholder="e.g. ARR" autoFocus={!isCanonical} disabled={isCanonical} />
                </Field>
                <Field name="description" label="Description">
                    <TextArea placeholder="Optional description" minRows={2} />
                </Field>
                ...
                {targetType === 'account' && (
                    <>
                        <Field name="displayType" label="Type" help={isCanonical ? canonicalReason : undefined}>
                            <Select
                ...
                {targetType === 'account' && (
                    <>
                        <Field name="sourceMode" label="Source">
                            {({ value, onChange }) => (
                                <SegmentedButton
                                    value={value}
                                    onChange={onChange}
