...
def validate_source_prefix(prefix: str | None) -> tuple[bool, str]:
    """
    Validate that prefix will form valid InsightsQL/Datastore identifiers.
...
def remove_named_tuples(type):
    ...
    from products.warehouse_sources.backend.models.table import DATASTORE_INSIGHTSQL_MAPPING
    ...
    while i < len(tokenified_type):
        ...
        if token == "DateTime64" or token == "DateTime32":
            ...
            token == "Nullable" or (len(token) == 1 and not token.isalnum()) or token in DATASTORE_INSIGHTSQL_MAPPING.keys()
        ):
            filtered_tokens.append(token)
        i += 1
...
DATASTORE_INSIGHTSQL_MAPPING: dict[str, DatabaseFieldFactory] = {
...
# Old-style column metadata stores only the Datastore type string and resolves through a
# mapping on every query, so retyping UUID in DATASTORE_INSIGHTSQL_MAPPING would flip every
...
LEGACY_DATASTORE_INSIGHTSQL_MAPPING: dict[str, DatabaseFieldFactory] = {
    **DATASTORE_INSIGHTSQL_MAPPING,
    "UUID": StringDatabaseField,
}

STR_TO_INSIGHTSQL_MAPPING: dict[str, DatabaseFieldFactory] = {
...
DATASTORE_TYPE_TO_INSIGHTSQL_LABEL = {
    "Int8": "integer",
    "Int16": "integer",
    "Int32": "integer",
    "Int64": "integer",
...
def postgres_column_to_dwh_column(_column_name: str, postgres_type: str, nullable: bool) -> dict[str, Any]:
    ...
    if struct_fields is not None:
        struct_datastore_type = f"Tuple({', '.join(str(field['datastore']) for field in struct_fields.values())})"
        if nullable:
            struct_datastore_type = f"Nullable({struct_datastore_type})"

        return {
            "datastore": struct_datastore_type,
    ...
    datastore_type: str | None = POSTGRES_TO_DATASTORE_TYPE.get(normalized_type)

    if datastore_type is None:
        if normalized_type.startswith("timestamp"):
            datastore_type = "DateTime64"
        elif normalized_type.startswith("numeric") or normalized_type.startswith("decimal"):
            datastore_type = "Decimal"
        elif "int" in normalized_type:
            datastore_type = "Int64"
        else:
            datastore_type = "String"

    if nullable:
        datastore_type = f"Nullable({datastore_type})"

    raw_datastore_type = clean_type(datastore_type)
    return {
        "datastore": datastore_type,
        "insightsql": DATASTORE_TYPE_TO_INSIGHTSQL_LABEL.get(raw_datastore_type, "string"),
        "valid": True,
    }
...
def datastore_column_to_dwh_column(_column_name: str, datastore_type: str, nullable: bool) -> dict[str, Any]:
    # The source is already Datastore, so the type string is a valid Datastore type — used verbatim.
    # `system.columns.type` usually already encodes nullability, so only wrap when it doesn't.
    ch_type = datastore_type.strip()
    ...
    if nullable and not already_nullable:
        if ch_type.startswith("LowCardinality(") and ch_type.endswith(")"):
            # LowCardinality must stay the outermost wrapper — Datastore rejects
    ...
    raw_datastore_type = clean_type(ch_type)
    return {
        "datastore": ch_type,
        "insightsql": DATASTORE_TYPE_TO_INSIGHTSQL_LABEL.get(raw_datastore_type, "string"),
        "valid": True,
...
def datastore_columns_to_dwh_columns(columns: list[tuple[str, str, bool]]) -> dict[str, dict[str, Any]]:
    return {
        column_name: datastore_column_to_dwh_column(column_name, datastore_type, nullable)
        for column_name, datastore_type, nullable in columns
    }
