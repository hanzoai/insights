...
mod tests {
    ...
    #[case::both(Some("insights-ios"), Some("3.1.0"), "insights-ios", "3.1.0")]
    ...
    // that matter: replay has no `Insights-Sdk-Info` header, so the user agent is
    ...
    #[case::lib_without_version(json!({"$lib": "insights-ios"}), "whatever", Some("insights-ios"), None)]
    #[case::insights_user_agent_fallback(json!({}), "insights-python/3.0.1", Some("insights-python"), None)]
