...
(`products/review/skills/`) mirrored into per-team `LLMSkill` rows by `lazy_seed.sync_canonical_*`.
Delivery is **pull** — the review / validation prompts instruct the sandbox agent to `skill-get` the
skill body over the Insights MCP — so these loaders only need to pin the current version per skill
(not the body).
"""
...
from products.review.backend.reviewer.models.issues_review import PerspectiveType
...
REVIEW_FN_PERSPECTIVE_PREFIX = "review-script-perspective-"
...
PERSPECTIVES: tuple[tuple[PerspectiveType, str], ...] = (
    (PerspectiveType.LOGIC_CORRECTNESS, f"{REVIEW_FN_PERSPECTIVE_PREFIX}logic-correctness"),
    (PerspectiveType.CONTRACTS_SECURITY, f"{REVIEW_FN_PERSPECTIVE_PREFIX}contracts-security"),
    (PerspectiveType.PERFORMANCE_RELIABILITY, f"{REVIEW_FN_PERSPECTIVE_PREFIX}performance-reliability"),
)
...
def load_perspectives_for_run(team_id: int, acting_user_id: int) -> list[LoadedPerspective]:
    ...
    enabled_names = sorted(
        ...
        .filter(user_id=acting_user_id, enabled=True, skill_name__startswith=REVIEW_FN_PERSPECTIVE_PREFIX)
    ...
    visible = visible_skill_names(
        team_id, acting_user_id, prefix=REVIEW_FN_PERSPECTIVE_PREFIX, canonical_names=CANONICAL_PERSPECTIVE_SKILL_NAMES
    )
...
REVIEW_FN_VALIDATION_PREFIX = "review-script-validation-"
REVIEW_FN_VALIDATION_SKILL_NAME = f"{REVIEW_FN_VALIDATION_PREFIX}criteria"
...
CANONICAL_VALIDATION_SKILL_NAMES: tuple[str, ...] = (REVIEW_FN_VALIDATION_SKILL_NAME,)
...
def load_validation_skill_for_run(team_id: int, acting_user_id: int) -> LoadedValidationSkill:
    ...
    has the canonical selected), then resolves the user's one enabled `review-script-validation-*` row
    ...
    skill_name, version = _load_single_active_skill(
        ...
        canonical_name=REVIEW_FN_VALIDATION_SKILL_NAME,
...
REVIEW_FN_BLIND_SPOTS_PREFIX = "review-script-blind-spots-"
REVIEW_FN_BLIND_SPOTS_SKILL_NAME = f"{REVIEW_FN_BLIND_SPOTS_PREFIX}general"

# Canonical blind-spots names `register_missing_blind_spots_config` auto-enables — one today, kept a
# tuple to mirror the multi-name perspective seed.
CANONICAL_BLIND_SPOTS_SKILL_NAMES: tuple[str, ...] = (REVIEW_FN_BLIND_SPOTS_SKILL_NAME,)
...
def load_blind_spots_skill_for_run(team_id: int, acting_user_id: int) -> LoadedBlindSpotsSkill:
    ...
    canonical selected), then resolves the user's one enabled `review-script-blind-spots-*` row
    (canonical fallback when none is enabled or the selection's row is dead; loud
    ...
    skill_name, version = _load_single_active_skill(
        ...
        canonical_name=REVIEW_FN_BLIND_SPOTS_SKILL_NAME,
...
REVIEW_FN_AUTHORING_PREFIX = "review-script-authoring"
REVIEW_FN_AUTHORING_SKILL_NAME = REVIEW_FN_AUTHORING_PREFIX
