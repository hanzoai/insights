...
Transferring a dashboard between projects deep-clones widget rows. `DashboardWidgetVisitor` (`insights/models/resource_transfer/visitors/dashboard_widget.py`) is `user_facing=True` (default) so widgets appear in transfer preview dependencies with `display_name` from widget `name` when set. `DashboardTile` visitor stays `user_facing=False`. Tests: `insights/api/test/test_resource_transfer.py` (`test_preview_returns_dashboard_with_widget_tiles`).
...
- **Sharing path** — `insights/api/sharing.py` sets `is_shared: True` on dashboard tile serializer context
...
- **Tests** — `insights/api/test/test_sharing.py` (widget tiles in shared payload); `DashboardWidgetItem.test.tsx` (public placeholder)
