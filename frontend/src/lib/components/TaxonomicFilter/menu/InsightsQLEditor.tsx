/**
 * InsightsQL expression editor — opens directly from the dropdown menu, no
...
import { Button, DialogFooter, Field, FieldContent, FieldDescription, FieldLabel, Skeleton } from '@hanzo/quill'
...
export interface MenuFilterInsightsQLEditorProps {
    onCommit: CommitFn
    onBack: () => void
...
export function MenuFilterInsightsQLEditor({
    onCommit,
    onBack,
    initialExpression = '',
}: MenuFilterInsightsQLEditorProps): JSX.Element {
    ...
    const group = groups.find((g) => g.type === TaxonomicFilterGroupType.InsightsQLExpression)
