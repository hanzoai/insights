...
import { Menu, MenuOverlay } from 'lib/elements/Menu/Menu'
            ...
            actions={
                alert ? (
                    <div className="flex items-center gap-2">
                        ...
                        {isSnoozed ? (
                            <Button size="small" type="secondary" onClick={unsnoozeAlert}>
                                Unsnooze
                            </Button>
                        ) : (
                            <Menu
                            ...
                            >
                                <Button size="small" type="secondary" sideIcon={<IconChevronDown />}>
                                    Snooze
                                </Button>
                            </Menu>
                        )}
...
function AlertStatusBand(): JSX.Element | null {
    ...
    const { destinationGroups, existingInsightsFunctionsLoading } = useValues(logsAlertNotificationLogic)
    ...
    if (alert.state === LogsAlertConfigurationStateEnumApi.Broken) {
        banners.push(
            <Banner key="broken" type="error" action={{ children: 'Reset alert', onClick: resetAlert }}>
    ...
    if (alert.state === LogsAlertConfigurationStateEnumApi.Snoozed && alert.snooze_until) {
        banners.push(
            <Banner key="snoozed" type="info" action={{ children: 'Unsnooze', onClick: unsnoozeAlert }}>
    ...
    if (alert.enabled && !existingInsightsFunctionsLoading && destinationGroups.length === 0) {
        banners.push(
            <Banner
                key="no-destinations"
                type="warning"
...
function ConfigurationTab(): JSX.Element {
    ...
    return (
        <Card className="max-w-3xl p-6" hoverEffect={false}>
