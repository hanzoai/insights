"""Old-pipeline snapshot and warmup-probe reads (Datastore) + cohort universe (ORM)."""
...
# string bind leans on Datastore coercing the constant, so the cast keeps the contract visible.
...
# query timeouts; these paged reads stream instead, so the Datastore side stays bounded and only
...
def load_old_membership(team_id: int, cohort_id: int, *, page_size: int = 500_000) -> set[str]:
    """Converged entered-set of one cohort from Datastore cohort_membership (offline host).
