...
export interface alertNotificationLogicActions {
    ...
    loadExistingInsightsFunctions: () => any
...
export const alertNotificationLogic = kea<alertNotificationLogicType>([
    ...
    listeners(({ actions, values, props }) => ({
        ...
        deleteExistingInsightsFunction: async ({ insightsFunction }) => {
            // Resolve the type from the closure up front — after the delete the InsightsFunction is
            // removed from state, so we can't derive it later.
            const type = notificationTypeFromTemplateId(insightsFunction.template_id)
            await deleteWithUndo({
                ...
                callback: (undo) => {
                    ...
                    if (type) {
                        insights.capture('insight alert destination deleted', {
        ...
        createPendingInsightsFunctions: async ({ alertId, alertName }) => {
            ...
            results.forEach((result, i) => {
                if (result.status === 'fulfilled') {
                    insights.capture('insight alert destination created', {
                        alert_id: alertId,
                        type: pending[i].type,
                    })
                }
        ...
        sendTestDeliverySuccess: ({ testDeliveryResult }) => {
            ...
            toast.success(`Test delivery started for ${deliveryTargets.join(' and ')}.`)
        },
        sendTestDeliveryFailure: ({ errorObject }) => {
            const detail = errorObject?.detail
            toast.error(typeof detail === 'string' ? detail : 'Unable to send the test. Try again.')
        },
    })),
