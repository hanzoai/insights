...
call (~cents, no sandbox), sequential.
    ...
    N_SAMPLES=5 OUT_FILE=products/review/eval/experiments/2026-07-oneshot-chunking-dedup/runs/chunker-offline-sample.md \
        python manage.py shell -c "exec(open('products/review/eval/experiments/2026-07-oneshot-chunking-dedup/sample_oneshot_chunker.py').read())"

Gotcha: a Insights Desktop harness shell overrides LLM_GATEWAY_URL to the app's own proxy —
...
from products.review.backend.reviewer.artefact_content import PRSnapshotArtefact, parse_artefact_content
...
OUT_FILE = os.environ.get(
    "OUT_FILE", "products/review/eval/experiments/2026-07-oneshot-chunking-dedup/runs/chunker-offline-sample.md"
)
