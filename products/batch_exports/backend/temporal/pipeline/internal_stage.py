...
class InternalStageResult:
    ...
    stage_folder: str
    # Total rows written to the stage (from Datastore's query summary), or None if unknown.
...
async def compute_num_partitions(
    ...
    """
    logger = LOGGER.bind()
    static_default = settings.BATCH_EXPORT_DATASTORE_S3_PARTITIONS
    ...
    min_partitions = settings.BATCH_EXPORT_DATASTORE_S3_MIN_PARTITIONS
    # guard against misconfiguration where max is set below min
    max_partitions = max(settings.BATCH_EXPORT_DATASTORE_S3_MAX_PARTITIONS, min_partitions)
    # guard against misconfiguration where the target is set to 0
    target_rows = max(settings.BATCH_EXPORT_DATASTORE_S3_TARGET_ROWS_PER_PARTITION, 1)
...
def _get_datastore_s3_staging_folder_url(folder: str) -> str:
    ...
    When running the stack locally, Datastore and the local object storage are both running in Docker so we use the hostname of the
    container.
    """
...
async def _write_batch_export_record_batches_to_internal_stage(
    ...
    async with get_client(
        ...
        # TODO: Strict limits are available in Datastore 26.4
        # We should uncomment all of these here to let max_insert_block_size_bytes dictate the size
        # once datastore is upgraded.
        # use_strict_insert_block_limits=1,
        # max_insert_block_size_rows=0,
        # max_insert_block_size_bytes=settings.BATCH_EXPORTS_DATASTORE_MAX_INSERT_BLOCK_SIZE_BYTES,
        min_insert_block_size_bytes=settings.BATCH_EXPORTS_DATASTORE_MAX_INSERT_BLOCK_SIZE_BYTES,
        ...
        for interval_start, interval_end in generate_query_ranges(full_range, done_ranges):
            ...
            if isinstance(query_or_model, RecordBatchModel):
                ...
                query_settings = query_or_model.get_datastore_request_settings()
            else:
            ...
            try:
                with TRACER.start_as_current_span("batch_export.stage.datastore_query") as query_span:
...
def _written_rows_from_summary(summary: dict[str, typing.Any] | None) -> int | None:
    """Extract `written_rows` from a Datastore query summary (its values are strings)."""
...
async def _execute_query(
    ...
    determined. On the happy path this comes from Datastore's response summary; on the
