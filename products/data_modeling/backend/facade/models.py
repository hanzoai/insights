...
their enums, and the in-module helpers cross-product object-consumers need (InsightsQL/query
builders that traverse the saved-query DAG, dispatch on ``isinstance``, or call model
methods). The models' heavy dependencies (InsightsQL, temporal) are deferred inside their
