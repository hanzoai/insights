...
def _run_query(*, fingerprint_rows, hourly_rows, master_rows, jobs_synced=True, limit=200):
    curated = mock.Mock()
    curated.repository = "Insights/insights"
...
class TestBrokenTestsQueryOverDatastore(DatastoreTestMixin, BaseTest):
    """The unit tests above mock curated.run, so the fingerprints/hourly SQL never actually runs.
    This drives query_broken_tests through real InsightsQL over seeded logs — the only place a SQL fault
    ...
    def _source(self) -> CuratedGitHubSource:
        ...
        return CuratedGitHubSource(
            team=self.team,
            tables=GitHubTables(
                pull_requests="unused", workflow_runs="unused", workflow_jobs=None, repository="Insights/insights"
            ),
        )
    ...
    def test_fingerprints_query_runs_and_filters_by_repo(self) -> None:
        self._insert_logs(
            [
                self._failure_log(repo="Insights/insights", test_id="insights/api/test_foo.py::test_bar"),
        ...
        assert [row.test_id for row in result.rows] == ["insights/api/test_foo.py::test_bar"]
        assert result.rows[0].repo == "Insights/insights"
