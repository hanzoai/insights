...
export interface logsAlertDetailSceneLogicValues {
    alertForm: LogsAlertFormType // logsAlertFormLogic
    alertFormChanged: boolean // logsAlertFormLogic
    currentTeamId: number | null // teamLogic
    activeTab: LogsAlertDetailTab
    alert: LogsAlertConfigurationApi | null
    alertId: string
    alertLoading: boolean
    breadcrumbs: Breadcrumb[]
    isDraft: boolean
    logsViewerUrl: string
    pendingEnableAfterSave: boolean
    sparkline7d: any[]
    sparkline7dFetched: boolean
    sparkline7dLoading: boolean
    sparkline7dSeries: {
        labels: string[]
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface logsAlertDetailSceneLogicActions {
    loadEvents: () => any // logsAlertEventHistoryLogic
    resetAlertForm: (values?: LogsAlertFormType | undefined) => {
    ...
    submitAlertFormSuccess: (alertForm: LogsAlertFormType) => {
    ...
    patchAlertLocally: (patch: Partial<LogsAlertConfigurationApi>) => {
        patch: Partial<LogsAlertConfigurationApi>
    }
    resetAlert: () => {
        value: true
    }
    setActiveTab: (tab: LogsAlertDetailTab) => {
        tab: LogsAlertDetailTab
    }
    snoozeAlert: (durationMinutes: number) => {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface logsAlertDetailSceneLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        alertId: (arg: any) => string
        isDraft: (alert: LogsAlertConfigurationApi | null) => boolean
        breadcrumbs: (alert: LogsAlertConfigurationApi | null) => Breadcrumb[]
        sparkline7dSeries: (sparkline7d: any[]) => {
            labels: string[]
            series: SparklineTimeSeries[]
        }
        logsViewerUrl: (alert: LogsAlertConfigurationApi | null) => string
    }
}

export type logsAlertDetailSceneLogicType = MakeLogicType<
    logsAlertDetailSceneLogicValues,
    logsAlertDetailSceneLogicActions,
    LogsAlertDetailSceneLogicProps,
    logsAlertDetailSceneLogicMeta
>
    ...
    listeners(({ actions, values, props }) => ({
        ...
        applyEnabledChange: async ({ enabled }) => {
            try {
                ...
                toast.error(e?.detail ?? e?.message ?? 'Failed to update alert')
            }
        },
        ...
        resetAlert: async () => {
            try {
                ...
                toast.success('Alert reset — next check will run shortly.')
                actions.loadAlert()
            } catch {
