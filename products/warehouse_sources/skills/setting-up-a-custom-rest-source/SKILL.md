...
description: >
  Connect an arbitrary REST API to the Insights data warehouse as a Custom source by authoring a JSON manifest, with no
  per-source code. Use when the user points at an API that has no built-in Insights connector — "import data from this
  ...
  creating the source. If the API already has a native Insights connector, use
...
per-source Python. The manifest is a `RESTAPIConfig`: the same shape that powers Insights's built-in REST connectors
...
A Custom source is the **fallback** — only correct when no native connector fits. Insights ships **hundreds** of native
...
engine injects it at run time, and Insights redacts it from every response. Putting a token inline is rejected at
validation.
...
schemas shape). Pick a short lowercase `prefix` — tables become `{prefix}_{resource_name}` in InsightsQL.
