...
class TestRedisUrlRouting(BaseTest):
    ...
    def test_get_cache_stats_uses_config_redis_url(self, mock_get_client):
        ...
        with patch.object(config.hypercache, "redis_url", "redis://dedicated:6379/1"):
            with patch("insights.models.team.team.Team.objects.count", return_value=10):
...
class TestPushHypercacheStatsMetrics(BaseTest):
    ...
    @patch("insights.storage.hypercache_manager.pushed_metrics_registry")
    def test_pushes_metrics_to_pushgateway(self, mock_registry_cm):
        ...
        assert registry.get_sample_value("insights_hypercache_coverage_percent", labels) == 85.5
        assert registry.get_sample_value("insights_hypercache_entries_total", labels) == 1000
        assert registry.get_sample_value("insights_hypercache_expiry_tracked_total", labels) == 950
        assert registry.get_sample_value("insights_hypercache_size_bytes", labels) == 1024000
    ...
    def test_skips_size_gauge_when_size_bytes_is_none(self, mock_registry_cm):
        ...
        assert registry.get_sample_value("insights_hypercache_size_bytes", labels) is None
        assert registry.get_sample_value("insights_hypercache_coverage_percent", labels) == 90.0
        assert registry.get_sample_value("insights_hypercache_entries_total", labels) == 500
        assert registry.get_sample_value("insights_hypercache_expiry_tracked_total", labels) == 500

    @patch("insights.storage.hypercache_manager.pushed_metrics_registry")
    @patch("insights.storage.hypercache_manager.logger")
...
class TestPushHypercacheTeamsProcessedMetrics(BaseTest):
    ...
    def test_pushes_metrics_to_pushgateway(self, mock_registry_cm):
        ...
        assert (
            registry.get_sample_value("insights_hypercache_teams_processed_last_run", {**base, "result": "success"})
            == 900
        )
        assert (
            registry.get_sample_value("insights_hypercache_teams_processed_last_run", {**base, "result": "failure"})
            == 100
        )
