...
from insights.cdp.services.icons import CDPIconsService, LogoDevBadGateway, LogoDevUnavailable
from insights.egress.limiter.outbound import get_outbound_rate_limiter
...
class TestCDPIconsService(SimpleTestCase):
    ...
    def test_icon_bytes_are_never_stored_and_browser_caching_is_directed(self) -> None:
        ...
        with (
            patch("insights.egress.logodev.transport.consume_logodev_sync", return_value=True),
            patch(
                "requests.request",
                side_effect=[
    ...
    def test_icon_fetch_reports_unavailable_when_budget_exhausted(self) -> None:
        ...
        with (
            patch("insights.egress.logodev.transport.consume_logodev_sync", return_value=False),
            patch("requests.request") as upstream,
        ):
    ...
    def test_icon_fetch_reports_unavailable_when_team_budget_exhausted(self) -> None:
        ...
        with (
            patch("insights.cdp.services.icons.get_outbound_rate_limiter") as limiter,
            patch("requests.request") as upstream,
        ):
