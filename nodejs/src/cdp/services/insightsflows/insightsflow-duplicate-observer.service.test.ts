...
const buildInvocation = (overrides: { id?: string; functionId?: string; eventUuid?: string | null } = {}) =>
    ...
    }) as unknown as CyclotronJobInvocationInsightsFlow
...
describe('InsightsFlowDuplicateObserverService', () => {
    jest.retryTimes(3)
    ...
    it('is a no-op when redis is null', async () => {
        const observer = new InsightsFlowDuplicateObserverService(null)
    ...
    it('is a no-op when invocation has no eventUuid', async () => {
        const observer = new InsightsFlowDuplicateObserverService(redis)
    ...
    it('writes to the mirror in parallel with the primary', async () => {
        ...
        const observer = new InsightsFlowDuplicateObserverService(redis, mirror)
        await observer.observe(buildInvocation({ id: 'inv-A' }), buildAction('action-1'))
