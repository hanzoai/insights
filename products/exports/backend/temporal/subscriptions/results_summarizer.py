...
from insights.security.llm_prompt_sanitization import GENERIC_VALUE_MAX_LEN, SERIES_LABEL_MAX_LEN, sanitize_user_text
...
def _summarize_generic(results: list[Any], columns: list[str] | None = None) -> str:
    ...
    - dict rows (most Insights query results): skip known noisy keys, join the rest.
    - list/tuple rows (InsightsQL / DataVisualizationNode): label each value with the
