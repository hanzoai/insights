...
from insights.temporal.ai.anomaly_investigation.prompts import SYSTEM_PROMPT
from insights.temporal.ai.anomaly_investigation.report import InvestigationReport, salvage_report
from insights.temporal.ai.anomaly_investigation.tools import (
...
async def run_investigation(
    *,
    ...
    handlers: dict[str, ToolHandler] = {
        "run_insightsql_query": lambda raw: toolkit.run_insightsql_query(RunInsightsQLQueryArgs.model_validate(raw)),
    ...
    tools_spec: list[tuple[str, str, type[BaseModel]]] = [
        (
            "run_insightsql_query",
            "Run a read-only InsightsQL SELECT query against the team's event data. Use sparingly and keep queries narrow.",
            RunInsightsQLQueryArgs,
        ),
        (
            "top_breakdowns",
        ...
        (
            ...
            (
                ...
                "interval. Prefer this over run_insightsql_query when you need the exact metric the "
    ...
    llm = MaxChatAnthropic(
        ...
        insights_properties={"ai_product": "alert_investigation_agent"},
    )
