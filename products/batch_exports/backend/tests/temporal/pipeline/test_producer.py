...
from insights.temporal.common import asyncpa
...
async def test_producer_resumes_from_offset_after_mid_file_failure(
    ...
    # Datastore writes one Arrow record batch per source block, so several separate inserts (plus a
    ...
    with override_settings(
        ...
        BATCH_EXPORT_DATASTORE_S3_PARTITIONS=1,
        BATCH_EXPORTS_DATASTORE_MAX_INSERT_BLOCK_SIZE_BYTES=1,
    ):
