...
from insights.exceptions_capture import bind_exception_context
...
async def test_exception_capture(fail: bool, capture_additional_properties: bool, temporal_client: Client):
    ...
    with (
        patch("insights.temporal.common.insights_client.capture_exception") as mock_ph_capture,
    ):
        async with Worker(
            temporal_client,
...
async def test_ambient_exception_context_reaches_top_level_capture(temporal_client: Client):
    ...
    with patch("insights.temporal.common.insights_client.capture_exception") as mock_ph_capture:
        async with Worker(
            temporal_client,
            task_queue=task_queue,
