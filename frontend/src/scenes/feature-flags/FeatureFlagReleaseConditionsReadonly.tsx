...
import { IconOpenInNew, IconSubArrowRight } from 'lib/elements/icons'
...
function PropertyValueDisplay({
    ...
    if (property.type === PropertyFilterType.Cohort) {
        return (
            <Button type="secondary" size="xsmall" to={urls.cohort(property.value)} sideIcon={<IconOpenInNew />}>
    ...
    return (
        <>
            {propertyValues.map((val, idx) => {
                ...
                return <Snack key={idx}>{display}</Snack>
            })}
        </>
    )
}
...
function PropertyFilterRow({
    ...
    return (
        <div className="flex items-center gap-1.5 flex-wrap text-sm">
            {isFirst ? (
                <Button icon={<IconSubArrowRight className="arrow-right" />} size="small" noPadding />
            ) : (
                <Button icon={<span className="text-xs font-medium">&</span>} size="small" noPadding />
            )}
            ...
            {property.type === PropertyFilterType.Flag ? (
                ...
                <Snack>{property.type === PropertyFilterType.Cohort ? 'Cohort' : property.key}</Snack>
            )}
...
function ConditionSetCard({
    ...
    return (
        <div className="border rounded p-4 bg-surface-primary">
            ...
            <div className="mt-3">
                <Tag type={rollout === 100 ? 'highlight' : rollout === 0 ? 'caution' : 'none'}>
