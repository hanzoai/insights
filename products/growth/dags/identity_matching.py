...
The job persists nothing on the Datastore cluster: each stage writes its output as Parquet
...
Environment restrictions: the job processes internal Insights data that only exists on
...
# (insights/taxonomy/taxonomy.py) excluding organic/email params like igshid or mc_cid.
...
# Double backslash: Datastore string literals unescape \\ to \, yielding the regex wv\).
...
# Internal-only data-safety guardrails: the training data lives in the Insights Cloud US
...
def validate_team_allowed(team_id: int) -> None:
    if settings.CLOUD_DEPLOYMENT == "EU":
        raise dagster.Failure("Identity matching does not run on Insights Cloud EU")
    if settings.CLOUD_DEPLOYMENT == "US" and team_id not in PROD_US_ALLOWED_TEAM_IDS:
        raise dagster.Failure(
            f"On Insights Cloud US identity matching may only process teams "
...
class IdentityMatchingConfig(dagster.Config):
    ...
    query_max_execution_time_seconds: int = pydantic.Field(
        default=60 * 60,
        gt=0,
        description="Datastore max_execution_time per query — ceiling that aborts a runaway query.",
    )
    query_max_memory_usage_gb: int = pydantic.Field(
        default=50,
        gt=0,
        description="Datastore max_memory_usage per query, in GB — per-host cap that guards against OOM.",
    )
    ...
    def query_guards(self) -> dict[str, str]:
        """Datastore per-query guard settings derived from the run config (see _base_settings)."""
...
# VALUES. datastore_driver's native-block insert path (`INSERT ... VALUES` with a list of tuples
...
def build_device_day_index(
    ...
    query = f"""
        ...
        SELECT
            ...
            min(timestamp),
            max(timestamp)
        FROM {settings.DATASTORE_DATABASE}.events
