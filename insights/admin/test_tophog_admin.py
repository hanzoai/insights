...
from insights.admin.admins.event_ingestion_restriction_config import (
...
class TestTopHogAdminHelpers(BaseTest):
    ...
    def test_create_restriction_url_prefills_add_form_from_key(self):
        ...
        parsed = urlparse(url)
        self.assertEqual(parsed.path, reverse("admin:insights_eventingestionrestrictionconfig_add"))
        self.assertEqual(
            parse_qs(parsed.query),
            {
                "token": ["phc_abc"],
