"""insightscli devex:feedback — send feedback about the dev experience to the devex team.

Feedback lands as a ``insightscli_feedback`` event in Insights's internal insightscli project,
carrying the same environment context that insightscli telemetry attaches (environment,
...
_EVENT = "insightscli_feedback"
_DEFAULT_HOST = "https://us.i.hanzo.ai"
...
def _context_properties() -> dict[str, Any]:
    ...
    insightscli's regular events, plus a few static platform props.
    """
    props: dict[str, Any] = {
        "os": platform.system(),
...
def devex_feedback(message: tuple[str, ...], category: str | None, yes: bool) -> None:
    """Send feedback about the dev experience to the Insights devex team.

    Humans and agents alike: when a insightscli command, the dev stack, or any repo
    tooling is slow, broken, confusing, or delightful, say so here. It lands as
    a `insightscli_feedback` event the devex team triages, tagged with your
    environment, branch, and whether an agent sent it.

    \b
        insightscli devex:feedback "dev:reset took 20 min and the schema restore silently failed"
        insightscli devex:feedback -c idea "add a insightscli db:snapshot to save/restore local state"
        echo "long note" | insightscli devex:feedback

    The message is the only free-form content sent; everything else is the same
    environment context insightscli telemetry already attaches. Run interactively to
    ...
    # like any Unix filter) so delayed producers such as `… | insightscli devex:feedback` work.
    ...
    if not text:
        raise click.ClickException('nothing to send — provide a message: insightscli devex:feedback "..."')
    ...
    if interactive and not yes:
        ...
        click.secho(
            "\nTagged with the same environment context insightscli telemetry attaches (environment, agent, branch, repo).\n",
            dim=True,
        )
