...
class TestUuidTimestampBounds(BaseTest):
    def _print(self, select: str, dialect: InsightsQLDialect = "datastore") -> str:
        query, _ = prepare_and_print_ast(
            parse_select(select),
            InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, modifiers=InsightsQLQueryModifiers()),
            dialect,
        )
        return query
    ...
    def test_insightsql_dialect_is_not_rewritten(self) -> None:
        printed = self._print(f"SELECT event FROM events WHERE uuid = '{UUID_LATE}'", dialect="insightsql")
        assert "greaterOrEquals" not in printed
