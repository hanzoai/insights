...
from insights.insightsql.observability import record_type_simplification
...
def simplify_argmax_over_non_nullable(node: _T_AST, context: InsightsQLContext) -> _T_AST:
    ...
    latest (max-version) row still wins — plain Datastore ``argMax`` skips NULLs and would return a
...
class TypeAwareSimplifier(CloningVisitor):
    def __init__(self, context: InsightsQLContext, dialect: InsightsQLDialect):
        super().__init__(clear_types=False)
        self.context = context
...
def _constant_type(node: ast.Expr, context: InsightsQLContext) -> ast.ConstantType | None:
...
def _fold_numeric_arithmetic_operation(node: ast.ArithmeticOperation, context: InsightsQLContext) -> ast.Constant | None:
...
def _fold_constant_conversion_call(node: ast.Call, dialect: InsightsQLDialect) -> ast.Constant | None:
    normalized_name = node.name.lower()
...
def _fold_literal_json_call(node: ast.Call, dialect: InsightsQLDialect) -> ast.Constant | None:
    normalized_name = node.name.lower()
