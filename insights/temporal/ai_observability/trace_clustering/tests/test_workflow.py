...
from insights.temporal.ai_observability.trace_clustering.clustering import perform_kmeans_with_optimal_k
from insights.temporal.ai_observability.trace_clustering.constants import DEFAULT_MAX_SAMPLES
from insights.temporal.ai_observability.trace_clustering.models import (
...
from insights.temporal.ai_observability.trace_clustering.workflow import DailyTraceClusteringWorkflow
...
class TestComputeActivityCapacityBackoff:
    """The compute activity must not let a Datastore capacity error retry immediately.

    Guards the incident fix: on `DatastoreAtCapacity` the activity backs off with jitter
    ...
    @pytest.mark.asyncio
    async def test_datastore_at_capacity_raises_jittered_backoff(self):
        ...
        from insights.temporal.ai_observability.trace_clustering import constants
        from insights.temporal.ai_observability.trace_clustering.activities import perform_clustering_compute_activity
        ...
        with patch(
            "insights.temporal.ai_observability.trace_clustering.activities._perform_clustering_compute",
        ...
        err = exc_info.value
        assert err.type == "DatastoreAtCapacity"
        assert err.non_retryable is False
        assert err.next_retry_delay is not None
        assert (
            constants.DATASTORE_AT_CAPACITY_BACKOFF_MIN
            <= err.next_retry_delay
            <= constants.DATASTORE_AT_CAPACITY_BACKOFF_MAX
        )
