...
import { Tag, TagProps, Tooltip } from '@hanzo/elements'
...
const SENTIMENT_TAG_TYPE: Record<SentimentLabel, TagProps['type']> = {
    positive: 'success',
    negative: 'danger',
    neutral: 'none',
}
...
export function SentimentTag({ label, score, scores, loading }: SentimentTagProps): JSX.Element {
    ...
    return (
        <Tooltip title={<span className="whitespace-pre-line">{buildTagTooltip(label, scores)}</span>}>
            <Tag type={tagType} size="small" className="LLMASentimentTag cursor-default capitalize">
...
export function SentimentBar({ label, score, loading, size = 'sm', messages }: SentimentBarProps): JSX.Element | null {
    if (loading) {
        return <Skeleton className={`h-1.5 ${size === 'full' ? 'w-3/4' : 'w-10'}`} />
    }
