...
describe('tileMetricExpression', () => {
    // Each column-aggregation metric type must emit its own InsightsQL aggregate function
    ...
    ])('produces the right InsightsQL fragment for %o', (metric, expected) => {
...
describe('accountsOverviewTilesLogic legacy localStorage tiles (read-only + tombstone)', () => {
    ...
    beforeEach(() => {
        localStorage.clear()
        initKeaTests()
        jest.spyOn(insights, 'capture').mockImplementation(() => undefined as any)
    })
    ...
    it('reads a pre-existing custom value on mount, emits a tombstone, and never writes it back', async () => {
        ...
        expect(insights.capture).toHaveBeenCalledWith(AccountsEvents.OverviewTilesLocalStorageRead, { tile_count: 1 })
    ...
    it('ignores a default-valued legacy key — no seed, no tombstone', async () => {
        ...
        expect(insights.capture).not.toHaveBeenCalledWith(
