...
import { Task, TaskRunStatus } from 'products/insights_ai/frontend/types/taskTypes'
import {
...
export const inboxReportDetailLogic = kea<inboxReportDetailLogicType>([
    ...
    selectors({
        ...
        // is `github_login`, NOT `account.name` (which is the installation's org/user, e.g. "Insights").
    ...
    listeners(({ actions, values, cache, props }) => ({
        ...
        updateReviewers: async ({ content }) => {
            try {
                ...
                toast.error(error?.detail || error?.message || 'Failed to update reviewers')
            } finally {
        ...
        postReviewComment: async ({ payload }) => {
            ...
            try {
                ...
                toast.error(reviewCommentError(error, "Couldn't post the comment to GitHub"))
            } finally {
        ...
        editReviewComment: async ({ commentId, body }) => {
            ...
            try {
                ...
                toast.error(reviewCommentError(error, "Couldn't save the edit"))
            }
        },
        ...
        deleteReviewComment: async ({ commentId }) => {
            ...
            try {
                ...
                toast.error(reviewCommentError(error, "Couldn't delete the comment"))
            }
        },
        ...
        toggleReviewCommentReaction: async ({ commentId, content }) => {
            ...
            if (mine) {
                ...
                try {
                    ...
                    toast.error(reviewCommentError(error, "Couldn't remove the reaction"))
                }
                return
            }
            ...
            try {
                ...
                toast.error(reviewCommentError(error, "Couldn't add the reaction"))
            }
        },
