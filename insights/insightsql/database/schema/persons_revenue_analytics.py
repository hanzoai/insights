...
from insights.insightsql.database.schema.util.revenue_analytics import get_table_kind, is_event_view
from insights.insightsql.errors import ResolutionError

from insights.exchange_rate_constants import EXCHANGE_RATE_DECIMAL_PRECISION
...
def _select_from_persons_revenue_analytics_table(context: InsightsQLContext) -> ast.SelectQuery | ast.SelectSetQuery:
    ...
    for views in all_views.values():
        ...
        if person_id_chain is not None:
            ...
            query = ast.SelectQuery(
                select=[
                    # `team_id` is required to make InsightsQL happy and edge-case free
