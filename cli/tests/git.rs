use insights_cli::utils::git::{get_git_info, get_git_info_from_env, get_remote_url, get_repo_name};
...
[remote "origin"]
    url = https://github.com/Insights/insights.git
...
fn assert_insights_git_info(worktree_dir: PathBuf, branch: &str, commit_id: &str) {
    ...
    assert_eq!(
        ...
        Some("https://github.com/Insights/insights.git")
    );
...
fn test_get_commit_sha_from_loose_ref() {
    ...
    let temp_root =
        std::env::temp_dir().join(format!("insights_cli_loose_ref_test_{}", Uuid::now_v7()));
    let git_dir = temp_root.join(".git");
    let branch = "main";
...
fn test_get_commit_sha_loose_ref_takes_precedence_over_packed_refs() {
    ...
    let temp_root = std::env::temp_dir().join(format!(
        "insights_cli_ref_precedence_test_{}",
        Uuid::now_v7()
    ));
    let git_dir = temp_root.join(".git");
    let branch = "main";
...
[core]
    repositoryformatversion = 0
[remote "origin"]
    url = https://github.com/Insights/insights
    fetch = +refs/heads/*:refs/remotes/origin/*
"#;
    let git_dir = make_git_dir_with_config(cfg);
    assert_eq!(
...
[core]
    repositoryformatversion = 0
[remote "origin"]
    url = git@github.com:Insights/insights.git
    fetch = +refs/heads/*:refs/remotes/origin/*
"#;
    let git_dir = make_git_dir_with_config(cfg);
    assert_eq!(
        get_remote_url(&git_dir).as_deref(),
        Some("git@github.com:Insights/insights.git")
    );
    assert_eq!(get_repo_name(&git_dir).as_deref(), Some("insights"));
    let _ = fs::remove_dir_all(git_dir.parent().unwrap());
}
...
[core]
    repositoryformatversion = 0
[remote "origin"]
    url = git@github.com:Insights/insights
    fetch = +refs/heads/*:refs/remotes/origin/*
"#;
    let git_dir = make_git_dir_with_config(cfg);
    assert_eq!(
...
fn test_get_git_info_from_regular_repo_packed_refs() {
    ...
    let temp_root =
        std::env::temp_dir().join(format!("insights_cli_regular_repo_test_{}", Uuid::now_v7()));
    let git_dir = temp_root.join(".git");
    ...
    assert_insights_git_info(temp_root.clone(), branch, commit_id);
...
fn test_get_git_info_from_worktree_git_file_cases() {
    ...
    for (case_name, absolute_gitdir, commondir, use_packed_ref) in [
        ...
        assert_insights_git_info(worktree_dir, &branch, commit_id);
...
fn test_get_git_info_from_worktree_config_precedence() {
    ...
    let temp_root = std::env::temp_dir().join(format!(
        "insights_cli_worktree_config_test_{}",
        Uuid::now_v7()
    ));
    let worktree_dir = temp_root.join("worktree");
    let common_git_dir = temp_root.join("repo.git");
    ...
    fs::write(
        ...
        r#"
[remote "origin"]
    url = https://github.com/Insights/worktree-override.git
"#,
    )
    ...
    assert_eq!(
        info.remote_url.as_deref(),
        Some("https://github.com/Insights/worktree-override.git")
    );
...
fn test_get_git_info_from_vercel_env() {
    let info = get_git_info_from_env(env_from([
        ...
        ("VERCEL_GIT_REPO_OWNER", "Insights"),
        ("VERCEL_GIT_REPO_SLUG", "insights"),
