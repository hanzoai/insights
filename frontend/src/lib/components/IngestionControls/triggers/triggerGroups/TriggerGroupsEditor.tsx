    ...
    return (
        <div className="border rounded overflow-hidden">
            <div className="flex items-center gap-2 p-2 pl-3">
                ...
                <Button size="xsmall" status="danger" onClick={onRemove}>
                    Remove
                </Button>
            </div>
            {isExpanded && (
        ...
        >
            <div className="border rounded p-4 bg-surface-primary space-y-4">
                <Field name="name" label="Group name">
                    <Input placeholder="e.g., Error Tracking, Feature Testing" fullWidth />
                </Field>

                <div className="flex gap-4">
                    <div className="flex-1">
                        <Field name="sampleRate" label="Sample rate (%)">
                            <Input type="number" min={0} max={100} fullWidth />
                        </Field>
                    </div>

                    <div className="flex-1">
                        <Field name="minDurationMs" label="Minimum duration (seconds)">
                            <Select options={SESSION_REPLAY_MINIMUM_DURATION_OPTIONS} fullWidth />
                        </Field>
                    </div>
                </div>

                <Field name="matchType" label="Match type">
                    <Select
                        options={[
                    ...
                    <div className="mb-4">
                        ...
                        {triggerGroup.urls.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-2">
                                {triggerGroup.urls.map((urlConfig) => (
                                    <Snack key={urlConfig.url} onClose={() => removeUrl(urlConfig.url)}>
                                        {urlConfig.url}
                                    </Snack>
                                ))}
                            </div>
                        ...
                        {isAddingUrl && (
                            <div className="border rounded p-3 bg-bg-3000 mb-2">
                                <Banner type="info" className="text-sm mb-2">
                                ...
                                <div className="flex gap-2 mt-1">
                                    <Input
                                        ...
                                        autoFocus
                                    />
                                    <Button type="secondary" onClick={() => setIsAddingUrl(false)}>
                                        Cancel
                                    </Button>
                                    <Button type="primary" onClick={() => addUrl(newUrl)}>
                                        Save
                                    </Button>
                                </div>
                                {triggerGroup.urls.length > 0 && (
                    ...
                    <div className="mb-4">
                        ...
                        {triggerGroup.flag && (
                            <div className="flex flex-wrap gap-2">
                                <Snack onClose={removeFlag}>{triggerGroup.flag}</Snack>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex justify-end gap-2 pt-2 border-t">
                    <Button type="secondary" onClick={onCancel} data-attr="trigger-group-cancel">
                        Cancel
                    </Button>
                    <Button
                        type="primary"
...
function DeleteLastGroupModal({ isOpen, onClose, onConfirm, groupName }: DeleteLastGroupModalProps): JSX.Element {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Delete last trigger group?">
            <div className="space-y-4">
                <p>
