...
from insights.schema import EventsQuery, InsightsQLQueryModifiers
...
# Columns the events list/retrieve endpoints read. The order is the contract for zipping InsightsQL
# result rows back into the dict shape `DatastoreEventSerializer` expects. Deliberately NOT "*":
...
def _execute_events_list_query(runner: EventsQueryRunner, database: Database) -> tuple[builtins.list[dict], bool]:
    """Execute the events-list InsightsQL query and return ``(page_rows, has_more)``.

    Split out from `LegacyEventsListQuery.run_page` so the progressive-window probing can be tested
    against the real window/date logic while stubbing only the Datastore round-trip. The pre-built
    `database` is handed to the executor so it isn't rebuilt per probe.
    """
    runner.paginator.execute_insightsql_query(
        runner.to_query(),
        query_type="events_list",
        team=runner.team,
        workload=Workload.OFFLINE,
        settings=InsightsQLGlobalSettings(max_threads=settings.DATASTORE_EVENT_LIST_MAX_THREADS),
        modifiers=runner.modifiers,
        timings=runner.timings,
        user=runner.user,
        context=InsightsQLContext(team=runner.team, database=database, enable_select_queries=True),
    )
...
def get_one_event(team: Team, pk: str) -> Optional[dict]:
    ...
    result = execute_insightsql_query(query, team=team, query_type="event_detail", modifiers=modifiers)
...
class LegacyEventsListQuery:
    """Backing query for the deprecated events list REST API (`insights/api/event.py`).

    Translates the endpoint's loose query params into a typed InsightsQL `EventsQuery` and runs it
    through `EventsQueryRunner`, owning the progressive-window probing and its result-count cache.
    The InsightsQL schema (database + modifiers) is built once here and reused across every window probe.
    ...
    def __init__(self, team: Team, user: Optional[User]) -> None:
        self.team = team
        self.user = user
        self.modifiers: InsightsQLQueryModifiers = create_default_modifiers_for_team(team)
    ...
    def run_page(
        ...
        *,
        ...
        # PATCH_EVENT_LIST_MAX_OFFSET clamps this deprecated endpoint's Datastore cost as a graduated
