...
class BingAdsSource(ResumableSource[BingAdsSourceConfig, BingAdsResumeConfig], OAuthMixin):
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        ...
        auth_friendly = (
            "Insights could not authenticate with Bing Ads. The connected account's OAuth credentials "
        ...
        service_principal_friendly = (
            "Your Microsoft tenant has not consented to Insights's Bing Ads connector "
        ...
        return {
            ...
            # Insights's own Azure AD application secret (BING_ADS_CLIENT_SECRET) is invalid or expired —
            # Microsoft rejects the token request with AADSTS7000215 for the app itself, not the connected
            # account. Reconnecting the integration can't fix it; only rotating Insights's app secret can, so
            ...
            "Invalid client data": (
                "Insights could not use the connected Bing Ads account (Microsoft returned 'Invalid client data'). "
            ...
            # Insights's Bing Ads OAuth application credentials aren't configured — an empty client_id makes
