...
import { CustomFunctionTemplateListLogicProps, customFunctionTemplateListLogic } from './customFunctionTemplateListLogic'

export function CustomFunctionTemplateList({
    extraControls,
    hideFeedback = false,
    ...props
}: CustomFunctionTemplateListLogicProps & { extraControls?: JSX.Element; hideFeedback?: boolean }): JSX.Element {
    const { loading, filteredTemplates, filters, templates, urlForTemplate } = useValues(
        customFunctionTemplateListLogic(props)
    )
    const { loadCustomFunctionTemplates, setFilters, resetFilters, registerInterest } = useActions(
        customFunctionTemplateListLogic(props)
    )
    const { openFeedbackDialog } = useActions(customFunctionRequestModalLogic)

    useEffect(() => loadCustomFunctionTemplates(), [props.type]) // oxlint-disable-line exhaustive-deps
