...
WIZARD_PACKAGE = "@hanzo/wizard@latest"
...
WIZARD_VERBOSE_LOG_PATH = "/tmp/insights-wizard.log"
...
def _format_wizard_output(result: ExecutionResult) -> str:
    sections = [f"Insights setup wizard output (exit code {result.exit_code})"]
    if result.stdout:
...
def run_wizard(input: RunWizardInput) -> None:
    """Run the Insights setup wizard in the sandbox before the agent starts.

    The wizard performs the real Insights integration (modifies source, installs deps, writes
    insights-setup-report.md), leaving the changes uncommitted in the working tree. The downstream
    ...
    with log_activity_execution(
        ...
        **ctx.to_log_context(),
        ...
        emit_agent_log(ctx.run_id, "info", "Running the Insights setup wizard")
        ...
        if result.exit_code == WIZARD_TIMEOUT_EXIT_CODE:
            ...
            emit_agent_log(ctx.run_id, "error", f"Insights setup wizard timed out after {minutes} minutes")
            raise RuntimeError(f"Insights setup wizard timed out after {minutes} minutes")
        if result.exit_code != 0:
            ...
            emit_agent_log(ctx.run_id, "error", f"Insights setup wizard failed (exit {result.exit_code}): {detail}")
            raise RuntimeError(f"Insights setup wizard failed (exit {result.exit_code}): {detail}")

        emit_agent_log(ctx.run_id, "info", "Insights setup wizard completed")
