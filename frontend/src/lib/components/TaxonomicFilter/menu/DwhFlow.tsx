 ...
 *   - InsightsQL fallback (lazy Monaco) when the active field has
 *     `allowInsightsQL: true` and the chosen value isn't a real column.
...
export interface MenuFilterDwhConfigProps {
    ...
    /** When supplied, the aggregation-target tab reads `funnelDataLogic` for context (group vs person, InsightsQL aggregation). */
...
export function MenuFilterDwhConfig({
    ...
    // InsightsQL mode — true whenever the active value isn't one of the field's
    ...
    // a disallowed type for this field counts as InsightsQL, matching the
    // dropdown.
    const activeFieldIsInsightsQL = useMemo(
        () => !!activeField?.allowInsightsQL && !activeFieldOptions.some((c) => c.name === activeFieldValue),
    ...
    const previewExpressionColumns: TablePreviewExpressionColumn[] = useMemo(() => {
        ...
        for (const f of dataWarehousePopoverFields) {
            ...
            const keyBase = `__${f.key}_insightsql_expression`
            let key = keyBase
            let suffix = 2
    ...
    const previewSelectedKey = useMemo(() => {
        const expr = previewExpressionColumns.find((c) => c.key.startsWith(`__${activeFieldKey}_insightsql_expression`))
    ...
    return (
        ...
        >
            <DialogContent
                ...
                // Table preview gets horizontal breathing room.
                ...
                <DialogBody className="w-full overflow-auto">
                    <div className="flex flex-col gap-4 min-w-0">
                        ...
                        {/* `data-not-quill` resets the colour-token rebinds
                            ...
                            Insights colours instead of inheriting Quill's. */}
                        ...
                        {activeField && (
                            <ActiveFieldEditor
                                ...
                                isInsightsQL={activeFieldIsInsightsQL}
...
/**
 * Active-field editor: column dropdown + optional InsightsQL fallback. Split
        ...
        <Field>
            ...
            <FieldContent className="gap-3">
                ...
                {field.allowInsightsQL &&
                    isInsightsQL && (
                        // Render the rich `InsightsQLEditor` inline (full width) —
                        // skipping the legacy `InsightsQLDropdown` button-trigger
...
function AggregationTargetContext({ insightProps, isInsightsQL }: AggregationTargetContextProps): JSX.Element {
    return (
        <FieldDescription className="!mt-0 flex flex-col gap-1">
            ...
            <span>
                ...
                <Link
                    to="https://hanzo.ai/docs/data-warehouse/join#table-joins"
                    target="_blank"
                    className="underline text-primary"
                >
                    For more help
                </Link>
                . {isInsightsQL ? '(currently using SQL expression)' : ''}
            </span>
        </FieldDescription>
    )
}
...
function FunnelAggregationDescription({ insightProps }: { insightProps: InsightLogicProps }): JSX.Element {
    ...
    const isAggregatingByInsightsQL = !!querySource?.funnelsFilter?.funnelAggregateByInsightsQL && !isAggregatingByGroup
    if (isAggregatingByInsightsQL) {
        return (
            <span>
...
const INSIGHTSQL_SENTINEL = '__insightsql__'
...
function ColumnSelect({ options, value, onChange, allowInsightsQL }: ColumnSelectProps): JSX.Element {
    const [open, setOpen] = useState(false)
    const triggerRef = useRef<HTMLButtonElement>(null)

    // InsightsQL mode whenever the value isn't a real column name AND the field
    ...
    // rendering as blank.
    const isInsightsQL = allowInsightsQL && !options.some((o) => o.name === value)
    const displayValue = isInsightsQL ? INSIGHTSQL_SENTINEL : value

    const items: ColumnSelectEntry[] = useMemo(
        () => [
            ...options.map((o) => ({ value: o.name, title: o.name, description: o.type as string })),
            ...(allowInsightsQL
                ? [{ value: INSIGHTSQL_SENTINEL, title: 'SQL expression', description: 'Use a InsightsQL expression' }]
                : []),
        ],
        [options, allowInsightsQL]
    )
    ...
    return (
        <Combobox
            ...
            onValueChange={(item: ColumnSelectEntry | null) =>
                onChange(item ? (item.value === INSIGHTSQL_SENTINEL ? '' : item.value) : '')
            }
        >
