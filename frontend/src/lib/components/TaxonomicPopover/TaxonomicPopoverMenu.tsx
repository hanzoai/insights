...
import { IconChevronDown, IconFilter } from '@hanzo/icons'
...
/**
 * Builds the trigger `Button` element — shared by the lazy placeholder
 * and the live menu trigger so the two are visually identical. Returns a
 * bare `Button` (no wrapper) so base-ui's `DropdownMenuTrigger` can
...
function buildTriggerButton(args: {
    ...
    return (
        <Button
            type="secondary"
            {...triggerButtonProps}
            // Button only auto-adds the dropdown chevron inside a
            // Dropdown; this trigger isn't, so default it explicitly
