...
describe('FlowManager', () => {
    ...
    let hub: Hub
    let manager: FlowManagerService
    ...
    beforeEach(async () => {
        ...
        manager = new FlowManagerService(hub.postgres, hub.pubSub, hub.encryptedFields)
    ...
    it('returns the script flow', async () => {
        let items = await manager.getFlowsForTeam(teamId1)
        ...
        manager['onFlowsReloaded'](teamId1, [flows[0].id])

        items = await manager.getFlowsForTeam(teamId1)

        expect(items.find((item) => item.id === flows[0].id)).toMatchObject({
            id: flows[0].id,
    ...
    describe('cache staleness bounds', () => {
        ...
        it('picks up an edit without a reload notification once the background refresh age passes', async () => {
            ...
            try {
                let items = await manager.getFlowsForTeam(teamId1)
                expect(items.find((item) => item.id === flows[0].id)?.name).toBe('Test Script Flow team 1')
                ...
                await waitForExpect(async () => {
                    const refreshed = await manager.getFlowsForTeam(teamId1)
                    expect(refreshed.find((item) => item.id === flows[0].id)?.name).toBe(
    ...
    describe('getFlowIdsForTeam', () => {
        it('returns function IDs', async () => {
            const result = await manager.getFlowIdsForTeams([teamId1, teamId2])

            expect(result[teamId1]).toHaveLength(2)
            expect(result[teamId1]).toContain(flows[0].id)
            expect(result[teamId1]).toContain(flows[1].id)

            expect(result[teamId2]).toHaveLength(1)
            expect(result[teamId2]).toContain(flows[2].id)
        })

        it('returns empty arrays for teams with no matching functions', async () => {
            const nonExistentTeamId = teamId2 + 1
            const result = await manager.getFlowIdsForTeams([nonExistentTeamId])
            expect(result[nonExistentTeamId]).toEqual([])
        })

        it('handles archived script flows', async () => {
            const originalResult = await manager.getFlowIdsForTeams([teamId1, teamId2])
            ...
            const result = await manager.getFlowIdsForTeams([teamId1])
            expect(result[teamId1]).toHaveLength(1)
            expect(result[teamId1]).not.toContain(flows[0].id)
        })
    })
