...
export function StepRandomCohortBranchConfiguration({
    ...
    node: Node<Extract<CustomFlowAction, { type: 'random_cohort_branch' }>>
}): JSX.Element {
    const action = node.data
    const { cohorts } = action.config

    const { edgesByActionId } = useValues(customFlowEditorLogic)
    const { setWorkflowAction, setWorkflowActionEdges } = useActions(customFlowEditorLogic)

    const nodeEdges = edgesByActionId[action.id] ?? []

    const setCohorts = (
        cohorts: Extract<CustomFlowAction, { type: 'random_cohort_branch' }>['config']['cohorts']
    ): void => {
