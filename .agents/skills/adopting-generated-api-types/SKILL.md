...
Insights generates TypeScript API client functions and types from Django serializers via the OpenAPI pipeline:
...
If no generated function exists, the backend endpoint may lack `@extend_schema` or `@validated_request`. Fix the backend first using the `improving-drf-endpoints` skill, then run `insightscli build:openapi`.
...
3. Run `pnpm --filter=@hanzo/frontend typescript:check` to verify no breakage
...
1. **TypeScript check:** `pnpm --filter=@hanzo/frontend typescript:check`
2. **Grep for leftover manual types:** search for the old type name across the codebase
3. **Run relevant tests:** `insightscli test <test_file>`
...
- **Regenerate types:** `insightscli build:openapi`
