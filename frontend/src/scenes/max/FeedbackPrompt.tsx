...
export function FeedbackPrompt({ conversationId, traceId }: FeedbackPromptProps): JSX.Element {
    ...
    useEffect(() => {
        ...
        if (pendingTicketSubmission && lastSubmittedTicketId && lastSubmittedTicketId !== ticketIdBeforeSubmission) {
            ...
            // The insights_ai_support_ticket_created event (with $ai_feedback_rating) is captured in
    ...
    async function handleSupportFormSubmit(): Promise<void> {
        ...
        if (!finalMessage) {
            toast.error('Please add a description before creating a ticket.')
            return
        }
        ...
        if (
            ...
            toast.error('Please choose a message type, topic, and severity level.')
            return
        }
