...
def _pr_lifecycle() -> contracts.PRLifecycle:
    return contracts.PRLifecycle(
        pull_request=contracts.PullRequest(
            ...
            repo=contracts.RepoRef(provider="github", owner="Insights", name="insights"),
...
class TestEngineeringAnalyticsAPI(APIBaseTest):
    ...
    def test_sources_serializes(self) -> None:
        sources = [
            contracts.GitHubSource(
                id="0192f000-0000-7000-8000-000000000000", repo="Insights/insights", prefix="older", synced=True
            ),
            contracts.GitHubSource(
                id="0192f000-0000-7000-8000-000000000001", repo="Insights/hanzo.ai", prefix="website"
            ),
        ]
        ...
        assert body[0] == {"id": sources[0].id, "repo": "Insights/insights", "prefix": "older", "synced": True}
    ...
    def test_pr_lifecycle_serializes(self) -> None:
        with mock.patch(f"{_VIEWS}.get_pr_lifecycle", return_value=_pr_lifecycle()) as get:
            response = self.client.get(self._url("pr_lifecycle"), {"pr_number": "10", "repo": "Insights/insights"})
        ...
        assert get.call_args.kwargs["repo"] == "Insights/insights"

    def test_pr_lifecycle_404_when_not_found(self) -> None:
        with mock.patch(f"{_VIEWS}.get_pr_lifecycle", return_value=None):
            response = self.client.get(self._url("pr_lifecycle"), {"pr_number": "999", "repo": "Insights/insights"})
    ...
    def test_resolve_branch_serializes(self) -> None:
        matches = [
            contracts.BranchPRMatch(repo="Insights/insights", number=42, title="Fix bug", state="merged"),
            contracts.BranchPRMatch(repo="Insights/insights", number=7, title=None, state=None),
        ]
        with mock.patch(f"{_VIEWS}.resolve_branch", return_value=matches) as resolve:
            response = self.client.get(self._url("resolve_branch"), {"branch": "feat/x", "repo": "Insights/insights"})

        assert response.status_code == status.HTTP_200_OK
        body = response.json()
        assert [(m["number"], m["repo"], m["title"], m["state"]) for m in body] == [
            (42, "Insights/insights", "Fix bug", "merged"),
            (7, "Insights/insights", None, None),
        ]
        assert resolve.call_args.kwargs["branch"] == "feat/x"
        assert resolve.call_args.kwargs["repo"] == "Insights/insights"
    ...
    def test_workflow_run_serializes(self) -> None:
        ...
        body = response.json()
        assert body["id"] == 7777
        assert body["repo"]["name"] == "insights"
    ...
    def test_workflow_runs_serializes(self) -> None:
        with mock.patch(f"{_VIEWS}.list_workflow_runs", return_value=[_workflow_run()]) as listing:
            response = self.client.get(
                self._url("workflow_runs"), {"workflow_name": "CI", "repo": "Insights/insights", "branch": "main"}
            )
        ...
        assert listing.call_args.kwargs["repo"] == "Insights/insights"
    ...
    def test_workflow_runner_costs_passes_branch_through(self) -> None:
        ...
        with mock.patch(f"{_VIEWS}.get_workflow_runner_costs", return_value=[]) as get_costs:
            response = self.client.get(
                ...
                {"workflow_name": "CI", "repo": "Insights/insights", "branch": "main"},
            )
    ...
    def test_pr_runs_serializes(self) -> None:
        with mock.patch(f"{_VIEWS}.list_pr_runs", return_value=[_workflow_run()]) as listing:
            response = self.client.get(self._url("pr_runs"), {"pr_number": "9100", "repo": "Insights/insights"})
    ...
    def test_pr_cost_serializes(self) -> None:
        ...
        with mock.patch(f"{_VIEWS}.get_pr_cost", return_value=summary) as getter:
            response = self.client.get(self._url("pr_cost"), {"pr_number": "10", "repo": "Insights/insights"})
        ...
        assert getter.call_args.kwargs["repo"] == "Insights/insights"
    ...
    def test_pr_lifecycle_400_on_malformed_repo(self) -> None:
        ...
        response = self.client.get(self._url("pr_lifecycle"), {"pr_number": "10", "repo": "Insights"})
    ...
    def test_requires_rollout_feature_flag(self) -> None:
        ...
        with mock.patch("hanzo_insights.feature_enabled", return_value=False):
            response = self.client.get(self._url("sources"))
