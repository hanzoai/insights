...
def _mascot_config(user: "User") -> dict | None:
    """Mirror core ``UserBasicSerializer.get_mascot_config`` so ``created_by`` output is identical."""
    config = user.mascot_config
    if not config:
        return None
...
def _user_basic_info(user: "User | None") -> contracts.TaskUserBasicInfo | None:
    ...
    return contracts.TaskUserBasicInfo(
        ...
        mascot_config=_mascot_config(user),
...
def _task_run_log_url(run: TaskRun) -> str | None:
    ...
    from insights.storage import object_storage  # noqa: PLC0415 — keep storage deps off the api import path
...
def is_signal_report_task(task_id: str | UUID, team_id: int) -> bool:
    """Whether the task is a genuine Signals report task rather than a Insights Code task.
    ...
    self-driving (the ``product-autonomy`` Inbox, which is generally available) — not the Insights
...
def get_active_wizard_cloud_run(team_id: int) -> contracts.WizardCloudRunDTO | None:
    ...
    running, or completed within the last day (so we can show "Insights is wired up" +
...
def create_and_run_task(
    ...
    **extra,
    ...
    instead of leaking the ORM ``Task``. ``team`` is a core ``insights.Team`` (not a tasks
...
def create_wizard_cloud_run(
    *,
    ...
    The workflow runs the published wizard in the sandbox (it integrates Insights), then the agent
    commits the changes, opens a PR on the user's repo, and keeps it green — it never implements
    Insights itself (see the wizard PR agent prompt). The wizard authenticates with its own scoped
    token (see ``create_wizard_oauth_access_token``), independent of the agent's sandbox token, so
    the agent runs with read-only Insights scopes.``wizard_config`` marks the run so the workflow runs the wizard pre-agent step.
    ...
    Insights absorbs the cost. Keep the pin inside that allowlist or the run fails at the gateway.
    """
    head_branch = generate_wizard_head_branch()
    prompt = build_wizard_pr_agent_prompt(head_branch)
    return create_and_run_task(
        team=team,
        title="Set up Insights",
        description=prompt,
...
def create_task_without_run(
    *,
    ...
    run via the warming facade. ``team`` is a core ``insights.Team`` (not a tasks model).
    """
...
def redeem_code_invite(code: str, user_id: int) -> contracts.CodeInviteRedeemResult:
    """Redeem a Insights Desktop invite for a user.
...
def signal_workflow_completion(run_id: str | UUID, status: str, error_message: str | None) -> None:
    ...
    from insights.temporal.common.client import sync_connect  # noqa: PLC0415 — keep temporalio off the api import path
...
def _send_wizard_pr_ready_email_for_pr(run: TaskRun) -> None:
    ...
    from insights.tasks.email import send_wizard_pr_ready_email  # noqa: PLC0415 - keep email task import lazy
...
def get_task_run_session(
    ...
    from insights.storage import object_storage  # noqa: PLC0415
...
def capture_relay_command_telemetry(
    ...
    *,
    ...
    """Emit Insights AI control-verb telemetry for a relayed agent command.
    ...
    generic-task usage stays out of the Insights AI funnels. Mirrors the old conversation-layer
...
def relay_task_run_message(
    ...
    *,
    ...
    from products.tasks.backend.temporal.client import (  # noqa: PLC0415 — keep temporalio off the api import path
        execute_insights_code_agent_relay_workflow,
    ...
    try:
        relay_id = execute_insights_code_agent_relay_workflow(
            run_id=str(run.id),
            text=trimmed,
...
def _trigger_task_processing_workflow(
    ...
    *,
    ...
    insights_mcp_scopes: Literal["read_only", "full"] = "full" if run_source in full_mcp_run_sources else "read_only"
    try:
...
def is_internal_debug_team(team_id: int | None) -> bool:
    """Whether the team is the Insights-internal debugging team. Mirrors the original view helper."""
...
def compute_repository_readiness(team_id: int, *, repository: str, window_days: int, refresh: bool) -> dict:
    ...
    from insights.models import Team  # noqa: PLC0415
...
def warm_task_sandbox(
    ...
    *,
    ...
    from insights.exceptions import QuotaLimitExceeded  # noqa: PLC0415 — keep billing deps off the api import path
    from insights.models import Team  # noqa: PLC0415
...
def beacon_task_presence(task_id: str | UUID, team_id: int, user_id: int | None, *, device_id) -> str:
    ...
    """
    from insights.models.user_push_token import UserPushToken  # noqa: PLC0415
...
def _agent_thread_updates_enabled(creator: User | None) -> bool:
    ...
    try:
        return bool(
            hanzo_insights.feature_enabled(AGENT_THREAD_UPDATES_FLAG, distinct_id, send_feature_flag_events=False)
        )
    except Exception:
