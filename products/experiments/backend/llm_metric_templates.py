...
def build_eval_pass_rate_metric(prompt_name: str) -> ExperimentRatioMetric:
    ...
    # InsightsQL property type inference, which casts the property to Float64 if any event in the
    ...
    applicable_filter = InsightsQLPropertyFilter(key="JSONExtractRaw(properties, '$ai_evaluation_applicable') != 'false'")
    return ExperimentRatioMetric(
        ...
        numerator=EventsNode(
            ...
            properties=[
                ...
                InsightsQLPropertyFilter(key="JSONExtractBool(properties, '$ai_evaluation_result')"),
                applicable_filter,
            ],
        ),
