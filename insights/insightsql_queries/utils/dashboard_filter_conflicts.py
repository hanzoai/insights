...
# insights/insightsql/property.py. Negated operators (is_not, not_icontains, not_regex) DO match unset
...
_INCOMPARABLE_FILTER_TYPES = ("cohort", "insightsql")
...
def _normalized_values(value: Any) -> list[str] | None:
    """None for valueless/empty filters (no-ops per insights/insightsql/property.py), else stringified values."""
    if value is None:
        return None
...
def _canonicalize_value(value: Any) -> str:
    ...
    # mixed-case strings like "True" stay untouched since string comparison in Datastore is case-sensitive
