...
from insights.insightsql.database.models import StringDatabaseField, TableNode
from insights.insightsql.direct_sql.datastore_adapter import (
    _fetch_capped_datastore_rows,
    ensure_read_only_raw_datastore_statement,
)
...
class TestDirectDatastoreTable(SimpleTestCase):
    def _table(self, database: str) -> DirectDatastoreTable:
        return DirectDatastoreTable(
            name="events",
            fields={},
            datastore_database=database,
            datastore_table_name="events",
            external_data_source_id="src",
        )

    def test_renders_escaped_database_and_table(self):
        self.assertEqual(self._table("mydb").to_printed_datastore(None), "mydb.events")

    def test_renders_table_only_when_no_database(self):
        self.assertEqual(self._table("").to_printed_datastore(None), "events")

    def test_escapes_special_identifiers(self):
        table = DirectDatastoreTable(
            name="t",
            fields={},
            datastore_database="my db",
            datastore_table_name="weird.name",
            external_data_source_id="s",
        )
        # backtick-escaped because they contain a space / dot
        self.assertEqual(table.to_printed_datastore(None), "`my db`.`weird.name`")
...
class TestDirectDatastoreTeamIdGuard(BaseTest):
    def _print(self, *, is_direct_query: bool) -> str:
        ...
        return prepare_and_print_ast(parse_select("SELECT col1 FROM external_events"), context, "datastore")[0]
...
class TestDatastoreReadOnlyGuard(SimpleTestCase):
    ...
    def test_allows_read_only(self, _name, sql):
        self.assertEqual(ensure_read_only_raw_datastore_statement(sql), sql)
    ...
    def test_rejects_settings_clause(self, _name, sql):
        with self.assertRaises(ExposedInsightsQLError):
            ensure_read_only_raw_datastore_statement(sql)
...
class TestDatastoreRowCap(SimpleTestCase):
    ...
    def test_returns_rows_and_types_under_cap(self):
        ...
        rows, column_names, column_types = _fetch_capped_datastore_rows(client, "SELECT n FROM t", None, 600)
    ...
    def test_raises_when_result_exceeds_cap(self):
        ...
        with patch("insights.insightsql.direct_sql.datastore_adapter.DIRECT_DATASTORE_MAX_ROWS", 3):
            with self.assertRaisesRegex(ExposedInsightsQLError, "Add a LIMIT clause"):
                _fetch_capped_datastore_rows(client, "SELECT n FROM t", None, 600)
    ...
    def test_raises_when_deadline_exceeded(self):
        ...
        with patch("insights.insightsql.direct_sql.datastore_adapter.perf_counter", side_effect=[0.0, 700.0]):
            with self.assertRaisesRegex(ExposedInsightsQLError, "execution time limit"):
                _fetch_capped_datastore_rows(client, "SELECT n FROM t", None, 600)

    def test_passes_parameters_through(self):
        client = self._stream_client([[(1,)]])
        params: dict[str, Any] = {"team_id": 1}
        _fetch_capped_datastore_rows(client, "SELECT n FROM t WHERE team = %(team_id)s", params, 600)
