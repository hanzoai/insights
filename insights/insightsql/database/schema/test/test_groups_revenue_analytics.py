...
@snapshot_datastore_queries
class TestGroupsRevenueAnalytics(DatastoreTestMixin, APIBaseTest):
    ...
    def setup_schema_sources(self):
        invoices_csv_path = Path("products/revenue_analytics/backend/insightsql_queries/test/data/stripe_invoices.csv")
        ...
        customers_csv_path = Path("products/revenue_analytics/backend/insightsql_queries/test/data/stripe_customers.csv")
    ...
    def test_get_revenue_for_schema_source_for_id_join(self):
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            ...
            for query in queries:
                response = execute_insightsql_query(parse_select(query), self.team, modifiers=self.MODIFIERS)
