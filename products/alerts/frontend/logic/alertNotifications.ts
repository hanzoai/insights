...
// Single source of truth for which destination InsightsFunction template each notification type uses.
...
// Maps a destination InsightsFunction's template_id back to the notification type, so analytics and
...
function buildAlertDestination(
    alertName: string,
    notification: PendingAlertNotification
): Pick<InsightsFunctionType, 'name' | 'template_id' | 'inputs'> {
