...
def _build_reviewers_content(
    ...
    github_login — no Insights user IDs are persisted. This is intentional:

    - Insights user enrichment happens at read time (in the artefact serializer via
...
async def _persist_agentic_report_artefacts(
    ...
    try:
        ...
        hanzo_insights.capture_exception(error)
        logger.exception(
