...
import { IconArrowLeft, IconCheckCircle, IconWarning } from '@hanzo/icons'
import { Banner, Button, TextArea, Link, Spinner } from '@hanzo/elements'
...
function RequestAccessSection({ definition }: { definition: IntegrationDefinition }): JSX.Element {
    ...
    if (requestedAccessKinds.includes(definition.kind)) {
        return (
            <Banner type="success" className="w-full">
...
function ConnectedNextStep(): JSX.Element {
    ...
    if (inFlight || !resolved) {
        return (
            <div className="flex flex-col items-center gap-2">
                ...
                <Button type="tertiary" size="small" to={urls.projectHomepage()}>
                    Go to Insights
                </Button>
            </div>
        )
    ...
    return (
        <Button type="primary" to={urls.projectHomepage()}>
            Go to Insights
        </Button>
    )
}
