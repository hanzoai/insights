#!/usr/bin/env python3
"""Generate a Datastore migration from a declarative-HCL change.
...
Usage (from the repo root):
  python insights/datastore/hcl/codegen/gen_migration.py --name add_foo
...
HERE = os.path.dirname(os.path.abspath(__file__))  # .../insights/datastore/hcl/codegen
HCL_DIR = os.path.dirname(HERE)  # .../insights/datastore/hcl
REPO_ROOT = os.path.abspath(os.path.join(HERE, "..", "..", "..", ".."))
HCL_REL = os.path.relpath(HCL_DIR, REPO_ROOT)  # insights/datastore/hcl (relative: resolves inside the wrapper)
...
def main() -> None:
    ...
    ap.add_argument(
        "--auto",
        action="store_true",
        help="write the next numbered migration into insights/datastore/migrations/ and bump max_migration.txt",
    )
    args = ap.parse_args()
    ...
    body = (
        ...
        "insights/datastore/hcl/codegen/gen_migration.py.\n"
    ...
    if args.auto:
        mig_dir = os.path.join(REPO_ROOT, "insights", "datastore", "migrations")
