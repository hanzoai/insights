...
WEBHOOK_NAME = "Insights data warehouse"
...
def _webhook_table_transformer(table: pa.Table) -> pa.Table:
    ...
    for data, meta in zip(table.column("data").to_pylist(), meta_column):
        ...
        # The Script template already drops deletions; this keeps a hand-configured webhook that
...
def create_webhook(company_domain: str, api_token: str, webhook_url: str) -> WebhookCreationResult:
    ...
    would burn one of the account's 40 webhook slots per table. The Script template drops every
...
def _remove_superseded_webhooks(
    ...
    credentials the Script function no longer accepts. Cleaning up after the create (rather than
