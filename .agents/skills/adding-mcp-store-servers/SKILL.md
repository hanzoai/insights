...
description: Add a third-party MCP server (Linear, Notion, GitHub, ...) to the Insights MCP store catalog. Use when asked to "add X to the MCP store", expand the MCP server marketplace, or fix a broken catalog entry. Covers finding the vendor's remote MCP endpoint, probing it (handshake, OAuth discovery, DCR), authoring the catalog entry in products/mcp_store/backend/catalog.py, verification tiers, and the operator handoff for servers without Dynamic Client Registration.
---
...
5. **Run the checks.**
   ...
   ```sh
   insightscli test products/mcp_store/backend/test/test_catalog_sync.py
   ```
...
- [ ] Redirect URI: `https://us.hanzo.ai/api/mcp_store/oauth_redirect/` (and the EU equivalent)
