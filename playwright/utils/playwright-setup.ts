...
export class PlaywrightSetup {
    ...
    /**
     * Seed a single InsightsFunctionTemplate row. Templates are global (not team-scoped) and
     * Playwright CI doesn't run sync_insights_function_templates, so any test that exercises
     * the workflow editor or script function configuration must seed the templates it needs.
     * Idempotent across parallel workers.
     */
    async seedInsightsFunctionTemplate(data: {
        template_id: string
        name?: string
        ...
        const result = await this.callSetupEndpoint('insights_function_template', { data })
        if (!result.success) {
            throw new Error(`Failed to seed script function template: ${result.error}`)
        }
