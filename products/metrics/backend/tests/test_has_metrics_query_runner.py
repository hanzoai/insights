...
class TestHasMetricsQueryRunner(DatastoreTestMixin, APIBaseTest):
    ...
    def test_has_metrics_respects_team_isolation(self):
        sync_execute("TRUNCATE TABLE IF EXISTS metrics1")
        cache.delete(f"team:{self.team.id}:has_metrics")
        # Row belongs to a different team — the InsightsQL team_id auto-filter
...
class TestHasMetricsAPI(DatastoreTestMixin, APIBaseTest):
    CLASS_DATA_LEVEL_SETUP = True
