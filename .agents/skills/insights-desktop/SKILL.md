...
description: >
  ...
  workspace imported from Insights/code, not part of the root frontend or Django build. Use when
  the user says /insights-desktop, or works on the Electron desktop app, apps/code, apps/web,
  apps/mobile, packages/core, packages/ui, packages/workspace-server, @hanzo/api-client,
  @hanzo/agent, or the agent framework. Pins the working directory to products/desktop, swaps
...
`products/desktop/` is the Insights desktop app (Electron + agent framework), imported from
Insights/code and kept as a **nested standalone workspace**: own `pnpm-workspace.yaml`, own
...
commands with that as cwd. Do not go fishing in `frontend/`, `insights/`, or other `products/*`
...
- Merging: `gh pr merge <number> --squash` into `master`. **`AGENTS.md` "Merging PRs" and
  `products/desktop/.claude/skills/merging-prs/` are stale** — they describe the Insights/code
  Trunk queue against `main`, which does not exist here. `products/desktop/POST-MIGRATION.md`
  has the correct flow.
- `insightscli ci:preflight` on push (pre-push hook) still runs repo-wide.
...
- **Editing** it is a separate, monorepo-side change: root CLAUDE.md rules apply again
  (`/improving-drf-endpoints`, `/django-migrations`, serializer `help_text`, `insightscli build:openapi`).
...
The tree is a verbatim copy of Insights/code at a pinned SHA. A resync replaces it wholesale, so
**edits to imported files are lost unless they are upstreamed or listed as intentional drift.**
Before changing a file, consider whether the fix belongs upstream in Insights/code. The drift
...
- After touching `@hanzo/platform`: rebuild or typecheck its `dist/`.
