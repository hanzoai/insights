"""Provision the managed Slack InsightsFunction behind a Customer analytics event stream.
...
def sync_event_stream_destination(stream: EventStream, *, team: "Team", user: "User | None") -> None:
    """Reconcile the stream's managed InsightsFunction with its current config and membership.
    ...
    instance = _managed_insights_function(stream)
    ...
    if not has_slack_config:
        ...
        config.pop("inputs")
    context = {
        # InsightsFunctionSerializer.create() reads context["request"].user for created_by.
    ...
    if instance is not None:
        serializer = InsightsFunctionSerializer(instance, data=config, partial=True, context=context)
    else:
        serializer = InsightsFunctionSerializer(data=config, context=context)
    serializer.is_valid(raise_exception=True)
    insights_function = serializer.save()
    logger.info(
    ...
    if stream.insights_function_id != insights_function.id:
        stream.insights_function_id = insights_function.id
        stream.save(update_fields=["insights_function_id"])
...
def send_test_slack_message(*, team_id: int, stream_id: str, user: "User") -> str | None:
    ...
    try:
        SlackIntegration(integration).client.chat_postMessage(
            ...
            text=(
                ":wave: This is a test message from Insights Customer analytics — "
        ...
        hint = " Invite the Insights bot to the channel and try again." if error == "not_in_channel" else ""
...
def archive_event_stream_destination(stream: EventStream) -> None:
    """Disable and soft-delete the stream's managed InsightsFunction. Invoked by the facade's
    ``delete_event_stream`` — the single deletion path for event streams."""
    function = _managed_insights_function(stream)
    if function is None:
        return
    ...
    logger.info(
        ...
        stream_id=str(stream.id),
        insights_function_id=str(function.id),
    )


def _managed_insights_function(stream: EventStream) -> InsightsFunction | None:
    if not stream.insights_function_id:
        return None
    return InsightsFunction.objects.filter(
        team_id=stream.team_id, id=stream.insights_function_id, type=_DESTINATION_TYPE, deleted=False
    ).first()
