...
from insights.temporal.data_modeling.run_workflow import (
...
async def test_run_dag_activity_activity_materialize_mocked(activity_environment, ateam, dag, posthog_table_names):
    ...
    with unittest.mock.patch("insights.temporal.data_modeling.run_workflow.materialize_model", new=magic_mock):
...
async def test_materialize_model_progress_tracking(ateam, bucket_name, minio_client):
    ...
    with (
        ...
        unittest.mock.patch("insights.temporal.data_modeling.run_workflow.insightsql_table", mock_insightsql_table),
        unittest.mock.patch("insights.temporal.data_modeling.run_workflow.get_query_row_count", return_value=6),
    ):
        job = await database_sync_to_async(DataModelingJob.objects.create)(
            team=ateam,
            status=DataModelingJob.Status.RUNNING,
...
async def test_materialize_model_empty_results(ateam, bucket_name, minio_client):
    ...
    with (
        ...
        unittest.mock.patch("insights.temporal.data_modeling.run_workflow.a_pause_saved_query_schedule"),
    ):
        job = await database_sync_to_async(DataModelingJob.objects.create)(
            team=ateam,
            status=DataModelingJob.Status.RUNNING,
...
async def test_create_default_modifiers_for_team_in_async_context(ateam):
    ...
    with unittest.mock.patch("insights.models.team.team.is_cloud", return_value=True):
