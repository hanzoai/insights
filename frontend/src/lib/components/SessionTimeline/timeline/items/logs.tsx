...
export class ConsoleLogLoader implements ItemLoader<ConsoleLogItem> {
    ...
    async loadBefore(cursor: Dayjs, limit: number): Promise<{ items: ConsoleLogItem[]; hasMoreBefore: boolean }> {
        ...
        const query = insightsql`SELECT timestamp, level, message FROM log_entries WHERE log_source = 'session_replay' AND log_source_id = ${this.sessionId} AND timestamp >= ${windowStart} AND timestamp < ${cursor} ORDER BY timestamp DESC LIMIT ${limit}`
    ...
    async loadAfter(cursor: Dayjs, limit: number): Promise<{ items: ConsoleLogItem[]; hasMoreAfter: boolean }> {
        ...
        const query = insightsql`SELECT timestamp, level, message FROM log_entries WHERE log_source = 'session_replay' AND log_source_id = ${this.sessionId} AND timestamp > ${cursor} AND timestamp <= ${windowEnd} ORDER BY timestamp ASC LIMIT ${limit}`
        const { items, hasMoreAfter } = await this.execute(query, limit, 'after')
        return { items, hasMoreAfter }
    }

    private async execute(
        query: ReturnType<typeof insightsql>,
        limit: number,
