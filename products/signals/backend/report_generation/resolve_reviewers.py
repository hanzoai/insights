...
def enrich_reviewer_dicts_with_org_members(
    ...
    *,
    ...
    """Enrich reviewer dicts (from artefact content) with fresh Insights user info.
...
def get_org_member_github_login_to_user_map(team_id: int) -> dict[str, User] | None:
    """Build a mapping of GitHub login -> Insights User for the team's org.
...
def get_org_member_github_logins_by_user_uuid(team_id: int, user_uuids: list[str]) -> dict[str, str]:
    """Build a mapping of Insights user UUID string -> GitHub login for org members on the team."""
    if not user_uuids:
        return {}
