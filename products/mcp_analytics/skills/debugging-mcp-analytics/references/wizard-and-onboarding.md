...
`npx -y @hanzo/wizard@latest mcp-analytics` runs an agentic codemod that instruments a
user's _own_ MCP server, in TypeScript/JavaScript **or Python**. It is not `wizard mcp add`,
which installs the Insights MCP server into a coding agent.
...
`developing-the-wizard.mdx` handbook page in `hanzo.ai` documents this.
...
The codemod also handles credentials (the project token, fetched via the Insights MCP
`insights:projects-get` tool), lifecycle (shutdown and flush), STDIO-safe logging (stderr is fine,
...
paths are different: they require a floor (`insights>=7.21`), which _can_ go stale, so check it
against the current `insights` release when touching the Python instrumentation.
...
1. **Team skills store** (Postgres) — reachable over MCP via `insights:skill-list` / `insights:skill-get`, and
...
2. **context-mill release** (`skills-mcp-resources.zip`) — the wizard/**install** skills,
   ...
   backend for `insights-cli api skill list|install`** (`services/mcp/src/cli/skills.ts`).
3. **`insights_ai` `build_skills`** — `products/insights_ai/scripts/build_skills.py` scans
   ...
   (`querying-insights-data`, the `exploring-mcp-*` set, `improving-mcp-tools`) and this skill.
   Consumed by Insights Desktop and Insights AI. **Not** by `insights-cli` — that is channel 2.

Two lookalikes are **not** MCP-analytics skills:
`products/managed_migrations/skills/testing-mcp-tools-locally/` and
`.agents/skills/implementing-mcp-tools/`. Both concern building and testing Insights's own MCP
server tools, not analyzing MCP usage.

Note the activation gap: `wizard mcp add` installs the Insights MCP server but does **not**
...
`mcpAnalyticsOnboardingLogic` drives all of it with a single **unbounded** InsightsQL query
