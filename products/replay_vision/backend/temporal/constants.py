...
# Each refresh is a Datastore count; bound the batch and parallelism so one run stays cheap.
ESTIMATES_MAX_PER_RUN = 400
ESTIMATE_REFRESH_CONCURRENCY = 4

LIST_STALE_ESTIMATES_TIMEOUT = dt.timedelta(seconds=60)
# Covers the estimate query's 30s Datastore cap plus the Postgres staleness check.
...
def replay_vision_distinct_id(team_id: int) -> str:
    """`insights_distinct_id` for analytics events emitted by Replay Vision when no human user is attributable."""
