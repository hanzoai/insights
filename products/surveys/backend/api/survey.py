...
class SurveyViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, viewsets.ModelViewSet):
    ...
    def destroy(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        # Insights-managed implementation detail, so deleting a survey that only has one
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                description=(
                    ...
                    "Adds one extra InsightsQL query per question, so leave off unless you need the breakdown."
                ),
            ),
        ],
        responses={
    ...
    def survey_responses_list(self, request: request.Request, **kwargs) -> Response:
        ...
        # combination at the API boundary rather than letting the InsightsQL helper raise later.
        ...
        if survey.questions and question_id:
            ...
            for idx, question in enumerate(survey.questions):
                if question.get("id", None) == question_id:
                    ...
                    # getSurveyResponse() InsightsQL function, which requires an integer first
    ...
    def generate_translations(self, request: request.Request, **kwargs):
        ...
        if not (settings.DEBUG or is_cloud()):
            raise exceptions.ValidationError(
                "survey translation generation is only supported in Insights Cloud or DEBUG mode"
            )
