...
import { CHART_REF_PREFIX } from 'lib/elements/Markdown'
...
export function resolveChartPlacements(
    ...
    // what the renderer can do with one: `Markdown` drops the `<p>` around a paragraph whose own
    ...
    // A reference also has to be all its paragraph holds. `Markdown` gives up the `<p>`
