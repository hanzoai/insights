...
export function getDirectQuerySelectionDescription(selectedSchema?: string | null): string {
    ...
    if (normalizedSchema) {
        return `Query selected Postgres tables from within Insights. Tables stay in the source database and are not synced into the data warehouse. You can't join data from these tables with other data in the Insights warehouse. Choose which tables from the "${normalizedSchema}" schema should be queryable.`
    }

    return "Query selected Postgres tables from within Insights. Tables stay in the source database and are not synced into the data warehouse. You can't join data from these tables with other data in the Insights warehouse. Enable each schema to choose which tables should be queryable."
}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isUnavailable && (
                            <Tooltip
                                ...
                                placement="top"
                            >
                                <Tag type="warning" className="cursor-help">
        ...
        },
        {
            ...
            render: function RenderSyncType(_: unknown, schema: ExternalDataSourceSyncSchema) {
                ...
                if (schema.sync_type === 'webhook') {
                    return <Tag type="success">Webhook</Tag>
                }
    ...
    return (
        <>
            ...
            <Modal
                ...
                footer={
                    <Button type="primary" onClick={() => setColumnSelectionSchema(null)}>
                        Done
                    </Button>
                }
            >
