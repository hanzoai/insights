...
response. Insights's experiment query filters these out via `in(properties.$feature_flag_response,
...
showing `data: null` rows is not, by itself, evidence that metric queries are failing.** Insights
