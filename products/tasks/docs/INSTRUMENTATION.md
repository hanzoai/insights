...
All analytics events tracked from the tasks backend via `hanzo_insights.capture()`.

All events include group analytics via `groups()` from `insights.event_usage`, which sets `instance`, `organization`, `customer`, and `project` groups where available.
...
The workflow emission feeds the `insights_tasks_task_run_failed_total` Prometheus counter and structured logging with these properties:
