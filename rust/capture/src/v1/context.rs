...
impl RequestContext {
    ...
    /// Parse `Insights-Sdk-Info` (`<canonical-$lib-name>/<semver>`, split at
...
mod tests {
    ...
    fn sdk_lib_and_version_parses_canonical_format() {
        let headers = valid_headers();
        let ctx = test_context(&headers).unwrap();
        assert_eq!(ctx.sdk_lib_and_version(), Some(("insights-rs", "1.0.0")));
    }
    ...
    fn sdk_lib_and_version_trims_whitespace() {
        ...
        let ctx = test_context(&headers).unwrap();
        assert_eq!(ctx.sdk_lib_and_version(), Some(("insights-rs", "1.2.3")));
    }
