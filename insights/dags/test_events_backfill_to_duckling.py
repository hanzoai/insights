...
from insights.dags.events_backfill_to_duckling import (
...
from insights.ducklake.common import EARLIEST_BACKFILL_DATE, NO_HISTORY_SENTINEL
...
def _disable_ducklake_file_partition_value_fixup_by_default():
    ...
    with patch(
        "insights.dags.events_backfill_to_duckling._ducklake_file_partition_value_fixup_enabled", return_value=False
    ):
        yield
...
class TestResolveDucklingTarget:
    @patch("insights.dags.events_backfill_to_duckling._resolve_table_names", return_value=("events", "persons"))
    @patch("insights.dags.events_backfill_to_duckling.get_duckgres_server_for_organization", return_value=None)
    @patch("insights.dags.events_backfill_to_duckling._get_org_id_for_team", return_value="org-1")
    def test_resolves_bucket_from_control_plane(
        ...
        with patch(
            ...
            return_value="insights-duckling-org-1-mw-prod-us",
        ) as mock_cp:
        ...
        assert target == DucklingTarget(
            ...
            bucket="insights-duckling-org-1-mw-prod-us",
    ...
    def test_control_plane_wins_over_stale_stored_server_bucket(self, _mock_org: MagicMock, _mock_tables: MagicMock):
        ...
        server = MagicMock(bucket="insights-duckling-stale-prod-us", bucket_region="us-east-1")
        with (
            patch(
                "insights.dags.events_backfill_to_duckling.get_duckgres_server_for_organization",
                return_value=server,
            ),
            patch(
                "products.data_warehouse.backend.presentation.views.managed_warehouse.cp_bucket_for",
        ...
        assert target.bucket == "insights-duckling-org-1-mw-prod-us"
    ...
    def test_falls_back_to_stored_server_when_control_plane_unavailable(
        ...
        server = MagicMock(bucket="insights-duckling-org-1-mw-prod-us", bucket_region="us-east-1")
        with (
            patch(
                "insights.dags.events_backfill_to_duckling.get_duckgres_server_for_organization",
                return_value=server,
...
class TestResolveTableNames:
    ...
    def test_passes_through_resolved_names(self):
        with patch(
            "insights.ducklake.team_state.resolve_events_persons_tables",
...
class TestEventsDDL:
    ...
    def test_events_ddl_honors_suffixed_table_name(self):
        ...
        result = conn.execute("DESCRIBE memory.insights.events_alpha").fetchall()
...
class TestPersonsDDL:
    ...
    def test_persons_ddl_honors_suffixed_table_name(self):
        ...
        result = conn.execute("DESCRIBE memory.insights.persons_beta").fetchall()
        assert {row[0] for row in result} == EXPECTED_DUCKLAKE_PERSONS_COLUMNS
        conn.close()


class TestParsePartitionKeyDates:
    @patch("insights.dags.events_backfill_to_duckling.timezone")
...
class TestDeleteErrorHandling:
    ...
    @patch("insights.dags.events_backfill_to_duckling.logger")
    def test_handles_missing_table(self, _label, fn_name, partition_date, mock_logger):
        from insights.dags import events_backfill_to_duckling as mod
...
class TestFullBackfillSensorEarliestDate:
    ...
    @patch("insights.ducklake.common.get_earliest_event_date_for_team")
    @patch("insights.ducklake.team_state.list_enabled_backfill_rows")
    @patch("insights.dags.events_backfill_to_duckling.timezone")
    @patch("insights.dags.events_backfill_to_duckling.stale_running_partitions", new=MagicMock(return_value=[]))
    def test_earliest_date_clamped(
        ...
        # Earliest date is cached on the row so later ticks never re-query Datastore.
    ...
    def _run_full_sensor(self, backfills, *, now, get_earliest, existing=None, get_runs=None):
        ...
        with (
            patch("insights.dags.events_backfill_to_duckling.timezone") as mock_tz,
            patch("insights.ducklake.team_state.list_enabled_backfill_rows") as mock_list_rows,
            patch("insights.dags.events_backfill_to_duckling.ManagedWarehouseBackfillPartition") as mock_projection,
            patch("insights.dags.events_backfill_to_duckling.record_backfill_outcome"),
            patch("insights.dags.events_backfill_to_duckling.stale_running_partitions", return_value=[]),
            patch("insights.ducklake.common.get_earliest_event_date_for_team") as mock_ge,
        ):
    ...
    def test_caps_earliest_lookups_per_tick(self):
        # 7 unresolved teams, cap is 5 → only 5 Datastore lookups this tick; the other two
...
class TestFullBackfillSensorCpEnumeration:
    ...
    def _run_cp_sensor(self, cp_rows, *, now, get_earliest=None):
        ...
        from insights.ducklake import cp_teams

        cp_teams.clear_cache()
        try:
            with (
                patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=cp_rows),
                patch("insights.dags.events_backfill_to_duckling.timezone") as mock_tz,
                patch("insights.dags.events_backfill_to_duckling.ManagedWarehouseBackfillPartition") as mock_projection,
                patch("insights.dags.events_backfill_to_duckling.record_backfill_outcome"),
                patch("insights.dags.events_backfill_to_duckling.stale_running_partitions", return_value=[]),
                patch("insights.ducklake.common.get_earliest_event_date_for_team", return_value=get_earliest),
                patch(
                    "products.data_warehouse.backend.presentation.views.managed_warehouse.push_team_earliest_event_date"
                ) as mock_push,
            ):
...
class TestDailyBackfillSensor:
    ...
    def test_catchup_is_bounded_per_tick_but_yesterday_always_emitted(self):
        ...
        with patch(
            "insights.dags.events_backfill_to_duckling.DAILY_BACKFILL_MAX_CATCHUP_PARTITIONS_PER_TICK",
            3,
        ):
            result = self._run_daily(
...
class TestGetClusterRetry:
    ...
    @patch("insights.dags.events_backfill_to_duckling.get_cluster")
...
class TestDuckLakeAddDataFilesPartitioning:
    ...
    def ducklake_env(self, tmp_path):
        ...
        conn.execute("CREATE SCHEMA IF NOT EXISTS test_lake.insights")
        conn.execute(EVENTS_TABLE_DDL.format(catalog="test_lake", table="events"))
        conn.execute(
            "ALTER TABLE test_lake.insights.events "
    ...
    def test_new_path_format_succeeds(self, ducklake_env):
        ...
        conn.execute(f"CALL ducklake_add_data_files('test_lake', 'events', '{path}', schema => 'insights')")

        # Verify the data is queryable
        result = conn.execute("SELECT count(*) FROM test_lake.insights.events").fetchone()
        assert result[0] == 1
    ...
    def test_hive_partitioning_false_with_plain_path_still_fails(self, ducklake_env):
        ...
        with pytest.raises(duckdb.InvalidInputException, match="invalid partition value"):
            conn.execute(
                ...
                f" schema => 'insights', hive_partitioning => false)"
            )
...
_S3_503_GLOB_ERROR = psycopg.errors.SyntaxErrorOrAccessRuleViolation(
    ...
    "'s3://insights-duckling-x-mw-prod-us/backfill/events/55513/year=2019/month=12/day=21/abc_' "
...
class TestConnectDuckgres:
    ...
    @patch("insights.dags.events_backfill_to_duckling.make_duckgres_conninfo", return_value="conninfo")
    @patch("insights.dags.events_backfill_to_duckling.psycopg.connect")
...
class TestDuckgresSessionRetry:
    ...
    @patch("insights.dags.events_backfill_to_duckling.time.sleep")
    @patch("insights.dags.events_backfill_to_duckling._connect_duckgres")
...
class TestDuckgresBackfillOptions:
    ...
    def test_enabled_requests_small_colocated_worker(self):
        with patch("insights.dags.events_backfill_to_duckling.DUCKGRES_WORKER_PROFILE_ENABLED", True):
            assert (
                _duckgres_backfill_options()
    ...
    def test_disabled_returns_empty_string(self):
        ...
        with patch("insights.dags.events_backfill_to_duckling.DUCKGRES_WORKER_PROFILE_ENABLED", False):
    ...
    def test_never_sets_statement_timeout(self, _label, enabled):
        ...
        with patch("insights.dags.events_backfill_to_duckling.DUCKGRES_WORKER_PROFILE_ENABLED", enabled):
...
class TestExportDatastoreRetries:
    """Both Datastore calls on the export path — the row-count estimate and the export
...
class TestExportFanOut:
    ...
    def test_events_export_sizes_fanout_to_row_count(self, target):
        ...
        # The S3 destination carries the {_partition_id} placeholder Datastore substitutes per bucket.
    ...
    @patch("insights.dags.events_backfill_to_duckling._resolve_duckling_target")
    ...
    def test_dry_run_does_not_touch_datastore(self, _label, export_fn, kwargs):
...
class TestDucklakeFilePartitionValueFixupHelper:
    ...
    @patch("insights.dags.events_backfill_to_duckling._open_catalog_conn")
...
class TestDucklakeFilePartitionValueFixupCatalogInteraction:
    ...
    def test_runtime_spec_mismatch_raises(self, mock_open_conn, target):
        ...
        with pytest.raises(RuntimeError, match="live catalog spec for insights.events"):
            _fixup_partition_values_for_added_files(MagicMock(), target, "events", "events", files)
    ...
    @patch("insights.dags.events_backfill_to_duckling.time.sleep", lambda _: None)
    @patch("insights.dags.events_backfill_to_duckling._open_catalog_conn")
...
class TestRegisterTriggersFixup:
    ...
    @patch("insights.dags.events_backfill_to_duckling._fixup_partition_values_for_added_files")
    @patch("insights.dags.events_backfill_to_duckling._ducklake_file_partition_value_fixup_enabled", return_value=True)
    ...
    @patch("insights.dags.events_backfill_to_duckling._fixup_partition_values_for_added_files")
    @patch("insights.dags.events_backfill_to_duckling._ducklake_file_partition_value_fixup_enabled", return_value=False)
...
class TestFannedOutLayoutRoundTrip:
    ...
    def lake(self, tmp_path):
        ...
        conn.execute(f"CREATE SCHEMA IF NOT EXISTS {DUCKLAKE_ALIAS}.insights")
        conn.execute(EVENTS_TABLE_DDL.format(catalog=DUCKLAKE_ALIAS, table="events"))
        conn.execute(
            f"ALTER TABLE {DUCKLAKE_ALIAS}.insights.events "
            "SET PARTITIONED BY (year(timestamp), month(timestamp), day(timestamp))"
        )
    ...
    def _count(self, conn):
        return conn.execute(f"SELECT count(*) FROM {DUCKLAKE_ALIAS}.insights.events").fetchone()[0]
    ...
    def test_allow_missing_handles_schema_evolution(self, lake):
        ...
        conn.execute(f"ALTER TABLE {DUCKLAKE_ALIAS}.insights.events ADD COLUMN captured_at TIMESTAMPTZ")
        ...
        with pytest.raises(duckdb.Error, match="captured_at"):
            conn.execute(
                f"CALL ducklake_add_data_files('{DUCKLAKE_ALIAS}', 'events', '{single_file}', schema => 'insights')"
            )
        ...
        cap_vals = [r[0] for r in conn.execute(f"SELECT captured_at FROM {DUCKLAKE_ALIAS}.insights.events").fetchall()]
