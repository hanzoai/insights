...
def test_resolve_core_path_passes_through_already_rooted_filenames() -> None:
    assert coverage_report.resolve_core_path("insights/api/auth.py", ["insights", "ee"], {}) == "insights/api/auth.py"
...
def test_resolve_core_path_resolves_against_the_owning_source(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert coverage_report.resolve_core_path("api/only_in_ee.py", ["insights", "ee"], {}) == "ee/api/only_in_ee.py"
...
def test_resolve_core_path_falls_back_to_first_source_when_no_candidate_exists(
    ...
    assert coverage_report.resolve_core_path("nowhere.py", ["insights", "ee"], {}) == "insights/nowhere.py"


def test_resolve_core_path_warns_and_prefers_insights_when_a_path_collides(
    ...
    monkeypatch.chdir(tmp_path)
    (tmp_path / "insights" / "api").mkdir(parents=True)
    (tmp_path / "insights" / "api" / "authentication.py").touch()
    (tmp_path / "ee" / "api").mkdir(parents=True)
    (tmp_path / "ee" / "api" / "authentication.py").touch()

    resolved = coverage_report.resolve_core_path("api/authentication.py", ["ee", "insights"], {})

    assert resolved == "insights/api/authentication.py"
    assert "::warning::" in capsys.readouterr().err


def test_resolve_core_path_caches_by_filename() -> None:
    cache: dict[str, str] = {"api/auth.py": "ee/api/auth.py"}
    assert coverage_report.resolve_core_path("api/auth.py", ["insights"], cache) == "ee/api/auth.py"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["insights/README.md"], False),  # non-.py under a backend root is not measured
        ([], False),
    ],
)
