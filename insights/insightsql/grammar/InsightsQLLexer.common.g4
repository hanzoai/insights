lexer grammar InsightsQLLexer;

// NB! We cat either InsightsQLLexter.cpp.g4 or InsightsQLLexter.python.g4 when generating the grammar.
...
NULL_SAFE_EQ: '<=>';
LT_EQ: '<=';
TAG_LT_SLASH: '</' -> type(LT_SLASH), pushMode(INSIGHTSQLX_TAG_CLOSE);
TAG_LT_OPEN: '<' {isOpeningTag()}? -> type(LT), pushMode(INSIGHTSQLX_TAG_OPEN);
LT: '<';
LT_SLASH: '</';
NOT_EQ: '!=' | '<>';
NOT_IREGEX: '!~*';
...
// ───────── INSIGHTSQLX TAG MODE for opening/self-closing tags ─────────
mode INSIGHTSQLX_TAG_OPEN;

TAG_SELF_CLOSE_GT : '/>' -> type(SLASH_GT), popMode;   // <tag …/>
TAG_OPEN_GT       :  '>' -> type(GT), popMode, pushMode(INSIGHTSQLX_TEXT);   // <tag …>
...
// ───────── INSIGHTSQLX TAG MODE for closing tags ─────────
mode INSIGHTSQLX_TAG_CLOSE;
...
// ───────── INSIGHTSQLX TEXT MODE ─────────
mode INSIGHTSQLX_TEXT;

INSIGHTSQLX_TEXT_TEXT
    : ~[<{]+ ; // everything except “{” or “<”

INSIGHTSQLX_TEXT_LBRACE
    : '{' -> type(LBRACE), pushMode(DEFAULT_MODE);

INSIGHTSQLX_TEXT_LT_SLASH
    : '</' -> type(LT_SLASH), popMode, pushMode(INSIGHTSQLX_TAG_CLOSE);

INSIGHTSQLX_TEXT_LT
    : '<' -> type(LT), pushMode(INSIGHTSQLX_TAG_OPEN);
