...
def _delete_specific_persons_via_personinsights(team_id: int, person_ids: list[int]) -> int:
    """Delete specific persons by id via personinsights.
    ...
    from insights.personinsights_client.proto import DeletePersonsRequest, GetPersonsRequest

    client = get_personinsights_client()
    if client is None:
        raise RuntimeError("personinsights client not configured")

    with personinsights_caller_tag("delete-persons/by-ids"):
        uuids: list[str] = []
...
def _delete_team_persons_batch_via_personinsights(team_id: int, batch_size: int) -> int:
    """Delete up to `batch_size` of a team's persons via personinsights, returning the count."""
    from insights.personinsights_client.caller_tag import personinsights_caller_tag
    from insights.personinsights_client.client import get_personinsights_client
    ...
    with personinsights_caller_tag("delete-persons/by-team"):
...
async def preclean_cohort_members_activity(inputs: PrecleanCohortMembersActivityInputs) -> None:
    ...
    insights_cohortpeople has no FK to insights_person, so it must be cleared explicitly.
    ...
    from insights.models.team.util import _delete_cohort_members_for_all_teams
...
async def delete_persons_activity(inputs: DeletePersonsActivityInputs) -> tuple[int, bool]:
    """Delete one batch of persons via personinsights, returning (deleted_count, should_continue)."""
    async with Heartbeater():
        logger = LOGGER.bind()
        ...
        if inputs.person_ids:
            ...
            if not id_slice:
                return 0, False
            deleted = await asyncio.to_thread(_delete_specific_persons_via_personinsights, inputs.team_id, id_slice)
            ...
            deleted = await asyncio.to_thread(
                _delete_team_persons_batch_via_personinsights, inputs.team_id, inputs.batch_size
            )
...
class DeletePersonsWorkflow(InsightsWorkflow):
    """Workflow to delete persons and their dependent rows from the persons database via personinsights.

    All deletion goes through personinsights RPCs (no direct database connection). For a
