...
produces for live ingest, and it's also the value InsightsQL falls back to for queries on
...
string extraction. Type-cast parity (toFloat / toBool / toDateTime applied at InsightsQL read
time) is covered separately in `test_property_types_dmat.py::TestDmatExtractionConsistency`.

Tests run actual Datastore SQL against an inserted row, so any drift between the SQL we
generate and the SQL Datastore actually evaluates surfaces here.

Two test classes:

- ``TestCoercionParity`` exercises the bare ``_generate_property_extraction_sql`` shape
  with the property name passed as a query parameter — the same path used by InsightsQL's
...
from insights.temporal.backfill_materialized_property.activities import _generate_property_extraction_sql
...
class TestCoercionParity:
    ...
    def test_extraction_sql_matches_fixture(self, team, case):
        ...
        # catch SQL-level drift (e.g. a Datastore upgrade that changes how `JSONExtractRaw`
        # quotes its output).
...
class TestDictBackedDispatchCoercion:
    ...
    every fixture case so plugin-server / InsightsQL JSON fallback / dict-based backfill
    agree byte-for-byte.
    """
