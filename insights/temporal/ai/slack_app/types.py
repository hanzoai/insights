"""Shared dataclasses for Insights Slack App Temporal workflows.
...
@dataclass
class InsightsSlackInboxOnboardingInputs:
    integration_id: int


@dataclass
class InsightsCodeSlackMentionWorkflowInputs:
    event: dict[str, Any]
    integration_id: int
    slack_team_id: str
...
def coerce_mention_workflow_inputs(inputs: object) -> InsightsCodeSlackMentionWorkflowInputs:
    ...
    """
    if isinstance(inputs, InsightsCodeSlackMentionWorkflowInputs):
        return inputs
    if isinstance(inputs, dict):
        known = {f.name for f in fields(InsightsCodeSlackMentionWorkflowInputs)}
        try:
            return InsightsCodeSlackMentionWorkflowInputs(**{k: v for k, v in inputs.items() if k in known})
        except TypeError as e:
            raise TypeError(
                "Could not coerce activity inputs into InsightsCodeSlackMentionWorkflowInputs "
                f"(keys={sorted(inputs)}): {e}"
            ) from e
    raise TypeError(
        f"Unexpected activity inputs type {type(inputs).__name__}; "
        "expected InsightsCodeSlackMentionWorkflowInputs or dict"
    )
...
class SlackAppMentionWorkflowInputs:
    ...
    ``InsightsCodeSlackMentionWorkflowInputs``. These fields exist only to
    carry state across ``continue_as_new`` — fresh starts leave them empty.
    """

    pending_messages: list[InsightsCodeSlackMentionWorkflowInputs] = field(default_factory=list)
...
@dataclass
class InsightsCodeSlackMentionCommandWorkflowInputs:
    event: dict[str, Any]
    ...
    # ``@Insights`` for mentions, ``/insights`` for the slash command.
    command_prefix: str = "@Insights"


@dataclass
class InsightsCodeRepoCascadeOutcome:
...
@dataclass
class InsightsCodeRulesCommandResult:
    status: str  # "not_a_command" | "handled" | "needs_picker"
    pending_rule_text: str | None = None


@dataclass
class InsightsCodeSlackMentionCommandResult:
