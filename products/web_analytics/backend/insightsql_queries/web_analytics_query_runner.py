...
from insights.insightsql.property import action_to_expr, apply_path_cleaning, get_property_type, property_to_expr
...
from insights.datastore.query_tagging import Feature, Product, get_query_tag_value, tag_queries
...
from products.web_analytics.backend.insightsql_queries.first_pageview_attribution import first_pageview_session_filter_expr
...
from products.web_analytics.backend.insightsql_queries.metrics import (
    WEB_ANALYTICS_QUERY_COUNTER,
    WEB_ANALYTICS_QUERY_DURATION,
    WEB_ANALYTICS_QUERY_ERRORS,
)
from products.web_analytics.backend.insightsql_queries.traffic_type import get_traffic_category_expr, get_traffic_type_expr
...
class WebAnalyticsQueryRunner(AnalyticsQueryRunner[WAR], ABC):
    ...
    def datastore_query_type(self) -> str | None:
        return None
    ...
    def calculate(self) -> WAR:
        ...
        with bound_contextvars(filters_eligibility_hash=self.filters_eligibility_hash):
            # Tag everything Datastore will see for this request in one call so
            ...
            # layer (`insights/api/services/query.py`) tags only the wrapping
            ...
            # payload: `system.query_log` retention is sub-day on prod Datastore,
            ...
            datastore_query_type: str | None = None
            ...
            try:
                ...
                try:
                    ...
                    datastore_query_type = self.datastore_query_type()
                except Exception:
                    query_strategy = query_strategy or "strategy_resolution_failed"
                    datastore_query_type = datastore_query_type or None
                ...
                logger.info(
                    ...
                    datastore_query_type=datastore_query_type,
                    breakdown=breakdown_label,
                    has_conversion_goal=has_conversion_goal,
                    pre_compute_strategy=pre_compute_strategy_label,
        ...
        return ast.Call(
            ...
            args=[
                ast.CompareOperation(
                    ...
                    right=self.query_date_range.date_from_as_insightsql(),
    ...
    def _previous_period_expression(self, field="start_timestamp"):
        ...
        return ast.Call(
            ...
            args=[
                ast.CompareOperation(
                    left=ast.Field(chain=[field]),
                    right=self.query_compare_to_date_range.date_from_as_insightsql(),
                    op=ast.CompareOperationOp.GtEq,
                ),
                ast.CompareOperation(
                    left=ast.Field(chain=[field]),
                    right=self.query_compare_to_date_range.date_to_as_insightsql(),
                    op=ast.CompareOperationOp.LtEq,
                ),
            ],
        )
    ...
    def session_where(self, include_previous_period: Optional[bool] = None):
        properties = [
            parse_expr(
                ...
                placeholders={
                    "date_from": (
                        self.query_date_range.previous_period_date_from_as_insightsql()
                        if include_previous_period
                        else self.query_date_range.date_from_as_insightsql()
                    ),
                    "date_to": self.query_date_range.date_to_as_insightsql(),
                },
            ),
        ...
        return property_to_expr(
            [
                parse_expr(
                    ...
                    placeholders={"date_from": self.query_date_range.date_from_as_insightsql()},
                ),
                parse_expr(
                    "events.timestamp <= {date_to}",
                    placeholders={"date_to": self.query_date_range.date_to_as_insightsql()},
                ),
            ],
            self.team,
        )
