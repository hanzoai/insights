...
Source of truth for the code: `products/web_analytics/backend/insightsql_queries/`.
...
Every strategy tags its Datastore queries with a `query_type` that lands in `system.query_log → log_comment.query_type`, so the tag reference at the bottom doubles as a triage tool.
