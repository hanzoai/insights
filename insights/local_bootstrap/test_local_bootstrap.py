...
from insights.local_bootstrap.config import BootstrapConfig, BootstrapConfigError, S3Location, TableImportConfig
from insights.local_bootstrap.importer import (
    ZERO_UUID,
    Progress,
    _accumulate_persons,
    _event_row_to_ch,
...
from insights.local_bootstrap.source import _matches_format
...
class TestEventRowMapping:
    def test_maps_all_datastore_columns_with_defaults(self):
        from insights.local_bootstrap.importer import _EVENT_COLUMNS
...
class TestAccumulatePersons:
    def _run(self, monkeypatch, rows):
        monkeypatch.setattr(
            "insights.local_bootstrap.importer.iter_table_rows",
...
class TestWritePersonsToPostgres:
    ...
    def _cleanup(self) -> None:
        ...
        with persons_db_connection(writer=True, autocommit=True) as conn, conn.cursor() as cursor:
            cursor.execute("DELETE FROM insights_persondistinctid WHERE team_id = %s", (self.TEAM_ID,))
        ...
        try:
            ...
            with persons_db_connection(writer=False) as conn, conn.cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    f"FROM insights_persondistinctid pdi JOIN {settings.PERSON_TABLE_NAME} p ON p.id = pdi.person_id "
                    "WHERE pdi.team_id = %s",
                    (self.TEAM_ID,),
                )
