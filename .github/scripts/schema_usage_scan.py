#!/usr/bin/env python3
"""Map insights.schema types to the products that depend on them.
...
WILDCARD = "*"
# schema_enums holds the enum classes split out of insights.schema — same schema.json names,
# so its importers depend on schema changes exactly like insights.schema importers do.
SCHEMA_MODULES = ("insights.schema", "insights.schema_enums")


def _product_name(rel_path: Path) -> str | None:
    # hyphenated to match @hanzo/products-<name>
...
class _SchemaUsageVisitor(ast.NodeVisitor):
    """Resolve insights.schema type references within a single module."""

    def __init__(self) -> None:
        self.types: set[str] = set()
        self.module_aliases: set[str] = set()  # names bound to insights.schema (e.g. `schema`, `ps`)
        self.insights_pkg = False  # `import insights[.schema]` → resolve `insights.schema.Foo`
        ...
        for alias in node.names:
            if alias.name in SCHEMA_MODULES:
                if alias.asname:
                    ...
                    self.insights_pkg = True
            elif alias.name == "insights" and alias.asname is None:
                self.insights_pkg = True
        self.generic_visit(node)

    def visit_Attribute(self, node: ast.Attribute) -> None:
        value = node.value
        # insights.schema.Foo / insights.schema_enums.Foo
        if (
            self.insights_pkg
            and isinstance(value, ast.Attribute)
            and value.attr in ("schema", "schema_enums")
            and isinstance(value.value, ast.Name)
            and value.value.id == "insights"
        ):
