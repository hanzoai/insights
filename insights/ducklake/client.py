...
from insights.ducklake.common import get_duckgres_config_for_org, is_dev_mode, sanitize_ducklake_identifier
from insights.ducklake.table_binding import bind_tables_to_ducklake
...
def make_duckgres_conninfo(team_id: int, *, organization_id: str | None = None) -> str:
    from insights.ducklake.common import _duckgres_dev_config, _get_org_id_for_team
...
_SEARCH_PATH_SCHEMAS = ["revenue", "stripe", "billing_public", "credit", "insights"]
...
def compile_insightsql_to_ducklake_sql(
    team_id: int,
    ...
    *,
    ...
    """Compile a InsightsQLQuery to DuckDB-dialect SQL for DuckLake.

    Returns ``(postgres_sql, values, insightsql_pretty)``. The ``values`` dict holds
    ...
    # Build the database up front so warehouse tables can be bound from the Datastore
    ...
    # Separate context for the DuckDB print — the InsightsQL round-trip below shouldn't
    # contribute to ``postgres_context.values``.
    postgres_context = InsightsQLContext(
        team_id=team_id,
        user=user,
    ...
    insightsql_pretty, _ = prepare_and_print_ast(parsed, insightsql_context, dialect="insightsql")

    return postgres_sql, dict(postgres_context.values), insightsql_pretty
...
def execute_ducklake_query(
    ...
    *,
    sql: str | None = None,
    query: InsightsQLQuery | None = None,
    ...
    Accepts either raw SQL or a InsightsQLQuery (which gets compiled to
    ...
    Team lookup used for variable substitution. Pass user so InsightsQL compilation
    ...
    insightsql_pretty: str | None = None
    values: dict[str, object] = {}
    if query:
        sql, values, insightsql_pretty = compile_insightsql_to_ducklake_sql(
            team_id,
            query,
            team=team,
            user=user,
...
def execute_ducklake_create_table(
    ...
    *,
    ...
    (as produced by ``compile_insightsql_to_ducklake_sql``). It is passed through to
