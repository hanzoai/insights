...
from products.warehouse_sources.backend.temporal.data_imports.sources.salesflare.settings import ENDPOINTS
...
class TestSalesflareSource:
    ...
    def test_get_source_config(self) -> None:
        ...
        assert config.docsUrl == "https://hanzo.ai/docs/cdp/sources/salesflare"
    ...
    @parameterized.expand(
        [
            ...
            ("connection_error", 0, False, "Could not connect to Salesflare: boom"),
        ]
    )
    @mock.patch("products.warehouse_sources.backend.temporal.data_imports.sources.salesflare.salesflare.check_access")
    def test_validate_credentials(
        ...
        message = (
            "Salesflare returned HTTP 500"
            if status == 500
            else ("Could not connect to Salesflare: boom" if status == 0 else None)
        )
    ...
    def test_get_resumable_source_manager_binds_resume_config(self) -> None:
        ...
        assert manager._data_class is SalesflareResumeConfig
