...
- A GitHub integration for the Insights org whose App installation covers `Insights/insights` with `Contents: Read & Write` and `Pull requests: Read & Write`. Note its integration id.
...
```json
{
    ...
    "repositories": [
        { "github_integration_id": GITHUB_INTEGRATION_ID, "full_name": "Insights/insights" }
    ],
...
- Leave `connectors.insights_mcp_scopes` at its `read_only` default and attach no MCP Store installations; the sweep needs neither.
