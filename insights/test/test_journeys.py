...
from insights.models import Person, Team
from insights.models.event.sql import EVENTS_DATA_TABLE, EVENTS_JSON_DATA_TABLE
from insights.models.event.util import _json_dumps_for_datastore
...
def journeys_for(
    ...
    """
    from insights.test.persons import create_people_bulk  # noqa: PLC0415
    ...
    # One bulk Datastore insert per table instead of two inserts per person.
    ...
    for event in all_events:
        ...
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA:
            json_rows += _event_row_values(
                in_memory_event,
                properties=_json_dumps_for_datastore(in_memory_event.properties),
                person_properties=_json_dumps_for_datastore(in_memory_event.person_properties),
                created_at=created_at,
            )
...
def _event_row_values(
    ...
    *,
    ...
    values = [
        ...
        f"'{person_properties}'",
        f"'{_format_datastore_timestamp(in_memory_event.person_created_at)}'",
        ...
        f"'{_format_datastore_timestamp(in_memory_event.group0_created_at)}'",
        f"'{_format_datastore_timestamp(in_memory_event.group1_created_at)}'",
        f"'{_format_datastore_timestamp(in_memory_event.group2_created_at)}'",
        f"'{_format_datastore_timestamp(in_memory_event.group3_created_at)}'",
        f"'{_format_datastore_timestamp(in_memory_event.group4_created_at)}'",
        f"'{created_at}'",
        "now()",
        "0",
    ]
...
def _format_datastore_timestamp(value: datetime) -> str:
...
def update_or_create_person(distinct_ids: list[str], team_id: int, **kwargs):
    from insights.test.persons import create_person, update_person  # noqa: PLC0415
