...
2. In your `max_tools.py`, define a new tool class inheriting from `MaxTool`:
   ...
   from ee.insightsai.llm import MaxChatOpenAI
   from ee.insightsai.tool import MaxTool
...
For an example, see `ee/insightsai/tools`:
...
2. Update `TOOLS_WITHOUT_ACCESS_CONTROL` in `ee/insightsai/test/test_tool.py` to remove your tool from the exempt list.
...
If your tool doesn't need access control (read-only, no protected resources), add it to `TOOLS_WITHOUT_ACCESS_CONTROL` in `ee/insightsai/test/test_tool.py`.
...
1. **Update the query executor and formatters** (`@ee/insightsai/context/insight/`):
...
```python
from pydantic import BaseModel, Field
from ee.insightsai.chat_agent.taxonomy.toolkit import TaxonomyAgentToolkit
from ee.insightsai.chat_agent.taxonomy.tools import base_final_answer
...
from ee.insightsai.chat_agent.taxonomy.nodes import TaxonomyAgentNode, TaxonomyAgentToolsNode
from ee.insightsai.chat_agent.taxonomy.agent import TaxonomyAgent
from ee.insightsai.chat_agent.taxonomy.types import TaxonomyAgentState
...
class LoopNode(TaxonomyAgentNode[TaxonomyAgentState, TaxonomyAgentState[MaxToolTaxonomyOutput]]):
    ...
    def _get_system_prompt(self) -> ChatPromptTemplate:
        ...
        The taxonomy agent comes with some prepackaged default prompts. Check them here ee/insightsai/graph/taxonomy/prompts.py
        """
        system = [
