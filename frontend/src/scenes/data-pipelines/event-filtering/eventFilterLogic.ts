...
// Limits — must match MAX_CONDITIONS and MAX_TREE_DEPTH in insights/models/event_filter_config.py
...
export const eventFilterLogic = kea<eventFilterLogicType>([
    ...
    forms(({ values }) => ({
        filterForm: {
            ...
            submit: async (formValues) => {
                ...
                await api.create(`v1/environments/${currentTeamId}/event_filter/`, payload)
                toast.success('Event filter saved')
            },
        },
    })),
    ...
    afterMount(({ actions, values }) => {
        const { currentTeamId } = values
        api.get(`v1/environments/${currentTeamId}/event_filter/`)
            .then((data) => {
                if (!data) {
                    return
                }
            ...
            .catch((error) => {
                toast.error(`Failed to load event filter config: ${error.message ?? error}`)
            })
    }),
])
