...
-- Full CREATE schema for the local-multi/data node. Apply to a fresh Datastore to build it.

CREATE TABLE insights.adhoc_events_deletion (
  team_id Int64,
  uuid UUID,
  created_at DateTime64(6, 'UTC') DEFAULT now64(),
  deleted_at DateTime,
  is_deleted UInt8 DEFAULT 0
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.adhoc_events_deletion', '{replica}-{shard}', deleted_at, is_deleted) ORDER BY (team_id, uuid) TTL deleted_at + toIntervalMonth(3) WHERE is_deleted = 1 SETTINGS index_granularity = 8192;
CREATE TABLE insights.ai_events (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('ai_events', 'insights', 'sharded_ai_events', cityHash64(concat(toString(team_id), '-', trace_id, '-', toString(toDate(timestamp)))));
CREATE TABLE insights.channel_definition (
  domain String,
  kind String,
  domain_type Nullable(String),
  type_if_paid Nullable(String),
  type_if_organic Nullable(String)
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.channel_definition', '{replica}-{shard}') ORDER BY (domain, kind) SETTINGS index_granularity = 8192;
CREATE TABLE insights.cohort_membership (
  team_id Int64,
  cohort_id Int64,
  person_id UUID,
  status Enum8('entered'=1, 'left'=2),
  last_updated DateTime64(6) DEFAULT now64()
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.cohort_membership', '{replica}-{shard}', last_updated) ORDER BY (team_id, cohort_id, person_id) SETTINGS index_granularity = 8192;
CREATE TABLE insights.cohortpeople (
  person_id UUID,
  cohort_id Int64,
  team_id Int64,
  sign Int8,
  version UInt64
) ENGINE = ReplicatedCollapsingMergeTree('/datastore/tables/noshard/insights.cohortpeople', '{replica}-{shard}', sign) ORDER BY (team_id, cohort_id, person_id, version) SETTINGS index_granularity = 8192;
...
) ENGINE = Distributed('aux', 'insights', 'sharded_conversion_goal_attributed_preaggregated', cityHash64(person_id));
CREATE TABLE insights.distributed_system_processes (
...
) ENGINE = Distributed('insights', 'system', 'processes') SETTINGS skip_unavailable_shards = 1;
CREATE TABLE insights.duplicate_events (
  team_id Int64,
  distinct_id String,
  event String,
  source_uuid UUID,
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.duplicate_events', '{replica}-{shard}') ORDER BY (team_id, distinct_id, event, inserted_at) PARTITION BY toYYYYMMDD(inserted_at) TTL inserted_at + toIntervalDay(7) SETTINGS index_granularity = 512;
CREATE TABLE insights.error_tracking_fingerprint_issue_state (
  team_id Int64,
  fingerprint String,
  issue_id UUID,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('aux', 'insights', 'raw_error_tracking_fingerprint_issue_state');
CREATE TABLE insights.error_tracking_issue_fingerprint_overrides (
  team_id Int64,
  fingerprint String,
  issue_id UUID,
  is_deleted Int8,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.error_tracking_issue_fingerprint_overrides', '{replica}-{shard}', version) ORDER BY (team_id, fingerprint) SETTINGS index_granularity = 512;
CREATE TABLE insights.events_dead_letter_queue (
  id UUID,
  event_uuid UUID,
  event String,
  properties String,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.events_dead_letter_queue', '{replica}-{shard}', _timestamp) ORDER BY (id, event_uuid, distinct_id, team_id) TTL toDate(_timestamp) + toIntervalWeek(4) SETTINGS index_granularity = 512;
CREATE TABLE insights.exchange_rate (
  currency String,
  date Date,
  rate Decimal(18, 10),
  version UInt32 DEFAULT toUnixTimestamp(now())
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.exchange_rate', '{replica}-{shard}', version) ORDER BY (date, currency) SETTINGS index_granularity = 8192;
CREATE TABLE insights.experiment_metric_events_preaggregated (
  team_id Int64,
  job_id UUID,
  entity_id String,
  timestamp DateTime64(6, 'UTC'),
...
CREATE TABLE insights.kafka_events_json (
  uuid UUID,
  event String,
...
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'msk_cluster', kafka_format = 'kafka_format = \'JSONEachRow\'', kafka_group_name = 'kafka_group_name = \'group1\'', kafka_skip_broken_messages = 100, kafka_topic_list = 'kafka_topic_list = \'datastore_events_json\'';
CREATE TABLE insights.kafka_performance_events (
  uuid UUID,
  session_id String,
  window_id String,
  pageview_id String,
...
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'msk_cluster', kafka_format = 'kafka_format = \'JSONEachRow\'', kafka_group_name = 'kafka_group_name = \'group1\'', kafka_topic_list = 'kafka_topic_list = \'datastore_performance_events\'';
CREATE TABLE insights.kafka_person_distinct_id (
  distinct_id String,
  person_id UUID,
  team_id Int64,
  _sign Nullable(Int8),
  is_deleted Nullable(Int8)
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'msk_cluster', kafka_format = 'kafka_format = \'JSONEachRow\'', kafka_group_name = 'kafka_group_name = \'group1\'', kafka_topic_list = 'kafka_topic_list = \'datastore_person_unique_id\'';
CREATE TABLE insights.kafka_person_overrides (
  team_id Int32,
  old_person_id UUID,
  ...
  version Int32
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'kafka:9092', kafka_format = 'JSONEachRow', kafka_group_name = 'datastore-person-overrides', kafka_topic_list = 'datastore_person_override';
CREATE TABLE insights.llma_metrics_daily (
  date Date,
  team_id UInt64,
  metric_name String,
  metric_value Float64
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.llma_metrics_daily', '{replica}-{shard}') ORDER BY (team_id, date, metric_name) PARTITION BY toYYYYMM(date) SETTINGS index_granularity = 8192;
CREATE TABLE insights.marketing_conversions_preaggregated (
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at Date DEFAULT today() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_marketing_conversions_preaggregated', cityHash64(person_id));
CREATE TABLE insights.marketing_costs_preaggregated (
  team_id Int64,
  job_id UUID,
  source_id String,
  source_name String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at Date DEFAULT today() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_marketing_costs_preaggregated', cityHash64(source_name, campaign_id));
CREATE TABLE insights.marketing_touchpoints_preaggregated (
  team_id Int64,
  job_id UUID,
  person_id UUID,
  touchpoint_timestamp DateTime64(6, 'UTC'),
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at Date DEFAULT today() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_marketing_touchpoints_preaggregated', cityHash64(person_id));
CREATE TABLE insights.message_assets (
  team_id Int64,
  function_kind LowCardinality(String),
  function_id String,
  parent_run_id String,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('aux', 'insights', 'message_assets_data');
CREATE TABLE insights.partitioned_sharded_insights_document_embeddings (
  team_id Int64,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.person', '{replica}-{shard}', version) ORDER BY (team_id, id) SETTINGS index_granularity = 8192;
CREATE TABLE insights.person_distinct_id (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedCollapsingMergeTree('/datastore/tables/noshard/insights.person_distinct_id', '{replica}-{shard}', _sign) ORDER BY (team_id, distinct_id, person_id) SETTINGS index_granularity = 8192;
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.person_distinct_id2', '{replica}-{shard}', version) ORDER BY (team_id, distinct_id) SETTINGS index_granularity = 512;
CREATE TABLE insights.person_distinct_id_overrides (
  team_id Int64,
  distinct_id String,
  person_id UUID,
  is_deleted Int8,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.person_distinct_id_overrides', '{replica}-{shard}', version) ORDER BY (team_id, distinct_id) SETTINGS index_granularity = 512;
CREATE TABLE insights.person_overrides (
  team_id Int32,
  old_person_id UUID,
  override_person_id UUID,
  merged_at DateTime64(6, 'UTC'),
  ...
  version Int32
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.person_overrides', '{replica}-{shard}', version) ORDER BY (team_id, old_person_id) PARTITION BY toYYYYMM(oldest_event) SETTINGS index_granularity = 8192;
CREATE TABLE insights.person_static_cohort (
  id UUID,
  person_id UUID,
  cohort_id Int64,
  team_id Int64,
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.person_static_cohort', '{replica}-{shard}', _timestamp) ORDER BY (team_id, cohort_id, person_id, id) SETTINGS index_granularity = 8192;
CREATE TABLE insights.pg_embeddings (
  domain String,
  team_id Int64,
  id String,
  ...
  is_deleted UInt8
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.pg_embeddings', '{replica}-{shard}', timestamp, is_deleted) ORDER BY (team_id, domain, id) SETTINGS index_granularity = 512;
CREATE TABLE insights.plugin_log_entries (
  id UUID,
  team_id Int64,
  plugin_id Int64,
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.plugin_log_entries', '{replica}-{shard}', _timestamp) ORDER BY (team_id, plugin_id, plugin_config_id, timestamp) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(timestamp) + toIntervalWeek(1) SETTINGS index_granularity = 512;
CREATE TABLE insights.property_definitions (
  team_id UInt32,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.property_definitions', '{replica}-{shard}', version) ORDER BY (team_id, type, coalesce(event, ''), name, coalesce(group_type_index, 255)) SETTINGS index_granularity = 8192;
CREATE TABLE insights.property_values_distributed (
...
) ENGINE = Distributed('aux', 'insights', 'property_values');
...
CREATE TABLE insights.query_log_archive_v2 (
  hostname LowCardinality(String),
  user LowCardinality(String),
  query_id String,
  initial_query_id String,
  ...
  lc_modifiers String
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.query_log_archive_new', '{replica}-{shard}') ORDER BY (team_id, event_date, event_time, query_id) PARTITION BY toYYYYMM(event_date) SETTINGS index_granularity = 8192;
CREATE TABLE insights.session_replay_features (
  session_id String,
  team_id Int64,
  distinct_id String,
...
) ENGINE = Distributed('aux', 'insights', 'sharded_session_replay_features', sipHash64(session_id));
CREATE TABLE insights.sharded_app_metrics (
  team_id Int64,
  timestamp DateTime64(6, 'UTC'),
  plugin_config_id Int64,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{shard}/insights.sharded_app_metrics', '{replica}') ORDER BY (team_id, plugin_config_id, job_id, category, toStartOfHour(timestamp), error_type, error_uuid) PARTITION BY toYYYYMM(timestamp) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_app_metrics2 (
  team_id Int64,
  timestamp DateTime64(6, 'UTC'),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{shard}/insights.sharded_app_metrics2', '{replica}') ORDER BY (team_id, app_source, app_source_id, instance_id, toStartOfHour(timestamp), metric_kind, metric_name) PARTITION BY toYYYYMM(timestamp) TTL toDate(timestamp) + toIntervalDay(90) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_distinct_id_usage (
  team_id Int64,
  distinct_id String,
  minute DateTime,
  event_count UInt64
) ENGINE = ReplicatedSummingMergeTree('/datastore/tables/{shard}/insights.distinct_id_usage', '{replica}', (event_count)) ORDER BY (team_id, minute, distinct_id) PARTITION BY toYYYYMMDD(minute) TTL toDate(minute) + toIntervalDay(7) SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_events (
  uuid UUID,
  event String,
  properties String CODEC(ZSTD(3)),
  timestamp DateTime64(6, 'UTC'),
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.events', '{replica}', _timestamp) ORDER BY (team_id, toDate(timestamp), event, cityHash64(distinct_id), cityHash64(uuid)) PARTITION BY toYYYYMM(timestamp) SAMPLE BY cityHash64(distinct_id) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_events_json (
  uuid UUID,
  event String,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_events_recent', '{replica}', _timestamp) ORDER BY (team_id, toStartOfHour(inserted_at), event, cityHash64(distinct_id), cityHash64(uuid)) PARTITION BY toStartOfDay(inserted_at) TTL toDateTime(inserted_at) + toIntervalDay(7) SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_experiment_exposures_preaggregated (
  team_id Int64,
  job_id UUID,
  entity_id String,
  variant String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at Date DEFAULT today() + toIntervalDay(7)
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.experiment_exposures_preaggregated', '{replica}', computed_at) ORDER BY (team_id, job_id, entity_id, breakdown_value) PARTITION BY toYYYYMMDD(expires_at) TTL expires_at SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_flag_evaluations (
  team_id Int64,
  uuid UUID,
  timestamp DateTime64(6, 'UTC'),
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.flag_evaluations', '{replica}') ORDER BY (team_id, flag_key, toDate(timestamp), cityHash64(distinct_id)) PARTITION BY toYYYYMM(timestamp) TTL toDate(timestamp) + toIntervalDay(90) SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_heatmaps (
  session_id String,
  team_id Int64,
  distinct_id String,
  timestamp DateTime64(6, 'UTC'),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.heatmaps', '{replica}') ORDER BY (type, team_id, toDate(timestamp), current_url, viewport_width) PARTITION BY toYYYYMM(timestamp) TTL toDate(timestamp) + toIntervalDay(90) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_ingestion_warnings (
  team_id Int64,
  source LowCardinality(String),
  type String,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.sharded_ingestion_warnings', '{replica}') ORDER BY (team_id, toHour(timestamp), type, source, timestamp) PARTITION BY toYYYYMMDD(timestamp) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_log_entries (
  team_id UInt64,
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_log_entries', '{replica}', _timestamp) ORDER BY (team_id, log_source, log_source_id, instance_id, timestamp) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(timestamp) + toIntervalDay(90) SETTINGS index_granularity = 1024, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_performance_events (
  uuid UUID,
  session_id String,
  window_id String,
  pageview_id String,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.performance_events', '{replica}') ORDER BY (team_id, toDate(timestamp), session_id, pageview_id, timestamp) PARTITION BY toYYYYMM(timestamp) TTL toDate(timestamp) + toIntervalWeek(3) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_insights_document_embeddings_buffer (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  model_name LowCardinality(String),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_insights_document_embeddings_buffer', '{replica}', inserted_at) ORDER BY (inserted_at, model_name, cityHash64(document_id)) PARTITION BY toDate(inserted_at) TTL inserted_at + toIntervalDay(1) SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_insights_document_embeddings_text_embedding_3_large_3072 (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  rendering LowCardinality(String),
  ...
  INDEX kafka_timestamp_minmax_sharded_insights_document_embeddings_text_embedding_3_large_3072 _timestamp TYPE minmax GRANULARITY 3,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_insights_document_embeddings_text_embedding_3_large_3072', '{replica}', inserted_at) ORDER BY (team_id, toDate(timestamp), product, document_type, rendering, cityHash64(document_id)) PARTITION BY toMonday(timestamp) TTL timestamp + toIntervalMonth(3) SETTINGS index_granularity = 512, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_insights_document_embeddings_text_embedding_3_small_1536 (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  rendering LowCardinality(String),
  ...
  INDEX kafka_timestamp_minmax_sharded_insights_document_embeddings_text_embedding_3_small_1536 _timestamp TYPE minmax GRANULARITY 3,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_insights_document_embeddings_text_embedding_3_small_1536', '{replica}', inserted_at) ORDER BY (team_id, toDate(timestamp), product, document_type, rendering, cityHash64(document_id)) PARTITION BY toMonday(timestamp) TTL timestamp + toIntervalMonth(3) SETTINGS index_granularity = 512, ttl_only_drop_parts = 1;
CREATE TABLE insights.sharded_preaggregation_results (
  team_id Int64,
  job_id UUID,
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{shard}/insights.preaggregation_results', '{replica}') ORDER BY (team_id, job_id, time_window_start, breakdown_value) PARTITION BY toYYYYMM(time_window_start) TTL expires_at SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_precalculated_events (
  team_id Int64,
  date Date,
  distinct_id String,
  person_id UUID,
  ...
  _partition UInt64,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_precalculated_events', '{replica}', _timestamp) ORDER BY (team_id, condition, date, distinct_id, uuid) PARTITION BY toYYYYMM(date) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_precalculated_person_properties (
  team_id Int64,
  distinct_id String,
  person_id UUID,
  condition String,
  ...
  _timestamp DateTime64(6),
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{shard}/insights.sharded_precalculated_person_properties', '{replica}', _timestamp) ORDER BY (team_id, condition, distinct_id) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_query_log_archive_old (
  hostname LowCardinality(String),
  user LowCardinality(String),
  query_id String,
  initial_query_id String,
  ...
  lc_dagster__owner String,
  lc_modifiers String
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.sharded_query_log_archive', '{replica}') ORDER BY (team_id, event_date, event_time, query_id) PARTITION BY toYYYYMM(event_date) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_raw_sessions (
  team_id Int64,
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{shard}/insights.raw_sessions', '{replica}') ORDER BY (team_id, toStartOfHour(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(session_id_v7, 80)), 1000))), cityHash64(session_id_v7), session_id_v7) PARTITION BY toYYYYMM(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(session_id_v7, 80)), 1000))) SAMPLE BY cityHash64(session_id_v7) SETTINGS index_granularity = 8192;
CREATE TABLE insights.sharded_raw_sessions_v3 (
  team_id Int64,
  session_id_v7 UInt128,
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{shard}/insights.raw_sessions_v3', '{replica}') ORDER BY (team_id, session_timestamp, session_id_v7) PARTITION BY toYYYYMM(session_timestamp) SETTINGS index_granularity = 8192, max_delay_to_insert = 10, parts_to_delay_insert = 250, parts_to_throw_insert = 1000;
CREATE TABLE insights.sharded_session_replay_embeddings (
  session_id String,
  team_id Int64,
  ...
  input String
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.session_replay_embeddings', '{replica}') ORDER BY (toDate(generation_timestamp), team_id, session_id) PARTITION BY toYYYYMM(generation_timestamp) TTL toDate(generation_timestamp) + toIntervalYear(1) SETTINGS index_granularity = 512;
CREATE TABLE insights.sharded_session_replay_events (
  session_id String,
  team_id Int64,
  distinct_id String,
  min_first_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{shard}/insights.session_replay_events', '{replica}') ORDER BY (toDate(min_first_timestamp), team_id, session_id) PARTITION BY toYYYYMM(min_first_timestamp) SETTINGS index_granularity = 512;
CREATE TABLE insights.sharded_sessions (
  session_id String,
  team_id Int64,
...
) ENGINE = Distributed('aux', 'insights', 'sharded_usage_report_events_preagg', sipHash64(date));
CREATE TABLE insights.web_bot_definition (
  id UInt64,
  parent_id UInt64,
  regexp String,
  keys Array(String),
  values Array(String)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_bot_definition', sipHash64(id));
CREATE TABLE insights.web_bounces_dimensional_preaggregated (
  team_id Int64,
  job_id UUID,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_bounces_dimensional_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_goals_preaggregated (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  action_id Int64,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_goals_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_overview_preaggregated (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_overview_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_pre_aggregated_bounces (
  period_bucket DateTime,
  team_id UInt64,
  host String,
  device_type String,
...
CREATE TABLE insights.web_pre_aggregated_bounces_staging (
  ...
  total_session_duration_state AggregateFunction(sum, Int64),
  total_session_count_state AggregateFunction(sum, UInt64)
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.web_pre_aggregated_bounces_staging', '{replica}-{shard}') ORDER BY (team_id, period_bucket, host, device_type, entry_pathname, end_pathname, browser, os, viewport_width, viewport_height, referring_domain, utm_source, utm_medium, utm_campaign, utm_term, utm_content, country_code, city_name, region_code, region_name, has_gclid, has_gad_source_paid_search, has_fbclid, mat_metadata_loggedIn, mat_metadata_backend) PARTITION BY toYYYYMMDD(period_bucket) SETTINGS index_granularity = 8192;
CREATE TABLE insights.web_pre_aggregated_stats (
  period_bucket DateTime,
  team_id UInt64,
  host String,
  device_type String,
  ...
  sessions_uniq_state AggregateFunction(uniq, String),
  pageviews_count_state AggregateFunction(sum, UInt64)
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.web_pre_aggregated_stats', '{replica}-{shard}') ORDER BY (team_id, period_bucket, host, device_type, pathname, entry_pathname, end_pathname, browser, os, viewport_width, viewport_height, referring_domain, utm_source, utm_medium, utm_campaign, utm_term, utm_content, country_code, city_name, region_code, region_name, has_gclid, has_gad_source_paid_search, has_fbclid, mat_metadata_loggedIn, mat_metadata_backend) PARTITION BY toYYYYMMDD(period_bucket) SETTINGS index_granularity = 8192;
CREATE TABLE insights.web_pre_aggregated_stats_staging (
  period_bucket DateTime,
  team_id UInt64,
  host String,
  device_type String,
  ...
  sessions_uniq_state AggregateFunction(uniq, String),
  pageviews_count_state AggregateFunction(sum, UInt64)
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.web_pre_aggregated_stats_staging', '{replica}-{shard}') ORDER BY (team_id, period_bucket, host, device_type, pathname, entry_pathname, end_pathname, browser, os, viewport_width, viewport_height, referring_domain, utm_source, utm_medium, utm_campaign, utm_term, utm_content, country_code, city_name, region_code, region_name, has_gclid, has_gad_source_paid_search, has_fbclid, mat_metadata_loggedIn, mat_metadata_backend) PARTITION BY toYYYYMMDD(period_bucket) SETTINGS index_granularity = 8192;
CREATE TABLE insights.web_pre_aggregated_teams (
  team_id UInt64,
  enabled_by String DEFAULT 'system',
  version UInt32 DEFAULT toUnixTimestamp(now())
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.web_analytics_team_selection', '{replica}-{shard}', version) ORDER BY (team_id) SETTINGS index_granularity = 8192;
CREATE TABLE insights.web_stats_dimensional_preaggregated (
  team_id Int64,
  job_id UUID,
  period_bucket DateTime,
  host String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_stats_dimensional_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_stats_frustration_preaggregated (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_stats_frustration_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_stats_paths_preaggregated (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  breakdown_value String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_stats_paths_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_stats_paths_preaggregated_pathkey (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  breakdown_value String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_stats_paths_preaggregated_pathkey', sipHash64(breakdown_value));
CREATE TABLE insights.web_stats_preaggregated (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  breakdown_by String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_stats_preaggregated', sipHash64(job_id));
CREATE TABLE insights.web_vitals_paths_preaggregated (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  path String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7)
) ENGINE = Distributed('aux', 'insights', 'sharded_web_vitals_paths_preaggregated', sipHash64(job_id));
CREATE TABLE insights.writable_events (
  uuid UUID,
  event String,
  properties String CODEC(ZSTD(3)),
  timestamp DateTime64(6, 'UTC'),
...
CREATE TABLE insights.writable_events_json (
  uuid UUID,
  event String,
  properties JSON(max_dynamic_types=8, max_dynamic_paths=256, `$active_feature_flags` Array(String), `$ai_experiment_id` Nullable(String), `$ai_http_status` Nullable(String), `$ai_is_error` Nullable(String), `$ai_model` Nullable(String), `$ai_parent_id` Nullable(String), `$ai_prompt_name` Nullable(String), `$ai_provider` Nullable(String), `$ai_session_id` Nullable(String), `$ai_span_id` Nullable(String), `$ai_total_cost_usd` Nullable(String), `$ai_trace_id` Nullable(String), `$anon_distinct_id` Nullable(String), `$app_build` Nullable(String), `$app_namespace` Nullable(String), `$app_version` Nullable(String), `$browser` Nullable(String), `$browser_version` Nullable(String), `$current_url` Nullable(String), `$device` Nullable(String), `$device_id` Nullable(String), `$device_model` Nullable(String), `$device_type` Nullable(String), `$el_text` Nullable(String), `$event_type` Nullable(String), `$exception_fingerprint` Nullable(String), `$exception_functions` Array(String), `$exception_issue_id` Nullable(String), `$exception_sources` Array(String), `$exception_types` Array(String), `$exception_values` Array(String), `$feature_flag` Nullable(String), `$feature_flag_payloads` Nullable(String), `$feature_flag_response` Nullable(String), `$geoip_city_name` Nullable(String), `$geoip_country_code` Nullable(String), `$geoip_country_name` Nullable(String), `$geoip_subdivision_1_code` Nullable(String), `$group_0` Nullable(String), `$group_1` Nullable(String), `$group_2` Nullable(String), `$group_3` Nullable(String), `$group_4` Nullable(String), `$groups` Nullable(String), `$host` Nullable(String), `$initial_pathname` Nullable(String), `$initial_referrer` Nullable(String), `$initial_referring_domain` Nullable(String), `$ip` Nullable(String), `$is_identified` Nullable(String), `$lib` Nullable(String), `$lib_custom_api_host` Nullable(String), `$lib_version` Nullable(String), `$lib_version__minor` Nullable(String), `$os` Nullable(String), `$os_name` Nullable(String), `$os_version` Nullable(String), `$pathname` Nullable(String), `$prev_pageview_max_content_percentage` Nullable(String), `$prev_pageview_max_scroll_percentage` Nullable(String), `$prev_pageview_pathname` Nullable(String), `$process_person_profile` Nullable(String), `$referrer` Nullable(String), `$referring_domain` Nullable(String), `$screen_height` Nullable(String), `$screen_name` Nullable(String), `$screen_width` Nullable(String), `$sent_at` Nullable(String), `$session_id` Nullable(String), `$survey_id` Nullable(String), `$survey_response` Nullable(String), `$survey_response_1` Nullable(String), `$time` Nullable(String), `$user_id` Nullable(String), `$viewport_height` Nullable(String), `$viewport_width` Nullable(String), `$web_vitals_CLS_value` Nullable(String), `$web_vitals_FCP_value` Nullable(String), `$web_vitals_INP_value` Nullable(String), `$web_vitals_LCP_value` Nullable(String), `$window_id` Nullable(String)),
  timestamp DateTime64(6, 'UTC'),
  ...
  consumer_breadcrumbs Array(String),
  historical_migration Bool DEFAULT false
) ENGINE = Distributed('insights', 'insights', 'sharded_events_json', sipHash64(distinct_id));
CREATE TABLE insights.writable_events_recent (
  uuid UUID,
  event String,
  properties String CODEC(ZSTD(3)),
  timestamp DateTime64(6, 'UTC'),
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = Distributed('insights_writable', 'insights', 'sharded_events_recent', sipHash64(distinct_id));
CREATE TABLE insights.writable_log_entries (
  team_id UInt64,
  log_source LowCardinality(String),
  log_source_id String,
  instance_id String,
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_log_entries', rand());
CREATE TABLE insights.writable_insights_document_embeddings_text_embedding_3_large_3072 (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  rendering LowCardinality(String),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_insights_document_embeddings_text_embedding_3_large_3072', cityHash64(document_id));
CREATE TABLE insights.writable_insights_document_embeddings_text_embedding_3_small_1536 (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  rendering LowCardinality(String),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_insights_document_embeddings_text_embedding_3_small_1536', cityHash64(document_id));
...
CREATE TABLE insights.writable_raw_sessions (
  team_id Int64,
  session_id_v7 UInt128,
  distinct_id AggregateFunction(argMax, String, DateTime64(6, 'UTC')),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
  ...
  page_screen_autocapture_uniq_up_to AggregateFunction(uniqUpTo(1), Nullable(UUID)),
  vitals_lcp AggregateFunction(argMin, Nullable(Float64), DateTime64(6, 'UTC'))
) ENGINE = Distributed('insights', 'insights', 'sharded_raw_sessions', cityHash64(session_id_v7));
CREATE TABLE insights.writable_raw_sessions_v3 (
  team_id Int64,
  session_id_v7 UInt128,
  session_timestamp DateTime64(3) DEFAULT fromUnixTimestamp64Milli(toUInt64(bitShiftRight(session_id_v7, 80))),
  distinct_id AggregateFunction(argMax, String, DateTime64(6, 'UTC')),
  ...
  emails SimpleAggregateFunction(groupUniqArrayArray(10), Array(String)),
  has_replay_events SimpleAggregateFunction(max, Bool)
) ENGINE = Distributed('insights', 'insights', 'sharded_raw_sessions_v3', cityHash64(session_id_v7));
CREATE TABLE insights.writable_session_replay_embeddings (
  session_id String,
  team_id Int64,
  embeddings Array(Float32),
  generation_timestamp DateTime64(6, 'UTC') DEFAULT now('UTC'),
  source_type LowCardinality(String),
  input String
) ENGINE = Distributed('insights', 'insights', 'sharded_session_replay_embeddings', sipHash64(session_id));
CREATE TABLE insights.writable_session_replay_events (
  session_id String,
  team_id Int64,
  distinct_id String,
  min_first_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
  ...
  surfacing_score SimpleAggregateFunction(max, Nullable(Float32)),
  retention_period_days SimpleAggregateFunction(max, Nullable(Int64))
) ENGINE = Distributed('insights', 'insights', 'sharded_session_replay_events', sipHash64(distinct_id));
CREATE TABLE insights.writable_sessions (
  session_id String,
  team_id Int64,
  distinct_id SimpleAggregateFunction(any, String),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
  ...
  pageview_count SimpleAggregateFunction(sum, Int64),
  autocapture_count SimpleAggregateFunction(sum, Int64)
) ENGINE = Distributed('insights', 'insights', 'sharded_sessions', sipHash64(session_id));
CREATE TABLE insights.writeable_performance_events (
  uuid UUID,
  session_id String,
  window_id String,
  pageview_id String,
...
CREATE MATERIALIZED VIEW insights.person_distinct_id_mv TO insights.person_distinct_id (distinct_id String, person_id UUID, team_id Int64, _sign Int16, _timestamp Nullable(DateTime), _offset UInt64) AS SELECT
  distinct_id,
  person_id,
  team_id,
...
CREATE MATERIALIZED VIEW insights.person_overrides_mv TO insights.person_overrides (team_id Int32, old_person_id UUID, override_person_id UUID, merged_at DateTime64(6, 'UTC'), oldest_event DateTime64(6, 'UTC'), version Int32) AS SELECT team_id, old_person_id, override_person_id, merged_at, oldest_event, version
FROM insights.kafka_person_overrides;
CREATE MATERIALIZED VIEW insights.insights_document_embeddings_text_embedding_3_large_3072_mv TO insights.writable_insights_document_embeddings_text_embedding_3_large_3072 (team_id Int64, product LowCardinality(String), document_type LowCardinality(String), rendering LowCardinality(String), document_id String, timestamp DateTime64(3, 'UTC'), inserted_at DateTime64(3, 'UTC'), content String, metadata String, embedding Array(Float64), _timestamp DateTime, _offset UInt64, _partition UInt64) AS SELECT
  team_id,
  product,
  document_type,
  rendering,
...
CREATE MATERIALIZED VIEW insights.insights_document_embeddings_text_embedding_3_small_1536_mv TO insights.writable_insights_document_embeddings_text_embedding_3_small_1536 (team_id Int64, product LowCardinality(String), document_type LowCardinality(String), rendering LowCardinality(String), document_id String, timestamp DateTime64(3, 'UTC'), inserted_at DateTime64(3, 'UTC'), content String, metadata String, embedding Array(Float64), _timestamp DateTime, _offset UInt64, _partition UInt64) AS SELECT
  team_id,
  product,
  document_type,
  rendering,
...
CREATE MATERIALIZED VIEW insights.sessions_mv TO insights.writable_sessions (session_id String, team_id Int64, distinct_id String, min_timestamp DateTime64(6, 'UTC'), max_timestamp DateTime64(6, 'UTC'), urls Array(String), entry_url AggregateFunction(argMin, String, DateTime64(6, 'UTC')), exit_url AggregateFunction(argMax, String, DateTime64(6, 'UTC')), initial_referring_domain AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_utm_source AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_utm_campaign AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_utm_medium AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_utm_term AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_utm_content AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_gclid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_gad_source AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_gclsrc AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_dclid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_gbraid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_wbraid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_fbclid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_msclkid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_twclid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_li_fat_id AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_mc_cid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_igshid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_ttclid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_epik AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_qclid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), initial_sccid AggregateFunction(argMin, String, DateTime64(6, 'UTC')), event_count_map Map(String, UInt64), pageview_count UInt64, autocapture_count UInt64) AS SELECT
...
CREATE VIEW insights.custom_metrics_test AS SELECT
  ...
  'gauge' AS type;
CREATE VIEW insights.persons_batch_export AS WITH
...
CREATE VIEW insights.persons_batch_export_backfill AS SELECT
...
CREATE TABLE insights.app_metrics (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_app_metrics', rand());
CREATE TABLE insights.app_metrics2 (
  team_id Int64,
  timestamp DateTime64(6, 'UTC'),
  app_source LowCardinality(String),
  app_source_id String,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_app_metrics2', rand());
CREATE TABLE insights.distinct_id_usage (
  team_id Int64,
  distinct_id String,
  minute DateTime,
  event_count UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_distinct_id_usage', sipHash64(distinct_id));
CREATE TABLE insights.distributed_events_recent (
  uuid UUID,
  event String,
  properties String CODEC(ZSTD(3)),
  timestamp DateTime64(6, 'UTC'),
  ...
  _offset UInt64,
  inserted_at DateTime64(6, 'UTC') DEFAULT now64()
) ENGINE = Distributed('insights_primary_replica', 'insights', 'sharded_events_recent', sipHash64(distinct_id));
CREATE TABLE insights.distributed_insights_document_embeddings (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  model_name LowCardinality(String),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'partitioned_sharded_insights_document_embeddings', cityHash64(document_id));
CREATE TABLE insights.distributed_insights_document_embeddings_text_embedding_3_large_3072 (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  rendering LowCardinality(String),
...
CREATE TABLE insights.distributed_insights_document_embeddings_text_embedding_3_small_1536 (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  rendering LowCardinality(String),
...
CREATE TABLE insights.events_json (
  uuid UUID,
  event String,
  properties JSON(max_dynamic_types=8, max_dynamic_paths=256, `$active_feature_flags` Array(String), `$ai_experiment_id` Nullable(String), `$ai_http_status` Nullable(String), `$ai_is_error` Nullable(String), `$ai_model` Nullable(String), `$ai_parent_id` Nullable(String), `$ai_prompt_name` Nullable(String), `$ai_provider` Nullable(String), `$ai_session_id` Nullable(String), `$ai_span_id` Nullable(String), `$ai_total_cost_usd` Nullable(String), `$ai_trace_id` Nullable(String), `$anon_distinct_id` Nullable(String), `$app_build` Nullable(String), `$app_namespace` Nullable(String), `$app_version` Nullable(String), `$browser` Nullable(String), `$browser_version` Nullable(String), `$current_url` Nullable(String), `$device` Nullable(String), `$device_id` Nullable(String), `$device_model` Nullable(String), `$device_type` Nullable(String), `$el_text` Nullable(String), `$event_type` Nullable(String), `$exception_fingerprint` Nullable(String), `$exception_functions` Array(String), `$exception_issue_id` Nullable(String), `$exception_sources` Array(String), `$exception_types` Array(String), `$exception_values` Array(String), `$feature_flag` Nullable(String), `$feature_flag_payloads` Nullable(String), `$feature_flag_response` Nullable(String), `$geoip_city_name` Nullable(String), `$geoip_country_code` Nullable(String), `$geoip_country_name` Nullable(String), `$geoip_subdivision_1_code` Nullable(String), `$group_0` Nullable(String), `$group_1` Nullable(String), `$group_2` Nullable(String), `$group_3` Nullable(String), `$group_4` Nullable(String), `$groups` Nullable(String), `$host` Nullable(String), `$initial_pathname` Nullable(String), `$initial_referrer` Nullable(String), `$initial_referring_domain` Nullable(String), `$ip` Nullable(String), `$is_identified` Nullable(String), `$lib` Nullable(String), `$lib_custom_api_host` Nullable(String), `$lib_version` Nullable(String), `$lib_version__minor` Nullable(String), `$os` Nullable(String), `$os_name` Nullable(String), `$os_version` Nullable(String), `$pathname` Nullable(String), `$prev_pageview_max_content_percentage` Nullable(String), `$prev_pageview_max_scroll_percentage` Nullable(String), `$prev_pageview_pathname` Nullable(String), `$process_person_profile` Nullable(String), `$referrer` Nullable(String), `$referring_domain` Nullable(String), `$screen_height` Nullable(String), `$screen_name` Nullable(String), `$screen_width` Nullable(String), `$sent_at` Nullable(String), `$session_id` Nullable(String), `$survey_id` Nullable(String), `$survey_response` Nullable(String), `$survey_response_1` Nullable(String), `$time` Nullable(String), `$user_id` Nullable(String), `$viewport_height` Nullable(String), `$viewport_width` Nullable(String), `$web_vitals_CLS_value` Nullable(String), `$web_vitals_FCP_value` Nullable(String), `$web_vitals_INP_value` Nullable(String), `$web_vitals_LCP_value` Nullable(String), `$window_id` Nullable(String)),
  timestamp DateTime64(6, 'UTC'),
...
CREATE TABLE insights.experiment_exposures_preaggregated (
  team_id Int64,
  job_id UUID,
  entity_id String,
  variant String,
  ...
  computed_at DateTime64(6, 'UTC') DEFAULT now(),
  expires_at Date DEFAULT today() + toIntervalDay(7)
) ENGINE = Distributed('insights', 'insights', 'sharded_experiment_exposures_preaggregated', cityHash64(entity_id));
CREATE TABLE insights.flag_evaluations (
  team_id Int64,
  uuid UUID,
  timestamp DateTime64(6, 'UTC'),
  inserted_at DateTime64(6, 'UTC') DEFAULT timestamp,
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_flag_evaluations', sipHash64(distinct_id));
CREATE TABLE insights.heatmaps (
  session_id String,
  team_id Int64,
  distinct_id String,
  timestamp DateTime64(6, 'UTC'),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_heatmaps', cityHash64(concat(toString(team_id), '-', session_id, '-', toString(toDate(timestamp)))));
CREATE TABLE insights.ingestion_warnings (
  team_id Int64,
  source LowCardinality(String),
  type String,
  details String CODEC(ZSTD(3)),
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_ingestion_warnings', rand());
CREATE TABLE insights.log_entries (
  team_id UInt64,
  log_source LowCardinality(String),
  log_source_id String,
  instance_id String,
...
CREATE TABLE insights.performance_events (
  uuid UUID,
  session_id String,
  window_id String,
  pageview_id String,
...
CREATE TABLE insights.preaggregation_results (
  team_id Int64,
  job_id UUID,
  time_window_start DateTime64(6, 'UTC'),
  expires_at DateTime64(6, 'UTC') DEFAULT now() + toIntervalDay(7),
  breakdown_value Array(String),
  uniq_exact_state AggregateFunction(uniqExact, UUID)
) ENGINE = Distributed('insights', 'insights', 'sharded_preaggregation_results', sipHash64(job_id));
CREATE TABLE insights.precalculated_events (
  team_id Int64,
  date Date,
  distinct_id String,
  person_id UUID,
  ...
  _partition UInt64,
  _offset UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_precalculated_events', sipHash64(distinct_id));
CREATE TABLE insights.precalculated_person_properties (
  team_id Int64,
  distinct_id String,
  person_id UUID,
  condition String,
  ...
  _timestamp DateTime64(6),
  _offset UInt64
) ENGINE = Distributed('insights', 'insights', 'sharded_precalculated_person_properties', sipHash64(distinct_id));
CREATE TABLE insights.raw_sessions (
  team_id Int64,
  session_id_v7 UInt128,
  distinct_id AggregateFunction(argMax, String, DateTime64(6, 'UTC')),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
CREATE TABLE insights.raw_sessions_v3 (
  team_id Int64,
  session_id_v7 UInt128,
...
CREATE TABLE insights.session_replay_embeddings (
  session_id String,
  team_id Int64,
  embeddings Array(Float32),
  generation_timestamp DateTime64(6, 'UTC') DEFAULT now('UTC'),
...
CREATE TABLE insights.session_replay_events (
  session_id String,
  team_id Int64,
  distinct_id String,
  min_first_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
CREATE TABLE insights.sessions (
  session_id String,
  team_id Int64,
  distinct_id SimpleAggregateFunction(any, String),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
CREATE VIEW insights.events_batch_export AS SELECT
  team_id AS team_id,
...
CREATE VIEW insights.events_batch_export_backfill AS SELECT
  team_id AS team_id,
  timestamp AS timestamp,
  event AS event,
  distinct_id AS distinct_id,
...
SETTINGS
  optimize_aggregation_in_order = 1;
CREATE VIEW insights.events_batch_export_recent AS SELECT
  team_id AS team_id,
  timestamp AS timestamp,
  event AS event,
  distinct_id AS distinct_id,
...
SETTINGS
  optimize_aggregation_in_order = 1;
CREATE VIEW insights.events_batch_export_unbounded AS SELECT
  team_id AS team_id,
  timestamp AS timestamp,
  event AS event,
  distinct_id AS distinct_id,
...
SETTINGS
  optimize_aggregation_in_order = 1;
CREATE VIEW insights.insights_document_embeddings_union_view AS SELECT
  team_id,
  product,
  document_type,
  rendering,
...
CREATE VIEW insights.raw_sessions_v AS SELECT
  session_id_v7,
...
CREATE VIEW insights.raw_sessions_v3_v AS SELECT
  session_id_v7,
  session_timestamp,
  team_id,
  argMaxMerge(distinct_id) AS distinct_id,
...
CREATE VIEW insights.sessions_v AS SELECT
  session_id,
  team_id,
  any(distinct_id) AS distinct_id,
  min(min_timestamp) AS min_timestamp,
