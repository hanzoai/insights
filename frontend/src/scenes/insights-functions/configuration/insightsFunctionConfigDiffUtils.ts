...
import { getDiffStats } from 'products/insights_ai/frontend/api/tools'

import { redactSecretInsightsFunctionInputs } from '../script-function-utils'

/** How a proposed field relates to the current config: newly set, cleared, or a value change. */
export type InsightsFunctionFieldStatus = 'added' | 'removed' | 'changed'

/** One field the agent's `cdp-functions-partial-update` proposes to change, resolved against current config. */
export interface InsightsFunctionFieldDiff {
    field: string
    ...
    kind: 'json' | 'text'
    status: InsightsFunctionFieldStatus
...
// The subset of a script function's config that `cdp-functions-partial-update` can touch and that reads
...
const DIFFABLE_FIELDS: DiffableField[] = [
    ...
    { field: 'script', label: 'Source code', kind: 'text' },
...
function redactCurrentValue(field: string, current: Record<string, unknown>): unknown {
    if (field === 'inputs') {
        return redactSecretInsightsFunctionInputs(
...
/**
 * Builds a field-level diff of a proposed `cdp-functions-partial-update` against the current script function
 ...
 */
export function buildInsightsFunctionConfigDiff(
    current: Record<string, unknown>,
    proposed: Record<string, unknown>
): InsightsFunctionFieldDiff[] {
    const diffs: InsightsFunctionFieldDiff[] = []
    for (const { field, label, kind, normalize } of DIFFABLE_FIELDS) {
        ...
        const status: InsightsFunctionFieldStatus =
