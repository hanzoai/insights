...
import { Skeleton, Tabs, Tag } from '@hanzo/elements'
...
function FlatTabCount({ tabKey }: { tabKey: InboxFlatListTabKey }): JSX.Element {
    ...
    if (count === null && countLoading) {
        return <Skeleton className="h-3 w-3 rounded" />
    }
...
export function InboxTabBar({
    ...
    const realTabs = visibleTabKeys.map((key) => ({
        ...
        label: (
            <span className="flex items-center gap-1.5">
                ...
                {INBOX_TAB_TAG[key] && (
                    <Tag type={INBOX_TAB_TAG[key] === 'Alpha' ? 'warning' : 'completion'} size="small">
                        {INBOX_TAB_TAG[key]}
                    </Tag>
                )}
            </span>
    ...
    return (
        <Tabs<InboxTabBarKey>
            ...
            className="min-w-0"
            // Hide Tabs' own bottom border + margin so the single full-width border lives on the
