...
use personinsights_common::grpc::{
...
const SERVICE_PREFIX: &str = "/personinsights.service.v1.PersonHogService/";
const REPLICA_PREFIX: &str = "/personinsights.replica.v1.PersonHogReplica/";
...
impl tonic::server::NamedService for RawProxyService {
    const NAME: &'static str = "personinsights.service.v1.PersonHogService";
}
...
impl RawProxyInner {
    async fn handle(&self, req: http::Request<BoxBody>) -> http::Response<BoxBody> {
        ...
        if let Some(processing_ms) = processing_ms {
            histogram!(
                "personinsights_router_transport_overhead_ms",
                "method" => method.clone(),
                "backend" => backend,
                "client" => client.clone(),
            )
            ...
            if let Some(call_ms) = channel_call_ms {
                ...
                histogram!(
                    "personinsights_router_network_overhead_ms",
                    "method" => method.clone(),
                    "backend" => backend,
                    "client" => client.clone(),
                )
    ...
    async fn raw_proxy_to_replica(
        ...
        let client = current_client_name();
        histogram!(
            "personinsights_router_body_collect_ms",
            "method" => method.clone(),
            "client" => client.clone(),
        )
    ...
    async fn forward_with_retry(
        ...
        for attempt in 0..=self.retry_config.max_retries {
            ...
            let ready_channel = match channel.ready().await {
                ...
                Err(e) => {
                    histogram!(
                        "personinsights_router_channel_ready_wait_ms",
                        "method" => method.clone(),
                        "client" => client.clone(),
                        "outcome" => "error",
                    )
            ...
            *req.headers_mut() = parts.headers.clone();
            ...
            match ready_channel.call(req).await {
                Ok(response) => {
                    ...
                    histogram!(
                        "personinsights_router_channel_call_ms",
                        "method" => method.clone(),
                        "client" => client.clone(),
                        "outcome" => "ok",
                    )
...
async fn retry_backoff(
    ...
    counter!(
        "personinsights_router_backend_retries_total",
        "method" => method.clone(),
...
impl Drop for ByteCountedBody {
    fn drop(&mut self) {
        histogram!(
            "personinsights_router_response_size_bytes",
...
mod tests {
    ...
    fn known_methods_matches_service_proto() {
        let proto = std::fs::read_to_string(concat!(
            ...
            "/../../proto/personinsights/service/v1/service.proto"
        ))
    ...
    async fn collect_body_limited_reassembles_multiple_frames() {
        let body = multi_frame_body(vec![b"hello ", b"personinsights ", b"world"]);
        let result = collect_body_limited(body, 100).await.unwrap();
        assert_eq!(&result[..], b"hello personinsights world");
    }
