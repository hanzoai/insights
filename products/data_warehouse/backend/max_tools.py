...
def get_insightsql_functions() -> str:
    global _insightsql_functions
...
These are the aggregated InsightsQL functions:
```
{str(ch_aggregations)}
```

And lastly these are some InsightsQL specific functions:
```
{str(ph_functions)}
```"""
...
You are a senior software engineer with deep expertise in SQL, specifically in the InsightsQL dialect.
Your job is to help users fix invalid or broken InsightsQL queries by identifying the errors and returning a corrected version of the query.

IMPORTANT: This is currently your primary task. Therefore `fix_insightsql_query` is currently your primary tool.
Use `fix_insightsql_query` when fixing any errors remotely related to InsightsQL.
It's very important to disregard other tools for these purposes - the user expects `fix_insightsql_query`.

NOTE: When calling the `fix_insightsql_query` tool, do not provide any response other than the tool call.
"""
...
- Only `SELECT` queries can be written in InsightsQL - so no `INSERT` or `UPDATE` for example
- InsightsQL supports the comparison operator `IN COHORT` and `NOT IN COHORT` to check whether persons are within a cohort or not. A cohort can be defined by id or string name
...
- InsightsQL statements should not end with a semi-colon - this is invalid syntax
...
Below is the current InsightsQL query and the error message
"""


def _get_schema_description(ai_context: dict[Any, Any], insightsql_context: InsightsQLContext, database: Database) -> str:
    serialized_database = database.serialize(insightsql_context)
    schema_description = ""

    try:
        query = ai_context.get("insightsql_query", None)
        if not query:
...
class InsightsQLQueryFixerTool(MaxTool):
    name: str = "fix_insightsql_query"
    description: str = "Fixes any error in the current InsightsQL query"
    ...
    def _run_impl(self) -> tuple[str, str | None]:
        ...
        insightsql_context = InsightsQLContext(team=self._team, user=self._user, enable_select_queries=True, database=database)

        all_table_names = database.get_all_table_names()
        schema_description = _get_schema_description(self.context, insightsql_context, database)
        ...
        for _ in range(3):
            try:
                ...
                parsed_result = self._parse_output(result, insightsql_context)
                break
...
{e.validation_message}
    ...
    def _parse_output(self, output, insightsql_context: InsightsQLContext):
        ...
        try:
            ...
            if err_msg.startswith(
                ...
                err_msg = "InsightsQL parsing error: this query isn't valid InsightsQL."
