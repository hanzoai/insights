...
export interface ExperimentApiEventSourceApi {
    ...
    id?: number | null
    kind: KindApi
    /** How to aggregate this source. Defaults to 'total' (event count). Use 'sum' together with math_property to aggregate a numeric property — e.g. a ratio numerator of revenue per order. Other options: 'avg', 'min', 'max', 'unique_session', 'dau', 'unique_group', 'insightsql'. */
    ...
    /** InsightsQL aggregation expression. Required when math is 'insightsql' — without it the metric silently falls back to a plain count/sum. */
...
export interface EndExperimentApi {
     ...
     */
    conclusion_comment?: string | null
    /** When true, open a draft pull request that removes the experiment's feature-flag code from the linked repository. Requires the requesting user to have access to Insights Desktop (403 otherwise). Only acts for allowlisted teams; ignored otherwise. */
...
export interface ExperimentFlagCleanupTaskApi {
     ...
     */
    pr_url: string | null
    /** Whether the requesting user can open the task in Insights Desktop. Cleanup tasks are visible to their creator only, so other viewers should not be shown a task link. */
...
export interface ExperimentMetricsRecalculationApi {
    ...
    /**
     * Datastore's total_rows_approx across running queries plus the final read_rows of finished ones. A soft ceiling revised mid-scan, so it can exceed or trail rows_read; treat rows_read as the reliable signal
     * @nullable
     */
