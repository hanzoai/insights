...
**When to load:** Pydantic config changes, `insightscli build:openapi`, Zod/OpenAPI drift, `ENUM_NAME_OVERRIDES`, or MCP `config_schema` updates.
...
bin/build-dashboard-widget-types.py  (insightscli build:widget-types — step 1)
...
generate-widget-config-zod.mjs  (insightscli build:widget-types — step 2)
...
| `generated/widget-configs.zod.ts`            | Friendly re-exports, inferred types, form `.pick()` schemas (`insightscli build:widget-types`)          |
...
| `widgets/widgetConfigValidation.ts`          | Shared InsightsQL filter helpers + `parseWidgetConfigApiError` — not the per-type schema               |
...
```bash
insightscli build:openapi   # openapi-schema → build:widget-types → openapi-types → MCP
```
...
**Local dev:** Vite reads **committed** files under `products/dashboards/frontend/generated/` — no regen on `insightscli up` / save. After `widget_specs/` or serializer changes, run `insightscli build:openapi` and **commit** the generated diff. No pre-commit hook.

**CI (`check-openapi-types` in `ci-backend.yml`):** runs the same `insightscli build:openapi`, then diffs generated outputs. Same-repo PRs may auto-commit drift; fork PRs and unpushed fixes fail with “run `insightscli build:openapi` locally”. Triggers on `products/**/backend/**` (incl. `widget_specs/`) and `products/*/frontend/generated/**`.
...
**Schema gen blocker:** `build:openapi-schema` uses `--fail-on-warn`. Polymorphic per-type OpenAPI serializers each use a singleton `ChoiceField` for `widget_type`; `dashboard.py` uses the full `EXPECTED_WIDGET_TYPES` list — drf-spectacular enum names collide. When shipping a new type, add `{YourWidgetTypeEnum: ["your_widget_type"]}` to `ENUM_NAME_OVERRIDES` in `insights/settings/web.py` — `insightscli build:widget-types` and `test_widget_openapi_enums.py` fail if a registry type lacks an override; spectacular collision test fails if the override hash is wrong. Diagnostic: `python manage.py find_enum_collisions` (logic in `insights/openapi/enum_collisions.py`). See `/improving-drf-endpoints`.

**Schema parity tests** (cheap drift guards — run when touching `widget_specs/`):

```bash
insightscli test products/dashboards/backend/api/test/test_widget_config_schema_parity.py
insightscli test products/dashboards/frontend/widgets/widgetConfigSchemaParity.test.ts
```
...
| Importing shared `insights.schema` models into widget config            | Prefer local Pydantic models in `configs.py` (e.g. `WidgetAssigneeFilter`) — avoids OpenAPI component name collisions in spectacular                                                                                                                                                                                                                                             |
...
| Editing generated Zod/TS without regen                                 | Run `insightscli build:openapi`, commit `products/dashboards/frontend/generated/*` — CI `check-openapi-types` diffs and fails or auto-commits                                                                                                                                                                                                                                          |
| `insightscli build:openapi-schema` fails on warnings                         | `--fail-on-warn` — use `find_enum_collisions` + `ENUM_NAME_OVERRIDES`; see [§ Codegen & CI](#codegen--ci)                                                                                                                                                                                                                                                                        |
