...
from insights.insightsql_queries.ai.sentiment_evaluations import (
...
from insights.insightsql_queries.ai.utils import parse_ai_properties, parse_ai_property_value
...
class TracesQueryDateRange(QueryDateRange):
    ...
    def date_from_for_filtering_as_insightsql(self) -> ast.Expr:
        return ast.Call(
            name="assumeNotNull",
            args=[
    ...
    def date_to_for_filtering_as_insightsql(self) -> ast.Expr:
        return ast.Call(
            name="assumeNotNull",
            args=[
...
class TracesQueryRunner(AnalyticsQueryRunner[TracesQueryResponse]):
    ...
    def _build_trace_ids_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        ...
        return parse_select(
            ...
            placeholders={
                ...
                "unbuffered_date_from": self._date_range.date_from_for_filtering_as_insightsql(),
                "unbuffered_date_to": self._date_range.date_to_for_filtering_as_insightsql(),
            },
        )
    ...
    def _get_trace_ids(self) -> tuple[list[str], datetime | None, datetime | None]:
        ...
        with self.timings.measure("traces_query_trace_ids_execute"), tags_context(product=Product.LLM_ANALYTICS):
            trace_ids_result = execute_insightsql_query(
    ...
    def _calculate(self):
        ...
        with self.timings.measure("traces_query_insightsql_execute"), tags_context(product=Product.LLM_ANALYTICS):
    ...
    def _get_where_clause(self, date_range: TracesQueryDateRange | None = None) -> ast.Expr:
        ...
        where_exprs: list[ast.Expr] = [
            ast.CompareOperation(
                ...
                right=effective_date_range.date_from_as_insightsql(),
            ),
            ast.CompareOperation(
                op=ast.CompareOperationOp.LtEq,
                left=ast.Field(chain=["events", "timestamp"]),
                right=effective_date_range.date_to_as_insightsql(),
            ),
        ]
