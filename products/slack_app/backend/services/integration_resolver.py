...
def user_resolution_failure_reply(
    ...
    Insights tried to match. Returns ``None`` for unknown values so callers can
    ...
    if failure_reason == "user_not_found":
        if slack_email:
            return (
                f"Sorry, I couldn't find {slack_email} in any Insights organization connected to this "
    ...
    if failure_reason == "no_team_access":
        ...
        return (
            f"Sorry, {subject} doesn't have access to any Insights project connected to this Slack "
    ...
    *,
    ...
    if slack_user_id:
        ...
        for default in defaults:
            ...
            # can overwrite the row at any time with `@Insights project <id>`.
...
class UserAndIntegrationsResolution:
    ...
    ``user`` is ``None`` when no Insights user resolved or none had access — both
    cases are silently logged inside ``resolve_user_for_workspace`` under
    ``insights_code_no_integration_found``, and the specific case is exposed via
...
def resolve_user_for_workspace(
    *,
    ...
    from products.slack_app.backend.api import get_slack_email_for_user, resolve_insights_user_from_event
    ...
    permissions = UserPermissions(user=insights_user)
    ...
    if not accessible_candidates:
        ...
        # earlier resolve_insights_user_from_event call, so this is free).
        slack_email = get_slack_email_for_user(probe, slack_user_id)
        logger.warning(
            "slack_app_no_integration_found",
