...
describe('alertNotificationLogic', () => {
    ...
    beforeEach(() => {
        initKeaTests()
        listSpy = jest.spyOn(api.insightsFunctions, 'list').mockResolvedValue({ results: [], count: 0 })
        createSpy = jest
            .spyOn(api.insightsFunctions, 'create')
            .mockImplementation(async (data) => ({ id: 'hf-1', ...data }) as InsightsFunctionType)
    })
    ...
    it('drops staged Slack notifications for a workspace removed by an integrations refresh', async () => {
        ...
        // saving it would create a InsightsFunction referencing a dead integration. The webhook
    ...
    it('creates a Discord destination InsightsFunction end-to-end', async () => {
        logic = alertNotificationLogic({ alertId: 'alert-123' })
        logic.mount()
        ...
        logic.actions.createPendingInsightsFunctions('alert-123', 'Daily revenue check')
        await expectLogic(logic).toFinishAllListeners()
    ...
    it('creates a Microsoft Teams destination InsightsFunction end-to-end', async () => {
        logic = alertNotificationLogic({ alertId: 'alert-123' })
        logic.mount()
