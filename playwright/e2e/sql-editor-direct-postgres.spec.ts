...
test.describe('SQL Editor direct Postgres queries', () => {
    test('creates a Postgres direct source and queries it successfully', async ({ page }) => {
        ...
        try {
            execFileSync('psql', [
                'postgresql://insights:insights@127.0.0.1:5432/insights',
                '-c',
                `
            ...
            await test.step('Open the direct Postgres source flow with direct query preselected', async () => {
                ...
                const connectionSelector = page.getByRole('button', { name: /Insights \(Datastore\)/ })
                ...
                await expect(page.getByText('How should Insights query this source?')).toBeVisible({ timeout: 60000 })
                    ...
                    async ({ name, table }) => {
                        const csrfToken =
                            document.cookie
                                ...
                                .find((cookie) => cookie.startsWith('insights_csrftoken='))
                                ?.split('=')
                                .slice(1)
                                .join('=') || ''
            ...
            await test.step('Run a successful raw query against the new connection', async () => {
                ...
                await page
                    .locator('[data-attr=insightsql-query-editor] [data-editor-ready="true"]')
                    .first()
                    .waitFor({ state: 'visible' })
                await page.locator('[data-attr=insightsql-query-editor]').click()
                await page
                    .locator('[data-attr=insightsql-query-editor]')
                    .press(`${process.platform === 'darwin' ? 'Meta' : 'Control'}+A`)
                await page.locator('[data-attr=insightsql-query-editor]').press('Delete')
                await page
                    .locator('[data-attr=insightsql-query-editor]')
