...
``insights.models.integration.OauthIntegration``: that one mints workspace-
...
def find_linked_insights_user(
    *,
    slack_user_id: str,
    slack_team_id: str,
    candidate_org_ids: set[UUID],
) -> User | None:
    """Return the Insights ``User`` linked to this Slack identity, scoped to the
    organizations connected to this workspace.

    Multiple Insights users may legitimately link to the same Slack identity
    (e.g. one Slack account → personal + work Insights accounts). When more
    ...
    **most-recently-linked** one — they just authenticated, presumably with
    ...
    # a different database (which `User` does in Insights's deployment), so a
...
# thread) to the Insights authorize view; longer would let the same Slack DM
...
class InviteToken(BaseModel):
    """Slack-side invite payload signed into the URL on the "Link my Insights
...
class CallbackState(BaseModel):
    ...
    invite context plus the Insights user who started the flow, so the
...
def build_invite_url(
    *,
    ...
    """Full https URL the Slack button opens. Routes through Insights's
    ...
    token = InviteToken(
        slack_team_id=slack_team_id,
        insights_team_id=insights_team_id,
        slack_user_id=slack_user_id,
        channel=channel,
        thread_ts=thread_ts,
    ).encode()
...
# claims) — the bare minimum we need to bind a Slack user to a Insights user.
...
def post_link_invite_message(
    *,
    ...
    Insights account. Visible only to ``slack_user_id``.
    ...
    another round-trip to Insights. Failures here are logged and swallowed:
    ...
    # public thread reply already names the address Insights tried to match.
    del slack_email
    fallback_text = "Link your Insights account to fix this for future mentions."
    blocks: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "Link my Insights account"},
                    "style": "primary",
                    "url": invite_url,
                }
            ],
