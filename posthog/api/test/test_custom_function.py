...
from posthog.api.custom_function import MAX_HOG_CODE_SIZE_BYTES, MAX_TRANSFORMATIONS_PER_TEAM
...
from posthog.models.custom_functions.custom_function import DEFAULT_STATE, CustomFunction, CustomFunctionState
...
EXAMPLE_FULL = {
    "name": "HogHook",
    "custom_script": "fetch(inputs.url, {\n  'headers': inputs.headers,\n  'body': inputs.payload,\n  'method': inputs.method\n});",
    "type": "destination",
    "code_language": "hog",
    "enabled": True,
    "inputs_schema": [
...
class TestCustomFunctionAPIWithoutAvailableFeature(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    def setUp(self):
        super().setUp()
    ...
    def test_create_custom_function_works_for_free_template(self):
    ...
    def test_sers_can_update_non_free_templates(self):
        ...
        update_response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{response.json()['id']}/",
            data=payload,
        )
    ...
    def test_internal_destinations_can_be_managed_without_addon(self):
        ...
        response = self.client.post(
            ...
            data={
                ...
                "custom_script": "fetch('https://example.com');",
        ...
        # Update it
        update_response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{function_id}/",
            data={"name": "New name"},
        )
...
class TestCustomFunctionAPI(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    def setUp(self):
        super().setUp()
    ...
    def _get_function_activity(
        ...
        params: dict = {"scope": "CustomFunction", "page": 1, "limit": 20}
        if function_id:
    ...
    def test_create_custom_function(self, *args):
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
                "type": "destination",
        ...
        assert response.json() == {
            ...
            "enabled": False,
            "custom_script": "fetch(inputs.url);",
            "bytecode": ["_H", INSIGHTSQL_BYTECODE_VERSION, 32, "url", 32, "inputs", 1, 2, 2, "fetch", 1, 35],
            "transpiled": None,
            "inputs_schema": [],
            "inputs": {},
            "filters": {
    ...
    def test_deletes_via_update(self, *args):
        ...
        list_res = self.client.get(f"/api/projects/{self.team.id}/custom_functions/")
    ...
    def test_can_undelete_custom_function(self, *args):
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={**EXAMPLE_FULL},
        )
        ...
        response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{id}/",
            data={"deleted": True},
        )
        assert response.status_code == status.HTTP_200_OK, response.json()
        assert (
            self.client.get(f"/api/projects/{self.team.id}/custom_functions/{id}").status_code == status.HTTP_404_NOT_FOUND
        )
        ...
        assert response.status_code == status.HTTP_200_OK, response.json()
        assert self.client.get(f"/api/projects/{self.team.id}/custom_functions/{id}").status_code == status.HTTP_200_OK
    ...
    def test_inputs_required(self, *args):
        ...
        # Check required
        res = self.client.post(f"/api/projects/{self.team.id}/custom_functions/", data={**payload})
    ...
    def test_inputs_mismatch_type(self, *args):
        ...
        for key, value in bad_inputs.items():
            res = self.client.post(
                f"/api/projects/{self.team.id}/custom_functions/", data={**payload, "inputs": {key: {"value": value}}}
            )
            assert res.json() == {
                "type": "validation_error",
                "code": "invalid_input",
    ...
    def test_secret_inputs_not_returned(self, *args):
        ...
        res = self.client.get(f"/api/projects/{self.team.id}/custom_functions/{res.json()['id']}")
        assert res.json()["inputs"] == expectation

        # Finally check the DB has the real value
        obj = CustomFunction.objects.get(id=res.json()["id"])
        assert obj.inputs == {}
    ...
    def test_secret_inputs_not_updated_if_not_changed(self, *args):
        ...
        res = self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{res.json()['id']}",
            data={
                "inputs": {
                    "secret1": {
                        "secret": True,
    ...
    def test_generates_hog_bytecode(self, *args):
        response = self.client.post(
            ...
            data={
                **EXAMPLE_FULL,
                "custom_script": "let i := 0;\nwhile(i < 3) {\n  i := i + 1;\n  fetch(inputs.url, {\n    'headers': {\n      'x-count': f'{i}'\n    },\n    'body': inputs.payload,\n    'method': inputs.method\n  });\n}",
            },
        )
        # JSON loads for one line comparison
        assert response.json()["bytecode"] == json.loads(
            f'["_H", {INSIGHTSQL_BYTECODE_VERSION}, 33, 0, 33, 3, 36, 0, 15, 40, 45, 33, 1, 36, 0, 6, 37, 0, 32, "url", 32, "inputs", 1, 2, 32, "headers", 32, "x-count", 36, 0, 42, 1, 32, "body", 32, "payload", 32, "inputs", 1, 2, 32, "method", 32, "method", 32, "inputs", 1, 2, 42, 3, 2, "fetch", 2, 35, 39, -52, 35]'
        ), response.json()

    def test_generates_inputs_bytecode(self, *args):
        response = self.client.post(f"/api/projects/{self.team.id}/custom_functions/", data=EXAMPLE_FULL)
        assert response.status_code == status.HTTP_201_CREATED, response.json()
    ...
    def test_loads_status_when_enabled_and_available(self, *args):
        with patch("posthog.plugins.plugin_server_api.requests.get") as mock_get:
            ...
            response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/{response.json()['id']}")
        ...
        with patch("posthog.plugins.plugin_server_api.requests.get") as mock_get:
            with patch("posthog.plugins.plugin_server_api.requests.patch") as mock_patch:
                with patch("posthog.plugins.plugin_server_api.INTERNAL_API_SECRET", internal_api_secret):
                    mock_get.return_value.status_code = status.HTTP_200_OK
                    mock_get.return_value.json.return_value = {
                        "state": CustomFunctionState.DISABLED.value,
                        "tokens": 0,
                    }
                    ...
                    assert response.json()["status"]["state"] == CustomFunctionState.DISABLED.value
                    ...
                    mock_patch.assert_called_once_with(
                        f"http://localhost:6738/api/projects/{self.team.id}/custom_functions/{response.json()['id']}/status",
    ...
    def test_list_with_filters_filter(self, *args):
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?filters={json.dumps(filters)}")
    ...
    def test_list_with_filter_groups_filter(self, *args):
        ...
        custom_function_id_1 = response.json()["id"]
        assert response.status_code == status.HTTP_201_CREATED, response.json()
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
        ...
        custom_function_id_2 = response.json()["id"]
        assert response.status_code == status.HTTP_201_CREATED, response.json()

        filter_groups: Any = [{"events": [{"id": "$pageview", "type": "events"}]}]
        response = self.client.get(
            f"/api/projects/{self.team.id}/custom_functions/?filter_groups={json.dumps(filter_groups)}"
        )
        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == custom_function_id_1
        ...
        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == custom_function_id_2
    ...
    def test_list_with_type_filter(self, *args):
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/")
        assert len(response.json()["results"]) == 2

        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?type=destination")
        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == destination_id

        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?type=transformation")
        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == transformation_id

        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?type=destination,site_app")
        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == destination_id

        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?type=destination,transformation")
        assert len(response.json()["results"]) == 2
    ...
    def test_list_with_enabled_filter(self, *args):
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?enabled=true")

        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == destination_id

        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?enabled=false")
        assert len(response.json()["results"]) == 1
        assert response.json()["results"][0]["id"] == transformation_id

        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?enabled=true,false")
        assert len(response.json()["results"]) == 2

    @patch("posthog.cdp.site_functions.transpile", side_effect=mock_transpile)
    def test_create_custom_function_with_site_app_type(self, mock_transpile_fn):
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
                "name": "Site App Function",
                "custom_script": "export function onLoad() { console.log('Hello, site_app'); }",
                "type": "site_app",
            },
        )
    ...
    @patch("posthog.cdp.site_functions.transpile", side_effect=mock_transpile)
    def test_create_custom_function_with_site_destination_type(self, mock_transpile_fn):
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
                "name": "Site Destination Function",
                "custom_script": "export function onLoad() { console.log('Hello, site_destination'); }",
    ...
    def test_cannot_modify_type_of_existing_custom_function(self):
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data=EXAMPLE_FULL,
        )
    ...
    def test_create_custom_function_with_invalid_typescript(self):
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
                "name": "Invalid Site App Function",
                "custom_script": "export function onLoad() { console.log('Missing closing brace');",
                "type": "site_app",
            },
        )
    ...
    def test_create_typescript_destination_with_inputs(self, mock_transpile_fn):
        payload = {
            ...
            "custom_script": "export function onLoad() { console.log(inputs.message); }",
            "type": "site_destination",
            "inputs_schema": [
    ...
    def test_compiles_valid_mappings(self):
        payload = {
            "name": "TypeScript Destination Function",
            "custom_script": "print(inputs.message)",
            "type": "destination",
            "mappings": [
                {
    ...
    def test_list_custom_functions_ordered_by_execution_order_and_updated_at(self):
    ...
    def test_can_call_a_test_invocation(self):
        with patch("posthog.api.custom_function.create_hog_invocation_test") as mock_create_hog_invocation_test:
            res = MagicMock(status_code=200, json=lambda: {"status": "success"})
            mock_create_hog_invocation_test.return_value = res

            response = self.client.post(
                f"/api/projects/{self.team.id}/custom_functions/new/invocations/",
                data={
                    "configuration": {
                        **EXAMPLE_FULL,
                    },
            ...
            assert mock_create_hog_invocation_test.call_args_list[0].kwargs["custom_function_id"] == "new"
            assert (
    ...
    def test_can_update_with_null_filters(self):
        ...
        response = self.client.post(
            ...
            data={
                "name": "Test Function",
                "type": "destination",
                "custom_script": "print('hello world')",
                "filters": {
                    "events": [{"id": "$pageview", "name": "$pageview", "type": "events", "order": 0}],
                    "filter_test_accounts": True,
                },
        ...
        function = CustomFunction.objects.get(id=function_id)
    ...
    def test_limits_transformation_functions_per_team(self):
        ...
        enabled_transformation = CustomFunction.objects.filter(
        ...
        self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{enabled_transformation.id}/",
            data={"deleted": True},
        )

        # Then enable a disabled transformation
        disabled_transformation = CustomFunction.objects.filter(
        ...
        response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{disabled_transformation.id}/",
            data={"enabled": True},
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_transformation_undeletion_puts_at_end(self, *args):
        ...
        # Delete function B
        delete_response = self.client.patch(
            f"/api/projects/{self.team.id}/custom_functions/{fn_b_id}/",
            data={"deleted": True},
        )
        ...
        list_response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/")
    ...
    def test_create_in_folder(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.json())
        custom_function_id = response.json()["id"]
        ...
        fs_entry = FileSystem.objects.filter(
            ...
            ref=str(custom_function_id),
        ).first()
        assert fs_entry is not None, "No FileSystem entry was created for this CustomFunction."
        assert "Special/Hog Destinations" in fs_entry.path

    def test_custom_function_template_fk_set_on_create(self):
        """
        When creating a CustomFunction with a template_id, the custom_function_template FK should be set to the latest template.
        When creating without a template_id, the FK should be null.
        """

        # Create a template in the DB
        _template = CustomFunctionTemplate.objects.create(
        ...
        # Create a CustomFunction with template_id
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
        ...
        assert custom_function.custom_function_template is not None, "FK should be set when template_id is provided"
        assert custom_function.custom_function_template.id == _template.id, "FK should point to the correct template"

        # Create a CustomFunction without template_id
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
                "name": "No Template FK",
        ...
        custom_function_id = response.json()["id"]
        custom_function = CustomFunction.objects.get(id=custom_function_id)
        assert custom_function.custom_function_template is None, "FK should be null when template_id is not provided"

    def test_custom_function_template_fk_validation_error_on_missing_template(self):
        """
        Creating a CustomFunction with a template_id that does not exist in the DB should raise a validation error and not create the object.
        """
        from posthog.models.custom_functions.custom_function import CustomFunction

        initial_count = CustomFunction.objects.count()
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/",
            data={
                "name": "Should Fail",
        ...
        assert CustomFunction.objects.count() == initial_count, "No CustomFunction should be created on error"
    ...
    def test_list_with_custom_limit_parameter(self):
        ...
        This verifies the frontend fix that adds limit=300 to CustomFunction list requests.
        """
        # Create 150 custom functions (more than the default page size of 100)
        for i in range(150):
            response = self.client.post(
                f"/api/projects/{self.team.id}/custom_functions/",
                data={
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?limit=50")
        assert response.status_code == status.HTTP_200_OK
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?limit=300")
        assert response.status_code == status.HTTP_200_OK
        ...
        response = self.client.get(f"/api/projects/{self.team.id}/custom_functions/?limit=50&offset=100")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.json()["results"]) == 50, "With limit=50 and offset=100, should return last 50 items"
        assert response.json()["count"] == 150, "Total count should still be 150"

    @patch("posthog.api.custom_function.posthoganalytics.feature_enabled", return_value=False)
    def test_enable_backfills_blocked_without_feature_flag(self, mock_feature_enabled):
        ...
        response = self.client.post(
            f"/api/projects/{self.team.id}/custom_functions/{function_id}/enable_backfills/",
        )
