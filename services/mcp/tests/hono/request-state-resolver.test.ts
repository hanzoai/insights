...
import { resolveFeatureFlagOverrides } from '@/lib/insights/flags'
...
describe('RequestStateResolver MCP client contexts', () => {
    ...
    it('captures consumer from a later request when initialize omitted the header', async () => {
        ...
        const insightsCode = makeProps({
            mcpClientName: 'Claude Desktop',
            mcpConsumer: 'insights-code',
        })
        const result = await makeResolver().resolve(insightsCode)

        expect(result.useSingleExec).toBe(true)
        expect(insightsCode.mode).toBe('cli')
        expect(result.requestContext.mcpConsumer).toBe('insights-code')
        expect(result.sessionContext?.mcpConsumer).toBe('insights-code')
        expect(mockSessionStore.get('mcpConsumer')).toBe('insights-code')
    })
})
