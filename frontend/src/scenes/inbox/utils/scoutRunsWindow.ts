// Ported from Insights Desktop `packages/core/src/scouts/scoutRunsWindow.ts`
...
export const SCOUT_AUTHOR_PROMPT = `I'd like to make a new scout for this Insights project.

Use the authoring-scouts skill from the Insights MCP to guide creating a new signals scout.

First, take a quick scan of this Insights project to ground your suggestions: skim its events, insights, dashboards, recently emitted signals, and the existing scout fleet so you understand what this product is and where automated monitoring would add value.
...
Use the exploring-scouts skill from the Insights MCP to survey the signals scout fleet on this project and give me a high-level overview:
...
Use the exploring-scouts skill from the Insights MCP to pull the most recent scout runs that emitted findings and walk me through the signals:
...
Use the exploring-scouts skill from the Insights MCP to dig into the \`${skillName}\` scout on this project:
