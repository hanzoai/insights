...
from insights.api.insights_connection import CONNECTION_MAX_INFLIGHT_PER_CONNECTION
...
FORWARD_PATH = "insights.api.insights_connection.requests.request"
...
class TestInsightsConnectionForward:
    ...
    def test_forward_requires_caller_token_to_cover_connection_scopes(self, client: HttpClient):
        ...
        client.force_login(self.user)
        with patch("insights.api.insights_connection.get_authenticator_scopes", return_value=["integration:write"]):
            with patch(FORWARD_PATH) as mock_request:
                response = client.post(
                    self._forward_url(),
                    {"method": "GET", "path": "api/projects/2/insights/"},
    ...
    def test_forward_times_out_when_target_exceeds_deadline(self, client: HttpClient):
        ...
        with patch("insights.api.insights_connection.monotonic", side_effect=[0.0, 10_000.0, 10_001.0]):
    ...
    def test_forward_rejects_when_connection_at_inflight_capacity(self, client: HttpClient):
        ...
        client.force_login(self.user)
        with patch(
            "insights.api.insights_connection.cache.incr", return_value=CONNECTION_MAX_INFLIGHT_PER_CONNECTION + 1
        ):
            with patch(FORWARD_PATH) as mock_request:
                response = client.post(
                    self._forward_url(),
