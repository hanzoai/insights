...
impl<'a, E: Emitter + Clone> Parser<'a, E> {
    ...
    fn parse_select_stmt_body(
        ...
        if columns.is_empty() {
            ...
            // side raises InsightsQLSyntaxError.
            return Err(ParseError::syntax(
        ...
        if self.eat_kw(Kw::Window)? {
            ...
            loop {
                ...
                let name = match name_tok.kind {
                    ...
                    // A WINDOW-clause name is an `identifier`, so only `kw_valid_as_identifier` keywords qualify — the Script-statement keywords (try / catch / finally / …) do not.
    ...
    /// Reject `selectStmt`-level `[USING] SAMPLE`: DuckDB's `USING SAMPLE`, which InsightsQL has no AST home for (only table-level `JoinExprTable` lands on `JoinExpr.sample`), so reject rather than silently drop. Matches the python + cpp visitors' `NotImplementedError`.
