...
from posthog.insightsql_queries.ai.utils import TaxonomyCacheMixin
...
class VectorSearchQueryRunner(TaxonomyCacheMixin, AnalyticsQueryRunner[VectorSearchQueryResponse]):
    ...
    def _calculate(self):
        ...
        with self.timings.measure("to_printed_insightsql"):
            insightsql = to_printed_insightsql(query, self.team)

        with tags_context(product=Product.MAX_AI), self.timings.measure("execute_insightsql_query"):
