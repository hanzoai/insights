...
class TestClassifyFailureType(TestCase):
    @parameterized.expand(
        [
            ...
            ("DatastoreQueryTimeOut", FAILURE_TYPE_USER),
            ("DatastoreQueryMemoryLimitExceeded", FAILURE_TYPE_USER),
            ...
            ("DatastoreAtCapacity", FAILURE_TYPE_SYSTEM),
    ...
    @parameterized.expand(
        [
            ...
            # mislabelled when passed as a live instance — only the in-scope DRF/InsightsQL types count.
            (DjangoValidationError("not a query error"),),
            (SyntaxError("a Python syntax error, not InsightsQL's"),),
        ]
    )
