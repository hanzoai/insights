...
def _stub_schema_resolver():
    ...
    with patch(
        ...
        return_value="insights_data_imports_team_1",
    ):
        yield
...
def test_existing_incremental_batch_zero_merges_after_first_sync() -> None:
    ...
    ensure_columns.assert_called_once_with(conn, "insights_data_imports_team_1", "stripe_customers", _parquet_schema())
...
def test_marks_duckgres_batch_applied_after_successful_mutation() -> None:
    ...
    replace_table.assert_called_once()
    mark_applied.assert_called_once_with(conn, "insights_data_imports_team_1", batch=batch)
...
class TestTableExistsProbeAndCache:
    ...
    def test_second_check_on_same_connection_skips_the_probe(self):
        ...
        assert _table_exists(conn, "insights_data_imports_team_1", "customers") is True
        assert _table_exists(conn, "insights_data_imports_team_1", "customers") is True
