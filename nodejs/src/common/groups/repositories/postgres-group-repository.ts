    ...
    implements GroupRepository, RawPostgresGroupRepository, GroupRepositoryTransaction
{
    ...
    async updateGroupsBatch(updates: GroupPropertiesToSetUpdate[]): Promise<Group[]> {
        ...
        const { rows } = await this.postgres.query<RawGroup>(
            ...
            `
            UPDATE insights_group AS g SET
    ...
    async insertGroupsBatch(creates: GroupCreate[]): Promise<GroupCreateResult[]> {
        ...
        const { rows } = await this.postgres.query<RawGroup & { inserted: boolean }>(
            PostgresUse.PERSONS_WRITE,
            `
            INSERT INTO insights_group AS g (team_id, group_key, group_type_index, group_properties, created_at, properties_last_updated_at, properties_last_operation, version)
    ...
    async insertGroupType(
        ...
        // wall-clock date that postdates their events — InsightsQL masks $group_N for events older than this.
