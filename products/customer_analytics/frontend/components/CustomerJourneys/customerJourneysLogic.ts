import { MakeLogicType, actions, connect, kea, key, listeners, path, props, reducers, selectors } from 'kea'
import { lazyLoaders } from 'kea-loaders'
import insights from 'insights-js'

import api from 'lib/api'
import { SelectOptions } from 'lib/elements/Select/Select'
import { toast } from 'lib/elements/Toast/Toast'
import { eventUsageLogic } from 'lib/utils/eventUsageLogic'
import { isUUIDLike } from 'lib/utils/guards'

import { FunnelsQuery, InsightVizNode } from '~/queries/schema/schema-general'
import { isInsightVizNode } from '~/queries/utils'
import { insightsApi } from '~/scenes/insights/utils/api'
import { FunnelVizType, PropertyFilterType, PropertyOperator, QueryBasedInsightModel } from '~/types'

import { CustomerJourneyApi } from 'products/customer_analytics/frontend/generated/api.schemas'

import type { Node } from '../../../../../frontend/src/queries/schema/schema-general'

export interface CustomerJourneysLogicProps {
    key?: string
    personId?: string
    groupKey?: string
    groupTypeIndex?: number
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface customerJourneysLogicValues {
    activeInsight: QueryBasedInsightModel | null
    activeInsightLoading: boolean
    activeJourney: CustomerJourneyApi | null
    activeJourneyFullQuery: InsightVizNode<FunnelsQuery> | null
    activeJourneyId: string | null
    filteredQuery: InsightVizNode<FunnelsQuery> | null
    isProfileMode: boolean
    journeyOptions: SelectOptions<string>
    journeys: CustomerJourneyApi[]
    journeysLoading: boolean
    queryOverride: InsightVizNode<FunnelsQuery> | null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface customerJourneysLogicActions {
    reportCustomerJourneyViewed: (
        journeyId: string,
        journeyName: string,
        stepCount: number,
        delay?: number | undefined
    ) => {
        delay: number | undefined
        journeyId: string
        journeyName: string
        stepCount: number
    } // eventUsageLogic
    addJourney: ({ insightId, name, description }: { description?: string; insightId: number; name: string }) => {
        insightId: number
        name: string
        description?: string
    }
    addJourneyFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    addJourneySuccess: (
        journeys: CustomerJourneyApi[],
        payload?: {
            insightId: number
            name: string
            description?: string
        }
    ) => {
        journeys: CustomerJourneyApi[]
        payload?: {
            insightId: number
            name: string
            description?: string
        }
    }
    deleteJourney: (journeyId: string) => string
    deleteJourneyFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    deleteJourneySuccess: (
        journeys: CustomerJourneyApi[],
        payload?: string
    ) => {
        journeys: CustomerJourneyApi[]
        payload?: string
    }
    loadActiveInsight: () => any
    loadActiveInsightFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadActiveInsightSuccess: (
        activeInsight: QueryBasedInsightModel<Node<Record<string, any>>> | null,
        payload?: any
    ) => {
        activeInsight: QueryBasedInsightModel<Node<Record<string, any>>> | null
        payload?: any
    }
    loadJourneys: () => any
    loadJourneysFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadJourneysSuccess: (
        journeys: CustomerJourneyApi[],
        payload?: any
    ) => {
        journeys: CustomerJourneyApi[]
        payload?: any
    }
    selectFirstJourneyIfNeeded: (journeys: CustomerJourneyApi[]) => {
        journeys: CustomerJourneyApi[]
    }
    setActiveJourneyId: (journeyId: string | null) => {
        journeyId: string | null
    }
    setQueryOverride: (query: InsightVizNode<FunnelsQuery>) => {
        query: InsightVizNode<FunnelsQuery>
    }
    updateJourney: ({ journeyId, name, description }: { description?: string; journeyId: string; name: string }) => {
        journeyId: string
        name: string
        description?: string
    }
    updateJourneyFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    updateJourneySuccess: (
        journeys: CustomerJourneyApi[],
        payload?: {
            journeyId: string
            name: string
            description?: string
        }
    ) => {
        journeys: CustomerJourneyApi[]
        payload?: {
            journeyId: string
            name: string
            description?: string
        }
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface customerJourneysLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        journeyOptions: (journeys: CustomerJourneyApi[]) => SelectOptions<string>
        activeJourney: (journeys: CustomerJourneyApi[], activeJourneyId: string | null) => CustomerJourneyApi | null
        activeJourneyFullQuery: (
            activeInsight: QueryBasedInsightModel<Node<Record<string, any>>> | null,
            queryOverride: InsightVizNode<FunnelsQuery> | null
        ) => InsightVizNode<FunnelsQuery> | null
        isProfileMode: (arg: any, arg2: any) => boolean
        filteredQuery: (
            activeJourneyFullQuery: InsightVizNode<FunnelsQuery> | null,
            isProfileMode: boolean,
            arg: any,
            arg2: any,
            arg3: any
        ) => InsightVizNode<FunnelsQuery> | null
    }
}

export type customerJourneysLogicType = MakeLogicType<
    customerJourneysLogicValues,
    customerJourneysLogicActions,
    CustomerJourneysLogicProps,
    customerJourneysLogicMeta
>

export const customerJourneysLogic = kea<customerJourneysLogicType>([
    path(['products', 'customer_analytics', 'frontend', 'components', 'CustomerJourneys', 'customerJourneysLogic']),
    props({} as CustomerJourneysLogicProps),
    key((props) => props.key ?? 'default'),
    connect(() => ({
        actions: [eventUsageLogic, ['reportCustomerJourneyViewed']],
    })),
    actions({
        setActiveJourneyId: (journeyId: string | null) => ({ journeyId }),
        selectFirstJourneyIfNeeded: (journeys: CustomerJourneyApi[]) => ({ journeys }),
        setQueryOverride: (query: InsightVizNode<FunnelsQuery>) => ({ query }),
    }),
    lazyLoaders(({ values }) => ({
        journeys: {
            __default: [] as CustomerJourneyApi[],
            loadJourneys: async (): Promise<CustomerJourneyApi[]> => {
                const response = await api.customerJourneys.list()
                return response.results
            },
            addJourney: async ({
                insightId,
                name,
                description,
            }: {
                insightId: number
                name: string
                description?: string
            }): Promise<CustomerJourneyApi[]> => {
                await api.customerJourneys.create({ insight: insightId, name, description })
                const response = await api.customerJourneys.list()
                return response.results
            },
            updateJourney: async ({
                journeyId,
                name,
                description,
            }: {
                journeyId: string
                name: string
                description?: string
            }): Promise<CustomerJourneyApi[]> => {
                await api.customerJourneys.update(journeyId, { name, description })
                const response = await api.customerJourneys.list()
                return response.results
            },
            deleteJourney: async (journeyId: string): Promise<CustomerJourneyApi[]> => {
                await api.customerJourneys.delete(journeyId)
                return values.journeys.filter((j) => j.id !== journeyId)
            },
        },
        activeInsight: {
            __default: null as QueryBasedInsightModel | null,
            loadActiveInsight: async () => {
                const journey = values.activeJourney
                if (!journey) {
                    return null
                }
                return await insightsApi.getByNumericId(journey.insight)
            },
        },
    })),
    reducers({
        activeJourneyId: [
            null as string | null,
            {
                setActiveJourneyId: (_, { journeyId }) => journeyId,
            },
        ],
        queryOverride: [
            null as InsightVizNode<FunnelsQuery> | null,
            {
                setQueryOverride: (_, { query }) => query,
                setActiveJourneyId: () => null,
                loadActiveInsightSuccess: () => null,
            },
        ],
    }),
    listeners(({ actions, values }) => ({
        loadJourneysSuccess: ({ journeys }) => {
            actions.selectFirstJourneyIfNeeded(journeys)
        },
        addJourneySuccess: ({ journeys }) => {
            toast.success('Customer journey created')
            actions.selectFirstJourneyIfNeeded(journeys)
        },
        addJourneyFailure: ({ error }) => {
            insights.captureException(error)
            toast.error(error || 'Failed to create customer journey')
        },
        updateJourneySuccess: ({ journeys }) => {
            toast.success('Customer journey updated')
            actions.selectFirstJourneyIfNeeded(journeys)
        },
        updateJourneyFailure: ({ error }) => {
            insights.captureException(error)
            toast.error(error || 'Failed to update customer journey')
        },
        deleteJourneySuccess: ({ journeys }) => {
            toast.success('Customer journey deleted')
            actions.selectFirstJourneyIfNeeded(journeys)
        },
        deleteJourneyFailure: ({ error }) => {
            insights.captureException(error)
            toast.error('Failed to delete customer journey')
        },
        selectFirstJourneyIfNeeded: ({ journeys }) => {
            if (journeys.length > 0) {
                const currentActive = values.activeJourneyId
                const stillExists = currentActive && journeys.some((j: CustomerJourneyApi) => j.id === currentActive)
                if (!stillExists) {
                    actions.setActiveJourneyId(journeys[0].id)
                }
            } else {
                actions.setActiveJourneyId(null)
            }
        },
        setActiveJourneyId: () => {
            actions.loadActiveInsight()
        },
        loadActiveInsightSuccess: () => {
            const journey = values.activeJourney
            const insight = values.activeInsight
            if (journey && insight) {
                const stepCount =
                    (insight.query as InsightVizNode<FunnelsQuery> | undefined)?.source?.series?.length ?? 0
                actions.reportCustomerJourneyViewed(journey.id, journey.name, stepCount)
            }
        },
    })),
    selectors({
        journeyOptions: [
            (s) => [s.journeys],
            (journeys: CustomerJourneyApi[]): SelectOptions<string> =>
                journeys.map((journey) => ({
                    value: journey.id,
                    label: journey.name,
                })),
        ],
        activeJourney: [
            (s) => [s.journeys, s.activeJourneyId],
            (journeys: CustomerJourneyApi[], activeId: string | null): CustomerJourneyApi | null => {
                if (!activeId) {
                    return null
                }
                return journeys.find((j) => j.id === activeId) || null
            },
        ],
        activeJourneyFullQuery: [
            (s) => [s.activeInsight, s.queryOverride],
            (
                activeInsight: QueryBasedInsightModel | null,
                queryOverride: InsightVizNode<FunnelsQuery> | null
            ): InsightVizNode<FunnelsQuery> | null => {
                if (queryOverride) {
                    return queryOverride
                }
                const query = activeInsight?.query
                if (!query || !isInsightVizNode(query)) {
                    return null
                }
                const source = query.source as FunnelsQuery
                return {
                    ...query,
                    full: true,
                    source: {
                        ...source,
                        funnelsFilter: {
                            ...source.funnelsFilter,
                            funnelVizType: FunnelVizType.Flow,
                        },
                    },
                } as InsightVizNode<FunnelsQuery>
            },
        ],
        isProfileMode: [
            () => [(_, p) => p.personId, (_, p) => p.groupKey],
            (personId: string | undefined, groupKey: string | undefined): boolean => !!personId || !!groupKey,
        ],
        filteredQuery: [
            (s) => [
                s.activeJourneyFullQuery,
                s.isProfileMode,
                (_, p) => p.personId,
                (_, p) => p.groupKey,
                (_, p) => p.groupTypeIndex,
            ],
            (
                query: InsightVizNode<FunnelsQuery> | null,
                isProfileMode: boolean,
                personId,
                groupKey,
                groupTypeIndex
            ): InsightVizNode<FunnelsQuery> | null => {
                if (!query || !isProfileMode) {
                    return query
                }
                if (personId && !isUUIDLike(personId)) {
                    return query
                }
                const entityFilter = personId
                    ? { type: PropertyFilterType.InsightsQL, key: `person_id = '${personId}'` }
                    : {
                          type: PropertyFilterType.Event,
                          key: `$group_${groupTypeIndex}`,
                          value: [String(groupKey)],
                          operator: PropertyOperator.Exact,
                      }
                return {
                    ...query,
                    full: false,
                    embedded: true,
                    source: {
                        ...query.source,
                        properties: [
                            ...(Array.isArray(query.source.properties) ? query.source.properties : []),
                            entityFilter,
                        ],
                    },
                } as InsightVizNode<FunnelsQuery>
            },
        ],
    }),
])
