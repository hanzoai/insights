...
def _require_string(value: Any, name: str, function_name: str) -> str:
    if not isinstance(value, str):
        raise ScriptVMException(f"Function {function_name} requires {name} to be a string, got {type(value).__name__}")
    return value
...
def _compile_regex(pattern: str, case_insensitive: bool = False) -> Any:
    try:
        ...
        raise ScriptVMException(f"Invalid regex pattern: {_format_regex_error(e)}") from e
...
def set_nested_value(obj, chain, value) -> Any:
    ...
    if isinstance(obj, dict):
        ...
        if chain[-1] <= 0:
            raise ScriptVMException(f"Script arrays start from index 1")
        if chain[-1] > len(obj):
            raise ScriptVMException(f"Index {chain[-1]} out of range for array of length {len(obj)}")
        obj[chain[-1] - 1] = value
    else:
