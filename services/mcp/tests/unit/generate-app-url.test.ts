    ...
    return {
        api: {
            getProjectBaseUrl: (projectId: string): string =>
                projectId === '@current' ? 'https://us.hanzo.ai' : `https://us.hanzo.ai/project/${projectId}`,
        },
        stateManager: {
...
describe('generate-app-url', () => {
    ...
    it('builds a person-by-UUID link with the plural /persons slug', async () => {
        ...
        expect(result.url).toBe('https://us.hanzo.ai/project/354703/persons/12857b3c-2916-536b-af70-1e43c442a942')
    })
    ...
    it('builds a person-by-distinct-id link with the singular /person slug', async () => {
        ...
        expect(result.url).toBe('https://us.hanzo.ai/project/354703/person/pekerr%40ou.org')
    })
    ...
    it('builds a single session replay link', async () => {
        ...
        expect(result.url).toBe('https://us.hanzo.ai/project/354703/replay/019e83d3-d262-7543-a677-5ea82d8e785c')
    })
    ...
    it('builds a multi-param event link and URL-encodes values', async () => {
        ...
        expect(result.url).toBe('https://us.hanzo.ai/project/354703/events/evt_1/2026-06-01T15%3A48%3A00Z')
    })
    ...
    it('uses the bare host (no project prefix) for global-scope pages', async () => {
        ...
        expect(result.url).toBe('https://us.hanzo.ai/instance/status')
    })
