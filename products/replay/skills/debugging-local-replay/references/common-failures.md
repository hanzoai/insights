...
**Cause:** `bin/wait-for-docker` polls Docker health checks for core services (db, redis7, kafka, datastore).
...
docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "db|redis7|kafka|datastore"
...
```bash
insightscli stop && insightscli start
```
...
**Cause:** The `bin/insights-node` script runs `pnpm build:cyclotron:dev` which triggers
...
- `Refused to execute script from '.../insights-recorder.js' because its MIME type ('text/html') is not executable`
...
**Cause:** The static recorder script files (`insights-recorder.js`, `lazy-recorder.js`)
...
- Upgrading insights-js without rebuilding

**Fix:**

```bash
pnpm --filter=@hanzo/frontend build
pnpm copy-scripts
```
...
docker exec insights-kafka-1 /opt/bitnami/kafka/bin/kafka-consumer-groups.sh \
