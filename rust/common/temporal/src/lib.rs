//! Shared client for starting Insights Python workflows from Rust services.
//!
...
//! as `insights.temporal.common.codec.EncryptionCodec`.
...
/// Connection and encryption settings for a Insights Temporal namespace.
#[derive(Clone, Debug)]
...
/// inputs for Python workflows and encrypts them with Insights's payload codec.
#[derive(Clone)]
...
/// Mirrors `insights.temporal.common.codec._load_as_bytes` and `_prepare_key`.
...
mod tests {
    ...
    fn builds_encrypted_python_workflow_request() {
        ...
        assert_eq!(request.namespace, "insights.prod");
