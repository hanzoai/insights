...
const HYPERCACHE_OPERATION_COUNTER_NAME: &str = "insights_hypercache_operation";
...
/// Multi-tier cache writer for Insights, matching Django's HyperCache write behavior.
///
...
mod tests {
    ...
    async fn test_set_writes_to_redis_and_s3() {
        ...
        redis.set_ret("insights:1:cache/teams/123/feature_flags/flags.json", Ok(()));
        redis.del_ret(
            "insights:1:cache/teams/123/feature_flags/flags.json:etag",
            Ok(()),
        );
    ...
    async fn test_set_s3_key_format() {
        ...
        let mut redis = MockRedisClient::new();
        redis.set_ret("insights:1:cache/teams/789/feature_flags/flags.json", Ok(()));
        redis.del_ret(
            "insights:1:cache/teams/789/feature_flags/flags.json:etag",
            Ok(()),
        );
    ...
    async fn test_set_with_etag_returns_correct_etag() {
        ...
        let mut redis = MockRedisClient::new();
        redis.set_ret("insights:1:cache/teams/1/feature_flags/flags.json", Ok(()));
        redis.set_ret(
            "insights:1:cache/teams/1/feature_flags/flags.json:etag",
            Ok(()),
        );
    ...
    async fn test_delete_removes_from_both() {
        ...
        let mut redis = MockRedisClient::new();
        redis.del_ret("insights:1:cache/teams/123/feature_flags/flags.json", Ok(()));
        redis.del_ret(
            "insights:1:cache/teams/123/feature_flags/flags.json:etag",
            Ok(()),
        );
    ...
    async fn test_redis_error_propagates() {
        ...
        let mut redis = MockRedisClient::new();
        redis.set_ret(
            "insights:1:cache/teams/123/feature_flags/flags.json",
