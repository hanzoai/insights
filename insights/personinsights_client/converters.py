...
if TYPE_CHECKING:
    ...
    from insights.personinsights_client.proto.generated.personinsights.types.v1 import group_pb2, person_pb2
...
def proto_person_to_model(
    ...
    """
    from insights.models.person import Person as PersonModel
...
def proto_group_to_model(group: group_pb2.Group) -> Group:
    ...
    from insights.models.group.group import Group as GroupModel
...
def fetch_group_type_mapping_result(project_id: int, group_type_index: int) -> GroupTypeMappingResult | None:
    """Fetch a single GroupTypeMappingResult via the personinsights gRPC client.

    Raises RuntimeError if the client is not configured (so callers fall back
    to ORM, consistent with ``_fetch_group_types_via_personinsights``).
