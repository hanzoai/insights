...
import { computeBillingTotals } from './useInsightsAIBillingCalculations'
...
describe('computeBillingTotals', () => {
    it('applies markup per event, skipping pass-through products', () => {
        const tree = [
            makeNode({ $ai_billable: true, $ai_total_cost_usd: 10, ai_product: 'insights_ai' }, [
                ...
                makeNode({ $ai_billable: true, $ai_total_cost_usd: 5, ai_product: 'insights_code' }),
            ]),
        ]
    ...
    it('ignores non-billable generations', () => {
        const tree = [makeNode({ $ai_billable: false, $ai_total_cost_usd: 10, ai_product: 'insights_ai' })]
