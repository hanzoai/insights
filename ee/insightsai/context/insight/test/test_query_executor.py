...
class TestAssistantQueryExecutor(NonAtomicBaseTest):
    ...
    async def test_run_and_format_query_sql(self, mock_process_query):
        ...
        query = AssistantInsightsQLQuery(query="SELECT count() FROM events")
        result, used_fallback = await self.query_runner.arun_and_format_query(query)
    ...
    @patch("ee.hogai.context.insight.query_executor.process_query_dict")
    async def test_run_and_format_query_handles_exposed_insightsql_error(self, mock_process_query):
        """Test handling of ExposedInsightsQLError"""

        mock_process_query.side_effect = ExposedInsightsQLError("InsightsQL error")

        query = AssistantInsightsQLQuery(query="SELECT invalid")

        with self.assertRaises(MaxToolRetryableError) as context:
            await self.query_runner.arun_and_format_query(query)

        self.assertIn("InsightsQL error", str(context.exception))
    ...
    async def test_compress_results_full_ui_queries(self):
        ...
        # Test InsightsQLQuery -> AssistantInsightsQLQuery casting
        insightsql_query = InsightsQLQuery(query="SELECT 1")
        insightsql_response: dict[str, Any] = {"results": [{"count": 100}], "columns": ["count"]}
        result = await self.query_runner._compress_results(insightsql_query, insightsql_response)
        self.assertIn("count\n100", result)
...
class TestAssistantQueryExecutorAsync(NonAtomicBaseTest):
    ...
    async def test_sql_query_default_limit_is_applied(self):
        query = AssistantInsightsQLQuery(query="SELECT arrayJoin(range(1, 100001))")
        result, used_fallback = await self.query_runner.arun_and_format_query(query)
        self.assertFalse(used_fallback)
...
class TestExecuteAndFormatQuery(NonAtomicBaseTest):
    ...
    async def test_excludes_sql_schema(self, mock_process_query):
        ...
        # Create query with dateRange (which might have None values for date_from/date_to if not set)
        query = AssistantInsightsQLQuery(query="SELECT 1")
        result = await execute_and_format_query(self.team, query)
