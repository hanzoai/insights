...
function TaggerMetrics(): JSX.Element {
    ...
    if (runStatsLoading) {
        return <Skeleton className="h-80 w-full mb-6" />
    }
...
function AIObservabilityTagsContent(): JSX.Element {
    ...
    const columns: TableColumns<Tagger> = [
        {
            title: 'Name',
            key: 'name',
            render: (_, tagger) => (
            ...
            render: (_, tagger) => {
                ...
                if (providerKeyIssue) {
                    return (
                        ...
                        >
                            <Tag type="warning" icon={<IconWarning />} data-attr="tagger-status-key-issue">
                                Key issue
                            </Tag>
                        </Tooltip>
                    )
                    ...
                    return (
                        <div className="flex flex-wrap gap-1">
                            {dist.slice(0, 5).map((t) => (
                                <Tag key={t.name} type="option">
                                    {t.name} ({t.percent}%)
                                </Tag>
                            ))}
                            {dist.length > 5 && <Tag type="muted">+{dist.length - 5}</Tag>}
                        </div>
                    )
                }
        ...
        },
        {
            ...
            key: 'method',
            render: (_, tagger) => (
                <Tag type={tagger.tagger_type === 'script' ? 'option' : 'caution'}>
                    {tagger.tagger_type === 'script' ? 'Script' : 'LLM'}
                </Tag>
            ),
        },
        {
