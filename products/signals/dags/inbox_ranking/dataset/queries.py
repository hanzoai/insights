...
# All regions' label telemetry lands in the US dogfood project (Insights internal, team 2).
LABELS_TEAM_ID = 2

# Naming scheme from products/error_tracking/backend/indexed_embedding.py (the shared
# document-embeddings infrastructure): distributed_insights_document_embeddings_<model_name>.
EMBEDDINGS_TABLE = "distributed_insights_document_embeddings_text_embedding_3_small_1536"
...
def utc_bound(value: datetime.datetime) -> str:
    """Render a UTC instant for a InsightsQL toDateTime() placeholder. The explicit +00:00 offset is
    load-bearing: InsightsQL parses bare datetime strings in the querying team's timezone (US/Pacific
...
def insightsql_rows(sql: str, *, team: Team, query_type: str, snapshot_end: datetime.datetime) -> list[tuple[Any, ...]]:
    response = execute_insightsql_query(
        ...
        # than the 60s InsightsQL default; the sort key (team, date, event) keeps the scan bounded.
        settings=InsightsQLGlobalSettings(max_execution_time=600),
...
def canonical_stream_rows(rows: list[tuple[Any, ...]]) -> dict[str, tuple[Any, ...]]:
    ...
    Datastore groups on the raw client-supplied id, so spelling variants of one UUID arrive as
    ...
    id wins, so the choice does not depend on Datastore's row order."""
