...
class TestSignalReportArtefactViewSet(APIBaseTest):
    ...
    def test_put_reviewers_ignores_task_header_and_attributes_to_requesting_user(self):
        # The reviewers write is app/user-only, so a supplied X-Insights-Task-Id must be ignored and
...
class TestSignalReportArtefactLogWriteViewSet(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            (
                "commit",
                {"repository": "Insights/insights", "branch": "fix/foo", "commit_sha": "abc123f", "message": "fix"},
            ),
    ...
    @parameterized.expand(
        [
            ...
            ("repo_selection", {"repository": "insights/insights", "reason": "where the code lives"}),
            (
                "signal_finding",
                {
                    "signal_id": "sig-1",
...
class TestSignalReportArtefactAttribution(APIBaseTest):
    ...
    def test_post_with_header_attributes_to_task(self):
        ...
        response = self._post_note(report, headers={"X-Insights-Task-Id": str(task.id)})
        assert response.status_code == status.HTTP_201_CREATED, response.json()
        artefact = SignalReportArtefact.objects.get(id=response.json()["id"])
        assert str(artefact.task_id) == str(task.id)
    ...
    def test_post_with_foreign_team_task_header_returns_400(self):
        ...
        response = self._post_note(report, headers={"X-Insights-Task-Id": str(foreign_task.id)})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert not SignalReportArtefact.objects.filter(report=report).exists()
    ...
    def test_post_with_malformed_task_header_returns_400(self, header_value):
        report = self._create_report()
        response = self._post_note(report, headers={"X-Insights-Task-Id": header_value})
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_post_with_unknown_task_header_returns_400(self):
        report = self._create_report()
        response = self._post_note(report, headers={"X-Insights-Task-Id": str(uuid.uuid4())})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_list_exposes_created_by_and_task_id(self):
        ...
        task = self._create_task()
        assert (
            self._post_note(report, headers={"X-Insights-Task-Id": str(task.id)}).status_code == status.HTTP_201_CREATED
        )
...
class TestSignalReportCommitDiff(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("missing_branch", {"repository": "Insights/insights", "commit_sha": "abc123f", "message": "m"}),
        ]
    )
