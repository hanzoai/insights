    ...
    fixedProperties: [
        {
            ...
            values: [{ type: PropertyFilterType.InsightsQL, key: "event = '$pageview'" }],
        },
    ],
    after: '-7d',
    limit: 10,
...
describe('performWideEventsQueryInTwoPhases', () => {
    ...
    describe('24h pre-stage', () => {
        ...
        // Boundary table: each row asserts the full sequence of `after` values Datastore sees.
    ...
    describe('two-phase mechanics (pre-stage empty, falling through to caller window)', () => {
        ...
        it('returns the phase-1 response without running phase 2 when nothing matches', async () => {
            emptyPreStage()
            const emptyResponse = { results: [], columns: [], types: [], insightsql: 'SELECT' }
