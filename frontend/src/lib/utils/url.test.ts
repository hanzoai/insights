...
describe('url utils', () => {
    ...
    describe('getRelativeNextPath', () => {
        const location = {
            ...
            protocol: 'https:',
            host: 'us.hanzo.ai',
            hostname: 'us.hanzo.ai',
            href: 'https://us.hanzo.ai/',
        } as Location

        it('returns relative path for same-origin absolute URL', () => {
            expect(getRelativeNextPath('https://us.hanzo.ai/test', location)).toBe('/test')
        })

        it('returns relative path for same-origin absolute URL with query and hash', () => {
            expect(getRelativeNextPath('https://us.hanzo.ai/test?foo=bar#baz', location)).toBe('/test?foo=bar#baz')
        })

        it('returns relative path for encoded same-origin absolute URL', () => {
            expect(getRelativeNextPath('https%3A%2F%2Fus.hanzo.ai%2Ftest', location)).toBe('/test')
        })
        ...
        it('preserves percent-encoded characters in nested query params', () => {
            // e.g. docs "Run in Insights" links: ?next=/sql?open_query=SELECT%0A... — decoding
        ...
        it('preserves percent-encoded query params in same-origin absolute URL', () => {
            expect(getRelativeNextPath('https://us.hanzo.ai/sql?open_query=SELECT%0A%2A', location)).toBe(
