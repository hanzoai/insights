...
"""
Syncs person/group data from Datastore into the persons Postgres database
using raw psycopg, bypassing Django ORM and Django's database configuration.

Uses the persons_db_connection utility from insights.persons_db for connections,
...
def sync_persons_to_postgres(
    source_team_id: int, target_team_id: int, person_table_name: str = "insights_person"
) -> None:
    ...
    with persons_db_connection(writer=True) as conn:
        with conn.cursor() as cur:
            uuid_to_pk = _insert_persons(cur, datastore_persons, target_team_id, person_table_name)
            _insert_person_distinct_ids(cur, datastore_distinct_ids, target_team_id, uuid_to_pk)
            _insert_groups(cur, datastore_groups, target_team_id)


def _insert_persons(
    cur: psycopg.Cursor[Any],
    datastore_persons: list[dict[str, Any]],
    target_team_id: int,
    person_table_name: str,
) -> dict[str, int]:
    if not datastore_persons:
        return {}
...
def _insert_person_distinct_ids(
    cur: psycopg.Cursor[Any],
    datastore_distinct_ids: list[dict[str, Any]],
    target_team_id: int,
    uuid_to_pk: dict[str, int],
) -> None:
    if not datastore_distinct_ids:
        return
    ...
    if rows:
        cur.executemany(
            "INSERT INTO insights_persondistinctid (team_id, distinct_id, person_id, version) "
...
def bulk_create_group_type_mappings(
    ...
    try:
        with persons_db_connection(writer=True) as conn:
            with conn.cursor() as cur:
                cur.executemany(
                    "INSERT INTO insights_grouptypemapping "
...
def delete_group_type_mappings(project_id: int) -> None:
    with persons_db_connection(writer=True) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM insights_grouptypemapping WHERE project_id = %s",
                (project_id,),
            )
...
def get_group_type_mapping_count(project_id: int) -> int:
    with persons_db_connection(writer=False) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) FROM insights_grouptypemapping WHERE project_id = %s",
                (project_id,),
            )
            result = cur.fetchone()
