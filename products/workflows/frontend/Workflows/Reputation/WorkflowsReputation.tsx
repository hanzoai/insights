...
import { Banner, Input, Table, Tag, TagType, Link, Tooltip } from '@hanzo/elements'
...
const HEALTH_TAG: Record<AwsTenantReputationHealthEnumApi, { label: string; type: TagType }> = {
...
const RATE_LEVEL_TAG: Record<RateLevel, { label: string; type: TagType }> = {
    healthy: { label: 'Healthy', type: 'success' },
...
// Must match the endpoint's window (InsightsFlowViewSet.REPUTATION_WINDOW_DAYS) and cap
// (InsightsFlowViewSet.WORKFLOW_REPUTATION_LIMIT).
        ...
        <div className="mt-4 space-y-2" data-attr="workflows-reputation-aws-findings">
            ...
            {aws.findings.map((finding, index) => (
                <div key={index} className="border rounded p-3 flex gap-3 items-start">
                    <Tag type={finding.impact === 'HIGH' ? 'danger' : 'warning'}>
            ...
            <div className="flex items-center gap-2">
                ...
                {aws && (
                    <Tooltip title="Based on real mailbox feedback across all of this project's workflow email.">
                        <Tag type={HEALTH_TAG[aws.health].type} data-attr="workflows-reputation-health-tag">
    ...
    return (
        <div className="space-y-4" data-attr="workflows-reputation">
            {awsReputation?.sending_status === 'DISABLED' && (
                <Banner type="error" data-attr="workflows-reputation-disabled-banner">
            ...
            <Banner type="info" data-attr="workflows-reputation-beta-banner">
