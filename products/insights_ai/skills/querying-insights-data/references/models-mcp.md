...
| `$mcp_tool_description`    | Tool description as seen by the agent (revisions over time), clipped to 512 chars on capture. Gap warning: the hono migration silently dropped this stamp, so there is a window (roughly Jun–Jul 2026) with no descriptions on Insights's hosted server; `notEmpty(...)` filters are mandatory.                                                                                                                                                                                                    |
