...
import { combineUrl, encodeParams, router, urlToAction } from 'kea-router'
...
/** Send an already-authenticated visitor who landed on `/login` wherever `?next=` pointed. */
                ...
                if (activeExportedScene?.logic) {
                    try {
                        ...
                        if (e instanceof Error && e.message.includes('Undefined key for logic')) {
                            insights.captureException(e, { source: 'sceneLogic.activeSceneLogic' })
                            return null
                        }
                        throw e
                    }
        ...
        setScene: ({ sceneKey, sceneId, exportedScene, params, scrollToTop }, _, __, previousState) => {
            ...
            if (exportedScene?.logic) {
                try {
                    ...
                    insights.captureException(error, { extra: { sceneId, sceneKey } })
        ...
        const guardRoute =
            ...
            (params, searchParams, hashParams, payload) => {
                try {
                    ...
                    insights.captureException(error, { extra: { source: 'sceneLogic.urlToAction' } })
