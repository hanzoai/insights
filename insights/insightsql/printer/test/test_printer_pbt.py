...
from insightsql_parser_rs import parse_string_literal_text
...
#   RUN_PBT=1 pytest insights/insightsql/printer/test/test_printer_pbt.py
...
# escape_insightsql_identifier / escape_datastore_identifier).
...
_STRING_ESCAPE_FUNCTIONS: list[tuple[str, Callable[[Any], str]]] = [
    ("insightsql", escape_insightsql_string),
    ("datastore", escape_datastore_string),
]
...
class TestInsightsQLIdentifier:
    ...
    def test_percent_always_rejected(self, s: str) -> None:
        with pytest.raises(QueryError, match='is not permitted as it contains the "%" character'):
            escape_insightsql_identifier(s)


_BACKTICK_IDENTIFIER_FUNCTIONS: list[tuple[str, Callable[[str], str]]] = [
    ("insightsql", escape_insightsql_identifier),
    ("datastore", escape_datastore_identifier),
]


class TestBacktickIdentifierRoundTrip:
    """Round-trip tests shared by InsightsQL and Datastore backtick-escaped identifiers."""
...
class TestDatastoreIdentifier:
    """Property-based tests specific to escape_datastore_identifier."""

    @given(s=st.from_regex(r"[A-Za-z_][A-Za-z0-9_]*", fullmatch=True))
    def test_simple_identifiers_returned_bare(self, s: str) -> None:
        assert escape_datastore_identifier(s) == s
...
class TestPostgresIdentifier:
    ...
    parse_string_literal_text (which applies Datastore-style backslash
...
class TestNumericEscaping:
    ...
    def test_finite_floats_parse_back(self, f: float) -> None:
        result = escape_insightsql_string(f)
        parsed = float(result)
        if f == 0.0:
            assert parsed == 0.0
        else:
    ...
    def test_nan(self) -> None:
        assert escape_insightsql_string(float("nan")) == "NaN"

    def test_positive_inf(self) -> None:
        assert escape_insightsql_string(float("inf")) == "Inf"
    ...
    def test_integer_values_roundtrip(self, n: int) -> None:
        result = escape_insightsql_string(n)
        assert int(result) == n
