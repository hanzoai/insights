...
/**
 * The single, blessed way to talk to the Insights API from the toolbar.
 *
...
export interface ToolbarApiOptions {
    ...
    context: string
    /**
     * Surface a `toast.error` when the request fails. Pass `true` to show the
...
export function extractErrorDetail(body: unknown, status: number, fallback: string): string {
    if (status === 401 || status === 403) {
        return 'Your toolbar session lacks permission. Please re-authenticate the toolbar from Insights.'
    }
...
function emitToast(toastOnError: boolean | string, error: ToolbarApiErrorInfo): void {
    ...
    toast.error(extractErrorDetail(error.body, error.status, fallback))
}
