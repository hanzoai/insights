"""Task attribution for signals API writes, derived from the `X-Insights-Task-Id` header.
...
bakes the agent's task id into an `X-Insights-Task-Id` header on its MCP config (and the
...
TASK_ID_HEADER = "X-Insights-Task-Id"


def resolve_task_id_from_header(request: Request, team_id: int) -> str | None:
    """Return the validated task id from `X-Insights-Task-Id`, or None when the header is absent.
