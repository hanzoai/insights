...
class BackfillGroupTypeCreatedAtWorkflow(InsightsWorkflow):
    """Recompute insights_grouptypemapping.created_at from the earliest event per group.

    Imported historical events get masked in InsightsQL because the group type mapping's
    ...
    async def run(self, input: BackfillGroupTypeCreatedAtInput) -> dict:
        plan = await workflow.execute_activity(
            ...
            # The Datastore min() scans the project's events; give it room.
