...
//!
//! Requires local PG (insights_persons) like the other writer integration
//! tests; the target is `personinsights_person_tmp`, which carries the size
...
use personinsights_writer::store::PersonDb;
use serde_json::{json, Value};
use uuid::Uuid;

const TARGET_TABLE: &str = "personinsights_person_tmp";
...
async fn cleanup(pool: &sqlx::PgPool) {
    sqlx::query("DELETE FROM personinsights_person_tmp WHERE team_id = $1")
        .bind(WELD_TEAM_ID)
        .execute(pool)
        .await
        .unwrap();
...
async fn every_admitted_fixture_lands_through_the_writers_real_upsert() {
    ...
    for (index, (name, fixture)) in hostile_fixtures().into_iter().enumerate() {
        ...
        let (stored,): (Value,) = sqlx::query_as(
            "SELECT properties FROM personinsights_person_tmp WHERE team_id = $1 AND id = $2",
        )
        .bind(WELD_TEAM_ID)
        .bind(index as i64 + 1)
        .fetch_one(&pool)
