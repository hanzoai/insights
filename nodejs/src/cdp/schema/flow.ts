...
const FlowOutputVariableSchema = z.object({
    key: z.string(),
...
const legacyStringOutputVariable = z.string().transform((key): z.infer<typeof FlowOutputVariableSchema> => ({ key }))
...
export const FlowSchema = z.object({
    ...
    // Optional masking config for the trigger, allows Flows to be rate limited per distinct ID or other property
    trigger_masking: z
        .object({
    ...
    // Selected by the worker (INSIGHTS_FLOW_FIELDS); pg returns timestamptz as a Date, fixtures use
...
export interface Flow extends z.infer<typeof FlowSchema> {}
export type FlowAction = z.infer<typeof FlowActionSchema> & Record<string, unknown>
export interface FlowEdge extends z.infer<typeof FlowEdgeSchema> {}
