...
The managed migrations support tools let Insights staff triage customer batch imports (managed migrations) from any MCP client — Claude Code, Claude Desktop, Cursor — instead of Django admin.
...
Insights staff only (`is_staff = True` on your Insights cloud user).
...
await fetch('/api/personal_api_keys/', {
  ...
  headers: {
    ...
    'X-Csrftoken': document.cookie.match(/insights_csrftoken=([^;]+)/)[1],
  },
...
Add the Insights MCP server with the key as a bearer token.
...
Both connections can coexist with your everyday Insights MCP connection — tools are namespaced per server, so the staff tools stay out of your daily driver's tool list.
