...
3. **Ad-hoc InsightsQL join** (one-off). Plain `JOIN` / `LEFT JOIN` in a query or view for a single analysis, no
   persistence. Fine inside a view's InsightsQL when the join is specific to that model.
...
Insights ships a managed exchange-rate dimension so you don't build one:
...
`dim_currency`) carry descriptive attributes. On Insights, dimensions are aliased (often materialized) views
