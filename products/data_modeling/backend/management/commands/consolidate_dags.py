...
# (BaseInsightsQLError covers parse errors and the BoundedResolver family), a parent that is not a
...
ADOPTABLE_SYNC_ERRORS = (BaseInsightsQLError, UnknownParentError, ObjectDoesNotExist)
...
class Command(BaseCommand):
    ...
    def _list_dag_schedules_or_abort(self, dag_id: str) -> set[str]:
        """Authoritative listing of a DAG's execute-dag schedule ids via the InsightsDagId search
    ...
    def _teardown_execute_dag_schedules(self, temporal: Client, dag_id: str) -> bool:
        ...
        authoritative listing (InsightsDagId search attribute) rather than an id formula, so an
