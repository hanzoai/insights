...
class SignalSourceConfig(UUIDModel):
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, related_name="signal_source_configs")
...
class SignalUserAutonomyConfig(UUIDModel):
    user = models.OneToOneField("insights.User", on_delete=models.CASCADE, related_name="signal_autonomy_config")
...
class SignalReport(UUIDModel):
    ...
    # charged (Insights-system origins, e.g. health-check scout findings). Prospective-only —
    ...
    conversation = deprecate_field(
        models.ForeignKey("insights_ai.Conversation", null=True, blank=True, on_delete=models.SET_NULL)
    )
...
class SignalReportArtefact(UUIDModel):
    ...
    created_by = models.ForeignKey("insights.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="+")
...
class SignalReportRefund(TeamScopedRootMixin, UUIDModel):
    ...
    # FKs to the hot insights_team / insights_user tables use db_constraint=False so creating this
    ...
    created_by = models.ForeignKey(
        "insights.User", on_delete=models.SET_NULL, null=True, blank=True, db_constraint=False, related_name="+"
    )
...
class SignalScoutConfig(ModelActivityMixin, TeamScopedRootMixin, UUIDModel):
    ...
    # stays the activity log's job. `db_constraint=False` because insights_user is a hot table
...
class SignalScoutRun(TeamScopedRootMixin, UUIDModel):
    ...
    # Datastore scan. Parallel to `emitted_count` (`len(emitted_finding_ids) == emitted_count`).
...
class SignalScoutEmission(TeamScopedRootMixin, UUIDModel):
    ...
    Datastore signal store or parsing `source_id`. It complements, not replaces, that store:
    Datastore is keyed for embedding/grouping, lags emit by the fire-and-forget Temporal
...
class SignalScratchpad(TeamScopedRootMixin, UUIDModel):
    ...
    read surface is intentional product design: any agent (Insights AI, ad-hoc
    investigators, other scouts) can read what the scout fleet has learned about
    a team.

    Distinct in shape from Insights AI's memory primitives (`CoreMemory`,
...
class SignalScoutNote(TeamScopedRootMixin, UUIDModel):
    ...
    # FKs to the hot insights_team / insights_user tables use db_constraint=False so creating
...
class SignalRepositoryAreaActivity(TeamScopedRootMixin, UUIDModel):
    ...
    *area* is a path prefix (see `area_for_path`); `""` means the repository root. Rows are
    ...
    # db_constraint=False: creating an FK constraint locks the hot insights_team table and
