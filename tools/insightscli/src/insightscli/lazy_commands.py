"""Lazy command import resolution for insightscli manifests."""
...
def _parse_import_string(command_name: str, import_string: Any) -> tuple[str, str]:
    if not isinstance(import_string, str) or import_string.count(":") != 1:
        raise click.ClickException(
            f"insightscli: command {command_name!r} has invalid import string {import_string!r}; expected 'module.path:attr'"
        )
...
def resolve_click_command(command_name: str, import_string: Any) -> click.Command:
    ...
    try:
        ...
        raise click.ClickException(f"insightscli: command {command_name!r} could not import {module_path!r}: {exc}") from exc

    try:
        command = getattr(module, attr)
    except AttributeError as exc:
        raise click.ClickException(
            f"insightscli: command {command_name!r} could not resolve {import_string!r}: {exc}"
        ) from exc

    if not isinstance(command, click.Command):
        raise click.ClickException(
            f"insightscli: command {command_name!r} resolved {import_string!r} to "
            f"{type(command).__name__}, expected click.Command"
        )

    if command.name != command_name:
        raise click.ClickException(
            f"insightscli: command {command_name!r} resolved {import_string!r} with Click name "
