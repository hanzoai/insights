...
from posthog.insightsql.database.models import LazyJoinToAdd, LazyTableToAdd
from posthog.insightsql.errors import ResolutionError
from posthog.insightsql.resolver import resolve_types
from posthog.insightsql.resolver_utils import get_long_table_name
from posthog.insightsql.transforms.property_types import PropertySwapper
...
class LazyTableResolver(TraversingVisitor):
    ...
    def __init__(
        ...
        self.dialect: InsightsQLDialect = dialect
    ...
    def visit_property_type(self, node: ast.PropertyType):
        ...
        if isinstance(table_type, ast.LazyJoinType) or isinstance(table_type, ast.LazyTableType):
            if self.context and self.context.within_non_insightsql_query:
                # If we're in a non-InsightsQL query, traverse deeper, just like we normally would have.
