...
jest.mock('~/common/utils/insights', () => {
    const original = jest.requireActual('~/common/utils/insights')
    return {
        ...original,
...
describe('LogsIngestionConsumer', () => {
        ...
        const consumer = new LogsIngestionConsumer(
            hub,
            {
                ...
                outputs: new IngestionOutputs<AppMetricsOutput | LogsOutput | LogsDlqOutput>({
                    ...
                    [LOGS_OUTPUT]: new SingleIngestionOutput(LOGS_OUTPUT, KAFKA_LOGS_DATASTORE, mockProducer, 'test'),
    ...
    describe('app metrics emission (integration)', () => {
        ...
        describe('sampling usage to app_metrics2', () => {
            ...
            const outputHeaders = (): Record<string, string> | undefined =>
                getProducedKafkaMessages().find((m) => m.topic === KAFKA_LOGS_DATASTORE)?.headers as
    ...
    describe('metric rules (generate metrics from logs)', () => {
        ...
        it('produces logs normally when the rules cache fetch throws (fail-open)', async () => {
            ...
            const logsOutput = getProducedKafkaMessages().filter((m) => m.topic === KAFKA_LOGS_DATASTORE)
        ...
        it('still counts records that a drop rule removes (pre-drop semantics)', async () => {
            ...
            // The info row is gone from the Datastore-bound output...
            const logsOutput = getProducedKafkaMessages().filter((m) => m.topic === KAFKA_LOGS_DATASTORE)
            expect(logsOutput).toHaveLength(1)
    ...
    describe('thread relief', () => {
        ...
        beforeEach(async () => {
            ...
            await hub.postgres.query(
                PostgresUse.COMMON_WRITE,
                `UPDATE insights_team SET logs_settings = $1 WHERE id = $2`,
    ...
    describe('TracesIngestionConsumer billing identity', () => {
        ...
        it('rate-limits against its own Redis key namespace, not the logs one', () => {
            ...
            expect(prefix).toBe('@insights-test/traces-rate-limiter/tokens')
    ...
    describe('script log transformations', () => {
        let transformerConsumer: LogsIngestionConsumer

        const buildTransformer = async (script: string): Promise<LogsTransformerService> => {
            const fn = {
            ...
            const manager = {
                getInsightsFunctionsForTeams: jest.fn().mockResolvedValue({ [team.id]: [fn] }),
                getInsightsFunctionIdsForTeams: jest.fn().mockResolvedValue({ [team.id]: [fn.id] }),
            }
