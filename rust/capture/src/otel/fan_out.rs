...
/// events. User-set resource attributes (e.g. `user.id`, `insights.ai.debug`)
...
fn apply_geoip_default(properties: &mut Map<String, Value>) {
    let alias = properties.remove("insights.geoip_disable");
...
mod tests {
            ...
            resource_spans: vec![ResourceSpans {
                ...
                scope_spans: vec![ScopeSpans {
                    ...
                    spans: vec![make_span(
                        ...
                        "http POST https://us.i.hanzo.ai/e",
                        vec![make_kv(
                            "http.request.method",
    ...
    fn test_noisy_resource_attrs_are_filtered() {
        ...
        assert_eq!(props["insights.ai.debug"], "true");
    }
    ...
    // insights.geoip_disable alias on resource is copied into $geoip_disable.
    #[case::alias_opt_in(vec![("insights.geoip_disable", false)], vec![], false)]
    // Both set: canonical wins, alias is consumed (not duplicated).
    #[case::canonical_wins_over_alias(
        vec![("insights.geoip_disable", false)],
    ...
    fn test_geoip_default_behavior(
        ...
        assert!(!props.contains_key("insights.geoip_disable"));
    }
}
