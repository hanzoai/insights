...
/// - Datastore DateTime64 format: "2024-01-01 12:00:00.000000" (assumed UTC)
pub fn parse_timestamp(timestamp_str: &str) -> Option<u64> {
    ...
    // Try parsing as Datastore DateTime64 format: "2024-01-01 12:00:00.000000"
    // This format doesn't include timezone, so we assume UTC
    if let Some(millis) = parse_datastore_timestamp(timestamp_str) {
        return Some(millis);
    }
...
/// Parse Datastore DateTime64 format: "2024-01-01 12:00:00.000000"
/// Returns milliseconds since epoch (assumes UTC)
pub fn parse_datastore_timestamp(timestamp_str: &str) -> Option<u64> {
...
mod tests {
    ...
    #[test]
    fn test_parse_datastore_timestamp_with_microseconds() {
        // Datastore DateTime64(6) format
        let result = parse_datastore_timestamp("2024-01-01 12:00:00.000000");
    ...
    #[test]
    fn test_parse_datastore_timestamp_with_partial_microseconds() {
        // Datastore may have varying precision
        assert!(parse_datastore_timestamp("2024-01-01 12:00:00.123").is_some());
        assert!(parse_datastore_timestamp("2024-01-01 12:00:00.123456").is_some());
    }

    #[test]
    fn test_parse_datastore_timestamp_without_microseconds() {
        let result = parse_datastore_timestamp("2024-01-01 12:00:00");
        assert!(result.is_some());
        assert_eq!(result.unwrap(), 1704110400000);
    }

    #[test]
    fn test_parse_datastore_timestamp_invalid() {
        assert!(parse_datastore_timestamp("2024-01-01T12:00:00").is_none()); // T separator
        assert!(parse_datastore_timestamp("not-a-timestamp").is_none());
        assert!(parse_datastore_timestamp("").is_none());
    }

    #[test]
    fn test_parse_timestamp_accepts_datastore_format() {
        // The main parse_timestamp function should also accept Datastore format
        assert!(parse_timestamp("2024-01-01 12:00:00.000000").is_some());
        assert!(parse_timestamp("2024-01-01 12:00:00").is_some());
    }

    #[test]
    fn test_datastore_timestamp_consistency() {
        // Same timestamp in different formats should produce same milliseconds
        let datastore = parse_timestamp("2024-01-01 12:00:00.000000").unwrap();
        let rfc3339 = parse_timestamp("2024-01-01T12:00:00.000Z").unwrap();
        assert_eq!(datastore, rfc3339);
    }
}
