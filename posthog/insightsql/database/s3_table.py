...
from posthog.insightsql.database.models import FunctionCallTable
...
def build_function_call(
    ...
    if re.match(r"^https:\/\/.+\.blob\.core\.windows\.net\/", url):
        ...
        if regex_result is None:
            raise ExposedInsightsQLError("Can't parse Azure blob storage URL")
        ...
        if not access_key or not access_secret:
            raise ExposedInsightsQLError("Azure blob storage has no access key or secret")
