...
from products.review.backend.reviewer.constants import BLIND_SPOT_PASS_NUMBER, VALIDATION_MAX_ATTEMPTS
from products.review.backend.reviewer.tools.select_perspectives import ChunkSelectionDTO, PerspectiveSelectionDTO
...
from products.review.backend.temporal.types import ReviewPRWorkflowInputs
from products.review.backend.temporal.workflow import (
...
async def _run_full_review_pr_workflow(
    *,
    ...
    # isn't a Insights user, so the workflow skips the review. review_labeled_prs / review_inbox_prs
...
async def test_review_pr_workflow_skips_when_author_maps_to_no_user():
    ...
    # publishes. Guards against dropping the gate (which would review non-Insights authors' PRs empty).
