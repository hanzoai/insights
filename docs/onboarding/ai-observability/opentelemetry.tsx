    ...
    return [
            ...
            content: (
                <>
                    <CalloutBox type="fyi" icon="IconInfo" title="Full working examples">
                        <Markdown>
                            The [Node.js](https://github.com/Insights/insights-js/tree/main/examples/example-ai-openai)
                            and
                    ...
                    <Markdown>
                        Install the OpenTelemetry SDK, Insights's OpenTelemetry helper, and an OpenTelemetry
        ...
        },
        {
            ...
            content: (
                <>
                    ...
                    <Markdown>
                        {dedent`
                            Insights identifies each event using the \`insights.distinct_id\` attribute on the OpenTelemetry
            ...
            content: (
                <>
                    <Markdown>
                        ...
                        is captured. Insights receives the emitted `gen_ai.*` span and converts it into an
                        `$ai_generation` event.
                    </Markdown>
                    ...
                    <Blockquote>
                        <Markdown>
                            {dedent`
                            **Note:** If you want to capture LLM events anonymously, omit the \`insights.distinct_id\` resource attribute. See our docs on [anonymous vs identified events](https://hanzo.ai/docs/data/anonymous-vs-identified-events) to learn more.
                            `}
                        </Markdown>
                    </Blockquote>
        ...
        },
        {
            ...
            content: (
                <>
                    <Markdown>
                        Insights translates standard OpenTelemetry GenAI semantic convention attributes into the same
                        `$ai_*` event properties our native SDK wrappers emit, so traces look the same in Insights
                        whether they arrive through OpenTelemetry or a native wrapper. The most common mappings:
                    </Markdown>

                    <Markdown>
                        {dedent`
                            | OpenTelemetry attribute | Insights event property |
                            | --- | --- |
                    ...
                    <Markdown>
                        {dedent`
                            - **Custom attributes pass through.** Any Resource or span attribute that isn't part of the known mapping is forwarded onto the event as-is, so you can add dimensions like \`conversation_id\` or \`tenant_id\` and filter on them in Insights.
                            ...
                            - **Vercel AI SDK, Pydantic AI, and Traceloop/OpenLLMetry** emit their own namespaces (\`ai.*\`, \`pydantic_ai.*\`, \`traceloop.*\`) and Insights normalizes those to the same \`$ai_*\` properties.
        ...
        },
        {
            ...
            content: (
                <>
                    ...
                    <Markdown>
                        {dedent`
                            **Direct OTLP export.** If you run an OpenTelemetry Collector, or want to export from a language that isn't Python or Node.js, point any OTLP/HTTP exporter directly at Insights's AI ingestion endpoint. Insights accepts OTLP over HTTP in both \`application/x-protobuf\` and \`application/json\`, authenticated with a \`Bearer\` token. The endpoint is signal-specific (traces only), so use the \`OTEL_EXPORTER_OTLP_TRACES_*\` variants rather than the general \`OTEL_EXPORTER_OTLP_*\` ones (the SDK appends \`/v1/traces\` to the latter and would 404).
                        `}
                    </Markdown>
