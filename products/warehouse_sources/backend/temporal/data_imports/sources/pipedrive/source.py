...
class PipedriveSource(
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Enter your Pipedrive API token to sync your Pipedrive CRM data into the Insights Data warehouse.
...
You can find your personal API token in Pipedrive under **Settings > Personal preferences > API**. The token inherits your user's permissions, so make sure your user can access the data you want to sync.""",
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/pipedrive",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
            ...
            webhookSetupCaption="""Insights registers one webhook on your Pipedrive account using your API token. Pipedrive does not sign deliveries, so Insights generates HTTP auth credentials, sets them on the webhook, and rejects any delivery that does not send them back.
...
4. Fill in **HTTP Auth username** and **HTTP Auth password**, then enter the same two values below so Insights can verify deliveries
5. Click **Save**
            ...
            webhookFields=cast(
                ...
                [
                    ...
                    SourceFieldInputConfig(
                        ...
                        required=True,
                        placeholder="",
                        caption="The HTTP auth password set on the Pipedrive webhook. Insights checks both values on every delivery.",
                        secret=True,
                    ),
                ],
            ),
