...
def test_mutation_runner_waits_for_capacity_and_retries_rejected_enqueue() -> None:
    ...
    with patch("insights.datastore.cluster.time.sleep"):
...
def test_exception_summary(snapshot, cluster: DatastoreCluster) -> None:
...
def wait_and_check_mutations_on_shards(
    cluster: DatastoreCluster, shard_mutations: Mapping[HostInfo, MutationWaiter]
) -> None:
...
def test_alter_mutation_single_command(cluster: DatastoreCluster) -> None:
    table = EVENTS_DATA_TABLE()
    count = 100

    # Start from a clean table. This table is shared and Datastore writes are not
...
def test_cycle_marker_survives_format_query(cluster: DatastoreCluster) -> None:
    """Pins the load-bearing assumption: Datastore's `formatQuery` preserves the
...
def test_find_existing_mutations_handles_multiline_formatted_command(cluster: DatastoreCluster) -> None:
    """Regression test: Datastore's `formatQuery` wraps long/nested commands across multiple
    ...
    # matches what Datastore stored in system.mutations.command, multi-line text and all.
    existing = cluster.map_all_hosts(runner.find_existing_mutations).result()
...
def test_alter_mutation_multiple_commands(cluster: DatastoreCluster) -> None:
    table = EVENTS_DATA_TABLE()
    count = 100
...
def test_map_hosts_by_role() -> None:
    ...
    cluster = DatastoreCluster(bootstrap_client_mock)
    ...
    with patch.object(DatastoreCluster, "_DatastoreCluster__get_task_function", mock_get_task_function):
...
def test_data_cluster_overrides_migrations_cluster_data_nodes() -> None:
    """DATA nodes should come from data_cluster (insights), not migrations cluster (insights_migrations)."""
    # insights_migrations has incomplete DATA — only 1 shard
    migrations_cluster_hosts = [
        ("data-partial", "9000", "1", "1", "online", "data"),
        ("endpoints-1", "9000", "1", "2", "online", "endpoints"),
    ]
    # insights has the full topology — 3 shards
    data_cluster_hosts = [
    ...
    def mock_execute(query, params):
        if params.get("name") == "insights_migrations":
    ...
    cluster = DatastoreCluster(
        bootstrap_client_mock,
        cluster="insights_migrations",
        data_cluster="insights",
    )

    # Should have 3 shards from insights, not 1 from insights_migrations
    assert cluster.num_shards == 3

    # Host only present in insights_migrations should still be in extra_hosts
    ...
    with patch.object(DatastoreCluster, "_DatastoreCluster__get_task_function", mock_get_task_function):
        cluster.map_hosts_by_role(lambda _: (), node_role=NodeRole.DATA).result()
        assert executed_roles.count("data") == 4  # all 4 DATA nodes from insights
        executed_roles.clear()

        cluster.map_hosts_by_role(lambda _: (), node_role=NodeRole.ENDPOINTS).result()
        assert executed_roles.count("endpoints") == 1  # still from insights_migrations
...
def test_map_hosts_with_combined_roles() -> None:
    ...
    bootstrap_client_mock.execute = Mock(side_effect=mock_execute)
    cluster = DatastoreCluster(bootstrap_client_mock, satellite_clusters=["aux"])
...
def test_satellite_dedup_same_physical_host() -> None:
    """In local dev, satellite clusters point to the same Datastore node as the main cluster.
    ...
    def mock_execute(query, params):
        ...
        if "satellite_name" not in params:
            return [("datastore", "9000", "1", "1", "online", "data")]
        else:
            return [("datastore", "9000", "1", "1", "online", "data")]
...
def test_lightweight_delete(cluster: DatastoreCluster) -> None:
    table = EVENTS_DATA_TABLE()
    count = 100
...
def test_alter_mutation_force_parameter(cluster: DatastoreCluster) -> None:
...
def test_query_repr_redacts_password_in_list_parameters():
    rendered = repr(Query("INSERT INTO t VALUES", [{"db": "insights", "password": "pw"}]))
    assert "pw" not in rendered
    assert "'password': '[REDACTED]'" in rendered
