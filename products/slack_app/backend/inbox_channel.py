"""Find-or-create the ``#insights-inbox`` channel and persist it as the team's signal notification channel."""
...
INBOX_CHANNEL_NAME = "insights-inbox"
...
def _find_inbox_channel(slack: SlackIntegration) -> tuple[str, str] | None:
    """Return (channel_id, "#name") for an existing #insights-inbox public channel, else None.
...
def ensure_inbox_channel(integration: Integration) -> tuple[str, str] | None:
    """Find-or-create #insights-inbox, persist it as the team's default notification channel,
    ...
    Idempotent: reuses the configured channel, then any existing #insights-inbox, before creating.
    """
