...
const makeInsightsFlow = (overrides: Partial<InsightsFlow> & { id: string; waitUntil?: boolean }): InsightsFlow => {
...
const makeGlobals = (overrides: Partial<InsightsFunctionInvocationGlobals>): InsightsFunctionInvocationGlobals =>
    ({
        ...
        ...overrides,
    }) as InsightsFunctionInvocationGlobals
...
class MatcherUnderTest extends CdpHogflowSubscriptionMatcherConsumer {
    ...
    constructor() {
        ...
        ;(this as any).hogFlowManager = {
            ...
            getInsightsFlowsForTeam: jest.fn().mockResolvedValue([]),
        }
        ...
        ;(this as any).insightsFunctionMonitoringService = {
    ...
    public get queueAppMetricMock(): jest.Mock {
        return (this as any).insightsFunctionMonitoringService.queueAppMetric
    }
    ...
    public setInsightsFlows(map: Record<string, InsightsFlow>): void {
        const byTeam: Record<number, InsightsFlow[]> = {}
        for (const flow of Object.values(map)) {
            byTeam[flow.team_id] = byTeam[flow.team_id] ?? []
            byTeam[flow.team_id].push(flow)
        }
        ;(this as any).hogFlowManager.getInsightsFlowsForTeams.mockResolvedValue(byTeam)
        ;(this as any).hogFlowManager.getInsightsFlowsForTeam.mockImplementation((teamId: number) =>
            Promise.resolve(byTeam[teamId] ?? [])
        )
    }

    public async runWake(invocationGlobals: InsightsFunctionInvocationGlobals[]): Promise<void> {
...
describe('CdpHogflowSubscriptionMatcherConsumer', () => {
    ...
    describe('wakeMatchingWorkflows', () => {
        ...
        it('passes both distinct_ids and person_ids to the lookup query', async () => {
            ...
            matcher.setInsightsFlows({ 'flow-1': makeInsightsFlow({ id: 'flow-1' }) })
            await matcher.runWake([
                makeGlobals({}),
                makeGlobals({
        ...
        it('correlates team_id with distinct_id so a cross-team pairing is never queried or woken', async () => {
            ...
            matcher.setInsightsFlows({
                'flow-1': makeInsightsFlow({ id: 'flow-1', team_id: 1 }),
                'flow-2': makeInsightsFlow({ id: 'flow-2', team_id: 2 }),
            })
        ...
        it('scopes the lookup to qualifying flows, excluding non-wait flows on the same team', async () => {
            matcher.setInsightsFlows({
                'flow-1': makeInsightsFlow({ id: 'flow-1' }),
                'flow-2': makeInsightsFlow({ id: 'flow-2', waitUntil: false }),
            })
            await matcher.runWake([makeGlobals({})])
            const lookup = matcher.calls.find((c) => c.sql.includes('SELECT id, team_id, function_id'))!
            expect(lookup).not.toBeUndefined()
        ...
        it('skips cyclotron entirely when no team in the batch has a wait_until_condition or conversion goal', async () => {
            // Default: no insightsflows configured → team has nothing the matcher could wake.
        ...
        it('does not wake when the wait step has events without bytecode (fail-closed)', async () => {
            // InsightsFlowSerializer compiles bytecode at save time; a row that reaches the
            ...
            matcher.wakeRows = [{ ...matcher.findRows[0], state: stateBuffer({ currentAction: { id: 'wait_node' } }) }]
            matcher.updateRowCount = 1
            const flow = makeInsightsFlow({ id: 'flow-1' })
            ;(flow.actions as any[])[1].config.events[0].filters.bytecode = []
            matcher.setInsightsFlows({ 'flow-1': flow })
        ...
        it('wakes only matching subset when multiple candidates returned', async () => {
            ...
            const noMatchFlow = makeInsightsFlow({ id: 'flow-no-match' })
    ...
    describe('_parseKafkaBatch', () => {
        ...
        beforeEach(() => {
            ...
            matcher.setInsightsFlows({ 'flow-1': makeInsightsFlow({ id: 'flow-1', team_id: 1 }) })
        })
        ...
        it('keeps events with a distinct_id but no person_id, drops events with neither', async () => {
            ...
            const distinctIds = result.map((g: InsightsFunctionInvocationGlobals) => g.event.distinct_id).sort()
            ...
            const distinctOnly = result.find(
                (g: InsightsFunctionInvocationGlobals) => g.event.distinct_id === 'only-distinct'
            )!
        ...
        it('skips events whose team has no actionable flow before paying for getTeam + conversion', async () => {
            ...
            expect(result.map((g: InsightsFunctionInvocationGlobals) => g.project.id)).toEqual([1])
    ...
    describe('_parsePersonBatch', () => {
        ...
        it('maps a person mutation to $person_updated globals keyed on person_id only', async () => {
            ...
            const globals = result[0] as InsightsFunctionInvocationGlobals
        ...
        it('skips deleted persons, persons with no id, and persons whose team has no actionable flow', async () => {
            ...
            expect(result.map((g: InsightsFunctionInvocationGlobals) => g.person?.id)).toEqual(['person-uuid-1'])
    ...
    describe('_parseInternalEventsBatch', () => {
        ...
        it('skips events with no identifiers and no-flow teams, but keeps a person-only event', async () => {
            ...
            expect(result).toHaveLength(1)
            expect((result[0] as InsightsFunctionInvocationGlobals).person?.id).toBe('person-1')
        })
        ...
        it('drops a malformed message (schema parse failure) without throwing', async () => {
            ...
            expect(result).toHaveLength(1)
            expect((result[0] as InsightsFunctionInvocationGlobals).event.event).toBe('$insight_alert_firing')
        })
    })
    ...
    describe('processMoveBatch', () => {
        ...
        it('skips a job parked on a delay step and scopes the lock to wait actions only', async () => {
            ...
            const flow = makeInsightsFlow({ id: 'flow-1', team_id: 1 })
            flow.actions.push({
                id: 'delay_node',
                name: 'Delay',
                type: 'delay',
    ...
    describe('wake matrix: events / property / action combinations', () => {
        ...
        const wakeWith = (event: string, properties: Record<string, any> = {}): InsightsFunctionInvocationGlobals =>
            makeGlobals({ event: { ...makeGlobals({}).event, event, properties } })
        const matchingEvent = (): InsightsFunctionInvocationGlobals => wakeWith('wakeup_event')
        const actionEvent = (): InsightsFunctionInvocationGlobals => wakeWith('action_event')
        const propertyEvent = (): InsightsFunctionInvocationGlobals => wakeWith('some_event', { plan: 'growth' })
        const unrelatedEvent = (): InsightsFunctionInvocationGlobals => wakeWith('unrelated_event', { plan: 'starter' })

        interface WakeCase {
            name: string
            config: Record<string, any>
            event: () => InsightsFunctionInvocationGlobals
            woken: boolean
        }
