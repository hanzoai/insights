...
T_UNOWNED = "insights/api/test/test_shared/TestShared::test_unowned"
...
class TestTeamCIHealthAPI(DatastoreTestMixin, APIBaseTest):
    # The two-window split, per-(team, nodeid) qualification bar, and unowned bucketing all
    # live in the InsightsQL rollup, so regressions only surface against real seeded trace_spans
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        connect_github_source_without_data(cls.team, prefix="teams", repository="Insights/insights")
    ...
    def _span(
        ...
        *,
        ...
        resource_pairs = [
            "'ci.repository', 'Insights/insights'",
            f"'ci.run_id', '{run}'",
