...
from insights.test.base import _create_event, _create_person, snapshot_datastore_queries
...
from insights.insightsql.database.schema.test.base import RevenueAnalyticsManagedViewsetsTestMixin, RevenueAnalyticsTestBase
...
class TestPersonsRevenueAnalytics(TestPersonsRevenueAnalyticsMixin):
    ...
    def test_get_revenue_for_schema_source_for_id_join(self):
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            ...
            for query in queries:
                response = execute_insightsql_query(parse_select(query), self.team, user=self.user, modifiers=self.MODIFIERS)
    ...
    def test_revenue_aggregated_per_person_across_multiple_customers(self):
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            table_results = execute_insightsql_query(
                parse_select(
            ...
            persons_results = execute_insightsql_query(
                parse_select(
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            query = TrendsQuery(
                **{
                ...
                modifiers=InsightsQLQueryModifiers(personsOnEventsMode=mode),
            )
