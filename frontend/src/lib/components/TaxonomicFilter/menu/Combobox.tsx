...
const HIDDEN_FROM_CHIPS: ReadonlySet<TaxonomicFilterGroupType> = new Set([
    ...
    // DataWarehouse + InsightsQL expression have their own dedicated panels —
...
export interface MenuFilterComboboxProps {
     ...
     * mode (where the field is a `Input` in the trigger row). Omitted in
...
export function MenuFilterCombobox({
    ...
    const filtered = useMemo<MenuFilterEntry[]>(() => {
        ...
        if (!q) {
            ...
            // the query grows, so a valid match shown for "insights te" can
            // vanish at "insights team". So only Fuse locally-sourced groups
    ...
    // comparable to the control/pill variants by feature-flag value (Insights
    ...
    useEffect(() => {
        ...
        insights.capture('taxonomic filter empty result', {
            surface: TAXONOMIC_FILTER_SURFACE,
            groupType: telemetryGroupType,
            searchQuery: trimmed,
        })
    ...
    const handleCheckOtherCategories = useCallback((): void => {
        insights.capture('taxonomic filter menu category changed', {
            fromChip: activeChip,
            toChip: 'all',
            via: 'empty-state',
        })
    ...
    // In input-trigger mode it's a Input matching the surrounding scene's
    ...
    // columns. The Input (input-trigger) is full width on its own; the
 ...
 * `Input`. base-ui drives a controlled `<input>` through its `render` prop —
 * value/onChange/ref/keyboard plus the combobox a11y attrs (`role`,
 * `aria-activedescendant`, `aria-expanded`, …). `Input`'s typed props don't
            ...
            render={(autoProps) => {
                ...
                return (
                    <Input
                        ...
                        // are valid `<input>` attributes Input forwards but
