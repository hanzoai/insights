"""Backfill the error_tracking_fingerprint_issue_state Datastore table from Postgres.
...
from insights.kafka_client.topics import KAFKA_ERROR_TRACKING_FINGERPRINT_ISSUE_STATE
...
class Command(BaseCommand):
    help = "Backfill error_tracking_fingerprint_issue_state Datastore table from Postgres."
    ...
    def handle(
        ...
        **options,
        ...
        # DatastoreProducer has a TEST-mode bypass that runs `sync_execute(sql, data)`
        # directly, which is what the backfill tests rely on to count rows. In prod it
        # routes through `insights.kafka_client.routing.get_producer(topic=...)` and
