    ...
    describe('fetchGroupTypeIndex()', () => {
        ...
        it('stamps created_at from the provided event timestamp', async () => {
            ...
            const { rows } = await hub.postgres.query(
                ...
                "SELECT created_at FROM insights_grouptypemapping WHERE project_id = 2 AND group_type = 'foo'",
                undefined,
                'fetchCreatedAt'
            )
        ...
        it('uses next available index after a group type is deleted', async () => {
            ...
            await hub.postgres.query(
                PostgresUse.PERSONS_WRITE,
                "DELETE FROM insights_grouptypemapping WHERE project_id = 2 AND team_id = 2 AND group_type = 'E'",
                undefined,
                'deleteGroupType'
            )
            ...
            await hub.postgres.query(
                PostgresUse.PERSONS_WRITE,
                "DELETE FROM insights_grouptypemapping WHERE project_id = 2 AND team_id = 2 AND group_type = 'C'",
                undefined,
                'deleteGroupType'
            )
