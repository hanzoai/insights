...
import { CustomFunctionIconLogicProps, customFunctionIconLogic } from './customFunctionIconLogic'
...
export function CustomFunctionIconEditable({
    size = 'medium',
    ...props
}: CustomFunctionIconLogicProps & { size?: 'small' | 'medium' | 'large' }): JSX.Element {
    const { possibleIconsLoading, showPopover, possibleIcons, searchTerm } = useValues(customFunctionIconLogic(props))
    const { setShowPopover, setSearchTerm } = useActions(customFunctionIconLogic(props))
    ...
    const content = (
        ...
        >
            <CustomFunctionIcon size={size} src={props.src} />
        </span>
    )
                ...
                <div className="p-1 w-100 deprecated-space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {possibleIcons?.map((icon) => (
                            ...
                            >
                                <CustomFunctionIcon src={icon.url} />
                            </span>
                        )) ??
...
export function CustomFunctionIcon({
    src,
    size = 'medium',
    className,
}: {
