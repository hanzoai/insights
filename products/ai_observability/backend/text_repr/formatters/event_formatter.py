...
def format_evaluation_text_repr(event: dict[str, Any], options: FormatterOptions | None = None) -> str:
    ...
    # Runtime hint: script evals are deterministic; llm_judge verdicts are probabilistic
...
def _maybe_decode_json(value: Any) -> Any:
    ...
    On `insights.ai_events`, the heavy columns (`input`, `output`,
...
def format_event_text_repr_from_ai_events_row(row: dict[str, Any], options: FormatterOptions | None = None) -> str:
    """Format an event whose heavy fields come from `insights.ai_events` columns.
    ...
    Use this from query callers that read `insights.ai_events` directly (i.e.
