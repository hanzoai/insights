...
Insights has a data catalog with a semantic layer of **canonical metrics**. Before you build a model for a
...
- A result is canonical **only** when `status = 'approved'` AND `is_drifted = false`. Never present a
  ...
  `insights:data-catalog-metric-run` and cite it rather than re-deriving.
...
- **Annotate columns.** Use `insights:saved-query-column-annotations-create` to describe the view and each
