...
# agent reuses its shape but takes no instructions from it. The run holds full-scope Insights MCP
# access (`insights_mcp_scopes="full"` below) and publishes to a repository an outsider controls, so a
...
SELF_DRIVING_HEAD_BRANCH_PREFIX = "insights-self-driving/"
...
def _head_branch_instruction(head_branch: str) -> str:
    return (
        ...
        "PR from it. This name was pre-generated for this run: Insights uses it to link the opened PR "
...
def _build_autostart_task_description(
    *,
    ...
    return (
        ...
        "making the footer '*Created with [Insights Desktop](https://hanzo.ai/code?ref=pr) "
...
def _fetch_source_references(team_id: int, report_id: str) -> list[SignalSourceReference]:
    """PR traceability is best-effort: a Datastore hiccup here must not block auto-start."""
    try:
...
def _create_implementation_task_if_absent(
    *,
    ...
    if exempt_reason and task_id:
        ...
        # best-effort Datastore lookup, so it must not run under the lock).
...
def _resolve_autostart_fallback_user(team_id: int) -> User | None:
    ...
    deactivated) — the task mints a Insights OAuth token as this user, so a departed or disabled
...
def _resolve_triggering_user(
    ...
    auto-start — but the agent mints a Insights OAuth token under the task's user, so it must run as
...
async def maybe_autostart_implementation_task(
    *,
    ...
    the agent mints a Insights OAuth token under the task's user, so running as the assignee would
    ...
    *reviewers_content*.
    ...
    ``billing_exempt_reason`` lets a caller that knows its origin is Insights-system (e.g. a future
...
async def maybe_autostart_from_report_artefacts(*, team_id: int, report_id: str) -> None:
    ...
    await maybe_autostart_implementation_task(
        ...
        # which would let one user act under another's Insights identity (reviewer impersonation).
