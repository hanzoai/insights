...
The split between user-actionable and Insights-owned errors is load-bearing:
user errors disable the evaluation without failing the Temporal activity, while
Insights-owned errors still raise so Temporal/Grafana stays useful.
"""
...
EvaluationErrorOwner = Literal["user", "insights"]
