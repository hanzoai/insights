...
description: >
  Explore and diagnose a Insights endpoint's execution logs — error messages, failed runs, cache
...
Every endpoint run emits one execution log entry to Insights's `log_entries` store. This skill
