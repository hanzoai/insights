...
describe('supportLogic', () => {
    describe('snippet helpers', () => {
        const mockedGetReplayUrl = insights.get_session_replay_url as jest.Mock
        ...
        it('rewrites the session line to the internal golink for staff triage', () => {
            // insights-js returns a project-scoped path, not one rooted at the current origin's /replay/ —
    ...
    describe('submitSupportTicket routing', () => {
        const ZENDESK_URL = 'https://insightshelp.zendesk.com/api/v2/requests.json'
        ...
        afterEach(() => {
            logic?.unmount()
            delete (insights as any).conversations
        ...
        it('creates a conversations ticket instead of a Zendesk one when the flag is on', async () => {
            ...
            ;(insights as any).conversations = { isAvailable: () => true, sendMessage }
        ...
        it.each([
            ...
            ;(insights as any).conversations = conversations
            if (flagOn) {
                enableConversationsFlag()
            }
        ...
        it('waits for the lazily-loaded extension before falling back to Zendesk', async () => {
            ;(insights as any).conversations = undefined
            enableConversationsFlag()
        ...
        it('captures insights_ai_support_ticket_created once on the conversations path when AI context is present', async () => {
            const sendMessage = jest.fn().mockResolvedValue({ ticket_id: 't1' })
            ;(insights as any).conversations = { isAvailable: () => true, sendMessage }
            enableConversationsFlag()
            ...
            expect(aiTicketCaptures()).toEqual([
                [
                    'insights_ai_support_ticket_created',
                    {
                        $ai_conversation_id: 'conv-1',
        ...
        it('captures insights_ai_support_ticket_created on the Zendesk fallback path with the zendesk ticket id', async () => {
