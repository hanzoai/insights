...
class TestProcessInsightsCodeTaskTermination(TestCase):
    def setUp(self):
    ...
    def _make_payload(self, run_id: str | None = None, user_id: str = "U_ALICE") -> dict:
        ...
        return {
            "type": "block_actions",
            "actions": [{"action_id": "insights_code_terminate_task", "value": value, "action_ts": "1234567890.123"}],
            "user": {"id": user_id},
            "team": {"id": "T_SLACK"},
            "channel": {"id": "C123"},
    ...
    def test_command_dispatched_on_success(self, mock_sync_connect, mock_send_cancel, mock_token):
        ...
        with patch("insights.models.integration.SlackIntegration"):
    ...
    def test_still_signals_on_4xx(self, mock_send_cancel, mock_token):
        ...
        with patch("insights.temporal.common.client.sync_connect") as mock_sync_connect:
            mock_handle = MagicMock()
            mock_client = MagicMock()
            mock_client.get_workflow_handle.return_value = mock_handle
            mock_sync_connect.return_value = mock_client
