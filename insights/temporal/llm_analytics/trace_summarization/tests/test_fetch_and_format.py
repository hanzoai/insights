...
from insights.temporal.llm_analytics.trace_summarization.fetch_and_format import (
    _format_generation_text_repr,
    fetch_and_format_activity,
)
from insights.temporal.llm_analytics.trace_summarization.models import FetchAndFormatInput
...
@patch(
    "insights.temporal.llm_analytics.trace_summarization.fetch_and_format.Heartbeater",
    _noop_heartbeater,
)
class TestFetchAndFormatActivity:
    ...
    async def test_trace_not_found_returns_skipped(self, mock_team):
        with (
            patch("insights.temporal.llm_analytics.trace_summarization.fetch_and_format.fetch_trace") as mock_fetch,
        ):
    ...
    async def test_generation_not_found_returns_skipped(self, mock_team):
        with patch(
            "insights.temporal.llm_analytics.trace_summarization.fetch_and_format.execute_insightsql_query"
        ) as mock_query:
    ...
    async def test_trace_success_stores_in_redis(self, mock_team):
        ...
        with (
            patch("insights.temporal.llm_analytics.trace_summarization.fetch_and_format.fetch_trace") as mock_fetch,
            patch(
                "insights.temporal.llm_analytics.trace_summarization.fetch_and_format.llm_trace_to_formatter_format"
            ) as mock_to_format,
            patch(
                "insights.temporal.llm_analytics.trace_summarization.fetch_and_format.format_trace_text_repr"
            ) as mock_format,
            patch("insights.temporal.llm_analytics.trace_summarization.fetch_and_format.get_async_client"),
            patch("insights.temporal.llm_analytics.trace_summarization.fetch_and_format.store_text_repr") as mock_store,
        ):
    ...
    async def test_oversized_trace_returns_skipped(self, mock_team):
        ...
        with (
            ...
            patch("insights.temporal.llm_analytics.trace_summarization.fetch_and_format.MAX_RAW_TRACE_SIZE", 10),
        ):
            mock_person = LLMTracePerson(
                uuid=str(uuid.uuid4()), distinct_id="test", created_at=datetime.now(UTC).isoformat(), properties={}
            )
