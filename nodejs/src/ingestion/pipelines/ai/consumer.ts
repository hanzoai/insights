...
export type AiConsumerConfig = CommonIngestionConsumerConfig &
    ...
    > &
    Pick<CommonConfig, 'CDP_FN_WATCHER_SAMPLE_RATE'>
 ...
 * script transformer and outputs, which the server must build because the lane can't
 * construct the cdp-owned transformer (boundary) and the same outputs instance
 * backs the transformer's monitoring. The personinsights client is owned by the AI
...
export function createAiConsumer(config: AiConsumerConfig, sharedScope: AiSharedScope) {
    ...
    // Client name for personinsights read metrics: pipeline + lane (e.g. "ai/main").
    ...
    return new CommonIngestionConsumerScope('ai', config, scope, ({ container }) =>
        createAiIngestionPipeline({
            ...
            cdpHogWatcherSampleRate: config.CDP_FN_WATCHER_SAMPLE_RATE,
