...
import { ReplayPlugin, Replayer, playerConfig } from 'insights-js/rrweb'
import { EventType, IncrementalSource, eventWithTime } from 'insights-js/rrweb-types'
import type { customEvent } from 'insights-js/rrweb-types'
...
function rejectHeatmapSnapshot(reason: keyof typeof SNAPSHOT_REJECTION_PROBLEM, htmlChars: number): void {
    insights.capture('in-app heatmap background snapshot rejected', { reason, html_chars: htmlChars })
    toast.error(
...
export interface sessionRecordingPlayerLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        ...
        toRRWebPlayerTime: (
            sessionPlayerData: SessionPlayerData,
            currentSegment: null | import('@hanzo/replay-shared').RecordingSegment
...
export const sessionRecordingPlayerLogic = kea<sessionRecordingPlayerLogicType>([
    ...
    listeners(({ props, values, actions, cache }) => ({
        ...
        snapshotSourceLoadExhausted: () => {
            console.error('Insights Recording Playback Error: A snapshot source repeatedly failed to load')
            actions.setPlayerError(
                values.isSnapshotUnauthorized ? 'snapshotUnauthorized' : 'snapshotSourceLoadExhausted'
            )
        },
        snapshotProcessingFailed: () => {
            console.error('Insights Recording Playback Error: Snapshot processing repeatedly failed')
        ...
        seekToTimestamp: ({ timestamp, forcePlay }, breakpoint) => {
            ...
            for (let hops = 0; target != null && hops < 10; hops++) {
                ...
                insights.capture('recording player seek clamped to next full snapshot', {
                    sessionId: values.sessionRecordingId,
