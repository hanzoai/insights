...
from insights.temporal.usage_report.storage import (
    chunk_key,
    chunks_prefix,
    delete_keys,
    manifest_key,
...
from insights.temporal.usage_report.types import WorkflowContext
...
def test_bucket_reads_billing_setting() -> None:
    ...
    with patch("insights.temporal.usage_report.storage.settings") as mock_settings:
        mock_settings.BILLING_USAGE_REPORTS_S3_BUCKET = "insights-billing-usage-reports"
        assert storage.bucket() == "insights-billing-usage-reports"
...
def test_write_jsonl_chunk_gzip_does_one_put_object_call() -> None:
    ...
    """
    with patch("insights.temporal.usage_report.storage.object_storage.write") as mock_write:
        write_jsonl_chunk_gzip(
            "some/key.jsonl.gz",
            [
...
def test_delete_keys_continues_on_failure() -> None:
    ...
    with patch("insights.temporal.usage_report.storage.object_storage.delete", side_effect=fake_delete):
