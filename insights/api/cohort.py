...
from insights.schema import ActorsQuery, InsightsQLQuery, ProductKey

from insights.insightsql.compiler.bytecode import create_bytecode
from insights.insightsql.constants import CSV_EXPORT_LIMIT, LimitContext
...
from insights.insightsql.property import PERSON_METADATA_FIELDS, property_to_expr

from insights.api.forbid_destroy_model import ForbidDestroyModel
from insights.api.routing import TeamAndOrgViewSetMixin
from insights.api.services.flags_service import FlagVersionConflictError, batch_evaluate_flag_for_team
...
from insights.models.filters.utils import earliest_timestamp_func
from insights.models.person.util import get_person_by_uuid, validate_person_uuids_exist
from insights.models.property.property import Property
...
from insights.utils import format_query_params_absolute_url, str_to_bool
...
# Mirrors SerializedPerson in insights/queries/actor_base_query.py.
...
# Note: is_date_exact is not yet supported (see insights/models/property/util.py)
# Keep in sync with OperatorType in insights/models/property/property.py
...
class CohortFilters(BaseModel, extra="forbid"):
    ...
    # into the cohort criteria at calculation time (see insightsql_cohort_query.py).
...
class CSVConfig:
    ...
    class ErrorMessages:
        ...
        MISSING_ID_COLUMN = "Multi-column CSV must contain at least one column with a supported ID header: 'person_id', 'Person .id' (Insights export format), 'distinct_id', 'distinct-id', or 'email'. Found columns: {columns}"
...
class CohortSerializer(SearchMatchTypeSerializerMixin, serializers.ModelSerializer):
    ...
    def _handle_static(
        ...
        from insights.tasks.calculate_cohort import (
    ...
    def _validate_and_process_ids(
        ...
        from insights.tasks.calculate_cohort import calculate_cohort_from_list
    ...
    def _validate_warehouse_access(self, attrs: dict) -> None:
        ...
        try:
            if properties:
                InsightsQLCohortQuery(
                    filter=Filter(data={"properties": properties}, team=team), team=team
                ).get_query_executor(user=user).generate_datastore_sql()
            if query:
                context = InsightsQLContext(team_id=team.pk, team=team, user=user, enable_select_queries=True)
                runner = get_query_runner(query, team=team, limit_context=LimitContext.COHORT_CALCULATION)
                prepare_ast_for_printing(runner.to_query(), context=context, dialect="datastore")
...
class CohortViewSet(TeamAndOrgViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def persons(self, request: Request, **kwargs) -> Response:
        ...
        with personinsights_caller_tag("cohorts/persons"):
    ...
    def remove_person_from_static_cohort(self, request: request.Request, **kwargs):
        ...
        # distinct-id fetch.
        with personinsights_caller_tag("cohorts/remove-person"):
