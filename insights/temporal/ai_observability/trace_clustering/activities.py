...
from insights.temporal.ai_observability.team_discovery import get_min_traces_override
from insights.temporal.ai_observability.trace_clustering import constants
from insights.temporal.ai_observability.trace_clustering.clustering import (
...
from insights.temporal.ai_observability.trace_clustering.data import (
    fetch_item_embeddings_for_clustering,
    fetch_item_summaries,
)
from insights.temporal.ai_observability.trace_clustering.event_emission import emit_cluster_events
from insights.temporal.ai_observability.trace_clustering.labeling import generate_cluster_labels
...
async def perform_clustering_compute_activity(inputs: ClusteringActivityInputs) -> ClusteringComputeResult:
    ...
    async with Heartbeater():
        ...
        except DatastoreAtCapacity as e:
            # The shared Datastore pool is saturated. Retrying immediately would only add load,
            # so back off with jitter to give the pool room to recover before the next attempt.
            backoff = random.uniform(
                constants.DATASTORE_AT_CAPACITY_BACKOFF_MIN.total_seconds(),
                constants.DATASTORE_AT_CAPACITY_BACKOFF_MAX.total_seconds(),
            )
            logger.warning(
                "clustering_compute_datastore_at_capacity",
                team_id=inputs.team_id,
                analysis_level=inputs.analysis_level,
                backoff_seconds=round(backoff, 1),
            )
            raise ApplicationError(
                "Datastore at capacity while fetching embeddings for clustering",
                type="DatastoreAtCapacity",
