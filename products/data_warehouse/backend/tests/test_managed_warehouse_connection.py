...
from insights.insightsql.direct_connection import get_direct_connection_source
...
def _source_schema(table_name: str, source_schema: str = "insights") -> SourceSchema:
    return SourceSchema(
...
class TestReconcileManagedWarehouseTables:
    ...
    def test_discovers_the_whole_org_catalog_and_makes_it_queryable(self) -> None:
        ...
        discovered = [
            ...
            _source_schema("customers", "insights_data_imports_prod"),
        ...
        assert set(
            ...
            "insights.events_prod",
            "insights.persons_prod",
            "insights.events_other",
            "insights_data_imports_prod.customers",
        ...
        allowed_query = InsightsQLQueryExecutor(
            query="SELECT uuid FROM insights.events_prod",
            team=team,
            connection_id=str(source.id),
        )
        sql, _context = allowed_query.generate_datastore_sql()
        ...
        with patch("insights.insightsql.direct_sql.postgres_adapter.psycopg.connect") as connect:
        ...
        cross_team_query = InsightsQLQueryExecutor(
            query=f"SELECT uuid FROM team_{other_team.id}.orders",
            team=team,
            connection_id=str(source.id),
        )
        cross_sql, _context = cross_team_query.generate_datastore_sql()
    ...
    def test_periodic_sweep_schedules_every_managed_project(self) -> None:
        ...
        with (
            patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=all_rows),
            patch(
    ...
    def test_periodic_sweep_skips_run_when_control_plane_unreachable(self) -> None:
        with (
            patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=None),
            patch(
                "products.data_warehouse.backend.tasks.tasks.schedule_managed_warehouse_tables_reconcile"
            ) as schedule,
        ):
    ...
    def test_reconciles_for_a_legacy_shared_tables_team(self) -> None:
        ...
        get_schemas.assert_called_once()
        source = ExternalDataSource.objects.get(team_id=team.id, prefix=MANAGED_WAREHOUSE_SOURCE_PREFIX)
        assert ExternalDataSchema.objects.filter(source_id=source.id, name="insights.events").exists()
