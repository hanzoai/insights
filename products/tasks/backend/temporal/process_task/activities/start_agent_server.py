...
def _resolve_protected_base_branch(ctx: TaskProcessingContext) -> str | None:
    ...
    insights-code/* branch the agent is meant to update — the agent must commit *to* that branch, so the
...
def _prepare_launch(ctx: TaskProcessingContext, scopes: InsightsMcpScopes, sandbox_id: str) -> _LaunchParams:
    try:
    ...
    if mcp_configs:
        ...
        emit_agent_log(
            ctx.run_id,
            "warn",
            "No MCP configs were resolved for this run. Insights MCP tools will be unavailable in the agent session.",
        )
...
def start_agent_server(input: StartAgentServerInput) -> StartAgentServerOutput:
    ...
    with log_activity_execution(
        ...
        **ctx.to_log_context(),
        ...
        params = _prepare_launch(ctx, input.insights_mcp_scopes, input.sandbox_id)
