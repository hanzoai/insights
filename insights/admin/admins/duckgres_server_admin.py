...
from insights.models import DuckgresServer, Organization, Team, User
...
class DuckgresServerAdmin(admin.ModelAdmin):
    ...
    change_list_template = "admin/insights/duckgres_server/change_list.html"
    change_form_template = "admin/insights/duckgres_server/change_form.html"
    ...
    def provision_view(self, request: HttpRequest) -> HttpResponse:
        ...
        if request.method == "GET":
            return render(
                request,
                "admin/insights/duckgres_server/provision_form.html",
        ...
        if team is None:
            return redirect(reverse("admin:insights_duckgresserver_provision"))
        ...
        if 200 <= resp.status_code < 300:
            ...
            return render(
                request,
                "admin/insights/duckgres_server/provision_result.html",
                {
    ...
    def enable_backfill_view(self, request: HttpRequest, object_id: str) -> HttpResponse:
        ...
        if request.method == "GET":
            return render(
                request,
                "admin/insights/duckgres_server/enable_backfill_form.html",
                {
                    **self.admin_site.each_context(request),
        ...
        if team is None:
            return redirect(reverse("admin:insights_duckgresserver_enable_backfill", args=[object_id]))
        ...
        if 200 <= resp.status_code < 300:
            return redirect(reverse("admin:insights_duckgresserver_change", args=[object_id]))
        return redirect(reverse("admin:insights_duckgresserver_enable_backfill", args=[object_id]))
    ...
    def deprovision_view(self, request: HttpRequest, object_id: str) -> HttpResponse:
        ...
        if request.method == "GET":
            return render(
                request,
                "admin/insights/duckgres_server/deprovision_confirm.html",
                {
                    **self.admin_site.each_context(request),
        ...
        if 200 <= resp.status_code < 300:
            return redirect(reverse("admin:insights_duckgresserver_changelist"))
        return redirect(reverse("admin:insights_duckgresserver_change", args=[object_id]))
