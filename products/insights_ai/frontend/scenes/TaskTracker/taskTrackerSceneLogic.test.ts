...
describe('taskTrackerSceneLogic', () => {
    ...
    // Insights AI can run without a repo: a description-only submit must still create and run the task with a
    ...
    it('marks seeded context sent for the created task so the first follow-up will not re-wrap it', async () => {
        ...
        expect(runBody?.pending_user_message).toContain('<insights_untrusted_context>')
