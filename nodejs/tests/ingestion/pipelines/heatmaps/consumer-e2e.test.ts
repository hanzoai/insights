...
async function fetchHeatmaps(datastore: Datastore, teamId: number): Promise<any[]> {
    return (await datastore.query(`SELECT * FROM heatmaps WHERE team_id = ${teamId}`)) as unknown as any[]
}
...
describe('Heatmaps consumer E2E', () => {
    const testWithTeamIngester = createTestWithTeamIngester({}, (infra) => {
        ...
        // produces to — so the registry's heatmaps producer reaches Datastore.
    ...
    testWithTeamIngester('extracts heatmap data to Datastore', {}, async ({ ingester, team, token }) => {
        ...
        await waitForExpect(async () => {
            const heatmaps = await fetchHeatmaps(datastore, team.id)
