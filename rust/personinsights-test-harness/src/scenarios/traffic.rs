...
//! insights_person; a startup sentinel round-trip proves the router serves
...
use personinsights_proto::personinsights::types::v1::ConsistencyLevel;
...
pub async fn run(args: TrafficArgs) -> Result<()> {
    ...
    gauge!("personinsights_traffic_enabled").set(if args.enabled { 1.0 } else { 0.0 });
    if !args.enabled {
    ...
    gauge!("personinsights_traffic_chaos_enabled").set(if args.chaos_enabled { 1.0 } else { 0.0 });
    ...
    loop {
        ...
        counter!("personinsights_traffic_epochs_total").increment(1);
        gauge!("personinsights_traffic_epoch_target_rps").set(rate);
        ...
        gauge!("personinsights_traffic_last_epoch_completed_timestamp_seconds").set(
            SystemTime::now()
...
/// personinsights charts follow: `app.kubernetes.io/name` equals the release
...
async fn run_hostile(
    ...
    while Instant::now() < deadline && !stop.load(Ordering::Relaxed) {
        ...
        counter!(
            "personinsights_traffic_hostile_total",
...
mod tests {
    ...
    fn vacuous_or_panicking_configurations_are_rejected() {
        let valid = TrafficArgs {
            ...
            pg_target_table: "personinsights_person_tmp".to_string(),
            pool_size: 200,
            ...
            chaos_leader_namespace: "personinsights-leader".to_string(),
            chaos_router_namespace: "personinsights-router-leader".to_string(),
            chaos_writer_namespace: "personinsights-writer".to_string(),
            chaos_etcd_endpoints: None,
            chaos_etcd_prefix: "/personinsights/".to_string(),
        };
