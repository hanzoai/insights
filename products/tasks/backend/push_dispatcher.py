...
1. **Feature flag.** ``insights-code-mobile-push`` must be enabled for the
...
from insights.tasks.push_notifications import send_user_push
...
PUSH_TITLE = "Insights Desktop"
FEATURE_FLAG_KEY = "insights-code-mobile-push"
...
def _notify_task_thread_message(message: TaskThreadMessage, mentioned_user_ids: Collection[int]) -> None:
    ...
    author_name = (author.first_name.strip() or author.email) if author else "Insights"
