...
class _LazyDagsterLogger:
    ...
    django.setup() (via insights.errors -> the datastore client), but the dagster logger is only
    ...
    def __getattr__(self, name: str) -> Any:
        if _LazyDagsterLogger._logger is None:
            ...
            _LazyDagsterLogger._logger = dagster.get_dagster_logger("datastore")
...
def ON_CLUSTER_CLAUSE(on_cluster=True):
    # The test Datastore is a single node: ON CLUSTER only adds distributed-DDL keeper
    ...
    if on_cluster and not TEST:
        return f"ON CLUSTER '{DATASTORE_CLUSTER}'"
    return ""


# Smoke-test only: when migrating against the multinode docker-compose stack
# from the host, every docker hostname (`datastore-aux`, …) is mapped to
...
#
# Canonical source: `docker-compose.multinode-datastore.yml` (per-service
# `ports:` blocks). Keep this map in sync when adding or renumbering nodes.
_MULTINODE_HOST_PORT_OVERRIDES: dict[str, tuple[str, int]] = {
    "datastore-data": ("localhost", 9000),
    "datastore-ai-events": ("localhost", 9100),
    "datastore-aux": ("localhost", 9200),
    "datastore-ops": ("localhost", 9300),
    "datastore-sessions": ("localhost", 9400),
    "datastore-logs": ("localhost", 9500),
}
...
class DatastoreCluster:
    def __init__(
        ...
        migrations_cluster = cluster or settings.DATASTORE_CLUSTER
        ...
        # insights_migrations may not include all DATA nodes — discover them from
        # the main insights cluster which has the complete shard topology
    ...
    def __hosts_by_roles(
        ...
        # (e.g. in local dev where satellite clusters point to the same Datastore instance)
...
def get_cluster(
    ...
    extra_hosts = []
    for host_config in map(copy, DATASTORE_PER_TEAM_SETTINGS.values()):
...
# not present in datastore_driver.errors.ErrorCodes; see Datastore src/Common/ErrorCodes.cpp
...
class MutationWaiter:
    ...
    def is_done(self, client: Client) -> bool:
        ...
        rows = client.execute(
            ...
            GROUP BY ALL
            """,
            {"database": settings.DATASTORE_DATABASE, "table": self.table, "mutation_ids": list(self.mutation_ids)},
        )
...
class MutationRunner(abc.ABC):
    ...
    def find_existing_mutations(self, client: Client, commands: Set[str] | None = None) -> Mapping[str, str]:
        ...
        # Datastore normalizes bare references against the connection database when
        # storing the mutation, and a bare-vs-qualified mismatch defeats the join.
        alter_prefix = f"ALTER TABLE {settings.DATASTORE_DATABASE}.{self.table} "
        ...
        mutations = client.execute(
            ...
            """,
            {
                f"__database": settings.DATASTORE_DATABASE,
    ...
    def run_on_shards(self, cluster: DatastoreCluster, shards: Iterable[int] | None = None) -> None:
        """
...
class AlterTableMutationRunner(MutationRunner):
    ...
    def get_statement(self, commands: Set[str]) -> str:
        return f"ALTER TABLE {settings.DATASTORE_DATABASE}.{self.table} " + ", ".join(commands)
...
class LightweightDeleteMutationRunner(MutationRunner):
    ...
    def get_statement(self, commands: Set[str]) -> str:
        ...
        return f"DELETE FROM {settings.DATASTORE_DATABASE}.{self.table}{partition_clause} WHERE {self.predicate}"
