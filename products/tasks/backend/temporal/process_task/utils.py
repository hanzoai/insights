...
from insights.models.user_integration import ReauthorizationRequired, UserGitHubIntegration, UserIntegration
from insights.temporal.oauth import TOKEN_EXPIRATION_SECONDS, InsightsMcpScopes, has_write_scopes
...
# model means "let Insights pick", so defaults can improve without rewriting
# stored loops. Mirrored by LOOP_DEFAULT_MODELS in insights-code's loops UI.
...
# (notably the Insights Desktop CLI) that still pass ``github_user_token`` on the run request.
...
def get_user_mcp_server_configs(
    ...
    *,
    ...
    The `x-insights-mcp-consumer` header is set on every config so the agent's
    identity propagates through the MCP Store proxy to whichever upstream MCP
    the user installed. The Insights MCP needs this to resolve single-exec mode
    (without it, calls to `exec` fail with "Tool exec not found"); non-Insights
    ...
    for installation in installations:
        headers = [
            ...
            {"name": "x-insights-mcp-consumer", "value": consumer},
        ]
        configs.append(
            McpServerConfig(
                type="http",
...
def build_imported_mcp_server_configs(
    ...
    Entries whose name collides with an already-resolved server (the Insights MCP
...
def get_relayed_mcp_server_names(task_run: TaskRun, existing_names: Iterable[str]) -> list[str]:
    ...
    Names whose entry collides with an already-resolved MCP config (the Insights MCP, an MCP Store
...
def _resolve_mcp_consumer(interaction_origin: str | None) -> str:
    """Map the task's interaction origin to the `x-insights-mcp-consumer` value.

    Slack-launched runs send `"slack"` and insights_ai (Max) runs send
    `"insights_ai"`; everything else (the Insights Desktop UI, API callers, missing
    ...
    if interaction_origin == "slack":
        return "slack"
    if interaction_origin == "insights_ai":
        return "insights_ai"
...
def get_sandbox_ph_mcp_configs(
    ...
    project_id: int,
    *,
    scopes: InsightsMcpScopes = "read_only",
    interaction_origin: str | None = None,
    task_id: str | None = None,
) -> list[McpServerConfig]:
    """Return Insights MCP server configurations for sandbox agents.

    `task_id` is baked into an `X-Insights-Task-Id` header so the MCP server (and through it the
    Insights API) can deterministically attribute the agent's writes to its task — the LLM never
    handles its own task id.

    Uses SANDBOX_MCP_URL if explicitly set, otherwise derives it from SITE_URL:
    - app.hanzo.ai / us.hanzo.ai → https://mcp.hanzo.ai/mcp
    - eu.hanzo.ai → https://mcp-eu.hanzo.ai/mcp
    - app.dev.insights.dev → https://mcp.dev.insights.dev/mcp
    ...
    headers = [
        ...
        {"name": "x-insights-project-id", "value": str(project_id)},
        {"name": "x-insights-mcp-version", "value": "2"},
        {"name": "x-insights-read-only", "value": str(read_only).lower()},
        {"name": "x-insights-mcp-consumer", "value": _resolve_mcp_consumer(interaction_origin)},
    ]
    if task_id:
        headers.append({"name": "X-Insights-Task-Id", "value": str(task_id)})
    return [McpServerConfig(type="http", name="insights", url=url, headers=headers)]
...
# able to write anywhere. Every permission here must be one the Insights GitHub App holds, or the
...
def _resolve_sandbox_github_token(
    ...
    *,
    ...
    1. Caller-supplied token cached at run-create time (backward compat for the
       Insights Desktop CLI — wins when present so self-managed tokens still work).
...
def get_git_identity_env_vars(task: Task, state: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    Bot-authored runs fall back to the Dockerfile defaults ("Insights Desktop" /
    code@hanzo.ai).
    """
    ...
    name = user.get_full_name() or user.first_name or "Insights User"
    email = user.email
