...
describe('bot-detection.template', () => {
    ...
    it.each([
        ['insights-python', 'python-httpx'],
        ['insights-node', 'axios/1.4.0'],
        ['insights-webhook', 'okhttp/4.10.0'],
        ['insights-ios', 'CFNetwork/1410.0.3'],
    ])('should skip Insights-curated bot UA list when $lib=%s (non-browser SDK)', async (lib, ua) => {
        mockGlobals = tester.createGlobals({
            event: {
                properties: {
                    $lib: lib,
    ...
    it('should skip Insights-curated bot IP list when $lib is a non-browser SDK', async () => {
        mockGlobals = tester.createGlobals({
            event: {
                properties: {
                    $lib: 'insights-python',
