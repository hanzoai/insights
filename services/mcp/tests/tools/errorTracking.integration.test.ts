...
describe('Error Tracking', { concurrent: false }, () => {
    ...
    // The issues list reads the denormalized Datastore table, which lags Postgres deletes
    ...
    async function waitForFirstLiveIssueDetails(
        ...
        while (true) {
            ...
            try {
                ...
                if (!(error instanceof InsightsApiError) || error.status !== 404 || Date.now() >= deadline) {
                    throw error
                }
            }
