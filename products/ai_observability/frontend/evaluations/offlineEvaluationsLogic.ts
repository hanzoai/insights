...
type InsightsQLPrimitive = string | number | boolean | null

type RawOfflineExperimentRow = [
    experiment_id: InsightsQLPrimitive,
    experiment_name: InsightsQLPrimitive,
    first_seen_at: InsightsQLPrimitive,
    last_seen_at: InsightsQLPrimitive,
    events_count: InsightsQLPrimitive,
    items_count: InsightsQLPrimitive,
    metric_pairs_count: InsightsQLPrimitive,
]

type RawOfflineExperimentMetricRow = [
    item_id: InsightsQLPrimitive,
    experiment_item_name: InsightsQLPrimitive,
    experiment_name: InsightsQLPrimitive,
    metric_name: InsightsQLPrimitive,
    metric_version: InsightsQLPrimitive,
    status: InsightsQLPrimitive,
    score: InsightsQLPrimitive,
    score_min: InsightsQLPrimitive,
    score_max: InsightsQLPrimitive,
    result_type: InsightsQLPrimitive,
    reasoning: InsightsQLPrimitive,
    trace_id: InsightsQLPrimitive,
    dataset_id: InsightsQLPrimitive,
    dataset_item_id: InsightsQLPrimitive,
    ai_input: InsightsQLPrimitive,
    ai_output: InsightsQLPrimitive,
    ai_expected: InsightsQLPrimitive,
    last_seen_at: InsightsQLPrimitive,
    target_id: InsightsQLPrimitive,
    target_type: InsightsQLPrimitive,
]
...
function asString(value: InsightsQLPrimitive): string | null {
...
function asNumber(value: InsightsQLPrimitive): number {
...
function asNumberOrNull(value: InsightsQLPrimitive): number | null {
    if (typeof value === 'number') {
                ...
                loadOfflineExperiments: async () => {
                    ...
                    try {
                        const response = await api.queryInsightsQL(
                            insightsql`
                                SELECT
                                    ${insightsql.raw(EXPERIMENT_ID_EXPRESSION)} as experiment_id,
                                    argMax(${insightsql.raw(EXPERIMENT_NAME_EXPRESSION)}, timestamp) as experiment_name,
                                    min(timestamp) as first_seen_at,
                                    max(timestamp) as last_seen_at,
                                    count() as events_count,
                                    uniqExactIf(
                                        ${insightsql.raw(EXPERIMENT_ITEM_ID_EXPRESSION)},
                                        ${insightsql.raw(EXPERIMENT_ITEM_ID_EXPRESSION)} IS NOT NULL
                                    ) as items_count,
                                    uniqExactIf(
                                        tuple(
                                            ${insightsql.raw(METRIC_NAME_EXPRESSION)},
                                            ${insightsql.raw(METRIC_VERSION_EXPRESSION)}
                                        ),
                                        ${insightsql.raw(METRIC_NAME_EXPRESSION)} IS NOT NULL
                                        AND ${insightsql.raw(METRIC_VERSION_EXPRESSION)} IS NOT NULL
                                    ) as metric_pairs_count
                                FROM events
                                WHERE
                                    event = '$ai_evaluation'
                                    AND ${insightsql.raw(EXPERIMENT_ID_EXPRESSION)} IS NOT NULL
                                    ${insightsql.raw(dateFromClause)}
                                    ${insightsql.raw(dateToClause)}
