...
export const integrationsLogic = kea<integrationsLogicType>([
            ...
            null as IntegrationType[] | null,
            {
                loadIntegrations: async () => {
                    ...
                    return res.results.map((integration) => {
                        return {
                            ...integration,
                            // TODO: Make the icons endpoint independent of script functions
            ...
            null as IntegrationType | null,
            {
                ...
                linkExistingGithubInstallation: async (installationId?: string) => {
                    try {
                        ...
                        toast.success('Linked the existing GitHub installation to this project.')
            ...
            null as IntegrationKind | null,
            {
                requestIntegrationAccess: async ({ kind }: { kind: IntegrationKind }) => {
                    try {
                        ...
                        toast.success('Request sent! Your project admins have been notified.')
                        return kind
                    } catch (e) {
                        toastApiError(e)
                        throw e
    ...
    listeners(({ actions, values }) => ({
        ...
        handleOauthCallback: async ({ kind, searchParams }) => {
            ...
            if (error) {
                toast.error(describeOAuthCallbackError(String(error)))
                router.actions.replace(replaceUrl)
                return
            }

            // Stripe marketplace installs redirect here without a Insights-minted state, so we
            // can't verify the callback against a CSRF cookie. Without that, an attacker could
            // capture their own Connect-OAuth callback URL and trick a logged-in Insights admin
            ...
            if (token !== getCookie('ph_oauth_state')) {
                toast.error('This connection attempt expired before it could finish. Please try connecting again.')
                router.actions.replace(replaceUrl)
                return
            }
    ...
    afterMount(({ actions }) => {
        ...
        if (params.has('github_setup_error')) {
            ...
            toast.error(describeGithubSetupError(reason, detail))
        } else if (params.has('github_install_pending')) {
            toast.info(GITHUB_INSTALL_PENDING_MESSAGE)
        }
    }),
