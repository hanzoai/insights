...
let rustAddon: typeof import('@hanzo/replay-anonymizer') | null = null
try {
    rustAddon = require('@hanzo/replay-anonymizer')
...
describe('ml-mirror-pipeline', () => {
    ...
    itAddon('anonymizes events before recording for an opted-in team', async () => {
        mockTeamService = {
            getTeamByOrg: jest.fn().mockResolvedValue(team(true)),
    ...
    it('drops sessions for a team that did not opt into AI training', async () => {
        mockTeamService = {
            getTeamByOrg: jest.fn().mockResolvedValue(team(false)),
            getRetentionPeriodByTeamId: jest.fn().mockResolvedValue(30),
        } as unknown as TeamService
