...
describe('buildActiveEnvironmentContextPrompt', () => {
    ...
    it('renders a single base URL line (scheme stripped, project segment appended) when a base URL is given', () => {
        const result = buildActiveEnvironmentContextPrompt(user, org, project, 'https://us.hanzo.ai')
        expect(result).toContain('Base URL: us.hanzo.ai — add /project/1 for project-scoped paths.')
        // Sits right after the project/org context line.
        const lines = (result ?? '').split('\n')
        expect(lines.indexOf('Base URL: us.hanzo.ai — add /project/1 for project-scoped paths.')).toBe(
            lines.indexOf(
                'You are currently in project "My App" (id: 1, token: token_1) within organization "Acme" (id: org_1).'
            ) + 1
        )
    ...
    it('renders the base URL without a project segment when no project is active', () => {
        const result = buildActiveEnvironmentContextPrompt(user, undefined, undefined, 'https://us.hanzo.ai')
        expect(result).toContain('Base URL: us.hanzo.ai.')
