...
def get_person_distinct_ids(team_id: int, distinct_id: str) -> list[str]:
    ...
    with personinsights_caller_tag("conversations/widget-person-distinct-ids"):
...
# Caches the Datastore-resolved $groups for a customer (see events._resolve_groups_from_analytics).
# Ticket conversations re-resolve groups on creation plus every customer message, so this bounds
# the Datastore fallback to one query per customer per TTL. Empty dict = negative cache
