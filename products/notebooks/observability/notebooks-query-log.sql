-- Notebooks Datastore observability — query_log_archive query pack.
--
...
-- everything below.
--
-- Run these through the internal Metabase against "Insights Datastore PROD <REGION>
-- Data Tier" (see the query-datastore-via-metabase skill):
--   insightscli metabase:login --region us
--   insightscli metabase:databases --region us
--   insightscli metabase:query --region us --database-id <id> < this-file
--
...
-- ---------------------------------------------------------------------------
-- 1. Headline: notebook query volume, latency, and Datastore cost per hour.
...
-- ---------------------------------------------------------------------------
-- 2. Rollout verification: which path, pool, and Datastore identity is actually
...
--    is the Datastore-side confirmation that they engaged.
--      user           — 'notebooks' once the dedicated CH user is provisioned, else the default
--      lc_workload    — 'offline' once DATASTORE_OFFLINE_CLUSTER_HOST is set on the worker fleet
...
-- ---------------------------------------------------------------------------
-- 3. Failures by Datastore exception code. The budget codes the materialize path
...
--    `insights_notebooks_frame_object_bytes` is the mode-independent equivalent, but
...
--    here with attempts > 1 is Datastore work spent on a query that was already
