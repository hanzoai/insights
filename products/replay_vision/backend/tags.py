...
# Mirrors `.strip("_")` for the Datastore expression below.
...
def datastore_slugify_sql(column_expr: str) -> str:
    """Datastore expression applying `slugify_tag`'s normalization to a string column, so the stored side of a
