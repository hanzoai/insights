...
from insights.api.cohort import COHORT_USED_IN_PAGE_SIZE, CohortFilters
...
from insights.models.person.util import get_person_by_id
from insights.models.property import BehavioralPropertyType
...
def _cohort_member_uuids(team_id: int, cohort: Cohort) -> set[str]:
    """Resolve a cohort's members to their person UUIDs via personinsights."""
...
def _cohort_member_distinct_ids(team_id: int, cohort: Cohort) -> set[str]:
    """Resolve a cohort's members to the union of their distinct IDs via personinsights."""
    member_ids = list_cohort_member_ids(team_id=team_id, cohort_id=cohort.pk)
...
class TestCohort(TestExportMixin, DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    @patch(
        "insights.tasks.calculate_cohort.calculate_cohort_ch.delay",
...
"""
    ...
    @patch(
        "insights.tasks.calculate_cohort.insert_cohort_from_filters.delay",
    ...
    @patch(
        "insights.tasks.calculate_cohort.calculate_cohort_from_list.delay",
...
"""
    ...
    @patch("insights.tasks.calculate_cohort.calculate_cohort_from_list.delay")
...
"""
    ...
    @patch("django.db.transaction.on_commit", side_effect=lambda func: func())
    @patch("insights.api.cohort.report_user_action")
    @patch("insights.tasks.calculate_cohort.chain")
    @patch("insights.tasks.calculate_cohort.calculate_cohort_ch.si")
    @patch("insights.tasks.calculate_cohort.calculate_cohort_ch.delay")
    ...
    @patch("insights.tasks.calculate_cohort.increment_version_and_enqueue_calculate_cohort")
...
class TestCohortUsedIn(DatastoreTestMixin, APIBaseTest):
...
class TestCalculateCohortCommand(APIBaseTest):
    def test_calculate_cohort_command_success(self):
        ...
        out = StringIO()
        with patch("insights.management.commands.calculate_cohort.calculate_cohort_ch") as mock_calculate_cohort:
    ...
    def test_calculate_cohort_command_error(self):
        ...
        out = StringIO()
        with patch(
            "insights.management.commands.calculate_cohort.calculate_cohort_ch",
...
"""
    ...
    @patch("insights.tasks.calculate_cohort.calculate_cohort_from_list.delay", side_effect=calculate_cohort_from_list)
    def test_static_cohort_csv_upload_email_lookup_uses_datastore(self, patch_calculate_cohort_from_list):
        """
        CSV upload with an email column always uses the Datastore pmat_email
    ...
    def test_insert_users_by_email_always_uses_datastore(self):
    ...
    def test_email_property_key_is_accepted_and_always_routes_to_datastore(self, _name, email_property_key):
    ...
    def test_datastore_email_lookup_failure_records_error_on_cohort(self):
