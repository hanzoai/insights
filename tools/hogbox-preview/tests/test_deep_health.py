"""Unit tests for the personinsights override parity + the authed deep-health gate.

Self-contained: no network, no live box. Two seams are covered:

  - ``write_override`` must mirror hogland's scripts/insights-preview-setup.sh —
    the PERSONFN_ADDR web env plus the personinsights-replica/router services. When
    those drifted, previews 500'd fleet-wide with "personinsights client not
...
try:
    ...
    from hogbox_preview.stack import InsightsPreviewStack
...
class _RecordingBackend:
    ...
    def exec(self, command, *, timeout: int = 120) -> ExecResult:
        ...
        if "docker logs" in command:
            return ExecResult(
                0, "Traceback (most recent call last):\n  RuntimeError: personinsights client not configured", ""
            )
        return ExecResult(0, "", "")


@unittest.skipUnless(HAVE_SDK, "insights-hogland SDK not installed")
class OverridePersonhogParityTest(unittest.TestCase):
    def _override(self, **kwargs) -> str:
        backend = _RecordingBackend()
        stack = InsightsPreviewStack(backend, **kwargs)
        stack.write_override()
        return backend.files[f"{stack.repo_dir}/{stack.OVERRIDE}"]

    def test_web_env_has_personinsights_addr(self):
        self.assertIn("PERSONFN_ADDR=personinsights-router:50052", self._override())

    def test_personinsights_services_defined(self):
        override = self._override()
        self.assertIn("personinsights-replica:", override)
        self.assertIn("personinsights-router:", override)
        # extends the shared base compose (HOBBY-style), not a standalone def.
        self.assertIn("file: docker-compose.base.yml", override)
        # replica points at the MAIN insights DB (persons live there single-node).
        self.assertIn("PRIMARY_DATABASE_URL: postgres://insights:insights@db:5432/insights", override)

    def test_image_tag_flows_into_personinsights_images(self):
        # The personinsights tag is the part of self.image after the last ':'.
        override = self._override(image="ghcr.io/insights/insights:abc123")
        self.assertIn("ghcr.io/insights/insights/personinsights-replica:abc123", override)
        self.assertIn("ghcr.io/insights/insights/personinsights-router:abc123", override)
...
class DeepHealthTest(unittest.TestCase):
    ...
    def test_raises_with_body_and_logs_on_500(self):
        ...
        failing = ExecResult(
            0,
            "STEP login 200\nSTEP api_login 200\nSTEP projects 500\nBODY_START\npersoninsights client not configured\nBODY_END\n",
            "",
        )
        ...
        self.assertIn("personinsights client not configured", msg)
        # ...and so is the web-log tail (the traceback that cost hours).
        self.assertIn("docker logs --tail 40 insights-web-1", msg)
    ...
    def test_no_seed_still_probes_the_preseeded_golden(self):
        ...
        backend = _RecordingBackend(probe_result=ExecResult(0, "DEEP_HEALTH_OK", ""))
        InsightsPreviewStack(backend, seed_demo_data=False).deep_health()
        self.assertTrue(any("DEEP_HEALTH_OK" in c for c in backend.execs))
    ...
    def test_no_seed_tolerates_failed_login_only(self):
        ...
        InsightsPreviewStack(backend, seed_demo_data=False).deep_health()  # no raise
    ...
    def test_seeded_run_fails_hard_on_login_failure(self):
        ...
        backend = _RecordingBackend(probe_result=login_fail)
        with self.assertRaises(RuntimeError):
            InsightsPreviewStack(backend, seed_demo_data=True).deep_health()

    def test_personinsights_services_started_with_deps(self):
        ...
        self.assertIn("personinsights-replica", InsightsPreviewStack.DEPS)
        self.assertIn("personinsights-router", InsightsPreviewStack.DEPS)
