...
import { Button, Dialog, Tabs } from '@hanzo/elements'
...
function outcome(item: SlowestQuery): { label: string; type: TagType } {
...
export function QueryPerformance(): JSX.Element {
    ...
    const precomputationColumns: TableColumns<PrecomputationTeam> = [
        {
            title: 'Team ID',
            dataIndex: 'team_id',
            width: 100,
                ...
                return (
                    <div className="flex items-center gap-1">
                        ...
                        {team.organization_arr != null && (
                            <Tag type="completion">ARR ${team.organization_arr.toLocaleString()}</Tag>
                        )}
                    </div>
                )
            },
    ...
    const slowestQueryColumns: TableColumns<SlowestQuery> = [
        {
            title: 'Result',
            width: 220,
            render: function Result(_, item): JSX.Element {
                const { label, type } = outcome(item)
                const tag = <Tag type={type}>{label}</Tag>
                if (!item.exception) {
                    return tag
                }
                ...
                return (
                    <div className="flex items-center gap-1">
                        ...
                        {item.organization_arr != null && (
                            <Tag type="completion">ARR ${item.organization_arr.toLocaleString()}</Tag>
                        )}
                    </div>
                )
            },
            ...
            render: function Metric(_, item): JSX.Element {
                ...
                return (
                    <div className="flex items-center gap-1 min-w-0">
                        ...
                        {item.experiment_metric_type && <Tag type="muted">{metricTypeLabel}</Tag>}
                    </div>
                )
            },
        },
            ...
            render: function Path(_, item) {
                ...
                const pathTag = (
                    ...
                    if (value === 'precomputed') {
                        return <Tag type="success">{label}: precomputed</Tag>
                    }
                    return (
                        <Tooltip title={reasonForDirect(item, table)}>
                            <Tag type="default">{label}: direct</Tag>
                        </Tooltip>
                    )
                }
    ...
    const subQueryColumns: TableColumns<SlowestQuery> = [
        {
            title: 'Build',
            width: 180,
            render: function SubQueryBuild(_, item) {
                return <Tag type="warning">build: {item.experiment_precompute_table || 'unknown'}</Tag>
            },
        },
        {
            title: 'Duration (ms)',
