...
from insights.schema import EventPropertyFilter, PropertyOperator, RecordingPropertyFilter, RecordingsQuery
...
def test_unscored_fallback_passes_filtered_thresholds():
    from insights.session_recordings.queries.session_recording_list_from_query import UNSCORED_SURFACING_SCORE
...
# Integration: actual Datastore query.


@freeze_time(_FROZEN_TIME)
class TestScannerCandidateQueryAgainstDatastore(DatastoreTestMixin):
    ...
    def test_filter_test_accounts_excludes_internal_users(self, team) -> None:
        ...
        create_person(team=team, distinct_ids=["internal"], properties={"email": "hi@hanzo.ai"})
