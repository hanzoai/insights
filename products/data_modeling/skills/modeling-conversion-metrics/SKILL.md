...
description: >
  ...
  Insights data-warehouse views (InsightsQL) or an external dbt project. Use when the user wants to model, define,
  ...
  beats a warehouse view. On Insights, model funnels in InsightsQL with windowFunnel; in dbt, stage the event
...
[`references/insights/`](references/insights/) and [`references/dbt/`](references/dbt/).
...
- **Saved funnel insight** (`insights:query-funnel`) — best for interactive analysis, native breakdowns, and
...
- **dbt** — when the team models in dbt or the events live outside Insights.
...
**Insights:** compute the funnel per unit with `windowFunnel(window)(timestamp, cond_1, …, cond_n)`, then
aggregate the max step reached into conversion rates. Recipes:
[`references/insights/funnel_conversion.sql`](references/insights/funnel_conversion.sql) and
[`conversion_by_breakdown.sql`](references/insights/conversion_by_breakdown.sql). Alias every column;
...
`modeling-warehouse-foundations` (mechanics), `query-funnel` / `querying-insights-data` (interactive funnels +
InsightsQL), `modeling-activation-metrics` (activation is a conversion into a retention-validated action),
