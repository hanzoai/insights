...
description: >
  ...
  editing the lazy API router (insights/api/rest_router.py or its __init__.py shim), deferring a
...
The guards live in `insights/test/test_startup_import_budget.py`. **When one fails, defer the import — don't remove an entry to dodge it.** Conversely, when you deliberately defer a significant heavy lib off setup, **add it to `FORBIDDEN_AT_SETUP`** (after confirming it's absent from a bare `django.setup()`). New imports nobody has named yet are caught by `test_no_new_heavy_imports_at_setup`: any package not in `insights/test/setup_import_baseline.txt` costing ≥100ms at setup fails the build — defer it; baseline only what every process genuinely needs at setup.
...
- New heavy dependency (vendor SDK, Temporal/AI/Datastore, pandas/pyarrow/scipy): import it function-locally on the path that uses it with `# noqa: PLC0415`, never at module scope.
- Schema types on a setup-path module: enums from `insights.schema_enums` (cheap); pydantic models from `insights.schema` only inside the method that uses them. No module-level `from insights.tasks...` on setup paths — `CeleryQueue` lives in `insights.celery_queues`.
...
- **Measuring the wrapper, not the work** — time inside the process; `importtime` + `tuna`, not pyinstrument; `grimp` for cycles and import-path enumeration.
