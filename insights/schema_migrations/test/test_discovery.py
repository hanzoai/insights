...
from insights.schema_migrations import LATEST_VERSIONS, MIGRATIONS, _discover_migrations
from insights.utils import to_json


def _discover_with_mocks(files: list[str], modules: list[MagicMock]) -> None:
    with (
        patch("insights.schema_migrations.os.listdir") as mock_listdir,
        patch("insights.schema_migrations.importlib.import_module") as mock_import,
    ):
...
def test_frontend_latest_versions_file_in_sync():
    ...
    assert committed == expected, (
        "frontend/src/queries/latest-versions.json is out of sync with insights/schema_migrations. "
        "Run `python bin/build-schema-latest-versions.py` (part of `pnpm run schema:build` / `insightscli build:schema`) "
