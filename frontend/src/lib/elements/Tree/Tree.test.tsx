...
import { Tree, TreeRef, TreeDataItem } from './Tree'
...
describe('Tree virtualization', () => {
    ...
    it('renders only the visible window while scrolling', async () => {
        ...
        const { container } = render(<Tree data={data} expandedItemIds={['root']} virtualized />)
    ...
    it('keeps ancestor rows mounted when focusing a deep descendant', async () => {
        ...
        const { container } = render(
            <Tree ref={treeRef} data={data} expandedItemIds={['root', 'nested']} virtualized />
        )
    ...
    it('scrolls an imperatively focused item to the top third after its folder expands', async () => {
        ...
        const { container, rerender } = render(<Tree ref={treeRef} data={data} expandedItemIds={[]} />)
        ...
        rerender(<Tree ref={treeRef} data={data} expandedItemIds={['root']} />)
    ...
    it('supports an overridden virtualized row height', async () => {
        ...
        const { container } = render(
            <Tree data={data} expandedItemIds={['root']} virtualized virtualizedRowHeight={40} />
        )
