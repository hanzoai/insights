...
Usage metrics are team-defined numeric measures that render on Customer Analytics profile pages — for example "weekly active users", "events in the last 7 days", or "revenue over the last 30 days". Each metric compiles a InsightsQL filter expression over the events table, optionally summing a numeric property.
...
`filters` | jsonb | NOT NULL | InsightsQL filter definition: `{"events": [...], "properties": [...]}`. Same shape as InsightsFunction filters
