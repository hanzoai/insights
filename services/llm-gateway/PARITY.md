...
✅ **Default to [`Insights/ai-gateway`](https://github.com/Insights/ai-gateway)** for new callers and features.
...
| Server-to-server Insights call | A `phs_` credential, team wallet billing, and informational event properties provide enough policy and attribution. Internal spend can use a Insights-owned team wallet. |
...
Existing Django callers should use `build_openai_client`, `build_async_openai_client`, or `build_async_anthropic_client` from [`insights/llm/gateway_client.py`](../../insights/llm/gateway_client.py). These builders keep a temporary Python fallback during rollout.

### ⛔ Stay on the Python gateway for now

| Use case                                                                                          | Blocking contract                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Insights Desktop, Insights Code, cloud agents, onboarding, Wizard, and similar first-party products | Trusted product identity, OAuth application allowlists, OAuth user attribution, server-minted credential requirements, per-product model policy, and product billing or unbilled policy are not available together in Go. An `ai_product` event property is not an authorization or billing boundary. |
...
- **Billing:** identify which team owns the Go credential and should pay, then confirm its wallet is funded for the expected usage. Internal workloads can debit a Insights-owned team wallet so spend stays attributable without charging a customer.
- **Attribution:** `X-Insights-Distinct-Id`, `X-Insights-Trace-Id`, and `X-Insights-Properties` provide enough event context. Go does not derive the event distinct ID from OpenAI `user`, Anthropic `metadata.user_id`, or the OAuth user. Caller-supplied properties are not trusted policy.
- **Provider behavior:** health-based routing or strict `X-Insights-Provider` pinning matches the caller's fallback requirements.
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Authentication               | `phs_` project secrets and `pha_` OAuth credentials with the literal `llm_gateway:read` scope; credential resolves team and revocation state.                      | `phx_` personal keys, wildcard OAuth scope handling, and product-specific API key or OAuth application rules.                                                          |
...
| Attribution                  | Distinct ID comes from `X-Insights-Distinct-Id` or falls back to team ID. Supports trace ID, custom JSON properties, team project token, provider, usage, and cost. | OAuth user identity, body-derived OpenAI or Anthropic end-user identity, gateway-owned `$ai_product` and `$ai_billable`, per-key properties, and feature flag headers. |
...
| Event metadata               | One `X-Insights-Properties` JSON object plus dedicated distinct ID, trace ID, and provider headers.                                                                 | `X-INSIGHTS-PROPERTY-*` and `X-INSIGHTS-FLAG-*` headers.                                                                                                                 |
...
- `Insights/insights` master at `75c2b2ff6845097a3eb6dc5214e3f4f660395fb8`
- `Insights/ai-gateway` main at `ea8230b6dbc4ebf6c4be83d359e561477a13b215`

## References

- Python gateway: [`services/llm-gateway`](./README.md)
- Shared clients: [`insights/llm/gateway_client.py`](../../insights/llm/gateway_client.py)
- Go gateway: [`Insights/ai-gateway`](https://github.com/Insights/ai-gateway), especially `docs/product.md`
