...
# InsightsQL name → DuckDB target name. Overlays POSTGRES_FUNCTION_RENAMES.
DUCKDB_FUNCTION_RENAMES: dict[str, str] = {
    # Datastore's ``any`` is "pick any row"; DuckDB has a native ``any_value`` aggregator.
    ...
    "toTypeName": "typeof",
    # DuckDB's ``strftime`` takes strftime-style format strings directly (same patterns InsightsQL uses)
