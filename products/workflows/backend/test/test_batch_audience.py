import pytest
from insights.test.base import BaseTest, DatastoreTestMixin, _create_person, flush_persons_and_events
...
def _uuid(index: int) -> str:
    # Only the last digit differs, so string ordering and Datastore UUID ordering agree.
    return f"01970000-0000-0000-0000-00000000000{index}"


class TestBatchAudience(DatastoreTestMixin, BaseTest):
    ...
    def test_use_flag_defaults_off_when_feature_enabled_raises(self):
        ...
        # hanzo_insights.feature_enabled() throw must fall back to the legacy
        # audience query, not 500 the preview endpoint or fail the resolver job.
        with patch(
            "products.workflows.backend.services.batch_audience.hanzo_insights.feature_enabled",
