...
jest.mock('insights-js', () => ({ __esModule: true, default: { capture: jest.fn() } }))
...
describe('SlotCoercer.buildMessage', () => {
    ...
    it('normalizes a bare {name, args} tool call', () => {
        const emit: EmitSpec = { toolCall: lit({ id: 'c2', name: 'search', args: { q: 'script' } }) }
        expect(coercer.buildMessage(emit, scope())?.tool_calls).toEqual([
            { type: 'function', id: 'c2', function: { name: 'search', arguments: { q: 'script' } } },
        ])
    })

    it('parses string tool-call arguments into an object', () => {
        const emit: EmitSpec = { toolCall: lit({ name: 'search', arguments: '{"q":"script"}' }) }
        expect(coercer.buildMessage(emit, scope())?.tool_calls?.[0].function.arguments).toEqual({ q: 'script' })
    })
