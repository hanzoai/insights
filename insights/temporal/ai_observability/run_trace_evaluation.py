...
the rest of the trace can arrive, then pulls the whole trace from Datastore and evaluates it
in one shot.
...
from insights.schema import DateRange, LLMTrace, QueryLogTags, TraceQuery
...
from insights.temporal.ai_observability.evaluation_errors import (
...
from insights.temporal.ai_observability.evaluation_types import EvaluationActivityResult
...
from insights.temporal.ai_observability.metrics import increment_emit_event_outcome, increment_errors
from insights.temporal.ai_observability.run_evaluation import (
    WorkflowResult,
...
def _fetch_trace(team: Team, trace_id: str, date_from: datetime, date_to: datetime) -> TraceFetchOutcome:
    """Fetch a single full trace from Datastore over an explicit window, with a cheap count
...
def _sample_recent_traces(
    ...
    query = ast.SelectQuery(
        ...
        select_from=ast.JoinExpr(table=ast.Field(chain=["insights", "ai_events"]), alias="ai_events"),
...
def _trace_io_preview(trace: LLMTrace) -> tuple[str, str]:
    ...
    preview. The Script body still sees the full trace globals; this is only for display."""
    input_preview = ""
    output_preview = ""
...
async def execute_trace_hog_eval_activity(inputs: ExecuteTraceEvaluationInputs) -> EvaluationActivityResult:
    ...
    if result["error"]:
        if result.get("unexpected"):
            # A genuine bug in our evaluation code (not the user's Script). Raise so the Temporal
            # interceptor reports it to error tracking and we get paged to investigate.
            raise ApplicationError(
                f"Script evaluation error: {result['error']}",
                non_retryable=True,
            )

        # The user's Script source itself errored — an expected outcome of running customer-authored
...
class RunTraceEvaluationWorkflow(InsightsWorkflow):
    @staticmethod
        ...
        if evaluation_type == "script":
            ...
            try:
                result = await temporalio.workflow.execute_activity(
                    ...
                    execute_inputs,
                    # > single-event judge timeout: the activity also fetches the trace from Datastore
