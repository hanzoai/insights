...
| `insights:cohorts-list`                      | Resolve a cohort name → id when scoping to a user segment.               |
| `insights:session-recording-playlist-create` | Save the resulting filter as a saved filter view (`type: 'filters'`).    |
...
`{insights_base_url}/replay/{id}` — never `/replay/home?sessionRecordingId={id}`. Note total matches so
