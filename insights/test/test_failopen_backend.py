...
from insights.db_backends.failopen.base import CircuitOpenError, DatabaseWrapper
from insights.db_circuit_breaker import BreakerDecision
...
class TestFailOpenBackend(SimpleTestCase):
    def setUp(self) -> None:
        ...
        breaker_patch = mock.patch("insights.db_backends.failopen.base.get_circuit_breaker", return_value=self.breaker)
        breaker_patch.start()
        self.addCleanup(breaker_patch.stop)

        self.super_patch = mock.patch("insights.db_backends.failopen.base.PostgresDatabaseWrapper.ensure_connection")
