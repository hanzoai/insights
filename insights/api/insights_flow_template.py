...
from insights.api.insights_flow import InsightsFlowMaskingSerializer, InsightsFlowVariableSerializer
from insights.api.log_entries import LogEntryMixin
from insights.api.routing import TeamAndOrgViewSetMixin
from insights.cdp.validation import InsightsFunctionFiltersSerializer
from insights.models import User
from insights.models.activity_logging.activity_log import Detail, log_activity
from insights.models.insights_flow.insights_flow_template import InsightsFlowTemplate
...
class PreventGlobalTemplateDatabaseOperations(BasePermission):
    ...
    def has_permission(self, request: Request, view) -> bool:
        ...
        if request.method == "POST":
            ...
            if scope == InsightsFlowTemplate.Scope.GLOBAL:
                return False

        return True

    def has_object_permission(self, request: Request, view, obj: InsightsFlowTemplate) -> bool:
        if request.method in SAFE_METHODS:
            return True

        # Block all modifications to global templates (stored in DB or files)
        # This includes updating team templates to global scope
        if obj.scope == InsightsFlowTemplate.Scope.GLOBAL or request.data.get("scope") == InsightsFlowTemplate.Scope.GLOBAL:
            return False

        return True


class InsightsFlowTemplateActionSerializer(serializers.Serializer):
    """
...
class InsightsFlowTemplateSerializer(serializers.ModelSerializer):
    ...
    actions = serializers.ListField(child=InsightsFlowTemplateActionSerializer(), required=True)
    ...
    def validate(self, data):
        instance = cast(Optional[InsightsFlowTemplate], self.instance)
        ...
        for action_data in actions:
            serializer = InsightsFlowTemplateActionSerializer(data=action_data, context=self.context)
            serializer.is_valid(raise_exception=True)
    ...
    def create(self, validated_data: dict, *args, **kwargs) -> InsightsFlowTemplate:
        ...
        if not validated_data.get("scope"):
            validated_data["scope"] = InsightsFlowTemplate.Scope.ONLY_TEAM
...
class InsightsFlowTemplateViewSet(TeamAndOrgViewSetMixin, LogEntryMixin, viewsets.ModelViewSet):
    ...
    def dangerously_get_queryset(self):
        ...
        qs = InsightsFlowTemplate.objects.filter(
            Q(team_id=self.team_id)
            | Q(scope=InsightsFlowTemplate.Scope.ORGANIZATION, team__organization_id=self.organization.id)
        ).exclude(scope=InsightsFlowTemplate.Scope.GLOBAL)
    ...
    def perform_destroy(self, instance: InsightsFlowTemplate):
...
class PublicInsightsFlowTemplateViewSet(
    ...
    queryset = InsightsFlowTemplate.objects.none()
