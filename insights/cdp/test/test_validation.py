...
class TestInsightsFunctionValidation(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_validate_transformation_inputs_allows_stl_and_runtime_functions(self):
        ...
        # are valid root identifiers because the Script VM falls back to STL/runtime lookups
    ...
    def test_insights_ticket_tags_schema_type_is_valid(self):
        inputs_schema = [
            {
                "key": "tags",
                "type": "insights_ticket_tags",
                "label": "Tags",
                "required": False,
            }
        ]
        validated = validate_inputs_schema(inputs_schema)
        assert validated[0]["type"] == "insights_ticket_tags"
    ...
    def test_insights_ticket_tags_compiles_per_element_bytecode(self, _name, value):
        # Regression guard for the InputsItemSerializer opt-in. Before insights_ticket_tags
        ...
        inputs_schema = [{"key": "tags", "type": "insights_ticket_tags", "required": False}]
        ...
        for entry in bytecode:
            assert isinstance(entry, list) and entry[:2] == ["_H", INSIGHTSQL_BYTECODE_VERSION], (
                "each element is itself a Script bytecode array"
            )
