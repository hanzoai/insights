...
caches the dotenv, so re-activate your shell or restart `insightscli` after editing
`.env`.
...
`insightscli start` runs this service automatically when the `ai_features` intent is
enabled, listed as `agent-proxy` on :8003 (watch mode). Enable the intent with
`insightscli dev:setup` if it is not already on.

```sh
insightscli start                  # Redis, Django (:8000), frontend (:8010), Temporal worker, agent-proxy (:8003)
```

To run only this service standalone (for example when `ai_features` is off):

```sh
insightscli start:agent-proxy      # this service on :8003 (watch mode); or: pnpm --filter @hanzo/agent-proxy dev
```
...
```sh
# From repo root
docker build -f services/agent-proxy/Dockerfile -t insights-agent-proxy .

# Or build the JS bundle only (no Docker)
pnpm --filter @hanzo/agent-proxy build
...
```sh
pnpm --filter @hanzo/agent-proxy test
```

## Type checking

```sh
pnpm --filter @hanzo/agent-proxy typecheck
```
