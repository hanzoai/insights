...
from insights.security.outbound_proxy import internal_httpx_async_client
...
from insights.temporal.session_replay.delete_recordings import object_storage as chunk_storage
...
async def load_recordings_with_person(input: RecordingsWithPersonInput) -> LoadRecordingsPage:
    ...
    logger.info("Querying Datastore", query_id=ch_query_id)
...
async def purge_deleted_metadata(input: PurgeDeletedMetadataInput) -> PurgeDeletedMetadataResult:
    """Purge metadata from Datastore for recordings that have been deleted.
