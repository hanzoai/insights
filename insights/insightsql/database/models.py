...
if TYPE_CHECKING:
    from insights.insightsql.ast import JoinExpr, LazyJoinType, SelectQuery
    from insights.insightsql.base import ConstantType
...
class DatabaseField(FieldOrTable):
    ...
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import UnknownType
...
class IntegerDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import IntegerType
...
class FloatDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import FloatType
...
class DecimalDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import DecimalType
...
class StringJSONDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import StringJSONType
...
class MapStringDatabaseField(StringJSONDatabaseField):
    """A physical Datastore `Map(String, String)` column presented like a JSON blob.
    ...
    JSONExtract — which Datastore rejects on a Map. See `datastore_property_resolution._substitute_value_read`.
    """
...
class StructDatabaseField(DatabaseField):
    ...
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import TupleType
...
class StringArrayDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import StringArrayType
...
class FloatArrayDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import ArrayType, FloatType
...
class DateDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import DateType
...
class DateTimeDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import DateTimeType
...
class BooleanDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import BooleanType
...
class UUIDDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from insights.insightsql.ast import UUIDType
...
class Table(FieldOrTable):
    ...
    top_level_settings: Optional[InsightsQLQuerySettings] = None
...
class TableNode(BaseModel):
    ...
    # opt-in nodes participate in the fallback, so Datastore/event tables stay exact-match.
    ...
    def _match_child(self, name: str) -> Optional["TableNode"]:
        ...
        # Datastore/event tables exact-match while letting Snowflake schemas/tables resolve
...
class LazyJoin(FieldOrTable):
    ...
    def resolve_table(self, context: "InsightsQLContext") -> Table:
    ...
    def resolve_join_to_add(
        self, join_to_add: "LazyJoinToAdd", context: "InsightsQLContext", node: "SelectQuery"
    ) -> "JoinExpr":
        from insights.insightsql.database.lazy_join_registry import get_lazy_join_resolver  # noqa: PLC0415 — circular import
...
class LazyTable(Table):
    """
    A table that is replaced with a subquery returned from `lazy_select(requested_fields: Dict[name, chain], modifiers: InsightsQLQueryModifiers, node: SelectQuery)`
    """
