...
from insightscli_commands.product import gh as gh_module
from insightscli_commands.product.checks import (
    CheckContext,
...
from insightscli_commands.product.isolation import (
...
class TestAbsenceChecks:
    ...
    def test_contract_check_forbidden_with_deferred_presentation_entries(
        ...
        import insightscli_commands.product.isolation as isolation_module
...
# error_tracking exposes its Datastore DDL to core's schema registry + frozen migrations.
[[interfaces]]
expose = [
    "backend\\.embedding.*",
...
def _make_ddl_repo(tmp_path: Path, *, migration_body: str = "", schema_body: str = "") -> Path:
    migrations = tmp_path / "insights" / "datastore" / "migrations"
    migrations.mkdir(parents=True)
    (migrations / "0001_x.py").write_text(migration_body)
    (tmp_path / "insights" / "datastore" / "schema.py").write_text(schema_body)
    return tmp_path
...
class TestPermanentInterfaceQualification:
    ...
    def test_unqualified_exposure_blocks_isolation_chain(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(isolation_module, "_datastore_ddl_imports", lambda _root: frozenset())
        ctx = _make_product(tmp_path, scripts=_WITH_SCRIPT, isolated=True)
        result = chain_check.run(ctx)
...
def test_presentation_bypass_entries(name: str, expected: int) -> None:
    from insightscli_commands.product.isolation import presentation_bypass_entries
...
class TestFacadeClassImports:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"queries.py": "from ..insightsql_queries.runner import Runner\n__all__ = ['Runner']\n"},
                {"insightsql_queries/runner.py": "class Runner:\n    pass\n"},
                set(),
            ),
            ...
            ({"queries.py": "from insights.models import Team\n__all__ = ['Team']\n"}, {}, set()),
...
class TestNarrowedTurboWiringSurface:
    @pytest.mark.parametrize(
        "inputs, expected",
        [
            (["backend/facade/**", "backend/insightsql_queries/**"], True),  # a garage dir counts as narrowing surface
            ...
            (["backend/insightsql_queries/**"], False),  # garage alone isn't a real contract surface
