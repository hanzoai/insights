...
from insights.insightsql.escape_sql import escape_snowflake_identifier
from insights.insightsql.printer.base import BasePrinter
from insights.insightsql.printer.postgres import PostgresPrinter
from insights.insightsql.printer.snowflake_functions import (
...
# InsightsQL type names accepted by a `::` cast, mapped to Snowflake types. Mirrors the cast
...
# Datastore/strftime format specifier → Snowflake TO_CHAR format element.
...
class SnowflakePrinter(PostgresPrinter):
    """Prints a InsightsQL AST as Snowflake SQL."""

    DIALECT_NAME: ClassVar[InsightsQLDialect] = "snowflake"
    ...
    def visit_tuple(self, node: ast.Tuple) -> str:
        # InsightsQL lowers `{...}` object literals to a tagged tuple
    ...
    def _visit_format_datetime(self, node: ast.Call) -> str:
        ...
        # quote the user escaped as `''` in InsightsQL) must be re-escaped as `''` — otherwise it closes
    ...
    def _translate_strftime_format(self, fmt: str) -> str:
        # Translate Datastore/strftime %-specifiers to Snowflake TO_CHAR elements.
