...
from insights.temporal.backfill_group_type_created_at.types import (
    ApplyBackfillInput,
...
async def plan_group_type_created_at_backfill(input: PlanBackfillInput) -> dict:
    ...
    Datastore, and proposes lowering created_at to that earliest timestamp wherever
    ...
    def fetch_mappings() -> dict:
        ...
        with persons_db_connection(writer=True) as conn, conn.cursor() as cursor:
            cursor.execute(
                "SELECT group_type, group_type_index, created_at FROM insights_grouptypemapping WHERE project_id = %s",
                [project_id],
            )
            mappings = [
...
def _build_backfill_plan(
    ...
    for mapping in mappings:
        ...
        if created_at is None:
            # No created_at means InsightsQL never masks this group — nothing to fix.
...
async def apply_group_type_created_at_backfill(input: ApplyBackfillInput) -> dict:
    ...
    The group type mapping table is personinsights-owned (managed=False) but its created_at
    has no personinsights RPC, so we write directly to the persons DB through the off-ORM
    connection util — then bust the group-types cache so InsightsQL reads the new value
        ...
        with persons_db_connection(writer=True, autocommit=True) as conn:
            for update in input.updates:
                ...
                with conn.cursor() as cursor:
                    cursor.execute(
                        "UPDATE insights_grouptypemapping SET created_at = %s "
