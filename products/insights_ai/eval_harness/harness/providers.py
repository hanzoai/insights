...
``ModalSandbox`` subclass pinned to the ``insights-sandbox-evals`` Modal app.
...
class DockerProviderStrategy(SandboxProviderStrategy):
    ...
    def start(self, stack: ExitStack) -> None:
        # Verify insights-sandbox-base is fresh (rebuilding when @hanzo/agent published a
...
class ModalProviderStrategy(SandboxProviderStrategy):
    ...
    against the ``insights-sandbox-evals`` app, so DEBUG-mode eval image builds
