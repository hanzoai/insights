...
class TestExecuteBridgeQuery(BaseTest):
    @patch("products.streamlit_apps.backend.logic.bridge.execute_insightsql_query")
    def test_returns_whitelisted_response(self, mock_execute):
        """Only the columns/results/types fields are surfaced — datastore SQL,
        insightsql AST, internal timings, and modifiers must NOT leak through."""
        # A fully-populated response: the internal fields (datastore SQL, insightsql AST)
        # are present on the object and must still not reach the caller.
        mock_execute.return_value = InsightsQLQueryResponse(
            results=[[1, "hello"]],
            columns=["id", "name"],
            types=[["Int64"], ["String"]],
            insightsql="SELECT ...",
    ...
    def test_passes_query_and_team(self, mock_execute):
        mock_execute.return_value = InsightsQLQueryResponse(results=[], columns=[])
        ...
        # user must reach InsightsQL execution for table-level RBAC to apply.
...
class TestStreamlitBridgeView(_StreamlitAppsFlagMixin, APIBaseTest):
    ...
    def test_successful_query_with_oauth(self, mock_execute):
        ...
        assert "datastore" not in data
        # End-to-end wiring guard: the token's minting user must reach InsightsQL
