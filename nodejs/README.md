# Insights Node.js services

Node.js services for Insights: ingestion pipeline, CDP, session recording, and more.
...
| ----------------------------- | --------------------------------------- | ---------------------------------- |
| Postgres (common)             | `test_insights`                          | `insights`                          |
| Postgres (persons)            | `test_persons`                          | `insights_persons`                  |
...
| Datastore                    | `insights_test`                          | `default`                          |
...
```bash
# From the repo root, with the dev stack's Postgres/Datastore/Kafka/Redis running:
pnpm --filter=@hanzo/nodejs setup:test
```

This runs Django's `setup_test_environment` (creates `test_insights` and the Datastore `insights_test` schema) plus the rust migrations for the persons, behavioral cohorts, and cyclotron test databases. Then:
