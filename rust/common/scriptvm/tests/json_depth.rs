//! The json->script deserialization recursion guard (`MAX_JSON_SERDE_DEPTH`) must clear any realistic
...
use scriptvm::{sync_execute, ExecutionContext, Program, MAX_JSON_SERDE_DEPTH};
...
fn return_deep_global(globals: Value) -> Result<Value, scriptvm::VmFailure> {
    let bc = vec![
        json!("_H"),
        json!(1),
        json!(OP_STRING),
