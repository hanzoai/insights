...
- If they only asked to find or rank users to interview (e.g. "who'd be a good fit to interview about the inbox?"), treat it as audience discovery: answer it with normal queries and **do not** create a topic. You may add a one-line offer afterwards, e.g. _"If you want, I can set these up as automated interviews run by Insights's AI voice agent — want me to do that?"_
- If intent is ambiguous, ask first, e.g. _"Quick check on what you're after: what kind of user research is this, and do you want Insights's AI voice agent to actually run the interviews (it calls people and works through your questions)? Or did you just want me to find the right users to talk to?"_
...
1. **Who to interview** — at least one of:
   ...
   - `interviewee_distinct_ids` — list of Insights distinct IDs
...
3. **If the cohort has more than 500 members**, stop and ask the user. Do not silently truncate, sample, or fall back to a different cohort — the user needs to choose. Surface:
   - The cohort name and count (e.g. "Insights Team has 28,563 members — over the 500 cap.")
   ...
   - Their options:
     - **Narrow the cohort** — describe the subset they actually want (e.g. "engineers only", "active in the last 30 days"). You can offer to write a more specific InsightsQL filter or create a new, smaller cohort via `cohorts-create`.
...
- ✅ "What made you decide to try Insights?"
- ❌ "Did you like Insights?"
...
- **Background the agent should know**: "the user just churned from the Scale plan; be empathetic", "this person tried Insights 6 months ago and bounced"
- **Persona**: "you are Sam, a Insights product researcher"
...
```json
{
  ...
  "questions": [
    "What were you hoping Insights would help with?",
...
- **Users matched but few have emails.** Insights stores whatever the SDK captures. If only a handful of matching users have email addresses on their person profile, surface the count and ask: take the smaller sample, fall back to `interviewee_distinct_ids` (the agent can still reach them via in-app delivery), or skip the behavioral query and let the user paste emails directly.
