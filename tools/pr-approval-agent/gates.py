...
# The resolver lives in the insights-owners package, a sibling under tools/. It is
...
def _owner_to_team_handle(owner: str) -> str | None:
    ...
    Team slugs become `@Insights/<slug>` (the shape the reviewer prompt expects);
...
def _build_insightscli_resolver(repo_root: Path, source: OwnershipSource) -> _HogliResolver:
    ...
    try:
        from insights_owners.resolver import (
...
OWNERSHIP_FORMATS: dict[str, OwnershipFormat] = {
    "insightscli-resolver": OwnershipFormat("path", _build_insightscli_resolver),
}
...
def detect_ownership(files: list[str], resolvers: list[OwnershipResolver]) -> dict:
    ...
    for f in files:
        ...
        teams = {o for o in owners if o.startswith("@Insights/")}
        if owners:
            owned_files += 1
...
# insights/personinsights_client/proto/generated/ falls through to re-review,
...
# api_key, etc. without touching the auth *system* or Insights's own billing.
