...
from insights.models.event.sql import BULK_INSERT_EVENT_SQL, EVENTS_JSON_DATA_TABLE, INSERT_EVENT_SQL
...
DATASTORE_JSON_MIN_INT = -(2**63)
DATASTORE_JSON_MAX_UINT = 2**64 - 1


def _normalize_datastore_json_value(value: Any) -> Any:
    if isinstance(value, bool) or value is None:
        return value
    if isinstance(value, int):
        if DATASTORE_JSON_MIN_INT <= value <= DATASTORE_JSON_MAX_UINT:
            return value
        return float(value)
    if isinstance(value, list):
        return [_normalize_datastore_json_value(item) for item in value]
    if isinstance(value, tuple):
        return [_normalize_datastore_json_value(item) for item in value]
    if isinstance(value, dict):
        return {key: _normalize_datastore_json_value(nested_value) for key, nested_value in value.items()}
    return value


def _json_dumps_for_datastore(value: dict[str, Any] | None) -> str:
    return json.dumps(_normalize_datastore_json_value(value or {}))
...
def create_event(
    ...
    p = DatastoreProducer()
    if TEST and settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA:
        json_data = {
            **data,
            "properties": _json_dumps_for_datastore(properties),
            "person_properties": _json_dumps_for_datastore(person_properties),
        }
...
def _resolve_person_for_bulk_event(team_id: int, distinct_id: str) -> Optional[Person]:
    """Resolve the person for a test event by distinct_id via personinsights."""
    try:
    ...
    for index, event in enumerate(events):
        ...
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA:
            json_event = {
                **event,
                "properties": _json_dumps_for_datastore(properties),
                "person_properties": _json_dumps_for_datastore(person_properties_for_insert),
            }
            json_params = {
                **json_params,
