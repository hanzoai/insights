...
class TestRerunGoogleAdsFailedInvocations(BaseTest):
    def setUp(self):
        super().setUp()
        # `InsightsFunction.save()` fires a reload signal that would try to talk to the CDP
        # API; silence it for the whole class so fixtures don't require the network.
        patcher = patch("products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers")
        patcher.start()
        self.addCleanup(patcher.stop)

    def _make_fn(self, *, template_id: str, enabled: bool, deleted: bool, name: str = "gads") -> InsightsFunction:
    ...
    def test_only_targets_active_non_deleted_google_ads_destinations(self):
        ...
        self.assertEqual(kwargs["function_kind"], "insights_function")
