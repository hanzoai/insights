...
* **Error-tracking** (lists / evidence-mid-investigation) — the demo seeds
  Postgres issue rows + raw exception events but the sandbox's Datastore
...
To run a single eval:
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_renders_entity_ui"
"""
