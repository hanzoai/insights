...
from insights.api.embedding_worker import async_generate_embedding, emit_embedding_request
...
Reject the group only when the signals need SEPARATE, unrelated fixes that no single engineer would take on together, such as "Fix various Insights AI issues" or "Address feature flag and authentication concerns".
...
async def assign_and_emit_signal_activity(input: AssignAndEmitSignalInput) -> AssignAndEmitSignalOutput:
    ...
    def do_assign_and_emit(suppress_promotion: bool) -> AssignAndEmitDbResult:
        with transaction.atomic():
            ...
            if isinstance(match_result, ExistingReportMatch):
                ...
                # promotion, but we still emit the signal to Datastore, marked as deleted
                # in metadata.
                #
                ...
                report = SignalReport.objects.create(
                    ...
                    # A report born from a Insights-system signal (e.g. health checks) must never
    ...
    try:
        ...
        # If we matched a deleted report, soft-delete all its stale signals in Datastore.
    ...
    if emitted_signals:
        await workflow.execute_activity(
            ...
            WaitForDatastoreInput(
                team_id=team_id,
                signals=[
                    WaitForDatastoreSignal(signal_id=sid, timestamp=result.timestamp)
                ...
                # semantic search — don't spend Datastore queries on the happy path.
                mode=WaitForDatastoreMode.OPTIMISTIC,
            ),
...
class TeamSignalGroupingWorkflow:
    ...
    async def run(self, input: TeamSignalGroupingInput) -> None:
        with hanzo_insights.new_context(capture_exceptions=False):
            hanzo_insights.tag("team_id", input.team_id)
            hanzo_insights.tag("product", "signals")
