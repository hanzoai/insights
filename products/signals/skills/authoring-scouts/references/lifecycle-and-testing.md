...
All three via `insights:scout-config-update` (get the `id` from `-config-list`), or set at creation time in the nested `config` object passed to `insights:scout-create-prepare`.
