...
Run:
    ...
    # --query-log-database-id 142 is Datastore PROD US - OFFLINE
...
from .backends.local import LocalDatastoreBackend
...
def _build_backend(args: argparse.Namespace) -> ExecutionBackend:
    ...
    if args.target == "local":
        return LocalDatastoreBackend(team_id=args.local_team_id)
...
def _ensure_django_setup() -> None:
    ...
    Insights's app-init that would otherwise connect to Postgres / Redis:

    - ``OPT_OUT_CAPTURE=1`` disables ``hanzo_insights`` self-capture
      (skips ``initialize_self_capture_api_token`` Postgres call in
      ``InsightsConfig.ready``).
...
def _spawn_one_sandbox(
    *,
    ...
    config = SandboxConfig(
        ...
        # Datastore source snapshot at /opt/datastore — the agent uses the
        ...
        # Most of pi-coding-agent's work is LLM round-trips and Datastore
        ...
        json.dumps(
            {
                ...
                "datastore_query": query.datastore_query,
                "insightsql_query": query.insightsql_query,
            },
            indent=2,
        )
        + "\n"
    ...
    try:
        ...
        env_blob = (
            f"CAMPAIGN_SQL={shlex.quote(query.datastore_query)}\n"
        ...
        try:
            harvested = harvest_artifacts(
                sandbox,
                original_sql=query.datastore_query,
                query_id=query.query_id,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Prefer the OFFLINE cluster — it's not under user load. (US: 142, Datastore PROD US - OFFLINE.)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "When --target=local, the team_id the dev Datastore contains data for. "
...
def _print_only(args: argparse.Namespace) -> int:
    ...
    if not args.test_query:
        ...
        if shutil.which("insightscli") is None:
            raise SystemExit("`insightscli` not on PATH; run from a flox environment with the workspace loaded")
    ...
    for i, q in enumerate(queries, start=1):
        ...
        preview = q.datastore_query.replace("\n", " ")[:200]
        print(f"        sql_preview: {preview}{'…' if len(q.datastore_query) > 200 else ''}")  # noqa: T201
        print()  # noqa: T201
    return 0
...
def _synthetic_test_query() -> SlowQuery:
    ...
    return SlowQuery(
        ...
        team_id=0,
        datastore_query=_TEST_QUERY_SQL,
        insightsql_query="",
...
def _fetch_filtered_queries(args: argparse.Namespace) -> list[SlowQuery]:
    ...
    if args.test_query:
        ...
        # Datastore SQL with no team_id predicate, so the column /
...
def _resolve_repo_root() -> Path:
    ...
    raise SystemExit(
        "could not locate the insights repo root (looked for manage.py + products/). "
        "Run the coordinator from inside the insights checkout."
    )
