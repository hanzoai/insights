...
const TROUBLESHOOTING_URL = 'https://hanzo.ai/docs/session-replay/troubleshooting'
...
export function diagnoseReplayCapture(eventProperties: Record<string, any> | null | undefined): ReplayCaptureDiagnosis {
    ...
    if (hasRecording === true) {
        return {
            ...
            reasons: [
                'Insights has a stored recording linked to this event\u2019s session (`$has_recording = true`).',
    ...
    if (recordingStatus === 'active' && flushedSize !== null && flushedSize > 0) {
        return {
            ...
            reasons: [
                'The SDK reported `$recording_status = active` and flushed recording data to Insights.',
