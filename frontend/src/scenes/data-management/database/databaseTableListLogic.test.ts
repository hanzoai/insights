...
describe('databaseTableListLogic', () => {
    ...
    it.each([
        ...
        expect(insights.captureException).not.toHaveBeenCalled()
    })
    ...
    it('resetConnectionScope drops the connection and reloads the project catalog', async () => {
        ...
        ;(performQuery as jest.Mock).mockResolvedValue({
            tables: { events: { name: 'events', type: 'insights' } },
            joins: [],
        })
    ...
    it('does not let a stale schema response overwrite the selected connection schema', async () => {
        let resolveInsightsQuery:
            | ((value: { tables: Record<string, { name: string; type: 'insights' }>; joins: never[] }) => void)
            | undefined
        ...
        const insightsRequest = logic.asyncActions.loadDatabase()
        ...
        resolveInsightsQuery?.({
            tables: {
                events: { name: 'events', type: 'insights' },
            },
            joins: [],
        })
        await insightsRequest
