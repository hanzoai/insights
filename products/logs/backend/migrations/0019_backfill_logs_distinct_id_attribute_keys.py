...
def backfill_distinct_id_attribute_keys(apps, schema_editor):
    ...
    # The ADD COLUMN default already filled every row with {insightsDistinctId}; only
    # rows with a customized single key need their value carried into the array.
    configs = TeamLogsConfig.objects.exclude(logs_distinct_id_attribute_key="insightsDistinctId")
    batch = []
