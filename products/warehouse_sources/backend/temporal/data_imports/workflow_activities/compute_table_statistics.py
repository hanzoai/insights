...
def _column_type(definition: Any) -> str:
    """Source-agnostic Datastore type string from a `DataWarehouseTable.columns` entry.

    Handles both the dict shape (`{"datastore": ...}`) and the legacy plain-string shape.
    """
    if isinstance(definition, dict):
        return definition.get("datastore") or definition.get("insightsql") or ""
...
class ComputeTableStatisticsWorkflow(InsightsWorkflow):
    @staticmethod
