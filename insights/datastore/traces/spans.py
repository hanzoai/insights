...
KAFKA_TOPIC = "datastore_traces"
KAFKA_GROUP = "datastore-traces-avro"
...
def _trace_attributes_mv(view_name: str, dest_table: str, attribute_type: str, inner_select: str):
    db = settings.DATASTORE_LOGS_CLUSTER_DATABASE
    return f"""
...
def KAFKA_TRACE_SPANS_AVRO_TABLE_SQL():
    return f"""
CREATE TABLE IF NOT EXISTS {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{KAFKA_TABLE_NAME}
(
    `uuid` String,
    `trace_id` String,
    `span_id` String,
...
def TRACE_SPANS_KAFKA_METRICS_TABLE_SQL():
    return f"""
CREATE TABLE IF NOT EXISTS {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{KAFKA_METRICS_TABLE_NAME}
(
    `_partition` UInt32,
    `_topic` String,
