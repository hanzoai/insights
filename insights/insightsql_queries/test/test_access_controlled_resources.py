...
from insights.insightsql_queries.access_controlled_resources import (
...
class TestQueriedAccessControlledResources(BaseTest):
    ...
    def test_insightsql_query_system_scopes(self, _name, sql, expected):
        assert queried_access_controlled_resources(InsightsQLQuery(query=sql), self.team) == expected

    def test_unparseable_insightsql_fails_closed(self):
        assert queried_access_controlled_resources(InsightsQLQuery(query="select from from"), self.team) is None
    ...
    def test_connection_query_partitions_on_source_and_warehouse_scopes(self):
        ...
        query = InsightsQLQuery(query="select * from public.users", connectionId="00000000-0000-0000-0000-000000000001")
    ...
    def test_warehouse_table_scope(self):
        ...
        result = queried_access_controlled_resources(InsightsQLQuery(query="select * from my_warehouse_table"), self.team)
    ...
    def test_external_warehouse_table_matched_by_raw_name(self):
        ...
        result = queried_access_controlled_resources(InsightsQLQuery(query="select * from stripe_customers"), self.team)
        assert result == {"external_data_source", "warehouse_table"}

    def test_warehouse_view_scope(self):
        DataWarehouseSavedQuery.objects.create(
            team=self.team, name="my_warehouse_view", query={"kind": "InsightsQLQuery", "query": "select 1 as a"}
        )
        result = queried_access_controlled_resources(InsightsQLQuery(query="select * from my_warehouse_view"), self.team)
    ...
    def test_warehouse_and_system_scopes_combined(self):
        ...
        result = queried_access_controlled_resources(
            InsightsQLQuery(query="select 1 from my_warehouse_table, system.notebooks"), self.team
        )
    ...
    def test_catalog_fetch_loads_only_name_fields(self):
        ...
        query = InsightsQLQuery(query="select * from stripe_customers")
    ...
    def test_warehouse_table_of_another_team_not_matched(self):
        ...
        result = queried_access_controlled_resources(InsightsQLQuery(query="select * from other_team_table"), self.team)
        assert result == set()
