...
mod tests {
    ...
    fn resolved_frames_under_application_id_promote_to_in_app() {
        ...
        let raw = raw_frame("a1.d", false, Some("com.insights.android.sample@3.0+3"));
        ...
        assert!(frames.iter().all(|f| f.in_app
            && f.module
                .as_deref()
                .unwrap()
                .starts_with("com.insights.android.sample.")));
    }

    #[test]
    fn resolved_in_app_classification() {
        let map_id = Some("com.insights.android.sample@3.0+3");
        let cases = [
            // (raw in_app, map_id, resolved class, expected)
            (
                false,
                map_id,
                "com.insights.android.sample.ErrorTrackingActivityKt",
                true,
            ),
            (
                false,
            ...
            (false, map_id, "com.insights.android.sampleother.Foo", false),
    ...
    fn unresolved_frames_keep_raw_in_app() {
        for raw_in_app in [false, true] {
            let raw = raw_frame("a1.d", raw_in_app, Some("com.insights.android.sample@3.0+3"));
