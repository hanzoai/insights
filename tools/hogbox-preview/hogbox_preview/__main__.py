...
def build_stack(backend: HoglandBackend, args: argparse.Namespace) -> InsightsPreviewStack:
    return InsightsPreviewStack(
        backend,
...
def cmd_health(args: argparse.Namespace) -> int:
    ...
    # too so this subcommand catches an unusable app (the personinsights-drift 500s
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--snapshot", default="alias:insights-preview-golden", help="golden snapshot id/alias")
    p.add_argument("--web-port", type=int, default=8000, help="in-guest port Insights serves on")
    p.add_argument("--image", default=InsightsPreviewStack.IMAGE, help="published insights image")
    p.add_argument(
        "--name",
        default="insights-preview",
    ...
    # alias insights-preview-golden) at 8 vCPU / 16 GB / 100 GB.
    ...
    up = sub.add_parser("up", help="provision + bring Insights up (one-shot)")
    up.add_argument("--box-id", default=None, help="reuse an existing box instead of restoring a new one")
    up.add_argument("--branch", default=None, help="insights branch to check out in the box")
    up.add_argument("--no-seed", action="store_true", help="skip demo-data seeding (generate_demo_data)")
    up.add_argument(
        "--reset-db",
        action="store_true",
        help="wipe pg+datastore first, so they migrate fresh & coherent with --image (use when baking a golden)",
    )
    ...
    mg = sub.add_parser("migrate", help="run postgres + datastore migrations on an existing box")
