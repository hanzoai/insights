...
-- Full CREATE schema for the prod-us/logs node. Apply to a fresh Datastore to build it.

CREATE TABLE insights.kafka_logs_avro (
  uuid String,
  trace_id String,
  span_id String,
  trace_flags Int32,
...
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'warpstream_logs', kafka_format = 'kafka_format = \'Avro\'', kafka_group_name = 'kafka_group_name = \'datastore-logs-avro-new\'', kafka_num_consumers = 32, kafka_poll_max_batch_size = 1000, kafka_poll_timeout_ms = 3000, kafka_skip_broken_messages = 100, kafka_thread_per_consumer = 1, kafka_topic_list = 'kafka_topic_list = \'datastore_logs\'';
CREATE TABLE insights.kafka_metrics_avro (
  uuid String,
  trace_id String,
  span_id String,
...
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'warpstream_metrics', kafka_format = 'kafka_format = \'Avro\'', kafka_group_name = 'kafka_group_name = \'datastore-metrics-avro-new\'', kafka_num_consumers = 8, kafka_poll_max_batch_size = 1000, kafka_poll_timeout_ms = 3000, kafka_skip_broken_messages = 100, kafka_thread_per_consumer = 1, kafka_topic_list = 'kafka_topic_list = \'datastore_metrics\'';
CREATE TABLE insights.kafka_trace_spans_avro (
  uuid String,
  trace_id String,
  span_id String,
  ...
  status_code Int32
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'warpstream_traces', kafka_format = 'kafka_format = \'Avro\'', kafka_group_name = 'kafka_group_name = \'datastore-traces-avro\'', kafka_num_consumers = 8, kafka_poll_max_batch_size = 1000, kafka_poll_timeout_ms = 3000, kafka_skip_broken_messages = 100, kafka_thread_per_consumer = 1, kafka_topic_list = 'kafka_topic_list = \'datastore_traces\'';
CREATE TABLE insights.log_attributes2 (
  team_id Int32,
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/logs/{shard}/log_attributes34', '{replica}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(time_bucket) TTL time_bucket + toIntervalDay(15) SETTINGS deduplicate_merge_projection_mode = 'drop', index_granularity = 8192;
CREATE TABLE insights.log_attributes3 (
  team_id Int32,
  time_bucket DateTime64(0),
  service_name LowCardinality(String),
  resource_fingerprint UInt64 DEFAULT 0,
  ...
  INDEX idx_attribute_key_n3 attribute_key TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 1,
  INDEX idx_attribute_value_n3 attribute_value TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 1
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/noshard/insights.log_attributes3', '{replica}-{shard}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value, severity_text) PARTITION BY toDate(original_expiry_time_bucket) TTL original_expiry_time_bucket SETTINGS deduplicate_merge_projection_mode = 'drop', index_granularity = 8192;
CREATE TABLE insights.log_attributes_distributed (
  team_id Int32,
  time_bucket DateTime64(0),
  service_name LowCardinality(String),
  resource_fingerprint UInt64 DEFAULT 0,
  ...
  original_expiry_time_bucket DateTime DEFAULT now(),
  severity_text LowCardinality(String)
) ENGINE = Distributed('logs', 'insights', 'log_attributes3');
CREATE TABLE insights.logs34 (
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/logs/{shard}/logs_billing_metrics', '{replica}') ORDER BY (team_id, time_bucket, service_name) PARTITION BY toYYYYMM(time_bucket) SETTINGS deduplicate_merge_projection_mode = 'rebuild', index_granularity = 8192;
CREATE TABLE insights.logs_billing_metrics_distributed (
  team_id Int32,
  time_bucket DateTime64(0),
  service_name LowCardinality(String),
  bytes_uncompressed SimpleAggregateFunction(sum, UInt64),
  bytes_compressed SimpleAggregateFunction(sum, UInt64),
  record_count SimpleAggregateFunction(sum, UInt64)
) ENGINE = Distributed('logs', 'insights', 'logs_billing_metrics');
CREATE TABLE insights.logs_distributed (
  time_bucket DateTime MATERIALIZED toStartOfDay(timestamp),
  original_expiry_timestamp DateTime64(6),
  uuid String,
  team_id Int32,
  ...
  _bytes_compressed UInt64,
  _record_count UInt64
) ENGINE = Distributed('logs', 'insights', 'logs34');
CREATE TABLE insights.logs_kafka_metrics (
  _partition UInt32,
  _topic String,
...
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/logs/{shard}/logs_kafka_metrics', '{replica}') ORDER BY (_topic, _partition) SETTINGS deduplicate_merge_projection_mode = 'rebuild', index_granularity = 8192;
CREATE TABLE insights.logs_kafka_metrics_distributed (
  _partition UInt32,
  _topic String,
  max_offset SimpleAggregateFunction(max, UInt64),
  max_observed_timestamp SimpleAggregateFunction(max, DateTime64(9)),
  ...
  max_created_at SimpleAggregateFunction(max, DateTime64(9)),
  max_lag SimpleAggregateFunction(max, UInt64)
) ENGINE = Distributed('logs', 'insights', 'logs_kafka_metrics');
CREATE TABLE insights.metric_attributes (
  team_id Int32,
  time_bucket DateTime64(0),
  service_name LowCardinality(String),
  resource_fingerprint UInt64 DEFAULT 0,
  ...
  INDEX idx_attribute_key_n3 attribute_key TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 1,
  INDEX idx_attribute_value_n3 attribute_value TYPE ngrambf_v1(3, 32768, 3, 0) GRANULARITY 1
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/logs/{shard}/insights.metric_attributes', '{replica}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(time_bucket) SETTINGS deduplicate_merge_projection_mode = 'drop', index_granularity = 8192;
CREATE TABLE insights.metric_samples1 (
  team_id Int32,
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/noshard/insights.metric_samples1', '{replica}-{shard}') ORDER BY (team_id, metric_name, series_fingerprint, timestamp) PARTITION BY toDate(timestamp) TTL toDateTime(timestamp) + toIntervalDay(30) SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
CREATE TABLE insights.metric_series1 (
  team_id Int32,
  metric_name LowCardinality(String),
  series_fingerprint UInt64 CODEC(DoubleDelta),
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/noshard/insights.metric_series1', '{replica}-{shard}', last_seen) ORDER BY (team_id, metric_name, series_fingerprint) TTL toDateTime(last_seen) + toIntervalDay(90) SETTINGS index_granularity = 8192;
CREATE TABLE insights.metrics1 (
  time_bucket DateTime MATERIALIZED toStartOfDay(timestamp),
  uuid String,
  team_id Int32,
  trace_id String,
  ...
  INDEX idx_attributes_str_values mapValues(attributes_map_str) TYPE bloom_filter(0.001) GRANULARITY 1,
  INDEX idx_observed_minmax observed_timestamp TYPE minmax GRANULARITY 1
) ENGINE = ReplicatedMergeTree('/datastore/tables/logs/{shard}/insights.metrics1', '{replica}') ORDER BY (team_id, time_bucket, service_name, metric_name, resource_fingerprint, timestamp) PARTITION BY toDate(timestamp) SETTINGS index_granularity = 8192, index_granularity_bytes = 104857600, ttl_only_drop_parts = 1;
CREATE TABLE insights.metrics_kafka_metrics (
  _partition UInt32,
  _topic String,
  max_offset SimpleAggregateFunction(max, UInt64),
  max_observed_timestamp SimpleAggregateFunction(max, DateTime64(9)),
  ...
  max_created_at SimpleAggregateFunction(max, DateTime64(9)),
  max_lag SimpleAggregateFunction(max, UInt64)
) ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/logs/{shard}/insights.metrics_kafka_metrics', '{replica}') ORDER BY (_topic, _partition) SETTINGS index_granularity = 8192;
CREATE TABLE insights.query_log_archive (
...
) ENGINE = Distributed('ops', 'insights', 'sharded_query_log_archive');
CREATE TABLE insights.trace_attributes (
  team_id Int32,
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/logs/{shard}/insights.trace_attributes', '{replica}') ORDER BY (team_id, attribute_type, time_bucket, resource_fingerprint, attribute_key, attribute_value) PARTITION BY toDate(original_expiry_time_bucket) TTL original_expiry_time_bucket SETTINGS index_granularity = 8192;
CREATE TABLE insights.trace_attributes_distributed (
  team_id Int32,
  original_expiry_time_bucket DateTime64(0),
  time_bucket DateTime64(0),
  service_name LowCardinality(String),
  ...
  attribute_type LowCardinality(String),
  attribute_count SimpleAggregateFunction(sum, UInt64)
) ENGINE = Distributed('logs', 'insights', 'trace_attributes');
CREATE TABLE insights.trace_spans (
...
CREATE TABLE insights.trace_spans_distributed (
  ...
  events Array(String),
  links Array(String)
) ENGINE = Distributed('logs', 'insights', 'trace_spans');
CREATE TABLE insights.trace_spans_kafka_metrics (
  _partition UInt32,
  _topic String,
  max_offset SimpleAggregateFunction(max, UInt64),
  max_observed_timestamp SimpleAggregateFunction(max, DateTime64(9)),
  ...
  max_created_at SimpleAggregateFunction(max, DateTime64(9)),
  max_lag SimpleAggregateFunction(max, UInt64)
) ENGINE = ReplicatedMergeTree('/datastore/tables/logs/{shard}/insights.trace_spans_kafka_metrics', '{replica}') ORDER BY (_topic, _partition) SETTINGS index_granularity = 8192;
CREATE TABLE insights.writable_query_log_archive (
  hostname LowCardinality(String),
  user LowCardinality(String),
  query_id String,
  initial_query_id String,
...
FROM
  (
    SELECT
      ...
      _bytes_uncompressed,
      _bytes_compressed
    FROM insights.logs34
  )
GROUP BY
  team_id, time_bucket, service_name;
CREATE MATERIALIZED VIEW insights.kafka_logs_avro_kafka_metrics_mv TO insights.logs_kafka_metrics (_partition UInt32, _topic String, max_offset SimpleAggregateFunction(max, UInt64), max_observed_timestamp SimpleAggregateFunction(max, DateTime64(6)), max_timestamp SimpleAggregateFunction(max, DateTime64(6)), max_created_at SimpleAggregateFunction(max, DateTime), max_lag SimpleAggregateFunction(max, Decimal(18, 6))) AS SELECT
  _partition,
  _topic,
...
GROUP BY
  _partition, _topic;
CREATE MATERIALIZED VIEW insights.kafka_metrics_avro_kafka_metrics_mv TO insights.metrics_kafka_metrics (_partition UInt64, _topic LowCardinality(String), max_offset SimpleAggregateFunction(max, UInt64), max_observed_timestamp SimpleAggregateFunction(max, DateTime64(6)), max_timestamp SimpleAggregateFunction(max, DateTime64(6)), max_created_at SimpleAggregateFunction(max, DateTime), max_lag SimpleAggregateFunction(max, Decimal(18, 6))) AS SELECT
  _partition,
  _topic,
  maxSimpleState(_offset) AS max_offset,
  maxSimpleState(observed_timestamp) AS max_observed_timestamp,
...
CREATE MATERIALIZED VIEW insights.logs34_to_resource_attributes TO insights.log_attributes2 (team_id Int32, time_bucket DateTime64(0), original_expiry_time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  time_bucket,
  original_expiry_time_bucket,
  service_name,
...
CREATE MATERIALIZED VIEW insights.logs34_to_resource_attributes3 TO insights.log_attributes3 (team_id Int32, time_bucket DateTime64(0), original_expiry_time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), severity_text LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  time_bucket,
  original_expiry_time_bucket,
  service_name,
...
FROM
  (
    SELECT
      ...
      attribute.2 AS attribute_value,
      sumSimpleState(1) AS attribute_count
    FROM insights.metrics1
    GROUP BY
      team_id, time_bucket, service_name, resource_fingerprint, attributes
  );
CREATE MATERIALIZED VIEW insights.metrics1_to_resource_attributes TO insights.metric_attributes (team_id Int32, time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  time_bucket,
  service_name,
  resource_fingerprint,
...
CREATE MATERIALIZED VIEW insights.trace_span_to_attributes TO insights.trace_attributes (team_id Int32, original_expiry_time_bucket DateTime64(0), time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  original_expiry_time_bucket,
  time_bucket,
  service_name,
...
FROM
  (
    SELECT
      ...
      attribute.2 AS attribute_value,
      sumSimpleState(1) AS attribute_count
    FROM insights.trace_spans
    GROUP BY
      team_id, original_expiry_time_bucket, time_bucket, service_name, resource_fingerprint, attribute
  );
CREATE MATERIALIZED VIEW insights.trace_span_to_resource_attributes TO insights.trace_attributes (team_id Int32, original_expiry_time_bucket DateTime64(0), time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  original_expiry_time_bucket,
  time_bucket,
  service_name,
...
CREATE MATERIALIZED VIEW insights.trace_span_to_span_attributes TO insights.trace_attributes (team_id Int32, original_expiry_time_bucket DateTime64(0), time_bucket DateTime64(0), service_name LowCardinality(String), resource_fingerprint UInt64, attribute_key LowCardinality(String), attribute_value String, attribute_type LowCardinality(String), attribute_count SimpleAggregateFunction(sum, UInt64)) AS SELECT
  team_id,
  original_expiry_time_bucket,
  time_bucket,
  service_name,
...
CREATE MATERIALIZED VIEW insights.trace_spans_to_kafka_metrics_mv TO insights.trace_spans_kafka_metrics (_partition UInt64, _topic LowCardinality(String), max_offset SimpleAggregateFunction(max, UInt64), max_observed_timestamp SimpleAggregateFunction(max, DateTime64(6)), max_timestamp SimpleAggregateFunction(max, DateTime64(6)), max_created_at SimpleAggregateFunction(max, DateTime), max_lag SimpleAggregateFunction(max, Decimal(18, 6))) AS SELECT
  _partition,
  _topic,
  maxSimpleState(_offset) AS max_offset,
  maxSimpleState(observed_timestamp) AS max_observed_timestamp,
...
GROUP BY
  _partition, _topic;
CREATE VIEW insights.custom_metrics_backups AS WITH
  ['DatastoreCustomMetric_BackupFailed', 'DatastoreCustomMetric_BackupSuccess', 'DatastoreCustomMetric_BackupCancelled', 'DatastoreCustomMetric_BackupAttempts'] AS names,
...
GROUP BY
  event_date;
CREATE VIEW insights.custom_metrics_dictionaries AS SELECT
  'DatastoreCustomMetric_DictionariesFailed' AS name,
  map(
    'instance',
    hostname(),
    'database',
...
CREATE VIEW insights.custom_metrics_part_counts AS SELECT
...
CREATE VIEW insights.custom_metrics_replication_queue AS WITH
  ['DatastoreCustomMetric_ReplicationQueueStuckEntries', 'DatastoreCustomMetric_ReplicationQueueMaxPostponedEntrySeconds', 'DatastoreCustomMetric_ReplicationQueueMaxErrorEntrySeconds'] AS names,
...
HAVING
  value > 0;
CREATE VIEW insights.custom_metrics_server_crash AS SELECT
  'DatastoreCustomMetric_ServerCrash' AS name,
  map('instance', hostname()) AS labels,
  count() AS value,
...
GROUP BY
  hostname();
CREATE VIEW insights.custom_metrics_table_sizes AS SELECT
  'DatastoreCustomMetric_TableTotalBytes' AS name,
...
CREATE VIEW insights.custom_metrics_test AS SELECT
  'DatastoreCustomMetric_Test' AS name,
  map('instance', hostname()) AS labels,
  1 AS value,
  'Test to check that the metric endpoint is working' AS help,
  'gauge' AS type;
CREATE TABLE insights.metric_samples (
  team_id Int32,
  metric_name LowCardinality(String),
  series_fingerprint UInt64 CODEC(DoubleDelta),
  timestamp DateTime64(6) CODEC(DoubleDelta),
  ...
  span_id String,
  trace_flags Int32
) ENGINE = Distributed('logs', 'insights', 'metric_samples1');
CREATE TABLE insights.metric_series (
  team_id Int32,
  metric_name LowCardinality(String),
  series_fingerprint UInt64 CODEC(DoubleDelta),
  metric_type LowCardinality(String),
  ...
  attributes Map(LowCardinality(String), String),
  last_seen DateTime64(6) CODEC(DoubleDelta)
) ENGINE = Distributed('logs', 'insights', 'metric_series1');
CREATE TABLE insights.metrics (
  time_bucket DateTime MATERIALIZED toStartOfDay(timestamp),
  uuid String,
  team_id Int32,
  trace_id String,
  ...
  time_minute DateTime ALIAS toStartOfMinute(timestamp),
  attributes Map(String, String) ALIAS mapApply((k, v) -> (left(k, -5), v), attributes_map_str)
) ENGINE = Distributed('logs', 'insights', 'metrics1');
CREATE VIEW insights.custom_metrics AS SELECT * REPLACE(toFloat64(value) AS value)
