    ...
    it('dual-registers on both US and EU, adds proxy callback to redirect_uris, and stores mapping', async () => {
        ...
        vi.stubGlobal(
            ...
            vi.fn().mockImplementation((url: string, init: RequestInit) => {
                ...
                if (new URL(url).hostname === 'us.hanzo.ai') {
        ...
        const request = new Request('https://oauth.hanzo.ai/oauth/register/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        for (const body of sentBodies) {
            ...
            expect(body.redirect_uris).toContain('https://oauth.hanzo.ai/oauth/callback/')
        }
