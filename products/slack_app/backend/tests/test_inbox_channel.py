...
class TestInboxChannel:
    ...
    def test_creates_channel_and_sets_team_default(self, mock_webclient_class):
        ...
        assert result == ("C999", "#insights-inbox")
        client.conversations_create.assert_called_once_with(name="insights-inbox", is_private=False)
        assert get_default_slack_notification_channel(self.team.id) == "C999|#insights-inbox"

    @patch("insights.models.integration.WebClient")
    def test_reuses_configured_channel(self, mock_webclient_class):
        client = self._client(mock_webclient_class)
        set_default_slack_notification_channel(self.team.id, "C777|#insights-inbox")
        client.conversations_info.return_value = {"channel": {"id": "C777"}}

        result = inbox_channel.ensure_inbox_channel(self.integration)

        assert result == ("C777", "#insights-inbox")
    ...
    def test_reuses_existing_channel_by_name_without_manage_scope(self, mock_webclient_class):
        ...
        client.conversations_list.return_value = {
            "channels": [{"id": "C555", "name": "insights-inbox"}],
            "response_metadata": {"next_cursor": ""},
        }

        result = inbox_channel.ensure_inbox_channel(self.integration)

        assert result == ("C555", "#insights-inbox")
        client.conversations_create.assert_not_called()
        assert get_default_slack_notification_channel(self.team.id) == "C555|#insights-inbox"
    ...
    def test_create_name_taken_falls_back_to_lookup(self, mock_webclient_class):
        ...
        client = self._client(mock_webclient_class)
        client.conversations_list.return_value = {
            "channels": [{"id": "C111", "name": "insights-inbox"}],
            "response_metadata": {"next_cursor": ""},
        }
        ...
        assert result == ("C111", "#insights-inbox")
        assert get_default_slack_notification_channel(self.team.id) == "C111|#insights-inbox"
    ...
    def test_is_inbox_channel_by_configured_id(self, mock_webclient_class):
        self._client(mock_webclient_class)
        set_default_slack_notification_channel(self.team.id, "C222|#insights-inbox")
    ...
    def test_is_inbox_channel_by_name_fallback(self, mock_webclient_class):
        client = self._client(mock_webclient_class)
        client.conversations_info.return_value = {"channel": {"id": "C333", "name": "insights-inbox"}}
    ...
    def test_configured_channel_replaced_when_definitely_gone(self, mock_webclient_class):
        ...
        assert result == ("C_NEW", "#insights-inbox")
        assert get_default_slack_notification_channel(self.team.id) == "C_NEW|#insights-inbox"
