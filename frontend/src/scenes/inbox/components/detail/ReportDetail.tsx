...
export function InboxDetailFrame({
    ...
    // Depends on placement alone. `Markdown` memoizes its anchor component on this callback, so
    ...
    // standard `Menu` on narrow ones; the primary action stays inline either way.
    const reportActions = useReportDetailActions(report)
    const overflowMenuItems: MenuItem[] = reportActions.map((action) => ({
        label: action.label,
        icon: action.icon,
            ...
            <div className={`flex flex-col gap-3.5 ${hasDiff ? 'mb-4' : 'mb-6 pb-5 border-b border-primary'}`}>
                ...
                <div className="flex flex-col gap-3 @2xl:flex-row @2xl:items-start @2xl:justify-between @2xl:gap-4">
                    ...
                    <div className="flex items-center gap-2 @2xl:shrink-0">
                        ...
                        {/* Buttons inline on wide layouts; collapse into a standard Menu kebab below @4xl. */}
                        ...
                        {overflowMenuItems.length > 0 && (
                            <Menu items={overflowMenuItems} placement="bottom-end">
                                <Button
                                    type="secondary"
                                    size="small"
                                    icon={<IconEllipsis />}
                ...
                <Tabs
                    ...
                    tabs={[
                        { key: 'overview', label: 'Overview', content: overviewBody },
                        {
                            ...
                            // `Tabs` renders only the active tab, so the rating row is repeated
