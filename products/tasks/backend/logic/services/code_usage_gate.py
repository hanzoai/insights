...
from insights.models import OAuthAccessToken
...
def _gateway_usage_url() -> str | None:
    ...
    if region == "US":
        base = "https://gateway.us.hanzo.ai"
    elif region == "EU":
        base = "https://gateway.eu.hanzo.ai"
    elif settings.DEBUG:
...
def get_insights_code_usage(user, team_id: int) -> CodeUsageStatus | None:
    """Fetch the team's insights_code usage from the LLM gateway.
...
def rate_limit_error_payload(usage: CodeUsageStatus) -> dict[str, Any]:
    """Structured 429 body the Insights Desktop client parses into its upgrade prompt.
    ...
    payload: dict[str, Any] = {
        ...
        "error": "You've reached your Insights Desktop usage limit.",
        "is_pro": usage.is_pro,
    }
        ...
        TaskRunErrorResponseSerializer(
            {
                ...
                "error": "Insights Desktop access is required to run tasks in the cloud.",
            }
        ).data,
...
def cloud_usage_limit_response(user, team_id: int, *, require_tasks_access: bool = True) -> Response | None:
    ...
    Insights Code (`tasks`) entitlement check while still applying the usage-limit cost backstop —
    ...
    usage = get_insights_code_usage(user, team_id)
    if usage is None:
