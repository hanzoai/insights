...
from insights.management.commands.migrate_ses_tenants import migrate_ses_tenants
...
class TestMigrateSESTenants(BaseTest):
    ...
    @patch("insights.management.commands.migrate_ses_tenants.boto3.client")
    ...
    def test_migrate_for_team(self, mock_boto_client):
        ...
        expected = [
            ...
            (f"team-{self.team.id}", "arn:aws:ses:us-east-1:123456789012:configuration-set/insights-messaging"),
            (
                f"team-{self.team.id}",
                "arn:aws:ses:us-east-1:123456789012:configuration-set/insights-messaging-untracked",
            ),
        ]
    ...
    def test_migrate_for_domain_filter(self, mock_boto_client):
        ...
        expected = [
            ...
            (f"team-{self.team.id}", "arn:aws:ses:eu-west-1:123456789012:configuration-set/insights-messaging"),
            (
                f"team-{self.team.id}",
                "arn:aws:ses:eu-west-1:123456789012:configuration-set/insights-messaging-untracked",
            ),
        ]
        assert sesv2.associations == expected
