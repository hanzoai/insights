//! Datastore layer: scan planning, row decoding, the streaming scanners (behavioral events and
//! person rows), and the client builder. Depends only on `domain` and `config` (plus the
//! `datastore` crate); never on `store` or `kafka`.
...
pub use client::{
    build_client, DatastoreClientError, DatastoreEndpoint, DatastoreJoinAlgorithm,
};
