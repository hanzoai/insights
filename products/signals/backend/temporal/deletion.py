...
class SignalReportDeletionWorkflow:
    ...
    2. Soft-delete all signals in Datastore (re-emit with metadata.deleted=True)
    2b. Wait for the last soft-deleted signal to land in Datastore
    ...
    async def _run_impl(self, inputs: SignalReportDeletionWorkflowInputs) -> None:
        ...
        # 2b. Wait for all soft-deleted signals to land in Datastore
        await workflow.execute_activity(
            wait_for_signal_in_datastore_activity,
            WaitForDatastoreInput(
                ...
                # semantic search are acceptable, so don't burn Datastore queries here.
