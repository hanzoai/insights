...
- SQL injection (value context): raw SQL, f-strings or `%`-formatting inside `.extra()` / `.raw()` / `cursor.execute()`, InsightsQL or Datastore SQL built by string concatenation around user-supplied values.
...
- **Default crypto keys with no production startup guard.** Pattern: `ENCRYPTION_SALT_KEYS = "00beef00..."` (or similar placeholder) in `insights/settings/access.py` with a comment-only "override in prod." If a default exists and there is no `if DEBUG is False and value == DEFAULT: raise` guard mirroring the existing `SECRET_KEY` one, that's the finding — operators who don't override get a known-key encryption.
