...
class TestPushDispatcher(TestCase):
    ...
    def test_notify_enqueues_push(self, _name, notify_fn, expected_body_fragment, mock_delay, _flag):
        ...
        self.assertEqual(title, "Insights Desktop")
    ...
    @patch("products.tasks.backend.push_dispatcher.hanzo_insights.feature_enabled", return_value=False)
    @patch("products.tasks.backend.push_dispatcher.send_user_push.delay")
    ...
    @patch(
        "products.tasks.backend.push_dispatcher.hanzo_insights.feature_enabled",
    ...
    def test_swallowed_failure_increments_metric(self, _name, expected_reason, exc):
        ...
        before = REGISTRY.get_sample_value("insights_tasks_push_dispatcher_failures_total", labels) or 0.0
        with patch("products.tasks.backend.push_dispatcher._enqueue_inner", side_effect=exc):
            notify_task_run_completed(self.task_run)
        after = REGISTRY.get_sample_value("insights_tasks_push_dispatcher_failures_total", labels) or 0.0
