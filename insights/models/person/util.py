...
PERSONFN_BATCH_SIZE: int = settings.PERSONFN_BATCH_SIZE
...
_get_client = require_personinsights_client
...
def _get_persons_for_uuid_batch(
    ...
    if mismatched:
        PERSONFN_TEAM_MISMATCH_TOTAL.labels(operation=operation, client_name=get_client_name()).inc(mismatched)
        logger.warning("personinsights_team_mismatch", operation=operation, team_id=team_id, dropped=mismatched)
...
def _batched_get_persons_by_uuids(
    ...
    """Fetch persons for the given UUIDs, one RPC per PERSONFN_BATCH_SIZE batch.
    ...
    callers of this helper don't multiply their load on the personinsights bulk pools. Keep
    opted-in values modest: each in-flight RPC can occupy up to 2 connections of a
    replica's 5-connection bulk Postgres pool.
    """
    client = _get_client()
    batches = [uuids[i : i + PERSONFN_BATCH_SIZE] for i in range(0, len(uuids), PERSONFN_BATCH_SIZE)]
    ...
    if TEST or max_workers <= 1:
        ...
        # doesn't inherit contextvars, so copy the current context per task to keep the personinsights
        ...
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="personinsights-uuid-batch") as executor:
            futures = [
                executor.submit(
...
def _batched_get_persons_by_distinct_ids(
    ...
    for i in range(0, len(distinct_ids), PERSONFN_BATCH_SIZE):
        batch = distinct_ids[i : i + PERSONFN_BATCH_SIZE]
...
def _batched_get_distinct_ids_for_persons(
    ...
    """Fetch each person's distinct_ids with their versions, one RPC per PERSONFN_BATCH_SIZE batch."""
    client = _get_client()
    distinct_ids_by_person: dict[int, list[DistinctIdForPerson]] = {}
    for i in range(0, len(person_ids), PERSONFN_BATCH_SIZE):
        batch_ids = person_ids[i : i + PERSONFN_BATCH_SIZE]
...
if TEST:
    ...
    def bulk_create_persons(persons_list: list[dict]):
        """Bulk-create persons in Datastore and return the distinct_id → Person mapping.

        Test-only.  Builds in-memory Person instances with synthetic primary keys
        and uuids (no persons DB write) and inserts the rows into Datastore.  The
        personinsights fake is seeded by the caller (insights.test.persons).
        """
        from insights.test.persons import _next_synthetic_pk  # noqa: PLC0415
...
def _fetch_persons_by_distinct_ids_via_personinsights(
...
def get_persons_by_distinct_ids(
    ...
    *,
    ...
    return personinsights_call(
        operation,
        lambda: _fetch_persons_by_distinct_ids_via_personinsights(
...
def get_persons_mapped_by_distinct_id(
    ...
    def personinsights_fn() -> dict[str, Person]:
...
def get_distinct_ids_for_persons(
    ...
    *,
    ...
    """Map each person_id to its distinct_ids via personinsights.
    ...
    distinct_ids_by_person = personinsights_call(
...
def _fetch_persons_by_uuids_via_personinsights(
...
def get_persons_by_uuids(team_id: int, uuids: list[str], *, distinct_id_limit: int | None = None) -> list[Person]:
    return personinsights_call(
        ...
        lambda: _fetch_persons_by_uuids_via_personinsights(team_id, uuids, distinct_id_limit=distinct_id_limit),
    )
...
def _fetch_person_by_id_via_personinsights(
    ...
    if resp.person.team_id != team_id:
        PERSONFN_TEAM_MISMATCH_TOTAL.labels(operation="get_person_by_id", client_name=get_client_name()).inc()
        logger.warning("personinsights_team_mismatch", operation="get_person_by_id", team_id=team_id)
        return None
...
def get_person_by_id(team_id: int, person_id: int, *, distinct_id_limit: int | None = None) -> Optional[Person]:
    return personinsights_call(
        "get_person_by_id",
        lambda: _fetch_person_by_id_via_personinsights(team_id, person_id, distinct_id_limit=distinct_id_limit),
    )


def _fetch_person_by_uuid_via_personinsights(
    ...
    if resp.person.team_id != team_id:
        PERSONFN_TEAM_MISMATCH_TOTAL.labels(operation="get_person_by_uuid", client_name=get_client_name()).inc()
        logger.warning("personinsights_team_mismatch", operation="get_person_by_uuid", team_id=team_id)
        return None
...
def get_person_by_uuid(team_id: int, uuid: str, *, distinct_id_limit: int | None = None) -> Optional[Person]:
    return personinsights_call(
        "get_person_by_uuid",
        lambda: _fetch_person_by_uuid_via_personinsights(team_id, uuid, distinct_id_limit=distinct_id_limit),
    )


def _fetch_person_by_distinct_id_via_personinsights(
    ...
    if resp.person.team_id != team_id:
        PERSONFN_TEAM_MISMATCH_TOTAL.labels(operation="get_person_by_distinct_id", client_name=get_client_name()).inc()
        logger.warning("personinsights_team_mismatch", operation="get_person_by_distinct_id", team_id=team_id)
        return None
...
def get_person_by_distinct_id(
    ...
    return personinsights_call(
        ...
        lambda: _fetch_person_by_distinct_id_via_personinsights(team_id, distinct_id, distinct_id_limit=distinct_id_limit),
    )


def get_person_by_pk_or_uuid(team_id: int, key: str, *, distinct_id_limit: int | None = None) -> Optional[Person]:
    """Look up a person by UUID or integer PK, routing through personinsights when enabled."""
    try:
        UUID(key)
...
def _validate_uuids_via_personinsights(team_id: int, uuids: list[str]) -> list[str]:
...
def validate_person_uuids_exist(team_id: int, uuids: list[str]) -> list[str]:
    return personinsights_call(
        ...
        lambda: _validate_uuids_via_personinsights(team_id, uuids),
    )
...
def get_person_ids_and_uuids_by_uuids(team_id: int, uuids: list[str], *, concurrency: int = 1) -> list[tuple[int, str]]:
    ...
    def personinsights_fn() -> list[tuple[int, str]]:
...
def get_person_uuids_by_distinct_ids(team_id: int, distinct_ids: list[str]) -> list[str]:
    ...
    properties and other heavy fields from personinsights.
    """
    if not distinct_ids:
        return []

    def personinsights_fn() -> list[str]:
...
def delete_persons_from_postgres(team_id: int, persons: list[Person]) -> None:
    """Delete Person rows (and associated PersonDistinctId rows) via the personinsights RPC.

    Processes in batches of 1000 (the RPC maximum).
    """

    def personinsights_fn() -> None:
    ...
    personinsights_call("delete_persons", personinsights_fn)


def delete_person(person: Person, distinct_ids: list[DistinctIdForPerson] | None = None) -> None:
    """Produce Datastore deletion tombstones for a person and its distinct_ids.
    ...
    """
    # This is racy https://github.com/Insights/insights/issues/11590
...
def _delete_person(
    ...
    create_person(
        ...
        # - insights/models/person/person.py:112 (split_person uses version + 101 to override deletes)
        version=version + 100,
...
def _get_distinct_ids_with_version(person: Person) -> list[DistinctIdForPerson]:
    def personinsights_fn() -> list[DistinctIdForPerson]:
    ...
    return personinsights_call("get_distinct_ids_with_version", personinsights_fn)
