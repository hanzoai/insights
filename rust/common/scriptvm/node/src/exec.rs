//! Batch execution of one Script program against many event-globals.
...
mod tests {
    ...
    #[test]
    fn insights_capture_errors_like_the_node_executor() {
        let results = run_batch(&call_fn_program("insightsCapture", "x"), &[json!({})], None);
        assert!(results[0]
            .error
            .as_deref()
            .unwrap()
            .contains("insightsCapture is not supported in transformations"));
    }
    ...
    // Pins the scriptvm crate's error formats the Node callers (rust-vm.ts isUnsupportedByRustVm)
