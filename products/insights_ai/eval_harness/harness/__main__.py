...
from .ports import PERSONFN_ROUTER_PORT
...
def _run(options: HarnessOptions) -> int:
    ...
    # Point person/group reads at the harness's own personinsights router before Django
    # loads settings. PERSONFN_ADDR is read from env once at django.setup(), and the
    # personinsights client is a cached singleton keyed off that setting at its first call.
    ...
    # singleton is already built. A PERSONFN_ADDR leaked from a sourced dev .env
    # would otherwise silently point eval reads at the dev persons DB.
    os.environ["PERSONFN_ADDR"] = f"127.0.0.1:{PERSONFN_ROUTER_PORT}"
