"""
Traffic type classification functions for InsightsQL.

Implemented as InsightsQL functions (rather than hardcoded in specific query runners) so they
can be used anywhere InsightsQL runs:
- SQL editor for ad-hoc analysis and exploration
- InsightsQLQuery runner for custom dashboards and insights
...
products.web_analytics.backend.insightsql_queries so that changes to bot data do not require
a InsightsQL review.
"""
...
from products.web_analytics.backend.insightsql_queries.bot_ip_definitions import (
    BOT_IP_DEFINITIONS,
...
def _build_bot_array_lookup(
    ...
    # labels[index] - array access (1-based in Datastore)
...
def get_bot_name(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    InsightsQL function: getBotName(user_agent[, ip])
...
def get_bot_operator(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    InsightsQL function: getBotOperator(user_agent[, ip])
...
def get_traffic_type(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    InsightsQL function: getTrafficType(user_agent[, ip])
...
def get_traffic_category(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    InsightsQL function: getTrafficCategory(user_agent[, ip])
...
def is_bot(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    InsightsQL function: isLikelyBot(user_agent[, ip])
...
def get_bot_type(node: ast.Call, args: list[ast.Expr]) -> ast.Expr:
    """
    InsightsQL function: getBotType(user_agent[, ip])
