...
from insights.schema import InsightsQLNotice
...
from insights.models import EventDefinition, PropertyDefinition, Team
...
def _warnings_for_unknown_references(
    ...
    for name in unknown_names:
        ...
        warnings.append(InsightsQLNotice(message=message, start=reference.start, end=reference.end, fix=fix))
...
def _build_fix(fix_context: FixContext | None, suggestion: str) -> str | None:
    ...
    # quick-fix can never produce broken InsightsQL (e.g. an event named `o'brien`, or a property with a
    # space). `escape_insightsql_identifier` rejects a few names (e.g. containing `%`); offer no fix then.
    try:
        if fix_context == "string":
            return escape_insightsql_string(suggestion)
        if fix_context == "property":
            return f"properties.{escape_insightsql_identifier(suggestion)}"
    except Exception:
        return None
    return None
