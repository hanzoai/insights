...
from insights.schema import PropertyType, PropertyValuesQuery, WebAnalyticsAssistantFilters
...
from insights.temporal.health_checks.registry import HEALTH_CHECKS, ensure_registry_loaded, get_detect_fn
...
class AssessHeatmapArgs(BaseModel):
    page_url: str = Field(
        ...
        "(e.g. 'https://hanzo.ai/pricing'). The trailing slash is ignored.",
    )
    date_from: str = Field(
        default="-7d",
...
def _execute(team: Team, user: User, stmt: ast.SelectQuery | ast.SelectSetQuery) -> Any:
    context = InsightsQLContext(team_id=team.pk, limit_top_select=False)
