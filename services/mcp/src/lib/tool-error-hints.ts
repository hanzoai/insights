 ...
 *
 * `handleToolError` returns the raw Insights API error verbatim. For a 5xx that
...
/**
 * The logs query endpoints scan Datastore and time out (surfacing as a 5xx)
...
interface RecoveryHintInput {
    /** The failed request URL (from `InsightsApiError.url`), if the error was an HTTP failure. */
