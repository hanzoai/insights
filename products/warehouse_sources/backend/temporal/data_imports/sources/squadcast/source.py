...
class SquadcastSource(ResumableSource[SquadcastSourceConfig, SquadcastResumeConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Enter your Squadcast refresh token to pull your incident response data into the Insights Data warehouse.

You can generate a refresh token from your Squadcast profile page (**Profile → Refresh Token**). Insights exchanges it for a short-lived access token on each sync.
...
The token inherits your user role, so the connecting user needs read access to the teams and resources you want to sync.""",
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/squadcast",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
