...
from insights.insightsql.database.database import Database, SerializedField, get_data_warehouse_table_name, serialize_fields
...
from products.warehouse_sources.backend.facade.insightsql import (
...
def _delete_hosted_upload_file(table: DataWarehouseTable) -> None:
    """Best-effort removal of a self-managed table's backing file from Insights's own bucket.
...
class TableSerializer(UserAccessControlSerializerMixin, serializers.ModelSerializer):
    ...
    insightsql_name = serializers.SerializerMethodField(
        read_only=True,
        help_text="Dotted name the table is queried by in InsightsQL (e.g. `googleanalytics.devices` or "
    ...
    def get_insightsql_name(self, table: DataWarehouseTable) -> str:
    ...
    def get_columns(self, table: DataWarehouseTable) -> list[SerializedField]:
        # Callers that only need the table list (e.g. a table picker) skip the expensive InsightsQL field
    ...
    def create(self, validated_data):
        ...
        # this would allow an external user to then access one of Insights's internal S3 buckets
    ...
    def validate_url_pattern(self, url_pattern):
        ...
        if s3_domain in url_pattern:
            raise serializers.ValidationError(
                "This URL points to Insights's internal storage and can't be used as a source. "
    ...
    def validate_name(self, name):
        if not self.instance or self.instance.name != name:
            # has_table covers system/insights tables and warehouse objects the requesting user can see;
...
class SimpleTableSerializer(UserAccessControlSerializerMixin, serializers.ModelSerializer):
    ...
    class Meta:
        model = DataWarehouseTable
        fields = ["id", "name", "insightsql_name", "columns", "row_count", "user_access_level"]
        read_only_fields = ["id", "name", "insightsql_name", "columns", "row_count", "user_access_level"]
    ...
    def get_columns(self, table: DataWarehouseTable) -> list[SerializedField]:
        # Callers that don't consume columns (e.g. the source list) skip the expensive InsightsQL
...
class CreateTableFromUploadSerializer(serializers.Serializer):
    ...
    table_name = serializers.CharField(help_text="Name the resulting table is queried by in InsightsQL.")
...
class TableViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_serializer_context(self) -> dict[str, Any]:
        ...
        # Building the InsightsQL database is only needed to serialize columns; a caller that opts out
    ...
    @extend_schema(
        ...
        description=(
            ...
            "new columns, because Insights serves a cached column snapshot until the table is refreshed. "
    ...
    def create_from_upload(self, request: request.Request, *args: Any, **kwargs: Any) -> response.Response:
        ...
        try:
            ...
            # The raw column-detection failure is a Datastore error that's opaque to users, so keep it
