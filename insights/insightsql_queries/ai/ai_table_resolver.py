...
from insights.insightsql_queries.ai.ai_property_rewriter import rewrite_expr_for_ai_events_table
...
if TYPE_CHECKING:
    ...
    from insights.insightsql.modifiers import InsightsQLQueryModifiers
    ...
    *,
    ...
    with tags_context(product=Product.LLM_ANALYTICS):
        ...
        with AI_EVENTS_QUERY_DURATION_SECONDS.labels(source="dedicated_table").time():
            result = execute_insightsql_query(query=query, placeholders=ai_placeholders, **kwargs)
        if result.results:
        ...
        if fall_back_to_events:
            ...
            with AI_EVENTS_QUERY_DURATION_SECONDS.labels(source="shared_table_fallback").time():
                return execute_insightsql_query(query=events_query, placeholders=events_placeholders, **events_kwargs)
        ...
        with AI_EVENTS_QUERY_DURATION_SECONDS.labels(source="retention_probe").time():
            probe = execute_insightsql_query(query=events_query, placeholders=events_placeholders, **events_kwargs)
        if probe.results:
