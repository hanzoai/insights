...
Rerun `insights:query-trends` at `interval: "hour"` scoped to the suspicious day(s). A
...
Run `insights:query-trends-actors` on the anomalous bucket (or breakdown value that
moved). For UI-shaped drops, pull session recordings for the same segment.

## 4. Errors / logs

`insights:query-error-tracking-issues-list` and `insights:query-logs` for the window. Confirm
timing, plausible mechanism, and user overlap before treating as the cause.

## 5. Cohort composition

Run `insights:query-lifecycle` on the same event. If the drop is concentrated in one
