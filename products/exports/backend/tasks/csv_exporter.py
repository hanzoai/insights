...
from insights.insightsql.constants import CSV_EXPORT_BREAKDOWN_LIMIT_INITIAL, CSV_EXPORT_BREAKDOWN_LIMIT_LOW, CSV_EXPORT_LIMIT
from insights.insightsql.errors import TableAccessDeniedError
from insights.insightsql.query import LimitContext

from insights.api.services.query import process_query_dict
from insights.event_usage import AnalyticsProps, EventSource
from insights.exceptions import DatastoreQuerySizeExceeded
...
from insights.tasks.exporter import EXPORT_TIMER
...
class CsvWriter(TabularWriter):
    ...
    def write_header(self, columns: list[str]) -> None:
        ...
        self._writer.writer.writerow([sanitize_formula_injection(c) for c in columns])
    ...
    while total < CSV_EXPORT_LIMIT:
        ...
        try:
            query_response = process_query_dict(
                ...
                # warehouse InsightsQL access control resolves against their access.
                user=exported_asset.created_by,
                pagination_cursor=cursor,
                analytics_props=analytics_props,
            )
        except DatastoreQuerySizeExceeded:
