...
from insights.temporal.weekly_digest.activities import (
...
async def mock_heartbeater():
    ...
    with patch("insights.temporal.weekly_digest.activities.Heartbeater"):
        yield
...
async def test_count_teams():
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_queryset):
...
async def test_count_organizations():
    ...
    with patch("insights.temporal.weekly_digest.activities.query_orgs_for_digest", return_value=mock_queryset):
...
async def test_generate_dashboard_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_new_dashboards", return_value=mock_dashboard_queryset
        ):
            with patch("insights.temporal.weekly_digest.activities.queryset_to_list", side_effect=mock_queryset_to_list):
                with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_generate_event_definition_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_new_event_definitions", return_value=mock_event_queryset
        ):
            with patch("insights.temporal.weekly_digest.activities.queryset_to_list", side_effect=mock_queryset_to_list):
                with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_generate_experiment_launched_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_experiments_launched",
...
async def test_generate_experiment_completed_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_experiments_completed",
            return_value=mock_experiment_queryset,
        ):
            with patch("insights.temporal.weekly_digest.activities.queryset_to_list", side_effect=mock_queryset_to_list):
                with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_generate_external_data_source_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_new_external_data_sources",
...
async def test_generate_feature_flag_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_new_feature_flags", return_value=mock_flag_queryset
        ):
            with patch("insights.temporal.weekly_digest.activities.queryset_to_list", side_effect=mock_queryset_to_list):
                with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_generate_survey_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_surveys_launched", return_value=mock_survey_queryset
        ):
            with patch("insights.temporal.weekly_digest.activities.queryset_to_list", side_effect=mock_queryset_to_list):
                with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_generate_filter_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch("insights.temporal.weekly_digest.activities.query_saved_filters", return_value=mock_filter_queryset):
            with patch("insights.temporal.weekly_digest.activities.queryset_to_list", side_effect=mock_queryset_to_list):
                with patch("insights.temporal.weekly_digest.activities.redis.from_url", side_effect=mock_redis_from_url):
...
async def test_generate_recording_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch("insights.temporal.weekly_digest.activities.get_ch_client", return_value=mock_ch_client):
            with patch("insights.temporal.weekly_digest.activities.database_sync_to_async") as mock_sync:
...
async def test_generate_user_notification_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch("insights.temporal.weekly_digest.activities.should_send_notification", return_value=True):
            with patch("insights.temporal.weekly_digest.activities.database_sync_to_async") as mock_sync:
...
async def test_generate_organization_digest_batch(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_orgs_for_digest", return_value=mock_org_queryset):
        with patch("insights.temporal.weekly_digest.activities.query_org_teams", return_value=mock_team_queryset):
            with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_send_weekly_digest_batch(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_orgs_for_digest", return_value=mock_org_queryset):
        with patch("insights.temporal.weekly_digest.activities.query_org_members", return_value=mock_member_queryset):
            with patch("insights.temporal.weekly_digest.activities.get_ph_client", return_value=mock_ph_client):
                with patch("insights.temporal.weekly_digest.activities.MessagingRecord.objects", mock_messaging_objects):
                    with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
...
async def test_generate_product_suggestion_lookup(mock_redis, common_input, digest):
    ...
    with patch("insights.temporal.weekly_digest.activities.query_teams_for_digest", return_value=mock_team_queryset):
        with patch(
            "insights.temporal.weekly_digest.activities.query_user_product_suggestions",
    ...
    with patch("insights.temporal.weekly_digest.activities.query_orgs_for_digest", return_value=mock_org_queryset):
        with patch("insights.temporal.weekly_digest.activities.query_org_members", return_value=mock_member_queryset):
            with patch("insights.temporal.weekly_digest.activities.get_ph_client", return_value=mock_ph_client):
                with patch("insights.temporal.weekly_digest.activities.MessagingRecord.objects", mock_messaging_objects):
                    with patch("insights.temporal.weekly_digest.activities.redis.from_url", return_value=mock_redis):
                        with patch("insights.temporal.weekly_digest.activities.LOGGER") as mock_logger:
