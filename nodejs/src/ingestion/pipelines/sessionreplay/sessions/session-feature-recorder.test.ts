...
describe('SessionFeatureRecorder', () => {
    ...
    describe('Network request tracking', () => {
        ...
        const makeInsightsNetworkEvent = (timestamp: number, duration?: number, status?: number): SnapshotEvent =>
            ({
                type: RRWebEventType.Plugin,
                timestamp,
                data: {
                    plugin: 'insights/network@1',
        ...
        it('should count requests from insights/network@1 plugin', () => {
            const events = [makeInsightsNetworkEvent(1000, 150, 200), makeInsightsNetworkEvent(2000, 300, 200)]
            recorder.recordMessage(createMessage(events))
            const result = recorder.end()!
        ...
        it('should aggregate across both plugin types', () => {
            const events = [
                ...
                makeInsightsNetworkEvent(2000, 200, 500),
            ]
            recorder.recordMessage(createMessage(events))
            const result = recorder.end()!
