...
from insights.insightsql.database.schema.exchange_rate import convert_currency_call
...
class SharedTouchpointsPrecompute:
    ...
    its own `ensure_precomputed` for the same window: redundant Datastore work, and concurrent
...
class ConversionGoalProcessor:
    ...
    # Per-goal timings. InsightsQLTimings is not thread safe and goals are built in parallel, so each
    # processor owns a clone (see InsightsQLTimings.clone_for_subquery); the runner merges them back once
    # the pool has joined. Defaults to a standalone instance for callers outside the read path.
    timings: InsightsQLTimings = dataclass_field(default_factory=InsightsQLTimings)
    ...
    def _precompute_materialized_event_properties(self) -> set[str]:
        ...
        # could still read it from the preagg table, bypassing the per-property InsightsQL masking.
        props = {"$session_id"}
    ...
    def _precompute_properties_restricted_for_user(self) -> bool:
        ...
        plain scalar columns, which bypasses the per-user property masking InsightsQL applies to
    ...
    def _build_attribution_from_precomputes(
        ...
        # Both ensure_precomputed calls can materialize inline (PG + Redis + Datastore) when a slice
    ...
    def _build_array_collection_from_precomputes(
        ...
        # touchpoint arrays (Datastore fills missing Array columns with []), handled downstream.
    ...
    def _build_distinct_preagg_rows(
        ...
        return ast.SelectQuery(
            select=row_columns,
            distinct=True,
            select_from=ast.JoinExpr(table=ast.Field(chain=["insights", table])),
            where=ast.And(
                exprs=[
                    ast.Call(
                        name="in",
    ...
    def _build_time_decay_weights(self, filtered_ts: ast.Expr, attribution_window_seconds: int) -> ast.Expr:
        ...
        # arrayMap input), so Datastore evaluates the exp() computation twice per row.
    ...
    def _build_position_based_weights(self, filtered_ts: ast.Expr) -> ast.Expr:
        ...
        # Wrap in ifNull to handle Nullable(Float64) from InsightsQL casts.
