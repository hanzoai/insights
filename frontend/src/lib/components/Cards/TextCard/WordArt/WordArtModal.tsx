...
import { Button, Input, Modal, SegmentedButton } from '@hanzo/elements'
...
export function WordArtModal({
    ...
    useEffect(() => {
        insights.capture('dashboard text tile word art gallery opened', { is_editing: isEditing })
    ...
    const save = (): void => {
        if (trimmedText) {
            insights.capture('dashboard text tile word art saved', {
                is_new: !isEditing,
                style,
                size,
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-center gap-2">
                    <Input
                        ...
                        className="flex-1"
                    />
                    <SegmentedButton value={size} onChange={setSize} options={SIZE_OPTIONS} size="small" />
                </div>
