...
class BackfillMaterializedPropertiesBatchWorkflow(InsightsWorkflow):
    """
    ...
    Flow:
      ...
      2. Sync the current (team_id, column_index) → property_name mapping to the
         Datastore-side ``dmat_slot_assignments`` table on every host and reload the
      ...
      5. Transition the assigned slots to READY. InsightsQL immediately starts using the
