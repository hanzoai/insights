...
from products.review.backend.models import ReviewReport
from products.review.backend.reviewer.models.github_meta import PRMetadata
from products.review.backend.reviewer.persistence import upsert_review_report
from products.review.backend.reviewer.tools.github_client import GitHubAPIError
from products.review.backend.reviewer.tools.publish_review import (
    PublishOutcome,
...
from products.review.backend.temporal.activities import _publish
...
_PUBLISH = "products.review.backend.reviewer.tools.publish_review.publish_review"
_SNAPSHOT = "products.review.backend.reviewer.tools.publish_review.load_pr_snapshot"
_AUTH = "products.review.backend.temporal.activities._installation_auth"
_PAGINATED = "products.review.backend.reviewer.tools.publish_review.github_api_get_paginated"
...
def test_review_already_posted_detects_our_own_markered_review() -> None:
    ...
    bot = {"user": {"login": "insights[bot]", "type": "Bot"}}
    ...
    with override_settings(REVIEWFN_GITHUB_BOT_LOGIN="insights[bot]"):
...
class TestPublishIdempotency(BaseTest):
    def _report(self, *, published_head_sha: str | None = None) -> str:
        report_id = upsert_review_report(
            ...
            pr_url="https://github.com/Insights/insights/pull/1",
    ...
    def test_first_publish_posts_promo_and_records_watermark(self, _auth, _snapshot, mock_publish) -> None:
        ...
        _publish(self.team.id, report_id, "sha1", 1, "Insights", "insights", 1, "should_fix")
    ...
    def test_new_head_publishes_without_promo(self, _auth, _snapshot, mock_publish) -> None:
        ...
        _publish(self.team.id, report_id, "sha2", 1, "Insights", "insights", 1, "should_fix")
        assert mock_publish.call_count == 1
