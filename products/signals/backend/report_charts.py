...
drag `insights.schema` onto every process's `django.setup()` path.
"""
...
def _unstorable_text(value: Any) -> str | None:
    ...
    query that merely spells the escape out (InsightsQL searching for it, say). Keys as well as values,
...
class ReportChart(BaseModel):
    ...
    `insights.schema`, which this module deliberately avoids (see the module docstring). The `kind`
