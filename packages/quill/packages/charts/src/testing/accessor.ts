// DOM-based inspector for script-charts. Reads what the chart rendered so tests
...
function readReferenceLine(el: HTMLElement): ReferenceLineSummary {
    ...
    const isLabel = labelEl?.getAttribute('data-attr') === 'script-chart-reference-line-label'
...
export function getHogChart<Meta = unknown>(
    ...
    if (!wrapper) {
        throw new Error('script-chart canvas has no parent wrapper')
    }
    ...
    return {
        ...
        get hasRightAxis(): boolean {
            return wrapper.querySelectorAll('[data-attr="script-chart-axis-tick-yr"]').length > 0
        },
        yTicks: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-axis-tick-y"]')).map(
                (el) => el.textContent ?? ''
            ),
        yRightTicks: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-axis-tick-yr"]')).map(
                (el) => el.textContent ?? ''
            ),
        xTicks: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-axis-tick-x"]')).map(
                (el) => el.textContent ?? ''
            ),
        xAxisLabel: () =>
            wrapper.querySelector<HTMLElement>('[data-attr="script-chart-axis-title-x"]')?.textContent ?? null,
        yAxisLabel: () =>
            wrapper.querySelector<HTMLElement>('[data-attr="script-chart-axis-title-y"]')?.textContent ?? null,
        xAxisLabelElement: () => wrapper.querySelector<SVGTextElement>('[data-attr="script-chart-axis-title-x"]'),
        yAxisLabelElement: () => wrapper.querySelector<SVGTextElement>('[data-attr="script-chart-axis-title-y"]'),
        referenceLines: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-reference-line"]')).map(
                readReferenceLine
            ),
        valueLabels: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-value-label"]')).map((el) => ({
                text: el.textContent ?? '',
                color: el.style.backgroundColor,
            })),
        anomalyPoints: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-anomaly-point"]')).map((el) => ({
                element: el,
                color: el.style.backgroundColor,
            })),
        slopeValueLabels: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-slope-value-label"]')).map(
                (el) => ({
                    text: el.textContent ?? '',
        ...
        slopeSeriesLabels: () =>
            Array.from(wrapper.querySelectorAll<HTMLElement>('[data-attr="script-chart-slope-series-label"]')).map(
                (el) => el.textContent ?? ''
            ),
        slopeLegendItems: () => {
            ...
            const legend = scope.querySelector<HTMLElement>('[data-attr="script-chart-slope-legend"]')
            if (!legend) {
                return []
            }
            return Array.from(legend.querySelectorAll<HTMLElement>('.truncate')).map((labelEl) => {
                const secondary = labelEl.nextElementSibling
                const isSecondary = secondary?.getAttribute('data-attr') === 'script-chart-legend-secondary'
                return {
        ...
        async waitForTooltip(timeout?: number): Promise<TooltipSnapshot<Meta>> {
            ...
            return {
                ...ctx,
                element,
                isPinned: element.classList.contains('script-charts-tooltip--pinned'),
