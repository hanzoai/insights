//! Internal Insights telemetry for Insights's own Rust services: global client
...
const SDK_DELIVERY_FAILURES: &str = "insights_sdk_delivery_failures_total";
...
/// Initialize the global Insights client used for internal service telemetry
/// (product analytics events and exception capture).
///
/// `endpoint` accepts both a bare host (`https://us.i.hanzo.ai`) and the
...
pub async fn init(
    ...
    let Some(api_key) = api_key else {
        ...
        tracing::warn!("Insights client disabled (no API key)");
        return Ok(());
    };
    ...
    let error_tracking = insights_rs::ErrorTrackingOptionsBuilder::default()
        .capture_panics(true)
        .in_app_exclude_paths(vec!["common_insights::".to_string()])
        .build()
        .expect("all error tracking options have defaults");
    let options = insights_rs::ClientOptionsBuilder::default()
    ...
    insights_rs::init_global(options).await?;
    tracing::info!("Insights client initialized");
    Ok(())
}
...
{
    if insights_rs::global_is_disabled() {
        return;
    }
    ...
    let mut options = insights_rs::CaptureExceptionOptions::new();
    for (key, value) in properties {
        options = options.property(key, value).expect(prop_err);
    }

    let send = async move {
        if let Err(e) = insights_rs::capture_exception_with(&*error, options).await {
            tracing::error!(error = ?e, "Failed to capture exception to Insights");
        }
    };
...
fn prepare_event_for_capture(
    mut event: insights_rs::Event,
    context: &ServiceContext,
) -> Option<insights_rs::Event> {
...
fn is_fatal_exception(event: &insights_rs::Event) -> bool {
...
fn report_sdk_error(service: &'static str, err: &InsightsError<'_>) {
    ...
    tracing::warn!(
        ...
        surface,
        reason,
        "insights-rs dropped telemetry after terminal delivery failure"
    );
}

/// Map a `InsightsError` to bounded `(surface, reason)` metric labels. `surface`
/// is the SDK network path that failed; `reason` classifies the cause. Both are
/// `&'static str` to keep the label set low-cardinality. `InsightsError` is
/// `#[non_exhaustive]`, hence the catch-all arm.
fn classify_sdk_error(err: &InsightsError<'_>) -> (&'static str, &'static str) {
    match err {
        ...
        InsightsError::Capture(f) => (
            "capture",
            f.error().map_or("partial_drop", sdk_error_reason),
        ),
        InsightsError::FeatureFlags(f) => ("flags", sdk_error_reason(f.error())),
        InsightsError::LocalEvaluation(f) => ("local_evaluation", sdk_error_reason(f.error())),
        _ => ("unknown", "other"),
    }
}

/// Bounded failure class for a `insights_rs::Error`. Mirrors the tag split the
...
/// `insights_rs::Error` is `#[non_exhaustive]`, hence the catch-all arm.
fn sdk_error_reason(err: &insights_rs::Error) -> &'static str {
    match err {
        insights_rs::Error::BillingLimitExceeded(_) => "quota",
        insights_rs::Error::BadRequest(_) => "bad_request",
        insights_rs::Error::ServerError { .. } => "server_error",
        insights_rs::Error::RateLimit => "rate_limited",
        insights_rs::Error::Unauthorized => "unauthorized",
        insights_rs::Error::Connection(_) => "transport",
        insights_rs::Error::Serialization(_) => "serialization",
        _ => "other",
    }
}

/// Strip a legacy capture path suffix from a configured Insights endpoint,
...
mod tests {
    ...
    fn normalize_host_strips_capture_suffixes() {
        let cases = [
            ...
            (
                "http://capture.insights.svc:3000",
                "http://capture.insights.svc:3000",
            ),
        ];
