...
def _input_references_integration(input_entry: Any, integration_id: int) -> bool:
    ...
    # (see nodejs/src/cdp/services/script-inputs.service.ts loadIntegrationInputs)
...
def _function_references_integration(function: InsightsFunction, integration_id: int) -> bool:
...
def get_enabled_insights_functions_using_integration(team_id: int, integration_id: int) -> list[InsightsFunction]:
    """Enabled, non-deleted script functions whose integration-typed inputs reference the integration.
    ...
    """
    functions = InsightsFunction.objects.filter(
        team_id=team_id,
        enabled=True,
        deleted=False,
