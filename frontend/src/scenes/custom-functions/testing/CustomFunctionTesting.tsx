...
} from '../configuration/customFunctionConfigurationLogic'
import { customFunctionTestLogic } from '../configuration/customFunctionTestLogic'
import { CustomFunctionFilters } from '../filters/CustomFunctionFilters'
import { tagTypeForLevel } from '../logs/LogsViewer'
import { CyclotronJobTestInvocationResultWithEventId, customFunctionTestingLogic } from './customFunctionTestingLogic'
...
const buildGlobals = (
    ...
    const globals = convertToCustomFunctionInvocationGlobals(row[0], row[1])
    globals.groups = {}
    ...
    globals.source = {
        name: customFunctionName ?? 'Unnamed',
...
export function CustomFunctionTesting(): JSX.Element | null {
    ...
    return (
        <BindLogic logic={customFunctionTestingLogic} props={{ id }}>
...
function TestRunnerOptions(): JSX.Element {
    const { selectingMany, eventsWithRetries, loadingRetries, selectedForRetry } = useValues(customFunctionTestingLogic)
    const { setSelectingMany, retryInvocation, selectForRetry, deselectForRetry, resetSelectedForRetry } =
        useActions(customFunctionTestingLogic)
    const { groupTypes, configuration } = useValues(customFunctionConfigurationLogic)
...
function RunsFilters(): JSX.Element {
    const { eventsLoading, baseEventsQuery } = useValues(customFunctionTestingLogic)
    const { loadEvents, changeDateRange, loadTotalEvents } = useActions(customFunctionTestingLogic)
    const { logicProps } = useValues(customFunctionConfigurationLogic)
        ...
        <>
            ...
            <LemonDropdown
                ...
                overlay={
                    ...
                    >
                        <CustomFunctionFilters embedded={true} showTriggerOptions={false} />
...
function TestingEventsList(): JSX.Element | null {
    const {
        ...
        selectingMany,
        selectedForRetry,
    } = useValues(customFunctionTestingLogic)
    const {
        retryInvocation,
        loadNextEventsPage,
    ...
    const { groupTypes, configuration, logicProps } = useValues(customFunctionConfigurationLogic)
    const id = logicProps.id ?? 'new'
    const { setSampleGlobals, toggleExpanded } = useActions(customFunctionTestLogic(logicProps))
                                    ...
                                    items={[
                                        ...
                                        },
                                        {
                                            ...
                                            onClick: () => {
                                                ...
                                                toggleExpanded(true)
                                                router.actions.push(urls.customFunction(id) + '?tab=configuration')
                                            },
                                        },
                                    ]}
                                >
