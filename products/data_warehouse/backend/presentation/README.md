...
1. **Source-domain behaviour** (how to talk to the upstream system, whether it supports a
   ...
   - **A boolean/value the API reads** → a flag on `_BaseSource` with a safe default
     (like `supports_column_selection`, `connection_host_fields`, `has_managed_insightsql_schema`).
     Branch on the flag.
...
2. **Direct-query engine behaviour** (how a SQL engine resolves a table location, builds and
   ...
   `insights/insightsql/direct_sql/` adapter registry; the warehouse-side materialization
...
(`DataWarehouseTable` rows, managed viewsets, script functions) stays here, keyed off what the
