Get one batch import (managed migration) job by id, from any Insights team, including the raw worker `state` and `import_config` blobs. Insights staff only: the backend requires a staff user AND a personal API key explicitly carrying the `batch_import_support:read` scope (a full-access `*` key is rejected; OAuth sign-in can never grant it).
