...
from products.review.backend.models import ReviewSkillConfig
from products.review.backend.reviewer.lazy_seed import (
    REVIEW_FN_SKILL_CATEGORY,
...
def test_discover_finds_the_validation_criteria_skill() -> None:
    ...
    assert {s.name for s in discover_canonical_validation()} == {REVIEW_FN_VALIDATION_SKILL_NAME}
...
class TestSyncCanonicalValidation(BaseTest):
    def test_creates_a_seeded_row_in_the_validation_category(self) -> None:
        ...
        assert result.created_skill_names == (REVIEW_FN_VALIDATION_SKILL_NAME,)
        row = LLMSkill.objects.get(team=self.team, name=REVIEW_FN_VALIDATION_SKILL_NAME, is_latest=True)
        # Both review-script skill sets share one category (one "Code review" tab); a wrong category
        # strands the skill — categorized skills are hidden from the default tab.
        assert row.category == REVIEW_FN_SKILL_CATEGORY
    ...
    def test_leaves_team_edited_row_alone(self) -> None:
        ...
        edited = LLMSkill.objects.get(team=self.team, name=REVIEW_FN_VALIDATION_SKILL_NAME, is_latest=True)
        edited.body = "the team rewrote the validation bar"
        edited.save(update_fields=["body"])

        result = sync_canonical_validation(self.team)

        assert REVIEW_FN_VALIDATION_SKILL_NAME in result.diverged_skill_names
        assert REVIEW_FN_VALIDATION_SKILL_NAME not in result.updated_skill_names
        latest = LLMSkill.objects.get(team=self.team, name=REVIEW_FN_VALIDATION_SKILL_NAME, is_latest=True)
...
class TestRegisterMissingValidationConfig(BaseTest):
    def test_seeds_the_canonical_validator_enabled_and_is_idempotent(self) -> None:
        ...
        rows = ReviewSkillConfig.objects.for_team(self.team.id).filter(
            user_id=self.user.id, skill_name__startswith=REVIEW_FN_VALIDATION_PREFIX
        )
    ...
    def test_does_not_re_enable_the_disabled_canonical(self) -> None:
        ...
        ReviewSkillConfig.objects.for_team(self.team.id).filter(
            user_id=self.user.id, skill_name=REVIEW_FN_VALIDATION_SKILL_NAME
...
class TestLoadValidationSkillForRun(BaseTest):
    def test_cold_user_gets_the_canonical_pinned(self) -> None:
        ...
        assert loaded.skill_name == REVIEW_FN_VALIDATION_SKILL_NAME
    ...
    def test_pins_the_team_edited_version(self) -> None:
        ...
        publish_skill_version(
            self.team,
            user=self.user,
            skill_name=REVIEW_FN_VALIDATION_SKILL_NAME,
        ...
        pinned = LLMSkill.objects.get(team=self.team, name=REVIEW_FN_VALIDATION_SKILL_NAME, version=loaded.version)
    ...
    def test_resolves_the_selected_custom_validator(self) -> None:
        ...
        configs.filter(user_id=self.user.id, skill_name=REVIEW_FN_VALIDATION_SKILL_NAME).update(enabled=False)
    ...
    def test_falls_back_to_canonical_when_the_selected_skill_row_is_missing(self) -> None:
        ...
        configs.filter(user_id=self.user.id, skill_name=REVIEW_FN_VALIDATION_SKILL_NAME).update(enabled=False)
        configs.create(
            team_id=self.team.id, user_id=self.user.id, skill_name=f"{REVIEW_FN_VALIDATION_PREFIX}ghost", enabled=True
        )
    ...
    def test_selection_is_per_user(self) -> None:
        ...
        assert theirs.skill_name == REVIEW_FN_VALIDATION_SKILL_NAME
...
class TestColdStartSyncSeedsValidation(BaseTest):
    def test_cold_start_sync_seeds_the_validation_skill(self) -> None:
        ...
        assert LLMSkill.objects.filter(team=self.team, name=REVIEW_FN_VALIDATION_SKILL_NAME, is_latest=True).exists()
