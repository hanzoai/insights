...
export function syncCopy({
    ...
    if (phase === 'completed') {
        const headline = selfDriving ? 'Self-driving is on' : 'Insights is wired up'
        if (prUrl) {
            return {
                headline,
                subtitle: prMerged
    ...
    const headline = selfDriving ? 'Setting up self-driving' : 'Setting up Insights'
    if (phase === 'connecting') {
...
export const UPCOMING_STEPS: Record<InstallationMode, string[]> = {
    ...
    local: ['Detecting your framework', 'Installing the Insights SDK', 'Wiring up event capture'],
}
