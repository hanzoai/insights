...
from insights.insightsql.ast import AST_CLASSES, InsightsQLXAttribute, InsightsQLXTag, UUIDType
from insights.insightsql.base import _VISIT_NAME_REPLACEMENTS, AST, camel_case_pattern
...
from insights.insightsql.visitor import CloningVisitor, TraversingVisitor, Visitor
...
class TestVisitor(BaseTest):
    ...
    def test_unknown_error_visitor(self):
        ...
        with self.assertRaises(InternalInsightsQLError) as e:
    ...
    def test_insightsql_exception_start_end(self):
        class EternalVisitor(TraversingVisitor):
            def visit_constant(self, node: ast.Constant):
                if node.value == 616:
                    raise InternalInsightsQLError("You tried accessing a forbidden number, perish!")
    ...
    def test_insightsql_visitor_naming_exceptions(self):
        class NamingCheck(Visitor):
            def visit_uuid_type(self, node: ast.Constant):
                return "visit_uuid_type"

            def visit_insightsqlx_tag(self, node: ast.Constant):
                return "visit_insightsqlx_tag"

            def visit_insightsqlx_attribute(self, node: ast.Constant):
                return "visit_insightsqlx_attribute"
        ...
        assert NamingCheck().visit(InsightsQLXAttribute(name="a", value="a")) == "visit_insightsqlx_attribute"
        assert NamingCheck().visit(InsightsQLXTag(kind="", attributes=[])) == "visit_insightsqlx_tag"
    ...
    def test_accept_raises_when_no_visit_method_and_no_unknown(self):
        ...
        with self.assertRaises(InsightsQLNotImplementedError) as ctx:
