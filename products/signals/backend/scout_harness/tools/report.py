...
class ReviewerInput:
    ...
    supplied `github_login` when both are given), so a scout that only knows a Insights user — e.g.
...
def _build_suggested_reviewers(
    ...
    *,
    ...
    only knows a Insights user can still route a report. Resolution is fail-loud: a `user_uuid` that
...
# Insights's internal analytics via the `hanzo_insights` SDK. Landing them in the team's own event stream
# lets a team act on its scout reports with no Insights-side wiring: InsightsQL/insights/alerts over the events,
# or a CDP destination (e.g. the Slack destination) filtering on the event and templating off `report_url`
# / `title` / `summary`. The `$` prefix marks a Insights-generated event (cf. `$session_summary_ready`,
...
def _forward_report_event_to_team(*, team: Team, forward: _ReportForward) -> None:
    ...
    `capture_internal` path, so the team can drive InsightsQL / alerts / CDP destinations off its reports.
