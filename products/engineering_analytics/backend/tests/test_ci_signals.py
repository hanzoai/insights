...
TEST_BUCKET = "test_storage_bucket-insights.products.engineering_analytics.signals"
...
def _run_row(
    ...
    *,
    ...
    return {
        ...
        "pull_requests": None,
        "repository": json.dumps({"full_name": "Insights/insights", "default_branch": default_branch}),
    }
...
def test_detect_all_raises_when_every_detector_fails() -> None:
    # A Datastore outage failing all three detectors must fail the activity so it retries and
    # alerts, not read as healthy CI with zero findings; one bad detector still must not
    # suppress the others' findings.
    curated = mock.Mock()
    boom = RuntimeError("datastore down")
...
class TestDetectForSourceMultiRepo(BaseTest):
    ...
    def test_one_repos_total_failure_does_not_abort_sibling_repos(self) -> None:
        ...
        def one_repo_fails(curated: CuratedGitHubSource) -> list[CISignalFinding]:
            if curated.repository.casefold() == "acme/one":
                raise RuntimeError("datastore down for this repo")
            return [
                CISignalFinding(
                    source_type=SOURCE_TYPE_FLAKY_CHECK,
                    source_id=f"{curated.repository}:ci:flaky",
    ...
    def test_all_repos_failing_re_raises_so_the_activity_retries(self) -> None:
        ...
        source = self._two_synced_repo_source()
        uac = UserAccessControl(user=self.user, team=self.team)
        with mock.patch(f"{_DETECT}.detect_all", side_effect=RuntimeError("datastore down")):
            with pytest.raises(RuntimeError):
...
class TestCISignalSourceAuthorization(BaseTest):
    ...
    def test_rollout_flag_is_evaluated_as_the_enabling_user(self) -> None:
        ...
        with mock.patch("hanzo_insights.feature_enabled", return_value=True) as flag_check:
...
class TestCISignalDetectors(DatastoreTestMixin, BaseTest):
    ...
    def test_flaky_check_emits_one_signal_per_job_per_week_not_per_rerun(self) -> None:
        ...
        # flaky jobs into 905 signals against real Insights/insights data: one card per occurrence.
        now = datetime.now(UTC).replace(tzinfo=None)
        rows = [
