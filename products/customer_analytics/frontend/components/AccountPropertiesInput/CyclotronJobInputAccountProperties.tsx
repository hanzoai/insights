...
export default function CyclotronJobInputAccountProperties({
    ...
    // Keys are custom property definition ids (stable across renames); values are Script expressions
            ...
            {entries.map(([id, hogValue]) => {
                ...
                return (
                    <div className="flex gap-2 items-center" key={id}>
                                ...
                                options={(definition.options ?? []).map((option) => ({
                                    ...
                                    label: (
                                        <span className="inline-flex items-center gap-1.5">
                                            <ColorGlyph colorToken={option.color as DataColorToken} size="small" />
                                            {option.label}
                                        </span>
                                    ),
                                }))}
                        ...
                        <Button icon={<IconX />} size="small" onClick={() => removeProperty(id)} />
                    </div>
                )
            })}
