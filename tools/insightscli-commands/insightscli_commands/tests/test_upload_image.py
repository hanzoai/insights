"""Tests for insightscli pr:upload-image.
...
from insightscli_commands import github_auth, pr_assets, upload_image
...
def test_command_prints_only_sha_pinned_markdown_to_stdout(png: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert re.fullmatch(
        rf"!\[diagram\]\(https://raw\.githubusercontent\.com/Insights/pr-assets/abc123/{_KEY_RE}\.png\)", line
    )
