...
} from '~/common/groups/repositories/datastore-group-repository'
...
export class BatchWritingGroupStore implements GroupStore {
    ...
    /** Datastore messages queued by create/fallback paths, drained by flush(). */
    ...
    constructor(
        ...
        this.datastoreGroupRepository = datastoreGroupRepository
    ...
    private async createGroupIndividually(pendingCreate: PendingGroupWrite): Promise<GroupFlushResult | null> {
        ...
        try {
            ...
            return {
                messages: [
                    this.datastoreGroupRepository.buildUpsertMessage(
                        update.team_id,
                        update.group_type_index,
                        update.group_key,
    ...
    private async createGroup(
        ...
        // The Datastore message rides the next flush's side effects instead
    ...
    /**
     * Queue a group's Datastore message for the next flush, which returns it
    ...
    private async executeOptimisticUpdate(update: GroupUpdate): Promise<GroupDatastoreMessage | null> {
        ...
        if (actualVersion !== undefined) {
            ...
            // The Datastore produce is returned to the caller and scheduled as
            // a side effect after flush, so flush latency stays DB-only.
            return this.datastoreGroupRepository.buildUpsertMessage(
                update.team_id,
                update.group_type_index,
                update.group_key,
                update.group_properties,
        ...
        if (latestGroup) {
            ...
            if (!propertiesUpdate.updated && update.created_at >= latestGroup.created_at) {
                ...
                // skip the write (and its Datastore produce) entirely.
