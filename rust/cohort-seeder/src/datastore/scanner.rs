//! The streaming Datastore scanner: drives one chunk's query, folds rows through the shared
...
use chrono::Utc;
use chrono_tz::Tz;
use cohort_core::datastore_timestamp_to_millis;
use cohort_core::day_idx_in_tz;
use cohort_core::events::CohortStreamEvent;
use cohort_core::scriptvm::VmErrorClass;
...
impl ChunkScanner {
    pub fn new(client: datastore::Client) -> Self {
        Self { client }
    }
...
fn fold_event(
    ...
    let Some(timestamp_ms) = datastore_timestamp_to_millis(&event.timestamp) else {
...
pub enum ScanError {
    ...
    #[error("building Datastore scan cursor")]
    Query(#[source] datastore::error::Error),
    #[error("streaming Datastore scan cursor")]
    Cursor(#[source] datastore::error::Error),
    #[error("aggregating Datastore scan row")]
