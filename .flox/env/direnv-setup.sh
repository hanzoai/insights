...
if ! grep -q "direnv hook" "$config_file" 2>/dev/null; then
    echo -e "\n# Initialize direnv - added by Insights's Flox activation hook (../insights/.flox/env/manifest.toml)\n$hook_command" >> "$config_file"
...
if ! grep -q "warn_timeout" "$direnv_config_file" 2>/dev/null; then
    echo -e "[global]\nwarn_timeout = 0 # Ignore timeout from this issue: https://github.com/direnv/direnv/issues/1065 - added by Insights's Flox activation hook (../insights/.flox/env/manifest.toml)" >> "$direnv_config_file"
