...
class TestComputeSdkHealth(SimpleTestCase):
    ...
    def test_legacy_java_is_reported_with_migration_guidance_without_a_version(self):
        ...
        assert "properties.$lib = 'insights-java'" in release.sql_query
...
class TestUiParityStrings(SimpleTestCase):
    ...
    def test_sql_query_matches_ui_template_exact(self):
        ...
        sql = _build_sql_query("insights-node", "1.230.1")
        expected = (
            "SELECT * FROM events WHERE timestamp >= NOW() - INTERVAL 7 DAY "
            "AND properties.$lib = 'insights-node' AND properties.$lib_version = '1.230.1' "
    ...
    def test_banner_matches_ui_copy(self):
        banner = _build_banner("insights-python", OutdatedTrafficAlert(version="7.0.0", threshold_percent=10.0))
    ...
    def test_banner_threshold_percent_rounds_near_integer_floats(self):
        ...
        below = _build_banner("insights-python", OutdatedTrafficAlert(version="7.0.0", threshold_percent=9.9999999))
        above = _build_banner("insights-python", OutdatedTrafficAlert(version="7.0.0", threshold_percent=10.0000001))
    ...
    @parameterized.expand(
        [
            ("sql_quote_escape", "insights-node", "1.0.0'; DROP TABLE events;--"),
            ("sql_union", "insights-node", "1.0.0' UNION SELECT password FROM users--"),
            ("sql_line_break", "insights-node", "1.0.0\n; DROP TABLE"),
            ("sql_trailing_space", "insights-node", "1.0.0 OR 1=1"),
            ("sql_semicolon", "insights-node", "1.0.0;SELECT"),
            ("lib_type_quote", "insights-node'; --", "1.0.0"),
            ("lib_type_space", "insights node", "1.0.0"),
            ("unicode_fullwidth", "insights-node", "１.０.０"),
            ("empty_version", "insights-node", ""),
    ...
    def test_sql_query_accepts_prerelease_suffix(self):
        ...
        sql = _build_sql_query("insights-node", "1.2.3-beta")
    ...
    def test_sql_query_accepts_build_metadata(self):
        ...
        sql = _build_sql_query("insights-node", "1.2.3+build.1")
...
class TestMobileGracePeriod(SimpleTestCase):
    ...
    @parameterized.expand(
        [
            ...
            ("flutter", "insights-flutter"),
            ("react_native", "insights-react-native"),
        ]
    )
...
class TestAssessReleasePopulatesUiFields(SimpleTestCase):
    def test_outdated_release_has_tooltip_sql_and_url(self):
        ...
        assert "insights-node" in result.sql_query
...
class TestAssessSdkBanners(SimpleTestCase):
    ...
    def test_no_banners_when_no_traffic_alerts(self):
        ...
        result = assess_sdk("insights-node", "1.5.0", entries, now=NOW)
        assert result is not None
...
class TestPromptInjectionDefense(SimpleTestCase):
    ...
    def test_crafted_lib_version_never_appears_in_assess_sdk_output(self, _name, payload):
        ...
        result = assess_sdk("insights-node", "2.0.0", entries, now=NOW, project_id=7)
        assert result is not None
    ...
    def test_defense_in_depth_build_reason_with_raw_unsafe_version(self):
        ...
        result = _build_reason("insights-node", poisoned, "2.0.0", [])
