...
from insights.schema import BreakdownType, MultipleBreakdownType, PropertyMathType, TrendsQuery

from insights.insightsql_queries.insights.trends.aggregation_operations import SUPPORTED_PROPERTY_MATH_FOR_HISTOGRAM_BREAKDOWN
...
from insights.insightsql_queries.validation.utils import get_query_insight_name
...
class ValidateDataWarehouseBreakdown:
    ...
    def validate(self, context: QueryValidationContext[TrendsQuery]) -> None:
        ...
        # `insightsql` breakdowns resolve against the FROM clause, which for a `DataWarehouseNode`
        # series is the warehouse table itself — so they're safe alongside `data_warehouse`.
        supported_multi_types = {MultipleBreakdownType.DATA_WAREHOUSE, MultipleBreakdownType.INSIGHTSQL}
        supported_single_types = {BreakdownType.DATA_WAREHOUSE, BreakdownType.INSIGHTSQL}
