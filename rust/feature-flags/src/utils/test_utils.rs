...
pub async fn insert_new_team_in_pg(
    ...
    let id = match team_id {
        ...
        None => {
            ...
            let row: (i32,) = sqlx::query_as("SELECT nextval('insights_team_id_seq')::int")
...
pub async fn insert_evaluation_tags_for_flag_in_pg(
    ...
    for tag_name in tag_names {
        ...
        let ctx_id: Uuid = sqlx::query_scalar(
            r#"
            INSERT INTO insights_evaluationcontext (id, name, team_id, created_at)
            VALUES ($1, $2, $3, NOW())
        ...
        sqlx::query(
            r#"
            INSERT INTO insights_featureflagevaluationcontext (id, feature_flag_id, evaluation_context_id, created_at)
            VALUES ($1, $2, $3, NOW())
...
pub async fn insert_experiment_for_flag_in_pg(
    ...
    sqlx::query(
        r#"
        INSERT INTO insights_experiment
...
pub async fn update_team_timezone(
    ...
    let mut conn = client.get_connection().await?;
    sqlx::query("UPDATE insights_team SET timezone = $1 WHERE id = $2")
        .bind(timezone)
        .bind(team_id)
        .execute(&mut *conn)
        .await?;
...
impl TestContext {
    ...
    pub async fn create_project_secret_api_key(
        ...
        sqlx::query(
            r#"INSERT INTO insights_projectsecretapikey
    ...
    pub async fn populate_cache_for_team_with_etag(
        ...
        let etag_key =
            format!("insights:1:cache/teams/{team_id}/feature_flags/flags_with_cohorts.json:etag");
        let pickled_etag =
    ...
    pub async fn remove_user_from_organization(
        ...
        let mut conn = self.non_persons_writer.get_connection().await?;
        sqlx::query(
            "DELETE FROM insights_organizationmembership WHERE user_id = $1 AND organization_id = $2",
        )
        .bind(user_id)
        .bind(org_id)
        .execute(&mut *conn)
    ...
    /// Sets a value in the insights_instancesetting table (upsert).
    ...
    pub async fn set_instance_setting(&self, key: &str, value: &str) -> Result<(), Error> {
        let mut conn = self.non_persons_writer.get_connection().await?;
        let full_key = format!("constance:insights:{key}");
        sqlx::query(
            "INSERT INTO insights_instancesetting (key, raw_value)
             VALUES ($1, $2)
    ...
    /// Deletes a value from the insights_instancesetting table.
    pub async fn delete_instance_setting(&self, key: &str) -> Result<(), Error> {
        let mut conn = self.non_persons_writer.get_connection().await?;
        let full_key = format!("constance:insights:{key}");
        sqlx::query("DELETE FROM insights_instancesetting WHERE key = $1")
            .bind(&full_key)
            .execute(&mut *conn)
            .await?;
        Ok(())
...
/// (`insights:auth_token:{sha256_hash}`). This helper hashes the raw token/key
