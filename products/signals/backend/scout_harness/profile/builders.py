...
from insights.models.product_intent.product_intent import ProductIntent
...
# Top-events Datastore query bounds. 7d is short enough to spot recent bursts and long
...
# than burning a full minute of Datastore on a best-effort orientation query.
...
def _recent_dashboards(team: Team) -> list[dict[str, Any]]:
    ...
    give us counts but only for Insights-internal teams via project 2), so this surfaces
...
def _recent_surveys(team: Team) -> dict[str, Any]:
    ...
    "Active" matches Insights's running semantics: not archived, already started
...
def _recent_insights_functions(team: Team) -> dict[str, Any]:
    """Script function orientation — total + enabled, plus 5 most-recently-modified.
    ...
    surface" work.
    """
    qs = InsightsFunction.objects.filter(team=team, deleted=False)
    total = qs.count()
    enabled = qs.filter(enabled=True).count()
    recent = qs.order_by("-updated_at")[:RECENT_ENTITY_LIMIT].values(
...
def _recent_hog_flows(team: Team) -> dict[str, Any]:
    ...
    """
    qs = InsightsFlow.objects.filter(team=team)
    total = qs.count()
...
def _top_events(team: Team) -> list[dict[str, Any]] | None:
    ...
    try:
        response = execute_insightsql_query(
            ...
            team=team,
            limit_context=None,
            settings=InsightsQLGlobalSettings(max_execution_time=TOP_EVENTS_MAX_EXECUTION_S),
        )
    except Exception:
        # Defensive: Datastore can be slow or unavailable. Profile build shouldn't
