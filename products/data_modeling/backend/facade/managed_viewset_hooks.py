...
class ProvidedView:
    """One view a product wants data_modeling to manage. ``query`` is the raw InsightsQL SELECT body —
    data_modeling wraps it into ``{"kind": "InsightsQLQuery", "query": ...}`` itself. ``fields`` are InsightsQL
