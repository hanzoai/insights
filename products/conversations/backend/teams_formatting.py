"""Convert between Microsoft Teams HTML and Insights rich_content format."""
...
def teams_html_to_content_and_rich_content(html_text: str) -> tuple[str, JSON | None]:
    """
    Convert Teams HTML message content to Insights content (plain text) and rich_content.
    ...
    doc structure for Insights's rich content model.
    """
    if not html_text:
        return "", None
...
def rich_content_to_teams_html(rich_content: JSON | None, fallback_content: str = "") -> str:
    """
    Convert Insights rich_content to HTML suitable for Teams Bot Framework replies.
...
def append_teams_attribution(reply_html: str, author_name: str) -> str:
    """Append a subtle "{name} via Support" footer to an outbound Teams reply.

    A Teams bot's display name is fixed in the app manifest, so every reply otherwise
    shows up from a nameless "Support" and customers can't tell which agent answered.
    ...
    if not author_name:
        return reply_html
    footer = f"<p><i>{html_mod.escape(author_name)} via Support</i></p>"
