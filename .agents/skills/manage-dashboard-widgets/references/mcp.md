...
1. Regenerate OpenAPI (`insightscli build:openapi`) — updates MCP tool schemas that embed supported type lists
2. Run `insightscli test services/mcp/tests/tools/dashboards.integration.test.ts`
3. Update unit schema snapshots if batch-add help text lists types: `insightscli test services/mcp/tests/unit/tool-schema-snapshots.test.ts`
