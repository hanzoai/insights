...
from insights.datastore.cluster import DatastoreCluster, LightweightDeleteMutationRunner
from insights.dags.data_deletion_requests import (
...
@pytest.mark.django_db
def test_execute_event_deletion_deletes_matching_events(cluster: DatastoreCluster):
    now = datetime.now()
...
@pytest.mark.django_db
def test_execute_event_deletion_delete_all_events_drops_every_event_for_team(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
...
@pytest.mark.django_db
def test_execute_event_deletion_applies_insightsql_predicate(cluster: DatastoreCluster):
    from insights.models.organization import Organization
    from insights.models.team import Team

    org = Organization.objects.create(name="test-org-insightsql")
    team = Team.objects.create(organization=org, name="test-team-insightsql")
    ...
    deletion_ctx = DeletionRequestContext(
        ...
        end_time=end_time,
        events=["$pageview"],
        insightsql_predicate="properties.$browser = 'Chrome'",
    )
    context = build_op_context()
    execute_event_deletion(context, cluster, deletion_ctx)
...
@pytest.mark.django_db
def test_execute_event_deletion_multiple_event_names(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
    end_time = now + timedelta(minutes=1)
...
@pytest.mark.django_db
def test_full_job_event_deletion(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
    end_time = now + timedelta(minutes=1)
...
@pytest.mark.django_db
def test_execute_event_deletion_deferred_queues_into_adhoc_table(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
    end_time = now + timedelta(minutes=1)
...
@pytest.mark.django_db
def test_full_job_event_deletion_deferred(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
    end_time = now + timedelta(minutes=1)
...
def test_verify_queued_request_promotes_when_events_gone():
    from insights.models.data_deletion_request import verify_queued_request
...
@pytest.mark.django_db
def test_verify_queued_request_keeps_status_when_events_remain(cluster: DatastoreCluster):
    from insights.models.data_deletion_request import verify_queued_request
...
@pytest.mark.django_db
def test_verify_queued_job_promotes_in_window_and_skips_old(cluster: DatastoreCluster):
    from django.utils import timezone

    from insights.dags.data_deletion_requests import verify_queued_deletion_requests_job
...
def test_verify_queued_job_registered_in_datastore_location():
    from insights.dags.locations.datastore import defs
...
@pytest.mark.django_db
def test_auto_approve_job_approves_small_pending_event_removal(cluster: DatastoreCluster):
    cluster.any_host(_truncate_writable_events).result()
    now = datetime.now()
...
@pytest.mark.django_db
def test_auto_approve_job_ignores_request_that_opted_out(cluster: DatastoreCluster):
    cluster.any_host(_truncate_writable_events).result()
    ...
    # requests the job can never act on costs in Datastore queries.
    assert request.count is None
    assert request.stats_calculated_at is None


@pytest.mark.django_db
def test_auto_approve_job_leaves_oversized_request_pending_but_refreshes_its_stats(cluster: DatastoreCluster):
    cluster.any_host(_truncate_writable_events).result()
    now = datetime.now()
    events = [(AUTO_APPROVE_TEAM_ID, "$pageview", uuid4(), now - timedelta(hours=i + 1)) for i in range(3)]
    cluster.any_host(partial(_insert_events, events)).result()
    ...
    with patch("insights.models.data_deletion_request.AUTO_APPROVE_MAX_EVENTS", 2):
        result = auto_approve_deletion_requests_job.execute_in_process()
    assert result.success
...
@pytest.mark.django_db
def test_auto_approve_job_skips_a_request_it_cannot_measure(cluster: DatastoreCluster):
    cluster.any_host(_truncate_writable_events).result()
    now = datetime.now()
    ...
    broken = _pending_event_removal(insightsql_predicate="this is not ) valid insightsql")
...
@pytest.mark.django_db
def test_auto_approve_sweep_does_not_let_stuck_requests_starve_new_ones(cluster: DatastoreCluster):
...
def test_auto_approve_job_registered_in_datastore_location():
    from insights.dags.locations.datastore import defs
...
@pytest.mark.django_db
def test_full_job_person_property_removal(cluster: DatastoreCluster):
...
@pytest.mark.django_db
def test_full_job_both_properties_and_person_properties(cluster: DatastoreCluster):
...
@pytest.mark.django_db
def test_full_job_person_property_removal_ignores_out_of_range_events(cluster: DatastoreCluster):
...
@pytest.mark.django_db
def test_full_job_person_property_removal_ignores_other_event_names(cluster: DatastoreCluster):
...
def _add_person_property_mat_columns(col_defs: list[tuple[str, str, str]], client: Client) -> None:
    ...
    db = settings.DATASTORE_DATABASE
...
@pytest.mark.django_db
def test_full_job_property_removal(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
    end_time = now + timedelta(minutes=1)
...
@pytest.mark.django_db
def test_full_job_property_removal_single_property(cluster: DatastoreCluster):
    now = datetime.now()
    start_time = now - timedelta(days=7)
    end_time = now + timedelta(minutes=1)
...
@pytest.mark.django_db
def test_full_job_property_removal_applies_insightsql_predicate(cluster: DatastoreCluster):
    """A property removal scoped by insightsql_predicate must clean only matching events."""
    from insights.models.organization import Organization
    from insights.models.team import Team

    org = Organization.objects.create(name="test-org-prop-insightsql")
    team = Team.objects.create(organization=org, name="test-team-prop-insightsql")
...
def _add_default_mat_columns(col_defs: list[tuple[str, str, str]], client: Client) -> None:
    ...
    ee/datastore/materialized_columns/columns.py.
...
@pytest.mark.django_db
def test_full_job_property_removal_leaves_events_ingested_after_marker_untouched(cluster: DatastoreCluster):
    from django.utils import timezone
...
@pytest.mark.django_db
def test_full_job_property_removal_rerun_after_delete_failure_does_not_duplicate(cluster: DatastoreCluster):
    now = datetime.now()
    props = json.dumps({"secret": "value", "keep": "yes"})
...
@pytest.mark.django_db
def test_full_job_property_removal_fails_on_residual_duplicates(cluster: DatastoreCluster):
    from django.utils import timezone
...
@pytest.mark.django_db
def test_single_shard_op_reexecution_completes_failed_request(cluster: DatastoreCluster):
    now = datetime.now()
    props = json.dumps({"secret": "value", "keep": "yes"})
    events = [(PROP_TEAM_ID, "$pageview", uuid4(), now - timedelta(hours=i + 1), props) for i in range(20)]
    cluster.any_host(partial(_insert_events_with_properties, events)).result()
    ...
    ctx = DeletionRequestContext(
        ...
        insightsql_predicate=request.insightsql_predicate or "",
...
@pytest.mark.django_db
def test_delete_person_events_op_runs_lightweight_delete_per_shard(cluster: DatastoreCluster):
...
@pytest.mark.django_db
def test_delete_person_events_op_noop_when_disabled(cluster: DatastoreCluster):
    ctx = PersonRemovalContext(
        request_id=str(uuid4()),
        team_id=TEAM_ID,
        person_uuids=[str(uuid4())],
...
@pytest.mark.django_db
def test_delete_person_events_op_resolves_distinct_ids_to_uuids(cluster: DatastoreCluster):
...
def test_delete_person_recordings_op_calls_helper_when_enabled():
    ...
    with patch("insights.dags.data_deletion_requests.queue_person_recording_deletion") as queue:
...
def test_delete_person_profiles_op_calls_helper_when_enabled():
    ...
    with patch("insights.dags.data_deletion_requests.delete_persons_profile") as deleter:
...
@pytest.mark.django_db
def test_data_deletion_request_person_removal_lifecycle(cluster: DatastoreCluster):
    p_uuid = str(uuid4())
    create_person(team_id=TEAM_ID, uuid=p_uuid, distinct_ids=["a"])
    request = DataDeletionRequest.objects.create(
        team_id=TEAM_ID,
    ...
    with (
        patch("insights.dags.data_deletion_requests.delete_persons_profile") as profile,
        patch("insights.dags.data_deletion_requests.queue_person_recording_deletion"),
    ):
