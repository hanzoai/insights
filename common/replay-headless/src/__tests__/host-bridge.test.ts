...
describe('HostBridge', () => {
    ...
    describe('waitForStart', () => {
        it('resolves when insights-player-start event fires', async () => {
            const promise = bridge.waitForStart()
            window.dispatchEvent(new Event('insights-player-start'))
        ...
        it('rejects on timeout', async () => {
            ...
            await expect(promise).rejects.toThrow('insights-player-start not received within 0.1s')
            jest.useRealTimers()
        })
    })
})
