...
from insights.insightsql_queries.insights.lifecycle.lifecycle_validation_rules import (
...
from insights.insightsql_queries.utils.query_date_range import QueryDateRange, compare_interval_length
...
class LifecycleQueryRunner(AnalyticsQueryRunner[LifecycleQueryResponse]):
            ...
            if day is not None:
                exprs.append(
                    ast.CompareOperation(
                        ...
                        right=self.query_date_range.date_to_start_of_interval_insightsql(
    ...
    def format_results(self, response: InsightsQLQueryResponse) -> list[dict[str, Any]]:
        """Format raw lifecycle InsightsQL response into status-grouped series.
        ...
        # Each status is grouped independently in Datastore, and groupArray does not guarantee that the
            ...
            events_query = parse_select(
                ...
                placeholders={
                    **self.query_date_range.to_placeholders(),
                    ...
                    "trunc_timestamp": self.query_date_range.date_to_start_of_interval_insightsql(self.timestamp_field),
                    "trunc_created_at": self.query_date_range.date_to_start_of_interval_insightsql(
                        ast.Field(chain=["created_at"])
                    ),
                    "trunc_epoch": self.query_date_range.date_to_start_of_interval_insightsql(
    ...
    def periods_query(self):
        with self.timings.measure("periods_query"):
            periods_query = parse_select(
                ...
                placeholders={
                    **self.query_date_range.to_placeholders(),
                    "date_to_plus_interval": self.query_date_range.date_to_start_of_interval_insightsql(
                        parse_expr(
