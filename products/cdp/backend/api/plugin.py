...
from insights.models.activity_logging.serializers import ActivityLogSerializer
...
from insights.plugins import can_configure_plugins, can_install_plugins, parse_url
from insights.plugins.access import can_globally_manage_plugins, has_plugin_access_level
from insights.plugins.plugin_server_api import populate_plugin_capabilities_on_workers
...
class PluginSerializer(serializers.ModelSerializer):
    ...
    insights_function_migration_available = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            "metrics",
            "public_jobs",
            "insights_function_migration_available",
        ]
...
class PluginViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def repository(self, request: request.Request, **kwargs):
        url = "https://raw.githubusercontent.com/Insights/integrations-repository/main/plugins.json"
...
class PluginConfigSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data: dict, *args: Any, **kwargs: Any) -> PluginConfig:
        ...
        # Try and create a script function if possible, otherwise create plugin
        from insights.cdp.legacy_plugins import insights_function_from_plugin_config
        from insights.event_usage import report_team_action
        ...
        try:
            insights_function_serializer = insights_function_from_plugin_config(validated_data, self.context)

            insights_function = insights_function_serializer.create(insights_function_serializer.validated_data)
            # A bit hacky - we return the non saved plugin config

            report_user_action(
                self.context["request"].user,
                "script function created from plugin config api",
                {
            ...
            # Return plugin config without saving if script function was created successfully
            return PluginConfig(**validated_data)

        except Exception as e:
            # If anything goes wrong with script function creation, capture the error but continue with plugin creation
            capture_exception(e)
            raise ValidationError(
                "Plugin creation is no longer possible. Please refer to the Script Functions documentation for more information."
            )
...
class PluginConfigViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def migrate(self, request: request.Request, **kwargs):
        ...
        insights_function_data = migrater.migrate(obj)
        insights_function_data["template_id"] = insights_function_data["id"]
        del insights_function_data["id"]

        if obj.enabled:
            insights_function_data["enabled"] = True

        insights_function_serializer = InsightsFunctionSerializer(data=insights_function_data, context=self.get_serializer_context())
        insights_function_serializer.is_valid(raise_exception=True)
        insights_function_serializer.save()

        if obj.enabled:
            obj.enabled = False
            obj.save()

        return Response(insights_function_serializer.data)
