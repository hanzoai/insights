...
from insights.session_recordings.ai_summary_cap import (
...
class TestRedisKey(BaseTest):
    def test_format(self):
        ...
        assert key == "insights/replay-summary-cap:42:2026-05"
        assert re.fullmatch(r"insights/replay-summary-cap:\d+:\d{4}-\d{2}", key)
    ...
    def test_uses_utc_when_now_omitted(self):
        ...
        with patch("insights.session_recordings.ai_summary_cap.datetime") as mock_dt:
...
class TestAtomicCheckAndConsume(BaseTest):
    ...
    def test_atomic_check_and_consume_is_overshoot_free(self):
        ...
        with patch("insights.session_recordings.ai_summary_cap.get_cap_for_team", return_value=3):
...
class TestRefund(BaseTest):
    ...
    def test_refund_swallows_redis_errors(self):
        ...
        with patch("insights.session_recordings.ai_summary_cap.get_client") as mock_client:
