...
from insights.event_usage import AGENT_EVENT_SOURCES, EventSource, get_event_source, report_user_action
...
# (nodejs/src/cdp/services/insightsflows/actions/delay.ts) that throws at runtime on mismatch.
...
def snapshot_flow_content(flow: InsightsFlow) -> dict:
...
# input key), mirroring InsightsFunction.encrypted_inputs. The worker re-merges them at execution time.
...
def _function_template_for_action(action: dict, template_cache: Optional[TemplateCache] = None) -> Optional[Any]:
    ...
    if template_cache is None:
        return InsightsFunctionTemplate.get_template(template_id)
    if template_id not in template_cache:
        template_cache[template_id] = InsightsFunctionTemplate.get_template(template_id)
...
def mask_trigger_config(
    instance: "InsightsFlow", secrets_by_action: dict[str, dict], template_cache: Optional[TemplateCache] = None
) -> Any:
...
# deeper in InsightsQL property compilation. Reject it up front with a clear 400 instead.
...
def _should_validate_strictly(context: dict, is_draft: Optional[bool]) -> bool:
    ...
    # only re-persist already-accepted data. Programmatic authoring clients (MCP, insights-code, raw API)
...
class InsightsFlowConfigFunctionInputsSerializer(serializers.Serializer):
...
class InsightsFlowEdgeSerializer(serializers.Serializer):
...
# rejects anything.
_FN_FLOW_WAIT_UNTIL_EVENT_SCHEMA = {
    "type": "object",
    "properties": {
        "filters": {
            "anyOf": [{"$ref": "#/components/schemas/InsightsFunctionFilters"}, {"type": "null"}],
            "description": (
    ...
    "anyOf": [
        ...
        },
        {
            ...
            "properties": {
                ...
                "events": {
                    "type": "array",
                    "items": _FN_FLOW_WAIT_UNTIL_EVENT_SCHEMA,
                    "description": (
...
class InsightsFlowActionTypeField(serializers.ChoiceField):
    ...
    def to_internal_value(self, data: Any) -> str:
        ...
        raise serializers.ValidationError(
            ...
            "Steps that act on Insights data don't get a type of their own - they are type 'function' "
    ...
    def _hint(data: Any) -> str:
        ...
        if "person" in lowered or "propert" in lowered or "contact" in lowered:
            return (
                " To set person properties, use template_id 'template-insights-update-person-properties' "
...
class InsightsFlowActionSerializer(serializers.Serializer):
    ...
    config = InsightsFlowActionConfigField(
        help_text=(
            ...
            "script templating ({person.x}, {event.x}); flat strings won't interpolate. "
    ...
    def validate(self, data):
        ...
        if data.get("type") == "trigger":
            if data.get("config", {}).get("type") in ["webhook", "manual", "tracking_pixel"]:
                ...
                if filters:
                    serializer = InsightsFunctionFiltersSerializer(data=filters, context=self.context)
                    if not strict:
                ...
                # The schedule definition lives on a separate InsightsFlowSchedule row, but a schedule trigger
        ...
        if "function" in data.get("type", "") or trigger_is_function:
            ...
            template = InsightsFunctionTemplate.get_template(template_id)
            if not template:
                if strict:
                ...
                function_config_serializer = InsightsFlowConfigFunctionInputsSerializer(
                    data={
...
class InsightsFlowVariableSerializer(serializers.ListSerializer):
...
class InsightsFlowMaskingSerializer(serializers.Serializer):
    ...
    hash = serializers.CharField(
        required=True,
        help_text="InsightsQL template defining the dedup/grouping key, e.g. '{person.id}' (once per person) within ttl.",
    )
...
@extend_schema_field(InsightsFunctionFiltersSerializer)
class InsightsFlowConversionEventFiltersField(serializers.JSONField):
    # Schema-typed as InsightsFunctionFilters for codegen, but runtime-lenient: drafts may hold invalid
    # filters, and InsightsFlowSerializer.validate compiles/validates these with draft leniency.
    pass


class InsightsFlowConversionEventSerializer(serializers.Serializer):
    filters = InsightsFlowConversionEventFiltersField(
        help_text=(
...
class InsightsFlowConversionSerializer(serializers.Serializer):
    ...
    events = serializers.ListField(
        child=InsightsFlowConversionEventSerializer(),
        required=False,
...
class InsightsFlowScheduleSerializer(serializers.ModelSerializer):
    class Meta:
        model = InsightsFlowSchedule
        fields = [
            "id",
            "rrule",
            "starts_at",
    ...
    def update(self, instance, validated_data):
        if any(field in validated_data for field in ("rrule", "starts_at", "timezone")):
            ...
            if instance.status != InsightsFlowSchedule.Status.PAUSED:
                instance.status = InsightsFlowSchedule.Status.ACTIVE
...
class AwsTenantReputationSerializer(serializers.Serializer):
    ...
    health = serializers.ChoiceField(
        ...
        help_text=(
            ...
            "tenant's sending is paused). Reflects AWS state only; Insights-initiated suspensions are "
...
class InsightsFlowMinimalSerializer(UserAccessControlSerializerMixin, serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        live_secrets = instance.encrypted_inputs or {} if isinstance(instance, InsightsFlow) else {}
        draft_secrets = instance.draft_encrypted_inputs or {} if isinstance(instance, InsightsFlow) else {}
        ...
        if "trigger" in data and isinstance(instance, InsightsFlow):
...
class InsightsFlowSummarySerializer(InsightsFlowMinimalSerializer):
    ...
    class Meta(InsightsFlowMinimalSerializer.Meta):
        fields = [
            "id",
            "name",
            "description",
...
class InsightsFlowSerializer(InsightsFlowMinimalSerializer):
    ...
    status = serializers.ChoiceField(
        choices=InsightsFlow.State.choices,
        required=False,
        help_text="draft (no execution), active (live), archived (disabled).",
    )
    trigger_masking = InsightsFlowMaskingSerializer(
        required=False,
        allow_null=True,
        help_text=(
            "Optional dedup/throttle on an already-matched trigger: {hash: <InsightsQL template>, "
    ...
    conversion = InsightsFlowConversionSerializer(
        required=False,
        allow_null=True,
        help_text=(
    ...
    exit_condition = serializers.ChoiceField(
        choices=InsightsFlow.ExitCondition.choices,
        required=False,
        help_text=(
    ...
    edges = serializers.ListField(
        child=InsightsFlowEdgeSerializer(),
        required=False,
        help_text=(
    ...
    actions = serializers.ListField(
        child=InsightsFlowActionSerializer(),
        required=True,
    ...
    def to_internal_value(self, data):
        ...
        instance = cast(Optional[InsightsFlow], self.instance) or self.context.get("instance")
    ...
    def validate(self, data):
        instance = cast(Optional[InsightsFlow], self.instance)
        is_draft = self.context.get("is_draft")
        ...
        if status == "active" and instance and instance.status != "active" and "actions" not in data:
            action_serializer = InsightsFlowActionSerializer(data=instance.actions, many=True, context=self.context)
        ...
        if data["trigger"].get("type") == "data-warehouse-table":
            data["exit_condition"] = InsightsFlow.ExitCondition.ONLY_AT_END
            if not is_draft:
        ...
        # drafts — same posture as InsightsFlowActionSerializer, so a conversion filter that can't compile
        ...
        if conversion is not None:
            ...
            if filters:
                serializer = InsightsFunctionFiltersSerializer(data={"properties": filters}, context=self.context)
                if not strict:
                    if serializer.is_valid():
            ...
            for event_config in conversion_events:
                ...
                if event_filters is not None:
                    event_serializer = InsightsFunctionFiltersSerializer(data=event_filters, context=self.context)
                    if not strict:
    ...
    def create(self, validated_data: dict, *args, **kwargs) -> InsightsFlow:
...
# Per-op required fields, validated in InsightsFlowGraphOperationSerializer.validate so a malformed op is
...
class InsightsFlowGraphOperationSerializer(serializers.Serializer):
    ...
    edge = InsightsFlowEdgeSerializer(
...
class InsightsFlowGraphUpdateSerializer(serializers.Serializer):
    ...
    operations = serializers.ListField(
        child=InsightsFlowGraphOperationSerializer(),
        allow_empty=False,
        help_text=(
...
class InsightsFlowActionEmailUpdateSerializer(serializers.Serializer):
    base_updated_at = serializers.DateTimeField(
        required=False,
        help_text=(
            "Optimistic concurrency: the updated_at (or draft_updated_at) last loaded. If the stored "
...
def _render_action_email_operations(
    ...
    for warning in validate_design(new_design):
        logger.info("insights_flow_action_email_design_warning", warning=warning, action_id=action_id)
    try:
...
class InsightsFlowInvocationSerializer(serializers.Serializer):
    configuration = InsightsFlowSerializer(
...
class InsightsFlowPublishRequestSerializer(serializers.Serializer):
...
class InsightsFlowPublishImpactMoveTargetSerializer(serializers.Serializer):
...
class InsightsFlowPublishImpactDeletedStepSerializer(serializers.Serializer):
    ...
    moves_to = InsightsFlowPublishImpactMoveTargetSerializer(
        allow_null=True,
...
class InsightsFlowPublishImpactEmptyVariableSerializer(serializers.Serializer):
...
class InsightsFlowPublishImpactScheduleConflictSerializer(serializers.Serializer):
...
class InsightsFlowPublishImpactSerializer(serializers.Serializer):
    deleted_steps = InsightsFlowPublishImpactDeletedStepSerializer(
        many=True,
    ...
    empty_variables = InsightsFlowPublishImpactEmptyVariableSerializer(
        many=True,
        help_text="Variables that render empty for runs predating their producer.",
    )
    schedule_conflicts = InsightsFlowPublishImpactScheduleConflictSerializer(
        many=True,
        help_text="Schedules overriding variables the draft removes.",
    )


class InsightsFlowPublishResponseSerializer(serializers.Serializer):
...
class InsightsFlowRevisionBasicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = InsightsFlowRevision
        fields = ["version", "created_at", "created_by"]
        read_only_fields = fields


class InsightsFlowRevisionSerializer(InsightsFlowRevisionBasicSerializer):
    class Meta(InsightsFlowRevisionBasicSerializer.Meta):
        fields = [*InsightsFlowRevisionBasicSerializer.Meta.fields, "content"]
        read_only_fields = fields


class InsightsFlowRevisionRestoreRequestSerializer(serializers.Serializer):
...
class InsightsFlowFilterSet(FilterSet):
    class Meta:
        model = InsightsFlow
...
class InsightsFlowPagination(LimitOffsetPagination):
    default_limit = 100
    max_limit = 500
...
class InsightsFlowViewSet(
    ...
    def get_serializer_class(self) -> type[BaseSerializer]:
        if self.action == "list":
            ...
            if self.request is not None and self._is_mcp_request(self.request):
                return InsightsFlowSummarySerializer
            return InsightsFlowMinimalSerializer
        return InsightsFlowSerializer
    ...
    def _emit_resource_edited(self, instance: InsightsFlow) -> None:
    ...
    def perform_create(self, serializer):
        if self._is_mcp_request(self.request) and serializer.validated_data.get("status") == InsightsFlow.State.ACTIVE:
            raise exceptions.ValidationError(
    ...
    def perform_update(self, serializer):
        # Guardrails for MCP/LLM callers (gated on x-insights-client: mcp; the frontend and raw API are
        # unaffected). We check the raw request payload, not serializer.validated_data — InsightsFlowSerializer.validate
        ...
        if self._is_mcp_request(self.request):
            ...
            if serializer.instance.status == InsightsFlow.State.ACTIVE and has_non_status:
        ...
        with transaction.atomic():
            try:
                ...
                before_update = InsightsFlow.objects.select_for_update().get(pk=instance_id)
            except InsightsFlow.DoesNotExist:
                before_update = None
    ...
    def perform_destroy(self, instance: InsightsFlow) -> None:
    ...
    def _refresh_action_redirects(self, target: InsightsFlow, old: InsightsFlow, new_actions: Optional[list]) -> None:
    ...
    def _stage_revision_bump(self, instance: InsightsFlow, before: InsightsFlow, validated_data: dict) -> bool:
    ...
    def _append_revisions(self, instance: InsightsFlow, before: InsightsFlow) -> None:
    ...
    def _write_draft(self, instance: InsightsFlow, locked: InsightsFlow, validated_data: dict) -> None:
    ...
    @extend_schema(request=InsightsFlowGraphUpdateSerializer, responses={200: InsightsFlowSerializer})
    ...
    def graph(self, request: Request, *args, **kwargs):
        ...
        op_serializer = InsightsFlowGraphUpdateSerializer(data=request.data)
        ...
        with transaction.atomic():
            ...
            route_to_draft = self._is_mcp_request(request) and locked.status == InsightsFlow.State.ACTIVE
            ...
            before_update = InsightsFlow.objects.get(pk=instance.pk)
            if route_to_draft:
                ...
                # save() mutates and returns `locked` in place, so it's the saved InsightsFlow from here on.
                serializer.save()
                if bump:
    ...
    @extend_schema(
        ...
        request=InsightsFlowActionEmailUpdateSerializer,
        responses={200: InsightsFlowSerializer},
    )
    ...
    def action_email(self, request: Request, *args, **kwargs):
        ...
        op_serializer = InsightsFlowActionEmailUpdateSerializer(data=request.data)
        op_serializer.is_valid(raise_exception=True)
        ...
        if operations:
            predicts_draft = self._is_mcp_request(request) and instance.status == InsightsFlow.State.ACTIVE
        ...
        self._emit_resource_edited(locked)
        self._report_workflow_action(
            "insights_flow_action_email_updated",
            locked,
            {
    ...
    def _maybe_reschedule_timing_edits(self, before: Optional[InsightsFlow], after: InsightsFlow) -> None:
        ...
        """
        if not before or after.status != InsightsFlow.State.ACTIVE:
            return
        if before.status != InsightsFlow.State.ACTIVE:
    ...
    @extend_schema(request=InsightsFlowPublishRequestSerializer, responses={200: InsightsFlowPublishResponseSerializer})
    @action(detail=True, methods=["POST"])
    def publish(self, request: Request, *args, **kwargs):
        ...
        param_serializer = InsightsFlowPublishRequestSerializer(data=request.data)
    ...
    @extend_schema(request=None, responses={200: InsightsFlowSerializer})
    @action(detail=True, methods=["POST"])
    ...
    @extend_schema(responses={200: InsightsFlowRevisionBasicSerializer(many=True)})
    # filter_backends=[]: don't inherit the viewset's InsightsFlow filterset — its fields would be
    ...
    @extend_schema(
        ...
        responses={200: InsightsFlowRevisionSerializer},
    )
    ...
    @extend_schema(
        ...
        request=InsightsFlowRevisionRestoreRequestSerializer,
        responses={200: InsightsFlowSerializer},
    )
    ...
    def restore_revision(self, request: Request, version: Optional[str] = None, *args, **kwargs):
        ...
        param_serializer = InsightsFlowRevisionRestoreRequestSerializer(data=request.data)
        param_serializer.is_valid(raise_exception=True)
    ...
    @extend_schema(request=InsightsFlowInvocationSerializer, responses={200: _FallbackSerializer})
    @action(detail=True, methods=["POST"])
    ...
    def asset_content(self, request: Request, *args, **kwargs):
        # Ownership-check the InsightsFlow first so other teams' assets can't be probed.
        obj = self.get_object()
    ...
    def metrics_global(self, request: Request, *args, **kwargs):
        ...
        # The Datastore query is only team-scoped, so intersect with the workflows the caller can
        # actually see. Keeps the aggregate consistent with workflows-list/-get access control (script
    ...
    def team_reputation(self, request: Request, **kwargs) -> Response:
        ...
        # default (personal-API-key-only) Datastore throttles, so without this a member could
    ...
    @extend_schema(methods=["GET"], responses=InsightsFlowBatchJobSerializer(many=True))
    @extend_schema(methods=["POST"], request=InsightsFlowBatchJobSerializer, responses=InsightsFlowBatchJobSerializer)
    # GET returns a bare list (no pagination) and ignores the viewset's InsightsFlow filterset; disable both so the
    ...
    @extend_schema(methods=["GET"], responses=InsightsFlowScheduleSerializer(many=True))
    @extend_schema(methods=["POST"], request=InsightsFlowScheduleSerializer, responses=InsightsFlowScheduleSerializer)
    # GET returns a bare list (no pagination) and ignores the viewset's InsightsFlow filterset; disable both so the
    # generated schema matches the actual response shape.
    @action(detail=True, methods=["GET", "POST"], pagination_class=None, filter_backends=[])
    ...
    @extend_schema(
        methods=["PATCH"],
        request=InsightsFlowScheduleSerializer,
        responses=InsightsFlowScheduleSerializer,
...
class InternalInsightsFlowViewSet(TeamAndOrgViewSetMixin, LogEntryMixin, AppMetricsMixin, viewsets.ModelViewSet):
    """
    ...
    def internal_process_due_schedules(self, request: Request, **kwargs) -> Response:
        ...
        def advance_next_run(schedule, after=None):
            ...
            if occurrences:
                ...
                schedule.status = InsightsFlowSchedule.Status.COMPLETED
        ...
        try:
            ...
            due_schedule_ids = list(
                InsightsFlowSchedule.objects.filter(
                    status=InsightsFlowSchedule.Status.ACTIVE, next_run_at__lte=timezone.now()
                ).values_list("id", flat=True)
            )
            ...
            for schedule_id in due_schedule_ids:
                try:
                    ...
                    if batch_job_params:
                        InsightsFlowBatchJob.objects.create(
                            **batch_job_params,
                            status=InsightsFlowBatchJob.State.QUEUED,
                        )
                ...
                try:
                    with transaction.atomic():
                        ...
                        schedule = (
                            # nosemgrep: idor-lookup-without-team
                            InsightsFlowSchedule.objects.select_for_update(skip_locked=True)
                            .filter(id=schedule_id, status=InsightsFlowSchedule.Status.ACTIVE, next_run_at__isnull=True)
                            .first()
                        )
                        if not schedule:
                            continue
    ...
    def internal_update_batch_job_status(self, request: Request, team_id: str, batch_job_id: str) -> Response:
        ...
        if new_status not in (InsightsFlowBatchJob.State.COMPLETED, InsightsFlowBatchJob.State.FAILED):
            return Response(
        ...
        try:
            batch_job = InsightsFlowBatchJob.objects.get(id=batch_job_id, team=team)
        except (InsightsFlowBatchJob.DoesNotExist, DjangoValidationError, ValueError):
        ...
        terminal_states = {
            InsightsFlowBatchJob.State.COMPLETED,
            InsightsFlowBatchJob.State.FAILED,
            InsightsFlowBatchJob.State.CANCELLED,
        }
