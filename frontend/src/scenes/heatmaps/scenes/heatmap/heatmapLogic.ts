    ...
    listeners(({ actions, values, props, cache }) => ({
        ...
        load: async () => {
            ...
            try {
                ...
                insights.capture('in-app heatmap viewed', {
        ...
        setScreenshotUrl: ({ url }) => {
            ...
            insights.capture('in-app heatmap screenshot ready', {
        ...
        createHeatmap: async ({ context }) => {
            ...
            try {
                ...
                insights.capture('in-app heatmap created', { heatmap_type: values.type, ...context })
                actions.creationCompleted(created.short_id)
                actions.loadSavedHeatmaps()
                router.actions.push(`/heatmaps/${created.short_id}`)
            } catch (error: unknown) {
                insights.capture('in-app heatmap creation failed', {
                    failure_category: getCreationFailureCategory(error),
                })
                toast.error(getApiErrorMessage(error, 'Failed to create heatmap'))
            } finally {
        ...
        updateHeatmap: async () => {
            ...
            try {
                ...
                toast.error(getApiErrorMessage(error, 'Failed to update heatmap'))
            } finally {
                actions.setLoading(false)
            }
        },
        ...
        setScreenshotLoaded: ({ screenshotLoaded }) => {
            if (screenshotLoaded) {
                insights.capture('in-app heatmap screenshot loaded', { width: values.widthOverride })
            }
        },
        setScreenshotError: ({ error }) => {
            if (error) {
                insights.capture('in-app heatmap screenshot failed', { error })
            }
        },
        exportHeatmap: () => {
