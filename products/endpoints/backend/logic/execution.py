...
from insights.ducklake.common import is_dev_mode
from insights.errors import ExposedCHQueryError
from insights.event_usage import get_request_analytics_properties, report_user_action
...
from insights.permissions import is_authenticated_via_project_secret_api_key
...
from products.endpoints.backend.metrics import (
    ...
    ENDPOINT_INSIGHTSQL_RESULT_ROWS,
...
# Deterministic, customer-caused Datastore cost guardrails → stable client `code` + remediation.
...
_QUERY_GUARDRAIL_ERRORS: tuple[type[Exception], ...] = (*_QUERY_PERFORMANCE_ERRORS, DatastoreAtCapacity)
    ...
    try:
        ...
        if materialized:
            ...
            context = (
                ...
                f"Common causes: invalid InsightsQL syntax, missing or renamed properties, query timeout, or incompatible variable overrides."
            )
        ...
        if executed_sql:
            parts.append(f"Executed InsightsQL: {executed_sql}")
...
class EndpointExecutionService(PydanticModelMixin):
    ...
    def validate_run_request(
        ...
        if data.filters_override is not None:
            if strategy.query_kind == "InsightsQLQuery":
                raise ValidationError({"filters_override": "Not allowed for InsightsQL endpoints. Use variables instead."})

        if offset is not None and not strategy.supports_pagination:
            raise ValidationError({"offset": "offset is only supported for InsightsQL endpoints"})
    ...
    def _should_shadow_ducklake(self, endpoint: Endpoint, version: EndpointVersion | None) -> bool:
        ...
        if version is None or version.query.get("kind") != "InsightsQLQuery":
            return False

        # is_dev_mode() is also true under the test suite (USE_LOCAL_SETUP = TEST or ...); never shadow
        # there — it builds a userless InsightsQL database and runs eagerly under Celery, breaking unrelated runs.
    ...
    def _maybe_shadow_ducklake(
        ...
        *,
        ...
        datastore_ms: float,
        datastore_cached: bool,
        datastore_row_count: int | None,
        limit: int | None,
        offset: int | None,
    ) -> None:
        """Fire-and-forget DuckLake shadow for timing comparison; never affects the response."""
        # Only shadow what Datastore actually executed: an inline, non-cached run. Cache hits
        # and materialized reads have no comparable DuckLake query and would amplify cost.
        if execution_type != "inline" or datastore_cached:
            return
        ...
        try:
            shadow_compare_ducklake_execution.delay(
                ...
                datastore_cached=datastore_cached,
                datastore_ms=datastore_ms,
                datastore_row_count=datastore_row_count,
                limit=limit,
                offset=offset,
            )
        except Exception:
    ...
    def execute(
        ...
        except DatastoreAtCapacity:
            execution_status = "capacity"
            error_label = "DatastoreAtCapacity"
            logger.warning("Endpoint query hit shared Datastore capacity", endpoint_name=endpoint.name)
        ...
        self._maybe_shadow_ducklake(
            ...
            version_obj,
            execution_type=execution_type,
            datastore_ms=_ch_query_ms,
            datastore_cached=cache_outcome == "hit",
            datastore_row_count=result_row_count,
            limit=limit,
            offset=offset,
        )
        ...
        try:
            if isinstance(result.data, dict):
                ...
                if isinstance(results_value, list):
                    ...
                    if query_kind_metric == "insightsql":
                        ENDPOINT_INSIGHTSQL_RESULT_ROWS.labels(execution_type=execution_type).observe(result_row_count)
        except Exception:
    ...
    def _execute_materialized_endpoint(
        ...
        materialized_insightsql_query = None
        query_kind = None
        saved_query = version.saved_query
        try:
            if not saved_query:
            ...
            # Only paginate flat-row InsightsQL results. Insight types get transformed
            ...
            _emit_endpoint_failure_signal(
                ...
                executed_sql=materialized_insightsql_query.query if materialized_insightsql_query else None,
                saved_query_status=saved_query.status if saved_query else None,
    ...
    def _execute_inline_endpoint(
        ...
        try:
            ...
            _emit_endpoint_failure_signal(
                ...
                version=version.version,
                query_kind=query_kind,
                executed_sql=query.get("query") if query_kind == "InsightsQLQuery" else None,
                endpoint_columns=version.columns,
            )
            raise
    ...
    def _is_interactive_session(self) -> bool:
        """Whether this request came from the Insights UI (session auth) rather than a
        programmatic credential (personal API key, OAuth, project secret API key, ...).

        Same idiom as get_event_source (insights/event_usage.py) minus its session-cookie
    ...
    def _execute_query_and_respond(
        ...
        variables_override: list[InsightsQLVariable] | None = None,
