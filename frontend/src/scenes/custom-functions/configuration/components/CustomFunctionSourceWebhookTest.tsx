...
import { customFunctionSourceWebhookTestLogic } from './customFunctionSourceWebhookTestLogic'

export function CustomFunctionSourceWebhookTest(): JSX.Element {
    const { logicProps, configurationChanged } = useValues(customFunctionConfigurationLogic)
    const { isTestInvocationSubmitting, testResult, expanded, exampleCurlRequest, testInvocation } = useValues(
        customFunctionSourceWebhookTestLogic(logicProps)
    )
    ...
    return (
        <Form logic={customFunctionSourceWebhookTestLogic} props={logicProps} formKey="testInvocation" enableFormOnSubmit>
            <div
                ref={testResultsRef}
                className={clsx(
                    'p-3 rounded border',
