"""The SQLV2 data plane: the sandbox's read path to Insights data.

Token-authed sandbox -> backend endpoints (like the result callback, plain
function views, no session). The sandbox never parses InsightsQL — it sends the query
...
and the SQL editor use), so no web worker ever waits on Datastore:
...
Wired in insights/urls.py at internal/notebooks/data_plane/query/.
"""
...
FRAME_STORE_FALLBACK_COUNTER = Counter(
    "insights_notebooks_frame_store_fallback_inline",
...
def _rows_to_arrow_bytes(
    ...
    # Column-by-column with a per-column string fallback: InsightsQL results can contain
    ...
    # Carry the InsightsQL/Datastore type names alongside the Arrow schema — the FE viz
    # layer needs them (numeric/date axis detection) and Arrow types are lossier.
    if types:
        table = table.replace_schema_metadata({"insightsql_types": json.dumps(types)})
...
@extend_schema(
    ...
    responses={
        ...
        400: OpenApiResponse(description="Invalid request body or InsightsQL syntax error"),
    ...
    description=(
        "Internal endpoint the notebook sandbox POSTs InsightsQL to. Authenticated with the signed "
        "data-plane token minted at run dispatch (no session). Validates and enqueues the query "
        "for the notebook's team via the async query manager — InsightsQL access controls apply — "
...
def notebook_sql_v2_data_plane(request: HttpRequest) -> HttpResponse:
    ...
    try:
        # Validate the user's InsightsQL up front so syntax errors fail here with a clear
    ...
    if data["delivery"] == "object":
        if frame_store.is_enabled():
            ...
            # worker Datastore client, which must stay off the urls.py import path.
