...
from insights.insightsql.escape_sql import escape_duckdb_identifier
from insights.insightsql.printer.duckdb_functions import DUCKDB_FUNCTION_HANDLERS_LOWER, DUCKDB_FUNCTION_RENAMES_LOWER
from insights.insightsql.printer.postgres import PostgresPrinter


class DuckDBPrinter(PostgresPrinter):
    """Prints a InsightsQL AST as DuckDB SQL.
    ...
    DIALECT_NAME: ClassVar[InsightsQLDialect] = "duckdb"
    ...
    def __init__(
        ...
        inherited_handlers = {
            # PG emulates these InsightsQL names via handler functions; DuckDB prefers the
    ...
    def _json_property_args(self, chain) -> list[str]:
        # DuckDB reads a JSON key beginning with `$` as a JSONPath root marker, so Insights's
