...
from insights.models.scoping.manager import TeamScopedQuerySet
...
def _datastore_type(data_type: str, is_nullable: bool) -> str:
...
def _make_schema(
    ...
    *,
    ...
    table = DataWarehouseTable.objects.create(
        ...
        columns={
            column["name"]: {"datastore": _datastore_type(column["data_type"], column.get("is_nullable", False))}
...
class TestColumnsFromTable:
    def test_skips_internal_plumbing_columns(self):
        # `_dlt_*`, `_ph_debug` and the partition key are hidden from the InsightsQL catalog and carry no
        # user-facing meaning — they must not be sent to the LLM for a description.
        table = DataWarehouseTable(
            columns={
                "id": {"datastore": "String"},
                "amount": {"datastore": "Nullable(Int64)"},
                "_dlt_id": {"datastore": "String"},
                "_dlt_load_id": {"datastore": "String"},
                "_ph_debug": {"datastore": "String"},
                "_ph_partition_key": {"datastore": "String"},
            }
        )
    ...
    def test_skips_columns_whose_name_exceeds_annotation_key_length(self):
        ...
        table = DataWarehouseTable(columns={"id": {"datastore": "String"}, long_name: {"datastore": "String"}})
...
class TestEnrichTableSemanticsSync:
    ...
    def test_renamed_columns_are_annotated_under_insightsql_visible_name(self):
        # Stripe's InsightsQL layer renames some raw columns (`created` -> `created_at`, `customer` ->
        ...
        annotations = _annotations(team, table)
        # Canonical descriptions land on the InsightsQL-visible names, not the raw `created` / `customer`.
    ...
    def test_enriches_only_columns_added_after_initial_run(self):
        ...
        table.columns["currency"] = {"datastore": "Nullable(String)"}
        table.save()
...
class TestEnrichTableSemanticsActivity:
    ...
    async def test_activity_captures_and_reraises_unexpected_errors(self):
        ...
        with (
            ...
            patch.object(enrich, "hanzo_insights") as mock_analytics,
        ):
