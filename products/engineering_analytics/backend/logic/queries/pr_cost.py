"""InsightsQL assembly of a PR's estimated CI cost, summed over the jobs of all its runs.
...
cost is computed exactly once, in Datastore, from the ``logic.cost`` constants. When the jobs source
...
(the PR list scans up to a thousand PRs) can't hit InsightsQL's default 100-row cap.
"""
