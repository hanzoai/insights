...
class TestLoginPrecheckAPI(APIBaseTest):
    ...
    def test_login_precheck_reports_password_login_available_for_user_with_password(self):
        User.objects.create_and_join(self.organization, "with_password@hanzo.ai", self.CONFIG_PASSWORD)

        response = self.client.post("/api/login/precheck", {"email": "with_password@hanzo.ai"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_login_precheck_reports_password_login_unavailable_for_passwordless_user(self):
        User.objects.create_and_join(self.organization, "no_password@hanzo.ai", None)

        response = self.client.post("/api/login/precheck", {"email": "no_password@hanzo.ai"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.json(),
            {
    ...
    def test_login_precheck_reports_password_login_unavailable_for_blank_password(self):
        ...
        user = User.objects.create_and_join(self.organization, "blank_password@hanzo.ai", None)
        User.objects.filter(pk=user.pk).update(password="")

        response = self.client.post("/api/login/precheck", {"email": "blank_password@hanzo.ai"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_login_precheck_returns_linked_social_provider_configured_on_instance(self):
        user = User.objects.create_and_join(self.organization, "github_user@hanzo.ai", None)
        ...
        with self.settings(SOCIAL_AUTH_GITHUB_KEY="key", SOCIAL_AUTH_GITHUB_SECRET="secret"):
            response = self.client.post("/api/login/precheck", {"email": "github_user@hanzo.ai"})
    ...
    def test_login_precheck_treats_unknown_email_as_password_login_available(self):
        ...
        response = self.client.post("/api/login/precheck", {"email": "nobody@hanzo.ai"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["password_login_available"], True)
        self.assertEqual(response.json()["social_providers"], [])

    def test_login_precheck_treats_inactive_passwordless_user_as_unknown(self):
        user = User.objects.create_and_join(self.organization, "deactivated@hanzo.ai", None)
        user.is_active = False
        user.save()

        response = self.client.post("/api/login/precheck", {"email": "deactivated@hanzo.ai"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["password_login_available"], True)
    ...
    def test_login_precheck_is_rate_limited_by_ip(self):
        ...
        with self.settings(E2E_TESTING=False):
            for i in range(30):
                response = self.client.post("/api/login/precheck", {"email": f"enumerate-{i}@hanzo.ai"})
                self.assertEqual(response.status_code, status.HTTP_200_OK, f"request {i} was throttled early")

            response = self.client.post("/api/login/precheck", {"email": "enumerate-30@hanzo.ai"})
    ...
    def test_login_precheck_is_rate_limited_for_an_authenticated_caller_probing_other_emails(self):
        ...
        with self.settings(E2E_TESTING=False):
            for i in range(30):
                response = self.client.post("/api/login/precheck", {"email": f"victim-{i}@hanzo.ai"})
                self.assertEqual(response.status_code, status.HTTP_200_OK, f"request {i} was throttled early")

            response = self.client.post("/api/login/precheck", {"email": "victim-30@hanzo.ai"})
            self.assertEqual(response.status_code, status.HTTP_429_TOO_MANY_REQUESTS)
    ...
    def test_login_precheck_prefers_the_exact_case_match_like_login_does(self):
        ...
        User.objects.create_and_join(self.organization, "casey@hanzo.ai", None)
        with_password = User.objects.create_and_join(self.organization, "casey-alt@hanzo.ai", self.CONFIG_PASSWORD)
        ...
        User.objects.filter(pk=with_password.pk).update(email="Casey@hanzo.ai")

        response = self.client.post("/api/login/precheck", {"email": "Casey@hanzo.ai"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["password_login_available"], True)
