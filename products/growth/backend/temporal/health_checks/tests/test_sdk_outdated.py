...
def _patch_check(github: dict | None, rows: list[tuple]):
    """Patch the three external boundaries the check touches: GitHub version data, Datastore, Redis cache."""
    return (
        patch(
        ...
        patch(
            "products.growth.backend.temporal.health_checks.sdk_outdated.execute_datastore_health_team_query",
            return_value=rows,
        ),
...
class TestSdkOutdatedCheck(SimpleTestCase):
    ...
    def test_detects_outdated_sdk_with_enriched_payload(self):
        github = {
            ...
            "insights-node": {"latestVersion": "3.0.0", "releaseDates": {}},
            "insights-python": {"latestVersion": "5.0.0", "releaseDates": {"4.0.0": OLD_RELEASE}},
        }
        rows = [
            _make_ch_row(1, "web", "2.0.0"),
            _make_ch_row(1, "insights-node", "3.0.0"),
            _make_ch_row(1, "insights-python", "4.0.0", event_count=900),
        ]
        ...
        assert issue.payload["sdk_name"] == "insights-python"
    ...
    def test_detects_legacy_java_without_version_and_recommends_migration(self):
        ...
        rows = [
            _make_ch_row(1, "insights-java", None),
            _make_ch_row(1, "insights-server", "2.7.5"),
        ]
        ...
        assert len(results[1]) == 1
        issue = results[1][0]
        assert issue.payload["sdk_name"] == "insights-java"
        assert issue.payload["latest_version"] == "1.2.0"
        assert issue.payload["migration_source"] == "com.insights.java:insights"
        assert issue.payload["migration_target"] == "com.insights:insights-server"
        assert "Migrate to com.insights:insights-server" in issue.payload["reason"]
    ...
    def test_sorts_partial_and_full_semver_versions_before_assessment(self):
        ...
        rows = [
            _make_ch_row(1, "insights-server", "1.2.0", event_count=100),
            _make_ch_row(1, "insights-server", "1.10", event_count=50),
        ]
    ...
    def test_escalates_to_critical_when_majority_outdated(self):
        github = {"insights-python": {"latestVersion": "5.0.0", "releaseDates": {"4.0.0": OLD_RELEASE}}}
        rows = [_make_ch_row(1, "insights-python", "4.0.0")]
    ...
    def test_skips_team_with_no_datastore_data(self):
