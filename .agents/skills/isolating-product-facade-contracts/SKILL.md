...
description: Plan and execute product isolation migrations to a facade plus contract layer in Insights, following the Visual review architecture. Use when a product still exposes internals (models/logic/views) across boundaries and needs migration toward contracts.py + facade/api.py + presentation separation, with a PR strategy that minimizes review latency and conflicts with parallel work.
---
...
**Prerequisite:** the product must already live under `products/<name>/`. This skill does not cover moving code out of `insights/`, `ee/`, or other shared directories — do that first.
...
4. [insights/models/team/README.md](../../../insights/models/team/README.md) (team extension model rule)
...
7. [.agents/security.md](../../../.agents/security.md) (SQL/InsightsQL security guidelines)
...
```bash
insightscli product:maturity <name>       # scores models, facade, presentation, boundaries, codegen
insightscli product:lint <name>           # structural lint + isolation chain (strict if facade/contracts.py exists)
insightscli product:isolate:scan <name>   # the recon: import map, coupling gate, preflight (see below)
```
...
- **Blind spots** — the scan reads `backend.*` imports only. A coupling count of
  ...
  imports (`dags/`) or non-import channels (config dotted strings, insightsql system
...
This is the third non-import coupling channel, alongside insightsql system tables and
...
- _Monkeypatch targets_ — a core test base patches a product module attribute
  (e.g. `insights/test/base.py` patches `execute_insightsql_query` on each runner
...
**Product-owned InsightsQL system tables.** When core mounts a product's federated
...
- _Can the reference be a normal facade import?_ Yes — table defs and lazy-join
  ...
  (e.g. `facade/insightsql.py`) and reroute core's import, like any other wiring.
- _Do the objects enter the **static** pickled catalog?_ Core builds the
  ...
  (`build_database_root_node` in `insights/insightsql/database/database.py`).
...
- Regenerate OpenAPI/types (`insightscli build:openapi`) when serializer/view changes affect API schema.
...
3. Introduce a thin facade in `backend/facade/api.py`.
   ...
   - Wiring that core registers (celery beat tasks, temporal workflows/activities/metrics/
     ...
     that too. Keep `facade/api.py` free of heavy imports (InsightsQL, temporalio) so config-only
     consumers don't drag them onto the `django.setup()` path — and split light shared
     tables (`facade/insightsql.py`) out of the heavy runner re-exports for the same reason.
...
5. Move presentation to consume the facade.
   ...
   - Decide **per view module, on a size estimate**, whether its facade refactor lands
     ...
     cheap (runners already return `insights.schema` types — no new contracts needed);
   ...
   - Deferred modules keep exact-pair `ignore_imports` entries in `pyproject.toml`'s
     ...
     `insightscli product:lint` enforces the gate — contract-check while `ignore_imports`
     entries exist fails lint. The skip is the reward the presentation-wave PR unlocks
     when it empties them.
   - The mechanical share is one command: `insightscli product:isolate:move <name>` (run
...
6. Enforce boundaries and verify. This is a four-step chain — each step depends
   on the previous one, and `insightscli product:lint` (via `IsolationChainCheck`)
   ...
   3. **`backend:contract-check` script** — add to `package.json` so
      ...
      left, presentation thin. `insightscli product:lint` blocks the script while deferred
   ...
   4. **Narrowed `turbo.json` inputs** — restrict `backend:contract-check`
      ...
      `backend/models.py` if insightsql system tables expose the product's
   ...
   - **Permanent-interface exception (irreducible import coupling).** Some
     import coupling genuinely cannot be drained: Datastore DDL modules
     (`backend.sql`, `backend.embedding`, …) are imported by core's
     `insights/datastore/schema.py` registry, `conftest.py`, and **frozen**
     Datastore migrations that hardcode the import path forever. You cannot
     ...
     it. The marker tells `insightscli product:lint` the block is a declared,
   ...
   - Verify with `tach check --dependencies --interfaces`, `lint-imports`
     (import-linter contract for presentation → facade), and `insightscli product:lint <name>`.
   - Use `insightscli product:maturity <name>` for a detailed breakdown of remaining
...
block is gone (`insightscli product:lint` rejects the script while leaks remain). With the
single-PR default, all core callers land in the same PR and this block never needs to
exist.

If `insights/` or `ee/` still imports product internals (`backend.models`,
...
`insightscli product:lint` flags any product that still has legacy leak interfaces
...
3. **Commit — structure + chain.** Presentation move and rewrites
   (`insightscli product:isolate:move`), strict-lint fixes, tach interface. Add
...
- **High (triple digits):** the sweep won't review as one unit. Land contracts + facade
  - parity tests first — that standalone review is meaningful here, because every slice
    ...
    but do NOT add `backend:contract-check` yet — `insightscli product:lint` rejects that
...
1. Design the facade functions its views need. Query endpoints get run-functions that
   take `insights.schema` query objects and return the runner's response types — those
...
- `insightscli product:lint <name>` shows no legacy leak warning and the isolation chain is intact.
