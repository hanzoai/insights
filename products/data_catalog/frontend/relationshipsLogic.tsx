import { MakeLogicType, actions, afterMount, kea, listeners, path, reducers, selectors } from 'kea'
import { loaders } from 'kea-loaders'
import { urlToAction } from 'kea-router'

import api, { ApiConfig, ApiError } from 'lib/api'
import { toast } from 'lib/elements/Toast/Toast'
import { deleteWithUndo } from 'lib/utils/deleteWithUndo'
import { viewLinkLogic } from 'scenes/data-warehouse/viewLinkLogic'
import { urls } from 'scenes/urls'

import { DataWarehouseViewLink } from '~/types'

import {
    dataCatalogRelationshipProposalsAcceptCreate,
    dataCatalogRelationshipProposalsList,
    dataCatalogRelationshipProposalsRejectCreate,
} from './generated/api'
import type { DataCatalogRelationshipProposalApi } from './generated/api.schemas'

export type RelationshipStatusFilter = 'all' | 'pending' | 'active' | 'rejected'
export type RelationshipRowStatus = 'pending' | 'active' | 'rejected'

export interface RelationshipRow {
    key: string
    sourceTableName: string
    sourceTableKey: string
    joiningTableName: string
    joiningTableKey: string
    fieldName: string
    rowStatus: RelationshipRowStatus
    confidence: number | null
    reasoning: string
    evidence: unknown
    rejectionReason: string
    reviewedBy: string | null
    viaCatalog: boolean
    proposalId: string | null
    joinId: string | null
}

function projectId(): string {
    return String(ApiConfig.getCurrentTeamId())
}

function apiErrorDetail(error: unknown): string | null {
    return error instanceof ApiError ? error.detail : null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface relationshipsLogicValues {
    actionsInFlight: Record<string, boolean>
    filteredRows: RelationshipRow[]
    joins: DataWarehouseViewLink[]
    joinsById: Record<string, DataWarehouseViewLink>
    joinsLoading: boolean
    pendingCount: number
    pendingCountLoading: boolean
    proposals: DataCatalogRelationshipProposalApi[]
    proposalsLoading: boolean
    rows: RelationshipRow[]
    statusFilter: RelationshipStatusFilter
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface relationshipsLogicActions {
    acceptProposal: (id: string) => {
        id: string
    }
    deleteJoin: (join: DataWarehouseViewLink) => {
        join: DataWarehouseViewLink
    }
    loadJoins: () => any
    loadJoinsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadJoinsSuccess: (
        joins: DataWarehouseViewLink[],
        payload?: any
    ) => {
        joins: DataWarehouseViewLink[]
        payload?: any
    }
    loadPendingCount: () => any
    loadPendingCountFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadPendingCountSuccess: (
        pendingCount: number,
        payload?: any
    ) => {
        pendingCount: number
        payload?: any
    }
    loadProposals: () => any
    loadProposalsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadProposalsSuccess: (
        proposals: DataCatalogRelationshipProposalApi[],
        payload?: any
    ) => {
        proposals: DataCatalogRelationshipProposalApi[]
        payload?: any
    }
    rejectProposal: (
        id: string,
        rejectionReason: string
    ) => {
        id: string
        rejectionReason: string
    }
    setActionInFlight: (
        id: string,
        inFlight: boolean
    ) => {
        id: string
        inFlight: boolean
    }
    setStatusFilter: (statusFilter: RelationshipStatusFilter) => {
        statusFilter: RelationshipStatusFilter
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface relationshipsLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        rows: (proposals: DataCatalogRelationshipProposalApi[], joins: DataWarehouseViewLink[]) => RelationshipRow[]
        filteredRows: (rows: RelationshipRow[], statusFilter: RelationshipStatusFilter) => RelationshipRow[]
        joinsById: (joins: DataWarehouseViewLink[]) => Record<string, DataWarehouseViewLink>
    }
}

export type relationshipsLogicType = MakeLogicType<
    relationshipsLogicValues,
    relationshipsLogicActions,
    Record<string, any>,
    relationshipsLogicMeta
>

export const relationshipsLogic = kea<relationshipsLogicType>([
    path(['products', 'data_catalog', 'frontend', 'relationshipsLogic']),
    actions({
        setStatusFilter: (statusFilter: RelationshipStatusFilter) => ({ statusFilter }),
        acceptProposal: (id: string) => ({ id }),
        rejectProposal: (id: string, rejectionReason: string) => ({ id, rejectionReason }),
        setActionInFlight: (id: string, inFlight: boolean) => ({ id, inFlight }),
        deleteJoin: (join: DataWarehouseViewLink) => ({ join }),
    }),
    loaders(() => ({
        // Badge count only — never pull the full proposal payloads (unbounded reasoning/evidence)
        // just to render a number on a tab the user may never open.
        pendingCount: [
            0,
            {
                loadPendingCount: async () => {
                    const response = await dataCatalogRelationshipProposalsList(projectId(), {
                        status: 'proposed',
                        limit: 1,
                    })
                    return response.count ?? 0
                },
            },
        ],
        proposals: [
            [] as DataCatalogRelationshipProposalApi[],
            {
                loadProposals: async () => {
                    const response = await dataCatalogRelationshipProposalsList(projectId())
                    return response.results || []
                },
            },
        ],
        joins: [
            [] as DataWarehouseViewLink[],
            {
                loadJoins: async () => {
                    const response = await api.dataWarehouseViewLinks.list()
                    return response.results || []
                },
            },
        ],
    })),
    reducers({
        statusFilter: [
            'all' as RelationshipStatusFilter,
            {
                setStatusFilter: (_, { statusFilter }) => statusFilter,
            },
        ],
        actionsInFlight: [
            {} as Record<string, boolean>,
            {
                setActionInFlight: (state, { id, inFlight }) => ({ ...state, [id]: inFlight }),
            },
        ],
    }),
    selectors({
        rows: [
            (s) => [s.proposals, s.joins],
            (proposals: DataCatalogRelationshipProposalApi[], joins: DataWarehouseViewLink[]): RelationshipRow[] => {
                const acceptedByJoinId = new Map<string, DataCatalogRelationshipProposalApi>()
                proposals
                    .filter((proposal) => proposal.status === 'accepted' && proposal.created_join)
                    .forEach((proposal) => acceptedByJoinId.set(proposal.created_join as string, proposal))

                const joinRows: RelationshipRow[] = joins.map((join) => {
                    const proposal = acceptedByJoinId.get(join.id)
                    return {
                        key: `join-${join.id}`,
                        sourceTableName: join.source_table_name || '',
                        sourceTableKey: join.source_table_key || '',
                        joiningTableName: join.joining_table_name || '',
                        joiningTableKey: join.joining_table_key || '',
                        fieldName: join.field_name || '',
                        rowStatus: 'active',
                        confidence: proposal?.confidence ?? null,
                        reasoning: proposal?.reasoning ?? '',
                        evidence: proposal?.evidence,
                        rejectionReason: '',
                        reviewedBy: proposal?.reviewed_by?.email ?? null,
                        viaCatalog: !!proposal,
                        proposalId: null,
                        joinId: join.id,
                    }
                })

                // An accepted proposal that never persisted a join would otherwise vanish — surface it
                // as an active row. A proposal whose created join is absent from the loaded joins was
                // deleted (soft delete drops it from the list), and the backend treats it as gone, so
                // it must not reappear as an uneditable active row.
                const orphanedAcceptedRows: RelationshipRow[] = proposals
                    .filter((proposal) => proposal.status === 'accepted' && !proposal.created_join)
                    .map((proposal) => ({
                        key: `proposal-${proposal.id}`,
                        sourceTableName: proposal.source_table_name,
                        sourceTableKey: proposal.source_table_key,
                        joiningTableName: proposal.joining_table_name,
                        joiningTableKey: proposal.joining_table_key,
                        fieldName: proposal.field_name,
                        rowStatus: 'active',
                        confidence: proposal.confidence ?? null,
                        reasoning: proposal.reasoning ?? '',
                        evidence: proposal.evidence,
                        rejectionReason: '',
                        reviewedBy: proposal.reviewed_by?.email ?? null,
                        viaCatalog: true,
                        proposalId: proposal.id,
                        joinId: null,
                    }))

                const proposalRows: RelationshipRow[] = proposals
                    .filter((proposal) => proposal.status !== 'accepted')
                    .map((proposal) => ({
                        key: `proposal-${proposal.id}`,
                        sourceTableName: proposal.source_table_name,
                        sourceTableKey: proposal.source_table_key,
                        joiningTableName: proposal.joining_table_name,
                        joiningTableKey: proposal.joining_table_key,
                        fieldName: proposal.field_name,
                        rowStatus: proposal.status === 'rejected' ? 'rejected' : 'pending',
                        confidence: proposal.confidence ?? null,
                        reasoning: proposal.reasoning ?? '',
                        evidence: proposal.evidence,
                        rejectionReason: proposal.rejection_reason || '',
                        reviewedBy: proposal.reviewed_by?.email ?? null,
                        viaCatalog: false,
                        proposalId: proposal.id,
                        joinId: null,
                    }))

                return [...joinRows, ...orphanedAcceptedRows, ...proposalRows]
            },
        ],
        filteredRows: [
            (s) => [s.rows, s.statusFilter],
            (rows: RelationshipRow[], statusFilter: RelationshipStatusFilter) =>
                statusFilter === 'all' ? rows : rows.filter((row) => row.rowStatus === statusFilter),
        ],
        joinsById: [
            (s) => [s.joins],
            (joins: DataWarehouseViewLink[]): Record<string, DataWarehouseViewLink> =>
                Object.fromEntries(joins.map((join) => [join.id, join])),
        ],
    }),
    listeners(({ values, actions }) => ({
        acceptProposal: async ({ id }) => {
            if (values.actionsInFlight[id]) {
                return
            }
            actions.setActionInFlight(id, true)
            try {
                await dataCatalogRelationshipProposalsAcceptCreate(projectId(), id)
                toast.success('Relationship accepted and promoted to a warehouse join')
                actions.loadProposals()
                actions.loadJoins()
                actions.loadPendingCount()
            } catch (error) {
                toast.error(apiErrorDetail(error) || 'Could not accept the relationship. Try again.')
            } finally {
                actions.setActionInFlight(id, false)
            }
        },
        rejectProposal: async ({ id, rejectionReason }) => {
            if (values.actionsInFlight[id]) {
                return
            }
            actions.setActionInFlight(id, true)
            try {
                await dataCatalogRelationshipProposalsRejectCreate(projectId(), id, {
                    rejection_reason: rejectionReason || undefined,
                })
                toast.success('Relationship rejected')
                actions.loadProposals()
                actions.loadPendingCount()
            } catch (error) {
                toast.error(apiErrorDetail(error) || 'Could not reject the relationship. Try again.')
            } finally {
                actions.setActionInFlight(id, false)
            }
        },
        deleteJoin: async ({ join }) => {
            await deleteWithUndo({
                endpoint: `environments/${projectId()}/warehouse_view_link`,
                object: { id: join.id, name: `${join.field_name} on ${join.source_table_name}` },
                callback: () => actions.loadJoins(),
            })
        },
        // Refresh after the shared join modal saves. Referencing actionTypes builds
        // viewLinkLogic without mounting it, so the catalog scene doesn't eagerly load
        // the database schema; the modal in RelationshipsTab mounts it when visible.
        [viewLinkLogic.actionTypes.submitViewLinkSuccess]: () => {
            actions.loadJoins()
        },
    })),
    urlToAction(({ actions }) => ({
        // Load the full proposals and joins only when the Relationships tab is actually viewed.
        [urls.dataCatalog()]: (_, searchParams) => {
            if (searchParams.tab === 'relationships') {
                actions.loadProposals()
                actions.loadJoins()
            }
        },
    })),
    afterMount(({ actions }) => {
        actions.loadPendingCount()
    }),
])
