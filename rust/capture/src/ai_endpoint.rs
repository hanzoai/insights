...
async fn ai_handler_inner(
    ...
    if let Err(reason) = validate_token(token) {
        ...
        return Err(CaptureError::from(reason).into());
    }
        ...
        Some(resolver) => match resolver.resolve(token).await {
            ...
            Err(crate::team::TeamResolveError::Unknown) => {
                state
                    .warnings
                    .refused(WarningType::IngestKeyUnknown, Some(token), "ai", Some(1));
                return Err(CaptureError::UnknownToken.into());
            }
    ...
    *attribution = Some(ai_request_context(
    ...
    if !parsed.blob_parts.is_empty() {
        ...
        let uploaded = blob_storage
            .upload_blobs(&team_id.to_string(), &parsed.event_uuid.to_string(), blobs)
            .await
            .map_err(|e| {
...
///
/// The endpoint has no `Insights-Sdk-Info` contract, so SDK identity comes from
