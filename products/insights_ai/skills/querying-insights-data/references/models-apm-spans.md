...
The `insights.trace_spans` table holds OpenTelemetry span data from instrumented services. Each row is one span — a unit of work in a distributed trace. Spans within the same trace share a `trace_id`; the parent-child hierarchy is reconstructed via `parent_span_id` → `span_id`.

**Namespacing:** Reference this table as `insights.trace_spans`, not bare `trace_spans` — it's registered under the `insights.` namespace in the InsightsQL database (see `insights/insightsql/database/database.py`). The same applies to `insights.trace_attributes`. Bare names fail with "Unknown table" at InsightsQL compile time. (Asymmetric with `logs`, which is registered at root level — `logs` works without a prefix.)

**Prefer the typed tools when they fit:** `insights:query-apm-spans` for span listing with structured filters, `insights:apm-trace-get` for full-trace fetches, `insights:apm-spans-aggregate` / `insights:apm-spans-tree` for aggregations. Reach for InsightsQL when you need cross-signal joins (with `logs` or `insights.metrics` by `trace_id`), exemplar lookups, or aggregations the typed tools don't expose.

## `insights.trace_spans`

OpenTelemetry spans. One row per span. Backed by Datastore `trace_spans_distributed`.
...
- **`trace_id`, `span_id`, `parent_span_id` are base64-encoded bytes**, not hex. The MCP layer (`insights:query-apm-spans`, `insights:apm-trace-get`) converts to hex via `hex(tryBase64Decode(...))` for display. Raw InsightsQL queries against this table see the base64 form.
...
- Cross-signal joins by `trace_id` work against `logs` (both store base64). For `insights.metrics`, exemplar extraction is not yet wired up in the ingestion pipeline — see the metrics reference for the current state.
- User InsightsQL queries on `insights.trace_spans` are capped at 50 GB read per query.

## `insights.trace_attributes`

AggregatingMergeTree rollup of span attribute values, partitioned by service and 10-minute bucket. Backs the attribute discovery endpoints used by `insights:apm-attributes-list` and `insights:apm-attribute-values-list`. Same `insights.` namespacing rule — reference as `insights.trace_attributes`.
...
Prefer `insights:apm-attributes-list` / `insights:apm-attribute-values-list` over querying this table directly — they handle the aggregation correctly.
