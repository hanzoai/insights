"""Tests for insightscli telemetry."""
...
from insightscli.cli import _outcome, _should_track, cli
...
def test_is_disabled_when_no_api_key_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    """Standalone insightscli users without a telemetry key get no-op telemetry by default."""
...
class TestFirstRunNotice:
    ...
    def test_shown_once(self, telemetry_config: Path, capsys):
        ...
        assert "insightscli collects anonymous usage data" in captured_first.err
...
class TestInvokeTelemetry:
    def test_invoke_fires_started_and_completed_events(self, monkeypatch: pytest.MonkeyPatch, telemetry_config: Path):
        ...
        with patch.object(telemetry._client, "_send_batch") as mock_send:
            ...
            assert "insightscli_version" in started["properties"]
    ...
    def test_first_run_shows_notice_and_emits_events(self, monkeypatch: pytest.MonkeyPatch, telemetry_config: Path):
        ...
        assert "insightscli collects anonymous usage data" in result.output
...
def _patch_cli_manifest(monkeypatch: pytest.MonkeyPatch, command_config: dict | None) -> None:
    """Point insightscli.cli at a fake manifest whose every command has *command_config*.

    Keeps core tests off Insights's insightscli.yaml per the framework boundary.
    """
    ...
    monkeypatch.setattr("insightscli.cli.get_manifest", lambda: _FakeManifest())
