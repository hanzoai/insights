...
# Batch accumulation targets for streaming to Delta Lake. Datastore yields
...
class DatastoreConnectionError(Exception):
    """Raised when we cannot establish or use a Datastore connection."""

    pass


# datastore-connect probes the server with `SELECT version(), timezone()` while
...
# that isn't a Datastore response — a proxy/load-balancer landing page, or a
...
# run a query. The endpoint isn't serving the Datastore HTTP interface, so a retry
...
NOT_A_DATASTORE_HTTP_RESPONSE = (
    "The host answered but did not return a valid Datastore response, so it isn't serving the "
    "Datastore HTTP interface on that host/port. Check the host, port, and HTTPS setting "
    "(and any tunnel or proxy in front of it)."
)


def _quote_identifier(identifier: str) -> str:
    """Quote a Datastore identifier with backticks.

    Datastore allows arbitrary identifiers when wrapped in backticks. We
...
# TLS read (Datastore Cloud cold-resume hanging up the first request, a proxy/LB idle cull, a
...
_TRANSIENT_CONNECT_DROP_SUBSTRINGS = (
    ...
    # The Datastore host (or a proxy/gateway in front of it) rate-limited the
    ...
    # datastore-connect already retries 429 for queries (query_retries), but
...
# datastore-connect surfaces an upstream rate-limit as a full HTTP response
...
def _apply_session_settings(client: DatastoreClient, settings: dict[str, Any]) -> None:
    ...
    profile (common on managed Datastore) reports every setting as readonly, and
    datastore-connect refuses to send those — raising ProgrammingError ("Setting
    ...
    for key, value in settings.items():
        try:
            ...
            structlog.get_logger().warning(
                "Datastore rejected session setting; falling back to server default",
                setting=key,
                exc_info=e,
            )
...
class _NoRedirectPoolManager(PoolManager):
    ...
    reachability the egress proxy exists to deny. A Datastore HTTP endpoint has no reason to
    redirect us, so refusing costs nothing: the 3xx response goes back to datastore-connect,
    ...
    datastore-connect passes its own `retries` on every request, which would take precedence
...
def _no_env_proxy_pool_manager(verify: bool, server_hostname: str | None = None) -> Any:
    ...
    datastore-connect only checks the proxy env vars when it builds its own
    pool manager, so handing it one is the sanctioned per-code-path opt-out
    from the egress proxy (see insights/security/outbound_proxy.py for the
    ...
    own hostname or the certificate can never match. Mirrors what datastore-connect does
    ...
    Built from datastore-connect's own options factory so the TCP keepalive tuning and
...
def _get_client(
    *,
    ...
    """Create a Datastore HTTP client.

    Uses datastore-connect, which speaks the HTTP/HTTPS interface. This is
    ...
    vars: "internal_team" for Insights-internal teams
    ...
    while True:
        try:
            client = get_client(
                ...
                database=database,
                username=user,
                # datastore-connect expects str; passwordless auth is empty string.
        ...
        except (DatastoreError, OSError, ssl.SSLError) as e:
            ...
            # covers TLS handshake failures that happen before Datastore sees
            # the request.
            attempt += 1
            message = str(e)
            if attempt < _MAX_CONNECT_ATTEMPTS and _is_retryable_connect_error(message):
                ...
                structlog.get_logger().warning(
                    "Transient Datastore connect error; retrying",
                    attempt=attempt,
                ...
                time.sleep(wait)
                continue
            raise DatastoreConnectionError(message) from e
        except ValueError as e:
            # The construction-time server probe got a response it couldn't parse as a
            # Datastore handshake (see NOT_A_DATASTORE_HTTP_RESPONSE). Deterministic, so
            # never retryable — don't spend the transient-retry budget on it.
            raise DatastoreConnectionError(NOT_A_DATASTORE_HTTP_RESPONSE) from e
...
def filter_datastore_incremental_fields(
    columns: list[tuple[str, str, bool]],
) -> list[tuple[str, IncrementalFieldType, bool]]:
    """Return columns suitable for use as an incremental cursor.

    Datastore type names are case-sensitive in metadata responses (e.g.
...
def get_schemas(
    *,
    ...
    Note: Datastore columns expose the *original* type string, including
    ...
    try:
        ...
        names_filter = ""
        if names:
            # datastore-connect formats tuples as `(a, b, c)`, which matches
            # Datastore's IN clause syntax. Lists would format as `[a, b, c]`
...
def _is_inner_table(table_name: str) -> bool:
    ...
    Datastore backs a materialized view created without an explicit `TO`
...
# Datastore always emits the target in `system.tables.create_table_query`
...
def get_datastore_row_count(
    *,
    ...
    Coverage:
    - MergeTree family: free — Datastore keeps a running counter.
...
# Regex helpers for parsing Datastore type strings.
...
class DatastoreColumn(Column):
    """Implementation of the `Column` protocol for a Datastore source.

    Attributes:
        name: The column's name.
        data_type: The original Datastore type string, possibly wrapped in
...
def _get_table(client: DatastoreClient, database: str, table_name: str) -> Table[DatastoreColumn]:
    ...
    for name, raw_type in cols_result.result_rows:
        _, nullable = _strip_type_modifiers(raw_type)
        columns.append(DatastoreColumn(name=name, data_type=raw_type, nullable=nullable))
...
def _get_primary_keys(client: DatastoreClient, database: str, table_name: str) -> list[str] | None:
    """Return the columns of the table's sorting key.

    Datastore's primary key is by definition a prefix of the sorting key,
...
def get_primary_keys_for_schemas(
    *,
    ...
    try:
        ...
        try:
            for table_name in table_names:
                try:
                    ...
                    structlog.get_logger().warning(
                        "Failed to detect primary keys for Datastore table",
                        table=table_name,
                        exc_info=e,
                    )
        finally:
        ...
        structlog.get_logger().warning("Failed to detect primary keys for Datastore schemas", exc_info=e)

    return result


# Row budget for the duplicate-PK probe. Datastore sorting keys are not
...
#   - "is unknown or readonly": datastore-connect validates session settings
...
#     (Datastore Cloud) and readonly user profiles.
...
#     execution, not Datastore Cloud's cold-resume wake-up latency or a scan
...
def _has_duplicate_primary_keys(
    ...
    Datastore sorting keys are *not* enforced unique. For incremental syncs
    ...
    3. Datastore cannot *prove* uniqueness anyway — only the user can.
    ...
    # LIMIT 1 lets Datastore short-circuit the moment it finds a duplicate.
    ...
    try:
        ...
        # source tables, and managed/readonly Datastore servers routinely
...
# datastore-connect surfaces a non-2xx HTTP status from the server (or a
...
# own. A real Datastore query error carries a `Code: NNN` instead. We match
...
def _get_partition_settings(
    client: DatastoreClient, database: str, table_name: str, logger: FilteringBoundLogger
) -> PartitionSettings | None:
    """Compute partition settings using `system.tables.total_bytes`.

    Datastore maintains compressed and uncompressed sizes per table — we
...
# Datastore types Arrow output can't emit directly — we coerce to String
# via toString() to avoid Datastore error 50 "Type is not supported by Arrow".
...
def _build_select_list(columns: list[DatastoreColumn]) -> str:
...
def _project_columns(
    ...
    names back to typed `DatastoreColumn`s so toString casting is preserved.
...
def _last_value_expr(incremental_field_type: Optional[IncrementalFieldType]) -> str:
    ...
    (Datastore's own on-disk representation) rather than a date/string, e.g. after a
...
def _build_query(
    *,
    ...
    Datastore can't emit as Arrow (UUID, IPv4, enums, arrays, ...) are
...
def _query_settings(chunk_size: int) -> dict[str, Any]:
    """Datastore server-side settings applied to every data query.
...
def datastore_source(
    *,
    ...
    """Build a SourceResponse that pulls a single Datastore table.
    ...
    with tunnel() as (host, port):
        ...
        try:
            ...
            # Datastore can only skip the sort if the ORDER BY column leads
            ...
            row_counts = get_datastore_row_count(
                host=host,
                port=port,
                database=database,
                user=user,
    ...
    def get_rows() -> Iterator[Any]:
        ...
        with tunnel() as (stream_host, stream_port):
            ...
            try:
                ...
                logger.info(f"Datastore query: {query}")

                # query_arrow_stream yields pa.RecordBatch chunks — one per
                # Datastore block, capped by max_block_size. We accumulate
