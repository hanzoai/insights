...
from insights.schema import AlertCalculationInterval

from insights.slo.context import SloSpec, slo_operation
from insights.slo.types import SloArea, SloOperation, SloOutcome
from insights.tasks.alerts.utils import calculation_interval_to_order, next_check_time, trigger_alert_insights_functions
...
class TestAlertUtils:
    ...
    @patch("insights.tasks.alerts.utils.alert_internal_event_delivered", return_value=True)
    @patch("insights.tasks.alerts.utils.flush_alert_internal_events")
    @patch("insights.tasks.alerts.utils.produce_alert_internal_event")
    def test_trigger_alert_insights_functions_uses_shared_delivery(
        self,
        _name: str,
        ...
        trigger_alert_insights_functions(alert, properties={"breaches": "test breach"})
    ...
    @patch("insights.tasks.alerts.utils.produce_alert_internal_event", return_value=None)
    def test_trigger_alert_insights_functions_ignores_enqueue_failure(self, _mock_produce: MagicMock) -> None:
        alert = MagicMock(spec=AlertConfiguration)
        alert.id = "00000000-0000-0000-0000-000000000001"
        alert.team_id = 2
        alert.last_checked_at = None
    ...
    @patch("insights.tasks.alerts.utils.alert_internal_event_delivered", return_value=False)
    @patch("insights.tasks.alerts.utils.flush_alert_internal_events")
    @patch("insights.tasks.alerts.utils.produce_alert_internal_event")
