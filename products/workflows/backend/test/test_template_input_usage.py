...
class TestTemplateInputUsage(BaseTest):
    def _create_flow(self, *, name: str, status: str, actions: list[dict]) -> InsightsFlow:
        return InsightsFlow.objects.create(team=self.team, name=name, status=status, actions=actions)
