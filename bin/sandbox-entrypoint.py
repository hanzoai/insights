...
# Datastore RESTORE has fully landed, or a failure reason if it dies before the
...
def export_environment(uid: int, gid: int) -> None:
    ...
    extra_vars = {
        ...
        # never activates. bin/insightscli and bin/ruff.sh prefer it over PATH. Equal
...
def install_python_deps() -> None:
    ...
    # Make insightscli available — normally done by flox on-activate.sh.
    insightscli_link = Path("/cache/python/bin/insightscli")
    insightscli_link.unlink(missing_ok=True)
    insightscli_link.symlink_to("/workspace/bin/insightscli")
...
def ensure_demo_data() -> None:
    ...
    result = run_quiet(
        [
            ...
            "insights",
            "-tAc",
            "SELECT 1 FROM insights_user WHERE email='test@hanzo.ai' LIMIT 1",
        ]
    )
...
def create_kafka_topics() -> None:
    ...
    for topic in ("datastore_events_json",):
...
def generate_mprocs_config() -> None:
    ...
    config_dir = WORKSPACE / ".insights/.generated"
    ...
    if not needs_seed:
        needs_seed = "_insights:" not in config_file.read_text()
    ...
    if needs_seed:
        ...
        # insightscli installed, and the structure is simple enough to emit by hand.
        lines = ["_insights:", "  intents:"]
        ...
        lines.append("  - migrate-datastore")
        lines.append("  - migrate-persons-db")
        lines.append("procs: {}")
        config_file.write_text("\n".join(lines) + "\n")

    _run_captured("insightscli dev:generate", ["insightscli", "dev:generate"])
        ...
        dedent("""\
            ...
            <project version="4">
              <component name="ProjectModuleManager">
                <modules>
                  <module fileurl="file://$PROJECT_DIR$/.idea/insights.iml" filepath="$PROJECT_DIR$/.idea/insights.iml" />
                </modules>
              </component>
            </project>
        """),
...
def user_phase() -> None:
    ...
    # Window 1: claude (launched immediately, may lack insightscli until uv sync finishes)
...
def _datastore_ready() -> bool:
    """True once Datastore answers a trivial query over HTTP."""
    try:
        ...
        with urllib.request.urlopen("http://datastore:8123/", data=b"SELECT 1", timeout=3) as resp:
...
def _wait_for_databases() -> None:
    ...
    restored. Datastore restores onto a *running* server, and the database can
    ...
    _wait_for(
        "Postgres",
        lambda: run_quiet(["psql", "-h", "db", "-U", "insights", "-d", "insights", "-tAc", "SELECT 1"]).returncode == 0,
        fail_check=_restore_failed_reason,
    )
    if os.environ.get("SANDBOX_USE_CACHE") == "1":
        _wait_for("Datastore cache restore", lambda: _restore_status() == "done", fail_check=_restore_failed_reason)
    else:
        _wait_for("Datastore", _datastore_ready, fail_check=_restore_failed_reason)
...
def run_setup() -> None:
    ...
    try:
        ...
        # generate_mprocs_config needs the insightscli symlink created by install_python_deps.
        ...
        run(["tmux", "-L", "sandbox", "new-window", "-t", "insights:", "-n", "phrocs", "bin/start --phrocs"])
        ...
        tmux = ["tmux", "-L", "sandbox"]
        subprocess.run([*tmux, "select-window", "-t", "insights:setup"])
        subprocess.run([*tmux, "rename-window", "-t", "insights:setup", "⚠ SETUP FAILED"])
...
def cache_init_phase() -> None:
    ...
    exists. Exits when done so the host script can snapshot Postgres + Datastore.
    """
    _setup_user_env()
