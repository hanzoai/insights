...
func (m Model) handleMascotKey(msg tea.KeyPressMsg, cmds []tea.Cmd) (Model, []tea.Cmd, bool) {
	switch {
	case key.Matches(msg, m.keys.Mascot), msg.Code == tea.KeyEscape:
		m.mascotMode = false
		m.dbg("mascot mode: exit")
...
func (m Model) handleNormalKey(msg tea.KeyPressMsg, cmds []tea.Cmd) (tea.Model, tea.Cmd) {
	...
	case key.Matches(msg, m.keys.Mascot):
		m.mascotMode = !m.mascotMode
		m.dbg("mascot mode: %v", m.mascotMode)
		if m.mascotMode {
			m.mascotX = 0
			m.mascotDir = 1
			m.mascotFrame = 0
