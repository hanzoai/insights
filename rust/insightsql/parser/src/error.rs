...
//! parser's error shape in [`common/insightsql_parser/parser_json.cpp`]. The
//! Python side ([`insights/insightsql/json_ast.py`]) special-cases this envelope
//! and raises `ExposedInsightsQLError` / `SyntaxError` from it.
...
pub struct ParseError {
    ...
    /// `InsightsQLSyntaxError`; everything else becomes `ExposedInsightsQLError`.
    pub kind: ErrorKind,
