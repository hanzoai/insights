...
class TestSQLV2BackendBaseURL(SimpleTestCase):
    @parameterized.expand(
        [
            ...
            ("prod_fallback", None, False, "https://us.hanzo.ai"),
        ]
    )
    def test_callback_url_base(self, _name: str, sandbox_api_url: str | None, debug: bool, expected_base: str) -> None:
        with override_settings(SANDBOX_API_URL=sandbox_api_url, DEBUG=debug, SITE_URL="https://us.hanzo.ai"):
...
class TestSQLV2ApplyPageBounds(SimpleTestCase):
    def test_naked_scan_is_bounded_in_place_not_wrapped(self) -> None:
        ...
        # Datastore pushes it into an aggregated view instead of scanning the whole table.
    ...
    def test_bounding_keeps_the_exact_query_text(self) -> None:
        ...
        # table-function args (numbers(50001) -> numbers), which then fails on Datastore.
    ...
    def test_raw_bounding_still_wraps_a_select_behind_a_comment(self) -> None:
        ...
        assert out.endswith(") as insights_notebook_page limit 301 offset 0")
    ...
    def test_terminator_is_stripped_even_behind_a_trailing_comment(self) -> None:
        ...
        assert ";" not in out.split(") as insights_notebook_page")[0]
        assert (
...
class TestSQLV2Callback(APIBaseTest):
    ...
    def test_frame_snapshot_never_lands_on_another_users_kernel(self):
        ...
        collaborator = User.objects.create_and_join(self.organization, "other@hanzo.ai", None)
    ...
    def test_insightsql_envelope_leaves_the_previous_snapshot_standing(self):
        # A insightsql run never enters the kernel, so it reports no frames and must not be read as
...
class TestSQLV2Run(APIBaseTest):
    ...
    def test_direct_enqueue_threads_user_and_private_query_id(self, _mock_enabled, mock_enqueue):
        # user_id drives InsightsQL/warehouse access control downstream (a userless run fails
    ...
    def test_query_restricted_member_cannot_run(self, _mock_enabled, mock_start):
        # A notebook editor whose query access is denied must not execute InsightsQL through the node.
    ...
    @patch("products.notebooks.backend.presentation.views.notebook.start_sql_v2_run_workflow")
    @patch("products.notebooks.backend.presentation.views.notebook.is_sql_v2_enabled", return_value=True)
    def test_insightsql_ref_whose_latest_run_was_duckdb_is_treated_as_not_run(self, _mock_enabled, mock_start):
        # A SQL node's runs can alternate engines; a duckdb run's code is raw SQL naming
        # kernel frames, so inlining it as a CTE would ship it to Datastore. The stale older
        # insightsql run must not be used either — the node's latest result is a local frame.
        with freeze_time("2026-07-04T00:00:00Z"):
    ...
    @patch("products.notebooks.backend.presentation.views.notebook.start_sql_v2_run_workflow")
    @patch("products.notebooks.backend.presentation.views.notebook.is_sql_v2_enabled", return_value=True)
    def test_insightsql_typo_with_refs_present_is_a_400_not_a_500(self, _mock_enabled, mock_start):
        # With refs present the user's code is parsed at dispatch, so a plain typo raises
        # ExposedInsightsQLError there — it must surface as a bad request, not a server error.
        self._record_done_run("node-df1", "select id from events")
        response = self.client.post(
            self.run_url,
    ...
    def test_sql_node_referencing_a_local_frame_reroutes_to_duckdb(self, _mock_enabled, mock_start):
        # Journey 5: a local (Python-made) frame can't push to Datastore, so the join runs in
        # the sandbox's DuckDB — SQL as written, insightsql refs shipped as materialization inputs.
        self._record_done_run("node-df2", "select id from persons")
        ...
        self.assertEqual(
            [(i["name"], i["kind"]) for i in dispatched.inputs], [("df2", "insightsql"), ("new_events", "local")]
        )
...
class TestSQLV2RunOnAConnection(APIBaseTest):
    ...
    def test_connection_reaches_the_enqueued_query(self, mock_enqueue, _mock_enabled):
        ...
        # against Datastore and failed with "Unknown table". The id has to ride the query.
    ...
    def test_raw_mode_ships_engine_sql_untouched(self, mock_enqueue, _mock_enabled):
        # Raw SQL never reaches the InsightsQL parser: `::text` and friends would fail to parse, and
    ...
    @parameterized.expand(
        [
            ("connection_cell_reading_a_insights_cell", True),
            ("insights_cell_reading_a_connection_cell", False),
        ]
    )
    @patch("products.notebooks.backend.presentation.views.notebook.enqueue_direct_run")
    ...
    def test_python_reading_a_connection_cell_says_what_to_do(self, mock_start, _mock_enabled):
        ...
        # Insights — so it must say that, not tell the user to move a Python cell onto a warehouse.
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn("Python cells can only read Insights results", response.json()["detail"])
        mock_start.assert_not_called()
...
class TestSQLV2RunPage(APIBaseTest):
    ...
    def _create_run(
        ...
        node_id="n1",
        code="select 1",
        node_type=NotebookNodeRun.NodeType.INSIGHTSQL,
        result_id=None,
    ) -> NotebookNodeRun:
        with team_scope(self.team.id):
    ...
    @patch("products.notebooks.backend.presentation.views.notebook.fetch_sql_v2_page")
    @patch("products.notebooks.backend.presentation.views.notebook.is_sql_v2_enabled", return_value=True)
    def test_insightsql_run_paging_is_retired(self, _mock_enabled, mock_fetch):
        # SQL results page client-side over the rows the result poll serves; the server
        # page endpoint must refuse insightsql runs instead of round-tripping to a kernel.
...
class TestSQLV2PageDispatch(APIBaseTest):
    ...
    def test_kernel_run_pages_by_result_id_not_code(self, mock_post):
        # A python/duckdb result pages by slicing its on-sandbox frame; its code is not a InsightsQL
...
class TestSQLV2RunResult(APIBaseTest):
    ...
    def test_running_direct_run_expires_to_failed_after_grace(self, _mock_enabled):
        # A RUNNING insightsql run with no query status left can never complete — this poll is
        # its watchdog. Within the grace window it keeps waiting (covers pre-deploy
        # kernel-executed insightsql runs whose callback is still due).
...
class TestSQLV2RunInterrupt(APIBaseTest):
    ...
    def test_run_on_a_collaborators_kernel_is_not_marked_terminal(self, _mock_enabled):
        ...
        other_user = self._create_user("collaborator@hanzo.ai")
    ...
    def test_direct_sql_run_interrupt_stops_the_query_without_a_kernel(
        ...
        # A direct (insightsql) run has no kernel to signal: interrupt marks the row abandoned and
...
class TestSQLV2Activities(APIBaseTest):
    ...
    def test_dispatch_activity_posts_to_ready_server(self, mock_post, mock_version):
        ...
        # Dropping cache_limit silently degrades every page fetch into a Datastore re-query.
...
class TestSQLV2DataPlaneEndpoint(APIBaseTest):
    ...
    def test_runs_query_and_returns_arrow(self):
        ...
        # The real Datastore type must survive the Arrow round-trip (schema metadata).
    ...
    def test_object_delivery_redirects_to_a_team_scoped_presigned_frame(self):
        ...
        # UUID column pins the type-normalization pass: Datastore's native Arrow output
    ...
    def test_identical_inflight_materializations_share_one_job(self):
        ...
        # job instead of stacking a second Datastore execution.
        from products.notebooks.backend.temporal import frame_materialize
    ...
    def test_insightsql_error_is_surfaced(self):
...
class TestSQLV2KernelPackage(SimpleTestCase):
    def test_arrow_contract_round_trip(self):
        ...
        # declared InsightsQL types ride through as schema metadata.
    ...
    def test_pages_within_the_cache_never_requery(self):
        # The whole point of the cache: paging must not re-run the query on Datastore.
    ...
    def test_data_plane_poll_aborts_when_the_cancel_event_fires(self):
        # Without the in-loop cancel check, an interrupted InsightsQL run keeps a thread polling
...
class TestSQLV2PythonNodeRun(SimpleTestCase):
    ...
    def test_execute_run_keeps_insightsql_nodes_off_the_kernel(self):
        # A pure-InsightsQL node must stay on the capped data-plane fetch — never spin up the kernel.
        payload = {
            "run_id": "r-insightsql",
            "code": "select 1",
            "callback_url": "http://backend/cb",
...
class TestSQLV2NodeRunMetrics(APIBaseTest):
    ...
    def _histogram_count(self, labels: dict[str, str]) -> float:
        ...
        return REGISTRY.get_sample_value("insights_notebooks_node_run_seconds_count", labels) or 0.0
    ...
    def test_callback_reports_the_run_with_its_outcome(self, envelope_status, expected_outcome, mock_report):
        ...
        before = self._histogram_count({"node_type": "insightsql", "outcome": expected_outcome})
        response = self.client.post(
            f"/internal/notebooks/runs/{run.id}/result/",
            data=json.dumps(
                {
        ...
        self.assertEqual(properties["node_type"], "insightsql")
        ...
        after = self._histogram_count({"node_type": "insightsql", "outcome": expected_outcome})
    ...
    @patch("products.notebooks.backend.presentation.views.notebook.is_sql_v2_enabled", return_value=True)
    @patch("products.notebooks.backend.sql_v2_metrics.report_user_or_team_action")
    def test_direct_run_reports_queued_and_datastore_phases(self, mock_report, _mock_enabled):
        ...
        self.assertGreaterEqual(properties["datastore_seconds"], 0)
