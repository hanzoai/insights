...
from insights.caching.utils import (
    ThresholdMode,
...
from insights.queries.property_values import (
    get_event_property_values_from_aggregated_table,
    get_person_property_values_for_key,
)
from insights.utils import convert_property_value, flatten, get_instance_region, relative_date_parse
...
class PropertyValuesQueryRunner(AnalyticsQueryRunner[PropertyValuesQueryResponse]):
    ...
    def to_query(self) -> ast.SelectQuery:
        ...
        # Person queries use raw SQL for speed (4s vs 30s) — move here when InsightsQL persons table gets faster
    ...
    def _use_property_values_table(self) -> bool:
        ...
        if not hanzo_insights.feature_enabled(
            ...
            return False
        # Restricted keys stay on the events scan: the table read bypasses InsightsQL
    ...
    def _calculate_person(self) -> PropertyValuesQueryResponse:
        # Use the raw SQL person query — the InsightsQL persons virtual table does full argMax dedup
    ...
    def _format_event_results(self, rows: list) -> list[PropertyValueItem]:
        ...
        for row in rows:
            ...
            if isinstance(raw, float | int | bool | uuid.UUID):
                ...
                # Datastore strips outer quotes from string values but leaves inner \" escapes,
