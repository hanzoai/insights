...
def _get_previous_jobs(saved_query_id: UUID, current_job_id: UUID, count: int) -> "QuerySet[DataModelingJob]":
    ...
    return (
        DataModelingJob.objects.filter(saved_query_id=saved_query_id, engine=DataModelingJobEngine.DATASTORE)
...
def should_pause_schedule_for_timeout(saved_query_id: UUID, current_job_id: UUID) -> tuple[bool, int]:
    ...
    timeouts that can occur due to temporary Datastore load.
    """
...
def _maybe_pause_schedule_on_timeout(job: DataModelingJob, saved_query: DataWarehouseSavedQuery) -> bool:
    ...
    schedules for transient timeouts that can occur due to temporary Datastore load.
    """
...
async def fail_materialization_activity(inputs: FailMaterializationInputs) -> None:
    ...
    try:
        ...
        if "Timeout exceeded" in error:
            ...
            if suspended:
                get_node_suspended_metric(DataModelingJobEngine.DATASTORE.value).add(1)
                await logger.ainfo(
                    f"Suspended node {inputs.node_id} (datastore) after {CONSECUTIVE_FAILURES_TO_SUSPEND} consecutive failures",
                )
