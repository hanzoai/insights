...
class TestParseInsightsResourceLink:
    def test_project_insight(self):
        assert parse_insights_resource_link("https://us.hanzo.ai/project/42/insights/abc123") == ("insight", "abc123")

    def test_project_insight_with_subpath(self):
        assert parse_insights_resource_link("https://eu.hanzo.ai/project/42/insights/abc123/edit") == (
            "insight",
            "abc123",
        )

    def test_short_insight_path(self):
        assert parse_insights_resource_link("https://app.hanzo.ai/i/xyz789") == ("insight", "xyz789")

    def test_project_dashboard(self):
        assert parse_insights_resource_link("https://us.hanzo.ai/project/3/dashboard/99") == ("dashboard", 99)

    def test_skips_insight_new(self):
        assert parse_insights_resource_link("https://x.com/project/1/insights/new") is None

    def test_support_ticket_by_number(self):
        assert parse_insights_resource_link("https://us.hanzo.ai/project/42/support/tickets/123") == ("ticket", "123")

    def test_support_ticket_by_uuid_ignores_fragment(self):
        assert parse_insights_resource_link("https://app.hanzo.ai/support/tickets/abc-uuid#panel=discussion") == (
            "ticket",
            "abc-uuid",
        )

    def test_skips_ticket_new(self):
        assert parse_insights_resource_link("https://x.com/support/tickets/new") is None

    def test_unrelated_url(self):
        assert parse_insights_resource_link("https://example.com/foo") is None
...
class TestHandleInsightsLinkUnfurl(APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_skips_ticket_without_viewer_access(
        ...
        member = User.objects.create_and_join(self.organization, "restricted-unfurl@hanzo.ai", "password")
