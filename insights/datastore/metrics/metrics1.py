...
def METRIC_ATTRIBUTES_MV():
    return f"""
CREATE MATERIALIZED VIEW IF NOT EXISTS {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{TABLE_NAME}_to_metric_attributes TO {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{METRIC_ATTRIBUTES_TABLE_NAME}
(
    `team_id` Int32,
...
def METRIC_RESOURCE_ATTRIBUTES_MV():
    return f"""
CREATE MATERIALIZED VIEW IF NOT EXISTS {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{TABLE_NAME}_to_resource_attributes TO {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{METRIC_ATTRIBUTES_TABLE_NAME}
(
    `team_id` Int32,
    `time_bucket` DateTime64(0),
    `service_name` LowCardinality(String),
