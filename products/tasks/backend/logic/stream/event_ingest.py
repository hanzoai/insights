...
STREAM_COMPLETE_CONTROL_TYPE = "_insights/stream_complete"
...
async def _capture_rtk_savings_if_needed(
    ...
    try:
        event_uuid = str(uuid5(NAMESPACE_URL, f"insights-task-rtk-savings:{claims.run_id}:{sequence}"))
...
def _parse_rtk_savings_properties(claims: SandboxEventIngestTokenPayload, event: dict) -> dict[str, str | int] | None:
    ...
    if not isinstance(notification, dict) or notification.get("method") != "_insights/rtk_savings":
        return None
