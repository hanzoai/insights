...
`owner/repo.endpoint`) and one webhook InsightsFunction whose `schema_mapping` routes incoming events
...
def _reconcile_repo_webhooks(
    *,
    ...
    webhook_url = get_webhook_url(str(insights_function.id))
    signing_secret = ((insights_function.encrypted_inputs or {}).get("signing_secret") or {}).get("value")
