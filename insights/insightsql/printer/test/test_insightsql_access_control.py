...
from insights.insightsql.errors import QueryError, TableAccessDeniedError
...
class TestAccessControlGuard(BaseTest):
    ...
    def test_build_access_control_guard_returns_none_for_admin(self):
        ...
        context = InsightsQLContext(team_id=self.team.pk, team=self.team, user=self.user)
        database = Database.create_for(team=self.team, user=self.user)
    ...
    def test_build_access_control_guard_returns_none_without_user(self):
        ...
        from insights.insightsql.database.schema.system import dashboards

        context = InsightsQLContext(team_id=self.team.pk, team=self.team, user=None)
        database = Database.create_for(team=self.team, user=None)
        context.database = database
    ...
    def test_blocked_ids_bind_as_single_sensitive_placeholder(self):
        ...
        sql = print_prepared_ast(prepared, context=context, dialect="datastore")
    ...
    def test_filtering_records_restricted_resource(self):
        ...
        context = InsightsQLContext(team_id=self.team.pk, team=self.team, user=self.user, enable_select_queries=True)
        prepared = prepare_ast_for_printing(
            parse_select("SELECT id FROM system.dashboards"), context=context, dialect="datastore"
        )
        assert prepared is not None
        print_prepared_ast(prepared, context=context, dialect="datastore")
...
class TestAccessControlIntegration(BaseTest):
    """Integration tests for access control in InsightsQL queries."""

    def _compile_select(self, query: str, context: InsightsQLContext) -> str:
        """Helper to compile a InsightsQL query to Datastore SQL."""
        from insights.insightsql.parser import parse_select

        node = parse_select(query)
        prepared = prepare_ast_for_printing(node, context=context, dialect="datastore")
        assert prepared is not None
        return print_prepared_ast(prepared, context=context, dialect="datastore")
...
@patch("hanzo_insights.feature_enabled", new=Mock(return_value=True))
class TestWarehouseTableAccessControl(BaseTest):
    ...
    def test_to_printed_insightsql_bypass_prints_warehouse_table_userless(self):
        # Guards the fail-closed fix: query runners print the response InsightsQL userless right after the
        ...
        with self.assertRaises(QueryError):
            to_printed_insightsql(query, self.team)
        printed = to_printed_insightsql(query, self.team, bypass_warehouse_access_control=True)
...
class TestWarehouseTableAccessControlFlagOff(BaseTest):
    """Regression guard: with insightsql-warehouse-access-control off, nothing is filtered."""
...
class TestWarehouseAccessControlEndToEnd(BaseTest):
    """End-to-end: execute_insightsql_query (the SQL editor path) raises QueryError
    on a denied warehouse table without ever reaching Datastore."""
    ...
    def test_execute_insightsql_query_raises_on_denied_warehouse_table_reached_through_a_join(self):
    ...
    def test_execute_insightsql_query_bypass_warehouse_access_control_skips_denial(self):
...
class TestWarehouseViewAccessControl(BaseTest):
    ...
    def setUp(self):
        ...
        self.allowed_view = DataWarehouseSavedQuery.objects.create(
            team=self.team,
            name="allowed_view",
            query={"kind": "InsightsQLQuery", "query": "SELECT 1 AS id"},
            columns={"id": "String"},
        )
    ...
    def test_shared_link_cache_key_differs_from_denied_member(self):
        ...
        self._create_ac(resource="warehouse_objects", access_level="none")
        query = InsightsQLQuery(query="SELECT id FROM denied_view")
        ...
        events_query = InsightsQLQuery(query="SELECT count() FROM events")
        assert (
    ...
    def test_allowed_materialized_view_still_resolves(self):
        ...
        # materialized data via insightsql_definition), so an allowed materialized view stays queryable.
