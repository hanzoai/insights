...
from products.review.backend.reviewer.tools.select_perspectives import PerspectiveSelectionDTO, apply_selection
...
from products.review.backend.temporal.types import TRIGGER_INBOX, TRIGGER_LABEL, ReviewPRWorkflowInputs
...
class ReviewPRWorkflow:
    ...
    async def run(self, inputs: ReviewPRWorkflowInputs) -> str:
        ...
        if acting.acting_user_id is None:
            workflow.logger.info(
                f"PR author '{meta.author_login}' is not a Insights org user on team {inputs.team_id}; "
