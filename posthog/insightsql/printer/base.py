...
from posthog.insightsql.ast import Constant, StringType
...
from posthog.insightsql.database.models import FunctionCallTable, Table
from posthog.insightsql.errors import ImpossibleASTError, QueryError, ResolutionError
from posthog.insightsql.escape_sql import escape_insightsql_identifier, escape_insightsql_string
from posthog.insightsql.functions import (
...
from posthog.insightsql.functions.mapping import (
    ALL_EXPOSED_FUNCTION_NAMES,
    INSIGHTSQL_COMPARISON_MAPPING,
    is_allowed_parametric_function,
)
from posthog.insightsql.printer.types import (
    JoinExprResponse,
    PrintableMaterializedColumn,
    PrintableMaterializedPropertyGroupItem,
)
from posthog.insightsql.resolver_utils import lookup_field_by_name
from posthog.insightsql.visitor import Visitor, clone_expr
...
class InsightsQLPrinter(Visitor[str]):
    ...
    def visit_join_expr(self, node: ast.JoinExpr) -> JoinExprResponse:
        ...
        if isinstance(node.type, ast.TableAliasType) or isinstance(node.type, ast.TableType):
            ...
            if isinstance(table_type.table, FunctionCallTable) and table_type.table.requires_args:
                ...
                raise QueryError(f"Table '{table_type.table.to_printed_insightsql()}' does not accept arguments")
    ...
    def visit_call(self, node: ast.Call):
        ...
        if node.name in INSIGHTSQL_COMPARISON_MAPPING:
            op = INSIGHTSQL_COMPARISON_MAPPING[node.name]
            if len(node.args) != 2:
            ...
            for stack_node in reversed(self.stack):
                if isinstance(stack_node, ast.SelectQuery):
                    break
                if stack_node != node and isinstance(stack_node, ast.Call) and find_insightsql_aggregation(stack_node.name):
                    raise QueryError(
    ...
    def visit_table_type(self, type: ast.TableType):
        return type.table.to_printed_insightsql()
    ...
    def visit_field_type(self, type: ast.FieldType):
        ...
        if (
            ...
            # :KLUDGE: Legacy person properties handling. Only used within non-InsightsQL queries, such as insights.
            if self.context.within_non_insightsql_query and field_sql == "events__pdi__person.properties":
                if self.context.modifiers.personsOnEventsMode != PersonsOnEventsMode.DISABLED:
                    field_sql = "person_properties"
                else:
                    field_sql = "person_props"
    ...
    def visit_insightsqlx_tag(self, node: ast.InsightsQLXTag):
        ...
        for attribute in node.attributes:
            if isinstance(attribute, ast.InsightsQLXAttribute) and attribute.name == "children":
        ...
        if children:
            children_contents = [
                self.visit(child) if isinstance(child, ast.InsightsQLXTag) else "{" + self.visit(child) + "}"
    ...
    def visit_insightsqlx_attribute(self, node: ast.InsightsQLXAttribute):
        if isinstance(node.value, ast.InsightsQLXTag):
    ...
    def _print_insightsql_identifier_or_index(self, name: str | int) -> str:
    ...
    def _merge_table_top_level_settings(self, settings: InsightsQLQuerySettings | None) -> dict[str, Any]:
    ...
    def _print_settings(self, settings: InsightsQLQuerySettings | dict[str, Any]) -> str | None:
        pairs = []
        ...
        for key, value in items:
            ...
            if isinstance(value, bool):
                ...
                formatted_items = ", ".join(self._print_insightsql_identifier_or_index(item) for item in value)
