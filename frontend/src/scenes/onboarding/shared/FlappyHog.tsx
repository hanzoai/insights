...
import flappyHogSplashSrc from './flappy-script/flappy-script-splash.png'
import robotHogSrc from './flappy-script/robot-script.png'
...
function checkCollision(hogY: number, pipes: Pipe[]): boolean {
    const script = getHogBounds(hogY)

    if (script.top < 0 || script.bottom > GAME_HEIGHT) {
        return true
    }
    ...
    return pipes.some(
        (pipe) =>
            script.right > pipe.x &&
            script.left < pipe.x + PIPE_WIDTH &&
            (script.top < pipe.gapY || script.bottom > pipe.gapY + PIPE_GAP)
    )
}
...
export function FlappyHogGame({ isActive = true }: { isActive?: boolean }): JSX.Element {
    ...
    useEffect(() => {
        ...
        script.src = robotHogSrc
        hogImageRef.current = script
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            {highScore > 0 && (
                <div className="mt-4">
                    <Button type="tertiary" size="small" onClick={handleClearHighScore}>
                        Clear high score
                    </Button>
                </div>
            )}
...
export function FlappyHog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }): JSX.Element {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Flappy Script" simple>
