...
class TestGroupsLimitPushdown(DatastoreTestMixin, APIBaseTest):
    def _team(self) -> Team:
        # Fresh team per test: Datastore isn't rolled back, and the inner team_id filter isolates this team's groups.
        return Team.objects.create(organization=self.organization)

    def _run(self, team: Team, query: str) -> tuple[str, list]:
        response = execute_insightsql_query(parse_select(query), team)
        assert response.datastore is not None
        return response.datastore, response.results
    ...
    def test_null_limit_does_not_crash_the_compiler(self):
        ...
        try:
            ...
            raise AssertionError("LIMIT NULL crashed the InsightsQL compiler with a TypeError")
        except Exception:
            pass
    ...
    def test_order_by_limit_returns_global_top_and_is_not_limited(self):
        ...
        # real-Datastore correctness: the global top-by-rank is returned
