...
class TestGetSandboxMcpConfigs(TestCase):
    ...
    def _expected_headers(self, *, read_only: bool = True, consumer: str = "insights-code") -> list[dict[str, str]]:
        return [
            ...
            {"name": "x-insights-project-id", "value": str(self.PROJECT_ID)},
    ...
    @parameterized.expand(
        [
            ("https://app.hanzo.ai", "https://mcp.hanzo.ai/mcp"),
            ("https://us.hanzo.ai", "https://mcp.hanzo.ai/mcp"),
            ("https://eu.hanzo.ai", "https://mcp-eu.hanzo.ai/mcp"),
            ("https://app.dev.insights.dev", "https://mcp.dev.insights.dev/mcp"),
        ]
    )
    ...
    def test_explicit_sandbox_mcp_url_takes_precedence(self) -> None:
        with patch("products.tasks.backend.temporal.process_task.utils.settings") as mock_settings:
            ...
            mock_settings.SITE_URL = "https://app.hanzo.ai"
            configs = get_sandbox_ph_mcp_configs(self.TOKEN, self.PROJECT_ID)
            assert configs == [
                McpServerConfig(
                    type="http",
        ...
        with patch("products.tasks.backend.temporal.process_task.utils.settings") as mock_settings:
            ...
            assert configs[0].headers == [
                *self._expected_headers(),
                {"name": "X-Insights-Task-Id", "value": "task-uuid-123"},
            ]
    ...
    def test_no_task_id_omits_attribution_header(self) -> None:
        with patch("products.tasks.backend.temporal.process_task.utils.settings") as mock_settings:
            ...
            mock_settings.SITE_URL = "https://app.hanzo.ai"
            configs = get_sandbox_ph_mcp_configs(self.TOKEN, self.PROJECT_ID)
            assert all(h["name"] != "X-Insights-Task-Id" for h in configs[0].headers)
    ...
    @parameterized.expand(
        [
            ...
            ("some-other-origin", "insights-code"),
            ("slack", "slack"),
...
class TestMcpServerConfigToDict(TestCase):
    def test_minimal_config(self) -> None:
        config = McpServerConfig(type="http", name="insights", url="https://mcp.hanzo.ai/mcp")
...
class TestFetchUserMcpServerConfigs(TestCase):
    ...
    def _expected_user_headers(self, *, consumer: str = "insights-code") -> list[dict[str, str]]:
        return [
            {"name": "Authorization", "value": f"Bearer {self.TOKEN}"},
            {"name": "x-insights-mcp-consumer", "value": consumer},
        ]
    ...
    def test_agent_shared_config_uses_agent_proxy_auth_without_internal_task_header(
        ...
        assert all(header["name"] != "X-Insights-Task-Id" for header in configs[0].headers)
    ...
    def test_strips_trailing_slash_from_api_url(self, mock_facade, mock_api_url) -> None:
        mock_api_url.return_value = "https://us.hanzo.ai/"
        mock_facade.return_value = [self._make_installation()]
...
class TestGetGitIdentityEnvVars(TestCase):
    ...
    def test_empty_name_falls_back_to_insights_user(self) -> None:
        user = self._make_user(first_name="", last_name="", email="anon@example.com")
        user.get_full_name.return_value = ""
        task = self._make_task(Task.OriginProduct.USER_CREATED, user=user)
        result = get_git_identity_env_vars(task)
        assert result["GIT_AUTHOR_NAME"] == "Insights User"
...
class TestBuildImportedMcpServerConfigs(TestCase):
    def test_returns_empty_for_none_empty_or_non_list(self):
        ...
        assert build_imported_mcp_server_configs([], {"insights"}) == []
    ...
    def test_builds_configs_dropping_collisions_and_malformed_entries(self):
        configs = build_imported_mcp_server_configs(
            [
                ...
                {"type": "sse", "name": "insights", "url": "https://shadow.example.com/mcp"},
    ...
    def test_collision_detection_is_case_insensitive(self):
        ...
        configs = build_imported_mcp_server_configs(
            [
                ...
                {"type": "http", "name": "INSIGHTS", "url": "https://b.example.com/mcp"},
...
class TestGetRelayedMcpServerNames(TestCase):
    ...
    def test_returns_valid_names(self):
        ...
        assert get_relayed_mcp_server_names(task_run, {"insights"}) == ["playwright", "internal-cli"]
    ...
    def test_drops_names_colliding_with_resolved_configs(self):
        task_run = self._task_run([{"name": "insights"}, {"name": "grafana"}, {"name": "playwright"}])

        # Names taken by already-resolved MCP configs (Insights MCP, MCP Store, imported) win.
        assert get_relayed_mcp_server_names(task_run, {"insights", "grafana"}) == ["playwright"]
...
@patch(
    ...
    return_value="https://us.hanzo.ai",
)
class TestBuildSandboxEnvironmentVariablesGateway(TestCase):
    ...
    @override_settings(
        SANDBOX_AI_GATEWAY_URL="https://ai-gateway.us.hanzo.ai",
        SANDBOX_AI_GATEWAY_PRODUCTS="signals_scout,signals_research",
    )
    def test_both_settings_inject_both_vars(self, _api, _jwt):
        env = self._build()
        self.assertEqual(env["AI_GATEWAY_URL"], "https://ai-gateway.us.hanzo.ai")
        self.assertEqual(env["AI_GATEWAY_PRODUCTS"], "signals_scout,signals_research")

    @override_settings(SANDBOX_AI_GATEWAY_URL="https://ai-gateway.us.hanzo.ai", SANDBOX_AI_GATEWAY_PRODUCTS=None)
