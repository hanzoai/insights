...
from insights.models import FeatureFlag, Team
from insights.models.organization import Organization
from insights.tasks.feature_flags import compute_feature_flag_metrics
...
class TestComputeFeatureFlagMetrics(BaseTest):
    def setUp(self) -> None:
        ...
        self.patcher = patch("insights.tasks.utils.pushed_metrics_registry", return_value=self.mock_context)
        self.patcher.start()
        self.settings_patcher = patch(
            "insights.tasks.feature_flags.settings.PROM_PUSHGATEWAY_ADDRESS", "http://localhost:9091"
        )
    ...
    def test_skips_queries_when_pushgateway_not_configured(self) -> None:
        ...
        with patch("insights.tasks.feature_flags.settings") as mock_settings:
