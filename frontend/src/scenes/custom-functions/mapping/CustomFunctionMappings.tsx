...
import { EntityTypes, CustomFunctionConfigurationType, CustomFunctionMappingType } from '~/types'
...
export const MappingSummary = memo(function MappingSummary({
    ...
    mapping: CustomFunctionMappingType
}): JSX.Element | null {
...
export function CustomFunctionMapping({
    index,
    mapping,
    onChange,
    parentConfiguration,
    ...
    index: number
    mapping: CustomFunctionMappingType
    onChange: (mapping: CustomFunctionMappingType | null) => void
    parentConfiguration: Pick<CustomFunctionConfigurationType, 'inputs_schema' | 'inputs'>
}): JSX.Element | null {
    const { groupsTaxonomicTypes } = useValues(groupsModel)
    const { showSource, sampleGlobalsWithInputs } = useValues(customFunctionConfigurationLogic)
...
export function CustomFunctionMappings(): JSX.Element | null {
    const { useMapping, mappingTemplates, configuration } = useValues(customFunctionConfigurationLogic)
    ...
    return (
        <LemonField name="mappings">
            {({
                ...
                value: CustomFunctionMappingType[] | undefined
                onChange: (mappings: CustomFunctionMappingType[]) => void
            }) => {
                ...
                const duplicateMapping = (mapping: CustomFunctionMappingType): void => {
                ...
                const removeMapping = (mapping: CustomFunctionMappingType): void => {
                    const index = mappingsValue.findIndex((m) => m === mapping)
                    if (index !== -1) {
                ...
                const toggleDisabled = (mapping: CustomFunctionMappingType): void => {
                    const index = mappingsValue.findIndex((m) => m === mapping)
                    if (index !== -1) {
                ...
                const renameMapping = (mapping: CustomFunctionMappingType): void => {
