...
Tickets live in the conversations product and are queryable via the Insights MCP `execute-sql` tool against `system.support_tickets` (project 2, US).
...
| Count drop over time                         | "pageviews declined", "tracking loss"                           | Layer split: raw stored counts vs query-side exclusion. `$pageview` vs `$pageleave` ratio, UA segmentation, SDK version pin. Bot-shaped traffic disappearing is common and is not a Insights bug |
...
| Channel type misclassification               | "shows as Direct", "wrong channel"                              | `insights/models/channel_type/channel_definitions.json` + the decision tree in `insights/insightsql/database/schema/channel_type.py`; unknown source + stripped referrer falls through to Direct       |
...
- Production query-side checks (per-team event series, UA splits, ingestion warnings): the `query-datastore-via-metabase` skill covers prod-us and prod-eu access.
