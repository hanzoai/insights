...
from insights.api.file_system.file_system_logging import log_api_file_system_view
from insights.api.file_system.folder_context_generation import (
    ContextGenerationSerializer,
    ContextGenerationSetSerializer,
)
from insights.api.file_system.folder_context_generation_service import (
    get_context_generation_task_id,
    set_context_generation_task_id,
)
from insights.api.file_system.folder_instructions import (
    FolderInstructionsPublishSerializer,
    FolderInstructionsSerializer,
    FolderInstructionsVersionSerializer,
)
from insights.api.file_system.folder_instructions_service import (
...
from insights.models.file_system.file_system_view_log import get_recent_file_system_items, recent_view_logs
from insights.models.file_system.unfiled_file_saver import save_unfiled_files
from insights.models.team import Team
from insights.models.user import User
from insights.temporal.oauth import SANDBOX_OAUTH_APP_CLIENT_IDS
...
class FileSystemViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def _scope_by_project_and_environment(self, queryset: QuerySet) -> QuerySet:
        """
        Show all objects belonging to the project, except for script functions, which are scoped by team.
        """
    ...
    def move(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        # Repair folder tree for items we *didn't* move (script functions in other teams under the moved folder)
...
class DesktopFileSystemViewSet(FileSystemViewSet):
    ...
    def _announce_canvas_created(self, request: Request, dashboard: FileSystem) -> None:
        ...
        The task sandbox stamps every MCP call with an X-Insights-Task-Id header, so
        ...
        """
        raw_task_id = (request.headers.get("X-Insights-Task-Id") or "").strip()
        try:
