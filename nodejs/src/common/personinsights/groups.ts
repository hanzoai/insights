...
export class PersonHogGroupOperations {
    ...
    > {
        ...
        for (let i = 0; i < teamIds.length; i += PERSONFN_BATCH_SIZE) {
            const batchTeamIds = teamIds.slice(i, i + PERSONFN_BATCH_SIZE)
            const batchGroupTypeIndexes = groupTypeIndexes.slice(i, i + PERSONFN_BATCH_SIZE)
            const batchGroupKeys = groupKeys.slice(i, i + PERSONFN_BATCH_SIZE)
