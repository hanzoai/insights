...
from insights.models.utils import CreatedMetaFields, DeletedMetaFields, UpdatedMetaFields, UUIDModel, UUIDTModel
...
class Thread(UUIDTModel, DeletedMetaFields):
    """A runtime conversation thread (LangGraph or sandbox), checkpointed below.

    Distinct from `agent_memory.Conversation`, the assistant API's message log;
    this row is what `message_routing.SandboxSession` locks and drives.
    """
    ...
    slack_workspace_domain = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Slack workspace subdomain (e.g. 'insights' for insights.slack.com)",
    )
...
class ConversationCheckpointBlob(UUIDTModel):
    ...
    """
    thread = models.ForeignKey(Thread, on_delete=models.CASCADE, related_name="blobs", null=True)
    checkpoint_ns = models.TextField(
        default="",
        help_text='Checkpoint namespace. Denotes the path to the subgraph node the checkpoint originates from, separated by `|` character, e.g. `"child|grandchild"`. Defaults to "" (root graph).',
    )
    ...
    async def aappend_core_memory(self, text: str) -> None:
        ...
        if len(new_text) > CORE_MEMORY_MAX_CHARACTERS:
            raise ValueError(
                ...
                " Please free up space in Settings → Insights AI before adding new memories."
            )
        self.text = new_text
        await self.asave()
    ...
    async def areplace_core_memory(self, original_fragment: str, new_fragment: str) -> None:
        ...
        if len(new_text) > CORE_MEMORY_MAX_CHARACTERS:
            raise ValueError(
                ...
                " Please free up space in Settings → Insights AI first."
            )
        self.text = new_text
        await self.asave()
...
class AgentArtifact(UUIDModel, CreatedMetaFields, UpdatedMetaFields, DeletedMetaFields):
    ...
    conversation = models.ForeignKey(Thread, on_delete=models.CASCADE, related_name="artifacts")
    team = models.ForeignKey(Team, on_delete=models.CASCADE)
