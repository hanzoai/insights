...
export function convertDataTableNodeToDataVisualizationNode(node: Node | null): Node | null {
    if (!isDataTableNodeWithInsightsQLQuery(node)) {
        return node
    }
 ...
 * For raw SQL / InsightsQL queries the client can't know without the backend — use
 ...
 * backend populated them (currently InsightsQL / SQL-editor responses). Always returns an array. */
...
// Mirrors escape_chars_map in insights/insightsql/escape_sql.py: backslash and control chars must be escaped so a quoted identifier round-trips losslessly through the InsightsQL parser.
const INSIGHTSQL_IDENTIFIER_ESCAPE_MAP: Record<string, string> = {
    '\b': '\\b',
    '\f': '\\f',
    '\r': '\\r',
    '\n': '\\n',
...
export function escapePropertyAsInsightsQLIdentifier(identifier: string): string {
    ...
    const escaped = Array.from(identifier, (c) => INSIGHTSQL_IDENTIFIER_ESCAPE_MAP[c] || c).join('')
    return !identifier.includes('"') ? `"${escaped}"` : `\`${escaped.replaceAll('`', '``')}\``
}

/** Quote each segment of a dotted InsightsQL reference independently. */
export function escapeDottedInsightsQLIdentifier(identifier: string): string {
    if (isQuoted(identifier) || !identifier.includes('.')) {
        return escapePropertyAsInsightsQLIdentifier(identifier)
    }

    return identifier
        .split('.')
        .map((segment) => escapePropertyAsInsightsQLIdentifier(segment))
        .join('.')
}
