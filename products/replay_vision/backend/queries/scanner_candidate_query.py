...
def eligibility_predicates() -> list[ast.Expr]:
    # Mirror the scan-time eligibility gate (fetch_session_events) on the same Datastore aggregates the scan reads, so
    ...
    def run(self) -> list[CandidateSession]:
        with tags_context(product=Product.REPLAY_VISION, feature=Feature.ENRICHMENT):
            response = execute_insightsql_query(
                ...
                settings=InsightsQLGlobalSettings(max_execution_time=self._max_execution_time_seconds),
    ...
    def _sampling_predicate(self) -> ast.Expr | None:
        ...
        return ast.CompareOperation(
            ...
            left=ast.Call(
                name="modulo",
                args=[
                    # concat rather than a second cityHash64 arg — InsightsQL pins cityHash64 to a single argument.
                    ast.Call(
                        name="cityHash64",
                        args=[
                            ast.Call(
