...
from insights.insightsql_queries.insights.funnels import FunnelTrendsUDF, FunnelUDF
from insights.insightsql_queries.insights.funnels.funnel_query_context import FunnelQueryContext
from insights.insightsql_queries.insights.funnels.funnel_time_to_convert import FunnelTimeToConvertUDF
from insights.insightsql_queries.insights.funnels.funnel_time_to_convert_bins import (
    ConversionTimeRange,
    compute_shared_bin_boundaries,
)
from insights.insightsql_queries.insights.funnels.funnel_validation_rules import (
...
from insights.insightsql_queries.query_runner import AnalyticsQueryRunner, ExecutionMode
...
from insights.insightsql_queries.validation.rules import DisallowUnsupportedDataWarehouseSettings
...
class FunnelsQueryRunner(AnalyticsQueryRunner[FunnelsQueryResponse]):
    ...
    def _calculate_single_period(
        self,
        funnel_class,
        timings_override: Optional[InsightsQLTimings] = None,
        ...
        # InsightsQLTimings is not thread-safe. Single-period runs fall back to the shared instance.
    ...
    def _run_in_parallel(self, tasks: list[Callable[[InsightsQLTimings], "_T"]], index_offset: int = 0) -> list["_T"]:
        """Run sub-query callables concurrently and return their results in order.

        Each task receives its own cloned `InsightsQLTimings` (not thread-safe to share). In unit tests
        ...
        def run(index: int, timings: InsightsQLTimings, query_tags: Optional[QueryTags] = None) -> None:
            try:
                # Worker threads start with an empty QueryTags ContextVar — restore the parent's
                # snapshot so execute_insightsql_query has the required feature/product tags.
    ...
    def _calculate_compare_tagged_rows(self) -> FunnelsQueryResponse:
        ...
        return FunnelsQueryResponse(
            ...
            timings=timings,
            insightsql=current_response.insightsql,
            modifiers=self.modifiers,
            resolved_date_range=ResolvedDateRangeResponse(
                date_from=self.query_date_range.date_from(),
                date_to=self.query_date_range.date_to(),
    ...
    def _run_time_to_convert_bounds(
        self, funnel: FunnelTimeToConvertUDF, timings: InsightsQLTimings
    ...
    def _run_time_to_convert_histogram(
        self, funnel: FunnelTimeToConvertUDF, boundaries: list[int], timings: InsightsQLTimings
    ) -> FunnelTimeToConvertResults:
        if not boundaries:
    ...
    def _execute_funnel_query(self, query: ast.SelectQuery, timings: InsightsQLTimings):
