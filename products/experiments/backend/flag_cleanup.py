...
# Insights SDK calls that read a flag, across languages — what the agent greps for.
FLAG_SDK_CALLS = (
    "isFeatureEnabled, getFeatureFlag, getFeatureFlagPayload, useFeatureFlag, useActiveFeatureFlags, "
    "onFeatureFlags, insights.isFeatureEnabled, insights.getFeatureFlag, feature_enabled, get_feature_flag"
)
...
def build_cleanup_prompt(experiment: Experiment, flag_key: str, plan: CleanupPlan) -> tuple[str, str]:
    ...
    description = "\n".join(
        [
            "Remove the scaffolding for a Insights experiment feature flag that is no longer needed, and open a draft pull request.",
            "",
            ...
            f'Search the repo for the flag key "{flag_key}" and for Insights SDK calls that read flags, e.g.:',
            f"  {FLAG_SDK_CALLS}",
            ...
            "- Code only. Do NOT change the flag in Insights, and do NOT touch unrelated code.",
