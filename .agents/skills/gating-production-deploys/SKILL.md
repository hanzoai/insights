...
description: >
  ...
  production deploy (a `commit_state_update` repository_dispatch to Insights/charts).
...
- a `commit_state_update` dispatch to `Insights/charts` (plus its deployer-token step).
...
```yaml
if: github.repository_owner == 'Insights' && vars.CD_DEPLOY_ENABLED == 'true'
```

Use this instead of hardcoding `github.repository == 'Insights/insights'`.
...
if: github.repository_owner == 'Insights' && vars.CD_DEPLOY_ENABLED == 'true' && github.ref == 'refs/heads/master'
...
(github.event_name == 'push' && github.ref == 'refs/heads/master' && needs.changes.outputs.files == 'true'
  && github.repository_owner == 'Insights' && vars.CD_DEPLOY_ENABLED == 'true')
