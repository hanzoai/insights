...
For the current Journey 1/2 scope every node is a pure-InsightsQL display node, so a
...
A run fetches up to `cache_limit` rows in one Datastore query and keeps them in
an in-memory per-run cache; `/page` requests within the cache are local slices
(no Datastore work, no held backend workers). Only paging beyond the cache — or
...
def request_interrupt(run_id: str) -> bool:
    ...
    state["event"].set()
    if state["kernel"]:
        # Deferred so a pure-InsightsQL interrupt never imports jupyter_client (heavy, sandbox-only);
...
def execute_run(payload: dict[str, Any]) -> None:
    ...
    runs in the ipykernel (materialize inputs, run code); a pure-InsightsQL display node
...
def fetch_page(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    on-disk result frame (its code is not a InsightsQL query, so there is no data-plane fallback).
