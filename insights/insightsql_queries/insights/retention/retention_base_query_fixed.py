...
from insights.schema import EntityType, RetentionEntity

from insights.insightsql import ast
from insights.insightsql.parser import parse_expr
from insights.insightsql.property import property_to_expr

from insights.insightsql_queries.insights.retention.retention_base_query_builder import RetentionBaseQueryBuilder
from insights.insightsql_queries.insights.utils.breakdowns import ALL_USERS_COHORT_ID, has_breakdown_filter
...
class RetentionFixedIntervalBaseQueryBuilder(RetentionBaseQueryBuilder):
    ...
    def _breakdown_extract_targets_events_table(self) -> bool:
        ...
        # data_warehouse_person_property go via joins or a constant, and insightsql is
    ...
    def _build_single_scan_query(
        ...
        start_of_interval_sql = self.query_date_range.get_start_of_interval_insightsql(source=timestamp_field)
    ...
    def _anchored_start_event_timestamps_expr(self, anchor_expr: ast.Expr) -> ast.Expr:
        ...
        return parse_expr(
            "if({within_window}, [{bucketed_anchor}], [])",
            {
                ...
                "bucketed_anchor": self.query_date_range.date_to_start_of_interval_insightsql(anchor_expr),
            },
        )
    ...
    def build_base_query_legacy(
        ...
        if self.has_property_aggregation:
            ...
            # inlining the groupArrayIf expressions. This prevents Datastore from creating a self-join on the events
        ...
        # This ensures Datastore uses the pre-aggregated arrays rather than re-executing the groupArrayIf inside
