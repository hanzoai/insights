...
Field-level contracts for eval authors. Paths are relative to `products/insights_ai/eval_harness/` unless noted.
...
The dataset is byte-for-byte reproducible under the fixed `EVAL_SEED` (`products/insights_ai/eval_harness/data_setup.py`), and noise generators use fixed seeds.

- Reference events, properties, flags, and insight names **exactly** as listed in the Hedgebox dataset reference (`products/insights_ai/evals/AGENTS.md`).
