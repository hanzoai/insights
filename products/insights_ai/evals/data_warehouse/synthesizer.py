...
# The InsightsQL field class is what drives ``information_schema.columns.data_type``.
# Only field classes in the warehouse ``STR_TO_INSIGHTSQL_MAPPING`` resolve to a real
...
# Datastore type is only the storage hint stored on the table's columns JSON.
_DATASTORE_FOR_INSIGHTSQL: dict[str, str] = {
...
class SynthColumn:
    ...
    @property
    def datastore_base(self) -> str:
        """The bare Datastore type (no ``Nullable`` wrapper) — used for CSV-backed tables."""
        return _DATASTORE_FOR_INSIGHTSQL[self.insightsql]

    @property
    def datastore(self) -> str:
        base = self.datastore_base
        return f"Nullable({base})" if self.nullable else base

    def to_columns_entry(self) -> tuple[str, dict[str, Any]]:
        return self.name, {"insightsql": self.insightsql, "datastore": self.datastore, "valid": True}
...
class SynthJoin:
    source_table: str
    source_key: str  # bare column name (a valid InsightsQL field chain)
    joining_table: str
    joining_key: str
...
class WarehouseSchemaSynthesizer:
    ...
    def _validate(self, warehouse: SynthWarehouse) -> None:
        ...
        decimal_tables = [t.name for t in warehouse.tables for c in t.columns if c.insightsql == "DecimalDatabaseField"]
