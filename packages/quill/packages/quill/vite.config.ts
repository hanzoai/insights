...
/**
 * Builds the JavaScript entry for @hanzo/quill. The CSS pipeline is
 ...
 *
 * IMPORTANT: `@hanzo/quill-primitives`, `@hanzo/quill-components`,
 * and `@hanzo/quill-blocks` are **not** externalized. They are
 ...
 * published dist would have `import from '@hanzo/quill-primitives'`
 ...
 *
 * `@hanzo/quill-tokens` stays external because it is independently
...
export default defineConfig({
    plugins: [
        dts({
            ...
            // @hanzo/quill-primitives / -components / -blocks (which
            ...
            rollupTypes: true,
            bundledPackages: ['@hanzo/quill-primitives', '@hanzo/quill-components', '@hanzo/quill-blocks'],
        }),
    ],
    build: {
        lib: {
