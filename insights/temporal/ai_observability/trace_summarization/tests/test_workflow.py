...
@patch(
    "insights.temporal.ai_observability.trace_summarization.sampling.Heartbeater",
    _noop_heartbeater,
)
class TestSampleItemsInWindowActivity:
    ...
    async def test_sample_traces_success(self, mock_team):
        ...
        with patch(
            "insights.temporal.ai_observability.trace_summarization.sampling.execute_insightsql_query"
        ) as mock_execute:
    ...
    async def test_generation_filter_is_applied_inside_argmaxif(self, mock_team):
        ...
        with patch(
            ...
            # Validate the query against the real InsightsQL printer — every function call
            ...
            await database_sync_to_async(prepare_and_print_ast, thread_sensitive=False)(
                materialized,
                InsightsQLContext(team_id=mock_team.id, enable_select_queries=True),
                "datastore",
            )
