...
from insights.rbac.user_access_control import NO_ACCESS_LEVEL
...
# Resolved names are interpolated into InsightsQL ``FROM`` clauses. Warehouse table names are
...
def resolve_github_tables(
    *,
    ...
    The curated InsightsQL runs team-scoped with no user and InsightsQL does not enforce per-user ACL on
