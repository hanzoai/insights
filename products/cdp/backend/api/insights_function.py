...
from products.cdp.backend.models.insights_functions.insights_function_revision import InsightsFunctionRevision
...
DRAFT_CONTENT_FIELDS = ("script", "inputs_schema", "inputs", "filters", "mappings", "masking")
...
def _publish_confirm_value(insights_function: InsightsFunction) -> str:
    draft_updated_at = insights_function.draft_updated_at.isoformat() if insights_function.draft_updated_at else ""
    updated_at = insights_function.updated_at.isoformat() if insights_function.updated_at else ""
    return f"{insights_function.id}:{draft_updated_at}:{updated_at}"


def mint_publish_confirm_token(insights_function: InsightsFunction) -> str:
    return TimestampSigner(salt=_PUBLISH_CONFIRM_SALT).sign(_publish_confirm_value(insights_function))
...
def split_content_secrets(content: dict) -> dict:
    ...
    Mirrors `InsightsFunction.move_secret_inputs` for the draft: a draft's secrets belong in
...
def snapshot_insights_function_content(insights_function: InsightsFunction) -> dict:
    snapshot = {field: getattr(insights_function, field) for field in DRAFT_CONTENT_FIELDS}
...
def comparable_content(content: dict) -> dict:
    ...
    `refresh_affected_insights_functions` recompiling filter bytecode after an action or cohort edit — so
...
def draft_changed_fields(insights_function: InsightsFunction) -> list[str]:
    ...
    live = comparable_content(snapshot_insights_function_content(insights_function))
    draft = comparable_content(insights_function.draft or {})
    changed = [field for field in DRAFT_CONTENT_FIELDS if draft.get(field) != live.get(field)]

    staged_secrets = insights_function.draft_encrypted_inputs or {}
    live_secrets = insights_function.encrypted_inputs or {}
...
class StaleInsightsFunctionUpdateError(exceptions.APIException):
...
class InsightsFunctionSerializer(InsightsFunctionMinimalSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "draft": {
                "help_text": (
                    "Config staged for review but not live yet: a full snapshot of script, inputs_schema, inputs, "
    ...
    def to_representation(self, data):
        is_instance = isinstance(data, InsightsFunction)
...
class InsightsFunctionInvocationSerializer(serializers.Serializer):
    configuration = InsightsFunctionSerializer(
...
class InsightsFunctionRevisionBasicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = InsightsFunctionRevision
        fields = ["version", "created_at", "created_by"]
        read_only_fields = fields


class InsightsFunctionRevisionSerializer(InsightsFunctionRevisionBasicSerializer):
    class Meta(InsightsFunctionRevisionBasicSerializer.Meta):
        fields = [*InsightsFunctionRevisionBasicSerializer.Meta.fields, "content"]
        read_only_fields = fields


class InsightsFunctionRevisionRestoreRequestSerializer(serializers.Serializer):
...
class InsightsFunctionPublishRequestSerializer(serializers.Serializer):
...
class InsightsFunctionPublishResponseSerializer(serializers.Serializer):
    ...
    changed_fields = serializers.ListField(
        ...
        allow_null=True,
        help_text=(
            "Config fields publishing would change (script, inputs_schema, inputs, filters, mappings, masking). "
            "Only set on previews."
        ),
    )
    function = InsightsFunctionSerializer(
        required=False,
        allow_null=True,
...
class InsightsFunctionViewSet(
    ...
    def _draft_test_configuration(self, insights_function: Optional[InsightsFunction]) -> dict:
        ...
        if insights_function is None or not insights_function.draft:
            raise exceptions.ValidationError({"use_draft": "This function has no staged draft to test."})

        serialized = self.get_serializer(insights_function).data
        ...
        inputs.update(insights_function.draft_encrypted_inputs or {})
    ...
    def invocations(self, request: Request, *args, **kwargs):
        ...
        if data.get("use_draft"):
            data = {**data, "configuration": self._draft_test_configuration(insights_function)}
        ...
        serializer = InsightsFunctionInvocationSerializer(
            data=data, context={**self.get_serializer_context(), "instance": insights_function}
        )
    ...
    def _sent_content_fields(self) -> set[str]:
        ...
        # tell us this — and trusting it would let a script-only edit overwrite already-staged inputs
        # with the live ones.
    ...
    def _should_route_to_draft(self, serializer: BaseSerializer, revisions_enabled: bool) -> bool:
        ...
        if not isinstance(instance, InsightsFunction) or not instance.enabled:
            return False
        if instance.type != InsightsFunctionType.DESTINATION:
            return False
        return bool(self._sent_content_fields())

    def _write_draft(
        self, instance: InsightsFunction, locked: InsightsFunction, serializer: BaseSerializer, validated_content: dict
    ) -> None:
        ...
        draft = {**snapshot_insights_function_content(locked), **(locked.draft or {})}
        for field in sent:
    ...
    def _record_revision(self, instance: InsightsFunction, before: InsightsFunction, before_content: dict) -> None:
        ...
        after_content = snapshot_insights_function_content(instance)
        ...
        InsightsFunction.objects.filter(pk=instance.pk).update(version=instance.version)
        ...
        if not InsightsFunctionRevision.objects.filter(insights_function=instance).exists():
            InsightsFunctionRevision.objects.create(
                team_id=self.team_id,
                insights_function=instance,
    ...
    def perform_update(self, serializer):
        ...
        if (
            ...
            and isinstance(serializer.instance, InsightsFunction)
        ...
        with transaction.atomic():
            try:
                ...
                locked = InsightsFunction.objects.select_for_update().get(pk=instance_id)
            ...
            if base_updated_at and guard_timestamp and guard_timestamp > base_updated_at:
                raise StaleInsightsFunctionUpdateError()
            if route_to_draft:
                ...
                before_content = snapshot_insights_function_content(locked) if locked else None
                serializer.save()
    ...
    def publish(self, request: Request, *args, **kwargs):
        ...
        param_serializer = InsightsFunctionPublishRequestSerializer(data=request.data)
        ...
        with transaction.atomic():
            ...
            # nosemgrep: idor-lookup-without-team (re-fetch of already-authorized instance for activity logging)
            before_update = InsightsFunction.objects.get(pk=instance.pk)
            before_content = snapshot_insights_function_content(before_update)
        ...
        log_activity_from_viewset(
            ...
            name=locked.name,
            previous=before_update,
            detail_type=humanize_insights_function_type(locked.type),
        )
    ...
    @extend_schema(request=None, responses={200: InsightsFunctionSerializer})
    @action(detail=True, methods=["POST"])
    ...
    @extend_schema(responses={200: InsightsFunctionRevisionBasicSerializer(many=True)})
    # filter_backends=[]: don't inherit the viewset's InsightsFunction filterset — its fields would be
    ...
    @extend_schema(
        ...
        responses={200: InsightsFunctionRevisionSerializer},
        filters=False,
    )
    ...
    def revision_detail(self, request: Request, version: Optional[str] = None, *args, **kwargs):
        ...
        instance = self.get_object()
        try:
            revision = InsightsFunctionRevision.objects.get(insights_function=instance, version=int(version or 0))
        except InsightsFunctionRevision.DoesNotExist:
            raise exceptions.NotFound("No such revision for this function.")
        return Response(InsightsFunctionRevisionSerializer(revision).data)

    @extend_schema(
        parameters=[
            OpenApiParameter("version", int, OpenApiParameter.PATH, description="Function version to restore.")
        ],
        request=InsightsFunctionRevisionRestoreRequestSerializer,
        responses={200: InsightsFunctionSerializer},
        filters=False,
    )
    ...
    def restore_revision(self, request: Request, version: Optional[str] = None, *args, **kwargs):
        ...
        if not use_destinations_revisions(self.team):
            raise exceptions.ValidationError(REVISIONS_DISABLED_MESSAGE)
        param_serializer = InsightsFunctionRevisionRestoreRequestSerializer(data=request.data)
        param_serializer.is_valid(raise_exception=True)
