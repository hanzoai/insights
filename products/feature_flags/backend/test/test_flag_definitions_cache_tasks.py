...
class TestRefreshExpiringFlagDefinitionsCacheEntries(PushGatewayTaskTestMixin, TestCase):
    @patch("insights.storage.cache_expiry_manager.refresh_expiring_caches")
    def test_refreshes_cache(self, mock_refresh: MagicMock) -> None:
        ...
        assert self.registry.get_sample_value("insights_flag_definitions_cache_refresh_successful_count") == 5
        assert self.registry.get_sample_value("insights_flag_definitions_cache_refresh_failed_count") == 0
...
class TestCleanupStaleFlagDefinitionsExpiryTrackingTask(PushGatewayTaskTestMixin, TestCase):
    @patch("insights.storage.cache_expiry_manager.cleanup_stale_expiry_tracking")
