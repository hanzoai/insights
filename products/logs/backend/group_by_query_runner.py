...
# InsightsQL expression that yields their display value. trace_id/span_id are stored
...
class LogsGroupByQueryRunner(AnalyticsQueryRunner[LogsQueryResponse], LogsQueryRunnerMixin):
    ...
    def to_query(self) -> ast.SelectQuery:
        ...
        # '' (native subscript), and InsightsQL's `!=` lets NULL through, so both are excluded via
        ...
        # nosemgrep: insightsql-fstring-audit - only interpolates the int self.group_limit, the module-level _TOTALS_WINDOW constant, and generated g0..gN aliases (no user input); grouping keys and date bounds flow in as ast placeholders
        query = parse_select(
            f"""
            SELECT
    ...
    def _calculate(self) -> LogsQueryResponse:
        # The group-by templates only reference insights-native tables (logs, log_attributes)
        # and the grouping key is a bound constant, so hand the executor a plain insights-only
