...
use insights_symbol_data::{read_symbol_data_with_byte_count, AppleDsym, ElfDebugInfo};
...
impl ParsedNativeSymbols {
    ...
    ///    `dir="InsightsExample"`) symcache resolves `full_path` as
    ///    `"/InsightsExample/Foo.swift"`, but the CLI built the manifest from the
    ///    callee CU's absolute path `"/Users/…/InsightsExample/Foo.swift"`.
...
/// `ElfDebugInfo` bundles (uploaded via `insights-cli debug-symbols upload`)
/// and `AppleDsym` bundles (uploaded via `insights-cli dsym upload`), so
