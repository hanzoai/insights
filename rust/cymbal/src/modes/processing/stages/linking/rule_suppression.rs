...
impl ValueOperator for RuleSuppression {
    ...
    async fn execute_value(&self, input: Self::Item, ctx: LinkingStage) -> OperatorResult<Self> {
        let mut rules = ctx
            .app_context
            .team_manager
            .get_suppression_rules(&ctx.app_context.insights_pool, input.team_id())
            .await?;
            ...
            match rule.should_suppress(&props_json, &input.uuid()) {
                ...
                Err(VmError::OutOfResource(resource)) if resource == "steps" => {
                    tracing::warn!(
                        ...
                        "suppression rule exceeded ScriptVM step budget for this event, skipping"
                    );
                    continue;
                }
                Err(err) => {
                    rule.disable(
                        &ctx.app_context.insights_pool,
                        err.to_string(),
                        props_json.clone(),
                    )
                    .await
