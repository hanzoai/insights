            ...
            startBulkScan: async ({ sessionIds }) => {
                ...
                try {
                    ...
                    if (started > 0) {
                        ...
                        toast.warning("No scans started — you've hit the in-flight or monthly credit limit.")
                    } else {
                        toast.error('No scans started. Please try again.')
                    }
                    // Started scans create pending observations server-side — refetch to reflect them.
                    actions.loadObservations()
                } catch (error: any) {
                    toast.error(`Bulk scan failed${error?.detail ? `: ${error.detail}` : ''}`)
                } finally {
                    actions.bulkScanDone()
                }
            },
