# ruff: noqa: T201
"""Route the Claude Agent SDK through Insights's internal Go ai-gateway.
...
def analytics_extra_properties() -> dict[str, object]:
    ...
    """
    raw = os.environ.get("STAMPFN_EXTRA_PROPERTIES", "")
    if not raw:
        return {}
    try:
...
def resolve_gateway_config() -> tuple[str, str] | None:
    """Validated (anthropic_base_url, sk-api_key), or None to use direct Anthropic.
...
def _properties_header(properties: dict[str, object]) -> str:
    # Single X-Insights-Properties JSON blob: the slugless Go gateway merges it onto
    # $ai_generation and ignores per-property x-insights-property-* headers. None
    ...
    if not clean:
        return ""
    return f"X-Insights-Properties: {json.dumps(clean, separators=(',', ':'))}"


def gateway_env(base_url: str, api_key: str, properties: dict[str, object]) -> dict[str, str]:
    # sk- secret on both auth vars (SDK sends it as Bearer or x-api-key).
    return {
