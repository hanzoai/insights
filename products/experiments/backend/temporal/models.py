...
# Retry delay for a calc attempt that bounced off the per-org Datastore concurrency limiter or the cluster's
...
# burns a full Datastore query per attempt without changing the outcome, so the activity fails non-retryable
...
# Datastore max_execution_time for the recalc metric query. Deliberately below
# METRIC_CALC_ACTIVITY_TIMEOUT_SECONDS (minus headroom for metric build, stats, and persistence) so a slow
# query fails INSIDE the activity with a typed Datastore error (159 TIMEOUT_EXCEEDED) instead of the activity
...
# also stops the orphaned query from burning Datastore for the full default 600s.
