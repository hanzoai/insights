...
submitting SDK-style internal telemetry events for Insights's own team (team 2) — the
hanzo_insights SDK integration handles that separately.
...
SDK_INFO = "insights-capture-v1-internal/1.0"
...
def _build_v1_headers(token: str, attempt: int) -> dict[str, str]:
    return {
        ...
        "User-Agent": SDK_INFO,
        "Insights-Sdk-Info": SDK_INFO,
        "Insights-Attempt": str(attempt),
        "Insights-Request-Id": str(uuid4()),
        "Insights-Request-Timestamp": datetime.now(UTC).isoformat(),
    }
        ...
        while True:
            ...
            try:
                ...
                logger.warning(
                    ...
                    event_source=event_source,
                    request_id=headers.get("Insights-Request-Id", ""),
...
def capture_internal(
    *,
    ...
    app on behalf of non-Insights-admin teams/projects.  PLEASE DO NOT write events directly
