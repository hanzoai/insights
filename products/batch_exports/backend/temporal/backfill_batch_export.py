...
# Datastore slot for hours. The estimate is advisory: when the timeout fires
...
async def _get_backfill_info_for_events(
    ...
    if start_at is not None:
        ...
        # Convert to UTC to avoid Datastore timezone parsing issues (e.g., UTC+05:45)
    ...
    # Datastore returns 1970-01-01 00:00:00 when there's no data
...
async def _get_backfill_info_for_sessions(
    ...
    Queries the sessions table via InsightsQL, filtering by $end_timestamp
...
async def get_backfill_info(inputs: GetBackfillInfoInputs) -> GetBackfillInfoOutputs:
    ...
    except DatastoreQueryTimeoutError:
        logger.warning(
    ...
    except DatastoreMemoryLimitExceededError:
        logger.warning(
    ...
    except DatastoreTooManyBytesError:
        logger.warning(
