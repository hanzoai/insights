...
from insights.temporal.ai_observability.metrics import ExecutionTimeRecorder
...
@parameterized.expand(
    [
        ...
        # registered for bucket overrides in insights/temporal/common/worker.py. If a future refactor renames
...
def test_registered_histogram_names_are_emitted(name: str, histogram_name: str):
    ...
    # ExecutionTimeRecorder.__exit__ calls bare get_metric_meter() inside insights.temporal.common.metrics,
    # so the patch target is the original definition (the ai_observability module just re-exports it).
    with patch("insights.temporal.common.metrics.get_metric_meter", return_value=mock_meter):
...
async def test_workflow_interceptor_emits_finished_failed_on_hard_failure():
    ...
    with (
        ...
        # Patch target is insights.temporal.common.metrics — that's where ExecutionTimeRecorder's bare
        # get_metric_meter() call resolves. Patching the ai_observability re-export leaves the real call live.
        patch("insights.temporal.common.metrics.get_metric_meter", return_value=MagicMock()),
        patch(
