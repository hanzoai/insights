...
The v1 schema contract lives with the insightscli quarantine tooling
(``tools/insightscli-commands/insightscli_commands/quarantine/core.py``); product isolation
...
therefore ``insightscli test:quarantine add``) would — locked by a golden test — so the
