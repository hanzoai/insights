...
carries the new notebook's `short_id` and a clickable URL in `_insightsUrl` (the tool enriches the
...
- **Chart a SQL-fallback series** —
  `{kind: "DataVisualizationNode", source: {kind: "InsightsQLQuery", query: "SELECT ..."}, display: "ActionsLineGraph"}`.
  Do **not** wrap a `InsightsQLQuery` in an `InsightVizNode`.
...
evidence:
  ...
  - source_id: aB12cD34
    description: >
      ...
      https://us.hanzo.ai/project/41233/notebooks/aB12cD34
summary: |
