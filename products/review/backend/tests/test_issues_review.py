...
from products.review.backend.reviewer.artefact_content import ReviewIssueFinding
...
from products.review.backend.reviewer.tools.issues_review import _covered_findings_for_chunk, build_review_prompt
...
def test_review_prompt_pins_the_skill_and_injects_wave_lenses_for_the_blind_spot_check() -> None:
    ...
    prompt = _render_prompt(
        skill_name="review-script-blind-spots-general",
        skill_version=3,
        same_turn_findings=[_issue("a.py", "covered wave problem")],
        dig_deeper=True,
        blind_spot_check=True,
        wave_perspectives={"review-script-perspective-logic-correctness": "Checks the change's logic."},
    )

    assert 'skill-get(skill_name="review-script-blind-spots-general", version=3)' in prompt
    assert "review-script-perspective-logic-correctness" in prompt
...
def test_review_prompt_gives_regular_perspectives_no_cross_perspective_context() -> None:
    ...
    assert 'skill-get(skill_name="review-script-perspective-logic-correctness", version=2)' in prompt
