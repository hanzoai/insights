...
import pytest
from insights.test.base import DatastoreTestMixin, NonAtomicBaseTest
...
from insights.persons_seed import insert_seed_distinct_id, insert_seed_group, insert_seed_person

pytestmark = pytest.mark.persons_db_direct


@pytest.mark.ee
class TestSyncPersonsToDatastore(NonAtomicBaseTest, DatastoreTestMixin):
    CLASS_DATA_LEVEL_SETUP = False

    def setUp(self):
        super().setUp()
        # NonAtomicBaseTest (TransactionTestCase) commits ORM fixtures so the command's raw
        # persons-DB read can see them, but it doesn't trigger the per-test Datastore reset the
    ...
    def test_group_sync_updates_group(self, mocked_ch_call):
        ...
        with persons_db_connection(writer=True, autocommit=True) as conn, conn.cursor() as cursor:
            cursor.execute(
                "UPDATE insights_group SET group_properties = %s WHERE id = %s",
