...
import { Button, Input, SegmentedButton, Select, Switch } from '@hanzo/elements'
    ...
    return (
        <div className="flex flex-col gap-4 max-w-3xl">
            <div className="flex flex-col gap-3">
                <Field.Pure label="Name" error={samplingFormErrors.name}>
                    <Input
                ...
                <Field.Pure label="Enabled">
                    <Switch checked={samplingForm.enabled} onChange={(v) => setSamplingFormValue('enabled', v)} />
                </Field.Pure>
            </div>

            <SceneSection title="Action" titleSize="sm">
                <Field.Pure label="What to do when a log matches">
                    <SegmentedButton
                    ...
                    <div className="relative h-24 border border-border rounded-md bg-bg-light px-2 py-1">
                        {!hasFilters ? (
                            ...
                            <div className="h-full flex flex-col gap-1 items-center justify-center text-muted text-xs">
                                ...
                                <Button size="xsmall" type="secondary" onClick={refreshFilterPreview}>
                                    Retry
                                </Button>
                            </div>
                        ) : series.length === 0 ? (
