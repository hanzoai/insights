...
describe('DataTable utils', () => {
    ...
    it.each([
        ...
        // Double-quoted aliases (InsightsQL grammar treats double quotes as quoted identifier delimiters)
        [`properties.$city AS "City Name"`, 'City Name'],
        [
    ...
    describe('ColumnConfigurator setColumns should not leak stale columns', () => {
        ...
        it('preserves columns for InsightsQL queries', () => {
            const result: DataTableNode = {
                kind: NodeKind.DataTableNode,
                columns: ['*', 'event', 'timestamp'],
                source: { kind: NodeKind.InsightsQLQuery, query: 'SELECT *' },
            }
