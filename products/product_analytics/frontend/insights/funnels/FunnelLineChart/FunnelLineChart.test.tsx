...
describe('FunnelLineChart', () => {
    ...
    describe('legend', () => {
        it('shows a legend item per breakdown series when showLegend is enabled', async () => {
            ...
            await screen.findByLabelText(/chart with/i)
            const legend = await screen.findByTestId('script-chart-timeseries-line-legend')
        ...
        it.each([
            ...
            await screen.findByLabelText(/chart with/i)
            expect(screen.queryByTestId('script-chart-timeseries-line-legend')).not.toBeInTheDocument()
        })
