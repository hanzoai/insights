...
| `insights:query-error-tracking-issues-list` | Find error tracking issues linked to the session         |
| `insights:vision-observations-list`         | Check for an existing Replay Vision AI summary           |
| `insights:vision-scanners-list`             | Find summarizer scanners (`scanner_type=summarizer`)     |
| `insights:vision-scanners-scan-session`     | Run a summarizer scanner on the session (slow, optional) |
| `insights:vision-scanners-create`           | Create a temporary summarizer scanner (ask first)        |
| `insights:vision-scanners-delete`           | Delete a temporary scanner after summarizing             |
...
```json
insights:session-recording-get
{
  "id": "<recording_id>"
}
```
...
1. **Check for an existing summary.** A scheduled scanner may already have one:
   ...
   ```json
   insights:vision-observations-list
   {
...
3. **Scan the session** with the chosen scanner. Warn this is async and takes
   ...
   ```json
   insights:vision-scanners-scan-session
   {
     "id": "<scanner_id>",
     "session_id": "<session_id>"
   }
...
2. **Create it disabled** so it never sweeps on a schedule — a disabled scanner
   ...
   ```json
   insights:vision-scanners-create
   {
