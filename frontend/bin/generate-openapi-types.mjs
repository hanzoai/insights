 ...
 * 4. @validated_request decorator in insights/api/ or ee/ -> core
 * 5. Otherwise -> skipped
 */
 ...
 * so recursive types and deeply nested unions (like InsightsQL query schemas)
...
// reason. Catching it here makes `insightscli build:openapi` itself exit non-zero
...
// Exit non-zero on any failure so the wrapping `insightscli build:openapi` gates
