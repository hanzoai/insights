...
from insights.errors import InternalCHQueryError, wrap_datastore_query_error
from insights.models.property import PropertyValidationError
...
class TestUnevaluableFiltersAsValidationErrors(SimpleTestCase):
    ...
    def test_datastore_value_parse_failure_surfaces_as_a_caller_error(self, _name, code):
        ...
        err = wrap_datastore_query_error(ServerException(raw, code=code))
        with self.assertRaises(ValidationError) as ctx, unevaluable_filters_as_validation_errors():
            raise err
    ...
    def test_other_internal_datastore_errors_stay_server_faults(self):
        ...
        err = wrap_datastore_query_error(ServerException("Logical error: invariant violated", code=49))
