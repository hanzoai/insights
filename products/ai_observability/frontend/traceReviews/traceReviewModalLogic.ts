...
function loadActiveScoreDefinitionsPage(
    ...
    search: string,
    offset: number
): ReturnType<typeof aiObservabilityScoreDefinitionsList> {
    return aiObservabilityScoreDefinitionsList(String(teamId), {
        archived: false,
        order_by: 'name',
...
export interface traceReviewModalLogicValues {
    currentTeamId: number | null // teamLogic
    canSave: boolean
    comment: string
    currentReview: TraceReview | null
    definitionOptionsLoading: boolean
    definitionResultsLabel: string | null
    definitionSearch: string
    hasMoreDefinitions: boolean
    isBusy: boolean
    isFormValid: boolean
    isOpen: boolean
    loadedDefinitions: ScoreDefinitionApi[]
    modalDataLoading: boolean
    removing: boolean
    saving: boolean
    scoreValues: Record<string, TraceReviewFormScoreValue>
    selectableDefinitions: ScoreDefinitionApi[]
    selectedDefinitionIds: string[]
    selectedDefinitionSnapshots: Record<string, ScoreDefinitionApi>
    selectedDefinitions: ScoreDefinitionApi[]
    submitPayload: TraceReviewUpsertPayload
    totalDefinitionCount: number
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface traceReviewModalLogicActions {
    cacheTraceReview: (review: TraceReview) => {
        review: TraceReview
    } // traceReviewsLazyLoaderLogic
    ...
    loadDefinitionOptions: (replace?: boolean) => {
        replace: boolean
    }
    loadDefinitionOptionsFailure: () => {
        value: true
    }
    loadDefinitionOptionsSuccess: (
        definitions: ScoreDefinitionApi[],
        totalCount: number,
        replace: boolean
    ) => {
        definitions: ScoreDefinitionApi[]
    ...
    populateForm: (review: TraceReview | null) => {
        review: TraceReview | null
    }
    removeCurrentReview: () => {
        value: true
    }
    removeCurrentReviewFailure: () => {
        value: true
    }
    removeCurrentReviewSuccess: () => {
        value: true
    }
    removeSelectedDefinition: (definitionId: string) => {
        definitionId: string
    }
    resetForm: () => {
        value: true
    ...
    saveCurrentReviewFailure: () => {
        value: true
    }
    saveCurrentReviewSuccess: (review: TraceReview) => {
        review: TraceReview
    }
    selectDefinition: (definition: ScoreDefinitionApi) => {
        definition: ScoreDefinitionApi
    }
    setComment: (comment: string) => {
        comment: string
    }
    setDefinitionSearch: (search: string) => {
        search: string
    }
    setScoreValue: (
        definitionId: string,
        value: TraceReviewFormScoreValue
    ) => {
        definitionId: string
        value: TraceReviewFormScoreValue
    }
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface traceReviewModalLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        ...
        hasMoreDefinitions: (loadedDefinitions: ScoreDefinitionApi[], totalDefinitionCount: number) => boolean
        ...
        isBusy: (modalDataLoading: boolean, saving: boolean, removing: boolean) => boolean
        canSave: (isFormValid: boolean, isBusy: boolean) => boolean
        submitPayload: (
            selectedDefinitions: ScoreDefinitionApi[],
            scoreValues: Record<string, TraceReviewFormScoreValue>,
            comment: string,
...
export type traceReviewModalLogicType = MakeLogicType<
    traceReviewModalLogicValues,
    traceReviewModalLogicActions,
    TraceReviewModalLogicProps,
    traceReviewModalLogicMeta
>

export const traceReviewModalLogic = kea<traceReviewModalLogicType>([
    path(['products', 'ai_observability', 'frontend', 'traceReviews', 'traceReviewModalLogic']),
    ...
    selectors({
        ...
        selectableDefinitions: [
            ...
            (loadedDefinitions: ScoreDefinitionApi[], selectedDefinitionIds: string[]): ScoreDefinitionApi[] =>
        ...
        hasMoreDefinitions: [
            ...
            (loadedDefinitions: ScoreDefinitionApi[], totalDefinitionCount: number): boolean =>
                loadedDefinitions.length < totalDefinitionCount,
        ],
        ...
        isBusy: [
            ...
            (modalDataLoading: boolean, saving: boolean, removing: boolean): boolean =>
                modalDataLoading || saving || removing,
        ],

        canSave: [
            (s) => [s.isFormValid, s.isBusy],
            (isFormValid: boolean, isBusy: boolean): boolean => isFormValid && !isBusy,
        ],
    ...
    listeners(({ actions, values, props }) => ({
        ...
        loadModalData: async () => {
            ...
            try {
                ...
                toast.error('Failed to load the current trace review.')
                actions.loadModalDataFailure()
            }
        },
        ...
        saveCurrentReview: async () => {
            ...
            try {
                ...
                actions.populateForm(review)
                actions.cacheTraceReview(review)
                toast.success('Trace review saved.')
                actions.closeModal()
            } catch (error) {
                toast.error(`Failed to save trace review. ${parseErrorMessage(error)}`)
                actions.saveCurrentReviewFailure()
            }
        },
        ...
        removeCurrentReview: async () => {
            ...
            try {
                ...
                actions.setTraceAsUnreviewed(props.traceId)
                toast.info('Trace review removed.')
                actions.closeModal()
            } catch (error) {
                toast.error(`Failed to remove trace review. ${parseErrorMessage(error)}`)
                actions.removeCurrentReviewFailure()
            }
        },
    })),
