...
- Has the user called `insights.opt_out_capturing()`?
...
but nothing ever gets flushed to Insights.
...
never complete — so the recording data never reaches Insights's backend.
...
- Reverse proxy config: is `/s/` routed to Insights capture?
