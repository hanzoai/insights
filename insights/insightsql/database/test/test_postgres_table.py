...
from insights.insightsql.database.postgres_table import PostgresTable, build_function_call
...
_SCOPE_GATED_METADATA_TABLES: frozenset[str] = frozenset(
    {
        ...
        # grants the same rows at resource level (insights_flow:read) with no object-level check.
        "message_categories",
...
class TestPostgresTable(BaseTest):
    ...
    def test_persons_model_table_resolves_to_persons_db(self):
        ...
        persons_ctx = InsightsQLContext(team_id=self.team.pk)
        build_function_call("insights_group", persons_ctx)
        assert any(str(v).endswith("_persons") for v in persons_ctx.values.values())

        default_ctx = InsightsQLContext(team_id=self.team.pk)
        build_function_call("insights_dashboard", default_ctx)
        assert not any(str(v).endswith("_persons") for v in default_ctx.values.values())

    def test_select(self):
        self._init_database()
        self.assertEqual(
            self._select("SELECT * FROM postgres_table LIMIT 10", dialect="insightsql"),
    ...
    def test_single_predicate(self):
        ...
        self.assertEqual(
            self._select("SELECT id FROM postgres_table LIMIT 10", dialect="insightsql"),
        ...
        self.assertEqual(
            ...
            f"SELECT postgres_table.id AS id FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS postgres_table WHERE and(equals(postgres_table.team_id, {self.team.pk}), greaterOrEquals(postgres_table.created_at, minus(today(), toIntervalDay(30)))) LIMIT 10",
        )
    ...
    def test_multiple_predicates(self):
        ...
        self.assertEqual(
            self._select("SELECT id FROM postgres_table LIMIT 10"),
            f"SELECT postgres_table.id AS id FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS postgres_table WHERE and(and(equals(postgres_table.team_id, {self.team.pk}), greaterOrEquals(postgres_table.created_at, minus(today(), toIntervalDay(30)))), notEquals(postgres_table.status, %(insightsql_val_5)s)) LIMIT 10",
        )
    ...
    def test_predicate_combined_with_user_where(self):
        ...
        self.assertEqual(
            self._select("SELECT id FROM postgres_table where name = 'test' LIMIT 10", dialect="insightsql"),
    ...
    def test_left_join_single_predicate(self):
        ...
        self.assertEqual(
            ...
            f"SELECT other_table.id AS id, postgres_table.name AS name FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS other_table LEFT JOIN postgresql(%(insightsql_val_6_sensitive)s, %(insightsql_val_7_sensitive)s, %(insightsql_val_5_sensitive)s, %(insightsql_val_8_sensitive)s, %(insightsql_val_9_sensitive)s) AS postgres_table ON and(and(equals(postgres_table.team_id, {self.team.pk}), greaterOrEquals(postgres_table.created_at, minus(today(), toIntervalDay(30)))), equals(other_table.ref_id, postgres_table.id)) WHERE equals(other_table.team_id, {self.team.pk}) LIMIT 10",
        )
    ...
    def test_left_join_multiple_predicates(self):
        ...
        self.assertEqual(
            ...
            f"SELECT other_table.id AS id FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS other_table LEFT JOIN postgresql(%(insightsql_val_6_sensitive)s, %(insightsql_val_7_sensitive)s, %(insightsql_val_5_sensitive)s, %(insightsql_val_8_sensitive)s, %(insightsql_val_9_sensitive)s) AS postgres_table ON and(and(and(equals(postgres_table.team_id, {self.team.pk}), greaterOrEquals(postgres_table.created_at, minus(today(), toIntervalDay(30)))), notEquals(postgres_table.status, %(insightsql_val_10)s)), equals(other_table.ref_id, postgres_table.id)) WHERE equals(other_table.team_id, {self.team.pk}) LIMIT 10",
        )
    ...
    def test_inner_join_predicate(self):
        ...
        self.assertEqual(
            ...
            f"SELECT other_table.id AS id, postgres_table.name AS name FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS other_table INNER JOIN postgresql(%(insightsql_val_6_sensitive)s, %(insightsql_val_7_sensitive)s, %(insightsql_val_5_sensitive)s, %(insightsql_val_8_sensitive)s, %(insightsql_val_9_sensitive)s) AS postgres_table ON equals(other_table.ref_id, postgres_table.id) WHERE and(and(equals(postgres_table.team_id, {self.team.pk}), greaterOrEquals(postgres_table.created_at, minus(today(), toIntervalDay(30)))), equals(other_table.team_id, {self.team.pk})) LIMIT 10",
        )
    ...
    def test_lazy_join_with_predicate(self):
        ...
        self.assertEqual(
            self._select("SELECT details.name FROM other_table LIMIT 10", dialect="insightsql"),
            "SELECT details.name FROM other_table LIMIT 10",
        )
        self.assertEqual(
            self._select("SELECT details.name FROM other_table LIMIT 10"),
            f"SELECT other_table__details.name AS name FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS other_table LEFT JOIN (SELECT postgres_table.name AS name, postgres_table.id AS other_table__details___id FROM postgresql(%(insightsql_val_6_sensitive)s, %(insightsql_val_7_sensitive)s, %(insightsql_val_5_sensitive)s, %(insightsql_val_8_sensitive)s, %(insightsql_val_9_sensitive)s) AS postgres_table WHERE and(equals(postgres_table.team_id, {self.team.pk}), greaterOrEquals(postgres_table.created_at, minus(today(), toIntervalDay(30))))) AS other_table__details ON equals(other_table.ref_id, other_table__details.other_table__details___id) WHERE equals(other_table.team_id, {self.team.pk}) LIMIT 10",
        )
    ...
    def test_predicate_with_nested_property_access(self):
        ...
        self.assertEqual(
            self._select("SELECT id FROM postgres_table LIMIT 10"),
            f"SELECT postgres_table.id AS id FROM postgresql(%(insightsql_val_1_sensitive)s, %(insightsql_val_2_sensitive)s, %(insightsql_val_0_sensitive)s, %(insightsql_val_3_sensitive)s, %(insightsql_val_4_sensitive)s) AS postgres_table WHERE and(equals(postgres_table.team_id, {self.team.pk}), ifNull(notEquals(replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(postgres_table.properties, %(insightsql_val_5)s), ''), 'null'), '^\"|\"$', ''), %(insightsql_val_6)s), 1)) LIMIT 10",
        )
...
class TestObjectAccessControlIdField(SimpleTestCase):
    ...
    still read the child rows through InsightsQL. The set of object-restrictable resources is
...
class TestSystemTableAccessScope(SimpleTestCase):
    ...
    on the umbrella's children, leaking access-controlled objects through InsightsQL."""
