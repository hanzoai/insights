...
from insights.insightsql.constants import MAX_BYTES_BEFORE_EXTERNAL_GROUP_BY, get_default_insightsql_global_settings
...
from products.experiments.backend.insightsql_queries.base_query_utils import experiment_window
...
from products.experiments.backend.insightsql_queries.experiment_query_builder import (
    ExperimentQueryBuilder,
    get_exposure_config_params_for_builder,
)
from products.experiments.backend.insightsql_queries.experiment_query_runner import (
    PRECOMPUTE_MAX_WINDOW_DAYS,
    ExperimentQueryRunner,
)
from products.experiments.backend.insightsql_queries.exposure_query_logic import get_entity_key
from products.experiments.backend.insightsql_queries.test.experiment_query_runner.base import ExperimentQueryRunnerBaseTest
...
class TestExperimentExposurePreaggregation(ExperimentQueryRunnerBaseTest):
    ...
    def test_group_aggregated_experiment_skips_precompute(self):
        # Group-aggregated builds always fail on Datastore (the INSERT can't resolve the
    ...
    def test_insert_settings_match_insightsql_defaults(self):
        """The INSERT sync_execute must use the same Datastore settings as
        regular InsightsQL queries (via execute_insightsql_query), except for
        INSERT-specific overrides like readonly, max_execution_time,
        insert_quorum, and load_balancing."""
        insightsql_settings = get_default_insightsql_global_settings(team_id=self.team.id).model_dump(exclude_none=True)
        ...
        for key, insightsql_value in insightsql_settings.items():
            if key in intentional_overrides:
                continue
            assert key in insert_settings, f"InsightsQL default setting '{key}' missing from INSERT settings"
            assert insert_settings[key] == insightsql_value, (
                f"Setting '{key}' differs: InsightsQL={insightsql_value}, INSERT={insert_settings[key]}"
            )
    ...
    def test_precomputed_not_in_filter_with_null_properties(self):
        ...
        same setting execute_insightsql_query applies to all regular queries — so that
    ...
    def test_falls_back_to_events_scan_on_lazy_computation_failure(self, expected_path, failing_method, other_method):
        ...
        with (
            ...
            # The non-failing path returns "not ready" so it neither errors nor hits Datastore,
            ...
            patch(
                "products.experiments.backend.insightsql_queries.experiment_query_runner.capture_exception"
            ) as mock_capture,
        ):
            result = self._run_experiment(experiment, metric)
