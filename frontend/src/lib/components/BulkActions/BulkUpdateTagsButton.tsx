...
export function BulkUpdateTagsButton({ resource, selectedIds, onSuccess }: BulkUpdateTagsButtonProps): JSX.Element {
    ...
    const submit = async (): Promise<void> => {
        ...
        try {
            ...
            if (skipped.length === 0) {
                toast.success(`Updated tags on ${updated.length} item${updated.length !== 1 ? 's' : ''}`)
            } else {
                toast.warning(
    ...
    return (
            ...
            overlay={
                <div className="p-3 space-y-3 w-80">
                    ...
                    <div className="flex gap-2 justify-end">
                        <Button size="small" type="secondary" onClick={close}>
                            Cancel
                        </Button>
                        <Button
                            size="small"
        ...
        >
            <Button type="secondary" size="small" icon={<IconPencil />} onClick={open}>
                Update tags
            </Button>
        </Popover>
    )
