...
"""
INSIGHTSQL_EXAMPLE_MESSAGE = """Example InsightsQL query for prompt "weekly active users that performed event ACTIVATION_EVENT on example.com/foo/ 3 times or more, by week":
...
REQUEST_MESSAGE = (
    "I need a robust InsightsQL query to get the following results: {prompt}\n"
...
def write_sql_from_prompt(prompt: str, *, current_query: Optional[str] = None, team: "Team", user: "User") -> str:
    ...
    messages: list[openai.types.chat.ChatCompletionMessageParam] = [
        ...
        {
            "role": "system",
            "content": INSIGHTSQL_EXAMPLE_MESSAGE,
        },
        {
            "role": "user",
    ...
    generated_valid_insightsql = False
    attempt_count = 0
    prompt_tokens_total, completion_tokens_total = 0, 0
    for _ in range(3):  # Try up to 3 times in case the generated SQL is not valid InsightsQL
        attempt_count += 1
        ...
        try:
            ...
            generated_valid_insightsql = True
            break

    report_user_action(
        user,
        "generated InsightsQL with AI",
        {
            "prompt": prompt,
            "response": candidate_sql or error,
            "result": (
                ("valid_insightsql" if generated_valid_insightsql else "invalid_insightsql") if candidate_sql else "prompt_unclear"
            ),
