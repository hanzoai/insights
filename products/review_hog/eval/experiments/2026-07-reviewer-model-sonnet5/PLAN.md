...
3. Dump:
   `LABEL=<label> RUN_SECONDS=<s> RUN_START_EPOCH=<epoch> OUT_DIR=products/review_hog/eval/experiments/2026-07-reviewer-model-sonnet5/runs flox activate -- bash -c "DJANGO_SETTINGS_MODULE=insights.settings python manage.py shell -c \"exec(open('products/review_hog/eval/scripts/dump_result.py').read())\""`
