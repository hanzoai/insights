...
reads on a queue that paces Datastore work anyway.
"""
...
from products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common import REVALIDATION_TRIGGER
...
@shared_task(
    ignore_result=True,
    # Same queue insight cache warming uses — prevents Datastore from being overwhelmed.
...
def revalidate_web_analytics_precompute(team_id: int, query: dict) -> None:
    ...
    try:
        ...
        # InsightsQL result cache entry, so the next user fetch is fresh on both layers.
        runner.run(
