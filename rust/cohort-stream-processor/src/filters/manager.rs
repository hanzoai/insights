...
impl CatalogHandle {
    ...
    /// Query `insights_cohort`, drop out-of-scope teams, rebuild the catalog, and swap it in.
...
mod tests {
    ...
    async fn refresh_builds_catalog_from_live_postgres() {
        let url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://insights:insights@localhost:5432/insights".to_string());
        let pool =
            common_database::get_pool_with_config(&url, common_database::PoolConfig::default())
                .expect("build insights_cohort pool");
        ...
        let stats = handle
            .refresh(&pool)
            .await
            .expect("refresh against live insights_cohort");
