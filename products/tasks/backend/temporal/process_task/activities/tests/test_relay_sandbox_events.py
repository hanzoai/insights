    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                {"type": "notification", "notification": {"method": "_insights/console"}},
                False,
            ),
            (
                "sandbox_output_notification",
                {"type": "notification", "notification": {"method": "_insights/sandbox_output"}},
                False,
            ),
            (
                "terminal_task_complete",
                {"type": "notification", "notification": {"method": "_insights/task_complete"}},
                False,
            ),
            (
                "terminal_error",
                {"type": "notification", "notification": {"method": "_insights/error"}},
                False,
            ),
            (
...
class TestIsActiveAgentUpdate:
    ...
    def test_non_session_update_is_not_active(self) -> None:
        assert (
            _is_active_agent_update({"type": "notification", "notification": {"method": "_insights/console"}}) is False
        )
...
class TestAgentActiveReactivation:
    ...
    def test_console_event_does_not_reactivate(self):
        event = {"type": "notification", "notification": {"method": "_insights/console"}}
        assert self._simulate_reactivation(event, agent_active=False) is False

    def test_sandbox_output_does_not_reactivate(self):
        event = {"type": "notification", "notification": {"method": "_insights/sandbox_output"}}
        assert self._simulate_reactivation(event, agent_active=False) is False
    ...
    def test_full_lifecycle_turn_then_idle_then_resume(self):
        ...
        for method in ("_insights/console", "_insights/sandbox_output"):
...
class TestRelaySandboxEventsErrorHandling:
    ...
    async def test_relay_loop_retries_retryable_stream_errors(
        ...
        terminal_event = SimpleNamespace(
            data='{"type":"notification","notification":{"method":"_insights/task_complete"}}'
        )
    ...
    async def test_keepalive_events_are_transport_only(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        terminal_event = {
            "type": "notification",
            "notification": {"method": "_insights/task_complete"},
        }
    ...
    async def test_permission_request_dispatches_to_broker(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        permission_event = {
            "type": "notification",
            "notification": {
                "method": "_insights/permission_request",
                "params": {
                    "requestId": "perm-1",
...
class TestFinalMessageTracker:
    ...
    @parameterized.expand(
        [
            ...
            ("non_session_method", {"type": "notification", "notification": {"method": "_insights/console"}}),
