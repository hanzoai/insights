...
from insights.insightsql.database.lazy_join_tags import GROUPS_REVENUE_ANALYTICS
...
GROUPS_TABLE_FIELDS: dict[str, FieldOrTable] = {
    ...
    "created_at": DateTimeDatabaseField(
        name="created_at", nullable=False, description="When the group was first created in Insights."
    ),
...
def _bare_limit_key_count(node: SelectQuery) -> int | None:
    # Deferred: insights.insightsql.property imports database schema modules, so a top-level import here is circular.
    from insights.insightsql.property import has_aggregation  # noqa: PLC0415
...
def _guarded_events_aliases(context: InsightsQLContext) -> frozenset[str]:
...
def _outer_events_prefilter(node: SelectQuery, context: InsightsQLContext):
    """
    ...
    from insights.insightsql.transforms.lazy_tables import find_field_chains  # noqa: PLC0415
