...
1. **Certify the sources.** Survey the most-queried warehouse tables/views. For the ones the team
   clearly relies on, `insights:data-catalog-certification-propose` (certify) them; mark obvious
...
3. **On the human's instruction**, promote with the confirmed-action tools:
   ...
   `insights:data-catalog-relationship-accept` / `-reject` (pass the `id` from the queue). A rejected
