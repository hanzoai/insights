...
from insights.settings.base_variables import DEBUG, STATIC_COLLECTION, TEST
...
# Dedicated key for signing Insights-issued JWTs (insights.jwt), so JWT signing can move off
...
# https://agent-proxy.us.hanzo.ai). The stream_token endpoint returns it to clients only when this
...
REVIEWFN_GITHUB_BOT_LOGIN: str = get_from_env("REVIEWFN_GITHUB_BOT_LOGIN", "")
...
FORCE_URL_VALIDATION: bool = get_from_env("POSTFN_FORCE_URL_VALIDATION", False, type_cast=str_to_bool)
