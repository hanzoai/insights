...
describe('session-replay-pipeline', () => {
    ...
    function createMessage(
        ...
        const headersWithDefaults = headers ?? { org: 'test-token' }
    ...
    describe('runSessionReplayPipeline', () => {
        ...
        it('correctly parses and passes headers to the restrictions step', async () => {
            ...
            const messages = [
                createMessage(0, 1, 'session-1', { org: 'team-token-123', distinctId: 'user-456' }),
                createMessage(0, 2, 'session-2', { org: 'team-token-789' }),
            ]
        ...
        it('filters out messages with invalid team', async () => {
            const teamServiceThatDropsSecond = {
                getTeamByOrg: jest.fn().mockImplementation((org: string) => {
                    if (org === 'invalid-org') {
            ...
            const messages = [
                createMessage(0, 1, 'session-1', { org: 'valid-org' }),
                createMessage(0, 2, 'session-2', { org: 'invalid-org' }),
                createMessage(0, 3, 'session-3', { org: 'valid-org' }),
            ]
        ...
        it('sends ingestion warning for old lib version', async () => {
            ...
            const messages = [createMessage(0, 1, 'session-1', { org: 'test-token', lib_version: '1.74.0' })]
        ...
        it('does not send ingestion warning for new lib version', async () => {
            ...
            const messages = [createMessage(0, 1, 'session-1', { org: 'test-token', lib_version: '1.75.0' })]
        ...
        it('does not send ingestion warning when no lib version header', async () => {
            ...
            const messages = [createMessage(0, 1, 'session-1', { org: 'test-token' })]
        ...
        it('sends ingestion warning when message timestamps are too old', async () => {
            ...
            const messages = [createMessageWithOldTimestamps(0, 1, 'session-1', 10, { org: 'test-token' })]
        ...
        it('does not record messages with invalid team to session batch', async () => {
            ...
            const messages = [createMessage(0, 1, 'session-1', { org: 'invalid-token' })]
        ...
        it('records TopFn metrics for multiple messages', async () => {
            ...
            const messages = [
                createMessage(0, 1, 'session-1', { org: 'token-1' }),
                createMessage(0, 2, 'session-2', { org: 'token-2' }),
                createMessage(0, 3, 'session-3', { org: 'token-1' }),
            ]
            ...
            expect(parseTimeRecorder!.record).toHaveBeenCalledWith(
                { org: 'token-1', session_id: 'session-1' },
                expect.any(Number)
            )
            expect(parseTimeRecorder!.record).toHaveBeenCalledWith(
                { org: 'token-2', session_id: 'session-2' },
                expect.any(Number)
            )
            expect(parseTimeRecorder!.record).toHaveBeenCalledWith(
                { org: 'token-1', session_id: 'session-3' },
                expect.any(Number)
            )
        })
        ...
        it('uses "unknown" token in TopFn metrics when token header is missing', async () => {
            ...
            mockCreateParseHeadersStep.mockReturnValue(
                (input: { message: Message; headers?: Record<string, string> }) => {
                    ...
                    return Promise.resolve(
                        ok({
                            ...input,
                            headers: { org: 'test-token', session_id: 'session-1', distinct_id: 'user-123' },
                        })
                    )
                }
            )
            ...
            const messageWithoutToken: Message = {
                ...
                key: Buffer.from('test-key'),
                timestamp: Date.now(),
                headers: [{ org: Buffer.from('test-token') }], // Token for team lookup
                size: 100,
            }
