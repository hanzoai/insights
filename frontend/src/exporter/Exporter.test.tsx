...
describe('Exporter (shared dashboard)', () => {
    it('sets document title with Insights suffix by default', () => {
        const initialTitle = document.title

        render(<Exporter {...makeDashboardExport()} />)

        expect(document.title).toBe('My shared dashboard • Insights')

        // restore to avoid leaking between tests
        document.title = initialTitle
    })

    it('omits Insights suffix from title when whitelabel is enabled', () => {
        const initialTitle = document.title
