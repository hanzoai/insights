from insights.schema import AccountsQuery, AccountsQueryResponse, CachedAccountsQueryResponse

from insights.insightsql import ast
from insights.insightsql.errors import BaseInsightsQLError, ExposedInsightsQLError
...
def _normalize_order_clause(raw: str) -> str:
    """Allow Django-style `-col` shorthand alongside native InsightsQL `col DESC`."""
    stripped = raw.strip()
...
class AccountsQueryRunner(AnalyticsQueryRunner[AccountsQueryResponse]):
    ...
    def _calculate(self) -> AccountsQueryResponse:
        ...
        return AccountsQueryResponse(
            ...
            metricsResults=metrics_results,
            insightsql=response.insightsql or "",
    ...
    def _compute_metrics_results(self, metrics: list[str]) -> list[float | int | None]:
        ...
        except (InternalCHQueryError, BaseInsightsQLError) as error:
    ...
    def _metric_evaluation_error(self, metrics: list[str], error: Exception) -> ExposedInsightsQLError:
        ...
        if isinstance(error, (ExposedInsightsQLError, ExposedCHQueryError)):
            detail = f"{detail} {error}"
        return ExposedInsightsQLError(detail)
