                ...
                {isDiffVisible ? (
                    ...
                    <>
                        ...
                        <div ref={markdownContainerRef}>
                            <Markdown className="mt-1 rounded border bg-bg-light p-3" generateHeadingIds>
                                {promptText}
                            </Markdown>
                        </div>
                    </>
            ...
            {variables.length > 0 && (
                <div className="flex flex-wrap items-center gap-1">
                    ...
                    {variables.map((variable) => (
                        <Tag key={variable} type="highlight" size="small">
                            {variable}
                        </Tag>
                    ))}
                </div>
    ...
    return (
        ...
        >
            <div className="space-y-3">
                ...
                <Field.Pure label="What changed?" help="Optional — shown in the version history.">
                    <Input
        ...
        <div className="mt-2 space-y-3" data-attr="llma-prompt-diff-view">
            ...
            {comparePromptLoading ? (
                <div className="space-y-2 rounded border p-4">
                    <Skeleton active className="h-4 w-full" />
                    <Skeleton active className="h-4 w-3/4" />
                    <Skeleton active className="h-4 w-1/2" />
                </div>
            ) : !comparePrompt ? (
                <Banner type="warning">
...
function buildPythonSnippet(
    ...
    return `from insights import Insights
...
function buildNodeSnippet(
    ...
    return `import { Prompts } from '@hanzo/ai'
import { Insights } from 'insights-node'

const insights = new Insights('${projectApiKey}', {
    host: '${host}',
        ...
        <div className="mb-6" data-attr="llma-prompt-code-snippets">
            <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <Button
                    type="secondary"
                    size="small"
                    to="https://hanzo.ai/docs/prompt-management"
                    targetBlank
...
function experimentStatusTag(experiment: ExperimentBasicApi): JSX.Element {
    if (experiment.archived) {
        return <Tag type="muted">Archived</Tag>
    }
    if (experiment.end_date) {
        return <Tag type="completion">Stopped</Tag>
    }
    if (experiment.start_date) {
        return <Tag type="success">Running</Tag>
    }
...
export function PromptExperiments({ prompt }: { prompt: LLMPrompt }): JSX.Element {
    ...
    const columns: TableColumns<ExperimentBasicApi> = [
        {
            title: 'Name',
            dataIndex: 'name',
            sticky: true,
...
export function PromptEditForm({
    ...
    return (
        <div className="mt-4 max-w-3xl space-y-4">
            {publishConflict ? (
                <Banner type="warning" data-attr="llma-prompt-publish-conflict-banner">
            ...
            >
                {isRenderingMarkdown ? (
                    <Markdown className="rounded border bg-bg-light p-3 whitespace-pre-wrap">
                            ...
                            <>
                                ...
                                {labelsEnabled ? (
                                    <div
                                        ...
                                        // (e.g. Tag's close button), which would otherwise let the
