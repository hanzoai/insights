...
- **Type** — leave empty (will internally default to `"product"`. The `"web"` value is reserved for no-code experiments configured visually with the Insights
  toolbar in a browser; it cannot be meaningfully driven via MCP. If a user asks for a
  no-code/toolbar experiment, point them to the Insights UI instead of creating one here.)
...
1. **Always show the experiment URL.** The `experiment-create` response includes `_insightsUrl` — always display this link so the user can view and configure the experiment in the UI.
