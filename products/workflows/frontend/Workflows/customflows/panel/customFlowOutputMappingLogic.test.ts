...
import { CustomFlowAction, CustomFlowActionNode } from '../types'
import { customFlowOutputMappingLogic, normalizeOutputVariable } from './customFlowOutputMappingLogic'

const makeActionNode = (id: string, outputVariable?: CustomFlowAction['output_variable']): CustomFlowActionNode => ({
    id,
...
describe('customFlowOutputMappingLogic', () => {
    describe('normalizeOutputVariable', () => {
        it.each([
            ...
            expect(normalizeOutputVariable(raw as CustomFlowAction['output_variable'])).toEqual(expected)
        })
    })

    describe('logic', () => {
        let logic: ReturnType<typeof customFlowOutputMappingLogic.build>
        let editorLogic: ReturnType<typeof customFlowEditorLogic.build>
        ...
        beforeEach(() => {
            ...
            editorLogic = customFlowEditorLogic({ id: WORKFLOW_ID })
            editorLogic.mount()

            logic = customFlowOutputMappingLogic({ id: WORKFLOW_ID })
            logic.mount()
        })
