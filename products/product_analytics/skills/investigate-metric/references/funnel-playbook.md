...
Run `insights:query-trends` on the events at step N-1 and step N. Steady entries with
falling completions = problem at that step. Falling entries = problem upstream.

## 3. Who dropped off

`insights:query-trends-actors` only accepts a trends source. Run trends on step N-1
...
Filter `insights:query-error-tracking-issues-list` and `insights:query-logs` to the surface
...
`insights:query-paths` with `endPoint` set to the failing step. Paths that don't reach
