...
object-consumers that genuinely need them (InsightsQL/view/query builders that traverse
relations, dispatch on ``isinstance``, call model methods, or use the package's query
helpers). Deliberately free of heavy imports (no Datastore→InsightsQL type tables, unlike
``facade.insightsql``), so importing it adds nothing beyond the models Django already loads
at ``django.setup()``.
