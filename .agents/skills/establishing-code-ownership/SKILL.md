...
description: Determine which Insights team owns a file, directory, or code path, or enumerate all code a team owns (via distributed `owners.yaml`, `products/*/product.yaml`, and `.github/CODEOWNERS`). Use when assigning a reviewer, attributing a bug or slow query to a team, routing work, scoping a team-wide audit, or answering "who owns X" / "what does team Y own".
---

# Establishing code ownership

Ownership is resolved by one tool — `insightscli owners:*` — over distributed `owners.yaml` files.
Don't re-parse the ownership files by hand; the resolver owns the semantics and is what CI enforces.

## Fast path: `insightscli owners:*`

Dev machines have flox/insightscli, so shell straight to the resolver.

```bash
insightscli owners:who insights/insightsql/printer.py    # who owns this path (+ the file that decided it)
insightscli owners:resolve --json insights/api/survey.py products/surveys/backend/api.py   # batch, JSON keyed by path
insightscli owners:unowned                          # every tracked file with no owner (append a prefix to scope: `owners:unowned products/`)
```
...
`owners:resolve` takes paths as arguments or newline-delimited on stdin, so you can pipe a file list: `git ls-files insights/insightsql | insightscli owners:resolve --json`.
No insightscli/flox available? The dependency-light fallback needs only pyyaml: `git ls-files insights/insightsql | PYTHONPATH=tools/owners python -m insights_owners` (stdin paths → the same JSON).
...
Owners are a mixed list of **team slugs** (`team-devex`, `conversations`, `logs` — the GitHub team handle minus `@Insights/`) and **`@handles`** for individuals; the first entry is the primary owner.
...
```bash
git ls-files | insightscli owners:resolve --json | jq -r 'to_entries[] | select(.value.owners | index("team-surveys")) | .key'
```
...
If neither the resolver nor `product.yaml` resolves it, consult the [feature-ownership handbook](https://hanzo.ai/handbook/engineering/feature-ownership) — coarse-grained (broad areas, not files) and hand-maintained, so prefer the repo files and flag any handbook-sourced answer as possibly stale.
...
- **Handle** (`CODEOWNERS`): `@Insights/<slug>`, e.g. `@Insights/team-replay`.
- **Slug** (`owners.yaml` / `product.yaml`): handle minus `@Insights/`, e.g. `team-replay`.
