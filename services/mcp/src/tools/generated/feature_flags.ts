...
const createFeatureFlag = (): ToolBase<typeof CreateFeatureFlagSchema, WithInsightsUrl<Schemas.FeatureFlag>> => ({
...
const featureFlagGetAll = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedFeatureFlagList>
> => ({
        ...
        return await withInsightsUrl(
            context,
            {
                ...
                results: await Promise.all(
                    (filtered.results ?? []).map((item) => withInsightsUrl(context, item, `/feature_flags/${item.id}`))
                ),
            },
            '/feature_flags'
        )
...
const featureFlagGetDefinition = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.FeatureFlag>
> => ({
...
const featureFlagsMyFlagsRetrieve = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.MyFlagsResponse[]>
> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof FeatureFlagsMyFlagsRetrieveSchema>) => {
        ...
        return await withInsightsUrl(context, result, '/feature_flags')
    },
})
...
const scheduledChangesList = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedScheduledChangeList>
> => ({
...
const updateFeatureFlag = (): ToolBase<typeof UpdateFeatureFlagSchema, WithInsightsUrl<Schemas.FeatureFlag>> => ({
