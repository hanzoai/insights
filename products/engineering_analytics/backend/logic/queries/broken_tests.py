...
Three reads, merged in Python because they span two Datastore clusters:
...
_FINGERPRINTS_SELECT = """
    SELECT
        ...
        -- Aliased away from `repo`: InsightsQL binds a WHERE identifier to a matching SELECT alias, so
