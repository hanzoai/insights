...
export function ScoutsFleetSection(): JSX.Element {
    ...
    if (scoutConfigsLoading && scoutConfigs === null) {
        return <Skeleton className="h-12 w-full rounded" />
    }
    ...
    if (scoutConfigs === null) {
        return (
            <div className="flex items-center gap-3 rounded border border-danger bg-danger-highlight px-4 py-3.5">
                ...
                <Button type="secondary" size="small" status="danger" onClick={() => loadScoutConfigs()}>
                    Retry
                </Button>
            </div>
        )
...
function ScoutAlphaBanner(): JSX.Element | null {
    ...
    return (
        <Banner type="info" dismissKey={`signals-scout-banner-${scoutBannerMessage}`}>
            {scoutBannerMessage}
        </Banner>
    )
}
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
                <span className="text-xs text-muted">
                    ...
                    created as <span className="font-mono text-[11px]">signals-scout-*</span> skills in your Insights
                    project.
                </span>
