...
All three kinds are team `LLMSkill` rows the review agents pull over MCP at run time. Insights ships
...
1. **Ground yourself.** Using the Insights MCP skill tools, `skill-list` the team's `review-script-*`
   ...
   `review-script-perspective-*` so the new lens doesn't re-cover ground an enabled one already owns
...
4. **Create the skill yourself with `insights:skill-create`** — actually create the team `LLMSkill`
