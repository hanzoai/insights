...
class TestUserInterviewSearch(_FeatureFlagEnabledMixin):
    ...
    def _insightsql_rows(self, rows: list[tuple[Any, ...]]) -> MagicMock:
        result = MagicMock()
        result.results = rows
        return result

    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_returns_ranked_matches(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows(
            [
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_clamps_similarity_to_unit_interval(self, _name, distance, expected, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([(str(self.interview_a.id), "transcript", distance)])
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_snippet_reflects_current_postgres_content(self, mock_embed, mock_insightsql):
        ...
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([(str(self.interview_a.id), "transcript", 0.1)])
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_truncates_long_content_snippet(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_skips_rows_for_deleted_interviews(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
    ...
    def test_search_forwards_document_types_filter(
        self, _name, document_types, expected_in_placeholders, mock_embed, mock_insightsql
    ):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([])
        ...
        placeholders = mock_insightsql.call_args.kwargs["placeholders"]
        self.assertEqual(set(placeholders["document_types"].value), expected_in_placeholders)

    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_resolves_topic_id_via_current_postgres_linkage(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([])
        ...
        insightsql_query = mock_insightsql.call_args.kwargs["query"]
        self.assertIn("document_id IN {scoped_document_ids}", insightsql_query)
        self.assertNotIn("JSONExtractString(metadata, 'topic_id')", insightsql_query)

    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_excludes_interviews_detached_from_topic(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([])
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_short_circuits_when_topic_has_no_interviews(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        # No call to InsightsQL should happen when the topic resolves to zero interview IDs.
        ...
        mock_insightsql.assert_not_called()
        mock_embed.assert_not_called()

    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_defaults_to_both_document_types_when_unset(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([])
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_enforces_default_limit_when_omitted(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([])
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_tags_datastore_query_for_attribution(self, mock_embed, mock_insightsql, mock_tag):
        from insights.datastore.query_tagging import Feature, Product
    ...
    def test_search_returns_502_when_embedding_service_fails(self, _mock_embed, mock_insightsql):
        response = self.client.post(self._url(), {"query": "x"}, content_type="application/json")
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_caps_scoped_document_ids_for_pathological_topic_size(self, mock_embed, mock_insightsql):
    ...
    @patch("products.user_interviews.backend.presentation.views.execute_insightsql_query")
    @patch("products.user_interviews.backend.presentation.views.generate_embedding")
    def test_search_does_not_leak_across_teams(self, mock_embed, mock_insightsql):
        mock_embed.return_value = self._embedding_response()
        mock_insightsql.return_value = self._insightsql_rows([])
