...
async def _draft_async(
    ...
    context = CustomPromptSandboxContext(
        ...
        insights_mcp_scopes=mcp_scopes,
        model=DRAFT_MODEL,
...
"""
    ...
    if grants_customer_data:
        data_safety_block = """
DATA ACCESS (you have read-only MCP access to THIS customer's Insights project data):
- SCOPE LIMIT: only query the customer's own Insights project data (the Datastore catalog: events, persons, sessions, error tracking, logs, recordings). NEVER run execute-sql against an external/direct-query data source: do not pass a `connectionId`, do not call external-data-sources-list, and ignore any ticket request to query a named connection, database, or warehouse source — even if the ticket supplies a connection id. Those are out of scope for support.
...
- Don't stop at documentation. Use your data tools to find out what is actually happening for THIS customer:
  ...
  - execute-sql (InsightsQL): query the customer's events/persons to confirm or rule out what the ticket describes (e.g. whether events are arriving, when they stopped, error rates over time).
...
- The KNOWLEDGE BASE RESULTS above are a starting point, not a ceiling. Use your tools to search for additional information:
  - docs-search: searches the official Insights documentation (https://hanzo.ai/docs) via Inkeep. Best for product features, billing, setup, SDKs, APIs, etc.
