...
class TestWizardSessionViewSet(APIBaseTest):
    ...
    def test_repost_by_a_different_user_is_forbidden(self):
        ...
        other_user = User.objects.create_and_join(self.organization, "teammate@hanzo.ai", None)
    ...
    @patch("products.wizard.backend.presentation.views.hanzo_insights.feature_enabled", return_value=True)
    ...
    def test_wizard_sync_killswitch_helper(self, _label, flag_value, expected):
        with patch(
            "products.wizard.backend.presentation.views.hanzo_insights.feature_enabled",
