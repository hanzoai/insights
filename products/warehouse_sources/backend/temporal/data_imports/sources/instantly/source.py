...
class InstantlySource(
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Sync campaigns, leads, emails, sending accounts, and analytics from Instantly into the Insights Data warehouse.
...
API keys are scope-gated: the key needs a read scope for every table you sync (`all:read` covers everything; narrower keys like `campaigns:read`, `leads:read`, `emails:read`, or `accounts:read` work for their own tables). The optional webhook event stream additionally needs `webhooks:all` and the Hypergrowth plan or above.""",
            ...
            docsUrl="https://hanzo.ai/docs/cdp/sources/instantly",
...
3. Add a custom header named `x-insights-webhook-secret` with a value of your choosing, and paste the same value into the webhook secret field below
