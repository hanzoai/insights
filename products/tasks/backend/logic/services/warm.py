...
class SandboxWarmer:
    ...
    def at_capacity(cls, origin_product: str, team: Team, user: User) -> bool:
        ...
        # `await_user_message` is stored literally (the field is alias-free in InsightsAIRunState); see the
    ...
    def warm(
        ...
        - ``extra_state`` carries product-specific Run state the generic warmer can't know (e.g. Insights
          AI's ``systemPrompt``, or a target ``branch``); it is merged into the warm Run's initial state.
        - ``create_pr`` is forwarded to the processing workflow; it defaults to ``False`` (Insights AI warm
