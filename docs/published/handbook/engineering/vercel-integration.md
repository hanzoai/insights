...
A reference for understanding how the Insights + Vercel Marketplace integration works end-to-end. Covers installation, billing, usage reporting, feature flag sync, SSO, and uninstall.

### Key concepts

Before diving in, a few Insights data model basics:
...
- **`BillingManager`** (`ee/billing/billing_manager.py`) is the intermediary Insights uses to talk to the Billing service. It handles license validation, org syncing, and the actual HTTP calls.
...
graph LR
    ...
    Vercel -->|"Install / Uninstall\nInvoice paid webhook"| Insights
    Insights -->|"Sync feature flags\nSSO token exchange"| Vercel
    Insights -->|"Create subscription\nCancel subscription"| Billing
    Billing -->|"Submit usage daily\nSubmit invoices"| Vercel
    Billing -->|"Create subscription\nGenerate invoices"| Stripe
    Stripe -->|"invoice.finalized\nwebhook"| Billing
```

The Insights repo handles installation, SSO, feature flag sync, and webhooks from Vercel. The Billing repo handles subscription management, usage reporting to Vercel, and invoice submission. They communicate via internal HTTP APIs through `BillingManager`.
...
A brand new user clicks "Add" in the Vercel Marketplace. Insights creates everything from scratch.

```mermaid
sequenceDiagram
    actor User
    participant VM as Vercel Marketplace
    participant PH as Insights
...
An existing Insights customer links their org via OAuth. Billing stays with Insights (no `billing_provider` change).
...
Vercel acts as the **payment collector**. Insights still uses Stripe to track usage and generate invoices, but Stripe never charges the customer directly.

```mermaid
graph LR
    subgraph Normal Insights Customer
    ...
    subgraph Vercel Customer
        ...
        B3[Insights/Stripe] -->|send_invoice| B4[Invoice created\nstatus: open]
...
- Reset to `"insights"` on uninstall
...
Insights reports usage to Vercel daily so it appears in the Vercel dashboard.
...
This fallback chain is what makes the uninstall flow resilient. After uninstall, `customer.billing_provider` is already reset to `"insights"`, but the subscription metadata still says `"vercel"`, so the final invoice is still submitted correctly.
...
```json
{
  ...
  "items": [
    {
      "billingPlanId": "price_xxx",
      "name": "Insights - Product Analytics",
      "price": "0.000025",
      "quantity": 1000000,
      "units": "events",
      "total": "25.00"
...
When a Vercel customer's payment fails, Vercel handles the initial retry and dunning communication. Insights only needs to act after the retry window expires.
...
Suspension status is for **display only** within Vercel. To actually block access, you must also stop serving the customer on the Insights side.
...
Insights automatically syncs feature flags and experiments to Vercel's experimentation platform via Django signals.
...
Vercel users can SSO into Insights without a separate login.
...
sequenceDiagram
    ...
    User->>Vercel: Click "Open Insights"
...
**Multi-region:** If the resource doesn't exist in the current region (e.g. US), Insights proactively redirects to the other region (EU). This is a region check, not a failure fallback.
...
sequenceDiagram
    ...
    User->>VM: Uninstall Insights integration
    ...
    alt Has unpaid invoices
        ...
        Billing->>Billing: Reset billing_provider to "insights"
...
**Critical detail:** The final invoice Celery task receives `billing_provider` as an explicit string argument at queue time (when it's still `"vercel"`). It does NOT read `customer.billing_provider` at execution time. This is why the reset to `"insights"` doesn't break final invoice submission. As an additional safety net, the [billing provider resolution fallback chain](#billing-provider-resolution-_resolve_billing_provider) also protects against this — even if the Celery argument were lost, the subscription metadata still retains the original billing provider value.

### Connectable uninstall (no billing)

For connectable integrations (linked existing accounts), the webhook handler simply deletes the `OrganizationIntegration` record. No billing service call is made since these customers keep their existing Insights billing.
...
Insights dashboard: [Vercel Billing Integration](https://us.hanzo.ai/project/2/dashboard/1404045) - covers customer integrity, invoice lifecycle, usage reporting, submission gap tracking, and financial reconciliation.

---

## 11. Contacting Vercel support

For integration or billing issues that need Vercel's involvement, post in the shared Slack channel [#insights-vercel](https://insights.slack.com/archives/C08LYBQ58N5) with a :ticket: reaction on the message. This flags it for the Vercel team to pick up.
