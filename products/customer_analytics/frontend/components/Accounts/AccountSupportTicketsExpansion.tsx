...
const columns: TableColumns<SupportTicketApi> = [
    {
        title: 'Ticket',
        key: 'ticket_number',
        render: (_, ticket) => (
            <Link
                to={ticket.deep_link}
                target="_blank"
                onClick={() => insights.capture(AccountsEvents.SupportTicketClicked)}
            >
    ...
    },
    {
        ...
        key: 'status',
        render: (_, ticket) => (
            <Tag type={ticket.status === 'resolved' ? 'success' : ticket.status === 'new' ? 'primary' : 'default'}>
