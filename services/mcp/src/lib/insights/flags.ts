...
/** Raw value from insights-node for a single flag — `true`/`false` for boolean flags, a variant string for multivariate. */
 ...
 * `x-insights-flag-overrides` header on the Hono path). Both are JSON objects.
 *
