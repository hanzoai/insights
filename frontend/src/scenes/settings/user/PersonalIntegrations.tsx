...
function GitHubInstallationRow({ integration }: { integration: PersonalGitHubIntegration }): JSX.Element {
    ...
    const handleDisconnect = (): void => {
        Dialog.open({
            ...
            description: (
                <>
                    <Banner type="warning" className="my-4 text-balance">
                        Any Insights Desktop agent runs <em>currently in progress</em> will be unable to push commits or
                        open pull requests on GitHub.
                    </Banner>
                    <p>
                        Insights will no longer be able to access repos from this installation or act on your behalf
                        there.
                    </p>
                </>
            ),
            ...
            {integrations.length === 0 ? (
                <div className="px-4 py-6 text-center text-sm text-secondary">
                    ...
                    <p className="text-xs text-muted text-balance">
                        Connect to let Insights access your repos, attribute commits, open pull requests, and assign
        ...
        description: (
            <div className="deprecated-space-y-2">
                <p className="text-sm text-secondary">
                    ...
                    Insights identity to.
                </p>
                <div className="divide-y rounded border">
                    {workspaces.map((workspace) => (
                        <Button
                            key={`${workspace.insights_team_id}:${workspace.slack_team_id}`}
                            fullWidth
                            type="tertiary"
                            icon={<IconSlack />}
                            sideIcon={<IconPlus />}
                        ...
                        >
                            <div className="flex flex-col min-w-0">
                                ...
                                <span className="text-xs text-secondary truncate">
                                    {workspace.insights_organization_name} · {workspace.insights_team_name}
                                </span>
                            </div>
                        </Button>
                    ))}
        ...
        <div className="divide-y rounded border bg-surface-primary">
            {slackIntegrationsLoading && slackIntegrations.length === 0 ? (
                ...
                <div className="px-4 py-6 text-center text-sm text-secondary">
                    ...
                    <p className="text-xs text-muted text-balance">
                        Link your Slack identity so @Insights mentions route to you even when your Slack email and
                        Insights email don't match.
                    </p>
                </div>
            ) : (
            ...
            {(linkableSlackWorkspacesLoading || hasLinkableWorkspaces) && (
                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-4 py-3">
                    ...
                    <span className="text-xs text-secondary text-balance">
                        You'll be redirected to Slack to authorize this Insights account. The link binds your Slack user
                        id to your Insights account — no Slack token is kept after the redirect.
                    </span>
                </div>
            )}
        </div>
