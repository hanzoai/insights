...
from insights.schema import EndpointRunRequest, InsightsQLQuery

from insights.ducklake.client import execute_ducklake_query
from insights.ducklake.common import get_duckgres_server_for_organization, is_dev_mode
...
def build_ducklake_insightsql_query(
    endpoint: Endpoint,
    ...
    *,
    ...
    """Build the InsightsQL an endpoint would run against DuckLake, matching the inline path's
    variable overrides and pagination so the shadow mirrors what Datastore executed."""
    ...
    insightsql_query = InsightsQLQuery(query=query["query"], variables=query.get("variables"))
    if plan.variables_override and insightsql_query.variables:
        for override in plan.variables_override:
            if insightsql_query.variables.get(override.variableId):
                insightsql_query.variables[override.variableId] = override
...
def run_ducklake_shadow_comparison(
    *,
    ...
    """Re-run an endpoint's InsightsQL against DuckLake and emit a comparison event. Runs in a
    ...
    try:
        insightsql_query = build_ducklake_insightsql_query(endpoint, version, team, data, limit=limit, offset=offset)
    ...
    if ducklake_row_count is not None and datastore_row_count is not None:
        row_count_match = ducklake_row_count == datastore_row_count
