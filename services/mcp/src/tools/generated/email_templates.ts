...
const workflowsCreateEmailTemplate = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.MessageTemplate>
> => ({
...
const workflowsListEmailTemplates = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.PaginatedMessageTemplateList>
> => ({
            ...
            context,
            {
                ...
                results: await Promise.all(
                    (filtered.results ?? []).map((item) =>
                        withInsightsUrl(context, item, `/workflows/library/templates/${item.id}`)
                    )
                ),
            },
