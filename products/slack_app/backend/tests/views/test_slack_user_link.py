...
class TestAuthorizeView:
    ...
    def test_unauthenticated_user_is_bounced_through_login(self, db):
        # `db` fixture: Insights's `login_required` consults `User.objects.exists()`
...
class TestCallbackView:
    ...
    def _state_for(self, user, insights_team_id, *, slack_team_id=SLACK_TEAM_ID, slack_user_id=SLACK_USER_ID):
    ...
    def test_session_mismatch_refuses_to_link(self, org_team_user, workspace_integration):
        ...
        # `state.insights_user_id` points at the attacker, but the callback
        # fires in a *different* Insights user's browser session (the victim,
        ...
        # State carries the *attacker's* insights_user_id (the flow's initiator),
    ...
    def test_org_mismatch_refuses_to_link(self, workspace_integration, db):
        # A user logged into Insights but NOT a member of the workspace's org —
