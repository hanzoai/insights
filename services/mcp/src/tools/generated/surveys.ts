...
const surveyCreate = (): ToolBase<
    ...
    WithInsightsUrl<Schemas.SurveySerializerCreateUpdateOnly>
> =>
    withUiApp('survey', {
        name: 'survey-create',
...
const surveyGet = (): ToolBase<typeof SurveyGetSchema, WithInsightsUrl<Schemas.Survey>> =>
    withUiApp('survey', {
        name: 'survey-get',
...
const surveyLaunch = (): ToolBase<typeof SurveyLaunchSchema, WithInsightsUrl<Schemas.Survey>> =>
    withUiApp('survey', {
        name: 'survey-launch',
...
const surveyStats = (): ToolBase<typeof SurveyStatsSchema, WithInsightsUrl<Schemas.SurveyStatsResponse>> =>
    withUiApp('survey-stats', {
        ...
        handler: async (context: Context, params: z.infer<typeof SurveyStatsSchema>) => {
            ...
            return await withInsightsUrl(context, result, `/surveys/${result.survey_id}`)
        },
    })

const SurveyStopSchema = SurveysStopParams.omit({ project_id: true })

const surveyStop = (): ToolBase<typeof SurveyStopSchema, WithInsightsUrl<Schemas.Survey>> =>
    withUiApp('survey', {
        name: 'survey-stop',
...
const surveysGetAll = (): ToolBase<typeof SurveysGetAllSchema, WithInsightsUrl<Schemas.PaginatedSurveyList>> =>
            ...
            return await withInsightsUrl(
                context,
                {
                    ...
                    results: await Promise.all(
                        (result.results ?? []).map((item) => withInsightsUrl(context, item, `/surveys/${item.id}`))
                    ),
                },
                '/surveys'
            )
