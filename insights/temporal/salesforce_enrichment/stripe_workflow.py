...
async def get_stripe_watermark_activity() -> tuple[str, str] | None:
    ...
    Returns ``(last_changed_at_iso, insights_organization_id)`` or ``None`` when
    no watermark is set.
    """
...
async def commit_stripe_watermark_activity(inputs: CommitStripeWatermarkInputs) -> None:
    ...
    await set_stripe_enrichment_watermark(
        ...
        inputs.insights_organization_id,
    )
...
async def enrich_stripe_page_activity(inputs: EnrichStripePageInputs) -> EnrichStripePageResult:
    ...
    Matching Account rows are looked up by ``Insights_Org_ID__c`` — the same join
    ...
    async with Heartbeater() as heartbeater:
        ...
        # The SQL orders by (last_changed_at, insights_organization_id) ASC so the
        # last row's keyset position is both this page's high-water mark and the
        # starting cursor for the next page.
        signals_by_org: dict[str, StripeSignals] = {s.insights_organization_id: s for s in signals_rows}
        ...
        next_cursor_org_id = last_row.insights_organization_id
...
class SalesforceStripeEnrichmentWorkflow(InsightsWorkflow):
            ...
            if state.pending_watermark_ts and state.pending_watermark_org_id:
                await workflow.execute_activity(
                    ...
                    CommitStripeWatermarkInputs(
                        ...
                        insights_organization_id=state.pending_watermark_org_id,
                    ),
                    start_to_close_timeout=dt.timedelta(seconds=30),
                    retry_policy=RetryPolicy(maximum_attempts=3),
                )
