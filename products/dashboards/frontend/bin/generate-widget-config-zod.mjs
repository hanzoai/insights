...
function readFormFieldsManifest() {
    if (!fs.existsSync(FORM_FIELDS_JSON)) {
        console.error(`Missing ${FORM_FIELDS_JSON}. Run insightscli build:widget-types first.`)
        process.exit(1)
    }
...
async function main() {
    if (!fs.existsSync(schemaPath)) {
        console.error(`OpenAPI schema not found at ${schemaPath}. Run insightscli build:openapi-schema first.`)
        process.exit(1)
    }
    ...
    if (configModelNames.length === 0) {
        console.error(
            `No *WidgetConfig schemas in the OpenAPI slice for ${WIDGET_CATALOG_OPERATION_ID}. ` +
                'Regenerate insightscli build:openapi-schema.'
        )
        process.exit(1)
    }
 ...
 * Regenerate: insightscli build:widget-types
 */
