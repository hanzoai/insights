...
export function TemplatePickerModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }): JSX.Element {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Choose a starting point" width={880}>
                ...
                <div className={fieldsHidden ? 'h-0 overflow-hidden' : ''}>
                    {compactHeader ? (
                        <div className="flex gap-2 items-center pl-2 pr-1 py-0.5 border-b shrink-0">
                            <Field name="subject" className="flex-2 min-w-40" renderError={() => null}>
                                {({ value, onChange, error }: ChildFunctionProps) => (
                                    <div className="flex gap-2 items-center flex-1">
                                        <Label className={error ? 'text-danger' : ''}>Subject</Label>
                                        <LiquidSupportedText
                                            value={value}
                                            onChange={onChange}
                                            globals={logicProps.variables}
                            ...
                            {preheaderVisible ? (
                                <Field name="preheader" className="flex-1 min-w-40" renderError={() => null}>
                                    {({ value, onChange }: ChildFunctionProps) => (
                                        <div className="flex gap-2 items-center flex-1">
                                            <Label info="This is the preview text that appears below the subject line in an inbox.">
                                                Preheader
                                            </Label>
                                            <LiquidSupportedText
                                                value={value}
                ...
                <div className="flex-1 min-h-0 flex relative p-4">
                    ...
                    <div className="flex flex-col flex-1">
                        ...
                        <div className="flex gap-2 items-center mt-2">
                            <Button type="secondary" onClick={() => setIsSaveTemplateModalOpen(true)}>
