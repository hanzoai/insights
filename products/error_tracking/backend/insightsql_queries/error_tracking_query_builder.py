...
from products.error_tracking.backend.insightsql_queries.error_tracking_query_runner_utils import (
...
def _fingerprint_hash_expr() -> ast.Call:
    ...
    name for grouping keys. Datastore versions disagree on whether the ``$``
...
class ErrorTrackingQueryBuilder:
    """Datastore-only query builder using the denormalized fingerprint table.
    ...
    * **Legacy single-query shape.** Retained for the edge case where a
    ...
    def insightsql_filters(self) -> InsightsQLFilters:
        ...
        return ast.SelectQuery(
            ...
            select_from=ast.JoinExpr(
                ...
                next_join=ast.JoinExpr(
                    table=ast.Field(chain=["insights", "error_tracking_fingerprint_issue_state"]),
                    alias="fp_state",
    ...
    def _inner_select_expressions(self) -> list[ast.Expr]:
        ...
        if self.query.withAggregations:
            ...
            # would divide by zero in Datastore.
