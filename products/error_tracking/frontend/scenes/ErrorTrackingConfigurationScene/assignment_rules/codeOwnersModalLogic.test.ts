...
describe('codeOwnersModalLogic', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(toast, 'warning').mockImplementation(jest.fn())
    ...
    it('derives parsed owners and concise parse errors from raw text', async () => {
        await expectLogic(logic, () => {
            logic.actions.setRawText(['frontend/** @hanzo/frontend', 'backend/**'].join('\n'))
        }).toMatchValues({
            parsedOwners: [{ owner: '@hanzo/frontend', patterns: ['frontend/**'], index: 0 }],
    ...
    it('loads assignable roles and users when opened and resets modal state', async () => {
        await expectLogic(logic, () => {
            logic.actions.setRawText('frontend/** @hanzo/frontend')
            logic.actions.setOwnerAssignee('@hanzo/frontend', { type: 'role', id: 'frontend' })
    ...
    it('keeps manual assignee picks over automatic matches', async () => {
        await expectLogic(logic, () => {
            logic.actions.setRawText('a/** @hanzo/error-tracking')
            logic.actions.setOwnerAssignee('@hanzo/error-tracking', { type: 'user', id: 123 })
        }).toMatchValues({
            ownerRows: [
                expect.objectContaining({
                    owner: '@hanzo/error-tracking',
    ...
    it('reports partial save failures while keeping successful creates', async () => {
        ...
        expect(loadRulesSpy).toHaveBeenCalledWith(ErrorTrackingRuleType.Assignment)
        expect(toast.warning).toHaveBeenCalledWith('Created 1 of 2 assignment rules. 1 failed.')
    ...
    it('reloads rules and shows an error when every create fails', async () => {
        ...
        expect(toast.error).toHaveBeenCalledWith('Failed to save assignment rules: nope')
    })
    ...
    it('deduplicates repeated rejection reasons across failed creates', async () => {
        ...
        expect(toast.error).toHaveBeenCalledWith('Failed to save assignment rules: assignee not found')
    })
})
