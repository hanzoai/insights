...
/**
 * The shared MCP tool-card presentation: Insights `exec` inner tools read `Call <tool>`; other MCP
...
export function getMcpToolPresentation(
    ...
    const isInsightsExec = !!message.innerToolName
    const isMcp =
        isInsightsExec || (!!message.rawServerName && !!message.rawToolName && message.rawServerName !== 'claude')
    const serverName = isInsightsExec ? 'insights' : message.rawServerName
    const toolLabel =
...
/**
 * The catch-all tool card — user-installed MCP tools, unmapped Insights `exec` inner tools, and Claude
