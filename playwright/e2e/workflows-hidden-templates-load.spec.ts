...
// `template-insights-capture` is shipped with `status: 'hidden'` (see
// nodejs/src/cdp/templates/_destinations/insights_capture/insights-capture.template.ts).
...
test.describe('Workflows hidden built-in templates load', () => {
    ...
    test.beforeAll(async ({ playwrightSetup }) => {
        ...
        // Playwright CI doesn't run sync_insights_function_templates, so the templates table is
        ...
        await playwrightSetup.seedInsightsFunctionTemplate({
            template_id: 'template-insights-capture',
            name: 'Capture a Insights event',
            status: 'hidden',
    ...
    test('opening a workflow whose step uses a hidden template renders its config (no "Template not found")', async ({
            ...
            async ({ teamId, payload }) => {
                const csrfToken =
                    document.cookie
                        .split(';')
                        .map((c) => c.trim())
                        .find((c) => c.startsWith('insights_csrftoken='))
                        ?.split('=')
                        .slice(1)
                        .join('=') || ''
                if (!csrfToken) {
                ...
                const response = await fetch(`/v1/environments/${teamId}/script_flows/`, {
                    method: 'POST',
                    credentials: 'include',
                    headers: {
        ...
        await test.step('the template config renders instead of the not-found fallback', async () => {
            // `template-insights-capture` declares an "Event name" input in its inputs_schema.
