...
class TestBuildManualInsertSQL(BaseTest):
    ...
    def test_uses_provided_modifiers_when_printing(self):
        ...
        from insights.schema import InsightsQLQueryModifiers, SessionTableVersion
        ...
        sessions_query = """
            ...
            GROUP BY time_window_start
        """
        modifiers = InsightsQLQueryModifiers(sessionTableVersion=SessionTableVersion.V2, sessionIdPushdown=True)
...
class TestComputationExecutorExecute(BaseTest):
    ...
    def test_stale_while_revalidate_must_stay_under_expiry_buffer(self):
        # A grace at/above EXPIRY_BUFFER_SECONDS could return PG jobs whose Datastore
...
class TestInsertSettings(BaseTest):
    def test_insert_settings_guarantee_read_your_writes(self):
        ...
        # not hold the request for Datastore's 600s default quorum wait.
        assert settings["insert_quorum_timeout"] == PREAGGREGATION_INSERT_QUORUM_TIMEOUT_MS
        assert settings["load_balancing"] == "in_order"
        assert settings["max_execution_time"] == INSIGHTSQL_INCREASED_MAX_EXECUTION_TIME
...
class TestInsertSettingsAppliedToInserts(BaseTest):
    ...
    def test_manual_insert_path_passes_insert_settings_to_datastore(self):
        with patch(
    ...
    def test_ast_insert_path_passes_insert_settings_to_datastore(self):
        job = PreaggregationJob.objects.create(
            team=self.team,
            query_hash="test_hash",
            time_range_start=datetime(2024, 1, 1, tzinfo=UTC),
...
class TestExecuteOOMAndBudget(DatastoreTestMixin, BaseTest):
    def _query_info(self) -> QueryInfo:
        s = parse_select(
            """
