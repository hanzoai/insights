...
import { setupJsdom, setupSyncRaf } from '@hanzo/quill-charts/testing'
...
function sliceLabels(): string[] {
    return Array.from(document.querySelectorAll('[data-attr="script-chart-pie-slice-label"]')).map(
        (el) => el.textContent ?? ''
    )
}

// Napped × mascot fixture: Spike 11, Thistle 4, Bramble 2, Prickles 2, Conker 0.
// Conker drops out (0% < the 5% slice-label threshold); total of the rest is 19,
// so percent mode renders 11/19, 4/19, 2/19, 2/19.
const pieByMascot = (trendsFilter: Record<string, unknown> = {}): ReturnType<typeof buildTrendsQuery> =>
    buildTrendsQuery({
        series: [{ kind: NodeKind.EventsNode, event: 'Napped', name: 'Napped' }],
        breakdownFilter: { breakdown: 'mascot', breakdown_type: 'event' },
...
describe('TrendsPieChart (ActionsPie)', () => {
    it.each([
        ...
        },
        {
            ...
            query: pieByMascot({ showPercentStackView: true }),
    ...
    describe('quill in-chart legend', () => {
        const getInChartLegend = (container: HTMLElement): HTMLElement | null =>
            container.querySelector<HTMLElement>('[data-attr="script-chart-pie-legend"]')

        it('renders the in-chart legend and suppresses the legacy side legend', async () => {
            const { container } = renderInsight({ query: pieByMascot({ showLegend: true }) })
            await screen.findByLabelText(/pie chart with/i, undefined, { timeout: 5000 })
