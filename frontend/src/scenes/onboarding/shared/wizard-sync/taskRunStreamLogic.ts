...
export function parseTaskRunStreamMessage(rawData: string): TaskRunStreamMessage | null {
    ...
    if (payload.type === 'notification') {
        ...
        if (payload.notification?.method !== '_insights/progress' || !params) {
            return null
        }
        return {
            kind: 'step',
...
export interface taskRunStreamLogicValues {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface taskRunStreamLogicActions {
    ...
    progressStepUpdated: (step: TaskRunProgressStep) => {
        step: TaskRunProgressStep
    }
    runStalled: () => {
        value: true
    }
    ...
    taskRunStateUpdated: (state: TaskRunStreamState) => {
        state: TaskRunStreamState
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface taskRunStreamLogicMeta {
    key: string
}

export type taskRunStreamLogicType = MakeLogicType<
    taskRunStreamLogicValues,
    taskRunStreamLogicActions,
    TaskRunStreamLogicProps,
    taskRunStreamLogicMeta
>
 ...
 *
 * Data events (`task_run_state`, `_insights/progress` notifications) arrive unnamed via `onmessage`;
 ...
 * schedules its own backed-off reconnect, per the insights/api/streaming.py consumer contract. A named
 ...
 * `taskRunStateUpdated` action. Polling carries run state only — `_insights/progress` step
...
export const taskRunStreamLogic = kea<taskRunStreamLogicType>([
    ...
    connect(() => ({
        ...
        actions: [onboardingEventUsageLogic, ['reportSelfDrivingOnboardingCloudRunCompleted']],
    })),
    actions({
        connect: true,
        disconnect: true,
    ...
    listeners(({ values, actions, props, cache, selectors }) => ({
        taskRunStateUpdated: ({ state }, _breakpoint, _action, previousState) => {
            ...
            if (report && !reportedTerminalRuns.has(props.runId)) {
                ...
                actions.reportSelfDrivingOnboardingCloudRunCompleted({
                    taskId: props.taskId,
                    runId: props.runId,
                    ...report,
                })
