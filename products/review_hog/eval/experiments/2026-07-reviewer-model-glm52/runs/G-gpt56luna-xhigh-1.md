...
- **Validator:** - **Checked:** `process_inbox_pr_review` in `products/stamp/backend/tasks/tasks.py` (decorator, all three retry paths), its receiver dispatch in `products/review_hog/backend/receivers.py` (`_start_stamp_review` → `queue_inbox_pr_review` under `transaction.on_commit`), and the webhook re-review carve-out `_inbox_rereview_carve_out` that runs on later deliveries.
