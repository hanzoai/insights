...
# Published Modal image name of the prebaked Insights dev-stack VM image. Unlike
...
DEV_STACK_IMAGE_NAME = "insights-dev-stack"
...
def vm_sandbox_default_custom_image(payload: object) -> str | None:
    ...
    variants are org-targeted, so e.g. Insights's own org can point its standard VM runs
    at the prebaked insights dev-stack image (see
...
def get_vm_sandbox_flag_payload(*, distinct_id: str, organization_id: str) -> object:
    ...
    return hanzo_insights.get_feature_flag_payload(
...
# Insights `exec` sub-tools that must be approved by the user before they run, passed to the
# agent-server as `--insightsExecPermissionRegex`. A matching sub-tool is relayed to the connected
...
INSIGHTS_EXEC_DESTRUCTIVE_SUB_TOOLS: tuple[str, ...] = (
    ...
    "insights-connection-forward-execute",
