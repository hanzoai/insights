...
from insights.kafka_client import routing
from insights.kafka_client.routing import (
    KafkaClusterProfile,
...
class CurrentTopicRoutingTest(TestCase):
    ...
    def test_env_overrides_add_new_topic(self):
        with override_settings(
            KAFKA_TOPIC_ROUTING_OVERRIDES=f"{KAFKA_CDP_DATASTORE_PRECALCULATED_PERSON_PROPERTIES}=cyclotron"
        ):
            mapping = current_topic_routing()
        self.assertEqual(
            mapping.get(KAFKA_CDP_DATASTORE_PRECALCULATED_PERSON_PROPERTIES),
