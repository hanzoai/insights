                ...
                {top.map(([term, count]) => (
                    <div key={term} className="flex items-center gap-2">
                        ...
                        <div className="w-40 shrink-0 flex">
                            <Tag type="option" title={term} className="max-w-full truncate">
                                {term}
                            </Tag>
                        </div>
                        {showBars ? (
                            <Progress percent={percent(count)} className="flex-1" />
                        ) : (
...
function MonitorOverview({ scannerId }: { scannerId: string }): JSX.Element {
    ...
    return (
        <OverviewPanel title="Verdict mix" subtitle={`${total} verdict${total === 1 ? '' : 's'}`} fill>
            <Progress percent={yesPct} />
            <div className="flex flex-wrap items-center gap-4 text-sm">
                <span className="flex items-center gap-2">
                    <Tag type="highlight">Yes</Tag>
                    <span className="tabular-nums">
                        {yesTotal} ({yesPct}%)
                    </span>
                </span>
                <span className="flex items-center gap-2">
                    <Tag type="default">No</Tag>
                    <span className="tabular-nums">
                        {noTotal} ({noPct}%)
                    </span>
                </span>
                {inconclusiveTotal > 0 && (
                    <span className="flex items-center gap-2">
                        <Tag type="muted">Inconclusive</Tag>
                        <span className="tabular-nums">
