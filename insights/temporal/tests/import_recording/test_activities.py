...
async def test_import_recording_data_success(tmp_path):
    ...
    with (
        patch(
            "insights.temporal.import_recording.activities.session_recording_v2_object_storage.async_client"
        ) as mock_storage_client,
        patch("insights.temporal.import_recording.activities.Path") as mock_path_cls,
    ):
...
async def test_import_replay_clickhouse_rows_success(tmp_path):
    ...
    with (
        patch("insights.temporal.import_recording.activities.get_client") as mock_get_client,
        patch("insights.temporal.import_recording.activities.Path") as mock_path_cls,
    ):
...
async def test_import_event_clickhouse_rows_no_file(tmp_path):
    ...
    with patch("insights.temporal.import_recording.activities.Path") as mock_path_cls:
