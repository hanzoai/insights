...
search can filter by exact outcome inside the same Datastore ranking query, with no second-pass lookup.
"""
...
from insights.kafka_client.topics import KAFKA_DOCUMENT_EMBEDDINGS_INPUT_TOPIC
...
def _result_metadata(model_output: AnyScannerOutput) -> dict[str, Any]:
    """The exact outcome to stamp into the embedding metadata so search can filter on it inside Datastore."""
