...
//!
//! The paged scan walks `insights_person.id` ascending across a live table, so the run sees
...
pub struct BatchFlagEvaluationRequest {
    ...
    /// Exclusive lower bound on `insights_person.id`; 0 (default) starts from the beginning.
    #[serde(default)]
    pub cursor: i64,
...
/// (minimum `insights_persondistinctid.id`) distinct_id, when one exists.
...
/// lowest `insights_persondistinctid.id` (a deterministic choice). Persons with no
...
async fn scan_persons_page(
    ...
    // covered by the existing person_id index on insights_persondistinctid.
    let query = r#"
        SELECT p.id, p.uuid, d.distinct_id
        FROM insights_person p
        LEFT JOIN LATERAL (
            SELECT pd.distinct_id
            FROM insights_persondistinctid pd
...
async fn handle_batch_flag_evaluation(
    ...
    // evaluation and InsightsQL/Datastore cohort membership. The live path reads the same
