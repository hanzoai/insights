...
Devex / hygiene rules for the Insights codebase. Run by the `semgrep-devex` job in `ci-security.yaml` in two passes:
...
1. Verify locally that `semgrep --config .semgrep/rules/devex/<rule>.yaml common/ ee/ frontend/ insights/ products/` returns zero findings.
