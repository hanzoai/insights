...
from insights.models.integration import GitHubIntegrationError
...
class TestGithubSource:
    ...
    def test_get_oauth_accounts_maps_repositories(self, mock_get_oauth, mock_github_integration):
        ...
        mock_github_integration.return_value.list_cached_repositories.return_value = (
            [{"full_name": "Insights/insights"}, {"full_name": "Insights/code"}],
            False,
        )

        accounts = self.source.get_oauth_accounts(1, self.team_id)

        assert [a.value for a in accounts] == ["Insights/insights", "Insights/code"]
        assert [a.display_name for a in accounts] == ["Insights/insights", "Insights/code"]
    ...
    def test_get_oauth_accounts_pushes_search_down(self, mock_get_oauth, mock_github_integration):
        ...
        self.source.get_oauth_accounts(1, self.team_id, search="insights")

        list_repos.assert_called_once_with(search="insights", limit=100, offset=0)
    ...
    def test_get_oauth_accounts_skips_repos_without_full_name(self, mock_get_oauth, mock_github_integration):
        mock_get_oauth.return_value = mock.MagicMock()
        mock_github_integration.return_value.list_cached_repositories.return_value = (
            [{"full_name": "Insights/insights"}, {"full_name": ""}, {"id": 1}],
            False,
        )

        accounts = self.source.get_oauth_accounts(1, self.team_id)

        assert [a.value for a in accounts] == ["Insights/insights"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("Insights/insights", None, ["insights/insights"]),
            (None, ["Insights/insights", "insights/insights", " Other/Repo "], ["insights/insights", "other/repo"]),
            # A non-empty `repositories` is the authoritative set; `repository` only marks bare naming.
            ("insights/insights", ["a/b"], ["a/b"]),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("issues", (None, "issues")),
            ("insights/insights.issues", ("insights/insights", "issues")),
            # Repo names can contain dots — a naive rpartition('.') would split `next.js` wrong.
            ("insights/next.js.issues", ("insights/next.js", "issues")),
            ("insights/next.js.pull_requests", ("insights/next.js", "pull_requests")),
            ...
            ("insights/insights.deployments", ("insights/insights", "deployments")),
            ("insights/insights.deployment_statuses", ("insights/insights", "deployment_statuses")),
            # Unrecognized suffixes stay whole so unknown rows don't get misrouted.
            ("insights/insights.not_an_endpoint", (None, "insights/insights.not_an_endpoint")),
        ],
    )
    ...
    def test_create_webhook_shares_one_secret_across_repos(self, mock_ensure):
        ...
        # One script function has one signing_secret input, so every repo's hook must share it.
