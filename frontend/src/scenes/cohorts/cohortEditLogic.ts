            ...
            submit: (cohort) => {
                ...
                if (cohort.id !== 'new') {
                    ...
                    if (
                        ...
                        toast.error('You need to add at least one filter criterion.')
                        return
                    }
                    actions.saveCohort({
                        ...cohort,
                ...
                loadUsedIn: async () => {
                    ...
                    try {
                        ...
                        if (!(error instanceof ApiError) || error.status !== 404) {
                            insights.captureException(error, { feature: 'cohort-used-in' })
                        }
    ...
    listeners(({ actions, values }) => ({
        ...
        removePersonFromCohort: async ({ personId }) => {
            if (!values.cohort.id || values.cohort.id === 'new') {
                toast.error('Cannot remove a person from an unsaved cohort')
                return
            }
            ...
            try {
                ...
                toast.error(error.detail || 'Failed to remove person from cohort')
                insights.captureException(error, { feature: 'cohort-remove-person' })
                return
            }
