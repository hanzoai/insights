...
use personinsights_writer::config::Config;
use personinsights_writer::consumer::ConsumerTask;
use personinsights_writer::kafka::PersonConsumer;
use personinsights_writer::pg::PgStore;
use personinsights_writer::store::PersonWriteStore;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Starting personinsights-writer");
    ...
    let mut manager = Manager::builder("personinsights-writer")
    ...
    tokio::spawn(async move {
        ...
        let metrics_router = setup_metrics_routes_with_overrides(
            health_router,
            &[(
                Matcher::Full("personinsights_writer_e2e_latency_ms".into()),
                &[
    ...
    let pool_config = PoolConfig {
        ...
        pool_name: Some("personinsights-writer".to_string()),
    ...
    // PG_MAX_CONNECTIONS against observed utilization during fallback.
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ticker.tick().await;
                metrics::gauge!("personinsights_writer_pg_pool_size").set(pool.size() as f64);
                metrics::gauge!("personinsights_writer_pg_pool_idle").set(pool.num_idle() as f64);
            }
        });
    }
