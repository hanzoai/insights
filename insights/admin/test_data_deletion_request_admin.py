...
from insights.admin.admins.data_deletion_request_admin import EDITABLE_FIELDS, DataDeletionRequestAdmin, dagster_run_url
from insights.models.data_deletion_request import DataDeletionRequest, ExecutionMode, RequestStatus, RequestType
...
class TestDataDeletionRequestAdminApprovalFlow(BaseTest):
    def setUp(self):
        ...
        self.user.save()
        datastore_team, _ = Group.objects.get_or_create(name="Datastore Team")
        self.user.groups.add(datastore_team)
    ...
    def _call_approve(self, method: str, request: DataDeletionRequest, data: dict | None = None, user=None):
        path = f"/admin/insights/datadeletionrequest/{request.pk}/approve/"
        if method == "GET":
        ...
        with patch("insights.admin.admins.data_deletion_request_admin.reverse", side_effect=_fake_reverse):
    ...
    def test_approve_view_rejects_non_datastore_team_user(self):
        request = self._pending_request()
...
class TestDataDeletionRequestAdminRetry(BaseTest):
    ...
    def _call_retry(self, method: str, request: DataDeletionRequest, user=None):
        path = f"/admin/insights/datadeletionrequest/{request.pk}/retry/"
        if method == "GET":
            http_request = self.factory.get(path)
        else:
    ...
    def test_retry_view_rejects_non_datastore_team_user(self):
        request = self._failed_request()
        self.user.groups.clear()
        response = self._call_retry("POST", request)
...
class TestDataDeletionRequestAdminChangeView(BaseTest):
    ...
    def _can_retry(self, request: DataDeletionRequest, in_datastore_team: bool) -> bool:
        if in_datastore_team:
            datastore_team, _ = Group.objects.get_or_create(name="Datastore Team")
            self.user.groups.add(datastore_team)
        else:
            self.user.groups.clear()

        path = f"/admin/insights/datadeletionrequest/{request.pk}/change/"
        http_request = self.factory.get(path)
        ...
        with (
            patch("insights.admin.admins.data_deletion_request_admin.reverse", side_effect=_fake_reverse),
    ...
    def test_can_retry_only_when_failed_and_datastore_team(self):
        failed = self._make_request(RequestStatus.FAILED)
        self.assertTrue(self._can_retry(failed, in_datastore_team=True))

    def test_can_retry_false_when_not_in_datastore_team(self):
        failed = self._make_request(RequestStatus.FAILED)
        self.assertFalse(self._can_retry(failed, in_datastore_team=False))
    ...
    def test_can_retry_false_for_non_failed_status(self, _name, status):
        ...
        self.assertFalse(self._can_retry(request, in_datastore_team=True))
...
class TestDataDeletionRequestAdminSubmitView(BaseTest):
    ...
    def _call_submit(self, request: DataDeletionRequest, method: str = "POST", data: dict | None = None):
        path = f"/admin/insights/datadeletionrequest/{request.pk}/submit/"
    ...
    def test_submit_get_exposes_can_submit_flag(self):
        ...
        path_ok = f"/admin/insights/datadeletionrequest/{ok.pk}/submit/"
        http_req_ok = self.factory.get(path_ok)
        http_req_ok.user = self.user
        _attach_messages(http_req_ok)

        path_empty = f"/admin/insights/datadeletionrequest/{empty.pk}/submit/"
...
class TestDataDeletionRequestAdminSaveModel(BaseTest):
    ...
    def _call_save(self, obj: DataDeletionRequest, changed_data: list[str] | None = None):
        path = f"/admin/insights/datadeletionrequest/{obj.pk}/change/"
        http_request = self.factory.post(path)
        http_request.user = self.user
        _attach_messages(http_request)
...
class TestDataDeletionRequestAdminEditLock(BaseTest):
    ...
    def _readonly_fields(self, obj):
        http_request = self.factory.get("/admin/insights/datadeletionrequest/")
        http_request.user = self.user
...
class TestDataDeletionRequestAdminChangeViewStatsAndLock(BaseTest):
    ...
    def _change_context(
        self, request: DataDeletionRequest, in_datastore_team: bool = False, session: dict | None = None
    ):
        if in_datastore_team:
            datastore_team, _ = Group.objects.get_or_create(name="Datastore Team")
            self.user.groups.add(datastore_team)
        ...
        http_request = self.factory.get(f"/admin/insights/datadeletionrequest/{request.pk}/change/")
        http_request.user = self.user
        _attach_messages(http_request)
        if session:
    ...
    def test_change_view_is_datastore_team_flag(self):
        request = self._make_request()
        self.assertTrue(self._change_context(request, in_datastore_team=True)["is_datastore_team"])
        self.assertFalse(self._change_context(request, in_datastore_team=False)["is_datastore_team"])
...
class TestDataDeletionRequestAdminStatsViewRedirects(BaseTest):
    ...
    def _call(self, view, request: DataDeletionRequest, user=None):
        http_request = self.factory.post(f"/admin/insights/datadeletionrequest/{request.pk}/x/")
        http_request.user = user or self.user
        _attach_messages(http_request)
        with patch("insights.admin.admins.data_deletion_request_admin.reverse", side_effect=_fake_reverse):
    ...
    def test_fetch_stats_redirects_to_change_page(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn("insights_datadeletionrequest_change", response.url)
        request.refresh_from_db()
    ...
    def test_preview_stats_rejects_non_datastore_team(self):
        request = self._person_request()
        self.user.groups.clear()
        ...
        self.assertIn("Only Datastore Team members can preview stats.", [str(m) for m in get_messages(http_request)])
...
class TestDataDeletionRequestAdminVerify(BaseTest):
    ...
    def _call_verify(self, request, user=None):
        path = f"/admin/insights/datadeletionrequest/{request.pk}/verify/"
        http_request = self.factory.post(path)
        http_request.user = user or self.user
        _attach_messages(http_request)
        with patch("insights.admin.admins.data_deletion_request_admin.reverse", side_effect=_fake_reverse):
    ...
    def test_verify_view_promotes_when_events_gone(self):
        ...
        with patch("insights.models.data_deletion_request.count_remaining_matching_events", return_value=0):
    ...
    def test_verify_view_keeps_queued_when_events_remain(self):
        request = self._queued_request()
        with patch("insights.models.data_deletion_request.count_remaining_matching_events", return_value=5):
            response = self._call_verify(request)
        self.assertEqual(response.status_code, 302)
        request.refresh_from_db()
    ...
    def test_verify_view_keeps_failed_when_events_remain(self):
        ...
        request.status = RequestStatus.FAILED
        request.save(update_fields=["status"])
        with patch("insights.models.data_deletion_request.count_remaining_matching_events", return_value=3):
            self._call_verify(request)
        request.refresh_from_db()
        self.assertEqual(request.status, RequestStatus.FAILED)
    ...
    def test_verify_view_verifies_property_removal(self):
        ...
        with patch("insights.models.data_deletion_request.count_remaining_property_events", return_value=0):
            self._call_verify(request)
        request.refresh_from_db()
        self.assertEqual(request.status, RequestStatus.COMPLETED)
    ...
    def test_verify_view_reports_person_removal_unsupported(self):
        ...
        with patch("insights.admin.admins.data_deletion_request_admin.count_remaining_for_request") as counted:
            self._call_verify(request)
        counted.assert_not_called()
        request.refresh_from_db()
        self.assertEqual(request.status, RequestStatus.FAILED)

    def test_verify_view_rejects_non_datastore_team(self):
        request = self._queued_request()
        self.user.groups.clear()
        self._call_verify(request)
        request.refresh_from_db()
...
class TestDataDeletionRequestAdminDuplicate(BaseTest):
    ...
    def _call_duplicate(self, queryset):
        http_request = self.factory.post("/admin/insights/datadeletionrequest/")
        http_request.user = self.user
        _attach_messages(http_request)
        with patch("insights.admin.admins.data_deletion_request_admin.reverse", side_effect=_fake_reverse):
...
class TestDagsterRunLink(SimpleTestCase):
    ...
    @parameterized.expand(
        [
            ("cloud_us", None, "US", f"https://insights.dagster.cloud/prod-us/runs/{RUN_ID}"),
            ("cloud_eu", None, "EU", f"https://insights.dagster.cloud/prod-eu/runs/{RUN_ID}"),
            ("dev", None, "DEV", f"https://insights.dagster.cloud/dev/runs/{RUN_ID}"),
            ("lowercase_deployment", None, "eu", f"https://insights.dagster.cloud/prod-eu/runs/{RUN_ID}"),
    ...
    def test_admin_renders_run_id_as_link(self):
        ...
        self.assertIn(f'href="https://insights.dagster.cloud/prod-eu/runs/{self.RUN_ID}"', rendered)
...
class TestDataDeletionRequestFormHidesPersonRemoval(SimpleTestCase):
    def test_person_removal_hidden_from_type_picker_and_fields(self):
        ...
        from insights.admin.admins.data_deletion_request_admin import PERSON_REMOVAL_FIELDS, DataDeletionRequestForm
