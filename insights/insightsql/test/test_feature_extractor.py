...
from insights.insightsql.feature_extractor import InsightsQLFeatureExtractor, extract_insightsql_features
from insights.insightsql.parser import parse_select
from insights.insightsql.resolver import resolve_types


class TestInsightsQLFeatureExtractor(BaseTest):
    ...
    def test_extracts_table_references(self, _name, sql, expected_tables, expected_events):
        features = extract_insightsql_features(parse_select(sql))
    ...
    def test_visitor_dedups(self):
        visitor = InsightsQLFeatureExtractor()
        visitor.visit(
            parse_select(
    ...
    def _resolve(self, sql: str) -> ast.SelectQuery | ast.SelectSetQuery:
        ...
        context = InsightsQLContext(database=database, team_id=self.team.pk, enable_select_queries=True)
        node = parse_select(sql)
        return cast(ast.SelectQuery | ast.SelectSetQuery, resolve_types(node, context, dialect="datastore"))
    ...
    def test_resolved_ast_uses_type_system(self):
        ...
        features = extract_insightsql_features(self._resolve("SELECT * FROM events WHERE event = '$exception'"))
    ...
    def test_resolved_event_field_on_non_events_table_ignored(self):
        ...
        from insights.insightsql.database.schema.query_log_archive import QueryLogArchiveTable
