...
// writes) therefore don't work through `insights-cli api`.
...
// what `insights-cli api` / the MCP actually requires.
...
export interface cliAuthorizeLogicValues {
...
export interface cliAuthorizeLogicActions {
    ...
    loadProjects: (organizationId: string) => string
    loadProjectsFailure: (
        error: string,
        errorObject?: any
    ) => {
    ...
    setScopePreset: (preset: string | null) => {
        preset: string | null
    }
    setScopeRadioValue: (
        key: string,
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface cliAuthorizeLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        organizations: (user: UserType | null) => OrganizationBasicType[]
        formScopeRadioValues: (authorize: CLIAuthorizeForm) => Record<string, string>
        filteredScopes: (searchTerm: string) => APIScope[]
        allAccessSelected: (authorize: CLIAuthorizeForm) => boolean
        scopePreset: (authorize: CLIAuthorizeForm) => string | null
        ...
        missingAgentScopes: (authorize: CLIAuthorizeForm, requestedUseCases: CLIUseCase[]) => boolean
    }
}

export type cliAuthorizeLogicType = MakeLogicType<
    cliAuthorizeLogicValues,
    cliAuthorizeLogicActions,
    Record<string, any>,
    cliAuthorizeLogicMeta
>
...
export const cliAuthorizeLogic = kea<cliAuthorizeLogicType>([
        ...
        projects: [
            [] as { id: number; name: string }[],
            {
                loadProjects: async (organizationId: string) => {
                    const response = await api.get(`v1/organizations/${organizationId}/projects/`)
    ...
    selectors(() => ({
        ...
        allAccessSelected: [
            (s) => [s.authorize],
            (authorize: CLIAuthorizeForm): boolean => !!authorize?.scopes?.includes('*'),
        ],
        scopePreset: [
            (s) => [s.authorize],
            (authorize: CLIAuthorizeForm): string | null => presetForScopes(authorize?.scopes ?? []),
        ],
        missingSchemaScopes: [
            (s) => [s.authorize, s.requestedUseCases],
            (authorize: CLIAuthorizeForm, requestedUseCases: CLIUseCase[]): boolean => {
