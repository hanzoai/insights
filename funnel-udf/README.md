...
4. Land a version of the insights repo with the updated `user_scripts` folder from the new branch (make sure this PR doesn't include changes to this file with the new version)
5. Run the `copy_udfs_to_datastore` action in the `insights_cloud_infra` repo to deploy the `user_scripts` folder to datastore
   - Verify that the new UDF works, by adapting the version in a CH query for a funnel insight.

6. After that deploy goes out, it is safe to land and deploy the full changes to the `insights` repo
