...
from posthog.insightsql.database.models import DatabaseField, LazyJoinToAdd, LazyTableToAdd
from posthog.insightsql.database.schema.util.uuid import (
    uuid_uint128_expr_to_timestamp_expr_v2,
    uuid_uint128_expr_to_timestamp_expr_v3,
)
from posthog.insightsql.errors import NotImplementedError, QueryError
from posthog.insightsql.functions.mapping import INSIGHTSQL_COMPARISON_MAPPING
from posthog.insightsql.helpers.timestamp_visitor import is_simple_timestamp_field_expression, is_time_or_interval_constant
...
class WhereClauseExtractor(CloningVisitor):
    ...
    def visit_call(self, node: ast.Call) -> ast.Expr:
        ...
        elif node.name in INSIGHTSQL_COMPARISON_MAPPING:
...
def rewrite_timestamp_field(expr: ast.Expr, timestamp_field: ast.Expr, context: InsightsQLContext) -> ast.Expr:
...
class RewriteTimestampFieldVisitor(CloningVisitor):
    ...
    def __init__(self, context: InsightsQLContext, timestamp_field: ast.Expr, *args, **kwargs):
    ...
    def visit_field(self, node: ast.Field) -> ast.Expr:
        ...
        from posthog.insightsql.database.schema.session_replay_events import RawSessionReplayEventsTable
        from posthog.insightsql.database.schema.sessions_v1 import SessionsTableV1
        from posthog.insightsql.database.schema.sessions_v2 import SessionsTableV2
        from posthog.insightsql.database.schema.sessions_v3 import SessionsTableV3
...
def is_session_id_string_expr(node: ast.Expr, context: InsightsQLContext) -> bool:
