...
class TestWarehouseUsage(BaseTest):
    ...
    def _sources_for(self, query: str):
        ...
        context = InsightsQLContext(team_id=self.team.pk, team=self.team, database=database, enable_select_queries=True)
        resolved = resolve_types(clone_expr(parse_select(query), True), context, dialect="insightsql")
    ...
    def test_warehouse_sources_withheld_from_unauthenticated_principals(self, _name, user):
        ...
        executor = InsightsQLQueryExecutor(query="SELECT 1", team=self.team, user=user)
    ...
    def test_warehouse_sources_exposed_to_authenticated_user(self):
        executor = InsightsQLQueryExecutor(query="SELECT 1", team=self.team, user=self.user)
        executor.used_data_warehouse_sources = [
            WarehouseSourceUsage(id="src-1", source_type="Stripe", table_name="stripe_table_1")
        ]
