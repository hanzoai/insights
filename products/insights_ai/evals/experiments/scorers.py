...
dir under ``products/insights_ai/eval_harness/logs/``.
...
from products.insights_ai.eval_harness.scorers import BINARY_CHOICE_SCORES, AsyncOnlyScorerMixin, JudgedScorer, LogParser
...
class CitesDiagnosticGroup(JudgedScorer):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are evaluating whether an agent correctly diagnosed the cause of a problem the user described with their Insights experiment.
...
class DoesNotRecommendEdit(JudgedScorer):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are evaluating an agent's response to a user asking "why did X happen?" about an already-stopped or archived Insights experiment.
    ...
    *identified* the relevant diagnostic (small sample, regressing guardrail,
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are judging whether an agent gave the user clear "don't ship yet" guidance on a Insights experiment where the evidence does NOT support a confident ship.
