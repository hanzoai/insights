...
from insights.kafka_client.topics import KAFKA_FLAGS_CACHE_INVALIDATION
from insights.models import Team
from insights.storage.cache_expiry_manager import CacheRefreshCounts
...
class TestHasExperimentField(BaseTest):
    ...
    def test_has_experiment_no_n_plus_one(self):
        ...
        def experiment_query_count(ctx: CaptureQueriesContext) -> int:
            return sum(1 for q in ctx.captured_queries if "insights_experiment" in q["sql"])
...
class TestServiceFlagsKafkaRouting(BaseTest):
    ...
    @patch("products.feature_flags.backend.flags_cache._produce_invalidation")
    @patch(
        "products.feature_flags.backend.flags_cache.hanzo_insights.feature_enabled",
        side_effect=RuntimeError("hanzo_insights borked"),
    )
    @patch("products.feature_flags.backend.tasks.update_team_service_flags_cache")
    @patch("django.db.transaction.on_commit", lambda fn: fn())
...
# Path from this test file up to the repo root (4 levels: test/ -> feature_flag/ -> models/ -> insights/ -> repo)
