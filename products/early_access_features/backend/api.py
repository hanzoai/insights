...
class MinimalEarlyAccessFeatureSerializer(serializers.ModelSerializer):
    ...
    (project-token auth), so consumers like hanzo.ai's roadmap can attribute a feature to
...
class EarlyAccessFeatureSerializerCreateOnly(EarlyAccessFeatureSerializer):
    ...
    def validate(self, data):
        # Insights's own dogfooding project requires a description on every new early access feature.
