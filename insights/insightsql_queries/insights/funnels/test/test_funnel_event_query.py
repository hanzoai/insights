...
def format_query(query: ast.SelectQuery):
    return sqlparse.format(query.to_insightsql(), keyword_case="upper", reindent=True)


def raw_query(query: ast.SelectQuery) -> str:
    # The single-line InsightsQL string, without sqlparse reindentation, so substring
    # assertions don't break on line wrapping.
    return query.to_insightsql()
...
class TestFunnelEventQuery(DatastoreTestMixin, APIBaseTest):
    ...
    def _create_data_warehouse_table(
        ...
        columns: dict[str, dict[str, str | bool]] = {
            id_field: id_column or {"insightsql": "StringDatabaseField", "datastore": "String", "valid": True},
    ...
    def test_dwh_uuid_id_field_null_guard(self, _name: str, datastore_type: str, nullable: bool) -> None:
        self._create_data_warehouse_table(
            name="payments_uuid",
            id_field="uuid_id",
            distinct_id_field="user_id",
            timestamp_field="created_at",
            id_column={"insightsql": "UUIDDatabaseField", "datastore": datastore_type, "valid": True},
        )
        dwh_node = FunnelsDataWarehouseNode(
            id="payments_uuid",
