import { MakeLogicType, actions, connect, kea, listeners, path, reducers } from 'kea'
import { forms } from 'kea-forms'
import type { DeepPartial, DeepPartialMap, FieldName, ValidationErrorType } from 'kea-forms'
import { router, urlToAction } from 'kea-router'

import { toast } from '@hanzo/elements'

import api from 'lib/api'
import { databaseTableListLogic } from 'scenes/data-management/database/databaseTableListLogic'
import { urls } from 'scenes/urls'

import { FileUploadFormat } from './fileUploadSource'

// Mirrors MAX_FILE_UPLOAD_SIZE_BYTES on the backend. Checked here too so an oversized file fails
// instantly instead of after uploading 50MB+ only to be rejected.
export const MAX_FILE_UPLOAD_SIZE_BYTES = 50 * 1024 * 1024

export const FILE_UPLOAD_ACCEPT: Record<FileUploadFormat, string> = {
    csv: '.csv',
    json: '.json,.ndjson',
    parquet: '.parquet',
}

const INSIGHTSQL_TABLE_NAME_REGEX = /^[a-zA-Z_][a-zA-Z0-9_]*$/

export interface FileUploadForm {
    files: File[]
    table_name: string
    file_format: FileUploadFormat
}

export function deriveTableName(filename: string): string {
    const withoutExtension = filename.replace(/\.[^.]+$/, '')
    const slug = withoutExtension
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
        .replace(/_+$/, '')
    if (!slug) {
        return 'uploaded_file'
    }
    return /^[0-9]/.test(slug) ? `_${slug}` : slug
}

export function parseFileUploadFormat(value: string | undefined): FileUploadFormat | null {
    const normalized = value?.toLowerCase()
    return normalized === 'csv' || normalized === 'json' || normalized === 'parquet' ? normalized : null
}

export function fileUploadFormErrors({ files, table_name }: Partial<FileUploadForm>): Record<string, unknown> {
    const file = files?.[0]
    return {
        files: !file
            ? 'Please choose a file to upload.'
            : file.size > MAX_FILE_UPLOAD_SIZE_BYTES
              ? 'This file is larger than 50MB. For bigger files, connect the bucket they live in as a self-managed source instead.'
              : undefined,
        table_name: !table_name
            ? 'Please enter a table name.'
            : !INSIGHTSQL_TABLE_NAME_REGEX.test(table_name)
              ? 'Table names must start with a letter or underscore and contain only letters, numbers, and underscores.'
              : undefined,
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface fileUploadSourceLogicValues {
    fileUpload: FileUploadForm
    fileUploadAllErrors: Record<string, any>
    fileUploadChanged: boolean
    fileUploadErrors: DeepPartialMap<FileUploadForm, ValidationErrorType>
    fileUploadHasErrors: boolean
    fileUploadManualErrors: Record<string, any>
    fileUploadTouched: boolean
    fileUploadTouches: Record<string, boolean>
    fileUploadValidationErrors: DeepPartialMap<FileUploadForm, ValidationErrorType>
    isFileUploadSubmitting: boolean
    isFileUploadValid: boolean
    showFileUploadErrors: boolean
    tableNameEdited: boolean
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface fileUploadSourceLogicActions {
    loadDatabase: (
        args_0?:
            | {
                  force?: boolean
              }
            | undefined
    ) => {
        force?: boolean
    } // databaseTableListLogic
    resetFileUpload: (values?: FileUploadForm) => {
        values?: FileUploadForm
    }
    selectFiles: (files: File[]) => {
        files: File[]
    }
    setFileFormat: (fileFormat: FileUploadFormat) => {
        fileFormat: FileUploadFormat
    }
    setFileUploadManualErrors: (errors: Record<string, any>) => {
        errors: Record<string, any>
    }
    setFileUploadValue: (
        key: FieldName,
        value: any
    ) => {
        name: FieldName
        value: any
    }
    setFileUploadValues: (values: DeepPartial<FileUploadForm>) => {
        values: DeepPartial<FileUploadForm>
    }
    setTableName: (tableName: string) => {
        tableName: string
    }
    submitFileUpload: () => {
        value: boolean
    }
    submitFileUploadFailure: (
        error: Error,
        errors: Record<string, any>
    ) => {
        error: Error
        errors: Record<string, any>
    }
    submitFileUploadRequest: (fileUpload: FileUploadForm) => {
        fileUpload: FileUploadForm
    }
    submitFileUploadSuccess: (fileUpload: FileUploadForm) => {
        fileUpload: FileUploadForm
    }
    touchFileUploadField: (key: string) => {
        key: string
    }
}

export type fileUploadSourceLogicType = MakeLogicType<fileUploadSourceLogicValues, fileUploadSourceLogicActions>

export const fileUploadSourceLogic = kea<fileUploadSourceLogicType>([
    path(['products', 'dataWarehouse', 'fileUploadSourceLogic']),
    connect(() => ({
        actions: [databaseTableListLogic, ['loadDatabase']],
    })),
    actions({
        selectFiles: (files: File[]) => ({ files }),
        setTableName: (tableName: string) => ({ tableName }),
        setFileFormat: (fileFormat: FileUploadFormat) => ({ fileFormat }),
    }),
    reducers({
        // Once the user types their own table name we stop overwriting it when they swap the file.
        tableNameEdited: [false, { setTableName: () => true }],
    }),
    forms(({ actions }) => ({
        fileUpload: {
            defaults: {
                files: [] as File[],
                table_name: '',
                file_format: parseFileUploadFormat(router.values.searchParams.format) ?? 'csv',
            } as FileUploadForm,
            errors: fileUploadFormErrors,
            submit: async ({ files, table_name, file_format }) => {
                const file = files[0]
                // The form's `errors` already block submission, but a stale/oversized file can only
                // be reported as a toast — the button is what the user is looking at.
                if (!file) {
                    toast.error('Please choose a file to upload.')
                    return
                }
                if (file.size > MAX_FILE_UPLOAD_SIZE_BYTES) {
                    toast.error('This file is larger than the 50MB upload limit.')
                    return
                }

                const formData = new FormData()
                formData.append('file', file)
                formData.append('file_format', file_format)

                let upload
                try {
                    upload = await api.dataWarehouseTables.uploadFile(formData)
                } catch (e: any) {
                    toast.error(e.data?.message ?? e.message ?? 'Could not upload the file.')
                    return
                }

                try {
                    await api.dataWarehouseTables.createFromUpload({
                        upload_id: upload.upload_id,
                        filename: upload.filename,
                        file_format,
                        table_name,
                    })
                } catch (e: any) {
                    toast.error(e.data?.message ?? e.message ?? 'Could not create the table.')
                    return
                }

                toast.success(`Table ${table_name} created`)
                actions.loadDatabase()
                router.actions.replace(urls.sources())
            },
        },
    })),
    listeners(({ actions, values }) => ({
        selectFiles: ({ files }) => {
            actions.setFileUploadValue('files', files)
            const file = files[0]
            if (file && !values.tableNameEdited) {
                actions.setFileUploadValue('table_name', deriveTableName(file.name))
            }
        },
        setTableName: ({ tableName }) => {
            actions.setFileUploadValue('table_name', tableName)
        },
        setFileFormat: ({ fileFormat }) => {
            actions.setFileUploadValue('file_format', fileFormat)
        },
    })),
    urlToAction(({ actions }) => ({
        [urls.dataWarehouseSourceNew()]: (_, searchParams) => {
            const format = parseFileUploadFormat(searchParams.format)
            if (format) {
                actions.setFileFormat(format)
            }
        },
    })),
])
