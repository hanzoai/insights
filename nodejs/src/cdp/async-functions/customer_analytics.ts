    ...
    if (!team.secret_api_token) {
        ...
        captureException(error, {
            tags: {
                ...
                template_id: context.invocation.insightsFunction.template_id ?? null,
            },
        })
        throw error
    }
...
registerAsyncFunction('insightsGetAccount', {
    execute: async (args, context, result) => {
        const [opts] = args as [Record<string, any> | undefined]
        const externalId = opts?.external_id

        if (!externalId || typeof externalId !== 'string') {
            throw new Error("[InsightsFunction] - insightsGetAccount call missing 'external_id' property")
        }

        const team = await getTeamWithSecretToken(context, 'insightsGetAccount')
    ...
    mock: (args, logs) => {
        logs.push({
            ...
            message: `Async function 'insightsGetAccount' was mocked with arguments:`,
        })
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `insightsGetAccount(${JSON.stringify(args[0], null, 2)})`,
        })
...
registerAsyncFunction('insightsUpdateAccount', {
    execute: async (args, context, result) => {
        const [opts] = args as [Record<string, any> | undefined]
        const externalId = opts?.external_id
        ...
        if (!externalId || typeof externalId !== 'string') {
            throw new Error("[InsightsFunction] - insightsUpdateAccount call missing 'external_id' property")
        }

        const team = await getTeamWithSecretToken(context, 'insightsUpdateAccount')
    ...
    mock: (args, logs) => {
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `Async function 'insightsUpdateAccount' was mocked with arguments:`,
        })
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `insightsUpdateAccount(${JSON.stringify(args[0], null, 2)})`,
        })
...
registerAsyncFunction('insightsSetAccountProperties', {
    execute: async (args, context, result) => {
        const [opts] = args as [Record<string, any> | undefined]
        const externalId = opts?.external_id
        ...
        if (!externalId || typeof externalId !== 'string') {
            throw new Error("[InsightsFunction] - insightsSetAccountProperties call missing 'external_id' property")
        }

        const team = await getTeamWithSecretToken(context, 'insightsSetAccountProperties')
    ...
    mock: (args, logs) => {
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `Async function 'insightsSetAccountProperties' was mocked with arguments:`,
        })
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `insightsSetAccountProperties(${JSON.stringify(args[0], null, 2)})`,
        })
...
registerAsyncFunction('insightsCreateAccount', {
    execute: async (args, context, result) => {
        const [opts] = args as [Record<string, any> | undefined]
        const externalId = opts?.external_id

        if (!externalId || typeof externalId !== 'string') {
            throw new Error("[InsightsFunction] - insightsCreateAccount call missing 'external_id' property")
        }

        const team = await getTeamWithSecretToken(context, 'insightsCreateAccount')
    ...
    mock: (args, logs) => {
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `Async function 'insightsCreateAccount' was mocked with arguments:`,
        })
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `insightsCreateAccount(${JSON.stringify(args[0], null, 2)})`,
        })
