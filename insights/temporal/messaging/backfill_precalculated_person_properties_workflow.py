...
from insights.temporal.messaging.datastore_concurrency import get_messaging_client
from insights.temporal.messaging.filter_storage import get_filters_and_properties
from insights.temporal.messaging.insightsql_compile import compile_insightsql_for_streaming
from insights.temporal.messaging.types import PersonPropertyFilter
...
def build_person_properties_select_fields(
    ...
    ``argmax_select`` wraps it in the per-person ``argMax`` aggregation: InsightsQL either rewrites to a
...
def get_query_duration_metric():
    """Histogram for Datastore query durations."""
    return temporalio.activity.metric_meter().create_histogram_float(
        "backfill_datastore_query_duration_seconds", "Duration of Datastore queries in seconds", unit="seconds"
    )
...
def evaluate_combined_filters_sync(
    ...
    try:
        ...
        if detailed_logging:
            LOGGER.warning(
                "ScriptVM evaluation returned non-dict result",
                person_id=person_id,
                result=result,
                result_type=type(result).__name__,
            )
...
async def backfill_precalculated_person_properties_activity(
    ...
    async with Heartbeater(
        ...
        kafka_producer = get_producer(topic=KAFKA_CDP_DATASTORE_PRECALCULATED_PERSON_PROPERTIES)
        ...
        try:
            ...
            query_duration_metric = metric_meter.create_histogram_float(
                "backfill_datastore_query_duration_seconds",
                "Duration of Datastore queries in seconds",
                unit="seconds",
            )
        ...
        # Build the per-property argmax_select fields — either targeted property accessors (so InsightsQL
        # can use materialized columns when present) or the full ``properties`` JSON as a fallback.
        property_alias_mapping: dict[str, str] = {}
        select_fields: dict[str, list[str | int]] = {}

        # A '%' in a property key makes the InsightsQL printer raise (it can resolve the accessor to a
        ...
        if person_properties and len(person_properties) <= MAX_OPTIMIZED_PROPERTIES and not has_identifier_unsafe_key:
            ...
            logger.info(f"Optimized query: fetching {len(person_properties)} specific properties via InsightsQL")
        else:
            ...
            if person_properties and len(person_properties) > MAX_OPTIMIZED_PROPERTIES:
                ...
                logger.warning(
                    "Property key contains '%' (not a valid InsightsQL identifier) - falling back to fetching all properties"
                )
            else:
                logger.warning(
        ...
        # optimize_aggregation_in_order lets Datastore aggregate as it streams the primary key in
        # order, so the GROUP BY + ORDER BY id never buffers the whole range. No explicit limit:
        # compile_insightsql_for_streaming uses LimitContext.COHORT_CALCULATION (LIMIT 1_000_000_000), and
        # ID ranges are bounded by batch_size (default 1000 persons), so the cap is never reached.
        persons_query_ast.settings = InsightsQLQuerySettings(optimize_aggregation_in_order=True)

        persons_query, query_params = await compile_insightsql_for_streaming(persons_query_ast, team_id=inputs.team_id)

        last_person_id = inputs.start_person_id
        batch_count = 0

        logger.info("Starting Datastore client connection and query execution")
        ...
        with tags_context(
            ...
            logger.info("Acquiring Datastore client connection", team_id=inputs.team_id)
            async with get_messaging_client(team_id=inputs.team_id) as client:
                logger.info(
                    "Datastore client connection established, starting query execution",
                    team_id=inputs.team_id,
                    query=persons_query,
                    query_params=query_params,
                )
                # Time the Datastore query execution
                ...
                async for row in client.stream_query_as_jsonl(persons_query, query_parameters=query_params):
                    if first_row:
                        ...
                        logger.info(
                            "First row received from Datastore query",
                            team_id=inputs.team_id,
                    ...
                    if person_properties and FULL_PROPERTIES_KEY not in row:
                        ...
                        for alias, original_prop_name in property_alias_mapping.items():
                            value = row.get(alias)
                            # The InsightsQL property extract maps a missing key to SQL NULL (-> None here),
                            ...
                            try:
                                ...
                                if detailed_logging_enabled:
                                    logger.info(
                                        ...
                                        matches=matches,
                                        kafka_topic=KAFKA_CDP_DATASTORE_PRECALCULATED_PERSON_PROPERTIES,
                                    )
                            except Exception as e:
                                logger.warning(
