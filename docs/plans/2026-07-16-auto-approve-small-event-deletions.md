...
`DataDeletionRequest` (Django admin only, `insights/admin/admins/data_deletion_request_admin.py`) requires every
request to walk the full `draft → pending → approved` path, with the APPROVED transition gated on Datastore Team
membership. Approval exists because Datastore deletes are heavyweight mutations that can degrade production query
...
Datastore stats itself, immediately before deciding, so it approves against a count it measured rather than one an
...
1. Refresh its Datastore stats and persist them.
...
3. Otherwise leave it PENDING for the Datastore Team, with the reason logged.
...
- **Only candidates get their stats refreshed**, not every pending request. A pending request sits until a human acts,
  so refreshing all of them would re-run the same (possibly expensive) Datastore query every 30 minutes forever, on
  requests the job can't act on anyway.
- **A failure on one request is logged and skipped**, never raised. One request whose InsightsQL predicate no longer
...
- **`submit_view` deliberately keeps no `DATASTORE_TEAM_GROUP` check**, unlike `approve_view` / `retry_view` / the
  verify path. Any staff user can submit a request that the job may then approve without a Datastore Team member
...
### `insights/models/data_deletion_request.py`

Already the shared home for this feature's Datastore logic (`event_removal_where`, `count_remaining_for_request`,
...
### `insights/dags/data_deletion_requests.py`
...
Registered in `insights/dags/locations/datastore.py`.

### `insights/admin/admins/data_deletion_request_admin.py`
...
### `insights/templates/admin/insights/datadeletionrequest/submit.html`
...
- `insights/models/test/test_data_deletion_request.py` — parameterized `auto_approve_blocker` (pure function, no DB).
- `insights/dags/tests/test_data_deletion_requests.py` — the job against the real cluster fixture: approves a small
...
- `insights/admin/test_data_deletion_request_admin.py` — submit lands in PENDING and records the opt-out; the checkbox
  is offered only to event removals.

## Verification

```bash
insightscli test insights/admin/test_data_deletion_request_admin.py
insightscli test insights/models/test/test_data_deletion_request.py
insightscli test insights/dags/tests/test_data_deletion_requests.py
uv run python manage.py makemigrations --check --dry-run
uv run python manage.py analyze_migration_risk --fail-on-blocked
ruff check insights --fix && ruff format insights
```

End-to-end in the local admin (`insightscli start`, then `/admin/insights/datadeletionrequest/`):
