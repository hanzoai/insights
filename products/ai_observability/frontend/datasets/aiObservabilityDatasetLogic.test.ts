...
describe('aiObservabilityDatasetLogic', () => {
    ...
    describe('dataset archiving', () => {
        ...
        it('restores the dataset and reloads the active list from Undo', async () => {
            ...
            const toastOptions = (toast.info as jest.Mock).mock.calls.at(-1)?.[1] as {
            ...
            expect(toast.success).toHaveBeenCalledWith('Test Dataset has been restored.')
        })
        ...
        it('shows an error when Undo cannot restore the dataset', async () => {
            ...
            expect(toast.error).toHaveBeenCalledWith("Couldn't restore dataset. Try again.")
        })
    })
    ...
    describe('dataset export', () => {
        ...
        it('reports a workflow start failure without tracking it as pending', async () => {
            ...
            expect(toast.error).toHaveBeenCalledWith('The export could not be started.')
    ...
    describe('dataset item archiving', () => {
        it('offers to reload items when archiving an outdated version', async () => {
            ...
            const toastOptions = (toast.error as jest.Mock).mock.calls.at(-1)?.[1] as {
                button: { action: () => void }
            }
            toastOptions.button.action()
            await expectLogic(logic).toFinishAllListeners()
        ...
        it('offers to reload items when Undo finds a newer version', async () => {
            ...
            const undoToastOptions = (toast.info as jest.Mock).mock.calls.at(-1)?.[1] as {
                button: { action: () => Promise<void> }
            }
            ...
            const errorToastOptions = (toast.error as jest.Mock).mock.calls.at(-1)?.[1] as {
                button: { action: () => void }
            }
