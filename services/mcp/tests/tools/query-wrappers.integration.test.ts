...
describe('Query Wrapper Integration Tests', { concurrent: false }, () => {
    ...
    describe('query-trends', () => {
        it('should execute a basic trends query and return formatted results', async () => {
            ...
            expect(result._insightsUrl).toMatch(/\/insights\/new#q=/)
            ...
            const result = (await tool.handler(context, {
                source: {
                    ...
                    pathsFilter: {
                        ...
                        startPoint: 'https://hanzo.ai/',
                        endPoint: 'https://us.hanzo.ai/',
                        pathEndKey: '3_https://us.hanzo.ai',
                    },
                },
            })) as any

            const sourcePathsFilter = result.query.source.source.pathsFilter
            expect(sourcePathsFilter.pathEndKey).toBe('3_https://us.hanzo.ai')
            expect(sourcePathsFilter.startPoint).toBe('https://hanzo.ai/')
            expect(sourcePathsFilter.endPoint).toBe('https://us.hanzo.ai/')
        })
    })
