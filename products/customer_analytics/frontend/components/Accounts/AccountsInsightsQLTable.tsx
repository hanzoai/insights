...
import { Button, ColorGlyph, Skeleton, Table, ProfilePicture } from '@hanzo/elements'
...
import { SortingIndicator } from 'lib/elements/Table/sorting'
...
import { ACCOUNTS_INSIGHTSQL_DATA_NODE_KEY } from '../../constants'
                ...
                <Link
                    ...
                    onClick={(event) => {
                        ...
                        if (!isAccountExpanded(accountId)) {
                            insights.capture(AccountsEvents.AccountOpened)
                        }
    ...
    if (definition.display_type === 'select') {
        ...
        return (
            <span className="inline-flex items-center gap-1.5">
                {option && <ColorGlyph colorToken={option.color as DataColorToken} size="small" />}
                <span>{value}</span>
            </span>
        )
    }
...
function SortableColumnHeader({ column, label }: { column: string; label: string }): JSX.Element {
    ...
    return (
        <span
            ...
            data-attr={`accounts-insightsql-sort-${column}`}
        >
            {label}
...
const SKELETON_COLUMNS: TableColumns<{ key: number }> = [
    {
        title: 'Account',
        width: COLUMN_WIDTHS.name,
        render: () => (
...
function AccountsInsightsQLSkeleton(): JSX.Element {
    return (
        <Table
            className="DataTable"
...
export function AccountsInsightsQLTable(): JSX.Element {
    const { insightsqlQuery, accountsQuerySource, sortedRowsTransformer } = useValues(accountsLogic)
    ...
    if ((responseLoading || !accountsQuerySource) && !response) {
        return <AccountsInsightsQLSkeleton />
    }
    return (
        <div className="@container">
            <DataTable
                uniqueKey="customer-analytics-accounts-insightsql"
                ...
                context={{
                    ...
                    dataNodeLogicKey: ACCOUNTS_INSIGHTSQL_DATA_NODE_KEY,
