...
- **Suggestion:** Rebase this migration onto the current leaf, using the next available migration number and dependency, then update `products/review_hog/backend/migrations/max_migration.txt`. Run `python manage.py rebase_migration review` or regenerate the migration and verify `makemigrations --check` reports no conflict.
...
- **Found:** the reviewer's Django claim is accurate (a raising `robust=False` hook stops later hooks), but the only trigger is the deferred import of `products.review_hog.backend.temporal.client`/`.types` actually raising. Those modules are imported on the first inbox review and cached in `sys.modules` thereafter, so the import is a no-op lookup; a real ImportError is a systemic deploy misconfiguration that breaks every Review inbox review anyway, not a transient per-save condition.
