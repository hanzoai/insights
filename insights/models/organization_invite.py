...
from insights.helpers.email_utils import EmailNormalizer, EmailValidationHelper
from insights.models.activity_logging.model_activity import ModelActivityMixin
from insights.models.onboarding_delegation import mark_delegators_accepted
...
def validate_private_project_access(value):
    from insights.rbac.user_access_control import ACCESS_CONTROL_LEVELS_MEMBER
...
class OrganizationInvite(ModelActivityMixin, UUIDTModel):
    ...
    def use(self, user: "User", *, prevalidated: bool = False) -> None:
        ...
        # path in `insights/api/organization_invite.py:delegate` locks `Organization` then
        ...
        if is_email_available(with_absolute_urls=True):
            from insights.tasks.email import send_member_join
...
def _unsuppress_delegator_onboarding_on_invite_delete(sender, instance: OrganizationInvite, **kwargs) -> None:
    ...
    from insights.models.user import OnboardingSkippedReason, User
