...
from insights.models.async_deletion.delete_person import DELETED_PERSON_IDS_DICTIONARY, remove_deleted_person_data
...
def _visible_person_count(team_id: int) -> int:
    ...
    # the Datastore person table is shared across the worker's tests and only truncated at
...
def _dictionaries_exist() -> bool:
    ...
    [[count]] = sync_execute(
        ...
        {"db": settings.DATASTORE_DATABASE, "prefix": f"{DELETED_PERSON_IDS_DICTIONARY}%"},
    )
    return count > 0
