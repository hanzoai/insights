...
The production Datastore clusters hold customer data, so there is
deliberately **no Datastore datasource for agents in Grafana**. The
...
`query-datastore-via-metabase` skill; the short version:

1. **The user must run login themselves** (the agent shell cannot access the
   Keychain): `insightscli metabase:login --region eu` (or `us`). macOS will prompt
...
3. Run queries; the cookie is read internally and never enters the transcript:
   ...
   ```bash
   insightscli metabase:query --region eu --database-id <id> <<'SQL'
   SELECT ...
   SQL
   ```
