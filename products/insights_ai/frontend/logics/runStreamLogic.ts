...
import {
    ...
    type InsightsPermissionRequestParams,
    type InsightsProgressParams,
    type InsightsUsageUpdateParams,
...
interface LiveStreamRegistryHost {
    __insightsAiLiveStreamControllers?: Set<LiveStreamEntry>
}
...
if (process.env.NODE_ENV === 'development') {
    ...
    registryHost.__insightsAiLiveStreamControllers?.forEach((entry) => entry.controller.abort())
    registryHost.__insightsAiLiveStreamControllers = liveStreamControllers
...
export interface RunStreamLogicProps {
    /**
     * Stable logic key. Insights AI passes the conversation id; a generic task viewer (no conversation)
     ...
     */
    streamKey: string
    /** Optional telemetry tag — present for Insights AI conversations, absent for a generic task viewer. */
...
function streamResumeKey(runId: string): string {
    return `insights-ai:stream-resume:${runId}`
}
...
/**
 * Recovers the raw text the user typed from a persisted `_insights/user_message`. The send paths
 * prepend context blocks when attachments are present — `<insights_trusted_context>` and/or
 * `<insights_untrusted_context>` from the frontend builder (`utils/insightsContextBlock.ts`), or the
 * legacy `<insights_context>` wrapper from the deprecated backend `context_wrapper.py` path and old
 ...
 */
const CONTEXT_BLOCK_TAGS = ['insights_trusted_context', 'insights_untrusted_context', 'insights_context']
...
/**
 * Pull rendered text out of a `_insights/user_message` frame's `content`. The seeder writes a plain
...
/** Latest-wins fold of an `_insights/usage_update` ext-notification onto the context-usage snapshot. */
export function foldUsageNotification(existing: ContextUsage | null, params: InsightsUsageUpdateParams): ContextUsage {
 ...
 * `data.type === 'permission_request'` SSE envelope or the `_insights/permission_request`
...
export function parsePermissionRequestFrame(
    frame: PermissionRequestFrame | InsightsPermissionRequestParams
    ...
    const rawServerName = String(toolCall.serverName ?? 'insights')
    const rawToolName = String(toolCall.toolName ?? '')
    const input = (toolCall.rawInput ?? toolCall.input ?? {}) as Record<string, unknown>

    // Canonical ACP tool name (e.g. `mcp__insights__exec`, or a built-in like `Bash`). The wire puts
...
function invocationFromToolCall(update: Record<string, unknown>): ToolInvocation | null {
    ...
    return {
        toolCallId,
        rawServerName: String(update.serverName ?? 'insights'),
...
export function foldLogToThread(entries: StoredEntry[], options: { isResumeRun: boolean }): FoldedThread {
    ...
    // Texts already rendered by a `_insights/user_message`, so a later identical `user_message_chunk`
    ...
    // `showDebugLogs`, like `_insights/console` rows). Deduped per turn — a send can echo in two wire
    ...
    for (const { entry, source } of entries) {
        ...
        if (method === '_insights/turn_complete') {
            items.push({ id: `turn-${separatorSeq++}`, type: 'turn_separator' })
            continue
        }
        if (method === '_insights/progress') {
        ...
        if (method === '_insights/compact_boundary') {
            items = items.filter((item) => !isPendingCompactingStatus(item))
            items.push({
        ...
        if (method === '_insights/task_notification') {
            items.push({
                id: `task-${taskSeq++}`,
        ...
        if (method === '_insights/user_message') {
        ...
        if (method?.startsWith('_insights/')) {
 ...
 *
 * Keyed by `streamKey` (the conversation id for Insights AI, the task id for a generic task viewer)
...
export const runStreamLogic = kea<runStreamLogicType>([
    ...
    path((key) => ['products', 'insights_ai', 'frontend', 'logics', 'runStreamLogic', key]),
    connect(() => ({
        values: [
            projectLogic,
            ['currentProjectId'],
    ...
    actions({
         ...
         * (`toolPolicy`): auto-approve built-in tools + non-destructive Insights exec, prompt
         ...
         * `_insights/permission_resolved` log entry. Clears the matching card and pins the id so
        ...
        /** Diagnostic `_insights/sdk_session` plumbing — no UI. */
    ...
    reducers({
        ...
        turnComplete: [
            ...
            {
                markTurnComplete: () => true,
                // A run emits `_insights/run_started` once; a follow-up message on the same run starts
    ...
    selectors({
         ...
         * run may never emit `_insights/turn_complete`), or the stream errors out.
         *
         ...
         * the first `_insights/run_started` frame, which `runStarted` alone misses.
         */
        isThinking: [
         ...
         * `ThreadView` shows a fixed "spinning up" indicator here until a real `_insights/progress`
    ...
    listeners(({ values, actions, cache, props }) => ({
        bootstrapRun: async ({ taskId, runId, justCreatedRun }, breakpoint) => {
            ...
            // `sseOpened`/`_insights/run_started`.
        ...
        sseOpened: () => {
            ...
            // It clears when the agent actually starts (`_insights/run_started`) or on reset.
        ...
        routePermissionRequest: ({ record, replayedFromHistory }) => {
            ...
            // Otherwise: persist/publish tools (dashboards, feature flags, surveys, script functions,
        ...
        autoApprovePermissionRequest: async ({ record, optionId }) => {
            ...
            insights.capture('permission_auto_approved', {
        ...
        ingestPermissionRequest: ({ record, replayedFromHistory }) => {
            ...
            const activeRun = cache.activeRun as { taskId: string; runId: string } | undefined
            const resolvedToolCall = resolveToolCall(record.rawToolCall)
            insights.capture('permission_requested', {
                conversation_id: props.conversationId,
                trace_id: values.traceId,
                request_id: record.requestId,
                tool_call_name: resolvedToolCall.resolvedKey,
        ...
        cancelRun: async ({ run }) => {
            // Cancel a run through the generic tasks relay — the same command Insights Desktop issues. The
        ...
        handleTerminalStatus: ({ status, errorMessage, replayedFromHistory }) => {
            ...
            insights.capture('task_run_terminated', {
                conversation_id: props.conversationId,
                trace_id: values.traceId,
                run_id: activeRun?.runId,
                task_id: activeRun?.taskId,
        ...
        handleStreamError: ({ errorTitle, retryable }) => {
            ...
            const activeRun = cache.activeRun as { taskId: string; runId: string } | undefined
            insights.capture('sandbox_stream_disconnected', {
                conversation_id: props.conversationId,
                trace_id: values.traceId,
                run_id: activeRun?.runId,
                task_id: activeRun?.taskId,
        ...
        ingestAcpFrame: ({ entry, source }) => {
            ...
            // Custom `_insights/*` notification namespace emitted by the agent-server. Thread items
            ...
            if (method === '_insights/run_started') {
                // TASK_RUN_STARTED telemetry — emit once per run on the first `_insights/run_started`
                ...
                if (!values.runStarted && !isReplay) {
                    const activeRun = cache.activeRun as { taskId: string; runId: string } | undefined
                    cache.turnStartedAtMs = Date.now()
                    insights.capture('task_run_started', {
                        conversation_id: props.conversationId,
                        trace_id: values.traceId,
                        run_id: activeRun?.runId,
                        task_id: activeRun?.taskId,
            ...
            if (method === '_insights/progress') {
                const progress = (notification.params ?? {}) as InsightsProgressParams
            ...
            if (isInsightsNotification(notification, '_insights/permission_request')) {
            ...
            if (isInsightsNotification(notification, '_insights/permission_resolved')) {
            ...
            // The agent reports, per turn, which Insights products an answer was grounded in.
            if (isInsightsNotification(notification, '_insights/resources_used')) {
            ...
            if (isInsightsNotification(notification, '_insights/usage_update')) {
            ...
            if (isInsightsNotification(notification, '_insights/sdk_session')) {
            ...
            // sent keys directly.
            if (isInsightsNotification(notification, '_insights/user_message')) {
            ...
            if (method?.startsWith('_insights/')) {
                // _insights/error, _insights/status, _insights/compact_boundary, _insights/task_notification
                // → rendered by the projection. _insights/console, _insights/sandbox_output,
                // _insights/git_checkpoint, … → no UI. No side effect either way.
                return
            }
            ...
            // effect is the same context-line recording as `_insights/user_message` above — resume
            ...
            if (update.sessionUpdate === 'tool_call_update') {
                ...
                if (
                    ...
                    const startedAt = cache.turnStartedAtMs as number | undefined
                    insights.capture('tool_call_completed', {
                        conversation_id: props.conversationId,
                        trace_id: values.traceId,
