...
description: "Diagnoses bias, anomalies, and strange-looking results on a specific Insights experiment. Covers empty / 0-exposure experiments, sample ratio mismatch, identity fragmentation, multi-variant exposure, uneven-split exclusion bias, significance traps (peeking, A/A, Bayesian vs Frequentist), Insights-vs-SQL discrepancies, and surprises after mid-run edits. Symptom-driven dispatch to the right diagnostic.\nTRIGGER when: user asks 'is my experiment biased?' or 'why 0 exposures?', references the bias banner, says a variant looks strange / wrong / off, sees significance flipping, notices Insights numbers disagreeing with their SQL, sees an A/A test showing significance, or reports surprises after mid-run edits.\nDO NOT TRIGGER when: creating a new experiment (use creating-experiments), only configuring rollout (use configuring-experiment-rollout) or metrics (use configuring-experiment-analytics), or only asking lifecycle questions (use managing-experiment-lifecycle)."
---

# Diagnosing experiment results

This skill answers: **My Insights experiment results look wrong, biased, or empty — what's going on?**
...
strongly it's verified — `[HIGH]` is verified directly in Insights code, `[MEDIUM]` is partially or
...
### D — Numbers don't match (Insights vs the user's SQL / raw count)
