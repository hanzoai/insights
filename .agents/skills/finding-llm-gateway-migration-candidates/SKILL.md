...
- deployment, sandbox, workflow, and secret wiring in other Insights repositories
...
Python's unbilled flag is not a blocker when an internal workload should debit a Insights-owned team wallet for spend attribution. Verify that the Go credential resolves to that team. Treat billing as blocked only when migration would charge a customer incorrectly, lose required customer budget policy, or violate a requirement to debit no wallet.
...
5. Can use a customer wallet intentionally or a Insights-owned wallet for internal spend.
