...
# ruff: noqa: T201
"""
Verify that the multinode Datastore smoke-test stack ended up in the expected
shape after `manage.py migrate_datastore` ran.

Connects to each Datastore node directly (via its published port) and asserts:
...
# Match the published ports in docker-compose.multinode-datastore.yml.
NODES: list[Node] = [
    Node("datastore-data", "localhost", 9000, "data"),
    Node("datastore-ai-events", "localhost", 9100, "ai_events"),
    Node("datastore-aux", "localhost", 9200, "aux"),
    Node("datastore-ops", "localhost", 9300, "ops"),
    Node("datastore-sessions", "localhost", 9400, "sessions"),
    Node("datastore-logs", "localhost", 9500, "logs"),
]
...
EXPECTED_CLUSTERS: dict[str, list[str]] = {
    "insights": ["datastore-data"],
    "insights_single_shard": ["datastore-data"],
    "insights_writable": ["datastore-data"],
    "insights_primary_replica": ["datastore-data"],
    "insights_migrations": [
        "datastore-ai-events",
        "datastore-aux",
        "datastore-data",
...
# Manifest of tables (in DATASTORE_DATABASE) per satellite role. Update when
