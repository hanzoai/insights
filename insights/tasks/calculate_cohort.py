...
def get_stuck_static_cohort_candidates_queryset() -> QuerySet:
    ...
    return (
        ...
        .filter(
            ...
            # (InsightsQL query or filter criteria). Excludes CSV-upload cohorts
...
@shared_task(
    ...
    # Auto-retry for transient Datastore errors with exponential backoff
...
def calculate_cohort_ch(cohort_id: int, pending_version: int, initiating_user_id: Optional[int] = None) -> None:
    with hanzo_insights.new_context():
        hanzo_insights.tag("feature", Feature.COHORT.value)
        hanzo_insights.tag("cohort_id", cohort_id)
        ...
        hanzo_insights.tag("team_id", cohort.team.id)
...
@shared_task(
    bind=True,
    ignore_result=True,
    # Auto-retry transient Datastore capacity errors with exponential backoff, matching the
...
def calculate_cohort_from_list(
    ...
    # insert path dedupes members already in the cohort (Datastore excludes existing UUIDs, the
...
def insert_cohort_from_query(cohort_id: int, team_id: Optional[int] = None) -> None:
    """
    One-time population task for static cohorts created from a InsightsQL query
    (e.g. duplicating a dynamic cohort as static).

    Inserts actors into Datastore person_static_cohort, then syncs to Postgres.
...
COHORT_BACKFILL_TRIGGER_TASK_COUNTER = Counter(
    "insights_cohort_backfill_trigger_task_total",
...
# Datastore-touching tasks: the person creator runs a sizing scan capped at 30 s.
@shared_task(
    ignore_result=True,
    acks_late=True,
