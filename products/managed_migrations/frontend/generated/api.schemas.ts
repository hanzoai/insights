...
export interface BatchImportSupportListApi {
    ...
    /**
     * Customer-facing status message shown in the Insights UI. Embedded URLs are redacted the same way as status_message.
     * @nullable
     */
...
export interface BatchImportAWSIAMSetupApi {
    /** Whether IAM role authentication is available on this Insights deployment. */
    available: boolean
    /** External ID to pin in the role trust policy's sts:ExternalId condition. Stable per project. */
    external_id: string
    /** ARN of Insights's import role -- the principal your role must trust. */
    insights_role_arn: string
