from insights.test.base import APIBaseTest
from unittest.mock import MagicMock, patch

from django.test import SimpleTestCase

from rest_framework import status

from insights.api.sdk_health import get_team_data


class TestGetTeamData(SimpleTestCase):
    @patch("insights.api.sdk_health.get_client")
    def test_reads_v2_team_cache(self, mock_get_client: MagicMock) -> None:
        redis_client = MagicMock()
        redis_client.get.return_value = b'{"insights-java": []}'
        mock_get_client.return_value = redis_client

        result = get_team_data(7, force_refresh=False)

        assert result == {"insights-java": []}
        redis_client.get.assert_called_once_with("sdk_versions:team:v2:7")

    @patch("products.growth.backend.team_sdk_versions.get_and_cache_team_sdk_versions")
    @patch("insights.api.sdk_health.get_client")
    def test_v2_cache_miss_fetches_fresh_data(self, mock_get_client: MagicMock, mock_get_and_cache: MagicMock) -> None:
        redis_client = MagicMock()
        redis_client.get.return_value = None
        mock_get_client.return_value = redis_client
        mock_get_and_cache.return_value = {"web": []}

        result = get_team_data(7, force_refresh=False)

        assert result == {"web": []}
        redis_client.get.assert_called_once_with("sdk_versions:team:v2:7")
        mock_get_and_cache.assert_called_once_with(7, redis_client)


class TestSdkHealthViewSet(APIBaseTest):
    """Tests for the /api/projects/{team_id}/sdk_health/report/ MCP-accessible endpoint."""

    def _url(self) -> str:
        return f"/api/projects/{self.team.pk}/sdk_health/report/"

    def test_requires_authentication(self) -> None:
        self.client.logout()
        response = self.client.get(self._url())
        assert response.status_code in (status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN)

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_cold_cache_returns_empty_healthy_report(self, mock_github, mock_team) -> None:
        # Both caches cold → endpoint must still respond 200 with a healthy (but empty) report
        # rather than a 500 — agents should see "nothing to report" not a service error.
        mock_team.return_value = None
        mock_github.return_value = {}

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["overall_health"] == "healthy"
        assert data["health"] == "success"
        assert data["needs_updating_count"] == 0
        assert data["team_sdk_count"] == 0
        assert data["sdks"] == []

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_happy_path_returns_digested_report(self, mock_github, mock_team) -> None:
        mock_team.return_value = {
            "insights-node": [
                {
                    "lib_version": "1.0.0",
                    "count": 100,
                    "max_timestamp": "2026-04-21T00:00:00Z",
                }
            ]
        }
        mock_github.return_value = {
            "insights-node": {
                "latestVersion": "2.0.0",
                "releaseDates": {"1.0.0": "2025-10-01T00:00:00Z"},
            }
        }

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["team_sdk_count"] == 1
        assert data["needs_updating_count"] == 1
        assert data["overall_health"] == "needs_attention"
        sdk = data["sdks"][0]
        assert sdk["lib"] == "insights-node"
        assert sdk["readable_name"] == "Node.js"
        assert sdk["is_outdated"] is True
        # UI-parity fields must round-trip through the serializer
        release = sdk["releases"][0]
        assert release["status_reason"].startswith("Released ")
        assert "insights-node" in release["sql_query"]
        assert release["activity_page_url"].startswith(f"/project/{self.team.pk}/")

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_legacy_java_without_version_returns_migration_assessment(self, mock_github, mock_team) -> None:
        mock_team.return_value = {
            "insights-java": [
                {
                    "lib_version": None,
                    "count": 42,
                    "max_timestamp": "2026-07-09T00:00:00Z",
                }
            ]
        }
        mock_github.return_value = {
            "insights-java": {
                "latestVersion": "1.2.0",
                "releaseDates": {},
            }
        }

        response = self.client.get(self._url())

        assert response.status_code == status.HTTP_200_OK
        sdk = response.json()["sdks"][0]
        assert sdk["lib"] == "insights-java"
        assert sdk["readable_name"] == "Java (legacy)"
        assert sdk["latest_version"] == "1.2.0"
        assert sdk["migration_required"] is True
        assert sdk["releases"][0]["version"] == "Not reported"
        assert "com.insights.java:insights" in sdk["reason"]
        assert "com.insights:insights-server" in sdk["reason"]

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_force_refresh_param_threaded_to_fetcher(self, mock_github, mock_team) -> None:
        mock_team.return_value = None
        mock_github.return_value = {}
        self.client.get(self._url() + "?force_refresh=true")
        assert mock_team.called
        # Accept either positional or keyword form so this test survives a future
        # refactor from get_team_data(team_id, force_refresh) to ...(team_id, force_refresh=...)
        args, kwargs = mock_team.call_args
        force_refresh = kwargs.get("force_refresh", args[1] if len(args) > 1 else None)
        assert force_refresh is True

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_partial_cache_missing_sdk_in_github_data_is_skipped(self, mock_github, mock_team) -> None:
        # team_data has two SDKs, but github cache only knows about one.
        # The defensive `.get(lib, {})` in SdkHealthViewSet.report is what makes this safe —
        # compute_sdk_health skips SDKs with no latest_version rather than KeyError.
        # If someone ever swaps `.get(lib, {})` back to `sdk_data[lib]` (like the legacy
        # flat endpoint does), this test will catch it.
        mock_team.return_value = {
            "insights-node": [{"lib_version": "1.0.0", "count": 100, "max_timestamp": "2026-04-21T00:00:00Z"}],
            "insights-python": [{"lib_version": "7.0.0", "count": 50, "max_timestamp": "2026-04-21T00:00:00Z"}],
        }
        mock_github.return_value = {
            "insights-node": {
                "latestVersion": "2.0.0",
                "releaseDates": {"1.0.0": "2025-10-01T00:00:00Z"},
            }
            # insights-python deliberately absent
        }

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        # Only the SDK with matching github data should be reported
        assert data["team_sdk_count"] == 1
        sdk_libs = [s["lib"] for s in data["sdks"]]
        assert sdk_libs == ["insights-node"]
        assert "insights-python" not in sdk_libs


class TestSdkHealthLegacyEndpoint(APIBaseTest):
    """Tests for the legacy /api/sdk_health/ endpoint consumed by the onboarding scene."""

    def _url(self) -> str:
        return "/api/sdk_health/"

    @patch("insights.api.sdk_health.get_team_data")
    def test_cold_cache_returns_empty_200(self, mock_team) -> None:
        mock_team.return_value = {}

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {}

    @patch("insights.api.sdk_health.get_team_data")
    def test_genuine_failure_returns_500(self, mock_team) -> None:
        mock_team.return_value = None

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.json() == {"error": "Failed to get SDK versions. Please try again later."}

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_missing_github_data_returns_500(self, mock_github, mock_team) -> None:
        mock_team.return_value = {
            "insights-node": [{"lib_version": "1.0.0", "count": 100, "max_timestamp": "2026-04-21T00:00:00Z"}]
        }
        mock_github.return_value = {}

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.json() == {"error": "Failed to get GitHub SDK data. Please try again later."}

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_partial_github_cache_skips_unknown_sdk(self, mock_github, mock_team) -> None:
        mock_team.return_value = {
            "insights-node": [{"lib_version": "1.0.0", "count": 100, "max_timestamp": "2026-04-21T00:00:00Z"}],
            "insights-java": [{"lib_version": None, "count": 50, "max_timestamp": "2026-04-21T00:00:00Z"}],
        }
        mock_github.return_value = {
            "insights-node": {
                "latestVersion": "2.0.0",
                "releaseDates": {"1.0.0": "2025-10-01T00:00:00Z"},
            }
        }

        response = self.client.get(self._url())

        assert response.status_code == status.HTTP_200_OK
        assert list(response.json()) == ["insights-node"]

    @patch("insights.api.sdk_health.get_team_data")
    @patch("insights.api.sdk_health.get_github_sdk_data")
    def test_happy_path_returns_combined_data(self, mock_github, mock_team) -> None:
        mock_team.return_value = {
            "insights-node": [
                {"lib_version": "1.0.0", "count": 100, "max_timestamp": "2026-04-21T00:00:00Z"},
            ]
        }
        mock_github.return_value = {
            "insights-node": {
                "latestVersion": "2.0.0",
                "releaseDates": {"1.0.0": "2025-10-01T00:00:00Z"},
            }
        }

        response = self.client.get(self._url())
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["insights-node"]["latest_version"] == "2.0.0"
        usage = data["insights-node"]["usage"]
        assert len(usage) == 1
        assert usage[0]["lib_version"] == "1.0.0"
        assert usage[0]["is_latest"] is False
        assert usage[0]["release_date"] == "2025-10-01T00:00:00Z"
