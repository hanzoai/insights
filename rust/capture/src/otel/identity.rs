...
const SPAN_DISTINCT_ID_KEYS: &[&str] = &[
    ...
    "ai.settings.context.insights_distinct_id",
...
/// override is set.
const RESOURCE_DISTINCT_ID_KEYS: &[&str] = &["insights.distinct_id", "user.id"];
...
mod tests {
    ...
    fn test_span_metadata_distinct_id_wins_over_resource() {
        ...
        let resource = resource_with(vec![string_kv("insights.distinct_id", "resource-user")]);
        assert_eq!(
    ...
    #[test]
    fn test_span_insights_distinct_id_wins_over_user_id() {
        let span_attrs = vec![
            string_kv("user.id", "user-id-value"),
            string_kv("insights.distinct_id", "insights-id-value"),
        ];
        assert_eq!(
    ...
    #[test]
    fn test_ai_telemetry_metadata_wins_over_insights_distinct_id() {
        let span_attrs = vec![
            string_kv("insights.distinct_id", "explicit"),
            string_kv("ai.telemetry.metadata.insights_distinct_id", "metadata"),
        ];
        assert_eq!(
            extract_distinct_id_for_span(&span_attrs, None, "fallback"),
            "metadata"
    ...
    #[test]
    fn test_resource_user_id_used_when_no_insights_id() {
    ...
    #[test]
    fn test_resource_insights_distinct_id_wins_over_user_id() {
    ...
    fn test_empty_span_value_falls_through_to_resource() {
        let span_attrs = vec![string_kv("insights.distinct_id", "")];
        let resource = resource_with(vec![string_kv("insights.distinct_id", "resource-user")]);
        assert_eq!(
            extract_distinct_id_for_span(&span_attrs, Some(&resource), "fallback"),
            "resource-user"
    ...
    fn test_empty_resource_value_falls_through_to_fallback() {
        let resource = resource_with(vec![
            string_kv("insights.distinct_id", ""),
            string_kv("user.id", ""),
        ]);
        assert_eq!(
