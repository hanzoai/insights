...
export type FlowTeamInfo = Pick<Flow, 'id' | 'team_id' | 'version'>

export class FlowManagerService {
    private lazyLoader: LazyLoader<Flow>
    private lazyLoaderByTeam: LazyLoader<FlowTeamInfo[]>
    ...
    constructor(
        ...
        this.pubSub.on<{ teamId: Team['id']; flowIds: Flow['id'][] }>('reload-script-flows', (message) => {
            const { teamId, flowIds } = message
            logger.debug('⚡', '[PubSub] Reloading script flows!', { teamId, flowIds })
            this.onFlowsReloaded(teamId, flowIds)
        })
    }

    public async getFlowsForTeams(teamIds: Team['id'][]): Promise<Record<Team['id'], Flow[]>> {
        const result = teamIds.reduce<Record<Team['id'], Flow[]>>((acc, teamId) => {
            acc[teamId] = []
            return acc
        }, {})

        const teamItemIds = await this.getFlowIdsForTeams(teamIds)
    ...
    public async getFlowIdsForTeams(teamIds: Team['id'][]): Promise<Record<Team['id'], string[]>> {
    ...
    public async getFlowsForTeam(teamId: Team['id']): Promise<Flow[]> {
        return (await this.getFlowsForTeams([teamId]))[teamId] ?? []
    }

    public async getFlow(id: Flow['id']): Promise<Flow | null> {
        return (await this.lazyLoader.get(id)) ?? null
    }

    public async getFlows(ids: Flow['id'][]): Promise<Record<Flow['id'], Flow | null>> {
        return await this.lazyLoader.getMany(ids)
    }

    private onFlowsReloaded(teamId: Team['id'], flowIds: Flow['id'][]): void {
        this.lazyLoaderByTeam.markForRefresh(teamId.toString())
        this.lazyLoader.markForRefresh(flowIds)
    }

    private async fetchTeamFlows(teamIds: string[]): Promise<Record<string, FlowTeamInfo[]>> {
        ...
        const byTeam: Record<string, FlowTeamInfo[]> = {}
    ...
    private async fetchFlows(ids: string[]): Promise<Record<string, Flow | undefined>> {
        logger.debug('[FlowManager]', 'Fetching script flows', { ids })
        ...
        return items.reduce<Record<string, Flow | undefined>>((acc, item) => {
    ...
    private mergeEncryptedInputs(item: Flow): void {
        ...
        if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
            ...
            try {
                ...
                logger.warn('[FlowManager]', 'Could not decrypt encrypted inputs - flow will run without them', {
        ...
        for (const action of item.actions ?? []) {
            ...
            // the same way. The source-webhook consumer builds its function from `flow.trigger`
