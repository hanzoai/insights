...
describe('signAwsRequest', () => {
    ...
    // Regression for the pre-refactor Script-side signer. Before signing moved out of
    // the Kinesis Script template, `test_template_aws_kinesis.py` asserted the exact
    ...
    it('matches the in-Script Kinesis signer for the test_template_aws_kinesis fixture', () => {
        const headers = signAwsRequest({
            method: 'POST',
...
describe('resolveAwsSigV4Credentials', () => {
    ...
    it('reports both missing inputs in the error message', () => {
        const result = resolveAwsSigV4Credentials(sigv4Refs, insightsFunctionWith())
    ...
    it('reports only the missing input when only one is absent', () => {
        const result = resolveAwsSigV4Credentials(
            sigv4Refs,
            insightsFunctionWith({ aws_access_key_id: { value: 'AKIDEXAMPLE' } })
        )
        expect(result.ok).toBe(false)
        if (!result.ok) {
            expect(result.error).toContain('aws_secret_access_key')
