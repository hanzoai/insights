...
use crate::{
    ...
    vm::{ScriptVM, MAX_JSON_SERDE_DEPTH},
    ExportedFunction,
};
...
pub type NativeFunction =
    Arc<dyn Fn(&ScriptVM, Vec<HogValue>) -> Result<HogValue, VmError> + Send + Sync>;

// The native and script STL registries are process-constant: build them once and hand out cheap clones
    ...
    [
        ...
        (
            ...
            native_func(err_to_null(|vm, args| {
                // Script: extractRegex(haystack, pattern)
        ...
        (
            ...
            native_func(err_to_null(|vm, args| {
                ...
                let json = match val {
                    ...
                    // Otherwise just convert the script to a json object
        ...
        // Operator-alias functions: the function forms of Script operators. Note these are the *function*
...
// we recurse
// `toString` semantics (reference `STLToString`): a top-level Script date renders as `YYYY-MM-DD` and a
// top-level Script datetime as an ISO 8601 string; every other value (including nested temporals inside
...
// Render a Script datetime as Luxon's `DateTime.fromSeconds(dt, {zone}).toISO()` does: millisecond
...
// Whole days from the Unix epoch to a Script date (reference: `day.diff(epoch, 'days').days` floored).
...
/// `toDateTime(input[, zone])` → a Script DateTime object `{ __hogDateTime__: true, dt, zone }`.
///
/// To match Datastore (the parity oracle), this VM orders Script temporals by `dt` seconds
/// ([`crate::values::compare_values`]) — the reference Python/TS ScriptVMs cannot order them, so their
...
fn to_datetime(vm: &ScriptVM, args: Vec<HogValue>) -> Result<HogValue, VmError> {
    if args.is_empty() || args.len() > 2 {
        return Err(VmError::NativeCallFailed(
...
/// `toDate(input)` → a Script Date object `{ __hogDate__: true, year, month, day }` in UTC.
fn to_date(vm: &ScriptVM, args: Vec<HogValue>) -> Result<HogValue, VmError> {
...
fn naive_to_seconds(naive: NaiveDateTime, zone: Option<&str>) -> Result<f64, VmError> {
    ...
    match tz.from_local_datetime(&naive) {
        // DST fold: Datastore resolves to a single instant; take the earlier one.
...
/// The Node ScriptVM is the ingestion shadow baseline, so preserving microseconds (as Python's
...
// Shared by the JSON-extract family: parse args[0] (a JSON string, or convert a Script value) and
...
fn json_path_value(vm: &ScriptVM, args: &[HogValue]) -> Result<Option<JsonValue>, VmError> {
...
fn trim_impl(vm: &ScriptVM, args: Vec<HogValue>, side: TrimSide) -> Result<HogValue, VmError> {
    if args.is_empty() || args.len() > 2 {
        return Err(VmError::NativeCallFailed(
...
// Epoch seconds of a Script Date/DateTime value (the `dt` field, or UTC midnight for a Date).
...
fn temporal_seconds(vm: &ScriptVM, value: &HogValue, name: &str) -> Result<f64, VmError> {
    value
        .deref(&vm.heap)?
...
// (epoch seconds, zone string) of a Script DateTime; zone defaults to UTC (Script Dates carry no zone).
fn hog_datetime_parts(vm: &ScriptVM, value: &HogValue, name: &str) -> Result<(f64, String), VmError> {
...
fn date_trunc_impl(vm: &ScriptVM, unit: &str, value: &HogValue) -> Result<HogValue, VmError> {
...
fn make_hog_interval(vm: &ScriptVM, args: &[HogValue], unit: &str) -> Result<HogValue, VmError> {
...
fn format_datetime_impl(vm: &ScriptVM, args: &[HogValue]) -> Result<HogValue, VmError> {
    ...
    let translated = translate_datastore_format(&format);
...
// Translate the reference's Datastore-style format tokens (% + token) to chrono strftime codes.
fn translate_datastore_format(format: &str) -> String {
...
// Port of the reference `isIPAddressInRange` (common/scriptvm/typescript/src/stl/ip.ts): is `address`
...
// previous raw digest. Mirrors common/scriptvm/typescript/src/stl/crypto.ts.
fn sha256_hmac_chain(vm: &ScriptVM, arg: &HogValue) -> Result<Vec<u8>, VmError> {
...
fn encoding_arg(vm: &ScriptVM, args: &[HogValue], idx: usize) -> Result<String, VmError> {
    match args.get(idx) {
        Some(v) => match v.deref(&vm.heap)? {
...
fn error_constructor(vm: &ScriptVM, args: &[HogValue], type_str: &str) -> Result<HogValue, VmError> {
    if args.len() > 2 {
        return Err(VmError::NativeCallFailed(format!(
...
// Build the Script error duck-type `{ __hogError__, type, message, payload? }`. Payload is included
...
fn strict_equals(vm: &ScriptVM, a: &HogValue, b: &HogValue) -> Result<bool, VmError> {
...
fn compare_fn(vm: &ScriptVM, args: &[HogValue], op: NumOp, name: &str) -> Result<HogValue, VmError> {
    assert_argc(args, 2, name)?;
...
fn arith_fn(vm: &ScriptVM, args: &[HogValue], op: NumOp, name: &str) -> Result<HogValue, VmError> {
    assert_argc(args, 2, name)?;
...
where
    F: Fn(&ScriptVM, Vec<HogValue>) -> Result<HogValue, VmError> + Send + Sync + 'static,
{
    Arc::new(func)
}
