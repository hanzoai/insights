...
from insights.datastore.kafka_engine import (
    CONSUMER_GROUP_FN_INVOCATION_RESULTS,
...
# DATA (so InsightsQL queries from the main cluster reach the data). Replay paginator
# also queries this. cluster=settings.DATASTORE_AUX_CLUSTER so the Distributed
...
KAFKA_FN_INVOCATION_RESULTS_TABLE_SQL = lambda: (
    f"""
...
ENGINE = {
        kafka_engine(
            ...
            group=CONSUMER_GROUP_FN_INVOCATION_RESULTS,
            named_collection=settings.DATASTORE_KAFKA_WARPSTREAM_CYCLOTRON_NAMED_COLLECTION,
        )
    }
...
AS SELECT
    ...
    _offset,
    _partition
FROM {KAFKA_FN_INVOCATION_RESULTS_TABLE}
"""
)
