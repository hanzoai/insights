...
(`<prefix>/team_<team_id>/<job_id>/...`); nothing is persisted on the Datastore cluster. Every
...
class IdentityMatchingLinkViewSet(TeamAndOrgViewSetMixin, viewsets.ViewSet):
    ...
    def _latest_job_id(self) -> str | None:
        ...
        with tags_context(product=Product.GROWTH, feature=Feature.QUERY):
            result = sync_execute(  # nosemgrep: datastore-injection-taint,datastore-fstring-param-audit — only the constant s3() structure/format and a team_id-derived path are interpolated; team_id is an int from the URL, all values parameterized
    ...
    def list(self, request: Request, **kwargs: Any) -> Response:
        ...
        with tags_context(product=Product.GROWTH, feature=Feature.QUERY):
            count_result = sync_execute(  # nosemgrep: datastore-injection-taint,datastore-fstring-param-audit — s3 path from team_id (int) and validated job_id UUID; WHERE built from literals, values parameterized
            ...
            rows = sync_execute(  # nosemgrep: datastore-injection-taint,datastore-fstring-param-audit — s3 path from team_id (int) and validated job_id UUID; WHERE built from literals, values parameterized
                f"""
                SELECT
                    lk.job_id,
                    lk.model_version,
    ...
    def runs(self, request: Request, **kwargs: Any) -> Response:
        self._assert_storage_configured()
        with tags_context(product=Product.GROWTH, feature=Feature.QUERY):
            links_rows = sync_execute(  # nosemgrep: datastore-injection-taint,datastore-fstring-param-audit — s3 path from team_id (int); values parameterized
                f"""
                SELECT
                    job_id,
            ...
            pairs_rows = sync_execute(  # nosemgrep: datastore-injection-taint,datastore-fstring-param-audit — s3 path from team_id (int); values parameterized
                f"""
