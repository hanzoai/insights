...
Lives in `insights/temporal/ai_observability/` (not `products/signals/`) because
...
`products.signals` cannot depend on `products.ai_observability`.
"""
...
SUMMARIZE_REPORT_SYSTEM_PROMPT = """You are a concise technical writer. Your job is to produce a short signal description from a Insights LLM evaluation REPORT.
...
class EmitEvalReportSignalWorkflow(InsightsWorkflow):
