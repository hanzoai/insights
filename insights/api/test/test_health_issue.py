...
class TestHealthIssueAPI(APIBaseTest):
    ...
    def test_retrieve_enriches_with_rendered_explanation(self):
        issue = self._create_issue(
            kind="sdk_outdated",
            payload={"sdk_name": "insights-python", "latest_version": "3.0.0", "reason": "insights-python is behind"},
        )
        ...
        data = response.json()
        self.assertEqual(data["title"], "insights-python SDK is outdated")
        self.assertEqual(data["summary"], "insights-python is behind")
        ...
        self.assertIn("update or replace the Insights SDK dependency", data["remediation"]["agent"])
    ...
    def test_sdk_issue_visible_even_when_latest_release_is_fresh(self):
        ...
        # SDKs like insights-python and insights-js always have a recent release, so any blanket
        ...
        # as unused setup.
        key = github_sdk_versions_key("insights-python")
        ...
        issue = self._create_issue(
            severity=HealthIssue.Severity.CRITICAL,
            payload={"sdk_name": "insights-python", "current_version": "7.14.1", "latest_version": "7.18.3"},
    ...
    @patch("insights.tasks.health_checks.evaluate_health_check_for_team.delay")
    ...
    @patch("insights.tasks.health_checks.evaluate_health_check_for_team.delay", side_effect=Exception("broker down"))
