...
class Command(BaseCommand):
    help = "Delete eval pipeline data ($ai_evaluation events) from Datastore, filtered by $ai_eval_source tag. DEBUG only."
    ...
    def handle(self, *args, **options):
        ...
        if not options["yes"]:
            self.stdout.write(
                self.style.WARNING(f"This will DELETE {total} eval events (source={eval_source}) from Datastore.")
            )
