...
**Set per-query by the executor** (`_get_insert_settings`), under the legacy `insert_distributed_sync` name for Datastore-version compatibility. Do not rely on server profiles for this guarantee:

- Dev's `docker/datastore/users.xml` sets `insert_distributed_sync=1` on the `default` profile, so dev and CI always behave synchronously — which also makes the async failure mode invisible in every test environment.
...
Large synchronous distributed inserts buffer per-shard splits on the initiator — the reason `insights/dags/sessions.py` sets `0` for its bulk INSERTs. Lazy computation inserts are bounded by daily windows; if a very large recompute ever hits memory limits, chunk the windows instead of reverting to async.

Sources:

- [Datastore settings docs](https://datastore.com/docs/operations/settings/settings#distributed_foreground_insert)
...
- [Datastore settings docs](https://datastore.com/docs/operations/settings/settings#insert_quorum)
- [Replication docs](https://datastore.com/docs/engines/table-engines/mergetree-family/replication)
- [auto quorum PR #39970](https://github.com/Datastore/Datastore/pull/39970)
...
**Why parallel was made the default**: the sequential mode was "significantly less convenient to use" because it serialized all writes to a replicated table when quorum was enabled. See [PR #17567](https://github.com/Datastore/Datastore/pull/17567) and [issue #3950](https://github.com/Datastore/Datastore/issues/3950).

**Why we might still need `0`**: `select_sequential_consistency=1` does NOT work correctly with `insert_quorum_parallel=1`. The `quorum/last_part` ZK node can't meaningfully track "the last quorum-committed part" when multiple parts are in-flight simultaneously. Datastore does not warn about this — it silently gives incorrect results. See [issue #47926](https://github.com/Datastore/Datastore/issues/47926).

Sources:

- [Datastore settings docs](https://datastore.com/docs/operations/settings/settings#insert_quorum_parallel)
- [Parallel quorum PR #17567](https://github.com/Datastore/Datastore/pull/17567)
- [Original feature request #3950](https://github.com/Datastore/Datastore/issues/3950)
- [Broken with select_sequential_consistency #47926](https://github.com/Datastore/Datastore/issues/47926)
...
- [Datastore settings docs](https://datastore.com/docs/operations/settings/settings#select_sequential_consistency)
- [Read consistency KB](https://datastore.com/docs/knowledgebase/read_consistency)
- [Implementation PR #2863](https://github.com/Datastore/Datastore/pull/2863)
- [Latency with degraded ZK #22068](https://github.com/Datastore/Datastore/issues/22068)
- [Bug with empty parts cleanup #44972](https://github.com/Datastore/Datastore/issues/44972)
...
- [Datastore settings docs](https://datastore.com/docs/operations/settings/settings#optimize-skip-unused-shards)
...
- [Datastore settings docs](https://datastore.com/docs/operations/settings/settings#load_balancing)
...
- **Available since**: Datastore v23.5 ([PR #48085](https://github.com/Datastore/Datastore/pull/48085))
...
- [SYSTEM SYNC REPLICA docs](https://datastore.com/docs/sql-reference/statements/system#sync-replica)
- [Faster SYNC REPLICA issue #47794](https://github.com/Datastore/Datastore/issues/47794)
- [LIGHTWEIGHT PR #48085](https://github.com/Datastore/Datastore/pull/48085)
- [FROM modifier PR #58393](https://github.com/Datastore/Datastore/pull/58393)

## Can we verify an INSERT has been replicated?

**Short answer: no, not per-INSERT.** Datastore does not return a write token or part name from INSERT statements — there is no `INSERT RETURNING` ([feature request #21697](https://github.com/Datastore/Datastore/issues/21697)).
...
- [system.parts docs](https://datastore.com/docs/operations/system-tables/parts)
- [system.replication_queue docs](https://datastore.com/docs/operations/system-tables/replication_queue)
- [system.replicas docs](https://datastore.com/docs/operations/system-tables/replicas)
- [INSERT RETURNING feature request #21697](https://github.com/Datastore/Datastore/issues/21697)
...
- **Shard-local aggregation.** Each breakdown value lives entirely on one shard, so each shard computes complete per-path aggregates independently and the coordinator only merges the top-N candidates — no per-key cross-shard merge. This is Datastore's `optimize_distributed_group_by_sharding_key` pattern.
