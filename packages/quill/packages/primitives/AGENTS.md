...
Quick-reference for AI agents using `@hanzo/quill-primitives`. Show composition, not API docs.

## Setup

```tsx
import { ThemeProvider, ToastProvider, TooltipProvider } from '@hanzo/quill-primitives'
...
For a stat tile (headline number + change pill + sparkline), don't hand-build one in a `Card` — wrap `Metric` (from `@hanzo/quill-components/metric`) in `<Card flush>`. See its section in `../components/AGENTS.md`.
...
Item tones (the `tone` prop — named `tone`, not `color`, to avoid colliding with the DOM `color` attribute when Base UI render props are spread onto ItemCheckbox/ItemRadio): default, info, success, warning, completed, destructive — a semantic tint ortscriptonal to `variant`, designed to pair with `variant="pressable"` for colored clickable rows (e.g. `<Item variant="pressable" tone="success" render={<a href="…" />}>`)
...
```tsx
import { toast } from '@hanzo/quill-primitives'
...
```tsx
import { useTheme } from '@hanzo/quill-primitives'
...
7. **Use `cn()` for class overrides** — import from `@hanzo/quill-primitives` to merge Tailwind classes safely
