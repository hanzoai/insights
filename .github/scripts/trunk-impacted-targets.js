...
// downstream". A change to insights/ can break any product, so it reports
...
const TRIPWIRES = [
    ...
    'Dockerfile*',
    'proto/**',
    // schema.json generates insights/schema.py, and both sides are committed.
    'frontend/src/queries/schema.json',
    'insights/schema.py',
    // products.json is loaded at runtime by insights/products.py and generated
...
// being guessed at.
const COMMON_PYTHON = ['insightsql_parser', 'scriptvm', 'ingestion', 'migration_utils', 'plugin_transpiler', 'alerting']
...
//
// insightscli and insightscli-commands are deliberately absent: ci-backend.yml drives the
// suite through insightscli, and insights/conftest.py imports
// insightscli_commands.quarantine.pytest_support on every pytest run.
...
function loadRustGraph(repoRoot) {
    try {
        ...
        // Longest directory first so rust/common/scriptvm resolves to its own crate
...
function computeTargets(changedFiles, context) {
    ...
    for (const file of changedFiles) {
        ...
        // a README under insights/ into the backend lane.
        //
        // The exception is markdown that is a build input: `insightscli build:skills`
        ...
        if (top === 'insights' || (top === 'ee' && segments[1] !== 'frontend')) {
            allPyProducts()
            continue
        }
        ...
        // one exception: docs/onboarding is the @hanzo/docs-onboarding
