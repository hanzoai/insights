...
export function NavTabBrowse(): JSX.Element {
    ...
    function handlePanelTriggerClick(item: PanelLayoutNavIdentifier): void {
        ...
        insights.capture('nav panel trigger clicked', { panel: item, is_open: isOpening })
        if (isOpening) {
    ...
    return (
        ...
        >
            <Collapsible
                ...
                onOpenChange={() => {
                    insights.capture('nav section toggled', {
                        section: 'project',
                ...
                <Collapsible.Panel className={cn('pl-2 pt-1', isLayoutNavCollapsed && 'items-center pl-0')}>
                    {isSidebarItemShown('home') && (
                        <NavLink
                            ...
                            onClick={() => insights.capture('nav item clicked', { item: 'home' })}
                            sideAction={{
                                onClick: () =>
                    ...
                    {isProductAutonomyEnabled && isSidebarItemShown('inbox') && (
                        <NavLink
                            ...
                            tag="beta"
                            onClick={() => insights.capture('nav item clicked', { item: 'inbox' })}
                        />
                    )}
                    ...
                    <NavLink
                        ...
                        onClick={() => insights.capture('nav item clicked', { item: 'activity' })}
                    />
                                    ...
                                    <Fragment key={item.identifier}>
                                        ...
                                        {item.identifier === 'Products' && showToolsSearchRow && (
                                            <ButtonPrimitive
                                                ...
                                                onClick={() => {
                                                    insights.capture('nav search clicked')
                        ...
                        {expandedNavSections.tools &&
                            (uiCustomizationEnabled ? (
                                <Link
                                    ...
                                    onClick={() => insights.capture('nav tools customize clicked')}
                                    buttonProps={{
                                        iconOnly: true,
                                        size: 'xs',
                                        className:
                                ...
                                <ButtonPrimitive
                                    ...
                                    onClick={() => {
                                        if (isEditMode) {
                                            insights.capture('nav tools edit saved')
                                            saveAndExitEditMode()
                                        } else {
                                            insights.capture('nav tools edit toggled', { is_editing: true })
                                            enterEditMode()
                                        }
                                    }}
