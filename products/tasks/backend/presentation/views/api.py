...
from insights.schema import QuerySchemaRoot
...
from insights.rate_limit import CodeInviteThrottle, TaskRunChartRenderThrottle
...
class TaskViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @validated_request(
        ...
        responses={
            ...
            403: OpenApiResponse(description="Endpoint is gated to Insights-internal debugging."),
        ...
        description=(
            "Insights-internal debug tool. Resolves a Slack permalink to the linked task, its runs, "
    ...
    def slack_thread_context(self, request, **kwargs):
        if not _is_internal_debug_team(self.team_id):
            return Response(
                {"detail": "slack-thread-context is restricted to Insights-internal debugging."},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    @validated_request(
        ...
        responses={
            ...
            404: OpenApiResponse(description="Task not found"),
            429: OpenApiResponse(
                response=TaskRunErrorResponseSerializer, description="Team is over its insights_code usage limit"
            ),
        },
        summary="Run task",
    ...
    def run(self, request, pk=None, **kwargs):
        ...
        # (`product-autonomy`), not the Insights Code (`tasks`) product, so they skip the Code
...
class TaskRunLivingArtifactViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @validated_request(
        ...
        description=(
            "Renders a Insights insight (ad-hoc query JSON or a saved insight) to a PNG server-side and registers "
    ...
    def chart(self, request, *args, **kwargs):
        ...
        chart_metadata: dict = {}
        if url:
            chart_metadata["insights_url"] = url
        artifact, error = tasks_facade.create_task_run_living_artifact(
            run_id,
            task_id,
            self.team_id,
...
class CodeInviteViewSet(viewsets.ViewSet):
    """API for redeeming Insights Desktop invite codes."""
    ...
    @validated_request(
        ...
        description="Redeem a Insights Desktop invite code to enable access.",
    )
    ...
    @extend_schema(
        ...
        description="Check whether the authenticated user has access to Insights Desktop and to Loops.",
    )
