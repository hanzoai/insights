...
describe('onboardingLogic — flow composition', () => {
    ...
    describe('install-step dedup — POSTFN_JS', () => {
        ...
        it('merges dropped setup task ids that differ from the survivor (SR.SetupSessionRecordings, ET.EnableErrorTracking)', () => {
            ...
            // their own task ids, which may include `IngestFirstEvent` from sibling insights-js
        ...
        it('all 7 insights-js products yield exactly one install step', () => {
            logic.actions.setProductKey(ProductKey.PRODUCT_ANALYTICS)
            logic.actions.setSecondaryProductKeys([
                ProductKey.WEB_ANALYTICS,
                ProductKey.SESSION_REPLAY,
    ...
    describe('install-step dedup — OPENTELEMETRY', () => {
        it('Logs install uses the OTEL dedup key (separate group from insights-js)', () => {
    ...
    describe('step ordering (bucket sort)', () => {
        it('puts the insights-js install first (bucket 0) for a multi-step PA primary flow', () => {
            logic.actions.setProductKey(ProductKey.PRODUCT_ANALYTICS)
        ...
        it('non-insights-js install steps land in bucket 1 (after insights-js install, before non-installs)', () => {
            logic.actions.setProductKey(ProductKey.PRODUCT_ANALYTICS)
            logic.actions.setSecondaryProductKeys([ProductKey.LOGS])
        ...
        it('puts the primary product install first, even when a insights-js install is secondary', () => {
            ...
            // (which has no insights-js dedup key) must render before the product analytics
