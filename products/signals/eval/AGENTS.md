...
Infrastructure is mocked — an in-memory embedding store (`mock.py`) replaces Datastore + Kafka, and a `ReportStore` replaces Postgres.
...
```bash
# Full run — captures eval results to Insights
...
| `POSTFN_PROJECT_API_KEY`      | Capturing eval results (skip with `--no-capture`)                                                          |
| `POSTFN_HOST`                 | Insights instance (defaults to `http://localhost:8010`)                                                     |
...
### Eval metrics captured to Insights
...
To delete captured eval events from Datastore (DEBUG mode only):
...
Run these in the Insights SQL editor.
