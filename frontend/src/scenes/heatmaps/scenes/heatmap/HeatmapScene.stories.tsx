...
const meta: Meta = {
    ...
    decorators: [
        mswDecorator({
            get: {
                '/v1/environments/:team_id/saved/hm_gen/': generatingSaved,
                '/v1/environments/:team_id/heatmap_screenshots/:id/content/': (_req, res, ctx) =>
...
export const IframeExample: Story = {
    ...
    decorators: [
        mswDecorator({
            get: {
                '/v1/environments/:team_id/saved/hm_iframe/': (_req, res, ctx) =>
                    res(ctx.status(200), ctx.json(makeIframeSaved())),
                '/v1/heatmap/': (_req, res, ctx) =>
                    res(
                        ctx.status(200),
                        ctx.json({
                            results: [
