...
def _viewer_has_insights_code_access(viewer: User | None) -> bool:
    """Fail closed: missing creator or any flag-service error suppresses the link.

    The Insights Desktop app is rolled out via cohort + invite redemption; surfacing
...
def post_slack_update(input: PostSlackUpdateInput) -> None:
    ...
    try:
        ...
        creator_has_access = _viewer_has_insights_code_access(task_run.task.created_by)
        task_url: str | None = (
