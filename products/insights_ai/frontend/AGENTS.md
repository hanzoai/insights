# Insights AI agent-run surface — frontend guide

This directory is the **composable Insights AI agent-run UI**: a conversation-agnostic library of surfaces
...
`products/insights_ai/frontend`. Max is a _consumer_ of this surface, not a dependency of it.
...
- **The Insights product data-tool renderers live in this surface and self-register — via the generic seam.**
  `api/tools` exposes `toolRegistry` and the convenience wrapper **`registerToolRenderers(entries)`**: the
  generic per-product mechanism for plugging in cards that display Insights entities. The data-tool widgets
...
- If Max needs something the surface doesn't express generically, **lift it to a generic prop/selector here
  ...
  `grep -rE "scenes/max|maxThreadLogic|MaxUIContext" products/insights_ai/frontend` must be empty of imports.
...
built-ins + read-only Insights `exec`, else surfaces a card) → answer (`respondToPermission`) → resolve (pins
...
`wrapWithInsightsContext` (`utils/insightsContextBlock.ts`) — a prefix of `<insights_trusted_context>`
(`type: 'instructions'` items, our own injected guidance) and/or `<insights_untrusted_context>` (everything
...
leading block on history replay (including the legacy `<insights_context>` wrapper still emitted by the
...
`toolResolver` (inner Insights MCP tool, e.g. `create_dashboard`). Subscribe with `useToolStreamListener({
...
Insights tools the resolved name may be `__insights_exec_unknown__` at `started` (the `command` streams in via
...
components/         # RunSurfaceImpl (the RunSurface compound, heavy chunk); ReadonlyRunSurfaceImpl (prepackaged
  ...
  tool/             #   tool registry + renderers (built-ins, generic MCP, EditDiffRenderer, diff/exec utils)
    widgets/        #     Insights product data-tool widgets (insight/dashboard/recordings/error-tracking/notebook/query) + registerDataToolRenderers
...
utils/              # thinkingMessages, insightsContextBlock (trusted/untrusted context-block builder)
