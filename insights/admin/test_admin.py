...
from insights.admin import _OAUTH_ADMIN_MODEL_NAMES, install_admin_app_list_overrides, register_all_admin
from insights.admin.admins.event_ingestion_restriction_config import EventIngestionRestrictionConfigAdmin
from insights.admin.admins.global_rate_limit_threshold_config import GlobalRateLimitThresholdConfigAdmin
from insights.admin.admins.personal_api_key_admin import PersonalAPIKeyAdmin
from insights.admin.admins.user_admin import UserAdmin, UserChangeForm
from insights.admin.inlines.organization_member_inline import OrganizationMemberForUserInline, OrganizationMemberInline
...
from insights.models.global_rate_limit_threshold_config import GlobalRateLimitThresholdConfig
...
class TestOAuthSidebarRegrouping(BaseTest):
    ...
    def test_oauth_models_moved_to_oauth_section(self):
        ...
        insights_apps = [app for app in result if app["app_label"] == "insights"]
        assert len(insights_apps) == 1
        insights_object_names = {model["object_name"] for model in insights_apps[0]["models"]}
        assert insights_object_names == {"User"}
...
class TestUserAdmin(BaseTest):
    ...
    def search_user_ids(self, search_term: str) -> list[int]:
        request = self.request_factory.get("/admin/insights/user/", {"q": search_term})
...
class TestProductAdminRegistration:
    ...
    # central `insights/admin/admins/` registry. They register only because
