...
from insights.temporal.data_modeling.activities.enrich_view_semantics import EnrichViewSemanticsInputs
from insights.temporal.data_modeling.metrics import (
    get_datastore_materialization_duration_metric,
...
from insights.temporal.data_modeling.workflows.enrich_view_semantics import EnrichViewSemanticsWorkflow
...
class MaterializeViewWorkflow(InsightsWorkflow):
    ...
    async def run(self, inputs: MaterializeViewWorkflowInputs) -> MaterializeViewWorkflowResult:
        ...
        # fallback to duckgres job if no datastore job was run
        if job_id is None:
    ...
    async def _collect_shadow_comparison(
        ...
        datastore_row_count: int,
        datastore_duration_seconds: float,
        inputs: MaterializeViewWorkflowInputs,
    ) -> None:
        ...
        try:
            ...
            row_count_matched = datastore_row_count == shadow_result.row_count
            status = "completed" if shadow_result.error is None else "failed"

            # prometheus metrics
            get_duckgres_shadow_finished_metric(status).add(1)
            get_datastore_materialization_duration_metric().record(datastore_duration_seconds)
            ...
            temporalio.workflow.logger.info(
                ...
                extra={
                    "datastore_rows": datastore_row_count,
                    "datastore_duration_seconds": round(datastore_duration_seconds, 2),
