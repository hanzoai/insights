...
description: >
  Author a Insights canvas with semantic HTML, CSS, and direct browser APIs — documents, articles,
...
The import allowlist still applies (react, react-dom, @hanzo/quill, recharts, lucide-react,
...
- The host toggles a `.dark` class on the document root when the user's Insights theme changes.
...
- No `fetch()`/`XMLHttpRequest`, no `<script>` tags, no dynamic `import()`, no remote assets —
  the sandbox blocks them. Insights data comes only through the `ph` bridge (see the
  `querying-canvas-data` skill), including `ph.capture` for interaction analytics.
- External links go through `ph.openExternal(url)` (hanzo.ai origins only), from a user
  interaction.
