...
function captureWizardStepCompleted(
    ...
    insights.capture('in-app heatmap creation wizard step completed', {
        step,
...
export const heatmapCreationLogic = kea<heatmapCreationLogicType>([
        ...
        pageDataCheck: [
            null as HeatmapDataCheckResult | null,
            {
                checkPageData: async (
                    ...
                    const query =
                        matchType === 'pattern'
                            ? insightsql`SELECT count() FROM heatmaps WHERE match(current_url, ${heatmapUrlPatternToRegex(
                                  url
                              )}) AND timestamp >= now() - INTERVAL 30 DAY`
                            : insightsql`SELECT count() FROM heatmaps WHERE trimRight(current_url, '/') = trimRight(${url}, '/') AND timestamp >= now() - INTERVAL 30 DAY`
                    let response: Awaited<ReturnType<typeof api.queryInsightsQL>> | null = null
                    try {
                        response = await api.queryInsightsQL(query, { scene: 'Heatmaps', productKey: 'heatmaps' })
                    } catch {
    ...
    listeners(({ actions, values }) => {
        ...
        return {
            ...
            checkPagePreflightSuccess: () => {
                ...
                insights.capture('in-app heatmap creation framing checked', {
            ...
            checkPageDataSuccess: ({ pageDataCheck }) => {
                ...
                insights.capture('in-app heatmap creation data checked', {
            ...
            markRecordingHandoff: ({ matchingRecordingCount }) => {
                insights.capture('in-app heatmap creation recording handoff', {
    ...
    afterMount(({ actions, values, cache }) => {
        ...
        insights.capture('in-app heatmap creation wizard started', {
            capture_enabled: values.captureEnabled,
        })
        if (!values.pageStepBlockReason && values.effectiveDataUrl) {
            actions.requestPageDataCheck('automatic')
    ...
    beforeUnmount(({ values, cache }) => {
        if (cache.wizardStarted && !values.terminalOutcome) {
            insights.capture('in-app heatmap creation wizard abandoned', {
