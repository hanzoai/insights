# insightsql_parser_rs

Hand-rolled Rust InsightsQL parser. Pratt + recursive descent. Same JSON AST
shape as the [C++ ANTLR parser](../../../common/insightsql_parser/) so the
...
[`insights/insightsql/parser.py`](../../../insights/insightsql/parser.py).

About **15× faster than the C++ parser on `parse_expr`** and **50–55×
on `parse_select`**, against the same input, on the same machine. The
numbers come from `insights/insightsql/scripts/parser_bench.py`; re-run
...
[`insights/insightsql/grammar/InsightsQLLexer.*.g4`](../../../insights/insightsql/grammar/InsightsQLParser.g4)

+ `InsightsQLParser.g4` via ANTLR4. The Rust parser does not consume those
...
| Path                                            | What it does                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`src/lib.rs`](src/lib.rs)                      | PyO3 entry points (`parse_expr_json`, `parse_select_json`, `parse_program_json`, `parse_order_expr_json`, `parse_full_template_string_json`). Each returns a JSON string; on error the JSON is an `{"error": true, ...}` envelope `insights/insightsql/json_ast.py` decodes into `InsightsQLSyntaxError` / `ExposedInsightsQLError`.                           |
| [`src/lex.rs`](src/lex.rs)                      | Lexer. Hand-rolled state machine matching the ANTLR-generated C++ lexer's tokens + mode stack (default / template-string / InsightsQLX-tag / InsightsQLX-text). When you add a new keyword to the grammar, add it here too.                                                                                                                             |
...
| `src/parse/{expr,select,program,join,cte,insightsqlx,template}.rs` | Per-rule parsing. Most grammar changes land in one of these.                                                                                                                                                                                                                                                                                  |
...
# rust source change.
uv pip install -e rust/insightsql/parser

# Or via maturin directly (faster incremental):
maturin develop --release --manifest-path rust/insightsql/parser/Cargo.toml

# Sanity check
python -c "import insightsql_parser_rs; print(insightsql_parser_rs.parse_expr_json('1 + 2'))"
```
...
[`.github/workflows/build-insightsql-parser-rs.yml`](../../../.github/workflows/build-insightsql-parser-rs.yml).
...
that the pytest PBT
([`insights/insightsql/test/test_parser_pbt.py`](../../../insights/insightsql/test/test_parser_pbt.py))
...
`hasattr(insightsql_parser_rs, "cov_snapshot")` and silently skips the third
steering label.
...
one for `insightsql_parser_rs`. That wrapper lives at
[`scripts/cov-rustc-wrapper.py`](scripts/cov-rustc-wrapper.py):

```bash
WRAPPER=$PWD/rust/insightsql/parser/scripts/cov-rustc-wrapper.py
...
maturin build --release --manifest-path rust/insightsql/parser/Cargo.toml \
...
VIRTUAL_ENV=$PWD/.flox/cache/venv uv pip install --reinstall --no-deps \
    /tmp/cov-wheel/insightsql_parser_rs-*.whl

# Verify the instrumented wheel is loaded.
python -c "import insightsql_parser_rs; print(hasattr(insightsql_parser_rs, 'cov_snapshot'))"

# Run the parity PBT; the rust_edges target appears in --hypothesis-show-statistics.
RUN_PBT=1 insightscli test insights/insightsql/test/test_parser_pbt.py::TestParserBackendEquivalence \
...
package name (e.g. `insightsql_parser_rs_cov`) so production deploys can't
accidentally pull the instrumented wheel.

## Publishing

The crate is pinned via the `insightsql-parser-rs==X.Y.Z` line in the
...
[`common/insightsql_parser`](../../../common/insightsql_parser) (the C++ parser
...
1. **Update [`InsightsQLLexer.*.g4`](../../../insights/insightsql/grammar/) and
    [`InsightsQLParser.g4`](../../../insights/insightsql/grammar/InsightsQLParser.g4).**
    ...
    instructions in
    [`insights/insightsql/grammar/README.md`](../../../insights/insightsql/grammar/README.md).
    The script rewrites `common/insightsql_parser/InsightsQL{Lexer,Parser}.{cpp,h,interp,tokens}`.
...
2. **Pick the AST emission.** Decide what JSON the cpp visitor should
    ...
    a new one in `insights/insightsql/ast.py`. The Python AST is shared
    between backends, so any new node has to land there first, otherwise
    `insights/insightsql/json_ast.py::deserialize_ast` will crash on it.

3. **Update the cpp visitor.** Add the `VISIT(YourNewRule)` arm in
    [`common/insightsql_parser/parser_json.cpp`](../../../common/insightsql_parser/parser_json.cpp).
...
4. **Pin the new behaviour with a regression test.** Add a test (and a
    ...
    [`insights/insightsql/test/_test_parser.py`](../../../insights/insightsql/test/_test_parser.py).
...
and a fresh `pip install ./common/insightsql_parser` are non-negotiable —
the diagnostics test what's installed, not what's on disk.

```bash
maturin develop --release --manifest-path rust/insightsql/parser/Cargo.toml
pip install --force-reinstall --no-deps ./common/insightsql_parser
```
...
1. **Generate a new divergence.** In priority order:
    ...
    + real production queries via `log_corpus_diagnostic.py` /
      `hog_corpus_diagnostic.py` (the script corpus has been at 100% for
...
### Regression tests in `insights/insightsql/test/`

```bash
insightscli test insights/insightsql/test/test_parser_cpp_json.py
insightscli test insights/insightsql/test/test_parser_rust_json.py
insightscli test insights/insightsql/test/test_parser_rust_py.py
```

The behaviour suite + regression pins live in
[`_test_parser.py`](../../../insights/insightsql/test/_test_parser.py)'s
...
### Property-based testing via `insights/insightsql/scripts/pbt_diagnostic.py`

```bash
PYTHONPATH=. python insights/insightsql/scripts/pbt_diagnostic.py \
    --n 5000 --rule program

# Per rule:
--rule expr     # standalone column expressions
--rule select   # SELECT / SELECT-set statements
--rule program  # full Script programs (declarations + statements + exprs)
```
...
minimal example via shrinkray (needs the optional `insightsql-parser-parity`
group — `uv sync --group insightsql-parser-parity`).
...
PYTHONPATH=. python insights/insightsql/scripts/log_corpus_diagnostic.py

# Script programs from production (transformations, destinations, …):
PYTHONPATH=. python insights/insightsql/scripts/hog_corpus_diagnostic.py
```

Both auto-download via `insightscli metabase:query` and cache locally under
`insights/insightsql/scripts/.local/`. Pass `--skip-download` to reuse the
existing dump while iterating. Failures are written one block per
divergence to a `.sql` / `.script` file the agent can chew through. Add
...
echo '<query>' | PYTHONPATH=. python insights/insightsql/scripts/shrink_query.py --rule select
```
...
which lives in the `insightsql-parser-parity` group rather than the default dev
install (its transitive deps — textual, libcst, black — are heavy). The
parity scripts in `insights/insightsql/scripts/` import it at module level, so
...
### Perf bench via `insights/insightsql/scripts/parser_bench.py`

```bash
CANDIDATE_BACKEND=rust-json PYTHONPATH=. \
    python insights/insightsql/scripts/parser_bench.py
```
...
tagged `insightsql_parser_position_only_mismatch` so position-only
...
parse_expr(src, backend=InsightsQLParserBackend.CPP_WITH_RUST_SHADOW)
```
...
[PR description](https://github.com/Insights/insights/pull/58949) has
...
```python
from insights.insightsql.parser import parse_expr, parse_select, parse_program

ast = parse_expr("1 + event.properties.$browser", backend="rust-json")
```

Backends live in `insights/insightsql/constants.InsightsQLParserBackend`:
...
| `rust-py`                 | This crate, returning `insights.insightsql.ast` dataclasses directly via PyO3 (no JSON round-trip). Production primary. |
