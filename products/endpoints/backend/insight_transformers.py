"""Transform materialized insight endpoint responses from flat InsightsQL to rich insight format.

When insight queries (TrendsQuery, RetentionQuery, LifecycleQuery) are materialized,
the S3 table stores flat InsightsQL data. At read time, SELECT * returns flat rows.
...
from insights.schema import InsightsQLQueryModifiers, InsightsQLQueryResponse
...
def transform_materialized_insight_response(
    ...
    """Transform flat InsightsQL results in-place into insight-specific response shape.
...
def _make_runner(original_query: dict, team: Team, now: datetime | None = None) -> "InsightRunner":
    ...
    modifiers = InsightsQLQueryModifiers(**modifiers_dict)
    runner = cast(
        "InsightRunner",
        get_query_runner(
...
def _strip_insightsql_fields(result: dict) -> None:
    """Remove InsightsQL-specific fields that don't belong in insight responses."""
...
def _transform_trends(result: dict, original_query: dict, team: Team, now: datetime | None = None) -> None:
    ...
    if not rows:
        result["results"] = []
        _strip_insightsql_fields(result)
        return
    ...
    per_series_responses: list[InsightsQLQueryResponse] = [
        InsightsQLQueryResponse(results=groups.get(i, []), columns=columns) for i in range(expected_series_count)
    ]
...
def _transform_lifecycle(result: dict, original_query: dict, team: Team, now: datetime | None = None) -> None:
    ...
    response = InsightsQLQueryResponse(results=rows, columns=columns)
