...
# below. It transitively imports from insights/dags which calls django.setup() — that
# causes a RuntimeError("populate() isn't reentrant") if we let it happen at module
# import time from insights/api/__init__.py. SDK_TYPES used to trigger the same chain
...
class SdkReleaseAssessmentSerializer(serializers.Serializer):
    ...
    activity_page_url = serializers.CharField(
        help_text=(
            ...
            "Combine with the user's Insights host (e.g. us.hanzo.ai) for a clickable link."
        ),
    )
...
class SdkAssessmentSerializer(serializers.Serializer):
    lib = serializers.ChoiceField(
        choices=SDK_TYPES,
        help_text="SDK identifier, e.g. 'web', 'insights-python', 'insights-node', 'insights-ios'.",
    )
...
class SdkHealthReportSerializer(serializers.Serializer):
    ...
    team_sdk_count = serializers.IntegerField(
        help_text="Number of distinct Insights SDKs the project is actively using."
    )
...
class SdkHealthViewSet(TeamAndOrgViewSetMixin, viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        description=(
            "Returns a pre-digested health assessment of the Insights SDKs the project is using. "
        ...
        parameters=[
            OpenApiParameter(
                ...
                required=False,
                description=(
                    "When true, bypasses the Redis cache and re-queries Datastore for SDK usage. "
...
def sdk_health(request: Request) -> Response:
    ...
    combined_data = {}
    for lib, entries in team_data.items():
        sdk_data_for_lib = sdk_data.get(lib)
        if not sdk_data_for_lib:
            continue
        combined_data[lib] = {
...
def get_team_data(team_id: int, force_refresh: bool) -> dict[str, list[SdkVersionEntry]] | None:
    ...
    try:
        ...
        if sdk_versions is not None:
            ...
            logger.error("sdk_health_team_datastore_empty", team_id=team_id)
