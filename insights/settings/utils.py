...
def assert_debug_not_in_production(*, debug: bool, cloud_deployment: Optional[str], test: bool) -> None:
    ...
    DEBUG relaxes authentication (e.g. the dev OAuth login bypass in insights/views.py) and exposes
    debugging surfaces, so it must never run on Insights Cloud US/EU or the hosted dev/staging env.
    TEST is excluded because the suite runs with DEBUG=1 (see insights/settings/overrides.py). E2E is
