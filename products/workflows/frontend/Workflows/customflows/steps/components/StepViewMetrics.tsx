...
import { CustomFlowEditorActionMetrics, customFlowEditorLogic } from '../../customFlowEditorLogic'
import { CustomFlowAction } from '../../types'

export function StepViewMetrics({ action }: { action: CustomFlowAction }): JSX.Element {
    const { actionMetricsById, actionMetricsByIdLoading } = useValues(customFlowEditorLogic)

    const metrics: CustomFlowEditorActionMetrics = actionMetricsById?.[action.id] ?? {
        actionId: action.id,
        succeeded: 0,
        failed: 0,
        filtered: 0,
