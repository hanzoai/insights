...
from insights.api import proxy_record_diagnostics as diagnostics
from insights.temporal.proxy_service.cloudflare import (
    CloudflareAPIError,
    CustomHostnameInfo,
    CustomHostnameSSL,
...
class TestCheckCloudflare(TestCase):
    @patch("insights.api.proxy_record_diagnostics.get_custom_hostname_by_domain")
    ...
    def test_fail_when_api_errors(self, get_mock):
        ...
        with patch("insights.api.proxy_record_diagnostics.capture_exception"):
            result, info = diagnostics._check_cloudflare(_record())
        self.assertEqual(result.status, "failed")
    ...
    def test_warn_when_unexpected_exception_escapes(self, _name, exc, get_mock):
        ...
        with patch("insights.api.proxy_record_diagnostics.capture_exception") as cap_mock:
            result, info = diagnostics._check_cloudflare(_record())
        self.assertEqual(result.status, "warned")
        self.assertIsNone(info)
        self.assertIsNone(result.remediation)
...
class TestCheckHttpChallenge(TestCase):
    @patch("insights.api.proxy_record_diagnostics.requests.get")
...
class TestCheckLiveEvent(TestCase):
    ...
    @patch("insights.api.proxy_record_diagnostics.requests.post")
...
class TestCheckCertExpiry(TestCase):
    ...
    @patch("insights.api.proxy_record_diagnostics.ssl_module.create_default_context")
    @patch("insights.api.proxy_record_diagnostics.socket.create_connection")
...
class TestDiagnoseOrchestrator(TestCase):
    ...
    @patch("insights.api.proxy_record_diagnostics._check_cert_expiry")
    @patch("insights.api.proxy_record_diagnostics.requests.post")
    @patch("insights.api.proxy_record_diagnostics.get_custom_hostname_by_domain")
    @patch("insights.api.proxy_record_diagnostics.dns.resolver.Resolver")
