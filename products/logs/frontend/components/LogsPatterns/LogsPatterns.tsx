...
import { Banner, Button, Table, Tag, Tooltip } from '@hanzo/elements'
...
function renderLevelCell(pattern: _LogPatternApi): JSX.Element {
    ...
    return canonical ? <LogTag level={canonical} /> : <Tag>{severity}</Tag>
}
...
function ChangeBadge({ entry }: { entry: _LogPatternDiffEntryApi }): JSX.Element {
    if (entry.classification === 'new') {
        return <Tag type="success">New</Tag>
    }
    if (entry.classification === 'gone') {
        return <Tag type="muted">Gone</Tag>
    }
...
export function LogsPatterns({ id }: { id: string }): JSX.Element {
    ...
    const columns: TableColumns<_LogPatternApi> = [
        {
            title: 'Level',
            key: 'severity',
            width: 0,
        ...
        },
        {
            ...
            render: (_, row) =>
                row.estimated_error_count > 0 ? (
                    <Tag type="danger">{renderEstimate(row.estimated_error_count, row.error_count)}</Tag>
                ) : (
    ...
    const compareColumns: TableColumns<_LogPatternDiffEntryApi> = [
        {
            title: 'Change',
            key: 'change',
            width: 0,
    ...
    return (
        <div className="flex-1 min-h-0 overflow-auto" data-attr="logs-patterns">
            {!compareEnabled && sampled && !patternsResponseLoading && !patternsError && (
                <Banner type="info" className="m-2" data-attr="logs-patterns-sample-info">
            ...
            {compareEnabled &&
                ...
                diffResponse.baseline.total_count === 0 && (
                    <Banner type="warning" className="m-2" data-attr="logs-patterns-empty-baseline">
