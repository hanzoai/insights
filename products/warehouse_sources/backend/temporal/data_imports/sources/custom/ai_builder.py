...
# Gateway product tag — registered in `insights/llm/gateway_client.py` and
...
DOCS_FETCH_USER_AGENT = "Insights-CustomSourceBuilder/1.0"
...
def build_system_prompt(reference_text: str) -> str:
    ...
    return "\n".join(
        [
            "You are an expert at connecting REST APIs to Insights's data warehouse. Given an API's "
            "documentation, you author a single RESTAPIConfig manifest (JSON) that Insights's generic "
