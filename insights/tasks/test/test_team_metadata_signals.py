...
from insights.tasks.team_metadata import update_related_teams_metadata_cache_task
...
class TestTeamMetadataInvalidationSignals(BaseTest):
    def test_organization_rename_enqueues_fanout(self) -> None:
        with patch("insights.tasks.team_metadata.update_related_teams_metadata_cache_task") as mock_task:
    ...
    def test_enqueue_failure_records_counter_and_does_not_propagate(self) -> None:
        with patch("insights.tasks.team_metadata.update_related_teams_metadata_cache_task") as mock_task:
            ...
            with patch("insights.tasks.team_metadata.HYPERCACHE_SIGNAL_UPDATE_COUNTER") as mock_counter:
                try:
                    with self.captureOnCommitCallbacks(execute=True):
                        self.organization.name = "Renamed org"
                        self.organization.save(update_fields=["name"])
...
class TestRelatedTeamsMetadataFanoutTask(BaseTest):
    def test_organization_fanout_enqueues_every_team(self) -> None:
        ...
        with patch("insights.tasks.team_metadata.update_team_metadata_cache_task") as mock_update:
    ...
    def test_logs_and_reraises_on_failure(self) -> None:
        with (
            patch("insights.tasks.team_metadata.update_team_metadata_cache_task") as mock_update,
            patch("insights.tasks.team_metadata.logger") as mock_logger,
...
class TestOrgProjectDeleteCascadesToTeamClear(BaseTest):
    def test_deleting_project_clears_team_metadata_via_cascade(self) -> None:
        ...
        with patch(
            "insights.tasks.team_metadata.clear_team_metadata_cache",
