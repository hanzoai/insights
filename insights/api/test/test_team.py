...
from insights.test.base import APIBaseTest, QueryMatchingTest, snapshot_postgres_queries_context
...
from insights.api.test.batch_exports.conftest import start_test_worker
    ...
    class TestTeamAPI(APIBaseTest, QueryMatchingTest):
        ...
        def test_update_test_filter_default_checked(self):
            response = self.client.patch(
                "/v1/environments/@current/", {"test_account_filters_default_checked": "true"}
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        def test_delete_bulky_postgres_data(self):
            ...
            from insights.models.cohort import Cohort, CohortPeople
            from insights.models.feature_flag.feature_flag import FeatureFlag, FeatureFlagHashKeyOverride

            # from insights.models.insight_caching_state import InsightCachingState
        ...
        def test_invalid_session_recording_sample_rates(
            ...
            response = self.client.patch(
                "/v1/environments/@current/", {"session_recording_sample_rate": provided_value}
            )
            assert response.status_code == status.HTTP_400_BAD_REQUEST
        ...
        def test_invalid_session_recording_network_payload_capture_config(
            ...
            response = self.client.patch(
                "/v1/environments/@current/", {"session_recording_network_payload_capture_config": provided_value}
            )
            assert response.status_code == status.HTTP_400_BAD_REQUEST
            assert response.json() == {
        ...
        @patch("insights.api.team.calculate_product_activation.delay", MagicMock())
