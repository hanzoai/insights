...
`@validated_request` from `insights/api/mixins.py` combines request validation,
...
Use `TypedRequest[T]` from `insights/api/mixins.py` to tell the type checker
...
@extend_schema(
    ...
    summary="Test Script evaluation code",
    description="Test Script evaluation code against sample events without saving.",
)
@action(detail=False, methods=["post"], url_path="test_hog")
def test_hog(self, request, **kwargs):
    ...
...
These are fixed via `ENUM_NAME_OVERRIDES` in `insights/settings/web.py`.
