...
import yaml
import click
from insightscli.manifest import REPO_ROOT, get_manifest
...
NODE_ARTIFACT_PATTERNS = (
    ...
    "frontend/@hanzo/apps-common/dist",
    "common/storybook/dist",
    "common/tailwind/dist",
    "common/plugin_transpiler/dist",
    "common/scriptvm/typescript/.parcel-cache",
    "common/scriptvm/typescript/dist",
    "nodejs/dist",
    "storybook-static",
    "playwright-report",
...
# ---------------------------------------------------------------------------
# doctor:zombies — find and kill orphaned Insights dev processes
# ---------------------------------------------------------------------------

# Executable basenames that should never be reported as Insights dev processes.
...
@dataclass
class DevProcess:
    """A detected Insights dev process."""
...
@click.command(
    name="doctor:zombies",
    help="Find and kill orphaned Insights dev processes",
)
...
def doctor_zombies(dry_run: bool, yes: bool, include_all: bool) -> None:
    """Find and kill orphaned Insights dev processes left behind after an unclean shutdown."""

    def _record() -> None:
        if not dry_run:
            hints.record_check_run("doctor:zombies")

    click.echo("Scanning for orphaned Insights dev processes...\n")

    processes = _scan_insights_processes(REPO_ROOT)

    if not processes:
        click.echo("No Insights dev processes found. Nothing to clean up.")
        _record()
        return
...
def _scan_insights_processes(repo_root: Path) -> list[DevProcess]:
    """Find all processes related to the Insights repo."""
...
def _matches_repo_path(args: str, repo_str: str, repo_prefix: str) -> bool:
    """Check if the command line references the exact repo path (not a prefix like hanzo.ai)."""
...
def _has_known_executable(args: str) -> bool:
    """Check if the command starts with a known Insights dev executable."""
...
_PREFLIGHT_PORTS: tuple[tuple[int, str], ...] = (
    ...
    (2181, "zookeeper"),
    (8123, "datastore-http"),
    (8443, "datastore-https"),
    (9000, "datastore-native"),
    (9440, "datastore-native-tls"),
    (9009, "datastore-interserver"),
    (9092, "kafka"),
    (5432, "postgres"),
    (6379, "redis"),
    (6399, "redis-cluster"),
...
def _compose_project_name() -> str:
    ...
    return os.environ.get("COMPOSE_PROJECT_NAME") or "insights"
...
def _scan_unheld_via_lsof(unheld: Sequence[tuple[int, str]]) -> list[PortHolder]:
    ...
    bounds (`doctor:zombies` already handles orphaned Insights processes).
    """
...
def _insights_shaped_projects(candidates: set[str]) -> set[str]:
    """Of the candidate foreign projects, keep only ones that look like a
    Insights dev stack: a datastore container in any state (a partial stack
    may hold ports while its datastore is stopped). An unrelated project
    that happens to hold postgres/redis ports gets reported, not torn down.
    """
    datastore_projects = {project for _container_id, project in _containers_for_service("datastore")}
    return candidates & datastore_projects
...
def _confirm_stack_teardown(stack: str, timeout: float = 30.0) -> bool:
    ...
    Runs on every `insightscli start`, so a hung or piped stdin must never block;
...
@click.option(
    "--yes", "-y", is_flag=True, help="Auto-confirm teardown of any foreign Insights stack found holding a port"
)
def doctor_ports(yes: bool) -> None:
    """Report what's holding the dev stack's host ports, and in a TTY, offer
    to tear down a stale foreign Insights stack. Fail-open: never blocks
    ...
    foreign_projects = _insights_shaped_projects(foreign_candidates)
...
# ---------------------------------------------------------------------------
# doctor:migrate-volumes — auto-salvage anonymous Datastore/ZooKeeper volumes
# ---------------------------------------------------------------------------

# (compose service, mount destination inside the container, named-volume
# suffix, human label). Must match the volumes: blocks for datastore/
# zookeeper in docker-compose.dev.yml.
_VOLUME_MIGRATIONS: tuple[tuple[str, str, str, str], ...] = (
    ("datastore", "/var/lib/datastore", "datastore-data", "Datastore data"),
...
def _has_prior_install(project_name: str) -> bool:
    ...
    who stopped the stack before updating has no datastore container left to
...
def _plan_volume_migration(project_name: str) -> list[VolumeMigrationStep] | None:
    ...
    Datastore and ZooKeeper state must move together — a replicated table
...
def _copy_volume(source_volume: str, dest_volume: str) -> bool:
    ...
    """
    container = f"insightscli-migrate-volumes-{os.getpid()}"
    ok = _run_ok(
        [
            "docker",
            "run",
...
# Datastore needs time to flush a multi-GB dataset on shutdown; docker's 10s
...
def _execute_volume_migration(plan: Sequence[VolumeMigrationStep], project_name: str) -> bool:
    ...
    one. A live `cp -a` against an actively-written Datastore/ZooKeeper data
...
def _print_volume_reset_warning() -> None:
    click.echo("⚠️  Switching Datastore/ZooKeeper to named docker volumes. This first start")
    click.echo("   recreates them empty, so local Datastore data resets once (happens once")
    click.echo("   per machine). Run 'insightscli migrations:run' after, or 'insightscli dev:reset' for a")
    click.echo("   full wipe plus demo data. See 'Local Datastore suddenly empty' in")
...
@click.command(
    ...
    help="Auto-migrate Datastore/ZooKeeper data to the new named docker volumes",
)
def doctor_migrate_volumes() -> None:
    """One-time migration for the named-volume switch in docker-compose.dev.yml.

    Datastore and ZooKeeper moved from anonymous to named volumes so that
    ...
    lock_path = _MIGRATION_LOCK_DIR / f"insightscli-migrate-volumes-{project_name}.lock"
    try:
...
def _do_migrate_volumes(project_name: str) -> None:
    ...
    if not _matching_containers(project_name, "datastore"):
        # No datastore container left to salvage from — either a fresh clone, or a
    ...
    if plan is not None and _execute_volume_migration(plan, project_name):
        click.echo("✅ Migrated Datastore/ZooKeeper data to the new named docker volumes automatically.")
        click.echo("   Datastore and ZooKeeper are stopped; run 'insightscli start' to bring them back up.")
        return
            ...
            for cache_dir in repo_root.glob(f"{depth}/{pattern}"):
                ...
                if exceeded:
                    return CheckResult(
                        ...
                        remediation="run `insightscli doctor:disk`",
                    )
...
def _check_zombies(repo_root: Path) -> CheckResult:
    ...
    if orphans:
        return CheckResult(
            ...
            remediation="run `insightscli doctor:zombies`",
        )
    return CheckResult(
        name="Zombie processes",
        status=CheckStatus.OK,
...
def _check_migrations() -> CheckResult:
    ...
    try:
        from insightscli_commands.migrations import _compute_migration_diff
        ...
        if parts:
            return CheckResult(
                ...
                status=CheckStatus.WARNING,
                summary=", ".join(parts),
                remediation="run `insightscli migrations:sync`",
            )
        ...
        return CheckResult(
            ...
            remediation="start the dev environment with `insightscli start`",
        )
...
def _check_ports() -> CheckResult:
    ...
    if foreign:
        return CheckResult(
            ...
            remediation="run `insightscli doctor:ports`",
        )
...
def _phrocs_info() -> tuple[str, str]:
    """Diagnose phrocs, which powers the ``insightscli start`` TUI.

    A blank ``insightscli start`` screen most often traces back to phrocs being
    ...
    if not path:
        return ("phrocs", "MISSING — `insightscli start` TUI needs it (see tools/phrocs/install.sh)")
...
def _phrocs_runtime_pairs(repo_root: Path) -> list[tuple[str, str]]:
    ...
    blank or broken ``insightscli start`` TUI: the generated config phrocs renders,
    ...
    if config.exists():
        ...
        pairs.append(("generated_config", f"MISSING ({config}) — run `insightscli dev:generate`"))
...
class _ManifestLike(Protocol):
    """The slice of ``insightscli.manifest.Manifest`` the import probe relies on."""
...
def _probe_command_imports(manifest: _ManifestLike) -> tuple[int, list[tuple[str, str]]]:
    ...
    a common cause of confusing ``insightscli`` startup behaviour.
    """
...
def _build_report(repo_root: Path, manifest: _ManifestLike) -> str:
    ...
    lines: list[str] = [f"insightscli doctor:report — {timestamp}"]
    ...
    # phrocs is the headline failure mode (blank `insightscli start`), so group its
