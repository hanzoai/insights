...
// Patterns longer than this are compiled fresh on every call instead of being cached. A Script program
...
fn like_to_regex(pattern: &str) -> String {
    ...
    for c in pattern.chars() {
        if escape {
            ...
            // `%` matches any run of characters, newline included (as in Datastore and the
    ...
    while let Some(val) = current {
        ...
        match val {
            Value::Array(values) => {
                ...
                // Script JSON-path indices are 1-based; the reference also allows negatives counting
