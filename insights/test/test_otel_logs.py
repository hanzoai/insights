...
from insights import otel_logs
from insights.otel_logs import otel_log_mirror_processor, reset_otel_logs_for_tests
...
class TestOtelLogMirror(SimpleTestCase):
    ...
    def _run(
        ...
        *,
        ...
        with mock.patch("insights.otel_logs._build_provider", return_value=provider):
    ...
    def test_factory_is_fail_soft_when_provider_build_fails(self) -> None:
        ...
        event_dict = {"event": "x", "logger": f"{_PREFIX}.x"}
        with mock.patch("insights.otel_logs._build_provider", side_effect=RuntimeError("boom")):
            mirror = otel_log_mirror_processor("replay-vision", logger_prefix=_PREFIX, attribute_allowlist=_ALLOWLIST)
