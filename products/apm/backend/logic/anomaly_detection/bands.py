...
class _RegistryBandModel:
    ...
    name: str
    detector_type: str  # a insights.schema.DetectorType value
    ...
    def _fit_metadata(self, samples: np.ndarray, observed: float) -> dict[str, float]:
        ...
        # sake: insights.tasks.__init__ imports Django models, so a module-level
        # import would make this whole package require django.setup() — the
        # detector must stay importable with no infra.
        from insights.tasks.alerts.detectors.registry import get_detector  # noqa: PLC0415
