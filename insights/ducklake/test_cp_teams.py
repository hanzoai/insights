...
from insights.ducklake.cp_teams import CPTeam, team_from_row
    ...
    @parameterized.expand(
        [
            (
                ...
                {"schema_name": "us_prod"},
                ("events_us_prod", "persons_us_prod", "insights_data_imports_us_prod"),
            ),
            (
                "pins_win_over_derivation",
                {
                    ...
                    "schema_data_imports_name": "insights_data_imports_team_7",
                },
                ("events", "persons", "insights_data_imports_team_7"),
            ),
            (
                "partial_pins_mix_with_derivation",
                {"schema_name": "beta", "events_table_name": "events_legacy"},
                ("events_legacy", "persons_beta", "insights_data_imports_beta"),
            ),
            (
                "empty_string_pins_are_treated_as_unset",
                {"schema_name": "beta", "events_table_name": "", "schema_data_imports_name": ""},
                ("events_beta", "persons_beta", "insights_data_imports_beta"),
            ),
        ]
    )
...
class TestTTLCache:
    ...
    def test_second_call_within_ttl_hits_cache(self) -> None:
        with patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=[_row()]) as mock_fetch:
    ...
    def test_fresh_read_bypasses_and_replaces_cached_rows(self) -> None:
        with patch(
            "insights.ducklake.cp_teams._fetch_org_rows",
    ...
    def test_org_invalidation_clears_org_and_global_rows_only(self) -> None:
        with (
            patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=[_row()]) as fetch_org,
            patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=[_row()]) as fetch_all,
        ):
            cp_teams.list_org_teams("org-1")
    ...
    def test_invalidation_during_fetch_prevents_stale_repopulation(self) -> None:
        ...
        with patch("insights.ducklake.cp_teams._fetch_org_rows", side_effect=fetch_rows) as fetch_org:
    ...
    def test_failed_fetches_are_not_cached(self) -> None:
        ...
        with patch("insights.ducklake.cp_teams._fetch_org_rows", side_effect=[None, [_row()]]) as mock_fetch:
    ...
    def test_expired_entry_is_refetched(self) -> None:
        with (
            patch("insights.ducklake.cp_teams.CACHE_TTL_SECONDS", 0.0),
            patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=[_row()]) as mock_fetch,
        ):
            cp_teams.list_org_teams("org-1")
            cp_teams.list_org_teams("org-1")
        assert mock_fetch.call_count == 2
    ...
    def test_list_enabled_backfills_filters_disabled_rows(self) -> None:
        ...
        with patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=rows):
