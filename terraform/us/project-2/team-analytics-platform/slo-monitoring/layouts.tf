resource "insights_dashboard_layout" "slo_monitoring" {
  dashboard_id = insights_dashboard.slo_monitoring.id
  tiles = concat(
    # Row 0 (y=0, h=5): Success rate + volume table.
    [
      {
        insight_id = insights_insight.slo_success_rate.id
      ...
      {
        insight_id = insights_insight.slo_volume.id
        layouts_json = jsonencode({
          sm = {
      ...
      for op_idx, op_key in sort(keys(local.slo_operations)) : concat(
        ...
        [
          for reg_idx, region in sort(local.slo_operations[op_key].regions) : {
            insight_id = insights_insight.slo_burn_rate["${op_key}_${lower(region)}"].id
            layouts_json = jsonencode({
              sm = {
        ...
        [
          for reg_idx, region in sort(local.slo_operations[op_key].regions) : {
            insight_id = insights_insight.slo_duration["${op_key}_${lower(region)}"].id
            layouts_json = jsonencode({
              sm = {
