...
from insights.schema import InsightsQLQueryModifiers, WebAnalyticsOrderByFields, WebStatsBreakdown
...
# InsightsQL template for the precompute INSERT — a state-converted version of
...
# `replaceRegexpAll` calls — see `apply_path_cleaning` — and Datastore
...
FROM insights.web_stats_frustration_preaggregated
...
def execute_read_query(
    *,
    ...
    """Read the precomputed FRUSTRATION rows via InsightsQL.

    Returns the raw `response.results` (list of tuples) so the caller can
    materialize without depending on InsightsQL's response type. Sort and
