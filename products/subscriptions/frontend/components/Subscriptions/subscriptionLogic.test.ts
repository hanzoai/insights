...
describe('subscriptionLogic', () => {
    ...
    it.each<[string, string, string]>([
        ...
        const clickedCaptures = (): any[][] =>
            (insights.capture as jest.Mock).mock.calls.filter(([name]) => name === 'dashboard subscribe nudge clicked')
    ...
    it('toasts and maps ApiError attr to manual errors on save failure', async () => {
        ...
        expect(toast.error).toHaveBeenCalledWith('Select at least one insight')
