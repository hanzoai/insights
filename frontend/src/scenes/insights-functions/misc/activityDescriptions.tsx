...
export function insightsFunctionActivityDescriber(logItem: ActivityLogItem, asNotification?: boolean): HumanizedChange {
    if (logItem.scope != 'InsightsFunction') {
        console.error('InsightsFunction describer received a non-InsightsFunction activity')
    ...
    if (logItem.activity == 'restored') {
        ...
        const functionName = nameOrLinkToInsightsFunction(logItem?.item_id, logItem?.detail.name)
