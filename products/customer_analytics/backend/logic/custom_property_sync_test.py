...
_EXECUTE = "products.customer_analytics.backend.logic.custom_property_sync.execute_insightsql_query"
...
class CustomPropertySyncTest(TeamScopedTestMixin, BaseTest):
    ...
    def test_external_id_scope_with_no_matching_account_writes_nothing(self):
        ...
        assert result.written == 0
        execute.assert_not_called()  # empty key set -> zero batches -> no Datastore query
...
class SyncCustomPropertiesForAccountTest(TeamScopedTestMixin, BaseTest):
    ...
    def test_swallows_errors_and_records_no_sync_outcome(self):
        with patch(_EXECUTE, side_effect=Exception("datastore down")):
            sync_custom_properties_for_account(team_id=self.team.id, external_id="acme")
...
class ReadViewAccessControlTest(DatastoreTestMixin, TeamScopedTestMixin, BaseTest):
    def test_userless_sync_reads_view_despite_warehouse_access_control(self):
        # The Celery sync runs with no user, so InsightsQL warehouse-view access control (flag on)
        ...
        view = DataWarehouseSavedQuery.objects.create(
            ...
            query={"kind": "InsightsQLQuery", "query": "SELECT 'acme' AS org_id, 100 AS health_score"},
...
class ReadViewLimitTest(DatastoreTestMixin, TeamScopedTestMixin, BaseTest):
    def test_reads_all_matching_rows_beyond_default_insightsql_limit(self):
        # An unfiltered, unlimited read gets capped at 100 rows by the InsightsQL default limit,
