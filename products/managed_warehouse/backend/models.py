...
class DuckgresServer(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    # Datastore EC2 role). Written at provision time so the duckling backfill reads the
...
class DuckgresSinkSchemaState(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    # db_constraint=False: a real FK constraint on insights_team locks that hot
    ...
    # constraint on insights_user takes a lock on that hot table when this table
...
class ManagedWarehouseSourceJob(TeamScopedRootMixin, CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    class Meta(TeamScopedRootMixin.Meta):
        db_table = "insights_managedwarehousesourcejob"
