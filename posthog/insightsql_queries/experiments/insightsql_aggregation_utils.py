...
def is_aggregation_function(function_name: str) -> bool:
    ...
    # First check the dedicated INSIGHTSQL_AGGREGATIONS dictionary
    normalized_insightsql_aggregations = [func.lower() for func in INSIGHTSQL_AGGREGATIONS.keys()]
    if normalized_name in normalized_insightsql_aggregations:
        return True

    # Also check other function dictionaries for aggregate functions
    for functions_dict in [INSIGHTSQL_CLICKHOUSE_FUNCTIONS, INSIGHTSQL_POSTCUSTOM_FUNCTIONS]:
...
def extract_aggregation_and_inner_expr(
    insightsql_expr: Union[str, ast.Expr],
) -> tuple[Optional[str], ast.Expr, Optional[list[ast.Expr]], bool]:
    """
    Extract the aggregation function, inner expression, parameters, and distinct flag from a InsightsQL expression.

    Args:
        insightsql_expr: Either a InsightsQL expression string or an already parsed AST expression
    ...
    if isinstance(insightsql_expr, str):
        expr = parse_expr(insightsql_expr)
    else:
        expr = insightsql_expr
