...
class TestUpdateInsightsFunctionCode(BaseTest):
    def setUp(self):
        super().setUp()
        ...
        with patch("products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers"):
            ...
            self.linkedin_function1 = InsightsFunction.objects.create(
                team=self.team,
                name="LinkedIn Function 1",
                type="destination",
                template_id="template-linkedin-ads",
                description="Test LinkedIn Function 1",
                script="const headers = {'LinkedIn-Version': '202409'}; return event;",
                enabled=True,
            )

            # LinkedIn destination with new API version (should not be updated)
            self.linkedin_function2 = InsightsFunction.objects.create(
                team=self.team,
                name="LinkedIn Function 2",
                type="destination",
                template_id="template-linkedin-ads",
                description="Test LinkedIn Function 2",
                script="const headers = {'LinkedIn-Version': '202508'}; return event;",
                enabled=True,
            )
            ...
            self.meta_ads_function1 = InsightsFunction.objects.create(
                team=self.team,
                name="Meta Ads Function 1",
                type="destination",
                template_id="template-meta-ads",
                description="Test Meta Ads Function 1",
                script="let res := fetch(f'https://graph.facebook.com/v21.0/{inputs.pixelId}/events', {}); return event;",
                enabled=True,
            )

            # Meta Ads destination with new API version (should not be updated)
            self.meta_ads_function2 = InsightsFunction.objects.create(
                team=self.team,
                name="Meta Ads Function 2",
                type="destination",
                template_id="template-meta-ads",
                description="Test Meta Ads Function 2",
                script="let res := fetch(f'https://graph.facebook.com/v25.0/{inputs.pixelId}/events', {}); return event;",
                enabled=True,
            )
    ...
    def test_update_linkedin_api_version_dry_run(self, mock_compile_hog):
        ...
        assert "'LinkedIn-Version': '202409'" in self.linkedin_function1.script
    ...
    def test_update_linkedin_api_version_actual_update(self, mock_compile_hog):
        ...
        self.linkedin_function1.refresh_from_db()
        assert "'LinkedIn-Version': '202508'" in self.linkedin_function1.script

        # Check that functions that didn't need updating weren't changed
        self.linkedin_function2.refresh_from_db()
        assert "'LinkedIn-Version': '202409'" not in self.linkedin_function2.script
        assert "'LinkedIn-Version': '202508'" in self.linkedin_function2.script
    ...
    def test_update_meta_ads_api_version_actual_update(self, mock_compile_hog):
        ...
        assert "graph.facebook.com/v21.0/" not in self.meta_ads_function1.script
        assert "graph.facebook.com/v25.0/" in self.meta_ads_function1.script

        # Check that the function already on the new version was left untouched
        self.meta_ads_function2.refresh_from_db()
        assert "graph.facebook.com/v25.0/" in self.meta_ads_function2.script
    ...
    def test_update_google_ads_api_version_bumps_v19_to_v23_and_excludes_v18(self, mock_compile_hog):
        ...
        with patch("products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers"):
            g_v18 = InsightsFunction.objects.create(
                team=self.team,
                name="G v18",
                type="destination",
                template_id="template-google-ads",
                script=base.replace("{version}", "v18"),
                enabled=True,
            )
            g_v19 = InsightsFunction.objects.create(
                team=self.team,
                name="G v19",
                type="destination",
                template_id="template-google-ads",
                script=base.replace("{version}", "v19"),
                enabled=True,
            )
            g_v21 = InsightsFunction.objects.create(
                team=self.team,
                name="G v21",
                type="destination",
                template_id="template-google-ads",
                script=base.replace("{version}", "v21"),
                enabled=True,
            )
            g_v24 = InsightsFunction.objects.create(
                team=self.team,
                name="G v24",
                type="destination",
                template_id="template-google-ads",
                script=base.replace("{version}", "v24"),
                enabled=True,
            )

        out = StringIO()
        call_command("update_insights_function_code", replace_key="google-ads-api-version-update", stdout=out)
        ...
        assert g_v18.script == base.replace("{version}", "v18")
        assert "googleads.googleapis.com/v24/" in g_v19.script and "/v19/" not in g_v19.script
        assert "googleads.googleapis.com/v24/" in g_v21.script and "/v21/" not in g_v21.script
        assert g_v24.script == base.replace("{version}", "v24")  # unchanged
    ...
    def test_update_skips_destinations_that_fail_to_compile(self, mock_compile_hog):
        ...
        with patch("products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers"):
            bad_function = InsightsFunction.objects.create(
                ...
                script="let res := fetch(f'https://graph.facebook.com/v21.0/{inputs.pixelId}/events BROKEN', {}); return event;",
                enabled=True,
            )
        ...
        assert "graph.facebook.com/v21.0/" in bad_function.script
    ...
    def test_missing_replace_key(self):
        ...
        out = StringIO()
        call_command("update_insights_function_code", stdout=out)
    ...
    def _create_teams_function(self, script: str) -> InsightsFunction:
        with patch("products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers"):
    ...
    def test_microsoft_teams_powerplatform_migration(self, _name, stale_hog):
        ...
        with patch(
            ...
            out = StringIO()
            call_command("update_insights_function_code", replace_key="microsoft-teams-powerplatform-url", stdout=out)
    ...
    def test_microsoft_teams_powerplatform_cu_path_migration(self):
        ...
        with patch(
            ...
            out = StringIO()
            call_command("update_insights_function_code", replace_key="microsoft-teams-powerplatform-cu-path", stdout=out)

        function.refresh_from_db()
        # Both the regex and the human-readable example in the error message are widened.
        assert "automations/direct/(.*/)?workflows/.*'" in function.script
        assert "automations/direct/[<cluster>/]workflows/...)'" in function.script
    ...
    def test_microsoft_teams_migration_leaves_functions_without_the_stale_block_untouched(self):
        ...
        function.refresh_from_db()
        assert POWERPLATFORM_BRANCH not in function.script
