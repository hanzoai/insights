...
const PROMOTED_CONTEXT_CASES: [string, string, string | number][] = AI_CONTEXT_PREFIXES.flatMap((prefix) => [
    [prefix, 'insights_distinct_id', 'user-1'],
...
describe('vercel-ai middleware', () => {
    ...
    describe('$ai_generation', () => {
        ...
        it.each(AI_CONTEXT_PREFIXES)('promotes insights_-prefixed values from %s', (prefix) => {
            const event = createEvent('$ai_generation', {
                'ai.operationId': 'ai.generateText.doGenerate',
                [`${prefix}insights_tags`]: ['beta'],
            })
            convertOtelEvent(event)

            expect(event.properties!['tags']).toEqual(['beta'])
            expect(event.properties![`${prefix}insights_tags`]).toBeUndefined()
        })

        it.each(AI_CONTEXT_PREFIXES)(
            'uses insights_distinct_id from %s as the event distinct_id without creating a bare distinct_id property',
            (prefix) => {
                const event = createEvent('$ai_generation', {
                    'ai.operationId': 'ai.generateText.doGenerate',
                    [`${prefix}insights_distinct_id`]: 'user-1',
        ...
        it('prefers telemetry metadata over runtime context', () => {
            const event = createEvent('$ai_generation', {
                'ai.operationId': 'ai.generateText.doGenerate',
                'ai.telemetry.metadata.insights_distinct_id': 'metadata-user',
                'ai.settings.context.insights_distinct_id': 'context-user',
            })
            convertOtelEvent(event)

            expect(event.distinct_id).toBe('metadata-user')
            expect(event.properties!['ai.telemetry.metadata.insights_distinct_id']).toBeUndefined()
            expect(event.properties!['ai.settings.context.insights_distinct_id']).toBeUndefined()
        })

        it('ignores empty insights_distinct_id metadata', () => {
            const event = createEvent('$ai_generation', {
                'ai.operationId': 'ai.generateText.doGenerate',
                'ai.telemetry.metadata.insights_distinct_id': '',
            })
            convertOtelEvent(event)

            expect(event.properties!['insights_distinct_id']).toBeUndefined()
    ...
    describe('Eve', () => {
        it('normalizes Eve attributes from AI SDK runtime context', () => {
            const event = createEvent('$ai_span', {
                ...
                'ai.settings.context.insights_distinct_id': 'user-1',
