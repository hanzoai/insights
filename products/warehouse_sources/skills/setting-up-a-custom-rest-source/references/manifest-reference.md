...
- **`client.base_url`** (required) — every resource path resolves against this. On Insights Cloud it must be `https://`.
...
Insights **adopts the OAuth2 secrets into a server-managed credential store** on the first validation, preview, or
...
- **Keep the entire `client.auth` block identical across the db-schema → preview → create calls of one setup.**
  ...
  auth-block field (`scopes`, token-request knobs, `client_auth_method`, …) makes Insights treat the re-submitted
