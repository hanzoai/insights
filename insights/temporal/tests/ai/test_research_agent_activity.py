...
class TestProcessResearchAgentActivity:
    ...
    async def test_activity_success_writes_to_stream(
        ...
        with (
            patch(
                "insights.temporal.ai.research_agent.ConversationRedisStream", return_value=mock_redis_stream
            ) as mock_redis_stream_class,
            patch("insights.temporal.ai.research_agent.ResearchAgentRunner", return_value=mock_assistant),
        ):
    ...
    async def test_activity_retrieves_team_user_conversation_from_db(
        ...
        with (
            patch("insights.temporal.ai.research_agent.ConversationRedisStream", return_value=mock_redis_stream),
            patch(
                "insights.temporal.ai.research_agent.ResearchAgentRunner", return_value=mock_assistant
    ...
    async def test_activity_forwards_billing_context(
        ...
        from insights.schema import MaxBillingContext, MaxBillingContextSettings, MaxBillingContextSubscriptionLevel
    ...
    async def test_activity_uses_heartbeat_callback(
        ...
        with (
            patch("insights.temporal.ai.research_agent.ConversationRedisStream", return_value=mock_redis_stream),
            patch("insights.temporal.ai.research_agent.ResearchAgentRunner", return_value=mock_assistant),
            patch("insights.temporal.ai.research_agent.activity", mock_activity),
        ):
            await process_research_agent_activity(research_agent_inputs)
