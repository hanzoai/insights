...
async fn test_property_definitions_dedupe_within_batch(db: PgPool) {
    ...
    let count: Option<i64> = sqlx::query_scalar!(
        r#"SELECT COUNT(*) FROM insights_propertydefinition WHERE name IN ('brand', 'current_page')"#
    )
    .fetch_one(&db)
    .await
    .unwrap();
    ...
    let brand = sqlx::query!(
        r#"SELECT property_type, is_numerical FROM insights_propertydefinition WHERE name = 'brand'"#
    )
    .fetch_one(&db)
    .await
    .unwrap();
...
async fn test_property_definitions_typed_row_wins_regardless_of_order(db: PgPool) {
    ...
    for name in ["rev_a", "rev_b"] {
        let row = sqlx::query!(
            r#"SELECT property_type, is_numerical FROM insights_propertydefinition WHERE name = $1"#,
            name
        )
        .fetch_one(&db)
        .await
    ...
    let count: Option<i64> = sqlx::query_scalar!(
        r#"SELECT COUNT(*) FROM insights_propertydefinition WHERE name IN ('rev_a', 'rev_b')"#
    )
    .fetch_one(&db)
    .await
    .unwrap();
...
async fn test_property_definitions_keeps_rows_that_differ_on_conflict_key(db: PgPool) {
    ...
    let count: Option<i64> = sqlx::query_scalar!(
        r#"SELECT COUNT(*) FROM insights_propertydefinition WHERE name = 'shared'"#
    )
    .fetch_one(&db)
    .await
    .unwrap();
...
async fn test_property_definitions_rewrite_the_row_only_when_a_type_is_resolved(db: PgPool) {
    ...
    for (name, stored, incoming, expected_type, expect_rewrite) in cases {
        ...
        let xmin_before: String =
            sqlx::query_scalar("SELECT xmin::text FROM insights_propertydefinition WHERE name = $1")
                .bind(name)
                .fetch_one(&db)
                .await
                .unwrap();
        ...
        let (xmin_after, property_type): (String, Option<String>) = sqlx::query_as(
            "SELECT xmin::text, property_type FROM insights_propertydefinition WHERE name = $1",
        )
        .bind(name)
        .fetch_one(&db)
        .await
...
async fn test_event_definitions_dedupe_within_batch(db: PgPool) {
    ...
    let count: Option<i64> = sqlx::query_scalar!(
        r#"SELECT COUNT(*) FROM insights_eventdefinition WHERE name IN ('$pageview', '$autocapture')"#
    )
    .fetch_one(&db)
    .await
    .unwrap();
...
async fn test_fk_violation_strips_dead_team_rows_and_keeps_them_cached(db: PgPool) {
    ...
    sqlx::query(r#"CREATE TABLE insights_team (id INTEGER PRIMARY KEY)"#)
        .execute(&db)
        .await
        .unwrap();
    sqlx::query(r#"INSERT INTO insights_team (id) VALUES (111)"#)
        .execute(&db)
        .await
        .unwrap();
    for table in [
        "insights_eventdefinition",
        "insights_propertydefinition",
        "insights_eventproperty",
    ] {
        sqlx::query(&format!(
            "ALTER TABLE {table} ADD CONSTRAINT {table}_team_fk FOREIGN KEY (team_id) REFERENCES insights_team(id)"
        ))
        .execute(&db)
        .await
        .unwrap();
    ...
    for (table, expected) in [
        ("insights_eventdefinition", 1i64),
        ("insights_propertydefinition", 10),
        ("insights_eventproperty", 10),
    ] {
        let live: i64 =
...
async fn test_unparseable_fk_falls_back_to_retry_and_uncache(db: PgPool) {
    ...
    sqlx::query(
        r#"CREATE TABLE insights_team (id INTEGER, project_id BIGINT, PRIMARY KEY (id, project_id))"#,
    )
    .execute(&db)
    .await
    .unwrap();
    sqlx::query(
        r#"ALTER TABLE insights_eventproperty ADD CONSTRAINT eventproperty_team_proj_fk
           FOREIGN KEY (team_id, project_id) REFERENCES insights_team(id, project_id)"#,
    )
    .execute(&db)
    .await
    .unwrap();
    ...
    let written: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM insights_eventproperty")
        .fetch_one(&db)
        .await
        .unwrap();
