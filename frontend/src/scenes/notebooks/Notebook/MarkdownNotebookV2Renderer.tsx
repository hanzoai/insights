    ...
    const convertExternalDataTransferToNodes = useCallback(
        (dataTransfer: DataTransfer): NotebookBlockNode[] | Promise<NotebookBlockNode[] | null> | null => {
            ...
            if (nodeType) {
                ...
                if (node) {
                    insights.capture('notebook node dropped', { node_type: nodeType, is_markdown: true })
                }
                return node ? [node] : null
            }

            // Entity links (`Link`) drag with only their href: map recognized Insights resource
            ...
            if (draggedUrl) {
                const resourceNode = convertDroppedInsightsUrlToMarkdownNode(draggedUrl)
                insights.capture('notebook node dropped', {
