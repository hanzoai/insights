...
# the main pool pods. The legacy `insights` namespace doesn't need a
# discriminator because its pgbouncers carry a different `name`.
POOLS = {
    "toolbox-django": {
        # Golden-chart deployment in the per-app namespace; the legacy `insights`
        # namespace is still available via `KUBE_NAMESPACE=insights`.
...
def main():
    ...
    try:
        ...
        parser.add_argument(
            ...
            help="Which pool to claim from. Defaults to toolbox-django (the original insights-toolbox-django pool).",
        )
        parser.add_argument(
            "--auto-delete",
            action="store_true",
        ...
        # at the legacy `insights` namespace during migration).
        ...
        # `insights-toolbox-django` namespace), so we may need a further
