...
import { ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT, AccountColumnDisplayState } from './accountsColumnConfigLogic'
...
// column name (e.g. `csm`) — InsightsQL resolves it against the SELECT alias at
// query-build time.
...
export function deserializeAccountsView(view: Partial<ColumnConfigurationApi>): AccountsViewState {
    ...
    return {
        columns: view.columns && view.columns.length > 0 ? view.columns : [...ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT],
