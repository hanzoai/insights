...
description: >
  ...
  per-customer/per-account revenue — on either Insights data-warehouse views (InsightsQL) or an external dbt
  ...
  cohort, product, or currency. On Insights, build on the managed revenue_analytics_* views (revenue_item,
...
Revenue reaches Insights two ways; both feed the same **managed `revenue_analytics_*` views**:
...
Insights auto-generates a curated set of views per source. **Do not re-derive revenue from raw Stripe
...
4. **Link revenue to people via metadata.** Person/group-level revenue needs
   `insights_person_distinct_id` metadata on the Stripe customer (or the person join). Without it, revenue is
   customer-level only.
...
**Insights:** write the InsightsQL (alias every column), `view-create`, verify with `view-get`, then
`view-materialize` the expensive monthly rollups (a daily `sync_frequency` is usually right for revenue).
Recipes: [`references/insights/`](references/insights/) — `mrr_and_arr.sql`, `gross_revenue_by_month.sql`,
...
dimensions), `querying-insights-data` (InsightsQL + the semantic-layer metric check).
