"""Tests for insightscli devex:feedback.
...
well-formed ``insightscli_feedback`` event, so the feature can't silently stop reaching the
...
from insightscli_commands import feedback
...
def test_send_without_api_key_never_posts(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(feedback, "_endpoint", lambda: ("https://us.i.hanzo.ai", ""))
    with patch.object(feedback.requests, "post") as post:
