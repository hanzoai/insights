...
from insights.datastore.traces.spans import TRACE_SPANS_DISTRIBUTED_TABLE_SQL, TRACE_SPANS_TABLE_SQL
...
T_RERUN_RECOVERY = "insights/api/test/test_rerun/TestRerun::test_green_on_attempt_2"
T_STALE_REREPORT = "insights/api/test/test_stale/TestStale::test_reported_twice"
T_CROSS_RUN_PASS = "insights/api/test/test_cross/TestCross::test_passes_in_another_run"
T_PASS_THEN_FAIL = "insights/api/test/test_order/TestOrder::test_fails_after_passing"
T_MATRIX_LEGS = "insights/api/test/test_legs/TestLegs::test_fails_in_two_legs"
T_IN_JOB_RETRY = "insights/api/test/test_injob/TestInJob::test_pytest_retry"
T_IN_JOB_SELECTOR = "insights/api/test/test_injob.py::TestInJob::test_pytest_retry"
T_THREE_PRS = "insights/api/test/test_three/TestThree::test_fails_on_three_prs"
T_TWO_PRS = "insights/api/test/test_two/TestTwo::test_fails_on_two_prs"
T_MASTER = "insights/api/test/test_master/TestMaster::test_breaks_trunk"
T_QUARANTINED = "insights/api/test/test_quarantined/TestQuarantined::test_still_fails"
T_OLD = "insights/api/test/test_old/TestOld::test_old_flake"
T_NO_RUN_ID = "insights/api/test/test_norun/TestNoRun::test_unstamped_spans"
T_TIE_A = "insights/api/test/test_tie_a/TestTie::test_retry"
T_TIE_B = "insights/api/test/test_tie_b/TestTie::test_retry"
T_FOREIGN = "insights/api/test/test_foreign/TestForeign::test_other_service"
T_OTHER_REPO = "insights/api/test/test_other_repo/TestOtherRepo::test_flaky"
...
class TestFlakyTestsAPI(DatastoreTestMixin, APIBaseTest):
    # The aggregation, qualification (HAVING), and ranking all live in the InsightsQL query, so the
    # regressions worth catching only surface against real seeded trace_spans rows.

    # DatastoreTestMixin flips this off (per-test teams); back on so one class-level team can
    ...
    def setUpTestData(cls) -> None:
        super().setUpTestData()
        connect_github_source_without_data(cls.team, prefix="flaky", repository="Insights/insights")
        ...
        rows = [
            ...
            cls._span(22, T_OTHER_REPO, "rerun_passed", ts=recent, run="1200", pr="1201", repo="Insights/hanzo.ai"),
    ...
    def test_evidence_is_counted_once_per_run(self) -> None:
        ...
        assert three_prs["selector"] == "insights/api/test/test_three.py::TestThree::test_fails_on_three_prs"
...
@pytest.mark.parametrize(
    "nodeid,expected",
    [
        ("insights/api/test/test_event/TestEvents::test_x", "insights/api/test/test_event.py::TestEvents::test_x"),
        ("insights/tasks/test/test_calc::test_sum", "insights/tasks/test/test_calc.py::test_sum"),
        ("insights/test/test_a/TestOuter/TestInner::test_x", "insights/test/test_a.py::TestOuter::TestInner::test_x"),
        ("test_bare", "test_bare"),
    ],
)
