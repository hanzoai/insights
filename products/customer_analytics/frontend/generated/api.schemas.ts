...
export interface ExternalAccountListAssignmentApi {
    /** Insights user id of the assigned user. */
    user_id: number
...
export interface AccountAssignmentApi {
    /** Insights user id of the assignee. */
    readonly id: number
...
export interface AccountRelationshipWriteApi {
    ...
    definition: string
    /** Insights user id of the assignee. Must be a member of the account's organization. */
    user: number
}
...
export interface CustomPropertyDefinitionApi {
    ...
    /** True when Insights writes this property itself. Its name and display type are fixed — an update changing either is rejected. */
 ...
 *
 * **Events** (default, when `source` is missing or `"events"`): InsightsFunction filter shape — `events: [...]`, optional `actions: [...]`, `properties: [...]`, `filter_test_accounts: bool`.
 *
 * **Data warehouse** (`source: "data_warehouse"`): `table_name` (synced DW table), `timestamp_field` (timestamp column or InsightsQL expression), `key_field` (column whose value matches the entity key). Currently DW metrics only render on group profiles — person profiles are not yet supported.
 */
