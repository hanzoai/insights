...
_CAPTURE = "insights.event_usage.hanzo_insights.capture"
...
class TestDataCatalogAnalytics(BaseTest):
    ...
    def test_drifted_approve_emits_blocked_event_and_still_raises(self, capture: MagicMock) -> None:
        ...
        Insight.objects.filter(pk=insight.pk).update(query={"kind": "InsightsQLQuery", "query": "select 1"})
        capture.reset_mock()
    ...
    @parameterized.expand(
        [
            ("definition_error", {"side_effect": ExposedInsightsQLError("no such table")}),
