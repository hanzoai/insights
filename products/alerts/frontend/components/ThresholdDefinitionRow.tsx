...
import { Banner, Input, SegmentedButton, Select } from '@hanzo/elements'
...
import { isAnyRowInsightsQLConfig, isFunnelsAlertConfig } from 'products/alerts/frontend/types'
...
export function ThresholdDefinitionRow({
    ...
    const relativeConditionDisabledReason =
        ...
        (isAnyRowInsightsQLConfig(alertForm.config) &&
            "Rows in any-row mode aren't a time series. Switch to 'the latest value' for relative conditions")

    return (
        <div className="space-y-2">
            {thresholdBoundsFormError ? <Banner type="error">{thresholdBoundsFormError}</Banner> : null}
            <AlertDefinitionRow>
                {supportsRelativeConditions && (
