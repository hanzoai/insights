...
from insights.api.integration import github_rate_limited_response
from insights.api.mixins import ValidatedRequest, validated_request
from insights.api.routing import TeamAndOrgViewSetMixin
from insights.auth import InternalAPIAuthentication, OAuthAccessTokenAuthentication, PersonalAPIKeyAuthentication
from insights.egress.github.transport import GitHubEgressBudgetExhausted, GitHubRateLimitError
...
from insights.models.github_integration_base import GitHubIntegrationBase
...
from insights.permissions import APIScopePermission, get_authenticator_scoped_team_ids, get_authenticator_scopes
...
class SignalSourceConfigViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def _trigger_session_analysis_setup(self) -> None:
        ...
        from insights.temporal.session_replay.summarization_sweep.schedule import a_upsert_team_schedule
...
class SignalReportViewSet(
    ...
    def _apply_signal_report_source_product_filter(self, queryset):
        ...
        # rather than run a second Datastore query for a strictly wider set.
    ...
    def _enriched_report_context(self, report: SignalReport) -> dict:
        ...
        # is missing, so a Datastore/Postgres hiccup must not turn an otherwise-available
    ...
    def partial_update(self, request: ValidatedRequest, *args, **kwargs) -> Response:
        ...
        # the edit is the agent's task when an `X-Insights-Task-Id` header is present, otherwise the
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                ...
                required=False,
                description=(
                    "Comma-separated list of Insights user UUIDs. Reports are kept if their suggested reviewers "
    ...
    def list(self, request, *args, **kwargs):
        ...
        # so a slow load can be attributed to Postgres (queryset annotations), Datastore (source
        ...
        # urls). The serializer degrades to empty values when a map is missing, so a Datastore or
    ...
    @extend_schema(
        ...
        description="Fetch all signals for a report from Datastore, including full metadata.",
    ...
    def signals(self, request, pk=None, **kwargs):
        """Fetch all signals for a report from Datastore, including full metadata."""
        report = self.get_object()
    ...
    def _request_attribution(self) -> ArtefactAttribution:
        ...
        `resolve_request_attribution` validates the `X-Insights-Task-Id` header with a DB lookup, and
    ...
    def refund(self, request: ValidatedRequest, pk=None, **kwargs):
        ...
        # Resolved before the locked transaction — a bad X-Insights-Task-Id header must 400
...
def append_suggested_reviewers(
    *,
    ...
    # the requesting user — never the X-Insights-Task-Id header. A task-attributed reviewers row has
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=(
            ...
            "`suggested_reviewers` content is enriched with Insights user info at read time."
        ),
...
class SignalReportArtefactViewSet(
    ...
    Writes are attributed: to the task named by the `X-Insights-Task-Id` header (set automatically
    ...
    @validated_request(
        ...
        responses={
            ...
            400: OpenApiResponse(
                ...
                "or an invalid X-Insights-Task-Id header."
            ),
            404: OpenApiResponse(description="Report not found for this project."),
        },
        parameters=[
    ...
    def create(self, request: ValidatedRequest, *args, **kwargs) -> Response:
        ...
        if artefact_type == SignalReportArtefact.ArtefactType.TASK_RUN:
            ...
            if not task_id:
                return Response(
                    {"error": "Provide content.task_id, or call with an X-Insights-Task-Id header."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
