"""Mirror persisted task-run log entries into the Insights Logs product via stdout.

Task-run logs are appended to object storage as one ACP notification envelope per line.
In every Insights cluster an OTel collector daemonset already tails container stdout and
ships JSON log lines into the region's internal Insights project's Logs product, parsing
...
batch straight to a logs ingest endpoint. The token pins the destination to Insights's own
...
def _severity(notification: dict) -> str:
    if notification.get("method") == "_insights/error":
        return "error"
    if notification.get("method") == "_insights/console":
        params = notification.get("params")
...
def _body(notification: dict, update: dict, session_update: str | None) -> str:
    ...
    if session_update in _READABLE_SESSION_UPDATES:
        ...
        body = f"[{session_update}]"
    elif notification.get("method") in ("_insights/console", "_insights/error"):
        message = params.get("message")
        if isinstance(message, str):
            body = message
    elif notification.get("method") == "_insights/sandbox_output":
