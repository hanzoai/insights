...
type CustomFlowStepBuilder<T extends CustomFlowAction['type']> = {
    type: T
    icon: (
        action: Extract<CustomFlowAction, { type: T }>,
        customFunctionTemplatesById: Record<string, CustomFunctionTemplateType>
    ) => JSX.Element
    color: (action: Extract<CustomFlowAction, { type: T }>, isDarkModeOn: boolean) => string
    renderConfiguration: (node: Node<Extract<CustomFlowAction, { type: T }>>) => JSX.Element
}

type CustomFlowStep<T extends CustomFlowAction['type']> = {
    type: T
    icon: JSX.Element
    color: string
    renderConfiguration: (node: Node<Extract<CustomFlowAction, { type: T }>>) => JSX.Element
...
const CustomFlowStepConfigs: Partial<{
    [K in CustomFlowAction['type']]: CustomFlowStepBuilder<K>
}> = {
    conditional_branch: {
    ...
    function: {
        type: 'function',
        icon: (action, customFunctionTemplatesById) => {
...
export function getCustomFlowStep<T extends CustomFlowAction['type']>(
    action: Extract<CustomFlowAction, { type: T }>,
    customFunctionTemplatesById: Record<string, CustomFunctionTemplateType>,
    isDarkModeOn = false
): CustomFlowStep<T> | undefined {
    const type = action.type
    const builder = CustomFlowStepConfigs[type] as CustomFlowStepBuilder<T> | undefined
    if (!builder) {
        return undefined
    }
    return {
        type,
        icon: builder.icon(action, customFunctionTemplatesById),
        color: builder.color(action, isDarkModeOn),
        renderConfiguration: builder.renderConfiguration,
    }
}

export function useCustomFlowStep<T extends CustomFlowAction['type']>(
    action?: Extract<CustomFlowAction, { type: T }>
): CustomFlowStep<T> | undefined {
    const { customFunctionTemplatesById } = useValues(workflowLogic)
    ...
    return useMemo(() => {
        ...
        return getCustomFlowStep(action, customFunctionTemplatesById, isDarkModeOn)
    }, [action, customFunctionTemplatesById, isDarkModeOn])
}
