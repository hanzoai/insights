...
from products.web_analytics.backend.insightsql_queries.web_stats_lazy_precompute import can_use_lazy_precompute
...
class TestWebStatsTableQueryRunner(
    FirstPageviewAttributionTestMixin, DatastoreTestMixin, APIBaseTest, FloatAwareTestCase
):
    ...
    def test_first_pageview_attribution_bypasses_lazy_precompute(self):
        ...
        def lazy_eligible(flag_on):
            with (
                patch(
                    "products.web_analytics.backend.insightsql_queries.web_stats_lazy_precompute._can_use_lazy_precompute_shared",
                    return_value=True,
                ),
...
class TestWebStatsTableNoJoinFastPath(DatastoreTestMixin, APIBaseTest):
    QUERY_TIMESTAMP = "2025-01-29"
...
class TestWebStatsTableSessionIdSetFastPath(DatastoreTestMixin, APIBaseTest):
    QUERY_TIMESTAMP = "2025-01-29"
    ...
    def test_session_id_set_paths_executes_with_pushdown_modifier_and_tag(self):
        ...
        with freeze_time(self.QUERY_TIMESTAMP):
            with override_settings(WEB_ANALYTICS_SESSION_ID_SET_TEAM_IDS=[self.team.pk]):
                runner = self._make_runner()
                original_execute = runner.paginator.execute_insightsql_query
                ...
                with patch.object(runner.paginator, "execute_insightsql_query", side_effect=record_calls):
                    runner.calculate()
