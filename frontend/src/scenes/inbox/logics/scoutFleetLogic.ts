...
export const scoutFleetLogic = kea<scoutFleetLogicType>([
    ...
    listeners(({ actions, values, cache }) => ({
            ...
            try {
                while (updatesToSend) {
                    ...
                    if (updatesToSend.run_cron_schedule === null && previousCronSchedule) {
                        toast.info('Scheduled run time cleared. The scout is back on its rolling interval.')
                    }
                ...
                toast.error(error?.detail || error?.message || 'Failed to update scout config')
            } finally {
        ...
        deleteScout: async ({ configId }) => {
            ...
            try {
                ...
                try {
                    ...
                    toast.error(error?.detail || error?.message || 'Failed to delete scout')
        ...
        startScoutChatTask: async ({ prompt, fallbackTitle, taskLabel }) => {
            ...
            try {
                // Deliberately repo-less: these prompts read Insights data over MCP and never touch
                ...
                try {
                    ...
                    toast.error(error?.detail || error?.message || `Failed to run ${taskLabel}`)
                }
                actions.startScoutChatTaskSuccess()
                router.actions.push(urls.taskDetail(task.id))
            } catch (error: any) {
                toast.error(error?.detail || error?.message || `Failed to start ${taskLabel}`)
                actions.startScoutChatTaskFailure()
            }
        },
        startRunsPolling: () => {
