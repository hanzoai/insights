...
/// The run/team-mismatch and out-of-range-day checks the scanner runs before touching Datastore.
/// Named by the scanner (not the store), so the store never references a Datastore type.
