# Diagnostic queries (InsightsQL, read-only)

Run via the Insights MCP `execute-sql` against the customer's project. Adjust the date and
survey ID. Tables: `events`, `static_cohort_people` (NOT `person_static_cohort` — that's
the Datastore name; InsightsQL exposes `static_cohort_people`), `persons`.
...
All `false` with empty `$groups` ⇒ group-aggregated flag without `insights.group()`.
