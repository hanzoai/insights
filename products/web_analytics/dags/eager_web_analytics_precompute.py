...
InsightsQL query result cache (6h staleness), which is the wrong clock for a
...
must not pile unthrottled Datastore work on top of a saturated cluster. It
...
from products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common import is_precompute_enabled_for_team
from products.web_analytics.dags.web_preaggregated_utils import check_for_concurrent_runs

# Pre-warm the InsightsQL bytecode VM at code-location load, while /code is on sys.path: the query path
# imports common.scriptvm lazily, and the Dagster grpc process can't resolve it on first query, so
# caching it here keeps that lazy import a hit.
import common.scriptvm.python.execute  # noqa: F401
...
# queries hitting Datastore. 10 sits comfortably inside the warmer user's
...
def _fetch_active_wa_team_ids(limit: int, lookback_hours: int = 24) -> list[int]:
    ...
    from insights.datastore.client import sync_execute  # noqa: PLC0415 — keep dagster import path light
    ...
    try:
        rows = sync_execute(
            ...
            LIMIT %(limit)s
            """,
            {"cluster": "insights", "hours": lookback_hours, "limit": limit},
        )
...
def warm_eager_baseline_op(context: dagster.OpExecutionContext, config: EagerWarmConfig) -> dict[str, int]:
    ...
    try:
        ...
        # Insights. `context.log` only reaches the Dagster UI — Insights's Dagster
        ...
        # teams at once to spread the Datastore load and cut total wall-clock.
        eligible: list[Team] = []
