...
generated by an autonomous agent with read-only access to the customer's Insights project
...
connection (anything sourced via a `connectionId` rather than the customer's own Insights
project data). Support diagnostics must stay within the customer's Insights project; a reply
