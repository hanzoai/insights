...
actually have a connection of that type (queried from the synced `insights_externaldatasource` replicas
...
Datastore, Neon, Convex, Google Sheets, Custom) introspect user-defined schemas and are out of scope.
...
- [ ] Lead gen forms and form responses. (skipped: the Lead Sync API needs `r_marketing_leadgen_automation`, which is not in Insights's LinkedIn OAuth app and belongs to a separate approval program)
...
- [ ] Ad account table (skipped: `/advertiser/info/` is not readable with the scope the Insights TikTok app holds, so the table would fail for every connection. Report tables already carry `currency`; timezone stays missing).
