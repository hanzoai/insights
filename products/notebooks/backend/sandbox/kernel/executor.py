...
Flow for a kernel node (python or duckdb — the kernel branches on node.type):
  1. materialize each InsightsQL input — the server streams the full CH result to a local Arrow
...
class KernelExecutor:
    ...
    def _materialize_inputs(
        ...
        """Fetch each InsightsQL input to a local Arrow file; return the kernel-facing input specs (paths only).
        ...
        for spec in payload.get("inputs") or []:
            ...
            kernel_inputs.append({"name": name, "kind": "insightsql", "path": frame_path})
