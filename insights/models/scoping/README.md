...
Related: [#47073](https://github.com/Insights/insights/pull/47073), [#50899](https://github.com/Insights/insights/pull/50899)
...
`insightscli product:bootstrap` scaffolds new products with the correct fail-closed base automatically — `ProductTeamModel` for separate-DB products, `TeamScopedRootMixin` for main-DB products. CI enforces it: any new team-scoped model on a non-fail-closed manager (i.e., not in `insights/models/scoping/baseline_unmigrated.txt`) fails `check-idor-model-coverage.py`.
...
class FeatureFlag(TeamScopedRootMixin):          # was RootTeamMixin — adds enforcement
    team = models.ForeignKey("insights.Team", ...)
```
...
Remove the marker once the underlying model migrates to `TeamScopedManager`. The decorator lives at `insights.scoping_audit` (not `insights.models.scoping`) so it can be imported at module load by Celery task files without triggering Django's app registry — see the file's docstring for the load-order story.
