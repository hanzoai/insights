...
import type { CyclotronJobInputSchemaType, InsightsFunctionSubTemplateType } from '../../../../types'
...
export interface WizardTrigger {
    key: InsightsFunctionSubTemplateIdType
    name: string
    description: string
}
...
export interface AlertWizardLogicProps {
    ...
    presetTriggerKey?: InsightsFunctionSubTemplateIdType
    // When set, the wizard pre-applies a `kind IN (...)` property filter on the
    // first event of the created InsightsFunction. Used by the health-alerts family
...
function creatableDestinations(
    triggerKey: InsightsFunctionSubTemplateIdType | null | undefined,
    destinations: WizardDestination[]
): WizardDestination[] {
    if (!triggerKey) {
        return destinations
...
interface WizardUrlState {
    ...
    triggerKey: InsightsFunctionSubTemplateIdType | null
}
...
// entry point can scope the resulting InsightsFunction to specific kinds.
//
// Top-level (vs `events[0].properties`) matches the convention of the trigger
// UI in InsightsFunctionFiltersInternal, so the kind filter remains visible and
...
// InsightsFunction is immediately identifiable in lists.
...
function extractDestinationKeyFromAlert(alert: InsightsFunctionType, allDestinations: WizardDestination[]): string | null {
...
export interface alertWizardLogicValues {
    activeSubTemplate: InsightsFunctionSubTemplateType | null;
    ...
    existingAlerts: InsightsFunctionType[];
    ...
    selectedKinds: string[] | null;
    selectedTemplate: InsightsFunctionTemplateType | null;
    selectedTemplateLoading: boolean;
    selectedTriggerKey: InsightsFunctionSubTemplateIdType | null;
...
export interface alertWizardLogicActions {
    ...
    loadExistingAlertsSuccess: (existingAlerts: InsightsFunctionType[], payload?: any) => {
        existingAlerts: InsightsFunctionType[];
        payload?: any;
    };
    ...
    loadTemplateSuccess: (selectedTemplate: InsightsFunctionTemplateType, payload?: string) => {
        selectedTemplate: InsightsFunctionTemplateType;
        payload?: string;
    };
    resetWizard: () => {
        value: true;
    ...
    setTriggerKey: (triggerKey: InsightsFunctionSubTemplateIdType) => {
        triggerKey: InsightsFunctionSubTemplateIdType;
    };
    submitConfiguration: () => {
        value: true;
...
export interface alertWizardLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        ...
        usedDestinationKeys: (existingAlerts: InsightsFunctionType[], allDestinations: WizardDestination[]) => Set<string>;
        sortedDestinations: (usedDestinationKeys: Set<string>, availableDestinations: WizardDestination[]) => WizardDestination[];
        ...
        activeSubTemplate: (selectedDestinationKey: string | null, selectedTriggerKey: InsightsFunctionSubTemplateIdType | null, allDestinations: WizardDestination[]) => InsightsFunctionSubTemplateType | null;
        requiredInputsSchema: (selectedTemplate: InsightsFunctionTemplateType | null, activeSubTemplate: InsightsFunctionSubTemplateType | null) => CyclotronJobInputSchemaType[];
        configuration: (selectedTemplate: InsightsFunctionTemplateType | null, inputValues: Record<string, {
            bytecode?: any;
            order?: number | undefined;
            secret?: boolean | undefined;
            templating?: "liquid" | "script" | undefined;
...
export const alertWizardLogic = kea<alertWizardLogicType>([
    ...
    actions({
        ...
        setTriggerKey: (triggerKey: InsightsFunctionSubTemplateIdType) => ({ triggerKey }),
    ...
    reducers(({ props: logicProps }) => ({
        ...
        selectedTriggerKey: [
            (logicProps.presetTriggerKey ?? null) as InsightsFunctionSubTemplateIdType | null,
            {
                setTriggerKey: (_, { triggerKey }) => triggerKey,
                restoreWizardState: (_, { state }) => state.triggerKey,
                resetWizard: () => (logicProps.presetTriggerKey ?? null) as InsightsFunctionSubTemplateIdType | null,
            },
        ],
        selectedKinds: [
        ...
        selectedTemplate: [
            null as InsightsFunctionTemplateType | null,
            {
                loadTemplate: async (templateId: string) => {
                    return await api.insightsFunctions.getTemplate(templateId)
                },
            },
        ],
    })),
    ...
    selectors(({ props: logicProps }) => ({
        ...
        usedDestinationKeys: [
            ...
            (existingAlerts: InsightsFunctionType[], allDestinations: WizardDestination[]): Set<string> => {
        ...
        requiredInputsSchema: [
            ...
            (
                selectedTemplate: InsightsFunctionTemplateType | null,
                activeSubTemplate: null | import('~/types').InsightsFunctionSubTemplateType
            ) => {
                if (!selectedTemplate) {
                    return []
                }
        ...
        configuration: [
            ...
            (selectedTemplate: InsightsFunctionTemplateType | null, inputValues: Record<string, CyclotronJobInputType>) => ({
    ...
    listeners(({ values, actions, props: logicProps }) => ({
        ...
        testConfiguration: async (_, breakpoint) => {
            ...
            if (!subTemplate) {
                toast.error('Template not found for this combination')
                return
            }
            ...
            let globals: CyclotronJobInvocationGlobals = {
                event: {
                    ...
                    properties: {
                        name: 'Test issue',
                        description: 'This is a test alert from Insights',
                    },
                },
                project: {
                    id: 0,
            ...
            try {
                await api.insightsFunctions.createTestInvocation('new', {
                    configuration,
                    globals,
                    mock_async_functions: false,
                })
                breakpoint()
                toast.success('Test invocation sent')
            } catch (e: any) {
                breakpoint()
                toast.error(e.detail || 'Test invocation failed')
            }
        ...
        submitConfiguration: async () => {
            try {
                ...
                toast.error(e.detail || 'Failed to create alert')
            } finally {
    ...
    urlToAction(({ actions, values, props: logicProps }) => ({
        '**': (_, searchParams) => {
            ...
            const wizardTrigger = searchParams.wizard_trigger as InsightsFunctionSubTemplateIdType | undefined
