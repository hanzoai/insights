...
# live in Insights/insights-cloud-infra (datastore/hcl/), not the OSS gate.
role "data" {
...
# (chschema #127). So the `insights` data cluster — modeled here only for `local`,
# prod goldens in insights-cloud-infra — is validated for `local` and absent for the
...
cluster "insights" {
  roles   = ["data"]
  aliases = ["insights_writable", "insights_primary_replica", "insights_single_shard"]
}
