...
class InternalSourceType(str, enum.Enum):
    """Source types for internal Insights products (not data warehouse imports)."""
...
# Each source defines its own fetcher (data warehouse uses InsightsQL, conversations uses Django ORM, etc.)
...
class SignalSourceTableConfig(BaseModel):
    ...
    fields: tuple[str, ...]
    # Optional filter clause (interpreted by the fetcher — InsightsQL for data warehouse, ORM for Postgres sources)
