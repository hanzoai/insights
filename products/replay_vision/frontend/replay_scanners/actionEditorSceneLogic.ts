        ...
        actionForm: {
            ...
            submit: async (form: VisionActionForm) => {
                ...
                if (values.isNew) {
                    ...
                    toast.success(form.mode === VisionActionModeEnumApi.Alert ? 'Alert created' : 'Digest created')
                ...
                toast.success(form.mode === VisionActionModeEnumApi.Alert ? 'Alert updated' : 'Digest updated')
                visionActionsLogic.findMounted({ scannerId })?.actions.loadActions()
    ...
    listeners(({ actions, values }) => ({
        ...
        loadAction: async ({ actionId }, breakpoint) => {
            ...
            try {
                ...
                toast.error(`Failed to load this action${error.detail ? `: ${error.detail}` : ''}`)
                actions.loadActionFailure()
            }
        },
        ...
        submitActionFormFailure: ({ error }: { error?: Error & { detail?: string } }) => {
            ...
            toast.error(`Failed to save ${noun}${error?.detail ? `: ${error.detail}` : ''}`)
        },
    })),
