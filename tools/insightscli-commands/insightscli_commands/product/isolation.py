...
# Datastore DDL consumed by core's schema registry and frozen migrations, which can never be
...
def has_legacy_interface_leaks(tach_content: str, module_path: str) -> bool:
    ...
    These are products where core (insights/ee) still imports internals directly,
...
def permanent_interface_modules(tach_content: str, module_path: str) -> set[str]:
    ...
    These are non-import-behavioral couplings — Datastore DDL imported by core's schema
...
# Datastore DDL, so the DDL-qualification rule below doesn't apply. warehouse_sources: core's
# InsightsQL direct-SQL adapters and system tables reach source internals through the facade's lazy
...
@functools.cache
def _datastore_ddl_imports(repo_root: Path) -> frozenset[str]:
    """Dotted module paths imported by the consumers of a permanent DDL interface outside the
    import-reroute path: the frozen Datastore migrations and the schema registry.
    ...
    """
    migrations_dir = repo_root / "insights" / "datastore" / "migrations"
    schema_file = repo_root / "insights" / "datastore" / "schema.py"
...
def unqualified_permanent_modules(
    ...
    import graph — Datastore DDL imported by a frozen migration or the schema registry. This is
    ...
    if not permanent_modules or module_path in _QUALIFICATION_EXEMPT_PRODUCTS:
        return set()
    imported = _datastore_ddl_imports(repo_root)
...
class IsolationStatus:
    ...
    # Marked permanent modules that aren't imported by any frozen Datastore migration or the
