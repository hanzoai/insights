...
describe('AwsS3SetupModal', () => {
    ...
    it('opens on the role tab with the trust policy requirements, including the external ID', async () => {
        ...
        // The external ID users must set in their trust policy is insights-{organization ID}
        await waitFor(() => {
            expect(screen.getByText(`insights-${MOCK_ORGANIZATION_ID}`)).toBeInTheDocument()
        })
    })

    it.each<[string, Region | null, string]>([
        ['US cloud', Region.US, 'arn:aws:iam::309986977637:role/insights-external-batch-exports'],
        ['EU cloud', Region.EU, 'arn:aws:iam::623789312881:role/insights-external-batch-exports'],
