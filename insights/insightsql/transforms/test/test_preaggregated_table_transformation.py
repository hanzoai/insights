...
from insights.insightsql.database.schema.web_analytics_preaggregated import WebPreAggregatedStatsTable
from insights.insightsql.parser import parse_select
from insights.insightsql.query import execute_insightsql_query
from insights.insightsql.transforms.preaggregated_table_transformation import (
...
class TestPreaggregatedTableTransformation(BaseTest, QueryMatchingTest):
    ...
    def _parse_and_transform(self, query: str):
        ...
        try:
            ...
            database = None
        context = InsightsQLContext(team_id=self.team.pk, team=self.team, database=database)
    ...
    def test_qualified_insights_events_transforms(self):
        original_query = "select count() from insights.events where event = '$pageview'"
        query = self._parse_and_transform(original_query)
    ...
    def test_multiple_ctes_transformation(self):
        ...
        # The assertions here are pretty odd, as debug printing of insightsql queries with CTEs does not work.
        node = parse_select(original_query)
        context = InsightsQLContext(team_id=self.team.pk, team=self.team, database=Database.create_for(team=self.team))
        transformed = do_preaggregated_table_transforms(node, context)
...
class TestPreaggregatedTableTransformationIntegration(APIBaseTest, DatastoreTestMixin):
    ...
    def test_basic_insightsql_query(self):
        ...
        response = execute_insightsql_query(
            ...
            team=self.team,
            modifiers=InsightsQLQueryModifiers(usePreaggregatedTableTransforms=True),
        )
        assert response.insightsql and PREAGGREGATED_TABLE_NAME in response.insightsql
        assert response.results == [(1, 1)]

    def test_complex_insightsql_select(self):
        """Test that complex InsightsQL queries are handled correctly."""
        # add a pageview to the combined table, so that we can be sure we are fetching the correct table
        self._insert_stats_row()
    ...
    def test_insightsql_inner_trend(self):
        # execute a insightsql query that roughly matchs the inner query of a trends query
        self._insert_stats_row()
        original_query = """
            SELECT
                count() AS total,
