...
from insights.tasks.alerts.utils import REAL_TIME_CADENCE_MINUTES, WRAPPER_NODE_KINDS, is_non_time_series_trend
...
def _validate_insightsql_alert_config(ctx: _AlertConfigValidationContext) -> None:
    ...
    try:
        ...
        raise ValueError(f"Alert has invalid InsightsQLAlertConfig: {ctx.config}")
    if parsed.evaluation == InsightsQLAlertEvaluation.ANY_ROW and ctx.parsed_condition.type != (
    ...
    if ctx.detector_config is not None and parsed.evaluation == InsightsQLAlertEvaluation.ANY_ROW:
...
_ALERT_CONFIG_VALIDATORS: dict[str, Callable[[_AlertConfigValidationContext], None]] = {
    "InsightsQLAlertConfig": _validate_insightsql_alert_config,
