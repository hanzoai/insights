...
`RelevantCommit` is defined here rather than reused from the generated `insights.schema`: importing
...
class SignalFinding(BaseModel):
    ...
    data_queried: str = Field(
        description=(
            "What Insights MCP queries you ran (e.g. execute-sql, query-run, insight-query) "
...
class RelevantCommit(BaseModel):
    ...
    `insights.schema.RelevantCommit` exactly so the two stay interchangeable on the wire — defined
    here to keep this module off the heavy `insights.schema` import path (see module docstring)."""
