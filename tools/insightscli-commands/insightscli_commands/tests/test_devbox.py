"""Tests for the insightscli devbox commands."""
...
from insightscli_commands.devbox import (
    cli as devbox_cli,
    coder,
...
def devbox_config_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    config_path = tmp_path / "insightscli_devbox.json"
...
class TestDevboxConfig:
    ...
    def test_save_git_identity_persists_trimmed_values(self, devbox_config_path: Path) -> None:
        devbox_config.save_git_identity(" Insights Engineer ", " test-user@example.com ")

        assert devbox_config.load_config() == {
            "git_name": "Insights Engineer",
...
class TestCoderConfig:
    ...
    def test_runtime_ready_requires_setup_when_coder_missing(
        ...
        assert "Run `insightscli devbox:setup`." in capsys.readouterr().out
...
class TestCoderVersion:
    ...
    def test_get_server_version_queries_buildinfo(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("insightscli_commands.devbox.coder.requests.get", return_value=mock_resp) as mock_get:
...
class TestDiagnoseUnreachableCoder:
    ...
    def test_dns_failure_dominates_other_causes(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(coder, "_tailscale_status", lambda: {"CurrentTailnet": {"Name": "hanzo.ai"}})
        ...
        assert "Tailscale tailnet: hanzo.ai" in diagnosis.facts
...
_REPO = "https://github.com/Insights/insights"
...
class TestWorkspaceCreation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"template": "insights-microvm", "preset": "Default (warm)"},
                ["Cold only"],
                "insights-microvm",
                "none",
                {"disk_size": "100", "repo": _REPO, "workspace_region": "us-east-1"},
...
class TestTemplatePresetResolution:
    ...
    def test_resolve_template_preset(
        ...
        result = coder.resolve_template_preset("insights-linux", requested)
...
class TestWorkspaceUpdate:
    ...
    doesn't declare) saved in the user's insightscli config would abort the
    ...
    @pytest.mark.parametrize(
        ...
        [
            lambda: coder.update_workspace(
                "devbox-test-user",
                parameters={"dotfiles_uri": _DOTFILES, "git_name": "Insights Engineer"},
            ),
            lambda: coder.update_workspace_parameters(
                "devbox-test-user",
                {"dotfiles_uri": _DOTFILES, "git_name": "Insights Engineer"},
            ),
        ],
    ...
    def test_update_paths_drop_unknown_params_and_retry(
        ...
        assert retried["git_name"] == "Insights Engineer"
...
class TestDevboxCommands:
    ...
    def test_plain_devbox_command_lists_available_workspace_commands(self) -> None:
        ...
        assert result.exit_code == 0
        assert "insightscli devbox:setup" in result.output
        assert "insightscli devbox:start" in result.output
        assert "insightscli devbox:list" in result.output
        assert "insightscli devbox:restart" in result.output
        assert "insightscli devbox:update" in result.output
        assert "insightscli devbox:destroy" in result.output
    ...
    def test_devbox_start_with_name_creates_labeled_workspace(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert captured["git_name"] == "Insights Engineer"
    ...
    def test_devbox_start_forwards_template_flag(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = runner.invoke(cli, ["devbox:start", "-t", "insights-microvm"])

        assert result.exit_code == 0, result.output
        assert captured["template"] == "insights-microvm"
        assert captured["preset"] == coder.DEFAULT_PRESET
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ["devbox-test-user"],
                "devbox-test-user",
                "insightscli devbox:start --region eu-central-1",
            ),
            (
                "eu-central-1",
    ...
    def test_devbox_forward_fails_early_when_local_port_is_in_use(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert "insightscli devbox:forward --port 8011" in result.output
...
class TestDevboxConfigCommands:
    ...
    def test_show_renders_saved_settings(
        ...
        assert result.exit_code == 0
        assert "Currently configured:" in result.output
        assert "Insights Engineer <engineer@example.com>" in result.output
    ...
    def test_rm_git_identity_clears_only_identity_keys(
        ...
        devbox_config.save_git_identity("Insights Engineer", "engineer@example.com")
        devbox_config.save_dotfiles_uri("https://github.com/user/dotfiles")
...
class TestDevboxSetupGate:
    """Cover the Y/n gate at the top of ``insightscli devbox:setup``."""
...
class TestStartExistingWorkspace:
    ...
    def test_skips_sync_for_running_workspace(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            devbox_cli,
            "load_config",
            lambda: {"git_name": "Insights Engineer", "git_email": "test-user@example.com"},
        )
        monkeypatch.setattr(
            devbox_cli,
            "update_workspace_parameters",
...
class TestCreateTask:
    ...
    def test_create_task_argv(
        ...
        assert captured == [["coder", "task", "create", "--template", "insights-linux", *expected_tail]]

    def test_create_task_argv_uses_selected_template(self, monkeypatch: pytest.MonkeyPatch) -> None:
        captured: list[list[str]] = []
        monkeypatch.setattr(coder, "_run_or_exit", lambda args: captured.append(args))

        coder.create_task("do it", template="insights-microvm")

        assert captured == [["coder", "task", "create", "--template", "insights-microvm", "do it"]]
...
class TestDevboxTaskCommand:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                {"prompt": "fix CI on PR #1234", "task_name": None, "quiet": False, "template": "insights-linux"},
            ),
            (
                ["devbox:task", "--name", "my-task", "-q", "do it"],
                {"prompt": "do it", "task_name": "my-task", "quiet": True, "template": "insights-linux"},
            ),
            (
                ["devbox:task", "-t", "insights-microvm", "do it"],
                {"prompt": "do it", "task_name": None, "quiet": False, "template": "insights-microvm"},
            ),
            (
                ["devbox:task", "--template", "insights-microvm", "do it"],
                {"prompt": "do it", "task_name": None, "quiet": False, "template": "insights-microvm"},
            ),
        ],
    )
    def test_options_forwarded_to_create_task(
        ...
        monkeypatch.setattr(
            devbox_cli,
            "create_task",
            lambda prompt, task_name=None, quiet=False, template="insights-linux": captured.update(
    ...
    def test_piped_stdin_passes_none_as_prompt(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert result.exit_code == 0
        assert captured == {"prompt": None, "task_name": None, "quiet": False, "template": "insights-linux"}
...
class TestMutagenSyncWrappers:
    ...
    def test_sync_create_builds_one_way_safe_argv(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        devbox_mutagen.sync_create(
            ...
            src="/local",
            dst="coder.devbox-test-user:/home/coder/insights",
            config_path=config_path,
            labels={"insightscli-workspace": "devbox-test-user"},
        )
        ...
        assert "--label" in args and args[args.index("--label") + 1] == "insightscli-workspace=devbox-test-user"
        assert args[-2:] == ["/local", "coder.devbox-test-user:/home/coder/insights"]
    ...
    def test_sync_list_returns_empty_when_mutagen_missing(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert devbox_mutagen.sync_list(label_selector="insightscli-workspace=devbox-test-user") == []
    ...
    def test_sync_list_parses_json_template_output(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert devbox_mutagen.sync_list(label_selector="insightscli-workspace=devbox-test-user") == sessions
    ...
    def test_lifecycle_uses_label_selector(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        devbox_mutagen.sync_terminate("insightscli-workspace=devbox-test-user")
        devbox_mutagen.sync_pause("insightscli-workspace=devbox-test-user")
        devbox_mutagen.sync_resume("insightscli-workspace=devbox-test-user")
        devbox_mutagen.sync_flush("insightscli-workspace=devbox-test-user")
        verbs = [args[2] for args in captured]
        assert verbs == ["terminate", "pause", "resume", "flush"]
        for args in captured:
            assert "--label-selector" in args
            assert args[args.index("--label-selector") + 1] == "insightscli-workspace=devbox-test-user"
    ...
    def test_terminate_swallows_no_sessions(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        devbox_mutagen.sync_terminate("insightscli-workspace=missing")
...
class TestKeepaliveShim:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "/Users/John Doe/.insightscli/bin/mutagen daemon run "
                "XPC_SERVICE_NAME=0 MUTAGEN_SSH_PATH=/Users/John Doe/.insightscli/mutagen-ssh-shim FOO=bar\n",
                "/Users/John Doe/.insightscli/mutagen-ssh-shim",
            ),
            (
                "/x/mutagen daemon run MUTAGEN_SSH_PATH=/Users/John Doe/.insightscli/mutagen-ssh-shim\n",
                "/Users/John Doe/.insightscli/mutagen-ssh-shim",
            ),
            (
                "/x/mutagen daemon run MUTAGEN_SSH_PATH=/home/u/.insightscli/mutagen-ssh-shim X=1\n",
                "/home/u/.insightscli/mutagen-ssh-shim",
            ),
...
class TestWorkspaceLabels:
    ...
    def test_labels_and_selector_agree(self) -> None:
        ...
        assert labels == {"insightscli-workspace": "devbox-test-user"}
...
class TestEnsureUserMutagenConfig:
    ...
    def test_copies_defaults_when_absent(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        # hand-maintained `common/insightsql_parser` C++ sources.
        assert "'.git'" in contents
        assert "'.flox/cache'" in contents
        assert "\n                - '.flox'\n" not in contents
        assert "'common/insightsql_parser'" not in contents
...
class TestDevboxSyncCommand:
    ...
    def test_json_emits_summary_with_true_conflict_total(self, monkeypatch: pytest.MonkeyPatch) -> None:
            ...
            lambda label_selector=None: [
                {
                    ...
                    "alpha": {"path": "/local"},
                    "beta": {"path": "coder.devbox-test-user:/home/coder/insights"},
                }
            ],
        )
        ...
        assert session["beta"] == "coder.devbox-test-user:/home/coder/insights"
    ...
    def test_terminate_invokes_label_scoped_terminate(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert result.exit_code == 0, result.output
        assert captured == ["insightscli-workspace=devbox-test-user"]
    ...
    def test_default_run_creates_one_way_sync(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        checkout = tmp_path / "insights"
        checkout.mkdir()
        monkeypatch.setattr(devbox_sync, "_detect_local_insights_checkout", lambda: checkout)
        ...
        assert captured["dst"] == "coder.devbox-test-user:/home/coder/insights"
        assert captured["labels"] == {"insightscli-workspace": "devbox-test-user"}
    ...
    def test_detect_local_checkout_walks_up_from_cwd(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        (checkout / "insightscli.yaml").write_text("")
        (checkout / ".git").mkdir()
        monkeypatch.chdir(checkout / "subdir")
        assert devbox_sync._detect_local_insights_checkout() == checkout.resolve()

    def test_detect_local_checkout_fails_outside_repo(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.chdir(tmp_path)
        with pytest.raises(SystemExit):
            devbox_sync._detect_local_insights_checkout()
...
class TestDevboxLifecycleSyncIntegration:
    ...
    def test_start_prints_sync_tip_when_no_session(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert result.exit_code == 0, result.output
        assert "insightscli devbox:sync" in result.output
    ...
    def test_start_omits_sync_tip_when_session_exists(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert result.exit_code == 0, result.output
        assert "Tip: run `insightscli devbox:sync" not in result.output
