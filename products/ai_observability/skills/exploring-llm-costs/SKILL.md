...
description: >
  Investigate LLM spend in Insights — total cost over time, cost by model,
...
Insights attaches per-call cost metadata to every `$ai_generation` and `$ai_embedding`
...
`insights:read-data-schema` before grouping. Don't guess names.
...
for ready-to-run JSON for `insights:insight-create`, `insights:dashboard-create`,
and `insights:alert-create`.

## Constructing UI links

- **Dashboard**: `https://app.hanzo.ai/ai-observability/dashboard`
- **Traces list** (sort by cost): `https://app.hanzo.ai/ai-observability/traces`
- **Generations list**: `https://app.hanzo.ai/ai-observability/generations`
- **Users list** (per-user cost): `https://app.hanzo.ai/ai-observability/users`
- **Single trace**: `https://app.hanzo.ai/ai-observability/traces/<trace_id>?timestamp=<url_encoded_iso>`
...
- For anything not covered here (new cost categories, changes to
  pricing lookup, provider additions), run `insights:docs-search` for
...
- Always set a time range — cost queries without one scan the full events table
- Token, cost, model, and `$ai_trace_id` properties are on `events` — but message _content_ (`$ai_input` / `$ai_output_choices`) lives only on the `insights.ai_events` table; see the traces skill's [event reference](../exploring-llm-traces/references/events-and-properties.md) if you need content alongside cost
- Always include `$ai_embedding` alongside `$ai_generation` when summing cost; embeddings are cheap per-call but add up at scale
- Costs are written at ingestion (see [Calculating LLM costs](https://hanzo.ai/docs/ai-observability/calculating-costs)) — if `$ai_total_cost_usd` is missing or zero, read `$ai_cost_model_source` first: `passthrough` means the SDK supplied costs; `custom` means custom token prices; `openrouter` / `manual` mean automatic lookup; missing means the model wasn't matched (unusual custom model, fine-tune). Grep: `countIf(properties.$ai_total_cost_usd IS NULL)` per `(model, source)`
