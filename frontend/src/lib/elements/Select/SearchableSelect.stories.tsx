...
import { SearchableSelect, SearchableSelectProps, SelectOptions } from '@hanzo/elements'
...
const Template: StoryFn<typeof SearchableSelect> = (props: SearchableSelectProps<any>) => {
    return (
        <div className="flex flex-row items-center w-full border p-4 gap-2">
            {(['small', 'medium', 'large', undefined] as const).map((size, index) => (
                <div className="flex flex-col" key={index}>
                    ...
                    <SearchableSelect {...props} size={size} />
                </div>
            ))}
        </div>
    )
