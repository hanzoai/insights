...
from insights.storage.llm_prompt_cache import (
...
from insights.storage.llm_prompt_cache_keys import prompt_label_cache_key, prompt_latest_cache_key
...
class TestLLMPromptCache(BaseTest):
    ...
    def test_transient_db_error_on_read_degrades_to_miss(self, _name, error_cls):
        ...
        with patch(
            "insights.storage.llm_prompt_cache._get_latest_prompt_from_db",
    ...
    def test_load_fn_reports_and_reraises_transient_db_error_as_dependency_unavailable(self, _name, error_cls, label):
        ...
        with (
            patch("insights.utils.capture_exception") as mock_capture,
            patch(f"insights.storage.llm_prompt_cache.{db_fn}", side_effect=error_cls("query_wait_timeout")),
        ):
...
class TestLLMPromptCacheSignals(BaseTest):
    def test_model_signal_invalidates_latest_and_exact_version_caches_on_commit(self):
        with (
            ...
            patch("insights.storage.llm_prompt_cache.invalidate_prompt_latest_cache") as mock_invalidate_latest,
            patch("insights.storage.llm_prompt_cache.invalidate_prompt_version_cache") as mock_invalidate_version,
        ):
    ...
    def test_label_cache_never_touches_object_storage(self):
        ...
        with patch("insights.storage.hypercache.object_storage") as mock_storage:
