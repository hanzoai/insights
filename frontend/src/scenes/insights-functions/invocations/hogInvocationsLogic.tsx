...
import { escapeInsightsQLString, insightsql } from '~/queries/utils'
 ...
 * cyclotron script queue) can actually run. Other types run elsewhere — source webhooks inline
 ...
 */
export const RERUNNABLE_FN_FUNCTION_TYPES: InsightsFunctionTypeType[] = ['destination', 'internal_destination']
...
const scheduleAutoRefresh = (
    ...
    cache.disposables.add(() => {
        const timeoutId = setTimeout(() => {
            ...
            // heavy aggregation on Datastore. Re-arm so polling resumes once it settles.
...
const parseRelativeHours = (value: string | undefined): number | null => {
    ...
    // Insights convention: uppercase `M` = minutes (lowercase `m` = month). See
...
export const dateClauseFor = (filters: HogInvocationsFilters): ReturnType<typeof insightsql.raw> => {
    ...
    // offset for any non-UTC project. Mirrors `toAbsoluteDatastoreTimestamp`.
    ...
    return insightsql.raw(`AND scheduled_at >= '${fmt(start)}' AND scheduled_at < '${fmt(end)}'`)
}
...
export const kindClauseFor = (
    ...
    if (filters.kind === 'invocations') {
        return insightsql.raw(`function_kind = '${props.functionKind}'`)
    }
    if (filters.kind === 'rerun_jobs') {
        return insightsql.raw(`function_kind = '${wrapperKind}'`)
    }
    return insightsql.raw(`function_kind IN ('${props.functionKind}', '${wrapperKind}')`)
}
...
export const problemClauseFor = (
    ...
    return insightsql.raw(
        `AND invocation_id IN (` +
            ...
            `WHERE log_source = ${escapeInsightsQLString(props.functionKind)} ` +
            `AND log_source_id = ${escapeInsightsQLString(props.id)} ` +
...
export const buildSearchClause = (
    ...
    const levelClause = levels.length
        ? `AND lower(level) IN (${levels.map((level) => escapeInsightsQLString(level.toLowerCase())).join(',')})`
        : ''
    // Escape ILIKE wildcards for the message arm so a term with % or _ (e.g. "50%") matches literally
    // (Datastore ILIKE uses backslash as its escape char); the exact-id arms use the raw term.
    const likeTerm = search.replace(/[\\%_]/g, '\\$&')
    return insightsql.raw(
        `AND (` +
            `invocation_id = ${escapeInsightsQLString(search)} ` +
            `OR event_uuid = ${escapeInsightsQLString(search)} ` +
            `OR distinct_id = ${escapeInsightsQLString(search)} ` +
            `OR person_id = ${escapeInsightsQLString(search)} ` +
            `OR invocation_id IN (` +
            `SELECT instance_id FROM log_entries ` +
            `WHERE log_source = ${escapeInsightsQLString(props.functionKind)} ` +
            `AND log_source_id = ${escapeInsightsQLString(props.id)} ` +
            `AND message ILIKE concat('%', ${escapeInsightsQLString(likeTerm)}, '%') ` +
            `${levelClause}))`
    )
}

/**
 * Tier selection for the sparkline. Each tier carries both the InsightsQL bucket
...
async function fetchSparkline(props: HogInvocationsLogicProps, filters: HogInvocationsFilters): Promise<SparklineData> {
    ...
    // `escapeInsightsQLString` handles all special chars (quotes, backslashes, null
    // bytes) using the same path as the `insightsql` template tag — a `.replace(/'/g, …)`
    // pass on its own is bypassable (e.g. `\' OR 1=1 --`).
    const optionalStatusClause = filters.status?.length
        ? insightsql.raw(`AND status IN (${filters.status.map(escapeInsightsQLString).join(',')})`)
        : insightsql.raw('')
    const optionalErrorKindClause = filters.error_kind?.length
        ? insightsql.raw(`AND error_kind IN (${filters.error_kind.map(escapeInsightsQLString).join(',')})`)
        : insightsql.raw('')
    ...
    const optionalPersonClause = filters.person_uuid
        ? insightsql.raw(`AND person_id = ${escapeInsightsQLString(filters.person_uuid)}`)
        : insightsql.raw('')
    ...
    const query = insightsql`
        SELECT
            ${insightsql.raw(bucketExpr)} AS bucket,
            status,
            count() AS n
        FROM (
            SELECT
...
async function fetchRunsPage(
    ...
    // named `scheduled_at` — otherwise InsightsQL substitutes the alias and produces
    // `max(max(scheduled_at))`.
    const orderClause =
        filters.order_by === 'first_scheduled'
            ? insightsql.raw('ORDER BY argMax(first_scheduled_at, version) DESC, invocation_id DESC')
            : insightsql.raw('ORDER BY max(scheduled_at) DESC, invocation_id DESC')
    const kindClause = kindClauseFor(props, filters)
    const dateClause = dateClauseFor(filters)
    const query = insightsql`
        SELECT
...
async function fetchProblemLevels(
    ...
    const idClause = insightsql.raw(`instance_id IN (${ids.map(escapeInsightsQLString).join(',')})`)
    const severityQuery = insightsql`
        ...
        personPropertiesById: [
            {} as Record<string, { properties: Record<string, any>; distinct_ids?: string[] }>,
            {
                hydratePeople: async ({ personIds }, breakpoint) => {
                    ...
                    await breakpoint(50)
                    const idList = insightsql.raw(toFetch.map((id) => `toUUID('${id}')`).join(','))
                    const query = insightsql`
    ...
    listeners(({ props, actions, values, cache }) => ({
        ...
        rerunInvocations: async ({ invocationIds }) => {
            ...
            try {
                ...
                toast.error(`Failed to enqueue rerun: ${e?.detail ?? e?.message ?? String(e)}`)
            }
        },
        bulkRerun: async ({ params }) => {
            ...
            try {
                ...
                toast.error(`Failed to enqueue re-run: ${e?.detail ?? e?.message ?? String(e)}`)
            }
        },
    })),
