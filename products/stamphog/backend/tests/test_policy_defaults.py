...
def test_overlay_absent_repo_policy_keeps_default_text_verbatim() -> None:
    ...
    assert effective[STAMPFN_POLICY_ENTRYPOINT] == _DEFAULT_POLICY.read_text()
    assert STAMPFN_STEERING_PATH not in effective


def test_overlay_partial_repo_policy_replaces_only_declared_sections() -> None:
    effective = _effective_policy_files("acme/widgets", {STAMPFN_POLICY_ENTRYPOINT: _SIZE_GATE_ONLY})
    merged = yaml.safe_load(effective[STAMPFN_POLICY_ENTRYPOINT])
...
def test_overlay_full_schema_repo_policy_is_itself() -> None:
    ...
    effective = _effective_policy_files("acme/widgets", {STAMPFN_POLICY_ENTRYPOINT: full})
    assert yaml.safe_load(effective[STAMPFN_POLICY_ENTRYPOINT]) == yaml.safe_load(full)
...
def test_overlay_unusable_repo_policy_fails_closed(repo_policy: str) -> None:
    ...
    with pytest.raises(RuntimeError):
        _effective_policy_files("acme/widgets", {STAMPFN_POLICY_ENTRYPOINT: repo_policy})
...
def test_overlay_of_partial_repo_policy_validates_through_engine_loader(tmp_path: Path) -> None:
    ...
    merged_path.write_text(effective[STAMPFN_POLICY_ENTRYPOINT])
...
def test_repo_guidance_and_steering_pass_through() -> None:
    files = {
        STAMPFN_REVIEW_GUIDANCE_PATH: "repo norms\n",
        STAMPFN_STEERING_PATH: "steer this way\n",
    }
    effective = _effective_policy_files("acme/widgets", files)
    assert effective[STAMPFN_REVIEW_GUIDANCE_PATH] == "repo norms\n"
    assert effective[STAMPFN_STEERING_PATH] == "steer this way\n"
