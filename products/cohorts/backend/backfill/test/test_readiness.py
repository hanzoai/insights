...
class TestBackfillReadiness(BaseTest):
    ...
    def test_stamp_uses_one_conditional_cohort_update(self) -> None:
        ...
        cohort_updates = [query["sql"] for query in queries if query["sql"].startswith('UPDATE "insights_cohort"')]
    ...
    def test_supersession_racing_the_stamp_rolls_the_cohort_back(self) -> None:
        ...
        def supersede_after_the_cohort_stamp(execute, sql, params, many, context):
            ...
            if not superseded and sql.startswith('UPDATE "insights_cohort" '):
