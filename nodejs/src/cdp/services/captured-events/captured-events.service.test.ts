...
import { CyclotronJobInvocationResult, InsightsFunctionCapturedEvent } from '../../types'
...
const buildCapturedEvent = (
    team_id: number,
    overrides: Partial<InsightsFunctionCapturedEvent> = {}
...
const buildResult = (capturedEvents: InsightsFunctionCapturedEvent[]): CyclotronJobInvocationResult =>
    ({ capturedInsightsEvents: capturedEvents }) as unknown as CyclotronJobInvocationResult
...
describe('CapturedEventsService', () => {
    ...
    describe('queueInvocationResults', () => {
        it('extracts capturedInsightsEvents and resolves api_token via teamManager', async () => {
        ...
        it('skips results with no captured events', async () => {
            await service.queueInvocationResults([
                { capturedInsightsEvents: [] } as unknown as CyclotronJobInvocationResult,
                { capturedInsightsEvents: undefined } as unknown as CyclotronJobInvocationResult,
            ])
            await service.flush()
