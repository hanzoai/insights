# Generated by Django 5.2.14 on 2026-05-20 15:18

import django.db.models.deletion
import django.db.models.expressions
from django.conf import settings
from django.db import migrations, models

import insights.schema

import insights.models.utils


class Migration(migrations.Migration):
    initial = True

    dependencies = [
        ("notebooks", "0003_add_kernel_timeouts"),
        ("insights", "1163_alter_batchexportrun_batch_export_and_more"),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.SeparateDatabaseAndState(
            state_operations=[
                migrations.CreateModel(
                    name="Alert",
                    fields=[
                        (
                            "id",
                            models.BigAutoField(
                                auto_created=True,
                                primary_key=True,
                                serialize=False,
                                verbose_name="ID",
                            ),
                        ),
                        ("name", models.CharField(max_length=100)),
                        ("target_value", models.TextField()),
                        ("anomaly_condition", models.JSONField(default=dict)),
                        (
                            "insight",
                            models.ForeignKey(
                                on_delete=django.db.models.deletion.CASCADE,
                                to="insights.insight",
                            ),
                        ),
                        (
                            "team",
                            models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to="insights.team"),
                        ),
                    ],
                    options={
                        "db_table": "insights_alert",
                    },
                ),
                migrations.CreateModel(
                    name="AlertConfiguration",
                    fields=[
                        ("created_at", models.DateTimeField(auto_now_add=True)),
                        (
                            "id",
                            models.UUIDField(
                                default=insights.models.utils.UUIDT,
                                editable=False,
                                primary_key=True,
                                serialize=False,
                            ),
                        ),
                        ("name", models.CharField(blank=True, max_length=255)),
                        ("config", models.JSONField(blank=True, default=dict, null=True)),
                        (
                            "calculation_interval",
                            models.CharField(
                                blank=True,
                                choices=[
                                    (
                                        insights.schema.AlertCalculationInterval["EVERY_15_MINUTES"],
                                        "every_15_minutes",
                                    ),
                                    (
                                        insights.schema.AlertCalculationInterval["HOURLY"],
                                        "hourly",
                                    ),
                                    (insights.schema.AlertCalculationInterval["DAILY"], "daily"),
                                    (
                                        insights.schema.AlertCalculationInterval["WEEKLY"],
                                        "weekly",
                                    ),
                                    (
                                        insights.schema.AlertCalculationInterval["MONTHLY"],
                                        "monthly",
                                    ),
                                ],
                                default=insights.schema.AlertCalculationInterval["DAILY"],
                                max_length=20,
                                null=True,
                            ),
                        ),
                        ("condition", models.JSONField(default=dict)),
                        ("detector_config", models.JSONField(blank=True, null=True)),
                        (
                            "state",
                            models.CharField(
                                choices=[
                                    (
                                        insights.schema.AlertState["FIRING"],
                                        insights.schema.AlertState["FIRING"],
                                    ),
                                    (
                                        insights.schema.AlertState["NOT_FIRING"],
                                        insights.schema.AlertState["NOT_FIRING"],
                                    ),
                                    (
                                        insights.schema.AlertState["ERRORED"],
                                        insights.schema.AlertState["ERRORED"],
                                    ),
                                    (
                                        insights.schema.AlertState["SNOOZED"],
                                        insights.schema.AlertState["SNOOZED"],
                                    ),
                                ],
                                default=insights.schema.AlertState["NOT_FIRING"],
                                max_length=10,
                            ),
                        ),
                        ("enabled", models.BooleanField(default=True)),
                        ("last_notified_at", models.DateTimeField(blank=True, null=True)),
                        ("last_checked_at", models.DateTimeField(blank=True, null=True)),
                        ("next_check_at", models.DateTimeField(blank=True, null=True)),
                        ("snoozed_until", models.DateTimeField(blank=True, null=True)),
                        (
                            "skip_weekend",
                            models.BooleanField(blank=True, default=False, null=True),
                        ),
                        (
                            "schedule_restriction",
                            models.JSONField(blank=True, default=None, null=True),
                        ),
                        ("investigation_agent_enabled", models.BooleanField(default=False)),
                        (
                            "investigation_gates_notifications",
                            models.BooleanField(default=False),
                        ),
                        (
                            "investigation_inconclusive_action",
                            models.CharField(
                                choices=[("notify", "Notify"), ("suppress", "Suppress")],
                                default="notify",
                                max_length=10,
                            ),
                        ),
                        (
                            "created_by",
                            models.ForeignKey(
                                blank=True,
                                null=True,
                                on_delete=django.db.models.deletion.SET_NULL,
                                to=settings.AUTH_USER_MODEL,
                            ),
                        ),
                        (
                            "insight",
                            models.ForeignKey(
                                on_delete=django.db.models.deletion.CASCADE,
                                to="insights.insight",
                            ),
                        ),
                        (
                            "team",
                            models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to="insights.team"),
                        ),
                    ],
                    options={
                        "db_table": "insights_alertconfiguration",
                    },
                ),
                migrations.CreateModel(
                    name="AlertCheck",
                    fields=[
                        (
                            "id",
                            models.UUIDField(
                                default=insights.models.utils.UUIDT,
                                editable=False,
                                primary_key=True,
                                serialize=False,
                            ),
                        ),
                        ("created_at", models.DateTimeField(auto_now_add=True)),
                        ("calculated_value", models.FloatField(blank=True, null=True)),
                        ("condition", models.JSONField(default=dict)),
                        ("targets_notified", models.JSONField(default=dict)),
                        ("error", models.JSONField(blank=True, null=True)),
                        (
                            "state",
                            models.CharField(
                                choices=[
                                    (
                                        insights.schema.AlertState["FIRING"],
                                        insights.schema.AlertState["FIRING"],
                                    ),
                                    (
                                        insights.schema.AlertState["NOT_FIRING"],
                                        insights.schema.AlertState["NOT_FIRING"],
                                    ),
                                    (
                                        insights.schema.AlertState["ERRORED"],
                                        insights.schema.AlertState["ERRORED"],
                                    ),
                                    (
                                        insights.schema.AlertState["SNOOZED"],
                                        insights.schema.AlertState["SNOOZED"],
                                    ),
                                ],
                                default=insights.schema.AlertState["NOT_FIRING"],
                                max_length=10,
                            ),
                        ),
                        ("anomaly_scores", models.JSONField(blank=True, null=True)),
                        ("triggered_points", models.JSONField(blank=True, null=True)),
                        ("triggered_dates", models.JSONField(blank=True, null=True)),
                        ("interval", models.CharField(blank=True, max_length=10, null=True)),
                        ("triggered_metadata", models.JSONField(blank=True, null=True)),
                        (
                            "investigation_status",
                            models.CharField(
                                blank=True,
                                choices=[
                                    ("pending", "pending"),
                                    ("running", "running"),
                                    ("done", "done"),
                                    ("failed", "failed"),
                                    ("skipped", "skipped"),
                                ],
                                max_length=10,
                                null=True,
                            ),
                        ),
                        (
                            "investigation_verdict",
                            models.CharField(
                                blank=True,
                                choices=[
                                    ("true_positive", "true_positive"),
                                    ("false_positive", "false_positive"),
                                    ("inconclusive", "inconclusive"),
                                ],
                                max_length=20,
                                null=True,
                            ),
                        ),
                        ("investigation_summary", models.TextField(blank=True, null=True)),
                        ("investigation_error", models.JSONField(blank=True, null=True)),
                        ("notification_sent_at", models.DateTimeField(blank=True, null=True)),
                        (
                            "notification_suppressed_by_agent",
                            models.BooleanField(default=False),
                        ),
                        (
                            "investigation_notebook",
                            models.ForeignKey(
                                blank=True,
                                null=True,
                                on_delete=django.db.models.deletion.SET_NULL,
                                related_name="+",
                                to="notebooks.notebook",
                            ),
                        ),
                        (
                            "alert_configuration",
                            models.ForeignKey(
                                on_delete=django.db.models.deletion.CASCADE,
                                to="alerts.alertconfiguration",
                            ),
                        ),
                    ],
                    options={
                        "db_table": "insights_alertcheck",
                    },
                ),
                migrations.CreateModel(
                    name="AlertSubscription",
                    fields=[
                        ("created_at", models.DateTimeField(auto_now_add=True)),
                        (
                            "id",
                            models.UUIDField(
                                default=insights.models.utils.UUIDT,
                                editable=False,
                                primary_key=True,
                                serialize=False,
                            ),
                        ),
                        ("subscribed", models.BooleanField(default=True)),
                        (
                            "alert_configuration",
                            models.ForeignKey(
                                on_delete=django.db.models.deletion.CASCADE,
                                to="alerts.alertconfiguration",
                            ),
                        ),
                        (
                            "created_by",
                            models.ForeignKey(
                                blank=True,
                                null=True,
                                on_delete=django.db.models.deletion.SET_NULL,
                                to=settings.AUTH_USER_MODEL,
                            ),
                        ),
                        (
                            "user",
                            models.ForeignKey(
                                limit_choices_to={
                                    "is_active": True,
                                    "organization_id": django.db.models.expressions.OuterRef(
                                        "alert_configuration__team__organization_id"
                                    ),
                                },
                                on_delete=django.db.models.deletion.CASCADE,
                                related_name="alert_subscriptions",
                                to=settings.AUTH_USER_MODEL,
                            ),
                        ),
                    ],
                    options={
                        "db_table": "insights_alertsubscription",
                        "unique_together": {("user", "alert_configuration")},
                    },
                ),
                migrations.AddField(
                    model_name="alertconfiguration",
                    name="subscribed_users",
                    field=models.ManyToManyField(
                        related_name="alert_configurations",
                        through="alerts.AlertSubscription",
                        through_fields=("alert_configuration", "user"),
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
                migrations.CreateModel(
                    name="Threshold",
                    fields=[
                        ("created_at", models.DateTimeField(auto_now_add=True)),
                        (
                            "id",
                            models.UUIDField(
                                default=insights.models.utils.UUIDT,
                                editable=False,
                                primary_key=True,
                                serialize=False,
                            ),
                        ),
                        ("name", models.CharField(blank=True, max_length=255)),
                        ("configuration", models.JSONField(default=dict)),
                        (
                            "created_by",
                            models.ForeignKey(
                                blank=True,
                                null=True,
                                on_delete=django.db.models.deletion.SET_NULL,
                                to=settings.AUTH_USER_MODEL,
                            ),
                        ),
                        (
                            "insight",
                            models.ForeignKey(
                                on_delete=django.db.models.deletion.CASCADE,
                                to="insights.insight",
                            ),
                        ),
                        (
                            "team",
                            models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to="insights.team"),
                        ),
                    ],
                    options={
                        "db_table": "insights_threshold",
                    },
                ),
                migrations.AddField(
                    model_name="alertconfiguration",
                    name="threshold",
                    field=models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        to="alerts.threshold",
                    ),
                ),
            ],
            database_operations=[],
        ),
    ]
