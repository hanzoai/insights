...
export function createInsightTooltipAccessor(element: HTMLElement): InsightTooltipAccessor {
    const isDefaultTooltip = (): boolean => !!element.querySelector('[data-attr="script-chart-tooltip-label"]')
    ...
    return Object.assign(createHogChartTooltip(element), {
        ...
        rows(): string[] {
            if (isDefaultTooltip()) {
                return defaultRows()
                    .map((r) => r.querySelector('[data-attr="script-chart-tooltip-series"]')?.textContent?.trim() ?? '')
                    .filter(Boolean)
            }
            const rows = element.querySelectorAll('tbody tr')
