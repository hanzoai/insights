...
class TestPostSlackUpdate(TestCase):
    ...
    def test_completed_run_with_pr_routes_through_post_pr_opened(
        ...
        mock_update_reaction.assert_called_once_with("mascot")
    ...
    def test_in_progress_run_posts_stage(self, mock_task_run_class, mock_handler_init, mock_post_progress):
        ...
        # mobile) rather than a desktop-only ``insights-code://`` deep link.
    ...
    def test_in_progress_with_pr_keeps_eyes_reaction(
        ...
        # Task is still running (PR opened mid-run) — reaction stays :eyes:, not :mascot:.
    ...
    @patch.object(SlackThreadHandler, "__init__", return_value=None)
    @patch("products.tasks.backend.models.TaskRun")
    def test_user_without_insights_code_access_omits_task_url(
        self,
        mock_task_run_class,
        _mock_handler_init,
        ...
        # When the task creator is not a Insights Desktop user, every handler call
