...
from products.data_warehouse.backend.datastore_helpers import (
    datastore_schema_metadata_to_dwh_columns,
...
from products.warehouse_sources.backend.facade.insightsql import insightsql_fields_and_structure_for_columns
...
def build_direct_table_for_schema(schema: "ExternalDataSchema", source: "ExternalDataSource") -> DirectSQLTable | None:
    ...
    if engine == "postgres":
        ...
        fields, _ = insightsql_fields_and_structure_for_columns(columns)
    ...
    if engine == "datastore":
        columns = datastore_schema_metadata_to_dwh_columns(metadata)
        if not columns:
            return None
        columns = filter_dwh_columns_by_enabled_columns(
            columns,
        ...
        fields, _ = insightsql_fields_and_structure_for_columns(columns)
        datastore_database, table_name = get_datastore_source_location(
            schema_name=schema.name,
            schema_metadata=metadata,
        ...
        return DirectDatastoreTable(
            ...
            datastore_table_name=table_name,
            external_data_source_id=str(source.id),
            connection_metadata=source.connection_metadata,
