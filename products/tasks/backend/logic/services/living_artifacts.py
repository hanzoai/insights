...
# Both scopes are still in Slack app review (see insights/helpers/slack_scopes.py), so the canvas
...
def _require_shareable_source_artifact(artifact: dict[str, Any]) -> dict[str, Any]:
    ...
    # uploads). Living artifacts are deliverables that leave Insights, so only files
...
def _slack_integration_for_mapping(mapping: Any):
    from insights.models.integration import SlackIntegration  # noqa: PLC0415
