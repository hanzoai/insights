...
from insights.plugins.plugin_server_api import reload_hog_flows_on_workers, reschedule_hog_flow_parked_jobs
...
def refresh_affected_hog_flows(team_id: Optional[int] = None, action_id: Optional[int] = None) -> int:
    affected_hog_flows: list[InsightsFlow] = []
    ...
    updates = InsightsFlow.objects.bulk_update(affected_hog_flows, ["updated_at"])
...
def reschedule_hog_flow_timing(
    ...
    if not InsightsFlow.objects.filter(team_id=team_id, id=hog_flow_id, status="active").exists():
        logger.info(
