...
from insights.slo.context import (
    SLO_REPO_ROOT,
    SloSpec,
    _build_error_origin,
    get_current_slo,
...
from insights.slo.types import SloArea, SloCompletedProperties, SloOperation, SloOutcome, SloStartedProperties
...
def test_slo_handle_tag_none_unsets_existing_key() -> None:
    from insights.slo.context import SloHandle
...
def test_build_error_origin_prefers_deepest_repo_frame() -> None:
    repo_frame = traceback.FrameSummary(
        filename=str(SLO_REPO_ROOT / "insights" / "tasks" / "alerts" / "checks.py"),
        lineno=123,
        name="check_alert",
    )
    ...
    with patch("insights.slo.context.traceback.extract_tb", return_value=[repo_frame, external_frame]):
...
@patch("insights.slo.context.emit_slo_completed")
@patch("insights.slo.context.emit_slo_started")
@patch("insights.slo.context.random.random")
...
@patch("insights.slo.context.emit_slo_completed")
@patch("insights.slo.context.emit_slo_started")
@patch("insights.slo.context.random.random", return_value=0.99)
