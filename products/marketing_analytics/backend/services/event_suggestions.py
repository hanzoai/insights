...
def _fetch_candidate_rows(team: Team, *, min_count: int, lookback_days: int) -> list[dict[str, Any]]:
    ...
    the per-event top-K utm_source via Datastore's `topK` aggregate."""
    since = timezone.now() - timedelta(days=lookback_days)
    aggregate_insightsql = """
        SELECT
            event,
            count() AS c,
    ...
    with tags_context(product=Product.MARKETING_ANALYTICS, feature=Feature.HEALTH_CHECK, team_id=team.pk):
        aggregated = execute_insightsql_query(aggregate_insightsql, team, placeholders=placeholders)
