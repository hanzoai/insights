...
class TestRunTaggerWorkflow:
    ...
    async def test_execute_tagger_activity(self, setup_data):
        ...
        with patch("insights.temporal.ai_observability.run_tagger.Client") as mock_client_class:
            ...
            with patch("insights.temporal.ai_observability.model_resolution.EvaluationConfig") as mock_eval_config:
    ...
    async def test_emit_tagger_event_activity(
        ...
        with patch("insights.temporal.ai_observability.run_tagger.Team.objects.get") as mock_team_get:
            with patch("insights.temporal.ai_observability.run_tagger.capture_internal") as mock_capture:
                ...
                if llm_props_present:
                    ...
                    assert llm_keys.isdisjoint(set(props)), (
                        f"Script tagger event leaked LLM-only props: {llm_keys & set(props)}"
                    )
    ...
    async def test_emit_tagger_event_activity_billing_limit(self, setup_data, status_code: int, should_raise: bool):
        ...
        with patch("insights.temporal.ai_observability.run_tagger.Team.objects.get", return_value=team):
            with patch("insights.temporal.ai_observability.run_tagger.capture_internal", return_value=capture_result):
                if should_raise:
    ...
    async def test_disable_tagger_activity(self, setup_data):
        ...
        with patch("insights.plugins.plugin_server_api.reload_taggers_on_workers"):
