...
class TestAttachLocalPackageMounts:
    ...
    def test_installs_runtime_dependencies_before_mounting_local_builds(self, tmp_path: Path) -> None:
        ...
        package = LocalPackage(
            ...
            sandbox_install_path="/scripts/node_modules/@hanzo/agent",
        )
        ...
        shared_package = LocalPackage(
            ...
            sandbox_install_path="/scripts/node_modules/@hanzo/shared",
        )
        ...
        with patch(
            "products.tasks.backend.logic.services.modal_sandbox.get_local_insights_code_packages",
    ...
    def test_start_agent_server_raises_on_health_check_failure(self, mock_sandbox: Any):
        mock_sandbox.execute = MagicMock(
            side_effect=[
                ...
                ExecutionResult(stdout="", stderr="", exit_code=0, error=None),  # --insightsExecPermissionRegex probe
...
class TestModalSandboxCommandEscaping:
    ...
    def test_clone_repository_shallow_flag(self, shallow, expected_in_command, not_expected_in_command):
        ...
        with patch.object(sandbox, "is_running", return_value=True):
            with patch.object(sandbox, "execute") as mock_execute:
                sandbox.clone_repository("Insights/insights", github_token="test-token", shallow=shallow)
                command = mock_execute.call_args[0][0]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("Insights/insights", "task-123", "run-456", "background"),
            ("org/repo; echo hacked", "task-123", "run-456", "background"),
            ("Insights/insights", "task; echo hacked", "run-456", "background"),
            ("Insights/insights", "task-123", "run$(whoami)", "background"),
            ("Insights/insights", "task-123", "run-456", "mode`id`"),
        ],
    )
...
class TestStartupFailureDiagnostics:
    ...
    @override_settings(SITE_URL="https://eu.hanzo.ai", SANDBOX_MCP_URL=None)
    def test_reports_blocked_egress_host(self):
        ...
        assert "mcp-eu.hanzo.ai" in diagnostics["failure_reason"]
    ...
    def test_reports_alive_without_session_when_no_block(self):
        ...
        def _exec(command: str, timeout_seconds: Any = None) -> ExecutionResult:
            if "printf" in command:
                return ExecutionResult(
                    stdout="gateway.us.hanzo.ai http_code=200", stderr="", exit_code=0, error=None
                )
...
class TestModalSandboxCreateAllowlist:
    ...
    def test_create_forwards_exact_outbound_domain_allowlist(self):
        domains = ["github.com", "api.github.com", "example.com", "*.hanzo.ai", "api.anthropic.com"]
...
class TestModalSandboxCreateImageFallback:
    ...
    def test_custom_image_overlay_failure_falls_back_to_bare_custom_image(self):
        ...
        config = SandboxConfig(name="t", template=SandboxTemplate.VM_BASE, custom_image_name="insights-dev-stack")
        ...
        assert "custom image insights-dev-stack" in sandbox.config.image_fallback
    ...
    def test_wedged_snapshot_restore_recovers_through_custom_image_and_records_fallback(self):
        ...
        config = SandboxConfig(
            ...
            custom_image_name="insights-dev-stack",
    ...
    def test_wedged_dev_stack_boot_is_probed_and_falls_back_to_base(self):
        ...
        assert sandbox.config.image_fallback is not None
        assert "custom image insights-dev-stack (unresponsive after restore)" in sandbox.config.image_fallback
    ...
    def test_spec_built_custom_images_are_not_probed(self):
        ...
        config = SandboxConfig(
            name="t", template=SandboxTemplate.VM_BASE, custom_image_name="insights-sandbox-custom-2-abc:latest"
        )
...
class TestLaunchDevStackBootstrap:
    def _sandbox(
        ...
        *,
        vm: bool = True,
        custom_image_name: str | None = "insights-dev-stack",
    ...
    @pytest.mark.parametrize("custom_image_name", [None, "insights-sandbox-custom-2-abc123:latest"])
...
class TestModalSandboxCreateSnapshot:
    ...
    def test_publish_filesystem_image_uses_published_image_snapshot_timeout(self, mock_sandbox: Any):
        ...
        assert mock_sandbox.publish_filesystem_image("insights-dev-stack") == "im-123"

        mock_sandbox._sandbox.snapshot_filesystem.assert_called_once_with(
            timeout=PUBLISHED_IMAGE_SNAPSHOT_TIMEOUT_SECONDS, ttl=None
        )
        image.publish.assert_called_once_with("insights-dev-stack")
...
class TestSessionInitProbeHosts:
    @pytest.mark.parametrize(
        ...
        [
            ("https://us.hanzo.ai", None, "mcp.hanzo.ai", "mcp-eu.hanzo.ai"),
            ("https://eu.hanzo.ai", None, "mcp-eu.hanzo.ai", "mcp.hanzo.ai"),
            (
                "https://us.hanzo.ai",
    ...
    @override_settings(
        SANDBOX_LLM_GATEWAY_URL="https://gateway.dev.insights.dev",
        SANDBOX_AI_GATEWAY_URL="https://ai-gateway.dev.insights.dev",
    )
    def test_includes_both_configured_gateway_hosts(self):
        ...
        hosts = _session_init_probe_hosts()
        assert "gateway.dev.insights.dev" in hosts
        assert "ai-gateway.dev.insights.dev" in hosts

    @override_settings(
        SANDBOX_LLM_GATEWAY_URL="https://gateway.us.hanzo.ai",
        SANDBOX_AI_GATEWAY_URL=None,
    )
    def test_deduplicates_against_static_hosts_and_skips_unset(self):
        hosts = _session_init_probe_hosts()
        assert hosts.count("gateway.us.hanzo.ai") == 1
