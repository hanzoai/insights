    ...
    listeners(({ actions, values, props, sharedListeners }) => ({
        ...
        createPairedSchedule: async () => {
            ...
            try {
                ...
                toast.error('Invalid enable cron expression')
                return
            }
            ...
            try {
                ...
                toast.error('Invalid disable cron expression')
                return
            }
            ...
            try {
                ...
                toast.error('Failed to create the enable schedule')
                return
            }
            ...
            // Both succeeded
            toast.success('Paired schedules created')
            resetScheduleForm()
        ...
        submitFeatureFlagFailure: async () => {
            // Collapsed Collapse panels don't render their children, so any inline error
        ...
        updateFeatureFlagArchivedSuccess: ({ featureFlagActiveUpdate }) => {
            if (featureFlagActiveUpdate) {
                toast.success(`Feature flag ${featureFlagActiveUpdate.archived ? 'archived' : 'unarchived'}`)
                actions.setFeatureFlag(featureFlagActiveUpdate)
                actions.updateFlag(featureFlagActiveUpdate)
            }
        },
        ...
        copyFlagSuccess: ({ featureFlagCopy }) => {
            if (featureFlagCopy?.success.length) {
                ...
                if (failure?.approval_pending) {
                    ...
                    toast.error(`Error while copying feature flag: ${errorMessage}`)
        ...
        saveDescriptionInline: async ({ name }) => {
            ...
            try {
                const savedFlag = await api.update(`v1/projects/${values.currentProjectId}/feature_flags/${flag.id}`, {
                    name,
                })
