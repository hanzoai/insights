...
/** Stable selector for the chart tooltip portal — script-charts renders this on
 *  the document root via FloatingPortal, so it can't be found inside the chart
 *  wrapper. Kept in sync with overlays/Tooltip.tsx. */
export const FN_CHARTS_TOOLTIP_SELECTOR = '[data-script-charts-tooltip]'
...
export function createHogChartTooltip(element: HTMLElement): HogChartTooltip {
    return {
        ...
        get isPinned(): boolean {
            return element.classList.contains('script-charts-tooltip--pinned')
        },
    }
}
...
const seriesLabelOf = (row: HTMLElement): string =>
    (row.querySelector('[data-attr="script-chart-tooltip-series"]')?.textContent ?? '').replace(/:\s*$/, '').trim()

const swatchColorOf = (row: HTMLElement): string =>
    row.querySelector<HTMLElement>('[data-attr="script-chart-tooltip-swatch"]')?.style.backgroundColor ?? ''
...
export function createDefaultTooltipAccessor(element: HTMLElement): DefaultTooltipAccessor {
    ...
    return Object.assign(createHogChartTooltip(element), {
        ...
        total(): string | undefined {
            const total = element.querySelector('[data-attr="script-chart-tooltip-total"]')
            return total?.querySelector('[data-attr="script-chart-tooltip-value"]')?.textContent ?? undefined
        },
    })
}
...
export function getHogChartTooltip(): HTMLElement | null {
    return document.querySelector(FN_CHARTS_TOOLTIP_SELECTOR)
}
