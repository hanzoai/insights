...
describe('Insights', { concurrent: false }, () => {
    ...
    describe('insight-query tool', () => {
        ...
        describe('result shapes by format and query type', () => {
            it('InsightsQL query with output_format=json returns table shape with columns and results arrays', async () => {
                const insight = await createTestInsight(generateUniqueKey('InsightsQL JSON Shape'))
            ...
            it('InsightsQL query with output_format=optimized keeps structured results and surfaces the formatted string', async () => {
                // The API client always sends X-Insights-Client: mcp, so the backend runs the
                ...
                const insight = await createTestInsight(generateUniqueKey('InsightsQL Optimized Shape'))
    ...
    describe('insight CRUD id handling', () => {
        ...
        it('insight-get accepts a short_id', async () => {
            ...
            // but the Insights API accepts both numeric id and short_id on the retrieve path.
