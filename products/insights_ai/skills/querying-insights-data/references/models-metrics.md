...
The `insights.metrics` table holds OpenTelemetry metric data points from instrumented services. Each row is one observation of a metric (a counter increment, a gauge sample, or a histogram bucket).

**Namespacing:** Reference this table as `insights.metrics`, not bare `metrics` — it's registered under the `insights.` namespace in the InsightsQL database (see `insights/insightsql/database/database.py`). The same applies to `insights.metric_attributes`. Bare names fail with "Unknown table" at InsightsQL compile time. (Asymmetric with `logs`, which is registered at root level.)

There is no typed `query-metrics` MCP tool yet — **InsightsQL is the primary interface for metrics**. The schema mirrors `logs` and `insights.trace_spans` deliberately so cross-signal joins are cheap, and `trace_id` / `span_id` are first-class columns on every metric row (the OpenTelemetry exemplar pattern).

> ⚠️ **Exemplar extraction is not yet wired up in the ingestion pipeline as of PR [#50936](https://github.com/Insights/insights/pull/50936)**. The `trace_id` and `span_id` columns exist on `insights.metrics`, but `rust/capture-logs/src/metric_record.rs` currently ignores the `_exemplars` field (prefixed with underscore → unused). Every ingested metric row has `trace_id = ''` and `span_id = ''` today. The exemplar-based cross-signal correlation patterns documented below describe the intended capability once exemplar ingestion lands.

## `insights.metrics`

OpenTelemetry metric points. One row per observation. Backed by Datastore `metrics1` (distributed alias `metrics`).
...
- **`trace_id` will be base64-encoded** (matching `logs` and `insights.trace_spans`) once exemplars are populated. Joins to those tables will be direct equality on `trace_id`. Use `hex(tryBase64Decode(trace_id))` to display in hex.
...
- User InsightsQL queries on `insights.metrics` are capped at 50 GB read per query.

## `insights.metric_attributes`

AggregatingMergeTree rollup of metric attribute values, partitioned by service and 10-minute bucket. Same pattern as `log_attributes` / `insights.trace_attributes`. Same `insights.` namespacing rule — reference as `insights.metric_attributes`.
...
Pair this with `insights:apm-trace-get` (or a SQL join on `trace_spans`) to inspect the trace that drove the spike.
