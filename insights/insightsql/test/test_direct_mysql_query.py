...
from insights.insightsql.direct_sql.mysql_adapter import mysql_error_to_message, mysql_field_type_to_datastore_type
...
class TestDirectMySQLQuery(APIBaseTest):
    ...
    def _create_table(
        ...
        return DataWarehouseTable.objects.create(
            ...
            or {
                ...
                "email": {"insightsql": "StringDatabaseField", "datastore": "String", "valid": True},
                "created_at": {"insightsql": "DateTimeDatabaseField", "datastore": "DateTime", "valid": True},
            },
        )
    ...
    def test_mysql_field_type_to_datastore_type(self, _name: str, type_code: int | None, expected: str):
        self.assertEqual(mysql_field_type_to_datastore_type(type_code), expected)
    ...
    def test_string_constants_are_parameterized(self):
        ...
        self.assertIn("%(insightsql_val_0)s", sql)
        self.assertEqual(context.values["insightsql_val_0"], "a@b.com")
    ...
    def test_raw_query_routes_to_mysql_executor(self):
        ...
        with patch(
            "insights.insightsql.direct_sql.mysql_adapter.MySQLAdapter.validate_source_config",
            return_value=(implementation, MagicMock()),
        ):
            with patch.object(InsightsQLQueryExecutor, "_capture_send_raw_query_translation_error"):
    ...
    def test_raw_query_rejects_unsafe_mysql_statements(self, _name: str, query: str):
        ...
        with patch("insights.insightsql.direct_sql.mysql_adapter.MySQLAdapter.validate_source_config") as mock_validate:
            with self.assertRaisesRegex(ExposedInsightsQLError, "Raw MySQL queries must be read-only SELECT statements."):
                executor.execute()
