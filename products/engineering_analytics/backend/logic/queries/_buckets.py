...
def bucket_expr(granularity: Granularity, column: str = "run_started_at") -> str:
    """Datastore truncation expression for ``column`` at ``granularity``. Week starts Monday (mode 1)."""
    if granularity == "hour":
...
def normalize_bucket(value: datetime | date, granularity: Granularity) -> datetime:
    ...
    Datastore can hand the bucket back as a ``date`` (date/week truncation) or a ``datetime``
