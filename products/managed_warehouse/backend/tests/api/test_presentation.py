...
def _onboarding_side_effects():
    ...
    with (
        ...
        patch(
            "products.managed_warehouse.backend.facade.connection.ensure_managed_warehouse_direct_source"
        ) as mock_ensure,
    ):
...
@patch("products.managed_warehouse.backend.presentation.views.hanzo_insights.feature_enabled")
...
def test_provision_sends_team_id_and_schema_name_to_control_plane(mock_request: MagicMock) -> None:
    ...
    assert teams_body == {
        ...
        "schema_data_imports_name": "insights_data_imports_prod_events",
    }
...
@parameterized.expand(
    [
        ("US", "insights-duckling-0194d6405db400006cde48d6114c0f99-mw-prod-us", "us-east-1"),
        ("EU", "insights-duckling-0194d6405db400006cde48d6114c0f99-mw-prod-eu", "eu-central-1"),
    ]
)
@pytest.mark.django_db
@patch("products.managed_warehouse.backend.presentation.views._request")
def test_provision_persists_bucket_returned_by_control_plane(
    ...
    assert resp.status_code == 202
    server = DuckgresServer.objects.get(organization_id=org.id)
    # Verbatim, not the locally-derived f"insights-duckling-{org.id}-prod-us".
...
def test_status_for_self_heals_stale_bucket(mock_request: MagicMock) -> None:
    ...
    DuckgresServer.objects.create(
        ...
        username="root",
        password="pw",
        bucket="insights-duckling-stale-prod-us",  # wrong/drifted
    )
    cp_bucket = "insights-duckling-0194d6405db400006cde48d6114c0f99-mw-prod-us"
...
def test_status_for_leaves_matching_bucket_untouched(mock_request: MagicMock) -> None:
    org = Organization.objects.create(name="Org")
    bucket = "insights-duckling-org-mw-prod-us"
    DuckgresServer.objects.create(
        organization_id=org.id,
        host="h",
        port=5432,
...
def test_status_for_refuses_to_reconcile_on_org_mismatch(mock_request: MagicMock) -> None:
    ...
    DuckgresServer.objects.create(
        ...
        username="root",
        password="pw",
        bucket="insights-duckling-mine-mw-prod-us",
    )
    mock_request.return_value = Response(
        {
            "org_id": "00000000-0000-0000-0000-000000000000",
            "state": "ready",
            "bucket": "insights-duckling-other-mw-prod-us",
        },
        status=200,
    )
    ...
    assert DuckgresServer.objects.get(organization_id=org.id).bucket == "insights-duckling-mine-mw-prod-us"
...
def test_status_for_without_bucket_leaves_row_alone(mock_request: MagicMock) -> None:
    ...
    DuckgresServer.objects.create(
        ...
        username="root",
        password="pw",
        bucket="insights-duckling-keep-mw-prod-us",
    )
    mock_request.return_value = Response({"org_id": str(org.id), "state": "ready"}, status=200)

    managed_warehouse.status_for(org.id)

    assert DuckgresServer.objects.get(organization_id=org.id).bucket == "insights-duckling-keep-mw-prod-us"
...
def test_status_for_strips_bucket_from_response(mock_request: MagicMock) -> None:
    ...
    org = Organization.objects.create(name="Org")
    mock_request.return_value = Response(
        {"org_id": str(org.id), "state": "ready", "bucket": "insights-duckling-x-mw-prod-us"}, status=200
    )
...
@pytest.mark.django_db
@patch("products.managed_warehouse.backend.presentation.views.is_enabled", return_value=False)
@patch("products.managed_warehouse.backend.presentation.views.internal_requests")
...
def test_onboard_team_creates_duckgres_row_with_legacy_names(
    ...
    # duckling DAG actually writes today (insights.events_<suffix> + insights_data_imports_<suffix>).
    ...
    assert mock_request.call_args_list[1].kwargs["json_body"] == {
        ...
        "schema_data_imports_name": "insights_data_imports_my_events",
    }
...
@pytest.mark.django_db
@patch("products.managed_warehouse.backend.presentation.views.delete_team")
...
@pytest.mark.django_db
@patch("products.managed_warehouse.backend.presentation.views.create_team")
