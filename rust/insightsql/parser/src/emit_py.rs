//! `PyEmitter` — `Emitter` impl that constructs `insights.insightsql.ast` dataclasses directly during parsing, bypassing the `serde_json::Value` intermediate tree.
//!
...
//!  - All `insights.insightsql.ast` classes are looked up *once* at [`PyEmitter::new`] and stored as `Bound<'py, PyAny>` references — a `getattr(ast_module, "Constant")` per node would dominate runtime over the json round-trip we're trying to beat.
...
/// Wrapper around a Python AST object plus the out-of-band `positions_locked` flag. The underlying `insights.insightsql.ast.*` dataclasses use `slots=True` so they can't carry an arbitrary "positions locked" attribute; the wrapper carries it instead.
///
...
/// Constructs `insights.insightsql.ast` instances directly. Holds the GIL token + cached class references for the full duration of one parse.
///
...
pub struct PyEmitter<'py> {
    ...
    cls_insightsqlx_tag: Bound<'py, PyAny>,
    cls_insightsqlx_attribute: Bound<'py, PyAny>,
    ...
    /// Python builtin `int` class — used by `constant_number_string` for arbitrary-precision integer literals (i64-wider hex/decimal). Cached once instead of `py.eval("int", ...)` per call.
...
impl<'py> PyEmitter<'py> {
    pub fn new(py: Python<'py>) -> PyResult<Self> {
        let ast_module = py.import("insights.insightsql.ast")?;
        ...
        Ok(Self {
            ...
            cls_insightsqlx_tag: ast_module.getattr("InsightsQLXTag")?,
            cls_insightsqlx_attribute: ast_module.getattr("InsightsQLXAttribute")?,
            ...
            cls_int: py.import("builtins")?.getattr("int")?,
    ...
    fn build(&self, class: &Bound<'py, PyAny>, kwargs: &Bound<'py, PyDict>) -> PyAst {
        match class.call(PyTuple::empty(self.py), Some(kwargs)) {
            Ok(obj) => PyAst {
                obj: obj.unbind(),
    ...
    fn wrap_prim(&self, obj: Py<PyAny>) -> PyAst {
        PyAst {
            obj,
            positions_locked: false,
            rust_sentinels: None,
...
impl<'py> Emitter for PyEmitter<'py> {
    ...
    fn bool(&self, v: bool) -> PyAst {
        self.wrap_prim(v.into_py_any(self.py).unwrap())
    }
    ...
    fn constant_number_string(&self, text: String) -> PyAst {
        ...
        let args = PyTuple::new(
            self.py,
            [
                text.into_py_any(self.py).unwrap(),
                base.into_py_any(self.py).unwrap(),
            ],
        )
        .unwrap();
    ...
    fn lambda(&self, args: Vec<String>, expr: PyAst) -> PyAst {
        let kw = PyDict::new(self.py);
        let args_list = PyList::empty(self.py);
        for a in args {
    ...
    fn dict_(&self, items: Vec<(PyAst, PyAst)>) -> PyAst {
        let kw = PyDict::new(self.py);
        let items_list = PyList::empty(self.py);
        for (k, v) in items {
            let pair = PyTuple::new(self.py, [k.obj, v.obj]).unwrap();
    ...
    fn columns_expr(
        ...
        if let Some(ex) = exclude {
            let ex_list = PyList::empty(self.py);
            for s in ex {
        ...
        if let Some(rep) = replace {
            let rep_dict = PyDict::new(self.py);
            for (k, v) in rep {
    ...
    fn catch_clause(&self, var: PyAst, ty: PyAst, body: PyAst) -> PyAst {
        ...
        let pair = PyTuple::new(self.py, [var.obj, ty.obj, body.obj]).unwrap();
    ...
    fn insightsqlx_tag(&self, kind: &str, attributes: Vec<PyAst>) -> PyAst {
        let kw = PyDict::new(self.py);
        kw.set_item("kind", kind).unwrap();
        kw.set_item("attributes", build_list(self.py, attributes))
            .unwrap();
        self.build(&self.cls_insightsqlx_tag, &kw)
    }
    fn insightsqlx_attribute(&self, name: &str, value: PyAst) -> PyAst {
        let kw = PyDict::new(self.py);
        kw.set_item("name", name).unwrap();
        kw.set_item("value", value.obj.bind(self.py)).unwrap();
        self.build(&self.cls_insightsqlx_attribute, &kw)
    }
    ...
    fn window_expr_empty(&self) -> PyAst {
        ...
        self.build(&self.cls_window_expr, &PyDict::new(self.py))
    }
    ...
    fn select_query_empty(&self) -> PyAst {
        ...
        let kw = PyDict::new(self.py);
        kw.set_item("select", PyList::empty(self.py)).unwrap();
    ...
    fn position(&self, _line: u32, _column: u32, offset: usize) -> PyAst {
        ...
        self.wrap_prim(offset.into_py_any(self.py).unwrap())
    }
    ...
    fn get_field(&self, v: &PyAst, name: &str) -> Option<PyAst> {
        ...
        if name == "ctes" {
            if let Ok(dict) = got.cast::<PyDict>() {
                let list = PyList::empty(self.py);
    ...
    fn set_field(&self, v: &mut PyAst, name: &str, value: PyAst) {
        ...
        if name == "ctes" {
            ...
            if let Ok(list) = val_bound.cast::<PyList>() {
                let dict = PyDict::new(self.py);
                for item in list.iter() {
                    let cte_name = item
                        .getattr("name")
