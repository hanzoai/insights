//! End-to-end coverage for the `on_error` hook wired in `common_insights::init`:
//! a terminal capture failure must emit `insights_sdk_delivery_failures_total`
//! with the right `service`/`surface`/`reason` labels.
//!
//! One test per binary: `init` builds a process-global Insights client and the
...
async fn init_hook_emits_delivery_failure_metric_on_terminal_reject() {
    ...
    common_insights::init("test-svc", Some("test-key"), &server.base_url())
        .await
        .expect("init should succeed when an api key is provided");

    insights_rs::capture(insights_rs::Event::new_anon("test_event"));
    insights_rs::flush().await;
    ...
    let hit = snapshotter
        ...
        .find_map(|(key, _, _, value)| {
            if key.key().name() != "insights_sdk_delivery_failures_total" {
                return None;
            }
