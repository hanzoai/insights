...
from insights.kafka_client.topics import KAFKA_CDP_DATASTORE_PREFILTERED_EVENTS
...
from insights.temporal.messaging.filter_storage import get_event_filters
from insights.temporal.messaging.types import BehavioralEventFilter
...
def parse_event_properties(properties_raw: Any, event_uuid: str) -> dict[str, Any]:
    """Parse event properties from Datastore, handling both string and dict formats."""
...
async def backfill_precalculated_events_activity(
    ...
    async with Heartbeater(
        ...
        # through the overrides here — mirroring the InsightsQL events↔overrides lazy join — so
            ...
            async with get_messaging_client(team_id=inputs.team_id) as client:
                async for row in client.stream_query_as_jsonl(events_query, query_parameters=query_params):
                    if first_row:
                        ...
                        logger.info(
                            "First row received from Datastore",
...
class BackfillPrecalculatedEventsWorkflow(InsightsWorkflow):
