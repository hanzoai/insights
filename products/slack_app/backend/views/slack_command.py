...
runs ``/insights ...`` in a channel or DM. The vocabulary mirrors the
``@Insights <command>`` mention path (``help``, ``rules ...``, ``project ...``);
...
from insights.models.integration import SlackIntegration, SlackIntegrationError, validate_slack_request
...
def slack_app_command_handler(request: HttpRequest) -> HttpResponse:
    ...
    # A bare ``/insights`` (no arguments) behaves as ``/insights help``.
    ...
    if region_route == ROUTE_NO_INTEGRATION:
        return _ephemeral_response(
            "This Slack workspace isn't connected to a Insights organization. "
            "Connect it from a project's *Integrations* page first."
        )
    if region_route == ROUTE_PROXY_FAILED:
        return _ephemeral_response("Couldn't reach the Insights backend — try again in a moment.")
