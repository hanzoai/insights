...
class CustomFunctionTemplate(UUIDTModel):
    """
    Django model for storing CustomFunction templates in the database.
    This model replaces the in-memory storage of templates and enables sha versioning,
    efficient storage, and easier updates for CustomFunction templates.
    """
    ...
    @classmethod
    def get_template(cls, template_id: str, sha: Optional[str] = None) -> Optional["CustomFunctionTemplate"]:
        """
    ...
    def compile_bytecode(self):
        """
        Compiles the Custom code_language code to bytecode and stores it in the bytecode field.
        This should be called after changing the code field.
        """
        if self.code_language != "custom_script":
            self.bytecode = None
            return
        try:
