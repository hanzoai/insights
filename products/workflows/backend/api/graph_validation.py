...
# in products/workflows/frontend/Workflows/insightsflows): conditional_branch has one branch per condition,
...
def validate_graph(actions: list[dict], edges: list[dict], abort_action: Optional[str] = None) -> list[str]:
    ...
    whole-flow checks in InsightsFlowSerializer — those validate config; this validates the graph wiring."""
    errors: list[str] = []
