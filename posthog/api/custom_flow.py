...
from posthog.api.custom_flow_batch_job import CustomFlowBatchJobSerializer
...
from posthog.plugins.plugin_server_api import create_custom_flow_invocation_test
...
class CustomFlowConfigFunctionInputsSerializer(serializers.Serializer):
...
class CustomFlowActionSerializer(serializers.Serializer):
    ...
    filters = CustomFunctionFiltersSerializer(required=False, default=None, allow_null=True)
    ...
    def validate(self, data):
        ...
        if data.get("type") == "trigger":
            if data.get("config", {}).get("type") in ["webhook", "manual", "tracking_pixel", "schedule"]:
                ...
                if filters:
                    serializer = CustomFunctionFiltersSerializer(data=filters, context=self.context)
                    if is_draft:
        ...
        if "function" in data.get("type", "") or trigger_is_function:
            ...
            template = CustomFunctionTemplate.get_template(template_id)
            if not template:
                if not is_draft:
                ...
                function_config_serializer = CustomFlowConfigFunctionInputsSerializer(
                    data={
...
class CustomFlowVariableSerializer(serializers.ListSerializer):
...
class CustomFlowMaskingSerializer(serializers.Serializer):
...
class CustomFlowMinimalSerializer(serializers.ModelSerializer):
...
class CustomFlowSerializer(CustomFlowMinimalSerializer):
    actions = serializers.ListField(child=CustomFlowActionSerializer(), required=True)
    trigger_masking = CustomFlowMaskingSerializer(required=False, allow_null=True)
    variables = CustomFlowVariableSerializer(required=False)
    ...
    def validate(self, data):
        instance = cast(Optional[CustomFlow], self.instance)
        ...
        if status == "active" and instance and instance.status != "active" and "actions" not in data:
            action_serializer = CustomFlowActionSerializer(data=instance.actions, many=True, context=self.context)
    ...
    def create(self, validated_data: dict, *args, **kwargs) -> CustomFlow:
...
class CustomFlowInvocationSerializer(serializers.Serializer):
    configuration = CustomFlowSerializer(write_only=True, required=False)
...
class CustomFlowFilterSet(FilterSet):
    class Meta:
        model = CustomFlow
        fields = ["id", "created_by", "created_at", "updated_at"]


class CustomFlowViewSet(TeamAndOrgViewSetMixin, LogEntryMixin, AppMetricsMixin, viewsets.ModelViewSet):
    scope_object = "INTERNAL"
    queryset = CustomFlow.objects.all()
    filter_backends = [DjangoFilterBackend]
    filterset_class = CustomFlowFilterSet
    log_source = "custom_flow"
    app_source = "custom_flow"

    def get_serializer_class(self) -> type[BaseSerializer]:
        return CustomFlowMinimalSerializer if self.action == "list" else CustomFlowSerializer
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.warning("Failed to capture custom_flow_created event", error=str(e))
    ...
    def perform_update(self, serializer):
        ...
        try:
            ...
            before_update = CustomFlow.objects.get(pk=instance_id)
        except CustomFlow.DoesNotExist:
            before_update = None
        ...
        if (
            ...
            and serializer.instance.status == CustomFlow.State.ACTIVE
        ):
            try:
                # Count edges and actions
                edges_count = len(serializer.instance.edges) if serializer.instance.edges else 0
                ...
                logger.warning("Failed to capture custom_flow_activated event", error=str(e))

    @action(detail=True, methods=["POST"])
    def invocations(self, request: Request, *args, **kwargs):
        try:
            custom_flow = self.get_object()
        except Exception:
            custom_flow = None

        serializer = CustomFlowInvocationSerializer(
            data=request.data, context={**self.get_serializer_context(), "instance": custom_flow}
        )
        if not serializer.is_valid():
            return Response(serializer.errors, status=400)

        res = create_custom_flow_invocation_test(
            team_id=self.team_id,
            custom_flow_id=str(custom_flow.id) if custom_flow else "new",
    ...
    def batch_jobs(self, request: Request, *args, **kwargs):
        ...
        if request.method == "POST":
            serializer = CustomFlowBatchJobSerializer(
                data={**request.data, "custom_flow": custom_flow.id}, context={**self.get_serializer_context()}
            )
            if not serializer.is_valid():
                return Response(serializer.errors, status=400)

            batch_job = serializer.save()
            return Response(CustomFlowBatchJobSerializer(batch_job).data)
        else:
            batch_jobs = CustomFlowBatchJob.objects.filter(custom_flow=custom_flow, team=self.team).order_by("-created_at")
            serializer = CustomFlowBatchJobSerializer(batch_jobs, many=True)
