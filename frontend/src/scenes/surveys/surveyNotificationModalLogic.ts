...
export type OpenSurveyNotificationDialogPayload = {
    notification?: InsightsFunctionType | null
...
function buildTemplateGlobals(survey: SurveyNotificationContext): CyclotronJobInvocationGlobals {
    return buildSurveyExampleInvocationGlobals({
        ...
        projectId: 1,
        projectName: 'Project',
        projectUrl: 'https://app.hanzo.ai/project/1',
    })
}
...
async function fetchLastSurveyResponseGlobals(query: EventsQuery | null): Promise<LastSurveyResponseResult> {
    ...
    try {
        ...
        return { status: 'ok', globals: convertToInsightsFunctionInvocationGlobals(event, person) }
    } catch {
...
async function buildSurveyNotificationPayload({
    ...
    editingNotification: InsightsFunctionType | null
    copiedNotification: InsightsFunctionType | null
}): Promise<Partial<InsightsFunctionType>> {
    const templateId = DESTINATION_OPTIONS.find((option) => option.value === form.destination)?.templateId
    const template = await api.insightsFunctions.getTemplate(templateId || 'template-slack')
...
function getInputValue(inputs: InsightsFunctionType['inputs'], key: string): unknown {
    return inputs?.[key]?.value
}

function getInputString(inputs: InsightsFunctionType['inputs'], key: string): string {
    const value = getInputValue(inputs, key)
    return typeof value === 'string' ? value : ''
}

function getInputNumber(inputs: InsightsFunctionType['inputs'], key: string): number | null {
    const value = getInputValue(inputs, key)
    return typeof value === 'number' ? value : null
}

function getDestinationForNotification(notification: InsightsFunctionType): DestinationKey {
...
function getSlackMessageFromNotification(notification: InsightsFunctionType, defaultMessage: string): string {
...
function getSlackIncludeButtons(notification: InsightsFunctionType): boolean {
    const blocks = getInputValue(notification.inputs, 'blocks')
...
function getSentEventPropertiesFromNotification(notification: InsightsFunctionType): EventPropertyFilter[] {
...
function buildSurveyNotificationFormFromNotification(
    notification: InsightsFunctionType,
    survey: SurveyNotificationContext,
...
function mergeResponseFiltersIntoExistingFilters(
    existingFilters: InsightsFunctionType['filters'],
    fallbackFilters: InsightsFunctionType['filters'],
    responseFilters: SurveyResponseFilter[]
): InsightsFunctionType['filters'] {
...
function updateSurveyNotificationPayload({
    ...
    return {
        ...
        inputs: {
            ...notification.inputs,
            ...(payload.inputs as Record<string, InsightsFunctionInputValue>),
        },
        ...
        script: notification.script ?? payload.script,
        icon_url: notification.icon_url ?? payload.icon_url,
    }
}

function getSurveyNotificationDestinationLabel(notification: InsightsFunctionType): string {
    const templateId = notification.template_id ?? notification.template?.id
...
function createCopiedSurveyNotificationPayload({
    ...
    return {
        ...
        script: remapSurveyResponseProperties(notification.script, survey) ?? template.code,
...
export interface surveyNotificationModalLogicValues {
    ...
    surveyNotifications: InsightsFunctionType[] // surveyLogic
...
export interface surveyNotificationModalLogicActions {
    ...
    loadSurveyNotificationsSuccess: (
        surveyNotifications: InsightsFunctionType[],
        payload?: any
    ) => {
        payload?: any
        surveyNotifications: InsightsFunctionType[]
        ...
        testResult: [
            null as CyclotronJobTestInvocationResult | null,
            {
                ...
                sendTestNotification: async ({ source }) => {
                    ...
                    return await api.insightsFunctions.createTestInvocation(id, {
        ...
        notificationForm: {
            ...
            submit: async (form: SurveyNotificationForm) => {
                ...
                if (values.editingNotification) {
                    await api.insightsFunctions.update(values.editingNotification.id, payload)
                } else {
                    await api.insightsFunctions.create(payload)
                }
            },
        },
    })),
    ...
    listeners(({ actions, values }) => ({
        ...
        sendTestNotificationSuccess: ({ testResult }) => {
            if (testResult?.status === 'success') {
                ...
                toast.error('Test notification failed — see logs below.')
            }
        },
        ...
        consumePendingDeepLink: () => {
            ...
            toast.error("We couldn't find that notification — it may have been removed.")
        },
        closeDialog: () => {
            actions.resetNotificationForm()
            actions.clearTestResult()
