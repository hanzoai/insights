...
- **Found:** Stamphog's `db_for_write` pins exist because Stamphog IS a registered product DB whose `db_for_read` returns `stamphog_db_reader`, a genuine lagging replica (`insights/product_db_router.py:27-35`, and its own code comments warn about a "lagged reader"). That rationale is specific to product-DB models and does not transfer to a main-DB model.
