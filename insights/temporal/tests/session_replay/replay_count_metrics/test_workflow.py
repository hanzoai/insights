...
from insights.temporal.session_replay.replay_count_metrics.workflow import ReplayCountMetricsWorkflow
...
async def test_activity_pushes_metrics():
    ...
    with patch(
        "insights.temporal.session_replay.replay_count_metrics.activities.get_client",
...
async def test_activity_raises_on_empty_result():
    ...
    with (
        ...
        pytest.raises(RuntimeError, match="Datastore returned empty result"),
    ):
        from insights.temporal.session_replay.replay_count_metrics.activities import collect_replay_count_metrics
