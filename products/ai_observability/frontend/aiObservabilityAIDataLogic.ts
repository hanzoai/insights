...
async function queryAIDataForEvent(params: LoadAIDataParams, source: AIDataQuerySource): Promise<AIData | null> {
    ...
    const response = await api.queryInsightsQL<AIDataQueryRow[]>(
        insightsql`
            SELECT
            ...
            FROM (
                SELECT
                    ...
                    timestamp,
                    ${insightsql.raw(source.inputExpression)} AS ai_input,
                    ${insightsql.raw(source.outputExpression)} AS ai_output,
                    ${insightsql.raw(source.outputChoicesExpression)} AS ai_output_choices,
                    ${insightsql.raw(source.inputStateExpression)} AS ai_input_state,
                    ${insightsql.raw(source.outputStateExpression)} AS ai_output_state,
                    ${insightsql.raw(source.toolsExpression)} AS ai_tools
                FROM ${insightsql.raw(source.from)}
                WHERE ${insightsql.raw(source.traceIdExpression)} = ${params.traceId}
