...
from insights.db_circuit_breaker import ProductDBCircuitBreaker, _get_redis
...
class TestProductDBCircuitBreaker(SimpleTestCase):
    def setUp(self) -> None:
        ...
        self.now_patch = mock.patch("insights.db_circuit_breaker._now", side_effect=lambda: self.clock["now"])
    ...
    def test_local_cache_short_circuits_redis_while_open(self) -> None:
        ...
        with mock.patch("insights.db_circuit_breaker._get_redis") as redis_mock:
    ...
    def test_fails_safe_open_when_redis_unavailable(self) -> None:
        with mock.patch("insights.db_circuit_breaker._get_redis", return_value=None):
            self.assertTrue(ProductDBCircuitBreaker().before_connect(ALIAS).allowed)
