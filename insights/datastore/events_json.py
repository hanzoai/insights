"""Datastore schema declarations for the native-JSON events tables.

Table names, JSON subcolumn type declarations, and index DDL for the events-JSON schema.
Django-free so the InsightsQL engine (printer, property planner) can import these without booting
Django; insights.models.event.sql re-exports the public names for existing callers.
"""
...
# an empty string. Arrays and maps stay non-nullable because Datastore does not support Nullable(Array(...)) or
# Nullable(Map(...)).
