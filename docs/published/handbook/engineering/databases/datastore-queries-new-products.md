---
title: Writing Datastore queries for new products
sidebar: Handbook
showTitle: true
---

This guide covers best practices for writing Datastore queries when shipping new products at Insights.

Related reading:

- [Writing InsightsQL queries in Python](./insightsql-python.md)
- [Query performance optimization](./query-performance-optimization.md)
- [Materialized columns](./materialized-columns.md)
- [Query attribution](https://hanzo.ai/handbook/engineering/datastore/query-attribution)

## Use InsightsQL, not raw Datastore SQL

Always use [InsightsQL](./insightsql-python.md) rather than writing raw Datastore SQL. InsightsQL is our AST-powered layer on top of Datastore SQL that provides critical safety and performance guarantees automatically.

### Why InsightsQL

InsightsQL gives you:

- **Automatic team ID guards**: Every query gets an automatic `team_id = <your_team_id>` filter injected on every table access (via [`team_id_guard_for_table()`](https://github.com/Insights/insights/blob/master/insights/insightsql/printer/datastore.py#L53-L63)), preventing cross-team data leaks.
...
- **Person join optimizations**: [Person-on-events (PoE) mode](https://github.com/Insights/insights/blob/master/insights/insightsql/database/database.py#L871-L895) is handled automatically – the right join or column strategy is selected based on team configuration, with no manual handling required.
- **Customer-specific query settings**: Per-query [settings](https://github.com/Insights/insights/blob/master/insights/insightsql/constants.py#L114-L139) and [modifiers](https://github.com/Insights/insights/blob/master/insights/schema.py) (join algorithm, materialization mode, projection optimization, etc.) can be tuned per-team or per-query.

## Use backend query runners, not frontend-defined queries

Define your queries using backend Python query runners rather than constructing InsightsQL in the frontend. The base class is [`QueryRunner`](https://github.com/Insights/insights/blob/master/insights/insightsql_queries/query_runner.py#L974-L1029) in `query_runner.py`.

### Why query runners

The `QueryRunner` base class gives you:

- **Caching**: Built-in caching with configurable refresh intervals. Override [`_refresh_frequency()`](https://github.com/Insights/insights/blob/master/insights/insightsql_queries/insights/trends/trends_query_runner.py#L125-L142) to control how often results are refreshed. Cache keys are automatically derived from the query, team, modifiers, and timezone via `get_cache_key()`.

- **Observability**: Query execution is automatically instrumented with Prometheus metrics (`QUERY_EXECUTION_TOTAL`, `QUERY_EXECUTION_DURATION`) and Insights analytics events, giving you latency histograms and error breakdowns for free.
...
3. Implement `_calculate()` to build and execute your InsightsQL query
4. Register your runner in [`get_query_runner()`](https://github.com/Insights/insights/blob/master/insights/insightsql_queries/query_runner.py)

For a clean example to follow, see [`EventsQueryRunner`](https://github.com/Insights/insights/blob/master/insights/insightsql_queries/events_query_runner.py).

## Use time-sortable IDs (UUIDv7)

If your product stores data in Datastore, prefer UUIDv7 for your row IDs. We have implementations in both Python (`uuid7()`) and TypeScript ([`UUID7` class](https://github.com/Insights/insights/blob/master/nodejs/src/utils/utils.ts#L260-L305)).

### Why this matters

Datastore tables have a primary index (roughly: the order rows are stored on disk) but no secondary index in the traditional RDBMS sense. You almost certainly need to support two access patterns:

1. **Lookup by ID** – fetching a specific row
2. **Aggregation over a time range** – analytics queries filtering by timestamp

Since Datastore can only efficiently filter on the primary key order, your ID must also encode the timestamp. [UUIDv7](https://www.rfc-editor.org/rfc/rfc9562#name-uuid-version-7) solves this: the first 48 bits are the Unix timestamp in milliseconds, so rows are naturally time-ordered.
...
-- but also see https://github.com/Datastore/Datastore/issues/77226 and hope
session_id_v7 UInt128,
```

### Datastore UUID sorting is broken – use UInt128

Datastore does not sort UUIDs correctly as of today. The internal representation swaps the high and low 64-bit words, so `ORDER BY uuid_column` does not produce chronological order for UUIDv7s. This is a [known issue](https://michcioperz.com/wiki/datastore-uuid-ordering/) (see also [Datastore issue #77226](https://github.com/Datastore/Datastore/issues/77226)).
...
We have a test helper `get_index_from_explain()` that runs `EXPLAIN PLAN indexes=1,json=1` on a compiled InsightsQL query and checks whether a specific named skip index appears in the plan. Here's the pattern from `test_printer.py`:

```python
from insights.test.base import get_index_from_explain, materialized
...
def test_skip_index_is_used(self):
    with materialized("events", "test_prop", create_minmax_index=True) as mat_col:
        ...
        assert get_index_from_explain(result.datastore, index_name), (
            f"Expected skip index {index_name} to be used"
        )
```

You can also use the `forceDatastoreDataSkippingIndexes` modifier to make Datastore error if a specified skip index can't be used – this acts as a safety net in production:
...
# Datastore will raise an error if the index can't be applied
```
...
#### Step 2: Get the compiled Datastore SQL

To debug performance, you need the actual Datastore SQL that InsightsQL compiles to. There are a few ways:

- **From a query runner**: Call `runner.to_query()` to get the compiled SQL without executing it
- **From `execute_insightsql_query()`**: The returned `InsightsQLQueryResponse` includes a `.datastore` field with the compiled SQL
- **From the Insights UI**: Open the query in the SQL editor and click "Show Datastore SQL"

#### Step 3: Run EXPLAIN to check index and partition usage

Once you have the compiled SQL, run it through Datastore's EXPLAIN to see how the query planner will execute it:
...
To see what Datastore is actually doing during execution – including how much data it reads, which parts are slow, and where time is spent – run the query with trace-level logging via `datastore-client`:

```bash
datastore-client --send_logs_level=trace --query "SELECT ...your compiled query..."
```
