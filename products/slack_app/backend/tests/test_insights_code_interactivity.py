...
from insights.schema_enums import AlertState
...
class TestInsightsCodeInteractivityHandler(TestCase):
    def setUp(self):
...
class TestRepoPickerOptions(TestCase):
    def setUp(self):
        ...
        cache.set(f"insights_code_repo_picker_ctx:{self.context_token}", self.context_payload, timeout=900)
    ...
    def test_options_returns_filtered_repos(self, mock_config, mock_get_repos):
        ...
        mock_get_repos.return_value = ["insights/insights", "insights/insights-js", "insights/scriptvm"]
        ...
        payload = {
            ...
            "value": "js",
            "user": {"id": "U123"},
            "block_id": f"insights_code_repo_picker_v1:{self.context_token}",
        }
        response = self._post_interactivity(payload)
        assert response.status_code == 200
        options = response.json()["options"]
        assert len(options) == 1
        assert options[0]["value"] == "insights/insights-js"

    @patch("products.slack_app.backend.api._get_full_repo_names")
    @patch("products.slack_app.backend.api.SlackIntegration.slack_config")
    def test_options_empty_query_returns_all(self, mock_config, mock_get_repos):
        mock_config.return_value = {"SLACK_APP_SIGNING_SECRET": self.signing_secret}
        mock_get_repos.return_value = ["insights/insights", "insights/insights-js"]
    ...
    def test_options_expired_token_returns_empty(self, mock_config):
        mock_config.return_value = {"SLACK_APP_SIGNING_SECRET": self.signing_secret}
        cache.delete(f"insights_code_repo_picker_ctx:{self.context_token}")
    ...
    def test_submit_signals_temporal_workflow(
        ...
        self.context_payload["workflow_id"] = "insights-code-mention-T12345:C001:1234.5678"
        cache.set(f"insights_code_repo_picker_ctx:{self.context_token}", self.context_payload, timeout=900)
            ...
            "actions": [
                {
                    "action_id": "insights_code_repo_select",
                    "block_id": f"insights_code_repo_picker_v1:{self.context_token}",
                    "selected_option": {"value": "insights/insights"},
        ...
        mock_sync_connect.return_value.get_workflow_handle.assert_called_once_with(
            "insights-code-mention-T12345:C001:1234.5678"
        )
            ...
            "actions": [
                {
                    ...
                    "block_id": f"insights_code_repo_picker_v2:{self.insights_code_integration.id}:U123:{self.context_token}:actions",
    ...
    def test_continue_as_bot_signals_authorship_confirmed(self, mock_config, mock_sync_connect, mock_webclient_class):
        ...
        payload = {
            ...
            "actions": [
                {
                    "action_id": "insights_code_continue_as_bot",
                    "value": json.dumps(
                        {
                            "workflow_id": "insights-code-mention-T12345:C001:1234.5678",
                            "integration_id": self.insights_code_integration.id,
                            "mentioning_slack_user_id": "U123",
                        }
                    ),
        ...
        mock_handle.signal.assert_called_once_with(InsightsCodeSlackMentionWorkflow.authorship_confirmed)
        mock_webclient_class.return_value.chat_update.assert_called_once()
    ...
    def test_submit_without_workflow_id_posts_expired(self, mock_config, mock_webclient_class):
        ...
        assert "insights again" in mock_client.chat_postMessage.call_args.kwargs["text"].lower()
    ...
    def test_terminate_action_starts_temporal_workflow(self, mock_config, mock_sync_connect, mock_asyncio_run):
        ...
        payload = {
            ...
            "actions": [
                {
                    "action_id": "insights_code_terminate_task",
                    "value": json.dumps(
                        {
                            "run_id": "run-1",
                            "integration_id": self.insights_code_integration.id,
    ...
    @patch("insights.models.integration.Integration")
    @patch("insights.models.integration.SlackIntegration")
    @patch("insights.temporal.common.client.sync_connect")
    def test_terminate_action_signals_workflow(
        ...
        from products.slack_app.backend.tasks import process_insights_code_task_termination
        ...
        process_insights_code_task_termination(payload)
...
class TestInteractivityRegionRouting(TestCase):
    ...
    def _post(self, payload: dict, *, host: str = "us.hanzo.ai", **extra_headers) -> Any:
        payload = {"team": {"id": "T12345"}, **payload}
        body_str = f"payload={json.dumps(payload)}"
        body = body_str.encode()
        signed = sign_slack_request(body, self.signing_secret)
    ...
    def _local_picker_payload(self) -> dict:
        # block_id "insights_code_repo_picker_v1:<token>" + a cached context that points at our
        # local integration is the simplest way to drive the "local" branch in the handler.
        token = "ctx-local-1"
        cache.set(
            f"insights_code_repo_picker_ctx:{token}",
            {
                "integration_id": self.insights_code_integration.id,
                "channel": "C001",
                "thread_ts": "1234.5678",
        ...
        return {
            ...
            "value": "",
            "user": {"id": "U123"},
            "block_id": f"insights_code_repo_picker_v1:{token}",
        }
    ...
    def _foreign_picker_payload(self) -> dict:
        ...
        return {
            ...
            "value": "",
            "user": {"id": "U999"},
            "block_id": "insights_code_repo_picker_v1:no-such-token",
        }
    ...
    def test_local_match_handles_without_proxy(self, mock_config, _mock_repos, mock_proxy):
        mock_config.return_value = {"SLACK_APP_SIGNING_SECRET": self.signing_secret}
        response = self._post(self._local_picker_payload(), host="us.hanzo.ai")
        assert response.status_code == 200
    ...
    def test_us_no_local_proxies_to_eu(self, mock_config, mock_proxy):
        ...
        response = self._post(self._foreign_picker_payload(), host="us.hanzo.ai")
    ...
    def test_eu_no_local_proxies_to_us(self, mock_config, mock_proxy):
        ...
        response = self._post(self._foreign_picker_payload(), host="eu.hanzo.ai")
    ...
    def test_proxy_failure_returns_502_for_actions(self, mock_config, mock_proxy):
        ...
        response = self._post(self._foreign_action_payload(), host="us.hanzo.ai")
    ...
    def test_terminate_hints_local_handles(self, mock_config, mock_sync_connect, _mock_asyncio_run, mock_proxy):
        ...
        response = self._post(payload, host="us.hanzo.ai")
...
class TestSignalsDismissReport(TestCase):
    ...
    def test_dismiss_suppresses_report_and_writes_artefact(self, mock_config, mock_requests_post, mock_is_org_member):
        ...
        # Attributed to the resolved Insights user, not system.
    ...
    def test_dismiss_refuses_non_org_member(self, mock_config, mock_requests_post, mock_is_org_member):
        ...
        mock_config.return_value = {"SLACK_APP_SIGNING_SECRET": self.signing_secret}
        mock_is_org_member.return_value = None  # clicker is not a Insights org member
        report = self._make_ready_report()
...
class TestInsightAlertSnooze(TestCase):
                ...
                "blocks": [
                    {
                        ...
                        "elements": [
                            ...
                            {"type": "button", "action_id": "view_alert", "url": "https://app.hanzo.ai/alerts/1"},
                        ],
                    }
                ],
            },
    ...
    def test_snooze_refuses_inactive_user(self, mock_config, mock_requests_post, mock_get_email):
        # Exercises the real resolve_insights_user_from_event path (not _is_org_member mocked
