...
class TestGetFlowBatchTriggerLimit(TestCase):
    @override_settings(
        Flow_BATCH_TRIGGER_LIMIT=5000,
        Flow_BATCH_TRIGGER_LIMIT_ELEVATED=50000,
        Flow_BATCH_TRIGGER_ELEVATED_TEAM_IDS={2, 99},
    )
    def test_returns_default_for_unlisted_team(self):
        assert get_flow_batch_trigger_limit(1) == 5000
        assert get_flow_batch_trigger_limit(7) == 5000
    ...
    def test_returns_elevated_for_listed_team(self):
        assert get_flow_batch_trigger_limit(2) == 50000
        assert get_flow_batch_trigger_limit(99) == 50000
    ...
    def test_returns_default_when_elevated_set_is_empty(self):
        assert get_flow_batch_trigger_limit(2) == 5000

    @override_settings(
        Flow_BATCH_TRIGGER_LIMIT=123,
        Flow_BATCH_TRIGGER_LIMIT_ELEVATED=456,
        Flow_BATCH_TRIGGER_ELEVATED_TEAM_IDS={42},
    )
    def test_returns_currently_configured_values(self):
        ...
        assert get_flow_batch_trigger_limit(42) == 456
        assert get_flow_batch_trigger_limit(1) == 123
