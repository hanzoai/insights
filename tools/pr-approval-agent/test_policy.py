...
def _ownership_wrong_locator_for_format(d: dict) -> None:
    d["ownership"]["sources"][0] = {"format": "insightscli-resolver", "glob": "products/*/product.yaml"}
...
def test_resolve_no_folder_file_uses_global() -> None:
    eff = resolve(gates.POLICY, ["insights/api/insight.py"])
    assert [s.path for s in eff.scopes] == [None]
    assert _scope(eff, None).max_files == gates.MAX_FILES
    assert eff.invalid_folder_files == ()
...
def test_size_gate_applies_mixed_leniency(n_global: int, expected_ok: bool) -> None:
    ...
    global_files = [{"filename": f"insights/api/m{i}.py", "additions": 5, "deletions": 0} for i in range(n_global)]

    pipeline = review_pr.Pipeline(pr_number=1, repo="Insights/insights")
    pipeline.pr = PRData(
        number=1,
