...
from insights.datastore.query_tagging import Feature, get_query_tags, reset_query_tags, tag_queries
...
from products.web_analytics.backend.insightsql_queries.web_lazy_precompute_common import is_background_warming_request
...
class TestCanonicalizeLazyReplay(BaseTest):
    ...
    def test_variant_fields_dropped_shape_fields_kept(self) -> None:
        query = {
            "kind": "WebStatsTableQuery",
            "useWebAnalyticsPrecompute": True,
            "properties": [{"key": "$host", "value": "hanzo.ai", "type": "event"}],
            "breakdownBy": "Browser",
...
class TestFleetQuerySelection(BaseTest):
    ...
    def test_selection_sql_survives_driver_percent_formatting(self, mock_exec: MagicMock) -> None:
        # datastore_driver %-formats the query when params are passed, so literal
...
class TestWarmableQueriesCaching(BaseTest):
    ...
    def test_second_run_reuses_cached_selection(self, mock_exec: MagicMock, _storage: _FakeObjectStorage) -> None:
        ...
        # again — this fails when the second run re-hits Datastore.
...
class TestWarmQueriesOp(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("transient_202_recovers", [DatastoreAtCapacity(), None], 2, False),
            (
                "persistent_202_fails",
                [DatastoreAtCapacity(), DatastoreAtCapacity(), DatastoreAtCapacity()],
                3,
                True,
            ),
        ]
    ...
    @parameterized.expand(
        [
            # A dead Datastore node can block every pool thread in a socket
    ...
    def test_duplicate_cache_keys_warm_once(self) -> None:
        ...
        # both be selected; replaying both wastes Datastore capacity and
