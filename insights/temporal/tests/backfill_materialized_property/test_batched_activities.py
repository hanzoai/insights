...
class TestBuildDictBackedUpdateCommand:
    def test_emits_one_set_clause_per_column_with_dict_dispatch(self):
        ...
        assert f"dictGetString('{settings.DATASTORE_DATABASE}.dmat_slot_assignments_dict', 'property_name'," in command
    ...
    def test_where_clause_uses_in_subselect_against_dict_table(self):
        ...
        assert (
            f"WHERE team_id IN (SELECT DISTINCT team_id FROM {settings.DATASTORE_DATABASE}.dmat_slot_assignments)"
            in command
        )
...
class TestActivateAndFailSlots:
    ...
    def test_fail_slots_records_error_message(self, team, activity_environment):
        ...
        failed = activity_environment.run(
            fail_slots,
            FailSlotsInputs(slot_ids=[str(slot.id)], error_message="Datastore OOM"),
        )

        assert failed == 1
        slot.refresh_from_db()
        assert slot.state == MaterializedColumnSlotState.ERROR
        assert slot.error_message == "Datastore OOM"


@pytest.mark.django_db
class TestRunBatchedMutation:
    @patch("insights.temporal.backfill_materialized_property.activities.AlterTableMutationRunner")
    @patch("insights.temporal.backfill_materialized_property.activities.get_cluster")
...
class TestPopulateSlotAssignments:
    ...
    def _fake_cluster_with_hosts(self, host_count: int = 3, fail_on_host: int | None = None) -> MagicMock:
        """Build a MagicMock DatastoreCluster whose `map_all_hosts(fn)` invokes `fn` once per
