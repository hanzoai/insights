...
these preserve InsightsQL semantics rather than blocking (see ``_REDSHIFT_ONLY_HANDLERS``).
"""
...
# Lowercased InsightsQL/Datastore function names whose Postgres rendering the Redshift engine rejects.
...
def _handle_concat(args: list[str]) -> str:
    # Redshift's CONCAT is strictly two-argument and NULL-propagating; InsightsQL's concat is
...
def _handle_position(args: list[str]) -> str:
    ...
    # STRPOS, which shares InsightsQL's (haystack, needle) argument order and 1-based result.
    if len(args) != 2:
...
def _handle_avg(args: list[str]) -> str:
    # Redshift's avg over integer inputs returns a truncated integer; InsightsQL's avg is a float.
...
# rewritten to a Redshift-native equivalent that keeps InsightsQL semantics.
