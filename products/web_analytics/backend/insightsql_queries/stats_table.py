...
from insights.datastore.query_tagging import clear_tag, get_query_tag_value
...
from products.web_analytics.backend.insightsql_queries.stats_table_pre_aggregated import StatsTablePreAggregatedQueryBuilder
from products.web_analytics.backend.insightsql_queries.stats_table_strategies import (
    ChannelTypeStrategy,
...
from products.web_analytics.backend.insightsql_queries.web_analytics_query_runner import WebAnalyticsQueryRunner, map_columns
...
class WebStatsTableQueryRunner(WebAnalyticsQueryRunner[WebStatsTableQueryResponse]):
    ...
    def datastore_query_type(self) -> str:
    ...
    def _uses_session_fields(self) -> bool:
        ...
        predicate. Routing them to the no-join strategy would make InsightsQL
    ...
    def _current_period_aggregate(self, function_name, column_name):
        ...
        return self.period_aggregate(
            function_name,
            column_name,
            self.query_date_range.date_from_as_insightsql(),
            self.query_date_range.date_to_as_insightsql(),
        )
    ...
    def _previous_period_aggregate(self, function_name, column_name):
        ...
        return self.period_aggregate(
            function_name,
            column_name,
            self.query_compare_to_date_range.date_from_as_insightsql(),
            self.query_compare_to_date_range.date_to_as_insightsql(),
        )
...
WHERE and(
    ...
    def _build_response_from_lazy(self, result: LazyStatsResult) -> WebStatsTableQueryResponse:
        ...
        Ordering, filtering, and pagination already happened in Datastore — see
    ...
    def _maybe_calculate_via_lazy_precompute(self) -> Optional[WebStatsTableQueryResponse]:
        ...
        falls through to the v2/raw InsightsQL path.
        """
    ...
    def _maybe_calculate_via_frustration_lazy_precompute(self) -> Optional[WebStatsTableQueryResponse]:
        ...
        Returns None when ineligible or on any failure, in which case the caller
        falls through to the live InsightsQL path.
        """
    ...
    def _calculate(self):
        ...
        # to prevent InsightsQL from automatically converting DateTime fields to team timezone
        modifiers = self.modifiers
        if self.used_preaggregated_tables:
            modifiers = self.modifiers.model_copy() if self.modifiers else InsightsQLQueryModifiers()
        ...
        response = self.paginator.execute_insightsql_query(
            query_type=self.datastore_query_type(),
            query=query,
            team=self.team,
            user=self.user,
