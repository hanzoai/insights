...
from insightscli_commands.doctor import (
...
def _neutralize_compose_project(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    # Several tests in this module hardcode "insights"-prefixed volume/project
    ...
    monkeypatch.setenv("COMPOSE_PROJECT_NAME", "insights")
    ...
    monkeypatch.setattr("insightscli_commands.doctor._MIGRATION_LOCK_DIR", tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("insightscli doctor", id="insightscli"),
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "/nix/store/abc123/bin/node --require /Users/x/code/github/insights/node_modules/.pnpm/tsx@4.20.5/node_modules/tsx/dist/preflight.cjs src/index.ts",
            id="node-with-code-in-path",
        ),
        pytest.param(
            "python /Users/x/code/github/insights/manage.py runserver",
            id="python-with-code-in-path",
        ),
        pytest.param(
            "granian asgi 127.0.0.1:8000 insights.asgi:application",
            id="granian",
        ),
        pytest.param(
            "celery -A insights worker",
            id="celery",
        ),
        pytest.param(
            "/Users/x/code/github/insights/rust/target/debug/capture",
            id="rust-capture",
        ),
    ],
)
def test_is_excluded_does_not_match_insights_processes(args: str) -> None:
...
class _FakeManifest:
    """Minimal stand-in for ``insightscli.manifest.Manifest`` (structural match)."""
...
def test_collect_import_targets_extracts_click_and_boot_modules() -> None:
    manifest = _FakeManifest(
        {
            "doctor": {"click": "insightscli_commands.doctor:doctor"},
            "doctor:report": {"click": "insightscli_commands.doctor:doctor_report"},
            "noclick": {"cmd": "echo hi"},
            "bad:format": {"click": "no_colon_here"},
        },
        boot_modules=["insightscli_commands.prechecks"],
    )
    targets = _collect_import_targets(manifest)

    assert ("doctor", "insightscli_commands.doctor", "doctor") in targets
    assert ("doctor:report", "insightscli_commands.doctor", "doctor_report") in targets
    # Boot modules import the module with no attribute to resolve.
    assert ("insightscli_commands.prechecks", "insightscli_commands.prechecks", None) in targets
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"ghost": {"click": "insightscli_commands.does_not_exist:thing"}},
            ("ghost", "ModuleNotFoundError"),
            id="missing-module",
        ),
        pytest.param(
            {"typo": {"click": "insightscli_commands.doctor:not_a_real_command"}},
...
def test_get_process_cwds_skips_lsof_for_empty_input(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fail)
...
def test_get_process_cwds_parses_and_batches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
...
def test_binary_arches_parses_file_output(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "insightscli_commands.doctor._run_output",
...
def test_binary_arches_empty_when_file_unavailable(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("insightscli_commands.doctor._run_output", lambda *_: None)
...
def _patch_phrocs(
    ...
    *,
    ...
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: which)
    monkeypatch.setattr("insightscli_commands.doctor.platform.machine", lambda: machine)

    def fake_run_output(cmd: list[str], *_a: object, **_k: object) -> str | None:
        return file_out if cmd[0] == "file" else version

    monkeypatch.setattr("insightscli_commands.doctor._run_output", fake_run_output)
...
def test_phrocs_runtime_pairs_reports_state(tmp_path: Path) -> None:
    config = tmp_path / ".insights" / ".generated" / "mprocs.yaml"
...
def test_phrocs_runtime_pairs_flags_missing_config(tmp_path: Path) -> None:
    ...
    assert "insightscli dev:generate" in pairs["generated_config"]
...
def test_sanitize_compose_name_strips_shell_metacharacters() -> None:
    ...
    assert _sanitize_compose_name("insights-evil$(touch /tmp/pwned);rm -rf /") == "insights-eviltouchtmppwnedrm-rf"


def _fake_docker_ps(
    monkeypatch: pytest.MonkeyPatch,
    port_scan_stdout: str = "",
    datastore_stdout: str = "",
    ...
    def fake_run(cmd: list[str], **_kwargs: object) -> SimpleNamespace:
        ...
        if cmd[:3] == ["docker", "ps", "-a"]:
            return SimpleNamespace(returncode=0, stdout=datastore_stdout, stderr="")
...
def test_scan_port_holders_own_project_is_not_flagged_foreign(monkeypatch: pytest.MonkeyPatch) -> None:
    _fake_docker_ps(monkeypatch, port_scan_stdout="datastore|insights|127.0.0.1:8123->8123/tcp")
    holders = {h.port: h for h in _scan_port_holders()}
    assert holders[8123].project == "insights"


def test_scan_port_holders_sanitizes_malicious_project_label(monkeypatch: pytest.MonkeyPatch) -> None:
    _fake_docker_ps(monkeypatch, port_scan_stdout="evilbox|insights-evil$(rm -rf /)|127.0.0.1:8010->8000/tcp")
    holders = {h.port: h for h in _scan_port_holders()}
    assert holders[8010].project == "insights-evilrm-rf"


def test_insights_shaped_projects_filters_by_datastore_label(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # datastore container is stopped, but another service still holds a
    # port, is still offered for teardown — not just one whose CH is running.
    def fake_run(cmd: list[str], **_kwargs: object) -> SimpleNamespace:
        assert "label=com.docker.compose.service=datastore" in cmd
        return SimpleNamespace(returncode=0, stdout="ch1|stopped-proj\n", stderr="")

    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
    result = _insights_shaped_projects({"stopped-proj", "unrelated-proj"})
...
def test_scan_unheld_via_lsof_disambiguates_port_and_range(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/lsof")

    holders = {h.port: h for h in _scan_unheld_via_lsof([(9000, "datastore-native"), (19000, "objectstorage")])}
...
def test_confirm_stack_teardown_times_out_to_no(monkeypatch: pytest.MonkeyPatch) -> None:
    # Runs on every `insightscli start`; if the timeout path regresses, a piped or
    # abandoned terminal hangs every developer's startup indefinitely.
    monkeypatch.setattr("insightscli_commands.doctor.select.select", lambda *_a, **_k: ([], [], []))
...
def test_confirm_stack_teardown_accepts_yes(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("insightscli_commands.doctor.select.select", lambda *a, **_k: (a[0], [], []))
    monkeypatch.setattr("insightscli_commands.doctor.sys.stdin.readline", lambda: "y\n")
...
def test_doctor_ports_silent_when_nothing_foreign(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/docker")
    _fake_docker_ps(monkeypatch, port_scan_stdout="datastore|insights|127.0.0.1:8123->8123/tcp")
...
def test_doctor_ports_reports_foreign_stack_noninteractively(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _fake_docker_ps(
        ...
        datastore_stdout="ch1|foreign-proj\n",
    )
...
def test_doctor_ports_teardown_respects_confirmation(
    ...
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/docker")
    monkeypatch.setattr("insightscli_commands.doctor.sys.stdin.isatty", lambda: True)
    monkeypatch.setattr("insightscli_commands.doctor.sys.stdout.isatty", lambda: True)
    monkeypatch.setattr("insightscli_commands.doctor._confirm_stack_teardown", lambda *_a, **_k: confirmed)
...
# datastore service has one named-volume mount plus several bind mounts for
# config files; zookeeper has three named-volume mounts.
_DATASTORE_MOUNTS = _mounts_json(
    [
        ("/idl", "", "bind"),
        ("/etc/datastore-server/config.xml", "", "bind"),
        ("/var/lib/datastore", "old-ch-vol", "volume"),
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param({"datastore": []}, "datastore", "insights", None, id="no-containers"),
        pytest.param({"datastore": ["ch1"]}, "datastore", "insights", "ch1", id="single-match"),
        pytest.param({"datastore": ["ch1", "ch2"]}, "datastore", "insights", None, id="ambiguous-two-matches"),
    ],
)
...
def test_find_service_container_ignores_matches_from_other_projects(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
    assert _find_service_container("insights", "datastore") is None
    ...
    [
        pytest.param(
            [
                ...
                {"Destination": "/var/lib/datastore", "Name": "old-ch-vol", "Type": "volume"},
            ],
            "/var/lib/datastore",
            "old-ch-vol",
        ...
        pytest.param(
            [{"Destination": "/var/lib/datastore", "Name": "/host/path", "Type": "bind"}],
            "/var/lib/datastore",
            None,
...
@pytest.mark.parametrize(
    ("run_output", "expected"),
    [
        pytest.param(_DATASTORE_MOUNTS, json.loads(_DATASTORE_MOUNTS), id="valid-mounts-json"),
...
def test_container_mounts(
    ...
    monkeypatch.setattr("insightscli_commands.doctor._run_output", lambda *_a, **_k: run_output)
...
def test_copy_volume(monkeypatch: pytest.MonkeyPatch, returncode: int, expected: bool) -> None:
    ...
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
    monkeypatch.setattr("insightscli_commands.doctor.os.getpid", lambda: 4242)
    ...
    assert calls[0] == [
        ...
        "--rm",
        "--name",
        "insightscli-migrate-volumes-4242",
        "-v",
        "old-vol:/from:ro",
        "-v",
        "new-vol:/to",
    ...
    if not expected:
        assert calls[1] == ["docker", "rm", "-f", "insightscli-migrate-volumes-4242"]


def _fake_migrate_dispatcher(
    *,
    project: str = "insights",
    named_volume_exists: bool = False,
    datastore_ids: Sequence[str] = ("ch1",),
    ...
    """
    calls: list[list[str]] = []
    mounts_by_container = mounts_by_container or {"ch1": _DATASTORE_MOUNTS, "zk1": _ZOOKEEPER_MOUNTS}
    ...
    def fake_run(cmd: list[str], **_kwargs: object) -> SimpleNamespace:
        ...
        if cmd[:3] == ["docker", "ps", "-a"]:
            ...
            ids = datastore_ids if service == "datastore" else zookeeper_ids
            return SimpleNamespace(returncode=0, stdout="\n".join(f"{cid}|{project}" for cid in ids), stderr="")
...
def test_doctor_migrate_volumes_noop_when_already_migrated(monkeypatch: pytest.MonkeyPatch) -> None:
    # This runs on every `insightscli start`; once the named volume exists, it must
    ...
    # check must not rely on datastore-data alone, or a migration interrupted
    ...
    assert calls == [
        [
            ...
            "volume",
            "inspect",
            "insights_datastore-data",
            "insights_zookeeper-data",
            "insights_zookeeper-datalog",
            "insights_zookeeper-logs",
        ]
    ]


def test_doctor_migrate_volumes_noop_on_fresh_install(monkeypatch: pytest.MonkeyPatch) -> None:
    # No named volumes yet *and* no prior datastore container or postgres
    # volume means there was never any old data — must stay silent, not warn
    # a brand new checkout.
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/docker")
    fake_run, calls = _fake_migrate_dispatcher(named_volume_exists=False, datastore_ids=(), zookeeper_ids=())
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)

    result = CliRunner().invoke(doctor_migrate_volumes, [])
    assert result.exit_code == 0
    assert result.output == ""
    # dest-volumes inspect + datastore existence check + prior-install (postgres) check
    assert len(calls) == 3
...
def test_doctor_migrate_volumes_warns_when_stack_was_stopped(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # who stopped the stack (rather than never having one) has no datastore
    # container to salvage from, but still has real prior data. Silence here
    # would mean an empty Datastore with no explanation.
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/docker")
    fake_run, calls = _fake_migrate_dispatcher(named_volume_exists=False, datastore_ids=(), zookeeper_ids=())

    def with_prior_install(cmd: list[str], **kwargs: object) -> SimpleNamespace:
        if cmd[:3] == ["docker", "volume", "inspect"] and cmd[3:] == ["insights_postgres-15-data"]:
            return SimpleNamespace(returncode=0, stdout="[]", stderr="")
        return fake_run(cmd, **kwargs)

    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", with_prior_install)

    result = CliRunner().invoke(doctor_migrate_volumes, [])
    assert result.exit_code == 0
    assert "Switching Datastore/ZooKeeper to named docker volumes" in result.output
...
def test_doctor_migrate_volumes_happy_path_migrates_both_services(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = CliRunner().invoke(doctor_migrate_volumes, [])
    assert result.exit_code == 0
    assert "Migrated Datastore/ZooKeeper data" in result.output
    ...
    # explicit grace period gives Datastore time to flush a multi-GB dataset;
    ...
    assert [
        ...
        "create",
        "--label",
        "com.docker.compose.project=insights",
        "--label",
        "com.docker.compose.volume=datastore-data",
        "insights_datastore-data",
    ] in calls
...
@pytest.mark.parametrize(
    ("datastore_ids", "zookeeper_ids", "mounts_by_container"),
    [
        pytest.param(("ch1",), (), None, id="zookeeper-container-already-removed"),
        pytest.param(("ch1", "ch2"), ("zk1",), None, id="ambiguous-datastore-containers"),
        pytest.param(
            ("ch1",),
            ("zk1",),
            {
                "ch1": _DATASTORE_MOUNTS,
...
def test_doctor_migrate_volumes_falls_back_without_touching_anything(
    ...
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/docker")
    fake_run, calls = _fake_migrate_dispatcher(
        datastore_ids=datastore_ids, zookeeper_ids=zookeeper_ids, mounts_by_container=mounts_by_container
    )
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
...
def test_doctor_migrate_volumes_copy_failure_rolls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    # Datastore and zookeeper-data copy cleanly, but zookeeper-datalog fails
    # verification. Restoring only datastore+zookeeper-data would leave a
    # replicated table with "replica already exists" once Datastore starts
    # against restored state but ZooKeeper doesn't know about it.
    monkeypatch.setattr("insightscli_commands.doctor.shutil.which", lambda _: "/usr/bin/docker")
    fake_run, calls = _fake_migrate_dispatcher(copy_returncode_by_dest={"insights_zookeeper-datalog": 1})
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", fake_run)
    ...
    assert not any(c[:2] == ["docker", "run"] and "insights_zookeeper-logs:/to" in c for c in calls)


def test_doctor_migrate_volumes_copy_timeout_rolls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    # A large Datastore volume on a slow disk can exceed the copy timeout.
    ...
    def timing_out_run(cmd: list[str], **kwargs: object) -> SimpleNamespace:
        if cmd[:2] == ["docker", "run"] and "insights_datastore-data:/to" in cmd:
            raise subprocess.TimeoutExpired(cmd=cmd, timeout=120)
        return fake_run(cmd, **kwargs)

    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", timing_out_run)

    result = CliRunner().invoke(doctor_migrate_volumes, [])
    assert result.exit_code == 0
    assert "Switching Datastore/ZooKeeper to named docker volumes" in result.output
    assert ["docker", "volume", "rm", "-f", "insights_datastore-data"] in calls
    assert ["docker", "start", "ch1", "zk1"] in calls
...
def test_doctor_migrate_volumes_interrupt_rolls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("insightscli_commands.doctor.subprocess.run", interrupted_run)
...
def test_run_ok_echoes_stderr_on_failure(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    # generic warning.
    monkeypatch.setattr(
        "insightscli_commands.doctor.subprocess.run",
...
def test_doctor_migrate_volumes_serializes_across_concurrent_worktrees(
    ...
    lock_path = tmp_path / "insightscli-migrate-volumes-insights.lock"
...
def test_doctor_migrate_volumes_refuses_symlinked_lock_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    # developer's account. Runs on every `insightscli start`, so this must stay a
    ...
    (tmp_path / "insightscli-migrate-volumes-insights.lock").symlink_to(victim)
