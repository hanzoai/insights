...
description: >
  ...
  on either Insights data-warehouse views (InsightsQL) or an external dbt project. Use when the user wants to
...
**Insights:** a view that, per unit, flags whether the activation criteria were met within N days of the first
event, plus time-to-activate; then an activation-rate rollup by signup cohort. Recipes:
[`references/insights/activation_flag.sql`](references/insights/activation_flag.sql),
[`activation_retention_lift.sql`](references/insights/activation_retention_lift.sql). Materialize the cohort
...
`querying-insights-data` (InsightsQL + the semantic-layer check for an approved activation definition).
