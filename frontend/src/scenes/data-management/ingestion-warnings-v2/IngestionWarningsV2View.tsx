...
import type { TagType } from 'lib/elements/Tag/Tag'
    ...
    return (
        <Tooltip title={onClick ? `Only show ${severity} warnings` : severity.toUpperCase()}>
            <div className="flex items-center gap-2">
                ...
                <Tag type={SEVERITY_TO_TAG_TYPE[severity] ?? 'muted'} onClick={onClick} forceClickable={!!onClick}>
                    {severity}
                </Tag>
            </div>
        </Tooltip>
...
function ActiveFilterTags(): JSX.Element | null {
    ...
    return (
        <div className="flex items-center gap-1 text-muted text-xs">
            Filtered by:
            {filters.category && (
                <Tag type="highlight" closable onClose={() => setFilters({ category: null })}>
                            ...
                            },
                            {
                                ...
                                render: function Render(_, summary: IngestionWarningV2Summary) {
                                    ...
                                    const docsUrl = docsAnchor
                                        ? `https://hanzo.ai/docs/data/ingestion-warnings#${docsAnchor}`
                                        : 'https://hanzo.ai/docs/data/ingestion-warnings'
                                    return (
