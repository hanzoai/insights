...
from insights.temporal.messaging.filter_storage import combine_filter_bytecodes, store_filters
...
class TestFlushKafkaBatchAsync:
    ...
    async def test_successful_batch_flush_async(self):
        ...
        with patch(
            "insights.temporal.messaging.backfill_precalculated_person_properties_workflow.asyncio.to_thread"
        ) as mock_thread:
...
class TestBackfillPrecalculatedPersonPropertiesActivity:
    ...
    @pytest.mark.asyncio
    async def test_activity_parameterizes_property_keys_in_datastore_query(self):
        ...
        original_prepare_and_print_ast = __import__(
            "insights.insightsql.printer", fromlist=["prepare_and_print_ast"]
        ...
        with (
            patch(
                "insights.temporal.messaging.backfill_precalculated_person_properties_workflow.get_filters_and_properties",
                return_value=(filters, [malicious_property], combine_filter_bytecodes(filters)),
            ),
            patch(
                "insights.temporal.messaging.datastore_concurrency.get_client",
                return_value=_AsyncClientContextManager(mock_client),
            ),
            patch(
                "insights.temporal.messaging.insightsql_compile.prepare_and_print_ast",
                side_effect=capturing_prepare_and_print_ast,
            ),
            patch("insights.temporal.messaging.backfill_precalculated_person_properties_workflow.get_producer"),
            patch(
                "insights.temporal.messaging.backfill_precalculated_person_properties_workflow.Heartbeater",
                _NoopHeartbeater,
            ),
            patch(
                "insights.temporal.messaging.backfill_precalculated_person_properties_workflow.get_person_properties_backfill_success_metric",
                return_value=Mock(),
            ),
        ):
        ...
        assert "node" in captured_ast, "compile_insightsql_for_streaming was never called"
    ...
    async def test_activity_falls_back_to_full_properties_for_percent_keys(self):
        ...
        # '%' is the one character InsightsQL refuses as an identifier, so the optimized per-property
...
class TestActivityRowConsumption:
    ...
    def _module(self, name: str) -> str:
        return f"insights.temporal.messaging.backfill_precalculated_person_properties_workflow.{name}"
    ...
    async def _run_with_rows(self, person_properties, rows):
        ...
        with (
            ...
            patch(self._module("compile_insightsql_for_streaming"), side_effect=compile_stub),
            patch(
                "insights.temporal.messaging.datastore_concurrency.get_client",
                return_value=_AsyncClientContextManager(mock_client),
            ),
...
class TestEvaluateCombinedFiltersSync:
    ...
    @patch("insights.temporal.messaging.backfill_precalculated_person_properties_workflow.LOGGER")
    def test_detailed_logging(self, _name, detailed, expected_result, expect_info, expect_warning, mock_logger):
        ...
        if expect_info:
            ...
            assert call_args[0][0] == "ScriptVM evaluation completed"
        ...
        if expect_warning:
            ...
            assert call_args[0][0] == "ScriptVM evaluation returned non-dict result"
        else:
