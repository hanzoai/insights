...
worker on ``settings.MCPA_TASK_QUEUE`` (provided locally by ``insightscli start``).
"""
...
from insights.temporal.mcp_analytics.intent_clustering.constants import CHILD_WORKFLOW_ID_PREFIX, WORKFLOW_NAME
from insights.temporal.mcp_analytics.intent_clustering.models import IntentClusteringWorkflowInputs
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            ...
            type=int,
            default=None,
            help="Window for both the session and Datastore queries. Default uses the task default (7 days).",
        )
        parser.add_argument(
            "--raw",
            action="store_true",
