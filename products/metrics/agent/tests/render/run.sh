...
new_case_dir
run_render minimal INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=app:9090
...
if [ "$UPDATE" != "1" ]; then
    ...
    tele=$(env -i PATH="$PATH" CONFIG_DIR="$CASE_DIR" RENDER_ONLY=1 \
        INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=app:9090 \
...
new_case_dir
run_render multi INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS='app:9090, worker:9091 ,,db-exporter:9187'

new_case_dir
run_render debug INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=app:9090 INSIGHTS_DEBUG=1

# Single quotes in a target are doubled so the rendered YAML stays valid.
new_case_dir
run_render quoted-target INSIGHTS_API_KEY=pk-test "SCRAPE_TARGETS=app's-host:9090"

# PERSIST_QUEUE backs the export queue with disk so a restart during a
# Insights outage loses nothing.
new_case_dir
run_render persist-queue INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=app:9090 PERSIST_QUEUE=1
...
new_case_dir
run_render sharded INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS='app:9090, worker:9091' SHARD_COUNT=4 SHARD_INDEX=2

# The shard index falls back to the trailing ordinal of the hostname
# (StatefulSet pods are named <name>-<ordinal>).
new_case_dir
run_render sharded-hostname INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS='app:9090, worker:9091' SHARD_COUNT=4 HOSTNAME=insights-metrics-agent-3

new_case_dir
expect_failure shard-index-out-of-range 'SHARD_INDEX must be less than SHARD_COUNT' INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=app:9090 SHARD_COUNT=2 SHARD_INDEX=2

new_case_dir
expect_failure shard-index-underivable 'SHARD_INDEX' INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=app:9090 SHARD_COUNT=2 HOSTNAME=nodigits
...
- job_name: 'custom'
  ...
  static_configs:
      - targets: ['legacy:8080']
EOF
run_render mounted-scrape-configs INSIGHTS_API_KEY=pk-test
...
new_case_dir
expect_failure missing-targets SCRAPE_TARGETS INSIGHTS_API_KEY=pk-test

new_case_dir
expect_failure empty-targets SCRAPE_TARGETS INSIGHTS_API_KEY=pk-test SCRAPE_TARGETS=' , '
