...
export class PersonHogPersonOperations {
    ...
    async fetchPersonsByDistinctIds(
        ...
        for (let i = 0; i < teamPersons.length; i += PERSONFN_BATCH_SIZE) {
            const batch = teamPersons.slice(i, i + PERSONFN_BATCH_SIZE)
    ...
    async fetchPersonsByPersonIds(
        ...
        const allPersons = await Promise.all(
            [...byTeam].map(async ([teamId, uuids]) => {
                ...
                for (let i = 0; i < uuids.length; i += PERSONFN_BATCH_SIZE) {
                    const batch = uuids.slice(i, i + PERSONFN_BATCH_SIZE)
