...
from insights.schema import DataWarehouseManagedViewsetKind
...
from insights.rate_limit import MaterializationRateThrottle, RunSavedQueryRateThrottle
from insights.rbac.access_control_api_mixin import AccessControlViewSetMixin
from insights.rbac.query_access import assert_user_can_read_query
...
class DataWarehouseSavedQuerySerializer(
    ...
    @extend_schema_field(
        {
            "type": "object",
            "properties": {
                "kind": {"type": "string", "enum": ["InsightsQLQuery"], "default": "InsightsQLQuery"},
                "query": {"type": "string"},
            },
            "required": ["query"],
        }
    ...
    query = QueryDefinitionField(
        help_text='InsightsQL query definition as a JSON object with a "query" key containing the SQL string and a "kind" key (always "InsightsQLQuery"). Format the SQL string multi-line with indentation and inline `--` comments for non-obvious logic — the SQL editor renders it verbatim, so avoid minified single-line SQL. Example: {"kind": "InsightsQLQuery", "query": "SELECT\\n    event,\\n    count() AS cnt\\nFROM events\\nGROUP BY event\\nLIMIT 100"}',
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "name": {
                "help_text": "Unique name for the view. Used as the table name in InsightsQL queries and the node name in the data modeling Node.",
            },
        }
    ...
    def update(self, instance: Any, validated_data: Any) -> Any:
        ...
        if sync_frequency and hanzo_insights.feature_enabled(
    ...
    def validate_query(self, query):
        if not isinstance(query, dict):
            raise exceptions.ValidationError(
                detail=(
                    ...
                    'Example: {"kind": "InsightsQLQuery", "query": "SELECT * FROM events WHERE timestamp >= now() - INTERVAL 7 DAY LIMIT 100"}'
                )
            )
        ...
        context = InsightsQLContext(team_id=team_id, user=user, enable_select_queries=True)
        try:
            select_ast = parse_select(query["query"])

            find_placeholders = FindPlaceholders()
            find_placeholders.visit(select_ast)
        except ExposedInsightsQLError as err:
    ...
    def validate_name(self, name):
        ...
        # has_table covers system/insights tables and warehouse objects the requesting user can see; it's
...
class DataWarehouseSavedQueryViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, viewsets.ModelViewSet):
    ...
    def _sync_frequency_managed_by_dag(self) -> bool:
        ...
        v2_enabled = hanzo_insights.feature_enabled(
            "data-modeling-backend-v2",
            str(self.team.uuid),
