...
from insights.models import OAuthAccessToken, Team
...
def _repo_config(team_id: int, *, digest_enabled: bool = True) -> StampRepoConfig:
    ...
    return StampRepoConfig.objects.for_team(team_id).create(
        team_id=team_id,
        repository=REPO,
...
def _make_pr_with_review(
    team_id: int, repo_config: StampRepoConfig, *, number: int, author: str, approved_at_sha: str | None
) -> PullRequest:
...
def test_signed_webhook_drives_review_and_posts_approval(team, stamp_chain: StampChain) -> None:
    ...
    recorder = stamp_chain.recorder
    author, head_sha = "devex-dev", "sha101a"
    recorder.register_pr(REPO, 101, _pr_object(101, author, head_sha), _pr_files())
    recorder.policy_files[".stamp/policy.yml"] = "version: 1\n"

    status = stamp_chain.post_webhook(_opened_event(101, author, head_sha), delivery_id=str(uuid.uuid4()))
    assert status == 202
    ...
    # An APPROVED verdict never hands off to Review — the review label is a refusal-only signal.
    assert [w for w in recorder.github_writes if w["kind"] == "add_label"] == []


@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_sandbox_destroy_failure_does_not_mask_a_completed_review(team, stamp_chain: StampChain) -> None:
    ...
    recorder.policy_files[".stamp/policy.yml"] = "version: 1\n"
    stamp_chain.sandbox_class.destroy_error = RuntimeError("sandbox teardown blew up")

    status = stamp_chain.post_webhook(_opened_event(109, author, head_sha), delivery_id=str(uuid.uuid4()))
    assert status == 202
...
def test_sandbox_gets_minted_short_lived_credential_and_closed_egress(
    ...
    with patch.dict(os.environ, worker_env):
        stamp_chain.post_webhook(_opened_event(110, "devex-dev", head_sha), delivery_id=str(uuid.uuid4()))

    config = stamp_chain.sandbox_class.created_configs[0]
    ...
    # internal_run:read is the server-mint provenance marker the gateway's stamp route demands
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_hosted_review_fails_closed_without_connecting_user(team, stamp_chain: StampChain) -> None:
    ...
    stamp_chain.post_webhook(_opened_event(111, "devex-dev", "sha111a"), delivery_id=str(uuid.uuid4()))
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_failed_run_still_dismisses_the_stale_approval_first(team, stamp_chain: StampChain) -> None:
    ...
    stamp_chain.post_webhook(_synchronize_event(113, "devex-dev", "sha113b"), delivery_id=str(uuid.uuid4()))
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_signal_review_started_posts_eyes_and_persists_reaction_id(team, stamp_chain: StampChain) -> None:
    # The moment a run commits to reviewing, it should show the same "review in flight" 👀 signal
    # STAMPFN_TRUSTED_REACTOR_BOTS reads off other reviewer bots — and the reaction id must be
    ...
    _run_activity(activities.signal_review_started, StampReviewInput(review_run_id=str(run.id), team_id=team.id))
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_signal_review_started_reaction_failure_does_not_fail_the_activity(team, stamp_chain: StampChain) -> None:
    ...
    result = _run_activity(
        activities.signal_review_started, StampReviewInput(review_run_id=str(run.id), team_id=team.id)
    )
...
def test_hosted_review_fails_closed_without_gateway_instead_of_anthropic_fallback(
    ...
    with patch.dict(os.environ, env_without_gateway, clear=True):
        stamp_chain.post_webhook(_opened_event(112, "devex-dev", "sha112a"), delivery_id=str(uuid.uuid4()))

    run = ReviewRun.objects.for_team(team.id).latest("created_at")
    assert run.status == ReviewRunStatus.FAILED
    assert "gateway" in (run.error or "").lower()
    assert not stamp_chain.sandbox_class.created_configs  # no sandbox was ever provisioned


@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_missing_policy_files_fall_back_to_server_defaults(team, stamp_chain: StampChain) -> None:
    # A target repo carrying neither .stamp/policy.yml nor review-guidance.md must still get a
    ...
    assert f"{STAMPFN_SANDBOX_REPO_DIR}/.stamp/steering.md" not in injected


@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_default_branch_steering_is_injected_into_sandbox(team, stamp_chain: StampChain) -> None:
    # A repo declaring .stamp/steering.md on its default branch must see it injected into the
    # checkout so the engine appends it to the reviewer guidance.
    _repo_config(team.id)
    recorder = stamp_chain.recorder
    recorder.policy_files[".stamp/steering.md"] = "Prefer squash merges.\n"
    ...
    injected = {path: payload.decode() for path, payload in stamp_chain.sandbox_writes}
    assert injected[f"{STAMPFN_SANDBOX_REPO_DIR}/.stamp/steering.md"] == "Prefer squash merges.\n"
...
def test_dismiss_stale_approvals(
    team,
    stamp_chain: StampChain,
    prior_head: str,
    prior_verdict: ReviewVerdict,
    prior_dismissed: bool,
    expect_dismissed: bool,
) -> None:
    # Every standing stamp approval must be dismissed and stamped when a new run starts —
    ...
    _run_activity(dismiss_stale_approvals, StampReviewInput(review_run_id=str(current.id), team_id=team.id))

    dismissals = [w for w in stamp_chain.recorder.github_writes if w["kind"] == "dismiss_review"]
    prior.refresh_from_db()
...
def test_dismiss_stale_approvals_github_side_sweep(
    ...
    stamp_chain.recorder.pr_reviews[(REPO, 130)] = [
        {
            "id": 6161,
            "state": review_state,
            "commit_id": "sha-old",
    ...
    with override_settings(STAMPFN_GITHUB_APP_SLUG=slug):
        result = _run_activity(
            dismiss_stale_approvals, StampReviewInput(review_run_id=str(current.id), team_id=team.id)
        )
...
@pytest.mark.parametrize(
    ...
    [
        ("stamp", "APPROVED", True, "stamp[bot]", "Bot", True),
        # Approval at a different commit doesn't cover this head — post fresh.
        ("stamp", "APPROVED", False, "stamp[bot]", "Bot", False),
        # A non-bot or wrong-login author isn't ours — post fresh.
        ("stamp", "APPROVED", True, "someone", "User", False),
        ("stamp", "APPROVED", True, "other-app[bot]", "Bot", False),
        # A dismissed review is not active — post fresh.
        ("stamp", "DISMISSED", True, "stamp[bot]", "Bot", False),
        # Slug unset: never adopt off a fuzzy match — post fresh.
        ("", "APPROVED", True, "stamp[bot]", "Bot", False),
    ],
    ids=[
...
def test_post_verdict_adopts_own_orphan_approval_instead_of_reposting(
    ...
    with override_settings(STAMPFN_GITHUB_APP_SLUG=slug):
        result = _run_activity(post_verdict, StampReviewInput(review_run_id=str(run.id), team_id=team.id))
...
def test_non_approve_terminal_sweeps_supersession_orphan(
    team, stamp_chain: StampChain, include_peer_live_approval: bool
) -> None:
    ...
    github_reviews = [
        {"id": 8181, "state": "APPROVED", "commit_id": "sha-old", "user": {"login": "stamp[bot]", "type": "Bot"}}
    ]
        ...
        github_reviews.append(
            {
                ...
                "state": "APPROVED",
                "commit_id": "sha-newer",
                "user": {"login": "stamp[bot]", "type": "Bot"},
            }
        )
    recorder.pr_reviews[(REPO, 150)] = github_reviews

    with override_settings(STAMPFN_GITHUB_APP_SLUG="stamp"):
        result = _run_activity(post_verdict, StampReviewInput(review_run_id=str(run.id), team_id=team.id))
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_failed_run_terminal_sweeps_supersession_orphan(team, stamp_chain: StampChain) -> None:
    ...
    recorder.pr_reviews[(REPO, 151)] = [
        {"id": 8282, "state": "APPROVED", "commit_id": "sha-old", "user": {"login": "stamp[bot]", "type": "Bot"}}
    ]
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_fetch_review_context_carries_inline_review_threads(team, stamp_chain: StampChain) -> None:
    ...
    _run_activity(fetch_review_context, StampReviewInput(review_run_id=str(run.id), team_id=team.id))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A multi-line raw error (e.g. a yaml.YAMLError echoing .stamp/policy.yml source lines) must
        # be reduced to its first line — run.error and the event are exposed to stamp:read, so the
...
def test_mark_review_failed_captures_failure_event(team, raw_error, expected_stored) -> None:
    ...
    # stamp_review_failed event next to the review-completed ones. The stored error is scrubbed to
    # its first line so raw exception text can't leak repo file content to stamp:read.
    repo_config = _repo_config(team.id)
    pull_request = PullRequest.objects.for_team(team.id).create(
        team_id=team.id, repo_config=repo_config, pr_number=101, author_login="devex-dev"
    )
    ...
    assert capture_fn.call_args.kwargs["event"] == "stamp_review_failed"
    assert capture_fn.call_args.kwargs["distinct_id"] == "devex-dev"
    props = capture_fn.call_args.kwargs["properties"]
    assert props["stamp_repo"] == REPO
    assert props["stamp_error"] == expected_stored


@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_reviewer_markdown_images_are_neutralized_before_posting(team, stamp_chain: StampChain) -> None:
    ...
    _run_activity(post_verdict, StampReviewInput(review_run_id=str(run.id), team_id=team.id))
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_approval_posted_while_losing_supersession_race_is_dismissed(team, stamp_chain: StampChain) -> None:
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_retry_after_head_move_never_rewrites_a_terminal_run(team, stamp_chain: StampChain) -> None:
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_bot_eyes_on_a_later_reactions_page_still_counts_as_in_flight(team, stamp_chain: StampChain) -> None:
    ...
    # stamp approve while that reviewer is still mid-review.
    repo_config = _repo_config(team.id)
    pull_request = PullRequest.objects.for_team(team.id).create(
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_mark_review_failed_dismisses_an_orphaned_approval(team, stamp_chain: StampChain) -> None:
    ...
    removals = [w for w in stamp_chain.recorder.github_writes if w["kind"] == "remove_reaction"]
...
def test_refused_verdict_strips_trigger_label_only_in_label_mode(
    team, stamp_chain: StampChain, review_mode: ReviewMode, expect_strip: bool
) -> None:
    ...
    repo_config.save()
    head_sha = "sha-refused"
    stamp_chain.recorder.register_pr(REPO, 101, _pr_object(101, "devex-dev", head_sha))
    pull_request = PullRequest.objects.for_team(team.id).create(
        team_id=team.id, repo_config=repo_config, pr_number=101, author_login="devex-dev"
    )
    run = ReviewRun.objects.for_team(team.id).create(
    ...
    run.refresh_from_db()
    assert run.verdict == ReviewVerdict.REFUSED
    label_removals = [w for w in stamp_chain.recorder.github_writes if w["kind"] == "remove_label"]
    if expect_strip:
        assert label_removals == [{"kind": "remove_label", "repo": REPO, "number": 101, "label": "stamp"}]
    else:
        assert label_removals == []

    # A refused PR hands off to Review by adding its trigger label, in both review modes —
    # stamp couldn't sign off, so a deeper second-opinion review is wanted regardless of how the
    # review was triggered.
    label_adds = [w for w in stamp_chain.recorder.github_writes if w["kind"] == "add_label"]
    assert label_adds == [{"kind": "add_label", "repo": REPO, "number": 101, "labels": ["review"]}]


@pytest.mark.parametrize(
    "fault",
    [
        # The client swallows a 422 (label missing on the repo) itself; a 500 raises StampGitHubError.
        pytest.param(422, id="swallows_missing_label"),
        pytest.param(500, id="client_raises_on_500"),
        # The egress layer raises these directly — not subclasses of StampGitHubError, so a narrow
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_refused_verdict_lands_even_when_review_handoff_fails(
    team,
    stamp_chain: StampChain,
    fault: int | Exception,
) -> None:
    # The Review handoff is a secondary, cross-product notification that runs AFTER the durable
    ...
    # A 422 is swallowed inside the client; a 500 raises StampGitHubError; a rate limit raises
    ...
    # they are not subclasses of StampGitHubError, so only a broad catch at the call site contains
    # them.
    repo_config = _repo_config(team.id)
    head_sha = "sha-refused-handoff"
    stamp_chain.recorder.register_pr(REPO, 101, _pr_object(101, "devex-dev", head_sha))
    if isinstance(fault, Exception):
        stamp_chain.recorder.add_label_side_effect = fault
    else:
        stamp_chain.recorder.add_label_response_override = fakes.FakeResponse(fault, text="handoff failure")
    pull_request = PullRequest.objects.for_team(team.id).create(
        team_id=team.id, repo_config=repo_config, pr_number=101, author_login="devex-dev"
    )
    run = ReviewRun.objects.for_team(team.id).create(
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_superseded_refusal_does_not_hand_off_to_review(team, stamp_chain: StampChain) -> None:
    # The Review handoff runs AFTER the conditional terminal save, so a refusal that loses the save
    # to a supersession (a synchronize/re-review delivery landing between the head guard and the save)
    # must not trigger Review for the stale refusal — a newer run may approve the same head. The run
    # returns skipped_superseded and the review label is never added.
    repo_config = _repo_config(team.id)
    ...
    assert result == {"verdict": "skipped_superseded"}
    assert [w for w in stamp_chain.recorder.github_writes if w["kind"] == "add_label"] == []
...
def test_merged_pr_digest_eligibility_gate(
    ...
    # cascade) only when a stamp-approved run exists at the exact merged head SHA — or the
    # repo is digest-only, where no approval can exist.
    repo_config = _repo_config(team.id)
    if not review_enabled:
        StampRepoConfig.objects.for_team(team.id).filter(id=repo_config.id).update(enabled=False)
    author, merged_head = "devex-dev", "sha-merged"
    stamp_chain.recorder.teams_by_login[author] = ["team-devex"]
    _make_pr_with_review(team.id, repo_config, number=101, author=author, approved_at_sha=approved_at_sha)

    status = stamp_chain.post_webhook(_merged_event(101, author, merged_head), delivery_id=str(uuid.uuid4()))
    assert status == 202
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_daily_digest_provisions_name_matched_channel_disabled(team, stamp_chain: StampChain) -> None:
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_repo_declared_digest_channel_short_circuits_author_cascade(team, stamp_chain: StampChain) -> None:
    # Regression guard: the repo-declared digest path. A repo that declares digest.channel in
    # .stamp/policy.yml groups all merged PRs under a "repo:" audience (skipping the author
    # cascade) and routes to the declared channel via the STAMPFN_CONFIG resolution source.
    repo_config = _repo_config(team.id)
    Integration.objects.create(
        team_id=team.id, kind="slack", config={"authed_user": {"id": "U1"}}, sensitive_config={"access_token": "x"}
    )
    author, merged_head = "devex-dev", "sha-merged"
    stamp_chain.recorder.teams_by_login[author] = ["team-devex"]  # would win if the cascade ran
    stamp_chain.recorder.policy_files[".stamp/policy.yml"] = "digest:\n  channel: eng-merges\n"
    _make_pr_with_review(team.id, repo_config, number=101, author=author, approved_at_sha=merged_head)

    stamp_chain.post_webhook(_merged_event(101, author, merged_head), delivery_id=str(uuid.uuid4()))
    pr = PullRequest.objects.for_team(team.id).get(repo_config=repo_config, pr_number=101)
    ...
    assert channel.resolution_source == ChannelResolutionSource.STAMPFN_CONFIG
    assert fakes.FakeSlackIntegration.posted_messages[0]["channel"] == "C-ENG"


@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_disabled_digest_channel_is_a_permanent_opt_out(team, stamp_chain: StampChain) -> None:
...
def test_auto_provision_skips_shared_channels(
    team, stamp_chain: StampChain, channel_flags: dict, expect_provisioned: bool
) -> None:
...
def test_post_verdict_stamps_digest_audience_only_at_approved_head(
    team, stamp_chain: StampChain, live_head: str, expected_audience: str
) -> None:
    ...
    stamp_chain.recorder.register_pr(REPO, 101, _pr_object(101, "devex-dev", live_head))
    stamp_chain.recorder.teams_by_login["devex-dev"] = ["team-devex"]
    pull_request = PullRequest.objects.for_team(team.id).create(
...
def test_sticky_comment_only_edits_the_apps_own_comment(
    team, stamp_chain: StampChain, comment_user_type: str, expect_patch: bool
) -> None:
    ...
    head_sha = "sha-refused"
    stamp_chain.recorder.register_pr(REPO, 101, _pr_object(101, "devex-dev", head_sha))
    stamp_chain.recorder.issue_comments[(REPO, 101)] = [
        {
            "id": 4242,
    ...
    writes = stamp_chain.recorder.github_writes
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_label_mode_synchronize_without_label_dismisses_stale_approval(team, stamp_chain: StampChain) -> None:
    ...
    status = stamp_chain.post_webhook(event, delivery_id=str(uuid.uuid4()))
    assert status == 202
...
@pytest.mark.django_db(databases=PRODUCT_DATABASES)
def test_inbox_review_approves_a_selfdriving_draft_pr_end_to_end(team, stamp_chain: StampChain) -> None:
    ...
    _repo_config(team.id)
    recorder = stamp_chain.recorder
    head_branch = "insights-self-driving/fix-the-thing-3f9a2c"
    pr_object = _pr_object(120, "insights-code[bot]", "sha120a")
    ...
    context = json.loads(dict(stamp_chain.sandbox_writes)[STAMPFN_SANDBOX_CONTEXT_PATH].decode())
    ...
    sandbox_env = stamp_chain.sandbox_class.created_configs[0].environment_variables
    assert json.loads(sandbox_env["STAMPFN_EXTRA_PROPERTIES"])["stamp_self_driving_review"] is True
