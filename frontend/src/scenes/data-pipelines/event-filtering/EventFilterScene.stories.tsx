...
// `appMetricsLogic`, which posts InsightsQL queries to `/query/InsightsQLQuery/`. The
...
- Second test case event name is "$drop" (was "$dro", "p" appended)
    ...
    play: async () => {
        ...
        await waitFor(
            () => {
                if (!document.querySelector('.Tag--danger')) {
...
- OR at the root, AND as the nested group
    ...
    play: async () => {
        ...
        await waitFor(() => {
            const modal = document.querySelector('.Modal pre')
