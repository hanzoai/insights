...
# Async-aware variant of hanzo_insights.scoped(), safe to stack on async Temporal activities.
    ...
    def decorator(func: _AsyncFn[P, R]) -> _AsyncFn[P, R]:
        if not inspect.iscoroutinefunction(func):
            raise TypeError(
                ...
                f"Use @hanzo_insights.scoped() for sync functions."
            )

        @functools.wraps(func)
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            with hanzo_insights.new_context(fresh=fresh, capture_exceptions=capture_exceptions):
