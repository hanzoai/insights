...
async def test_upsert_creates_when_missing(org_team) -> None:
    ...
    assert {"InsightsTeamId", "InsightsScheduleType", "InsightsScheduleFingerprint"} <= keys
...
async def test_upsert_stamps_fingerprint_attribute(org_team) -> None:
    ...
    assert pairs["InsightsTeamId"] == scanner.team_id
    assert pairs["InsightsScheduleType"] == SCANNER_SCHEDULE_TYPE
    assert pairs["InsightsScheduleFingerprint"] == expected
