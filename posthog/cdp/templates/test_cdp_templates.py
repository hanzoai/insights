...
class TestTemplatesGeneral(BaseTest):
    ...
    def test_templates_are_valid(self):
        for template in CUSTOM_FUNCTION_TEMPLATES:
    ...
    def test_sync_template_to_db(self):
        template_data = dataclasses.asdict(CUSTOM_FUNCTION_TEMPLATES[0])
    ...
    def test_sync_existing_template(self):
        template_data = CUSTOM_FUNCTION_TEMPLATES[0]
        template_id = template_data.id
        template = sync_template_to_db(template_data)
        assert CustomFunctionTemplate.objects.filter(template_id=template_id).count() == 1
