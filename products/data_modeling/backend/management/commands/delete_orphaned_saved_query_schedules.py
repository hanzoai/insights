...
class Command(BaseCommand):
    ...
    async def _find_orphans_from_listing(self, temporal, team_ids: set[int] | None = None) -> set[str]:
        ...
        If team_ids is provided, uses InsightsTeamId search attribute to pre-filter
    ...
    def _build_team_filter_query(team_ids: set[int]) -> str:
        """Build a Temporal visibility query to filter schedules by InsightsTeamId."""
        if len(team_ids) == 1:
            return f"InsightsTeamId = {next(iter(team_ids))}"
        return f"InsightsTeamId IN ({','.join(str(t) for t in sorted(team_ids))})"
