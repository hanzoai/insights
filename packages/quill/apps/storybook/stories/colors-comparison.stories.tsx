...
import { semanticColors } from '@hanzo/quill-tokens'
...
/**
 * Insights color values — sourced from `frontend/src/styles/base.scss`.
 * Hardcoded as static values so this story renders without loading
 * Insights's stylesheet. Update if `base.scss` changes.
 */
...
type Row = {
    ...
    insights?: { token: string; light: string; dark: string }
}
...
function phRow(name: keyof typeof ph): Row['insights'] {
...
const rows: Row[] = [
    ...
    // ── Status: backgrounds ─────────────────────────────
    {
        ...
        insights: phRow('bg-fill-error-secondary'),
    },
    {
        label: 'Success bg',
        quill: quillRow('success'),
        insights: phRow('bg-fill-success-secondary'),
    },
    {
        label: 'Warning bg',
        quill: quillRow('warning'),
        insights: phRow('bg-fill-warning-secondary'),
    },
    {
        label: 'Info bg',
        quill: quillRow('info'),
        insights: phRow('bg-fill-info-secondary'),
    },
    // ── Status: foregrounds ─────────────────────────────
    {
        label: 'Destructive text',
        quill: quillRow('destructive-foreground'),
        insights: phRow('text-error'),
    },
    {
        label: 'Success text',
        quill: quillRow('success-foreground'),
        insights: phRow('text-success'),
    },
    {
        label: 'Warning text',
        quill: quillRow('warning-foreground'),
        insights: phRow('text-warning'),
    },
    {
        label: 'Info text',
    ...
    // ── Borders & rings ─────────────────────────────────
    {
        ...
        insights: phRow('border-primary'),
    },
    {
        label: 'Input border',
        quill: quillRow('input'),
        insights: phRow('border-secondary'),
    },
    {
        label: 'Focus ring',
        quill: quillRow('ring'),
    ...
    // ── Interactive fills ───────────────────────────────
    {
        ...
        insights: phRow('bg-fill-button-tertiary-hover'),
    },
    {
        label: 'Selected fill',
        quill: quillRow('fill-selected'),
        insights: phRow('bg-fill-button-tertiary-active'),
    },
    {
        label: 'Expanded fill',
        quill: quillRow('fill-expanded'),
        insights: phRow('bg-fill-button-panel-active'),
    },
]
    ...
    return (
        <div className="grid grid-cols-[260px_1fr_1fr] gap-4 items-start py-3 border-b border-border/40">
            ...
            <div className="flex flex-col gap-1">
                {row.insights ? (
                    <>
                        <SidePair light={row.insights.light} dark={row.insights.dark} />
                        <span className="text-xs text-muted-foreground font-mono">{row.insights.token}</span>
                    </>
                ) : (
                    <EmptyCell />
                )}
...
export const QuillVsInsights: Story = {
    render: () => (
        <div className="space-y-4 max-w-7xl">
            <div className="space-y-1">
                <h2 className="text-base font-semibold">Quill vs Insights color tokens</h2>
                <p className="text-sm text-muted-foreground">
                    Side-by-side comparison of quill semantic tokens and their Insights `base.scss` equivalents. Each
