...
class TestUkCompaniesHouseSource:
    ...
    def test_source_config_is_released(self) -> None:
    ...
    def test_source_config_fields(self) -> None:
        ...
        assert [f.name for f in fields] == ["api_key", "company_numbers"]
        ...
        company_numbers = fields[1]
        assert isinstance(company_numbers, SourceFieldInputConfig)
        assert company_numbers.secret is False
        assert company_numbers.required is True
    ...
    def test_get_schemas_honors_the_names_filter(self) -> None:
        schemas = self.source.get_schemas(self.config, team_id=123, names=[OFFICERS])

        assert [schema.name for schema in schemas] == [OFFICERS]
    ...
    def test_get_resumable_source_manager_is_bound_to_the_resume_config(self) -> None:
        manager = self.source.get_resumable_source_manager(_make_inputs(OFFICERS))

        assert isinstance(manager, ResumableSourceManager)
        assert manager._data_class is UkCompaniesHouseResumeConfig
