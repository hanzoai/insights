...
import { insightsFunctionsList } from 'products/cdp/frontend/generated/api'
import type {
    InsightsFunctionMinimalApi,
                ...
                loadPipelines: async () => {
                    ...
                    const perTeamItems = await Promise.all(
                        org.teams.map(async (team): Promise<PipelineItem[]> => {
                            ...
                            try {
                                const initial: PaginatedInsightsFunctionMinimalListApi = await insightsFunctionsList(
                                    String(team.id),
                                    {
                                ...
                                console.warn(`Failed to load script functions for team ${team.id}`, e)
                            }
                            try {
