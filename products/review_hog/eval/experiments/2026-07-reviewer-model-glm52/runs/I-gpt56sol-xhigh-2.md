...
- **Found (premise not in place):** `tasks` is not routed to a product DB — `products/db_routing.yaml` lists only `stamphog`/`visual_review`/`warehouse_sources_queue`. `TaskRun` reads go through `ReplicaRouter.db_for_read`, which returns `"default"` (primary) for any model not in the `READ_REPLICA_OPT_IN` env list (`insights/dbrouter.py:22-31`). The stamphog writer-pin invariant the finding cites applies because stamphog's product DB always reads a replica; that does not transfer to a main-DB model.
