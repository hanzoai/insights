...
``github_workflow_runs.json`` (a real Insights/insights snapshot captured with
...
storage and Datastore from the insightscli stack.
...
from insights.datastore.logs.logs34 import TABLE_NAME as LOGS_LOCAL_TABLE
...
SEED_REPOSITORY = "Insights/insights"
            ...
            demo_runs.append(
                {
                    ...
                    "head_commit": {
                        "message": subject,
                        "author": {"name": "Insights Bot", "email": "bot@hanzo.ai"},
                    },
                }
            )
    return demo_runs
...
def _demo_multi_push(
    ...
    demo_pr = {
        ...
        "base": {"repo": {"full_name": "Insights/insights"}},
        "labels": ["demo"],
    }
...
# insights.trace_spans, not the warehouse). Each team gets a distinct trend shape so the
...
_SPAN_TEAMS: list[tuple[str, str, list[tuple[str, str, int, int]]]] = [
    (
        ...
        "insights/session_recordings/test",
        [
    ...
    (
        ...
        "insights/insightsql/test",
        [
            ("TestInsightsQLParser", "test_lambda_scope_resolution", 2, 3),
            ("TestInsightsQLPrinter", "test_materialized_column_rewrite", 1, 1),
        ],
    ),
    (
        ...
        [
            ("TestInsightsFunctions", "test_retry_backoff_jitter", 2, 3),
            ("TestDestinationFilters", "test_event_property_globs", 1, 1),
        ],
    ),
    (
        "team-product-analytics",  # mildly improving
        "insights/insightsql_queries/insights/test",
        [
    ...
    (
        "team-insights-code",  # creeping up
    ...
    (
        "datastore",  # low and flat
        "insights/datastore/test",
        [
...
# the two lines would always coincide). Real Insights/insights merges ~40 PRs a day, so the
...
_LOG_FAILURE_DETAILS = (
    ...
    "TimeoutError: Datastore query 8f3aa21b4c9d timed out after 30000 ms",
...
def _sql_escape(value: object) -> str:
    # For single-quoted Datastore literals; job fields originate from the GitHub API via the fixture.
...
def _seed_ci_failure_logs(team: Team, jobs: list[dict[str, Any]]) -> int:
    ...
    # untouched. InsightsQL's `logs` reads `logs_distributed` (over logs34, not the legacy `logs`
...
def _warehouse_endpoint() -> str:
    # Datastore runs in docker, so a localhost object-storage endpoint must be
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if not settings.OBJECT_STORAGE_ENABLED or not settings.OBJECT_STORAGE_ACCESS_KEY_ID:
            raise CommandError("Object storage is not configured — start the dev stack first (insightscli start).")
        ...
        # Always normalize timestamps to a Datastore-friendly format; rebasing is optional.
