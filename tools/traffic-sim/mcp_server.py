#!/usr/bin/env python3
"""Local MCP server for traffic-sim — verify Insights instrumentation from any MCP client.

Exposes three tools that wrap the CLI handlers:
- simulate_new_user
- simulate_returning_user
- check_insights_loading

Each tool launches a Playwright browser, drives it against the given URL,
and returns a structured summary of what Insights events fired.
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "watch which events fire. Use check_insights_loading to inspect how "
        "the Insights snippet is loaded across pages."
    ),
)
...
def _summarize_visits(results: list[cli.VisitResult]) -> dict[str, Any]:
    ...
    for r in results:
        visit_network = [e for req in r.insights_requests for e in req.events]
        visit_console = cli.extract_insights_events_from_console(r.console_lines)
    ...
    success = sum(sum(1 for req in r.insights_requests if req.status in (200, 204)) for r in results)
    requests = sum(len(r.insights_requests) for r in results)
    errors = [r.error for r in results if r.error]
    return {
        "total_visits": len(results),
        "insights_requests": requests,
        "insights_requests_ok": success,
...
async def simulate_new_user(
    ...
    Args:
        ...
        insights_host: Insights ingestion host. Defaults to https://us.i.hanzo.ai.
                      Set to https://eu.i.hanzo.ai for the EU cloud, or your
                      self-hosted host (e.g. https://ph.example.com).
        verbose: Print every Insights request and console line to stderr.
    """
    run_id = uuid.uuid4().hex[:8]
    insights_domains = cli.resolve_insights_domains(insights_host)
...
async def check_insights_loading(
    ...
    For each URL, returns whether insights is detected, how it was loaded
    (head_snippet / snippet / array_js_only), the init config, and runtime
    state. Use this to confirm that all pages on a site initialize Insights
