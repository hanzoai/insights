...
from insights.schema import ExperimentQuery
...
def _calculate_experiment_metric_for_recalculation_sync(
    ...
    with team_scope(state.team_id, canonical=True):
        ...
        try:
        ...
        except (ConcurrencyLimitExceeded, DatastoreAtCapacity) as e:
            message = str(e)[:_MAX_ERROR_MESSAGE_LENGTH]
            if is_final_attempt:
                _store_result(
                    experiment_id=experiment_id,
            ...
            logger.warning(
                "Experiment metric recalculation deferred by Datastore backpressure",
                experiment_id=experiment_id,
                metric_uuid=metric_uuid,
