...
class TestQuickstartCommand:
    ...
    def test_quickstart_displays_help(self) -> None:
        ...
        assert "insightscli" in result.output
        assert "insightscli start -d" not in result.output
...
class TestLoadEnvFile:
    ...
    def test_skip_pattern_filters_matching_values(
        ...
        """
        from insightscli.cli import _load_env_file
...
class TestApplyEnvConfig:
    ...
    secrets_config and we don't depend on Insights's insightscli.yaml. The mocked
    manifest also keeps existing Insights tests in the same file unaffected.
    """
    ...
    def test_no_env_config_is_noop(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Generic insightscli installs with no `config.env` should not touch os.environ.

        The PyPI insightscli package must stay transparent for users who don't opt in.
        """
        from insightscli.cli import _apply_env_config

        monkeypatch.setattr("insightscli.cli.get_manifest", lambda: self._make_manifest())
    ...
    def test_marker_hit_with_wrap_binary_present_reexecs(self, tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        AND the wrap binary is on PATH, insightscli re-execs under the wrap.
        ...
        # argv re-runs insightscli (so the child completes the original invocation)
        assert any("insightscli" in arg for arg in argv)
