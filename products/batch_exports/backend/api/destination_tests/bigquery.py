...
from insights.models.integration import GoogleCloudServiceAccountIntegration
...
class BigQueryVerifyServiceAccountOwnershipTestStep(DestinationTestStep):
    ...
    description = "Confirm that the current Insights organization owns a BigQuery service account by ensuring its organization ID is set as part of the service account description."
    ...
    async def _run_step(self) -> DestinationTestStepResult:
        ...
        try:
            ...
            return DestinationTestStepResult(
                status=Status.FAILED,
                message=f"No permission to read service account's '{service_account_email}' description. Have you granted the Insights service account a role with `iam.serviceAccounts.get`?",
            )
        except NotFound:
            return DestinationTestStepResult(
                status=Status.FAILED,
        ...
        if f"insights:{self.organization_id}" not in sa.description:
            return DestinationTestStepResult(
                status=Status.FAILED,
