...
from insights.temporal.data_imports.pipelines.pipeline.typings import SourceInputs
from insights.temporal.data_imports.sources.bigquery.source import BigQuerySource
from insights.temporal.data_imports.sources.generated_configs import (
...
def test_bigquery_get_schemas():
    with mock.patch(
        "insights.temporal.data_imports.sources.bigquery.source.get_bigquery_schemas", return_value={"table": []}
    ):
...
def test_bigquery_get_schemas_with_existing_destination_tables():
    with mock.patch(
        ...
        return_value={"table": [], "__insights_import_0000_0000": []},
    ):
        source_cls = BigQuerySource()
        schemas = source_cls.get_schemas(mock.ANY, 1)
        assert len(schemas) == 1
...
def test_bigquery_destination_table_default():
    ...
    with (
        ...
        mock.patch(
            "insights.temporal.data_imports.sources.bigquery.source.delete_all_temp_destination_tables",
        ) as mock_delete_all_temp_destination_tables,
        mock.patch(
            "insights.temporal.data_imports.sources.bigquery.source.bigquery_source",
        ) as mock_bigquery_source,
        mock.patch(
            "insights.temporal.data_imports.sources.bigquery.source.delete_table",
    ...
    mock_delete_all_temp_destination_tables.assert_called_once_with(
        dataset_id="dataset-id",
        table_prefix="__insights_import_schema_id",
    ...
    mock_bigquery_source.assert_called_once_with(
        ...
        should_use_incremental_field=False,
        logger=mock.ANY,
        bq_destination_table_id=f"project-id.dataset-id.__insights_import_schema_id_job_id_{str(parser.parse('2025-01-01T12:00:00.000Z').timestamp()).replace('.', '')}",
        incremental_field=None,
        incremental_field_type=None,
        db_incremental_field_last_value=None,
    )
    ...
    mock_delete_table.assert_called_once_with(
        table_id=f"project-id.dataset-id.__insights_import_schema_id_job_id_{str(parser.parse('2025-01-01T12:00:00.000Z').timestamp()).replace('.', '')}",
        project_id=config.key_file.project_id,
        location=None,
        private_key=config.key_file.private_key,
        private_key_id=config.key_file.private_key_id,
...
def test_bigquery_destination_table_with_temporary_dataset_set():
    ...
    mock_bigquery_source.assert_called_once_with(
        ...
        should_use_incremental_field=False,
        logger=mock.ANY,
        bq_destination_table_id=f"project-id.some-other-dataset-id.__insights_import_schema_id_job_id_{str(parser.parse('2025-01-01T12:00:00.000Z').timestamp()).replace('.', '')}",
        incremental_field=None,
        incremental_field_type=None,
        db_incremental_field_last_value=None,
    )
    ...
    mock_delete_table.assert_called_once_with(
        table_id=f"project-id.some-other-dataset-id.__insights_import_schema_id_job_id_{str(parser.parse('2025-01-01T12:00:00.000Z').timestamp()).replace('.', '')}",
        project_id=config.key_file.project_id,
        location=None,
        private_key=config.key_file.private_key,
        private_key_id=config.key_file.private_key_id,
