...
impl<D: PersonDb + 'static> PersonWriteStore<D> {
    ...
    pub async fn upsert_batch(&self, persons: Vec<Person>) -> BatchOutcome {
        ...
        histogram!("personinsights_writer_flush_duration_ms")
            .record(start.elapsed().as_secs_f64() * 1000.0);
        histogram!("personinsights_writer_flush_rows").record(total as f64);
        outcome
    }
    ...
    pub async fn upsert_rows_parallel(&self, persons: Vec<Person>) -> RowFallbackOutcome {
        ...
        let results: Vec<(Person, Result<(), WriteError>)> = stream::iter(persons)
            .map(|p| async move {
                gauge!("personinsights_writer_row_fallback_in_flight").increment(1.0);
                let res = self.upsert_row(&p).await;
                gauge!("personinsights_writer_row_fallback_in_flight").decrement(1.0);
                (p, res)
            })
            .buffer_unordered(concurrency)
            .collect()
            .await;
        histogram!("personinsights_writer_row_fallback_duration_ms")
            .record(start.elapsed().as_secs_f64() * 1000.0);
        ...
        for (person, result) in results {
            match result {
                ...
                Err(e) => {
                    counter!(
                        "personinsights_writer_unapplyable_rows_total",
    ...
    async fn run_parallel_chunks(&self, chunks: Vec<Vec<Person>>) -> BatchOutcome {
        ...
        if !transient.is_empty() {
            counter!("personinsights_writer_chunk_retry_rows_total").increment(transient.len() as u64);
        }
        if !data_failed.is_empty() {
            counter!("personinsights_writer_chunk_fallback_rows_total")
...
fn partial_from_failed_chunk(chunk: Vec<Person>, kind: WriteErrorKind) -> BatchOutcome {
    match kind {
        WriteErrorKind::Transient => {
            counter!("personinsights_writer_chunk_retry_rows_total").increment(chunk.len() as u64);
            BatchOutcome::Partial {
                transient: chunk,
                data_failed: Vec::new(),
            }
        ...
        WriteErrorKind::Data | WriteErrorKind::PropertiesSizeViolation => {
            counter!("personinsights_writer_chunk_fallback_rows_total").increment(chunk.len() as u64);
            BatchOutcome::Partial {
                transient: Vec::new(),
                data_failed: chunk,
            }
...
fn classify_join_error(e: JoinError) -> FatalError {
    ...
    counter!("personinsights_writer_chunk_fatal_total").increment(1);
