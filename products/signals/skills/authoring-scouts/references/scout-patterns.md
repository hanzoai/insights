...
The single most useful thing to internalize: **a scout is not limited to Insights analytics events.** It can watch anything the project can see, and the report / dedupe / memory contract is identical regardless of where the data comes from.
...
| **The data warehouse**       | `execute-sql` over `system.information_schema.*` to confirm columns, then `execute-sql`. **Any source Insights ingests becomes a queryable table** — see the warehouse-backed pattern below.                                                                           |
| **Insights product entities** | dedicated list/get tools (insights, dashboards, surveys, error issues, experiments, flags) plus `execute-sql` over `system.*`.                                                                                                                                        |
...
| **Warehouse-backed source**                 | the signal lives in a non-Insights source synced into the warehouse.                                                                                  | a Slack-channel-sync scout (below)                                                |
...
| **State ∩ code intersection**               | the signal is the _overlap_ of a Insights entity's state and what's in the source repo.                                                               | a feature-flag-cleanup scout (below)                                              |
...
- **Don't reinvent the scoring.** When the metric is a **saved time-series insight**, score it with Insights's own detectors via `alert-simulate` rather than hand-rolling anomaly math — it already handles seasonality and the team's own alert thresholds.
...
- **Proven variants:**
  ...
  - **Automation liveness** — the watched entity is a Insights automation (a workflow, a CDP destination): configured-active with zero successes _and_ zero failures while the trigger has volume is the silently-dark shape a delivery-failure watcher misses.
...
**The pattern that lets a scout watch anything Insights can ingest.** A non-Insights source (a Slack channel, a billing system, a CRM, a support tool, a social-listening feed) is synced into the data warehouse on a schedule; the scout reads the resulting table with `execute-sql` and turns it into signals.
...
When one bespoke event captured into Insights carries the whole signal (a product's own telemetry, a feedback event, a domain-specific action).
...
- **Requirements & gotchas — specific to reaching outside the sandbox:**
  - Needs network reach to the target and the runtime (e.g. `node`/`npx`, `git`, `curl`).
    ...
    A target **outside** that allowlist (an arbitrary docs site, arxiv.org, a vendor status page) needs `network_access: "full"` on the scout's config (`insights:scout-config-update`, or the nested `config` at creation), or every fetch is blocked.
...
A composition of the external-tool/code pattern with a Insights-entity read, where **neither source alone is the signal — the overlap is.** The scout reads an entity's state from Insights (via the normal MCP tools) and reads the source repo (via the clone-and-grep machinery of the external-tool pattern), and reports only where the two intersect in an actionable way.
...
- **Canonical example — feature-flag cleanup.** A fully-rolled-out-for-a-long-time flag is dead weight _only if its key is still referenced in code_; a flag that's gone from code is already cleaned up, and a flag still doing targeting work isn't a candidate.
  So the discriminator is the **intersection**: `Insights says STALE/fully-rolled-out` **AND** `the key still appears at a real SDK call site in non-test source`.
  Insights does the staleness detection server-side (`feature-flag-get-all` `active:"STALE"`), the clone-and-grep half confirms the code reference, and the finding is a P3 cleanup recommendation with the exact file:line call sites and a ready-to-paste cleanup prompt.
...
- This shape generalizes past feature flags: any "Insights entity whose code footprint determines whether its state is a problem" fits it — a cohort/insight referencing an event that the code stopped emitting, a deprecated SDK method still called, a tracked event with no capture call left in source.
- **And it generalizes past "Insights state ∩ code": the two halves can be any two independently-readable sources whose overlap is the signal.** Proven variations:
  - **code ∩ data (the inverse direction)** — a newly-shipped user-facing surface in the repo **AND** no matching capture event in the project's stream: an instrumentation gap.
    Here the code half _should_ produce Insights state and doesn't; confirm the gap on the data side with `read-data-schema` / a stream query before reporting.
...
For a surface where **detection already exists** — a billing system's per-customer spike detector, an incident/alerting pipeline that already pages humans, Insights's own health checks, a support or triage channel where a bot already classifies every item.
...
- **Watched data:** the detector's own output — pre-detected spike events, alert/escalation rows, tickets carrying pre-classified priority/severity.
  Often reached via the warehouse-backed pattern when the detector lives outside Insights.
...
- The canonical in-repo relatives are `signals-scout-health-checks` (judgment over Insights's health issues) and `signals-scout-insight-alerts` (missed firings of alerts the team already configured) — this pattern is the same shape pointed at _any_ detector, in or out of Insights.
...
A scout runs with Insights MCP read scopes, sandbox network access (the TRUSTED allowlist by default, any site when its config sets `network_access=full`), and the ability to write inbox reports — so any content it ingests is a prompt-injection surface, and the harness does **not** add an injection guard for you.
...
- **Opt-in scoping via tags** — let users opt entities into a scout by tagging them in Insights (e.g. only funnels tagged `<scout-scope>` get scored).
