...
def _cleanup(conn: psycopg.Connection[Any]) -> None:
    with conn.cursor() as cur:
        for team_id in (SYNC_TEST_TEAM_ID, SYNC_TEST_TARGET_TEAM_ID):
            cur.execute("DELETE FROM insights_persondistinctid WHERE team_id = %s", (team_id,))
            cur.execute(f"DELETE FROM {settings.PERSON_TABLE_NAME} WHERE team_id = %s", (team_id,))
            cur.execute("DELETE FROM insights_group WHERE team_id = %s", (team_id,))
        for project_id in (SYNC_TEST_PROJECT_ID, SYNC_TEST_TARGET_PROJECT_ID):
            cur.execute("DELETE FROM insights_grouptypemapping WHERE project_id = %s", (project_id,))
        ...
        with persons_db_connection(writer=True) as conn:
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT distinct_id, person_id FROM insights_persondistinctid WHERE team_id = %s",
                        (SYNC_TEST_TEAM_ID,),
                    )
                    rows = cur.fetchall()
                assert len(rows) == 1
        ...
        with persons_db_connection(writer=True) as conn:
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT COUNT(*) FROM insights_persondistinctid WHERE team_id = %s",
                        (SYNC_TEST_TEAM_ID,),
                    )
                    row = cur.fetchone()
        ...
        with persons_db_connection(writer=True) as conn:
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT group_key, group_properties, group_type_index FROM insights_group WHERE team_id = %s",
                        (SYNC_TEST_TEAM_ID,),
                    )
                    rows = cur.fetchall()
                assert len(rows) == 1
    ...
    def test_copy_transfers_mappings_between_projects(self):
        try:
            ...
            with persons_db_connection(writer=False) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT team_id, group_type, name_singular FROM insights_grouptypemapping "
