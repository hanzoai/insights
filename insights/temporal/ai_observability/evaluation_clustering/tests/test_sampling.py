...
from insights.temporal.ai_observability.evaluation_clustering.models import SamplerActivityInputs
from insights.temporal.ai_observability.evaluation_clustering.sampling import (
...
@patch(
    "insights.temporal.ai_observability.evaluation_clustering.sampling.Heartbeater",
    _noop_heartbeater,
)
class TestSampleAndEmbedForJobActivity:
    ...
    async def test_enqueues_one_embedding_per_row(self, mock_team):
        # Row shape mirrors the InsightsQL SELECT:
        ...
        with (
            patch(
                "insights.temporal.ai_observability.evaluation_clustering.sampling.execute_insightsql_query"
            ) as mock_execute,
            patch(
                "insights.temporal.ai_observability.evaluation_clustering.sampling.LLMTracesSummarizerEmbedder"
            ) as mock_embedder_cls,
            patch(
                "insights.temporal.ai_observability.evaluation_clustering.sampling._fetch_evaluation_descriptions",
    ...
    async def test_event_filter_propagated_to_query(self, mock_team):
        ...
        with (
            ...
            patch("insights.temporal.ai_observability.evaluation_clustering.sampling.LLMTracesSummarizerEmbedder"),
        ):
            mock_execute.return_value.results = []
            await sample_and_embed_for_job_activity(inputs)

            placeholders = mock_execute.call_args.kwargs["placeholders"]
            # filter_expr is a real InsightsQL expr (not the pass-through True literal) when filters are provided
