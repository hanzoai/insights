...
Converts JSON strings returned by the C++ InsightsQL parser back into Python AST objects.
...
# shadow mismatches on every Script program containing a dict literal or a
# try/catch.
...
def _deserialize_node(data: Any) -> Any:
    ...
    if data.get("error") is True:
        ...
        # `ExposedInsightsQLError`) so callers can distinguish it, matching
        # what the C++ parser raises natively.
        if error_type == "SyntaxError":
            raise InsightsQLSyntaxError(message, start=start.get("offset"), end=end.get("offset"))
        raise ExposedInsightsQLError(message, start=start.get("offset"), end=end.get("offset"))
