...
from insights.helpers.dashboard_templates import create_data_ops_dashboard
...
from products.cdp.backend.facade.models import InsightsFunction, InsightsFunctionState, InsightsFunctionType
...
class DataWarehouseViewSet(TeamAndOrgViewSetMixin, viewsets.ViewSet):
    ...
    def property_values(self, request: Request, **kwargs) -> Response:
        with (
            ...
            result = execute_insightsql_query(query, team=self.team, user=request.user)
    ...
    def total_rows_stats(self, request: Request, **kwargs) -> Response:
        ...
        """
        # Every number here is scoped to a billing period read from the billing service,
        # an enterprise feature this fork does not carry. With no period to scope to, the
        # response says so through billing_available rather than inventing a window.
        return Response(
    ...
    def data_health_issues(self, request: Request, **kwargs) -> Response:
        ...
        try:
            ...
            script_destinations = InsightsFunction.objects.filter(
                team_id=self.team_id,
                deleted=False,
            ...
            for destination in script_destinations:
                try:
        ...
        responses={
            202: inline_serializer(
                ...
                fields={
                    ...
                    "org": serializers.CharField(help_text="duckgres org identifier (the Insights organization id)"),
            ...
            200: inline_serializer(
                ...
                fields={
                    ...
                    "org": serializers.CharField(
                        required=False,
                        help_text="duckgres org identifier (the Insights organization id)",
                    ),
                },
            )
        },
    ...
    @extend_schema(
        responses={
            200: inline_serializer(
                ...
                fields={
                    "org_id": serializers.CharField(help_text="duckgres org identifier (the Insights organization id)"),
