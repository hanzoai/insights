...
import { useCustomFlowStep } from '../CustomFlowSteps'
import { StepViewMetrics } from './StepViewMetrics'
import { StepViewLogicProps, stepViewLogic } from './stepViewLogic'

export function StepView({ action }: { action: CustomFlowAction }): JSX.Element {
    const {
        selectedNode,
        mode,
        nodesById,
        ...
        animatingEdgePair,
    } = useValues(customFlowEditorLogic)
    const { setSelectedNodeId, startCopyingNode, startMovingNode } = useActions(customFlowEditorLogic)
    ...
    const Step = useCustomFlowStep(action)
