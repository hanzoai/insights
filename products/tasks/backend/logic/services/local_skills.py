...
instead build skills into `products/insights_ai/dist/skills/` and bind-mount
...
`products/insights_ai/dist/skills/`; `.agents/skills/` is the user's own
...
BUILT_SKILLS_RELATIVE_PATH = Path("products/insights_ai/dist/skills")
...
def populate_skills_directory(destination: Path, base_dir: Path | None = None) -> None:
    """Copy rendered skills from ``products/insights_ai/dist/skills/`` into ``destination``.

    Requires the dist directory to already be populated (by
    ``insightscli build:skills`` or CI). Does not fall back to raw source trees:
...
class LocalSkillsCache:
    ...
    def ensure_built(self) -> Path:
        ...
        raise RuntimeError(f"No rendered local skills at {self.dist_dir}. Run `insightscli build:skills` to populate it.")
    ...
    def _build(self, source_hash: str) -> None:
        from products.insights_ai.scripts.build_skills import SkillBuilder

        builder = SkillBuilder(self.base_dir, self.base_dir / "products", self.base_dir / "products" / "insights_ai")
    ...
    def _iter_source_files(self) -> list[Path]:
        ...
        builder_script = self.base_dir / "products" / "insights_ai" / "scripts" / "build_skills.py"
