...
- Use string-based foreign key references to avoid circular imports (e.g., `models.ForeignKey("insights.Team", on_delete=models.CASCADE)`)
  - A `ForeignKey` **targeting a hot table** (`insights.Team`, `insights.User`, `insights.Organization`, `insights.Project`, including `settings.AUTH_USER_MODEL`) is unsafe even within the same database, and even for a brand-new `CreateModel`: building the FK constraint takes a `SHARE ROW EXCLUSIVE` lock on the _referenced parent_ table, which can stall a deploy under write traffic. `HotTableAlterPolicy` blocks this in CI. Either declare the FK with `db_constraint=False` (no parent lock at all, app-level enforcement only) or add it as a real constraint with the two-phase `AddForeignKeyNotValid` / `ValidateForeignKey` helpers. See [Foreign Keys to Hot Tables](https://github.com/Insights/insights/blob/master/docs/published/handbook/engineering/safe-django-migrations.md#foreign-keys-to-hot-tables). (This is a different problem from the cross-database FK limitation below — that one is about FKs _across separate product databases_.)
...
New products get their own Postgres database by default (`insightscli product:bootstrap` adds a route automatically).
...
Locally (`DEBUG=1`), it auto-connects to `insights_visual_review` on localhost. In prod, the infrastructure handles env vars and connections automatically. If the env var is absent, the route is silently skipped.
...
Every model that stores tenant data **must** have a `team_id` field. This is Insights's primary tenant isolation boundary — without it, there's no way to enforce that one team can't see another's data.

For product databases, use `ProductTeamModel` as your base class. It provides `team_id` plus a fail-closed manager that raises `TeamScopeError` when queries run without team context. See `insights/models/scoping/README.md` for the full API.

```python
from insights.models.scoping.product_mixin import ProductTeamModel
