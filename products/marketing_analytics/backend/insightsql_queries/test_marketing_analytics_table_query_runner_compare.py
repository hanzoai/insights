...
class TestMarketingAnalyticsTableQueryRunnerCompare(DatastoreTestMixin, BaseTest):
    maxDiff = None
    ...
    def test_multi_source_business_metrics_validation_with_compare(self):
        ...
        assert {
            ...
            "query": pretty_print_in_tests(response.insightsql, self.team.pk),
        } == self.snapshot
    ...
    def test_conversion_goal_with_mixed_match_fields(self):
        ...
        that Datastore doesn't support in JOIN ON clauses.
        """
        facebook_info = self._setup_csv_table("facebook_ads")
        ...
        assert "campaign_costs.match_key" in response.insightsql, "Expected match_key in campaign_costs"
        assert "ucg.match_key" in response.insightsql, "Expected match_key in unified conversion goals"
        assert "equals(campaign_costs.match_key, ucg.match_key)" in response.insightsql, (
