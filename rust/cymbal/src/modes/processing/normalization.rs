...
//!
//! Different Insights SDKs historically disagreed on the order of two things:
//!
...
fn lib_rules() -> &'static [LibRule] {
    ...
    RULES.get_or_init(|| {
        ...
        let crash_first_frames = [
            ...
            "insights-rs",
            "insights-elixir",
            // Java: `insights-server` is the current java server SDK identifier
            // (module in the insights-android repo, shares the android coercer);
            // `insights-java` is the tombstoned legacy SDK, included defensively.
        ...
        let mut rules: Vec<LibRule> = crash_first_frames
            ...
            .map(|lib| LibRule {
                ...
                canonical_since: match lib {
                    // insights-rs ships the flip in 0.19.0 (Insights/insights-rs#176).
                    "insights-rs" => Some(Version::new(0, 19, 0)),
                    // insights-php ships the flip in 4.11.0 (Insights/insights-php#200).
                    "insights-php" => Some(Version::new(4, 11, 0)),
                    // insights-go ships the flip in 1.20.0 (Insights/insights-go#254).
                    "insights-go" => Some(Version::new(1, 20, 0)),
                    // android + the java server SDK flip together (shared
                    // coercer, one release train: Insights/insights-android#603),
                    // but each module reports its own version stream.
                    "insights-android" => Some(Version::new(3, 56, 0)),
                    "insights-server" => Some(Version::new(2, 9, 0)),
                    // insights-elixir ships the flip in 2.13.0 (Insights/insights-elixir#160).
                    "insights-elixir" => Some(Version::new(2, 13, 0)),
                    // insights-flutter ships its Dart flip in 5.33.0 and raises
                    // its Android SDK floor to the canonical 3.56.0 release.
                    "insights-flutter" => Some(Version::new(5, 33, 0)),
                    // `insights-java` is the tombstoned legacy SDK: it will
        ...
        rules.push(LibRule {
            ...
            // insights-python ships the flip in 7.31.0 (Insights/insights-python#728).
...
mod test {
    ...
    fn legacy_wire_order_reconstructs_pre_flip_order() {
        ...
        let legacy = legacy_wire_order(Some("insights-go"), &canonical).expect("go is in the table");
        ...
        .into();
        let legacy =
            legacy_wire_order(Some("insights-python"), &canonical).expect("python is in the table");
        ...
        assert!(legacy_wire_order(Some("insights-node"), &canonical).is_none());
    ...
    fn python_cutoff_gates_exception_list_normalization_by_version() {
        ...
        for version in [Some("7.30.1"), Some("7.30.0"), Some("garbage"), None] {
            ...
            let legacy = normalize_wire_order(&mut list, Some("insights-python"), version);
        ...
        for version in ["7.31.0", "7.31.1", "7.32.0", "8.0.0"] {
            ...
            let legacy = normalize_wire_order(&mut list, Some("insights-python"), Some(version));
        ...
        let legacy = legacy_wire_order(Some("insights-python"), &canonical)
    ...
    fn elixir_cutoff_gates_normalization_by_version() {
        for version in [Some("2.12.1"), Some("2.1.0"), Some("garbage"), None] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-elixir"), version);
            assert!(legacy.is_some(), "{version:?} should normalize");
        ...
        for version in ["2.13.0", "2.13.1", "2.14.0", "3.0.0"] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-elixir"), Some(version));
            assert!(legacy.is_none(), "{version} should pass through");
        ...
        let canonical: ExceptionList =
            vec![exception_with_frames("Boom", &["main", "boom"])].into();
        let legacy = legacy_wire_order(Some("insights-elixir"), &canonical)
            .expect("reconstruction must ignore the cutoff");
        assert_eq!(frame_names(&legacy[0]), vec!["boom", "main"]);
    }
    ...
    fn android_server_and_flutter_cutoffs_gate_normalization_by_version() {
        ...
        for (lib, below, at) in [
            ("insights-android", "3.55.2", "3.56.0"),
            ("insights-server", "2.8.1", "2.9.0"),
            ("insights-flutter", "5.32.1", "5.33.0"),
        ] {
        ...
        let legacy = normalize_wire_order(&mut list, Some("insights-java"), Some("99.0.0"));
        assert!(legacy.is_some(), "insights-java has no cutoff");
        assert_eq!(frame_names(&list[0]), vec!["boom", "main"]);
    }
    ...
    fn go_cutoff_gates_normalization_by_version() {
        ...
        for version in [Some("1.19.0"), Some("1.2.3"), Some("devel"), None] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-go"), version);
            assert!(legacy.is_some(), "{version:?} should normalize");
            assert_eq!(frame_names(&list[0]), vec!["boom", "main"]);
        }
        ...
        for version in ["1.20.0", "1.20.1", "1.21.0", "2.0.0"] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-go"), Some(version));
            assert!(legacy.is_none(), "{version} should pass through");
            assert_eq!(frame_names(&list[0]), vec!["main", "boom"]);
        }
        ...
        let canonical: ExceptionList =
            vec![exception_with_frames("Boom", &["main", "boom"])].into();
        let legacy = legacy_wire_order(Some("insights-go"), &canonical)
            .expect("reconstruction must ignore the cutoff");
        assert_eq!(frame_names(&legacy[0]), vec!["boom", "main"]);
    }
    ...
    fn php_cutoff_gates_normalization_by_version() {
        ...
        for version in [Some("4.10.0"), Some("4.9.0"), Some("dev-main"), None] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-php"), version);
            assert!(legacy.is_some(), "{version:?} should normalize");
            assert_eq!(frame_names(&list[0]), vec!["boom", "main"]);
        }
        ...
        for version in ["4.11.0", "4.11.1", "4.12.0", "5.0.0"] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-php"), Some(version));
            assert!(legacy.is_none(), "{version} should pass through");
            assert_eq!(frame_names(&list[0]), vec!["main", "boom"]);
        }
        ...
        let canonical: ExceptionList =
            vec![exception_with_frames("Boom", &["main", "boom"])].into();
        let legacy = legacy_wire_order(Some("insights-php"), &canonical)
            .expect("reconstruction must ignore the cutoff");
        assert_eq!(frame_names(&legacy[0]), vec!["boom", "main"]);
    }
    ...
    fn rs_cutoff_gates_normalization_by_version() {
        ...
        for version in [Some("0.18.0"), Some("0.3.7"), Some("garbage"), None] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-rs"), version);
            assert!(legacy.is_some(), "{version:?} should normalize");
            assert_eq!(frame_names(&list[0]), vec!["boom", "main"]);
        }
        ...
        for version in ["0.19.0", "0.19.1", "0.20.0", "1.0.0"] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
            let legacy = normalize_wire_order(&mut list, Some("insights-rs"), Some(version));
            assert!(legacy.is_none(), "{version} should pass through");
            assert_eq!(frame_names(&list[0]), vec!["main", "boom"]);
        }
        ...
        let canonical: ExceptionList =
            vec![exception_with_frames("Boom", &["main", "boom"])].into();
        let legacy = legacy_wire_order(Some("insights-rs"), &canonical)
            .expect("reconstruction must ignore the cutoff");
        assert_eq!(frame_names(&legacy[0]), vec!["boom", "main"]);
    }
    ...
    fn java_server_libs_both_normalize() {
        ...
        // (`insights-server`) and legacy tombstoned (`insights-java`) identifiers.
        for lib in ["insights-java", "insights-server"] {
            let mut list: ExceptionList =
                vec![exception_with_frames("Boom", &["main", "boom"])].into();
    ...
    fn crash_first_lib_reverses_frames_only() {
        // insights-go sends crash-first frames; the list order is left alone.
        ...
        let legacy = normalize_wire_order(&mut list, Some("insights-go"), Some("1.2.3"));
    ...
    fn python_reverses_exception_list_only() {
        // insights-python sends the list root-cause-first; frames stay put.
        let mut list: ExceptionList = vec![
            exception_with_frames("RootCause", &["main", "boom"]),
            exception_with_frames("Wrapper", &["main", "wrap"]),
        ]
        ...
        let legacy = normalize_wire_order(&mut list, Some("insights-python"), Some("3.0.0"));
    ...
    fn unknown_lib_is_untouched() {
        let mut list: ExceptionList =
            vec![exception_with_frames("Error", &["main", "boom"])].into();
        assert!(normalize_wire_order(&mut list, Some("insights-martian"), None).is_none());
        assert_eq!(frame_names(&list[0]), vec!["main", "boom"]);
    }
    ...
    fn normalized_crash_first_matches_native_canonical_fingerprint() {
        ...
        assert!(normalize_wire_order(&mut crash_first, Some("insights-rs"), None).is_some());
