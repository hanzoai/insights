...
from insights.api.team import TeamLogsConfigSerializer
...
class TestTeamLogsConfig(APIBaseTest):
    ...
    def test_patch_updates_distinct_id_keys(self, _name: str, prefix: str):
        response = self.client.patch(
            self._url(prefix),
            {"logs_distinct_id_attribute_keys": ["user.id", "insightsDistinctId"]},
            format="json",
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["logs_distinct_id_attribute_keys"], ["user.id", "insightsDistinctId"])

        config = get_or_create_team_extension(self.team, TeamLogsConfig)
        self.assertEqual(config.logs_distinct_id_attribute_keys, ["user.id", "insightsDistinctId"])

    @parameterized.expand(URL_PREFIXES)
    def test_patch_persists_across_requests(self, _name: str, prefix: str):
        self.client.patch(
            self._url(prefix),
            {"logs_distinct_id_attribute_keys": ["insights.distinct_id"]},
            format="json",
        )

        response = self.client.get(self._url(prefix))
        self.assertEqual(response.json()["logs_distinct_id_attribute_keys"], ["insights.distinct_id"])
    ...
    def test_patch_updates_session_id_keys_preserving_order(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["logs_session_id_attribute_keys"], ["session.id", "insightsSessionId"])

        config = get_or_create_team_extension(self.team, TeamLogsConfig)
        self.assertEqual(config.logs_session_id_attribute_keys, ["session.id", "insightsSessionId"])
...
INVALID_KEY_LISTS = [
    ("empty_list", []),
    ("blank_entry", ["insightsSessionId", ""]),
    ("whitespace_entry", ["insightsSessionId", "   "]),
