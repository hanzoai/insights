...
from insights.temporal.ai import AI_WORKFLOWS, SIGNALS_WORKFLOWS
from insights.temporal.ai.session_summary.activities.capture_timing import capture_timing_activity
...
from insights.temporal.ai.session_summary.state import (
    StateActivitiesEnum,
...
from insights.temporal.ai.session_summary.summarize_session import get_llm_single_session_summary_activity
from insights.temporal.ai.session_summary.summarize_session_group import (
...
from insights.temporal.ai.session_summary.types.group import (
...
from insights.temporal.tests.ai.conftest import AsyncRedisTestContext
...
class TestSummarizeSessionGroupWorkflow:
    ...
    def execute_test_environment(
        ...
        with (
            ...
            patch(
                "insights.temporal.ai.session_summary.summarize_session_group.SessionReplayEvents.get_group_metadata",
                return_value=MockMetadataDict(),
            ),
            patch(
                "insights.temporal.ai.session_summary.summarize_session_group._get_db_events_per_page",
            ...
            patch("insights.temporal.ai.session_summary.activities.patterns.async_connect", return_value=mock_client),
        ):
            yield
    ...
    async def test_execute_summarize_session_group(
        ...
        with patch(
            "insights.temporal.ai.session_summary.summarize_session_group._start_session_group_summary_workflow",
    ...
    async def test_start_session_group_summary_workflow_fetches_from_db(
        ...
        with patch(
            "insights.temporal.ai.session_summary.summarize_session_group.async_connect",
...
class TestPatternExtractionChunking:
    ...
    async def test_all_sessions_fit_in_single_chunk(
        ...
        with patch(
            "insights.temporal.ai.session_summary.activities.patterns.estimate_tokens_from_strings"
        ) as mock_estimate:
...
async def test_combine_patterns_from_chunks_activity(
    ...
    with (
        patch(
            "insights.temporal.ai.session_summary.activities.patterns.get_llm_session_group_patterns_combination"
        ) as mock_combine,
        patch("temporalio.activity.info") as mock_activity_info,
    ):
        mock_activity_info.return_value.workflow_id = "test_workflow_id"
...
async def test_run_patterns_extraction_with_chunking_and_redis_keys(
    ...
    with (
        patch(
            "insights.temporal.ai.session_summary.summarize_session_group.split_session_summaries_into_chunks_for_patterns_extraction_activity",
            new_callable=AsyncMock,
        ) as mock_split,
        patch(
            "insights.temporal.ai.session_summary.summarize_session_group.extract_session_group_patterns_activity",
            new_callable=AsyncMock,
        ) as mock_extract,
        patch(
            "insights.temporal.ai.session_summary.activities.patterns.get_llm_session_group_patterns_combination",
...
class TestSessionBatchFetchExpectedSkips:
    ...
    async def test_short_sessions_are_expected_skips(self, ateam: Team, auser: User):
        ...
        from insights.temporal.ai.session_summary.types.group import SessionBatchFetchOutput
        ...
        with (
            ...
            patch("insights.temporal.ai.session_summary.summarize_session_group.get_async_client"),
        ):
