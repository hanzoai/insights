...
from products.review.backend.temporal.activities import FetchPRDataInput, ReviewMeta, _fetch_and_persist
from products.review.backend.temporal.client import _build_inputs
...
class TestBuildInputs(BaseTest):
    ...
    @parameterized.expand([("bare_name", "insights"), ("nested_path", "a/b/c")])
    ...
    def test_workflow_id_lowercases_a_mixed_case_repository(self) -> None:
        ...
        _inputs, workflow_id = self._build(repository="Insights/Hanzo.ai", head_branch="Feat")

        assert workflow_id == f"review-branch:{self.team.id}:insights/hanzo.ai:feat"
