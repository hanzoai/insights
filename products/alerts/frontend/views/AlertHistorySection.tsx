...
import { isAnyRowInsightsQLConfig } from '../types'
...
export function AlertHistorySectionSkeleton(): JSX.Element {
    return (
        <div className="mt-10 space-y-2" aria-busy="true" aria-label="Loading alert history">
            ...
            <Skeleton className="h-56 w-full min-h-56" />
...
export function AlertHistorySection({
    ...
    const isAnyRowSqlAlert = isAnyRowInsightsQLConfig(alert?.config)

    const checkHistoryColumns = useMemo((): TableColumn<AlertCheck, keyof AlertCheck | undefined>[] => {
        const columns: TableColumn<AlertCheck, keyof AlertCheck | undefined>[] = [
            {
                title: 'Status',
                key: 'state',
