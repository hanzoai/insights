...
// Bound live-dashboard query fan-out to protect Datastore capacity.
...
const performLiveQuery = <N extends InsightsQLQuery | TrendsQuery>(
    query: N,
    signal: AbortSignal
...
export interface liveWebAnalyticsMetricsLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        hasActiveFilters: (liveFilters: WebAnalyticsPropertyFilter[]) => boolean
    }
}
...
export const liveWebAnalyticsMetricsLogic = kea<liveWebAnalyticsMetricsLogicType>([
    ...
    selectors({
        ...
        hasActiveFilters: [
            (s) => [s.liveFilters],
            (liveFilters: WebAnalyticsPropertyFilter[]): boolean => liveFilters.length > 0,
        ],
    }),
        ...
        loadInitialData: async () => {
            ...
            try {
                ...
                if (data.failedQueries.length > 0) {
                    toast.warning(`Some live metrics failed to load: ${data.failedQueries.join(', ')}`, {
...
// understood by the livestream filter. Array values produce one param per entry, which the
...
export const appendFilterParams = (url: URL, filters: WebAnalyticsPropertyFilter[]): void => {
    for (const f of filters) {
        ...
        for (const v of values) {
            ...
            url.searchParams.append('property', `${f.key}=${String(v)}`)
        }
    }
}
...
interface LiveQueryData {
    usersPageviews: InsightsQLQueryResponse | null
    device: InsightsQLQueryResponse | null
    browser: InsightsQLQueryResponse | null
    paths: TrendsQueryResponse | null
    referrer: InsightsQLQueryResponse | null
    geo: InsightsQLQueryResponse | null
    recentUsers: InsightsQLQueryResponse | null
    bot: InsightsQLQueryResponse | null
    city: InsightsQLQueryResponse | null
    allFailed: boolean
...
const loadQueryData = async ({
    ...
    const queryParams: Pick<InsightsQLQuery, 'values' | 'filters'> = filtersEnabled
        ? {
              filters: {
                  properties: filters,
    ...
    const createBreakdownQuery = (property: string, alias: string, name: string): InsightsQLQuery => ({
        kind: NodeKind.InsightsQLQuery,
        query: `SELECT
                    minute_bucket,
                    mapFromArrays(
    ...
    const referrerQuery: InsightsQLQuery = {
        kind: NodeKind.InsightsQLQuery,
        query: `SELECT
                    toStartOfMinute(timestamp) AS minute_bucket,
    ...
    const geoQuery: InsightsQLQuery = {
        kind: NodeKind.InsightsQLQuery,
        query: `SELECT
                    minute_bucket,
                    mapFromArrays(
    ...
    // the default InsightsQL row cap (100) silently truncates high-traffic projects.
    const botQuery: InsightsQLQuery = {
        kind: NodeKind.InsightsQLQuery,
        query: `SELECT
                    minute_bucket,
                    mapFromArrays(
    ...
    const cityQuery: InsightsQLQuery | null = includeCity
        ? {
              kind: NodeKind.InsightsQLQuery,
              query: `SELECT
                    minute_bucket,
    ...
    const recentUsersQuery: InsightsQLQuery | null =
    ...
    const jobs: { key: LiveQueryKey; query: InsightsQLQuery | TrendsQuery }[] = [
    ...
    settled.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
            if (key === 'paths') {
                ...
                data[key] = result.value as InsightsQLQueryResponse
            }
...
const getRecentUsersByLastSeenEntries = (response: InsightsQLQueryResponse): [string, number][] => {
...
const addUserDataToBuckets = (
    usersPageviewsResponse: InsightsQLQueryResponse | null,
...
const addBreakdownDataToBuckets = (
    response: InsightsQLQueryResponse | null,
    bucketMap: Map<number, SlidingWindowBucket>,
...
const addReferrerDataToBuckets = (
    referrerResponse: InsightsQLQueryResponse | null,
    bucketMap: Map<number, SlidingWindowBucket>
): void => {
    if (!referrerResponse) {
        return
...
const addGeoDataToBuckets = (
    geoResponse: InsightsQLQueryResponse | null,
    bucketMap: Map<number, SlidingWindowBucket>
): void => {
    if (!geoResponse) {
        return
...
const addCityDataToBuckets = (
    cityResponse: InsightsQLQueryResponse | null,
    bucketMap: Map<number, SlidingWindowBucket>
): void => {
    if (!cityResponse) {
        return
...
const addBotDataToBuckets = (
    botResponse: InsightsQLQueryResponse | null,
    bucketMap: Map<number, SlidingWindowBucket>
): void => {
    if (!botResponse) {
        return
