...
from insights.test.base import APIBaseTest, DatastoreTestMixin, _create_person, flush_persons_and_events
...
# sharded/distributed and absent from the test Datastore schema). Created per execution test.
...
class TestInsightsQLCohortQuery(DatastoreTestMixin, APIBaseTest):
    ...
    def test_dynamic_cohort_id_is_not_injectable(self) -> None:
        ...
        for cohort_type in ("dynamic-cohort", "static-cohort"):
            ...
            with self.assertRaises(ValueError):
                InsightsQLCohortQuery(cohort=cohort).get_query()
    ...
    def test_multiple_person_properties_optimization(self, mock_feature_enabled: MagicMock) -> None:
        ...
        self.assertIn(
            "and(isNotNull(persons.properties___email), ifNull(ilike(toString(persons.properties___email), %(insightsql_val_8)s), 0), ifNull(notILike(toString(persons.properties___email), %(insightsql_val_9)s), 1))",
            query_str,
        )
    ...
    def test_person_metadata_cohort_membership_end_to_end(self) -> None:
        # Persons need a deterministic created_at in BOTH Postgres and Datastore.
    ...
    def test_filter_test_accounts_excludes_person_matches_and_tracks_team_settings(self) -> None:
        internal = _create_person(
            ...
            properties={"email": "employee@hanzo.ai", "plan": "paid"},
            immediate=True,
        )
...
class TestInsightsQLRealtimeCohortQuery(DatastoreTestMixin, APIBaseTest):
                    ...
                    "values": [
                        {
                            ...
                            "bytecode": ["_H", 1, 32, "email", 32, "@hanzo.ai", 2, "icontains", 2],
                        }
                    ],
                }
            ],
    ...
    def test_cohort_membership_in_cohort_direct(self) -> None:
        ...
        query_ast = insightsql_query.get_dynamic_cohort_condition(prop)
    ...
    def test_behavioral_performed_event_with_date_range(self) -> None:
        ...
        query_str = InsightsQLRealtimeCohortQuery(cohort=cohort).query_str("datastore")

        self.assertIn("precalculated_events", query_str)
        # Both sides of the window should be emitted. InsightsQL prints `>=`/`<=` as
        # `greaterOrEquals(...)` / `lessOrEquals(...)` function calls for Datastore.
    ...
    def test_sibling_single_property_groups_under_or_merge(self) -> None:
        ...
        tuple_pattern = r"tuple\(%\(insightsql_val_\d+\)s,\s*%\(insightsql_val_\d+\)s,\s*%\(insightsql_val_\d+\)s\)"
    ...
    def test_merged_property_with_empty_hashes_raises_error(self) -> None:
        ...
        with self.assertRaises(ValueError) as context:
            insightsql_query.get_person_condition(prop)
    ...
    def test_create_merged_property_with_empty_hashes_raises_error(self) -> None:
        ...
        with self.assertRaises(ValueError) as context:
            insightsql_query._create_merged_property(template, [], is_or_group=True)
    ...
    def test_single_scan_optimization(
        ...
        query_str = InsightsQLRealtimeCohortQuery(cohort=cohort).query_str("datastore").lower()
    ...
    def test_collect_person_property_hashes_operator_mismatch_guard(
        ...
        query = InsightsQLRealtimeCohortQuery(cohort=cohort)
