...
MYSQL_FIELD_TYPE_TO_DATASTORE_TYPE: dict[int, str] = {
...
def mysql_field_type_to_datastore_type(type_code: int | None) -> str:
    if type_code is None:
        return "String"
    return MYSQL_FIELD_TYPE_TO_DATASTORE_TYPE.get(type_code, "String")
    ...
    for token in statement.flatten():
        if token.ttype in sqlparse_tokens.Comment:
            if _is_executable_mysql_comment(token.value):
                raise ExposedInsightsQLError(RAW_MYSQL_READ_ONLY_ERROR)
            continue
...
class MySQLAdapter:
    engine = "mysql"
    dialect: InsightsQLDialect | None = "mysql"
    ...
    def validate_source_config(
        ...
        if not (is_direct_capable(source) and source.direct_engine == self.engine):
            raise ExposedInsightsQLError("Invalid direct MySQL connection.")
        ...
        if not valid_host:
            raise ExposedInsightsQLError(host_errors or "Invalid MySQL host.")
    ...
    def execute(self, request: DirectQueryRequest) -> DirectQueryResult:
        ...
        except (pymysql.MySQLError, ExposedInsightsQLError) as error:
            span.set_attribute("error_type", error.__class__.__name__)
            if request.debug:
                return DirectQueryResult(results=[], types=[], print_columns=[], error=mysql_error_to_message(error))
            raise ExposedInsightsQLError(mysql_error_to_message(error)) from error

        span.set_attribute("row_count", len(results))
        types: list[tuple[str, str]] = [
            (column[0], mysql_field_type_to_datastore_type(column[1])) for column in description
        ]
