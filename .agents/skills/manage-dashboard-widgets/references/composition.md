...
When the widget surfaces data from an **existing Insights product** (variant in an existing `groupId`, or first widget in a product area that already has a scene):
...
- **Analytics** — `getDashboardWidgetDefinition` still dedupes a Insights `captureException` per canonical type — add the registry entry

## Widget settings modal

`Modal` + sections per `Edit*WidgetModal` — no shared wrapper. Copy `EditErrorTrackingWidgetModal.tsx`.
...
```tsx
<Modal … footer={/* Cancel + Save with saveDisabledReason / saving */}>
...
- Per-type persisted config + modal form schema from `generated/widget-configs.zod.ts`; colocated `*WidgetConfigValidation.ts` for API error parsing only (reuse `widgets/widgetConfigValidation.ts` for shared InsightsQL helpers — no hand-rolled field guards)
