...
description: >
  Creates and updates GitHub Actions secrets for Insights workflows. Use when
...
# Managing GitHub Actions secrets for Insights

Insights centralizes all GitHub Actions secrets at the **organization** level
...
- **Always** create secrets on the `insights` org, not on a repo.
...
Pipe the secret value into `gh secret set` with `--org insights`. The example
...
pbpaste | gh secret set INSIGHTSOS_PACKAGER_KEY --org insights
```
...
```sh
# From a file
gh secret set INSIGHTSOS_PACKAGER_KEY --org insights < secret.txt

# Restrict to selected repositories at creation time
gh secret set INSIGHTSOS_PACKAGER_KEY --org insights \
  --visibility selected --repos Insights/insights,Insights/insights-foss

# Update which repos can access an existing org secret
gh secret set INSIGHTSOS_PACKAGER_KEY --org insights \
  --visibility selected --repos Insights/insights
```

Verify:

```sh
gh secret list --org insights | grep INSIGHTSOS_PACKAGER_KEY
```

## Creating or updating a secret via the GitHub UI

1. Open <https://github.com/organizations/Insights/settings/secrets/actions>.
2. Click **New organization secret** (or the existing secret to update it).
3. Set the **Name** (SCREAMING_SNAKE_CASE, descriptive, like `INSIGHTSOS_PACKAGER_KEY`).
4. Paste the **Value**.
...
- Do not run `gh secret set NAME` without `--org insights` — that creates a
...
Default answer: at the org level via `gh secret set --org insights`, granted
