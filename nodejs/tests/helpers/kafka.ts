...
import {
    KAFKA_APP_METRICS_2,
    KAFKA_BUFFER,
    KAFKA_CDP_DATASTORE_BEHAVIORAL_COHORTS_MATCHES,
...
// Topics that the Datastore Kafka engine tables and the ingestion stack expect to
// exist. Created once (idempotently via ensureKafkaTopics) rather than deleted and
// recreated per test, so Datastore's Kafka consumers keep their partition assignments.
...
// isolated input topic without deleting the shared topics Datastore subscribes to.
 ...
 * Unlike resetKafka, this preserves Datastore Kafka engine consumer connections,
