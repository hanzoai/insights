...
``insights/temporal/mcp_analytics/intent_clustering/activities.py`` orchestrates
...
algorithm validatable without touching Datastore, Postgres, or the embedding
service.
...
from insights.api.embedding_worker import EmbeddingResponse, async_generate_embedding
...
from products.mcp_analytics.backend.insightsql_queries.base import EFFECTIVE_DESCRIPTION_SQL, EFFECTIVE_TOOL_SQL
...
# Bound on sessions sampled from Datastore for the corpus. Keeps the IN-tuple
...
# execute_insightsql_query injects LIMIT 100 into any query without an explicit
# LIMIT — far below what the per-session queries return at production scale
# (one-plus rows per corpus session). Cap explicitly at the InsightsQL per-query
...
# happens inside the corpus SQL (InsightsQL left → leftUTF8) so oversized values
# never leave Datastore — up to MAX_QUERY_ROWS of them would otherwise
...
# what leaves Datastore and lands in worker memory; the honest case (a few hundred
...
# JSONExtract of a Nullable into Array(String) is a Datastore type error.
...
def top_corpus_tools(records: list[IntentRecord], max_tools: int = MAX_TOOLS_IN_SNAPSHOT) -> set[str]:
    ...
    (the Datastore aggregation, the embedding fan-out) is bounded to this set."""
