...
from insights.temporal.session_replay.delete_recordings.workflow import (
...
TEST_CONFIG = DeletionConfig(deleted_by="test@hanzo.ai")
        ...
        async with Worker(
            ...
            result = await env.client.execute_workflow(
                ...
                RecordingsWithPersonInput(
                    ...
                    config=DeletionConfig(deleted_by="test@hanzo.ai", dry_run=True),
                ),
                id=workflow_id,
                task_queue=task_queue_name,
            )
    ...
    async with await WorkflowEnvironment.start_time_skipping() as env:
        async with Worker(
            ...
            result = await env.client.execute_workflow(
                DeleteRecordingsWithTeamWorkflow.run,
                RecordingsWithTeamInput(
                    team_id=TEST_TEAM_ID, config=DeletionConfig(deleted_by="test@hanzo.ai", dry_run=True)
                ),
                id=workflow_id,
                task_queue=task_queue_name,
            )
    ...
    async with await WorkflowEnvironment.start_time_skipping() as env:
        async with Worker(
            ...
            result = await env.client.execute_workflow(
                DeleteRecordingsWithTeamWorkflow.run,
                RecordingsWithTeamInput(
                    team_id=TEST_TEAM_ID, config=DeletionConfig(deleted_by="test@hanzo.ai", batch_size=100)
                ),
                id=workflow_id,
                task_queue=task_queue_name,
            )
...
async def test_rate_limiting_sleeps_when_execution_is_fast(num_sessions, max_per_second, expected_sleep):
    ...
    async with await WorkflowEnvironment.start_time_skipping() as env:
        async with Worker(
            ...
            with patch("insights.temporal.session_replay.delete_recordings.workflow.asyncio.sleep", mock_sleep):
                await env.client.execute_workflow(
                    DeleteRecordingsWithTeamWorkflow.run,
                    RecordingsWithTeamInput(
                        team_id=TEST_TEAM_ID,
                        config=DeletionConfig(deleted_by="test@hanzo.ai", max_deletions_per_second=max_per_second),
                    ),
                    id=str(uuid.uuid4()),
                    task_queue=task_queue_name,
                )
        ...
        async with Worker(
            ...
            with patch("insights.temporal.session_replay.delete_recordings.workflow.asyncio.sleep", mock_sleep):
                await env.client.execute_workflow(
                    ...
                    RecordingsWithTeamInput(
                        team_id=TEST_TEAM_ID,
                        config=DeletionConfig(deleted_by="test@hanzo.ai", max_deletions_per_second=0),
                    ),
                    id=str(uuid.uuid4()),
                    task_queue=task_queue_name,
                )
...
def test_delete_recordings_with_person_workflow_parse_inputs():
    result = DeleteRecordingsWithPersonWorkflow.parse_inputs(
        [
            '{"distinct_ids": ["id1", "id2"], "team_id": 123, "config": {"deleted_by": "test@hanzo.ai", "batch_size": 50}}'
        ]
    )
...
def test_delete_recordings_with_team_workflow_parse_inputs():
    result = DeleteRecordingsWithTeamWorkflow.parse_inputs(
        ['{"team_id": 456, "config": {"deleted_by": "test@hanzo.ai", "dry_run": true, "batch_size": 50}}']
    )
...
def test_delete_recordings_with_query_workflow_parse_inputs():
    result = DeleteRecordingsWithQueryWorkflow.parse_inputs(
        [
            '{"query": "date_from=-7d", "team_id": 789, "config": {"deleted_by": "test@hanzo.ai", "dry_run": false, "batch_size": 75}, "query_limit": 500}'
        ]
    )
        ...
        async with Worker(
            ...
            result = await env.client.execute_workflow(
                ...
                RecordingsWithSessionIdsInput(
                    ...
                    total_session_ids=3,
                    team_id=TEST_TEAM_ID,
                    config=DeletionConfig(deleted_by="test@hanzo.ai", reason="test cleanup"),
                ),
                id=workflow_id,
                task_queue=task_queue_name,
            )
...
def test_delete_recordings_with_session_ids_workflow_parse_inputs():
    result = DeleteRecordingsWithSessionIdsWorkflow.parse_inputs(
        [
            '{"s3_prefix": "deletion-inputs/wf-123/", "total_chunks": 3, "chunk_size": 10000, "total_session_ids": 25000, "team_id": 123, "config": {"deleted_by": "test@hanzo.ai", "batch_size": 50}}'
        ]
    )
...
def test_queue_delete_team_recordings(team_ids, expected_calls):
    from insights.tasks.tasks import _queue_delete_team_recordings
    ...
    with patch("insights.temporal.common.client.async_connect", side_effect=fake_connect):
        _queue_delete_team_recordings(team_ids, deleted_by="test@hanzo.ai")
...
def test_queue_delete_team_recordings_raises_when_temporal_unavailable():
    ...
    with patch("insights.temporal.common.client.async_connect", side_effect=fail_connect):
        with pytest.raises(RuntimeError, match="Temporal unavailable"):
            _queue_delete_team_recordings([1], deleted_by="test@hanzo.ai")
