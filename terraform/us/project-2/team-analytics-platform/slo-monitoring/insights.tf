locals {
  ...
  # events with matching properties.operation value (see insights/slo/types.py).
  # ===========================================================================
  slo_operations = {
    export = {
      name    = "Exports"
    ...
    query_service = {
      ...
      # insights/settings/web.py. Documentation only; the dashboard SQL reads
...
# ---------------------------------------------------------------------------
resource "insights_insight" "slo_burn_rate" {
  ...
  dashboard_ids = [insights_dashboard.slo_monitoring.id]
...
# ---------------------------------------------------------------------------
resource "insights_insight" "slo_duration" {
  for_each = local.slo_operation_regions
...
# ---------------------------------------------------------------------------
resource "insights_insight" "slo_success_rate" {
...
# ---------------------------------------------------------------------------
resource "insights_insight" "slo_volume" {
...
resource "insights_insight" "alert_delivery_by_type" {
...
resource "insights_insight" "alert_delivery_failure_rate" {
