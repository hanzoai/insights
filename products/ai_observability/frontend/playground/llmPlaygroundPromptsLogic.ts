...
export const llmPlaygroundPromptsLogic = kea<llmPlaygroundPromptsLogicType>([
    ...
    listeners(({ actions, values }) => ({
        resetPlayground: () => {
            insights.capture('llma playground reset')
        ...
        addPromptConfig: () => {
            ...
            insights.capture('llma playground prompt config added', {
                prompt_count: values.promptConfigs.length,
            })
        },
        removePromptConfig: ({ promptId }) => {
            insights.capture('llma playground prompt config removed', {
                prompt_count: values.promptConfigs.length,
            })
        ...
        addMessage: () => {
            insights.capture('llma playground message added', {
                message_count: values.activePromptConfig?.messages.length ?? 0,
            })
        },
        deleteMessage: () => {
            insights.capture('llma playground message removed', {
                message_count: values.activePromptConfig?.messages.length ?? 0,
            })
        },
        setTools: ({ tools }) => {
            insights.capture('llma playground tools configured', {
        ...
        setupPlaygroundFromEvent: async ({ payload }) => {
            ...
            insights.capture('llma playground opened from source', {
            ...
            try {
                if (payload.sourcePromptName) {
                    try {
                        ...
                        toast.error('Error loading prompt for playground')
                    }
                    return
                }
                ...
                if (payload.sourceEvaluationId) {
                    try {
                        ...
                        if (!teamId) {
                            toast.error('Could not determine team')
                            return
                        }
                        ...
                        toast.error('Error loading evaluation for playground')
                    }
                    return
                }
        ...
        saveToLinkedPrompt: async ({ promptId }) => {
            insights.capture('llma playground saved to source', { action: 'save_to_linked_prompt' })
            const { linkedSource, promptConfigs } = values
            if (!linkedSource.promptName) {
                toast.error('No linked prompt to save to')
                actions.saveComplete()
                return
            }
            const prompt = promptConfigs.find((p) => p.id === promptId)
            if (!prompt) {
                toast.error('No prompt configuration to save')
                actions.saveComplete()
                return
            }
            try {
                ...
                toast.error(getApiErrorDetail(error) || 'Failed to update prompt')
            } finally {
                actions.saveComplete()
            }
        },
        ...
        saveToLinkedEvaluation: async ({ promptId, modelConfig }) => {
            insights.capture('llma playground saved to source', { action: 'save_to_linked_evaluation' })
            const { linkedSource, promptConfigs } = values
            if (!linkedSource.evaluationId) {
                toast.error('No linked evaluation to save to')
                actions.saveComplete()
                return
            }
            const prompt = promptConfigs.find((p) => p.id === promptId)
            ...
            try {
                ...
                toast.error(getApiErrorDetail(error) || 'Failed to update evaluation')
            } finally {
                actions.saveComplete()
            }
        },
        ...
        saveAsNewPrompt: async ({ promptId, name }) => {
            insights.capture('llma playground saved to source', { action: 'save_as_new_prompt' })
            ...
            try {
                ...
                toast.error(getApiErrorDetail(error) || 'Failed to save prompt')
            } finally {
                actions.saveComplete()
            }
        },
        ...
        saveAsNewEvaluation: async ({ promptId, name, modelConfig }) => {
            insights.capture('llma playground saved to source', { action: 'save_as_new_evaluation' })
            const prompt = values.promptConfigs.find((p) => p.id === promptId)
            if (!prompt) {
                toast.error('No prompt configuration to save')
                actions.saveComplete()
            ...
            try {
                ...
                toast.error(getApiErrorDetail(error) || 'Failed to save evaluation')
            } finally {
                actions.saveComplete()
            }
        },
