...
from insights.helpers.email_utils import ESPSuppressionReason, ESPSuppressionResult
...
class TestCodeBasedVerifierSuppressionIntegration(SimpleTestCase):
    ...
    @patch("insights.helpers.two_factor_session.hanzo_insights.feature_enabled")
    ...
    @patch("insights.helpers.two_factor_session.hanzo_insights.feature_enabled")
    @patch("insights.helpers.two_factor_session.check_esp_suppression")
    @patch("insights.helpers.two_factor_session.hanzo_insights.capture")
...
class TestCodeBasedVerificationGlobalDisable(SimpleTestCase):
    ...
    def test_set_get_clear_round_trip(self):
        set_code_based_verification_global_disable(
            reason="email pipeline down", ttl_seconds=3600, disabled_by="support@hanzo.ai"
        )
        ...
        self.assertEqual(state["disabled_by"], "support@hanzo.ai")
    ...
    def test_reason_is_stripped_and_required(self):
        with self.assertRaises(ValueError):
            set_code_based_verification_global_disable(
                reason="   ", ttl_seconds=3600, disabled_by="support@hanzo.ai"
            )
        self.assertFalse(is_code_based_verification_globally_disabled())
    ...
    def test_invalid_ttl_rejected(self, _name, ttl_seconds):
        with self.assertRaises(ValueError):
            set_code_based_verification_global_disable(
                reason="reason", ttl_seconds=ttl_seconds, disabled_by="support@hanzo.ai"
            )
        self.assertFalse(is_code_based_verification_globally_disabled())
    ...
    @patch("insights.helpers.two_factor_session.get_client")
