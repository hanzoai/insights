            ...
            submit: async (formValues) => {
                ...
                try {
                    ...
                    if (isNew) {
                        ...
                        llmSkillsLogic.findMounted()?.actions.loadSkills(false)
                        toast.success('Skill version published successfully')
                    ...
                    const detail =
                        error !== null && typeof error === 'object' && 'detail' in error
                            ...
                            : undefined
                    toast.error(detail || 'Failed to save skill')
                    throw error
                }
            },
        },
    ...
    listeners(({ actions, props, values }) => ({
        addUploadedFiles: async ({ files }) => {
            ...
            for (const { path, file } of files) {
                ...
                if (file.size > SKILL_FILE_MAX_BYTES) {
                    toast.error(`Couldn't add ${path}: files must be 1 MB or smaller`)
                    continue
                }
                ...
                try {
                    ...
                    toast.error(`Couldn't add ${path}: only text files are supported`)
                    continue
                }
                if (content.includes('\u0000')) {
                    toast.error(`Couldn't add ${path}: only text files are supported`)
        ...
        downloadSkill: async () => {
            ...
            try {
                ...
                toast.error(detail || (e instanceof Error ? e.message : 'Failed to export skill'))
            } finally {
        ...
        deleteSkill: async () => {
            if (props.skillName !== 'new' && values.skill && isSkill(values.skill)) {
                try {
                    ...
                    toast.info(`${values.skill.name || 'Skill'} has been archived.`)
                    llmSkillsLogic.findMounted()?.actions.loadSkills(false)
