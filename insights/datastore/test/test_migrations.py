...
class TestUniqueMigrationPrefixes(TestCase):
    ...
    def test_max_migration_txt_is_valid(self):
        ...
        self.assertTrue(
            ...
            "max_migration.txt does not exist in datastore/migrations/. "
    ...
    def test_alter_on_replicated_tables_has_correct_flag(self):
        ...
        try:
            for deployment in CLOUD_DEPLOYMENTS_TO_CHECK:
                if not deployment:
                    continue
                with mock.patch("insights.settings.CLOUD_DEPLOYMENT", deployment):
                    for name, module in self._checked_modules():
        ...
        if violations:
            error_message = "Found Datastore migration operations with convention violations:\n\n"
            for v in violations:
            ...
            error_message += "For more information, see insights/datastore/migrations/AGENTS.md\n"
            self.fail(error_message)
    ...
    def test_no_on_cluster_in_migration_source_strings(self):
        ...
        if violations:
            ...
            msg += "\nDatastore migrations must not use `ON CLUSTER` — do not put it in new code, "
            msg += "for old SQL use ON_CLUSTER_CLAUSE(False) "
            msg += "and run via node_roles=NodeRole.X per-shard. See insights/datastore/migrations/AGENTS.md.\n"
            self.fail(msg)
