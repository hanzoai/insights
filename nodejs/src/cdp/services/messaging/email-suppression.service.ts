...
/**
 * Per-team email suppression list, backed by `insights_messagesuppression`.
 *
...
export class EmailSuppressionService {
    ...
    public async recordTransientBounces(teamId: number, emails: string[], diagnostic?: string): Promise<void> {
        ...
        const query = `
            INSERT INTO insights_messagesuppression
            ...
            ON CONFLICT (team_id, identifier) DO UPDATE SET
                transient_bounce_count = insights_messagesuppression.transient_bounce_count + 1,
                last_bounce_at = NOW(),
                ...
                suppressed = CASE
                    WHEN insights_messagesuppression.source = 'MANUAL' THEN insights_messagesuppression.suppressed
                    WHEN insights_messagesuppression.transient_bounce_count + 1 >= $3 THEN true
                    ELSE insights_messagesuppression.suppressed END,
                suppressed_at = CASE
                    WHEN insights_messagesuppression.source <> 'MANUAL'
                        AND insights_messagesuppression.suppressed = false
                        AND insights_messagesuppression.transient_bounce_count + 1 >= $3 THEN NOW()
                    ELSE insights_messagesuppression.suppressed_at END,
                reason = CASE
                    WHEN insights_messagesuppression.source <> 'MANUAL'
                        AND insights_messagesuppression.transient_bounce_count + 1 >= $3 THEN $2
                    ELSE insights_messagesuppression.reason END,
                -- A provably-undeliverable address coming back over threshold un-deletes itself.
                deleted = CASE
                    WHEN insights_messagesuppression.source <> 'MANUAL'
                        AND insights_messagesuppression.transient_bounce_count + 1 >= $3 THEN false
                    ELSE insights_messagesuppression.deleted END,
                updated_at = NOW()
        `
        ...
        const query = `
            ...
            ON CONFLICT (team_id, identifier) DO UPDATE SET
                ...
                suppressed = CASE
                    ...
                    ELSE true END,
                suppressed_at = CASE
                    WHEN insights_messagesuppression.source = 'MANUAL' THEN insights_messagesuppression.suppressed_at
                    WHEN insights_messagesuppression.suppressed = false THEN NOW()
                    ELSE insights_messagesuppression.suppressed_at END,
                reason = CASE
                    WHEN insights_messagesuppression.source = 'MANUAL' THEN insights_messagesuppression.reason
                    ELSE EXCLUDED.reason END,
                -- A provably-undeliverable address coming back un-deletes itself.
                deleted = CASE
                    WHEN insights_messagesuppression.source = 'MANUAL' THEN insights_messagesuppression.deleted
                    ELSE false END,
                updated_at = NOW()
        `
    ...
    public async recordDeliveries(teamId: number, emails: string[], deliveryTimestamp?: string): Promise<void> {
        ...
        const query = `
            UPDATE insights_messagesuppression
    ...
    private async loadSuppressed(keys: string[]): Promise<Record<string, boolean>> {
        ...
        for (const [teamId, identifiers] of byTeam) {
            const result = await this.postgres.query<{ identifier: string }>(
                ...
                `SELECT identifier
                 FROM insights_messagesuppression
                 WHERE team_id = $1
                   AND identifier = ANY($2)
                   AND suppressed = true
                   AND deleted = false`,
