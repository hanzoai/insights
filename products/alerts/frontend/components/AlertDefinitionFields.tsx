...
import { InsightsQLAlertPreview } from 'products/alerts/frontend/logic/insightsqlAlertPreview'
import { isFunnelsAlertConfig, isInsightsQLAlertConfig } from 'products/alerts/frontend/types'

import { InsightsQLAlertPreviewBanner, InsightsQLAlertPreviewRowsTable } from './InsightsQLAlertPreview'
...
export function FunnelAlertPreviewBanner({ preview }: { preview: FunnelAlertPreview | null }): JSX.Element | null {
    ...
    const statusTag = preview.hasBounds ? (
        <Tag type={wouldFire ? 'danger' : 'success'} className="mr-2">
...
export function InsightsQLDefinitionFields({
    alertForm,
    insightsqlPreview,
    insightsqlColumns,
    ...
    alertForm: AlertFormType
    insightsqlPreview: InsightsQLAlertPreview | null
    insightsqlColumns: string[] | null
    insightsqlValueColumnOptions: { label: string; value: string }[]
    insightsqlLabelColumnOptions: { label: string; value: string }[]
    showInlinePreview: boolean
    onSetAlertFormValue: <K extends keyof AlertFormType>(key: K, value: AlertFormType[K]) => void
}): JSX.Element {
    const hasMultipleColumns = (insightsqlColumns?.length ?? 0) > 1
    const config = isInsightsQLAlertConfig(alertForm.config) ? alertForm.config : null
    const showValueColumn = hasMultipleColumns || !!config?.column
    const valueColumnOptions = [...insightsqlValueColumnOptions]
    ...
    return (
        <>
            <AlertDefinitionRow label="When">
                <Group name={['config']}>
                    {showValueColumn && (
                        <Field name="column" className="flex-auto">
                            <Select
                                fullWidth
                                data-attr="alertForm-insightsql-column"
                    ...
                    <Field name="evaluation" className="flex-auto">
                        {({ value, onChange }) => (
                            <Select
                                fullWidth
                                data-attr="alertForm-insightsql-evaluation"
            ...
            {hasMultipleColumns && (
                <div className="flex gap-3 items-center">
                    ...
                    <Group name={['config']}>
                        <Field name="label_column" className="flex-auto">
                            {/* Prefilled with the first non-evaluated column by alertFormLogic. */}
                            <Select
                                fullWidth
                                data-attr="alertForm-insightsql-label-column"
                                placeholder="select label column"
                                options={insightsqlLabelColumnOptions}
                            />
                        </Field>
                    </Group>
                </div>
