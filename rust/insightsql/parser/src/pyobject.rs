//! Convert a parser-built `serde_json::Value` AST into `insights.insightsql.ast` dataclass instances directly, skipping the JSON-string serialise step on the Rust side AND the orjson + `_deserialize_node` walk on the Python side. Mirrors [`insights/insightsql/json_ast.py`].
//!
...
pub(crate) fn raise_error_envelope(
    ...
    let errors_module = match py.import("insights.insightsql.errors") {
        Ok(m) => m,
        Err(e) => return e,
    };
...
impl<'py> Converter<'py> {
    ...
    /// Top-level entry: convert the root `Value` to a Python AST instance. Raises `ExposedInsightsQLError` / `SyntaxError` / `ParsingError` if the parser returned an error envelope.
    ...
    fn build_error(&self, map: &serde_json::Map<String, Value>) -> PyErr {
        let error_type = map
            ...
            .unwrap_or("ExposedInsightsQLError");
        let message = map
            .get("message")
            .and_then(Value::as_str)
