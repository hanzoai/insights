...
# slo_operation emits through hanzo_insights.capture; patch that boundary to inspect the SLO events.
_SLO_CAPTURE = "insights.slo.events.hanzo_insights.capture"
...
_ALL_FAILED_RUN = (
    ["### s0\n\n_Query failed to run (ExposedInsightsQLError)_"],
    1,
    [QueryStepDiagnostic("s0", "SELECT bad", False, "ExposedInsightsQLError")],
)
...
def _spec(steps: int = 1) -> EnrichedPromptSpec:
    return EnrichedPromptSpec(
        ...
        plan=QueryPlan(
            overall_intent="i",
            steps=[QueryPlanStep(description=f"s{n}", insightsql="SELECT 1") for n in range(steps)],
        ),
    )
...
def _spec_with_window_placeholder() -> EnrichedPromptSpec:
    return EnrichedPromptSpec(
        ...
        plan=QueryPlan(
            overall_intent="i",
            steps=[QueryPlanStep(description="s0", insightsql="SELECT count() FROM events WHERE {{date_range}}")],
        ),
...
async def test_successful_report_emits_slo_success(
    ...
    mock_run.return_value = (
        ...
        0,
        [
            QueryStepDiagnostic(description="s0", insightsql="SELECT 1", ok=True, error_type=None),
            QueryStepDiagnostic(description="s1", insightsql="SELECT 2", ok=True, error_type=None),
        ],
    )
...
async def test_degraded_report_still_synthesizes(
    ...
    mock_run.return_value = (
        ["### s0\n\n_Query failed to run (ExposedInsightsQLError) — metric not computed, not empty data._"],
        1,
        [QueryStepDiagnostic(description="s0", insightsql="SELECT bad", ok=False, error_type="ExposedInsightsQLError")],
    )
    ...
    # The failed step's generated InsightsQL + error type are surfaced for persistence/debugging.
...
@patch(f"{_RP}.MaxChatOpenAI")
async def test_request_insightsql_fix_returns_fixed_query(mock_chat: MagicMock) -> None:
    structured = mock_chat.return_value.with_structured_output.return_value
    structured.invoke.return_value = InsightsQLFix(fixed_insightsql="SELECT 2")
    result = await _arequest_insightsql_fix(
        original_insightsql="SELECT 1",
        error_message="boom",
        step_description="d",
        context_blob="c",
        team=MagicMock(),
...
@patch(f"{_RP}.MaxChatOpenAI")
async def test_request_insightsql_fix_returns_none_on_wrong_type(mock_chat: MagicMock) -> None:
    structured = mock_chat.return_value.with_structured_output.return_value
    structured.invoke.return_value = "not a InsightsQLFix"
    result = await _arequest_insightsql_fix(
        original_insightsql="SELECT 1",
        error_message="boom",
        step_description="d",
...
@patch(f"{_RP}.MaxChatOpenAI")
async def test_request_insightsql_fix_grounds_prompt_in_project_schema(
    ...
    structured = mock_chat.return_value.with_structured_output.return_value
    structured.invoke.return_value = InsightsQLFix(fixed_insightsql="SELECT 2")
    await _arequest_insightsql_fix(
        original_insightsql="SELECT 1",
...
async def test_run_steps_non_retryable_error_degrades_to_placeholder(mock_executor_cls: MagicMock) -> None:
    ...
    assert diagnostics[0].insightsql == "SELECT 1"
...
async def test_run_steps_retries_then_succeeds(mock_executor_cls: MagicMock, mock_fix: AsyncMock) -> None:
    ...
    # The diagnostic tracks the fixed query (current_insightsql), not the original SELECT 1.
    assert diagnostics[0].ok is True
    assert diagnostics[0].insightsql == "SELECT fixed"
    # Proves the in-place write-back; test_freeze_carries_post_fix_insightsql covers the guard -> freeze path.
    assert spec.plan.steps[0].insightsql == "SELECT fixed"
...
async def test_run_steps_breaks_early_when_fix_returns_same_query(
    ...
    assert diagnostics[0].error_type == "ExposedInsightsQLError"
...
@pytest.mark.parametrize(
    "exc,expected",
    [
        (ExposedInsightsQLError("Unable to resolve field 'operaton'"), "Unable to resolve field 'operaton'"),
        (ResolutionError("Unknown field: signups"), "Unknown field: signups"),
        # A plain InternalInsightsQLError (not a ResolutionError) can echo team-scoped data — stays type-only.
        (InternalInsightsQLError("internal detail with a team-scoped id"), None),
        ...
        (_wrap(RuntimeError("boom"), context=ExposedInsightsQLError("bad thing")), "bad thing"),
...
def test_safe_error_message_strips_null_bytes():
    ...
    exc = ExposedInsightsQLError("bad\x00field")
...
def _frozen_plan() -> dict:
    return {
        ...
        "plan": QueryPlan(
            ...
            steps=[QueryPlanStep(description="counts", insightsql="SELECT count() FROM events WHERE {{date_range}}")],
        ).model_dump(),
    }
...
def test_plan_to_freeze_requires_no_failures(total_steps: int, failed_count: int, should_freeze: bool) -> None:
    ...
    plan = QueryPlan(
        overall_intent="i",
        steps=[
            QueryPlanStep(description=f"s{n}", insightsql="SELECT count() FROM events WHERE {{date_range}}")
...
@pytest.mark.parametrize(
    "fixed_insightsql,expected_frozen_insightsql",
    [
        ...
        pytest.param(
            ...
            "SELECT uniq(person_id) FROM events WHERE {{date_range}}",
            id="post_fix_insightsql_is_frozen",
        ),
...
async def test_freeze_carries_post_fix_insightsql(
    ...
    if expected_frozen_insightsql is None:
        assert result.plan_to_persist is None
        return
    assert result.plan_to_persist is not None
    assert result.plan_to_persist["version"] == AI_QUERY_PLAN_VERSION
    assert result.plan_to_persist["plan"]["steps"][0]["insightsql"] == expected_frozen_insightsql
...
async def test_run_steps_substitutes_fresh_window_into_placeholder_sql(mock_executor_cls: MagicMock) -> None:
    ...
    spec = EnrichedPromptSpec(
        ...
        plan=QueryPlan(
            overall_intent="i",
            steps=[QueryPlanStep(description="s", insightsql="SELECT count() FROM events WHERE {{date_range}}")],
        ),
    )
