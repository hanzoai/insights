...
class Command(BaseCommand):
    ...
    def _undelete_metadata(self, session_ids: list[tuple[str, int]]) -> None:
        self.stdout.write("\nUndeleting metadata in Datastore...")
        ...
        sync_execute(
            f"ALTER TABLE sharded_session_replay_events ON CLUSTER '{DATASTORE_CLUSTER}'"
            " UPDATE is_deleted = 0"
            " WHERE session_id IN %(session_ids)s AND team_id IN %(team_ids)s",
            {"session_ids": session_id_list, "team_ids": list(team_ids)},
            settings={"mutations_sync": 2},
        )
        self.stdout.write(f"  Datastore: undeleted {len(session_id_list)} sessions")
