...
import { findNextAction } from '../flow-utils'
import { ActionHandler, ActionHandlerOptions, ActionHandlerResult } from './action.interface'

type Action = Extract<FlowAction, { type: 'random_cohort_branch' }>

export class RandomCohortBranchHandler implements ActionHandler {
    execute({
        invocation,
        action,
    }: ActionHandlerOptions<Extract<FlowAction, { type: 'random_cohort_branch' }>>): ActionHandlerResult {
        const nextAction = getRandomCohort(invocation, action)
        return { nextAction, result: { assigned_cohort: nextAction.id } }
    }
}

export function getRandomCohort(invocation: CyclotronJobInvocationFlow, action: Action): FlowAction {
    ...
    if (cohorts.length === 0) {
        return findNextAction(invocation.flow, action.id)
    }
    ...
    for (const [index, cohort] of cohorts.entries()) {
        ...
        if (random <= cumulativePercentage) {
            return findNextAction(invocation.flow, action.id, index)
        }
    }
    ...
    return findNextAction(invocation.flow, action.id, cohorts.length - 1)
}
