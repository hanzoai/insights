//! Rust ScriptVM exposed to Node via napi-rs, for executing ingestion transformations.
//!
...
//! `isKnownBotIp`) mirror `nodejs/src/cdp/script-transformations/transformation-functions.ts`; call
...
/// Run one Script program against one event-globals synchronously on the calling thread. This is the
