...
import { HogTransformerService, createHogTransformerService } from '~/cdp/script-transformations/script-transformer.service'
...
describe('IngestionConsumer', () => {
    ...
    describe('event batching', () => {
        ...
        it('should split AI events with large properties into events + ai_events', async () => {
            ...
            const producedMessages = mockProducerObserver.getProducedKafkaMessages()
            const eventsTopicMessages = producedMessages.filter((m) => m.topic === 'datastore_events_json_test')
            const aiEventsTopicMessages = producedMessages.filter((m) => m.topic === 'datastore_ai_events_json_test')
    ...
    describe('transformations', () => {
        ...
        const insertInsightsFunction = async (insightsFunction: Partial<InsightsFunctionType>) => {
            ...
            const item = await _insertInsightsFunction(infra.postgres, team.id, {
                script,
                bytecode,
        ...
        it(
            ...
            async () => {
                ...
                infra.config.CDP_FN_WATCHER_SAMPLE_RATE = 1
                ...
                const fetchAndCacheSpy = jest.spyOn(concreteTransformer, 'fetchAndCacheInsightsFunctionStates')
                const clearStatesSpy = jest.spyOn(concreteTransformer, 'clearInsightsFunctionStates')
        ...
        it(
            ...
            async () => {
                // Create a new ingester with the sample rate we want to test
                infra.config.CDP_FN_WATCHER_SAMPLE_RATE = 0
                const localIngester = await createIngestionConsumer(infra)
