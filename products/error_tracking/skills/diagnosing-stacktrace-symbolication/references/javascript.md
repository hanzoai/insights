...
Companion to [../SKILL.md](../SKILL.md) for JavaScript and TypeScript web apps. Covers `@hanzo/rollup-plugin`,
`@hanzo/webpack-plugin`, `@hanzo/nextjs-config`, `@hanzo/nuxt`, and direct `insights-cli sourcemap` invocations.
...
```bash
# pnpm
pnpm list @hanzo/rollup-plugin @hanzo/webpack-plugin @hanzo/nextjs-config @hanzo/nuxt @hanzo/cli vite rollup webpack next nuxt --depth 8

# npm
npm ls @hanzo/rollup-plugin @hanzo/webpack-plugin @hanzo/nextjs-config @hanzo/nuxt @hanzo/cli vite rollup webpack next nuxt

# yarn (classic)
yarn list --pattern '@hanzo/* vite rollup webpack next nuxt' --depth=0

# bun
bun pm ls | grep -E '@hanzo/|^├── (vite|rollup|webpack|next|nuxt)@'
```

Use non-zero depth — `@hanzo/cli` and Rollup are often transitive dependencies of the build plugin or framework.
...
Search for Insights upload config:

```bash
rg -n "insights|sourcemap|sourceMap|deleteAfterUpload|releaseName|releaseVersion|projectId|envId" .
```
...
```bash
rg -n "chunkId=|_insightsChunkIds|sourceMappingURL" dist
```

Expected local artifact shape:

- JS has a `chunkId` marker and `_insightsChunkIds` registration.
...
- `sourcesContent` is present when source context should appear in Insights.
...
signal that the bug is upstream of Insights upload — the CLI faithfully uploads whatever is on disk.
...
After [Step 3](../SKILL.md#step-3---check-symbol-sets-in-insights) extracts a JS symbol set with
`insights-cli symbol-sets extract`, run the helper on the resulting directory:
...
a redirect error pointing back at `insights-cli symbol-sets extract`.
...
For plugin-based builds, set `logLevel: "debug"` inside the same `insights({ ... })` options object that holds
...
- Move the Insights plugin later in the plugin order so it sees final emitted JS chunks.
