...
from products.review.backend.temporal.activities import _installation_auth
...
_CAN_ACCESS = "insights.models.integration.GitHubIntegration.installation_can_access_repository"
...
class TestInstallationAuth(BaseTest):
    ...
    def test_returns_the_token_and_the_github_installation_id(self, _can_access: MagicMock) -> None:
        ...
        # the Insights row pk — several rows can share one installation, which has ONE real budget.
        self._integration(installation_id="9876543")

        token, installation_id = _installation_auth(self.team.id, "Insights/insights")
    ...
    def test_no_accessible_installation_raises_retryably(self, _can_access: MagicMock) -> None:
        ...
        with pytest.raises(ApplicationError) as err:
            _installation_auth(self.team.id, "Insights/insights")
