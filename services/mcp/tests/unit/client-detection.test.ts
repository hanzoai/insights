...
describe('MCPClientProfile', () => {
    ...
    describe('isInsightsCodeConsumer()', () => {
        ...
        it.each([['slack'], ['insights'], ['Insights-Code'], ['insights_ai'], ['']])(
            'returns false for %s',
            (consumer) => {
                expect(new MCPClientProfile({ consumer }).isInsightsCodeConsumer()).toBe(false)
            }
        )

        it('returns false when consumer is undefined', () => {
            expect(new MCPClientProfile({}).isInsightsCodeConsumer()).toBe(false)
        })
    })
