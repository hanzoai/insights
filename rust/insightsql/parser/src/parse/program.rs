//! Script program-statement parsing — the imperative-language layer
//! built on top of the InsightsQL expression grammar.
//!
//! Grammar (InsightsQLParser.g4):
//!
...
impl<'a, E: Emitter + Clone> Parser<'a, E> {
    ...
    fn parse_var_decl(&mut self) -> Result<E::Value, ParseError> {
        ...
        let name = match name_tok.kind {
            ...
            // and the Script-statement keywords. `kw_valid_as_identifier`
    ...
    fn parse_statement_inner(&mut self) -> Result<E::Value, ParseError> {
        match self.peek() {
            ...
            //
            // Exception: a `<` that begins a InsightsQLX tag (`return <Tag/>`,
            // `return <a>x</a>`) is a return VALUE, not the less-than operator
            // binding `return` as a Field. `peek_next_starts_insightsqlx_tag` probes
    ...
    fn parse_return_stmt(&mut self) -> Result<E::Value, ParseError> {
        ...
        //     (varDecl), a Script-statement keyword, or a statement/block
...
/// and for the eight Script-statement keywords that are NOT in the
