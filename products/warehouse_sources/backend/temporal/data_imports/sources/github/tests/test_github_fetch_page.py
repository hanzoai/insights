...
def test_fetch_page_gates_on_egress_budget_when_installation_known():
    ...
    with (
        mock.patch("insights.egress.github.transport.consume_github_installation_sync", return_value=False) as gate,
...
def test_fetch_page_skips_gate_on_pat_path():
    ...
    with (
        mock.patch("insights.egress.github.transport.consume_github_installation_sync") as gate,
        mock.patch.object(github, "make_tracked_session", return_value=session),
    ):
