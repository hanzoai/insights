...
interface InsertTileOverlayProps {
    ...
    disabled?: boolean
    getMenuItems: (targetX: number, targetY: number, targetW?: number) => MenuItem[]
}
...
function InsertionStrip({
    ...
    // Move the "+" to follow the cursor by writing to the DOM directly — no setState, so the Menu
    ...
    return (
        ...
        >
            ...
            >
                <Menu items={menuItems} onVisibilityChange={setMenuOpen}>
                    <Button
                        size="xsmall"
                        type="primary"
                        icon={<IconPlusSmall />}
