...
import { FixtureCustomFlowBuilder, SimpleCustomFlowRepresentation } from '~/cdp/_tests/builders/customflow.builder'
import { createScriptExecutionGlobals, insertCustomFunctionTemplate, insertIntegration } from '~/cdp/_tests/fixtures'
...
import { CUSTOM_SCRIPT_FILTERS_EXAMPLES } from '../../_tests/examples'
...
import { CustomFlowExecutorService, createCustomFlowInvocation } from './customflow-executor.service'
...
describe('Customflow Executor', () => {
    let executor: CustomFlowExecutorService
    let hub: Hub
    ...
    beforeEach(async () => {
        ...
        const customFunctionTemplateManager = new CustomFunctionTemplateManagerService(hub.postgres)
        const customFlowFunctionsService = new CustomFlowFunctionsService(
            hub.SITE_URL,
            customFunctionTemplateManager,
            scriptExecutor
        )
        ...
        await insertCustomFunctionTemplate(hub.postgres, insightsCaptureTemplate)

        executor = new CustomFlowExecutorService(customFlowFunctionsService, recipientPreferencesService)
    })
    ...
    describe('general event processing', () => {
        ...
        it('can execute a simple customflow', async () => {
            ...
            expect(result).toEqual({
                ...
                invocation: {
                    ...
                    teamId: 1,
                    customFlow: invocation.customFlow,
                    person: {
                        id: 'person_id',
                        name: 'John Doe',
                        properties: {
                    ...
                    functionId: invocation.customFlow.id,
                    queue: 'customflow',
        ...
        it('can execute a customflow with async function delays', async () => {
            const action = customFlow.actions.find((action) => action.id === 'function_id_1')!
            ...
            expect(result.invocation.state.currentAction!.customFunctionState).toEqual(expect.any(Object))
            ...
            expect(result2.invocation.state.currentAction!.customFunctionState).toEqual(expect.any(Object))
        ...
        describe('action filtering', () => {
            beforeEach(() => {
                const action = customFlow.actions.find((action) => action.id === 'function_id_1')!
                action.filters = CUSTOM_SCRIPT_FILTERS_EXAMPLES.pageview_or_autocapture_filter.filters
            })
    ...
    describe('actions', () => {
        const createCustomFlow = (flow: SimpleCustomFlowRepresentation): CustomFlow => {
            return new FixtureCustomFlowBuilder()
        ...
        describe('early exit conditions', () => {
            ...
            it('should not exit early if exit condition is exit_only_at_end', async () => {
                ...
                const invocation2 = createExampleCustomFlowInvocation(customFlow, {
                    event: {
                        ...createScriptExecutionGlobals().event,
                        event: 'not-a-pageview',
                        properties: { name: 'John Doe', $current_url: 'https://hanzo.ai' },
            ...
            it('should exit early if exit condition is exit_on_conversion', async () => {
                customFlow.exit_condition = 'exit_on_conversion'
                customFlow.conversion = {
                    window_minutes: 10,
                    filters: CUSTOM_SCRIPT_FILTERS_EXAMPLES.pageview_or_autocapture_filter.filters,
                }
            ...
            it('should exit early if exit condition is exit_on_trigger_not_matched', async () => {
                customFlow.exit_condition = 'exit_on_trigger_not_matched'
                customFlow.trigger = {
                    type: 'event',
                    filters: CUSTOM_SCRIPT_FILTERS_EXAMPLES.pageview_or_autocapture_filter.filters ?? {},
                }

                const invocation1 = createExampleCustomFlowInvocation(customFlow, {
                    event: {
                        ...createScriptExecutionGlobals().event,
                        event: '$pageview',
                        properties: { name: 'John Doe', $current_url: 'https://hanzo.ai' },
            ...
            it('should exit early if exit condition is exit_on_trigger_not_matched_or_conversion', async () => {
                ...
                customFlow.exit_condition = 'exit_on_trigger_not_matched_or_conversion'
                customFlow.trigger = {
                    type: 'event',
                    filters: CUSTOM_SCRIPT_FILTERS_EXAMPLES.no_filters.filters ?? {},
                }
            ...
            describe('on_error handling', () => {
                ...
                describe('execute error handling when error is returned, not thrown', () => {
                    it('continues to next action when on_error is continue', async () => {
                        const action = customFlow.actions.find((a) => a.id === 'function_id_1')!
                    ...
                    it('does NOT continue to next action when on_error is abort', async () => {
                        ...
                        expect(loggerErrorSpy).toHaveBeenCalledWith(
                            '🦔',
                            expect.stringContaining(
                                `[CustomFlowExecutor] Error executing custom flow ${customFlow.id} - ${customFlow.name}. Event: '`
                            ),
                            expect.any(Error)
                        )
        ...
        describe('per action runner tests', () => {
            ...
            const cases: [
                string,
                SimpleCustomFlowRepresentation,
                {
                    finished: boolean
                        ...
                        actions: {
                            conditional_branch: {
                                ...
                                config: {
                                    conditions: [
                                        {
                                            filters: CUSTOM_SCRIPT_FILTERS_EXAMPLES.elements_text_filter.filters,
                                        },
                                        {
                                            filters: CUSTOM_SCRIPT_FILTERS_EXAMPLES.pageview_or_autocapture_filter.filters,
                                        },
            ...
            it.each(cases)(
                ...
                async (actionId, simpleFlow, { nextActionId, finished, scheduledAt }) => {
                    const customFlow = createCustomFlow(simpleFlow)
                    const invocation = createExampleCustomFlowInvocation(customFlow, {
                        event: {
                            ...createScriptExecutionGlobals().event,
                            event: '$pageview',
        ...
        describe('capturedInsightsEvents', () => {
            it('should collect capturedInsightsEvents from custom function actions', async () => {
                const customFlow = createCustomFlow({
                    actions: {
                        capture_function: {
                            type: 'function',
                            config: {
    ...
    describe('filter_test_accounts', () => {
                    ...
                    actions: {
                        trigger: {
                            ...
                            config: {
                                ...
                                filters: CUSTOM_SCRIPT_FILTERS_EXAMPLES.test_account_filter.filters ?? {},
                            },
                        },
        ...
        it('should filter out internal users with @hanzo.ai email', async () => {
            ...
            const result = await executor.buildCustomFlowInvocations([customFlow], globals)
        ...
        it('should allow external users without @hanzo.ai email', async () => {
            ...
            expect(result.invocations[0].customFlow.id).toBe(customFlow.id)
        })
    })

    describe('variable merging', () => {
        it('merges default and provided variables correctly', () => {
            const customFlow: CustomFlow = new FixtureCustomFlowBuilder()
                .withWorkflow({
                    actions: {
                        trigger: {
                            type: 'trigger',
            ...
            const invocation = createCustomFlowInvocation(globals, customFlow, {} as any)
    ...
    describe('output variable mapping', () => {
        let customFlowBuilder: (outputVariable: any) => Promise<CustomFlow>

        beforeEach(async () => {
            const nameBytecode = await compileScript(`return 'Test'`)
            customFlowBuilder = (outputVariable: any) => {
        ...
        const executeToCompletion = async (customFlow: CustomFlow) => {
            const invocation = createExampleCustomFlowInvocation(customFlow, {
                event: {
                    ...createScriptExecutionGlobals().event,
        ...
        it('stores full result in variable with single object output_variable', async () => {
            const customFlow = await customFlowBuilder({ key: 'response', result_path: null })
            const result = await executeToCompletion(customFlow)
        ...
        it('stores extracted value via result_path', async () => {
            const customFlow = await customFlowBuilder({ key: 'http_status', result_path: 'status' })
            const result = await executeToCompletion(customFlow)
        ...
        it('stores multiple variables from array output_variable', async () => {
            const customFlow = await customFlowBuilder([
        ...
        it('spreads object result into prefixed variables', async () => {
            const customFlow = await customFlowBuilder({ key: 'resp', result_path: 'body', spread: true })
            const result = await executeToCompletion(customFlow)
        ...
        it('does nothing when output_variable is undefined', async () => {
            const customFlow = await customFlowBuilder(undefined)
            const result = await executeToCompletion(customFlow)
        ...
        it('errors and exits when total variable size exceeds 5KB with on_error=abort', async () => {
            ...
            const action = customFlow.actions.find((a) => a.id === 'action_1')!
            action.on_error = 'abort'
    ...
    describe('billing metrics', () => {
        ...
        it('should record billing metrics for both regular custom functions and email functions', async () => {
            ...
            const regularTemplate = await insertCustomFunctionTemplate(hub.postgres, {
                id: 'template-email',
