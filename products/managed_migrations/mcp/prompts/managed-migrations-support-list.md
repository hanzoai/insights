List batch import (managed migration) jobs across ALL Insights teams, for troubleshooting customer migrations. Insights staff only: the backend requires a staff user AND a personal API key explicitly carrying the `batch_import_support:read` scope. A full-access (`*`) key is rejected; OAuth sign-in can never grant this scope.
...
- `status_message` is the developer-facing debugging signal; `display_status_message` is what the customer sees in the Insights UI.
