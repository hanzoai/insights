...
class TestSeatAPIAdminPermissions(BaseSeatAPITest):
    ...
    def test_list_requires_admin(self, mock_request, _mock_license, _mock_token):
        ...
        response = self.client.get("/api/seats/?product_key=insights_code")
    ...
    def test_retrieve_other_user_requires_admin(self, mock_request, _mock_license, _mock_token):
        self._auth_as_member()
        response = self.client.get(f"/api/seats/{self.admin_user.distinct_id}/?product_key=insights_code")
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_retrieve_me_allowed_for_member(self, mock_request, _mock_license, _mock_token):
        ...
        self._auth_as_member()
        response = self.client.get("/api/seats/me/?product_key=insights_code")
        assert response.status_code == status.HTTP_200_OK

    @patch("products.tasks.backend.presentation.views.seat_api.requests.request")
    def test_delete_other_user_requires_admin(self, mock_request, _mock_license, _mock_token):
        self._auth_as_member()
        response = self.client.delete(f"/api/seats/{self.admin_user.distinct_id}/?product_key=insights_code")
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_patch_other_user_requires_admin(self, mock_request, _mock_license, _mock_token):
        ...
        response = self.client.patch(
            ...
            {"product_key": "insights_code", "plan_key": "insights-code-200-20260301"},
            format="json",
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_create_without_user_distinct_id_returns_400(self, mock_request, _mock_license, _mock_token):
        ...
        response = self.client.post(
            "/api/seats/",
            {"product_key": "insights_code", "plan_key": "insights-code-free-20260301"},
            format="json",
        )
    ...
    def test_create_for_other_user_requires_admin(self, mock_request, _mock_license, _mock_token):
        ...
        response = self.client.post(
            "/api/seats/",
            {"product_key": "insights_code", "plan_key": "insights-code-free-20260301", "user_distinct_id": "other-id"},
            format="json",
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_retrieve_non_org_member_rejected(self, mock_request, _mock_license, _mock_token):
        self._auth_as_admin()
        response = self.client.get(f"/api/seats/{self.external_user.distinct_id}/?product_key=insights_code")
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_delete_non_org_member_rejected(self, mock_request, _mock_license, _mock_token):
        self._auth_as_admin()
        response = self.client.delete(f"/api/seats/{self.external_user.distinct_id}/?product_key=insights_code")
        assert response.status_code == status.HTTP_403_FORBIDDEN
...
class TestSeatAPIMeResolution(BaseSeatAPITest):
    ...
    def test_retrieve_me_resolves_distinct_id(self, mock_request, _mock_license, _mock_token):
        mock_request.return_value = _billing_response({"seat": MOCK_SEAT})
        self._auth_as_member()
        self.client.get("/api/seats/me/?product_key=insights_code")
    ...
    def test_delete_me_resolves_distinct_id(self, mock_request, _mock_license, _mock_token):
        ...
        self._auth_as_member()
        self.client.delete("/api/seats/me/?product_key=insights_code")
        _, kwargs = mock_request.call_args
        assert str(self.user.distinct_id) in kwargs["url"]
...
class TestSeatAPIRetiredProducts(BaseSeatAPITest):
    def _assert_retired(self, response: _JsonTestResponse) -> None:
        ...
        assert response.json() == {
            "error": (
                "You can no longer create, upgrade, or reactivate Insights Desktop seats. "
                "Insights Desktop with usage-based billing launched on July 16, 2026, "
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                "PATCH",
                "/api/seats/me/",
                {"product_key": "insights_code", "plan_key": "insights-code-pro-200-20260301"},
            ),
            (
                "reactivate",
                "POST",
    ...
    @patch("products.tasks.backend.presentation.views.seat_api.requests.request")
    def test_insights_code_mutation_is_rejected_locally(
        self,
        _name: str,
        method: str,
        path: str,
    ...
    @parameterized.expand(
        [
            (
                "create",
                "POST",
                "/api/seats/?product_key=insights_code",
                {
                    "plan_key": "insights-code-free-20260301",
                    "user_distinct_id": "me",
                },
            ...
            (
                "upgrade",
                "PATCH",
                "/api/seats/me/?product_key=insights_code",
                {"plan_key": "insights-code-pro-200-20260301"},
            ),
            (
                "reactivate",
                "POST",
                "/api/seats/me/reactivate/?product_key=insights_code",
                {},
            ),
        ]
    )
    @patch("products.tasks.backend.presentation.views.seat_api.requests.request")
    def test_insights_code_query_param_mutation_is_rejected_locally(
        self,
        _name: str,
        method: str,
        path: str,
...
class TestSeatAPIResponseUnwrapping(BaseSeatAPITest):
    ...
    def test_204_returns_no_content(self, mock_request, _mock_license, _mock_token):
        ...
        mock_request.return_value = billing_resp
        self._auth_as_member()
        response = self.client.delete("/api/seats/me/?product_key=insights_code")
...
MOCK_PRO_V2_SEAT = {
    ...
    "user_distinct_id": "user-abc",
    "product_key": "insights_code",
    "plan_key": "insights-code-pro-200-20260422",
    "status": "active",
    "end_reason": None,
    "created_at": "2026-04-01T00:00:00Z",
    "active_until": None,
...
class TestSeatAPIBestPlan(BaseSeatAPITest):
    ...
    def test_single_org_returns_that_seat(self, _mock_token, mock_request, _mock_license):
        ...
        self._auth_as_admin()
        response = self.client.get("/api/seats/me/?product_key=insights_code&best=true")
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["plan_key"] == "insights-code-free-20260301"
    ...
    def test_multi_org_returns_pro_over_free(self, _mock_token, mock_request, _mock_license):
        ...
        response = self.client.get("/api/seats/me/?product_key=insights_code&best=true")
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["plan_key"] == "insights-code-200-20260301"
    ...
    def test_best_ignored_for_non_me_pk(self, _mock_token, mock_request, _mock_license):
        mock_request.return_value = _billing_response({"seat": MOCK_SEAT})
        self._auth_as_admin()
        response = self.client.get(f"/api/seats/{self.user.distinct_id}/?product_key=insights_code&best=true")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_pro_v2_prefix_recognized(self, _mock_token, mock_request, _mock_license):
        ...
        response = self.client.get("/api/seats/me/?product_key=insights_code&best=true")
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["plan_key"] == "insights-code-pro-200-20260422"
