...
from insights.models.activity_logging.activity_log import Change, changes_between, load_activity
...
# Python cells read upstream results through the data plane, which only reaches Insights.
_CROSS_ENGINE_REF_ERROR_FOR_PYTHON = (
    "'{name}' ran on a warehouse connection, and Python cells can only read Insights results. "
    "Re-run '{name}' on Insights to use it here."
)
_LOCAL_FRAME_REF_ERROR = (
    "'{name}' is a Python dataframe, so only a cell running on Insights can read it. "
    "Switch this cell to Insights to use it."
)
...
def classify_request_source(request: Request) -> tuple[str, dict[str, str | None]]:
    ...
    programmatic client. ``mcp_consumer`` is the wrapping app the Insights MCP server forwards
    (``insights-code``/``insights-cli`` for first-party Insights Desktop), which tells it apart from a
...
class NotebookSerializer(NotebookMinimalSerializer):
    ...
    def get_parent_resource(self, obj: Notebook) -> dict | None:
        # Group parents are skipped: ResourceNotebook stores group PK but personinsights has no get-by-pk RPC.
...
def _collab_user_name(user: User) -> str:
    return user.get_full_name() or "Wandering Script"
...
class NotebookViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def _require_query_access(self) -> None:
        # SQLV2 runs arbitrary InsightsQL and returns analytics rows, so notebook access alone is not
    ...
    def sql_v2_run(self, request: Request, **kwargs):
        ...
        # Only SQL nodes carry a connection; python always runs in the kernel against Insights.
        ...
        # Resolve each referenced insightsql node to its last-run query (not its live editor text),
        ...
        insightsql_node_ids = {spec["node_id"] for spec in ref_specs.values() if spec["kind"] == "insightsql"}
        ...
        latest_runs = (
            ...
            .filter(notebook=notebook, node_id__in=insightsql_node_ids, status=NotebookNodeRun.Status.DONE)
        ...
        # one will. A SQL node's runs can alternate between insightsql and duckdb (Journey 5
        ...
        for other_node_id, run_id, run_code, run_type, run_connection_id, run_send_raw in latest_runs:
            if run_type != NotebookNodeRun.NodeType.INSIGHTSQL:
                continue
        ...
        for name, spec in ref_specs.items():
            if spec["kind"] == "local":
                # A kernel frame lives in the sandbox, which only reaches Insights's own data — a
                # connection run can't be rerouted there, so mark it unusable instead.
                refs[name] = (
                    SQLV2Ref(kind="insightsql", node_id=None, unavailable_reason=_LOCAL_FRAME_REF_ERROR.format(name=name))
        ...
        try:
            if node_type == "python":
                ...
                # Raw SQL is the connection's own dialect, so the InsightsQL parser can't read it and
                # there is nothing to inline: it reaches the engine exactly as written.
                run_code, inputs = code, []
            else:
                # A SQL node pushes to Datastore — unless it references a local frame, which
                # reroutes it to the sandbox's DuckDB (Journey 5).
                node_type, run_code, inputs = resolve_sql_node_run(code, refs)
        # ExposedInsightsQLError: with refs present the user's own code is parsed at dispatch, so a
        # plain typo raises here — it's a bad query (400 with the parse message), not a 500.
        except (SQLV2ReferenceError, ExposedInsightsQLError) as e:
        ...
        try:
            if node_type == NotebookNodeRun.NodeType.INSIGHTSQL:
                # Direct lane: a pure-InsightsQL run never touches the sandbox — it rides the
    ...
    def sql_v2_run_result(self, request: Request, run_id: str | None = None, **kwargs):
        ...
        # Direct (insightsql) runs have no callback: this poll advances the row from the async
    ...
    def sql_v2_run_page(self, request: Request, run_id: str | None = None, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        notebook = self._get_notebook_for_kernel()
        # Paging re-queries Datastore and returns analytics rows, so gate it on query access
        ...
        if run.node_type == NotebookNodeRun.NodeType.INSIGHTSQL:
    ...
    def sql_v2_run_interrupt(self, request: Request, run_id: str | None = None, **kwargs):
        ...
        if run.node_type == NotebookNodeRun.NodeType.INSIGHTSQL:
            # A direct (insightsql) run has no kernel to signal, so stop it at the query manager
