...
class TestOAuthModels(TestCase):
    ...
    valid_custom_scheme_uris = [
        ("simple custom scheme", "insights-code://callback"),
        ("custom scheme with path", "myapp://oauth/callback"),
        ("reverse domain style", "com.insights.code://oauth"),
    ...
    def test_mixed_custom_scheme_and_https_redirect_uris(self):
        app = OAuthApplication.objects.create(
            ...
            client_type=OAuthApplication.CLIENT_CONFIDENTIAL,
            authorization_grant_type=OAuthApplication.GRANT_AUTHORIZATION_CODE,
            redirect_uris="https://example.com/callback insights-code://oauth http://localhost:3000/callback",
            organization=self.organization,
            algorithm="RS256",
        )
        self.assertIn("insights-code://", app.redirect_uris)
    ...
    def test_get_allowed_schemes_extracts_schemes_from_redirect_uris(self):
        ...
        self.assertIn("insights-code", schemes)
