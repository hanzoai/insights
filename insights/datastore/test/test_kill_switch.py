...
from insights.datastore.client.execute import (
...
class TestGetKillSwitchLevel:
    ...
    def test_returns_normalized_level(self, _name: str, raw_value: object, expected: KillSwitchLevel):
        ...
        with patch("insights.models.instance_setting.get_instance_setting", return_value=raw_value):
    ...
    def test_caches_across_same_ttl(self):
        ...
        with patch("insights.models.instance_setting.get_instance_setting", mock):
...
class TestGetTeamKillSwitchLevel:
    ...
    def test_per_team_lookup(
        ...
        settings = {
            "DATASTORE_KILL_SWITCH_FULL_TEAMS": full,
            "DATASTORE_KILL_SWITCH_LIGHT_TEAMS": light,
        }
        with patch("insights.models.instance_setting.get_instance_setting", side_effect=lambda name: settings[name]):
    ...
    def test_global_get_kill_switch_level_ignores_per_team_lists(self):
        ...
        _get_kill_switch_team_sets.cache_clear()
        settings = {
            "DATASTORE_KILL_SWITCH": "off",
            "DATASTORE_KILL_SWITCH_FULL_TEAMS": [42],
    ...
    def test_team_sets_are_cached(self):
        ...
        settings = {
            "DATASTORE_KILL_SWITCH_FULL_TEAMS": [42],
            "DATASTORE_KILL_SWITCH_LIGHT_TEAMS": [7],
        }
...
class TestResolveKillSwitchLevel:
    ...
    def test_precedence(
        ...
        with (
            patch("insights.datastore.client.execute.get_kill_switch_level", return_value=global_level),
            patch("insights.datastore.client.execute.get_team_kill_switch_level", return_value=team_level),
        ):
    ...
    def test_none_team_id_returns_global_only(self):
        ...
        with (
            patch("insights.datastore.client.execute.get_kill_switch_level", return_value=KillSwitchLevel.LIGHT),
            patch("insights.datastore.client.execute.get_team_kill_switch_level", team_mock),
        ):
...
class TestKillSwitchCacheWarming:
    ...
    @patch("insights.caching.warming.logger")
    def test_cache_warming_skipped(self, _name: str, level: KillSwitchLevel, mock_logger: MagicMock):
        with patch("insights.datastore.client.execute.get_kill_switch_level", return_value=level):
    ...
    @patch("insights.datastore.client.execute.get_kill_switch_level", return_value=KillSwitchLevel.OFF)
    @patch("insights.caching.warming.largest_teams", return_value=[])
