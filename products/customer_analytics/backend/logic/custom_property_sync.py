...
Reads a configured view via InsightsQL, matches each row to an account by external_id, and writes the
...
def _read_view(team: Team, view_name: str, columns: list[str], key_column: str, external_ids: list[str]) -> list:
    ...
    keep each query small. An unfiltered read would be silently capped by the InsightsQL default limit —
    ...
    with tags_context(product=Product.CUSTOMER_ANALYTICS, feature=Feature.ACCOUNTS, team_id=team.pk):
        for start in range(0, len(external_ids), _SYNC_KEYS_PER_QUERY):
            ...
            batch_rows = execute_insightsql_query(query, team=team, bypass_warehouse_access_control=True).results or []
