...
const TEAM_COLUMNS: &str = "
    ...
    recording_domains,
    COALESCE(
        (SELECT minimal_flag_called_events FROM feature_flags_teamfeatureflagsconfig WHERE team_id = insights_team.id),
        false
...
impl Team {
    ...
    pub async fn from_pg_by_id(client: PostgresReader, team_id: i32) -> Result<Team, FlagError> {
        ...
        let query = format!("SELECT {TEAM_COLUMNS} FROM insights_team WHERE id = $1");
        let row = sqlx::query_as::<_, Team>(&query)
            .bind(team_id)
            .fetch_one(&mut *conn)
            .await?;
