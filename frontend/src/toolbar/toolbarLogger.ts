 ...
 * log records to Insights's /i/v1/logs endpoint (OTLP format).
 *
 * Usage:
...
function sendLog(level: LogLevel, context: string, message: string, properties?: Record<string, unknown>): void {
    try {
        if (!toolbarInsightsJS?.has_opted_in_capturing?.()) {
            return
        }

        const config = toolbarInsightsJS?.config
        if (!config) {
            return
        }
        ...
        const distinctId = toolbarInsightsJS.get_distinct_id?.()
            ...
            resourceLogs: [
                {
                    resource: {
                        attributes: toAttributes({
                            'service.name': 'insights-toolbar',
                            host: window.location.host,
                            ...(distinctId ? { 'insights.distinct_id': distinctId } : {}),
                        }),
                    },
                    scopeLogs: [
                        {
        ...
        // Never throw on customer pages — matches the contract of toolbarInsightsJS.capture()
    }
}

function log(level: LogLevel, context: string, message: string, properties?: Record<string, unknown>): void {
    const prefix = `[Insights Toolbar][${context}]`
