...
class TestFunnelsGeneratorNode(BaseTest):
    ...
    def test_schema_does_not_require_aggregation_by_insightsql(self):
        """Catches the regression where the schema set funnelAggregateByInsightsQL."""
        schema = AssistantFunnelsQuery(series=[], funnelsFilter=AssistantFunnelsFilter())
        assert schema.funnelsFilter is not None
        self.assertIsNone(schema.funnelsFilter.funnelAggregateByInsightsQL)
