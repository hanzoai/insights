...
// roughly twice that often so a slow Datastore page doesn't lose the lock.
 ...
 *   1. Parses the state.
 *   2. Runs one page of Datastore + rehydrate + enqueue work via `RerunPaginatorService`.
...
export class CdpRerunWorkerConsumer extends CdpConsumerBase<PluginsServerConfig> {
    ...
    private datastoreClient: DatastoreClient | null = null
    ...
    constructor(config: PluginsServerConfig, deps: CdpConsumerBaseDeps, jobQueues: RerunJobQueues) {
        ...
        // pages — script functions to kafka, script flows to postgres-v2.
    ...
    override async start(): Promise<void> {
        ...
        const chScheme = this.config.DATASTORE_SECURE ? 'https' : 'http'
        const chPort = this.config.DATASTORE_SECURE ? 8443 : 8123
        this.datastoreClient = createDatastoreClient({
            url: `${chScheme}://${this.config.DATASTORE_HOST}:${chPort}`,
            username: this.config.DATASTORE_USER,
            password: this.config.DATASTORE_PASSWORD || undefined,
            database: this.config.DATASTORE_DATABASE,
            request_timeout: 60_000,
            max_open_connections: 10,
            ...(this.config.DATASTORE_SECURE
                ? {
        ...
        this.worker = new CyclotronV2Worker({
            ...
            // Rerun jobs are heavy (each runs a full Datastore query) — the
            // default of 1 pulls them one at a time so a single replica can be
            // deployed at high concurrency without overwhelming Datastore.
    ...
    private async handleJob(job: CyclotronV2DequeuedJob): Promise<void> {
        ...
        // Heartbeat the lock while the page is running. Datastore queries
