...
class TestSuggestConversionGoalsDatastoreTopkAndCount(DatastoreTestMixin, BaseTest):
    ...
    @pytest.mark.asyncio
    async def test_topk_insightsql_runs_and_custom_events_appear_as_candidates(self) -> None:
...
class TestSuggestConversionGoalsDatastoreSorting(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestSuggestConversionGoalsDatastoreFiltering(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
...
class TestSuggestConversionGoalsDatastoreCountIf(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
    ...
    @pytest.mark.asyncio
    async def test_countif_insightsql_computes_utm_coverage_correctly(self) -> None:
        response = await suggest_conversion_goals(self.team, min_count=50, lookback_days=30)
...
class TestSuggestConversionGoalsDatastoreTopN(DatastoreTestMixin, BaseTest):
    CLASS_DATA_LEVEL_SETUP = False
