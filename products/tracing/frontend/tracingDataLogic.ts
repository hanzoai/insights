...
function captureTracingResults(count: number, queryType: 'spans' | 'aggregation'): void {
    if (count === 0) {
        insights.capture('tracing no results returned', { query_type: queryType })
    } else {
        insights.capture('tracing results returned', { count, query_type: queryType })
    }
}
...
export interface tracingDataLogicActions {
    setFeatureFlags: (flags: string[], variants: Record<string, boolean | string>, payloads?: import("lib/logic/featureFlagLogic").FeatureFlagPayloads | undefined) => {
        flags: string[];
        payloads: import("lib/logic/featureFlagLogic").FeatureFlagPayloads;
    ...
    setSort: (orderBy: TracingOrderBy, orderDirection: TracingOrderDirection) => {
    ...
    clearSpansFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    clearSpansSuccess: (spans: never[], payload?: {
        value: true;
    }) => {
        spans: never[];
        payload?: {
    ...
    fetchAggregationFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    ...
    fetchDurationHistogramFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    fetchDurationHistogramSuccess: (rawDurationHistogram: {
        bucket_ns: number;
        count: number;
        service: string;
    }[], payload?: any) => {
    ...
    fetchLatencyHeatmapFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    fetchLatencyHeatmapSuccess: (rawLatencyHeatmap: {
        bucket_ns: number;
        count: number;
        time: string;
    }[], payload?: any) => {
    ...
    fetchMatchingCountsFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    fetchMatchingCountsSuccess: (matchingCounts: {
        count: number;
        traceCount: number;
    }, payload?: any) => {
        matchingCounts: {
    ...
    fetchNextPageFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    fetchNextPageSuccess: (spans: Span[], payload?: {
        value: true;
    }) => {
        spans: Span[];
        payload?: {
    ...
    fetchSpanTreeFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    fetchSpanTreeSuccess: (spanTree: {
        current: SpanTreeNode[];
        previous: SpanTreeNode[] | null;
        spanName: string;
    }, payload?: {
    ...
    fetchSpans: () => any;
    fetchSpansFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    fetchSpansSuccess: (spans: Span[], payload?: any) => {
        spans: Span[];
        payload?: any;
    };
    fetchSparkline: () => any;
    ...
    fetchSparklineSuccess: (rawSparklineData: {
        count: number;
        service: string;
        time: string;
    }[], payload?: any) => {
    ...
    handleFilterChange: (filterType: string, extraProps?: Record<string, unknown>) => {
    ...
    loadMoreTraceSpansFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    loadMoreTraceSpansSuccess: (traceSpans: Span[], payload?: {
        value: true;
    }) => {
        traceSpans: Span[];
        payload?: {
    ...
    loadTraceSpansFailure: (error: string, errorObject?: any) => {
        error: string;
        errorObject?: any;
    };
    loadTraceSpansSuccess: (traceSpans: Span[], payload?: {
        traceId: string;
        ts?: string | null;
    }) => {
        traceSpans: Span[];
    ...
    setTracePagination: (hasMore: boolean, nextOffset: number | null) => {
        hasMore: boolean;
...
export interface tracingDataLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        ...
        totalMatchingFilters: (matchingCounts: {
            count: number;
            traceCount: number;
        }, filters: TracingFilters) => number;
        ...
        visibleRowDurationRange: (visibleRowRange: VisibleRowRange | null, listRowDurations: number[], isDurationMode: boolean) => VisibleDurationRange | null;
        visibleRowDateRange: (visibleRowRange: VisibleRowRange | null, listRows: Span[], orderBy: TracingOrderBy) => VisibleSpanTimeRange | null;
    };
}

export type tracingDataLogicType = MakeLogicType<tracingDataLogicValues, tracingDataLogicActions, TracingDataLogicProps, tracingDataLogicMeta>;
...
export const tracingDataLogic = kea<tracingDataLogicType>([
    ...
    listeners(({ actions, values }) => ({
        handleFilterChange: ({ filterType, extraProps }) => {
            insights.capture('tracing filter changed', { filter_type: filterType, ...extraProps })
            actions.runQuery()
        },
        ...
        applyHeatmapBrush: ({ selection }) => {
            ...
            insights.capture('tracing filter changed', {
        ...
        // Feature flags land asynchronously (insights.js /flags), potentially after the first
        ...
        fetchSpansFailure: ({ error }) => {
            if (!isUserInitiatedError(error)) {
                toast.error(`Failed to load traces: ${error}`)
                insights.capture('tracing query failed', { query_type: 'spans', error_message: String(error) })
            }
        },
        fetchMatchingCountsFailure: ({ error }) => {
            if (!isUserInitiatedError(error)) {
                toast.error(`Failed to load the matching count: ${error}`)
            }
        },
        fetchAggregationFailure: ({ error }) => {
            if (!isUserInitiatedError(error)) {
                toast.error(`Failed to load span aggregation: ${error}`)
            }
        },
        fetchSpanTreeFailure: ({ error }) => {
            if (!isUserInitiatedError(error)) {
                toast.error(`Failed to load call tree: ${error}`)
            }
        },
        loadMoreTraceSpansFailure: ({ error }) => {
            if (!isUserInitiatedError(error)) {
                toast.error(`Failed to load more spans: ${error}`)
            }
        },
        ...
        fetchLatencyHeatmapFailure: ({ error }) => {
            if (!isUserInitiatedError(error)) {
                toast.error(`Failed to load the latency heatmap: ${error}`)
            }
        },
    })),
