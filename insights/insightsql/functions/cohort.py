...
from insights.schema_enums import InlineCohortCalculation

INLINE_COHORT_THRESHOLD_SECONDS = 10


def _is_inline_flag_enabled(context: InsightsQLContext) -> bool:
...
def inline_cohort_query(
    ...
    return InsightsQLCohortQuery(cohort=cohort, team=context.team).get_query()
...
def cohort_query_node(node: ast.Expr, context: InsightsQLContext) -> ast.Expr:
    return cohort(node, [node], context)


def cohort(node: ast.Expr, args: list[ast.Expr], context: InsightsQLContext) -> ast.Expr:
    from products.cohorts.backend.models.cohort import Cohort
