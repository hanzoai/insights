...
from insights.datastore.query_tagging import Product, tag_queries, tags_context
...
from products.experiments.backend.insightsql_queries import MULTIPLE_VARIANT_KEY, get_baseline_variant_key
...
from products.experiments.backend.insightsql_queries.cuped_config import get_cuped_config
from products.experiments.backend.insightsql_queries.error_handling import experiment_error_handler
...
from products.experiments.backend.insightsql_queries.experiment_query_context import ExperimentPrecomputationContext
...
class ExperimentQueryRunner(QueryRunner):
    ...
    def _metric_events_precompute_applicable(self) -> bool:
        ...
        if isinstance(self.metric, ExperimentMeanMetric):
            ...
            # and InsightsQL expressions don't fit the Float64 numeric_value column.
    ...
    def _evaluate_experiment_query(
        ...
        self.insightsql = experiment_query_debug[0]
        self.datastore_sql = experiment_query_debug[1]

        modifiers = create_default_modifiers_for_team(self.team)
        if hanzo_insights.feature_enabled(
            "insightsql-session-id-pushdown",
            str(self.team.id),
    ...
    def to_actors_query(self) -> ast.SelectQuery:
        ...
        if funnel_step == -1:
            ...
            from insights.schema import ActionsNode, EventsNode
        ...
        from insights.schema import ActionsNode, ExperimentEventExposureConfig
        ...
        from products.experiments.backend.insightsql_queries.experiment_funnel_actors_query_builder import (
