...
// Technical limitation: Script functions can only call insightsCapture once per invocation.
...
if (droppedCount > 0) {
    print(f'Warning: Dropped {droppedCount} additional log(s). Script functions can only emit one event per invocation.')
}
...
// the visual format of Insights cookieless distinct IDs (132 bits of entropy —
...
let props := {
    ...
    // Insights standard properties. $ip and $raw_user_agent are appended
    // below when forward_ip_and_user_agent is enabled (default on, since
    // Insights's GeoIP and UA enrichment depend on them); flip the toggle
...
`,
    ...
    inputs_schema: [
        ...
        },
        {
            ...
            description:
                'When enabled (default), $ip, $raw_user_agent, proxy_client_ip, and proxy_user_agent are emitted on each event so Insights can run GeoIP and user-agent enrichment. Disable if you want raw client identifiers stripped — distinct_id derivation still uses them as inputs, but they will not appear on the emitted event.',
            secret: false,
            required: false,
            default: true,
        },
        {
            ...
            description:
                'Used only when strategy is "custom". Supports placeholders {day}, {ip}, {host}, {ua}, {path}, {project_id} (literal string substitution, not Script evaluation; unknown placeholders are left as-is). The salt secret is intentionally not exposed as a placeholder. The result is prefixed with "http_log_". Empty value or all-empty substitutions fall back to rotating_salt.',
            secret: false,
            required: false,
            // Templating disabled so {placeholder} braces are not interpreted as Script
            // expressions at the input layer. Substitution happens inside the Script code.
            templating: false,
        },
    ],
}
