...
class _GitServer:
    def __init__(self):
        ...
        def repo():
            return git.synthesize_repo(_marketplace_tree(self._epoch), author="Insights", message="marketplace")
...
def test_full_clone_yields_spec_marketplace_tree(git_server):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        skill_md = os.path.join(dest, "plugins", "insights-skill-store", "skills", "make-fractals", "SKILL.md")
        script = os.path.join(
            dest, "plugins", "insights-skill-store", "skills", "make-fractals", "scripts", "mandelbrot.py"
        )
