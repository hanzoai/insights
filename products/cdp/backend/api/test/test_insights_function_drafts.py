...
class DraftTestCase(APIBaseTest):
    ...
    def _agent_patch(self, function_id: str, payload: dict):
        return self.client.patch(self._url(function_id), payload, headers={"x-insights-client": "mcp"})
    ...
    def _revisions(self, function_id: str):
        return InsightsFunctionRevision.objects.for_team(self.team.id).filter(insights_function_id=function_id)


class TestInsightsFunctionDrafts(DraftTestCase):
    ...
    @parameterized.expand(
        [
            ...
            ("transformation", True, {"type": "transformation", "script": "return event"}, True),
    ...
    def test_enabling_with_a_draft_open_is_refused(self):
        ...
        assert InsightsFunction.objects.get(id=function_id).enabled is False
    ...
    def test_draft_never_carries_secret_values(self):
        ...
        stored_draft = InsightsFunction.objects.get(id=function_id).draft
    ...
    def test_current_base_updated_at_is_accepted(self):
        ...
        response = self._agent_patch(
            function_id,
            {"script": "fetch(inputs.url, {'method': 'PATCH'});", "base_updated_at": staged["draft_updated_at"]},
        )
    ...
    def test_create_as_enabled(self, _name: str, from_agent: bool, function_type: str, expected: int):
        ...
        headers = {"x-insights-client": "mcp"} if from_agent else {}
...
class TestInsightsFunctionRevisions(DraftTestCase):
    ...
    def test_no_revision_is_written(self, _name: str, payload: dict, flag_on: bool):
        ...
        assert InsightsFunction.objects.get(id=function_id).version == 1
    ...
    def test_recompiled_filter_bytecode_does_not_create_a_revision(self):
        ...
        # changing, which is what refresh_affected_insights_functions does after an action or cohort edit.
        # queryset.update() so no signal or serializer runs.
        InsightsFunction.objects.filter(id=function_id).update(filters={"source": "events", "bytecode": ["_H", 1, 999]})
