import { PostScript as Insights } from 'insights-node'
...
if (process.env.NODE_ENV === 'test' && insights) {
    void insights.disable()
}
...
export function initSuperProperties(eventsIngestionPipeline: string | null = null): void {
    if (insights) {
        const superProperties: Record<string, any> = {
            ...
            // Super properties matching Python hanzo_insights.super_properties (insights/apps.py)
        ...
        void insights.register(superProperties)
    }
}
...
export async function isFeatureFlagEnabled(
    ...
    try {
        const isEnabled = await insights.isFeatureEnabled(key, distinctId, options)
...
export function shutdown(): Promise<void> | null {
    return insights ? insights.shutdown() : null
}

export function flush(): void {
    if (insights) {
        void insights.flush().catch(() => null)
    }
}
...
export function captureException(exception: any, hint?: Partial<ExceptionHint>): void {
    if (insights) {
        ...
        insights.captureException(exception, undefined, additionalProperties)
    }
}
