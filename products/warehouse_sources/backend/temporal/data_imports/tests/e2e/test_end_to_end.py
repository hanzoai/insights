...
from insights.temporal.ducklake.ducklake_register_data_imports_workflow import DuckLakeRegisterDataImportsWorkflow
...
from products.warehouse_sources.backend.temporal.data_imports.sources.common.rest_source.rest_client import (
    RESTClient as InsightsRESTClient,
)
...
def mock_paddle_client():
    ...
    # `_execute_run`'s `InsightsRESTClient.paginate` patch. This fixture only bypasses
...
async def _run(
    ...
    if not ignore_assertions:
        ...
        assert status_rows[0]["timestamp"] == format_datastore_timestamp(run.finished_at)
...
async def _execute_run(workflow_id: str, inputs: ExternalDataWorkflowInputs, mock_data_response):
    ...
    with (
        ...
        mock.patch.object(InsightsRESTClient, "paginate", mock_paginate_pages),
...
async def test_in_place_repartition_to_finer_datetime_format(team, postgres_config, postgres_connection, minio_client):
    ...
    count_before = await sync_to_async(execute_insightsql_query)("SELECT count() FROM postgres_test_repartition", team)
    ...
    count_after = await sync_to_async(execute_insightsql_query)("SELECT count() FROM postgres_test_repartition", team)
...
async def test_v3_delta_commit_metadata_and_idempotency_fallback(team, stripe_customer, mock_stripe_client):
    ...
    res = await sync_to_async(execute_insightsql_query)(
...
async def test_stripe_webhook_s3_charges(team, stripe_charge, mock_stripe_client, minio_client):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT * FROM stripe_charge", team)
    assert len(res.results) == 1

    # Create a webhook InsightsFunction linked to this schema via inputs
    schema = await sync_to_async(ExternalDataSchema.objects.get)(id=inputs.external_data_schema_id)
...
async def test_stripe_webhook_consumer_e2e(team, stripe_charge, mock_stripe_client, minio_client):
    ...
    # 2. Create a webhook InsightsFunction linked to this schema
    schema = await sync_to_async(ExternalDataSchema.objects.get)(id=inputs.external_data_schema_id)
    schema_id_str = str(schema.id)
    await sync_to_async(InsightsFunction.objects.create)(
        team=team,
...
async def test_mysql_full_refresh(team, mysql_config, mysql_connection):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT id, email, age FROM mysql_users_full_refresh", team)
...
async def test_mysql_incremental_integer_cursor(team, mysql_config, mysql_connection):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT id FROM mysql_events_int_incremental ORDER BY id", team)
...
async def test_mysql_schema_evolution(team, mysql_config, mysql_connection):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT * FROM mysql_orders_evolution", team)
...
async def test_postgres_xmin_sync(team, postgres_config, postgres_connection):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT id, name FROM postgres_xmin_table ORDER BY id", team)
...
async def test_postgres_xmin_wraparound_or_range(team, postgres_config, postgres_connection):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT id, name FROM postgres_xmin_wrap ORDER BY id", team)
    assert [(r[0], r[1]) for r in res.results] == [(1, "a")]
...
async def test_postgres_switch_to_xmin_rebuilds_table(team, postgres_config, postgres_connection):
    ...
    res = await sync_to_async(execute_insightsql_query)("SELECT id, name FROM postgres_switch_tbl ORDER BY id", team)
    assert [(r[0], r[1]) for r in res.results] == [(1, "a")]
