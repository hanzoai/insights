...
from posthog.models.custom_functions.custom_function import (
...
class CustomFunctionStatusSerializer(serializers.Serializer):
    state = serializers.ChoiceField(choices=[state.value for state in CustomFunctionState])
    tokens: serializers.IntegerField = serializers.IntegerField()


class CustomFunctionMinimalSerializer(serializers.ModelSerializer):
    created_by = UserBasicSerializer(read_only=True)
    status = CustomFunctionStatusSerializer(read_only=True, required=False, allow_null=True)
    template = CustomFunctionTemplateSerializer(read_only=True)
...
class CustomFunctionMaskingSerializer(serializers.Serializer):
...
class CustomFunctionSerializer(CustomFunctionMinimalSerializer):
    template = CustomFunctionTemplateSerializer(read_only=True)
    masking = CustomFunctionMaskingSerializer(required=False, allow_null=True)
    type = serializers.ChoiceField(choices=CustomFunctionType.choices, required=False, allow_null=True)
    ...
    def to_internal_value(self, data):
        ...
        instance = cast(Optional[CustomFunction], self.context.get("instance", self.instance))
        ...
        template = None
        if data["template_id"]:
            template = CustomFunctionTemplate.get_template(data["template_id"])
            if not template:
                properties = {"team_id": team.id, "template_id": data.get("template_id")}
                if instance and instance.id:
                    properties["custom_function_id"] = instance.id
                capture_exception(
    ...
    def validate(self, attrs):
        ...
        if hog_type == "transformation" and attrs.get("enabled", False):
            ...
            apply_limit = is_create or (isinstance(self.instance, CustomFunction) and not self.instance.enabled)

            if apply_limit:
                # Count enabled and non-deleted transformations
                transformation_count = CustomFunction.objects.filter(
        ...
        if "hog" in attrs:
            ...
            if hog_code_size > MAX_HOG_CODE_SIZE_BYTES:
                raise serializers.ValidationError(
                    {
                        "custom_script": f"HOG code exceeds maximum size of {MAX_HOG_CODE_SIZE_BYTES // 1024}KB. Please simplify your code or contact support if you need this limit increased."
                    }
                )
        ...
        if is_create:
            if not attrs.get("hog"):
                raise serializers.ValidationError({"custom_script": "Required."})

        return attrs

    def to_representation(self, data):
        encrypted_inputs = data.encrypted_inputs or {} if isinstance(data, CustomFunction) else {}
    ...
    def create(self, validated_data: dict, *args, **kwargs) -> CustomFunction:
        request = self.context["request"]
        validated_data["created_by"] = request.user

        template_id = validated_data.get("template_id")
        if template_id:
            db_template = CustomFunctionTemplate.objects.get(template_id=template_id)
            if not db_template:
                raise serializers.ValidationError({"template_id": f"No template found for id '{template_id}'"})
            validated_data["custom_function_template"] = db_template
    ...
    def _get_highest_execution_order(self, team_id: int) -> int:
        ...
        highest_order = (
            CustomFunction.objects.filter(team_id=team_id, type="transformation", deleted=False)
    ...
    def update(self, instance: CustomFunction, validated_data: dict, *args, **kwargs) -> CustomFunction:
        ...
        # Standard update
        res: CustomFunction = super().update(instance, validated_data)

        if res.enabled and res.status.get("state", 0) == CustomFunctionState.DISABLED.value:
            res.set_function_status(CustomFunctionState.DEGRADED.value)

        return res


class CustomFunctionInvocationSerializer(serializers.Serializer):
    configuration = CustomFunctionSerializer(write_only=True)
...
class CustomFunctionFilterSet(FilterSet):
...
@extend_schema(tags=["custom_functions"])
class CustomFunctionViewSet(
    ...
    scope_object = "custom_function"
    queryset = CustomFunction.objects.all()
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    search_fields = ["name", "description"]
    filterset_class = CustomFunctionFilterSet
    ...
    def get_serializer_class(self) -> type[BaseSerializer]:
        if self.action == "list":
            ...
            return CustomFunctionMinimalSerializer
        return CustomFunctionSerializer
    ...
    def icons(self, request: Request, *args, **kwargs):
        ...
        icons = CDPIconsService().list_icons(query, icon_url_base="/api/projects/@current/custom_functions/icon/?id=")
    ...
    def invocations(self, request: Request, *args, **kwargs):
        try:
            custom_function = self.get_object()
        except Exception:
            custom_function = None

        serializer = CustomFunctionInvocationSerializer(
            data=request.data, context={**self.get_serializer_context(), "instance": custom_function}
        )
        ...
        res = create_hog_invocation_test(
            ...
            custom_function_id=str(custom_function.id) if custom_function else "new",
    ...
    def perform_create(self, serializer):
        ...
        log_activity_from_viewset(
            ...
            detail_type=humanize_custom_function_type(serializer.instance.type),
        )
    ...
    def perform_update(self, serializer):
        ...
        try:
            ...
            before_update = CustomFunction.objects.get(pk=instance_id)
    ...
    def rearrange(self, request: Request, *args, **kwargs) -> Response:
        """Update the execution order of multiple CustomFunctions."""
        team = self.team
        ...
        with transaction.atomic():
            ...
            functions = {
                str(f.id): f
                for f in CustomFunction.objects.filter(
            ...
            if missing_ids:
                raise exceptions.ValidationError(f"CustomFunction with id {missing_ids.pop()} does not exist")
            ...
            for function_id, function in functions.items():
                ...
                if old_order != new_order:
                    ...
                    log_activity(
                        ...
                        detail=Detail(
                            name=function.name,
                            type=humanize_custom_function_type(function.type),
                            changes=[
                                Change(
                                    type="CustomFunction",
                                    action="changed",
        ...
        transformations = CustomFunction.objects.filter(team=team, type="transformation", deleted=False).order_by(
            "execution_order"
        )
    ...
    def enable_backfills(self, request: Request, *args, **kwargs):
        ...
        if custom_function.batch_export_id:
        ...
        batch_export_data = {
            ...
            "interval": "day",
            "model": "events",
            "filters": custom_function.filters.get("events", []) if custom_function.filters else [],
            "destination": {
                "type": "Workflows",
                "config": {},
            },
        ...
        custom_function.batch_export_id = batch_export.id
        custom_function.save(update_fields=["batch_export_id"])
