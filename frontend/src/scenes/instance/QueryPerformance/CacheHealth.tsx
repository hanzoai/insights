...
import { TimeSeriesBarChart } from '@hanzo/quill-charts'
...
function ttlStatus(partition: string): { label: string; type: TagType } {
...
function TableCard({ table, growth }: { table: CacheTableStats; growth: CacheGrowthResponse | null }): JSX.Element {
    if (table.unavailable) {
        return (
            <Card hoverEffect={false} className="flex-1 min-w-72">
...
export function CacheHealth(): JSX.Element {
    ...
    const partitionColumns: TableColumns<CachePartitionRow> = [
        {
            title: 'Expiry day',
            width: 220,
        ...
        ...tables.flatMap((table): TableColumns<CachePartitionRow> => {
        ...
        <>
            <div className="flex items-center justify-between gap-2 mb-2">
                <p className="text-muted text-sm mb-0">
                    Physical footprint of the experiment preaggregation tables (totals from Datastore system.parts,
            ...
            <div className="flex flex-wrap gap-4 mb-4">
                {!cacheHealth && cacheHealthLoading
                    ? [0, 1].map((i) => (
                          <Card key={i} hoverEffect={false} className="flex-1 min-w-72">
