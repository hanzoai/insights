...
-- Full CREATE schema for the prod-us/batch_exports node. Apply to a fresh Datastore to build it.
...
CREATE TABLE insights.sharded_events_recent (
  uuid UUID,
  event String,
  properties String,
...
) ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/batch_exports/{shard}/insights.sharded_events_recent', '{replica}', _timestamp) ORDER BY (team_id, toStartOfHour(inserted_at), event, cityHash64(distinct_id), cityHash64(uuid)) PARTITION BY toStartOfDay(inserted_at) TTL toDateTime(inserted_at) + toIntervalDay(7) SETTINGS index_granularity = 8192, parts_to_delay_insert = 800, parts_to_throw_insert = 1500, ttl_only_drop_parts = 1;
