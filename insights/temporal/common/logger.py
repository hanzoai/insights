...
`insights.management.commands.start_temporal_worker`.
    ...
    def __call__(self, name: str | None = None, write: bool = True, produce: bool = True) -> Logger:
        ...
        * We will only write to a file (stdout by default), meaning no logs will be
        ...
        if name:
            ...
            # NOTE: This means we should pass "insights.temporal.common.logger" as the
            # name for this module's logger, if any is used.
            _, resolved_name = _find_first_app_frame_and_name(["insights.temporal.common.logger"])
    ...
    * Spawning a task to shutdown gracefully on worker shutdown.
    ...
    base_processors: list[structlog.types.Processor] = [
        ...
        structlog.processors.CallsiteParameterAdder(
            ...
            additional_ignores=["insights.temporal.common.logger", "WrapperLogger"],
        ),
    ]
...
class KafkaLogProducerFromQueueAsync:
    ...
    def __init__(
        ...
        self.logger = structlog.get_logger("insights.temporal.common.logger.KafkaLogProducerFromQueueAsync")
