...
describe('toolResolver', () => {
    describe('resolveToolKey', () => {
        it('parses the inner tool name out of a single-exec call command', () => {
            const resolved = resolveToolKey('insights', 'exec', { command: 'call insight-create {"name":"Signups"}' })
        ...
        it('parses --json single-exec call commands', () => {
            const resolved = resolveToolKey('insights', 'exec', {
        ...
        it('maps discovery verbs to sentinels', () => {
            expect(resolveToolKey('insights', 'exec', { command: 'tools' }).resolvedKey).toEqual(
                '__insights_exec_tools__'
            )
            expect(resolveToolKey('insights', 'exec', { command: 'search recordings' }).resolvedKey).toEqual(
                '__insights_exec_search__'
            )
        })

        it('falls back to unknown sentinel for malformed commands', () => {
            expect(resolveToolKey('insights', 'exec', { command: '!!!' }).resolvedKey).toEqual(
                '__insights_exec_unknown__'
            )
        })
        ...
        it.each([
            ...
            const resolved = resolveToolKey('insights', 'exec', { command })
        ...
        it.each([['call'], ['call --json'], ['call --confirm --json']])(
            ...
            (command) => {
                const resolved = resolveToolKey('insights', 'exec', { command })
                expect(resolved.resolvedKey).toEqual('__insights_exec_unknown__')
        ...
        it('resolves exec invocations when the canonical tool name only arrives in metadata', () => {
            expect(
                resolveToolCall({
                    ...
                    meta: { claudeCode: { toolName: 'mcp__insights__exec' } },
                })
            ).toEqual({
                resolvedKey: 'query-trends',
                innerToolName: 'query-trends',
                innerInput: { kind: 'TrendsQuery', series: [] },
                claudeToolName: 'mcp__insights__exec',
            })
        })
