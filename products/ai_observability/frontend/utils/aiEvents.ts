...
import { InsightsQLQuery, NodeKind, ProductKey } from '~/queries/schema/schema-general'
 ...
 * 2. Fallback: Query Datastore directly for recent events (for new users)
 */
export async function hasRecentAIEvents(): Promise<boolean> {
    ...
    // Fallback: query Datastore directly for recent events (new users)
    const response = await api.query<InsightsQLQuery>(
        {
            kind: NodeKind.InsightsQLQuery,
            query: insightsql`SELECT 1 FROM events WHERE event IN ${[...AI_EVENT_NAMES]} AND timestamp > now() - INTERVAL 12 HOUR LIMIT 1`,
