...
from insights.models.group.sql import INSERT_GROUP_SQL
...
def create_group(
    ...
    from insights.personinsights_client.converters import proto_group_to_model
    from insights.personinsights_client.proto import CreateGroupRequest
...
def save_group(group: Group, *, operation: str = "group_save") -> None:
    """Save a Group's group_properties via personinsights. Only group_properties is synced."""
    from insights.personinsights_client.client import personinsights_call, require_personinsights_client
    from insights.personinsights_client.proto import UpdateGroupRequest
    ...
    personinsights_call(operation, _update, caller_tag="group/save_group")


def get_group_by_key(team_id: int, group_type_index: int, group_key: str) -> Group | None:
    """Fetch a single Group via personinsights. Returns None when the group does not exist."""
    from insights.personinsights_client.client import personinsights_call, require_personinsights_client
    from insights.personinsights_client.converters import proto_group_to_model
    from insights.personinsights_client.proto import GetGroupRequest
    ...
    return personinsights_call("get_group_by_key", _fetch, caller_tag="group/get_group_by_key")


def get_groups_by_identifiers(team_id: int, group_type_index: int, group_keys: list[str]) -> list[Group]:
    """Fetch multiple Groups via personinsights."""
    from insights.personinsights_client.client import personinsights_call, require_personinsights_client
    from insights.personinsights_client.converters import proto_group_to_model
    from insights.personinsights_client.proto import GetGroupsRequest, GroupIdentifier
    ...
    def _fetch() -> list[Group]:
        groups: list[Group] = []
        for i in range(0, len(group_keys), settings.PERSONFN_BATCH_SIZE):
            identifiers = [
                GroupIdentifier(group_type_index=group_type_index, group_key=key)
                for key in group_keys[i : i + settings.PERSONFN_BATCH_SIZE]
            ]
    ...
    return personinsights_call("get_groups_by_identifiers", _fetch, caller_tag="group/get_groups_by_identifiers")
...
def get_groups_by_type_indices(team_id: int, group_type_indices: set[int], group_keys: set[str]) -> list[Group]:
    ...
    def _fetch() -> list[Group]:
        ...
        groups: list[Group] = []
        for i in range(0, len(identifiers), settings.PERSONFN_BATCH_SIZE):
            resp = client.get_groups(
                GetGroupsRequest(team_id=team_id, group_identifiers=identifiers[i : i + settings.PERSONFN_BATCH_SIZE])
            )
            groups.extend(proto_group_to_model(g) for g in resp.groups if g.id)
        return groups

    return personinsights_call("get_groups_by_type_indices", _fetch, caller_tag="group/get_groups_by_type_indices")
...
def _escape_datastore_like(value: str) -> str:
...
def list_groups(
    ...
    *,
    ...
    """List groups for the public groups API, served from Datastore.
    ...
    """
    from insights.insightsql import ast  # noqa: PLC0415 — keep InsightsQL off this widely-imported module's import path
    from insights.insightsql.query import execute_insightsql_query  # noqa: PLC0415

    from insights.models.team import Team  # noqa: PLC0415
    ...
    if group_key_contains:
        where_exprs.append(
            ast.CompareOperation(
                ...
                right=ast.Constant(value=f"%{_escape_datastore_like(group_key_contains)}%"),
            )
        )
            ...
            ast.Or(
                exprs=[
                    ast.CompareOperation(
                        ...
                        right=ast.Constant(value=f"%{_escape_datastore_like(search)}%"),
                    ),
                    ast.CompareOperation(
                        op=ast.CompareOperationOp.Eq,
    ...
    response = execute_insightsql_query(query, team=team, query_type="groups_list")
