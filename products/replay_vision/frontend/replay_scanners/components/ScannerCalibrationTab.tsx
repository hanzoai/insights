...
// data-attr must live on each option: SegmentedButton renders no element of its own that takes one.
...
const SUGGESTION_STATUS_TAGS: Record<string, { type: TagType; label: string; tooltip: string }> = {
    applied: {
        type: 'success',
        label: 'Applied',
...
const EVALUATION_OUTCOME_TAGS: Record<string, { type: TagType; label: string }> = {
                    ...
                    <>
                        ...
                        {downTotal > 0 && (
                            <Tooltip title="Results you rated wrong that changed under the suggested prompt">
                                <Tag type={summary.fixed > 0 ? 'success' : 'muted'}>
                        ...
                        {upTotal > 0 && (
                            <Tooltip title="Results you rated right that are unchanged under the suggested prompt">
                                <Tag type={summary.regressed > 0 ? 'danger' : 'success'}>
                                    {summary.kept} of {upTotal} right results kept
                                </Tag>
                            </Tooltip>
                        )}
                        {summary.errors > 0 && <Tag type="muted">{summary.errors} failed to run</Tag>}
                    </>
                )}
            ...
            {detailsOpen && (
                <Table
                    ...
                    columns={
                        [
                            {
                                ...
                                key: 'outcome',
                                render: (_, result) => (
                                    <Tag type={EVALUATION_OUTCOME_TAGS[result.outcome]?.type ?? 'muted'}>
                        ...
                        ] as TableColumns<PromptEvaluationResultApi>
                    }
        ...
        body = (
            <div className="flex flex-wrap items-center justify-between gap-2 py-2">
                <span className="text-muted text-sm">
                    {ratedCount === 0
                        ? 'Rate results below to get Insights AI recommendations here.'
        ...
        body = (
            <div className="space-y-3">
                <p className="text-sm m-0">
                    Insights AI reviewed your rated results and found no prompt changes to recommend.
                </p>
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                {suggestionStale && currentSuggestion && (
                    <Tooltip title="Refreshes automatically about once a day; regenerate to update now">
                        <Tag type="warning">New ratings since this was generated</Tag>
                    </Tooltip>
                )}
        ...
        <div className="border rounded p-4 bg-surface-primary space-y-3">
            <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="ml-auto">
                    <SegmentedButton size="xsmall" value={mode} onChange={setMode} options={CHART_MODE_OPTIONS} />
                </div>
            </div>
            {versionAccuracy.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5 text-xs tabular-nums">
                    {versionAccuracy.map((entry) => (
                        ...
                        >
                            <Tag type={entry.isCurrent ? 'highlight' : 'muted'}>
                                {entry.pct !== null
...
export function ScannerCalibrationTab({ scannerId }: { scannerId: string }): JSX.Element {
    ...
    const columns: TableColumns<ReplayObservationApi> = [
        {
            title: 'Session',
            key: 'session',
            width: 260,
        ...
        },
        {
            ...
            render: (_, obs) => {
                ...
                return (
                    <Tooltip title={tag.tooltip}>
                        <Tag type={tag.type} className="font-mono">
                            {tag.label}
                        </Tag>
                    </Tooltip>
                )
