...
// Ownership is resolved by the shared insightscli resolver (distributed owners.yaml +
...
function resolveOwners(filenames) {
    ...
    // importable.
    const launcher =
        "import sys, runpy; sys.path.insert(0, 'tools/owners'); runpy.run_module('insights_owners', run_name='__main__')"
...
// `@Insights/<slug>` for teams, `@handle` for users — then classify.
function mapResolvedOwner(rawOwner) {
    if (!rawOwner) {
        return null
    }
    const token = rawOwner.startsWith('@') ? rawOwner : `@Insights/${rawOwner}`
...
function classifyOwner(owner) {
    if (owner.startsWith('@Insights/')) {
        return { type: 'team', name: owner.replace('@Insights/', ''), owner }
    }
    if (owner.startsWith('@')) {
