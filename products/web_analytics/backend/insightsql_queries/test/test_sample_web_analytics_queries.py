"""
Sample web analytics queries for InsightsQL snapshot testing.

This is a special test file designed to capture InsightsQL query snapshots from the web analytics components.
...
or to come up with some SQL queries. We already have the datastore snapshots, but by using insightsQL we can
...
test methods for each breakdown type. Each test method generates its own InsightsQL snapshot.

Let's try to keep this up to date with the components used in the WebAnalyticsScene.
"""

from insights.test.base import APIBaseTest, DatastoreTestMixin, snapshot_insightsql_queries
...
class TestSampleWebAnalyticsQueries(DatastoreTestMixin, APIBaseTest):
    """
    Sample web analytics queries that demonstrate InsightsQL query generation.

    These tests capture snapshots of the InsightsQL queries generated by our web analytics components.
