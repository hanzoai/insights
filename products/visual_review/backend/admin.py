...
`insights/models/scoping/README.md`.

Registration: each `ModelAdmin` uses `@admin.register(<Model>)` directly.
`register_all_admin()` in `insights/admin/__init__.py` runs Django's
`autodiscover_modules('admin')` lazily on first admin access, which fires
these decorators. The `admin-modeladmin-needs-register-decorator` semgrep
rule enforces the convention.
"""
...
def _team_link(team_id: int | None) -> str | SafeString:
    if team_id is None:
        return "—"
    url = reverse("admin:insights_team_change", args=[team_id])
...
class RunAdmin(admin.ModelAdmin):
    ...
    def approved_by_link(self, run: Run) -> str | SafeString:
        ...
        if run.approved_by_id is None:
            return "—"
        url = reverse("admin:insights_user_change", args=[run.approved_by_id])
...
class RunSnapshotAdmin(admin.ModelAdmin):
    ...
    def reviewed_by_link(self, snapshot: RunSnapshot) -> str | SafeString:
        if snapshot.reviewed_by_id is None:
            return "—"
        url = reverse("admin:insights_user_change", args=[snapshot.reviewed_by_id])
...
class ToleratedHashAdmin(admin.ModelAdmin):
    ...
    def created_by_link(self, hash_obj: ToleratedHash) -> str | SafeString:
        if hash_obj.created_by_id is None:
            return "—"
        url = reverse("admin:insights_user_change", args=[hash_obj.created_by_id])
...
class QuarantinedIdentifierAdmin(admin.ModelAdmin):
    ...
    def created_by_link(self, q: QuarantinedIdentifier) -> str | SafeString:
        if q.created_by_id is None:
            return "—"
        url = reverse("admin:insights_user_change", args=[q.created_by_id])
