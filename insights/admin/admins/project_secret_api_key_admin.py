...
from insights.admin.admins.api_key_roll_mixin import RollApiKeyAdminMixin
from insights.api.project_secret_api_key import roll_project_secret_api_key_and_notify
from insights.models import ProjectSecretAPIKey
...
class ProjectSecretAPIKeyAdmin(RollApiKeyAdminMixin):
    ...
    def team_link(self, key: ProjectSecretAPIKey) -> SafeString:
        return format_html(
            '<a href="{}">{}</a>',
            reverse("admin:insights_team_change", args=[key.team_id]),
            key.team_id,
        )
    ...
    def created_by_link(self, key: ProjectSecretAPIKey) -> str | SafeString:
        ...
        return format_html(
            '<a href="{}">{}</a>',
            reverse("admin:insights_user_change", args=[key.created_by.pk]),
