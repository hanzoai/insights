...
export function AddWidgetModal({ isOpen, onClose, loading, onAdd }: AddWidgetModalProps): JSX.Element {
    ...
    function handleFeedbackClicked(): void {
        insights.capture('dashboard add widget modal - feedback clicked')
        onClose()
    }

    function handleProductIntroClicked(productType: ProductKey): void {
        insights.capture('dashboard add widget modal - product intro clicked', { product_type: productType })
    }
    ...
    return (
        <Modal
            ...
            onClose={onClose}
            title="Add widget"
            description="Bring context from your different Insights products into one dashboard."
            width={1200}
            footer={
                <>
                    <Button
                    ...
                    {DASHBOARD_WIDGET_CATALOG_GROUPS.map((group, groupIndex) => {
                        ...
                        return (
                            <Fragment key={group.groupId}>
                                {groupIndex > 0 ? <Divider className="col-span-full my-0" /> : null}
                                <div
                                    role="button"
                                    tabIndex={0}
