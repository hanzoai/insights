...
A Insights-registered OAuth source (`SourceFieldOauthConfig`, `OAuthMixin`, a new `IntegrationKind`)
...
Worked example: the Resend source — insights/insights **#73107** (code) and Insights/charts **#13413**
(env wiring).
...
like Resend's `POST /oauth/register`). Register **all** Insights callbacks up front — providers pin
`redirect_uri` to an exact match:

- `https://us.hanzo.ai/integrations/<kind>/callback`
- `https://eu.hanzo.ai/integrations/<kind>/callback`
- `https://app.dev.insights.dev/integrations/<kind>/callback`
...
`insights/settings/integrations.py` (convention: `<SOURCE>_APP_CLIENT_ID` / `<SOURCE>_APP_CLIENT_SECRET`,
...
## 3. Charts — map env var → secret key (Insights/charts PR)
...
- **`shared/insights-django/common.yaml`** → `secret_env:` (store `insights-django-shared-secrets`) — the
...
## 4. Secrets — write the values into AWS Secrets Manager (Insights/secrets)

`Insights/secrets` is a **CLI over AWS Secrets Manager — values are never committed to git.** Use the
...
```yaml
secrets:
  insights-django-shared-secrets:
...
- Adding **keys to existing** secrets (`insights-django-shared-secrets` already exists) needs no infra
  change. Creating a **brand-new** secret store also needs an EKS-access grant in
  `terraform/modules/eks/external-secrets.tf` (insights-cloud-infra).
- ExternalSecrets sync from AWS SM to Kubernetes hourly; the CLI offers an immediate Argo force-sync
  after writing (needs the Insights tailnet), else `kubectl annotate es <name> force-sync=$(date +%s) --overwrite -n insights`.
