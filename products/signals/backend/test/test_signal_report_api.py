...
from insights.egress.github.transport import GitHubEgressBudgetExhausted
...
class TestSignalReportListAPI(APIBaseTest):
    ...
    def test_source_products_present_on_retrieve(self):
        ...
        # scout_name flows from the Datastore meta through the view's map split into the serializer.
    ...
    def test_source_products_resilient_to_datastore_failure_on_retrieve(self):
        report = self._create_report()
...
class TestSignalReportSuppressionAPI(APIBaseTest):
    ...
    def test_resolve_feedback_reaches_the_status_change_label(self, _name, body, expected_reason):
        ...
        report = self._create_report()
        with patch("products.signals.backend.receivers.hanzo_insights.capture") as mock_capture:
    ...
    def test_state_transition_resilient_to_datastore_failure(self):
        # A Datastore hiccup during best-effort enrichment must not 500 an already-committed
...
class TestSignalReportTaskAssociationViaArtefacts(APIBaseTest):
    ...
    def test_associate_defaults_to_header_task(self):
        ...
        response = self._associate(report, {}, headers={"X-Insights-Task-Id": str(task.id)})
        assert response.status_code == status.HTTP_201_CREATED, response.json()
        artefact = SignalReportArtefact.objects.get(report=report, type=SignalReportArtefact.ArtefactType.TASK_RUN)
...
class TestSignalReportContentUpdateAPI(APIBaseTest):
    ...
    def test_edit_attributed_to_task_when_header_present(self):
        ...
        response = self.client.patch(
            ...
            format="json",
            headers={"X-Insights-Task-Id": str(task.id)},
        )
        assert response.status_code == status.HTTP_200_OK
        artefacts = self._artefacts(report, SignalReportArtefact.ArtefactType.TITLE_CHANGE)
        assert len(artefacts) == 1
...
class TestSignalReportPrEndpoints(APIBaseTest):
    ...
    def test_review_comment_writes_are_scoped_to_the_reports_pr(
        ...
        user_github.return_value.get_pull_request_review_comment.return_value = {
            ...
            "comment": {
                "id": 99,
                "pull_request_url": f"https://api.github.com/repos/Insights/insights/pulls/{comment_pr}",
            },
        }
    ...
    def test_pr_checks_success_returns_checks(self):
        ...
        assert github.return_value.get_pull_request_checks.call_args.args == ("Insights/insights", 7)
