    ...
    describe('keyed instances', () => {
        ...
        it('merges pinned filters into queryFilterGroup without exposing them to editable state', () => {
            ...
            try {
                ...
                expect(JSON.stringify(pinned.values.filterGroup)).not.toContain('insightsDistinctId')
                ...
                expect(inner.values[0]).toMatchObject({ key: 'insightsDistinctId', value: ['distinct-id-1'] })
                ...
                expect(combined.values[0]).toMatchObject({ key: 'insightsDistinctId' })
