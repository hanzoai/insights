...
any InsightsQL query that calls `apply_path_cleaning`. They are the right answer when
...
- slugs: `/posts/why-i-love-insights`
...
- **Project InsightsQL** (via `execute-sql`):
...
- **Backreferences in `alias` need double-escaping** — Datastore's
  ...
  doubled, so use `\\1` in `path_cleaning_filters` / InsightsQL to get the `\1`
  backreference at the Datastore layer.
