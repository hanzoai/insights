...
from posthog.schema import AssistantInsightsQLQuery, HumanMessage
...
async def serialize_database(team: Team):
    ...
    context = InsightsQLContext(team=team, database=database, enable_select_queries=True)
...
async def call_graph(entry: DatasetInput, *args):
    ...
    if maybe_viz_message:
        ...
        return EvalOutput(
            ...
            sql_query=content.query.query if isinstance(content.query, AssistantInsightsQLQuery) else None,
        )
