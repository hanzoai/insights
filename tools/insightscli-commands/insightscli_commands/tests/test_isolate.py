...
import insightscli_commands.product.isolate as iso
from insightscli_commands.product.isolate import (
    Reference,
    _module_dotted,
...
@pytest.mark.parametrize(
    ...
    [
        ("products.logs.backend.models", "insights/api/team.py", "model-access"),
        ("products.logs.backend.logs_query_runner", "insights/insightsql_queries/query_runner.py", "query-runner"),
        ("products.logs.backend.tasks", "insights/tasks/scheduled.py", "celery-task"),
        ("products.logs.backend.temporal.metrics", "insights/temporal/common/worker.py", "temporal-wiring"),
        ("products.logs.backend.models", "insights/insightsql/database/schema/test/test_system_tables.py", "test-fixture"),
        ("products.logs.backend.models", "insights/api/test_team.py", "test-fixture"),
        ("products.logs.backend.alert_utils", "insights/api/team.py", "other-internal"),
    ],
)
...
def test_scan_references_classifies_imports_and_strings(tmp_path: Path) -> None:
    caller = tmp_path / "insights" / "api" / "thing.py"
...
def test_core_coupling_count_only_counts_core_imports() -> None:
    refs = [
        Reference(
            file="insights/api/team.py", line=1, module="products.x.backend.models", kind="model-access", is_import=True
        ),
...
def test_find_views_path_accepts_presentation_views_package(tmp_path: Path) -> None:
    from insightscli_commands.product.paths import find_views_path
