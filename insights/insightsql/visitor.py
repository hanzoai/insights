...
from insights.insightsql.base import AST, Expr
from insights.insightsql.errors import BaseInsightsQLError
from insights.insightsql.utils import is_simple_value
...
class CloningVisitor(Visitor[Any]):
    ...
    def visit_insightsqlx_tag(self, node: ast.InsightsQLXTag):
        return ast.InsightsQLXTag(kind=node.kind, attributes=[self.visit(a) for a in node.attributes])

    def visit_insightsqlx_attribute(self, node: ast.InsightsQLXAttribute):
        if isinstance(node.value, list):
            return ast.InsightsQLXAttribute(
                name=node.name,
        ...
        return ast.InsightsQLXAttribute(name=node.name, value=self.visit(value))
