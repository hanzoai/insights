...
from insights.api.organization import OrganizationSerializer
from insights.api.services.flags_service import get_flags_from_service
from insights.api.shared import OrganizationBasicSerializer, TeamBasicSerializer
from insights.api.utils import ClassicBehaviorBooleanFieldSerializer, action, unparsed_hostname_in_allowed_url_list
...
from insights.constants import INVITE_DAYS_VALIDITY, PERMITTED_FORUM_DOMAINS
...
from insights.models import OrganizationInvite, Team, User, UserScenePersonalisation
from insights.models.oauth import OAuthGrant, find_oauth_refresh_token
from insights.models.onboarding_delegation import cancel_pending_delegation, clear_delegation_state
...
from insights.models.user import (
...
class UserSerializer(serializers.ModelSerializer):
    ...
    anonymize_data = ClassicBehaviorBooleanFieldSerializer(
        help_text="Whether Insights should anonymize events captured for this user when identified."
    )
    ...
    def to_representation(self, instance: Any) -> Any:
        # Refresh Insights person properties so the dogfood instance keeps the user's
        # org/project/role properties current. hanzo_insights.capture is non-blocking
...
def toolbar_oauth_callback(request):
    ...
    toolbar_param = f"__insights_toolbar=code:{quoted_code},client_id:{quoted_client_id}"
...
def prepare_toolbar_preloaded_flags(request):
    ...
    try:
        ...
        # SDK traffic — the toolbar launch is internal Insights UI, not an SDK call.
...
def redirect_to_site(request):
    ...
    # see https://github.com/Insights/insights/issues/9671
