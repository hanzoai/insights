...
from products.insights_ai.eval_harness import runner
from products.insights_ai.eval_harness.config import SandboxedEvalCase
from products.insights_ai.eval_harness.harness.cli import parse_args
from products.insights_ai.eval_harness.harness.live_server import EvalLiveServer
from products.insights_ai.eval_harness.harness.providers import ModalProviderStrategy, SandboxProviderStrategy
...
def test_eval_live_server_routes_event_ingest_through_asgi() -> None:
    with (
        patch("insights.asgi._ensure_post_fork_init"),
        patch("insights.utils.initialize_self_capture_api_token", new=AsyncMock()),
    ):
...
def test_setup_django_disables_self_capture_before_settings_load() -> None:
    result = subprocess.run(
        [
            ...
            "-c",
            (
                "from products.insights_ai.eval_harness.harness.django_env import setup_django\n"
                "setup_django()\n"
