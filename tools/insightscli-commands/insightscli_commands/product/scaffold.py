...
def _team_scope_template_vars(*, separate_db: bool) -> dict[str, str]:
    ...
    if separate_db:
        return {
            ...
            "team_base_import": "from insights.models.scoping.product_mixin import ProductTeamModel",
        }
    return {
        "team_base_mixin": "TeamScopedRootMixin",
        "team_base_import": "from insights.models.scoping.root_mixin import TeamScopedRootMixin",
    }
...
def _add_to_frontend_package_json(product_name: str, *, dry_run: bool) -> None:
    pkg_name = f"@hanzo/products-{product_name.replace('_', '-')}"
    ...
    *,
    ...
    if not dry_run:
        ...
        click.secho(
            ...
            "Read insights/models/scoping/README.md for the contract before writing your first model.",
            fg="cyan",
        )
