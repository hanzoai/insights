"""Insights-specific telemetry property hooks.

Registers a hook that enriches every ``command_started`` / ``command_completed``
event with Insights-developer context (process manager, devenv state, org
...
def _git_branch() -> str | None:
    """Current branch name — the join key from a insightscli run back to its PR.

    "HEAD" (detached) and failures collapse to None so they don't pollute the
    dimension. Cached per process: insightscli commands are short-lived, so the branch
...
def _check_email_domain() -> bool:
    """Fallback signal: git ``user.email`` ends with ``@hanzo.ai``."""
    parser = configparser.RawConfigParser()
    try:
        parser.read([Path.home() / ".gitconfig", REPO_ROOT / ".git" / "config"])
        email = parser.get("user", "email", fallback="")
        return email.endswith("@hanzo.ai")
    except Exception:
        return False


def _check_github_org_membership() -> bool | None:
    """Use ``gh api`` to check Insights org membership. Returns None if gh is unavailable."""
    try:
        result = subprocess.run(
            ["gh", "api", "/user/memberships/orgs/Insights", "--silent"],
            capture_output=True,
            timeout=5,
        )
...
def _is_insights_dev() -> bool:
    """Return True if the current user is a Insights GitHub org member.
    ...
    config["is_insights_org_member"] = is_member
    config["org_check_timestamp"] = time.time()
    _save_config(config)
    return is_member


def _insights_telemetry_properties(command: str | None = None) -> dict[str, Any]:
...
register_telemetry_properties(_insights_telemetry_properties)
