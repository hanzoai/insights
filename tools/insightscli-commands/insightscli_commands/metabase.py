...
Workflow:
    insightscli metabase:login --region us|eu|dev     # opens browser, captures cookies
    insightscli metabase:databases --region us|eu|dev # list databases with current IDs
    insightscli metabase:query --region us|eu|dev \\  # run SQL against /api/dataset
        --database-id <id> < query.sql
    insightscli metabase:cookie --region us|eu|dev    # print cached cookie header (humans)

Cookies are cached at ~/.config/insights/metabase/cookie-{region} with mode 0600.
...
REGIONS: dict[str, str] = {
    "us": "metabase.prod-us.insights.dev",
    "eu": "metabase.prod-eu.insights.dev",
    "dev": "metabase.dev.insights.dev",
}
...
CACHE_DIR: Path = Path.home() / ".config" / "insights" / "metabase"
...
def metabase_cookie(region: str, check: bool) -> None:
    ...
    if cookie_header is None:
        raise click.ClickException(
            f"No cached cookie for region {region}. Run `insightscli metabase:login --region {region}`.",
        )
    if check and not _check_cookie(REGIONS[region], cookie_header):
        raise click.ClickException(
            f"Cached cookie for {region} is no longer valid. Run `insightscli metabase:login --region {region}` to refresh.",
        )
    # No trailing newline so $(insightscli metabase:cookie) yields a clean header.
...
def _metabase_get(region: str, path: str, timeout: float = 30.0) -> Any:
    ...
    if response.status_code in (301, 302):
        raise click.ClickException(
            ...
            f"Run `insightscli metabase:login --region {region}` to refresh cookies.",
        )
...
def _metabase_post_dataset(region: str, database_id: int, sql: str, timeout: float = 120.0) -> Any:
    ...
    if response.status_code == 404:
        raise click.ClickException(
            ...
            f"Run `insightscli metabase:databases --region {region}` to see current IDs.",
        )
...
@click.option(
    ...
    type=int,
    required=True,
    help="Database ID (get from `insightscli metabase:databases --region <region>`)",
)
@click.option(
    "--file",
    "sql_file",
