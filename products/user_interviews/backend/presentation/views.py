...
from insights.permissions import InsightsFeatureFlagPermission
from insights.rate_limit import UserInterviewInviteThrottle
...
class UserInterviewSerializer(serializers.ModelSerializer):
    ...
    def _attempt_to_map_speaker_names(self, transcript: str, interviewee_emails: list[str]) -> dict[str, str] | None:
        ...
        assignment_response = OpenAI(
            insights_client=hanzo_insights.default_client, base_url=settings.OPENAI_BASE_URL
...
""".strip(),
    ...
    def _summarize_transcript(self, transcript: str) -> str:
        summary_response = OpenAI(insights_client=hanzo_insights.default_client).responses.create(  # type: ignore
            model="gpt-4.1-mini",
            insights_trace_id=self._ai_trace_id,
            insights_distinct_id=self.context["request"].user.distinct_id,
            input=[
...
# interviews ships a giant IN list to Datastore. Realistic topics are tiny (<100); the
...
class UserInterviewSearchResultSerializer(serializers.Serializer):
    ...
    interviewee_identifier = serializers.CharField(
        help_text="Email or Insights distinct ID of the interviewee.",
    )
    topic_id = serializers.UUIDField(
        allow_null=True,
...
class UserInterviewViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    insights_feature_flag = "user-interviews"
    permission_classes = [InsightsFeatureFlagPermission]
...
class UserInterviewTopicSerializer(serializers.ModelSerializer):
    ...
    interviewee_distinct_ids = serializers.ListField(
        ...
        required=False,
        help_text="Insights distinct IDs of people to interview. May be combined with interviewee_emails.",
    )
...
class IntervieweeIdentifierRequestSerializer(serializers.Serializer):
    identifier = serializers.CharField(
        ...
        help_text=(
            "Email address or Insights distinct ID for the interviewee. Email-shaped values "
...
class PreviewInviteRequestSerializer(serializers.Serializer):
    interviewee_identifier = serializers.CharField(
        ...
        max_length=400,
        help_text=(
            "Which targeted interviewee to render the preview for (an email or Insights distinct ID "
...
class UserInterviewTopicViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "public interview viewer with no Insights auth required."
        ),
    )
    ...
    def send_invites(self, request: Request, *args: Any, **kwargs: Any) -> response.Response:
        if not is_email_available():
            return response.Response(
                {"error": "Email is not configured for this Insights instance."},
...
class IntervieweeContextSerializer(serializers.ModelSerializer):
    ...
    interviewee_identifier = serializers.CharField(
        max_length=400,
        help_text="Identifier for the interviewee — typically an email address or Insights distinct ID. Must match a value in the parent topic's interviewee_emails or interviewee_distinct_ids.",
    )
    agent_context = serializers.CharField(
        max_length=10000,
