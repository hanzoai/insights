...
from insights.insightsql.printer.redshift_functions import (
...
class RedshiftPrinter(PostgresPrinter):
    """Prints a InsightsQL AST as Amazon Redshift SQL.
    ...
    otherwise silently return different results than InsightsQL promises: integer division is cast to
    ...
    DIALECT_NAME: ClassVar[InsightsQLDialect] = "redshift"
    ...
    def visit_arithmetic_operation(self, node: ast.ArithmeticOperation) -> str:
        if node.op == ast.ArithmeticOperationOp.Div:
            # Redshift `/` on two integers truncates; InsightsQL division is always float.
