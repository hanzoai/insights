import { isInsightsExecTool } from '../components/tool/insightsExecDisplay'
import type { PermissionRequestRecord } from '../types/streamTypes'
import { resolveToolCall } from '../utils/toolResolver'

// Re-exported so existing importers (and tests) keep resolving the exec-tool check from here.
export { isInsightsExecTool } from '../components/tool/insightsExecDisplay'
 ...
 * Insights `exec` auto-approve; destructive sub-tools prompt; persist/publish sub-tools prompt only
 ...
 *
 * Insights `exec` is detected by canonical tool name and by the parsed verb/sub-tool (robust to a
...
export function defaultPermissionDecision(record: PermissionRequestRecord): PermissionDecision {
    ...
    if (isExec) {
        ...
        if (innerToolName) {
            return isInsightsDestructiveSubTool(innerToolName) ? 'prompt' : 'auto_allow'
        }
