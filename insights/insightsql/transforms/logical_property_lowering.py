...
figure out. On Datastore a second pass (`datastore_property_resolution`) runs next and swaps the node for a faster column
...
- The numeric/boolean cast — on Datastore the swapper wraps the property in a cast before this pass runs, so the cast
  simply ends up wrapping the `PropertyAccess` and is carried through untouched.
- Access control — the node still points at the raw blob `Field`, which the Datastore printer drops restricted keys
...
from insights.insightsql.database.models import StringJSONDatabaseField
...
def lower_property_access(node: _T_AST, context: InsightsQLContext) -> _T_AST:
    """Lower JSON-blob property reads to `PropertyAccess`, including the within-non-InsightsQL (lightweight-DELETE) path."""
