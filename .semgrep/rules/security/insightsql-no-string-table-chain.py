"""Fixtures for insightsql-no-string-table-chain rule.
...
from insights.insightsql.database.schema.events import EVENTS_TABLE_TYPES, EventsTable
...
def bad_events_check(node: ast.JoinExpr) -> bool:
    # ruleid: insightsql-no-string-table-chain
...
def bad_qualified_check(field: ast.Field) -> bool:
    # ruleid: insightsql-no-string-table-chain
    return field.chain == ["insights", "events"]


def bad_ai_events_check(field: ast.Field) -> bool:
    # ruleid: insightsql-no-string-table-chain
    return field.chain == ["insights", "ai_events"]
...
def good_isinstance_check(context: InsightsQLContext, field: ast.Field) -> bool:
    # ok: insightsql-no-string-table-chain
...
def good_events_family_check(context: InsightsQLContext, field: ast.Field) -> bool:
    # ok: insightsql-no-string-table-chain
    if context.database is None:
        return False
    resolved = context.database.get_table([str(c) for c in field.chain])
...
def good_persons_isinstance(context: InsightsQLContext, field: ast.Field) -> bool:
    # ok: insightsql-no-string-table-chain
