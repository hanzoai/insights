...
def _stored_email_value(flow: InsightsFlow, from_draft: bool = False) -> dict:
...
class TestInsightsFlowActionEmailAPI(APIBaseTest):
    def setUp(self):
        super().setUp()
    ...
    def _create_flow(self, status: str = "draft", with_design: bool = True) -> str:
        ...
        create = self.client.post(f"/v1/projects/{self.team.id}/hog_flows", flow)
        assert create.status_code == 201, create.json()
        flow_id = create.json()["id"]
        if status != "draft":
            activate = self.client.patch(f"/v1/projects/{self.team.id}/hog_flows/{flow_id}", {"status": status})
    ...
    def _patch_email(self, flow_id: str, body: dict, action_id: str = "email_1", mcp: bool = True):
        url = f"/v1/projects/{self.team.id}/hog_flows/{flow_id}/actions/{action_id}/email"
        if mcp:
    ...
    def test_design_op_updates_block_and_rerenders_html(self, mock_render):
        ...
        email = _stored_email_value(InsightsFlow.objects.get(pk=flow_id))
    ...
    def test_design_moved_between_render_and_lock_rejected_with_409(self):
        ...
        def concurrent_edit_lands_during_render(design: dict) -> str:
            ...
            InsightsFlow.objects.filter(pk=flow_id).update(actions=flow.actions)
    ...
    def test_mcp_patch_composes_on_existing_draft(self, _mock_render):
        ...
        graph = self.client.patch(
            f"/v1/projects/{self.team.id}/hog_flows/{flow_id}/graph",
    ...
    def test_design_ops_compose_on_a_drafted_design(self, _mock_render):
        ...
        email = _stored_email_value(InsightsFlow.objects.get(pk=flow_id), from_draft=True)
...
class TestInsightsFlowEmailTemplateReference(APIBaseTest):
    def setUp(self):
        super().setUp()
        sync_template_to_db(_email_function_template())
    ...
    def _post_flow(self, email_config: dict, mcp: bool = True):
        ...
        url = f"/v1/projects/{self.team.id}/hog_flows"
        if mcp:
    ...
    def test_template_uuid_materializes_body_at_create_and_step_is_patchable(self, _mock_render):
        ...
        patch_response = self.client.patch(
            f"/v1/projects/{self.team.id}/hog_flows/{flow.pk}/actions/email_1/email",
    ...
    def test_uuid_in_template_id_is_coerced_and_then_materialized(self):
        ...
        assert response.status_code == 201, response.json()
        config = next(a for a in InsightsFlow.objects.get(pk=response.json()["id"]).actions if a["id"] == "email_1")[
            "config"
        ]
    ...
    def test_falsy_body_placeholders_do_not_clobber_template_content(self):
        ...
        assert response.status_code == 201, response.json()
        stored = _stored_email_value(InsightsFlow.objects.get(pk=response.json()["id"]))
        assert stored["subject"] == "Library subject"
        assert stored["html"] == "<p>Library html</p>"
    ...
    def test_materialized_template_content_is_capped_per_save(self):
        ...
        with patch(
            ...
            response = self.client.post(f"/v1/projects/{self.team.id}/hog_flows", flow, HTTP_X_INSIGHTS_CLIENT="mcp")
