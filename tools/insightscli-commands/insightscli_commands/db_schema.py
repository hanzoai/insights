"""Postgres schema restore helpers for insightscli."""
...
from insightscli_commands.github_auth import github_headers, github_token

ArtifactMode = Literal["off", "auto", "on"]

GITHUB_REPOSITORY = "Insights/insights"
...
def _find_repo_root() -> Path:
    ...
    for parent in current.parents:
        if (parent / "insightscli.yaml").exists():
            return parent
    return Path.cwd()
...
def _psql_admin(sql: str) -> None:
    ...
    _run([*DOCKER_COMPOSE, "exec", "-T", "db", "psql", "-U", "insights", "postgres", "-c", sql])
...
def _ensure_migration_defaults(target_db: str) -> None:
    _run(
        ...
        env={"DATABASE_URL": f"postgres://insights:insights@localhost:5432/{target_db}"},
    )
...
def is_database_empty(target_db: str) -> bool:
    ...
    result = subprocess.run(
        [*DOCKER_COMPOSE, "exec", "-T", "db", "psql", "-qAt", "-U", "insights", target_db, "-c", query],
        cwd=REPO_ROOT,
        env=os.environ.copy(),
        capture_output=True,
        text=True,
...
def _create_postgres_backup() -> Path:
    ...
    backup_file = backup_dir / f"insights_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql.gz"
    command = [*DOCKER_COMPOSE, "exec", "-T", "db", "pg_dumpall", "--clean", "-U", "insights"]
...
# (tools/insightscli/src/insightscli/command_types.py). YAML-backed children silently
# accept it via the insightscli registration wrapper; `click:` entries are loaded
# as-is, so each non-confirming command must declare a no-op --yes itself.
_insightscli_yes_compat = click.option(
    "--yes",
    "-y",
    is_flag=True,
    expose_value=False,
    hidden=True,
    help="Compatibility flag; accepted when forwarded by composite insightscli commands.",
)
...
def _recreate_test_db() -> None:
    try:
        restore_schema_dump(target_db=os.environ.get("TARGET_DB", "test_insights"), recreate=True, ensure_defaults=True)
    except Exception as exc:
        raise click.ClickException(str(exc)) from exc
...
@click.command(
    name="db:restore-schema", help="Fetch and restore the latest compatible schema into the local insights database"
)
...
def db_restore_schema(yes: bool) -> None:
    ...
    try:
        ...
        restore_schema_dump(target_db="insights", recreate=False, ensure_defaults=False)
        _run([str(REPO_ROOT / "bin" / "migrate")])
        _ensure_migration_defaults("insights")
    except Exception as exc:
        raise click.ClickException(str(exc)) from exc
