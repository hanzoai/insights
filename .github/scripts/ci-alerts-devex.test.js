...
function run(github, { history = [], now = minutes(0), env = {} } = {}) {
    ...
    return ciAlertsDevex(
        { context: { repo: { owner: 'Insights', repo: 'insights' } }, github, core },
...
describe('ci-alerts-devex', () => {
    ...
    for (const conclusion of ['failure', 'timed_out']) {
        it(`posts a new anchor + initial thread on 5 consecutive ${conclusion}`, async () => {
            ...
            assert.match(
                body,
                /https:\/\/us\.insights\.com\/project\/2\/engineering-analytics\/repos\/Insights\/insights\/actions\/workflows\/Backend%20CI\?q=master/
            )
    ...
    it('threads a reply when a new workflow joins the failing set', async () => {
        ...
        assert.match(thread, /engineering-analytics\/repos\/Insights\/insights\/actions\/workflows\/Frontend%20CI\?q=master/)
    })
    ...
    it('fetchWorkflowRuns reads the fresh index and drops non-terminal runs (regression)', async () => {
        ...
        const result = await ciAlertsDevex.fetchWorkflowRuns(github, 'Insights', 'insights', 'ci-backend.yml', 40)
