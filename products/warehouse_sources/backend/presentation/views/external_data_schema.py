...
from insights.api.log_entries import LogEntryRequestSerializer, LogEntrySerializer, fetch_log_entries
...
def source_supports_column_selection(source_type: str) -> bool:
    ...
    Excludes managed-schema sources (Stripe, Paddle, Zendesk): their InsightsQL tables expose a
    ...
    try:
        ...
        capture_exception(e)
        return False
    return not source.has_managed_insightsql_schema
...
class ExternalDataSchemaSerializer(UserAccessControlSerializerMixin, serializers.ModelSerializer):
    ...
    def get_table(self, schema: ExternalDataSchema) -> Optional[dict]:
        ...
        # Serializing table columns requires the full InsightsQL Database, which is expensive to build.
        ...
        if include_columns:
            ...
            if not insightsql_context:
                insightsql_context = Database.create_for(
                    team_id=self.context["team_id"],
                    user=cast(User, self.context["request"].user),
                )
            table_context["database"] = insightsql_context
    ...
    def update(self, instance: ExternalDataSchema, validated_data: dict[str, Any]) -> ExternalDataSchema:
        ...
        if "enabled_columns" in validated_data:
            ...
            if enabled_columns is not None:
                # Managed-schema sources expose a fixed canonical InsightsQL schema; dropping a
        ...
        if source.is_direct_query:
            ...
            # Warehouse mode: hide newly-disabled columns from InsightsQL immediately. Restoration
...
class ExternalDataSchemaViewset(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def check_object_permissions(self, request: Request, obj: Any) -> None:
        ...
        if request.method not in ("GET", "HEAD", "OPTIONS") and isinstance(obj, ExternalDataSchema):
            if obj.source.is_system_managed:
                raise PermissionDenied("This schema is managed by Insights and cannot be changed through this API.")
    ...
    def safely_get_queryset(self, queryset):
        ...
        # derives the dotted InsightsQL name from it), and `source` by `get_api_version_deprecation` for
