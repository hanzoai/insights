...
test.describe('Events', () => {
    test.beforeEach(async ({ page }) => {
        await page.route('/v1/event/values?key=%24browser', (route) =>
            route.fulfill({
                status: 200,
        ...
        await page.route('/v1/projects/@current/property_definitions/?limit=5000', (route) =>
            route.fulfill({
                status: 200,
        ...
        await page.route('/v1/projects/*/property_definitions?is_feature_flag=false&search=&*', (route) =>
            route.fulfill({
                status: 200,
                path: path.resolve(__dirname, '../mocks/events/property_definitions.json'),
            })
        ...
        await page.route('/v1/projects/*/property_definitions?is_feature_flag=false&search=%24time*', (route) =>
            route.fulfill({
                status: 200,
        ...
        await page.route('/v1/projects/*/property_definitions?is_feature_flag=false&search=%24browser*', (route) =>
            route.fulfill({
                status: 200,
        ...
        await page.route('/v1/projects/*/property_definitions?is_feature_flag=true*', (route) =>
            route.fulfill({
                status: 200,
        ...
        await page.route('/v1/event/values/?key=$browser_version', (route) =>
            route.fulfill({
                status: 200,
                body: JSON.stringify([{ name: '96' }, { name: '97' }]),
            })
    ...
    test.skip('Apply 1 overall filter', async ({ page }) => {
        ...
        await page.waitForResponse('/v1/event/values?key=%24browser')
