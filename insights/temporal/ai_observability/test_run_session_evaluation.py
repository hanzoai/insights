...
from insights.temporal.ai_observability.evaluation_payload import payload_budget_bytes
...
class TestCountSessionEvents:
    ...
    def test_never_falls_back_to_the_stripped_events_table(self):
        ...
        with patch(
            "insights.temporal.ai_observability.run_session_evaluation.query_ai_events",
...
class TestFetchSessionForEvaluation:
    def test_queries_in_evaluation_mode_with_both_date_bounds(self):
        with (
            patch("insights.temporal.ai_observability.run_session_evaluation.Team"),
            patch(
                "insights.temporal.ai_observability.run_session_evaluation._sum_session_payload_bytes",
                return_value=0,
            ),
            patch(
                "insights.temporal.ai_observability.run_session_evaluation._count_session_events",
                return_value=_SessionEventCount(event_count=3, first_seen=datetime(2026, 7, 19, tzinfo=UTC)),
            ),
            patch(
                "insights.temporal.ai_observability.run_session_evaluation.SessionQueryRunner"
    ...
    def test_floors_date_from_at_retention_for_a_session_older_than_retention(self):
        ...
        # lookback math, so the fetch must not ask Datastore for data it can never return.
        window_start = datetime(2026, 7, 20, tzinfo=UTC)
...
class TestExecuteSessionActivities:
    ...
    def test_judge_rejects_a_non_judge_evaluation(self):
        with pytest.raises(ApplicationError, match="Unsupported evaluation type"):
            execute_session_llm_judge_activity(
                ExecuteSessionEvaluationInputs(
                    evaluation={"evaluation_type": "script", "output_type": "boolean"},
                    team_id=1,
                    session_id="s-1",
                    window_start=datetime.now(UTC).isoformat(),
                )
    ...
    def test_judge_skips_without_judging_when_the_session_is_truncated(self):
        with (
            ...
            patch("insights.temporal.ai_observability.run_session_evaluation.call_llm_judge") as mock_call_llm_judge,
        ):
