...
from insights.insightsql_queries.ai.ai_table_resolver import AIEventsUnavailableError, query_ai_events
        ...
        "oneOf": [
            ...
            },
            {
                ...
                "properties": {
                    "source": {
                        "type": "string",
                        "description": "Script source code. Must return true (pass), false (fail), or null for N/A.",
                        "minLength": 1,
                    }
                },
                "additionalProperties": False,
...
class EvaluationSerializer(serializers.ModelSerializer):
    ...
    model_configuration = ModelConfigurationSerializer(
        ...
        help_text=(
            ...
            "omit this field to keep the current model; null is rejected. When switching an llm_judge to script or "
    ...
    evaluation_config = _EvaluationConfigField(
        required=False,
        help_text=(
            "Configuration dict. For 'llm_judge': {prompt}; for 'script': {source}; "
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "evaluation_type": {
                "help_text": (
                    "'llm_judge' uses an LLM to score outputs against a prompt; 'script' runs deterministic Script code; "
    ...
    def _validate_keyless_resolution(self, data: dict) -> None:
        ...
        `insights/temporal/ai_observability/model_resolution.py`). A pinned key, or an explicit
...
class EvaluationListSerializer(EvaluationSerializer):
    ...
    Gated on the ``X-Insights-Client: mcp`` header so the web UI keeps the full shape
...
class TestHogRequestSerializer(serializers.Serializer):
    source = serializers.CharField(
        required=True,
        min_length=1,
        help_text="Script source code to test. Must return a boolean (true = pass, false = fail) or null for N/A.",
...
class TestHogResultItemSerializer(serializers.Serializer):
    ...
    reasoning = serializers.CharField(allow_null=True, help_text="Script evaluation reasoning string, if any.")
...
def _test_hog_over_sessions(
    *,
    ...
    report_user_action(
        request.user,
        "llma evaluation script code tested",
        {
            "sample_count": sample_count,
...
class EvaluationViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def test_hog(self, request: Request, **kwargs) -> Response:
        ...
        try:
            ...
            logger.exception("Unexpected error compiling Script source")
        ...
        if target == EvaluationTarget.SESSION.value:
            return _test_hog_over_sessions(
                ...
                bytecode=bytecode,
                condition_filter=condition_filter,
                sample_count=min(sample_count, SESSION_TEST_FN_MAX_SAMPLES),
                allows_na=allows_na,
