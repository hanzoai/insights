...
from insights.api.oauth.cimd import (
...
from insights.api.oauth.client_name import sanitize_client_name
from insights.models.oauth import OAuthApplication, TokenEndpointAuthMethod, create_cimd_verification_token
from insights.scopes import OAUTH_SCOPES_HIDDEN, PRIVILEGED_SCOPES

VALID_CIMD_URL = "https://app.example.com/.well-known/oauth-client-metadata.json"


def _make_metadata(url: str = VALID_CIMD_URL, com_insights: dict | None = None, **overrides: object) -> dict:
    ...
    if com_insights is not None:
        metadata["com.insights"] = com_insights
...
def _document_fetches(mock_get, url: str = VALID_CIMD_URL) -> list:
    ...
    (the Datastore health ping) lands on the same mock, so counting raw calls would not
...
@patch("insights.api.oauth.cimd.is_url_allowed", return_value=(True, None))
...
def test_validate_cimd_url_ssrf_blocked(mock_return, url, expected_error):
    with patch("insights.api.oauth.cimd.is_url_allowed", return_value=mock_return):
        valid, error = validate_cimd_url(url, perform_dns_check=True)
        assert valid is False
        assert error == expected_error


@patch("insights.security.url_validation.resolve_host_ips", return_value={ip_address("93.184.216.34")})
...
class TestFetchAndUpsertCimdApplication(APIBaseTest):
    ...
    def test_blocked_name_uses_default(self, mock_get, _url_mock):
        metadata = _make_metadata(client_name="Insights Official Client")
        mock_get.return_value = _mock_response(metadata, headers={})
    ...
    def test_blocked_url_prevents_fetch(self, mock_get, _url_mock):
        from insights.api.oauth.cimd import block_cimd_url, unblock_cimd_url
...
class TestGetOrCreateCimdApplication(APIBaseTest):
    ...
    @patch("insights.api.oauth.cimd.cache")
    @patch("insights.api.oauth.cimd.requests.Session.get")
    def test_stale_cache_returns_immediately_and_queues_refresh(self, mock_get, mock_cache, _url_mock):
        ...
        with patch("insights.api.oauth.cimd.refresh_cimd_metadata_task") as mock_task:
    ...
    @patch("insights.api.oauth.cimd.capture_exception")
    @patch("insights.api.oauth.cimd.fetch_and_upsert_cimd_application")
...
class TestApplyProvisioningDefaults(APIBaseTest):
    ...
    @patch("insights.api.oauth.cimd.hanzo_insights.capture")
    @patch("insights.api.oauth.cimd.requests.Session.get")
...
class TestCIMDVerificationToken(APIBaseTest):
    ...
    def test_valid_verification_token_links_app_to_organization(self, mock_get, _url_mock):
        ...
        metadata = _make_metadata(insights_verification_token=plaintext)
        mock_get.return_value = _mock_response(metadata, headers={})
    ...
    def test_refresh_unlinks_app_when_token_removed(self, mock_get, _url_mock):
        ...
        mock_get.return_value = _mock_response(_make_metadata(insights_verification_token=plaintext), headers={})
        app = fetch_and_upsert_cimd_application(VALID_CIMD_URL)
        assert app is not None
        self.assertEqual(app.organization_id, self.organization.id)
    ...
    def test_non_string_verification_token_is_ignored(self, mock_get, _url_mock):
        metadata = _make_metadata()
        metadata["insights_verification_token"] = {"not": "a string"}
        mock_get.return_value = _mock_response(metadata, headers={})
...
class TestCIMDAuthorizeIntegration(APIBaseTest):
    ...
    def test_cimd_rate_limit_rejects_new_client(self, mock_get, _url_mock):
        ...
        with patch("insights.api.oauth.views.CIMD_THROTTLE_CLASSES", new=[mock_throttle_cls]):
...
@patch("insights.security.url_validation.resolve_host_ips", return_value={ip_address("93.184.216.34")})
class TestCIMDComInsightsNamespace(APIBaseTest):
    """Tests for the com.insights namespace: scopes and nested verification_token."""

    # (d) dual-read: both com.insights.verification_token and the legacy
    # insights_verification_token must link the app to the organization.
    @parameterized.expand(
        [
            ("nested",),
            ("top_level",),
    ...
    def test_verification_token_dual_read(self, token_placement, mock_get, _url_mock):
        ...
        if token_placement == "nested":
            metadata = _make_metadata(com_insights={"verification_token": plaintext})
        else:
            metadata = _make_metadata(insights_verification_token=plaintext)
    ...
    def test_scopes_written_to_app_on_creation(self, _name, input_scopes, expected_scopes, mock_get, _url_mock):
        metadata = _make_metadata(com_insights={"scopes": input_scopes})
        mock_get.return_value = _mock_response(metadata, headers={})
    ...
    def test_duplicate_scopes_deduped(self, mock_get, _url_mock):
        metadata = _make_metadata(com_insights={"scopes": ["insight:read", "dashboard:write", "insight:read"]})
        mock_get.return_value = _mock_response(metadata, headers={})
    ...
    # (b) absent com.insights.scopes on refresh leaves existing scopes untouched.
    @patch("insights.api.oauth.cimd.requests.Session.get")
    def test_absent_scopes_on_refresh_leaves_existing_untouched(self, mock_get, _url_mock):
        metadata_create = _make_metadata(com_insights={"scopes": ["insight:read"]})
        mock_get.return_value = _mock_response(metadata_create, headers={})
        fetch_and_upsert_cimd_application(VALID_CIMD_URL)

        real_cache.delete(_fetch_lock_key(VALID_CIMD_URL))
        # Refresh with metadata that has no com.insights.scopes.
        mock_get.return_value = _mock_response(_make_metadata(), headers={})
        refreshed = fetch_and_upsert_cimd_application(VALID_CIMD_URL)
    ...
    def test_present_scopes_on_refresh_override_existing(self, mock_get, _url_mock):
        metadata_create = _make_metadata(com_insights={"scopes": ["insight:read", "dashboard:write"]})
        mock_get.return_value = _mock_response(metadata_create, headers={})
        fetch_and_upsert_cimd_application(VALID_CIMD_URL)

        real_cache.delete(_fetch_lock_key(VALID_CIMD_URL))
        metadata_refresh = _make_metadata(com_insights={"scopes": ["survey:read"]})
    ...
    # com.insights.optional_scopes carries the required/optional split: required `scopes` and the
    ...
    def test_optional_scopes_written_to_app_on_creation(self, mock_get, _url_mock):
        metadata = _make_metadata(
            com_insights={"scopes": ["insight:read"], "optional_scopes": ["dashboard:read", "llm_gateway:read"]}
        )
        mock_get.return_value = _mock_response(metadata, headers={})
    ...
    def test_optional_scopes_refresh_together_with_scopes(self, mock_get, _url_mock):
        mock_get.return_value = _mock_response(
            _make_metadata(com_insights={"scopes": ["insight:read"], "optional_scopes": ["dashboard:read"]}), headers={}
        )
        fetch_and_upsert_cimd_application(VALID_CIMD_URL)

        real_cache.delete(_fetch_lock_key(VALID_CIMD_URL))
        mock_get.return_value = _mock_response(
            _make_metadata(com_insights={"scopes": ["survey:read"], "optional_scopes": ["experiment:read"]}), headers={}
        )
        refreshed = fetch_and_upsert_cimd_application(VALID_CIMD_URL)
    ...
    def test_refresh_cannot_grant_non_grantable_scopes(self, mock_get, _url_mock):
        mock_get.return_value = _mock_response(_make_metadata(com_insights={"scopes": ["insight:read"]}), headers={})
        fetch_and_upsert_cimd_application(VALID_CIMD_URL)
        ...
        mock_get.return_value = _mock_response(_make_metadata(com_insights={"scopes": escalated}), headers={})
        refreshed = fetch_and_upsert_cimd_application(VALID_CIMD_URL)
    ...
    # absent com.insights.scopes on initial creation → empty scopes list.
    @patch("insights.api.oauth.cimd.requests.Session.get")
    ...
    # A present, non-empty com.insights.scopes that strips to nothing is rejected, not
    ...
    def test_all_non_grantable_scopes_on_creation_rejected(self, mock_get, _url_mock):
        ...
        mock_get.return_value = _mock_response(_make_metadata(com_insights={"scopes": only_non_grantable}), headers={})
...
class TestResolveScopes(SimpleTestCase):
    ...
    def test_absent_or_malformed_field_returns_none(self) -> None:
        ...
        self.assertIsNone(_resolve_scopes({"com.insights": {}}))
        # Malformed partner JSON: a non-list scopes value hits the runtime guard and returns None.
        self.assertIsNone(_resolve_scopes(cast(CIMDMetadataDocument, {"com.insights": {"scopes": "not-a-list"}})))
    ...
    def test_explicit_empty_list_is_use_default(self) -> None:
        ...
        self.assertEqual(_resolve_scopes({"com.insights": {"scopes": []}}), [])

    def test_partial_strip_keeps_grantable(self) -> None:
        resolved = _resolve_scopes({"com.insights": {"scopes": ["insight:read", "llm_gateway:read"]}})
        self.assertEqual(resolved, ["insight:read"])

    def test_all_non_grantable_raises(self) -> None:
        with self.assertRaises(CIMDValidationError):
            _resolve_scopes({"com.insights": {"scopes": ["llm_gateway:read", "not_a_real_scope:write"]}})
