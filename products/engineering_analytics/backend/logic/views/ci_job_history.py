...
never dropped when its run row is missing. Datastore fills the unmatched run side with type
defaults (empty repo/sha, ``pr_number`` 0), not NULL — InsightsQL doesn't set ``join_use_nulls`` — so a
...
Nothing here is registered as a global InsightsQL view; it is provisioned per-team as a non-materialized
``DataWarehouseSavedQuery`` by data_modeling's managed-viewset sync (kind ``engineering_analytics``).
"""

from typing import TYPE_CHECKING

from insights.insightsql.database.models import DateTimeDatabaseField, FieldOrTable, IntegerDatabaseField, StringDatabaseField
