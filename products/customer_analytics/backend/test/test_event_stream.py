...
class TestEventStreamViewSet(APIBaseTest):
    ...
    def _destination(self, stream: dict) -> InsightsFunction:
        insights_function_id = EventStream.objects.unscoped().get(id=stream["id"]).insights_function_id
        assert insights_function_id is not None
        return InsightsFunction.objects.get(id=insights_function_id)

    def _group_filter(self, function: InsightsFunction) -> dict:
        return (function.filters or {})["properties"][0]

    def _inputs(self, function: InsightsFunction) -> dict:
        return function.inputs or {}

    def _filtered_events(self, function: InsightsFunction) -> list[dict]:
    ...
    def test_create_without_slack_defers_destination_until_channel_is_set(self):
        ...
        self.assertIsNone(EventStream.objects.unscoped().get(id=stream["id"]).insights_function_id)
    ...
    def test_deleting_owner_deletes_stream(self):
        stream = self._create_stream()
    ...
    def test_list_is_team_scoped(self):
        ...
        other_org = Organization.objects.create(name="other org")
        other_team = Team.objects.create(organization=other_org, name="other team")
        other_owner = self._create_user("other-owner@hanzo.ai")
