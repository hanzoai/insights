...
from insights.event_usage import groups, report_team_action
...
def resolve_insights_user_for_slack(email: str | None, team: Team) -> User | None:
    """Match a Slack user's email to a Insights user within the team's organization."""
    if not email:
        return None
    membership = (
...
def create_or_update_slack_ticket(
    *,
    ...
    # Check if this Slack user is a Insights team member
    insights_user = resolve_insights_user_for_slack(user_info.get("email"), team)
    is_team_member = insights_user is not None
...
def _record_last_slack_message(
    ...
    configured support channels. Bots and Insights teammates aren't customers, so their messages
    ...
    try:
        ...
        if not email or resolve_insights_user_for_slack(email, team):
            return
...
def _nudge_classifier_verdict(
    ...
    from insights.llm.gateway_client import get_llm_client  # noqa: PLC0415 — keeps heavy SDKs off the setup path
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            extra_headers={
                "x-insights-property-feature": NUDGE_CLASSIFIER_FEATURE,
                "x-insights-property-$ai_span_name": NUDGE_CLASSIFIER_FEATURE,
                "x-insights-property-slack_channel_id": channel,
                "x-insights-property-slack_thread_ts": message_ts,
            },
            messages=[
...
def _should_send_nudge(
    ...
    if not settings.DEBUG:
        user_info = resolve_slack_user(client, slack_user_id)
        if resolve_insights_user_for_slack(user_info.get("email"), team):
            return NudgeDecision(send=False, classifier_verdict="skipped")
...
def _backfill_thread_replies(
    ...
    insights_user_cache: dict[str, User | None] = {}
    ...
    for reply in thread_replies:
        ...
        if reply_user not in insights_user_cache:
            insights_user_cache[reply_user] = resolve_insights_user_for_slack(user_info.get("email"), team)
        insights_user = insights_user_cache[reply_user]
        is_team_member = insights_user is not None
...
def _track_bot_joined_channel(event: dict, team: Team, slack_team_id: str, *, own_bot_user_id: str | None) -> None:
    """Fire an internal Insights event when our own bot is added to a channel.

    Unlike the human join/leave alerts, this isn't gated on any team setting — it's usage
    analytics for Insights, not a customer-facing Slack message. Only the bot's own join is
...
def _handle_member_event(
    ...
    *,
    ...
    if resolve_insights_user_for_slack(slack_user.get("email"), team):
        return
