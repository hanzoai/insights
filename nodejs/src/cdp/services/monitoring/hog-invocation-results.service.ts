...
const isInsightsFlowInvocation = (invocation: CyclotronJobInvocation): invocation is CyclotronJobInvocationInsightsFlow =>
...
const microsecondsSinceEpoch = (): string => {
    // BigInt avoids the 53-bit cap so the number lines up with Datastore UInt64.
...
const isoMicroseconds = (date: Date): string => {
    // Datastore DateTime64(6) accepts 'YYYY-MM-DD HH:MM:SS.ffffff'.
...
// in at execute time. Persisting them to Datastore for 30 days would expand
// the blast radius of any leak. On rerun we re-resolve inputs from the
// current script function config + integration store, so this is also a no-op
...
// rehydrates `groups`/`person` (loadInsightsFunctions) and the executor rebuilds
...
// this field is opaque — so the Datastore Kafka engine still parses the
 ...
 * `invocation_globals` back off Datastore. Rows written before field
...
const sumDurationMs = (invocation: CyclotronJobInvocation): number | null => {
    ...
    // Per-step timings are fractional ms (perf.now() deltas). Datastore stores
 ...
 * `InsightsFunctionMonitoringService` (which handles aggregated metrics + log
...
export class HogInvocationResultsService {
    ...
    private buildLifecycleRow(
        ...
        // Both script function and script flow state carry a `rerunAttempts`
     ...
     * blob goes in `invocation_globals` (never exposed via InsightsQL — same
