...
description: >
  Investigate logs in a Insights project: verify a service or deployment is healthy, explain an error spike,
...
One `insights:logs-patterns` call compresses millions of lines into at most 200 templates,
and one `insights:logs-patterns-diff` call answers "what is different about now vs. before" directly.
Raw rows (`insights:query-logs`) are the last step of an investigation, never the first.
...
- Analytics over product events, persons, or insights — that's `querying-insights-data`.
- InsightsQL exposes a `logs` table via `insights:execute-sql`, but do not investigate through it:
...
2. Orient with `insights:logs-services-create`: is the service still logging at all, and what is its error_rate now?
   A service that went silent fails verification just as hard as one that started erroring.
3. `insights:logs-patterns-diff` with `query.dateRange` from the change time to now and `baselineDateRange`
...
4. Check volume continuity with `insights:logs-count-ranges` spanning before and after the boundary:
   a rate discontinuity (crash loop, restart storm, silence) shows up here even when message content looks unchanged.
5. Drill only the suspects: pivot each suspicious pattern to raw lines via its `match_regex` with `insights:query-logs`.
...
1. Localize it: `insights:logs-count-ranges` over the user's window, then recurse into the dense bucket(s) — each bucket's
   `date_from`/`date_to` feeds the next call. Stop after 3–4 levels.
2. Explain it: `insights:logs-patterns-diff` with the spike as `query.dateRange` and the window just before as
...
`insights:logs-patterns-diff` first: incident window vs. a known-good window just before (or omit the baseline for
same-window-last-week). Suspects are `new` entries and the biggest `rate_ratio` shifts; pivot each to raw lines.
If the failing service is unknown, find it first with `insights:logs-facet-values-create` faceting `service_name`
under `severityLevels: ["error", "fatal"]`.

### "What is this stream saying?" — unfamiliar service

`insights:logs-patterns` over the last hour, scoped to the service. Scan templates by `estimated_count` and
...
discover the right keys with `insights:logs-attributes-list` / `insights:logs-attribute-values-list`,
size the result with `insights:logs-count`, then pull rows with `insights:query-logs`.
...
- `insights:query-logs` requires an explicit `query.dateRange` — omitting it is a 400, not a default window.
...
- `severityLevels` matches the six canonical lowercase buckets against `severity_text` exactly.
  Zero rows on a severity filter → check the stored values with `insights:logs-attribute-values-list { key: "severity_text" }`.
