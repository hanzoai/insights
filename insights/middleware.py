...
from insights.helpers.user_devices import set_known_device_cookie
from insights.models import Team, User
from insights.models.activity_logging.utils import (
...
API_REQUESTS_LATENCY_SECONDS = Histogram(
    "insights_api_requests_latency_seconds",
...
def per_request_logging_context_middleware(
    ...
    def middleware(request: HttpRequest) -> HttpResponse:
        ...
        # that do not use Host header app.hanzo.ai. This is important as we
        # roll out CloudFront in front of app.hanzo.ai. We can get the host
        ...
        # Forwarded by the Insights MCP server (services/mcp) on every API call.
        ...
        # default (see `insights/otel_instrumentation.py`), so the span is in
        # scope here. If that invariant ever changes, `set_attribute` becomes a
        # silent no-op on the non-recording span.
        mcp_session_id = sanitize_header_value(request.headers.get("X-Insights-Mcp-Session-Id"))
        mcp_conversation_id = sanitize_header_value(request.headers.get("X-Insights-Mcp-Conversation-Id"))
...
class InsightsTokenCookieMiddleware(MiddlewareMixin):
    """
...
class SessionAgeMiddleware:
    ...
    def __call__(self, request: HttpRequest):
        ...
        # but the modifications were silently dropped by InsightsTokenCookieMiddleware's session
    ...
    def __call__(self, request):
        ...
        if is_admin_view:
            ...
            csp_parts = [
                ...
                f"font-src 'self' {resource_url} https://app-static.eu.hanzo.ai https://app-static-prod.hanzo.ai https://d1sdjtjk6xzm7.cloudfront.net https://fonts.gstatic.com https://cdn.jsdelivr.net https://assets.faircado.com https://use.typekit.net",
                "worker-src 'self'",
                "child-src 'none'",
                "object-src 'none'",
...
READ_ONLY_IMPERSONATION_ALLOWLISTED_PATHS: list[str | re.Pattern] = [
    ...
    # `insights.scopes.downgrade_scopes_to_read_only` in `OAuthAuthorizationView` so the
...
def impersonated_session_logout(request: HttpRequest) -> HttpResponse:
    ...
    provided (e.g. `/` to return to the Insights app), otherwise back to the
    ...
    restore_original_login(request)
    return redirect(safe_next or f"/admin/insights/user/{impersonated_user_pk}/change/")
