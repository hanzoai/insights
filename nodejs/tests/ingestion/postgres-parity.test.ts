...
/**
 * Waits until Datastore's Kafka engine is consuming the person topic, by producing probe
 ...
 * Datastore's background scheduler and intermittently starves the person consumer this suite
 ...
 */
async function waitForDatastorePersonConsumer(datastore: Datastore): Promise<void> {
    await ensureKafkaTopics(await datastore.getKafkaEngineTopics(), extraServerConfig)
...
describe('postgres parity', () => {
    jest.retryTimes(1) // Reduced from 5 to limit timeout amplification when kafka/datastore is degraded
    ...
    beforeAll(async () => {
        ...
        // topics causes Datastore StorageKafka consumers to lose partition
        ...
        await waitForDatastorePersonConsumer(datastore)
    })
    ...
    beforeEach(async () => {
        ...
        // This provides both PostgreSQL and Datastore isolation without truncating
    ...
    test('createPerson', async () => {
        ...
        await datastore.delayUntilEventIngested(() => datastore.fetchPersons(teamId))
