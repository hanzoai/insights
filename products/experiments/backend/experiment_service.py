...
from insights.api.cohort import CohortSerializer, get_active_flags_using_cohort
from insights.api.utils import ServiceRequest
from insights.datastore.query_tagging import Feature, Product, tags_context
from insights.event_usage import EventSource, report_user_action
...
from insights.models.activity_logging.utils import get_changed_fields_local
from insights.models.filters.filter import Filter
from insights.models.person.util import get_person_ids_and_uuids_by_uuids
...
from products.experiments.backend.insightsql_queries import CONTROL_VARIANT_KEY, get_baseline_variant_key
from products.experiments.backend.insightsql_queries.base_query_utils import is_threshold_supported_math
from products.experiments.backend.insightsql_queries.experiment_metric_fingerprint import compute_metric_fingerprint
from products.experiments.backend.insightsql_queries.exposure_query_logic import (
    build_exposure_event_conditions,
    get_exposure_event_and_property,
)
from products.experiments.backend.insightsql_queries.funnel_validation import FunnelDWValidator
...
# Feature flag (in Insights's internal project) gating which teams auto-open flag-cleanup PRs when an
...
# time spent scanning $feature_flag_called events (Datastore) and the number of exposed users we
...
# Concurrent personinsights batch lookups while resolving the exposed set. Deliberately modest: each
...
class ExperimentService:
    ...
    def validate_metric_event_names(self, metrics: list[dict] | None) -> None:
        ...
        project-scoped (see insights/api/event_definition.py), so a user in a
        ...
        # mirrors the picker SQL in insights/api/event_definition.py so sibling
    ...
    def create_experiment(
        ...
        *,
        ...
        # doesn't hold insights_experiment / insights_filesystem locks open across an external SDK call.
    ...
    # Datastore and Postgres (non-transactional side effects), so rather than rely on rollback we
    ...
    def freeze_exposure(self, experiment: Experiment, *, request: Any) -> Experiment:
        ...
        runs the guards to fail fast, then the expensive Datastore scan and cohort build.
        ...
        """
        # Phase timings are logged at the end: the freeze spans several backends (Datastore scan,
        # personinsights RPCs, cohort build with its cache side effects, flag save) whose relative cost
        ...
        # Separate checkpoint so scan_ms measures only the Datastore scan — the guard above can
        ...
        # aren't fetched from personinsights a second time.
        ...
        try:
            with transaction.atomic():
                ...
                # products/approvals/backend/actions/feature_flags.py and hanzo.ai/docs/settings/approvals.
        ...
        try:
            with tags_context(
                ...
                response = execute_insightsql_query(
                    query,
                    team=self.team,
                    settings=InsightsQLGlobalSettings(max_execution_time=FREEZE_EXPOSURE_QUERY_TIMEOUT_SECONDS),
            ...
            # limit, and Datastore's own pre-execution time estimate — and get the same
    ...
    def _resolve_exposed_persons(self, person_uuids: list[str]) -> list[tuple[int, str]]:
        ...
        share one (field-masked) personinsights pass instead of fetching the persons twice. Membership
        ...
        """
        # Opt into the concurrent personinsights batch fan-out: the exposed set is large (up to
    ...
    def _create_exposure_snapshot_cohort(self, experiment: Experiment, id_uuid_pairs: list[tuple[int, str]]) -> Cohort:
        ...
        _resolve_exposed_persons so the insert doesn't re-fetch the persons from personinsights.
        """
    ...
    def _cleanup_pr_flag_enabled(self) -> bool:
        # Our backend's hanzo_insights client points at Insights's own internal project, so we gate a
    ...
    def _report_experiment_ended(
        ...
        *,
        ...
        # previously cached results, never triggers a Datastore query or
    ...
    def update_experiment(
        ...
        *,
        ...
        # write below. Insights has no ATOMIC_REQUESTS, so _sync_feature_flag_on_update
            ...
            if status:
                ...
                if status_enum and status_enum != ExperimentQueryStatus.ALL:
                    ...
                    all_groups_stamped = RawSQL(
                        """EXISTS (
                            SELECT 1 FROM insights_featureflag ff
                            WHERE ff.id = insights_experiment.feature_flag_id
