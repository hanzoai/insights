...
use personinsights_common::async_gzip::{AsyncGzipConfig, AsyncGzipLayer};
use personinsights_common::grpc::{tracked_tcp_incoming, GrpcLoadShedLayer, GrpcMetricsLayer};
use personinsights_coordination::authority::AuthorityClock;
use personinsights_coordination::pod::{PodConfig, PodHandle};
use personinsights_coordination::store::PersonhogStore;
use personinsights_proto::personinsights::leader::v1::person_hog_leader_server::PersonHogLeaderServer;
...
use personinsights_leader::cache::{DirtyIndex, PartitionedCache};
use personinsights_leader::config::Config;
use personinsights_leader::coordination::LeaderHandoffHandler;
use personinsights_leader::fencing::{
    preregister_fencing_metrics, FencedChangelogProducers, FencedProducerConfig,
};
use personinsights_leader::inflight::InflightTracker;
use personinsights_leader::pg::{validate_table_name, PgFallback};
use personinsights_leader::recovery::{ChangelogRecovery, RecoveryConfig};
use personinsights_leader::service::{sweep_idle_locks, PersonHogLeaderService, PropertySizeLimits};
use personinsights_leader::warming::{
    fetch_writer_committed_offsets, WarmClientPools, WarmingConfig, WarmingRetryPolicy,
};
use personinsights_leader::warnings::WarningsProducer;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // panics. Matches personinsights-router / cymbal / ingestion-consumer.
    ...
    tracing::info!("Starting personinsights-leader service");
    ...
    let mut manager = Manager::builder("personinsights-leader")
    ...
    for reason in ["surrendered", "stale"] {
        counter!(
            "personinsights_leader_authority_lapsed_rejections_total",
            "reason" => reason
        )
        .increment(0);
        for phase in ["warm", "resume"] {
            counter!(
                "personinsights_leader_authority_lapsed_acquires_total",
                "phase" => phase,
                "reason" => reason
            )
            .increment(0);
    ...
    for phase in ["warm", "resume"] {
        counter!(
            "personinsights_leader_authority_lapsed_mid_acquire_total",
            "phase" => phase
        )
        .increment(0);
    }
    counter!("personinsights_leader_unresolved_versions_total").increment(0);
    counter!("personinsights_leader_unresolved_versions_spilled_total").increment(0);
    gauge!("personinsights_leader_unresolved_versions").set(0.0);
    ...
    let fallback = if config.fallback_database_url.is_empty() {
        ...
        let pool_config = common_database::PoolConfig {
            ...
            pool_name: Some("personinsights-leader-fallback".to_string()),
    ...
    let emitted_versions = Arc::new(personinsights_leader::emitted::EmittedVersions::new(
    ...
    // margin.
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                gauge!("personinsights_leader_authority_valid").set(if authority.is_valid() {
                    1.0
                } else {
                    0.0
                });
                gauge!("personinsights_leader_authority_age_ms")
                    .set(authority.since_confirmed().as_secs_f64() * 1000.0);
                gauge!("personinsights_leader_authority_margin_ms")
    ...
    let advertise_address =
        personinsights_leader::config::derive_advertise_address(&config.grpc_address, &config.pod_ip)
...
async fn run_dirty_index_prune_loop(
    ...
    loop {
        ...
        gauge!("personinsights_leader_dirty_index_size").set(dirty_index.len() as f64);
        gauge!("personinsights_leader_dirty_index_max_entries").set(dirty_index.max_entries() as f64);
        gauge!("personinsights_leader_cache_weight_bytes").set(cache.usage_bytes() as f64);
        ...
        if pruned > 0 {
            counter!("personinsights_leader_dirty_index_pruned_total").increment(pruned as u64);
        }
        ...
        for partition in &partitions {
            ...
            gauge!(
                "personinsights_leader_writer_uncommitted_offsets",
...
fn preregister_metrics() {
    for fenced in ["true", "false"] {
        counter!("personinsights_leader_indeterminate_outcomes_total", "fenced" => fenced).increment(0);
    }
    counter!("personinsights_leader_unresolved_versions_total").increment(0);
    gauge!("personinsights_leader_unresolved_versions").set(0.0);
    counter!("personinsights_leader_warmed_messages_total").increment(0);
    counter!("personinsights_leader_warm_retries_exhausted_total", "stage" => "committed_offset")
        .increment(0);
    counter!("personinsights_leader_warm_retries_exhausted_total", "stage" => "fetch_watermarks")
        .increment(0);
    for stage in ["committed_offset", "fetch_watermarks"] {
        counter!("personinsights_leader_warm_retries_total", "stage" => stage).increment(0);
    }
}
