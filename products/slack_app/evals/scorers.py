...
from products.insights_ai.eval_harness.scorers import BINARY_CHOICE_SCORES, JUDGE_MODEL, JudgedScorer
from products.insights_ai.eval_harness.scorers.contract import Score
...
EXPECTED_KEY = "insights_product_answer"
...
class AnsweredAboutInsightsProduct(JudgedScorer):
    """Binary: does the answer explain Insights's own product for the capability asked about?

    Opts in via ``expected={"insights_product_answer": {"capability": "..."}}``.
    """
    ...
    def __init__(self, **kwargs):
        super().__init__(
            name="insights_product_answer",
            prompt_template="""
You are evaluating an answer the Insights Slack app gave to a customer asking how to set something up.

Insights ships its own product for the capability below, so the answer should be about that product — what to enable, install, or configure inside Insights.
...
Does the answer explain how to do this with Insights's own product? An answer that walks through Insights setup steps, names the Insights product or its settings, or links Insights docs for it qualifies — even if some details are wrong. An answer that only discusses instrumenting the customer's own code, only asks clarifying questions, or is about some other tool does NOT qualify. Answer `yes` or `no`.
""".strip(),
...
class NoThirdPartyRecommendation(JudgedScorer):
    """Binary: does the answer avoid steering the customer to a non-Insights product?

    This is the regression that prompted the suite — the agent recommending an
    external tool for a capability Insights already sells. Opts in via the same
    ``expected={"insights_product_answer": {"capability": "..."}}`` entry.
    """
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are checking whether an answer from the Insights Slack app sent a customer to a competitor.

Insights ships its own product for the capability below, so recommending someone else's product for it is a failure — the customer is already paying for ours.
