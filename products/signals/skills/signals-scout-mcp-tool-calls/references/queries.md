...
- Filter `properties.$mcp_source = 'insights_mcp_analytics'` — keeps SDK-instrumented events
  (both Insights's hono server and external customer servers), excludes pre-SDK legacy events.
...
- **Presence check = `isNotNull(properties.X)`.** This is the one reliable way to test whether an
  ...
  both resolve unreliably in InsightsQL for the MCP props (verified: they returned >100% coverage and
...
- **`$mcp_error_type` is quirky — never do value equality on the bare property.** It gives
  ...
  - **Classified failures (positive membership):**
    ...
    On Insights's own data only ~4% of failures are classified.
...
- The `$mcp_exec_tool_call_name` fallback is genuinely empty/NULL when absent, so the coalesce above is
  correct as written.
- **`$mcp_error_message` does not exist on Insights's own (hono) data** — it's an external-SDK-only field.
...
WHERE event = '$mcp_tool_call'
    AND properties.$mcp_source = 'insights_mcp_analytics'
...
- `pct_failures_classified` high → **hono regime with useful classes**: use query 3a. But don't assume
  this is high just because you're on Insights's own data — most `$mcp_is_error` failures are _tool-result_
  errors (the handler returned `{isError:true}` gracefully) which never get classified, so `error_type`
  stays `'None'`. On Insights's own project only ~4% of failures carry a real class. When
...
- `pct_with_category` ≥ ~50 → **per-category report grain** (query 9 is the aggregation layer). Hono
  ...
  (verified: Insights's own project sits at ~72%). ~0 → external-SDK regime, fall back to the
  per-tool report grain.
- `pct_with_intent` ≥ ~20 → intent lens (query 5) is worth running. (On Insights's own data this is ~100%.)
...
"no detail" marker is `error_type IN ('', 'None')` **and** no message — on Insights's own data
...
them together. Validated against real telemetry (the tuple `groupArrayIf` works in InsightsQL; keep
...
- The `Uncategorized` bucket is dominated by bare `exec` rows (discovery verbs, wrapper
  ...
  sanity-check, not an owning team (verified: on Insights's own project it is exactly those two
