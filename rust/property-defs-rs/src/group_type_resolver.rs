...
use crate::{
    config::Config,
    metrics_consts::{
        GROUP_TYPE_CACHE, PERSONFN_ERRORS_TOTAL, PERSONFN_RESOLVE_DURATION,
        PERSONFN_RESOLVE_ERRORS, PERSONFN_RETRIES_TOTAL, PERSONFN_TERMINAL_ERRORS_TOTAL,
    },
...
///
/// Emits `personinsights_errors_total` on every failed attempt, `personinsights_retries_total`
/// on each retry, and `personinsights_terminal_errors_total` when giving up.
        ...
        match make_call().await {
            ...
            Err(status) => {
                ...
                if !is_retryable(status.code()) || attempt == max_retries {
                    metrics::counter!(
                        PERSONFN_TERMINAL_ERRORS_TOTAL,
                        "method" => method,
                        "client" => client,
                ...
                warn!(
                    ...
                    error = %status,
                    "Retrying after transient personinsights error"
                );
...
pub struct GroupTypeResolver {
    ...
    personinsights_client: Option<PersonHogServiceClient<Channel>>,
    max_retries: u32,
    initial_backoff_ms: u64,
    max_backoff_ms: u64,
}
...
impl GroupTypeResolver {
    pub fn new(config: &Config) -> Self {
        ...
        let personinsights_client = if !config.personinsights_addr.is_empty() {
            let timeout = std::time::Duration::from_millis(config.personinsights_timeout_ms);
            let connect_timeout =
                std::time::Duration::from_millis(config.personinsights_connect_timeout_ms);
            match Channel::from_shared(config.personinsights_addr.clone()) {
                Ok(endpoint) => {
                    let channel = endpoint
                        ...
                        .connect_lazy();
                    info!(
                        addr = %config.personinsights_addr,
                        timeout_ms = config.personinsights_timeout_ms,
                        connect_timeout_ms = config.personinsights_connect_timeout_ms,
                        max_retries = config.personinsights_max_retries,
                        "Created personinsights gRPC client"
                    );
                ...
                Err(e) => {
                    warn!(
                        addr = %config.personinsights_addr,
                        error = %e,
                        "Failed to create personinsights gRPC channel"
                    );
                    None
                }
            }
        ...
        Self {
            ...
            max_retries: config.personinsights_max_retries,
            initial_backoff_ms: config.personinsights_initial_backoff_ms,
            max_backoff_ms: config.personinsights_max_backoff_ms,
        }
    }

    /// Returns true if `cache_key` has a live (unexpired) resolution-miss entry. Expired
    /// entries are removed so the next lookup re-attempts resolution via personinsights.
    ...
    pub async fn resolve(&self, updates: &mut [Update]) -> Result<(), anyhow::Error> {
        ...
        for (idx, update) in updates.iter_mut().enumerate() {
            ...
            if let Some(index) = self.cache.get(&cache_key) {
                ...
                // Known-unresolvable within the TTL window: skip the personinsights lookup and
        ...
        // Batch resolve all uncached group types via personinsights
        if !to_resolve.is_empty() {
            let resolved_map = match self.resolve_via_personinsights(&to_resolve).await {
                Ok(map) => map,
                Err(e) => {
                    let error_type = e
                        .downcast_ref::<Status>()
                    ...
                    warn!(error = %e, error_type = %error_type, "personinsights group type resolution failed");
                    metrics::counter!(PERSONFN_RESOLVE_ERRORS).increment(1);
                    return Err(e);
                }
            };
            ...
            for (idx, group_name, team_id) in to_resolve {
                ...
                if let Some(&index) = resolved_map.get(&(group_name.clone(), team_id)) {
                    ...
                    // Record the miss so repeated unresolvable keys don't re-hit personinsights for
    ...
    async fn resolve_via_personinsights(
        &self,
        ...
        let client = self
            .personinsights_client
            .clone()
            .ok_or_else(|| anyhow::anyhow!("personinsights client not configured"))?;
        ...
        metrics::histogram!(PERSONFN_RESOLVE_DURATION).record(elapsed_ms);
