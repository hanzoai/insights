...
POST (mirroring Insights Desktop's agent-server). The kernel-server fetches the
node's capped result page from the data-plane endpoint (real Datastore data via
InsightsQL) and POSTs the envelope back to the token-authed callback endpoint. The
...
hardening swaps them for the RS256 sandbox event-ingest JWTs used by Insights Desktop.
"""
...
# (no Datastore re-query); paging beyond it falls back to a LIMIT/OFFSET re-query.
...
def is_sql_v2_enabled(user: User | None) -> bool:
    ...
    return bool(hanzo_insights.feature_enabled(REVAMPED_PY_NOTEBOOKS_FLAG, user.distinct_id, **kwargs))
...
def dispatch_sql_v2_run(
    ...
    a insightsql node keeps the flat `code` the capped-fetch path reads — the paths stay additive.
    """
...
def fetch_sql_v2_page(notebook: Notebook, user: User | None, run: NotebookNodeRun, offset: int, limit: int) -> dict:
    ...
    A insightsql run pages by re-querying its stored code through the data plane with
    LIMIT/OFFSET; a kernel run (python/duckdb) pages by slicing its on-sandbox result
    frame, keyed by `result_id` — its code is not a InsightsQL query, so no data-plane
