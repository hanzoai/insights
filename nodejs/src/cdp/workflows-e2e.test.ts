...
const CYCLOTRON_NODE_DB_URL =
    process.env.CYCLOTRON_NODE_DATABASE_URL ?? 'postgres://insights:insights@localhost:5432/test_cyclotron_node'
const CYCLOTRON_DB_URL =
    process.env.CYCLOTRON_DATABASE_URL ?? 'postgres://insights:insights@localhost:5432/test_cyclotron'
...
describe.each(['postgres-v2' as const, 'postgres' as const])('Workflows E2E (%s)', (mode) => {
    ...
    beforeEach(async () => {
        ...
        if (mode === 'postgres-v2') {
            ...
            flowQueue = new CyclotronJobQueuePostgres(hub.CONSUMER_BATCH_SIZE, hub)
        }
    ...
    afterEach(async () => {
        ...
        await Promise.all([eventsConsumer?.stop() ?? Promise.resolve(), flowWorker?.stop() ?? Promise.resolve()])
    ...
    async function triggerWorkflow(eventGlobals: InsightsFunctionInvocationGlobals): Promise<void> {
    ...
    async function triggerDwhWorkflow(rowGlobals: InsightsFunctionInvocationGlobals): Promise<void> {
    ...
    describe('conditional branch workflow', () => {
        ...
        it('should take branch A when event matches the first condition', async () => {
            globals = createGlobals({
                event: '$pageview',
                properties: { $current_url: 'https://hanzo.ai/pricing' },
            })
            await triggerWorkflow(globals)
    ...
    describe('live edit while a run is parked (follow-live contract)', () => {
        ...
        async function applyLiveEdit(flow: Flow): Promise<void> {
            ...
            ;(flowWorker as any).flowManager.lazyLoader.markForRefresh(flow.id)
        }
    ...
    describe('wait_until_condition: condition matches immediately', () => {
        beforeEach(async () => {
            ...
            // Event matches the condition: $pageview with insights in URL
            globals = createGlobals({
                event: '$pageview',
    ...
    describeMatcher('wait_until_condition: subscription matcher wakes parked jobs', () => {
        ...
        it('wakes a parked job when a later event satisfies the property condition', async () => {
            ...
            // A later $pageview with a insights URL satisfies the property condition.
            await matcher.processBatch([
                createGlobals({ event: '$pageview', properties: { $current_url: 'https://hanzo.ai' } }),
            ])
        ...
        it('wakes a parked condition wait on a person-property change with no analytics event', async () => {
            ...
            // event (only a datastore_person mutation) must still wake the wait. The matcher's
            ...
            // person_id = 'uuid' — the same id the datastore_person mutation below targets. The resolved
            ...
            // A datastore_person mutation for that person — the person now matches `plan=enterprise`.
            ...
            const personMessage = {
                value: Buffer.from(
                    JSON.stringify({
                        id: 'uuid',
                        team_id: team.id,
                        properties: JSON.stringify({ email: 'test@hanzo.ai', plan: 'enterprise' }),
                        is_deleted: 0,
                        is_identified: 1,
        ...
        // Wait conditions authored as InsightsQL expressions must be woken by the streams, not only by the
        // 10-minute poll — otherwise removing the poll would silently strand them. These three cover
        // the distinct InsightsQL condition shapes real production flows use; each bytecode is exactly what
        // the Django serializer compiles for that expression (verified via compile_filters_bytecode).
        // If InsightsQL compilation changes, or a stream stops carrying what the bytecode reads (e.g. the
        // person wake dropping properties), one of these fails.
        it('wakes a InsightsQL event-name wait condition via the events stream', async () => {
            // InsightsQL: event = 'billing_added'
            await createWaitUntilWorkflow({
                condition: {
                    filters: {
                        bytecode: ['_H', 1, 32, 'billing_added', 32, 'event', 1, 1, 11],
                        properties: [{ type: 'insightsql', key: "event = 'billing_added'" }],
                    },
                },
                max_wait_duration: '5m',
            })
        ...
        it('wakes a InsightsQL event+property wait condition only on the matching event and property', async () => {
            // InsightsQL: event = 'cal_booking' and properties.trigger_event = 'BOOKING_CREATED'
            await createWaitUntilWorkflow({
                condition: {
                    filters: {
                        bytecode: [
        ...
        it('wakes a InsightsQL dynamic-key person-property wait condition via the person stream', async () => {
            // InsightsQL: person.properties[concat(toString(person.properties.shop_id), '_billing_status')] = 'active'
            await createWaitUntilWorkflow({
                condition: {
                    filters: {
                        bytecode: [
            ...
            await triggerWorkflow(createGlobals())
            await expectParked()
            // datastore_person mutation sets 42_billing_status = 'active' with no analytics event.
            const personMessage = {
                value: Buffer.from(
                    JSON.stringify({
                        id: 'uuid',
        ...
        // reference. The matcher re-keys the wait off the datastore_person_distinct_id repoint so it
        ...
        const survivorPersonRow = (): any => ({
            ...
            uuid: 'new-uuid',
            team_id: team.id,
            properties: { email: 'test@hanzo.ai', plan: 'enterprise' },
            properties_last_updated_at: {},
            properties_last_operation: null,
            created_at: DateTime.utc(),
            version: 2,
        ...
        it('anchors a wait whose condition is still false, so a later person update can wake it', async () => {
            ...
            const personWithoutPlan = { ...survivorPersonRow(), properties: { email: 'test@hanzo.ai' } }
    ...
    describe('batch workflow: event.distinct_id resolved at the worker', () => {
        ...
        it("backfills {event.distinct_id} from the person's distinct_id at dequeue", async () => {
            ...
            // The script template's `{event.distinct_id}` resolved at runtime to the value
            // the worker backfilled — proving the full chain: postgres lookup → CyclotronPerson.distinct_id
            // → state.event.distinct_id mutation → script input resolution.
            expect(mockFetch).toHaveBeenCalledWith(
    ...
    describe('data warehouse table trigger workflow', () => {
        ...
        it('does not look up a person for warehouse rows (no synthetic distinct_id resolution)', async () => {
            ...
            const personSpy = jest.spyOn((flowWorker as any).personsManager, 'getCyclotronPerson')
    ...
    describe('insights_ticket_tags input resolves templated values per element', () => {
        // Regression guard for the templating opt-in in insights/cdp/validation.py.
        ...
        beforeEach(async () => {
            await insertInsightsFunctionTemplate(hub.postgres, {
                ...
                inputs_schema: [
                    { key: 'url', type: 'string', required: true },
                    { key: 'tags', type: 'insights_ticket_tags', required: true },
                ],
            })
                            ...
                            config: {
                                ...
                                inputs: {
                                    ...
                                    tags: {
                                        ...
                                        // `insights_ticket_tags` value like
...
describe('Workflows E2E (email queue)', () => {
    ...
    beforeEach(async () => {
        ...
        const flowConsumerQueue = new CyclotronJobQueuePostgresV2(hub.CONSUMER_BATCH_SIZE, hub)
    ...
    afterEach(async () => {
        await Promise.all([
            ...
            flowWorker?.stop() ?? Promise.resolve(),
    ...
    it('does not emit duplicate Resuming / Executing / pause logs for the email-queue routing reschedule', async () => {
        ...
        // (see InsightsFunctionHandler.execute in actions/insights_function.ts). That second dequeue
    ...
    it('rate-limited variant processes emails end-to-end through the dedicated bucket', async () => {
        ...
        const bucketKey = '@insights-test/ses-e2e/bucket'
        await deleteKeysWithPrefix(limiterValkey, '@insights-test/ses-e2e/')
    ...
    it('rate-limits dequeue when the bucket drains, then drains the queue as it refills', async () => {
        ...
        const bucketKey = '@insights-test/ses-e2e-throttled/bucket'
        await deleteKeysWithPrefix(limiterValkey, '@insights-test/ses-e2e-throttled/')
    ...
    it('does not increment the limiter counter on idle polls (no work, no metric)', async () => {
        ...
        const bucketKey = '@insights-test/ses-e2e-idle/bucket'
        await deleteKeysWithPrefix(limiterValkey, '@insights-test/ses-e2e-idle/')
    ...
    it('claims only the visible row count (sparse traffic does not drain the bucket)', async () => {
        ...
        const bucketKey = '@insights-test/ses-e2e-presize/bucket'
        await deleteKeysWithPrefix(limiterValkey, '@insights-test/ses-e2e-presize/')
    ...
    describe('message_assets bulk capture', () => {
        ...
        it('produces one message_assets row per workflow email with the rendered HTML and metadata', async () => {
            const flow = buildEmailWorkflow('Asset bulk-capture single')
            await insertFlow(hub.postgres, flow)
        ...
        it('bulk-captures every asset when multiple workflow runs share a batch', async () => {
            ...
            const flow = buildEmailWorkflow('Bulk asset capture')
            await insertFlow(hub.postgres, flow)
...
describe('Workflows E2E: batch resolver dispatch via cdp-api', () => {
    ...
    it('POST /batch_invocations with flag on creates a resolver cyclotron job with the right shape', async () => {
        ...
        const response = await supertest(app)
            .post(`/v1/projects/${team.id}/insights_flows/${flow.id}/batch_invocations/${parentRunId}`)
            .send({
    ...
    it('rejects with 400 when the workflow is not a batch trigger', async () => {
        ...
        await supertest(app)
            .post(`/v1/projects/${team.id}/insights_flows/${inserted.id}/batch_invocations/${parentRunId}`)
    ...
    it('persistent fetch failure exhausts MAX_RESOLVER_ATTEMPTS → flips to pendingTerminal=failed and writes status=failed', async () => {
        ...
        mockInternalFetch.mockImplementation((url: string, opts: any) => {
            if (url.includes('/user_blast_radius_persons')) {
                fetchAttempts += 1
                return Promise.reject(new Error('Datastore permanently rejecting the audience query'))
            }
            if (url.includes('/batch_jobs/') && url.endsWith('/status')) {
                statusPuts.push(parseJSON(opts.body) as { status: string })
                return Promise.resolve({
...
describe('Workflows E2E (janitor poison-pill recovery, postgres-v2)', () => {
    ...
    beforeEach(async () => {
        ...
        // KAFKA_FN_INVOCATION_RESULTS isn't in TEST_KAFKA_TOPICS — the janitor
    ...
    it('reverts to legacy mark-failed (records nothing) when recovery is disabled', async () => {
        ...
        expect(result.poisonedIds).toEqual([id])
        expect(mockProducerObserver.getProducedKafkaMessagesForTopic(KAFKA_FN_INVOCATION_RESULTS)).toHaveLength(0)
