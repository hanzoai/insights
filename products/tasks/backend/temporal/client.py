...
def _resolve_mcp_scopes(task_run: TaskRun) -> InsightsMcpScopes:
    """Best-effort scope posture for the reconciler when ``pending_dispatch`` didn't carry
    ``insights_mcp_scopes`` (pre-reconciler rows, or the bootstrap/start path). Mirrors
...
def redispatch_orphaned_task_run(run_id: str) -> str:
    ...
    workflow_input = ProcessTaskInput(
        ...
        insights_mcp_scopes=dispatch_params.get("insights_mcp_scopes") or _resolve_mcp_scopes(task_run),
    )
...
def execute_bake_dev_stack_image_workflow(publish_name: str = DEV_STACK_IMAGE_NAME) -> None:
    """Start (or restart) the bake of the prebaked Insights dev-stack VM image.
...
def execute_insights_code_agent_relay_workflow(
    run_id: str,
    text: str,
    ...
    workflow_id = f"insights-code-agent-relay-{run_id}-{relay_id}"

    client = sync_connect()
    asyncio.run(
        client.start_workflow(
            "insights-code-agent-relay",
