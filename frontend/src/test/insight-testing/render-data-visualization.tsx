...
/** Minimal `[columnName, datastoreType]` + row-major results — the same shape the
...
export function buildInsightsQLResponse({ columns, types, results }: DataVizFixture): InsightsQLQueryResponse {
    return {
        results,
        columns,
        types,
...
export function renderDataVisualization(props: RenderDataVisualizationProps): ReturnType<typeof render> {
    ...
    const cachedResults = buildInsightsQLResponse(props.response)
