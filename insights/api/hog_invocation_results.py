...
# columns: Datastore's analyzer resolves a name in WHERE/HAVING to a same-named
...
def _decode_invocation_globals(stored: str) -> dict[str, Any]:
    ...
    script-invocation-results service). Legacy rows predate compression and are stored
...
class HogInvocationResultsRequestSerializer(serializers.Serializer):
    ...
    after = serializers.CharField(
        ...
        "Defaults to -7d — bounds the Datastore partition scan, so widen it explicitly for older runs.",
    )
