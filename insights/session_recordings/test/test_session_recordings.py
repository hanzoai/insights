...
from insights.models import Organization, SessionRecording, User
...
class TestSessionRecordings(APIBaseTest, DatastoreTestMixin, QueryMatchingTest):
    ...
    @patch(
        "insights.insightsql.database.database.feature_enabled_or_false",
    ...
    def test_session_ids_results_follow_the_requested_order(self, _name: str, persisted: bool):
        ...
        # requested recording is persisted there is nothing left to look up in Datastore, so the
    ...
    def test_capture_diagnostics_degrades_gracefully_on_datastore_error(self):
        session_recording_id = "session_1"
        produce_replay_summary(session_id=session_recording_id, team_id=self.team.pk, distinct_id="d1")

        with patch(
            "insights.session_recordings.session_recording_api.get_latest_session_event_properties",
    ...
    @patch(
        "insights.session_recordings.session_recording_api.SessionRecordingViewSet._delete_via_recording_api",
        return_value=[],
    )
    ...
    @parameterized.expand(
        [
            (
                "at_capacity",
                DatastoreAtCapacity(),
                "Datastore is at capacity. Try again later.",
            ),
            (
                "timeout_exceeded",
    ...
    def test_sync_execute_ch_at_capacity_retry_then_503(self):
        """Test that list_blocks throws DatastoreAtCapacity multiple times and eventually returns 503"""
        call_count = 0
    ...
    def test_bulk_delete_works_for_datastore_only_recordings(self, _mock_delete_via_recording_api):
        create_person(
            team=self.team,
            distinct_ids=["user1"],
            properties={"email": "test@example.com"},
    ...
    @patch("insights.session_recordings.session_recording_api.logger")
    ...
    @patch("insights.session_recordings.session_recording_api.execute_summarize_session_video_stream")
    ...
    @patch(
        "insights.session_recordings.session_recording_api._cancel_summary_workflow",
...
class TestCancelSummaryWorkflowHelper:
    ...
    async def test_swallows_not_found_for_idempotent_cancel(self) -> None:
        ...
        from insights.session_recordings.session_recording_api import _cancel_summary_workflow
        ...
        with patch(
            "insights.session_recordings.session_recording_api.async_connect",
