...
class Command(BaseCommand):
    help = (
        ...
        "but no corresponding OrganizationIntegration in Insights (due to broken deauthorization webhooks)."
    )
    ...
    def handle(self, *args, **options):
        ...
        for org_id in org_ids:
            try:
                ...
                self.stdout.write(self.style.WARNING(f"  {org_id}: organization not found in Insights, skipping"))
                not_found += 1
                continue
