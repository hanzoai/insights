...
from insights.insightsql.errors import BaseInsightsQLError, SyntaxError
from insights.insightsql.json_ast import deserialize_ast
...
from insights.schema_enums import ParserMode
...
# down — `insightsql_parser` (cpp) is still available as the production
...
try:
    ...
    logger.exception("insightsql_parser_rs import failed; rust-json and rust-py backends disabled")
    capture_exception(
        _import_err,
        additional_properties={"insightsql_parser_rs_import_error": _RUST_IMPORT_ERROR_REPR},
    )

    def _rust_parser_unavailable(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(
            f"insightsql_parser_rs is not importable (packaging error); original ImportError: {_RUST_IMPORT_ERROR_REPR}"
        )
...
RULE_TO_PARSE_FUNCTION: dict[InsightsQLParserBackend, dict[ParseRule, Callable]] = {
    "cpp-json": {
...
# Parser version per backend, resolved once at import. cpp/rust ship as wheels (`insightsql-parser`, `insightsql-parser-rs`).
_BACKEND_VERSION: dict[InsightsQLParserBackend, str] = {
    "cpp-json": _parser_version("insightsql-parser"),
    "rust-json": _parser_version("insightsql-parser-rs"),
    "rust-py": _parser_version("insightsql-parser-rs"),
}
...
DEFAULT_BACKEND: InsightsQLParserBackend = "cpp-json"


# `parserMode` (a InsightsQLQueryModifier) selects the parser backend per query.
...
_PARSER_MODE_BACKENDS: dict[ParserMode, tuple[InsightsQLParserBackend, InsightsQLParserBackend | None]] = {
...
# `InsightsQLQueryModifier`, so a client can supply it — but the C++ backend's recursion guard is
...
def _resolve_parser_mode(
    parser_mode: ParserMode | None, backend: InsightsQLParserBackend | None
) -> tuple[InsightsQLParserBackend, InsightsQLParserBackend | None]:
...
class InsightsQLParserShadowMismatch(Exception):
...
_SHADOW_COMPARISONS = Counter(
    "insightsql_parser_shadow_comparisons_total",
...
def _run_shadow_comparison(
    rule: ParseRule,
    statement: str,
    primary_backend: InsightsQLParserBackend,
    shadow_backend: InsightsQLParserBackend,
    primary_node: Any,
    start: int | None,
) -> None:
    ...
    Increments `insightsql_parser_shadow_comparisons_total` (run count + agreement rate, tagged by parser version). The
    ...
    divergence_properties = {
        "insightsql_parser_rule": rule_label,
        "insightsql_parser_primary": primary_backend,
        "insightsql_parser_shadow": shadow_backend,
        "insightsql_parser_primary_version": primary_version,
        "insightsql_parser_shadow_version": shadow_version,
        "insightsql_parser_statement": statement,
    }
    try:
        shadow_node = _invoke_parser(shadow_backend, rule, statement, start)
    except BaseInsightsQLError as err:
        # Shadow rejects input the primary accepted: a divergence (raises in TEST).
        _count("shadow_rejected")
        capture_exception(err, additional_properties={**divergence_properties, "insightsql_parser_shadow_throw": "true"})
        if test_mode:
            raise
        return
    ...
    mismatch = InsightsQLParserShadowMismatch(
    ...
    capture_exception(
        mismatch,
        additional_properties={**divergence_properties, "insightsql_parser_position_only_mismatch": position_only},
    )
...
_PARSE_CACHE_EVENTS = Counter(
    "insightsql_parse_cache_events_total",
    "InsightsQL parse-cache lookups",
    labelnames=["origin", "result", "rule"],
)
_PARSE_CACHE_SIZE = Gauge(
    "insightsql_parse_cache_size",
    "Current entries in the InsightsQL parse cache (compare against the configured maxsize to spot saturation)",
    labelnames=["cache"],
    multiprocess_mode="livemax",
)
_PARSE_CACHE_MAXSIZE = Gauge(
    "insightsql_parse_cache_maxsize",
    "Configured maxsize of the InsightsQL parse cache",
    labelnames=["cache"],
    multiprocess_mode="livemax",
)
...
_PARSE_STATEMENT_LENGTH = Histogram(
    "insightsql_parse_statement_length_chars",
    "Length of InsightsQL statements passed to the parser, in characters",
    labelnames=["rule"],
...
def _invoke_parser(backend: InsightsQLParserBackend, rule: ParseRule, statement: str, start: int | None) -> Any:
...
def _parse_cached(
    rule: ParseRule,
    statement: str,
    backend: InsightsQLParserBackend,
...
def parse_string_template(
    ...
    *,
    backend: InsightsQLParserBackend | None = None,
