...
class EmailOutboxMessage(UUIDModel):
    ...
    comment = models.OneToOneField("insights.Comment", on_delete=models.CASCADE)
    ...
    class Meta:
        ...
        db_table = "insights_conversations_email_outbox_message"
        indexes = [
            # Sweeper query: pending rows due for an attempt, oldest first.
            models.Index(fields=["status", "next_attempt_at"], name="insights_con_outbox_due_idx"),
        ]
