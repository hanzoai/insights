...
# Derived from insights/models/ai_events/sql.py DDL. Keep in sync when adding columns.
...
class AiColumnToPropertyRewriter(CloningVisitor):
    ...
    def visit_join_expr(self, node: ast.JoinExpr) -> ast.JoinExpr:
        ...
        # Swap FROM insights.ai_events -> FROM events (also handle aliased form).
        # String comparison is safe here because AiEventsTable is only mounted at the
        # qualified `insights.ai_events` path — there is no top-level `ai_events` form —
        # and this rewriter runs pre-resolution without a database context.
        # nosemgrep: insightsql-no-string-table-chain
        if isinstance(new_node.table, ast.Field) and new_node.table.chain == ["insights", "ai_events"]:
...
def _has_ai_events_from(query: ast.SelectQuery) -> bool:
    """Check if a SELECT query has FROM insights.ai_events.

    AiEventsTable is only mounted at the `insights.ai_events` path (no top-level form),
    ...
    return (
        ...
        # nosemgrep: insightsql-no-string-table-chain
        and query.select_from.table.chain == ["insights", "ai_events"]
    )
