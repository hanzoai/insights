...
import { InsightsApiError, ToolInputValidationError } from '@/lib/errors'
...
describe('exec tool', () => {
    ...
    describe('call command', () => {
        ...
        it('returns ONLY the formatted table when result has __formatted_results_override and mode is optimized', async () => {
            ...
            expect(result).not.toContain('_insightsUrl')
        ...
        it('still TOON-encodes when __formatted_results_override is absent', async () => {
            ...
            expect(result).toContain('_insightsUrl')
        ...
        it('keeps agent CLI informational data inside the trust boundary in --json mode', async () => {
            ...
            const exec = createExec([tool], 'insights-cli')
        ...
        it('keeps UI data in structuredContent (not _meta) when the tool has no formatted table', async () => {
            const tool = makeMockTool({
                _meta: { ui: { resourceUri: 'ui://insights/mock-app.html' } },
            })
            const exec = createExec([tool], 'insights-code')
            ...
            expect(result._meta.ui.resourceUri).toBe('ui://insights/mock-app.html')
            expect(result._meta['ui/resourceUri']).toBe('ui://insights/mock-app.html')
        ...
        // Inline-exec UI-app hosts: Insights Desktop (via consumer) plus Claude Code and
        ...
        it.each([
            ['insights-code consumer', 'insights-code', undefined],
        ...
        it('carries the payload once — in structuredContent — when there is no formatted override', async () => {
            ...
            expect(result.content[0]!.text).toBe(STRUCTURED_CONTENT_ONLY_TEXT)
            expect(result.content[0]!.text).not.toContain('_insightsUrl')
            const structured = result.structuredContent as { results: unknown }
            expect(structured.results).toEqual([{ data: [1, 2, 3], count: 6 }])
            expect(result._meta[APP_DATA_META_KEY]).toBeUndefined()
        })

        // insights_ai is sent as its own consumer for attribution but is NOT a UI-apps host.
        it.each([[undefined], ['cline'], ['claude-code'], ['slack'], ['insights_code'], ['insights_ai']])(
    ...
    describe('deprecated tool redirects', () => {
        it.each([
            ...
            ['query-generate-insightsql-from-question', 'execute-sql'],
