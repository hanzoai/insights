...
def send_analytics_to_insights(event, data):
    hanzo_insights.project_api_key = "sTMFPsFhdP1Ssg"  # ty: ignore[invalid-assignment]
...
def process_error(
    ...
    if alert:
        if async_migrations_emails_enabled():
            from insights.tasks.email import send_async_migration_errored_email
    ...
    from insights.async_migrations.runner import attempt_migration_rollback
...
def trigger_migration(migration_instance: AsyncMigration, fresh_start: bool = True):
    from insights.tasks.async_migrations import run_async_migration
