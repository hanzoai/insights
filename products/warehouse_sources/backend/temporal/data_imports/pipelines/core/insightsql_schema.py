...
class InsightsQLSchema:
    ...
    def add_field(self, field: pa.Field, column: pa.ChunkedArray) -> None:
        ...
        insightsql_type = self._map_arrow_type(field.type)

        if insightsql_type is StringDatabaseField:
    ...
    def _map_arrow_type(self, arrow_type: pa.DataType) -> type[DatabaseField]:
        if pa.types.is_time(arrow_type):
            ...
            # Datastore's deltaLake()/Parquet readers surface binary columns as String, so
            # db_columns always reports "String" for these — map to the same InsightsQL type or
