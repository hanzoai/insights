...
description: >
  Create a Insights endpoint with the right shape on the first try — covers query kind choice, name
  conventions, what to expose as variables (InsightsQL code_name vs insight breakdown),
...
saved InsightsQL or insight queries as callable HTTP routes — the configuration choices made at
...
- "Help me turn this InsightsQL into a callable endpoint"
- A new caller (mobile app, customer-facing dashboard, downstream pipeline) needs Insights data
...
- An **external system** (someone else's code) needs to call Insights for data
...
- An internal Insights dashboard or insight needs the data — use the insight directly; an endpoint
...
- **InsightsQL** (`InsightsQLQuery`) — raw SQL written by the user. Variables defined via `{variables.x}`
...
- **Insight** — wraps an existing insight definition. Best supported for `TrendsQuery`,
  ...
  variables** — rewrite those as InsightsQL if you need either.

InsightsQL is the more flexible choice. Pick insight only when the user is genuinely re-publishing an
...
**For InsightsQL endpoints**, variables are declared in the query payload with `code_name`, `type`,
...
3. Walk through the query — confirm it's InsightsQL or insight, and that the columns/shape make sense.
...
- "Got it. A few choices:
   - Name: how about monthly_active_users? Lowercase, descriptive,
     no version baggage.
   - The query — is it InsightsQL or are you exposing an existing insight?
     InsightsQL gives you exact column control; insight reuses what you've
     already built.
...
[user picks: InsightsQL, lookback_days + org_id variables, 3600s TTL,
...
- **InsightsQL endpoints are more flexible than insight endpoints.** Default to InsightsQL unless the
...
- **Materialised endpoints require all variables to be passed.** Calls without them are
  ...
  (Optional/partial variables on materialised endpoints are a known limitation the Insights team
...
- **Share friction via `agent-feedback`.** If a limitation gets in the way (eligibility rules,
  required variables, the TTL enum), send the Insights team a note — it's how the product and these
  tools improve.
