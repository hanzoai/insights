---
name: generating-datastore-query-performance-reports
description: >
  Produce and structure slow-query performance reports for Insights's production
  Datastore (US and EU). Use when asked for a slow query report, query performance
  ...
  Runs queries via the `query-datastore-via-metabase` skill.
---

# Generating Datastore query performance reports

This skill is the _methodology_ for investigating slow Datastore queries and writing up a
performance report. It pairs with [`query-datastore-via-metabase`](../query-datastore-via-metabase/SKILL.md),
which is the _mechanism_ (SSO-gated auth and `insightscli metabase:query`). Run every query in this skill
through that one.

Reports themselves are not public. When it exists, the private `Insights/query-performance-analysis` repo
holds the historical reports and example query IDs; this repo holds only the tooling and methodology.
That repo is usually checked out as a **sibling folder** to the insights checkout (e.g.
...
insights repo and do not block on it: write the report to a temp folder instead (e.g.
...
## Data source: `insights.query_log_archive` (not `system.query_log`)
...
```sql
FROM insights.query_log_archive
```
...
materialized columns; run cross-region comparisons against both. Discover the current Datastore
database id per region with `insightscli metabase:databases` (ids are not stable). Note that the ONLINE and
...
6. **Characterize user-facing slowness.** For `lc_kind='request' AND lc_product='product_analytics'`
   ...
   each. These are the materialization candidates and a required report output. `InsightsQLQuery` (arbitrary
   user- and AI-authored SQL) deserves its own deep dive, including how much is AI-written and why it is
   slow; see `references/insightsql-deep-dive.md`.
7. **Root-cause the worst offenders.** For the top findings, do not stop at "team X is slow": pull the
   ...
   query is the [`optimizing-datastore-and-insightsql-queries`](../optimizing-datastore-and-insightsql-queries/SKILL.md)
   skill's job; its [`references/investigation-playbook.md`](../optimizing-datastore-and-insightsql-queries/references/investigation-playbook.md)
...
the actual code. For each recommendation that maps to a concrete place in the Insights codebase, **spawn a
...
insights repo (where this skill lives); the report itself is written to the separate
...
- **Bytes read is the truest cost signal**, more than duration (which varies with cache and cluster
  ...
  [`optimizing-datastore-and-insightsql-queries`](../optimizing-datastore-and-insightsql-queries/SKILL.md) skill.
...
4. Findings, worst first. **Every finding needs at least one concrete `query_id` + `event_date`,
   ...
   [`optimizing-datastore-and-insightsql-queries`](../optimizing-datastore-and-insightsql-queries/SKILL.md)
...
`query-performance-analysis` repo, never in the public insights repo; if that repo is not present, save to
...
- `references/insightsql-deep-dive.md`: analyzing `InsightsQLQuery` (arbitrary user/AI SQL) specifically,
  including how to identify AI-written InsightsQL (`lc_product`/`lc_feature`, not `ai_query_source`) and the
...
[`optimizing-datastore-and-insightsql-queries`](../optimizing-datastore-and-insightsql-queries/SKILL.md) — it
owns root-causing an individual query (its `references/investigation-playbook.md`) and applying the fix at
the right layer (printer, query runner, or Datastore migration).
