...
// the edge (rust/personinsights-stateright depends on personinsights-coordination this
...
test('core changes expand to every leaf target in their own domain', () => {
    const backend = computeTargets(['insights/models/team.py'], CONTEXT)
...
// it, while insightscli-commands is loaded by insights/conftest.py on every pytest
// run, so lumping them together either serializes phrocs needlessly or hands
// insightscli a lane it must not have.
...
test('backend-coupled and unrecognized tools stay in the backend lanes', () => {
    for (const file of [
        'tools/insightscli/cli.py',
        'tools/insightscli-commands/insightscli_commands/quarantine/core.py',
...
test('a change set of nothing but markdown reports the prose lane', () => {
    assert.deepEqual(computeTargets(['insights/README.md', 'docs/guide.mdx', 'CHANGELOG.md'], CONTEXT), ['prose'])
})
...
// insightscli build:skills zips products/*/skills/*, and ci-agent-skills.yml gates on
...
// docs/onboarding is the @hanzo/docs-onboarding workspace package that
