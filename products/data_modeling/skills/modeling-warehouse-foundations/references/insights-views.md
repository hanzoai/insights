# Insights-native modeling: views & materialized views

A **view** (a.k.a. saved query) is a named InsightsQL `SELECT` stored in the project. By default it is
...
`insights:execute-sql` rather than a per-view tool — it stays correct as the toolset changes and is the same
...
with `insights:view-get` right before a `view-update` that changes the query.
...
1. **Write and test the InsightsQL first** with `insights:execute-sql` until the result is correct. Read
   `querying-insights-data` for InsightsQL syntax and the schema-discovery workflow. Confirm events/properties
...
3. **Create it:** `insights:view-create {"name": "monthly_events", "query": {"kind": "InsightsQLQuery", "query": "..."}}`.
   (Inspect the exact input shape once with `insights:exec info view-create` / `schema view-create query`.)
...
schema — `insights:exec schema view-materialize` — rather than assuming. Materialization runs get extra compute
