...
from insights.schema import Breakdown, BreakdownFilter, MultipleBreakdownType, TrendsFilter, TrendsQuery

from insights.insightsql.errors import ExposedInsightsQLError

from insights.constants import TRENDS_BOLD_NUMBER, TRENDS_CUMULATIVE, TRENDS_PIE
...
class TestFormula(DatastoreTestMixin, APIBaseTest):
    ...
    @snapshot_datastore_queries
    def test_breakdown_insightsql_with_as_alias(self):
        self._run(
            {
                "breakdownFilter": {
    ...
    def test_breakdown_insightsql_alias_forms(self, _name: str, breakdown_expr: str):
        response = self._run(
            {
                "breakdownFilter": {
    ...
    def test_multiple_breakdowns_with_insightsql_aliases(self):
        response = self._run(
            {
                "breakdownFilter": BreakdownFilter(
                    breakdowns=[
                        Breakdown(property='properties.location AS "Location"', type=MultipleBreakdownType.INSIGHTSQL),
                        Breakdown(
                            property="person.properties.$some_prop AS `Some Prop`",
                            type=MultipleBreakdownType.INSIGHTSQL,
                        ),
                    ]
                ),
            }
        ...
        baseline = self._run(
            {
                "breakdownFilter": BreakdownFilter(
                    breakdowns=[
                        Breakdown(property="properties.location", type=MultipleBreakdownType.INSIGHTSQL),
                        Breakdown(property="person.properties.$some_prop", type=MultipleBreakdownType.INSIGHTSQL),
                    ]
                ),
            }
        )
    ...
    def test_formula_with_insightsql_math_no_matching_events(self):
                    ...
                    "series": [
                        {
                            ...
                            "name": "custom insightsql",
                            "math": "insightsql",
                            "math_insightsql": "sum(toFloat(properties.xyz))",
                        },
                        {
                            "kind": "EventsNode",
                            "event": "session start",
        ...
        # Series A (insightsql math with no data) should be treated as 0, not None
