...
from insights.insightsql.database.models import LazyJoin, Table
...
def add_postgres_foreign_key_lazy_joins(
    ...
    if not isinstance(insightsql_table.name, str):
        return

    namespace = insightsql_table.name.split(".")[:-1]
    ...
    for column_name in columns.keys():
        ...
        if insightsql_table.fields.get(field_name):
            continue
...
def _add_foreign_key_lazy_join(
    *,
    ...
    resolved_target = _resolve_target_table(
        ...
        source_table_name=insightsql_table.name if isinstance(insightsql_table.name, str) else None,
        database=database,
    )
    if resolved_target is None:
        return

    join_table, target_insightsql_table = resolved_target
    if not _is_same_external_scope(insightsql_table, target_insightsql_table, warehouse_table):
        return

    insightsql_table.fields[field_name] = LazyJoin(
    ...
    target_table_name = target_insightsql_table.name if isinstance(target_insightsql_table.name, str) else None
    source_table_name = insightsql_table.name if isinstance(insightsql_table.name, str) else None
    ...
    if target_insightsql_table.fields.get(reverse_field_name) is not None:
        return

    target_insightsql_table.fields[reverse_field_name] = LazyJoin(
        from_field=to_field,
        to_field=from_field,
        join_table=insightsql_table,
        resolver=FOREIGN_KEY,
    )
...
def _is_same_external_scope(
    source_insightsql_table: Table,
    target_insightsql_table: Table,
    warehouse_table: DataWarehouseTable,
) -> bool:
    source_table_name = source_insightsql_table.name if isinstance(source_insightsql_table.name, str) else None
    target_table_name = target_insightsql_table.name if isinstance(target_insightsql_table.name, str) else None
    ...
    if source is not None and source.access_method == ExternalDataSource.AccessMethod.DIRECT:
        return isinstance(target_insightsql_table, DirectSQLTable) and target_insightsql_table.external_data_source_id == str(
            source.id
        )
...
def _find_inferred_foreign_key(
    *,
    ...
    for candidate in _candidate_target_tables(base_name=base_name, namespace=namespace):
        ...
        target_insightsql_table = database.get_table(candidate)
        target_table_name = target_insightsql_table.name if isinstance(target_insightsql_table.name, str) else None
        ...
        target_column = _find_inferred_target_column(column=column, target_insightsql_table=target_insightsql_table)
...
def _find_inferred_target_column(*, column: str, target_insightsql_table: Table) -> str | None:
    for candidate in (column, "id"):
        if target_insightsql_table.fields.get(candidate) is not None:
            return candidate
...
def _candidate_target_tables(*, base_name: str, namespace: list[str]) -> list[str]:
    local_candidates = [base_name, f"{base_name}s", f"insights_{base_name}"]
