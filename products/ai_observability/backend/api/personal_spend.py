...
Surfaces the requesting user's own LLM spend across Insights products (last N
days) by running a fixed set of InsightsQL queries against Insights's internal cloud
...
from insights.rate_limit import PersonalSpendBurstThrottle, PersonalSpendDailyThrottle, PersonalSpendSustainedThrottle
...
SUPPORTED_PRODUCTS = frozenset({"insights_code"})
...
class _SpendQueryParamsSerializer(serializers.Serializer):
    date_from = serializers.CharField(
        ...
        help_text=(
            ...
            "(`-7d`, `-1m`, etc.) — same parser used elsewhere in Insights. Defaults to `-30d`. The "
    ...
    refresh = serializers.BooleanField(
        required=False,
        default=False,
        help_text="If true, bypass the result cache and re-run the underlying queries against Datastore.",
    )
...
class _ProductBreakdownRowSerializer(serializers.Serializer):
    product = serializers.CharField(
        allow_null=True,
        help_text="Value of the `ai_product` property on the event (e.g. `insights_code`, `background_agents`). Null when unset.",
    )
...
def _email_filter(email: str) -> ast.Expr:
    # With person-on-events, the event row carries `person_id` directly; InsightsQL then joins to
...
def _fetch_by_day(
    ...
    result = execute_insightsql_query(
        ...
        team=team,
        # InsightsQL wraps `timestamp` in the team's configurable timezone by default, which would
...
def _compute_spend_analysis(
    *,
    ...
    # Tag the underlying Datastore reads with the LLM_ANALYTICS product so they show up
    # in the existing per-product Prometheus + cost-attribution dashboards alongside the
    # rest of AI observability traffic. Wraps every call into `_fetch_*` -> InsightsQL.
...
class PersonalSpendViewSet(_PersonalSpendUserViewSet):
    """
    Returns the requesting user's personal LLM spend analysis across Insights products.

    Queries are run server-side against Insights's internal analytics team
    ...
    and the InsightsQL printer uses `pmat_email` on the main cluster's `person`
...
_EU_REDIRECT_TARGET = "https://us.hanzo.ai/api/llm_analytics/@me/spend/"
...
def personal_spend_eu_redirect(request: HttpRequest) -> HttpResponseRedirect:
    ...
    EU Insights Cloud forwards its product LLM telemetry to Insights Cloud US, so the
...
CROSS_REGION_SIGNATURE_HEADER = "X-Insights-Spend-Signature"
...
def _cross_region_target_url() -> str:
    ...
    return f"https://us.hanzo.ai{_CROSS_REGION_INTERNAL_PATH}"
...
class PersonalSpendEUProxyViewSet(_PersonalSpendUserViewSet):
    ...
    @extend_schema(
        ...
        description="EU mirror of the personal spend endpoint — proxies to Insights Cloud US, where the data lives.",
        tags=["AI observability"],
    )
