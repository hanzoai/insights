...
LOCAL_ROOTS = ("insights", "ee", "products", "common", "dags")
...
FULL_RUN_PATTERNS = (
    ...
    ".github/datastore-versions.json",
    "docker-compose",
    "docker/datastore/",
    "bin/wait-for-docker",
...
def classify_test_file(path: str) -> TestFeatures:
    ...
    if path.startswith("insights/api/test/"):
    ...
    return TestFeatures(
        ...
        uses_datastore="datastore" in lower_path or "datastore" in lower_imports,
...
def _is_api_surface_change(path: str) -> bool:
    parts = PurePosixPath(path).parts
    if path.startswith("insights/api/"):
        return True
...
def ast_select_tests(changed_files: list[str], features_by_path: dict[str, TestFeatures]) -> AstSelection:
    ...
    insights_api_tokens: set[str] = set()
    ...
    for path in changed_files:
        ...
        if product:
            ...
            insights_api_tokens.update(_tokens_for_changed_file(path))
    ...
    if insights_api_tokens:
        api_tests = [
            path
            for path, features in features_by_path.items()
            if path.startswith("insights/api/test/") and features.is_django_api_test
        ]
        token_tests = [
            path
            for path, features in features_by_path.items()
            if path.startswith("insights/api/test/") and set(features.api_tokens).intersection(insights_api_tokens)
        ]
        _add_group(groups, "insights_api_route_tokens", token_tests)
        if not token_tests:
            _add_group(groups, "insights_api_client_fallback", api_tests)

    # ── 5. Temporal / Datastore broad matching ──────────────────────
    ...
    if any("datastore" in path for path in changed_files):
        _add_group(
            groups,
            "datastore",
            {path for path, features in features_by_path.items() if features.uses_datastore},
        )
...
_CORE_IGNORED_PREFIXES = ("insights/dags/", "common/scriptvm/python/test/")
...
def segments_for_test_file(path: str) -> frozenset[str]:
    ...
    is_poe = (
        ...
        or path.startswith("insights/api/test/test_insight")
        or path == "insights/api/test/dashboards/test_dashboard.py"
    )
    if is_poe:
        return frozenset({"core", "poe"})
    if path.startswith(("insights/", "ee/")):
...
def format_summary(result: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        "Shadow-only. Compares Snob's import-graph selection with Insights-specific AST groups for API-client and other non-import-shaped tests.",
        "",
