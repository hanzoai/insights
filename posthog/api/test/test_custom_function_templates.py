...
MOCK_NODE_TEMPLATES = json.loads(
    open(os.path.join(os.path.dirname(__file__), "__data__/custom_function_templates.json")).read()
)
...
class TestCustomFunctionTemplates(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    def setUp(self):
        super().setUp()
        ...
        self.webhook_template = CustomFunctionTemplate.objects.create(
        ...
        self.deprecated_template = CustomFunctionTemplate.objects.create(
    ...
    def test_list_function_templates(self):
        response = self.client.get("/api/projects/@current/custom_function_templates/")
    ...
    def test_filter_function_templates(self):
        response1 = self.client.get("/api/projects/@current/custom_function_templates/?type=notfound")
        assert response1.status_code == status.HTTP_200_OK, response1.json()
        assert len(response1.json()["results"]) == 0

        response2 = self.client.get("/api/projects/@current/custom_function_templates/?type=destination")
        response3 = self.client.get("/api/projects/@current/custom_function_templates/")

        assert response2.json()["results"] == response3.json()["results"]
        assert len(response2.json()["results"]) > 5

        response4 = self.client.get("/api/projects/@current/custom_function_templates/?type=site_destination")
        assert len(response4.json()["results"]) > 0

        response5 = self.client.get("/api/projects/@current/custom_function_templates/?types=site_destination,destination")
        assert len(response5.json()["results"]) > 0

    def test_retrieve_function_template(self):
        response = self.client.get("/api/projects/@current/custom_function_templates/template-slack")
        assert response.status_code == status.HTTP_200_OK, response.json()
    ...
    def test_retrieve_function_template_with_other_type(self):
        response = self.client.get("/api/projects/@current/custom_function_templates/template-site-destination")
        assert response.status_code == status.HTTP_200_OK, response.json()
    ...
    def test_public_list_function_templates(self):
        self.client.logout()
        response = self.client.get("/api/public_custom_function_templates/")
    ...
    def test_get_specific_deprecated_template_from_db(self):
        ...
        response = self.client.get(f"/api/projects/@current/custom_function_templates/template-deprecated")
    ...
    def test_template_updates_are_reflected(self):
        ...
        initial_response = self.client.get("/api/projects/@current/custom_function_templates/template-slack")
        assert initial_response.status_code == status.HTTP_200_OK
        assert initial_response.json()["name"] == template_slack.name

        # Create a modified sha of the template
        modified_template = CustomFunctionTemplateDC(
        ...
        updated_response = self.client.get("/api/projects/@current/custom_function_templates/template-slack")
    ...
    def test_public_custom_function_templates_are_sorted_by_usage(self):
        for i in range(10):
    ...
    def test_unauthenticated_updates_are_blocked(self, method: str):
        ...
        response = getattr(self.client, method)(
            "/api/projects/@current/custom_function_templates/template-slack",
            data=payload,
        )

        assert response.status_code in {status.HTTP_405_METHOD_NOT_ALLOWED}, response.json()

        unchanged_template = self.client.get("/api/projects/@current/custom_function_templates/template-slack")
