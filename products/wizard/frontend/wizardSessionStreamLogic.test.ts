...
function makeSession(overrides: Partial<WizardSessionDTOApi> = {}): WizardSessionDTOApi {
    return {
        session_id: 'sess-1',
        team_id: 997,
        workflow_id: 'insights-integration',
        skill_id: 'install',
...
describe('wizardSessionStreamLogic polling mode', () => {
    ...
    beforeEach(async () => {
        ...
        logic = wizardSessionStreamLogic({ workflowId: 'insights-integration' })
        logic.mount()
...
describe('wizardSessionStreamLogic SSE mode', () => {
    ...
    beforeEach(async () => {
        ...
        capture = jest.spyOn(insights, 'capture').mockImplementation(() => undefined as never)
        logic = wizardSessionStreamLogic({ workflowId: 'insights-integration' })
        logic.mount()
        // connect() no-ops into an error without a project — wait for the test bootstrap to provide one.
        await expectLogic(projectLogic).toMatchValues({ currentProjectId: expect.any(Number) })
