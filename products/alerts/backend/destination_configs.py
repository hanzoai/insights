...
WEBHOOK_HEADERS = {"Content-Type": "application/json", "X-Insights-Webhook-Version": "1"}

_FN_FUNCTION_NAME_MAX_LEN = 400
...
def clip_insights_function_name(name: str) -> str:
    if len(name) <= _FN_FUNCTION_NAME_MAX_LEN:
        return name
    return name[: _FN_FUNCTION_NAME_MAX_LEN - 1] + "…"
    ...
    *,
    ...
    return AlertDestinationConfig(
        ...
        payload={
            ...
            "name": clip_insights_function_name(f"{product_name} — {alert_name} ({spec.display_kind}) → {destination_name}"),
