...
description: 'Required reading before writing any InsightsQL/SQL or calling execute-sql against Insights. Use whenever the user wants to search, find, or do complex aggregations Insights entities (insights, dashboards, cohorts, feature flags, experiments, surveys, script flows, data warehouse, persons, etc.) and query analytics data (trends, funnels, retention, lifecycle, paths, stickiness, web analytics, error tracking, logs, sessions, LLM traces). Also the first stop for a governed business number (MRR, activation, revenue): check the semantic layer (canonical metrics in system.information_schema.metrics) for an approved definition before deriving from raw events. Covers InsightsQL syntax differences from Datastore SQL, system table schemas (system.*), available functions, query examples, and the schema-discovery workflow.'
---

# Querying data in Insights

The [guidelines](./references/guidelines.md) contain the same instructions as `insights:execute-sql`. If you've already read `insights:execute-sql`, you don't need to read them again.

## When to use this skill

### Finding a specific Insights entity

When the user wants to find a specific entity created in Insights (insights, dashboards, cohorts, feature flags, experiments, surveys, script flows, data warehouse items, etc.), or when a list/search tool returns too many results to narrow down:

1. Read the appropriate schema reference under Data Schema to understand the entity's table and columns.
2. Use `insights:execute-sql` to query the system table and find the matching entity (typically returning its ID).
3. Use the dedicated read tool for that entity type (e.g. `insights:insight-get`, `insights:dashboard-get`) to retrieve the full entity by ID.
...
2. Adapt the example query (if one was found) to the user's request and run it via `insights:execute-sql`. If no example fit, compose the query from scratch using the Data Schema and InsightsQL References.
...
1. Look for a canonical metric with `insights:execute-sql` (there is no list tool). The table is usually empty; an empty result just means no governed definition exists, so derive the number normally.
...
- [APM / tracing (`insights.trace_spans`)](./references/models-apm-spans.md)
...
- [Script Flows](./references/models-script-flows.md)
- [Script Functions](./references/models-insights-functions.md)
- [Integrations](./references/models-integrations.md)
- [AI observability events (`insights.ai_events`)](./references/models-ai-observability-events.md)
...
- [Metrics (`insights.metrics`)](./references/models-metrics.md)
...
- [Sparkline, SemVer, Session replays, Actions, Translation, HTML tags and links, Text effects, and more](./references/insightsql-extensions.md)
- [SQL variables](./references/models-variables.md).
- [Available functions in InsightsQL](./references/available-functions.md). IMPORTANT: the list is long, so read data using bash commands like grep.
