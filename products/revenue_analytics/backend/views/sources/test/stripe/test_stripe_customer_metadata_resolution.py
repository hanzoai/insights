...
from insights.schema import CurrencyCode, InsightsQLQueryModifiers, InsightsQLQueryResponse
...
def _nullable_columns(basic_types: dict[str, str]) -> dict[str, dict[str, str | bool]]:
    from products.warehouse_sources.backend.facade.insightsql import DATASTORE_INSIGHTSQL_MAPPING, clean_type

    return {
        key: {
            "insightsql": DATASTORE_INSIGHTSQL_MAPPING[clean_type(value)].__name__,
            "datastore": f"Nullable({value})",
            "valid": True,
        }
...
CUSTOMER_TEST_BUCKET = "test_storage_bucket-insights.revenue_analytics.customer_metadata_resolution.stripe_customers"
SUBSCRIPTION_TEST_BUCKET = (
    "test_storage_bucket-insights.revenue_analytics.customer_metadata_resolution.stripe_subscriptions"
)
INVOICE_TEST_BUCKET = "test_storage_bucket-insights.revenue_analytics.customer_metadata_resolution.stripe_invoices"
CHARGE_TEST_BUCKET = "test_storage_bucket-insights.revenue_analytics.customer_metadata_resolution.stripe_charges"


class TestStripeCustomerMetadataResolution(DatastoreTestMixin, APIBaseTest):
    ...
    def setUp(self):
        ...
        self.customer_csv.write(
            ...
            '"{""insights_person_distinct_id"": ""bob_distinct""}"\n'
        ...
        self.subscription_csv.write(
            ...
            '"{""insights_person_distinct_id"": ""alice_distinct""}"\n'
        )
    ...
    def _select_all_from_view(self, view_name: str) -> InsightsQLQueryResponse:
    ...
    def test_views_work_with_nullable_metadata_columns(self):
        for schema in [CUSTOMER_SCHEMA, REVENUE_ITEM_SCHEMA]:
            response = self._select_all_from_view(f"stripe.insights_test.{schema.source_suffix}")
