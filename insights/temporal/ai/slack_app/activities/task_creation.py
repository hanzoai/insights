...
from insights.temporal.ai.slack_app.attachments import (
    PreparedSlackAttachments,
    build_slack_attachment_prompt_text,
    get_slack_bot_token,
    prepare_slack_file_artifacts,
)
from insights.temporal.ai.slack_app.helpers import block_if_team_over_quota, safe_react
from insights.temporal.ai.slack_app.types import InsightsCodeSlackMentionWorkflowInputs, SlackAppModelOverride
...
- To update a prior deliverable, GET the returned artifact id or POST new `content`, `content_base64`, or source artifact fields to `$POSTFN_API_URL/api/projects/$POSTFN_PROJECT_ID/tasks/$POSTFN_TASK_ID/runs/$POSTFN_TASK_RUN_ID/living_artifacts/<artifact_id>/edit/`.
...
- For Slack deliverables, create a living artifact before claiming delivery. POST to `$POSTFN_API_URL/api/projects/$POSTFN_PROJECT_ID/tasks/$POSTFN_TASK_ID/runs/$POSTFN_TASK_RUN_ID/living_artifacts/` with `$POSTFN_PERSONAL_API_KEY` using adapter `slack_message`. To update a prior deliverable, GET the returned artifact id or POST new `content` to `$POSTFN_API_URL/api/projects/$POSTFN_PROJECT_ID/tasks/$POSTFN_TASK_ID/runs/$POSTFN_TASK_RUN_ID/living_artifacts/<artifact_id>/edit/`.
- Run artifacts that are not your uploaded outputs (plans, context, tree snapshots, checkpoints, user uploads) are internal: never deliver them to Slack or mention them in your reply.
...
def _slack_followup_message_id(channel: str, message_ts: str | None, thread_ts: str) -> str:
    ...
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"insights:slack_followup:{channel}:{message_ts or thread_ts}"))
...
def _build_insights_code_task_description(
    initiator_text: str,
    ...
    *,
    ...
    if mentioner_entry:
        if thread_author_entry and mentioner_entry["author"] == thread_author_entry["author"]:
            ...
            role_lines[-1] = (
                f"Thread started by and tagged the Insights app: {mentioner_entry['author']} "
                "(their message below the closing tag is the actual request)"
            )
        else:
            role_lines.append(
                f"Tagged the Insights app: {mentioner_entry['author']} "
                "(their message below the closing tag is the actual request)"
            )
...
def derive_mention_workflow_id(inputs: InsightsCodeSlackMentionWorkflowInputs) -> str:
    ...
    return f"insights-code-mention-{inputs.slack_team_id}:{suffix}"


@activity.defn
@close_db_connections
def create_insights_code_task_for_repo_activity(
    ...
    if SlackThreadTaskMapping.objects.filter(
        ...
        logger.info(
            "insights_code_task_creation_skipped_existing_mapping",
            channel=channel,
            thread_ts=thread_ts,
            integration_id=inputs.integration_id,
        )
    ...
    # Title is shown in Insights Desktop's UI (task lists, PR titles) where the
    ...
    description = _build_insights_code_task_description(
        user_text,
        thread_messages,
        user_message_ts,
    ...
    try:
        ...
        logger.warning("insights_code_slack_permalink_failed", channel=channel, thread_ts=thread_ts)
    ...
    # File into the creator's personal "#me" channel so the task surfaces in Insights Desktop's
    ...
    try:
        ...
        logger.warning(
            "insights_code_personal_channel_resolution_failed",
    ...
    try:
        ...
        logger.exception(
            "insights_code_task_creation_failed",
            error=str(e),
            team_id=integration.team_id,
            channel=channel,
            thread_ts=thread_ts,
        ...
        try:
            ...
            logger.warning("insights_code_error_notification_failed", channel=channel, thread_ts=thread_ts)
        return

    logger.info(
        "insights_code_task_created",
        team_id=integration.team_id,
        repository=repository,
        channel=channel,
        thread_ts=thread_ts,
    ...
    if task_run:
        ...
        try:
            ...
            logger.exception(
                "insights_code_persist_initial_run_state_failed",
...
@activity.defn
@close_db_connections
def forward_insights_code_followup_activity(
    inputs: InsightsCodeSlackMentionWorkflowInputs,
    channel: str,
    thread_ts: str,
    slack_user_id: str,
    ...
    try:
        ...
        logger.info("insights_code_followup_not_handled", channel=channel, thread_ts=thread_ts)
        return False
    ...
    if slack_user_id != mapping.mentioning_slack_user_id:
        ...
        # thread. Try to resolve them to a Insights user with access to the same team
        ...
        if not resolved:
            logger.info(
                "insights_code_followup_unauthorized_actor",
                channel=channel,
                thread_ts=thread_ts,
        ...
        # to the user's Insights email rather than interpolating literal "None: "
        ...
        logger.info(
            "insights_code_followup_cross_user_authorized",
            channel=channel,
            thread_ts=thread_ts,
    ...
    if not sandbox_url:
        logger.info("insights_code_followup_sandbox_not_ready", channel=channel, thread_ts=thread_ts)
        slack.client.chat_postMessage(
            channel=channel,
            thread_ts=thread_ts,
    ...
    logger.info("insights_code_followup_forwarded", channel=channel, thread_ts=thread_ts, task_run_id=str(task_run.id))
    return True
...
def _resume_task_with_new_run(
    ...
    extra_state: dict[str, Any] = {
        ...
        # Insights sub-tool gate stays open so the agent doesn't make a permission roundtrip.
        "initial_permission_mode": "bypassPermissions",
    ...
    try:
        ...
        logger.exception(
            "insights_code_resume_create_run_failed",
            channel=channel,
            thread_ts=thread_ts,
    ...
    if prepared_attachments.has_files:
        ...
        if pending_updates:
            try:
                ...
                logger.exception(
                    "insights_code_resume_attachment_state_update_failed",
                    channel=channel,
                    thread_ts=thread_ts,
                    task_id=str(mapping.task_id),
    ...
    try:
        ...
        logger.exception(
            "insights_code_resume_workflow_start_failed",
            channel=channel,
            thread_ts=thread_ts,
            task_id=str(mapping.task_id),
            run_id=str(new_run.id),
