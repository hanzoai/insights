...
description: >
  Signals scout for Insights AI observability. Watches LLM traces for cost, latency, error,
...
**`references/lenses.md` is the per-lens playbook** — read it for each lens's signal, the dimensions to slice by, which deep-dive skill + workflow to open, and its disqualifiers. The deep-dive skills (`exploring-llm-costs` / `-traces` / `-evaluations` / `-clusters`, plus `querying-insights-data` for InsightsQL) are baked into the sandbox and hold the actual, maintained queries — **read the matching one when you go deep on a lens rather than reinventing its SQL.**
...
- key `pattern:llm_analytics:dimensions` — _"Useful splits for this team: ai_product (insights_ai / code / mcp / wizard), model, feature. tenant_id not set."_
...
- **Provider-side incidents** — 429/5xx surges during a known upstream outage are not a Insights-side bug; check status timing first.
...
- `execute-sql` — the workhorse for trends and breakdowns over `$ai_*` events (read `insights:querying-insights-data` for InsightsQL discipline).
...
- `llma-evaluation-summary-create` — optional AI pass/fail/N/A pattern summary (billed, rate-limited, currently prone to 500s — a drill-down, not the spine). Pair with `llma-evaluation-get` / `-test-script`.
- `llma-tagger-list` / `llma-score-definition-list` — the enrichment config surface (auto-taggers and scorers — LLM/Script jobs that can silently break).
...
Deep-dive skills (baked into the sandbox — read the matching one when you go deep, don't reinvent its queries): `insights:exploring-llm-costs`, `insights:exploring-llm-traces`, `insights:exploring-llm-evaluations`, `insights:exploring-llm-clusters`, and `insights:querying-insights-data`. See `references/lenses.md` for which skill maps to which lens.
