...
## ::NB:: Sync this list with frontend/src/queries/nodes/InsightsQLX/render.tsx
## Sanitization is done on the client side, as we can never be sure what tuple('__hx_tag', 'tag_name') comes our way.
INSIGHTSQLX_COMPONENTS = ["Sparkline", "RecordingButton", "ExplainCSPReport"]
INSIGHTSQLX_TAGS_SPECIAL = [
    "a",
    "blink",
    "marquee",
    "redacted",
]
INSIGHTSQLX_TAGS_NO_ATTRIBUTES = [
    "em",
    "strong",
    "span",
    "div",
...
INSIGHTSQLX_TAGS = INSIGHTSQLX_TAGS_SPECIAL + INSIGHTSQLX_TAGS_NO_ATTRIBUTES


def convert_tag_to_hx(node: ast.InsightsQLXTag) -> ast.Tuple:
    attrs: list[ast.Expr] = [
