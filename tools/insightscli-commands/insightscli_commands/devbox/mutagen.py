...
Mirrors ``coder.py`` shape: a managed binary lives in ``~/.insightscli/bin/``,
...
_MANAGED_MUTAGEN_DIR = Path.home() / ".insightscli" / "bin"
...
_SSH_SHIM_DIR = Path.home() / ".insightscli" / "mutagen-ssh-shim"
_KEEPALIVE_COUNT = 3

# Baked at generation time (see ensure_ssh_shim): only when the ssh/scp target is
# a devbox (a `coder.*` host -- the sole thing insightscli syncs to) do we bump
...
# Managed by insightscli (`insightscli devbox:sync`) -- do not edit; regenerated on sync.
...
def _mutagen_platform() -> tuple[str, str]:
    ...
    if system not in ("darwin", "linux"):
        _fail(f"Unsupported OS for mutagen install: {system}. insightscli devbox:sync supports macOS and Linux only.")
    ...
    if machine in ("x86_64", "amd64"):
        ...
        _fail(f"Unsupported CPU arch for mutagen install: {machine}. insightscli devbox:sync supports amd64 and arm64 only.")
...
def _mutagen_bin() -> str:
    """Return the path to the insightscli-managed mutagen binary, falling back to PATH."""
...
def _install_mutagen(*, verbose: bool = False) -> None:
    """Install mutagen into ``~/.insightscli/bin`` from the official GitHub release.
...
def ensure_daemon_with_shim() -> None:
    ...
    register; instead we let mutagen auto-start the daemon as a child of a insightscli
...
_USER_CONFIG_PATH = Path.home() / ".insightscli" / "mutagen.yml"
...
def ensure_user_mutagen_config() -> Path:
    """Seed (and refresh) ``~/.insightscli/mutagen.yml`` from the packaged defaults.
...
# Single source of truth for the label insightscli stamps on every sync session, so the
# create side (sync_create) and the lookup side (selector) can never drift apart.
_WORKSPACE_LABEL_KEY = "insightscli-workspace"


def workspace_labels(workspace: str) -> dict[str, str]:
    """Return the labels insightscli attaches when creating a sync session for ``workspace``."""
    return {_WORKSPACE_LABEL_KEY: workspace}


def workspace_label_selector(workspace: str) -> str:
    """Return the mutagen ``--label-selector`` string for a insightscli workspace."""
