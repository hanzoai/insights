...
`KAFKA_DATASTORE_SESSION_REPLAY_EVENTS` — the same topic ingestion uses. The
...
from insights.temporal.session_replay.surfacing_scoring_sweep.activities import _publish_scores
from insights.temporal.session_replay.surfacing_scoring_sweep.constants import KAFKA_PRODUCE_FLUSH_TIMEOUT_S

ACTIVITIES_MODULE = "insights.temporal.session_replay.surfacing_scoring_sweep.activities"
    ...
    def test_publishes_one_message_per_row_and_flushes_once(self, id_frame: pd.DataFrame) -> None:
        ...
        with mock.patch(f"{ACTIVITIES_MODULE}.get_producer") as get_producer_mock:
            ...
            for call in producer.produce.call_args_list:
                assert call.kwargs["topic"] == KAFKA_DATASTORE_SESSION_REPLAY_EVENTS

            # One get_producer() lookup for the loop, none for flush (we reuse
            # the same singleton). flush is called once after the loop.
            get_producer_mock.assert_called_once_with(topic=KAFKA_DATASTORE_SESSION_REPLAY_EVENTS)
