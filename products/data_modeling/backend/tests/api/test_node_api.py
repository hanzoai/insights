...
from insights.temporal.common.logger import resolve_log_source
...
class TestNodeViewSet(APIBaseTest):
    def setUp(self):
        super().setUp()
        self.dag_id = f"insights_{self.team.id}"
    ...
    def test_list_nodes_filters_by_team(self):
        ...
        other_dag = DAG.objects.create(team=other_team, name=f"insights_{other_team.id}")
        Node.objects.create(
            team=other_team,
            dag=other_dag,
            name="other_table",
    ...
    def _suspend(self, node: Node) -> None:
        mark_node_suspended(node, engine="datastore", reason="boom", job_id=str(uuid4()), fingerprint=None)
        node.save()
    ...
    def test_run_resumes_every_node_it_was_asked_to_run(self, mock_sync_connect):
        ...
        downstream = Node.objects.create(
            ...
            saved_query=DataWarehouseSavedQuery.objects.create(
                name="downstream_view", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
            ),
            type=NodeType.VIEW,
        )
    ...
    def test_lineage_does_not_leak_other_teams_nodes(self, lookup_param):
        ...
        other_saved_query = DataWarehouseSavedQuery.objects.create(
            name="other_view", team=other_team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
        )
    ...
    def test_lineage_multi_level(self):
        sq_b = DataWarehouseSavedQuery.objects.create(
            name="view_b", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
        )
        sq_c = DataWarehouseSavedQuery.objects.create(
            name="view_c", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
        )
    ...
    def test_lineage_no_dependencies(self):
        sq_standalone = DataWarehouseSavedQuery.objects.create(
            name="standalone", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
        )
    ...
    def test_dag_field_rejects_other_teams_dag(self, method):
        other_team = Team.objects.create(organization=self.organization)
        foreign_dag = DAG.objects.create(team=other_team, name=f"insights_{other_team.id}")
    ...
    def _create_managed_node(self) -> Node:
        ...
        sq = DataWarehouseSavedQuery.objects.create(
            name="managed_view", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
        )
...
class TestNodeAccessControl(WarehouseAccessControlTestMixin):
    ...
    def setUp(self):
        super().setUp()
        self.dag = DAG.objects.create(team=self.team, name=f"insights_{self.team.id}")
        self.node = Node.objects.create(
            team=self.team,
            dag=self.dag,
            saved_query=DataWarehouseSavedQuery.objects.create(
                name="suspended_view", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
            ),
            type=NodeType.VIEW,
        )
        mark_node_suspended(self.node, engine="datastore", reason="boom", job_id=str(uuid4()), fingerprint=None)
        self.node.save()
...
class TestEdgeViewSet(APIBaseTest):
    ...
    def _create_managed_edge(self) -> Edge:
        ...
        sq = DataWarehouseSavedQuery.objects.create(
            name="m_view", team=self.team, query={"query": "SELECT 1", "kind": "InsightsQLQuery"}
        )
