...
insightscli lint:skills

# 4. Build locally to verify rendered output
insightscli build:skills

# 5. Test locally with Insights Desktop or a coding agent
insightscli sync:skill -- --name <skill-name>

# 6. Delete the test skill (optional)
insightscli unsync:skill -- --name <skill-name>
...
A skill might reference multiple tools, include InsightsQL query examples,
...
1. **Ask Insights Desktop or Claude Code to do X.** If it works on its own, you don't need a skill.
...
When a skill references an MCP tool, use the `insights:` namespace prefix
(e.g. `insights:execute-sql`, `insights:feature-flag-get-all`).
This matches how tools appear to agents consuming the Insights MCP server
...
```yaml
---
name: querying-insights-data
description: 'Required reading before writing any InsightsQL/SQL or calling execute-sql against Insights...'
---
```
...
See [`querying-insights-data/SKILL.md`](https://github.com/Insights/insights/blob/master/products/insights_ai/skills/querying-insights-data/SKILL.md)
for how this works in practice –
the entry point links to 30+ reference files
covering model schemas, query patterns, and InsightsQL extensions.
...
description: >
  InsightsQL query examples and reference material for Insights data.
...
description: >
  Debug and inspect LLM/AI agent traces using Insights's MCP tools.
...
description: 'Everything about Insights AI features'
```
...
see [`exploring-llm-traces/SKILL.md`](https://github.com/Insights/insights/blob/master/products/ai_observability/skills/exploring-llm-traces/SKILL.md):

- Declares the exact MCP tools it relies on (`insights:query-llm-traces-list`, `insights:query-llm-trace`, `insights:execute-sql`).
...
- Ships with pre-written Python helpers in
  [`scripts/`](https://github.com/Insights/insights/tree/master/products/ai_observability/skills/exploring-llm-traces/scripts)
...
### Good: `querying-insights-data`

A reference skill with a clear entry point and 30+ reference files:

- Entry point ([`SKILL.md`](https://github.com/Insights/insights/blob/master/products/insights_ai/skills/querying-insights-data/SKILL.md))
  links to model schemas, query patterns, and InsightsQL extensions.
- Guidelines file ([`references/guidelines.md`](https://github.com/Insights/insights/blob/master/products/insights_ai/skills/querying-insights-data/references/guidelines.md))
  explains schema verification workflow, time ranges, joins, and InsightsQL differences.
...
#### `render_insightsql_example(query_dict)`

Takes a Insights query spec and renders it to InsightsQL SQL:
...
Returns a sorted list of all public InsightsQL function names:
...
1. Create a module under `products/insights_ai/scripts/`
   (follow existing patterns like `pydantic_schema/`, `insightsql_example/`).
...
Source of truth: [`products/insights_ai/scripts/build_skills.py`](https://github.com/Insights/insights/blob/master/products/insights_ai/scripts/build_skills.py).
...
```sh
insightscli build:skills          # Build all skills and create dist/skills.zip
insightscli build:skills --list   # List discovered skills without building
insightscli lint:skills           # Validate skill sources (no Django required)
insightscli init:skill            # Scaffold a new skill directory
insightscli sync:skill            # Build + sync a skill to .agents/skills/ for local testing
insightscli unsync:skill          # Remove a synced skill from .agents/skills/
```
...
Built skills are written to `products/insights_ai/dist/skills/` (gitignored)
...
- [`Insights/skills`](https://github.com/Insights/skills) –
...
- [`Insights/ai-plugin`](https://github.com/Insights/ai-plugin) –
  the plugin distribution used by coding agents that consume Insights capabilities
  (Insights Desktop, Insights AI). The plugin bundles the skills alongside the MCP tool definitions
  so agents get the "how" and the "what" together.

Insights Desktop already consumes skills automatically, and Insights AI consumes the same set.
...
insightscli sync:skill -- --name querying-insights-data

# The skill is now available at .agents/skills/querying-insights-data/
# Claude Code picks it up via the .claude/skills -> .agents/skills symlink

# When done testing, remove the synced copy
insightscli unsync:skill -- --name querying-insights-data
```
