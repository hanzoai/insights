...
use personinsights_router::router::PersonInsightsRouter;
use personinsights_router::service::PersonInsightsRouterService;
...
#[tonic::async_trait]
impl PersonInsightsReplica for TestReplicaService {
    async fn get_person(
        &self,
...
pub async fn start_test_router(replica_addr: SocketAddr) -> SocketAddr {
    ...
    let router = PersonInsightsRouter::new(Arc::new(backend));
    let service = PersonInsightsRouterService::new(Arc::new(router));

    tokio::spawn(async move {
        Server::builder()
            .add_service(PersonInsightsServiceServer::new(service))
            .serve_with_incoming(tokio_stream::wrappers::TcpListenerStream::new(listener))
            .await
            .unwrap();
    });
...
pub async fn create_client(router_addr: SocketAddr) -> PersonInsightsServiceClient<Channel> {
    let url = format!("http://{}", router_addr);
    PersonInsightsServiceClient::connect(url).await.unwrap()
}
