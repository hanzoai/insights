...
from products.insights_ai.eval_harness import base
from products.insights_ai.eval_harness.engines.types import AggregateScore, EvalSummary
from products.insights_ai.eval_harness.harness.reporting import ProgressReporter, SuiteRunResult
from products.insights_ai.eval_harness.harness.transcript import RunTranscript
from products.insights_ai.eval_harness.scorers import ExitCodeZero
...
async def test_reporter_output_is_labeled_and_reserves_pass_for_the_run(
    ...
    await reporter.record_insights_evaluations_url(
    ...
    assert "Insights: https://us.hanzo.ai/" in output
...
def test_sandboxed_eval_run_adds_exit_code_scorer(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    ctx = MagicMock(insights_client=None, case_filter=None)
