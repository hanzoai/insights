...
def create_webhook(region: Optional[str], api_key: str, webhook_url: str) -> WebhookCreationResult:
    ...
    (``auth_type: "basic"``), so Insights generates a random password and stores the exact
    ``Authorization`` header SparkPost will send. The Script template rejects any delivery whose
