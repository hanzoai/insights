...
# CUSTOMER_IO_TEMPLATE_ID_MAP in insights/email.py.
...
def get_signed_pdf_presigned_url(document: LegalDocument) -> str | None:
    ...
    return object_storage.get_presigned_url(
        ...
        content_disposition=f'attachment; filename="Insights-{document.document_type}-{document.id}.pdf"',
    )
...
def send_pandadoc_envelope(document: LegalDocument) -> bool:
    ...
    try:
        client.send_document(
            ...
            subject=f"Please sign: Insights {document.document_type}",
            message=(
                f"Hi,\n\n"
...
def fire_legal_document_submitted_event(document: LegalDocument, distinct_id: str) -> None:
    """
    Capture the submission to Insights for analytics. No longer a critical path —
...
def fire_legal_document_signed_event(document: LegalDocument, distinct_id: str | None = None) -> None:
    """Capture the signed milestone to Insights for the same analytics funnel."""
