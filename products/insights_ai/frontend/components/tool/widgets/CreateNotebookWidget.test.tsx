...
import { lookupToolRenderer, toolRegistry } from 'products/insights_ai/frontend/api/tools'
...
describe('CreateNotebookWidget', () => {
    it.each(['notebooks-create', 'notebooks-partial-update', 'notebooks-retrieve', 'notebook-edit'])(
        ...
        (key) => {
            ...
            // The notebook widget is gated on a trusted Insights-exec origin, so pass `fromInsightsExec`.
    ...
    describe('extractNotebook', () => {
        it('reads short_id, title, and the _insightsUrl enrichment from the REST payload', () => {
            const notebook = extractNotebook(
                toolMessage({
                    ...
                    version: 1,
                    _insightsUrl: 'https://us.hanzo.ai/project/1/notebooks/aBcDe123',
                })
            )
            expect(notebook).toEqual({
                shortId: 'aBcDe123',
                title: 'Churn deep dive',
                url: 'https://us.hanzo.ai/project/1/notebooks/aBcDe123',
            })
        })
