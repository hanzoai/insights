...
description: >
  Triage Insights error tracking issues during a daily or on-call review.
...
| `insights:query-session-recordings-list`     | Find replays of users hitting an issue                                    |
| `insights:inbox-reports-list`                | Pre-curated actionable signals if the project uses Inbox                  |
...
- `web` — insights-js (browser)
- `insights-node`, `insights-python`, `insights-ruby`, `insights-go`, `insights-php`, `insights-java`, `insights-elixir` — server SDKs
- `insights-edge` — Cloudflare Workers / edge runtime
- `insights-ios`, `insights-android`, `insights-react-native`, `insights-flutter` — mobile
...
frame and URL, not just a title. Use `insights:query-error-tracking-issue-events` rather than
...
- If the project uses Inbox (`insights:inbox-reports-list`), check it first — Insights
  may have already curated the most actionable issues so you avoid re-deriving them.
- Provide each row's `_insightsUrl` (returned on every issue row) so the user can jump
