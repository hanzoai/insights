...
Pure data layer — no Braintrust, Insights, or model dependency. Same
...
``ToolSearch(select:mcp__insights__<tool>)`` as interchangeable
...
def _parse_exec_command(command: str) -> tuple[str, dict[str, Any]] | None:
    ...
    the Insights MCP through one ``exec`` tool):
