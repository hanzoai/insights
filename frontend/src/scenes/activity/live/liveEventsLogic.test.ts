    ...
    describe('addEvents host extraction', () => {
        it('records the host for a valid $current_url', async () => {
            await expectLogic(logic, () => {
                logic.actions.addEvents([makeLiveEvent('https://app.hanzo.ai/insights')])
            }).toMatchValues({
                eventHosts: ['https://app.hanzo.ai'],
            })
        })
        ...
        it.each([
            {
                ...
                properties: [
                    eventFilter('$current_url', PropertyOperator.Exact, 'https://app.hanzo.ai'),
                    eventFilter('$browser', PropertyOperator.IContains, 'chrome'),
                ],
                expectedProperty: ['$current_url=https://app.hanzo.ai'],
            },
            {
