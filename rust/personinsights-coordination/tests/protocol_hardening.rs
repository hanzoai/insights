...
#[async_trait]
impl personinsights_coordination::pod::HandoffHandler for FlakyHandoffHandler {
    ...
    async fn warm_partition(&self, partition: u32) -> Result<()> {
        if self.fail_warm.load(Ordering::SeqCst) {
            ...
            return Err(personinsights_coordination::error::Error::invalid_state(
...
async fn a_pod_attempt_failure_preserves_registration_and_partitions() {
    ...
    let pod = personinsights_coordination::pod::PodHandle::new(
        Arc::clone(&store),
        personinsights_coordination::pod::PodConfig {
