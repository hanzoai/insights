...
export class ConditionalBranchHandler implements ActionHandler {
    async execute({
        ...
        Extract<CustomFlowAction, { type: 'conditional_branch' | 'wait_until_condition' }>
...
export async function checkConditions(
    ...
    action: Extract<CustomFlowAction, { type: 'conditional_branch' }>
): Promise<{
    ...
    for (const [index, condition] of action.config.conditions.entries()) {
        ...
        if (filterResults.match) {
            return {
                nextAction: findNextAction(invocation.customFlow, action.id, index),
            }
        }
    }
