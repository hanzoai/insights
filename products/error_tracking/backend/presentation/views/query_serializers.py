...
from insights.api.documentation import PropertyItemSerializer, extend_schema_field
...
def validate_filter_group(value: list[dict[str, object]]) -> list[dict[str, object]]:
    for item in value:
        if item.get("type") == "insightsql":
            raise serializers.ValidationError("InsightsQL property filters are not supported here.")
    return value
...
class ErrorTrackingIssuesListQueryRequestSerializer(serializers.Serializer):
    ...
    filterGroup = serializers.ListField(
        ...
        required=False,
        default=list,
        help_text="Advanced flat AND property filters. Prefer typed shortcut fields when they fit. InsightsQL filters are rejected.",
    )
    ...
    library = StringOrStringListField(
        required=False, help_text="Filter by SDK/library value from event $lib, for example insights-js."
    )
    ...
    personId = serializers.UUIDField(required=False, help_text="Filter by exact Insights person UUID.")
...
class ErrorTrackingIssueEventsQueryRequestSerializer(serializers.Serializer):
    ...
    filterGroup = serializers.ListField(
        ...
        required=False,
        default=list,
        help_text="Advanced flat AND property filters applied to sampled events. InsightsQL filters are rejected.",
    )
    searchQuery = serializers.CharField(
        required=False,
        max_length=500,
