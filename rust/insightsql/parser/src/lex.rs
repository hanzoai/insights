//! InsightsQL lexer (default mode).
//!
//! Produces tokens matching the ANTLR grammar at
//! [`insights/insightsql/grammar/InsightsQLLexer.common.g4`] for everything outside the
//! template-string and InsightsQLX modes. Those modes maintain a stack and switch
...
pub enum TokenKind {
    ...
    QMark,   // `?`
    Hash,    // `#`
    LtSlash, // `</` for InsightsQLX closing tags (recognised even outside
    // tag mode so the parser can route on it)
    SlashGt, // `/>` for InsightsQLX self-closing tags
...
/// One variant per keyword recognised by the InsightsQL lexer. Kept in
...
const KEYWORDS: &[(&str, Kw)] = &[
    ...
    ("with", Kw::With),
    ("within", Kw::Within),
    // YEAR has a `YYYY` alias per InsightsQLLexer.common.g4 line 140
...
pub struct Lexer<'a> {
    src: &'a [u8],
    pos: usize,
    /// Mirrors cpp's `INSIGHTSQLX_TAG_OPEN` / `INSIGHTSQLX_TAG_CLOSE` lexer modes
    ...
    /// comment. Toggled by the InsightsQLX tag parser; see
    /// `parse_insightsqlx_tag_element`.
    in_insightsqlx_tag: bool,
}
...
impl<'a> Lexer<'a> {
    ...
    pub fn in_insightsqlx_tag(&self) -> bool {
        self.in_insightsqlx_tag
    }

    pub fn set_in_insightsqlx_tag(&mut self, on: bool) {
        self.in_insightsqlx_tag = on;
    }
    ...
    pub fn next_token(&mut self) -> Result<Token, ParseError> {
        ...
        let kind = match b {
            ...
            b'#' => match self.peek_byte(1) {
                ...
                _ if !self.in_insightsqlx_tag => {
                    self.skip_line_comment();
                    return self.next_token();
                }
                // Inside a InsightsQLX tag there is no HASH_COMMENT rule; emit
    ...
    fn skip_trivia(&mut self) -> Result<(), ParseError> {
        loop {
            ...
            // `--` or `//` line comment (InsightsQL accepts both forms).
            ...
            // default mode only — InsightsQLX tag modes reject `#` instead).
    ...
    fn lex_string(&mut self, start: usize) -> Result<TokenKind, ParseError> {
        ...
        loop {
            match self.peek_byte(0) {
                ...
                Some(b'\\') => {
                    // `ESCAPE_CHAR_COMMON` (`InsightsQLLexer.common.g4:145`)
