...
interface OpenApiParam {
    ...
    /**
     * Set by the Django factories in insights/api/openapi_parameters.py for
...
function loadOpenApi(): OpenApiSpec {
    if (!fs.existsSync(OPENAPI_PATH)) {
        console.error(`OpenAPI schema not found at ${OPENAPI_PATH}. Run \`insightscli build:openapi-schema\` first.`)
        process.exit(1)
    }
 ...
 * 'script-{id}' → { prefix: 'script-', field: 'id', source: 'result' }
...
function composeToolSchema(
    ...
    // in insights/api/openapi_parameters.py) are stringified-JSON-objects on the
...
function buildEnrichment(config: ToolConfig, category: CategoryConfig, resultVar = 'result'): string {
    ...
    if (config.list && config.enrich_url) {
        ...
        const enriched = [
            `await withInsightsUrl(context, {`,
            `            ...${resultVar},`,
            `            results: await Promise.all((${resultVar}.results ?? []).map((item) => withInsightsUrl(context, item, \`${baseUrl}${joiner}${prefix}\${item.${field}}${suffix}\`))),`,
            `        }, '${baseUrl}')`,
        ].join('\n')
    ...
    if (config.list) {
        return `        return ${wrapped(`await withInsightsUrl(context, ${resultVar}, '${baseUrl}')`)}\n`
    }
    ...
    if (config.enrich_url) {
        ...
        return `        return ${wrapped(`await withInsightsUrl(context, ${resultVar}, \`${baseUrl}${joiner}${prefix}\${${sourceExpr}}${suffix}\`)`)}\n`
    }
...
function generateToolCode(
    ...
    // Response enrichment — adds _insightsUrl for "View in Insights" links
    ...
    let needsWithInsightsUrl = false
    const hasEnrichment = !!(config.list || config.enrich_url)
    if (config.list && config.enrich_url) {
        needsWithInsightsUrl = !!responseType
        resultType = responseType ? `WithInsightsUrl<${responseType}>` : 'unknown'
...
function generateCategoryFile(
    ...
    for (const [name, config, resolved] of enabledTools) {
        ...
        if (result.needsWithInsightsUrl) {
            hasWithInsightsUrl = true
        }
    ...
    // Build tool-utils import (WithInsightsUrl type + withInsightsUrl runtime helper)
    const toolUtilsTypeImports: string[] = []
    const toolUtilsValueImports: string[] = []
    if (hasWithInsightsUrl) {
        toolUtilsTypeImports.push('WithInsightsUrl')
    }
    if (hasWithAgentNote) {
    ...
    if (hasEnrichment) {
        toolUtilsValueImports.push('withInsightsUrl')
    }
    if (hasAgentNote) {
