...
description: >
  Shared foundations for building reusable data models in Insights, on either of two stacks: Insights-native
  data-warehouse views / materialized views (InsightsQL, via the view-* MCP tools), or an external dbt project
  (sources.yml + staging/marts + schema tests) run against your own or Insights's managed warehouse. Read
  before authoring any specific business model — covers the Insights-vs-dbt decision, the view-create →
  view-materialize → sync_frequency workflow and the InsightsQL column-aliasing rule, the dbt project skeleton and
  ...
  materialized view, or dbt model in Insights, or which of the two stacks to use.
---
...
2. **Alias every column in a Insights view.** `insights:view-create` rejects `SELECT *` and any unaliased
...
4. **Don't build on the revenue _dashboard_.** Insights's standalone Revenue analytics dashboard is being
...
5. **dbt is not integrated into Insights.** There is no Insights dbt connector — dbt runs _externally_. See the
...
## Insights-native path

The lifecycle is: write InsightsQL → `view-create` (virtual view, re-runs on every read) → optionally
...
`sync_frequency` values, nesting, and cleanup: [`references/insights-views.md`](references/insights-views.md).

## dbt / external path

A conventional three-layer project: `sources.yml` declaring the Insights/warehouse tables you sync out, thin
...
`convertCurrency(from, to, amount, timestamp?)` InsightsQL function over a hand-rolled rate table. See
...
- Writing the InsightsQL itself: `querying-insights-data`. Checking view health afterwards:
