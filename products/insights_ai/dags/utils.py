...
def compose_datastore_dump_path(project_id: int, dir_name: str, code_version: str | None = None) -> str:
    """Compose S3 path for Datastore dumps with consistent hashing"""
    hash_suffix = get_consistent_hash_suffix(dir_name, code_version=code_version)
    return f"{EVALS_S3_PREFIX}/datastore_queries/{project_id}/{dir_name}/{hash_suffix}.avro"
...
class ResultsFormatter:
    ...
    def format(
        ...
        body_parts = [
            f"🧠 **AI eval results** for dataset [{self.dataset_name}](https://insights.hanzo.ai/llm-analytics/datasets/{self.dataset_id})",
    ...
    def _build_traces_url(self, result: dict[str, Any]) -> str:
        ...
        return f"https://insights.hanzo.ai/llm-analytics/traces?filters={urllib.parse.quote(json.dumps(traces_filter))}"
