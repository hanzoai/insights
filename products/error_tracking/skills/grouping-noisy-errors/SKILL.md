...
description: >
  Consolidate Insights error tracking issues that are the same actual
...
| `insights:error-tracking-issues-merge-create`   | Merge existing issues into a target                    |
| `insights:error-tracking-issues-split-create`   | Surgically split fingerprints back out if a merge errs |
| `insights:error-tracking-grouping-rules-create` | Auto-group future events into one issue                |
| `insights:error-tracking-grouping-rules-list`   | Check existing grouping rules before adding new ones   |
| `insights:error-tracking-issues-partial-update` | Rename or re-describe the target after a merge         |
...
```json
insights:query-error-tracking-issues-list
{
...
```json
insights:query-error-tracking-issue-events
{
...
- `$lib` is the same SDK. The browser/JS SDK captures `$lib` as `web` (not
  `insights-js`); server SDKs use `insights-python`, `insights-node`, etc. Confirm
...
- **Different SDKs / platforms.** `web` (browser/JS) vs `insights-python` vs
  `insights-node` are different call sites.
...
```json
insights:error-tracking-issues-merge-create
{
  "id": "<target_issue_id>",
...
function names) are arrays at capture time. Insights's property filters
...
`insights-js`, so a rule filtering on `insights-js` silently never matches. Verify
with `read-data-schema` (`event_property_values` for `$lib` on `$exception`)
before baking a value into the rule:

```json
insights:error-tracking-grouping-rules-create
{
  "filters": {
    "type": "AND",
    "values": [
...
(`insights:error-tracking-grouping-rules-list`) — if a rule already partially
