...
from products.review.backend.reviewer.lazy_seed import sync_canonical_blind_spots
from products.review.backend.reviewer.skill_loader import (
    REVIEW_FN_BLIND_SPOTS_PREFIX,
    REVIEW_FN_BLIND_SPOTS_SKILL_NAME,
    REVIEW_FN_PERSPECTIVE_PREFIX,
    REVIEW_FN_VALIDATION_PREFIX,
)
from products.skills.backend.models.skills import LLMSkill

_CUSTOM = f"{REVIEW_FN_BLIND_SPOTS_PREFIX}security-sweep"
...
class TestReviewBlindSpotsConfigAPI(APIBaseTest):
    def setUp(self) -> None:
        ...
        self.base = f"/v1/projects/{self.team.id}/review/blind_spots"
    ...
    def test_list_shows_canonical_active_and_custom_inactive(self) -> None:
        ...
        assert active_by_name[REVIEW_FN_BLIND_SPOTS_SKILL_NAME] is True
    ...
    def test_selecting_a_custom_skill_flips_the_canonical_off(self) -> None:
        ...
        assert configs.get(skill_name=REVIEW_FN_BLIND_SPOTS_SKILL_NAME).enabled is False
    ...
    def test_selecting_leaves_other_prefixes_enabled(self) -> None:
        ...
        others = (f"{REVIEW_FN_PERSPECTIVE_PREFIX}logic-correctness", f"{REVIEW_FN_VALIDATION_PREFIX}criteria")
        for name in others:
    ...
    def test_list_excludes_other_prefixes(self) -> None:
        ...
        self._author_custom(name=f"{REVIEW_FN_VALIDATION_PREFIX}criteria")

        res = self.client.get(f"{self.base}/")

        assert res.status_code == 200
        assert {item["skill_name"] for item in res.json()} == {REVIEW_FN_BLIND_SPOTS_SKILL_NAME}
    ...
    def test_select_rejects_deactivating(self) -> None:
        ...
        res = self.client.patch(f"{self.base}/{REVIEW_FN_BLIND_SPOTS_SKILL_NAME}/", {"active": False}, format="json")

        assert res.status_code == 400

    def test_patch_rejects_a_non_blind_spots_name(self) -> None:
        res = self.client.patch(
            f"{self.base}/{REVIEW_FN_PERSPECTIVE_PREFIX}logic-correctness/", {"active": True}, format="json"
        )
        assert res.status_code == 400

    def test_patch_404_for_unknown_skill(self) -> None:
        res = self.client.patch(
            f"{self.base}/{REVIEW_FN_BLIND_SPOTS_PREFIX}does-not-exist/", {"active": True}, format="json"
        )
    ...
    def test_environment_url_resolves_to_the_canonical_team(self) -> None:
        ...
        url = f"/v1/projects/{env.id}/review/blind_spots/{REVIEW_FN_BLIND_SPOTS_SKILL_NAME}/"
        ...
        config = ReviewSkillConfig.objects.for_team(self.team.id).get(
            user_id=self.user.id, skill_name=REVIEW_FN_BLIND_SPOTS_SKILL_NAME
        )
    ...
    def test_cold_team_can_select_the_canonical_immediately(self) -> None:
        ...
        res = self.client.patch(
            f"/v1/projects/{cold.id}/review/blind_spots/{REVIEW_FN_BLIND_SPOTS_SKILL_NAME}/",
            {"active": True},
            format="json",
        )
