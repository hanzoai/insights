...
/// Column list for `insights_cohort` queries. Must match the fields in `Cohort` (sqlx::FromRow).
...
impl Cohort {
    ...
    pub async fn list_from_pg(
        ...
        let query = format!(
            "SELECT {COHORT_COLUMNS} FROM insights_cohort AS c \
...
mod tests {
    ...
    fn test_dynamic_cohort_datetime_filter_uses_team_timezone() {
        ...
        // InsightsQL/Datastore cohort path. For a Pacific team, the filter "2024-06-01"
        ...
        // so they are not a member — matching InsightsQL cohort membership.
    ...
    fn test_evaluate_dynamic_cohorts_with_negated_cohort_reference_in_or_group() {
        ...
        let test_cases = [
            (json!("someone@hanzo.ai"), false), // in the negated cohort -> should NOT match
    ...
    fn test_deeply_nested_cohort_filters_error_instead_of_overflowing() {
        ...
        let leaf = json!({"key": "email", "type": "person", "value": "@hanzo.ai", "operator": "icontains"});
        ...
        let cohorts = vec![cohort];
        let target_properties = HashMap::from([("email".to_string(), json!("a@hanzo.ai"))]);
        assert!(matches!(
    ...
    fn test_evaluate_dynamic_cohorts_with_legacy_property_group_type() {
        ...
        let test_cases = [
            (json!("a@hanzo.ai"), json!("pro"), true), // both match -> AND true
            (json!("a@hanzo.ai"), json!("free"), false), // plan fails -> AND false
    ...
    fn test_cohort_with_between_leaf_parses_and_evaluates() {
        ...
        // and InsightsQL, but previously unknown to this evaluator) must parse as a real
