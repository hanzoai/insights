...
def parse_env_file(path: Path) -> dict[str, str]:
    ...
    Handles what Insights's env files actually use: blank lines, ``#`` comments,
...
def load_env_file() -> None:
    ...
    is loaded here: insightscli already loads ``.env.local`` / ``.env.development`` /
    ``.env.services`` before `insightscli evals:sandboxed` reaches this process, and its
    ...
    correctly. ``setdefault`` keeps insightscli's precedence: anything already exported
    (by the shell or by insightscli's env loading) wins over ``.env``.
    """
    ...
    *,
    ...
    if lines:
        raise PreflightError(
            ...
            "or keep them in .env.local and run via `insightscli evals:sandboxed`."
        )
