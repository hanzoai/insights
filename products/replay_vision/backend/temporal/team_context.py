...
def _core_event_names() -> frozenset[str]:
    ...
    from insights.taxonomy.taxonomy import CORE_FILTER_DEFINITIONS_BY_GROUP  # noqa: PLC0415
...
def fetch_event_descriptions(team: Team, columns: list[str], rows: list[list[Any]]) -> dict[str, str]:
    ...
    found = dict(
        EnterpriseEventDefinition.objects.filter(
            # Definitions are project-scoped with a team-scoped legacy fallback; mirrors insights/api/event_definition.py.
