...
description: >
  Get Insights data into a canvas correctly: the host-injected `ph` SDK (loadInsight, query,
  capture, openExternal, navigate), the data hierarchy (saved insights first, typed query nodes
  second, inline InsightsQL last), per-insight-type result shapes, date-range wiring, and event capture
  from a canvas. Use whenever a canvas shows metrics, charts, tables, or any Insights data, or
...
a canvas talks to Insights. Credentials stay in the host; `fetch()`, insights-js, and hand-rolled
clients fail in the sandbox.

## Data hierarchy — back every metric with a saved insight

1. **Preferred — save an insight, load it by reference.** Use the Insights MCP insight tools to
   ...
   Insights UI exactly (sessionization, unique users, breakdowns, bounce rate). Never fabricate a
...
3. **Last resort — inline InsightsQL**: `ph.query("SELECT …")`, only when no insight kind can express
   the metric; you then own the SQL and its date window.

For web-analytics boards specifically, use the web-analytics query kinds — raw InsightsQL subtly gets
...
`ph.loadInsight` short id in `capabilities.insights.insights`, every `ph.capture` event name in
...
- Inline InsightsQL escape hatch only: never bake `now()` or a hardcoded INTERVAL. Compute unix bounds
...
- `ph.openExternal(url)` — opens `https://hanzo.ai` / `*.hanzo.ai` URLs only, and only from
