...
│  ─ InsightsQL expression         │ → submenu / page: SQL editor → commit on save
└─────────────────────────────┘
```
...
hides power-user options (DWH / InsightsQL) inside a focused sub-flow so
...
- **InsightsQL expression**: only render when the consumer's
  `taxonomicGroupTypes` includes `InsightsQLExpression`.

These two groups (`DataWarehouse`, `InsightsQLExpression`) are **removed from
...
## InsightsQL expression flow

Click "InsightsQL expression" → menu closes → **expression editor opens**
...
- Category chips below — `All` plus one per visible group (excluding
  Recent / Pinned / Suggested / DataWarehouse / InsightsQLExpression — all
...
Only DWH config and InsightsQL editor live here. (Property "View details"
...
- **One open-state machine** at the parent level — discriminated union
  is fine: `{ kind: 'closed' } | { kind: 'menu' } | { kind: 'combobox', drillTo?: GroupType } | { kind: 'dwh-config', table } | { kind: 'insightsql-edit' }`. Transitions are explicit.
...
- **DWH / InsightsQL data is consumer-supplied** — no kea coupling inside the
