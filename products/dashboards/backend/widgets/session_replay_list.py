...
from insights.schema import RecordingOrder, RecordingOrderDirection, RecordingsQuery
...
from insights.session_recordings.playlist_filters import convert_playlist_to_recordings_query
from insights.session_recordings.session_recording_api import run_recordings_list_query
from insights.session_recordings.utils import filter_from_params_to_query, recordings_query_has_event_filters
...
def _build_collection_session_ids(team: Team, collection_id: str, user: User | None) -> list[str] | None:
    ...
    # them so no None leaks into session_ids and corrupts the Datastore IN clause. Cap the count so a huge
    # collection can't blow up the materialized list or the Datastore IN clause (the widget shows far fewer).
    return list(
