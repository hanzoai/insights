...
def print_iql_value(obj, marked: set | None = None):
    if marked is None:
        marked = set()
    if isinstance(obj, dict) and is_iql_datetime(obj):
        return f"DateTime({float(obj['dt'])}, {escape_string(obj['zone'] or 'UTC')})"
    if isinstance(obj, dict) and is_iql_date(obj):
        return f"Date({obj['year']}, {obj['month']}, {obj['day']})"
    if isinstance(obj, dict) and is_iql_error(obj):
        return (
            f"{obj['type']}({print_iql_value(obj['message'])}"
            + (f", {print_iql_value(obj['payload'])}" if "payload" in obj and obj["payload"] is not None else "")
            + ")"
        )
    if isinstance(obj, dict) and is_iql_closure(obj):
        return print_iql_value(obj["callable"], marked)
    if isinstance(obj, dict) and is_iql_callable(obj):
        return f"fn<{escape_identifier(obj.get('name', 'lambda'))}({print_iql_value(obj['argCount'])})>"
    if isinstance(obj, list) or isinstance(obj, dict) or isinstance(obj, tuple):
        ...
        try:
            if isinstance(obj, list):
                return f"[{', '.join([print_iql_value(o, marked) for o in obj])}]"
            if isinstance(obj, dict):
                return f"{{{', '.join([f'{print_iql_value(key, marked)}: {print_iql_value(value, marked)}' for key, value in obj.items()])}}}"
            if isinstance(obj, tuple):
                if len(obj) < 2:
                    return f"tuple({', '.join([print_iql_value(o, marked) for o in obj])})"
                return f"({', '.join([print_iql_value(o, marked) for o in obj])})"
        finally:
...
def print_iql_string_output(obj):
    if isinstance(obj, str):
        return str(obj)
    return print_iql_value(obj)
