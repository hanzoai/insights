...
from insights.schema import AgentMode
...
class TestProcessChatAgentActivity:
    ...
    async def test_process_conversation_activity_success(
        ...
        with (
            patch(
                "insights.temporal.ai.chat_agent.ConversationRedisStream", return_value=mock_redis_stream
            ) as mock_redis_stream_class,
            patch("insights.temporal.ai.chat_agent.ChatAgentRunner", return_value=mock_assistant),
        ):
    ...
    async def test_process_conversation_activity_streaming_error(
        ...
        with (
            patch("insights.temporal.ai.chat_agent.ConversationRedisStream", return_value=mock_redis_stream),
            patch("insights.temporal.ai.chat_agent.ChatAgentRunner", return_value=mock_assistant),
        ):
    ...
    async def test_process_conversation_activity_no_message(
        ...
        with (
            patch("insights.temporal.ai.chat_agent.ConversationRedisStream", return_value=mock_redis_stream),
            patch(
                "insights.temporal.ai.chat_agent.ChatAgentRunner", return_value=mock_assistant
    ...
    async def test_heartbeat_callback(self, conversation_inputs, mock_redis_stream, mock_assistant):
        ...
        with (
            patch("insights.temporal.ai.chat_agent.ConversationRedisStream", return_value=mock_redis_stream),
            patch("insights.temporal.ai.chat_agent.ChatAgentRunner", return_value=mock_assistant),
            patch("insights.temporal.ai.chat_agent.activity", mock_activity),
        ):
    ...
    async def test_starts_queued_workflow(self, conversation_inputs, mock_redis_stream, mock_assistant):
        ...
        with (
            patch("insights.temporal.ai.chat_agent.ConversationQueueStore", return_value=queue_store),
            patch("insights.temporal.ai.chat_agent.ConversationRedisStream", return_value=mock_redis_stream),
            patch("insights.temporal.ai.chat_agent.ChatAgentRunner", return_value=mock_assistant),
            patch("insights.temporal.ai.chat_agent.async_connect", return_value=mock_client),
        ):
