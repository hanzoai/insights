...
export const areAlertsSupportedForInsight = (
    ...
    return containsInsightsQLQuery(query)
}
        ...
        alertDestinationCounts: [
            {} as Record<string, number>,
            {
                loadAlertDestinationCounts: async (alerts: AlertType[]) => {
                    ...
                    return response.results.reduce<Record<string, number>>((counts, insightsFunction) => {
                        const alertIdProperty = insightsFunction.filters?.properties?.find(
