...
from insightscli_commands.workflow_lint.check import CheckResult, WorkflowCheck
from insightscli_commands.workflow_lint.checks import CHECKS, _build_lookup, get_check
from insightscli_commands.workflow_lint.checks.cache_writes import (
...
from insightscli_commands.workflow_lint.checks.checkout_full_depth import CheckoutFullDepthCheck
from insightscli_commands.workflow_lint.checks.dorny_negation import DornyNegationCheck
from insightscli_commands.workflow_lint.checks.job_timeouts import JobTimeoutsCheck
from insightscli_commands.workflow_lint.checks.pr_concurrency import PrConcurrencyCheck
from insightscli_commands.workflow_lint.checks.pr_event_fanout import PrEventFanoutCheck
from insightscli_commands.workflow_lint.checks.required_gates import RequiredGateCheck
from insightscli_commands.workflow_lint.checks.semgrep_services_coverage import SemgrepServicesCoverageCheck
from insightscli_commands.workflow_lint.cli import cmd_lint_workflows
from insightscli_commands.workflow_lint.model import PR_TRIGGERS, Workflow, WorkflowParseError, read_workflows
...
class TestPrConcurrencyCheck:
    ...
    @pytest.mark.parametrize(
        "marker,exempted",
        [
            ("# insightscli-lint: allow-master-cancel -- cache warmer, latest wins", True),
            ("# insightscli-lint: allow-master-cancel", False),
            ("# insightscli-lint: allow-master-cancel --", False),
        ],
    )
        ...
        _write(
            ...
            jobs:
              mirror:
                ...
                timeout-minutes: 5
                steps:
                  # insightscli-lint: allow-full-depth-checkout -- mirror needs full blobs
        ...
        _write(
            ...
            jobs:
              mirror:
                ...
                timeout-minutes: 5
                steps:
                  # insightscli-lint: allow-full-depth-checkout
                  - uses: actions/checkout@v6
                    with:
                      fetch-depth: 0
            """,
...
def _nested_allow_marker_gate() -> str:
    return _off_convention_gate().replace(
        ...
        """      changes:
        env:
          # insightscli-lint: not-a-required-gate - nested keys cannot exempt a job
          thing_tests:
""",
    )
...
class TestRequiredGateCheck:
    ...
    @pytest.mark.parametrize(
        "marker,exempted",
        [
            ("insightscli-lint: not-a-required-gate", False),
            ("insightscli-lint: not-a-required-gate — decides a side effect, emits no check", True),
        ],
