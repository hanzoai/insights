...
const actionCreate = (): ToolBase<typeof ActionCreateSchema, WithInsightsUrl<Schemas.Action>> =>
    withUiApp('action', {
        name: 'action-create',
...
const actionGet = (): ToolBase<typeof ActionGetSchema, WithInsightsUrl<Schemas.Action>> =>
    withUiApp('action', {
        name: 'action-get',
...
const actionUpdate = (): ToolBase<typeof ActionUpdateSchema, WithInsightsUrl<Schemas.Action>> =>
    withUiApp('action', {
        name: 'action-update',
...
const actionsGetAll = (): ToolBase<typeof ActionsGetAllSchema, WithInsightsUrl<Schemas.PaginatedActionList>> =>
                ...
                context,
                {
                    ...
                    results: await Promise.all(
                        (result.results ?? []).map((item) =>
                            withInsightsUrl(context, item, `/data-management/actions/${item.id}`)
                        )
                    ),
                },
