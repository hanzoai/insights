...
const PRS: PullRequestListItemApi[] = [
    ...
    apiPr({
        ...
        repo: { provider: 'github', owner: 'insights', name: 'insights-js' },
...
const SOURCES: GitHubSourceApi[] = [
    { id: 'src-older', repo: 'insights/insights', prefix: 'older', synced: true },
    { id: 'src-newer', repo: 'insights/hanzo.ai', prefix: 'website', synced: true },
]
...
describe('engineeringAnalyticsLogic', () => {
    ...
    it('filters by state, author, repo, ci status, and search', () => {
        const rows = [
            ...
            makePr({ number: 2, state: 'merged', authorHandle: 'bob', repoName: 'insights-js' }),
    ...
    it('exposes source options and the multi-source flag only when more than one source exists', async () => {
        ...
        expect(logic.values.sourceOptions).toEqual([
            { value: 'src-older::insights/insights', label: 'insights/insights' },
            { value: 'src-newer::insights/hanzo.ai', label: 'insights/hanzo.ai' },
        ])
    })
    ...
    it('lists one option per configured repo of a multi-repo source and scopes to the picked repo', async () => {
        ...
        mockSources.mockResolvedValue([
            { id: 'src-multi', repo: 'insights/insights', prefix: 'multi', synced: true },
            { id: 'src-multi', repo: 'insights/hanzo.ai', prefix: 'multi', synced: true },
        ])
        logic = engineeringAnalyticsLogic()
        logic.mount()
        ...
        expect(logic.values.hasMultipleSources).toBe(true)
        expect(logic.values.sourceOptions).toEqual([
            { value: 'src-multi::insights/insights', label: 'insights/insights' },
            { value: 'src-multi::insights/hanzo.ai', label: 'insights/hanzo.ai' },
        ])

        logic.actions.setScope('src-multi', 'insights/hanzo.ai')
        await expectLogic(logic).toDispatchActions(['setScope', 'loadCardsSuccess'])
        expect(logic.values.selectedScope).toBe('src-multi::insights/hanzo.ai')
        expect(mockCiCards).toHaveBeenLastCalledWith('1', { source_id: 'src-multi', repo: 'insights/hanzo.ai' })
    })
    ...
    it.each([
        ...
        router.actions.push(url(), { source: 'src-newer', repo: 'insights/hanzo.ai' })
        await expectLogic(logic).toDispatchActions(['setScope'])
        expect(logic.values.sourceId).toBe('src-newer')
        expect(logic.values.scopeRepo).toBe('insights/hanzo.ai')
    })
    ...
    it('resetFilters returns every filter to defaults and clears hasActiveFilters', async () => {
        ...
        logic.actions.setRepo('insights/insights')
    ...
    it('maps the quarantine endpoint into rows with counts and owner options', async () => {
        ...
        expect(logic.values.quarantine?.repoFullName).toBe('Insights/insights')
    ...
    it.each([
        ['product: selector', 'product:batch-exports', '@Insights/team-batch-exports'],
        ['products/ path', 'products/web_analytics/backend/test_foo.py::T::t', '@Insights/team-web-analytics'],
        ['plain nodeid', 'insights/api/test/test_foo.py::T::t', ''],
    ...
    it('a successful submit closes the modal and reloads the register', async () => {
        ...
        expect(mockQuarantineRequest).toHaveBeenCalledWith(
            '1',
            expect.objectContaining({ operation: 'quarantine', repo: 'Insights/insights', runner: 'jest' })
        )
        expect(logic.values.quarantineModal).toBeNull()
    ...
    it('a failed submit keeps the modal open so the user can retry', async () => {
        ...
        mockQuarantineRequest.mockRejectedValue({ detail: "The App isn't installed on Insights." })
        logic = engineeringAnalyticsLogic()
        logic.mount()
