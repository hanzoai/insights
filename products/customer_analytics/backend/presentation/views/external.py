...
from insights.api.utils import ErrorResponseSerializer
from insights.auth import ProjectSecretAPIKeyAuthentication
from insights.models import Team
from insights.permissions import get_authenticator_scopes, is_authenticated_via_project_secret_api_key
from insights.rate_limit import PersonalOrProjectSecretApiKeyRateThrottle, ProjectSecretApiKeyTeamRateThrottle
...
def _workflow_id_from_request(request: Request) -> str | None:
    """The originating InsightsFlow workflow id, when the request comes from a workflow step.
    ...
    """
    insights_flow_id = request.headers.get(INSIGHTS_FLOW_ID_HEADER)
...
class ExternalAccountUpdateSerializer(serializers.Serializer):
    ...
    # Each value accepts a `insights_assignee` object `{type, id}`, or `null` to end the
...
class ExternalAccountAssignmentSerializer(serializers.Serializer):
    user_id = serializers.IntegerField(help_text="Insights user id of the assigned user.")
