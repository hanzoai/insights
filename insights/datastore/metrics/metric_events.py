...
from insights.datastore.table_engines import Distributed, MergeTreeEngine, ReplacingMergeTree, ReplicationScheme
...
# metric->trace pivot. Lives on the logs Datastore cluster next to logs/traces.
#
...
def METRIC_SERIES_TABLE_SQL():
    return f"""
CREATE TABLE IF NOT EXISTS {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{SERIES_TABLE_NAME}
(
    `team_id` Int32,
...
def METRIC_SAMPLES_TABLE_SQL():
    return f"""
CREATE TABLE IF NOT EXISTS {settings.DATASTORE_LOGS_CLUSTER_DATABASE}.{SAMPLES_TABLE_NAME}
(
    `team_id` Int32,
    `metric_name` LowCardinality(String),
    `series_fingerprint` UInt64 CODEC(DoubleDelta),
...
def _distributed_sql(distributed_name: str, data_table: str) -> str:
    return "CREATE TABLE IF NOT EXISTS {database}.{distributed} AS {database}.{table_name} ENGINE = {engine}".format(
        ...
        engine=Distributed(data_table=data_table, cluster=settings.DATASTORE_LOGS_CLUSTER),
