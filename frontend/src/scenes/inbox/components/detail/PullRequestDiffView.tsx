...
// `themeType`, which we drive off Insights's own light/dark state.
 ...
 * our own (`renderCustomHeader`) so it reads in Insights's visual language. Inspection only — no
...
export function PullRequestDiffView({
    ...
    return (
        <div className="flex flex-col gap-3">
            {files.map((file) => (
                // Card chrome (bordered, rounded surface) so the diff sits in Insights's visual language;
