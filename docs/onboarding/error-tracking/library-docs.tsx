...
export const JavaErrorTrackingInstallation = createInstallation(
    createLibraryDocsSteps({
        ...
        docsLink: 'https://hanzo.ai/docs/libraries/java#request-context',
        language: 'java',
        code: 'insights.captureException(error);',
    })
)
...
export const KMPErrorTrackingInstallation = createInstallation(
    createLibraryDocsSteps({
        ...
        guidance:
            'The Kotlin Multiplatform SDK supports handled exceptions through `Insights.captureException`. The shared KMP API does not currently enable automatic exception capture.',
        docsLink: 'https://hanzo.ai/docs/libraries/kmp#error-tracking',
        language: 'kotlin',
        code: `try {
    riskyOperation()
...
export const ConvexErrorTrackingInstallation = createInstallation(
    createLibraryDocsSteps({
        ...
        guidance:
            "Configure Convex's first-party Insights Error Tracking destination to capture uncaught exceptions automatically. Convex exception reporting currently requires a Convex Pro plan. Use `captureException` for handled errors that need custom properties.",
        docsLink: 'https://hanzo.ai/docs/libraries/convex#6-capture-exceptions-with-custom-properties',
        language: 'typescript',
        code: `await insights.captureException(ctx, {
    error,
    distinctId: userId,
