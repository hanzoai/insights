...
create or replace TABLE logs AS logs32 ENGINE = Distributed('insights', 'default', 'logs32');
...
CREATE OR REPLACE TABLE trace_spans_distributed AS trace_spans ENGINE = Distributed('insights', 'default', 'trace_spans');
...
CREATE OR REPLACE TABLE trace_attributes_distributed AS trace_attributes ENGINE = Distributed('insights', 'default', 'trace_attributes');
...
ENGINE = Kafka('kafka:9092', 'datastore_traces', 'datastore-spans-avro', 'Avro')
SETTINGS
...
select 'datastore logs tables initialised successfully!';
