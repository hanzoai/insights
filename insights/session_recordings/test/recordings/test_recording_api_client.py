...
class TestDeleteRecordings:
    ...
    async def test_successful_delete(self, client, mock_session):
        ...
        result = await client.delete_recordings(["s1", "s2"], 1, deleted_by="test@hanzo.ai")

        assert result == []
        mock_session.post.assert_called_once_with(
            "http://localhost:6740/api/projects/1/recordings/delete",
            json={"session_ids": ["s1", "s2"], "deleted_by": "test@hanzo.ai"},
        )
...
class TestRecordingApiClientContextManager:
    ...
    async def test_raises_error_when_url_not_configured(self):
        with patch("insights.session_recordings.recordings.recording_api_client.settings") as mock_settings:
    ...
    async def test_creates_client_with_configured_url(self):
        with patch("insights.session_recordings.recordings.recording_api_client.settings") as mock_settings:
            ...
            with patch(
                "insights.session_recordings.recordings.recording_api_client.aiohttp.ClientSession"
    ...
    async def test_warns_when_secret_missing_in_production(self):
        with (
            patch("insights.session_recordings.recordings.recording_api_client.settings") as mock_settings,
            patch("insights.session_recordings.recordings.recording_api_client.logger") as mock_logger,
            patch(
                "insights.session_recordings.recordings.recording_api_client.aiohttp.ClientSession"
            ) as mock_client_session,
        ):
