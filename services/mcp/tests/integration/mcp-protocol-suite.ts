...
export type ProtocolTestHarness = {
     ...
     * Insights API. Tests that hit the upstream API (projects-get, switch-project,
...
function buildStreamableClient(
    ...
    const transport = new StreamableHTTPClientTransport(new URL('/mcp', harness.baseUrl), {
        ...
        fetch: harness.fetch,
        requestInit: { headers: { Authorization: `Bearer ${token}`, 'x-insights-mcp-mode': 'tools' } },
    })
...
export function defineMcpProtocolTests(
    ...
    describe(`MCP protocol (${label})`, () => {
        ...
        it('reports server name and version after the initialize handshake', () => {
            ...
            expect(info?.name).toBe('Insights')
        ...
        it('exposes a known Insights tool (organization-get)', async () => {
            const { tools } = await client.listTools()
        ...
        it('reads a context-mill resource by URI', async () => {
            const { resources } = await client.listResources()
            const cmResource = resources.find((r) => r.uri.startsWith('insights://'))
            if (!cmResource) {
                throw new Error('expected at least one insights:// resource from context-mill')
            }
        ...
        it('returns empty contents for an unknown resource URI', async () => {
            ...
            const result = await client.readResource({ uri: 'insights://does-not-exist' })
        ...
        it('isolates state between concurrent sessions on different tokens', async ({ skip }) => {
            ...
            try {
                ...
                expect(a.client.getServerVersion()?.name).toBe('Insights')
                expect(b.client.getServerVersion()?.name).toBe('Insights')
...
export function defineSessionTrackingTests(
    ...
    describe(`MCP session tracking (${label})`, () => {
        ...
        it('exec mode gets a session ID that persists through calls', async () => {
            const harness = await getHarness()
            await connectClient(harness, { 'x-insights-mcp-mode': 'cli' })
            const sessionId = transport.sessionId
...
export function defineHttpRouteTests(
    ...
    describe(`MCP HTTP routes (${label})`, () => {
        it('redirects GET / to the docs', async () => {
            ...
            expect(res.headers.get('location')).toContain('hanzo.ai')
        })
        ...
        it('redirects /.well-known/oauth-authorization-server to the authorization server', async () => {
            ...
            // `oauth.hanzo.ai`, CI runs a local auth server at
...
export function defineAuthTests(
    ...
    describe(`MCP auth (${label})`, () => {
        ...
        it('returns 401 for a token that does not look like a Insights API key', async () => {
            const harness = await getHarness()
            ...
            // that started letting bad-prefix tokens through to Insights would
...
// Tool behavior against the real Insights API. These tests assert on the actual
...
export function defineToolBehaviorTests(
    ...
    describe(`MCP tool behavior (${label})`, () => {
        ...
        it('switch-project validates the project: a non-existent project id returns an error', async () => {
            // switch-project fetches the project from Insights before committing
        ...
        it('execute-sql runs a trivial InsightsQL query against the upstream', async ({ skip }) => {
            if (!harness.projectId) {
    ...
    describe(`MCP catalog filtering (${label})`, () => {
        async function listToolsWithQuery(
            ...
            const transport = new StreamableHTTPClientTransport(url, {
                ...
                fetch: harness.fetch,
                requestInit: { headers: { Authorization: `Bearer ${harness.token}`, 'x-insights-mcp-mode': 'tools' } },
            })
...
export function defineSessionLifecycleTests(
    ...
    describe(`MCP session lifecycle (${label})`, () => {
        ...
        it('advertises tools, resources, and prompts capabilities on initialize', async () => {
            ...
            expect(json.result?.serverInfo?.name).toBe('Insights')
        ...
        it('handles a repeated initialize on the same connection', async () => {
            ...
            expect(aJson.result?.serverInfo?.name).toBe('Insights')
            expect(bJson.result?.serverInfo?.name).toBe('Insights')
        })
    })
}
...
export function defineResourceCatalogTests(
    ...
    describe(`MCP resource catalog (${label})`, () => {
        ...
        it('lists both UI app and context-mill resources', async () => {
            ...
            expect(uris.some((u) => u.startsWith('insights://'))).toBe(true)
        })
        ...
        it('reads two distinct resources without cross-contamination', async () => {
            ...
            const cm = resources.find((r) => r.uri.startsWith('insights://'))
            if (!ui || !cm) {
                throw new Error('expected at least one ui:// and one insights:// resource')
            }
...
// tools. Clients send `x-insights-mcp-mode: cli` to enter this mode. The exec
...
export function defineStatelessProtocolTests(
    ...
    describe(`MCP stateless protocol 2026-07-28 (${label})`, () => {
        ...
        it('answers server/discover with no prior handshake and no session minting', async () => {
            ...
            expect(result.serverInfo).toMatchObject({ name: 'Insights' })
            ...
            expect(serverInfoOf(json)?.name).toBe('Insights')
        ...
        it('merges server identity into a tool result that carries its own _meta', async () => {
            // Only the exec path with a Insights Desktop consumer puts `_meta`
            ...
            const { json } = await postSingle(
                ...
                1,
                { 'x-insights-mcp-mode': 'cli', 'x-insights-mcp-consumer': 'insights-code' }
            )
