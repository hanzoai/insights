...
Insights's canvas-based charting library, used for trends, dashboards, and any in-app chart that needs to render thousands of points smoothly. D3 powers the scales; the canvas does the drawing; React handles overlays.

```tsx
import { LineChart } from '@hanzo/quill-charts'
import type { ChartTheme, Series } from '@hanzo/quill-charts'
...
**Tokens (optional but recommended)** — load `@hanzo/quill-tokens/color-system.css`
...
those generated for a non-Insights host, add this package to your Tailwind
...
- `DEFAULT_CHART_COLORS` — fallback palette used when the token vars aren't loaded
  ...
  not black. Load `@hanzo/quill-tokens/color-system.css` to get the real tokens.
...
```tsx
import { Sparkline } from '@hanzo/quill-charts'
