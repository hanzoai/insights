...
from insights.insightsql_queries.sessions_query_runner import SUPPORTED_PERSON_PROPERTY_OPERATORS, SessionsQueryRunner
...
class TestSessionsQueryRunner(DatastoreTestMixin, APIBaseTest):
    ...
    def test_to_query_generates_insightsql_ast(self):
        """Test to_query generates valid InsightsQL AST structure."""
        query = SessionsQuery(
            after="2024-01-01",
            kind="SessionsQuery",
    ...
    def test_person_display_name_field(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert person_display["display_name"] == "user1@hanzo.ai"
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                ["name", "email"],
                {"name": "Person user1", "email": "user1@hanzo.ai"},
                "Person user1",
            ),
        ]
    )
    ...
    def test_person_display_name_combined_with_other_columns(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert row[1]["display_name"] == "user1@hanzo.ai"
    ...
    def test_orderby_person_display_name(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert display_names == ["userC@hanzo.ai", "userB@hanzo.ai", "userA@hanzo.ai"]
    ...
    def test_person_display_name_multiple_persons(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            display_names = [row[1]["display_name"] for row in response.results]
            assert display_names == ["alice@hanzo.ai", "bob@hanzo.ai", "charlie@hanzo.ai"]
    ...
    def test_arbitrary_person_property_field(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert isinstance(response, CachedSessionsQueryResponse)
            assert len(response.results) == 1
            assert response.results[0][1] == "user1@hanzo.ai"
    ...
    def test_arbitrary_person_property_orderby(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert emails == ["alice@hanzo.ai", "bob@hanzo.ai", "charlie@hanzo.ai"]
    ...
    def test_person_property_combined_with_display_name(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert isinstance(response, CachedSessionsQueryResponse)
            assert len(response.results) == 1
            assert response.results[0][1]["display_name"] == "user1@hanzo.ai"
    ...
    def test_filter_by_person_property(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            query = SessionsQuery(
                ...
                select=["session_id", "person.properties.email"],
                properties=[
                    PersonPropertyFilter(key="email", value="bob@hanzo.ai", operator="exact", type="person")
                ],
            )
            ...
            assert isinstance(response, CachedSessionsQueryResponse)
            assert len(response.results) == 1
            assert response.results[0][1] == "bob@hanzo.ai"
    ...
    def test_filter_by_person_property_icontains(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert isinstance(response, CachedSessionsQueryResponse)
            assert len(response.results) == 1
            assert response.results[0][1] == "alice@hanzo.ai"
    ...
    def test_session_property_column(self):
        ...
        _create_person(
            team_id=self.team.pk,
            distinct_ids=["user1"],
            properties={"email": "user1@hanzo.ai"},
        )
        _create_event(
            team=self.team,
            event="$pageview",
    ...
    def test_session_property_with_person_property(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            assert isinstance(response, CachedSessionsQueryResponse)
            assert len(response.results) == 1
            assert response.results[0][2] == "user1@hanzo.ai"
    ...
    def test_anonymous_session_identified_later(self):
        ...
        _create_person(
            ...
            properties={"email": "identified@hanzo.ai", "name": "Identified User"},
        )
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            person_display = response.results[0][1]
            assert person_display["display_name"] == "identified@hanzo.ai"
            assert response.results[0][2] == "identified@hanzo.ai"
    ...
    @parameterized.expand(
        [
            ("exact_match", "exact", "user1@hanzo.ai", True),
            ("exact_no_match", "exact", "other@hanzo.ai", False),
            ("is_not_match", "is_not", "other@hanzo.ai", True),
            ("is_not_no_match", "is_not", "user1@hanzo.ai", False),
            ...
            ("starts_with_no_match", "starts_with", "insights", False),
            ("not_starts_with_match", "not_starts_with", "insights", True),
            ("not_starts_with_no_match", "not_starts_with", "user1", False),
            ("ends_with_match", "ends_with", "hanzo.ai", True),
            ("ends_with_no_match", "ends_with", "user1", False),
            ("not_ends_with_match", "not_ends_with", "user1", True),
            ("not_ends_with_no_match", "not_ends_with", "hanzo.ai", False),
            ("regex_match", "regex", r"user\d+@insights\.com", True),
            ("regex_no_match", "regex", r"admin@.*", False),
            ("not_regex_match", "not_regex", r"admin@.*", True),
            ("not_regex_no_match", "not_regex", r"user\d+@insights\.com", False),
    ...
    def test_session_id_event_property_filter_short_circuits_events_subquery(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            # The generated InsightsQL should NOT contain "FROM events"
            assert "FROM events" not in response.insightsql
    ...
    def test_session_id_event_property_with_other_event_filters_keeps_subquery(self):
        ...
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            printed = to_printed_insightsql(insightsql, team=self.team)
    ...
    def test_default_after_is_one_hour(self):
        with freeze_time("2024-01-01T14:00:00Z"):
            ...
            runner = SessionsQueryRunner(query=query, team=self.team)
            printed = to_printed_insightsql(runner.to_query(), team=self.team)
