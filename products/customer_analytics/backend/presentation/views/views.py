...
from insights.api.tagged_item import TaggedItemViewSetMixin
from insights.exceptions import Conflict
from insights.helpers.impersonation import is_impersonated
from insights.models.user import User
from insights.permissions import get_authenticator_scopes, is_service_auth
from insights.rbac.access_control_api_mixin import AccessControlViewSetMixin
from insights.rbac.user_access_control import UserAccessControl, model_to_resource
...
class CustomerJourneyViewSet(
    ...
    def create(self, request: Request, *args, **kwargs) -> Response:
        ...
        if not api.insight_belongs_to_team(self.team_id, data.insight):
            raise ValidationError({"insight": "The insight does not belong to this project."})
        try:
...
def _synthesize_notebook_content(text_content, existing_content):
    ...
    renders ``content`` — so without this the result is a blank page. Returns ``None`` when the
    caller already supplied usable content (or no markdown).

    Built by the notebooks product's own converter, which keeps the Markdown in a markdown node
    rather than flattening it into ProseMirror nodes up front. That is the same representation a
    notebook created from Markdown anywhere else in the product carries, so these render and
    round-trip back to Markdown identically."""
    has_usable_content = (
