...
# other consumers has to have one removed by hand before Insights can be added.
...
def _register_webhook(
    ...
    if len(existing_urls) >= MAX_WEBHOOK_URLS_PER_TYPE:
        raise ValueError(f"already has {MAX_WEBHOOK_URLS_PER_TYPE} URLs, remove one in Mailgun before adding Insights")
...
def create_webhook(api_key: str, region: str, webhook_url: str) -> WebhookCreationResult:
    """Register Insights on every Mailgun webhook type, on every sending domain of the account.
...
def delete_webhook(api_key: str, region: str, webhook_url: str) -> WebhookDeletionResult:
    """Remove Insights from every webhook type it was registered on, leaving other consumers'
    URLs in place."""
    base_url = base_url_for_region(region)
    session = make_tracked_session(redact_values=(api_key,))
