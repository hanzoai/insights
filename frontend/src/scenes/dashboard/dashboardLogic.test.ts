...
import * as dashboardWidgetUtils from '@hanzo/products-dashboards/frontend/utils'
...
import { InsightsQLVariable, InsightVizNode, NodeKind, TrendsQuery } from '~/queries/schema/schema-general'
...
describe('dashboardLogic', () => {
    ...
    describe('tile streaming failure classification', () => {
        let toastErrorSpy: jest.SpiedFunction<typeof toast.error>

        beforeEach(silenceKeaLoadersErrors)
        afterEach(resumeKeaLoadersErrors)

        beforeEach(() => {
            toastErrorSpy = jest.spyOn(toast, 'error').mockImplementation(() => 'toast-id')
            logic = dashboardLogic({ id: 5 })
            logic.mount()
        })
        ...
        it('routes a 403 status to access denied and other errors to a toast', async () => {
            ...
            expect(toastErrorSpy).toHaveBeenCalledWith(expect.stringContaining('something broke'))
        })
    ...
    describe('dashboard variables', () => {
        ...
        const mountDashboardWithVariable = async ({
            ...
            dashboardOverride?: Partial<InsightsQLVariable>
            insightOverride?: Partial<InsightsQLVariable>
        }): Promise<void> => {
            router.actions.push(
                '/',
                urlValue === undefined
        ...
        it.each([
            ...
            async (
                ...
                dashboardOverride: Partial<InsightsQLVariable> | undefined,
                insightOverride: Partial<InsightsQLVariable> | undefined,
    ...
    describe('text tiles', () => {
        let toastInfoSpy: jest.SpiedFunction<typeof toast.info>

        beforeEach(async () => {
            toastInfoSpy = jest.spyOn(toast, 'info').mockImplementation(() => 'toast-id')
            logic = dashboardLogic({ id: 5 })
            logic.mount()
            await expectLogic(logic).toFinishAllListeners()
        })
        ...
        it('shows undo toast when removing a text tile', async () => {
            ...
            const toastContent = toastInfoSpy.mock.calls.at(-1)?.[0]
        ...
        it('restores the tile and suppresses the undo toast when the API call fails', async () => {
            ...
            const toastErrorSpy = jest.spyOn(toast, 'error').mockImplementation(() => 'toast-id')
