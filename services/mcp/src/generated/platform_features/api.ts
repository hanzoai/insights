 ...
 *
 * Insights API - MCP 22 enabled ops
...
export const PartialUpdateBody = /* @__PURE__ */ zod.object({
    ...
    is_ai_training_opted_in: zod
        .boolean()
        .nullish()
        .describe('When True, this organization allows its data to be used to train Insights AI models.'),
...
export const AdvancedActivityLogsListQueryParams = /* @__PURE__ */ zod.object({
    ...
    clients: zod
        ...
        .describe('Filter by API clients that generated the activity (from x-insights-client header).'),
    detail_filters: zod
        .string()
        .optional()
        .describe(
    ...
    insightsql_filter: zod.string().optional().describe('Reserved for future InsightsQL-based filtering.'),
    ip_addresses: zod
        .array(zod.string())
...
/**
 * Update the authenticated user's pinned sidebar tabs and/or homepage for the current team. Pass `@me` as the UUID. Send `tabs` to replace the pinned tab list, `homepage` to set the home destination (any Insights URL — dashboard, insight, search results, scene). Either field may be omitted to leave it unchanged; sending `homepage: null` or `{}` clears the homepage.
 */
...
export const UserHomeSettingsPartialUpdateBody = /* @__PURE__ */ zod.object({
    ...
    homepage: zod
        ...
        .optional()
        .describe(
            "Tab descriptor for the user's chosen home page — the destination opened when they click the Insights logo or hit `\/`. Set to a tab descriptor to pick a homepage, send `null` or `{}` to clear it and fall back to the project default."
        ),
})
