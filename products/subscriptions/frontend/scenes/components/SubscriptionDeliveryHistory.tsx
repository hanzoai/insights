...
function deliveryStatusTag(row: SubscriptionDeliveryApi): JSX.Element {
    ...
    return partialDeliveryTag(row) ?? <Tag type={tagType}>{label}</Tag>
}
...
/** Tag and text cells share a row height; middle-align `td` so badges line up with copy. */
...
function buildDeliveryColumns(): TableColumns<SubscriptionDeliveryApi> {
    return [
        {
            title: 'Created',
            key: 'created_at',
...
): TableColumns<SubscriptionDeliveryApi>[number] {
    return {
        title: 'Useful?',
        key: 'feedback',
        className: DELIVERY_TABLE_CELL_CLASS,
...
/** Mirrors `@hanzo/quill-primitives` Empty layout (no table chrome). Icon tile uses app tokens (`bg-bg-light`) instead of `bg-muted`, which reads as a dark slab in this theme. */
                    ...
                    {showTable && showStatusFilter ? (
                        <div className="flex flex-wrap items-center gap-3 shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <Select<DeliveryListStatusFilter | null>
                                    size="small"
