...
from insights.schema import PersonsOnEventsMode, RecordingsQuery
...
from insights.session_recordings.queries.session_recording_list_from_query import (
...
from ee.datastore.models.test.test_cohort import get_person_ids_by_cohort_id
...
class TestSessionRecordingsListFromQuery(DatastoreTestMixin, APIBaseTest):
    ...
    def test_operand_or_person_filters(self):
        ...
        create_person(team=self.team, distinct_ids=[user], properties={"email": "test@hanzo.ai"})

        second_user = "test_operand_or_filter-second_user"
        create_person(team=self.team, distinct_ids=[second_user], properties={"email": "david@hanzo.ai"})
...
class TestDatastoreSessionRecordingsListFromQuery(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_effect_of_poe_settings_on_query_generated(
        ...
        with self.settings(
            ...
            # columns break later runs' query snapshots when the Datastore schema is reused.
    ...
    def test_event_filter_with_person_properties_materialized(
        ...
        # leftover materialized columns exist in the CI Datastore instance.
        mat_mock = (
            nullcontext()
            if materialize_person_props
            else patch(
                "ee.datastore.materialized_columns.columns.get_materialized_columns",
                return_value={},
            )
        )
