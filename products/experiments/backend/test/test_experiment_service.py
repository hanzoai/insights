...
from insights.schema import EventsNode, ExperimentMetric
...
class TestExperimentService(APIBaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("threshold_on_insightsql", "insightsql"),
        ]
    )
    ...
    def test_freeze_exposure_success(self):
        ...
        # must not fetch the persons from personinsights a second time.
    ...
    def test_freeze_exposure_multi_group_flag(self):
        ...
        internal_group = {
            "properties": [{"key": "email", "value": "@hanzo.ai", "operator": "icontains", "type": "person"}],
            "rollout_percentage": 100,
    ...
    @parameterized.expand(
        [
            ("timeout", DatastoreQueryTimeOut),
            ("memory_limit", DatastoreQueryMemoryLimitExceeded),
            ("estimated_too_long", DatastoreEstimatedQueryExecutionTimeTooLong),
        ]
    )
    def test_freeze_exposure_rejects_when_scan_is_too_big(self, _name: str, exception_class: type[Exception]):
        ...
        # All three "scan too big" Datastore errors must map to a friendly 400, not a 500.
        with patch(
            "products.experiments.backend.experiment_service.execute_insightsql_query",
    ...
    def test_freeze_exposure_fails_and_cleans_up_when_cohort_population_fails(self):
        ...
        with (
            ...
            patch(
                ...
                side_effect=RuntimeError("datastore insert failed"),
            ),
        ):
    ...
    def test_freeze_exposure_applies_to_filters_edited_during_snapshot_build(self):
        ...
        assert groups[0]["rollout_percentage"] == 50
        assert {"key": "email", "value": "@hanzo.ai", "operator": "icontains", "type": "person"} in groups[0][
            "properties"
        ]
    ...
    def test_unfreeze_exposure_keeps_user_edits_made_while_frozen(self) -> None:
        ...
        user_condition = {"key": "email", "value": "@hanzo.ai", "operator": "icontains", "type": "person"}
    ...
    def test_event_validation_uses_project_scope(self):
        ...
        # (see insights/api/event_definition.py), so users in multi-team projects see
    ...
    def test_sibling_team_can_use_legacy_primary_team_event(self):
        ...
        # The picker mirrors this exact predicate (insights/api/event_definition.py),
...
@patch("hanzo_insights.feature_enabled", new=MagicMock(return_value=True))
