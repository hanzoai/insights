...
from insights.errors import ExposedCHQueryError, QueryErrorCategory, look_up_datastore_error_code_meta
...
def classify_experiment_query_error(error: Exception) -> str:
    ...
    exceptions `insights/errors.py` already produces — never from message parsing or HTTP status.
    ...
    if isinstance(error, (ValidationError, ExposedInsightsQLError)):
        return "validation_error"
    if isinstance(error, (DatastoreQueryTimeOut, DatastoreEstimatedQueryExecutionTimeTooLong)):
        return "timeout"
    if isinstance(error, DatastoreQueryMemoryLimitExceeded):
        return "out_of_memory"
    if isinstance(error, (DatastoreAtCapacity, ConcurrencyLimitExceeded)):
        return "rate_limited"
    if isinstance(error, ServerException):
        meta = look_up_datastore_error_code_meta(error)
...
def experiment_error_handler(method: F) -> F:
    ...
    def wrapper(*args, **kwargs):
        ...
        except (ValidationError, ExposedInsightsQLError) as e:
            # ValidationErrors and ExposedInsightsQLErrors are already user-facing, let them through.
