...
```bash
insightscli evals [SELECTOR ...] [flags]
```

`insightscli evals:sandboxed` is a back-compat alias of `insightscli evals`; both run the same harness.

Run it from a flox shell — the personinsights build needs flox's Rust toolchain (`cargo`, `pkg-config`, OpenSSL), and outside it the preflight build fails.

Or invoke the harness module directly:

```bash
python -m products.insights_ai.eval_harness.harness [SELECTOR ...] [flags]
```

No manual env sourcing is needed on either path.
The harness loads the repo-root `.env` itself (shell values win), and `insightscli evals` additionally layers in `.env.local` / `.env.development` / `.env.services` through insightscli's standard env loading — including 1Password resolution when `.env.local` holds `op://` references.
...
The harness needs the Rust toolchain (`cargo`, provided by the flox shell): person and group reads go through personinsights with no ORM fallback, so it builds and runs `personinsights-replica` + `personinsights-router` from `rust/` against the test persons DB.
...
python -m products.insights_ai.eval_harness.harness

# two domains
python -m products.insights_ai.eval_harness.harness experiments sql

# one suite, one case
python -m products.insights_ai.eval_harness.harness eval_sql --eval churn

# remote sandboxes, every case at once
python -m products.insights_ai.eval_harness.harness --provider modal
...
Every docker run verifies the `insights-sandbox-base` image is fresh before any case starts: it rebuilds when `@hanzo/agent` has published a newer version than the one baked into the image, or when the Dockerfile changed since the image was built.
...
(The modal DEBUG image is rebuilt by Modal whenever the Dockerfile or build context changes, but a new `@hanzo/agent` publish alone does not invalidate it — a known limitation.)
...
Eval sandboxes run in the dedicated `insights-sandbox-evals` Modal app, separate from production and local development sandboxes.
...
These phases can issue large Datastore copies or direct inserts, so the independent limit protects Datastore from RAM exhaustion even when Modal sandbox capacity is unbounded while letting managed environments prepare cases faster.
...
- `products/insights_ai/evals/<domain>/` — the built-in tree, reserved for Max and other agent suites.
...
1. Create `products/insights_ai/evals/<domain>/eval_<name>.py` (or `products/<product>/evals/eval_<name>.py`). The directory becomes the suite's domain.
...
```bash
python -m products.insights_ai.eval_harness.harness --list | grep my_thing
```
...
The final summary gives labeled suite and case totals, the score gate, total duration, and one block per experiment with scorer averages, Insights and Braintrust URLs, and the agent-log directory.
...
```text
products/insights_ai/eval_harness/logs/harness/<timestamp>_<id>.log
```
