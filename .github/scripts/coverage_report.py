...
def resolve_core_path(filename: str, sources: list[str], cache: dict[str, str]) -> str:
    ...
    ``insights``), not repo-relative — even with relative_files. Pick the source whose joined
    path exists in the checkout; insights wins ties since it's the bulk of core. Coverage doesn't
    tell us which source a given <class> actually came from, so when the same relative path
    exists under more than one source (e.g. an ``ee/`` file shadowing a ``insights/`` one), the
    ...
    resolved = stripped
    if not stripped.startswith(("insights/", "ee/")):
        candidates = [
            src
            for src in sorted(sources, key=lambda s: 0 if s == "insights" else 1)
...
def aggregate_core(artifacts_dir: Path) -> tuple[dict[str, set[int]], dict[str, set[int]]]:
    """Union covered / valid line numbers per file across the core (insights/ee) coverage XMLs.

    Filenames are stored relative to their <source> root (insights or ee); each is resolved
...
def write_combined_cobertura(
    ...
    Product files are rewritten via repo_path_for(); core (insights/ee) files are already
...
def diff_touches_backend(compare_branch: str) -> bool | None:
    ...
    for line in proc.stdout.splitlines():
        ...
        if not path.endswith(".py"):
            continue
        if path.startswith(("insights/", "ee/")) or (path.startswith("products/") and "/backend/" in path):
            return True
    return False
...
def main() -> int:
    ...
    parser.add_argument(
        "--core-artifacts", type=Path, help="dir of core (insights/ee) coverage-core-* artifacts to include"
    )
