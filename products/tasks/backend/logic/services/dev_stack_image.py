"""Bakes the prebaked Insights dev-stack VM image.

Boots a plain VM-base sandbox, runs `bake-insights-dev-stack.sh` inside it (install the
dev toolchain the VM base lacks, pre-pull the dev compose images, bring the stack up,
run the Django/persons/Datastore and Rust-driven migrations, shut everything down
...
payload: a payload variant with `"default_custom_image": "insights-dev-stack"` makes it
the default base for that org's VM runs (see `vm_sandbox_default_custom_image` in
`products/tasks/backend/constants.py`). `insightscli start` on those runs then skips the
...
BAKE_SCRIPT_LOCAL_PATH = Path("products/tasks/backend/sandbox/images/bake-insights-dev-stack.sh")
BAKE_SCRIPT_SANDBOX_PATH = "/tmp/bake-insights-dev-stack.sh"
