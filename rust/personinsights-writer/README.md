...
Consumes person state updates from the `personinsights_updates` Kafka topic and batch-upserts them to Postgres, closing the durability loop between the personinsights-leader's in-memory cache and persistent storage.

## Architecture

```text
personinsights_updates (Kafka, compacted)
    │
    ▼
┌──────────────────────────────────────────────────────┐
│  personinsights-writer                                    │
...
|--------|------|-------------|
| `personinsights_writer_messages_consumed_total` | counter | Messages decoded from Kafka |
| `personinsights_writer_decode_errors_total` | counter | Proto decode failures |
| `personinsights_writer_kafka_errors_total` | counter | Kafka recv errors |
| `personinsights_writer_flushes_total` | counter | Total flush events |
| `personinsights_writer_flushes_by_trigger_total{trigger}` | counter | Flush trigger breakdown (timer, size, backpressure, shutdown) |
| `personinsights_writer_rows_upserted_total{mode}` | counter | Rows PG reported as affected (mode: chunk, row) |
| `personinsights_writer_rows_version_skipped_total{mode}` | counter | Rows skipped by version guard (mode: chunk, row) |
| `personinsights_writer_upsert_errors_total{mode}` | counter | PG write failures (mode: chunk, row) |
| `personinsights_writer_batch_fallback_total` | counter | Batches that fell back to per-row |
| `personinsights_writer_unbindable_field_total{field}` | counter | Rows whose field could not be bound losslessly (invariant violation) |
| `personinsights_writer_unapplyable_rows_total{kind}` | counter | Rows PG refused non-transiently (invariant violation, halts the flush) |
| `personinsights_writer_chunk_fallback_rows_total` | counter | Rows from data-failed chunks sent to per-row fallback |
| `personinsights_writer_chunk_retry_rows_total` | counter | Rows from transient-failed chunks retried as a batch |
| `personinsights_writer_chunk_fatal_total` | counter | Chunk tasks that panicked or were cancelled |
| `personinsights_writer_row_fallback_duration_seconds` | histogram | Duration of the per-row fallback for a batch |
| `personinsights_writer_row_fallback_in_flight` | gauge | Concurrent per-row upserts in flight during fallback |
| `personinsights_writer_pg_pool_size` | gauge | Total sqlx pool connections (sampled every 5s) |
| `personinsights_writer_pg_pool_idle` | gauge | Idle sqlx pool connections (sampled every 5s) |
| `personinsights_writer_offset_commits_total` | counter | Successful offset commits |
| `personinsights_writer_offset_commit_errors_total` | counter | Failed offset commits |
| `personinsights_writer_flush_duration_seconds` | histogram | PG write latency per flush |
| `personinsights_writer_flush_rows` | histogram | Rows per flush |
| `personinsights_writer_channel_send_duration_seconds` | histogram | Time waiting on the writer channel (backpressure indicator) |
| `personinsights_writer_e2e_latency_seconds` | histogram | End-to-end latency from Kafka message creation to PG commit |
| `personinsights_writer_buffer_size` | gauge | Current buffer entries |
| `personinsights_writer_partition_offset{partition}` | gauge | Current offset per partition |
...
| `KAFKA_TOPIC` | `personinsights_updates` | Topic to consume |
| `KAFKA_CONSUMER_GROUP` | `personinsights-writer` | Consumer group ID |
| `KAFKA_CONSUMER_OFFSET_RESET` | `earliest` | Offset reset policy |
| `DATABASE_URL` | (required) | Postgres connection string |
| `PG_MAX_CONNECTIONS` | `20` | Connection pool size |
| `PG_TARGET_TABLE` | `personinsights_person_tmp` | Target table (`insights_person` for production cutover) |
