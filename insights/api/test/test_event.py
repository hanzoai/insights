...
from insights.models import Element, Organization, PropertyDefinition, User
from insights.models.event.legacy_events_query import _execute_events_list_query
...
class TestEvents(DatastoreTestMixin, APIBaseTest):
    ...
    def test_filter_events_by_event_name(self):
        ...
        # Auth/team/membership/instance-setting lookups, plus the InsightsQL pipeline's per-probe
        ...
        # personinsights, not Postgres, so it's not in this count. Was 16 before passing team=team
    ...
    def test_filter_events_by_properties(self):
        ...
        # Auth/team/membership/access-control/instance-setting lookups, plus the InsightsQL
        # pipeline's per-probe access-control checks. The progressive-window loop probes several
        # windows on this sparse dataset; the InsightsQL schema is built once and shared across them.
        # Group-type-mapping is read via personinsights, not Postgres, so it's not in this count.
        ...
        expected_queries = 22 if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA else 23
...
class TestEventListRestrictedProperties(DatastoreTestMixin, APIBaseTest):
    def setUp(self):
        super().setUp()
...
class TestEventListTimeWindowOptimization(DatastoreTestMixin, APIBaseTest):
    def test_cache_key_generation(self):
        from insights.models.event.legacy_events_query import _get_event_list_cache_key, _get_limit_size_category
    ...
    @patch("insights.models.event.legacy_events_query.cache")
    @patch("insights.models.event.legacy_events_query._execute_events_list_query")
    ...
    @patch("insights.api.event.EVENT_LIST_MAX_LIMIT", 6000)
    @patch("insights.models.event.legacy_events_query.cache")
    @patch("insights.models.event.legacy_events_query.LegacyEventsListQuery.run_page")
