...
class TestVisionActionDelivery(APIBaseTest):
    def setUp(self) -> None:
        ...
        # Saving a InsightsFunction pushes it to the CDP workers; there are none in tests.
        self.reload_patcher = patch(
            "products.cdp.backend.models.insights_functions.insights_function.reload_insights_functions_on_workers",
        )
    ...
    def _request(self) -> Request:
        # provision_delivery threads a DRF Request into InsightsFunctionSerializer, which reads request.user.
    ...
    def _destinations(self, action: VisionAction) -> list[InsightsFunction]:
        """The action's live internal_destination InsightsFunctions — found by the vision_action_id filter
    ...
    @staticmethod
    def _inputs(fn: InsightsFunction) -> dict[str, Any]:
        assert fn.inputs is not None
        return fn.inputs

    @staticmethod
    def _filters(fn: InsightsFunction) -> dict[str, Any]:
    ...
    def test_webhook_target_provisions_webhook_destination(self) -> None:
        ...
        self.assertEqual(inputs["headers"]["value"]["X-Insights-Webhook-Version"], "1")
