...
describe('toolRegistry', () => {
    ...
    // The Insights product data-tools self-register via `widgets/registerDataToolRenderers`, which
    ...
    // (e.g. "notebooks-create") and return an arbitrary `_insightsUrl`. Such a call is NOT from the
    // trusted single-exec Insights server (no inner tool name was parsed, so `fromInsightsExec` is false),
    ...
    ])('gates product widget %s on a trusted Insights-exec origin', (key, displayName) => {
    ...
    // Insights single-exec discovery verbs are keyed by their sentinel key (from `resolveToolKey`); the
    // registry gives each a fitting icon + displayName instead of the wrench fallback.
    const execVerbCases: [string, string][] = [
        ['__insights_exec_tools__', 'List tools'],
        ['__insights_exec_search__', 'Search tools'],
        ['__insights_exec_info__', 'Read tool'],
        ['__insights_exec_schema__', 'Inspect schema'],
        ['__insights_exec_unknown__', 'Run command'],
    ]
