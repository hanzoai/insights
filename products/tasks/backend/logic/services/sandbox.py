...
class SandboxTemplate(str, Enum):
    ...
    # sandboxes like stamp that never run the agent server. See
...
PUBLIC_SANDBOX_REPOS: frozenset[str] = frozenset({"insights/hedgebox", "insights/.github"})
...
class SandboxBase(ABC):
    ...
    def launch_dev_stack_bootstrap(self) -> bool:
        ...
        bake-insights-dev-stack.sh): it restores the compose /etc/hosts aliases the sandbox
        boot wiped and starts dockerd. Launching it detached at provision time overlaps
        that warmup with the repo clone and agent-server boot, so a task-time `insightscli start`
        ...
        """
        # Only a sandbox that actually booted the Insights-published dev-stack image gets
        ...
        #   - the reserved name (user images always publish as insights-sandbox-custom-*),
    ...
    def agent_server_supports_exec_permission_regex(self) -> bool:
        ...
        --insightsExecPermissionRegex; unsupported binaries degrade to server-side auto-approval of
        exec sub-tools instead of crashing at launch."""
        result = self.execute(
            "grep -q insightsExecPermissionRegex /scripts/node_modules/.bin/agent-server", timeout_seconds=10
        )
        return result.exit_code == 0
...
def _get_modal_docker_sandbox_class() -> SandboxClass:
    ...
    class ModalDockerSandbox(ModalSandbox):
        DEFAULT_APP_NAME = "insights-sandbox-modal-docker-default"
        NOTEBOOK_APP_NAME = "insights-sandbox-modal-docker-notebook"
...
def _get_modal_evals_sandbox_class() -> SandboxClass:
    ...
    class ModalEvalsSandbox(ModalSandbox):
        DEFAULT_APP_NAME = "insights-sandbox-evals"
        NOTEBOOK_APP_NAME = "insights-sandbox-evals"
