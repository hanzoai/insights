...
Datastore queries and must never hold the row lock or affect the check outcome.
"""
...
from insights.schema import AlertState, NodeKind
...
# Each target costs a handful of synchronous Datastore queries in the
...
def run_metrics_alert_investigation(alert: AlertConfiguration, alert_check: AlertCheck) -> None:
    ...
    persistence transaction — this issues Datastore queries.
    """
    try:
...
def _run_investigation(alert: AlertConfiguration, alert_check: AlertCheck) -> str:
    # Deferred: the facade pulls in InsightsQL machinery that must stay off the
