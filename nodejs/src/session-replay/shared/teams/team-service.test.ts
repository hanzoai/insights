...
describe('TeamService', () => {
    ...
    beforeEach(() => {
        ...
        fetchSpy = jest.spyOn(teamService as any, 'fetchOrgProjectsWithRecordings').mockResolvedValue({
            orgMap: {
                'valid-org': { teamId: 1, consoleLogIngestionEnabled: true },
                'valid-org-2': { teamId: 2, consoleLogIngestionEnabled: false },
            },
    ...
    describe('getTeamByOrg', () => {
        ...
        it('should return null if teamId is missing', async () => {
            fetchSpy.mockResolvedValue({
                orgMap: {
                    org: { teamId: null as any, consoleLogIngestionEnabled: true },
                },
                retentionMap: { 1: '30d', 2: '1y' },
                encryptionMap: { 1: true, 2: false },
            })
        ...
        it('should eventually update cache after successful refresh', async () => {
            ...
            const mockFetchPromise = Promise.resolve({
                orgMap: {
                    'valid-org': { teamId: 1, consoleLogIngestionEnabled: false },
                },
                retentionMap: { 1: '30d', 2: '1y' },
                encryptionMap: { 1: true, 2: false },
            })
        ...
        it('should eventually return null when team is removed after refresh', async () => {
            ...
            const mockFetchPromise = Promise.resolve({
                orgMap: {
                    'valid-org-2': { teamId: 2, consoleLogIngestionEnabled: false }, // Remove valid-token
                },
                retentionMap: { 2: '1y' },
