...
/// and must NOT surface the `X-Insights-Rate-Limit-Warning` response
...
async fn bot_response_does_not_carry_rate_limit_warning_header() -> Result<()> {
    ...
    assert!(
        res.headers().get("X-Insights-Rate-Limit-Warning").is_none(),
        "bot short-circuit must NOT surface X-Insights-Rate-Limit-Warning, got {:?}",
        res.headers().get("X-Insights-Rate-Limit-Warning"),
    );
...
async fn non_bot_response_carries_rate_limit_warning_header() -> Result<()> {
    ...
    assert_eq!(res.status(), StatusCode::OK);
    assert!(
        res.headers().get("X-Insights-Rate-Limit-Warning").is_some(),
        "non-bot path must surface X-Insights-Rate-Limit-Warning when IP warn fires",
    );
