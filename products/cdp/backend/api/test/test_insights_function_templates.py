...
MOCK_NODE_TEMPLATES = json.loads(
    open(os.path.join(os.path.dirname(__file__), "__data__/insights_function_templates.json")).read()
)
...
class TestInsightsFunctionTemplates(DatastoreTestMixin, APIBaseTest, QueryMatchingTest):
    def setUp(self):
        ...
        self.webhook_template = InsightsFunctionTemplate.objects.create(
        ...
        self.deprecated_template = InsightsFunctionTemplate.objects.create(
    ...
    def test_template_updates_are_reflected(self):
        ...
        modified_template = InsightsFunctionTemplateDC(
    ...
    def test_public_insights_function_templates_are_sorted_by_usage(self):
        for i in range(10):
    ...
    def test_hidden_templates_included_in_project_list(self):
        # Hidden templates are internal building blocks (e.g. template-insights-capture,
        # template-insights-update-person-properties, email, twilio). The workflow editor pulls them
    ...
    @parameterized.expand(
        [
            ("list", "/insights_function_templates/"),
            ("retrieve", "/insights_function_templates/template-slack"),
        ]
    )
    def test_project_route_authenticates_personal_api_key(self, _name, suffix):
        ...
        response = self._get_with_pak(suffix, ["insights_function:read"])
        assert response.status_code == status.HTTP_200_OK, response.json()

    def test_project_route_enforces_insights_function_scope(self):
        # A token without the insights_function scope is rejected — scope_object is enforced, not decorative.
        response = self._get_with_pak("/insights_function_templates/", ["feature_flag:read"])
