...
CAPTURE_PATH = "products.signals.backend.scout_harness.tools.report.hanzo_insights.capture"
...
class TestScoutReportAPI(APIBaseTest):
    ...
    def test_emit_report_writes_autostart_artefacts(self) -> None:
        ...
        assert repo is not None and '"insights/insights"' in repo.content
    ...
    def test_emit_report_fires_autostart_when_surfaced(self) -> None:
        run = _make_run(self.team)
        payload = self._payload(repository="Insights/Insights", priority="P1", priority_explanation="big blast radius")
    ...
    def test_emit_report_skips_autostart_and_artefacts_when_suppressed(self) -> None:
        ...
        run = _make_run(self.team)
        payload = self._payload(repository="Insights/Insights", priority="P1", priority_explanation="x")
        with (
...
class TestWantsRepoSelection(SimpleTestCase):
    ...
    @parameterized.expand(
        [
            ...
            ("explicit_repository_always_selects", "insights/insights", None, None, True),
