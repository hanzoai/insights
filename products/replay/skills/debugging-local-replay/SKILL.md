...
description: >
  ...
  seems broken after insightscli start. Covers the full local pipeline:
...
- **SDK not initialized with recording.** Check the local app's Insights initialization —
  `session_recording` must not be explicitly disabled.
- **Wrong Insights host.** The local app must point to `http://localhost:8000` (or wherever
...
- **Recorder script failed to load (MIME type / CORS error).** The browser console may show
  `MIME type ('text/html') is not executable` for `insights-recorder.js` or a CORS error for
...
```bash
docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "kafka|db|redis7|datastore|seaweed"
```
