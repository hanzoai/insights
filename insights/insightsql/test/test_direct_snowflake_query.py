...
from insights.insightsql.direct_sql.snowflake_adapter import (
    snowflake_error_to_message,
    snowflake_field_type_to_datastore_type,
...
from insights.insightsql.snowflake_connection_cache import (
...
# reports the integer code, not a name — see snowflake_field_type_to_datastore_type.
FIXED = 0
REAL = 1
TEXT = 2
DATE = 3
...
class TestDirectSnowflakeQuery(APIBaseTest):
    ...
    def test_snowflake_field_type_to_datastore_type(
        self, _name: str, type_code: object | None, scale: object | None, expected: str
    ):
        self.assertEqual(snowflake_field_type_to_datastore_type(type_code, scale), expected)
    ...
    def test_execute_sets_statement_timeout_and_maps_types(self):
        ...
        with patch(
            "insights.insightsql.direct_sql.snowflake_adapter.SnowflakeAdapter.validate_source_config",
    ...
    def test_execute_rejects_result_over_row_cap(self):
        ...
        with patch("insights.insightsql.direct_sql.snowflake_adapter.DIRECT_SNOWFLAKE_MAX_ROWS", 3):
            with patch(
                "insights.insightsql.direct_sql.snowflake_adapter.SnowflakeAdapter.validate_source_config",
                return_value=(implementation, MagicMock()),
            ):
...
class TestSnowflakeConnectionCache(unittest.TestCase):
    ...
    def test_reopens_after_ttl(self):
        ...
        with patch("insights.insightsql.snowflake_connection_cache.SNOWFLAKE_CONNECTION_CACHE_TTL_SECONDS", 0):
