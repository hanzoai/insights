...
describe('hogFlowEditorLogic', () => {
    ...
    describe('conditional branch naming', () => {
        const createMockInsightsFlow = (conditionNames?: (string | undefined)[]): InsightsFlow => ({
            id: 'test-flow',
            team_id: 1,
            version: 1,
            name: 'Test Flow',
        ...
        it('should use default labels when condition names are not provided', () => {
            const mockFlow = createMockInsightsFlow()
            logic.actions.resetFlowFromInsightsFlow(mockFlow)
        ...
        it('should use custom names when provided for conditional branches', () => {
            const mockFlow = createMockInsightsFlow(['User is premium', 'User is in trial'])
            logic.actions.resetFlowFromInsightsFlow(mockFlow)
        ...
        it('should fall back to default labels for conditions without names', () => {
            const mockFlow = createMockInsightsFlow(['User is premium', undefined])
            logic.actions.resetFlowFromInsightsFlow(mockFlow)
        ...
        it('should update edge labels when conditions are modified', () => {
            ...
            const updatedAction: InsightsFlowAction = {
                id: 'branch',
                name: 'Conditional Branch',
                description: '',
                type: 'conditional_branch',
            ...
            logic.actions.resetFlowFromInsightsFlow(updatedFlow)
        ...
        it('should use custom names for wait_until_condition when provided', () => {
            const mockFlow: InsightsFlow = {
                ...createMockInsightsFlow(),
                actions: [
                    {
                        id: 'trigger',
                        name: 'Trigger',
    ...
    describe('graph identity across rebuilds', () => {
        ...
        const makeFlow = (branchName: string = 'Branch'): InsightsFlow => ({
            id: 'test-flow',
            team_id: 1,
            version: 1,
            name: 'Test Flow',
        ...
        it('replaces only the changed node reference when one action changes', async () => {
            ...
            const byId = (id: string): InsightsFlowActionNode | undefined =>
    ...
    describe('showDropzones branch-join placement', () => {
        const makeNode = (id: string): InsightsFlowActionNode =>
            ({
                id,
                type: 'action',
                data: { id, type: 'exit', name: id, description: '', config: {} } as unknown as InsightsFlowAction,
                position: { x: 0, y: 0 },
                handles: [],
