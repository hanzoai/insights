    ...
    describe('MCP mode parsing', () => {
        const modeTests = [
            ...
            },
            {
                ...
                headers: { 'x-insights-mcp-mode': 'tools' },
                params: '',
                expected: 'tools' as const,
            },
            {
                description: 'cli from header',
                headers: { 'x-insights-mcp-mode': 'cli' },
                params: '',
            ...
            },
            {
                ...
                headers: { 'x-insights-mcp-mode': 'banana' },
                params: '',
                expected: undefined,
            },
            {
                description: 'legacy exec value ignored (only tools or cli accepted)',
                headers: { 'x-insights-mcp-mode': 'exec' },
                params: '',
                expected: undefined,
            },
            {
        ...
        it.each(modeTests)('parses $description', ({ headers, params, expected }) => {
            const url = new URL(`https://example.com/mcp${params}`)
            const headerValue = headers['x-insights-mcp-mode'] ?? null
    ...
    describe('MCP consumer parsing', () => {
        const consumerTests = [
            ...
            },
            {
                ...
                headers: { 'x-insights-mcp-consumer': 'plugin' },
                params: '',
                expected: 'plugin',
            },
            {
                description: 'insights-code from header',
                headers: { 'x-insights-mcp-consumer': 'insights-code' },
                params: '',
                expected: 'insights-code',
            },
            {
            ...
            },
            {
                ...
                headers: { 'x-insights-mcp-consumer': 'plugin\x00\x1b' },
                params: '',
                expected: 'plugin',
            },
        ]
        ...
        it.each(consumerTests)('parses $description', ({ headers, params, expected }) => {
            const url = new URL(`https://example.com/mcp${params}`)
            const headerValue = headers['x-insights-mcp-consumer'] ?? null
