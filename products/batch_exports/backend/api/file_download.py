...
from products.batch_exports.backend.insightsql_source import (
    ...
    validate_insightsql_query_for_batch_export,
)
...
INSIGHTSQL_QUERY_HELP_TEXT = (
    "InsightsQL SELECT query whose results are exported. Placeholders are not currently supported, "
...
class FileDownloadInsightsQLRequestSerializer(serializers.Serializer):
    """Typed configuration for the insightsql model."""

    file = FileDownloadDestinationFileConfigSerializer()
    model = serializers.ChoiceField(choices=["insightsql"])
    insightsql_query = serializers.CharField(help_text=INSIGHTSQL_QUERY_HELP_TEXT)


def check_insightsql_batch_exports_enabled(team: Team) -> None:
    """Raise if InsightsQL-powered batch exports are not enabled for the team."""
    if not hanzo_insights.feature_enabled(
        "insightsql-batch-exports",
        str(team.uuid),
        ...
        raise PermissionDenied("InsightsQL batch exports are not enabled for this team.")
...
class FileDownloadBatchExportOnDemandSerializer(serializers.Serializer):
    ...
    file = FileDownloadDestinationFileConfigSerializer()
    model = serializers.ChoiceField(choices=["events", "persons", "sessions", "insightsql"])

    # Only specific to events
    include = serializers.ListField(child=serializers.CharField(), required=False)
    exclude = serializers.ListField(child=serializers.CharField(), required=False)

    # Only specific to insightsql
    insightsql_query = serializers.CharField(required=False, help_text=INSIGHTSQL_QUERY_HELP_TEXT)

    # Run attributes; required for all models except insightsql, which runs as of now
    ...
    def validate(self, data):
        if data["model"] == "insightsql":
            return self._validate_insightsql(data)

        if data.get("insightsql_query") is not None:
            raise ValidationError("'insightsql_query' is only supported when 'model' is 'insightsql'")
    ...
    def _validate_insightsql(self, data: dict) -> dict:
        team = self.context["get_team"]()
        check_insightsql_batch_exports_enabled(team)
        ...
        if data.get("data_interval_start") is not None or data.get("data_interval_end") is not None:
            raise ValidationError(
                "'data_interval_start' and 'data_interval_end' are not supported when 'model' is 'insightsql': "
                "the query runs as of the time the export starts"
            )

        if data.get("include") is not None or data.get("exclude") is not None:
            raise ValidationError("'include' and 'exclude' are not supported when 'model' is 'insightsql'")

        insightsql_query = data.get("insightsql_query")
        if not insightsql_query:
            raise ValidationError("'insightsql_query' is required when 'model' is 'insightsql'")

        try:
            validate_insightsql_query_for_batch_export(insightsql_query, team)
        except UnsupportedInsightsQLQueryError as e:
            raise ValidationError({"insightsql_query": str(e)}) from e
    ...
    def create(self, validated_data: dict) -> BatchExportRun:
        ...
        source = None
        if model == "insightsql":
            source = BatchExportSource(team_id=team_id, insightsql_query=validated_data.pop("insightsql_query"))
            # For now, InsightsQL exports have no data interval: the query runs as of now.
    ...
    @extend_schema(
        request=PolymorphicProxySerializer(
            ...
            serializers={
                ...
                "insightsql": FileDownloadInsightsQLRequestSerializer,
            },
        ...
        try:
            start_file_download_batch_export(
                ...
                batch_export_model=BatchExportModel(
                    ...
                    schema=None,
                    insightsql_query=source.insightsql_query if source is not None else None,
                ),
