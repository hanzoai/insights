...
from posthog.schema import InsightsQLQueryModifiers, IntervalType, WebTrendsMetric, WebTrendsQuery
...
from posthog.insightsql_queries.web_analytics.trends_pre_aggregated_query_builder import TrendsPreAggregatedQueryBuilder
...
class TestTrendsPreAggregatedQueryBuilder(ClickhouseTestMixin, APIBaseTest):
    def _create_mock_runner(self, query: WebTrendsQuery, modifiers: Optional[InsightsQLQueryModifiers] = None):
        ...
        runner.modifiers = modifiers or InsightsQLQueryModifiers(
    ...
    def test_basic_query_structure_single_metric(self):
        ...
        insightsql_query, _ = prepare_and_print_ast(ast_query, context=context, dialect="insightsql")

        # Verify the query structure
        self.assertIn("SELECT", insightsql_query)
        self.assertIn("toStartOfDay(period_bucket) AS bucket", insightsql_query)
        self.assertIn("uniqMerge(persons_uniq_state) AS unique_users", insightsql_query)
        self.assertIn("FROM web_pre_aggregated_bounces", insightsql_query)
        self.assertIn("GROUP BY bucket", insightsql_query)
        self.assertIn("ORDER BY bucket ASC", insightsql_query)
    ...
    def test_multiple_metrics(self):
        ...
        self.assertIn("uniqMerge(persons_uniq_state) AS unique_users", insightsql_query)
        self.assertIn("sumMerge(pageviews_count_state) AS page_views", insightsql_query)
        self.assertIn("uniqMerge(sessions_uniq_state) AS sessions", insightsql_query)
        self.assertIn("sumMerge(bounces_count_state) AS bounces", insightsql_query)
    ...
    def test_weekly_interval(self):
        ...
        self.assertIn("toStartOfWeek(period_bucket) AS bucket", insightsql_query)
    ...
    def test_monthly_interval(self):
        ...
        self.assertIn("toStartOfMonth(period_bucket) AS bucket", insightsql_query)
    ...
    def test_session_metrics(self):
        ...
        self.assertIn("sumMerge(total_session_duration_state) AS session_duration", insightsql_query)
        self.assertIn("sumMerge(total_session_count_state) AS total_sessions", insightsql_query)
