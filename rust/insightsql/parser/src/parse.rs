//! InsightsQL parser entry points + Pratt expression parser.
//!
...
//! the JSON error envelope expected by [`insights/insightsql/json_ast.py`].
//!
//! Implementation strategy: Pratt parser with explicit binding powers, full
//! precedence ladder from [`insights/insightsql/grammar/InsightsQLParser.g4`]'s
...
pub fn parse_expr_with_emit<E: Emitter + Clone>(
    ...
    // a `{ … }` Script-block body stops at `}`, leaving the operator for
...
pub fn parse_select_with_emit<E: Emitter + Clone>(
    ...
    let mut p = Parser::new_with_emit(src, emit)?;
    // `select` rule top-level admits a bare InsightsQLX tag element as the
    ...
    let result = if p.peek_starts_insightsqlx_tag() {
        p.parse_insightsqlx_tag_element()?
    } else {
...
pub fn parse_full_template_string_with_emit<E: Emitter + Clone>(
    ...
    // (see `insights/insightsql/parser.py::parse_string_template`). Strip
...
pub(crate) struct Parser<'a, E: Emitter = JsonEmitter> {
    ...
    /// (line 236 < 237 in InsightsQLParser.g4), so the ORDER BY there
    ...
    /// Set by the Script-program statement parser while parsing a
    ...
    /// to a synthetic `Eof` token. Used inside InsightsQLX tag-body parsing:
    /// cpp's `INSIGHTSQLX_TEXT` lexer mode admits any byte except `<` / `{`,
    /// but rust's mode-less lexer would reject punctuation like `&` /
    /// `!` / `@` when pre-loading peek1 across a `>` / `/>` / closing
    /// `>` boundary. `parse_insightsqlx_children` byte-walks the body
    /// directly and re-seeks the lexer, so peek1's transient invalid
    /// state is recoverable.
    pub(crate) insightsqlx_text_lookahead_depth: u32,
...
impl<'a, E: Emitter + Clone> Parser<'a, E> {
    ...
    pub(crate) fn with_pos_emit(src: &'a str, pos: usize, emit: E) -> Result<Self, ParseError> {
        ...
        Ok(Self {
            ...
            limit_body_depth: 0,
            pivot_in_stop: None,
            insightsqlx_text_lookahead_depth: 0,
...
impl<'a, E: Emitter + Clone> Parser<'a, E> {
    ...
    pub(crate) fn bump(&mut self) -> Result<Token, ParseError> {
        let next = match self.lexer.next_token() {
            Ok(t) => t,
            Err(_e) if self.insightsqlx_text_lookahead_depth > 0 => {
                // We're inside a InsightsQLX tag body (`INSIGHTSQLX_TEXT` lexer
    ...
    pub(crate) fn expect_eof(&mut self) -> Result<(), ParseError> {
        ...
        // `InsightsQLSyntaxError` (the deserialiser triggers on the substring
...
pub(crate) fn parse_number_literal<E: Emitter>(
    ...
    // unconditionally rejects it (matches Datastore and pre-pg16
    ...
    if src.starts_with("0o") || src.starts_with("0O") {
        return Err(ParseError::syntax(
            format!(
                "InsightsQL does not support `0o`-prefixed octal integer literals; got `{src}`. Use a plain decimal literal instead."
            ),
            0,
            src.len(),
        ));
    ...
    if let Some(rest) = src.strip_prefix("0b").or_else(|| src.strip_prefix("0B")) {
        ...
        let magnitude = u64::from_str_radix(rest, 2).map_err(|_| {
            ParseError::syntax(
                format!("InsightsQL binary integer literals are limited to 64 bits; got `{src}`."),
                0,
                src.len(),
            )
        })?;
...
/// `insights.insightsql.constants.RESERVED_KEYWORDS = [*KEYWORDS, "team_id"]`
...
///   - the Script-statement keywords FN / FUN / LET / WHILE / THROW / TRY
...
pub(crate) fn kw_acts_as_ident_in_primary(kw: Kw) -> bool {
    !matches!(
        ...
        // Script-statement keywords — also OMITTED from the `keyword`
...
pub(crate) fn interval_call_name(unit: &str) -> Option<&'static str> {
    ...
    // `yyyy` is a lexer-level alias for YEAR (InsightsQLLexer.common.g4
