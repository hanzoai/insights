...
@pytest.mark.parametrize(
    ...
    [
        (DataWarehouseCredential, "insights_datawarehousecredential"),
        (DataWarehouseTable, "insights_datawarehousetable"),
        (ExternalDataJob, "insights_externaldatajob"),
        (ExternalDataSchema, "insights_externaldataschema"),
        (ExternalDataSource, "insights_externaldatasource"),
    ],
)
def test_db_table_preserved_across_split(model: type[Model], expected_db_table: str) -> None:
    ...
    the `insights_*` table names must remain unchanged or prod reads break silently."""
...
def test_clean_type_unwraps_low_cardinality(datastore_type: str, expected: str) -> None:
    """`ai_events` exposes LowCardinality columns (event, model, provider, ...). clean_type must
    unwrap LowCardinality so the Datastore->InsightsQL mapping lookup resolves instead of KeyError-ing."""
    cleaned = clean_type(datastore_type)
    assert cleaned == expected
    assert cleaned in DATASTORE_INSIGHTSQL_MAPPING


@pytest.mark.parametrize(
    "datastore_type,nullable,expected",
    [
        ("String", False, "String"),
        ("String", True, "Nullable(String)"),
        ("Nullable(String)", True, "Nullable(String)"),
        # LowCardinality must stay outermost — Datastore rejects Nullable(LowCardinality(...)).
...
def test_datastore_column_to_dwh_column_nullable_wrapping(datastore_type: str, nullable: bool, expected: str) -> None:
    assert datastore_column_to_dwh_column("col", datastore_type, nullable)["datastore"] == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A bare digit-string cursor on a date/time-typed field (e.g. a Datastore column Arrow
