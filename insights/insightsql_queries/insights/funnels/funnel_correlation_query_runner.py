...
from insights.constants import AUTOCAPTURE_EVENT
from insights.insightsql_queries.insights.funnels import FunnelUDF
...
from insights.models.event.util import ElementSerializer
...
class FunnelCorrelationQueryRunner(AnalyticsQueryRunner[FunnelCorrelationResponse]):
    ...
    def _calculate(self) -> FunnelCorrelationResponse:
        ...
        events, skewed_totals, insightsql, response = self._calculate_internal()
    ...
    def _calculate_internal(self) -> tuple[list[EventOddsRatio], bool, str, InsightsQLQueryResponse]:
        query = self.to_query()
        ...
        if not response.results:
            ...
            return [], True, insightsql, response
        ...
        return events, skewed_totals, insightsql, response
    ...
    def events_actor_query(self) -> ast.SelectQuery:
        ...
        date_from = self._date_range().date_from_as_insightsql()
        date_to = self._date_range().date_to_as_insightsql()
    ...
    def _get_events_right_join_query(self) -> str:
        ...
        but the right table is Datastore's hash-build side, and that must be the
        ...
        elif self._insightsql_aggregation_expr() is not None:
    ...
    def _get_aggregation_target_join_query(self) -> str:
        ...
        # When the funnel aggregates by a custom InsightsQL expression, funnel_actors.actor_id
        ...
        if self._insightsql_aggregation_expr() is not None:
            return """
            JOIN funnel_actors
    ...
    def _insightsql_aggregation_expr(self) -> ast.Expr | None:
        """Parsed AST for funnelAggregateByInsightsQL when set and non-trivial, else None for
        ...
        aggregate_by_insightsql = funnels_filter.funnelAggregateByInsightsQL if funnels_filter else None
        if aggregate_by_insightsql and aggregate_by_insightsql != "person_id":
            return parse_expr(aggregate_by_insightsql)
        return None

    def _aggregation_join_placeholders(self) -> dict[str, ast.Expr]:
        expr = self._insightsql_aggregation_expr()
