...
from insights.errors import wrap_datastore_query_error
...
from insights.query_cache.failures import BUDGET_EXTENDED, BUDGET_INTERACTIVE, QueryFailureRecord


def _memory_error(message: str):
    return wrap_datastore_query_error(ServerException(message, code=241))
...
class TestQueryFailureHandling(SimpleTestCase):
    @parameterized.expand(
        [
            ...
            ("timeout", DatastoreQueryTimeOut(), "timeout"),
            ("too_slow", DatastoreEstimatedQueryExecutionTimeTooLong(), "too_slow"),
            ("query_size", DatastoreQuerySizeExceeded(), "query_size"),
            ("at_capacity", DatastoreAtCapacity(), None),
            ("concurrency_limit", ConcurrencyLimitExceeded("busy"), None),
            ("too_many_simultaneous", wrap_datastore_query_error(ServerException("busy", code=202)), None),
    ...
    def test_build_failure_exception_preserves_class_and_status(self):
        with freeze_time("2026-01-01T00:00:00Z"):
            original_detail = str(DatastoreQueryTimeOut().detail)
            ...
            assert isinstance(error, DatastoreQueryTimeOut)
    ...
    def test_build_failure_exception_first_failure_wording(self):
        with freeze_time("2026-01-01T00:00:00Z"):
            original_detail = str(DatastoreQueryMemoryLimitExceeded().detail)
            record = _record("memory_limit", 1, original_detail, open_until=datetime.now(UTC) + timedelta(minutes=2))

            error = build_failure_exception(record)
            assert isinstance(error, DatastoreQueryMemoryLimitExceeded)
