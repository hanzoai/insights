...
from insights.datastore.query_tagging import Feature, Product, get_query_tags, tags_context
...
_INSIGHTSQL = {"kind": "InsightsQLQuery", "query": "select count() as c from events"}
...
_OK_PAYLOAD = {"results": [[1]], "insightsql": "SELECT 1"}
...
class TestMetricRunExecution(DatastoreTestMixin, APIBaseTest):
    ...
    def test_run_envelope_matches_direct_execution(self) -> None:
        ...
        metric = upsert_metric(team=self.team, user=self.user, name="mrr", description="d", definition=_INSIGHTSQL)
        ...
        assert body["kind"] == "InsightsQLQuery"
        assert body["is_drifted"] is False
        assert "/sql?open_query=" in body["insights_url"]
...
class TestMetricRunPreparation(APIBaseTest):
    ...
    def test_date_overrides_reach_executed_query_and_deep_link(self) -> None:
        ...
        linked = _decoded_insight_link(envelope["insights_url"])
        assert linked["kind"] == "InsightVizNode"
        assert linked["source"] == executed

    def test_insightsql_values_round_trip_through_deep_link(self) -> None:
        ...
        linked = _decoded_sql_editor_link(envelope["insights_url"])
    ...
    def test_error_payload_is_a_failed_run(self) -> None:
        ...
        metric = upsert_metric(team=self.team, user=self.user, name="prep", description="d", definition=_INSIGHTSQL)
    ...
    @parameterized.expand(
        [
            ("exposed_insightsql", ExposedInsightsQLError("no such table"), ValidationError),
            ("exposed_datastore", ExposedCHQueryError("memory limit"), ValidationError),
...
class TestMetricRunAttribution(APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
        # The markdown path exercises the same attribution code without a Datastore query.
        upsert_metric(
            team=self.team,
            user=self.user,
            name="activation",
...
class TestMetricRunTagging(APIBaseTest):
    def test_run_tags_datastore_query_with_data_catalog_product(self) -> None:
        # run_metric executes queries via process_query_dict directly, so it must set the query tags
        # itself. Untagged, Datastore queries warn in prod and hard-fail in local dev; TEST disables
        # tag enforcement, so only this explicit assertion guards against dropping the tags.
        metric = upsert_metric(team=self.team, user=self.user, name="tagged", description="d", definition=_INSIGHTSQL)
    ...
    def test_run_propagates_is_query_service_from_access_method(
        ...
        metric = upsert_metric(team=self.team, user=self.user, name="svc", description="d", definition=_INSIGHTSQL)
        captured: dict[str, object] = {}
