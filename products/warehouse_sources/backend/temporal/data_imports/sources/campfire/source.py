...
class CampfireSource(ResumableSource[CampfireSourceConfig, CampfireResumeConfig]):
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="""Enter your Campfire API key to pull your accounting data into the Insights Data warehouse.

Create an API user and key on the [API keys page](https://app.meetcampfire.com/v2/settings/api-keys) in your Campfire settings. A `view only` role is enough, since Insights only reads data. The key is shown once, so copy it when you create it.
""",
            iconPath="/static/services/campfire.png",
            docsUrl="https://hanzo.ai/docs/cdp/sources/campfire",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
