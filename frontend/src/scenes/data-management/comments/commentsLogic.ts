...
const openUrls: Partial<Record<ActivityScope, (c: CommentType) => string | null>> = {
    ...
    [ActivityScope.FN_FUNCTION]: (c) => (c.item_id ? urls.insightsFunction(c.item_id) : null),
...
export const commentsLogic = kea<commentsLogicType>([
    ...
    listeners(({ actions }) => ({
        ...
        completeCommentFailure: (e) => {
            insights.captureException(e, { action: 'data management scene completing comment' })
            toast.error('Could not mark task complete, refresh and try again')
        },
        reopenCommentFailure: (e) => {
            insights.captureException(e, { action: 'data management scene reopening comment' })
            toast.error('Could not reopen task, refresh and try again')
        },
