...
def _make_introspection_response(**overrides):
    ...
    data = {
        ...
        "scoped_teams": [42],
        "client_id": "insights-streamlit-apps-first-party",
        "exp": 99999999999,
    }
...
class TestCreateAppConfigValidation:
    ...
    def test_succeeds_with_valid_env(self, monkeypatch):
        ...
        assert app["insights_team_id"] == 42
        assert app["insights_streamlit_client_id"] == "insights-streamlit-apps-first-party"
...
class TestIntrospectTokenSecurity:
    ...
    async def test_accepts_token_with_matching_team_and_client_id(self):
        ...
        result = await streamlit_auth_proxy._introspect_token(
            ...
            "valid-token",
            team_id=42,
            expected_client_id="insights-streamlit-apps-first-party",
    ...
    async def test_rejects_token_from_other_application(self):
        ...
        session = _make_fake_session(
            ...
            body=_make_introspection_response(
                scoped_teams=[42],
                client_id="some-other-insights-app",
            ),
        )
    ...
    async def test_rejects_token_with_bridge_scope_on_iframe_path(self):
        ...
        sandbox→Insights backchannel hop.
        """
        session = _make_fake_session(
            status=200,
            body=_make_introspection_response(
...
class TestTokenInjectionHostScoping:
    ...
    foreign host, so appending _insights_token there would leak the viewer's
    token cross-origin."""
...
class TestTokenInjectionWithoutModalToken:
    ...
    the insights token (gating on the modal token would leave every Streamlit
    sub-request unauthenticated on Docker)."""

    def test_injects_insights_token_with_empty_modal_token(self):
        out = streamlit_auth_proxy._inject_token_into_html(
            b"<html><head></head><body><img src='/x.png'></body></html>", "", "ptok"
        ).decode()
        # insights token still appended to relative URLs and JS shim present
        assert "_insights_token=ptok" in out
