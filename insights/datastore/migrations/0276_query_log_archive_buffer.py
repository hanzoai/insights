...
# insights-cloud-infra (ansible managed_schemas), which repoints the writable at the buffer. This
...
# locally only, where insights-cloud-infra does not run, to keep dev parity.
...
operations = [
    ...
    # (not insights-cloud-infra), so created everywhere — cloud and local.
    ...
    # insights-cloud-infra. Drop + recreate is safe (Distributed holds no data, not replicated).
    *(
        []
        if _IS_CLOUD
        else [
