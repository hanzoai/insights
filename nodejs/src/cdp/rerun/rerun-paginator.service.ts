...
// Datastore's `DateTime64` parser only accepts 'YYYY-MM-DD HH:MM:SS[.fff]'.
...
const toDatastoreDateTime = (value: string): string => {
    if (!value) {
        return value
    }
 ...
 * cdp-events-consumer — script functions go to kafka, script flows to postgres-v2.
 ...
 * the cyclotron plumbing testable in isolation from the Datastore paging.
 */
export class RerunPaginatorService {
            ...
            if (queuedInvocations.length > 0) {
                ...
                if (function_kind === 'hog_flow') {
                    ...
                    if (runningIds.size > 0) {
                        logger.warn('Rerun skipping script invocations that are still in-flight', {
                            rerun_function_kind: function_kind,
                            rerun_function_id: function_id,
                    ...
                    if (invocationsToEnqueue.length > 0) {
                        await this.jobQueues.insights_function.queueInvocations(invocationsToEnqueue)
                    }
                }
    ...
    /**
     * Of the given script-function invocation ids, return those whose latest
    ...
    private async fetchPage(teamId: number, state: RerunJobState): Promise<InvocationRow[]> {
        ...
        // Datastore `DateTime64` only parses 'YYYY-MM-DD HH:MM:SS[.fff]'. Convert
        ...
        const windowStart = toDatastoreDateTime(filter.window_start)
        const windowEnd = toDatastoreDateTime(filter.window_end)
        const cursorScheduledAt = state.progress.cursor?.scheduled_at
            ? toDatastoreDateTime(state.progress.cursor.scheduled_at)
            : ''

        // Keyset pagination on (scheduled_at, invocation_id). When the cursor
        // is undefined we start from the top of the window. Datastore needs
        ...
        const result = await this.datastore.query({
            query: `/* team_id:${teamId} query_type:hog_invocation_rerun_page */
                SELECT
                    ...
                    -- which Datastore rejects. Any caller setting max_attempts (the poison-
    ...
    private async rehydrateBatch(
        ...
        // script function manager are LazyLoader-backed and batch their DB lookups
    ...
    private async rehydrateInvocation(
        ...
        if (functionKind === 'insights_function') {
            const insightsFunction = await this.insightsFunctionManager.getInsightsFunction(functionId)
            if (!insightsFunction || insightsFunction.team_id !== teamId) {
                return null
            }

            // Only re-enqueue types a cyclotron worker executes. Others (source webhooks,
            // transformations, site_*) run elsewhere and would never drain from the script
            ...
            if (!RERUNNABLE_FN_FUNCTION_TYPES.has(insightsFunction.type)) {
                logger.warn('⚠️', 'Skipping rerun of invocation with non-rerunnable function type', {
                    functionId,
                    teamId,
                    type: insightsFunction.type,
                    invocation_id: row.invocation_id,
                })
                return null
            }
            ...
            // from the current script function config, so the rerun runs against
            ...
            const parsed = InsightsFunctionInvocationGlobalsSchema.safeParse(parsedGlobals)
            if (!parsed.success) {
            ...
            const persistedGlobals = parsedGlobals as InsightsFunctionInvocationGlobalsWithInputs
        ...
        if (functionKind === 'hog_flow') {
            ...
            const filterGlobals: InsightsFunctionFilterGlobals = convertToInsightsFunctionFilterGlobal({
            ...
            invocation.state = {
                ...
                // Sticky rerun counter — mirror the script function path so the
