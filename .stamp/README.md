...
Declarative policy for the stamp PR-approval merge gate (`tools/pr-approval-agent/`).
...
- `policy.yml` - the global machine policy: deny categories, allow-list, size gate, tier thresholds, dismiss-time triviality rules, the folder delegation contract, and the ownership source (the `insightscli-resolver` input that feeds the reviewer's advisory team context via the shared insightscli resolver). Trusted data. Each rule's `rationale` records why the rule became what it is (which false positives drove an exclusion, and when) - historical justification like a commit message, not a claim about the present.
- `review-guidance.md` - the trusted review-norms prose injected into the reviewer's system prompt. Ordinary repo-formatted markdown. Editing it changes the production prompt directly, so update deliberately - the `stamp_policy` deny guarantees a human reviews every change.

## Proposing a policy change

Open a PR that edits these files.
Stamp can never auto-approve it: the `stamp_policy` deny category matches `.stamp/**`, any `AGENT_APPROVALS.md`, and `tools/pr-approval-agent/**`, so every change to the gate's own policy or engine routes to a human reviewer.
...
A folder may carry an `AGENT_APPROVALS.md` with a `stamp:` frontmatter block plus advisory prose.
Resolution:
...
- The frontmatter is a positive allow-list: only keys named in the `overrides` contract in `policy.yml` are read, within their ceilings. Anything else (unknown key, out-of-bounds value, unparseable frontmatter) invalidates the whole file - frontmatter and prose. An invalid file contributes nothing itself, but it does not cancel its ancestors: files under it still ride an ancestor's grant, or fall to the global pool if the chain grants nothing. Rationale: an author who can write an invalid file could equally delete it, so treating invalid as absent grants no extra power, and every `AGENT_APPROVALS.md` edit is human-reviewed via the `stamp_policy` deny anyway.
