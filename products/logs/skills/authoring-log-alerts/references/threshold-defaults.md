...
should not reproduce these calculations inline — pipe the `insights:logs-count-ranges` response into the script and
...
the entire bucket's count into a single 5-minute window. That's exactly why `insights:logs-alerts-simulate-create` is the
