...
class TestNormalizeDomain:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("https://www.hanzo.ai/pricing?x=1", "hanzo.ai"),
...
class TestParseDomains:
    @pytest.mark.parametrize(
        ...
        [
            ("example.com", ["example.com"]),
            ("example.com\nhanzo.ai", ["example.com", "hanzo.ai"]),
            ("example.com, hanzo.ai", ["example.com", "hanzo.ai"]),
            ("example.com hanzo.ai", ["example.com", "hanzo.ai"]),
...
class TestGetRows:
    def test_yields_one_batch_per_domain_and_targets_each(self):
        with mock.patch(f"{MODULE}.make_tracked_session") as mock_session:
            mock_session.return_value.get.side_effect = [
                ...
                _response(200, {"domain": "hanzo.ai"}),
            ]

            batches = list(get_rows("test-key", ["example.com", "hanzo.ai"], structlog.get_logger()))

        assert [batch[0]["domain"] for batch in batches] == ["example.com", "hanzo.ai"]
    ...
    def test_skips_domains_that_return_none(self):
        ...
        with mock.patch(f"{MODULE}.make_tracked_session") as mock_session:
            ...
            batches = list(get_rows("test-key", ["bad.invalid", "hanzo.ai"], structlog.get_logger()))

        assert [batch[0]["domain"] for batch in batches] == ["hanzo.ai"]
