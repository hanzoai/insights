...
class TestGetS3PathForPartition:
    ...
    def test_get_s3_path_for_partition_local(self):
        with patch(
            "insights.dags.events_backfill_to_ducklake.OBJECT_STORAGE_ENDPOINT",
...
class TestGetS3FunctionArgs:
    ...
    def test_local_mode_redacts_credentials(self):
        with patch(
            "insights.dags.events_backfill_to_ducklake.OBJECT_STORAGE_ACCESS_KEY_ID",
            "test_key_id",
        ):
            with patch(
                "insights.dags.events_backfill_to_ducklake.OBJECT_STORAGE_SECRET_ACCESS_KEY",
                "test_secret",
            ):
                args, safe_args = get_s3_function_args(
