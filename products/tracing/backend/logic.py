...
from insights.insightsql_queries.query_runner import AnalyticsQueryRunner, ExecutionMode, QueryRunner
...
def translate_span_filter(span_filter: SpanPropertyFilter) -> None:
    """Translate UI/API filter values into Datastore column representations, in place.

    The filter UI stores human-readable forms — hex ids, seconds for duration, label
    strings for `kind`/`status_code` — but the Datastore columns are base64 bytes,
...
def with_span_attribute_type_suffix(prop: SpanPropertyFilter) -> SpanPropertyFilter:
    ...
    Span attributes live in typed Datastore Maps; the property-group resolver routes a key by its
...
class TraceSpansQueryRunner(TraceSpansQueryRunnerMixin, AnalyticsQueryRunner[TraceSpansQueryResponse]):
    ...
    def _flat_spans(self) -> bool:
        ...
        Datastore's memory limit for hot child attributes (e.g. code.filepath). Flat mode skips the
    ...
    def to_query(self) -> ast.SelectQuery:
        ...
        if not by_duration:
            ...
            if cursor is not None:
                ...
                # Coarse day bound on time_bucket lets Datastore prune parts via the primary index.
    ...
    def _build_flat_spans_query(self, *, by_duration: bool, order_dir: str) -> ast.SelectQuery:
        ...
        # the paginator (see _calculate). The coarse UTC day bound lets Datastore prune parts first —
...
def run_service_names_query(
    ...
    response = execute_insightsql_query(
        ...
        team=team,
        workload=Workload.LOGS,
        filters=query_date_range.to_insightsql_filters(),
    ...
    query = parse_select(
        ...
        FROM (
            SELECT
                attribute_key,
                sum(attribute_count)
            FROM insights.trace_attributes
