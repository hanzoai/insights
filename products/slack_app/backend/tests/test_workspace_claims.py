...
from insights.models.integration import Integration, validate_slack_request
...
class TestSlackWorkspaceClaimsView(TestCase):
    ...
    def setUp(self):
        ...
        self.signing_secret = "insights-code-workspace-claims-secret"
    ...
    def test_other_kinds_for_same_id_do_not_count(self, mock_config):
        # Same integration_id can be reused across Insights integration kinds (e.g. a GitHub install
...
class TestDoesOtherRegionClaimWorkspace(TestCase):
    ...
    def setUp(self):
        cache.clear()
        self.signing_secret = "insights-code-helper-test-secret"
    ...
    def test_targets_eu_when_called_from_us(self):
        self._call(self._response(200, {"claimed": False}), incoming_host="us.hanzo.ai")
        assert "eu.hanzo.ai" in self.last_call.args[0]
        assert self.last_call.args[0].endswith("/slack/workspace/claims/")

    def test_targets_us_when_called_from_eu(self):
        self._call(self._response(200, {"claimed": False}), incoming_host="eu.hanzo.ai")
        assert "us.hanzo.ai" in self.last_call.args[0]
        assert self.last_call.args[0].endswith("/slack/workspace/claims/")
    ...
    def test_request_exception_returns_none(self):
        ...
        with (
            ...
            result = does_other_region_claim_workspace(
                slack_team_id="T123", kinds=["slack"], incoming_host="us.hanzo.ai"
            )
        assert result is None
    ...
    def test_definitive_true_answer_is_cached(self):
        ...
        with (
            ...
            first = does_other_region_claim_workspace(
                slack_team_id="T_CACHE", kinds=["slack"], incoming_host="eu.hanzo.ai"
            )
    ...
    def test_definitive_false_answer_is_cached(self):
        ...
        with (
            ...
            first = does_other_region_claim_workspace(
                slack_team_id="T_CACHE_FALSE", kinds=["slack"], incoming_host="eu.hanzo.ai"
            )
            second = does_other_region_claim_workspace(
                slack_team_id="T_CACHE_FALSE", kinds=["slack"], incoming_host="eu.hanzo.ai"
            )
    ...
    def test_none_answer_is_not_cached(self):
        ...
        with (
            ...
            first = does_other_region_claim_workspace(
                slack_team_id="T_FLAKE", kinds=["slack"], incoming_host="eu.hanzo.ai"
            )
            second = does_other_region_claim_workspace(
                slack_team_id="T_FLAKE", kinds=["slack"], incoming_host="eu.hanzo.ai"
            )
    ...
    def test_cache_is_keyed_by_kinds(self):
        ...
        with (
            ...
            mock_config.return_value = {"SLACK_APP_SIGNING_SECRET": self.signing_secret}
            mock_post.return_value = self._response(200, {"claimed": True})
            does_other_region_claim_workspace(slack_team_id="T_KIND", kinds=["slack"], incoming_host="eu.hanzo.ai")
            assert mock_post.call_count == 1
    ...
    def test_signed_request_is_accepted_by_validator(self):
        ...
        assert sent_headers["X-Insights-Region-Proxied"] == "1"
