...
from insights.api.integration import (
...
from insights.egress.github.transport import GitHubRateLimitError, github_request
from insights.exceptions_capture import capture_exception
from insights.models.integration import GITHUB_REPOSITORY_REFRESH_COOLDOWN_SECONDS, Integration
...
from insights.rate_limit import UserAuthenticationThrottle
...
class UserGitHubIntegrationItemSerializer(serializers.Serializer):
    id = serializers.UUIDField(help_text="Insights UserIntegration row id.")
...
class UserGitHubLinkStartRequestSerializer(serializers.Serializer):
    team_id = serializers.IntegerField(
        required=False,
        allow_null=True,
        help_text="Optional team/project id (e.g. Insights Desktop); web UI uses the session's current team.",
    )
    connect_from = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Optional client hint (e.g. insights_code) for return routing after OAuth.",
    )
...
class UserSlackIntegrationItemSerializer(serializers.Serializer):
    ...
    slack_user_id = serializers.CharField(help_text="Slack user id this Insights account is linked to.")
...
class UserSlackLinkableWorkspaceItemSerializer(serializers.Serializer):
    ...
    slack_team_name = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="Slack workspace display name as known by Insights.",
    )
...
class UserIntegrationViewSet(viewsets.GenericViewSet):
    ...
    def github_destroy(self, request: Request, installation_id: str, **_kwargs) -> Response:
        ...
        # Notify GitHub to uninstall the App, but only if no other Insights team or user
            ...
            results.append(
                {
                    ...
                    "insights_team_name": integration.team.name,
                    "insights_organization_name": integration.team.organization.name,
...
def _resolve_team_for_github_start(user: User, request: Request):
    ...
    Insights Desktop passes ``team_id`` (project/team) in the JSON body because the
