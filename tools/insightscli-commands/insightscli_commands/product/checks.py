...
class MisplacedFilesCheck(ProductCheck):
    ...
    # `insightsql_queries` is the established home for InsightsQL query runners across
...
class IsolationChainCheck(ProductCheck):
    ...
    def run(self, ctx: CheckContext) -> CheckResult:
        ...
        if facade_violations:
            ...
            remedies = (
                "move it to a garage (backend/insightsql_queries/, backend/max_tools.py, backend/temporal/, "
        ...
        # depend on these modules outside the import graph (Datastore DDL in the schema registry
        ...
        if has_narrowed and status.uncovered_permanent_exposures:
            ...
            result.issues.append(
                ...
                "graph (Datastore DDL in the schema registry and frozen migrations), so a change to them "
        ...
        # modules core depends on outside the import graph (Datastore DDL in a frozen migration or
        ...
        if status.unqualified_permanent_exposures:
            ...
            result.issues.append(
                ...
                "frozen Datastore migration or the Datastore schema registry — so they don't qualify as a "
...
class ProductYamlOwnersCheck(ProductCheck):
    """Validates product.yaml owner slugs against GitHub team slugs in the Insights org.
    ...
    def run(self, ctx: CheckContext) -> CheckResult:
        ...
        for owner in owners:
            ...
            if owner not in gh_teams:
                result.issues.append(
                    f"owner '{owner}' is not a GitHub team in the Insights org. "
                    f"See https://github.com/orgs/Insights/teams"
                )
...
class OrphanedTestFilesCheck(ProductCheck):
    ...
    _EXTERNAL_RUNNER_PREFIXES = (
        "dags/",  # ci-dagster.yml: pytest insights/dags products/**/dags
    )
