 ...
 * enforce a 60-char combined server_name + tool_name limit ("insights" is 7 chars).
 *
 ...
 * `_insightsUrl` link on tool results), checked against the generated app-url manifest
 * plus the SettingSectionId union. Pointing it at an API path is the easy mistake.
 *
 * Usage:
 *   pnpm --filter=@hanzo/mcp lint-tool-names
 */
...
/**
 * `url_prefix` is the frontend app route used to build the `_insightsUrl` link on tool results, so a
