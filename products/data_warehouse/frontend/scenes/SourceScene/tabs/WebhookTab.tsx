...
export function WebhookTab({ id }: { id: string }): JSX.Element {
    ...
    const insightsFunctionId = webhookInfo.insights_function?.id
    const hasConfiguration = !!sourceConfig && (sourceConfig.webhookFields?.length ?? 0) > 0
    const visibleSections = WEBHOOK_SECTIONS.filter(
        (key) => (key !== 'configuration' || hasConfiguration) && (key !== 'activity' || !!insightsFunctionId)
    )
    ...
    switch (activeSection) {
        ...
        case 'activity':
            body = (
                <>
                    {insightsFunctionId && <WebhookMetricsSection insightsFunctionId={insightsFunctionId} />}
                    {insightsFunctionId && <WebhookLogsSection insightsFunctionId={insightsFunctionId} />}
                </>
            )
            break
        default:
        ...
        <Card hoverEffect={false} className="space-y-3">
            ...
            <Form logic={webhookTabLogic} props={formLogicProps} formKey="webhookFieldInputs" enableFormOnSubmit>
                <div className="space-y-3 ph-no-capture">
                    ...
                    <Button type="primary" htmlType="submit" loading={isWebhookFieldInputsSubmitting}>
                        Save changes
                    </Button>
                </div>
            </Form>
...
function WebhookMetricsSection({ insightsFunctionId }: { insightsFunctionId: string }): JSX.Element {
    const logicKey = `webhook-metrics-${insightsFunctionId}`
    const logic = appMetricsLogic({
        ...
        forceParams: {
            ...
            appSourceId: insightsFunctionId,
...
function WebhookStatusSection({
    ...
    return (
        <Card hoverEffect={false} className="space-y-4">
            ...
            {externalStatus?.error && <Banner type="info">{externalStatus.error}</Banner>}
        </Card>
    )
}
        ...
        >
            ...
            <div className="flex flex-wrap gap-1">
                {missingEvents.map((event) => (
                    <Tag key={event} type="warning" className="text-xs">
                        {event}
                    </Tag>
                ))}
            </div>
                            ...
                            content: (
                                <div className="flex flex-wrap gap-1">
                                    {externalStatus.enabled_events.map((event) => (
                                        <Tag key={event} type="muted" className="text-xs">
                                            {event}
                                        </Tag>
                                    ))}
                                </div>
...
function WebhookDeleteSection({
    ...
    const handleDelete = (): void => {
        Dialog.open({
            ...
            description:
                'This will delete the webhook from Insights and attempt to remove it from the source. This action cannot be undone.',
            primaryButton: {
    ...
    return (
        <Card hoverEffect={false} className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <p className="mb-0">Remove the webhook from Insights and the source.</p>
                    {!canDelete && (
