...
description: >
  ABSOLUTE MUST to debug and inspect LLM/AI agent traces using Insights's MCP tools.
  ...
  or analyze token usage and costs. Also use when raw SQL/InsightsQL against
  `events.properties.$ai_input` / `$ai_output_choices` returns empty — message content lives only
  on the dedicated `insights.ai_events` table.
---

# Exploring LLM traces with MCP tools

Insights captures LLM/AI agent activity as traces. Each trace is a tree of events representing
...
| `insights:execute-sql`           | Ad-hoc SQL for complex trace analysis                         |
...
- `/ai-observability/traces/<trace_id>` or legacy `/llm-analytics/traces/<trace_id>` / `/llm-observability/traces/<trace_id>` is a single trace. Fetch it with `insights:query-llm-trace`.
- `/ai-observability/sessions/<session_id>` or legacy `/llm-analytics/sessions/<session_id>` is an AI session, not a trace. Fetch traces with `insights:query-llm-traces-list` filtered by event property `$ai_session_id`.
...
For a trace URL, call `insights:query-llm-trace` with:
...
For a session URL, call `insights:query-llm-traces-list` with:
...
- `_insightsUrl` — **always include this in your response** so the user can click through to the UI
...
The trace tools return `_insightsUrl` — always surface this to the user.

You can also construct links manually:

- **Trace detail**: `https://app.hanzo.ai/ai-observability/traces/<trace_id>?timestamp=<url_encoded_timestamp>&event=<optional_event_id>`
- **Traces list with filters**: returned in `_insightsUrl` from `query-llm-traces-list`
...
When presenting findings, always include the relevant Insights URL so the user can verify.

## Finding traces

Use `insights:query-llm-traces-list` to search and filter traces.
...
`insights:read-data-schema` first to discover what properties and values actually exist in the project's
...
1. **Confirm AI events exist** — call `insights:read-data-schema` with `kind: "events"` and look for `$ai_*` events
2. **Find filterable properties** — call `insights:read-data-schema` with `kind: "event_properties"` and `event_name: "$ai_generation"` (or another AI event) to see what properties are captured
3. **Get actual values** — call `insights:read-data-schema` with `kind: "event_property_values"`, `event_name: "$ai_generation"`, and `property_name: "$ai_model"` to see real model names in use
...
Use `insights:read-data-schema` to discover what custom properties exist, then filter:

1. Call `insights:read-data-schema` with `kind: "event_properties"` and `event_name: "$ai_trace"` to find custom properties
...
- [Single trace retrieval](./references/example-llm-trace.md.j2) — fetches a single trace by ID with all events and properties (renders the `TraceQuery` InsightsQL)
...
```json
[{ "type": "text", "text": "{\"results\": [...], \"_insightsUrl\": \"...\"}" }]
```
...
- Always include the `_insightsUrl` in your response so the user can click through
...
- In raw SQL, heavy content (`$ai_input` / `$ai_output` / `$ai_output_choices` / `$ai_input_state` / `$ai_output_state` / `$ai_tools`) lives only on the `insights.ai_events` table, not `events.properties` — see the [event reference](./references/events-and-properties.md) for the column mapping and trace-id-anchored query patterns
