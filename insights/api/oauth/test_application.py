...
class TestOrganizationOAuthApplicationViewSet(APIBaseTest):
    ...
    def test_list_returns_apps_for_current_org(self):
        ...
        response = self.client.get("/v1/organizations/@current/oauth_applications/")
    ...
    def test_unauthenticated_request_is_rejected(self):
        ...
        response = unauthenticated_client.get("/v1/organizations/@current/oauth_applications/")

        self.assertIn(response.status_code, [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN])

    def test_read_only_no_create(self):
        response = self.client.post(
            "/v1/organizations/@current/oauth_applications/",
            {"name": "New App"},
        )
    ...
    def test_read_only_no_detail_endpoint(self):
        ...
        response = self.client.get(f"/v1/organizations/@current/oauth_applications/{app.id}/")
