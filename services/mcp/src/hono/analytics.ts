...
import type { MCPAnalyticsIntentSource } from '@hanzo/mcp-analytics'
...
export async function trackInitEvent(state: ResolvedState): Promise<void> {
    try {
        ...
        getInsightsClient().captureInitialize({
...
export async function trackToolCall(
    ...
    try {
        ...
        getInsightsClient().captureToolCall({
            toolName,
            durationMs,
            isError,
            distinctId: state.distinctId,
 ...
 * together. Lands in LLM analytics, where online evaluations (LLM judge / Script) can
...
// sources carry `client_secret`, and script-function inputs carry `secret` values.
...
export async function trackToolsList(toolNames: string[], state: ResolvedState): Promise<void> {
    try {
        ...
        getInsightsClient().captureToolsList({
            toolNames,
            distinctId: state.distinctId,
            groups,
            ...(sessionUuid ? { sessionId: sessionUuid } : {}),
