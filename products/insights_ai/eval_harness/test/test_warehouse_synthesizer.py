...
def test_decimal_column_is_unique_to_the_type_needle():
    ...
    decimal_owners = [t.name for t in warehouse.tables for c in t.columns if c.insightsql == "DecimalDatabaseField"]
...
def test_retrieval_needle_is_queryable_with_duck_typed_content():
    ...
    assert all(c.insightsql == "StringDatabaseField" for c in needle.columns)
