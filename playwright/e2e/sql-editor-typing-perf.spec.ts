...
//     pnpm --filter=@hanzo/playwright exec playwright test sql-editor-typing-perf --workers 1
//
...
// subquery tail. This is what makes the per-keystroke InsightsQL parse expensive.
...
async function goToSqlEditor(page: Page): Promise<void> {
    ...
    await expect(page.getByTestId('insightsql-query-editor')).toBeVisible()
...
test.describe('SQL editor typing performance', () => {
    ...
    test('measure main-thread blocking while typing in a long query', async ({ page }, testInfo) => {
        ...
        const editorArea = page.getByTestId('insightsql-query-editor')
