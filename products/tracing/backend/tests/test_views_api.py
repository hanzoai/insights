...
class TestTracingViewAPI(APIBaseTest):
    base_url: str
    ...
    def _valid_payload(self, **overrides) -> dict:
        defaults = {
            ...
            "filters": {"serviceNames": ["checkout"], "viewMode": "traces"},
        }
    ...
    @patch("products.tracing.backend.presentation.views_api.report_user_action")
    def test_create(self, mock_report):
        data = self._create_via_api()
        assert data["name"] == "Slow checkout traces"
        assert data["filters"] == {"serviceNames": ["checkout"], "viewMode": "traces"}
        assert data["pinned"] is False
        assert data["short_id"] is not None
        assert data["created_by"]["id"] == self.user.pk

        mock_report.assert_called_once()
        assert mock_report.call_args[0][1] == "tracing view created"
        assert mock_report.call_args[0][2]["name"] == "Slow checkout traces"
    ...
    def test_retrieve(self):
        ...
        assert response.json()["name"] == "Slow checkout traces"
    ...
    def test_partial_update(self, mock_report):
        ...
        mock_report.assert_called_once()
        assert mock_report.call_args[0][1] == "tracing view updated"
        assert mock_report.call_args[0][2]["name"] == "Renamed view"

    def test_partial_update_preserves_filters(self):
        created = self._create_via_api(filters={"serviceNames": ["api"], "viewMode": "spans"})
        ...
        assert response.json()["filters"] == {"serviceNames": ["api"], "viewMode": "spans"}
    ...
    def test_delete(self, mock_report):
        ...
        assert not TracingView.objects.unscoped().filter(pk=created["id"]).exists()

        mock_report.assert_called_once()
        assert mock_report.call_args[0][1] == "tracing view deleted"
        assert mock_report.call_args[0][2]["name"] == "Slow checkout traces"
    ...
    def test_create_with_complex_filters(self):
        filters = {
            "dateRange": {"date_from": "-24h", "date_to": None},
    ...
    def test_list_only_own_team(self):
        ...
        TracingView.objects.unscoped().create(team=team2, name="Other team view", created_by=self.user)
    ...
    def test_cannot_retrieve_other_teams_view(self):
        team2 = Team.objects.create(organization=self.organization, name="Team 2")
        other_view = TracingView.objects.unscoped().create(team=team2, name="Other", created_by=self.user)
    ...
    def test_cannot_delete_other_teams_view(self):
        ...
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert TracingView.objects.unscoped().filter(pk=other_view.id).exists()
