"""Hand-written Hypothesis strategies for variable-content InsightsQL tokens.

The grammar codegen (``insights/insightsql/scripts/build_grammar_strategies.py``)
...
# All InsightsQL reserved keywords (hand-mirrored from InsightsQLParser.g4's
...
# generated strings recognisable; that's still a valid InsightsQL identifier.
...
# ``common/insightsql_parser/parser_json.cpp`` calls ``std::stoll(text)``
...
# landed upstream in ``common/insightsql_parser`` 1.3.42; once
...
# ─── Variable-content tokens for InsightsQLX / template strings ────────────────────
...
# Body text inside a InsightsQLX tag (`<tag>here</tag>`). INSIGHTSQLX_TEXT_TEXT excludes
# `<` and `{` (the tag and interpolation triggers).
insightsqlx_text_token: st.SearchStrategy[str] = st.text(
    alphabet="abcdefghijklmnopqrstuvwxyz 0123456789", min_size=1, max_size=8
)
