"""Recompute-oracle reads: Datastore member-set / segmentation queries + the Django run context.

Mixed IO (Datastore + ORM), precedented by :mod:`.snapshots`. The pure classification of what these
return lives in :mod:`.recompute`; this module only fetches.

Person resolution mirrors the seeder's overrides join (``rust/cohort-seeder/src/datastore/sql.rs``):
