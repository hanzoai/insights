...
import { LogEntry } from 'scenes/custom-functions/logs/logsViewerLogic'

import { Optional } from '~/types'

import { CustomFlowAction } from '../types'

export type CustomFlowStepNodeProps = NodeProps & {
    data: CustomFlowAction
    type: CustomFlowAction['type']
}
...
const _commonActionFields = {
    ...
    output_variable: z // The CustomFlow-level variable to store the output of this action into
        .union([
            z.object({
                key: z.string(),
...
export const CustomFlowTriggerSchema = z.discriminatedUnion('type', [
    z.object({
...
export const CustomFlowActionSchema = z.discriminatedUnion('type', [
    // Trigger
    z.object({
        ..._commonActionFields,
        type: z.literal('trigger'),
        config: CustomFlowTriggerSchema,
    }),
    // Branching
    z.object({
        ..._commonActionFields,
...
): action is Extract<CustomFlowAction, { type: 'function_email' | 'function_sms' }> => {
    return ['function_email', 'function_sms'].includes(action.type)
}

export const isFunctionAction = (
    action: CustomFlowAction
): action is Extract<CustomFlowAction, { type: 'function' | 'function_sms' | 'function_email' }> => {
...
> => {
    ...
    const trigger = action as Extract<CustomFlowAction, { type: 'trigger' }>
    return ['webhook', 'tracking_pixel', 'manual', 'schedule'].includes(trigger.config.type)
}

export interface CustomFlowTestResult {
