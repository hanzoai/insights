...
class TestConversationEvents(BaseTest):
    ...
    @patch("products.conversations.backend.events.get_persons_by_distinct_ids", side_effect=Exception("personinsights down"))
    def test_capture_ticket_created_person_lookup_failure_falls_through_to_email_fallback(
        ...
        # A transient personinsights failure must degrade to the email fallback, not lose attribution.
        from insights.models.person.person import Person
    ...
    @patch(
        ...
        side_effect=Exception("datastore unavailable"),
    )
    @patch("products.conversations.backend.events.get_persons_by_distinct_ids")
    def test_capture_ticket_created_analytics_failure_falls_through_to_person_property(
        ...
        # A transient Datastore failure in the analytics fallback must degrade to the profile fallback.
        from insights.models.person.person import Person
    ...
    def test_capture_ticket_created_email_fallback_groups(
        ...
        """Non-web channels stuff the email into distinct_id; org is resolved via the email->Datastore fallback."""
        from insights.models.person.person import Person
        ...
        mock_get_persons.return_value = []
        # ...but the Datastore email lookup returns the person with their real distinct_id
        person = Person(team_id=self.team.id, is_identified=True)
        person._distinct_ids = ["real-did-1"]
    ...
    @patch("products.conversations.backend.events.capture_internal")
    @patch("products.conversations.backend.events.get_groups_by_identifiers", side_effect=Exception("personinsights down"))
    @patch("products.conversations.backend.events.get_group_types_for_project")
    @patch("products.conversations.backend.events._resolve_groups_from_analytics", return_value=None)
    @patch("products.conversations.backend.events.get_persons_by_distinct_ids")
    ...
    def test_capture_ticket_created_email_fallback_person_property_org(
        ...
        # distinct_id (the email) resolves no identified person...
        mock_get_persons.return_value = []
        # ...the Datastore email lookup finds the person, whose real distinct_id has no membership
    ...
    @patch("products.conversations.backend.events.capture_internal")
    @patch("insights.insightsql.query.execute_insightsql_query")
    @patch("products.conversations.backend.events.get_group_types_for_project")
    @patch("products.conversations.backend.events.get_persons_by_distinct_ids")
    def test_capture_ticket_created_analytics_fallback_groups(
        self, _name, customer_key, mock_get_persons, mock_group_types, mock_insightsql, mock_capture
    ):
        ...
        mock_insightsql.return_value.results = [["org-eu-123", customer_key]]
    ...
    def test_capture_ticket_created_analytics_fallback_no_groups(
        self, _name, results, mock_get_persons, mock_group_types, mock_insightsql, mock_capture
    ):
        from insights.models.person.person import Person

        mock_get_persons.return_value = [Person(team_id=self.team.id, is_identified=True)]
        mock_group_types.return_value = [{"group_type": "organization", "group_type_index": 1}]
        mock_insightsql.return_value.results = results
    ...
    def test_capture_ticket_created_membership_hit_skips_analytics_fallback(
        self, mock_get_persons, mock_insightsql, mock_capture
    ):
        from insights.models.person.person import Person
    ...
    def test_capture_ticket_created_email_channel_analytics_fallback_groups(
        self, mock_get_persons, mock_get_by_email, mock_group_types, mock_insightsql, mock_capture
    ):
        ...
        mock_group_types.return_value = [{"group_type": "organization", "group_type_index": 0}]
        mock_insightsql.return_value.results = [["org-eu-123", ""]]
    ...
    def test_analytics_fallback_result_is_cached(
        self, _name, results, expect_groups, mock_get_persons, mock_group_types, mock_insightsql, mock_capture
    ):
        from insights.models.person.person import Person
    ...
    def test_capture_ticket_created_no_person_means_no_event_keyed_attribution(
        self, _name, lookups_fail, mock_get_persons, mock_get_by_email, mock_group_types, mock_insightsql, mock_capture
    ):
        if lookups_fail:
            mock_get_persons.side_effect = Exception("personinsights down")
            mock_get_by_email.side_effect = Exception("person query failed")
        else:
            mock_get_persons.return_value = []
            mock_get_by_email.return_value = {}
        mock_group_types.return_value = [{"group_type": "organization", "group_type_index": 0}]
        mock_insightsql.return_value.results = [["attacker-org", ""]]
    ...
    def test_capture_ticket_created_identified_person_without_org_falls_back_to_slack_channel(
        self, mock_get_persons, mock_group_types, mock_insightsql, mock_capture, mock_get_account
    ):
        ...
        mock_get_persons.return_value = [Person(team_id=self.team.id, is_identified=True)]
        mock_group_types.return_value = [{"group_type": "organization", "group_type_index": 1}]
        mock_insightsql.return_value.results = []
        mock_get_account.return_value = ca_contracts.AccountRef(
            id=str(uuid.uuid4()), name="Acme Corp", external_id="acme-org-1"
        )
...
class TestResolveGroupsFromAnalyticsDatastore(DatastoreTestMixin, APIBaseTest):
    """End-to-end coverage of the cross-region fallback against real Datastore events.
    ...
    insights.group(type, key) without properties — to prove resolution doesn't depend on
