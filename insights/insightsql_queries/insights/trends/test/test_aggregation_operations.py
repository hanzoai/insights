...
@pytest.mark.django_db
def test_insightsql_math_strips_outer_user_alias(math_insightsql: str):
    """The user's outer alias on a `math=insightsql` expression is shadowed by the downstream
    `AS total` wrap. Leaving it in place causes Datastore's new analyzer to reject the
    ...
    """
    team = Team()
    series = EventsNode(event="$pageview", math="insightsql", math_insightsql=math_insightsql)
    query_date_range = QueryDateRange(date_range=None, interval=None, now=datetime.now(), team=team)
