...
class NonReportableError(Exception):
    ...
    always caused by the customer's config or the upstream API (never a Insights defect) and that
    retrying can't resolve, so a tracked exception would only be noise."""


# Bound error strings so a multi-MB str(e) (Datastore 5xx body, Playwright HTML dump)
