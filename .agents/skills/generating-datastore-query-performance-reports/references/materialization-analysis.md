...
`Insights/query-performance-analysis` repo as `analysis/<date>-materialization-candidates.md`.

Run everything via `query-datastore-via-metabase`. Split wide column-list checks into batches of ~5-6
...
`JSONExtractString(properties, '$foo')` in user-written InsightsQL, which creates an `ast.Call` that skips
`visit_property_type()`, rather than `properties.$foo` which materializes. Known sources: InsightsQL /
...
FROM clusterAllReplicas(insights, system, columns)
...
running anything, the operator should sanity-check with #team-datastore that the cluster is not in a bad
...
- Create: `create_materialized_column` (team-datastore location, per region). Backfilling adds cluster
