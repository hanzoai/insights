...
describe('batchExportBackfillsLogic', () => {
    ...
    describe('cancelBackfill', () => {
        it('calls cancel API and reloads backfills', async () => {
            ...
            expect(toast.success).toHaveBeenCalledWith('Backfill has been cancelled.')
        })
        ...
        it('shows error toast on cancel failure', async () => {
            ...
            expect(toast.error).toHaveBeenCalledWith('Failed to cancel backfill. Please try again.')
        })
    })
    ...
    describe('backfillCreated - estimate polling', () => {
        ...
        it('polls until estimate becomes available', async () => {
            ...
            expect(toast.info).toHaveBeenCalledWith('Estimated ~5,000 rows to export', expect.anything())
        })
        ...
        it('ignores polling errors and continues', async () => {
            ...
            expect(toast.info).toHaveBeenCalledWith('Estimated ~3,000 rows to export', expect.anything())
        ...
        it('cancel button in toast calls cancel API', async () => {
            ...
            const toastCall = (toast.info as jest.Mock).mock.calls[0]
            ...
            expect(toast.success).toHaveBeenCalledWith('Backfill cancelled')
        })
    })
})
