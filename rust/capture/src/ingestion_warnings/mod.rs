...
//!   built from the `Insights-Sdk-Info` header it requires. Legacy's is the
...
/// the bound v1 puts on the `Insights-Sdk-Info` header (real values are ~20
...
mod tests {
    ...
    fn first_event_supplies_attribution_for_the_whole_batch() {
        let events = vec![
            ...
            raw_event(json!({"$lib": "insights-python", "$lib_version": "9.9.9"})),
        ];
        assert_eq!(
