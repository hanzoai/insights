import {
    ...
    type InsightsNotificationParamsByMethod,
    type StoredLogEntry,
    isKeepaliveFrame,
...
// Canonical `_insights/*` payloads as the emitters produce them (products/tasks for the core
// methods, the agent adapters for the rest).
const NOTIFICATION_PARAMS_BY_METHOD: { [M in keyof InsightsNotificationParamsByMethod]: Record<string, unknown>[] } = {
    '_insights/console': [{ sessionId: RUN_ID, level: 'info', message: 'Provisioning sandbox' }],
    '_insights/progress': [
        {
            sessionId: RUN_ID,
            ...
            group: 'setup',
            detail: 'Insights/insights @ master',
        },
        { sessionId: RUN_ID, step: 'start_agent', status: 'completed', label: 'Starting agent', group: 'setup' },
    ],
    '_insights/sandbox_output': [{ sessionId: RUN_ID, stdout: 'ok\n', stderr: '', exitCode: 0 }],
    '_insights/user_message': [{ content: 'Why did checkout conversion drop last week?' }],
    '_insights/usage_update': [
        {
    ...
    '_insights/sdk_session': [{ taskRunId: RUN_ID, sessionId: 'sess_a1b2c3', adapter: 'claude' }],
    '_insights/resources_used': [
        {
            sessionId: 'sess_a1b2c3',
            products: [
    ...
    '_insights/permission_resolved': [{ requestId: 'perm-1', toolCallId: 'toolu_01', optionId: 'allow_once' }],
    '_insights/run_started': [{ sessionId: 'sess_a1b2c3', runId: RUN_ID, taskId: TASK_ID, agentVersion: '1.42.0' }],
...
describe('wireTypes guards', () => {
    ...
    it('tolerates the Codex split frames and the Claude combined frame with "has field" checks', () => {
        const [used, usedNullCost, breakdown, combined] = NOTIFICATION_PARAMS_BY_METHOD['_insights/usage_update'].map(
            (params) => notificationOf(notification('_insights/usage_update', params))
        )
        for (const wireNotification of [used, usedNullCost]) {
            if (!isInsightsNotification(wireNotification, '_insights/usage_update')) {
        ...
        if (!isInsightsNotification(breakdown, '_insights/usage_update')) {
            throw new Error('expected usage_update notification')
        }
        ...
        if (!isInsightsNotification(combined, '_insights/usage_update')) {
            throw new Error('expected usage_update notification')
        }
    ...
    it('exposes typed progress params after narrowing', () => {
        const wireNotification = notificationOf(
            notification('_insights/progress', NOTIFICATION_PARAMS_BY_METHOD['_insights/progress'][0])
        )
        if (!isInsightsNotification(wireNotification, '_insights/progress')) {
            throw new Error('expected progress notification')
        }
        expect(wireNotification.params?.label).toEqual('Cloning repository')
        expect(wireNotification.params?.detail).toEqual('Insights/insights @ master')
    })
    ...
    it('still classifies degenerate notification bodies as notification frames', () => {
        ...
        expect(isNotificationFrame(notification('_insights/turn_complete'))).toBe(true)
        expect(
            isNotificationFrame({
                type: 'notification',
                notification: { method: '_insights/console', params: 'not-an-object' },
            })
        ).toBe(true)
    })
})
