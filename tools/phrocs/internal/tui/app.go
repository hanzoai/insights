...
type Model struct {
	...
	// Mascot mode: animated mascot walks across the output pane
	mascotMode  bool
	mascotX     int
	mascotY     int // vertical offset from ground (positive = up)
	mascotVelY  int // vertical velocity (positive = upward, decreases each tick)
	mascotDir   int // +1 right, -1 left
	mascotFrame int
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		} else if m.mascotMode {
			m, cmds, handled = m.handleMascotKey(msg, cmds)
		}
		if !handled {
