...
class TestLiveReportIdsForSourceIds(APIBaseTest):
    def test_query_compiles_against_datastore(self) -> None:
        ...
        # `source_product` in an outer WHERE. If InsightsQL rejects either, the helper raises on every
        # call and the caller degrades to "no linked reports" for everyone, silently. Run it for
        # real against an empty Datastore: it must compile and return an empty map.
    ...
    def test_no_source_ids_skips_datastore(self) -> None:
        with patch("products.signals.backend.temporal.signal_queries.execute_insightsql_query") as execute:
...
class TestReportsSourceIdFilter(APIBaseTest):
    ...
    def test_filters_to_reports_the_source_record_contributed_to(self) -> None:
        ...
        # A single source_product narrows the Datastore scan.
    ...
    def test_absent_filter_does_not_touch_datastore(self) -> None:
        with patch(VIEWS_FETCH) as fetch:
