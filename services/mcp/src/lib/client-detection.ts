 ...
 *   `x-insights-mcp-mode` override — see `resolveMode`.
 *
 ...
 *
 * - `isInsightsCodeConsumer()` matches the `x-insights-mcp-consumer` header
 *   sent by the Insights Desktop Tasks wrapper.
 *
...
// Value sent in `x-insights-mcp-consumer` by Insights Desktop (the Tasks sandbox
...
// `Cowork` supports both), so they get the same treatment as the Insights Desktop
// consumer.
...
export class MCPClientProfile {
    ...
    isInlineExecUiHost(): boolean {
        ...
        // Like Insights Desktop, these hosts surface `structuredContent` to the model, so
...
export function isInsightsCodeConsumer(mcpConsumer: string | undefined): boolean {
    return new MCPClientProfile({ consumer: mcpConsumer }).isInsightsCodeConsumer()
}
