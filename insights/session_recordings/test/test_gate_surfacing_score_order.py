...
from insights.session_recordings.utils import gate_surfacing_score_order
...
def _user() -> User:
    ...
    return cast(User, SimpleNamespace(distinct_id="abc123", email="nicholas.w@hanzo.ai"))
...
class TestGateSurfacingScoreOrder(TestCase):
    ...
    def test_non_surfacing_orders_never_evaluate_the_flag(self, _name, order, expected):
        with mock.patch("insights.session_recordings.utils.hanzo_insights.feature_enabled") as feature_enabled:
            query = _query(order)
    ...
    def test_surfacing_score_kept_only_for_rollout_or_experiment_test_arm(
        ...
        with (
            mock.patch(
                "insights.session_recordings.utils.hanzo_insights.feature_enabled", return_value=surfacing_enabled
            ),
            mock.patch(
                "insights.session_recordings.utils.hanzo_insights.get_feature_flag", return_value=experiment_variant
            ),
        ):
