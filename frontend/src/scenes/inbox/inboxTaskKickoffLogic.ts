...
function buildCreatePrReportPrompt(report: SignalReport, feedback?: string): string {
    const base = `Act on Insights Inbox report "${report.title ?? report.id}" (id ${report.id}). Investigate the root cause using the report's contributing findings, implement the fix, and open a PR.${
        report.summary ? `\n\nReport summary:\n${report.summary}` : ''
    }`
...
function buildDiscussReportPrompt(reportUrl: string, question: string): string {
    ...
    return `Answer this question about the Insights Inbox report at ${reportUrl}:\n\n${question.trim()}`
}
    ...
    listeners(({ actions, values }) => ({
        discussReport: async ({ report, reportUrl, question }) => {
            ...
            if (values.aiConsentDisabledReason) {
                toast.error(values.aiConsentDisabledReason)
            ...
            try {
                ...
                toast.error(error?.detail || error?.message || "Couldn't ask AI about this report. Try again.")
        ...
        createPrFromReport: async ({ report }) => {
            ...
            try {
                ...
                toast.error(error?.detail || error?.message || 'Failed to start PR task')
