...
pub trait RowBinaryRead: Read {
    ...
    // Datastore `String` is byte-typed: user-data fields (breakdown keys, event
    ...
    // Datastore writes UUID as two little-endian u64s (high half then low half),
...
mod tests {
    ...
    // Fixture pins Datastore's UUID byte order: two LE u64s (hi, lo).
    // This matches `SELECT toUUID('01020304-0506-0708-090a-0b0c0d0e0f10') FORMAT RowBinary`.
    #[test]
    fn uuid_datastore_byte_order() {
