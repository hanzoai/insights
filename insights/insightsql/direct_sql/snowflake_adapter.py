...
from insights.insightsql.snowflake_connection_cache import cached_snowflake_connection
...
# with no LIMIT. InsightsQL-authored queries already carry a LIMIT from the printer.
...
# reused session. InsightsQL-authored SQL never emits these — only the raw passthrough path can — so
...
SNOWFLAKE_FIELD_NAME_TO_DATASTORE_TYPE: dict[str, str] = {
    "REAL": "Float64",
    "TEXT": "String",
    "BOOLEAN": "Bool",
    "DATE": "Date",
...
def snowflake_field_type_to_datastore_type(type_code: object | None, scale: object | None = None) -> str:
    ...
    return SNOWFLAKE_FIELD_NAME_TO_DATASTORE_TYPE.get(name, "String")
...
def validate_snowflake_account_id(account_id: str | None) -> str:
    ...
    if not candidate or not _SNOWFLAKE_ACCOUNT_ID_RE.fullmatch(candidate):
        raise ExposedInsightsQLError("Invalid Snowflake account identifier.")
    return candidate
...
def ensure_read_only_raw_snowflake_statement(sql: str) -> str:
    ...
    if len(statements) != 1 or statements[0].get_type() != "SELECT":
        raise ExposedInsightsQLError(RAW_SNOWFLAKE_READ_ONLY_ERROR)
    ...
    for token in statements[0].flatten():
        ...
        if token.ttype in sqlparse_tokens.Name:
            ...
            if name.startswith("SYSTEM$") or name in _RAW_SNOWFLAKE_BLOCKED_FUNCTIONS:
                raise ExposedInsightsQLError(RAW_SNOWFLAKE_BLOCKED_FUNCTION_ERROR)
    return sql
...
def _fetch_capped_snowflake_rows(cursor: snowflake.connector.cursor.SnowflakeCursor) -> list:
    ...
    if len(rows) > DIRECT_SNOWFLAKE_MAX_ROWS:
        ...
        raise ExposedInsightsQLError(DIRECT_SNOWFLAKE_ROW_CAP_ERROR)
    return list(rows)


class SnowflakeAdapter:
    engine = "snowflake"
    dialect: InsightsQLDialect | None = "snowflake"
    ...
    def validate_source_config(
        ...
        if not (is_direct_capable(source) and source.direct_engine == self.engine):
            raise ExposedInsightsQLError("Invalid direct Snowflake connection.")
    ...
    def execute(self, request: DirectQueryRequest) -> DirectQueryResult:
        ...
        (`prepare_raw_sql`), InsightsQL-authored queries only ever emit SELECT, and the session pins
        ...
        except (snowflake.connector.errors.Error, ExposedInsightsQLError) as error:
            span.set_attribute("error_type", error.__class__.__name__)
            if request.debug:
                return DirectQueryResult(
                    results=[], types=[], print_columns=[], error=snowflake_error_to_message(error)
                )
            raise ExposedInsightsQLError(snowflake_error_to_message(error)) from error

        span.set_attribute("row_count", len(results))
        types: list[tuple[str, str]] = [
            (column[0], snowflake_field_type_to_datastore_type(column[1], column[5] if len(column) > 5 else None))
