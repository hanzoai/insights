...
Intent mirrors ``ee/scriptai/eval/ci/eval_retention.py`` — the CI version asserts
on the exact ``AssistantRetentionQuery`` Max produces, this version exercises
the same intents end-to-end through the sandboxed agent + Insights MCP tools
and judges the retention query the agent ran via the ``query-retention`` MCP
tool.

To run:
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_retention"
"""

from __future__ import annotations

from insights.schema import AssistantRetentionEventsNode, AssistantRetentionFilter, AssistantRetentionQuery
...
from products.insights_ai.evals.product_analytics.scorers import (
    INSIGHT_WRITE_TOOLS,
