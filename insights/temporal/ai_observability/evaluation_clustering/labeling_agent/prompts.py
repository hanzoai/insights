...
Each cluster contains `$ai_evaluation` events — judgments produced by LLM-as-a-judge evaluators or rule-based (Script) evaluators over LLM generations. Evaluations were grouped by the semantic similarity of their `reasoning` text, so each cluster represents a recurring **failure mode, passing pattern, or N/A scenario** the evaluators keep running into.
...
- `runtime` — either `llm_judge` (LLM-as-judge) or `script` (deterministic rule-based)
...
- **get_evaluator_config(evaluator_id=..., evaluator_name=...)**: Fetch the evaluator's full configuration — name, description, runtime, and either the llm_judge **prompt text** or the **script source code** that decides pass/fail. Provide exactly one of `evaluator_id` or `evaluator_name`. Use this when your cluster-label description would benefit from explicitly grounding in the evaluator's *rubric*: what criterion it checks, what thresholds or phrasing it uses. Especially useful for script-runtime clusters whose reasoning is terse ("OK", "Total tokens 17250 exceeds 4000") — reading the script source tells you what the passing threshold actually is.
...
- **Distinguish evaluator runtimes** if mixed — an `llm_judge` cluster "about factuality" and a `script` cluster "failing deterministic length checks" are very different even if they co-cluster.
