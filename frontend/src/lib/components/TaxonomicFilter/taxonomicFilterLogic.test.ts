...
describe('taxonomicFilterLogic', () => {
    ...
    beforeEach(() => {
        useMocks({
            get: {
                ...
                '/v1/environments/:team/sessions/property_definitions': ({ request }) => {
                    const search = new URL(request.url).searchParams.get('search')
                    const results = search
    ...
    it('taxonomicGroups keeps a stable reference across equal-but-freshly-allocated object props', () => {
        ...
        const propsA: TaxonomicFilterLogicProps = {
            ...logic.props,
            metadataSource: { kind: NodeKind.InsightsQLQuery, query: 'select 1' },
        }
    ...
    describe('SQL expression (InsightsQLExpression) group commits its value', () => {
        let insightsqlLogic: ReturnType<typeof taxonomicFilterLogic.build>
        const onChange = jest.fn()

        beforeEach(() => {
            onChange.mockClear()
            insightsqlLogic = taxonomicFilterLogic({
                taxonomicFilterLogicKey: 'insightsqlExpressionTest',
                taxonomicGroupTypes: [TaxonomicFilterGroupType.InsightsQLExpression],
                onChange,
            })
            insightsqlLogic.mount()
        })

        afterEach(() => {
            insightsqlLogic.unmount()
        })

        it('getValue returns the expression so the headless menu can commit it', () => {
            const group = insightsqlLogic.values.taxonomicGroups.find(
                (g) => g.type === TaxonomicFilterGroupType.InsightsQLExpression
            )
        ...
        it('selectItem with the derived value fires onChange with the expression, not null', async () => {
            ...
            await expectLogic(insightsqlLogic, () => {
                insightsqlLogic.actions.selectItem(group, value, item as any)
...
describe('propertyTaxonomicGroupProps', () => {
    ...
    describe('person properties group labels only core person properties as Insights properties', () => {
        const { getPopoverHeader } = propertyTaxonomicGroupProps(CORE_FILTER_DEFINITIONS_BY_GROUP.person_properties)

        it.each([
            {
                property: 'email',
                expected: 'Insights property',
                description: 'email is a core Insights person property',
            },
            {
                property: '$email',
                expected: 'Property',
        ...
        it.each([
            {
                property: '$browser',
                expected: 'Insights property',
                description: '$browser is a core Insights event property',
            },
            {
                property: 'my_custom_prop',
                expected: 'Property',
                description: 'custom event properties are not labeled as Insights properties',
            },
        ])('$description', ({ property, expected }) => {
            expect(getPopoverHeader!(makePropDef(property))).toBe(expected)
        })
