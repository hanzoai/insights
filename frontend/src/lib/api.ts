...
export class ApiRequest {
    ...
    public script(teamId?: TeamType['id']): ApiRequest {
        return this.projectsDetail(teamId).addPathComponent('script')
    }
...
/**
 * Insights session/distinct tracing headers. Omitted in OAuth mode because the cross-origin CORS
...
function tracingHeaders({ includeDistinctId = false } = {}): Record<string, string> {
    ...
    return {
        ...(sessionId ? { 'X-INSIGHTS-SESSION-ID': sessionId } : {}),
        ...(distinctId ? { 'X-INSIGHTS-DISTINCT-ID': distinctId } : {}),
    }
}
...
// a Insights event so we can aggregate which claim shape is causing 401s. Sensitive
// fields are masked.
function captureLivestream401Debug(url: string, authHeader: string | undefined, serverErrorData: any): void {
    try {
        ...
        if (!authHeader) {
            insights.capture('livestream_401_debug', {
                ...props,
...
const api = {
    ...
    notebooks: {
        ...
        async sqlV2Run(
            ...
            data: {
                node_id: string
                code: string
                refs?: Record<string, { node_id: string; kind: 'insightsql' | 'local' }>
                node_type?: 'insightsql' | 'python'
                output_name?: string
            }
    ...
    signalScout: {
        runs: {
            ...
            // in a single Datastore round-trip, instead of one query per run.
    ...
    externalDataSources: {
        ...
        async check_cdc_prerequisites(
            payload: {
                source_type: ExternalDataSourceType
                cdc_management_mode: 'insights' | 'self_managed'
                tables?: string[]
    ...
    integrations: {
        ...
        authorizeUrl(params: { kind: string; next?: string; extraParams?: Record<string, string> }): string {
            ...
            // insights connection's `region`/`scopes`) rides along in `extraParams` rather than
            ...
            onopen: async (response) => {
                ...
                if (response.status === 429) {
                    ...
                    if (response.status === 401) {
                        ...
                        insights.capture('livestream_sse_non_ok_non_401', {
                            url,
                    ...
                    onOpen?.()
                    if (isLivestreamUrl) {
                        insights.capture('livestream_sse_opened', {
                            url,
                            status: response.status,
                        })
                    }
...
async function handleFetch(
    ...
    if (!response.ok) {
        ...
        if (insights.capture) {
            insights.capture('client_request_failure', {
                pathname,
                method,
                duration,
                status: response.status,
