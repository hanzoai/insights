...
impl<'a, E: Emitter + Clone> Parser<'a, E> {
        ...
        loop {
            ...
            if let Some((lbp, rbp, op)) = infix_bp(kind) {
                ...
                if kind == TokenKind::Percent {
                    ...
                    // a InsightsQLX tag: `1 % <a/>` is `1 modulo <tag>`. Don't treat
                    ...
                    if (!peek_can_start_clause_body(next) || is_pure_infix_op(next))
                        && !self.peek_next_starts_insightsqlx_tag()
                    {
                        break;
                    }
                }
    ...
    pub(crate) fn parse_prefix(&mut self) -> Result<E::Value, ParseError> {
        ...
        match self.peek() {
            ...
            TokenKind::Keyword(Kw::Not) => {
                ...
                // `not <tag>` — a `<` that begins a InsightsQLX tag is NOT the
                ...
                if self.peek_next_starts_insightsqlx_tag() {
            ...
            _ => {
                // `<Tag ...>` — InsightsQLX tag in column position (cpp's
                ...
                if self.peek_starts_insightsqlx_tag() {
                    return self.parse_insightsqlx_tag_element();
                }
                self.parse_primary()
            }
        }
    ...
    fn parse_primary(&mut self) -> Result<E::Value, ParseError> {
        ...
        match tok.kind {
            ...
            // A InsightsQLX tag is a valid interval value (`interval <t/> second` →
            ...
            // string: at expression level `expect_eof` rejects, but inside a Script
    ...
    /// char). Datastore stores intervals as Int64, so the count is accepted
    ...
    fn parse_columns_expr(&mut self) -> Result<E::Value, ParseError> {
            ...
            )
        {
            ...
            //       (line 288 of InsightsQLParser.g4). cpp wraps the asterisk
            ...
            // The qualifier is the grammar's `identifier`, so only keywords admitted by `kw_valid_as_identifier` qualify — Script-statement keywords (try/catch/finally) and the rest of the omitted set are not Field qualifiers, so cpp rejects `columns(try.*)`; without this gate rust took it as a qualified-asterisk ColumnsExpr.
    ...
    /// Script block (`(x) -> { stmt stmt }`). The block form lets a
    ...
    pub(crate) fn try_bare_list_lambda(&mut self) -> Result<Option<E::Value>, ParseError> {
        ...
        // rule omits NULL / INF / NAN and the Script-statement keywords).
    ...
    pub(crate) fn set_lexer_pos(&mut self, pos: usize) -> Result<(), ParseError> {
        // Carry the InsightsQLX tag-mode flag across the re-seek — restores /
    ...
    /// cpp's grammar (InsightsQLParser.g4:219-225) has these as bare-LPAREN
    ...
    pub(crate) fn parse_ident_lead(&mut self) -> Result<E::Value, ParseError> {
            ...
            if matches!(self.peek(), TokenKind::Keyword(Kw::Over)) {
                ...
                let (over_expr, over_identifier) = if self.peek() == TokenKind::LParen {
                    ...
                    let id = match tok.kind {
                        ...
                        // A window name is an `identifier`, which admits only `kw_valid_as_identifier` keywords — the Script-statement keywords (try / catch / finally / …) are not valid window names.
                        TokenKind::Keyword(kw) if kw_valid_as_identifier(kw) => {
                            identifier_text(self.text(tok), tok.kind)
                        }
                        _ => {
        ...
        while self.peek() == TokenKind::Dot {
            ...
            match part.kind {
                ...
                // EXCEPT/INTERSECT and Script-statement keywords; gate
    ...
    pub(crate) fn peek_is_clause_terminator(&self) -> bool {
        ...
        // subquery, `{}` placeholder, or a insightsqlx `<` tag. `from + 1`
        ...
        match kind {
            ...
            TokenKind::Dot => {
                ...
                match part.kind {
                    ...
                    // NAN/EXCEPT/INTERSECT and the Script-statement keywords
    ...
    fn try_special_infix(
        ...
        match kind {
            ...
            TokenKind::Keyword(Kw::Is) => {
                ...
                // neither follows (e.g. Script program source like `this is a
            ...
            TokenKind::Keyword(Kw::As) => {
                ...
                // keywords (NULL/INF/NAN/EXCEPT/INTERSECT and the Script-
...
/// Is `text` a InsightsQL `DECIMAL_LITERAL` (base-10 integer) as the lexer would
