...
import { customFlowOutputMappingLogic } from './customFlowOutputMappingLogic'

export function CustomFlowEditorPanelBuildDetail(): JSX.Element | null {
    const { selectedNode, workflow, categories, categoriesLoading } = useValues(customFlowEditorLogic)
    const { setWorkflowAction, setMode } = useActions(customFlowEditorLogic)
    const { logicProps } = useValues(workflowLogic)
    const { mappings, pendingPath, testLoading, testError, testResultData, shakePickButton } = useValues(
        customFlowOutputMappingLogic(logicProps)
    )
    const {
        setSelectedActionId,
        setMappings,
        ...
        cancelPendingPath,
        runOutputTest,
    } = useActions(customFlowOutputMappingLogic(logicProps))
                            ...
                            <div className="flex gap-2">
                                ...
                                <CategorySelect
                                    onChange={(categoryId) => {
                                        ...
                                        } as Extract<CustomFlowAction, { type: 'function_email' | 'function_sms' }>)
                                    }}
