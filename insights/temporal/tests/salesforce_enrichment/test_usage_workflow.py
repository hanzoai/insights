...
from insights.temporal.salesforce_enrichment.usage_workflow import (
    EnrichPageResult,
...
WORKFLOW_MODULE = "insights.temporal.salesforce_enrichment.usage_workflow"
...
class TestCacheOrgMappingsActivity(TestCase):
    ...
    async def test_force_rebuild_replaces_existing_cache(
        ...
        mock_sf = MagicMock()
        mock_sf.query_all.return_value = {"records": [{"Id": "001ABC", "Insights_Org_ID__c": "org-uuid-1"}]}
        mock_sf_client.return_value = mock_sf
...
class TestEnrichOrgPageActivity(TestCase):
    ...
    async def test_enriches_page_successfully(self, _mock_close, mock_get_page, mock_sf_client, _mock_heartbeat):
        mock_get_page.return_value = [
            {"salesforce_account_id": "001ABC", "insights_org_id": "uuid-1"},
            {"salesforce_account_id": "001DEF", "insights_org_id": "uuid-2"},
        ]
    ...
    async def test_handles_salesforce_partial_failure(
        ...
        mock_get_page.return_value = [
            {"salesforce_account_id": "001ABC", "insights_org_id": "uuid-1"},
            {"salesforce_account_id": "001DEF", "insights_org_id": "uuid-2"},
            {"salesforce_account_id": "001GHI", "insights_org_id": "uuid-3"},
        ]
