"""Pre-execution validation for user-submitted InsightsQL queries.

Applied by ``InsightsQLQueryRunner._calculate`` when ``is_query_service`` is true —
...
OFFSET_NOT_ALLOWED_MESSAGE = (
    ...
    "For bulk data extraction, use batch exports: https://hanzo.ai/docs/cdp/batch-exports"
)
...
# batch exports.
INSIGHTSQL_PERSONAL_API_KEY_OFFSET_ALLOWED_FLAG = "insightsql-personal-api-key-offset-allowed"
    ...
    try:
        return bool(
            hanzo_insights.feature_enabled(
                INSIGHTSQL_PERSONAL_API_KEY_OFFSET_ALLOWED_FLAG,
                org_id,
...
def validate_user_query(node: ast.SelectQuery | ast.SelectSetQuery, team: Team) -> None:
    """Enforce user-query restrictions on a parsed InsightsQL AST.
