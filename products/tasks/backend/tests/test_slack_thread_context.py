...
from insights.models import Integration, Organization, OrganizationMembership, Team, User
...
class TestSlackThreadContextEndpoint(_SlackThreadContextBase):
    ...
    def _create_fixture(
        self,
        *,
        slack_mention_workflow_id: str | None = "insights-code-mention-T_SLACK:Ev01",
        ...
        run = TaskRun.objects.create(
            ...
            state=run_state,
            output={"pr_url": "https://github.com/insights/insights/pull/1"},
        )
    ...
    def test_non_internal_team_is_forbidden(self):
        ...
        with patch("products.tasks.backend.presentation.views.api._is_internal_debug_team", return_value=False):
            response = self.client.get(self._url("https://insights.slack.com/archives/C0ACRAMJUAG/p1779956938619299"))
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_malformed_url_returns_400(self):
        response = self.client.get(self._url("https://insights.slack.com/not/a/permalink"))
    ...
    def test_happy_path_returns_task_and_runs(self):
        ...
        with patch(
            "insights.storage.object_storage.get_presigned_url",
        ...
        assert run_payload["pr_url"] == "https://github.com/insights/insights/pull/1"
        assert run_payload["task_processing_workflow_id"] == f"task-processing-{task.id}-{run.id}"
        assert run_payload["mention_workflow_id"] == "insights-code-mention-T_SLACK:Ev01"
    ...
    def test_reply_url_uses_thread_ts_query_param(self):
        self._create_fixture()
        reply_url = (
            "https://insights.slack.com/archives/C0ACRAMJUAG/p1779957091477899"
            "?thread_ts=1779956938.619299&cid=C0ACRAMJUAG"
        )
        with patch("insights.storage.object_storage.get_presigned_url", return_value=None):
    ...
    def test_multiple_runs_returned_oldest_first(self):
        ...
        second_run = TaskRun.objects.create(
            ...
            state={
                ...
                "slack_mention_workflow_id": "insights-code-mention-T_SLACK:Ev02",
            },
        )
        with patch("insights.storage.object_storage.get_presigned_url", return_value=None):
            response = self.client.get(self._url("https://insights.slack.com/archives/C0ACRAMJUAG/p1779956938619299"))
        ...
        assert body["runs"][1]["mention_workflow_id"] == "insights-code-mention-T_SLACK:Ev02"
    ...
    def test_temporal_url_includes_workflow_id_when_configured(self):
        ...
        assert run_payload["mention_workflow_url"] == (
            "https://temporal.example.com/namespaces/prod/workflows/insights-code-mention-T_SLACK:Ev01"
        )
        ...
        with override_settings(TEMPORAL_UI_HOST="https://temporal.example.com", TEMPORAL_NAMESPACE="prod"):
            with patch(
                ...
                response = self.client.get(
                    self._url("https://insights.slack.com/archives/C0ACRAMJUAG/p1779956938619299")
                )
