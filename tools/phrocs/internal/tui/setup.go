...
// Result of async insightscli dev:list-units call.
...
// Result of async insightscli dev:apply call.
...
func (m Model) enterSetupMode() Model {
	...
	// Pre-check intents from the current config's _insights section
	checked := make(map[string]bool)
	if m.configPath != "" {
		if phCfg, err := config.LoadInsightsConfig(m.configPath); err == nil && phCfg != nil {
...
// startListUnits kicks off an async insightscli dev:list-units call.
...
// startDevApply kicks off an async insightscli dev:apply call.
...
func runHogliListUnits(intents []string) ([]string, error) {
	...
	cmd := exec.Command(insightscliPath, args...)
...
func runHogliDevApply(intents []string, excludes []string) (string, error) {
	...
	output, err := cmd.Output()
	if err != nil {
		return "", fmt.Errorf("insightscli dev:apply: %s", strings.TrimSpace(stderr.String()))
	}
