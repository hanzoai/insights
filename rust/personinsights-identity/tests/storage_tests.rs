...
use personinsights_identity::storage::{IdentityStorage, PersonStub, StubOutcome};
...
impl TestContext {
    ...
    async fn person_count(&self) -> i64 {
        sqlx::query_scalar("SELECT count(*) FROM insights_person WHERE team_id = $1")
            .bind(self.team_id as i32)
            .fetch_one(&self.pool)
            .await
    ...
    async fn insert_tombstoned_distinct_id(&self, distinct_id: &str, person_id: i64, version: i64) {
        sqlx::query(
            r#"
            INSERT INTO insights_persondistinctid (distinct_id, person_id, team_id, version, is_deleted)
            VALUES ($1, $2, $3, $4, true)
            "#,
        )
        .bind(distinct_id)
    ...
    async fn person_state(&self, person_id: i64) -> (bool, Option<i64>) {
        sqlx::query_as(
            "SELECT is_deleted, version FROM insights_person WHERE team_id = $1 AND id = $2",
        )
        .bind(self.team_id as i32)
        .bind(person_id)
        .fetch_one(&self.pool)
    ...
    async fn distinct_id_state(&self, distinct_id: &str) -> Option<(i64, bool, Option<i64>)> {
        sqlx::query_as(
            r#"
            SELECT person_id, is_deleted, version FROM insights_persondistinctid
            WHERE team_id = $1 AND distinct_id = $2
            "#,
        )
        .bind(self.team_id as i32)
...
async fn extra_distinct_ids_always_get_version_one() {
    ...
    // always get version 1 and let Datastore emit an override.
