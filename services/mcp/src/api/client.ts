...
export interface ApiConfig {
    ...
    baseUrl: string
    /**
     * Public-facing base URL used when building links the user clicks (e.g. `_insightsUrl`).
     ...
     * (e.g. https://us.hanzo.ai) in tool responses.
     */
    ...
    /**
     * Sandbox-provisioned task id (from the inbound `x-insights-task-id` MCP header). Forwarded
     * to the Insights API as `X-Insights-Task-Id` on every call so writes can be attributed to
...
export class ApiClient {
        ...
        const defaultHeaders: HeadersInit = {
            ...
            ...(this.config.clientUserAgent
                ? {
                      ...
                      // Insights API can attach it to analytics events for MCP source attribution.
                      'x-insights-mcp-user-agent': this.config.clientUserAgent,
                  }
                : {}),
            // Forward MCP clientInfo fields from the initialize request so the
            // Insights API can attach them to analytics events.
            ...(this.config.mcpClientName ? { 'x-insights-mcp-client-name': this.config.mcpClientName } : {}),
            ...(this.config.mcpClientVersion ? { 'x-insights-mcp-client-version': this.config.mcpClientVersion } : {}),
            ...(this.config.mcpProtocolVersion
                ? { 'x-insights-mcp-protocol-version': this.config.mcpProtocolVersion }
                : {}),
            ...(this.config.mcpConsumer ? { 'x-insights-mcp-consumer': this.config.mcpConsumer } : {}),
            ...(this.config.oauthClientName ? { 'x-insights-mcp-oauth-client-name': this.config.oauthClientName } : {}),
            ...
            ...(this.config.mcpSessionId ? { 'x-insights-mcp-session-id': this.config.mcpSessionId } : {}),
            ...(this.config.mcpConversationId
                ? { 'x-insights-mcp-conversation-id': this.config.mcpConversationId }
                : {}),
            // Forward the sandbox task id so API writes are attributed to the agent's task.
            ...(this.config.taskId ? { 'X-Insights-Task-Id': this.config.taskId } : {}),
    ...
    async request<T = unknown>(opts: {
        ...
        if (!result.success) {
            ...
            // typed errors (e.g. InsightsPermissionError) that fetchJson throws.
            throw result.error
        }
    ...
    /**
     * Maps a non-OK Insights API response to the typed error that best describes
    ...
    private buildApiError(response: Response, errorText: string, url: string, method: string): Error {
        ...
        if (response.status === 429) {
            return new InsightsRateLimitError({
                body: errorText,
                url,
                method,
        ...
        if (response.status === 403 && errorData?.code === 'permission_denied') {
            ...
            // Warn, not error: InsightsPermissionError is thrown and handled by callers,
        ...
        if (errorData?.type === 'validation_error') {
            ...
            return new InsightsValidationError({
                detail,
            ...
            try {
                ...
                if (response.status === 429) {
                    ...
                    const rateLimitFailure = async (): Promise<Result<T>> => ({
                        success: false,
                        error: new InsightsRateLimitError({
    ...
    insights({ projectId }: { projectId: string }): Endpoint {
        return {
            ...
            validate: async ({
                ...
                query: string
                language: 'insightsQL' | 'insightsQLExpr' | 'script' | 'scriptTemplate'
                connectionId?: string
            }): Promise<
                Result<{
                    isValid: boolean
