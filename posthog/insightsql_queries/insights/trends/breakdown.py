...
from posthog.insightsql.constants import BREAKDOWN_VALUE_MAX_LENGTH, LimitContext
from posthog.insightsql.parser import parse_expr
from posthog.insightsql.property import apply_path_cleaning
...
from posthog.insightsql_queries.insights.trends.utils import get_properties_chain
...
def insightsql_to_string(expr: ast.Expr) -> ast.Call:
...
class Breakdown:
        ...
        if (
            ...
            return [
                ast.Alias(
                    ...
                    expr=insightsql_to_string(ast.Field(chain=["__in_cohort", "cohort_id"])),
                )
            ]
    ...
    def _get_breakdown_col_expr(
        ...
        if breakdown_type == "cohort":
            ...
            return ast.Alias(
                alias=alias,
                expr=insightsql_to_string(ast.Constant(value=cohort_breakdown)),
            )

        if breakdown_type == "insightsql" or breakdown_type == "event_metadata":
