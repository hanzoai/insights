...
import click
from insightscli.core.manifest import get_services_for_command
...
def _format_command_help(cmd_name: str, cmd_config: dict, underlying_cmd: str) -> str:
    ...
    """
    from insightscli.core.manifest import get_manifest
...
def _run_prechecks(prechecks: list[dict[str, Any]], yes: bool = False) -> bool:
    ...
    for check in prechecks:
        ...
        if check_type == "migrations":
            ...
            try:
                from insightscli.migrations import _compute_migration_diff, _get_cached_migration
                ...
                if diff.orphaned:
                    ...
                    click.echo("Run 'insightscli migrations:sync' to roll them back.\n")

                    if not yes:
                        if not click.confirm("Continue anyway?", default=False):
                            click.echo("Aborted. Run 'insightscli migrations:sync' first.")
                            return False
...
def execute_command_config(
    ...
    if isinstance(config, str):
        ...
        cmd_args = [bin_insightscli, config]
        if confirmed:
            cmd_args.append("--yes")
        ...
        if "insightscli" in config:
            ...
            if prompt:
                ...
                if should_run:
                    # Run the insightscli command
                    click.echo(f"✨ Executing: {insightscli_cmd}")
                    cmd_args = [bin_insightscli, insightscli_cmd]
                    if confirmed:
                        cmd_args.append("--yes")
                    _run(cmd_args, env=env)
                elif "else" in config:
...
class InsightscliCommand(Command):
