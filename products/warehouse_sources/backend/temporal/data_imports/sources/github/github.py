...
def validate_credentials(
    ...
    if repo.count("/") != 1 or not all(repo.split("/")):
        return (
            False,
            "Enter the repository as owner/repo (for example, insights/insights), not a full URL or just the owner name.",
        )
...
def github_source(
    ...
    def items() -> Iterator[Any] | AsyncIterator[Any]:
        if webhook_enabled:
            ...
            # The Script template lands the nested workflow_job / workflow_run object as the
...
def create_repo_webhook(
    ...
    if response.status_code in (200, 201):
        ...
        # extra_inputs — the framework persists it onto the script function's
