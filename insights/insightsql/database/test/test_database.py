...
import pytest
from insights.test.base import BaseTest, FuzzyInt, QueryMatchingTest, snapshot_postgres_queries
...
from insights.insightsql.database.database import (
...
from insights.insightsql.database.direct_redshift_table import DirectRedshiftTable
from insights.insightsql.database.direct_snowflake_table import DirectSnowflakeTable
from insights.insightsql.database.lazy_join_tags import FOREIGN_KEY
...
class TestBuildDatabaseRootNode(TestCase):
    ...
    @parameterized.expand([("with_insights_tables", True), ("without_insights_tables", False)])
    def test_build_database_root_node_matches_fresh_construction(self, _name: str, include_insights_tables: bool):
        cached = build_database_root_node(include_insights_tables=include_insights_tables)
        fresh = _construct_database_root_node(include_insights_tables=include_insights_tables)
    ...
    @parameterized.expand([("with_insights_tables", True), ("without_insights_tables", False)])
    def test_build_database_root_node_catalog_stays_picklable(self, _name: str, include_insights_tables: bool):
    ...
    def test_catalog_pickle_allowlist_covers_every_catalog_class(self):
        ...
        blob = pickle.dumps(_construct_database_root_node(include_insights_tables=True), pickle.HIGHEST_PROTOCOL)
        ...
        assert not unlisted, (
            f"The static InsightsQL catalog pickles classes from modules the restricted unpickler rejects: "
            f"{sorted(unlisted)}. Add each module to _CATALOG_PICKLE_MODULES in "
            f"insights/insightsql/database/database.py, or the catalog will fail to load at request time."
        )
    ...
    def test_catalog_unpickler_allowlists_catalog_classes_and_rejects_others(self):
        ...
        assert unpickler.find_class("insights.insightsql.database.models", "StringDatabaseField") is not None
        assert unpickler.find_class("insights.datastore.workload", "Workload") is not None
        for module, name in [
            ("os", "system"),
            ("builtins", "eval"),
            ("subprocess", "Popen"),
            ("insights.models", "Team"),
        ]:
...
class TestDatabase(BaseTest, QueryMatchingTest):
    ...
    def assertPrintedSqlMatchesSnapshot(self, printed: str) -> None:
        ...
        use_new_events_schema_snapshot = (
            settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA and "events_json" in normalized_sql.lower()
        )

        assert normalized_sql == self._schema_snapshot(use_new_events_schema_snapshot)

    def test_create_insightsql_database_team_id_and_team_must_be_the_same(self):
    ...
    def test_create_insightsql_database_must_have_either_team_id_or_team(self):
        with self.assertRaises(ValueError, msg="Either team_id or team must be provided"):
            Database.create_for()

    def test_create_insightsql_database_raises_query_error_for_missing_team(self):
    ...
    def test_serialize_database_warehouse_table_s3_with_hyphens(self):
        ...
        assert field.insightsql_value == "`id-hype`"
    ...
    def test_serialize_database_warehouse_table_source(self):
        ...
        assert field is not None
        assert field.name == "id"
        assert field.insightsql_value == "id"
        assert field.type == "string"
        assert field.schema_valid is True
    ...
    def test_create_insightsql_database_ignores_tables_of_deleted_sources(self):
        ...
        assert cast(InsightsQLDataWarehouseTable, database.get_table("pull_requests")).url == "s3://live/*"

    def test_create_insightsql_database_keeps_self_managed_table_without_source(self):
        ...
        assert cast(InsightsQLDataWarehouseTable, database.get_table("self_managed")).url == "s3://self/*"

    def test_create_insightsql_database_resolves_duplicate_live_table_names_to_newest(self):
        ...
        assert cast(InsightsQLDataWarehouseTable, database.get_table("pull_requests")).url == "s3://newer/*"
    ...
    def test_serialize_database_warehouse_table_source_query_count(self):
        ...
        with self.assertNumQueries(4):
            database.serialize(InsightsQLContext(team_id=self.team.pk, database=database))
    ...
    @patch("insights.insightsql.query.sync_execute", return_value=([], []))
    @pytest.mark.usefixtures("unittest_snapshot")
    ...
    def test_database_with_warehouse_tables_and_saved_queries_n_plus_1(self, patch_execute):
        ...
        with self.assertNumQueries(max_queries):
            modifiers = create_default_modifiers_for_team(
                self.team, modifiers=InsightsQLQueryModifiers(useMaterializedViews=True)
            )
    ...
    def test_build_from_sources_performs_no_io(self, patch_execute):
        ...
        # A dataWarehouseEventsModifier so the define_mappings path (get_datastore_column_type, the
    ...
    def test_build_from_sources_performs_no_io_for_direct_postgres(self, patch_execute):
        # Direct-query mode builds a DirectPostgresTable, whose insightsql_definition reads the source's
        ...
        # No direct_postgres_schema in options, so insightsql_definition falls back to job_inputs["schema"].
        DataWarehouseTable.objects.create(
            name="direct_table",
            format="Parquet",
            team=self.team,
            ...
            external_data_source_id=source.id,
            url_pattern="s3://test/*",
            columns={"id": {"datastore": "Int64", "insightsql": "integer"}},
        )
    ...
    def test_build_from_sources_resolves_direct_snowflake_case_insensitively(self, patch_execute):
        ...
        DataWarehouseTable.objects.create(
            ...
            columns={"N_NAME": {"datastore": "String", "insightsql": "string"}},
        )
    ...
    def test_dual_mode_database_excludes_row_filtered_schemas(self):
        ...
        serialized = database.serialize(InsightsQLContext(team_id=self.team.pk, database=database))
    ...
    def test_dual_mode_database_serializes_virtual_tables_from_schema_rows(self):
        ...
        # Only the virtual entries serialize in dual mode — no insights/system tables, no S3 rows.
    ...
    def test_dual_mode_database_applies_warehouse_access_control(self):
        ...
        sources = Database._fetch_sources(team=self.team, connection_id=str(source.id))
        sources.is_insightsql_warehouse_access_control_enabled = True
    ...
    def test_database_warehouse_joins_on_system_table_are_serialized(self):
        ...
        context = InsightsQLContext(team_id=self.team.pk, database=db)
    ...
    def test_selecting_from_persons_ignores_future_persons(self):
        ...
        query, _ = prepare_and_print_ast(parse_select(sql), context, dialect="datastore")
        assert (
            "equals(argMax(person.is_deleted, person.version), 0), less(argMax(toTimeZone(person.created_at, %(insightsql_val_0)s), person.version), plus(now64(6, %(insightsql_val_1)s), toIntervalDay(1))"
            in query
        ), query
    ...
    def test_selecting_persons_from_events_ignores_future_persons(self):
        ...
        context = InsightsQLContext(
            ...
            # disable PoE
            modifiers=create_default_modifiers_for_team(
                self.team, InsightsQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.DISABLED)
            ),
        )
        sql = "select person.id from events"
        query, _ = prepare_and_print_ast(parse_select(sql), context, dialect="datastore")
        assert (
            "less(argMax(toTimeZone(person.created_at, %(insightsql_val_0)s), person.version), plus(now64(6, %(insightsql_val_1)s), toIntervalDay(1)))"
            in query
        ), query
    ...
    def test_data_warehouse_events_modifiers_when_view_and_table_share_a_name(
        ...
        DataWarehouseSavedQuery.objects.create(
            team=self.team,
            name=shared_name,
            query={"query": "SELECT 1 AS ignored_value", "kind": "InsightsQLQuery"},
    ...
    def test_global_database_skips_loading_direct_query_tables(self):
        ...
        direct_table = DataWarehouseTable.objects.create(
            name="insights_activitylog",
            format="Parquet",
            team=self.team,
            credential=credentials,
            external_data_source=direct_source,
            url_pattern="direct://postgres",
            columns={"id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True}},
        )
        warehouse_table = DataWarehouseTable.objects.create(
            ...
            url_pattern="s3://test/*",
            columns={"id": {"insightsql": "string", "datastore": "String", "schema_valid": True}},
        )

        with patch(
            "insights.insightsql.database.database._preload_active_external_data_schemas",
    ...
    def test_adds_foreign_key_joins_for_direct_postgres_tables(self):
        ...
        team_table = DataWarehouseTable.objects.create(
            ...
            url_pattern="direct://postgres",
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "name": {"insightsql": "string", "datastore": "String", "schema_valid": True},
            },
        )
        activitylog_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "team_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
            },
        )

        ExternalDataSchema.objects.create(name="insights_team", team=self.team, source=source, table=team_table)
        ExternalDataSchema.objects.create(
            name="insights_activitylog",
            team=self.team,
            source=source,
        ...
        database = Database.create_for(team=self.team, connection_id=str(source.id))
        activitylog = database.get_table("insights_activitylog")
        team = database.get_table("insights_team")

        assert isinstance(activitylog.fields.get("team"), LazyJoin)
        assert isinstance(team.fields.get("insights_activitylogs"), LazyJoin)
    ...
    def test_direct_postgres_foreign_key_joins_ignore_deleted_schemas(self):
        ...
        activitylog_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "actor_key": {"insightsql": "string", "datastore": "String", "schema_valid": True},
            },
        )
    ...
    def test_direct_postgres_foreign_key_joins_do_not_leak_to_insights_tables(self):
        credentials = DataWarehouseCredential.objects.create(
            access_key="test_key", access_secret="test_secret", team=self.team
        )
        source = ExternalDataSource.objects.create(
        ...
        activitylog_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "person_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
            },
        )
        ...
        assert not database.has_table("postgres.ph3.insights_activitylog")
        assert persons.fields.get("insights_activitylogs") is None
    ...
    def test_direct_postgres_foreign_key_uses_table_names_only(self):
        ...
        customer_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "email": {"insightsql": "string", "datastore": "String", "schema_valid": True},
            },
        )
        order_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "customer_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
            },
        )
    ...
    def test_direct_postgres_inferred_foreign_key_uses_matching_column_and_skips_self_reference(self):
        ...
        invoice_table = DataWarehouseTable.objects.create(
            ...
            url_pattern="direct://postgres",
            columns={
                "invoice_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "customer_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "invoice_date": {"insightsql": "date", "datastore": "Date", "schema_valid": True},
            },
        )
        payment_table = DataWarehouseTable.objects.create(
            ...
            url_pattern="direct://postgres",
            columns={
                "payment_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "invoice_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "paid_at": {"insightsql": "datetime", "datastore": "DateTime", "schema_valid": True},
            },
        )
    ...
    def test_direct_postgres_foreign_key_join_allows_user_traversal(self):
        ...
        team_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "user_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
            },
        )

        ExternalDataSchema.objects.create(name="insights_user", team=self.team, source=source, table=user_table)
        ExternalDataSchema.objects.create(
            name="insights_team",
            team=self.team,
            source=source,
        ...
        prepare_and_print_ast(parse_select("SELECT t.user.email FROM insights_team t"), context, dialect="postgres")
    ...
    def test_adds_foreign_key_joins_for_non_direct_postgres_tables(self):
        ...
        database = Database.create_for(team=self.team)
        activitylog = database.get_table("postgres.ph3.insights_activitylog")
        team = database.get_table("postgres.ph3.insights_team")
    ...
    def test_serialize_direct_postgres_skips_foreign_key_join_when_target_table_is_missing(self):
        ...
        only_table = DataWarehouseTable.objects.create(
            ...
            columns={
                "id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
                "session_id": {"insightsql": "integer", "datastore": "Int64", "schema_valid": True},
            },
        )
    ...
    def test_direct_postgres_foreign_keys_ignore_invalid_metadata(self):
        ...
        ExternalDataSchema.objects.create(
            ...
            sync_type_config={
                "schema_metadata": {
                    "foreign_keys": [
                        1,
                        {"column": 2, "target_table": "insights_user", "target_column": "id"},
                        {"column": "person(", "target_table": "insights_user", "target_column": "id"},
                    ]
                }
            },
        )
        ...
        assert "insights_activitylog" in serialized
        assert database.get_table("insights_activitylog").fields.get("person") is None
    ...
    def test_serialize_direct_postgres_reserved_table_names_override_insights_tables(self) -> None:
        credentials = DataWarehouseCredential.objects.create(
            access_key="test_key", access_secret="test_secret", team=self.team
        )
        source = ExternalDataSource.objects.create(
        ...
        DataWarehouseTable.objects.create(
            ...
            external_data_source=source,
            url_pattern="direct://postgres",
            columns={"email": {"insightsql": "StringDatabaseField", "datastore": "Nullable(String)", "schema_valid": True}},
        )
    ...
    def test_direct_postgres_reserved_table_names_do_not_hide_insights_tables_without_connection(self) -> None:
        credentials = DataWarehouseCredential.objects.create(
            access_key="test_key", access_secret="test_secret", team=self.team
        )
        source = ExternalDataSource.objects.create(
        ...
        assert database.has_table("events")
        assert database.has_table("persons")
        assert isinstance(serialized["events"], DatabaseSchemaInsightsTable)
        assert isinstance(serialized["persons"], DatabaseSchemaInsightsTable)
    ...
    def test_direct_postgres_direct_mode_includes_tables_materialized_from_views(self) -> None:
        ...
        DataWarehouseSavedQuery.objects.create(
            ...
            query={"kind": "InsightsQLQuery", "query": "SELECT event FROM events"},
        )
        DataWarehouseTable.objects.create(
            name="materialized_table",
            format="Parquet",
    ...
    def test_deleted_direct_postgres_schema_does_not_reenable_table(self) -> None:
        ...
        assert not database.has_table("insights_dashboard")
    ...
    def test_insights_qualified_table_names_are_resolvable(self):
        database = Database.create_for(team=self.team)

        for table_name in ROOT_TABLES__DO_NOT_ADD_ANY_MORE.keys():
            qualified_name = f"insights.{table_name}"
    ...
    def test_insights_qualified_table_names_resolve_in_select(self):
        database = Database.create_for(team=self.team)
        context = InsightsQLContext(
            team_id=self.team.pk,
            enable_select_queries=True,
        ...
        prepare_and_print_ast(parse_select("select * from insights.events"), context, dialect="datastore")
    ...
    def test_database_serialization_handles_invalid_sources_gracefully(self):
        ...
        # Create an invalid source with characters that break InsightsQL identifier rules
        ...
        database = Database.create_for(team=self.team)
        context = InsightsQLContext(team_id=self.team.pk, database=database)
    ...
    def test_warehouse_join_on_persons_with_partial_columns_mid_sync(self):
        ...
        DataWarehouseTable.objects.create(
            ...
            name="farm_size_table",
            columns={
                "user_email": {"datastore": "String", "insightsql": "StringDatabaseField"},
            },
            credential=credential,
            url_pattern="https://bucket.s3/data/*",
        )
    ...
    def test_create_for_with_synthetic_user_skips_user_rbac(self):
        from insights.auth import ProjectSecretAPIKeyUser
        ...
        with patch(
            "insights.insightsql.database.database._compute_system_table_access_decision", side_effect=spy
        ) as decision:
