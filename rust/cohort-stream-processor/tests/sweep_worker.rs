...
use cohort_stream_processor::stage1::{
    datastore_timestamp_to_millis, Stage1State, StateVariant, StatefulRecord,
};
...
/// lower bound is the space-separated Datastore shape; the upper bound is the real cohort-date-picker
...
async fn single_does_not_evict_before_its_calendar_midnight_and_does_after() {
    ...
    let kolkata_day = day_idx_in_tz(datastore_timestamp_to_millis(early).unwrap(), Kolkata);
...
async fn sweep_dormant_person_left_is_emitted() {
    ...
    let t0_ms = datastore_timestamp_to_millis(t0).unwrap();
...
fn explicit_range_lower_bound_is_the_literal_calendar_day_under_a_negative_offset_team() {
    ...
    //
    // Timestamps are passed as their UTC equivalents (the Datastore form is read as UTC); the comment on
