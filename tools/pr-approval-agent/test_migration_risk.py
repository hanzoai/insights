...
def test_success_check_pairs_each_migration_with_its_max_migration_txt() -> None:
    ...
    files = [
        "insights/migrations/1125_x.py",
        "products/signals/backend/migrations/0042_y.py",
        "insights/api/some.py",
    ]
    check = _check(
        analyzed_paths=[
            "insights/migrations/1125_x.py",
...
def test_bypass_scoped_to_intersection_of_marker_and_pr() -> None:
    ...
    (e.g. Datastore migration in the same PR) must not be bypassed even when
    the Django side concluded success."""
    files = [
        "insights/migrations/1125_x.py",
        "insights/datastore/migrations/0099_unrelated.py",
    ]
    # Marker only lists the Django one — the analyzer doesn't touch Datastore.
    check = _check(analyzed_paths=["insights/migrations/1125_x.py"])
...
def test_max_migration_txt_paired_only_for_analyzed_dirs() -> None:
    """Sibling pairing must follow the analyzer's scope — a Datastore
    ...
    assert "insights/datastore/migrations/max_migration.txt" not in safe
    assert "insights/migrations/max_migration.txt" in safe
...
def test_empty_marker_means_no_bypass() -> None:
    ...
    files = ["insights/datastore/migrations/0099_x.py"]
...
def test_malformed_or_missing_marker_falls_back_to_no_bypass(summary: str) -> None:
    ...
    assert safe_migration_files([check], ["insights/migrations/1125_x.py"]) == set()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            [_check(conclusion="failure", analyzed_paths=["insights/migrations/1125_x.py"])], id="completed-failure"
        ),
        pytest.param(
            [_check(conclusion="neutral", analyzed_paths=["insights/migrations/1125_x.py"])], id="completed-needs-review"
        ),
...
def test_no_bypass_unless_check_completed_with_success(check_runs: list[dict]) -> None:
    ...
    assert safe_migration_files(check_runs, ["insights/migrations/1125_x.py"]) == set()
...
def test_latest_completed_run_wins_over_older_and_in_flight() -> None:
    ...
    older_failure = _check(
        ...
        analyzed_paths=["insights/migrations/1125_x.py"],
    )
    ...
    assert safe_migration_files([older_failure, newer_success, in_flight], ["insights/migrations/1125_x.py"]) == {
        "insights/migrations/1125_x.py",
        "insights/migrations/max_migration.txt",
    }
...
def test_pending_when_pr_has_migrations_and_no_completed_check(check_runs: list[dict]) -> None:
    ...
    assert migration_check_pending(check_runs, ["insights/migrations/1125_x.py"]) is True
...
def test_not_pending_once_check_has_completed(conclusion: str) -> None:
    ...
    assert (
        migration_check_pending(
            [_check(conclusion=conclusion, analyzed_paths=["insights/migrations/1125_x.py"])],
            ["insights/migrations/1125_x.py"],
        )
        is False
    )
...
def test_not_pending_when_pr_has_no_real_migrations() -> None:
    ...
    assert migration_check_pending([_check(status="in_progress", conclusion=None)], ["insights/api/views.py"]) is False
