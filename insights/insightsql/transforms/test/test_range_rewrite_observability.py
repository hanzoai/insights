...
These run the real Datastore property-resolution transform (not the record_* hooks in isolation) so they pin the
...
from insights.test.base import APIBaseTest, DatastoreTestMixin, cleanup_materialized_columns
...
class TestMaterializedRangeRewriteObservability(DatastoreTestMixin, APIBaseTest):
    def setUp(self) -> None:
        super().setUp()
    ...
    def _range_rewrite_counts(
        ...
        *,
        ...
        stats = InsightsQLTypeObservability(dialect="datastore", source="probe")
        with patch("insights.insightsql.printer.utils.create_insightsql_type_observability", return_value=stats):
            prepare_and_print_ast(parse_select(sql), context, "datastore")
    ...
    def test_skipped_outcome_flows_through_to_prometheus(self):
        labels = {"engine": "current", "dialect": "datastore", "source": "probe", "result": "skipped"}
        before = REGISTRY.get_sample_value("insightsql_materialized_range_rewrite_total", labels) or 0.0
        ...
        after = REGISTRY.get_sample_value("insightsql_materialized_range_rewrite_total", labels) or 0.0
