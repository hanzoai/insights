...
class TestToolbarOAuthCallbackExchange(ToolbarOAuthAuthorizeMixin, APIBaseTest):
    ...
    def test_callback_redirects_with_code_in_redirect_flow(self):
        ...
        assert "__insights_toolbar=code:auth_code_123" in redirect_url
    ...
    def test_callback_preserves_original_url_fragment(self):
        ...
        assert response.status_code == 302
        redirect_url = response["Location"]
        assert "#section1&__insights_toolbar=code:auth_code_123" in redirect_url

    def test_callback_strips_insights_hash_from_redirect(self):
        insights_hash = "%7B%22action%22%3A%22ph_authorize%22%2C%22token%22%3A%22phc_test%22%7D"
        state = self._authorize_and_get_state(redirect_url=f"https://example.com/page#__insights={insights_hash}")
        response = self.client.get(f"/toolbar_oauth/callback?code=auth_code_123&state={state}")

        assert response.status_code == 302
        redirect_url = response["Location"]
        assert "__insights=" not in redirect_url.split("__insights_toolbar")[0]
        assert "__insights_toolbar=code:auth_code_123" in redirect_url

    def test_callback_strips_insights_toolbar_hash_from_redirect(self):
        state = self._authorize_and_get_state(
            redirect_url="https://example.com/page#__insights_toolbar=code:old,client_id:old"
        )
        response = self.client.get(f"/toolbar_oauth/callback?code=auth_code_123&state={state}")
    ...
    def test_callback_user_without_team_returns_400(self):
        ...
        state = self._authorize_and_get_state()
        with patch("insights.models.user.User.team", new_callable=property, fget=lambda self: None):
            response = self.client.get(f"/toolbar_oauth/callback?code=abc&state={state}")
        assert response.status_code == 400
    ...
    def test_callback_app_url_root_domain(self):
        ...
        response = self.client.get(f"/toolbar_oauth/callback?code=abc&state={state}")
        assert response.status_code == 302
        assert response["Location"].startswith("https://example.com#__insights_toolbar=")

    def test_callback_strips_both_insights_and_toolbar_hash(self):
        insights_hash = "%7B%22action%22%3A%22ph_authorize%22%7D"
        redirect_url = f"https://example.com/page#__insights={insights_hash}&__insights_toolbar=code:old,client_id:old"
        state = self._authorize_and_get_state(redirect_url=redirect_url)
        response = self.client.get(f"/toolbar_oauth/callback?code=auth_code_123&state={state}")

        assert response.status_code == 302
        location = response["Location"]
        assert "__insights=" not in location.split("__insights_toolbar")[0]
        assert "code:old" not in location
        assert "__insights_toolbar=code:auth_code_123" in location
    ...
    def test_callback_preserves_spa_hash_route(self):
        ...
        assert response.status_code == 302
        location = response["Location"]
        assert "/dashboard/settings?__insights_toolbar=code:abc" in location
        assert "/dashboard/settings&__insights_toolbar" not in location

    def test_callback_preserves_fragment_params_after_stripping_toolbar(self):
        state = self._authorize_and_get_state(
            redirect_url="https://example.com/page#section1&__insights_toolbar=code:old,client_id:old&other=value"
        )
        ...
        assert "__insights_toolbar=code:new_code" in location
...
class TestNormalizeAndValidateAppUrl(APIBaseTest):
    ...
    @parameterized.expand(
        [
            (
                "strips __insights param",
                "https://example.com/page#__insights=%7B%7D",
                "https://example.com/page",
            ),
            (
                "strips __insights_toolbar param",
                "https://example.com/page#__insights_toolbar=code:old,client_id:old",
                "https://example.com/page",
            ),
            (
                "strips both params",
                "https://example.com/page#__insights=%7B%7D&__insights_toolbar=code:old,client_id:old",
                "https://example.com/page",
            ),
            (
                "strips both params reversed order",
                "https://example.com/page#__insights_toolbar=code:old,client_id:old&__insights=%7B%7D",
                "https://example.com/page",
            ),
            (
                "preserves SPA hash route",
                "https://example.com/#/login&__insights_toolbar=code:old,client_id:old",
                "https://example.com/#/login",
            ),
            (
                "preserves other fragment params",
                "https://example.com/page#section1&__insights_toolbar=code:old,client_id:old&other=value",
                "https://example.com/page#section1&other=value",
            ),
            (
                "preserves fragment with only non-insights params",
                "https://example.com/page#section1&__insights=%7B%7D&tab=2",
