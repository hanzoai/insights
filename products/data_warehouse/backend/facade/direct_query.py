...
Re-exports the virtual-table builder the InsightsQL database builder calls for dual-mode
(synced, direct-query-enabled) connections. Its own facade submodule — separate from
``facade.insightsql``, which ``database.py`` imports at module level — so the engine helper
