            ...
            [] as CustomerProfileConfigType[],
            {
                loadConfigs: async () => {
                    try {
                        ...
                        toast.error('Failed to load customer profile configs')
                        throw error
                    }
                },
                createConfig: async ({ config }) => {
                    try {
                        ...
                        toast.success('Customer profile config created successfully')
                        return [...values.configs, newConfig]
                    } catch (error) {
                        toast.error('Failed to create customer profile config')
                        throw error
                    }
                },
                updateConfig: async ({ id, config }) => {
                    try {
                        ...
                        toast.success('Customer profile config updated successfully')
                        return values.configs.map((c) => (c.id === id ? updatedConfig : c))
                    } catch (error) {
                        toast.error('Failed to update customer profile config')
                        throw error
                    }
                },
                deleteConfig: async ({ id }) => {
                    try {
                        ...
                        toast.success('Customer profile config deleted successfully')
                        return values.configs.filter((c) => c.id !== id)
                    } catch (error) {
                        toast.error('Failed to delete customer profile config')
                        throw error
                    }
                },
            },
