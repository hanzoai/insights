...
class TestLiveDebuggerBreakpointModel(DatastoreTestMixin, BaseTest):
            ...
            [
                {
                    ...
                    "distinct_id": "user123",
                    "timestamp": datetime.now(),  # nosemgrep: test-datetime-now-without-freeze (must match Datastore server-side now())
                    "properties": {
    ...
    def test_get_breakpoint_hits_time_filter(self):
        ...
        # nosemgrep: test-datetime-now-without-freeze (must match Datastore server-side now())
        now = datetime.now()
