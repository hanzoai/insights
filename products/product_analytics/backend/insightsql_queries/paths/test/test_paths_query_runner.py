...
from insights.schema import CachedPathsQueryResponse
...
class TestPaths(DatastoreTestMixin, APIBaseTest):
    ...
    def test_custom_insightsql_paths(self):
        _create_person(team_id=self.team.pk, distinct_ids=["person_1"])
        _create_person(team_id=self.team.pk, distinct_ids=["person_2"])
        _create_person(team_id=self.team.pk, distinct_ids=["person_3"])
        _create_person(team_id=self.team.pk, distinct_ids=["person_4"])
        ...
        r = PathsQueryRunner(
            query={
                "kind": "PathsQuery",
                "pathsFilter": {
                    "includeEventTypes": ["insightsql"],
                    "pathsInsightsQLExpression": "event || properties.a",
                },
            },
            team=self.team,
        ).run()
    ...
    def test_paths_start_and_end_trailing_slashes(self, _name: str, start_point: str, end_point: str) -> None:
        # `construct_event_insightsql` strips trailing slashes from `$current_url`
