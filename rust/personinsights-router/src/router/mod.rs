...
use crate::backend::PersonInsightsBackend;
use routing::{get_consistency, route_request};

/// Macro to call a backend method with timing instrumentation.
///
/// Records metrics:
/// - `personinsights_router_backend_requests_total` - counter by method and backend
/// - `personinsights_router_backend_duration_ms` - histogram by method and backend
/// - `personinsights_router_backend_errors_total` - counter on errors
macro_rules! call_backend {
    ($self:expr, $decision:expr, $method_name:expr, $method:ident, $request:expr) => {{
        ...
        counter!(
            "personinsights_router_backend_requests_total",
            "method" => $method_name,
        ...
        histogram!(
            "personinsights_router_backend_duration_ms",
            "method" => $method_name,
            "backend" => backend_name
        )
        .record(duration_ms);
        ...
        if result.is_err() {
            counter!(
                "personinsights_router_backend_errors_total",
                "method" => $method_name,
                "backend" => backend_name
            )
            .increment(1);
...
/// PersonInsightsRouter coordinates routing requests to the appropriate backend.
///
/// # Routing Logic
///
/// | Data Type       | Operation | Consistency | Target Backend           |
/// |-----------------|-----------|-------------|--------------------------|
/// | Person data     | Read      | EVENTUAL    | personinsights-replica        |
/// | Person data     | Read      | STRONG      | personinsights-leader (Phase 2) |
/// | Person data     | Write     | -           | personinsights-leader (Phase 2) |
/// | Non-person data | Read      | EVENTUAL    | personinsights-replica        |
/// | Non-person data | Read      | STRONG      | personinsights-replica (primary) |
/// | Non-person data | Write     | -           | personinsights-replica        |
///
...
/// - `personinsights_router_backend_requests_total` - counter by method and backend
/// - `personinsights_router_backend_duration_ms` - histogram by method and backend
/// - `personinsights_router_backend_errors_total` - counter by method and backend
pub struct PersonInsightsRouter {
    replica_backend: Arc<dyn PersonInsightsBackend>,
    // Phase 2: Add leader_backend for person data writes and strong consistency reads
    // leader_backend: Option<Arc<dyn PersonInsightsBackend>>,
}

impl PersonInsightsRouter {
    pub fn new(replica_backend: Arc<dyn PersonInsightsBackend>) -> Self {
    ...
    fn get_backend(&self, decision: RouteDecision) -> Result<&dyn PersonInsightsBackend, Status> {
        match decision {
            RouteDecision::Replica => Ok(self.replica_backend.as_ref()),
            RouteDecision::Leader => {
                // Phase 2: Return leader backend when available
                Err(Status::unimplemented(
                    "Strong consistency for person data requires personinsights-leader (not yet implemented)",
                ))
            }
        }
    }
