...
def _create_insights_function_template(
...
class TestGetOrCreateWebhookInsightsFunction:
    def test_returns_error_when_no_webhook_template(self):
        _, team = _create_org_and_team()
        source = MagicMock()
        source.webhook_template = None

        result = get_or_create_webhook_insights_function(team, source, "source-123", [])

        assert result.insights_function is None
    ...
    def test_creates_insights_function_with_schema_mapping(self):
        _, team = _create_org_and_team()
        ...
        result = get_or_create_webhook_insights_function(team, source, "source-123", schemas)

        assert result.insights_function_created is True
        assert result.error is None
        assert result.insights_function is not None
        assert result.insights_function.inputs is not None

        mapping = result.insights_function.inputs["schema_mapping"]["value"]
        assert mapping["customer"] == str(schemas[0].id)
        assert mapping["invoice"] == str(schemas[1].id)
        assert result.insights_function.inputs["source_id"]["value"] == "source-123"
    ...
    def test_includes_extra_inputs(self):
        ...
        result = get_or_create_webhook_insights_function(
            team, source, "source-123", schemas, extra_inputs={"signing_secret": "sec_123"}
        )

        assert result.insights_function is not None
        assert result.insights_function.inputs is not None
        assert result.insights_function.inputs["signing_secret"]["value"] == "sec_123"

    def test_updates_existing_insights_function(self):
        _, team = _create_org_and_team()
        _create_insights_function_template()
        source = _make_webhook_source()
        ext_source = _create_external_data_source(team)
        ...
        first_result = get_or_create_webhook_insights_function(team, source, "source-123", schemas)
    ...
    def test_merges_schema_mapping_on_update(self):
        ...
        get_or_create_webhook_insights_function(team, source, "source-123", customers_schema)

        invoices_schema = _create_schemas(team, ext_source, ["Invoices"])
        result = get_or_create_webhook_insights_function(team, source, "source-123", invoices_schema)

        assert result.insights_function is not None
        assert result.insights_function.inputs is not None

        result.insights_function.refresh_from_db()
        mapping = result.insights_function.inputs["schema_mapping"]["value"]
        assert "customer" in mapping
    ...
    def test_webhook_url_uses_cloud_deployment(self, cloud_deployment: str, expected_host: str):
        ...
        with patch("products.data_warehouse.backend.logic.external_data_source.webhooks.settings") as mock_settings:
            mock_settings.CLOUD_DEPLOYMENT = cloud_deployment
            result = get_or_create_webhook_insights_function(team, source, "source-123", schemas)

        assert result.webhook_url.startswith(expected_host)
        assert result.insights_function is not None
        assert str(result.insights_function.id) in result.webhook_url
    ...
    def test_insights_function_fields_match_template(self):
        _, team = _create_org_and_team()
        db_template = _create_insights_function_template(name="My Template")
        source = _make_webhook_source()
        ext_source = _create_external_data_source(team)
        schemas = _create_schemas(team, ext_source, ["Customers"])
        ...
        script = result.insights_function
        assert script is not None
        assert script.name == db_template.name
        assert script.script == db_template.code
        assert script.template_id == db_template.template_id
        assert script.type == "warehouse_source_webhook"
