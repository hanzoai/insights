...
_VALIDATE_CONNECTION_HINTS: list[tuple[str, str]] = [
    (
        ...
        "Host could not be resolved. Check the host is spelled correctly and reachable from Insights.",
    ),
    (
    ...
    (
        "No route to host",
        "Could not reach the host. Check the host is correct and that Insights's IP addresses are allowed through your firewall.",
    ),
    (
...
class MySQLSource(SQLSource[MySQLSourceConfig], SSHTunnelMixin, ValidateDatabaseHostMixin):
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            # wrong, the SSH key/credentials are rejected, or a firewall blocks Insights's IPs. The
            ...
            _SSH_HANDSHAKE_EOF_ERROR: "Could not connect to your SSH tunnel — the gateway accepted the connection but closed it during the SSH handshake. Check that the SSH host and port point to an SSH server (not the database port), that the bastion is running and reachable, and that Insights's IP addresses are allowed through its firewall, then re-enable the sync.",
            ...
            "SSH tunnel host not allowed": (
                "Insights rejected the SSH tunnel host for this source because it either couldn't "
            ...
            "is blocked because of many connection errors": "Your MySQL/MariaDB server has blocked Insights's host after too many interrupted connections (error 1129). Ask your database admin to run 'FLUSH HOSTS' (or 'mysqladmin flush-hosts') and consider raising 'max_connect_errors', then retry the sync.",
            ...
            # Insights's connecting host, so the handshake is rejected before any credentials are
            ...
            "is not allowed to connect to this MySQL server": "Your MySQL/MariaDB server isn't allowing connections from Insights's host (error 1130). Ask your database admin to grant access for the connecting host (or allow our IP / SSH-tunnel host), then retry the sync.",
            ...
            "(1142,": "Insights's database user doesn't have SELECT permission on a table this sync reads (MySQL error 1142). Ask your database admin to grant SELECT on it, or remove that table from the sync, then resync.",
