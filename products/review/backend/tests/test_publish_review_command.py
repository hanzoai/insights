...
from products.review.backend.reviewer.tools.publish_review import PublishOutcome

_PUBLISH = "products.review.backend.management.commands.publish_review.publish_persisted_review"
_INTEGRATION = (
    "products.review.backend.management.commands.publish_review.GitHubIntegration.first_for_team_repository"
)
_STALE = "products.review.backend.management.commands.publish_review._stale_head_warning"
_URL = "https://github.com/Insights/insights/pull/7"
...
class TestPublishReviewCommand(BaseTest):
    def _report(self, *, run_count: int, head_sha: str = "sha7", markdown: str = "# body") -> str:
        report_id = upsert_review_report(
            team_id=self.team.id, repository="Insights/insights", pr_url=_URL, pr_metadata=_meta()
        )
