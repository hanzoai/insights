...
use personinsights_leader::cache::{CacheLookup, DirtyIndex, PartitionedCache, PersonCacheKey};
use personinsights_leader::warming::{warm_from_kafka, WarmClientPools, WarmingConfig};
...
async fn warming_starts_from_writer_committed_offset() {
    ...
    // and must be warmed.
    commit_writer_offset_at(&cluster, "personinsights-writer", 0, 5);
