...
describe('createReadOnlyProcessGroupsStep', () => {
    ...
    it.each([
        ...
        const result = await step(
            createInput({
                processPerson,
                preparedEvent: createTestPreIngestionEvent({
                    properties: hasGroups ? { $groups: { org: 'insights' } } : {},
                }),
            })
        )
    ...
    it('enriches properties with $group_N using read-only fetch', async () => {
        ...
        const input = createInput({
            preparedEvent: createTestPreIngestionEvent({
                properties: { $groups: { org: 'insights', project: 'insights-js' } },
            }),
        })
        ...
        if (result.type === PipelineResultType.OK) {
            expect(result.value.preparedEvent.properties).toEqual({
                $groups: { org: 'insights', project: 'insights-js' },
                $group_0: 'insights',
                $group_1: 'insights-js',
            })
        }
    ...
    it('skips unknown group types without writing', async () => {
        ...
        const input = createInput({
            preparedEvent: createTestPreIngestionEvent({
                properties: { $groups: { org: 'insights', unknown_type: 'value' } },
            }),
        })
        const result = await step(input)
        ...
        if (result.type === PipelineResultType.OK) {
            expect(result.value.preparedEvent.properties).toEqual({
                $groups: { org: 'insights', unknown_type: 'value' },
                $group_0: 'insights',
            })
        }
    })
