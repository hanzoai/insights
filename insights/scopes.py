...
# `insightscli build:openapi` to regenerate after editing this file.
...
# Server-only provenance marker for OAuth tokens minted for Insights's built-in
...
INTERNAL_API_SCOPE_OBJECTS: frozenset[APIScopeObject] = frozenset(
    {
        ...
        # it on the internal products that share the Insights Desktop OAuth app so a user's
...
def downgrade_scopes_to_read_only(scope_str: str) -> str:
    ...
    - `*` is the full-access wildcard (see `insights/permissions.py` — `if "*" in key_scopes`
...
# oauth_toolkit) keeps `insights.scopes` importable without Django setup, which
...
# `OAuthValidator._ALWAYS_ALLOWED_SCOPES` in `insights/api/oauth/views.py`.
...
def filter_to_unprivileged_scopes(scopes: Iterable[object]) -> list[str]:
    ...
    (the RFC 7591 `scope` string, split before it gets here) and CIMD (`com.insights.scopes`).
...
def scopes_within_ceiling(
    ...
    *,
    ...
    Insights Desktop CLI) until wildcard retirement; provisioning leaves it `False`
...
def get_oauth_scopes_supported() -> list[str]:
    ...
    `insights/temporal/oauth.py:create_oauth_access_token_for_user`) and never passes
