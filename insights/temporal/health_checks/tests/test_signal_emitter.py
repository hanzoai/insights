...
from insights.temporal.health_checks.processing import _process_batch_detection
from insights.temporal.health_checks.signal_emitter import emit_health_check_signals
...
class TestEmitHealthCheckSignals(SimpleTestCase):
    def test_calls_emit_signal_with_weight_and_extra(self):
        ...
        with (
            patch("insights.temporal.health_checks.signal_emitter._check_class_for_kind", return_value=_SignalCheck),
            patch("products.signals.backend.facade.api.emit_signal", new=emit_mock),
            patch("insights.temporal.health_checks.signal_emitter.Team") as team_model,
        ):
            issue = _make_issue()
    ...
    def test_check_without_override_emits_nothing(self):
        emit_mock = AsyncMock()
        with (
            patch("insights.temporal.health_checks.signal_emitter._check_class_for_kind", return_value=_NoSignalCheck),
            patch("products.signals.backend.facade.api.emit_signal", new=emit_mock),
        ):
    ...
    def test_unregistered_kind_emits_nothing(self):
        emit_mock = AsyncMock()
        with (
            patch("insights.temporal.health_checks.signal_emitter._check_class_for_kind", return_value=None),
            patch("products.signals.backend.facade.api.emit_signal", new=emit_mock),
        ):
    ...
    def test_emit_signal_failure_is_swallowed_and_captured(self):
        ...
        with (
            ...
            patch("products.signals.backend.facade.api.emit_signal", new=emit_mock),
            patch("insights.temporal.health_checks.signal_emitter.Team") as team_model,
            patch("insights.temporal.health_checks.signal_emitter.capture_exception") as capture,
        ):
            issue = _make_issue()
...
class TestSeamEmitsSignalsOnFiringOnly(SimpleTestCase):
    ...
    def test_signal_emitted_for_newly_active_not_resolved(self, _name: str, dry_run: bool):
        ...
        with (
            patch("insights.temporal.health_checks.processing.upsert_issues_with_deltas", return_value=[firing]),
            patch(
                "insights.temporal.health_checks.processing.resolve_stale_issues_with_deltas", return_value=[resolved]
            ),
            patch("insights.temporal.health_checks.processing.emit_health_check_alert"),
            patch("insights.temporal.health_checks.processing.emit_health_check_signals") as emit_signals,
        ):
