...
(`DEFAULT_ROLLOUT_TEAM_IDS`) on Insights Cloud, and is fully overridable via the
...
from insights.dags.common import JobOwners, chunk_ranges
from insights.models import Team

from products.web_analytics.backend.insightsql_queries.web_dimensional_precompute import (
...
# Built-in rollout audience used when the env var is unset: Insights's internal
# dogfood project plus one high-volume pilot team, for the v2 side-by-side. Applied
# on Insights Cloud only (see get_selected_team_ids).
...
def get_selected_team_ids() -> list[int]:
    ...
    Insights Cloud; self-hosted defaults to none so the job never runs for unrelated
