...
from insights.temporal.ai.slack_app.types import InsightsCodeSlackMentionWorkflowInputs, coerce_mention_workflow_inputs


def test_coerce_returns_dataclass_unchanged():
    inputs = InsightsCodeSlackMentionWorkflowInputs(event={"ts": "1.2"}, integration_id=7, slack_team_id="T1")
...
def test_coerce_rebuilds_dataclass_from_dict():
    ...
    assert isinstance(coerced, InsightsCodeSlackMentionWorkflowInputs)
...
def test_coerce_raises_with_context_when_required_fields_missing(payload):
    with pytest.raises(TypeError, match="InsightsCodeSlackMentionWorkflowInputs"):
