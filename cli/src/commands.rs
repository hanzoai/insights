...
pub struct Cli {
    ...
    /// Skip SSL certificate verification when talking to the Insights API. Use only with
...
pub enum Commands {
    ...
    #[command(about = "Upload Apple dSYM debug symbol files to Insights")]
    Dsym {
        #[command(subcommand)]
        cmd: DsymSubcommand,
    },
    ...
    #[command(
        about = "Agent-first Insights API tools",
        long_about = "Agent-first Insights API tools.\n\n\
            Exposes Insights's MCP tool catalog through a shell-friendly interface so coding \
            agents (and the humans driving them) can discover, inspect, and call Insights API \
        ...
        after_help = "Commands:\n  \
            ...
            skill list [--json]                                List installable Insights agent skills\n  \
            skill install [--force] <skill-id>                 Install a skill into .agents/skills/\n  \
            agents-md install [--path AGENTS.md]               Install the Insights steering snippet\n\n\
            Run `insights-cli api --agent-help` for the full agent-facing usage guide.\n\n\
            Destructive tools require --confirm. Use --dry-run before mutations.\n\n\
            Limitation: `login` grants the MCP scope set minus the writes Insights \
...
pub enum ExpCommand {
    ...
    #[command(about = "Upload hermes sourcemaps to Insights", hide = true)]
    Hermes {
        #[command(subcommand)]
        cmd: HermesSubcommand,
    },

    #[command(about = "Upload proguard mapping files to Insights", hide = true)]
    Proguard {
        #[command(subcommand)]
        cmd: ProguardSubcommand,
    },

    #[command(about = "Upload iOS/macOS dSYM files to Insights", hide = true)]
    Dsym {
        #[command(subcommand)]
        cmd: DsymSubcommand,
    },
...
mod tests {
    ...
    fn dry_run_defaults_to_false() {
        let cli = Cli::try_parse_from(["insights-cli", "login"]).unwrap();
        assert!(!cli.dry_run);
    }

    #[test]
    fn dry_run_flag_sets_true_before_subcommand() {
        let cli = Cli::try_parse_from(["insights-cli", "--dry-run", "login"]).unwrap();
        assert!(cli.dry_run);
    }
    ...
    fn dry_run_classifies_every_command() {
        ...
        for (argv, expected) in cases {
            let cli = Cli::try_parse_from(std::iter::once(&"insights-cli").chain(argv.iter()))
