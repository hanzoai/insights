...
export interface codeOwnersModalLogicValues {
    meFirstMembers: OrganizationMemberType[] // assigneeSelectLogic
    ...
    dateRange: string
    hasParsedOwners: boolean
    isOpen: boolean
    mappingRows: CodeOwnerOwnerMapping[]
    mappingUnresolvedCount: number
    matchResults: Record<string, MatchCount | null>
    matchResultsLoading: boolean
    ownerRows: CodeOwnerRuleCandidate[]
    ownersToMap: string[]
    parseErrors: CodeownersError[]
    parsedOwners: OwnerGroup[]
    rawText: string
    savableRows: CodeOwnerRuleCandidate[]
    saving: SaveAllResult
    savingLoading: boolean
    step: 'configure' | 'impact' | 'paste'
    unmatchedCount: number
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface codeOwnersModalLogicActions {
    ensureAssigneeTypesLoaded: () => {
        value: true
    } // assigneeSelectLogic
    loadRoles: () => any // rolesLogic
    backToMapping: () => {
        value: true
    }
    backToPaste: () => {
    ...
    resetMatchResults: () => any
    resetMatchResultsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface codeOwnersModalLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        parsedOwners: (rawText: string) => OwnerGroup[]
        ownerRows: (
            parsedOwners: OwnerGroup[],
            roles: RoleType[],
        ...
        savableRows: (ownerRows: CodeOwnerRuleCandidate[]) => CodeOwnerRuleCandidate[]
        mappingRows: (ownerRows: CodeOwnerRuleCandidate[], ownersToMap: string[]) => CodeOwnerOwnerMapping[]
        hasParsedOwners: (parsedOwners: OwnerGroup[]) => boolean
        parseErrors: (rawText: string) => CodeownersError[]
        unmatchedCount: (ownerRows: CodeOwnerRuleCandidate[]) => number
        mappingUnresolvedCount: (mappingRows: CodeOwnerOwnerMapping[]) => number
    }
}

export type codeOwnersModalLogicType = MakeLogicType<
    codeOwnersModalLogicValues,
    codeOwnersModalLogicActions,
    Record<string, any>,
    codeOwnersModalLogicMeta
>
...
export const codeOwnersModalLogic = kea<codeOwnersModalLogicType>([
    ...
    listeners(({ actions, values }) => ({
        ...
        saveAllFailure: ({ errorObject }) => {
            rulesLogic({ ruleType: ErrorTrackingRuleType.Assignment }).actions.loadRules()
            toast.error(errorObject instanceof Error ? errorObject.message : 'Failed to save assignment rules')
        },
    })),
