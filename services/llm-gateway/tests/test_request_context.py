...
from llm_gateway.request_context import (
    RequestContext,
    apply_insights_context_from_headers,
    get_insights_flags,
...
class TestApplyInsightsContextFromHeaders:
    @pytest.fixture(autouse=True)
    ...
    def test_sets_properties_and_flags_from_headers(self) -> None:
        ...
        request.headers.items.return_value = [
            ...
            ("X-INSIGHTS-FLAG-EXPERIMENT", "test"),
            ("Content-Type", "application/json"),
        ]
        set_request_context(RequestContext(request_id="req-123"))

        apply_insights_context_from_headers(request)

        assert get_insights_properties() == {"variant": "memes"}
        assert get_insights_flags() == {"experiment": "test"}
    ...
    def test_leaves_existing_context_unchanged_without_matching_headers(self) -> None:
        ...
        set_request_context(
            RequestContext(
                request_id="req-123",
                insights_properties={"existing": "property"},
                insights_flags={"existing": "flag"},
            )
        )

        apply_insights_context_from_headers(request)

        assert get_insights_properties() == {"existing": "property"}
        assert get_insights_flags() == {"existing": "flag"}
