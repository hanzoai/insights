...
from insights.insightsql.functions.mapping import INSIGHTSQL_AGGREGATIONS
...
assert set(AGGREGATION_TO_STATE_MAPPING.values()).issubset(INSIGHTSQL_AGGREGATIONS_KEYS_SET), (
    f"All supported state aggregation functions must be in INSIGHTSQL_AGGREGATIONS. Missing: {set(AGGREGATION_TO_STATE_MAPPING.values()) - INSIGHTSQL_AGGREGATIONS_KEYS_SET}"
)
...
assert set(STATE_TO_MERGE_MAPPING.values()).issubset(INSIGHTSQL_AGGREGATIONS_KEYS_SET), (
    f"All supported aggregation merge functions must be in INSIGHTSQL_AGGREGATIONS. Missing: {set(STATE_TO_MERGE_MAPPING.values()) - INSIGHTSQL_AGGREGATIONS_KEYS_SET}"
)
...
def transform_query_to_state_aggregations(query: QueryType) -> QueryType:
    ...
    Args:
        query: The InsightsQL query AST to transform
    """
...
def combine_queries_with_state_and_merge(*query_strings: str) -> QueryType:
    ...
    Args:
        *query_strings: Variable number of InsightsQL query strings to combine
