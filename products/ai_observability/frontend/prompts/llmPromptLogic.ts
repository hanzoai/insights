            ...
            submit: async (formValues) => {
                ...
                try {
                    ...
                    if (isNew) {
                        ...
                        llmPromptsLogic.findMounted()?.actions.loadPrompts(false)
                        toast.success(`Published v${savedPrompt.version}`)
                    ...
                    if (error instanceof ApiError && error.status === 409) {
                        ...
                        toast.error(STALE_PROMPT_ERROR_MESSAGE)
                        throw error
                    }
    ...
    listeners(({ actions, asyncActions, props, values }) => ({
        ...
        setLabel: async ({ labelName, version }) => {
            try {
                ...
                toast.success(`${labelName} now points at v${version}`)
                llmPromptsLogic.findMounted()?.actions.loadPrompts(false)
            } catch (error) {
                toast.error(getApiErrorDetail(error) || 'Failed to set label')
                if (error instanceof ApiError && error.status === 409) {
        ...
        removeLabel: async ({ labelName }) => {
            try {
                ...
                if (!(error instanceof ApiError && error.status === 404)) {
                    toast.error(getApiErrorDetail(error) || 'Failed to remove label')
                    return
                }
            }
            if (isPrompt(values.prompt)) {
            ...
            toast.info(`Label ${labelName} removed`)
            llmPromptsLogic.findMounted()?.actions.loadPrompts(false)
        },
        ...
        deletePrompt: async () => {
            if (props.promptName !== 'new' && values.prompt && isPrompt(values.prompt)) {
                try {
                    ...
                    toast.info(`${values.prompt.name || 'Prompt'} has been archived.`)
                    llmPromptsLogic.findMounted()?.actions.loadPrompts(false)
                    ...
                    toast.error(getApiErrorDetail(error) || 'Failed to archive prompt')
                }
            }
        },
        ...
        loadComparePromptFailure: () => {
            toast.error('Failed to load comparison version')
        },
    })),
