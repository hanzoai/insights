         ...
         score_chunk_activity(spec)    (× N, runs in parallel)
            internally:
              ...
              4. get_producer(REPLAY) → Kafka topic
                 (datastore_session_replay_events)
...
- One JSONEachRow message per scored session on
  `KAFKA_DATASTORE_SESSION_REPLAY_EVENTS` — the same topic ingestion uses.
...
`insights/temporal/session_replay/session_summary/activities/video_based/a7d_tag_and_highlight_session.py`.
...
The command uses the same `insights.storage.object_storage` client the
...
`total_fetched` is rows the feature SELECT returned from Datastore;
...
- **Extend the feature set.** The initial production booster covers the
  ...
  `insights/session_recordings/sql/session_replay_feature_sql.py` (CH
