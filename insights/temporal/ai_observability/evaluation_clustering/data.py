"""InsightsQL data access for Stage B evaluation clustering.
...
# InsightsQL timeout is 60s, which can be tight for high-volume eval jobs.
...
class EvaluationMetadata:
    ...
    evaluation_runtime: str | None  # "llm_judge" | "script"
...
def fetch_evaluation_embeddings(
    ...
    Passing bounds also lets Datastore prune date partitions on the
    ...
    with tags_context(product=Product.LLM_ANALYTICS, feature=Feature.QUERY, team_id=team.id):
        result = execute_insightsql_query(
            ...
            team=team,
            settings=InsightsQLGlobalSettings(max_execution_time=CLUSTERING_QUERY_MAX_EXECUTION_TIME),
        )
...
def fetch_evaluation_metadata(
    ...
    Previously this was a single InsightsQL query with ``LEFT JOIN events AS g ON ...``.
    Datastore 26.3's join analyzer rejects the resulting ON expression because
    InsightsQL rewrites ``=`` as ``ifNull(equals(a, b), isNull(a) AND isNull(b))`` for
...
def fetch_generation_contents(
    ...
    *,
    ...
    # Read the heavy `input` / `output_choices` columns from `insights.ai_events`,
...
def fetch_evaluator_configs(team: Team, evaluator_ids: list[str]) -> dict[str, dict]:
    ...
    script code) rather than inferring from reasoning text alone. Bounded because
...
def _coerce_bool(value: object) -> bool | None:
    """Datastore JSONExtract on boolean properties returns the string `'true'`/`'false'`.
