...
function SessionSummaryFeedbackSurvey(): JSX.Element | null {
    ...
    useEffect(() => {
        insights.getSurveys((surveys: any[]) => {
            const match = surveys.find((s: any) => s.id === SESSION_SUMMARY_FEEDBACK_SURVEY_ID)
            if (match) {
                insights.capture('survey shown', { $survey_id: SESSION_SUMMARY_FEEDBACK_SURVEY_ID })
                setSurvey(match)
            }
        })
    }, [])
    ...
    return (
        <div className="border rounded p-3 mt-3">
            <div className="flex items-start justify-between">
                ...
                <Button size="xsmall" icon={<IconX />} onClick={() => setShowFeedbackSurvey(false)} />
            </div>
            {submitted ? (
