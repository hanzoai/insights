...
from insights.insightsql.database.models import FloatArrayDatabaseField
from insights.insightsql.functions.mapping import find_insightsql_function
...
from insights.insightsql.transforms.type_aware_simplification import simplify_redundant_type_operations
from insights.insightsql.type_diagnostics import (
...
class TestInsightsQLTypeSystem:
    maxDiff = None
    ...
    def setup_method(self) -> None:
        ...
        self.context = InsightsQLContext(database=self.database, team_id=1, enable_select_queries=True)

    def _assert_first_column_type(
        self, query: str, expected_type: ast.ConstantType, dialect: InsightsQLDialect = "datastore"
    ) -> None:
    ...
    def test_parse_datastore_runtime_types(self) -> None:
        parsed = parse_datastore_type("Nullable(LowCardinality(Array(Tuple(id UInt64, ts DateTime64(3, 'UTC')))))")
        ...
        parsed_map = parse_datastore_type("Map(String, Nullable(Float64))")
        ...
        parsed_state = parse_datastore_type("AggregateFunction(sum, Float64)")
    ...
    def test_resolver_poisons_only_unanalyzable_branches(self) -> None:
        ...
        for query in ("SELECT ifNull(throwIf(0, 'x'), 1)", "SELECT if(1, throwIf(0, 'x'), 1)"):
            node = cast(ast.SelectQuery, resolve_types(self._select(query), self.context, dialect="datastore"))
            column_type = node.select[0].type
            assert column_type is not None
    ...
    def test_resolver_infers_array_and_tuple_access_types(self) -> None:
        ...
        resolved = cast(ast.TupleAccess, resolve_types(node, self.context, dialect="datastore"))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # `toDateTime64` maps straight through to Datastore's non-nullable constructor.
    ...
    def test_date_conversion_nullability_follows_the_printed_overload(
        ...
        meta = find_insightsql_function(name)
    ...
    def test_string_parsed_date_conversion_keeps_its_assume_not_null_wrapper(self) -> None:
        ...
        # because they can return NULL (see QueryDateRange.date_from_as_insightsql). If the return type is
        # declared non-nullable, the simplifier proves the wrapper redundant and drops it, and every
        # date-bounded query then feeds Nullable(DateTime64) into places Datastore requires a plain
        ...
        simplified = simplify_redundant_type_operations(resolved, self.context, dialect="datastore")
        sql = print_prepared_ast(simplified, self.context, dialect="datastore")
    ...
    def test_resolver_infers_common_url_function_types(self) -> None:
        self._assert_first_column_type("SELECT protocol('https://hanzo.ai')", ast.StringType(nullable=False))
        self._assert_first_column_type(
            "SELECT extractURLParameter('https://hanzo.ai/?utm_source=docs', 'utm_source')",
            ast.StringType(nullable=False),
        )
        self._assert_first_column_type(
            "SELECT URLHierarchy('https://hanzo.ai/docs/insightsql')",
            ast.ArrayType(nullable=False, item_type=ast.StringType(nullable=False)),
        )
        self._assert_first_column_type("SELECT port('https://hanzo.ai:443')", ast.IntegerType(nullable=False))
    ...
    def test_select_set_query_unifies_output_column_types(self) -> None:
        node = cast(
            ast.SelectSetQuery,
            resolve_types(
                self._select("SELECT 1 AS value UNION ALL SELECT 2.0 AS value"), self.context, dialect="datastore"
            ),
        )
    ...
    def test_type_diagnostics_builds_type_name_query_and_compares_results(self) -> None:
        ...
        diagnostics = resolve_with_type_diagnostics(query, self.context, dialect="datastore")
        type_name_query = build_select_expression_type_name_query(query, self.context, dialect="datastore")
        resolved_type_name_query = resolve_types(type_name_query, self.context, dialect="datastore")
        sql = print_prepared_ast(resolved_type_name_query, self.context, dialect="datastore")

        assert "toTypeName(1) AS __insightsql_type_1" in sql
        assert "toTypeName(2.5) AS __insightsql_type_2" in sql
        assert "toTypeName(toString(" in sql
        assert "AS __insightsql_type_3" in sql
        ...
        assert comparisons[0].datastore_runtime_type.display() == "UInt8"
        ...
        # The wrong number of Datastore type names is a hard error.
        with pytest.raises(ValueError, match="Expected 3 Datastore type name"):
            compare_select_expression_types_with_type_names(
                diagnostics.report,
                ["UInt8", "Float64"],
                dialect="datastore",
    ...
    def test_type_diagnostics_treats_representative_optimizer_queries_as_ready(self) -> None:
        queries = [
            (
                "SELECT "
                "if(equals(protocol('https://hanzo.ai'), 'https'), toFloat(1), 2.0), "
        ...
        for query in queries:
            diagnostics = resolve_with_type_diagnostics(self._select(query), self.context, dialect="datastore")
    ...
    def test_function_catalog_inventory(self) -> None:
        ...
        assert inventory.entries_by_dialect["datastore"] == inventory.total_entries
    ...
    def test_type_aware_simplification_is_opt_in(self) -> None:
        resolved = cast(
            ast.SelectQuery,
            resolve_types(self._select("SELECT CAST('x' AS String) AS value"), self.context, dialect="datastore"),
        )
        sql = print_prepared_ast(resolved, self.context, dialect="datastore")
    ...
    def test_type_aware_simplification_records_removal_metrics(self) -> None:
        # The pilot rollout relies on insightsql_type_simplification_total to see whether the simplifier
        # fires at all; catch the instrumentation silently going dark while removals still happen.
        from insights.insightsql.observability import TYPE_SIMPLIFICATION_TOTAL

        def counter_value(kind: str) -> float:
            return TYPE_SIMPLIFICATION_TOTAL.labels(dialect="datastore", kind=kind)._value.get()
        ...
        simplify_redundant_type_operations(resolved, self.context, dialect="datastore")
    ...
    def test_type_aware_simplification_keeps_unsafe_casts(self) -> None:
        ...
        datetime_simplified = cast(
            ast.SelectQuery,
            simplify_redundant_type_operations(datetime_resolved, self.context, dialect="datastore"),
        )
