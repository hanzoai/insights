 ...
 *  - UI-only bulk fields (`datastore_sql`, `insightsql`, `insight`, and
...
describe('transformExperimentResults', () => {
    ...
    it('strips UI-only bulk fields (SQL bodies, step_sessions) but keeps statistical data', () => {
        ...
        // act on those event ids. datastore_sql / insightsql are also debug-only. Strip both,
        ...
        expect(data.datastore_sql).toBeUndefined()
        expect(data.insightsql).toBeUndefined()
