...
description: >
  ...
  Insights feature at scale and streams the results to a Slack channel as a
...
The workflow is **mixed, and leans UI**. Trace inspection and filter discovery (steps 1-2) are MCP-driven. Eval creation, dry-running, and enabling (steps 4-5) are MCP-driven _when_ `insights:llma-evaluation-*` tools are exposed to your agent — but they often aren't, in which case fall back to the UI (Data pipeline → destinations for the alert is always UI). Each step flags its UI fallback. Expect to finish in the UI even when you start from chat.
...
**Pattern B — Insights AI agent mode.** For features the user interacts with _via_ Insights AI in a specific agent mode (error tracking, product analytics, session replay, SQL, flags, surveys, AI observability). Filter on `ai_product = 'insights_ai' AND agent_mode = '<mode>'`. This requires PR #55160 (merged April 2026) to be deployed, which threads `agent_mode` and `supermode` onto every `$ai_generation` emitted by the chat agent loop. A useful ergonomic side-effect: `agent_mode IS NOT NULL` is a reliable "user-facing chat turn" filter — batch jobs and tool-internal LLM calls go through different code paths and have `agent_mode=null`, so they're excluded for free.

If the user asks "what are users trying to DO in [ET / replay / SQL / flags / surveys] mode of Insights AI", that's Pattern B. If they ask "what use cases does [standalone feature] cover", that's Pattern A. Pick the pattern first — the prompt, filter, and Slack channel naming all follow from it.

## Prerequisites

| Requirement                                                                                              | How to verify                                                                                                                                                                                                             |
| -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (Pattern A) Feature emits `$ai_generation` events with a stable `$ai_trace_id` pattern                   | `insights:execute-sql` for distinct `$ai_trace_id` prefixes                                                                                                                                                                |
| (Pattern B) `agent_mode` property is present on recent `$ai_generation` events                           | `insights:execute-sql` group-by `properties.agent_mode` on recent `ai_product='insights_ai'` events. Null bucket is normal (batch jobs + tool-internal calls) — you want non-null coverage across the modes you care about. |
| `$session_id` is attached to the `$ai_generation` events (links trace to trigger session)                | `insights:execute-sql` for `countIf($session_id IS NOT NULL) / count()`                                                                                                                                                    |
...
| Tool                                               | Purpose                                                                                                                                                                                                                                                           |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `insights:query-llm-traces-list`                    | Find sample traces matching the feature's `$ai_trace_id` pattern                                                                                                                                                                                                  |
| `insights:query-llm-trace`                          | Inspect a specific trace's contents end-to-end                                                                                                                                                                                                                    |
| `insights:execute-sql`                              | Verify trace volume, session_id coverage, eval result distributions                                                                                                                                                                                               |
| `insights:llma-evaluation-create`                   | (**often unexposed** — UI fallback: AI observability → Evaluations → New) Create the LLM-judge eval (disabled at first)                                                                                                                                           |
| `insights:llma-evaluation-run`                      | (**often unexposed** — UI fallback: the eval's detail page has a "Run on event" button) Dry-run the eval against specific generations during prompt iteration                                                                                                     |
| `insights:llma-evaluation-update`                   | (**often unexposed** — UI fallback: edit the eval in AI observability → Evaluations) Tweak the prompt / enable when ready                                                                                                                                         |
| `insights:llma-evaluation-summary-create`           | (**often unexposed** — UI fallback: the eval detail page has a "Summarize results" button) After the feed is running, get an AI summary of pass/N/A patterns to validate signal quality                                                                           |
| `insights:workflows-list` / `insights:workflows-get` | (**often unexposed** — UI: Data pipeline → Workflows) Browse existing workflow configs — useful for cloning an existing feed's structure when setting up a new one. Read-only; no create/update tool is exposed yet, so step 6's Slack workflow setup is UI-only. |

Before starting, **check which of the `insights:llma-evaluation-*` tools are actually exposed in your agent's MCP tool set.** If they aren't loaded, treat steps 4-5 as UI walkthroughs rather than tool calls.
...
Note: `coalesce(..., '')` is load-bearing — `splitByChar` on a nullable column errors out in InsightsQL otherwise.

**Pattern B (Insights AI agent mode):** verify coverage and volume for the mode you're targeting.
...
WHERE timestamp > now() - INTERVAL 3 DAY
    AND event = '$ai_generation'
    AND properties.ai_product = 'insights_ai'
...
```json
insights:query-llm-traces-list
{
  "properties": [
...
{
  "properties": [
    { "type": "event", "key": "ai_product", "operator": "exact", "value": "insights_ai" },
...
```text
You are analyzing a Insights [FEATURE NAME] trace to extract its real use case.
...
| Insights AI in error tracking mode | `A user asked Insights AI about`            |
| Insights AI in session replay mode | `A user asked Insights AI about`            |
| Insights AI in SQL mode            | `A user asked Insights AI to write SQL for` |

Note: `supermode='plan'` is a sub-filter that layers _on top of_ an `agent_mode` row — it's not its own row. If you want plan-mode-only, filter `agent_mode='<mode>' AND supermode='plan'` and pick an opener like `"A user asked Insights AI to plan"`.
...
**If `insights:llma-evaluation-create` is exposed**, use this payload:
  ...
  "conditions": [
    {
      ...
      "properties": [
        ...
        // Pattern B — Insights AI agent mode (use these INSTEAD of the trace_id filter):
        // { "key": "ai_product", "type": "event", "operator": "exact", "value": "insights_ai" },
...
**If `insights:llma-evaluation-run` is exposed:**
...
**If `insights:llma-evaluation-update` is exposed:**
...
Workflow setup is not MCP-accessible for writes (`insights:workflows-list` / `insights:workflows-get` are read-only). The steps below are a UI walkthrough.

**Prereq:** before you start, invite the Insights Slack bot to your target channel (`/invite @Insights` in the Slack channel). Without this, the Slack dispatch step will fail with an opaque permission error at send time, not at save time — easy to miss.
...
- **Sender / bot display name:** something that reads well in the channel (e.g. `Insights Usage Feed`)
...
[
  ...
  },
  {
    ...
    "elements": [
      {
        "url": "https://us.hanzo.ai/project/<project_id>/ai-observability/traces/{event.properties.$ai_trace_id}?event={event.properties.$ai_target_event_id}",
        "text": { "text": "View Trace", "type": "plain_text" },
        "type": "button"
      },
      {
        "url": "https://us.hanzo.ai/project/<project_id>/replay/{event.properties.$session_id}",
...
The `{event.properties.X}` and `{person.X}` placeholders are valid Insights template syntax and resolve at send time.
...
- Insights/insights#54952 — threads `trigger_session_id` through to `$ai_generation` events on the session summary backend
- (Followup PR — threads `$session_id` onto `$ai_evaluation` events specifically)

## Worked example B (Pattern B): Insights AI in error tracking mode

Pattern: an `agent_mode = 'error_tracking'` scoped feed streaming to a `#<team>-usage-feed` channel, answering "what are users actually trying to DO when they chat with Insights AI in error tracking mode?" Mode sizing varies by an order of magnitude or more across agent modes — spot-check volume per §Step 1 before wiring, because a high-volume mode can flood a channel. Opener: `"A user asked Insights AI about"`.

Enabling PR: Insights/insights#55160 — threads `agent_mode` and `supermode` onto every `$ai_generation` emitted by the chat agent loop. Wiring lives in `ee/scriptai/core/agent_modes/executables.py` (`AgentExecutable._get_model`) and passes the dict through the existing `insights_properties` field on `MaxChatMixin` in `ee/scriptai/llm.py`. Before this PR, scoping a Insights AI eval to a specific mode wasn't possible — you'd end up evaluating every Insights AI generation, which produced noisy feeds with low single-digit PASS rates.
...
**If `insights:llma-evaluation-summary-create` is exposed:**
