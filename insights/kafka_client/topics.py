...
from insights.settings.data_stores import SUFFIX
from insights.settings.kafka import KAFKA_PREFIX
from insights.settings.utils import get_from_env

KAFKA_EVENTS_JSON = f"{KAFKA_PREFIX}datastore_events_json{SUFFIX}"
...
KAFKA_PERSON = f"{KAFKA_PREFIX}datastore_person{SUFFIX}"
KAFKA_PERSON_UNIQUE_ID = f"{KAFKA_PREFIX}datastore_person_unique_id{SUFFIX}"  # DEPRECATED_DO_NOT_USE
KAFKA_PERSON_DISTINCT_ID = f"{KAFKA_PREFIX}datastore_person_distinct_id{SUFFIX}"
KAFKA_PERFORMANCE_EVENTS = f"{KAFKA_PREFIX}datastore_performance_events{SUFFIX}"
...
KAFKA_GROUPS = f"{KAFKA_PREFIX}datastore_groups{SUFFIX}"
KAFKA_INGESTION_WARNINGS = f"{KAFKA_PREFIX}datastore_ingestion_warnings{SUFFIX}"
# DEPRECATED: superseded by datastore_app_metrics2 (see KAFKA_APP_METRICS2 below).
# Kept only because the legacy `kafka_app_metrics` Datastore Kafka-engine table
# still references this topic in its DDL — see insights/models/app_metrics/sql.py.
# No producer writes to it. Remove once the underlying tables are dropped.
KAFKA_APP_METRICS = f"{KAFKA_PREFIX}datastore_app_metrics{SUFFIX}"
KAFKA_APP_METRICS2 = f"{KAFKA_PREFIX}datastore_app_metrics2{SUFFIX}"
KAFKA_METRICS_TIME_TO_SEE_DATA = f"{KAFKA_PREFIX}datastore_metrics_time_to_see_data{SUFFIX}"
...
# from recordings consumer to datastore
KAFKA_DATASTORE_SESSION_REPLAY_EVENTS = f"{KAFKA_PREFIX}datastore_session_replay_events{SUFFIX}"
KAFKA_DATASTORE_SESSION_REPLAY_FEATURES = f"{KAFKA_PREFIX}datastore_session_replay_features{SUFFIX}"
KAFKA_DATASTORE_SESSION_RECORDING_EVENTS = f"{KAFKA_PREFIX}datastore_session_recording_events{SUFFIX}"

KAFKA_ERROR_TRACKING_ISSUE_FINGERPRINT = f"{KAFKA_PREFIX}datastore_error_tracking_issue_fingerprint{SUFFIX}"
KAFKA_ERROR_TRACKING_FINGERPRINT_ISSUE_STATE = (
    f"{KAFKA_PREFIX}datastore_error_tracking_fingerprint_issue_state{SUFFIX}"
)
KAFKA_ERROR_TRACKING_ISSUE_FINGERPRINT_EMBEDDINGS = (
    f"{KAFKA_PREFIX}datastore_error_tracking_issue_fingerprint_embeddings{SUFFIX}"
)
KAFKA_DOCUMENT_EMBEDDINGS_TOPIC = f"{KAFKA_PREFIX}datastore_document_embeddings{SUFFIX}"
...
KAFKA_CDP_DATASTORE_PRECALCULATED_PERSON_PROPERTIES = (
    f"{KAFKA_PREFIX}datastore_precalculated_person_properties{SUFFIX}"
)
KAFKA_CDP_DATASTORE_PREFILTERED_EVENTS = f"{KAFKA_PREFIX}datastore_prefiltered_events{SUFFIX}"
