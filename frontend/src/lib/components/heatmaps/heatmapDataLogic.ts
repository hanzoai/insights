    ...
    listeners(({ actions, values, props }) => ({
        ...
        loadMoreAreaEvents: async () => {
            ...
            try {
                ...
                toast.error(e instanceof Error ? e.message : 'Failed to load more events')
                return
            }
        ...
        loadHeatmapFailure: ({ error }) => {
            toast.error(error || 'Heatmap query failed')
            actions.setIsReady(true)
        },
        loadAreaEventsFailure: ({ error }) => {
            toast.error(error || 'Failed to load events for selected area')
        },
    })),
