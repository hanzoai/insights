...
`workflow_activities/repartition_table.py`). Everything is observable via Insights events.
"""
...
def capture_repartition_event(event: str, props: dict[str, Any]) -> None:
    hanzo_insights.capture(distinct_id=get_machine_id(), event=event, properties=props)
...
async def maybe_flag_for_coarsening(
    ...
    *,
    ...
    the feature flag (a Team fetch plus a Insights API call) dead last. Post-load detection runs this
...
async def maybe_flag_for_repartition(
    ...
    *,
    ...
    evaluation is a `Team.objects.get()` plus a Insights API call) to avoid re-evaluating it here; when
