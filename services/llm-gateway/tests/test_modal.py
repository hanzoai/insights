...
def _modal_settings(**overrides: Any) -> Settings:
    base: dict[str, Any] = {
        "modal_api_base": "https://insights--glm.us-east.modal.direct/v1",
        "modal_kimi_api_base": "https://insights--kimi.us-west.modal.direct/v1",
        "modal_key": "wk-test",
...
def test_kimi_uses_its_endpoint_with_shared_credentials() -> None:
    assert ensure_modal_model_configured(KIMI_MODEL, _modal_settings()) == (
        "https://insights--kimi.us-west.modal.direct/v1",
        "wk-test",
        "ws-test",
    )
...
async def test_modal_anthropic_drops_clear_thinking_when_thinking_is_disabled() -> None:
    ...
    with patch("llm_gateway.modal_routing.send_modal_request", new=AsyncMock(return_value={})) as send_request:
        await send_modal_anthropic_messages(request, MagicMock(), False, "insights_code")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0.0, {"insights_code": 1.0}, True, True),
        (1.0, {"insights_code": 0.0}, True, False),
    ],
)
def test_should_route_glm_to_modal(
    ...
    assert (
        should_route_glm_to_modal(GLM_MODEL, product="insights_code", user_key="user-1", settings=settings) is expected
    )


@pytest.mark.parametrize("product", ["twig", "array"])
def test_product_aliases_resolve_in_fraction_lookup(product: str) -> None:
    # Legacy aliases must read insights_code's per-product fraction, or its ramp/rollback would
    # silently skip alias traffic.
    settings = _modal_settings(
        glm_modal_traffic_fraction=0.0, glm_modal_product_traffic_fractions={"insights_code": 1.0}
    )
...
def test_should_route_only_modal_served_models() -> None:
    ...
    assert (
        should_route_glm_to_modal("@cf/moonshotai/kimi-k2.6", product="insights_code", user_key="u", settings=settings)
        is False
    )
...
def test_partial_fraction_is_sticky_and_monotonic() -> None:
    ...
    for user in ("user-1", "user-2", "user-3", "user-4", "user-5"):
        decisions = [
            should_route_glm_to_modal(GLM_MODEL, product="insights_code", user_key=user, settings=s) for s in settings
        ]
...
def test_partial_fraction_splits_pinned_users() -> None:
    ...
    assert should_route_glm_to_modal(GLM_MODEL, product="insights_code", user_key="3", settings=settings) is True
    assert should_route_glm_to_modal(GLM_MODEL, product="insights_code", user_key="2", settings=settings) is False
