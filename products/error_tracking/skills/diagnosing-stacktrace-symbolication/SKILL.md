...
description: >
  Help users debug Insights Error Tracking stack-trace symbolication for any supported platform — JavaScript/TypeScript
  web, React Native (Hermes), Android (Proguard / R8), or iOS / macOS (dSYM). The Insights symbol-set lookup flow is
  ...
  uploaded, Insights symbol sets show last_used but frames are not readable, chunk IDs or dSYM UUIDs do not match,
...
Work through the user's build and Insights symbol sets as one pipeline: build config -> generated symbol artifacts
(JavaScript source maps, Hermes maps, Proguard mappings, or dSYM bundles) -> uploaded symbol set in Insights ->
...
Step 3 of the workflow (symbol-set lookup in Insights) is identical across platforms — `insights-cli symbol-sets
...
Look at the app repo's build scripts and Insights upload config. Confirm which Insights package handles the upload
(`@hanzo/rollup-plugin`, `@hanzo/webpack-plugin`, `@hanzo/nextjs-config`, `@hanzo/nuxt`, or direct
`insights-cli`) and which directory or asset it processes. See the platform reference for build-tool-specific
config inspection.

For debugging, prefer a build where symbol artifacts remain on disk after upload so you can compare local
artifacts against what Insights received. JavaScript example with the Vite plugin (the platform reference covers
...
If local artifacts already look wrong, fix the build before debugging the Insights upload.

### Step 3 - Check symbol sets in Insights
...
- `insights:error-tracking-symbol-sets-list` with `ref=<chunk_id>` returns the matching row.
- `insights:error-tracking-symbol-sets-retrieve` with the ID returns the same shape (and confirms permissions).
- `insights:error-tracking-symbol-sets-download-retrieve` returns a one-hour presigned URL pointing at the uploaded
...
- `last_used` updating means Insights found and loaded that symbol set. It does not guarantee the frame resolved.
- `has_uploaded_file: false` means the upload did not complete.
- A non-null `failure_reason` means Insights could not parse or load the uploaded symbol data.

The downloaded file is a Insights symbol-data container (compressed Rust-encoded payload), not plain JSON. Extract
it with `insights-cli`:

```bash
insights-cli symbol-sets extract symbolset.bin -o ./extracted
# or, without installing globally:
#   npx @hanzo/cli symbol-sets extract symbolset.bin -o ./extracted
#   bunx @hanzo/cli symbol-sets extract symbolset.bin -o ./extracted
```

`insights-cli symbol-sets extract` handles all four symbol-set types (source-and-map, hermes, proguard, dSYM) and
...
- `Token not found`: Insights loaded the symbol data but the captured generated position did not match any token in
...
- Use the latest Insights build plugin and `insights-cli`.
...
From an affected Insights error event, collect one minified application frame:
