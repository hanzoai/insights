...
a synchronous Datastore INSERT, on the request thread. Instrumentation put ~10s of a ~13s dashboard load
there, with Datastore itself only ~1.2s.
...
# the ceiling for when both of those fail. Must stay well under the framework's 48h Datastore expiry
