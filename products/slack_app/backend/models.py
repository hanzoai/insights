...
class SlackThreadTaskMapping(UUIDModel):
    ...
    team = models.ForeignKey("insights.Team", on_delete=models.CASCADE, related_name="slack_thread_task_mappings")
...
class SlackSettings(UUIDModel):
    ...
    Currently stores the routing default — which Insights integration a mention
    ...
    - ``slack_user_id`` set → that Slack user's personal settings for this workspace.
      Written by the Slack `@Insights project <id>` directive or the user-level
      settings UI.
    - ``slack_user_id IS NULL`` → workspace-wide fallback, applied when an
      ...
      Insights project-level settings UI by a team admin, or via the Slack
      `@Insights project workspace <id>` directive by a Slack workspace
      admin/owner.
...
class SlackChannel(UUIDModel):
    ...
    in the channel has acknowledged that Insights data answered there may be
    ...
    workspace can be connected to multiple Insights projects, and once any
    ...
    approved_by = models.ForeignKey(
        ...
        help_text="The Insights user who clicked Approve. Carries the email and audit trail.",
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
