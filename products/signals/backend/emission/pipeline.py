...
from insights.llm.gateway_client import build_async_anthropic_client, resolve_ai_gateway_config
...
def _signals_extra_headers(
    ...
    In Go-gateway mode the labels ride on the `X-Insights-Properties` JSON blob and the caller owns
    ...
    drops the `x-insights-property-<key>` per-header form, so that shape is only used on the
    ...
    if gateway_mode:
        ...
        return {"X-Insights-Properties": json.dumps(blob)}
    return {f"x-insights-property-{key}": value for key, value in labels.items()}
