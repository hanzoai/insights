...
from insights.temporal.tests.utils.events import generate_test_events, insert_event_values_in_datastore
...
class TestFileDownloadInsightsQL:
    """File download batch exports created from a user-defined InsightsQL query."""

    INSIGHTSQL_FLAG_PATCH_TARGET = "products.batch_exports.backend.api.file_download.hanzo_insights.feature_enabled"

    @pytest.fixture
    def enable_insightsql_flag(self):
        """Enable the insightsql-batch-exports feature flag for the duration of a test."""
        with unittest.mock.patch(self.INSIGHTSQL_FLAG_PATCH_TARGET, return_value=True):
            yield

    @pytest.fixture
    async def insightsql_export_test_events(self, datastore_client, team):
        """Insert events for this and another team directly into the events table.

        A insightsql export reads the main (sharded) events table with no interval filter, so
        ...
        with unittest.mock.patch(self.INSIGHTSQL_FLAG_PATCH_TARGET, return_value=False) as mock_flag:
            response = await async_client.post(
                f"/api/projects/{team.pk}/file_download_batch_exports",
                {
                    "file": {"format": "Parquet"},
                    "model": "insightsql",
                    "insightsql_query": "SELECT event AS event FROM events",
                },
                content_type="application/json",
            )

        assert response.status_code == status.HTTP_403_FORBIDDEN, response.json()
        assert mock_flag.call_args[0][0] == "insightsql-batch-exports"
        mock_start_file_download_export.assert_not_called()
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ...
                "'insightsql_query' is required when 'model' is 'insightsql'",
                id="missing-query",
            ),
            pytest.param(
                {"insightsql_query": "this is not insightsql"},
                "Failed to parse InsightsQL query",
                id="unparseable-query",
            ),
            pytest.param(
                {"insightsql_query": "SELECT event AS event FROM events WHERE {filters}"},
                "Placeholders are not supported",
                id="placeholder-query",
            ),
            pytest.param(
                {"insightsql_query": "SELECT count() FROM events"},
                "must be a field or have an alias",
                id="unaliased-expression-column",
            ),
            pytest.param(
                {"insightsql_query": "SELECT x AS x FROM no_such_table"},
                "Invalid InsightsQL query",
                id="unknown-table",
            ),
            pytest.param(
                {"data_interval_start": "2026-01-01T00:00:00+00:00", "data_interval_end": "2026-01-02T00:00:00+00:00"},
                "not supported when 'model' is 'insightsql'",
                id="intervals-with-insightsql",
            ),
            pytest.param(
                {"include": ["my-event"]},
                "'include' and 'exclude' are not supported when 'model' is 'insightsql'",
                id="include-with-insightsql",
            ),
            pytest.param(
                {"model": "events"},
                "'insightsql_query' is only supported when 'model' is 'insightsql'",
                id="query-with-events-model",
            ),
            pytest.param(
                {"model": "events", "insightsql_query": None},
                "'data_interval_start' and 'data_interval_end' are required",
                id="events-model-missing-intervals",
            ),
        ],
    )
    @pytest.mark.usefixtures("enable_insightsql_flag")
    @pytest.mark.django_db(transaction=True)
    ...
    async def test_create(self, async_client: AsyncClient, team, user, mock_start_file_download_export):
        """A insightsql create request stores the query on a source and threads it to the workflow.

        The run's data interval is faked as now/now: insightsql exports have no interval, but
        everything downstream formats concrete bounds.
        """
        await async_client.aforce_login(user)
        insightsql_query = "SELECT event AS event, distinct_id AS distinct_id FROM events"
        ...
        response = await async_client.post(
            f"/api/projects/{team.pk}/file_download_batch_exports",
            {
                "file": {"format": "Parquet"},
                "model": "insightsql",
                "insightsql_query": insightsql_query,
            },
            content_type="application/json",
        )
        ...
        assert on_demand.model == "insightsql"
        assert on_demand.source is not None
        assert on_demand.source.insightsql_query == insightsql_query
        ...
        assert batch_export_model.name == "insightsql"
        assert batch_export_model.insightsql_query == insightsql_query
    ...
    async def test_end_to_end(self, async_client: AsyncClient, temporal_client, team, user, insightsql_export_test_events):
        ...
        expected_rows = sorted((e["event"], e["distinct_id"], "Chrome") for e in insightsql_export_test_events)
