...
from insights.models.utils import UpdatedMetaFields, UUIDModel
...
class ProductPushCampaign(UUIDModel, UpdatedMetaFields):
    ...
    # No DB constraints on the FKs: insights_organization and insights_user are hot
...
class OrganizationEnrichment(UUIDModel):
    # db_constraint=False keeps CreateModel off insights_organization's lock path (hot table)
    organization = models.OneToOneField(
        "insights.Organization", on_delete=models.CASCADE, db_constraint=False, related_name="enrichment_record"
    )
