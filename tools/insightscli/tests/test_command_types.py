...
from insightscli.command_types import CompositeCommand, DirectCommand, _find_insightscli_executable


class TestDirectCommandExecution:
    """Test DirectCommand handles shell operators and arguments correctly."""

    @patch("insightscli.command_types._run")
...
class TestCompositeCommandExecution:
    ...
    def test_executes_all_steps_in_sequence(self, mock_run: MagicMock) -> None:
        ...
        bin_insightscli = str(REPO_ROOT / "bin" / "insightscli")
        ...
        assert calls == [
            [bin_insightscli, "docker:services:down"],
            [bin_insightscli, "docker:services:up"],
            [bin_insightscli, "migrations:run"],
        ]
    ...
    def test_inline_steps_execute_directly(self, mock_run: MagicMock) -> None:
        ...
        # Third is a insightscli command
        assert mock_run.call_args_list[2][0][0] == [bin_insightscli, "dev:reset"]
...
class TestRunFunctionErrorHandling:
    ...
    def test_run_raises_systemexit_on_command_failure(self) -> None:
        ...
        from insightscli.command_types import _run
...
class TestConfirmationFeature:
    ...
    def test_composite_command_passes_yes_flag_to_steps(self, mock_run: MagicMock) -> None:
        ...
        calls = [call[0][0] for call in mock_run.call_args_list]
        assert calls == [
            [bin_insightscli, "docker:services:down", "--yes"],
            [bin_insightscli, "docker:services:up", "--yes"],
        ]
    ...
    def test_composite_command_passes_yes_after_user_confirms(
        ...
        mock_run.assert_called_once_with([bin_insightscli, "docker:services:down", "--yes"], env={})


class TestPromptStep:
    """Test prompt steps with conditional execution using insightscli: format."""

    @patch("click.confirm")
    @patch("insightscli.command_types._run")
    def test_prompt_runs_insightscli_when_confirmed(self, mock_run: MagicMock, mock_confirm: MagicMock) -> None:
        """Prompt guard runs insightscli command when user says yes."""
        from insightscli.manifest import REPO_ROOT
        ...
        mock_run.assert_called_once_with([bin_insightscli, "db:restore-schema"], env={})
    ...
    def test_prompt_runs_else_when_declined(self, mock_run: MagicMock, mock_confirm: MagicMock) -> None:
        ...
        mock_confirm.assert_called_once_with("Restore schema?", default=False)
        mock_run.assert_called_once_with([bin_insightscli, "migrations:run"], env={})
    ...
    def test_prompt_skips_when_declined_without_else(self, mock_run: MagicMock, mock_confirm: MagicMock) -> None:
        ...
        cmd = CompositeCommand(
            "test",
            {"steps": [{"insightscli": "optional:thing", "prompt": "Run optional step?"}]},
        )
        cmd.execute()
    ...
    @patch("insightscli.command_types._run")
    def test_insightscli_without_prompt_runs_directly(self, mock_run: MagicMock) -> None:
        """insightscli: without prompt runs the command directly."""
        from insightscli.manifest import REPO_ROOT
        ...
        cmd = CompositeCommand(
            "test",
            {"steps": [{"insightscli": "migrations:run"}]},
        )
        cmd.execute()
    ...
    @patch("insightscli.command_types._run")
    def test_string_step_equivalent_to_insightscli(self, mock_run: MagicMock) -> None:
        """String step is equivalent to insightscli: step."""
        from insightscli.manifest import REPO_ROOT
        ...
        cmd2 = CompositeCommand("test2", {"steps": [{"insightscli": "migrations:run"}]})
...
class TestFindHogliExecutable:
    ...
    def test_prefers_bin_insightscli_when_present(self, tmp_path: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
        """If ``<repo>/bin/insightscli`` exists it's used directly (Insights monorepo case)."""
        fake_repo = tmp_path
        (fake_repo / "bin").mkdir()
        (fake_repo / "bin" / "insightscli").write_text("#!/bin/sh\n")
        monkeypatch.setattr("insightscli.command_types.REPO_ROOT", fake_repo)

        assert _find_insightscli_executable() == [str(fake_repo / "bin" / "insightscli")]

    def test_falls_back_to_path_when_bin_missing(self, tmp_path: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
        """Without ``bin/insightscli``, fall back to whatever ``insightscli`` is on PATH."""
        monkeypatch.setattr("insightscli.command_types.REPO_ROOT", tmp_path)
        monkeypatch.setattr("insightscli.command_types.shutil.which", lambda name: "/opt/venv/bin/insightscli")

        assert _find_insightscli_executable() == ["/opt/venv/bin/insightscli"]

    def test_falls_back_to_python_module_when_path_missing(
        self, tmp_path: MagicMock, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Without ``bin/insightscli`` or PATH entry, use ``python -m insightscli``."""
        monkeypatch.setattr("insightscli.command_types.REPO_ROOT", tmp_path)
        monkeypatch.setattr("insightscli.command_types.shutil.which", lambda name: None)

        assert _find_insightscli_executable() == [sys.executable, "-m", "insightscli"]
