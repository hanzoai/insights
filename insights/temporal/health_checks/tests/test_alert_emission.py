...
from insights.temporal.health_checks.alerts import EVENT_FIRING, EVENT_RESOLVED, emit_health_check_alert
from insights.temporal.health_checks.framework import AlertContent, HealthCheck
...
class TestEmitHealthCheckAlert(SimpleTestCase):
    @patch("insights.temporal.health_checks.alerts._check_class_for_kind", return_value=_StubCheck)
    @patch("insights.temporal.health_checks.alerts.produce_internal_event")
    ...
    @patch("insights.temporal.health_checks.alerts._check_class_for_kind", return_value=None)
    @patch("insights.temporal.health_checks.alerts.produce_internal_event")
    ...
    def test_failure_is_swallowed_and_captured(
        ...
        with (
            patch("insights.temporal.health_checks.alerts._check_class_for_kind", return_value=check_cls),
            patch(
                "insights.temporal.health_checks.alerts.produce_internal_event",
                side_effect=produce_side_effect,
            ) as produce,
            patch("insights.temporal.health_checks.alerts.capture_exception") as capture,
        ):
