...
def test_row_to_dict_with_date_and_query():
    row = {"keys": ["2026-04-15", "insights"], "clicks": 10, "impressions": 100, "ctr": 0.1, "position": 4.5}
    out = _row_to_dict(row, ["date", "query"])

    assert out["date"] == dt.date(2026, 4, 15)
    assert out["query"] == "insights"
...
def test_row_to_dict_prefers_api_date_over_iter_date():
    ...
    row = {"keys": ["2026-04-15", "insights"], "clicks": 1, "impressions": 1, "ctr": 1.0, "position": 1.0}
