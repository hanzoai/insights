...
class QueryStatusManager:
    ...
    @property
    def datastore_query_status_key(self) -> str:
    ...
    def store_query_status(self, query_status: QueryStatus):
        value = SafeJSONRenderer().render(query_status.model_dump(exclude={"datastore_query_progress"}))
    ...
    def _store_datastore_query_progress_dict(self, query_progress_dict):
        value = json.dumps(query_progress_dict)
        self.redis_client.set(self.datastore_query_status_key, value, ex=self.STATUS_TTL_SECONDS)
    ...
    def _get_datastore_query_progress_dict(self):
        try:
            byte_results = self.redis_client.get(self.datastore_query_status_key)
        except Exception:
    ...
    def update_datastore_query_progresses(self, datastore_query_progresses):
        datastore_query_progress_dict = self._get_datastore_query_progress_dict()
        for datastore_query_progress in datastore_query_progresses:
            datastore_query_progress_dict[datastore_query_progress["query_id"]] = datastore_query_progress
        self._store_datastore_query_progress_dict(datastore_query_progress_dict)
    ...
    def get_datastore_progresses(self) -> Optional[DatastoreQueryProgress]:
        try:
            datastore_query_progress_dict = self._get_datastore_query_progress_dict()
            query_progress = {
                "bytes_read": 0,
            ...
            for single_query_progress in datastore_query_progress_dict.values():
                for k in query_progress.keys():
                    query_progress[k] += single_query_progress[k]
            return DatastoreQueryProgress(**query_progress)
        except Exception as e:
    ...
    def delete_query_status(self) -> None:
        ...
        self.redis_client.delete(self.datastore_query_status_key)
...
def _shared_link_user_for(sharing_configuration_id: int, team: "Team") -> Optional["User"]:
    ...
    from insights.models.sharing_configuration import SharingConfiguration  # noqa: PLC0415
...
def cancel_query(team_id: int, query_id: str, dequeue_only: bool = False) -> str:
    ...
    If the query is not queued, it will be cancelled on datastore.

    If dequeue_only is True, only tries to revoke the task, not cancel the query on datastore.
    Useful as we don't want to overwhelm datastore with KILL queries.
    """
    manager = QueryStatusManager(query_id, team_id)
