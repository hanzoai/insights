...
func TestRedisShardedPubSub(t *testing.T) {
	...
	t.Run("multi-shard delivery", func(t *testing.T) {
		...
		ctx := context.Background()
		for _, token := range tokens {
			broker.Publish(ctx, InsightsEvent{
				Token:      token,
				Event:      "$pageview",
		...
		for i, token := range tokens {
			...
			evt, ok := msg.(ResponseInsightsEvent)
			require.True(t, ok, "expected ResponseInsightsEvent for %s", token)
	...
	t.Run("multiple subscribers same token", func(t *testing.T) {
		...
		evt1, ok := msg1.(ResponseInsightsEvent)
		require.True(t, ok)
		evt2, ok := msg2.(ResponseInsightsEvent)
		require.True(t, ok)
	...
	t.Run("full round-trip with filtering", func(t *testing.T) {
		...
		filteredEvt, ok := filteredMsg.(ResponseInsightsEvent)
		require.True(t, ok)
