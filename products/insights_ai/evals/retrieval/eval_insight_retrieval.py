...
* ``skill_loaded`` — did the agent load ``querying-insights-data`` before
...
* ``lookup_id_in_output`` — does the agent's final message include the
  seeded insight's ID? Proves it actually queried Insights rather than
  hallucinating a plausible-looking ID.

To run:
    flox activate -- bash -c "set -a; source .env; set +a; python -m products.insights_ai.eval_harness.harness eval_insight_retrieval"
"""
...
from products.insights_ai.eval_harness.seeders.insight import seed_insight_noise
from products.insights_ai.evals.retrieval.scorers import LookupIdInOutput, SkillLoaded

SKILL_NAME = "querying-insights-data"
    ...
    cases: list[SandboxedEvalCase] = [
        SandboxedEvalCase(
            ...
            prompt=(
                ...
                "in this Insights project and return its numeric ID. Reply with just the ID."
            ),
