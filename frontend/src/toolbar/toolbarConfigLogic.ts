...
export type ApiHostSource = 'insights_api_host' | 'api_url' | 'fallback_rejected' | 'fallback_absent'
...
export interface toolbarConfigLogicValues {
    ...
    authConfirmModalVisible: boolean
    authStatus: 'authenticating' | 'checking' | 'error' | 'idle'
    buttonVisible: boolean
    clientId: string | null
    dataAttributes: string[]
    experimentId: ExperimentIdType | null
    isAuthenticated: boolean
    isTrustedUiHost: boolean
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface toolbarConfigLogicActions {
    authenticate: () => {
        value: true
    }
    clearUserIntent: () => {
    ...
    closeAuthConfirmModal: () => {
        value: true
    }
    closeUiHostConfigModal: () => {
        value: true
    }
    confirmAuthenticate: () => {
        value: true
...
export interface toolbarConfigLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        apiHost: (apiHostResolution: { host: string; source: ApiHostSource }) => string
        ...
        isTrustedUiHost: (uiHost: string) => boolean
    }
}

export type toolbarConfigLogicType = MakeLogicType<
    toolbarConfigLogicValues,
    toolbarConfigLogicActions,
    ToolbarProps,
    toolbarConfigLogicMeta
>
...
export const toolbarConfigLogic = kea<toolbarConfigLogicType>([
    ...
    selectors({
        insights: [(s) => [s.props], (props: ToolbarProps) => props.insights ?? null],
        // Insights app URL used for OAuth and navigation links.
        //
        ...
        uiHost: [
            ...
            (props: ToolbarProps): string => {
                ...
                const fromRouter = canonicalizeUiHost(
                    (props.insights as any)?.requestRouter?.uiHost as string | undefined
                )
                if (fromRouter) {
                    return fromRouter
                }

                // Fallback for old insights-js without requestRouter.
                const fromConfig = canonicalizeUiHost(props.insights?.config?.ui_host)
                if (fromConfig) {
                    return fromConfig
                }
        ...
        apiHostResolution: [
            ...
            (props: ToolbarProps): { host: string; source: ApiHostSource } => {
                const rawConfig = props.insights?.config?.api_host
                const fromConfig = canonicalizeApiHost(rawConfig)
                if (fromConfig) {
                    return { host: fromConfig, source: 'insights_api_host' }
                }
                if (rawConfig) {
                    toolbarLogger.warn('config', 'Invalid insights.config.api_host, rejected', {
                        api_host: rawConfig,
                    })
                }
        ...
        dataAttributes: [(s) => [s.props], (props: ToolbarProps): string[] => props.dataAttributes ?? []],
        isAuthenticated: [(s) => [s.accessToken], (accessToken: string | null) => !!accessToken],
        toolbarFlagsKey: [(s) => [s.props], (props: ToolbarProps): string | undefined => props.toolbarFlagsKey],
        // True when uiHost is a Insights Cloud host (us/eu) — safe to skip the
        // "are you sure you want to authenticate here?" confirmation.
        isTrustedUiHost: [(s) => [s.uiHost], (uiHost: string): boolean => isInsightsCloudHost(uiHost)],
    }),
    ...
    listeners(({ values, actions }) => ({
        authenticate: () => {
            ...
            if (values.authStatus === 'error') {
                toolbarInsightsJS.capture('toolbar ui host config modal opened', { ui_host: values.uiHost })
            ...
            // This prevents phishing via crafted #__insights= hash params with a
            // malicious uiHost — the user sees the target domain and can cancel.
            // Skip for Insights Cloud (us/eu) where the target is already trusted.
        ...
        confirmAuthenticate: async () => {
            ...
            try {
                ...
                toast.error('Failed to start authentication. Ensure you are on a secure (HTTPS) page.')
            ...
            // Strip __insights hash params before building the redirect URL.
            // insights-js reads these at load time but never cleans them from the URL.
            // Including them would cause a re-initialization loop after OAuth callback.
            const hash = window.location.hash
                .replace(/[#?&]__insights=[^&]*/g, '')
        ...
        persistConfig: () => {
            ...
            // Persist OAuth tokens separately so they survive insights-js overwriting LOCALSTORAGE_KEY
    ...
    afterMount(({ props, values, actions, cache }) => {
        ...
        //
        // Skip the HEAD when:
        // - uiHost is a trusted Insights Cloud host (always reachable, no value in
        ...
        //   self-hosted / SSO / reverse-proxy customers whose Insights app works
        //   fine for real API calls but CORS-rejects the cheap HEAD)
        if (isInsightsCloudHost(values.uiHost)) {
            if (authParams) {
 ...
 * - URLs with userinfo like `https://us.hanzo.ai@evil.com` — these display
 *   misleadingly in confirmation dialogs where a hurried user may skim for "us.hanzo.ai"
 *
...
/** Restore OAuth tokens from a separate localStorage key that survives insights-js overwrites. */
function restoreOAuthTokens(
    ...
    if (canonicalStoredUiHost && canonicalStoredUiHost !== values.uiHost) {
        // Stored tokens were issued for a different Insights app — an attacker may
        ...
        toolbarInsightsJS.capture('toolbar oauth tokens discarded', {
    ...
    // Accept them only when the current uiHost is a trusted Insights Cloud host —
    ...
    if (!canonicalStoredUiHost && !isInsightsCloudHost(values.uiHost)) {
...
/** Set up Insights instrumentation and capture the "toolbar loaded" event. */
function initInstrumentation(
    ...
    if (props.instrument) {
        ...
        if (props.distinctId) {
            toolbarInsightsJS.identify(props.distinctId, props.userEmail ? { email: props.userEmail } : {})
        }
    }

    const loadStart = (window as any).__insights_toolbar_load_start as number | undefined
    delete (window as any).__insights_toolbar_load_start
    const loadDurationMs = loadStart ? Math.round(performance.now() - loadStart) : undefined

    toolbarInsightsJS.capture('toolbar loaded', {
...
/**
 * Run a CORS HEAD check against the Insights app to verify uiHost is reachable.
...
function verifyUiHostReachability(
    ...
    const uiHostSource = (props.insights as any)?.requestRouter?.uiHost
        ? 'request_router'
        : props.insights?.config?.ui_host
          ? 'insights_config'
          : props.insights?.config?.api_host
            ? 'insights_api_host'
            : 'window_origin'
    ...
    void safeFetch(`${values.uiHost}/toolbar_oauth/check`, {
        ...
        .then((response) => {
            ...
            actions.setAuthStatus('idle')
            toolbarInsightsJS.capture('toolbar ui host check', {
                ...checkBaseProps,
                status: 'ok',
...
async function exchangeCodeForTokens(
    ...
    try {
        ...
        if (access) {
            toolbarInsightsJS.capture('toolbar oauth exchange', {
                status: 'success',
        ...
        toast.error('Authentication failed due to a network error. Please try again.')
        return false
    } finally {
        actions.setAuthStatus('idle')
    }
