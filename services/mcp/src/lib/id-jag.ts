...
// (`insights.api.id_jag`) are issued with this header so the resource server
...
// Cheap header-only inspection — signature verification is the Insights API's
// job (`IDJagAccessTokenAuthentication` in `insights/auth.py`). We just need
