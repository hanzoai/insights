...
const HJH8YSXW_MARKDOWN = [
    ...
    '<Tag data={[1,2,3]} />',
    '',
    '<Query noViewquery={{"kind":"DataTableNode","source":{"kind":"InsightsQLQuery","query":"select event, count() from events group by event"}}} isDefaultFilterApplied />',
    '',
    'asdasdas3',
    '',
    'rere',
...
Paragraph with **bold**, *italic*, <u>underline</u>, \`code\`, and [link](https://hanzo.ai).
  ...
  1. Child`)
    ...
    it('strips unsafe protocols from markdown links', () => {
        const document = parseMarkdownNotebook(
            'Safe [link](https://hanzo.ai), unsafe [link](javascript:alert), relative [link](/project), and mail [link](mailto:test@example.com).'
        )
        const node = document.nodes[0]

        expect(serializeMarkdownNotebook(document)).toEqual(
            'Safe [link](https://hanzo.ai), unsafe link, relative link, and mail link.'
        )
        ...
        expect(inlineNodesToHtml(node.children)).toEqual(
            'Safe <a href="https://hanzo.ai">link</a>, unsafe link, relative link, and mail link.'
        )
    })
    ...
    it('round-trips component string props with reversible escaping', () => {
        const props = {
            src: 'https://hanzo.ai/embed?one=1&two=2',
    ...
    it('decodes legacy HTML entities in component string props', () => {
        const markdown =
            '<Embed src="https://hanzo.ai/embed?one=1&amp;two=2" title="A &quot;quoted&quot; &lt;embed&gt; &amp; title" />'
        const expectedMarkdown = `<Embed src=${JSON.stringify('https://hanzo.ai/embed?one=1&two=2')} title=${JSON.stringify('A "quoted" <embed> & title')} />`
        const document = parseMarkdownNotebook(markdown)
...
7. Third`
    ...
    it('opens links on modifier-click while editing but not on plain click', () => {
        ...
        const { container } = render(
            createElement(MarkdownNotebook, { value: withNotebookTitle('See [docs](https://hanzo.ai/docs)') })
        )
        ...
        expect(windowOpen).toHaveBeenCalledWith('https://hanzo.ai/docs', '_blank', 'noopener')

        // View mode keeps native navigation: the handler must not add a second open
        windowOpen.mockClear()
        const { container: viewContainer } = render(
            createElement(MarkdownNotebook, {
                value: withNotebookTitle('See [docs](https://hanzo.ai/docs)'),
                mode: 'view',
            })
        )
    ...
    it('opens the link editor automatically when the selection is inside a link, without stealing focus', () => {
        const { container } = render(
            createElement(MarkdownNotebook, { value: withNotebookTitle('See [docs](https://hanzo.ai/docs) here') })
        )
        ...
        expect(input.value).toEqual('https://hanzo.ai/docs')
    ...
    it('applies the link and cancels the keystroke when pressing Enter in the link editor', () => {
        ...
        const input = container.querySelector('[aria-label="Link URL"]') as HTMLInputElement
        fireEvent.change(input, { target: { value: 'https://hanzo.ai' } })
        ...
        expect(onChange).toHaveBeenLastCalledWith(withNotebookTitle('[Hello](https://hanzo.ai) world out there'))
    })
...
`)
    ...
    it('strips unsafe protocols from pasted HTML links', () => {
        const element = document.createElement('div')
        element.innerHTML =
            'Safe <a href="https://hanzo.ai">link</a> unsafe <a href="javascript:alert">link</a> relative <a href="/project">link</a>'
        const inlineNodes = htmlElementToInlineNodes(element)

        expect(serializeInlineNodes(inlineNodes)).toEqual('Safe [link](https://hanzo.ai) unsafe link relative link')
        expect(inlineNodesToHtml(inlineNodes)).toEqual(
            'Safe <a href="https://hanzo.ai">link</a> unsafe link relative link'
        )
    })
...
<Embed hideFilters src="https://hanzo.ai" title="Insights" />`
...
<Embed src="https://example.com" />`
    ...
    it('does not remount a newly inserted component when the matching remote save arrives', () => {
        ...
        const registry = createMarkdownNotebookRegistry([
            {
                ...
                label: 'Embed',
                category: 'Media',
                defaultProps: { src: 'https://hanzo.ai', title: 'Insights' },
                ViewComponent: () => {
                    renderComponent()
                    useEffect(() => {
                        mountComponent()
        ...
        expect(savedMarkdown).toEqual(
            `${TEST_NOTEBOOK_TITLE_MARKDOWN}\n\n<Embed src="https://hanzo.ai" title="Insights" />`
        )
...
${queryMarkdown}`)
    ...
    it('clears the slash command query with Cmd+A then Backspace', () => {
        ...
        expect(lineInsertMenuButton?.classList.contains('Button--active')).toBe(true)
        expect(onChange).toHaveBeenLastCalledWith(`${TEST_NOTEBOOK_TITLE_MARKDOWN}\n\n `)
    })
...
> Quote outro`)
    ...
    it('shows both the heading and blockquote buttons active for a quoted heading selection', () => {
        ...
        expect(getFormattingStyleButton(container, 'Heading 2').classList.contains('Button--active')).toBe(true)
        expect(getFormattingStyleButton(container, 'Blockquote').classList.contains('Button--active')).toBe(true)
    })
...
- **Second** item`)
    ...
    it('shows the shared block style for same-style text row selections', () => {
        ...
        expect(getFormattingStyleButton(container, 'Text').classList.contains('Button--active')).toBe(true)
    })
...
\`\`\``)
    ...
    it('converts the active block style back to text when clicked again', () => {
        ...
        expect(quoteButton.classList.contains('Button--active')).toBe(true)
    ...
    it('shows only block style controls for selected code block text', () => {
        ...
        expect(container.querySelector('.MarkdownNotebook__format-toolbar')).toBeInstanceOf(HTMLElement)
        expect(getFormattingStyleButton(container, 'Code').classList.contains('Button--active')).toBe(true)
    ...
    it('adds a link from the formatting toolbar', () => {
        const onChange = jest.fn()
        const { container } = render(createElement(MarkdownNotebook, { value: 'Insights docs', onChange }))
        const textBlock = container.querySelector(NOTEBOOK_TEST_EDITABLE_SELECTOR) as HTMLElement
        ...
        fireEvent.change(linkInput, { target: { value: 'https://hanzo.ai/docs' } })
        fireEvent.keyDown(linkInput, { key: 'Enter' })

        expect(textBlock.querySelector('a')?.getAttribute('href')).toEqual('https://hanzo.ai/docs')
        expect(onChange).toHaveBeenLastCalledWith('# [Insights](https://hanzo.ai/docs) docs')
    })

    it('edits an existing link from the formatting toolbar', () => {
        const onChange = jest.fn()
        const { container } = render(
            createElement(MarkdownNotebook, { value: '[Insights](https://hanzo.ai) docs', onChange })
        )
        const textBlock = container.querySelector(NOTEBOOK_TEST_EDITABLE_SELECTOR) as HTMLElement
        ...
        expect(linkInput.value).toEqual('https://hanzo.ai')
        fireEvent.change(linkInput, { target: { value: 'https://hanzo.ai/docs' } })
        fireEvent.click(
    ...
    it('does not open the link editor for a collapsed selection inside an existing link', () => {
        const { container } = render(createElement(MarkdownNotebook, { value: '[Insights](https://hanzo.ai) docs' }))
        const textBlock = container.querySelector(NOTEBOOK_TEST_EDITABLE_SELECTOR) as HTMLElement
        const linkedTextNode = textBlock.querySelector('a')?.firstChild
    ...
    it('keeps the formatting toolbar available after adding or removing a link on the row', () => {
        const { container } = render(createElement(MarkdownNotebook, { value: 'Insights docs' }))
        const textBlock = container.querySelector(NOTEBOOK_TEST_EDITABLE_SELECTOR) as HTMLElement
...
Tail with **bold** text`)
    ...
    it('pastes a URL over selected text as a link', () => {
        ...
        pastePlainText(textBlock, 'https://hanzo.ai/docs')

        expect(textBlock.querySelector('a')?.getAttribute('href')).toEqual('https://hanzo.ai/docs')
        expect(textBlock.textContent).toEqual('Insights docs')
        expect(onChange).toHaveBeenLastCalledWith('# [Insights](https://hanzo.ai/docs) docs')
    })
    ...
    it('pastes a URL over selected list item text as a link', () => {
        const onChange = jest.fn()
        const { container } = render(
            createElement(MarkdownNotebook, { value: withNotebookTitle('- Insights docs'), onChange })
        )
        ...
        pastePlainText(listItem, 'https://hanzo.ai/docs')

        expect(listItem.querySelector('a')?.getAttribute('href')).toEqual('https://hanzo.ai/docs')
        expect(listItem.textContent).toEqual('Insights docs')
        expect(onChange).toHaveBeenLastCalledWith(
            `${TEST_NOTEBOOK_TITLE_MARKDOWN}\n\n- [Insights](https://hanzo.ai/docs) docs`
        )
    })
...
| Insights | 12 |`),
        ...
        pastePlainText(cells[2], 'https://hanzo.ai/docs')

        expect(cells[2].querySelector('a')?.getAttribute('href')).toEqual('https://hanzo.ai/docs')
        expect(cells[2].textContent).toEqual('Insights')
        expect(onChange).toHaveBeenLastCalledWith(`${TEST_NOTEBOOK_TITLE_MARKDOWN}

| Name | Count |
| --- | --- |
| [Insights](https://hanzo.ai/docs) | 12 |`)
    })
...
<Query hideResults query={{"kind":"DataTableNode","source":{"kind":"EventsQuery"}}} />`,
    ...
    it('does not suggest the query body or schema kinds as the title placeholder', () => {
        ...
        rerender(
            createElement(MarkdownNotebook, {
                value: '<Query query={{"kind":"DataTableNode","source":{"kind":"InsightsQLQuery","query":"select event from events"}}} />',
            })
        )
    ...
    it('shows embed title before url in the filters panel', () => {
        const { container } = render(
            createElement(MarkdownNotebook, {
                value: '<Embed hideResults src="https://hanzo.ai/docs" title="Insights docs" />',
            })
        )
        ...
        expect(inputs[0].value).toEqual('Insights docs')
        expect(inputs[1].placeholder).toEqual('https://example.com/embed')
        expect(inputs[1].value).toEqual('https://hanzo.ai/docs')
    })
    ...
    it('shows SQL component titles as colored tags with icons', () => {
        const markdown = `<Query query={{"kind":"DataTableNode","source":{"kind":"InsightsQLQuery","query":"select event from events"}}} />`
        const { container } = render(createElement(MarkdownNotebook, { value: markdown }))
