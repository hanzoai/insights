 ...
 * 2. Queueing handlers (`fetch`, `sendEmail`, `insightsGetTicket`, `insightsUpdateTicket`) stage
...
registerAsyncFunction('foobar', {
     ...
     */
    execute: async (args, _context, result) => {
        const [foo, bar, baz] = args as [string | undefined, Record<'key', number> | undefined, string | undefined]

        // Do something with the argument - this can be a fetch request, a database query, or any other asynchronous operation
        await new Promise((resolve) => setTimeout(resolve, bar?.['key'] || 100))
    ...
    mock: (args, logs) => {
        ...
        logs.push({
            level: 'info',
            timestamp: DateTime.now(),
            message: `foobar('${args[0]}', ${JSON.stringify(args[1], null, 2)}, '${args[2]}')`,
        })
