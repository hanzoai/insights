...
from products.stamphog.backend.temporal.constants import (
    ...
    STAMPFN_POLICY_PATHS,
    STAMPFN_REVIEW_GUIDANCE_PATH,
    STAMPFN_REVIEWFN_LABEL,
    STAMPFN_SANDBOX_CONTEXT_PATH,
    STAMPFN_SANDBOX_ENGINE_DIR,
    STAMPFN_SANDBOX_OWNERS_DIR,
    STAMPFN_SANDBOX_REPO_DIR,
    STAMPFN_TRUSTED_REACTOR_BOTS,
)
...
# basename of each STAMPFN_POLICY_PATHS entry (policy.yml / review-guidance.md): a repo with no
...
def _reviewer_environment(run: ReviewRun) -> dict[str, str]:
    ...
    env = {
        "STAMPFN_REPO_DIR": STAMPFN_SANDBOX_REPO_DIR,
...
def _sandbox_egress_allowlist() -> list[str]:
    ...
    engine's pinned deps, the gateway for LLM calls, and the Insights capture host for telemetry.
    STAMPFN_SANDBOX_EXTRA_EGRESS_DOMAINS is the ops escape hatch for a missing legitimate host.
    ...
    for url_env in ("AI_GATEWAY_URL", "POSTFN_HOST"):
        ...
        if host:
            domains.append(host)
    domains.extend(settings.STAMPFN_SANDBOX_EXTRA_EGRESS_DOMAINS)
...
def fetch_review_context(input: StamphogReviewInput) -> dict:
    ...
    for path in (*STAMPFN_POLICY_PATHS, *STAMPFN_OPTIONAL_POLICY_PATHS):
    ...
    in_flight = sorted(
        {
            ...
            and (login := (reaction.get("user") or "").lower()) in STAMPFN_TRUSTED_REACTOR_BOTS
            and login != own_login
            and (created := parse_datetime(reaction.get("created_at") or "")) is not None
            and (now - created).total_seconds() <= STAMPFN_BOT_EYES_MAX_AGE_SECONDS
        }
    )
    if in_flight:
...
def run_review_in_sandbox(input: StamphogReviewInput) -> dict:
    ...
    invocation = build_reviewer_invocation(
        ...
        repo=repo,
        engine_dir=STAMPFN_SANDBOX_ENGINE_DIR,
        context_path=STAMPFN_SANDBOX_CONTEXT_PATH,
    ...
    try:
        ...
        command = f"cd {shlex.quote(STAMPFN_SANDBOX_REPO_DIR)} && {_harden_reviewer_command(invocation.command)}"
...
def post_verdict(input: StamphogReviewInput) -> dict:
    ...
    if is_refusal:
        try:
            client.add_pr_label(repo, pull_request.pr_number, STAMPFN_REVIEWFN_LABEL)
        except Exception:
...
def mark_review_failed(input: MarkReviewFailedInput) -> None:
    ...
    # breakage next to the review-completed events. ph_scoped_capture, not hanzo_insights.capture —
...
def _clone_pr(sandbox: SandboxBase, repo: str, base_sha: str, head_sha: str, pr_number: int, token: str) -> None:
    ...
    clone = (
        f"rm -rf {shlex.quote(STAMPFN_SANDBOX_REPO_DIR)} && "
        f"{auth} clone --single-branch {shlex.quote(repo_url)} {shlex.quote(STAMPFN_SANDBOX_REPO_DIR)}"
    )
    ...
    checkout = f"cd {shlex.quote(STAMPFN_SANDBOX_REPO_DIR)} && {fetch_specs} && git checkout {shlex.quote(head_sha)}"
...
def _effective_policy_files(repo: str, policy_files: dict[str, str]) -> dict[str, str]:
    ...
    """
    defaulted = [path for path in STAMPFN_POLICY_PATHS if path not in policy_files]
    if defaulted:
        activity.logger.info(f"Policy files for {repo} using hosted defaults: {defaulted}")

    effective = {
        STAMPFN_POLICY_ENTRYPOINT: _overlay_policy_yaml(
            repo,
            _read_default_policy_file(STAMPFN_POLICY_ENTRYPOINT),
            policy_files.get(STAMPFN_POLICY_ENTRYPOINT),
        ),
        STAMPFN_REVIEW_GUIDANCE_PATH: policy_files.get(STAMPFN_REVIEW_GUIDANCE_PATH)
        or _read_default_policy_file(STAMPFN_REVIEW_GUIDANCE_PATH),
    }
    for path in STAMPFN_OPTIONAL_POLICY_PATHS:
...
def _inject_policy_files(sandbox: SandboxBase, policy_files: dict[str, str]) -> None:
    ...
    """
    for path in (*STAMPFN_POLICY_PATHS, *STAMPFN_OPTIONAL_POLICY_PATHS):
        sandbox.execute(f"rm -f {shlex.quote(f'{STAMPFN_SANDBOX_REPO_DIR}/{path}')}", timeout_seconds=30)
    for path, content in policy_files.items():
        _write_sandbox_file(sandbox, f"{STAMPFN_SANDBOX_REPO_DIR}/{path}", content)
...
def _ship_engine(sandbox: SandboxBase) -> None:
    ...
    engine_dir = shlex.quote(STAMPFN_SANDBOX_ENGINE_DIR)
    sandbox.execute(f"rm -rf {engine_dir} && mkdir -p {engine_dir}", timeout_seconds=30)
    for name, content in files.items():
        sandbox.write_file(f"{STAMPFN_SANDBOX_ENGINE_DIR}/{name}", content.encode())
    _ship_owners_package(sandbox)


def _ship_owners_package(sandbox: SandboxBase) -> None:
    """Ship the insights-owners resolver package the engine's ownership format imports.

    The default policy declares a ``insightscli-resolver`` ownership source, and gates.py imports
    ``insights_owners`` from ``tools/owners`` next to the engine dir. Same trust posture as the
    ...
    """
    package_dir = _SERVER_ENGINE_DIR.parent / "owners" / "insights_owners"
    if not package_dir.is_dir():
        raise RuntimeError(f"owners package source dir not found: {package_dir}")
    target = f"{STAMPFN_SANDBOX_OWNERS_DIR}/insights_owners"
    quoted = shlex.quote(STAMPFN_SANDBOX_OWNERS_DIR)
