...
handles unwrapping the ``insights:exec`` cli wrapper the MCP server serves.
"""
...
from products.insights_ai.eval_harness.log_parser import LogParser, ToolCall
...
class IssuesListToolUsed(_ToolUsedScorer):
    ...
    SQL/InsightsQL workaround instead of the typed tool.
    """
...
class IssuesListInputAlignment(_AlignmentClassifier):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are comparing two argument shapes for the Insights `query-error-tracking-issues-list` MCP tool.
...
class IssueInputAlignment(_AlignmentClassifier):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are comparing the ACTUAL arguments an agent passed to Insights's `query-error-tracking-issue` MCP tool against the EXPECTED shape, given the USER_PROMPT.
...
class EventsArgsAlignment(_AlignmentClassifier):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            prompt_template="""
You are comparing the ACTUAL arguments an agent passed to Insights's `query-error-tracking-issue-events` MCP tool against the EXPECTED shape, given the USER_PROMPT. ACTUAL may contain multiple successful calls; every call must be consistent with EXPECTED.
