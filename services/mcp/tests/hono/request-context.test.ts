...
describe('RequestContext', () => {
    describe('ApiClient construction', () => {
        ...
        it('passes POSTFN_PUBLIC_URL as publicBaseUrl to the ApiClient', async () => {
            ...
            expect(config.baseUrl).toBe('http://insights-web-django.insights.svc.cluster.local:8000')
        ...
        it('falls back to POSTFN_API_BASE_URL when POSTFN_PUBLIC_URL is not set', async () => {
            ...
            const config = mockApiClientCtor.mock.calls[0]![0]
            expect(config.baseUrl).toBe('https://us.hanzo.ai')
            expect(config.publicBaseUrl).toBe('https://us.hanzo.ai')
        })
    ...
    describe('buildClientProperties', () => {
        it('includes all request properties', () => {
            ...
            expect(result).toMatchObject({
                $ai_product: 'mcp',
                $mcp_source: 'insights_mcp_analytics',
                $mcp_server_name: 'Insights',
