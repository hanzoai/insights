...
def test_redis_url_with_default_uses_settings():
    config = RedisConfig()
    with patch("insights.temporal.export_recording.activities.settings") as mock_settings:
...
async def test_build_recording_export_context_success():
    ...
    with (
        patch("insights.temporal.export_recording.activities.ExportedRecording.objects", mock_qs),
        patch("insights.temporal.export_recording.activities.database_sync_to_async") as mock_db_sync,
    ):
...
async def test_export_replay_datastore_rows_success():
    ...
    with (
        patch("insights.temporal.export_recording.activities.get_client") as mock_get_client,
        patch("insights.temporal.export_recording.activities.SessionReplayEvents.get_metadata_query") as mock_query,
        patch("insights.temporal.export_recording.activities.get_async_client") as mock_get_async_client,
    ):
...
async def test_export_recording_data_prefix_success():
    ...
    with (
        patch("insights.temporal.export_recording.activities.SessionRecording", return_value=mock_recording),
        patch("insights.temporal.export_recording.activities.database_sync_to_async") as mock_db_sync,
        patch("insights.temporal.export_recording.activities.list_blocks") as mock_list_blocks,
        patch("insights.temporal.export_recording.activities.get_async_client") as mock_get_async_client,
    ):
        mock_db_sync.side_effect = lambda fn: AsyncMock(return_value=fn())
...
async def test_export_recording_data_success():
    ...
    with (
        ...
        patch("insights.temporal.export_recording.activities.list_blocks") as mock_list_blocks,
        patch(
            "insights.temporal.export_recording.activities.session_recording_v2_object_storage.async_client"
...
async def test_store_export_data_success(tmp_path):
    ...
    with (
        patch("insights.temporal.export_recording.activities.get_async_client") as mock_get_async_client,
        patch("insights.temporal.export_recording.activities.Path") as mock_path_cls,
        patch("insights.temporal.export_recording.activities.shutil.make_archive") as mock_make_archive,
        patch("insights.temporal.export_recording.activities.shutil.rmtree"),
        patch(
            "insights.temporal.export_recording.activities.session_recording_v2_object_storage.async_client"
        ) as mock_storage_client,
        patch("insights.temporal.export_recording.activities.ExportedRecording.objects") as mock_record_qs,
        patch("insights.temporal.export_recording.activities.database_sync_to_async") as mock_db_sync,
    ):
        mock_get_async_client.return_value = mock_redis
...
async def test_store_export_data_s3_upload_failure():
    ...
    with (
        patch("insights.temporal.export_recording.activities.get_async_client") as mock_get_async_client,
        patch("insights.temporal.export_recording.activities.Path") as mock_path_cls,
        patch("insights.temporal.export_recording.activities.shutil.make_archive"),
        patch(
            "insights.temporal.export_recording.activities.session_recording_v2_object_storage.async_client"
        ) as mock_storage_client,
    ):
...
async def test_cleanup_export_data_success():
    ...
    with patch("insights.temporal.export_recording.activities.get_async_client") as mock_get_async_client:
        mock_get_async_client.return_value = mock_redis
