...
/**
 * One detail-pane action, rendered either inline as a `Button` (wide layouts) or as a
 * `Menu` item (the "…" overflow on narrow layouts). Keeping actions as data lets both
...
export function useReportDetailActions(report: SignalReport): ReportDetailAction[] {
    ...
    const onRestoreClick = async (): Promise<void> => {
        ...
        try {
            ...
            toast.error(error?.detail || error?.message || 'Failed to restore report')
        } finally {
