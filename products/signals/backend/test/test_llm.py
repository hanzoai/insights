...
async def test_gateway_mode_omits_legacy_stage_header():
    ...
    # In gateway mode the labels ride on the builder's X-Insights-Properties blob; the per-key
...
async def test_fallback_mode_sends_legacy_stage_header():
    ...
    assert client.messages.create.call_args.kwargs["extra_headers"] == {"x-insights-property-ai_stage": "match"}
