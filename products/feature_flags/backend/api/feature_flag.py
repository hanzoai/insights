...
from insights.schema import ProductKey, PropertyOperator

from insights.insightsql.property import parse_semver

from insights.api.cohort import CohortSerializer
from insights.api.documentation import FeatureFlagFiltersSchemaSerializer, extend_schema
...
from insights.api.services.flags_service import RETRYABLE_FLAGS_SERVICE_EXCEPTIONS, get_flags_from_service
...
from insights.api.utils import ClassicBehaviorBooleanFieldSerializer, ErrorResponseSerializer, action
...
from insights.models.activity_logging.activity_log import Detail, load_activity, log_activity
from insights.models.activity_logging.activity_page import ActivityLogPaginatedResponseSerializer, activity_page_response
from insights.models.activity_logging.model_activity import ImpersonatedContext
from insights.models.person.point_in_time_properties import (
    build_person_properties_at_time,
    get_person_and_distinct_ids_for_identifier,
)
from insights.models.property import Property
from insights.models.property.property import STRING_PREFIX_SUFFIX_OPERATORS
from insights.permissions import TeamSecretTokenPermission, get_authenticator_scopes, is_service_auth
from insights.ph_client import feature_enabled_or_false
from insights.queries.base import determine_parsed_date_for_property_matching
...
from insights.settings.feature_flags import REMOTE_CONFIG_RATE_LIMITS
from insights.utils import is_valid_regex, str_to_bool
from insights.views import format_bytes
...
# results in a disabled logger because `insights.api.feature_flag` is loaded during
# Django startup. Remove this logger together with the helper once the scope is enforced.
scope_audit_logger = structlog.get_logger("insights.feature_flag_scope_audit")
...
class EvaluationContextSerializerMixin(serializers.Serializer):
    ...
    def _log_evaluation_context_change(self, obj: FeatureFlag, before: list[str], after: list[str]) -> None:
        from insights.models.activity_logging.activity_log import Change, Detail
...
class FeatureFlagSerializer(
    ...
    def get_can_edit(self, feature_flag: FeatureFlag) -> bool:
        ...
        from insights.rbac.user_access_control import AccessControlLevel, access_level_satisfied_for_resource
    ...
    def _validate_flag_reference(self, flag_reference):
        ...
        from insights.utils import safe_int
    ...
    def to_representation(self, instance):
        ...
        if group_key_props:
            ...
            if group_type_indices:
                ...
                if group_keys:
                    from insights.models.group.util import get_groups_by_type_indices
...
def _create_usage_dashboard(feature_flag: FeatureFlag, user):
    from insights.helpers.dashboard_templates import create_feature_flag_dashboard
...
def _update_feature_flag_dashboard(feature_flag: FeatureFlag, old_key: str) -> None:
    from insights.helpers.dashboard_templates import update_feature_flag_dashboard
...
#   pytest insights/models/feature_flag/test/test_flags_cache.py -k "test_serializer_output_matches_fixture_schema"
...
# Datastore cost attribution: this viewset currently has no direct Datastore calls —
# all Datastore work is delegated to helpers (user_blast_radius.py, flag_analytics.py)
# that already tag their queries. If you add a new Datastore query reachable from an
...
# See insights/models/feature_flag/user_blast_radius.py for the pattern.
...
class FeatureFlagViewSet(
    ...
    def matching_ids(self, request: request.Request, **kwargs):
        ...
        """
        from insights.rbac.user_access_control import access_level_satisfied_for_resource
    ...
    def bulk_delete(self, request: request.Request, **kwargs):
        ...
        from insights.models.activity_logging.activity_log import LogActivityEntry, bulk_log_activity
        from insights.rbac.user_access_control import access_level_satisfied_for_resource
        from insights.tasks.remote_config import update_team_remote_config
    ...
    def evaluation_reasons(self, request: request.Request, **kwargs):
        ...
        # Insights UI debug endpoint, not customer SDK traffic. Pass the internal
    ...
    def test_evaluation(self, request: request.Request, **kwargs):
        ...
        # personinsights RPC outage, a genuinely missing person, and bad input can be told apart
        ...
        try:
            ...
            # personinsights is unreachable/erroring. This is a transient dependency failure, not a
            # missing person or bad input, so surface a distinct retryable 503 rather than a 500.
            logger.exception("personinsights RPC failed resolving person for flag test evaluation", extra=log_context)
            capture_exception(e)
            return Response(
        ...
        if timestamp:
            try:
                # Tag the Datastore call so query_log attribution stays correct.
                ...
                # Datastore / infra failures keep the generic 500 but log the real cause.
    ...
    def _remote_config_response(self, request: request.Request, **kwargs) -> Response:
        ...
        try:
            ...
            # SDKs can't parse as JSON. The body mirrors the drf-exceptions-script envelope the Rust
