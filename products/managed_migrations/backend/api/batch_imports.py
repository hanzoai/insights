...
class BatchImportS3SourceCreateSerializer(BatchImportTrialOptionsMixin, BatchImportSerializer):
    ...
    role_arn = serializers.RegexField(
        ...
        allow_blank=True,
        help_text=(
            "ARN of an IAM role in your AWS account that trusts Insights's import role. "
    ...
    def _validate_role_access(self, data: dict) -> None:
        ...
        try:
            ...
            import_role = sts.assume_role(
                ...
                RoleSessionName="insights-managed-migration-validation",
                DurationSeconds=900,
            )["Credentials"]
        except (ClientError, BotoCoreError) as err:
            # Insights-side failure (our role grant or STS), not the customer's setup: skip
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Insights could not assume this IAM role. Verify the role exists, its trust policy "
                "allows Insights's import role, and the External ID matches the one shown in setup."
            )
...
class BatchImportAWSIAMSetupSerializer(serializers.Serializer):
    ...
    available = serializers.BooleanField(
        help_text="Whether IAM role authentication is available on this Insights deployment."
    )
    external_id = serializers.CharField(
        help_text="External ID to pin in the role trust policy's sts:ExternalId condition. Stable per project."
    )
    insights_role_arn = serializers.CharField(
        allow_blank=True, help_text="ARN of Insights's import role -- the principal your role must trust."
    )
...
class BatchImportViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "Insights's import role ARN, and ready-to-paste trust/permission policy JSON."
        ),
    )
    @action(methods=["GET"], detail=False)
    def aws_iam_setup(self, request: Request, **kwargs) -> Response:
        external_id = get_aws_external_id(self.team)
        insights_role_arn = settings.MANAGED_MIGRATIONS_IMPORT_ROLE_ARN

        if not insights_role_arn or not self._is_iam_role_enabled_for_team():
        ...
        trust_policy = json.dumps(
            {
                ...
                "Statement": [
                    {
                        "Effect": "Allow",
                        "Principal": {"AWS": insights_role_arn},
        ...
        serializer = BatchImportAWSIAMSetupSerializer(
            {
                "available": True,
                "external_id": external_id,
                "insights_role_arn": insights_role_arn,
