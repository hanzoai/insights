...
describe('Script Executor', () => {
    ...
    describe('getSensitiveValues', () => {
        it('masks the nested secrets of every integration in an integration_multi input', () => {
            ...
            const values = executor.getSensitiveValues(insightsFunction, inputs)
    ...
    describe('general event processing', () => {
        ...
        it('can handle selecting entire object', async () => {
            ...
            expect(result.invocation.queueParameters).toMatchInlineSnapshot(`
                {
                  "body": "{"event":{"uuid":"uuid","event":"test","elements_chain":"","distinct_id":"distinct_id","url":"http://localhost:8000/events/1","properties":{"$lib_version":"1.2.3"},"timestamp":"2024-06-07T12:00:00.000Z"},"groups":{},"nested":{"foo":"http://localhost:8000/events/1"},"person":{"id":"uuid","name":"test","url":"http://localhost:8000/persons/1","properties":{"email":"test@hanzo.ai","first_name":"Pumpkin"}},"event_url":"http://localhost:8000/events/1-test"}",
                  "headers": {
                    "email": "test@hanzo.ai",
                    "first_name": "Pumpkin",
                  },
        ...
        it('queues up an async function call', async () => {
            ...
            expect(body).toEqual({
                ...
                person: {
                    ...
                    properties: { email: 'test@hanzo.ai', first_name: 'Pumpkin' },
                },
        ...
        it('merges previousResult into execute() result', async () => {
            ...
            expect(result.capturedInsightsEvents).toEqual(previousResult.capturedInsightsEvents)
        ...
        it('rebuilds mapping inputs when an invocation arrives without inputs (rerun path)', async () => {
            ...
            const invocation = createExampleInvocation(mappingFn, {
                person: {
                    ...
                    name: 'test',
                    url: 'http://localhost:8000/persons/1',
                    properties: { email: 'test@hanzo.ai', $initial_gclid: 'INITIAL_TOKEN_ABC' },
                },
            })
        ...
        it('limits the execution time and exits appropriately', async () => {
            ...
            expect(result.logs.map((log) => log.message)).toEqual([
                ...
                expect.stringContaining(
                    'Error executing function on event uuid: ScriptVMException: Execution timed out after 0.55 seconds. Performed'
                ),
            ])
        })
    })
    ...
    describe('insightsCaptue', () => {
        it('captures events', async () => {
            ...
            const result = await executor.execute(createExampleInvocation(fn))
            expect(result?.capturedInsightsEvents).toMatchInlineSnapshot(`
                [
                  {
                    "distinct_id": "distinct_id",
                    "event": "test (copy)",
        ...
        it('falls back to person.id for distinct_id when event.distinct_id is empty (batch invocations)', async () => {
            ...
            const globals = createHogExecutionGlobals({
                ...
                person: {
                    ...
                    name: 'Batch Person',
                    url: 'http://localhost:8000/persons/1',
                    properties: { email: 'batch@hanzo.ai' },
                },
            } as any)
            ...
            expect(result?.capturedInsightsEvents[0].distinct_id).toBe('person-uuid-123')
        })
        ...
        it('ignores events that have already used their insightsCapture 10 times', async () => {
            ...
            const result = await executor.execute(createExampleInvocation(fn, globals))
            expect(result?.capturedInsightsEvents).toEqual([])
    ...
    describe('insightsGetTicket and insightsUpdateTicket', () => {
        ...
        it('insightsGetTicket queues internal fetch with correct params', async () => {
        ...
        it('insightsUpdateTicket queues internal fetch with correct params', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
        ...
        it('insightsGetTicket errors when ticket_id is missing', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
        ...
        it('insightsUpdateTicket errors when ticket_id is missing', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
    ...
    describe('insightsGetAccount', () => {
        ...
        it('insightsGetAccount queues internal fetch with the external_id query param', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
        ...
        it('insightsGetAccount errors when external_id is missing', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
        ...
        it('insightsGetAccount errors when the team has no secret API token', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
        ...
        it('insightsUpdateAccount queues a PATCH with external_id merged into the body', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
        ...
        it('insightsUpdateAccount errors when external_id is missing', async () => {
            jest.spyOn(hub.teamManager, 'getTeam').mockResolvedValue({
                id: 1,
                secret_api_token: 'test-secret-token',
            } as any)
    ...
    describe('produceToWarehouseWebhooks', () => {
        ...
        // "Invalid InsightsQL bytecode, stack is empty, can not pop".
    ...
    describe('executeFetch', () => {
        ...
        it('completes successful fetch', async () => {
            ...
            expect(result.finished).toBe(false)
            expect(result.error).toBeUndefined()
            expect(result.invocation.queue).toBe('script')
        ...
        describe('aws_sigv4', () => {
            // `secret: true` InsightsFunction inputs land in `encrypted_inputs` after
            ...
            const seedAwsCredentialInputs = (invocation: CyclotronJobInvocationInsightsFunction) => {
                invocation.insightsFunction.encrypted_inputs = {
                    ...(invocation.insightsFunction.encrypted_inputs ?? {}),
            ...
            it('produces a fresh signature on retry instead of reusing the original', async () => {
                ...
                // in Script" path this would push the signature past AWS's 5-minute
        ...
        describe('with non_failure_status_codes', () => {
            ...
            const setNonFailureConfig = (
                ...
                invocation.insightsFunction.inputs_schema = [
                    ...(invocation.insightsFunction.inputs_schema ?? []),
                    {
        ...
        describe('self-loop guard', () => {
            ...
            const setSelfLoopDepth = (invocation: CyclotronJobInvocationInsightsFunction, depth: number): void => {
                invocation.state.globals.event.properties = {
                    ...invocation.state.globals.event.properties,
                    [SELF_LOOP_DEPTH_PROPERTY]: { [invocation.insightsFunction.id]: depth },
                }
            }
            ...
            const setOtherFunctionDepth = (invocation: CyclotronJobInvocationInsightsFunction, depth: number): void => {
                invocation.state.globals.event.properties = {
                    ...invocation.state.globals.event.properties,
            ...
            it.each([
                ...
                expect(result.error).toBeUndefined()
                expect(parseJSON(sent.getBody()!).properties[SELF_LOOP_DEPTH_PROPERTY][invocation.insightsFunction.id]).toBe(
                    stampedTo
                )
    ...
    describe('routeEmailToQueue', () => {
        ...
        it('should preserve the same job ID (no new job created)', () => {
            const insightsFunction = createInsightsFunction({ name: 'Email function' })
            const invocation: CyclotronJobInvocationInsightsFunction = {
                ...createExampleInvocation(insightsFunction),
                queueParameters: {
                    type: 'email',
    ...
    describe('email queue routing', () => {
        const createEmailInvocation = (): CyclotronJobInvocationInsightsFunction => {
            const insightsFunction = createInsightsFunction({ name: 'Email function', team_id: 123 })
            const invocation: CyclotronJobInvocationInsightsFunction = {
                ...createExampleInvocation(insightsFunction),
                teamId: 123,
                queueParameters: {
