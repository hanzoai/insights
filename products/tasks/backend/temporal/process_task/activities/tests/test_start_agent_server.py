...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("master", "insights-code/ci-test-break", "master"),
...
def test_resolve_protected_base_branch(mocker, pr_base, branch, expected) -> None:
    ...
    context = _context(github_integration_id=42, repository="Insights/insights", branch=branch)
...
def test_resolve_protected_base_falls_back_to_branch_on_error(mocker) -> None:
    ...
    context = _context(github_integration_id=42, repository="Insights/insights", branch="insights-code/fix")
    assert _resolve_protected_base_branch(context) == "insights-code/fix"
...
def test_ensure_repository_on_disk_passes_when_repo_present(mocker) -> None:
    ...
    _ensure_repository_on_disk(_context(repository="Insights/insights"), sandbox)

    # The precheck must probe the same path the clone writes to.
    assert sandbox_repo_path("Insights/insights") in sandbox.execute.call_args.args[0]
...
def test_ensure_every_repository_is_on_disk(mocker) -> None:
    ...
    _ensure_repository_on_disk(
        _context(
            ...
            state={"repositories": ["Insights/insights", "Insights/insights-js"]},
        ),
        sandbox,
    )

    assert [call.args[0] for call in sandbox.execute.call_args_list] == [
        f"test -d {sandbox_repo_path('Insights/insights')}",
        f"test -d {sandbox_repo_path('Insights/insights-js')}",
    ]
