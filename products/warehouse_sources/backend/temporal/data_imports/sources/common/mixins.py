...
def is_team_allowlisted_for_internal_hosts(team_id: int) -> bool:
    """Whether this team may point warehouse sources at Insights-internal hosts.
...
def resolve_safe_host(host: str, team_id: int | None) -> HostResolution:
    ...
    # Insights-managed DuckLake hosts resolve to internal IPs but are safe.
...
def _require_loopback(host: str) -> str:
    ...
    Datastore skips the egress proxy for tunneled connections on the strength of the tunnel
