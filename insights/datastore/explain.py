...
def find_all_reads(explain: dict) -> list[dict]:
    """
    Looks for a Plan/Subplan with "Indexes" element. Datastore' MergeTree engine uses "ReadFromMergeTree"
...
def execute_explain_get_index_use(datastore_sql: str, context: InsightsQLContext) -> QueryIndexUsage:
    # try:
    # nosemgrep: datastore-fstring-param-audit - datastore_sql is InsightsQL-compiled, not user input
    explain_results = sync_execute(
        f"EXPLAIN PLAN indexes=1,json=1 {datastore_sql}",
        context.values,
