        ...
        if request.POST.get("api_key"):
            ...
            if isinstance(data, dict):
                if data.get("$token"):
                    ...
                    token = data["api_key"]  # server-side libraries like insights-python and insights-ruby
...
# Queries that should be granted an extended Datastore timeout via LimitContext.QUERY_ASYNC.
...
def is_async_query(query: dict) -> bool:
    """Check if a query should be granted the extended Datastore timeout (LimitContext.QUERY_ASYNC).
