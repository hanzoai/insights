...
type InsightsConnectRegion = 'US' | 'EU'

const REGION_OPTIONS: { value: InsightsConnectRegion; label: string }[] = [
...
function InsightsConnectionRow({ integration }: { integration: IntegrationType }): JSX.Element {
        ...
        <div className="flex items-center gap-4 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                    <span className="font-semibold">{integration.display_name || 'Insights'}</span>
...
function ConnectModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }): JSX.Element {
    const [region, setRegion] = useState<InsightsConnectRegion>('EU')
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title="Connect a Insights project"
            description="You'll be sent to the target Insights to sign in and approve the access below."
            width={560}
            footer={
                <Button
                    type="primary"
        ...
        >
            <div className="flex flex-col gap-2 h-[60vh]">
                <div className="flex flex-col gap-1">
                    <label className="font-semibold">Insights region</label>
                    <Select<InsightsConnectRegion>
                        value={region}
...
export function PersonalInsightsConnections(): JSX.Element {
    ...
    // show this user's own connections. `kind` is compared as a string because `insights` lives in the
    // generated integration types, not the legacy manual IntegrationKind union.
    const connections = (integrations ?? []).filter(
        (i) => (i.kind as string) === 'insights' && i.created_by?.id === user?.id
    )
            ...
            <div className="divide-y rounded border bg-surface-primary">
                {connections.length === 0 ? (
                    <div className="px-4 py-6 text-center">
                        <p className="mb-1">No Insights projects connected yet</p>
                        <p className="text-muted text-sm mb-0">
                            Connect another Insights project to act in it through its API, for example to dispatch tasks
                            that must run there.
                        </p>
                    </div>
                ) : (
                    connections.map((integration) => (
                        <InsightsConnectionRow key={integration.id} integration={integration} />
                    ))
                )}
                <div className="p-2">
                    <Button type="secondary" icon={<IconPlus />} onClick={() => setModalOpen(true)}>
                        {connections.length === 0 ? 'Connect a Insights project' : 'Connect another project'}
                    </Button>
                </div>
            </div>
