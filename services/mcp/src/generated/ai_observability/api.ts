...
export const LlmAnalyticsPersonalSpendListQueryParams = /* @__PURE__ */ zod.object({
    ...
    date_from: zod
        ...
        .describe(
            'Start of the spend window. Accepts absolute dates (`2026-04-23`) or relative strings (`-7d`, `-1m`, etc.) — same parser used elsewhere in Insights. Defaults to `-30d`. The window between `date_from` and `date_to` cannot exceed 90 days.'
        ),
    date_to: zod
        .string()
    ...
    product: zod
        ...
        .describe(
            'Required `ai_product` key to scope the tool \/ model \/ trace breakdowns to a single product. Only the following products are currently supported: insights_code.'
        ),
    refresh: zod
        .boolean()
        .default(llmAnalyticsPersonalSpendListQueryRefreshDefault)
        .describe('If true, bypass the result cache and re-run the underlying queries against Datastore.'),
})
...
export const EvaluationRunsCreateBody = /* @__PURE__ */ zod.object({
    ...
    timestamp: zod.iso
        .datetime({ offset: true })
        .describe('ISO 8601 timestamp of the target event (needed for efficient Datastore lookup).'),
    event: zod
        .string()
...
export const EvaluationsListQueryParams = /* @__PURE__ */ zod.object({
    ...
    evaluation_type: zod
        .enum(['script', 'llm_judge', 'sentiment'])
        .optional()
        .describe(
...
export const EvaluationsCreateBody = /* @__PURE__ */ zod.object({
    ...
    evaluation_type: zod
        .enum(['llm_judge', 'script', 'sentiment'])
        .describe('\* `llm_judge` - LLM as a judge\n\* `script` - Script\n\* `sentiment` - Sentiment analysis')
        .describe(
            "'llm_judge' uses an LLM to score outputs against a prompt; 'script' runs deterministic Script code; 'sentiment' classifies user-message sentiment.\n\n\* `llm_judge` - LLM as a judge\n\* `script` - Script\n\* `sentiment` - Sentiment analysis"
        ),
    evaluation_config: zod
        .union([
            ...
            zod.object({
                source: zod
                    .string()
                    .min(1)
                    .describe('Script source code. Must return true (pass), false (fail), or null for N\/A.'),
            }),
            zod.object({
                source: zod
                    .enum(['user_messages'])
        ...
        .optional()
        .describe(
            "Configuration dict. For 'llm_judge': {prompt}; for 'script': {source}; for 'sentiment': {source: 'user_messages'}."
        ),
    output_type: zod
    ...
    model_configuration: zod
        ...
        .optional()
        .describe(
            'Provider and model for an llm_judge evaluation. Required when creating or switching to llm_judge. To add or replace a model, provide both provider and model. On an existing configured llm_judge, omit this field to keep the current model; null is rejected. When switching an llm_judge to script or sentiment, set this field to null. Legacy llm_judge evaluations without a model remain editable without adding one. The nested provider_key_id may be null.'
        ),
 ...
 */
export const EvaluationsTestScriptCreateParams = /* @__PURE__ */ zod.object({
    project_id: zod
        .string()
        .describe(
            "Project ID of the project you're trying to access. To find the ID of the project, make a call to \/api\/projects\/."
...
export const EvaluationsTestScriptCreateBody = /* @__PURE__ */ zod.object({
    source: zod
        .string()
        .min(1)
        .describe('Script source code to test. Must return a boolean (true = pass, false = fail) or null for N\/A.'),
    sample_count: zod
        .number()
        .min(1)
...
export const LlmAnalyticsClusteringConfigSetEventFiltersCreateBody = /* @__PURE__ */ zod.object({
    event_filters: zod
        .array(zod.record(zod.string(), zod.unknown()))
        .describe(
            'Insights property filters to save for automated clustering jobs. Pass an empty array to clear filters.'
        ),
})
...
export const TaggersCreateBody = /* @__PURE__ */ zod.object({
    ...
    tagger_config: zod
        .union([
            ...
            zod.object({
                source: zod.string().min(1).describe('Script source code to classify a generation into tags.'),
                tags: zod
                    .array(
                        zod.object({
                            name: zod
                    ...
                    .optional()
                    .describe('Optional tag whitelist. Leave empty to allow any tag returned by the Script code.'),
            }),
        ])
        .describe(
            "Tagger configuration. For tagger_type 'llm': {prompt, tags, min_tags?, max_tags?}. For tagger_type 'script': {source, tags?}."
        ),
    conditions: zod
        .array(
            zod.object({
        ...
        .union([
            zod.object({
                ...
                provider_key_id: zod
                    ...
                    .nullish()
                    .describe(
                        'Existing LLM provider key UUID for the current project. Do not invent this value; use a real provider key ID returned by Insights, or omit\/null when no provider key should be pinned.'
                    ),
            }),
            zod.null(),
        ])
 ...
 */
export const TaggersTestScriptCreateParams = /* @__PURE__ */ zod.object({
    project_id: zod
        .string()
        .describe(
            "Project ID of the project you're trying to access. To find the ID of the project, make a call to \/api\/projects\/."
...
export const TaggersTestScriptCreateBody = /* @__PURE__ */ zod.object({
    source: zod
        .string()
        .min(1)
        .describe('Script source code to test. Return a tag name string, a list of tag name strings, or null.'),
    sample_count: zod
        .number()
        .min(1)
