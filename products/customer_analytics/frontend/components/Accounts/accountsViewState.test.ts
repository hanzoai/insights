...
import { ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT } from './accountsColumnConfigLogic'
import {
...
describe('serializeAccountsView / deserializeAccountsView', () => {
    ...
    it('omits empty filters and serializes no sort', () => {
        const payload = serializeAccountsView({
            columns: [...ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT],
            sortOrder: null,
            filters: {
                search: '',
                tags: [],
    ...
    it('falls back to default columns when a row has none', () => {
        ...
        expect(state.columns).toEqual(ACCOUNTS_INSIGHTSQL_DEFAULT_SELECT)
    })
})
