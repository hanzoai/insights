...
class TestSessionSampleCapture:
    ...
    @parameterized.expand(
        [
            ...
            ("webhook_management", lambda: delete_webhook("fb_test", "https://us.hanzo.ai/webhook")),
        ]
    )
...
class TestWebhookManagement:
    def test_create_webhook_captures_signing_secret(self) -> None:
        ...
        with patch.object(featurebase, "make_tracked_session", return_value=session):
            result = create_webhook("fb_test", "https://us.hanzo.ai/webhook")

        assert result.success is True
        assert result.extra_inputs == {"signing_secret": "whsec_abc"}
        assert result.pending_inputs == []
        payload = session.post.call_args.kwargs["json"]
        assert payload["url"] == "https://us.hanzo.ai/webhook"
    ...
    def test_delete_webhook_matches_by_url(self) -> None:
        session = _session_with(
            get=_json_response({"data": [{"id": "wh1", "url": "https://us.hanzo.ai/webhook"}], "nextCursor": None}),
            delete=_json_response({"id": "wh1", "deleted": True}),
        )
        with patch.object(featurebase, "make_tracked_session", return_value=session):
            result = delete_webhook("fb_test", "https://us.hanzo.ai/webhook")
    ...
    def test_sync_webhook_events_patches_drifted_topics(self) -> None:
        session = _session_with(
            get=_json_response(
                {
                    "data": [{"id": "wh1", "url": "https://us.hanzo.ai/webhook", "topics": ["post.created"]}],
                    "nextCursor": None,
                }
            ),
        ...
        with patch.object(featurebase, "make_tracked_session", return_value=session):
            result = sync_webhook_events("fb_test", "https://us.hanzo.ai/webhook", ["post.created", "post.updated"])
    ...
    def test_sync_webhook_events_noop_when_topics_match(self) -> None:
        ...
        with patch.object(featurebase, "make_tracked_session", return_value=session):
            result = sync_webhook_events("fb_test", "https://us.hanzo.ai/webhook", ["post.created"])
        assert result.success is True
    ...
    def test_get_external_webhook_info(self) -> None:
        ...
        with patch.object(featurebase, "make_tracked_session", return_value=session):
            info = get_external_webhook_info("fb_test", "https://us.hanzo.ai/webhook")
