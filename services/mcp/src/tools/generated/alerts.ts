...
const alertCreate = (): ToolBase<typeof AlertCreateSchema, WithInsightsUrl<Schemas.Alert>> => ({
    name: 'alert-create',
...
const alertGet = (): ToolBase<typeof AlertGetSchema, WithInsightsUrl<Schemas.Alert>> => ({
    name: 'alert-get',
...
const alertUpdate = (): ToolBase<typeof AlertUpdateSchema, WithInsightsUrl<Schemas.Alert>> => ({
    name: 'alert-update',
...
const alertsList = (): ToolBase<typeof AlertsListSchema, WithInsightsUrl<Schemas.PaginatedAlertList>> => ({
    name: 'alerts-list',
            ...
            context,
            {
                ...
                results: await Promise.all(
                    (result.results ?? []).map((item) =>
                        withInsightsUrl(context, item, `/alerts?alert_type=insights&alert_id=${item.id}`)
                    )
                ),
            },
            '/alerts'
