...
from insights.api.github_callback.state import (
...
from insights.models.integration import GitHubInstallationAccess, GitHubUserAuthorization, Integration
from insights.models.user_integration import (
...
class TestUserIntegrationEndpoints(APIBaseTest):
    ...
    def test_list_returns_multiple_github_integrations(self):
        ...
        _create_user_integration(
            ...
            config={
                ...
                "account": {"type": "Organization", "name": "Insights"},
                "github_user": {"login": "octocat", "id": 99},
    ...
    @patch(
        "insights.api.user_integration.UserGitHubIntegration.uninstall_if_last_reference",
    ...
    def test_github_start_returns_install_url_when_no_team_github(self, _mock_settings):
        ...
        self.assertIn("github.com/apps/insights-dev/installations/new", data["install_url"])

    @override_settings(
        GITHUB_APP_CLIENT_ID="gh_client_123", SITE_URL="https://us.hanzo.ai", GITHUB_APP_CLIENT_SECRET="s"
    )
    def test_github_start_returns_oauth_discover_url_for_insights_code_without_team_github(self):
        ...
        self.assertIn("redirect_uri=https%3A%2F%2Fus.hanzo.ai%2Fcomplete%2Fgithub-link%2F", url)

    @override_settings(GITHUB_APP_CLIENT_ID="gh_client_123", SITE_URL="https://us.hanzo.ai")
    ...
    def test_github_start_insights_code_fast_path_returns_oauth_url(self):
        Integration.objects.create(
            team=self.team,
            kind="github",
            integration_id="12345678",
    ...
    def test_github_start_insights_code_skips_fast_path_when_already_linked(self, _mock_settings):
    ...
    @override_settings(GITHUB_APP_CLIENT_ID="gh_client_123")
    @patch("insights.api.user_integration._has_unlinked_github_installations", return_value=False)
    @patch(
        "insights.api.github_callback.types.get_instance_settings",
        return_value={"GITHUB_APP_SLUG": "insights-dev"},
    )
    ...
    def test_github_link_oauth_callback_creates_user_integration_without_installation_in_query(
        ...
        mock_user_from_code.assert_called_once_with(
            "test_code", redirect_uri="https://us.hanzo.ai/complete/github-link/"
        )
    ...
    def test_github_link_oauth_discover_redirects_to_app_install_when_no_installations(
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn("github.com/apps/insights-dev/installations/new", response["Location"])
    ...
    @parameterized.expand(
        [
            ("insights_code", "/account-connected/github-integration"),
            ("insights_mobile", "insights://github/callback"),
        ]
    )
    @override_settings(GITHUB_APP_CLIENT_ID="client_id", GITHUB_APP_CLIENT_SECRET="client_secret")
    @patch("insights.egress.transport.transport.requests.request")
    ...
    def test_github_link_redirects_to_mobile_deep_link_with_error(self):
        ...
        state = "test_state_insights_mobile_error"
        store_unified_authorize_state(
            GitHubAuthorizeState(
                token=state,
                flow=FlowKind.PERSONAL_INSTALL,
                user_id=self.user.id,
                connect_from="insights_mobile",
            ),
        )
        ...
        self.assertEqual(response.status_code, 302)
        loc = response["Location"]
        self.assertIn("insights://github/callback", loc)
        self.assertIn("provider=github", loc)
    ...
    def test_github_link_personal_install_without_code_recovers_via_oauth_discover(self):
        ...
        self.assertEqual(discover_state.connect_from, "insights_code")
...
class TestUserGitHubIntegration(APIBaseTest):
    ...
    @override_settings(GITHUB_APP_CLIENT_ID="client_id", GITHUB_APP_CLIENT_SECRET="client_secret")
    @patch("insights.models.user_integration.requests.post")
    ...
    def test_refresh_clears_expiry_metadata_when_github_omits_expiry_fields(self, mock_post):
        ...
        # https://app.graphite.com/github/pr/Insights/insights/55303#comment-PRRC_kwDODg-Tdc67nmop
        mock_response = MagicMock()
        mock_response.json.return_value = {
...
class TestUserGitHubIntegrationFromInstallation(APIBaseTest):
    ...
    def test_different_installation_creates_second_integration(self):
        ...
        integration = user_github_integration_from_installation(
            ...
            GitHubInstallationAccess(
                installation_id="67890",
                installation_info={"account": {"type": "Organization", "login": "insights"}},
...
class TestGithubUserFromCode(APIBaseTest):
    ...
    def test_returns_full_authorization_including_tokens(self, mock_post, mock_get):
        ...
        from insights.models.integration import GitHubIntegration
...
class TestUserIntegrationSlackEndpoints(APIBaseTest):
    ...
    def _enable_flag(self) -> Any:
        return patch("insights.api.user_integration.is_slack_app_oauth_enabled", return_value=True)
    ...
    def test_slack_linkable_returns_workspaces_user_can_still_link(self):
        ...
        self.assertEqual(results[0]["insights_team_id"], self.team.id)
