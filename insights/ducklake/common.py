...
# Insights Cloud US, eu-central-1 for Insights Cloud EU.
...
def is_dev_mode() -> bool:
    ...
    try:
        from insights.settings import USE_LOCAL_SETUP
...
def duckgres_data_imports_schema(team_id: int) -> str:
    ...
    own schema. Historically that was ``insights_data_imports_team_{team_id}``.
...
# cache "nothing to backfill" instead of re-querying Datastore. Far enough in the future
...
def _get_datastore_cluster() -> DatastoreCluster:
    ...
    # docstring); the cluster helpers pull insights.settings in at import.
    from insights.datastore.cluster import get_cluster  # noqa: PLC0415
...
def get_earliest_event_date_for_team(team_id: int) -> datetime | None:
    ...
    """
    # Deferred for the same Django-less-import reason as _get_datastore_cluster.
    from insights.datastore.client.connection import NodeRole, Workload  # noqa: PLC0415
    from insights.cloud_utils import is_cloud  # noqa: PLC0415

    cluster = _get_datastore_cluster()
...
def resolve_team_earliest_event_date(team_id: int) -> date:
    ...
    events, so callers can cache the result and never re-query Datastore for the team.
    """
