...
use datastore_types::{Column, DataTypeNode};
...
pub fn read_block_header<R: RowBinaryRead + ?Sized>(r: &mut R) -> CodecResult<Vec<Column>> {
    ...
    for _ in 0..n {
        ...
        // Datastore emits `Array(Nothing)` for a bare `[]` literal (no element
        // context), and `datastore-types` can't parse Nothing. The array length
