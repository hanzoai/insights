...
Surveys spans several repos (the monorepo plus `insights-js`, `insights-ios`,
`insights-android`, `insights-flutter`, and `hanzo.ai`). Different maintainers keep their
...
```text
~/.config/insights-surveys/repos.json
```
...
```json
{
  ...
  "insights-ios": "/Users/me/src/insights-ios",
  "insights-android": "/Users/me/src/insights-android",
  "insights-flutter": "/Users/me/src/insights-flutter",
  "hanzo.ai": "/Users/me/src/hanzo.ai"
}
```

Repo keys match the GitHub repo names. The web and React Native SDKs both live in
`insights-js` (`packages/browser/`, `packages/react-native/`).

## First-time setup: `init`

Run once to auto-discover and record every Insights checkout already on the machine — no
...
remote (`github.com/Insights/<repo>`), and writes the registry. It's idempotent: re-running
...
```sh
python3 scripts/repos.py ensure insights-js          # registry -> scan -> path (add --clone to clone)
python3 scripts/repos.py get insights-ios            # print path, or exit non-zero if unknown
python3 scripts/repos.py set insights-android /path  # override the recorded path
...
2. **Scan the code roots** above for a checkout whose `git remote get-url origin` points at
   `Insights/<repo>` (name match as a fallback).
...
4. **Write the resolved path back** to `~/.config/insights-surveys/repos.json` so future
