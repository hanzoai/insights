...
- `insightsql` — custom InsightsQL expression (requires `math_insightsql` to be set to a valid InsightsQL aggregation expression, e.g. `count(distinct properties.$session_id)`)
