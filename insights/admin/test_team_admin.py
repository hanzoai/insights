...
from insights.admin.admins.team_admin import TeamAdmin
from insights.admin.inlines.team_inline import TeamInline
from insights.llm.gateway_internal_client import (
...
class TestTeamAdminSetApiTokenView(BaseTest):
    def setUp(self) -> None:
        ...
        self.set_api_token_url = f"/admin/insights/team/{self.team.pk}/set-api-token/"
        self.team_change_url = f"/admin/insights/team/{self.team.pk}/change/"

        reverse_patcher = patch(
            "insights.admin.admins.team_admin.reverse",
            side_effect=lambda name, args=None, kwargs=None: (
                self.team_change_url if name == "admin:insights_team_change" else self.set_api_token_url
            ),
        )
    ...
    def test_get_renders_form_with_current_token(self) -> None:
        ...
        with patch("insights.admin.admins.team_admin.render") as mock_render:
...
class TestTeamAdminLLMGateway(BaseTest):
    ...
    def test_policy_cache_blob_absent(self) -> None:
        with patch(
            "insights.storage.team_llm_gateway_policy_cache.get_team_llm_gateway_policy_from_redis",
...
class TestTeamAdminAIGatewayWallet(BaseTest):
    def setUp(self) -> None:
        ...
        self.factory = RequestFactory()
        self.admin = TeamAdmin(Team, AdminSite())
        self.credit_url = f"/admin/insights/team/{self.team.pk}/add-ai-gateway-credit/"
        self.team_change_url = f"/admin/insights/team/{self.team.pk}/change/"

        reverse_patcher = patch(
            "insights.admin.admins.team_admin.reverse",
            side_effect=lambda name, args=None, kwargs=None: (
                self.team_change_url if name == "admin:insights_team_change" else self.credit_url
            ),
        )
        reverse_patcher.start()
        self.addCleanup(reverse_patcher.stop)
    ...
    def test_wallet_field_renders_actions_without_fetching(self) -> None:
        with patch("insights.admin.admins.team_admin.get_wallet") as mock_get_wallet:
    ...
    def test_wallet_view_not_configured_degrades_gracefully(self) -> None:
        with patch("insights.admin.admins.team_admin.get_wallet", side_effect=AIGatewayNotConfigured()):
            with patch("insights.admin.admins.team_admin.render") as mock_render:
    ...
    def test_wallet_view_unavailable_degrades_gracefully(self) -> None:
        with patch("insights.admin.admins.team_admin.get_wallet", side_effect=AIGatewayInternalError("boom")):
            with patch("insights.admin.admins.team_admin.render") as mock_render:
                self.admin.ai_gateway_wallet_view(self._get(), str(self.team.pk))
    ...
    def test_wallet_view_renders_balance(self) -> None:
        ...
        with patch("insights.admin.admins.team_admin.get_wallet", return_value=wallet):
            with patch("insights.admin.admins.team_admin.render") as mock_render:
                self.admin.ai_gateway_wallet_view(self._get(), str(self.team.pk))
    ...
    def test_add_credit_get_renders_form_with_nonce(self) -> None:
        ...
        template = mock_render.call_args.args[1]
        context = mock_render.call_args.args[2]
        assert template == "admin/insights/team/add_ai_gateway_credit_form.html"
    ...
    def test_add_credit_post_valid_calls_client_and_redirects_to_change(self) -> None:
        ...
        with patch("insights.admin.admins.team_admin.add_credit", return_value=result) as mock_add:
    ...
    def test_add_credit_post_rejected_inputs_redirect_to_form_without_calling_client(
        ...
        with patch("insights.admin.admins.team_admin.add_credit") as mock_add:
            response = self.admin.add_ai_gateway_credit_view(request, str(self.team.pk))
        assert response.status_code == 302
    ...
    def test_add_credit_post_gateway_error_logs_and_redirects_to_form(self) -> None:
        ...
        with patch("insights.admin.admins.team_admin.add_credit", side_effect=AIGatewayInternalError("nope")):
            with patch("insights.admin.admins.team_admin.logger") as mock_logger:
                response = self.admin.add_ai_gateway_credit_view(request, str(self.team.pk))
        assert response.status_code == 302
        assert response["Location"] == self.credit_url
    ...
    def test_add_credit_duplicate_redirects_to_change(self) -> None:
        ...
        with patch("insights.admin.admins.team_admin.add_credit", return_value=result):
            response = self.admin.add_ai_gateway_credit_view(request, str(self.team.pk))
        assert response.status_code == 302
        assert response["Location"] == self.team_change_url
    ...
    def test_add_credit_impersonated_session_is_captured_and_displayed(self) -> None:
        ...
        result = CreditResult(team_id=self.team.id, entry_id="e1", amount_usd="5", balance_usd="5", duplicate=False)
        with patch("insights.admin.admins.team_admin.is_impersonated", return_value=True):
            with patch("insights.admin.admins.team_admin.add_credit", return_value=result):
                self.admin.add_ai_gateway_credit_view(request, str(self.team.pk))
    ...
    def test_add_credit_survives_audit_write_failure(self) -> None:
        ...
        result = CreditResult(team_id=self.team.id, entry_id="e1", amount_usd="5", balance_usd="5", duplicate=False)
        with patch("insights.admin.admins.team_admin.add_credit", return_value=result):
            with patch("insights.admin.admins.team_admin.log_activity", side_effect=Exception("boom")):
                response = self.admin.add_ai_gateway_credit_view(request, str(self.team.pk))
        assert response.status_code == 302
        assert response["Location"] == self.team_change_url
...
class TestTeamAdminEmailSendingSuspension(BaseTest):
    def setUp(self) -> None:
        ...
        self.factory = RequestFactory()
        self.admin = TeamAdmin(Team, AdminSite())
        self.suspend_url = f"/admin/insights/team/{self.team.pk}/suspend-email-sending/"
        self.team_change_url = f"/admin/insights/team/{self.team.pk}/change/"

        reverse_patcher = patch(
            "insights.admin.admins.team_admin.reverse",
            side_effect=lambda name, args=None, kwargs=None: (
                self.team_change_url if name == "admin:insights_team_change" else self.suspend_url
            ),
        )
        reverse_patcher.start()
        self.addCleanup(reverse_patcher.stop)

        suspend_email_patcher = patch("insights.admin.admins.team_admin.send_email_sending_suspended")
        unsuspend_email_patcher = patch("insights.admin.admins.team_admin.send_email_sending_unsuspended")
        notification_patcher = patch("insights.admin.admins.team_admin.create_notification")
    ...
    def test_get_renders_reason_form(self) -> None:
        ...
        assert mock_render.call_args.args[1] == "admin/insights/team/suspend_email_sending_form.html"
