...
class BackfillInsightsFlowBillableActionTypesTest(TestCase):
    ...
    def test_backfill_with_various_billable_action_types(self):
        ...
        flow1 = InsightsFlow.objects.create(
            team=self.team,
        ...
        flow2 = InsightsFlow.objects.create(
            team=self.team,
            created_by=self.user,
        ...
        flow3 = InsightsFlow.objects.create(
            team=self.team,
            created_by=self.user,
        ...
        # Run the command
        out = StringIO()
        call_command("backfill_flow_billable_action_types", stdout=out)
    ...
    def test_dry_run_mode(self):
        ...
        out = StringIO()
        call_command("backfill_flow_billable_action_types", "--dry-run", stdout=out)
    ...
    def test_recomputation_of_wrong_values(self):
        ...
        call_command("backfill_flow_billable_action_types")
        flow.refresh_from_db()
    ...
    def test_batch_processing(self):
        ...
        InsightsFlow.objects.filter(name__startswith="Batch Flow Test").delete()
        ...
        out = StringIO()
        call_command("backfill_flow_billable_action_types", "--page-size", "10", stdout=out)

        output = out.getvalue()

        # Check all were processed
        flows = InsightsFlow.objects.filter(id__in=created_ids)
