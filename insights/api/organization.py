...
from insights.caching.organization_serializer_cache import (
...
from insights.rate_limit import InsightsAIAccessRequestIPThrottle, InsightsAIAccessRequestUserThrottle
...
from insights.tasks.email import send_insights_ai_access_request
    ...
    def validate_is_ai_training_opted_in(self, value: bool | None) -> bool | None:
        if self.instance and self.instance.is_ai_training_opted_in != value:
            ...
            if self.instance.is_ai_training_locked:
                raise serializers.ValidationError(
                    "AI training opt-in is locked for this organization and cannot be changed. Contact Insights support if you need to update this setting.",
                    code="locked",
                )
        return value
...
class OrganizationViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def dangerously_get_permissions(self):
        ...
        # Any org member may ask an admin to enable Insights AI — enabling still requires admin.
    ...
    def perform_destroy(self, organization: Organization):
        ...
        from insights.temporal.delete_teams.dispatch import start_delete_organization_workflow
    ...
    @action(
        ...
        throttle_classes=[InsightsAIAccessRequestUserThrottle, InsightsAIAccessRequestIPThrottle],
    )
    def request_ai_access(self, request: Request, **kwargs) -> Response:
        """Notify organization admins that a member is requesting Insights AI be enabled."""
        organization = self.organization
        user = cast(User, request.user)

        # Nothing to request if Insights AI is already enabled for the org.
        if organization.is_ai_data_processing_approved:
            raise exceptions.ValidationError("Insights AI is already enabled for this organization.")
