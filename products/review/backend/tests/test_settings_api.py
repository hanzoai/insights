...
class TestReviewUserSettingsAPI(APIBaseTest):
    ...
    def test_settings_are_per_user(self) -> None:
        ...
        other = User.objects.create_and_join(self.organization, "other-settings@hanzo.ai", None)
    ...
    def test_get_seeds_the_authoring_skill_idempotently(self) -> None:
        ...
        rows = LLMSkill.objects.filter(team=self.team, name="review-script-authoring")
        assert rows.count() == 1
