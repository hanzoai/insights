...
class UsageReportCounters:
    ...
    # Insights Desktop Billing Credits
    insights_code_credits_used_in_period: int
    insights_code_token_credits_used_in_period: int
    ...
    active_script_destinations_in_period: int
    active_script_transformations_in_period: int
...
def combine_insights_code_credits(token_credits: int, compute_credits: int) -> int:
...
@timed_log()
@retry(tries=QUERY_RETRIES, delay=QUERY_RETRY_DELAY, backoff=QUERY_RETRY_BACKOFF)
def get_teams_with_active_script_destinations_in_period() -> list:
    return list(
...
@timed_log()
@retry(tries=QUERY_RETRIES, delay=QUERY_RETRY_DELAY, backoff=QUERY_RETRY_BACKOFF)
def get_teams_with_active_script_transformations_in_period() -> list:
    return list(
        InsightsFunction.objects.filter(
...
def _get_all_usage_data(period_start: datetime, period_end: datetime) -> dict[str, Any]:
    ...
    token_credits = get_teams_with_insights_code_credits_used_in_period(period_start, period_end)
    ...
    return {
        ...
        "teams_with_insights_code_credits_used_in_period": token_credits,
        ...
        "teams_with_active_script_destinations_in_period": get_teams_with_active_script_destinations_in_period(),
        "teams_with_active_script_transformations_in_period": get_teams_with_active_script_transformations_in_period(),
...
def _get_team_report(all_data: dict[str, Any], team: Team) -> UsageReportCounters:
    ...
    return UsageReportCounters(
        ...
        insights_code_credits_used_in_period=combine_insights_code_credits(
            all_data["teams_with_insights_code_credits_used_in_period"].get(team.id, 0),
            all_data["teams_with_sandbox_compute_credits_used_in_period"].get(team.id, 0),
        ),
        insights_code_token_credits_used_in_period=all_data["teams_with_insights_code_credits_used_in_period"].get(
            team.id, 0
        ),
        ...
        active_script_destinations_in_period=all_data["teams_with_active_script_destinations_in_period"].get(team.id, 0),
        active_script_transformations_in_period=all_data["teams_with_active_script_transformations_in_period"].get(
            team.id, 0
        ),
