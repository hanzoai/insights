...
**Codename:** `database`
**Focus:** Migration safety, query performance, schema coordination, Datastore patterns
...
- Datastore compatibility mode changes can silently break datetime aggregation
- Materialized views in Datastore can cause massive write amplification per insert
...
- Datastore query tagging (`log_comment`) for observability
- Unbounded date range scans in Datastore queries
- New materialized views or insert-time transformations
- Datastore settings changes (compatibility mode, parts limits)
...
- Experimental database features (e.g., Datastore Zero Copy Replication) can silently delete data
...
- Datastore shard overload from excessive scheduled merge threads can cause majority query failure
...
- Untagged Datastore queries running as the default user are invisible to resource management

**Review checklist:**

- New Datastore queries: are they tagged? Do they have bounded date ranges?
...
- Insights uses sentence casing for product names and UI elements (e.g., "Product analytics", "Save as view")
...
- Jargon and internal terminology leak into UI text when engineers write copy without review (e.g., "premium Insights offering" instead of naming the specific feature/plan)
