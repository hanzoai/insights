...
Validates Insights OAuth access tokens via the /oauth/introspect endpoint on every request,
...
it before forwarding to us. The backend duplicates the token as `_insights_modal_token` which
...
`_insights_token` to all same-host browser sub-requests. This keeps every request authenticated
...
insights_apps.query() shim to run InsightsQL via Insights. The bridge forwarder reads its bearer
...
# Raised from aiohttp's 8190 default: localhost-served sandboxes receive Insights's
...
(function(){
  var MK="_modal_connect_token",MV=%s,PK="_insights_token",PV=%s;
  function a(u){
...
# Bridge token for the sandbox → Insights InsightsQL hop. Loaded once at startup from
...
_AUTH_PARAM_KEYS = ("_insights_token", "_insights_modal_token", "_modal_connect_token")
...
async def _introspect_token(
    ...
    *,
    ...
    """Validate an OAuth token via Insights /oauth/introspect (self-introspection), with caching.
...
def _inject_token_into_html(body: bytes, modal_token: str, insights_token: str) -> bytes:
    ...
    insights_token_attr = html.escape(insights_token, quote=True)
    ...
    def _replace(m: re.Match) -> str:
        ...
        sep = "&" if "?" in url else "?"
        token_qs = f"_insights_token={insights_token_attr}"
        if modal_token_attr:
    ...
    insights_js = json.dumps(insights_token).replace("</", "<\\/")
    js_shim = _TOKEN_INJECTION_JS_TEMPLATE % (modal_js, insights_js)
...
async def proxy_handler(request: web.Request) -> web.StreamResponse:
    ...
    token = request.query.get("_insights_token")
    if not token:
        ...
        log.warning("request: no _insights_token in query (%s %s)", request.method, path)
    ...
    introspection = await _introspect_token(
        session,
        token,
        team_id=request.app["insights_team_id"],
        expected_client_id=request.app["insights_streamlit_client_id"],
...
async def _proxy_http(request: web.Request, target: str) -> web.StreamResponse:
    ...
    insights_token = request.query.get("_insights_token")
    # Per-request capture — Modal forwards the connect token as
    # _insights_modal_token; we re-inject it into HTML responses.
    modal_connect_token = request.query.get("_insights_modal_token")
    ...
    async with client_session.request(
        ...
        # Inject whenever we have a insights token; the modal token is optional
        ...
        if is_html and insights_token:
            body = await resp.read()
            body = _inject_token_into_html(body, modal_connect_token or "", insights_token)
...
async def _bridge_query_handler(request: web.Request) -> web.Response:
    """Forward in-sandbox InsightsQL queries from insights_apps.query() to Insights.
...
def create_app() -> web.Application:
    ...
    app = web.Application()
    app["insights_team_id"] = team_id
    app["insights_streamlit_client_id"] = client_id
...
async def _run_both_apps(
    ...
    # sandbox is served from localhost (Docker), the browser attaches Insights's
