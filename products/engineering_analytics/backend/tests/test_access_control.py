...
The curated queries run via execute_insightsql_query(team=...) with no request user, and InsightsQL does
...
The curated InsightsQL run is stubbed to empty results so the test isolates the resolver's
...
# Every curated query runs InsightsQL through this method; stub it to empty so a resolved source
...
_EXECUTE_INSIGHTSQL = "products.engineering_analytics.backend.logic.queries._curated.execute_insightsql_query"
# Schema build reads this to decide whether per-table warehouse ACL is enforced.
_FLAG = "insights.insightsql.database.database.feature_enabled_or_false"
...
class TestEngineeringAnalyticsAccessControl(WarehouseAccessControlTestMixin):
    ...
    def test_denied_user_cannot_write_quarantine_to_source_b(self) -> None:
        ExternalDataSource.objects.filter(pk=self.source_b.pk).update(job_inputs={"repository": "Insights/secret"})
        github = mock.Mock()
        ...
        github.create_pull_request.return_value = {
            "success": True,
            "pr_url": "https://github.com/Insights/secret/pull/99",
        }
...
class TestEngineeringAnalyticsWarehouseAcl(WarehouseAccessControlTestMixin):
    """Per-table warehouse ACL on the real curated read path, once insightsql-warehouse-access-control is on.

    #61686 makes a userless InsightsQL build fail closed -- every warehouse table is stripped from the schema
    -- which is why the curated GitHub reads (historically run with no user) 500'd when the flag rolled
    out. The fix forwards the request user into execution. These build the InsightsQL schema exactly as the
    ...
    def _schema_as_runner_builds_it(self, user_access_control: UserAccessControl | None) -> tuple[Database, dict]:
        ...
        with mock.patch(_EXECUTE_INSIGHTSQL, side_effect=_capture):
