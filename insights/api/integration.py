...
from insights.api.github_callback import state as github_callback_state
...
from insights.api.github_callback.types import (
    FlowKind,
...
from insights.domain_connect import discover_domain_connect, extract_root_domain_and_host, get_available_providers
...
from insights.helpers.fuzzy_search import fuzzy_filter
...
from insights.tasks.email import send_integration_access_request
from insights.utils import is_relative_url
...
def _verify_stripe_install_signature(state: str, user_id: str, account_id: str, install_signature: str) -> bool:
    ...
    callback URL could link an attacker's Stripe account onto a victim's Insights team.
...
class GitHubRepoSerializer(serializers.Serializer):
    ...
    can_push = serializers.BooleanField(
        required=False,
        help_text="Whether the Insights GitHub App has write access — required to open pull requests.",
    )
...
class SlackChannelSerializer(serializers.Serializer):
    ...
    is_member = serializers.BooleanField(
        help_text="True if the Insights Slack app is a member of the channel and can post to it."
    )
    ...
    is_private_without_access = serializers.BooleanField(
        help_text="True if the channel is private and the Insights Slack app cannot access it."
    )
...
class SlackChannelsResponseSerializer(serializers.Serializer):
    channels = SlackChannelSerializer(many=True, help_text="Slack channels visible to the Insights Slack app.")
...
class IntegrationSerializer(serializers.ModelSerializer, UserAccessControlSerializerMixin):
    ...
    def _build_integration(self, validated_data: Any) -> Any:
        ...
        if validated_data["kind"] in GoogleCloudIntegration.supported_kinds:
            ...
            # a Insights-minted CSRF state token — Stripe drives the OAuth flow itself.
            ...
            if validated_data["kind"] == "stripe":
                try:
                    ...
                    stripe_integration.write_insights_secrets(team_id, request.user)
...
class GitHubPrepareCallbackRequestSerializer(serializers.Serializer):
    next = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Relative URL to redirect to after GitHub setup completes (e.g. account-connected for Insights Desktop).",
    )
...
class GitHubOAuthAuthorizeRequestSerializer(serializers.Serializer):
    ...
    connect_from = serializers.ChoiceField(
        required=False,
        choices=["insights_code"],
        help_text="Originating surface for the connect flow; only 'insights_code' is recognized.",
    )
...
class IntegrationViewSet(
    ...
    def safely_get_queryset(self, queryset: QuerySet) -> QuerySet:
        # A `insights` connection holds the creator's delegated grant into another project plus personal
    ...
    def perform_destroy(self, instance) -> None:
        ...
        if instance.kind == "stripe":
            try:
                stripe_integration = StripeIntegration(instance)
                stripe_integration.clear_insights_secrets()
            except Exception as e:
                capture_exception(e)
    ...
    def channels(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        # Key on the Integration row PK (unique per Insights team × Slack workspace), not
    ...
    def github_prepare_callback(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        Used when the user opens an existing installation's settings on github.com (e.g. Insights
    ...
    def github_oauth_authorize(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        oauth_url = build_team_oauth_authorize_url(
            ...
            if request.data.get("connect_from") == "insights_code"
            else None,
        )
    ...
    def domain_connect_apply_url(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        """
        from insights.domain_connect import (
