...
from insights.persons_seed import insert_seed_distinct_id, insert_seed_person
from insights.temporal.sync_person_distinct_ids.activities import (
    find_orphaned_persons,
    lookup_pg_distinct_ids,
    mark_ch_only_orphans_deleted,
    sync_distinct_ids_to_ch,
)
from insights.temporal.sync_person_distinct_ids.workflow import (
    SyncPersonDistinctIdsWorkflow,
    SyncPersonDistinctIdsWorkflowInputs,
)
from insights.temporal.tests.sync_person_distinct_ids.conftest import (
...
class TestSyncPersonDistinctIdsWorkflow:
    ...
    async def _create_test_data(self):
        ...
        # Persons-DB rows only (no Datastore): the fixable persons are orphans precisely because
        # their distinct id lives in PG but not CH, so use the low-level inserts rather than the
        # create_person/add_distinct_id helpers (which also mirror into Datastore).
        @sync_to_async
        def create_pg_data():
