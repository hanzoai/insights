...
description: >
  Guides Insights engineers through dashboard widget platform work — ship a new
  ...
  Use for WidgetSpec, widget_specs/, widget-configs.zod.ts, insightscli build:openapi,
...
For **Insights engineers** working on dashboard **widget tiles** in the repo.
...
5. **Config contract = Pydantic SSOT** — `widget_specs/configs.py` + `WidgetSpec` in `registry.py`; `insightscli build:openapi` for OpenAPI/FE Zod/MCP. Runtime PATCH stays `JSONField`. See [config-and-codegen.md](references/config-and-codegen.md).
...
```bash
insightscli test products/dashboards/backend/api/test/test_run_widgets.py
insightscli test products/dashboards/backend/api/test/test_dashboard_widgets.py
insightscli test products/dashboards/frontend/widgets/registry.test.tsx
```

**Ship gate (required before the PR):** [checklist §8](references/checklist-new-widget-type.md#8-tests) + `insightscli build:openapi` + dedicated Storybook stories per [checklist §5b](references/checklist-new-widget-type.md#5b-storybook) (component + edit modal — required, not just the catalog overview).
...
insightscli test products/dashboards/backend/api/test/test_widget_openapi_enums.py   # new widget_type only
```

**Update:** tests for layers touched — at minimum `test_run_widgets.py` if BE changed, `products/dashboards/frontend/widgets/` if FE changed, `insightscli build:openapi` if config OpenAPI changed. See [managing-existing-widgets.md](references/managing-existing-widgets.md) routing table **Also check** column.
