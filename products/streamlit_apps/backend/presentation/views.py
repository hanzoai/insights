...
from insights.api.mixins import TypedRequest, validated_request
from insights.api.routing import TeamAndOrgViewSetMixin
from insights.api.utils import action
from insights.models.user import User
from insights.rate_limit import DatastoreBurstRateThrottle
...
# The sandbox bridge executes the active version's InsightsQL as that version's author,
# so tokens driving these paths must themselves hold query:read — otherwise a
# streamlit_app-only key escalates to InsightsQL reads through code it uploads, or by
...
class StreamlitAppViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet):
    ...
    @action(methods=["POST"], detail=True, url_path="upload_version", throttle_classes=[DatastoreBurstRateThrottle])
    ...
    @action(
        ...
        throttle_classes=[DatastoreBurstRateThrottle],
    )
    ...
    @action(methods=["POST"], detail=True, url_path="start", throttle_classes=[DatastoreBurstRateThrottle])
    ...
    @action(methods=["GET"], detail=True, url_path="connect_info", throttle_classes=[DatastoreBurstRateThrottle])
    ...
    @action(methods=["POST"], detail=True, url_path="restart", throttle_classes=[DatastoreBurstRateThrottle])
