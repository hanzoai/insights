...
import { PipelineEvent, PluginsServerConfig, ProjectId, RawDatastoreEvent, RedisPool, Team } from '../../src/types'
...
/**
 * Waits until Datastore's Kafka engine is consuming, by producing probe messages until one
 * lands in the ingestion_warnings table.
 *
 * Before probing we create every topic Datastore's Kafka engine tables subscribe to. Otherwise
 * any table whose topic is missing keeps retrying "Can't get assignment", which saturates
 * Datastore's background scheduler and intermittently starves the consumers we depend on — the
 ...
 */
export async function waitForDatastoreKafkaConsumer(datastore: Datastore): Promise<void> {
    await ensureKafkaTopics(await datastore.getKafkaEngineTopics())
    ...
    try {
        await waitForExpect(async () => {
            ...
            const result = await datastore.query<{ count: number }>(
...
export async function retryDatastoreOperation<T>(
    operation: () => Promise<T>,
    ...
    if (throwOnFailure && lastError) {
        ...
        console.warn(`[DEBUG] Datastore ${operationName} failed after all retries (non-fatal):`, lastError?.message)
        return null
    }
...
export async function fetchEvents(datastore: Datastore, teamId: number) {
...
export async function fetchIngestionWarnings(datastore: Datastore, teamId: number) {
    const queryResult = (await retryDatastoreOperation(
        () =>
            datastore.query(`
                SELECT *
