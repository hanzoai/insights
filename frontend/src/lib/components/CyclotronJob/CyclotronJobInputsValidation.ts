...
// Roots of the templating globals available to script functions. Anchoring the
 ...
 * or `{ … }` script syntax in a Liquid field).
 */
const detectTemplatingMismatch = (value: unknown, language: 'script' | 'liquid'): string | undefined => {
    ...
    if (language === 'liquid') {
        // Strip valid Liquid ({{ }} / {% %}) first, then look for leftover script-style
    ...
    // Script field. Beyond `{{ global.… }}`, a pipe filter inside double braces (`{{ x | upcase }}`)
    // or a `{% … %}` tag is unambiguously Liquid regardless of what it references — Script has no
    // single-pipe operator and no percent tags, so these fail Script compilation at activation.
    if (
...
const detectInputWarning = (
    ...
    const language = input.templating ?? 'script'
