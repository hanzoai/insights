...
description: >
  ...
  via `insights:insight-create` or `insights:insight-update`. Use when the agent
...
Insights renders TrendsQuery insights with a built-in axis formatter. Use it
...
are in that same currency — e.g. revenue events that Insights auto-converts to
...
> Insights's `duration` formatter which auto-picks seconds / minutes / hours
...
`formula`/`postfix` anti-pattern, fix it in the same `insights:insight-update`
