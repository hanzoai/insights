...
def _pk_uniqueness_probe_timeout_error() -> QueryTimeoutException:
    ...
    return QueryTimeoutException(
        ...
        'Insights assumed "id" was unique to sync incrementally but could not confirm it within the '
        'timeout. Add a primary key, or ensure the "id" column has an appropriate index created, so '
        "Insights can sync this table incrementally."
    )
...
# bastion refusing Insights's IPs, or a proxy that resets the stream. sshtunnel doesn't wrap it (it
