...
export const STATUS_TAG_TYPES: Record<ManagedWarehouseReadinessStateEnumApi, TagType> = {
...
export function StatusTag({ readinessState }: { readinessState: ManagedWarehouseReadinessStateEnumApi }): JSX.Element {
    return <Tag type={STATUS_TAG_TYPES[readinessState]}>{STATUS_LABELS[readinessState]}</Tag>
}

// Per-schema detail columns: used by the drill-down modal, scoped to one source's schemas.
export const sourceSchemaColumns: TableColumns<ManagedWarehouseSourceTableStatusApi> = [
    {
        title: 'Table',
