...
from products.review.backend.models import ReviewReport, ReviewReportArtefact, ReviewSkillConfig
from products.review.backend.reviewer.artefact_content import (
    PerspectiveSelectionArtefact,
    ReviewIssueFinding,
    ValidationVerdict,
)
from products.review.backend.reviewer.constants import BLIND_SPOT_PASS_NUMBER
from products.review.backend.reviewer.models.github_meta import PRMetadata
from products.review.backend.reviewer.models.perspective_selection import ChunkPerspectiveSelection
...
def _expected_reads(team_id: int, report: ReviewReport, turn: TurnStats) -> int | None:
    ...
    enabled = (
        ...
        .filter(user_id=report.acting_user_id, enabled=True, skill_name__startswith=REVIEW_FN_PERSPECTIVE_PREFIX)
        .count()
    )
