...
def test_capture_review_completed_includes_familiarity_and_provenance(
    ...
    # Downstream InsightsQL queries key on these property names and null/empty
    ...
    if populated:
        pipeline.classification = {
            "ownership": {"teams": ["@Insights/team-devex"], "team_count": 1, "individuals": [], "cross_team": False}
        }
...
def test_capture_review_completed_merges_server_extras_base_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    # The hosted server stamps runtime/team context through STAMPFN_EXTRA_PROPERTIES; the event's
...
def test_prerequisites_draft_gate_respects_the_self_driving_carve_out(self_driving: bool, expect_pass: bool) -> None:
    ...
    pipeline = Pipeline(pr_number=1, repo="Insights/insights", self_driving=self_driving)
    pr = _fake_pr(head_sha="abc123")
    pr.draft = True
    pipeline.pr = pr
...
def test_self_driving_prerequisites_still_block_changes_requested() -> None:
    ...
    pipeline = Pipeline(pr_number=1, repo="Insights/insights", self_driving=True)
    pr = _fake_pr(head_sha="abc123")
    pr.draft = True
