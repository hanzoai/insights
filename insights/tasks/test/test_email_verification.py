...
from insights.tasks.email import send_email_verification
...
class TestEmailVerificationRecipient(SimpleTestCase):
    ...
    def test_verification_recipient(
        ...
        with (
            patch("insights.tasks.email.User.objects.get", return_value=user),
            patch("insights.tasks.email.hanzo_insights.capture"),
            patch("insights.tasks.email.EmailMessage") as MockEmailMessage,
        ):
