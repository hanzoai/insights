...
Each `QuerySpec` describes one fetch — typically one Datastore query, one
...
those as a single fetch to avoid re-running the same heavy Datastore pass
...
* Re-running the workflow for the same `period_start`/`period_end` will
  ...
  (counts of dashboards, feature flags, surveys, active script destinations,
...
Period queries (`kind="period"`) are re-run safe: they filter Datastore on
...
behavior — see `insights/tasks/usage_report.py:_get_all_usage_data` for the
canonical reference.
"""
...
from insights.models.group_type_mapping import count_group_type_mappings_per_team
from insights.tasks.usage_report import (
    ...
    get_teams_with_active_script_destinations_in_period,
    get_teams_with_active_script_transformations_in_period,
    ...
    get_teams_with_insights_function_calls_in_period,
    get_teams_with_insights_function_fetch_calls_in_period,
    ...
    get_teams_with_insights_code_credits_used_in_period,
...
QUERIES: list[QuerySpec] = [
    ...
    QuerySpec(
        ...
        output="multi",
        # Keep this fan-out in sync with `_get_team_report` in `insights.tasks.usage_report`.
        ...
        multi_keys_mapping={
            ...
            "insights_pi_events": "teams_with_insights_pi_events_count_in_period",
            "insights_ai_events": "teams_with_insights_ai_events_count_in_period",
    ...
    # ---- Datastore: feature flag requests -----------------------------------
    QuerySpec(
    ...
    # ---- Datastore: data warehouse / batch exports usage --------------------
    QuerySpec(
    ...
    # ---- Datastore: exceptions / error tracking events ----------------------
    QuerySpec(
    ...
    # ---- Datastore: script functions / CDP ------------------------------------
    QuerySpec(
        name="teams_with_insights_function_calls_in_period",
        fn=get_teams_with_insights_function_calls_in_period,
    ),
    QuerySpec(
        name="teams_with_insights_function_fetch_calls_in_period",
        fn=get_teams_with_insights_function_fetch_calls_in_period,
    ),
    QuerySpec(
    ...
    QuerySpec(
        name="teams_with_insights_code_credits_used_in_period",
        fn=get_teams_with_insights_code_credits_used_in_period,
    ),
    ...
    QuerySpec(
        name="teams_with_active_script_destinations_in_period",
        fn=get_teams_with_active_script_destinations_in_period,
        timeout_minutes=5,
        kind="snapshot",
    ),
    QuerySpec(
        name="teams_with_active_script_transformations_in_period",
        fn=get_teams_with_active_script_transformations_in_period,
        timeout_minutes=5,
        kind="snapshot",
    ),
    QuerySpec(
