...
from posthog.api.insights_flow_batch_job import InsightsFlowBatchJobSerializer
...
from posthog.models.insights_flow.insights_flow import BILLABLE_ACTION_TYPES, InsightsFlow
from posthog.models.insights_function_template import InsightsFunctionTemplate
from posthog.plugins.plugin_server_api import create_insights_flow_invocation_test
...
class InsightsFlowActionSerializer(serializers.Serializer):
    ...
    filters = InsightsFunctionFiltersSerializer(required=False, default=None, allow_null=True)
...
class InsightsFlowMinimalSerializer(serializers.ModelSerializer):
...
class InsightsFlowSerializer(InsightsFlowMinimalSerializer):
    actions = serializers.ListField(child=InsightsFlowActionSerializer(), required=True)
    trigger_masking = InsightsFlowMaskingSerializer(required=False, allow_null=True)
    variables = InsightsFlowVariableSerializer(required=False)
...
class InsightsFlowInvocationSerializer(serializers.Serializer):
    configuration = InsightsFlowSerializer(write_only=True, required=False)
...
class InsightsFlowViewSet(TeamAndOrgViewSetMixin, LogEntryMixin, AppMetricsMixin, viewsets.ModelViewSet):
    ...
    def get_serializer_class(self) -> type[BaseSerializer]:
        return InsightsFlowMinimalSerializer if self.action == "list" else InsightsFlowSerializer
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.warning("Failed to capture insights_flow_created event", error=str(e))
    ...
    def perform_update(self, serializer):
        ...
        try:
            ...
            before_update = InsightsFlow.objects.get(pk=instance_id)
        ...
        if (
            ...
            try:
                ...
                logger.warning("Failed to capture insights_flow_activated event", error=str(e))
