...
Sandboxed agents are background AI agents that run in isolated cloud containers with access to Insights data,
GitHub repositories, and code execution.
Use them when your feature needs an autonomous agent that reads Insights data, writes code, and produces artifacts like PRs or reports.
...
- A **scoped OAuth access token** for the Insights API (6-hour TTL)
- A **GitHub installation token** for repo operations
- Access to the **Insights MCP server** for querying data
...
task = Task.create_and_run(
    ...
    user_id=user.id,
    insights_mcp_scopes="read_only",  # or "full" if the agent needs write access
)
```
...
Every sandboxed agent gets a scoped OAuth access token that controls what Insights resources it can access.
...
Task.create_and_run(
    # ...
    insights_mcp_scopes=["query:read", "feature_flag:read", "experiment:read"],
)
```
...
See `insights/temporal/oauth.py` for the full list.
...
The sandbox comes with access to the Insights MCP server,
which exposes Insights resources as tools the agent can call —
listing feature flags, running InsightsQL queries, searching session recordings, etc.
...
- `/scripts/plugins/insights/skills/` – plugin discovery
...
See the [example README](https://github.com/Insights/insights/blob/master/products/tasks/backend/logic/services/mts_example/README.md) for details on adapting it to your own use case.
...
Direct `git commit` and `git push` commands are blocked in the sandbox to ensure all commits are properly signed by GitHub. A PATH shim (`git-guard.sh` at `/opt/insights/bin/git`) intercepts these subcommands while passing all other git operations through to the real binary.

If an agent attempts to run `git commit` or `git push`, it will see:

```text
git commit is disabled in Insights Desktop: commits must be signed.
...
- `default_custom_image` — a Modal image name that VM runs fall back to when no custom image was picked.
  ...
  Insights's own org points at the prebaked dev-stack image (below), everyone else keeps the plain VM base.
...
`insightscli start` on a fresh VM pays for multi-gigabyte docker pulls and the full Django + persons +
Datastore migration history — and dead-ends anyway, because the lean VM base lacks the dev
toolchain flox provides on dev machines (brotli, phrocs, Go, Rust). For runs on the Insights
monorepo we bake all of that ahead of time:
the `bake-dev-stack-image` Temporal workflow boots a plain VM-base sandbox,
runs `bake-insights-dev-stack.sh` inside it (install the dev toolchain, pre-pull the dev compose
...
Modal image name `insights-dev-stack` (see `products/tasks/backend/logic/services/dev_stack_image.py`).
...
state and already-migrated databases, so a task-time `insightscli start` only applies the migrations
...
`/opt/insights/dev-stack-bake.json` names it under `bootstrap`), then from the checkout run
`uv sync`, `source .venv/bin/activate`, `insightscli start -y -d`, and `insightscli wait`. Detached mode is
...
so `insightscli wait` is what blocks until every process reports ready.

Provisioning also fires that helper detached as soon as the sandbox is up (best-effort, only on
runs that booted the Insights-published `insights-dev-stack` image itself — never a user-authored
...
or the Insights Desktop settings UI.
...
the `sandbox_custom_images` API returns 403 (and the Insights Desktop UI hides the feature)
...
The flow, driven from the Insights Desktop Environments → Cloud tab:
...
1. Create a personal dev GitHub App (see the [Cloud runs setup guide](https://github.com/Insights/insights/blob/master/docs/internal/sandboxes-setup-guide.md#github-app) for details)
2. Run `python manage.py setup_background_agents`
3. Run `insightscli start`
...
For advanced setup options (Modal sandboxes, local agent packages, MCP), see the [Cloud runs setup guide](https://github.com/Insights/insights/blob/master/docs/internal/sandboxes-setup-guide.md).

**Tip:** Set `SANDBOX_REPO_MOUNT_MAP` to bind-mount local repositories into the Docker container and skip cloning from GitHub. Format: `SANDBOX_REPO_MOUNT_MAP=org/repo:/local/path` (e.g., `SANDBOX_REPO_MOUNT_MAP=Insights/insights:~/Developer/insights`). This can significantly reduce sandbox startup time for large repos.

## Questions?

If you're unsure whether a sandboxed agent is the right fit for your use case,
ask in **#team-insights-ai** on Slack.
