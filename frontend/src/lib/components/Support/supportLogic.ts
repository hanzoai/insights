...
import { EMAIL_SUPPORT_BUTTON, toast } from 'lib/elements/Toast/Toast'
...
// The recording lives in Insights's own telemetry project, which the reporting user is not a member
// of, so this link is for Insights staff triaging the ticket/issue — never the user. We rewrite to the
// internal http://go/session/ golink to make that explicit. insights-js returns a project-scoped path
...
function getSessionReplayLink(): string {
    const replayUrl = insights.get_session_replay_url?.({ withTimestamp: true, timestampLookBack: 30 })
    if (!replayUrl) {
        return ''
    }
...
function getErrorTrackingLink(uuid?: string): string {
    ...
    return `\nExceptions: https://us.hanzo.ai/project/2/error_tracking?filterGroup=${filterGroup}`
}
...
async function waitForConversations(timeoutMs = 5000): Promise<boolean> {
    ...
    return !!insights.conversations?.isAvailable()
}

// Mirrors the widget message serializer cap (WidgetMessageSerializer.message). Support submits post
// through insights.conversations.sendMessage (the widget endpoint), so guard against the same cap.
...
] as const satisfies SelectOptions<string>
...
const TARGET_AREA_TO_NAME_PRODUCTS = [
    ...
    {
        value: 'insights-ai',
        'data-attr': `support-form-target-area-insights-ai`,
        label: 'Insights AI',
    },
    {
        value: 'insights-mcp',
        'data-attr': `support-form-target-area-insights-mcp`,
        label: 'Insights MCP',
    },
    {
        value: 'analytics',
...
export type SupportFormFields = {
    ...
    tags?: string[]
    // Set when the ticket originates from a Insights AI (/ticket, feedback) handover, so the created
...
export const supportLogic = kea<supportLogicType>([
    ...
    listeners(({ actions, props, values }) => ({
        ...
        submitSupportTicket: async (formValues: SupportFormFields) => {
            ...
            // Attribute Insights AI (/ticket, feedback) handovers to the conversation. The ticket id
            ...
            if (values.conversationsFlagEnabled && (await waitForConversations())) {
                ...
                try {
                    const response = await insights.conversations!.sendMessage(
                        outgoingMessage,
                    ...
                    insights.capture('support ticket send failed', {
                    ...
                    toast.error("Oops, the message couldn't be sent. Please try again in a moment.", {
