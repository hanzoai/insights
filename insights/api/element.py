...
from insights.schema import DateRange, ProductKey
...
from insights.models.element.element import build_attributes_filter, chain_to_element_dicts
...
class ElementViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                description=(
                    ...
                    "Supports event, person, cohort, element, and InsightsQL property filter types."
                ),
            ),
            OpenApiParameter(
    ...
    def stats(self, request: request.Request, **kwargs) -> response.Response:
        ...
        with (
            ...
            with timer("prepare_for_query"), tracer.start_as_current_span("elements_api_stats.prepare_for_query"):
                ...
                # 0 would silently return no rows (SAMPLE 0); out-of-range values 500 in Datastore
                ...
                # InsightsQL resolves property access per the team's modifiers (materialized
    ...
    def values(self, request: request.Request, **kwargs) -> response.Response:
        with (
            ...
            # no explicit team filter: execute_insightsql_query scopes the events table to self.team
            select = parse_select(
                """
            ...
            result = execute_insightsql_query(query=select, team=self.team, query_type="elements_values").results
