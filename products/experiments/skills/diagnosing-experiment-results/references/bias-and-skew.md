...
Open the Exposures tab. Insights runs a chi-squared test once total exposures ≥ 100 and flags SRM at
...
1. **Bot traffic hashing into a single variant.** Server-side flag evaluations from bots are
   ...
   `$lib` values like `insights-python`, `insights-node`, `insights-ruby`, `insights-go`,
   `insights-php`). High server-side share + no bot filter is the signature.

   _Fix path:_ enable the **Bot detector** Script Function template at _Settings → Data pipeline →
   Transformations_. Filters known crawler user agents before ingestion. Cannot be enabled via
   MCP — guide the user to the UI.

   <!-- Source for maintainers: docs at https://hanzo.ai/docs/experiments/troubleshooting#diagnosing-sample-ratio-mismatch-srm
   (item 1, "ranked by frequency"). Template lives in
   insights/api/test/__data__/insights_function_templates.json — search for "known_bot_filter_list". -->
...
5. **Ad-blockers / network drops.** Prevent flag calls from reaching Insights at all.
   ...
   _Fix path:_ set up a [reverse proxy](https://hanzo.ai/docs/advanced/proxy) so capture
...
haven't been linked — Insights sees them as separate persons, each correctly assigned a variant.
...
usage and incognito / stealth browsing are _not_ fixable from Insights's side — and the users who
...
payload doesn't include `distinctID` — so `insights-js` initializes with whatever's in persistence (often
the anonymous ID). The bootstrap value gets reported under the anonymous ID; then `insights-js` calls
...
users see test, Android users see control), the resulting assignment is **not random**. Insights's
statistics assume randomization, so this invalidates the standard significance interpretation.

Insights doesn't prevent this in the UI — but the user should understand that significance calculations
