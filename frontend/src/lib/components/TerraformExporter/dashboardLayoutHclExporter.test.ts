    ...
    describe('sanitizes correctly', () => {
        ...
        it.each(testCases)('converts "$input" to "$expected"', ({ input, expected }) => {
            ...
            const hcl = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.test.id',
                insightIdReplacements: new Map(),
            }).hcl

            expect(hcl).toContain(`resource "insights_dashboard_layout" "${expected}"`)
        })
    })
    ...
    describe('generates valid hcl', () => {
        it('generates valid HCL for a complete layout', () => {
            ...
            const result = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.my_dashboard.id',
                insightIdReplacements: new Map([[100, 'insights_insight.my_insight.id']]),
                projectId: 42,
            })

            expect(result.hcl).toContain(`import {
  to = insights_dashboard_layout.my_dashboard
  id = "42/1"
}

resource "insights_dashboard_layout" "my_dashboard" {
  dashboard_id = insights_dashboard.my_dashboard.id
  tiles = [
    {
      insight_id = insights_insight.my_insight.id
    ...
    },
    {
      ...
      color = "blue"
        ...
        it('omits layouts_json when layouts is empty', () => {
            ...
            const result = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.no_layouts.id',
                insightIdReplacements: new Map(),
            })
      ...
      insight_id = 500
        ...
        it('excludes import block for new dashboards', () => {
            ...
            const result = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.new.id',
                insightIdReplacements: new Map(),
            })
        ...
        it('respects includeImport option', () => {
            ...
            const hclWithImport = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.test_dashboard.id',
                insightIdReplacements: new Map(),
                includeImport: true,
            }).hcl
    ...
    describe('filters unusable tiles', () => {
        it('excludes tiles with neither insight nor text', () => {
            ...
            const result = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.empty_tiles.id',
                insightIdReplacements: new Map(),
            })

            expect(result.hcl).toContain(`resource "insights_dashboard_layout" "empty_tiles" {
  dashboard_id = insights_dashboard.empty_tiles.id
  tiles = [
    {
      insight_id = 700
    },
    {
      text_body = "Keep me"
        ...
        it('excludes deleted tiles from output', () => {
            ...
            const result = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.mixed_tiles.id',
                insightIdReplacements: new Map(),
            })
    ...
    describe('falls back to hardcoded ids', () => {
        it('uses hardcoded insight_id when no replacement exists', () => {
            ...
            const result = generateDashboardLayoutHCL(dashboard, {
                dashboardTfReference: 'insights_dashboard.partial_refs.id',
                insightIdReplacements: new Map([[1300, 'insights_insight.replaced.id']]),
            })

            expect(result.hcl).toContain(`tiles = [
    {
      insight_id = insights_insight.replaced.id
    },
    {
      insight_id = 1301
    },
