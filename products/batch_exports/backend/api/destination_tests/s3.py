...
class S3AssumeRoleTestStep(DestinationTestStep):
    ...
    async def _run_step(self) -> DestinationTestStepResult:
        ...
        external_id = f"insights-{self.organization_id}"
        try:
            _ = await get_credentials_using_user_aws_role(
                self.aws_role_arn,
                external_id,
                session_name="Insights-batch-exports-test",
                policy_statements=[
                    PolicyStatement(
            ...
            if error_code == "AccessDenied":
                ...
                return DestinationTestStepResult(
                    status=Status.FAILED,
                    message=f"We couldn't assume '{self.aws_role_arn}' due to an 'AccessDenied' error. Ensure that the trust policy attached to the role allows 'sts:AssumeRole' for the Insights role, that the 'ExternalId' condition is set, and that the role actually exists.",
                )
