...
from insights.insightsql.printer.utils import print_prepared_ast
...
def prepare_executable_query(saved_query: DataWarehouseSavedQuery) -> None:
    """Rebuild the saved query's executable InsightsQL from its endpoint version.
    ...
    saved_query.query = build_endpoint_insightsql(
        version.query,
        saved_query.team,
...
class EndpointMaterializationService:
    ...
    def enable_materialization(
        ...
        try:
            ...
            raise
        except (ExposedInsightsQLError, MaterializationNotSupportedError) as e:
    ...
    def _configure_saved_query(
        ...
        """Point the saved query at the version's materializable InsightsQL and sync cadence."""
        saved_query.query = build_endpoint_insightsql(
            version.query,
            self.team,
    ...
    def preview(
        ...
        insightsql_query = convert_insight_query_to_insightsql(version.query, self.team, user=self.user)
        ...
        if version.query.get("variables"):
            ...
            if variable_infos:
                ...
                if query_str:
                    try:
                        ...
                        logger.warning(
                            "materialization_preview: converted InsightsQL failed to parse; "
            ...
            transformed_query_str = insightsql_query.get("query")
        ...
        try:
            ...
            # Type resolution (to_printed_insightsql) needs the materialized table to exist in the
            ...
            if version.is_materialized:
                execution_query_str = to_printed_insightsql(
