"""Helpers for batch exports powered by a user-defined InsightsQL query.
...
class UnsupportedInsightsQLQueryError(Exception):
    """Raised when a InsightsQL query cannot be used to power a batch export."""


def parse_insightsql_select_for_batch_export(insightsql_query: str) -> ast.SelectQuery | ast.SelectSetQuery:
    """Parse a InsightsQL SELECT query intended to power a batch export.

    Placeholders are not currently supported in batch exports, they will be coming soon...

    Raises:
        UnsupportedInsightsQLQueryError: If the query cannot be parsed as a SELECT or
            contains placeholders, which batch exports have no way to resolve.
        InternalInsightsQLError: Left to propagate. An internal InsightsQL engine error is our
    ...
    """
    try:
        parsed = parse_select(insightsql_query)
    except ExposedInsightsQLError as e:
        raise UnsupportedInsightsQLQueryError(f"Failed to parse InsightsQL query: {e}") from e
    ...
    if placeholders.has_filters or placeholders.placeholder_fields or placeholders.placeholder_expressions:
        raise UnsupportedInsightsQLQueryError("Placeholders are not supported in batch export queries")

    return parsed


def create_insightsql_context_for_batch_export(team: "Team", values: dict[str, typing.Any] | None = None) -> InsightsQLContext:
    """Build the InsightsQLContext batch exports use to resolve and print a query.
...
def _validate_select_columns_are_named(parsed: ast.SelectQuery | ast.SelectSetQuery) -> None:
    ...
    for expr in select_query.select:
        if not isinstance(expr, ast.Field | ast.Alias):
            raise UnsupportedInsightsQLQueryError(
                "Every column in the SELECT clause must be a field or have an alias (e.g. `count() AS event_count`)"
            )


def validate_insightsql_query_for_batch_export(insightsql_query: str, team: "Team") -> None:
    """Validate a InsightsQL query can power a batch export for the given team.
    ...
    Raises:
        UnsupportedInsightsQLQueryError: If the query cannot power a batch export.
        InternalInsightsQLError: Left to propagate, as in `parse_insightsql_select_for_batch_export`.
    """
    parsed = parse_insightsql_select_for_batch_export(insightsql_query)
    _validate_select_columns_are_named(parsed)

    context = create_insightsql_context_for_batch_export(team)
    try:
        prepare_ast_for_printing(parsed, context=context, dialect="datastore", stack=[])
    except ExposedInsightsQLError as e:
        raise UnsupportedInsightsQLQueryError(f"Invalid InsightsQL query: {e}") from e
