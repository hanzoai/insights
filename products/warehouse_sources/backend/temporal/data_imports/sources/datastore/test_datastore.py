...
from products.warehouse_sources.backend.temporal.data_imports.sources.datastore import datastore as ch_module
from products.warehouse_sources.backend.temporal.data_imports.sources.datastore.datastore import (
    ...
    filter_datastore_incremental_fields,
    get_primary_keys_for_schemas,
)
from products.warehouse_sources.backend.temporal.data_imports.sources.datastore.source import (
    _REDIRECTED,
...
class TestFilterDatastoreIncrementalFields:
    @pytest.mark.parametrize(
    ...
    def test_supported_types(self, type_name, expected_type):
        result = filter_datastore_incremental_fields([("col", type_name, False)])
    ...
    def test_preserves_nullable_flag(self):
        result = filter_datastore_incremental_fields([("col", "Nullable(Int64)", True)])
    ...
    def test_multiple_columns(self):
        ...
        result = filter_datastore_incremental_fields(columns)
        assert result == [
...
class TestBuildQuery:
    @staticmethod
    def _cols(*specs: tuple[str, str]) -> list[DatastoreColumn]:
        return [DatastoreColumn(name=n, data_type=t, nullable=False) for n, t in specs]
    ...
    def test_incremental_casts_date_last_value_through_todate32(self):
        ...
        # (Datastore's own on-disk representation), which `greater(Date, UInt16)`
    ...
    def test_wraps_arrow_unsupported_types_in_to_string(self):
        query, _ = _build_query(
            ...
            table_name="events",
            columns=[
                DatastoreColumn(name="id", data_type="Int64", nullable=False),
                DatastoreColumn(name="user_id", data_type="UUID", nullable=False),
                DatastoreColumn(name="ip", data_type="Nullable(IPv4)", nullable=True),
                DatastoreColumn(name="tags", data_type="Array(String)", nullable=False),
                DatastoreColumn(name="status", data_type="LowCardinality(Enum8('a' = 1))", nullable=False),
            ],
            should_use_incremental_field=False,
            incremental_field=None,
        )
...
class TestProjectColumns:
    @staticmethod
    def _cols(*names: str) -> list[DatastoreColumn]:
        return [DatastoreColumn(name=n, data_type="String", nullable=False) for n in names]

    @staticmethod
    def _names(cols: list[DatastoreColumn]) -> list[str]:
...
class TestGetDatastoreRowCount:
    ...
    def _run(self, responses):
        ...
        with patch.object(ch_module, "_get_client", return_value=client):
            return ch_module.get_datastore_row_count(
                host="h",
                port=1,
                database="default",
                user="u",
...
class TestDatastoreColumnToArrowField:
    @pytest.mark.parametrize(
    ...
    def test_simple_type_mappings(self, data_type, expected_arrow_type):
        col = DatastoreColumn("test_col", data_type, nullable=True)
    ...
    def test_datetime_no_tz(self):
        col = DatastoreColumn("ts", "DateTime", nullable=False)
        field = col.to_arrow_field()
        assert field.type == pa.timestamp("s")

    def test_datetime_with_tz(self):
        col = DatastoreColumn("ts", "DateTime('UTC')", nullable=False)
        field = col.to_arrow_field()
    ...
    def test_datetime64_precision(self, data_type, expected_unit):
        col = DatastoreColumn("ts", data_type, nullable=False)
        field = col.to_arrow_field()
        assert field.type == pa.timestamp(expected_unit)

    def test_datetime64_with_tz(self):
        col = DatastoreColumn("ts", "DateTime64(6, 'America/New_York')", nullable=False)
        field = col.to_arrow_field()
    ...
    def test_decimal_types(self, data_type, expected_precision, expected_scale):
        col = DatastoreColumn("amt", data_type, nullable=False)
        field = col.to_arrow_field()
    ...
    def test_nullable_wrapper(self):
        col = DatastoreColumn("id", "Nullable(Int64)", nullable=True)
        field = col.to_arrow_field()
        assert field.type == pa.int64()
        assert field.nullable is True

    def test_low_cardinality_wrapper(self):
        col = DatastoreColumn("name", "LowCardinality(String)", nullable=False)
        field = col.to_arrow_field()
        assert field.type == pa.string()
        assert field.nullable is False

    def test_nullable_low_cardinality(self):
        col = DatastoreColumn("name", "LowCardinality(Nullable(String))", nullable=True)
        field = col.to_arrow_field()
        assert field.type == pa.string()
        assert field.nullable is True

    def test_unknown_type_maps_to_string(self):
        col = DatastoreColumn("mystery", "AggregateFunction(uniq, UInt64)", nullable=True)
        field = col.to_arrow_field()
        assert field.type == pa.string()


class TestDatastoreSourceNonRetryableErrors:
    @pytest.fixture
    def source(self):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "Could not resolve the Datastore host",
            "Connection refused",
            ...
            "HTTPDriver for https://host:8443 received Datastore error code 241\n Code: 241. "
            ...
            "HTTPDriver for https://host:8443 received Datastore error code 243\n Code: 243. "
            ...
            # UNKNOWN_TYPE (code 50) — a column type Datastore can't serialize to Arrow,
            # e.g. an AggregateFunction state column on an aggregating materialized view.
            "HTTPDriver for https://host:8443 received Datastore error code 50\n Code: 50. "
            ...
            # Host answered 2xx with a non-Datastore body — `_get_client` wraps the driver's
...
class TestDatastoreSourceRetryableErrors:
    @pytest.fixture
    def source(self):
        return DatastoreSource()
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # from datastore-connect's HTTPDriver (no proxy CONNECT tunnel involved).
            "HTTPDriver for https://example.ngrok-free.dev:443 returned response code 502",
            "HTTPDriver for https://example.ngrok-free.dev:443 returned response code 503",
...
class TestIsTransientConnectDrop:
    @pytest.mark.parametrize(
        "message",
        [
            # The exact wrapped message that reached error tracking from Datastore Cloud.
            ...
            # returned a 502 while opening the CONNECT tunnel to a Datastore host.
            "Error HTTPSConnectionPool(host='h', port=8443): Max retries exceeded with url: /? "
            ...
            "Tunnel connection failed: 503 Service Unavailable",
            "Tunnel connection failed: 504 Gateway Timeout",
            # HTTP 429 rate-limit at connect time — datastore-connect doesn't
...
class TestGetClientTransientRetry:
    ...
    def test_retries_transient_drop_then_succeeds(self):
        client = MagicMock()
        ssl_eof = DatastoreError("[SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol")
        with (
    ...
    def test_retries_connect_time_429_then_succeeds(self):
        # datastore-connect raises OperationalError for a 429 exhausted at the
    ...
    def test_does_not_retry_deterministic_failure(self):
        cert_error = DatastoreError("certificate verify failed")
        with (
            patch.object(ch_module.time, "sleep") as mock_sleep,
    ...
    def test_wraps_non_datastore_response_probe_error(self):
        # datastore-connect's construction-time probe unpacks the reply into two values;
        # a non-Datastore 2xx body raises a bare ValueError. We wrap it as a connection
        ...
        with (
            ...
            with pytest.raises(DatastoreConnectionError, match="did not return a valid Datastore response"):
                self._connect()
        assert mock_get_client.call_count == 1
        mock_sleep.assert_not_called()
...
class TestTranslateError:
    def test_matches_substring_inside_long_error(self):
        ...
        assert DatastoreSource._translate_error(msg) == "Invalid user or password"

    def test_returns_none_for_unrecognised_error(self):
        assert DatastoreSource._translate_error("Some random error") is None
    ...
    def test_404_maps_to_wrong_interface_message(self):
        ...
        translated = DatastoreSource._translate_error(msg)
        assert translated is not None
        assert "404" in translated

    def test_non_datastore_response_maps_to_actionable_message(self):
        # The wrapped probe error must surface the actionable "not serving Datastore"
        # message during onboarding, not fall through to the generic one.
        assert DatastoreSource._translate_error(NOT_A_DATASTORE_HTTP_RESPONSE) == NOT_A_DATASTORE_HTTP_RESPONSE
    ...
    def test_transient_gateway_responses_ask_to_retry(self, code):
        ...
        assert DatastoreSource._translate_error(msg) == _TEMPORARILY_UNAVAILABLE
    ...
    def test_redirect_responses_name_the_redirect(self, code):
        ...
        msg = f"HTTPDriver for https://host:8443 returned response code {code}"
        assert DatastoreSource._translate_error(msg) == _REDIRECTED
...
class TestGetSchemas:
    """Tests `get_schemas` with a fully mocked Datastore client."""
...
class TestSourceClassValidateCredentials:
    ...
    def test_returns_error_when_datastore_connection_fails(self):
        from products.warehouse_sources.backend.temporal.data_imports.sources.datastore import source as source_module
        from products.warehouse_sources.backend.temporal.data_imports.sources.datastore.datastore import (
            DatastoreConnectionError,
        )

        source = source_module.DatastoreSource()

        config = MagicMock()
        config.host = "play.datastore.com"
        ...
        with patch.object(source, "ssh_tunnel_is_valid", return_value=(True, None)):
            with patch.object(source, "is_database_host_valid", return_value=(True, None)):
                with patch.object(
                    source,
                    "get_schemas",
                    side_effect=DatastoreConnectionError("Code: 516. Authentication failed"),
                ):
        ...
        with patch.object(source, "ssh_tunnel_is_valid", return_value=(True, None)):
            with patch.object(source, "is_database_host_valid", return_value=(True, None)):
                with patch.object(
                    source,
                    "get_schemas",
                    side_effect=DatastoreConnectionError("something weird happened"),
                ):
                    valid, msg = source.validate_credentials(config, team_id=1)
...
class TestHasDuplicatePrimaryKeys:
    ...
    def test_fails_safe_to_true_on_datastore_error(self):
        client = MagicMock()
        client.query.side_effect = DatastoreError("Code: 62. DB::Exception: Syntax error")
    ...
    def test_captures_unexpected_datastore_error(self):
        from products.warehouse_sources.backend.temporal.data_imports.sources.datastore import datastore as ch_module
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "HTTPDriver received Datastore error code 241\n Code: 241. DB::Exception: Query memory limit "
            ...
            "HTTPDriver received Datastore error code 159\n Code: 159. DB::Exception: Timeout exceeded: "
    ...
    def test_probe_budget_exhaustion_not_captured(self, error_msg):
        ...
        client = MagicMock()
        client.query.side_effect = DatastoreError(error_msg)
    ...
    @pytest.mark.parametrize(
        "error_msg",
        [
            # datastore-connect rejects readonly/unknown session settings
            # client-side (Datastore Cloud, readonly user profiles).
            ...
            # max_execution_time cap does (e.g. Datastore Cloud cold-resume).
            "Error HTTPSConnectionPool(host='example.datastore.cloud', port=8443): Read timed out. (read timeout=120)",
        ],
    )
    def test_expected_probe_failures_not_captured(self, error_msg):
        ...
        with patch(
            "products.warehouse_sources.backend.temporal.data_imports.sources.datastore.datastore.capture_exception"
        ) as mock_capture:
    ...
    def test_unexpected_probe_failures_are_captured(self):
        client = MagicMock()
        client.query.side_effect = DatastoreError("Code: 999. DB::Exception: Keeper exception")
...
class TestGetIncrementalRowCount:
    ...
    def test_returns_none_on_error(self):
        client = MagicMock()
        client.query.side_effect = DatastoreError("timeout")
...
class TestGetRowsBatching:
    ...
    def _run_get_rows(self, blocks):
        """Invoke `datastore_source(...).items()` against a stream of `blocks`."""
        from contextlib import contextmanager

        from products.warehouse_sources.backend.temporal.data_imports.sources.datastore import datastore as ch_module

        # Minimal discovery stubs so datastore_source builds a SourceResponse.
        ...
        with (
            ...
            patch.object(ch_module, "get_datastore_row_count", return_value={}),
        ):
            response = ch_module.datastore_source(
                tunnel=fake_tunnel,
                user="u",
                password="p",
                database="db",
...
class TestDatastoreReconcileSchemaMetadata(BaseTest):
    """The Datastore-specific override that routes through the shared reconcile helper."""

    def test_persists_schema_metadata_for_datastore_source(self) -> None:
        source = ExternalDataSource.objects.create(team=self.team, source_type=ExternalDataSourceType.DATASTORE)
        schema = ExternalDataSchema.objects.create(team=self.team, source=source, name="events")

        result = DatastoreSource().reconcile_schema_metadata(
            source=source,
            source_schemas=[
                SourceSchema(
                    name="events",
...
class TestBypassEnvProxy:
    """The proxy bypass for Insights-internal direct connections.

    `bypass_env_proxy=True` must hand datastore-connect a pool manager, which
    ...
    def test_eviction_releases_the_manager_everywhere_it_is_retained(self):
        ...
        # bounded cache only helps if eviction also drops the manager from datastore-connect's
        # process-global registry, the other place that would retain it for the worker's life.
        from datastore_connect.driver.httputil import all_managers
...
class TestGetClientEgressProxyRouting:
    ...
    Insights's runtime points HTTP_PROXY at the Smokescreen egress proxy, and Smokescreen blocks
    loopback. Routing a tunneled connection through it means the request never reaches the SSH
    tunnel's local forwarded port, so the tunnel opens no channel to the customer's Datastore
    ...
    def test_tunneled_https_verifies_against_the_database_hostname(self) -> None:
        ...
        # between the SSH server and Datastore.
        with patch.object(ch_module, "get_client") as mock_get_client:
            _get_client(
                host="127.0.0.1",
                port=8443,
                ...
                server_hostname="datastore.internal",
            )
        manager = mock_get_client.call_args.kwargs["pool_mgr"]
        pool = manager.connection_from_host("127.0.0.1", 8443, scheme="https")
        assert pool.assert_hostname == "datastore.internal"
        assert pool.conn_kw["server_hostname"] == "datastore.internal"
...
class TestDirectQueryClientBypassEnvProxy:
    """`direct_query_client` backs the InsightsQL direct-SQL adapter, and unlike every other
    `_get_client` call site on `DatastoreSource` it once omitted `bypass_env_proxy` entirely —
    an allowlisted internal team's direct query silently routed through the egress proxy and
    failed to reach the Insights-internal host it was pointed at.

    A tunneled connection must bypass for a different reason: the address is our own loopback
    bind, which the proxy blocks, so a customer team with a tunnel never reached its Datastore.
    """
