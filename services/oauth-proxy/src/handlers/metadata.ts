 ...
 *
 * Returns metadata pointing to oauth.hanzo.ai endpoints.
 * MCP clients and other OAuth integrations use this to discover where to register,
 * authorize, and exchange tokens.
 *
 * We always cache the information exposed by the authoritative source - us.hanzo.ai
 * and simply alter the endpoint-related fields to point to oauth.hanzo.ai.
 *
 * The actual underlying metadata is defined in insights/api/oauth/views.py#OAuthAuthorizationServerMetadataView.get()
 */
...
async function fetchAuthoritativeMetadata(): Promise<WellKnownOAuthAuthorizationServerMetadata> {
    const response = await fetch('https://us.hanzo.ai/.well-known/oauth-authorization-server')
    if (!response.ok) {
...
export async function handleMetadata(request: Request): Promise<Response> {
    ...
    // Alter the endpoint-related fields to point to the oauth.hanzo.ai base domain.
