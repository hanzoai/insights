...
class TestLazyJoins(BaseTest):
    ...
    def _schema_snapshot(self, printed: str):
        ...
        if settings.DATASTORE_INSIGHTSQL_USE_NEW_EVENTS_SCHEMA and "events_json" in printed:
    ...
    def test_sample_on_lazy_table_is_stripped(self):
        # A lazy table expands into an aggregating subquery; Datastore rejects a SAMPLE modifier on
        ...
        context = InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, modifiers=InsightsQLQueryModifiers())
        expr = parse_select("select session_id, $channel_type from sessions SAMPLE 10")
        printed, _ = prepare_and_print_ast(expr, context, "datastore")
    ...
    def test_person_override_join_ordered_before_dependent_warehouse_joins(self):
        ...
        # join must still come before the dependent one: Datastore resolves joins left-to-right, so
