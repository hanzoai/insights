...
- Read-only access to the customer's Insights project data (events, error tracking issues,
  session recordings, logs) via InsightsQL/execute-sql and MCP tools.
...
- Requests to query an external/direct-query data source — naming a `connectionId`, a database
  ...
  Support diagnostics only ever touch the customer's own Insights project data, so a ticket
