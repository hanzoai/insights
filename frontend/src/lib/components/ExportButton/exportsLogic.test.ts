    ...
    describe('createExport toast', () => {
        ...
        it.each(createCases)(
            ...
            async ({
                ...
                // The loading spinner is driven by toast.promise, so the user always sees "Preparing export…".
                ...
                const toastOptions = jest.mocked(toast.promise).mock.calls[0][2]
                expect(toastOptions?.button?.label).toEqual(expectsViewExportsButton ? 'View exports' : undefined)
                const runPromise = jest.mocked(toast.promise).mock.calls[0][0]
        ...
        it('offers a Download button when the create call outlived the user gesture', async () => {
            ...
            try {
                ...
                jest.mocked(toast.success).mock.calls.at(-1)![1]!.button!.action()
        ...
        it('notifies once with a Download button that routes through downloadExport', async () => {
            ...
            expect(jest.mocked(toast.success).mock.calls).toEqual([
                [
                    'Export complete!',
                    expect.objectContaining({ button: expect.objectContaining({ label: 'Download' }) }),
                ],
            ...
            jest.mocked(toast.success).mock.calls[0][1]!.button!.action()
        ...
        it('surfaces the failure and stops tracking when a tracked async export fails', async () => {
            ...
            expect(toast.error).toHaveBeenCalledWith('Export failed: render crashed')
