...
correct. Insights precomputes exposure data on a schedule; until the first precomputation lands,
...
insights/insightsql_queries/experiments/experiment_query_runner.py. Verify before citing. -->
...
Insights SDKs deduplicate `$feature_flag_called` to avoid flooding ingestion with identical exposure
events. The _scope_ of "duplicate" varies by SDK:

- **insights-js** dedupes per identity across sessions by default. Returning users who evaluated the
  ...
  cache each session.
- **insights-node / hanzo_insights (Python)** dedupe per `distinct_id` within the process
...
- **Web with returning users (`insights-js`).** In the SDK init config, set
...
- **Server-side long-lived workers (`insights-node`, `hanzo_insights`).** Two paths, pick one:
...
the custom exposure event just like any other experiment. Insights uses the property for _variant
...
Common cause of partial or zero data. The SDK call goes out, but the request never reaches Insights.

**Fix:** set up a [reverse proxy](https://hanzo.ai/docs/advanced/proxy) so capture requests come from
...
1. **Read the live filter back to the user.** Summarize the rows in plain language so they can
   ...
   specific cohorts).
2. **Estimate the exclusion rate.** For each filter row, translate to InsightsQL and count events that
...
- B2 (`identify()` timing)
- `insights is not defined` (SDK init order — initialize Insights before any flag call)
- The flag is genuinely off — `feature_flag.active === false`, or rollout `0%`, or the user is outside
  release conditions

**Fix:** walk the user through their SDK setup. Verify in this order: (a) is Insights initialized?
...
`insights:diagnosing-sdk-health` skill — outdated SDKs are a frequent root cause of the "no exposures
