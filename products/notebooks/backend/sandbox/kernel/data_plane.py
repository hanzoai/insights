"""Client for the backend data plane — the sandbox's only read path to Insights data.

POSTs a InsightsQL query to the backend's data-plane endpoint (authed with the
...
the rows come back as an Arrow stream. No backend web worker waits on Datastore
...
# Total budget for one inline query: enqueue + Celery pickup + Datastore execution.
...
def _table_to_rows_and_types(table: "pa.Table") -> tuple[list[str], list[tuple[Any, ...]], list[list[str]]]:
    ...
    Types come from the `insightsql_types` schema metadata the data plane attaches (the
    real Datastore type names); when absent they are approximated from the Arrow
    ...
    raw_types = metadata.get(b"insightsql_types")
    if raw_types:
        try:
