...
# SDK / InsightsQL function names that legitimately appear as code in skills (e.g. `emit_signal(...)`,
...
# in skill prose. The near-miss gate keeps SDK/InsightsQL examples (e.g. `get_feature_flag(...)`) quiet,
...
class SkillRenderer:
    ...
    def __init__(self) -> None:
        from products.insights_ai.scripts.audit_constants import audit_constants
        ...
        self.env = _create_jinja_env(
            ...
            render_insightsql_example=render_insightsql_example,
            insightsql_functions=insightsql_functions,
...
class SkillBuilder:
    ...
    _SYNCED_SKILLS_MARKER = "# Synced product skills (managed by insightscli sync:skill)"
...
def main() -> None:
    ...
    if args.sync:
        if not args.name:
            builder.list_skills()
            print("\nUsage: insightscli sync:skill -- --name <skill-name>")
            return
        if args.clean:
