...
from insights.insightsql.errors import ExposedInsightsQLError, InternalInsightsQLError, ResolutionError
...
from insights.slo.context import SloSpec, slo_operation
from insights.slo.types import SloArea, SloOperation
...
from products.exports.backend.temporal.subscriptions.ai_subscription.prompts import (
    ...
    INSIGHTSQL_FIX_PROMPT,
    INSIGHTSQL_FIX_PROMPT_NAME,
...
# Wall-clock bounds for the in-band LLM + InsightsQL pipeline. The caller's outer deadline (Temporal
...
_INSIGHTSQL_STEP_TIMEOUT_SECONDS = 60.0
...
# Per-step query-fix budget: the planner occasionally emits InsightsQL that fails to parse, so we feed the
...
# The planner may emit up to MAX_QUERY_PLAN_STEPS steps; bound how many run their Datastore query at
...
def _safe_error_message(exc: BaseException) -> Optional[str]:
    # InsightsQL/Datastore error text can echo team-scoped identifiers, so only the query-structure error
    # classes (which describe the field/property the planner referenced) are safe to surface to the
    # subscription owner — the same trust boundary the InsightsQL repair loop uses when forwarding to the
    ...
    while current is not None and id(current) not in seen:
        if isinstance(current, (ExposedInsightsQLError, ResolutionError)):
            return str(current)
        seen.add(id(current))
...
class QueryStepDiagnostic:
    # Per-step audit record persisted to the delivery's content_snapshot. The generated InsightsQL and the
...
def _plan_to_freeze(
    ...
    *,
    ...
    # Steps already carry their final InsightsQL by this point — see the write-back in `run_step`.
    # Never freeze a plan the next delivery is better off re-planning: a plan with any failed step would
    # replay that broken InsightsQL every run, and a step without any window placeholder would scan unbounded
    # every run.
    ...
    # InsightsQL every delivery, whereas re-planning gives the planner and fix loop another shot (and lets the
    ...
    if failed_count:
        ...
        return None
    if not all(any(token in step.insightsql for token in WINDOW_PLACEHOLDERS) for step in plan.steps):
        logger.warning(
...
async def _synthesize(
    ...
    insights_properties: dict[str, Union[str, int]] = {
        "feature": "ai_subscription",
        "stage": "synthesis",
        "trace_id": str(uuid.uuid4()),
    }
    if trace_correlation_id is not None:
        insights_properties["subscription_id"] = trace_correlation_id
...
async def _run_steps(
    ...
    async def run_step(step: QueryPlanStep) -> tuple[str, QueryStepDiagnostic]:
        # `current_insightsql` keeps the window-agnostic form (the `{{date_range}}` placeholder) so it round-trips
        # through the fix LLM unchanged; the run's fresh bounds are substituted into `executable_insightsql` on
        # every attempt. The diagnostic records the executed SQL (placeholder resolved) for debugging.
        current_insightsql = step.insightsql
        ...
        for attempt in range(_MAX_QUERY_FIX_RETRIES + 1):
            ...
            try:
                ...
                step.insightsql = current_insightsql
                return (
                    f"### {safe_description}\n\n{safe_formatted}",
                    QueryStepDiagnostic(description=safe_description, insightsql=executable_insightsql, ok=True, error_type=None),
                )
            except Exception as exc:
                last_exc = exc
                ...
                fixed = await _arequest_insightsql_fix(
                    original_insightsql=current_insightsql,
                ...
                if not fixed or fixed.strip() == current_insightsql.strip():
                    break
                current_insightsql = fixed
        ...
        # type only — Datastore errors can echo team-scoped identifiers
        ...
        return (
            ...
            QueryStepDiagnostic(
                ...
                insightsql=window.render_window_filter(current_insightsql),
                ok=False,
    ...
    async def run_step_bounded(step: QueryPlanStep) -> tuple[str, QueryStepDiagnostic]:
        # Hold a slot for the whole step (query + any fix/rerun) so concurrent Datastore load stays bounded.
...
def _fix_project_context_block(context_blob: str) -> str:
    ...
    # ai-subscription-insightsql-fix prompt. The <project_context> is untrusted data, framed as such.
    return (
...
async def _arequest_insightsql_fix(
    *,
    original_insightsql: str,
    error_message: str,
    ...
    insights_properties: dict[str, Union[str, int]] = {"feature": "ai_subscription", "stage": "query_fix"}
    if trace_correlation_id is not None:
        insights_properties["subscription_id"] = trace_correlation_id
    ...
    llm = MaxChatOpenAI(
        ...
        billable=True,
        insights_properties=insights_properties,
    ).with_structured_output(InsightsQLFix, method="json_schema", include_raw=False)

    fix_prompt = await database_sync_to_async(resolve_prompt, thread_sensitive=False)(
        team, INSIGHTSQL_FIX_PROMPT_NAME, INSIGHTSQL_FIX_PROMPT
    )
    rendered = render_prompt(
        fix_prompt,
        {"description": step_description, "error": error_message, "original_insightsql": original_insightsql},
    )
    ...
    if not isinstance(result, InsightsQLFix):
        return None
    fixed = result.fixed_insightsql.strip()
