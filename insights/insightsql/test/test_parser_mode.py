...
from insights.schema import ParserMode

from insights.insightsql import (
    ast,
    parser as parser_module,
)
from insights.insightsql.errors import SyntaxError as InsightsQLSyntaxError
...
class TestParserMode(BaseTest):
    ...
    def test_resolve_parser_mode_default_shadows_in_prod(self):
        with patch("insights.insightsql.parser.settings") as mock_settings:
    ...
    def test_resolve_parser_mode_drops_shadow_when_rust_unavailable(self):
        with patch("insights.insightsql.parser._RUST_PARSER_AVAILABLE", False):
    ...
    def test_client_cpp_parser_mode_does_not_reach_the_parser(self):
        ...
        from insights.insightsql.query import InsightsQLQueryExecutor  # noqa: PLC0415

        executor = InsightsQLQueryExecutor(
            query="select 1",
            team=self.team,
            modifiers=InsightsQLQueryModifiers(parserMode=ParserMode.CPP_ONLY),
        )
        with patch("insights.insightsql.query.parse_select", wraps=parse_select) as spy:
            executor._parse_query()
        self.assertIsNone(spy.call_args.kwargs["parser_mode"])

    def test_shadow_silent_when_backends_agree(self):
        with patch("insights.insightsql.parser._SHADOW_SAMPLE_RATE", 1.0):
            with patch("insights.insightsql.parser.capture_exception") as captured:
    ...
    def test_shadow_raises_mismatch_in_test_mode(self):
        ...
        with patch("insights.insightsql.parser._invoke_parser", side_effect=only_shadow_diverges):
            with self.assertRaises(InsightsQLParserShadowMismatch):
                parse_select(
    ...
    def test_shadow_captures_mismatch_without_raising_in_prod(self):
        ...
        self.assertIsInstance(captured.call_args.args[0], InsightsQLParserShadowMismatch)
    ...
    def test_shadow_raises_when_shadow_rejects_primary_accepted_input_in_test_mode(self):
        ...
        def shadow_throws_parser_class(backend, rule, statement, start):
            if backend == "rust-json":
                raise InsightsQLSyntaxError("simulated rust-json regression")
            return real_invoke(backend, rule, statement, start)

        with patch("insights.insightsql.parser._invoke_parser", side_effect=shadow_throws_parser_class):
            with patch("insights.insightsql.parser.capture_exception"):
                with self.assertRaises(InsightsQLSyntaxError):
    ...
    def test_shadow_rejection_counts_and_captures_sql_without_raising_in_prod(self):
        ...
        def shadow_throws_parser_class(backend, rule, statement, start):
            if backend == "rust-py":
                raise InsightsQLSyntaxError("simulated rust-py rejection")
            return real_invoke(backend, rule, statement, start)
        ...
        with patch("insights.insightsql.parser.settings") as mock_settings:
            ...
            with patch("insights.insightsql.parser._SHADOW_SAMPLE_RATE", 1.0):
                with patch("insights.insightsql.parser._invoke_parser", side_effect=shadow_throws_parser_class):
                    with patch("insights.insightsql.parser._SHADOW_COMPARISONS") as counter:
                        with patch("insights.insightsql.parser.capture_exception") as captured:
                            node = parse_select(
        ...
        self.assertIn("shadow_rejected_probe", props["insightsql_parser_statement"])
        self.assertEqual(props["insightsql_parser_shadow_version"], parser_module._BACKEND_VERSION["rust-py"])
    ...
    def test_shadow_swallows_non_parser_packaging_error_even_in_test_mode(self):
        ...
        with patch("insights.insightsql.parser._invoke_parser", side_effect=shadow_throws_packaging):
            with patch("insights.insightsql.parser.capture_exception") as captured:
                node = parse_select("select 1 from events", parser_mode=ParserMode.CPP_WITH_RUST_SHADOW)
        self.assertIsInstance(node, ast.SelectQuery)
        captured.assert_called_once()
    ...
    def test_shadow_divergence_counts_as_disagree_and_captures_sql_in_prod(self):
        ...
        self.assertIsInstance(captured.call_args.args[0], InsightsQLParserShadowMismatch)
        props = captured.call_args.kwargs["additional_properties"]
        self.assertEqual(props["insightsql_parser_rule"], "select")
        self.assertIn("sql_attach_probe", props["insightsql_parser_statement"])
        self.assertEqual(props["insightsql_parser_primary_version"], parser_module._BACKEND_VERSION["cpp-json"])
        self.assertEqual(props["insightsql_parser_shadow_version"], parser_module._BACKEND_VERSION["rust-py"])
