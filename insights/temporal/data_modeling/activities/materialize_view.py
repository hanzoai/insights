...
from insights.insightsql.errors import ParsingError
...
# Limits concurrent Datastore queries per worker. Each worker pod runs a single
...
MAX_CONCURRENT_DATASTORE_QUERIES = 10
_datastore_query_semaphore = asyncio.Semaphore(MAX_CONCURRENT_DATASTORE_QUERIES)


class EmptyInsightsQLResponseColumnsError(Exception):
    def __init__(self):
        super().__init__("After running a InsightsQL query, no columns were returned")
...
def _force_nullable(batch: pa.RecordBatch) -> pa.RecordBatch:
    ...
    Datastore emits non-nullable columns for expressions, constants, concat()/toString(),
...
async def _write_empty_parquet_for_zero_rows(table_uri: str, schema: pa.Schema, logger: FilteringBoundLogger) -> str:
    ...
    # write_table() on an empty table emits a 0-row row group, which Datastore rejects,
...
async def insightsql_table(query: str, team: Team, logger: FilteringBoundLogger, view_name: str | None = None):
    """Execute a InsightsQL query and yield batches of results."""
    ...
    # Userless materialization context; bypass warehouse InsightsQL access control so the model query
    ...
    prepared_insightsql_query = await database_sync_to_async_pool(prepare_ast_for_printing)(
        query_node,
        context=context,
        dialect="datastore",
        settings=settings,
    ...
    async with _datastore_query_semaphore, get_datastore_client() as client:
    ...
    # to view resolution
    arrow_prepared_insightsql_query = await database_sync_to_async_pool(prepare_ast_for_printing)(
        query_node,
        context=context,
        dialect="datastore",
        stack=[],
    ...
    async with (
        ...
        get_datastore_client(max_block_size=DATASTORE_MAX_BLOCK_SIZE_ROWS) as client,
    ):
        batches = []
        batches_size = 0
...
async def materialize_view_activity(inputs: MaterializeViewInputs) -> MaterializeViewResult:
    ...
    async with Heartbeater():
        ...
        insightsql_query = typing.cast(dict, objects.saved_query.query)["query"]
        ...
        async for batch, ch_types in insightsql_table(insightsql_query, objects.team, logger):
