...
from insights.insightsql.resolver import Resolver, ResolverFactory
...
class BoundedResolverError(QueryError):
    """Base class for bounded InsightsQL view-dependency resolution failures.

    Inherits from QueryError so callers that catch the public InsightsQL exception
...
DAG_RESOLUTION_TOTAL = Counter(
    ...
    "Total InsightsQL view-dependency resolutions performed, labelled by terminal status.",
...
DAG_RESOLUTION_DURATION_SECONDS = Histogram(
    ...
    "Wall-clock time spent resolving InsightsQL view dependencies.",
    buckets=(0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0, 30.0),
)
DAG_RESOLUTION_VIEW_DEPTH = Histogram(
    "data_modeling_dag_resolution_view_depth",
    "Maximum nested view depth observed on successful InsightsQL dependency resolution.",
...
def _scope_for_union_branches(select_set: ast.SelectSetQuery, scope: CteScope) -> CteScope:
    ...
    A leading WITH covers every branch of the union, but InsightsQL parses it onto the first
...
def get_parents_from_model_query(team: Team, model_name: str, model_query: str) -> set[str]:
    ...
    if context.database is None:
        # Internal DAG parsing (no user); bypass warehouse InsightsQL access control so parent-table
    ...
    resolver = BoundedResolver(context=context, dialect="insightsql", initial_view_name=model_name)
...
class DataWarehouseModelPathManager(models.Manager["DataWarehouseModelPath"]):
    ...
    def get_insightsql_database(self, team: Team) -> Database:
        ...
        # Internal model-path resolution (no user); bypass warehouse InsightsQL access control.
    ...
    def update_paths_from_query(
        ...
        # include_hidden surfaces `insights.*`-namespaced tables (e.g. insights.ai_events) so they
        # resolve as parents here too — matching the create path's get_table()-based resolution.
        database = self.get_insightsql_database(team)
        insights_table_names = set(database.get_insights_table_names()) | set(
            database.get_insights_table_names(include_hidden=True)
        )
