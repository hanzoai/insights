...
from insights.temporal.messaging.backfill_precalculated_events_workflow import flush_kafka_batch_async
...
from insights.temporal.messaging.filter_storage import combine_filter_bytecodes, extract_person_property_filters
...
# Datastore. This workflow is retained only until the Python implementation is deleted, so the
...
# Latest surviving mapping per overridden distinct_id; mirrors the InsightsQL
...
# Cohort membership here is queried by person_id (InsightsQLRealtimeCohortQuery), not distinct_id,
...
async def reconcile_team_precalculated_events_activity(inputs: ReconcileTeamInputs) -> ReconcileTeamResult:
    ...
    async with Heartbeater(details=(f"Loading person overrides for team {inputs.team_id}",)) as heartbeater:
        with tags_context(
            ...
            async with get_messaging_client(team_id=inputs.team_id) as client:
                kafka_producer = get_producer(topic=KAFKA_CDP_DATASTORE_PREFILTERED_EVENTS)
                kafka_results: list = []
                rows_checked = 0
                rows_corrected = 0
                overridden_distinct_ids = 0
                ...
                async def reconcile_batch(batch_overrides: dict[str, str]) -> None:
                    ...
                    async for row in client.stream_query_as_jsonl(
                        ...
                        produce_result = await asyncio.to_thread(
                            kafka_producer.produce,
                            topic=KAFKA_CDP_DATASTORE_PREFILTERED_EVENTS,
                            data={
                                "uuid": str(row["uuid"]),
                                "team_id": inputs.team_id,
                                "person_id": current_person_id,
...
class ReconcilePrecalculatedDataWorkflow(InsightsWorkflow):
    ...
    async def run(self, inputs: ReconcilePrecalculatedDataWorkflowInputs) -> None:
        ...
        async def reconcile_team(team_id: int) -> tuple[ReconcileTeamResult, ReconcilePersonPropertiesResult]:
            ...
            # Sequential, not gathered: both activities touch the same team's Datastore data,
