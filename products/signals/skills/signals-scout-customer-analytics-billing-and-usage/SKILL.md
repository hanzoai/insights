...
description: >
  ...
  inbox.
compatibility: >
  Designed for the Insights Signals agent in a Claude sandbox with Insights MCP scopes:
...
- **Billed usage** (your target): the traffic the account's own customers generate through the account's Insights SDKs, pre-aggregated in the billing views. This is what you score.
- **Insights-app engagement** (context only): this project's `events`, keyed by the `organization` group — the account's team members using the Insights app itself. It can tell you whether humans are still logging in; it can never confirm or deny a billed-usage move, because billed traffic does not flow through this project's event stream.
...
- **Verify the billing views and their account join.**
  ...
  - `billing_invoice_line_items_by_org` — one row per org/period/product (`cleaned_description`, `amount` in cents, `period_end`). Monthly grain — the per-product MRR share. Exclude `cleaned_description LIKE 'Insights Cloud Credit%'`.
...
- **The account grain for app-engagement context is configured, not discovered.**
  ...
  Use it only for the Insights-app engagement context reads — never as a billed-usage source.
...
WHERE organization_id = {org_id}
  ...
  AND cleaned_description NOT LIKE 'Insights Cloud Credit%'
...
Insights-app engagement is the supporting witness, not the scorer: if the account's team activity (`$group_0`-keyed `events`) is unchanged while their billed volume doubled, nobody is rolling out a feature — lean loop.
...
- **Author** when the move clears every gate: >30% vs the same-weekday 4-week baseline, account total flat (quantify both), staked account, share floor respected, seasonality checked, context sweep done.
  ...
  **Route `suggested_reviewers` to the account's managers** — the users holding an _active relationship_ on the account:
  ...
  JOIN postgres.insights_user AS u ON u.id = rel.user_id
  ...
  Never route from the account's CRM `properties` fields — `account_owner` names the champion inside the customer's own org, never a notification target; only relationship rows are Insights-side assignments, and the emit path validates each `user_uuid` is a project member anyway.
...
- `execute-sql` — the primary scorer: `system.accounts` (roster, staking, CRM ids), the billing views from Orient, `system.account_relationships` + `system.account_relationship_definitions` + `postgres.insights_user` (reviewer routing), and `$group_0`-keyed `events` for app-engagement context only.
