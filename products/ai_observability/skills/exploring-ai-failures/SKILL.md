...
| `insights:llma-evaluation-list`           | Find existing evals whose failures might reveal a new mode               |
| `insights:llma-evaluation-summary-create` | Summarize an existing eval's failures into patterns                      |
| `insights:generate-app-url`               | Build a region- and project-qualified deep link to a trace or list       |
...
`query-llm-trace` does not return a `_insightsUrl`, so build links with `insights:generate-app-url` —
...
`https://us.hanzo.ai/project/<id>/ai-observability/traces/<trace_id>`), so a user not already on the
