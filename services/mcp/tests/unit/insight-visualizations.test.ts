    ...
    describe('inferVisualizationType', () => {
        describe('inference from result shape (captured API fixtures)', () => {
            it.each([
                ...
                ['insightsql table', insightResults.insightsqlTable, 'table'],
        ...
        describe('fallback to query kind', () => {
            ...
            it.each([
                ['DataVisualizationNode wrapping InsightsQLQuery', 'DataVisualizationNode', 'InsightsQLQuery', 'table'],
        ...
        describe('type guards', () => {
            it.each([
                ...
                ['isInsightsQLResult', isInsightsQLResult, insightResults.insightsqlTable],
            ...
            it('isInsightsQLResult requires both columns and results to be arrays', () => {
                expect(isInsightsQLResult({ columns: ['a'], results: 'nope' })).toBe(false)
                expect(isInsightsQLResult({ columns: ['a'] })).toBe(false)
            })
        })
    })
