...
from insights.insightsql.database.models import IntegerDatabaseField, SavedQuery, StringDatabaseField, TableNode
from insights.insightsql.database.schema.events import EventsTable
from insights.insightsql.database.schema.util.where_clause_extractor import EventsPredicatePushdownExtractor
from insights.insightsql.parser import parse_select
from insights.insightsql.printer.utils import prepare_and_print_ast
...
from insights.insightsql.transforms.events_predicate_pushdown import (
...
class TestEventsPredicatePushdownTransform(BaseTest):
    ...
    def _print_select(self, select: str, modifiers: InsightsQLQueryModifiers | None = None):
        ...
        query, _ = prepare_and_print_ast(
            ...
            InsightsQLContext(
                ...
                modifiers=modifiers if modifiers is not None else InsightsQLQueryModifiers(pushDownPredicates=True),
            ),
            "datastore",
        )
    ...
    def test_unexpected_internal_error_degrades_to_flat_query(self):
        ...
        flat = self._print_select(select, modifiers=InsightsQLQueryModifiers(pushDownPredicates=False))
        with patch.object(EventsPredicatePushdownTransform, "_build_subquery", side_effect=RuntimeError("boom")):
            guarded = self._print_select(select, modifiers=InsightsQLQueryModifiers(pushDownPredicates=True))
    ...
    def test_restricted_property_still_stripped_after_pushdown(self):
        ...
        def _print(push_down: bool) -> str:
            context = InsightsQLContext(
                team_id=self.team.pk,
                enable_select_queries=True,
                modifiers=InsightsQLQueryModifiers(pushDownPredicates=push_down),
            )
            context.restricted_properties = {("email", PropertyDefinition.Type.EVENT)}
            query, _ = prepare_and_print_ast(parse_select(select), context, "datastore")
            return pretty_print_in_tests(query, self.team.pk)
    ...
    def test_non_pushable_prewhere_bails(self):
        ...
        # bails rather than emit an invalid PREWHERE on a subquery. (Such a query is already invalid Datastore
...
class TestEventsPredicatePushdownTransformUnit:
    ...
    def test_should_apply_pushdown_with_valid_query(self):
        ...
        context = InsightsQLContext(team_id=1)
    ...
    def test_should_not_apply_pushdown_without_effective_limit(self):
        ...
        transform = EventsPredicatePushdownTransform(InsightsQLContext(team_id=1))
    ...
    def test_should_apply_pushdown_for_top_level_without_explicit_limit(self):
        ...
        transform = EventsPredicatePushdownTransform(InsightsQLContext(team_id=1), top_level_select_ids={id(node)})
...
class TestEventsSubexprHoister(BaseTest):
    ...
    def setUp(self):
        ...
        self.context = InsightsQLContext(team_id=self.team.pk, database=self.database, enable_select_queries=True)

    def _run(self, query: str) -> tuple[EventsSubexprHoister, list[ast.Expr], ast.SelectQueryAliasType]:
        resolved = resolve_types(parse_select(query), self.context, dialect="datastore")
...
# Result-equivalence tests that run real Datastore queries. Unlike the printer tests above (which only
...
# any SQL Datastore rejects (e.g. an outer-alias leak), which string snapshots cannot catch.
class _PushdownExecutionTestBase(DatastoreTestMixin, APIBaseTest):
    snapshot: Any
    ...
    def _modifiers(self, *, push_down: bool) -> InsightsQLQueryModifiers:
        extra = {"propertyGroupsMode": self._property_groups_mode} if self._property_groups_mode is not None else {}
        return InsightsQLQueryModifiers(pushDownPredicates=push_down, **extra)
    ...
    def _results(self, select: str, *, push_down: bool):
        return execute_insightsql_query(select, team=self.team, modifiers=self._modifiers(push_down=push_down)).results
...
class TestEventsPredicatePushdownExecution(_PushdownExecutionTestBase):
    ...
    def test_exec_cohort_predicate_declines_and_stays_equivalent(
        ...
        def mods(push: bool) -> InsightsQLQueryModifiers:
        ...
        printed, _ = prepare_and_print_ast(
            parse_select(select),
            InsightsQLContext(team_id=self.team.pk, enable_select_queries=True, modifiers=mods(True)),
            "datastore",
        )
        assert ") AS events LEFT JOIN" not in printed, f"{_name}: cohort predicate must not push:\n{printed}"
        on = execute_insightsql_query(select, team=self.team, modifiers=mods(True)).results
        off = execute_insightsql_query(select, team=self.team, modifiers=mods(False)).results
    ...
    def test_exec_poe_person_property_filter(self, _name, poe_mode, expect_push):
        ...
        def mods(push: bool) -> InsightsQLQueryModifiers:
            return InsightsQLQueryModifiers(pushDownPredicates=push, personsOnEventsMode=poe_mode)
...
class TestEventsPredicatePushdownMaterializedExecution(_PushdownExecutionTestBase):
    ...
    The test Datastore has no materialized columns by default, so the rest of the suite never prints
    ...
    def _create_data(self) -> None:
        ...
        # column is computed at insert time (Datastore MATERIALIZED columns are evaluated on INSERT).
...
class TestEventsPredicatePushdownPropertyGroupsExecution(_PushdownExecutionTestBase):
    """Pushdown must stay a pure optimization under propertyGroupsMode=OPTIMIZED (the Insights Cloud default).
