...
describe('reverseProxyCheckerLogic', () => {
    ...
    it('should swallow server errors silently instead of showing a toast', async () => {
        // Regression test: previously a 500 from the InsightsQL endpoint would propagate
        ...
        const captureExceptionSpy = jest.spyOn(insights, 'captureException').mockImplementation(() => undefined)
        ...
        expect(captureExceptionSpy).toHaveBeenCalledWith(
            ...
            expect.objectContaining({ insights_source: 'reverseProxyCheckerLogic.loadHasReverseProxy' })
        )
