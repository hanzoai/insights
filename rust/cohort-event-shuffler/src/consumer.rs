//! Pipelined consume → filter → re-key → produce loop for `datastore_events_json`.
//!
...
/// Cheap gate parse for every event; a full `DatastoreEvent` parse only for survivors, over the
...
fn decode_gated(payload: &[u8], team_index: &TeamIndex) -> Decoded {
    ...
    match classify_gate(&gate, team_index) {
        ...
        GateOutcome::Forward => {
            let Ok(mut event) = serde_json::from_slice::<DatastoreEvent>(payload) else {
                return Decoded::Unparseable;
            };
...
impl EventShuffler {
    ...
    fn intake(
        ...
        let forward = Box::new(PendingForward {
            event: CohortStreamEvent::from_datastore(
                *event,
                person_id,
                partition.0,
                source_offset.0,
...
mod tests {
    use super::*;
    use crate::event::sample_datastore_event;
    ...
    fn dropped_event_never_materializes_blobs() {
        ...
        // unparseable.
        let mut value: Value =
            serde_json::to_value(sample_datastore_event(99, Some("p"))).unwrap();
    ...
    fn person_id_presence_matches_full_parse_semantics() {
        ...
        let base: Value =
            serde_json::to_value(sample_datastore_event(2, Some("ignored"))).unwrap();
    ...
    fn survivor_fully_materializes_the_forward_envelope() {
        let payload = serde_json::to_vec(&sample_datastore_event(2, Some("p"))).unwrap();
