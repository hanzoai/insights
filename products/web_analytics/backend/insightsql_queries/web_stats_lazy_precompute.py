...
# InsightsQL template for the precompute INSERT. The lazy_computation framework
...
# InsightsQL read template — substituted via `parse_select(..., placeholders=...)` so
...
FROM insights.web_stats_preaggregated
...
def _breakdown_having_expr(breakdown_by: WebStatsBreakdown) -> ast.Expr:
    ...
    follows Datastore's 1-based indexing). For VIEWPORT specifically, the raw
...
def execute_read_query(
    *,
    ...
    """Read the precomputed rows via InsightsQL with SQL-side filtering, ordering and
...
def execute_lazy_precomputed_read(
    ...
    try:
        ...
        # Convert team-tz bounds to tz-aware UTC. We keep `tzinfo` so the InsightsQL
