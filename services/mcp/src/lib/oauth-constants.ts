...
export const USER_AGENT = `insights/mcp-server; version: ${packageJson.version}`

export interface GetUserAgentOptions {
    clientUserAgent?: string | undefined
    /** `x-insights-mcp-consumer` — self-identifier of the wrapping app (e.g. `insights-code`, `slack`). */
...
export function getUserAgent(opts: GetUserAgentOptions = {}): string {
    ...
    if (clientUserAgent) {
        const match = clientUserAgent.match(/insights\/([\w.-]+)/)
        if (match) {
...
// Generated from `insights/scopes.py` — keep in sync with `insightscli build:openapi`.
