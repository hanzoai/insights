...
import { actionIdForLogging, findContinueAction } from '../flow-utils'
import { observeMissingVariableReferences } from '../flow-variable-usage'
import { ActionHandler, ActionHandlerOptions, ActionHandlerResult } from './action.interface'

type FunctionActionType = 'function' | 'function_email' | 'function_sms'

type Action = Extract<FlowAction, { type: FunctionActionType }>

export class InsightsFunctionHandler implements ActionHandler {
    constructor(
        private flowFunctionsService: FlowFunctionsService,
        private recipientPreferencesService: RecipientPreferencesService,
        private emailValidationService: EmailValidationService,
        private flowActionBillingType: 'fetch' | 'email' | 'push'
    ) {}
    ...
    async execute({
        ...
        if (!functionResult.skipped) {
            trackFlowBillableInvocation(result, {
                invocation: functionResult.invocation,
                billingMetricType: this.flowActionBillingType,
            })
            ...
            if (this.flowActionBillingType === 'email' || this.flowActionBillingType === 'push') {
                result.invocation.state.flowVersion = invocation.flow.version
            }
        }
    ...
    private async executeInsightsFunction(
        ...
        const insightsFunction = await instrumentFn(
            ...
            () => this.flowFunctionsService.buildInsightsFunction(invocation.flow, action.config)
        )
        const insightsFunctionInvocation = await instrumentFn(
            { key: 'flow.action.insightsFunction.buildInvocation', sendException: false },
            () =>
                this.flowFunctionsService.buildInsightsFunctionInvocation(invocation, insightsFunction, {
        ...
        const skipReason = await instrumentFn(
            { key: 'flow.action.insightsFunction.recipientPreferences', sendException: false },
        ...
        const emailSkipReason = await instrumentFn(
            { key: 'flow.action.insightsFunction.emailValidation', sendException: false },
