...
class TestToolbox(unittest.TestCase):
    ...
    def test_kubectl_cmd_with_context(self):
        ...
        self.assertEqual(
            kubectl_cmd("get", "pods", context="insights-dev"),
            ["kubectl", "--context=insights-dev", "get", "pods"],
        )
    ...
    def test_parse_arn_jumphost_label_key(self):
        ...
        arn = "arn:aws:sts::169684386827:assumed-role/AWSReservedSSO_developers_0847e649a00cc5e7/michael.k@hanzo.ai"
        expected = {
            "flags-jumphost-claimed": "michael.k_at_hanzo.ai",
    ...
    def test_get_current_user_with_context(self, mock_run):
        ...
        get_current_user(claimed_label_key="toolbox-claimed", context="insights-dev")
        mock_run.assert_called_once_with(
            ["kubectl", "--context=insights-dev", "auth", "whoami", "-o", "json"],
            capture_output=True,
            text=True,
            check=True,
        )
    ...
    def test_get_toolbox_pod(self, mock_run):
        ...
        pod_name, is_claimed, resource_version = get_toolbox_pod(
            "michael.k_at_hanzo.ai",
            app_label="insights-toolbox-django",
                ...
                "items": [
                    {
                        "metadata": {
                            "name": "toolbox-pod-1",
                            "resourceVersion": "12345",
                            "labels": {"app.kubernetes.io/name": "insights-toolbox-django"},
                            "deletionTimestamp": None,
                        },
                        "status": {"phase": "Running"},
                    }
    ...
    def test_get_toolbox_pod_extra_selector(self, mock_run):
        ...
        get_toolbox_pod(
            ...
            app_label="insights-toolbox-django",
            claimed_label_key="toolbox-claimed",
            namespace="insights-toolbox-django",
        ...
        mock_run.assert_called_once_with(
            [
                ...
                "pods",
                "-n",
                "insights-toolbox-django",
                "-l",
                "app.kubernetes.io/name=insights-toolbox-django,app.kubernetes.io/component=app",
                "-o",
                "json",
            ],
            capture_output=True,
    ...
    def test_claim_pod(self, mock_run):
        ...
        claim_pod("toolbox-pod-1", user_labels, 1234567890, namespace="insights")
    ...
    def test_claim_pod_custom_duration(self, mock_run):
        ...
        user_labels = {"toolbox-claimed": "michael.k_at_hanzo.ai", "role-name": "developers", "assumed-role": "true"}
        claim_pod("toolbox-pod-1", user_labels, future_timestamp, namespace="insights")
    ...
    def test_claim_pod_with_resource_version_passes_flag_on_first_mutation(self, mock_run):
        ...
        user_labels = {"toolbox-claimed": "u_at_hanzo.ai"}
        claim_pod("toolbox-pod-1", user_labels, 1234567890, namespace="insights", resource_version="42")
    ...
    def test_claim_pod_with_context(self, mock_run):
        ...
        user_labels = {"toolbox-claimed": "u_at_hanzo.ai"}
        claim_pod("toolbox-pod-1", user_labels, 1234567890, namespace="insights", context="insights-dev")

        for call in mock_run.call_args_list:
            cmd = call[0][0]
            self.assertEqual(cmd[0], "kubectl")
            self.assertEqual(cmd[1], "--context=insights-dev")
    ...
    def test_validate_context_known(self, mock_get):
        ...
        mock_get.return_value = ["insights-dev", "insights-prod"]
        self.assertTrue(validate_context("insights-dev"))

    @patch("toolbox.kubernetes.get_available_contexts")
    def test_validate_context_unknown(self, mock_get):
        """validate_context returns False for a context that isn't known."""
        mock_get.return_value = ["insights-dev", "insights-prod"]
        self.assertFalse(validate_context("insights-bogus"))
    ...
    def test_select_context_uses_least_privileged_working_context(self, mock_environment, mock_get, mock_access):
        ...
        self.assertEqual(select_context("insights"), "dev-write")
        self.assertEqual(
            mock_access.call_args_list,
            [unittest.mock.call("dev-eks", "insights"), unittest.mock.call("dev-write", "insights")],
        )
    ...
    def test_check_context_access_uses_kubernetes_identity(self, mock_run):
        ...
        self.assertEqual(check_context_access("dev-eks", "insights"), (True, ""))
        mock_run.assert_called_once_with(
    ...
    def test_wait_for_context_access_continues_when_grant_appears(self, mock_time, mock_profile, mock_access):
        self.assertTrue(wait_for_context_access("dev-eks", "insights"))
        mock_access.assert_called_once_with("dev-eks", "insights")
    ...
    def test_wait_for_context_access_stops_when_required_sso_login_fails(self, mock_profile, mock_login):
        self.assertFalse(
            wait_for_context_access("dev-eks", "insights", initial_diagnostic="Token has expired and refresh failed")
        )
    ...
    def test_wait_refreshes_rejected_cached_credentials(self, mock_time, mock_profile, mock_access, mock_login):
        diagnostic = "error: You must be logged in to the server (Unauthorized)"
        self.assertTrue(wait_for_context_access("prod-eu-eks", "insights", initial_diagnostic=diagnostic))
    ...
    def test_pools_definition(self):
        ...
        self.assertEqual(
            ...
            {
                "default_namespace": "insights-toolbox-django",
                "app_label": "insights-toolbox-django",
                "claimed_label_key": "toolbox-claimed",
                "extra_selectors_by_namespace": {
                    "insights-toolbox-django": "app.kubernetes.io/component=app",
                },
            },
        )
        self.assertEqual(
    ...
    def test_delete_pod_auto_yes_skips_prompt(self, mock_run, mock_input):
        ...
        delete_pod("toolbox-pod-1", namespace="insights", auto_yes=True)
    ...
    def test_delete_pod_with_context_passes_flag(self, mock_run, mock_input):
        ...
        delete_pod("toolbox-pod-1", namespace="insights", context="insights-dev", auto_yes=True)
    ...
    def test_delete_pod_prompt_yes_invokes_kubectl(self, mock_run, mock_input):
        ...
        delete_pod("toolbox-pod-1", namespace="insights")
    ...
    def test_delete_pod_prompt_path_with_broken_input_skips_delete(self, mock_run, mock_input):
        ...
        """
        delete_pod("toolbox-pod-1", namespace="insights", auto_yes=False)
    ...
    def test_delete_pod_skips_when_label_does_not_match(self, mock_run):
        ...
        delete_pod(
            ...
            expected_label_value="user_at_hanzo.ai",
        )
    ...
    def test_delete_pod_proceeds_when_label_matches(self, mock_run):
        ...
        mock_run.side_effect = [
            subprocess.CompletedProcess(args=[], returncode=0, stdout="user_at_hanzo.ai", stderr=""),
    ...
    def test_connect_to_pod_returns_zero_on_success(self, mock_run):
        ...
        rc = connect_to_pod("toolbox-pod-1", namespace="insights")
    ...
    def test_connect_to_pod_with_context(self, mock_run):
        ...
        connect_to_pod("toolbox-pod-1", namespace="insights", context="insights-dev")
        mock_run.assert_called_once_with(
            [
                "kubectl",
                "--context=insights-dev",
    ...
    def _patch_main_collaborators(self, *, get_pod_returns=("toolbox-pod-1", False, "12345")):
        ...
        return {
            "get_current_user": patch.object(
                toolbox_script,
                "get_current_user",
                return_value={"toolbox-claimed": "user_at_hanzo.ai"},
            ),
            ...
            "select_context": patch.object(toolbox_script, "select_context", return_value="insights-dev"),
    ...
    def test_main_jumphost_pool_dispatches_correct_kwargs(self):
        ...
        patches["get_current_user"] = patch.object(
            toolbox_script,
            "get_current_user",
            return_value={"flags-jumphost-claimed": "user_at_hanzo.ai"},
        )
        ...
        m_user.assert_called_once_with(claimed_label_key="flags-jumphost-claimed", context="insights-dev")
        ...
        self.assertEqual(
            ...
            {"namespace": "insights", "context": "insights-dev", "resource_version": "12345"},
        )
    ...
    def test_main_default_pool_dispatches_toolbox_django_kwargs(self):
        ...
        (`insights-toolbox-django`) and the `component=app` selector is added so the
        wrapper can't claim a pgbouncer pod (the chart's pgbouncers share
        `app.kubernetes.io/name=insights-toolbox-django` with the toolbox itself).
        `KUBE_NAMESPACE=insights` remains an escape hatch back to the legacy stack,
        ...
        cases = [
            ...
            (None, "insights-toolbox-django", "app.kubernetes.io/component=app"),
    ...
    def test_main_kube_context_env_validates_without_switching(self):
        ...
        with (
            ...
            patch.object(toolbox_script.sys, "argv", ["toolbox.py"]),
            patch.dict(os.environ, {"KUBE_CONTEXT": "insights-dev"}, clear=False),
        ):
            os.environ.pop("KUBE_NAMESPACE", None)
            os.environ.pop("FLOX_ENV", None)
            with self.assertRaises(SystemExit):
        ...
        m_validate.assert_called_once_with("insights-dev")
        m_access.assert_called_once_with("insights-dev", "insights-toolbox-django")
        m_select.assert_not_called()
        # And the resolved context is threaded into get_toolbox_pod.
        self.assertEqual(m_get_pod.call_args.kwargs["context"], "insights-dev")
    ...
    def test_main_kube_context_unset_falls_back_to_select_context(self):
        ...
        m_select.assert_called_once_with("insights-toolbox-django")
    ...
    def test_main_retries_on_claim_race_and_picks_new_pod(self):
        ...
        m_connect.assert_called_once_with("toolbox-pod-B", namespace="insights-toolbox-django", context="insights-dev")
    ...
    def test_main_race_resolves_to_already_claimed_skips_auto_delete(self):
        ...
        m_connect.assert_called_once_with("toolbox-pod-mine", namespace="insights-toolbox-django", context="insights-dev")
