 ...
 * array results the array is wrapped as `{ results, _insightsUrl }` — spreading an array into
 * an object (`{ ...arr }`) would otherwise corrupt it into `{ 0: …, 1: …, _insightsUrl: … }`.
 */
export type WithInsightsUrl<T = unknown> = T extends readonly (infer U)[]
    ? { results: U[]; _insightsUrl: string }
    : T & { _insightsUrl: string }

/** Adds _insightsUrl to a result. Wraps raw arrays in `{ results, _insightsUrl }` (see type above). */
export async function withInsightsUrl<T>(context: Context, result: T, path: string): Promise<WithInsightsUrl<T>> {
    ...
    if (Array.isArray(result)) {
        return { results: result, _insightsUrl: fullUrl } as unknown as WithInsightsUrl<T>
    }

    return { ...result, _insightsUrl: fullUrl } as WithInsightsUrl<T>
}
 ...
 * array is wrapped as `{ results, _agentNote }`, mirroring `withInsightsUrl`.
 */
