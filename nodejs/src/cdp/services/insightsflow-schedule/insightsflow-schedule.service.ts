...
export class InsightsFlowScheduleService {
    ...
    start(): void {
        ...
        logger.info('InsightsFlowScheduleService: starting...')

        this.running = true
        this.pollPromise = this.pollLoop()
        logger.info('InsightsFlowScheduleService: started, polling every ' + this.pollIntervalMs + 'ms')
    }
    ...
    async pollAndDispatch(): Promise<boolean> {
        ...
        try {
            ...
            if (fetchError || !fetchResponse) {
                logger.error('InsightsFlowScheduleService: failed to call Django endpoint', {
            ...
            if (fetchResponse.status !== 200) {
                ...
                logger.error('InsightsFlowScheduleService: Django endpoint returned error', {
            ...
            if (data.initialized.length > 0) {
                ...
                logger.info('InsightsFlowScheduleService: initialized schedules', {
            ...
            if (data.failed.length > 0) {
                ...
                logger.error('InsightsFlowScheduleService: schedules failed to process', {
            ...
            if (data.processed.length > 0) {
                ...
                logger.info('InsightsFlowScheduleService: processed due schedules', {
            ...
            logger.error('InsightsFlowScheduleService: failed to poll', { error: String(err) })
            schedulerPollCounter.inc({ status: 'error' })
    ...
    isHealthy(): HealthCheckResult {
        if (!this.running) {
            return new HealthCheckResultError('InsightsFlowScheduleService is not running', {})
        }
        if (Date.now() - this.lastSuccessfulPollAt > this.healthTimeoutMs) {
            return new HealthCheckResultError('InsightsFlowScheduleService has not polled successfully recently', {
