...
- **Kernel-lane runs (python/duckdb)**: the sandbox callback writes the envelope,
  as before.
- **Direct-lane runs (insightsql, no sandbox — `sql_v2_direct.py`)**: the run rides the
...
- **Interrupt (Journey 9):** for kernel-lane runs,
  ...
  **direct (insightsql) runs** there is no kernel to signal, so the endpoint marks the
  ...
  still-queued Celery task, otherwise it KILLs the query on Datastore under the
...
### Why not a Redis Stream (like Insights Desktop)

Insights Desktop streams many incremental agent events and needs replay on reconnect
...
**Now: client-side for insightsql runs.** A direct run fetches up to
`RESULT_CACHE_ROWS` (300) rows in one Datastore query; the full capped set rides
the result poll (`rows`) while the manager's Redis result is alive (~20 min), and
the browser pages it locally — the server page endpoint refuses insightsql runs.
...
client-side pages). The kernel-server's per-run insightsql cache and `/page` re-query
...
pyarrow mmap). There is no data-plane fallback — the run's code is not a InsightsQL query —
...
- **`KernelRuntime.server_url` / `server_connect_token`** are stored **plaintext**
  (`TextField`). This matches Insights Desktop, which keeps `sandbox_url` /
