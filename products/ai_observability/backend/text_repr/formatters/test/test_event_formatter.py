    ...
    def test_evaluation_hog_runtime(self):
        event = {
            "properties": {
                ...
                "$ai_evaluation_result": True,
                "$ai_evaluation_runtime": "script",
            }
        }
        result = format_evaluation_text_repr(event)
        assert "EVALUATION: Length check | Result: PASS (script)" in result
...
class TestFormatEventTextReprFromAiEventsRow:
    ...
    are stored on `insights.ai_events` as JSON-encoded strings. The shared
