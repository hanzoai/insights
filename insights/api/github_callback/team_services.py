...
from insights.api.github_callback import (
    redirects,
...
from insights.models.user_integration import UserIntegration, user_github_integration_from_installation
...
def _connect_from_for_next(next_url: str) -> str | None:
    ...
    return connect_from if connect_from == "insights_code" else None
...
def authorize_link_existing_installation(
    *,
    ...
    lets a second Insights project reuse a GitHub App that installs only once per org.
...
def build_team_github_oauth_authorize_url(
    *,
    ...
    resolved_connect_from = connect_from if connect_from == "insights_code" else _connect_from_for_next(next_url)
