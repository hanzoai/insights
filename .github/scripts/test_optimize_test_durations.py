...
<testsuite name="pytest"><testcase classname="insights.test_foo.TestThing" name="test_one" time="0.5"/></testsuite>
"""
...
class TestJUnitCallTimeCorrection:
    ...
    def test_floors_migration_tax_contamination(self):
        ...
        durations = {"insights/x.py::T::test_a": 408.0, "insights/x.py::T::test_b": 2.0}
        shards = [self._shard("core-1", {"insights/x.py::T::test_a": 4.3, "insights/x.py::T::test_b": 2.0})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/x.py::T::test_a"] == 4.3
        assert result.corrected_durations["insights/x.py::T::test_b"] == 2.0
    ...
    def test_floors_flat_default_placeholder(self):
        ...
        durations = {"insights/x.py::T::test_a": 60.0}
        shards = [self._shard("core-1", {"insights/x.py::T::test_a": 0.5})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/x.py::T::test_a"] == 0.5

    def test_leaves_genuinely_slow_test_untouched(self):
        # Real end-to-end test: recorded ~= call, small gap, not flooded.
        durations = {"insights/x.py::T::test_slow": 102.0}
        shards = [self._shard("core-1", {"insights/x.py::T::test_slow": 101.5})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/x.py::T::test_slow"] == 102.0
    ...
    def test_leaves_gray_zone_setup_untouched(self):
        ...
        durations = {"insights/x.py::T::test_setup_heavy": 42.0}
        shards = [self._shard("core-1", {"insights/x.py::T::test_setup_heavy": 1.0})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/x.py::T::test_setup_heavy"] == 42.0
    ...
    def test_keeps_high_recorded_test_with_small_gap(self):
        ...
        durations = {"insights/x.py::T::test_slow": 150.0}
        shards = [self._shard("core-1", {"insights/x.py::T::test_slow": 140.0})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/x.py::T::test_slow"] == 150.0
        assert result.carriers_found == 0
    ...
    def test_suffix_match_when_path_prefix_differs(self):
        ...
        durations = {"insights/api/test/x.py::T::test_a": 408.0}
        shards = [self._shard("core-1", {"api/test/x.py::T::test_a": 3.0})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/api/test/x.py::T::test_a"] == 3.0

    def test_no_junit_match_leaves_value(self):
        # No JUnit entry for the test — can't verify, so keep the value.
        durations = {"insights/x.py::T::test_a": 408.0}
        shards = [self._shard("core-1", {"insights/x.py::T::test_other": 1.0})]
        result = MigrationTaxCorrector(durations, junit_shards=shards).correct()
        assert result.corrected_durations["insights/x.py::T::test_a"] == 408.0
