...
from insights.api.oauth.views import _impersonation_ai_processing_block, _impersonator_id_for_request
from insights.auth import OAuthAccessTokenAuthentication
from insights.helpers.impersonation import is_impersonated
from insights.middleware import IMPERSONATION_READ_ONLY_SESSION_KEY
from insights.models import OAuthApplication, Organization, OrganizationMembership, Team, User
from insights.models.oauth import OAuthAccessToken, OAuthApplicationAccessLevel, OAuthGrant, OAuthRefreshToken
...
class TestImpersonationOAuthRevocation(BaseTest):
    def test_user_logged_out_revokes_only_impersonated_tokens(self) -> None:
        admin = User.objects.create_user(email="admin@hanzo.ai", password="x", first_name="A")
        admin.is_staff = True
        admin.save()
        ...
        with patch("insights.helpers.impersonation.is_impersonated_session", return_value=True):
            logout(request)
