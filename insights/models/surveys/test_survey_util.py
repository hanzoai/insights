...
from insights.models.surveys.util import (
    _build_coalesce_query,
    _build_id_based_key,
    _build_index_based_key,
    get_survey_response_datastore_query,
)
...
class TestSurveyResponseFunctions(TestCase):
    ...
    def test_get_survey_response_datastore_query_with_question_id(self):
        """Test the full query generation with a specific question ID"""
        query = get_survey_response_datastore_query(1, "abc123")
        expected = """COALESCE(
        NULLIF(JSONExtractString(properties, '$survey_response_abc123'), ''),
    ...
    def test_get_survey_response_datastore_query_without_question_id(self):
        """Test the full query generation with just a question index"""
        query = get_survey_response_datastore_query(0)
        expected = """COALESCE(
    ...
    def test_get_survey_response_datastore_query_multiple_choice(self):
        """Test the full query generation with a multiple choice question"""
        query = get_survey_response_datastore_query(1, "abc123", True)
        expected = """if(
