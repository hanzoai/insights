...
@pytest.mark.parametrize(
    ...
    [
        "invalid/path",
        "insights/noshard/unknown-20240101075404/",
        "insights/table/noshard/20240101075404/",
        "",
    ],
)
...
def test_get_latest_backups_skips_unparseable_paths():
    ...
    mock_s3.get_client().list_objects_v2.return_value = {
        "CommonPrefixes": [
            {"Prefix": "insights/test/noshard/full-20240101075404/"},
            {"Prefix": "insights/test/noshard/garbage/"},
            {"Prefix": "insights/test/noshard/inc-20240201075404/"},
        ]
    }

    config = BackupConfig(database="insights", table="test", incremental=True)
...
def test_get_latest_backup_empty():
    ...
    config = BackupConfig(database="insights", table="dummy", incremental=True)
    context = dagster.build_op_context()
    result = get_latest_backups(context=context, config=config, s3=mock_s3)
...
def test_get_latest_backup(table: str):
    ...
    mock_s3.get_client().list_objects_v2.return_value = {
        "CommonPrefixes": [
            {"Prefix": f"insights/{f'{table}/' if table else ''}noshard/full-20230101075404/"},
            {"Prefix": f"insights/{f'{table}/' if table else ''}noshard/full-20240101075404/"},
            {"Prefix": f"insights/{f'{table}/' if table else ''}noshard/inc-20240301075404/"},
            {"Prefix": f"insights/{f'{table}/' if table else ''}noshard/inc-20230301075404/"},
            {"Prefix": f"insights/{f'{table}/' if table else ''}noshard/inc-20240201075404/"},
        ]
    }

    config = BackupConfig(database="insights", table=table, incremental=True)
    context = dagster.build_op_context()
    result = get_latest_backups(context=context, config=config, s3=mock_s3)
...
def test_get_latest_successful_backup_returns_latest_backup():
    config = BackupConfig(database="insights", table="test", incremental=True)
    backup1 = Backup(database="insights", date="20240201075404", incremental=True, table="test")
    ...
    backup2 = Backup(database="insights", date="20240101075404", incremental=False, table="test")
...
def test_cleanup_skips_or_proceeds_based_on_backup_log_status(status_value: Optional[str], expect_cleanup: bool):
    ...
    current_backup = Backup(database="insights", table="test", date="20240202000000", incremental=True)
    prior_backups = [
        Backup(database="insights", table="test", date="20240201000000", incremental=False),
        Backup(database="insights", table="test", date="20240102000000", incremental=True),
        Backup(database="insights", table="test", date="20240101000000", incremental=False),
    ]
...
def test_cleanup_skips_when_no_full_backup():
    ...
    prior_backups = [
        Backup(database="insights", table="test", date="20240101000000", incremental=True),
    ]
...
def test_cleanup_current_full_backup_does_not_check_log():
    config = BackupConfig(database="insights", table="test", incremental=False)
    # Current backup is the full we just created; prior list has an older full
    current_backup = Backup(database="insights", table="test", date="20240201000000", incremental=False)
    prior_backups = [
        Backup(database="insights", table="test", date="20240101000000", incremental=False),
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (
            [
                {
                    "Key": "insights/test/noshard/full-20240101000000/.backup",
                    "Code": "AccessDenied",
        ...
        (
            [
                {
                    "Key": "insights/test/noshard/full-20240101000000/data/part1",
                    "Code": "AccessDenied",
                    "Message": "Access Denied",
                }
            ],
...
def test_cleanup_delete_objects_response_handling(s3_errors, s3_deleted, expect_failure):
    config = BackupConfig(database="insights", table="test", incremental=False)
    current_backup = Backup(database="insights", table="test", date="20240202000000", incremental=False)
    prior_backups = [
        Backup(database="insights", table="test", date="20240101000000", incremental=False),
    ]
...
def test_cleanup_deletes_failed_recent_backups():
    config = BackupConfig(database="insights", table="test", incremental=True)
    current_backup = Backup(database="insights", table="test", date="20240203000000", incremental=True)
    prior_backups = [
        Backup(database="insights", table="test", date="20240202000000", incremental=True),
        Backup(database="insights", table="test", date="20240201000000", incremental=False),
    ]
...
def test_wait_for_backup_raises_on_max_tries_exceeded():
    config = BackupConfig(database="insights", table="test", incremental=False)
    backup = Backup(database="insights", table="test", date="20240201000000", shard=1)
    ...
    with (
        ...
        patch("insights.dags.backups.MAX_WAIT_TRIES", 2),
    ):
