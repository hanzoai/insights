...
`PathsQuery` doesn't support `compareFilter`. Run `insights:query-paths` twice with
date ranges of equal length and compare which edges gained or lost volume.

```json
insights:query-paths
{
  "kind": "PathsQuery",
...
A "drop in paths A → B" is often a drop in A or B alone. Run `insights:query-trends` on
