...
The status must come from a **real signal**: a data-existence query (InsightsQL count / exists API), the product's opt-in flag, or an entity count for creation-first products. Never a dismissal flag — `has_completed_onboarding_for` is routing metadata, not evidence of data.
...
- **Wizard vs primary action**: SDK-installed products set `wizard: { slug }` (the slug must exist in `@hanzo/wizard`); creation-first products (flags, surveys) set `primaryAction` instead. Self-hosted degrades automatically: no cloud → the terminal hides and the manual path is promoted.
...
- **Illustration**: `pngHoggie()` from `lib/brand/hoggies`. The slot renders nothing — this product ships no empty-state artwork — but keeping the slot means a layout that already reserves space for one still lays out correctly. Never hardcode an image URL (e.g. Cloudinary) to fill it.
...
- `pnpm --filter=@hanzo/frontend typescript:check`, storybook snapshots stable.
