...
from posthog.schema import AssistantInsightsQLQuery, NodeKind
...
class InsightsQLQuerySyntaxCorrectness(Scorer):
    def _name(self):
    ...
    def _evaluate(self, output: PlanAndQueryOutput) -> Score:
        ...
        if isinstance(output["query"], AssistantInsightsQLQuery):
...
- For session duration, `session.$session_duration` should be used instead of things like `properties.$session_duration`.""",
            ...
            InsightsQLQuerySyntaxCorrectness(),
