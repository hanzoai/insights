...
We default to open but when adding a new feature we should consider if it should be MIT licensed or Enterprise edition licensed. Everything in the `ee` folder is covered by [a different license](https://github.com/hanzoai/insights/blob/main/ee/LICENSE). It's easy to move things from `ee` to open, but not the other way.
