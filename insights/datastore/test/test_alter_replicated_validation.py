...
class TestShardedAlterRouting(unittest.TestCase):
    def _exec_with_cloud(self, node_roles: list[NodeRole], cluster: mock.MagicMock) -> None:
        with (
            mock.patch("insights.datastore.client.migration_tools.settings.E2E_TESTING", False),
            mock.patch("insights.datastore.client.migration_tools.settings.DEBUG", False),
            mock.patch("insights.datastore.client.migration_tools.settings.CLOUD_DEPLOYMENT", "US"),
            mock.patch(
                "insights.datastore.client.migration_tools.get_migrations_cluster",
    ...
    def test_local_or_test_falls_through_to_per_shard(self):
        cluster = _build_cluster_mock()
        with (
            mock.patch("insights.datastore.client.migration_tools.settings.DEBUG", True),
            mock.patch(
                "insights.datastore.client.migration_tools.get_migrations_cluster",
                return_value=cluster,
            ),
