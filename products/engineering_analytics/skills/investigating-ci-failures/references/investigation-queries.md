...
Copy-ready InsightsQL for each step of the investigation workflow. All of these run via `execute-sql`
(or `insights:execute-sql` through MCP). The two views are non-materialized — results are always
