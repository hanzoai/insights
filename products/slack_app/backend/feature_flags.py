...
- **Remote evaluation** (``only_evaluate_locally=False``) — no deployment needs a
  ...
  Insights's flags endpoint with the project token.
...
UNTAGGED_THREAD_FOLLOWUPS_FLAG = "insights-slack-app-untagged-thread-followups"
...
def is_slack_app_oauth_enabled(integration: Integration, slack_team_id: str) -> bool:
    ...
    integrations) decisions. Keyed on the Slack workspace + Insights org."""
    try:
        return bool(
...
def is_slack_app_home_enabled(integration: Integration) -> bool:
    ...
    Slack-triggered task runs. Keyed on the Slack workspace + Insights org."""
    try:
        return bool(
            hanzo_insights.feature_enabled(
                SLACK_APP_HOME_FLAG,
...
def is_slack_app_model_classifier_enabled(integration: Integration) -> bool:
    ...
    Insights org, matching the other Slack-app gates.
...
def is_slack_app_agent_design_enabled(integration: Integration) -> bool:
    ...
    Keyed on the Slack workspace + Insights org, matching ``is_slack_app_home_enabled``."""
    try:
        return bool(
            hanzo_insights.feature_enabled(
...
def is_slack_app_canvas_file_artifacts_enabled(integration: Integration) -> bool:
    ...
    verify the granted scopes at point of use. Keyed on the Slack workspace + Insights org."""
    try:
        return bool(
            hanzo_insights.feature_enabled(
...
def is_slack_app_living_artifacts_enabled(integration: Integration) -> bool:
    ...
    Keyed on the Slack workspace + Insights org. This is the umbrella artifact gate;
...
def is_slack_app_untagged_thread_followups_enabled(integration: Integration, slack_team_id: str) -> bool:
    ...
    a fresh ``@Insights`` mention. Keyed on the Slack workspace + Insights org."""
    try:
        return bool(
            hanzo_insights.feature_enabled(
