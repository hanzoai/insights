...
- `DUCKGRES_USERNAME=insights`
- `DUCKGRES_PASSWORD=insights`
...
- **Schema**: `insights_data_modeling_team_<team_id>`
- **Table**: `<model_label>` (derived from saved query name)
- **Example**: `ducklake.insights_data_modeling_team_123.my_saved_query`

### Data Imports and Data Import Registration

- **Schema**: `insights_data_imports_team_<team_id>`
- **Table**: `<source_type>_<prefix>_<normalized_name>` (prefix is user-defined on the external data source)
- **Example**: `ducklake.insights_data_imports_team_123.stripe_prod_invoices`
...
1. **Start the dev stack**
   Run `insightscli start` (or `bin/start`) so Postgres, SeaweedFS, Temporal, and all DuckLake defaults are up. Make sure the `ducklake-data-modeling-copy-workflow` feature flag is enabled for the team you plan to use.
...
5. **Query the new DuckLake table**
   The copy activity creates a table at `ducklake.insights_data_modeling_team_<team_id>.<model_label>`. From any DuckDB shell you can inspect it, for example:
   ...
   duckdb -c "
     ...
     ATTACH 'ducklake:postgres:dbname=ducklake host=localhost user=insights password=insights'
     ...
     SELECT * FROM ducklake.insights_data_modeling_team_${TEAM_ID}.${MODEL_LABEL} LIMIT 10;
   "
   ```
...
1. **Start the dev stack**
   Run `insightscli start` (or `bin/start`) so Postgres, Duckgres, SeaweedFS, Temporal, and all DuckLake defaults are up. Enable either `ducklake-data-imports-copy-workflow` for the existing Delta-copy path or `ducklake-data-imports-registration-workflow` for the prepared-Parquet path.
...
5. **Query the new DuckLake table**
   The copy activity creates a table at `ducklake.insights_data_imports_team_<team_id>.<source_type>_<prefix>_<table_name>`. From any DuckDB shell you can inspect it:
   ...
   duckdb -c "
     ...
     -- Query a specific table
     SELECT * FROM ducklake.insights_data_imports_team_${TEAM_ID}.${SOURCE_TYPE}_${PREFIX}_${TABLE_NAME} LIMIT 10;
   "
   ```
