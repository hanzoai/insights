...
from insights.settings.base_variables import CLOUD_DEPLOYMENT, DEBUG
...
# Insights project where Temporal worker logs are ingested, used to build logs links
...
# X-Insights-Properties blob. SANDBOX_AI_GATEWAY_PRODUCTS limits the switch to named
...
# OTLP destinations for agent-server run telemetry (Insights Logs/APM).
# Full ingest URLs (e.g. https://us.i.hanzo.ai/i/v1/logs and .../i/v1/traces)
...
# client_id of the OAuthApplication used to mint the access token the Insights setup wizard
...
# Mirror persisted task-run logs into the Insights Logs product (dogfooding).
...
# Insights project's Logs, so no transport or credentials are needed here. Only runs whose
...
# Insights's own internal logs project — so this is the internal project's API key, never
...
DATASTORE_MAX_EXECUTION_TIME: int = get_from_env("DATASTORE_MAX_EXECUTION_TIME", 86400, type_cast=int)  # 1 day
DATASTORE_MAX_MEMORY_USAGE: int = get_from_env(
    "DATASTORE_MAX_MEMORY_USAGE", 150 * 1000 * 1000 * 1000, type_cast=int
)  # 150GB
DATASTORE_MAX_BLOCK_SIZE_DEFAULT: int = get_from_env("DATASTORE_MAX_BLOCK_SIZE_DEFAULT", 10000, type_cast=int)
# Comma separated list of overrides in the format "team_id:block_size"
DATASTORE_MAX_BLOCK_SIZE_OVERRIDES: dict[int, int] = dict(
    [map(int, o.split(":")) for o in os.getenv("DATASTORE_MAX_BLOCK_SIZE_OVERRIDES", "").split(",") if o]  # type: ignore
)
...
STAMPFN_TASK_QUEUE = _set_temporal_task_queue("stamphog-task-queue")
...
# Per-worker cap on concurrent offline-cluster Datastore queries issued by the messaging
...
MESSAGING_DATASTORE_MAX_CONCURRENT_QUERIES: int = get_from_env(
    "MESSAGING_DATASTORE_MAX_CONCURRENT_QUERIES", 10, type_cast=int
)
