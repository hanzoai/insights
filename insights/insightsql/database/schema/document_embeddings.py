...
from insights.insightsql.transforms.order_by_pushdown import push_down_order_by, resolve_alias, unwrap_alias
...
DOCUMENT_EMBEDDINGS_FIELDS: dict[str, FieldOrTable] = {
    ...
    "product": StringDatabaseField(
        name="product",
        nullable=False,
        description="Insights product the embedded document belongs to, e.g. 'error_tracking'.",
    ),
...
INSIGHTSQL_MODEL_TABLES = {table.to_printed_insightsql(): table for table in INSIGHTSQL_EMBEDDING_TABLES.values()}


class DocumentEmbeddingsTable(LazyTable):
    description: str = "Vector embeddings of Insights documents for semantic search; queries must filter on `model_name` to route to the right model-specific table."
    ...
    def lazy_select(
        ...
        if model_name and model_name in INSIGHTSQL_EMBEDDING_TABLES:
            model_table = INSIGHTSQL_EMBEDDING_TABLES[model_name]
            table_name = model_table.to_printed_insightsql()
