...
export type TreeDragEndEvent = DragEndEvent & { position: TreeDropPosition }
...
export type TreeProps = TreeBaseProps & {
    ...
    onDragEnd?: (dragEvent: TreeDragEndEvent) => void
...
export type TreeNodeProps = TreeBaseProps & {
    ...
    isDragging?: boolean
    /** Internal callback threaded from <Tree> down to rows for insertion-line positioning during reorder drags. */
    onReorderPositionChange?: (id: string, position: TreeDropPosition) => void
}

export interface TreeRef {
    getVisibleItems: () => TreeDataItem[]
    focusItem: (id: string, options?: TreeFocusOptions) => void
}
...
type TreeItemRowProps = Omit<TreeNodeProps, 'data'> & {
    item: TreeDataItem
    ariaSetSize: number
    ariaPosInSet: number
...
const TreeItemRow = forwardRef<HTMLDivElement, TreeItemRowProps>(
    (
        {
            item,
            ariaSetSize,
                            ...
                            childrenContent={
                                item.children ? (
                                    <AccordionPrimitive.Content className="relative">
                                        <TreeNode data={item.children} depth={depth + 1} size={size} {...props} />
...
const Tree = forwardRef<TreeRef, TreeProps>(
    (
        ...
        useImperativeHandle(ref, () => ({
            getVisibleItems,
            focusItem: (id: string, options: TreeFocusOptions = {}) => {
