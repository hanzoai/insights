...
from insights.insightsql.direct_sql.capability import direct_capable_source_types

from insights.api.routing import TeamAndOrgViewSetMixin
from insights.api.utils import action
from insights.event_usage import EventSource, get_event_source, is_wizard_self_driving_program, report_user_action
...
from insights.rbac.user_access_control import UserAccessControlSerializerMixin, access_level_satisfied_for_resource

from products.cdp.backend.facade.api import InsightsFunctionSerializer
from products.cdp.backend.facade.models import InsightsFunction
...
from products.data_warehouse.backend.facade.api import (
    ...
    delete_webhook_and_insights_function,
    ...
    get_or_create_webhook_insights_function,
...
RESERVED_SOURCE_NAME_MESSAGE = "This source name is reserved by Insights."
...
DIRECT_QUERY_UNSUPPORTED_SOURCE_MESSAGE = (
    "Direct query mode is currently supported only for Postgres, MySQL, Snowflake, Redshift, and Datastore sources."
)
# Engines surfaced on a direct connection's `connection_metadata.engine` (duckdb backs direct Postgres).
DIRECT_CONNECTION_ENGINE_CHOICES = ["duckdb", "postgres", "mysql", "snowflake", "redshift", "datastore"]
...
class ExternalDataSourceConnectionOptionSerializer(serializers.ModelSerializer):
    ...
    supports_insightsql = serializers.SerializerMethodField(
        help_text="Whether InsightsQL queries compile for this connection. When false, only raw SQL (sendRawQuery) works.",
    )
    ...
    def get_supports_insightsql(self, source: ExternalDataSource) -> bool:
        # Function-local: keeps the direct-SQL driver imports off the django.setup() path.
        from insights.insightsql.direct_sql.capability import direct_supports_insightsql  # noqa: PLC0415

        return direct_supports_insightsql(source)

    class Meta:
        model = ExternalDataSource
        fields = ["id", "prefix", "engine", "source_type", "access_method", "supports_insightsql", "description"]
        read_only_fields = ["id", "prefix", "engine", "source_type", "access_method", "supports_insightsql", "description"]
...
class DirectConnectionSourceOptionSerializer(serializers.Serializer):
    ...
    source_type = serializers.ChoiceField(
        choices=ExternalDataSourceType.choices,
        read_only=True,
        help_text="The source type to start a direct-connection setup for (e.g. 'Postgres', 'Datastore').",
    )
...
class ExternalDataSourceSerializers(UserAccessControlSerializerMixin, serializers.ModelSerializer):
    ...
    created_via = serializers.ChoiceField(
        ...
        help_text=(
            ...
            "`wizard` for the setup wizard and `self_driving` for the Insights Desktop app "
...
class ExternalDataSourceCreateSerializer(serializers.Serializer):
    ...
    prefix = serializers.CharField(
        ...
        allow_null=True,
        allow_blank=True,
        help_text="Prefix added to the table names Insights creates in InsightsQL. Does not filter which tables are imported.",
    )
    description = serializers.CharField(
    ...
    created_via = serializers.ChoiceField(
        ...
        # transport (the wizard, Insights Desktop, or the wizard's self-driving program).
        choices=[
        ...
        help_text=(
            ...
            "they are derived server-side for wizard- and Insights Desktop-driven MCP calls. Defaults to `api`."
        ),
    )
    direct_query_enabled = serializers.BooleanField(
        required=False,
...
class SourceSetupSerializer(serializers.Serializer):
    ...
    prefix = serializers.CharField(
        ...
        allow_blank=True,
        help_text=(
            "Prefix added to the table names Insights creates in InsightsQL, e.g. 'stripe' produces stripe_charges. "
...
class SourceSetupWebhookSerializer(serializers.Serializer):
    ...
    webhook_url = serializers.CharField(
        allow_null=True, help_text="The Insights endpoint the external service delivers events to."
    )
...
class SourceConnectLinkSerializer(serializers.Serializer):
    ...
    connect_url = serializers.CharField(
        help_text=(
            "Full URL to share with the user. It opens the source's connection form in Insights — "
...
def _find_unresolved_secret_refs(payload: Any) -> list[str]:
    ...
    caller must resolve to real values before they reach Insights. If one slips through, source creation
    ...
    return Response(
        ...
        data={
            "message": (
                ...
                "`{'secretRef': ...}` objects — Insights cannot resolve them. Resolve the secret to its real "
...
class ExternalDataSourceViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, viewsets.ModelViewSet):
    ...
    # source type ("Stripe", "Postgres") and the InsightsQL table prefix.
    ...
    def check_object_permissions(self, request: Request, obj: Any) -> None:
        ...
        if request.method not in ("GET", "HEAD", "OPTIONS") and isinstance(obj, ExternalDataSource):
            if obj.is_system_managed:
                raise PermissionDenied("This source is managed by Insights and cannot be changed through this API.")
    ...
    def get_serializer_context(self) -> dict[str, Any]:
        ...
        # Building the full InsightsQL Database and serializing per-schema table columns is expensive
    ...
    def _create_external_data_source(
        ...
        *,
        ...
        # The setup wizard and Insights Desktop drive creation through the MCP tools, which inject
        ...
        if created_via == ExternalDataSource.CreatedVia.MCP:
            transport_created_via = {
                ...
                EventSource.INSIGHTS_CODE: ExternalDataSource.CreatedVia.SELF_DRIVING,
            }
            created_via = transport_created_via.get(get_event_source(request), created_via)
            # The wizard's `self-driving` onboarding program shares the generic `insights/wizard`
        ...
        for schema in payload_schemas:
            ...
            if direct_engine_adapter is not None and is_direct_query and should_sync:
                ...
                # not just on subsequent updates — otherwise users see all columns in InsightsQL until
        ...
        # `source` (web/api/mcp/wizard/insights_code) is derived from the request by report_user_action;
        # `created_via` is the caller's explicit intent (with one exception: the machine-injected `mcp`
        # is upgraded above when the transport identifies the wizard or Insights Desktop). They usually
    ...
    def _setup_cdc_resources(
        ...
        """
        management_mode = payload.get("cdc_management_mode", "insights")
        logger.info(
    ...
    def refresh_schemas(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        with transaction.atomic():
            ...
            elif isinstance(source, (SQLSource, DatastoreSource)) and source.supports_column_selection:
                # Datastore isn't a SQLSource but exposes the same column-selection
    ...
    def database_schema(self, request: Request, *arg: Any, **kwargs: Any):
        ...
        # Cache the CDC flag once: in non-DEBUG environments this calls hanzo_insights.feature_enabled,
    ...
    def setup(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        # the InsightsQL Database on this hot path.
    ...
    def store_credentials(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        Backs the source connect page: the user enters credentials directly in Insights, they are
    ...
    def check_cdc_prerequisites(self, request: Request, *arg: Any, **kwargs: Any):
        ...
        management_mode = request.data.get("cdc_management_mode", "insights")
        if management_mode not in ("insights", "self_managed"):
            return Response(
                status=status.HTTP_400_BAD_REQUEST,
                data={"message": "cdc_management_mode must be 'insights' or 'self_managed'."},
            )
    ...
    def check_cdc_prerequisites_for_source(self, request: Request, *arg: Any, **kwargs: Any):
        ...
        Body params: ``cdc_management_mode`` (``"insights"`` | ``"self_managed"``),
    ...
    def wizard(self, request: Request, *arg: Any, **kwargs: Any):
        # The documented-tables catalog is only consumed by the hanzo.ai docs build (via the
    ...
    def connect_link(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        data = {
            ...
            "instructions": (
                f"Share this link with the user. They {action_phrase} directly in Insights — never ask them to "
                ...
                "single-use, expire after 24 hours, and are only visible to and consumable by the Insights user "
    ...
    def webhook_info(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        insights_function = InsightsFunction.objects.filter(
            team=self.team,
        ...
        schema_mapping = {}
        if insights_function.inputs:
            schema_mapping = insights_function.inputs.get("schema_mapping", {}).get("value", {})

        webhook_field_names = {f.name for f in (source.get_source_config.webhookFields or [])}
        all_inputs = InsightsFunctionSerializer(insights_function).data.get("inputs") or {}
        ...
        return Response(
            ...
            data={
                ...
                "insights_function": {
                    ...
                    "enabled": insights_function.enabled,
                    "created_at": insights_function.created_at.isoformat(),
                    "status": insights_function.status,
                },
    ...
    def update_webhook_inputs(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        success, error = source.webhook_inputs_updated(
            config,
            get_webhook_url(insights_function.id),
            self.team.pk,
            inputs,
            api_version=source.resolve_api_version(instance.api_version),
        )
    ...
    def delete_webhook(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        result = delete_webhook_and_insights_function(
            team=self.team,
            source=source,
            config=config,
            source_id=str(instance.pk),
