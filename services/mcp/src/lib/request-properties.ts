...
export type RequestProperties = {
    ...
    // Sandbox-provisioned task id: forwarded to the Insights API as `X-Insights-Task-Id` on every
    ...
    // `?flag_overrides=` or the `x-insights-flag-overrides` header. Parsed and gated
...
export function parseRequestProperties(
    ...
    const readOnlyRaw = header(request, 'x-insights-read-only') || params.get('readonly')
    ...
    return {
        ...
        organizationId: header(request, 'x-insights-organization-id') || params.get('organization_id') || undefined,
        projectId: header(request, 'x-insights-project-id') || params.get('project_id') || undefined,
        ...
        mcpConsumer: sanitizeHeaderValue(
            header(request, 'x-insights-mcp-consumer') || params.get('consumer') || undefined
        ),
        ...
        mode: parseMcpMode(header(request, 'x-insights-mcp-mode') || params.get('mode')),
        taskId: sanitizeHeaderValue(header(request, 'x-insights-task-id')),
        transport,
        requestStartTime: Date.now(),
        featureFlagOverrides: header(request, 'x-insights-flag-overrides') || params.get('flag_overrides') || undefined,
    }
}
