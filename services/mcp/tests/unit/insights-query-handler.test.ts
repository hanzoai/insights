...
function createContext(overrides?: { getData?: unknown; queryData?: unknown }): MockHandles {
    ...
    const context = {
        api: {
            ...
            request: vi.fn(),
            getProjectBaseUrl: vi.fn().mockReturnValue('https://us.hanzo.ai/project/1'),
        },
        stateManager: {
...
describe('queryHandler — link reflects overrides', () => {
    it('links to the bare saved insight when no overrides are passed', async () => {
        ...
        expect(result._insightsUrl).toBe('https://us.hanzo.ai/project/1/insights/abc12345')
        expect(result.insight.url).toBe('https://us.hanzo.ai/project/1/insights/abc12345')
    })
    ...
    it('encodes filters_override into the link query string', async () => {
        ...
        const expected = `https://us.hanzo.ai/project/1/insights/abc12345?filters_override=${encodeURIComponent(
            JSON.stringify(filtersOverrideObject)
        )}`
        expect(result._insightsUrl).toBe(expected)
    ...
    it('encodes both variables_override and filters_override into the link', async () => {
        ...
        const expected = `https://us.hanzo.ai/project/1/insights/abc12345?variables_override=${encodeURIComponent(
...
describe('queryHandler — result shape for UI rendering', () => {
    ...
    it('keeps structured results for a InsightsQL insight in optimized output', async () => {
        const { context } = createContext({
