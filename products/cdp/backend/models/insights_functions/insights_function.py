...
TYPES_THAT_RELOAD_PLUGIN_SERVER = (
    ...
    InsightsFunctionType.TRANSFORMATION_LOG,
...
TYPES_WITH_EXECUTION_ORDER = (InsightsFunctionType.TRANSFORMATION, InsightsFunctionType.TRANSFORMATION_LOG)

# Function types a cyclotron worker actually executes, so a "rerun" — which re-enqueues
# the stored invocation onto the cyclotron script queue — can run to completion. Every other
...
# rerun paginator (`RERUNNABLE_FN_FUNCTION_TYPES`) and the frontend invocations UI.
...
class InsightsFunction(FileSystemSyncMixin, UUIDTModel):
    ...
    # Used when the source language is Script (everything except the "site_*" types)
    ...
    @classmethod
    def get_file_system_unfiled(cls, team: "Team", surface: str = DEFAULT_SURFACE) -> QuerySet["InsightsFunction"]:
        base_qs = InsightsFunction.objects.filter(team=team, deleted=False)
        return cls._filter_unfiled_queryset(
            base_qs, team, type__startswith="insights_function/", ref_field="id", surface=surface
        )

    def get_file_system_representation(self) -> FileSystemRepresentation:
        folder = "Unfiled/Destinations"
        href = f"/pipeline/destinations/script-{self.pk}/configuration"
        type = self.type

        if self.type == InsightsFunctionType.SITE_APP:
            folder = "Unfiled/Site apps"
            href = f"/pipeline/site-apps/script-{self.pk}/configuration"
        elif self.type == InsightsFunctionType.TRANSFORMATION:
            folder = "Unfiled/Transformations"
            href = f"/pipeline/transformations/script-{self.pk}/configuration"
        elif self.type == InsightsFunctionType.TRANSFORMATION_LOG:
            folder = "Unfiled/Transformations"
    ...
    @property
    def url(self):
        return absolute_uri(f"/project/{self.team_id}/pipeline/destinations/script-{str(self.id)}")
...
def action_saved(sender, instance: Action, created, **kwargs):
    # Whenever an action is saved we want to load all script functions using it
...
def cohort_saved(sender, instance, **kwargs):
    ...
    # When a cohort changes, recompile script functions for any team that uses
    ...
    if team.test_account_filters and any(
        ...
        refresh_affected_insights_functions.delay(cohort_id=instance.id)
