#!/bin/sh
# Behavior tests + golden diff for the insights-metrics-agent Helm chart.
# Usage: products/metrics/agent/tests/helm/run.sh [--update-golden]
set -u
cd "$(dirname "$0")"
CHART=../../chart/insights-metrics-agent
...
out=$(render --set insights.apiKey=pk-test)
...
assert_contains default-pinned-image "image: 'insights/metrics-agent:$APP_VERSION'" "$out"
...
assert_not_contains default-key-not-in-configmap 'pk-test' "$configmap_only"

# --- existingSecret: chart must not create its own Secret ---
out=$(render --set insights.existingSecret=my-secret)
...
sum_a=$(render --set insights.apiKey=pk-a | grep 'checksum/secret:')
sum_b=$(render --set insights.apiKey=pk-b | grep 'checksum/secret:')
...
out=$(render --set insights.apiKey=pk-test -f values/static-targets.yaml)
...
# --- rbac disabled ---
out=$(render --set insights.apiKey=pk-test --set rbac.create=false --set serviceAccount.create=false --set serviceAccount.name=external-sa)
...
out=$(render --set insights.apiKey=pk-test --set serviceAccount.create=false)
assert_contains sa-name-required 'serviceAccount.name is required' "$out"

# --- eu host flows into the rendered collector config ---
out=$(render --set insights.apiKey=pk-test --set insights.host=https://eu.i.hanzo.ai)
assert_contains eu-host 'https://eu.i.hanzo.ai/i/v1/metrics' "$out"

# --- ingest path override (proxies, test sinks) ---
out=$(render --set insights.apiKey=pk-test --set insights.host=http://sink:4318 --set insights.ingestPath=/v1/metrics)
...
out=$(render --set insights.apiKey=pk-test --set shards=3)
...
static=$(render --set insights.apiKey=pk-test --set shards=3 -f values/static-targets.yaml)
assert_contains sharded-static-job-hashmod 'action: hashmod' "$static"

# --- podEnv passthrough: extra agent env vars ---
out=$(render --set insights.apiKey=pk-test --set podEnv.INSIGHTS_DEBUG=1 --set podEnv.SCRAPE_JOB_NAME=custom)
...
out=$(render --set insights.apiKey=pk-test --set persistence.enabled=true)
assert_contains persist-env 'name: PERSIST_QUEUE' "$out"
assert_contains persist-pvc 'kind: PersistentVolumeClaim' "$out"
assert_contains persist-mount 'mountPath: /var/lib/insights-agent' "$out"
...
out=$(render --set insights.apiKey=pk-test --set persistence.enabled=true --set shards=3)
...
default=$(render --set insights.apiKey=pk-test | grep -v '^[[:space:]]*$')
