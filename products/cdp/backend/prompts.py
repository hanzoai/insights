...
Use `create_insights_function_inputs` when answering ANY requests remotely related to creating, modifying, or managing input variables for script functions.
It's very important to disregard other tools for these purposes - the user expects `create_insights_function_inputs`.

NOTE: When calling the `create_insights_function_inputs` tool, do not provide any response other than the tool call.
...
Use `create_insights_function_filters` when answering ANY requests remotely related to setting up filters, event matching, property filtering, or trigger conditions for script functions.
It's very important to disregard other tools for these purposes - the user expects `create_insights_function_filters`.

NOTE: When calling the `create_insights_function_filters` tool, do not provide any response other than the tool call.
...
Script's standard library
Script's standard library includes the following functions and will expand. To see the the most update-to-date list, check the Python VM's stl/__init__.py file.
...
Here are examples of the syntax. Do not use any other functions since they are not supported in Script:

Syntax
Comments
Script comments start with //. You can also use SQL style comments with -- or C++ style multi line blocks with /*.
...
Arrays in Script (and our SQL flavor) are 1-indexed!
...
Tuples in Script (and our SQL flavor) are 1-indexed!
...
let square := (a) -> a * a
print(square(4))
See Script's standard library for a list of built-in functions.
...
Here are some more rules around Script:

Here are a few key differences compared to other programming languages:

- Variable assignment in Script is done with the := operator, as = and == are both used for equality comparisons in SQL
- You must type out and, or and not. Currently && and ! raise syntax errors, whereas || is used as the string concatenation operator.
- All arrays in Script start from index 1. Yes, for real. Trust us, we know. However that's how SQL has always worked, so we adopted it.
...
- Never use arr[a:b]; Script does not support slice syntax. Use substring(str, offset, length) for strings.
- delete does not work in Script.
...
TRANSFORMATION_STRUCTURE_MESSAGE = """A Script transformation is a top-level script, NOT a wrapped function.
...
// SELECT statement
select: (selectSetStmt | selectStmt | insightsqlxTagElement) EOF;
...
//    | EXTRACT LPAREN interval FROM columnExpr RPAREN                                      # ColumnExprExtract   // Interferes with a function call
    ...
    | insightsqlxTagElement                                                                    # ColumnExprTagElement
...
tableExpr
    ...
    | insightsqlxTagElement                   # TableExprTag
