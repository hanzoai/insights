...
# Only these (Datastore-backed) evaluators are gated to once per team-local day. The cheap DB-backed
...
def streak_arm_for_user(user: User) -> str | None:
    if not user.distinct_id:
        return None
    try:
        variant = hanzo_insights.get_feature_flag(STREAK_CADENCE_FLAG, str(user.distinct_id))
    except Exception:
        return None
