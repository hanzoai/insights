...
| DuckLake load (duckgres)             | `pipeline_v3/duckgres/`                                                                                | A second queue-consumer deployment (`run_warehouse_sources_duckgres_load`) applying the same batches to the DuckLake store, plus its backfill planner.                                                                                                                                              |
...
- `core/` - version-neutral pieces both pipelines use: `batcher.py`, `arrow_utils.py`, `partitioning.py`, `insightsql_schema.py`, `sinks.py`, the repartitioner, and `typings.py` (`PipelineResult`).
