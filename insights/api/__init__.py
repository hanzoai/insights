...
# `insights.api.rest_router`, imported lazily — only when a name it defines or re-exports
# (the router objects, view modules like `dashboard`) is first accessed, which in practice
# is when the URLconf resolves (first web request). Real submodules (`insights.api.monitoring`,
# `.file_system`, ...) resolve directly without building the aggregator, so a plain
# `django.setup()` (shell, migrate, celery, CI) stays cheap. See insights/api/rest_router.py.
...
def __getattr__(name: str) -> Any:
    ...
    from insights.api import rest_router  # noqa: PLC0415
...
if TYPE_CHECKING:
    from insights.api.rest_router import (
