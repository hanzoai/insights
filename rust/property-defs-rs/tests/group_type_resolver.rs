...
fn make_config(addr: &str) -> property_defs_rs::config::Config {
    ...
    config.personinsights_addr = addr.to_string();
    config.personinsights_timeout_ms = 5000;
    config.personinsights_max_retries = 2;
    config.personinsights_initial_backoff_ms = 1;
    config.personinsights_max_backoff_ms = 10;
...
#[tokio::test]
async fn test_personinsights_resolves_group_types() {
...
#[tokio::test]
async fn test_personinsights_resolves_multiple_teams() {
...
#[tokio::test]
async fn test_personinsights_unresolved_group_type_preserved_not_cleared() {
...
#[tokio::test]
async fn test_negative_cache_skips_personinsights_within_ttl() {
    // A miss is cached for the TTL window so repeated unresolvable keys don't re-hit personinsights.
    ...
    // Second resolve of the same key is served from the negative cache: no personinsights call,
...
async fn test_negative_cache_reattempts_after_ttl_expiry() {
    ...
    // resolution via personinsights (this is what lets a legitimate new group resolve once its
    // mapping lands).
    let call_count = Arc::new(AtomicUsize::new(0));
    let mock = MockPersonHogService::with_mappings_counted(
        vec![GroupTypeMappingsByKey {
...
#[tokio::test]
async fn test_personinsights_failure_returns_error() {
...
#[tokio::test]
async fn test_personinsights_caches_resolved_types() {
    let call_count = Arc::new(AtomicUsize::new(0));
    let mock = MockPersonHogService::with_mappings_counted(
        vec![GroupTypeMappingsByKey {
            key: 1,
...
#[tokio::test]
async fn test_no_personinsights_client_returns_error() {
    // Empty addr = no personinsights client created
...
async fn test_no_retries_when_max_retries_is_zero() {
    ...
    let addr = start_mock_server(mock).await;
    let mut config = make_config(&format!("http://{addr}"));
    config.personinsights_max_retries = 0;
    let resolver = GroupTypeResolver::new(&config);
...
async fn test_end_to_end_poisoned_group_def_recovers_and_persists(db: PgPool) {
    ...
    let call_count = Arc::new(AtomicUsize::new(0));
    // First personinsights lookup returns no mapping (the miss that poisons); the second returns the
    ...
    // TTL 0 makes round 1's negative-cache entry immediately stale, so round 2 re-drives personinsights.
    ...
    // The negative entry has expired (TTL 0), so personinsights is re-driven and now resolves to 5.
    ...
    let persisted_index: Option<i16> = sqlx::query_scalar!(
        r#"SELECT group_type_index FROM insights_propertydefinition WHERE type = 3"#
    )
    .fetch_one(&db)
    .await
    .unwrap();
    ...
    assert_eq!(
        ...
        2,
        "round 2 must re-drive personinsights after the TTL lapse"
    );
