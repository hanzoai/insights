...
_START = "products.review.backend.api.trigger.start_review_pr_workflow"
...
class TestReviewTriggerApi(APIBaseTest):
    def setUp(self):
        ...
        self._settings_ctx = self.settings(
            REVIEWFN_TEAM_ID=self.trigger_team.id,
            REVIEWFN_RUN_USER_ID=self.run_user.id,
        )
    ...
    def test_valid_trigger_starts_workflow_and_publishes_by_default(self, mock_start):
        resp = self.client.post(
            TRIGGER_URL,
            {"repo": "Insights/insights", "pr_number": 123},
            format="json",
    ...
    def test_publish_flag_passes_through(self, mock_start):
        resp = self.client.post(
            TRIGGER_URL,
            {"repo": "Insights/insights", "pr_number": 5, "publish": False},
            format="json",
            HTTP_AUTHORIZATION="Bearer secret-token",
        )
        self.assertEqual(resp.status_code, status.HTTP_202_ACCEPTED, resp.content)
    ...
    def test_invalid_token_rejected(self, _name, auth_header, mock_start):
        resp = self.client.post(
            TRIGGER_URL,
            {"repo": "Insights/insights", "pr_number": 1},
            format="json",
    ...
    def test_allowlist_is_case_insensitive(self, mock_start):
        resp = self.client.post(
            TRIGGER_URL,
            {"repo": "insights/insights", "pr_number": 7},
            format="json",
            HTTP_AUTHORIZATION="Bearer secret-token",
        )
        self.assertEqual(resp.status_code, status.HTTP_202_ACCEPTED, resp.content)
    ...
    @override_settings(REVIEWFN_TEAM_ID=None)
    @patch(_START, return_value="wf-1")
    ...
    @override_settings(DEBUG=False, TEST=False, REVIEWFN_TRIGGER_TOKEN=None)
    @patch(_START, return_value="wf-1")
    ...
    @override_settings(REVIEWFN_RUN_USER_ID=None)
    @patch(_START, return_value="wf-1")
    ...
    def test_inactive_integration_creator_falls_back_to_active_org_member(self, _name, keep_membership, mock_start):
        departed = User.objects.create(email="departed@hanzo.ai", is_active=False)
        if keep_membership:
