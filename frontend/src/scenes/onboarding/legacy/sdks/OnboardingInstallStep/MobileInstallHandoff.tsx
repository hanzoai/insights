...
export function MobileInstallHandoff({
    ...
    useEffect(() => {
        ...
        insights.capture('mobile install handoff shown', { product_key: productKey })
    }, [productKey])
    ...
    const handleShare = async (): Promise<void> => {
        insights.capture('mobile install handoff clicked', { method: 'native_share' }, { send_instantly: true })

        try {
            await navigator.share({
                title: 'Finish setting up Insights',
                text: "Here's where I left off setting up Insights. Open this on your computer to continue.",
                url: buildHandoffUrl(),
            })
            insights.capture('mobile install handoff share completed', {}, { send_instantly: true })
            setShareState('sent')
        } catch (err) {
            const name = err instanceof Error ? err.name : 'unknown'
            if (name === 'AbortError') {
                insights.capture('mobile install handoff share cancelled', {}, { send_instantly: true })
                return
            }
            insights.capture('mobile install handoff share failed', { error: name }, { send_instantly: true })
    ...
    const handleCopy = async (shouldCapture = true): Promise<void> => {
        ...
        if (success) {
            if (shouldCapture) {
                insights.capture('mobile install handoff clicked', { method: 'copy_link' })
            }
            setShareState('sent')
        } else {
            insights.capture('mobile install handoff copy failed')
        }
    }

    const handleContinueHere = (): void => {
        insights.capture('mobile install handoff clicked', { method: 'continue_here' }, { send_instantly: true })
        onContinueHere()
    }
                ...
                {installationComplete ? (
                    <>
                        ...
                        <p className="text-muted">
                            Insights is receiving events from your project. You&apos;re ready to continue.
                        </p>
                        <div className="pt-2">
                            <Button
                                type="primary"
                    ...
                    <>
                        ...
                        <p className="text-muted">
                            Insights&apos;s install command needs to run on a computer with your code. We can share the
