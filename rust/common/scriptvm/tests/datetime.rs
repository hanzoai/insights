//! Coverage for the `toDateTime`/`toDate` natives and Script-temporal comparison. This VM orders
//! temporals by `dt` seconds to match Datastore (the reference Python/TS ScriptVMs cannot order them);
//! naive strings parse as UTC, not the process-local timezone.

use chrono::{NaiveDate, TimeZone, Utc};
use scriptvm::{sync_execute, ExecutionContext, Program};
use serde_json::{json, Value};

// Opcode numeric values (mirror common/scriptvm/python/operation.py).
...
fn legacy_default_compares_temporals_structurally_not_by_epoch() {
    ...
    // distinct objects. The opt-in coercing path treats them as equal (Datastore `is_date_exact`);
