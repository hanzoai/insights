...
on-demand precomputed Datastore tables via the `lazy_computation` framework.
...
from insights.schema import WebOverviewQuery, WebStatsTableQuery, WebVitalsPathBreakdownQuery

from insights.insightsql import ast
from insights.insightsql.parser import parse_select
from insights.insightsql.placeholders import find_placeholders
from insights.insightsql.property import get_property_type, property_to_expr
from insights.insightsql.transforms.preaggregated_table_transformation import is_integer_timezone
...
# insights-js hard cap, covering effectively the whole population).
...
def check_common_eligible(runner: LazyPrecomputeRunner, *, require_integer_timezone: bool = True) -> None:
    ...
    # Rollout gate: shared Insights feature flag AND per-query opt-out.
    #   - `web-analytics-precompute-toggle` (Insights feature flag): the same
...
class _GlobalizeSessionTupleIn(TraversingVisitor):
    ...
    InsightsQL has no grammar for `GLOBAL IN`, and the sessions_v2 pushdown rewrite
...
def build_insert_select_ast(template: str, placeholders: dict[str, ast.Expr]) -> ast.SelectQuery:
    ...
    substituted, and session-id tuple INs upgraded to GLOBAL IN (no InsightsQL grammar for it)."""
