...
from products.warehouse_sources.backend.temporal.data_imports.sources.datastore.datastore import (
    ...
    get_datastore_row_count,
    get_connection_metadata as get_datastore_connection_metadata,
    get_primary_keys_for_schemas as get_datastore_primary_keys_for_schemas,
    get_schemas as get_datastore_schemas,
)
...
GENERIC_CONNECTION_ERROR = (
    "Could not connect to Datastore. Check that the host and port are correct, the HTTPS setting matches "
    "your server, and that Insights's IP addresses are allowed through any firewall."
)
...
# waking (idle Datastore Cloud services routinely 5xx the first request), so
...
DatastoreErrors: dict[str, str] = {
    ...
    "nodename nor servname provided": "Could not resolve the Datastore host",
    ...
    "returned response code 404": "We reached your Datastore host but it returned a 404, so it isn't serving the Datastore HTTP interface on that host/port. Please check the host, port, and HTTPS setting (and any tunnel or proxy in front of it).",
...
class DatastoreSource(SimpleSource[DatastoreSourceConfig], SSHTunnelMixin, ValidateDatabaseHostMixin):
    ...
    api_docs_url = "https://datastore.com/docs"
    ...
    def get_source_config(self) -> SourceConfig:
        return SourceConfig(
            ...
            caption="Enter your Datastore connection details to pull data into the Insights Data warehouse. Datastore databases can be very large — we stream the data in Arrow batches to keep memory bounded.",
            iconPath="/static/services/datastore.png",
            docsUrl="https://hanzo.ai/docs/cdp/sources/datastore",
            fields=cast(
                list[FieldType],
                [
                    SourceFieldInputConfig(
                        ...
                        required=False,
                        placeholder="https://user:password@play.datastore.com:8443/default",
                        secret=True,
                    ),
                    SourceFieldInputConfig(
                        name="host",
                        ...
                        type=SourceFieldInputConfigType.TEXT,
                        required=True,
                        placeholder="play.datastore.com",
                        secret=False,
                    ),
                    SourceFieldInputConfig(
                        name="port",
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            # wrong, the SSH key/credentials are rejected, or a firewall blocks Insights's IPs. It's
            ...
            "Could not establish session to SSH gateway": "Could not connect to your SSH tunnel. Check that the SSH host, port, and credentials are correct, the bastion host is running and reachable, and that Insights's IP addresses are allowed through its firewall.",
            "Could not resolve the Datastore host": None,
            ...
            # datastore-connect's HTTP driver got a 404 back while opening the
            # connection ("HTTPDriver for <url> returned response code 404").
            # The host responded but isn't serving the Datastore HTTP interface
            # on that path — typically a tunnel/proxy pointing at the wrong
            # service or an offline endpoint. A real Datastore server never
            ...
            # isn't a Datastore response. The endpoint isn't serving the Datastore HTTP
            ...
            "did not return a valid Datastore response": NOT_A_DATASTORE_HTTP_RESPONSE,
            # MEMORY_LIMIT_EXCEEDED — the source Datastore server (per-query
            ...
            "Code: 241": "Your Datastore server ran out of memory while we were reading a table. This usually means the database is too small for the table being synced. Try scaling up your Datastore service (or raising its memory limit), or sync a smaller table or use an incremental sync, then resume.",
            # NOT_ENOUGH_SPACE — the source Datastore server couldn't reserve
            ...
            "Code: 243": "Your Datastore server ran out of disk space while we were reading a table (it couldn't reserve space for a temporary file). Try scaling up your Datastore service or freeing disk space, or sync a smaller table or use an incremental sync, then resume.",
            ...
            "not found or has no columns": "We couldn't find this table in your Datastore database — it may have been dropped or renamed. If you were syncing a materialized view, sync it by its own name rather than its internal `.inner_id.<uuid>` table (those names change whenever the view is recreated). Remove or re-point this table in your source, then resync.",
            # UNKNOWN_TYPE (code 50) raised while Datastore streams our extraction
            # query as Arrow: a selected column has a type Datastore can't serialize
            ...
            "is not supported for conversion into Arrow data format": "One of the columns in this table has a type Datastore can't export (for example an `AggregateFunction` state column on an aggregating materialized view). Deselect that column in this schema's column settings, or sync a view that finalizes it, then resync.",
        }

    def get_retryable_errors(self) -> set[str]:
        # `_get_client` already retries dropped connections and rate limits in-process
        # (see datastore.py's `_is_retryable_connect_error`) before re-raising as
        # `DatastoreConnectionError`. Bare HTTP 502/503/504 responses skip that
    ...
    def direct_query_client(
        self,
        config: DatastoreSourceConfig,
        team_id: int,
        *,
        query_timeout: int,
        settings: dict[str, Any] | None = None,
    ) -> Iterator["DatastoreClient"]:
        """Open a client against the source's Datastore for a single direct (InsightsQL) query.

        Opens the SSH tunnel (if any) for the life of the query and yields a datastore-connect
    ...
    def get_schemas(
        ...
        for table_name, columns in db_schemas.items():
            incremental_field_tuples = filter_datastore_incremental_fields(columns)
            # In Datastore the table's ORDER BY (sorting key) is the only access
    ...
    def validate_credentials(
        ...
        try:
            ...
            return (
                False,
                e.value
                or "Could not connect to Datastore via the SSH tunnel. Please check all connection details are valid.",
            )
        except DatastoreConnectionError as e:
            message = self._translate_error(str(e))
            return False, message or GENERIC_CONNECTION_ERROR
        except (DatabaseError, OperationalError, DatastoreError) as e:
            message = self._translate_error(str(e))
            if message is None:
                capture_exception(e)
    ...
    def _translate_error(error_msg: str) -> str | None:
        ...
        for key, value in DatastoreErrors.items():
            if key in lowered:
                return value
        return None

    def get_connection_metadata(self, config: DatastoreSourceConfig, team_id: int) -> dict[str, object]:
        with self.with_ssh_tunnel(config, team_id) as (host, port):
            return get_datastore_connection_metadata(
                host=host,
                port=port,
                database=config.database,
                user=config.user,
    ...
    def reconcile_schema_metadata(
        ...
        column picker populates. Datastore isn't a `SQLSource`, but the
