...
then produces a Kafka message to `KAFKA_DATASTORE_SESSION_REPLAY_EVENTS` with identity values for every non-tagging
...
async def emit_classifier_tags_activity(inputs: EmitClassifierTagsInputs) -> None:
    ...
    tag_row_ts = format_datastore_timestamp(session_start + timedelta(microseconds=1))
    payload = {
    ...
    def emit() -> None:
        with producer_scope(
            topic=KAFKA_DATASTORE_SESSION_REPLAY_EVENTS, flush_timeout=KAFKA_DELIVERY_TIMEOUT_S
        ) as producer:
            result = producer.produce(topic=KAFKA_DATASTORE_SESSION_REPLAY_EVENTS, data=payload)
