...
async def tag_and_highlight_session_activity(
    ...
    """Write session tags and highlight flag to Datastore via Kafka."""
    try:
...
def _produce_to_kafka(
    ...
    """Produce a Kafka message to write tags and highlight flag to Datastore.
    ...
    """
    tag_row_ts = format_datastore_timestamp(session_start_time + timedelta(microseconds=1))
    data = {
        "session_id": inputs.session_id,
    ...
    get_producer(topic=KAFKA_DATASTORE_SESSION_REPLAY_EVENTS).produce(
        topic=KAFKA_DATASTORE_SESSION_REPLAY_EVENTS, data=data
    )
