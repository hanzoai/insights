...
from insights.ducklake import cp_teams, team_state
...
def _patch_org_rows(rows):
    return patch("insights.ducklake.cp_teams._fetch_org_rows", return_value=rows)
...
class TestDataImportsSchema:
    def test_resolves_from_cp_row(self) -> None:
        ...
        with _patch_org_rows([_cp_row(team, "cp_schema")]):
            assert team_state.data_imports_schema(team.id) == "insights_data_imports_cp_schema"

    def test_without_cp_row_falls_back_to_team_id_schema(self) -> None:
        org, team = _team()
        with _patch_org_rows([]):
            assert team_state.data_imports_schema(team.id) == f"insights_data_imports_team_{team.id}"
...
class TestListEnabledBackfillRows:
    def test_returns_cp_rows_with_server_shim(self) -> None:
        ...
        with patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=cp_rows):
    ...
    def test_returns_empty_without_raising_when_cp_down(self) -> None:
        with patch("insights.ducklake.cp_teams._fetch_all_rows", return_value=None):
            assert team_state.list_enabled_backfill_rows("test") == []
