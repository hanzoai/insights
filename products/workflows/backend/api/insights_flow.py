...
class InsightsFlowViewSet(
    ...
    def _stage_revision_bump(self, instance: InsightsFlow, before: InsightsFlow, validated_data: dict, enabled: bool) -> bool:
    ...
    def graph(self, request: Request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            route_to_draft = False
            if self._is_mcp_request(request) and locked.status == InsightsFlow.State.ACTIVE:
                if not revisions_enabled:
                    raise exceptions.ValidationError(MCP_ACTIVE_EDIT_REJECTION)
                route_to_draft = True
