...
def _fetch_event_properties_from_datastore(team: Team, inputs: EventPropertiesWorkflowInputs) -> dict[str, object]:
    ...
    with ERROR_TRACKING_EVENT_PROPERTIES_READ_DURATION.labels(source="datastore").time():
    ...
    if not response.results:
        ERROR_TRACKING_EVENT_PROPERTIES_READS.labels(source="datastore", outcome="miss").inc()
    ...
    ERROR_TRACKING_EVENT_PROPERTIES_READS.labels(source="datastore", outcome="hit").inc()
    return properties
...
def fetch_event_properties(team: Team, inputs: EventPropertiesWorkflowInputs) -> dict[str, object]:
    ...
    if properties is not None:
        return properties
    return _fetch_event_properties_from_datastore(team, inputs)
