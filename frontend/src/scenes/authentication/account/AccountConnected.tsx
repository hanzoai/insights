...
// Per-kind deep link host. GitHub already uses `insights-code://integration`;
...
const DEEP_LINK_HOSTS: Record<Exclude<AccountConnectedKind, 'invalid'>, string> = {
    'github-login': 'insights-code://integration',
    'github-integration': 'insights-code://integration',
    'slack-integration': 'insights-code://slack-integration',
}

function insightsCodeDeepUrl(
...
/**
 * Unified return page for Insights Desktop / web: social SSO link (`github-login`), personal GitHub
 * integration (`github-integration`), or team Slack integration (`slack-integration`). Navigates
 * to the matching `insights-code://…` deep link so the desktop app refreshes its integrations —
...
export function AccountConnected({ kind }: AccountConnectedProps): JSX.Element {
    ...
    // show the success state. Insights Desktop refreshes its integrations via a desktop deep link.
    ...
    // edge case) can't crash `insightsCodeDeepUrl` with an undefined deep-link host.
    const hasValidKind = isValidKind(kind)

    useEffect(() => {
        if (hasValidKind && !startedFromSlack) {
            window.location.href = insightsCodeDeepUrl(kind, searchParams)
        }
            ...
            <BridgePage view="account-connected">
                <div className="flex flex-col items-center gap-4 text-center max-w-lg mx-auto">
                    <p className="text-muted mb-0">
                        This link is not valid. Return to Insights from the product you started from.
                    </p>
                </div>
            </BridgePage>
        )
    ...
    return (
        <BridgePage view="account-connected">
            <div className="flex flex-col items-center gap-4 text-center max-w-lg mx-auto">
                ...
                {showLoginLine && <p className="text-muted mb-0">You can now log into Insights using {label}.</p>}
                {startedFromSlack ? (
                    <p className="text-muted mb-0">
                        {isError
                    ...
                    <p className="text-muted mb-0">
                        <strong>Returning to Insights Desktop…</strong>
                        <br />
                        <em>If this hasn't happened automatically, get back to the Insights Desktop app manually.</em>
                    </p>
                )}
            </div>
        </BridgePage>
