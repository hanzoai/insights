...
def test_build_agent_server_command_gates_exec_permission_regex(sandbox: DockerSandbox):
    with_flag = sandbox._build_agent_server_command(
        None, "t1", "r1", "interactive", True, insights_exec_permission_regex=INSIGHTS_EXEC_PERMISSION_REGEX
    )
    assert f"--insightsExecPermissionRegex {shlex.quote(INSIGHTS_EXEC_PERMISSION_REGEX)}" in with_flag
    ...
    assert "--insightsExecPermissionRegex" not in without_flag
...
def test_exec_permission_regex_matches_gated_sub_tools(sub_tool: str, should_match: bool):
    ...
    assert bool(re.search(INSIGHTS_EXEC_PERMISSION_REGEX, sub_tool, re.IGNORECASE)) is should_match
