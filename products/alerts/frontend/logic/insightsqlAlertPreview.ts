...
import { AlertConfig, isInsightsQLAlertConfig } from '../types'
import { hasThresholdBounds, valueBreachesBounds } from './alertPreviewShared'

/** Mirror of the backend's ANY_ROW_MAX_ROWS (products/alerts/backend/evaluation/insightsql.py) — keep
 * the two in sync. Advisory only; the backend extractor is the evaluation-time authority. */
export const INSIGHTSQL_ANY_ROW_MAX_ROWS = 50
...
export const INSIGHTSQL_LAST_ROW_MAX_ROWS = 50000

/** One result row as the alert would read it, for the configure-time preview table. */
export interface InsightsQLAlertPreviewRow {
 ...
 * `InsightsQLExtractor` in products/alerts/backend/evaluation/insightsql.py; any rule change there must
 * land here and in both test suites. */
export type InsightsQLAlertPreview =
    | { status: 'no-rows' }
    | { status: 'bad-shape' }
    ...
    | {
          ...
          rows: InsightsQLAlertPreviewRow[]
      }
...
/** Mirror of the backend's `_column_is_numeric` (products/alerts/backend/evaluation/insightsql.py): a
...
export function deriveInsightsQLAlertPreview(
    ...
    const insightsqlConfig = isInsightsQLAlertConfig(config) ? config : null
    const mode = insightsqlConfig?.evaluation ?? 'last_row'
    ...
    if (mode === 'any_row' && rows.length > INSIGHTSQL_ANY_ROW_MAX_ROWS) {
        return { status: 'too-many-rows', rowCount: rows.length }
    }
    if (mode === 'last_row' && rows.length >= INSIGHTSQL_LAST_ROW_MAX_ROWS) {
    ...
    if (insightsqlConfig?.column != null) {
        const index = columnNames?.indexOf(insightsqlConfig.column) ?? -1
        if (index < 0) {
            return { status: 'missing-column', column: insightsqlConfig.column, columnNames }
        }
        valueIndex = index
    ...
    if (insightsqlConfig?.label_column != null) {
        const index = columnNames?.indexOf(insightsqlConfig.label_column) ?? -1
        if (index < 0) {
            return { status: 'missing-column', column: insightsqlConfig.label_column, columnNames }
        }
        labelIndex = index
    } else if (lastRow.length > 1) {
    ...
    const previewRows: InsightsQLAlertPreviewRow[] = rows.map((row, i) => {
