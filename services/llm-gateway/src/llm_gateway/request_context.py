...
class RequestContext:
    ...
    insights_properties: dict[str, str] | None = None
    insights_flags: dict[str, str] | None = None
...
def set_insights_properties(properties: dict[str, str] | None) -> None:
    ctx = request_context_var.get()
    if ctx is None:
        return
    request_context_var.set(replace(ctx, insights_properties=properties))


def get_insights_properties() -> dict[str, str] | None:
    ctx = request_context_var.get()
    return ctx.insights_properties if ctx else None


def set_insights_flags(flags: dict[str, str] | None) -> None:
    ctx = request_context_var.get()
    if ctx is None:
        return
    request_context_var.set(replace(ctx, insights_flags=flags))


def set_insights_context(
    ...
    if ctx is None:
        return
    request_context_var.set(replace(ctx, insights_properties=properties, insights_flags=flags))


def get_insights_flags() -> dict[str, str] | None:
    ctx = request_context_var.get()
    return ctx.insights_flags if ctx else None
...
def apply_insights_context_from_headers(request: Request) -> None:
    properties = extract_insights_properties_from_headers(request)
    flags = extract_insights_flags_from_headers(request)

    if properties:
        set_insights_properties(properties)
    if flags:
        set_insights_flags(flags)
