...
class Command(BaseCommand):
    help = (
        "Show unified signal pipeline status across Temporal, Datastore, and Postgres.\n\n"
        "Checks the active Signals ingestion workflows (buffer + grouping-v2), "
        "Datastore embeddings, and SignalReport table to give a complete view "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="In --wait mode, wait until at least this many signals appear in Datastore",
        )
    ...
    def _wait_loop(self, team, options):
        ...
        while True:
            ...
            ch_count = status["datastore"].get("total_signals", 0)
    ...
    def _collect_status(self, team, report_id=None):
        ...
        datastore_status = self._check_datastore(team, report_id)
    ...
    def _check_datastore(self, team, report_id=None):
        try:
            count_query = """
                SELECT
                    count() as total,
            ...
            return {"error": f"Datastore query failed: {e}"}
