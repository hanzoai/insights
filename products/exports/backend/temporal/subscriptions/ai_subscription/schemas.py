...
class QueryPlanStep(BaseModel):
    ...
    query_type: Literal["insightsql"] = Field("insightsql", description="MVP: always 'insightsql'.")
    insightsql: str = Field(..., max_length=5000, description="A InsightsQL SELECT statement scoped to the team's events.")
...
class InsightsQLFix(BaseModel):
    fixed_insightsql: str = Field(
        ...,
        description="A InsightsQL SELECT statement (flat, or with a single FROM-subquery) that addresses the original step intent.",
    )
