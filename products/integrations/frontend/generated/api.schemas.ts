...
export interface RoleExternalReferenceApi {
    ...
    /** Insights role UUID this external role maps to. */
    role: string
    readonly created_at: string
    readonly created_by: UserBasicApi
}
 ...
 * * `slack` - Slack
 * * `slack-insights-code` - Slack Insights Code
 * * `snapchat` - Snapchat
 * * `snowflake` - Snowflake
 * * `stripe` - Stripe
...
export const IntegrationKindEnumApi = {
    ...
    Salesforce: 'salesforce',
    Slack: 'slack',
    SlackInsightsCode: 'slack-insights-code',
    Snapchat: 'snapchat',
    Snowflake: 'snowflake',
    Stripe: 'stripe',
...
export interface SlackChannelApi {
    ...
    is_private: boolean
    /** True if the Insights Slack app is a member of the channel and can post to it. */
    is_member: boolean
    /** True if the channel is shared with another Slack workspace. */
    is_ext_shared: boolean
    /** True if the channel is private and the Insights Slack app cannot access it. */
    is_private_without_access: boolean
}

export interface SlackChannelsResponseApi {
    /** Slack channels visible to the Insights Slack app. */
...
export interface GitHubOAuthAuthorizeRequestApi {
    ...
    next?: string
    /** Originating surface for the connect flow; only 'insights_code' is recognized.
     *
     * * `insights_code` - insights_code */
...
export interface GitHubPrepareCallbackRequestApi {
    /** Relative URL to redirect to after GitHub setup completes (e.g. account-connected for Insights Desktop). */
    next?: string
