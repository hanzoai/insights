...
class EventStream(TeamScopedRootMixin, UUIDModel, CreatedMetaFields, UpdatedMetaFields):
    ...
    InsightsFunction destination (referenced by ``insights_function_id``) whose filters are rebuilt
    ...
    created_by = models.ForeignKey("insights.User", on_delete=models.CASCADE, db_constraint=False)

    enabled = models.BooleanField(default=False)
    event_names = models.JSONField(default=list)

    slack_integration = models.ForeignKey(
        "insights.Integration", on_delete=models.SET_NULL, null=True, blank=True, related_name="+"
    )
    ...
    insights_function_id = models.UUIDField(null=True, blank=True)
