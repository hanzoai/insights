...
def flatten_block_text(node: Any) -> list[str]:
    ...
    Slack alert posts (subscriptions, log alerts, script-function destinations) often
...
def extract_message_text(msg: dict) -> str:
    # Always include `text` and `blocks`/`attachments`: Insights's own alert templates put
...
def collect_thread_messages(
    ...
    for index, msg in enumerate(raw_messages):
        ...
        # always the originating message (e.g. a Insights alert) that's the actual context
        # for the task. Filtering it by bot_id breaks workspaces where the alerting Slack
        # app and the `@Insights` code app share an installation identity.
