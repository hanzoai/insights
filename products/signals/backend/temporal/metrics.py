...
def increment_ch_wait_timeout() -> None:
    """Count a give-up of the wait-for-signal-in-Datastore consistency wait."""
    if not _in_temporal_context():
        return
    get_metric_meter().create_counter(
        "signals_ch_wait_timeouts_total",
        "Times the wait-for-signal-in-Datastore activity gave up after its max wait",
    ).add(1)
...
def increment_ch_wait_query(mode: str, reason: str) -> None:
    """Count Datastore confirmation queries by wait mode and reason."""
    if not _in_temporal_context():
        return
    get_metric_meter({"mode": mode, "reason": reason}).create_counter(
        "signals_ch_wait_queries_total",
        "Datastore confirmation queries issued by signals waits",
    ).add(1)
