...
description: Write or update the user-facing hanzo.ai documentation for a Insights Data warehouse import source. Use when adding a new source doc, fixing an inconsistent or stub source doc, or standardizing the docs at contents/docs/cdp/sources. Covers the canonical template, shared snippets, the auto-rendered <SourceParameters /> and <SourceTables /> components, frontmatter, and the docsUrl/slug rule that prevents 404s.
---

# Documenting Data warehouse sources

User-facing source docs live in the **hanzo.ai** repo (not this one) at
...
Assume a sibling hanzo.ai checkout (e.g. `../hanzo.ai`).
...
**Datastore** (database). Don't invent sections the source doesn't need.
...
2. The source's `docsUrl` in `get_source_config`: `https://hanzo.ai/docs/cdp/sources/<slug>`.
...
a doc, run the audit from this (insights) repo:
...
Renaming a published doc also needs a 301 in `hanzo.ai/vercel.json` for both
