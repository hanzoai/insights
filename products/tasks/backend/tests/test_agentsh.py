...
class TestGeneratePolicyYaml(TestCase):
    ...
    def test_always_includes_gateway_domains(self):
        policy = yaml.safe_load(generate_policy_yaml([]))
        allow_rule = next(rule for rule in policy["network_rules"] if rule["name"] == "allow-domains")
        self.assertIn("gateway.us.hanzo.ai", allow_rule["domains"])
        self.assertIn("gateway.eu.hanzo.ai", allow_rule["domains"])
    ...
    def test_go_gateway_hosts_reachable(self):
        ...
        policy = yaml.safe_load(generate_policy_yaml([]))
        allow_rule = next(rule for rule in policy["network_rules"] if rule["name"] == "allow-domains")
        self.assertIn("ai-gateway.us.hanzo.ai", allow_rule["domains"])
        self.assertIn("ai-gateway.eu.hanzo.ai", allow_rule["domains"])
    ...
    def test_configured_gateway_host_reachable_outside_debug(self):
        # Dev's gateway host is outside *.hanzo.ai, so only the
        ...
        policy = yaml.safe_load(generate_policy_yaml([]))
        allow_rule = next(rule for rule in policy["network_rules"] if rule["name"] == "allow-domains")
        self.assertIn("ai-gateway.dev.insights.dev", allow_rule["domains"])
    ...
    @override_settings(DEBUG=False, SANDBOX_AI_GATEWAY_URL="ai-gateway.dev.insights.dev")
    def test_schemeless_setting_warns_and_admits_nothing(self):
        ...
        allow_rule = next(rule for rule in policy["network_rules"] if rule["name"] == "allow-domains")
        self.assertNotIn("ai-gateway.dev.insights.dev", allow_rule["domains"])
