...
Datastore table, referenced in InsightsQL as **`insights.ai_events`**. The `events` table keeps only the lightweight metadata (token counts, costs,
...
`insights.ai_events` is `ORDER BY (team_id, trace_id, timestamp)`, so **`trace_id` is the access
...
`insights.ai_events` for you. Drop to the SQL below only for custom analysis (aggregations, joins,
...
`events` table first to get the trace IDs, then fetch the heavy content from `insights.ai_events`
...
In raw SQL these live only on `insights.ai_events`, not `events.properties` —
