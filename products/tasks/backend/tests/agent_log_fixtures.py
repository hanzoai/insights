...
def _agent_error_line(message: str, category: str | None = None) -> str:
    """Build a `_insights/error` notification line as the sandbox agent emits on a
    ...
    return json.dumps({"notification": {"method": "_insights/error", "params": params}})
...
def _console_line(message: str = "agentsh network events", method: str = "_insights/console") -> str:
...
def _progress_line(status: str = "failed", step: str = "agent", label: str = "Running agent") -> str:
    # A `_insights/progress` notification. The workflow's failure/cancel handlers emit one with
