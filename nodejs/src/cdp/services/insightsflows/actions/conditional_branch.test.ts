...
describe('action.conditional_branch', () => {
    ...
    describe('wait_until_condition eventMatched short-circuit', () => {
        let waitInvocation: CyclotronJobInvocationInsightsFlow
        let waitAction: Extract<InsightsFlowAction, { type: 'wait_until_condition' }>
        let handler: ConditionalBranchHandler

        beforeEach(() => {
            const waitFlow = new FixtureInsightsFlowBuilder()
                .withWorkflow({
                    actions: {
            ...
            waitInvocation = createExampleInsightsFlowInvocation(waitFlow)
