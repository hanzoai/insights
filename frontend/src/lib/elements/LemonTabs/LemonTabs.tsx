...
export function Tabs<T extends string | number>({
    ...
    const realTabs = tabs.filter(Boolean) as Tab<T>[]
    ...
    return (
        <div
            ...
            data-attr={dataAttr}
        >
            <ul className={cn('Tabs__bar', barClassName)} role="tablist" ref={containerRef}>
                <div
                    ...
                    {realTabs.map((tab) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    {tab.link ? (
                                        <Link className="Tabs__tab-content" to={tab.link}>
                                            {content}
                                        </Link>
                                    ) : (
                                        <div className="Tabs__tab-content">{content}</div>
                                    )}
                                </li>
                            </Tooltip>
                        )
            ...
            {activeTab && 'content' in activeTab && (
                <div className={cn('Tabs__content', sceneInset && 'p-4')} key={activeKey}>
                    {activeTab.content}
                </div>
            )}
        </div>
