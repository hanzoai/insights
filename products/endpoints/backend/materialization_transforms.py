...
from insights.insightsql.functions.aggregations import COMBINATORS
from insights.insightsql.functions.mapping import find_insightsql_aggregation
from insights.insightsql.modifiers import create_default_modifiers_for_team
from insights.insightsql.parser import parse_select
from insights.insightsql.placeholders import find_placeholders
...
def _print_materialized_insightsql(
    query: ast.Expr,
    team: Team,
    ...
    *,
    ...
    """Like to_printed_insightsql, but without the implicit top-level row cap — a materialized table must hold every row."""
...
def convert_insight_query_to_insightsql(
    query: dict[str, Any],
    team: Team,
    *,
    user: User | None = None,
    ...
    insightsql_string = _print_materialized_insightsql(
        combined_query_ast,
        team,
...
def _strip_combinators(func_name: str) -> str | None:
    """Strip Datastore combinator suffixes to find the base aggregate function.

    Uses the COMBINATORS registry from insights.insightsql.functions.aggregations.
...
def get_reaggregation(func_name: str) -> AggregateReaggregation | None:
    ...
    Handles Datastore combinators by stripping suffixes to find the base function.
...
def _is_aggregate(func_name: str) -> bool:
    ...
    ``find_insightsql_aggregation`` only covers names in the registry; combinator-derived names
    (``sumIf``, ``maxIf``, ``countArrayIf``, …) live outside it and need ``_strip_combinators``.
    """
    return find_insightsql_aggregation(func_name) is not None or _strip_combinators(func_name) is not None
...
def analyze_variables_for_materialization(
    ...
    if has_cte_vars and isinstance(ast_node, ast.SelectQuery) and ast_node.ctes:
        ...
        for var in result_vars:
            ...
            # returning N rows × 2 cols). No correct rewrite exists — Datastore doesn't
...
def _select_alias_conflict(select: ast.SelectQuery, vars_in_context: list[MaterializableVariable]) -> Optional[str]:
    ...
    existing_aliases = {
        alias.alias: alias.expr.to_insightsql() for alias in select.select or [] if isinstance(alias, ast.Alias)
    }
    for var in vars_in_context:
        existing = existing_aliases.get(var.code_name)
        if existing is not None and existing != _variable_column_expr(var).to_insightsql():
...
def transform_select_for_materialized_table(select_exprs: list[ast.Expr], team: Team) -> list[MaterializedColumn]:
    ...
    for expr in select_exprs:
        ...
        if isinstance(expr, ast.Alias):
            ...
            field = ast.Field(chain=[expr.to_insightsql()])
...
def transform_query_for_materialization(
    ...
    *,
    ...
    transformed_query_str = _print_materialized_insightsql(
        transformed_ast,
        team,
        user=user,
        bypass_warehouse_access_control=bypass_warehouse_access_control,
...
def replace_breakdown_sentinels_in_query(insightsql_query: dict) -> dict:
    """Strip internal sentinel string literals from InsightsQL so S3-stored data is clean. The 'other' sentinel
    is embedded in the InsightsQL by the query builder (in if() and ORDER BY expressions) regardless of
    breakdown_hide_other_aggregation, which only affects post-processing."""
    query_text = insightsql_query.get("query")
    ...
    return {**insightsql_query, "query": query_text}


def build_endpoint_insightsql(
    insight_query: dict,
    team: Team,
    bucket_overrides: dict[str, str] | None = None,
    *,
    ...
    """Run the full endpoint conversion pipeline: prepare → convert insight to InsightsQL → apply variable
    ...
    insightsql_query = convert_insight_query_to_insightsql(
        mat_query,
        team,
        user=user,
        bypass_warehouse_access_control=bypass_warehouse_access_control,
    ...
    if insight_query.get("variables"):
        ...
        if can_materialize and variable_infos:
            insightsql_query = transform_query_for_materialization(
                insightsql_query,
                variable_infos,
                team,
    ...
    return replace_breakdown_sentinels_in_query(insightsql_query)
