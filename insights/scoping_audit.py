...
Lives at the top of the `insights` package (not under `insights.models`)
because Celery task modules import this at module load — and importing
from `insights.models.*` at that point triggers `insights.models.__init__`
to load every model in the codebase, which fails before Django's app
registry is ready (e.g. during `insights.apps.py` initialization for
modules pulled in by the celery app config).

The framework's other scoping helpers (TeamScopedManager,
ProductTeamModel, team_scope, etc.) live in `insights.models.scoping`
