...
from insights.temporal.ai_observability.evaluation_llm_judge import DEFAULT_JUDGE_MODEL
from insights.temporal.ai_observability.evaluation_types import EvaluationActivityResult
from insights.temporal.ai_observability.metrics import increment_emit_event_outcome
...
_STATUS_REASON_SUBJECTS = {
    ...
    "script_error": "Your AI observability evaluation was disabled because its Script code failed",
}
...
def build_evaluation_event_properties(
    ...
    if evaluation_type == "llm_judge" and not result.get("skipped"):
        ...
        properties["$ai_evaluation_key_type"] = "byok" if result.get("is_byok") else "insights"
...
async def emit_internal_telemetry_activity(inputs: EmitInternalTelemetryInputs) -> None:
    ...
    from insights.tasks.usage_report import get_ph_client
