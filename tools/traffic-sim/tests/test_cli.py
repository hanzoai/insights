...
class TestExtractInsightsEventsFromConsole:
    @pytest.mark.parametrize(
        "lines,expected",
        [
            ([], []),
            (
                [cli.ConsoleLine(timestamp="t", type="log", text='[Insights.js] send "$pageview"', is_insights=True)],
                ["$pageview"],
            ),
            (
                [
                    cli.ConsoleLine(timestamp="t", type="log", text='[Insights.js] send "$pageview"', is_insights=True),
                    cli.ConsoleLine(
                        timestamp="t", type="log", text='[Insights.js] send "$autocapture"', is_insights=True
                    ),
                ],
            ...
            (
                ...
                [
                    cli.ConsoleLine(
                        timestamp="t", type="log", text='[Insights.js] send "signup_completed"', is_insights=True
                    )
                ],
                ["signup_completed"],
            ),
            (
                # Non-insights console lines are ignored even if they look similar.
                [cli.ConsoleLine(timestamp="t", type="log", text='[Insights.js] send "$pageview"', is_insights=False)],
                [],
            ),
        ],
    )
    def test_extracts_event_names(self, lines, expected):
        assert cli.extract_insights_events_from_console(lines) == expected
...
class TestAddTrackingParams:
    def test_adds_required_params(self):
        ...
        assert params["__insights_debug"] == ["true"]
...
class TestBuildParser:
    ...
    def test_custom_insights_host(self):
        ...
        assert args.insights_host == "https://eu.i.hanzo.ai"
...
class TestAnalyticsCapture:
    def test_is_insights_matches_default_domains(self):
        ...
        assert not cap._is_insights("https://example.com/api")

    def test_is_insights_matches_custom_domain(self):
        cap = cli.AnalyticsCapture(insights_domains=("hanzo.ai", "ph.example.com"))
        assert cap._is_insights("https://ph.example.com/i/v0/e/")
        assert cap._is_insights("https://us.i.hanzo.ai/capture/")
        assert not cap._is_insights("https://other.example.com/api")

    @pytest.mark.parametrize(
        "url",
        [
            # Hostname containing hanzo.ai as a path/query component is not a match.
            "https://evil.com/hanzo.ai/capture",
            "https://evil.com/?proxy=us.i.hanzo.ai",
            # Hostname that merely ends with hanzo.ai without a leading dot is not a match.
            "https://nothanzo.ai/capture",
            # Subdomain attack: hanzo.ai placed before the registered domain.
            "https://hanzo.ai.evil.com/capture",
        ],
    )
