...
export class FlowFunctionsService {
    ...
    async buildInsightsFunctionInvocation(
        ...
        const teamId = invocation.flow.team_id
        ...
        const globalsWithSource: InsightsFunctionInvocationGlobals = {
            ...
            source: {
                name: insightsFunction.name ?? `Script flow: ${invocation.flow.id}`,
                url: `${projectUrl}/workflows/${invocation.flow.id}/workflow?node=${insightsFunction.id}`,
            },
            project: {
