...
import {
    ...
    CustomFunctionConfigurationContextId,
    CustomFunctionConfigurationType,
    CustomFunctionMappingType,
...
import type { customFunctionConfigurationLogicType } from './customFunctionConfigurationLogicType'

export interface CustomFunctionConfigurationLogicProps {
    logicKey?: string
...
const VALIDATION_RULES = {
    SITE_DESTINATION_REQUIRES_MAPPINGS: (data: CustomFunctionConfigurationType) =>
        data.type === 'site_destination' && (!data.mappings || data.mappings.length === 0)
            ? 'You must add at least one mapping'
            : undefined,
    INTERNAL_DESTINATION_REQUIRES_FILTERS: (data: CustomFunctionConfigurationType) =>
        data.type === 'internal_destination' && data.filters?.events?.length === 0
            ? 'You must choose a filter'
            : undefined,
} as const

const NEW_FUNCTION_TEMPLATE: CustomFunctionTemplateType = {
    id: 'new',
    free: false,
    type: 'destination',
    name: '',
...
export const TYPES_WITH_GLOBALS: CustomFunctionTypeType[] = ['transformation', 'destination']
export const TYPES_WITH_REAL_EVENTS: CustomFunctionTypeType[] = ['destination', 'site_destination', 'transformation']
export const TYPES_WITH_VOLUME_WARNING: CustomFunctionTypeType[] = ['destination', 'site_destination']

const TYPE_TO_PRODUCT_KEY: Partial<Record<CustomFunctionTypeType, ProductKey>> = {
...
export function sanitizeInputs(
    data: Pick<CustomFunctionMappingType, 'inputs_schema' | 'inputs'>
...
export function sanitizeConfiguration(data: CustomFunctionConfigurationType): CustomFunctionConfigurationType {
    ...
    const payload: CustomFunctionConfigurationType = {
        ...data,
...
export const templateToConfiguration = (template: CustomFunctionTemplateType): CustomFunctionConfigurationType => {
    ...
    let mappings: CustomFunctionMappingType[] | undefined
...
export const customFunctionConfigurationLogic = kea<customFunctionConfigurationLogicType>([
    path((id) => ['scenes', 'pipeline', 'customFunctionConfigurationLogic', id]),
    props({} as CustomFunctionConfigurationLogicProps),
    key(({ id, templateId, subTemplateId, logicKey }: CustomFunctionConfigurationLogicProps) => {
    ...
    actions({
        ...
        resetForm: true,
        upsertCustomFunction: (configuration: CustomFunctionConfigurationType) => ({ configuration }),
        duplicate: true,
        duplicateFromTemplate: true,
        resetToTemplate: true,
        deleteCustomFunction: true,
        ...
        setUnsavedConfiguration: (configuration: CustomFunctionConfigurationType | null) => ({ configuration }),
        ...
        reportAICustomFunctionRejected: true,
        reportAICustomFunctionPromptOpen: true,
        ...
        reportAICustomFunctionInputsPrompted: true,
        reportAICustomFunctionInputsAccepted: true,
        reportAICustomFunctionInputsRejected: true,
        reportAICustomFunctionInputsPromptOpen: true,
    }),
    reducers(({ props }) => ({
        sampleGlobals: [
        ...
        hasHadSubmissionErrors: [
            false,
            {
                upsertCustomFunctionFailure: () => true,
            },
        ],

        unsavedConfiguration: [
            null as { timestamp: number; configuration: CustomFunctionConfigurationType } | null,
            { persist: true },
            {
    ...
    loaders(({ actions, props, values, cache }) => ({
        template: [
            null as CustomFunctionTemplateType | null,
            {
                loadTemplate: async () => {
                    ...
                    const res = await api.customFunctions.getTemplate(props.templateId)
        ...
        customFunction: [
            ...
            {
                loadCustomFunction: async () => {
                    if (!props.id || props.id === 'new') {
                        return null
                    }

                    return await api.customFunctions.get(props.id)
                },

                upsertCustomFunction: async ({ configuration }) => {
                    const isNew = !props.id || props.id === 'new'
                    const res = isNew
                        ? await api.customFunctions.create(configuration)
                        : await api.customFunctions.update(props.id!, configuration)
                    ...
                    refreshTreeItem('custom_function/', res.id)
            ...
            null as CyclotronJobInvocationGlobals | null,
            {
                loadSampleGlobals: async ({ eventId }, breakpoint) => {
                    ...
                    try {
                        ...
                        const globals = convertToCustomFunctionInvocationGlobals(event, person)
                        globals.groups = {}
    ...
    forms(({ values, props, asyncActions }) => ({
        configuration: {
            defaults: {} as CustomFunctionConfigurationType,
            ...
            errors: (data) => {
                return {
                    ...
                    filters: VALIDATION_RULES.INTERNAL_DESTINATION_REQUIRES_FILTERS(data) as unknown as DeepPartialMap<
                        CustomFunctionConfigurationType['filters'],
                        ValidationErrorType
                    >,
                    inputs: (values.inputFormErrors ?? {}) as DeepPartialMap<
                        CustomFunctionConfigurationType['inputs'],
                        ValidationErrorType
                    >,
                }
            },
            submit: async (data) => {
                ...
                // Only sent on create
                payload.template_id = props.templateId || values.customFunction?.template?.id
                ...
                await asyncActions.upsertCustomFunction(payload as CustomFunctionConfigurationType)
            },
        },
    })),
    selectors(() => ({
        logicProps: [() => [(_, props) => props], (props: CustomFunctionConfigurationLogicProps) => props],
        type: [
            (s) => [s.configuration, s.customFunction],
            (configuration, customFunction) => configuration?.type ?? customFunction?.type ?? 'loading',
        ],
        hasGroupsAddon: [
        ...
        teamHasCohortFilters: [
            ...
            (currentTeam: TeamType | null, configuration: CustomFunctionConfigurationType | null) => {
        ...
        useMapping: [
            (s) => [s.customFunction, s.template],
            // If the function has mappings, or the template has mapping templates, we use mappings
            (customFunction, template) => Array.isArray(customFunction?.mappings) || template?.mapping_templates?.length,
        ],
        defaultFormState: [
            (s) => [s.template, s.customFunction],
            (template, customFunction): CustomFunctionConfigurationType | null => {
                if (template) {
        ...
        templateId: [
            (s) => [s.template, s.customFunction],
            (template, customFunction) => template?.id || customFunction?.template?.id,
        ],

        loading: [
            (s) => [s.customFunctionLoading, s.templateLoading],
            (customFunctionLoading, templateLoading) => customFunctionLoading || templateLoading,
        ],
        loaded: [(s) => [s.customFunction, s.template], (customFunction, template) => !!customFunction || !!template],

        contextId: [
            (s) => [s.configuration],
            (configuration): CustomFunctionConfigurationContextId => {
                return eventToCustomFunctionContextId(configuration.filters?.events?.[0]?.id)
            },
        ],
        ...
        willReEnableOnSave: [
            (s) => [s.configuration, s.customFunction],
            (configuration, customFunction) => {
                const hogState = customFunction?.status?.state ?? 0
        ...
        willChangeEnabledOnSave: [
            (s) => [s.configuration, s.customFunction],
            (configuration, customFunction) => {
                return configuration?.enabled !== (customFunction?.enabled ?? false)
            },
        ],
        ...
        mappingTemplates: [
            (s) => [s.customFunction, s.template],
            (customFunction, template) => template?.mapping_templates ?? customFunction?.template?.mapping_templates ?? [],
        ],
        ...
        currentInputs: [
            ...
            (newInputs: CyclotronJobInputSchemaType[] | null, configuration: CustomFunctionConfigurationType) => {
        ...
        canEditSource: [
            (s) => [s.type, s.template, s.customFunction],
            (type, template, customFunction) => {
                const codeLanguage = template?.code_language || customFunction?.template?.code_language
        ...
        isLegacyPlugin: [
            ...
            (template, customFunction) => {
                return (template?.id || customFunction?.template?.id)?.startsWith('plugin-')
            },
        ],
    })),

    listeners(({ actions, values, cache }) => ({
        reportAICustomFunctionPrompted: () => {
            posthog.capture('ai_custom_function_prompted', { type: values.type })
        },
        reportAICustomFunctionAccepted: () => {
            posthog.capture('ai_custom_function_accepted', { type: values.type })
        },
        reportAICustomFunctionRejected: () => {
            posthog.capture('ai_custom_function_rejected', { type: values.type })
        },
        reportAICustomFunctionPromptOpen: () => {
            posthog.capture('ai_custom_function_prompt_open', { type: values.type })
        },
        reportAIFiltersPrompted: () => {
            posthog.capture('ai_custom_function_filters_prompted', { type: values.type })
        },
        reportAIFiltersAccepted: () => {
            posthog.capture('ai_custom_function_filters_accepted', { type: values.type })
        },
        reportAIFiltersRejected: () => {
            posthog.capture('ai_custom_function_filters_rejected', { type: values.type })
        },
        reportAIFiltersPromptOpen: () => {
            posthog.capture('ai_custom_function_filters_prompt_open', { type: values.type })
        },
        reportAICustomFunctionInputsPrompted: () => {
            posthog.capture('ai_custom_function_inputs_prompted', { type: values.type })
        },
        reportAICustomFunctionInputsAccepted: () => {
            posthog.capture('ai_custom_function_inputs_accepted', { type: values.type })
        },
        reportAICustomFunctionInputsRejected: () => {
            posthog.capture('ai_custom_function_inputs_rejected', { type: values.type })
        },
        reportAICustomFunctionInputsPromptOpen: () => {
            posthog.capture('ai_custom_function_inputs_prompt_open', { type: values.type })
        },
        loadTemplateSuccess: () => actions.resetForm(),
        loadCustomFunctionSuccess: () => {
            actions.resetForm()
        },
        upsertCustomFunctionSuccess: () => {
            actions.resetForm()
        },

        upsertCustomFunctionFailure: ({ errorObject }) => {
        ...
        resetForm: () => {
            ...
            const config: CustomFunctionConfigurationType = {
                ...baseConfig,
                ...cache.configFromUrl,
            }
        ...
        duplicate: async () => {
            if (values.customFunction) {
                const newConfig = {
                    ...values.configuration,
                    name: `${values.configuration.name} (copy)`,
                }
                // TODO: What to do if no template?
                const originalTemplate = values.customFunction.template!
                router.actions.push(urls.customFunctionNew(originalTemplate.id), undefined, {
                    configuration: newConfig,
                })
            }
        },
        duplicateFromTemplate: async () => {
            if (values.customFunction?.template) {
                const newConfig: CustomFunctionTemplateType = {
                    ...values.customFunction.template,
                }
                router.actions.push(urls.customFunctionNew(values.customFunction.template.id), undefined, {
                    configuration: newConfig,
                })
            }
        },
        resetToTemplate: async () => {
            const template = values.customFunction?.template ?? values.template
            if (template) {
        ...
        deleteCustomFunction: async () => {
            const customFunction = values.customFunction
            if (!customFunction) {
                return
            }
    ...
    subscriptions(({ props, actions, cache }) => ({
        customFunction: (customFunction) => {
            if (customFunction && props.templateId) {
                ...
                router.actions.replace(urls.customFunction(customFunction.id), router.values.searchParams)
            }
        },
    ...
    urlToAction(({ actions, values, cache }) => ({
        [urls.customFunctionNew(':templateId')]: (_, __, hashParams) => {
