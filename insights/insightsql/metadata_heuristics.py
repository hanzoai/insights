...
class MetadataHeuristic:
    def run(self, query: ast.SelectQuery | ast.SelectSetQuery) -> list[InsightsQLNotice]:
...
def run_metadata_heuristics(query: ast.SelectQuery | ast.SelectSetQuery) -> list[InsightsQLNotice]:
...
def _fingerprint_select_query(query: ast.SelectQuery) -> SubqueryFingerprint:
    ...
    where = query.where.to_insightsql().strip().lower() if query.where else None
