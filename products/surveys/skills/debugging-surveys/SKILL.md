...
description: >-
  Debug, support, and build Insights Surveys across the backend and all five SDKs
  (web/insights-js, iOS, Android, Flutter, React Native). Use whenever a Surveys
...
Insights Surveys is a no-code in-app form builder. A customer creates a survey in the
Insights UI; it must then be evaluated and rendered by whichever SDK their app runs.
...
- **Android ships no survey UI.** The app (or the Flutter plugin) must provide a `InsightsSurveysDelegate`. "Survey never renders on Android" is often a missing delegate, not a Insights bug.
...
- **Behavioral cohort in a realtime flag.** A cohort with `performed_event`/behavioral filters can't be evaluated in realtime flag bytecode (`"Unsupported behavioral filter for realtime bytecode"`, `insights/api/cohort.py`). The cohort shows a `bytecode_error`. Surveys/flags can't use it directly — the customer must make a _static_ copy of the cohort and target that.
...
Read-only InsightsQL templates for the disambiguators and confirmations above live in
[references/diagnostic-queries.md](references/diagnostic-queries.md). Run them via the
Insights MCP `execute-sql` against the customer's project.
...
1. **Insights MCP tools** (`survey`, `feature-flag`, `cohorts`, `execute-sql`, `activity-log`, `persons`) against the customer's project. This is read-only by default and the safest way to inspect config and run queries — no impersonation, no write risk. Use this first.
...
Voice derived from the Insights handbook support values (reassuringly human, humble, ship
...
- **Link every Insights entity** by ID. Cohorts: `https://<us|eu>.hanzo.ai/project/<id>/cohorts/<cohort_id>`. Flags: `.../feature_flags/<flag_id>`. Surveys: `.../surveys/<survey_id>`. Match the customer's instance (US vs EU).
...
- **Do NOT offer to "hop on a call" or book a meeting.** Insights support is async-first. Close with "We're always here if you need a follow-up."
...
3. Implement in the SDK repos (`insights-js` covers both web and React Native), then
   `insights-ios`, `insights-android`, and the Flutter Dart layer. Remember Flutter's split:
...
4. Update the `hanzo.ai` docs (`contents/docs/surveys/`) and this parity table.
