...
export function initLiquidLanguage(monaco: Monaco): void {
     ...
     * languages, so we'll do so with a noop iqlLiquid language registration as a flag like we do with other languages
     * in this directory
     */
    if (!monaco.languages.getLanguages().some((lang) => lang.id === 'iqlLiquid')) {
        monaco.languages.register({ id: 'iqlLiquid' })
