...
use crate::scriptvm::{build_behavioral_globals, build_person_property_globals, CohortEvaluator};
...
use crate::stage1::time::datastore_timestamp_to_millis;
...
// The sync composition runs in blocking contexts, so its direct `read_event_snapshot` + `apply` are
// sanctioned; production folds through `process_event_offloaded`, which the crate-wide lint keeps
// free of raw `CohortStore` calls.
...
pub(crate) fn plan_event(
    ...
    let Some(event_ms) = datastore_timestamp_to_millis(&event.timestamp) else {
...
/// - `Stale`: the record advances its dedup + `last_seen` only; no ScriptVM, no transitions.
/// - `SkipEval`: the record adopts the event stamp; no ScriptVM, no transitions.
