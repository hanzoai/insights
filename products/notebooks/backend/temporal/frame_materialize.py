...
inputs): the activity prints the InsightsQL through the guarded executor (team database +
access controls), executes it over the Datastore HTTP interface with `FORMAT ArrowStream`,
...
Datastore overload raise retryable errors, and Temporal's retry policy provides the
exponential backoff with a hard schedule-to-close deadline. Datastore `priority` is
...
from insights.schema import QueryStatus
...
from insights.datastore.client.execute_async import QueryNotFoundError, QueryStatusManager, get_query_status
from insights.datastore.client.limit import ConcurrencyLimitExceeded, RateLimit
...
from insights.temporal.common.datastore import (
    ...
    DatastoreTooManyBytesError,
    DatastoreTooManyRowsOrBytesError,
)
...
# raised to INSIGHTSQL_INCREASED_MAX_EXECUTION_TIME by the NOTEBOOK_MATERIALIZE limit context.
...
# Client-side timeouts on the Datastore stream. Temporal cannot interrupt a sync activity
...
# only when the writer finalizes cleanly. Datastore streams `200 OK` before execution
...
# Datastore exception codes worth a specific user-facing message when a query dies
...
FRAME_MATERIALIZE_SECONDS_HISTOGRAM = Histogram(
    "insights_notebooks_frame_materialize_seconds",
    "End-to-end wall-clock duration of a successful materialize (print + Datastore + upload).",
    buckets=[1, 5, 15, 30, 60, 120, 300, 600],
)
# The relay is a single-pass pipeline, so these two are complementary slices of its wall
# clock: which side the thread was blocked on tells us whether Datastore production or
# object-store ingestion is the bottleneck.
FRAME_DATASTORE_SECONDS_HISTOGRAM = Histogram(
    "insights_notebooks_frame_datastore_seconds",
    "Time a successful materialize spent blocked on Datastore (response headers plus body reads).",
    buckets=[0.5, 1, 5, 15, 30, 60, 120, 300, 600],
)
FRAME_UPLOAD_SECONDS_HISTOGRAM = Histogram(
    "insights_notebooks_frame_upload_seconds",
...
class FrameMaterializeInputs:
    ...
    user_id: int | None
    # The wrapped InsightsQL (outer LIMIT/OFFSET applied by the data plane). Only ever executed
    # after printing through the guarded InsightsQL executor — never handed to Datastore raw.
    query: str
    query_hash: str
    cache_key: str
...
def _generate_sql(
    ...
    *,
    ...
    """Print InsightsQL to guarded Datastore SQL with the standing caps applied."""
    executor = InsightsQLQueryExecutor(
        ...
        settings=InsightsQLGlobalSettings(max_bytes_to_read=_MAX_BYTES_TO_READ, max_threads=_MAX_THREADS),
        pretty=False,
    )
    if output_format:
        executor.context.output_format = output_format
    sql, context = executor.generate_datastore_sql()
    return sql, context.values


def _describe_columns(
    client: DatastoreClient, printed_sql: str, values: dict[str, object], ch_query_id: str
) -> list[tuple[str, str]]:
    """Return the printed query's output columns as (name, Datastore type) pairs."""
...
def _stringify_function_for(ch_type: str) -> str | None:
    """The conversion a column needs so Datastore's Arrow output stays kernel-friendly.
...
def _print_datastore_sql(
    client: DatastoreClient, team: Team, user: User | None, query: str, ch_query_id: str
) -> tuple[str, dict[str, object]]:
    """Print the InsightsQL to guarded Datastore SQL with `FORMAT ArrowStream` and the standing caps.

    Two passes when needed (the data-modeling recipe): print once, DESCRIBE the printed
    query (metadata only — no execution), and if any output column would leave Datastore
    as Arrow binary, wrap the query so those columns are stringified, then print the
    wrapper. Field names go through the InsightsQL AST/printer, never string splicing.
    """
...
class MidStreamQueryError(Exception):
    """The Datastore response body ended without the Arrow end-of-stream marker."""
...
class _ArrowTailReader:
    ...
    time spent blocked in reads, so the Datastore and upload halves of the relay's wall
    ...
    for lookup_attempt in range(_QUERY_LOG_LOOKUP_ATTEMPTS):
        ...
        try:
            rows = sync_execute(
                ...
                LIMIT 1
                """,
                {"cluster": settings.DATASTORE_CLUSTER, "query_id": ch_query_id},
            )
        except Exception:
            return None
        if rows:
...
def materialize_frame(inputs: FrameMaterializeInputs) -> str:
    """Stream the frame's Datastore result into the object store; return the object key.

    Raises on failure so Temporal retries per policy; user-safe InsightsQL errors are written
    ...
    with tags_context(
        ...
        # Datastore may still be draining, and the failure path looks the id up in
        ...
        try:
            with _materialize_slots(inputs.team_id, inputs.query_id):
                # Everything that touches Datastore or Postgres lives inside the slots —
                ...
                printed_sql, context_values = _print_datastore_sql(client, team, user, inputs.query, ch_query_id)
                ...
                with client.post_query(
                    ...
                    if relay.tail != _ARROW_STREAM_EOS_MARKER:
                        # Datastore failed mid-stream but closed the body cleanly (or an
                        ...
                        raise MidStreamQueryError("Datastore stream ended without the Arrow end-of-stream marker")
        except ConcurrencyLimitExceeded:
            raise  # retryable — Temporal backs off and re-attempts
        except ExposedInsightsQLError as exc:
            ...
            # Deterministic budget failures Datastore rejects before streaming.
            # Re-executing the same heavy query just burns Datastore and ends on the same
            # wall: terminal, with a user-facing message.
            message = (
                _RESULT_SIZE_MESSAGE if isinstance(exc, DatastoreTooManyRowsOrBytesError) else _RESOURCE_BUDGET_MESSAGE
            )
            ...
            # The stream failed after Datastore already sent its 200 — either the chunked
            ...
            # The raw Datastore message may embed query fragments — log it, don't expose it.
            logger.warning(
    ...
    # Datastore time = waiting for response headers plus every blocking body read; upload
    # time = the rest of the relay's wall clock (part handoff and S3 backpressure).
    datastore_seconds = (headers_received - query_started) + relay.read_seconds
    ...
    FRAME_DATASTORE_SECONDS_HISTOGRAM.observe(datastore_seconds)
    ...
    logger.info(
        ...
        datastore_seconds=round(datastore_seconds, 3),
...
class NotebookFrameMaterializeWorkflow(InsightsWorkflow):
    ...
    async def run(self, input: FrameMaterializeInputs) -> None:
        try:
            await workflow.execute_activity(
                ...
                # the job fails with a clear error instead of piling onto Datastore.
