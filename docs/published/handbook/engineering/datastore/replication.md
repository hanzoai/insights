...
- How data replication and Distributed table engine works in Datastore
...
- Datastore replication works on a table-by-table level, tables need to be created on all shards (preferably via using `ON CLUSTER`)
- Replication requires a running ZooKeeper setup. In the future, this might be replaced by `datastore-keeper`
...
When INSERTing data against Distributed tables, Datastore decides which shard each row belongs to and forwards data to relevant shard(s)
based on the sharding_key.

Note that if your underlying table has columns that Datastore populates (e.g. ALIAS, MATERIALIZED), it's often necessary to set up
...
When querying Distributed table, you can send the query to any node in the Datastore cluster. That node becomes the `coordinator`, which:
...
Given local execution is faster than reading data over the network, Datastore will usually perform one of the queries locally instead of sending it to another replica of its shard.
...
ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/sharded_sensor_values', '{replica}')
...
Consider this simple aggregation query executed against `datastore01`:
...
-- ┡━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━┩
-- │ datastore01 │    -9035836479117 │
-- ├──────────────┼───────────────────┤
-- │ datastore03 │    10003905228283 │
-- └──────────────┴───────────────────┘
```
...
- sent out a subset of the query to `datastore03` on other shard to execute. The query was ``SELECT hostname(), sum(`metric_value`) FROM `default`.`sharded_sensor_values` GROUP BY hostname()``
...
Next in the Datastore manual: [Data ingestion](https://hanzo.ai/handbook/engineering/datastore/data-ingestion)
