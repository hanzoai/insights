...
# ENETUNREACH / EHOSTUNREACH at connect time: the host resolved to a public address Insights can't
# route to. The common cause is a host that only accepts IPv6 (Insights egresses over IPv4) — for
# example a Supabase direct-connection host — or a firewall dropping Insights's IPs. Deterministic
# for the configured host, so give an actionable message instead of the generic connect fallback.
_HOST_UNREACHABLE_ERROR = (
    "Insights reached the network but couldn't open a connection to the database host. This usually "
    "means the host only accepts IPv6 connections (Insights connects over IPv4), or a firewall is "
    "blocking Insights's IP addresses. Use a host that's reachable over IPv4 (for example a "
    "connection pooler), enable your provider's IPv4 add-on, or add Insights's IP addresses to your "
...
PostgresErrors = {
    ...
    # A public host Insights resolved but can't route to (IPv6-only host, or a firewall dropping our
    ...
    "received invalid response to SSL negotiation": "Insights reached the host and port you configured, but the server didn't respond like a PostgreSQL server speaking SSL. Check that the host and port point at your PostgreSQL server (not an HTTP, proxy, or edge endpoint) and that the database is running.",
    ...
    # until one frees up — a config/capacity condition on the customer's pooler, not a Insights bug.
...
RLS_WARNING_MESSAGE = (
    "Row-level security is active on this table for the sync role, so Insights can only read "
...
_FOREIGN_SERVER_UNREACHABLE_ERROR = (
    "A table selected for sync is a postgres_fdw foreign table and Insights could not connect to the "
...
# down, or its firewall blocks Insights's IPs. The raw message tells the user nothing actionable, so
...
_SSH_GATEWAY_UNREACHABLE_MESSAGE = (
    "Could not connect to your SSH tunnel — Insights couldn't open a session to the SSH gateway. "
    "Check that the SSH host and port point to a reachable SSH server (not the database port), that "
    "the bastion is running, and that Insights's IP addresses are allowed through its firewall."
)
...
class PostgresSource(SQLSource[PostgresSourceConfig], SSHTunnelMixin, ValidateDatabaseHostMixin):
            ...
            fields=cast(
                ...
                [
                    ...
                    SourceFieldInputConfig(
                        ...
                        caption=(
                            "Must be reachable from the public internet. Add Insights's egress IP addresses to your "
    ...
    def get_non_retryable_errors(self) -> dict[str, str | None]:
        return {
            ...
            "incremental_field and incremental_field_type can't be None": (
                ...
                "Insights can't build its sync query. Choose an incremental field for the table in its "
            ...
            # A Neon-style proxy rejects the connection because Insights's egress IP isn't on the
            ...
            "address not in tenant allow_list": (
                "Your database provider rejected the connection because Insights's IP address is not on "
                'its configured allow list ("address not in tenant allow_list"). Add Insights\'s egress IP '
            ...
            "permission denied for function": (
                "Insights's database role isn't allowed to execute a function used by one or more of the "
            ...
            "permission denied for schema": (
                "Insights's database role isn't allowed to access a schema that contains one or more of the "
            ...
            "permission denied for": (
                "Insights's database role isn't allowed to read one or more of the tables you selected to sync "
            ...
            "no application protocol": (
                "Insights couldn't complete a TLS handshake with the host and port you configured — "
            ...
            "received invalid response to SSL negotiation": (
                "Insights reached the host and port you configured, but the server didn't respond "
            ...
            # refusing Insights's IPs, a proxy resetting the stream). sshtunnel doesn't wrap it, so
            ...
            _SSH_HANDSHAKE_EOF_ERROR: (
                ...
                "that Insights's IP addresses are allowed through its firewall, then re-enable the sync."
            ),
            ...
            "exceeded the compute time quota": (
                ...
                'exceeded its compute-time quota ("exceeded the compute time quota"). Insights can\'t '
            ...
            "cluster is read-only": (
                ...
                'rejecting Insights\'s queries ("cluster is read-only"). Providers such as PlanetScale '
            ...
            "Hot standby mode is disabled": (
                "Insights connected to a PostgreSQL standby (read replica) that isn't accepting "
            ...
            "kept canceling reads due to conflict with recovery": (
                "Insights repeatedly hit Postgres recovery conflicts while reading from your read replica "
            ...
            "out of memory": (
                "Your database server ran out of memory while Insights was reading from it "
            ...
            "invalid input syntax for type integer": "Insights tried to resume this table's incremental sync from a non-integer cursor value against an integer incremental field, which your database rejects. This usually means the incremental field's type doesn't match its data. Please reset and fully re-sync this table, or pick a different incremental field.",
            "invalid input syntax for type bigint": "Insights tried to resume this table's incremental sync from a non-integer cursor value against a bigint incremental field, which your database rejects. This usually means the incremental field's type doesn't match its data. Please reset and fully re-sync this table, or pick a different incremental field.",
            ...
            "in a read-only transaction": (
                ...
                "Insights is reading it (for example a view or trigger that refreshes a materialized "
                "view), and Insights reads inside a read-only transaction so the write is rejected "
                '("cannot execute ... in a read-only transaction"). Insights never writes to your '
            ...
            "user mapping not found for": (
                ...
                "Insights's database role has no user mapping for the foreign server it points at "
    ...
    def cleanup_cdc_resources_on_deletion(self, source: "ExternalDataSource") -> None:
        """Drop the Temporal schedule + Insights-managed slot/publication.
        ...
        if cdc_config.management_mode != "insights":
            return
    ...
    def source_for_pipeline(self, config: PostgresSourceConfig, inputs: SourceInputs) -> SourceResponse:
        ...
        from products.warehouse_sources.backend.temporal.data_imports.sources.postgres.exceptions import (
            ...
            InsightsDatabaseConnectionError,
        )

        ssh_tunnel = self.make_ssh_tunnel_func(config, inputs.team_id)

        # This reads sync metadata from Insights's own database, not the customer's Postgres. A
        ...
        try:
            ...
            raise InsightsDatabaseConnectionError("Failed to load sync metadata from Insights's database") from e
        ...
        # storage key when present, otherwise the row name) so InsightsQL reads from where we wrote.
