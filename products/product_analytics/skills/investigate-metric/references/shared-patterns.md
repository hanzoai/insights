...
- When to reach for InsightsQL
- InsightsQL insights
- InsightsQL quantile template
- Property discovery
...
Use the typed query tools by default. Use `insights:execute-sql` only when the question
...
For saved insights with `query.kind === "InsightsQLQuery"`, the playbook routing is shape-based
...
Run the insight's SQL through `insights:execute-sql` to get the data, then follow the
...
Use `insights:read-data-schema` to discover events and properties before filtering or
...
`insights:query-trends-actors` only accepts a trends source. The selector fields are `day`,
`series`, and (if breakdown) `breakdown`:

```json
insights:query-trends-actors
{
...
`insights:query-session-recordings-list`. Watching three or four is often faster than
running more queries. Fetch individual ones with `insights:session-recording-get`.

## Error / logs cross-check

Run `insights:query-error-tracking-issues-list` and `insights:query-logs` for the anomaly window.
