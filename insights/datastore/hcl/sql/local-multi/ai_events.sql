...
-- Full CREATE schema for the local-multi/ai_events node. Apply to a fresh Datastore to build it.

CREATE TABLE insights.kafka_ai_events_json (
  uuid UUID,
  event String,
  properties String,
...
) ENGINE = Kafka() SETTINGS kafka_broker_list = 'msk_cluster', kafka_format = 'kafka_format = \'JSONEachRow\'', kafka_group_name = 'kafka_group_name = \'group1\'', kafka_topic_list = 'kafka_topic_list = \'datastore_ai_events_json\'';
...
CREATE TABLE insights.sharded_ai_events (
  uuid UUID,
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/insights.ai_events', '{replica}') ORDER BY (team_id, trace_id, timestamp) PARTITION BY toYYYYMM(drop_date) TTL drop_date SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
