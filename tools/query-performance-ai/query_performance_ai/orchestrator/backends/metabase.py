"""Metabase-backed execution: shell out to `insightscli metabase:query`.

We reuse `insightscli` for the same reason `slow_queries.py` does: cookie
...
# Datastore buffers query_log writes; the cluster's `flush_interval_milliseconds`
...
# settings clause that Datastore rejects with a clear parse error.
...
def _tag_with_log_comment(sql: str, log_comment: str) -> str:
    ...
    per Datastore string-literal rules (``''`` for embedded ``'``).
    """
    ...
    if matches:
        ...
        # is fine — Datastore doesn't care about settings ordering.
        last = matches[-1]
...
class MetabaseBackend(ExecutionBackend):
    ...
    def run(self, sql: str, *, timeout_s: int) -> ExecutionResult:
        ...
        # authoritative Datastore-side metrics (query_duration_ms,
        ...
        body, round_trip_ms = self._exec_via_insightscli(tagged_sql, timeout_s=timeout_s)
    ...
    def _exec_via_insightscli(self, sql: str, *, timeout_s: int) -> tuple[dict, float]:
        """Invoke ``insightscli metabase:query`` with ``sql`` on stdin.
        ...
        if result.returncode != 0:
            ...
            # `insightscli metabase:query` exits non-zero with a `Query failed: …`
    ...
    def _fetch_query_log_metrics(self, run_id: str) -> tuple[float, int, int, str] | None:
        ...
        """
        sql = _query_log.build_lookup_sql(run_id, table_expr="clusterAllReplicas(insights, system, query_log)")
        if sql is None:
            return None
        for attempt in range(_QUERY_LOG_POLL_ATTEMPTS):
            ...
            try:
                body, _ = self._exec_via_insightscli(sql, timeout_s=15)
            except BackendError:
                return None
            if body.get("status") == "failed" or body.get("error"):
                return None
