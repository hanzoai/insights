...
describe('PushNotificationService', () => {
    ...
    describe('executeSendPushNotification', () => {
        ...
        it('stamps workflow, action and invocation ids into the data payload so opens can be attributed', async () => {
            ...
            // Nested under the single `insights` key, JSON-encoded: that is the only entry the SDKs read,
            // and FCM's `data` map only accepts string values. Sibling keys would be silently ignored.
            expect(parseJSON(body.message.data.insights)).toEqual({
        ...
        it('does not let a custom data key shadow the correlation payload', async () => {
            ...
            ;(invocation.queueParameters as any).payload.data = { insights: 'not-the-real-workflow' }
            mockTrackedFetch.mockResolvedValue({
                fetchError: null,
                fetchResponse: {
                    status: 200,
            ...
            const body = parseJSON(mockTrackedFetch.mock.calls[0][0].fetchParams.body)
            expect(parseJSON(body.message.data.insights).workflow_id).toBe(invocation.functionId)
        })
        ...
        it('reschedules a transient failure and re-runs the send on the retry (round trip)', async () => {
            ...
            // The P1 guard: queueParameters must be restored, or the retry dequeue resumes the script VM
        ...
        it('prunes an unregistered FCM token and skips the channel without erroring', async () => {
            ...
            expect(result.error).toBeUndefined()
            expect(result.capturedInsightsEvents).toContainEqual(
                expect.objectContaining({
                    event: '$set',
    ...
    describe('APNS path', () => {
        ...
        it('mirrors APNS provider token reads and writes to Valkey', async () => {
            ...
            expect(mirrorSet).toHaveBeenCalledWith(
                expect.stringContaining('@hanzo/apns-provider-jwt/'),
                expect.any(String),
                'EX',
                2700
            )
    ...
    describe('multiple channels', () => {
        ...
        it('delivers to every channel in the list, not just the first', async () => {
            ...
            // The bug: a per-channel loop in script only ran once, so only the first channel delivered.
