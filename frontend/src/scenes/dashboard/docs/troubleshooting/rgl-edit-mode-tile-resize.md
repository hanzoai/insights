...
The white/orange resize preview is painted by RGL on `.react-resizable-handle`. Insights also renders decorative `DashboardResizeHandles` (`.handle`) on the grid item. If `.handle` is not anchored to the full grid cell, its SVG overlays sit above the preview and look like content covering the resize ghost.
