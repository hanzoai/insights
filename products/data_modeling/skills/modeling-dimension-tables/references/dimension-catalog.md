...
| **Currency**           | `currency` (ISO-4217)                   | **Built in** — use `convertCurrency()`; no table needed on Insights.            | Only build a `dim_currency_rate` in dbt (no equivalent there) or for a non-default provider. |
...
- **Upload / seed** — small, hand-maintained lookups (country→region, plan→tier). On Insights: upload a CSV
