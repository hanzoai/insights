...
from posthog.insightsql_queries.insights.trends.breakdown import BREAKDOWN_NULL_STRING_LABEL, BREAKDOWN_OTHER_STRING_LABEL
...
class TestTrendsPersons(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_trends_breakdown_insightsql_persons(self):
        self._create_events()
        source_query = TrendsQuery(
            series=[EventsNode(event="$pageview")],
            dateRange=DateRange(date_from="-7d"),
            breakdownFilter=BreakdownFilter(breakdown="properties.some_property", breakdown_type=BreakdownType.INSIGHTSQL),
        )
    ...
    def trends_all_cohort_breakdown_persons(self, inCohortVia: str):
        ...
        source_query.modifiers = InsightsQLQueryModifiers(inCohortVia=inCohortVia)
    ...
    def test_trends_multiple_breakdowns_insightsql_persons(self):
        self._create_events()
        source_query = TrendsQuery(
            series=[EventsNode(event="$pageview")],
            dateRange=DateRange(date_from="-7d"),
            breakdownFilter=BreakdownFilter(
                breakdowns=[Breakdown(property="properties.some_property", type=BreakdownType.INSIGHTSQL)],
                breakdown_limit=1,
            ),
        )
