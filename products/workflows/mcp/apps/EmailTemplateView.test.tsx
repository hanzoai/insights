...
const baseTemplate: EmailTemplateData = {
    ...
    content: {
        templating: 'liquid',
        email: {
            subject: 'Welcome to Insights!',
            text: 'Hi there',
            html: '<html><body><h1>Hi {{ person.properties.name }}</h1></body></html>',
        },
    },
    _insightsUrl: 'https://app.hanzo.ai/project/1/workflows/library/templates/abc-123',
}
...
describe('EmailTemplateView', () => {
    ...
    it('renders the subject and the email html in a script-disabled sandboxed iframe', () => {
        ...
        expect(screen.getByText('Welcome to Insights!')).toBeTruthy()
    ...
    it('links the edit button to the _insightsUrl', () => {
        render(<EmailTemplateView template={baseTemplate} />)

        const link = screen.getByText(/open in editor/i).closest('a')
        expect(link?.getAttribute('href')).toBe(baseTemplate._insightsUrl)
    })

    it('omits the edit link when there is no _insightsUrl', () => {
        const { _insightsUrl, ...withoutUrl } = baseTemplate
    ...
    it('falls back to plain text when there is no html', () => {
        const noHtml: EmailTemplateData = {
            ...baseTemplate,
            content: { templating: 'liquid', email: { subject: 'Welcome to Insights!', text: 'Plain body only' } },
        }
