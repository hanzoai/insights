    ...
    try:
        async with session.client("kms") as kms:
            resp = await kms.create_key(
                Description="Insights batch exports test key",
...
async def external_aws_role_arn(
    ...
    This simulates the role shown to users of Insights, so that they can grant
...
def external_id(request, aorganization) -> str | None:
    try:
        ...
        return f"insights-{aorganization.id}"
