...
export class RecordingService {
    ...
    async listBlocks(sessionId: string, teamId: number): Promise<RecordingBlock[]> {
        if (!this.datastore) {
            throw new Error('Datastore client not initialized')
        }
    ...
    private async deletePostgresRecords(sessionIds: string[], teamId: number): Promise<void> {
        ...
        const results = await Promise.allSettled([
            ...
            this.postgres.query(
                PostgresUse.COMMON_WRITE,
                `DELETE FROM insights_exportedrecording WHERE team_id = $1 AND session_id = ANY($2)`,
                [teamId, sessionIds],
                'deleteExportedRecordings'
            ),
            this.postgres.query(
                PostgresUse.COMMON_WRITE,
                `DELETE FROM insights_comment WHERE team_id = $1 AND scope = 'recording' AND item_id = ANY($2)`,
                [teamId, sessionIds],
    ...
    private async logActivity(sessionIds: string[], teamId: number, deletedBy: string): Promise<void> {
        ...
        await this.postgres.query(
            PostgresUse.COMMON_WRITE,
            `INSERT INTO insights_activitylog (id, team_id, is_system, activity, item_id, scope, detail, created_at)
