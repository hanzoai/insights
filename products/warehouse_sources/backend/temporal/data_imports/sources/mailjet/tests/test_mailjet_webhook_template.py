...
AUTHORIZATION_HEADER = "Basic " + base64.b64encode(b"insights:s3cr3t").decode()
SCHEMA_MAPPING = {"messageevent": "schema_message_events"}


class TestMailjetWarehouseWebhookTemplate(BaseInsightsFunctionTemplateTest):
    template = template
    ...
    def test_every_registered_event_type_routes_to_the_message_event_table(self, event_type):
        # Every event type Insights subscribes to in settings.py must have a route in the template,
    ...
    def test_grouped_delivery_is_acknowledged_and_dropped(self):
        # Mailjet's Version 2 callbacks post a JSON array. Insights registers Version 1, but a
    ...
    @parameterized.expand(
        [
            ("wrong_credentials", "Basic " + base64.b64encode(b"insights:wrong").decode(), {}, 401),
