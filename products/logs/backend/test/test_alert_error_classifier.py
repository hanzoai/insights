...
from insights.errors import ExposedCHQueryError, InternalCHQueryError, QueryErrorCategory
...
class TestClassifyAlertError(TestCase):
    ...
    def test_user_error_from_exposed_insightsql_error_surfaces_its_message(self) -> None:
        exc = ExposedInsightsQLError("Alert filter has an invalid property")
        with _with_category(QueryErrorCategory.USER_ERROR):
            result = classify(exc)
    ...
    def test_user_error_from_internal_ch_error_falls_back_to_unknown(self) -> None:
        ...
        assert "Insights" in result.user_message
    ...
    def test_unknown_message_mentions_insights(self) -> None:
