...
INSIGHTS_DESKTOP_MOBILE_APP_CLIENT_ID_US = "a5TY7w9IjFYfes6dkPgZe6envclWw3bm2UD8ZTlm"
INSIGHTS_DESKTOP_MOBILE_APP_CLIENT_ID_EU = "1A7vO138Fh5sYmJislicN4F5HnttI6urmFttxPDU"
...
INSIGHTS_DESKTOP_OAUTH_CLIENT_IDS = frozenset(
    {
...
SandboxOAuthApplication = Literal["array", "insights_ai"]
...
INTERNAL_SCOPES: list[str] = [
    ...
    # gateway requires it on the internal products that share the Insights Desktop OAuth app
    # (background_agents, signals, slack_app, conversations) so a user's own OAuth token
    # can't route around the insights_code free-tier model gate through those.
    "internal_run:read",
]
...
# Derived from insights.scopes so the token issued to a sandboxed agent cannot
...
InsightsMcpScopes = McpScopePreset | list[str]
...
def has_write_scopes(scopes: InsightsMcpScopes) -> bool:
    if isinstance(scopes, str):
...
def get_insights_ai_app() -> OAuthApplication:
    region = get_instance_region()
    client_id = _get_client_id_for_region(
        region=region,
...
def get_sandbox_oauth_app(application: SandboxOAuthApplication = "array") -> OAuthApplication:
    if application == "insights_ai":
        return get_insights_ai_app()
...
def get_wizard_app() -> OAuthApplication:
    return _get_oauth_app_for_client_id(
        settings.WIZARD_CLOUD_RUN_OAUTH_CLIENT_ID, "Insights Wizard", get_instance_region()
    )
