...
from query_performance_ai.orchestrator import slow_queries
...
def test_build_sql_with_available_dictionaries_adds_has_all_filter() -> None:
    sql = slow_queries.build_sql(
        ...
        lookback_hours=24,
        limit=1,
        available_dictionaries=["insights.web_pre_aggregated_teams_dict"],
    )
    assert "used_dictionaries" in sql
    assert "'insights.web_pre_aggregated_teams_dict'" in sql
...
def test_ch_string_array_handles_apostrophes_and_braces() -> None:
    ...
    # Single quotes inside the value get doubled per Datastore string-literal rules.
...
def test_parse_metabase_response_typed_records() -> None:
    ...
    assert rows[0].datastore_query == "SELECT count() FROM events"
    assert rows[0].insightsql_query == '{"kind":"InsightsQLQuery"}'
    assert rows[1].read_bytes == 12345
    assert rows[1].datastore_query == "SELECT 2"


def test_fetch_slow_queries_propagates_insightscli_failure() -> None:
    completed: Any = subprocess.CompletedProcess(args=["insightscli"], returncode=1, stdout="", stderr="boom")
    with mock.patch.object(subprocess, "run", return_value=completed):
        with pytest.raises(RuntimeError, match="insightscli metabase:query failed"):
...
def test_fetch_slow_queries_passes_args_via_stdin() -> None:
    ...
    completed: Any = subprocess.CompletedProcess(args=["insightscli"], returncode=0, stdout=payload, stderr="")
    ...
    assert args[0][:5] == ["insightscli", "metabase:query", "--region", "eu", "--database-id"]
    assert "99" in args[0]
