...
from insights.schema import FilterLogicalOperator, RecordingsQuery
...
def test_estimate_samples_only_positive_events_subqueries(
    ...
    sql = prepare_and_print_ast(built, InsightsQLContext(team_id=scanner.team.pk, enable_select_queries=True), "datastore")[
        0
    ]
...
def test_estimate_corrects_count_only_when_sampling_is_sound(
    ...
    with patch(
        "products.replay_vision.backend.queries.scanner_volume_estimate.execute_insightsql_query",
...
class TestRefreshScannerEstimate:
    ...
    def test_raises_and_leaves_fields_untouched_when_the_estimate_query_errors(self) -> None:
        scanner = _make_scanner()
        with patch(_ESTIMATE_QUERY, side_effect=RuntimeError("datastore down")):
