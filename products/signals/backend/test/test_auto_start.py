...
@pytest.mark.parametrize(
    ...
    [
        ...
        # producing "insights-self-driving/-<hex>".
        ("🎉🎉", "implementation"),
...
def test_generate_self_driving_head_branch_is_readable_and_valid(title, expected_slug):
    ...
    assert re.fullmatch(rf"insights-self-driving/{re.escape(expected_slug)}-[0-9a-f]{{6}}", branch)
...
def test_create_implementation_task_if_absent_is_idempotent(organization, team):
    ...
    assert call_kwargs["self_driving_head_branch"].startswith("insights-self-driving/")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("signals-scout-health-checks", None, "insights_health_check"),
        # Other scouts and pipeline reports stay billable.
        ("signals-scout-general", None, None),
        (None, None, None),
        # A caller that knows its Insights-system origin can declare the exemption explicitly.
        (None, "insights_onboarding", "insights_onboarding"),
    ],
)
