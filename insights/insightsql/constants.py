...
MAX_SELECT_POSTFN_AI_LIMIT = 500  # 500 rows
# Default limit for Insights AI queries
DEFAULT_POSTFN_AI_RETURNED_ROWS = 100
...
# extract the array via JSONExtract(..., 'Array(String)'). See insights/insightsql/property.py and the
...
type InsightsQLDialect = Literal["insightsql", "datastore", "postgres", "duckdb", "mysql", "snowflake", "redshift"]

# All dialects that compile to an external SQL database queried directly (as opposed to
# Datastore / InsightsQL). MySQL shares the standard-SQL keyword surface (CURRENT_DATE & co.)
...
SQL_TARGET_DIALECTS: frozenset[InsightsQLDialect] = frozenset({"postgres", "duckdb", "mysql", "redshift"})

type InsightsQLParserBackend = Literal["cpp-json", "rust-json", "rust-py"]
...
def get_max_limit_for_context(limit_context: LimitContext) -> int:
    ...
    elif limit_context == LimitContext.POSTFN_AI:
        return MAX_SELECT_POSTFN_AI_LIMIT  # 500
    else:
...
def get_default_limit_for_context(limit_context: LimitContext) -> int:
    ...
    if limit_context == LimitContext.EXPORT:
        ...
        return DEFAULT_POSTFN_AI_RETURNED_ROWS  # 100
    elif limit_context == LimitContext.HEATMAPS:
        return MAX_SELECT_HEATMAPS_LIMIT  # 1M
    elif limit_context == LimitContext.COHORT_CALCULATION:
        return MAX_SELECT_COHORT_CALCULATION_LIMIT  # 1b
...
class InsightsQLQuerySettings(BaseModel):
...
# Settings applied on top of all InsightsQL queries.
class InsightsQLGlobalSettings(InsightsQLQuerySettings):
    model_config = ConfigDict(extra="forbid")
    ...
    # https://github.com/Datastore/Datastore/issues/64487
    optimize_min_equality_disjunction_chain_length: Optional[int] = 4294967295
    # A bugfix workaround for a distributed-planner crash under the new analyzer. Datastore rewrites
    ...
    # https://github.com/Datastore/Datastore/issues/82941
...
def get_default_insightsql_global_settings(
    team_id: int | None = None,
    base: InsightsQLGlobalSettings | None = None,
) -> InsightsQLGlobalSettings:
    return base.model_copy(deep=True) if base is not None else InsightsQLGlobalSettings()
