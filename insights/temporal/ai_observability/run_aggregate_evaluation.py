...
from insights.temporal.ai_observability.evaluation_llm_judge import LLM_JUDGE_RETRY_POLICY
...
from insights.temporal.ai_observability.metrics import increment_errors, increment_settle_poll
...
def check_trace_settled_activity(inputs: CheckTraceSettledInputs) -> str:
    ...
    if last_seen is None:
        ...
        # Datastore): keep polling — the max-age cap is the backstop. Settling on NULL
...
def check_session_settled_activity(inputs: CheckSessionSettledInputs) -> str:
    ...
    team = Team.objects.get(id=inputs.team_id)
    # Tagged like every other AI-observability query so it routes to the LLM_ANALYTICS Datastore
...
# a cadence derived from the quiet period alone would schedule ~60k Datastore aggregates for one
...
class RunAggregateEvaluationWorkflow(InsightsWorkflow):
    @staticmethod
