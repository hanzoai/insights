...
import { isDataVisualizationNode, isInsightsQLQuery, isInsightQueryNode } from '~/queries/utils'
...
export const NOTEBOOK_NODE_TYPE_TO_MARKDOWN_TAG: Partial<Record<NotebookNodeType, string>> = {
    ...
    [NotebookNodeType.InsightsQLSQL]: 'InsightsQLSQL',
...
/** Maps a dropped Insights resource URL to its markdown component block, or null when the URL isn't
...
export function convertDroppedInsightsUrlToMarkdownNode(url: string): NotebookBlockNode | null {
    let parsed: URL
    try {
...
function getNotebookArtifactVisualizationQuery(block: VisualizationBlock): NotebookPropValue | null {
    ...
    const query: QuerySchemaRoot | DataVisualizationNode | InsightVizNode = isInsightsQLQuery(source)
...
function serializeLegacyQueryNode(node: JSONContent): string {
    ...
    if (isNotebookObjectProp(query) && query.kind === NodeKind.InsightsQLQuery) {
...
// serialize as `query={{...}}`. Only `{`/`[`-prefixed strings are touched, so plain text and InsightsQL
