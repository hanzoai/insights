...
1. Add the enum value, InsightsFunction template ID, and required fields in `products/alerts/backend/destination_configs.py`.
2. Extend validation and `build_alert_destination_config(...)` without changing existing payloads.
3. Add or update the transport template under `insights/cdp/templates/<destination>/template_<destination>.py` and its adjacent tests.
4. Add alert-specific template compatibility in `frontend/src/scenes/insights-functions/sub-templates/sub-templates.ts`.
...
- Do not describe producer acknowledgement as final destination delivery. InsightsFunction execution and external destination delivery happen downstream.
...
- Keep trigger/destination compatibility derived from InsightsFunction sub-templates.
- Add reusable UI options only when their backend input contract is shared.
- Keep product-only fields in the product UI and pass the resulting InsightsFunction inputs into the wizard contract.
...
- Logs tests for lifecycle, scheduling, InsightsFunction delivery, and rollback changes.
...
- A wizard option has no shared InsightsFunction input contract.
