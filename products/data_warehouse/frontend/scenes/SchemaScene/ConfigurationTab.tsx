        ...
        case 'sync-method':
            return (
                <div className="flex flex-col gap-6">
                    <SyncMethodSection sourceId={sourceId} schema={schema} />
...
function DetailsSection({
    ...
    const syncedTableName = schema.table?.insightsql_name ?? schema.table?.name
    ...
    return (
        <div>
                ...
                <div className="flex items-center justify-between">
                    ...
                    {schema.status ? (
                        <Tooltip title={schema.latest_error ?? undefined} interactive={!!schema.latest_error}>
                            <Tag type={StatusTagSetting[schema.status] || 'default'}>{schema.status}</Tag>
                        </Tooltip>
                    ) : (
            ...
            <div className="mt-4 flex gap-2 flex-wrap">
                ...
                {syncHistoryUrl && (
                    <Button type="secondary" to={syncHistoryUrl}>
                        View sync history
                    </Button>
                )}
            </div>
...
function SyncMethodSection({ sourceId, schema }: { sourceId: string; schema: ExternalDataSourceSchema }): JSX.Element {
    ...
    const persistSyncMethod = async (
        ...
        const applyUpdate = async (): Promise<void> => {
            ...
            try {
                ...
                toast.success('Sync method saved')
                loadSchema()
            } catch (e: any) {
                toast.error(e?.message || "Can't save sync method at this time")
            } finally {
                setSaving(false)
            }
        }
...
function ApiVersionSection({
    ...
    const persist = async (resyncAfter: boolean): Promise<void> => {
        ...
        try {
            ...
            if (resyncAfter) {
                resyncSchema(schema)
                toast.success('API version saved — full resync queued')
            } else {
                toast.success('API version saved')
            }
            loadSchema()
        } catch (e: any) {
            toast.error(e?.detail || e?.message || "Can't save API version at this time")
        } finally {
            setSaving(false)
        }
    }
...
function ColumnsAndRowFiltersSection({
    ...
    const { disabledReason: editorDisabledReason } = useSchemaEditorAccess(schema)
        ...
        if (resyncAfter) {
            ...
            void api.externalDataSchemas
                ...
                .then(() => {
                    updateSchema(next)
                    resyncSchema(next)
                    toast.success('Saved — full resync queued')
                })
                .catch((e: any) => {
                    toast.error(e?.message || "Can't save at this time")
                })
            return
        }
        updateSchema(next)
...
function ScheduleSection({
    ...
    const frequencyOptions: SelectOption<DataWarehouseSyncInterval>[] = allowedSyncFrequencies().map((value) => ({
        value,
    ...
    const handleSave = async (): Promise<void> => {
        ...
        try {
            ...
            toast.error(e?.message || "Can't save schedule at this time")
        } finally {
            setSaving(false)
        }
    }
        ...
        <div>
            ...
            <div className="border rounded p-4 bg-surface-primary flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-muted max-w-md">
                        How often Insights pulls new data from the source. Shorter intervals mean fresher data but more
            ...
            <div className="flex items-start justify-between gap-4">
                ...
                {currentTeam?.timezone !== 'UTC' && currentTeam?.timezone !== 'GMT' && (
                    <div className="flex items-center gap-1 text-xs shrink-0">
                        <span>UTC</span>
                        <Switch size="xsmall" checked={isProjectTime} onChange={setIsProjectTime} />
...
function DescriptionSourceTag({ source }: { source?: string }): JSX.Element | null {
    ...
    return (
        <Tag type={source === 'user_edited' ? 'success' : 'muted'} size="small">
...
function DescriptionsSectionContent({
    ...
    return (
        <div>
            <SectionHeader
                title="Descriptions"
                description="Describe what this table and its columns mean. These descriptions help Insights AI write correct queries against your data. Descriptions are generated automatically (from the source's documentation or AI) and anything you edit here is preserved."
            />
            <div className="border rounded p-4 bg-surface-primary">
                <DescriptionRow
                    columnName=""
