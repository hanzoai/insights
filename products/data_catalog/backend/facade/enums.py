...
# query definition). Not a insights.schema query kind — the query runner never executes it; an agent
...
# kind to its insights.schema model; execution wraps node kinds in a single-series TrendsQuery (they
# have no query runner of their own); the API picks query throttles from the same split.
INSIGHTSQL_DEFINITION_KIND = "InsightsQLQuery"
