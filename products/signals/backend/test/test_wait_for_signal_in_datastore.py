...
from products.signals.backend.temporal.signal_queries import (
    WaitForDatastoreInput,
    WaitForDatastoreMode,
    WaitForDatastoreSignal,
...
def _signals(count: int, *, include_inserted_at: bool = True) -> list[WaitForDatastoreSignal]:
    now = timezone.now()
    return [
        WaitForDatastoreSignal(
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(WaitForDatastoreMode.OPTIMISTIC, 0, id="optimistic_trusts_store_and_skips_datastore"),
        pytest.param(WaitForDatastoreMode.CH_CONFIRMED, 1, id="ch_confirmed_runs_single_confirm"),
    ],
)
async def test_store_confirmation_short_circuits(mode, expected_ch_queries):
    ...
    with (
        ...
        patch(f"{MODULE}.execute_insightsql_query_with_retry", AsyncMock(return_value=_ch_result(2))) as ch,
    ...
    # touching Datastore at all, ch_confirmed with exactly one confirm query — and
...
async def test_signal_without_inserted_at_accepts_any_store_hit():
    ...
    with (
        ...
        patch(f"{MODULE}.execute_insightsql_query_with_retry", AsyncMock(return_value=_ch_result(1))) as ch,
        patch(f"{MODULE}.asyncio.sleep", AsyncMock()) as sleep,
    ):
        await _run(signals, mode=WaitForDatastoreMode.OPTIMISTIC)
...
async def test_unconfirmed_store_defers_datastore_until_grace_period_elapses(store_behavior):
    signals = _signals(1)
    with (
        patch(f"{MODULE}.Team", _team_model_mock()),
    ...
    # The store never confirms, so Datastore must not be touched during the 5-minute
    ...
    # Datastore query; an early Datastore poll would return sooner and shrink the
    # store count, and a store-gated Datastore would never run and time out instead.
...
async def test_gives_up_after_max_wait_and_records_timeout():
    ...
    with (
        ...
        patch(f"{MODULE}.execute_insightsql_query_with_retry", AsyncMock(return_value=_ch_result(0))) as ch,
        patch(f"{MODULE}.asyncio.sleep", AsyncMock()),
        patch(f"{MODULE}.metrics.increment_ch_wait_timeout") as timeout_metric,
    ):
        await _run(signals, max_wait_time_seconds=30)

    # A wait shorter than the grace period still checks Datastore once, on the final
