...
class TestRunHogEvalTestTool(BaseTest):
    ...
    def test_compilation_error(self):
        tool = self._make_tool()
        result, artifact = _run_tool(tool, source="this is not valid script {{{{", sample_count=1)
    ...
    def test_result_formatting_includes_previews(self, mock_query):
        ...
        mock_response.results = [
            _make_event(
                ai_input=[{"role": "user", "content": "What is Insights?"}],
                ai_output_choices=[{"message": {"role": "assistant", "content": "Insights is analytics."}}],
            )
        ]
        mock_query.return_value = mock_response
    ...
    def test_heavy_column_remerge_from_ai_events(self, mock_query):
        ...
        `properties` so the Script body can read `properties.$ai_input`. This test
        ...
        tool = self._make_tool()
        # Script body reads `properties.$ai_input` — this must be populated post-merge.
