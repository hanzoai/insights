...
from insights.temporal.ducklake import ducklake_register_data_imports_workflow as registration_module
from insights.temporal.ducklake.ducklake_register_data_imports_workflow import (
...
async def test_prepare_registration_pins_the_import_jobs_prepared_generation(ateam):
    ...
    assert metadata.landing_uri == (
        f"s3://ducklake-dev/insights_data_imports_team_{ateam.id}/postgres_customers/_imports/{schema.id}/job-123"
    )
...
def test_copy_activity_uses_s3_copy_and_local_duckgres_postgres_connection(monkeypatch):
    ...
    assert s3.copies == [
        (
            ...
            "ducklake/insights_data_imports_team_1/postgres_customers/_imports/schema/job/"
...
def _activity_inputs() -> DuckLakeRegisterDataImportsActivityInputs:
    return DuckLakeRegisterDataImportsActivityInputs(
        ...
        metadata=DuckLakeRegisterDataImportsMetadata(
            ...
            landing_uri="s3://ducklake/insights_data_imports_team_1/postgres_customers/_imports/schema/job",
