...
def build_alert_where_expr(
    *,
    ...
    placeholder that resolves to `true` when no InsightsQLFilters are passed to
...
class AlertCheckQuery:
    """Lightweight count query against the logs Datastore cluster for alert checks.
    ...
    See the [Logs Alerting RFC](https://github.com/Insights/requests-for-comments-internal/blob/main/engineering/2026-03-03-logs-alerting.md)
    """
    ...
    def execute_bucketed(self, interval_minutes: int, *, limit: int = 10_000) -> list[BucketedCount]:
        ...
        # Wrapping in toStartOfMinute() lets Datastore match the projection's
    ...
    def _run_query(self, query: ast.SelectQuery | ast.SelectSetQuery) -> InsightsQLQueryResponse:
...
class BatchedAlertCheckQuery:
    ...
    def _run_query(self, query: ast.SelectQuery) -> InsightsQLQueryResponse:
        return execute_insightsql_query(
            query_type="alert_check",
            query=query,
            team=self.team,
