...
function sliceLabelLines(): string[][] {
    return Array.from(document.querySelectorAll('[data-attr="script-chart-pie-slice-label"]')).map((el) =>
...
describe('SqlPieGraph', () => {
    ...
    it('colors breakdown slices from per-breakdown resultCustomizations', () => {
        ...
        const swatchColors = Array.from(document.querySelectorAll<HTMLElement>('.ColorGlyph')).map(
            (el) => el.style.color
        )
