    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                lambda self, token: (
                    f"/v1/projects/{self.other_team.id}/session_recordings/{self.session_id}?sharing_access_token={token}"
                ),
            ),
        ]
    )
...
@pytest.mark.ee
class TestSessionRecordingsSharingAccessControl(APIBaseTest, DatastoreTestMixin, QueryMatchingTest):
    def setUp(self):
        super().setUp()
        ...
        self.member_user = User.objects.create_and_join(self.organization, "member@hanzo.ai", "testtest")
    ...
    def test_enable_sharing_requires_editor_access(self, access_level: str, expected_status: int) -> None:
        ...
        response = self.client.patch(
            f"/v1/projects/{self.team.id}/session_recordings/{self.session_id}/sharing",
            {"enabled": True},
        )
    ...
    def test_sharing_response_exposes_user_access_level(self) -> None:
        ...
        response = self.client.get(f"/v1/projects/{self.team.id}/session_recordings/{self.session_id}/sharing")
